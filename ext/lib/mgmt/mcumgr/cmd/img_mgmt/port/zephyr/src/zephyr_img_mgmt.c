...
zephyr_img_mgmt_flash_area_id(int slot)
{
    ...
    switch (slot) {
    case 0:
        fa_id = FLASH_AREA_IMAGE_0_ID;
        break;

    case 1:
        fa_id = FLASH_AREA_IMAGE_1_ID;
        break;
...
img_mgmt_impl_erase_slot(void)
{
    ...
    rc = zephyr_img_mgmt_flash_check_empty(FLASH_AREA_IMAGE_1_ID,
                                           &empty);
    if (rc != 0) {
        return MGMT_ERR_EUNKNOWN;
    }
    ...
    if (!empty) {
        rc = boot_erase_img_bank(FLASH_AREA_IMAGE_1_ID);
        if (rc != 0) {
            return MGMT_ERR_EUNKNOWN;
        }
    }
