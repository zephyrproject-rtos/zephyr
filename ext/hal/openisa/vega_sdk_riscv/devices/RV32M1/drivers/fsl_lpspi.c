...
void LPSPI_SlaveInit(LPSPI_Type *base, const lpspi_slave_config_t *slaveConfig)
{
    ...
    base->TCR = LPSPI_TCR_CPOL(slaveConfig->cpol) | LPSPI_TCR_CPHA(slaveConfig->cpha) |
                LPSPI_TCR_LSBF(slaveConfig->direction) | LPSPI_TCR_FRAMESZ(slaveConfig->bitsPerFrame - 1) | LPSPI_TCR_PCS(slaveConfig->whichPcs);
...
void DMA_SPI0_INT_IRQHandler(void)
{
    assert(s_lpspiHandle[0]);
...
void DMA_SPI1_INT_IRQHandler(void)
{
    assert(s_lpspiHandle[1]);
...
void DMA_SPI2_INT_IRQHandler(void)
{
    assert(s_lpspiHandle[2]);
...
void DMA_SPI3_INT_IRQHandler(void)
{
    assert(s_lpspiHandle[3]);
