...
#define QMSPI_MAX_FREQ_KHZ          ((QMSPI_INPUT_CLOCK_FREQ_HZ) / 1000ul)
#define QMSPI_MIN_FREQ_KHZ          (QMSPI_MAX_FREQ_KHZ / 256ul)

#define QMSPI_GIRQ_NUM              18u
#define QMSPI_GIRQ_POS              1u
#define QMSPI_GIRQ_OFS              (((QMSPI0_GIRQ_NUM) - 8) * 20u)
#define QMSPI_GIRQ_BASE_ADDR        0x4000E0C8ul
/* Sleep Enable 4 bit 8 */
#define QMSPI_PCR_SLP_EN_ADDR       0x40080140ul
#define QMSPI_PCR_SLP_EN_BITPOS     8u

#define QMSPI_GIRQ_SRC_ADDR        (QMSPI_GIRQ_BASE_ADDR)
#define QMSPI_GIRQ_ENSET_ADDR      (QMSPI_GIRQ_BASE_ADDR + 0x04ul)
#define QMSPI_GIRQ_RESULT_ADDR     (QMSPI_GIRQ_BASE_ADDR + 0x08ul)
#define QMSPI_GIRQ_ENCLR_ADDR      (QMSPI_GIRQ_BASE_ADDR + 0x0Cul)
 ...
 */
#define QMSPI_TX_DMA_DEVDIR     ((QMSPI_TX_DMA_REQ_ID << 1) + 0x01ul)
#define QMSPI_RX_DMA_DEVDIR     ((QMSPI_RX_DMA_REQ_ID << 1) + 0x00ul)
...
#define QMSPI_TX_FIFO_LEN           8ul
#define QMSPI_RX_FIFO_LEN           8ul

#define QMSPI_M_ACT_SRST_OFS        0ul
#define QMSPI_M_SPI_MODE_OFS        1ul
#define QMSPI_M_CLK_DIV_OFS         2ul
#define QMSPI_CTRL_OFS              4ul
#define QMSPI_EXE_OFS               8ul
#define QMSPI_IF_CTRL_OFS           0x0Cul
#define QMSPI_STS_OFS               0x10ul
#define QMSPI_BUF_CNT_STS_OFS       0x14ul
#define QMSPI_IEN_OFS               0x18ul
#define QMSPI_BUF_CNT_TRIG_OFS      0x1Cul
#define QMSPI_TX_FIFO_OFS           0x20ul
#define QMSPI_RX_FIFO_OFS           0x24ul
#define QMSPI_CSTM_OFS              0x28ul
/* 0 <= n < QMSPI_MAX_DESCR */
#define QMSPI_DESCR_OFS(n)          (0x30ul + ((uint32_t)(n) << 2))
#define QMSPI_DESC0_OFS             0x30ul
...
#define QMSPI_C_XFR_NUNITS_MASK0 0x7FFFul
 ...
 * Register Access
 */
#define MEC_QMSPI_MODE()        REG32(QMSPI_MODE_ADDR)
#define MEC_QMSPI_MODE_ACTRST() REG8(QMSPI_MODE_ADDR)
#define MEC_QMSPI_MODE_SIG()    REG8(QMSPI_MODE_ADDR + 1ul)
#define MEC_QMSPI_MODE_FDIV()   REG16(QMSPI_MODE_ADDR + 2ul)

/* Control register */
#define MEC_QMSPI_CTRL()        REG32(QMSPI_CTRL_ADDR)

/* Execute register */
#define MEC_QMSPI_EXE()         REG8(QMSPI_EXE_ADDR)

/* Interface Control register */
#define MEC_QMSPI_IFC()         REG8(QMSPI_IFC_ADDR)

/* Status register */
#define MEC_QMSPI_STS()         REG32(QMSPI_STS_ADDR)

/* Buffer Count Status register (read-only) */
#define MEC_QMSPI_BCNT_STS()    REG32(QMSPI_BUFCNT_STS_ADDR)
/* b[15:0] = TX buffer count */
#define MEC_QMSPI_BCNT_TX_STS() REG16(QMSPI_BUFCNT_STS_ADDR)
/* b[31:15] = RX buffer count */
#define MEC_QMSPI_BCNT_RX_STS() REG16(QMSPI_BUFCNT_STS_ADDR + 2ul)

/* Interrupt Enable register */
#define MEC_QMSPI_IEN() REG32(QMSPI_IEN_ADDR)

/* TX FIFO write-only */
#define MEC_QMSPI_TXB_32() REG32(QMSPI_TXB_ADDR)
#define MEC_QMSPI_TXB_16() REG16(QMSPI_TXB_ADDR)
#define MEC_QMSPI_TXB_8()  REG8(QMSPI_TXB_ADDR)

/* RX FIFO read-only */
#define MEC_QMSPI_RXB_32() REG32(QMSPI_RXB_ADDR)
#define MEC_QMSPI_RXB_16() REG16(QMSPI_RXB_ADDR)
#define MEC_QMSPI_RXB_8()  REG8(QMSPI_RXB_ADDR)

/*
 * Descriptor registers
 * 0 <= n < QMSPI_MAX_DESCR
 */
#define MEC_QMSPI_DESCR(id) REG32(QMSPI_DESCR_ADDR(id))
#define MEC_QMSPI_DESCR0() REG32(QMSPI_DESCR_ADDR(0))
#define MEC_QMSPI_DESCR1() REG32(QMSPI_DESCR_ADDR(1))
#define MEC_QMSPI_DESCR2() REG32(QMSPI_DESCR_ADDR(2))
#define MEC_QMSPI_DESCR3() REG32(QMSPI_DESCR_ADDR(3))
#define MEC_QMSPI_DESCR4() REG32(QMSPI_DESCR_ADDR(4))

#define MEC_QMSPI_DESCR_NUNITS(id, nu) \
    MEC_QMSPI_DESCR(id) = ((MEC_QMSPI_DESCR(id) & ~(QMSPI_C_XFR_NUNITS_MASK)) +\
    ((nu & QMSPI_C_XFR_NUNITS_MASK0) << QMSPI_C_XFR_NUNITS_POS))
...
typedef struct {
	...
	uint8_t RSVD1[8];
	QMSPI_DESCR DESCR[QMSPI_MAX_DESCR];	/*!< (@ 0x00000030) QMSPI Descriptors 0-4 */
} MEC_QMSPI;
