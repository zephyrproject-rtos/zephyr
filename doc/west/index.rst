...
West was added to the Zephyr project in order to fulfill two fundamental
requirements:
...
identified in order to avoid the common pitfalls of tools of this kind.
...
In this context, there are a few reasons to be able to work with multiple Git
...
:ref:`application`.
While CMake is a very powerful sytem and language, its two-stage nature
...
In order to unify and streamline the developer's experience when working with
Zephyr, west was introduced. It is very important to note that west does not
...
first in order to use the rest of functionality included in ``west``, since
...
any commands different from ``init`` to the cloned west installation.
...
A west installation is the result of running ``west init`` on a folder. As
