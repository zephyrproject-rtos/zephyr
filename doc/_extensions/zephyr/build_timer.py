...
This makes the benefit of incremental and partial (``html-minimal``) builds
...
# timings of different profiles (e.g. html vs html-minimal) from clobbering each
# other.
PROFILE_TAGS = (
    "skip_doxygen",
    "skip_kconfig",
...
def _cache_path(app: Sphinx) -> Path | None:
    """Path to the timing cache, confined to the (trusted) doctree directory.

    The directory comes from configuration (the Sphinx ``-d`` option), so the
    target is canonicalized and validated to stay within it before any file
    I/O, guarding the read/write against path traversal.
    """
    base = os.path.realpath(app.doctreedir)
    target = os.path.realpath(os.path.join(base, CACHE_FILENAME))
...
def _on_builder_inited(app: Sphinx) -> None:
    ...
    # Updated by env-before-read-docs; stays 0 for a build that reads nothing.
    app._build_timer_read_count = 0
...
def _on_build_finished(app: Sphinx, exception) -> None:
    ...
    read_count = getattr(app, "_build_timer_read_count", 0)
    ...
    # Persist the updated timings. The full-build reference only needs its
    # duration (it is always a full build by definition).
    entry["last"] = {"seconds": elapsed, "kind": kind}
    if is_full:
        entry["full"] = {"seconds": elapsed}
    cache[key] = entry
