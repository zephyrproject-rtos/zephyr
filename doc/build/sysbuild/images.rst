...
Images can be added to project in various ways, multiple ways can be used simultaneously, they can
be added in the following ways:
...
An example of how to create such an image:
...
more complex to create but is the preferred method for adding images.
...
reading the output of image configuration (Kconfig and devicetree) which can be used to allow an
...
These can safely be used in an application/board/SoC ``sysbuild.cmake`` file, as that file is
...
added to when adding a new type that any project should be able to select.
