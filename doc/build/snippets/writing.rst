...
Specific configuation for revisions of boards is also supported which will be applied after the
common files:
...
The above example will use :file:`first.overlay` for all revisions of the ``baz`` board, and will
also include :file:`extra_0_7_0.overlay` when building for revision ``0.7.0`` of the ``baz``
board (``baz@0.7.0``).
