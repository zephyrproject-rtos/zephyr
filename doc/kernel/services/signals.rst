...
event has occurred using a predefined integer value (see signal numbers).
...
send and receive signals (see `Signal API Implementation <signals_implementation>`).
...
created with an initially-empty :c:struct:`k_sig_set` representing it's set of
masked signals. If a signal is masked (i.e. part of the set), the
corresponding bit in the signal set is 1, otherwise it is 0. Thus an initially
empty :c:struct:`k_sig_set` is zero-valued. Signal sets should not be modified
directly, but indirectly via the functions provided in the signal API.
...
1. :c:macro:`K_SIG_BLOCK` - the signals in set provided are added to the mask
...
with the integer valued signal, :c:func:`k_sig_queue()` also accepts a
