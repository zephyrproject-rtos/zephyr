...
- **Flushing**: A thread can flush a pipe, which resets its internal state and
...
The pipe can be flushed by calling :c:func:`k_pipe_flush`. Flushing a pipe
resets its internal state and ends all pending operations with an error code.

The following example demonstrates flushing a pipe in response to a critical
error:
