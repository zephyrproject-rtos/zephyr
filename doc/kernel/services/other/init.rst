...
.. mermaid::
   ...
   :alt: Flowchart of the boot sequence. From z_cstart the system runs the
       ...
       hooks, the PRE_KERNEL level, the deprecated PRE_KERNEL_2 level, kernel
...
``EARLY``, ``PRE_KERNEL`` and the deprecated ``PRE_KERNEL_2`` run before
...
Each of those contiguous runs is called a *band* in the rest of this page:
the numeric band, the devicetree-ordered band and the anchored band. A band
is a region *within* a level, not a level of its own; the kernel walks a
level exactly once, and the bands are just the order the linker has already
placed its entries in. Which band an entry lands in follows from how it is
registered, and the boundaries between them are what the ordering rules below
are stated against.
...
.. code-block:: c
   ...
   #define SYS_ANCHOR_shell \
           SYS_ANCHOR_AFTER_IF(CONFIG_SETTINGS, SYS_ANCHOR_settings, shell)
...
automatic-ordering band of its level, its devicetree dependencies are satisfied
as well.
