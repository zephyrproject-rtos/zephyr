...
- ``ZASSERT_LEVEL_ON`` -- assertions are checked; on failure only the location is
...
If the condition is false and the module's level is at least ``ZASSERT_LEVEL_ON``, a fatal error is
raised. The message and its arguments are only compiled in and printed at
...
For an assertion with a fixed level, use :c:macro:`ZASSERT_ON` or
:c:macro:`ZASSERT_VERBOSE`. These forms select the level at the call site and
declare nothing in scope:
...
The ``ZASSERT_LEVEL_ON`` and ``ZASSERT_LEVEL_VERBOSE`` values from
:c:enum:`zassert_level` configure module assertion levels, while ``ZASSERT_ON()``
and ``ZASSERT_VERBOSE()`` perform fixed-level assertions.
...
.. note::
   ...
   - Using ``ZASSERT()`` with no module in scope is a compile error. Select a
     module first, or use the fixed-level ``ZASSERT_ON()`` or
     ``ZASSERT_VERBOSE()`` form.
