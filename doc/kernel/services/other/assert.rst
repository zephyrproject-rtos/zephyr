...
Every module resolves to one of four levels from :c:enum:`zassert_level`:
...
values from :c:enum:`zassert_level` configure module assertion levels, while
...
``Off`` / ``Terse`` / ``Normal`` / ``Verbose`` choice
...
level: :kconfig:option:`CONFIG_ASSERT_VERBOSE` (now ``y`` by default) maps to
``Verbose``, :kconfig:option:`CONFIG_ASSERT_NO_COND_INFO`,
:kconfig:option:`CONFIG_ASSERT_NO_MSG_INFO` and
:kconfig:option:`CONFIG_ASSERT_NO_FILE_INFO` map to ``Terse``, and a disabled or
