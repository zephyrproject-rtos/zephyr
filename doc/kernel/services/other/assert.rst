...
The choice defaults to ``Off`` unless the module opts into an on-by-default by giving
``CONFIG_ASSERT_MODULE_<module>_DEFAULTS_ON`` a ``default y`` (for example from a subsystem debug switch).
The level stays overridable from :file:`prj.conf`.

Example
-------

The :zephyr:code-sample:`assert` sample demonstrates enabling
assertions for a single file while global assertions are disabled.
A condensed version:
