...
:c:func:`sys_clock_hw_cycles_per_sec` (which on almost all platforms is a
runtime constant that evaluates to :kconfig:option:`CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC`
and is fixed for the lifetime of the system). On platforms where the active
system timer frequency can change at runtime, the kernel can be notified of
the new value using :kconfig:option:`CONFIG_SYSTEM_CLOCK_HW_CYCLES_PER_SEC_RUNTIME_UPDATE`;
in that case, application code should not assume that conversions derived from
:c:func:`sys_clock_hw_cycles_per_sec` are based on a single immutable frequency.
...
.. note::
  ...
  the system timer frequency as a single **global** value. It is therefore not
  compatible with per-CPU frequency scaling configurations where different CPUs
  could observe different system timer frequencies.
