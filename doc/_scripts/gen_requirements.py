...
# Mapping of StrictDoc requirement fields to requirement need options.
ITEM_ATTRIBUTES = [("STATUS", "status"), ("TYPE", "rtype"), ("COMPONENT", "component")]


def render_requirement_rst(out, req):
    """Render a requirement as a sphinx-needs ``req`` need. The UID is the need
    id (and cross-reference target); StrictDoc Parent relations become a
    ``trace`` link so the requirement hierarchy can be rendered."""
    uid = req_uid(req)
    if not uid:
        return
    title = clean(req.get("TITLE"))
    out.append(f".. req:: {title or uid}".rstrip())
    out.append(f"   :id: {uid}")
...
def write_rst(rst_out, documents):
    ...
    # Prune pages from earlier runs so removed/renamed documents do not linger
    # as orphaned (not-in-any-toctree) pages.
    for stale in rst_out.glob("*.rst"):
        stale.unlink()
