...
Builds on :mod:`gen_traceability_report` (the needs.json traceability model) and
...
together with the ``implements`` linkage (``@satisfies`` on the kernel API)
...
* ``unresolved``   -- implementation links exist but none map to a body
                      (macros)
* ``no-impl``      -- the requirement has no ``implements`` link
...
from gen_traceability_report import build_model, load_needs, srs_sortkey  # noqa: E402
...
class App:
    ...
    def __init__(self, args):
        ...
        self.model = build_model(load_needs(args.json))
...
const ADQ_HELP={
 ...
 'no-impl':'the requirement has no implements link (@satisfies)',
 'no-cov':'the verifying tests have no coverage data in this run'};
