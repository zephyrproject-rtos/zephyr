...
"""
Turn the documentation build's traceability JSON export into a
self-contained, interactive HTML traceability report.

The input is the ``needs.json`` export written by the docs build (sphinx-needs
``needs_build_json``, by default ``doc/_build/html/needs.json``). The output is a
single HTML file (no external assets, works offline) with:
...
Usage::
    ...
    python doc/_scripts/gen_traceability_report.py \\
        [--json doc/_build/html/needs.json] \\
...
# still classify as tests when the need carries no type.
TEST_RE = re.compile(r"^(?:\w+__)?test_")


def classify(need):
    """Kind of a need: prefer the sphinx-needs type, fall back to ID shape."""
    nid = need["id"]
    ntype = need.get("type") or ""
    if ntype == "req":
        return "syrs" if SYRS_RE.match(nid) else "srs" if SRS_RE.match(nid) else "other"
    if ntype == "design_need":
        return "design"
    if ntype == "test":
        # A "test" need whose ID is not test-shaped (e.g. a macro carrying
        # @verifies) is exactly what the non-test-verifier check must flag.
        return "test" if TEST_RE.match(nid) else "other"
    if ntype == "impl":
        return "other"
    if SRS_RE.match(nid):
        return "srs"
    if SYRS_RE.match(nid):
        return "syrs"
    if nid.startswith("DESIGN-"):
        return "design"
    if TEST_RE.match(nid):
        return "test"
    return "other"


def load_needs(path):
    """Load a sphinx-needs ``needs.json`` export and return the needs as a
    list of dicts, tolerating the versioned envelope."""
    data = json.loads(Path(path).read_text())
    versions = data.get("versions")
    if isinstance(versions, dict) and versions:
        version = data.get("current_version")
        vdata = versions.get(version) or next(iter(versions.values()))
        return list(vdata.get("needs", {}).values())
...
LINK_KEYS = ("validates", "implements", "fulfills", "trace",
             "validates_back", "implements_back", "fulfills_back", "trace_back")


def build_model(needs):
    """Reduce a sphinx-needs export into the compact model the report consumes.

    ``needs`` is the list of need dicts from :func:`load_needs`. Forward links
    (``validates``, ``implements``, ``fulfills``, ``trace``) live on the
    verifying/implementing/design needs; sphinx-needs adds the ``*_back``
    reverse lists on the requirements.
    """
    by_id = {n["id"]: n for n in needs}

    def links(n, key):
        return list(n.get(key) or [])

    # All ids referenced by any relationship, to detect dangling links.
    referenced = set()
    for n in needs:
        for key in LINK_KEYS:
            referenced.update(links(n, key))
    ...
    for n in needs:
        ...
        if kind == "srs":
            reqs[n["id"]] = {
                ...
                "rtype": n.get("rtype") or "",
                "tests": sorted(links(n, "validates_back")),
                "design": sorted(links(n, "fulfills_back")),
                "parents": sorted(links(n, "trace")),
                "document": n.get("docname") or "",
            }
        elif kind == "syrs":
            syrs[n["id"]] = {
                "id": n["id"],
                ...
                "component": n.get("component") or "(none)",
                "status": n.get("status") or "(none)",
                "children": sorted(links(n, "trace_back"), key=lambda u: srs_sortkey(u)),
            }
        elif kind == "design":
            design[n["id"]] = {
                "id": n["id"],
                "title": n.get("title") or "",
                "fulfills": sorted(links(n, "fulfills"), key=srs_sortkey),
                "document": n.get("docname") or "",
            }
        else:  # test or other (implementation symbols)
            tests[n["id"]] = {
                "id": n["id"],
                "caption": n.get("title") or "",
                "validates": sorted(links(n, "validates"), key=srs_sortkey),
                "implements": sorted(links(n, "implements"), key=srs_sortkey),
                "is_test": kind == "test",
                "document": n.get("docname") or "",
            }
...
def main(argv=None):
    ...
    p.add_argument("--json", default="doc/_build/html/needs.json",
                   help="Path to the sphinx-needs export (needs.json).")
    ...
    src = Path(args.json)
    if not src.is_file():
        p.error(f"needs.json not found: {src}\n"
                "Build the docs first (it is written to <build>/html/needs.json).")
    model = build_model(load_needs(src))
    print_summary(model)
