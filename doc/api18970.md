...
* **isr-ok** if the function can be safely called from an ISR even if it
...
name              | reschedule | sleep | no-wait | isr-ok | supervisor
----------------- | ---------- | ----- | ------- | ------ | ----------
`k_busy_wait`     | no         | -     | -       | -      | no
`k_sem_give`      | yes        | no    | -       | -      | no
`k_sleep`         | yes        | yes   | no      | no     | no
`k_sem_take`      | yes        | yes   | yes     | no     | no
`k_thread_create` | ?          | ?     | ?       | ?      | yes
...
called from interrupt context to the function can force an execution
path that will not cause the invoking thread to sleep.

This attribute is intended for **sleep** functions that may be
indirectly invoked from interrupt context with arguments that would
attempt to cause the invoking thread to sleep.

Functions that are **isr-ok** may be invoked from interrupt context, and
will return an error if they would have tried to sleep.
