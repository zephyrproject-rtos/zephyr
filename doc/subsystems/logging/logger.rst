...
increased. The log message contains information about which arguments contain
duplicated strings. Buffers are freed together with the log message. A mechanism
used to gather information about usage of :cpp:func:`log_strdup` requires that
explicit string arguments must be put into a :cpp:func:`log_str` call and
arguments starting with '*' (e.g. *\*data*) or '(' (e.g. (*a + b*)) must be put
into :c:macro:`LOG_WRAP`.

.. code-block:: c

   char local_str[] = "abc";
   static const char const_str[] = "abc";

   LOG_INF("logging transient string: %s", log_strdup(local_str));
   local_str[0] = '\0'; /* String can be modified, logger will use duplicate."

   /* log_str() required. */
   LOG_INF("logging explicit string: %s", log_str("explicit string"));

   /* log_str() not required.*/
   LOG_INF("logging constant string: %s", const_str);
