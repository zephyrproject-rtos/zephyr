...
pytest harness
   :zephyr_file:`tests/net/conformance/ttcn3_runner.py`, shared by every
   test. It decides whether the suite can run at all, takes the interface
...
:file:`conftest.py` is the same in every test directory and does one thing:
put the shared runner on ``sys.path`` and re-export the lock fixture.
...
.. warning::
   ...
   * ``host-interface`` in the :file:`boards/native_sim.overlay` of each test on
     ``zethL2``, which decides where the system under test appears
   * ``system.pt.interface`` in the configuration file of each suite on
     ``zethL2``, which decides where the suite listens
