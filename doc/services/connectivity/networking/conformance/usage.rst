...
* **The** ``zeth`` **interface**, or ``zethL2`` for a suite that works below
  the IP layer. See :ref:`ttcn3_interfaces`.
...
Used by the suites that work over sockets, where the tester is just another
host on the link:
...
Some suites have to be run as root. DHCP is defined on ports 67 and 68 and
...
For a suite whose test cases create parallel test components, start it through
the main controller instead:
