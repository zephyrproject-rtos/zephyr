...
Supplying entries when import is disabled makes ``CONNECT`` return
``-ENOTSUP`` without invoking the backend. When import is enabled, entries are
best-effort hints. A backend skips invalid, expired, or unsupported entries,
including entries whose supplicant address does not match
