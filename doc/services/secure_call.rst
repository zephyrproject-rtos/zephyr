...
.. note::
   ...
   The specific mechanisms named above — the SAU and MPC/PPC bus filters, the
   ``SG`` instruction and ``cmse_nonsecure_entry`` veneers, and the CMSE ``TT``
   instruction for pointer validation — are the ARMv8-M TrustZone-M realization.
   The design is meant to port to other architectures with equivalent primitives.
   ARC SecureShield, for example, uses the MPU for isolation, the ``sjli`` (Secure
   Jump and Link Indexed) secure jump table for entry, and an MPU probe for buffer
   validation. RISC-V has an emerging Lightweight Isolation effort aimed at the
   same lightweight-call use case, distinct from the system-level World Identifiers.
   To keep the generic layer architecture-neutral, pointer validation is delegated
   to the arch backend through :c:func:`arch_secure_call_buffer_accessible`.
