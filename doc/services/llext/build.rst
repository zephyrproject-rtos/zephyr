...
``LLEXT_EDK_TOOLCHAIN_NAME``
    The toolchain variant used in the Zephyr build.

Depending on the toolchain, the following variables may also be exported:

``LLEXT_EDK_TOOLCHAIN_TARGET``
    The target tuple for the compiler used in the Zephyr build.

``LLEXT_EDK_TOOLCHAIN_TRIPLE``
    The target triple for the compiler used in the Zephyr build.
