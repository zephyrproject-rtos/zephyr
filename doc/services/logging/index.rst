...
:kconfig:option:`CONFIG_LOG_BACKEND_IPC_FSC` and
:kconfig:option:`CONFIG_LOG_FRONTEND_IPC_FSC` are used in the remote domain to
...
In the root domain, :kconfig:option:`CONFIG_LOG_LINK_IPC` adds a link named ``cpu1``.
Messages received from the remote domain are stored in one of two ways:

* If :kconfig:option:`CONFIG_LOG_LINK_IPC_RX_HOLD` is enabled (default when the IPC
  ...
  :kconfig:option:`CONFIG_LOG_LINK_IPC_RX_HOLD_QUEUE_SIZE` entries. The queue should not
...
* Otherwise, messages are copied into a dedicated packet buffer of
  :kconfig:option:`CONFIG_LOG_LINK_IPC_BUFFER_SIZE` bytes.

In the remote domain, messages can be sent to the root domain either by a backend or by
a frontend:

* :kconfig:option:`CONFIG_LOG_BACKEND_IPC` sends messages when they are processed by
  ...
  buffer (:kconfig:option:`CONFIG_LOG_BACKEND_IPC_ZERO_COPY`).
...
(:kconfig:option:`CONFIG_LOG_BACKEND_IPC_FSC` and
:kconfig:option:`CONFIG_LOG_FRONTEND_IPC_FSC`, see `Cross-domain log message`_).
