...
* If deferred logging is used and log messages are prefixed with the thread name
  ...
  stopped and its ``struct k_thread`` is freed, the log system might still try to access that
  structure when handling the message later. This creates an use-after-free scenario.
...
.. code-block:: c
   ...
   log_flush();  // flush log bufferis before freeing the struct k_thread
