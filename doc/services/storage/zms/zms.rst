...
- Supports larger partition size and sector size (64 bits address space)
- Supports larger IDs (32 bits) to store ID/Value pairs
...
- Supports larger write-block-size (Only for platforms that need this)
- Enables customization of sector count (Still under experimentation)
...
ZMS vs NVS, what should I choose ?
==================================

- If you are using a non erasable technology device like RRAM or MRAM, **ZMS** is definitely the
...
- For devices with large write_block_size and/or needs a sector size that is different than the
  classical flash page size (equal to erase_block_size), **ZMS** is also the best fit as there is
  the possibility to customize these parameters and add the support of these devices in ZMS.
- For classical flash technology devices, **NVS** is recommended as it has low footprint (smaller
...
- If your application needs more than 64K IDs for storage, **ZMS** is recommended here as it
