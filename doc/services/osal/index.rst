...
Porting each one re-implements the same Zephyr kernel translations. The OS
Abstraction Layer centralizes those translations behind a small ``osal_*`` API
so an adapter shrinks to a thin name and return-code mapping. It is enabled
with :kconfig:option:`CONFIG_OSAL`.
...
* Fixed-item message queue (send, receive, send-to-front, send-from-ISR, peek)
* Intrusive FIFO (pointer-passing, prepend, arbitrary remove)
* Event group (bit-level wait/notify, backed by :c:struct:`k_event`)
* Thread/task with priority get/set and suspend/resume (stack allocated per
...
Enable the layer with :kconfig:option:`CONFIG_OSAL`. It is typically pulled in
by a consumer's Kconfig with ``select`` rather than set by the application:
...
* The message queue uses copy semantics (fixed-size items). For
  pointer-passing without a copy and for arbitrary removal of an enqueued
  item, use the intrusive FIFO. Both the queue and the FIFO support prepend.
