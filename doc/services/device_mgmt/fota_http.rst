...
transfer before the reboot. An image rejected by that comparison, or by the
downgrade check below, is erased from the slot. After a download that
failed part way, :c:func:`fota_http_apply` refuses the slot until another
download into it succeeds, while the partial data stays in place for a
resume.
...
#. :c:func:`fota_http_download` or :c:func:`fota_http_download_async`
   fetches the image into the secondary slot. The optional
   :c:struct:`fota_http_download_params` members ``sha256``, ``resume``
   and the TLS ones only exist when their Kconfig option is enabled.
...
TLS handshake. With :kconfig:option:`CONFIG_FOTA_HTTP_ASYNC`,
:c:func:`fota_http_download_async` runs the same transfer on a thread
owned by the library, sized by
:kconfig:option:`CONFIG_FOTA_HTTP_THREAD_STACK_SIZE`, and reports the
result through a completion callback. The shell command selects the
asynchronous variant so the shell stack size does not matter.
Applications that only call :c:func:`fota_http_download` leave it
disabled and do not pay for the thread stack.
...
:c:func:`fota_http_get_download_slot` reports which slot that is. This only
...
it is older, erasing the rejected image so it cannot be applied by mistake.
This is a convenience check for the application. An enforced
