...
The decision about suspending devices in a low power state is done based on the
state and if it has setting the property ``zephyr,pm-device-disabled``. Here is
an example of a target with two low power states with only triggering device power
management:
