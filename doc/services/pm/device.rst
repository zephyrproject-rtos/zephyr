...
Device Power Management (DEVICE PM) in Zephyr is a feature which makes
device driver's PM operations available outside of the device driver,
enabling power management of the device. This feature is enabled by
selecting :kconfig:option:`CONFIG_PM_DEVICE`.
...
- ``SUSPENDED``: Device hardware is powered, but the device is not needed by the
  ...
  power domain is no longer needed by the system. If the device itself is a
  power domain, devices on this power domain will be powered off in this state.

  Note that this state is **NOT** a "low-power"/"partially operable" state,
  those are configured using device driver specific APIs, and apply only while
...
- ``SUSPENDING``: Device hardware is powered, but the device has been scheduled
  to be suspended, as it is no longer needed by the system. This state is used
  when delegating suspension of a device to the PM subsystem, optionally with
  residency to avoid unnecessary suspend/resume cycles. The device will be
  unscheduled in case the device becomes needed by the system.

  No device driver API calls must occur in this state.

  Note that this state is opaque to the device driver (no
  :c:enum:`pm_device_action` is called as this state is entered)
