...
work queue, the user will experience almost the same behavior as the regular listeners, since the
async listeners run before all other threads after the VDED execution, since the system work queue
is a cooperative thread with -1 priority.
