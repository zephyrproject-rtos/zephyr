...
required_images: <list of images>
    ...
    If required test scenario is not available in tree or build is failed,
    ...
    Tests on QEMU platforms will be skipped if test does not have an own image.

no_own_image: <True|False> (default False)
    If true, the test scenario will not be built. Instead, it
    will be used the first test scenario from ``required_images`` list.
    No support for QEMU platforms.
