...
- **Vendor-specific features must be encapsulated.**
  ...
  Vendor-only mechanisms should live within the vendor’s SoC directory.
  If multiple SoCs from the same vendor share internal logic, a vendor-specific
...
- **Vendor-specific public APIs should be avoided.**

  Only consider exposing a vendor-specific public API if it is expected to evolve
  into a stable, cross-vendor Zephyr API and can follow the API lifecycle.
...
5. Vendor-Specific Public API (Discouraged)
