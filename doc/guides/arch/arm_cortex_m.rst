...
+---------------------------------+-----------------------------------+-----------------+---------+--------+-----------+--------+---------+------------+------------+
|                                 |   HW-assisted stack checking      |        N        |   N     |   N    |    N      |    N   |Y [#f2]_ |     Y      |   Y        |
+---------------------------------+-----------------------------------+-----------------+---------+--------+-----------+--------+---------+------------+------------+
...
+---------------------------------+-----------------------------------+-----------------+---------+--------+-----------+--------+---------+------------+------------+
| Execute from SRAM               |                                   |        N        |   N     |   Y    |    Y      |    Y   |    N    |     Y      |   Y        |
+---------------------------------+-----------------------------------+-----------------+---------+--------+-----------+--------+---------+------------+------------+
...
In Arm v6-M and Armv7-M architecture variants, thread stacks are additionally required to be align with a value equal to their size,
...
stack pointer manipulation during context switching, without affecting the current execution context in handler mode.
...
to be selected taking into consideration nested interrupts, each pushing an additional stack frame.
...
* When switching-out the current thread, the processor stores
   ...
   * the PSP of the current thread which points to beginning of the current thread's exception
...
* When switching-in a new thread the processor
   ...
   * restores the FP callee-saved registers if the switched-in thread have had
...
Stack overflows trigger the dedicated UsageFault exception provided by Armv8-M.
...
HW interrupts are allocated a priority level lower than the SVC.
...
interrupt. If the ZLI feature is enabled (see :kconfig:`CONFIG_ZERO_LATENCY_IRQS`), then
...
priority. While this fulfils the requirement of locking interrupts, the consequence
is that kernel runtime errors will escalate to HardFault.
...
implemented.). arch_irq_lock() masks all system and HW interrupts with the exception of

* SVCs
* processor faults
* ZLIs

This allows zero latency interrupts and system (processor and kernel) faults to be handled
in exactly the same way, regardless of whether IRQs have been locked or not when the error
occurs.
...
* the mechanism is essential for detecting stack overflows in supervisor threads, or
  user threads in privileged mode; stack overflows in thread user mode will always be
...
* :kconfig:`CONFIG_MPU_STACK_GUARD` will normally reserve one MPU region for programming
  the stack guard (in certain Armv8 configurations with :kconfig:`CONFIG_MPU_GAP_FILLING`
...
* When :kconfig:`CONFIG_HW_STACK_PROTECTION` is enabled on Armv8-M platforms the native
...
* One MPU region programs the entire flash area as read-execute.
  ...
  which programs the flash are with RWX permissions. If :kconfig:`CONFIG_USERSPACE` is
...
* One MPU region programs the entire SRAM area with privileged-only
  ...
  SRAM. (An exception to this setting is when :kconfig:`CONFIG_MPU_GAP_FILLING` is enabled;
...
Additional *static* MPU regions are programmed once during system boot. These regions
...
* a possibly unprivileged RW region for GCOV code coverage accouning area, when :kconfig:`CONFIG_COVERAGE_GCOV` is enabled
...
Certain thread-specific MPU regions are programmed dynamically, at each thread context switch:

* an unprivileged RW region for the current hread's stack area (for user threads)
...
In Armv8 processors the MPU architecture does not allow programmed MPU regions to
