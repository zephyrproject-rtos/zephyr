...
steering committee and should follow the project licencing and contribution
guidelines.

A request for a new module should be initiated using an RFC issue in the
Zephyr project issue tracking system with details about the module and how it
integrates into the project. If the request is approved, a new repository is
created and initiated with basic information that would allow submitting code to
the module project following the project contribution guidelines.
...
#. Submit an issue with details about the module to be created
...
#. Submit a new entry into the :zephyr_file:`west.yml` with the following
   information:
...
Where 23 in the example above indicated the pull request submitted to the
*my_module* repository.
...
Reviewing and Merging Changes Related to Modules
*************************************************

Specifying the pull request number in the revision field will allow building the Zephyr main tree with the changes submitted to the
module. The pull request to master will need to be marked as ``DNM`` (Do Not
Merge) to allow for the module to first be merged. Once the module is merged,
the revision will need to be changed either by submitted or by the maintainer
to the commit hash of the module which reflects the changes.
