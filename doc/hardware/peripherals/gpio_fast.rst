...
Backend selection is automatic: the dispatch macros use ``DT_BINDING_COMPAT_TOKEN()``
...
.. code-block:: c
   ...
   #include <zephyr/sys/delay_nops.h>
...
drivers use :c:macro:`DELAY_NOPS` (from ``<zephyr/sys/delay_nops.h>``) to emit
compile-time NOP delays between ``gpio_fast_set()`` and ``gpio_fast_clear()``
calls. NOP counts can be computed using :c:macro:`ns_to_cyc_at_hz_floor32` (from
``<zephyr/sys/time_units.h>``) with the CPU clock frequency from
``GPIO_FAST_CPU_FREQ(node_id)`` and overhead subtracted.
...
``GPIO_FAST_SET_CYCLES_atmel_sam0_gpio``).
...
SAM0 (no instruction cache, flash wait states add jitter).
...
.. code-block:: c
   ...
   #define MY_NODE    DT_ALIAS(my_pin)
   #define MY_BACKEND GPIO_FAST_COMPAT(MY_NODE, gpios)
   #define MY_CPU_FREQ GPIO_FAST_CPU_FREQ(MY_NODE)
   ...
   BUILD_ASSERT(SET_CYCLES < ns_to_cyc_at_hz_floor32(400, MY_CPU_FREQ),
                "gpio_fast_set is too slow for WS2812 T0H (400 ns)");
   BUILD_ASSERT(CLEAR_CYCLES < ns_to_cyc_at_hz_floor32(450, MY_CPU_FREQ),
                "gpio_fast_clear is too slow for WS2812 T1L (450 ns)");

   /* NOP counts computed from CPU clock frequency. */
...
(e.g. ``zephyr_gpio_fast_atmel_sam0_gpio.h``) in ``drivers/gpio/fast/<vendor>/``.
...
``DT_BINDING_COMPAT_TOKEN()`` to token-paste the controller's compatible onto the
...
.. note::
   ...
   Drivers should use ``BUILD_ASSERT`` with :c:macro:`ns_to_cyc_at_hz_floor32`,
   ``GPIO_FAST_CPU_FREQ(node_id)``, and
...
1. Create ``drivers/gpio/fast/<vendor>/zephyr_gpio_fast_<compat>.h``
   ...
   hyphens replaced by underscores (e.g. ``atmel_sam0_gpio``).
...
4. Add a ``#ifdef CONFIG_GPIO_*`` / ``#include`` block
   to ``gpio_fast.h``.
5. List the platform under Vendor Support in this document and in the next
   release notes.
