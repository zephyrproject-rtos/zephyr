...
and is a single-controller multidrop bus. This means one controller initiates
all data exchanges with the peripheral devices.
...
Zephyr's 1-Wire controller API is used to interact with 1-Wire peripheral
devices like
...
* The link layer handles basic communication functions such as bus reset,
  ...
  be added.
* The 1-Wire network layer handles all means for peripheral identification and bus
  arbitration.
  This includes ROM commands like Match ROM, or Search ROM.

   * All peripheral devices have a unique 64-bit identification number, which
     includes a 8-bit `1-Wire Family Code`_ and a 8-bit CRC.
   * In order to find peripherals on the bus, the standard specifies an search
     algorithm which successively detects all peripherals on the bus.
...
* Transport layer and Presentation layer functions are not implemented in the
  generic 1-Wire driver and therefore must be handled in individual peripheral
  drivers.
