...
* Starting a read for sensors directly from an interrupt handler. No dedicated
  thread needed saving precious stack space. No work queue needed introducing variable latency.
* Starting a read for multiple sensors simultaneously from a single call context (interrupt/thread/work queue).
...
* Far less likely triggers are missed due to long held interrupt masks from callbacks
  and context swapping.
...
.. literalinclude:: ../../../../samples/sensor/temp_polling/src/main.c
   :language: c
...
.. literalinclude:: ../../../../samples/sensor/multiple_temp_polling/src/main.c
   :language: c
...
configuration. A stream specifies the set of triggers to capture and if data should
be captured with the event.



.. literalinclude:: ../../../../samples/sensor/accel_stream/src/main.c
   :language: c
