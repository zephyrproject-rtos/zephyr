...
The API copies the **raw** value as the board encodes it into a
caller-supplied buffer, not a semantic meaning ("v3.2 with the radio
populated"). Mapping the raw value to something meaningful is application or
board-integrator policy and stays out of this driver class on purpose. The
identity is a variable-length byte sequence, following the same convention as
:c:func:`hwinfo_get_device_id`: big-endian (most significant byte first), so
a hex dump of the buffer reads as the intended value on any host.
