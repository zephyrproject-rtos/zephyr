...
Virtiofs
=========
This driver provides support for `virtiofs <https://virtio-fs.gitlab.io/>`_ - a filesystem allowing virtual machine guest to access
...
starting with device readable part, containing the FUSE input header and input data, and ending it with device writeable part, with place
for FUSE output header and output data.
