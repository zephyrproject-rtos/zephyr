...
element of a clock tree that takes one (or multiple) frequencies as output and
...
initialize a clock output device. For devices defined in devicetree, using
clocks defined within their ``clock-outputs`` property,
...
* Multiplexer clocks, which implement :c:struct:`cclock_management_mux_api`.
...
.. graphviz::
   ...
   digraph G {
     ...
     {rank=same; uart_output->clock_management_get_rate->clock_management_clk_rate_0 [dir=both, minlen=3];}
     {rank=same; clock_management_clk_rate_0->clock_management_clk_rate_1->clock_management_clk_rate_2
         [dir=both, minlen=3];}
...
* :c:func:`clock_configure_recalc` (or the multiplexer/root clock specific
  ...
  will produce.
* :c:func:`clock_notify_children` will be called to validate that all children
...
* :c:func:`clock_notify_children` will be called again to notify children of the
  rate change.
