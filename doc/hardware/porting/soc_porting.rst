...
:file:`Kconfig.soc`
  ...
  of the SoC family name, whilst ``soc_name``, ``soc_series_name`` and ``soc_family_mame`` are
  pure lowercase letter versions of these (uppercase for Kconfig names, lowercase for Kconfig
  values). If these fields do not appear in the :file:`soc.yml` file then they should not be
  present in the :file:`Kconfig.soc` file.
