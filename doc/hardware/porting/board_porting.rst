...
board that only needs its SoC described in terms of another one, which is covered by
:ref:`soc_porting_base`. This property exists for cases where a board combines multiple
SoCs which have no relationship to each other outside that board, such as a simulated board
built on one SoC while using the Kconfig of the SoC it simulates.
