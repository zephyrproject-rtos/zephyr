...
remain board agnostic, please follow these guidelines
when porting a board you intend to contribute to Zephyr:
...
Keep subsystems disabled by default
  Subsystems must remain **disabled by default**, unless required to support a component essential
  to the board's basic operations or otherwise stated in these recommendations.
  otherwise stated in these recommendations.

Configure system clock and tick source
  A functioning system clock and tick source must be set up.
...
Enable pinmux and interrupt controllers
