...
Shields are described by their mechanical connectors, and the electrical interface of the
signals that are routed via the connectors.

Mapping of the connector pins to the GPIOs of the board's microcontroller is done via a GPIO
nexus node in the devicetree file of the board. The same node is used by the shield
overlay file, if the shield uses GPIO functionality.
Also electrical buses which are routed via the connector have devicetree node labels, for example
I2C, UART and SPI buses.
The board's devicetree file and the shield's overlay file are used together at build time.

Zephyr supports a number of shield interfaces, one of them is the well-known Arduino
form factor.

For example a board without an accelerometer, but having an Arduino connector, can
use an accelerometer via an Arduino shield. The devicetree file of the board defines
an ``arduino_i2c`` node label for the I2C bus which is exposed to the Arduino connector.
An accelerometer shield with Arduino connector will also use the ``arduino_i2c`` node label
to describe how the accelerometer is connected. This is done in the ``.overlay`` file of the
shield.
