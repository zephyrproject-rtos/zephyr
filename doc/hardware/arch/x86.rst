...
marked by dts :c:macro:`DT_CHOSEN_SRAM_ADDR` and
:c:macro:`DT_CHOSEN_SRAM_SIZE` while the virtual address space is marked by
...
- Physical and virtual address spaces must be disjoint. This is
  ...
  - If :kconfig:option:`CONFIG_X86_PAE` is enabled (``=y``), each address space
    ...
    - ``DT_CHOSEN_SRAM_ADDR == 0x00000000`` and
  ...
  - Both ``DT_CHOSEN_SRAM_ADDR`` and ``CONFIG_KERNEL_VM_BASE``
