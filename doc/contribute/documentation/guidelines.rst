...
.. _doc_guidelines_shell_modules:

Shell modules
=============

Many drivers and subsystems come with a shell module, i.e. a root :ref:`shell <shell_api>` command
(``gpio``, ``can``, ``sensor``, ...) and its subcommands. Their documentation should not stop at
listing these commands: like the :ref:`can_shell` and :ref:`eeprom_shell` pages, it should explain
what the module is useful for and walk the reader through typical tasks, with example shell
sessions. The directives described below take care of the parts that can be generated, i.e. how to
enable the module and the reference of its commands, the latter being extracted from an actual
firmware build so that it never goes out of sync with the code.

The documentation of a shell module is typically a ``shell.rst`` page next to the API documentation
of the subsystem (and listed in its ``toctree``), with the following structure:

#. A :rst:dir:`zephyr:shell-module` directive at the beginning of the page, whose content describes
   what the module is useful for.
#. Task-oriented sections showing how to use the module, each with example shell sessions in
   ``console`` code blocks. Refer to commands using the :rst:role:`zephyr:shell-command` role, so
...
.. rst:directive:: .. zephyr:shell-module:: command

   Documents the shell module whose root command is given as argument. The (optional) content of
   the directive is rendered as the description of the module, and its first paragraph, which
   should therefore be kept short, is used as the summary of the module in the table generated by
   :rst:dir:`zephyr:shell-module-listing`.
   Instructions on how to enable the module, build an application with it and get help on its
   commands are generated from the directive options and rendered after the description.

   The directive also builds the :zephyr_file:`doc/_scripts/shell_command_tree` application with
   the Kconfig options of the module, runs it, and registers the tree of commands it reports so
   that :rst:dir:`zephyr:shell-command-reference` can render it and
   :rst:role:`zephyr:shell-command` references can be validated.
   ...
   For example::
      ...
      .. zephyr:shell-module:: gpio
         ...
         The GPIO shell provides a ``gpio`` command with a set of subcommands for the
         :ref:`shell <shell_api>` module. It allows ...

   .. rst:directive:option:: kconfig
      :type: text

      The Kconfig option enabling the shell module (e.g. ``CONFIG_GPIO_SHELL``).

   .. rst:directive:option:: depends
      :type: text

      Optional, space-separated list of the other Kconfig options that need to be enabled for the
      shell module to be available (e.g. ``CONFIG_GPIO``).

   .. rst:directive:option:: platform
      :type: text

      The platform for which the ``shell_command_tree`` application is built, and on which it is
      run. It needs to be able to build the shell module and to run in the documentation build
      environment. Defaults to ``native_sim/native/64``.

   .. note::

      Building and running firmware requires a working Zephyr build environment. This step can be
      skipped by building the documentation with the ``SKIP_SHELL_REFERENCE`` option (see
      :ref:`zephyr_doc`), in which case a note is shown in place of the command reference and
      references to subcommands link to the module instead of being validated.

.. rst:directive:: .. zephyr:shell-command-reference:: [command]

   Renders the reference (description and usage) of all the commands of a shell module, as reported
   by the firmware built by :rst:dir:`zephyr:shell-module`. The module is the one documented in the
   current document, unless its root command is given as argument.

.. rst:directive:: .. zephyr:shell-module-listing::

   Shows a table of all the shell modules documented with :rst:dir:`zephyr:shell-module`, with
   their description and the Kconfig option enabling them.

.. rst:role:: zephyr:shell-command

   References a shell command documented with :rst:dir:`zephyr:shell-module`, using its full name.
   For example::

      Use :zephyr:shell-command:`gpio conf` to configure a pin.

   Will render as:

      Use :zephyr:shell-command:`gpio conf` to configure a pin.

   The documentation build warns about references to commands that do not (or no longer) exist in
   the firmware, which keeps hand-written documentation in sync with the code.
