...
- Commits in a pull request must represent distinct, reviewable, logical units
  of change that retain bisectibility. Unrelated changes should not be present
  in the same commit. Intermediary changes or non-final artifacts that occur in
  the git history during development of a pull request must be squashed out of
  these fundamental commits whenever bisectability allows it. This includes, but
  is not limited to, renaming files that have been introduced in earlier
  commits, or rewriting newly added code.
