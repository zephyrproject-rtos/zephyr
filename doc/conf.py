...
# Kconfig, external board/sample/snippet content and shell command references
# extracted from firmware builds) when previewing narrative changes. The
# options below turn the matching generators off; the zephyr.partial_build
# extension degrades the content they would have produced (placeholder
# directives, plain-text references) so the build still passes under -W. Such
# a build is preview-only: it can neither validate cross-references nor be
# published.
SKIP_DOXYGEN, SKIP_KCONFIG, SKIP_EXTERNAL_CONTENT, SKIP_SHELL_REFERENCE = (
    tags.has(t)  # pylint: disable=undefined-variable  # noqa: F821
    for t in ("skip_doxygen", "skip_kconfig", "skip_external_content", "skip_shell_reference")
)
...
zephyr_generate_shell_reference = not SKIP_SHELL_REFERENCE
