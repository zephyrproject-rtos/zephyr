...
if "ZEPHYR_BASE" not in os.environ:
    sys.stderr.write("$ZEPHYR_BASE environment variable undefined. Did you source zephyr-env.sh?\n")
    exit(1)
ZEPHYR_BASE = os.environ["ZEPHYR_BASE"]

# The following code extracts version information by reading the VERSION file.
...
try:
    ...
    if version_major is not None and version_minor is not None and \
    patchlevel is not None and extraversion is not None:
        version = release = version_major + '.' + version_minor + '.' + patchlevel
        if extraversion != '':
            version = release = version + '-' + extraversion
    else:
        sys.stderr.write('Warning: Could not extract kernel version from VERSION file\n')
        version = release = "unknown version"
        version = release = os.getenv('KERNELVERSION','1.10.99')
