...
A minimal skeleton example follows. A complete server/client pair is included
in the repository under the samples directory; see the server and client for a
runnable example.

- Server sample: :zephyr_file:`samples/bluetooth/l2cap_server_simple`
- Client sample: :zephyr_file:`samples/bluetooth/l2cap_client_simple`
...
.. note::
   ...
   ``CONFIG_BT_MAX_CONN`` and ``bt_conn_index(conn)``. See the client sample
   for how to open a channel and send data to the server.
