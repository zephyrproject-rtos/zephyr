...
- **Returns**: A valid client socket descriptor on success or a negative value on error.
  Users should retry the call if they receive -EAGAIN.


``void net_latmon_start(int client, net_latmon_measure_t measure_f)``
---------------------------------------------------------------------
Starts the latency monitoring process.

- **Parameters**:
  - ``client``: The client socket descriptor.
  - ``measure_f``: A callback function that calculates latency deltas.
...
2. **Connection Handling**:
   ...
   If no connection is received within the timeout period, the service broadcasts its IP address using UDP and returns -EAGAIN.
