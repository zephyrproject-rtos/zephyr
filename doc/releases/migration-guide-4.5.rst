...
* LVGL now renders directly in its ``RGB565_SWAPPED`` color format for displays reporting
  ...
  needed for these displays and must not be enabled together with that pixel format. This invalid
  combination is rejected during LVGL initialization because it would otherwise byte-swap the
  buffer twice. The ``t_deck``, ``m5stack_core2`` and ``wio_terminal`` boards no longer enable the
  option by default.
