...
* The ``CONFIG_BT_RECV_CONTEXT`` Kconfig choice and its options ``CONFIG_BT_RECV_WORKQ_SYS``
  ...
  Tune :kconfig:option:`CONFIG_BT_RX_STACK_SIZE` (the RX thread stack) and, since
  low-priority RX no longer runs on the system workqueue, you can reduce
  :kconfig:option:`CONFIG_SYSTEM_WORKQUEUE_STACK_SIZE` accordingly.
