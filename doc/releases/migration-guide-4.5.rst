...
* A build now only loads the Kconfig and CMake trees of the SoC it targets, instead of the trees of
  ...
  and ``myra`` have been converted: their directories hold only :file:`soc.yml`, and their
  ``SOC_OSD32MP15X`` and ``SOC_MYRA`` Kconfig symbols are generated by the build system, selecting
  the symbol of the underlying STM32 SoC. Board targets and board Kconfig files are unchanged, as
  is ``CONFIG_SOC``, which already resolved to the underlying SoC. A board that pairs two
  otherwise independent SoC trees declares that with the ``requires`` property in its
  :file:`board.yml`, see :ref:`board_porting_requires`. As a temporary escape hatch, setting the
  CMake variable ``HWM_LOAD_ALL_SOCS`` restores the previous behaviour of loading all SoC trees.
