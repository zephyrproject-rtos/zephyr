...
static pthread_cond_t  posix_soc_cond_cpu  = PTHREAD_COND_INITIALIZER;
/*mutex for the conditional variable posix_soc_cond_cpu*/
static pthread_mutex_t posix_soc_mtx_cpu   = PTHREAD_MUTEX_INITIALIZER;
 ...
 */
static void posix_soc_change_cpu_state_and_wait(bool halted)
{
	if (pthread_mutex_lock(&posix_soc_mtx_cpu)) {
		simulation_engine_print_error_and_exit(
			"Posix arch core: error on pthread_mutex_lock()\n");
	}

	posix_soc_cpu_halted = halted;

	if (pthread_cond_signal(&posix_soc_cond_cpu)) {
 ...
 */
	...
	while (posix_soc_cpu_halted == halted) {
 ...
 */
		...
		pthread_cond_wait(&posix_soc_cond_cpu, &posix_soc_mtx_cpu);
	}

	if (pthread_mutex_unlock(&posix_soc_mtx_cpu)) {
		simulation_engine_print_error_and_exit(
			"Posix arch core: error on pthread_mutex_lock()\n");
	}
}
 ...
 */
	posix_soc_change_cpu_state_and_wait(false);
}
 ...
 * running again
 */
	posix_soc_change_cpu_state_and_wait(true);
 ...
 */
	...
	void irq_handler(void); /*the board must define this*/
	irq_handler();
...
void posix_soc_boot_cpu(void)
{
	...
	while (posix_soc_cpu_halted == false) {
		pthread_cond_wait(&posix_soc_cond_cpu, &posix_soc_mtx_cpu);
	}
	if (pthread_mutex_unlock(&posix_soc_mtx_cpu)) {
		simulation_engine_print_error_and_exit(
