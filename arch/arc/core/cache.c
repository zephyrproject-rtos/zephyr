...
#define DC_CTRL_IM_MASK              0x40  /* d-cache invalidate mode bit */
#define DC_CTRL_RGN_OP_MASK          0xe00 /* d-cache region operation mask */

#define MMU_BUILD_PAE_MASK 0x1000 /* physical address extension enable mask */

#define SLC_CTRL_DISABLE_MASK 0x1   /* SLC disable mask */
#define SLC_CTRL_IM_MASK      0x40  /* SLC invalidate mode mask  */
#define SLC_CTRL_BS_MASK      0x100 /* SLC busy status mask */
#define SLC_CTRL_RGN_OP_MASK  0xe00 /* SLC region operation mask */
...
static void slc_flush_region(void *start_addr_ptr, size_t size)
{
	...
	K_SPINLOCK(&slc_lock) {
		...
		z_arc_v2_aux_reg_write(_ARC_V2_SLC_RGN_START,
				       start_addr & ~(CONFIG_SLC_LINE_SIZE - 1));
