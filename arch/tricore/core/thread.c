		     ...
		     k_thread_entry_t entry, void *p1, void *p2, void *p3)
{
	...
	thread->arch.prs = FIELD_GET(K_PROTECTION_SET_MASK, thread->base.user_options);
...
FUNC_NORETURN void arch_user_mode_enter(k_thread_entry_t user_entry, void *p1, void *p2, void *p3)
{
	...
	/* TriCore userspace transition is not yet implemented. Reaching
	 * this point with CONFIG_USERSPACE=y is a build configuration
	 * the port does not support; panic loudly instead of falling off
	 * the end of a FUNC_NORETURN.
	 */
	k_panic();
	CODE_UNREACHABLE;
}
							      ...
							      void *p2, void *p3)
{
	...
	/* main() returned with CONFIG_MULTITHREADING=n: there is no
	 * scheduler to fall back to, so loop forever to honor the
	 * FUNC_NORETURN contract.
	 */
	for (;;) {
	}
	CODE_UNREACHABLE;
