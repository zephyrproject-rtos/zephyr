...
void z_tricore_fatal_error(unsigned int reason, const struct arch_esf *lower)
{
	...
	if (upper != NULL) {
		...
		LOG_ERR("A12: %08x A13: %08x A14: %08x A15: %08x", upper->a12, upper->a13,
			upper->a14, upper->a15);
		LOG_ERR("D8:  %08x D9:  %08x D10: %08x D11: %08x", upper->d8, upper->d9, upper->d10,
			upper->d11);
		LOG_ERR("D12: %08x D13: %08x D14: %08x D15: %08x", upper->d12, upper->d13,
			upper->d14, upper->d15);
		LOG_ERR("PC:  %08x SP:  %08x PSW: %08x PCXI: %08x", lower->a11, upper->a10,
			upper->psw, upper->pcxi);
	}
#endif
...
void z_tricore_fault(uint8_t trap_class, uint8_t tin)
{
	...
	z_tricore_fatal_error(K_ERR_CPU_EXCEPTION, lower);
}
