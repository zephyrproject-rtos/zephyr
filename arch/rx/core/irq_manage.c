...
int arch_irq_connect_dynamic(unsigned int irq, unsigned int priority,
			     ...
			     uint32_t flags)
{
	__ASSERT(irq < CONFIG_NUM_IRQS, "trying to connect to invalid interrupt (%u)", irq);
	__ASSERT(irq >= CONFIG_GEN_IRQ_START_VECTOR, "trying to connect to reserved interrupt (%u)",
		 irq);
