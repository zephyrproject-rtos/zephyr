...
/**
 * @file
 * @brief New thread creation for ARM11
 *
 * Core thread related primitives for the ARM1176 processor architecture.
 */
...
#if defined(CONFIG_USERSPACE)
static inline bool z_arm_thread_has_priv_stack(struct k_thread *thread)
{
	return thread->arch.priv_stack_start != 0U;
}

static inline void z_arm_thread_priv_stack_adjust(struct k_thread *thread, int adjust)
{
	thread->arch.priv_stack_start += adjust;
}
#else
static inline bool z_arm_thread_has_priv_stack(struct k_thread *thread)
{
	...
	bool use_large_guard)
{
	if (use_large_guard) {
		...
		if ((thread->arch.mode &
			...
			thread->arch.mode |= Z_ARM_MODE_MPU_GUARD_FLOAT_Msk;
			if (z_arm_thread_has_priv_stack(thread)) {
				/* User thread */
				z_arm_thread_priv_stack_adjust(thread, FP_GUARD_EXTRA_SIZE);
			} else {
				/* Privileged thread */
...
int arch_float_enable(struct k_thread *thread, unsigned int options)
{
	ARG_UNUSED(thread);
	ARG_UNUSED(options);
	/* This is not supported on ARM11 */
	return -ENOTSUP;
}
#endif /* CONFIG_FPU && CONFIG_FPU_SHARING */
