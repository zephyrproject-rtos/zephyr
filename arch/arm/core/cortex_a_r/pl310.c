 ...
 * latencies) come from the arm,pl310-cache devicetree node.
 */
...
/* AUX_CTRL associativity strap (bit 16): 0 = 8-way, 1 = 16-way */
...
/*
 * Firmware-handoff contract: the boot firmware (u-boot / JTAG loader) must hand
 * off with the PL310 disabled, or with no dirty lines belonging to memory Zephyr
 * will reuse (i.e. it quiesced DMA and cleaned the L2 before jumping to
 * Zephyr). This function disables and invalidates the controller
 * unconditionally, so any dirty data still present at entry is discarded.
 *
