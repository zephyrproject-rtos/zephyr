...
/*
 * Version 2 adds callee-saved registers r4-r11 (requires
...
/*
 * AArch32 GDB remote protocol uses 17 registers in this order:
 *   r0-r12, SP(r13), LR(r14), PC(r15), CPSR
 * This is the same numbering as Cortex-M (which has xPSR at slot 16),
 * so the Cortex-M GDB stub can parse this block without modification.
 */
struct arm_arch_block {
	struct {
		...
		uint32_t lr;
		uint32_t pc;
		uint32_t xpsr; /* CPSR captured at fault time */
		uint32_t sp;

		/* callee-saved r4-r11, only populated in V2 */
		uint32_t r4;
		uint32_t r5;
		uint32_t r6;
		uint32_t r7;
...
/*
 * Keep this off the stack — the struct is large enough to warrant a static.
 */
...
void arch_coredump_info_dump(const struct arch_esf *esf)
{
	...
	/*
	 * Basic frame registers — always present (pushed by exception assembly).
	 */
...
{
	return COREDUMP_TGT_ARM_CORTEX_A_R;
}
