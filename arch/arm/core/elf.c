			...
			uint32_t *upper, uint32_t *lower)
{
	...
	*offset = (sign << SHIFT_THM_JUMPS_SIGN) | ((~(j_one ^ sign) & 1) << SHIFT_THM_BL_J1) |
 ...
 */
int32_t arch_elf_relocate(elf_rela_t *rel, uintptr_t loc, uintptr_t sym_base_addr,
			const char *sym_name, uintptr_t load_bias)
{
	...
	case R_ARM_CALL:
		/* fall-through */
	case R_ARM_JUMP24:
		ret = jumps_handler(reloc_type, loc, sym_base_addr, sym_name, &offset);
		break;
	...
	case R_ARM_PREL31:
		ret = prel31_handler(reloc_type, loc, sym_base_addr, sym_name, &offset);
		break;
	...
	case R_ARM_MOVT_PREL:
		movs_handler(reloc_type, loc, sym_base_addr, sym_name, &offset);
		break;

	case R_ARM_THM_CALL:
		/* fall-through */
	case R_ARM_THM_JUMP24:
		ret = thm_jumps_handler(reloc_type, loc, sym_base_addr, sym_name, &offset);
		break;
	...
	case R_ARM_THM_MOVT_PREL:
		thm_movs_handler(reloc_type, loc, sym_base_addr, sym_name, &offset);
		break;
