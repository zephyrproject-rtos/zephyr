...
volatile uint32_t *arm_m_exc_lr_ptr;
...
/* Called out of z_arm_fault() */
bool arm_m_iciit_check(uint32_t psp)
{
	struct hw_frame_base *f = (void *)psp;

	/* Look for undefined instruction faults from our stub */
	if (f->pc != (uint32_t) arm_m_iciit_stub) {
		return false;
	}
		      ...
		      void *arg3)
{
	...
	arm_m_lto_refs[0] = arm_m_must_switch;
#endif
...
#if !defined(CONFIG_FPU) && defined(CONFIG_QEMU_TARGET)
	/* Workaround for an apparent bug with qemu where the
	 * interrupt return from the WFI instruction in
	 * arch_cpu_idle() has the FPU state bit set in LR, despite
	 * having pushed the non-FPU frame.
	 */
	fpu = false;
#endif
