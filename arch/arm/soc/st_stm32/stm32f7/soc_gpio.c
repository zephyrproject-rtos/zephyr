...
{
	switch (func) {
	case STM32F7X_PIN_CONFIG_BIAS_HIGH_IMPEDANCE:
	case STM32F7X_PIN_CONFIG_BIAS_PULL_UP:
	case STM32F7X_PIN_CONFIG_BIAS_PULL_DOWN:
		return 0x0;
	case STM32F7X_PIN_CONFIG_DRIVE_PUSH_PULL:
	case STM32F7X_PIN_CONFIG_DRIVE_PUSH_UP:
	case STM32F7X_PIN_CONFIG_DRIVE_PUSH_DOWN:
	case STM32F7X_PIN_CONFIG_DRIVE_OPEN_DRAIN:
	case STM32F7X_PIN_CONFIG_DRIVE_OPEN_UP:
	case STM32F7X_PIN_CONFIG_DRIVE_OPEN_DOWN:
		return 0x1;
	case STM32F7X_PIN_CONFIG_AF_PUSH_PULL:
	case STM32F7X_PIN_CONFIG_AF_PUSH_UP:
	case STM32F7X_PIN_CONFIG_AF_PUSH_DOWN:
	case STM32F7X_PIN_CONFIG_AF_OPEN_DRAIN:
	case STM32F7X_PIN_CONFIG_AF_OPEN_UP:
	case STM32F7X_PIN_CONFIG_AF_OPEN_DOWN:
		return 0x2;
	case STM32F7X_PIN_CONFIG_ANALOG:
		return 0x3;
	}
...
int stm32_gpio_flags_to_conf(int flags, int *pincfg)
{
	...
	if (direction == GPIO_DIR_OUT) {
		if (pud == GPIO_PUD_PULL_UP) {
			*pincfg = STM32F7X_PIN_CONFIG_DRIVE_PUSH_UP;
		} else if (pud == GPIO_PUD_PULL_DOWN) {
			*pincfg = STM32F7X_PIN_CONFIG_DRIVE_PUSH_DOWN;
		} else {
			*pincfg = STM32F7X_PIN_CONFIG_DRIVE_PUSH_PULL;
		}
	} else if (direction == GPIO_DIR_IN) {
		if (pud == GPIO_PUD_PULL_UP) {
			*pincfg = STM32F7X_PIN_CONFIG_BIAS_PULL_UP;
		} else if (pud == GPIO_PUD_PULL_DOWN) {
			*pincfg = STM32F7X_PIN_CONFIG_BIAS_PULL_DOWN;
		} else {
			*pincfg = STM32F7X_PIN_CONFIG_BIAS_HIGH_IMPEDANCE;
		}
	} else {
		return -ENOTSUP;
	}
