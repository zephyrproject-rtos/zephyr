...
static void copy_to_ctx(struct gdb_ctx *ctx, const struct arch_esf *stack)
{
	...
	if (bsa - (uint32_t *)stack > 4) {
		num_laddr_regs = 8;
	} else if (bsa - (uint32_t *)stack > 8) {
		num_laddr_regs = 12;
	} else if (bsa - (uint32_t *)stack > 12) {
		num_laddr_regs = 16;
	} else {
		num_laddr_regs = 4;
	}
...
static void restore_from_ctx(struct gdb_ctx *ctx, const struct arch_esf *stack)
{
	...
	if ((uint32_t *)bsa - (uint32_t *)stack > 4) {
		num_laddr_regs = 8;
	} else if ((uint32_t *)bsa - (uint32_t *)stack > 8) {
		num_laddr_regs = 12;
	} else if ((uint32_t *)bsa - (uint32_t *)stack > 12) {
		num_laddr_regs = 16;
	} else {
		num_laddr_regs = 4;
	}
