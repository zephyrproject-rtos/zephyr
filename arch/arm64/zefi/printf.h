...
/* Tiny, but not-as-primitive-as-it-looks formatted-output helpers
 * (zefi_printf / zefi_snprintf / zefi_sprintf). Handles %d, %x, %p, %c
 * and %s only, allows a "l" qualifier on %d and %x (and silently
 * ignores one %s/%c/%p). Accepts, but ignores, field width and
 * precision values that match: the regex: [0-9]*\.?[0-9]*
 *
 * Named zefi_* to avoid colliding with ISO C reserved identifiers
...
static inline int zefi_snprintf(char *buf, unsigned long len, const char *f, ...)
{
	int ret;
...
static inline int zefi_sprintf(char *buf, const char *f, ...)
{
	int ret;
...
static inline int zefi_printf(const char *f, ...)
{
	int ret;
	struct _pfr r = {0};
