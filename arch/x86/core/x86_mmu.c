			...
			pentry_t *old_val_ptr, pentry_t mask, uint32_t options)
{
	...
	for (int level = 0; level < NUM_LEVELS; level++) {
		 ...
		 *
		 * This is not a caller programming error but a genuine
		 * runtime condition, so it must not be compiled out by
		 * the CHECKIF() configuration.
		 */
		if ((*entryp & MMU_PS) != 0U) {
...
static uint8_t page_pool[PTABLE_COPY_SIZE * CONFIG_X86_MAX_ADDITIONAL_MEM_DOMAINS]
