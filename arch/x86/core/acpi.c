...
static void find_rsdp(void)
{
	...
	/* Let's first look at EFI configuration table, if EFI is enabled */
	if (IS_ENABLED(CONFIG_X86_EFI_SYSTEM_TABLE)) {
		...
		rsdp = efi_config_get_vendor_table_by_guid(&rsdp_guid_2);
		if (rsdp != NULL) {
			goto out;
		}

		rsdp = efi_config_get_vendor_table_by_guid(&rsdp_guid_1);
		if (rsdp != NULL) {
			goto out;
		}
	}
