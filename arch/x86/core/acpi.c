...
static void find_rsdp(void)
{
	...
	if ((POINTER_TO_UINT(search_phys) >= 0x80000UL) &&
		...
		for (unsigned int i = 0; i < 1024U/8U; i++) {
			if (search[i] == ACPI_RSDP_SIGNATURE) {
				rsdp_phys = search_phys + (size_t)i * 8U;
				rsdp = (void *)&search[i];
				goto found;
			}
		}
	...
	rsdp_phys = 0U;
	for (unsigned int i = 0; i < 128U*1024U/8U; i++) {
		if (search[i] == ACPI_RSDP_SIGNATURE) {
			rsdp_phys = search_phys + (size_t)i * 8U;
			rsdp = (void *)&search[i];
			goto found;
...
struct acpi_cpu *z_acpi_get_cpu(int n)
{
	...
	while (offset < madt->sdt.length) {
		...
		if (entry->type == ACPI_MADT_ENTRY_CPU) {
			...
			if ((cpu->flags & ACPI_CPU_FLAGS_ENABLED) != 0U) {
				if (n == 0) {
					return cpu;
				}
...
z_acpi_get_dev_scope_paths(struct acpi_dmar_dev_scope *dev_scope, int *n)
{
	...
	uint8_t n0 = (dev_scope->length - ACPI_DMAR_DEV_SCOPE_MIN_SIZE) /
		ACPI_DMAR_DEV_PATH_SIZE;
	*n = (int)n0;
