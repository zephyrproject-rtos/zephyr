...
{
	const unsigned int VECTORS_PER_PRIORITY = 16U;
	const unsigned int MAX_PRIORITY = 13U;
...
void z_x86_ipi_setup(void)
{
	...
	x86_irq_funcs[(unsigned int)CONFIG_SCHED_IPI_VECTOR - IV_IRQS] =
		(void *) z_sched_ipi;

	/* TLB shootdown handling */
	x86_irq_funcs[(unsigned int)CONFIG_TLB_IPI_VECTOR - IV_IRQS] = z_x86_tlb_ipi;
}
