...
int cobs_encode(struct net_buf_simple *src, struct net_buf_simple *dst)
{
	...
	if (net_buf_simple_tailroom(dst) < max_encoded_size) {
		return -ENOSPC;
	}

	uint8_t *code_ptr = net_buf_simple_add(dst, 1);
	uint8_t code = 1;
	...
	for (size_t i = 0; i < src->len; i++) {
		if (src->data[i] == 0) {
			...
			*code_ptr = code;
			code_ptr = net_buf_simple_add(dst, 1);
			code = 1;
		} else {
			/* Add non-zero byte to output */
			net_buf_simple_add_u8(dst, src->data[i]);
			code++;
...
{
	__ASSERT(src->len > 2, "Invalid COBS decode length, %d", src->len);
	...
	while (i < src_len - 1) {
		...
		if (net_buf_simple_tailroom(dst) < (code - 1)) {
			return -ENOMEM;
		}
		...
		for (uint8_t j = 1; j < code; j++) {
			...
			net_buf_simple_add_u8(dst, src_data[i++]);
		}

		if (code < 0xFF && i < src_len - 1) {
			/* Add implicit zero */
			if (net_buf_simple_tailroom(dst) < 1) {
				return -ENOMEM;
			}
			net_buf_simple_add_u8(dst, 0);
		}
