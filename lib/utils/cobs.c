...
{
	return cobs_encode_delimiter(src, dst, flags, 0);
}

int cobs_decode_delimiter(struct net_buf *src, struct net_buf *dst, uint32_t flags,
			  uint8_t delimiter)
{
	/* Base case: len of 1 without delimiter is decoded to [] */
	if (src->len == 1) {
		return 0;
	}

	/* Base case: len of 2 with delimiter is decoded to [] */
	if (src->len == 2 && (flags & COBS_FLAG_DELIMITER)) {
		return 0;
	}
	...
	while (src->len > 0) {
		...
		if (offset == delimiter && !(flags & COBS_FLAG_DELIMITER)) {
			return -EINVAL;
		}

		/* Copy data if offset > 1 */
		if (offset > 1) {
...
{
	return cobs_decode_delimiter(src, dst, flags, 0);
}
