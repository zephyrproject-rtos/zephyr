...
static inline unsigned int _bitpool_find_lsb_set(atomic_val_t val)
{
	...
	if (sizeof(val) == sizeof(int)) {
		ret = __builtin_ffs(val);
	} else if (sizeof(val == sizeof(long))) {
		ret = __builtin_ffsl(val);
	} else if (sizeof(val) == sizeof(long long)) {
		ret = __builtin_ffsll(val);
	} else {
...
{
	size_t cnt = ATOMIC_BITMAP_SIZE(source->bitcnt);
	const atomic_t *s = source->bitmap;

	K_SPINLOCK(source->spinlock) {
		while (cnt--) {
			*target++ = atomic_get(s++);
		}
	}
...
void bitpool_atomic_write_internal(const struct bitpool_atomic *target,
				   const atomic_val_t *source)
{
	size_t cnt = ATOMIC_BITMAP_SIZE(target->bitcnt);
	atomic_t *t = target->bitmap;

	K_SPINLOCK(target->spinlock) {
		while (cnt--) {
			atomic_set(t++, *source++);
		}
	}
				 ...
				 const atomic_val_t *new_value)
{
	...
	K_SPINLOCK(target->spinlock) {
		status = _bitpool_cmp(target->bitmap, old_value, cnt);
		if (status) {
			status = _bitpool_cpy(target->bitmap, old_value, new_value, cnt);
			     ...
			     const size_t bitcnt)
{
	...
	while (1) {
		while (((first_zero_bit = _bitpool_find_lsb_set(~part) - 1) < 0) &&
		       (n < bitmap_size)) {
			part = source[n++];
			if (val) {
				part = ~part;
