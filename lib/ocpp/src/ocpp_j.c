			       ...
			       ocpp_session_t *ses)
{
	...
	ret = frame_rpc_call_req(buf, len, Authorize, (uint32_t)ses,
				 auth);
	if (ret) {
		goto out;
	}
			       ...
			       ocpp_session_t *ses)
{
	...
	ret = frame_rpc_call_req(buf, len, Heartbeat,
				 (uint32_t)ses, hb);
	if (ret) {
		json_object_put(hb);
	}
			       ...
			       ocpp_cp_info_t *cpi)
{
	...
	ret = frame_rpc_call_req(buf, len, BootNotification,
				 (uint32_t)ses, txn);
	if (ret) {
		goto out;
	}
...
static int frame_meter_val_msg(char *buf, int len, ocpp_session_t *ses,
...
static int frame_stop_txn_msg(char *buf, int len, ocpp_session_t *ses,
			      int Wh, char *reason, char *timestamp)
{
	...
	ret = frame_rpc_call_req(buf, len, StopTransaction,
				 (uint32_t)ses, txn);
	if (ret) {
		goto out;
	}
...
static int frame_start_txn_msg(char *buf, int len, ocpp_session_t *ses,
			       int Wh, int reserv_id, char *timestamp)
{
	...
	ret = frame_rpc_call_req(buf, len, StartTransaction, (uint32_t)ses,
				 txn);
	if (ret) {
		goto out;
	}
			    ...
			    ocpp_idtag_info_t *idtag_info)
{
	...
	switch(*str) {
	case 'A':	//accepted
		idtag_info->auth_status = AUTH_ACCEPTED;
		break;

	case 'B':	//blocked
		idtag_info->auth_status = AUTH_BLOCKED;
		break;

	case 'E':	//expired
		idtag_info->auth_status = AUTH_EXPIRED;
		break;

	case 'I':	//invalid
		idtag_info->auth_status = AUTH_INVALID;
		break;

	case 'C':	//concurrenttx
		idtag_info->auth_status = AUTH_CONCURRENT_TX;
		break;
...
static int parse_authorize_msg(char *buf, ocpp_idtag_info_t *idtag_info)
{
        struct json_object *root;
	int ret = 0;
...
static int parse_bootnotification_msg(char *buf, boot_notif_t *binfo)
{
        struct json_object *root;
	struct json_object *tmp;
	char *str;
...
static ocpp_msg_fp_t ocpp_json_parser[PduMsgEnd] = {
	[BootNotification]	= (ocpp_msg_fp_t)parse_bootnotification_msg,
	[Authorize]		= (ocpp_msg_fp_t)parse_authorize_msg,
	[StartTransaction]	= (ocpp_msg_fp_t)parse_start_txn_msg,
	[StopTransaction]	= (ocpp_msg_fp_t)parse_authorize_msg,
	[MeterValues]		= NULL,
	[Heartbeat]		= (ocpp_msg_fp_t)parse_heartbeat_msg,
	[GetConfiguration]	= (ocpp_msg_fp_t)parse_getconfig_msg,
	[ChangeConfiguration]	= (ocpp_msg_fp_t)parse_changeconfig_msg,
	[RemoteStartTransaction]= (ocpp_msg_fp_t)parse_remote_start_txn_msg,
	[RemoteStopTransaction]	= (ocpp_msg_fp_t)parse_remote_stop_txn_msg,
	[UnlockConnector]	= (ocpp_msg_fp_t)parse_unlock_connectormsg,
};

static ocpp_msg_fp_t ocpp_json_frame[PduMsgEnd] = {
	[BootNotification]	= (ocpp_msg_fp_t)frame_bootnotif_msg,
	[Authorize]		= (ocpp_msg_fp_t)frame_authorize_msg,
	[StartTransaction]	= (ocpp_msg_fp_t)frame_start_txn_msg,
	[StopTransaction]	= (ocpp_msg_fp_t)frame_stop_txn_msg,
	[MeterValues]		= (ocpp_msg_fp_t)frame_meter_val_msg,
	[Heartbeat]		= (ocpp_msg_fp_t)frame_heartbeat_msg,
	[GetConfiguration]	= (ocpp_msg_fp_t)frame_getconfig_msg,
	[ChangeConfiguration]	= (ocpp_msg_fp_t)frame_status_resp_msg,
	[RemoteStartTransaction]= (ocpp_msg_fp_t)frame_status_resp_msg,
	[RemoteStopTransaction]	= (ocpp_msg_fp_t)frame_status_resp_msg,
	[UnlockConnector]	= (ocpp_msg_fp_t)frame_status_resp_msg,
};
