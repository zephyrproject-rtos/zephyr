					 ...
					 const struct heaptrace_record_info *info)
{
	shell_print(sh,
		    ...
		    info->ptr, info->size, heaptrace_heap_id_to_name(info->heap_id),
...
static bool heaptrace_heapinfo_cb(const struct heaptrace_heap_info *info, void *user_data)
{
	...
	shell_print(sh,
		    ...
		    heaptrace_heap_id_to_name(info->heap_id), (unsigned long)info->heap_id,
...
static bool heaptrace_resize_dump_cb(const struct heaptrace_resize_info *info, void *user_data)
{
	...
	shell_print(sh,
		    ...
		    heaptrace_heap_id_to_name(info->heap_id), info->old_end, info->new_end,
