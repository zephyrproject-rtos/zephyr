...
LOG_MODULE_REGISTER(heaptrace, CONFIG_HEAPTRACE_LOG_LEVEL);
...
#if defined(CONFIG_HEAPTRACE_BACKTRACE)
	/* The event passed the filter and will be recorded: drop the lock and
	 * unwind outside the critical section.  This callback already fires
	 * from inside the allocator's own lock, and capturing under both
	 * locks with interrupts disabled made every allocation pay for a full
	 * unwind in a critical section.  Events discarded above (resize
	 * halves, filtered-out allocations) skip the unwind entirely.
	 */
	k_spin_unlock(&heaptrace_lock, key);
