...
#if defined(CONFIG_HEAPTRACE_BACKTRACE)
	/* The event passed the filter and will be recorded: drop heaptrace
	 * around the backtrace capture so the unwind does not nest inside an
	 * extra lock.  This callback still runs under heap_listener_lock
	 * (interrupts masked) -- the unlock here removes one nesting level,
	 * not the critical section.  Events discarded above (resize halves,
	 * filtered-out allocations) skip the unwind entirely.
	 */
	k_spin_unlock(&heaptrace_lock, key);
