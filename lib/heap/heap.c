...
static inline void check_chunk_canary(struct z_heap *h, chunkid_t c, void *mem)
{
	verify_chunk_canary(h, c, mem);

	/* Poison canary to detect double-free */
...
#define check_chunk_canary(h, c, mem) do { } while (false)
