	...
	do {
		...
		if (state_has_error(state)
			...
			if ((state & ST_CHANGING_MASK) == ST_OFF) {
				state = ST_CHANGING_FROM_OFF | ST_BIT_UNSUBMITTED;
			} else if ((state & ST_CHANGING_MASK) == ST_CHANGING_FROM_OFF) {
				state &= ~ST_BIT_CANCEL;
			} else if ((state & ST_CHANGING_MASK) == ST_CHANGING_FROM_ON) {
				state |= ST_BIT_CANCEL;
			} else {
				rc = -EALREADY;
			}
			...
			if ((state & ST_CHANGING_MASK) == ST_ON) {
				state = ST_CHANGING_FROM_ON | ST_BIT_UNSUBMITTED;
			} else if ((state & ST_CHANGING_MASK) == ST_CHANGING_FROM_ON) {
				state &= ~ST_BIT_CANCEL;
			} else if ((state & ST_CHANGING_MASK) == ST_CHANGING_FROM_OFF) {
				state |= ST_BIT_CANCEL;
			...
			if (state_has_error(state)) {
				...
				state |= ST_BIT_RESET | ST_BIT_CHANGING | ST_BIT_UNSUBMITTED;
			} else {
				rc = -EALREADY;
			}
			...
			if (res < 0) {
				...
				if (cancel) {
					loop = true;
					evt = (state == ST_ON) ? EVT_RELEASE : EVT_REQUEST;
				} else {
					state |= ST_BIT_UNNOTIFIED;
				}
				PRINTK("pev[%d] settle %s : %s\n", depth, state_s(state), cancel ? " CANCEL" : "settle");
