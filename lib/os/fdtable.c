...
void zvfs_libc_file_alloc_cb(int fd, const char *mode, uintptr_t fp)
{
	ARG_UNUSED(fd);
	ARG_UNUSED(mode);
	ARG_UNUSED(fp);
...
uintptr_t zvfs_libc_file_alloc(int fd, const char *mode, uintptr_t *fp, k_timeout_t timeout)
{
	ARG_UNUSED(mode);
	ARG_UNUSED(timeout);

	return (uintptr_t)&fdtable[fd];
}

void zvfs_libc_file_free(uintptr_t fp)
{
	ARG_UNUSED(fp);
}
