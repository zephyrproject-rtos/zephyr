...
#define SERVICE_REFS_MAX ((uint16_t)UINT16_MAX)
			 ...
			 struct onoff_client *cli)
{
	...
	if ((rv == 0)
	    && ((cli->notify.flags
		 & ~BIT_MASK(ONOFF_CLIENT_EXTENSION_POS)) != 0U)) {
		rv = -EINVAL;
	}
			  ...
			  k_spinlock_key_t key)
{
	...
	uint16_t state = mgr->flags & ONOFF_STATE_MASK;
	int res = 0;
	bool processing = ((mgr->flags & ONOFF_FLAG_PROCESSING) != 0U);
	...
	do {
		 ...
		 * over recheck.
		 */
		if ((mgr->flags & ONOFF_FLAG_COMPLETE) != 0U) {
			mgr->flags &= (uint16_t)~ONOFF_FLAG_COMPLETE;
			evt = EVT_COMPLETE;
		} else if ((mgr->flags & ONOFF_FLAG_RECHECK) != 0U) {
