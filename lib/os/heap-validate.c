 ...
 * bit...
 */
static inline void check_nexts(struct z_heap *h, unsigned int bidx)
{
	struct z_heap_bucket *b = &h->buckets[bidx];

	bool emptybit = (h->avail_buckets & ((uint32_t)1 << bidx)) == 0U;
	bool emptylist = b->next == 0U;
...
bool sys_heap_validate(struct sys_heap *heap)
{
	 ...
	 */
	for (unsigned int b = 0; b <= bucket_idx(h, h->end_chunk); b++) {
		chunkid_t c0 = h->buckets[b].next;
		uint32_t n = 0;

		check_nexts(h, b);

		for (c = c0; c != 0U && (n == 0U || c != c0);
		...
		bool empty = (h->avail_buckets & ((uint32_t)1 << b)) == 0U;
		bool zero = n == 0U;
	 ...
	 * Mark them USED.
	 */
	for (unsigned int b = 0U; b <= bucket_idx(h, h->end_chunk); b++) {
		chunkid_t c0 = h->buckets[b].next;
		int n = 0;
...
static bool rand_alloc_choice(struct z_heap_stress_rec *sr)
{
	...
	if (sr->blocks_alloced == 0U) {
		...
		uint32_t full_pct = (uint32_t)((100U * sr->bytes_alloced) / sr->total_bytes);
...
static size_t rand_alloc_size(struct z_heap_stress_rec *sr)
{
	 ...
	 */
	unsigned int scale = 4U + (unsigned int)__builtin_clz(rand32());

	return rand32() & (((size_t)1U << scale) - 1U);
}
	...
	*result = (struct z_heap_stress_result) {0};
	...
	for (uint32_t i = 0; i < op_count; i++) {
		if (rand_alloc_choice(&sr)) {
			...
			result->total_frees++;
			sr.blocks[b] = sr.blocks[sr.blocks_alloced - 1U];
			sr.blocks_alloced--;
			sr.bytes_alloced -= sz;
			sr.free_fn(sr.arg, p);
		}
...
{
	unsigned int i, nb_buckets = bucket_idx(h, h->end_chunk) + 1U;
	...
	for (i = 0; i < nb_buckets; i++) {
		...
		if (count != 0U) {
			printk("%9u %12u %12u %12u %12zu\n",
			       i, ((chunksz_t)1 << i) - 1U + min_chunk_size(h), count,
	...
	printk("\n%zd free bytes, %zd allocated bytes, overhead = %zd bytes (%zd.%zd%%)\n",
	       ...
	       (1000U * overhead + total/2U) / total / 10U,
	       (1000U * overhead + total/2U) / total % 10U);
}
