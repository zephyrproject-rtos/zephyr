...
static void _assert_complete(struct pixel_stream *strm, bool begin)
{
	for (; strm != NULL; strm = strm->next) {
		...
		__ASSERT(ring_buf_size_get(&strm->ring) == 0,
			 "Core %s did not empty its input buffer, %u bytes left",
			 strm->name, ring_buf_size_get(&strm->ring));
...
static inline void pixel_stream_to_frame(const uint8_t *src_buf, size_t src_size,
					 ...
					 uint16_t dst_width, int bytes_per_pixel, va_list ap)
{
	...
	/* Connect the steps of the stream together */
	for (struct pixel_stream *next = step; next != NULL; next = va_arg(ap, void *)) {
		step = step->next = next;
	}
			       ...
			       uint8_t *dst_buf, size_t dst_size, uint16_t dst_width, ...)
{
	...
	va_start(ap, dst_width);
	pixel_stream_to_frame(src_buf, src_size, src_width, dst_buf, dst_size, dst_width, 1, ap);
	va_end(ap);
}
				...
				uint8_t *dst_buf, size_t dst_size, uint16_t dst_width, ...)
{
	...
	va_start(ap, dst_width);
	pixel_stream_to_frame(src_buf, src_size, src_width, dst_buf, dst_size, dst_width, 2, ap);
	va_end(ap);
}
				...
				uint8_t *dst_buf, size_t dst_size, uint16_t dst_width, ...)
{
	...
	va_start(ap, dst_width);
	pixel_stream_to_frame(src_buf, src_size, src_width, dst_buf, dst_size, dst_width, 3, ap);
	va_end(ap);
}
