...
static ssize_t pipe_read_vmeth(void *obj, void *buffer, size_t count)
{
	...
	if (((*ptr & FS_O_READ) == 0) || atomic_get(&p_pipe_desc->readOpened) == 0) {
		errno = EACCES;
		return -1;
	}
	...
	if ((*ptr & O_NONBLOCK) == O_NONBLOCK) {
		...
		while (count > 0 && atomic_get(&p_pipe_desc->writeOpened) == 1) {
			ssize_t read = 0;
...
static ssize_t pipe_write_vmeth(void *obj, const void *buffer, size_t count)
{
	...
	if (((*ptr & FS_O_WRITE) == 0) || atomic_get(&p_pipe_desc->writeOpened) == 0) {
		errno = EACCES;
		return -1;
	}
	...
	if ((*ptr & O_NONBLOCK) == O_NONBLOCK) {
		...
		while (count > 0 && atomic_get(&p_pipe_desc->readOpened) == 1) {
			ssize_t written = 0;
...
static int pipe_close_vmeth(void *obj)
{
	...
	if ((*ptr & FS_O_WRITE)) {
		p_pipe_desc =
			(struct pipe_desc *)((char *)ptr - offsetof(struct pipe_desc, flags_write));
		atomic_clear(&p_pipe_desc->writeOpened);
		k_sem_give(&p_pipe_desc->sync);
	} else if ((*ptr & FS_O_READ) == FS_O_READ) {
		p_pipe_desc =
			(struct pipe_desc *)((char *)ptr - offsetof(struct pipe_desc, flags_read));
		atomic_clear(&p_pipe_desc->readOpened);
		k_sem_give(&p_pipe_desc->sync);
	} else {
		errno = EINVAL;
		return -1;
	...
	if (atomic_get(&p_pipe_desc->readOpened) == 0 &&
	    atomic_get(&p_pipe_desc->writeOpened) == 0) {
		k_pipe_flush(&p_pipe_desc->pipe);
		atomic_clear(&p_pipe_desc->isUsed);
	}
...
int pipe(int fildes[2])
{
	...
	for (i = 0; i < CONFIG_POSIX_PIPES_MAX; i++) {
		if (atomic_cas(&pipe_desc_array[i].isUsed, 0, 1)) {
			break;
		}
	}
	...
	atomic_set(&pipe_desc_array[i].readOpened, 1);
	atomic_set(&pipe_desc_array[i].writeOpened, 1);
