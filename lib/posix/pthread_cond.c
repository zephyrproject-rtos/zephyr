 ...
 */
BUILD_ASSERT(CONFIG_MAX_PTHREAD_COND_COUNT < PTHREAD_COND_MASK_INIT,
...
{
	return mark_pthread_cond_uninitialized(cond);
}
...
struct posix_cond *get_posix_cond(pthread_cond_t cond)
{
	...
	if (!is_pthread_cond_initialized(cond)) {
		return NULL;
	}
...
struct posix_cond *to_posix_cond(pthread_cond_t *cvar)
{
	...
	*cvar = mark_pthread_cond_initialized(bit);
