...
# Normalize paths to use forward slashes to avoid CMake escape sequence issues on Windows
file(TO_CMAKE_PATH "${APP_DIR}" APP_DIR_NORMALIZED)
file(TO_CMAKE_PATH "${APPLICATION_SOURCE_DIR}" APPLICATION_SOURCE_DIR_NORMALIZED)
file(TO_CMAKE_PATH "${DOTCONFIG}" DOTCONFIG_NORMALIZED)
file(TO_CMAKE_PATH "${KCONFIG_BOARD_DIR}" KCONFIG_BOARD_DIR_NORMALIZED)
file(TO_CMAKE_PATH "${KCONFIG_BINARY_DIR}" KCONFIG_BINARY_DIR_NORMALIZED)
file(TO_CMAKE_PATH "${EDT_PICKLE}" EDT_PICKLE_NORMALIZED)
file(TO_CMAKE_PATH "${ZEPHYR_BASE}" ZEPHYR_BASE_NORMALIZED)
file(TO_CMAKE_PATH "${PYTHON_EXECUTABLE}" PYTHON_EXECUTABLE_NORMALIZED)
file(TO_CMAKE_PATH "${KCONFIG_ROOT}" KCONFIG_ROOT_NORMALIZED)
file(TO_CMAKE_PATH "${DTS_POST_CPP}" DTS_POST_CPP_NORMALIZED)
file(TO_CMAKE_PATH "${PROJECT_BINARY_DIR}" PROJECT_BINARY_DIR_NORMALIZED)
file(TO_CMAKE_PATH "${TOOLCHAIN_KCONFIG_DIR}" TOOLCHAIN_KCONFIG_DIR_NORMALIZED)

# Load the module Kconfig file into CMake
include("${KCONFIG_BINARY_DIR}/kconfig_module_dirs.cmake")

set(COMMON_KCONFIG_ENV_SETTINGS
  PYTHON_EXECUTABLE=${PYTHON_EXECUTABLE_NORMALIZED}
  srctree=${ZEPHYR_BASE_NORMALIZED}
  ${kconfig_env_dirs}
  ...
  APP_DIR=${APP_DIR_NORMALIZED}
  CONFIG_=${KCONFIG_NAMESPACE}_
  KCONFIG_CONFIG=${DOTCONFIG_NORMALIZED}
  KCONFIG_BOARD_DIR=${KCONFIG_BOARD_DIR_NORMALIZED}
  BOARD=${BOARD}
  BOARD_REVISION=${BOARD_REVISION}
  BOARD_QUALIFIERS=${BOARD_QUALIFIERS}
  HWM_SCHEME=${HWM}
  KCONFIG_BINARY_DIR=${KCONFIG_BINARY_DIR_NORMALIZED}
  APPLICATION_SOURCE_DIR=${APPLICATION_SOURCE_DIR_NORMALIZED}
  ZEPHYR_TOOLCHAIN_VARIANT=${ZEPHYR_TOOLCHAIN_VARIANT}
  TOOLCHAIN_KCONFIG_DIR=${TOOLCHAIN_KCONFIG_DIR_NORMALIZED}
  TOOLCHAIN_HAS_NEWLIB=${_local_TOOLCHAIN_HAS_NEWLIB}
  TOOLCHAIN_HAS_PICOLIBC=${_local_TOOLCHAIN_HAS_PICOLIBC}
  EDT_PICKLE=${EDT_PICKLE_NORMALIZED}
...
list(APPEND COMMON_KCONFIG_ENV_SETTINGS
  ARCH=*
  ARCH_DIR=${ZEPHYR_BASE_NORMALIZED}/arch
)
...
set(EXTRA_KCONFIG_TARGET_COMMAND_FOR_menuconfig
  ${ZEPHYR_BASE_NORMALIZED}/scripts/kconfig/menuconfig.py
  )

set(EXTRA_KCONFIG_TARGET_COMMAND_FOR_guiconfig
  ${ZEPHYR_BASE_NORMALIZED}/scripts/kconfig/guiconfig.py
  )

set(EXTRA_KCONFIG_TARGET_COMMAND_FOR_hardenconfig
  ${ZEPHYR_BASE_NORMALIZED}/scripts/kconfig/hardenconfig.py
  )

set(EXTRA_KCONFIG_TARGET_COMMAND_FOR_traceconfig
  ${ZEPHYR_BASE_NORMALIZED}/scripts/kconfig/traceconfig.py
  ${DOTCONFIG_NORMALIZED}
  ${PROJECT_BINARY_DIR_NORMALIZED}/kconfig-trace.md
  )
