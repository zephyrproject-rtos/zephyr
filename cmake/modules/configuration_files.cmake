...
zephyr_get(APPLICATION_CONFIG_DIR SYSBUILD LOCAL)
...
function(zephyr_family_series_file)
  cmake_parse_arguments(ZFILE "DTS;KCONFIG" "PATH;CONF_FILE;SUFFIX" "" ${ARGN})
  if(ZFILE_UNPARSED_ARGUMENTS)
    message(FATAL_ERROR "zephyr_file(${ARGV0} <val> ...) given unknown arguments: ${ZFILE_UNPARSED_ARGUMENTS}")
  endif()

  if(NOT ZFILE_DTS AND NOT ZFILE_KCONFIG)
    message(FATAL_ERROR "zephyr_file(${ARGV0} <val> ...) given unknown arguments: ${ZFILE_UNPARSED_ARGUMENTS}")
  endif()
  ...
  if(ZFILE_KCONFIG)
    if(SOC_FAMILY)
      if(ZFILE_SUFFIX AND EXISTS ${ZFILE_PATH}/socs/families/${SOC_FAMILY}_${FILE_SUFFIX}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/families/${SOC_FAMILY}_${FILE_SUFFIX}.conf)
      elseif(EXISTS ${ZFILE_PATH}/socs/families/${SOC_FAMILY}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/families/${SOC_FAMILY}.conf)
      endif()
    endif()

    if(SOC_SERIES)
      if(ZFILE_SUFFIX AND EXISTS ${ZFILE_PATH}/socs/series/${SOC_SERIES}_${FILE_SUFFIX}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/series/${SOC_SERIES}_${FILE_SUFFIX}.conf)
      elseif(EXISTS ${ZFILE_PATH}/socs/series/${SOC_SERIES}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/series/${SOC_SERIES}.conf)
      endif()
    endif()

    list(PREPEND extra_files ${CONF_FILE})
    set(CONF_FILE ${extra_files} PARENT_SCOPE)
  elseif(ZFILE_DTS)
    if(SOC_FAMILY)
      if(ZFILE_SUFFIX AND EXISTS ${ZFILE_PATH}/socs/families/${SOC_FAMILY}_${FILE_SUFFIX}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/families/${SOC_FAMILY}_${FILE_SUFFIX}.overlay)
      elseif(EXISTS ${ZFILE_PATH}/socs/families/${SOC_FAMILY}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/families/${SOC_FAMILY}.overlay)
      endif()
    endif()

    if(SOC_SERIES)
      if(ZFILE_SUFFIX AND EXISTS ${ZFILE_PATH}/socs/series/${SOC_SERIES}_${FILE_SUFFIX}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/series/${SOC_SERIES}_${FILE_SUFFIX}.overlay)
      elseif(EXISTS ${ZFILE_PATH}/socs/series/${SOC_SERIES}.conf)
        list(APPEND extra_files ${ZFILE_PATH}/socs/series/${SOC_SERIES}.overlay)
      endif()
    endif()

    list(PREPEND extra_files ${DTC_OVERLAY_FILE})
    set(DTC_OVERLAY_FILE ${extra_files} PARENT_SCOPE)
  endif()
endfunction()
...
if(NOT DEFINED CONF_FILE)
  ...
  zephyr_family_series_file(KCONFIG PATH ${APPLICATION_CONFIG_DIR} SUFFIX ${FILE_SUFFIX})
...
if(NOT DEFINED DTC_OVERLAY_FILE)
  zephyr_family_series_file(DTS PATH ${APPLICATION_CONFIG_DIR} SUFFIX ${FILE_SUFFIX})
