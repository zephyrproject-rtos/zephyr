...
function(zephyr_mcuboot_tasks)
  ...
  if(NOT "${CONFIG_MCUBOOT_GENERATE_UNSIGNED_IMAGE}")
    ...
    foreach(file keyfile keyfile_enc)
      ...
      #  3. board
      #  4. west topdir (this one might not exist if west is not installed)
      #
      if(NOT IS_ABSOLUTE "${${file}}")
        if(EXISTS "${APPLICATION_CONFIG_DIR}/${${file}}")
          set(${file} "${APPLICATION_CONFIG_DIR}/${${file}}")
        elseif(EXISTS "${BOARD_DIR}/${${file}}")
          set(${file} "${BOARD_DIR}/${${file}}")
        else()
          ...
          if(NOT WEST OR NOT WEST_TOPDIR)
            message(FATAL_ERROR "Can't sign images for MCUboot: west not found. "
                                "To fix, install west and ensure it's on PATH.")
          endif()
      ...
      if(NOT EXISTS "${${file}}")
        message(FATAL_ERROR "Can't sign images for MCUboot: can't find file ${${file}} "
                            ...
                            "BOARD_DIR=\"${BOARD_DIR}\", and WEST_TOPDIR=\"${WEST_TOPDIR}\")")
      endif()
    endforeach()
  endif()
