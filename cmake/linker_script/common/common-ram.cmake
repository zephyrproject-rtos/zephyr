...
if(CONFIG_DCACHE_LINE_SIZE)
  # Resolve __DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME from sections.h so the
  # CMake generator emits exactly the same section name as the .ld scripts.
  if(NOT DEFINED __DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME)
    zephyr_get_include_directories_for_lang(C current_includes)

    file(WRITE ${CMAKE_CURRENT_BINARY_DIR}/dcacheline_data_probe.c
        "#include <zephyr/linker/sections.h>\n__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME\n")
    ...
    execute_process(
      COMMAND ${CMAKE_C_COMPILER} -E -P
              ...
              -include ${AUTOCONF_H}
              ${CMAKE_CURRENT_BINARY_DIR}/dcacheline_data_probe.c
    ...
    if(probe_rc EQUAL 0 AND probe_out MATCHES "^[A-Za-z_][A-Za-z0-9_]*$")
      set(__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME "${probe_out}")
    else()
      message(FATAL_ERROR "Could not extract __DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME "
              "from'zephyr/linker/sections'")
    endif()
  endif()

  zephyr_linker_section(NAME ".${__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME}" GROUP DATA_REGION
    SUBALIGN ${CONFIG_DCACHE_LINE_SIZE} NOINPUT ${XIP_ALIGN_WITH_INPUT} ENDALIGN ${CONFIG_DCACHE_LINE_SIZE}
  )
  zephyr_linker_section_configure(SECTION ".${__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME}"
    ALIGN ${CONFIG_DCACHE_LINE_SIZE}
    SYMBOLS __dcacheline_exclusive_data_start
  )
  zephyr_linker_section_configure(SECTION .${__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME}
  ...
  zephyr_linker_section_configure(SECTION ".${__DCACHELINE_EXCLUSIVE_DATA_SECTION_NAME}"
    ALIGN ${CONFIG_DCACHE_LINE_SIZE}
    SYMBOLS __dcacheline_exclusive_data_end
  )
endif()
...
if(CONFIG_FIDO2)
  zephyr_iterable_section(NAME fido2_transport GROUP DATA_REGION ${XIP_ALIGN_WITH_INPUT})
endif()
