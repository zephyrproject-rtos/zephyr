...
static struct wifi_eap_config eap_config[] = {
	...
	"PEAP", "auth=TLS"},
	{ WIFI_SECURITY_TYPE_EAP_TLS_SHA256,    WIFI_EAP_TYPE_TLS,  WIFI_EAP_TYPE_NONE,
	"TLS",  NULL},
};
#endif
	...
	if (params->security != WIFI_SECURITY_TYPE_NONE) {
		...
		if (params->security == WIFI_SECURITY_TYPE_SAE_HNP ||
			...
			if (params->suiteb_type == WIFI_SUITEB_192) {
				if (params->TLS_cipher == WIFI_EAP_TLS_ECC_P384) {
					if (!wpa_cli_cmd_v("set_network %d openssl_ciphers \"%s\"",
					    resp.network_id, cipher_config.openssl_ciphers))
						goto out;
			...
			if (enterprise_creds.client_cert2_len) {
				...
				if (!wpa_cli_cmd_v(
					    "set_network %d client_cert2 \"blob://client_cert\"",
					    resp.network_id)) {
					goto out;
				}
			}
			...
			if (enterprise_creds.client_key2_len) {
				...
				if (!wpa_cli_cmd_v(
					    "set_network %d private_key2 \"blob://private_key\"",
					    resp.network_id)) {
					goto out;
				}
