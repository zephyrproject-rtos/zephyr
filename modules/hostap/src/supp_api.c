...
static struct wifi_eap_config eap_config[] = {
	...
	"PEAP", "auth=MSCHAPV2"},
	{ WIFI_SECURITY_TYPE_EAP_PEAP_GTC, WIFI_EAP_TYPE_PEAP, WIFI_EAP_TYPE_GTC, "PEAP", "auth=GTC"},
	{ WIFI_SECURITY_TYPE_EAP_TTLS_MSCHAPV2, WIFI_EAP_TYPE_TTLS, WIFI_EAP_TYPE_NONE, "TTLS",
	"auth=MSCHAPV2"},
	{ WIFI_SECURITY_TYPE_EAP_PEAP_TLS, WIFI_EAP_TYPE_PEAP, WIFI_EAP_TYPE_TLS, "PEAP", "auth=TLS"},
	{ WIFI_SECURITY_TYPE_EAP_TLS_SHA256,    WIFI_EAP_TYPE_TLS,  WIFI_EAP_TYPE_NONE,	"TLS", NULL},
};

int process_cipher_config(struct wifi_connect_req_params *params, struct wifi_eap_cipher_config *cipher_config)
{
	...
	if (params->suiteb_type == WIFI_SUITEB) {
		...
		cipher_config->key_mgmt = (params->ft_used ? "FT-EAP" : "WPA-EAP");
	}
...
{
	if (security == WIFI_SECURITY_TYPE_EAP_TLS ||
				       ...
				       bool mode_ap)
{
...
#ifdef CONFIG_WIFI_NM_WPA_SUPPLICANT_CRYPTO_ENTERPRISE
	struct wifi_eap_cipher_config cipher_config = {NULL,
	    "DEFAULT:!EXP:!LOW", "CCMP", "CCMP", "AES-128-CMAC", NULL};
	char *method;
	char phase1[256] = {0};
	char *phase2 = NULL;
	unsigned int index;
	...
	if (params->security != WIFI_SECURITY_TYPE_NONE) {
		...
		if (params->security == WIFI_SECURITY_TYPE_SAE_HNP ||
			...
			if (!wpa_cli_cmd_v("set openssl_ciphers \"%s\"", cipher_config.openssl_ciphers))
				goto out;
