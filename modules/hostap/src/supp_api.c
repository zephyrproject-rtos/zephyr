...
int supplicant_ap_config_params(const struct device *dev, struct wifi_ap_config_params *params)
{
	...
	if (params->type & WIFI_AP_CONFIG_PARAM_MAX_INACTIVITY) {
		...
		if (ret) {
			wpa_printf(MSG_ERROR, "Failed to set maximum inactivity duration for stations");
		}
		else {
			wpa_printf(MSG_INFO, "Set maximum inactivity duration for stations: %d (s)", params->max_inactivity);
		}
	}
