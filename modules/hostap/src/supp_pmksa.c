				       ...
				       size_t entry_count)
{
	...
	for (i = 0; i < entry_count; i++) {
		...
		for (existing = wpa_sm_pmksa_cache_head(wpa_s->wpa); existing != NULL;
			...
			if (memcmp(existing->aa, converted->aa, ETH_ALEN) == 0 &&
			    memcmp(existing->spa, converted->spa, ETH_ALEN) == 0) {
				if (existing->pmk_len == converted->pmk_len &&
				    memcmp(existing->pmk, converted->pmk, existing->pmk_len) == 0 &&
				    memcmp(existing->pmkid, converted->pmkid, PMKID_LEN) == 0) {
