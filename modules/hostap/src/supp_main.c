...
int zephyr_wifi_send_event(const struct wpa_supplicant_event_msg *msg)
{
	...
	if (ctx->supplicant != msg->ctx) {
		LOG_WRN("Supplicant context mismatch!");
		return -ENOENT;
	}
...
struct wpa_supplicant *zephyr_get_handle_by_ifname(const char *ifname)
{
	...
	ret = k_sem_take(&interface_ready, K_SECONDS(2));
	if (ret) {
...
{
	// FIXME, cannot access ifaces as it is supplicant internal data
