...
static int mspi_if_acquire(struct mspi_dev_cfg *cfg)
{
	...
	if (cfg == active_cfg) {
		...
		/* First acquisition: apply the full static configuration
		 * (frequency, CE number/polarity, clock phase, endianness,
		 * DQS, ...) instead of only the register/data-specific
		 * fields.
		 */
