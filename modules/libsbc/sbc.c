		...
		uint32_t bit_rate)
{
	...
	encoder->sbc_encoder_params.s16ChannelMode = (int16_t)ch_mode;
	encoder->sbc_encoder_params.s16NumOfSubBands = (int16_t)subband;
	encoder->sbc_encoder_params.s16NumOfBlocks = (int16_t)blk_len;
	encoder->sbc_encoder_params.s16AllocationMethod = (int16_t)alloc_mthd;
	switch (samp_freq) {
	case 16000u:
...
int sbc_encode(struct sbc_encoder *encoder, const void *pcm, uint32_t nbytes, void *out)
{
	...
	ret = SBC_Encode(&encoder->sbc_encoder_params, (int16_t*)pcm, out);
	if (ret != nbytes) {
		return -1;
	}
	return 0;
