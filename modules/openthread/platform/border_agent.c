...
static bool s_border_agent_is_init;

#if defined(CONFIG_OPENTHREAD_BORDER_AGENT_EPHEMERAL_KEY_ENABLE)
/* Byte values, 9 bytes for the key, one for null terminator. */
static uint8_t s_ephemeral_key_string[10];

static uint32_t s_ephemeral_key_timeout;
static bool     s_epskc_active;

static otError generate_ephemeral_key(void);
static void    handle_border_agent_ephemeral_key_callback(void *aContext);
...
void border_agent_init(otInstance *instance)
{
	...
	if (!s_border_agent_is_init) {
		uint8_t txt_buffer[128] = {0};
		uint16_t txt_buffer_len = 0;

		otBorderAgentSetMeshCoPServiceBaseName(instance, base_service_instance_name);
		append_vendor_txt_data(txt_buffer, &txt_buffer_len);
		otBorderAgentSetVendorTxtData(ot_instance, txt_buffer, txt_buffer_len);
#if defined(CONFIG_OPENTHREAD_BORDER_AGENT_EPHEMERAL_KEY_ENABLE)
...
{
#if defined(CONFIG_OPENTHREAD_BORDER_AGENT_EPHEMERAL_KEY_ENABLE)
	otBorderAgentEphemeralKeySetEnabled(ot_instance, false);
	s_epskc_active = false;
#endif /* CONFIG_OPENTHREAD_BORDER_AGENT_EPHEMERAL_KEY_ENABLE */
	s_border_agent_is_init = false;
}
...
static void append_vendor_txt_data(uint8_t *txt_data, uint16_t *txt_data_len)
{
	*txt_data_len = 0;
	...
	otDnsTxtEntry txt_entries[] = {
				       {.mKey = "vn", .mValue = (uint8_t *)vendor_name,
					.mValueLength = strlen(vendor_name)},
				       {.mKey = "mn", .mValue = (uint8_t *)model_name,
					.mValueLength = strlen(model_name)}};
 ...
 */
void __attribute__((weak)) print_ephemeral_key(const char *ephemeral_key, uint32_t timeout)
{
...
void __attribute__((weak)) print_ephemeral_key_expired_message(void)
{
...
static otError generate_ephemeral_key(void)
{
	...
	memset(s_ephemeral_key_string, 0, sizeof(s_ephemeral_key_string));

	VerifyOrExit(otPlatEntropyGet((uint8_t *)&random, sizeof(random)) == OT_ERROR_NONE,
		     error = OT_ERROR_FAILED);
	random %= 100000000;
	i += snprintf((char *)s_ephemeral_key_string, sizeof(s_ephemeral_key_string),
		      "%08u", random);
	VerifyOrExit(otVerhoeffChecksumCalculate((const char *)s_ephemeral_key_string,
		     &verhoeff_checksum) == OT_ERROR_NONE,
		     error = OT_ERROR_FAILED);
	i += snprintf((char *)&s_ephemeral_key_string[i], sizeof(s_ephemeral_key_string) - i,
		      "%c", verhoeff_checksum);
exit:
	return error;
}
...
static void handle_border_agent_ephemeral_key_callback(void *context)
{
	...
	case OT_BORDER_AGENT_STATE_STARTED:
		snprintf(formatted_epskc, sizeof(formatted_epskc), "%.3s %.3s %.3s",
			s_ephemeral_key_string, s_ephemeral_key_string + 3,
			s_ephemeral_key_string + 6);
		print_ephemeral_key(formatted_epskc, (uint32_t)(s_ephemeral_key_timeout / 1000UL));
		s_epskc_active = true;
		break;
...
static otError border_agent_enable_epskc_service(uint32_t timeout)
{
	...
	VerifyOrExit(s_border_agent_is_init, error = OT_ERROR_INVALID_STATE);

	s_ephemeral_key_timeout = (timeout &&
	...
	error = otBorderAgentEphemeralKeyStart(ot_instance, (const char *)s_ephemeral_key_string,
					       s_ephemeral_key_timeout, 0);
