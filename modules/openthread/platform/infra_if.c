...
otError infra_if_init(otInstance *instance, struct net_if *ail_iface)
{
	...
	ail_iface_idx = (uint32_t)net_if_get_by_iface(ail_iface_ptr);
...
int handle_ra_input(struct net_icmp_ctx *ctx, struct net_pkt *pkt, struct net_icmp_ip_hdr *hdr,
		    struct net_icmp_hdr *icmp_hdr, void *user_data)
{
	...
	if (net_buf_linearize(req->buff_and_len.buffer, sizeof(req->buff_and_len.buffer),
			      pkt->buffer, 0, length) == length) {
		req->buff_and_len.length = length;
		memcpy(&req->addr, hdr->ipv6->src, sizeof(req->addr));
		req->cb = infra_if_handle_backbone_ra;
...
static void infra_if_handle_backbone_ra(struct br_msg_ctx_tag *msg_ctx_ptr)
{
	otPlatInfraIfRecvIcmp6Nd(
		ot_instance, ail_iface_idx, &msg_ctx_ptr->addr,
		(const uint8_t *)&msg_ctx_ptr->buff_and_len.buffer[sizeof(struct net_ipv6_hdr)],
		msg_ctx_ptr->buff_and_len.length);
}
...
otError infra_if_start_ra_listener(void)
{
	...
	VerifyOrExit(net_icmp_init_ctx(&ra_ctx, NET_ICMPV6_RA, 0, handle_ra_input) == 0,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(net_icmp_init_ctx(&rs_ctx, NET_ICMPV6_RS, 0, handle_ra_input) == 0,
		     error = OT_ERROR_FAILED);
