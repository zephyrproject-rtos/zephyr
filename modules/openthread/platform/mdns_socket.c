...
static void process_mdns_message(struct br_msg_ctx_tag *msg_ctx_ptr);
...
static otError mdns_socket_init(uint32_t ail_iface_idx)
{
	...
	mdns_sock = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	...
	VerifyOrExit(setsockopt(mdns_sock, SOL_SOCKET, SO_BINDTODEVICE, &if_req, sizeof(if_req)) ==
			     0,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(setsockopt(mdns_sock, SOL_SOCKET, SO_REUSEADDR, &on, sizeof(on)) == 0,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(setsockopt(mdns_sock, SOL_SOCKET, SO_REUSEPORT, &on, sizeof(on)) == 0,
		     error = OT_ERROR_FAILED);
	mreq.ipv6mr_ifindex = ail_iface_idx;
	net_ipv6_addr_create(&mreq.ipv6mr_multiaddr, 0xff02, 0, 0, 0, 0, 0, 0, 0x00fb);
	VerifyOrExit(
		setsockopt(mdns_sock, IPPROTO_IPV6, IPV6_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) == 0,
		error = OT_ERROR_FAILED);
	VerifyOrExit(setsockopt(mdns_sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &mcast_hops,
				sizeof(mcast_hops)) == 0,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(setsockopt(mdns_sock, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &mcast_hops,
				sizeof(mcast_hops)) == 0,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(bind(mdns_sock, (struct sockaddr *)&addr, sizeof(addr)) == 0,
		     error = OT_ERROR_FAILED);
...
static otError mdns_socket_deinit(void)
{
	...
	VerifyOrExit(close(mdns_sock) == 0, error = OT_ERROR_FAILED);
...
static void mdns_send_multicast(otMessage *message, uint32_t ail_iface_idx)
{
	...
	sendto(mdns_sock, buffer, length, 0, (struct sockaddr *)&addr, sizeof(addr));
exit:
...
static void mdns_receive_handler(struct net_socket_service_event *evt)
{
	...
	ssize_t len = 0;
	struct br_msg_ctx_tag *req = NULL;
	...
	memset(req, 0, sizeof(struct br_msg_ctx_tag));

	len = recvfrom(mdns_sock, req->buff_and_len.buffer, sizeof(req->buff_and_len.buffer), 0,
		       (struct sockaddr *)&addr, &addrlen);
	VerifyOrExit(len > 0);

	memcpy(&req->addr_info.mAddress, &addr.sin6_addr, sizeof(req->addr_info.mAddress));
	req->buff_and_len.length = (uint16_t)len;
	req->addr_info.mPort = htons(addr.sin6_port);
...
static void process_mdns_message(struct br_msg_ctx_tag *msg_ctx_ptr)
{
	...
	VerifyOrExit(otMessageAppend(ot_message, msg_ctx_ptr->buff_and_len.buffer,
				     msg_ctx_ptr->buff_and_len.length) == OT_ERROR_NONE);
