...
static uint32_t s_ail_iface_idx;
...
static otError mdns_socket_init_v6(uint32_t ail_iface_idx)
{
	...
	VerifyOrExit(zsock_setsockopt(mdns_sock_v6, IPPROTO_IPV6, IPV6_MULTICAST_IF,
				      &s_ail_iface_idx, sizeof(s_ail_iface_idx)) == 0,
		     error = OT_ERROR_FAILED);
...
static void mdns_send_multicast(otMessage *message, uint32_t ail_iface_idx)
{
	...
	VerifyOrExit(zsock_sendto(mdns_sock_v6, buffer, length, 0, (struct sockaddr *)&addr_v6,
		     sizeof(addr_v6)) > 0);
#if defined(CONFIG_NET_IPV4)
	VerifyOrExit(zsock_sendto(mdns_sock_v4, buffer, length, 0, (struct sockaddr *)&addr_v4,
		     sizeof(addr_v4)) > 0);
#endif /* CONFIG_NET_IPV4 */
exit:
...
#if defined(CONFIG_NET_IPV4)
	if (send_ipv4) {
		VerifyOrExit(zsock_sendto(mdns_sock_v4, buffer, length, 0,
...
static void mdns_receive_handler(struct net_socket_service_event *evt)
{
	...
	if (family == AF_INET) {
		...
		len = recvfrom(mdns_sock_v4, req->buffer, sizeof(req->buffer), 0,
		...
		len = recvfrom(mdns_sock_v6, req->buffer, sizeof(req->buffer), 0,
	...
	req->addr_info.mInfraIfIndex = s_ail_iface_idx;
