...
/* The only IRQ lines the blob attaches at runtime are the emb-side IPC
 * IRQs 61-63 (its intc_init hooks ipc_emb_msg_irq / ipc_emb_cfmback_irq
 * / ipc_emb_tx_irq); every other WiFi IRQ is owned and statically
 * connected by the port.  Route the runtime attach through fixed
 * trampolines connected at boot so the shim does not need
 * DYNAMIC_INTERRUPTS (whose RAM ISR table costs 8 bytes per IRQ).
 */
#define BL_IRQ_SLOT_FIRST 61
#define BL_IRQ_SLOT_CNT   3
...
static struct bl_irq_slot bl_irq_slots[BL_IRQ_SLOT_CNT];

static void bl_irq_trampoline(const void *param)
{
	const struct bl_irq_slot *s = param;
...
static int bl_irq_slots_connect(void)
{
	IRQ_CONNECT(BL_IRQ_SLOT_FIRST, BL_IRQ_PRIO, bl_irq_trampoline, &bl_irq_slots[0],
		    BL_IRQ_FLAGS);
	IRQ_CONNECT(BL_IRQ_SLOT_FIRST + 1, BL_IRQ_PRIO, bl_irq_trampoline, &bl_irq_slots[1],
		    BL_IRQ_FLAGS);
	IRQ_CONNECT(BL_IRQ_SLOT_FIRST + 2, BL_IRQ_PRIO, bl_irq_trampoline, &bl_irq_slots[2],
		    BL_IRQ_FLAGS);
	return 0;
}
SYS_INIT(bl_irq_slots_connect, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);

static void bl_irq_attach(int32_t n, void *f, void *arg)
{
	int idx = (int)n - BL_IRQ_SLOT_FIRST;

	LOG_DBG("irq_attach n=%d f=%p", (int)n, f);

	if ((idx < 0) || (idx >= BL_IRQ_SLOT_CNT)) {
		LOG_ERR("irq_attach: unexpected IRQ %d", (int)n);
		return;
	}

	bl_irq_slots[idx].fn = (void (*)(void *))f;
	bl_irq_slots[idx].arg = arg;
}
...
/* assert_err() reports the failing condition and then spins forever at
 * whatever interrupt level it was called from, which presents as a dead
 * CPU with no fault report.  Trap it so the condition is attributable.
 */
void __wrap_assert_err(const char *cond, const char *param, int line)
{
	printk("\n*** bl60x FW ASSERT: %s | %s | line %d ***\n", (cond != NULL) ? cond : "?",
	       (param != NULL) ? param : "?", line);
	k_panic();
}
 ...
 * operation; a ~100-character line is ~9ms of blocking console output in
 * the MAC's real-time path.
 */
