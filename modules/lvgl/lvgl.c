...
#define ALLOC_VTILE_BUFFER                                                                         \
	((IS_MONOCHROME_DISPLAY == 1) && (CONFIG_LV_Z_MONOCHROME_VTILE_CONVERSION_BUFFER == 1))
...
#if ALLOC_VTILE_BUFFER
static uint8_t mono_vtile_buf[BUFFER_SIZE]
#ifdef CONFIG_LV_Z_VBD_CUSTOM_SECTION
	Z_GENERIC_SECTION(.lvgl_buf)
#endif
		__aligned(CONFIG_LV_Z_VDB_ALIGN);
#endif /* ALLOC_VTILE_BUFFER */
...
#if ALLOC_VTILE_BUFFER
	lvgl_set_mono_vtile_buffer(mono_vtile_buf, BUFFER_SIZE);
#endif /* ALLOC_VTILE_BUFFER */
...
static int lvgl_allocate_rendering_buffers(lv_display_t *display)
{
	...
	lvgl_set_mono_vtile_buffer(vtile_buf, buf_size);
#endif /* ALLOC_VTILE_BUFFER */
