...
static struct k_mutex nema_mutexes[MUTEX_MAX + 1];
#endif
...
#if defined(NEMA_MULTI_PROCESS) || defined(NEMA_MULTI_THREAD)
	k_mutex_init(&nema_mutexes[MUTEX_RB]);
	k_mutex_init(&nema_mutexes[MUTEX_MALLOC]);
	k_mutex_init(&nema_mutexes[MUTEX_FLUSH]);
#endif
...
nema_buffer_t nema_buffer_create(int size)
{
	...
	LV_ASSERT_MSG(bo.base_virt != NULL,
...
void nema_buffer_flush(nema_buffer_t *bo)
{
	...
	sys_cache_data_flush_range((void *)bo->base_phys,
				   bo->size);
...
int nema_mutex_lock(int mutex_id)
{
	...
	if ((mutex_id >= 0) && (mutex_id <= MUTEX_MAX)) {
		ret = (k_mutex_lock(&nema_mutexes[mutex_id],
				    K_FOREVER) == 0) ? 0 : -1;
	}
...
int nema_mutex_unlock(int mutex_id)
{
	...
	if ((mutex_id >= 0) && (mutex_id <= MUTEX_MAX)) {
		ret = (k_mutex_unlock(&nema_mutexes[mutex_id]) == 0) ? 0 : -1;
	}
