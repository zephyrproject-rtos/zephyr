...
#if defined(CONFIG_LOCAL_DOMAIN_DVFS_TEST)
	LOG_DBG("%s", __func__);
...
#define DOWNSCALE_SAFETY_TIMEOUT (K_USEC(CONFIG_LOCAL_DOMAIN_DOWNSCALE_SAFETY_TIMEOUT_US))
...
int32_t ld_dvfs_configure_hsfll(enum dvfs_frequency_setting oppoint)
{
	...
	} else if (NRF_DOMAIN == NRF_DOMAIN_APPLICATION) {
...
int ld_dvfs_wait_abb_statusana_locked(void)
{
	bool abb_locked = false;

#if defined(CONFIG_LOCAL_DOMAIN_DVFS_TEST)
	abb_locked = true;
#else
	/* TODO: Do we need to check ABB_STATUSANA_STATUSVALID_Msk? */
	NRFX_WAIT_FOR((NRF_ABB->STATUSANA & ABB_STATUSANA_LOCKED_Msk),
		      ABB_STATUS_CHANGE_CHECK_MAX_ATTEMPTS,
		      ABB_STATUS_CHANGE_CHECK_INTERVAL_US,
		      abb_locked);

	LOG_DBG("NRF_ABB->STATUS 0x%x\n", NRF_ABB->STATUS);
	LOG_DBG("NRF_ABB->STATUSANA 0x%x\n", NRF_ABB->STATUSANA);

#endif

	return abb_locked ? 0 : (-ETIMEDOUT);
}
