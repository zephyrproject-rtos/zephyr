...
#if DT_NODE_HAS_PROP(DT_NODELABEL(uicr), nfct_pins_as_gpios)
#define NRF_CONFIG_NFCT_PINS_AS_GPIOS
#endif

#if DT_NODE_HAS_PROP(DT_NODELABEL(uicr), gpio_as_nreset)
#define NRF_CONFIG_GPIO_AS_PINRESET
#endif

#endif /* DT_NODE_EXISTS(DT_NODELABEL(uicr)) */

#if DT_NODE_EXISTS(DT_NODELABEL(tampc)) && DT_NODE_HAS_PROP(DT_NODELABEL(tampc), swd_pins_as_gpios)
