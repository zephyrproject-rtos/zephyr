...
static void bt_nxp_setup_sco(struct bt_conn *acl_conn, struct bt_hci_cp_setup_sync_conn *cp)
{
...
static void bt_nxp_accept_sco(struct bt_hci_cp_accept_sync_conn_req *cp)
{
	uint16_t voice_setting;
...
BT_SCO_HCI_CB_DEFINE(hci_nxp_sco_hci_cbs) = {
	.setup = bt_nxp_setup_sco,
	.accept = bt_nxp_accept_sco,
};
