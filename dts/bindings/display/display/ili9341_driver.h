...
#define ILI9341_REGS_INIT(inst)                                                                       \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, gamset)) == ILI9341_GAMSET_LEN,        \
		     "ili9341: Error length gamma set (GAMSET) register");                         \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, ifmode)) == ILI9341_IFMODE_LEN,        \
		     "ili9341: Error length frame rate control (IFMODE) register");                \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, frmctr1)) == ILI9341_FRMCTR1_LEN,      \
		     "ili9341: Error length frame rate control (FRMCTR1) register");               \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, disctrl)) == ILI9341_DISCTRL_LEN,      \
		     "ili9341: Error length display function control (DISCTRL) register");         \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pwctrl1)) == ILI9341_PWCTRL1_LEN,      \
		     "ili9341: Error length power control 1 (PWCTRL1) register");                  \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pwctrl2)) == ILI9341_PWCTRL2_LEN,      \
		     "ili9341: Error length power control 2 (PWCTRL2) register");                  \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, vmctrl1)) == ILI9341_VMCTRL1_LEN,      \
		     "ili9341: Error length VCOM control 1 (VMCTRL1) register");                   \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, vmctrl2)) == ILI9341_VMCTRL2_LEN,      \
		     "ili9341: Error length VCOM control 2 (VMCTRL2) register");                   \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pgamctrl)) == ILI9341_PGAMCTRL_LEN,    \
		     "ili9341: Error length positive gamma correction (PGAMCTRL) register");       \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, ngamctrl)) == ILI9341_NGAMCTRL_LEN,    \
		     "ili9341: Error length negative gamma correction (NGAMCTRL) register");       \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pwctrla)) == ILI9341_PWCTRLA_LEN,      \
		     "ili9341: Error length power control A (PWCTRLA) register");                  \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pwctrlb)) == ILI9341_PWCTRLB_LEN,      \
		     "ili9341: Error length power control B (PWCTRLB) register");                  \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pwseqctrl)) == ILI9341_PWSEQCTRL_LEN,  \
		     "ili9341: Error length power on sequence control (PWSEQCTRL) register");      \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, timctrla)) == ILI9341_TIMCTRLA_LEN,    \
		     "ili9341: Error length driver timing control A (TIMCTRLA) register");         \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, timctrlb)) == ILI9341_TIMCTRLB_LEN,    \
		     "ili9341: Error length driver timing control B (TIMCTRLB) register");         \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, pumpratioctrl)) ==                     \
			     ILI9341_PUMPRATIOCTRL_LEN,                                            \
		     "ili9341: Error length Pump ratio control (PUMPRATIOCTRL) register");         \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, enable3g)) == ILI9341_ENABLE3G_LEN,    \
		     "ili9341: Error length enable 3G (ENABLE3G) register");                       \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, ifctl)) == ILI9341_IFCTL_LEN,          \
		     "ili9341: Error length frame rate control (IFCTL) register");                 \
	BUILD_ASSERT(DT_PROP_LEN(DT_INST_PROP(inst, etmod)) == ILI9341_ETMOD_LEN,          \
