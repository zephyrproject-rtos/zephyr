...
#define UPLOAD_FLASH_AREA FLASH_AREA(image_1_nonsecure)
#else
...
int flash_img_init_id(struct flash_img_context *ctx, uint8_t area_id)
{
	...
	return (rc != 0) ? rc : flash_img_init_area(ctx, fa);
}
...
int flash_img_init_area(struct flash_img_context *ctx, const struct flash_area *fa)
{
	...
	return stream_flash_init(&ctx->stream, fa->fa_dev, ctx->buf,
...
{
	return flash_img_init_area(ctx, UPLOAD_FLASH_AREA);
}
