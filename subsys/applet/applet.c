...
/* k_object_alloc() applies this for us; the plain heap path must do it itself. */
#ifdef ARCH_DYNAMIC_OBJ_K_THREAD_ALIGNMENT
#define APPLET_THREAD_ALIGN ARCH_DYNAMIC_OBJ_K_THREAD_ALIGNMENT
#else
#define APPLET_THREAD_ALIGN __alignof__(struct k_thread)
#endif
...
#else
	slot->thread = k_heap_aligned_alloc(&applet_slot_heap, APPLET_THREAD_ALIGN,
