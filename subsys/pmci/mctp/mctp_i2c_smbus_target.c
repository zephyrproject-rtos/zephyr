...
{
	const uint8_t hdr[] = {
		(uint8_t)((dest_addr_7bit << 1) | 0U),
		cmd,
		count,
	};
	uint8_t crc = 0U;

	crc = crc8_ccitt(crc, hdr, sizeof(hdr));
	crc = crc8_ccitt(crc, data, count);

	return crc;
}

static struct mctp_binding_i2c_smbus_target *
mctp_i2c_smbus_target_from_config(struct i2c_target_config *cfg)
{
...
static struct mctp_binding_i2c_smbus_target *
mctp_i2c_smbus_target_from_work(struct k_work *work)
{
	return CONTAINER_OF(work, struct mctp_binding_i2c_smbus_target,
			    tx_work);
}

static struct mctp_binding_i2c_smbus_target *
mctp_i2c_smbus_target_from_binding(struct mctp_binding *binding)
{
	return CONTAINER_OF(binding, struct mctp_binding_i2c_smbus_target,
			    binding);
}
...
static int tgt_write_requested(struct i2c_target_config *config)
{
	struct mctp_binding_i2c_smbus_target *b =
		mctp_i2c_smbus_target_from_config(config);
...
{
	struct mctp_binding_i2c_smbus_target *b =
		mctp_i2c_smbus_target_from_work(work);
	uint8_t mctp_len;
	...
	memcpy(&out[3], b->tx_buf, mctp_len);
...
{
	struct mctp_binding_i2c_smbus_target *b =
		mctp_i2c_smbus_target_from_binding(binding);
	...
	/* TX runs from workqueue context, so keep a private copy before
	 * returning ownership of pkt to libmctp.
	 */
