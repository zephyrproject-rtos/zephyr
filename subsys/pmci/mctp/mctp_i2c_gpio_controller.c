...
static void rx_completion(struct rtio *r, const struct rtio_sqe *sqe, void *arg0)
{
	...
	gpio_pin_configure_dt(&b->endpoint_gpios[cb->index], GPIO_INT_ENABLE);
...
static const uint8_t mctp_tx_msg_len_addr = MCTP_I2C_GPIO_TX_MSG_LEN_ADDR;
static const uint8_t mctp_tx_msg_addr = MCTP_I2C_GPIO_TX_MSG_ADDR;
...
static void mctp_start_rx(struct mctp_binding_i2c_gpio_controller *b, bool chained_rx)
{
	...
	struct mpsc_node *node = mpsc_pop(&b->tx_q);
				 ...
				 gpio_port_pins_t pins)
{
	...
	gpio_pin_configure_dt(&b->endpoint_gpios[cb_data->index], GPIO_INT_DISABLE);

	mpsc_push(&b->tx_q, &cb_data->q);
...
int mctp_i2c_gpio_controller_tx(struct mctp_binding *binding, struct mctp_pktbuf *pkt)
{
	...
	/* TODO have a semaphore to queue threads on this section */

	/* Asynchronous bindings may return -EBUSY */
	if (rtio_sqe_acquirable(b->r_tx) < 4) {
		LOG_ERR("not enough sqes");
		return -EBUSY;
	}
	...
	write_data_sqe->iodev_flags |=  RTIO_IODEV_I2C_RESTART | RTIO_IODEV_I2C_STOP;
...
int mctp_i2c_gpio_controller_start(struct mctp_binding *binding)
{
	...
	for (int i = 0; i < b->num_endpoints; i++) {
		gpio_pin_interrupt_configure_dt(&b->endpoint_gpios[i], GPIO_INT_EDGE_TO_ACTIVE);
	}
