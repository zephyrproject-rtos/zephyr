...
/* Set by the linker script. A level ends where the next one starts. */
extern const struct safety_test __safety_test_start[];
extern const struct safety_test __safety_test_EARLY_start[];
extern const struct safety_test __safety_test_PRE_KERNEL_1_start[];
extern const struct safety_test __safety_test_PRE_KERNEL_2_start[];
extern const struct safety_test __safety_test_POST_KERNEL_start[];
extern const struct safety_test __safety_test_APPLICATION_start[];
extern const struct safety_test __safety_test_end[];

static const struct safety_test *const levels[] = {
	__safety_test_EARLY_start,
	__safety_test_PRE_KERNEL_1_start,
	__safety_test_PRE_KERNEL_2_start,
	__safety_test_POST_KERNEL_start,
	__safety_test_APPLICATION_start,
	__safety_test_end,
};

#define SAFETY_TEST_FOREACH(_test)                                                                 \
	for (const struct safety_test *_test = __safety_test_start; _test < __safety_test_end; \
	     _test++)
...
static void invoke_failure_hooks(const struct safety_test_context *ctx,
				 const struct safety_test_result_record *rec)
{
...
__weak enum safety_test_action safety_test_safe_state(const struct safety_test_context *ctx,
						      const struct safety_test_result_record *rec)
{
	ARG_UNUSED(ctx);
	ARG_UNUSED(rec);
	...
	if (IS_ENABLED(CONFIG_SAFETY_TEST_DEFAULT_ACTION_CONTINUE)) {
				    ...
				    enum safety_test_init_level level)
{
	...
	const bool can_log = safety_test_can_log(level);
	...
	action = safety_test_safe_state(&ctx, rec);
				       ...
				       struct safety_test_result_record *out)
{
	 ...
	 */
	timed = safety_test_can_log(level);
	 ...
	 */
	rec.over_budget = timed && test->max_duration_us != 0 &&
			  ...
			  struct safety_test_result_record *out)
{
	...
	/* Written up front so an early return still leaves the caller a defined
	 * record rather than whatever was on its stack.
	 */
	if (out != NULL) {
		*out = rec;
	}
	...
	if (runtime_allowed(test, opts)) {
		...
		if (rec.result == SAFETY_TEST_RESULT_FAIL &&
			...
			handle_critical_failure(test, &rec, SAFETY_TEST_LEVEL_APPLICATION);
		}
...
int safety_test_run_level(enum safety_test_init_level level, struct safety_test_stats *stats)
{
	...
	for (const struct safety_test *test = levels[level]; test < levels[level + 1];
	     test++) {
		...
		if (r == SAFETY_TEST_RESULT_FAIL &&
		    (test->flags & SAFETY_TEST_FLAG_CRITICAL) != 0) {
			handle_critical_failure(test, &rec, level);
		}
	}
