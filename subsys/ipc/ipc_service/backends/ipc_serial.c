				...
				size_t payload_len)
{
	...
	if (remote_local_id == 0U) {
		LOG_WRN("Received invalid remote ID");
		return;
	}
...
static void process_frame(struct serial_data *data, const uint8_t *frame, size_t frame_len)
{
	...
	case SERIAL_CMD_DATA: {
		...
		const struct ipc_ept_cfg *cfg = NULL;
		void *priv = NULL;
		...
		if (cfg->cb.received != NULL) {
			cfg->cb.received(&payload[1], payload_len - 1, priv);
		}
		break;
	}
	default:
...
static void rx_work_handler(struct k_work *work)
{
	...
	while ((claimed = ring_buf_get_claim(&data->rx_ringbuf, &buf, UINT32_MAX)) > 0) {
		uint32_t consumed = 0;

		while (consumed < claimed) {
			int ret = cobs_decoder_write(&data->decoder, &buf[consumed],
						     claimed - consumed);
		...
		ring_buf_get_finish(&data->rx_ringbuf, claimed);
	}
}
...
static int serial_send(const struct device *instance, void *token, const void *msg, size_t len)
{
	...
	if (!data->opened || !ept->bound || ept->remote_id == 0) {
		return -ENOTCONN;
	}
