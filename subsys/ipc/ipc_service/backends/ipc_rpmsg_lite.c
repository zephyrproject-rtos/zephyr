...
static bool check_endpoints_freed(struct ipc_rpmsg_lite_instance *ipc_rpmsg_inst)
{
	...
	for (size_t i = 0; i < NUM_ENDPOINTS; i++) {
		...
		if (atomic_get(&rpmsg_ept->bound) == 1) {
			return false;
		}
	}
			  ...
			  struct ipc_rpmsg_lite_ept *ipc_rpmsg_ept, const char *name, uint32_t dest)
{
	...
	atomic_set(&ipc_rpmsg_ept->bound, 1);
...
static int ept_cb(void *payload, uint32_t payload_len, uint32_t src, void *priv)
{
	 ...
	 */
	if (payload_len == 0) {
		if (atomic_get(&ept->bound) == 0) {
			if (ept->dest == RL_ADDR_ANY) {
				ept->dest = src;
			}
			atomic_set(&ept->bound, 1);
			bound_cb(ept);
		}
		return RL_SUCCESS;
	}
...
void platform_notify(uint32_t vector_id)
{
	 ...
	 * The notification owner (the instance without a notification-parent DT
	 * property) holds the shared TX channel used for all notifications.
	 * The remote side demultiplexes using RL_GET_LINK_ID(vector_id) in
	 * mbox_callback().
	 */
	const struct backend_config_t *owner = NULL;

	for (int i = 0; i < NUM_INSTANCES; i++) {
		if (g_inst_conf_ref[i] && g_inst_conf_ref[i]->is_notification_owner) {
			owner = g_inst_conf_ref[i];
			break;
		}
	}

	if (owner == NULL || !owner->mbox_tx.dev) {
		return;
	}
...
static int mbox_init(const struct device *instance)
{
	...
	if (IS_ENABLED(CONFIG_THREAD_NAME)) {
		...
		k_thread_name_set(k_work_queue_thread_get(&data->mbox_wq), name);
	}
...
register_ept_on_host(struct ipc_rpmsg_lite_instance *ipc_rpmsg_inst, const struct ipc_ept_cfg *cfg)
{
	...
	atomic_set(&ipc_rpmsg_ept->bound, 0);
...
static int send_nocopy(const struct device *instance, void *token, const void *msg, size_t len)
{
	...
	ret = rpmsg_lite_send_nocopy(ipc_rpmsg_lite_inst->rpmsg_lite_inst, ipc_rpmsg_ept->ep,
...
static int open(const struct device *instance)
{
	...
	if (!atomic_cas(&data->state, STATE_READY, STATE_OPENING)) {
		return -EALREADY;
	}
	...
	if (conf->role == ROLE_HOST) {
		...
		ipc_rpmsg_inst->rpmsg_lite_inst = rpmsg_lite_master_init(
			(void *)conf->shm_addr, conf->shm_size, conf->link_id, RL_NO_FLAGS, NULL);
#else
		ipc_rpmsg_inst->rpmsg_lite_inst = rpmsg_lite_master_init(
			(void *)conf->shm_addr, conf->shm_size, conf->link_id, RL_NO_FLAGS);
#endif
...
#else
		ipc_rpmsg_inst->rpmsg_lite_inst = rpmsg_lite_remote_init(
			(void *)conf->shm_addr, conf->link_id, RL_NO_FLAGS);
#endif
	}
...
static int close(const struct device *instance)
{
	...
	if (!atomic_cas(&data->state, STATE_INITED, STATE_CLOSING)) {
		return -EALREADY;
	}
	...
	/*
	 * Tear down the MBOX layer first (disable -> drain -> stop WQ).
	 * This prevents pending work items (queued before MBOX disable) from
	 * executing after the RPMSG-Lite layer is destroyed.
	 */
	err = mbox_deinit(instance);
	if (err != 0) {
		/* Nothing destroyed yet, safe to restore */
		atomic_set(&data->state, STATE_INITED);
		return err;
	}
	...
	if (err != 0) {
		/* MBOX and WQ are gone; let open() attempt full reinit. */
		atomic_set(&data->state, STATE_READY);
		return err;
	}

	ipc_rpmsg_inst->rpmsg_lite_inst = NULL;

	/* Reset instance state for a potential future open(). */
	memset(ipc_rpmsg_inst, 0, sizeof(*ipc_rpmsg_inst));
	k_mutex_init(&ipc_rpmsg_inst->mtx);
...
static int backend_init(const struct device *instance)
{
	...
	/*
	 * Device-tree assertions:
	 * - Notification owner (the instance without notification-parent) must provide mboxes.
	 * - Non-owner instances (with notification-parent) must not provide mboxes.
	 */
	if (conf->is_notification_owner) {
		__ASSERT(conf->mbox_tx.dev != NULL && conf->mbox_rx.dev != NULL,
			 "Notification owner must provide 'mboxes'");
	} else {
		__ASSERT(conf->mbox_tx.dev == NULL && conf->mbox_rx.dev == NULL,
			 "Non-owner instances must not provide 'mboxes' (use 'notification-parent')");
	}
...
#define DEFINE_BACKEND_DEVICE(i)                                                               \
	static struct backend_config_t backend_config_##i = {                                  \
		...
		.mbox_tx = COND_CODE_1(DT_INST_NODE_HAS_PROP(i, mboxes),                       \
	...
	DEVICE_DT_INST_DEFINE(i, &backend_init, NULL, &backend_data_##i,                       \
			      &backend_config_##i, POST_KERNEL,                                \
			      CONFIG_IPC_SERVICE_REG_BACKEND_PRIORITY, &backend_ops);
/* clang-format on */
