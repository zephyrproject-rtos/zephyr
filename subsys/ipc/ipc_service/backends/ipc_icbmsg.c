...
#define SINGLE_SYMMETRIC_INST                                                                      \
	COND_CODE_1(DT_NUM_INST_STATUS_OKAY(zephyr_ipc_icbmsg),                                    \
		(IS_EQ(DT_INST_PROP(0, tx_blocks), DT_INST_PROP(0, rx_blocks))), (0))

#if !SINGLE_SYMMETRIC_INST
#define ASYMMETRIC_INST 1
#endif
...
/* If single and symmetric instance, use fixed slot count. */
#define GET_SLOT_COUNT(val)                                                                        \
	COND_CODE_1(SINGLE_SYMMETRIC_INST,                                                         \
		((uint32_t)ROUND_UP(DT_INST_PROP(0, tx##_blocks), 4)), (val))
...
{
	const struct icbmsg_config *config = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(config->rx_msg_q.slot_count);
	struct icbmsg_data *data = instance->data;
	uint8_t idx;
...
static void msg_q_garbage_collect(const struct device *instance)
{
	struct icbmsg_data *data = instance->data;
	const struct icbmsg_config *config = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(config->tx_msg_q.slot_count);
	uint32_t remote_idx;
	uint32_t local_idx;
	uint32_t cnt;
...
static int open(const struct device *instance)
{
	...
	LOG_DBG("Shm_q TX prod: %p, cons: %p, slot_count: %u", (void *)config->tx_msg_q.prod_shmq,
		(void *)config->tx_msg_q.cons_shmq, GET_SLOT_COUNT(config->tx_msg_q.slot_count));
	LOG_DBG("Shm_q RX prod: %p, cons: %p, slot_count: %u", (void *)config->rx_msg_q.prod_shmq,
		(void *)config->rx_msg_q.cons_shmq, GET_SLOT_COUNT(config->rx_msg_q.slot_count));
...
{
	const struct icbmsg_config *conf = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(conf->tx_msg_q.slot_count);
	struct icbmsg_data *data = instance->data;
