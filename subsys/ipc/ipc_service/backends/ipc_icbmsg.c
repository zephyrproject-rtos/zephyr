...
struct icbmsg_shm_q {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, slots);
};
...
#define GET_INST_SLOT_COUNT(i, dir) ROUND_UP(DT_INST_PROP(i, dir##_blocks), 4)

#define GET_SLOT_COUNT(cfg, dir)                                                                   \
	COND_CODE_1(DT_NUM_INST_STATUS_OKAY(zephyr_ipc_icbmsg),                                    \
		((uint32_t)GET_INST_SLOT_COUNT(0, dir)), (cfg->dir##_msg_q.slot_count))

BUILD_ASSERT(NUM_EPT <= EPT_CONTROL, "Too many endpoints");

/** Cache flush. */
#define MSG_QUEUE_CACHE_FLUSH(shmq, cfg, dir)                                                      \
	do {                                                                                       \
		sys_cache_data_flush_range((void *)shmq, sizeof(*shmq) + GET_SLOT_COUNT(cfg, dir));\
		compiler_barrier();                                                                \
	} while (0)

/** Cache invalidate. */
#define MSG_QUEUE_CACHE_INV(shmq, cfg, dir)                                                        \
	do {                                                                                       \
		sys_cache_data_invd_range((void *)shmq, sizeof(*shmq) + GET_SLOT_COUNT(cfg, dir)); \
		compiler_barrier();                                                                \
	} while (0)
...
static void msg_q_reset(const struct device *instance)
{
	...
	MSG_QUEUE_CACHE_FLUSH(config->tx_msg_q.prod_shmq, config, tx);
	MSG_QUEUE_CACHE_FLUSH(config->rx_msg_q.cons_shmq, config, rx);
}
...
{
	const struct icbmsg_config *config = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(config, rx);
	struct icbmsg_data *data = instance->data;
	uint8_t idx;
...
static void msg_q_garbage_collect(const struct device *instance)
{
	struct icbmsg_data *data = instance->data;
	const struct icbmsg_config *config = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(config, tx);
	uint32_t remote_idx;
	uint32_t local_idx;
	uint32_t cnt;

	MSG_QUEUE_CACHE_INV(config->tx_msg_q.cons_shmq, config, tx);
...
static void handle_pending_messages(const struct device *instance)
{
	...
	MSG_QUEUE_CACHE_INV(config->rx_msg_q.prod_shmq, config, rx);
...
static int open(const struct device *instance)
{
	...
	LOG_DBG("Shm_q TX prod: %p, cons: %p, slot_count: %u", (void *)config->tx_msg_q.prod_shmq,
		(void *)config->tx_msg_q.cons_shmq, GET_SLOT_COUNT(config, tx));
	LOG_DBG("Shm_q RX prod: %p, cons: %p, slot_count: %u", (void *)config->rx_msg_q.prod_shmq,
		(void *)config->rx_msg_q.cons_shmq, GET_SLOT_COUNT(config, rx));
...
#define GET_MSG_Q_AREA_SIZE(i, dir) \
	CACHE_ALIGN(i, sizeof(struct icbmsg_shm_q) + GET_INST_SLOT_COUNT(i, dir))
...
#define DEFINE_BACKEND_DEVICE(i)                                                                   \
	...
	BUILD_ASSERT((GET_BLOCK_SIZE(i, tx) >= BLOCK_ALIGNMENT) &&                                 \
			     (GET_BLOCK_SIZE(i, tx) < GET_MEM_SIZE(i, tx)),                        \
		     "TX region is too small for provided number of blocks");                      \
	BUILD_ASSERT((GET_BLOCK_SIZE(i, rx) >= BLOCK_ALIGNMENT) &&                                 \
			     (GET_BLOCK_SIZE(i, rx) < GET_MEM_SIZE(i, rx)),                        \
...
{
	const struct icbmsg_config *conf = instance->config;
	uint32_t slot_count = GET_SLOT_COUNT(conf, tx);
	struct icbmsg_data *data = instance->data;
