...
static inline unsigned int _bitpool_find_lsb_set(atomic_val_t val)
{
	...
	unsigned int lsb_set_pos;

	if (sizeof(val) == sizeof(int)) {
		lsb_set_pos = __builtin_ffs(val);
	} else if (sizeof(val == sizeof(long))) {
		lsb_set_pos = __builtin_ffsl(val);
	} else if (sizeof(val) == sizeof(long long)) {
		lsb_set_pos = __builtin_ffsll(val);
	} else {
	...
	if (sizeof(val) == sizeof(int)) {
		...
		op = op ^ (op & (op - 1));
		if (op == 0) {
			msb_set_pos = ATOMIC_BITS;
		} else {
			msb_set_pos = __builtin_clz(op);
		}
	} else if (sizeof(val) == sizeof(long)) {
		...
		if (op == 0) {
			...
			msb_set_pos = __builtin_clzl(op);
		}
	} else if (sizeof(val) == sizeof(long long)) {
		unsigned long long op = val;

		op = op ^ (op & (op - 1));
		if (op == 0) {
			msb_set_pos = ATOMIC_BITS;
		} else {
			msb_set_pos = __builtin_clzll(op);
		}
	} else {
		__ASSERT(0, "Unexpected size of the atomic value");
		return -1;
