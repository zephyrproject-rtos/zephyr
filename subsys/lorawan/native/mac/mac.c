 ...
 *
 * A symbol timeout closes an idle RX window before the next window.
			     ...
			     int64_t tx_done, const char *win_name)
{
	...
	symbol_timeout = RX_MIN_SYMBOLS +
		DIV_ROUND_UP(2U * RX_SETUP_MARGIN_MS * rx_dr->bw, BIT(rx_dr->sf));

	timeout = radio_airtime_params(rx_dr->sf, rx_dr->bw,
				       rx_dr->max_payload + PHY_OVERHEAD);
	timeout += RX_TIMEOUT_MARGIN_MS;

	rx_ret = radio_rx(rx_freq, rx_dr, symbol_timeout, timeout,
