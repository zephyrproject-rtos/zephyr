...
/**
 * @brief LoRaWAN records shared by MAC and storage backends.
 *
 * Storage backends map these IDs to their own keys. Keep existing values stable
 * when adding records, as a storage backend may use the IDs directly.
 */
enum lorawan_nvm_id {
	LORAWAN_NVM_CRYPTO,
	LORAWAN_NVM_MAC_GROUP1,
	LORAWAN_NVM_MAC_GROUP2,
	LORAWAN_NVM_SECURE_ELEMENT,
	LORAWAN_NVM_REGION_GROUP1,
	LORAWAN_NVM_REGION_GROUP2,
	LORAWAN_NVM_CLASS_B,
	LORAWAN_NVM_DEV_NONCE,
};
 ...
 *
 * MAC backends own serialization and validation of record contents.
 ...
 *
 * MAC backends decide when state must be stored and how to handle failures.
 * Write durability follows the selected storage backend's guarantees.
 *
 * @param id Record identifier.
