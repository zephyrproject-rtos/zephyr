...
LOG_MODULE_REGISTER(mp_aud_i2s_src, CONFIG_MP_LOG_LEVEL);
...
static void mp_aud_i2s_src_update_caps(struct mp_src *src);

/* The base class's callback takes no direction, so bind it to receive. */
static int mp_aud_i2s_src_get_audio_caps(const struct device *dev, struct audio_caps *caps)
{
	return i2s_get_caps(dev, caps, I2S_DIR_RX);
}

static int mp_aud_i2s_src_set_property(struct mp_object *obj, uint32_t key, const void *val)
{
	struct mp_aud_i2s_src *self = (struct mp_aud_i2s_src *)obj;

	switch (key) {
	case MP_PROP_AUD_I2S_SRC_CODEC_DEVICE:
		self->codec_dev = (const struct device *)val;

		/* Codec set, update supported caps */
		mp_aud_i2s_src_update_caps(&self->aud_src.src);
		break;
	case MP_PROP_AUD_I2S_SRC_CLK_ROLE:
		...
		self->clk_role = (enum mp_aud_i2s_codec_clk_role)(uintptr_t)val;
		break;
	default:
...
static int mp_aud_i2s_src_get_property(struct mp_object *obj, uint32_t key, void *val)
{
	struct mp_aud_i2s_src *self = (struct mp_aud_i2s_src *)obj;
	...
	case MP_PROP_AUD_I2S_SRC_CODEC_DEVICE:
		*(const struct device **)val = self->codec_dev;
		break;
	case MP_PROP_AUD_I2S_SRC_CLK_ROLE:
		*(enum mp_aud_i2s_codec_clk_role *)val = self->clk_role;
		break;
	default:
 ...
 */
static struct mp_caps *mp_aud_i2s_src_supported_caps(struct mp_src *src)
{
	struct mp_aud_i2s_src *self = (struct mp_aud_i2s_src *)src;
	...
	if (self->codec_dev == NULL) {
		return mp_aud_src_supported_caps(src);
	}
	...
	if (i2s_get_caps(pool->aud_dev, &caps_eff, I2S_DIR_RX) != 0) {
...
static void mp_aud_i2s_src_update_caps(struct mp_src *src)
{
	struct mp_caps *caps = mp_aud_i2s_src_supported_caps(src);

	mp_src_update_caps(src, caps);
	mp_caps_unref(caps);
}

static int mp_aud_i2s_src_set_caps(struct mp_src *src, struct mp_caps *caps)
{
	struct mp_aud_i2s_src *self = (struct mp_aud_i2s_src *)src;
	struct mp_aud_buffer_pool *pool = CONTAINER_OF(src->pool, struct mp_aud_buffer_pool, pool);
	...
	if (self->codec_dev != NULL) {
		...
		if (self->clk_role == MP_AUD_I2S_TARGET_CODEC_CONTROLLER) {
...
static int mp_aud_i2s_src_acquire_buffer(struct mp_buffer_pool *pool, struct net_buf **buffer)
{
...
static int mp_aud_i2s_src_start(struct mp_buffer_pool *pool)
{
	struct mp_aud_buffer_pool *aud_pool = CONTAINER_OF(pool, struct mp_aud_buffer_pool, pool);
...
static int mp_aud_i2s_src_stop(struct mp_buffer_pool *pool)
{
	struct mp_aud_buffer_pool *aud_pool = CONTAINER_OF(pool, struct mp_aud_buffer_pool, pool);
...
void mp_aud_i2s_src_init(struct mp_element *self)
{
	...
	struct mp_aud_i2s_src *aud_i2s_src = (struct mp_aud_i2s_src *)src;

	/* Init base class */
	mp_aud_src_init(&aud_i2s_src->aud_src.src.element);
	...
	aud_i2s_src->clk_role = MP_AUD_I2S_CONTROLLER_CODEC_TARGET;

	aud_i2s_src->aud_src.get_audio_caps = mp_aud_i2s_src_get_audio_caps;

	src_parent_get_property = self->object.get_property;
	src_parent_set_property = self->object.set_property;
	self->object.get_property = mp_aud_i2s_src_get_property;
	self->object.set_property = mp_aud_i2s_src_set_property;

	src->set_caps = mp_aud_i2s_src_set_caps;
	src->pool->acquire_buffer = mp_aud_i2s_src_acquire_buffer;
	src->pool->start = mp_aud_i2s_src_start;

	pool_parent_stop = src->pool->stop;
	src->pool->stop = mp_aud_i2s_src_stop;

	mp_aud_i2s_src_update_caps(src);
}
