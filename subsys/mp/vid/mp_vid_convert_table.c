...
static int convert_nv12_to_rgb565(struct mp_vid_convert *conv, const struct net_buf *in,
				  struct net_buf *out)
{
...
static int convert_xrgb_to_argb(struct mp_vid_convert *conv, const struct net_buf *in,
				struct net_buf *out)
{
...
static int convert_argb_to_xrgb(struct mp_vid_convert *conv, const struct net_buf *in,
				struct net_buf *out)
{
	uint32_t pixels = (uint32_t)conv->width * (uint32_t)conv->height;
	const uint8_t *src = in->data;
...
const struct mp_vid_convert_desc mp_vid_convert_descs[] = {
...
const size_t mp_vid_convert_descs_len = ARRAY_SIZE(mp_vid_convert_descs);
