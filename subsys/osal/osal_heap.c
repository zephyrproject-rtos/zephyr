 ...
 * CONFIG_OSAL_MAX_OBJECTS. Queue ring storage is drawn from this same heap, so
 * a large queue consumes far more than one control block; size
 * CONFIG_OSAL_MAX_OBJECTS for the largest queue, not just the object count.
 */
