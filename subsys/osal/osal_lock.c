...
void osal_crit_exit(osal_spinlock_t lock)
{
	...
	/*
	 * Only the owning CPU may exit. A stray exit from a non-owner (or an
	 * exit without a matching enter) must not touch the owner-private count
	 * or restore another CPU's interrupt state.
	 */
	if (atomic_get(&c->owner_cpu) != (atomic_val_t)arch_curr_cpu()->id) {
		arch_irq_unlock(k);
		return;
	}
