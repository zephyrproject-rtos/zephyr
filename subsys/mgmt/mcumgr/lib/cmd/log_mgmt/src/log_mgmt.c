...
#define LOG_MGMT_HANDLER_CNT ARRAY_SIZE(log_mgmt_handlers)
...
static int
log_mgmt_encode_entry(CborEncoder *enc, const struct log_mgmt_entry *entry, size_t *out_len,
		      struct log_mgmt_enc_ctxt *lmec)
{
...
static int
log_encode_entries(const struct log_mgmt_log *log, CborEncoder *enc, int64_t timestamp,
		   uint32_t index)
{
	struct CborCntWriter cnt_writer;
	...
	err |= cbor_encoder_create_array(&cnt_encoder, &entries, CborIndefiniteLength);
	...
	rc = log_mgmt_impl_foreach_entry(log->name, &filter, log_mgmt_cb_encode, &ctxt);
	if (rc < 0) {
		/*
...
static int
log_encode(const struct log_mgmt_log *log, CborEncoder *ctxt, int64_t timestamp, uint32_t index)
{
	CborEncoder logs;
	CborError err;
	int rc;
...
log_mgmt_show(struct mgmt_ctxt *ctxt)
{
	...
	err |= cbor_encoder_create_array(&ctxt->encoder, &logs, CborIndefiniteLength);
...
log_mgmt_module_list(struct mgmt_ctxt *ctxt)
{
	...
	err |= cbor_encoder_create_map(&ctxt->encoder, &modules, CborIndefiniteLength);
...
log_mgmt_logs_list(struct mgmt_ctxt *ctxt)
{
	...
	err |= cbor_encoder_create_array(&ctxt->encoder, &log_list, CborIndefiniteLength);
...
log_mgmt_level_list(struct mgmt_ctxt *ctxt)
{
	...
	err |= cbor_encoder_create_map(&ctxt->encoder, &level_map, CborIndefiniteLength);
