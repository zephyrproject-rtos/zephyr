...
#ifdef CONFIG_MCUMGR_GRP_TRANSPORT
static bool smp_dummy_bridge_connect(struct smp_transport_bridge *bridge, bool direction,
...
static void smp_dummy_bridge_disconnect(struct smp_transport_bridge *bridge, bool direction)
{
	ARG_UNUSED(bridge);
	ARG_UNUSED(direction);
}
