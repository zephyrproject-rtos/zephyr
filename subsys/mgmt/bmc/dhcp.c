...
static void start_dhcpv4_client(struct net_if *iface, void *user_data)
{
	...
	LOG_INF("DHCPv4 start on %s[%d]", net_if_get_device(iface)->name,
...
static void restart_dhcpv4_client(struct net_if *iface, void *user_data)
{
	...
	LOG_INF("DHCPv4 restart on %s[%d]", net_if_get_device(iface)->name,
		net_if_get_by_iface(iface));
...
static void stop_dhcpv4_client(struct net_if *iface, void *user_data)
{
	...
	LOG_INF("DHCPv4 stop on %s[%d]", net_if_get_device(iface)->name,
		net_if_get_by_iface(iface));
	net_dhcpv4_stop(iface);
}

static void addr_add_handler(struct net_mgmt_event_callback *cb, uint64_t mgmt_event,
			     struct net_if *iface)
{
	ARG_UNUSED(cb);
	...
	for (int i = 0; i < NET_IF_MAX_IPV4_ADDR; i++) {
		...
		LOG_INF("New DHCPv4 lease on %s[%d]", net_if_get_device(iface)->name,
			net_if_get_by_iface(iface));
		LOG_INF("    Address: %s",
			   ...
			   enum net_dhcpv4_msg_type msg_type, struct net_if *iface)
{
	...
	LOG_DBG("DHCPv4 option %d: %s", cb->option,
...
{
	net_mgmt_init_event_callback(&mgmt_cb, addr_add_handler, NET_EVENT_IPV4_ADDR_ADD);
