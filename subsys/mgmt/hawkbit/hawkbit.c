				 ...
				 struct hawkbit_context *hb_context)
{
	...
	if (rsp->body_found) {
		...
		stream_flash_progress_save(&hb_context->flash_ctx.stream, "hawkbit_download");))
#endif
	}
...
static void s_probe(void *o)
{
	...
	if (s->hb_context.results.base._links.configData.href) {
	...
	if (s->hb_context.results.base._links.deploymentBase.href) {
...
static void s_cancel(void *o)
{
	...
	struct hawkbit_cancel cancel = {
		...
		.status.result.finished = hawkbit_status_finished(
...
static void s_download(void *o)
{
	...
	/* The area_id has to be fetched before the download starts
	 * because it is not available after the download is complete.
	 */
	const struct flash_area *flash_area_ptr = s->hb_context.flash_ctx.flash_area;
