...
#if defined(CONFIG_FOTA_HTTP_ASYNC)
...
BUILD_ASSERT(PARTITION_EXISTS(slot3_partition), "Image 1 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 3
BUILD_ASSERT(PARTITION_EXISTS(slot5_partition), "Image 2 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 4
BUILD_ASSERT(PARTITION_EXISTS(slot7_partition), "Image 3 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 5
BUILD_ASSERT(PARTITION_EXISTS(slot9_partition), "Image 4 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 6
BUILD_ASSERT(PARTITION_EXISTS(slot11_partition), "Image 5 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 7
BUILD_ASSERT(PARTITION_EXISTS(slot13_partition), "Image 6 has no secondary slot partition");
#endif
#if CONFIG_UPDATEABLE_IMAGE_NUMBER >= 8
BUILD_ASSERT(PARTITION_EXISTS(slot15_partition), "Image 7 has no secondary slot partition");
#endif
...
static int check_length(struct fota_http_ctx *dl)
{
	...
	ret = flash_area_read(fa, boot_get_image_start_offset(dl->area_id), &hdr, sizeof(hdr));
	flash_area_close(fa);
	if (ret != 0) {
		return ret;
	}
...
static int download_locked(const struct fota_http_download_params *params)
{
	...
	if (params->resume) {
		...
		if (ret != 0) {
			LOG_WRN("Failed to load the download progress (%d)", ret);
		} else if (flash_img_bytes_written(&dl->flash) == 0) {
			LOG_INF("No download progress to resume");
	...
	ret = check_image(dl);
	if (ret == -EILSEQ || ret == -EPERM) {
		LOG_INF("Erasing the rejected image");
		(void)boot_erase_img_bank(dl->area_id);
	}
	if (ret != 0) {
		goto out;
	}
...
int fota_http_apply(uint8_t image_index, bool permanent)
{
	...
	if (ctx.area_id == area_id && ctx.progress.written > 0 && ctx.result != 0) {
		LOG_ERR("The last download into flash area %d failed (%d)", area_id, ctx.result);
		return -ENOEXEC;
	}
