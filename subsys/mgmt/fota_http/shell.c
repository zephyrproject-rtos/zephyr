...
static void done_cb(int result, void *user_data)
{
	...
	shell_print(dl->sh, "Stored %zu bytes in the secondary slot",
...
static int cmd_download(const struct shell *sh, size_t argc, char **argv)
{
	...
	for (size_t i = 2; i < argc; i++) {
		if (strcmp(argv[i], "resume") == 0) {
			...
			params.resume = true;
#else
			shell_error(sh, "Resume support is not enabled");
			return -ENOTSUP;
#endif
...
static int cmd_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	slot = fota_http_get_download_slot(0);
	if (slot >= 0) {
		shell_print(sh, "Upload slot: flash area %d", slot);
	}

	ret = fota_http_get_image_header(0, &header);
	if (ret == 0) {
		size_t written = fota_http_nvm_bytes_written();
	...
	shell_print(sh, "Last download: %zu bytes", fota_http_nvm_bytes_written());
...
static int cmd_erase(const struct shell *sh, size_t argc, char **argv)
{
	...
	slot = fota_http_get_download_slot(image_index);
	if (slot < 0) {
