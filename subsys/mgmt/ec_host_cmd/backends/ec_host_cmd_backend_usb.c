...
static struct net_buf *ec_host_cmd_buf_alloc(const uint8_t ep, size_t size, void *const data)
{
...
static int ec_host_cmd_signal_event(struct usbd_class_data *c_data)
{
...
static int handle_out_transfer(struct usbd_class_data *c_data)
{
	int ret;
	struct ec_host_cmd_usb_ctx *ctx = usbd_class_get_private(c_data);
	size_t expected_len =
	...
	if (ctx->state == USB_EC_HOST_CMD_STATE_RECEIVING) {
		...
		ctx->usb_rx_buf = ec_host_cmd_buf_alloc(
			ec_host_cmd_get_out_ep(c_data), expected_len - ctx->rx_ctx->len,
...
static int ec_host_cmd_request(struct usbd_class_data *c_data, struct net_buf *buf, int err)
{
	struct ec_host_cmd_usb_ctx *ctx = usbd_class_get_private(c_data);
	struct usbd_context *uds_ctx = usbd_class_get_ctx(c_data);
	...
	if (err) {
		LOG_ERR("EP: 0x%02x, err: %d", bi->ep, err);
