...
{
	struct mpipe_dump_writer w = {.print = print, .ctx = ctx};
...
{
	struct mpipe_dump_ctx dump = {
		.writer = {.print = print, .ctx = ctx},
	};
	struct mpipe_element *self = (struct mpipe_element *)bin;

	__ASSERT_NO_MSG(bin != NULL);

	dump_index_bin(&dump, bin);

	dump_print(&dump.writer, "digraph mpipe_pipeline {\n");
	dump_print(&dump.writer, "  rankdir=LR;\n");
	dump_print(&dump.writer, "  node [shape=record, style=filled, fontname=\"sans\"];\n");
	dump_print(&dump.writer, "  label=\"");
	dump_element_name(&dump, self);
	dump_print(&dump.writer, " %s%s\";\n", mpipe_dump_state_str(self->current_state),
		   dump.truncated ? " (truncated)" : "");

	dump_dot_nodes(&dump);
	dump_dot_edges(&dump);

	dump_print(&dump.writer, "}\n");

	/* The rendering ends on a newline, so this only catches a truncated line */
	dump_flush(&dump.writer);
