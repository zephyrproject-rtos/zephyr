...
LOG_MODULE_REGISTER(mpipe_vid_buffer_pool, CONFIG_MPIPE_LOG_LEVEL);
 ...
 */
static int mpipe_vid_buffer_pool_find(struct mpipe_vid_buffer_pool *vid_pool,
...
static int mpipe_vid_buffer_pool_stop(struct mpipe_buffer_pool *pool)
{
	int ret = 0;
	struct mpipe_vid_buffer_pool *vid_pool = (struct mpipe_vid_buffer_pool *)pool;
...
static int mpipe_vid_buffer_pool_acquire_buffer(struct mpipe_buffer_pool *pool,
						struct net_buf **buf)
{
	struct mpipe_vid_buffer_pool *vid_pool = (struct mpipe_vid_buffer_pool *)pool;
	...
	idx = mpipe_vid_buffer_pool_find(vid_pool, vbuf);
	if (idx >= 0) {
...
static int mpipe_vid_buffer_pool_release_buffer(struct mpipe_buffer_pool *pool, struct net_buf *buf)
{
	struct mpipe_vid_buffer_pool *vid_pool = (struct mpipe_vid_buffer_pool *)pool;
	struct video_buffer *vbuf = mpipe_buffer_get_meta(buf)->driver_buf;
	k_spinlock_key_t key;
	bool flushing;
	int idx;
	int ret = 0;
...
void mpipe_vid_buffer_pool_init(struct mpipe_buffer_pool *pool, struct mpipe_vid_object *obj)
{
	...
	pool->release_buffer = mpipe_vid_buffer_pool_release_buffer;
}
