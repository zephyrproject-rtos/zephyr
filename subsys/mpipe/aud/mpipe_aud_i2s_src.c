...
static int (*src_parent_set_property)(struct mpipe_object *obj, uint32_t key, const void *val);
static int (*src_parent_get_property)(struct mpipe_object *obj, uint32_t key, void *val);
...
static int mpipe_aud_i2s_src_acquire_buffer(struct mpipe_buffer_pool *pool, struct net_buf **buffer)
{
	...
	/* The pool was torn down during i2s_read(): the block is no longer ours. */
