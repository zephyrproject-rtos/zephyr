...
static int mpipe_aud_i2s_src_acquire_buffer(struct mpipe_buffer_pool *pool, struct net_buf **buffer)
{
	...
	/*
	 * A teardown dismantles the pool while this thread may still be inside
	 * i2s_read(). The block it comes back with then belongs to a slab whose
	 * backing store has already been released, so it matches nothing here
	 * and freeing it panics the kernel on an invalid pointer.
	 */
	if (!pool->started || aud_pool->mem_slab->buffer == NULL) {
		return -EPIPE;
	}
			   ...
			   const struct device *i2s_dev)
{
	...
	aud_i2s_src->pool.aud_dev = i2s_dev;
