...
struct morse_tx_data {
	struct counter_alarm_cfg dot_tick;
	const uint8_t *data;
	size_t data_idx;
	size_t data_size;
					  ...
					  const struct device *morse)
{
	...
	counter_get_value(cfg->tmr, &now);
	counter_cancel_channel_alarm(cfg->tmr, cfg->rx_tmr_chan);

	if (counter_set_channel_alarm(cfg->tmr, cfg->rx_tmr_chan,
				      &ctx->rx_data.dot_tick)) {
		LOG_ERR("Fail to set RX timer");
		return -EIO;
	}
	...
	if (ctx->rx_data.rx_fsm == MORSE_CODE_RX_BIT_FSM_IDLE) {
		...
		if (now > ctx->rx_data.now) {
			ctx->rx_data.ticks = now - ctx->rx_data.now;
		} else {
			ctx->rx_data.ticks = ctx->rx_data.top
					   - ctx->rx_data.now
					   + now;
		}
	}
	...
	LOG_DBG("Now: %d, Ticks: %d", now, ctx->rx_data.ticks);
...
static void morse_word_blank_handler(const struct device *dev, uint8_t chan_id,
				     uint32_t ticks, void *user_data)
{
	const struct device *mdev = user_data;
	struct morse_data *ctx = mdev->data;

	ctx->rx_data.ticks = ctx->rx_data.dot_tick.ticks;
	ctx->rx_data.rx_fsm = MORSE_CODE_RX_BIT_FSM_BLANK_PHASE;
...
static void morse_dot_tick_handler(const struct device *dev, uint8_t chan_id,
				   uint32_t ticks, void *user_data)
{
	const struct device *mdev = user_data;
	...
	counter_set_channel_alarm(cfg->tmr, cfg->tx_tmr_chan, &ctx->tx_data.dot_tick);
...
int morse_send(const struct device *dev, const uint8_t *data, const size_t size)
{
	...
	ret = counter_set_channel_alarm(cfg->tmr, cfg->tx_tmr_chan,
					&ctx->tx_data.dot_tick);
	if (ret < 0) {
		LOG_ERR("Error %d. Fail to set dot_tick", ret);
		return ret;
	}
...
int morse_set_config(const struct device *dev, uint16_t speed)
{
	...
	ctx->tx_data.dot_tick.ticks = counter_us_to_ticks(cfg->tmr, dot_time);
	ticks_tol = counter_us_to_ticks(cfg->tmr, dot_tol);
	rx_dot_ticks = counter_us_to_ticks(cfg->tmr, dot_time);
	ctx->rx_data.dot_tick.ticks = (rx_dot_ticks + ticks_tol) * MORSE_9DOTS;
	ctx->rx_data.dot_ticks = rx_dot_ticks - ticks_tol;

	counter_start(cfg->tmr);
...
static int morse_init(const struct device *dev)
{
	...
	if (cfg->tmr != NULL) {
		LOG_DBG("Timer");
		if (!device_is_ready(cfg->tmr)) {
			LOG_ERR("Error: Timer device %s is not ready", cfg->tmr->name);
			return -ENODEV;
		}
	}
	...
	ctx->rx_data.top = counter_get_max_top_value(cfg->tmr);
	ctx->rx_data.now = 0;
...
#define MORSE_INIT(n)								\
	...
	static const struct morse_config morse_cfg_##n = {			\
		.tmr = DEVICE_DT_GET(DT_INST_PROP(n, tmr)),			\
		.tx_tmr_chan = DT_INST_PROP_OR(n, tx_tmr_chan, 0),		\
		.rx_tmr_chan = DT_INST_PROP_OR(n, rx_tmr_chan, 0),		\
