...
static uint8_t rx_data[CONFIG_MORSE_SHELL_RX_BUF_SIZE];
...
static void morse_shell_rx_cb_handler(void *ctx, enum morse_rx_state status, uint32_t data)
{
	...
	/* sizeof(buffer) -1 [index 0] -1 [EOF] -1 [SPACE] */
	if (rx_counter == (CONFIG_MORSE_SHELL_RX_BUF_SIZE - 3)) {
		rx_data[rx_counter] = 0x00;
		rx_counter = 0;

		LOG_INF("RX flush: %s", rx_data);
	}
