...
static void thread_print_cb(struct thread_analyzer_info *info)
{
	...
	THREAD_ANALYZER_PRINT(
		...
		info->usage.total_cycles, info->usage.total_cycles_delta);
#else
	THREAD_ANALYZER_PRINT(
...
#ifdef CONFIG_THREAD_RUNTIME_STATS_BETWEEN_COLLECTIONS
		info.utilization_delta = (info.usage.execution_cycles_delta * 100U) /
			rt_stats_all.execution_cycles_delta;
#endif
	}
#endif
	cb(&info);
