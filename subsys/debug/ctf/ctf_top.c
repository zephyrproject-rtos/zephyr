...
void sys_trace_thread_switched_in(void)
{
	...
	if (is_idle_thread(thread)) {
		...
		ctf_middle_thread_switch_in((u32_t)(uintptr_t)thread);
	}
}

void sys_trace_thread_switched_out(void)
{
	ctf_middle_thread_switch_out();
}
...
void sys_trace_thread_pending(struct k_thread *thread)
{
	ctf_middle_thread_pending((u32_t)(uintptr_t)thread);
}
