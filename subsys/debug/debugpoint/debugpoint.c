...
static bool cpu_startup_supported(void)
{
#if defined(CONFIG_SMP) && DT_NODE_EXISTS(DT_PATH(cpus))
#define CPU_DEFERRED_START(node_id) DT_PROP_OR(node_id, zephyr_deferred_start, 0),
	static const bool deferred_start[] = {
		DT_FOREACH_CPU(CPU_DEFERRED_START)
	};
#undef CPU_DEFERRED_START

	/* Match z_smp_init(): logical CPU 0 always starts; later CPUs may be deferred. */
	for (unsigned int cpu = 1U;
	     cpu < MIN(arch_num_cpus(), ARRAY_SIZE(deferred_start)); cpu++) {
		if (deferred_start[cpu]) {
			return false;
		}
	}
#endif
