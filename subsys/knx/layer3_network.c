...
{
	LOG_DBG("ind src=" KNX_ADDR_FMT " dst=" KNX_ADDR_FMT,
		KNX_ADDR_VAL(pkt->src), KNX_ADDR_VAL(pkt->dst));
...
void N_Data_Group__ind(struct knx_pkt *pkt)
{
	...
	LOG_DBG("ind src=" KNX_ADDR_FMT " dst=" KNX_GROUP_FMT,
		KNX_ADDR_VAL(pkt->src), KNX_GROUP_VAL(pkt->dst));
#endif
