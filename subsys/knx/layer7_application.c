...
static void send_broadcast_response(knx_priority_t priority,
...
static void send_group_response(uint16_t tsap, knx_priority_t priority,
				const uint8_t *apdu, uint8_t apdu_len)
{
	struct knx_pkt *resp = knx_pkt_alloc(K_NO_WAIT);
...
void A_GroupValue_Read__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_GroupValue_Read__Rcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_GroupValue_Write__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_GroupValue_Write__Rcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_IndividualAddress_Write__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddress_Write__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_IndividualAddress_Read__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddress_Read__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddress_Read__Rcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddress_Read__Acon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_IndividualAddress_Read__ind(struct knx_pkt *pkt)
{
	...
	k_work_reschedule_for_queue(&knx_app_wq, &s_ia_read_resp_work,
				    K_MSEC(delay_ms));
}
...
void A_IndividualAddressSerialNumber_Read__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddressSerialNumber_Read__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_IndividualAddressSerialNumber_Read__res(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddressSerialNumber_Read__Rcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddressSerialNumber_Read__Acon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }

void A_IndividualAddressSerialNumber_Write__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_IndividualAddressSerialNumber_Write__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_Restart__ind(struct knx_pkt *pkt)
{
	...
	LOG_DBG("ind restart_type=%u erase_code=%u channel=%u",
		restart_type, erase_code, channel);
...
void A_PropertyValue_Read__ind(struct knx_pkt *pkt)
{
	...
	uint16_t start_index         = (((uint16_t)(pkt->lsdu[4] & 0x0F)) << 8)
				       | pkt->lsdu[5];
	...
	LOG_DBG("ind obj=%u prop=%u count=%u start=%u",
		object_index, property_id, number_of_elements, start_index);

	data_length = interface_read_property(object_index, property_id,
...
void A_PropertyValue_Write__ind(struct knx_pkt *pkt)
{
	...
	LOG_DBG("ind obj=%u prop=%u count=%u start=%u data_len=%u",
		object_index, property_id, number_of_elements, start_index, data_length);
	 ...
	 */
	ok = interface_write_property(object_index, property_id, start_index,
				      number_of_elements, &pkt->lsdu[6], data_length);
	 ...
	 */
	bool verify_mode = (__memory.device_control & 0x04u) != 0
	...
	if (!ok) {
		...
		resp_len = interface_read_property(object_index, property_id,
						   start_index, &actual_count,
						   property_data, MAX_PROPERTY_READ_SIZE);
		if (resp_len == 0) {
			actual_count = 0;
...
void A_PropertyDescription_Read__ind(struct knx_pkt *pkt)
{
	...
	LOG_DBG("ind obj=%u prop=%u idx=%u",
		object_index, property_id, property_index);

	interface_read_property_description(object_index, property_id,
					    property_index,
					    &write_enable, &type,
...
void A_Memory_Read__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_Memory_Read__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_Memory_Read__ind(struct knx_pkt *pkt)
{
	...
	if (number == 0 || number > 63 ||
	    (uint32_t)memory_address + number > memory_size) {
...
void A_Memory_Write__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void A_Memory_Write__Lcon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_Memory_Write__Acon(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void A_UserMemoryBit_Write__ind(struct knx_pkt *pkt)
{
	...
	if (number == 0 || number > 5 ||
	    (uint32_t)memory_address + number > sizeof(__memory)) {
		LOG_WRN("ind: invalid num=%u or addr=0x%04x", number, memory_address);
		if (verify_mode) {
