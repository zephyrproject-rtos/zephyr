...
static const struct gpio_dt_spec s_prog_led =
	GPIO_DT_SPEC_GET(DT_CHOSEN(knx_prog_led), gpios);
...
struct property object_device_properties[] = {
    KNX_PROP_SCALAR(PID_OBJECT_TYPE, PDT_UNSIGNED_INT, false, ReadLv3 | WriteLv0, &_object_type),
    KNX_PROP_CHARS(PID_OBJECT_NAME, false, ReadLv3 | WriteLv0, _name),
    KNX_PROP_SCALAR(PID_SERIAL_NUMBER, PDT_GENERIC_06, false, ReadLv3 | WriteLv0, &_serial_number),
    KNX_PROP_SCALAR(PID_PRODUCT_ID, PDT_GENERIC_10, false, ReadLv3 | WriteLv0, &_product_id),
    KNX_PROP_SCALAR(PID_MANUFACTURER_ID, PDT_UNSIGNED_INT, false, ReadLv3 | WriteLv0, &_manufacturer_id),
    /*
     ...
     */
    KNX_PROP_SCALAR(PID_DEVICE_CONTROL, PDT_BITSET8, true, ReadLv3 | WriteLv3, &__memory.device_control),
     ...
     * not yet implemented.
     */
    KNX_PROP_SCALAR(PID_MAX_RETRY_COUNT, PDT_UNSIGNED_CHAR, true, ReadLv3 | WriteLv3, &_max_retry_count),
    KNX_PROP_SCALAR(PID_ERROR_FLAGS, PDT_BITSET8, false, ReadLv3 | WriteLv0, &_error_flags),
    KNX_PROP_SCALAR(PID_HARDWARE_TYPE, PDT_GENERIC_06, false, ReadLv3 | WriteLv0, &_hardware_type),
    ...
    KNX_PROP_SCALAR(PID_ROUTING_COUNT, PDT_UNSIGNED_CHAR, true, ReadLv3 | WriteLv3, &__memory.routing_count),
    KNX_PROP_SCALAR(PID_PROGMODE, PDT_BITSET8, true, ReadLv3 | WriteLv3, &_prog_mode),
    KNX_PROP_SCALAR(PID_MAX_APDU_LENGTH, PDT_UNSIGNED_INT, false, ReadLv3 | WriteLv0, &_max_apdu_length),
    ...
    KNX_PROP_SCALAR(PID_DEVICE_DESCRIPTOR, PDT_GENERIC_02, false, ReadLv3 | WriteLv0, &_device_descriptor),
...
void device_factory_format(void)
{
    ...
    LOG_WRN("device_factory_format: wiping individual address and all tables");
    ...
    LOG_WRN("device_factory_format: done — individual address is now 0xFFFF");
}
...
uint16_t device_descriptor(void)      { return knx_u16_be_get(_device_descriptor); }
uint16_t device_manufacturer_id(void) { return ((uint16_t)_manufacturer_id[0] << 8) | _manufacturer_id[1]; }
...
uint16_t device_version(void)       { return ((uint16_t)_version[0] << 8) | _version[1]; }
uint8_t  device_routing_count(void) { return __memory.routing_count; }
