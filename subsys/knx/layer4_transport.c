...
static inline void __set_state(int s) { __state = s; }
static inline int  __get_state(void)  { return __state; }
...
static void send_control_telegram(uint8_t pduType, uint8_t seqNo)
{
	...
	if (!pkt) {
		LOG_ERR("send_control_telegram: alloc failed");
		return;
	}
...
static void send_ack_telegram(uint8_t seqNo)  { send_control_telegram(T_ACK_PDU,  seqNo); }
static void send_nack_telegram(uint8_t seqNo) { send_control_telegram(T_NACK_PDU, seqNo); }
 ...
 */
static void l7_dispatch_apdu(knx_addr_t source, knx_priority_t priority,
			      uint8_t *tsdu, uint8_t octet_count, bool connected)
{
	uint16_t apduType = (((uint16_t)tsdu[0] << 8) | tsdu[1]) & 0x3FF;
	struct knx_pkt *pkt = knx_pkt_alloc(K_NO_WAIT);

	if (!pkt) {
		LOG_ERR("l7_dispatch_apdu: alloc failed");
		return;
	}
	...
	pkt->hop_count      = 7;
	pkt->asap           = source; /* connection address, or the sender's own address for connectionless, serves as TSAP */
	...
	default:
		...
		default:
			LOG_WRN("%s: unhandled APCI 0x%03x",
				connected ? "connected" : "individual", apduType);
			break;
		}
		break;
	}
...
{
	ARG_UNUSED(s); ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
}

static void A1(knx_addr_t source, knx_priority_t p, uint8_t *d, uint8_t n)
{
	ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
	LOG_DBG("A1: accept connection from " KNX_ADDR_FMT, KNX_ADDR_VAL(source));
...
{
	ARG_UNUSED(s); ARG_UNUSED(p);
	LOG_DBG("A3: re-ACK seq=%u (SeqNoRcv=%u)", received_seq_no(d, n), _seqNoRcv);
...
{
	ARG_UNUSED(s); ARG_UNUSED(p);
	LOG_DBG("A4: NAK seq=%u (SeqNoRcv=%u)", received_seq_no(d, n), _seqNoRcv);
...
{
	ARG_UNUSED(source); ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
	LOG_DBG("A5: remote disconnect");
...
{
	ARG_UNUSED(s); ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
	LOG_DBG("A6: active disconnect (event %u)", (unsigned)m_last_event);
...
{
	ARG_UNUSED(s);
	LOG_DBG("A7: send seq=%u", _seqNoSend);
...
static void A8(knx_addr_t s, knx_priority_t p, uint8_t *d, uint8_t n)
{
	...
	if (_savedConnectingValid) {
		...
		LOG_DBG("A8: processing queued req");
		A7(_savedTsapConnecting, _savedPriorityConnecting,
		   _savedFrameConnecting, _savedFrameConnectingLength);
	}
}
...
{
	ARG_UNUSED(s); ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
	LOG_DBG("A9: retransmit rep=%d", _rep_count);
	retransmit_saved_frame();
	_rep_count++;
	enable_ack_timeout();
	enable_connection_timeout();
...
{
	ARG_UNUSED(p); ARG_UNUSED(d); ARG_UNUSED(n);
	LOG_DBG("A12: connect to " KNX_ADDR_FMT, KNX_ADDR_VAL(source));
	_connection_address = source;
...
#define RUN_EVENT(num, src, prio, data, len) do {			\
	m_last_event = (num);						\
...
void T_Data_Group__req(struct knx_pkt *pkt)
{
	...
	if (group_addr == 0) {
		LOG_ERR("T_Data_Group__req: no address for TSAP %u", pkt->tsap);
		knx_pkt_free(pkt);
		return;
	}
	pkt->dst = group_addr;
...
void T_Data_Group__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Data_Group__ind(struct knx_pkt *pkt)
{
	 ...
	 * to the default case.
	 */
	uint16_t apci = (((uint16_t)(pkt->lsdu[0] & 0x03u)) << 8)
		      | (pkt->lsdu[1] & 0xC0u);

	LOG_DBG("T_Data_Group__ind tsap=%u APCI=0x%03x", tsap, apci);
	...
	for (;;) {
		...
		case A_GroupValue_Write:
			...
			break;
		default:
			LOG_WRN("T_Data_Group__ind: unknown APCI 0x%03x", apci);
			break;
		}
	}
...
void T_Data_Tag_Group__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void T_Data_Tag_Group__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void T_Data_Tag_Group__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Data_Broadcast__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Data_Broadcast__ind(struct knx_pkt *pkt)
{
	...
	default:
		LOG_DBG("T_Data_Broadcast__ind: unhandled APCI 0x%03x", apci);
		break;
	}
...
void T_Data_SystemBroadcast__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Data_SystemBroadcast__ind(struct knx_pkt *pkt)
{
	...
	LOG_DBG("SBC APCI=0x%03x",
		((uint16_t)pkt->lsdu[0] << 8) | pkt->lsdu[1]);
	knx_pkt_free(pkt);
}
...
void T_Data_Individual__ind(struct knx_pkt *pkt)
{
	...
	case T_Ack:
		if (src == _connection_address &&
		    T_SEQ_NO_OF_PDU(tsdu[0]) == _seqNoSend) {
...
void T_Connect__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void T_Connect__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Disconnect__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void T_Disconnect__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
void T_Data_Connected__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void T_Data_Connected__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
