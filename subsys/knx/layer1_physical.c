...
static void LOG_FRAME(struct knx_pkt *pkt, char *buf, int max_len)
{
	...
	knx_addr_type_t    addr_type = std_frame ? ((pkt->buf[5] & 0x80u) >> 7) : ((pkt->buf[1] & 0x80u) >> 7);
	knx_addr_t         src = std_frame ? (((uint16_t)pkt->buf[1] << 8) | pkt->buf[2]) : (((uint16_t)pkt->buf[2] << 8) | pkt->buf[3]);
	knx_addr_t         dst = std_frame ? (((uint16_t)pkt->buf[3] << 8) | pkt->buf[4]) : (((uint16_t)pkt->buf[4] << 8) | pkt->buf[5]);
	...
	{
		pos += snprintf(buf + pos, max_len - pos, "[%u.%u.%u -> %i.%i.%i]", KNX_ADDR_VAL(src), KNX_ADDR_VAL(dst));
	}
	else
	{
		pos += snprintf(buf + pos, max_len - pos, "[%u.%u.%u -> %i/%i/%i]", KNX_ADDR_VAL(src), KNX_GROUP_VAL(dst));
	}
	...
	else
	{
		...
		else
		{
			...
			switch (apci) {
				case 0x000:
					pos += snprintf(buf + pos, max_len - pos, " GroupValue_Read %i/%i/%i", KNX_ADDR_VAL(dst));
					break;
				case 0x040:
					pos += snprintf(buf + pos, max_len - pos, " GroupValue_Response %i/%i/%i", KNX_ADDR_VAL(dst));
					pos += snprintf(buf + pos, max_len - pos, " [");
					...
					break;
				case 0x080:
					pos += snprintf(buf + pos, max_len - pos, " GroupValue_Write %i/%i/%i", KNX_ADDR_VAL(dst));
					pos += snprintf(buf + pos, max_len - pos, " [");
					for (int i = 2; i < lsdu_len; i++)
					{
						pos += snprintf(buf + pos, max_len - pos, "%02X ", lsdu[i]);
	...
	switch (p_class) {
		...
		case Ind_end_of_Frame: {
			...
			if (corrupt) {
				 ...
				 */
				LOG_WRN("RX: %u-byte frame had flagged octet(s) — discarding",
					len);
				break;
			}
