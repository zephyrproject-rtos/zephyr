...
struct property object_application_program_properties[] = {
    ...
    KNX_PROP_SCALAR(PID_PROGRAM_VERSION, PDT_GENERIC_05, true, ReadLv3 | WriteLv3, &__memory.program_version),
    KNX_PROP_SCALAR(PID_PEI_TYPE, PDT_UNSIGNED_CHAR, false, ReadLv3 | WriteLv0, &_pei_type),
    KNX_PROP_SCALAR(PID_TABLE_REFERENCE, PDT_UNSIGNED_LONG, false, ReadLv3 | WriteLv0, &table_reference_application_program),
...
void object_application_program_init(void)
{
	...
	LOG_INF("PID_PROGRAM_VERSION declared as %02X %02X %02X %02X %02X "
		...
		__memory.program_version[0], __memory.program_version[1],
		__memory.program_version[2], __memory.program_version[3],
		__memory.program_version[4], mfr,
		(unsigned)CONFIG_KNX_APPLICATION_PROGRAM_ID,
		(unsigned)(CONFIG_KNX_APPLICATION_PROGRAM_VERSION >> 4),
		(unsigned)(CONFIG_KNX_APPLICATION_PROGRAM_VERSION & 0x0Fu));
	memory_modified();
#else
