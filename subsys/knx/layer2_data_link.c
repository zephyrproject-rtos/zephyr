...
void L_Data__req(struct knx_pkt *pkt)
{
	...
	if (lsdu_len <= 16) {
		...
		pkt->buf[5] = ((pkt->addr_type << 7) & 0x80u)
			    | ((pkt->hop_count << 4) & 0x70u)
			    | ((lsdu_len - 1u) & 0x0Fu);
		...
		pkt->buf[1] = ((pkt->addr_type << 7) & 0x80u)
			    | ((pkt->hop_count << 4) & 0x70u);
		 ...
		 */
		LOG_ERR("L_Data__req: LSDU %u octets exceeds the %u this device "
			"advertises (PID_MAX_APDU_LENGTH = %u)",
			lsdu_len, (unsigned)KNX_MAX_LSDU_OCTETS,
			(unsigned)KNX_MAX_APDU_OCTETS);
		knx_pkt_free(pkt);
		return;
	}
...
static void l_tx_thread_fn(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (l_tx_dequeue_next(&pkt) != 0) {
			k_poll_event_init(&tx_poll_events[0],
					   K_POLL_TYPE_MSGQ_DATA_AVAILABLE,
					   K_POLL_MODE_NOTIFY_ONLY, &l_tx_msgq_hi);
			k_poll_event_init(&tx_poll_events[1],
					   K_POLL_TYPE_MSGQ_DATA_AVAILABLE,
		...
		if (ret == -EMSGSIZE) {
			...
			LOG_ERR("TX: extended frame of %u bytes exceeds the driver's "
				"64-byte block limit, dropping", wire_len);
		}
...
{
	struct knx_pkt *pkt;
	static uint32_t nbframe = 0;
	static uint32_t nberror = 0;
	...
	if (crc != raw[raw_len - 1]) {
		++nberror;
		LOG_WRN("RX: CRC error (got 0x%02x expected 0x%02x) [%i%%]",
			raw[raw_len - 1], crc, (nberror*100)/nbframe);
		knx_pkt_free(pkt);
	...
	pkt->src          = std_frame
		? (((uint16_t)raw[1] << 8) | raw[2])
		: (((uint16_t)raw[2] << 8) | raw[3]);
	pkt->dst          = std_frame
		? (((uint16_t)raw[3] << 8) | raw[4])
	 ...
	 *
	 * 2. Dropping the frame here does NOT suppress the bus acknowledgement.
	 ...
	 */
	{
		...
		if (pkt->repeated && s_last_rx.valid &&
...
void L_Init(void)
{
	k_thread_create(&l_tx_thread_data, l_tx_stack, L_TX_STACK_SIZE,
			l_tx_thread_fn, NULL, NULL, NULL,
			K_PRIO_PREEMPT(CONFIG_KNX_L2_TX_PRIO), 0, K_NO_WAIT);
	k_thread_name_set(&l_tx_thread_data, "knx_l2_tx");

	k_thread_create(&knx_rx_thread_data, knx_rx_stack, KNX_RX_STACK_SIZE,
			knx_rx_thread_fn, NULL, NULL, NULL,
			K_PRIO_PREEMPT(CONFIG_KNX_L2_RX_PRIO), 0, K_NO_WAIT);
