...
{
    dpt10_t time;
    time.day = (payload[0] >> 5) & 0x07;
    time.hour = payload[0] & 0x1F;
    time.minute = payload[1] & 0x3F;
    time.second = payload[2] & 0x3F;
    return time;
}
#endif
...
{
    return (int32_t)((((uint32_t)payload[0]) << 24) |
                     (((uint32_t)payload[1]) << 16) |
                     (((uint32_t)payload[2]) << 8) |
                     ((uint32_t)payload[3]));
}
...
float dpt14_decode(const uint8_t *payload)
{
    ...
    converter.u = (((uint32_t)payload[0]) << 24) |
                  (((uint32_t)payload[1]) << 16) |
                  (((uint32_t)payload[2]) << 8) |
                  ((uint32_t)payload[3]);
    return converter.f;
...
uint8_t dpt15_encode(dpt15_t data, uint8_t *payload)
{
    ...
    payload[3] = ((data.error ? 1 : 0) << 7) |
                 ((data.permission ? 1 : 0) << 6) |
                 ((data.read_direction ? 1 : 0) << 5) |
                 ((data.encrypted ? 1 : 0) << 4) |
...
uint8_t dpt19_encode(dpt19_t datetime, uint8_t *payload)
{
    ...
    payload[6] = ((datetime.fault ? 1 : 0) << 7) |
                 ((datetime.working_day ? 1 : 0) << 6) |
                 ((datetime.no_working_day ? 1 : 0) << 5) |
                 ((datetime.no_year ? 1 : 0) << 4) |
                 ((datetime.no_date ? 1 : 0) << 3) |
                 ((datetime.no_day_of_week ? 1 : 0) << 2) |
                 ((datetime.no_time ? 1 : 0) << 1) |
                 (datetime.summer_time ? 1 : 0);
...
{
    return (int64_t)((((uint64_t)payload[0]) << 56) |
