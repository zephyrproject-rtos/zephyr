 ...
 */
uint32_t table_reference_addresses =
	IS_ENABLED(CONFIG_KNX_AUTO_LOAD_TABLES)
		? __builtin_bswap32(offsetof(struct Memory, addresses)) : 0U;
uint32_t table_reference_associations =
	IS_ENABLED(CONFIG_KNX_AUTO_LOAD_TABLES)
		? __builtin_bswap32(offsetof(struct Memory, associations)) : 0U;
uint32_t table_reference_application_program =
	IS_ENABLED(CONFIG_KNX_AUTO_LOAD_TABLES)
		? __builtin_bswap32(offsetof(struct Memory, application_program)) : 0U;
uint32_t table_reference_group_object =
	IS_ENABLED(CONFIG_KNX_AUTO_LOAD_TABLES)
		? __builtin_bswap32(offsetof(struct Memory, group_object)) : 0U;
uint32_t table_reference_interface_program =
	IS_ENABLED(CONFIG_KNX_AUTO_LOAD_TABLES)
		? __builtin_bswap32(offsetof(struct Memory, interface_program)) : 0U;

/*
 * Canonical big-endian offsets — used by load_state_machine_event() and
 * by the auto-load paths in the individual object files.
 */
const uint32_t k_table_ref_addresses =
	__builtin_bswap32(offsetof(struct Memory, addresses));
const uint32_t k_table_ref_associations =
...
const uint32_t k_table_ref_group_object =
...
static int knx_mem_set(const char *key, size_t len, settings_read_cb read_cb, void *cb_arg)
{
	...
	LOG_DBG("memory_read: loaded %zu bytes (layout version %u)",
		len, KNX_MEMORY_LAYOUT_VERSION);
	return 0;
}
...
bool memory_read(void)
{
	...
	if (ret != 0) {
		LOG_ERR("memory_read: settings_subsys_init failed: %d", ret);
		return false;
	}

	ret = settings_register(&s_knx_settings);
	if (ret != 0 && ret != -EEXIST) {
		LOG_ERR("memory_read: settings_register failed: %d", ret);
		return false;
	}
	...
	if (ret != 0) {
		...
		LOG_WRN("memory_read: nothing usable restored, using defaults");
		memory_reset();
		restored = false;
	}

	/* Validate magic header */
	if (__memory.start[0] != 0xA5 || __memory.start[1] != 0xAD ||
	    __memory.start[2] != 0xAF || __memory.start[3] != 0xFE) {
		LOG_WRN("memory_read: invalid magic, reinitialising");
		memory_reset();
		restored = false;
	}
...
{
	k_work_reschedule(&s_flush_work,
			  K_MSEC(CONFIG_KNX_SETTINGS_DEBOUNCE_MS));
}
...
bool memory_read(void)
{
	LOG_WRN("memory_read: CONFIG_SETTINGS not enabled, using defaults");
	memory_reset();
	return false;
}
...
{
	/* No-op without settings backend. */
	LOG_WRN("memory_write: CONFIG_SETTINGS not enabled, changes are volatile");
}
...
bool memory_user_write(uint32_t offset, size_t len, uint8_t *data)
{
	if (offset + len > sizeof(__memory)) {
		LOG_ERR("memory_user_write(0x%08x, %zu): outside memory range",
			offset, len);
		return false;
	}
