...
knx_u16_be_t object_interfaces_types[NUMBER_OF_INTERFACE + 1] = {
    KNX_U16_BE(NUMBER_OF_INTERFACE),
    KNX_U16_BE(OT_DEVICE),
    KNX_U16_BE(OT_ADDR_TABLE),
    KNX_U16_BE(OT_ASSOC_TABLE),
    KNX_U16_BE(OT_GRP_OBJ_TABLE),
    KNX_U16_BE(OT_APPLICATION_PROG),
    KNX_U16_BE(OT_INTERFACE_PROG)
};

struct property *object_interfaces[NUMBER_OF_INTERFACE] = {
    object_device_properties,
...
static struct property *get_property(uint8_t object_index,
                                     uint8_t property_id,
                              ...
                              uint8_t data_length)
{
    ...
    if (load_state == NULL) {
        LOG_ERR("load_state_machine_event: no PID_LOAD_STATE_CONTROL for obj %u", object_index);
        return false;
    }

    if (data_length < LSM_MIN_EVENT_OCTETS) {
        LOG_WRN("load_state_machine_event obj=%u: empty PDU, no event to apply", object_index);
        return false;
    }
    ...
    if (event == LE_ADDITIONAL_LOAD_CONTROLS && data_length < LSM_MIN_ALLOC_OCTETS) {
         ...
         */
        LOG_WRN("load_state_machine_event obj=%u: LE_ADDITIONAL_LOAD_CONTROLS needs %u "
                "octets, got %u", object_index, LSM_MIN_ALLOC_OCTETS, data_length);
        return false;
    }
    ...
    case LS_LOADING:
        ...
        case LE_ADDITIONAL_LOAD_CONTROLS: {
            if (data[1] != 0x0B) {
                LOG_ERR("LS_LOADING LE_ADDITIONAL_LOAD_CONTROLS: bad cmd 0x%02x", data[1]);
                *state = LS_ERROR;
                set_error_code(object_index, E_GOT_UNDEF_LOAD_CMD);
                break;
            }
            ...
            if (size > capacity) {
                LOG_ERR("LE_ADDITIONAL_LOAD_CONTROLS obj=%u: requested %u octets "
                        ...
                        object_index, (unsigned)size, (unsigned)capacity);
                *state = LS_ERROR;
            ...
            if (fill) {
                 ...
                 * uniformly.
                 */
                uint8_t *base = (uint8_t *)&__memory + memory_table_offset(object_index);

                memset(base, fill_byte, size);
                memory_modified();
            }
            LOG_DBG("LE_ADDITIONAL_LOAD_CONTROLS obj=%u size=%u/%u fill=%d ref=0x%08x",
                    object_index, (unsigned)size, (unsigned)capacity, fill,
...
void interface_read_property_description(uint8_t object_index,
                                         uint8_t property_id,
                                         uint8_t property_index,
                                         bool *write_enable,
                                         ...
                                         uint8_t *property_index_found)
{
    struct property *object = get_property(object_index, property_id,
                                           property_index, property_index_found);

    LOG_DBG("obj=%u prop=%u idx=%u %s",
            object_index, property_id, property_index,
...
uint8_t interface_read_property(uint8_t object_index,
                                ...
                                uint8_t max_size)
{
    ...
    LOG_DBG("obj=%u prop=%u start=%u %s",
            object_index, property_id, start_index,
            object ? "found" : "not found");
    ...
    if (start_index <= nb) {
        ...
        if (data_size == 0) {
            ...
            LOG_WRN("obj=%u prop=%u: unsupported PDT %u",
                    object_index, property_id, object->description.property_datatype);
            return 0;
        }
...
uint8_t interface_function_property_command(uint8_t object_index,
					     uint8_t property_id,
					     ...
					     uint8_t max_out)
{
	LOG_DBG("function_property_command obj=%u prop=%u (not implemented)",
		object_index, property_id);
	ARG_UNUSED(data_in); ARG_UNUSED(data_in_len);
	ARG_UNUSED(data_out); ARG_UNUSED(max_out);
	return 0;
}

uint8_t interface_function_property_state_read(uint8_t object_index,
					       uint8_t property_id,
					       const uint8_t *data_in,
					       uint8_t data_in_len,
					       uint8_t *data_out,
					       uint8_t max_out)
{
	LOG_DBG("function_property_state_read obj=%u prop=%u (not implemented)",
		object_index, property_id);
	ARG_UNUSED(data_in); ARG_UNUSED(data_in_len);
	ARG_UNUSED(data_out); ARG_UNUSED(max_out);
	return 0;
...
bool interface_write_property(uint8_t object_index,
                              PropertyID property_id,
                              ...
                              uint8_t data_length)
{
    ...
    LOG_DBG("obj=%u prop=%u start=%u nelem=%u data_len=%u %s",
            object_index, property_id, start_index, number_of_elements, data_length,
            object ? "found" : "not found");
    ...
    if (!object->description.write_enable) {
        LOG_WRN("obj=%u prop=%u: read-only property, write refused",
                object_index, property_id);
        return false;
    }
    ...
    if (data_size == 0) {
        ...
        LOG_WRN("obj=%u prop=%u: unsupported PDT %u for write",
                object_index, property_id, object->description.property_datatype);
        return false;
    }
     ...
     */
    uint8_t *start_data = (uint8_t *)object->value.data +
    ...
    if (object->description.max_nr_of_elem > 1 &&
        ...
        LOG_WRN("obj=%u prop=%u: start %u + %u elements exceeds %u slots",
                object_index, property_id, start_index, number_of_elements,
    ...
    if ((uint16_t)number_of_elements * data_size > data_length) {
        LOG_WRN("obj=%u prop=%u: %u elements of %u octets need %u octets, PDU carries %u",
                ...
                (unsigned)((uint16_t)number_of_elements * data_size), data_length);
        return false;
    }
