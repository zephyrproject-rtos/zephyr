...
static char _name[] = "GroupObjects";
...
static group_object_updated_handler _group_object_updated_callback[GROUP_OBJECT_COUNT + 1] = {0,};
static group_object_read_handler _group_object_read_handler[GROUP_OBJECT_COUNT + 1] = {0,};
...
struct property object_group_device_properties[] = {
    ...
    KNX_PROP_SCALAR(PID_TABLE_REFERENCE, PDT_UNSIGNED_LONG, false, ReadLv3 | WriteLv0, &table_reference_group_object),
    KNX_PROP_ARRAY(PID_TABLE, PDT_GENERIC_02, false, ReadLv3 | WriteLv0,
                   GROUP_OBJECT_COUNT, &__memory.group_object[0], __memory.group_object, true),
...
void group_object_read_requested(uint8_t asap)
{
    if (_group_object_read_handler[asap] != NULL)
    {
        ...
        LOG_DBG("read handler for ASAP %u %s the value", asap, updated ? "updated" : "left unchanged");
    }
}
