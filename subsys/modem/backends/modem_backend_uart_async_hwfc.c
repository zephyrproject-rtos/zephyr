...
static int modem_backend_uart_async_hwfc_rx_enable(struct modem_backend_uart *backend)
{
	...
	if (ret) {
		rx_buf_unref(&backend->async, buf);
		return ret;
	}
...
static void modem_backend_uart_async_hwfc_rx_recovery(struct modem_backend_uart *backend)
{
	...
	if (!atomic_test_and_clear_bit(&backend->async.state,
				       MODEM_BACKEND_UART_ASYNC_STATE_RECOVERY_BIT)) {
...
{
	if (!atomic_test_bit(&backend->async.state, MODEM_BACKEND_UART_ASYNC_STATE_OPEN_BIT) &&
	    !atomic_test_bit(&backend->async.state, MODEM_BACKEND_UART_ASYNC_STATE_RECOVERY_BIT) &&
	    !atomic_test_bit(&backend->async.state, MODEM_BACKEND_UART_ASYNC_STATE_TRANSMIT_BIT)) {
		return true;
	}
						   ...
						   struct uart_event *evt, void *user_data)
{
	...
	case UART_RX_RDY:
		...
		if (err) {
			LOG_WRN("RX queue overflow: %d (dropped %u)", err, evt->data.rx.len);
				   ...
				   const struct modem_backend_uart_config *config)
{
	...
	k_msgq_init(&backend->async.rx_queue, (char *)&backend->async.rx_queue_buf,
		sizeof(struct rx_queue_event_t), CONFIG_MODEM_BACKEND_UART_ASYNC_HWFC_BUFFER_COUNT);

	backend->async.transmit_buf = config->transmit_buf;
	backend->async.transmit_buf_size = config->transmit_buf_size;
	k_work_init(&backend->async.rx_disabled_work, modem_backend_uart_async_hwfc_notify_closed);
