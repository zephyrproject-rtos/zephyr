	...
	uint16_t bufs_size, uint16_t device_readable)
{
	...
	struct virtq *virtqueue = api->get_virtqueue(dev, virtq);
...
{
	const struct virtio_driver_api *api = dev->api;
	struct virtio_fs_config *fs_config = api->get_device_specific_config(dev);
	...
	ret = api->commit_feature_bits(dev);
	if (ret != 0) {
		return ret;
	}

	ret = api->init_virtqueues(dev, virtiofs_queue_enum_cb, NULL);
	if (ret != 0) {
...
int virtiofs_unlink(const struct device *dev, const char *fname)
{
	...
	fuse_create_unlink_req(&req, fname_len);
	...
	LOG_INF("sending FUSE_UNLINK %s, unique=%" PRIu64, fname, req.in_header.unique);
	uint32_t used_len = virtiofs_send_receive(dev, REQUEST_QUEUE, buf, 3, 2);

	LOG_INF("received FUSE_UNLINK response, unique=%" PRIu64, req.out_header.unique);

	return virtiofs_validate_response(
		&req.out_header, FUSE_UNLINK, used_len, sizeof(req.out_header)
	);
}
