...
LOG_MODULE_REGISTER(virtiofs_fuse, CONFIG_VIRTIOFS_LOG_LEVEL);
...
void fuse_fill_header(struct fuse_in_header *hdr, uint32_t len, uint32_t opcode, uint64_t nodeid)
{
	...
	hdr->nodeid = nodeid;
	hdr->uid = CONFIG_VIRTIOFS_UID_VALUE;
	hdr->gid = CONFIG_VIRTIOFS_GID_VALUE;
	hdr->pid = CONFIG_VIRTIOFS_PID_VALUE;
	hdr->total_extlen = 0;
}
...
{
	fuse_fill_header(
		&req->in_header, sizeof(req->in_header) + fname_len, FUSE_UNLINK, FUSE_ROOT_INODE
	);
}
