...
static int nvs_ate_valid(struct nvs_fs *fs, uint16_t entry_offset,
...
static int nvs_lookup_cache_rebuild(struct nvs_fs *fs)
{
	...
	while (true) {
		...
		if (ate.id != 0xFFFF && *cache_entry == NVS_LOOKUP_CACHE_NO_ADDR &&
		    nvs_ate_valid(fs, (ate_addr & ADDR_OFFS_MASK), &ate)) {
			 ...
			 const struct nvs_ate *entry)
{
	...
	if ((nvs_ate_crc8_check(entry)) || (position > entry_offset)) {
		return 0;
	}
...
static int nvs_recover_last_ate(struct nvs_fs *fs, uint32_t *addr)
{
	...
	*addr -= ate_size;
	...
	while (ate_end_addr > data_end_addr) {
		...
		if (nvs_ate_valid(fs, (ate_end_addr & ADDR_OFFS_MASK), &end_ate)) {
...
static int nvs_gc(struct nvs_fs *fs)
{
	...
	do {
		...
		if (!nvs_ate_valid(fs, (gc_prev_addr & ADDR_OFFS_MASK), &gc_ate)) {
			continue;
		}
		...
		do {
			...
			if ((wlk_ate.id == gc_ate.id) &&
			    (nvs_ate_valid(fs, (wlk_prev_addr & ADDR_OFFS_MASK), &wlk_ate))) {
				break;
			}
...
static int nvs_startup(struct nvs_fs *fs)
{
	...
	while (fs->ate_wra >= fs->data_wra) {
		...
		if (nvs_ate_valid(fs, (fs->ate_wra & ADDR_OFFS_MASK), &last_ate)) {
	...
	if (rc) {
		...
		while ((addr & ADDR_OFFS_MASK) < (fs->sector_size - ate_size)) {
			...
			if (nvs_ate_valid(fs, (addr & ADDR_OFFS_MASK), &gc_done_ate) &&
			    (gc_done_ate.id == 0xffff) &&
			    (gc_done_ate.len == 0U)) {
				gc_done_marker = true;
				break;
...
ssize_t nvs_write(struct nvs_fs *fs, uint16_t id, const void *data, size_t len)
{
	...
	while (1) {
		...
		if ((wlk_ate.id == id) &&
		    (nvs_ate_valid(fs, (rd_addr & ADDR_OFFS_MASK), &wlk_ate))) {
			prev_found = true;
			break;
		}
...
ssize_t nvs_calc_free_space(struct nvs_fs *fs)
{
	...
	while (1) {
		...
		if (nvs_ate_valid(fs, (step_prev_addr & ADDR_OFFS_MASK), &step_ate)) {
