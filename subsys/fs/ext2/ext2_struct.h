...
#define EXT2_DATA_SBLOCK(fs) \
	((struct disk_superblock *)((uint8_t *)(fs)->sblock->memory + (fs)->sblock_offset))

struct ext2_data {
	struct block *sblock;       /* superblock */
	...
	void (*close_backend)(struct ext2_data *fs);
	int64_t (*get_device_size)(struct ext2_data *fs);
	int64_t (*get_write_size)(struct ext2_data *fs);
	int (*read_page)(struct ext2_data *fs, void *buf, uint32_t len);
	int (*write_page)(struct ext2_data *fs, const void *buf, uint32_t len);
	int (*read_superblock)(struct ext2_data *fs, struct disk_superblock *sb);
