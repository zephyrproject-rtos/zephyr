...
int ext2_format(struct ext2_data *fs, struct ext2_cfg *cfg)
{
	...
	uint32_t mem_per_inode = cfg->bytes_per_inode + sizeof(struct disk_inode);

	uint32_t inodes_per_block = cfg->block_size / sizeof(struct disk_inode);
	...
	struct block *sb_block = get_block(fs, sb_block_num);
	struct block *bg_block = get_block(fs, bg_block_num);
	struct block *bbitmap_block = get_block(fs, bbitmap_block_num);
	struct block *ibitmap_block = get_block(fs, ibitmap_block_num);
	struct block *in1_block = get_block(fs, in1_block_num);
	struct block *root_dir_blk_block = get_block(fs, root_dir_blk_num);
	...
	struct disk_superblock *sb =
		(struct disk_superblock *)((uint8_t *)sb_block->memory + sb_offset);
	...
	sb->s_def_resgid = 0;
	sb->s_first_ino = 11;
	sb->s_inode_size = sizeof(struct disk_inode);
	...
	struct disk_bgroup *bg = bg_block->memory;
	...
	/* Inode table */
	struct disk_inode *in1 = in1_block->memory;
	...
	uint8_t *bbitmap = bbitmap_block->memory;
	...
	uint8_t *ibitmap = ibitmap_block->memory;

	memset(ibitmap, 0, cfg->block_size);
	ibitmap[0] = 0xff;
	ibitmap[1] = 0x03;

	memset(root_dir_blk_block->memory, 0, cfg->block_size);

	struct disk_dentry *de = root_dir_blk_block->memory;

	de->de_inode = 2;
	de->de_rec_len = sizeof(struct disk_dentry) + 4;
	de->de_name_len = 1;
	de->de_file_type = EXT2_FT_DIR;
	memset(de->de_name, '.', 1);

	de = (struct disk_dentry *)(((uint8_t *)de) + de->de_rec_len);
	de->de_inode = 2;
	...
	block_set_dirty(sb_block);
	block_set_dirty(bg_block);
	block_set_dirty(in1_block);
	block_set_dirty(bbitmap_block);
	block_set_dirty(ibitmap_block);
	block_set_dirty(root_dir_blk_block);

out:
	drop_block(sb_block);
	drop_block(bg_block);
	drop_block(in1_block);
	drop_block(bbitmap_block);
	drop_block(ibitmap_block);
	drop_block(root_dir_blk_block);
	return ret;
}
