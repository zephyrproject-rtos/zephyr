 ...
 * Copyright (c) 2025,2026 Conny Marco Menebröcker
 *
	...
	while (true) {
		if (wtx == 0U) {
			...
			uint16_t payload_len =
				MIN(tx_data_size, tx_data_len - (block * tx_data_size));

			block_size = hdr_len + payload_len;
			memcpy(&tx_frame[hdr_len], &tx_data[block * tx_data_size], payload_len);
