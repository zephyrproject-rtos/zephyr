	...
	while (node != NULL) {
		...
		if (iodev_sqe->sqe.iodev == NULL) {
			...
			while (curr->sqe.flags & (RTIO_SQE_TRANSACTION | RTIO_SQE_CHAINED)) {
				...
				__ASSERT(transaction != chained, "Expected chained or transaction flag, not both");
#endif
				node = rtio_mpsc_pop(&iodev_sqe->r->sq);
				next = CONTAINER_OF(node, struct rtio_iodev_sqe, q);
				LOG_DBG("curr %p, next %p", (void *)curr, (void *)next);
				curr->next = next;
				curr = next;
				curr->r = r;

				__ASSERT(curr != NULL, "Expected a valid sqe following transaction or chain flag");
			}

			LOG_DBG("tail of chain starting at %p is %p", (void *)iodev_sqe, (void *)curr);
			curr->next = NULL;
			curr->r = r;

			/* Start the list */
			LOG_DBG("starting iodev_sqe %p", (void *)iodev_sqe);
...
void rtio_executor_ok(struct rtio_iodev_sqe *iodev_sqe, int result)
{
	...
	LOG_DBG("iodev_sqe returned ok %p", (void *)iodev_sqe);
