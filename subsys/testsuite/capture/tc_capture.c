...
static size_t capture_linearize(char *dst, size_t dst_size)
{
	...
	if (capture_wrapped) {
		for (size_t i = capture_head; (i < sizeof(capture_buf)) && (len < dst_size); i++) {
...
static void log_capture_process(const struct log_backend *const backend, union log_msg_generic *msg)
{
	ARG_UNUSED(backend);
...
#if defined(CONFIG_PRINTK)
	/*
	 * Starting twice without an intervening stop would chain the hook onto
	 * itself and recurse forever on the next character printed.
	 */
	__ASSERT(__printk_get_hook() != capture_char_out, "%s() called while already capturing",
		 __func__);
