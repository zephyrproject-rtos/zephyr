...
static ssize_t settings_file_read_fn(void *back_end, void *data, size_t len)
{
	struct settings_file_read_fn_arg *rd_fn_arg = back_end;
...
static int settings_file_load(struct settings_store *cs)
{
	...
	while (file_get_next_entry(&file, &loc) == 0) {
		lines++;

		rc = fs_read(&file, name, loc.len_name);
		if (rc < 0) {
			continue;
		}
		name[loc.len_name] = '\0';

		/* Setup and call set routine */
		read_fn_arg.stor_ctx = &file;
		read_fn_arg.seek = loc.off_value;
		...
		ch->h_set(name_argc - 1, &name_argv[1], loc.len_value,
...
/* Initialize the file backend. This must be called after the root fs has been
 * initialized (mounted).
 */
