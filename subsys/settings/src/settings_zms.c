...
static int settings_zms_update_ll(struct settings_zms *cf, uint32_t name_hash)
{
	int rc = 0;
...
static int settings_zms_delete(struct settings_zms *cf, uint32_t name_hash)
{
	...
	rc = settings_zms_update_ll(cf, name_hash);
	if (rc < 0) {
		return rc;
	}
...
static int settings_zms_load(struct settings_store *cs, const struct settings_load_arg *arg)
{
	...
	while (ll_hash_id) {
		 ...
		 * setting's value.
		 */
		rc1 = zms_read(&cf->cf_zms, ZMS_NAME_HASH_ID(ll_hash_id), &name, sizeof(name));
		...
		if (ZMS_COLLISION_NUM(ll_hash_id) > cf->hash_collision_num) {
			cf->hash_collision_num = ZMS_COLLISION_NUM(ll_hash_id);
		}
			     ...
			     size_t val_len)
{
	...
	do {
		rc = zms_read(&cf->cf_zms, name_hash, &rdname, sizeof(rdname));
		if (rc == -ENOENT) {
			/* Let's verify other name hashes if some collisions occurred */
			for (int i = 0; i < cf->hash_collision_num; i++) {
				rc = zms_read(&cf->cf_zms,
					      name_hash + i * LSB_GET(ZMS_COLLISIONS_MASK), &rdname,
					      sizeof(rdname));
				if (rc == -ENOENT) {
					continue;
				} else if (rc < 0) {
			...
			/* This hash doesn't exist in storage save the entry unless it is a delete
			 */
			goto no_hash_collision;
		} else if (rc < 0) {
			/* error while reading */
		...
		/* At this step, the hash exist, let's find out if the entry exist or if it is
		 * a hash collision.
		 */
		rdname[rc] = '\0';
		if (!strcmp(name, rdname)) {
			/* Hash exist and the names are equal, we should not write the names
			 * again.
			 */
			write_name = false;
			goto no_hash_collision;
		...
		/* There is a hash collision here, let's increment the hash number until
		 * we find a free hash, starting from the original one.
		 */
		name_hash = (name_hash + LSB_GET(ZMS_COLLISIONS_MASK)) % ZMS_HASH_MASK;
		collision_num++;

	} while (collision_num < ZMS_MAX_COLLISIONS);

	/* if we reach ZMS_MAX_COLLISIONS there is no more space to store hash values */
...
int settings_zms_get_last_hash_ids(struct settings_zms *cf)
{
	struct settings_hash_linked_list settings_element;
