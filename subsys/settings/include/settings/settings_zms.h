 ...
 * The 2 MSB bits have always the same value 10, the lsb for the name's hash is 0
 * and the hash_collisions_bits is configurable through CONFIG_SETTINGS_ZMS_MAX_COLLISIONS_BITS.
 * If we detect a collision between hashed names we increment the hash_collision_bits
 * until we find a free hash.
 * Separately, we store a linked list using the names hash with lsb = 1.
 * The linked list is used to maintain a relation between all hashes. This is necessary to load all
 * settings at initialization.
 * The bit representation of the 32 bits hash number is the following:
 * --------------------------------------------------------------
 ...
 * LL_bit : = 0 when this is a name's hash
 *	    = 1 when this is the linked list hash corresponding to the name's hash
 *
...
#define ZMS_UPDATE_COLLISION_NUM(x, y) ((x & ~ZMS_COLLISIONS_MASK) | y)
