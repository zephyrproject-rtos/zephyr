						  ...
						  struct in6_addr *dst)
{
	...
	for (i = 0; i < CONFIG_NET_IPV6_FRAGMENT_MAX_COUNT; i++) {
		...
		if (k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&reassembly[i].timer)) &&
		    reassembly[i].id == id &&
	...
	k_work_reschedule(&reassembly[avail].timer,
			      ...
			      struct in6_addr *dst)
{
	...
	for (i = 0; i < CONFIG_NET_IPV6_FRAGMENT_MAX_COUNT; i++) {
		...
		remaining = k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&reassembly[i].timer));
		if (remaining) {
