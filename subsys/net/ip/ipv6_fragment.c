...
static struct net_pkt *reassemble_packet(struct net_ipv6_reassembly *reass)
{
	NET_PKT_DATA_ACCESS_CONTIGUOUS_DEFINE(ipv6_access, struct net_ipv6_hdr);
	...
	/* The packet does not contain a link layer header, so it MUST NOT be
	 * passed to L2; there is a special check for that in process_data()
	 * when handling the packet. The caller hands it back to the IP stack
