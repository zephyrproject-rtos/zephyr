...
#if NET_LOG_LEVEL >= LOG_LEVEL_DBG
	NET_FRAG_CHECK_IF_NOT_IN_USE(frag, atomic_get(&frag->ref) + 1);
#endif

	net_pkt_alloc_add(frag, false, caller, line);

#if CONFIG_NET_PKT_LOG_LEVEL >= LOG_LEVEL_DBG
	NET_DBG("%s (%s) [%d] frag %p ref %ld (%s():%d)",
		pool2str(pool), get_name(pool), get_frees(pool),
		frag, atomic_get(&frag->ref), caller, line);
#endif
...
#endif /* NET_LOG_LEVEL >= LOG_LEVEL_DBG */
{
	...
	NET_DBG("%s (%s) [%d] frag %p ref %ld (%s():%d)",
		...
		frag, atomic_get(&frag->ref) + 1, caller, line);
#endif
...
{
#if CONFIG_NET_PKT_LOG_LEVEL >= LOG_LEVEL_DBG
	NET_DBG("pkt %p parent %p frag %p ref %ld (%s:%d)",
		pkt, parent, frag, atomic_get(&frag->ref), caller, line);
#endif
...
#if CONFIG_NET_PKT_LOG_LEVEL >= LOG_LEVEL_DBG
		NET_FRAG_CHECK_IF_NOT_IN_USE(new, atomic_get(&new->ref) + 1);

		net_pkt_alloc_add(new, false, caller, line);

		NET_DBG("%s (%s) [%d] frag %p ref %ld (%s():%d)",
			pool2str(pool), get_name(pool), get_frees(pool),
			new, atomic_get(&new->ref), caller, line);
#endif
	} while (size);
