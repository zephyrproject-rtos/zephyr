...
static void ipv6_ns_reply_timeout(struct k_work *work)
{
	...
	for (i = 0; i < CONFIG_NET_IPV6_MAX_NEIGHBORS; i++) {
		...
		if (remaining > 0) {
			if (!k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&ipv6_ns_reply_timer))) {
...
static void ipv6_nd_restart_reachable_timer(struct net_nbr *nbr, int64_t time)
{
	...
	remaining = k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&ipv6_nd_reachable_timer));
