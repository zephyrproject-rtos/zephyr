		...
		struct gptp_md_sync_info *sync_send)
{
	...
	/*
	 * Compute correction field according to
	 * IEEE802.1AS 11.2.14.2.3.
	 *
	 * The correction_field already contains the timestamp
	 * of the sync message.
	 *
	 * TODO: if the value to be stored in correction_field
	 * is too big to be represented, the field should
	 * be set to all 1's except the most significant bit.
	 */
...
static void gptp_md_init_pdelay_req_state_machine(int port)
{
	...
	k_timer_init(&state->pdelay_timer,
		     gptp_md_pdelay_req_timeout, NULL);
