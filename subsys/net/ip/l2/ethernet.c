...
#define print_vlan_ll_addrs(pkt, type, tci, len, src, dst)		   \
	do {								   \
		...
		NET_DBG("iface %p src %s dst %s type 0x%x tag %d pri %d "  \
			...
			net_sprint_ll_addr((dst)->addr,			   \
					   sizeof(struct net_eth_addr)),   \
			type, net_eth_get_vid(tci), net_eth_get_pcp(tci),  \
			(size_t)len);					   \
	} while (0)
#else
...
struct net_if *net_eth_get_vlan_iface(struct ethernet_context *ctx, u16_t tag)
{
	...
	/* TODO: This function should be very fast as it is called for every
	 * received packet.
	 */
				      ...
				      struct net_pkt *pkt)
{
	...
	if (net_eth_is_vlan_enabled(ctx, iface)) {
		if (type == NET_ETH_PTYPE_VLAN) {
			...
			iface = net_eth_get_vlan_iface(ctx,
						       net_pkt_vlan_tag(pkt));
			if (!iface) {
				NET_DBG("No such interface for VLAN tag %d",
					net_pkt_vlan_tag(pkt));
				return NET_DROP;
			}

			if (iface != net_pkt_iface(pkt)) {
				NET_DBG("Setting iface %p to pkt %p", iface,
					pkt);
			}
			...
			/* It might be that VLAN is not enabled for this
			 * specific interface, so check it next.
			 */
			u16_t tag = net_eth_get_vlan_tag(iface);

			if (tag != NET_VLAN_TAG_UNSPEC) {
				/* VLAN is enabled for this interface but
				 * the received pkt does not have proper
				 * packet type so drop it.
				 */
				NET_DBG("VLAN enabled (tag %d) but pkt is "
					"wrong type (0x%x)", tag, type);
				return NET_DROP;
			}
		}
	}
