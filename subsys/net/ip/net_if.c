				  ...
				  struct net_if_addr *ifaddr)
{
	...
	if (net_if_is_up(iface)) {
		...
		if (!net_ipv6_start_dad(iface, ifaddr)) {
			...
			if (!k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&dad_timer))) {
...
void net_if_start_rs(struct net_if *iface)
{
	...
	if (!net_ipv6_start_rs(iface)) {
		...
		/* FUTURE: use schedule, not reschedule. */
		if (!k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&rs_timer))) {
...
static void address_lifetime_timeout(struct k_work *work)
{
	...
	if (next_update != UINT32_MAX) {
		...
		k_work_reschedule(&address_lifetime_timer,
				      K_MSEC(next_update));
	}
...
static void prefix_lifetime_timeout(struct k_work *work)
{
	...
	if (next_update != UINT32_MAX) {
		k_work_reschedule(&prefix_lifetime_timer,
				      K_MSEC(next_update));
	}
...
static void iface_ipv6_init(int if_count)
{
	...
	k_work_init_delayable(&address_lifetime_timer, address_lifetime_timeout);
