			  ...
			  bool is_remote)
{
	...
	if (IS_ENABLED(CONFIG_NET_IPV6) &&
		...
		if (is_remote) {
			net_ipaddr_copy(&addr6, &ip_hdr->ipv6->src);
		} else {
			net_ipaddr_copy(&addr6, &ip_hdr->ipv6->dst);
		}
		...
		if (is_remote) {
			net_ipaddr_copy(&addr4, &ip_hdr->ipv4->src);
		} else {
			net_ipaddr_copy(&addr4, &ip_hdr->ipv4->dst);
		}
