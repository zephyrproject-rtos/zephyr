...
{
	if (!k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&ipv4auto_timer)) ||
	    timeout < k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&ipv4auto_timer))) {
		k_work_cancel_delayable(&ipv4auto_timer);
		k_work_reschedule(&ipv4auto_timer, K_MSEC(timeout));

		NET_DBG("Next wakeup in %d ms",
			k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&ipv4auto_timer)));
	}
}
