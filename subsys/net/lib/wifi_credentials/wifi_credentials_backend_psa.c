...
BUILD_ASSERT(CONFIG_WIFI_CREDENTIALS_MAX_ENTRIES <= CONFIG_PSA_KEY_ID_WIFI_CREDENTIALS_RANGE_SIZE,
...
int wifi_credentials_backend_init(void)
{
	...
	for (size_t i = 0; i < CONFIG_WIFI_CREDENTIALS_MAX_ENTRIES; ++i) {
		...
		size_t key_id = i + CONFIG_PSA_KEY_ID_WIFI_CREDENTIALS_RANGE_START;
...
int wifi_credentials_store_entry(size_t idx, const void *buf, size_t buf_len)
{
	...
	psa_set_key_id(&key_attributes, idx + CONFIG_PSA_KEY_ID_WIFI_CREDENTIALS_RANGE_START);
...
{
	psa_status_t ret = psa_destroy_key(idx + CONFIG_PSA_KEY_ID_WIFI_CREDENTIALS_RANGE_START);
...
{
	size_t length_read = 0;
	size_t key_id = idx + CONFIG_PSA_KEY_ID_WIFI_CREDENTIALS_RANGE_START;
	psa_status_t ret;
