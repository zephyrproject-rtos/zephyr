...
static void sntp_service_handler(struct net_socket_service_event *pev)
{
	...
	ret = zsock_recvfrom(client, rxbuffer, SNTP_SERVER_BUFFER_SIZE, 0,
	...
	sntp_reply.rx_tm_s = htonl(now.tv_sec + OFFSET_1970_JAN_1);
	sntp_reply.rx_tm_f = htonl(NS2FRAC(now.tv_nsec));
	ret = sys_clock_gettime(SYS_CLOCK_REALTIME, &now);
	if (ret < 0) {
		LOG_ERR("system clock error");
		return;
	}
	sntp_reply.tx_tm_s = htonl(now.tv_sec + OFFSET_1970_JAN_1);
	sntp_reply.tx_tm_f = htonl(NS2FRAC(now.tv_nsec));
...
static int setup_sntp_service_socket(struct net_sockaddr_in6 *addr)
{
	...
	sock = zsock_socket(NET_AF_INET6, SOCK_DGRAM, IPPROTO_UDP);
	if (sock < 0) {
	...
	if (ret == 0 && opt) {
		...
		if (ret < 0) {
			LOG_WRN("disabling IPV6_V6ONLY failed");
		}
	}
