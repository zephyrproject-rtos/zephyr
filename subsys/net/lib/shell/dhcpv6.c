...
static int cmd_net_dhcpv6_server_status(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (ret == -ENOENT) {
		if (iface == NULL) {
			PR_WARNING("DHCPv6 server is not running\n");
		} else {
			PR_WARNING("DHCPv6 server is not running on interface "
				   "%d\n", idx);
		}
	} else if (count == 0) {
...
static int cmd_net_dhcpv6_client_start(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 2) {
		...
		if (!IS_ENABLED(CONFIG_NET_IPV6_ND_RA_TX)) {
			PR_WARNING("Set %s to enable %s support, the downstream "
				   "interfaces are ignored.\n",
				   "CONFIG_NET_IPV6_ND_RA_TX",
				   "prefix delegation onto a downstream link");
		}
