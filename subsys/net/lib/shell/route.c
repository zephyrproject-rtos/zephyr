...
{
#if defined(CONFIG_NET_NATIVE_IPV6) && (CONFIG_NET_IPV6_ROUTE)
	...
	route = net_route_ipv6_add(iface, &prefix, NET_IPV6_DEFAULT_PREFIX_LEN,
...
#else /* CONFIG_NET_NATIVE_IPV6 && CONFIG_NET_IPV6_ROUTE */
	PR_INFO("Set %s and %s to enable native %s support."
			" And enable CONFIG_NET_IPV6_ROUTE.\n",
			"CONFIG_NET_NATIVE", "CONFIG_NET_IPV6", "IPv6");
#endif /* CONFIG_NET_NATIVE_IPV6 && CONFIG_NET_IPV6_ROUTE */
	return 0;
}
...
static int cmd_net_ip6_route_del(const struct shell *sh, size_t argc, char *argv[])
{
	...
	route = net_route_ipv6_lookup(iface, &prefix);
	if (route) {
...
#if defined(CONFIG_NET_IPV6_ROUTE) || defined(CONFIG_NET_IPV6_ROUTE_MCAST)
