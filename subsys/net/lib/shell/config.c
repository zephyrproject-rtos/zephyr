...
static struct net_init_config config;
#endif
...
static int cmd_net_config(const struct shell *sh, size_t argc, char *argv[])
{
...
#if defined(CONFIG_NET_CONFIG_SETTINGS_SHELL_ACCESS)
	static struct net_init_config current_config;
	struct net_init_config *cfg;
	int ret;
	...
	ARRAY_FOR_EACH(cfg->network_interfaces, i) {
		struct net_init_config_network_interfaces *iface_cfg, *my_cfg;

		PR("Network interface: %d\n", i + 1);

		iface_cfg = &cfg->network_interfaces[i];
		my_cfg = &config.network_interfaces[i];
		...
		if (iface_cfg->flags[0].value != NULL) {
			PR("\t%sflags %s\n",
		...
		if (IS_ENABLED(MY_CFG_NET_IPV6)) {
			...
			ARRAY_FOR_EACH(iface_cfg->ipv6.ipv6_addresses, j) {
				...
				if (iface_cfg->ipv6.ipv6_addresses[j].value == NULL) {
					continue;
				}
			...
			ARRAY_FOR_EACH(iface_cfg->ipv6.ipv6_multicast_addresses, j) {
				...
				if (iface_cfg->ipv6.ipv6_multicast_addresses[j].value == NULL) {
					continue;
				}
			...
			ARRAY_FOR_EACH(iface_cfg->ipv6.prefixes, j) {
				if (iface_cfg->ipv6.prefixes[j].address == NULL) {
					continue;
				}
...
skip_ipv6:
		if (IS_ENABLED(CONFIG_NET_IPV4) && iface_cfg->ipv4.status) {
			...
			if (IS_ENABLED(CONFIG_NET_DHCPV4_SERVER)) {
				PR("\t\t%sipv4.dhcpv4_server %s\n",
				   CHANGED(&my_cfg->ipv4, &iface_cfg->ipv4, dhcpv4_enabled),
				   iface_cfg->ipv4.dhcpv4_enabled ? "enabled" : "disabled");
			...
			PR("\t\t%sipv4.ipv4_autoconf_enabled %s\n",
			   CHANGED(&my_cfg->ipv4, &iface_cfg->ipv4,
				   ipv4_autoconf_enabled),
			   iface_cfg->ipv4.ipv4_autoconf_enabled ? "yes" : "no");

			ARRAY_FOR_EACH(iface_cfg->ipv4.ipv4_addresses, j) {

				if (iface_cfg->ipv4.ipv4_addresses[j].value == NULL) {
					continue;
				}
			...
			ARRAY_FOR_EACH(iface_cfg->ipv4.ipv4_multicast_addresses, j) {
				...
				if (iface_cfg->ipv4.ipv4_multicast_addresses[j].value == NULL) {
					continue;
				}
...
#define CHECK_SUB_OPTION_ARRAY(opt, name, var, val, cfg)		\
	if (strcmp(option, #opt "." #name) == 0) {			\
		if (array_idx < 0 || array_idx >= ARRAY_SIZE((cfg)->opt.name)) { \
			...
			(cfg)->opt.name[array_idx].var = val;		\
...
static int cmd_net_config_set(const struct shell *sh, size_t argc, char *argv[])
{
	...
	CHECK_SUB_OPTION(ipv4, gateway, value, iface_cfg);
	CHECK_SUB_BOOL_OPTION(ipv4, ipv4_autoconf_enabled, value, iface_cfg);
	CHECK_SUB_BOOL_OPTION(ipv4.dhcpv4_server, status, value, iface_cfg);
	CHECK_SUB_OPTION(ipv4.dhcpv4_server, base_address, value, iface_cfg);
	...
	CHECK_SUB_OPTION(sntp, server, value, &config);
