				    ...
				    enum http_url_flags flags)
{
	...
	for (i = 0; i < CONFIG_HTTP_SERVER_NUM_URLS; i++) {
		...
		if ((root_url == NULL) || !root_url->is_used) {
			continue;
		}
...
ws_only:
	if (ctx->cb.recv) {
		...
		if (ctx->websocket.data_waiting == 0) {
			...
			if ((ctx->websocket.msg_type_flag & WS_FLAG_CLOSE) != 0) {
				NET_DBG("[%p] Close request from peer", ctx);
				http_close(ctx);
				return;
			}

			if ((ctx->websocket.msg_type_flag & WS_FLAG_PING) != 0) {
