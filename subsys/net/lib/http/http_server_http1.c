...
static int handle_http1_static_resource(struct http_resource_detail_static *static_detail,
		...
		if (static_detail->common.content_encoding != NULL &&
			...
			snprintk(http_response, sizeof(http_response),
				 ...
				 static_detail->common.content_type == NULL
					 ? "text/html"
					 : static_detail->common.content_type,
				 len, static_detail->common.content_encoding);
		} else {
			snprintk(http_response, sizeof(http_response), RESPONSE_TEMPLATE "\r\n",
				 "Content-Type: ",
				 static_detail->common.content_type == NULL
					 ? "text/html"
					 : static_detail->common.content_type,
...
#define SEND_RESPONSE(_template, _content_type)                                                    \
	({                                                                                         \
		char http_response[sizeof(_template) + sizeof("Content-Type: \r\n") +              \
				   HTTP_SERVER_MAX_CONTENT_TYPE_LEN + sizeof("xxxx") +             \
				   sizeof("\r\n")];                                                \
		snprintk(http_response, sizeof(http_response), _template,                          \
			 "Content-Type: ", _content_type == NULL ? "text/html" : _content_type);   \
			   ...
			   struct http_client_ctx *client)
{
	...
	ret = SEND_RESPONSE(RESPONSE_TEMPLATE_CHUNKED, dynamic_detail->common.content_type);
	if (ret < 0) {
		return ret;
	}
	...
	while (1) {
		...
		if (send_len > 0) {
			...
			ret = http_server_sendall(client, dynamic_detail->data_buffer, send_len);
			if (ret < 0) {
				return ret;
			}
			    ...
			    struct http_client_ctx *client)
{
	...
	while (copy_len > 0) {
		...
		if (copy_len == remaining && client->parser_state == HTTP1_MESSAGE_COMPLETE_STATE) {
			status = HTTP_SERVER_DATA_FINAL;
		} else {
			status = HTTP_SERVER_DATA_MORE;
		}
...
static int handle_http1_dynamic_resource(struct http_resource_detail_dynamic *dynamic_detail,
					 struct http_client_ctx *client)
{
	...
	if (dynamic_detail->holder != NULL && dynamic_detail->holder != client) {
		ret = http_server_sendall(client, conflict_response, sizeof(conflict_response) - 1);
		if (ret < 0) {
			LOG_DBG("Cannot write to socket (%d)", ret);
			return ret;
		}
	...
	case HTTP_PATCH:
		...
		goto not_supported;
	case HTTP_DELETE:
		if (user_method & BIT(HTTP_DELETE)) {
			return dynamic_get_req(dynamic_detail, client);
		}
...
{
	struct http_client_ctx *ctx = CONTAINER_OF(parser, struct http_client_ctx, parser);
	...
	if (offset + length > sizeof(ctx->header_buffer) - 1U) {
		...
		if (parser->state == s_header_value_discard_ws) {
			 ...
			 */
			if (strncasecmp(ctx->header_buffer, "Upgrade", sizeof("Upgrade") - 1) ==
			    0) {
				ctx->has_upgrade_header = true;
			} else if (strncasecmp(ctx->header_buffer, "Sec-WebSocket-Key",
	...
	if (offset + length > sizeof(ctx->header_buffer) - 1U) {
		...
		if (parser->state == s_header_almost_done) {
			if (ctx->has_upgrade_header) {
				if (strncasecmp(ctx->header_buffer, "h2c", sizeof("h2c") - 1) ==
				    0) {
					ctx->http2_upgrade = true;
				} else if (strncasecmp(ctx->header_buffer, "websocket",
...
int handle_http1_request(struct http_client_ctx *client)
{
	...
	parsed = http_parser_execute(&client->parser, &client->parser_settings, client->cursor,
				     client->data_len);
	...
	if (client->has_upgrade_header) {
		static const char upgrade_required[] = "HTTP/1.1 426 Upgrade required\r\n"
						       "Upgrade: ";
		static const char upgrade_msg[] = "Content-Length: 13\r\n\r\n"
						  "Wrong upgrade";
		const char *needed_upgrade = "h2c\r\n";

		if (client->websocket_upgrade) {
			if (IS_ENABLED(CONFIG_HTTP_SERVER_WEBSOCKET)) {
				detail = get_resource_detail(client->url_buffer, &path_len, true);
				if (detail == NULL) {
					goto not_found;
				}
...
upgrade_not_found:
		ret = http_server_sendall(client, upgrade_required, sizeof(upgrade_required) - 1);
		if (ret < 0) {
			LOG_DBG("Cannot write to socket (%d)", ret);
			return ret;
		}

		ret = http_server_sendall(client, needed_upgrade, strlen(needed_upgrade));
		if (ret < 0) {
			LOG_DBG("Cannot write to socket (%d)", ret);
			return ret;
		}

		ret = http_server_sendall(client, upgrade_msg, sizeof(upgrade_msg) - 1);
		if (ret < 0) {
			LOG_DBG("Cannot write to socket (%d)", ret);
			return ret;
		}
