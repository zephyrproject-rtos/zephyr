			       ...
			       struct net_if **iface)
{
	...
	net_sin(addr)->sin_port = htons(peer->ctx->wg_ctx->port);
			     ...
			     struct wg_peer *peer)
{
	...
	net_pkt_set_wg_peer_id(pkt, peer->id);
	net_pkt_set_wg_iface(pkt, target_iface);
...
static int wg_ctrl_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	net_pkt_set_iface(pkt, net_pkt_wg_iface(pkt));
...
static int interface_send(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (ctx->wg_ctx->ifindex == 0) {
		...
		if (IS_ENABLED(CONFIG_NET_IPV6) && net_pkt_family(pkt) == AF_INET6) {
			...
			NET_DBG("Unknown address family %d", net_pkt_family(pkt));
			net_buf_unref(buf);
			return -EAFNOSUPPORT;
		}
	...
	net_sin(net_sad(&src_addr))->sin_port = htons(ctx->wg_ctx->port);
	...
	net_pkt_set_wg_iface(pkt_encrypted, iface);
		       ...
		       struct net_if **peer_iface)
{
	...
	if (net_sin(net_sad(&peer->cfg_endpoint))->sin_port == 0) {
		net_sin(net_sad(&peer->cfg_endpoint))->sin_port = htons(WG_DEFAULT_PORT);
	}
