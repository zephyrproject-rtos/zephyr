...
/* Buffer for raw packet: Ethernet header + custom header + payload */
static uint8_t raw_packet_buffer[NET_ETH_MAX_FRAME_SIZE];
		      ...
		      struct zperf_results *results)
{
	...
	if (packet_size < param->hdr_len) {
		NET_ERR("Packet size must be >= header length");
		return -EINVAL;
	}

	/* Create raw packet socket */
	raw_sock = zsock_socket(AF_PACKET, SOCK_RAW, htons(param->ethertype));
	if (raw_sock < 0) {
	...
	raw_addr.sll_protocol = htons(param->ethertype);
	raw_addr.sll_halen = sizeof(param->dst_mac);
	memcpy(raw_addr.sll_addr, param->dst_mac, sizeof(param->dst_mac));

	/* Bind to the interface */
	ret = zsock_bind(raw_sock, (struct sockaddr *)&raw_addr, sizeof(raw_addr));
	if (ret < 0) {
	...
	/* Prepare the packet buffer with custom header and random payload */
	memset(raw_packet_buffer, 0, sizeof(raw_packet_buffer));

	/* Copy custom header if provided */
	if (param->hdr && param->hdr_len > 0) {
		memcpy(raw_packet_buffer, param->hdr, param->hdr_len);
	}

	/* Fill rest with random data as payload */
	if (packet_size > param->hdr_len) {
		sys_rand_get(raw_packet_buffer + param->hdr_len, packet_size - param->hdr_len);
	}
	...
	do {
		...
		/* Optionally refresh random payload for each packet */
		if (packet_size > param->hdr_len) {
			/* Update a portion of payload for variation */
			uint32_t payload_offset = param->hdr_len + (nb_packets % 4);

			if (payload_offset < packet_size) {
				raw_packet_buffer[payload_offset] = (uint8_t)(nb_packets & 0xFF);
			}
		}
			   ...
			   zperf_callback callback, void *user_data)
{
	...
	/* Store header in local buffer since caller's buffer may go out of scope */
	if (param->hdr && param->hdr_len > 0) {
