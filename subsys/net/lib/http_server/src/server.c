...
static unsigned char settings_frame[9] = {
    ...
    0x04, /* Type(8) - 0x04 means setting frames used for configuration or
	     acknowledgement */
    0x00, /* Flags(8) - 0x00 means unused flags and 0x01 means ACK */
    0x00, 0x00, 0x00,
    0x00}; /* Reserved(1), Stream Identifier(31) - 0x00 means that this frame is
	related to the overall connection, and not for a specific stream */

static unsigned char settings_ack[9] = {
    0x00, 0x00, 0x00, /* Length(24) */
    0x04, /* Type(8) - 0x04 means setting frames used for configuration or
	     acknowledgement */
    0x01, /* Flags(8) - 0x01 means ACK */
    0x00, 0x00, 0x00, 0x00}; /* Reserved(1), Stream Identifier(31) */

Http2Frame frames[MAX_FRAMES];
...
void handle_http2_request(struct http2_server_ctx *ctx, int i, int valread)
{
	...
	if (valread < 0) {
		...
		printf("The client does not support HTTP/2.\n");
	    } else {
		printf("The client support HTTP/2.\n");
	    }
	}
    }
	...
	unsigned char ubuffer[BUFFER_SIZE];
	...
	frame_count = parse_http2_frames(ubuffer, remaining_length, frames);
	printf("frames count: %d\n", frame_count);
	print_http2_frames(frames, frame_count);
	stream_header_id = find_headers_frame_stream_id(frames, frame_count);
    ...
    ssize_t n =
	sendall(ctx->client_fds[i].fd, settings_frame, sizeof(settings_frame));
    if (n < 0) {
	perror("ERROR writing to socket");
	exit(EXIT_FAILURE);
    }

    n = sendall(ctx->client_fds[i].fd, settings_ack, sizeof(settings_ack));
    if (n < 0) {
	perror("ERROR writing to socket");
	exit(EXIT_FAILURE);
    }
    ...
    n = sendall(ctx->client_fds[i].fd, settings_frame, sizeof(settings_frame));
    if (n < 0) {
	perror("ERROR writing to socket");
	exit(EXIT_FAILURE);
    }

    unsigned char response_headers_frame[11];
    generate_response_headers_frame(response_headers_frame, stream_header_id);

    n = sendall(ctx->client_fds[i].fd, response_headers_frame,
    ...
    size_t content_size = sizeof(content2);
    sendData(ctx->client_fds[i].fd, content2, content_size, 0x00, 0x01, 0x01);
...
int http2_server_start(int server_fd, struct http2_server_ctx *ctx)
{
    ...
    while (1) {
	int ret = poll(ctx->client_fds, ctx->num_clients + 1, -1);
	if (ret < 0) {
	    perror("poll failed");
	    exit(EXIT_FAILURE);
	}
	...
	for (int i = 0; i <= ctx->num_clients; i++) {
		...
		if (i == 0) {
		    int new_socket = accept_new_client(server_fd);
		    ...
		    /* Read the client data */
		    int valread =
			recv(ctx->client_fds[i].fd, buffer, BUFFER_SIZE - 1, 0);
		    printf("valread length: %u\n", valread);
		    if (valread < 0) {
			...
			http_parser_execute(&parser, &parserSettings, buffer,
					    BUFFER_SIZE);

			/* Check the client preface */
			if (strncmp(buffer, preface, 24) != 0 &&
			    hasUpgradeHeader == 1) {
			    printf("The client does "
				   "not support "
				   "HTTP/2.\n");
...
void generate_response_headers_frame(unsigned char *response_headers_frame,
				     int new_stream_id)
{
    response_headers_frame[0] = 0x00;
    response_headers_frame[1] = 0x00;
    response_headers_frame[2] = 0x02;
    ...
    response_headers_frame[10] = 0x90; /* HPACK accept-encoding: gzip, deflate */
}
