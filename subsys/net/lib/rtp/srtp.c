...
static int srtp_index_estimate(const struct srtp_stream *stream, uint16_t seq, uint32_t *v,
			       uint64_t *index)
{
	int64_t roc_estimate = stream->roc;
	...
	if (roc_estimate > UINT32_MAX) {
		return -EKEYEXPIRED;
	}

	*v = (uint32_t)roc_estimate;
...
int srtp_policy_check(const struct srtp_policy *policy)
{
	if (policy == NULL) {
		return -EINVAL;
	}
	...
	if (policy->auth == SRTP_AUTH_NULL && !srtp_cipher_is_aead(policy->cipher)) {
		NET_WARN("Unauthenticated SRTP policy: no integrity or replay protection");
	}
...
int srtp_stream_protect(struct srtp_stream *stream, uint8_t *buf, size_t pkt_len, size_t buf_size,
			size_t *out_len)
{
	size_t hdr_len;
	size_t tag_len;
...
int srtp_stream_unprotect(struct srtp_stream *stream, uint8_t *buf, size_t pkt_len,
			  size_t *out_len)
{
	size_t hdr_len;
	size_t tag_len;
