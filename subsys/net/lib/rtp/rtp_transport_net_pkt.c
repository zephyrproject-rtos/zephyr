...
#ifdef CONFIG_SRTP
BUILD_ASSERT(CONFIG_SRTP_NET_PKT_BUF_SIZE > RTP_MIN_HEADER_LEN + SRTP_TRAILER_MAX_LEN,
							 ...
							 struct net_pkt *pkt)
{
	...
	if (len > sizeof(srtp_ctx->net_pkt_buf)) {
		NET_DBG("SRTP packet of %zu bytes exceeds CONFIG_SRTP_NET_PKT_BUF_SIZE (%d)", len,
			CONFIG_SRTP_NET_PKT_BUF_SIZE);
		return NET_DROP;
	}
					    ...
					    uint8_t padding)
{
	...
	len = rtp_packet_serialize(rtp_pkt, padding, srtp_ctx->net_pkt_buf,
				   sizeof(srtp_ctx->net_pkt_buf) - SRTP_TRAILER_MAX_LEN);
	if (len < 0) {
		ret = len;
		goto unlock;
	}
