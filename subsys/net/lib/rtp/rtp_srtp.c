...
/* Locking: srtp_ctx->lock guards the receive stream binding table (rx_bound,
 * rx_users, rx_enabled) and is never held across crypto or the receive
 * callback; each stream's own lock serializes that stream's crypto state, so
 * different sources unprotect concurrently; net_pkt_lock guards the staging
 * buffer for the whole net_pkt receive path, callback included. Order:
 * session->lock (RTP API), then net_pkt_lock, then either srtp_ctx->lock or
 * a stream lock; those two are never nested with each other.
 */
			 ...
			 const struct srtp_policy *rx_policy, struct srtp_session_ctx *srtp_ctx)
{
	...
	k_condvar_init(&srtp_ctx->rx_drained);
	...
	if (rx_policy != NULL) {
		ret = srtp_policy_check(rx_policy);
		if (ret < 0) {
			goto fail;
		}

		/* Keep a copy of the policy and key material for late binding
		 * of receive streams to their SSRC; srtp_policy_check()
...
int rtp_session_clear_srtp(struct rtp_session *session)
{
	...
	/* Unpublish first: the transports and rtp_session_recv() snapshot
	 * the pointer once per packet, so no new packet processing enters
	 * once it is cleared. rtp_session_send() holds the session lock, so
	 * this also waits out an in-flight transmission.
	 */
	(void)k_mutex_lock(&session->lock, K_FOREVER);
	...
	/* Stop new receive stream holds (rtp_srtp_rx_stream_get() re-checks
	 * under the lock), then wait for in-flight packets to drop theirs
	 * before the streams are destroyed.
	 */
	srtp_ctx->rx_enabled = false;

	for (size_t i = 0; i < ARRAY_SIZE(srtp_ctx->rx); i++) {
		while (srtp_ctx->rx_users[i] > 0U) {
			(void)k_condvar_wait(&srtp_ctx->rx_drained, &srtp_ctx->lock, K_FOREVER);
		}

		if (srtp_ctx->rx_bound[i]) {
			(void)srtp_stream_deinit(&srtp_ctx->rx[i]);
			srtp_ctx->rx_bound[i] = false;
		}
	}
...
#ifdef CONFIG_RTP_TRANSPORT_NET_PKT
	(void)k_mutex_lock(&srtp_ctx->net_pkt_lock, K_FOREVER);
		     ...
		     size_t *out_len)
{
	...
	return srtp_stream_protect(&srtp_ctx->tx, buf, pkt_len, buf_size, out_len);
}

static int rtp_srtp_rx_stream_get(struct srtp_session_ctx *srtp_ctx, uint32_t ssrc, size_t *slot)
{
	int free_slot = -1;
	bool found = false;
	int ret;
	...
	/* Re-checked under the lock: rtp_session_clear_srtp() clears the
	 * flag before draining, so no new hold is taken during or after
	 * teardown.
	 */
	if (!srtp_ctx->rx_enabled) {
		(void)k_mutex_unlock(&srtp_ctx->lock);
		return -ENOENT;
	}
 ...
 */
static void rtp_srtp_rx_stream_put(struct srtp_session_ctx *srtp_ctx, size_t slot)
{
	struct srtp_stream *stream = &srtp_ctx->rx[slot];
	...
	if (!srtp_ctx->rx_enabled) {
		/* rtp_session_clear_srtp() owns the teardown and waits for
		 * the holds to drain.
		 */
		if (srtp_ctx->rx_users[slot] == 0U) {
			(void)k_condvar_signal(&srtp_ctx->rx_drained);
		}
	} else if (srtp_ctx->rx_users[slot] == 0U && !stream->seq_initialized) {
...
int rtp_srtp_unprotect(struct rtp_session *session, uint8_t *buf, size_t pkt_len, size_t *out_len)
{
	...
	ret = rtp_srtp_rx_stream_get(srtp_ctx, sys_get_be32(&buf[8]), &slot);
	if (ret < 0) {
		return ret;
	}

	ret = srtp_stream_unprotect(&srtp_ctx->rx[slot], buf, pkt_len, out_len);

	rtp_srtp_rx_stream_put(srtp_ctx, slot);
