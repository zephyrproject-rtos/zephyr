...
static int rtp_session_start_tx(struct rtp_session *session)
{
	...
	if (!rtp_srtp_tx_started(session)) {
		...
		/* SRTP receivers bootstrap the rollover counter to zero from
		 * the first packet they see, and without key management there
		 * is no way to signal an early wrap (RFC 3711 section 8).
		 * Keep the initial index in the lower half of the sequence
		 * space so a receiver joining before the first wrap estimates
		 * it correctly, as libwebrtc does.
		 */
		if (rtp_srtp_tx_enabled(session)) {
			session->sequence_number &= 0x7FFFU;
		}
	}
