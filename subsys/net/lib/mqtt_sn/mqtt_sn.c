...
struct mqtt_sn_gateway {
	...
	char gw_id;
	int64_t adv_timer;
	struct mqtt_sn_addr gw_addr;
};

K_MEM_SLAB_DEFINE_STATIC(publishes, sizeof(struct mqtt_sn_publish),
			 CONFIG_MQTT_SN_LIB_MAX_PUBLISH, 4);
K_MEM_SLAB_DEFINE_STATIC(topics, sizeof(struct mqtt_sn_topic),
			 CONFIG_MQTT_SN_LIB_MAX_TOPICS, 4);
K_MEM_SLAB_DEFINE_STATIC(gateways, sizeof(struct mqtt_sn_gateway),
			 CONFIG_MQTT_SN_LIB_MAX_GATEWAYS, 4);
...
static int encode_and_send(struct mqtt_sn_client *client,
			   ...
			   uint8_t broadcast_radius)
{
	...
	if (broadcast_radius) {
		err = client->transport->sendto(client,
						client->tx.data,
						client->tx.len,
						NULL,
						broadcast_radius);
		...
		if (gw == NULL || gw->gw_addr.addr_len == 0) {
...
static struct mqtt_sn_gateway *mqtt_sn_gw_create(uint8_t gw_id,
						 short duration,
						 struct mqtt_sn_addr gw_addr)
{
	struct mqtt_sn_gateway *gw;
	...
	memcpy(gw->gw_addr.addr, gw_addr.addr, gw_addr.addr_len);
	gw->gw_addr.addr_len = gw_addr.addr_len;
	gw->gw_id = gw_id;
	if (duration == -1) {
		gw->adv_timer = duration;
	} else {
		gw->adv_timer = k_uptime_get() +
			(duration * CONFIG_MQTT_SN_LIB_N_ADV * MSEC_PER_SEC);
	}

	return gw;
}

static struct mqtt_sn_gateway *mqtt_sn_gw_find_id(struct mqtt_sn_client *client,
						  uint16_t gw_id)
{
	struct mqtt_sn_gateway *gw;
...
static void mqtt_sn_do_gwinfo(struct mqtt_sn_client *client)
{
	...
	struct mqtt_sn_data addr = {.data = gw->gw_addr.addr,
								.size = gw->gw_addr.addr_len};
...
{
	struct mqtt_sn_gateway *gw;
	struct mqtt_sn_addr gwaddr = {.addr_len = gw_addr.size};

	memcpy(gwaddr.addr, gw_addr.data, gw_addr.size);
	...
	gw = mqtt_sn_gw_create(gw_id, -1, gwaddr);
	if (!gw) {
		return -ENOMEM;
	}
...
int mqtt_sn_search(struct mqtt_sn_client *client, uint8_t radius)
{
	...
	client->ts_searchgw = k_uptime_get() +
		(T_SEARCHGW_MSEC * sys_rand8_get() / 255);
...
static void handle_advertise(struct mqtt_sn_client *client,
			     struct mqtt_sn_param_advertise *p,
			     struct mqtt_sn_addr rx_addr)
{
	...
	if (gw == NULL) {
		...
		gw->adv_timer = k_uptime_get() +
				(p->duration * CONFIG_MQTT_SN_LIB_N_ADV * MSEC_PER_SEC);
	}
...
static void handle_gwinfo(struct mqtt_sn_client *client,
			  struct mqtt_sn_param_gwinfo *p,
			  struct mqtt_sn_addr rx_addr)
{
	struct mqtt_sn_evt evt = {.type = MQTT_SN_EVT_GWINFO};
	struct mqtt_sn_gateway *gw;
	struct mqtt_sn_addr *gw_add;
	...
	if (p->gw_add.size > 0) {
		struct mqtt_sn_addr gwaddr = {.addr_len = p->gw_add.size};

		memcpy(gwaddr.addr, p->gw_add.data, p->gw_add.size);
		gw_add = &gwaddr;
	} else {
		gw_add = &rx_addr;
	}

	gw = mqtt_sn_gw_create(p->gw_id, -1, *gw_add);
	if (!gw) {
		return;
	}
...
static int handle_msg(struct mqtt_sn_client *client, struct mqtt_sn_addr rx_addr)
{
	int err;
	struct mqtt_sn_param p;
...
int mqtt_sn_input(struct mqtt_sn_client *client)
{
	...
	net_buf_simple_reset(&client->rx);
	rx_addr.addr_len = CONFIG_MQTT_SN_LIB_MAX_ADDR_SIZE;

	next_frame_size = client->transport->recvfrom(client,
							client->rx.data, client->rx.size,
							rx_addr.addr, &rx_addr.addr_len);
	if (next_frame_size <= 0) {
		return next_frame_size;
	}
