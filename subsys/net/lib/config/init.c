...
#define CHECK_FIELD(field, size)					\
	BUILD_ASSERT(sizeof(((struct net_init_config *)0)->network_interfaces[0].field) >= (size))
...
CHECK_FIELD(ipv6.ipv6_multicast_addresses[0].value, INET6_ADDRSTRLEN);
CHECK_FIELD(ipv6.prefixes[0].address, INET6_ADDRSTRLEN);
BUILD_ASSERT(sizeof(((struct net_init_config *)0)->sntp.server) >= INET6_ADDRSTRLEN);
...
static int net_config_settings_get(const char *name, char *val, int val_len_max)
{
	...
	if (settings_name_steq(name, "user", &next) && !next) {
		if (val_len_max != sizeof(struct net_init_config)) {
			NET_ERR("Error net_init_config too large %zu (expecting %zu)",
				(size_t)val_len_max, sizeof(struct net_init_config));
			return -EINVAL;
		}
...
#define UPDATE_IFACE_STR_VAL(x)						\
	if (iface_cfg->__##x##_changed) {				\
		strcpy(cfg->network_interfaces[i].x,			\
		       net_init_config_user.network_interfaces[i].x);	\
		cfg->network_interfaces[i].__##x##_changed = true;	\
	} else {							\
		strcpy(cfg->network_interfaces[i].x,			\
...
#define UPDATE_FLAGS_VAL(x)						\
	if (iface_cfg->flags[j].__##x##_changed) {			\
		strcpy(cfg->network_interfaces[i].flags[j].x,		\
...
#define UPDATE_IPV6_ADDR_VAL(x)						\
	if (iface_cfg->ipv6.ipv6_addresses[j].__##x##_changed) {	\
		strcpy(cfg->network_interfaces[i].ipv6.ipv6_addresses[j].x, \
...
#define UPDATE_IPV6_MADDR_VAL(x)					\
	if (iface_cfg->ipv6.ipv6_multicast_addresses[j].__##x##_changed) { \
		strcpy(cfg->network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x, \
...
#define UPDATE_IPV6_PREFIX_STR_VAL(x)					\
	if (iface_cfg->ipv6.prefixes[j].__##x##_changed) {		\
		strcpy(cfg->network_interfaces[i].ipv6.prefixes[j].x,	\
		       net_init_config_user.network_interfaces[i].ipv6.prefixes[j].x); \
		cfg->network_interfaces[i].ipv6.prefixes[j].__##x##_changed = true; \
	} else {							\
		strcpy(cfg->network_interfaces[i].ipv6.prefixes[j].x,	\
...
#define UPDATE_IPV4_AUTOCONF_VAL(x)					\
	if (iface_cfg->ipv4.ipv4_autoconf.__##x##_changed) {		\
		cfg->network_interfaces[i].ipv4.ipv4_autoconf.x =	\
			net_init_config_user.network_interfaces[i].ipv4.ipv4_autoconf.x; \
		cfg->network_interfaces[i].ipv4.ipv4_autoconf.__##x##_changed = true; \
	} else {							\
		cfg->network_interfaces[i].ipv4.ipv4_autoconf.x =	\
			config->network_interfaces[i].ipv4.ipv4_autoconf.x; \
		cfg->network_interfaces[i].ipv4.ipv4_autoconf.__##x##_changed = false; \
	}

#define UPDATE_IPV4_DHCPV4_VAL(x)					\
	if (iface_cfg->ipv4.dhcpv4.__##x##_changed) {			\
		cfg->network_interfaces[i].ipv4.dhcpv4.x =		\
		net_init_config_user.network_interfaces[i].ipv4.dhcpv4.x; \
		cfg->network_interfaces[i].ipv4.dhcpv4.__##x##_changed = true; \
	} else {							\
		cfg->network_interfaces[i].ipv4.dhcpv4.x =		\
			config->network_interfaces[i].ipv4.dhcpv4.x;	\
		cfg->network_interfaces[i].ipv4.dhcpv4.__##x##_changed = false; \
	}
...
#define UPDATE_IPV4_STR_VAL(x)						\
	if (iface_cfg->ipv4.__##x##_changed) {				\
		strcpy(cfg->network_interfaces[i].ipv4.x,		\
		       net_init_config_user.network_interfaces[i].ipv4.x); \
		cfg->network_interfaces[i].ipv4.__##x##_changed = true;	\
	} else {							\
		strcpy(cfg->network_interfaces[i].ipv4.x,		\
...
#define UPDATE_IPV4_ADDR_VAL(x)						\
	if (iface_cfg->ipv4.ipv4_addresses[j].__##x##_changed) {	\
		strcpy(cfg->network_interfaces[i].ipv4.ipv4_addresses[j].x, \
...
#define UPDATE_IPV4_MADDR_VAL(x) \
	if (iface_cfg->ipv4.ipv4_multicast_addresses[j].__##x##_changed) {	\
		strcpy(cfg->network_interfaces[i].ipv4.ipv4_multicast_addresses[j].x, \
...
#define UPDATE_IPV4_DHCPV4_SERVER_STR_VAL(x)				\
	if (iface_cfg->ipv4.dhcpv4_server.__##x##_changed) {		\
		strcpy(cfg->network_interfaces[i].ipv4.dhcpv4_server.x,	\
		       net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server.x); \
		cfg->network_interfaces[i].ipv4.dhcpv4_server.__##x##_changed = true; \
	} else {							\
		strcpy(cfg->network_interfaces[i].ipv4.dhcpv4_server.x,	\
...
#define SET_IFACE_STR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].__##x##_changed == true) { \
		if (strcmp(cfg->network_interfaces[i].x,		\
			   net_init_config_user.network_interfaces[i].x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].x, \
			       cfg->network_interfaces[i].x);		\
		}							\
	} else if (cfg->network_interfaces[i].__##x##_changed &&	\
		   strcmp(cfg->network_interfaces[i].x,			\
			  config->network_interfaces[i].x) != 0) {	\
		strcpy(net_init_config_user.network_interfaces[i].x,	\
		       cfg->network_interfaces[i].x);			\
		net_init_config_user.network_interfaces[i].__##x##_changed = true; \
	}
...
#define SET_FLAGS_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].flags[j].__##x##_changed == true) { \
		if (strcmp(cfg->network_interfaces[i].flags[j].x,	\
			   net_init_config_user.network_interfaces[i].flags[j].x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].flags[j].x, \
			       cfg->network_interfaces[i].flags[j].x);	\
		}							\
	} else if (cfg->network_interfaces[i].flags[j].__##x##_changed && \
		strcmp(cfg->network_interfaces[i].flags[j].x, \
		       config->network_interfaces[i].flags[j].x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].flags[j].x, \
		       cfg->network_interfaces[i].flags[j].x);		\
...
#define SET_IPV6_ADDR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv6.ipv6_addresses[j]. \
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv6.ipv6_addresses[j].x, \
			   net_init_config_user.network_interfaces[i].	\
			   ipv6.ipv6_addresses[j].x) != 0) {		\
			strcpy(net_init_config_user.network_interfaces[i]. \
			       ipv6.ipv6_addresses[j].x,		\
			       cfg->network_interfaces[i].ipv6.ipv6_addresses[j].x); \
		}							\
	} else if (cfg->network_interfaces[i].ipv6.ipv6_addresses[j].__##x##_changed && \
		   strcmp(cfg->network_interfaces[i].ipv6.ipv6_addresses[j].x, \
			  config->network_interfaces[i].ipv6.ipv6_addresses[j].x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].ipv6.ipv6_addresses[j].x, \
		       cfg->network_interfaces[i].ipv6.ipv6_addresses[j].x); \
...
#define SET_IPV6_MADDR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv6.ipv6_multicast_addresses[j].\
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x, \
	   net_init_config_user.network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].ipv6. \
		   ...
		   strcmp(cfg->network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x, \
			config->network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].ipv6.	\
		       ipv6_multicast_addresses[j].x,			\
		       cfg->network_interfaces[i].ipv6.ipv6_multicast_addresses[j].x); \
...
#define SET_IPV6_PREFIX_STR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv6.prefixes[j].\
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv6.prefixes[j].x, \
		   net_init_config_user.network_interfaces[i].ipv6.prefixes[j].x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].ipv6.prefixes[j].x, \
			       cfg->network_interfaces[i].ipv6.prefixes[j].x); \
		}							\
	} else if (cfg->network_interfaces[i].ipv6.prefixes[j].__##x##_changed && \
		   strcmp(cfg->network_interfaces[i].ipv6.prefixes[j].x, \
			  config->network_interfaces[i].ipv6.prefixes[j].x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].ipv6.prefixes[j].x, \
		       cfg->network_interfaces[i].ipv6.prefixes[j].x);	\
		net_init_config_user.network_interfaces[i].ipv6.prefixes[j].\
			__##x##_changed = true;				\
...
#define SET_IPV4_DHCPV4_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv4.dhcpv4.	\
	    __##x##_changed == true) {					\
		if (cfg->network_interfaces[i].ipv4.dhcpv4.x !=		\
		    net_init_config_user.network_interfaces[i].ipv4.dhcpv4.x) {	\
			net_init_config_user.network_interfaces[i].ipv4.dhcpv4.x = \
				cfg->network_interfaces[i].ipv4.dhcpv4.x; \
		}							\
	} else if (cfg->network_interfaces[i].ipv4.dhcpv4.__##x##_changed && \
		   cfg->network_interfaces[i].ipv4.dhcpv4.x !=		\
		   config->network_interfaces[i].ipv4.dhcpv4.x) {	\
		net_init_config_user.network_interfaces[i].ipv4.dhcpv4.x = \
			cfg->network_interfaces[i].ipv4.dhcpv4.x;	\
		net_init_config_user.network_interfaces[i].ipv4.dhcpv4.	\
			__##x##_changed = true;				\
	}

#define SET_IPV4_AUTOCONF_VAL(x)					\
	if (net_init_config_user.network_interfaces[i].ipv4.ipv4_autoconf. \
	    __##x##_changed == true) {					\
		if (cfg->network_interfaces[i].ipv4.ipv4_autoconf.x !=	\
		    net_init_config_user.network_interfaces[i].ipv4.ipv4_autoconf.x) { \
			net_init_config_user.network_interfaces[i].ipv4.ipv4_autoconf.x = \
				cfg->network_interfaces[i].ipv4.ipv4_autoconf.x; \
		}							\
	} else if (cfg->network_interfaces[i].ipv4.ipv4_autoconf.__##x##_changed && \
		   cfg->network_interfaces[i].ipv4.ipv4_autoconf.x !=	\
		   config->network_interfaces[i].ipv4.ipv4_autoconf.x) { \
		net_init_config_user.network_interfaces[i].ipv4.ipv4_autoconf.x = \
			cfg->network_interfaces[i].ipv4.ipv4_autoconf.x; \
...
#define SET_IPV4_STR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv4.__##x##_changed == true) { \
		if (strcmp(cfg->network_interfaces[i].ipv4.x,			\
			   net_init_config_user.network_interfaces[i].ipv4.x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].ipv4.x, \
			       cfg->network_interfaces[i].ipv4.x);	\
		}							\
	} else if (cfg->network_interfaces[i].ipv4.__##x##_changed &&	\
		strcmp(cfg->network_interfaces[i].ipv4.x, \
		       config->network_interfaces[i].ipv4.x) != 0) {	\
		strcpy(net_init_config_user.network_interfaces[i].ipv4.x, \
		       cfg->network_interfaces[i].ipv4.x);		\
		net_init_config_user.network_interfaces[i].ipv4.__##x##_changed = true; \
	}
...
#define SET_IPV4_ADDR_VAL(x)						\
	if (net_init_config_user.network_interfaces[i].ipv4.ipv4_addresses[j]. \
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv4.ipv4_addresses[j].x, \
			   net_init_config_user.network_interfaces[i].	\
		   ...
		   strcmp(cfg->network_interfaces[i].ipv4.ipv4_addresses[j].x, \
			  config->network_interfaces[i].ipv4.ipv4_addresses[j].x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].ipv4.ipv4_addresses[j].x, \
		       cfg->network_interfaces[i].ipv4.ipv4_addresses[j].x); \
...
#define SET_IPV4_MADDR_VAL(x) \
	if (net_init_config_user.network_interfaces[i].ipv4.ipv4_multicast_addresses[j].\
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv4.ipv4_multicast_addresses[j].x, \
			   net_init_config_user.network_interfaces[i].ipv4. \
			   ipv4_multicast_addresses[j].x) != 0) {	\
			strcpy(net_init_config_user.network_interfaces[i].ipv4.	\
		   ...
		   strcmp(cfg->network_interfaces[i].ipv4.ipv4_multicast_addresses[j].x, \
			  config->network_interfaces[i].ipv4.           \
			  ipv4_multicast_addresses[j].x) != 0) {        \
		strcpy(net_init_config_user.network_interfaces[i].      \
...
#define SET_IPV4_DHCPV4_SERVER_STR_VAL(x)					\
	if (net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server. \
	    __##x##_changed == true) {					\
		if (strcmp(cfg->network_interfaces[i].ipv4.dhcpv4_server.x, \
		       net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server.x) != 0) { \
			strcpy(net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server.x, \
			       cfg->network_interfaces[i].ipv4.dhcpv4_server.x); \
		}							\
	} else if (cfg->network_interfaces[i].ipv4.dhcpv4_server.__##x##_changed && \
		   strcmp(cfg->network_interfaces[i].ipv4.dhcpv4_server.x, \
			  config->network_interfaces[i].ipv4.dhcpv4_server.x) != 0) { \
		strcpy(net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server.x, \
		       cfg->network_interfaces[i].ipv4.dhcpv4_server.x); \
		net_init_config_user.network_interfaces[i].ipv4.dhcpv4_server. \
			__##x##_changed = true;				\
