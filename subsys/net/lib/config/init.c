				      ...
				      const struct net_init_config_iface *cfg)
{
	...
	for (int j = 0; j < NET_IF_MAX_IPV6_ADDR; j++) {
	...
	for (int j = 0; j < NET_IF_MAX_IPV6_MADDR; j++) {
	...
	for (int j = 0; j < NET_IF_MAX_IPV6_PREFIX; j++) {
				      ...
				      const struct net_init_config_iface *cfg)
{
	...
	/* First set all the static addresses and then enable DHCP */
	for (int j = 0; j < NET_IF_MAX_IPV4_ADDR; j++) {
		if (net_ipv4_is_addr_unspecified(&cfg->ipv4.address[j])) {
			continue;
		}

		ifaddr = net_if_ipv4_addr_add(iface,
				(struct in_addr *)&ipv4->address[j],
		...
		if (ifaddr == NULL) {
			NET_DBG("Cannot %s %s %s to iface %d", "add", "address",
				net_sprint_ipv4_addr(&ipv4->address[j]),
				ifindex);
			continue;
		}
	...
	for (int j = 0; j < NET_IF_MAX_IPV4_MADDR; j++) {
	...
	if (ipv4->netmask_len > 0) {
		struct in_addr netmask = { 0 };

		netmask.s_addr = BIT_MASK(ipv4->netmask_len);
...
static void vlan_setup_from_generated(struct net_if *iface,
				      int ifindex,
				      ...
				      const struct net_init_config_iface *cfg)
{
	...
	ret = net_eth_vlan_enable(iface, vlan->tag);
	if (ret < 0) {
					      ...
					      const char *app_info)
{
	...
	for (int i = 0; i < NET_CONFIG_IFACE_COUNT; i++) {
		...
		vlan_setup_from_generated(iface, ifindex, config, cfg);
