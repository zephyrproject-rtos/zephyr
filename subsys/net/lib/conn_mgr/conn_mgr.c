...
static bool conn_mgr_is_if_oper_up(int index)
{
	return (iface_states[index] & CMGR_IF_STATE_UP) != 0;
}
...
static void conn_mgr_notify_if_readiness(int index)
{
	...
	bool readiness = iface_states[index] & CMGR_IF_STATE_READY;
...
static void conn_mgr_act_on_changes(void)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(iface_states); idx++) {
		...
		is_l4_ready =	conn_mgr_is_if_oper_up(idx) && is_ip_ready;
		was_l4_ready =	iface_states[idx] & CMGR_IF_STATE_READY;
