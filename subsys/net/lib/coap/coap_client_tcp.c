...
static int read_csm_option_value_u32(struct coap_client_tcp_option *option)
{
	unsigned int n, i;
	n = 0;
	i = MIN(4, option->len);
	while (i--) {
		n |= options->value[i] & 0xff;
		n <<= 8;
	}
...
static int handle_response_tcp(struct coap_client_tcp *client, const struct coap_packet *response)
{
	...
	/* CSM handling - process options before matching to request */
	if (response_code == COAP_SIGNAL_CODE_CSM) {
		...
		if (cms_option_num > 0) {
			client->max_block_size = read_csm_option_value_u32(&csm_option);
			LOG_DBG("CSM: Max message size updated to %d", client->max_block_size);
		}
		...
		/* Continue to match request for app callback */
	}
