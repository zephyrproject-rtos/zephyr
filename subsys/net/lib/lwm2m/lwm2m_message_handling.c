...
struct lwm2m_obj_path *lwm2m_read_first_path_ptr(sys_slist_t *lwm2m_path_list);
...
int lwm2m_register_payload_handler(struct lwm2m_message *msg)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(lwm2m_engine_obj_list(), obj, node) {
		...
		SYS_SLIST_FOR_EACH_CONTAINER(lwm2m_engine_obj_inst_list(),
					     obj_inst, node) {
...
int handle_request(struct coap_packet *request,
			  struct lwm2m_message *msg)
{
	int r;
	uint8_t code;
 ...
 */
int32_t retransmit_request(struct lwm2m_ctx *client_ctx,
...
int generate_notify_message(struct lwm2m_ctx *ctx,
				   struct observe_node *obs,
				   void *user_data)
{
	struct lwm2m_message *msg;
