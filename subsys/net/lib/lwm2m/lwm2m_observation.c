...
uint8_t to_hex_digit(uint8_t digit);
...
int lwm2m_notify_observer_path(struct lwm2m_obj_path *path)
{
	...
	for (i = 0; i < lwm2m_sock_ndfs(); ++i) {
		SYS_SLIST_FOR_EACH_CONTAINER(&lwm2m_sock_ctx()[i]->observer, obs, node) {
...
void engine_remove_observer_by_id(uint16_t obj_id, int32_t obj_inst_id)
{
	...
	for (i = 0; i < lwm2m_sock_ndfs(); ++i) {
		SYS_SLIST_FOR_EACH_CONTAINER_SAFE(
			&lwm2m_sock_ctx()[i]->observer, obs, tmp, node) {
			engine_observe_single_path_id_remove(lwm2m_sock_ctx()[i], obs, obj_id,
							     obj_inst_id);

			if (sys_slist_is_empty(&obs->path_list)) {
				remove_observer_from_list(lwm2m_sock_ctx()[i], prev_node, obs);
			} else {
				prev_node = &obs->node;
			}
		}
