...
#define XFER_THREAD_STACK_SIZE 8192
#define XFER_THREAD_PRIORITY K_LOWEST_APPLICATION_THREAD_PRIO
			...
			struct latmon_data *data)
{
	...
	/* return -1 if we have to keep sampling, 0 if all samples collected */
}
...
static void xfer_thread_func(void *p1, void *p2, void *p3)
{
	...
	for (;;) {
		if (k_msgq_get(&xfer_msgq, &sample, K_FOREVER)) {
...
static void monitor_thread_func(void *p1, void *p2, void *p3)
{
	...
	while (!abort_monitor) {
		for (;;) {
			...
			if (!prepare_sample_data(delta, conf, data)) {
				/* Sample data ready */
				break;
			}
		}

		if (enqueue_sample_data(data) < 0) {
			break;
		}
	};
out:
