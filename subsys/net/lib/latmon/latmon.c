...
struct latmon_message {
	net_latmon_get_delta_t get_delta_func;
	int socket;
};
...
static ssize_t to_latmus(int socket, const void *buf, size_t count)
{
	...
	while (count > 0) {
		...
		bytes_written = send(socket, p, count, 0);
		if (bytes_written < 0) {
			if (errno == EINTR)
				continue;
			return -1;
...
static int send_sample_data(int socket, struct latmon_data *data)
{
	...
	return (to_latmus(socket, &ndata, sizeof(ndata)) <= 0 ? -1 : 0);
}

static int send_trailing_data(int socket, struct latmus_conf *conf,
			struct latmon_data *data)
{
	...
	if (data->current_samples) {
		if (send_sample_data(socket, data) < 0) {
			return -1;
		}
	}
	...
	ret = to_latmus(socket, data->histogram, count);
...
static int monitor_entry(void *p1, void *p2, void *p3)
{
	...
	while (!abort_monitor) {
		...
		if (send_sample_data(msg->socket, data) < 0) {
			break;
		}
	}
...
static void handle_connection(struct latmon_message *msg)
{
	...
	memset(data, 0, sizeof(*data));
	for (;;) {
		len = recv(msg->socket, &req, sizeof(req), 0);
		stop_monitoring();
		if (get_latmus_conf(len, &req, conf) < 0) {
			/* we might have to send the histogram */
			if (send_trailing_data(msg->socket, conf, data) < 0) {
				break;
			}
			memset(conf, 0, sizeof(*conf));
			continue;
