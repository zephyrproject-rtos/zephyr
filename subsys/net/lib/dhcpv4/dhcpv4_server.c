...
static struct in_addr net_dhcpv4_server_find_free_address(
	struct dhcpv4_server_ctx *ctx, struct dhcpv4_client_id *client_id);
				   ...
				   uint8_t optlen)
{
	...
	if (selected == NULL) {
		...
		ret = address_provider_callback(ctx->iface, &client_id, &selected_addr,
						address_provider_callback_user_data);
		if (ret == 0) {
			...
			selected->addr = net_dhcpv4_server_find_free_address(ctx, &client_id);
			probe = true;
		}
				  ...
				  uint8_t optlen)
{
	...
	if (ret == 0) {
		...
		if (address_validator_callback != NULL) {
			ret = address_validator_callback(ctx->iface, &client_id,
				&requested_ip, address_validator_callback_user_data);
			if (ret != 0) {
		...
		if ((slot->state == DHCPV4_SERVER_ADDR_RESERVED ||
			...
			if (slot->state == DHCPV4_SERVER_ADDR_RESERVED &&
				...
				LOG_DBG("Address %s expired",
					net_sprint_ipv4_addr(&slot->addr));
				printk("Resetting index %d from timeout\n", i);
				reset_slot(slot);
			}
		}
