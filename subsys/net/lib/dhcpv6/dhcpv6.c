...
static int dhcpv6_handle_dns_server_option(struct net_pkt *pkt)
{
	...
	struct sockaddr_in6 dnses[MAX_DNS_SERVERS] = { 0 };
	...
	ret = dhcpv6_parse_option_dns_servers(pkt, length, dnses, &server_count);
	if (ret < 0) {
		goto out;
	}

	for (uint8_t i = 0; i < server_count; i++) {
		dnses[i].sin6_family = AF_INET6;
