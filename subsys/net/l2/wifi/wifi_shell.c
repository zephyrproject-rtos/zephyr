				 ...
				 enum wifi_iface_mode iface_mode)
{
	...
	while ((opt = getopt_long(argc, argv, "s:p:k:e:w:b:c:m:t:a:B:K:S:T:A:V:I:P:g:d:Rh:i:",
		...
		case 'P':
			...
			break;
		case 'd':
			params->domain_match = state->optarg;
...
static int cmd_wifi_btwt_setup(const struct shell *sh, size_t argc, char *argv[])
{
	...
	{
		params.btwt.btwt_set_cfg[i].btwt_id 		= (uint8_t)shell_strtol(argv[idx++], 10, &err);
		params.btwt.btwt_set_cfg[i].btwt_mantissa 	= (uint16_t)shell_strtol(argv[idx++], 10, &err);
		params.btwt.btwt_set_cfg[i].btwt_exponent 	= (uint8_t)shell_strtol(argv[idx++], 10, &err);
		params.btwt.btwt_set_cfg[i].btwt_nominal_wake = (uint8_t)shell_strtol(argv[idx++], 10, &err);
		if(err)
		{
			PR_ERROR("Parse btwt parameters (err %d)\n", err);
			return -EINVAL;
