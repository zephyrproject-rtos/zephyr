...
static void handle_wifi_scan_result(struct net_mgmt_event_callback *cb)
{
	...
	if (scan_result == 1U) {
		print(context.sh, SHELL_NORMAL,
		      "\n%-4s | %-32s %-5s | %-13s | %-4s | %-15s | %-17s | %-12s \n",
		      "Num", "SSID", "(len)", "Chan (Band)", "RSSI", "Security", "BSSID", "TWT Support");
	}

	print(context.sh, SHELL_NORMAL, "%-4d | %-32s %-5u | %-4u (%-6s) | %-4d | %-15s | %s | %s\n",
		 ...
		 entry->twt_support ? "Enabled":"Disabled");
}
