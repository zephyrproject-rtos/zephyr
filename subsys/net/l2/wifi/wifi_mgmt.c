...
NET_MGMT_REGISTER_REQUEST_HANDLER(NET_REQUEST_WIFI_NEIGHBOR_REP_COMPLETE, wifi_neighbor_rep_complete);
...
void wifi_mgmt_raise_neighbor_rep_recv_event(struct net_if *iface, char *inbuf, size_t buf_len)
{
	...
	if (sscanf(buf, "%s bssid=%s info=%s op_class=%d chan=%d phy_type=%d",
		...
		if (!match && (roaming_params.neighbor_rep.neighbor_cnt < MAX_NEIGHBOR_AP_LIMIT)) {
			...
			roaming_params.neighbor_rep.neighbor_ap[idx].bssid_info[len] = (uint8_t)'\0';
...
static int wifi_btm_query(uint32_t mgmt_request, struct net_if *iface,
			   void *data, size_t len)
{
	const struct device *dev = net_if_get_device(iface);
	const struct wifi_mgmt_ops *const wifi_mgmt_api = get_wifi_api(iface);
...
static int wifi_wps_pbc(uint32_t mgmt_request, struct net_if *iface,
			   void *data, size_t len)
{
	const struct device *dev = net_if_get_device(iface);
	const struct wifi_mgmt_ops *const wifi_mgmt_api = get_wifi_api(iface);
	...
	return wifi_mgmt_api->wps_pbc(dev);
}
...
static int wifi_ap_wps_pbc(uint32_t mgmt_request, struct net_if *iface,
			   void *data, size_t len)
{
	const struct device *dev = net_if_get_device(iface);
	const struct wifi_mgmt_ops *const wifi_mgmt_api = get_wifi_api(iface);

	if (wifi_mgmt_api == NULL || wifi_mgmt_api->ap_wps_pbc == NULL) {
		return -ENOTSUP;
	}

	return wifi_mgmt_api->ap_wps_pbc(dev);
}

NET_MGMT_REGISTER_REQUEST_HANDLER(NET_REQUEST_WIFI_AP_WPS_PBC, wifi_ap_wps_pbc);

static int wifi_ap_wps_pin(uint32_t mgmt_request, struct net_if *iface,
			   void *data, size_t len)
{
	const struct device *dev = net_if_get_device(iface);
	const struct wifi_mgmt_ops *const wifi_mgmt_api = get_wifi_api(iface);
	...
	if (wifi_mgmt_api == NULL || wifi_mgmt_api->ap_wps_pin == NULL) {
		return -ENOTSUP;
	}

	return wifi_mgmt_api->ap_wps_pin(dev, params);
}

NET_MGMT_REGISTER_REQUEST_HANDLER(NET_REQUEST_WIFI_AP_WPS_PIN, wifi_ap_wps_pin);
