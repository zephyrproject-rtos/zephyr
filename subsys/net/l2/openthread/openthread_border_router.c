...
#define OT_BR_STACK_SIZE CONFIG_OPENTHREAD_ZEPHYR_BR_THREAD_STACK_SIZE
#define OT_BR_PRIORITY   CONFIG_OPENTHREAD_ZEPHYR_BR_THREAD_PRIORITY
...
K_MEM_SLAB_DEFINE(border_router_messages_slab, sizeof(struct br_msg_ctx_tag),
		  CONFIG_OPENTHREAD_ZEPHYR_BR_MSG_POOL_NUM, sizeof(void *));
...
int openthread_start_border_router_services(struct net_if *ot_iface, struct net_if *ail_iface)
{
	...
	VerifyOrExit(udp_plat_init(instance, ail_iface_ptr, ot_iface) == OT_ERROR_NONE,
		     error = OT_ERROR_FAILED);
	VerifyOrExit(mdns_plat_socket_init(instance, ail_iface_index) == OT_ERROR_NONE,
		     error = OT_ERROR_FAILED);
	...
	VerifyOrExit(otMdnsRegisterHost(instance, &mdns_host, 0, mdns_handle_register_cb) ==
			     OT_ERROR_NONE,
		     error = OT_ERROR_FAILED);
...
void openthread_border_router_post_message(struct br_msg_ctx_tag *msg_context)
{
...
static void openthread_border_router_process(struct k_work *work)
{
	...
	do {
		context =
			(struct br_msg_ctx_tag *)k_fifo_get(&border_router_msg_rx_fifo, K_NO_WAIT);
		if (context) {
			if (!context->socket) {
				context->cb(context);
			} else {
				...
				otMessageAppend(ot_message, context->buff_and_len.buffer,
						context->buff_and_len.length);
