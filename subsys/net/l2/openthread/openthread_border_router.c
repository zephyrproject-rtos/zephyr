...
int openthread_start_border_router_services(struct net_if *ot_iface, struct net_if *ail_iface)
{
	...
	infra_if_init(instance, ail_iface_ptr);

	/* Call OpenThread API */
	otPlatInfraIfStateChanged(instance, ail_iface_index, true);
	otBorderRoutingInit(instance, ail_iface_index, true);
...
static int openthread_stop_border_router_services(struct net_if *ot_iface, struct net_if *ail_iface)
{
	...
	if (is_border_router_started) {
		openthread_mutex_lock();
		/* De-initialize platform modules first */

		/* Call OpenThread API */
		otPlatInfraIfStateChanged(instance, ail_iface_index, false);
		otBorderRoutingSetEnabled(instance, false);
				   ...
				   struct net_if *iface)
{
	...
	if (mgmt_event == NET_EVENT_IF_DOWN) {
		openthread_stop_border_router_services(ot_context->iface, iface);
		return;
	}
}
