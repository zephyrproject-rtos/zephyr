 ...
 *
 */
typedef struct br_msg_ctx_tag br_msg_ctx;
typedef void (*br_msg_callback)(struct br_msg_ctx_tag *msg_ctx_ptr);
typedef struct br_buff_and_len_tag {
	uint8_t buffer[1500];
	uint16_t length;
} br_buff_and_len;
struct br_msg_ctx_tag {
	void *unused;
	br_msg_callback cb;
	otUdpSocket *socket;

	br_buff_and_len buff_and_len;
	union {
