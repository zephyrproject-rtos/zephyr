...
{
	if (!k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&lldp_tx_timer)) ||
	    timeout < k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&lldp_tx_timer))) {
		k_work_cancel_delayable(&lldp_tx_timer);
		k_work_reschedule(&lldp_tx_timer, K_MSEC(timeout));

		NET_DBG("Next wakeup in %d ms",
			k_ticks_to_ms_ceil32(k_work_delayable_remaining_get(&lldp_tx_timer)));
	}
}
