				  ...
				  bool is_joined)
{
	...
	if (IS_ENABLED(CONFIG_NET_VLAN) && net_eth_is_vlan_interface(iface)) {
		 ...
		 * it there failed, so there is nothing to leave now.
		 */
...
out:
	 ...
	 *
	 * The address is given back when programming it failed, so the caller
	 * does not have to leave a group it failed to join.
	 */
	if (program) {
...
out:
	...
	/* Like when joining, a device that cannot filter has nothing to unset
