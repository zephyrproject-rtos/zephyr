...
static struct k_thread ptp_announce_sender_thread_data;

K_KERNEL_STACK_DEFINE(ptp_sync_sender_thread_stack, NET_PTP_MASTER_STACK_SIZE);
static struct k_thread ptp_sync_sender_thread_data;
...
void net_ptp_master_init(void)
{
	...
	k_thread_create(&ptp_announce_sender_thread_data, ptp_announce_sender_thread_stack,
	...
	k_thread_name_set(&ptp_announce_sender_thread_data, "announce_sender");

	k_thread_create(&ptp_sync_sender_thread_data, ptp_sync_sender_thread_stack,
		K_KERNEL_STACK_SIZEOF(ptp_sync_sender_thread_stack),
		(k_thread_entry_t)ptp_sync_sender,
		NULL, NULL, NULL, K_PRIO_COOP(5), 0, K_NO_WAIT);
	k_thread_name_set(&ptp_sync_sender_thread_data, "sync_sender");
}
