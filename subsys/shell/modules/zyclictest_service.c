...
static int cmd_zyclictest_stop(const struct shell *sh, size_t argc, char **argv)
{
	...
	shell_print(sh, "Max-Latency:\t%6d\t%6d", zyc_data.max_lat[ZYC_INT],
		    zyc_data.max_lat[ZYC_THR]);
	shell_print(sh, "Errors:     \t%6d\t%6d", zyc_data.cnt_err[ZYC_INT],
		    zyc_data.cnt_err[ZYC_THR]);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_zyclictest,
	SHELL_CMD_ARG(start, NULL, SHELL_HELP("Start Zyclictest", "[-i <interval-us>] [-p <prio>]"),
