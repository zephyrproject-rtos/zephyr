			...
			struct shell_static_entry *d_entry)
{
	...
	if (IS_ENABLED(CONFIG_SHELL_CMDS)	&&
	    (strcmp("select", (*argv)[0]) == 0)	&&
	    !shell_cmd_is_selected(shell)) {
		*argv = *argv + 1;
		*argc = *argc - 1;
	}
...
static inline void select_cmd_exit_handle(const struct shell *shell, char data)
{
	if (!IS_ENABLED(CONFIG_SHELL_CMDS)) {
		return;
	}

	if (data == SHELL_VT100_ASCII_CTRL_S && shell->ctx->selected_cmd) {
		shell_cmd_line_erase(shell);
		shell_internal_fprintf(shell, SHELL_WARNING,
				       "Unselected command: %s\n",
				       shell->ctx->selected_cmd->syntax);
	...
	while (true) {
		...
		case SHELL_RECEIVE_DEFAULT:
			...
			default:
				...
				select_cmd_exit_handle(shell, data);
				break;
			}
			break;
...
static void shell_log_process(const struct shell *shell)
{
	...
	do {
		if (!IS_ENABLED(CONFIG_LOG_IMMEDIATE)) {
			...
			processed = shell_log_backend_process(
							    shell->log_backend);
		}
