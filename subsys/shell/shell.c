...
#define LOG_LEVEL CONFIG_SHELL_LOG_LEVEL
LOG_MODULE_REGISTER();
...
/* variables to print commands help */
static u8_t const tab_len = 2;		/* tab used to print help */
static char const cmd_sep[] = " - ";	/* commands separator */
static char const opt_sep[] = ", ";	/* options separator */
static char const help_text_sep = ':';  /* help text separator */
 ...
 * shell_write function is called, it must be ensured that IO buffer of fprintf is
...
struct shell_cmd_entry const * root_cmd_find(const char *syntax)
{
	...
	struct shell_cmd_entry const *cmd;
 ...
 */
static void formatted_text_print(const struct shell *shell,
				 char const *str,
...
static void print_cmd_help(const struct shell *shell)
{
...
static void print_option(const struct shell *shell, const char *opt_name,
			 u16_t width, const char *opt_help)
{
	const size_t offset = width + tab_len + 1;
	...
	/* print option name */
	shell_fprintf(shell, SHELL_NORMAL, "%*c%-*s%c",
		      tab_len, ' ',
		      width, opt_name,
		      help_text_sep);
	...
	/* print option help */
	formatted_text_print(shell, opt_help, offset, false);
}

static void print_options_help(const struct shell *shell,
			       struct shell_getopt_option const *opt,
			       size_t opt_cnt)
{
	static char const help_opt[] = "-h, --help";

	u16_t longest_string = shell_strlen(help_opt) - shell_strlen(opt_sep);
	...
	for (size_t i = 0; i < opt_cnt; ++i) {
		...
		longest_string = len > longest_string ? len : longest_string;
	}

	longest_string += shell_strlen(opt_sep) + tab_len;

	/* help option is printed first */
	print_option(shell, help_opt, longest_string, "Show command help.");

	memset(shell->ctx->temp_buff, 0, longest_string + 1);
	...
	for (size_t i = 0; i < opt_cnt; ++i) {
		...
		print_option(shell, shell->ctx->temp_buff, longest_string,
			     opt[i].optname_help);
	}
}
...
static void print_subcmd_help(const struct shell *shell)
{
	...
	do {
		...
		longest_string = longest_string > len ? longest_string : len;
	...
	while (true) {
		...
		u16_t field_width = longest_string + tab_len;
		shell_fprintf(shell, SHELL_NORMAL, "%*c%-*s%c",
			      tab_len, ' ',
			      field_width, entry->syntax,
			      help_text_sep);
		field_width += tab_len + 1;

		if (entry->help) {
			formatted_text_print(shell, entry->help, field_width,
					     false);
		} else {
			cursor_next_line_move(shell);
		}
...
void shell_help_print(const struct shell *shell,
	...
	print_cmd_help(shell);
	print_options_help(shell, opt, opt_len);
	print_subcmd_help(shell);
}
