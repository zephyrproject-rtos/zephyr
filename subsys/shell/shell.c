...
#if defined(CONFIG_SHELL_ALIASES)
	struct shell_static_entry dloc;
	const char *first_space = strstr(alias_command, " ");
	size_t cmd_len = first_space ? (size_t)(first_space - alias_command)
				     : strlen(alias_command);
	char root_cmd[CONFIG_SHELL_CMD_BUFF_SIZE];

	if ((cmd_len == 0U) || (cmd_len >= sizeof(root_cmd))) {
		return false;
	}

	memcpy(root_cmd, alias_command, cmd_len);
	root_cmd[cmd_len] = '\0';
...
static int execute(const struct shell *sh)
{
	...
	if (IS_ENABLED(CONFIG_SHELL_ALIASES)) {
		...
		ret = z_shell_expand_alias(cmd_buf, CONFIG_SHELL_CMD_BUFF_SIZE);
		if (ret == -E2BIG) {
			z_shell_fprintf(sh, SHELL_ERROR,
