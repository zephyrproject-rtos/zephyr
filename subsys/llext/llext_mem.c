...
/*
 * Initialize the memory partition associated with the extension segments
 */
			...
			uintptr_t start, size_t len)
{
	...
	LOG_DBG("segment %d: start 0x%zx, size %zd", mem_idx, (size_t)start, len);
}
			      ...
			      enum llext_mem mem_idx)
{
	...
	if (ldr->sects[mem_idx].sh_type != SHT_NOBITS &&
		...
		/* Can directly use data in the binary, if peek is supported */
...
void llext_free_segments(struct llext *ext)
{
	for (int i = 0; i < LLEXT_MEM_COUNT; i++) {
		if (ext->mem_on_heap[i]) {
			LOG_DBG("freeing memory for segment %d", i);
