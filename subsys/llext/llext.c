...
static int llext_map_sections(struct llext_loader *ldr, struct llext *ext)
{
	int i, ret;
	size_t pos;
	elf_shdr_t shdr, rodata = {.sh_addr = ~0}, high_shdr = {.sh_offset = 0}, low_shdr = {.sh_offset = ~0};
	const char *name;

	ldr->sects[LLEXT_MEM_RODATA].sh_addr = 0;
	...
	for (i = 0, pos = ldr->hdr.e_shoff;
		...
		if (shdr.sh_addr && !(shdr.sh_flags & (SHF_WRITE | SHF_EXECINSTR)) &&
			...
			rodata = shdr;
			LOG_DBG("rodata: select %#x name %s", shdr.sh_addr, name);
		}
	...
	if (!ldr->sects[LLEXT_MEM_RODATA].sh_addr)
