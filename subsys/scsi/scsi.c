 ...
 */
static int scsi_exec(struct scsi_device *sdev, struct scsi_cmd *cmd)
{
 ...
 */
static void scsi_setup_read(struct scsi_cmd *pccb, uint64_t start, uint32_t blocks)
{
 ...
 */
static void scsi_setup_write(struct scsi_cmd *pccb, uint64_t start, uint32_t blocks)
{
	if ((blocks > (uint32_t)UINT16_MAX) || (start > (uint64_t)UINT32_MAX)) {
 ...
 */
static int scsi_test_unit_ready(struct scsi_device *scsi_dev, int32_t lun)
{
	int ret;
 ...
 * @return 0 if successful, or a negative error code on failure.
 */
int scsi_write(struct scsi_device *scsi_dev, uint64_t sector,
 ...
 *
 * @param sdev Pointer to the scsi_device structure.
 * @param req Pointer to the ioctl arguments.
 *
 * @return 0 if successful, or a negative error code on failure.
 */
static int scsi_ioctl_sg_io(struct scsi_device *sdev, struct sg_io_req *req)
{
...
int scsi_ioctl(struct scsi_device *sdev, int32_t cmd, void *arg)
{
	...
	case SG_IO:
		...
		ret = scsi_ioctl_sg_io(sdev, (struct sg_io_req *)arg);
		break;
	default:
		ret = -EINVAL;
		break;
