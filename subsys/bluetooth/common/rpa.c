...
LOG_MODULE_REGISTER(bt_rpa, LOG_LEVEL);
...
static int ah(const uint8_t irk[16], const uint8_t r[3], uint8_t out[3])
{
	...
	LOG_DBG("irk %s", bt_hex_real(irk, 16));
	LOG_DBG("r %s", bt_hex_real(r, 3));
...
bool bt_rpa_irk_matches(const uint8_t irk[16], const bt_addr_t *addr)
{
	...
	LOG_DBG("IRK %s bdaddr %s", bt_hex_real(irk, 16), bt_addr_str_real(addr));
...
int bt_rpa_create(const uint8_t irk[16], bt_addr_t *rpa)
{
	...
	LOG_DBG("Created RPA %s", bt_addr_str_real((bt_addr_t *)rpa->val));
