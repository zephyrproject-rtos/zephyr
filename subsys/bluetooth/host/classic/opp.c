...
#define LOG_LEVEL CONFIG_BT_OPP_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(bt_opp);
...
{
	struct bt_opp_client *client = CONTAINER_OF(goep, struct bt_opp_client, _goep);

	atomic_set(&client->_transport_state, BT_OPP_TRANSPORT_STATE_CONNECTED);

	LOG_DBG("client %p RFCOMM connected", client);

	if (client->cb != NULL && client->cb->rfcomm_connected != NULL) {
...
static void opp_client_transport_disconnected(struct bt_goep *goep)
{
	...
	if (client->cb != NULL && client->cb->rfcomm_disconnected != NULL) {
...
{
	struct bt_opp_client *client = CONTAINER_OF(obex_client, struct bt_opp_client, _client);
	...
	LOG_DBG("client %p OBEX connect rsp 0x%02x version 0x%02x mopl %u", client, rsp_code,
		version, mopl);

	if (client->cb != NULL && client->cb->connect != NULL) {
		client->cb->connect(client, (enum bt_opp_rsp_code)rsp_code, version, mopl, buf);
	}
}
					  ...
					  struct net_buf *buf)
{
	...
	if (client->cb != NULL && client->cb->disconnect != NULL) {
		client->cb->disconnect(client, (enum bt_opp_rsp_code)rsp_code, buf);
	}
}
				   ...
				   struct net_buf *buf)
{
	...
	if (client->cb != NULL && client->cb->push != NULL) {
		client->cb->push(client, (enum bt_opp_rsp_code)rsp_code, buf);
	}
}
				   ...
				   struct net_buf *buf)
{
	...
	if (client->cb != NULL && client->cb->pull_bcard != NULL) {
		client->cb->pull_bcard(client, (enum bt_opp_rsp_code)rsp_code, buf);
	}
}
				     ...
				     struct net_buf *buf)
{
	...
	if (client->cb != NULL && client->cb->abort != NULL) {
		client->cb->abort(client, (enum bt_opp_rsp_code)rsp_code, buf);
	}
}
...
struct net_buf *bt_opp_client_create_pdu(struct bt_opp_client *client, struct net_buf_pool *pool)
{
	if (client == NULL) {
		return NULL;
	}
...
{
	struct bt_opp_server *server = CONTAINER_OF(goep, struct bt_opp_server, _goep);

	atomic_set(&server->_transport_state, BT_OPP_TRANSPORT_STATE_CONNECTED);

	LOG_DBG("server %p RFCOMM connected", server);

	if (server->cb != NULL && server->cb->rfcomm_connected != NULL) {
...
static void opp_server_transport_disconnected(struct bt_goep *goep)
{
	...
	if (server->cb != NULL && server->cb->rfcomm_disconnected != NULL) {
...
{
	struct bt_opp_server *server = CONTAINER_OF(obex_server, struct bt_opp_server, _server);
	...
	if (server->cb != NULL && server->cb->connect != NULL) {
...
static void opp_server_obex_disconnect_cb(struct bt_obex_server *obex_server, struct net_buf *buf)
{
	...
	if (server->cb != NULL && server->cb->disconnect != NULL) {
				   ...
				   struct net_buf *buf)
{
	...
	if (server->cb != NULL && server->cb->push != NULL) {
				   ...
				   struct net_buf *buf)
{
	...
	if (server->cb != NULL && server->cb->pull_bcard != NULL) {
...
static void opp_server_obex_abort_cb(struct bt_obex_server *obex_server, struct net_buf *buf)
{
	...
	if (server->cb != NULL && server->cb->abort != NULL) {
...
struct net_buf *bt_opp_server_create_pdu(struct bt_opp_server *server, struct net_buf_pool *pool)
{
	if (server == NULL) {
		return NULL;
	}
