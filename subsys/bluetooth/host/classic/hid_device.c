...
static void hid_send_handshake(struct bt_hid_device *hid, uint8_t result_code)
{
	...
	/* HANDSHAKE is a header-only PDU whose header is written by the stack,
	 * so no HID header headroom is reserved here: hid_hs_pool is sized for
	 * exactly one header octet on top of the transport headers, and
	 * reserving the header as headroom too would leave no tailroom.
	 */
