					      ...
					      uint8_t rsp_code, struct net_buf *buf)
{
	...
	if (rsp_code == BT_AVRCP_RSP_NOT_IMPLEMENTED) {
		avrcp_ct_cb->notification_rsp(get_avrcp_ct(avrcp), tid,
					      BT_AVRCP_STATUS_NOT_IMPLEMENTED, 0, NULL);
		return;
	}
...
int bt_avrcp_init(void)
{
	...
	ARRAY_FOR_EACH(avrcp_connection, i) {
		...
		memset(bt_avrcp_tg_pool[i].interim_sent, 0,
		       sizeof(bt_avrcp_tg_pool[i].interim_sent));
	}
