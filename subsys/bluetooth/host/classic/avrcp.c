					       ...
					       uint8_t tid, struct net_buf *buf)
{
	...
	if (abort_continuing) {
		...
		atomic_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_SENDING_CONTINUING);
		...
		/* SENDING_CONTINUING: set sending flag, clear abort flag */
		atomic_set_bit(tx->flags, BT_AVRCP_TG_FLAG_SENDING_CONTINUING);
...
static void bt_avrcp_tg_vendor_tx_work(struct k_work *work)
{
	...
	if (atomic_test_bit(tx->flags, BT_AVRCP_TG_FLAG_ABORT_CONTINUING) ||
	    (tx->rsp == BT_AVRCP_RSP_REJECTED)) {
		...
		atomic_test_and_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_ABORT_CONTINUING);
		goto done;
	}
	...
	} else if (atomic_test_bit(tx->flags, BT_AVRCP_TG_FLAG_SENDING_CONTINUING)) {
		/* Last fragment */
	...
	atomic_test_and_clear_bit(tx->flags, BT_AVRCP_TG_FLAG_SENDING_CONTINUING);
					      ...
					      uint8_t rsp_code, struct net_buf *buf)
{
	...
	if ((ct->ct_notify[event_id].interim_received == 1) && (rsp_code == BT_AVRCP_RSP_CHANGED)) {
		...
		if (ct->ct_notify[event_id].cb != NULL) {
			...
			cb(event_id, (struct bt_avrcp_event_data *)event_data);
		}
	}
	return;
handle_error:
	for (int i = 0; i <= BT_AVRCP_EVT_VOLUME_CHANGED; i++) {
				...
				struct net_buf *buf)
{
	...
	if ((avrcp_tg_cb == NULL) || (avrcp_tg_cb->get_caps_req == NULL)) {
...
err_rsp:
	...
	if (err < 0) {
		LOG_ERR("Failed to send get_caps error response");
	}
}
						    ...
						    uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	if (cmd->event_id > BT_AVRCP_EVT_VOLUME_CHANGED) {
		LOG_ERR("Invalid event_id");
		return;
	}

	get_avrcp_tg(avrcp)->tg_notify[cmd->event_id].registered = true;
	avrcp_tg_cb->register_notification_req(get_avrcp_tg(avrcp), tid, cmd->event_id,
					       cmd->interval);
	return;
						      ...
						      uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->list_player_app_setting_attrs_req(get_avrcp_tg(avrcp), tid);
	return;
						     ...
						     uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->list_player_app_setting_vals_req(get_avrcp_tg(avrcp), tid, attr_id);
	return;
							...
							uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->get_curr_player_app_setting_val_req(get_avrcp_tg(avrcp), tid, buf);
	return;
						   ...
						   uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->set_player_app_setting_val_req(get_avrcp_tg(avrcp), tid, buf);
	return;
							 ...
							 uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->get_player_app_setting_attr_text_req(get_avrcp_tg(avrcp), tid, buf);
	return;
							...
							uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->get_player_app_setting_val_text_req(get_avrcp_tg(avrcp), tid, buf);
	return;
						    ...
						    uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->inform_displayable_char_set_req(get_avrcp_tg(avrcp), tid, buf);
	return;
						 ...
						 uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->inform_batt_status_of_ct_req(get_avrcp_tg(avrcp), tid, battery_status);
	return;
					    ...
					    uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->set_absolute_volume_req(get_avrcp_tg(avrcp), tid, absolute_volume);
	return;
					  ...
					  uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->get_element_attrs_req(get_avrcp_tg(avrcp), tid, buf);
	return;
					...
					uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->get_play_status_req(get_avrcp_tg(avrcp), tid);
	return;
					    ...
					    uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->set_addressed_player_req(get_avrcp_tg(avrcp), tid, player_id);
	return;
				  ...
				  uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->play_item_req(get_avrcp_tg(avrcp), tid, buf);
	return;
					   ...
					   uint8_t ctype_or_rsp, struct net_buf *buf)
{
	...
	avrcp_tg_cb->add_to_now_playing_req(get_avrcp_tg(avrcp), tid, buf);
	return;
			       ...
			       uint8_t tid)
{
	...
	if (err == -EOPNOTSUPP) {
		bt_avrcp_tg_send_browsing_general_reject_rsp(get_avrcp_tg(avrcp), tid,
