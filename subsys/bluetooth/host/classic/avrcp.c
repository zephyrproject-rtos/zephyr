...
error_rsp:
	return bt_avrcp_tg_send_set_browsed_player_rsp(get_avrcp_tg(avrcp), tid, rsp_buf);
}
...
int bt_avrcp_ct_set_browsed_player(struct bt_avrcp_ct *ct, uint8_t tid, uint16_t player_id)
{
	...
	if (!ct->avrcp->browsing_session_connected) {
					    ...
					    struct net_buf *buf)
{
	...
	if (!tg->avrcp->browsing_session_connected) {
		LOG_ERR("Browsing session not connected");
		return -ENOTCONN;
	}
	...
	if (net_buf_headroom(buf) < sizeof(struct bt_avrcp_avc_brow_pdu)) {
		LOG_ERR("Not enough headroom in buffer");
		return -ENOMEM;
	}
