...
static struct bt_hfp_ag_call *_get_call_with_flag(struct bt_hfp_ag *ag, int bit, bool clear)
{
	struct bt_hfp_ag_call *call;
...
#define get_call_with_flag(ag, bit) _get_call_with_flag(ag, bit, false)
#define get_call_clear_flag(ag, bit) _get_call_with_flag(ag, bit, true)
...
void ag_sco_connected(struct bt_conn *conn, uint8_t err)
{
	ARG_UNUSED(conn);
	ARG_UNUSED(err);
}

void ag_sco_disconnected(struct bt_conn *conn, uint8_t reason)
{
	ARG_UNUSED(reason);
