...
static int cover_art_parse_headers(struct net_buf *buf)
{
	...
	if (err) {
		bt_shell_print("Fail to parse BIP Headers");
	}
...
void tg_l2cap_connected(struct bt_avrcp_tg *tg, struct bt_avrcp_cover_art_tg *cover_art_tg)
{
	ca_tg = cover_art_tg;
	bt_shell_print("Cover Art TG L2CAP Connected: %p", cover_art_tg);
}

void tg_l2cap_disconnected(struct bt_avrcp_cover_art_tg *tg)
{
	ca_tg = NULL;
	bt_shell_print("Cover Art TG L2CAP Disconnected: %p", tg);
}

void tg_connect(struct bt_avrcp_cover_art_tg *tg, uint8_t version, uint16_t mopl,
		struct net_buf *buf)
{
...
void tg_disconnect(struct bt_avrcp_cover_art_tg *tg, struct net_buf *buf)
{
	bt_shell_print("Cover Art TG %p disconn req", tg);
	cover_art_parse_headers(buf);
}

void tg_abort(struct bt_avrcp_cover_art_tg *tg, struct net_buf *buf)
{
	bt_shell_print("Cover Art TG %p abort req", tg);
	cover_art_parse_headers(buf);
}

void tg_get_image_properties(struct bt_avrcp_cover_art_tg *tg, bool final, struct net_buf *buf)
{
...
void tg_get_image(struct bt_avrcp_cover_art_tg *tg, bool final, struct net_buf *buf)
{
...
void tg_get_linked_thumbnail(struct bt_avrcp_cover_art_tg *tg, bool final, struct net_buf *buf)
{
...
static int cmd_tg_get_image_properties(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_obex_add_header_body_or_end_body(buf, ca_tg_mopl,
...
static int cmd_tg_get_image(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_obex_add_header_body_or_end_body(buf, ca_tg_mopl, sizeof(jpeg_1000001) - offset,
...
static int cmd_ct_l2cap_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	psm = shell_strtoul(argv[1], 0, &err);
	if (err != 0) {
...
#if defined(CONFIG_BT_AVRCP_TG_COVER_ART)
#define RSP_HELP "<rsp: continue, success, error> [rsp_code]"
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(connect, NULL, RSP_HELP, cmd_tg_connect, 2, 1),
	SHELL_CMD_ARG(disconnect, NULL, RSP_HELP, cmd_tg_disconnect, 2, 1),
	SHELL_CMD_ARG(abort, NULL, RSP_HELP, cmd_tg_abort, 2, 1),
	SHELL_CMD_ARG(get_image_properties, NULL, RSP_HELP, cmd_tg_get_image_properties, 2, 1),
	SHELL_CMD_ARG(get_image, NULL, RSP_HELP, cmd_tg_get_image, 2, 0),
	SHELL_CMD_ARG(get_linked_thumbnail, NULL, RSP_HELP, cmd_tg_get_linked_thumbnail, 2, 1),
