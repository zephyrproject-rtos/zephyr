...
static int cmd_send_set_browsed_player_rsp(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	if (argc >= 2) {
		...
		status = 0x04U; /* AVRCP_STATUS_SUCCESS */
	}
	...
	if (argc >= 6) {
		folder_depth = (uint8_t)strtoul(argv[5], NULL, 0);
	} else {
		folder_depth = 1U;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(send_browsed_player_rsp, NULL, "Send SetBrowsedPlayer response\n"
		      "Usage: send_browsed_player_rsp <status> <uid_counter> <num_items> "
		      "<charset_id> <folder_depth> <folder_name>", cmd_send_set_browsed_player_rsp,
		      1, 6),
	SHELL_SUBCMD_SET_END);
