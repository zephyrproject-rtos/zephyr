...
static void avrcp_notify_changed_cb_rsp(uint8_t event_id, struct bt_avrcp_event_data *data)
{
...
{
	bt_shell_print("Inform displayable char rsp: tid=0x%02x, status=%02x", tid, status);
}
...
static struct bt_avrcp_ct_cb app_avrcp_ct_cb = {
	...
	.browsing_general_reject_rsp = avrcp_browsing_general_reject_rsp,
	...
	.inform_displayable_char_set_rsp = avrcp_inform_displayable_char_rsp,
...
static struct bt_avrcp_tg_cb app_avrcp_tg_cb = {
	...
	.get_caps_req = avrcp_get_caps_req,
	...
	.inform_displayable_char_set_req = avrcp_inform_displayable_char_set_req,
						       ...
						       char *argv[])
{
	...
	err = bt_avrcp_tg_send_set_player_app_setting_val_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_inform_displayable_char_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_inform_displayable_char_set_rsp(default_tg, tg_tid, status);
	if (err < 0) {
...
static int cmd_tg_send_get_play_status_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_get_play_status_rsp(default_tg, tg_tid, BT_AVRCP_STATUS_SUCCESS,
						   buf);
	if (err < 0) {
...
static int cmd_tg_send_change_path_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_change_path_rsp(default_tg, tg_tid, status, 10);
	if (err < 0) {
...
static int cmd_tg_send_browsing_general_reject_rsp(const struct shell *sh, int argc, char *argv[])
{
	...
	err = bt_avrcp_tg_send_browsing_general_reject_rsp(default_tg, tg_tid, reason);
	if (err < 0) {
