...
static void avrcp_passthrough_rsp(struct bt_avrcp *avrcp, struct bt_avrcp_passthrough_rsp *rsp)
{
	if (rsp->response == BT_AVRCP_RSP_ACCEPTED) {
		bt_shell_print("AVRCP passthough command accepted, operation id = 0x%02x",
			       rsp->operation_id);
	} else {
		bt_shell_print(
			"AVRCP passthough command rejected, operation id = 0x%02x, response = %d",
			rsp->operation_id, rsp->response);
	}
}
...
static int cmd_passthrough(const struct shell *sh, bt_avrcp_operation_id_t operation_id,
			   const uint8_t *payload, uint8_t len)
{
	...
	if (default_avrcp != NULL) {
		bt_avrcp_passthrough(default_avrcp, operation_id, payload, len);
	} else {
		shell_error(sh, "AVRCP is not connected");
	}
...
{
	return cmd_passthrough(sh, BT_AVRCP_PANEL_PLAY, NULL, 0);
}

static int cmd_pause(const struct shell *sh, int32_t argc, char *argv[])
{
	return cmd_passthrough(sh, BT_AVRCP_PANEL_PAUSE, NULL, 0);
}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(get_subunit, NULL, "<address>", cmd_get_subunit_info, 2, 0),
	SHELL_CMD_ARG(play, NULL, "<address>", cmd_play, 2, 0),
	SHELL_CMD_ARG(pause, NULL, "<address>", cmd_pause, 2, 0), SHELL_SUBCMD_SET_END);
