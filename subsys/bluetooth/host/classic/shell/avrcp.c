...
static void avrcp_notification_rsp(uint8_t event_id, struct bt_avrcp_event_data *data)
{
	...
	case BT_AVRCP_EVT_TRACK_CHANGED:
		bt_shell_print(" TRACK_CHANGED: identifier=%016llx",
			      (unsigned long long)data->identifier);
		break;
...
static void avrcp_set_absolute_volume_rsp(struct bt_avrcp_ct *ct, uint8_t tid,
					  uint8_t absolute_volume)
{
	bt_shell_print("AVRCP set_absolute_volume_rsp: tid=0x%02x, absolute_volume=0x%02x",
		       tid, absolute_volume);
}
...
static void avrcp_passthrough_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	if (cmd->data_len > 0U) {
		if (buf->len < sizeof(struct bt_avrcp_passthrough_opvu_data)) {
			bt_shell_print("Invalid passthrough data: buf length = %u, expected_len >= %zu",
...
static int cmd_ct_list_app_attrs(const struct shell *sh, int argc, char *argv[])
{
	...
	int err = bt_avrcp_ct_list_player_app_setting_attrs(default_ct, get_next_tid());
	if (err < 0) {
