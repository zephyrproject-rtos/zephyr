...
static int cmd_hid_register(const struct shell *sh, int32_t argc, char *argv[])
{
	...
	if (err != 0) {
		bt_shell_error("register cb fail");
		return err;
	}

	err = bt_sdp_register_service(&hid_rec);
	if (err != 0) {
		bt_shell_error("register desc fail");
		return err;
	}

	hid_registered = 1;
	bt_shell_print("register success");
	return err;
}
...
static int cmd_hid_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (!default_conn) {
		bt_shell_error("bt not connected");
		return -ENOEXEC;
	}

	default_hid = bt_hid_device_connect(default_conn);
	if (!default_hid) {
		bt_shell_error("fail to connect hid device");
		return -ENOEXEC;
	}
