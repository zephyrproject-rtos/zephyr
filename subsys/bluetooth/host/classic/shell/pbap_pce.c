...
static void pbap_connected(struct bt_pbap_pce *pbap, uint16_t mopl)
{
	bt_shell_print("pbap connect success %d", mopl);
}
...
static int cmd_connect_rfcomm(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err =  bt_pbap_pce_rfcomm_connect(default_conn, channel, &pbap_pce_app->pbap_pce);
        if (err != 0) {
...
static int cmd_connect_l2cap(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 3) {
        ...
        err = bt_pbap_pce_l2cap_connect(default_conn, psm, &pbap_pce_app->pbap_pce);
        if (err != 0) {
