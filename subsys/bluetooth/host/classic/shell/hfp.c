...
static void hf_connected(struct bt_conn *conn, struct bt_hfp_hf *hf)
{
	hf_conn = conn;
	hfp_hf = hf;
	shell_print(ctx_shell, "HF connected");
}
...
{
	shell_print(ctx_shell, "Turn off ECNR: %d", err);
}
...
{
	shell_print(ctx_shell, "3way call %p waiting. number %s type %d", call, number, type);
}
...
{
	shell_print(ctx_shell, "Voice recognition %s", activate ? "activate" : "deactivate");
}

#if defined(CONFIG_BT_HFP_HF_ENH_VOICE_RECG)
void hf_vre_state(struct bt_hfp_hf *hf, uint8_t state)
{
	shell_print(ctx_shell, "Voice recognition engine state %d", state);
}
...
void hf_textual_representation(struct bt_hfp_hf *hf, char *id, uint8_t type, uint8_t operation,
			       char *text)
{
	shell_print(ctx_shell, "Text id %s, type %d, operation %d, string %s", id, type, operation,
		    text);
}
...
{
	if (number) {
		shell_print(ctx_shell, "Requested phone number %s", number);
	} else {
		shell_print(ctx_shell, "Failed to request phone number");
	}
}

void hf_subscriber_number(struct bt_hfp_hf *hf, const char *number, uint8_t type, uint8_t service)
{
	shell_print(ctx_shell, "Subscriber number %s, type %d, service %d", number, type, service);
}
...
static int ag_memory_dial(struct bt_hfp_ag *ag, const char *location, char **number)
{
	...
	shell_print(ctx_shell, "ag memory dial");
...
static int ag_number_call(struct bt_hfp_ag *ag, const char *number)
{
	...
	shell_print(ctx_shell, "ag number call");
...
{
	shell_print(ctx_shell, "AG Voice recognition %s", activate ? "activate" : "deactivate");
}

#if defined(CONFIG_BT_HFP_AG_ENH_VOICE_RECG)
void ag_ready_to_accept_audio(struct bt_hfp_ag *ag)
{
	shell_print(ctx_shell, "hf is ready to accept audio");
}
...
{
	shell_print(ctx_shell, "DTMF code is %c", code);
}
...
void ag_hf_indicator_value(struct bt_hfp_ag *ag, enum hfp_ag_hf_indicators indicator,
			   uint32_t value)
{
	shell_print(ctx_shell, "indicator %d value %d", indicator, value);
}
...
SHELL_CMD_ARG_REGISTER(hfp, &hfp_cmds, "Bluetooth HFP shell commands",
			   cmd_default, 1, 1);
