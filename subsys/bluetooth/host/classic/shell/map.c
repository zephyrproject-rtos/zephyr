...
static void mce_mas_select(void)
{
	...
	bt_shell_print("No connected MCE MAS instance available for conn %s", addr);
}
...
static void mce_mas_rfcomm_connected(struct bt_conn *conn, struct bt_map_mce_mas *mce_mas)
{
	...
	bt_shell_print("MCE MAS RFCOMM connected: %p, addr: %s, instance_id: %u", mce_mas, addr,
		       inst->instance_id);
...
static void mce_mas_l2cap_connected(struct bt_conn *conn, struct bt_map_mce_mas *mce_mas)
{
	...
	default_mce_mas = mce_mas;
	conn_addr_str(conn, addr, sizeof(addr));
	bt_shell_print("MCE MAS L2CAP connected: %p, addr: %s, instance_id: %u", mce_mas, addr,
		       inst->instance_id);
	bt_shell_print("Selected MCE MAS instance_id %u as default", inst->instance_id);
}
...
static int cmd_mce_mas_select(const struct shell *sh, size_t argc, char *argv[])
{
	...
	conn_addr_str(default_conn, addr, sizeof(addr));
	shell_error(sh, "No matching MCE MAS instance available for conn %s", addr);
	return -ENOEXEC;
}
...
static int cmd_mce_mns_send_event(const struct shell *sh, size_t argc, char *argv[])
{
	...
	buf = bt_map_mce_mns_create_pdu(&inst->mce_mns, &mas_tx_pool);
	if (buf == NULL) {
		shell_error(sh, "Fail to allocate tx buffer");
		return -ENOBUFS;
	}
...
static void mse_mas_select(void)
{
	...
	bt_shell_print("No connected MSE MAS instance available for conn %s", addr);
}
...
static void mse_mas_rfcomm_connected(struct bt_conn *conn, struct bt_map_mse_mas *mse_mas)
{
	...
	conn_addr_str(conn, addr, sizeof(addr));
	bt_shell_print("MSE MAS RFCOMM connected: %p, addr: %s, instance_id: %u", mse_mas, addr,
		       inst->instance_id);
...
static void mse_mas_l2cap_connected(struct bt_conn *conn, struct bt_map_mse_mas *mse_mas)
{
	...
	conn_addr_str(conn, addr, sizeof(addr));
	bt_shell_print("MSE MAS L2CAP connected: %p, addr: %s, instance_id: %u", mse_mas, addr,
		       inst->instance_id);
...
static int cmd_mse_mas_select(const struct shell *sh, size_t argc, char *argv[])
{
	...
	conn_addr_str(default_conn, addr, sizeof(addr));
	shell_error(sh, "No matching MSE MAS instance available for conn %s", addr);
	return -ENOEXEC;
}
