...
static uint8_t sdp_avrcp_ct_user(struct bt_conn *conn, struct bt_sdp_client_result *result,
				 const struct bt_sdp_discover_params *params)
{
	char addr[BT_ADDR_STR_LEN];
	uint16_t param, version;
	...
	if (result && result->resp_buf) {
		bt_shell_print("SDP AVRCP CT data@%p (len %u) hint %u from remote %s",
			       result->resp_buf, result->resp_buf->len, result->next_record_hint,
			       addr);

		err = bt_sdp_get_proto_param(result->resp_buf, BT_SDP_PROTO_L2CAP, &param);
		if (err < 0) {
			bt_shell_error("AVRCP CT PSM not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP CT L2CAP PSM param 0x%04x", param);

		err = bt_sdp_get_profile_version(result->resp_buf, BT_SDP_AV_REMOTE_SVCLASS,
						 &version);
		if (err < 0) {
			bt_shell_error("AVRCP CT version not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP CT version param 0x%04x", version);

		err = bt_sdp_get_features(result->resp_buf, &features);
		if (err < 0) {
			bt_shell_error("AVRCP CT Features not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP CT Supported Features param 0x%04x", features);
	} else {
		bt_shell_print("No SDP AVRCP CT data from remote %s", addr);
	}
done:
	return BT_SDP_DISCOVER_UUID_CONTINUE;
}

static uint8_t sdp_avrcp_tg_user(struct bt_conn *conn, struct bt_sdp_client_result *result,
				 const struct bt_sdp_discover_params *params)
{
	char addr[BT_ADDR_STR_LEN];
	uint16_t param, version;
	...
	if (result && result->resp_buf) {
		bt_shell_print("SDP AVRCP TG data@%p (len %u) hint %u from remote %s",
			       result->resp_buf, result->resp_buf->len, result->next_record_hint,
			       addr);

		err = bt_sdp_get_proto_param(result->resp_buf, BT_SDP_PROTO_L2CAP, &param);
		if (err < 0) {
			bt_shell_error("AVRCP TG PSM not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP TG L2CAP PSM param 0x%04x", param);

		err = bt_sdp_get_profile_version(result->resp_buf, BT_SDP_AV_REMOTE_SVCLASS,
						 &version);
		if (err < 0) {
			bt_shell_error("AVRCP TG version not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP TG version param 0x%04x", version);

		err = bt_sdp_get_features(result->resp_buf, &features);
		if (err < 0) {
			bt_shell_error("AVRCP TG Features not found, err %d", err);
			goto done;
		}
		bt_shell_print("AVRCP TG Supported Features param 0x%04x", features);
	} else {
		bt_shell_print("No SDP AVRCP TG data from remote %s", addr);
	}
done:
	return BT_SDP_DISCOVER_UUID_CONTINUE;
}
