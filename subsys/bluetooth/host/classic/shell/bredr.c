...
static int cmd_rfcomm_connect(const struct shell *sh, size_t argc, char *argv[])
{
	uint8_t channel;
	int err;
...
static int cmd_rfcomm_send(const struct shell *sh, size_t argc, char *argv[])
{
...
static int cmd_rfcomm_disconnect(const struct shell *sh, size_t argc, char *argv[])
{
	int err;
...
static int cmd_rfcomm_send_rpn(const struct shell *sh, size_t argc, char *argv[])
{
	int err;
...
SHELL_STATIC_SUBCMD_SET_CREATE(rfcomm_cmds,
	SHELL_CMD_ARG(register, NULL, HELP_NONE, cmd_rfcomm_register, 1, 0),
	SHELL_CMD_ARG(connect, NULL, "<channel>", cmd_rfcomm_connect, 2, 0),
	SHELL_CMD_ARG(disconnect, NULL, HELP_NONE, cmd_rfcomm_disconnect, 1, 0),
	SHELL_CMD_ARG(send, NULL, "<number of packets>", cmd_rfcomm_send, 2, 0),
	SHELL_CMD_ARG(rpn, NULL, "Send RPN command with default settings",
		      cmd_rfcomm_send_rpn, 1, 0),
	SHELL_SUBCMD_SET_END
);
#endif /* CONFIG_BT_RFCOMM */
