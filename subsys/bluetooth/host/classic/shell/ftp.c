...
struct ftp_client_instance {
	...
	char pwd[CONFIG_BT_OBEX_AUTH_PWD_LEN + 1];
	uint8_t auth_state;
	uint8_t rsp_code;
	uint8_t flags;
	bool goep_v2;
...
static void ftp_client_clear_auth_state(struct ftp_client_instance *inst)
{
	inst->auth_state = 0;
			       ...
			       uint16_t mopl, struct net_buf *buf)
{
	...
	if (rsp_code == BT_FTP_RSP_CODE_SUCCESS) {
		...
		if ((inst->auth_state & LOCAL_AUTH_ENABLED) != 0) {
			...
			err = bt_obex_verify_auth_response((uint8_t *)inst->pwd, strlen(inst->pwd),
							   inst->local_nonce, (uint8_t *)auth.data);
			if (err != 0) {
				bt_shell_error(
...
static int cmd_client_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 1) {
		pwd = argv[1];
		if (strlen(pwd) > CONFIG_BT_OBEX_AUTH_PWD_LEN) {
			shell_error(sh, "[password] too long (max %u)",
				    CONFIG_BT_OBEX_AUTH_PWD_LEN);
			return -EINVAL;
		}
	}
	...
	if (pwd != NULL) {
		...
		strncpy(inst->pwd, pwd, sizeof(inst->pwd) - 1);
		 ...
		 */
		err = bt_obex_generate_nonce((uint8_t *)inst->pwd, strlen(inst->pwd),
					     inst->local_nonce);
		if (err != 0) {
		...
		err = bt_obex_calculate_request_digest((uint8_t *)inst->pwd, strlen(inst->pwd),
						       inst->peer_nonce, digest);
		if (err != 0) {
...
static int cmd_client_push_file(const struct shell *sh, size_t argc, char *argv[])
{
	...
	uint16_t body_len = sizeof(FTP_FILE_BODY) - 1;
	uint16_t len = 0;
	bool final;
	...
	if (inst->tx_cnt > 0U && inst->tx_cnt < body_len) {
		goto continue_req;
	}
...
continue_req:
	err = bt_obex_add_header_body_or_end_body(buf, inst->mopl, body_len - inst->tx_cnt,
...
static int cmd_server_register(const struct shell *sh, size_t argc, char *argv[])
{
	int err;

	ARRAY_FOR_EACH(ftp_server_instances, i) {
		struct ftp_server_instance *inst = &ftp_server_instances[i];

		err = bt_ftp_server_register(&inst->server, &ftp_server_cb);
		if (err != 0) {
			shell_error(sh, "Failed to register FTP server [%zu] (err %d)", i, err);
			return -ENOEXEC;
		}
	}
	shell_print(sh, "FTP server(s) registered");
...
static void ftp_server_clear_auth_state(struct ftp_server_instance *inst)
{
	inst->auth_state = 0;
	memset(inst->pwd, 0, sizeof(inst->pwd));
	memset(inst->local_nonce, 0, sizeof(inst->local_nonce));
...
static int cmd_server_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (pwd != NULL && strlen(pwd) > CONFIG_BT_OBEX_AUTH_PWD_LEN) {
		shell_error(sh, "[password] too long (max %u)", CONFIG_BT_OBEX_AUTH_PWD_LEN);
		return -EINVAL;
	}
	...
	if (rsp_code == BT_OBEX_RSP_CODE_SUCCESS) {
		if ((inst->auth_state & PEER_AUTH_ENABLED) != 0U) {
			...
			err = bt_obex_calculate_request_digest(
				(uint8_t *)inst->pwd, strlen(inst->pwd), inst->peer_nonce, digest);
			if (err != 0) {
...
static int cmd_server_pull_folder_listing(const struct shell *sh, size_t argc, char *argv[])
{
	...
	uint16_t body_len = sizeof(FTP_FOLDER_LISTING) - 1;
	uint8_t rsp_code = BT_FTP_RSP_CODE_SUCCESS;
	uint16_t len = 0;
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	ftp_server_cmds,
	SHELL_CMD_ARG(register, NULL, HELP_NONE, cmd_server_register, 1, 0),
