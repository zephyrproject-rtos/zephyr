...
{
	bt_shell_print("BIP server %p get_caps req, final %s, data len %d", server,
...
{
	bt_shell_print("BIP server %p get_image_list req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static void bip_server_get_image_properties(struct bt_bip_server *server, bool final,
					    struct net_buf *buf)
{
	bt_shell_print("BIP server %p get_image_properties req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
{
	bt_shell_print("BIP server %p get_image req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static void bip_server_get_linked_thumbnail(struct bt_bip_server *server, bool final,
					    struct net_buf *buf)
{
	bt_shell_print("BIP server %p get_linked_thumbnail req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static void bip_server_get_linked_attachment(struct bt_bip_server *server, bool final,
					     struct net_buf *buf)
{
	bt_shell_print("BIP server %p get_linked_attachment req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static void bip_server_get_partial_image(struct bt_bip_server *server, bool final,
					 struct net_buf *buf)
{
	bt_shell_print("BIP server %p get_partial_image req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static void bip_server_get_monitoring_image(struct bt_bip_server *server, bool final,
					    struct net_buf *buf)
{
	bt_shell_print("BIP server %p get_monitoring_image req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
{
	bt_shell_print("BIP server %p get_status req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_put_image_final = false;
static void bip_server_put_image(struct bt_bip_server *server, bool final, struct net_buf *buf)
{
	server_put_image_final = final;
	bt_shell_print("BIP server %p put_image req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_put_linked_thumbnail_final = false;
					    ...
					    struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p put_linked_thumbnail req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_put_linked_attachment_final = false;
					     ...
					     struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p put_linked_attachment req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_remote_display_final = false;
static void bip_server_remote_display(struct bt_bip_server *server, bool final, struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p remote_display req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_delete_image_final = false;
static void bip_server_delete_image(struct bt_bip_server *server, bool final, struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p delete_image req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_start_print_final = false;
static void bip_server_start_print(struct bt_bip_server *server, bool final, struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p start_print req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}

static bool server_start_archive_final = false;
static void bip_server_start_archive(struct bt_bip_server *server, bool final, struct net_buf *buf)
{
	...
	bt_shell_print("BIP server %p start_archive req, final %s, data len %d", server,
		       final ? "true" : "false", buf->len);
	bip_parse_headers(buf);
}
...
static uint8_t client_get_caps_rsp_code = 0;
static void bip_client_get_caps(struct bt_bip_client *client, uint8_t rsp_code, struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_caps rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_image_list_rsp_code = 0;
				      ...
				      struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_image_list rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_image_properties_rsp_code = 0;
					    ...
					    struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_image_properties rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_image_rsp_code = 0;
static void bip_client_get_image(struct bt_bip_client *client, uint8_t rsp_code,
				 struct net_buf *buf)
{
	client_get_image_rsp_code = rsp_code;
	bt_shell_print("BIP client %p get_image rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_linked_thumbnail_rsp_code = 0;
					    ...
					    struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_linked_thumbnail rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_linked_attachment_rsp_code = 0;
					     ...
					     struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_linked_attachment rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_partial_image_rsp_code = 0;
					 ...
					 struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_partial_image rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_monitoring_image_rsp_code = 0;
					    ...
					    struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p get_monitoring_image rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_get_status_rsp_code = 0;
static void bip_client_get_status(struct bt_bip_client *client, uint8_t rsp_code,
				  struct net_buf *buf)
{
	client_get_status_rsp_code = rsp_code;
	bt_shell_print("BIP client %p get_status rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_put_image_rsp_code = 0;
static bool client_put_image_final = false;
				 ...
				 struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p put_image rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_put_linked_thumbnail_rsp_code = 0;
static bool client_put_linked_thumbnail_final = false;
					    ...
					    struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p put_linked_thumbnail rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_put_linked_attachment_rsp_code = 0;
static bool client_put_linked_attachment_final = false;
					     ...
					     struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p put_linked_attachment rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_remote_display_rsp_code = 0;
static bool client_remote_display_final = false;
				      ...
				      struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p remote_display rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_delete_image_rsp_code = 0;
static bool client_delete_image_final = false;
				    ...
				    struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p delete_image rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_start_print_rsp_code = 0;
static bool client_start_print_final = false;
				   ...
				   struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p start_print rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}

static uint8_t client_start_archive_rsp_code = 0;
static bool client_start_archive_final = false;
				     ...
				     struct net_buf *buf)
{
	...
	bt_shell_print("BIP client %p start_archive rsp, rsp_code %s, data len %d", client,
		       bt_obex_rsp_code_to_str(rsp_code), buf->len);
	bip_parse_headers(buf);
}
...
static int cmd_register_rfcomm(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (rfcomm_server.server.rfcomm.channel) {
...
static int cmd_register_l2cap(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (l2cap_server.server.l2cap.psm) {
...
static int cmd_bip_client_get_caps(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_obex_add_header_conn_id(bip_app.tx_buf, bip_app.client._conn_id);
	if (err != 0) {
...
static int cmd_bip_client_put_image(const struct shell *sh, size_t argc, char *argv[])
{
	...
	len = BT_OBEX_PDU_LEN(bip_app.client._client.tx.mopl);
	len = MIN(len - bip_app.tx_buf->len, net_buf_tailroom(bip_app.tx_buf));
	if (len > BT_OBEX_HDR_LEN_OF_HEADER_BODY) {
		len = BT_OBEX_DATA_LEN_OF_HEADER_BODY(len);
		len = MIN(sizeof(jpeg_1000001) - offset, len);
		if ((offset + len) >= sizeof(jpeg_1000001)) {
			err = bt_obex_add_header_end_body(bip_app.tx_buf, len,
							  jpeg_1000001 + offset);
			client_put_image_final = true;
		} else {
			err = bt_obex_add_header_body(bip_app.tx_buf, len, jpeg_1000001 + offset);
		}
...
static int cmd_bip_client_start_print(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (len > BT_OBEX_HDR_LEN_OF_HEADER_BODY) {
		len = BT_OBEX_DATA_LEN_OF_HEADER_BODY(len);
		len = MIN(sizeof(PRINT_CONTROL_OBJECT) - offset, len);
		if ((offset + len) >= sizeof(PRINT_CONTROL_OBJECT)) {
			err = bt_obex_add_header_end_body(bip_app.tx_buf, len,
							  print_control_obj + offset);
			client_start_print_final = true;
		} else {
			err = bt_obex_add_header_body(bip_app.tx_buf, len,
						      print_control_obj + offset);
		}
...
static int cmd_bip_server_get_caps(const struct shell *sh, size_t argc, char *argv[])
{
	...
	len = BT_OBEX_PDU_LEN(bip_app.server._server.tx.mopl);
	len = MIN(len - bip_app.tx_buf->len, net_buf_tailroom(bip_app.tx_buf));
	if (len > BT_OBEX_HDR_LEN_OF_HEADER_BODY) {
		len = BT_OBEX_DATA_LEN_OF_HEADER_BODY(len);
		len = MIN(sizeof(IMAGING_CAPS_BODY) - offset, len);
		if ((offset + len) >= sizeof(IMAGING_CAPS_BODY)) {
			err = bt_obex_add_header_end_body(bip_app.tx_buf, len, caps_body + offset);
		} else {
			err = bt_obex_add_header_body(bip_app.tx_buf, len, caps_body + offset);
		}
...
static int cmd_bip_server_get_image_list(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (len > BT_OBEX_HDR_LEN_OF_HEADER_BODY) {
		len = BT_OBEX_DATA_LEN_OF_HEADER_BODY(len);
		len = MIN(sizeof(IMAGE_LIST_BODY) - offset, len);
		if ((offset + len) >= sizeof(IMAGE_LIST_BODY)) {
			err = bt_obex_add_header_end_body(bip_app.tx_buf, len,
							  image_list_body + offset);
		} else {
			err = bt_obex_add_header_body(bip_app.tx_buf, len,
						      image_list_body + offset);
		}
...
static int cmd_bip_server_get_image_properties(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (len > BT_OBEX_HDR_LEN_OF_HEADER_BODY) {
		len = BT_OBEX_DATA_LEN_OF_HEADER_BODY(len);
		len = MIN(sizeof(IMAGE_PROPERTIES_BODY) - offset, len);
		if ((offset + len) >= sizeof(IMAGE_PROPERTIES_BODY)) {
			err = bt_obex_add_header_end_body(bip_app.tx_buf, len,
							  image_properties_body + offset);
		} else {
			err = bt_obex_add_header_body(bip_app.tx_buf, len,
						      image_properties_body + offset);
		}
				 ...
				 const struct bt_sdp_discover_params *params)
{
	...
	err = bt_sdp_get_goep_l2cap_psm(result->resp_buf, &l2cap_psm);
	if (err != 0) {
	...
	err = bt_sdp_get_functions(result->resp_buf, &functions);
	if (err != 0) {
