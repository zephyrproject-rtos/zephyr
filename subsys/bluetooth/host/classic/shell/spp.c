...
static int spp_parse_rfcomm_channel(const char *str, uint8_t *channel)
{
	...
	if (end == str || *end != '\0' || val < SPP_RFCOMM_CHANNEL_MIN ||
	    val > SPP_RFCOMM_CHANNEL_MAX) {
		return -EINVAL;
	}
...
static int bt_spp_server_register(struct bt_spp_server *server)
{
	...
	if (server == NULL || server->sdp_record == NULL) {
		bt_shell_error("SPP: server register failed (server=%p, sdp_record=%p)", server,
			       server ? server->sdp_record : NULL);
		return -EINVAL;
	}
...
static int cmd_spp_register_with_uuid(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (bt_spp_uuid_from_str(uuid, (struct bt_uuid *)&uuid128) < 0) {
		shell_error(sh, "SPP: invalid UUID string: '%s'", uuid);
		return -ENOEXEC;
	}

	if (uuid128.uuid.type != BT_UUID_TYPE_128) {
		shell_error(sh, "SPP: only 128-bit UUID is supported (got type=%u)",
			    uuid128.uuid.type);
		return -ENOEXEC;
	}
...
static int bt_spp_send(struct bt_spp *spp, struct net_buf *buf)
{
	...
	if (atomic_get(&spp->state) != SPP_STATE_CONNECTED) {
		bt_shell_error("SPP: send rejected (not connected, state=%ld)",
			       (long)atomic_get(&spp->state));
		return -ENOTCONN;
	}

	err = bt_rfcomm_dlc_send(&spp->rfcomm_dlc, buf);
	if (err < 0) {
		bt_shell_error("SPP: RFCOMM send failed (len=%u, err=%d)", (unsigned int)buf->len,
			       err);
		return err;
	}
...
static int cmd_spp_send(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (err < 0) {
		shell_error(sh, "SPP: send command failed (err=%d)", err);
		net_buf_unref(buf);
		return -ENOEXEC;
	}
...
static int bt_spp_disconnect(struct bt_spp *spp)
{
	...
	if (atomic_get(&spp->state) != SPP_STATE_CONNECTED) {
		bt_shell_error("SPP: disconnect rejected (not connected, state=%ld)",
			       (long)atomic_get(&spp->state));
		return -ENOTCONN;
	}
	...
	if (err < 0) {
		bt_shell_error("SPP: RFCOMM disconnect failed (err=%d)", err);
		return err;
	}
...
static int cmd_spp_disconnect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (err < 0) {
		shell_error(sh, "SPP: disconnect command failed (err=%d)", err);
		return -ENOEXEC;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(register_with_uuid, NULL,
		      "<bt-uuid128> e.g. 00001101-0000-1000-8000-00805F9B34FB",
