...
#define MAX_RFCOMM_COUNT 10

static uint8_t credit_limit;
static bool hold_credit;
static struct bt_rfcomm_server server[MAX_RFCOMM_COUNT];
static struct bt_rfcomm_chan chan[MAX_RFCOMM_COUNT];
...
LISTIFY(MAX_RFCOMM_COUNT, SDP_ATTRS_DEFINE, (;), rfcomm_attrs);
...
static struct bt_sdp_record rfcomm_rec[MAX_RFCOMM_COUNT] = {
	LISTIFY(MAX_RFCOMM_COUNT, SDP_REC_DEFINE, (,), rfcomm_attrs),
};

static uint8_t discovered_server_channel[MAX_RFCOMM_COUNT];
...
static int cmd_register(const struct shell *sh, size_t argc, char *argv[])
{
	...
	unsigned long end = MAX_RFCOMM_COUNT - 1;
	...
	if (argc > 1) {
		...
		if (start >= MAX_RFCOMM_COUNT) {
			shell_error(sh, "%s is out of index %u", argv[1], MAX_RFCOMM_COUNT);
			return -EINVAL;
		}
		end = start;
	}
...
static int cmd_send(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (argc > 2) {
		...
		if (len > CONFIG_BT_RFCOMM_L2CAP_MTU) {
			shell_error(sh, "%s > %u", argv[2], CONFIG_BT_RFCOMM_L2CAP_MTU);
			return -EINVAL;
		}
	}
...
SHELL_CMD_ARG_REGISTER(rfcomm, &rfcomm_cmds, "Bluetooth RFCOMM sh commands", cmd_rfcomm, 1, 1);
