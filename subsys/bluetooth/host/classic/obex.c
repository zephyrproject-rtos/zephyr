...
#define OBEX_ClIENT(node) CONTAINER_OF(node, struct bt_obex_client, _node);
...
static int obex_make_uuid(union bt_obex_uuid *uuid, const uint8_t *data, uint16_t len)
{
	switch (len) {
...
static int obex_server_connect(struct bt_obex_server *server, uint16_t len, struct net_buf *buf)
{
	...
	if (err == 0) {
		err = obex_make_uuid(&server->_target, target_data, target_len);
		if (err != 0) {
...
static int obex_server_recv(struct bt_obex *obex, struct net_buf *buf)
{
	...
	if (hdr->code == BT_OBEX_OPCODE_CONNECT) {
		...
		if (err == 0) {
			...
			err = obex_make_uuid(&u, target_data, target_len);
			if (err != 0) {
			       ...
			       struct net_buf *buf)
{
	...
	if (atomic_test_bit(&client->_flags, BT_OBEX_HAS_TARGET) &&
		...
		err = obex_make_uuid(&u, who_data, who_len);
		if (err != 0) {
...
int bt_obex_connect(struct bt_obex_client *client, uint16_t mopl, struct net_buf *buf)
{
	...
	if (atomic_test_bit(&client->_flags, BT_OBEX_HAS_TARGET)) {
		...
		err = obex_make_uuid(&client->_target, target_data, target_len);
		if (err != 0) {
			LOG_WRN("Unsupported target header len %u", target_len);
			return err;
		}
