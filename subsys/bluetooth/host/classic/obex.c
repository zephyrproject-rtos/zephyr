...
static struct bt_obex_client *obex_get_last_client(struct bt_obex *obex)
{
	...
	if (client == NULL) {
		LOG_WRN("Get the last client.");
		return NULL;
	}

	opcode = atomic_get(&client->_pre_opcode);

	if (opcode != BT_OBEX_OPCODE_ABORT) {
		LOG_WRN("Last opcode %x is not ABORT", opcode);
		return NULL;
	}

	/* Recover `client->_opcode` and clear `client->_pre_opcode`. */
	if (atomic_set(&client->_opcode, opcode) != 0) {
		LOG_WRN("opcode is not 0");
	}

	if (atomic_clear(&client->_pre_opcode) != opcode) {
		LOG_WRN("Last opcode has been changed");
	}
