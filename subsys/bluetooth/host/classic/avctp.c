...
static inline uint8_t get_avctp_packet_num(uint16_t total_bytes, uint16_t l2cap_mtu)
{
	...
	if (total_bytes <= l2cap_mtu - BT_AVCTP_HDR_SIZE_SINGLE) {
		return 1;
	}
...
static void avctp_tx_processor(struct k_work *item)
{
	...
	max_payload_size = MIN(session->br_chan.tx.mtu, session->tx_payload_size);
	...
	remaining = buf->len - user_data->sent_len;
	if (user_data->sent_len == 0U) {
		pkt_type = BT_AVCTP_PKT_TYPE_START;
		chunk_size = max_payload_size - BT_AVCTP_HDR_SIZE_START;
	} else {
		bool is_last = (remaining <= (max_payload_size - BT_AVCTP_HDR_SIZE_CONTINUE_END));

		pkt_type = is_last ? BT_AVCTP_PKT_TYPE_END : BT_AVCTP_PKT_TYPE_CONTINUE;
		chunk_size = MIN(remaining, max_payload_size - BT_AVCTP_HDR_SIZE_CONTINUE_END);
	}

	err = send_fragmented_avctp(session, user_data, pkt_type, buf->data + user_data->sent_len,
				    chunk_size);
	if (err < 0) {
	...
	if (user_data->sent_len == buf->len) {
		net_buf_pull_mem(buf, user_data->sent_len);
		avctp_tx_remove(buf);
		net_buf_unref(buf);
	}
}
			 ...
			 uint8_t tid)
{
	...
	return bt_avctp_send_common(session, buf, cr, tid, session->pid, 1, BT_AVCTP_IPID_NONE);
}
