...
#define IS_AVCTP_SRV_SUPPORTED() (event_cb != NULL)
#define IS_AVCTP_BROWSING_SRV_SUPPORTED() (browsing_event_cb != NULL)
...
static int avctp_l2cap_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	switch (sys_be16_to_cpu(hdr->pid)) {
#if defined(CONFIG_BT_AVRCP)
	case BT_SDP_AV_REMOTE_SVCLASS:
		break;
#endif
	default:
...
static int avctp_browsing_l2cap_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
...
#if defined(CONFIG_BT_AVRCP)
	if (sys_be16_to_cpu(hdr->pid) == BT_SDP_AV_REMOTE_SVCLASS) {
		return session->ops->recv(session, buf);
	}
#endif
	...
	return 0; /* No need to report to upper layer */
}
				       ...
				       struct bt_l2cap_chan **chan)
{
	...
	if (!IS_AVCTP_BROWSING_SRV_SUPPORTED()) {
					     ...
					     uint8_t tid, uint16_t pid)
{
	...
	if (session == NULL || session->br_chan.chan.conn == NULL) {
		LOG_ERR("Invalid AVCTP browsing session (NULL)");
		return NULL;
	}

	if (tid > 0x0F) {
		LOG_ERR("Invalid transaction label: 0x%02X", tid);
		return NULL;
	}
