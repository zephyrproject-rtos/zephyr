...
static struct k_mutex avctp_server_lock;
...
int bt_avctp_connect(struct bt_conn *conn, struct bt_avctp *session)
{
	...
	return bt_l2cap_chan_connect(conn, &session->br_chan.chan, session->psm);
}
			      ...
			      struct bt_l2cap_chan **chan)
{
	...
	struct bt_avctp_l2cap_server *l2cap_server;
	int err;

	LOG_DBG("conn %p", conn);

	l2cap_server = CONTAINER_OF(server, struct bt_avctp_l2cap_server, l2cap);

	k_mutex_lock(&avctp_server_lock, K_FOREVER);

	if (!sys_slist_find(&avctp_l2cap_server, &l2cap_server->node, NULL)) {
		LOG_WRN("Invalid l2cap server");
		k_mutex_unlock(&avctp_server_lock);
		return -EINVAL;
	}
	...
	err = l2cap_server->accept(conn, &session);
	if (err) {
...
int bt_avctp_init(void)
{
	...
	k_mutex_init(&avctp_server_lock);
	return 0;
}
