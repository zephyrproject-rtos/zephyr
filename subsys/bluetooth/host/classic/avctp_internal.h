...
/* AVCTP Browsing channel connect.The AVCTP control channel must be connected before calling
 * this function. The browsing channel is logically dependent on the control session.
 */
int bt_avctp_browsing_connect(struct bt_conn *conn, struct bt_avctp_browsing *session);

/* AVCTP Browsing channel disconnect. The browsing channel must be disconnected before disconnect
 * the AVCTP control channel.
 */
