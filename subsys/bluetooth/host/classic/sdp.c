...
{
	struct bt_sdp_record *rec = NULL, *next;
...
static uint16_t parse_service_search_pattern(struct net_buf *buf, struct net_buf_simple *ssp)
{
	...
	ssp->data = net_buf_pull_mem(buf, data_elem.data_size);
	ssp->len = data_elem.data_size;
	ssp->size = data_elem.data_size;
	ssp->__buf = ssp->data;
...
int bt_sdp_register_service(struct bt_sdp_record *service)
{
	...
	if (!sys_slist_is_empty(&sdp_db)) {
		...
		head = CONTAINER_OF(sys_slist_peek_tail(&sdp_db), struct bt_sdp_record, node);
		index = head->index + 1;
