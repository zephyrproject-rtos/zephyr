				 ...
				 const struct bt_sdp_discover_params *param)
{
	...
	return bt_sdp_send(&session->chan.chan, buf, BT_SDP_SVC_SEARCH_REQ,
			   session->tid);
}
				 ...
				 const struct bt_sdp_discover_params *param)
{
	...
	return bt_sdp_send(&session->chan.chan, buf, BT_SDP_SVC_ATTR_REQ,
			   session->tid);
}
...
static int sdp_client_receive_ss(struct bt_sdp_client *session, struct net_buf *buf)
{
	...
	if ((record_len + SDP_CONT_STATE_LEN_SIZE + cstate->length) >
		buf->len) {
	 ...
	 */
	if (!current_count && cstate->length == 0U &&
		session->cstate.length == 0U) {
		LOG_DBG("record for service record handle 0x%x not found",
			session->param->handle);
