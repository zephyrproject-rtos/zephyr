...
static uint16_t copy_attribute_from_buf(struct net_buf *buf, struct net_buf_simple *src,
					uint32_t *offset, uint32_t *index, uint16_t *space)
{
	...
	*offset = end;
	...
	/* Range start ~ end */
	if (*index < start) {
		len = MIN(src->len, *space);
		net_buf_add_mem(buf, src->data, len);
		*space -= len;
		*index += len;
		return len;
	}
	...
	if (*index < end) {
		len = end - *index;
		len = MIN(len, *space);
		net_buf_add_mem(buf, &src->data[*index - start], len);
		*space -= len;
		*index += len;
		return len;
	}
...
static uint16_t copy_attribute_from_elem(struct net_buf *buf, const struct bt_sdp_data_elem *elem,
					 uint32_t *offset, uint32_t *index, uint16_t *space)
{
	uint32_t end;
	uint32_t start = *offset;
	...
	end = start + elem->data_size;
	*offset = end;

	/* Range start ~ end */
	if (*index < start) {
		len = MIN(elem->data_size, *space);
		net_buf_add_mem(buf, elem->data, len);
		*space -= len;
		*index += len;
		return len;
	}
	...
	if (*index < end) {
		len = end - *index;
		len = MIN(len, *space);
		net_buf_add_mem(buf, &((const uint8_t *)elem->data)[*index - start], len);
		*space -= len;
		*index += len;
		return len;
	}
			       ...
			       uint8_t nest_level)
{
	...
	if ((elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_SEQ_UNSPEC ||
	    (elem->type & BT_SDP_TYPE_DESC_MASK) == BT_SDP_ALT_UNSPEC) {
		copy_len = copy_attribute_from_buf(buf, &attr_buf, offset, index, space);
		total_len += copy_len;
		if (*space == 0) {
			goto exit;
		}
		...
		copy_len = copy_attribute_from_elem(buf, elem, offset, index, space);
		total_len += copy_len;
		if (*space == 0) {
			goto exit;
		}
