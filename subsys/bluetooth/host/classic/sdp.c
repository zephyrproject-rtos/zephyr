...
static void sdp_client_disconnected(struct bt_l2cap_chan *chan)
{
	...
	(void)memset(&session->reqs, 0,
		     sizeof(*session) - offsetof(struct bt_sdp_client, reqs));
}
...
void sdp_client_released(struct bt_l2cap_chan *chan)
{
	...
	if (!sys_slist_is_empty(&session->reqs_next)) {
		...
		conn = bt_conn_lookup_index(session - &bt_sdp_client_pool[0]);
		...
		if (err) {
			...
			(void)memset(&session->chan, 0,
				     sizeof(*session) - offsetof(struct bt_sdp_client, chan));
		}
		k_sem_give(&session->sem_lock);
...
static int sdp_client_get_session_for_req(struct bt_conn *conn,
					  const struct bt_sdp_discover_params *params)
{
	...
	if (session->state == SDP_CLIENT_CONNECTING ||
		...
		sys_slist_append(&session->reqs,
				 (sys_snode_t *)&params->_node);
		k_sem_give(&session->sem_lock);
		return 0;
	}
	...
	if (session->state == SDP_CLIENT_DISCONNECTING) {
		sys_slist_append(&session->reqs_next,
			(sys_snode_t *)&params->_node);
		k_sem_give(&session->sem_lock);
		return 0;
	}
		    ...
		    const struct bt_sdp_discover_params *params)
{
	...
	return sdp_client_get_session_for_req(conn, params);
}
