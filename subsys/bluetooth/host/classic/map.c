...
#define SET_NTF_REG_REQUIRED_AP BT_MAP_APPL_PARAM_TAG_ID_NTF_STATUS
...
#define SET_MSG_STATUS_REQUIRED_AP BT_MAP_APPL_PARAM_TAG_ID_STATUS_IND
...
#define GET_MAS_INST_INFO_REQUIRED_HDR                                                             \
	BT_OBEX_HEADER_ID_CONN_ID, BT_OBEX_HEADER_ID_TYPE, BT_OBEX_HEADER_ID_APP_PARAM
#define GET_MAS_INST_INFO_REQUIRED_AP BT_MAP_APPL_PARAM_TAG_ID_MAS_INST_ID
...
#define SET_NTF_FILTER_REQUIRED_HDR                                                                \
	BT_OBEX_HEADER_ID_CONN_ID, BT_OBEX_HEADER_ID_TYPE, BT_OBEX_HEADER_ID_APP_PARAM
#define SET_NTF_FILTER_REQUIRED_AP BT_MAP_APPL_PARAM_TAG_ID_NTF_FILTER_MASK
...
/* Generates a paired MCE+MSE MAS callback getter function for field _name */
#define MAP_DEFINE_MAS_CB_GETTERS(_name)                                                           \
	static mce_mas_cb_t mce_mas_get_cb_##_name(struct bt_map_mce_mas *mce_mas)                 \
	{                                                                                          \
		return mce_mas->_cb->_name;                                                        \
	}                                                                                          \
	static mse_mas_cb_t mse_mas_get_cb_##_name(struct bt_map_mse_mas *mse_mas)                 \
	{                                                                                          \
		return mse_mas->_cb->_name;                                                        \
	}

/* Generates a paired MCE+MSE MNS callback getter function for field _name */
#define MAP_DEFINE_MNS_CB_GETTERS(_name)                                                           \
	static mce_mns_cb_t mce_mns_get_cb_##_name(struct bt_map_mce_mns *mce_mns)                 \
	{                                                                                          \
		return mce_mns->_cb->_name;                                                        \
	}                                                                                          \
	static mse_mns_cb_t mse_mns_get_cb_##_name(struct bt_map_mse_mns *mse_mns)                 \
	{                                                                                          \
		return mse_mns->_cb->_name;                                                        \
	}

/* Declares one entry in map_mas_functions[].
 * _op_get : bool – true for GET operations, false for PUT operations
 * _NAME   : uppercase operation token (e.g. SET_NTF_REG)
 * _name   : lowercase field name matching the cb struct member (e.g. set_ntf_reg)
 */
#define MAP_MAS_FUNCTION(_op_get, _NAME, _name)                                                    \
	{                                                                                          \
		.type = BT_MAP_HDR_TYPE_##_NAME, .op_get = (_op_get),                              \
		.ap = MAP_REQUIRED_APP_PARAM_LIST(_NAME##_REQUIRED_AP),                            \
		.hdr = MAP_REQUIRED_HDR_LIST(_NAME##_REQUIRED_HDR),                                \
		.get_mce_mas_cb = mce_mas_get_cb_##_name,                                          \
		.get_mse_mas_cb = mse_mas_get_cb_##_name,                                          \
	}

/* Declares one entry in map_mns_functions[].
 * _op_get : bool – true for GET operations, false for PUT operations
 * _NAME   : uppercase operation token (e.g. SEND_EVENT)
 * _name   : lowercase field name matching the cb struct member (e.g. send_event)
 */
#define MAP_MNS_FUNCTION(_op_get, _NAME, _name)                                                    \
	{                                                                                          \
		.type = BT_MAP_HDR_TYPE_##_NAME, .op_get = (_op_get),                              \
		.ap = MAP_REQUIRED_APP_PARAM_LIST(_NAME##_REQUIRED_AP),                            \
		.hdr = MAP_REQUIRED_HDR_LIST(_NAME##_REQUIRED_HDR),                                \
		.get_mce_mns_cb = mce_mns_get_cb_##_name,                                          \
		.get_mse_mns_cb = mse_mns_get_cb_##_name,                                          \
	}

MAP_DEFINE_MNS_CB_GETTERS(send_event)
MAP_DEFINE_MAS_CB_GETTERS(set_ntf_reg)
MAP_DEFINE_MAS_CB_GETTERS(get_folder_listing)
MAP_DEFINE_MAS_CB_GETTERS(get_msg_listing)
MAP_DEFINE_MAS_CB_GETTERS(get_msg)
MAP_DEFINE_MAS_CB_GETTERS(set_msg_status)
MAP_DEFINE_MAS_CB_GETTERS(push_msg)
MAP_DEFINE_MAS_CB_GETTERS(update_inbox)
MAP_DEFINE_MAS_CB_GETTERS(get_mas_inst_info)
MAP_DEFINE_MAS_CB_GETTERS(set_owner_status)
MAP_DEFINE_MAS_CB_GETTERS(get_owner_status)
MAP_DEFINE_MAS_CB_GETTERS(get_convo_listing)
MAP_DEFINE_MAS_CB_GETTERS(set_ntf_filter)

static const struct map_mas_function map_mas_functions[] = {
	MAP_MAS_FUNCTION(false, SET_NTF_REG, set_ntf_reg),
	MAP_MAS_FUNCTION(true, GET_FOLDER_LISTING, get_folder_listing),
	MAP_MAS_FUNCTION(true, GET_MSG_LISTING, get_msg_listing),
	MAP_MAS_FUNCTION(true, GET_MSG, get_msg),
	MAP_MAS_FUNCTION(false, SET_MSG_STATUS, set_msg_status),
	MAP_MAS_FUNCTION(false, PUSH_MSG, push_msg),
	MAP_MAS_FUNCTION(false, UPDATE_INBOX, update_inbox),
	MAP_MAS_FUNCTION(true, GET_MAS_INST_INFO, get_mas_inst_info),
	MAP_MAS_FUNCTION(false, SET_OWNER_STATUS, set_owner_status),
	MAP_MAS_FUNCTION(true, GET_OWNER_STATUS, get_owner_status),
	MAP_MAS_FUNCTION(true, GET_CONVO_LISTING, get_convo_listing),
	MAP_MAS_FUNCTION(false, SET_NTF_FILTER, set_ntf_filter),
};

static const struct map_mns_function map_mns_functions[] = {
	MAP_MNS_FUNCTION(false, SEND_EVENT, send_event),
};
...
static void mce_mas_connect(struct bt_obex_client *client, uint8_t rsp_code, uint8_t version,
			    uint16_t mopl, struct net_buf *buf)
{
	...
	if ((c->_cb != NULL) && (c->_cb->connect != NULL)) {
		c->_cb->connect(c, rsp_code, version, mopl, buf);
	}
}

static void mce_mas_disconnect(struct bt_obex_client *client, uint8_t rsp_code, struct net_buf *buf)
{
	struct bt_map_mce_mas *c = CONTAINER_OF(client, struct bt_map_mce_mas, _client);
	...
	if ((c->_cb != NULL) && (c->_cb->disconnect != NULL)) {
...
static void mce_mas_setpath(struct bt_obex_client *client, uint8_t rsp_code, struct net_buf *buf)
{
	...
	if ((c->_cb != NULL) && (c->_cb->set_folder != NULL)) {
...
/* Defines a public bt_map_mce_mas_xxx() API wrapper.
 * _op_get : bool – true for GET, false for PUT
 * _NAME   : uppercase operation token (e.g. SET_NTF_REG)
 * _name   : lowercase function name suffix  (e.g. set_ntf_reg)
 */
#define MAP_MCE_MAS_API(_op_get, _NAME, _name)                                                     \
	int bt_map_mce_mas_##_name(struct bt_map_mce_mas *mce_mas, bool final,                     \
				   struct net_buf *buf)                                            \
	{                                                                                          \
		return mce_mas_get_or_put(mce_mas, (_op_get), BT_MAP_HDR_TYPE_##_NAME, final,      \
					  buf);                                                    \
	}

MAP_MCE_MAS_API(false, SET_NTF_REG, set_ntf_reg)
MAP_MCE_MAS_API(true, GET_FOLDER_LISTING, get_folder_listing)
MAP_MCE_MAS_API(true, GET_MSG_LISTING, get_msg_listing)
MAP_MCE_MAS_API(true, GET_MSG, get_msg)
MAP_MCE_MAS_API(false, SET_MSG_STATUS, set_msg_status)
MAP_MCE_MAS_API(false, PUSH_MSG, push_msg)
MAP_MCE_MAS_API(false, UPDATE_INBOX, update_inbox)
MAP_MCE_MAS_API(true, GET_MAS_INST_INFO, get_mas_inst_info)
MAP_MCE_MAS_API(false, SET_OWNER_STATUS, set_owner_status)
MAP_MCE_MAS_API(true, GET_OWNER_STATUS, get_owner_status)
MAP_MCE_MAS_API(true, GET_CONVO_LISTING, get_convo_listing)
...
static void mce_mns_connect(struct bt_obex_server *server, uint8_t version, uint16_t mopl,
			    struct net_buf *buf)
{
	...
	if ((s->_cb != NULL) && (s->_cb->connect != NULL)) {
		s->_cb->connect(s, version, mopl, buf);
	}
}

static void mce_mns_disconnect(struct bt_obex_server *server, struct net_buf *buf)
{
	struct bt_map_mce_mns *s = CONTAINER_OF(server, struct bt_map_mce_mns, _server);
	...
	if ((s->_cb != NULL) && (s->_cb->disconnect != NULL)) {
...
static void mce_mns_abort(struct bt_obex_server *server, struct net_buf *buf)
{
	...
	if ((s->_cb != NULL) && (s->_cb->abort != NULL)) {
		s->_cb->abort(s, buf);
		return;
	}
...
static int mce_mns_rfcomm_accept(struct bt_conn *conn,
				 ...
				 struct bt_goep **goep)
{
	return mce_mns_accept(conn, server, MAP_TRANSPORT_TYPE_RFCOMM, goep);
}
...
static int mce_mns_l2cap_accept(struct bt_conn *conn, struct bt_goep_transport_l2cap_server *server,
				struct bt_goep **goep)
{
	return mce_mns_accept(conn, server, MAP_TRANSPORT_TYPE_L2CAP, goep);
}
...
static void mse_mas_connect(struct bt_obex_server *server, uint8_t version, uint16_t mopl,
			    struct net_buf *buf)
{
...
static void mse_mas_disconnect(struct bt_obex_server *server, struct net_buf *buf)
{
	struct bt_map_mse_mas *s = CONTAINER_OF(server, struct bt_map_mse_mas, _server);
...
static int mse_mas_rfcomm_accept(struct bt_conn *conn,
				 ...
				 struct bt_goep **goep)
{
	return mse_mas_accept(conn, server, MAP_TRANSPORT_TYPE_RFCOMM, goep);
}
...
static int mse_mas_l2cap_accept(struct bt_conn *conn, struct bt_goep_transport_l2cap_server *server,
				struct bt_goep **goep)
{
	return mse_mas_accept(conn, server, MAP_TRANSPORT_TYPE_L2CAP, goep);
}
...
/* Defines a public bt_map_mse_mas_xxx() API wrapper.
 * _op_get   : bool – true for GET, false for PUT
 * _NAME     : uppercase operation token (e.g. SET_NTF_REG)
 * _name     : lowercase function name suffix  (e.g. set_ntf_reg)
 */
#define MAP_MSE_MAS_API(_op_get, _NAME, _name)                                                     \
	int bt_map_mse_mas_##_name(struct bt_map_mse_mas *mse_mas, uint8_t rsp_code,               \
				   struct net_buf *buf)                                            \
	{                                                                                          \
		return mse_mas_get_or_put_rsp(mse_mas, (_op_get), BT_MAP_HDR_TYPE_##_NAME,         \
					      rsp_code, buf);                                      \
	}

MAP_MSE_MAS_API(false, SET_NTF_REG, set_ntf_reg)
MAP_MSE_MAS_API(true, GET_FOLDER_LISTING, get_folder_listing)
MAP_MSE_MAS_API(true, GET_MSG_LISTING, get_msg_listing)
MAP_MSE_MAS_API(true, GET_MSG, get_msg)
MAP_MSE_MAS_API(false, SET_MSG_STATUS, set_msg_status)
MAP_MSE_MAS_API(false, PUSH_MSG, push_msg)
MAP_MSE_MAS_API(false, UPDATE_INBOX, update_inbox)
MAP_MSE_MAS_API(true, GET_MAS_INST_INFO, get_mas_inst_info)
MAP_MSE_MAS_API(false, SET_OWNER_STATUS, set_owner_status)
MAP_MSE_MAS_API(true, GET_OWNER_STATUS, get_owner_status)
MAP_MSE_MAS_API(true, GET_CONVO_LISTING, get_convo_listing)
...
static void mse_mns_connect(struct bt_obex_client *client, uint8_t rsp_code, uint8_t version,
			    uint16_t mopl, struct net_buf *buf)
{
...
static void mse_mns_disconnect(struct bt_obex_client *client, uint8_t rsp_code, struct net_buf *buf)
{
	struct bt_map_mse_mns *c = CONTAINER_OF(client, struct bt_map_mse_mns, _client);
