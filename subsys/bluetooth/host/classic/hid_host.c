...
{
	/* Copy the contents before clearing the claim. hid_req_take() only runs
	 * on the RX thread when a reply arrives or from the transaction timeout,
	 * both of which happen after hid_req_claim() finished filling hid->req in,
	 * so the copy can never observe a half-written transaction. This ordering
	 * replaces the three-state token the claim used to publish.
	 */
	*req = hid->req;
...
static void hid_cleanup(struct bt_hid_host *hid)
{
	 ...
	 */
	/* The two connection flags are written only on the RX callback chain, so
	 * reading them together here is not an atomic snapshot but is still
	 * correct: nothing else can set them while this runs.
	 */
	__ASSERT(!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_CONNECTED) &&
			 !atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_CONNECTED),
...
static int hid_control_handle(struct bt_hid_host *hid, struct net_buf *buf, uint8_t control)
{
	 ...
	 * is deferred to the system workqueue so the channels are not disconnected
	 * from inside the receive callback. Destroying the bonding is left to the
	 * application, see the vc_unplug callback.
	 */
...
static int hid_session_init(struct bt_hid_host *hid, enum bt_hid_role role)
{
	/* The work items are re-initialized below, so refuse to reset the instance
	 * while any of them is still pending or running from a prior connection.
	 */
	if ((k_work_delayable_busy_get(&hid->intr_work) != 0) ||
	    (k_work_delayable_busy_get(&hid->trans_work) != 0) ||
	    (k_work_delayable_busy_get(&hid->vcu_work) != 0)) {
		LOG_WRN("HID Host %p work still pending or running", hid);
		return -EBUSY;
	}
				      ...
				      struct bt_l2cap_chan **chan)
{
	...
	err = hid_session_init(hid, BT_HID_ROLE_ACCEPTOR);
	if (err != 0) {
		return err;
	}
...
int bt_hid_host_connect(struct bt_conn *conn, struct bt_hid_host **hid)
{
	...
	err = hid_session_init(inst, BT_HID_ROLE_INITIATOR);
	if (err != 0) {
		return err;
	}
