...
static const char *hid_ctrl_type_str(uint8_t type)
{
	switch (type) {
	case BT_HID_CTRL_TYPE_NONE:
		return "none";
	case BT_HID_CTRL_TYPE_GET_REPORT:
		return "get-report";
	case BT_HID_CTRL_TYPE_SET_REPORT:
		return "set-report";
	case BT_HID_CTRL_TYPE_GET_PROTOCOL:
		return "get-protocol";
	case BT_HID_CTRL_TYPE_SET_PROTOCOL:
		return "set-protocol";
	default:
		return "unknown";
	}
...
{
	if (atomic_test_and_set_bit(&hid->flags, BT_HID_HOST_FLAG_REQ_PENDING)) {
		LOG_ERR("transaction %s pending", hid_ctrl_type_str(hid->req.type));
		return -EBUSY;
	}

	/* The claim is published before the contents and the timer are in place,
	 * but neither the RX thread nor the timeout can consume it until the PDU
	 * this claim guards has been sent, which happens after this returns. See
	 * the ordering note in hid_req_take().
	 */
	hid->req = *req;

	/* Rescheduled rather than scheduled: k_work_schedule() keeps the delay of an
	 * item that is already pending, so a leftover timeout would decide the
	 * deadline of this transaction.
	 */
	bt_work_reschedule(&hid->trans_work, HID_TRANS_TIMEOUT);

	/* hid_cleanup() may have released the association since the claim above, in
	 * which case the timeout just armed is left on a released instance. See the
	 * ordering note in hid_cleanup().
	 */
	if (!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_REQ_PENDING)) {
		k_work_cancel_delayable(&hid->trans_work);
		LOG_WRN("association released while claiming transaction %s",
			hid_ctrl_type_str(req->type));
		return -ENOTCONN;
	}

	LOG_DBG("transaction %s started", hid_ctrl_type_str(req->type));
...
{
	if (atomic_test_and_clear_bit(&hid->flags, BT_HID_HOST_FLAG_REQ_PENDING)) {
		k_work_cancel_delayable(&hid->trans_work);
	}
}
...
{
	/* Copy the contents before clearing the claim. hid_req_take() only runs on
	 * the RX thread, when a reply or a VIRTUAL_CABLE_UNPLUG arrives, or from the
	 * transaction timeout, all of which happen after hid_req_claim() finished
	 * filling hid->req in, so the copy can never observe a half-written
	 * transaction.
	 */
	*req = hid->req;
	...
	LOG_DBG("transaction %s done", hid_ctrl_type_str(req->type));
	k_work_cancel_delayable(&hid->trans_work);
...
static void hid_close_handler(struct k_work *work)
{
	...
	struct bt_hid_host *hid = CONTAINER_OF(dwork, struct bt_hid_host, close_work);
	struct bt_l2cap_chan *chan;
	int err;

	/* HID spec v1.1.2 Section 5.2.2: INTR is closed before CTRL. Whichever
	 * channel is still attached is the one to close; a released association has
	 * neither and nothing is left to do.
	 */
	if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED)) {
		chan = &hid->intr_session.br_chan.chan;
	} else if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED)) {
		chan = &hid->ctrl_session.br_chan.chan;
	} else {
		LOG_DBG("nothing left to close");
		return;
	}

	LOG_DBG("closing remaining channel in state %s", hid_state_str(hid->state));
	...
	if ((err != 0) && (err != -EALREADY)) {
		LOG_WRN("chan disconnect failed (%d)", err);
	}
}
...
static void hid_trans_timeout_handler(struct k_work *work)
{
	 ...
	 */
	LOG_WRN("transaction %s timed out in state %s", hid_ctrl_type_str(req.type),
...
static void hid_l2cap_ctrl_connected(struct bt_l2cap_chan *chan)
{
	...
	/* A disconnect requested before this connect completed left the instance in
	 * DISCONNECTING, so close the channel instead of advancing an abandoned one.
	 */
	if (hid->state == BT_HID_STATE_DISCONNECTING) {
		LOG_DBG("CTRL connected after disconnect requested, closing");
		(void)bt_l2cap_chan_disconnect(&hid->ctrl_session.br_chan.chan);
		return;
	}

	hid_set_state(hid, BT_HID_STATE_CTRL_CONNECTED);
	atomic_set_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_CONNECTED);

	if (!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INITIATOR)) {
		...
		bt_work_reschedule(&hid->close_work, HID_INTR_CONN_TIMEOUT);
		return;
	}

	atomic_set_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED);

	err = bt_l2cap_chan_connect(chan->conn, &hid->intr_session.br_chan.chan,
				    BT_L2CAP_PSM_HID_INTR);
	if (err != 0) {
		LOG_ERR("INTR connect failed (%d)", err);
		atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED);
		hid_set_state(hid, BT_HID_STATE_DISCONNECTING);
		bt_work_reschedule(&hid->close_work, K_NO_WAIT);
		return;
	}
...
static void hid_l2cap_intr_connected(struct bt_l2cap_chan *chan)
{
	 ...
	 */
	if (!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_CONNECTED)) {
	...
	atomic_set_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_CONNECTED);
	k_work_cancel_delayable(&hid->close_work);
...
{
	/* Releasing the instance only detaches it from the connection, so L2CAP has
	 * to be done with both channels first: a callback for a channel of this
	 * association would otherwise be dispatched to whatever is allocated next.
	 * A channel that is only connecting is attached as well, which is why the
	 * ATTACHED flags are checked here and not the CONNECTED ones. They are
	 * cleared only on the RX callback chain, so reading them together is not an
	 * atomic snapshot but is still correct: nothing can set them while this runs.
	 */
	__ASSERT(!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED) &&
			 !atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED),
		 "release with channels still attached");

	/* Publish the release before cancelling, so a context that arms a work item
	 * concurrently settles the race itself: an arm before this point is undone
	 * by the cancels below, one after it sees the released instance and cancels
	 * its own work. hid_req_claim() and hid_vcu_tx_cb() rely on this.
	 */
	atomic_clear(&hid->flags);
...
static void hid_l2cap_ctrl_disconnected(struct bt_l2cap_chan *chan)
{
	...
	atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_CONNECTED);
	atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED);
	 ...
	 */
	if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_CONNECTED)) {
		hid_set_state(hid, BT_HID_STATE_DISCONNECTING);
		/* Closed from hid_close_handler(), not from here: see its comment. */
		bt_work_reschedule(&hid->close_work, K_NO_WAIT);

		return;
	}

	/* INTR never came up but is still attached, so its disconnected callback is
	 * still to come and finishes the teardown there. Releasing the instance here
	 * would hand it to the next association, which that callback would then tear
	 * down. hid_l2cap_intr_connected() closes the channel if it does come up.
	 */
	if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED)) {
		hid_set_state(hid, BT_HID_STATE_DISCONNECTING);
		return;
...
static void hid_l2cap_intr_disconnected(struct bt_l2cap_chan *chan)
{
	...
	atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_CONNECTED);
	atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_INTR_ATTACHED);
	...
	/* CTRL is not up but still attached, so its disconnected callback finishes
	 * the teardown, see hid_l2cap_ctrl_disconnected().
	 */
	if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED)) {
		hid_set_state(hid, BT_HID_STATE_DISCONNECTING);
		return;
	}
...
static int hid_handshake_handle(struct bt_hid_host *hid, struct net_buf *buf, uint8_t param)
{
	...
	case BT_HID_CTRL_TYPE_SET_PROTOCOL:
		...
		if (param == BT_HID_HS_RSP_SUCCESS) {
			bool boot = (req.protocol == BT_HID_PROTOCOL_BOOT_MODE);

			atomic_set_bit_to(&hid->flags, BT_HID_HOST_FLAG_BOOT_MODE, boot);
			LOG_DBG("protocol mode %s", boot ? "boot" : "report");
		}
...
static int hid_data_handle(struct bt_hid_host *hid, struct net_buf *buf, uint8_t param)
{
	...
	/* A Control-channel DATA only ever answers a GET_* (a SET_* is acked by a
	 * HANDSHAKE), so peek at the pending type without consuming; the type is
	 * only acted on once hid_req_take() wins the atomic claim below.
	 */
	if (!atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_REQ_PENDING)) {
	...
	case BT_HID_CTRL_TYPE_GET_PROTOCOL: {
		...
		/* The transaction is now consumed, so a malformed reply is reported to
		 * the application as an error rather than dropped, which would leave the
		 * application waiting for a reply that can no longer arrive.
		 */
		if (buf->len < sizeof(uint8_t)) {
			LOG_WRN("Malformed GET_PROTOCOL DATA (len %u)", buf->len);
			if ((hid_cb != NULL) && (hid_cb->get_protocol != NULL)) {
				hid_cb->get_protocol(hid, BT_HID_HS_RSP_ERR_UNKNOWN,
						     atomic_test_bit(&hid->flags,
								     BT_HID_HOST_FLAG_BOOT_MODE)
							     ? BT_HID_PROTOCOL_BOOT_MODE
							     : BT_HID_PROTOCOL_REPORT_MODE);
		...
		boot = (FIELD_GET(BT_HID_PROTOCOL_MASK, net_buf_pull_u8(buf)) ==
			BT_HID_PROTOCOL_BOOT_MODE);
		atomic_set_bit_to(&hid->flags, BT_HID_HOST_FLAG_BOOT_MODE, boot);

		if ((hid_cb != NULL) && (hid_cb->get_protocol != NULL)) {
			hid_cb->get_protocol(hid, BT_HID_HS_RSP_SUCCESS,
					     boot ? BT_HID_PROTOCOL_BOOT_MODE
						  : BT_HID_PROTOCOL_REPORT_MODE);
		}
		break;
	}
	...
	case BT_HID_CTRL_TYPE_GET_REPORT:
		/* HID spec v1.1.2 Section 3.2.1 allows a single outstanding Control
		 * channel transaction, so a DATA message that arrives while a GET_REPORT
		 * is pending is the reply to it, whatever report type it carries.
		 */
		if (!hid_req_take(hid, &req)) {
			return 0;
		}
	...
	default:
		/* The outstanding transaction is a SET_*, which a HANDSHAKE (not a
		 * DATA) acknowledges, so this message does not belong to it and the
		 * transaction is left pending for its HANDSHAKE.
		 */
		LOG_DBG("Drop unexpected CTRL DATA for transaction %u", hid->req.type);
		break;
	}
...
static int hid_control_handle(struct bt_hid_host *hid, struct net_buf *buf, uint8_t control)
{
	 ...
	 */
	if (hid_req_take(hid, &req)) {
		LOG_DBG("transaction %s discarded on VC unplug", hid_ctrl_type_str(req.type));
	}
	 ...
	 * by disconnecting INTR then CTRL rather than by a HANDSHAKE. The teardown
	 * is deferred to the Bluetooth workqueue so the channels are not
	 * disconnected from inside the receive callback. Destroying the bonding is
	 * left to the application, see the vc_unplug callback.
	 */
	bt_work_schedule(&hid->vcu_work, K_NO_WAIT);
...
/* Attach a pool instance to a new association. The work items are initialized
 * once for the whole pool from hid_init(), so this cannot fail.
 */
static void hid_session_init(struct bt_hid_host *hid, enum bt_hid_role role)
{
	...
	/* Clears every state flag: no transaction is outstanding, both channels
	 * are disconnected, and per HID spec v1.1.2 Section 2.1.2 the default
	 * protocol mode is Report Protocol Mode (BT_HID_HOST_FLAG_BOOT_MODE unset).
	 * The role is stored right after, so it survives the reset.
	 */
	atomic_clear(&hid->flags);
	atomic_set_bit_to(&hid->flags, BT_HID_HOST_FLAG_INITIATOR,
			  role == BT_HID_ROLE_INITIATOR);
}
				      ...
				      struct bt_l2cap_chan **chan)
{
	...
	atomic_set_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED);
				      ...
				      struct bt_l2cap_chan **chan)
{
	 ...
	 * has to be established before the interrupt channel. The state also rules
	 * out a second interrupt channel: it leaves BT_HID_STATE_CTRL_CONNECTED as
...
static int hid_init(void)
{
	...
	/* A k_work_delayable may not be re-initialized while the kernel still holds
	 * it, and an item cancelled on teardown can outlive the association that
	 * armed it. Initializing the pool once keeps instance reuse free of that.
	 */
	if (!initialized) {
		ARRAY_FOR_EACH(hid_conn_pool, i) {
			k_work_init_delayable(&hid_conn_pool[i].close_work,
					      hid_close_handler);
			k_work_init_delayable(&hid_conn_pool[i].trans_work,
					      hid_trans_timeout_handler);
			k_work_init_delayable(&hid_conn_pool[i].vcu_work, hid_vcu_work_handler);
		}
...
int bt_hid_host_connect(struct bt_conn *conn, struct bt_hid_host **hid)
{
	 ...
	 */
	atomic_set_bit(&inst->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED);

	err = bt_l2cap_chan_connect(conn, &inst->ctrl_session.br_chan.chan,
				    BT_L2CAP_PSM_HID_CTRL);
	if (err != 0) {
		LOG_ERR("CTRL connect failed (%d)", err);
		/* L2CAP can fail through its own channel cleanup, which already ran the
		 * disconnected callback and released the instance.
		 */
		if (inst->state != BT_HID_STATE_DISCONNECTED) {
			atomic_clear_bit(&inst->flags, BT_HID_HOST_FLAG_CTRL_ATTACHED);
			hid_cleanup(inst);
		}
		return err;
	}
...
int bt_hid_host_disconnect(struct bt_hid_host *hid)
{
	...
	} else if (atomic_test_bit(&hid->flags, BT_HID_HOST_FLAG_CTRL_CONNECTED)) {
 ...
 * the instance is valid and its Control channel is ready to carry a request.
 *
 ...
 * allocating and formatting one that would then be rejected. Serializing
 * against a concurrent request is left to hid_req_claim(), not checked here.
 */
static int hid_check_ready(const struct bt_hid_host *hid)
{
	if (hid == NULL) {
		return -EINVAL;
	}
...
{
	struct bt_hid_req req = {
		.type = BT_HID_CTRL_TYPE_GET_REPORT,
		.report_type = type,
	 ...
	 */
	if (buffer_size > (hid->ctrl_session.br_chan.rx.mtu - sizeof(struct bt_hid_hdr))) {
...
{
	struct bt_hid_req req = {
		.type = BT_HID_CTRL_TYPE_SET_REPORT,
		.report_type = type,
	};
	struct bt_hid_hdr *hdr;
	int err;
...
{
	struct bt_hid_req req = {
		.type = BT_HID_CTRL_TYPE_GET_PROTOCOL,
	};
	struct net_buf *buf;
	int err;
...
{
	struct bt_hid_req req = {
		.type = BT_HID_CTRL_TYPE_SET_PROTOCOL,
		.protocol = protocol,
	};
	struct net_buf *buf;
	int err;
	...
	if ((protocol != BT_HID_PROTOCOL_BOOT_MODE) && (protocol != BT_HID_PROTOCOL_REPORT_MODE)) {
		return -EINVAL;
	}
...
int bt_hid_host_output_report(struct bt_hid_host *hid, struct net_buf *buf)
{
	...
	hdr = net_buf_push(buf, sizeof(struct bt_hid_hdr));
	hdr->header = BT_HID_BUILD_HDR(BT_HID_MSG_TYPE_DATA,
				       FIELD_PREP(BT_HID_PARAM_REPORT_TYPE_MASK,
						  BT_HID_REPORT_TYPE_OUTPUT));

	err = bt_l2cap_chan_send(&hid->intr_session.br_chan.chan, buf);
	if (err != 0) {
		LOG_ERR("Output_Report send failed (%d)", err);
		/* Restore the buffer to the caller's layout so a retry does not prepend
		 * a second header, mirroring bt_hid_host_set_report().
		 */
		net_buf_pull(buf, sizeof(struct bt_hid_hdr));
		return err;
	}
...
static void hid_vcu_tx_cb(struct bt_conn *conn, void *user_data, int err)
{
	...
	/* Runs on the TX notify workqueue, so it can arrive after the peer already
	 * acknowledged the unplug and the pool slot was reused. Consuming the claim
	 * taken before the send ties this completion to the association that sent
	 * the PDU: hid_cleanup() clears the flags.
	 */
	if (!atomic_test_and_clear_bit(&hid->flags, BT_HID_HOST_FLAG_VCU_SENT)) {
		LOG_DBG("VCU completion for a released association, ignoring");
		return;
	}
	 ...
	 * on the Bluetooth workqueue because this callback runs in the TX context.
	 */
	if (err != 0) {
		...
		bt_work_schedule(&hid->vcu_work, HID_TRANS_TIMEOUT);
	}

	/* hid_cleanup() may have raced the claim above; a released instance here
	 * means the work just armed has to be taken back.
	 */
	if (hid->state == BT_HID_STATE_DISCONNECTED) {
		k_work_cancel_delayable(&hid->vcu_work);
	}
...
int bt_hid_host_virtual_cable_unplug(struct bt_hid_host *hid)
{
	 ...
	 * never raced by a local disconnect. The claim lets that callback tell
	 * whether the association it was sent for still exists.
	 */
	atomic_set_bit(&hid->flags, BT_HID_HOST_FLAG_VCU_SENT);

	err = bt_l2cap_br_chan_send_cb(&hid->ctrl_session.br_chan.chan, buf,
				       hid_vcu_tx_cb, hid);
	if (err != 0) {
		LOG_ERR("VCU send failed (%d)", err);
		atomic_clear_bit(&hid->flags, BT_HID_HOST_FLAG_VCU_SENT);
		net_buf_unref(buf);
		return err;
	}

	/* Destroying the bonding is left to the application, see the API doc. */
