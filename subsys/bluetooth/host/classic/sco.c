...
int bt_sco_stream_connect(struct bt_conn *sco, struct bt_sco_stream *stream)
{
	...
	if (sco->sco.chan->stream == stream) {
		LOG_ERR("Stream has been connected");
		return -EALREADY;
	}
	...
	if (stream->sco == sco) {
		LOG_ERR("Stream is already connected");
		return -EALREADY;
	}

	if (stream->sco != NULL) {
		LOG_ERR("Stream is already connected to another SCO %p", stream->sco);
		return -EBUSY;
	}
...
int bt_sco_stream_send(struct bt_sco_stream *stream, struct net_buf *buf)
{
	...
	if (bt_dev.br.sco_mtu == 0) {
		LOG_ERR("Voice over HCI is unsupported");
		return -ENOTSUP;
	}
