...
#define LOG_DBG_ENABLED IS_ENABLED(CONFIG_BT_DEBUG_L2CAP)
...
LOG_MODULE_REGISTER(bt_l2cap_br, LOG_LEVEL);
			      ...
			      uint16_t len, struct net_buf *buf)
{
	...
	LOG_DBG("scid 0x%04x flags 0x%02x result 0x%02x len %u", scid, flags,
	       result, opt_len);
