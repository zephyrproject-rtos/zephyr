...
static struct bt_audio_ep broadcast_sink_endpoints
...
static void broadcast_sink_set_endpoint_state(struct bt_audio_ep *ep, uint8_t state)
{
	uint8_t old_state;
...
static void broadcast_sink_iso_connected(struct bt_iso_chan *chan)
{
	...
	broadcast_sink_set_endpoint_state(ep, BT_AUDIO_EP_STATE_STREAMING);
					    ...
					    uint8_t reason)
{
	...
	broadcast_sink_set_endpoint_state(ep, BT_AUDIO_EP_STATE_IDLE);
...
{
	for (int i = 0; i < ARRAY_SIZE(broadcast_sink_endpoints); i++) {
		if (PART_OF_ARRAY(broadcast_sink_endpoints[i], ep)) {
			return true;
		}
	}
...
static void broadcast_sink_endpoint_init(struct bt_audio_ep *ep)
{
	BT_DBG("ep %p", ep);
...
static struct bt_audio_ep *broadcast_sink_new_endpoint(uint8_t index)
{
	struct bt_audio_ep *cache = NULL;
	size_t size;

	cache = broadcast_sink_endpoints[index];
	size = ARRAY_SIZE(broadcast_sink_endpoints[index]);

	for (size_t i = 0; i < ARRAY_SIZE(broadcast_sink_endpoints[index]); i++) {
		...
		if (ep->stream == NULL) {
			 ...
			 * stream pointer.
			 */
			broadcast_sink_endpoint_init(ep);
			return ep;
		}
	}
						...
						struct bt_codec *codec)
{
	...
	ep = broadcast_sink_new_endpoint(index);
	if (ep == NULL) {
				 ...
				 const uint8_t broadcast_code[16])
{
	...
	for (size_t i = 0; i < stream_count; i++) {
		...
		broadcast_sink_set_endpoint_state(ep, BT_AUDIO_EP_STATE_QOS_CONFIGURED);
	}
