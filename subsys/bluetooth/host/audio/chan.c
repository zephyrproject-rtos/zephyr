			     ...
			     const struct bt_codec_qos *qos)
{
	if (chan->cap->qos.latency < qos->latency) {
		BT_DBG("Latency not within range: max %u latency %u",
			chan->cap->qos.latency, qos->latency);
		return false;
	}

	if (!IN_RANGE(chan->cap->qos.pd_min, chan->cap->qos.pd_max, qos->pd)) {
		BT_DBG("Presentation Delay not within range: min %u max %u pd %u",
			chan->cap->qos.pd_min, chan->cap->qos.pd_max,
			qos->pd);
		return false;
	}
	...
	*out_unicast_group = NULL;
	...
	for (size_t i = 0; i < num_chan; i++) {
		...
		if (chan->state != BT_AUDIO_CHAN_IDLE &&
			...
			BT_DBG("Incorrect channel[%zu] %p state: %u",
			       i, chan, chan->state);
...
int bt_audio_unicast_group_delete(struct bt_audio_unicast_group *unicast_group)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&unicast_group->chans, chan, node) {
		if (chan->state != BT_AUDIO_CHAN_IDLE &&
		    chan->state != BT_AUDIO_CHAN_CONFIGURED) {
			BT_DBG("chan %p is not idle", chan);
			return -EINVAL;
		}
	}
