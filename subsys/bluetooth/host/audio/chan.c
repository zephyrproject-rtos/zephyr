...
static struct bt_audio_broadcast_source broadcast_source[BROADCAST_SRC_CNT];
...
static int generate_broadcast_id(struct bt_audio_broadcast_source *source)
{
	...
	do {
		...
		/* Ensure uniqueness */
		unique = true;
		for (int i = 0; i < ARRAY_SIZE(broadcast_source); i++) {
			if (&broadcast_source[i] == broadcast_source) {
				continue;
			}

			if (broadcast_source[i].broadcast_id == source->broadcast_id) {
				unique = false;
				break;
			}
		}
				     ...
				     struct bt_codec_qos *qos)
{
	...
	source = NULL;
	for (index = 0; index < ARRAY_SIZE(broadcast_source); index++) {
		if (broadcast_source[index].bis[0] == NULL) { /* Find free entry */
			source = &broadcast_source[index];
			break;
		}
	}

	if (source == NULL) {
		BT_DBG("Could not allocate any more broadcast_source");
		return -ENOMEM;
	}
