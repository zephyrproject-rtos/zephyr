			   ...
			   const bt_addr_le_t *addr, const char *key)
{
	...
	for (int8_t i = 5; i >= 0; i--) {
		/* We are not checking hex2char return value as we supply valid 0-15 as input */
		hex2char(addr->a.val[i] >> 4, &path[offset]);
		offset++;
		/* We are not checking hex2char return value as we supply valid 0-15 as input */
		hex2char(addr->a.val[i] & 0xf, &path[offset]);
		offset++;
	}
	/* We are not checking hex2char return value as we supply valid 0-1 as input */
	hex2char(addr->type, &path[offset]);
	offset++;
...
static int bt_settings_encode_key_no_addr(char *path, size_t path_size, const char *key)
{
	...
	const size_t total_len = prefix_len + sizeof('/') + key_len + sizeof('\0');
	size_t offset = 0U;
