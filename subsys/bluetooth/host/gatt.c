...
{
	if (bt_le_addr_is_bonded(conn->id, &conn->le.dst)) {
		int err;
...
static void sc_save(uint8_t id, bt_addr_le_t *peer, uint16_t start, uint16_t end)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS) && modified &&
	    bt_le_addr_is_bonded(cfg->id, &cfg->peer)) {
		sc_store(cfg);
	}
}
				      ...
				      const bt_addr_le_t *id_addr)
{
	...
	bool is_bonded = bt_le_addr_is_bonded(conn->id, &conn->le.dst);
...
static void gatt_delayed_store_enqueue(uint8_t id, const bt_addr_le_t *peer_addr,
				       enum delayed_store_flags flag)
{
	bool bonded = bt_le_addr_is_bonded(id, peer_addr);
...
static void gatt_store_ccc_cf(uint8_t id, const bt_addr_le_t *peer_addr)
{
	...
	if (bt_le_addr_is_bonded(id, peer_addr)) {
...
void bt_gatt_connected(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_BT_SETTINGS_CCC_LAZY_LOADING) &&
	    bt_le_addr_is_bonded(conn->id, &conn->le.dst)) {
