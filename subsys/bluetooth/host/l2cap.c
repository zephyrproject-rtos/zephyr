				...
				size_t *length)
{
	...
	if (L2CAP_LE_CID_IS_DYN(lechan->tx.cid) && (lechan->_sdu_remaining == 0)) {
...
#if defined(CONFIG_BT_L2CAP_DYNAMIC_CHANNEL)
	size_t payload_bytes_sent = *length - sizeof(struct bt_l2cap_hdr);

	if (lechan->_sdu_remaining > payload_bytes_sent) {
		lechan->_sdu_remaining -= payload_bytes_sent;
	} else {
		lechan->_sdu_remaining = 0;
	}
	LOG_DBG("%s done: amount %u, payload_bytes %u, _pdu_remaining %u, _sdu_remaining %u",
		__func__, amount, payload_bytes_sent, lechan->_pdu_remaining,
