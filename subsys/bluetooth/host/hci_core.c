 ...
 */
#define CHECK_NUM_OF_ACL_PKTS(num) \
	do { \
		if (CONFIG_BT_BUF_ACL_TX_COUNT != (num)) { \
			LOG_WRN("Num of Controller's ACL packets != ACL bt_conn_tx contexts" \
				" (%u != %u)", (num), CONFIG_BT_BUF_ACL_TX_COUNT); \
		} \
	} while (0)

#define CHECK_NUM_OF_ISO_PKTS(num) \
	do { \
		if (CONFIG_BT_ISO_TX_BUF_COUNT != (num)) { \
			LOG_WRN("Num of Controller's ISO packets != ISO bt_conn_tx contexts" \
				"(%u != %u)", (num), CONFIG_BT_ISO_TX_BUF_COUNT); \
		} \
	} while (0)
