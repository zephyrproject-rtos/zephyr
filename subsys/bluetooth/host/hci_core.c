...
/*  BT_HCI_NODE for testing purposes, hcixx can be used to get multiple nodes
 *  for multiple controllers. Example for second controller:
 *  #define BT_HCI_NODE(index)	DT_CAT(zephyr_bt_hci, index)
 */
#define BT_HCI_NODE(index)	DT_CHOSEN(zephyr_bt_hci)
#define BT_HCI_DEV(index)  DEVICE_DT_GET(BT_HCI_NODE(index))
#define BT_HCI_BUS  BT_DT_HCI_BUS_GET(BT_HCI_NODE(0))
#define BT_HCI_NAME BT_DT_HCI_NAME_GET(BT_HCI_NODE(0))
#define BT_HCI_QUIRKS BT_DT_HCI_QUIRKS_GET(BT_HCI_NODE(0))
#else
...
static struct bt_dev *bt_dev_alloc(uint8_t dev_id)
{
...
#if DT_HAS_CHOSEN(zephyr_bt_hci)
	if (dev_id == 0) {
		hdev->hci = BT_HCI_DEV(0);
	} else if (dev_id == 1) {
		hdev->hci = BT_HCI_DEV(1);
	} else {
		hdev->hci = NULL;
		return NULL;
	}
...
static void handle_event(struct bt_dev *hdev, uint8_t event, struct net_buf *buf, const struct event_handler *handlers,
			 size_t num_handlers)
{
	int err;
...
void bt_send_one_host_num_completed_packets(uint16_t handle)
{
	...
	err = bt_hci_cmd_send(bt_dev_get(0), BT_HCI_OP_HOST_NUM_COMPLETED_PACKETS, buf);
			 ...
			 struct net_buf **rsp)
{
	...
	err = bt_hci_cmd_send(hdev, opcode, net_buf_ref(buf));
	if (err) {
		net_buf_unref(buf);
		return err;
	}
...
static void hci_cmd_done(struct bt_dev *hdev, uint16_t opcode, uint8_t status, struct net_buf *evt_buf)
{
...
static int read_le_local_supported_features(struct bt_dev *hdev)
{
	...
	if (IS_ENABLED(CONFIG_BT_LE_EXTENDED_FEAT_SET) &&
		...
		err = bt_hci_cmd_send_sync(hdev, BT_HCI_OP_LE_READ_ALL_LOCAL_SUPPORTED_FEATURES, NULL,
					   &rsp);
		if (err != 0) {
			return err;
		}
...
int bt_enable_mc(uint8_t dev_id, bt_ready_cb_t cb)
{
	...
	if (IS_ENABLED(CONFIG_ZTEST) && hdev->hci == NULL) {
