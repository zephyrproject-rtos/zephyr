...
/* Pool for ATT responses */
#if CONFIG_BT_ATT_RESPONSE_COUNT > 0
NET_BUF_POOL_FIXED_DEFINE(att_rsp_pool, CONFIG_BT_ATT_RESPONSE_COUNT,
				       ...
				       size_t len)
{
	...
	case ATT_CONFIRMATION:
#if CONFIG_BT_ATT_RESPONSE_COUNT > 0
		/* Use dedicated buffer pool for responses/confirmations as they
		 * are self generated by the stack and will never be queued.
		 */
		buf = bt_l2cap_create_pdu_timeout(&att_rsp_pool, 0,
						  BT_ATT_TIMEOUT);
#else
