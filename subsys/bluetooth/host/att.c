...
static void bt_att_sent(struct bt_l2cap_chan *ch)
{
	...
	/* `chan->att` is freed on the BT RX thread (att_chan_detach() ->
	 * att_reset()) when the bearer disconnects. For UATT this callback runs
	 * on the system workqueue, so hold the scheduler lock across the queue
	 * processing and re-read `chan->att` under it: att_chan_detach() clears
	 * `chan->att` before att_reset() frees it, so a non-NULL read under the
	 * lock guarantees the context stays valid until k_sched_unlock().
	 */
	k_sched_lock();
