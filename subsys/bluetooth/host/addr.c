...
int bt_addr_le_from_str(const char *str, const char *type, bt_addr_le_t *addr)
{
	...
	/* Check if the string uses the new prefix format (P:... or R:...). This
	 * legacy two-token parsing is kept independent of
	 * CONFIG_BT_ADDR_LE_LEGACY_STRING_OUTPUT (which only affects generated
	 * output) so that existing scripts and shell usage keep working
	 * indefinitely, regardless of that Kconfig's eventual removal.
	 */
