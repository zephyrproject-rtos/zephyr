...
LOG_MODULE_REGISTER(bt_adv, LOG_LEVEL);
...
void bt_hci_le_adv_set_terminated(struct net_buf *buf)
{
	...
	LOG_DBG("status 0x%02x adv_handle %u conn_handle 0x%02x num %u",
...
void bt_hci_le_scan_req_received(struct net_buf *buf)
{
	...
	LOG_DBG("handle %u peer %s", evt->handle, bt_addr_le_str_real(&evt->addr));
