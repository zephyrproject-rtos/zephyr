...
static int cmd_frame_space_update(const struct shell *sh, size_t argc, char *argv[])
{
	...
	const struct bt_conn_le_frame_space_update_param params = {
		.frame_space_min = shell_strtoul(argv[1], 10, &err),
		.frame_space_max = shell_strtoul(argv[2], 10, &err),
		.phys = shell_strtoul(argv[3], 10, &err),
		.spacing_types = shell_strtoul(argv[4], 10, &err),
	};
