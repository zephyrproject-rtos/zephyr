...
LOG_MODULE_REGISTER(bt_rfcomm, LOG_LEVEL);
				       ...
				       uint8_t credits)
{
	...
	LOG_DBG("dlc %p updated credits %u", dlc,
...
static int rfcomm_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	case BT_RFCOMM_DM:
		...
		break;
	default:
		LOG_WRN("Unknown/Unsupported RFCOMM Frame type 0x%02x",
			frame_type);
		break;
	}
				  ...
				  uint8_t hci_status)
{
	...
	LOG_DBG("session %p status 0x%02x encr 0x%02x", session, hci_status,
	       conn->encrypt);
