...
LOG_MODULE_REGISTER(bt_sdp, LOG_LEVEL);
			      ...
			      struct bt_sdp_record **matching_recs)
{
	...
	if (((data_elem.type & BT_SDP_TYPE_DESC_MASK) != BT_SDP_SEQ_UNSPEC) &&
		...
		LOG_WRN("Invalid type %x in service search pattern",
			data_elem.type);
		return BT_SDP_INVALID_SYNTAX;
	}
	...
	while (uuid_list_size) {
		...
		if ((data_elem.type & BT_SDP_TYPE_DESC_MASK) !=
		    BT_SDP_UUID_UNSPEC) {
			LOG_WRN("Invalid type %u in service search pattern",
				data_elem.type);
			return BT_SDP_INVALID_SYNTAX;
		}
				...
				uint16_t tid)
{
	...
	for (; cont_state < num_services; cont_state++) {
		...
		if (pkt_full) {
			...
			LOG_DBG("Packet full, num_services_covered %u",
			       cont_state);
	...
	*num_filters = 0U;
	...
	while (size) {
		...
		if ((data_elem.type & BT_SDP_TYPE_DESC_MASK) != BT_SDP_UINT8) {
			LOG_WRN("Invalid type %u in attribute ID list",
				data_elem.type);
			return BT_SDP_INVALID_SYNTAX;
		}
			     ...
			     uint16_t tid)
{
	...
	LOG_DBG("svc_rec_hdl %u, max_att_len 0x%04x, cont_state %u", svc_rec_hdl,
				    ...
				    uint16_t tid)
{
	...
	LOG_DBG("max_att_len 0x%04x, state.current_svc %u, state.last_att %u",
	...
	for (; next_svc < num_services; next_svc++) {
		...
		if (state.pkt_full && !dry_run) {
			LOG_DBG("Packet full, state.last_att %u",
			       state.last_att);
			dry_run = true;
