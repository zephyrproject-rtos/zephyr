		    ...
		    uint16_t len, const uint8_t *mac)
{
	...
	bt_crypto_toolbox_aes_cmac(key, m, len, out);
...
static int smp_f4_test(void)
{
	...
	err = bt_crypto_toolbox_f4(u, v, x, z, res);
	if (err) {
		return err;
	}
...
static int smp_f5_test(void)
{
	...
	err = bt_crypto_toolbox_f5(w, n1, n2, &a1, &a2, mackey, ltk);
	if (err) {
		return err;
	}
...
static int smp_f6_test(void)
{
	...
	err = bt_crypto_toolbox_f6(w, n1, n2, r, io_cap, &a1, &a2, res);
	if (err)
		return err;
...
static int smp_g2_test(void)
{
	...
	err = bt_crypto_toolbox_g2(u, v, x, y, &val);
	if (err) {
		return err;
	}
