...
{
	print(NULL, "Incoming RFCOMM conn %p\n", conn);
...
static void cmd_rfcomm_register(const struct shell *shell,
				size_t argc, char *argv[])
{
	int ret;
			    ...
			    size_t argc, char *argv[])
{
	...
	while (count--) {
		buf = bt_rfcomm_create_pdu(&data_pool);
...
#if defined(CONFIG_BT_RFCOMM)
	SHELL_CMD(rfcomm-register, NULL, "<channel>", cmd_rfcomm_register),
	SHELL_CMD(rfcomm-connect, NULL, "<channel>", cmd_rfcomm_connect),
	SHELL_CMD(rfcomm-send, NULL, "<number of packets>", cmd_rfcomm_send),
	SHELL_CMD(rfcomm-disconnect, NULL, HELP_NONE, cmd_rfcomm_disconnect),
#endif /* CONFIG_BT_RFCOMM */
