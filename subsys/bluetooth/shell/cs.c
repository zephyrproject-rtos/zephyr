...
static int cmd_cs_test_simple(const struct shell *sh, size_t argc, char *argv[])
{
	...
	params.sub_mode = BT_HCI_OP_LE_CS_TEST_SUB_MODE_UNUSED;
	...
	if (params.role != BT_CS_TEST_ROLE_INITIATOR &&
			params.role != BT_CS_TEST_ROLE_REFLECTOR) {
		shell_help(sh);
	...
	params.cs_sync_antenna_selection = BT_CS_TEST_CS_SYNC_ANTENNA_SELECTION_OPT_ONE;
