				...
				uint8_t const *const data)
{
	...
	*val_ptr++ = len;
	...
	/* Get reference to aux ptr in AUX_ADV_IND PDU */
	(void)memcpy(&aux_ptr, &hdr_data[ULL_ADV_HDR_DATA_AUX_PTR_PTR_OFFSET],
		     sizeof(aux_ptr));

	ull_adv_aux_chain_pdu_append(pdu, aux_ptr, adv->lll.phy_s,
				     adv->lll.phy_flags,
				     EVENT_SYNC_B2B_MAFS_US, 6);
#endif /* CONFIG_BT_CTLR_ADV_PDU_LINK */
...
#if defined(CONFIG_BT_CTLR_ADV_PDU_LINK)
	ull_adv_aux_chain_pdu_duplicate(ter_pdu_prev, ter_pdu, aux_ptr,
					lll_sync->adv->phy_s,
					lll_sync->adv->phy_flags,
