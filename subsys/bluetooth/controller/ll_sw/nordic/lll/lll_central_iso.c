...
static int prepare_cb(struct lll_prepare_param *p)
{
	...
	uint32_t overhead = lll_preempt_calc(ull, (TICKER_ID_CONN_ISO_BASE +
						   cig_lll->handle),
					     ticks_at_event);
...
static void isr_tx(void *param)
{
	...
	if (se_curr < cis_lll->nse) {
		...
		subevent_us = radio_tmr_ready_restore();
		subevent_us += next_cis_lll->offset;
...
isr_rx_next_subevent:
	if (cie || (se_curr == cis_lll->nse)) {
		...
		/* Fetch next CIS */
		cig_lll = ull_conn_iso_lll_group_get_by_stream(cis_lll);
		next_cis_lll = ull_conn_iso_lll_stream_get_by_group(cig_lll, &cis_handle_curr);
		if (!next_cis_lll || !next_cis_lll->active) {
			goto isr_rx_done;
		}

		/* TODO: Adjust sn and nesn on flushing Tx and Rx */
