...
static void isr_done(void *param)
{
	...
	if (trx_cnt) {
		...
		if (lll->role) {
			...
			e->perip.start_to_address_actual_us =
				radio_tmr_aa_restore() - radio_tmr_ready_get();
			e->perip.window_widening_event_us =
				lll->perip.window_widening_event_us;
			e->perip.preamble_to_addr_us = preamble_to_addr_us;

			/* Reset window widening, as anchor point sync-ed */
			lll->perip.window_widening_event_us = 0;
			lll->perip.window_size_event_us = 0;
		}
	}
#endif /* CONFIG_BT_PERIPHERAL */
