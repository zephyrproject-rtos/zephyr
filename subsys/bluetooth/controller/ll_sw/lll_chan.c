...
static struct chan_metrics_chan *chan_metrics_count_inc(uint8_t chan_curr)
{
	struct chan_metrics_chan *chan;

	/* Mitigate metrics overflow, half the collected good and count value */
	chan = &chan_metrics.chan[chan_curr];
...
void lll_chan_metrics_chan_bad(uint8_t chan_curr)
{
	struct chan_metrics_chan *chan;
	uint16_t diff;

	/* Increment the per channel metrics count */
	chan = chan_metrics_count_inc(chan_curr);
...
void lll_chan_metrics_chan_good(uint8_t chan_curr)
{
	struct chan_metrics_chan *chan;
