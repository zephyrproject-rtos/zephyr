...
static u32_t isr_rx_scan_report(u8_t rssi_ready, u8_t rl_idx, bool dir_report)
{
	...
	if (0) {
		...
		radio_pdu_node_rx->hdr.type = NODE_RX_TYPE_MESH_REPORT;
#endif /* CONFIG_BT_HCI_MESH_EXT */
...
#if defined(CONFIG_BT_HCI_MESH_EXT)
	if (radio_pdu_node_rx->hdr.type == NODE_RX_TYPE_MESH_REPORT) {
		/* save the directed adv report flag */
...
static inline u32_t isr_close_adv_mesh(void)
{
	...
	_radio.scanner.ticks_window =
		TICKER_US_TO_TICKS(_radio.advertiser.scan_window_ms * 1000 +
				   radio_tmr_end_get() -
	...
	ticks_anchor = _radio.ticks_anchor +
		       TICKER_US_TO_TICKS(_radio.mesh_adv_end_us +
...
static inline void event_conn_param_rsp(struct connection *conn)
{
	...
	if (conn->llcp_conn_param.status) {
		...
		rej = (void *)&pdu->payload.llctrl.ctrldata.reject_ext_ind;
	...
	pdu->payload.llctrl.opcode =
		PDU_DATA_LLCTRL_TYPE_CONN_PARAM_RSP;
	rsp = (void *)&pdu->payload.llctrl.ctrldata.conn_param_rsp;
					  ...
					  u16_t event_counter)
{
	...
	if (conn->llcp.phy_upd_ind.initiate) {
		...
		if (node_tx) {
			...
			ind = &pdu_ctrl_tx->payload.llctrl.ctrldata.phy_upd_ind;
				...
				u8_t *rx_enqueue)
{
	...
	pdu_data_rx = (struct pdu_data *)radio_pdu_node_rx->pdu_data;
	ind = &pdu_data_rx->payload.llctrl.ctrldata.phy_upd_ind;
...
#if defined(CONFIG_BT_HCI_MESH_EXT)
	} else if (radio_pdu_node_rx->hdr.type == NODE_RX_TYPE_MESH_ADV_CPLT) {
		LL_ASSERT(_radio.advertiser.is_enabled);
		_radio.advertiser.is_enabled = 0;
