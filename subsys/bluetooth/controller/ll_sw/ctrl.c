...
#if defined(CONFIG_BLUETOOTH_CONTROLLER_CONN_PARAM_REQ) || \
    defined(CONFIG_BLUETOOTH_CONTROLLER_PHY)
				  ...
				  u8_t *rx_enqueue)
{
	...
	if (_radio.conn_curr->role) {
		_radio.conn_curr->slave.ticks_to_offset =
		    _radio.conn_curr->llcp_conn_param.ticks_to_offset_next;
	}
	...
	if (!_radio.conn_curr->llcp_conn_param.is_internal) {
		     ...
		     u8_t *rx_enqueue)
{
	...
	case PDU_DATA_LLCTRL_TYPE_CONN_PARAM_REQ:
		...
		if (((_radio.conn_curr->llcp_req == _radio.conn_curr->llcp_ack) &&
			 ...
			 * parameters
			 */
			_radio.conn_curr->llcp_conn_param.interval =
				pdu_data_rx->payload.llctrl.ctrldata.conn_param_req.interval_min;
			_radio.conn_curr->llcp_conn_param.latency =
				pdu_data_rx->payload.llctrl.ctrldata.conn_param_req.latency;
			_radio.conn_curr->llcp_conn_param.timeout =
			...
			if ((_radio.conn_curr->llcp_conn_param.interval !=
				...
				*rx_enqueue = 1;
				...
				_radio.conn_curr->llcp_conn_param.is_internal = 0;
				_radio.conn_curr->llcp_type = LLCP_CONNECTION_UPDATE;
				_radio.conn_curr->llcp_ack--;
			} else {
				_radio.conn_curr->llcp_conn_param.win_size = 1;
				_radio.conn_curr->llcp_conn_param.win_offset_us = 0;
				_radio.conn_curr->llcp_conn_param.state =
					LLCP_CONN_STATE_RSP;
				_radio.conn_curr->llcp_conn_param.is_internal = 0;
					   ...
					   u32_t ticks_at_expire)
{
	...
	if (conn->llcp_conn_param.state) {
...
#endif /* CONFIG_BLUETOOTH_CONTROLLER_SCHED_ADVANCED */
			state = conn->llcp_conn_param.state;
			if ((state == LLCP_CONN_STATE_RSP) && !conn->role) {
		...
		if (conn->role) {
			...
			/* TODO: ??? what is this from old code?
			 *
			 * if (conn->llcp_conn_param.is_internal == 2) {
			 *	conn_update_req(conn);
			 * }
			 */
		} else {
...
static u32_t conn_update_req(struct connection *conn)
{
	...
	if (!conn->role) {
		...
		conn->conn_param_req.latency = conn->latency;
		conn->conn_param_req.timeout = conn->conn_interval *
			conn->supervision_reload * 125 / 1000;
		/* conn->conn_param_req.instant     = 0; */
		conn->conn_param_req.state =
...
#endif /* CONFIG_BLUETOOTH_CONTROLLER_CONN_PARAM_REQ  || PHY */
