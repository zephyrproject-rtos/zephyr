...
uint8_t ull_fsu_update_eff_from_local(struct ll_conn *conn)
{
	...
	fsu_min = MAX(conn->lll.fsu.local.fsu_min, EVENT_IFS_LOW_LAT_US);
	fsu_max = MAX(conn->lll.fsu.local.fsu_max, EVENT_IFS_LOW_LAT_US);
...
uint8_t ull_fsu_init(struct ll_conn *conn)
{
	...
	conn->lll.fsu.local.fsu_min = EVENT_IFS_LOW_LAT_US;
