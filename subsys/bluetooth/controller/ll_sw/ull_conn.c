...
uint8_t ull_dle_update_eff_rx(struct ll_conn *conn)
{
	...
	const uint16_t eff_rx_octets =
		GENERIC_MAX(GENERIC_MIN(conn->lll.dle.local.max_rx_octets,
					conn->lll.dle.remote.max_tx_octets),
	...
	const uint16_t eff_rx_time =
		GENERIC_MAX(GENERIC_MIN(conn->lll.dle.local.max_rx_time,
					conn->lll.dle.remote.max_tx_time),
			    min_eff_rx_time);
...

{
	...
	const uint16_t eff_tx_octets =
		GENERIC_MAX(GENERIC_MIN(conn->lll.dle.local.max_tx_octets,
					conn->lll.dle.remote.max_rx_octets),
			    PDU_DC_PAYLOAD_SIZE_MIN);
	...
	const uint16_t eff_tx_time =
		GENERIC_MAX(GENERIC_MIN(conn->lll.dle.local.max_tx_time,
					conn->lll.dle.remote.max_rx_time),
			    min_eff_tx_time);
