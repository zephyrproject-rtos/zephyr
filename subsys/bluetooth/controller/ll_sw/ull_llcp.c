...
uint8_t ull_cp_conn_update(struct ll_conn *conn, uint16_t interval_min, uint16_t interval_max, uint16_t latency, uint16_t timeout)
{
	...
	if (feature_conn_param_req(conn)) {					//TODO(tosk): fex.valid required?
		ctx = create_local_procedure(PROC_CONN_PARAM_REQ);		//TODO(tosk): Check for conn->llcp.conn_param.disabled?
	...
	case PROC_CONN_UPDATE:
		...
		ctx->data.cu.interval = interval_max;
		ctx->data.cu.latency = latency;
		ctx->data.cu.timeout = timeout;
		break;
#if defined(CONFIG_BT_CTLR_CONN_PARAM_REQ)
	case PROC_CONN_PARAM_REQ:
		ctx->data.conn_param.interval_min = interval_min;
		ctx->data.conn_param.interval_max = interval_max;
		ctx->data.conn_param.latency = latency;
		ctx->data.conn_param.timeout = timeout;
	...
	/* TODK(tosk): Check what to handle (ADV_SCHED) from this legacy fct.  */
	//event_conn_upd_init()
