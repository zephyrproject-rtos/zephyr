...
static inline u16_t aux_handle_get(struct ll_scan_aux_set *aux);
...
void ull_scan_aux_setup(struct node_rx_hdr *rx, u8_t phy,
			struct ll_scan_aux_set *aux)
{
	...
	if (aux_ptr->offs_units) {
		aux_offset_us += (u32_t)aux_ptr->offs * 300U;
	} else {
		aux_offset_us += (u32_t)aux_ptr->offs * 30U;
	}
