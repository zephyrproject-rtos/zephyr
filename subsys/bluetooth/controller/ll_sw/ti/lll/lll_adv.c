...
static inline int isr_rx_pdu(struct lll_adv *lll, u8_t devmatch_ok,
...
static inline int isr_rx_sr_report(struct pdu_adv *pdu_adv_rx, u8_t rssi_ready);
...
static int prepare_cb(struct lll_prepare_param *prepare_param)
{
	...
	radio_gpio_pa_setup();
	radio_gpio_pa_lna_enable(remainder_us + radio_tx_ready_delay_get(0, 0) -
...
static void chan_prepare(struct lll_adv *lll)
{
	...
	if (upd) {
		 ...
		 * scan response.
		 */
		memcpy(&scan_pdu->scan_rsp.addr[0], &pdu->adv_ind.addr[0],
		       BDADDR_SIZE);
	}
#else
	radio_set_scan_rsp_data(scan_pdu->scan_rsp.data,
				scan_pdu->len -
					sizeof(scan_pdu->scan_rsp.addr));
	ARG_UNUSED(upd);
#endif /* !CONFIG_BT_CTLR_PRIVACY */
			     ...
			     u8_t irkmatch_id, u8_t rssi_ready)
{
	...
	if ((pdu_rx->type == PDU_ADV_TYPE_SCAN_REQ) &&
		...
		snprintf(
			...
			"access_addr: 0x%02x%02x%02x%02x\n\t"
			...
			x->latency,
			x->timeout,
			x->chan_map[0], x->chan_map[1], x->chan_map[2], x->chan_map[3], x->chan_map[4],
			x->hop,
			x->sca
		);
		printk( "%s\n", log_buffer );
...
static inline bool isr_rx_sr_adva_check(struct pdu_adv *adv, struct pdu_adv *sr)
{
