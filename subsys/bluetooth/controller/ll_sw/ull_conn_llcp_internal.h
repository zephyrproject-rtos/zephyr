...
u16_t ll_conn_handle_get(struct ull_cp_conn *conn);
struct ull_cp_conn *ll_conn_get(u16_t handle);
struct ull_cp_conn *ll_connected_get(u16_t handle);
...
int ull_conn_llcp(struct ull_cp_conn *conn, u32_t ticks_at_expire, u16_t lazy);
void ull_conn_done(struct node_rx_event_done *done);
void ull_conn_tx_demux(u8_t count);
void ull_conn_tx_lll_enqueue(struct ull_cp_conn *conn, u8_t count);
...
struct ull_cp_conn *ull_conn_tx_ack(u16_t handle, memq_link_t *link,
				struct node_tx *tx);
