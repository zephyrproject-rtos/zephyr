...
uint8_t *ll_addr_get(uint8_t addr_type, uint8_t *const p_bdaddr);
uint8_t ll_addr_set(uint8_t addr_type, uint8_t const *const p_bdaddr);

#if defined(CONFIG_BT_CTLR_HCI_ADV_HANDLE_MAPPING)
uint8_t ll_adv_set_by_hci_handle_get(uint8_t hci_handle, uint8_t *const handle);
uint8_t ll_adv_set_by_hci_handle_get_or_new(uint8_t hci_handle,
					    uint8_t *const handle);
uint8_t ll_adv_set_hci_handle_get(uint8_t handle);
uint8_t ll_adv_iso_by_hci_handle_get(uint8_t hci_handle, uint8_t *const handle);
uint8_t ll_adv_iso_by_hci_handle_new(uint8_t hci_handle, uint8_t *const handle);
#else
...
uint8_t ll_adv_params_set(uint8_t handle, uint16_t evt_prop, uint32_t interval,
		       ...
		       const uint8_t *const direct_addr, uint8_t chan_map,
		       uint8_t filter_policy, uint8_t *const tx_pwr,
		       uint8_t phy_p, uint8_t skip, uint8_t phy_s, uint8_t sid,
		       uint8_t sreq);
...
uint8_t ll_configure_data_path(uint8_t data_path_dir,
			       ...
			       const uint8_t *const vs_config);
uint8_t ll_read_iso_tx_sync(uint16_t handle, uint16_t *const seq,
			    uint32_t *const timestamp, uint32_t *const offset);
uint8_t ll_read_iso_link_quality(uint16_t  handle,
				 uint32_t *const tx_unacked_packets,
				 uint32_t *const tx_flushed_packets,
				 uint32_t *const tx_last_subevent_packets,
				 uint32_t *const retransmitted_packets,
				 uint32_t *const crc_error_packets,
				 uint32_t *const rx_unreceived_packets,
				 uint32_t *const duplicate_packets);
uint8_t ll_setup_iso_path(uint16_t handle, uint8_t path_dir, uint8_t path_id,
			  ...
			  const uint8_t *const codec_config);
...
uint8_t ll_iso_test_end(uint16_t handle, uint32_t *const received_cnt,
			uint32_t *const missed_cnt, uint32_t *const failed_cnt);
uint8_t ll_iso_read_test_counters(uint16_t handle, uint32_t *const received_cnt,
				  uint32_t *const missed_cnt,
				  uint32_t *const failed_cnt);
...
uint8_t ll_rl_add(const bt_addr_le_t *const id_addr, const uint8_t pirk[16],
...
uint8_t ll_rl_crpa_get(const bt_addr_le_t *const id_addr,
		       bt_addr_t *const crpa);
uint8_t ll_rl_lrpa_get(const bt_addr_le_t *const id_addr,
		       bt_addr_t *const lrpa);
...
void ll_df_read_ant_inf(uint8_t *const switch_sample_rates,
			uint8_t *const num_ant,
			uint8_t *const max_switch_pattern_len,
			uint8_t *const max_cte_len);

/* Downstream - Data */
void *ll_tx_mem_acquire(void);
void ll_tx_mem_release(void *const node_tx);
int ll_tx_mem_enqueue(uint16_t handle, void *const node_tx);

/* Upstream - Num. Completes, Events and Data */
uint8_t ll_rx_get(void **const node_rx, uint16_t *const handle);
void ll_rx_dequeue(void);
void ll_rx_mem_release(void **const node_rx);

/* Downstream - ISO Data */
void *ll_iso_tx_mem_acquire(void);
void ll_iso_tx_mem_release(const void *const tx);
int ll_iso_tx_mem_enqueue(uint16_t handle, const void *const tx);
