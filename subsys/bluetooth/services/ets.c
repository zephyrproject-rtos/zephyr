...
/* Work item for sending indications (reusable for CCC enable and time writes) */
static void send_indication_work_handler(struct k_work *work)
{
	int err;

	if (!ets_cb || !ets_cb->read_elapsed_time || !ets_cb->read_clock_status ||
	    !ets_cb->read_clock_capabilities) {
	...
	/* Read clock status */
	err = ets_cb->read_clock_status(&indicate_data.clock_status);
	if (err) {
	...
	err = ets_cb->read_clock_capabilities(&indicate_data.clock_capabilities);
	if (err) {
		LOG_ERR("Failed to read clock capabilities for indication: %d", err);
		return;
	}

	/* Send indication using the static indicate_data buffer */
	err = bt_ets_indicate(&indicate_data.et);
	if (err && err != -ENOTCONN) {
...
static void ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
	...
	LOG_INF("ETS indications %s", indicate_enabled ? "enabled" : "disabled");

	if (ets_cb && ets_cb->indication_changed) {
				 ...
				 uint16_t len, uint16_t offset)
{
	...
	err = ets_cb->read_clock_capabilities(&response.clock_capabilities);
	if (err) {
		LOG_ERR("ETS read_clock_capabilities callback failed: %d", err);
		return BT_GATT_ERR(BT_ATT_ERR_UNLIKELY);
	}
				  ...
				  const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (!ets_cb || !ets_cb->write_elapsed_time) {
		LOG_WRN("ETS write not supported");
	...
	/* Let application validate and set the time */
...
int bt_ets_init(const struct bt_ets_cb *cb)
{
	...
	__ASSERT(cb->read_clock_capabilities != NULL,
		 "`read_clock_capabilities` callback is required for ETS");

	if (!cb || !cb->read_elapsed_time || !cb->read_clock_status ||
	    !cb->read_clock_capabilities) {
		return -EINVAL;
	}
...
int bt_ets_indicate(const struct bt_ets_elapsed_time *elapsed_time)
{
	...
	if (!ets_cb || !ets_cb->read_clock_status || !ets_cb->read_clock_capabilities) {
		LOG_ERR("ETS callbacks not set");
		return -EINVAL;
	}
	...
	err = ets_cb->read_clock_status(&indicate_data.clock_status);
	if (err) {
		LOG_ERR("Failed to read clock status: %d", err);
		return err;
	}

	err = ets_cb->read_clock_capabilities(&indicate_data.clock_capabilities);
	if (err) {
		LOG_ERR("Failed to read clock capabilities: %d", err);
		return err;
	}
