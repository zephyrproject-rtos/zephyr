...
static void send_indication_work_handler(struct k_work *work)
{
	...
	if ((ets_cb == NULL) || (ets_cb->read_elapsed_time == NULL) ||
	    (ets_cb->read_clock_status == NULL) || (ets_cb->read_clock_capabilities == NULL)) {
...
int bt_ets_init(const struct bt_ets_cb *cb)
{
	...
	if ((cb == NULL) || (cb->read_elapsed_time == NULL) || (cb->read_clock_status == NULL) ||
	    (cb->read_clock_capabilities == NULL)) {
...
int bt_ets_indicate(const struct bt_ets_elapsed_time *elapsed_time)
{
	...
	if ((ets_cb == NULL) || (ets_cb->read_clock_status == NULL) ||
	    (ets_cb->read_clock_capabilities == NULL)) {
