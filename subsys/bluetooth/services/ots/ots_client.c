			      ...
			      struct bt_conn *conn)
{
	...
	LOG_DBG("ctx: %p, conn: %p, written: %d",
		       ...
		       struct bt_conn *conn, struct net_buf *buf)
{
	...
	LOG_DBG("Incoming L2CAP data, context: %p, conn: %p, len: %u, offset: %u",
	...
	if (cur_inst->rcvd_size > cur_object->size.cur) {
		LOG_WRN("Received %u but expected maximum %u",
...
{
	LOG_DBG("Request OP Code: %s",
	       lit_olcp_request[req_opcode]);
	LOG_DBG("Result Code    : %s",
			       ...
			       const void *data, uint16_t length)
{
	...
	if (err) {
		...
		if (length == OTS_FEATURE_LEN) {
			...
			LOG_DBG("features : oacp 0x%x, olcp 0x%x",
			       inst->otc_inst->features.oacp,
			       inst->otc_inst->features.olcp);
		} else {
			LOG_DBG("Invalid length %u (expected %u)",
		...
		if (length == BT_OTS_OBJ_ID_SIZE) {
			...
			if (cur_object->id != OTS_CLIENT_UNKNOWN_ID &&
				...
				(void)bt_ots_obj_id_to_str(cur_object->id, str,
							   sizeof(str));
				LOG_INF("Read Obj Id %s not selected obj Id %s",
					t, str);
			} else {
				...
				const void *data, uint16_t length)
{
	...
	if (data) {
		if (length <= CONFIG_BT_OTS_OBJ_MAX_NAME_LEN) {
			...
			LOG_WRN("Invalid length %u (expected max %u)",
				      ...
				      const void *data, uint16_t length)
{
	...
	if (err) {
		...
		LOG_INF("Object properties (raw) : 0x%x",
			cur_object->props);
		...
		LOG_WRN("Invalid length %u (expected %u)",
			 ...
			 struct dirlisting_record_t *rec)
{
	...
	if (rec->name_len > 0) {
		...
		if (rec->name_len >= sizeof(rec->metadata.name_c)) {
			LOG_WRN("Name length %u too long, invalid record",
				rec->name_len);
			return -EINVAL;
		}
				    ...
				    bt_ots_client_dirlisting_cb cb)
{
	...
	while (net_buf.len) {
		...
		if (net_buf.len < sizeof(uint16_t)) {
			LOG_WRN("incorrect DirListing record, len %u too short",
				net_buf.len);
			return -EINVAL;
		}
