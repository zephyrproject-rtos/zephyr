...
void bt_bas_bls_set_battery_charge_level(enum bt_bas_bls_battery_charge_level level)
{
	...
	/* Return if CONFIG_BT_BAS_BCS is not enabled. */
	if (!IS_ENABLED(CONFIG_BT_BAS_BCS)) {
		return;
	}

	/*
	 * Set the BCS Critical Power State bit as per BAS 1.1 specification section 3.4.1.1:
	 * The BCS Critical Power State bit should be set to true if the Battery Charge Level
	 * is set to Critical in the Power State field.
	 */
...
void bt_bas_bls_set_service_required(enum bt_bas_bls_service_required value)
{
	...
	/*
	 * Set the BCS Immediate Service Required bit as per BAS 1.1 specification section 3.4.1.1:
	 * The BCS Immediate Service Required bit should be set to true if the service Required bit
	 * is set to true in the Additional Status field.
	 */
