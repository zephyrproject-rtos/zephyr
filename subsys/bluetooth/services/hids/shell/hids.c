...
#define HIDS_SHELL_MOUSE_REPORT_LEN 3U
#define HIDS_SHELL_MAX_VALUE_LEN    8U
...
static const struct hids_shell_report {
	...
	{.id = HIDS_SHELL_REPORT_ID_LED, .type = BT_HIDS_REPORT_TYPE_OUTPUT, .len = 1},
	{.id = HIDS_SHELL_REPORT_ID_FEATURE, .type = BT_HIDS_REPORT_TYPE_FEATURE, .len = 1},
};
...
static void hids_set_report(struct bt_conn *conn, enum bt_hids_report_type report_type,
			    uint8_t report_id, const uint8_t *data, uint16_t len)
{
	ARG_UNUSED(conn);

	/* The service stores the value, so this only reports the write */
	bt_shell_print("HIDS: SET_REPORT type %u id %u: %s", (unsigned int)report_type, report_id,
		       bt_hex(data, len));
}

static void hids_protocol_mode_changed(struct bt_conn *conn, enum bt_hids_protocol_mode protocol)
{
	ARG_UNUSED(conn);

	bt_shell_print("HIDS: Protocol Mode changed to %s",
		       protocol == BT_HIDS_PROTOCOL_BOOT ? "Boot" : "Report");
}
...
static void hids_ccc_changed(struct bt_conn *conn, uint8_t report_id,
			     enum bt_hids_report_type report_type, bool enabled)
{
	ARG_UNUSED(conn);

	bt_shell_print("HIDS: Notifications %s for report type %u id %u",
		       enabled ? "enabled" : "disabled", (unsigned int)report_type, report_id);
}
...
static int cmd_register(const struct shell *sh, size_t argc, char *argv[])
{
	const struct bt_hids_register_param param = {
		...
		.info = {
			.bcd_hid = 0x0111,
			.b_country_code = 0x00,
			.flags = BT_HIDS_INFO_FLAG_REMOTE_WAKE |
				 BT_HIDS_INFO_FLAG_NORMALLY_CONNECTABLE,
		},
		/* clang-format on */
...
{
	enum bt_hids_protocol_mode mode;
	bool suspended;
	int err;
	...
	if (default_conn == NULL) {
		...
		shell_print(sh, "Protocol Mode: %s",
			    mode == BT_HIDS_PROTOCOL_BOOT ? "Boot" : "Report");
