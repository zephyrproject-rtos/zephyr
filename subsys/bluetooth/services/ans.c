					    ...
					    uint8_t flags)
{
	...
	case BT_ANS_CTRL_ENABLE_NEW_ALERT:
		if ((alert_sup_cat_bit_mask & (1U << command.category)) == 0) {
			LOG_DBG("Received control point request for unsupported category: %d",
				command.category);
	...
	case BT_ANS_CTRL_NOTIFY_NEW_ALERT_IMMEDIATE:
		rc = transmit_new_alert(NULL, command.category);
		if (rc != 0) {
	...
	case BT_ANS_CTRL_NOTIFY_UNREAD_IMMEDIATE:
		rc = transmit_unread_alert(NULL, command.category);
		if (rc != 0) {
...
static int notify_new_alert_category(struct bt_conn *conn, uint8_t cat)
{
	...
	if (!bt_gatt_is_subscribed(conn, &ans_svc.attrs[3], BT_GATT_CCC_NOTIFY)) {
		LOG_DBG("Client not subscribed to New Alert category");
		return 0;
	}
...
static int notify_unread_alert_category(struct bt_conn *conn, uint8_t cat)
{
	...
	if (!bt_gatt_is_subscribed(conn, &ans_svc.attrs[3], BT_GATT_CCC_NOTIFY)) {
		LOG_DBG("Client not subscribed to Unread Alert category");
		return 0;
	}
...
static int transmit_unread_alert(struct bt_conn *conn, enum bt_ans_cat category)
{
	...
	/* Nothing to do if notify is disabled */
	if (!bt_gatt_is_subscribed(conn, &ans_svc.attrs[6], BT_GATT_CCC_NOTIFY)) {
		return 0;
	}
			    ...
			    const char *text)
{
	...
	int size = MIN(BT_ANS_MAX_TEXT_STR_SIZE - 1, strlen(text));

	utf8_lcpy(new_alerts[cat_in].text_string, text, size);
	ret = k_mutex_unlock(&new_alert_mutex);
	__ASSERT(ret == 0, "Unable to unlock mutex (err: %d)", ret);
