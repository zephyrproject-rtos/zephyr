...
enum ccp_client_flag {
	CCP_CLIENT_FLAG_BUSY,

	CCP_CLIENT_FLAG_NUM_FLAGS, /* keep as last */
};
...
struct bt_ccp_client {
	...
	ATOMIC_DEFINE(flags, CCP_CLIENT_FLAG_NUM_FLAGS);
};
...
static void populate_bearers(struct bt_ccp_client *client, struct bt_ccp_client_bearers *bearers)
{
	size_t i = 0;
...
{
	struct bt_ccp_client *client = get_client_by_conn(conn);
	struct bt_ccp_client_bearers bearers = {0};
	...
	if (IS_ENABLED(CONFIG_BT_TBS_CLIENT_TBS)) {
		for (uint8_t i = 0U; i < tbs_count; i++) {
			...
			if (idx >= ARRAY_SIZE(client->bearers)) {
				LOG_WRN("Discoverd more TBS instances (%u) than the CCP client "
					"supports %zu",
...
int bt_ccp_client_register_cb(struct bt_ccp_client_cb *cb)
{
	...
	if (sys_slist_find(&ccp_client_cbs, &cb->_node, NULL)) {
		return -EEXIST;
	}

	sys_slist_append(&ccp_client_cbs, &cb->_node);
...
int bt_ccp_client_unregister_cb(struct bt_ccp_client_cb *cb)
{
	...
	if (!sys_slist_find_and_remove(&ccp_client_cbs, &cb->_node)) {
		return -EALREADY;
	}
...
int bt_ccp_client_read_bearer_uci(struct bt_ccp_client_bearer *bearer)
{
	struct bt_ccp_client *client;
	int err;
