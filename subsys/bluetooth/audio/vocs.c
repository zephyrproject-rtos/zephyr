...
LOG_MODULE_REGISTER(bt_vocs, LOG_LEVEL);
				  ...
				  const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (notify) {
		...
		LOG_DBG("New state: offset %d, counter %u",
				 ...
				 const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (len >= sizeof(inst->srv.output_desc)) {
		LOG_DBG("Output desc was clipped from length %u to %zu",
