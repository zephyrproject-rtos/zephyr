...
LOG_MODULE_REGISTER(bt_mcs, LOG_LEVEL);
			     ...
			     uint16_t len, uint16_t offset)
{
	...
	LOG_DBG("Icon URL read, offset: %d, len:%d, URL: %s", offset, len,
	       url);
				...
				void *buf, uint16_t len, uint16_t offset)
{
	...
	LOG_DBG("Track title read, offset: %d, len:%d, title: %s", offset, len,
	       title);
				      ...
				      uint8_t flags)
{
	...
	if (IS_ENABLED(CONFIG_BT_DEBUG_MCS)) {
		...
		LOG_DBG("Current track write: offset: %d, len: %d, track ID: %s",
		       offset, len, str);
	}
				   ...
				   uint8_t flags)
{
	...
	if (IS_ENABLED(CONFIG_BT_DEBUG_MCS)) {
		char str[BT_OTS_OBJ_ID_STR_LEN];
		(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
		LOG_DBG("Next  track write: offset: %d, len: %d, track ID: %s",
		       offset, len, str);
	}
				      ...
				      uint8_t flags)
{
	...
	if (IS_ENABLED(CONFIG_BT_DEBUG_MCS)) {
		char str[BT_OTS_OBJ_ID_STR_LEN];
		(void)bt_ots_obj_id_to_str(id, str, sizeof(str));
		LOG_DBG("Current group ID write: offset: %d, len: %d, track ID: %s",
		       offset, len, str);
	}
...
{
	LOG_DBG("Notifying command opcodes supported: %d (0x%08x)", opcodes,
	       opcodes);
