...
static struct tbs_inst *lookup_inst_by_uri_scheme(const char *uri, uint8_t uri_len)
{
...
static struct bt_tbs_call *call_alloc(struct tbs_inst *inst, uint8_t state, const char *uri,
				      uint16_t uri_len)
{
			   ...
			   const char *friendly_name)
{
	...
	call = call_alloc(inst, BT_TBS_CALL_STATE_INCOMING, from, strlen(from));
	if (call == NULL) {
		return -ENOMEM;
	}
