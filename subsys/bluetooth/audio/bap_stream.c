			 ...
			 struct bt_audio_codec_cfg *codec_cfg)
{
	...
	if (bt_bap_stream_valid_ase_op(stream->conn, ep, BT_ASCS_QOS_OP)) {
	...
	SYS_SLIST_FOR_EACH_CONTAINER(&group->streams, stream, _node) {
		...
		if (bt_bap_stream_valid_ase_op(conn, ep, BT_ASCS_QOS_OP)) {
			LOG_ERR("Invalid state %s for op %s for ep %p",
				bt_bap_ep_state_str(ep->state), bt_ascs_op_str(BT_ASCS_CONFIG_OP),
				ep);
			return -EBADMSG;
		}
	}
...
bool bt_bap_stream_valid_ase_op(const struct bt_conn *conn, const struct bt_bap_ep *ep,
				uint8_t ase_op)
{
	/* TODO: Should we have this function for each BAP role? We need to check the ops per conn
	 * type Having one in ASCS and one in bap_unicast_client would allow us to better
	 * semantically apply the checks, but will distribute the checks more Having a single check
	 * can still be done using IS_ENABLED without any memory overhead
	 */
