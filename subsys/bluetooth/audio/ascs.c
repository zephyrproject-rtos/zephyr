...
static void ascs_cp_rsp_add(const struct bt_conn *conn, uint8_t id, uint8_t code, uint8_t reason)
{
...
static ssize_t ascs_config(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		if (!cfg->ase || cfg->ase > ASE_COUNT) {
			...
			ascs_cp_rsp_add(conn, cfg->ase, BT_BAP_ASCS_RSP_CODE_INVALID_ASE,
					BT_BAP_ASCS_REASON_NONE);
			continue;
		}
		...
		if (!ase) {
			ascs_cp_rsp_add(conn, cfg->ase, BT_BAP_ASCS_RSP_CODE_NO_MEM,
					BT_BAP_ASCS_REASON_NONE);
...
static ssize_t ascs_qos(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		if (!is_valid_ase_id(qos->ase)) {
			ascs_cp_rsp_add(conn, qos->ase, BT_BAP_ASCS_RSP_CODE_INVALID_ASE,
					BT_BAP_ASCS_REASON_NONE);
		...
		if (!ase) {
			...
			ascs_cp_rsp_add(conn, qos->ase, BT_BAP_ASCS_RSP_CODE_INVALID_ASE_STATE,
					BT_BAP_ASCS_REASON_NONE);
			continue;
		}
		...
		ascs_cp_rsp_add(conn, qos->ase, rsp.code, rsp.reason);
	}
...
static ssize_t ascs_enable(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		if (!is_valid_ase_id(meta->ase)) {
			ascs_cp_rsp_add(conn, meta->ase, BT_BAP_ASCS_RSP_CODE_INVALID_ASE,
					BT_BAP_ASCS_REASON_NONE);
		...
		if (!ase) {
			...
			ascs_cp_rsp_add(conn, meta->ase, BT_BAP_ASCS_RSP_CODE_INVALID_ASE_STATE,
					BT_BAP_ASCS_REASON_NONE);
			continue;
		}
...
static ssize_t ascs_metadata(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		if (meta->len > CONFIG_BT_AUDIO_CODEC_CFG_MAX_METADATA_SIZE) {
			...
			ascs_cp_rsp_add(conn, meta->ase, BT_BAP_ASCS_RSP_CODE_NO_MEM,
					BT_BAP_ASCS_REASON_NONE);
			continue;
		}
