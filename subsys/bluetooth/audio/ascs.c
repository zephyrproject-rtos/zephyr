...
static void ascs_cp_rsp_add(const struct bt_conn *conn, uint8_t ase_id, uint8_t code, uint8_t reason)
{
...
static ssize_t ascs_enable(struct bt_conn *conn, struct net_buf_simple *buf)
{
	...
	for (uint8_t i = 0U; i < req->num_ases; i++) {
		...
		if (!ase) {
			LOG_DBG("Invalid operation for idle ase 0x%02x", meta->ase_id);
