...
LOG_MODULE_REGISTER(bt_ascs, LOG_LEVEL);
			       ...
			       uint8_t state)
{
	...
	LOG_DBG("ase %p conn %p", ase, ase->ascs->conn);
...
void ascs_ep_set_state(struct bt_audio_ep *ep, uint8_t state)
{
	...
	LOG_DBG("ep %p id 0x%02x %s -> %s", ep, ep->status.id,
...
static void ascs_iso_disconnected(struct bt_iso_chan *chan, uint8_t reason)
{
	...
	if (ep->status.state == BT_AUDIO_EP_STATE_RELEASING) {
		...
		if (ase == NULL) {
			LOG_WRN("Could find ASE based on ASCS %p with ep %p",
				ascs, ep);
			return;
		}
			    ...
			    uint8_t reason)
{
	...
	LOG_DBG("id 0x%02x op %s (0x%02x) code %s (0x%02x) reason %s (0x%02x)",
						 ...
						 uint8_t cis_id)
{
	...
	LOG_DBG("ascs %p cig_id 0x%02x cis_id 0x%02x", ascs, cis_id, cis_id);
					 ...
					 struct bt_audio_iso *audio_iso)
{
	...
	LOG_DBG("stream %p, dir %u audio_iso %p", stream, dir, audio_iso);

	if (dir == BT_AUDIO_DIR_SOURCE) {
		if (audio_iso->source_stream == NULL) {
			audio_iso->source_stream = stream;
		} else if (audio_iso->source_stream != stream) {
			LOG_WRN("Bound with source_stream %p already", audio_iso->source_stream);
			return -EADDRINUSE;
		}
		...
		if (audio_iso->sink_stream == NULL) {
			...
			LOG_WRN("Bound with sink_stream %p already", audio_iso->sink_stream);
			return -EADDRINUSE;
		}
			     ...
			     uint16_t len, uint16_t offset)
{
	...
	LOG_DBG("conn %p attr %p buf %p len %u offset %u", conn, attr, buf, len,
	       offset);
			     ...
			     uint8_t len, struct bt_codec *codec)
{
	...
	LOG_DBG("ep %p dir %u codec id 0x%02x cid 0x%04x vid 0x%04x len %u",
		      ...
		      struct net_buf_simple *buf)
{
	...
	if (ase->ep.stream != NULL) {
		...
		if (err != 0 || stream == NULL) {
			LOG_ERR("Config failed, err: %d, stream %p",
			       err, stream);
...
static bool ascs_parse_metadata(struct bt_data *data, void *user_data)
{
	...
	if (result->count > CONFIG_BT_CODEC_MAX_METADATA_COUNT) {
		LOG_ERR("Not enough buffers for Codec Config Metadata: %zu > %zu",
			     ...
			     uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	LOG_DBG("conn %p attr %p buf %p len %u op %s (0x%02x)", conn,
