...
static int decode_bis_data(struct net_buf_simple *buf,
			   struct bt_audio_base_bis_data *bis)
{
	uint8_t len;

	if (buf->len < BT_AUDIO_BASE_BIS_DATA_MIN_SIZE) {
	...
	if (len > 0) {
		...
		while (ltv_buf.len != 0) {
			struct bt_codec_data *bis_codec_data = &bis->data[bis->data_count];
			const int err = decode_codec_ltv(&ltv_buf, bis_codec_data);
...
static int decode_subgroup(struct net_buf_simple *buf,
			   struct bt_audio_base_subgroup *subgroup)
{
	...
	while (ltv_buf.len != 0) {
		...
		const int err = decode_codec_ltv(&ltv_buf, codec_data);

		if (err != 0) {
			LOG_DBG("Failed to decode codec config data for entry %u: %d",
				codec->data_count - 1, err);
	...
	while (ltv_buf.len != 0) {
		...
		const int err = decode_codec_ltv(&ltv_buf, metadata);

		if (err != 0) {
			LOG_DBG("Failed to decode codec metadata for entry %u: %d",
				codec->meta_count - 1, err);
