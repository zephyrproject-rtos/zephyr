...
static void do_track_change_notifications(struct mpl_mediaplayer *pl)
{
	...
	bt_mcs_track_title_changed(pl->group->track->title);
	bt_mcs_track_position_changed(pl->group->track->duration);
	bt_mcs_track_duration_changed(pl->track_pos);
#ifdef CONFIG_BT_MPL_OBJECTS
	bt_mcs_current_track_id_changed(pl->group->track->id);
	if (pl->group->track->next) {
		bt_mcs_next_track_id_changed(pl->group->track->next->id);
	} else {
		/* Send a zero value to indicate that there is no next track */
		bt_mcs_next_track_id_changed(MPL_NO_TRACK_ID);
	}
#endif /* CONFIG_BT_MPL_OBJECTS */
}
...
{
#ifdef CONFIG_BT_MPL_OBJECTS
	bt_mcs_current_group_id_changed(pl->group->id);
#endif /* CONFIG_BT_MPL_OBJECTS */
}
...
static void mpl_set_state(uint8_t state)
{
	...
	bt_mcs_media_state_changed(media_player.state);
}
...
static uint8_t playing_state_command_handler(const struct mpl_cmd *command)
{
	...
	case MEDIA_PROXY_OP_FAST_REWIND:
		...
		bt_mcs_seeking_speed_changed(media_player.seeking_speed_factor);
		break;
	case MEDIA_PROXY_OP_FAST_FORWARD:
		/* We're in playing state, seeking speed must have been zero */
		media_player.seeking_speed_factor = MPL_SEEKING_SPEED_FACTOR_STEP;
...
static void set_track_position(int32_t position)
{
	...
	if (new_pos != old_pos || new_pos == 0) {
		...
		if (media_player.state != MEDIA_PROXY_STATE_PLAYING) {
			bt_mcs_track_duration_changed(new_pos);
		}
	}
}
...
static void set_playback_speed(int8_t speed)
{
	...
	if (speed != media_player.playback_speed_param) {
		...
		bt_mcs_playback_speed_changed(media_player.playback_speed_param);
	}
}
...
static void set_next_track_id(uint64_t id)
{
	...
	if (find_track_by_id(&media_player, id, &group, &track)) {
		...
		bt_mcs_next_track_id_changed(id);
		return;
	}
...
static void set_playing_order(uint8_t order)
{
	if (order != media_player.playing_order) {
		if (BIT(order - 1) & media_player.playing_orders_supported) {
			...
			bt_mcs_playing_order_changed(media_player.playing_order);
		}
	}
}
...
static void parse_search(const struct mpl_search *search)
{
	...
	bt_mcs_search_results_id_changed(media_player.search_results_id);
}
...
{
	bt_mcs_player_name_changed(media_player.name);
}

void mpl_test_player_icon_url_changed_cb(void)
{
	bt_mcs_icon_url_changed(media_player.icon_url);
}
...
{
	bt_mcs_track_title_changed(media_player.group->track->title);
}

void mpl_test_duration_changed_cb(void)
{
	bt_mcs_track_position_changed(media_player.group->track->duration);
}

void mpl_test_position_changed_cb(void)
{
	bt_mcs_track_duration_changed(media_player.track_pos);
}
...
{
	bt_mcs_current_track_id_changed(media_player.group->track->id);
}

void mpl_test_next_track_id_changed_cb(void)
{
	bt_mcs_next_track_id_changed(media_player.group->track->next->id);
}

void mpl_test_parent_group_id_changed_cb(void)
{
	bt_mcs_parent_group_id_changed(media_player.group->id);
}

void mpl_test_current_group_id_changed_cb(void)
{
	bt_mcs_current_group_id_changed(media_player.group->id);
}
#endif /* CONFIG_BT_MPL_OBJECTS */
...
{
	bt_mcs_media_state_changed(media_player.playing_order);
}

void mpl_test_opcodes_supported_changed_cb(void)
{
	bt_mcs_commands_supported_changed(media_player.opcodes_supported);
}
