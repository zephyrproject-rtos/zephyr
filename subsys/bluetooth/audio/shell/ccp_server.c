...
static struct bt_ccp_server_bearer *bearers[CONFIG_BT_CCP_SERVER_BEARER_COUNT];

static int cmd_ccp_server_init(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_ccp_server_register_bearer(&gtbs_param, &bearers[0]);
	if (err != 0) {
	...
	for (int i = 1; i < CONFIG_BT_CCP_SERVER_BEARER_COUNT; i++) {
		...
		err = bt_ccp_server_register_bearer(&tbs_param, &bearers[i]);
		if (err != 0) {
...
static int cmd_ccp_server_set_bearer_name(const struct shell *sh, size_t argc, char *argv[])
{
	...
	err = bt_ccp_server_set_bearer_provider_name(bearers[index], name);
	if (err != 0) {
...
static int cmd_ccp_server_get_bearer_name(const struct shell *sh, size_t argc, char *argv[])
{
	unsigned long index = 0;
	const char *name;
	int err = 0;
	...
	err = bt_ccp_server_get_bearer_provider_name(bearers[index], &name);
	if (err != 0) {
...
static int cmd_ccp_server_get_bearer_uci(const struct shell *sh, size_t argc, char *argv[])
{
	unsigned long index = 0;
	const char *uci;
	int err = 0;
	...
	err = bt_ccp_server_get_bearer_uci(bearers[index], &uci);
	if (err != 0) {
...
static int cmd_ccp_server(const struct shell *sh, size_t argc, char **argv)
{
	if (argc > 1) {
...
SHELL_STATIC_SUBCMD_SET_CREATE(ccp_server_cmds,
			       SHELL_CMD_ARG(init, NULL, "Initialize CCP server",
					     cmd_ccp_server_init, 1, 0),
			       SHELL_CMD_ARG(set_bearer_name, NULL,
					     "Set bearer name [index] <name>",
					     cmd_ccp_server_set_bearer_name, 2, 1),
			       SHELL_CMD_ARG(get_bearer_name, NULL, "Get bearer name [index]",
					     cmd_ccp_server_get_bearer_name, 1, 1),
			       SHELL_CMD_ARG(get_bearer_uci, NULL, "Get bearer UCI [index]",
					     cmd_ccp_server_get_bearer_uci, 1, 1),
			       SHELL_SUBCMD_SET_END);

SHELL_CMD_ARG_REGISTER(ccp_server, &ccp_server_cmds, "Bluetooth CCP server shell commands",
		       cmd_ccp_server, 1, 1);
