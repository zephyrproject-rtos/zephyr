...
static bool unicast_group_foreach_stream_cb(struct bt_cap_stream *cap_stream, void *user_data)
{
	...
	if (bap_stream->ep != NULL) {
		...
		if (broadcast_assistant_recv_states[bt_conn_index(bap_stream->conn)]
			    .recv_state_count == 0U) {
			return false; /* Stop iterating as it does not have a (discovered) BASS */
		}
		...
		if (ep_info.state == BT_BAP_EP_STATE_STREAMING &&
			...
			__ASSERT_NO_MSG(data->active_sink_streams_cnt <
					ARRAY_SIZE(data->active_sink_streams));
...
static int cmd_cap_handover_unicast_to_broadcast(const struct shell *sh, size_t argc, char *argv[])
{
	...
	if (lookup_data.active_sink_streams_cnt > MAX_CAP_BROADCAST_STREAMS) {
		shell_error(sh, "Too many active sink streams (%zu > %d), cannot handover",
			    lookup_data.active_sink_streams_cnt, MAX_CAP_BROADCAST_STREAMS);
...
static void populate_active_stream_conns(struct bt_conn *conn, void *data)
{
	...
	/* Store conn in next empty index */
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(broadcast_to_unicast, NULL,
		      ...
		      "deleted) [conns <count>|all] [preset <preset_name>]",
