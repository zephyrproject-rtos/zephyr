...
static inline void codec_data_set_chan_alloc(struct bt_codec_data *data, enum bt_audio_location loc)
{
	const uint32_t loc_32 = loc;

	data->data.type = BT_CODEC_CONFIG_LC3_CHAN_ALLOC;
	data->data.data_len = sizeof(loc_32);
	sys_put_le32(loc_32, data->value);
}

static inline int codec_set_chan_alloc(struct bt_codec *codec, enum bt_audio_location loc)
{
	for (size_t i = 0U; i < codec->data_count; i++) {
		struct bt_codec_data *data = &codec->data[i];
	...
	if (codec->data_count < CONFIG_BT_CODEC_MAX_DATA_COUNT) {
		struct bt_codec_data *data = &codec->data[codec->data_count++];
