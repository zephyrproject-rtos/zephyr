				      ...
				      const struct bt_audio_codec_cfg *codec_cfg)
{
	...
	/* Check the specialized `codec_cap` for GMAP first, before the generic BAP one */
	 ...
	 */
	if ((BIT(frame_dur) & supported_frame_dur) == 0) {
		bt_shell_warn("Unsupported frame duration: 0x%02X (0x%02X)", (uint8_t)frame_dur,
