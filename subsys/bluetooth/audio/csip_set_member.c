				...
				const struct bt_csip_set_member_register_param *param)
{
	...
	inst->attrs[attr_cnt].user_data =
		(void *)&set_sirk_audio_user_data[ARRAY_INDEX(svc_insts, inst)];
	attr_cnt++;
	...
	if (IS_ENABLED(CONFIG_BT_CSIP_SET_MEMBER_SIRK_NOTIFIABLE)) {
		...
		inst->attrs[attr_cnt].user_data =
			(void *)&set_sirk_audio_ccc_user_data[ARRAY_INDEX(svc_insts, inst)];
		attr_cnt++;
	}
	...
	if (param->set_size > 0U) {
		...
		inst->attrs[attr_cnt].read = bt_audio_read_chrc;
		inst->attrs[attr_cnt].user_data =
			(void *)&set_size_audio_user_data[ARRAY_INDEX(svc_insts, inst)];
		attr_cnt++;
		...
		if (IS_ENABLED(CONFIG_BT_CSIP_SET_MEMBER_SIZE_NOTIFIABLE)) {
			...
			inst->attrs[attr_cnt].write = bt_gatt_attr_write_ccc;
			inst->attrs[attr_cnt].user_data =
				(void *)&set_size_audio_ccc_user_data[ARRAY_INDEX(svc_insts, inst)];
			attr_cnt++;
		}
	}
	if (param->lockable) {
		...
		inst->attrs[attr_cnt].user_data =
			(void *)&set_lock_audio_user_data[ARRAY_INDEX(svc_insts, inst)];
		attr_cnt++;
		...
		inst->attrs[attr_cnt].write = bt_gatt_attr_write_ccc;
		inst->attrs[attr_cnt].user_data =
			(void *)&set_lock_audio_ccc_user_data[ARRAY_INDEX(svc_insts, inst)];
		attr_cnt++;
	}
	...
	if (param->rank > 0U) {
		...
		inst->attrs[attr_cnt].read = bt_audio_read_chrc;
		inst->attrs[attr_cnt].user_data =
			(void *)&rank_audio_user_data[ARRAY_INDEX(svc_insts, inst)];
		attr_cnt++;
	}
