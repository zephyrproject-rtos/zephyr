...
static bool valid_register_param(const struct bt_csip_set_member_register_param *param)
{
	...
	if (!IS_ENABLED(CONFIG_BT_CSIP_SET_MEMBER_LOCK_SUPPORT) && param->lockable) {
		LOG_DBG("Lock cannot be enabled if set lock support is not enabled");
		return false;
	}

	if (!IS_ENABLED(CONFIG_BT_CSIP_SET_MEMBER_RANK_SUPPORT) && param->rank > 0U) {
		LOG_DBG("Rank cannot be greater than 0 if rank support is not enabled");
		return false;
	}
					 ...
					 uint8_t rank)
{
	...
	if (!IS_ENABLED(CONFIG_BT_CSIP_SET_MEMBER_RANK_SUPPORT)) {
		LOG_DBG("rank support is not enabled");
		return -ENOTSUP;
	}
...
#elif
	if (rank != 0U) {
		LOG_DBG("Invalid rank %u for service", rank);
		return -EINVAL;
	}

	if (!IN_RANGE(rank, 1U, size)) {
		LOG_DBG("Invalid rank: %u for size %u", rank, size);
		return -EINVAL;
	}
#endif
