				...
				enum bt_security_err security_err)
{
	...
	const bool bonded = bt_le_bond_exists(conn->id, addr);

	LOG_DBG("%s security changed err %d level %d (%sbonded)",
		bt_addr_le_str(bt_conn_get_dst(conn)), security_err, level, bonded ? "" : "not ");
...
static void disconnected_cb(struct bt_conn *conn, uint8_t reason)
{
	...
	LOG_DBG("Disconnected: %s (reason %u)", bt_addr_le_str(addr), reason);
