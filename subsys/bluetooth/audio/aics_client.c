...
#ifdef CONFIG_BT_DEBUG_AICS_CLIENT
...
LOG_MODULE_REGISTER(bt_aics_client, LOG_LEVEL);
				   ...
				   const void *data, uint16_t length)
{
	...
	if (handle == inst->cli.state_handle) {
		if (length == sizeof(*state)) {
			...
			LOG_DBG("Inst %p: Gain %d, mute %u, gain_mode %u, counter %u",
					 ...
					 const void *data, uint16_t length)
{
	...
	if (data) {
		if (length == sizeof(*state)) {
			LOG_DBG("Gain %d, mute %u, gain_mode %u, counter %u",
					...
					const void *data, uint16_t length)
{
	...
	if (data) {
		LOG_HEXDUMP_DBG((const uint8_t *) data, length, "Input description read");
