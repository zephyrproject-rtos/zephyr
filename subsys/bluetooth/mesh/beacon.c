...
static bool secure_beacon_send(struct bt_mesh_subnet *sub, void *cb_data)
{
	...
	adv = bt_mesh_adv_main_create(BT_MESH_ADV_BEACON,
				      PROV_XMIT, K_NO_WAIT);
	if (!adv) {
	...
	bt_mesh_beacon_create(sub, &adv->b);

	bt_mesh_adv_send(adv, &send_cb, sub);
...
static int unprovisioned_beacon_send(void)
{
	...
	if (prov->uri) {
		...
		adv = bt_mesh_adv_main_create(BT_MESH_ADV_URI,
					      UNPROV_XMIT, K_NO_WAIT);
		if (!adv) {
