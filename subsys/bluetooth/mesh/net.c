...
static bool msg_cache_match(struct net_buf_simple *pdu, struct bt_mesh_net_rx *rx)
{
	uint16_t i;
	...
	for (i = msg_cache_next; i > 0U;) {
		if (msg_cache[--i].src == SRC(pdu->data) &&
		    ...
		    msg_cache[i].net_idx == rx->sub->net_idx) {
			return true;
		}
	}
			...
			const struct bt_mesh_net_cred *cred)
{
	...
	if (rx->net_if == BT_MESH_NET_IF_ADV && msg_cache_match(out, rx)) {
