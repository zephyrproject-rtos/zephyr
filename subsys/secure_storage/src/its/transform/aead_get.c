...
#endif /* CONFIG_SECURE_STORAGE_ITS_TRANSFORM_AEAD_SCHEME_AES_GCM */
...
static psa_status_t hash_data_into_key(size_t data_len, const void *data, uint8_t key[
		...
		uint8_t key[static CONFIG_SECURE_STORAGE_ITS_TRANSFORM_AEAD_KEY_SIZE])
{
	...
	if (hwinfo_ret != 0) {
		...
		if (hwinfo_ret < sizeof(data.device_id)) {
			bzero(data.device_id + hwinfo_ret, sizeof(data.device_id) - hwinfo_ret);
		}
	}
	data.uid = uid;
