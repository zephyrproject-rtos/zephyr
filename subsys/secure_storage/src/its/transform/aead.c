...
BUILD_ASSERT(CONFIG_SECURE_STORAGE_ITS_TRANSFORM_OUTPUT_OVERHEAD
	     == CIPHERTEXT_SIZE - CONFIG_SECURE_STORAGE_ITS_MAX_DATA_SIZE
...
BUILD_ASSERT(sizeof(struct stored_entry) == SECURE_STORAGE_ITS_TRANSFORM_OUTPUT_SIZE);
...
psa_status_t secure_storage_its_transform_to_store(
			...
			psa_storage_create_flags_t create_flags,
			uint8_t stored_data[static SECURE_STORAGE_ITS_TRANSFORM_OUTPUT_SIZE],
...
psa_status_t secure_storage_its_transform_from_store(
			...
			const uint8_t stored_data[static SECURE_STORAGE_ITS_TRANSFORM_OUTPUT_SIZE],
