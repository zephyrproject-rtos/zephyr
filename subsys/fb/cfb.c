				...
				bool draw_bg)
{
	...
	for (size_t g_x = 0; g_x < fptr->width; g_x++) {
		...
		for (size_t g_y = 0; g_y < fptr->height;) {
			...
			if (offset == 0 || g_y == 0) {
				/*
				 * The case of drawing the first byte of the glyph or
				 * starting to draw with a tile-aligned position case.
				 * In this case, no glyph is above it. Use 0 instead of it.
				 */
				byte = 0;
			...
			/*
			 * `g_y == 0` case is to render the first byte of the glyph.
			 * In this case, renders to align with the tile boundary.
			 * Onward for second bytes, render begins at the tile border.
			 * We need not care to mask for offset.
			 */
			if (font_is_msbfirst) {
				byte = ((byte << 8) | next_byte) >> (offset);
				bg_mask = (g_y ? 0xFF : LSB_BIT_MASK(8 - offset));
			} else {
				byte = ((next_byte << 8) | byte) >> (8 - offset);
				bg_mask = (g_y ? 0xFF : MSB_BIT_MASK(8 - offset));
			}
