...
static int sdhc_spi_init(struct device *dev)
{
	...
	data->cs = device_get_binding(
		DT_SPI_DEV_CS_GPIO_LABEL(DT_INST(0, zephyr_mmc_spi_slot)));
	__ASSERT_NO_MSG(data->cs != NULL);

	data->pin = DT_SPI_DEV_CS_GPIO_PIN(DT_INST(0, zephyr_mmc_spi_slot));
	data->flags = DT_SPI_DEV_CS_GPIO_FLAGS(DT_INST(0, zephyr_mmc_spi_slot));
