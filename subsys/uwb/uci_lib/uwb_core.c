...
{
	if (UWB_STATUS_CODE_SUCCESS != uwb_tml_write((uint8_t *)frame, frame_len)) {
		return -1;
	}
	return 0;
}
...
static void uwb_uci_rx_handler(const uint8_t *const packet, const uint32_t packet_len)
{
	...
	if ((header->mt == UCI_MT_NTF) && (header->gid == UCI_GID_CORE) &&
		...
		if (UCI_STATUS_CODE_UCI_MESSAGE_RETRY == packet[UCI_HEADER_SIZE]) {
...
int uwb_uci_init(void)
{
	...
	/* Not initialized */
	if (UWB_STATUS_CODE_SUCCESS != k_mutex_init(&g_uci_context.mutex)) {
		return -1;
	}
		       ...
		       uint8_t mt_rsp, uint8_t gid_rsp, uint8_t oid_rsp, bool wait_for_response)
{
	...
	uint32_t backoff_value = CONFIG_UCI_RESPONSE_TIMEOUT_MS;
	...
	while (retry_count < UCI_MAX_RETRIES) {
		...
		do {
			...
			if (current_payload_len > (CONFIG_UCI_FRAME_LEN - UCI_HEADER_SIZE)) {
		...
		do {
			const k_timeout_t response_timeout = Z_TIMEOUT_MS(backoff_value);

			uwb_status_code_t sem_ret =
				k_sem_take(&g_uci_context.response_semaphore, response_timeout);

			if (sem_ret != UWB_STATUS_CODE_SUCCESS) {
				/* Timeout or no response, retry entire packet */
				LOG_ERR("Timed out waiting for response");
				retry_count++;
				/* Backoff for longer time */
				backoff_value += retry_count * 10;
				ret = -1;
				break;
			}
			/* Semaphore was produced - parse and take next actions */
			uci_control_packet_header_t *header =
				(uci_control_packet_header_t *)rsp_ptr;

			if ((header->mt == UCI_MT_NTF) && (header->gid == UCI_GID_CORE) &&
			    (header->oid == UCI_MSG_CORE_GENERIC_ERROR_NTF)) {
				if (UCI_STATUS_CODE_UCI_MESSAGE_RETRY == rsp_ptr[UCI_HEADER_SIZE]) {
					/* Received retry */
					LOG_DBG("Retry received");
					retry_count++;
					/* Reset backoff timeout */
					backoff_value = CONFIG_UCI_RESPONSE_TIMEOUT_MS;
			...
			if (p_waiting_response->response_len) {
				*(p_waiting_response->response_len) = response_len_dup - offset;
			}
			k_mutex_unlock(&g_uci_context.mutex);
		} while (pbf == 1);
		if (ret == 0) {
		...
		*response_len = response_len_dup;
		...
		while (credit_ntf_retry_count < UCI_MAX_RETRIES) {
			if (k_sem_take(&g_uci_context.response_semaphore, credit_ntf_timeout) ==
				...
				if ((header->mt == UCI_MT_NTF) && (header->gid == UCI_GID_CORE) &&
				    (header->oid == UCI_MSG_CORE_GENERIC_ERROR_NTF)) {
					if (UCI_STATUS_CODE_UCI_MESSAGE_RETRY ==
