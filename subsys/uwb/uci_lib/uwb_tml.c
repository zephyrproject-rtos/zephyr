...
K_THREAD_STACK_DEFINE(TMLREAD_stack, CONFIG_UCI_READER_STACK_SIZE);

#if DT_HAS_CHOSEN(zephyr_uwb_uci)
#define UWB_UCI_NODE DT_CHOSEN(zephyr_uwb_uci)
#define UWB_UCI_DEV  DEVICE_DT_GET(UWB_UCI_NODE)
#define UWB_UCI_BUS  UWB_DT_UCI_BUS_GET(UWB_UCI_NODE)
#define UWB_UCI_NAME UWB_DT_UCI_NAME_GET(UWB_UCI_NODE)
#else
#error "zephyr,uwb-uci chosen property is mandatory"
#endif
...
uwb_status_code_t uwb_tml_init(void)
{
	...
	if (guwb_tml_context.is_initialized) {
		...
		if (0 != uwb_uci_open(UWB_UCI_DEV)) {
			return UWB_TML_STATUS_CODE_TRANSPORT_FAILED;
		}
	...
	if (k_mutex_init(&guwb_tml_context.read_mutex) != UWB_STATUS_CODE_SUCCESS) {
		LOG_ERR("Failed to create read mutex");
		return UWB_TML_STATUS_CODE_TML_INIT_FAILED;
	}

	/* Open the device file to which data is read/written */
	int ret = uwb_uci_open(UWB_UCI_DEV);

	if (ret != 0) {
		status = UWB_TML_STATUS_CODE_TRANSPORT_FAILED;
	} else {
 ...
 *
 * Returns          UWB status:
 *                  UWB_STATUS_CODE_SUCCESS - threads initialized successfully
 *                  UWB_STATUS_CODE_FAILED - initialization failed due to system error
 *
 */
static uwb_status_code_t uwb_tml_start_thread(void)
{
	...
	guwb_tml_context.reader_thread = k_thread_create(
		...
		NULL, NULL, NULL, K_PRIO_COOP(CONFIG_UCI_READER_THREAD_PRIO), 0, K_NO_WAIT);
	if (!guwb_tml_context.reader_thread) {
...
static void uwb_tml_reader_thread(void *p_param)
{
	...
	while (guwb_tml_context.b_thread_done) {
		...
		if (1 == read_enabled) {
			...
			no_bytes_wr_rd =
				uwb_uci_recv(UWB_UCI_DEV, pBuffer, CONFIG_UCI_MESSAGE_LEN_MAX);
			...
			if (no_bytes_wr_rd > CONFIG_UCI_MESSAGE_LEN_MAX) {
				...
				LOG_DBG("%s : Read IRQ Timedout, re-read the packet", __func__);
...
uwb_status_code_t uwb_tml_write(uint8_t *p_data, uint16_t data_len)
{
	...
	if (!guwb_tml_context.is_initialized) {
		...
		return UWB_STATUS_CODE_NOT_INITIALIZED;
	}

	uwb_status_code_t wStatus = UWB_STATUS_CODE_SUCCESS;
	int32_t no_bytes_wr_rd = 0;
	...
	no_bytes_wr_rd = uwb_uci_send(UWB_UCI_DEV, p_data, data_len);
	if (no_bytes_wr_rd == -EBUSY) {
		wStatus = UWB_TML_STATUS_CODE_TML_BUSY;
	} else if (no_bytes_wr_rd < 0) {
		LOG_ERR("Command error in write");
		wStatus = UWB_STATUS_CODE_FAILED;
	} else {
