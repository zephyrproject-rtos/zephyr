...
{
	if (p == NULL || p_rcv_data_pkt == NULL || len == 0) {
		LOG_ERR("Invalid parameters: p or p_rcv_data_pkt or len is NULL");
		return;
	}
	uint32_t offset = 0;
	...
	if (offset + p_rcv_data_pkt->data_size <= len) {
...
void uwb_print_data_rcv_ntf(const uwb_data_receive_notification_t *p_rcv_data_pkt)
{
	if (p_rcv_data_pkt != NULL) {
		...
		if (p_rcv_data_pkt->status == UCI_STATUS_CODE_OK) {
			LOG_INF("data_size                             : %d",
