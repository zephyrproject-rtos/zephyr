		  ...
		  unsigned char *sig, size_t sig_size)
{
	...
	status = psa_sign_message(key_id, alg, builder->base, builder->buf - builder->base, sig,
				  sig_size, &sig_len_out);
		    ...
		    const unsigned char *sig, size_t sig_size)
{
	...
	status = psa_verify_message(key_id, alg, builder->base, builder->buf - builder->base, sig,
				    sig_size);
	ret = (status == PSA_SUCCESS) ? 0 : -EINVAL;
