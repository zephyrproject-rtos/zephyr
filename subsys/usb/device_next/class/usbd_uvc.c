...
static const struct uvc_guid_quirk uvc_guid_quirks[] = {
	...
	{VIDEO_PIX_FMT_GREY, UVC_FORMAT_GUID("Y800")},
};
...
static int uvc_add_int(struct net_buf *buf, uint16_t size, int64_t int64, bool is_sig)
{
	if (buf->size < size) {
		LOG_WRN("wLength %u too small for control size %u, casting the value to fit",
			buf->size, size);
		size = buf->size;
	}
	...
	switch (size) {
	case 1:
		net_buf_add_u8(buf, is_sig ? (int8_t)int64 : (uint8_t)int64);
		return 0;
	case 2:
		net_buf_add_le16(buf, is_sig ? (int16_t)int64 : (uint16_t)int64);
		return 0;
	case 4:
		net_buf_add_le32(buf, is_sig ? (int32_t)int64 : (uint32_t)int64);
		return 0;
	case 8:
		net_buf_add_le64(buf, is_sig ? (int64_t)int64 : (uint64_t)int64);
		return 0;
	default:
...
static int uvc_get_int(const struct net_buf *buf, uint16_t size, int64_t *int64, bool is_sig)
{
	uint8_t data[sizeof(uint64_t)] = {0};

	if (buf->len != size) {
		LOG_ERR("invalid wLength %u for SET request, expected %u", buf->len, size);
		return -EINVAL;
	}

	/* Do not cast buf->data directly in case it is not aligned */
	memcpy(data, buf->data, MIN(sizeof(data), size));

	switch (size) {
	case 1:
		*int64 = is_sig ? *(int8_t *)data : *(uint8_t *)data;
		return 0;
	case 2:
		*int64 = is_sig ? (int16_t)sys_le16_to_cpu(*(uint16_t *)data)
				: (uint16_t)sys_le16_to_cpu(*(uint16_t *)data);
		return 0;
	case 4:
		*int64 = is_sig ? (int32_t)sys_le32_to_cpu(*(uint32_t *)data)
				: (uint32_t)sys_le32_to_cpu(*(uint32_t *)data);
		return 0;
	case 8:
		*int64 = is_sig ? (int64_t)sys_le64_to_cpu(*(uint64_t *)data)
				: (uint64_t)sys_le64_to_cpu(*(uint64_t *)data);
		return 0;
	default:
...
static int uvc_map_vc_int(bool is_get, int64_t *int64, const int *map, size_t map_sz)
{
	if (*int64 < 0) {
		return -ERANGE;
	}
	...
	LOG_ERR("Value %lli out of range [0, %zu] for this control type", *int64, map_sz);
	return -ERANGE;
}

static int uvc_convert_vc_int(bool is_get, int64_t *int64, uint32_t cid)
{
	...
	case VIDEO_CID_EXPOSURE_AUTO:
		return uvc_map_vc_int(is_get, int64, ct_ae_mode, ARRAY_SIZE(ct_ae_mode));
	default:
		return 0;
	}
}

static int uvc_add_vc_int(struct net_buf *buf, const struct uvc_control_map *map, int64_t int64)
{
	int ret;
	...
	return uvc_add_int(buf, map->size, int64, map->type == UVC_CONTROL_SIGNED);
}

static int uvc_get_vc_int(const struct net_buf *buf, const struct uvc_control_map *map,
			  int64_t *int64)
{
	bool is_sig = map->type == UVC_CONTROL_SIGNED;
	int ret;

	ret = uvc_get_int(buf, map->size, int64, is_sig);
	if (ret != 0) {
		return ret;
	}

	LOG_DBG("Received %s %u-bit value %lld, converting and applying",
		is_sig ? "signed" : "unsigned", map->size * 8, *int64);

	return uvc_convert_vc_int(false, int64, map->cid);
}
...
static int uvc_get_vs_probe(const struct device *dev, struct net_buf *buf, uint8_t request)
{
	...
	switch (request) {
	case UVC_GET_INFO:
		return uvc_add_int(buf, 1, UVC_INFO_SUPPORTS_GET | UVC_INFO_SUPPORTS_SET, false);
	case UVC_GET_LEN:
		return uvc_add_int(buf, 2, sizeof(struct uvc_probe), false);
	case UVC_GET_DEF:
...
static int uvc_get_vc_ctrl(const struct device *dev, struct net_buf *buf,
			   const struct uvc_control_map *map, uint8_t request)
{
	struct uvc_data *data = dev->data;
	const struct device *video_dev = data->video_dev;
	struct video_ctrl_query cq = {.id = map->cid};
	...
	ret = video_query_ctrl(video_dev, &cq);
	if (ret != 0) {
	...
	is64 = (cq.type == VIDEO_CTRL_TYPE_INTEGER64);

	switch (request) {
	case UVC_GET_MIN:
		return uvc_add_vc_int(buf, map, is64 ? cq.range.min64 : cq.range.min);
	case UVC_GET_MAX:
		return uvc_add_vc_int(buf, map, is64 ? cq.range.max64 : cq.range.max);
	case UVC_GET_RES:
		return uvc_add_vc_int(buf, map, is64 ? cq.range.step64 : cq.range.step);
	case UVC_GET_DEF:
		return uvc_add_vc_int(buf, map, is64 ? cq.range.def64 : cq.range.def);
	case UVC_GET_CUR:
		...
		return uvc_add_vc_int(buf, map, is64 ? ctrl.val64 : ctrl.val);
	case UVC_GET_INFO:
		return uvc_add_int(buf, 1, UVC_INFO_SUPPORTS_GET | UVC_INFO_SUPPORTS_SET, false);
	case UVC_GET_LEN:
		return uvc_add_int(buf, buf->size, map->size, false);
	default:
...
static int uvc_set_vc_ctrl(const struct device *dev, const struct net_buf *buf,
			   const struct uvc_control_map *map)
{
	...
	ret = video_query_ctrl(video_dev, &cq);
	if (ret != 0) {
		LOG_ERR("Failed to query the video device for control 0x%x", cq.id);
		return ret;
	}
	...
	ret = uvc_get_vc_int(buf, map, &int64);
	if (ret != 0) {
		return ret;
	}
	...
	LOG_DBG("Setting CID 0x08%x to %llu", map->cid, int64);
...
static int uvc_get_errno(const struct device *dev, struct net_buf *buf, uint8_t request)
{
	...
	switch (request) {
	case UVC_GET_INFO:
		return uvc_add_int(buf, 1, UVC_INFO_SUPPORTS_GET, false);
	case UVC_GET_CUR:
		return uvc_add_int(buf, 1, data->err, false);
	default:
		LOG_WRN("Unsupported request type %u", request);
		return -ENOTSUP;
	}
			       ...
			       struct net_buf *const buf)
{
	...
	case UVC_OP_VS_COMMIT:
		...
		break;
	case UVC_OP_VC_CTRL:
		errno = -uvc_get_vc_ctrl(dev, buf, map, setup->bRequest);
		break;
...
static void uvc_set_vs_bitrate_range(struct uvc_frame_discrete_descriptor *desc,
				     uint64_t frmival_nsec, struct video_format *fmt)
{
...
static uint32_t uvc_get_mask(const struct device *video_dev, const struct uvc_control_map *list)
{
	...
	for (int i = 0; list[i].size != 0; i++) {
		struct video_ctrl_query cq = {.id = list[i].cid};

		ok = video_query_ctrl(video_dev, &cq) == 0;
...
static int uvc_enqueue(const struct device *dev, struct video_buffer *vbuf)
{
	struct uvc_data *data = dev->data;
...
static int uvc_dequeue(const struct device *dev, struct video_buffer **vbuf, k_timeout_t timeout)
{
	struct uvc_data *data = dev->data;
...
static int uvc_get_format(const struct device *dev, struct video_format *fmt)
{
	struct uvc_data *data = dev->data;
...
static int uvc_set_stream(const struct device *dev, bool enable, enum video_buf_type type)
{
	struct uvc_data *data = dev->data;
...
#ifdef CONFIG_POLL
static int uvc_set_signal(const struct device *dev, struct k_poll_signal *sig)
{
	struct uvc_data *data = dev->data;
