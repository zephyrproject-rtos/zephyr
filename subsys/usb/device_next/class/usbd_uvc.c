			  ...
			  struct net_buf *buf)
{
	...
	case GET_DEF:
		return net_buf_add_mem(buf, &strm->default_probe, size);
	case GET_MIN:
	case GET_MAX:
	case GET_CUR:
			      ...
			      const struct video_format_cap *cap, bool min)
{
	...
	desc->frame_discrete.dwMaxVideoFrameBufferSize =
		sys_cpu_to_le32(w * h * video_pix_fmt_bpp(cap->pixelformat));
	...
	for (int i = 0; video_enum_frmival(strm->video_dev, VIDEO_EP_OUT, &fie) == 0; i++) {
		...
		br = MAX(fmt.pitch, fmt.width) * fmt.height *
		     fie.discrete.denominator / fie.discrete.numerator;
		min_bitrate = MIN(min_bitrate, br);
		max_bitrate = MAX(max_bitrate, br);
	}
