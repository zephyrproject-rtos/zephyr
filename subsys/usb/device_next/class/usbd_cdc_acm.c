...
static struct net_buf *cdc_acm_rx_buf_alloc(struct net_buf_pool *pool,
					    struct usbd_class_data *const c_data, const uint8_t ep)
{
	ARG_UNUSED(c_data);
...
static void cdc_acm_rx_fifo_handler(struct k_work *work)
{
	...
	while (buf = cdc_acm_rx_buf_alloc(data->rx_fifo.pool, c_data, cdc_acm_get_bulk_out(c_data)),
	       buf != NULL) {
