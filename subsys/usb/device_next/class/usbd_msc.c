...
static struct net_buf *msc_buf_alloc_mps(const uint8_t ep, struct usbd_class_data *const c_data)
{
	...
	if (USBD_SUPPORTS_HIGH_SPEED &&
		...
		net_buf_reserve(buf, buf->len - 64U);
	}
...
static struct net_buf *msc_buf_alloc_data(const uint8_t ep, struct msc_bot_ctx *ctx, size_t max_len)
{
	struct net_buf *buf = NULL;
	struct udc_buf_info *bi;

	max_len = MIN(max_len, CONFIG_USBD_MSC_SCSI_BUFFER_SIZE);
	buf = net_buf_alloc_with_data(&msc_ep_pool, ctx->scsi_buf, max_len, K_NO_WAIT);
	if (buf == NULL) {
		return NULL;
	}

	/* For bulk out, the buf needs to be reset.
	 * For bulk in, buf is reset too, so the net_buf_add needs to be called to add data.
	 */
	net_buf_reset(buf);
	bi = udc_get_buf_info(buf);
	bi->ep = ep;
...
static void msc_queue_bulk_out_ep(struct usbd_class_data *const c_data, bool cbw)
{
	...
	if (!cbw) {
		buf = msc_buf_alloc_data(ep, ctx, scsi_cmd_remaining_data_len(lun));
	} else {
		buf = msc_buf_alloc_mps(ep, c_data);
	}
...
{
	if (ctx->scsi_bytes - ctx->scsi_offset != 0) {
		return ctx->scsi_bytes - ctx->scsi_offset;
	}

	return scsi_cmd_remaining_data_len(lun);
}
...
static void msc_process_read(struct msc_bot_ctx *ctx)
{
	...
	buf = msc_buf_alloc_data(ep, ctx, msc_bulk_in_data_len(ctx, lun));
	/* The pool is large enough to support all allocations. Failing alloc
	 * indicates either a memory leak or logic error.
	 */
	__ASSERT_NO_MSG(buf);
	...
	if (ctx->scsi_bytes - ctx->scsi_offset == 0) {
		...
		bytes_queued = ctx->scsi_bytes - ctx->scsi_offset;
		net_buf_add(buf, bytes_queued);
			      ...
			      uint8_t *buf, size_t len)
{
	...
	if ((len > 0) && (scsi_cmd_remaining_data_len(lun) > 0)) {
		...
		__ASSERT(tmp <= len, "Processed less data than requested");
		if (tmp == 0) {
...
static void msc_send_csw(struct msc_bot_ctx *ctx)
{
	...
	ep = msc_get_bulk_in(ctx->class_node);
	buf = msc_buf_alloc_mps(ep, ctx->class_node);
	/* The pool is large enough to support all allocations. Failing alloc
	 * indicates either a memory leak or logic error.
	 */
	__ASSERT_NO_MSG(buf);
