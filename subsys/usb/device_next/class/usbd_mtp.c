				    ...
				    struct net_buf *const buf)
{
	...
	LOG_DBG("%s: Class request 0x%x (Recipient: %x)", __func__, setup->bRequest,
...
static int usbd_mtp_request_handler(struct usbd_class_data *c_data, struct net_buf *buf, int err)
{
	...
	if (bi->ep == mtp_get_bulk_out(c_data)) {
		...
		if (buf_resp == NULL) {
			LOG_ERR("%s: Buffer allocation failed!, REF COUNT %u", __func__,
				buf_resp->ref);
			return -1;
		}
		...
		if (mtp_packet_pending(ctx)) {
			...
			if (!(buf_resp->len % mtp_get_bulk_in_mps(c_data))) {
				udc_ep_buf_set_zlp(buf_resp);
			}
...
static int usbd_mtp_init(struct usbd_class_data *c_data)
{
	...
	mtp_ctx->max_packet_size = mtp_get_bulk_in_mps(c_data);
