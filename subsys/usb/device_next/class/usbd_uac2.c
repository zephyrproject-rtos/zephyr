...
static void uac2_disable(struct usbd_class_data *const c_data)
{
	...
	struct uac2_ctx *ctx = dev->data;
	const struct uac2_cfg *cfg = dev->config;
	const bool microframes = usbd_bus_speed(c_data->uds_ctx) == USBD_SPEED_HS;
	atomic_val_t as_active;
	unsigned int as_idx;

	as_active = atomic_clear(&ctx->as_active);

	while ((as_idx = find_lsb_set(as_active) - 1) != UINT_MAX) {
		ctx->ops->terminal_update_cb(dev, cfg->as_terminals[as_idx], /* alternate */ 0,
					     microframes, ctx->user_data);
