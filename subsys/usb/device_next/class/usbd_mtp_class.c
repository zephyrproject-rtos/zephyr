...
#define MTP_DEFAULT_MAGIC         0xFFFFFFFFU
#define MTP_ROOT_OBJ_HANDLE       MTP_DEFAULT_MAGIC
#define MTP_ASSOSCIATION_SIZE     MTP_DEFAULT_MAGIC
#define MTP_FREE_SPACE_OBJ_UNUSED MTP_DEFAULT_MAGIC
...
static inline void mark_op_complete(struct mtp_context *ctx, uint16_t err_code)
{
	...
	if (err_code != MTP_RESP_OK) {
		LOG_WRN("Operation %s completed with code: 0x%X (%s)",
			mtp_code_to_string(ctx->op_state.code), err_code,
			mtp_code_to_string(err_code));
	}
}
...
static void data_header_push(struct net_buf *buf, struct mtp_context *ctx, uint32_t data_len)
{
	/* DATA Block Header */
	struct mtp_header hdr;
...
MTP_CMD_HANDLER(MTP_OP_GET_DEVICE_INFO)
{
	...
	data_header_push(buf, ctx, buf->len);
...
MTP_CMD_HANDLER(MTP_OP_GET_OBJECT_HANDLES)
{
	...
	uint32_t parent_id =
		(obj_handle.value == MTP_ROOT_OBJ_HANDLE ? 0xff : obj_handle.object_id);

	if (obj_format_code) {
		net_buf_push_mem(buf, &found_files, sizeof(uint32_t));
		data_header_push(buf, ctx, buf->len);
		mark_op_complete(ctx, MTP_RESP_SPECIFICATION_BY_FORMAT_UNSUPPORTED);
		return;
	}

	/* Host wants all root objects on all partitions */
	if (partition_id == 0xffff) {
		for (int part_idx = 1; part_idx < ARRAY_SIZE(partitions); ++part_idx) {
			for (int i = 0; i < partitions[part_idx].files_count; ++i) {
				if (partitions[part_idx].objlist[i].handle.parent_id == parent_id) {
					net_buf_add_le32(
						buf, partitions[part_idx].objlist[i].handle.value);
					found_files++;
				}
			}
		}
		...
		for (int i = 0; i < partitions[partition_id].files_count; ++i) {
			if (partitions[partition_id].objlist[i].handle.parent_id == parent_id) {
				LOG_INF("Found %s in partition %d",
					partitions[partition_id].objlist[i].name, partition_id);
				net_buf_add_le32(buf,
						 partitions[partition_id].objlist[i].handle.value);
				found_files++;
			}
		}
	}
...
MTP_CMD_HANDLER(MTP_OP_GET_OBJECT_INFO)
{
	...
	if (partitions[partition_id].objlist[object_id].handle.value == obj_handle.value) {
		...
		if (partitions[partition_id].objlist[object_id].handle.parent_id == 0xff) {
			...
			net_buf_add_le32(buf, 0); /* Root parent handle is 0x00000000 */
		} else {
			mtp_object_handle_t parent_handle =
...
MTP_CMD_HANDLER(MTP_OP_GET_OBJECT)
{
	...
	if (ctx->op_state.phase == MTP_PHASE_INIT) {
		...
		/* Add the Packet Header */
		data_header_push(buf, ctx, ctx->transfer_state.total_size);
	}
...
MTP_CMD_HANDLER(MTP_OP_SEND_OBJECT_INFO)
{
	...
	/* Initial checks */
	if (ctx->op_state.phase != MTP_PHASE_DATA) {
		LOG_ERR("Invalid phase %d, expected %d", ctx->op_state.phase, MTP_PHASE_DATA);
		err_code = MTP_RESP_GENERAL_ERROR;
		goto exit;
	}
	...
	new_obj->handle.parent_id = (dest_parent_id == MTP_ROOT_OBJ_HANDLE ? 0xff : dest_parent_id);
...
MTP_CMD_HANDLER(MTP_OP_DELETE_OBJECT)
{
	...
	if (obj_handle.value == 0xffffffff) {
...
int mtp_commands_handler(struct mtp_context *ctx, struct net_buf *buf_in, struct net_buf *buf_resp)
{
	...
	switch (ctx->op_state.phase) {
	case MTP_PHASE_INIT:
		LOG_DBG("Phase [INIT], New Op code: 0x%x", mtp_command->hdr.code);
	...
	case MTP_PHASE_DATA:
		...
		if (ctx->op_state.code != MTP_OP_SEND_OBJECT &&
			...
			LOG_ERR("Invalid phase %d for op_code %d", ctx->op_state.phase,
				ctx->op_state.code);
			mark_op_complete(ctx, MTP_RESP_INCOMPLETE_TRANSFER);
			mtp_send_response(ctx, buf_resp);
		}
		break;
	case MTP_PHASE_COMPLETE:
		mtp_send_response(ctx, buf_resp);
		LOG_DBG("Phase [COMPLETE]: Response sent for Op code 0x%x", ctx->op_state.code);
		return buf_resp->len;
	case MTP_PHASE_CANCELED:
		LOG_ERR("Unexpected cmd while an operation cancelling is in progress");
		return -EBUSY;
	default:
		__ASSERT(0, "Invalid phase %d for op_code %d", ctx->op_state.phase,
			 ctx->op_state.code);
	}
	...
	default:
		...
		mark_op_complete(ctx, MTP_RESP_OPERATION_NOT_SUPPORTED);
		mtp_send_response(ctx, buf_resp);
		break;
	}
...
static int mtp_send_response(struct mtp_context *ctx, struct net_buf *buf)
{
	...
	LOG_DBG("Sending Confirmation (0x%x), TID: %u", ctx->op_state.err, ctx->transaction_id);
