...
static struct mtp_object_t *add_object(uint8_t partition_id, uint8_t parent_id,
				       enum fs_dir_entry_type type, const char *name, uint32_t size)
{
	struct partition_t *partition = &partitions[partition_id];

	if (partition->files_count >= MAX_FILES) {
		LOG_ERR("No object handle available [file count: %u]", partition->files_count);
		return NULL;
	}

	uint16_t obj_id = partition->files_count;
	struct mtp_object_t *obj = &partition->objlist[obj_id];

	memset(obj, 0x00, sizeof(struct mtp_object_t));
...
static void remove_object(uint8_t partition_id, uint8_t object_id)
{
	struct partition_t *partition = &partitions[partition_id];

	memset(&partition->objlist[object_id], 0x00, sizeof(struct mtp_object_t));
...
static int dir_traverse(uint8_t partition_id, const char *root_path, uint32_t parent)
{
	...
	while (1) {
		...
		struct mtp_object_t *obj =
			add_object(partition_id, parent, entry.type, entry.name, entry.size);
		...
		if (entry.type == FS_DIR_ENTRY_DIR) {
			...
			err = dir_traverse(partition_id, path, obj->handle.object_id);
			if (err) {
...
static int transfer_state_init(struct mtp_context *ctx, struct mtp_object_t *obj,
			       fs_mode_t open_flags, uint32_t total_size)
{
	int err;

	traverse_path(obj, ctx->transfer_state.filepath);
	fs_file_t_init(&ctx->transfer_state.file);
	err = fs_open(&ctx->transfer_state.file, ctx->transfer_state.filepath, open_flags);
	if (err) {
		return err;
	}
	ctx->transfer_state.storage_id = obj->handle.partition_id;
...
MTP_CMD_HANDLER(MTP_OP_OPEN_SESSION)
{
	...
	if (ctx->session_opened == false) {
		...
		send_response_with_params(ctx, buf, err_code, params, 1);
		return;
	}
...
MTP_CMD_HANDLER(MTP_OP_GET_OBJECT_INFO)
{
	...
	if (partitions[partition_id].objlist[object_id].handle.value == obj_handle.value) {
		...
		/* Object Size */
		if (partitions[partition_id].objlist[object_id].handle.type == FS_DIR_ENTRY_DIR) {
			net_buf_add_le32(buf, MTP_ASSOSCIATION_SIZE);
		} else {
...
MTP_CMD_HANDLER(MTP_OP_GET_OBJECT)
{
	...
	if (ctx->phase == MTP_PHASE_REQUEST) {
		...
		err = transfer_state_init(ctx, &partitions[partition_id].objlist[object_id],
					  FS_O_READ,
					  partitions[partition_id].objlist[object_id].size);
		if (err) {
...
MTP_CMD_HANDLER(MTP_OP_SEND_OBJECT_INFO)
{
	...
	int32_t ret = 0;
	uint16_t err_code = MTP_RESP_OK;
	struct mtp_object_t *new_obj = NULL;
	...
	obj_type = (ObjectFormat == MTP_FORMAT_ASSOCIATION) ? FS_DIR_ENTRY_DIR : FS_DIR_ENTRY_FILE;
	...
	ret = fs_statvfs(partitions[dest_partition_id].mountpoint, &fs_stat);
	if (ret < 0) {
		LOG_ERR("Failed to statvfs %s (%d)", partitions[dest_partition_id].mountpoint, ret);
		err_code = MTP_RESP_GENERAL_ERROR;
		goto exit;
	}

	if (obj_size > (fs_stat.f_bfree * fs_stat.f_frsize)) {
		LOG_ERR("Not enough space to store file %u > %lu", obj_size,
			(fs_stat.f_bfree * fs_stat.f_frsize));
		err_code = MTP_RESP_STORAGE_FULL;
		goto exit;
	}

	new_obj = add_object(dest_partition_id, dest_parent_id, obj_type, obj_name, obj_size);
	if (new_obj == NULL) {
		err_code = MTP_RESP_STORAGE_FULL;
		goto exit;
	}
...
exit:
	...
	if (new_obj && err_code == MTP_RESP_OK) {
		uint32_t params[3] = {GENERATE_STORAGE_ID(new_obj->handle.partition_id),
				      GET_PARENT_HANDLE(new_obj), new_obj->handle.value};
		...
		if (new_obj) {
			transfer_state_reset(ctx);
			remove_object(dest_partition_id, new_obj->handle.object_id);
		}
		send_response_code(ctx, buf, err_code);
	}
}
...
MTP_CMD_HANDLER(MTP_OP_SEND_OBJECT)
{
	...
	ctx->transfer_state.chunks_sent++;
	ctx->transfer_state.transferred += ret;
	...
	if (obj_handle.value == MTP_ALLROOTOBJECTS) {
		...
		if (partitions[partition_id].read_only == false) {
			...
			if (ret) {
				...
				remove_object(partition_id, object_id);
			}
		} else {
...
int mtp_control_to_dev(struct mtp_context *ctx, uint8_t request, const struct net_buf *const buf)
{
	...
	case MTP_REQUEST_CANCEL:
		...
		if (strnlen(ctx->transfer_state.filepath, MAX_OBJPATH_LEN) > 0) {
			/* Discard incomplete object and free its handle */
			if (ctx->op_state.code == MTP_OP_SEND_OBJECT) {
				fs_close(&ctx->transfer_state.file);
				fs_unlink(ctx->transfer_state.filepath);

				/* Invalidate the handle allocated by the preceding SendObjectInfo
				 */
				uint8_t part_id = ctx->transfer_state.storage_id;
				uint16_t obj_id = partitions[part_id].files_count - 1;

				remove_object(part_id, obj_id);
			}
