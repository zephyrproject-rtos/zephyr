...
static const struct usb_dfu_descriptor dfu_desc = {
	...
	.wDetachTimeOut = sys_cpu_to_le16(100),
...
struct usbd_dfu_data {
	struct usb_desc_header **const rt_mode_descs;
...
static __noinit struct usb_if_descriptor rt_if0_desc;

/* Run-Time mode descriptors. No endpoints, identical for high and full speed. */
static struct usb_desc_header *rt_mode_descs[] = {
	(struct usb_desc_header *) &rt_if0_desc,
...
static void rt_detach_work(struct k_work *work)
{
...
static int usbd_dfu_preinit(void)
{
	...
	init_if_desc(&rt_if0_desc, 0, USB_DFU_PROTOCOL_RUNTIME);

	STRUCT_SECTION_FOREACH(usbd_dfu_image, i) {
		if (n >= CONFIG_USBD_DFU_NUMOF_IMAGES) {
			LOG_ERR("Cannot register USB DFU image %s", i->name);
			return -ENOMEM;
		}
	...
	k_work_init_delayable(&dfu_data.dwork, rt_detach_work);
 ...
 */
SYS_INIT(usbd_dfu_preinit, POST_KERNEL, 13);
				       ...
				       const enum usb_dfu_state next)
{
	...
	if (image->next != NULL) {
		return image->next(image->priv, data->state, next);
	}
			 ...
			 const struct usb_setup_packet *const setup)
{
	...
	case USB_DFU_REQ_DNLOAD:
		...
		if (data->image == NULL || data->image->w_cb == NULL) {
	...
	case USB_DFU_REQ_UPLOAD:
		...
		if (data->image == NULL || data->image->r_cb == NULL) {
			dfu_error(c_data, DFU_ERROR, ERR_VENDOR);
			return -ENOTSUP;
		}
			   ...
			   const struct usb_setup_packet *const setup)
{
	...
	/* Just assume that the poll time has expired. */
			     ...
			     const struct usb_setup_packet *const setup)
{
	...
	/* For now, just assume that the poll time has expired */
...
static int rt_mode_control_to_host(struct usbd_class_data *const c_data,
				   const struct usb_setup_packet *const setup,
				   struct net_buf *const buf)
{
	struct usbd_dfu_data *data = usbd_class_get_private(c_data);
...
static int rt_mode_control_to_dev(struct usbd_class_data *const c_data,
				  const struct usb_setup_packet *const setup,
...
static void *rt_mode_get_desc(struct usbd_class_data *const c_data,
			      const enum usbd_speed speed)
{
	struct usbd_dfu_data *data = usbd_class_get_private(c_data);

	return data->rt_mode_descs;
}

static int rt_mode_init(struct usbd_class_data *const c_data)
{
	struct usbd_dfu_data *data = usbd_class_get_private(c_data);
...
struct usbd_class_api rt_mode_api = {
	.control_to_host = rt_mode_control_to_host,
	.control_to_dev = rt_mode_control_to_dev,
	.get_desc = rt_mode_get_desc,
	.init = rt_mode_init,
};

USBD_DEFINE_CLASS(dfu_runtime, &rt_mode_api, &dfu_data, NULL);
			 ...
			 struct net_buf *const buf)
{
	...
	ret = image->r_cb(image->priv, setup->wValue, size, buf->data);
	if (ret >= 0) {
		net_buf_add(buf, ret);
			   ...
			   const struct net_buf *const buf)
{
	...
	ret = image->w_cb(image->priv, setup->wValue, size, buf->data);
	if (ret < 0) {
		errno = -ENOTSUP;
		dfu_error(c_data, DFU_ERROR, ERR_UNKNOWN);
	}
			    ...
			    const uint8_t iface, const uint8_t alternate)
{
	...
	STRUCT_SECTION_FOREACH(usbd_dfu_image, i) {
		if (i->if_desc->bAlternateSetting == alternate) {
			data->image = i;
			break;
		}
	}
...
static int dfu_mode_init(struct usbd_class_data *const c_data)
{
	...
	STRUCT_SECTION_FOREACH(usbd_dfu_image, i) {
		if (i->if_desc->bAlternateSetting == data->alternate) {
			data->image = i;
		}

		if (usbd_add_descriptor(uds_ctx, i->sd_nd)) {
			LOG_ERR("Failed to add string descriptor");
		} else {
			i->if_desc->iInterface = usbd_str_desc_get_idx(i->sd_nd);
		}
	}
