			     ...
			     const bool sync)
{
	...
	return api->submit_report(dev, size, report, sync);
}
			...
			const struct hid_device_ops *const ops)
{
	...
	return api->register_app(dev, rdesc, rsize, ops);
}
...
{
	if (id != 0U) {
		LOG_ERR("Set Idle for %p ID %u duration %u cannot be propagated",
			dev, id, duration);
	}
}
		     ...
		     const uint8_t *data, uint32_t data_len, uint32_t *bytes_ret)
{
	...
	ret = hid_device_submit_report(dev, data_len, data, false);
	if (bytes_ret != NULL) {
...
int usb_hid_set_proto_code(const struct device *dev, uint8_t proto_code)
{
	...
	LOG_DBG("Protocol code is set using DT property boot-protocol-code");
			     ...
			     const struct hid_ops *ops)
{
	for (unsigned int i = 0; i < ARRAY_SIZE(wrappers); i++) {
		if (wrappers[i].dev == dev) {
			...
			hid_device_register(dev, desc, size, wrappers[i].ops);
		}
	}
