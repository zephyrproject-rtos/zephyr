...
int hid_device_set_in_polling(const struct device *dev, const unsigned int period)
{
	const struct hid_device_driver_api *const api = dev->api;

	if (IS_ENABLED(CONFIG_USBD_HID_SET_POLLING_PERIOD)) {
		return api->set_in_polling(dev, period);
	}

	return -ENOTSUP;
}

int hid_device_set_out_polling(const struct device *dev, const unsigned int period)
{
	const struct hid_device_driver_api *const api = dev->api;

	if (IS_ENABLED(CONFIG_USBD_HID_SET_POLLING_PERIOD)) {
		return api->set_out_polling(dev, period);
	}
