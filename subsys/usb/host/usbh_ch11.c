...
LOG_MODULE_REGISTER(usbh_ch11, CONFIG_USBH_LOG_LEVEL);
...
/* Port features */
int usbh_req_set_hcfs_port_enable(struct usb_device *const udev,
				  const uint8_t port)
{
	return usbh_req_set_feature_port(udev, port, USB_HCFS_PORT_ENABLE);
}

int usbh_req_clear_hcfs_port_enable(struct usb_device *const udev,
				    const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_PORT_ENABLE);
}

int usbh_req_set_hcfs_port_suspend(struct usb_device *const udev,
				   const uint8_t port)
{
	return usbh_req_set_feature_port(udev, port, USB_HCFS_PORT_SUSPEND);
}

int usbh_req_clear_hcfs_port_suspend(struct usb_device *const udev,
				     const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_PORT_SUSPEND);
}

int usbh_req_set_hcfs_port_reset(struct usb_device *const udev,
				 const uint8_t port)
{
	return usbh_req_set_feature_port(udev, port, USB_HCFS_PORT_RESET);
}

int usbh_req_clear_hcfs_port_reset(struct usb_device *const udev,
				   const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_PORT_RESET);
}

int usbh_req_set_hcfs_port_power(struct usb_device *const udev,
				 const uint8_t port)
{
	return usbh_req_set_feature_port(udev, port, USB_HCFS_PORT_POWER);
}

int usbh_req_clear_hcfs_port_power(struct usb_device *const udev,
				   const uint8_t port)
{
...
int usbh_req_clear_hcfs_c_port_connection(struct usb_device *const udev,
					  const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_C_PORT_CONNECTION);
}

int usbh_req_clear_hcfs_c_port_enable(struct usb_device *const udev,
				      const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_C_PORT_ENABLE);
}

int usbh_req_clear_hcfs_c_port_suspend(struct usb_device *const udev,
				       const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_C_PORT_SUSPEND);
}

int usbh_req_clear_hcfs_c_port_over_current(struct usb_device *const udev,
					    const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_C_PORT_OVER_CURRENT);
}

int usbh_req_clear_hcfs_c_port_reset(struct usb_device *const udev,
				     const uint8_t port)
{
	return usbh_req_clear_feature_port(udev, port, USB_HCFS_C_PORT_RESET);
}

int usbh_req_set_hcfs_port_test(struct usb_device *const udev,
				const uint8_t port)
{
	return usbh_req_set_feature_port(udev, port, USB_HCFS_PORT_TEST);
}

int usbh_req_set_hcfs_port_indicator(struct usb_device *const udev,
				     const uint8_t port)
{
...
{
	LOG_DBG("Querying wHubStatus and wHubChange for hub");
