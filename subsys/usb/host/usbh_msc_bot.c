...
static uint32_t msc_bot_effective_timeout_ms(void)
{
	...
	return SCSI_DEFAULT_TIMEOUT_MS;
}
...
static int msc_bbb_cbw_out(struct usb_device *udev, const struct usbh_msc_iface *msc,
			   const struct usb_msc_bot_cbw *cbw, uint32_t cmd_tag, bool data_in,
			   uint32_t data_len, const char *phase,
			   unsigned int stale_full_cbw_retries)
{
...
static int msc_bbb_data_phase(struct usb_device *udev, const struct usbh_msc_iface *msc,
			      ...
			      uint32_t *data_actlen)
{
	...
	*data_actlen = 0U;
	...
	USBH_MSC_FLOW_INF("msc_flow: %s DATA %s len=%u", phase, data_in ? "IN" : "OUT", data_len);
	if (data_in) {
		ret = msc_bot_bulk_in_one(udev, msc->ep_in_addr, data_len, phase, data_in_out);
		if (ret == -EPIPE) {
			LOG_WRN("msc_bbb(%s): DATA: stall - CLEAR_FEATURE(HALT) bulk IN", phase);
			ret = msc_bot_recover_bulk_in_protocol_stall(
				udev, msc->ep_in_addr, msc->ep_out_addr, uhc_host, phase,
				"DATA stall bulk IN");
			if (ret != 0) {
				LOG_ERR("msc_bbb(%s): clear HALT bulk IN after DATA stall err=%d",
					phase, ret);
		...
		*data_actlen = (uint32_t)(*data_in_out)->len;
		...
		USBH_MSC_FLOW_INF("msc_flow: %s DATA IN OK bytes=%u", phase, *data_actlen);
	} else {
		ret = msc_bot_bulk_out_one(udev, msc->ep_out_addr, data_out_payload, data_len,
					   phase);
		if (ret == -EPIPE) {
			LOG_WRN("msc_bbb(%s): DATA: stall - CLEAR_FEATURE(HALT) bulk OUT", phase);
			ret = usbh_req_clear_sfs_halt(udev, msc->ep_out_addr);
			if (ret != 0) {
				LOG_ERR("msc_bbb(%s): clear HALT bulk OUT after DATA stall err=%d",
					phase, ret);
				return ret;
			}
			return 0;
		...
		*data_actlen = data_len;
		...
		USBH_MSC_FLOW_INF("msc_flow: %s DATA OUT OK len=%u", phase, *data_actlen);
	}

	return 0;
}

static int msc_bbb_csw_in(struct usb_device *udev, const struct usbh_msc_iface *msc,
			  const char *phase, struct net_buf **data_in_out,
			  const struct device *uhc_host, struct net_buf **in_csw_out,
			  unsigned int *stale_full_cbw_retries, bool *retry_full_cbw)
{
	struct net_buf *in_csw = NULL;
	unsigned int csw_stall_retries = 0U;
	int ret;

	*retry_full_cbw = false;
	*in_csw_out = NULL;
	...
	for (;;) {
		...
		if (csw_stall_retries == 0U) {
			LOG_WRN("msc_bbb(%s): STATUS: CSW IN stall — clear HALT, retry CSW",
				phase);
 ...
 *
 * Strict phase sequence (BOT — no overlap): CBW (bulk OUT) → optional DATA (one bulk IN or
 * OUT) → CSW (bulk IN). After DATA completes — including short final packet (xHCI
 * COMP_SHORT_PACKET) — only one CSW bulk IN is issued for this CBW; no second DATA transfer.
 *
 * DATA-IN shorter than dCBWDataTransferLength is valid when CSW residue matches (BOT §5.2),
 * e.g. MODE SENSE; fixed-length commands need residue == d_len − actual. Overrun vs CBW is
 * −EIO. Mis-framed DATA surfaces as PHASE_ERR / residue mismatch.
 *
 * −EPIPE from @c msc_bot_bulk_in_one is handled as CSW STALL recovery only in the STATUS
 * loop below (logger phase @c "CSW"): CLEAR_FEATURE(HALT) only (no @ref
 * usbh_ep_sync_after_clear_feature; see @ref msc_bot_recover_bulk_in_protocol_stall), retry
 * CSW once; second CSW STALL → @ref usbh_msc_bot_reset (class reset §5.3.4 + clear HALT
 * IN/OUT + UHC bulk sync when @a uhc_host) + full CBW re-issue (same tag), bounded by
 * MSC_BBB_STALE_CSW_FULL_CBW_RETRIES.
...
retry_full_cbw:
	ret = msc_bbb_cbw_out(udev, msc, cbw, cmd_tag, data_in, data_len, phase,
			      stale_full_cbw_retries);
	if (ret != 0) {
		return ret;
	}

	ret = msc_bbb_data_phase(udev, msc, data_in, data_len, data_out_payload, phase,
			       data_in_out, uhc_host, &data_actlen);
	if (ret != 0) {
		return ret;
	}
	 ...
	 */
	ret = msc_bbb_csw_in(udev, msc, phase, data_in_out, uhc_host, &in_csw,
			     &stale_full_cbw_retries, &retry_full_cbw);
	if (retry_full_cbw) {
		goto retry_full_cbw;
	}
	if (ret != 0) {
