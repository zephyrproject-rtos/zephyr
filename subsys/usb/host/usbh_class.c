 ...
 *
 * The first matched class will stop the loop, and the status will be updated
 * so that classes are only matched for a single USB function at a time.
 *
 * USB functions will only have one class matching, and calling
				      ...
				      const uint8_t iface)
{
	...
	STRUCT_SECTION_FOREACH(usbh_class_node, c_node) {
		...
		if (ret == -ENOTSUP) {
			LOG_DBG("Class %s not supporting this device, skipping",
				c_data->name);
			continue;
		}
