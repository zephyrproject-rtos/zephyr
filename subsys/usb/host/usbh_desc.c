...
bool usbh_desc_is_valid(const void *const desc_ptr, const void *const desc_end,
			const size_t expected_size, const uint8_t type)
{
	...
	if ((const uint8_t *)desc > (const uint8_t *)desc_end) {
		return false;
	}

	/* Avoid too short descriptors */
	if ((const uint8_t *)desc + expected_size > (const uint8_t *)desc_end) {
		return false;
	}
	...
	if (type != 0 && type != desc->bDescriptorType) {
		return false;
	}
...
const void *usbh_desc_get_next(const void *const desc, const void *const desc_end)
{
	...
	if (!usbh_desc_is_valid(desc, desc_end, sizeof(const struct usb_desc_header), 0)) {
		return NULL;
	}

	next = (const uint8_t *)desc + ((const struct usb_desc_header *)desc)->bLength;

	if (!usbh_desc_is_valid(next, desc_end, sizeof(const struct usb_desc_header), 0)) {
		return NULL;
	}
	...
	for (; desc != NULL; desc = usbh_desc_get_next(desc, desc_end)) {
		if (desc->bDescriptorType == USB_DESC_INTERFACE) {
			...
			if (usbh_desc_is_valid_interface(desc, desc_end) &&
			    ((const struct usb_if_descriptor *)desc)->bAlternateSetting != 0) {
				return desc;
			}
...
const void *usbh_desc_get_by_iface(const void *const desc_ptr, const void *const desc_end,
				   const uint8_t iface)
{
	if (desc_ptr == NULL || desc_end == NULL) {
		return NULL;
	}

	for (const struct usb_desc_header *desc = desc_ptr;
	     desc != NULL;
	     desc = usbh_desc_get_next(desc, desc_end)) {
		if (usbh_desc_is_valid_interface(desc, desc_end) &&
		    ((const struct usb_if_descriptor *)desc)->bInterfaceNumber == iface) {
			return desc;
		}
		if (usbh_desc_is_valid_association(desc, desc_end) &&
		    ((const struct usb_association_descriptor *)desc)->bFirstInterface == iface) {
			return desc;
		}
	}
...
const void *usbh_desc_get_cfg_end(const struct usb_device *const udev)
{
	...
	return (const uint8_t *)cfg_desc + cfg_desc->wTotalLength;
}
...
const void *usbh_desc_get_next_function(const void *const desc_ptr, const void *const desc_end)
{
	const void *desc = desc_ptr;
	uint8_t skip_num = 1;

	if (desc == NULL || desc_end == NULL) {
		return NULL;
	}

	/* Skip all interfaces the Association descriptor contains */
	if (usbh_desc_is_valid_association(desc, desc_end)) {
		skip_num = ((const struct usb_association_descriptor *)desc)->bInterfaceCount;
	}
