...
{
	const struct usb_host_ep *host_ep = USB_EP_DIR_IS_IN(ep_addr) ? udev->ep_in : udev->ep_out;

	for (unsigned int i = 0; i < ARRAY_SIZE(udev->ep_in); i++) {
		const struct usb_ep_descriptor *const ep_d = (void *)host_ep[i].desc;

		if (ep_d->bEndpointAddress == ep_addr) {
			return ep_d;
		}
	}
