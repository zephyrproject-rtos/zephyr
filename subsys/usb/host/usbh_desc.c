...
const void *usbh_desc_get_iface(const struct usb_device *const udev, const uint8_t iface)
{
	...
	if (c_desc->bNumInterfaces > UHC_INTERFACES_MAX) {
		LOG_ERR("Invalid number of USB interfaces in configuration descriptor");
		return NULL;
	}
...
{
	const struct desc_header *desc = udev->cfg_desc;

	for (; desc != NULL; desc = usbh_desc_get_next(desc)) {
		const struct usb_association_descriptor *ass_d = (const void *)desc;

		if (usbh_desc_is_valid_association(desc) &&
