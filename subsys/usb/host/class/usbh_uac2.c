					       ...
					       enum usb_device_speed speed)
{
	...
	if (bInterval == 0) {
		LOG_WRN("bInterval is 0, defaulting to 1ms");
		return 1000;
	}

	interval = 1U << (bInterval - 1);
...
cleanup:
	...
	if (new_buf == NULL) {
		LOG_ERR("ISO OUT: buffer alloc failed, transfer dropped");
		for (uint8_t i = 0; i < host_data->out_transfer_count; i++) {
			if (host_data->audio_out_transfer[i] == xfer) {
				      ...
				      uint16_t size)
{
	...
	resolved_subtype = mapping->unit_subtype;
	if (TERMINAL_TYPE_IS_ENCODED(resolved_subtype)) {
		resolved_subtype = (stream_dir == USBH_CAPTURE)
				? TERMINAL_TYPE_CAPTURE(resolved_subtype)
				: TERMINAL_TYPE_PLAYBACK(resolved_subtype);
		if (resolved_subtype == 0) {
			LOG_ERR("Control not applicable for %s stream",
	...
	entity_id = get_entity_id(entity, resolved_subtype);
	if (entity_id == 0) {
		LOG_ERR("Invalid entity ID for subtype 0x%02x", resolved_subtype);
		return -EINVAL;
	}
