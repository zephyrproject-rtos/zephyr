 ...
 * the changed ports and schedules the hub work. The work runs on the bus work
 * queue of the host stack, together with the bus events, because handling a
 * port change requires synchronous control requests, whose completions are
 * handled by the host stack thread.
 */
...
static int hub_int_cb(struct usb_device *const udev, struct uhc_transfer *const xfer)
{
	...
	(void)usbh_bus_work_submit(&hub->work);
...
static int usbh_hub_removed(struct usbh_class_data *const c_data)
{
	...
	/*
	 * The removal runs on the bus work queue too, so the hub work is not
	 * running, it is at most pending and is cancelled right away.
	 */
