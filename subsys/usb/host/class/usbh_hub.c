				 ...
				 struct usb_hub_port_status *port_sts)
{
	...
	if (ret != 0) {
		LOG_ERR("Failed to connect device on port %d: %d", port_num, ret);
		return ret;
	}
				   ...
				   struct usb_hub_port *const port_instance)
{
	...
	LOG_DBG("Device connected to port %d, scheduling reset and enumeration", port_num);
	port_instance->state = PORT_STATE_DISABLED;
	...
	if (hub_changed) {
		...
		if (clear_over_current) {
			...
			for (uint8_t i = 0; i < hub_data->port_count; i++) {
				...
				if (ret != 0) {
					LOG_ERR("Failed to re-power port %d after over-current: %d",
						i + 1, ret);
				}
			}
		}
...
static void hub_process(struct k_work *work)
{
	...
	if (hub_data->state != HUB_STATE_INIT) {
		k_mutex_unlock(&hub_data->lock);
		LOG_WRN("Hub state changed during initialization, skipping");
		return;
	}
...
static struct usbh_class_filter hub_filters[] = {
	{
		/* All hub types report bDeviceProtocol == 0x00; match interface only. */
		.flags = USBH_CLASS_MATCH_CODE_TRIPLE,
		.class = USB_HUB_CLASS_CODE,
		.sub = USB_HUB_SUBCLASS_CODE,
		.proto = USB_HUB_PROTOCOL_FS,
	},
	{
		/* HS hub with single TT: proto only meaningful in the interface descriptor */
		.flags = USBH_CLASS_MATCH_CODE_TRIPLE,
		.class = USB_HUB_CLASS_CODE,
		.sub = USB_HUB_SUBCLASS_CODE,
		.proto = USB_HUB_PROTOCOL_HS_SINGLE_TT,
	},
	{
		/* HS hub with multiple TTs: proto only meaningful in the interface descriptor */
		.flags = USBH_CLASS_MATCH_CODE_TRIPLE,
		.class = USB_HUB_CLASS_CODE,
		.sub = USB_HUB_SUBCLASS_CODE,
		.proto = USB_HUB_PROTOCOL_HS_MULTI_TT,
	},
	{0},
};
