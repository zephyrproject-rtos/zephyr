...
static int testusb_parse_descriptors(struct testusb_data *const data,
				     ...
				     uint8_t *const iface)
{
	const struct usb_if_descriptor *iface_desc;
	...
	*iface = iface_desc->bInterfaceNumber;
	LOG_DBG("bInterfaceNumber %u bAlternateSetting %u",
		iface_desc->bInterfaceNumber, iface_desc->bAlternateSetting);

	iface_desc = usbh_desc_get_next_alt_setting(iface_desc);
	if (iface_desc == NULL && iface_desc->bInterfaceNumber == 0) {
		return -ENODEV;
	}

	LOG_DBG("bInterfaceNumber %u bAlternateSetting %u",
		iface_desc->bInterfaceNumber, iface_desc->bAlternateSetting);
	desc = (void *)iface_desc;
			 ...
			 struct usb_device *const udev, const uint8_t iface)
{
	...
	if (testusb_parse_descriptors(data, udev, &my_iface) != 0) {
		return -ENOTSUP;
	}

	err = usbh_device_interface_set(udev, my_iface, 1, false);
	if (err != 0) {
		return err;
	}
