...
#define CDC_ECM_ETH_MAX_FRAME_SIZE	(NET_ETH_MTU + sizeof(struct net_eth_hdr))
#define CDC_ECM_LANGID_EN_US		0x0409

#define ECM_MAC_ADDR_LEN		NET_ETH_ADDR_LEN
		       ...
		       struct net_pkt *pkt)
{
	...
	xfer = usbh_xfer_alloc_with_buf(priv->udev, ep, cb, priv, buf_size);
	if (!xfer) {
		return -ENOMEM;
	}
 ...
 * 12 hex character string into a 6-byte MAC address.
 */
static int cdc_ecm_fetch_mac(struct usbh_cdc_ecm_data *priv)
{
	char mac_str[ECM_MAC_ADDR_LEN * 2 + 1];
	struct net_buf *buf;
	int ret;
	...
	ret = usbh_req_desc_str(priv->udev, priv->imac_str_idx,
				CDC_ECM_LANGID_EN_US, buf);
	if (ret != 0) {
		LOG_ERR("Failed to read MAC string descriptor: %d", ret);
		goto out;
	}

	ret = usbh_desc_str_to_ascii(buf->data, buf->len, mac_str, sizeof(mac_str));
	if (ret != 0) {
		LOG_ERR("Failed to convert MAC string descriptor: %d", ret);
		goto out;
	}

	if (hex2bin(mac_str, ECM_MAC_ADDR_LEN * 2, priv->mac_addr,
		    sizeof(priv->mac_addr)) != sizeof(priv->mac_addr)) {
		LOG_ERR("Invalid MAC string descriptor");
		ret = -EINVAL;
		goto out;
	}
