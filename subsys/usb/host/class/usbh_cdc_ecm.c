					...
					struct net_eth_addr *const eth_mac)
{
	...
	for (unsigned int i = 0; i < supported_langs; i++) {
		...
		ret = usbh_desc_str_utf16le_to_ascii(mac_str_desc_buf, mac_str,
						     ARRAY_SIZE(mac_str));
		if (ret != 0) {
			continue;
		}
					...
					struct uhc_transfer *const xfer)
{
	...
	case USB_CDC_CONNECTION_SPEED_CHANGE:
		...
		if (ctx->link_changed) {
			ctx->link_changed = false;
			LOG_INF("network link %s, speed [UL %u bps / DL %u bps]",
				link_state ? "up" : "down", ctx->upload_speed, ctx->download_speed);
		}
		k_mutex_unlock(&ctx->mutex);
