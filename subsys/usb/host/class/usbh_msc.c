...
#define MSC_CSW_STATUS_PHASE_ERROR 0x02U

/* Bulk-Only Mass Storage Reset, class request on the interface. */
#define MSC_REQ_BOT_RESET 0xFFU
#define MSC_REQ_TYPE_OUT  0x21U
...
/* One poll round issues TEST UNIT READY and REQUEST SENSE, and a round that
 * gets no answer spends a transfer timeout per wrapper plus the cancellation
 * wait. The deadline has to outlast that, or the loop polls exactly once.
 */
BUILD_ASSERT(CONFIG_USBH_MSC_UNIT_READY_TIMEOUT_MS > 2 * CONFIG_USBH_MSC_XFER_TIMEOUT_MS,
	     "CONFIG_USBH_MSC_UNIT_READY_TIMEOUT_MS must outlast two bulk transfer timeouts");

/* A medium that answers but stays Not Ready for longer than the deadline gets
 * another identify pass, so a slow spin-up does not leave the unit unusable
 * until it is physically reattached.
 */
#define MSC_START_ATTEMPTS 2U
...
/* A transfer is matched to the command that queued it by generation rather
 * than by address. The transfer pool is a slab, so a released transfer is
 * handed straight back to the next command at the same address, and a late
 * completion of the previous one would otherwise pass an address comparison.
 */
struct msc_xfer_cookie {
	struct usbh_msc_lun *lun;
	uint32_t seq;
};
...
{
	const struct msc_xfer_cookie *const cookie = xfer->priv;
	struct usbh_msc_lun *data;

	ARG_UNUSED(udev);

	if (cookie == NULL || cookie->lun == NULL) {
		return 0;
	}

	data = cookie->lun;

	if (atomic_get(&data->active_seq) != (atomic_val_t)cookie->seq) {
		/* A completion of a command that is no longer waiting. */
		return 0;
	}
			 ...
			 const size_t len, size_t *const transferred)
{
	...
	/* Stamp this command's generation into the cookie the completion
	 * matches against. Serialized by the caller's mutex.
	 */
	data->cookie.lun = data;
	data->cookie.seq = (uint32_t)atomic_inc(&data->xfer_seq) + 1U;

	xfer = usbh_xfer_alloc(udev, ep, msc_xfer_cb, &data->cookie);
	if (xfer == NULL) {
		return -ENOMEM;
	}
	...
	atomic_set(&data->active_seq, (atomic_val_t)data->cookie.seq);
	...
	if (ret != 0) {
		/* Cancel through the controller rather than the device, which
		 * the removal handler frees while a command is in flight.
		 */
		 ...
		 */
		if (k_sem_take(&data->xfer_sem, MSC_XFER_TIMEOUT) != 0) {
			completed = false;
		}
...
out:
	atomic_set(&data->active_seq, (atomic_val_t)MSC_SEQ_NONE);

	if (!completed) {
		/* No completion was ever observed, so the controller may still
		 * reference the transfer and its buffer. Releasing them here
		 * would hand live memory back to the pools, so they are left
		 * behind for the controller to reclaim.
		 */
		LOG_ERR("Transfer not completed, leaving it to the controller");
		return ret;
	}
...
/* Bulk-Only Transport requires a class reset followed by clearing the halt on
 * both bulk endpoints whenever the host and the device disagree about which
 * phase a command is in. Without it the device keeps waiting for a wrapper the
 * host has already moved past and every later command fails.
 */
static int msc_bot_reset(struct usbh_msc_lun *const data)
{
	struct usb_device *const udev = data->udev;
	int ret;
	...
	ret = usbh_req_setup(udev, MSC_REQ_TYPE_OUT, MSC_REQ_BOT_RESET, 0U, data->iface, 0U, NULL);
	if (ret != 0) {
		LOG_ERR("Bulk-only reset failed: %d", ret);
		return ret;
	}

	ret = usbh_req_clear_sfs_halt(udev, data->ep_in);
	if (ret != 0) {
		LOG_ERR("Failed to clear the IN halt: %d", ret);
		return ret;
	}

	ret = usbh_req_clear_sfs_halt(udev, data->ep_out);
	if (ret != 0) {
		LOG_ERR("Failed to clear the OUT halt: %d", ret);
		return ret;
	}
			...
			const bool dir_in, const bool allow_short)
{
	...
	if (len > 0) {
		...
		if (ret != 0) {
			 ...
			 * before the status stage can be read. The pointer is
			 * read once, because the removal handler clears it
			 * without the mutex.
			 */
			(void)usbh_req_clear_sfs_halt(udev, dir_in ? data->ep_in : data->ep_out);
	...
	if (ret != 0) {
		/* Bulk-Only Transport allows the status stage itself to stall
		 * once. Clear the halt and read it again before giving up on
		 * the transport.
		 */
		LOG_DBG("CSW transfer failed: %d, retrying once", ret);
		...
		(void)usbh_req_clear_sfs_halt(udev, data->ep_in);

		ret = msc_bulk_xfer(data, data->ep_in, (uint8_t *)&csw, sizeof(csw), &transferred);
		if (ret != 0) {
			LOG_ERR("CSW transfer failed: %d", ret);
			(void)msc_bot_reset(data);
			return ret;
		}
	}
	...
	if (transferred < sizeof(csw) || sys_le32_to_cpu(csw.dCSWSignature) != MSC_CSW_SIGNATURE ||
		...
		/* Host and device disagree about the command in flight, so the
		 * transport has to be resynchronized before the next one.
		 */
	...
	if (csw.bCSWStatus == MSC_CSW_STATUS_PHASE_ERROR) {
		LOG_ERR("Phase error, resetting the transport");
		(void)msc_bot_reset(data);
		return -EIO;
	}
...
static void msc_start_work(struct k_work *work)
{
	...
	for (unsigned int i = 0U; i < MSC_START_ATTEMPTS; i++) {
		if (!data->attached) {
			return;
		}
...
static int usbh_msc_removed(struct usbh_class_data *const c_data)
{
	 ...
	 *
	 * Wake a command that is waiting on the controller. Clearing the
	 * generation first keeps a completion racing this handler from also
	 * signalling and leaving a count behind for the next command.
	 */
	if (atomic_set(&data->active_seq, (atomic_val_t)MSC_SEQ_NONE) !=
	    (atomic_val_t)MSC_SEQ_NONE) {
