...
LOG_MODULE_REGISTER(usbh_hub_mgr, CONFIG_USBH_LOG_LEVEL);
...
static void usbh_hub_mgr_process_data(struct usbh_hub_mgr_data *hub_mgr_data)
{
	...
	for (port_index = 0; port_index <= hub_mgr_data->num_ports; ++port_index) {
		...
		if (port_index == 0) {
			...
			LOG_INF("Hub status: 0x%04x, change: 0x%04x", 
				hub_mgr_data->hub_instance.hub_status.wHubStatus,
				hub_mgr_data->hub_instance.hub_status.wHubChange);
					...
					struct usb_device *child_udev)
{
	...
	if (!parent_hub || !child_udev) {
		return -EINVAL;
	}

	child_hub = find_hub_mgr_by_udev(child_udev);
...
static void usbh_hub_port_process(struct k_work *work)
{
	...
	case HUB_PORT_RUN_PORT_ATTACHED:
		...
		struct usb_device *new_udev = usbh_connect_device(hub_mgr_data->uhs_ctx, port_instance->speed);
		if (new_udev != NULL) {
	...
	case HUB_PORT_RUN_CHECK_CHILD_HUB:
		...
		if (port_instance->udev) {
			ret = usbh_hub_establish_parent_child_relationship(hub_mgr_data, port_num, port_instance->udev);
			if (ret != 0) {
	...
	if (port_instance->udev) {
		...
		if (disconnected_udev) {
			...
			if (child_hub) {
				...
				usbh_disconnect_device(hub_mgr_data->uhs_ctx, disconnected_udev);
			}
		}
	}
	process_complete = true;
...
exit_processing:
	...
	if (process_complete || port_instance->port_status == HUB_PORT_RUN_INVALID) {
		hub_mgr_data->current_port = 0;
		hub_mgr.processing_hub = NULL;
		port_instance->port_status = HUB_PORT_RUN_WAIT_PORT_CHANGE;
			  ...
			  const uint8_t iface)
{
	...
	if (CONFIG_USBH_HUB_MAX_COUNT == total_hubs)
	{
	...
	if (0U == hub_mgr.total_hubs) {
		...
		LOG_INF("Root Hub connected (level=%d)", hub_mgr_data->hub_level);
	}
