...
static int usbh_hub_mgr_start_interrupt(
 ...
 */
static int usbh_hub_mgr_resubmit_interrupt_in(
	struct usbh_hub_mgr_data *hub_mgr_data,
	struct uhc_transfer *xfer)
{
	...
	if (!hub_mgr_data->connected || !hub_mgr_data->int_ep ||
	...
	struct usbh_hub_mgr_data *hub_mgr_data)
{
	...
	xfer = usbh_xfer_alloc(hub_mgr_data->hub_udev,
			       hub_mgr_data->int_ep->bEndpointAddress,
			       usbh_hub_mgr_interrupt_in_cb,
			       (void *)hub_mgr_data);
	if (!xfer) {
 ...
 */
static struct usbh_hub_mgr_data *find_hub_mgr_by_udev(
 ...
 */
static void usbh_hub_mgr_process_data(
	struct usbh_hub_mgr_data *hub_mgr_data)
{
	uint16_t hub_status;
	uint16_t hub_change;
	...
	if (hub_mgr_data->state != HUB_STATE_OPERATIONAL) {
		LOG_DBG("Hub level %d not operational yet, deferring "
			"interrupt", hub_mgr_data->hub_level);
		k_mutex_unlock(&hub_mgr_data->lock);
		/* Resubmit interrupt to process later */
		if (!hub_mgr_data->int_active &&
		    !hub_mgr_data->being_removed) {
	...
	for (port_index = 0; port_index <= hub_mgr_data->num_ports;
	     ++port_index) {
		...
		if (port_index == 0) {
			...
			struct usb_hub_status *hub_sts =
				&hub_mgr_data->hub_instance.hub_status;

			LOG_INF("Hub level %d status changed, processing",
				hub_mgr_data->hub_level);
			ret = usbh_hub_get_hub_status(
			...
			if (hub_sts->wHubChange &
				...
				if (ret != 0) {
					LOG_ERR("Failed to clear hub local "
						"power feature: %d", ret);
				}
			}
			...
			if (hub_sts->wHubChange &
				...
				if (ret != 0) {
					LOG_ERR("Failed to clear hub "
						"over-current feature: %d",
						ret);
				}
			}
		} else {
...
{
	struct usbh_hub_mgr_data *hub_mgr_data =
		(struct usbh_hub_mgr_data *)xfer->priv;
...
resubmit:
	/* Resubmit transfer */
	if (hub_mgr_data->connected &&
	    hub_mgr_data->state == HUB_STATE_OPERATIONAL) {
		int ret = usbh_hub_mgr_resubmit_interrupt_in(hub_mgr_data,
							      xfer);
		if (ret) {
			LOG_ERR("Failed to resubmit interrupt transfer: %d",
				ret);
		}
	} else {
		usbh_xfer_free(hub_mgr_data->hub_udev, xfer);
...
static void usbh_hub_mgr_process(struct k_work *work)
{
	...
	case HUB_RUN_GET_DESCRIPTOR_7: {
		...
		hub_desc = (struct usb_hub_descriptor *)
			   hub_mgr_data->hub_instance.hub_desc_buf;
		...
		if (hub_mgr_data->num_ports > USB_HUB_MAX_PORTS) {
			LOG_ERR("Too many ports: %d",
				hub_mgr_data->num_ports);
			hub_mgr_data->hub_status = HUB_RUN_INVALID;
			break;
		}
		...
		total_hub_desc_len = 7 + ((hub_mgr_data->num_ports + 7) >> 3) +
				     1;
	...
	case HUB_RUN_SET_PORT_POWER:
		...
		if (hub_mgr_data->port_list == NULL) {
			hub_mgr_data->port_list = k_malloc(
				hub_mgr_data->num_ports *
		...
		if (hub_mgr_data->port_index < hub_mgr_data->num_ports) {
			...
			ret = usbh_hub_set_port_feature(
				&hub_mgr_data->hub_instance,
				hub_mgr_data->port_index,
				USB_HUB_FEATURE_PORT_POWER);
			if (ret == 0) {
				process_success = 1;
				LOG_DBG("Setting port %d power",
					hub_mgr_data->port_index);
				k_work_submit(&hub_mgr_data->hub_work.work);
				break;
			}
		...
		for (int i = 0; i < hub_mgr_data->num_ports; i++) {
			...
			hub_mgr_data->port_list[i].reset_count =
				CONFIG_USBH_HUB_PORT_RESET_TIMES;
			hub_mgr_data->port_list[i].port_status =
				HUB_PORT_RUN_WAIT_PORT_CHANGE;
			hub_mgr_data->port_list[i].state =
		...
		hub_mgr_data->state = HUB_STATE_OPERATIONAL;
		need_prime_interrupt = 1;
		LOG_INF("Hub initialization completed, starting interrupt "
			"monitoring");
		break;
	...
	if (need_prime_interrupt) {
		...
		if (!hub_mgr_data->int_active &&
			...
			if (ret) {
				LOG_ERR("Failed to start interrupt "
					"monitoring: %d", ret);
			}
		}
 ...
 */
static void usbh_hub_mgr_recursive_disconnect(
	struct usbh_hub_mgr_data *hub_mgr_data)
{
	if (!hub_mgr_data) {
		return;
	...
	for (int i = 0; i < hub_mgr_data->num_ports; i++) {
		if (hub_mgr_data->port_list &&
			...
			if (child_hub) {
				LOG_DBG("Found child Hub on port %d, "
					"recursing", i + 1);
				usbh_hub_mgr_recursive_disconnect(child_hub);

			} else {
				LOG_DBG("Disconnecting device on port %d",
					i + 1);
				usbh_disconnect_device(hub_mgr_data->uhs_ctx,
						       port_udev);
			}
	...
	if (!hub_mgr_data->being_removed) {
		LOG_DBG("Triggering Hub level %d removal",
			hub_mgr_data->hub_level);
		usbh_disconnect_device(hub_mgr_data->uhs_ctx,
				       hub_mgr_data->hub_udev);
	}
}

/**
 * @brief Print hub information
 */
 ...
 */
static int usbh_hub_establish_parent_child_relationship(
...
static void usbh_hub_port_process(struct k_work *work)
{
	...
	if (port_num == 0 || port_num > hub_mgr_data->num_ports ||
	    !hub_mgr_data->port_list) {
	...
	if (!hub_mgr_data->hub_udev ||
	    hub_mgr_data->hub_udev->state != USB_STATE_CONFIGURED) {
	...
	LOG_DBG("Processing port %d, status=%d", port_num,
		port_instance->port_status);
	...
	case HUB_PORT_RUN_CHECK_C_PORT_CONNECTION: {
		...
		spec_status = ((uint32_t)port_sts->wPortChange << 16) |
			      port_sts->wPortStatus;

		LOG_DBG("Port %d status: wPortStatus=0x%04x, "
			"wPortChange=0x%04x", port_num,
			port_sts->wPortStatus, port_sts->wPortChange);

		if (spec_status &
		    (1UL << USB_HUB_FEATURE_C_PORT_CONNECTION)) {
			/* Connection status change */
			port_instance->port_status =
				HUB_PORT_RUN_GET_PORT_CONNECTION;
			ret = usbh_hub_clear_port_feature(
				&hub_mgr_data->hub_instance, port_num,
				USB_HUB_FEATURE_C_PORT_CONNECTION);
			if (ret != 0) {
				LOG_ERR("Failed to clear port connection "
					"change: %d", ret);
				goto error_recovery;
			}
			LOG_DBG("Port %d: Cleared connection change bit",
				port_num);
			k_work_submit(&hub_mgr_data->port_work.work);
			return;

		} else if (spec_status &
			   (1UL << USB_HUB_FEATURE_PORT_CONNECTION)) {
			/* Device connected, need reset */
			LOG_INF("Device connected to port %d, starting reset",
				port_num);
			port_instance->port_status =
		...
		if (spec_status & (1UL << (USB_HUB_FEATURE_C_PORT_RESET))) {
			...
			port_instance->port_status =
			   ...
			   (1UL << (USB_HUB_FEATURE_C_PORT_ENABLE))) {
			...
			port_instance->port_status =
			   ...
			   (1UL << (USB_HUB_FEATURE_C_PORT_OVER_CURRENT))) {
		...
		if (feature != 0) {
			ret = usbh_hub_clear_port_feature(
				...
				feature);
			if (ret != 0) {
				LOG_ERR("Failed to clear feature %d: %d",
					feature, ret);
				goto error_recovery;
			}
			LOG_DBG("Port %d: Cleared feature %d", port_num,
				feature);
			k_work_submit(&hub_mgr_data->port_work.work);
			return;
		}
	...
	case HUB_PORT_RUN_GET_PORT_CONNECTION: {
		...
		ret = usbh_hub_get_port_status(&hub_mgr_data->hub_instance,
					       ...
					       &port_change);
		if (ret != 0) {
			LOG_ERR("Failed to get port connection status: %d",
				ret);
			goto error_recovery;
		}
	...
	case HUB_PORT_RUN_CHECK_PORT_CONNECTION: {
		...
		if (spec_status & (1UL << USB_HUB_FEATURE_PORT_CONNECTION)) {
			...
			LOG_INF("Port %d connection confirmed, resetting",
				port_num);
			port_instance->port_status =
				HUB_PORT_RUN_WAIT_PORT_RESET_DONE;
			ret = usbh_hub_set_port_feature(
	...
	case HUB_PORT_RUN_WAIT_PORT_RESET_DONE:
		...
		if (!hub_mgr_data->int_active &&
			...
			ret = usbh_hub_mgr_start_interrupt(hub_mgr_data);
			if (ret) {
				LOG_ERR("Failed to restart interrupt for "
					"reset wait: %d", ret);
			}
		}
		LOG_DBG("Port %d waiting for reset completion interrupt",
			port_num);
		return;
	...
	case HUB_PORT_RUN_WAIT_C_PORT_RESET: {
		...
		ret = usbh_hub_get_port_status(&hub_mgr_data->hub_instance,
					       ...
					       &port_change);
		if (ret != 0) {
			LOG_ERR("Failed to get port status for reset check: "
				"%d", ret);
			goto error_recovery;
		}
	...
	case HUB_PORT_RUN_CHECK_C_PORT_RESET: {
		...
		if (spec_status & (1UL << USB_HUB_FEATURE_C_PORT_RESET)) {
			if (port_instance->reset_count == 0) {
				...
				/* Detect device speed */
				if (spec_status &
				    (1UL << USB_HUB_FEATURE_PORT_HIGH_SPEED)) {
					port_instance->speed =
						USB_SPEED_SPEED_HS;
				} else if (spec_status &
					   (1UL << USB_HUB_FEATURE_PORT_LOW_SPEED)) {
					port_instance->speed =
						USB_SPEED_SPEED_LS;
				} else {
					port_instance->speed =
				...
				LOG_INF("Device ready on port %d (speed: %s)",
					port_num,
					port_instance->speed ==
						USB_SPEED_SPEED_HS ?
					"HIGH" :
			...
			if (ret != 0) {
				LOG_ERR("Failed to clear port reset feature: "
					"%d", ret);
				goto error_recovery;
			}
			k_work_submit(&hub_mgr_data->port_work.work);
		...
		LOG_DBG("Port %d reset not completed, checking again",
			port_num);
		port_instance->port_status =
	...
	case HUB_PORT_RUN_RESET_AGAIN: {
		...
		ret = usbh_hub_get_port_status(&hub_mgr_data->hub_instance,
					       ...
					       &port_change);
		if (ret != 0) {
			LOG_ERR("Failed to get port status for reset again: "
				"%d", ret);
			goto error_recovery;
		}
	...
	case HUB_PORT_RUN_PORT_ATTACHED:
		...
		if (udev != NULL) {
			...
			if (hub_mgr_data->hub_udev->speed ==
			    USB_SPEED_SPEED_HS) {
				udev->hs_hub_addr =
					hub_mgr_data->hub_udev->addr;
				udev->hs_hub_port = port_num;
			} else {
				udev->hs_hub_addr =
					hub_mgr_data->hub_udev->hs_hub_addr;
				udev->hs_hub_port =
					hub_mgr_data->hub_udev->hs_hub_port;
			}
			...
			LOG_INF("Device enumeration completed for port %d, "
				"addr=%d", port_num, udev->addr);
			...
			/* update total think time */
			{
				struct usb_device *hub_udev =
					hub_mgr_data->hub_instance.hub_udev;
		...
		if (udev == NULL) {
			LOG_ERR("Device enumeration failed for port %d",
				port_num);
			...
			if (port_instance->reset_count > 0) {
				...
				port_instance->port_status =
					HUB_PORT_RUN_WAIT_PORT_CHANGE;
				LOG_WRN("Port %d enumeration failed, %d "
					"retries left", port_num,
					port_instance->reset_count);
				k_work_reschedule(&hub_mgr_data->port_work,
						  K_MSEC(1000));
				return;
			...
			LOG_ERR("Port %d enumeration max retries "
				"exceeded", port_num);
		}
	...
	case HUB_PORT_RUN_CHECK_CHILD_HUB: {
		...
		usbh_hub_establish_parent_child_relationship(
			hub_mgr_data,
			find_hub_mgr_by_udev(port_instance->udev),
			port_num);
...
process_disconnection:
	if (port_instance->udev) {
		...
		if (udev) {
			...
			if (child_hub) {
				LOG_INF("Child Hub disconnected, triggering "
					"recursive removal");
				usbh_hub_mgr_recursive_disconnect(child_hub);
			} else {
...
exit_processing:
	...
	if (need_restart_interrupt && !hub_mgr_data->int_active &&
	    !hub_mgr_data->being_removed) {
		ret = usbh_hub_mgr_start_interrupt(hub_mgr_data);
		if (ret) {
			LOG_ERR("Failed to restart interrupt monitoring: %d",
				ret);
		}
	}
...
error_recovery:
	...
	if (port_instance->reset_count > 0) {
		...
		LOG_WRN("Port %d error recovery, %d retries left",
			port_num, port_instance->reset_count);
			      ...
			      const uint8_t iface)
{
	...
	if (hub_mgr.total_hubs == CONFIG_USBH_HUB_MAX_COUNT) {
		LOG_ERR("Maximum number of hubs reached (%d)",
			CONFIG_USBH_HUB_MAX_COUNT);
		return -ENOMEM;
	}
	...
	desc_start = usbh_desc_get_by_iface(usbh_desc_get_cfg(udev), cfg_end,
					    iface);
	if (desc_start == NULL) {
	...
	if (desc_end == NULL) {
		/* If no next function exists, use configuration */
		/* descriptor end */
		desc_end = cfg_end;
	}
	...
	/* Parse interrupt endpoint within correct descriptor range */
	...
	while (desc_buf < desc_end_buf) {
		struct usb_desc_header *header =
			(struct usb_desc_header *)desc_buf;
		...
		if (header->bDescriptorType == USB_DESC_ENDPOINT) {
			struct usb_ep_descriptor *ep_desc =
				(struct usb_ep_descriptor *)desc_buf;
			...
			if ((ep_desc->bEndpointAddress & USB_EP_DIR_MASK) ==
				...
				LOG_DBG("Found hub interrupt IN endpoint "
					"0x%02x", ep_desc->bEndpointAddress);
				break;
			}
		}
	...
	k_work_init_delayable(&hub_mgr_data->hub_work,
			      usbh_hub_mgr_process);
	k_work_init_delayable(&hub_mgr_data->port_work,
			      usbh_hub_port_process);
...
static int usbh_hub_mgr_removed(struct usbh_class_data *const cdata)
{
	...
	if (hub_mgr_data->interrupt_transfer && hub_mgr_data->int_active) {
		ret = usbh_xfer_dequeue(hub_mgr_data->hub_udev,
					hub_mgr_data->interrupt_transfer);
		if (ret != 0) {
			LOG_ERR("Failed to dequeue interrupt transfer: %d",
				ret);
		}
		...
		usbh_xfer_free(hub_mgr_data->hub_udev,
			       hub_mgr_data->interrupt_transfer);
	...
	LOG_INF("Hub (level %d, Vendor ID: 0x%04x, Product ID: 0x%04x) "
		"removal completed", hub_mgr_data->hub_level,
