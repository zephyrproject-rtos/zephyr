			      ...
			      const uint8_t iface)
{
	...
	const struct usb_desc_header *header = (const struct usb_desc_header *)desc_start;

	while (header != NULL) {
		/* Stop if we've reached the next function */
		if (desc_end != NULL && header >= (const struct usb_desc_header *)desc_end) {
			break;
		}

		if (header->bDescriptorType == USB_DESC_ENDPOINT) {
			struct usb_ep_descriptor *ep_desc =
				(struct usb_ep_descriptor *)header;
