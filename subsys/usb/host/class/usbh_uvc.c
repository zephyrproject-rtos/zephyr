...
static int uvc_host_configure_device(struct uvc_device *uvc_dev)
{
	...
	ctrl_iface = uvc_dev->current_control_interface;
	stream_iface = uvc_dev->current_stream_iface_info.current_stream_iface;
	...
	ret = usbh_device_interface_set(uvc_dev->udev, ctrl_iface->bInterfaceNumber, 0, false);
	if (ret != 0) {
	...
	ret = usbh_device_interface_set(uvc_dev->udev, stream_iface->bInterfaceNumber, 0, false);
...
static int usbh_uvc_parse_vc_desc(struct uvc_device *uvc_dev, const void *const desc_beg,
...
static void usbh_uvc_parse_vs_interface_alt_desc(struct uvc_device *uvc_dev, uint8_t iface,
						 const void *const desc_beg)
{
	...
	if (desc_beg == NULL) {
		if_desc = (const void *)usbh_desc_get_iface(uvc_dev->udev, iface);
		if (if_desc == NULL) {
			return;
		}
	...
	for (; desc != NULL; desc = usbh_desc_get_next(desc)) {
		...
		for (i = 0; i < CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE; i++) {
			...
			uvc_dev->stream_ifaces[i] = (const void *)if_desc;
				      ...
				      struct uvc_device *uvc_dev, uint8_t iface)
{
	...
	iad_desc = (const void *)usbh_desc_get_iad(uvc_dev->udev, iface);
	...
	desc = (const void *)usbh_desc_get_next(iad_desc);
	if (desc == NULL) {
		LOG_ERR("No descriptor found after IAD");
		return -EBADMSG;
	}

	for (int i = 0; i < iad_desc->bInterfaceCount; i++) {
		if_desc = (const void *)usbh_desc_get_iface(uvc_dev->udev, iface + i);
		...
		if (if_desc->bInterfaceClass == USB_BCC_VIDEO &&
			...
			usbh_uvc_parse_vs_interface_alt_desc(uvc_dev, if_desc->bInterfaceNumber,
							     vs_end);
...
static uint32_t uvc_host_get_frame_default_intervals(const struct uvc_frame_descriptor *frame_desc)
{
	...
	if (interval_type == 0) {
		...
		max_interval = sys_le32_to_cpu(
			discrete_desc->dwFrameInterval[discrete_desc->bFrameIntervalType - 1]);
	}
 ...
 */
static int uvc_host_find_format(struct uvc_device *uvc_dev, uint32_t pixelformat, uint16_t width,
				uint16_t height, const struct uvc_format_descriptor **format,
				const struct uvc_frame_descriptor **frame, uint32_t *frame_interval)
{
	if (!uvc_dev || !format || !frame || !frame_interval) {
		return -EINVAL;
	}

	LOG_DBG("Looking for format: %s %ux%u", VIDEO_FOURCC_TO_STR(pixelformat), width, height);
	...
	for (int i = 0; i < uncompressed_info->num_uncompressed_formats; i++) {
		...
		if (desc_pixelformat == pixelformat) {
			...
			if (uvc_host_find_frame_in_format((const void *)format_desc, width, height,
							  UVC_VS_FRAME_UNCOMPRESSED, frame,
							  frame_interval) == 0) {
	...
	LOG_ERR("Format %s %ux%u not supported by device", VIDEO_FOURCC_TO_STR(pixelformat), width,
		height);
	return -ENOTSUP;
}

/**
 * @brief Process a single endpoint descriptor for bandwidth selection
 */
static bool uvc_host_process_endpoint_for_bandwidth(
	const struct usb_ep_descriptor *ep_desc, const struct usb_if_descriptor *if_desc,
	enum usb_device_speed device_speed, uint32_t required_bandwidth,
	uint32_t max_payload_transfer_size, uint32_t *optimal_bandwidth,
	const struct usb_if_descriptor **selected_interface,
	const struct usb_ep_descriptor **selected_endpoint, uint32_t *selected_payload_size,
	int ep_index)
{
	...
	LOG_DBG("Iface %u Alt %u EP[%d]: addr=0x%02x, mps=%u, payload=%u, interval=%u (%s), bw=%u "
		"B/s",
		if_desc->bInterfaceNumber, if_desc->bAlternateSetting, ep_index,
		ep_desc->bEndpointAddress, max_packet_size, ep_payload_size, interval,
		(device_speed == USB_SPEED_SPEED_HS) ? "uframes" : "frames", ep_bandwidth);

	/* Check if this endpoint meets requirements and is better than current selection */
	if (ep_bandwidth >= required_bandwidth && ep_payload_size >= max_payload_transfer_size &&
		...
		*selected_endpoint = (struct usb_ep_descriptor *)ep_desc;
...
/**
 * @brief Scan endpoints in an interface for suitable bandwidth
 */
static bool uvc_host_scan_interface_endpoints(
	const struct usb_if_descriptor *if_desc, enum usb_device_speed device_speed,
	uint32_t required_bandwidth, uint32_t max_payload_transfer_size,
	uint32_t *optimal_bandwidth, const struct usb_if_descriptor **selected_interface,
	const struct usb_ep_descriptor **selected_endpoint, uint32_t *selected_payload_size)
{
	const struct usb_desc_header *desc;
	int ep_count = 0;
	...
	while ((desc = usbh_desc_get_next(desc)) != NULL && ep_count < if_desc->bNumEndpoints) {
		...
		if (desc->bDescriptorType == USB_DESC_ENDPOINT) {
			...
			if (uvc_host_process_endpoint_for_bandwidth(
				    ep_desc, if_desc, device_speed, required_bandwidth,
				    max_payload_transfer_size, optimal_bandwidth,
				    selected_interface, selected_endpoint, selected_payload_size,
				    ep_count)) {
				found = true;
			}
					       ...
					       uint32_t required_bandwidth)
{
	...
	stream_info = &uvc_dev->current_stream_iface_info;
	...
	for (int i = 0; i < CONFIG_USBH_VIDEO_MAX_STREAM_INTERFACE && uvc_dev->stream_ifaces[i];
	     i++) {
		const struct usb_if_descriptor *if_desc = uvc_dev->stream_ifaces[i];
		...
		if (uvc_host_scan_interface_endpoints(if_desc, device_speed, required_bandwidth,
						      max_payload_transfer_size, &optimal_bandwidth,
						      &selected_interface, &selected_endpoint,
...
static uint32_t uvc_host_calculate_required_bandwidth(const struct uvc_format_info *current_format)
{
	...
	if (current_format->width == 0 || current_format->height == 0 || current_format->fps == 0) {
		LOG_ERR("Invalid format parameters: %ux%u@%ufps", current_format->width,
			current_format->height, current_format->fps);
		return 0;
	}
	...
	case VIDEO_PIX_FMT_JPEG:
		bytes_per_pixel = 3;
		bandwidth = (current_format->width * current_format->height * current_format->fps *
			     bytes_per_pixel) /
			    UVC_MJPEG_COMPRESSION_RATIO;
		break;
	...
	default:
		bandwidth =
			current_format->width * current_format->height * current_format->fps * 3;
	...
	LOG_DBG("Calculated bandwidth: %u bytes/sec for %s %ux%u@%ufps", bandwidth,
		VIDEO_FOURCC_TO_STR(current_format->pixelformat), current_format->width,
		current_format->height, current_format->fps);
					     ...
					     uint8_t control_selector, void *data, uint8_t data_len)
{
	...
	LOG_DBG("UVC control request: req=0x%02x, cs=0x%02x, len=%u", request, control_selector,
		data_len);
	...
	if ((bmRequestType & 0x80) && data && buf && buf->len > 0) {
		...
		if (buf->len != data_len) {
			LOG_WRN("UVC GET request: expected %u bytes, got %zu bytes", data_len,
				buf->len);
		}
 ...
 */
static int uvc_host_set_format(struct uvc_device *uvc_dev, uint32_t pixelformat, uint32_t width,
			       uint32_t height)
{
	struct uvc_stream_iface_info *stream_info;
	const struct usb_if_descriptor *stream_iface;
	...
	ret = uvc_host_find_format(uvc_dev, pixelformat, width, height, &format, &frame,
				   &frame_interval);
	if (ret != 0) {
		LOG_ERR("Unsupported format: %s %ux%u", VIDEO_FOURCC_TO_STR(pixelformat), width,
			height);
		return ret;
	}
	...
	LOG_INF("Setting format: %s %ux%u (format_index=%u, frame_index=%u, interval=%u)",
		VIDEO_FOURCC_TO_STR(pixelformat), width, height, format->bFormatIndex,
		frame->bFrameIndex, frame_interval);
	...
	ret = usbh_device_interface_set(uvc_dev->udev, stream_iface->bInterfaceNumber,
	...
	LOG_INF("UVC format set successfully: %s %ux%u@%ufps", VIDEO_FOURCC_TO_STR(pixelformat),
...
static int uvc_host_set_frame_rate(struct uvc_device *uvc_dev, uint32_t fps)
{
	...
	const struct uvc_frame_descriptor *frame_desc = uvc_dev->current_format.frame_ptr;
	...
	LOG_INF("Setting frame rate: %u fps -> %u fps", fps, 10000000 / best_frame_interval);
 ...
 */
static int uvc_host_initiate_transfer(struct uvc_device *uvc_dev, struct video_buffer *const vbuf)
{
	struct uvc_stream_iface_info *stream_info;
	const struct usb_ep_descriptor *stream_ep;
	struct net_buf *buf;
	struct uhc_transfer *xfer;
	int ret;

	stream_info = &uvc_dev->current_stream_iface_info;
	stream_ep = stream_info->current_stream_ep;

	if (!vbuf || !uvc_dev || !stream_ep) {
	...
	xfer = usbh_xfer_alloc(uvc_dev->udev, stream_ep->bEndpointAddress,
	...
	buf = usbh_xfer_buf_alloc(uvc_dev->udev, stream_info->cur_ep_mps_mult);
	if (!buf) {
 ...
 */
static int uvc_host_continue_transfer(struct uvc_device *uvc_dev, struct uhc_transfer *const xfer,
					 ...
					 struct video_frmival_enum *frmival_enum)
{
	...
	interval_data = ((const uint8_t *)frame_ptr) +
			offsetof(struct uvc_frame_descriptor, bFrameIntervalType) + 1;
	...
	if (interval_type == 0) {
		...
		LOG_DBG("Discrete interval[%u]: %u/10000000 (%u ns)", frmival_enum->index,
			frmival_enum->discrete.numerator, frmival_enum->discrete.numerator * 100);
	} else {
						      ...
						      void *data, uint8_t data_len)
{
	...
	LOG_DBG("UVC control request: req=0x%02x, cs=0x%02x, entity=0x%02x, len=%u", request,
		control_selector, entity_id, data_len);
	...
	LOG_DBG("Successfully completed UVC control request 0x%02x to entity %d", request,
		entity_id);
...
static bool uvc_host_pu_supports_control(struct uvc_device *uvc_dev, uint32_t bmcontrol_bit)
{
	...
	const struct uvc_processing_unit_descriptor *pud = uvc_dev->uvc_descriptors.vc_pud;
...
static bool uvc_host_ct_supports_control(struct uvc_device *uvc_dev, uint32_t bmcontrol_bit)
{
	...
	const struct uvc_camera_terminal_descriptor *vc_ctd = uvc_dev->uvc_descriptors.vc_ctd;
 ...
 */
static int uvc_host_query_control_range(struct uvc_device *uvc_dev, uint8_t entity_id,
					...
					struct video_ctrl_range *range)
{
	...
	/* Query minimum value */
	ret = uvc_host_control_unit_and_terminal_request(uvc_dev, UVC_GET_MIN, map->selector,
	...
	/* Query maximum value */
	ret = uvc_host_control_unit_and_terminal_request(uvc_dev, UVC_GET_MAX, map->selector,
							 entity_id, data, map->size);
	if (ret == -EPIPE) {
	...
	ret = uvc_host_control_unit_and_terminal_request(uvc_dev, UVC_GET_RES, map->selector,
							 entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
	...
	/* Query default value */
	ret = uvc_host_control_unit_and_terminal_request(uvc_dev, UVC_GET_DEF, map->selector,
							 entity_id, data, map->size);
	if (ret == -EPIPE) {
		LOG_WRN("Request 0x%02x not supported by device for entity %d, control selector "
			"0x%02x",
 ...
 */
static int uvc_host_init_unit_controls(struct uvc_device *uvc_dev, const struct device *dev,
				       uint8_t unit_subtype, uint8_t entity_id,
				       uint32_t supported_controls, int *initialized_count)
{
	const struct uvc_control_map *map;
	size_t map_length;
	int ret;
...
static int uvc_host_init(struct usbh_class_data *const c_data, struct usbh_context *const uhs_ctx)
{
	...
	struct uvc_device *uvc_dev = (void *)dev->data;
...
#ifdef CONFIG_POLL
	if (uvc_dev->sig) {
		k_poll_signal_raise(uvc_dev->sig, VIDEO_LINK_UP);
		LOG_DBG("UVC device connected signal raised");
	}
#endif
	k_mutex_unlock(&uvc_dev->lock);

	LOG_INF("UVC device (addr=%d) initialization completed", uvc_dev->udev->addr);
	return 0;
...
#ifdef CONFIG_POLL
	if (uvc_dev->sig) {
		k_poll_signal_raise(uvc_dev->sig, VIDEO_LINK_DOWN);
 ...
 */
static int usbh_uvc_set_frmival(const struct device *dev, struct video_frmival *frmival)
{
	struct uvc_device *uvc_dev = dev->data;
	uint32_t fps;
	int ret;
 ...
 */
static int usbh_uvc_get_frmival(const struct device *dev, struct video_frmival *frmival)
{
	struct uvc_device *uvc_dev = dev->data;
...
{
	struct uvc_device_descriptors *desc = &uvc_dev->uvc_descriptors;
...
static int usbh_uvc_set_ctrl(const struct device *dev, uint32_t id)
{
	...
	if (!uvc_host_is_control_supported(uvc_dev, unit_subtype, entity_id, BIT(map->bit))) {
 ...
 */
static int uvc_host_get_control_value(struct uvc_device *uvc_dev, uint32_t cid, int32_t *value)
{
	const struct uvc_control_map *map;
	uint8_t unit_subtype;
	uint8_t entity_id;
	...
	ret = uvc_host_control_unit_and_terminal_request(uvc_dev, UVC_GET_CUR, map->selector,
							 entity_id, data, map->size);
	if (ret != 0) {
	...
	switch (map->size) {
	case 1:
		*value = (map->type == UVC_CONTROL_SIGNED) ? (int32_t)(int8_t)data[0]
...
static int usbh_uvc_get_volatile_ctrl(const struct device *dev, uint32_t id)
{
	...
	switch (id) {
	case VIDEO_CID_AUTOGAIN:
		ret = uvc_host_get_control_value(uvc_dev, VIDEO_CID_GAIN, &ctrls->gain.val);
		if (ret != 0) {
...
#define USBH_VIDEO_DT_DEVICE_DEFINE(n)                                                             \
	...
	DEVICE_DT_INST_DEFINE(n, uvc_host_preinit, NULL, &uvc_device_##n, NULL, POST_KERNEL,       \
			      CONFIG_VIDEO_INIT_PRIORITY, &uvc_host_video_api);                    \
	USBH_DEFINE_CLASS(uvc_host_c_data_##n, &uvc_host_class_api, (void *)DEVICE_DT_INST_GET(n), \
			  usbh_uvc_filters);                                                       \
