...
struct uvc_device_descriptors {
	...
	struct uvc_stream_header_descriptor *vs_output_header;
};
...
find_frame:
	...
	uint8_t *desc_buf = (uint8_t *)format_ptr +
				   ...
				   const void *const desc_end)
{
	...
	for (; desc != NULL && (uint8_t *)desc < (uint8_t *)desc_end;
		...
		case UVC_VS_OUTPUT_HEADER: {
			...
			if (desc->bLength < sizeof(struct uvc_stream_header_descriptor)) {
				LOG_ERR("Invalid VS output header descriptor length: %u",
					desc->bLength);
				return -EINVAL;
			}

			uvc_dev->uvc_descriptors.vs_output_header = header_desc;
			LOG_DBG("Found VS output header: formats=%u", header_desc->bNumFormats);
			break;
		}
...
{
	const void *const desc_beg = usbh_desc_get_cfg_beg(uvc_dev->udev);
...
static int uvc_host_removed(struct usbh_class_data *const c_data)
{
	...
	if (uvc_dev->streaming && uvc_dev->video_transfer_count > 0) {
