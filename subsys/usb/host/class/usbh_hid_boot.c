...
static int continue_transfer(struct uhc_transfer *const xfer)
{
	...
	buf = usbh_xfer_buf_alloc(xfer->udev, USBH_HID_BOOT_REPORT_LEN_BYTES);
	if (!buf) {
...
static int initiate_transfers(struct hid_boot_data *priv, struct usb_device *const udev, uint8_t ep)
{
	struct net_buf *buf;
	int ret;

	priv->int_xfer = usbh_xfer_alloc(udev, ep, hid_boot_req_cb, priv);
	if (!priv->int_xfer) {
		LOG_ERR("Failed to allocate transfer");
		return -ENOMEM;
	}

	buf = usbh_xfer_buf_alloc(udev, USBH_HID_BOOT_REPORT_LEN_BYTES);
	if (!buf) {
		LOG_ERR("Failed to allocate buffer");
			       ...
			       const uint8_t iface)
{
	...
	if (initiate_transfers(priv, udev, ep_addr)) {
		return -ENOTSUP;
	}
