 ...
 *
 * @param[in] buf Pointer to the buffer of string descriptor index 0 to parse.
 ...
 * @retval -EINVAL if the descriptor in @p buf is malformed.
 */
 ...
 * to fail.
 *
 * @param[in] buf Pointer to the buffer of string descriptor to convert.
 ...
 *
 * @retval 0 on success.
 * @retval -EINVAL if @p str_len is zero, or the descriptor in @p buf is malformed.
 * @retval -ENOTSUP if a non-ASCII character (> 0x7F) is encountered.
 */
int usbh_desc_str_to_ascii(const struct net_buf *const buf, char *const str,
