...
#define FLASH_AREA_SUB_FOO(part)						\
	{.fa_id = DT_FIXED_PARTITION_ID(part),					\
	 .fa_off = DT_REG_ADDR(part),						\
...
#define FLASH_AREA_SUB_FOOO(part)	\
	COND_CODE_1(DT_NODE_HAS_STATUS_OKAY(DT_MTD_FROM_FIXED_SUBPARTITION(part)), \
		(FLASH_AREA_SUB_FOO(part)), ())

#define FOREACH_SUB_PARTITION(n) DT_FOREACH_CHILD(DT_DRV_INST(n), FLASH_AREA_SUB_FOOO)
...
#define DT_DRV_COMPAT fixed_subpartitions
	DT_INST_FOREACH_STATUS_OKAY(FOREACH_SUB_PARTITION)
};
