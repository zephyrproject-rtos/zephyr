...
struct ctapble_gatt_callbacks {
	...
	/**
	 * @brief Callback for a disconnected FIDO Bluetooth LE connection.
	...
	/**
	 * @brief Callback for changes to FIDO Bluetooth LE Status notifications.
	...
	/**
	 * @brief Callback when suitable connection parameters are available.
	 *
	 * Called without the GATT connection mutex held.
	 *
	 * @param cid Transport connection identifier whose parameters are ready.
	 */
	void (*conn_params_changed)(uint32_t cid);
};

/**
 * @brief Callbacks from the FIDO Bluetooth LE framing layer.
 */
struct ctapble_framing_callbacks {
	/**
	 * @brief Callback for a completely reassembled FIDO Bluetooth LE MSG command.
	 ...
	 * @retval non-zero Message could not be accepted; value is translated to a Bluetooth LE
	 * error
	 */
	int (*message_received)(struct bt_conn *conn, const uint8_t *data, size_t len);

	/**
	 * @brief Callback for a received FIDO Bluetooth LE CANCEL command.
	 */
	void (*cancel_received)(struct bt_conn *conn);
	...
	/**
	 * @brief Callback when a queued MSG response finishes or is discarded.
	 * @param cid Transport connection identifier of the response
	 */
	void (*response_finished)(uint32_t cid);
};

/**
 * @brief Initialize the FIDO Bluetooth LE GATT layer.
...
/**
 * @brief Shut down the FIDO Bluetooth LE GATT layer.
 */
...
/**
 * @brief Get the CID assigned to a ready FIDO Bluetooth LE connection.
 * @param conn Connection to query
 * @param cid  Destination for the transport connection identifier
 * @retval true  @p conn is the current ready FIDO Bluetooth LE connection and @p cid was written
 * @retval false @p conn is not ready/current or @p cid is NULL
 */
bool ctapble_gatt_conn_get_cid(struct bt_conn *conn, uint32_t *cid);

/**
 * @brief Check whether a CID identifies the current ready FIDO Bluetooth LE connection.
...
/**
 * @brief Check whether a connection is ready for FIDO Bluetooth LE traffic.
 * @param conn Connection to check
 * @retval true  @p conn is the current claimed connection and its link
 *               parameters satisfy the FIDO Bluetooth LE timing requirements
 * @retval false @p conn is not ready for FIDO Bluetooth LE protocol traffic
 */
bool ctapble_gatt_conn_is_ready(struct bt_conn *conn);

/**
 * @brief Send a notification on the FIDO Bluetooth LE Status characteristic.
 ...
 * @retval -ENOTCONN @p conn is not the current ready FIDO Bluetooth LE connection
...
/**
 * @brief Get the effective connection interval for a Bluetooth LE connection.
 *
 * The effective interval accounts for peripheral latency.
 *
 * @param conn        Connection to query.
 * @param interval_us Destination for the effective interval in microseconds.
 *
 * @retval 0 Successful.
 * @retval -EINVAL Invalid argument or connection parameters.
 * @retval non-zero Error returned by bt_conn_get_info().
 */
int ctapble_gatt_effective_conn_interval_get(struct bt_conn *conn, uint64_t *interval_us);

/**
 * @brief Initialize the FIDO Bluetooth LE framing layer.
...
/**
 * @brief Shut down the FIDO Bluetooth LE framing layer and release queued state.
 */
...
/**
 * @brief Submit a received FIDO Bluetooth LE fragment for asynchronous reassembly.
 * @param conn Connection that received the fragment
 * @param data Fragment data
 * @param len  Length of @p data in bytes
...
/**
 * @brief Discard queued KEEPALIVE commands for a connection.
 *
 * The currently active TX frame is not affected.
 *
 * @param conn Connection whose queued KEEPALIVE commands should be discarded.
 */
void ctapble_framing_purge_keepalives(struct bt_conn *conn);

/**
 * @brief Queue a complete FIDO Bluetooth LE command for transmission.
 * @param conn    Connection on which to send the command
 * @param command FIDO Bluetooth LE command identifier
 ...
 * @retval -EMSGSIZE @p len exceeds the maximum FIDO Bluetooth LE message size
 * @retval -ESHUTDOWN The framing layer is not initialized or is shutting down
 * @retval -ENOTCONN @p conn is NULL or is not the current ready FIDO Bluetooth LE connection
...
#endif /* FIDO2_TRANSPORT_FIDO2_TRANSPORT_BLE_H_ */
