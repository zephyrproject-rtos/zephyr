...
static void tx_schedule(k_timeout_t delay)
{
	...
	if ((atomic_get(&framing_ctx.initialized) == 0) ||
	    (atomic_get(&framing_ctx.stopping) != 0)) {
		return;
	}
...
static void tx_frame_finish(struct ctapble_tx_frame *frame)
{
	if (frame == NULL) {
		return;
	}

	if (frame->command == CTAPBLE_CMD_MSG) {
		framing_ctx.callbacks.response_finished(frame->cid);
	}
...
{
	uint64_t effective_interval_us;
	uint64_t minimum_us;
	int ret;

	minimum_us = (uint64_t)minimum_ms * USEC_PER_MSEC;

	ret = ctapble_gatt_effective_conn_interval_get(conn, &effective_interval_us);
	if (ret != 0) {
		LOG_ERR("Failed to get Bluetooth LE connection interval: %d", ret);
		return K_MSEC(minimum_ms);
	}

	return K_USEC(MAX(minimum_us, effective_interval_us));
}

static int conn_effective_interval_get(struct bt_conn *conn, k_timeout_t *delay)
{
	uint64_t effective_interval_us;
	int ret;
	...
	*delay = K_USEC(effective_interval_us);
...
static void tx_notify_complete(struct bt_conn *conn, void *user_data)
{
	...
	if ((framing_ctx.tx.active != NULL) && framing_ctx.tx.in_flight) {
		framing_ctx.tx.in_flight = false;
		if ((atomic_get(&framing_ctx.stopping) != 0) ||
...
static void tx_work_handler(struct k_work *work)
{
	...
	if ((atomic_get(&framing_ctx.stopping) != 0) || framing_ctx.tx.in_flight) {
		mutex_unlock(&tx_mutex);
	...
	if (!old_initial_sent) {
		...
		/* CTAP 2.2 Section 11.4.10 requires the sequence number to wrap to 0 after 0x7f. */
	...
	if ((framing_ctx.tx.active == frame) && framing_ctx.tx.in_flight) {
		...
		if (tx_error_is_transient(ret) && (atomic_get(&framing_ctx.stopping) == 0)) {
			retry = true;
		} else {
			failed = tx_active_remove_locked();
		}
	...
	if (retry) {
		...
		ret = conn_effective_interval_get(frame->conn, &delay);
		if (ret != 0) {
			LOG_WRN("Failed to get Bluetooth LE connection interval: %d", ret);
			tx_schedule(K_NO_WAIT);
		} else {
			tx_schedule(delay);
		}
...
static void process_fragment(struct bt_conn *conn, const uint8_t *data, uint16_t len)
{
	...
	/* Serialize readiness validation and RX state updates with disconnect cleanup. */
...
int ctapble_framing_init(const struct ctapble_framing_callbacks *callbacks)
{
	if ((callbacks == NULL) || (callbacks->message_received == NULL) ||
	    ...
	    (callbacks->response_finished == NULL)) {
		return -EINVAL;
	}
	...
	(void)atomic_clear(&framing_ctx.stopping);
...
void ctapble_framing_shutdown(void)
{
	...
	if ((atomic_get(&framing_ctx.initialized) == 0) ||
	    !atomic_cas(&framing_ctx.stopping, 0, 1)) {
		mutex_unlock(&tx_mutex);
		mutex_unlock(&rx_submit_mutex);
