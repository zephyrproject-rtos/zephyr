...
enum ctapble_transport_flag {
	CTAPBLE_FLAG_INITIALIZED,
	CTAPBLE_FLAG_STOPPING,
	CTAPBLE_FLAG_TRANSACTION_ACTIVE,
	CTAPBLE_FLAG_KEEPALIVE_ACTIVE,
};
...
static int keepalive_period_get(struct bt_conn *conn, k_timeout_t *period)
{
	...
	*period = K_USEC(MAX((uint64_t)CONFIG_FIDO2_BLE_KEEPALIVE_INTERVAL_MS * USEC_PER_MSEC,
...
static void keepalive_work_handler(struct k_work *work)
{
	...
	if (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_KEEPALIVE_ACTIVE) == 0) {
...
static void keepalive_stop(uint32_t cid)
{
	...
	if (stop) {
		atomic_clear_bit(&transport_ctx.flags, CTAPBLE_FLAG_KEEPALIVE_ACTIVE);
...
static int transaction_claim(uint32_t cid)
{
	...
	if ((atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_INITIALIZED) == 0) ||
	    (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING) != 0)) {
		ret = -ESHUTDOWN;
	} else if (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE)) {
		ret = -EBUSY;
	} else {
		transport_ctx.transaction_cid = cid;
		atomic_set_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE);
	}
...
static bool transaction_matches(uint32_t cid)
{
	...
	matches = atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE) &&
		  transport_ctx.transaction_cid == cid;
...
static bool transaction_release(uint32_t cid)
{
	...
	if (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE) &&
	    transport_ctx.transaction_cid == cid) {
		atomic_clear_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE);
...
static bool transaction_detach(uint32_t *cid)
{
	...
	active = atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE);
	if (active) {
...
static void conn_params_changed(uint32_t cid)
{
	struct bt_conn *conn;
	k_ticks_t remaining;
	k_timeout_t period;
	...
	if ((atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_INITIALIZED) == 0) ||
	    (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING) != 0) ||
	    (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_KEEPALIVE_ACTIVE) == 0) ||
	    ((uint32_t)atomic_get(&transport_ctx.keepalive_cid) != cid)) {
		mutex_unlock(&keepalive_mutex);
	...
	ret = keepalive_period_get(conn, &period);
	if (ret == 0 && !K_TIMEOUT_EQ(period, transport_ctx.keepalive_period)) {
		/* Do not postpone an already scheduled keepalive when the interval grows. */
		remaining = k_timer_remaining_ticks(&transport_ctx.keepalive_timer);
		remaining = MAX((k_ticks_t)1, MIN(remaining, period.ticks));
		transport_ctx.keepalive_period = period;
		k_timer_start(&transport_ctx.keepalive_timer, K_TICKS(remaining), period);
	}
	...
	if (ret != 0) {
		LOG_WRN("Failed to update Bluetooth LE keepalive period: %d", ret);
	}
}

static void response_finished(uint32_t cid)
{
	transaction_release(cid);
}
...
static const struct ctapble_gatt_callbacks gatt_callbacks = {
	...
	.conn_params_changed = conn_params_changed,
};
			     ...
			     fido2_transport_cancel_cb_t cancel_cb)
{
	...
	if (atomic_test_and_set_bit(&transport_ctx.flags, CTAPBLE_FLAG_INITIALIZED)) {
		return -EALREADY;
	}

	atomic_clear_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING);
	...
	transport_ctx.transaction_cid = 0;
	atomic_clear_bit(&transport_ctx.flags, CTAPBLE_FLAG_KEEPALIVE_ACTIVE);
	(void)atomic_clear(&transport_ctx.keepalive_cid);
...
fail:
	...
	atomic_clear_bit(&transport_ctx.flags, CTAPBLE_FLAG_INITIALIZED);
	return ret;
}
...
static void bluetooth_le_notify(uint32_t cid, enum fido2_wire_status status)
{
	...
	if ((atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_INITIALIZED) == 0) ||
	    (atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING) != 0) ||
	    !ctapble_gatt_cid_is_current(cid)) {
		return;
	}
	...
	/* Serialize the interval lookup and timer start with connection parameter changes. */
	mutex_lock(&keepalive_mutex, K_FOREVER);

	if ((atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING) != 0) ||
	    !ctapble_gatt_cid_is_current(cid)) {
		mutex_unlock(&keepalive_mutex);
	...
	if (ret != 0) {
		...
		bt_conn_unref(conn);
		LOG_WRN("Failed to get Bluetooth LE keepalive period: %d", ret);
		return;
	}

	(void)atomic_set(&transport_ctx.keepalive_cid, (atomic_val_t)cid);
	(void)atomic_set(&transport_ctx.keepalive_status, status_byte);
	atomic_set_bit(&transport_ctx.flags, CTAPBLE_FLAG_KEEPALIVE_ACTIVE);

	/* Send one status immediately, then repeat no faster than the effective link interval. */
	transport_ctx.keepalive_period = period;
	k_timer_start(&transport_ctx.keepalive_timer, period, period);
	...
	if (ret != 0) {
		LOG_WRN("Failed to queue initial Bluetooth LE KEEPALIVE: %d, transaction active: "
			"%d",
			ret,
			atomic_test_bit(&transport_ctx.flags, CTAPBLE_FLAG_TRANSACTION_ACTIVE));
	}
}
...
static void bluetooth_le_shutdown(void)
{
	...
	if (atomic_test_and_set_bit(&transport_ctx.flags, CTAPBLE_FLAG_STOPPING)) {
		return;
	}
