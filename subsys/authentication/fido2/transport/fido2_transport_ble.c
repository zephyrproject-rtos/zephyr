...
static void keepalive_work_handler(struct k_work *work)
{
	...
	if (atomic_get(&transport_ctx.keepalive_active) == 0) {
...
static int transaction_claim(uint32_t cid)
{
	...
	if ((atomic_get(&transport_ctx.initialized) == 0) ||
	    (atomic_get(&transport_ctx.stopping) != 0)) {
		ret = -ESHUTDOWN;
...
static int message_received(struct bt_conn *conn, const uint8_t *data, size_t len)
{
	...
	if ((atomic_get(&transport_ctx.initialized) == 0) ||
	    (atomic_get(&transport_ctx.stopping) != 0) || (transport_ctx.recv_cb == NULL)) {
		return -ESHUTDOWN;
	}
...
static void conn_params_changed(uint32_t cid)
{
	...
	if ((atomic_get(&transport_ctx.initialized) == 0) ||
	    (atomic_get(&transport_ctx.stopping) != 0) ||
	    (atomic_get(&transport_ctx.keepalive_active) == 0) ||
			     ...
			     fido2_transport_cancel_cb_t cancel_cb)
{
	...
	if (!atomic_cas(&transport_ctx.initialized, 0, 1)) {
		return -EALREADY;
	}

	atomic_clear(&transport_ctx.stopping);
...
static void bluetooth_le_notify(uint32_t cid, enum fido2_wire_status status)
{
	...
	if ((atomic_get(&transport_ctx.initialized) == 0) ||
	    (atomic_get(&transport_ctx.stopping) != 0) || !ctapble_gatt_cid_is_current(cid)) {
		return;
	}
	...
	if ((atomic_get(&transport_ctx.stopping) != 0) || !ctapble_gatt_cid_is_current(cid)) {
		mutex_unlock(&keepalive_mutex);
	...
	(void)atomic_set(&transport_ctx.keepalive_active, 1);
...
static void bluetooth_le_shutdown(void)
{
	...
	if (!atomic_cas(&transport_ctx.stopping, 0, 1)) {
		return;
	}
