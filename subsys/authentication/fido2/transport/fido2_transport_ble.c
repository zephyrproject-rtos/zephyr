...
struct ctapble_transport_context {
	...
	/** Set once shutdown has started; restart is not supported. */
	atomic_t stopping;
	...
	/** Current timer period, protected by keepalive_mutex. */
...
static int transaction_claim(uint32_t cid)
{
	...
	if (!atomic_get(&transport_ctx.initialized) || atomic_get(&transport_ctx.stopping)) {
		ret = -ESHUTDOWN;
...
static void conn_params_changed(uint32_t cid)
{
	...
	if (!atomic_get(&transport_ctx.initialized) || atomic_get(&transport_ctx.stopping) ||
	    !atomic_get(&transport_ctx.keepalive_active) ||
...
static void bluetooth_le_notify(uint32_t cid, enum fido2_wire_status status)
{
	...
	mutex_lock(&keepalive_mutex, K_FOREVER);
	if (atomic_get(&transport_ctx.stopping) || !ctapble_gatt_cid_is_current(cid)) {
		mutex_unlock(&keepalive_mutex);
		bt_conn_unref(conn);
		return;
	}
