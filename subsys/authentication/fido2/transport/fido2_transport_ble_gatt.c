...
enum ctapble_conn_param_state {
	CTAPBLE_CONN_PARAMS_NOT_READY,
	CTAPBLE_CONN_PARAMS_UPDATING,
	CTAPBLE_CONN_PARAMS_READY,
};
...
struct ctapble_gatt_context {
	...
	/* CTAP connection identifier state */
	uint32_t cid;
	uint32_t next_cid;
	...
	enum ctapble_conn_param_state conn_params_state;
};
...
static ssize_t revision_write(struct bt_conn *conn, const struct bt_gatt_attr *attr,
			      const void *buf, uint16_t len, uint16_t offset, uint8_t flags);
static ssize_t status_ccc_write(struct bt_conn *conn, const struct bt_gatt_attr *attr,
				uint16_t value);
...
BT_GATT_SERVICE_DEFINE(ctapble_service, BT_GATT_PRIMARY_SERVICE(BT_UUID_FIDO2_SERVICE),
		       ...
		       BT_GATT_CCC_WITH_WRITE_CB(NULL, status_ccc_write, CTAPBLE_CCC_PERM),
...
static uint64_t conn_effective_interval_us(uint32_t interval_us, uint16_t latency)
{
	return (uint64_t)interval_us * (latency + 1U);
}

static bool conn_params_values_suitable(uint32_t interval_us, uint16_t latency)
{
	...
	/*
	 * Peripheral latency allows skipping connection events, so the maximum
	 * effective interval is interval * (latency + 1).
	 */
	effective_interval_us = conn_effective_interval_us(interval_us, latency);

	return effective_interval_us <= ((uint64_t)CTAPBLE_MAX_CONN_INTERVAL_MS * USEC_PER_MSEC);
}

static bool conn_params_suitable(struct bt_conn *conn)
{
	...
	if (ret != 0) {
		LOG_WRN("Failed to get Bluetooth LE connection info: %d", ret);
		return false;
	}

	if (info.type != BT_CONN_TYPE_LE) {
		LOG_ERR("Unexpected Bluetooth connection type: %d", info.type);
		return false;
	}

	if (info.le.interval_us == 0U) {
		LOG_ERR("Bluetooth LE connection interval is zero");
		return false;
	}

	return conn_params_values_suitable(info.le.interval_us, info.le.latency);
}
...
static struct bt_conn *conn_detach(struct bt_conn *conn, uint32_t *cid)
{
	...
	if ((gatt_ctx.conn != NULL) && ((conn == NULL) || (gatt_ctx.conn == conn))) {
		...
		gatt_ctx.cid = 0;
		gatt_ctx.conn_params_state = CTAPBLE_CONN_PARAMS_NOT_READY;
	}
...
static bool conn_access_allowed(struct bt_conn *conn)
{
	...
	allowed = (atomic_get(&gatt_ctx.initialized) != 0) && (conn != NULL) &&
		  (atomic_get(&gatt_ctx.stopping) == 0) &&
...
static void conn_params_state_set(struct bt_conn *conn, enum ctapble_conn_param_state state)
{
	uint32_t cid = 0;

	mutex_lock(&conn_mutex, K_FOREVER);

	if ((atomic_get(&gatt_ctx.initialized) != 0) && (atomic_get(&gatt_ctx.stopping) == 0) &&
	    (gatt_ctx.conn == conn)) {
		gatt_ctx.conn_params_state = state;
		if (state == CTAPBLE_CONN_PARAMS_READY) {
			cid = gatt_ctx.cid;
		}
	}
	...
	/* The transport may acquire keepalive_mutex and look up this connection again. */
	if (cid != 0) {
		gatt_ctx.callbacks.conn_params_changed(cid);
	}
}
...
static bool conn_is_claimed(struct bt_conn *conn)
{
	bool claimed;

	mutex_lock(&conn_mutex, K_FOREVER);

	claimed = (atomic_get(&gatt_ctx.initialized) != 0) &&
		  (atomic_get(&gatt_ctx.stopping) == 0) && (conn != NULL) &&
		  (gatt_ctx.conn == conn);
...
static int conn_params_request(struct bt_conn *conn)
{
	int ret;
...
static bool conn_get_cid(struct bt_conn *conn, uint32_t *cid)
{
	...
	current = (atomic_get(&gatt_ctx.initialized) != 0) &&
		  (atomic_get(&gatt_ctx.stopping) == 0) &&
		  (gatt_ctx.conn_params_state == CTAPBLE_CONN_PARAMS_READY) && (conn != NULL) &&
		  (gatt_ctx.conn == conn);
	if (current && (cid != NULL)) {
		*cid = gatt_ctx.cid;
	}
...
bool ctapble_gatt_cid_is_current(uint32_t cid)
{
	...
	current = (atomic_get(&gatt_ctx.initialized) != 0) &&
		  (atomic_get(&gatt_ctx.stopping) == 0) && (gatt_ctx.conn != NULL) &&
		  (gatt_ctx.conn_params_state == CTAPBLE_CONN_PARAMS_READY) &&
		  (gatt_ctx.cid == cid);
...
struct bt_conn *ctapble_gatt_conn_ref(uint32_t cid)
{
	...
	if ((atomic_get(&gatt_ctx.initialized) != 0) && (atomic_get(&gatt_ctx.stopping) == 0) &&
	    (gatt_ctx.conn != NULL) && (gatt_ctx.conn_params_state == CTAPBLE_CONN_PARAMS_READY) &&
	    (gatt_ctx.cid == cid)) {
			...
			bt_gatt_complete_func_t func, void *user_data)
{
	...
	if (atomic_get(&gatt_ctx.notifications_enabled) == 0) {
		return -EACCES;
	}
...
int ctapble_gatt_effective_conn_interval_get(struct bt_conn *conn, uint64_t *interval_us)
{
	...
	*interval_us = conn_effective_interval_us(info.le.interval_us, info.le.latency);
				   ...
				   const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	...
	if (atomic_get(&gatt_ctx.revision_selected) == 0) {
			      ...
			      const void *buf, uint16_t len, uint16_t offset, uint8_t flags)
{
	 ...
	 */
	(void)atomic_set(&gatt_ctx.notifications_enabled,
			 bt_gatt_is_subscribed(conn, status_attr, BT_GATT_CCC_NOTIFY));

	if (conn_params_suitable(conn)) {
		conn_params_state_set(conn, CTAPBLE_CONN_PARAMS_READY);
	} else {
		conn_params_state_set(conn, CTAPBLE_CONN_PARAMS_UPDATING);

		if (conn_params_request(conn) != 0) {
			conn_params_state_set(conn, CTAPBLE_CONN_PARAMS_NOT_READY);
			return BT_GATT_ERR(BT_ATT_ERR_UNLIKELY);
		}
	}

	/* Control Point writes remain blocked until this negotiation completes. */
	(void)atomic_set(&gatt_ctx.revision_selected, 1);
				...
				uint16_t value)
{
	...
	if ((atomic_get(&gatt_ctx.initialized) != 0) && (atomic_get(&gatt_ctx.stopping) == 0) &&
	    (gatt_ctx.conn == conn)) {
		cid = gatt_ctx.cid;
		(void)atomic_set(&gatt_ctx.notifications_enabled, enabled);
	}
...
static void disconnected(struct bt_conn *conn, uint8_t reason)
{
	...
	(void)atomic_clear(&gatt_ctx.notifications_enabled);
	(void)atomic_clear(&gatt_ctx.revision_selected);

	LOG_INF("CTAP BLE link closed: 0x%02x", reason);

	if (atomic_get(&gatt_ctx.initialized) != 0) {
...
static bool le_param_req(struct bt_conn *conn, struct bt_le_conn_param *param)
{
	...
	return conn_params_values_suitable(BT_CONN_INTERVAL_TO_US(param->interval_max),
					   param->latency);
}
			     ...
			     uint16_t timeout)
{
	...
	suitable = conn_params_values_suitable(BT_CONN_INTERVAL_TO_US(interval), latency);
	...
	LOG_WRN("Bluetooth LE connection parameters unsuitable for FIDO");
}
...
static void le_param_update_rejected(struct bt_conn *conn, uint8_t hci_err)
{
	...
	LOG_WRN("FIDO Bluetooth LE connection parameter update rejected: 0x%02x", hci_err);

	conn_params_state_set(conn, CTAPBLE_CONN_PARAMS_NOT_READY);

	ret = bt_conn_disconnect(conn, BT_HCI_ERR_REMOTE_USER_TERM_CONN);
	if (ret != 0) {
		LOG_ERR("Disconnect due to rejected connection parameters failed ret: %d", ret);
	}
}
...
int ctapble_gatt_init(const struct ctapble_gatt_callbacks *callbacks)
{
	...
	if ((callbacks == NULL) || (callbacks->fragment_received == NULL) ||
	    ...
	    (callbacks->conn_params_changed == NULL)) {
		return -EINVAL;
	}
	...
	(void)atomic_clear(&gatt_ctx.stopping);
	(void)atomic_set(&gatt_ctx.initialized, 1);
...
void ctapble_gatt_shutdown(void)
{
	...
	/*
	 * Close GATT admission before detaching the active connection.
	 * All connection access paths are serialized by conn_mutex.
	 */
	(void)atomic_set(&gatt_ctx.stopping, 1);
