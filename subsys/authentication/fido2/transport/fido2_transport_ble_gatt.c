...
struct ctapble_gatt_context {
	...
	/** Set when the current connection parameters satisfy FIDO BLE timing. */
...
static void conn_params_state_set(struct bt_conn *conn, enum ctapble_conn_param_state state)
{
	...
	if (atomic_get(&gatt_ctx.initialized) && !atomic_get(&gatt_ctx.stopping) &&
	    (gatt_ctx.conn == conn)) {
...
static bool conn_is_claimed(struct bt_conn *conn)
{
	...
	claimed = atomic_get(&gatt_ctx.initialized) && !atomic_get(&gatt_ctx.stopping) &&
		  (conn != NULL) && (gatt_ctx.conn == conn);
...
static bool conn_get_cid(struct bt_conn *conn, uint32_t *cid)
{
	...
	current = atomic_get(&gatt_ctx.initialized) && !atomic_get(&gatt_ctx.stopping) &&
			     ...
			     uint16_t timeout)
{
	...
	if (ret != 0) {
		LOG_ERR("Disconnect failed ret: %d", ret);
	}
}
