...
const j1939_routing_callback_t j1939_app_routing_table[256] = {
	...
	/* PF = 0x0xFE(254)   */ j1939_default_route,
...
/**
 * @brief Runs the specific funciton in j1939_app_routing_table for the PGN in the current message
 * @param message Message to be processed
...
{
	j1939_node_t ret_node = NULL;

	STRUCT_SECTION_FOREACH(j1939_node_cfg, node) {
		if (address == j1939_ac_get_source_address(node)) {
			ret_node = node;
			break;
		}
	}
						  ...
						  j1939_pdu_format_t pf, j1939_node_t node)
{
	...
	(void)j1939_app_routing_table[pf](message, node);
#else
	result = j1939_app_routing_table[pf](message, node);
#endif
