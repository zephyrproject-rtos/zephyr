 ...
 */
static uint8_t *J1939_App_AddStringNoNull(const uint8_t *source, uint8_t *destination);
...
static uint8_t *J1939_App_AddStringNoNull(const uint8_t *source, uint8_t *destination)
{
	/* We will copy from the source, to the destination, up to, but not including the NULL
	 * pointer
	 */
	while (*source != 0) {
		*destination++ = *source++;
