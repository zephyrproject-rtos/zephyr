 ...
 *========================================================================
 */
/* 1. Store all j1939_name_t structures as the packed 8 byte array instead of in the unpacked
...
static j1939_counter_t J1939Ac_ArbitraryAddressListIndex[CONFIG_J1939_NODES_COUNT];
/* Keeps track of the index of the next address to use */
...
void j1939_ac_init(void)
{
	STRUCT_SECTION_FOREACH(j1939_node_cfg, node) {
		/* Retrieve the default source address we will claim from the application specific
		 * module
		 */
		node->j1939_ac_bus_info.source = j1939_ac_app_get_default_source_address(node);

#ifdef J1939AC_SELF_CONFIGURABLE
		J1939Ac_ArbitraryAddressListIndex[node_index] = 0;

		for (uint16_t index = 0; index < J1939AC_ALT_ADDRESS_LIST_LENGTH; index++) {
			/* Initialize the list of alternate source addresses for address claim to
			 * the NULL address
			 */
			/* This is done here, just so the application developer doesn't forget to do
			 * it in the
			 */
			/* application file */
			J1939Ac_ArbitraryAddressCapableSourceAddressList[node_index][index] =
				J1939_NULL_ADDRESS;
		}
#endif

		j1939_ac_app_generate_name(node);
		...
		node->node_state = J1939_AC_STATE_WAITING_STARTUP_INIT;
		node->j1939_ac_is_requested = false;

		node->recorded_bus_info_count = 0;

		/* Initialize the J1939 NAME/source address information */
		for (uint16_t index = 0; index < CONFIG_J1939_MAX_NODES_IN_SYSTEM; index++) {
			node->recorded_bus_info[index].source = J1939_NULL_ADDRESS;

#if defined(J1939_RECORD_ADDRESS_CLAIMED_NAMES)
			node->recorded_bus_info[index].name.idNumber = 0;
			node->recorded_bus_info[index].name.mfgCode = 0;
			node->recorded_bus_info[index].name.ecuInstance = 0;
			node->recorded_bus_info[index].name.functionInstance = 0;
			node->recorded_bus_info[index].name.function = 0;
			node->recorded_bus_info[index].name.vehicleSystem = 0;
			node->recorded_bus_info[index].name.vehicleSystemInstance = 0;
			node->recorded_bus_info[index].name.industryGroup = 0;
			node->recorded_bus_info[index].name.isSelfConfig = false;
			node->recorded_bus_info[index].name.reservedBit = 0;
#endif
		}
	}
...
void j1939_ac_task(void)
{
	...
	STRUCT_SECTION_FOREACH(j1939_node_cfg, node) {
		...
		default:
			/* Shouldn't ever get here.  If we do we go to the cannot claim state.  If
			 * someone requests address claim again, we reset and try to claim
			 * again.
			 */
...
/**
 * @brief Initiates an address claim for the specified node
 *
 * @param node Node to initiate an address claim for
 *
 * @return None /
 */
static void j1939_ac_claim_address(j1939_node_t node)
{
	...
	id = j1939_build_message_id(
		0, 0, J1939_PR_ADDRESS_CLAIMED,
		j1939_build_pgn_from_pdu(J1939_PF_ADDRESS_CLAIMED, J1939_GLOBAL_ADDRESS),
		/* TODO - not use j1939_ac_bus_info - need to refactor*/
		node->source_address);
...
void j1939_ac_process_request(j1939_node_t node)
{
	...
	if (node->node_state == J1939_AC_STATE_CLAIMED) {
		...
		node->j1939_ac_is_requested = true;
	}
...
/**
 * @brief Checks for address contention
 *
 * @param node Node to check for contention on
 * @param data Name data to check with
 *
...
/**
 * @brief Adds address claimed information to the recorded bus information table Note that this
 * function may be VERY expensive.  We must be locked in a critical section prior to this call, and
 * will remain locked until we return.  Because of the required name comparison, we may remain
 * locked for an extended period of time.  This will occur any time a new address claim is received.
 *
 * @param source Address to add
 * @param nameData J1939 name information(8 byte array)
 * @param node Node to add for
 *
 * @return None /
 */
static void j1939_ac_update_recorded_bus_info(j1939_source_address_t source,
				const uint8_t *nameData, j1939_node_t node)
{
	...
	/* NOTES: */
	/* Not checking nameData for valid pointer here because it is guaranteed by the function */
	...
	/* architectures.  They are needed on architectures that have bytes that are longer than 8
	 * bits
	 */
	...
	if (addName) {
		if (J1939Ac_RecordedBusInfoCount[node] < CONFIG_J1939_MAX_NODES_IN_SYSTEM) {
			J1939Ac_RecordedBusInfo[node][J1939Ac_RecordedBusInfoCount[node]].source =
				source;
			J1939Ac_RecordedBusInfo[node][J1939Ac_RecordedBusInfoCount[node]].name =
				newName;
...
bool j1939_ac_is_name_match(j1939_name_t *name1, j1939_name_t *name2)
{
	...
	if (name1 && name2) {
		result = (name1->idNumber == name2->idNumber) &&
			 (name1->mfgCode == name2->mfgCode) &&
...
void j1939_ac_name_config_to_byte_array(j1939_node_t node, uint8_t *array)
{
	if (node && array) {
		...
		array[2] =
			(uint8_t)(((uint8_t)(((j1939_manufacturer_code_t)node->manufacturer_code &
					0x07) << 5)) | (LOBYTE(HIWORD(node->id_number)) & 0x1F));
		array[3] = (uint8_t)((j1939_manufacturer_code_t)(node->manufacturer_code & 0x7FF) >>
					3);
		array[4] = (uint8_t)(((node->function_instance & 0x1F) << 3) |
					(node->ecu_instance & 0x07));
								...
								j1939_counter_t index)
{
	...
	if (index < node->recorded_bus_info_count) {
...
bool j1939_ac_is_received(const struct can_frame *message, j1939_node_t node)
{
	...
	if (message) {
		/* don't check node since it is required to be good by iter sections*/
		source = j1939_get_source_address(message->id);

		CriticalSection_Lock();

		/* If the source address of the address claimed message matches the source
		 * address of the J1939 address node it came in on, do contention checks
		 */
		if ((source == node->j1939_ac_bus_info.source) &&
			(node->j1939_ac_bus_info.source != J1939_NULL_ADDRESS)) {
			/* Check name contention with message just received */
			if (j1939_ac_check_address_contention(node, message->data)) {
				/* We lost contention. */
				/* Move to J1939_AC_STATE_LOST_CONTENTION. */
				node->node_state = J1939_AC_STATE_LOST_CONTENTION;
			} else {
	...
	/* Return false so we don't destroy the message.  This allows other functions to also
	 * process the message
	 */
	return false;
}
...
{
	/* TODO - need to range check node here */
	if (true) {
...
/**
 * @brief Handles the case when we cannot claim an address
 *
 * @param Node we are looking at
 *
 * @return None /
 */
static void j1939_ac_cannot_claim_delay(j1939_node_t node)
...
/**
 * @brief Handles the start of an address claim sequence
 *
 * @param node J1939 node to handle
 *
 * @return None /
 */
static void j1939_ac_address_claim_start(j1939_node_t node)
...
/**
 * @brief Handles the waiting case of an address claim sequence
 *
 * @param node J1939 address node to handle
 *
 * @return None /
 */
static void j1939_ac_address_claim_waiting(j1939_node_t node)
{
	...
	if ((j1939_timer_get_time() - node->ac_timestamp) > J1939_AC_WAIT_TIME) {
		...
		UpdateJ1939LowLevelFiltering(node, node->j1939_ac_bus_info.source);
#endif
...
/**
 * @brief Process the contention lost case for the specified J1939 address node
 *
 * @param node Node to handle for
 *
 * @return None /
 */
static void j1939_ac_lost_contention(j1939_node_t node)
{
	...
	if (tempAddress == J1939_NULL_ADDRESS) {
		...
		node->j1939_ac_bus_info.source = tempAddress;
		node->node_state = J1939_AC_STATE_START;
	}
	CriticalSection_Unlock();
#else
...
/**
 * @brief Handles the common conention handling that occurs whether or not you have a configurable
 * address
 *
...
static void j1939_ac_common_contention_handling(j1939_node_t node)
{
	...
	/* CAN timer is used for a pseudo-random delay, before sending CANNOT CLAIM ADDRESS */
	...
	w16BitName = (uint16_t)(node->j1939_ac_bus_info.name.idNumber & 0xFFFF);
...
bool j1939_ac_get_name_config_node(j1939_node_t node, j1939_name_t *kname)
{
	...
	if (kname != NULL) {
		CriticalSection_Lock();
		*kname = node->j1939_ac_bus_info.name;
		CriticalSection_Unlock();
		result = true;
	}
...
bool j1939_ac_get_name_array_node(j1939_node_t node, uint8_t *data)
{
	...
	if (data != NULL) {
		CriticalSection_Lock();
		/* TODO - need to decide on whether to keep around ac bus info or just
		 * use name info direct
		 */
		j1939_ac_name_config_to_byte_array(node, data);
		CriticalSection_Unlock();
...
bool j1939_ac_set_name_config_node(j1939_node_t node, j1939_name_t *name)
{
	...
	if (name != NULL) {
		CriticalSection_Lock();
		node->j1939_ac_bus_info.name = *name;
		CriticalSection_Unlock();
		result = true;
	}
...
static j1939_source_address_t j1939_ac_get_next_alternate_address(j1939_node_t node)
{
	...
	do {
		if (J1939Ac_ArbitraryAddressListIndex[node] < J1939AC_ALT_ADDRESS_LIST_LENGTH) {
			...
			/* We have reached the end of our array and therefore have no source
			 * addresses.  Break out of the loop and return the null address.
			 */
