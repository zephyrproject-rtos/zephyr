 ...
 * initializes it for transmission. `j1939_tp_update_send_message_times` will subsequently start
 ...
 * address and node.
 * @note The caller of the function is responsible for calling `j1939_tp_free_completed_buffer`()
 * with the returned pointer when they are done using the buffer
...
/**
 * @brief Frees the specified transport buffer retrieved using
 * `j1939_tp_get_completed_session_buffer`.
