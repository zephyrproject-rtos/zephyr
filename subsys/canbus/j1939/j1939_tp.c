...
typedef struct j1939_tp_receive_message_info_s {
	...
	uint8_t packetCountDown; /* Countdown on the maximum number of packets we can request
				  * at a time
				  */
	...
	j1939_counter_t currentByteOffset; /* Received byte offset into DataObject */
...
typedef struct j1939_tp_transmit_message_info_s {
	j1939_destination_address_t
		destination;              /* Address to send message to (global = BAM message) */
...
void j1939_tp_update_receive_message_times(j1939_timer_t callPeriod)
{
	...
	for (index1 = 0; index2 && (index1 < CONFIG_J1939TP_NUMBER_OF_TP_RX_SESSIONS); index1++) {
		if (J1939Tp_ReceiveMessageList[index1].state == j1939_tp_state_holding) {
			...
			check = j1939_tp_register_user_callback(
				J1939Tp_ReceiveMessageList[index1].pgn,
				J1939Tp_ReceiveMessageList[index1].incomingDataObject,
					...
					j1939_tp_callback_t callback)
{
	...
	if (index < CONFIG_J1939TP_NUM_ALLOWED_RECEIVE_PGN) {
		...
		node->j1939_tp_register_pgn_list[node->j1939_tp_register_pgn_index++].callback =
			callback;
	} else {
		/* the list is full! */
		result = false;
...
bool j1939_tp_register_extended_length_messages(j1939_pgn_t pgn, j1939_node_t node)
{
	...
	CriticalSection_Lock();
	for (index = 0; (index < node->j1939_tp_register_pgn_index) &&
			(index < CONFIG_J1939TP_NUM_ALLOWED_RECEIVE_PGN); index++) {
		if (node->j1939_tp_register_pgn_list[index].pgn == pgn) {
...
void j1939_tp_process_tpcm_message(const struct can_frame *message, j1939_node_t node)
{
	...
	if (message) {
		...
		case J1939TP_CM_RTS: /* for TP rx messages */
			j1939_tp_process_bam_rts(message->data[0],
		...
		case J1939TP_CM_EOMACK: /* for TP tx messages */
			j1939_tp_process_eom_ack(j1939_get_source_address(message->id),
						 message->data, node);
			break;
		...
		case J1939TP_CM_ERTS:
			pgn = MAKEDWORD(MAKEWORD(message->data[5], message->data[6]),
					MAKEWORD(message->data[7], 0));
...
{
	/* not checking if node is valid since it should only be created via iterable section*/
	for (j1939_counter_t index = 0; (index < node->j1939_tp_register_pgn_index) &&
					(index < CONFIG_J1939TP_NUM_ALLOWED_RECEIVE_PGN);
		index++) {
		if (node->j1939_tp_register_pgn_list[index].pgn == pgn) {
			return true;
						...
						j1939_source_address_t source, j1939_node_t node)
{
	...
	for (j1939_counter_t index = 0; (index < node->j1939_tp_register_pgn_index) &&
		...
		if (node->j1939_tp_register_pgn_list[index].pgn == pgn) {
			...
			/* Yes PGN is supported. If call back is not a NULL pointer, then
			 * we're ready to go.
			 */
			if (callback != NULL) {
				/* Call user function. */
		...
		if ((j1939_tp_transmit_message_list[index].pgn == pgn) &&
			...
			if ((j1939_tp_transmit_message_list[index].numPacketsLeftCtsAllows == 0) &&
				...
				if (nextPacketToSend != j1939_tp_transmit_message_list[index]
					...
					j1939_tp_transmit_message_list[index]
						.lowestAblePacketSequenceNum = nextPacketToSend;
					j1939_tp_transmit_message_list[index].nextRequestedPacket =
						0;
				}
				...
				/* Send out a connection abort, due to receiver's ineptitude,
				 * here!!!
				 */
				...
				j1939_tp_num_open_transmit_connections--;
				j1939_tp_free_buffer(
					j1939_tp_transmit_message_list[index].dataRegion);
				j1939_tp_transmit_message_list[index].state =
					j1939_tp_state_available;
			}
