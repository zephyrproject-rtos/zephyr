			...
			if (j1939_tp_transmit_message_list[index1].destination !=
				...
				if (deltaTime >= J1939TP_BAM_TIME) {
					...
					if (j1939_tp_transmit_message_list[index1]
						...
						if (j1939_tp_transmit_message_list[index1].node <
						    CONFIG_J1939_NODES_COUNT) {
							j1939_tp_transmit_message_list[index1]
								.node->j1939_tp_transmit_bam =
								false;
						}
					} else {
...
bool j1939_tp_register_extended_length_messages(j1939_pgn_t pgn, j1939_node_t node)
{
	...
	if (node < CONFIG_J1939_NODES_COUNT) {
		...
		if (index < CONFIG_J1939TP_NUM_ALLOWED_RECEIVE_PGN) {
			/* add PGN to the list and increment the index */
			if (node->j1939_tp_register_pgn_index <
			    CONFIG_J1939TP_NUM_ALLOWED_RECEIVE_PGN) {
				node->j1939_tp_register_pgn_list
					[node->j1939_tp_register_pgn_index++]
						.pgn = pgn;
				CriticalSection_Unlock();
				return true;
			} else {
