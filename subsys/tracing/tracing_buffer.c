...
{
	size_t space = rb.size - (rb.put.head - rb.get.tail);

	return area_claim(&rb, &rb.put, data, MIN(size, space));
}
...
{
	size_t valid = rb.put.tail - rb.get.head;

	return area_claim(&rb, &rb.get, data, MIN(size, valid));
}
