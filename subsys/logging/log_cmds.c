...
static const char * severity_lvls[] = {
	"none",
	"err",
	"wrn",
	"inf",
...
static const char * severity_lvls_sorted[] = {
	"dbg",
	"err",
	"inf",
	"none",
...
static void cmd_log_self_status(const struct shell *shell, size_t argc, char **argv)
{
...
static int module_id_get(const char * p_name)
{
	u32_t modules_cnt = log_sources_count();
...
SHELL_CMD_REGISTER(log, &sub_log_stat, "Commands for controlling logger", cmd_log);
