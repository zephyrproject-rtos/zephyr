...
static void hda_log_dma_flush(void)
{
	...
	sys_cache_data_flush_range(&hda_log_buf[0], CONFIG_LOG_BACKEND_ADSP_HDA_SIZE);
...
static inline uint32_t acquire_output(k_spinlock_key_t *key)
{
	int32_t id  = -1;
...
static inline void release_output(uint32_t id, k_spinlock_key_t *key)
{
	output_flush(id);
	if (k_is_in_isr()) {
			   ...
			   uint32_t cnt)
{
	...
	uint32_t id = acquire_output(&key);
