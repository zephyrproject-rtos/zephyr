...
struct acp_log_backend {
	...
	bool panicked;     /* panic() ran: flush every message synchronously    */
...
#define ACP_LOG_WINDOW_SIZE (LOG_HOST_BUFFER_SIZE | PAGE_ENABLE)
...
#define LOG_DMA_POLL_TIMEOUT_US   500U

/* Run-status of the log channel alone; the other channels carry audio DMA. */
static inline bool acp_log_dma_ch9_busy(void)
{
	acp_dma_ch_sts_t sts = (acp_dma_ch_sts_t)io_reg_read(PU_REGISTER_BASE + ACP_DMA_CH_STS);

	return (sts.bits.dmachrunsts & BIT(LOG_DMA_CH)) != 0U;
}

/*
 * Bounded wait for channel 9 to fall idle.  CCOUNT rather than
 * k_cycle_get_64(): this SoC has no CONFIG_TIMER_HAS_64BIT_CYCLE_COUNTER, so
 * k_cycle_get_64() returns a constant 0 and a deadline derived from it never
 * expires, turning the poll below into an unbounded spin.
 */
static int acp_log_dma_wait_ch9_idle(uint32_t timeout_us)
{
	uint32_t poll_start = k_cycle_get_32();
	uint32_t poll_span = k_us_to_cyc_ceil32(timeout_us);

	do {
		if (!acp_log_dma_ch9_busy()) {
			return 0;
		}
	} while ((k_cycle_get_32() - poll_start) < poll_span);
...
static void acp_log_dma_isr(const void *arg)
{
	...
	(void)acp_log_dma_wait_ch9_idle(LOG_DMA_POLL_TIMEOUT_US);
...
static void acp_log_dma_descriptor(uint32_t idx, const uint8_t *src, uint32_t dst, uint32_t bytes,
				   bool ioc)
{
...
static int acp_log_dma_run(uint32_t ndesc, bool can_block)
{
	...
	if (can_block && k_sem_take(&acp_log_dma_done, K_USEC(LOG_DMA_POLL_TIMEOUT_US)) == 0) {
		return 0;
	}

	if (acp_log_dma_wait_ch9_idle(LOG_DMA_POLL_TIMEOUT_US) == 0) {
		return 0;
	}
 ...
 */
static int acp_log_dma_copy_to_host(uint32_t staging_off, uint32_t host_off, uint32_t nbytes,
				    bool can_block)
{
	...
	acp_log_dma_descriptor(LOG_DSCR_IDX, log_dma_staging + staging_off, dst_base + host_off,
			       first, can_block && first >= nbytes);
...
static void acp_log_console(const uint8_t *in, size_t length)
{
	...
	for (size_t i = 0; i < length; i++) {
		...
		win[acp_log.write_off % LOG_HOST_BUFFER_SIZE] = in[i];
		acp_log.write_off++;
	}
...
static void init(const struct log_backend *const backend)
{
	...
	const char *msg1 = "ACP Log Backend Initialized (immediate mode)\n";
#else
	acp_log.read_off = 0U;
	...
	k_work_queue_start(&acp_log_flush_wq, acp_log_flush_stack,
			   K_THREAD_STACK_SIZEOF(acp_log_flush_stack), ACP_LOG_FLUSH_WQ_PRIORITY,
			   NULL);

	k_timer_start(&acp_log_flush_timer, K_USEC(LOG_DMA_FLUSH_PERIOD_US),
...
{
	if (!IS_ENABLED(CONFIG_LOG_MODE_DEFERRED) || !acp_log.ready) {
		return;
	}

#if defined(CONFIG_LOG_MODE_DEFERRED)
	/*
	 * The panic can land mid-transfer on a worker flush.  Let channel 9
	 * drain first: reprogramming its descriptors underneath an active
	 * transfer would truncate whatever is already on its way to the host.
	 */
	(void)acp_log_dma_wait_ch9_idle(LOG_DMA_POLL_TIMEOUT_US);
...
static inline void dropped(const struct log_backend *const backend, uint32_t cnt)
{
...
static void process(const struct log_backend *const backend, union log_msg_generic *msg)
{
	...
	/*
	 * log_panic() runs the backend panic() hooks before draining the
	 * deferred queue, and the flush timer and work queue are both dead by
	 * then, so anything staged from here on would never reach the host.
	 * Push each message out as it is formatted instead.
	 */
	if (IS_ENABLED(CONFIG_LOG_MODE_DEFERRED) && acp_log.panicked) {
		acp_log_panic_flush();
	}
}
...
const struct log_backend_api log_backend_acp_api = {
	.process = process,
	.dropped = (IS_ENABLED(CONFIG_LOG_MODE_IMMEDIATE) || IS_ENABLED(CONFIG_LOG_MODE_MINIMAL))
			   ? NULL
			   : &dropped,
	.panic = panic,
