...
static int char_out(uint8_t *data, size_t length, void *ctx)
{
	...
	/* Always return the number of bytes consumed */
	return (int)length;
}
...
/* Log output instance */
LOG_OUTPUT_DEFINE(rpmsg_output, char_out, rpmsg_buffer, CONFIG_LOG_BACKEND_RPMSG_BUFFER_SIZE);
...
static int log_backend_rpmsg_cb(struct rpmsg_endpoint *ept, void *data, size_t len, uint32_t src,
				void *priv)
{
	/* No incoming messages expected - just return success */
