...
static K_SEM_DEFINE(log_init_sync, 0, 1);
...
static const struct log_link *get_link_domain(u8_t *domain_id)
{
	...
	for (int i = 0; i < log_link_count(); i++) {
		link = log_link_get(i);
...
static void runtime_filters_init(void)
{
	/*
	...
	for (int i = 0; i < log_sources_count(Z_LOG_LOCAL_DOMAIN_ID); i++) {
		u32_t *filters = get_dynamic_filter(Z_LOG_LOCAL_DOMAIN_ID, i);
		u8_t level = log_compiled_level_get(Z_LOG_LOCAL_DOMAIN_ID, i);
...
static void link_domain_names_init(const struct log_link *link)
{
	...
	for (u8_t i = 0; i < cnt; i++) {
		...
		if (name) {
			...
			if (len == 0) {
				k_free(name);
				link->ctrl_blk->domain_names[i] = NULL;
			} else {
				heap_used += len;
			}
		} else {
...
static void log_links_init(void)
{
	...
	for (int i = 0; i < log_link_count(); i++) {
		...
		domain_cnt = log_link_domains_count(link);
			    ...
			    u32_t level)
{
	...
	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING)) {
		...
		if (backend == NULL) {
			...
			level = MIN(level, max);
			set_runtime_filter(log_backend_id_get(backend),
					   domain_id, src_id, filters, level);
		}
	}
			    ...
			    u32_t level)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(
		src_id < log_sources_count(Z_LOG_LOCAL_DOMAIN_ID),
			...
			u32_t level)
{
	...
	if (IS_ENABLED(CONFIG_LOG_MULTIDOMAIN)) {
		k_sem_take(&log_init_sync, K_FOREVER);
	}
...
void z_vrfy_z_log_string_from_user(u32_t src_level_val, const char *str)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(domain_id == Z_LOG_LOCAL_DOMAIN_ID,
		"Invalid log domain_id"));
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(
		source_id < log_sources_count(Z_LOG_LOCAL_DOMAIN_ID),
		"Invalid log source id"));
				    ...
				    const u8_t *data, u32_t len)
{
	...
	Z_OOPS(Z_SYSCALL_VERIFY_MSG(
		...
		log_sources_count(Z_LOG_LOCAL_DOMAIN_ID),
		"Invalid log source id"));
