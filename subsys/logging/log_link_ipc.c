...
static void msgs_dropped(const struct log_link_ipc *link_ipc, uint32_t cnt)
{
	LOG_WRN("Remote: %s dropped messages: %d", link_ipc->link->name, cnt);
	z_log_dropped(false, cnt);
}
...
static void dropped_timer_cb(struct k_timer *timer)
{
	...
	msgs_dropped(link_ipc, cnt);
}

static void local_msgs_dropped(const struct log_link_ipc *link_ipc, uint32_t cnt)
{
	uint32_t prev = atomic_add(&link_ipc->data->dropped_cnt, cnt);
	k_timeout_t timeout = K_MSEC(CONFIG_LOG_LINK_IPC_DROPPED_MSG_TIMEOUT);
				...
				struct log_link_config *config)
{
	...
	if (!IS_ENABLED(CONFIG_LOG_LINK_IPC_RX_HOLD) && link_ipc->buffer) {
...
static union log_msg_generic *link_remote_get_msg(const struct log_link *link)
{
	...
	if (IS_ENABLED(CONFIG_LOG_LINK_IPC_RX_HOLD) && link_ipc->msg_buffer) {
				...
				size_t len)
{
	...
	if (!IS_ENABLED(CONFIG_LOG_LINK_IPC_RX_HOLD) && link_ipc->buffer) {
		...
		uint8_t *src = msg->data.log_msg.data;
		size_t offset = 0;
		uint32_t cnt;

		len -= offsetof(struct log_ipc_service_msg, data.log_msg.data);

		/* IPC message may contain multiple log messages, each is put into the mpsc buffer
		 * as a separate item.
		 */
		for (cnt = 0; cnt < msg->status; cnt++) {
			union mpsc_pbuf_generic *src_msg = (union mpsc_pbuf_generic *)&src[offset];
			size_t wlen = log_msg_generic_get_wlen(src_msg);
			union mpsc_pbuf_generic *dst_msg;

			__ASSERT_NO_MSG(offset + wlen * sizeof(uint32_t) <= len);

			dst_msg = mpsc_pbuf_alloc(link_ipc->buffer, wlen, timeout);
			if (dst_msg == NULL) {
				local_msgs_dropped(link_ipc, msg->status - cnt);
				break;
			}

			memcpy(dst_msg, src_msg, wlen * sizeof(uint32_t));
			mpsc_pbuf_commit(link_ipc->buffer, dst_msg);
			offset += wlen * sizeof(uint32_t);
		}

		z_log_msg_remote_notify(cnt);
	} else {
		struct log_link_ipc_msg_buffer *msg_buffer = spsc_acquire(link_ipc->msg_buffer);
		int err;

		if (msg_buffer == NULL) {
			local_msgs_dropped(link_ipc, msg->status);
			return;
		}
...
void log_link_ipc_received_cb(const void *buffer, size_t len, void *priv)
{
	...
	if (msg->status == Z_LOG_IPC_SERVICE_STATUS_ERR) {
		data->status = -EIO;
		goto exit;
	} else {
		data->status = 0;
	...
	case Z_LOG_IPC_SERVICE_ID_DROPPED:
		msgs_dropped(link_ipc, msg->data.dropped.dropped);
		rsp = false;
		break;
...
#if defined(IPC_NODE) && !defined(CONFIG_LOG_LINK_IPC_MULTI_CLI)
...
LOG_LINK_IPC_DEFINE(cpu1, IPC_NODE,
	COND_CODE_1(CONFIG_LOG_LINK_IPC_RX_HOLD,
		(0), (CONFIG_LOG_LINK_IPC_BUFFER_SIZE)), true);
#endif
