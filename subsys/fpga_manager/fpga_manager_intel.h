 ...
 */
#ifndef ZEPHYR_SUBSYS_FPGA_MANAGER_INTEL_H_
#define ZEPHYR_SUBSYS_FPGA_MANAGER_INTEL_H_
...
struct fm_lock_data {
	uint32_t response_status : 1;
	struct k_sem reconfig_data_sem;
	struct k_sem reconfig_data_send_done_sem;
};
...
enum smc_cmd_code {
	...
	FPGA_RECONFIG_DATA = 0x08,
	/* SMC COMMAND ID to check Reconfig status to SDM via mailbox */
	FPGA_RECONFIG_STATUS
};

enum fpga_config_stages {
	/* Will init SiP SVC client services  */
	FPGA_CONFIG_INIT,
	/* Disable all bridges */
	FPGA_BRIDGE_DISABLE,
	/* FPGA cancel stage */
	...
	FPGA_RECONFIG_SEND,
	/* Send the bitstream data */
	FPGA_RECONFIG_DATA_SEND,
	/* Check the Reconfig status */
...
enum mbox_reconfig_resp {
	/* Mailbox reconfig header */
	MBOX_RECONFIG_HEADER,
	/* Mailbox reconfig dma count */
	MBOX_RECONFIG_DMA_COUNT,
	/* Mailbox reconfig dma size */
	MBOX_RECONFIG_DMA_SIZE
};
