...
static struct settings_file config_init_settings_file = {
	.cf_name = CONFIG_NSETTINGS_FS_FILE,
	.cf_maxlines = CONFIG_NSETTINGS_FS_MAX_LINES
};
...
#elif defined(CONFIG_NSETTINGS_FCB)
#include "fcb.h"
#include "settings_fcb.h"

static struct flash_sector settings_fcb_area[
		CONFIG_NSETTINGS_FCB_NUM_AREAS + 1];

static struct settings_fcb config_init_settings_fcb = {
	.cf_fcb.f_magic = CONFIG_NSETTINGS_FCB_MAGIC,
	.cf_fcb.f_sectors = settings_fcb_area,
};

static void settings_init_fcb(void)
{
	u32_t cnt = CONFIG_NSETTINGS_FCB_NUM_AREAS + 1;
	int rc;
	const struct flash_area *fap;

	rc = flash_area_get_sectors(CONFIG_NSETTINGS_FCB_FLASH_AREA, &cnt,
				    settings_fcb_area);
	if (rc != 0 && rc != -ENOMEM) {
		k_panic();
	}
	...
	if (rc != 0) {
		rc = flash_area_open(CONFIG_NSETTINGS_FCB_FLASH_AREA, &fap);
...
int settings_subsys_init(void)
{
	 ...
	 * Must be called after root FS has been initialized.
	 */
	err = fs_mkdir(CONFIG_NSETTINGS_FS_DIR);
	/*
	 * The following lines mask the file exist error.
	 */
	if (err == -EEXIST) {
