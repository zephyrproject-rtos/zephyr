...
static uint32_t calculate_backoff_timeout(struct zbus_proxy_agent_config *config, uint8_t attempts)
{
	...
	if (u32_mul_overflow(config->tracking.ack_timeout_start_ms, multiplier, &timeout)) {
...
static int zbus_proxy_agent_stop_tracking(struct zbus_proxy_agent_config *config, uint32_t msg_id)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(&config->tracking.tracking_msg_list, buf, tmp, node) {
		...
		if (*msg_id_ptr == msg_id) {
			...
			/* If in ISR context OR work is running, it is not safe to cancel
			 * the work item. Just remove reference to config so that timeout
			 * handler knows it has been ACKed and does not attempt to resend
			 * or reschedule itself.
			 */
			data->config = NULL;

			if (!k_is_in_isr() &&
			    !(k_work_busy_get(&data->work.work) & K_WORK_RUNNING)) {
				/* Safe to cancel */
...
zbus_proxy_agent_find_tracking_msg(struct zbus_proxy_agent_config *config, uint32_t msg_id)
{
	struct net_buf *buf;
	uint32_t *msg_id_ptr;
...
static void zbus_proxy_agent_ack_timeout_handler(struct k_work *work)
{
	...
	key = irq_lock();
	current_data = zbus_proxy_agent_find_tracking_msg(data->config, expected_msg_id);
...
static int zbus_proxy_agent_init(struct zbus_proxy_agent_config *config)
{
	...
	LOG_DBG("Proxy agent %s with backend initialized successfully", config->backend.name);
	return 0;
}
