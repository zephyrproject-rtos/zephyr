...
LOG_MODULE_REGISTER(zbus_proxy_agent_uart, CONFIG_ZBUS_PROXY_AGENT_LOG_LEVEL);

static inline uint8_t *get_rx_buffer(struct zbus_proxy_agent_uart_config *config, int idx)
{
	return (uint8_t *)config->rx.async_rx_buf + (idx * config->rx.async_rx_buf_size);
}
						...
						void *config)
{
	...
	case UART_RX_RDY:
		if (!evt->data.rx.buf || evt->data.rx.len == 0) {
			LOG_ERR("Received invalid UART data");
			break;
		}

		LOG_DBG("Received UART chunk: %zu bytes", evt->data.rx.len);

		ret = process_rx_chunk(uart_config, evt->data.rx.buf, evt->data.rx.len);
		if (ret < 0) {
	...
	case UART_RX_BUF_REQUEST:
		ret = uart_rx_buf_rsp(dev, get_rx_buffer(uart_config, uart_config->rx.buf_idx),
				      uart_config->rx.async_rx_buf_size);
		if (ret < 0) {
			LOG_ERR("Failed to provide RX buffer: %d", ret);
		} else {
			uart_config->rx.buf_idx =
				(uart_config->rx.buf_idx + 1) % uart_config->rx.async_rx_buf_count;
			LOG_DBG("Provided RX buffer %d (size: %zu bytes)", uart_config->rx.buf_idx,
				uart_config->rx.async_rx_buf_size);
		}
		break;
...
static int zbus_proxy_agent_uart_backend_init(void *config)
{
	...
	ret = uart_rx_enable(uart_config->dev, get_rx_buffer(uart_config, 0),
			     uart_config->rx.async_rx_buf_size, uart_config->rx.rx_buf_timeout_us);
	if (ret < 0 && ret != -EBUSY) {
		LOG_ERR("Failed to enable UART RX: %d", ret);
		return ret;
	}

	LOG_DBG("ZBUS Proxy agent UART initialized for device %s (chunk size: %zu bytes)",
		uart_config->dev->name, uart_config->rx.async_rx_buf_size);
	return 0;
}
...
static int zbus_proxy_agent_uart_backend_send(void *config, uint8_t *data, size_t length)
{
	...
	if (!uart_config || !data || length == 0) {
		LOG_ERR("Invalid parameters for UART backend send");
		return -EINVAL;
	}

	padded_frame_size = calculate_padded_frame_size(length, uart_config->rx.async_rx_buf_size);
						     ...
						     void *user_data)
{
	...
	if (!uart_config || !recv_cb) {
		LOG_ERR("Invalid parameters for setting UART backend receive callback");
		return -EINVAL;
	}
...
/* Instantiate proxy agents from on device tree nodes */
DT_FOREACH_STATUS_OKAY(zephyr_zbus_proxy_agent_uart, ZBUS_PROXY_AGENT_INSTANTIATE_UART);
