...
void async_listener_work_handler(struct k_work *item)
{
	...
	__ASSERT(k_fifo_is_empty(async_listener->message_fifo) == 0, "fifo element required");
	...
	if (buf == NULL) {
		LOG_ERR("Could not retrieve message from fifo");
		return;
	}
	...
	/* The work should be executed up to the fifo is empty */
	if (k_fifo_is_empty(async_listener->message_fifo) == 0) {

		struct k_work_q *wq = NULL;

		IF_ENABLED(CONFIG_ZBUS_ASYNC_LISTENER_ISOLATED_WORK_QUEUE, (wq = chan->work_queue;))

		if (wq != NULL) {
			k_work_submit_to_queue(wq, item);
		} else {
			k_work_submit(item);
		}
	}
					...
					struct net_buf *buf)
{
	...
	case ZBUS_OBSERVER_ASYNC_LISTENER_TYPE: {
		...
		if (wq != NULL) {
			ret = k_work_submit_to_queue(wq, obs->work);
		} else {
			ret = k_work_submit(obs->work);
		}
		if (ret < 0) {
			return ret;
		}
...
#endif /* CONFIG_ZBUS_OBSERVER_ASYNC_LISTENER_TYPE */
	default:
...
static inline int _zbus_vded_exec(const struct zbus_channel *chan, k_timepoint_t end_time)
{
...
#if defined(CONFIG_ZBUS_MSG_SUBSCRIBER) || defined(CONFIG_ZBUS_ASYNC_LISTENER)
...
#endif /* CONFIG_ZBUS_MSG_SUBSCRIBER || CONFIG_ZBUS_ASYNC_LISTENER */
		...
		if (err) {
			...
			if (err == -ENOMEM) {
				if (IS_ENABLED(CONFIG_ZBUS_MSG_SUBSCRIBER) ||
				    IS_ENABLED(CONFIG_ZBUS_ASYNC_LISTENER)) {
					net_buf_unref(buf);
				}
				return err;
			}
	...
	if (IS_ENABLED(CONFIG_ZBUS_MSG_SUBSCRIBER) || IS_ENABLED(CONFIG_ZBUS_ASYNC_LISTENER)) {
		net_buf_unref(buf);
	}
