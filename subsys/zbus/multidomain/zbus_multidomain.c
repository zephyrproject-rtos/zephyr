...
static void zbus_proxy_agent_sent_ack_timeout_handler(struct k_work *work)
{
	...
	LOG_WRN("Sent message ID %d timed out waiting for acknowledgment", data->msg.id);

	data->transmit_attempts++;
	if (data->transmit_attempts < CONFIG_ZBUS_MULTIDOMAIN_MAX_TRANSMIT_ATTEMPTS) {
		LOG_WRN("Retrying to send message ID %d (attempt %d)", data->msg.id,
			data->transmit_attempts);
		if (data->config) {
			...
			if (ret < 0) {
				...
				LOG_DBG("Resent message ID %d (attempt %d)", data->msg.id,
					data->transmit_attempts);
			}
		}
	} else {
		LOG_ERR("Max transmit attempts (%d) reached for message ID %d, giving up",
			CONFIG_ZBUS_MULTIDOMAIN_MAX_TRANSMIT_ATTEMPTS, data->msg.id);
		if (data->config) {
			int ret =
				zbus_proxy_agent_sent_ack_timeout_stop(data->config, data->msg.id);
			if (ret < 0 && ret != -ENOENT) {
