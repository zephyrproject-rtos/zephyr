...
uint8_t process_buf[2048];

static void gnss_rtk_process_work_handler(struct k_work *work)
{
	static uint8_t work_buf[2048];
	uint32_t len = ring_buf_get(&process_ringbuf, work_buf, ARRAY_SIZE(work_buf));
	uint32_t offset = 0;
	int err;
	...
	do {
		...
		err = rtk_decoder_frame_get(work_buf + offset, len - offset, &frame, &frame_len);
		if (err == 0) {
			LOG_HEXDUMP_DBG(frame, frame_len, "Frame received");

			/* Publish results */
			struct rtk_data rtk_data = {
				.data = frame,
				.len = frame_len,
			};
			rtk_publish_data(&rtk_data);

			offset += frame_len;
		}
	} while ((err == 0) && (len > offset));
}
...
static void rtk_uart_isr_callback(const struct device *dev, void *user_data)
{
	...
	if (uart_irq_rx_ready(dev)) {
		 ...
		 * processing.
		 */
		k_work_reschedule(&gnss_rtk_process_work, K_MSEC(10));
	}
}
