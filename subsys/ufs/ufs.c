...
static inline void ufshc_write_reg(const struct ufs_host_controller *ufshc,
				   uintptr_t reg_offset, uint32_t data)
{
	sys_write32(data, (uintptr_t)(ufshc->mmio_base) + reg_offset);
}

static inline uint32_t ufshc_read_reg(const struct ufs_host_controller *ufshc,
				      uintptr_t reg_offset)
{
	return sys_read32((uintptr_t)(ufshc->mmio_base) + reg_offset);
}
...
static void ufshc_find_slot_in_trl(struct ufs_host_controller *ufshc, uint32_t *slot)
{
	...
	for (uint32_t index = 0; index < ufshc->xfer_req_depth; index++) {
 ...
 */
int ufshc_send_uic_cmd(struct ufs_host_controller *ufshc, struct ufshc_uic_cmd *uic_cmd)
{
	int ret;
	uint32_t arg1;
	uint32_t events;
 ...
 * @return 0 on success, negative error code on failure
 */
int ufshc_configure_speedgear(struct ufs_host_controller *ufshc, uint32_t speed_gear)
{
	int ret = 0;
...
static int32_t ufshc_get_lun_info(struct ufs_host_controller *ufshc)
{
	...
	for (uint8_t Index = 0U; Index < (dev_info->max_lu_supported); Index++) {
 ...
 * @return 0 on success, negative error code on failure.
 */
int ufs_init(const struct device *ufshc_dev, struct ufs_host_controller **ufshc)
{
	int err;
 ...
 * @return 0 on success, a negative error code on failure.
 */
int ufshc_exec_raw_upiu_cmd(struct ufs_host_controller *ufshc,
 ...
 * @return 0 on success, a negative error code on failure.
 */
int ufshc_rw_descriptors(struct ufs_host_controller *ufshc,
 ...
 * @return 0 on success, a negative error code on failure.
 */
int ufshc_rw_attributes(struct ufs_host_controller *ufshc,
 ...
 * @return 0 on success, a negative error code on failure.
 */
int ufshc_rw_flags(struct ufs_host_controller *ufshc,
