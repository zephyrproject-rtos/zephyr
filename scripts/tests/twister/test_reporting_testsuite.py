...
def test_discard_report(class_testrunner, platforms_list, all_scenarios_dict, caplog, tmpdir):
    ...
    class_testrunner.suites = all_scenarios_dict
    filename = tmpdir.mkdir("test_discard").join("discard_report.csv")
    with pytest.raises(SystemExit):
        class_testrunner.discard_report(filename)
    ...
    class_testrunner.apply_filters(**kwargs)
    class_testrunner.discard_report(filename)
...
def test_csv_report(class_testrunner, instances_fixture, tmpdir):
    ...
    class_testrunner.instances = instances_fixture
    filename = tmpdir.mkdir("test_csv").join("twister_csv_report.csv")
    class_testrunner.csv_report(filename)
    ...
    for instance in class_testrunner.instances.values():
...
def test_xunit_report(class_testrunner, test_data,
                      instances_fixture, platforms_list, all_scenarios_dict):
    ...
    class_testrunner.apply_filters(**kwargs)
    class_testrunner.instances = instances_fixture
    inst1 = class_testrunner.instances.get("demo_board_2/scripts/tests/twister/test_data/testcases/tests/test_a/test_a.check_1")
    inst2 = class_testrunner.instances.get("demo_board_2/scripts/tests/twister/test_data/testcases/tests/test_a/test_a.check_2")
    inst1.status = "failed"
    inst2.status = "skipped"

    filename = test_data + "twister.xml"
    fails, passes, errors, skips = class_testrunner.xunit_report(filename)
    assert os.path.exists(filename)
    ...
    for index in range(1, len(class_testrunner.instances)+1):
...
def test_compare_metrics(class_testrunner, test_data, instances_fixture, caplog):
...
def test_target_report(class_testrunner, instances_fixture, tmpdir_factory):
    ...
    class_testrunner.xunit_report = MagicMock(side_effect=class_testrunner.xunit_report)
    class_testrunner.target_report(outdir, "abc", append=False)
    assert class_testrunner.instances
    class_testrunner.xunit_report.assert_called()
