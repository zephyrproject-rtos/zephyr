...
def test_check_build_or_run(class_testrunner, monkeypatch, all_scenarios_dict, platforms_list, build_only, slow, harness, platform_type, platform_sim, device_testing, fixture, expected):
    ...
    testcase = class_testrunner.suites.get('scripts/tests/twister/test_data/testcases/tests/test_a/test_a.check_1')
    ...
    testinstance = TestInstance(testcase, platform, class_testrunner.outdir)
...
def test_create_overlay(class_testrunner, all_scenarios_dict, platforms_list, enable_asan, enable_ubsan, enable_coverage, coverage_platform, platform_type, expected_content):
    """Test correct content is written to testcase_extra.conf based on if conditions
    TO DO: Add extra_configs to the input list"""
    class_testrunner.suites = all_scenarios_dict
    testcase = class_testrunner.suites.get('scripts/tests/twister/test_data/testcases/samples/test_app/sample_test.app')
    class_testrunner.platforms = platforms_list
    platform = class_testrunner.get_platform("demo_board_2")
...
def test_calculate_sizes(class_testrunner, all_scenarios_dict, platforms_list):
