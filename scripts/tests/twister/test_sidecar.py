...
def test_can_configure_reads_namespaced_config(tmp_path):
    instance = _make_instance(tmp_path, {"can": {"iface": "vcan9", "tools_apps": ["echo {iface}"]}})
    sidecar = CanSidecar()
    sidecar.configure(instance)
...
def test_can_setup_skips_without_root(tmp_path):
    ...
    with (
        ...
        mock.patch("subprocess.run", return_value=mock.Mock(returncode=1, stderr="")),
    ):
...
def test_can_host_ready_needs_ip_and_root(tmp_path):
    instance = _make_instance(tmp_path)
    sidecar = CanSidecar()
    sidecar.configure(instance)
    ...
    with (
        mock.patch("shutil.which", return_value="/sbin/ip"),
...
def test_can_setup_skips_modprobe_when_missing(tmp_path):
    instance = _make_instance(tmp_path)
    sidecar = CanSidecar()
    sidecar.configure(instance)

    def _which(tool):
        return None if tool == "modprobe" else f"/sbin/{tool}"

    with (
        mock.patch("shutil.which", side_effect=_which),
        mock.patch("os.geteuid", return_value=0),
        mock.patch("subprocess.run", return_value=mock.Mock(returncode=0, stderr="")) as run_mock,
    ):
        assert sidecar.setup() is True

    commands = [call.args[0] for call in run_mock.call_args_list]
    assert not any("modprobe" in cmd for cmd in commands)
...
def test_can_teardown_removes_iface(tmp_path):
    ...
    with (
        mock.patch("os.geteuid", return_value=0),
        mock.patch("subprocess.run", return_value=mock.Mock(returncode=0)) as run_mock,
    ):
        sidecar.teardown()
