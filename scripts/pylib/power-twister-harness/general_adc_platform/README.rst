...
The firmware running on GeneralPowerShield is described at :zephyr_file:`README.rst
<samples/drivers/adc/adc_power_measure/README.rst>`
...
the channels_p and channels_n is mapped to the GeneralPowerShield adc channel settings.
...
6. The power sheild support two dft mode 'console' or 'shell', os environment ``POWER_SHIELD_SHELL``
   is used to control this. Note: samples need configure ``CONFIG_SHELL_ADC_PS=y``
