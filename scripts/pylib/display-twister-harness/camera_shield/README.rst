...
1. Setup camera to capture display content
 ...
 - uvc compatible camera with 200M resolution
...
video fingerprint for given screen shots will be recorded in directory './fingerprints' by default
   ...
   - set environment variable to "DISPLAY_TEST_FOLDER"

```bash
DISPLAY_TEST_FOLDER=~/camera_shield/
```

3. Run test
```bash
# export the fingerprints path
export DISPLAY_TEST_FOLDER=<your path with fingerprints subfolder inside>
...
scripts/twister --device-testing --hardware-map /home/ubuntu/nxp/frdm_mcxn947/map.yaml -T tests/drivers/display/display_check/
...
1. the captured fingerprint has build in "name" defined in "metadata" when generating.
2. the DUT case will match the name with captured one.
3. you can put mutliply fingure prints in one folder, it will increase compare time,
