...
class TwisterHarnessConfig:
    ...
    def create(cls, config: pytest.Config) -> TwisterHarnessConfig:
        ...
        if test_params_file := get_test_param_file(config):
            ...
            test_params = HarnessPytestConfig(platform='unknown')
        ...
        if config.option.west_flash_extra_args:
            ...
            west_flash_extra_args = [w.strip() for w in next(csv.reader([test_params.west_flash_extra_args]))]
        ...
        if not test_params.duts:
            ...
            device_from_cli = DeviceConfig(
                ...
                platform=config.option.platform or test_params.platform,
                ...
                flash_before=config.option.flash_before or test_params.flash_before,
                ...
                fixtures=config.option.fixtures or test_params.twister_fixtures,
                extra_test_args=config.option.extra_test_args
            )
            return cls(devices=[device_from_cli], test_params=test_params)
        ...
        for dut in test_params.duts:
            ...
            device = DeviceConfig(
                ...
                platform=dut.platform or config.option.platform,
                serial_configs=serial_configs,
                ...
                fixtures=config.option.fixtures or dut.fixtures,
                extra_test_args=config.option.extra_test_args
            )
