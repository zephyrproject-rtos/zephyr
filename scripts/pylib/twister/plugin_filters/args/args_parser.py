...
class _FilterDictionary:
    ...
    def __init__(self, path, args:Optional[list]=None, kwargs:Optional[dict]=None):
        self.path = path
...
def _handle_json_path_filter(filter_args):
    ...
    if not os.path.isfile(filter_args):
        logger.error(
            "Provided path %s doesn't lead to a file. "
            "Halting twister process...",
            filter_args
        )
        sys.exit(1)
...
def parse_arguments(options):
    if filter_args := options.plugin_filter:
        ...
        options.plugin_filter = _handle_json_filter(
            filter_args, _handle_json_string_filter
        )
    elif filter_args := options.plugin_filter_json_path:
        options.plugin_filter = _handle_json_filter(
            filter_args, _handle_json_path_filter
        )
