...
class ProjectBuilder(FilterBuilder):
    ...
    def cmake_assemble_args(extra_args, handler, conf_files, extra_conf_files, extra_overlay_confs,
        ...
        # Merge all extra Kconfig fragments into a single EXTRA_CONF_FILE, keeping the
        # order in which they have to be applied: extra_conf_files first, then the
        # overlay fragments and finally the fragment generated by twister itself.
        extra_conf = list(extra_conf_files) + list(extra_overlay_confs)

        additional_overlay_path = os.path.join(
            build_dir, "twister", "testsuite_extra.conf"
        )
        if os.path.exists(additional_overlay_path):
            extra_conf.append(additional_overlay_path)
