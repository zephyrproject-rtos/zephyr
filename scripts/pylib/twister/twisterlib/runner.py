...
class TwisterRunner:
    ...
    def _required_applications_are_ready(
    ...
    def _required_applications_failed(
            self, instance: TestInstance, processing_ready: dict[str, TestInstance]
    ):
        ...
        return found_failed_app

    def required_applications_processed(self, instance: TestInstance, processing_queue: deque,
                                        processing_ready: dict[str, TestInstance], task):
        if not instance.required_applications:
            return True

        if not self._required_applications_are_ready(instance, processing_ready):
        ...
        if self._required_applications_failed(instance, processing_ready):
    ...
    def _pipeline_mgr(self, processing_queue: deque, processing_ready: dict[str, TestInstance],
        ...
        while True:
            try:
                ...
                if not self.required_applications_processed(
        ...
        try:
            if sys.platform == 'linux':
                with self.jobserver.get_job():
                    return self._pipeline_mgr(processing_queue, processing_ready, lock, results)
            else:
                return self._pipeline_mgr(processing_queue, processing_ready, lock, results)
        except Exception as e:
