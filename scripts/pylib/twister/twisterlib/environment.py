...
def add_parse_arguments(parser=None):
    ...
    case_select = parser.add_argument_group(
        "Test case selection",
        """
Artificially long but functional example:
   ...
   "kernel.fifo.poll" is one of the test section names in
    ...
    test_plan_report_xor.add_argument(
        ...
        metavar="FILENAME",
        action="store",
        help="""Write a list of tests and platforms to be run
        to %(metavar)s file and stop execution.
    ...
    case_select.add_argument(
        ...
        default=[],
        type=norm_path,
        help="""Base directory to recursively search for test cases.
        All testcase.yaml files under here will be processed.
        May be called multiple times.
        Defaults to the samples/ and tests/ directories at the base of the Zephyr tree.
        """,
    )
    ...
    test_plan_report_xor.add_argument(
        "--list-tests",
        action="store_true",
        help="""List of all sub-test functions recursively found
        in all '--testsuite-root' arguments.
        Note different sub-tests can share the same section name
        and come from different directories.
        The output is flattened and reports '--sub-test' names only, not their directories.
        For instance net.socket.getaddrinfo_ok and net.socket.fd_set
        belong to different directories.
        """,
    )

    test_plan_report_xor.add_argument(
        "--test-tree", action="store_true", help="Output the test plan in a tree form."
    )

    platform_group_option.add_argument(
        "-G", "--integration", action="store_true", help="Run integration tests."
    )

    platform_group_option.add_argument(
        "--emulation-only", action="store_true", help="Only build and run emulation platforms."
    )

    run_group_option.add_argument(
        "--device-testing",
        action="store_true",
        help="""Test on device directly.
        Specify the serial device to use with the '--device-serial' option.
        """,
    )
    ...
    run_group_option.add_argument(
        ...
        and create a hardware map file to be used with '--device-testing'
        """,
    )
    ...
    device.add_argument(
        ...
        E.g 'twister --device-testing --device-serial-pty <script>'
        """,
    )
    ...
    device.add_argument(
        ...
        This will be used for testing on hardware that is listed in the file.
        """,
    )
    ...
    parser.add_argument(
        ...
        This is useful for devices that share the same port for programming and serial console
        or use soft-USB, where flash must come first.
        """,
    )
    ...
    test_or_build.add_argument(
        ...
        action="store_true",
        help="Generate artifacts for testing, do not attempt to run the code on targets.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="""Globally adjust tests timeouts by specified multiplier.
        The resulting test timeout would be multiplication of test timeout value,
        board-level timeout multiplier and global timeout multiplier (this parameter)
        """,
    )
    ...
    test_xor_subtest.add_argument(
        ...
        action="append",
        type=norm_path,
        help="""Run only the specified testsuite scenario.
        These are named by <path/relative/to/Zephyr/base/section.name.in.testcase.yaml>
        """,
    )
    ...
    test_xor_subtest.add_argument(
        "--sub-test",
        action="append",
        help="""Recursively find sub-test functions
        and run the entire test section where they were found, including all sibling test functions.
        Sub-tests are named by: section.name.in.testcase.yaml.function_name_without_test_prefix
    ...
    parser.add_argument(
        "--pytest-args",
        action="append",
        help="""Pass additional arguments to the pytest subprocess.
        This parameter will extend the pytest_args from the harness_config in YAML file.
        """,
    )
    ...
    valgrind_asan_group.add_argument(
        "--enable-valgrind",
        action="store_true",
        help="""Run binary through valgrind and check for several memory access errors.
        Valgrind needs to be installed on the host.
        This option only works with host binaries
        such as those generated for the native_sim configuration
        and is mutually exclusive with '--enable-asan'.
        """,
    )
    ...
    valgrind_asan_group.add_argument(
        "--enable-asan",
        action="store_true",
        help="""Enable address sanitizer to check for several memory access errors.
        Libasan needs to be installed on the host.
        This option only works with host binaries
        such as those generated for the native_sim configuration
        and is mutually exclusive with '--enable-valgrind'.
        """,
    )
    ...
    parser.add_argument(
        ...
        All .yaml files in the directory will be processed.
        The directory should have the same structure in the main Zephyr tree:
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Allow to use the pytest plugin installed by pip for pytest tests.",
    )
    ...
    parser.add_argument(
        ...
        If unspecified, test all arches.
    ...
    parser.add_argument(
        ...
        This option is useful when running a large number of tests on different hosts
    ...
    parser.add_argument(
        ...
        "--coverage",
        action="store_true",
        help="Generate coverage reports. Implies '--enable-coverage'.",
    )
    ...
    parser.add_argument(
        ...
        "--clobber-output",
        action="store_true",
        help="""Cleaning the output directory will simply delete it
        instead of the default policy of renaming.
        """,
    )

    parser.add_argument(
        "--cmake-only", action="store_true", help="Only run cmake, do not build or run the tests."
    )

    parser.add_argument(
        "--coverage-basedir", default=ZEPHYR_BASE, help="Base source directory for coverage report."
    )
    ...
    parser.add_argument(
        ...
        Defaults to what was selected with '--platform'.
        """,
    )
    ...
    parser.add_argument(
        ...
        help=f"""Output formats to use for generated coverage reports, as a comma-separated list.
        Valid options for 'gcovr' tool are:
        {','.join(supported_coverage_formats['gcovr'])}
        (html by default).
        Valid options for 'lcov' tool are:
        {','.join(supported_coverage_formats['lcov'])}
        (html,lcov by default).
        """,
    )
    ...
    parser.add_argument(
        ...
        By default, no levels are used for filtering and do the selection based on existing filters.
        """,
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="""Disable extended test suite name verification at the beginning of Ztest test.
        This option could be useful for tests or platforms,
        which from some reasons cannot print early logs.
        """,
    )
    ...
    parser.add_argument(
        "--enable-coverage", action="store_true", help="Enable code coverage using gcov."
    )
    ...
    parser.add_argument(
        ...
        Libasan needs to be installed on the host.
        This option only works with host binaries
        such as those generated for the native_sim configuration and when '--enable-asan' is given.
        """,
    )
    ...
    parser.add_argument(
        ...
        to check for undefined behaviour during program execution.
        It uses an optional runtime library to provide better error diagnostics.
        This option only works with host binaries
        such as those generated for the native_sim configuration.
        """,
    )
    ...
    parser.add_argument(
        "--filter",
        choices=['buildable', 'runnable'],
        default='runnable' if "--device-testing" in sys.argv else 'buildable',
        help="""Filter tests to be built and executed.
        By default everything is built
        and if a test is runnable (emulation or a connected device), it is run.
        This option allows, for example, to only build tests that can actually be run.
        Runnable is a subset of buildable.
        """,
    )
    ...
    parser.add_argument(
        "--force-toolchain",
        action="store_true",
        help="Do not filter based on toolchain, use the set toolchain unconditionally",
    )
    ...
    footprint_group.add_argument(
        ...
        action="store_true",
        help="""Generate detailed json reports with ROM/RAM symbol sizes
        for each test image built using additional build option '--target footprint'.
        """,
    )
    ...
    footprint_group.add_argument(
        ...
        choices=['all', 'ROM', 'RAM'],
        const="all",
        help="""Select which memory area symbols' data to collect
        as 'footprint' property of each test suite built,
        and report in twister_footprint.json together
    ...
    footprint_group.add_argument(
        ...
        warn the user for any of the footprint metric change
        which is greater or equal to the specified percentage value.
    ...
    footprint_group.add_argument(
        ...
        action="append",
        metavar='FILENAME',
        help="""Ignore all other command line options and just produce a report to stdout
        with ROM/RAM section sizes on the specified binary images.
        """,
    )
    ...
    parser.add_argument(
        ...
        "--inline-logs",
        action="store_true",
        help="Upon test failure, print relevant log data to stdout instead of just a path to it.",
    )

    parser.add_argument(
        "--ignore-platform-key", action="store_true", help="Do not filter based on platform key."
    )
    ...
    test_plan_report_xor.add_argument(
        "--list-tags", action="store_true", help="List all tags occurring in selected tests."
    )

    parser.add_argument(
        "--log-file", metavar="FILENAME", action="store", help="Specify a file where to save logs."
    )
    ...
    parser.add_argument(
        ...
        const='pass',
        nargs='?',
        help="""Cleanup test artifacts.
        The default behavior is 'pass' which only removes artifacts of passing tests.
        If you wish to remove all artificats including those of failed tests, use 'all'.""",
    )
    ...
    test_xor_generator.add_argument(
        "-k", "--make", action="store_true", help="Use the unix Makefile generator with CMake."
    )
    ...
    parser.add_argument(
        ...
        "--no-clean",
        action="store_true",
        help="""Re-use the outdir before building.
        Will result in faster compilation since builds will be incremental.
        """,
    )
    ...
    parser.add_argument(
        "--aggressive-no-clean",
        action="store_true",
        help="""Re-use the outdir before building and do not re-run cmake.
        Will result inmuch faster compilation since builds will be incremental.
        This option might result in build failures and inconsistencies
        if dependencies change or when applied on a significantly changed code base.
        Use on your own risk.
        It is recommended to only use this option for local development
        and when testing localized change in a subsystem.
        """,
    )
    ...
    parser.add_argument(
        ...
        Names will follow PATH_TO_TEST/SCENARIO_NAME schema
    ...
    parser.add_argument(
        ...
        With this arg a test name will be a scenario name e.g. sample.basic.helloworld.
        """,
    )
    ...
    parser.add_argument(
        "-o",
        "--report-dir",
        help="""Output reports containing results of the test run into the specified directory.
        The output will be both in JSON and JUNIT format (twister.json and twister.xml).
        """,
    )

    parser.add_argument(
        "--overflow-as-errors", action="store_true", help="Treat RAM/SRAM overflows as errors."
    )

    parser.add_argument(
        "--report-filtered", action="store_true", help="Include filtered tests in the reports."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="""Exclude platforms and do not build or run any tests on those platforms.
    ...
    parser.add_argument(
        ...
        action='store_true',
        help="""With '--generate-hardware-map',
        tries to use persistent names for serial devices on platformsthat support this feature
        (currently only Linux).
        """,
    )

    parser.add_argument("--vendor", action="append", default=[], help="Vendor filter for testing")
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="""Platform filter for testing. This option may be used multiple times.
        Test suites will only be built/run on the platforms specified.
        If this option is not used, then platforms marked as default in the platform metadata file
        will be chosen to build and test.
        """,
    )
    ...
    parser.add_argument(
        "--pre-script",
        help="""Specify a pre script.
        This will be executed before device handler open serial port and invoke runner.
        """,
    )
    ...
    parser.add_argument(
        ...
        action="append",
        metavar="FILENAME",
        help="""Load list of test scenarios under quarantine.
        The entries in the file need to correspond to the test scenarios names
        as in corresponding tests .yaml files.
        These scenarios will be skipped with quarantine as the reason.
        """,
    )
    ...
    parser.add_argument(
        "--quarantine-verify",
        action="store_true",
        help="""Use the list of test scenarios under quarantine
        and run them to verify their current status.
        """,
    )
    ...
    parser.add_argument(
        "--report-suffix",
        help="""Add a suffix to all generated file names,
        for example to add a version or a commit ID.
        """,
    )
    ...
    parser.add_argument(
        "--report-all-options",
        action="store_true",
        help="""Show all command line options applied, including defaults,
        as environment.options object in twister.json. Default: show only non-default settings.
        """,
    )
    ...
    parser.add_argument(
        "--retry-build-errors", action="store_true", help="Retry build errors as well."
    )
    ...
    parser.add_argument(
        ...
        help="""Execute time-consuming test cases that have been marked as 'slow' in testcase.yaml.
        Normally these are only built.
        """,
    )
    ...
    parser.add_argument(
        "--enable-slow-only",
        action="store_true",
        help="""Execute time-consuming test cases
        that have been marked as 'slow' in testcase.yaml only.
        This also sets the option '--enable-slow'
        """,
    )

    parser.add_argument(
        "--seed", type=int, help="Seed for native_sim pseudo-random number generator"
    )
    ...
    parser.add_argument(
        ...
        The shortened build path will be used by CMake
        for generating the build system and executing the build.
        Use this option if you experience build failures related to path length,
        for example on Windows OS.
        This option can only be used with '--ninja' argument (to use Ninja build generator).
        """,
    )
    ...
    parser.add_argument(
        ...
        Default is to not do any tag filtering.
        Multiple invocations are treated as a logical 'or' relationship.
        """,
    )

    parser.add_argument(
        "--timestamps", action="store_true", help="Print all messages with time stamps."
    )
    ...
    parser.add_argument(
        ...
        "--no-update",
        action="store_true",
        help="""Do not update the results of the last run.
        This option is only useful when reusing the same output directory of Twister,
        for example when re-running failed tests with '--only-failed' or '--no-clean'.
    ...
    parser.add_argument(
        ...
        nargs='?',
        const=[],
        help="""Uses west instead of ninja or make to flash when running with '--device-testing'.
        Supports comma-separated argument list, e.g.

        'twister --device-testing --device-serial /dev/ttyACM0
    ...
    parser.add_argument(
        "--west-runner",
        help="""Uses the specified west runner instead of default when running with '--west-flash'.
    ...
    parser.add_argument(
        ...
        May be called multiple times.
        The key-value entries will be prefixed with -D before being passed to CMake.
        E.g 'twister -x=USE_CCACHE=0' will translate to 'cmake -DUSE_CCACHE=0'
    ...
    parser.add_argument(
        ...
        "--dry-run",
        action="store_true",
        help="""Create the filtered list of test cases, but don't actually run them.
        Useful if you're just interested in the test plan generated for every run
        and saved in the specified output directory ('testplan.json').
        """,
    )
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="""Alternative test configuration root/s.
        When a test is found, Twister will check if a test configuration file exists
        in any of the alternative test configuration root folders.
        For example, given $test_root/tests/foo/testcase.yaml,
        Twister will use $alt_config_root/tests/foo/testcase.yaml if it exists
        """,
    )
...
def parse_arguments(parser, args, options=None, on_init=True):
    ...
    if options.coverage_formats:
        for coverage_format in options.coverage_formats.split(','):
            if coverage_format not in supported_coverage_formats[options.coverage_tool]:
                logger.error(
                    f"Unsupported coverage report formats:'{options.coverage_formats}' "
    ...
    if (not options.device_testing) and (
        options.device_serial or options.device_serial_pty or options.hardware_map
    ):
        logger.error(
    ...
    if (
        ...
        and (options.device_serial or options.device_serial_pty)
        and len(options.platform) != 1
    ):
        logger.error(
    ...
    if options.footprint_from_buildlog:
        logger.warning(
            "WARNING: Using --footprint-from-buildlog will give inconsistent results"
    ...
    if len(options.extra_test_args) > 0:
        ...
        if options.extra_test_args[0] != "--":
            ...
            logger.error(
                "Unrecognized arguments found: '%s'."
    ...
    if on_init and not options.allow_installed_plugin and PYTEST_PLUGIN_INSTALLED:
        logger.error(
            "By default Twister should work without pytest-twister-harness plugin being installed,"
            " so please uninstall it by `pip uninstall pytest-twister-harness`"
            " and `git clean -dxf scripts/pylib/pytest-twister-harness`."
        )
        sys.exit(1)
    elif on_init and options.allow_installed_plugin and PYTEST_PLUGIN_INSTALLED:
        logger.warning("You work with installed version of " "pytest-twister-harness plugin.")
