...
class Platform:
    ...
    def load(self, board, target, aliases, data=None):
        ...
        self.sysbuild = vdata.get("sysbuild", data.get("sysbuild", self.sysbuild))
        self.twister = vdata.get("twister", data.get("twister", self.twister))

        # if no RAM size is specified by the board, take a default of 128K
        self.ram = vdata.get("ram", data.get("ram", self.ram))
        # if no flash size is specified by the board, take a default of 512K
        self.flash = vdata.get("flash", data.get("flash", self.flash))
        ...
        self.tier = vdata.get("tier", data.get("tier", self.tier))
        self.type = vdata.get('type', data.get('type', self.type))
        self.simulation = vdata.get('simulation', data.get('simulation', self.simulation))
        self.simulation_exec = vdata.get('simulation_exec', data.get('simulation_exec', self.simulation_exec))
