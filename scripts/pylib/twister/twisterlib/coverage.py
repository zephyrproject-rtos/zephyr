...
# Self-contained dashboard for test_matrix.json. The matrix is inlined at
# __MATRIX_DATA__ and all derived metrics are computed in the browser.
...
def build_test_matrix_html(matrix, out_html):
    """Render a self-contained interactive dashboard from a coverage matrix."""
    payload = json.dumps(matrix, sort_keys=True).replace("</", "<\\/")
    with open(out_html, "w") as fp:
        fp.write(_MATRIX_HTML_TEMPLATE.replace("__MATRIX_DATA__", payload))
    return out_html
...
def run_per_test_coverage(options, instance):
    ...
    build_test_matrix_html(
        matrix, os.path.join(build_dir, "coverage", "test_matrix.html"))
...
def run_coverage(options, testplan) -> tuple[bool, dict]:
    ...
    if getattr(options, "coverage_per_test", False):
        ...
        if infos:
            ...
            html_path = build_test_matrix_html(
                matrix, os.path.join(matrix_dir, "test_matrix.html"))
            logger.info(f"Per-test coverage matrix generated: {json_path}")
            logger.info(f"Per-test coverage dashboard generated: {html_path}")
