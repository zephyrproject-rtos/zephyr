...
class HardwareDataWithCores(HardwareData):
    ...
    cores: list[HardwareDataWithCores] = field(default_factory=list)
    ...
    def from_dict(cls, data: dict[str, Any]) -> HardwareDataWithCores:
        ...
        if 'cores' in data:
            data['cores'] = [HardwareDataWithCores.from_dict(core) for core in data['cores']]
        return cls(**data)
...
class HarnessPytestConfig:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        result = asdict(self)
        # Handle DUT objects to filter out None values
