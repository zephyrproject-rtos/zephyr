...
class Script(Harness):
    ...
    def _get_test_scripts(self) -> list[str]:
        ...
        input_sources = (
            config.get('tests_scripts', ['tests_scripts']) if config else ['tests_scripts']
        )
        tests_scripts = []
        for src in input_sources:
            ...
            if not os.path.exists(source):
                ...
                # A directly specified file is included as-is
    ...
    def _add_testcase_from_script(self, script: str, rc: int, duration: float) -> None:
        ...
        if rc == 0:
            ...
            tc.status = TwisterStatus.FAIL
            tc.reason = f"Script {script_name} failed with return code {rc}"
            tc.output = '\n'.join(self._output)

    def _update_test_status(self):
        self.instance.status = self.status or TwisterStatus.PASS
