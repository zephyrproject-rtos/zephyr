...
class Pytest(ReportHarness):
    ...
    def generate_command(self):
        ...
        # it created). The binary handler, which runs native targets and is the
        # only one carrying the latter, already appends both; do the same here
        # so a pytest test gets them too.
        if handler.type_str == 'native':
            extra_test_args = [
