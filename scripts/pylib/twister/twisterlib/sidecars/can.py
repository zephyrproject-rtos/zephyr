...
class CanSidecar(Sidecar):
    ...
    Creating a ``vcan`` interface needs ``ip`` and root; the sidecar runs ``ip``
    through ``sudo -n`` when not already root. A host that lacks either leaves
    the test build-only (see :meth:`host_ready`) rather than hanging on a
    password prompt, and an unsupported ``vcan`` skips it at run time.
    ...
    def host_ready(self) -> bool:
        """Creating a vcan interface needs ``ip`` and root or passwordless sudo."""
        return shutil.which('ip') is not None and self._has_root()

    def setup(self) -> bool:
        if not self.host_ready():
            self._skip("creating a vcan interface needs ip and root or passwordless sudo")
            return False

        # Best effort: the vcan module is usually autoloaded by "ip link add",
        # and modprobe is not on every host (a container, for instance).
        if shutil.which('modprobe') is not None:
            subprocess.run([*self._sudo(), 'modprobe', 'vcan'], capture_output=True)
