    ...
    <archive root>/<Manufacturer>/<ApplicationProgramRef RefId>.xml
        -> <ApplicationProgram>
             <Static><Code><RelativeSegment Size="..."/></Code>   (= application
                     ...
                     <ComObjectTable><ComObject Number=... Name=... ObjectSize=.../></ComObjectTable>
...
def find_hardware_member(zf):
    ...
    if len(candidates) > 1:
        sys.exit(
            "error: more than one manufacturer folder has a Hardware.xml (%s) — "
            "not supported by this script" % ', '.join(candidates))
    return candidates[0]
    ...
    if program_ref:
        for p in programs:
            if p.get('Id') == program_ref:
                return p
        sys.exit("error: --program-ref %r not among: %s" % (
            program_ref, ', '.join(p.get('Id') for p in programs)))
    if len(programs) > 1:
        listing = '\n'.join(
            "  %s -> ApplicationProgramRef=%s" % (
                p.get('Id'),
                find_by_localname(p, 'ApplicationProgramRef').get('RefId'))
            for p in programs)
        sys.exit(
...
def find_application_program_member(zf, hardware_member, app_ref):
    ...
    candidate = "%s/%s.xml" % (mfr_dir, app_ref)
    if candidate in zf.namelist():
        return candidate
    matches = [n for n in zf.namelist() if n.endswith('/%s.xml' % app_ref)]
    if matches:
        return matches[0]
    sys.exit("error: no XML member for application program %r in the archive" % app_ref)
...
def resolve_parameter_type(app_root, type_ref, cache):
    ...
    result = None
    pt = next((e for e in iter_by_localname(app_root, 'ParameterType')
                if e.get('Id') == type_ref), None)
    if pt is not None:
        for child in pt:
            ...
            if lname == 'TypeFloat':
                ...
                if 'Double' in enc:
                    result = ('double', 8, 8, "IEEE-754 double (%s)" % enc)
                else:
                    result = ('float', 4, 4, "IEEE-754 single (%s)" % enc)
                break
            if lname == 'TypeNumber':
                ...
                ctype = "%sint%d_t" % ('' if signed else 'u', width)
                result = (ctype, nbytes, nbytes,
                          "TypeNumber %s, %d bit" % (child.get('Type'), bits))
                break
            if lname == 'TypeRestriction':
                ...
                result = (ctype, nbytes, nbytes,
                          "TypeRestriction (enum), %d bit — value/Text meaning not "
                          "reproduced, check the .knxprod by hand" % bits)
                break
...
def build_application_program_members(app_root, code_segment):
    ...
    for p in iter_by_localname(app_root, 'Parameter'):
        ...
        if mem is None or mem.get('CodeSegment') != seg_id:
            continue
        entries.append((int(mem.get('Offset', '0')),
                         int(mem.get('BitOffset', '0')), p))
    ...
    for i, (offset, bit_offset, p) in enumerate(entries):
        ...
        notes = []
        if bit_offset != 0:
            notes.append("BitOffset=%d (shares a byte with another parameter) — "
        ...
        if resolved is None:
            ...
            nbytes = max(1, nbytes)
            notes.append("unrecognized ParameterType %r for %r — defaulted to a "
                         "raw %d-byte field, verify by hand"
                         % (p.get('ParameterType'), name, nbytes))
            if nbytes == 1:
                members.append(Member(decl, 'uint8_t', 1, 1, '; '.join(notes)))
            else:
                members.append(Member("%s[%d]" % (decl, nbytes), 'uint8_t',
                                       nbytes, 1, '; '.join(notes)))
        else:
...
_GO_KEYWORDS = (
    (('temperature', 'temp'), 'Type_DPT_Value_Temp', 4, 4,
     "guessed from the name (\"temp\"): DPT 9.001 float cache"),
    (('voltage', 'volt'), 'Type_DPT_Value_Volt', 4, 4,
...
def guess_go_member(name, bits):
    ...
    note = ("TODO: no name keyword matched a known DPT — defaulted to a raw "
            "%d-byte type for ObjectSize=%d bit(s); check the real DPT in ETS "
            "and add a Type_DPT_* alias to dpt.h if it's reusable" % (nbytes, bits))
    if ctype:
        return ctype, nbytes, nbytes, note
    return 'uint8_t[%d]' % nbytes, nbytes, 1, note
...
def flag_summary(comobject):
    return ''.join(letter for attr, letter in _FLAG_LETTERS
                   if comobject.get(attr) == 'Enabled')


def build_group_object_members(app_root):
    comobjects = sorted(iter_by_localname(app_root, 'ComObject'),
    ...
    for c in comobjects:
        ...
        name = c.get('Name') or c.get('Text') or ("GO%d" % number)
        decl = dedupe(sanitize(name), seen_names)
        ...
        if bits is None:
            ...
            note = "TODO: unparsable ObjectSize=%r, defaulted to 1 byte" % c.get('ObjectSize')
        else:
            ctype, size, align, note = guess_go_member(name, bits)

        comment = "ASAP %d, flags=%s (ObjectSize=%s) — %s" % (
            number, flag_summary(c) or '-', c.get('ObjectSize'), note)

        if '[' in ctype:
            base_ctype, bracket = ctype.split('[', 1)
            members.append(Member("%s[%s" % (decl, bracket), base_ctype, size, align,
                                   comment, macro))
        else:
...
def render_struct(struct_name, members):
    ...
    for m in members:
        decl_line = "\t%s %s;" % (m.ctype, m.decl)
        if m.comment:
            lines.append("%-40s /* %s */" % (decl_line, c_comment_safe(m.comment)))
        else:
...
def render_go_macros(members):
    ...
    for i, m in enumerate(members, start=1):
        lines.append("#define %-*s  %d" % (width, m.macro, i))
    return '\n'.join(lines)


def render_header(app_info, ap_members, ap_size_decl, ap_size_computed,
                   go_members, go_size_computed, knxprod_path):
    ...
    header.append(" * from %s," % c_comment_safe(knxprod_path))
    header.append(" * application program %r (%r, ApplicationNumber=%d, "
                   "ApplicationVersion=%d," % (
                       c_comment_safe(app_info['Id']),
                       c_comment_safe(app_info.get('Name', '')),
                       app_number, app_version))
    header.append(" * MaskVersion=%s, PeiType=%s)." % (
    ...
    header.append(" *         %s > <this file>" % c_comment_safe(knxprod_path))
    header.append(" *")
    header.append(" * Kconfig values this application's prj.conf must set to match:")
    header.append(" *     CONFIG_KNX_APPLICATION_PROGRAM_ID        = 0x%04X" % app_number)
    header.append(" *     CONFIG_KNX_APPLICATION_PROGRAM_VERSION   = 0x%02X" % app_version)
    header.append(" *     CONFIG_KNX_APPLICATION_PROGRAM_DATA_SIZE = %d"
                   " (.knxprod RelativeSegment Size%s)" % (
                       ap_size_decl,
                       "" if ap_size_decl == ap_size_computed else
                       "; sizeof(struct application_program_data) computed here = %d"
                       " -- MISMATCH, check the struct above" % ap_size_computed))
    header.append(" *     CONFIG_KNX_GROUP_OBJECT_COUNT            = %d" % len(go_members))
    header.append(" *     CONFIG_KNX_GROUP_OBJECTS_DATA_SIZE       = %d"
                   " (sizeof(struct group_objects_data) computed here)" % go_size_computed)
    header.append(" *")
    ...
    header.append(" * zephyr/subsys/knx/ or any application_w1, application_helios, ... daughterboard")
...
def main():
    ...
    parser.add_argument('--program-ref', metavar='ID',
    ...
    with zipfile.ZipFile(args.knxprod) as zf:
        ...
        if app_ref_elem is None:
            sys.exit("error: Hardware2Program %r has no ApplicationProgramRef"
                      % hw2p.get('Id'))
    ...
    if app_program is None:
        sys.exit("error: %s has no ApplicationProgram element" % ap_member_path)

    app_info = dict(app_program.attrib)

    code_segment, other_segments = pick_code_segment(app_program)
    if code_segment is None:
        sys.exit("error: no <RelativeSegment> (Static/Code) found — nothing to "
                  "generate struct application_program_data from")
    if other_segments:
        for s in other_segments:
            print("warning: extra RelativeSegment %r (Size=%s) ignored, only %r is used"
    ...
    if ap_size_computed != ap_size_decl:
        print("warning: sizeof(struct application_program_data) = %d but the "
              ".knxprod RelativeSegment declares Size=%d"
              % (ap_size_computed, ap_size_decl), file=sys.stderr)
    ...
    for m in ap_members + go_members:
        if m.comment.startswith('TODO') or 'TODO' in m.comment:
            print("warning: %s%s: %s" % (m.decl, (" (%s)" % m.macro) if m.macro else "",
                                          m.comment), file=sys.stderr)

    text = render_header(app_info, ap_members, ap_size_decl, ap_size_computed,
                          go_members, go_size_computed, args.knxprod)
