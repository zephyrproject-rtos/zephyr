...
def compute_kconfig_values(knxprod_path, program_ref=None):
    ...
    for s in other_segments:
        print("warning: extra RelativeSegment %r (Size=%s) ignored, only %r is used"
              % (s.get('Name'), s.get('Size'), code_segment.get('Name')), file=sys.stderr)
    ...
    if ap_size_computed != ap_size_decl:
        print("warning: sizeof(struct application_program_data) would be %d but the "
              ".knxprod RelativeSegment declares Size=%d — writing the declared %d "
              "(CONFIG_KNX_APPLICATION_PROGRAM_DATA_SIZE only needs to be >=)"
              % (ap_size_computed, ap_size_decl, ap_size_decl), file=sys.stderr)
    ...
    return {
        'CONFIG_KNX_APPLICATION_PROGRAM_ID': '0x%04X' % app_number,
        'CONFIG_KNX_APPLICATION_PROGRAM_VERSION': '0x%02X' % app_version,
...
def update_prj_conf(text, values, knxprod_path):
    ...
    for i, line in enumerate(lines):
        ...
        lines[i] = "%s=%s" % (name, new_value)
        if old_value != new_value:
            report.append("changed %s: %s -> %s" % (name, old_value, new_value))
        else:
            report.append("unchanged %s=%s" % (name, new_value))

    if remaining:
        if lines and lines[-1] != '':
            lines.append('')
        lines.append("# Application identity/sizing generated from %s" % knxprod_path)
        lines.append("# by zephyr/scripts/utils/configure_project_from_knxprod.py")
        for name, new_value in remaining.items():
            lines.append("%s=%s" % (name, new_value))
            report.append("added %s=%s" % (name, new_value))
