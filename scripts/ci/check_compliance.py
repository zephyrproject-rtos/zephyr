...
class Identity(ComplianceTest):
    ...
    def run(self):
        for shaidx in get_shas(COMMIT_RANGE):
            ...
            def git_log(fmt, shaidx=shaidx):
                return git("log", f'--format={fmt}', "--no-use-mailmap",
                           "-n 1", shaidx)

            auth_name, auth_email, body = map(git_log, ['%an', '%ae', '%b'])
            ...
            if not match_signoff:
                ...
                failures.append(f"{shaidx}: Signoff line ({signoff}) does not "
                                "follow the syntax: First Last <email>.")
