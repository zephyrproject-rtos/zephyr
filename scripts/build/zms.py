...
class DataInfo:
    ...
    additional_info: Union[DataCRC, Metadata]   # union { uint32_t data_crc; uint32_t metadata; };
...
class FsZmsAte:
    ...
    len: int
    id: int
    content: Union[SmallData, DataInfo, Metadata]
    id_size: ZmsIdSize
...
class FsZmsAteEmpty(FsZmsAte):
    ...
    def __init__(self, cycle_cnt, id_size):
        ...
        if id_size == ZmsIdSize.ID_32_BIT:
            metadata = Metadata(metadata=FsZmsAteEmpty._serialize_metadata())
            content = DataInfo(offset=0x0, additional_info=metadata)
        else:
            # For 64-bit, content is just metadata
            content = Metadata(metadata=FsZmsAteEmpty._serialize_metadata())
        
        super().__init__(id=head_id,
                         len=0xffff,
                         content=content,
                         cycle_cnt=cycle_cnt,
                         id_size=id_size)
...
class FsZmsAteClose(FsZmsAte):
    def __init__(self, cycle_cnt, offset, id_size):
        ...
        if id_size == ZmsIdSize.ID_32_BIT:
            ...
            # For 64-bit, content is just metadata
            content = Metadata(metadata=0xffffffff)
...
class FsZms(fs_core.Fs):
    ...
    def _is_close_ate(self, ate):
        ...
        close_ate_cmp = FsZmsAteClose(cycle_cnt=ate.cycle_cnt, 
                                      offset=self._get_close_offset(ate),
                                      id_size=self.id_size)
    ...
    def _parse_sector(self, sector):
        data_ptr = 0
        records = {}
        sector_status = fs_core.FsSectorStatus.NA
        ...
        empty_ate = FsZmsAte.deserialize(sector[ate_ptr:(ate_ptr + ate_size)],
        ...
        ate_ptr -= ate_size
        close_ate = FsZmsAte.deserialize(sector[ate_ptr:(ate_ptr + ate_size)],
                                         self.write_block_size,
                                         self.id_size)
        ...
        while ate_ptr >= 0:
            ...
            ate = FsZmsAte.deserialize(sector[ate_ptr:(ate_ptr + ate_size)],
                                       self.write_block_size,
                                       self.id_size)
