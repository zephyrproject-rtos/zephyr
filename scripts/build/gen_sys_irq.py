...
def write_sys_dt_irq_foreach_intl_macros(f, tree):
    ...
    def format_intls(intc: INTC, branches: list[INTBranch]) -> str:
        return '\n'.join([
            f'#define SYS_DT_IRQ_NUM_INTLS_{intc.ord} {len(branches)}',
        ])

    def format_impl_intls(intc: INTC, impl_branches: list[INTBranch]) -> str:
        return '\n'.join([
            f'#define SYS_DT_IRQ_NUM_IMPL_INTLS_{intc.ord} {len(impl_branches)}',
            f'#define SYS_DT_IRQ_HAS_IMPL_INTLS_{intc.ord} {"1" if impl_branches else "0"}',
        ])
    ...
    def format_foreach_invocation(branch: INTBranch, var: bool) -> str:
        return (f'fn(id, {branch.intln}'
                f'{", __VA_ARGS__" if var else ""}'
                ')')

    def format_foreach_invocations(branches: list[INTBranch], var: bool) -> list[str]:
        return [format_foreach_invocation(branch, var) for branch in branches]

    def format_foreach_macro(intc: INTC, branches: list[INTBranch], name: str, sep: bool, var: bool) -> str:
        signature = format_foreach_signature(intc, name, sep, var)

        if not branches:
            return signature

        separator = ' __DEBRACKET sep ' if sep else ' '
        return signature + ' ' + separator.join(format_foreach_invocations(branches, var))
