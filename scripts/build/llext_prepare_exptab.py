...
class ZephyrElfExptabPreparator:
    ...
    def __init__(self, elf_path: str, log: logging.Logger, slid_listing_path: str | None):
        ...
        self.elf_fd = open(self.elf_path, 'rb+')  # noqa: SIM115
    ...
    def _prepare_exptab_for_slid_linking(self):
        ...
        IMPLEMENTATION NOTES:
          ...
          Also note that symbol names are always NULL-terminated.
        ...
        for name_ptr, export_address in self.exptab_manipulator:
            name = read_symbol_name(name_ptr)
            exports_list.append((name, export_address))
    ...
    def _prepare_exptab_for_str_linking(self):
        ...
        #
        self.log.warn("_prepare_exptab_for_str_linking: do nothing")
        return 0
    ...
    def _prepare_inner(self):
        ...
        if (self.exptab_section.size % self.lcs_struct.size) != 0:
            self.log.error(
                f"export table size (0x{self.exptab_section.size:X}) "
        ...
        if self.expstrtab_section:
            self.log.debug(
                f"exports strtab section at file offset 0x{self.expstrtab_section.offset:X}"
            )
        else:
...
def _parse_args(argv):
    ...
    parser.add_argument(
        ...
        "--slid-listing",
        help=(
            "write the SLID listing to a file (only useful "
