...
"""
    ...
    def __read_intlist(self, intlist_data):
        ...
        for irq in intlist["interrupts"]:
            self.__log.debug("{0:<3} {1:<5} {2}".format(hex(irq[0]), irq[1], irq[2]))
    ...
    def __parse_intlist(self, intlist):
        ...
        for irq, flags, sname in intlist["interrupts"]:
            if self.__config.test_isr_direct(flags):
                ...
                if not swt:
                    self.__log.error(
                        "Regular Interrupt %d declared with section name %s "
                        "but no SW ISR_TABLE in use" % (irq, sname)
                    )
                ...
                for i in range(nvec):
                    ...
                    if sname in swt[i]:
                        self.__log.error(
                            (
                                "Attempting to register the same section name \"{}\"for"
                                + "different interrupts: {} and {}"
                            ).format(sname, i, irq)
                        )
    ...
    def __isr_spurious_entry(self, irq):
        return '_Z_ISR_TABLE_ENTRY({func}, NULL, "{sect}");'.format(
            func=self.__config.swt_spurious_handler, sect=self.__isr_generated_section(irq)
        )
    ...
    def __irq_spurious_entry(self, irq):
        return '_Z_ISR_DIRECT_TABLE_ENTRY({func}, "{sect}");'.format(
            func=self.__config.vt_default_handler, sect=self.__irq_spurious_section(irq)
        )
    ...
    def __write_isr_handlers(self, fp):
        for i in range(self.__nv):
            if len(self.__swt[i]) <= 0:
                ...
                fp.write(
                    '/* ISR: {} implemented in app in "{}" section. */\n'.format(
                        i, self.__swt[i][0]
                    )
                )
    ...
    def __write_shared_handlers(self, fp):
        fp.write(
            "extern struct z_shared_isr_table_entry {}[{}];\n".format(
                self.__config.shared_array_name, self.__nv
            )
        )
        ...
        for i in range(self.__nv):
            ...
            for j in range(shared_cnt):
                if (swt_len <= 1) or (swt_len <= j):
                    ...
                    fp.write(
                        '/* Shared isr {} entry {} implemented in "{}" section*/\n'.format(
                            i, j, self.__swt[i][j]
                        )
                    )
