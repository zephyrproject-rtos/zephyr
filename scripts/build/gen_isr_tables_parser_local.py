...
def _linker_align(size):
    """Return the largest power-of-2 that divides size.

    lld requires ALIGN() arguments to be powers of 2, while GNU ld accepts
    arbitrary values.  The largest power-of-2 divisor of a struct's sizeof
    equals the struct's natural C alignment, so using it in ALIGN() is both
    correct and compatible with both linkers.
    """
    return size & -size
...
"""
    ...
    def __write_linker_shared(self, fp):
        fp.write(f". = ALIGN({_linker_align(self.__shared_isr_table_entry_size)});\n")
    ...
    def __write_linker_isr(self, fp):
        fp.write(f". = ALIGN({_linker_align(self.__swi_table_entry_size)});\n")
