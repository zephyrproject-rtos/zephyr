...
class AggregateTypeMember:
    def __init__(self, offset, member_name, member_type, member_offset):
        ...
        if isinstance(member_offset, list):
            ...
            if member_offset[0] == 0x23:
                ...
                raise Exception(
                    "not yet supported location operation "
    ...
    def __repr__(self):
        return (
            f"<member {self.member_name:s}, type {self.member_type:d}, "
...
def find_kobjects(elf, syms):
    ...
    for die in variables:
        ...
        if "DW_AT_location" not in die.attributes:
            debug_die(
                die, f"No location information for object '{name:s}'; possibly stack allocated"
            )
            continue
        ...
        if opcode != DW_OP_addr:
            ...
            if opcode == DW_OP_fbreg:
                ...
                debug_die(
                    die, f"kernel object '{name:s}' unexpected exprloc opcode {hex(opcode):s}"
                )
            continue
