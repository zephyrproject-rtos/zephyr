...
STRUCT_Z_LLEXT_DISCARDED_CONST_SYMBOL_DESC = "PPP"  # three pointers
...
def _parse_args(argv):
    ...
    parser.add_argument(
        ...
        type=Path,
        required=False,
        help=".config file of build - used to check for missing Kconfig symbols",
    )
    parser.add_argument(
        "-v",
        "--verbose",
...
def main(argv=None):
    ...
    if args.dotconfig_file:
        kconfig_symbols = parse_dotconfig(args.dotconfig_file)
    else:
    ...
    with open(args.elf_file, 'rb') as elf_fd:
        ...
        # Find strtab structure and create helper to read strings
    ...
    with open(args.output_report, 'w') as out_fd:
        ...
        # Adapt output depending on pointer size
        if ptr_size == 4:
            address_format = "08x"
        else:
            address_format = "016x"

        for group_name, exports in discarded_exports.items():
            ...
            elif kconfig_symbols:  # Don't check Kconfig is group name is wrong
                # Verify that Kconfig symbol for group exists if we have
                # .config, but only if the group's name is in uppercase
                kcfg_sym = GROUP_EXPORT_KCONFIG_PREFIX + group_name
                if kconfig_symbols.get(kcfg_sym) is None:
                    log.warning("Could not find Kconfig symbol %s", kcfg_sym)
                    out_fd.write(
                        f"Could not find Kconfig symbol {kcfg_sym}:\n"
                        f"symbol group will never be included in the LLEXT export table!\n"
                    )

            out_fd.write("=" * 100 + "\n")

            # Write symbols listing
            out_fd.write(f"{'Symbol Name':60s} | Address\n")
            out_fd.write("-" * 80 + "\n")
            for e in exports:
                out_fd.write(f"{e.name:60s} | {e.address:{address_format}}\n")
