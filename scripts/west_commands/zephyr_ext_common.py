...
def cached_runner_config(build_dir, cache, pfx=''):
    ...
    board_dir = cache[   pfx + 'ZEPHYR_RUNNER_CONFIG_BOARD_DIR']
    elf_file = cache.get(pfx + 'ZEPHYR_RUNNER_CONFIG_KERNEL_ELF')
    hex_file = cache.get(pfx + 'ZEPHYR_RUNNER_CONFIG_KERNEL_HEX')
    bin_file = cache.get(pfx + 'ZEPHYR_RUNNER_CONFIG_KERNEL_BIN')
