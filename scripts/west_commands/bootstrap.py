...
from west.manifest import VenvPyRequirement, PyRequirementType, BinRequirementUrl
...
class Bootstrap(WestCommand):
    ...
    def do_add_parser(self, parser_adder):
        ...
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Enable hermetic builds. The resulting virtual environment "
                + "will OVERRIDE your path variable to only include the "
                + "required python and binary dependencies"
            ),
        )
        return parser
    ...
    def do_run(self, args, remainder) -> None:
        ...
        py_requirements = Requirements(
            requirements=[
                project_dir / req.pattern
                for req in self.manifest.venv.py_requirements
                if req.type == PyRequirementType.REQUIREMENTS
            ],
            constraints=[
                project_dir / req.pattern
                for req in self.manifest.venv.py_requirements
                if req.type == PyRequirementType.CONSTRAINTS
            ],
            module_dirs=[
                project_dir / req.pattern
                for req in self.manifest.venv.py_requirements
                if req.type == PyRequirementType.DIRECTORY
            ],
            modules=[
                req.pattern
                for req in self.manifest.venv.py_requirements
                if req.type == PyRequirementType.PACKAGE
            ],
        )
    ...
    def _check_bin_requirements(self) -> None:
        for name, url_map in self.manifest.venv.bin_requirements.items():
            ...
            if _GLOBAL_DEFINITIONS["platform"] in url_map:
                ...
                for key in url_map.keys():
                    match = re.match(
                        pattern=key, string=_GLOBAL_DEFINITIONS["platform"]
                    )
                    if match:
            ...
            url_entry: BinRequirementUrl = url_map[url_key]
            self.extra_paths += [install_path / path for path in url_entry.paths]
