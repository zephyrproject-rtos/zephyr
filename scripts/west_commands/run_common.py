...
def add_parser_common(command, parser_adder=None, parser=None):
    ...
    # Remember to update scripts/west_commands/completion/west-completion.bash
    # if you add or remove flags
...
def do_run_common(command, user_args, user_runner_args):
    ...
    # If flashing of a signed file is requested, replace the bin and hex
    # file paths with their signed equivalents, as generated by `west sign`.
    # e.g.
    #     build/zephyr/custom_name.bin -> build/zephyr/zephyr.signed.bin
    #     build/zephyr/custom_name.hex -> build/zephyr/zephyr.signed.hex
    if hasattr(args, 'signed') and args.signed:
        def path_convert_to_signed(path):
            directory = path.rsplit('/', 1)[0]
            ext = path.rsplit('.', 1)[1]
            return f"{directory}/zephyr.signed.{ext}"
        args.bin_file = path_convert_to_signed(args.bin_file)
        args.hex_file = path_convert_to_signed(args.hex_file)
