...
def main():
    ...
    if kconf.warnings:
        error_out = False
        if not args.forced_input_configs:
            error_out = True

        for warning in kconf.warnings:
            if not any([re.search(w, warning) for w in warn_only]):
                # At least one warning is not a warn_only, fail the Kconfig.
                error_out = True

        # Turn all warnings into errors, so that e.g. assignments to undefined
        # Kconfig symbols become errors (unless not explicitly disabled via
        # --no-werror).
        #
...
def parse_args():
    ...
    parser.add_argument(
        "--werror", action=argparse.BooleanOptionalAction, help="Handle kconfig warnings as error"
    )
