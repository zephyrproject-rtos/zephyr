...
menuconfig.menuconfig() function with an existing Kconfig instance. The second
option is a bit inflexible in that it will still load and save .config, etc.
...
def _init_styles():
    ...
    _PATH_STYLE          = _style(curses.COLOR_BLACK, curses.COLOR_WHITE,  curses.A_BOLD                     )

    # Separator below menu path, with title and arrows pointing up
    _TOP_SEP_STYLE       = _style(curses.COLOR_BLACK, curses.COLOR_YELLOW, curses.A_BOLD,   curses.A_STANDOUT)
    ...
    _BOT_SEP_STYLE       = _style(curses.COLOR_BLACK, curses.COLOR_YELLOW, curses.A_BOLD,   curses.A_STANDOUT)

    # Help window with keys at the bottom
    _HELP_STYLE          = _style(curses.COLOR_BLACK, curses.COLOR_WHITE,  curses.A_BOLD                     )


    # Frame around dialog boxes
    _DIALOG_FRAME_STYLE  = _style(curses.COLOR_BLACK, curses.COLOR_YELLOW, curses.A_BOLD,   curses.A_STANDOUT)
    ...
    _INFO_TOP_LINE_STYLE = _style(curses.COLOR_BLACK, curses.COLOR_YELLOW, curses.A_BOLD,   curses.A_STANDOUT)
    ...
    _INFO_BOT_SEP_STYLE  = _style(curses.COLOR_BLACK, curses.COLOR_YELLOW, curses.A_BOLD,   curses.A_STANDOUT)

    # Help window with keys at the bottom of the information display
    _INFO_HELP_STYLE     = _style(curses.COLOR_BLACK, curses.COLOR_WHITE,  curses.A_BOLD                     )
...
# Next color pair index to be created
_color_attrib_i = 1
...
def _style(fg_color, bg_color, attribs, no_color_extra_attribs=0):
    ...
    global _color_attrib_i

    if not curses.has_colors():
        return attribs | no_color_extra_attribs

    if (fg_color, bg_color) in _color_attribs:
        # Reuse already created color pair
        color_attrib = _color_attribs[(fg_color, bg_color)]
    else:
        # Create new color pair
        curses.init_pair(_color_attrib_i, fg_color, bg_color)
        color_attrib = _color_attribs[(fg_color, bg_color)] = \
            curses.color_pair(_color_attrib_i)
        _color_attrib_i += 1
...
def _draw_main():
    ...
    _safe_addstr(_help_win, 0, 0, "[Space/Enter] Toggle/enter   [ESC] Leave menu   [S] Save")
    _safe_addstr(_help_win, 1, 0, "[M] Save minimal config      [?] Symbol info    [Q] Quit (prompts for save)")
...
def _display_info(node):
    ...
    while True:
        ...
        elif c in (curses.KEY_LEFT, "h", "H", "q", "Q", "\x1B"):  # \x1B = ESC
...
def _draw_info_display(node, lines, scroll, top_line_win, text_win,
    ...
    _safe_addstr(help_win, 0, 0, "[ESC/q] Return to menu")
...
def _direct_dep_info(sc):
    ...
    return 'Direct dependencies (value: "{}"):\n{}\n' \
           .format(TRI_TO_STR[expr_value(sc.direct_dep)],
                   _split_expr_info(sc.direct_dep, 0))
