...
class Sip:
    ...
    def from_sip(name: str,
        ...
        if sip is None:
            return None
        soc = Soc.from_soc(systems.get_soc(sip.soc), variants)
        return Sip(name, soc)
...
def board_v2_qualifiers(board):
    ...
    for sip in board.sips:
        if sip.soc.cpuclusters:
            for c in sip.soc.cpuclusters:
                id_str = sip.name + '/' + c.name
                qualifiers_list.append(id_str)
                for v in c.variants:
                    qualifiers_list.extend(variant_v2_qualifiers(v, id_str))
        else:
            qualifiers_list.append(sip.name)
            for v in sip.soc.variants:
                qualifiers_list.extend(variant_v2_qualifiers(v, sip.name))
