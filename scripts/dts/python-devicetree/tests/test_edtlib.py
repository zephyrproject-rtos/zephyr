...
def test_props():
    ...
    verify_props(props_node,
                 ['int', 'negative',
                  ...
                  'negative-array', 'array', 'uint8-array',
                  'string', 'string-array',
                  'phandle-ref', 'phandle-refs',
                  'path'],
                 ['int', 'int',
                  'boolean', 'boolean',
                  'array', 'array', 'uint8-array',
                  'string', 'string-array',
                  'phandle', 'phandles',
                  'path'],
                 [1, -1,
                  True, False,
                  [-100,-10,0,10,100], [1,2,3], b'\x124',
                  'foo', ['foo','bar','baz'],
...
def test_cpu_binding_type_conflict(tmp_path):
    ...
    assert "uint32-or-uint64" in msg
...
def test_uint32_or_uint64_type():
    '''type: uint32-or-uint64 assembles 1-cell and 2-cell DTS values into a scalar int'''

    with from_here():
        edt = edtlib.EDT("test.dts", ["test-bindings"])

    node = edt.get_node("/uint32-or-uint64-node")

    # 1-cell value: plain 32-bit integer returned as scalar
    assert node.props["val-32"].val == 100000000
    assert node.props["val-32"].spec.type == "uint32-or-uint64"
...
def test_uint32_or_uint64_three_cell_error(tmp_path):
    '''type: uint32-or-uint64 rejects a 3-cell DTS value with a clear error'''
...
/ {
    ...
    #size-cells = <1>;
    n {
        compatible = "vnd,uint32-or-uint64-test";
        val-32 = <1 2 3>;
    };
};
""")
...
def test_uint32_or_uint64_min_max(tmp_path):
    '''type: uint32-or-uint64 min: constraint is enforced on the assembled value'''
...
def test_uint32_or_uint64_binding_errs(tmp_path):
    '''Binding validator: min:/max: accepted on uint32-or-uint64; min-len: rejected'''

    def check_binding_err(yaml_content, expected_err):
        binding_file = tmp_path / "bad-uint32-or-uint64.yaml"
        binding_file.write_text(yaml_content)
        with pytest.raises(edtlib.EDTError) as e:
            edtlib.Binding(str(binding_file), {})
        assert str(e.value) == expected_err

    path = str(tmp_path / "bad-uint32-or-uint64.yaml")

    # min-len: on uint32-or-uint64 must be rejected
    check_binding_err(
        """\
description: test
compatible: "bad"
...
""",
        ...
        f"requires an array type, but has type 'uint32-or-uint64'")
...
""",
        ...
        f"'type: int', 'type: uint32-or-uint64', or 'type: array', but has type 'string'")
