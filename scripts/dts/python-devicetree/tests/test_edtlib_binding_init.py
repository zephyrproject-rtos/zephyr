...
def test_binding_description_overwrite() -> None:
    ...
    assert "Binding description (multi)." == binding.description
...
def test_binding_compat_overwrite() -> None:
    ...
    """
    binding = load_binding("test-bindings-init/compat_desc.yaml")
    assert "vnd,compat-desc" == binding.compatible

    binding = load_binding("test-bindings-init/compat_desc_multi.yaml")
    assert "vnd,compat-desc-multi" == binding.compatible
...
def test_child_binding_description_overwrite() -> None:
    ...
    assert "Child-binding description." == child_binding.description

    child_binding = child_binding_of(
        "test-bindings-init/compat_desc_multi.yaml"
    )
    # When inherited multiple times, the first "description:" wins.
    assert "Child-binding description (base)." == child_binding.description
...
def test_child_binding_compat_overwrite() -> None:
    ...
    child_binding = child_binding_of("test-bindings-init/compat_desc.yaml")
    # Overwrite inherited description.
    assert "vnd,child-compat-desc" == child_binding.compatible

    child_binding = child_binding_of(
        "test-bindings-init/compat_desc_multi.yaml"
    )
    # When inherited multiple times, the first "compatible:" wins.
    assert "vnd,child-compat-desc-base" == child_binding.compatible
...
def test_grandchild_binding_description_overwrite() -> None:
    ...
    """
    grandchild_binding = grandchild_binding_of(
        "test-bindings-init/compat_desc.yaml"
    )
    assert "Grandchild-binding description." == grandchild_binding.description

    grandchild_binding = grandchild_binding_of(
        "test-bindings-init/compat_desc_multi.yaml"
    )
    assert (
        "Grandchild-binding description (base)."
        == grandchild_binding.description
    )
...
def test_grandchild_binding_compat_overwrite() -> None:
    ...
    # Overwrite inherited description.
    assert "vnd,grandchild-compat-desc" == grandchild_binding.compatible

    grandchild_binding = grandchild_binding_of(
        "test-bindings-init/compat_desc_multi.yaml"
    )
    # When inherited multiple times, the first "compatible:" wins.
    assert "vnd,grandchild-compat-desc-base" == grandchild_binding.compatible
...
def test_filter_inherited_propspecs_basics() -> None:
    ...
    # Allowed properties.
    assert {"child-prop-1", "child-prop-2"} == set(
        child_binding.prop2specs.keys()
    )
    child_binding = child_binding_of("test-bindings-init/simple_blocklist.yaml")
    # Non blocked properties.
    assert {"child-prop-2", "child-prop-3"} == set(
        child_binding.prop2specs.keys()
    )

    # GrandChild-binding level.
    grandchild_binding = grandchild_binding_of(
        "test-bindings-init/simple_allowlist.yaml"
    )
    # Allowed properties.
    assert {"grandchild-prop-1", "grandchild-prop-2"} == set(
        grandchild_binding.prop2specs.keys()
    )
    grandchild_binding = grandchild_binding_of(
        "test-bindings-init/simple_blocklist.yaml"
    )
    # Non blocked properties.
    assert {"grandchild-prop-2", "grandchild-prop-3"} == set(
        grandchild_binding.prop2specs.keys()
    )
