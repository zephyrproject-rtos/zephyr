...
def write_freq(node):
    # Writes the clock frequency for the node, if any. Derived by searching
    ...
    if not isinstance(clocks_prop.val[0], edtlib.ControllerAndData):
        err("'clocks' property on {!r} should be a phandle-array"
            .format(clocks_prop.node))
        ...
        if "fixed-clock" in clock.controller.compats:
            if "clock-frequency" not in clock.controller.props:
                err("{!r} is a 'fixed-clock' but lacks a 'clock-frequency' "
                    "property".format(clock.controller))
