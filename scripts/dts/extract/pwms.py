...
##
# @brief Manage pwm-x directive.
# insert <pwm-prop-name>-_controller
# insert pwm cells based on pwm-controller definitions
#
class DTPWMs(DTDirective):

    def __init__(self):
        pass

    ##
    # @brief Extract pwm information.
    #
    # @param node_address Address of node owning the pinctrl definition.
    # @param yaml YAML definition for the owning node.
    # @param prop pwm key
    # @param def_label Define label string of client node owning the pwm
    #                  definition.
    #
    def _extract_controller(self, node_address, yaml, prop, prop_values, def_label, index):
        ...
        if l_cell is not None:
            ...
            if 'use-prop-name' in generation:
                ...
                l_cellname = convert_string_to_label('pwm' + '_' +
                                                                'controller')
    ...
    def _extract_cells(self, node_address, yaml, prop, prop_values, def_label, index, controller):
        ...
        if 'use-prop-name' in generation:
            ...
            l_cell = [convert_string_to_label('pwm')]
    ...
    #
    # @param node_address Address of node owning the pwm definition.
    # @param yaml YAML definition for the owning node.
    # @param prop pwm property name
    ...
    def extract(self, node_address, yaml, prop, def_label):
        ...
        if 'pwm' in prop:
            ...
            raise Exception(
                "DTPWMs.extract called with unexpected directive ({})."
                    .format(prop))

##
# @brief Management information for pwm
pwms = DTPWMs()
