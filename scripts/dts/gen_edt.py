...
def reg_to_dict(reg: edtlib.Register) -> dict:
    return {
        'name': reg.name,
        'addr': f'{hex(reg.addr) if reg.addr is not None else None}',
        'size': f'{hex(reg.size) if reg.size is not None else None}',
...
def prop_to_dict(prop_name: str, prop: edtlib.Property) -> dict:
...
def pinctrl_to_dict(pinctrl: edtlib.PinCtrl) -> dict:
    return {
        'name': pinctrl.name,
        'conf_nodes': [node_to_dict(p) for p in pinctrl.conf_nodes]
    } if pinctrl is not None else {}


def ctrl_to_dict(ctrl: edtlib.ControllerAndData) -> dict:
    return {
        'name': ctrl.name,
        'basename': ctrl.basename,
        'data': ctrl.data,
        'controller': node_to_dict(ctrl.controller)
    } if ctrl is not None else {}


def node_to_dict(node: edtlib.Node) -> dict:
    return {
        'name': node.name,
        ...
        'props': [prop_to_dict(p_name, p) for p_name, p in node.props.items()],
        'interrupts': [ctrl_to_dict(i) for i in node.interrupts],
        'pinctrls': [pinctrl_to_dict(p) for p in node.pinctrls]
