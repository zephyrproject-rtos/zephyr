...
Rationale:
- factorize command line parser initialization (options and arguments
  definitions)
...
- a standalone mode (raw CLI entry point): DTShCli is then responsible for
  initializing a default CLI parser, and actually parsing the command line
  arguments when entering DTShCli.run()
- a passive mode: some external code (e.g. a WestCommand) is responsible
  ...
  and actually parsing the command line arguments needed to call DTShCli.run()
  in passive mode
"""
...
class DTShArgvParser(argparse.ArgumentParser):
    ...
    def init(parser: argparse.ArgumentParser) -> None:
        ...
        Args:
            parser: The parser to initialize (passive mode).
        """
    ...
    def __init__(self) -> None:
        """Initialize a default parser (standalone mode)."""
        super().__init__(
            prog="dtsh",
...
class DTShCli:
    ...
    def __init__(
        ...
        Args:
            parser: An externally managed parser to configure (passive mode).
              If unset, the default parser is used (standalone mode).
        """
        if parser:
            self._parser = parser
        else:
    ...
    def run(self, args: Optional[argparse.Namespace] = None) -> None:
        ...
        Args:
            args: Parsed arguments, e.g. when called from a West command
              (passive mode). If unset, standalone mode is assumed,
...
def run() -> None:
    """Installed entry point (standalone mode)."""
    cli = DTShCli()
