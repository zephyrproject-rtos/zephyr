...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["std::tuple<decltype(std::vector<T>())> value;"],
            id="template-inside-decltype",
        ),
        pytest.param(
            ["std::tuple<decltype(std::vector<std::pair<int, int>>())> value;"],
            id="nested-template-inside-decltype",
        ),
        pytest.param(
            ["public: template <typename T> struct X {};"],
            id="template-after-public",
        ),
        pytest.param(
            ["protected: template <typename T> void f();"],
            id="template-after-protected",
        ),
        pytest.param(
            ["private: template <typename T> struct X {};"],
            id="template-after-private",
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["std::vector<int> value = left<right>limit;"],
            {("SPACING", 1, "<"): 1, ("SPACING", 1, ">"): 1},
            id="comparison-after-assignment",
        ),
        pytest.param(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ["template <typename T> using V = Foo<T>;"],
            {("SPACING", 1, "<"): 1, ("SPACING", 1, ">"): 1},
            id="unqualified-id-after-assignment",
        ),
        pytest.param(
            ["template <bool E = N > 0> struct value {};"],
        ...
        pytest.param(
            ["template <int N = 1 >> 2> struct value {};"],
            {("SPACING", 1, "<"): 1, ("SPACING", 1, ">"): 1},
            id="unparenthesized-right-shift-aborts",
        ),
        pytest.param(
            [
                "template <int N = 1 >> 2> struct value {};",
            ...
            {("SPACING", 1, "<"): 1, ("SPACING", 1, ">"): 1, ("SPACING", 2, ">"): 1},
            id="right-shift-abort-does-not-swallow",
        ),
        pytest.param(
            ["template <bool E = N >= 0> struct value {};"],
            {("SPACING", 1, "<"): 1, ("SPACING", 1, ">"): 1},
