...
#VNG VBLUno boards
if [ $BOARD == "nrf51_vbluno51" ] || [ $BOARD == "nrf52_vbluno52" ]
then
	command -v pyocd-flashtool >/dev/null 2>&1 || { echo >&2 "Can't flash VBLUno board,"\
							  "pyocd-flashtool (pyOCD) is not installed."\
							  "Aborting."; exit 1; }	
	pyocd-flashtool $HEX_NAME
else
#Nordic boards
