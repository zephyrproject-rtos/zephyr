...
# If pykwalify is installed, then the validate the yaml
try:
...
def print_ip_elems(dict, elem, name, family, indent):
    if elem in dict.keys():
        ...
        if elem_value['status'] == "enabled":
            ...
            print(indent + "."+ name + " = {")
            print_enabled_elem(elem_value, 'status', 'is_enabled', "\t" + indent)
            print_address_elem(elem_value, 'addresses', 'address', family, "\t" + indent)
            ...
            if elem == 'IPv4':
                ...
                print_int_elem(elem_value, 'netmask-len', 'netmask_len', "\t" + indent)
