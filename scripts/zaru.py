...
try:
    ...
    sys.exit("Python3 babeltrace2 module is missing. Please install it.\n"
...
def get_symbols_from_elf(elf_file, verbose=False):
    ...
    # dict: {addr: symbol}
    addr_to_symbol = {r.match(line).group(1): r.match(line).group(4) for line in l if r.match(line) is not None}
...
def get_ctf_metadata_file():
    """ Get CTF metadata.

    Get CTF Common Trace Format) metadata full path.
    """

    zephyr_base = get_zephyr_base()
    if zephyr_base is None:
        sys.exit("Can not find CTF metadata, please set 'ZEPHYR_BASE' env. variable!")

    return zephyr_base / "subsys/instrumentation/ctf/metadata"
...
def get_and_print_trace(port, elf, annotate_ret=False, verbose=False):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        metadata_file = get_ctf_metadata_file()
        shutil.copy(metadata_file, tmpdir)
            ...
            for msg in msg_it:
                if isinstance(msg, bt2._EventMessageConst):
                    ...
                    if callee_symbol is None:
                        print(Fore.RED + f"Symbol address {callee} could not be resolved! "
        ...
        while current_event:
            ...
            if next_event:
                ...
                if cur_type == "entry":
                    if next_type == "exit" and cur_func == next_func:
                        # Same function, so use compact print.
                        line = cur_func + "();"
                        thread = cur_tn
                        per_thread_line = {"thread": thread, "line": line.rjust(per_thread_func_nest_depth[cur_tn] + len(line))}
            ...
            if cur_type == "entry":
                line = cur_func + "() {"
                per_thread_line = {"thread": cur_tn, "line": line.rjust(per_thread_func_nest_depth[cur_tn] + len(line))}
                line_buffer_second_half.append(per_thread_line)

                # Advance nesting.
                per_thread_func_nest_depth[cur_tn] = per_thread_func_nest_depth[cur_tn] + func_nest_depth_step
                ...
                if per_thread_func_nest_depth[cur_tn] >= 0:
                    line = "};"
                    if annotate_ret:
                        line = line + Fore.WHITE + Style.DIM + f"   /* {cur_func} */" + Fore.RESET + Style.RESET_ALL
                    per_thread_line = {"thread": cur_tn, "line": line.rjust(per_thread_func_nest_depth[cur_tn] + len(line))}

                else:
                    # Found an orphanated exit, so adjust indentation for all the previous lines to
                    # the correct nesting depth, i.e. justify lines at right by 'func_nest_depth_step'.
                    # Only the lines of current thread(cur_tn) are justified, since each thread has a
                    # different stack, the nesting depth is kept per thread.
                    line_buffer_second_half = \
                        [{"thread": l["thread"], "line": l["line"].rjust(func_nest_depth_step + len(l["line"]))} if l["thread"] == cur_tn else l for l in line_buffer_second_half]

                    # Then inform the orphaned exit.
                    per_thread_line = {"thread": cur_tn, "line": Fore.RED + f"}}; --> orphaned '{cur_func}'" + Fore.RESET}
                ...
                line = Fore.LIGHTYELLOW_EX + f"/* <-- Scheduler switched IN thread '{cur_tn}' */" + Fore.RESET + Style.RESET_ALL
                line_buffer_second_half.append({"thread": "00000", "line": line})
            else: # switched_out
                line = Fore.LIGHTYELLOW_EX + f"/* --> Scheduler switched OUT from thread '{cur_tn}' */" + Fore.RESET + Style.RESET_ALL
                line_buffer_second_half.append({"thread": "00000", "line": line})
        ...
        line_buffer_second_half = [l["line"] for l in line_buffer_second_half]
        traces = [" ".join(halves) for halves in tuple(zip(line_buffer_first_half, line_buffer_second_half))]

        # Print header
        if traces:
            print("Thread Name".rjust(20), "Thread ID".rjust(14), "", "CPU".rjust(3), "", "Mode".rjust(4), "", "Timestamp".rjust(12), "", "Function(s)".rjust(19))
            print("    ".ljust(100, "-"))
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for msg in msg_it:
                if isinstance(msg, bt2._EventMessageConst):
                    ...
                    assert callee_symbol is not None, \
                        f"Symbol address {callee} could not be resolved! Are you sure FW flashed matches provided ELF file in build dir?"
        ...
        for trace in ge:
            ...
            if event_type == "entry":
                ...
                sched_ts = "{:.6f}".format(ftrace_ts)

                sched_cpu = "{:03d}".format(int(cpu))
                ...
                system_trace_switched_out_event = f"{tn}-{tid} ({tid}) [{sched_cpu}] .... {sched_ts}: sched_switch: prev_comm={tn} prev_pid={tid} prev_prio=0 prev_state=T ==> "

                next_trace = next(ge, None)
                if not next_trace:
                    print(Fore.RED +  "Warning: A switched in event is expected after a switched out! Skipping sched switch event." + Fore.RESET)
                    continue

                next_event_type, _, next_tid, _, _, next_ts, next_tn = next_trace.values()
                assert next_event_type == "switched_in", "A switched in event is expected after a switched out!"
                ...
                system_trace_switched_in_event = f"next_comm={next_tn} next_pid={next_tid} next_prio=0\n"

                system_trace_switch_event = system_trace_switched_out_event + system_trace_switched_in_event
            ...
            if tn not in named_thread_list:
                ...
                trace_event = {"args": {"name": tn}, "cat": "__metadata", "name": "thread_name", "ph": "M", "pid": tid, "tid": tid, "ts": 0}
        ...
        if verbose:
            print(f"Found {len(trace_events)} event(s), {len(named_thread_list)} thread(s), {len(system_trace_events)} context switch(es).")
...
def get_and_print_profile(port, elf, n, verbose=False):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for i, (callee, delta_t) in enumerate(profiles):
            ...
            if callee_symbol is None:
                print(Fore.RED + f"Can't resolve address {callee}, skipping it..."
            ...
            print(color + (f'{percent_delta_t:.2f}' + "%").rjust(6), callee, callee_symbol.ljust(20), Fore.WHITE)
    ...
    if args.list:
        ...
        if addr["trigger"]:
            ...
            if func_name:
                ...
                print(f"Trigger is set to unresolved address {addr['trigger']} entry. Is it a stale address that needs to be reset?")
        else:
        ...
        if addr["stopper"]:
            ...
            if func_name:
                ...
                print(f"Stopper is set to unresolved address {addr['trigger']} exit. Is it a stale address that needs to be reset?")
        else:
            ...
            if args.trigger not in symbol_to_addr:
                ...
                if not set_trigger_addr(sport, address):
                    ...
                    if args.trigger == "0":
                        ...
                        print(f"New trigger set at entry of '{args.trigger}' function, at address 0x{address}.")
            ...
            if args.stopper not in symbol_to_addr:
                ...
                if not set_stopper_addr(sport, address):
                    ...
                    if args.stopper == "0": # "0" is a special name/address
                        ...
                        print(f"New stopper set at exit of '{args.stopper}' function, at address 0x{address}.")
...
def profile(args):
    ...
    elf_file = get_elf_file(args, args.verbose)
    num_profiles = get_and_print_profile(sport, elf_file, args.n, args.verbose)
    if num_profiles == 0:
...
def print_message_on_empty_buffer(command):
    ...
    print(f"{command.title()} buffer is empty. Please check the trigger "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Zephyr compiler-managed system profiling and tracing tool.")
    parser.add_argument("--serial", default=SERIAL, help=f"defaults to '{SERIAL}'. It can be also a TCP/IP port, e.g 'socket://localhost:4321'.")
    parser.add_argument("--build-dir", help=f"build dir where flashed {ELF} is. If not given zaru will try to guess it.")
    ...
    trace_parser.add_argument('--reboot', '-r', action='store_true', help="reboot target before getting traces.")
    trace_parser.add_argument('--list', '-l', action='store_true', help="list trigger and stopper functions set in the target.")
    trace_parser.add_argument('--trigger', '-t', metavar="FUNC_NAME", type=str, help="set FUNC_NAME as the trigger function. When called it turns on instrumentation.")
    trace_parser.add_argument('--stopper', '-s', metavar="FUNC_NAME", type=str, help="set FUNC_NAME as the stopper function. When returned it turns off instrumentation.")
    ...
    profile_parser.add_argument('--reboot', '-r', action='store_true', help="reboot target before profiling.")
    profile_parser.add_argument('-n', nargs='?', type=int, default=100, help="show first N most expensive functions.")
