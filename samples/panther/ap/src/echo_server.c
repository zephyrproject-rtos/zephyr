...
//TODO	net_dhcpv4_start(net_if_get_default());
#else
			       ...
			       struct net_context **tcp_recv6)
{
	...
	printk("echo server:get IPv4 UDP net context\n");
	...
	printk("echo server:IPv4 UDP binded on port %d\n",
	...
	if (tcp_recv4) {
		...
		printk("echo server:IPv4 TCP server is listening on %d.%d.%d.%d:%d\n",
				my_addr4.sin_addr.s4_addr[0],
				my_addr4.sin_addr.s4_addr[1],
				my_addr4.sin_addr.s4_addr[2],
				my_addr4.sin_addr.s4_addr[3],
				ntohs(my_addr4.sin_port));
	}
#endif
			 ...
			 void *user_data)
{
	...
	sa_family_t family;
	int ret;
printk("echo server:tcp_received\n");
	if (!pkt) {
		/* EOF condition */
		return;
	}
			 ...
			 void *user_data)
{
	...
	printk("echo server:tcp_accepted: from %d.%d.%d.%d:%d\n",
			((struct sockaddr_in*)addr)->sin_addr.s4_addr[0],
			((struct sockaddr_in*)addr)->sin_addr.s4_addr[1],
			((struct sockaddr_in*)addr)->sin_addr.s4_addr[2],
			((struct sockaddr_in*)addr)->sin_addr.s4_addr[3],
			ntohs(((struct sockaddr_in*)addr)->sin_port));
	printk("echo server:tcp_accepted:call net_context_recv(). Wait for data\n");

	ret = net_context_recv(context, tcp_received, 0, NULL);
	printk("echo server:tcp_accepted:net_context_recv return %d\n", ret);
	if (ret < 0) {
...
#if defined(CONFIG_NET_IPV4)
	printk("echo server:call accept\n");
	ret = net_context_accept(tcp_recv4, tcp_accepted, 0, NULL);
	printk("echo server:accept return %d\n", ret);
	if (ret < 0) {
