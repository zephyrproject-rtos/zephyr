...
#define W25Q256JV_JEDEC_ID				0x001970efU
...
#define SPI_FLASH_READ_DUAL_OPCODE			0xbbu
#define SPI_FLASH_READ_QUAD_OPCODE			0xebu
...
#define SPI0_NODE			DT_NODELABEL(spip0)
				  ...
				  uint8_t *data, size_t datasz, uint8_t opcode)
{
	...
	struct spi_buf spi_bufs[3] = {0};
	int err = 0;
	struct spi_config spi_cfg_addr = {0};
	...
	spi_bufs[0].buf = &txdata[0];
	spi_bufs[0].len = 1;
	spi_bufs[1].buf = &txdata[1];
	if (spi_cfg_data == &spi_cfg_quad)
		spi_bufs[1].len = spi_addr->byte_len + 3;
	else
		spi_bufs[1].len = spi_addr->byte_len + 1;
	spi_bufs[2].buf = data;
	spi_bufs[2].len = datasz;

	const struct spi_buf_set txset_op = {
		.buffers = &spi_bufs[0],
		.count = 1,
	};

	const struct spi_buf_set txset_addr = {
		.buffers = &spi_bufs[1],
		.count = 1,
	};
	...
	err = spi_transceive(dev, spi_cfg_cmd, &txset_op, NULL);
	spi_cfg_cmd->operation &= ~SPI_HOLD_ON_CS;
	if (err) {
		return err;
	}

	memcpy(&spi_cfg_addr, spi_cfg_data, sizeof(struct spi_config));
	spi_cfg_addr.operation |= SPI_HOLD_ON_CS;
	err = spi_transceive(dev, &spi_cfg_addr, &txset_addr, NULL);
	if (err) {
		return err;
	}
...
int main(void)
{
	...
	} else if (jedec_id == W25Q256JV_JEDEC_ID) {
		printf("W25Q256JV 32Mbyte SPI flash\n");
