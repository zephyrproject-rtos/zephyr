...
static void rpi_board_info(const struct device *dev)
{
	uint32_t revision = 0;
	int ret;
...
int main(void)
{
	const struct device *dev = DEVICE_DT_GET_ANY(raspberrypi_bcm283x_firmware);

	if (!device_is_ready(dev)) {
		printk("Raspberry Pi Firmware not found\n");
		return 0;
	}
