				      ...
				      struct onoff_client *cli)
{
	...
	printk("Waiting for clock %s\n", clk_dev->name);
	do {
		rc = sys_notify_fetch_result(&cli->notify, &res);
	} while (rc == -EAGAIN);

	if (res < 0) {
		printk("Clock %s could not be started: %d\n",
			clk_dev->name, res);
		return -ENODEV;
	}

	printk("Clock %s started\n", clk_dev->name);
...
int main(void)
{
	...
	const struct device *clk_dev[] = {
		...
		DEVICE_DT_GET_OR_NULL(DT_CLOCKS_CTLR(DT_NODELABEL(grtc))),
	};
	...
	const struct nrf_clock_spec cpu_clk_128 = {
		.frequency = MHZ(128),
	};
	rc = request_and_wait_for_clock(cpu_clk_dev, &cpu_clk_128, &cpu_cli);
	if (rc >= 0) {
		rate = 0;
		(void)clock_control_get_rate(cpu_clk_dev, 0, &rate);
		...
		nrf_clock_control_release(cpu_clk_dev, &cpu_clk_128);
	}
