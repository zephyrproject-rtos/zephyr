...
int main(void)
{
	...
	/* --- Test 5: global brightness sweep --- */
	uint8_t all_on[13];
 
	memset(all_on, 0xFF, sizeof(all_on));
 
	struct display_buffer_descriptor all_on_desc = {
		.buf_size = sizeof(all_on),
		.width    = 13,
		.height   = 8,
		.pitch    = 13,
	};
 
	display_write(disp, 0, 0, &all_on_desc, all_on);
	printk("Brightness sweep start (all LEDs on)\n");
 
	/* Fade out: 255 -> 0 in steps of 16 */
	for (int b = 255; b >= 0; b -= 16) {
		display_set_brightness(disp, (uint8_t)b);
		printk("Brightness: %d\n", b);
		k_sleep(K_MSEC(300));
	}
 
	/* Ensure we hit exactly 0 */
	display_set_brightness(disp, 0);
	k_sleep(K_MSEC(500));
 
	/* Fade in: 0 -> 255 in steps of 16 */
