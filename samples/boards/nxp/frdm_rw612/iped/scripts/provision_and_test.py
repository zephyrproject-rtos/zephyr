...
_ALLOWED_COMMANDS = frozenset({"blhost", "nxpimage", "west"})


def _safe_join(parent, *parts):
    """Join path components and verify the result stays under *parent*."""
    joined = os.path.realpath(os.path.join(parent, *parts))
    real_parent = os.path.realpath(parent)
    if not (joined == real_parent or joined.startswith(real_parent + os.sep)):
        raise ValueError(f"Path traversal: {joined!r} escapes {real_parent!r}")
    return joined
...
def run_cmd(cmd, check=True, capture=True, timeout=30, cwd=None):
    ...
    if cmd[0] not in _ALLOWED_COMMANDS:
        raise ValueError(f"Unexpected command {cmd[0]!r}")
...
def step_extract(zephyr_bin, output_dir):
    ...
    fcb_bin = _safe_join(output_dir, "fcb.bin")
    img_ver_bin = _safe_join(output_dir, "image_version.bin")
    headerless_bin = _safe_join(output_dir, "headerless.bin")
    padded_bin = _safe_join(output_dir, "headerless_padded.bin")
