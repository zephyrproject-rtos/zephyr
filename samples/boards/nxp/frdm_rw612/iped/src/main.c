...
/* Access memc driver IPED APIs (IPWR_EN control) */
...
#define FLEXSPI_DEV DEVICE_DT_GET(DT_PARENT(DT_CHOSEN(zephyr_flash_controller)))
...
{
	int rc = memc_flexspi_iped_ipwr_enable(FLEXSPI_DEV, true);

	zassert_equal(rc, 0, "%s failed", __func__);
}

static void ipwr_disable(void)
{
	int rc = memc_flexspi_iped_ipwr_enable(FLEXSPI_DEV, false);
