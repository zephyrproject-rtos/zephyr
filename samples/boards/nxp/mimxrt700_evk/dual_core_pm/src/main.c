...
/*
 * Wait for the operator to press ENTER. The run is a single pass, so it must
 * not start before the measurement equipment is armed; once it is running it
 * needs no further input, which is what lets a test rig traverse the whole
 * matrix in one go.
 */
static void wait_for_enter(void)
{
	printk("\nPress ENTER to run all %u cases once: ", (uint32_t)ARRAY_SIZE(test_cases));
...
int main(void)
{
	...
	printk("\nCPU0: run: %u cases, window=%d s, settle=%d s\n",
	       (uint32_t)ARRAY_SIZE(test_cases), CONFIG_SAMPLE_RT700_DUAL_CORE_PM_SLEEP_SECONDS,
	       CONFIG_SAMPLE_RT700_DUAL_CORE_PM_SETTLE_SECONDS);

	/*
	 * One pass over the matrix. Each case runs its window for SLEEP_SECONDS
	 * and is separated from the next by SETTLE_SECONDS of quiet, so the
	 * windows show up as distinct plateaus on a current trace.
	 */
	for (uint32_t i = 0; i < ARRAY_SIZE(test_cases); i++) {
		if (i != 0U) {
			k_sleep(K_SECONDS(CONFIG_SAMPLE_RT700_DUAL_CORE_PM_SETTLE_SECONDS));
		}

		run_test_case(i + 1U, &test_cases[i]);
	}

	printk("\nCPU0: run complete: %u cases\n", (uint32_t)ARRAY_SIZE(test_cases));
