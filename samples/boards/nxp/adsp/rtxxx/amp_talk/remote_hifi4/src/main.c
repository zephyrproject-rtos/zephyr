...
static metal_phys_addr_t shm_physmap[] = {SHM_START_ADDR_HIFI4};
...
static unsigned char ipc_virtio_get_status(struct virtio_device *dev)
{
	...
	sys_cache_data_invd_range((void *)VDEV_STATUS_ADDR_HIFI4, sizeof(status));
	status = sys_read8(VDEV_STATUS_ADDR_HIFI4);
...
void app_task(void *arg1, void *arg2, void *arg3)
{
	...
	printk("OpenAMP[DSP] demo started\r\n");

	status = metal_init(&metal_params);
	if (status != 0) {
		printk("[DSP] metal_init: failed - error code %d\n", status);
		return;
	}

	/* declare shared memory region */
	metal_io_init(io, (void *)SHM_START_ADDR_HIFI4, shm_physmap, SHM_SIZE_HIFI4, -1, 0, NULL);

	/* setup IPM */
	if (!device_is_ready(ipm_handle)) {
		printk("[DSP] IPM device is not ready\n");
		return;
	}
	...
	if (status != 0) {
		printk("[DSP] ipm_set_enabled failed\n");
		return;
	}
	...
	if (vqueue[0] == NULL) {
		printk("[DSP] virtqueue_allocate failed to alloc vqueue[0]\n");
		return;
	}
	vqueue[1] = virtqueue_allocate(VRING_SIZE);
	if (vqueue[1] == NULL) {
		printk("[DSP] virtqueue_allocate failed to alloc vqueue[1]\n");
		return;
	}
	...
	vdev.func = &dispatch;
	rvrings[0].io = io;
	rvrings[0].info.vaddr = (void *)VRING_TX_ADDRESS_HIFI4;
	rvrings[0].info.num_descs = VRING_SIZE;
	rvrings[0].info.align = VRING_ALIGNMENT;
	rvrings[0].vq = vqueue[0];

	rvrings[1].io = io;
	rvrings[1].info.vaddr = (void *)VRING_RX_ADDRESS_HIFI4;
	...
	if (status != 0) {
		printk("[DSP] rpmsg_init_vdev failed %d\n", status);
		return;
	}

	rdev = rpmsg_virtio_get_rpmsg_device(&rvdev);
	status = rpmsg_create_ept(ep, rdev, "k", RPMSG_ADDR_ANY, RPMSG_ADDR_ANY, endpoint_cb,
				  rpmsg_service_unbind);
	if (status != 0) {
		printk("[DSP] rpmsg_create_ept failed %d\n", status);
		return;
	}

	while (message < 99) {
		message = receive_message();
		printk("[DSP] Remote core received a message: %d\n", message);

		message++;
		status = send_message(message);
		if (status < 0) {
			printk("[DSP] send_message(%d) failed with status %d\n", message, status);
			goto _cleanup;
		}
	}
...
_cleanup:
	...
	k_msleep(100);
	printk("[DSP] OpenAMP demo ended.\n");
}

int main(void)
{
	printk("[DSP] Starting application thread!\n");
