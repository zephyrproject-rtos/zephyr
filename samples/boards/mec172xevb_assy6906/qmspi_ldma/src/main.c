...
static int spi_flash_erase_region(const struct device *dev, struct spi_config *spi_cfg,
				  uint8_t erase_opcode, struct spi_address *spi_addr, int timeout_ms)
{
	int err = 0;
	...
	err = spi_flash_fd_wr_cpd(dev,spi_cfg, erase_opcode, cmdparams, spi_addr->byte_len, NULL, 0);
	if (err) {
		return err;
	}
...
int main(void)
{
	...
	for (int i = 0; i < num_sectors; i++) {
		...
		err = spi_flash_fd_wr_cpd(spi_dev, &spi_cfg, SPI_FLASH_WRITE_ENABLE_OPCODE, NULL, 0, NULL, 0);
		if (err) {
		...
		err = spi_flash_erase_region(spi_dev, &spi_cfg, SPI_FLASH_ERASE_SECTOR_OPCODE, &spi_addr, 1000u);
		if (err) {
	...
	for (int i = 0; i < num_pages; i++) {
		...
		printf("Page at 0x%08x: Transmit SPI flash Write-Enable, Page-Program, and poll status\n", spi_addr.addr);
		err = spi_flash_fd_wr_cpd(spi_dev, &spi_cfg, SPI_FLASH_WRITE_ENABLE_OPCODE, NULL, 0, NULL, 0);
		if (err) {
	...
	err = spi_flash_read_hd_sync(spi_dev, &spi_cfg, &spi_cfg_dual, &spi_addr, &tb1.b[0], spi_len,
				     SPI_FLASH_READ_DUAL_OPCODE);
	if (err) {
	...
	err = spi_flash_read_hd_sync(spi_dev, &spi_cfg, &spi_cfg_quad, &spi_addr, &tb1.b[0], spi_len,
				     SPI_FLASH_READ_QUAD_OPCODE);
	if (err) {
