...
/* The engine reads and writes these over 2D-DMA and the results are
 * invalidated afterwards, so they start and end on a cache line.
 */
#define PPA_SRM_BUF_ALIGN CONFIG_ESP32_CACHE_L2_LINE_SIZE
		   ...
		   bool mirror_x, bool mirror_y, uint32_t *out_w, uint32_t *out_h)
{
	...
	*out_h = h;
	...
	sys_cache_data_flush_range(dst, ROUND_UP(OUT_W * OUT_H * BPP, PPA_SRM_BUF_ALIGN));
	sys_cache_data_flush_range(src, ROUND_UP(SRC_W * SRC_H * BPP, PPA_SRM_BUF_ALIGN));
	...
	sys_cache_data_invd_range(dst, ROUND_UP(OUT_W * OUT_H * BPP, PPA_SRM_BUF_ALIGN));
...
static int present(void)
{
	...
	sys_cache_data_flush_range(screen, ROUND_UP(SCREEN_W * SCREEN_H * BPP, PPA_SRM_BUF_ALIGN));

	int err = display_write(display_dev, 0, 0, &d, screen);
...
int main(void)
{
	...
	screen = shared_multi_heap_aligned_alloc(
		SMH_REG_ATTR_EXTERNAL, PPA_SRM_BUF_ALIGN,
		ROUND_UP(SCREEN_W * SCREEN_H * BPP, PPA_SRM_BUF_ALIGN));
	src = shared_multi_heap_aligned_alloc(SMH_REG_ATTR_EXTERNAL, PPA_SRM_BUF_ALIGN,
					      ROUND_UP(SRC_W * SRC_H * BPP, PPA_SRM_BUF_ALIGN));
	dst = shared_multi_heap_aligned_alloc(SMH_REG_ATTR_EXTERNAL, PPA_SRM_BUF_ALIGN,
					      ROUND_UP(OUT_W * OUT_H * BPP, PPA_SRM_BUF_ALIGN));
	...
	while (1) {
		uint32_t i = step % ARRAY_SIZE(steps);
		uint32_t ow, oh;
