...
static void rx_callback(const struct device *dev, uint32_t channel, void *user_data,
			struct mbox_msg *msg)
{
	printk("Pong [%s] (on channel %d [data %llu, %d])\n", core_a ? "core A" : "core B", channel,
	       *((uint64_t *)msg->data), (int)msg->size);
}
...
int main(void)
{
	...
	mtu = mbox_mtu_get(dev);
	if ((mtu < 0) || ((size_t)mtu > sizeof(data))) {
	...
	while (1) {
		struct mbox_msg msg = {.data = &data, .size = mtu};
