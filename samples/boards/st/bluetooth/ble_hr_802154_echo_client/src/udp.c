...
LOG_MODULE_DECLARE(ble_hr_802154_echo_client_sample, LOG_LEVEL_DBG);
	...
	while (true) {
		...
		for (int i = 0; i < ARRAY_SIZE(events); i++) {
			...
			if (events[i].signal == &udp_kill) {
				...
				if (events[i].signal == &udp6_ctrl.tx_signal) {
...
static void wait_reply(struct k_timer *timer)
{
	...
	struct sample_data __maybe_unused *data = (ctrl == conf.ipv4.udp.ctrl)
						  ? &conf.ipv4 : &conf.ipv6;
