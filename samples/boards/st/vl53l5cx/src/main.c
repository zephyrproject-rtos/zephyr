 ...
 * 0mm -> white, 4000mm -> black
 */
static lv_color_t distance_to_gray(int32_t d_mm)
{
	if (d_mm > CONFIG_SAMPLE_VL53L5CX_DIST_MAX_MM) {
		d_mm = CONFIG_SAMPLE_VL53L5CX_DIST_MAX_MM;
	}

	uint8_t gray = (uint8_t)(255 - (d_mm * 255 / CONFIG_SAMPLE_VL53L5CX_DIST_MAX_MM));
...
static int32_t q29_to_mm(int32_t q29)
{
	return (int32_t)(((int64_t)q29 * 1000) >> 29);
}
...
void sensor_processing_callback(int result, uint8_t *buf, uint32_t buf_len, void *userdata)
{
	...
	for (int i = 0; i < SENSOR_CELL_N; i++) {
		...
		/* Update the display */
		color = distance_to_gray(q29_to_mm(decoded_data.readings[0].value));
...
int main(void)
{
	...
	if (!device_is_ready(sensor_dev)) {
		LOG_ERR("Sensor device not ready, aborting test");
		return 0;
	}
