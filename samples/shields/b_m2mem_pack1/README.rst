...
A board with an M.2 Key A slot wired according to the ST serial memory
connector pinout (see `UM3611`_), exposing the ``m2mem_connector`` node and
the labels listed in the :ref:`st_b_m2mem_pack1_shield` shield documentation,
with a B-M2MEM-PACK1 module fitted. The tests write to the flash at offset
...
This example uses a :zephyr:board:`nucleo_u3c5zi_q` board. Select the shield
variant that matches the fitted module:
