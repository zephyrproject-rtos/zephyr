...
static void lis2duxs12_acc_trig_handler(const struct device *dev, const struct sensor_trigger *trig)
{
	sensor_sample_fetch_chan(dev, SENSOR_CHAN_ALL);
...
static void lsm6dso16is_config(const struct device *lsm6dso16is)
{
	...
	if (sensor_attr_set(lsm6dso16is, SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_CONFIGURATION,
			    &mode_attr) < 0) {
...
static void lsm6dsv16x_config(const struct device *lsm6dsv16x)
{
	...
	if (sensor_attr_set(lsm6dsv16x, SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_CONFIGURATION,
			    &mode_attr) < 0) {
...
static void lis2duxs12_config(const struct device *lis2duxs12)
{
	...
	if (sensor_attr_set(lis2duxs12, SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_SAMPLING_FREQUENCY,
			    &odr_attr) < 0) {
		printk("Cannot set sampling frequency for LSM6DSV16X accel\n");
		return;
	}
	...
	if (sensor_attr_set(lis2duxs12, SENSOR_CHAN_ACCEL_XYZ, SENSOR_ATTR_FULL_SCALE, &fs_attr) <
	    0) {
