...
int main(void)
{
	...
	while (1) {
		...
		printf("GRVX: %c%d.%06d; GRVY: %c%d.%06d; GRVZ: %c%d.%06d; GRVW: %c%d.%06d\n",
		       (game_rv[0].val1 < 0 || game_rv[0].val2 < 0) ? '-' : ' ', ABS(game_rv[0].val1), ABS(game_rv[0].val2),
		       (game_rv[1].val1 < 0 || game_rv[1].val2 < 0) ? '-' : ' ', ABS(game_rv[1].val1), ABS(game_rv[1].val2),
		       (game_rv[2].val1 < 0 || game_rv[2].val2 < 0) ? '-' : ' ', ABS(game_rv[2].val1), ABS(game_rv[2].val2),
		       (game_rv[3].val1 < 0 || game_rv[3].val2 < 0) ? '-' : ' ', ABS(game_rv[3].val1), ABS(game_rv[3].val2)
		);
	}
	return 0;
}
