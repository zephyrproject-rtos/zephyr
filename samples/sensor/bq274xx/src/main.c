...
{
	int status = 0;
	struct sensor_value voltage, current, state_of_charge, full_charge_capacity,
		remaining_charge_capacity, avg_power, int_temp, current_standby, current_max_load,
		state_of_health;
	...
	while (1) {
		...
		status = sensor_sample_fetch_chan(dev, SENSOR_CHAN_GAUGE_STDBY_CURRENT);
		if (status < 0) {
		...
		bq274xx_show_values("Max Load Current in Amps", current_max_load);
		...
		status = sensor_sample_fetch_chan(dev, SENSOR_CHAN_GAUGE_STATE_OF_HEALTH);
		if (status < 0) {
		...
		status = sensor_sample_fetch_chan(dev, SENSOR_CHAN_GAUGE_AVG_POWER);
		if (status < 0) {
		...
		status = sensor_sample_fetch_chan(dev, SENSOR_CHAN_GAUGE_FULL_CHARGE_CAPACITY);
		if (status < 0) {
		...
		status = sensor_sample_fetch_chan(dev, SENSOR_CHAN_GAUGE_REMAINING_CHARGE_CAPACITY);
		if (status < 0) {
