...
//#define TRIG_X(trig, opt, ...) (uint8_t)(trig), (uint8_t)(OPT_FROM_TOKEN(opt)),
//const uint8_t trigger_state[] = {TRIGGER_LIST};
//#undef TRIG_X
#else
...
//#if CONFIG_SAMPLE_STREAM
//	for (size_t i = 0; i < ARRAY_SIZE(trigger_state); i+=2) {
//		LOG_WRN("Trigger(%d): [%d]", trigger_state[i], trigger_state[i+1]);
//	}
//#endif /* CONFIG_SAMPLE_STREAM */
