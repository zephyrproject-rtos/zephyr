...
int main(void)
{
	...
	struct sensor_decoder_context temp_decoder = SENSOR_DECODER_CONTEXT_INIT(
		SENSOR_DECODER_DT_GET(DT_ALIAS(dht0)), buf, SENSOR_CHAN_AMBIENT_TEMP, 0);

	while (1) {
		/* Blocking read */
		rc = sensor_read(dht_iodev, &dht_ctx, buf, sizeof(buf));
