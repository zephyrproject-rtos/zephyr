...
int main(void)
{
	...
	if (sensor_trigger_set(dev, &trigger, data_ready_handler) < 0) {
	...
	if (sensor_trigger_set(dev, &trigger, threshold_handler) < 0) {
	...
	if (sensor_trigger_set(dev, &trigger, delta_handler) < 0) {
		printf("Cannot configure threshold trigger!!!\n");
		return 0;
	}
#endif
