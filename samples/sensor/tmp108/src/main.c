...
void temperature_one_shot(const struct device *dev, const struct sensor_trigger *trigger)
{
...
void temperature_alert(const struct device *dev, const struct sensor_trigger *trigger)
{
...
void enable_temp_alerts(const struct device *tmp108)
{
	...
	sensor_attr_set(tmp108, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_ALERT, &thermostat_mode);

	sensor_attr_set(tmp108, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_HYSTERESIS,
			&alert_hysterisis);

	sensor_attr_set(tmp108, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_UPPER_THRESH,
			&alert_upper_thresh);

	sensor_attr_set(tmp108, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_LOWER_THRESH,
			&alert_lower_thresh);

	sensor_trigger_set(tmp108, &sensor_trigger_type_temp_alert, temperature_alert);
}
...
void enable_one_shot(const struct device *tmp108)
{
	...
	sensor_attr_set(tmp108, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_TMP108_ONE_SHOT_MODE, NULL);

	sensor_trigger_set(tmp108, &sensor_trigger_type_temp_one_shot, temperature_one_shot);
}
...
void get_temperature_continuous(const struct device *tmp108)
{
	...
	result = sensor_channel_get(tmp108, SENSOR_CHAN_AMBIENT_TEMP, &temp_value);
...
int main(void)
{
	...
	sensor_attr_set(temp_sensor, SENSOR_CHAN_AMBIENT_TEMP,
