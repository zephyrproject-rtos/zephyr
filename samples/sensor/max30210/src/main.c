...
void handler(const struct device *dev, const struct sensor_trigger *trig)
{
	...
	// ret = max30210_reg_read(dev, 0x00, &interrupt_status, 1); // Read interrupt status register
...
ret = sensor_attr_set(max30210_dev, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_UPPER_THRESH, &(struct sensor_value){.val1 = 28, .val2 = 0});
if (ret < 0) {
	printf("Failed to set upper threshold: %d\n", ret);
	return ret;
}
ret = sensor_attr_set(max30210_dev, SENSOR_CHAN_AMBIENT_TEMP, SENSOR_ATTR_LOWER_THRESH, &(struct sensor_value){.val1 = 20, .val2 = 0});
if (ret < 0) {
...
// 	return ret;
// }
// // ret = max30210_reg_write(max30210_dev, 0x22, alarm_high_setup, 2); // Clear TEMP_ALARM_HIGH_SETUP register
// // if (ret < 0) {
// // 	printf("Failed to write TEMP_ALARM_HIGH_SETUP: %d\n", ret);
// // 	return ret;
// // }
