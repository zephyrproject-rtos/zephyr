...
def test_sensor_shell_get(shell: Shell):
    ...
    lines = shell.exec_command('sensor get sensor@1 54')
    assert any(['channel type=54(gauge_state_of_health)' in line for line in lines]), 'expected response not found'
    ...
    assert any(['channel type=60(gauge_desired_charging_current)' in line for line in lines]), 'expected response not found'
