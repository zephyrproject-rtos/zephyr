...
def test_sensor_info(shell: Shell):
...
def test_sensor_get(shell: Shell):
    ...
    assert any(['channel idx=53(gauge_time_to_empty)' in line for line in lines]), 'expected response not found'
...
def test_sensor_attr_get(shell: Shell):
...
def test_sensor_attr_set(shell: Shell):
...
def test_sensor_trig(shell: Shell):
