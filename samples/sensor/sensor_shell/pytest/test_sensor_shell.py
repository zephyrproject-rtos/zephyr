...
def test_sensor_shell_get(shell: Shell):
    ...
    assert any(['channel type=33(voltage)' in line for line in lines]), 'expected response not found'

    lines = shell.exec_command('sensor get sensor@1 55')
    assert any(['channel type=55(gauge_state_of_health)' in line for line in lines]), 'expected response not found'
    ...
    assert any(['channel type=61(gauge_desired_charging_current)' in line for line in lines]), 'expected response not found'
