...
static atomic_t current_state = ATOMIC_INIT(PAJ7620_NORMAL_MODE); /* global system state*/
...
void paj7620_button_work_handler(struct k_work *work)
{
 ...
 if  (atomic_get(&current_state) ==  PAJ7620_NORMAL_MODE){
    val.val1 = (int32_t)PAJ7620_SLEEP_MODE;
	ret = sensor_attr_set(dev, (enum sensor_channel)SENSOR_CHAN_PAJ7620_GESTURES, 
	(enum sensor_attribute)SENSOR_ATTR_POWER_MODE, &val);
    if (ret == 0 ){
		atomic_set(&current_state , PAJ7620_SLEEP_MODE);
		printf("Device in Sleep Mode\n");
	}
 }
 else {
	val.val1 = (int32_t)PAJ7620_NORMAL_MODE;
	ret = sensor_attr_set(dev, (enum sensor_channel)SENSOR_CHAN_PAJ7620_GESTURES, 
	(enum sensor_attribute)SENSOR_ATTR_POWER_MODE, &val);
    if (ret == 0 ){
		atomic_set(&current_state , PAJ7620_NORMAL_MODE);
