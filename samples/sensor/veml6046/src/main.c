...
static void read_with_attr(const struct device *dev, int it, int pdd, int gain)
{
	...
	sen.val1 = it;
	ret = sensor_attr_set(dev, SENSOR_CHAN_LIGHT, SENSOR_ATTR_VEML6046_IT, &sen);
	if (ret) {
		printf("Failed to set it attribute ret: %d\n", ret);
	}
	sen.val1 = pdd;
	ret = sensor_attr_set(dev, SENSOR_CHAN_LIGHT, SENSOR_ATTR_VEML6046_PDD, &sen);
	if (ret) {
		printf("Failed to set pdd attribute ret: %d\n", ret);
	}
	sen.val1 = gain;
	ret = sensor_attr_set(dev, SENSOR_CHAN_LIGHT, SENSOR_ATTR_VEML6046_GAIN, &sen);
	if (ret) {
	...
	sensor_channel_get(dev, SENSOR_CHAN_VEML6046_IR_RAW_COUNTS, &ir_raw);
...
static void read_with_all_attr(const struct device *dev)
{
	...
	for (it = VEML6046_IT_3_125; it <= VEML6046_IT_400; it++) {
		for (pdd = VEML6046_SIZE_2_2; pdd <= VEML6046_SIZE_1_2; pdd++) {
			for (gain = VEML6046_GAIN_1; gain <= VEML6046_GAIN_0_5; gain++) {
