...
static void trigger_handler(const struct device *dev, const struct sensor_trigger *trig)
{
	...
	printf("Data_ready TIME(ms) %lld MX: %f MY: %f MZ: %f\n", current_time, report_mag[0],
	       report_mag[1], report_mag[2]);
}
#endif
...
int main(void)
{
	...
	oversampling.val1 = 2; /* averaging mode (one of 0,2,4,8) */
	oversampling.val2 = 0;
	sensor_attr_set(dev, SENSOR_CHAN_MAGN_XYZ, SENSOR_ATTR_SAMPLING_FREQUENCY, &sampling_freq);
	sensor_attr_set(dev, SENSOR_CHAN_MAGN_XYZ, SENSOR_ATTR_OVERSAMPLING, &oversampling);
