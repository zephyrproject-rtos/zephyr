...
#define UART_IRQ_NUM   DT_IRQN(UART_NODE)
#define UART_EIID      DT_IRQ_BY_IDX(UART_NODE, 0, irq)
...
static void hart1_init_thread(void *p1, void *p2, void *p3)
{
	...
	riscv_imsic_enable_eiid(UART_EIID);
...
int main(void)
{
	...
	for (int target_hart = 0; target_hart < CONFIG_MP_MAX_NUM_CPUS; target_hart++) {
		printk("Injecting MSI to hart %d (EIID %d)...\n", target_hart, UART_EIID);
		riscv_aia_route_to_hart(UART_IRQ_NUM, target_hart, UART_EIID);
		k_msleep(100); /* Let routing take effect */
		riscv_aia_inject_msi(target_hart, UART_EIID);
