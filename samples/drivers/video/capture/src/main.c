...
int main(void)
{
	...
	if (video_set_frmival(uvc_host, &frmival) == 0) {
		LOG_INF("- Target frame rate set to: %f fps",
			1.0 * frmival.denominator / frmival.numerator);
	} else {
		LOG_WRN("- Failed to set target frame rate %d fps, using device default",
			CONFIG_VIDEO_TARGET_FPS);
	}
#endif
	...
	buffers[0]->type = type;
	video_enqueue(uvc_host, buffers[0]);

	LOG_INF("Capture started");

	/* Grab video frames. TODO: buffers[0] is used directly now, need to use fifo to control video buffer  */
	vbuf->type = type;
	while (1) {
		err = k_poll(evt, ARRAY_SIZE(evt), timeout);
		if (err != 0 && err != -EAGAIN) {
...
#if DT_HAS_CHOSEN(zephyr_display)
		video_display_frame(display_dev, buffers[0], fmt);
#endif
		err = video_enqueue(uvc_host, buffers[0]);
		if (err) {
