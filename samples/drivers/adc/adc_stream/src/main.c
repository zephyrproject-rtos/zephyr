...
static int print_adc_stream(const struct device *dev, struct rtio_iodev *iodev,
				struct adc_sequence *seq)
{
	...
	/* Start the streams */
	adc_stream(iodev, &adc_ctx, (void *)seq, &handles);
...
int main(void)
{
	...
	ret = print_adc_stream(adc_channels[0].dev, &iodev, &sequence);
