...
static int get_lcd_device(const struct device **dev)
{
	...
	LOG_INF("14 Segment LCD Sample Application");
...
static int print_title_and_clear(const struct device *dev)
{
	...
	/*
	 * Send a test string to the 14-segment LCD display using standard Zephyr API.
...
static int check_panel_properties(const struct device *dev)
{
	...
	if (caps.columns < EXPECTED_LCD_COLUMNS_MIN) {
		LOG_WRN("This auxdisplay example needs at least 6 columns, found just "
			"%d, texts are going to be truncated",
			caps.columns);
	...
	if (SLCD_PANEL_NUM_INDICATORS < EXPECTED_LCD_INDICATORS_MIN) {
		LOG_WRN("This auxdisplay example needs at least 4 indicators, found "
			"just %d, bar tests could have an esthetically unexpected behaviour",
...
enum custom_character {
	...
	CUSTOM_CHARACTER_BLOCKY_E,
	CUSTOM_CHARACTER_BLOCKY_F,
	CUSTOM_CHARACTER_BLOCKY_Y,
	CUSTOM_CHARACTER_COUNT
};

static const uint8_t custom_character_data[][EXPECTED_LCD_SEGMENT_TYPE] = {
	...
	{0xff, 0x0, 0x0, 0xff, 0xff, 0xff, 0xff, 0x0, 0x0, 0x0, 0xff, 0x0, 0x0, 0x0},
	{0xff, 0x0, 0x0, 0x0, 0xff, 0xff, 0xff, 0x0, 0x0, 0x0, 0xff, 0x0, 0x0, 0x0},
	{0x0, 0xff, 0x0, 0x0, 0x0, 0xff, 0xff, 0x0, 0x0, 0x0, 0xff, 0x0, 0xff, 0x0},
};
BUILD_ASSERT(ARRAY_SIZE(custom_character_data) == CUSTOM_CHARACTER_COUNT,
	     "Missing or too many custom character data!");

static const char *const custom_character_names[] = {
	"full",     "degree",   "low-ring", "prefix-d", "prefix-c", "prefix-m",
	"prefix-u", "prefix-n", "blocky-E", "blocky-F", "blocky-Y",
};
BUILD_ASSERT(ARRAY_SIZE(custom_character_data) == CUSTOM_CHARACTER_COUNT,
	     "Missing or too many custom character names!");
...
static int set_custom_characters(const struct device *dev)
{
	...
	for (int i = 0; i < CUSTOM_CHARACTER_COUNT; i++) {
		uint8_t *character_data = (uint8_t *)custom_character_data[i];
...
static int write_all_characters(const struct device *dev)
{
	...
	const char msg_blocky_letters[] = {/* E F Y */
					   custom_character_codes[CUSTOM_CHARACTER_BLOCKY_E],
					   custom_character_codes[CUSTOM_CHARACTER_BLOCKY_F],
					   custom_character_codes[CUSTOM_CHARACTER_BLOCKY_Y],
					   'E',
					   'F',
					   'Y',
					   0};
	...
	k_msleep(1000);
	auxdisplay_cursor_position_set(dev, AUXDISPLAY_POSITION_ABSOLUTE, 0, 0);
	auxdisplay_write(dev, (const uint8_t *)msg_blocky_letters, strlen(msg_blocky_letters));
	k_msleep(1000);
	auxdisplay_cursor_position_set(dev, AUXDISPLAY_POSITION_ABSOLUTE, 0, 0);
