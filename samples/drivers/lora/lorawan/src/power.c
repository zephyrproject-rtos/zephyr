...
static void low_power_state_exit(void)
{
	...
	//SYS_LOG_DBG("-- Low Power State exit !");
}

static int low_power_state_entry(void)
{
	if (pm_state == SYS_POWER_STATE_CPU_LPS) {
        //SYS_LOG_DBG("--> Entering into Constant Latency State -");
	} else {
        //SYS_LOG_DBG("--> Entering into Low Power State -");
	}
...
static void low_power_suspend_exit(void)
{
	...
	resume_devices();
    //SYS_LOG_DBG("-- Low Power Suspend State exit!\n");
}

static int low_power_suspend_entry(void)
{
    SYS_LOG_DBG("--> Entering into Deep Sleep State -");
