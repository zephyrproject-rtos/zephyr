...
static const struct adc_dt_spec adc_channel = ADC_DT_SPEC_GET(DT_NODELABEL(resources));
static const struct device *opamp_dev = DEVICE_DT_GET(DT_PHANDLE(DT_NODELABEL(resources), opamp));
static const int32_t adc_read_delay_ms = DT_PROP(DT_NODELABEL(resources), adc_read_delay_ms);
