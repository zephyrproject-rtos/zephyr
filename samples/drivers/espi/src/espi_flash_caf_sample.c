...
/* Ensure test buffer multiples of max flash request size*/
#define TEST_BUF_SIZE   (MAX_FLASH_REQUEST * 16U)


static const struct device *const espi_dev = DEVICE_DT_GET(DT_NODELABEL(espi0));

uint8_t flash_write_buf[TEST_BUF_SIZE];
uint8_t flash_read_buf[TEST_BUF_SIZE];
