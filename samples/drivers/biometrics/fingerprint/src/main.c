...
static int enroll_fingerprint(uint16_t template_id)
{
	...
	ret = biometric_enroll_capture(fingerprint, K_SECONDS(5));
	if (ret < 0) {
...
static void test_verification(uint16_t template_id)
{
	...
	if (ret >= 0) {
		LOG_INF("Match! Confidence: %d", ret);
	} else if (ret == -ENOENT) {
		LOG_WRN("No match");
...
static void test_identification(void)
{
	...
	ret = biometric_match(fingerprint, BIOMETRIC_MATCH_IDENTIFY, 0, K_SECONDS(5));

	if (ret >= 0) {
		biometric_attr_get(fingerprint, BIOMETRIC_ATTR_PRIV_START, &matched_id);
		LOG_INF("Matched ID %d, confidence %d", matched_id, ret);
	} else if (ret == -ENOENT) {
