...
/* Pattern the host sends through the bridge controller. */
...
/* Pattern the MCU serial target sends back to the bridge controller. */
...
static void target_thread(void *p1, void *p2, void *p3)
{
	...
	const struct spi_config cfg = {
		.frequency = 1875000,
		.operation = SPI_OP_MODE_PERIPHERAL | SPI_WORD_SET(8) | SPI_LINES_SINGLE,
		.slave = 0,
	};
...
int main(void)
{
	...
	const struct spi_config mcfg = {
		.frequency = 1875000,
		.operation = SPI_OP_MODE_CONTROLLER | SPI_WORD_SET(8) | SPI_LINES_SINGLE,
		.peripheral = 0,
	};
	struct spi_buf tx_buf = {
		.buf = (uint8_t *)host_tx,
	...
	printk("peripheral spi: %s\n", target_spi->name);
	printk("\n[full-duplex: bridge controller <-> MCU serial peripheral]\n");

	/* Arm the serial peripheral first so it is ready when the bridge clocks it. */
...
#else
	printk("\n[bridge controller -> extra header SPI peripheral]\n");
	...
	if (memcmp(host_rx, host_tx, sizeof(host_tx)) == 0) {
		printk("PASS: RX matches TX (peripheral echoes or MOSI-MISO jumpered)\n");
	} else {
		printk("INFO: RX differs from TX (peripheral responded with its own data or MISO idle)\n");
	}
#endif
