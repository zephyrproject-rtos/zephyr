...
void uart_cb(const struct device *dev, void *user_data)
{
    ...
    if (uart_irq_tx_ready(dev) && tx_idx < len) {
        int bytes_written = uart_fifo_fill(dev, (const uint8_t *)&tx_data[tx_idx], len - tx_idx);
    ...
    if (uart_irq_tx_complete(dev) && tx_idx >= len) {
        uart_irq_tx_disable(dev); // Disable TX interrupts
        tx_idx = 0;
        tx_busy = false;
        LOG_DBG("TX Complete");
        k_sem_give(&tx_done_sem); // Signal that transmission is complete
    }
    ...
    if (uart_irq_rx_ready(dev)) {
        ...
        while (uart_fifo_read(dev, &rx_data, 1)) {
            ...
            // Add data processing here if needed
        }
    }
}
...
int main(void)
{
    uart1 = DEVICE_DT_GET(DT_NODELABEL(usart1)); // First UART RS485
    ...
    uart_irq_rx_enable(uart1); // Enable RX interrupts if necessary
    uart_irq_tx_disable(uart1); // Disable TX interrupts until transmission starts
    ...
    while (1) {
        ...
        if (!tx_busy) {
            tx_busy = true;
            uart_irq_tx_enable(uart1); // Enable TX interrupts to start transmission
