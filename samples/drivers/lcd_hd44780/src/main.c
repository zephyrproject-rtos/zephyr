...
void _pi_lcd_4bits_wr(const struct device *gpio_dev, u8_t bits)
{
	/* High bits */
...
void _pi_lcd_8bits_wr(const struct device *gpio_dev, u8_t bits)
{
	/* High bits */
	GPIO_PIN_WR(gpio_dev, GPIO_PIN_PC21_D7, LOW);
	GPIO_PIN_WR(gpio_dev, GPIO_PIN_PC22_D6, LOW);
...
void _pi_lcd_data(const struct device *gpio_dev, u8_t bits)
{
...
void _pi_lcd_command(const struct device *gpio_dev, u8_t bits)
{
...
void _pi_lcd_write(const struct device *gpio_dev, u8_t bits)
{
...
/** Set curson position */
void pi_lcd_set_cursor(const struct device *gpio_dev, u8_t col, u8_t row)
{
	size_t max_lines;
...
void pi_lcd_init(const struct device *gpio_dev, u8_t cols, u8_t rows,
		 u8_t dotsize)
{
	if (rows > 1) {
