...
int main(void)
{
	...
	int ret;
	uint32_t tx_idx;
	const struct device *sai1_a = DEVICE_DT_GET(DT_ALIAS(sai1a));
	const struct device *sai1_b = DEVICE_DT_GET(DT_ALIAS(sai1b));

	if (!device_is_ready(sai1_a)) {
		printf("I2S device not ready\n");
		return -ENODEV;
	}

	if (!device_is_ready(sai1_b)) {
		printf("I2S device not ready\n");
		return -ENODEV;
	}
	...
	ret = i2s_configure(sai1_a, I2S_DIR_TX, &i2s_cfg);
	if (ret < 0) {
		printf("Failed to configure I2S stream\n");
		return ret;
	}
	ret = i2s_configure(sai1_b, I2S_DIR_TX, &i2s_cfg);
	if (ret < 0) {
		printf("Failed to configure I2S stream\n");
		return ret;
	}
	...
	tx_idx = 0;
	/* Send first block */
	ret = i2s_write(sai1_a, tx_block[tx_idx], BLOCK_SIZE);
	if (ret < 0) {
		printf("Could not write TX buffer %d\n", tx_idx);
		return ret;
	}
	ret = i2s_write(sai1_b, tx_block[tx_idx++], BLOCK_SIZE);
	if (ret < 0) {
		printf("Could not write TX buffer %d\n", tx_idx);
		return ret;
	}
	/* Trigger the I2S transmission */
	ret = i2s_trigger(sai1_a, I2S_DIR_TX, I2S_TRIGGER_START);
	if (ret < 0) {
		printf("Could not trigger I2S tx\n");
		return ret;
	}
	ret = i2s_trigger(sai1_b, I2S_DIR_TX, I2S_TRIGGER_START);
	if (ret < 0) {
		printf("Could not trigger I2S tx\n");
		return ret;
	}
	...
	/* Drain TX queue */
	ret = i2s_trigger(sai1_a, I2S_DIR_TX, I2S_TRIGGER_DRAIN);
	if (ret < 0) {
		printf("Could not trigger I2S tx\n");
		return ret;
	}

	ret = i2s_trigger(sai1_b, I2S_DIR_TX, I2S_TRIGGER_DRAIN);
	if (ret < 0) {
		printf("Could not trigger I2S tx\n");
		return ret;
	}
