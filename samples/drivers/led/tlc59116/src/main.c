...
int main(void)
{
	const struct device *const led_dev = DEVICE_DT_GET_ANY(ti_tlc59116);
	uint8_t ch_buf[16] = {0};
	int i, ret;

	if (!led_dev) {
		LOG_ERR("No device with compatible ti,tlc59116 found");
		return 0;
