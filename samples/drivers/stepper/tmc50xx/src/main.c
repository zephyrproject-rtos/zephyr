...
int32_t ping_pong_target_position =
	CONFIG_STEPS_PER_REV * CONFIG_PING_PONG_N_REV * DT_PROP(DT_ALIAS(stepper_driver), micro_step_res);
...
int main(void)
{
	...
	LOG_DBG("Microstep resolution is %d\n", micro_step_res);
	...
	int32_t tmc_velocity = DT_PROP(DT_ALIAS(motion_controller), vmax) * CONFIG_MAX_VELOCITY_MULTIPLIER;
	...
	for (;;) {
		if (k_sem_take(&steps_completed_sem, K_FOREVER) == 0) {
			...
			if (stepper_get_actual_position(stepper_controller, &actual_position) == 0) {
