...
static void stepper_callback(const struct device *dev, const enum stepper_motor_event event,
			     void *user_data)
{
	switch (event) {
...
int main(void)
{
	...
	stepper_motor_set_microstep_interval(stepper_motor, CONFIG_STEP_INTERVAL_NS);
	...
	for (;;) {
		...
		case STEPPER_MODE_PING_PONG_ABSOLUTE:
			ping_pong_target_position *= -1;
			stepper_motor_move_to(stepper_motor, ping_pong_target_position);
			LOG_INF("mode: ping pong absolute");
			break;
		case STEPPER_MODE_ROTATE_CW:
			stepper_motor_run(stepper_motor, STEPPER_MOTOR_DIRECTION_POSITIVE);
			LOG_INF("mode: rotate cw");
			break;
		case STEPPER_MODE_ROTATE_CCW:
			stepper_motor_run(stepper_motor, STEPPER_MOTOR_DIRECTION_NEGATIVE);
...
static void monitor_thread(void)
{
	for (;;) {
		...
		stepper_motor_get_actual_position(stepper_motor, &actual_position);
