...
/**
 * @brief Stores a new permanent, persistent prime256v1 (ecdsa-with-SHA256) key
 *        in ITS, associating it with the specified unique key identifier.
 *
 * This function will store a new permament prime256v1 key in internal trusted
					  ...
					  u8_t *key_data)
{
	...
	size_t key_len = 32;
	size_t data_len;
	u8_t data_out[65] = { 0 }; /* ECDSA public key = 65 bytes. */
	int comp_result;

	LOG_INF("Persisting SECP256R1 key as #%d", (u32_t)key_id);
	al_dump_log();
 ...
 */
static psa_status_t crp_hash_payload(u8_t *msg, size_t msg_len,
				     u8_t *hash, size_t hash_buf_size,
				     size_t *hash_len)
{
	psa_status_t status;
...
static psa_status_t crp_verify_sign(psa_key_id_t key_id,
				    u8_t *hash, size_t hash_len,
				    u8_t *sig, size_t sig_len)
{
	psa_status_t status;
	psa_key_handle_t key_handle;
...
static psa_status_t crp_dest_key(psa_key_id_t key_id)
{
	...
	LOG_INF("Destroyed persistent key #%d", (u32_t)key_id);
	al_dump_log();
...
{
	psa_status_t status;
	u8_t msg[] = "Please hash and sign this message.";
	u8_t hash[PSA_HASH_SIZE(PSA_ALG_SHA_256)] = { 0 };
	size_t hash_len;
	u8_t sig[PSA_VENDOR_ECDSA_SIGNATURE_MAX_SIZE] = { 0 };
	size_t sig_len;
