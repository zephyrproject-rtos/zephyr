...
1. Build Zephyr with a non-secure configuration
   ...
   .. code-block:: bash
      ...
      cd <ZEPHYR_ROOT>
      west build -p -b mps2_an521_nonsecure samples/tfm_integration/psa_firmware
...
and run it in qemu via the ``run`` command.
   ...
   .. code-block:: bash
      ...
      cd <ZEPHYR_ROOT>
      west build -p -b mps2_an521_nonsecure samples/tfm_integration/psa_firmware -t run
