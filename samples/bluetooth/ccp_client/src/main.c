...
static int discover_services(void)
{
	...
	err = bt_ccp_client_discover(peer_conn, &client);
	if (err != 0) {
...
static int read_bearer_uci(struct bt_ccp_client_bearer *bearer)
{
	int err;
...
int main(void)
{
	...
	LOG_INF("CCP Client initialized");

	while (true) {
		err = reset_ccp_client();
		if (err != 0) {
			break;
		}
