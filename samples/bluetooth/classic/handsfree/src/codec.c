...
#define CODEC_RX_FLAG_STOPED 0
#define CODEC_RX_FLAG_STARTED 1
#define CODEC_RX_FLAG_STOPING 2
...
static void codec_rx_task(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		...
		if (atomic_test_and_clear_bit(codec_rx_flag, CODEC_RX_FLAG_STOPING)) {
			...
			atomic_set_bit(codec_rx_flag, CODEC_RX_FLAG_STOPED);
		}
...
int codec_init(uint8_t air_mode)
{
	...
	static k_tid_t codec_rx_thread_id = NULL;
...
int codec_rx_start(codec_rx_cb_t cb)
{
	...
	if (!atomic_test_bit(codec_rx_flag, CODEC_RX_FLAG_STOPED)) {
	...
	atomic_clear_bit(codec_rx_flag, CODEC_RX_FLAG_STOPED);
...
int codec_rx_stop(void)
{
	...
	atomic_set_bit(codec_rx_flag, CODEC_RX_FLAG_STOPING);
	k_sem_give(&codec_rx_thread_notify);
...
int codec_tx(const uint8_t *data, uint32_t len)
{
	...
	/* Only start start the TX when more than one frame wrote. */
