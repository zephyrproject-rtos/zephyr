/* voice_over_voice.c - voice implementation for Bluetooth Hands-Free Profile */
...
#define VOICE_RX DT_ALIAS(pcm_rxtx)
#define VOICE_TX DT_ALIAS(pcm_rxtx)
#else
#define VOICE_RX DT_ALIAS(pcm_rx)
#define VOICE_TX DT_ALIAS(pcm_tx)
#endif
...
#define RX_BLOCK_COUNT (CONFIG_VOICE_BUFFERS)
#define TX_BLOCK_COUNT (CONFIG_VOICE_BUFFERS)
#define TIMEOUT        (2 * 1000 / CONFIG_AUDIO_TRANSFER_INTERVAL)

static const struct device *voice_rx_dev = DEVICE_DT_GET(VOICE_RX);
static const struct device *voice_tx_dev = DEVICE_DT_GET(VOICE_TX);
...
static int configure_voice_streams(const struct device *voice_rx_dev,
				   const struct device *voice_tx_dev, struct i2s_config *config)
{
	int err;

	config->mem_slab = &rx_mem_slab;
	err = i2s_configure(voice_rx_dev, I2S_DIR_RX, config);
	if (err < 0) {
		printk("Failed to configure VOICE RX stream: %d\n", err);
		return err;
	}

	config->mem_slab = &tx_mem_slab;
	err = i2s_configure(voice_tx_dev, I2S_DIR_TX, config);
	if (err < 0) {
		printk("Failed to configure VOICE TX stream: %d\n", err);
		return err;
	}
...
static struct k_sem voice_rx_thread_notify;
static voice_rx_cb_t voice_rx_cb;

#define VOICE_RX_FLAG_STOPPED  0
#define VOICE_RX_FLAG_STARTED  1
#define VOICE_RX_FLAG_STOPPING 2

static atomic_t voice_rx_flag[1];
...
static void voice_rx_task(void *p1, void *p2, void *p3)
{
	...
	while (true) {
		err = k_sem_take(&voice_rx_thread_notify, K_FOREVER);
		if (err < 0) {
			continue;
		}

		if (!atomic_test_bit(voice_rx_flag, VOICE_RX_FLAG_STARTED)) {
			continue;
		} else {
			k_sem_give(&voice_rx_thread_notify);
		}

		err = i2s_read(voice_rx_dev, &mem_block, &block_size);
		if (err < 0) {
			continue;
		}

		if (voice_rx_cb != NULL) {
			voice_rx_cb((const uint8_t *)mem_block, block_size);
		}
...
int voice_init(struct bt_conn *sco_conn, uint8_t air_mode, uint8_t codec_id)
{
	...
	if (!device_is_ready(voice_rx_dev)) {
		printk("%s is not ready\n", voice_rx_dev->name);
		return -EINVAL;
	}

	if (voice_rx_dev != voice_tx_dev && !device_is_ready(voice_tx_dev)) {
		printk("%s is not ready\n", voice_tx_dev->name);
		return -EINVAL;
	}
	...
	err = configure_voice_streams(voice_rx_dev, voice_tx_dev, &config);
	if (err < 0) {
		return err;
	}

	if (voice_rx_thread_id == NULL) {
		k_sem_init(&voice_rx_thread_notify, 0, 1);
		voice_rx_thread_id = k_thread_create(&voice_rx_thread, voice_rx_thread_stack,
						   ...
						   K_PRIO_COOP(CONFIG_VOICE_RX_THREAD_PRIO), 0,
						   K_NO_WAIT);
		k_thread_name_set(voice_rx_thread_id, "HFP VOICE RX");
	}

	atomic_set_bit(voice_rx_flag, VOICE_RX_FLAG_STOPPED);
	is_initiated = true;
...
int voice_rx_start(voice_rx_cb_t cb)
{
	...
	if (!atomic_test_bit(voice_rx_flag, VOICE_RX_FLAG_STOPPED)) {
		return 0;
	}

	voice_rx_cb = cb;

	err = i2s_trigger(voice_rx_dev, I2S_DIR_RX, I2S_TRIGGER_START);
	if (err < 0) {
		printk("Failed to trigger start on RX: %d\n", err);
		return err;
	}

	atomic_clear_bit(voice_rx_flag, VOICE_RX_FLAG_STOPPED);
	atomic_set_bit(voice_rx_flag, VOICE_RX_FLAG_STARTED);
	k_sem_give(&voice_rx_thread_notify);
...
int voice_tx(const uint8_t *data, uint32_t len)
{
	...
	err = i2s_write(voice_tx_dev, mem_block, BLOCK_SIZE);
	if (err < 0) {
