...
#define PCM_RX_FLAG_STOPED 0
#define PCM_RX_FLAG_STARTED 1
#define PCM_RX_FLAG_STOPING 2
...
int pcm_init(uint8_t air_mode)
{
	...
	static k_tid_t pcm_rx_thread_id = NULL;
	...
	atomic_set_bit(pcm_rx_flag, PCM_RX_FLAG_STOPED);
	is_initiated = true;
...
int pcm_rx_start(pcm_rx_cb_t cb)
{
	...
	if (!atomic_test_bit(pcm_rx_flag, PCM_RX_FLAG_STOPED)) {
		return 0;
	}
	...
	atomic_clear_bit(pcm_rx_flag, PCM_RX_FLAG_STOPED);
...
int pcm_tx(const uint8_t *data, uint32_t len)
{
	...
	if (!tx_started && tx_count > 1) {
		err = i2s_trigger(pcm_tx_dev, I2S_DIR_TX, I2S_TRIGGER_START);
		if (err < 0) {
