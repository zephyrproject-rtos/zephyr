...
/* k_mem_slab_init() requires a word-aligned buffer as well as a word-aligned block size */
static __NOCACHE __aligned(sizeof(void *)) uint8_t mem_slab_buffer[
	CONFIG_A2DP_BOARD_CODEC_PLAY_COUNT * A2DP_SBC_DATA_PLAY_SIZE_48K];
...
void codec_play_configure(uint32_t sample_rate, uint8_t sample_width, uint8_t channels)
{
	 ...
	 * tolerates.
	 */
	err = i2s_configure(codec_tx, I2S_DIR_TX, &config);
	if (err != 0) {
		printk("failed to configure i2s tx: %d\n", err);
		return;
	}

	err = audio_codec_configure(codec_dev, &audio_cfg);
	if (err != 0) {
	...
	err = k_mem_slab_init(&mem_slab, mem_slab_buffer, block_size,
