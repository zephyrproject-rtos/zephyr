...
- A board with Bluetooth BR/EDR support and at least four buttons exposed
  ...
  :zephyr:board:`native_sim <native_sim>` the bundled overlay defines these buttons
...
On :zephyr:board:`native_sim <native_sim>`, where there are no physical buttons, the
bundled overlay wires the four buttons to the emulated GPIO controller. To drive
them from an SDL window, add a ``zephyr,gpio-emul-sdl`` child to that controller
(see :zephyr_file:`samples/subsys/display/lvgl/boards/native_sim.overlay` for the
pattern) and enable ``CONFIG_GPIO_EMUL_SDL``. Combined with a USB HCI dongle as
the controller and a second dongle running a Bluetooth HID Host, this drives the
full send path end to end.
