...
/* Byte indices and length of the Report Protocol mouse input report. The
 * layout matches HID_MOUSE_REPORT_DESC() below: buttons + X + Y + wheel. That
 * descriptor declares no Report ID, so Report Protocol reports carry no ID
 * byte. Boot Protocol is different, see HID_BOOT_MOUSE_* below.
 */
...
/* Boot Protocol Mode mouse report. Unlike Report Protocol, the HID spec
 * (v1.1.2, 3.3.2 and Table 3.15) mandates a leading Report ID for boot
 * reports: a mouse boot report is 4 octets, Report ID 2 followed by the
 * standard 3-octet boot report (buttons + X + Y). Every report sent in Boot
 * Protocol Mode, over GET_REPORT or asynchronously on the Interrupt channel,
 * must include this Report ID.
 */
#define HID_BOOT_MOUSE_REPORT_ID 0x02U
#define HID_BOOT_MOUSE_REPORT_LEN 4U
...
static void send_mouse_report(const uint8_t report[MOUSE_REPORT_COUNT])
{
	...
	/* Report Protocol sends buttons + X + Y + wheel with no Report ID. Boot
	 * Protocol prepends Report ID 2 and drops the wheel byte, so it carries
	 * the ID plus buttons + X + Y (HID spec v1.1.2, 3.3.2).
	 */
	if (hid_boot_mode) {
		net_buf_add_u8(buf, HID_BOOT_MOUSE_REPORT_ID);
		net_buf_add_mem(buf, report, 3);
	} else {
		net_buf_add_mem(buf, report, MOUSE_REPORT_COUNT);
	}
...
{
	/* Idle mouse report. Report Protocol carries buttons + X + Y + wheel
	 * with no Report ID; Boot Protocol carries Report ID 2 followed by
	 * buttons + X + Y (HID spec v1.1.2, 3.3.2 and Table 3.15).
	 */
	static const uint8_t report[] = {0x00, 0x00, 0x00, 0x00};
	static const uint8_t boot_report[] = {HID_BOOT_MOUSE_REPORT_ID, 0x00, 0x00, 0x00};
