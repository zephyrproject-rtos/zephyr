...
static bool is_nap_device(const struct bt_br_discovery_result *result)
{
	...
	/*
	 * Dedicated NAPs advertise LAN/Network Access Point. Phones that share
	 * their network over Bluetooth often keep Phone as the major class and
	 * may only set the Networking service class bit, or leave CoD unchanged.
	 */
...
static void pan_connected(struct bt_pan *pan)
{
	...
	/* Restart so discovery runs after the PAN carrier is up. */
...
static void discovery_timeout_cb(const struct bt_br_discovery_result *results, size_t count)
{
	for (size_t i = 0; i < count; i++) {
		printk("Device[%zu]: %s, rssi %d, cod 0x%02x%02x%02x\n", i,
		       bt_addr_str(&results[i].addr), results[i].rssi, results[i].cod[2],
		       results[i].cod[1], results[i].cod[0]);
