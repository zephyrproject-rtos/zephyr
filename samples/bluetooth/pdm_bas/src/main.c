...
#elif defined(CONFIG_AUDIO_DMIC)
...
static int fill_dmic_buf(int16_t *buf)
{
	...
	if (ret < 0) {
		printk("%s: Failed to configure the driver: %d\n", __func__, ret);
	...
	if (ret < 0) {
		printk("%s: START trigger failed: %d\n", __func__, ret);
	...
	printk("%s: got buffer %p of %u bytes\n", __func__, buffer, size);
	...
	if (ret < 0) {
		printk("%s: STOP trigger failed: %d\n", __func__, ret);
		return ret;
	}
...
#if !defined(CONFIG_USB_DEVICE_AUDIO) && !defined(CONFIG_AUDIO_DMIC)
...
#if defined(CONFIG_LIBLC3) && (defined(CONFIG_USB_DEVICE_AUDIO) || defined(CONFIG_AUDIO_DMIC))
