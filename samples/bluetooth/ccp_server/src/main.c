...
static const struct bt_data ad[] = {
	...
	IF_ENABLED(CONFIG_BT_CCP_SERVER_BEARER_COUNT > 1,
...
static int reset_ccp_server(void)
{
	int err;
...
static int init_ccp_server(void)
{
...
int main(void)
{
	...
	LOG_INF("CCP Server initialized");

	while (true) {
		err = reset_ccp_server();
		if (err != 0) {
		...
		/* Start advertising as a CCP Server, which includes setting the required
		 * advertising data based on the roles we support.
		 */
		err = advertise();
		if (err != 0) {
			continue;
		...
		/* After advertising we expect CCP Clients to connect to us and eventually
		 * disconnect again. As a CCP Server we just react to their requests and not do
		 * anything else.
		 */
