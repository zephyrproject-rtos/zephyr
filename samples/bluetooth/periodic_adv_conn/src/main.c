...
static bool response_data_cb(struct bt_data *data, void *user_data)
{
	...
	if (data->type == BT_DATA_NAME_COMPLETE &&
	    ...
	    memcmp(data->data, CONFIG_SAMPLE_PERIODIC_ADV_CONN_PEER_NAME,
		    sizeof(CONFIG_SAMPLE_PERIODIC_ADV_CONN_PEER_NAME) - 1) == 0) {
		*name_matches = true;
		return false;
	}
			...
			struct net_buf_simple *buf)
{
	...
	bt_data_parse(buf, response_data_cb, &name_matches);
	if (!name_matches) {
		return;
	}
...
static bool data_cb(struct bt_data *data, void *user_data)
{
	...
	case BT_DATA_NAME_COMPLETE:
		if (data->data_len == sizeof(CONFIG_SAMPLE_PERIODIC_ADV_CONN_PEER_NAME) - 1 &&
		    memcmp(data->data, CONFIG_SAMPLE_PERIODIC_ADV_CONN_PEER_NAME,
			    sizeof(CONFIG_SAMPLE_PERIODIC_ADV_CONN_PEER_NAME) - 1) == 0) {
			*name_matches = true;
			return false;
