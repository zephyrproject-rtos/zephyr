...
#define CS_FREQUENCY_HZ(ch)     (CS_FREQUENCY_MHZ(ch) * 1000000.0)
#define SPEED_OF_LIGHT_M_PER_S  (299792458.0)
#define SPEED_OF_LIGHT_NM_PER_S (SPEED_OF_LIGHT_M_PER_S / 1000000000.0)
...
static double linear_regression(double *x_values, double *y_values, uint8_t n_samples)
{
	if (n_samples == 0) {
		return 0.0;
	}
	...
	double b_est_upper = 0.0;
	double b_est_lower = 0.0;
...
static void bubblesort_2(double *array1, double *array2, uint16_t len)
{
	bool swapped;
	double temp;
...
static double estimate_distance_using_phase_slope(struct iq_sample_and_channel *data, uint8_t len)
{
	int32_t combined_i;
	int32_t combined_q;
	uint16_t num_angles = 0;
	static double theta[MAX_NUM_SAMPLES];
	static double frequencies[MAX_NUM_SAMPLES];
	...
	for (uint8_t i = 1; i < num_angles; i++) {
		double difference = theta[i] - theta[i - 1];
	...
	double phase_slope = linear_regression(frequencies, theta, num_angles);

	double distance = -phase_slope * (SPEED_OF_LIGHT_M_PER_S / (4 * PI));

	return distance / 1000000.0; /* Scale to meters. */
}

static double estimate_distance_using_time_of_flight(uint8_t n_samples)
{
	double tof;
	double tof_mean = 0.0;
	...
	double tof_mean_ns = tof_mean / 2.0;
