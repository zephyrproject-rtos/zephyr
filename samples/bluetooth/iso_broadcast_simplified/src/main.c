...
NET_BUF_POOL_FIXED_DEFINE(bis_tx_pool, CONFIG_BT_ISO_MAX_CHAN,
			  BT_ISO_SDU_BUF_SIZE(CONFIG_BT_ISO_TX_MTU), 8, NULL);

static K_SEM_DEFINE(sem_big_cmplt, 0, CONFIG_BT_ISO_MAX_CHAN);
static K_SEM_DEFINE(sem_big_term, 0, CONFIG_BT_ISO_MAX_CHAN);
...
{
	printk("ISO disconnected with reason 0x%02x\n", reason);
...
static struct bt_iso_chan *bis[CONFIG_BT_ISO_MAX_CHAN] = { &bis_iso_chan };
...
int main(void)
{
	...
	while (true) {
		...
		buf = net_buf_alloc(&bis_tx_pool, K_MSEC(10));
		if (!buf) {
