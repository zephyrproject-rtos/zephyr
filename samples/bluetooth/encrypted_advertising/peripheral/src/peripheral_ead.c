...
static int update_ad_data(struct bt_le_ext_adv *adv)
{
	...
	if (err != 0) {
		LOG_ERROR("Error during first encryption");
		return -1;
	}
	...
	if (err != 0) {
		LOG_ERROR("Error during second encryption");
		return -1;
	}
...
static int stop_and_delete_adv(struct bt_le_ext_adv *adv)
{
	...
	if (err) {
		LOG_ERROR("Failed to stop advertiser (err %d)", err);
		return -1;
	}
...
{
	if (err) {
		LOG_ERROR("Failed to connect to %s %u %s", bt_conn_dst_str(conn), err,
...
static int init_bt(void)
{
	...
	if (err) {
		LOG_ERROR("Failed to register bt_conn_auth_cb (err %d)", err);
		return -1;
	}
...
int run_peripheral_sample(int get_passkey_confirmation(struct bt_conn *conn))
{
	...
	if (err) {
		LOG_ERROR("Failure during security update");
		return -5;
	}
