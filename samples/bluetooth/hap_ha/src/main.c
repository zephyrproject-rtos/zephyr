...
{
	printk("CONFIG_TARGET _CHANNEL FALSE\n");
	uint32_t result = 0U;
...
static bool adv_rpa_expired_cb(struct bt_le_ext_adv *adv)
{
	...
	if (err != 0) {
		printk("%s: Failed to generate RSI (err %d)\n", __func__, err);
		return false;
	}
	...
	if (err) {
		printk("%s: Failed to set advertising data (err %d)\n", __func__, err);
		return false;
	}
...
static int hap_ha_init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_ASCS_ASE_SRC)) {
		...
		printk("HAP_HA SETUP: Microphone found\n");
	}
...
int main(void)
{
	...
	//HAP_HA init
		err = has_server_init();
		if (err != 0) {
			printk("HAS Server initialization failed (err %d)\n", err);
			return 0;
		}
	
		err = bap_unicast_sr_init();
		if (err != 0) {
			printk("BAP Unicast Server initialization failed (err %d)\n", err);
			return 0;
		}

		err = hap_ha_init();
		if (err != 0) {
			printk("HAP_ha init failed (err %d)\n", err);
			return 0;
		}
	...
	//BAP_HA init
		err = init_bap_sink();
		if (err != 0) {
			printk("BAP Broadcast Sink initialization failed (err %d)\n", err);
			return 0;
		}
