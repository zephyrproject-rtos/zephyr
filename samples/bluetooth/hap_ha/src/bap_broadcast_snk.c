...
static uint16_t req_pa_interval;
...
static void stream_started_cb(struct bt_bap_stream *bap_stream)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to start stream RX: %d\n", err);
	}
...
static void stream_stopped_cb(struct bt_bap_stream *bap_stream, uint8_t reason)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to stop stream RX: %d\n", err);
	}
					  ...
					  void *user_data)
{
	...
	LOG_DBG("%s: for each BIS in a subgroup\n", __func__);
...
static int pa_sync_without_past(const bt_addr_le_t *addr, uint8_t adv_sid, uint16_t pa_interval)
{
	...
	err = bt_le_per_adv_sync_create(&param, &pa_sync);
	if (err != 0) {
		LOG_WRN("Could not create PA sync: %d\n", err);
	} else {
			  ...
			  bool past_avail, uint16_t pa_interval)
{
	...
	LOG_INF("Received request to sync to PA (PAST %s available): %u\n", past_avail ? "" : "not ",
		recv_state->pa_sync_state);

	req_recv_state = recv_state;
	req_pa_interval = pa_interval;
...
#if !IS_ENABLED(CONFIG_USE_SPECIFIC_BROADCAST_CHANNEL)
...
#else  /* !CONFIG_USE_SPECIFIC_BROADCAST_CHANNEL */
...
#endif /* CONFIG_USE_SPECIFIC_BROADCAST_CHANNEL */
...
static int bap_broadcast_snk_reset(void)
{
	...
	req_pa_interval = BT_BAP_PA_INTERVAL_UNKNOWN;
...
static enum broadcast_snk_state broadcast_snk_state_wait_ba(void)
{
	...
	if (err != 0) {
		/* PAST not available, create PA sync via scanning */
		if (req_recv_state == NULL) {
			LOG_WRN("No recv state available for PA sync\n");

			return BROADCAST_SNK_STATE_RESET;
		}

		err = pa_sync_without_past(&req_recv_state->addr,
					  req_recv_state->adv_sid,
					  req_pa_interval);
		if (err != 0) {
			LOG_WRN("Failed to create PA sync without PAST: %d\n", err);
