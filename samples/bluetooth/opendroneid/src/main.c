...
ODID_MessagePack_data message_pack;
...
static void update_payload(uint8_t turn)
{
	...
	case 0: /* BasicID */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[0], &encoded, sizeof(ODID_BasicID_encoded));
	...
	case 1: /* Location */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[1], &encoded, sizeof(ODID_Location_encoded));
	...
	case 2: /* Auth */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[2], &encoded, sizeof(ODID_Auth_encoded));
	...
	case 3: /* SelfID */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[3], &encoded, sizeof(ODID_SelfID_encoded));
	...
	case 4: /* System */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[4], &encoded, sizeof(ODID_System_encoded));
	...
	case 5: /* OperatorID */
		...
		if (err == ODID_SUCCESS) {
			...
			memcpy(&message_pack.Messages[5], &encoded, sizeof(ODID_OperatorID_encoded));
...
int main(void)
{
	...
	odid_initMessagePackData(&message_pack);
	message_pack.MsgPackSize = 6;
