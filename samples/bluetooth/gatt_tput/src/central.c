...
void app_central_set_throttle(uint16_t kb_s)
{
	...
	sys_put_le16(kb_s, val);
	err = bt_gatt_write_without_response(conn, h.throttle_value, val, sizeof(val), false);
	if (err) {
		LOG_ERR("Throttle write failed (%d)", err);
	} else {
		LOG_INF("Throttle set to %u KB/s", kb_s);
	}
