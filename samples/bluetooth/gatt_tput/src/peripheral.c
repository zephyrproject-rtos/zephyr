...
/* Throttled pacing as fixed-point notifications/event (x1000) plus a carry so a
 * fractional rate enqueues zero on some events instead of rounding up to one.
 */
static uint32_t notif_rate_milli;
static uint32_t notif_accum_milli;
...
static void calc_pacing(void)
{
	...
	if (target_kb_s > 0U) {
		/* target is KB/s; x8 -> kbps. Fixed-point notifications/event (x1000) so a
		 * fractional rate can enqueue zero on some events instead of being rounded
		 * up to one, which would overshoot low targets.
		 */
		uint32_t kbps = (uint32_t)target_kb_s * 8U;
		uint32_t payload = (packet_size > 0U) ? packet_size : 1U;
		uint64_t num = (uint64_t)kbps * conn_interval * 5U;
		uint32_t den = payload * 32U;

		notif_rate_milli = (uint32_t)((num * 1000U + den / 2U) / den);
	} else {
		notif_rate_milli = (uint32_t)max_packets_per_event * 1000U;
	}

	uint32_t max_milli = (uint32_t)max_packets_per_event * 1000U;

	if (notif_rate_milli > max_milli) {
		notif_rate_milli = max_milli;
	}
	notifications_per_event = (uint8_t)((notif_rate_milli + 500U) / 1000U);
...
static void notif_timer_expiry(struct k_timer *timer)
{
	...
	if (notif_rate_milli == 0U) {
		/* Pacing not computed yet: fall back to a full credit window. */
		add = APP_TX_CREDITS;
	} else {
		notif_accum_milli += notif_rate_milli;
		add = (uint8_t)MIN(notif_accum_milli / 1000U, (uint32_t)PENDING_CAP);
		notif_accum_milli -= (uint32_t)add * 1000U;
	}

	if (add == 0U) {
		/* Fractional target: this event enqueues nothing. */
		return;
	}
...
{
	uint16_t val = sys_cpu_to_le16(target_kb_s);
...
static void stream_thread(void)
{
	while (1) {
		...
		/* Resolve the conn ref and usable ATT MTU once, then drain the whole
		 * available TX-credit window in a tight burst. Keeping the conn mutex /
		 * ref / get_mtu overhead out of the per-packet path lets the connection
		 * event fill up before it closes.
		 */
		...
		while (notify_enabled && atomic_get(&pending) > 0 &&
		       k_sem_take(&tx_credits, K_NO_WAIT) == 0) {
			...
			if (err) {
				/* -ENOMEM = congested: give the credit back and end the
				 * burst; a freed buffer or the next tick re-wakes us.
				 */
				k_sem_give(&tx_credits);
				if (err != -ENOMEM) {
		...
		/* Burst done (credits exhausted, queue drained, or congested): wait for
		 * a credit to free (notify_sent) or the next timer tick.
		 */
...
/* Restart advertising once the disconnected connection object is fully recycled;
 * restarting from the disconnected callback can return -ENOMEM because the
 * connection has not been freed yet.
 */
