...
static K_SEM_DEFINE(sem_bis_synced, 0U, CONFIG_BT_AUDIO_BROADCAST_SNK_STREAM_COUNT);
...
static uint8_t sink_broadcast_code[BT_BAP_BROADCAST_CODE_SIZE];
			      ...
			      const uint8_t broadcast_code[BT_BAP_BROADCAST_CODE_SIZE])
{
	...
	(void)memcpy(sink_broadcast_code, broadcast_code,
			   ...
			   const uint32_t bis_sync_req[BT_BAP_SCAN_DELEGATOR_MAX_SUBGROUPS])
{
	...
	if (bis_synced && requested_bis_sync != bis_sync_req[0]) {
		 ...
		 * synced, it means that the requested BIS sync has changed.a
		 */
		int err;
		 ...
		 * is back to  0.
		 */
		err = bt_audio_broadcast_sink_stop(broadcast_sink);
		if (err != 0) {
