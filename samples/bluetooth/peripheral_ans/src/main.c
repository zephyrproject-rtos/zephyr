...
int main(void)
{
	...
	ret = bt_ans_set_new_alert_support_category(BT_ANS_CAT_HIGH_PRI_ALERT, true);
	if (ret != 0) {
		LOG_ERR("Unable to set new alert support category! (err: %d)", ret);
	}

	ret = bt_ans_set_unread_support_category(BT_ANS_CAT_SIMPLE_ALERT, true);
	if (ret != 0) {
		LOG_ERR("Unable to set unread support category! (err: %d)", ret);
	}
	...
	while (true) {
		...
		ret = bt_ans_notify_new_alert(BT_ANS_CAT_SIMPLE_ALERT, num_new, test_msg,
					      strlen(test_msg));
		if (ret != 0) {
		...
		ret = bt_ans_notify_new_alert(BT_ANS_CAT_HIGH_PRI_ALERT, num_new, high_pri_msg,
					      strlen(high_pri_msg));
		if (ret != 0) {
			LOG_ERR("Failed to push new alert! (err: %d)", ret);
		}
