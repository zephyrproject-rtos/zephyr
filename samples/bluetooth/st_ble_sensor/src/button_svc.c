...
int button_init(gpio_callback_handler_t handler)
{
	...
	if (!gpio_is_ready_dt(&button)) {
		LOG_ERROR("Error: button GPIO device %s is not ready", button.port->name);
		return -ENODEV;
	}

	ret = gpio_pin_configure_dt(&button, GPIO_INPUT);
	if (ret != 0) {
		LOG_ERROR("Error %d: can't configure button on GPIO %s pin %d", ret,
	...
	if (ret != 0) {
		LOG_ERROR("Error %d: can't configure button interrupt on "
			  "GPIO %s pin %d",
