...
/**
 * The base_recv_cb() function will populate struct base_bis_data with the BIS index and
...
/**
 * The base_recv_cb() function will populate struct base_subgroup_data with BIS data
 * for each subgroup.
 *
 * The subgroup_cnt is the number of subgroups in the BASE.
 */
struct base_subgroup_data {
	struct base_bis_data bis[CONFIG_BT_BAP_BASS_MAX_SUBGROUPS];
	uint8_t subgroup_cnt;
};

static struct base_subgroup_data base_subgroup; /* holds data from base_recv_cb */
static uint32_t
...
static bool bis_get_channel_allocation_cb(const struct bt_bap_base_subgroup_bis *bis,
					  void *user_data)
{
	struct base_bis_data *base_bis = user_data;
	...
	err = bt_audio_codec_cfg_get_chan_allocation(
		&codec_cfg, &base_bis->audio_allocation[bis->index].value, true);
	if (err != 0) {
	...
	base_bis->audio_allocation[bis->index].valid = true;

	return true; /* continue to next BIS */
}

/**
 * Called for each subgroup in the BASE. Will populate the struct base_bis_data variable with
					      ...
					      void *user_data)
{
	...
	struct base_subgroup_data *data = user_data;
	struct base_bis_data *base_bis = &data->bis[data->subgroup_cnt];
	struct bt_audio_codec_cfg codec_cfg;
	int err;
	...
	err = bt_bap_base_subgroup_get_bis_indexes(subgroup, &base_bis->bis_index_bitfield);
	if (err != 0) {
	...
	err = bt_bap_base_subgroup_foreach_bis(subgroup, bis_get_channel_allocation_cb, base_bis);
	if (err != 0) {
	...
	for (uint8_t idx = 1U; idx <= BT_ISO_BIS_INDEX_MAX; idx++) {
		if (base_bis->bis_index_bitfield & BT_ISO_BIS_INDEX_BIT(idx)) {
			if (!base_bis->audio_allocation[idx].valid) {
				base_bis->audio_allocation[idx].value =
					subgroup_chan_allocation_available
						? subgroup_chan_allocation
						: BT_AUDIO_LOCATION_MONO_AUDIO;
				base_bis->audio_allocation[idx].valid = true;
			}
			 ...
			 size_t base_size)
{
	...
	(void)memset(&base_subgroup, 0, sizeof(base_subgroup));

	/* Get BIS index data for each subgroup */
	err = bt_bap_base_foreach_subgroup(base, subgroup_get_valid_bis_indexes_cb, &base_subgroup);
	if (err != 0) {
			   ...
			   const uint32_t bis_sync_req[CONFIG_BT_BAP_BASS_MAX_SUBGROUPS])
{
	...
	if (!bis_sync_req_no_pref) {
		...
		if (bis_sync_req_cnt > 1U) {
			printk("Only request sync to 1 subgroup unless no preference!\n");

			return -EINVAL;
		}

		if (stream_count > CONFIG_BT_BAP_BROADCAST_SNK_STREAM_COUNT) {
			printk("Too many BIS requested for sync!\n");
...
static uint32_t select_bis_sync_bitfield(struct base_subgroup_data *base_sg_data,
...

{
	...
	for (int i = 0; i < CONFIG_BT_BAP_BASS_MAX_SUBGROUPS; i++) {
		...
		for (int idx = 1; idx <= BT_ISO_BIS_INDEX_MAX; idx++) {
			const struct bis_audio_allocation *bis_alloc =
				&base_sg_data->bis[i].audio_allocation[idx];

			if (!base_sg_data->bis[i].audio_allocation[idx].valid) {
	...
	for (uint8_t i = 0; i < CONFIG_BT_BAP_BASS_MAX_SUBGROUPS; i++) {
		if (bis_sync_req[i] != 0) {
			...
			result |= bis_sync_req[i] & base_sg_data->bis[i].bis_index_bitfield;
		}
	}
...
wait_for_pa_sync:
		...
		sync_bitfield = select_bis_sync_bitfield(&base_subgroup, requested_bis_sync);
