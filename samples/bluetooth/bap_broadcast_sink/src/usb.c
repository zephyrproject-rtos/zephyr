...
K_MEM_SLAB_DEFINE_STATIC(usb_out_buf_pool, USB_STEREO_FRAME_SIZE, USB_ENQUEUE_COUNT, 4);
...
static void uac2_sof_cb(const struct device *dev, void *user_data)
{
	...
	if (err != 0) {
		...
		/* usbd_uac2_send is inconsistent about when it takes ownership
		 * The following cases may happen where it does not take ownership
		 */
...
static void uac2_buf_release_cb(const struct device *dev, uint8_t terminal, void *buf,
				void *user_data)
{
	/* buf may be NULL sometimes for some reason
	 * https://github.com/zephyrproject-rtos/zephyr/issues/84664
	 */
	if (buf != NULL) {
