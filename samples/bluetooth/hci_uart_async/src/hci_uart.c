...
static void send_hw_error(void)
{
	...
	/* HW error is on c2h queue. Delivery is async. */
}
...
void callback(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	if (evt->type == UART_RX_DISABLED) {
		k_poll_signal_raise(&uart_h2c_rx_sig, 0);
	}
	if (evt->type == UART_RX_STOPPED) {
		k_poll_signal_raise(&uart_h2c_rx_sig, evt->data.rx_stop.reason);
	}
	if (evt->type == UART_TX_DONE) {
		k_poll_signal_raise(&uart_c2h_tx_sig, 0);
	}
}
...
static int hci_uart_init(void)
{
	...
	/* Is CONFIG_UART_ASYNC_API not implemented/enabled for the
	 * selected `hci_uart_dev`?
	 */
...
const struct {
	...
	.cc = {.ncmd = 1, /* TODO: Should this value come from a config? */
