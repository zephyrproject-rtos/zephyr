 ...
 * to a transport-agnostic sink (pcm_sink.h, backed by pcm_sink_i2s.c or
 * pcm_sink_none.c). Keeping the decode and output stages behind those two
...
static K_WORK_DEFINE(bringup_work, bringup_work_handler);
static K_WORK_DEFINE(teardown_work, teardown_work_handler);
...
static void pending_lock_take(void)
{
	__maybe_unused int err = k_mutex_lock(&pending_lock, K_FOREVER);

	__ASSERT(err == 0, "k_mutex_lock failed with %d", err);
}

static void pending_lock_give(void)
{
	__maybe_unused int err = k_mutex_unlock(&pending_lock);
...
{
	struct sdu_item *item = k_fifo_get(&sdu_fifo, K_NO_WAIT);
...
static void playback_worker(void *a, void *b, void *c)
{
	 ...
	 * avoids overflowing the worker stack into adjacent .bss.
	 */
...
{
	static K_THREAD_STACK_DEFINE(worker_stack,
	...
	worker_tid = k_thread_create(
		&worker_thread, worker_stack, K_THREAD_STACK_SIZEOF(worker_stack),
		playback_worker, NULL, NULL, NULL,
		K_PRIO_PREEMPT(CONFIG_SAMPLE_BT_AUDIO_PLAYBACK_WORKER_PRIORITY), 0, K_NO_WAIT);

	ret = k_thread_name_set(worker_tid, "bt_audio_playback");
	if (ret != 0 && ret != -ENOSYS) {
		/* Cosmetic only; -ENOSYS just means CONFIG_THREAD_NAME is disabled. */
		LOG_WRN("k_thread_name_set failed: %d", ret);
	}
...
int sample_bt_audio_playback_start(const struct sample_bt_audio_pcm_cfg *cfg)
{
	...
	if (cfg->chan_cnt == 0U || cfg->chan_cnt > SAMPLE_BT_AUDIO_PCM_SINK_MAX_CHAN) {
		LOG_DBG("Unsupported chan_cnt %u", cfg->chan_cnt);
		return -EINVAL;
	}

	if (cfg->frame_blocks_per_sdu == 0U) {
		LOG_DBG("Unsupported frame_blocks_per_sdu %u", cfg->frame_blocks_per_sdu);
		return -EINVAL;
	}
	...
	if (ret < 0) {
		LOG_DBG("k_work_submit failed: %d", ret);
		return ret;
	}
...
/* Two configs are equal when every negotiated LC3 parameter matches, so a stop()+start()
 * with an unchanged configuration need not restart the stream.
 */
static bool pcm_cfg_eq(const struct sample_bt_audio_pcm_cfg *a,
		       const struct sample_bt_audio_pcm_cfg *b)
{
	return a->freq_hz == b->freq_hz && a->frame_duration_us == b->frame_duration_us &&
	       a->octets_per_frame == b->octets_per_frame && a->chan_cnt == b->chan_cnt &&
	       a->frame_blocks_per_sdu == b->frame_blocks_per_sdu;
}
...
static void bringup_work_handler(struct k_work *work)
{
	...
	/* A session may still be up: a stop()+start() reconfigure leaves it running because
	 * the interleaved teardown saw active_intent restored. If the new config matches, the
	 * stream is already in the desired state; otherwise tear the stale session down here
	 * so the new config takes effect.
	 */
	if (atomic_get(&running) != 0) {
		if (pcm_cfg_eq(&local_cfg, &active_cfg)) {
			return;
		}
		teardown_session();
	}
	...
	err = sample_bt_audio_lc3_decode_setup(cfg->freq_hz, cfg->frame_duration_us,
					       cfg->chan_cnt);
	if (err != 0) {
		LOG_ERR("LC3 decoder setup failed: %d", err);
		return;
	}
	...
	err = sample_bt_audio_pcm_sink_configure(&sink_cfg);
	if (err != 0) {
		LOG_ERR("PCM sink configure failed: %d", err);
		return;
	}

	err = sample_bt_audio_pcm_sink_start();
	if (err != 0) {
		LOG_ERR("PCM sink start failed: %d", err);
		return;
	}
	...
	/* Clear any park signal left by a prior teardown or fault before resuming: the worker
	 * is parked on resume_sem here and cannot publish worker_parked_sem concurrently, so
	 * this is race-free. It keeps worker_parked_sem empty while running == 1, so a later
	 * teardown's take matches exactly that stop's park.
	 */
...
/* Park the worker, release any queued SDUs and stop the sink. Runs on the system
 * workqueue - from teardown_work_handler, or from bringup_work_handler when a new
 * configuration must replace a still-running session - so it never overlaps a bring-up.
 */
static void teardown_session(void)
{
	bool was_running;
	 ...
	 */
	pending_lock_take();
	was_running = atomic_set(&running, 0) != 0;
	pending_lock_give();

	/* Wait for the worker to park: it gives worker_parked_sem once it observes
	 * running == 0 and has stopped touching the sink. worker_parked_sem is empty while
	 * running == 1 (bringup clears it before resuming), so this take matches this stop's
	 * park without a reset that could erase a concurrent fault-path signal.
	 */
	...
	/* worker_handle_fault() already stops the sink before it parks, so only stop it here
	 * when we drove the running -> 0 transition ourselves; otherwise the fault path owns
	 * the shutdown.
	 */
	if (was_running) {
		sample_bt_audio_pcm_sink_stop();
...
void sample_bt_audio_playback_recv(const struct bt_iso_recv_info *info, struct net_buf *buf)
{
	 ...
	 * running is published only after the decoder is set up, so the locked check below
	 * also guarantees the decoder is ready.
	 */
	pending_lock_take();
