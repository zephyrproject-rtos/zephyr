...
/* The I2S drivers do no D-cache maintenance on TX blocks, so on cores with a data cache
 * the DMA could read stale PCM from a cacheable buffer. __nocache places the slab in the
 * non-cacheable region (CONFIG_NOCACHE_MEMORY) and is a no-op otherwise.
 */
 ...
 */
static K_MUTEX_DEFINE(codec_lock);

/* k_mutex clears the owner on the final unlock, so the owner check alone is exact. */
#define CODEC_LOCK_ASSERT()                                                                        \
	__ASSERT(codec_lock.owner == k_current_get(), "codec_lock not held by %p",               \
		 k_current_get())

static void codec_lock_take(void)
{
	__maybe_unused int err = k_mutex_lock(&codec_lock, K_FOREVER);

	__ASSERT(err == 0, "k_mutex_lock failed with %d", err);
}

static void codec_lock_give(void)
{
	__maybe_unused int err = k_mutex_unlock(&codec_lock);

	__ASSERT(err == 0, "k_mutex_unlock failed with %d", err);
}
 ...
 * already queued and only returns to READY once the TX queue empties. The driver frees
 * each TX block back to the slab once sent, so the slab being fully free means the DMA
 * no longer owns any block and a subsequent bring-up may reinitialise it.
 */
static void wait_for_tx_drain(void)
{
	const uint32_t frame_us = sink_cfg.frame_duration_us;
	...
	/* Worst case: every block queued, plus one frame of margin. */
	deadline = sys_timepoint_calc(K_USEC((uint64_t)(I2S_MEM_SLAB_BLOCKS + 1U) * frame_us));

	while (k_mem_slab_num_free_get(&i2s_mem_slab) < I2S_MEM_SLAB_BLOCKS) {
		if (sys_timepoint_expired(deadline)) {
			LOG_WRN("I2S TX drain timed out");
			return;
		}

		k_sleep(K_USEC(frame_us));
	}
}
...
static int configure_i2s_and_codec(void)
{
	...
	ret = k_mem_slab_init(&i2s_mem_slab, i2s_slab_buffer, sink_cfg.block_bytes,
			      I2S_MEM_SLAB_BLOCKS);
	if (ret < 0) {
		LOG_DBG("k_mem_slab_init failed: %d", ret);
		return ret;
	}
	...
	if (ret != 0) {
		LOG_DBG("i2s_configure failed: %d", ret);
		return ret;
	}

	ret = audio_codec_configure(codec_dev, &codec_cfg);
	if (ret != 0) {
		LOG_DBG("audio_codec_configure failed: %d", ret);
		return ret;
	}
...
int sample_bt_audio_pcm_sink_init(void)
{
	if (!device_is_ready(codec_tx)) {
		LOG_DBG("%s not ready", codec_tx->name);
		return -EIO;
	}
	if (!device_is_ready(codec_dev)) {
		LOG_DBG("%s not ready", codec_dev->name);
		return -EIO;
	}
...
int sample_bt_audio_pcm_sink_configure(const struct sample_bt_audio_pcm_sink_cfg *cfg)
{
	...
	codec_lock_give();
	if (ret != 0) {
		(void)atomic_set(&configured, 0);
		return ret;
	}

	(void)atomic_set(&configured, 1);
...
int sample_bt_audio_pcm_sink_start(void)
{
	...
	if (ret == -EIO) {
		...
		LOG_DBG("i2s_trigger PREPARE failed: %d", ret);
	...
	for (uint32_t i = 0; i < I2S_PREFILL_BLOCKS; i++) {
		...
		if (wret < 0) {
			LOG_DBG("silence pre-fill %u failed: %d", i, wret);
			audio_codec_stop_output(codec_dev);
			codec_lock_give();
			return wret;
		}
	...
	if (ret != 0) {
		LOG_DBG("i2s_trigger START failed: %d", ret);
		audio_codec_stop_output(codec_dev);
		codec_lock_give();
		return ret;
	}
...
void sample_bt_audio_pcm_sink_stop(void)
{
	...
	/* Stop the codec only after the queued blocks have drained: codecs whose
	 * stop_output() gates the DAC would otherwise mute the tail still being clocked out.
	 */
	codec_lock_take();
	audio_codec_stop_output(codec_dev);
	codec_lock_give();
