 ...
 * are the set the sink stage accepts rather than a BAP-preset restriction. The
 * helper uses them to configure LC3, the I2S peripheral and the codec driver.
 */
struct sample_bt_audio_pcm_cfg {
	uint32_t freq_hz;             /**< LC3 rate the sink accepts: 16000, 32000 or 48000 */
 ...
 *
 * Asynchronous: records the stop intent and defers the blocking teardown
 * (decode-worker park, I2S drain and codec shutdown) to the system workqueue,
 * then returns immediately. It takes an internal mutex, so call it from thread
