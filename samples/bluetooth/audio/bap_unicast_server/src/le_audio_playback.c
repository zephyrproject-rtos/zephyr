...
#define I2S_MEM_SLAB_BLOCKS ((uint32_t)CONFIG_BAP_UNICAST_SERVER_I2S_DMA_BLOCK_COUNT)

/* Depth of the decode -> I2S worker handoff queue. */
#define PCM_MSGQ_DEPTH ((uint32_t)CONFIG_BAP_UNICAST_SERVER_PCM_QUEUE_DEPTH)
...
static __I2S_NOCACHE __aligned(4) uint8_t
	i2s_slab_buffer[CONFIG_BAP_UNICAST_SERVER_I2S_DMA_BLOCK_COUNT * PCM_MAX_BLOCK_BYTES];
 ...
 *
 * Default to full scale (255 -> WM8962 register 127, +6 dB). The WM8962 output-volume
 * curve is heavily bottom-weighted: only the top of the 0..127 range is clearly audible,
 * so a mid VCS value maps to a near-inaudible register setting. Start at max so playback
 * is loud by default; a VCS client can still turn it down via le_audio_playback_set_volume().
 */
static uint8_t                cached_vcs_volume = 255U;
static bool                   cached_mute;


static K_THREAD_STACK_DEFINE(i2s_worker_stack, CONFIG_BAP_UNICAST_SERVER_I2S_WORKER_STACK_SIZE);
...
int le_audio_playback_init(void)
{
	...
	i2s_worker_tid = k_thread_create(&i2s_worker_thread, i2s_worker_stack,
					 ...
					 K_PRIO_COOP(CONFIG_BAP_UNICAST_SERVER_I2S_WORKER_PRIORITY),
					 0, K_NO_WAIT);
