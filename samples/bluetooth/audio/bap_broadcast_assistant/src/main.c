...
static bool parse_broadcast_source_data(struct bt_data *data, void *user_data)
{
	...
	if (info->broadcast_id != BT_BAP_INVALID_BROADCAST_ID && info->bt_name[0] != '\0' &&
	    info->broadcast_name[0] != '\0') {
		/* We have found all we are looking for, stop parsing */
		return false;
	}
}
...
static bool parse_scan_delegator_data(struct bt_data *data, void *user_data)
{
	...
	if (info->has_svc_data && info->bt_name[0] != '\0') {
		/* We have found all we are looking for, stop parsing */
		return false;
	}
}
