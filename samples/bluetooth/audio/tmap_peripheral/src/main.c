...
static void conn_lock_take(void)
{
	__maybe_unused int err = k_mutex_lock(&conn_lock, K_FOREVER);

	__ASSERT(err == 0, "k_mutex_lock failed with %d", err);
}

static void conn_lock_give(void)
{
	__maybe_unused int err = k_mutex_unlock(&conn_lock);
 ...
 * disconnect (which drops default_conn) cannot free it while it is in use.
 */
...
static void media_play_timeout(struct k_work *work)
{
	...
	err = mcp_send_cmd(conn, BT_MCS_OPC_PAUSE);
	bt_conn_unref(conn);
	if (err != 0) {
...
static void serve_connection(struct bt_conn *conn)
{
	...
	if (IS_ENABLED(CONFIG_TMAP_PERIPHERAL_ROLE_UMR) &&
		...
		/* Play media with MCP */
		err = mcp_send_cmd(conn, BT_MCS_OPC_PLAY);
		if (err != 0) {
...
int main(void)
{
	...
	while (true) {
		...
		/* Keep a pending control demo timer from acting on the next peer. */
		if (IS_ENABLED(CONFIG_TMAP_PERIPHERAL_ROLE_CT)) {
			(void)k_work_cancel_delayable(&call_terminate_set_work);
		}
		if (IS_ENABLED(CONFIG_TMAP_PERIPHERAL_ROLE_UMR)) {
			(void)k_work_cancel_delayable(&media_pause_set_work);
		}
