				      ...
				      const struct bt_bap_scan_delegator_recv_state *state)
{
	...
	__ASSERT(err == 0, "Failed to get adv info: %d\n", err);
	...
	/* TODO: We need to copy the receive state */
}
...
static int discover_bass(void)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to discover BASS on the sink: %d\n", err);
		return err;
	}
...
{
	static struct bt_cap_unicast_audio_start_stream_param stream_param[CAP_STREAM_TX_MAX] = {0};
	int err;
...
static int setup_broadcast_adv(void)
{
	...
	if (err != 0) {
		LOG_ERR("Unable to create extended advertising set: %d\n", err);
		return err;
	}
	...
	if (err != 0) {
		LOG_ERR("Failed to set periodic advertising parameters: %d\n", err);
...
static int set_base_data(void)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to get encoded BASE: %d\n", err);
		return err;
	}
	...
	if (err != 0) {
		LOG_ERR("Failed to set periodic advertising data: %d\n", err);
		return err;
	}
...
static int handover_unicast_to_broadcast(void)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to handover unicast audio to broadcast: %d\n", err);
		return err;
	}
	...
	if (err != 0) {
		LOG_ERR("Failed to enable periodic advertising: %d\n", err);
		return err;
	}
	...
	err = set_base_data();
	if (err != 0) {
		LOG_ERR("Failed to set BASE data: %d\n", err);
		return err;
	}
	...
	while (true) {
		...
		/* Once the receive state indicates a non-empty BIS*/
		...
		if (bis_sync_cnt > 0) {
			...
			if (bis_sync_cnt != expected_bis_synced) {
				LOG_ERR("Remote synced to subgroup[0]: 0x%08X, but we expected %u "
					"BIS "
					"syncs",
					peer.bis_sync,
					broadcast_create_param.subgroup_params[0].stream_count);
...
static int handover_broadcast_to_unicast(void)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to enable extended advertising: %d\n", err);
		return err;
	}
...
static int init_cap_handover(void)
{
	...
	if (err != 0) {
		LOG_ERR("Failed to generate broadcast ID: %d\n", err);
		return err;
	}
	...
	if (err != 0) {
		LOG_ERR("Failed to setup broadcast adv: %d\n", err);
		return err;
	}
...
static int reset_cap_handover(void)
{
	...
	if (broadcast_source != NULL) {
		...
		if (k_sem_count_get(&sem_broadcast_stopped) == 0U) {
			...
			if (err != 0) {
				/* -EBADMSG is returned if the broadcast source is already stopped
				 */
