...
void mtu_change_cb(struct bt_conn *conn, uint8_t err, struct bt_gatt_exchange_params *params)
{
	if (err) {
...
/**
 * Start DTLS and Challenge-Response auth methods.
 *
 * @return 0 on success, else negative error value
 */
static int start_authentication(void)
{
	int err = auth_lib_start(&auth_conn_bt);

	if (err) {
		LOG_ERR("Failed to start DTLS auth method, err: %d", err);
		return err;
	}
	...
	if (err) {
		LOG_ERR("Failed to start Challenge-Response auth method, err: %d", err);
		return err;
	}
			     ...
			     struct bt_gatt_discover_params *params)
{
	...
	if (auth_desc_index >= AUTH_SVC_GATT_COUNT) {
		...
		/* Start DTLS and Challenge-Response auth process */
		err = start_authentication();

		if (err == 0) {
			LOG_INF("Started auth services.");
		}
...
void main(void)
{
	...
	printk("Starting multi auth client.\n");

	/* Instance 1 uses DTLS auth method over Bluetooth. */
	err = auth_lib_init(&auth_conn_bt, AUTH_DTLS_INST, auth_status, NULL,
			    &dtls_certs_param,
			    AUTH_CONN_CLIENT | AUTH_CONN_DTLS_AUTH_METHOD);

	if (err) {
		LOG_ERR("Failed to init DTLS authentication service, err: %d.", err);
		idle_function(); /* does not return */
	}

	/* Instance 2 uses Challenge-Response auth method over Serial transport. */
	...
	if (err) {
		LOG_ERR("Failed to init Challenge-Response authentication service, err: %d.", err);
		idle_function(); /* does not return */
	}


	/* Configure the UART for serial transport. */
	err = config_uart();

	if (err) {
		LOG_ERR("Failed to configure the UART, err: %d.", err);
		idle_function(); /* does not return */
	}
