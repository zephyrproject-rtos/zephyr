...
int main(void)
{
	...
	if (result != 0) {
		LOG_ERROR("Namespace string to UUID failed, error: %s (%d)", strerror(result),
			  result);
		return -1;
	}
	result = uuid_generate_v5(&uuid_v5_namespace, "www.example.com", strlen("www.example.com"),
				  &uuid_v5);
	if (result != 0) {
		LOG_ERROR("UUID v5 generation failed, error: %s (%d)", strerror(result), result);
		return -1;
	}
