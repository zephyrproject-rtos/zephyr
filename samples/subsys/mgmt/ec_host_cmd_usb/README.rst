...
In the VM check that the USB device has been mapped to the VM, and bind the
cros-ec-usb to it.
...
devicetree. The simplest way to test them is to hack those in that file as well, for example the ``cros-ec-keyb`` driver that is normally using a devicetree node:
...
.. code-block:: c
    ...
    static const struct mfd_cell keyb_cell = {
           .name = "cros-ec-keyb", .swnode = &keyb_swnode,
    };
