...
Build with the :file:`tls.conf` Kconfig fragment to accept ``https`` URLs.
The sample registers the PEM certificate in :file:`src/ca_cert.pem` as the
trusted CA; the build converts it into a C array included by
:file:`src/main.c`. It is a self-signed certificate for ``localhost`` and
...
address and copy :file:`server.pem` over :file:`src/ca_cert.pem`:
