		...
		case USBH_DEVICE_CONNECTED:
			...
			while (caps.format_caps[i].pixelformat) {
				...
				LOG_INF("  %s width [%u; %u; %u] height [%u; %u; %u]",
					...
					caps.format_caps[i].width_min, caps.format_caps[i].width_max, caps.format_caps[i].width_step,
					caps.format_caps[i].height_min, caps.format_caps[i].height_max, caps.format_caps[i].height_step);
				i++;
			}
			...
			for (i = 0; i < CONFIG_VIDEO_BUFFER_POOL_NUM_MAX; i++) {
				...
				if (vbuf == NULL) {
					LOG_ERR("Unable to alloc video buffer %d/%d", i, CONFIG_VIDEO_BUFFER_POOL_NUM_MAX);
					break;
				}
				LOG_DBG("Allocated buffer %d: vbuf=%p, buffer=%p, size=%zu", 
					i, vbuf, vbuf->buffer, vbuf->size);
			...
			if (i < CONFIG_VIDEO_BUFFER_POOL_NUM_MAX) {
				LOG_ERR("Only allocated %d/%d buffers!", i, CONFIG_VIDEO_BUFFER_POOL_NUM_MAX);
			}
		...
		case USBH_DEVICE_DISCONNECTED:
			for (i = 0; i < allocated_count; i++) {
				if (allocated_vbufs[i]) {
					...
					LOG_DBG("Released buffer %d: vbuf=%p", i, allocated_vbufs[i]);
