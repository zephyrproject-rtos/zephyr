			      ...
			      enum usbh_uac2_stream_dir dir)
{
	...
	const char *indent;
	int16_t min_db, max_db;
	int16_t min_frac, max_frac;
	uint16_t i;
	int ret;
	...
	for (i = 0; i < query.range.num_subranges; i++) {
		...
		min_frac = (int16_t)((abs(query.range.subranges[i].min16 % 256) * 100) / 256);
		max_frac = (int16_t)((abs(query.range.subranges[i].max16 % 256) * 100) / 256);

		if (query.range.num_subranges == 1) {
			LOG_INF("%sMIN: %d.%02d dB (0x%04x)", indent,
				min_db, min_frac,
				(uint16_t)query.range.subranges[i].min16);
			LOG_INF("%sMAX: %d.%02d dB (0x%04x)", indent,
				max_db, max_frac,
				(uint16_t)query.range.subranges[i].max16);
			LOG_INF("%sRES: %d/256 dB (0x%04x)", indent,
				query.range.subranges[i].step16,
				(uint16_t)query.range.subranges[i].step16);
		} else {
			       ...
			       enum usbh_uac2_stream_dir dir)
{
	...
	target_volume = min_volume + (volume_step * (int16_t)g_host_cur_volume);
			   ...
			   uint8_t bit_resolution)
{
	...
	ctx->bytes_per_sample = (bit_resolution + 7U) / 8U;
	ctx->phase            = 0.0;
	ctx->phase_increment  = SINE_TWO_PI * SINE_TONE_FREQUENCY_HZ / sample_rate;
}
...
static uint16_t generate_sine_wave(uint8_t *buffer, uint16_t max_len, void *user_data)
{
	...
	num_frames = max_len / bytes_per_frame;
	actual_len = num_frames * bytes_per_frame;

	if (ctx->bytes_per_sample != 2U) {
		memset(buffer, 0, actual_len);
		return actual_len;
	}

	for (frame = 0U; frame < num_frames; frame++) {
		sample = (int16_t)(sin(ctx->phase) * SINE_TONE_AMPLITUDE);

		for (ch = 0U; ch < ctx->channels; ch++) {
			out16[sample_idx++] = sample;
		}

		ctx->phase += ctx->phase_increment;
		if (ctx->phase >= SINE_TWO_PI) {
			ctx->phase -= SINE_TWO_PI;
		}
	}
...
static bool is_rate_supported(uint32_t rate, const struct uac2_ctrl_range *range)
{
	const struct uac2_ctrl_subrange *sub;
	uint16_t j;

	for (j = 0; j < range->num_subranges; j++) {
		sub = &range->subranges[j];
		if (sub->umin32 == sub->umax32) {
			/* Discrete value. */
			if (rate == sub->umin32) {
				return true;
			}
			...
			/* Continuous range. */
			if (rate >= sub->umin32 && rate <= sub->umax32) {
				return true;
			}
		}
	}
...
static void log_rates_for_dir(const struct device *uac2_dev,
			       enum usbh_uac2_stream_dir dir)
{
	...
	LOG_INF("%s Sample Rates:", dir == USBH_CAPTURE ? "Capture" : "Playback");

	for (i = 0; i < ARRAY_SIZE(test_rates); i++) {
		LOG_INF("  %u Hz: %s", test_rates[i],
			is_rate_supported(test_rates[i], &query.range)
			     ...
			     const struct device_caps *caps)
{
	...
	if (caps->has_playback) {
		log_rates_for_dir(uac2_dev, USBH_PLAYBACK);
	}

	if (caps->has_capture) {
		log_rates_for_dir(uac2_dev, USBH_CAPTURE);
	}
				   ...
				   const struct device_caps *caps)
{
	...
	if (caps->has_playback) {
		...
		ret = set_initial_volume(uac2_dev, USBH_PLAYBACK);
		if (ret != 0) {
			return ret;
		}

		ret = unmute_audio(uac2_dev, USBH_PLAYBACK);
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (caps->has_capture) {
		...
		if (ret != 0) {
			LOG_WRN("Failed to set capture sample rate to 48000 Hz: %d", ret);
		} else {
		...
		ret = set_initial_volume(uac2_dev, USBH_CAPTURE);
		if (ret != 0) {
			return ret;
		}

		ret = unmute_audio(uac2_dev, USBH_CAPTURE);
		if (ret != 0) {
			return ret;
		}
	}
				...
				const struct device_caps *caps)
{
	...
	LOG_INF("Playback started - playing %d Hz sine wave continuously",
		SINE_TONE_FREQUENCY_HZ);
	return 0;
}
...
int main(void)
{
	...
	while (true) {
		...
		if (err != 0) {
			LOG_ERR("Failed to configure audio format: %d", err);
			k_sleep(K_MSEC(50));
			continue;
		}
		...
		if (err != 0 && err != -ENOTSUP) {
			LOG_ERR("Failed to start audio playback: %d", err);
			k_sleep(K_MSEC(50));
			continue;
		}
		...
		if (err != 0) {
			LOG_ERR("Failed to start audio recording: %d", err);
			stop_audio_playback(uac2_dev, &caps);
			k_sleep(K_MSEC(50));
			continue;
		}
