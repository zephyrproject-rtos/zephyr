...
int main(void)
{
	...
	if (video_dev == NULL || !device_is_ready(video_dev)) {
		LOG_ERR("%s: video source is not ready or failed to initialize",
			video_dev->name);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		LOG_ERR("Failed to set %s format", video_dev->name);
		return ret;
	}
	...
	while (true) {
		LOG_DBG("enqueuing to %s", video_dev->name);
		...
		LOG_DBG("dequeueing from %s", video_dev->name);
