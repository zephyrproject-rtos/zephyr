...
The sample identifies itself with the Zephyr Project vendor ID and its own :ref:`product ID
<usbd_vid_pid>`, which the ``i2c-tiny-usb`` driver does not know about. Until the pair is added to
the driver ID table upstream, use the feature described in the `"Dynamic USB device IDs" LWN.net
article`_ to make the driver claim the device:
...
.. _"Dynamic USB device IDs" LWN.net article: https://lwn.net/Articles/160944/
