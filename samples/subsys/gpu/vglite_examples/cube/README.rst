.. zephyr:code-sample:: cube
   :name: VGLite cube sample
   :relevant-api: display_interface

   Demonstrate VGLite GPU 3D cube rendering with isometric projection.

Overview
********

This example shows how to use the VGLite component for hardware-accelerated 2D
graphics rendering. This application draws a 3D cube object with each face having
high render quality, demonstrating VGLite's advanced rendering and transformation
capabilities.

The cube demo showcases VGLite's ability to render complex 3D-like graphics using
2D vector paths with isometric projection. The cube face assets are rendered from
VG (Vector Graphics) paths, and the cube has a moving effect achieved using
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 zephyr/samples/subsys/gpu/vglite_examples/cube/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 zephyr/samples/subsys/gpu/vglite_examples/cube/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/zephyr/boards/nxp/mimxrt595_evk/mimxrt595_evk.overlay" -DCONFIG_DEBUG=y -DCONFIG_DEBUG_OPTIMIZATIONS=n -DCMAKE_BUILD_TYPE=Debug -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500
...
.. code-block:: bash
   ...
   west build -p always -b mimxrt1170_evk/mimxrt1176/cm7 zephyr/samples/subsys/gpu/vglite_examples/cube/ --shield rk055hdmipi4m -DBOARD_REVISION=A -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt1170

For :zephyr:board:`mimxrt700_evk`:

.. code-block:: bash

   west build -p always -b mimxrt700_evk/mimxrt798s/cm33_cpu0 zephyr/samples/subsys/gpu/vglite_examples/cube/ --shield rk055hdmipi4ma0 -DVGLITE_OS=zephyr -DDTC_OVERLAY_FILE=$(west topdir)/zephyr/boards/nxp/mimxrt700_evk/mimxrt700_evk.overlay -DVGLITE_PLATFORM=rt700 -DEXTRA_CPPFLAGS="-DCPU_MIMXRT798SGFOA_cm33_core0"

For :zephyr:board:`mimxrt595_evk`:

.. code-block:: bash

   west build -p always -b mimxrt595_evk/mimxrt595s/cm33 zephyr/samples/subsys/gpu/vglite_examples/cube/ --shield rk055hdmipi4ma0 -- -DDTC_OVERLAY_FILE="$(west topdir)/zephyr/boards/nxp/mimxrt595_evk/mimxrt595_evk.overlay" -DVGLITE_OS=zephyr -DVGLITE_PLATFORM=rt500

Running the Demo
================

When the example runs, you can see a 3D cube object with each face having high
render quality displayed on the screen. The cube face assets are rendered from
VG (Vector Graphics) paths, and the cube has a smooth moving effect achieved
using isometric projection transformations.

The demo demonstrates:

- **Isometric projection** - 3D-like rendering using 2D transformations
- **Vector graphics paths** - High-quality face rendering using VG paths
- **Smooth animation** - Continuous cube rotation and movement
- **Multi-face rendering** - Each cube face rendered with distinct graphics
