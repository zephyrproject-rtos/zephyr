 ...
 * The returned qoi data should be free()d after use.
 */
void *qoi_encode_block(const void *data, const qoi_desc *desc, qoi_enc_state *state,
		       size_t *out_len);
...
{
	bytes[(*p)++] = (0xff000000U & v) >> 24;
	bytes[(*p)++] = (0x00ff0000U & v) >> 16;
	bytes[(*p)++] = (0x0000ff00U & v) >> 8;
	bytes[(*p)++] = (0x000000ffU & v);
}
...
void *qoi_encode(const void *data, const qoi_desc *desc, int *out_len)
{
	...
	if (data == NULL || out_len == NULL || desc == NULL || desc->width == 0 ||
	    desc->height == 0 || desc->channels < 3 || desc->channels > 4 || desc->colorspace > 1 ||
	...
	for (px_pos = 0; px_pos < px_len; px_pos += channels) {
		if (run > 0) {
			...
			if (b1 == QOI_OP_RGB) {
				...
				/* b1 & 0x3f ensures index is always 0-63 for index[64] array */
				unsigned int idx = (unsigned int)(b1 & 0x3f);
