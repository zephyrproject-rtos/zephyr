...
/*
 * The region comes from user_data rather than being hard coded, so one test
 * function can cover several regions: each is another registration, not another
 * copy of the march.
 */
struct ram_region {
...
static const struct ram_region scratch_region = {
	.base = (volatile uint32_t *)&scratch,
	.words = SAMPLE_SCRATCH_WORDS,
};

static int ram_march_run(const struct safety_test_context *ctx)
{
	const struct ram_region *region = ctx->user_data;

	return march_c_minus(region->base, region->words);
}

SAFETY_TEST_DEFINE_EX(ram_march, SAFETY_TEST_CAT_RAM, SAFETY_TEST_LEVEL_PRE_KERNEL_2, 10,
		      SAFETY_TEST_FLAG_BOOT_OK | SAFETY_TEST_FLAG_RUNTIME_OK |
			      SAFETY_TEST_FLAG_DESTRUCTIVE,
		      ram_march_run, "March C- over the sample's scratch RAM",
		      &scratch_region, 2000);
/*
