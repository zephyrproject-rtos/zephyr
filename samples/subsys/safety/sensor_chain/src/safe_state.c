...
static void on_failure(const struct safety_test_context *ctx,
		       const struct safety_test_result_record *rec)
{
	atomic_set(&fault_flag, 1);

	/* What a hook may do depends on the level the test ran at. */
	switch (ctx->init_level) {
	case SAFETY_TEST_LEVEL_EARLY:
	case SAFETY_TEST_LEVEL_PRE_KERNEL_1:
		/* No system timer, so no logging. The flag above is all we get. */
		break;
	case SAFETY_TEST_LEVEL_PRE_KERNEL_2:
		/* Logging works from here, but no driver has initialised yet. */
		LOG_ERR("test '%s' failed with %d after %u us", ctx->test->name,
			rec->error_code, rec->duration_us);
		break;
	default:
		LOG_ERR("test '%s' failed with %d after %u us", ctx->test->name,
			rec->error_code, rec->duration_us);
					       ...
					       const struct safety_test_result_record *rec)
{
	 ...
	 */
	if (safety_test_can_log(ctx->init_level)) {
		LOG_ERR("safe state reached for '%s', returning %s", ctx->test->name,
			action_name(action));
	}
