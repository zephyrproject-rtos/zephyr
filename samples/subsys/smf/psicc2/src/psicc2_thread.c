...
LOG_MODULE_REGISTER(psicc2_thread);
...
K_THREAD_STACK_DEFINE(psicc2_thread_stack, PSICC2_THREAD_STACK_SIZE);
K_MSGQ_DEFINE(psicc2_msgq, sizeof(struct psicc2_event), PSICC2_THREAD_EVENT_QUEUE_SIZE, 1);

static struct k_thread psicc2_thread_data;
static k_tid_t psicc2_thread_tid;

static void psicc2_thread(void *arg1, void *arg2, void *arg3)
{
	smf_set_initial(SMF_CTX(&s_obj), &demo_states[STATE_INITIAL]);
	while (1) {
		int rc = k_msgq_get(&psicc2_msgq, &s_obj.event, K_FOREVER);
...
{
	psicc2_thread_tid =
		k_thread_create(&psicc2_thread_data, psicc2_thread_stack,
				K_THREAD_STACK_SIZEOF(psicc2_thread_stack), psicc2_thread, NULL,
				NULL, NULL, PSICC2_THREAD_PRIORITY, 0, K_NO_WAIT);

	k_thread_name_set(psicc2_thread_tid, "psicc2_thread");
}
