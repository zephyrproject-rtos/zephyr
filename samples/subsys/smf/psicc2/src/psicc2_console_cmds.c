...
#include "psicc2_thread.h"
#include <stdlib.h>
#include <ctype.h>

static int cmd_psicc2_event(const struct shell *sh, size_t argc, char **argv)
{
	struct psicc2_event event;
	...
	int rc = k_msgq_put(&psicc2_msgq, &event, K_NO_WAIT);
...
static int cmd_psicc2_terminate(const struct shell *sh, size_t argc, char **argv)
{
	struct psicc2_event event = {.event_id = EVENT_TERMINATE};
	int rc = k_msgq_put(&psicc2_msgq, &event, K_NO_WAIT);
...
SHELL_STATIC_SUBCMD_SET_CREATE(sub_psicc2,
			       SHELL_CMD_ARG(event, NULL, "Send event to State Machine",
					     cmd_psicc2_event, 2, 0),
			       SHELL_CMD_ARG(terminate, NULL,
					     "Send terminate event to State Machine",
					     cmd_psicc2_terminate, 1, 0),
			       SHELL_SUBCMD_SET_END);

/* Creating root (level 0) command "demo" */
SHELL_CMD_REGISTER(psicc2, &sub_psicc2, "PSICC2 demo state machine commands", NULL);
