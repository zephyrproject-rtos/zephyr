 ...
 *
 * @param p_nam Source name
 *
 * @return Source ID.
 */
static int log_source_id_get(const char *p_name)
{

	for (int i = 0; i < log_src_cnt_get(CONFIG_LOG_DOMAIN_ID); i++) {
		if (strcmp(log_source_name_get(CONFIG_LOG_DOMAIN_ID, i), p_name)
		    == 0) {
			return i;
		}
	}
...
static void instance_logging_showcase(void)
{
	...
	if (IS_ENABLED(CONFIG_LOG_RUNTIME_FILTERING)) {
		...
		log_filter_set(NULL, 0,
			       log_source_id_get(INST1_NAME), LOG_LEVEL_WARN);
		...
		log_filter_set(NULL, 0, log_source_id_get(INST1_NAME), LOG_LEVEL_NONE);
		log_filter_set(NULL, 0, log_source_id_get(INST2_NAME), LOG_LEVEL_NONE);
...
static void performance_showcase(void)
{
	...
	do {
		LOG_INFO("performance test - log message %d", cnt++);
...
void main(void)
{
	...
	int err = LOG_INIT(timestamp_get, timestamp_freq());
