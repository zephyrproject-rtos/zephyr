...
* ``CONFIG_ASSERT_MODULE_MYMODULE_LEVEL_ON=y`` selects ``ZASSERT_LEVEL_ON``.
...
``ZASSERT_LEVEL_OFF`` (0), ``ZASSERT_LEVEL_ON`` (1), or ``ZASSERT_LEVEL_VERBOSE`` (2).
The choice defaults to ``ZASSERT_LEVEL_ON``.
This sample overrides that default in :file:`prj.conf` by selecting the verbose setting.
