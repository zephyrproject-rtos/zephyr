...
static void evt_cb(struct mqtt_sn_client *client, const struct mqtt_sn_evt *evt)
{
	...
	case MQTT_SN_EVT_PINGRESP: /* Received a PINGRESP */
		...
		break;
	case MQTT_SN_EVT_ADVERTISE: /* Received a PINGRESP */
		LOG_INF("MQTT-SN event EVT_ADVERTISE");
		break;
	case MQTT_SN_EVT_GWINFO: /* Received a PINGRESP */
		LOG_INF("MQTT-SN event EVT_GWINFO");
		break;
	case MQTT_SN_EVT_SEARCHGW: /* Received a PINGRESP */
...
static void process_thread(void)
{
	 ...
	 * int err;
	 * LOG_DBG("Parsing Broadcast IP " CONFIG_NET_SAMPLE_MQTT_SN_GATEWAY_IP);
	 * gwaddr.sin_family = AF_INET;
	 * gwaddr.sin_port = htons(CONFIG_NET_SAMPLE_MQTT_SN_GATEWAY_PORT);
	 * err = inet_pton(AF_INET, CONFIG_NET_SAMPLE_MQTT_SN_GATEWAY_IP, &gwaddr.sin_addr);
	 * __ASSERT(err == 1, "inet_pton() failed %d", err);
	 * struct mqtt_sn_data gwaddr_data = {.data = (struct sockaddr *)&bcaddr,
