.. zephyr:code-sample:: online-connectivity-check
   ...
   Use online connectivity checker in connection manager to verify that
   we have connectivity to a user-specified network/host.

Overview
********

This sample application enables the online connectivity checker in network
connection manager. The checker tries to connect to user-specified host either
by an ICMP ping request, or by a HTTP(S) GET request.
If the connection can be established, a ``NET_EVENT_CONNECTIVITY_ONLINE`` is
generated and application can monitor this event and then act on it.
If the connectivity to user-specified host is lost, then
a ``NET_EVENT_CONNECTIVITY_OFFLINE`` event is generated.
...
on the host side so that Linux host can act as a router.
...
Running connectivity-checker in Docker Container
================================================

You can run this ``connectivity-check`` sample application in Linux host and
make it connect to a Docker container which contains needed server components
like HTTP and HTTPS servers, a DNS server and DHCP server.
...
Run the test script which compiles the connectivity-check application for ``native-sim``
...
:kbd:`CTRL+A` :kbd:`x`. The application will run couple of minutes and tries to
