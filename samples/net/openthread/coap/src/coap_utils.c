...
static uint8_t g_coap_buf[COAP_MAX_BUF_SIZE];
static uint8_t g_coap_dev_id[COAP_DEVICE_ID_SIZE];
			 ...
			 otCoapResponseHandler handler, void *ctx, otCoapCode code)
{
	...
	if (err != OT_ERROR_NONE) {
		const char *err_str = otThreadErrorToString(err);

		LOG_ERR("Failed to append uri-path: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
	...
	if (err != OT_ERROR_NONE) {
		...
		LOG_ERR("Failed to set payload marker: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
	...
	if (err != OT_ERROR_NONE) {
		...
		LOG_ERR("Failed to set append payload to response: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
	...
	if (err != OT_ERROR_NONE) {
		...
		LOG_ERR("Failed to send the request: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
...
int coap_resp_send(otMessage *req, const otMessageInfo *req_info, uint8_t *buf, int len)
{
	...
	if (err != OT_ERROR_NONE) {
		...
		LOG_ERR("Failed to initialize the response: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
	...
	if (err != OT_ERROR_NONE) {
		...
		LOG_ERR("Failed to send the response: %s", err_str);
		ret = -1; /* Find a better error code */
		goto err;
	}
		     ...
		     coap_req_handler_put put_fn, coap_req_handler_get get_fn)
{
	...
	if (msg_code == OT_COAP_CODE_PUT && put_fn) {
		...
		otMessageRead(msg, otMessageGetOffset(msg), g_coap_buf, len);
		ret = put_fn(ctx, g_coap_buf, len);
		if (ret) {
			return ret;
		}
...
const char *coap_device_id(void)
{
	...
	if (g_coap_dev_id[0] != '\0') {
		return g_coap_dev_id;
	}
	...
	for (i = 0; i < 8; i++) {
		...
		sprintf(g_coap_dev_id + i * 2, "%02x", eui64.m8[i]);
	}
	g_coap_dev_id[i * 2] = '\0';
...
int coap_init(void)
{
	...
	if (err != OT_ERROR_NONE) {
		...
		return -1; /* TODO: Find a better error code */
	}
