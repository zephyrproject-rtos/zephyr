...
static void send_delay_req(struct ptp_header *fup_ptp_hdr);
...
static void send_delay_req(struct ptp_header *fup_ptp_hdr)
{
	...
	ptp_hdr.major_sdo_id = fup_ptp_hdr->major_sdo_id;
	ptp_hdr.version_ptp = 2;
	ptp_hdr.minor_version_ptp = 0;
	ptp_hdr.message_length = sizeof(ptp_hdr) + sizeof(ptp_body);
	ptp_hdr.domain_number = fup_ptp_hdr->domain_number;
	ptp_hdr.minor_sdo_id = fup_ptp_hdr->minor_sdo_id;
...
static void adjust_clock(void)
{
	...
	if (llabs(offset_ns) > NSEC_PER_SEC / 2) {
		 ...
		 * t4 + t2_t3_delay_ns / 2 + delay_ns
		 */
		int64_t current_ns = ptp_ts_net_to_ns(&state.t4) + t2_t3_delay_ns / 2 + delay_ns;
...
int ptp_slave_init(struct net_if *iface)
{
	...
	memcpy(state.port_identity, iface_addr->addr, iface_addr->len);
