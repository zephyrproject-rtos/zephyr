.. zephyr:code-sample:: net-srtp
   :name: SRTP
   :relevant-api: Srtp

   Send or receive an SRTP audio stream over a UDP multicast.

Overview
********

This sample demonstrates how to use the Zephyr SRTP API to transmit and receive
...
``239.0.1.1`` on port ``5004``.

The **source** role sends a 100 Hz sine wave (16-bit mono, 8000 samples/s) as
SRTP packets using payload type 97. Packets are sent every 50 ms, each carrying
800 samples (one complete period of the tone).

The **sink** role joins the multicast group and receives the stream using a
callback-based API. It tracks its state (idle, active, or timed out) and logs a
warning if no packet arrives within 500 ms.
...
When a source (e.g., ffmpeg) is running alongside, the sink produces:
...
.. code-block:: console
   ...
   [00:00:00.620,000] <wrn> main: Timeout occurred
