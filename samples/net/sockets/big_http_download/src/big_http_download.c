...
#define KiB	*1024
#define MiB	*1024 KiB
#define Bytes2KiB(Bytes)	(((float)Bytes)/(1.0f KiB))
#define Bytes2MiB(Bytes)	(((float)Bytes)/(1.0f MiB))
...
# else
	...
	while (1) {
		...
		cur_bytes += len;
		printf("Load progress: %u Bytes; %.3f KiB; %.3f MiB\r",
...
void main(void)
{
	...
	do {
		...
		printf("Total downloaded so far: %.3f MiB\n",
