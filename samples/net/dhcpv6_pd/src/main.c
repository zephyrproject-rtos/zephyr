...
#if defined(CONFIG_NET_DHCPV6_SERVER)
	{
		/* The address pool and the prefix pool must not overlap,
		 * otherwise a client would end up advertising the subnet its
		 * own WAN address is taken from onto its LAN. The delegated
		 * prefixes are /56 so that a requesting router can carve
		 * several downstream /64s out of the one it is given.
		 */
