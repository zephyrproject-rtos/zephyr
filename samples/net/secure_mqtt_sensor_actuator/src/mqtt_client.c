...
static void on_mqtt_publish(struct mqtt_client *const client, const struct mqtt_evt *evt)
{
	...
	if (rc < 0) {
		LOG_ERROR("Failed to read received MQTT payload [%d]", rc);
		return;
	}
...
static void mqtt_event_handler(struct mqtt_client *const client, const struct mqtt_evt *evt)
{
	...
	case MQTT_EVT_CONNACK:
		if (evt->result != 0) {
			LOG_ERROR("MQTT Event Connect failed [%d]", evt->result);
			break;
		}
		on_mqtt_connect();
		break;
	...
	case MQTT_EVT_PUBREL:
		if (evt->result != 0) {
			LOG_ERROR("MQTT PUBREL error [%d]", evt->result);
			break;
		}
	...
	case MQTT_EVT_SUBACK:
		if (evt->result == MQTT_SUBACK_FAILURE) {
			LOG_ERROR("MQTT SUBACK error [%d]", evt->result);
			break;
		}
...
static int poll_mqtt_socket(struct mqtt_client *client, int timeout)
{
	...
	if (rc < 0) {
		LOG_ERROR("Socket poll error [%d]", rc);
	}
...
static int get_mqtt_payload(struct mqtt_binstr *payload)
{
	...
	if (rc != 0) {
		LOG_ERROR("Failed to get sensor sample [%d]", rc);
		return rc;
	}
	...
	if (rc != 0) {
		LOG_ERROR("Failed to encode JSON object [%d]", rc);
		return rc;
	}
...
int app_mqtt_publish(struct mqtt_client *client)
{
	...
	if (rc != 0) {
		LOG_ERROR("Failed to get MQTT payload [%d]", rc);
	}
...
int app_mqtt_process(struct mqtt_client *client)
{
	...
	if (rc != 0) {
		if (fds[0].revents & POLLIN) {
			...
			if (rc != 0) {
				LOG_ERROR("MQTT Input failed [%d]", rc);
				return rc;
			}
			/* Socket error */
			if (fds[0].revents & (POLLHUP | POLLERR)) {
				LOG_ERROR("MQTT socket closed / error");
				return -ENOTCONN;
			}
		}
	} else {
		...
		rc = mqtt_live(client);
		if (rc != 0) {
			LOG_ERROR("MQTT Live failed [%d]", rc);
			return rc;
		}
	}
...
int app_mqtt_init(struct mqtt_client *client)
{
	...
	if (rc != 0) {
		LOG_ERROR("Failed to resolve broker hostname [%s]", gai_strerror(rc));
		return -EIO;
	}
	if (result == NULL) {
		LOG_ERROR("Broker address not found");
		return -ENOENT;
	}
