...
- **Application Logic** (``samples/net/latmon/main.c``):
...
- **Connect GPIO pins for transmit (Zephyr to SUT) and receive (SUT to Zephyr)**
  ...
  On **FRMD-K64F**, the sample code uses the **Arduino header J2**, ``pin 20`` for transmit the pulse to
...
On the host and to build and flash the Zephyr FRMD-K6F board with the Latmon sample:

.. code-block:: console

   host$ west build -b frdm_k64f samples/net/latmon
   host$ west flash
...
.. code-block:: console
   ...
   sut$ latmus -I gpiochip2,23,falling-edge -O gpiochip2,21 -z -g"histogram" "broadcast"
...
.. code-block:: console
  ...
  root@sut:~# latmus -I gpiochip2,23,falling-edge -O gpiochip2,21 -Z -g"histogram" broadcast
