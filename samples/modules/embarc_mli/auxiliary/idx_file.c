...
static void bin2val(void *out_data_, uint8_t *in_data, uint8_t type_sz, uint32_t el_count)
{
	...
	case 2:
		for (; i < el_count; ++i, j += type_sz)
			((uint16_t *)out_data_)[i] =
				(uint16_t)(in_data[j]) << 8 | (uint16_t)(in_data[j + 1]);
		break;
	case 4:
		for (; i < el_count; ++i, j += type_sz)
			((uint32_t *)out_data_)[i] =
				(uint32_t)(in_data[j]) << 24 | (uint32_t)(in_data[j + 1]) << 16 |
				(uint32_t)(in_data[j + 2]) << 8 | (uint32_t)(in_data[j + 3]);
		break;
	case 8:
		for (; i < el_count; ++i, j += type_sz)
			((uint64_t *)out_data_)[i] =
				(uint64_t)(in_data[j]) << 56 | (uint64_t)(in_data[j + 1]) << 48 |
				(uint64_t)(in_data[j + 2]) << 40 |
				(uint64_t)(in_data[j + 3]) << 32 |
				(uint64_t)(in_data[j + 4]) << 24 |
				(uint64_t)(in_data[j + 5]) << 16 | (uint64_t)(in_data[j + 6]) << 8 |
...
static void val2BEbin(uint8_t *out_data_, void *in_data, uint8_t type_sz, uint32_t el_count)
{
	...
	case 2:
		for (; i < el_count; ++i, j += type_sz) {
			...
			out_data_[j + 1] = (uint8_t)(val16 & 0xFF);
			val16 >>= 8;
	...
	case 4:
		for (; i < el_count; ++i, j += type_sz) {
			...
			out_data_[j + 3] = (uint8_t)(val32 & 0xFF);
			val32 >>= 8;
			out_data_[j + 2] = (uint8_t)(val32 & 0xFF);
			val32 >>= 8;
			out_data_[j + 1] = (uint8_t)(val32 & 0xFF);
			val32 >>= 8;
			out_data_[j] = (uint8_t)(val32 & 0xFF);
		}
		break;
	case 8:
		for (; i < el_count; ++i, j += type_sz) {
			val64 = ((uint64_t *)in_data)[i];
			out_data_[j + 7] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 6] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 5] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 4] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 3] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 2] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j + 1] = (uint8_t)(val64 & 0xFF);
			val64 >>= 8;
			out_data_[j] = (uint8_t)(val64 & 0xFF);
		}
		break;
	}
	return;
...
tIdxRetVal idx_file_read_data(tIdxDescr *descr_, void *data_, uint32_t *shape_)
{
	...
	if (shape_ != NULL) {
		...
		for (i = 0; i < descr_->num_dim; ++i) {
			if (fread((void *)buffer, 1, HEADER_ELEM_SZ, descr_->opened_file) !=
			    HEADER_ELEM_SZ)
				return IDX_ERR_FILE_ACC;
				    ...
				    uint32_t *shape_, uint32_t *shape_dims_, tIdxDataType *el_type_)
{
	...
	if ((data_sz_[0] < descr.num_elements * el_size) || (shape_dims_[0] < descr.num_dim)) {
