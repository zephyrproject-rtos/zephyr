 ...
 *
 * @detail Perform float-to-fx transformation of data,
 *          according to *dst* tensor element type parameters.
 *          Function won't touch any fields of *dst* structurefilling,
 *          only memory it provides as *data*
 *
 * @param[in] src - Pointer to input array of float values
 * @param[in] src_size - Size of input array.
 * @param[out] dst - Pointer to output FX tensor structure.
 *                  Must provide valid data pointer to array of sufficient size,
 ...
 *
 * @detail Perform fx-to-float transformation of data,
 *          according to *src* tensor element type parameters.
 *          dst array must be sufficient to hold all transformed data
 *          (dst_size == total number of elements inside tensor)
 *
