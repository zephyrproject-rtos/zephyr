.. zephyr:code-sample:: mctp_endpoint_sample
   ...
   This sample shows how to create an MCTP endpoint (a reactive node in a
   network of MCTP nodes) over UART. MCTP is a multiplexed messaging protocol
   that works over a variety of transports (i2c, i3c, uart, pcie, etc).

Overview
********
Sets up an MCTP node that listens on a UART for messages targeting a particular
MCTP endpiont id with the message "hello". Responds to this "hello" message with
"world".
...
sample such that this boards UART tx pin connects to the host boards rx pin,
and this boards UART rx pin connects to the host boards tx pin. The boards
