...
MCTP_USB_DEFINE(mctp0, DT_NODELABEL(mctp_usb0), USBD_MCTP_SUBCLASS_MANAGED_DEVICE_ENDPOINT,
...
int main(void)
{
	LOG_INF("MCTP Endpoint EID: %d on %s", mctp0.endpoint_id, CONFIG_BOARD_TARGET);
	...
	mctp_register_bus(mctp_ctx, &mctp0.binding, mctp0.endpoint_id);
