...
MCTP_USB_DT_DEFINE(mctp_endpoint, DT_NODELABEL(mctp_usb));
...
int main(void)
{
	LOG_INF("MCTP Endpoint EID: %d on %s", mctp_endpoint.endpoint_id, CONFIG_BOARD_TARGET);
	...
	mctp_register_bus(mctp_ctx, &mctp_endpoint.binding, mctp_endpoint.endpoint_id);
