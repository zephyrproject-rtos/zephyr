...
Select the sender mode sample by building the project with the
...
An alternative way is to specify ``-DEXTRA_CONF_FILE=overlay-sender.conf`` when
running ``west build`` or ``cmake``.

Receiver mode
=============

Select the receiver mode sample by building the project with the
...
An alternative way is to specify ``-DEXTRA_CONF_FILE=overlay-receiver.conf`` when
running ``west build`` or ``cmake``.
