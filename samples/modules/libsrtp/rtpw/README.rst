.. zephyr:code-sample:: rtpw
   ...
   Send and receive data using the SRTP protocol using libsrtp.

Overveiw
********

A simple demo of using the SRTP protocol using libsrtp to send and receive data over network.
The sample could be sender or receiver.
To test SRTP via a Linux machine, you should build the `libsrtp`_ test program.

To test the sender while PC is the receiver.
...
To test the receiver while PC is sender.
Open a terminal and enter this command:
...
The SRTP key and the server port can be changed via Kconfig.
...
Select the sender mode by building the project with the
``prj_sender.conf`` overlay file enabled using this command:

.. zephyr-app-commands::
   :zephyr-app: samples/modules/libsrtp/rtpw
   :board: frdm_mcxn947/mcxn947/cpu0
   :conf: "prj.conf prj_sender.conf"
   :goals: build
   :compact:
...
Select the receiver mode by building the project with the
``prj_receiver.conf`` overlay file enabled using this command:

.. zephyr-app-commands::
   :zephyr-app: samples/modules/libsrtp/rtpw
   :board: frdm_mcxn947/mcxn947/cpu0
   :conf: "prj.conf prj_receiver.conf"
   :goals: build
   :compact:
