...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  * Do not mark the sample as build only. A sample should be able to build *and*
    ...
    if the harness is set to ``console``.
  * The default configuration for the sample must be in the :file:`prj.conf`
    ...
    sample.
  * The sample should verify the default configuration of the sample and any
  ...
  * Sample output can be validated by leveraging the ``harness_config`` regex option,
    wherever applicable. And a basic human readable output for each sample is required.
  * Use ``platform_allow`` to limit run to the platforms the sample was actually
    verified on only when Kconfig/dts filters are not appliable.
