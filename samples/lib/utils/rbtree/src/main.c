...
int main(void)
{
	...
	struct user_data *max_node = CONTAINER_OF(rb_get_max(&tree), struct user_data, rbnode);
	struct user_data *min_node = CONTAINER_OF(rb_get_min(&tree), struct user_data, rbnode);
	...
	rb_remove(&tree, rb_get_max(&tree));
	rb_remove(&tree, rb_get_min(&tree));

	RB_FOR_EACH(&tree, n) {
		struct user_data *ent = CONTAINER_OF(n, struct user_data, rbnode);

		printk("after removal n -> %d\n", ent->n);
	}
