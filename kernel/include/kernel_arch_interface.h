 ...
 * Behavior when providing unaligned addresses/sizes is undefined.
 *
 ...
 *
 * If the requested caching mode is unsupported and there is no safe
 * fallback (for example, falling back to uncached if caching was requested
 * and caching is not supported by the CPU at all), or otherwise bad flags
 * are passed in, this will return an error.
 *
 * Mappings in the kernel's part of the address space must affect all threads
 * on the system. This is defined as all virtual memory addresses that
 * are equal to or greater than CONFIG_KERNEL_VIRTUAL_BASE_ADDRESS.
 *
