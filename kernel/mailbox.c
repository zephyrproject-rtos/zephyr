...
static int mbox_message_put_walk_op(struct k_thread *thread, void *data)
{
	...
	/*
	 * As this is a match and the final step in the walk operation, it
	 * is safe to remove the thread from the wait queue without deferring
	 * that to a post-walk operation.
	 */
...
#if (CONFIG_NUM_MBOX_ASYNC_MSGS > 0)
	if ((walk_data->thread->base.thread_state & _THREAD_DUMMY) != 0U) {
