...
static bool thread_active_elsewhere(struct k_thread *thread)
{
	...
	for (uint8_t i = 0; i < (uint8_t)CONFIG_MP_NUM_CPUS; i++) {
		if ((i != currcpu) &&
...
static int32_t z_tick_sleep(k_ticks_t ticks)
{
	...
	if (Z_TICK_ABS(ticks) <= 0) {
		/*? What's the aim here? uint32_t = int64_t + uint32_t */
		expected_wakeup_ticks = ticks + sys_clock_tick_get_32();
	} else {
		/*? What's the aim here? uint32_t = int64_t */
...
int32_t z_impl_k_sleep(k_timeout_t timeout)
{
	...
	/*? What's the aim here? int32_t = uint64_t */
