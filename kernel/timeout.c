...
static inline bool time_after_eq64(int64_t a, int64_t b)
{
	return (k_ticks_diff_t)(a - b) >= 0;
}

static inline bool time_before64(int64_t a, int64_t b)
{
	return (k_ticks_diff_t)(a - b) < 0;
}
#else
...
static inline bool time_after_eq32(int32_t a, int32_t b)
{
	return (k_ticks_diff_t)(a - b) >= 0;
}

static inline bool time_before32(int32_t a, int32_t b)
{
	return (k_ticks_diff_t)(a - b) < 0;
}
#endif
...
#ifdef CONFIG_TIMEOUT_64BIT
	if (time_after_eq64(expiry, now)) {
		return expiry - now;
	}
#else
	if (time_after_eq32(expiry, (int32_t)now)) {
		return expiry - now;
	}
#endif
	return 0;
...
void sys_clock_announce(int32_t ticks)
{
	 ...
	 */
#ifdef CONFIG_TIMEOUT_64BIT
	while ((t = first()) != NULL && time_after_eq64(curr_tick, t->dticks)) {
#else
	while ((t = first()) != NULL && time_after_eq32((int32_t)curr_tick, t->dticks)) {
#endif
		struct timeout_heap_elem elem;
