...
static void test_k_thread_create_join(void)
{
...
void test_main(void)
{
	ztest_test_suite(pthread_pressure,
		ztest_unit_test_setup_teardown(test_k_thread_create_join, setup, NULL),
		ztest_unit_test_setup_teardown(test_pthread_create_join, setup, NULL)
	);

	ztest_run_test_suite(pthread_pressure);
}
...
static void setup(void)
{
	if (IS_ENABLED(CONFIG_TEST_PTHREADS)) {
		...
		for (int i = 0; i < NUM_THREADS; ++i) {
			zassert_ok(pthread_attr_init(&pthread_attrs[i]), NULL);
			zassert_ok(pthread_attr_setstack(&pthread_attrs[i], thread_stacks[i],
							 STACK_SIZE), NULL);
			zassert_ok(pthread_attr_setschedpolicy(&pthread_attrs[i], SCHED_FIFO), NULL);
			zassert_ok(pthread_attr_setschedparam(&pthread_attrs[i], &param), NULL);
		}
	}
