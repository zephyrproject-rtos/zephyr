...
#else
	/* In case of SMP platforms we cannot rely on yielding to switch to
	 * child_func_sem_wait and not switch back until it calls sem_wait,
	 * because both our thread and child_func_sem_wait's one might be
	 * executed simultaneously. Ideally we would like to
	 * child_func_sem_wait notify us that it called sem_wait, but that
	 * would require some operation that atomically calls sem_wait and
	 * notifies us, as child_func_sem_wait becomes descheduled after
	 * calling sem_wait and cannot notify us after that. Instead we are
	 * sleeping here for long enough time for child_func_sem_wait to
	 * call sem_wait
	 */
	sleep(1);
#endif
