...
static void *test_pthread_attr_set_inheritsched_parent_fn(void *arg)
{
	ARG_UNUSED(arg);

	pthread_t child;

	zassert_ok(pthread_create(&child, NULL,
					  test_pthread_attr_set_inheritsched_child_fn, NULL));
...
static void test_pthread_attr_set_inheritsched_common(int inheritsched)
{
	...
	zassert_ok(pthread_create(&parent, &attr,
					  test_pthread_attr_set_inheritsched_parent_fn, NULL));
