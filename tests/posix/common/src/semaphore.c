...
ZTEST(semaphore, test_named_semaphore)
{
	...
	memset(long_sem_name, 'a', sizeof(long_sem_name) - 1);
	long_sem_name[sizeof(long_sem_name) - 1] = '\0';

	sem1 = sem_open(long_sem_name, 0, 0, 0);
	zassert_equal(errno, ENAMETOOLONG, "\"%s\" should be longer than %d", long_sem_name,
