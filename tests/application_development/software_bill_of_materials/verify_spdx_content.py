...
# Generated by the module's CMakeLists as empty ar archives (just the 8-byte
# magic). The -l:FILENAME one is deliberately not named lib*.a, since that is
# the whole point of that linker flag.
FILE_BLOB_IMPORTED = "./zephyr/blobs/libsbom_blob_imported.a"
FILE_BLOB_FLAG = "./zephyr/blobs/libsbom_blob_flag.a"
FILE_BLOB_LITERAL = "./zephyr/blobs/sbom_blob_literal.a"
BLOB_CONTENT = b"!<arch>\n"
BLOB_VERSION = "1.2.3"
...
class TestBlobs:
    ...
    The used module links three fake blobs the three ways real HAL modules link
    theirs: as an imported library (hal_stm32 style), as -lNAME/-L linker flags
    (hal_espressif style) and as an -l:FILENAME flag naming the archive
    verbatim (the way hal_stm32 links the STM32WB0 BLE stack). All three must
    show up in the SBOM.
    """

    @pytest.fixture(params=[FILE_BLOB_IMPORTED, FILE_BLOB_FLAG, FILE_BLOB_LITERAL])
    ...
    def test_blobs_static_linked_into_image(self, build_doc, zephyr_doc):
        ...
        for blob_name in (FILE_BLOB_IMPORTED, FILE_BLOB_FLAG, FILE_BLOB_LITERAL):
