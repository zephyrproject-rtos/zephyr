...
static inline int prepare_listen_sock_udp_v4(struct net_sockaddr_in *addr)
{
	int ret, sock;

	zassert_not_null(addr, "null net_sockaddr");

	ret = zsock_socket(NET_AF_INET, NET_SOCK_DGRAM, NET_IPPROTO_UDP);
	zassert_true(ret >= 0, "socket open failed");

	sock = ret;

	zassert_equal(addr->sin_family, NET_AF_INET, "Invalid family");
...
static inline int prepare_listen_sock_udp_v6(struct net_sockaddr_in6 *addr)
{
	int ret, sock;

	zassert_not_null(addr, "null net_sockaddr");

	ret = zsock_socket(NET_AF_INET6, NET_SOCK_DGRAM, NET_IPPROTO_UDP);
	zassert_true(ret >= 0, "socket open failed");

	sock = ret;

	zassert_equal(addr->sin6_family, NET_AF_INET6, "Invalid family");
				       ...
				       int *sock, struct net_sockaddr_in *sockaddr)
{
	...
	*sock = zsock_socket(NET_AF_INET, NET_SOCK_DGRAM, NET_IPPROTO_UDP);
	...
	sockaddr->sin_port = net_htons(port);
	rv = zsock_inet_pton(NET_AF_INET, addr, &sockaddr->sin_addr);
	zassert_equal(rv, 1, "inet_pton failed");
}

static inline void prepare_sock_udp_v6(const char *addr, uint16_t port,
				       int *sock, struct net_sockaddr_in6 *sockaddr)
{
	int rv;
	...
	*sock = zsock_socket(NET_AF_INET6, NET_SOCK_DGRAM, NET_IPPROTO_UDP);
	...
	sockaddr->sin6_family = NET_AF_INET6;
	sockaddr->sin6_port = net_htons(port);
	rv = zsock_inet_pton(NET_AF_INET6, addr, &sockaddr->sin6_addr);
	zassert_equal(rv, 1, "inet_pton failed");
}
				       ...
				       enum net_ip_protocol_secure proto)
{
	...
	*sock = zsock_socket(NET_AF_INET, NET_SOCK_STREAM, proto);
	zassert_true(*sock >= 0, "socket open failed");
				       ...
				       enum net_ip_protocol_secure proto)
{
	...
	*sock = zsock_socket(NET_AF_INET6, NET_SOCK_STREAM, proto);
	zassert_true(*sock >= 0, "socket open failed");
					...
					enum net_ip_protocol_secure proto)
{
	...
	*sock = zsock_socket(NET_AF_INET, NET_SOCK_DGRAM, proto);
	zassert_true(*sock >= 0, "socket open failed");
					...
					enum net_ip_protocol_secure proto)
{
	...
	*sock = zsock_socket(NET_AF_INET6, NET_SOCK_DGRAM, proto);
	zassert_true(*sock >= 0, "socket open failed");
