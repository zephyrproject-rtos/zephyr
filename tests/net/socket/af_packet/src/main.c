...
static struct net_if_mcast_monitor mcast_mon_l2;
static struct net_if_mcast_monitor mcast_mon_ip;
...
static int mcast_ip_count;

static void mcast_l2_cb(struct net_if *iface, const struct net_linkaddr *addr,
			bool add_membership)
{
	mcast_l2_data.iface = iface;
	mcast_l2_data.add_membership = add_membership;
	mcast_l2_data.count++;

	(void)net_linkaddr_copy(&mcast_l2_data.addr, addr);
}

static void mcast_ip_cb(struct net_if *iface, const struct net_addr *addr,
			bool is_joined)
{
	ARG_UNUSED(iface);
	ARG_UNUSED(addr);
...
static void mcast_mreq_init(struct net_packet_mreq *mreq, struct net_if *iface)
{
	...
	memset(&mcast_l2_data, 0, sizeof(mcast_l2_data));
	mcast_ip_count = 0;
}
...
ZTEST(socket_packet, test_packet_sock_add_drop_membership)
{
	...
	net_if_mcast_mon_register_l2(&mcast_mon_l2, ud.first, mcast_l2_cb);
	...
	zassert_equal(mcast_l2_data.count, 1, "L2 monitor not called");
	zassert_equal(mcast_l2_data.iface, ud.first, "Called for wrong iface");
	zassert_true(mcast_l2_data.add_membership, "Membership not added");
	zassert_equal(mcast_l2_data.addr.len, sizeof(mcast_lladdr),
		      "Invalid link address length (%d)", mcast_l2_data.addr.len);
	zassert_equal(mcast_l2_data.addr.type, NET_LINK_ETHERNET,
		      "Invalid link address type (%d)", mcast_l2_data.addr.type);
	zassert_mem_equal(mcast_l2_data.addr.addr, mcast_lladdr,
			  sizeof(mcast_lladdr), "Invalid link address");

	ret = zsock_setsockopt(packet_sock_1, ZSOCK_SOL_PACKET,
			       ZSOCK_PACKET_DROP_MEMBERSHIP, &mreq, sizeof(mreq));
	zassert_ok(ret, "Cannot drop membership (%d)", errno);

	zassert_equal(mcast_l2_data.count, 2, "L2 monitor not called");
	zassert_false(mcast_l2_data.add_membership, "Membership not dropped");

	net_if_mcast_mon_unregister(&mcast_mon_l2);
}
...
ZTEST(socket_packet, test_packet_sock_membership_other_iface)
{
	...
	/* The monitor only watches the first interface so it must not be
	 * called for a membership change on the second one.
	 */
	net_if_mcast_mon_register_l2(&mcast_mon_l2, ud.first, mcast_l2_cb);

	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 0, "L2 monitor called unexpectedly");
	...
	/* A monitor watching all interfaces must be called instead */
	net_if_mcast_mon_register_l2(&mcast_mon_l2, NULL, mcast_l2_cb);

	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 1, "L2 monitor not called");
	zassert_equal(mcast_l2_data.iface, ud.second, "Called for wrong iface");

	net_if_mcast_mon_unregister(&mcast_mon_l2);
}

ZTEST(socket_packet, test_packet_sock_membership_monitor_levels)
{
	struct net_packet_mreq mreq;
	...
	/* IP level and L2 level monitors share one list, so each level must
	 * only ever call the monitors registered for that level.
	 */
	net_if_mcast_mon_register(&mcast_mon_ip, ud.first, mcast_ip_cb);
	net_if_mcast_mon_register_l2(&mcast_mon_l2, ud.first, mcast_l2_cb);
	...
	zassert_equal(mcast_l2_data.count, 1, "L2 monitor not called");
	zassert_equal(mcast_ip_count, 0, "IP monitor called for a L2 event");

	net_if_mcast_monitor(ud.first, &addr, true);

	zassert_equal(mcast_ip_count, 1, "IP monitor not called");
	zassert_equal(mcast_l2_data.count, 1, "L2 monitor called for an IP event");

	/* Registering an already registered monitor at the other level must
	 * switch it over instead of leaving it registered at both levels.
	 */
	net_if_mcast_mon_register(&mcast_mon_l2, ud.first, mcast_ip_cb);

	mcast_mreq_group(&mreq, ud.first, 0x02);
	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 1, "L2 callback still registered");

	net_if_mcast_monitor(ud.first, &addr, true);
	zassert_equal(mcast_ip_count, 3, "IP callback not called (%d)",
		      mcast_ip_count);

	net_if_mcast_mon_unregister(&mcast_mon_l2);
	net_if_mcast_mon_unregister(&mcast_mon_ip);

	/* Balance the two IP level joins above, as the Ethernet L2 counts the
	 * users of a group and the interface is shared with the other tests.
	 */
...
ZTEST(socket_packet, test_packet_sock_membership_close)
{
	...
	zassert_equal(mcast_l2_data.count, 4, "Memberships not dropped on close");
	zassert_false(mcast_l2_data.add_membership, "Membership not dropped");
...
ZTEST(socket_packet, test_packet_sock_membership_two_sockets)
{
	...
	/* Every socket reports its own join and leave... */
	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 1, "L2 monitor not called");
	...
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 2, "Second join not reported");
	...
	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_DROP_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot drop membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 3, "Leave not reported");
	zassert_equal(eth_filter_data.count, 1,
	...
	zassert_equal(mcast_l2_data.count, 4, "Membership not dropped on close");
	zassert_false(mcast_l2_data.add_membership, "Membership not dropped");
...
ZTEST(socket_packet, test_packet_sock_membership_duplicate)
{
	...
	/* The same socket joining twice needs two leaves */
	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership (%d)", errno);
	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_ADD_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot add membership again (%d)", errno);
	zassert_equal(mcast_l2_data.count, 1, "Second join reported");

	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_DROP_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot drop membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 1, "Group dropped too early");

	ret = mcast_membership(packet_sock_1, ZSOCK_PACKET_DROP_MEMBERSHIP, &mreq);
	zassert_ok(ret, "Cannot drop membership (%d)", errno);
	zassert_equal(mcast_l2_data.count, 2, "Group not dropped");
...
/* The Ethernet L2 has a L2 multicast monitor of its own which programs the
 * group to the receive filter of the device.
 */
ZTEST(socket_packet, test_packet_sock_membership_eth_filter)
{
	struct net_packet_mreq mreq;
	int ret;
...
/* The Ethernet L2 needs a monitor of its own for each level, registering the
 * L2 one must not stop the IP level joins from reaching the receive filter.
 */
