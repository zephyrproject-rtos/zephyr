 ...
 *
 * @param arg0 a pointer to the int representing the server filed descriptor
 * @param arg1 ignored
 * @param arg2 ignored
 */
static void server_thread_fn(void *arg0, void *arg1, void *arg2)
{
	...
	__ASSERT(r != -1, "accept() failed (%d)", r);
	client_fd = r;

	memset(addrstr, '\0', sizeof(addrstr));
	addrstrp = (char *)inet_ntop(AF_INET, &sa.sin_addr,
	addrstr, sizeof(addrstr));
	__ASSERT(addrstrp != NULL, "inet_ntop() failed (%d)", errno);
	...
	__ASSERT(r != -1, "recv() failed (%d)", errno);
	__ASSERT(r == SECRET_SIZE, "expected: %zu actual: %d", SECRET_SIZE, r);

	NET_DBG("calling send()");
	r = send(client_fd, SECRET, SECRET_SIZE, 0);
	__ASSERT(r != -1, "send() failed (%d)", errno);
	__ASSERT(r == SECRET_SIZE, "expected: %zu actual: %d", SECRET_SIZE, r);

	NET_DBG("closing client fd");
	r = close(client_fd);
	__ASSERT(r != -1, "close() failed on the server fd (%d)", errno);
}
...
static void test_common(int peer_verify)
{
	...
	r = k_sem_take(&server_sem, K_MSEC(1999));
...
void test_main(void)
{
	...
	if (IS_ENABLED(CONFIG_TLS_CREDENTIALS)) {
		...
		r = tls_credential_add(CA_CERTIFICATE_TAG,
			...
			ca, sizeof(ca));
		__ASSERT(r == 0, "failed to add CA Certificate (%d)", r);

		r = tls_credential_add(SERVER_CERTIFICATE_TAG,
			TLS_CREDENTIAL_SERVER_CERTIFICATE,
			server, sizeof(server));
		__ASSERT(r == 0, "failed to add Server Certificate (%d)", r);

		r = tls_credential_add(SERVER_CERTIFICATE_TAG,
			TLS_CREDENTIAL_PRIVATE_KEY,
			server_privkey, sizeof(server_privkey));
		__ASSERT(r == 0, "failed to add Server Private Key (%d)", r);

		r = tls_credential_add(CLIENT_CERTIFICATE_TAG,
			TLS_CREDENTIAL_SERVER_CERTIFICATE,
			client, sizeof(client));
		__ASSERT(r == 0, "failed to add Client Certificate (%d)", r);

		r = tls_credential_add(CLIENT_CERTIFICATE_TAG,
			TLS_CREDENTIAL_PRIVATE_KEY,
			client_privkey, sizeof(client_privkey));
		__ASSERT(r == 0, "failed to add Client Private Key (%d)", r);
	}
