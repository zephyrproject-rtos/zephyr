...
ZTEST(net_socket_udp, test_v4_mcast_loop)
{
	...
	mcast_addr.sin_port = htons(SERVER_PORT);
	memcpy(&mcast_addr.sin_addr, &my_mcast_addr2, sizeof(mcast_addr.sin_addr));

	recv_sock = zsock_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	zassert_true(recv_sock >= 0, "Cannot create socket (%d)", -errno);

	bind_addr.sin_family = AF_INET;
	bind_addr.sin_port = htons(SERVER_PORT);
	bind_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	ret = zsock_bind(recv_sock, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
	zassert_ok(ret, "Cannot bind socket (%d)", -errno);

	ret = zsock_setsockopt(recv_sock, IPPROTO_IP, IP_ADD_MEMBERSHIP,
			       &mreqn, sizeof(mreqn));
	 ...
	 */
	send_sock = zsock_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
	...
	ret = zsock_bind(send_sock, (struct sockaddr *)&bind_addr, sizeof(bind_addr));
	zassert_ok(ret, "Cannot bind socket (%d)", -errno);

	ret = zsock_setsockopt(send_sock, IPPROTO_IP, IP_MULTICAST_LOOP,
			       &loop, sizeof(loop));
