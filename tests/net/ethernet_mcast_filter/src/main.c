 ...
 * was told about it, as the L2 keeps track of the link addresses and informs
 * the driver only when the first user joins or the last user leaves.
 */
 ...
 * the tests would depend on each other as the L2 keeps track of the
...
ZTEST(net_ethernet_mcast_filter, test_maddr_shared_mac)
{
	...
	/* The filter is still needed by mcast_addr4_same_mac, so the L2 must
	 * not have told the driver to drop it.
	 */
	check_filter_installed(&eth_filter_context, &mcast_mac4, eth_filter_iface);
