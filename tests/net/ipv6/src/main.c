...
static int tester_send(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (icmp->type == NET_ICMPV6_NS) {
		if (expecting_dad) {
			...
			if (dad_time[0] == 0) {
				dad_time[0] = k_uptime_get();
			} else if (dad_time[1] == 0) {
				dad_time[1] = k_uptime_get();
			} else if (dad_time[2] == 0) {
				dad_time[2] = k_uptime_get();
			}
...
static void test_dad_timeout(void)
{
	...
	/* We should have received three DAD queries, make sure they are spaced
	 * in about 10ms apart.
	 */
	...
	zassert_true((dad_time[2] - dad_time[0]) < 100,
		     "DAD timers took too long time [%lld] [%lld] [%lld]",
