...
ZTEST(net_ipv6, test_ipv6_ext_hdr_len_bounds_4)
{
	...
	pkt = net_pkt_alloc_with_buffer(iface, sizeof(ipv6_ext_hdr_err_4), NET_AF_INET6, 0,
					K_MSEC(WAIT_TIME));
...
static bool wait_for_nbr_removed(struct net_if *iface, struct net_in6_addr *addr,
				 int32_t timeout_ms)
{
	int64_t end = k_uptime_get() + timeout_ms;
...
ZTEST(net_ipv6, test_na_no_options)
{
	...
	had_nbr = net_ipv6_nbr_lookup(TEST_NET_IF, &peer_addr) != NULL;
