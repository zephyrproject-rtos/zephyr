...
Name resolution over IPv4 and IPv6, and the answers that come back: the records
they carry, the name compression, the cache flush bit, and the silence that
should follow a query for a name the responder does not own.

See :ref:`ttcn3_known_gaps` for the divergences from RFC 6762 that the suite
records.
