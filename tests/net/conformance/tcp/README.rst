...
server on port 4242 that accepts a connection and echoes what it is sent, so
the suite can open one and drive it, and a client that connects out to port
4243 on the peer every three seconds, so the suite can look at what Zephyr
sends when it is the one starting. Nothing about the test is compiled into it
and there is no control channel, so the TCP being exercised is the one that
ships.
