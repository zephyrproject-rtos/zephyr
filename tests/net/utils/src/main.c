...
ZTEST(test_utils_fn, test_parse_ipv4_overflow)
{
	...
	struct sockaddr *addr = net_sad(&saddr);
	bool ret;
	 ...
	 */
	{
		...
		zassert_equal(net_sin(addr)->sin_family, AF_INET,
			      "Case 3: family must be AF_INET");
		zassert_equal(net_sin(addr)->sin_port, htons(80),
	 ...
	 */
	{
		...
		zassert_equal(net_sin(addr)->sin_port, htons(0),
	 ...
	 */
	{
		...
		zassert_equal(net_sin(addr)->sin_port, htons(65535),
...
ZTEST(test_utils_fn, test_parse_ipv6_overflow)
{
	 ...
	 */
	{
		...
		(void)memset(addr, 0, sizeof(struct sockaddr_in6));
	 ...
	 */
	{
		...
		zassert_equal(net_sin6(addr)->sin6_family, AF_INET6,
			      "Case 3: family must be AF_INET6");
		zassert_equal(net_sin6(addr)->sin6_port, htons(80),
			      "Case 3: port must be 80");
	}
	...
	/* Case 6: Regression, port 0 (lower boundary).                      */
	{
		...
		ret = net_ipaddr_parse(str, strlen(str), addr);
		zassert_true(ret, "Case 6: port 0 must be accepted");
		zassert_equal(net_sin6(addr)->sin6_port, htons(0),
			      "Case 6: port must be 0");
	}
	...
	/* Case 7: Regression, port 65535 (upper boundary).                  */
	{
		...
		ret = net_ipaddr_parse(str, strlen(str), addr);
		zassert_true(ret, "Case 7: port 65535 must be accepted");
		zassert_equal(net_sin6(addr)->sin6_port, htons(65535),
			      "Case 7: port must be 65535");
	}
	 ...
	 * address-copy path (end up to INET6_ADDRSTRLEN-1) is also safe.
	 */
	{
		...
		zassert_equal(net_sin6(addr)->sin6_port, htons(8080),
