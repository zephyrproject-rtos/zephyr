...
/* Whether the handler for the leased address arriving stops or restarts the
 * client.
 */
static bool stop_on_addr_add;
static bool restart_on_addr_add;
...
/* Restarting from that same callback leaves the reply belonging to an exchange
 * the client has left. The restart re-enters INIT-REBOOT, so the state the
 * reply arrived in comes back and only the transaction identifier tells the
 * two apart. The address the reply had added goes back, and the exchange the
 * callback started is the one that binds.
 */
ZTEST(dhcpv4_tests, test_restart_from_the_address_added_callback)
{
	struct net_if *iface;
	uint32_t evt;
	...
	/* Stopping keeps the address as the hint, so the client comes back in
	 * INIT-REBOOT and the reply below arrives in that state.
	 */
	net_dhcpv4_stop(iface);

	k_event_set(&events, 0U);
	restart_on_addr_add = true;

	net_dhcpv4_start(iface);

	evt = k_event_wait_all(&events, EVT_ADDR_ADD | EVT_ADDR_DEL | EVT_DHCP_BOUND,
			       false, WAIT_TIME);
	zassert_equal(evt, EVT_ADDR_ADD | EVT_ADDR_DEL | EVT_DHCP_BOUND,
		      "Reply of the abandoned exchange was entered as a lease %08x", evt);

	zassert_true(init_reboot_request_count >= 2U,
		     "The exchange started from the callback sent no request of its own (%u)",
		     init_reboot_request_count);
	zassert_not_null(net_if_ipv4_addr_lookup_by_iface(iface, &leased_addr),
			 "Client did not bind after being restarted from the callback");
