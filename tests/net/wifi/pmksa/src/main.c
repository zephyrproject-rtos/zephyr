			...
			struct wifi_connect_req_params *params)
{
	...
	connect_called = true;
	zassert_true(params->pmksa_entry_count <= ARRAY_SIZE(captured_entries),
		     "CONNECT supplied too many test entries");
	captured_count = params->pmksa_entry_count;
	for (size_t i = 0U; i < captured_count; ++i) {
		captured_entries[i] = params->pmksa_entries[i];
	}
...
static struct wifi_pmksa_cache_event event_info;
...
#else
ZTEST(net_wifi_pmksa, test_connect_rejects_pmksa_entries_when_import_disabled)
{
	zassert_equal(request_connect(&valid_entry, 1U), -ENOTSUP,
		      "CONNECT accepted a PMKSA entry with import disabled");
	zassert_false(connect_called, "Unsupported PMKSA import reached the backend");
}
#endif

ZTEST(net_wifi_pmksa, test_connect_propagates_backend_error)
{
	connect_result = -EIO;

	zassert_equal(request_connect(NULL, 0U), -EIO, "CONNECT backend error was not propagated");
	zassert_true(connect_called, "CONNECT backend was not called");
}

ZTEST(net_wifi_pmksa, test_connect_rejects_null_count_mismatch)
{
	struct wifi_connect_req_params params = {
		.ssid = ssid,
		.ssid_length = sizeof(ssid) - 1U,
...
#ifdef CONFIG_WIFI_MGMT_PMKSA_IMPORT
ZTEST(net_wifi_pmksa, test_connect_import_forwards_malformed_hints)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(malformed); ++i) {
		connect_called = false;
		zassert_equal(request_connect(&malformed[i], 1U), 0,
			      "Malformed record %zu blocked CONNECT", i);
		zassert_true(connect_called, "Malformed record did not reach CONNECT");
		zassert_equal(captured_count, 1U, "Malformed record count changed");
		zassert_true(pmksa_entry_equal(&captured_entries[0], &malformed[i]),
			     "Malformed record %zu changed", i);
	}
}
...
static void expect_get_error(int expected_error, uint32_t expected_count)
{
	struct wifi_pmksa_cache_query query = {
		.entry = valid_entry,
		.index = 1U,
	...
	zassert_equal(query.entry_count, expected_count, "GET error returned an unexpected count");
...
{
	get_result = -ENOENT;
	expect_get_error(-ENOENT, backend_entry_count);

	wifi_mgmt_api.pmksa_get = NULL;
	expect_get_error(-ENOTSUP, 0U);
	wifi_mgmt_api.pmksa_get = fake_pmksa_get;

	zassert_equal(net_if_down(wifi_iface), 0, "Failed to bring interface down");
	expect_get_error(-ENETDOWN, 0U);
}
...
ZTEST(net_wifi_pmksa, test_indexed_export_propagates_backend_errors)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(errors); ++i) {
		get_result = errors[i];
		expect_get_error(errors[i], 0U);
	}
}

ZTEST(net_wifi_pmksa, test_cache_events_identify_profile_and_bssid)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(events); ++i) {
		const struct wifi_pmksa_cache_event info = {
			.ssid = "pmksa-test",
			.ssid_length = sizeof("pmksa-test") - 1U,
			.bssid = {0x02, 0x00, 0x00, 0x00, 0x00, 0x09},
		};

		event_seen = 0U;
		k_sem_reset(&pmksa_event_received);
		wifi_mgmt_raise_pmksa_cache_event(wifi_iface, events[i], &info);
		...
		zassert_equal(event_info.ssid_length, sizeof("pmksa-test") - 1U,
			      "PMKSA event SSID length changed");
		zassert_mem_equal(event_info.ssid, ssid, event_info.ssid_length,
				  "PMKSA event SSID changed");
		zassert_mem_equal(event_info.bssid, bssid, sizeof(bssid),
				  "PMKSA event BSSID changed");
	}
}
#endif
...
ZTEST(net_wifi_pmksa, test_iface_status_initializes_usage_and_propagates_errors)
{
	...
	for (size_t i = 0U; i < ARRAY_SIZE(results); ++i) {
		...
		zassert_equal(status_usage_before_dispatch, WIFI_PMKSA_CACHE_USAGE_UNKNOWN,
