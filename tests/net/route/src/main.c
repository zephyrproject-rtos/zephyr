...
static void route_get_nexthop(void)
{
	...
	if (!nexthop) {
		zassert_true(0, "Route get nexthop failed");
	}

	if (!net_ipv6_addr_cmp(nexthop, &peer_addr)) {
		zassert_true(0, "Route nexthop does not match");
	}
}
...
static void route_del_nexthop(void)
{
	...
	if (ret <= 0) {
		zassert_true(0, "Route del nexthop failed");
	}
}
...
static void route_del_nexthop_again(void)
{
	...
	if (ret >= 0) {
		zassert_true(0,
			"Route del again nexthop failed");
	}
}
...
static void route_add_many(void)
{
	...
	for (i = 0; i < max_routes; i++) {
		...
		test_routes[i] = net_route_add(my_iface,
					  ...
					  &peer_addr);
		zassert_not_null(test_routes[i],
				"Route add failed");
		}
}
