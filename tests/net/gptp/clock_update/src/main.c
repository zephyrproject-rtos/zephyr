...
#define TEST_SCALED_PPM                                                                    \
	((CONFIG_PRECISION_TIMING_PI_KP + CONFIG_PRECISION_TIMING_PI_KI) *                  \
	 PRECISION_CLOCK_SCALED_PPM_ONE / 1000)

/* gptp_apply_clock_update() is exposed only in Ztest builds. */
...
static void before(void *fixture)
{
	...
	precision_pi_init(&pi, (double)CONFIG_PRECISION_TIMING_PI_KP / 1000.0,
...
ZTEST(gptp_clock_update, test_small_offset_adjusts_rate)
{
	...
	zassert_equal(fake_data.scaled_ppm, TEST_SCALED_PPM);
