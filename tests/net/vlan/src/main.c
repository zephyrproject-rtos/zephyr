...
static int eth_tx(struct net_if *iface, struct net_pkt *pkt)
{
	...
	if (test_started) {
		...
		zassert_equal(context->expecting_tag,
			      net_eth_get_vid(ntohs(hdr->vlan.tci)),
...
static enum eth_hw_caps eth_vlan_get_capabilities(struct device *dev)
{
	return 0;
}
...
static struct ethernet_api api_funcs = {
	...
	.get_capabilities = eth_vlan_get_capabilities,
};
...
static void vlan_tci_test(void)
{
	...
	tci = net_eth_set_vid(tci, tag);
	tci = net_eth_set_pcp(tci, priority);

	zassert_equal(tag, net_eth_get_vid(tci), "Invalid VLAN tag\n");
	zassert_equal(priority, net_eth_get_pcp(tci),
		      "Invalid VLAN priority\n");
}
