...
def _zephyr_host_iface() -> str | None:
    """The host interface the offloaded driver binds to by default.

    Mirrors nsos_adapt_auto_ifindex(): the interface used for the default route
    (default gateway). Only addresses of this interface are mirrored onto the
    Zephyr net_if, so a family is only expected when this interface provides it.
    """
    try:
        with open("/proc/net/route") as route:
            next(route)  # Skip the header line.
            for line in route:
                fields = line.split()
                # Default route: destination 0.0.0.0 with a gateway set.
...
def _host_addresses(version: int) -> set[str]:
    """Routable IPv4 (version 4) or IPv6 (version 6) addresses of the interface
    the offloaded driver binds to."""
    iface = _zephyr_host_iface()
...
def resolved_addresses(unlaunched_dut: DeviceAdapter) -> set[str]:
    ...
    def on_state_change(zeroconf, service_type, name, state_change):
        ...
        if info is None:
            return
        addrs = {
            str(ip) for ip in map(ipaddress.ip_address, info.parsed_addresses()) if _routable(ip)
        }
        if not addrs:
            return
