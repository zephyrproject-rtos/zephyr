...
ZTEST(lwm2m_registry, test_opaque)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(tc); i++) {
		...
		memset(buf_set, pattern, res_maxlen);
		memset(buf_get, ~pattern, res_maxlen);
		...
		if (buf_set_len < sizeof(buf_set)) {
			/* written too much? */
			zassert_equal(buf_get[buf_set_len], ~pattern, "case %d: %s", i, tc[i].name);
