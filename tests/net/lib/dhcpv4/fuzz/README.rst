...
The state the client is in when the case arrives bounds the window in a
second sense. :c:func:`net_dhcpv4_start` leaves the client in ``INIT``
and defers ``SELECTING`` to a work item armed up to
:kconfig:option:`CONFIG_NET_DHCPV4_INITIAL_DELAY_MAX` seconds out, which
no case window reaches. Both ``dhcpv4_handle_msg_offer()`` and
``dhcpv4_handle_msg_ack()`` fall straight out of their state switch in
``INIT``, so a case injected there is decoded by the option parser and
then discarded, and the code that reads the offered address and the
server id out of the message never runs. The harness therefore places
the client itself: each case is injected twice, once in ``SELECTING``
and once in ``REQUESTING``, the states those two handlers act in. The
``REQUESTING`` run also presets ``requested_ip``, which an OFFER would
have filled in, so an ACK reaches ``dhcpv4_enter_bound()`` rather than
stopping at the address add.
...
Fuzzing a sequence rather than a message is what would cover it. The two
states the harness starts from are the ones a server message is answered
in; no case is injected while the client is in ``INIT_REBOOT``,
``RENEWING``, ``REBINDING`` or ``BOUND``, so the behaviour an ACK or a
NAK gets in those is not covered either.
...
The corpus is a seed set, not an accumulating one: each seed names one
input condition, a new one belongs only when it names a condition no
existing seed does, and it is minimised (``-minimize_crash=1``, then
``-merge=1``) before its hex is added to :file:`seeds.txt`. Two seeds
may still share a branch, as ``opt_zero_length`` and
``opt_subnet_mask_oversize`` do from either side of the subnet mask's
fixed length check, and a seed a control above names by filename stays
even where ``offer`` or ``ack`` repeats its condition in a larger
message, as ``plain_callback_domain_name`` does.
