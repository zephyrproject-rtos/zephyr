 ...
 *
 * Each case is injected twice, once with the client placed in SELECTING
 * and once in REQUESTING - the two states its OFFER and ACK handlers act
 * in. Left where net_dhcpv4_start() puts it the client sits in INIT,
 * where both handlers fall out of their state switch and nothing past
 * the option parser is reached.
 *
 ...
 * The larger value is headroom for a mutated case.
 */
...
/* Stands in for the address an OFFER would have put in requested_ip, for
 * the run that starts in REQUESTING. Same RFC 5737 documentation range as
 * the server address above.
 */
static const struct net_in_addr offered_ip = { { { 192, 0, 2, 2 } } };
...
/* Neither offer's nor ack's malformed vendor option claims this inner
 * code, so reaching this handler means the well-formed path is being
 * exercised.
 */
		    ...
		    enum net_dhcpv4_state state)
{
	...
	/* net_dhcpv4_start() leaves the client in INIT and defers SELECTING
	 * to a work item armed CONFIG_NET_DHCPV4_INITIAL_DELAY_MAX seconds
	 * out at most, far beyond the window a case runs in. Both
	 * dhcpv4_handle_msg_offer() and dhcpv4_handle_msg_ack() fall straight
	 * out of their state switch while the state is INIT, so a case
	 * injected there never reaches the code that reads the offered
	 * address and the server id out of the message. Put the client in the
	 * state the message under test is answered in.
	 */
	iface->config.dhcpv4.state = state;

	/* REQUESTING is only ever entered from an OFFER, which is what fills
	 * requested_ip in. An ACK handled with it still zero stops at
	 * net_if_ipv4_addr_add() instead of reaching dhcpv4_enter_bound().
	 */
	if (state == NET_DHCPV4_REQUESTING) {
		iface->config.dhcpv4.requested_ip = offered_ip;
	}
...
int main(void)
{
	...
	while (true) {
		/* Not K_FOREVER: net_dhcpv4_stop() disarms the client's own
		 * work item at the end of a case, so nothing is left armed
		 * and an idle CPU asks the tick driver for its maximum. The
		 * native simulator runs whole events rather than bounded
		 * time and takes that timeout in one step, so uptime gains
		 * INT32_MAX ticks per case and the client decodes lease
		 * times against a clock no device reaches. Waiting for the
		 * window LLVMFuzzerTestOneInput() gives the simulator keeps
		 * the next event inside it.
		 */
		if (k_sem_take(&fuzz_sem, K_TICKS(FUZZ_TICKS)) != 0) {
			continue;
		}

		/* Run the case handed off via interrupt, once in each of the
		 * two states the client answers a server message in: SELECTING,
		 * where an OFFER is handled, and REQUESTING, where an ACK is.
		 * One state alone leaves the other handler unreached for every
		 * case.
		 */
		run_one(iface, fuzz_buf, fuzz_sz, NET_DHCPV4_SELECTING);
		run_one(iface, fuzz_buf, fuzz_sz, NET_DHCPV4_REQUESTING);
	}
