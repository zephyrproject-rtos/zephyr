...
/* ---------------------------------------------------------------------------
 * Fuzz case processing
 *
 * Executed on main()'s thread, having been handed off from fuzz_isr().
 *
...
static void fuzz_one(const uint8_t *data, size_t sz)
{
...
static void fuzz_isr(const void *arg)
{
	...
	/* Do NOT process the input here: the MQTT library takes a k_mutex and
	 * k_mutex_lock() asserts !arch_is_in_isr() (kernel/mutex.c:152).
	 * Hand off to main()'s thread instead, as the upstream fuzz sample
	 * does.
	 */
	k_sem_give(&fuzz_sem);
}
...
int main(void)
{
	...
	while (true) {
		...
		fuzz_one(g_fuzz_data, g_fuzz_sz);
	}
}
 ...
 *
 * The mutation dictionary is tests/net/lib/mqtt_fuzz/mqtt5.dict; the seed
 * corpus is external state passed in via $MQTT_FUZZ_CORPUS (see README.rst).
 * ---------------------------------------------------------------------------
 */
#if defined(CONFIG_BOARD_NATIVE_SIM)
