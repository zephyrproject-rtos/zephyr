...
{
	zassert_true(test_gauge_m.value == 0, "Gauge value is not 0");

	prometheus_gauge_set(&test_gauge_m, 1);

	zassert_true(test_gauge_m.value == 1, "Gauge value is not 1");

	prometheus_gauge_set(&test_gauge_m, 2);

	zassert_true(test_gauge_m.value == 2, "Gauge value is not 2");
}
