...
ZTEST(test_collector, test_prometheus_collector_register)
{
	...
	zassert_true(test_counter_m.value == 0, "Counter value is not 0");

	ret = prometheus_counter_inc(counter);
	zassert_true(ret == 0, "Error incrementing counter");

	zassert_true(counter->value == 1, "Counter value is not 1");
}
