...
ZTEST(test_counter, test_prometheus_counter_inc)
{
	...
	prometheus_counter_inc(&test_counter_m);

	zassert_true(test_counter_m.value == 1, "Counter value is not 1");

	prometheus_counter_inc(&test_counter_m);

	zassert_true(test_counter_m.value == 2, "Counter value is not 2");
}
