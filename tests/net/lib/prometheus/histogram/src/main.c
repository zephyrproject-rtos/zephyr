...
ZTEST(test_histogram, test_histogram_observe)
{
	...
	zassert_true(ret == 0, "Error observing histogram");

	zassert_true(test_histogram_m.sum == 1.0, "Histogram value is not 1");

	ret = prometheus_histogram_observe(&test_histogram_m, 2);
	zassert_true(ret == 0, "Error observing histogram");

	zassert_true(test_histogram_m.sum == 3.0, "Histogram value is not 2");
}
