...
{
	zassert_true(test_summary_m.sum == 0, "Histogram value is not 0");

	prometheus_summary_observe(&test_summary_m, 1);

	zassert_true(test_summary_m.sum == 1, "Histogram value is not 1");

	prometheus_summary_observe(&test_summary_m, 2);

	zassert_true(test_summary_m.sum == 2, "Histogram value is not 2");
}
