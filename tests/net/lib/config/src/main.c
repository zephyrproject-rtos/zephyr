...
static struct net_if *iface5;
static struct net_if *iface6;
static struct net_if *iface7;
static struct net_if *iface8;
static struct net_if *iface9;
static struct net_if *iface10;
...
struct net_if_test net_eth_iface3_data;
struct net_if_test net_eth_iface4_data;
struct net_if_test net_eth_iface5_data;
struct net_if_test net_dummy_iface6_data;
struct net_if_test net_dummy_iface7_data;
struct net_if_test net_dummy_iface8_data;
struct virtual_test_context virtual_test_iface9_data;
struct virtual_test_context virtual_test_iface10_data;
...
ETH_NET_DEVICE_INIT(eth_3, "eth_3", eth_dev_init, NULL,
		    &net_eth_iface3_data, NULL, CONFIG_ETH_INIT_PRIORITY,
		    &eth_api_funcs, NET_ETH_MTU);

ETH_NET_DEVICE_INIT(eth_4, "eth_4", eth_dev_init, NULL,
		    &net_eth_iface4_data, NULL, CONFIG_ETH_INIT_PRIORITY,
		    &eth_api_funcs, NET_ETH_MTU);

ETH_NET_DEVICE_INIT(eth_5, "eth_5", eth_dev_init, NULL,
		    &net_eth_iface5_data, NULL, CONFIG_ETH_INIT_PRIORITY,
		    &eth_api_funcs, NET_ETH_MTU);
...
NET_DEVICE_INIT_INSTANCE(net_iface8_test, "dummy_8", iface8, NULL, NULL,
			 &net_dummy_iface8_data, NULL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,
			 &dummy_iface_api, DUMMY_L2, DUMMY_L2_CTX_TYPE, 127);

NET_VIRTUAL_INTERFACE_INIT(virtual_iface9_test, "virtual_9", NULL, NULL,
			   &virtual_test_iface9_data, NULL,
			   CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,
			   &virtual_test_iface_api, VIRTUAL_TEST_MTU);

NET_VIRTUAL_INTERFACE_INIT(virtual_iface10_test, "virtual_10", NULL, NULL,
			   &virtual_test_iface10_data, NULL,
			   CONFIG_KERNEL_INIT_PRIORITY_DEFAULT,
			   &virtual_test_iface_api, VIRTUAL_TEST_MTU);
...
static void *iface_setup(void)
{
	...
	((struct net_if_test *)net_if_get_device(iface4)->data)->idx = idx;

	idx = net_if_get_by_iface(iface5);
	((struct net_if_test *)net_if_get_device(iface5)->data)->idx = idx;

	idx = net_if_get_by_iface(iface6);
	((struct net_if_test *)net_if_get_device(iface6)->data)->idx = idx;

	idx = net_if_get_by_iface(iface7);
	((struct net_if_test *)net_if_get_device(iface7)->data)->idx = idx;

	idx = net_if_get_by_iface(iface8);
	((struct net_if_test *)net_if_get_device(iface8)->data)->idx = idx;

	DBG("Ethernet interfaces:\n"
	    "\t[%d] iface1 %p, [%d] iface2 %p, [%d] iface3 %p, "
	    "[%d] iface4 %p, [%d] iface5 %p\n",
...
#define EXPECTED_ETH_IFACE_COUNT (5 * CONFIG_NET_VLAN_COUNT)
	zassert_equal(eth_if_count, EXPECTED_ETH_IFACE_COUNT,
		      "Invalid number of Ethernet interface found, expected %d got %d",
		      EXPECTED_ETH_IFACE_COUNT, eth_if_count);

	zassert_not_null(iface1, "Ethernet interface 1");
	zassert_not_null(iface2, "Ethernet interface 2");
	zassert_not_null(iface3, "Ethernet interface 3");
	zassert_not_null(iface4, "Ethernet interface 4");
	zassert_not_null(iface5, "Ethernet interface 5");

	DBG("Dummy interfaces:\n"
	    "\t[%d] iface6 %p, [%d] iface7 %p, [%d] iface8 %p\n",
	    net_if_get_by_iface(iface6), iface6,
	    net_if_get_by_iface(iface7), iface7,
	    net_if_get_by_iface(iface8), iface8);

	zassert_equal(dummy_if_count, 3,
		      "Invalid number of Dummy interface found, expected %d got %d",
		      3, eth_if_count);

	zassert_not_null(iface6, "Dummy interface 6");
	zassert_not_null(iface7, "Dummy interface 7");
	zassert_not_null(iface8, "Dummy interface 8");

	DBG("Virtual interfaces:\n"
	    "\t[%d] iface9 %p, [%d] iface10 %p\n",
	    net_if_get_by_iface(iface9), iface9,
	    net_if_get_by_iface(iface10), iface10);
...
static int get_ifindex(const struct net_init_config_iface *cfg)
{
	int ifindex = 0;
...
ZTEST(net_config, test_interface_names)
{
	...
	zassert_true(NET_CONFIG_IFACE_COUNT > 0);

	for (int i = 0; i < NET_CONFIG_IFACE_COUNT; i++) {
		ifindex = get_ifindex(&config->iface[i]);
	}
}

#if defined(CONFIG_NET_IPV4)
static void check_ipv4(const struct net_init_config_iface *cfg)
{
	struct net_if *iface;
	struct in_addr addr;
	int ifindex;
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv4.address); i++) {
		struct net_if_addr *ifaddr;
		struct net_if *iface_addr;

		if (net_ipv4_is_addr_unspecified(&cfg->ipv4.address[i])) {
			continue;
		}

		ifaddr = net_if_ipv4_addr_lookup(&cfg->ipv4.address[i],
						 &iface_addr);
		zassert_not_null(ifaddr, "Address %s not found.",
				 net_sprint_ipv4_addr(&cfg->ipv4.address[i]));
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv4.mcast_address); i++) {
		struct net_if_mcast_addr *ifmaddr;
		struct net_if *iface_addr;

		if (net_ipv4_is_addr_unspecified(&cfg->ipv4.mcast_address[i])) {
			continue;
		}

		ifmaddr = net_if_ipv4_maddr_lookup(&cfg->ipv4.mcast_address[i],
						   &iface_addr);
		zassert_not_null(ifmaddr, "Multicast address %s not found.",
				 net_sprint_ipv4_addr(&cfg->ipv4.mcast_address[i]));
		zassert_equal(iface, iface_addr, "Invalid network interface. "
			      "Got %p (%d) expected %p (%d).",
			      iface_addr, net_if_get_by_iface(iface_addr),
			      iface, net_if_get_by_iface(iface));
	...
	if (!net_ipv4_is_addr_unspecified(&cfg->ipv4.gateway)) {
		zassert_mem_equal(&iface->config.ip.ipv4->gw,
				  &cfg->ipv4.gateway,
				  sizeof(struct in_addr),
				  "Mismatch gateway address. "
				  "Expecting %s got %s.",
				  net_sprint_ipv4_addr(&cfg->ipv4.gateway),
				  net_sprint_ipv4_addr(&iface->config.ip.ipv4->gw));
	}

	if (cfg->ipv4.netmask_len > 0) {
		struct in_addr gen_mask;

		addr = net_if_ipv4_get_netmask(iface);
		gen_mask.s_addr = BIT_MASK(cfg->ipv4.netmask_len);
	...
	if (cfg->ipv4.ttl > 0) {
		zassert_equal(net_if_ipv4_get_ttl(iface), cfg->ipv4.ttl,
			      "TTL mismatch, expecting %d got %d",
			      cfg->ipv4.ttl, net_if_ipv4_get_ttl(iface));
	}

	if (cfg->ipv4.mcast_ttl > 0) {
		zassert_equal(net_if_ipv4_get_mcast_ttl(iface), cfg->ipv4.mcast_ttl,
			      "Multicast TTL mismatch, expecting %d got %d",
			      cfg->ipv4.mcast_ttl, net_if_ipv4_get_mcast_ttl(iface));
	}

	if (cfg->ipv4.is_dhcpv4) {
#if defined(CONFIG_NET_DHCPV4)
		zassert_equal(iface->config.dhcpv4.state,
			      NET_DHCPV4_INIT,
			      "DHCPv4 not in correct state, expecting '%s' got '%s'",
			      net_dhcpv4_state_name(NET_DHCPV4_INIT),
			      net_dhcpv4_state_name(iface->config.dhcpv4.state));
#endif
	}

	if (cfg->ipv4.is_ipv4_autoconf) {
...
#else
static inline void check_ipv4(const struct net_init_config_iface *cfg)
{
}
#endif

#if defined(CONFIG_NET_IPV6)
static void check_ipv6(const struct net_init_config_iface *cfg)
{
	struct net_if *iface;
	int ifindex;
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv6.address); i++) {
		struct net_if_addr *ifaddr;
		struct net_if *iface_addr;

		if (net_ipv6_is_addr_unspecified(&cfg->ipv6.address[i])) {
			continue;
		}

		ifaddr = net_if_ipv6_addr_lookup(&cfg->ipv6.address[i],
						 &iface_addr);
		zassert_not_null(ifaddr, "Address %s not found.",
				 net_sprint_ipv6_addr(&cfg->ipv6.address[i]));
		zassert_equal(iface, iface_addr, "Invalid network interface. "
			      "Got %p (%d) expected %p (%d).",
			      iface_addr, net_if_get_by_iface(iface_addr),
			      iface, net_if_get_by_iface(iface));
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv6.mcast_address); i++) {
		struct net_if_mcast_addr *ifmaddr;
		struct net_if *iface_addr;

		if (net_ipv6_is_addr_unspecified(&cfg->ipv6.mcast_address[i])) {
			continue;
		}

		ifmaddr = net_if_ipv6_maddr_lookup(&cfg->ipv6.mcast_address[i],
						   &iface_addr);
		zassert_not_null(ifmaddr, "Multicast address %s not found.",
				 net_sprint_ipv6_addr(&cfg->ipv6.mcast_address[i]));
		zassert_equal(iface, iface_addr, "Invalid network interface. "
			      "Got %p (%d) expected %p (%d).",
			      iface_addr, net_if_get_by_iface(iface_addr),
			      iface, net_if_get_by_iface(iface));
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv6.prefix); i++) {
		struct net_if_ipv6_prefix *prefix;

		if (net_ipv6_is_addr_unspecified(&cfg->ipv6.prefix[i].address)) {
			continue;
		}

		prefix = net_if_ipv6_prefix_lookup(iface,
						   (struct in6_addr *)&cfg->ipv6.prefix[i].address,
						   cfg->ipv6.prefix[i].len);
		zassert_not_null(prefix, "Prefix %s/%d not found.",
				 net_sprint_ipv6_addr(&cfg->ipv6.prefix[i].address),
				 cfg->ipv6.prefix[i].len);
		zassert_equal(prefix->len, cfg->ipv6.prefix[i].len,
			      "Prefix len differs, expected %u got %u",
			      cfg->ipv6.prefix[i].len,
			      prefix->len);
		if (cfg->ipv6.prefix[i].lifetime == 0xffffffff) {
	...
	if (cfg->ipv6.mcast_hop_limit > 0) {
		zassert_equal(net_if_ipv6_get_mcast_hop_limit(iface),
			      cfg->ipv6.mcast_hop_limit,
			      "Multicast hop limit mismatch, expecting %d got %d",
			      cfg->ipv6.mcast_hop_limit,
			      net_if_ipv6_get_mcast_hop_limit(iface));
	}

	if (cfg->ipv6.dhcpv6.is_enabled) {
#if defined(CONFIG_NET_DHCPV6)
		zassert_equal(iface->config.dhcpv6.state,
			      NET_DHCPV6_INIT,
			      "DHCPv6 not in correct state, expecting '%s' got '%s'",
...
#define IF_ENABLED_IPV4(cfg) COND_CODE_1(CONFIG_NET_IPV4, \
					 ((cfg)->ipv4.is_enabled), (false))

		if (IF_ENABLED_IPV4(cfg)) {
			check_ipv4(cfg);
		}
	}
}
...
#define IF_ENABLED_IPV6(cfg) COND_CODE_1(CONFIG_NET_IPV6, \
					 ((cfg)->ipv6.is_enabled), (false))

		if (IF_ENABLED_IPV6(cfg)) {
			check_ipv6(cfg);
		}
	}
}
