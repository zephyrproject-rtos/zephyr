...
static void check_ipv4(const struct net_init_config_iface *cfg)
{
	...
	for (int i = 0; i < ARRAY_SIZE(cfg->ipv4.address); i++) {
		...
		if (net_ipv4_is_addr_unspecified(&cfg->ipv4.address[i].ipv4)) {
			continue;
		}

		ifaddr = net_if_ipv4_addr_lookup(&cfg->ipv4.address[i].ipv4,
						 &iface_addr);
		zassert_not_null(ifaddr, "Address %s not found.",
				 net_sprint_ipv4_addr(&cfg->ipv4.address[i].ipv4));
		...
		if (cfg->ipv4.address[i].netmask_len > 0) {
			struct in_addr gen_mask;

			addr = net_if_ipv4_get_netmask_by_addr(iface,
							       &cfg->ipv4.address[i].ipv4);
			gen_mask.s_addr = BIT_MASK(cfg->ipv4.address[i].netmask_len);
