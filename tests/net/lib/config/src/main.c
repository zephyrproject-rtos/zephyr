...
static uint8_t *net_iface_get_mac(const struct device *dev)
{
	...
	memcpy(data->ll_addr.addr, data->mac_addr, data->ll_addr.len);
...
static int virtual_test_interface_start(const struct device *dev)
{
	...
	DBG("Starting iface %d", net_if_get_by_iface(ctx->iface));
...
static int virtual_test_interface_stop(const struct device *dev)
{
	...
	DBG("Stopping iface %d", net_if_get_by_iface(ctx->iface));
...
static void iface_cb(struct net_if *iface, void *user_data)
{
	...
	DBG("Interface %p %s (%s) [%d]\n", iface, name, iface2str(iface),
...
static int get_ifindex(const struct net_cfg_interfaces *cfg)
{
	int ifindex = -1;
...
ZTEST(net_config, test_interface_00_names)
{
	...
	for (int i = 0; i < NET_CONFIG_NETWORK_INTERFACE_COUNT; i++) {
		ifindex = get_ifindex(&config->interfaces[i]);
	}
}
...
static void check_ipv4(const struct net_cfg_interfaces *cfg)
{
	struct net_if *iface;
	struct in_addr addr;
	int ifindex;
	...
	ARRAY_FOR_EACH(cfg->ipv4.ipv4_addresses, i) {
		...
		if (cfg->ipv4.ipv4_addresses[i].value[0] == '\0') {
			continue;
		}

		zassert_true(parse_mask(cfg->ipv4.ipv4_addresses[i].value,
	...
	ARRAY_FOR_EACH(cfg->ipv4.ipv4_multicast_addresses, i) {
		...
		if (cfg->ipv4.ipv4_multicast_addresses[i].value[0] == '\0') {
			continue;
		}

		zassert_true(parse_mask(cfg->ipv4.ipv4_multicast_addresses[i].value,
					strlen(cfg->ipv4.ipv4_multicast_addresses[i].value),
	...
	if (cfg->ipv4.ipv4_autoconf.status) {
...
#else
static inline void check_ipv4(const struct networking *cfg, int index)
{
	ARG_UNUSED(cfg);
	ARG_UNUSED(index);
}
...
static void check_ipv6(const struct net_cfg_interfaces *cfg)
{
	struct net_if *iface;
	int ifindex;
	...
	ARRAY_FOR_EACH(cfg->ipv6.ipv6_addresses, i) {
		...
		if (cfg->ipv6.ipv6_addresses[i].value[0] == '\0') {
			continue;
		}

		zassert_true(parse_mask(cfg->ipv6.ipv6_addresses[i].value,
	...
	ARRAY_FOR_EACH(cfg->ipv6.ipv6_multicast_addresses, i) {
		...
		if (cfg->ipv6.ipv6_multicast_addresses[i].value[0] == '\0') {
			continue;
		}

		zassert_true(parse_mask(cfg->ipv6.ipv6_multicast_addresses[i].value,
	...
	ARRAY_FOR_EACH(cfg->ipv6.prefixes, i) {
		...
		if (cfg->ipv6.prefixes[i].address[0] == '\0') {
			continue;
		}

		zassert_true(parse_mask(cfg->ipv6.prefixes[i].address,
...
#else
static inline void check_ipv6(const struct net_cfg_interfaces *cfg)
{
}
#endif

ZTEST(net_config, test_interface_01_ipv4_addresses)
{
	const struct networking *config;
	int ifindex = 0;
...
#define IF_ENABLED_IPV4(cfg, i)						\
		COND_CODE_1(CONFIG_NET_IPV4,				\
			    ((cfg)->interfaces[i].ipv4.status), \
			    (false))

		if (IF_ENABLED_IPV4(config, i)) {
			check_ipv4(&config->interfaces[i]);
		}
	}
}

ZTEST(net_config, test_interface_02_ipv6_addresses)
{
	const struct networking *config;
	int ifindex = 0;
...
#define IF_ENABLED_IPV6(cfg, i)						\
		COND_CODE_1(CONFIG_NET_IPV6,				\
			    ((cfg)->interfaces[i].ipv6.status),	\
			    (false))

		if (IF_ENABLED_IPV6(config, i)) {
			check_ipv6(&config->interfaces[i]);
		}
	}
}

ZTEST(net_config, test_interface_03_vlan)
{
	const struct networking *config;
	int ifindex = 0;
	int vlan_count = 0;
	...
	ARRAY_FOR_EACH(config->interfaces, i) {
		...
		const struct net_cfg_vlan *vlan;
		struct net_if *iface;
		uint16_t tag;
...
ZTEST(net_config, test_interface_04_flags)
{
	const struct networking *config;
	int ifindex = 0;
