...
int parse_result(const char *addrstr, struct arithmetic_result *res)
{
	if (sscanf(addrstr, "[{\"x\":%d,\"y\":%d,\"result\":%d}]", &res->x, &res->y,
		   &res->result) == 3) {
		return 0;
	}
	return -1;
...
static void test_handle_post_request(void)
{
	int r;
	int client_fd;
	...
	struct arithmetic_result expected = {.x = 10, .y = 20, .result = 30};

	struct arithmetic_result result;

	memset(addrstr, 0, sizeof(addrstr));
	r = recv(client_fd, addrstr, sizeof(addrstr), 0);
	zassert_not_equal(r, -1, "recv() failed (%d)", errno);

	zassert_true(parse_result(addrstr, &result) == 0,
		     "Failed to parse the struct from the JSON string");
	zassert_equal(result.x, expected.x, "Expected x to be 10, but got %d", result.x);
	zassert_equal(result.y, expected.y, "Expected y to be 20, but got %d", result.y);
	zassert_equal(result.result, expected.result, "Expected result to be 30, but got %d",
		      result.result);
...
ZTEST(framework_tests_rest, test_handle_post_request)
{
	test_handle_post_request();
}

ZTEST_SUITE(framework_tests_rest, NULL, NULL, NULL, NULL, NULL);
