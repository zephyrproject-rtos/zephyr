...
static struct net_pkt *create_orig_pkt(const struct in6_addr *src,
					const struct in6_addr *dst)
{
	struct net_pkt *pkt;

	pkt = net_pkt_alloc_with_buffer(test_iface, sizeof(struct net_ipv6_hdr),
					AF_INET6, TEST_IPPROTO_NONE,
					K_SECONDS(1));
...
static struct net_pkt *create_orig_icmpv6_pkt(const struct in6_addr *src,
					      const struct in6_addr *dst,
					      uint8_t icmpv6_type)
{
	struct net_pkt *pkt;
	...
	net_pkt_cursor_init(pkt);
	zassert_ok(net_ipv6_finalize(pkt, IPPROTO_ICMPV6), "Cannot finalize IPv6 pkt");
...
ZTEST(icmpv6_send_error_fn, test_send_error_rfc4443_matrix)
{
	for (size_t i = 0; i < ARRAY_SIZE(send_error_cases); i++) {
		...
		zassert_ok(net_addr_pton(AF_INET6, tc->src, &src),
			   "%s: bad src address", tc->name);
		zassert_ok(net_addr_pton(AF_INET6, tc->dst, &dst),
...
ZTEST(icmpv6_send_error_fn, test_send_error_icmpv6_orig)
{
	...
	zassert_ok(net_addr_pton(AF_INET6, "2001:db8::1", &src), "bad src address");
	zassert_ok(net_addr_pton(AF_INET6, "2001:db8::2", &dst), "bad dst address");
