		       ...
		       size_t *out_len)
{
	...
	return srtp_stream_protect(stream, buf, sizeof(kat_rtp), buf_size, out_len);
}
...
ZTEST_P(srtp_tests, test_protect_kat)
{
	...
	memcpy(buf, kat_rtp, sizeof(kat_rtp));
	zassert_ok(srtp_stream_protect(&tx_stream, buf, sizeof(kat_rtp), sizeof(buf), &out_len));
...
ZTEST_P(srtp_tests, test_unprotect_kat)
{
	...
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, tc->srtp_len, &out_len));
...
ZTEST_P(srtp_tests, test_mki)
{
	...
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, out_len, &out_len));
	zassert_equal(out_len, sizeof(kat_rtp), "%s", tc->name);
	zassert_mem_equal(buf, kat_rtp, out_len, "%s", tc->name);
}
...
ZTEST(srtp_tests, test_mki_mismatch)
{
	...
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, pkt_len, &out_len), -ENOENT);

	/* A packet without the MKI is malformed for this stream */
	memcpy(buf, pkt, pkt_len);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, pkt_len - sizeof(kat_mki), &out_len),
		      -ENOENT);
	 ...
	 */
	memcpy(buf, pkt, pkt_len);
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, pkt_len, &out_len));
...
ZTEST(srtp_tests, test_roc_wrap)
{
	...
	zassert_ok(srtp_stream_protect(&tx_stream, buf, sizeof(kat_rtp_roc1), sizeof(buf),
				       &out_len));
...
ZTEST(srtp_tests, test_replay)
{
	...
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, len12, &out_len));
	memcpy(buf, pkt10, len10);
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, len10, &out_len));

	/* Replays are rejected */
	memcpy(buf, pkt10, len10);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, len10, &out_len), -EALREADY);
	memcpy(buf, pkt12, len12);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, len12, &out_len), -EALREADY);

	/* Still in order for the unseen packet */
	memcpy(buf, pkt11, len11);
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, len11, &out_len));
	memcpy(buf, pkt11, len11);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, len11, &out_len), -EALREADY);
}
...
ZTEST(srtp_tests, test_replay_too_old)
{
	...
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, len_new, &out_len));

	/* The old packet now falls outside the replay window */
	memcpy(buf, pkt_old, len_old);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, len_old, &out_len), -EALREADY);
}
...
ZTEST_P(srtp_tests, test_tamper)
{
	...
	memcpy(buf, pkt, pkt_len);
	buf[4] ^= 0x01;
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, pkt_len, &out_len), -EBADMSG, "%s",
		      tc->name);
	...
	/* Truncated packet */
	memcpy(buf, pkt, pkt_len);
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, RTP_MIN_HEADER_LEN + 4, &out_len),
		      -EBADMSG, "%s", tc->name);
...
ZTEST(srtp_tests, test_wrong_key)
{
	...
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, sizeof(kat_srtp_cm_sha1_80), &out_len),
		      -EBADMSG);
}
...
ZTEST(srtp_tests, test_key_expiry)
{
	...
	memcpy(buf, kat_rtp, sizeof(kat_rtp));
	zassert_equal(srtp_stream_protect(&tx_stream, buf, sizeof(kat_rtp), sizeof(buf), &out_len),
		      -EKEYEXPIRED);
...
ZTEST(srtp_tests, test_kdr_rederivation)
{
	...
	zassert_ok(srtp_stream_unprotect(&rx_stream, pkt_a, len_a, &out_len));
	zassert_mem_equal(pkt_a + 12, kat_rtp + 12, sizeof(kat_rtp) - 12);
	zassert_ok(srtp_stream_unprotect(&rx_stream, pkt_b, len_b, &out_len));
...
ZTEST_P(srtp_tests, test_short_payload)
{
	...
	zassert_ok(srtp_stream_protect(&tx_stream, buf, pkt_len, sizeof(buf), &out_len));
	zassert_ok(srtp_stream_unprotect(&rx_stream, buf, out_len, &out_len));
...
ZTEST(srtp_tests, test_kdr_forged_packet_keeps_keys)
{
	...
	buf[2] = 0xf0;
	buf[3] = 0x00;
	zassert_equal(srtp_stream_unprotect(&rx_stream, buf, pkt_len, &out_len), -EBADMSG);
...
ZTEST(srtp_tests, test_no_tag_room)
{
	...
	memcpy(buf, kat_rtp, sizeof(kat_rtp));
	zassert_equal(srtp_stream_protect(&tx_stream, buf, sizeof(kat_rtp), sizeof(buf) - 6,
					  &out_len),
		      -ENOMEM);
	...
	memcpy(buf, kat_rtp, sizeof(kat_rtp));
	zassert_equal(srtp_stream_protect(&rx_stream, buf, sizeof(kat_rtp), sizeof(buf), &out_len),
		      -ENOMEM);
}
...
ZTEST(srtp_tests, test_deinit_destroys_keys)
{
	...
	memcpy(buf, kat_rtp, sizeof(kat_rtp));
	zassert_true(srtp_stream_protect(&tx_stream, buf, sizeof(kat_rtp), sizeof(buf),
					 &out_len) < 0,
...
ZTEST(srtp_tests, test_session_set_srtp)
{
	...
	/* Re-installation after a clear requires a fresh master key: the new
	 * transmit stream restarts its packet index (RFC 3711 3.3.1 and 9.1).
	 */
	zassert_ok(rtp_session_set_srtp(&session, &policy_gcm_256, NULL, &ctx));
	zassert_ok(rtp_session_clear_srtp(&session));
}
...
ZTEST(srtp_tests, test_session_initial_sequence_number)
{
	static struct rtp_session session;
	static struct srtp_session_ctx ctx;
	...
	k_mutex_init(&session.lock);
	session.ssrc = KAT_SSRC;
	session.rtp_context.role = RTP_ROLE_SOURCE;

	zassert_ok(rtp_session_set_srtp(&session, &policy_cm_sha1_80, NULL, &ctx));

	/* The initial sequence number is assigned before the unset address
	 * family fails the start; without SRTP it spans the full 16-bit
	 * range, with SRTP it must stay in the lower half so receivers
	 * bootstrap the rollover counter correctly (RFC 3711 section 8).
	 */
	for (int i = 0; i < 16; i++) {
		zassert_equal(rtp_session_start(&session), -ENOTSUP);
		zassert_true(session.sequence_number < 0x8000U,
			     "Initial sequence number %u in the upper half",
