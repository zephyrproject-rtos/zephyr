...
static struct net_stats_eth *eth_get_stats(const struct device *dev)
{
	ARG_UNUSED(dev);
...
ZTEST(net_virtual, test_virtual_11_recv_data_on_non_virtual_iface)
{
	...
	pkt = net_pkt_alloc_with_buffer(iface, sizeof(*frame), AF_UNSPEC, 0, PKT_ALLOC_TIME);
	...
	frame->hdr.vlan.tpid = htons(NET_ETH_PTYPE_VLAN);
	frame->hdr.vlan.tci = net_eth_vlan_set_vid(0, vid);
	frame->hdr.type = htons(NET_ETH_PTYPE_OAM);
