...
/* The public entry point flushes the same entries as the link-down path, and
 * a NULL interface reaches every interface.
 */
ZTEST(net_ipv6_nbr_cache_clear, test_public_flush)
{
	struct net_linkaddr lladdr = {
		.type = NET_LINK_DUMMY,
		.len = 6,
		.addr = {0x00, 0x00, 0x5e, 0x00, 0x53, 0xbb},
	};
	struct net_if_addr *ifaddr;
	struct net_nbr *nbr;
	int ret;
	...
	net_if_ipv6_nbr_flush(iface);

	zexpect_is_null(net_ipv6_nbr_lookup(iface, &peer_addr),
			"Dynamic neighbor was not flushed");
	zexpect_not_null(net_ipv6_nbr_lookup(iface, &static_addr),
			 "Static neighbor must survive a flush");

	/* Learn it again and flush without naming the interface. */
	nbr = net_ipv6_nbr_add(iface, &peer_addr, &lladdr, false, NET_IPV6_NBR_STATE_STALE);
	zassert_not_null(nbr, "Cannot add dynamic neighbor");
	...
	zexpect_is_null(net_ipv6_nbr_lookup(iface, &peer_addr),
			"Dynamic neighbor was not flushed on every interface");
	zexpect_not_null(net_ipv6_nbr_lookup(iface, &static_addr),
			 "Static neighbor must survive a flush");
}
