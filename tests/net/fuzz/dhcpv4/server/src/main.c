 ...
 * gives a mutated case headroom. The window holds one timer event
 * whatever its length, so the size costs no throughput.
 */
...
int main(void)
{
	...
	while (true) {
		/* Not K_FOREVER: nothing stays armed between cases, so
		 * an idle CPU hands the tick driver SYS_CLOCK_MAX_WAIT,
		 * the one-shot tick timer schedules a single event
		 * there, and the simulator runs whole events, so it
		 * reaches that one in a single step. Measured on
		 * native_sim/native/64: bounded, a case advances uptime
		 * by 1010 ms; with K_FOREVER most cases advance it by
		 * 21474836470 ms, leaving the server to age its own
		 * leases against a clock no device reaches.
		 */
		if (k_sem_take(&fuzz_sem, K_TICKS(FUZZ_TICKS)) != 0) {
			continue;
		}
