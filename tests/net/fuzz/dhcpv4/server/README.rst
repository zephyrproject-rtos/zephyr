...
seed does, and it costs nothing to raise: the window holds one timer
event whatever its length.
...
.. code-block:: console
   ...
   python3 tests/net/fuzz/common/gen_corpus.py \
       tests/net/fuzz/dhcpv4/server/seeds.txt /tmp/dhcpv4-server-corpus
   export ZEPHYR_TOOLCHAIN_VARIANT=host/llvm
   west build -p -b native_sim/native/64 tests/net/fuzz/dhcpv4/server
...
as hex in :file:`seeds.txt` and the shared
:file:`../../common/gen_corpus.py` writes the directory of binary seeds
libFuzzer wants. Each seed is the DHCP message body, which is what the
harness appends after the IPv4 and UDP headers.
...
``-merge=1``) before its hex is added. Naming a condition and adding
coverage are not the same test, and two seeds here stay on the first:
``discover`` is the plain message the other Discovers are built out of,
so it reaches nothing ``client_id`` and ``hostname`` do not, and the
unrecognised option ``hostname`` carries is scanned past by every seed
that lacks options 50, 54 and 61 while the parser probes for them. Both
name a condition the table above gives a filename, which is what a
reader looks the corpus up by.
