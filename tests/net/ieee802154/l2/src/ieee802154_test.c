...
{
	static uint8_t broadcast_addr[IEEE802154_SHORT_ADDR_LENGTH] = {0xff, 0xff};
	struct ieee802154_mpdu mpdu;
	struct net_pkt *pkt;
	size_t offset = 0;
	bool result = false;
	...
	net_pkt_lladdr_dst(pkt)->addr = broadcast_addr;
	net_pkt_lladdr_dst(pkt)->len = sizeof(broadcast_addr);
