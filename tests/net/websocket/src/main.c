		    ...
		    void *user_data)
{
	...
	/* The pkt will contain websocket header because we are bypassing
	 * any websocket message parsing here. So we need to skip the websocket
	 * header here. The return header length is 2 bytes here only if
	 * the returned message < 127 bytes long.
	 */
...
void test_v6_connect(void)
{
	...
	ret = net_app_connect(&app_ctx_v6, K_FOREVER);
	zassert_equal(ret, 0, "websocket IPv6 client connect");
}
...
void test_v6_close(void)
{
	...
	ret = net_app_close(&app_ctx_v6);
	zassert_equal(ret, 0, "websocket IPv6 client close");
}

/* The chunk_size tells how many bytes at a time to send.
 * This is not the same as HTTP chunk!
 */
void test_v6_send_recv(int chunk_size)
{
	...
	for (i = 0; i < sizeof(ws_test_msg); i += chunk_size) {
		...
		ret = net_app_send_buf(&app_ctx_v6, &ws_test_msg[i],
				       chunk_size,
				       &server_addr,
				       sizeof(struct sockaddr_in6),
				       timeout,
...
void test_v6_send_recv_1(void)
{
	...
	zassert_false(failure, "Send test 1 failed");
}
...
void test_v6_send_multi_msg(void)
{
	...
	for (i = 0; i < sizeof(ws_big_msg); i += chunk_size) {
		...
		ret = net_app_send_buf(&app_ctx_v6, &ws_big_msg[i],
				       chunk_size,
				       &server_addr,
				       sizeof(struct sockaddr_in6),
				       timeout,
