...
static struct in_addr reass_err_src_addr = { { { 0xc0, 0x00, 0x02, 0x01 } } };
static struct in_addr reass_err_dst_addr = { { { 0xc0, 0x00, 0x02, 0x02 } } };
						   ...
						   bool more)
{
	...
	pkt = net_pkt_rx_alloc_with_buffer(iface, total_len, AF_INET, IPPROTO_UDP,
					   K_NO_WAIT);
