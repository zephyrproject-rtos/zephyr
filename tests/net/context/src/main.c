...
static void net_ctx_connect_v6(void)
{
	...
	if (ret || cb_failure) {
		zassert_true(0, "Context connect IPv6 UDP test failed");
	}
	...
	ret = net_context_listen(tcp_v6_ctx, (struct sockaddr *)&addr,
				 connect_cb, 0, INT_TO_POINTER(AF_INET6));
	if (ret || cb_failure) {
		zassert_true(0, "Context connect IPv6 TCP test failed");
	}
#endif /* CONFIG_NET_TCP */
}
...
static void net_ctx_connect_v4(void)
{
	...
	if (ret || cb_failure) {
		zassert_true(0, "Context connect IPv4 UDP test failed");
	}
...
static void net_ctx_accept_v6(void)
{
	...
	if (ret != -EINVAL || cb_failure) {
		zassert_true(0, "Context accept IPv6 UDP test failed");
	}
}
...
static void net_ctx_accept_v4(void)
{
	...
	if (ret != -EINVAL || cb_failure) {
		zassert_true(0, "Context accept IPv4 UDP test failed");
	}
}
...
static void net_ctx_send_v6(void)
{
	...
	ret = net_context_send(pkt, send_cb, 0, INT_TO_POINTER(test_token),
			       INT_TO_POINTER(AF_INET6));
	if (ret || cb_failure) {
		zassert_true(0, "Context send IPv6 UDP test failed");
	}
}
...
static void net_ctx_send_v4(void)
{
	...
	ret = net_context_send(pkt, send_cb, 0, INT_TO_POINTER(test_token),
			       INT_TO_POINTER(AF_INET));
	if (ret || cb_failure) {
		zassert_true(0, "Context send IPv4 UDP test failed");
	}
}
...
static void net_ctx_sendto_v6(void)
{
	...
	ret = net_context_sendto(pkt, (struct sockaddr *)&addr,
				 ...
				 INT_TO_POINTER(AF_INET6));
	if (ret || cb_failure) {
		zassert_true(0, "Context sendto IPV6 UDP test failed");
	}
}
...
static void net_ctx_recv_v6(void)
{
	...
	ret = net_context_recv(udp_v6_ctx, recv_cb, 0,
			       INT_TO_POINTER(AF_INET6));
	if (ret || cb_failure) {
		zassert_true(0, "Context recv IPv6 UDP test failed");
	}
	...
	if (!recv_cb_called) {
		zassert_true(0, "No data received on time, "
...
static void net_ctx_recv_v4(void)
{
	...
	ret = net_context_recv(udp_v4_ctx, recv_cb, 0,
			       INT_TO_POINTER(AF_INET));
	if (ret || cb_failure) {
		zassert_true(0, "Context recv IPv4 UDP test failed");
	}
...
static void net_ctx_recv_v6_fail(void)
{
	...
	if (!k_sem_take(&wait_data, WAIT_TIME)) {
		zassert_true(0, "Semaphore triggered but should not");
	}

	if (recv_cb_called) {
		zassert_true(0, "Data received but should not have, "
			 "IPv6 recv test failed");
	}
