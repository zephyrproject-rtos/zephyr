...
uint8_t fifo_popped_data[DATA_LEN];
uint8_t lifo_popped_data[DATA_LEN];
uint8_t lifo_pushed_data[DATA_LEN];
uint8_t stack_popped_data[DATA_LEN];
uint8_t stack_pushed_data[DATA_LEN];
uint8_t ring_buffer_pushed_data[DATA_LEN];
uint8_t ring_buffer_popped_data[DATA_LEN];
...
void stack_pop(void)
{
	...
	for (int i = 0; i < DATA_LEN; i++) {
		...
		stack_popped_data[i] = (uint8_t)data;
		k_mutex_unlock(&data_mutex);
	}
}
...
void ring_buf_pop(void)
{
	for (int i = 0; i < DATA_LEN; i++) {
		...
		k_mutex_lock(&data_mutex, K_FOREVER);
		ring_buffer_popped_data[i] = data;
		k_mutex_unlock(&data_mutex);
	}
}
...
void data_summary(void)
{
	...
	log_print_data("\nPopped data from Fifo:\n", fifo_popped_data, DATA_LEN);
	fprintf(output_file, "\n-------------------------------------------------------------------"
			     "-------------");
	bool is_test_passed = compare_arrays(fifo_pushed_data, fifo_popped_data, DATA_LEN, false);
	...
	log_print_data("\nPopped data from Lifo.\n", lifo_popped_data, DATA_LEN);
	fprintf(output_file, "\n-------------------------------------------------------------------"
			     "-------------");
	is_test_passed = compare_arrays(lifo_pushed_data, lifo_popped_data, DATA_LEN, true);
	test_result_message =
		is_test_passed ? "\nLifo: " GREEN("PASSED") : "\nLifo: " RED("FAILED");
	fprintf(output_file, "%s", test_result_message);
	fprintf(output_file, "\n-------------------------------------------------------------------"
	...
	log_print_data("\nPopped data from Stack:\n", stack_popped_data, DATA_LEN);
	fprintf(output_file, "\n-------------------------------------------------------------------"
			     "-------------");
	is_test_passed = compare_arrays(stack_pushed_data, stack_popped_data, DATA_LEN, true);
	test_result_message =
		is_test_passed ? "\nStack: " GREEN("PASSED") : "\nStack: " RED("FAILED");
	fprintf(output_file, "%s", test_result_message);
	fprintf(output_file, "\n-------------------------------------------------------------------"
	...
	log_print_data("\nPopped data from Ring Buffer:\n", ring_buffer_popped_data, DATA_LEN);
	fprintf(output_file, "\n-------------------------------------------------------------------"
			     "-------------");
	is_test_passed =
		compare_arrays(ring_buffer_pushed_data, ring_buffer_popped_data, DATA_LEN, false);
