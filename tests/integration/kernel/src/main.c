...
/* Thread B gives the semaphore for Thread A to continue. */
/* Once the control is returned back to Thread A, it extracts all items from the liifo. */
/* Verify the data's correctness. */
...
ZTEST(kernel, test_mutex_usage)
{
	...
	/* Verify data after the end of Thread A */
	zassert_equal(mutex_data, NUMBER_OF_ITERATIONS);
	/* Wait for end of Thread B */
	k_thread_join(&tdata_2, K_FOREVER);
	/* Verify data after the end of Thread B */
	zassert_equal(mutex_data, 0);
