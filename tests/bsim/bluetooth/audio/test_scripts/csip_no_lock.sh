...
Execute ./bs_${BOARD_TS}_tests_bsim_bluetooth_audio_prj_conf \
  ...
  -RealEncryption=1 -rs=20 -D=4 -argstest rank 1 size 3 not-lockable

Execute ./bs_${BOARD_TS}_tests_bsim_bluetooth_audio_prj_conf \
  -v=${VERBOSITY_LEVEL} -s=${SIMULATION_ID} -d=2 -testid=csip_set_member \
  -RealEncryption=1 -rs=30 -D=4 -argstest rank 2 size 3 not-lockable

Execute ./bs_${BOARD_TS}_tests_bsim_bluetooth_audio_prj_conf \
  -v=${VERBOSITY_LEVEL} -s=${SIMULATION_ID} -d=3 -testid=csip_set_member \
  -RealEncryption=1 -rs=40 -D=4 -argstest rank 3 size 3 not-lockable
