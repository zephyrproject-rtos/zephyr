			   ...
			   struct net_buf *buf)
{
	...
	if (info->ts == test_stream->last_info.ts) {
		...
		if (test_stream->valid_rx_cnt > 1U) {
			LOG_ERR("Duplicated timestamp received after %u valid RX: %u\n",
	...
	if (info->seq_num == test_stream->last_info.seq_num) {
		...
		if (test_stream->valid_rx_cnt > 1U) {
			LOG_ERR("Duplicated PSN received after %u valid RX: %u\n",
	...
	if (info->flags & BT_ISO_FLAGS_ERROR) {
		...
		if (test_stream->valid_rx_cnt > 1U &&
			...
			LOG_ERR("ISO receive error after %u valid RX\n", test_stream->valid_rx_cnt);
		}
	}
	...
	if (info->flags & BT_ISO_FLAGS_LOST) {
		...
		if (!TEST_FLAG(test_stream->stopping) && test_stream->valid_rx_cnt > 1U) {
			LOG_ERR("ISO receive lost after %u valid RX\n", test_stream->valid_rx_cnt);
			is_err = true;
		}
	}
