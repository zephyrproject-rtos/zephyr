...
static void test_bass_add_source(void)
{
	...
	if (!bt_addr_le_eq(&recv_state.addr, &add_src_param.addr)) {
		...
		FAIL("Unexpected addr %s != %s", addr, expected_addr);
		return;
	}

	if (recv_state.adv_sid != add_src_param.adv_sid) {
		FAIL("Unexpected SID: %u", recv_state.adv_sid);
		return;
	}

	if (recv_state.pa_sync_state != BT_BAP_PA_STATE_NOT_SYNCED) {
		FAIL("Unexpected PA sync state: %d", recv_state.pa_sync_state);
		return;
	}

	if (recv_state.encrypt_state != BT_BAP_BIG_ENC_STATE_NO_ENC) {
		FAIL("Unexpected BIG encryption state: %d", recv_state.pa_sync_state);
		return;
	}

	if (recv_state.broadcast_id != add_src_param.broadcast_id) {
		FAIL("Unexpected broadcast ID: 0x%06X != 0x%06X", recv_state.broadcast_id,
		     add_src_param.broadcast_id);
		return;
	}

	if (recv_state.num_subgroups != add_src_param.num_subgroups) {
		FAIL("Unexpected number of subgroups: %u", recv_state.num_subgroups);
		return;
	}
...
static void test_bass_mod_source(uint32_t bis_sync)
{
	...
	if (subgroup.bis_sync == 0) {
		if (recv_state.subgroups[0].bis_sync != 0U) {
			FAIL("Unexpected BIS sync value: %u", recv_state.subgroups[0].bis_sync);
			return;
		}
	} else {
		printk("Waiting for BIS sync\n");

		/* Wait for another notification, which will either request a broadcast code for
		 * encrypted broadcasts, or have the BIS sync values set
		 */
		WAIT_FOR_FLAG(flag_recv_state_updated);
		if (recv_state.encrypt_state == BT_BAP_BIG_ENC_STATE_BCODE_REQ) {
			...
			if (recv_state.subgroups[0].bis_sync != 0U) {
				FAIL("Unexpected BIS sync value: %u",
				     recv_state.subgroups[0].bis_sync);
				return;
			}
		} else if (recv_state.subgroups[0].bis_sync != subgroup.bis_sync) {
			FAIL("Unexpected BIS sync value: %u != %u",
			     recv_state.subgroups[0].bis_sync, subgroup.bis_sync);
			return;
		}
	}
...
static void test_bass_mod_source_long_meta(void)
{
	...
	if (memcmp(recv_state.subgroups[0].metadata, subgroup.metadata, subgroup.metadata_len) !=
	    0) {
		FAIL("Unexpected metadata (len %u / %u)", recv_state.subgroups[0].metadata_len,
		     subgroup.metadata_len);
		return;
	}
