...
#ifdef CONFIG_BT_CCP_SERVER
LOG_MODULE_REGISTER(ccp_server, CONFIG_LOG_DEFAULT_LEVEL);
...
{
	for (int i = 0; i < CONFIG_BT_CCP_SERVER_BEARER_COUNT; i++) {
		if (bearers[i] != NULL) {
			const int err = bt_ccp_server_unregister_bearer(bearers[i]);
...
static void test_main(void)
{
	...
	PASS("CCP Server Passed\n");
}

static const struct bst_test_instance test_ccp_server[] = {
	{
		.test_id = "ccp_server",
...
struct bst_test_list *test_ccp_server_install(struct bst_test_list *tests)
{
	return bst_add_tests(tests, test_ccp_server);
}
#else
struct bst_test_list *test_ccp_server_install(struct bst_test_list *tests)
{
