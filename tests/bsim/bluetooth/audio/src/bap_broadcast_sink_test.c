				  ...
				  struct bt_le_per_adv_sync_synced_info *info)
{
	...
	if (sync == pa_sync) {
		...
		LOG_INF("PA sync %p synced for broadcast sink with broadcast ID 0x%06X via PAST",
...
{
	LOG_INF("Requested to sync to PA by %p with past_available %d and pa_interval 0x%04X for "
		"receive state %p",
...
static int pa_sync_term_req_cb(struct bt_conn *conn,
			       const struct bt_bap_scan_delegator_recv_state *recv_state)
{
	LOG_INF("Requested to terminate sync to PA by %p for receive state %p", conn, recv_state);
...
static void test_pa_sync_delete(void)
{
	...
	LOG_INF("Terminated PA sync %p", pa_sync);
...
static void pa_sync_past(void)
{
	...
	LOG_INF("Subscribing to PAST from %p", default_conn);
...
static void test_broadcast_sink_create(void)
{
	...
	LOG_INF("Creating the broadcast sink with broadcast_id 0x%06X", broadcaster_broadcast_id);
