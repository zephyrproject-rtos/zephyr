...
static void init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_PRIVACY) && IS_ENABLED(CONFIG_BT_LL_SW_SPLIT)) {
		uint8_t my_bd_addr[BT_ADDR_SIZE] = {
			bsim_args_get_global_device_nbr(), 0xde, 0xd4, 0xc3, 0xb2, 0xa1};
		bt_ctlr_set_public_addr((const uint8_t *)&my_bd_addr[0]);
	}
...
static void update_conn_params(void)
{
	...
	/* Use a fixed connection interval to make timing more deterministic for the test
	 * 75 ms interval is long enough to prevent ISO errors during the stream establishment
	 * but short enough to not cause long delays in the test
	 */
	UNSET_FLAG(flag_conn_updated);
	err = bt_conn_le_param_update(default_conn,
				      BT_LE_CONN_PARAM(BT_GAP_MS_TO_CONN_INTERVAL(75),
						       BT_GAP_MS_TO_CONN_INTERVAL(75), 0,
