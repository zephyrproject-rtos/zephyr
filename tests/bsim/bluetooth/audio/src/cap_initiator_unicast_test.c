						 ...
						 struct bt_bap_unicast_group **unicast_group)
{
	...
	struct bt_codec_qos *snk_qos[CAP_AC_MAX_SNK];
	struct bt_codec_qos *src_qos[CAP_AC_MAX_SRC];
...
static int set_chan_alloc(enum bt_audio_location loc, struct bt_codec *codec)
{
					      ...
					      struct bt_bap_unicast_group *unicast_group)
{
	...
	struct bt_codec_qos *snk_qos[CAP_AC_MAX_SNK] = {0};
	struct bt_codec_qos *src_qos[CAP_AC_MAX_SRC] = {0};
	struct bt_codec *snk_codecs[CAP_AC_MAX_SNK] = {0};
	struct bt_codec *src_codecs[CAP_AC_MAX_SRC] = {0};
	...
	for (size_t i = 0U; i < param->conn_cnt; i++) {
		for (size_t j = 0U; j < param->snk_cnt[i]; j++) {
			...
			if (param->conn_cnt > 1) {
				set_chan_alloc(BIT(i), stream_param->codec);
			}
		}
