...
CREATE_FLAG(flag_bearer_signal_strength_read);
	...
	void *user_data)
{
	...
	SET_FLAG(flag_bearer_signal_strength_read);
}
...
static void read_bearer_name(struct bt_ccp_call_control_client_bearer *bearer)
{
	...
	if (err != 0) {
		FAIL("Failed to read name of bearer %p: %d\n", bearer, err);
		return;
	}
...
static void read_bearer_uci(struct bt_ccp_call_control_client_bearer *bearer)
{
	...
	if (err != 0) {
		FAIL("Failed to read UCI of bearer %p: %d\n", bearer, err);
		return;
	}
...
static void read_bearer_tech(struct bt_ccp_call_control_client_bearer *bearer)
{
	...
	if (err != 0) {
		FAIL("Failed to read technology of bearer %p: %d\n", bearer, err);
		return;
	}
...
static void read_bearer_uri_schemes(struct bt_ccp_call_control_client_bearer *bearer)
{
	...
	if (err != 0) {
		FAIL("Failed to read URI supported schemes of bearer %p: %d", bearer, err);
		return;
	}
...
static void read_bearer_signal_strength(struct bt_ccp_call_control_client_bearer *bearer)
{
	...
	UNSET_FLAG(flag_bearer_signal_strength_read);
	...
	WAIT_FOR_FLAG(flag_bearer_signal_strength_read);
}
