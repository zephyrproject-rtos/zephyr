...
static void test_iso_sync_receiver(void)
{
	...
	TEST_ASSERT(bt_addr_le_eq(&remote_addr, &ev_addr), "%s != %s",
		    bt_addr_le_str(&remote_addr), bt_addr_le_str(&ev_addr));
	TEST_ASSERT(status == 0, "Sync failed with status %u", status);
	LOG_INF("Device %s: periodic synced %u status %u", bt_addr_le_str(&remote_addr),
		sync_handle, status);

	bsim_btp_wait_for_gap_periodic_biginfo(&ev_addr, &sid, &num_bis, &encryption);
	TEST_ASSERT(bt_addr_le_eq(&remote_addr, &ev_addr), "%s != %s",
		    bt_addr_le_str(&remote_addr), bt_addr_le_str(&ev_addr));
	LOG_INF("Device %s: BIGinfo sid %u num_bis %u enc %u", bt_addr_le_str(&remote_addr), sid,
		num_bis, encryption);
	...
	TEST_ASSERT(bt_addr_le_eq(&remote_addr, &ev_addr), "%s != %s",
		    bt_addr_le_str(&remote_addr), bt_addr_le_str(&ev_addr));
	LOG_INF("Device %s: Data path of BIS %u is setup", bt_addr_le_str(&remote_addr), bis_id);
	...
	do {
		...
		LOG_INF("Device %s: BIS Stream RX BIS %u len %u flags %u TS %u seq_num %u",
			bt_addr_le_str(&remote_addr), ev->bis_id, ev->data_len, ev->flags,
			ev->ts, ev->seq_num);
	...
	LOG_INF("Device %s: Periodic sync lost (reason %u)", bt_addr_le_str(&remote_addr),
		reason);
