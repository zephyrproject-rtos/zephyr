...
Quick start
-----------
1. Ensure BabbleSim is set up and the required environment variables are configured.
2. From this directory, either:
   ...
   - Or run with Twister, then execute:

     ``west twister -T . -p <board> -i --no-clean``
     ``./run.sh``

   Replace ``<board>`` with a supported simulation board, for example
   ``nrf52_bsim/native``.
