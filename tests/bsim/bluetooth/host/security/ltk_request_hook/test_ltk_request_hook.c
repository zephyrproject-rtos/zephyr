...
void test_ltk_request_hook(void)
{
	...
	if (peripheral) {
		...
		err = bt_conn_register_ltk_request_cb(app_ltk_request_cb);
		if (err) {
	...
	if (peripheral) {
		...
		/* When central starts encryption, the bt_hook_conn_ltk_request
		 * hook is triggered on the peripheral. See the hook now to
		 * follow the flow.
		 */
		k_sem_take(&ltk_request_sem, K_FOREVER);

		LOG_INF("Peripheral responds with the same LTK.");
		err = bt_conn_le_ltk_reply(conn, oob_preshared_ltk);
		if (err) {
...
static struct bst_test_list *test_installer(struct bst_test_list *installed_tests)
{
	static const struct bst_test_instance tests[] = {
		{
			.test_id = "test_ltk_request_hook",
			.test_main_f = test_ltk_request_hook,
		},
		BSTEST_END_MARKER,
	};
