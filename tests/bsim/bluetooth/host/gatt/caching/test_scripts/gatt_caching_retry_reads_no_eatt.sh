...
simulation_id="${BOARD_TS}_gatt_caching_retry_reads_no_eatt"
...
Execute ./${test_exe} -v=${verbosity_level} -s=${simulation_id} -d=0 \
    -testid=gatt_client_retry_reads_no_eatt -RealEncryption=1

Execute ./${test_exe} -v=${verbosity_level} -s=${simulation_id} -d=1 \
    -testid=gatt_server_no_eatt -RealEncryption=1
