...
static void dev_suspend_changed_cb(struct bt_conn *conn, bool suspended)
{
	...
	last_ctrl_point_val = suspended ? TEST_CTRL_POINT_SUSPEND
					: TEST_CTRL_POINT_EXIT_SUSPEND;
...
static void test_hogp_device_basic(void)
{
	...
	TEST_ASSERT(last_ctrl_point_val == TEST_CTRL_POINT_SUSPEND,
	...
	bk_sync_wait();
	WAIT_FOR_FLAG(flag_ctrl_point_received);
	TEST_ASSERT(last_ctrl_point_val == TEST_CTRL_POINT_EXIT_SUSPEND,
