...
static void reconfigure_cig(void)
{
	...
	cig_param.num_cis = ARRAY_SIZE(channels) - 1;
...
static void connect_cis(void)
{
	...
	printk("Waiting for CIS connections\n");
	ARRAY_FOR_EACH_PTR(test_chans, test_chan) {
		printk("Waiting for test_chan %p to be connected\n", test_chan);
		WAIT_FOR_FLAG(test_chan->flag_iso_connected);
		printk("test_chan %p connected\n", test_chan);
	}
}
...
static void reset_bluetooth(void)
{
	...
	cig = NULL;
	ARRAY_FOR_EACH_PTR(test_chans, test_chan) {
		test_chan->disconnect_reason = 0U;
	}
