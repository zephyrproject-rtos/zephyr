...
static void init(void)
{
	...
	TEST_ASSERT(bt_dev.le.iso_mtu >= BT_HCI_ISO_SDU_HDR_SIZE + 1, "Invalid ISO MTU: %u < %d",
		    bt_dev.le.iso_mtu, BT_HCI_ISO_SDU_HDR_SIZE + 1);
	...
	for (size_t i = 0U; i < ARRAY_SIZE(iso_chans); i++) {
		...
		iso_qos.tx->sdu =
			MIN(bt_dev.le.iso_mtu - BT_HCI_ISO_SDU_HDR_SIZE, ARRAY_SIZE(mock_iso_data));
	}
