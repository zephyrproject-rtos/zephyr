...
static void setup_backchannels(void)
{
        ...
        backchannels = bs_open_back_channel(self, device_nbrs, channel_numbers, ARRAY_SIZE(device_nbrs));
...
void bt_testlib_bs_sync()
{
	static uint64_t counter = 0;

	LOG_DBG("bt_testlib_bs_sync %llu d%u enter", counter, get_device_nbr());

        /* Device 0 acts as hub. */
        if (get_device_nbr() == 0) {
                for (int i = 1; i < n_devs; i++) {
                        uint64_t counter_cfm = bs_bc_recv_uint(backchannels[i]);
                ...
                uint64_t counter_cfm = bs_bc_recv_uint(backchannels[0]);
                __ASSERT(counter_cfm == counter, "%luu %luu", counter_cfm, counter);
        }

        LOG_DBG("bt_testlib_bs_sync %llu d%u exit", counter, get_device_nbr());
