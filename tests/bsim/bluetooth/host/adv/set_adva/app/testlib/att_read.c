...
static inline uint8_t gatt_discover_service_cb(struct bt_conn *conn,
	...
	if (!ctx->att_err) {
		...
		if (ctx->result_end_handle) {
			*ctx->result_end_handle = 0;
			/* Output 'group end handle'. TODO:
			 * Document how all of this maps to the ATT RSPs.
			 */
				     ...
				     uint16_t start_handle, uint16_t end_handle)
{
	...
	struct bt_testlib_gatt_discover_service_closure _ctx = {
		.result_handle = result_handle,
	...
	struct bt_testlib_gatt_discover_service_closure *const ctx = &_ctx;
...
static inline uint8_t gatt_discover_char_cb(struct bt_conn *conn, const struct bt_gatt_attr *attr,
					    struct bt_gatt_discover_params *params)
{
	LOG_DBG("gatt_discover_char_cb conn %u attr %p params %p", bt_conn_index(conn), attr,
		params);
					    ...
					    uint16_t svc_end_handle)
{
	...
	struct bt_testlib_gatt_discover_char_closure _ctx = {
	...
	struct bt_testlib_gatt_discover_char_closure *const ctx = &_ctx;
