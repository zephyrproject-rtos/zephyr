			   ...
			   struct bt_conn *conn)
{
	...
	if (central) {
		...
		NET_BUF_SIMPLE_DEFINE(attr_value, BT_ATT_MAX_ATTRIBUTE_LEN);

		/* Perform the whole long read operation. */
		EZ(btt_gatt_long_read(&attr_value, NULL, conn, bearer, chrc_value_handle, 0));
		 ...
		 */
		for (; attr_value.len; read_count++) {
			uint16_t encoded_len;
			uint16_t padding_size;

			LOG_INF("Verifying read %u", read_count);

			__ASSERT(attr_value.len >= sizeof(encoded_len),
				 "Incomplete encoded length");
			encoded_len = net_buf_simple_pull_le16(&attr_value);

			padding_size = (encoded_len - 2);
			LOG_INF("Padding size %u", padding_size);

			/* Check and discard padding. */
			for (int i = 0; i < padding_size; i++) {
				__ASSERT(attr_value.len, "Unexpected end of buffer");
				__ASSERT(net_buf_simple_pull_u8(&attr_value) == 0,
