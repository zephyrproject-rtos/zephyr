...
static void resolve_reports(void)
{
	...
	for (uint8_t i = 0U; i < reports_cnt; i++) {
		...
		switch (report->type) {
		case BT_HIDS_REPORT_TYPE_INPUT:
			...
			break;
		case BT_HIDS_REPORT_TYPE_OUTPUT:
			...
			break;
		case BT_HIDS_REPORT_TYPE_FEATURE:
...
static void test_hids_client(void)
{
	...
	verify_protocol_mode(BT_HIDS_PROTOCOL_REPORT);

	input = report_by_type(BT_HIDS_REPORT_TYPE_INPUT);
	output = report_by_type(BT_HIDS_REPORT_TYPE_OUTPUT);
	...
	gatt_write_cmd(protocol_mode_handle, BT_HIDS_PROTOCOL_BOOT);
	bk_sync_send();
	bk_sync_wait();
	verify_protocol_mode(BT_HIDS_PROTOCOL_BOOT);

	gatt_write_cmd(protocol_mode_handle, BT_HIDS_PROTOCOL_REPORT);
	bk_sync_send();
	bk_sync_wait();
	verify_protocol_mode(BT_HIDS_PROTOCOL_REPORT);
	...
	/* The server re-registers; read Protocol Mode so the GATT callback claims
	 * the per-connection slot on behalf of the already-encrypted link.
	 */
	bk_sync_wait();
	verify_protocol_mode(BT_HIDS_PROTOCOL_REPORT);
	bk_sync_send();
