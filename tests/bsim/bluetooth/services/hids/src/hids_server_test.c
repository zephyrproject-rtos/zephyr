 ...
 * the part the service needs: it registers the HID Service and requests
 * security on every connection.
 *
...
static enum bt_hids_report_type set_report_type;
...
static void set_long_report(void)
{
	...
	err = bt_hids_report_set(BT_HIDS_REPORT_TYPE_FEATURE, TEST_REPORT_ID_FEATURE_LONG, value,
				 sizeof(value));
	TEST_ASSERT(err == 0, "Failed to set the long Feature Report: %d", err);
}

static enum bt_hids_protocol_mode last_protocol_mode;
...
static void set_report(struct bt_conn *conn, enum bt_hids_report_type report_type,
		       uint8_t report_id, const uint8_t *data, uint16_t len)
{
	...
	if (report_type == BT_HIDS_REPORT_TYPE_FEATURE && len == sizeof(feature_value)) {
...
static void protocol_mode_changed(struct bt_conn *conn, enum bt_hids_protocol_mode protocol)
{
	ARG_UNUSED(conn);
...
static void ccc_changed(struct bt_conn *conn, uint8_t report_id,
			enum bt_hids_report_type report_type, bool enabled)
{
	ARG_UNUSED(conn);

	/* Only the Input Report has a CCC descriptor */
	TEST_ASSERT(report_type == BT_HIDS_REPORT_TYPE_INPUT, "CCC of Report Type %u changed",
...
static void verify_set_report(enum bt_hids_report_type report_type, uint8_t report_id,
			      const uint8_t *data, uint16_t len)
{
	TEST_ASSERT(set_report_type == report_type, "SET_REPORT of Report Type %u",
		    (unsigned int)set_report_type);
...
static void verify_per_connection_defaults(void)
{
	...
	TEST_ASSERT(mode == BT_HIDS_PROTOCOL_REPORT, "Protocol Mode is %u", mode);
...
static void test_hids_server(void)
{
	...
	err = bt_hids_report_set(BT_HIDS_REPORT_TYPE_INPUT, TEST_REPORT_ID_INPUT, test_input_data,
				 TEST_INPUT_REPORT_LEN);
	TEST_ASSERT(err == 0, "Failed to set the Input Report: %d", err);
	err = bt_hids_report_set(BT_HIDS_REPORT_TYPE_FEATURE, TEST_REPORT_ID_FEATURE, feature_value,
	...
	verify_set_report(BT_HIDS_REPORT_TYPE_OUTPUT, TEST_REPORT_ID_OUTPUT, test_output_data,
	...
	bk_sync_wait();
	WAIT_FOR_FLAG(flag_set_report);
	verify_set_report(BT_HIDS_REPORT_TYPE_FEATURE, TEST_REPORT_ID_FEATURE, test_feature_data,
	...
	TEST_ASSERT(last_protocol_mode == BT_HIDS_PROTOCOL_BOOT, "Protocol Mode is %u",
		    last_protocol_mode);
	err = bt_hids_get_protocol_mode(server_conn, &mode);
	TEST_ASSERT(err == 0, "Failed to get the Protocol Mode: %d", err);
	TEST_ASSERT(mode == BT_HIDS_PROTOCOL_BOOT, "Protocol Mode is %u", mode);
	...
	bk_sync_wait();
	WAIT_FOR_FLAG(flag_protocol_mode_changed);
	TEST_ASSERT(last_protocol_mode == BT_HIDS_PROTOCOL_REPORT, "Protocol Mode is %u",
		    last_protocol_mode);
	UNSET_FLAG(flag_protocol_mode_changed);
	bk_sync_send();
	...
	/* Signal the client that unregistering is done and we are still connected. */
	bk_sync_send();

	/* Registering while a Host is connected has to track it, as the
	 * connected callback has already run for that Host.  GATT callbacks
	 * call conn_alloc() so the first access after re-registration claims
	 * the slot on behalf of the already-encrypted Host.
	 */
	err = bt_hids_register(&hids_param);
	TEST_ASSERT(err == 0, "Failed to register the HID Service again: %d", err);

	/* Ask the client to read Protocol Mode; that GATT access will claim the
	 * slot via conn_alloc() and put per-connection state back to defaults.
	 */
	bk_sync_send();
	bk_sync_wait();
