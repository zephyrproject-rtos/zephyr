...
#if defined(CONFIG_USERSPACE)
static K_WATCHPOINT_DEFINE(g_user_api_private_wp, (void *)&g_watched, 1U,
			   K_WATCHPOINT_WRITE, callback, NULL);

ZTEST_USER(watchpoint, test_user_add_rejected)
{
	struct k_watchpoint wp = K_WATCHPOINT_INITIALIZER(
		(void *)&g_user_watched, 1U, K_WATCHPOINT_WRITE, callback, NULL);
	struct k_watchpoint *const descriptors[] = {
		&wp, NULL, &g_user_api_private_wp, (struct k_watchpoint *)UINTPTR_MAX,
	};

	zassert_true(k_is_user_context());
	for (size_t i = 0; i < ARRAY_SIZE(descriptors); i++) {
		zassert_equal(k_watchpoint_add(descriptors[i]), -EPERM);
	}
	zassert_equal(atomic_get(&wp._state), 0);
	zassert_equal(wp._handle, 0U);
}

ZTEST_USER(watchpoint, test_user_remove_rejected)
{
	struct k_watchpoint wp = {0};
	struct k_watchpoint *const descriptors[] = {
		&wp, NULL, &g_user_api_private_wp, (struct k_watchpoint *)UINTPTR_MAX,
	};
	...
	zassert_true(k_is_user_context());
	for (size_t i = 0; i < ARRAY_SIZE(descriptors); i++) {
		zassert_equal(k_watchpoint_remove(descriptors[i]), -EPERM);
	}
	zassert_equal(atomic_get(&wp._state), 0);
	zassert_equal(wp._handle, 0U);
}

ZTEST_USER(watchpoint, test_user_state_query_rejected)
{
	/* A nonzero state makes an unguarded query return true. */
	struct k_watchpoint wp = { ._state = ATOMIC_INIT(1) };
	const struct k_watchpoint *const descriptors[] = {
		&wp, NULL, &g_user_api_private_wp, (const struct k_watchpoint *)UINTPTR_MAX,
	};

	zassert_true(k_is_user_context());
	for (size_t i = 0; i < ARRAY_SIZE(descriptors); i++) {
		zassert_false(k_watchpoint_is_active(descriptors[i]));
	}
	zassert_equal(atomic_get(&wp._state), 1);
}
...
{
	static K_WATCHPOINT_DEFINE(wp, (void *)&g_user_watched, 1U,
				   K_WATCHPOINT_WRITE, callback, NULL);
	...
	zexpect_not_null(g_cb_pc);
#if defined(CONFIG_WATCHPOINT_CALLSTACK)
	/* The architecture unwinder may not walk user-mode stacks. */
	zexpect_true(g_cb_callstack_depth > 0U);
	zexpect_equal(g_cb_callstack[0], (uintptr_t)g_cb_pc);
#endif
	zassert_ok(k_watchpoint_remove(&wp));
