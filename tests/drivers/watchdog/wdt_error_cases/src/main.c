...
#define WDT_TEST_FLAGS \
	(WDT_DISABLE_SUPPORTED | WDT_FLAG_RESET_SOC_SUPPORTED | \
	 ...
	 (!DT_NODE_HAS_STATUS_OKAY(DT_INST(0, test_wdt_error_cases)) ? 0 : \
	 (DT_PROP(DT_CONFIG, opt_pause_in_sleep) ? WDT_OPT_PAUSE_IN_SLEEP_SUPPORTED : 0) | \
	 (DT_PROP(DT_CONFIG, opt_pause_halted_by_dbg) ? \
	  WDT_OPT_PAUSE_HALTED_BY_DBG_SUPPORTED : 0) | \
	 (DT_PROP(DT_CONFIG, feed_can_stall) ? WDT_FEED_CAN_STALL : 0) | \
	 (DT_PROP(DT_CONFIG, window_min) ? WDT_WINDOW_MIN_SUPPORTED : 0) | \
	 (DT_PROP(DT_CONFIG, opt_pause_in_sleep_requires_pm) ? WDT_OPT_PAUSE_IN_SLEEP_REQUIRES_PM : 0) | \
	 (DT_PROP(DT_CONFIG, flag_reset_cpu_core) ? WDT_FLAG_RESET_CPU_CORE_SUPPORTED : 0) | \
	 (DT_PROP(DT_CONFIG, flag_reset_none) ? WDT_FLAG_RESET_NONE_SUPPORTED : 0)))
