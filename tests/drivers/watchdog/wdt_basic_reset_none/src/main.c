...
static uint32_t wdt_test_state WDT_TEST_NOINIT_ATTR;

static void wdt_test_set_state(uint32_t state)
{
	wdt_test_state = state;
	sys_cache_data_flush_range(&wdt_test_state, sizeof(wdt_test_state));
}
...
static void wdt_callback(const struct device *dev, int channel_id)
{
	...
	wdt_test_set_state(WDT_TEST_CALLBACK_FIRED);
}
