...
#define WDT_TEST_FEEDS_COMPLETE  0x57445402U
#define WDT_TEST_CALLBACK_FIRED  0x57445403U
#define WDT_TEST_CALLBACK_EARLY  0x57445404U
...
static void wdt_test_set_state(uint32_t state)
{
	wdt_test_state = state;
...
static void wdt_callback(const struct device *dev, int channel_id)
{
	...
	wdt_test_set_state(wdt_test_state == WDT_TEST_FEEDS_COMPLETE ?
			   WDT_TEST_CALLBACK_FIRED : WDT_TEST_CALLBACK_EARLY);
}
...
static int test_wdt_callback_reset_none(void)
{
	...
	if (wdt_test_state == WDT_TEST_CALLBACK_FIRED) {
		...
		wdt_test_set_state(WDT_TEST_IDLE);
		return TC_PASS;
	}

	if (wdt_test_state == WDT_TEST_CALLBACK_EARLY) {
		TC_PRINT("Watchdog callback fired before feeding completed\n");
		wdt_test_set_state(WDT_TEST_IDLE);
		return TC_FAIL;
	}
	...
	wdt_test_set_state(WDT_TEST_ARMED);
	...
	if (wdt_interrupted_flag != 0) {
		irq_unlock(key);
		TC_PRINT("Watchdog callback fired before feeding completed\n");
		return TC_FAIL;
	}

	wdt_test_set_state(WDT_TEST_FEEDS_COMPLETE);
	irq_unlock(key);
	...
	err = wdt_disable(wdt);
	if (err == -EPERM) {
#if DT_HAS_COMPAT_STATUS_OKAY(nxp_ewm)
		TC_PRINT("Watchdog cannot be disabled after callback, skip\n");
		wdt_test_set_state(WDT_TEST_IDLE);
		return TC_SKIP;
#else
