...
ZTEST(wm8960_test, test_wm8960_init)
{
	...
	/* Check that the clocking register has the expected reset value */
	LOG_INF("Clocking register: 0x%04x",
		wm8960_emul_get_reg(wm8960_emul, 0x04));
...
ZTEST(wm8960_test, test_wm8960_volume_control)
{
	...
	uint16_t left_dac_vol = wm8960_emul_get_volume(wm8960_emul, 0) & 0xFF;
	uint16_t right_dac_vol = wm8960_emul_get_volume(wm8960_emul, 1) & 0xFF;
	uint16_t left_hp_vol = wm8960_emul_get_volume(wm8960_emul, 2) & 0xFF;
	uint16_t right_hp_vol = wm8960_emul_get_volume(wm8960_emul, 3) & 0xFF;

	zassert_equal(
		left_dac_vol,
		0xFF,
		"Left DAC volume should be 0xFF, got 0x%02x",
		left_dac_vol
	);
	zassert_equal(
		right_dac_vol,
		0xFF,
		"Right DAC volume should be 0xFF, got 0x%02x",
		right_dac_vol
	);
	zassert_equal(
		left_hp_vol,
		0x40,
		"Left headphone volume should be 0x40, got 0x%02x",
		left_hp_vol
	);
	zassert_equal(
		right_hp_vol,
		0x40,
		"Right headphone volume should be 0x40, got 0x%02x",
		right_hp_vol
	);
	...
	zassert_true(wm8960_emul_is_muted(wm8960_emul, 2), "Left channel should be muted");
	zassert_true(wm8960_emul_is_muted(wm8960_emul, 3), "Right channel should be muted");
	...
	zassert_false(wm8960_emul_is_muted(wm8960_emul, 2), "Left channel should be unmuted");
	zassert_false(wm8960_emul_is_muted(wm8960_emul, 3), "Right channel should be unmuted");
}
...
ZTEST(wm8960_test, test_wm8960_volume_boundary_values)
{
	...
	left_hp_vol = wm8960_emul_get_volume(wm8960_emul, 2) & 0xFF;
	zassert_equal(left_hp_vol, 0x00, "Left volume should be 0x00, got 0x%02x", left_hp_vol);
	...
	left_hp_vol = wm8960_emul_get_volume(wm8960_emul, 2) & 0xFF;
	zassert_equal(left_hp_vol, 0x7F, "Left volume should be 0x7F, got 0x%02x", left_hp_vol);
}
...
ZTEST(wm8960_test, test_wm8960_individual_channel_control)
{
	...
	uint16_t left_vol = wm8960_emul_get_volume(wm8960_emul, 2) & 0xFF;
	uint16_t right_vol = wm8960_emul_get_volume(wm8960_emul, 3) & 0xFF;
	...
	/* Verify left output is muted and right is not */
	zassert_true(wm8960_emul_is_muted(wm8960_emul, 2), "Left channel should be muted");
	zassert_false(wm8960_emul_is_muted(wm8960_emul, 3), "Right channel shouldn't be muted");
}
...
ZTEST(wm8960_test, test_wm8960_speaker_output)
{
	...
	if (wm8960_emul_is_speaker_enabled(wm8960_emul)) {
		...
		LOG_INF("Speaker volumes - Left: 0x%04x, Right: 0x%04x",
			wm8960_emul_get_volume(wm8960_emul, 4),
			wm8960_emul_get_volume(wm8960_emul, 5));
	}
