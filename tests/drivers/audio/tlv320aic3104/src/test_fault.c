...
ZTEST(tlv320_fault, test_hprcom_untouched_without_the_board_property)
{
	const uint8_t sentinel = 0xA5;

	tlv320aic3104_emul_set_val(0, R_HPRCOM_LEVEL, sentinel);

	configure_codec();

	zassert_equal(tlv320aic3104_emul_last_val(0, R_HPRCOM_LEVEL), sentinel,
		      "R72 must not be written without hprcom-vcm-output");
	zassert_equal(tlv320aic3104_emul_last_val(0, R_HP_OUTPUT_DRIVER_CTRL) & 0x38, 0,
		      "R38 D5-D3 must stay at its reset mode");
}
