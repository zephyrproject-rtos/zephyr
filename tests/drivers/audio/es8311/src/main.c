...
/*
 * Registers the driver depends on and must therefore write: a previous firmware, or this
 * driver's own previous boot on a part with no reset pin, can hand it any of them.
 */
...
/*
 * A part at this address that is not an ES8311 must not come up ready: an identity check that
 * only warns leaves every later register write going to whatever part is really there. Seed a
 * wrong id, re-run init(), and assert -ENODEV with the device NOT ready.
 */
ZTEST(es8311, test_init_wrong_chip_id_is_fatal)
{
	/*
	 * A foreign part at this address is refused fatally: a chip-id check that leaves the device
	 * ready sends every later register write to whatever is really there. This runs against its
	 * own never-initialised instance rather than writing Zephyr's internal device state. 0x0000
	 * is what a pulled-down bus answers, and nothing is written to it.
	 */
...
ZTEST(es8311, test_configure_write_order)
{
	...
	for (int i = 0; i < n; i++) {
		...
		/*
		 * The LAST 0x12: configure() writes it twice, in the quiesce that powers the DAC
		 * down before the clock tree moves and at the end to power it back up. The
		 * constraint is on the power-UP, which must follow the bias and references.
		 */
...
/*
 * A capture-only route powers the ADC up and the DAC DOWN. Register 0x00 does not reset the
 * register file, so a DAC a previous route powered up would otherwise stay live behind a caller
 * that asked for capture only.
 */
...
/*
 * The clock tree is ratiometric: MCLK is 8 * BCLK and a 16-bit stereo frame carries 32 bit clocks,
 * so every DIVIDER takes one value at every rate. DAC_OSR (0x04) is not a divider: field N means 4N
 * * fs, and the vendor table uses 128 * fs at and below 16 kHz and 64 * fs above. ADC_OSR really is
 * rate-independent (adc_osr 0x10 on every single-speed row), so the two are asserted differently.
 */
...
ZTEST(es8311, test_input_mute_survives_configure)
{
	...
	/*
	 * That assertion alone cannot tell a surviving INPUT_MUTE from a lost one: configure()
	 * leaves RX stopped, and a stopped RX is muted whatever the cached value says. start(RX)
	 * is the first write that reads adc_mute instead, so it is where the caller's mute has to
	 * be checked.
	 */
	zassert_ok(audio_codec_start(codec, AUDIO_DAI_DIR_RX), "start(RX) failed");
	zassert_equal(reg_get(ES8311_REG_SDP_OUT), 0x0CU | ES8311_SDP_MUTE,
...
/*
 * On a playback-only route apply_properties() must not touch the ADC serial port at all. The
 * route is what stops it: the stopped-RX re-mute in phase 1 would otherwise fire, because a
 * route without capture leaves input_stopped true, and a write that failed there would set the
 * error that stops the speaker unmute for a direction the caller never configured.
 */
...
/*
 * The concurrency test; everything above runs on one thread. apply_properties() must hold the codec
 * lock across its register writes, not only across the read of the cached state, or stop_output()
 * can slip into the gap, mute the DAC, and have that write overwritten by the value
 * apply_properties() sampled before the gap: cache muted, speaker live. The emulator parks
 * apply_properties() inside its first write (0x32, which stop_output() never touches) while a
 * second thread calls stop_output().
 */
...
/*
 * The stop thread announces itself on both sides of the call, so the test asserts that
 * stop_output() is still BLOCKED while the apply thread is parked, rather than inferring it from
 * which write landed last. It runs at a cooperative priority and the test thread hands over with
 * k_yield(): the ztest thread is itself cooperative, so nothing preempts it, and once it yields the
 * stop thread keeps the CPU until it blocks, which it can only do on the codec mutex.
 */
...
/*
 * configure() must quiesce the part before it moves the clock tree: gating the clock of a DAC that
 * is still powered and unmuted freezes the modulator on the last sample, a DC step into the
 * amplifier. Only the write ORDER can show that the mutes and power-downs came before the first
 * clock-manager write.
 */
...
/*
 * A configure() that dies part-way through must not leave the driver steering by a route that no
 * longer describes the chip: some clocks gated, a converter powered down, a reference moved. After
 * a failed configure() the driver has NO route, and start_output(), stop_output() and
 * apply_properties() must touch nothing. The failure is walked across every transfer, since the
 * interesting failures are in the middle of the sequence.
 */
ZTEST(es8311, test_failed_configure_disarms_start_but_never_stop)
{
	...
	for (int n = 0; n < 64; n++) {
		...
		/*
		 * MUTING is not gated. A configure() that died on its FIRST write changed nothing:
		 * the old route's DAC is still powered, unmuted and driving the amplifier, yet the
		 * route cache has been cleared. A stop_output() gated on that cache would do
		 * nothing, a fail-open. Seed the mute register to "not muted" so the assertion
		 * needs a real write.
		 */
...
/*
 * Every test above starts from an all-zero register file, the state the driver would like to find.
 * The driver never resets the part (a reset costs seconds of analog settling on the capture path)
 * and the ES8311 has no reset pin, so on real hardware it inherits its whole register file from
 * whatever ran last. The tests below vary that.
 */
...
ZTEST(es8311, test_playback_only_still_clears_adc2dac_sel)
{
	...
	/*
	 * 0x44 bit 7 is ADC2DAC_SEL, a PLAYBACK control in a register that otherwise belongs to the
	 * ADC. A playback-only configure() that skipped it left a chip handed over with the bit set
	 * playing its own microphone instead of the caller's audio, with configure() returning 0
	 * and every checked register reading back as intended.
	 */
	reg_put(ES8311_REG_ADC_MUX, ES8311_ADC2DAC_SEL);
...
ZTEST(es8311, test_the_driver_never_resets_the_register_file)
{
	...
	/*
	 * Both reference drivers open with 0x00 = 0x1F and this driver does not: measured on
	 * hardware, the capture noise floor is then exactly zero for about six seconds at every
	 * sample rate. A cold power-on, which charges the same reference capacitors from zero, is
	 * alive at the first sample, so the reset puts the part somewhere a cold start never goes.
	 * This looks at what the driver WROTE, not for a wiped register file: RST_DIG is "reset
	 * digital except control port block", so no value of 0x00 resets the register file.
	 */
	emul_es8311_reset_log(emul);
...
/*
 * emul_es8311_reset() puts the emulated CHIP back to a clean register file but cannot touch the
 * DRIVER: dac_volume_code, adc_volume_code, output_mute, adc_mute and the two stopped flags live in
 * struct es8311_data and persist across tests. This fixture resets the four properties through the
 * PUBLIC API and re-starts the output after the configure(). The precondition is ASSERTED, since
 * some inherited states make the next test pass for the wrong reason.
 */
...
ZTEST(es8311, test_init_quiesces_the_microphone_not_just_the_speaker)
{
	...
	/*
	 * A warm reboot out of a capture route: the SoC resets and stops the bit clock, the codec
	 * does not. The part comes up with its PGA powered and MIC1 still selected at whatever gain
	 * the last firmware chose, and muting the serial output leaves all of that running. init()
	 * has to put BOTH halves somewhere safe.
	 */
...
/*
 * A read on this bus is a write-then-read with a repeated start, which can break while a plain
 * two-byte write still gets through. A mute done as a read-modify-write cannot survive that:
 * i2c_reg_update_byte_dt() returns on the failed read WITHOUT attempting the write, so the DAC
 * stays as loud as it was while the caller gets an error for an operation never made.
 */
...
/*
 * init() exists because a warm reboot does not reach this codec: it can come up with a powered,
 * unmuted DAC on the amplifier and a live PGA on a live microphone, and every write it makes ends
 * one of those. A transient error on the FIRST of them must not skip the rest.
 */
...
/*
 * configure() leaves the DAC muted, so the only converter it opens is the microphone. Its serial
 * port must be the LAST write, with 0x44 (ADC2DAC_SEL, which wires the ADC into the DAC)
 * normalised before it, or a chip handed over with that bit set would route its microphone into
 * the path in the window. Only the write ORDER can show that.
 */
...
/*
 * configure() clears its route cache before it touches the chip, which makes the driver FORGET a
 * converter, not STOP one. Take a live capture route and a configure() whose first write fails: the
 * ADC is powered, the PGA live, MIC1 wired in, and the cache says there is no capture, so
 * apply_properties() will not go near it and stop_output() only mutes the DAC. This walks the
 * failure across every transfer and checks the HARDWARE before calling anything else;
 * test_failed_configure_disarms_start_but_never_stop asks instead whether the caller can recover
...
/*
 * set_property(OUTPUT_MUTE, true) + apply_properties() is the API's other way of silencing the
 * speaker. A failed write to the DAC VOLUME register, which is not a safety register and has
 * nothing to do with the request, must not throw the mute away. The fault is injected BY REGISTER,
 * not by transfer index, so it keeps meaning "break the DAC volume write" when the sequence order
 * changes.
 */
...
/*
 * A failure in one direction must not suppress the safety mute in the other: a DAC volume
 * glitch must not leave the MICROPHONE open when the caller asked for both.
 */
...
/*
 * A part that cannot be READ cannot be identified, and this driver then writes nothing to it. On a
 * bus whose reads fail while its writes still land that means a warm reboot out of playback leaves
 * a DAC this driver could have muted and did not; the alternative is writing six registers chosen
 * for an ES8311 into whatever is really at the address. This pins that choice.
 */
...
/*
 * apply_properties() does the microphone unmute before the speaker unmute, and the speaker unmute
 * is gated on nothing above it having failed. The gate has to be re-read at each unmute, not
 * sampled once before phase 3, so the FIRST unmute's failure disarms the SECOND. The neighbour test
 * above covers a phase-1 failure blocking an unmute; this covers the first phase-3 unmute failing
 * and taking the second down with it.
 */
ZTEST(es8311, test_a_failed_unmute_stops_the_next_unmute)
{
	...
	/*
	 * And both STARTED. configure() leaves the two directions stopped, and phase 3 skips a
	 * stopped direction, so without this the failure below lands on a phase-1 mute and
	 * neither unmute runs -- which left this test passing with the gate it is named for
	 * deleted.
	 */
...
/*
 * init() releases INI_REG before it quiesces, and a failure of that write must not abandon the
 * quiesce: whatever state the register file is in, the DAC mute, the power-downs and the
 * microphone disconnect are still worth attempting.
 */
...
/*
 * fail_from(idx) returns -EIO on transfer idx and every one after it, BEFORE the write takes
 * effect, so the error-path quiesce fails on the same dead bus. An opener is then safe only if it
 * is genuinely the LAST write. configure() leaves the DAC muted, so the only converter it opens is
 * the microphone, and that is its last write: under a persistent fail-before-effect BOTH converters
 * stay muted on every route. A write that takes effect and then errors on a bus that dies forever
 * is a different fault; the driver's contract there is best-effort.
 */
...
/*
 * Reconfiguring a LIVE route: full-duplex, DAC unmuted by start_output(), microphone open, the
 * state a reconfigure meets in service. Part A: a successful reconfigure must silence the OLD
 * route's converters BEFORE it moves the clock tree. Part B: a FAILED reconfigure must never unmute
 * the DAC; opening the speaker is start_output()'s job, so configure() writes 0x31 only as MUTE,
 * wherever the bus dies.
 */
...
/*
 * Both board-policy devicetree properties reach the register they name: codec_profile
 * (native_sim.overlay) sets each to its non-default value -- right slot, 30 dB PGA -- and the
 * default node proves the defaults. The output mode is compiled in, not a property. This is what
 * makes everest,es8311 a device profile rather than one board's fixed tuning.
 */
...
ZTEST(es8311, test_a_mic_unmute_that_lands_then_errors_is_best_effort_remuted)
{
	...
	/*
	 * RX has to be STARTED first. The fixture leaves it stopped, and a stopped RX is re-muted
	 * in phase 1 -- a write to this same register, so the injected failure would land there
	 * and the unmute below would never run. Both assertions would still pass, on the phase-1
	 * mute: deleting the re-mute this test is named for left the suite green until RX was
	 * started here.
	 */
	zassert_ok(audio_codec_start(codec, AUDIO_DAI_DIR_RX), "start(RX) failed");
...
/*
 * The microphone half of the healing above. Two tests used to cover this by accident, because
 * they left RX stopped and their injected failure landed on this phase-1 re-mute; both now
 * start RX to reach the unmutes they are named for, so the re-mute needs a test of its own.
 */
...
/*
 * The other half of that rollback. When the SPEAKER unmute fails, the microphone unmute this
 * same apply() sent is backed out, so a failed apply() aims BOTH converters at muted rather
 * than leaving a live capture path behind a playback error. Note the microphone was already
 * open here -- start(RX) opened it -- so what is backed out is this call's write, not the
 * caller's lifecycle.
 */
ZTEST(es8311, test_a_failed_speaker_unmute_backs_out_the_microphone)
{
	...
	/* Muted hardware, both cached unmuted and both directions started: apply() reaches both
	 * unmutes, and only the speaker's fails.
	 */
	reg_put(ES8311_REG_DAC_MUTE, ES8311_DAC_MUTE_ON);
	emul_es8311_fail_write_to(emul, ES8311_REG_DAC_MUTE);
	zassert_true(audio_codec_apply_properties(codec) < 0,
...
/*
 * On a bus that fails partway through the cleanup the writes that land are a prefix, so the
 * speaker must be muted first. This pins the order the error-path quiesce emits: speaker mute,
 * microphone mute, DAC data port, then the three power-downs.
 */
