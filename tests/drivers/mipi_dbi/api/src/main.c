...
ZTEST(mipi_dbi_api, test_mipi_dbi_command_write)
{
	...
	for (int i = 0; i < ARRAY_SIZE(devices); ++i) {
		for (int j = 0; j < ARRAY_SIZE(modes); ++j) {
			config.mode = modes[j];
			rslt = mipi_dbi_command_write(devices[i], &config, cmd, data, sizeof(data));
...
ZTEST(mipi_dbi_api, test_mipi_dbi_command_write_cmd_only)
{
	...
	for (int i = 0; i < ARRAY_SIZE(devices); ++i) {
		for (int j = 0; j < ARRAY_SIZE(modes); ++j) {
			config.mode = modes[j];
			rslt = mipi_dbi_command_write(devices[i], &config, cmd, NULL, 0);
			zassert_equal(rslt, 0, "Expected 0 but was %u", rslt);
		}
	}
}
...
ZTEST(mipi_dbi_api, test_mipi_dbi_write_display)
{
	...
	for (int i = 0; i < ARRAY_SIZE(devices); ++i) {
		for (int j = 0; j < ARRAY_SIZE(modes); ++j) {
			config.mode = modes[j];
			rslt = mipi_dbi_write_display(devices[i], &config, data, &descriptor,
...
ZTEST(mipi_dbi_api, test_mipi_dbi_reset)
{
	...
	for (int i = 0; i < ARRAY_SIZE(devices); ++i) {
		for (int j = 0; j < ARRAY_SIZE(modes); ++j) {
			rslt = mipi_dbi_reset(devices[i], 100);
			zassert_equal(rslt, 0, "Expected 0 but was %u", rslt);
		}
	}
}
