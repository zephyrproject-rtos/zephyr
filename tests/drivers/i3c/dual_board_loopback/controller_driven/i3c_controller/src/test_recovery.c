...
ZTEST(dual_board_loopback, test_recovery_bus_recover_api)
{
	...
	zassert_ok(rc, "i3c_recover_bus failed (%d)", rc);

	/* Recovery is only meaningful if the bus works afterwards. */
	rc = i3c_transfer_retry(&target_b_desc, &msg, 1);
	zassert_ok(rc, "transfer after i3c_recover_bus failed (%d)", rc);
}
...
ZTEST(dual_board_loopback, test_recovery_invalid_ccc)
{
	...
	zassert_ok(rc, "bus unusable after a rejected CCC (%d)", rc);
}
