 ...
 * (dual_board_loopback_stage_read) so the target queues exactly the length
...
ZTEST(dual_board_loopback, test_xfer_single_byte_write)
{
...
ZTEST(dual_board_loopback, test_xfer_single_byte_read)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
	...
	size_t qlen = dual_board_loopback_stage_read(pattern, sizeof(pattern));
...
ZTEST(dual_board_loopback, test_xfer_fifo_boundary_writes)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_xfer_fifo_boundary_reads)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
	...
	for (size_t i = 0; i < ARRAY_SIZE(lens); i++) {
		 ...
		 */
		size_t qlen = dual_board_loopback_stage_read(pattern, lens[i]);
...
ZTEST(dual_board_loopback, test_xfer_write_then_read_repeated_start)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_xfer_multi_message_array)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_xfer_max_payload_256)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_xfer_back_to_back_stress)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_xfer_helpers_write_read_burst)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
	...
	/* Stage once before retries */
	size_t qlen1 = dual_board_loopback_stage_read(pattern1, sizeof(pattern1));
	...
	/* Stage once before retries */
	size_t qlen4 = dual_board_loopback_stage_read(pattern4, sizeof(pattern4));
