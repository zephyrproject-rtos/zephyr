...
ZTEST(dual_board_loopback, test_ibi_enable_disable)
{
	...
	zassert_ok(rc, "ibi_disable failed (%d)", rc);
	target_b_desc.ibi_cb = NULL;
}

ZTEST(dual_board_loopback, test_ibi_target_intr_received)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
...
ZTEST(dual_board_loopback, test_ibi_tir_with_mandatory_byte)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
	...
	zassert_equal(last_ibi_len, 4, "payload len %u, want 4", last_ibi_len);
...
/* Five bytes is the IP maximum (mandatory MDB plus four data bytes) and the
 * only length that spans more than one IBI_QUEUE_STATUS entry, so it is the
 * only one that exercises payload reassembly. A four-byte payload fits a
 * single entry and stayed green throughout the truncation bug.
 */
ZTEST(dual_board_loopback, test_ibi_tir_extended_payload)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();

	static const uint8_t want[5] = {0xDE, 0xAD, 0xBE, 0xEF, 0x5A};
	...
	rc = ibi_raise_and_get_rc("RAISE_TIR DEADBEEF5A");
	zassert_ok(rc, "target i3c_ibi_raise failed (%d)", rc);
	rc = k_sem_take(&ibi_sem, K_MSEC(500));
	zassert_ok(rc, "IBI with extended payload not received within 500ms");
	zassert_equal(last_ibi_len, sizeof(want), "payload len %u, want %zu", last_ibi_len,
		      sizeof(want));
	for (size_t i = 0; i < sizeof(want); i++) {
		zassert_equal(last_ibi_payload[i], want[i], "payload[%zu]=0x%02x want 0x%02x", i,
			      last_ibi_payload[i], want[i]);
	}
 ...
 */
ZTEST(dual_board_loopback, test_ibi_hot_join)
{
	ztest_test_skip();
	int rc;
	uint8_t probe = 0xC3;
	...
	printk("[hj-dbg] post-HJ: target_b.da=0x%02x (was 0x%02x), waited=%dms\n",
	       target_b_desc.dynamic_addr, old_da, waited_ms);
...
ZTEST(dual_board_loopback, test_ibi_controller_role_request)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();

	/* The target rejects its own MR request unless CR events are enabled. */
	struct i3c_ccc_events events = {.events = I3C_CCC_EVT_CR};
	int rc = i3c_ccc_do_events_set(&target_b_desc, true, &events);

	zassert_ok(rc, "ENEC CR failed (%d)", rc);

	rc = ibi_raise_and_get_rc("RAISE_MR");
	zassert_ok(rc, "target i3c_ibi_raise(MR) failed (%d)", rc);

	(void)i3c_ccc_do_events_set(&target_b_desc, false, &events);
	k_msleep(10);
}

ZTEST(dual_board_loopback, test_ibi_back_to_back)
{
	I3C_DUAL_BOARD_LOOPBACK_REQUIRE_TARGET_DA();
 ...
 */
ZTEST(dual_board_loopback, test_ibi_hot_join_stress)
{
	ztest_test_skip();
	const int iters = 50;
	int hj_response_rc;
...
ZTEST(dual_board_loopback, test_ibi_not_compiled)
{
	ztest_test_skip();
}
