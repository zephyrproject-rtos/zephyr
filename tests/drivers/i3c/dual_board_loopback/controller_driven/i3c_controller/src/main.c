...
/*
 * Placeholder descriptor for a permanently wired on-bus target some
 * benches have alongside the test boards.  It must be attached so the
...
#define BENCH_PERMANENT_TARGET_PID                                                                 \
	((((uint64_t)CONFIG_I3C_DUAL_BOARD_LOOPBACK_BENCH_PERMANENT_TARGET_PID_HI) << 32) |                   \
	 (uint64_t)CONFIG_I3C_DUAL_BOARD_LOOPBACK_BENCH_PERMANENT_TARGET_PID_LO)

struct i3c_device_desc bench_permanent_target_desc = {
	.bus = DEVICE_DT_GET(DT_ALIAS(test_i3c)),
	.dev = &i3c_peer_dev,
	.pid = BENCH_PERMANENT_TARGET_PID,
	.init_dynamic_addr = CONFIG_I3C_DUAL_BOARD_LOOPBACK_BENCH_PERMANENT_TARGET_INIT_DA,
	.flags = 0,
};
#endif /* CONFIG_I3C_DUAL_BOARD_LOOPBACK_BENCH_HAS_PERMANENT_TARGET */
...
ZTEST_SUITE(dual_board_loopback, NULL, suite_setup, suite_before_each, suite_after_each, suite_teardown);
