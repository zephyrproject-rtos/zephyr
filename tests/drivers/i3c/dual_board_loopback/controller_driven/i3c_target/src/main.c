 ...
 *
 * Reactive harness for the dual_board_loopback TARGET (Board B).
 *
...
LOG_MODULE_REGISTER(dual_board_loopback_target, LOG_LEVEL_INF);
 ...
 *
 * The controller drives staging over the sync UART (handle_stage_read)
 * and reads exactly the length it staged.
 *
 * Reading fewer bytes than were staged leaves the remainder in SLV_TX and
 * breaks the following transfer every time - measured over 10 attempts as
 * 9 x -ENXIO and 1 read that returned stale data with rc 0.  Reading more
 * than was staged is harmless.  A re-enumeration clears the residue.
 */
...
#ifdef CONFIG_I3C_DUAL_BOARD_LOOPBACK_TARGET_VENDOR_WRAPPER_GATE
	uint32_t ext_gate = sys_read32(base + CONFIG_I3C_DUAL_BOARD_LOOPBACK_TARGET_WRAPPER_GATE_OFFSET);
#else
...
static void handle_soft_rst(void)
{
	...
	/* Mask and clear interrupts before the reset, as dw_i3c_full_reset() does. */
	sys_write32(0x00000000U, base + 0x44U); /* INTR_SIGNAL_EN */
	...
	/* The core needs to settle before any further access; without this the
	 * wrapper-gate write below is intermittently swallowed.
	 */
	k_busy_wait(200);
	 ...
	 */
	sys_write32(CONFIG_I3C_DUAL_BOARD_LOOPBACK_TARGET_WRAPPER_GATE_VALUE,
		    base + CONFIG_I3C_DUAL_BOARD_LOOPBACK_TARGET_WRAPPER_GATE_OFFSET);
#endif

	/* Step 4: poll until SOFT_RST self-clears.  Only bit 0 is polled: the
	 * queue/FIFO flush bits clear on a different schedule.
	 */
	uint32_t timeout = 100000U;

	while ((sys_read32(base + 0x34U) & 0x01U) && --timeout) {
	}
	if (timeout == 0U) {
...
#ifndef CONFIG_I3C_DUAL_BOARD_LOOPBACK_TARGET_SOFT_RST_CLEARS_DEVICE_ADDR
	/*
