...
#define LAB_THRESHOLD ((double)CONFIG_LAB_THRESHOLD)
...
static void *test_pattern_setup(void)
{
	...
	struct video_control ctrl = {
		.id = VIDEO_CID_TEST_PATTERN, .val = 1,
	};
	size_t bsize;
	int ret;
	...
	zassert(bsize > 0, "size to allocate is zero");

	/* Alloc video buffers and enqueue for capture */
	zassert(caps.min_vbuf_count <= CONFIG_VIDEO_BUFFER_POOL_NUM_MAX,
		"not enough buffers");
	zassert(bsize <= CONFIG_VIDEO_BUFFER_POOL_SZ_MAX,
		"buffers too large");
...
ZTEST(test_pattern, test_pattern_10_frames)
{
	struct video_buffer *vbuf = &(struct video_buffer){
		.type = VIDEO_BUF_TYPE_OUTPUT
	};
	...
	for (int i = 0; i < 10; i++) {
		...
		zassert(is_colorbar_ok(vbuf->buffer, &fmt), "detecting color bar in the frame");
