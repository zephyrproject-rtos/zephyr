 ...
 * Copyright (c) 2026 Bhavin Umatiya
 *
...
#define SPIM_OP	 (SPI_OP_MODE_MASTER | SPI_MODE | SPI_HALF_DUPLEX)
#define SPIS_OP	 (SPI_OP_MODE_SLAVE | SPI_MODE | SPI_HALF_DUPLEX)

#define DUT_SPI_NODE DT_ALIAS(dut_spi_dt)
#define DUT_SPI_SLV_NODE DT_ALIAS(dut_spis)
static struct spi_dt_spec spim = SPI_DT_SPEC_GET(DUT_SPI_NODE, SPIM_OP);
static const struct device *spis_dev = DEVICE_DT_GET(DUT_SPI_SLV_NODE);
...
static uint8_t *buf_alloc(size_t len, bool is_spim)
{
	int *idx = is_spim ? &tdata.spim_alloc_idx : &tdata.spis_alloc_idx;
	uint8_t *buf = is_spim ? spim_buffer : spis_buffer;
	size_t total = is_spim ? sizeof(spim_buffer) : sizeof(spis_buffer);
	uint8_t *rv;
...
static void work_handler(struct k_work *work)
{
	...
	struct test_data *td = (struct test_data *)dwork;
	int rv;
