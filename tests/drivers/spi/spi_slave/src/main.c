...
static void run_test(bool m_same_size, bool s_same_size, bool async)
{
	...
	if (!async) {
		...
		rv = spi_transceive_signal(spis.bus, &spis.config, tdata.stx_set, tdata.srx_set,
					   &async_sig);
		if (rv == -ENOTSUP) {
			ztest_test_skip();
		}
...
static void before(void *not_used)
{
	...
	memset(spim_buffer, 0, sizeof(spim_buffer));
	memset(spis_buffer, 0, sizeof(spis_buffer));
