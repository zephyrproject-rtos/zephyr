...
#define SPIM_OP	 (SPI_OP_MODE_MASTER | SPI_MODE | DT_PROP_OR(DT_NODELABEL(dut_spi_dt), duplex, 0))
#define SPIS_OP	 (SPI_OP_MODE_SLAVE | SPI_MODE | DT_PROP_OR(DT_NODELABEL(dut_spis_dt), duplex, 0))
...
static const struct spi_config spis_config = {
	...
	.frequency = DT_PROP_OR(DT_NODELABEL(dut_spis_dt), spi_max_frequency, 0),
};
...
static void test_short_rx(bool async)
{
	...
	if ((SPIM_OP & SPI_HALF_DUPLEX) == SPI_HALF_DUPLEX) {
		TC_PRINT("Not tested in half duplex mode\n");
		ztest_test_skip();
	}
