...
ZTEST(xlnx_clk_wiz_device, test_device_is_ready)
{
	...
	zassert_true(device_is_ready(dev), "clock_control device is not ready");
}
...
ZTEST(xlnx_clk_wiz_on_off, test_on_clk0)
{
	...
	zassert_equal(ret, 0, "clock_control_on(clk0) failed: %d", ret);
}
...
ZTEST(xlnx_clk_wiz_on_off, test_on_clk_last)
{
	...
	zassert_equal(ret, 0, "clock_control_on(clk_last=%u) failed: %d", CLK_ID_LAST, ret);
}
...
ZTEST(xlnx_clk_wiz_on_off, test_off_clk0)
{
	...
	zassert_equal(ret, 0, "clock_control_off(clk0) failed: %d", ret);
}
...
ZTEST(xlnx_clk_wiz_on_off, test_off_clk_last)
{
	...
	zassert_equal(ret, 0, "clock_control_off(clk_last=%u) failed: %d", CLK_ID_LAST, ret);
}
...
ZTEST(xlnx_clk_wiz_get_rate, test_get_rate_clk0_nonzero)
{
	...
	zassert_equal(ret, 0, "clock_control_get_rate(clk0) failed: %d", ret);
	zassert_not_equal(rate, 0U, "clock_control_get_rate(clk0) returned 0 Hz");
}
...
ZTEST(xlnx_clk_wiz_get_rate, test_get_rate_all_valid_clks)
{
	...
	for (uint32_t id = 0U; id < NUM_OUT_CLKS; id++) {
		...
		zassert_equal(ret, 0, "clock_control_get_rate(clk%u) failed: %d", id, ret);
		zassert_not_equal(rate, 0U, "clock_control_get_rate(clk%u) returned 0 Hz", id);
	}
}
...
ZTEST(xlnx_clk_wiz_set_rate, test_set_rate_100mhz_clk0)
{
	...
	zassert_equal(ret, 0, "clock_control_set_rate(clk0, 100 MHz) failed: %d", ret);
}
...
ZTEST(xlnx_clk_wiz_set_rate, test_set_rate_200mhz_clk0)
{
	...
	ret = clock_control_set_rate(dev, CLK_SYS(CLK_ID_FIRST),
				     (clock_control_subsys_rate_t)&target);
	zassert_equal(ret, 0, "clock_control_set_rate(clk0, 200 MHz) failed: %d", ret);
}
...
ZTEST(xlnx_clk_wiz_set_rate, test_set_rate_300mhz_clk0)
{
	...
	ret = clock_control_set_rate(dev, CLK_SYS(CLK_ID_FIRST),
				     (clock_control_subsys_rate_t)&target);
	zassert_equal(ret, 0, "clock_control_set_rate(clk0, 300 MHz) failed: %d", ret);
}
...
static void roundtrip_check(uint32_t clk_id, uint32_t target_hz)
{
	...
	ret = clock_control_set_rate(dev, CLK_SYS(clk_id), (clock_control_subsys_rate_t)&target_hz);
	zassert_equal(ret, 0, "set_rate(clk%u, %u Hz) failed: %d", clk_id, target_hz, ret);

	ret = clock_control_get_rate(dev, CLK_SYS(clk_id), &actual);
	zassert_equal(ret, 0, "get_rate(clk%u) after set_rate failed: %d", clk_id, ret);
