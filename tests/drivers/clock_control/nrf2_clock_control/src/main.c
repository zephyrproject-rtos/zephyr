...
static void test_clock_control_request(const struct test_clk_context *clk_contexts, size_t contexts_size)
{
...
ZTEST(nrf2_clock_control, test_cpuapp_hsfll_control)
{
	...
	TC_PRINT("APP DOMAIN HSFLL test\n");
	/* Wait for the DVFS init to complete */
	k_msleep(3000);
	test_clock_control_request(cpuapp_hsfll_test_clk_contexts, ARRAY_SIZE(cpuapp_hsfll_test_clk_contexts));
