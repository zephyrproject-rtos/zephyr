	...
	if (DT_NUM_CLOCKS(DT_NODELABEL(adc1)) > 1) {
		...
		case STM32_SRC_HCLK:
			zassert_equal(dev_actual_clk_src, RCC_ADCCLKSOURCE_HCLK,
					"Expected ADC1 src: HCLK (0x%lx). Actual ADC1 src: 0x%x",
		...
		case STM32_SRC_HSI:
			...
			break;
		case STM32_SRC_I2S_CKIN:
			zassert_equal(dev_actual_clk_src, RCC_ADCCLKSOURCE_PIN,
					"Expected ADC1 src: I2S_CKIN (0x%lx). Actual ADC1 src: 0x%x",
					RCC_ADCCLKSOURCE_PIN, dev_actual_clk_src);
			break;
		case STM32_SRC_TIMG:
