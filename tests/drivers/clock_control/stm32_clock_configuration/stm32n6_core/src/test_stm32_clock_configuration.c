...
#else
	/* Case not expected */
	zassert_true(IS_ENABLED(STM32_SYSCLK_SRC_PLL) ||
		     IS_ENABLED(STM32_SYSCLK_SRC_HSE) ||
		     IS_ENABLED(STM32_SYSCLK_SRC_HSI) ||
...
ZTEST(stm32n6_clock_core_config, test_pll_src)
{
	...
	zassert_equal(RCC_PLLSOURCE_HSE, pll_src,
			...
			pll_src);
#elif STM32_PLL_SRC_I2S_CKIN
	zassert_equal(RCC_PLLSOURCE_I2S_CKIN, pll_src,
			"Expected PLL src: I2S_ckin. Actual PLL src: %d",
			pll_src);
#else
