...
#define ADC_CHILD_DEV(node_id) DEVICE_DT_GET(node_id),
#define ADC_CHILDREN(inst)     DT_INST_FOREACH_CHILD_STATUS_OKAY(inst, ADC_CHILD_DEV)

#define ADC_CHILD_VREF_MV(node_id) DT_PROP(DT_PARENT(node_id), vref_mv),
#define ADC_CHILDREN_VREF(inst)    DT_INST_FOREACH_CHILD_STATUS_OKAY(inst, ADC_CHILD_VREF_MV)
...
static const struct device *const adcs[] = {DT_INST_FOREACH_STATUS_OKAY(ADC_CHILDREN)};

#ifndef CONFIG_ADC_STM32_VREFINT_CALIBRATE
static const uint16_t adc_dt_vref_mv[] = {DT_INST_FOREACH_STATUS_OKAY(ADC_CHILDREN_VREF)};
#endif
