...
/*static int sector_mask_from_offset(const struct device *dev, off_t offset, size_t size,
				   uint64_t *mask)
{
	...
	*mask = ((1UL << (end_page.index + 1)) - 1) & ~((1UL << start_page.index) - 1);

	return 0;
}*/

static void *flash_npcx_setup(void)
{
	...
	/*TC_PRINT("Sector mask for offset 0x%x size 0x%x is 0x%llx\n", TEST_AREA_OFFSET,
		 EXPECTED_SIZE, sector_mask);*/

	/* Check if region is not write protected. */
	/*rc = flash_ex_op(flash_dev, FLASH_STM32_EX_OP_SECTOR_WP, (uintptr_t)NULL, &wp_status);
	...
	/*if (wp_status.protected_mask & sector_mask) {*/
		...
		rc = flash_ex_op(flash_dev, FLASH_NPCX_EX_OP_EXEC_UMA, (uintptr_t)&op_in, NULL);
...
static void flash_npcx_teardown(void *fixture)
{
	/*
...
ZTEST(flash_npcx, test_npcx_write_protection)
{
	struct npcx_ex_ops_uma_in op_in = {
...
/*ZTEST(flash_npcx, test_npcx_readout_protection_disabled)
{
	struct flash_stm32_ex_op_rdp rdp_status;
	int rc;

	rc = flash_ex_op(flash_dev, FLASH_STM32_EX_OP_RDP, (uintptr_t)NULL, &rdp_status);
...
ZTEST(flash_npcx, test_npcx_block_registers)
{
...
ZTEST_SUITE(flash_npcx, NULL, flash_npcx_setup, NULL, NULL, flash_npcx_teardown);
