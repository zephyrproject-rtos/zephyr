...
/*
 * APLIC MSI delivery mode and IMSIC tests.
 *
 * Only built when CONFIG_RISCV_APLIC_MSI is enabled (see CMakeLists.txt);
 * the EIID/GENMSI/MSIADDRCFG fields and the IMSIC interrupt files tested
 * here only exist when the APLIC domain delivers interrupts as MSIs.
 */
...
/* Test APLIC register address constants specific to MSI delivery mode */
ZTEST(intc_riscv_aia, test_aplic_msi_register_addresses)
{
