...
ZTEST(ina237_0, test_default_config)
{
	...
	zexpect_equal(0xFB68, config->adc_config, "0xFB68 != adc_config (0x%x)",
		      config->adc_config);
...
static void test_bus_voltage(struct ina237_fixture *fixture)
{
	...
	const int16_t voltage_reg_vectors[] = {
		32767, 27200, /* 85V maximum voltage */
		1000,  100,   1, 0,
	};
...
static void test_power(struct ina237_fixture *fixture)
{
	...
	const uint32_t power_reg_vectors[] = {
		16777215, 65535, 32767, 1000, 100, 1, 0,
	};
...
static void test_temperature(struct ina237_fixture *fixture)
{
	...
	const int16_t temp_reg_vectors[] = {
		16000,                                /* 125C */
		1000,  100, 1, 0, -100, -1000, -5120, /* -40C */
	};
...
static void test_vshunt(struct ina237_fixture *fixture)
{
	...
	const int16_t vshunt_reg_vectors[] = {
		...
		27200, 1000, 100,   1,      0,
		-1,    -100, -1000, -32768, /* minimum shunt voltage of -163.84 mV or -40.96 mV */
	};
...
static struct ina237_fixture fixtures[] = {DT_INST_FOREACH_STATUS_OKAY(INA237_FIXTURE_ENTRY)};

/* Create a test suite for each enabled ina237 device node */
#define INA237_TESTS(inst)                                                                         \
	ZTEST(ina237_##inst, test_shunt_cal)                                                       \
	{                                                                                          \
		test_shunt_cal(&fixtures[inst]);                                                   \
	}                                                                                          \
	ZTEST(ina237_##inst, test_current)                                                         \
	{                                                                                          \
		test_current(&fixtures[inst]);                                                     \
	}                                                                                          \
	ZTEST(ina237_##inst, test_bus_voltage)                                                     \
	{                                                                                          \
		test_bus_voltage(&fixtures[inst]);                                                 \
	}                                                                                          \
	ZTEST(ina237_##inst, test_power)                                                           \
	{                                                                                          \
		test_power(&fixtures[inst]);                                                       \
	}                                                                                          \
	ZTEST(ina237_##inst, test_temperature)                                                     \
	{                                                                                          \
		test_temperature(&fixtures[inst]);                                                 \
	}                                                                                          \
	ZTEST(ina237_##inst, test_vshunt)                                                          \
	{                                                                                          \
		test_vshunt(&fixtures[inst]);                                                      \
	}                                                                                          \
