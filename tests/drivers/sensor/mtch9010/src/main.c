...
#define TEST_STRING_MODE_CAPACITIVE "capacitive"
#define TEST_STRING_MODE_CONDUCTIVE "conductive"

#define TEST_STRING_OUTPUT_FORMAT_DELTA   "delta"
#define TEST_STRING_OUTPUT_FORMAT_CURRENT "current"
#define TEST_STRING_OUTPUT_FORMAT_BOTH    "both delta and current"
#define TEST_STRING_OUTPUT_FORMAT_DV      "mplab data visualizer"
...
ZTEST(mtch9010_utility, test_result_decode)
{
	...
	struct mtch9010_result_t testResult;
	...
	zassert_equal(testResult.measurement, 12345, "Decoded value does not match expected");
	...
	zassert_equal(testResult.delta, 10, "Decoded value does not match expected");

	/* Test Current and Delta decode */
	ret = mtch9010_decode_char_buffer(test_pattern_3, MTCH9010_OUTPUT_FORMAT_BOTH, &testResult);

	zassert_equal(ret, 0, "Unable to decode test_pattern_3");
	zassert_equal(testResult.prev_measurement, 12345, "Previous value does not match expected");
	zassert_equal(testResult.measurement, 999, "Decoded value does not match expected");
	zassert_equal(testResult.delta, 12405, "Decoded value does not match expected");

	/* Test MPLAB Data Visualizer Format (should fail) */
	ret = mtch9010_decode_char_buffer(
		test_pattern_4, MTCH9010_OUTPUT_FORMAT_MPLAB_DATA_VISUALIZER, &testResult);
...
/* Check configured operating mode */
ZTEST_F(mtch9010_config, test_config_mode)
{
	const struct mtch9010_config_t *config = fixture->dev->config;

	if (config->mode == MTCH9010_CAPACITIVE) {
		zassert_str_equal(DT_PROP(DUT_NODE, mtch9010_operating_mode),
				  TEST_STRING_MODE_CAPACITIVE,
				  "Conductive mode specified, but capacitive was set");
	} else if (config->mode == MTCH9010_CONDUCTIVE) {
		zassert_str_equal(DT_PROP(DUT_NODE, mtch9010_operating_mode),
				  TEST_STRING_MODE_CONDUCTIVE,
				  "Capacitive mode specified, but conductive was set");
	} else {
		zassert_unreachable("Unknown Operating Mode specified in DeviceTree");
	}
}
...
ZTEST_F(mtch9010_config, test_output_format)
{
	...
	if (config->extended_mode_enable) {
		...
		case MTCH9010_OUTPUT_FORMAT_DELTA: {
			zassert_str_equal(DT_PROP_OR(DUT_NODE, mtch9010_extended_output_format, ""),
					  TEST_STRING_OUTPUT_FORMAT_DELTA,
					  "Delta output mode specified, but was not set");
			break;
		}
		case MTCH9010_OUTPUT_FORMAT_CURRENT: {
			zassert_str_equal(DT_PROP_OR(DUT_NODE, mtch9010_extended_output_format, ""),
					  TEST_STRING_OUTPUT_FORMAT_CURRENT,
					  "Current output mode specified, but was not set");
			break;
		}
		case MTCH9010_OUTPUT_FORMAT_BOTH: {
			zassert_str_equal(
				DT_PROP_OR(DUT_NODE, mtch9010_extended_output_format, ""),
				TEST_STRING_OUTPUT_FORMAT_BOTH,
				"Both Delta & Current output mode specified, but was not set");
			break;
		}
		case MTCH9010_OUTPUT_FORMAT_MPLAB_DATA_VISUALIZER: {
			zassert_str_equal(
				DT_PROP_OR(DUT_NODE, mtch9010_extended_output_format, ""),
				TEST_STRING_OUTPUT_FORMAT_DV,
				"MPLAB Data Visualizer output mode specified, but was not set");
			break;
		}
		default: {
			zassert_unreachable("Unknown Output Format specified in DeviceTree");
			break;
		}
		}
	} else {
...
{
	const struct mtch9010_config_t *config = fixture->dev->config;
	const struct mtch9010_data_t *data = fixture->dev->data;
