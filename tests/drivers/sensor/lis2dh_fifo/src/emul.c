...
{
	struct lis2dh_test_bus *bus = target->data;
...
static int transfer(const struct emul *target, uint8_t reg, uint8_t *bytes, size_t len, bool read)
{
	struct lis2dh_test_bus *bus = target->data;
	const struct lis2dh_test_config *cfg = target->cfg;
...
static int transfer_i2c(const struct emul *target, struct i2c_msg *msgs, int num_msgs, int addr)
{
	struct lis2dh_test_bus *bus = target->data;
	...
	if (num_msgs == 1) {
		...
		return transfer(target, cmd & 0x7fU, msgs[0].buf + 1, 1, false);
	}
	...
	return transfer(target, cmd & 0x7fU, msgs[1].buf, msgs[1].len,
			(msgs[1].flags & I2C_MSG_READ) != 0U);
}

static int transfer_spi(const struct emul *target, const struct spi_config *cfg,
			const struct spi_buf_set *tx, const struct spi_buf_set *rx)
{
	...
	return transfer(target, cmd & 0x3fU, read ? rx->buffers[1].buf : tx->buffers[1].buf, len,
			read);
}

static int init(const struct emul *target, const struct device *parent)
{
	struct lis2dh_test_bus *bus = target->data;

	ARG_UNUSED(parent);
	bus->regs[LIS2DH_REG_WAI] = LIS2DH_CHIP_ID;
	lis2dh_test_reset(target);
	return 0;
}
