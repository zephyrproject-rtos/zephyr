...
struct ina228_fixture {
	...
	const struct device *dev_advanced;
	const struct emul *target_advanced;
};
...
static void *ina228_setup(void)
{
	static struct ina228_fixture fixture = {
		...
		.dev_advanced = DEVICE_DT_GET(DT_NODELABEL(ina228_advanced_test)),
		.target_basic = EMUL_DT_GET(DT_NODELABEL(ina228_default_test)),
		.target_advanced = EMUL_DT_GET(DT_NODELABEL(ina228_advanced_test)),
	};

	zassert_not_null(fixture.dev_basic);
	zassert_not_null(fixture.dev_advanced);
	zassert_not_null(fixture.target_basic);
	zassert_not_null(fixture.target_advanced);
...
static void ina228_before(void *f)
{
	...
	zassert_true(device_is_ready(fixture->dev_advanced), "I2C device %s is not ready",
		     fixture->dev_advanced->name);
}
...
ZTEST_F(ina228, test_example_from_data_sheet)
{
	...
	const uint16_t expected_shunt_cal = 0x0FD2;
	struct sensor_value sensor_val;
	double reading;

	ina228_emul_get_reg_16(fixture->target_basic, INA228_REG_CONFIG, &resulting_registervalue);
	/*
	 * RST		0	0... .... .... ....
	 * RSTACC	0	.0.. .... .... ....
	...
	ina228_emul_get_reg_16(fixture->target_basic, INA228_REG_ADC_CONFIG,
			       &resulting_registervalue);
	/*
	 * MODE		15	1111 .... .... ....
	 * VBUSCT	5	.... 101. .... ....
	...
	ina228_emul_get_reg_16(fixture->target_basic, INA228_REG_SHUNT_CAL,
			       &resulting_registervalue);
	resulting_shunt_cal = resulting_registervalue & 0x7FFF;
	zexpect_within(resulting_shunt_cal, expected_shunt_cal, 2,
		       "Expexted shunt calib setting 0x%04X, got 0x%04X", expected_shunt_cal,
		       resulting_shunt_cal);
	...
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VSHUNT, 0x4BF00 << 4);

	/* Datasheet value 314572 decimal */
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_CURRENT, 0x4CCCC << 4);

	/* Datasheet value 245760 decimal */
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VBUS, 0x3C000 << 4);

	/* Datasheet value 4718604 decimal */
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_POWER, 0x48000C);
	...
	ina228_emul_set_reg_16(fixture->target_basic, INA228_REG_DIETEMP, 0x0C80);
	...
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(6.0, reading, 1.0e-3, "Got %.6f A", reading);
	...
	zexpect_within(288, reading, 0.01, "Got %.6f W", reading);

	zassert_ok(sensor_channel_get(fixture->dev_basic, SENSOR_CHAN_CHARGE, &sensor_val));
	reading = sensor_value_to_double(&sensor_val);
	/* Some difference due to limited resolution in devicetree LSB setting */
	zexpect_within(21600, reading, 1, "Got %.6f C", reading);

	zassert_ok(sensor_channel_get(fixture->dev_basic, SENSOR_CHAN_ENERGY, &sensor_val));
	reading = sensor_value_to_double(&sensor_val);
	/* Some difference due to limited resolution in devicetree LSB setting */
	zexpect_within(1036800, reading, 50, "Got %.6f J", reading);
...
ZTEST_F(ina228, test_negative_values)
{
	...
	/* Current LSB = 19073 nA according to devicetree settings.
	 * -100 = 0xFFF9C in 20 bit two's complement
	 * This corresponds to -1.907 mA
	 */
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_CURRENT, 0xFFF9C << 4);

	/* Charge LSB = 19073 nC (same as devicetree setting for current LSB).
	 * -100 = 0xFFFFFFFF9C in 40 bit two's complement
	 * This corresponds to -1.907 mC
	 */
	 ...
	 */
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VSHUNT, 0xFFF9C << 4);

	/* Temperature LSB = 7.8125 mdegC
	 * -100 = 0xFF9C in 20 bit two's complement
	 * This corresponds to -0.78125 deg C
	 */
	ina228_emul_set_reg_16(fixture->target_basic, INA228_REG_DIETEMP, 0xFF9C);

	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VBUS, 0x000000);
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_POWER, 0x000000);
	...
	zassert_ok(sensor_channel_get(fixture->dev_basic, SENSOR_CHAN_CURRENT, &sensor_val));
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(-1.907e-3, reading, 1.0e-6, "Got %.6f A", reading);

	zassert_ok(sensor_channel_get(fixture->dev_basic, SENSOR_CHAN_CHARGE, &sensor_val));
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(-1.907e-3, reading, 1.0e-6, "Got %.6f C", reading);
...
ZTEST_F(ina228, test_invalid_channel)
{
	...
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_CURRENT, 0x0000);
	ina228_emul_set_reg_40(fixture->target_basic, INA228_REG_CHARGE, 0x0000000000);
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VSHUNT, 0x00000);
	ina228_emul_set_reg_16(fixture->target_basic, INA228_REG_DIETEMP, 0x0000);
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_VBUS, 0x000000);
	ina228_emul_set_reg_24(fixture->target_basic, INA228_REG_POWER, 0x000000);
	ina228_emul_set_reg_40(fixture->target_basic, INA228_REG_ENERGY, 0x0000000000);
...
/**
 * @brief Test device tree settings
 *
 * Use other settings in the device tree, and verify that correct values are
 * written to the configuration registers.
 */
ZTEST_F(ina228, test_other_devicetree_settings)
{
	uint16_t resulting_registervalue;
	uint16_t resulting_shunt_cal;
	uint16_t expected_shunt_cal;
	uint16_t resulting_tempco;
	uint16_t expected_tempco;

	ina228_emul_get_reg_16(fixture->target_advanced, INA228_REG_CONFIG,
			       &resulting_registervalue);
	/*
	 * RST		0	0... .... .... ....
	 * RSTACC	0	.0.. .... .... ....
	 * CONVDLY	150	..10 0101 10.. ....
	 * TEMPCOMP	1	.... .... ..1. ....
	 * ADCRANGE	1	.... .... ...1 ....
	 */
	zexpect_equal(resulting_registervalue, 0x25B0, "CONFIG: got 0x%04X",
		      resulting_registervalue);

	ina228_emul_get_reg_16(fixture->target_advanced, INA228_REG_ADC_CONFIG,
			       &resulting_registervalue);
	/*
	 * MODE		11	1011 .... .... ....
	 * VBUSCT	0	.... 000. .... ....
	 ...
	 * AVG		3	.... .... .... .011
	 */
	zexpect_equal(resulting_registervalue, 0xB1D3, "ADC_CONFIG: got 0x%04X",
		      resulting_registervalue);

	ina228_emul_get_reg_16(fixture->target_advanced, INA228_REG_SHUNT_CAL,
			       &resulting_registervalue);
	resulting_shunt_cal = resulting_registervalue & 0x7FFF;
	/* 10000 x 20000 x 4 x 1.31072e-5 = 10485.76 */
	expected_shunt_cal = 0x28F5;
	zexpect_within(resulting_shunt_cal, expected_shunt_cal, 2,
		       "Expexted shunt calib setting 0x%04X, got 0x%04X", expected_shunt_cal,
		       resulting_shunt_cal);

	ina228_emul_get_reg_16(fixture->target_advanced, INA228_REG_SHUNT_TEMPCO,
			       &resulting_registervalue);
	resulting_tempco = resulting_registervalue & 0x3FFF;
	/* 1000 ppm */
	expected_tempco = 0x3E8;
	zexpect_equal(resulting_tempco, expected_tempco,
		      "Expexted tempco setting 0x%04X, got 0x%04X", expected_tempco,
		      resulting_tempco);
}
