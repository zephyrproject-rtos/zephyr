...
ZTEST_F(ina228, test_negative_values)
{
	...
	zassert_ok(sensor_read_and_decode(fixture->dev_basic, channels, ARRAY_SIZE(channels),
					  &shift, value, 4));

	q31_to_sensor_value(value[0], shift, &sensor_val);
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(-1.907e-3, reading, 1.0e-5, "Got %.6f A", reading);

	q31_to_sensor_value(value[1], shift, &sensor_val);
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(-1.907e-3, reading, 1.0e-5, "Got %.6f C", reading);

	q31_to_sensor_value(value[2], shift, &sensor_val);
	reading = sensor_value_to_double(&sensor_val);
	zexpect_within(-0.78125, reading, 1.0e-3, "Got %.3f deg C", reading);

	q31_to_sensor_value(value[3], shift, &sensor_val);
	reading = sensor_value_to_double(&sensor_val);
