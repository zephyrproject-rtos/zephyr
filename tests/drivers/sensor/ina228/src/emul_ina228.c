...
LOG_MODULE_DECLARE(INA228, CONFIG_SENSOR_LOG_LEVEL);
				    ...
				    int addr)
{
	...
	if (num_msgs == 1) {
		...
		if (expected_register_size != sizeof(uint16_t)) {
			LOG_ERR("Illegal to write to register %u as it has size %u bytes", reg_addr,
	...
	read_len = msgs[1].len;
	reg_addr = msgs[0].buf[0];
	if ((reg_addr >= NUM_REGS) && (reg_addr != INA228_REG_MANUFACTURER_ID) &&
	    (reg_addr != INA228_REG_DEVICE_ID)) {
		LOG_ERR("Invalid register address for read: %u", reg_addr);
		return -EIO;
	}

	if (reg_addr == INA228_REG_MANUFACTURER_ID) {
