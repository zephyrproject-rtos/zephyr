...
/*
 * die-temperature timeout path. With self-clear disabled the TEMP_EN bit is held
 * set, so the driver polls the full wait window without ever seeing the
 * measurement complete and the fetch fails with -ETIMEDOUT.
 */
ZTEST_F(maxm86161, test_die_temp_hold)
{
	...
	/* The poll loop runs to its limit and reports a timeout. */
	zassert_equal(sensor_sample_fetch_chan(fixture->dev, SENSOR_CHAN_DIE_TEMP), -ETIMEDOUT,
		      "Expected -ETIMEDOUT when TEMP_EN never clears");
