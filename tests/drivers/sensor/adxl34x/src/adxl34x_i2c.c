 ...
 */
#define adxl34x_i2c_fixture adxl34x_fixture

LOG_MODULE_DECLARE(adxl34x_test, CONFIG_SENSOR_LOG_LEVEL);

static void adxl34x_i2c_suite_before(void *fixture)
{
	 ...
	 */
	Z_TEST_SKIP_IFNDEF(ADXL34X_TEST_I2C);
 ...
 * test verifies the devices which support i2c are available. The spi devices are not defined in
 ...
 * @see device_is_ready()
 */
ZTEST_USER_F(adxl34x_i2c, test_device_is_ready_for_i2c_tests)
{
	/* The devices below should be able to be used in these tests. For this build the overlay
	 * was used which defines only the i2c devices, so the spi devices are not used here.
	 */
...
ZTEST_SUITE(adxl34x_i2c, NULL, adxl34x_suite_setup, adxl34x_i2c_suite_before, NULL, NULL);
