 ...
 */
#define adxl34x_extended_fixture adxl34x_fixture

LOG_MODULE_DECLARE(adxl34x_test, CONFIG_SENSOR_LOG_LEVEL);

static void adxl34x_extended_suite_before(void *fixture)
{
	 ...
	 */
	Z_TEST_SKIP_IFNDEF(ADXL34X_TEST_EXTENDED);
 ...
 * test verifies the devices which support extended functionality are available. Because this
 ...
 * @see device_is_ready()
 */
ZTEST_USER_F(adxl34x_extended, test_device_is_ready_for_extended_tests)
{
...
ZTEST_SUITE(adxl34x_extended, NULL, adxl34x_suite_setup, adxl34x_extended_suite_before, NULL, NULL);
