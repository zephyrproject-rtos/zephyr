...
ZTEST(lis2dh_decoder, test_frame_count_bounds_and_has_trigger)
{
	uint8_t buffer[sizeof(struct lis2dh_encoded_header) +
		       FIFO_SAMPLES * LIS2DH_ENCODED_SAMPLE_SIZE] __aligned(8) = {0};
	uint8_t decoded[sizeof(struct sensor_three_axis_data) +
			(FIFO_SAMPLES - 1U) * sizeof(struct sensor_three_axis_sample_data)]
		__aligned(8);
	struct lis2dh_encoded_header *header = (void *)buffer;
	...
	/* A one-shot buffer is never reported as a FIFO trigger source. */
	header->is_fifo = 0U;
	header->trigger = SENSOR_TRIG_FIFO_FULL;
	zassert_false(decoder->has_trigger(buffer, SENSOR_TRIG_FIFO_FULL));
	zassert_false(decoder->has_trigger(buffer, SENSOR_TRIG_FIFO_WATERMARK));

	/* One frame below the hardware maximum must still decode cleanly. */
	header->is_fifo = 1U;
	header->trigger = SENSOR_TRIG_FIFO_WATERMARK;
	header->sample_count = FIFO_SAMPLES - 1U;
	header->period_ns = 100U;
	header->timestamp_ns = 3100U;
	header->scale = 100000U;
	header->shift = 5;
	for (size_t i = 0U; i < FIFO_SAMPLES - 1U; i++) {
		put_sample(buffer, i, (int16_t)((i + 1U) << 4), 0, 0);
	}

	zassert_ok(decoder->get_frame_count(buffer, channel, &frame_count));
	zassert_equal(frame_count, FIFO_SAMPLES - 1U);
	zassert_equal(decoder->decode(buffer, channel, &fit, FIFO_SAMPLES - 1U, data),
		      FIFO_SAMPLES - 1U);
	zassert_equal(fit, FIFO_SAMPLES - 1U);
	zassert_equal(data->readings[FIFO_SAMPLES - 2U].timestamp_delta, 3000U);

	/* The count field cannot exceed the hardware FIFO depth. */
	header->sample_count = FIFO_SAMPLES + 1U;
	zassert_equal(decoder->get_frame_count(buffer, channel, &frame_count), -EINVAL);
	zassert_equal(decoder->decode(buffer, channel, &fit, 1U, data), -EINVAL);
}
