...
static int configure_with_mask(const struct device *dev, enum i2s_dir dir, uint32_t mask,
			       size_t block_size)
{
	int ret;
	...
	for (size_t i = 0; i + offset < word_count; i++) {
		...
		if (buf[i + offset] != expected) {
			TC_PRINT("mismatch at word %u: expected 0x%04x got 0x%04x\n",
				 (unsigned int)i, (uint16_t)expected, (uint16_t)buf[i + offset]);
			return -TC_FAIL;
		}
	}
	return TC_PASS;
...
/* Drivers without channel masking support reject a nonzero mask at configure. */
bool tdm_channel_masking_supported(const void *global_state)
{
	ARG_UNUSED(global_state);

	return device_is_ready(dev_i2s) &&
	       configure_with_mask(dev_i2s, I2S_DIR_TX, BIT(0), BLOCK_SIZE) == 0;
}
...
/** @brief Disabling every channel must be rejected at configure.
 *
 * - i2s_configure() returns -EINVAL for TX and for RX when channel_disable_mask
...
ZTEST_USER(tdm_channel_masking, test_channel_mask_all_disabled_rejected)
{
	...
	ret = configure_with_mask(dev_i2s, I2S_DIR_TX, BIT_MASK(2), BLOCK_SIZE);
	zassert_equal(ret, -EINVAL, "all-disabled TX mask should return -EINVAL, got %d", ret);

	ret = configure_with_mask(dev_i2s, I2S_DIR_RX, BIT_MASK(2), BLOCK_SIZE);
	zassert_equal(ret, -EINVAL, "all-disabled RX mask should return -EINVAL, got %d", ret);
}
...
/** @brief Reconfiguring one direction drops its previous channel slots.
 *
 * - A duplex transfer with all slots enabled, then an I2S_DIR_TX-only
 *   configure that disables one TX slot.
 * - RX still receives a full block, so no stale TX slot stayed enabled.
 */
ZTEST_USER(tdm_channel_masking, test_channel_mask_reconfigure_single_dir)
{
	int ret;
	size_t rx_size;
	char tx_buf[BLOCK_SIZE] = {0};
	...
	ret = configure_with_mask(dev_i2s, I2S_DIR_BOTH, 0, BLOCK_SIZE);
	zassert_equal(ret, 0, "configure failed");
	...
	ret = configure_with_mask(dev_i2s, I2S_DIR_TX, BIT(1), BLOCK_SIZE / 2);
	zassert_equal(ret, 0, "TX configure with mask failed");

	ret = i2s_buf_write(dev_i2s, tx_buf, BLOCK_SIZE / 2);
	zassert_equal(ret, 0, "TX write failed");
