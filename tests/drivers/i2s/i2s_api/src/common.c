...
ZTEST_DMEM int32_t data_l[SAMPLE_NO] = {
...
ZTEST_DMEM int32_t data_r[SAMPLE_NO] = {
...
static void fill_buf(int32_t *tx_block, int att)
{
...
static int verify_buf(int32_t *rx_block, int att)
{
			       ...
			       struct k_mem_slab *slab)
{
	...
	fill_buf((uint32_t *)tx_block, att);
...
static int rx_block_read_slab(const struct device *dev_i2s, int att, struct k_mem_slab *slab)
{
	...
	ret = verify_buf((uint32_t *)rx_block, att);
	if (ret < 0) {
...
int configure_stream(const struct device *dev_i2s, enum i2s_dir dir)
{
	...
	i2s_cfg.word_size = (uint8_t)CONFIG_I2S_TEST_WORD_SIZE;
	i2s_cfg.channels = 2U;
	i2s_cfg.format = (uint8_t)CONFIG_I2S_TEST_DATA_FORMAT;
	i2s_cfg.frame_clk_freq = FRAME_CLK_FREQ;
	i2s_cfg.block_size = BLOCK_SIZE;
	i2s_cfg.timeout = TIMEOUT;

	TC_PRINT("Word size: %d, frame clock: %d, data format: %d\n", i2s_cfg.word_size,
		 i2s_cfg.frame_clk_freq, i2s_cfg.format);
