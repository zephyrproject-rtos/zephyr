...
ZTEST(cordic_api, test_phase)
{
	...
	const float x[] = {1.0f};
	const float y[] = {0.5f};
	int args[2] = {F32_TO_Q31(x[0]), F32_TO_Q31(y[0])};
	int res[2];
	...
	ret = cordic_compute(dev, args, args_len);
	zassert_equal(ret, 0, "cordic_compute failed: %d", ret);

	ret = cordic_read_result(dev, res, res_len);
	...
	args[0] = F32_TO_Q15(x[0]);
	args[1] = F32_TO_Q15(y[0]);
	ret = cordic_compute(dev, args, args_len);
	zassert_equal(ret, 0, "cordic_compute failed: %d", ret);
