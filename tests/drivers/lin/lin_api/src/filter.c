...
static int test_lin_filter_test_startup(void)
{
	...
	ret = lin_set_event_callback(lin_commander, lin_filter_test_commander_callback, NULL);
	if (ret != 0) {
		return ret;
	}

	/* Set responder event handler */
	ret = lin_set_event_callback(lin_responder, lin_filter_test_responder_callback, NULL);
	if (ret != 0) {
		return ret;
	}
