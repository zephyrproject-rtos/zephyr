...
static void lin_filter_test_slave_callback(const struct device *dev, const struct lin_event *event,
					   void *user_data)
{
	ARG_UNUSED(dev);
...
static void lin_filter_test_master_callback(const struct device *dev, const struct lin_event *event,
					    void *user_data)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(user_data);
...
static int test_lin_filter_test_startup(void)
{
	...
	ret = lin_set_callback(lin_master, lin_filter_test_master_callback, NULL);
	if (ret != 0) {
		return ret;
	}

	/* Set slave event handler */
	ret = lin_set_callback(lin_slave, lin_filter_test_slave_callback, NULL);
	if (ret != 0) {
		return ret;
	}

	/* Configure LIN master and slave nodes */
...
static int test_lin_filter_set(uint8_t frame_id, uint8_t mask)
{
	...
	return lin_set_rx_filter(lin_slave, &filter);
}
...
static int test_lin_header_send(uint8_t frame_id)
{
	...
	return lin_send(lin_master, &msg, K_FOREVER);
}
