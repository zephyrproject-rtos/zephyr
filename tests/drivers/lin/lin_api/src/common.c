...
ZTEST_DMEM const struct device *const lin_master = DEVICE_DT_GET(LIN_MASTER);
ZTEST_DMEM const struct device *const lin_slave = DEVICE_DT_GET(LIN_SLAVE);
ZTEST_DMEM const struct device *const master_phy =
	DEVICE_DT_GET_OR_NULL(DT_PHANDLE(LIN_MASTER, phys));

ZTEST_DMEM const struct lin_config master_cfg = {
	.mode = LIN_MODE_COMMANDER,
	.baudrate = LIN_BUS_BAUDRATE,
	.break_len = LIN_BUS_BREAK_LEN_MASTER,
	.break_delimiter_len = LIN_BUS_BREAK_DELIMITER_LEN,
	.flags = 0,
};

ZTEST_DMEM const struct lin_config slave_cfg = {
	.mode = LIN_MODE_RESPONDER,
	.baudrate = LIN_BUS_BAUDRATE,
	.break_len = LIN_BUS_BREAK_LEN_SLAVE,
	.break_delimiter_len = LIN_BUS_BREAK_DELIMITER_LEN,
...
ZTEST_DMEM struct lin_msg master_msg;
ZTEST_DMEM struct lin_msg slave_msg;
