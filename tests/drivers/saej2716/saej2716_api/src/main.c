 ...
 */
ZTEST(saej2716_api, test_register_callback)
{
	int err;

	err = saej2716_register_callback(dev, SAEJ2716_CHANNEL, rx_serial_frame_cb,
					 rx_fast_frame_cb, NULL);
	zassert_equal(err, 0, "Failed to set rx callback (err %d)", err);

	saej2716_register_callback(dev, SAEJ2716_CHANNEL, NULL, NULL, NULL);
	zassert_equal(err, 0, "Failed to set rx callback (err %d)", err);
