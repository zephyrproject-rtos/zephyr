...
ZTEST(opamp_stm32, test_init_opamp)
{
	...
	/* Expect TRIM values not being zero in any case */
	zassert_not_equal(LL_OPAMP_GetTrimmingValue(OPAMP_LL_DEV, LL_OPAMP_TRIMMING_NMOS), 0,
			  "%s: OPAMP's N-MOS TRIM value shall not be zero after init",
			  opamp_dev->name);
	zassert_not_equal(LL_OPAMP_GetTrimmingValue(OPAMP_LL_DEV, LL_OPAMP_TRIMMING_PMOS), 0,
			  "%s: OPAMP's P-MOS TRIM value shall not be zero after init",
			  opamp_dev->name);
