...
static void *counter_pico_pit_setup(void)
{
	static struct counter_pico_pit_fixture fixture = {
		...
		.pit_channel_1 = DEVICE_DT_GET(DT_NODELABEL(rpi_pico_pit_channel)),
...
static void counter_pico_pit_before(void *f)
{
	...
	counter_set_top_value(fixture->pit_channel_2, &top_cfg);
	counter_stop(fixture->pit_channel_1);
	counter_stop(fixture->pit_channel_2);
}
...
ZTEST_F(counter_pico_pit, test_value_increase_over_time)
{
	...
	k_busy_wait(50000);
	(void)counter_get_value(fixture->pit_channel_1, &value_2);
	zassert_true(value_1 < value_2, "Counter value has incresed (%u should be greater than %u)",
		     value_2, value_1);
}
...
ZTEST_F(counter_pico_pit, test_two_pit_channels_top_interrupts)
{
	...
	counter_set_top_value(fixture->pit_channel_2, &top_cfg_2);
