...
ZTEST(memc_ram_api, test_get_size)
{
	const struct device *dev = MEMC_DEV;
	uint64_t size = 0;
	int rc;

	rc = memc_get_size(dev, &size);
	if (rc == -ENOTSUP) {
		ztest_test_skip();
	}

	zassert_ok(rc, "memc_get_size failed: %d", rc);
	zassert_true(size > 0, "reported size is zero");
	zassert_true(OFFSET_B + BUF_SIZE <= size,
		     "test offset exceeds device size");
}

ZTEST(memc_ram_api, test_read_id)
{
	const struct device *dev = MEMC_DEV;
	uint8_t id[1] = {0};
	int rc;

	rc = memc_read_id(dev, id, sizeof(id));
	if (rc == -ENOTSUP) {
		ztest_test_skip();
	}

	zassert_ok(rc, "memc_read_id failed: %d", rc);
	zassert_not_equal(id[0], 0x00, "vendor ID is zero");
	zassert_not_equal(id[0], 0xFF, "vendor ID is 0xFF (no device?)");
}

ZTEST(memc_ram_api, test_get_mem_base)
{
	const struct device *dev = MEMC_DEV;

	/* NULL = bus-transaction, !NULL = memory-mapped. Both valid. */
	(void)memc_get_mem_base(dev);
}
