...
LOG_MODULE_REGISTER(stepper_ctrl_api, CONFIG_STEPPER_LOG_LEVEL);

struct stepper_ctrl_fixture {
...
static void stepper_print_event_callback(const struct device *dev, enum stepper_ctrl_event event,
					 void *user_data)
{
	...
	case STEPPER_CTRL_EVENT_STEPS_COMPLETED:
		k_poll_signal_raise(&stepper_signal, STEPPER_CTRL_EVENT_STEPS_COMPLETED);
		break;
	case STEPPER_CTRL_EVENT_LEFT_END_STOP_DETECTED:
		k_poll_signal_raise(&stepper_signal, STEPPER_CTRL_EVENT_LEFT_END_STOP_DETECTED);
		break;
	case STEPPER_CTRL_EVENT_RIGHT_END_STOP_DETECTED:
		k_poll_signal_raise(&stepper_signal, STEPPER_CTRL_EVENT_RIGHT_END_STOP_DETECTED);
		break;
	case STEPPER_CTRL_EVENT_STOPPED:
		k_poll_signal_raise(&stepper_signal, STEPPER_CTRL_EVENT_STOPPED);
		break;
	default:
		break;
	}
...
static void *stepper_ctrl_setup(void)
{
	static struct stepper_ctrl_fixture fixture = {
	...
	zassert_equal(stepper_ctrl_set_event_cb(fixture.dev, fixture.callback,
						       (void *)fixture.dev),
...
{
	int err = stepper_ctrl_set_microstep_interval(fixture->dev, 0);
	if (err == -ENOSYS) {
		ztest_test_skip();
	}
...
ZTEST_F(stepper_ctrl, test_actual_position)
{
	int32_t pos = 100u;
	int ret;

	ret = stepper_ctrl_set_reference_position(fixture->dev, pos);
	zassert_equal(ret, 0, "Failed to set reference position");

	ret = stepper_ctrl_get_actual_position(fixture->dev, &pos);
	zassert_equal(ret, 0, "Failed to get actual position");
	zassert_equal(pos, 100u, "Actual position not set correctly");
}

ZTEST_F(stepper_ctrl, test_move_to_positive_step_count)
{
	int32_t pos = 10u;
	...
	ret = stepper_ctrl_set_microstep_interval(fixture->dev, 100 * USEC_PER_SEC);
	if (ret == -ENOSYS) {
		ztest_test_skip();
	}

	zassert_ok(stepper_ctrl_move_to(fixture->dev, pos));
	...
	POLL_AND_CHECK_SIGNAL(
		stepper_signal, stepper_ctrl_event, STEPPER_CTRL_EVENT_STEPS_COMPLETED,
		K_MSEC(pos * (100 + CONFIG_STEPPER_TEST_TIMING_TIMEOUT_TOLERANCE_PCT)));

	zassert_ok(stepper_ctrl_get_actual_position(fixture->dev, &actual_steps));
...
ZTEST_F(stepper_ctrl, test_move_to_negative_step_count)
{
	int32_t pos = -10;
	int32_t actual_steps;
	bool moving = false;
...
ZTEST_F(stepper_ctrl, test_move_by_positive_step_count)
{
	int32_t steps = 20;
	int32_t actual_steps;
	bool moving = false;
	...
	zassert_ok(stepper_ctrl_move_by(fixture->dev, steps));
	zassert_ok(stepper_ctrl_is_moving(fixture->dev, &moving));
...
ZTEST_F(stepper_ctrl, test_move_by_negative_step_count)
{
	int32_t steps = -20;
	int32_t actual_steps;
	bool moving = false;
...
ZTEST_F(stepper_ctrl, test_run_positive_direction)
{
	uint64_t step_interval = 20000000;
	int32_t actual_steps = 0;
	int ret;

	ret = stepper_ctrl_set_microstep_interval(fixture->dev, step_interval);
	if (ret == -ENOSYS) {
		ztest_test_skip();
	}
...
ZTEST_F(stepper_ctrl, test_run_negative_direction)
{
	uint64_t step_interval = 20000000;
	int32_t actual_steps = 0;
	int ret;
...
ZTEST_F(stepper_ctrl, test_stop)
{
	...
	if (ret == 0) {
		...
		stepper_ctrl_is_moving(fixture->dev, &is_moving);
...
ZTEST_F(stepper_ctrl, test_move_zero_steps)
{
	bool moving;
	int32_t pos = 0;
	int32_t actual_steps;
	...
	err = stepper_ctrl_set_microstep_interval(fixture->dev, 100 * USEC_PER_SEC);
	if (err == -ENOSYS) {
		ztest_test_skip();
	}
	zassert_ok(err, "Failed to set microstep interval");

	zassert_ok(stepper_ctrl_move_by(fixture->dev, pos));
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_ctrl_event,
			      STEPPER_CTRL_EVENT_STEPS_COMPLETED, K_NO_WAIT);
	zassert_ok(stepper_ctrl_is_moving(fixture->dev, &moving));
	zassert_false(moving, "%s reported moving even after completion of steps",
		      fixture->dev->name);
	zassert_ok(stepper_ctrl_move_to(fixture->dev, pos));
