...
struct k_poll_event stepper_motor_event;
...
static void stepper_print_event_callback(const struct device *dev, enum stepper_motor_event event,
					 void *user_data)
{
	...
	case STEPPER_MOTOR_EVENT_STEPS_COMPLETED:
		k_poll_signal_raise(&stepper_signal, STEPPER_MOTOR_EVENT_STEPS_COMPLETED);
		break;
	case STEPPER_MOTOR_EVENT_LEFT_END_STOP_DETECTED:
		k_poll_signal_raise(&stepper_signal, STEPPER_MOTOR_EVENT_LEFT_END_STOP_DETECTED);
		break;
	case STEPPER_MOTOR_EVENT_RIGHT_END_STOP_DETECTED:
		k_poll_signal_raise(&stepper_signal, STEPPER_MOTOR_EVENT_RIGHT_END_STOP_DETECTED);
		break;
	case STEPPER_MOTOR_EVENT_STOPPED:
		k_poll_signal_raise(&stepper_signal, STEPPER_MOTOR_EVENT_STOPPED);
		break;
	default:
		break;
	}
...
static void *stepper_setup(void)
{
	...
	k_poll_event_init(&stepper_motor_event, K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY,
			  &stepper_signal);

	zassert_not_null(fixture.dev);
	zassert_equal(stepper_motor_set_event_callback(fixture.dev, fixture.callback,
						       (void *)fixture.dev),
...
static void stepper_before(void *f)
{
	...
	(void)stepper_motor_set_reference_position(fixture->dev, 0);
...
{
	int err = stepper_motor_set_microstep_interval(fixture->dev, 0);
	if (err == -ENOSYS) {
		ztest_test_skip();
	}
...
ZTEST_F(stepper, test_actual_position)
{
	...
	ret = stepper_motor_set_reference_position(fixture->dev, pos);
	zassert_equal(ret, 0, "Failed to set reference position");

	ret = stepper_motor_get_actual_position(fixture->dev, &pos);
...
ZTEST_F(stepper, test_move_to_positive_step_count)
{
	...
	ret = stepper_motor_set_microstep_interval(fixture->dev, 100 * USEC_PER_SEC);
	if (ret == -ENOSYS) {
		ztest_test_skip();
	}

	zassert_ok(stepper_motor_move_to(fixture->dev, pos));
	zassert_ok(stepper_motor_is_moving(fixture->dev, &moving));
	zassert_true(moving, "%s reported not moving after move_to", fixture->dev->name);

	POLL_AND_CHECK_SIGNAL(
		stepper_signal, stepper_motor_event, STEPPER_MOTOR_EVENT_STEPS_COMPLETED,
		K_MSEC(pos * (100 + CONFIG_STEPPER_TEST_TIMING_TIMEOUT_TOLERANCE_PCT)));

	zassert_ok(stepper_motor_get_actual_position(fixture->dev, &actual_steps));
...
ZTEST_F(stepper, test_move_by_positive_step_count)
{
	...
	zassert_ok(stepper_motor_move_by(fixture->dev, steps));
	zassert_ok(stepper_motor_is_moving(fixture->dev, &moving));
...
ZTEST_F(stepper, test_run_positive_direction)
{
	...
	ret = stepper_motor_set_microstep_interval(fixture->dev, step_interval);
	if (ret == -ENOSYS) {
		ztest_test_skip();
	}

	zassert_ok(stepper_motor_run(fixture->dev, STEPPER_MOTOR_DIRECTION_POSITIVE));
	k_usleep(110000);
...
ZTEST_F(stepper, test_run_negative_direction)
{
	...
	zassert_ok(stepper_motor_run(fixture->dev, STEPPER_MOTOR_DIRECTION_NEGATIVE));
	k_usleep(110000);
...
ZTEST_F(stepper, test_stop)
{
	...
	(void)stepper_motor_run(fixture->dev, STEPPER_MOTOR_DIRECTION_POSITIVE);

	/* Stop the stepper */
	int ret = stepper_motor_stop(fixture->dev);
	bool is_moving;

	if (ret == 0) {
		POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_motor_event,
				      STEPPER_MOTOR_EVENT_STOPPED, K_NO_WAIT);
		zassert_equal(user_data_received, fixture->dev, "User data not received");

		/* Check if the stepper is stopped */
		stepper_motor_is_moving(fixture->dev, &is_moving);
...
ZTEST_F(stepper, test_move_zero_steps)
{
	...
	err = stepper_motor_set_microstep_interval(fixture->dev, 100 * USEC_PER_SEC);
	if (err == -ENOSYS) {
		ztest_test_skip();
	}
	zassert_ok(err, "Failed to set microstep interval");

	zassert_ok(stepper_motor_move_by(fixture->dev, pos));
	POLL_AND_CHECK_SIGNAL(stepper_signal, stepper_motor_event,
			      STEPPER_MOTOR_EVENT_STEPS_COMPLETED, K_NO_WAIT);
	zassert_ok(stepper_motor_is_moving(fixture->dev, &moving));
	zassert_false(moving, "%s reported moving even after completion of steps",
		      fixture->dev->name);
	zassert_ok(stepper_motor_move_to(fixture->dev, pos));
