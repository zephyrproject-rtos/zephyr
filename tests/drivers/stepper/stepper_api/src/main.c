...
#define RUNTIME                                                                                    \
	(K_MSEC(1000 +                                                                             \
		DIV_ROUND_UP(2*MSEC_PER_SEC, CONFIG_USTEPS_PER_SECOND))) /* 1sec + time 2 steps*/
...
ZTEST_F(stepper_api, test_get_micro_step_res)
{
	...
	zassert_equal(res, DT_PROP(DT_ALIAS(stepper), micro_step_res),
		      "Micro step resolution not set correctly, should be %u but is %u",
		      DT_PROP(DT_ALIAS(stepper), micro_step_res), res);
}
...
ZTEST_F(stepper_api, test_move_to_positive_direction_movement)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_move_to_negative_direction_movement)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_move_to_positive_direction_velocity)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_move_to_negative_direction_velocity)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_move_by_positive_step_count)
{
	int32_t steps;
...
ZTEST_F(stepper_api, test_move_by_negative_step_count)
{
	int32_t steps;
...
ZTEST_F(stepper_api, test_move_by_positive_direction_velocity)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_move_by_negative_direction_velocity)
{
	int32_t pos;
...
ZTEST_F(stepper_api, test_run_positive_direction_correct_position)
{
	int32_t steps;
...
ZTEST_F(stepper_api, test_run_negative_direction_correct_position)
{
	int32_t steps;
