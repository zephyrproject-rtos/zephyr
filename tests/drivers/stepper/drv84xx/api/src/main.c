...
static void drv84xx_api_print_event_callback(const struct device *dev, const uint8_t stepper_idx,
...
static void drv84xx_api_before(void *f)
{
	...
	(void)stepper_set_reference_position(fixture->dev, CONFIG_STEPPER_IDX, 0);
	(void)stepper_set_microstep_interval(fixture->dev, CONFIG_STEPPER_IDX, 20000000);
...
ZTEST_F(drv84xx_api, test_move_to_positive_direction_movement)
{
	...
	(void)stepper_set_microstep_interval(fixture->dev, CONFIG_STEPPER_IDX, 20000000);
	(void)stepper_set_event_callback(fixture->dev, CONFIG_STEPPER_IDX, fixture->callback, NULL);
	(void)stepper_move_to(fixture->dev, CONFIG_STEPPER_IDX, pos);
	...
	(void)stepper_get_actual_position(fixture->dev, CONFIG_STEPPER_IDX, &pos);
...
ZTEST_F(drv84xx_api, test_move_to_is_moving_true_while_moving)
{
	...
	(void)stepper_set_event_callback(fixture->dev, CONFIG_STEPPER_IDX, fixture->callback, NULL);
	(void)stepper_move_to(fixture->dev, CONFIG_STEPPER_IDX, pos);
	(void)stepper_is_moving(fixture->dev, CONFIG_STEPPER_IDX, &moving);
...
ZTEST_F(drv84xx_api, test_move_by_zero_steps_no_movement)
{
	...
	(void)stepper_set_microstep_interval(fixture->dev, CONFIG_STEPPER_IDX, 20000000);
	(void)stepper_set_event_callback(fixture->dev, CONFIG_STEPPER_IDX, fixture->callback, NULL);
	(void)stepper_move_by(fixture->dev, CONFIG_STEPPER_IDX, steps);
	(void)k_poll(&stepper_event, 1, K_SECONDS(5));
	unsigned int signaled;
	int result;
	...
	zassert_equal(result, STEPPER_EVENT_STEPS_COMPLETED,
		      "Event was not STEPPER_EVENT_STEPS_COMPLETED event");
	(void)stepper_get_actual_position(fixture->dev, CONFIG_STEPPER_IDX, &steps);
...
ZTEST_F(drv84xx_api, test_run_positive_direction_correct_position)
{
	...
	(void)stepper_set_microstep_interval(fixture->dev, CONFIG_STEPPER_IDX, step_interval);
	(void)stepper_run(fixture->dev, CONFIG_STEPPER_IDX, STEPPER_DIRECTION_POSITIVE);
	k_busy_wait(110000);

	(void)stepper_get_actual_position(fixture->dev, CONFIG_STEPPER_IDX, &steps);
	zassert_true(IN_RANGE(steps, 1, 6), "Current position should be between 4 and 6 but is %d",
		     steps);
}
...
ZTEST_F(drv84xx_api, test_run_negative_direction_correct_position)
{
	...
	(void)stepper_set_microstep_interval(fixture->dev, CONFIG_STEPPER_IDX, step_interval);
	(void)stepper_run(fixture->dev, CONFIG_STEPPER_IDX, STEPPER_DIRECTION_NEGATIVE);
	k_busy_wait(110000);

	(void)stepper_get_actual_position(fixture->dev, CONFIG_STEPPER_IDX, &steps);
	zassert_true(IN_RANGE(steps, -6, -1),
...
ZTEST_SUITE(drv84xx_api, NULL, drv84xx_api_setup, drv84xx_api_before, NULL, NULL);
