...
ZTEST(stepper_shell, test_stepper_motor_move_by)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper move_by " FAKE_STEPPER_CONTROLLER " 1000");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_move_by_fake, fake_stepper_controller_dev,
				   err);
	zassert_equal(fake_stepper_motor_move_by_fake.arg1_val, 1000, "wrong microsteps value");
}

ZTEST(stepper_shell, test_stepper_motor_set_microstep_interval)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper set_microstep_interval " FAKE_STEPPER_CONTROLLER
					" 200");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_set_microstep_interval_fake,
				   fake_stepper_controller_dev, err);
	zassert_equal(fake_stepper_motor_set_microstep_interval_fake.arg1_val, 200,
...
ZTEST(stepper_shell, test_stepper_motor_set_reference_position)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper set_reference_position " FAKE_STEPPER_CONTROLLER
					" 100");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_set_reference_position_fake,
				   fake_stepper_controller_dev, err);
	zassert_equal(fake_stepper_motor_set_reference_position_fake.arg1_val, 100,
		      "wrong actual position value");
}

ZTEST(stepper_shell, test_stepper_motor_get_actual_position)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper get_actual_position " FAKE_STEPPER_CONTROLLER);

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_get_actual_position_fake,
				   fake_stepper_controller_dev, err);
}

ZTEST(stepper_shell, test_stepper_motor_move_to)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper move_to " FAKE_STEPPER_CONTROLLER " 200");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_move_to_fake, fake_stepper_controller_dev,
				   err);
	zassert_equal(fake_stepper_motor_move_to_fake.arg1_val, 200, "wrong target position value");
}

ZTEST(stepper_shell, test_stepper_motor_run)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper run " FAKE_STEPPER_CONTROLLER " positive");

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_run_fake, fake_stepper_controller_dev, err);
	zassert_equal(fake_stepper_motor_run_fake.arg1_val, STEPPER_MOTOR_DIRECTION_POSITIVE,
		      "wrong direction value");
}

ZTEST(stepper_shell, test_stepper_motor_run_invalid_direction)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
...
ZTEST(stepper_shell, test_stepper_motor_stop)
{
	const struct shell *sh = shell_backend_dummy_get_ptr();
	int err = shell_execute_cmd(sh, "stepper stop " FAKE_STEPPER_CONTROLLER);

	ASSERT_STEPPER_FUNC_CALLED(fake_stepper_motor_stop_fake, fake_stepper_controller_dev, err);
...
ZTEST(stepper_shell, test_stepper_controller_info)
{
	...
	zassert_equal(fake_stepper_motor_is_moving_fake.call_count, 1,
		      "is_moving function not called");
	zassert_equal(fake_stepper_motor_get_actual_position_fake.call_count, 1,
