...
#else
static ZTEST_BMEM uint8_t chained_read_buf[2][8];
...
#else
static ZTEST_BMEM uint8_t long_rx_buf[1024];
static ZTEST_BMEM uint8_t long_rx_buf2[1024];
static ZTEST_BMEM uint8_t long_tx_buf[1000];
#endif /* NOCACHE_MEM */
				...
				struct uart_event *evt, void *user_data)
{
	...
	case UART_RX_DISABLED:
		...
		break;
	case UART_RX_BUF_REQUEST:
		uart_rx_buf_rsp(dev, long_next_buffer, 1024);
