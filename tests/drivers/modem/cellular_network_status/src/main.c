...
/* The neighbour getter reads the vendor capability flag, so the devices below
 * carry a config naming a vendor that reports neighbours, or one that does not.
 */
...
/* A caller buffer smaller than the cached list is filled to capacity and the
 * truncation reported, so a caller that sized low still reads what fits.
 */
ZTEST(cellular_network_status, test_neighbor_buffer_too_small)
{
	...
	zassert_equal(cellular_get_neighbor_cells(&dev, out, &count), -ENOMEM,
		      "a too-small buffer must report the truncation");
	zassert_equal(count, 1, "count must report the number written");
...
/* A vendor whose scripts never report neighbours must be distinguishable from one
 * that simply has not measured yet.
 */
ZTEST(cellular_network_status, test_neighbor_unsupported_vendor)
{
	...
	count = ARRAY_SIZE(out);
	zassert_equal(cellular_get_neighbor_cells(&dev, out, &count), -ENOSYS,
		      "a vendor that never reports neighbours must return -ENOSYS");
}
