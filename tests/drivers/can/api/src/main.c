...
const struct zcan_frame test_std_msg_2 = {
	.id_type = CAN_STANDARD_IDENTIFIER,
	.rtr     = CAN_DATAFRAME,
	.std_id  = TEST_CAN_STD_ID_2,
	.dlc     = 8,
	.data    = {1, 2, 3, 4, 5, 6, 7, 8}
};
#ifdef CONFIG_CAN_FD_MODE
const struct zcan_frame test_std_msg_fd_1 = {
	.ext_id  = TEST_CAN_STD_ID_1,
	.fd      = 1,
	.rtr     = CAN_DATAFRAME,
	.id_type = CAN_STANDARD_IDENTIFIER,
	.dlc     = 0xf,
...
static void tx_std_isr_1(u32_t error_flags, void *arg)
{
	const struct zcan_frame *msg = (const struct zcan_frame *)arg;

	k_sem_give(&tx_cb_sem);

	zassert_equal(msg->std_id, TEST_CAN_STD_ID_1, "Arg does not match");
}

static void tx_std_isr_2(u32_t error_flags, void *arg)
{
	const struct zcan_frame *msg = (const struct zcan_frame *)arg;

	k_sem_give(&tx_cb_sem);

	zassert_equal(msg->std_id, TEST_CAN_STD_ID_2, "Arg does not match");
}

static void tx_ext_isr_1(u32_t error_flags, void *arg)
{
	const struct zcan_frame *msg = (const struct zcan_frame *)arg;

	k_sem_give(&tx_cb_sem);

	zassert_equal(msg->ext_id, TEST_CAN_EXT_ID_1, "Arg does not match");
}

static void tx_ext_isr_2(u32_t error_flags, void *arg)
{
	const struct zcan_frame *msg = (const struct zcan_frame *)arg;

	k_sem_give(&tx_cb_sem);

	zassert_equal(msg->ext_id, TEST_CAN_EXT_ID_2, "Arg does not match");
}
...
static void send_test_msg(struct device *can_dev, const struct zcan_frame *msg)
{
	int ret;
...
static void send_test_msg_nowait(struct device *can_dev,
			 ...
			 const struct zcan_frame *msg2)
{
	...
	if (filter1->id_type == CAN_STANDARD_IDENTIFIER) {
		if (filter1->std_id_mask != CAN_STD_ID_MASK) {
			mask = 0x0F;
		}
	} else {
		if (filter1->ext_id_mask != CAN_EXT_ID_MASK) {
			mask = 0x0F;
		}
	}
