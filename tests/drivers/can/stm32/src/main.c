...
static void test_filter_handling(void)
{
	...
	filter_id_1 = can_add_rx_msgq(can_dev, &can_msgq, &test_ext_masked_filter);
	zassert_not_equal(filter_id_1, -ENOSPC,
			  "Filter full even for a single one");
	zassert_true((filter_id_1 >= 0), "Negative filter number");

	filter_id_2 = can_add_rx_msgq(can_dev, &can_msgq, &test_std_filter);
	...
	filter_id_1 = can_add_rx_msgq(can_dev, &can_msgq, &test_std_some_filter);
	zassert_not_equal(filter_id_1, -ENOSPC,
	...
	can_remove_rx_filter(can_dev, filter_id_1);
	filter_id_1 = can_add_rx_msgq(can_dev, &can_msgq, &test_ext_filter);
	zassert_not_equal(filter_id_1, -ENOSPC,
			  "Filter full when overriding the first one");
	zassert_true((filter_id_1 >= 0), "Negative filter number");
