...
/*
 * The shape of hardware this scenario puts the core on, from Kconfig. Spelled
 * as TC_* below so the body of the file reads as plain macros.
 */
...
{
	/* The round trip above walks the first few ticks, where the arithmetic
	 * has room to spare. The ceiling is where it has none: the arm span is
	 * capped at TIMER_CORE_MAX_SPAN_TICKS, and a product formed a tick past
	 * it is what wraps. Checked at the cap itself rather than near it, and
	 * at several baseline phases since the remainder rides along in both
	 * directions.
	 */
	tc_reset(1, false);
	...
	for (uint32_t phase = 0; phase < 16U; phase++) {
		...
		/* Past everything the counter resolves the conversion saturates
		 * rather than wrapping round to a small count.
		 */
...
{
	/* Zero is what the kernel asks for when a timeout is already due. The
	 * timer has to come back armed rather than not at all, and no earlier
	 * than now.
	 */
	if (!IS_ENABLED(CONFIG_TICKLESS_KERNEL)) {
		ztest_test_skip();
...
/* Everything the core derived for this scenario, so a log says which shape of
 * hardware the failure was on without going back to tests.yaml.
 */
static void *tc_print_config(void)
{
	/* The precomputed values only exist once the core has initialised. */
	tc_reset(1, false);
