...
#define WORK_ITERATIONS_MAX (1UL << 22)
...
/*
 * Number of work_sink increments that takes at least a few ticks, or 0 if the
 * clock does not advance while the CPU merely spins. The measurements below
 * are meaningless in that case and skip themselves.
 */
...
static void calibrate_work(void)
{
	...
	for (uint32_t iterations = 1024; iterations <= WORK_ITERATIONS_MAX; iterations *= 2) {
		...
		if (taken < 0) {
			/* No clock that advances on its own; leave it at 0. */
			break;
		}
		if (taken >= 4) {
...
ZTEST(system_clock_driver, test_system_clock_empty_queue_does_not_starve_cpu)
{
	...
	armed = ticks_for_work(work_iterations);
	k_timer_stop(&timer);

	idle = ticks_for_work(work_iterations);
...
{
	const uint32_t tick_us = (uint32_t)k_ticks_to_us_ceil64(1);
	...
	baseline = ticks_for_work(work_iterations);
	...
	for (uint32_t i = 0; i < SWEEP_STEPS; i++) {
		...
		taken = ticks_for_work(work_iterations);
