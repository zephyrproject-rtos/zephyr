...
void test_eeprom_slave(void)
{
	...
	eeprom_0 = device_get_binding(DT_EEPROM_SLAVE_0_LABEL);
	zassert_not_null(eeprom_0, "EEPROM device %s not found",
			 DT_EEPROM_SLAVE_0_LABEL);

	LOG_INF("Found EEPROM device %s", DT_EEPROM_SLAVE_0_LABEL);

	eeprom_1 = device_get_binding(DT_EEPROM_SLAVE_1_LABEL);
	zassert_not_null(eeprom_1, "EEPROM device %s not found",
			 DT_EEPROM_SLAVE_1_LABEL);

	LOG_INF("Found EEPROM device %s", DT_EEPROM_SLAVE_1_LABEL);
	...
	LOG_INF("EEPROM %s Attached !", DT_EEPROM_SLAVE_0_LABEL);

	ret = i2c_slave_driver_register(eeprom_1);
	zassert_equal(ret, 0, "Failed to register EEPROM %s",
		      DT_EEPROM_SLAVE_1_LABEL);

	LOG_INF("EEPROM %s Attached !", DT_EEPROM_SLAVE_1_LABEL);

	/* Run Tests without bus access conflicts */
	run_full_read(i2c_0, DT_EEPROM_SLAVE_1_BASE_ADDRESS, eeprom_1_data);
	run_full_read(i2c_1, DT_EEPROM_SLAVE_0_BASE_ADDRESS, eeprom_0_data);

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		run_partial_read(i2c_0, DT_EEPROM_SLAVE_1_BASE_ADDRESS,
				 eeprom_1_data, offset);
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		run_partial_read(i2c_1, DT_EEPROM_SLAVE_0_BASE_ADDRESS,
				 eeprom_0_data, offset);
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		run_program_read(i2c_0, DT_EEPROM_SLAVE_1_BASE_ADDRESS,
				 offset);
	}

	for (offset = 0 ; offset < TEST_DATA_SIZE-1 ; ++offset) {
		run_program_read(i2c_1, DT_EEPROM_SLAVE_0_BASE_ADDRESS,
				 offset);
	}
