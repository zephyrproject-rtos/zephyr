...
static atomic_t callback_sequence;
static atomic_t suspend_order;
static atomic_t turn_off_order;
...
static int child_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	if (action == PM_DEVICE_ACTION_SUSPEND) {
		atomic_set(&suspend_order, atomic_inc(&callback_sequence) + 1);
		if (atomic_get(&block_suspend)) {
			k_sem_give(&suspend_entered);
			(void)k_sem_take(&suspend_continue, K_FOREVER);
		}
	} else if (action == PM_DEVICE_ACTION_TURN_OFF) {
		atomic_set(&turn_off_order, atomic_inc(&callback_sequence) + 1);
...
{
	const struct edge_context *context = arg;

	edge_ret = gpio_emul_input_set(monitor_gpio.port, monitor_gpio.pin,
				       context->value);
...
static void *gpio_monitor_setup(void)
{
	...
	if (IS_ENABLED(CONFIG_POWER_DOMAIN_GPIO_MONITOR_INITIAL_READ)) {
		...
		pm_device_init_off(test_child);
	} else {
		pm_device_init_suspended(test_child);
	}
	zassert_ok(pm_device_runtime_enable(test_child));
	atomic_clear(&callback_sequence);
	atomic_clear(&suspend_order);
	atomic_clear(&turn_off_order);
...
ZTEST(gpio_monitor, test_isr_deferral_and_resubmission)
{
	...
	if (IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME)) {
		zassert_true(
			WAIT_FOR(((ret = pm_device_runtime_get(test_child)) == 0),
				 100000, k_msleep(1)),
		...
		zassert_equal(atomic_get(&turn_on_count), 1);
		atomic_set(&turn_on_count, 0);
		zassert_ok(pm_device_action_run(test_child, PM_DEVICE_ACTION_RESUME));
	}
	...
	zassert_true(atomic_get(&suspend_order) > 0);
	zassert_true(atomic_get(&turn_off_order) > atomic_get(&suspend_order));
	...
	if (IS_ENABLED(CONFIG_PM_DEVICE_RUNTIME)) {
		...
		zassert_equal(pm_device_runtime_usage(test_child), -ENOSYS);
	}
}
