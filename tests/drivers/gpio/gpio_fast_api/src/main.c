 ...
 * set_input, set_output) and compile-time macros (GPIO_FAST_CPU_FREQ,
 * ns_to_cyc_at_hz_floor32, DELAY_NOPS) on the emulated GPIO driver via
 * the generic fallback path. The dispatch macros automatically fall back
 * to the generic backend because no vendor backend matches the emulated
 * GPIO controller's compatible (zephyr,gpio-emul).
 */
...
/* GPIO_FAST_CPU_FREQ compile-time DT resolution check. */
#define TEST_CPU_FREQ GPIO_FAST_CPU_FREQ(TEST_NODE)
BUILD_ASSERT(TEST_CPU_FREQ == 1000000,
	     "GPIO_FAST_CPU_FREQ should resolve to fast-gpio-cpu-clock-frequency (1 MHz)");

/* ns_to_cyc_at_hz_floor32 compile-time conversion checks. */
BUILD_ASSERT(ns_to_cyc_at_hz_floor32(0, TEST_CPU_FREQ) == 0,
	     "0 ns should be 0 cycles");
BUILD_ASSERT(ns_to_cyc_at_hz_floor32(1000000000ULL, TEST_CPU_FREQ) == TEST_CPU_FREQ,
	     "1 second in ns should equal the CPU frequency");
BUILD_ASSERT(ns_to_cyc_at_hz_floor32(1000, TEST_CPU_FREQ) >
	     ns_to_cyc_at_hz_floor32(100, TEST_CPU_FREQ),
...
ZTEST(gpio_fast_api, test_nops)
{
	...
	/*
	 * Verify DELAY_NOPS compiles and doesn't crash.
