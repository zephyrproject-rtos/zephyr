...
#define TEST_NEGOTIATION_RETRIES 100
...
ZTEST(pcie_vc, test_enable_times_out_when_negotiation_never_completes)
{
	pending_status_reg = TEST_VC_RES_STATUS_REG(1);
	pending_reads_remaining = INT_MAX;

	zassert_equal(pcie_vc_enable(TEST_BDF), -ETIMEDOUT);

	zassert_equal(write_count, 1U);
	zassert_equal(write_regs[0], TEST_VC_RES_CTRL_REG(1));
	zassert_equal(status_reads_after_write, TEST_NEGOTIATION_RETRIES);
}
