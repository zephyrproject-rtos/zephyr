...
#define _COLOR_MAPPING(led_node_id)                                                                \
	const uint8_t test_color_mapping_##led_node_id[] = DT_PROP(led_node_id, color_mapping)
...
#define LED_CONTROLLER_INFO(node_id)                                                               \
	DT_FOREACH_CHILD(node_id, COLOR_MAPPING)                                                   \
                                                                                                   \
	const struct led_info test_led_info[] = {DT_FOREACH_CHILD(node_id, LED_INFO)};             \
                                                                                                   \
...
ZTEST_USER(led_user, test_led_get_info)
{
	...
	for (led = 0; led < num_leds; led++) {
		...
		zassert_str_equal(info->label, test_led_info[led].label,
				  "LED %d - label: %s instead of %s", led, info->label,
				  test_led_info[led].label);

		zassert_equal(info->index, test_led_info[led].index,
			      "LED %d - index: %d instead of %d", led, info->index,
			      test_led_info[led].index);

		zassert_equal(info->num_colors, test_led_info[led].num_colors,
			      "LED %d - num_colors: %d instead of %d", led, info->num_colors,
			      test_led_info[led].num_colors);

		TC_PRINT("LED %d - label: %s, index: %d, num_colors: %d", led, info->label,
		...
		for (col = 0; col < info->num_colors; col++) {
			zassert_equal(
				info->color_mapping[col], test_led_info[led].color_mapping[col],
				"LED %d - color_mapping[%d]=%d instead of %d", led, col,
				info->color_mapping[col], test_led_info[led].color_mapping[col]);
...
ZTEST_USER(led_user, test_led_set_brightness)
{
	...
	for (led = 0; led < num_leds; led++) {
		...
		for (level = 0; level <= BRIGHTNESS_MAX; level++) {
			...
			zassert_ok(ret, "LED %d - failed to set brightness to %d", led, level);
		}
