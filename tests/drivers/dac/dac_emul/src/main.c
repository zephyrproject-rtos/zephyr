...
extern int dac_emul_value_get(const struct device *dev, uint8_t channel, uint32_t *value);

/*** Setup and Teardown ***/

static void *dac_emul_setup(void)
{
	zassert_not_null(dac_emul0, "DAC emulator 0 device not found");
	zassert_not_null(dac_emul_unconfigured, "DAC emulator 1 device not found");
	zassert_not_null(dac_emul_many, "DAC emulator many device not found");
...
static void dac_emul_before(void *fixture)
{
	ARG_UNUSED(fixture);
...
ZTEST(dac_emul_tests, test_device_ready)
{
	zassert_true(device_is_ready(dac_emul0), "Device should be ready");
	zassert_true(device_is_ready(dac_emul_unconfigured), "Device should be ready");
	zassert_true(device_is_ready(dac_emul_many), "Device should be ready");
}
...
ZTEST(dac_emul_tests, test_read_value_basic)
{
	/* Setup and write */
	struct dac_channel_cfg cfg = {
		.channel_id = 0,
	...
	ret = dac_write_value(dac_emul0, 0, 512);
	zassert_equal(ret, 0, "Write should succeed");
	...
	ret = dac_emul_value_get(dac_emul0, 0, &value);
	zassert_equal(ret, 0, "Read should succeed");
	zassert_equal(value, 512, "Read value should match");
}
...
ZTEST(dac_emul_tests, test_channel_reconfiguration)
{
	/* Initial setup with 8-bit resolution */
	struct dac_channel_cfg cfg8 = {
		.channel_id = 0,
		.resolution = 8,
	};
	int ret = dac_channel_setup(dac_emul0, &cfg8);

	zassert_equal(ret, 0, "Initial setup should succeed");

	/* Write 8-bit value */
	ret = dac_write_value(dac_emul0, 0, 200);
	zassert_equal(ret, 0, "Write should succeed");

	/* Reconfigure to 12-bit resolution */
	struct dac_channel_cfg cfg12 = {
		.channel_id = 0,
		.resolution = 12,
	};
	ret = dac_channel_setup(dac_emul0, &cfg12);
	zassert_equal(ret, 0, "Reconfiguration should succeed");

	/* Now write a 12-bit value */
	ret = dac_write_value(dac_emul0, 0, 3000);
	zassert_equal(ret, 0, "Write with new resolution should succeed");
	...
	ret = dac_emul_value_get(dac_emul0, 0, &value);
	zassert_equal(ret, 0, "Read should succeed");
	zassert_equal(value, 3000, "Value should match");

	/* Try to write old 8-bit max (should still work) */
	ret = dac_write_value(dac_emul0, 0, 255);
	zassert_equal(ret, 0, "Write 8-bit value with 12-bit config should succeed");
}
...
ZTEST_SUITE(dac_emul_tests, NULL, dac_emul_setup, dac_emul_before, NULL, NULL);
