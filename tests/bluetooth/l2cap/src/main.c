...
NET_BUF_POOL_DEFINE(data_tx_pool, 1, CONFIG_BT_L2CAP_TX_MTU,
...
{
	zassert_equal(net_buf_frags_len(buf), sizeof(data),
		     "Test channel unexpected len received");

	zassert_mem_equal(buf->data, data, sizeof(data),
			  "Data received doesn't match");
...
static struct bt_l2cap_le_chan test_ch = {
	.chan.ops = &l2cap_ops,
};
...
void test_l2cap_connect(void)
{
	...
	zassert_equal_ptr(conn, le_ch.chan.conn,
			"Test channel connection does not match");

	zassert_false(k_sem_take(&conn_sem, K_MSEC(100)),
		       "Tests channel connected not called within timeout");

	zassert_equal(le_ch.chan.state, BT_L2CAP_CONNECTED,
...
void test_l2cap_send_recv(void)
{
	...
	zassert_false(bt_l2cap_chan_send(&le_ch.chan, buf) < 0,
		     "Test channel send failed");
...
void test_l2cap_disconnect(void)
{
	...
	zassert_equal(le_ch.chan.state, BT_L2CAP_DISCONNECTED,
		     "Test channel unexpected state");
}
