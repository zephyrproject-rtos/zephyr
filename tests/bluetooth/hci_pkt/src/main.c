...
ZTEST(bt_hci_lockstep, test_cmd_complete_without_status)
{
...
ZTEST(bt_hci_lockstep, test_timeout_uses_credit)
{
	struct fake_transport xport;
	int err;
...
ZTEST(bt_hci_lockstep, test_allowance_revoked_before_send)
{
	struct fake_transport xport;
	int err;
...
ZTEST(bt_hci_lockstep, test_nop_while_waiting_is_no_response)
{
	struct fake_transport xport;
...
ZTEST(bt_hci_lockstep, test_send_error_after_revoke)
{
	check_send_error_keeps_allowance(nop_cc_revoke, sizeof(nop_cc_revoke), -EAGAIN, 1);
}

ZTEST(bt_hci_lockstep, test_send_error_after_grant)
{
	check_send_error_keeps_allowance(nop_cc_grant, sizeof(nop_cc_grant), -EIO, 2);
}

ZTEST(bt_hci_lockstep, test_response_before_send_error)
{
	struct fake_transport xport;
