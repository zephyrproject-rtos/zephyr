...
{
//  struct pdu_data_llctrl_feature_req *p = param;
...
void helper_pdu_verify_feature_req(const char *file, u32_t line, struct pdu_data *pdu, void *param)
{
//      struct pdu_data_llctrl_version_ind *p = param;

        zassert_equal(pdu->ll_id, PDU_DATA_LLID_CTRL, NULL);
        zassert_equal(pdu->llctrl.opcode, PDU_DATA_LLCTRL_TYPE_FEATURE_REQ, NULL);
        zassert_equal(pdu->llctrl.feature_req.features[0],0xA0,NULL); /* EGON */
}

void helper_pdu_verify_feature_rsp(const char *file, u32_t line, struct pdu_data *pdu, void *param)
{
//      struct pdu_data_llctrl_version_ind *p = param;

        zassert_equal(pdu->ll_id, PDU_DATA_LLID_CTRL, NULL);
        zassert_equal(pdu->llctrl.opcode, PDU_DATA_LLCTRL_TYPE_FEATURE_RSP, "Response: %d\n", pdu->llctrl.opcode);
        zassert_equal(pdu->llctrl.feature_rsp.features[0],0x00,"Response: %d\n", pdu->llctrl.feature_rsp.features[0]); /* EGON */
