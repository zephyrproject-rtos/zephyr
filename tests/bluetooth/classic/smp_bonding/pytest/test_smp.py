...
async def smp_bonding_init_001(hci_port, shell, dut, address) -> None:
...
async def smp_bonding_init_005(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def smp_bonding_rsp_010(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def smp_bonding_rsp_011(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def smp_bonding_rsp_014(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = Device.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
class TestSmpServer:
    def test_smp_bonding_init_001(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        """Verify pairing can be established when both devices are in Non-bondable mode with
        local device as L2CAP client."""
        logger.info(f'test_smp_bonding_init_001 {smp_initiator_dut}')
        hci, iut_address = smp_initiator_dut
        asyncio.run(smp_bonding_init_001(hci, shell, dut, iut_address))

    def test_smp_bonding_init_005(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        """Verify pairing and bonding can be established when both devices are
        in General Bonding mode."""
        logger.info(f'test_smp_bonding_init_005 {smp_initiator_dut}')
        hci, iut_address = smp_initiator_dut
        asyncio.run(smp_bonding_init_005(hci, shell, dut, iut_address))

    def test_smp_bonding_rsp_010(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        logger.info(f'test_smp_bonding_rsp_010 {smp_initiator_dut}')
        hci, iut_address = smp_initiator_dut
        asyncio.run(smp_bonding_rsp_010(hci, shell, dut, iut_address))

    def test_smp_bonding_rsp_011(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        logger.info(f'test_smp_bonding_rsp_011 {smp_initiator_dut}')
        hci, iut_address = smp_initiator_dut
        asyncio.run(smp_bonding_rsp_011(hci, shell, dut, iut_address))

    def test_smp_bonding_rsp_014(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        """To verify that pairing succeeds when both local and peer devices are configured
        as General Bonding, with the local device acting as an L2CAP server."""
        logger.info(f'test_smp_bonding_rsp_014 {smp_initiator_dut}')
        hci, iut_address = smp_initiator_dut
        asyncio.run(smp_bonding_rsp_014(hci, shell, dut, iut_address))
