...
server behavior, run under :ref:`Twister <twister_script>`. It uses two Zephyr boards running
the same test firmware image: the device under test (DUT) acts as the BIP server, driven
directly by Twister/pytest over its shell, while a second board (the harness device) acts as
the BIP client, driven over its own shell by ``pytest/conftest.py``.

Prerequisites
*************

Running the test suite on hardware requires two :zephyr:board:`mimxrt1170_evk@B/mimxrt1176/cm7`
boards, each with its own J-Link probe: the DUT runs as the BIP server and is driven directly
by Twister/pytest, while the second board is the harness device, runs as the BIP client, and
...
.. code-block:: shell
   ...
   west twister -v -p mimxrt1170_evk@B/mimxrt1176/cm7 --device-testing -T tests/bluetooth/classic/b2b/bip_server_test --force-platform --west-flash --west-runner=jlink --pytest-args="-p no:embed_test_pytest" --hardware-map C:\Users\<user>\dut.yaml --no-detailed-test-id --short-build-path

``--force-platform`` is required because the ``bluetooth.classic.bip_server_b2b`` scenario in
