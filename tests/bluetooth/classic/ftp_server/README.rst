.. _bluetooth_classic_ftp_server_tests:

Bluetooth Classic FTP Server Tests
##################################

Overview
********

This test suite exercises the Bluetooth Classic File Transfer Profile server against a second
Zephyr board acting as the FTP client, using :ref:`Twister <twister_script>` multi-DUT support
(:ref:`twister_multi_duts_testing`).

Both boards run the same image built from this directory: it enables
:kconfig:option:`CONFIG_BT_FTP_SERVER` and :kconfig:option:`CONFIG_BT_FTP_CLIENT`, and the
``test_ftp`` shell commands select the role at runtime. The device under test takes the server
role, the second reserved device the client role. No host PC Bluetooth controller is involved.
...
.. code-block:: shell
   ...
   west twister -v -p mimxrt1170_evk@B/mimxrt1176/cm7 -T tests/bluetooth/classic/ftp_server -O ftp_server --force-platform --device-testing --hardware-map map.yaml --west-flash --west-runner=jlink -X bt_classic_multi_test
