...
#define USER_DEFINED_HDR 0x7F
#define USER_DEFINED_HDR_VAL                                                                       \
	"Header for FTP Test. This parameter will be included in the GET "                         \
	"request to force multiple packets during transmission. The length of"                     \
	"this must fit within FTP_MOPL to ensure proper fragmentation handling."
...
/* Convert a big-endian UTF-16 encoded string to ASCII.
 * unicode_buf contains the UTF-16 data (2 bytes per character).
 * unicode_len is the total length in bytes (including null terminator pair).
 * buf_out receives the ASCII string.
 * buf_size is the maximum size of buf_out.
 * Returns 0 on success, or a negative error code.
 */
static int unicode_to_ascii(const char *unicode_buf, uint16_t unicode_len, char *buf_out,
			    uint16_t buf_size)
{
	uint16_t char_count;

	if (unicode_len < 2U || (unicode_len % 2U) != 0U) {
		return -EINVAL;
	}

	char_count = (unicode_len / 2U) - 1U;

	if (char_count >= buf_size) {
		bt_shell_error("Output buffer too small: need %u, have %u", char_count + 1U,
			       buf_size);
		return -ENOMEM;
	}

	for (uint16_t i = 0U; i < char_count; i++) {
		if (unicode_buf[i * 2U] != '\x00') {
			bt_shell_warn("Non-ASCII character at position %u, replacing with '?'", i);
			buf_out[i] = '?';
		} else {
			buf_out[i] = unicode_buf[i * 2U + 1U];
		}
	}

	buf_out[char_count] = '\0';
...
static bool ftp_parse_headers_cb(struct bt_obex_hdr *hdr, void *user_data)
{
	...
	case BT_OBEX_HEADER_ID_SRM_PARAM:
		...
		break;
	case BT_OBEX_HEADER_ID_NAME:
		name[0] = '\x00';
		unicode_to_ascii((const char *)hdr->data, hdr->len, name, sizeof(name));
		bt_shell_print("OBEX Name: %s", name);
		break;
	case BT_OBEX_HEADER_ID_DEST_NAME:
		name[0] = '\x00';
		unicode_to_ascii((const char *)hdr->data, hdr->len, name, sizeof(name));
		bt_shell_print("OBEX DEST Name: %s", name);
		break;
	case BT_OBEX_HEADER_ID_PERM:
		if (hdr->len == 4) {
			bt_shell_print("OBEX Permissions: 0x%08x", sys_get_be32(hdr->data));
		}
		break;
	case USER_DEFINED_HDR:
		bt_shell_print("OBEX User Defined: %.*s", hdr->len, hdr->data);
		break;
...
static int parse_no_split(const struct shell *sh, size_t argc, char *argv[], bool *enable_no_split)
{
	ARG_UNUSED(sh);

	for (size_t i = 1; i < argc; i++) {
		if (strcmp(argv[i], "no_split") == 0) {
			*enable_no_split = true;
			break;
		}
	}
...
static int parse_chunked_req(const struct shell *sh, size_t argc, char *argv[], bool *enable_srm)
{
	ARG_UNUSED(sh);

	for (size_t i = 1; i < argc; i++) {
		if (strcmp(argv[i], "chunked_req") == 0) {
			*enable_srm = true;
			break;
		}
	}
...
static int add_header_user_defined(struct net_buf *buf, const char *str)
{
	size_t total;
	uint16_t len;
	...
	net_buf_add_u8(buf, USER_DEFINED_HDR);
...
static int cmd_client_pull_folder_listing(const struct shell *sh, size_t argc, char *argv[])
{
	...
	(void)parse_chunked_req(sh, argc, argv, &enable_chunk);
	...
	if (inst->rsp_code == BT_OBEX_RSP_CODE_CONTINUE) {
		...
		if (inst->chunked_req) {
			err = add_header_user_defined(buf, USER_DEFINED_HDR_VAL);
			if (err != 0) {
				shell_error(sh, "Failed to add user defined header %d", err);
				goto failed;
			}
...
continue_req:
	err = bt_ftp_client_pull_folder_listing(&inst->client, !inst->chunked_req, buf);
	if (err == 0) {
		return 0;
	}
...
static int cmd_client_push_file(const struct shell *sh, size_t argc, char *argv[])
{
	...
	(void)parse_no_split(sh, argc - 1, &argv[1], &enable_no_split);
	...
	if (enable_no_split) {
		body = FTP_FILE_BODY_SHORT;
		body_len = strlen(FTP_FILE_BODY_SHORT);
	} else {
		body = FTP_FILE_BODY;
		body_len = strlen(FTP_FILE_BODY);
	}
...
static int cmd_client_pull_file(const struct shell *sh, size_t argc, char *argv[])
{
	...
	(void)parse_chunked_req(sh, argc - 1, &argv[1], &enable_chunk);
...
continue_req:
	err = bt_ftp_client_pull_file(&inst->client, !inst->chunked_req, buf);
	if (err == 0) {
		return 0;
	}
...
#define FTP_FOLDER_LISTING_SHORT                                                                   \
	"<?xml version=\"1.0\"?>\r\n"                                                              \
	"<!DOCTYPE folder-listing SYSTEM \"obex-folder-listing.dtd\">\r\n"                         \
	"<folder-listing version=\"1.0\">\r\n"                                                     \
	"</folder-listing>"
...
static int cmd_server_pull_folder_listing(const struct shell *sh, size_t argc, char *argv[])
{
	...
	(void)parse_srm(sh, argc, argv, &enable_srm);
	(void)parse_srmp(sh, argc, argv, &enable_srmp);
	(void)parse_no_split(sh, argc, argv, &enable_no_split);
	...
	if (enable_no_split) {
		body = FTP_FOLDER_LISTING_SHORT;
		body_len = strlen(FTP_FOLDER_LISTING_SHORT);
	} else {
		body = FTP_FOLDER_LISTING;
		body_len = strlen(FTP_FOLDER_LISTING);
	}
...
#define RSP_HELP_3 "<rsp: noerror, error> [rsp_code] [srm] [srmp] [no_split]"
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(pull_folder_listing, NULL, "[name <folder_name>] [srm] [srmp] [chunked_req]",
		      cmd_client_pull_folder_listing, 1, 5),
	SHELL_CMD_ARG(push_file, NULL, "<filename> [srm] [no_split]", cmd_client_push_file, 2, 2),
	SHELL_CMD_ARG(pull_file, NULL, "<filename> [srm] [srmp] [chunked_req]",
		      cmd_client_pull_file, 2, 3),
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(pull_folder_listing, NULL, RSP_HELP_3, cmd_server_pull_folder_listing, 2, 4),
	SHELL_CMD_ARG(push_file, NULL, RSP_HELP_2, cmd_server_push_file, 2, 3),
	SHELL_CMD_ARG(pull_file, NULL, RSP_HELP_3, cmd_server_pull_file, 2, 4),
...
SHELL_CMD_ARG_REGISTER(test_ftp, &ftp_cmds, "Bluetooth FTP shell commands", cmd_common, 1, 1);
