...
class Delegate(PairingDelegate):
    ...
    def confirm(self, auto: bool = False) -> bool:
        """
...
def sbc_codec_configuration():
    return MediaCodecCapabilities(
        ...
        media_codec_information=SbcMediaCodecInformation(
            sampling_frequency=SbcMediaCodecInformation.SamplingFrequency.SF_48000,
            channel_mode=SbcMediaCodecInformation.ChannelMode.MONO,
            block_length=SbcMediaCodecInformation.BlockLength.BL_8,
...
async def a2dp_source_case_register_ep(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test API register_ep
    """
    logger.info('<<< a2dp_source_case_register_ep ...')

    async with await open_transport_or_link(hci_port) as _:
        await send_cmd_to_iut(shell, dut, "br clear all", None)
        await send_cmd_to_iut(shell, dut, "a2dp_source register_cb", 'success')
        logger.info("stack only supports sbc")
        await send_cmd_to_iut(
            shell,
            dut,
            "a2dp_source register_ep source meg",
            'register_ep - <type: sink or source> <value: sbc>',
        )
        await send_cmd_to_iut(
            shell, dut, "a2dp_source register_ep source sbc", 'SBC source endpoint is registered'
        )


async def a2dp_source_case_connect(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test API a2dp connect,disconnect
    """
    logger.info('<<< a2dp_source_case_connect ...')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        async def fail_confirm(self, auto: bool = False) -> bool:
            """
            Respond yes or no to a Pairing confirmation question.
            The `auto` parameter stands for automatic confirmation.
            """
        ...
        def on_avdtp_connection(server):
            source = server
            sink_endpoint = source.add_sink(codec_capabilities=source_codec_capabilities())
            sink_endpoint.on('rtp_packet', on_rtp_packet)
            logger.info("a2dp connect successfully")

        listener = Listener.for_device(device)
        listener.on('connection', on_avdtp_connection)

        shell.exec_command("a2dp_source connect")
        ...
        await send_cmd_to_iut(shell, dut, 'a2dp_source connect', 'a2dp connected')
        await send_cmd_to_iut(shell, dut, 'a2dp_source disconnect', 'a2dp disconnected')

        await bumble_acl_disconnect(shell, dut, device, connection)


async def a2dp_source_case_disconnect(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test API a2dp disconnect
    """
    logger.info('<<< a2dp_source_case_disconnect ...')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        await send_cmd_to_iut(shell, dut, 'a2dp_source disconnect', 'fail to send disconnect cmd')
        ...
        await send_cmd_to_iut(shell, dut, 'a2dp_source disconnect', 'a2dp disconnected')
        await send_cmd_to_iut(shell, dut, 'a2dp_source disconnect', 'a2dp is not connected')


async def a2dp_source_case_discover_get_cap(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test API a2dp discover and get capabilities
    """
    logger.info('<<< a2dp_source_case_discover_get_cap ...')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info("a2dp sink creates an a2dp connection to source")
        sink = await Protocol.connect(connection)
        found, _ = await wait_for_shell_response(dut, "a2dp connected")

        logger.info("a2dp sink register endpoint")
        sink.add_sink(codec_capabilities=source_codec_capabilities())

        logger.info("a2dp sink sends a2dp discover cmd to sink")
        remote_endpoints = await sink.discover_remote_endpoints()
        for endpoint in remote_endpoints:
            logger.info(endpoint)
        assert len(remote_endpoints) >= 1

        logger.info("a2dp source sends a2dp discover cmd to sink")
        await send_cmd_to_iut(shell, dut, 'a2dp_source discover_peer_eps', 'Bitpool Range:')

        await a2dp_disconnect(sink)
        found, _ = await wait_for_shell_response(dut, "a2dp disconnected")
        assert found is not False
...
async def a2dp_source_case_sink_to_source(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test All a2dp commands are sent from sink to source
    """
    logger.info('<<< a2dp_source_case_sink_to_source ...')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        sink = await Protocol.connect(connection)
        source_endpoint = sink.add_sink(codec_capabilities=source_codec_capabilities())
        remote_endpoints = await sink.discover_remote_endpoints()
        for endpoint in remote_endpoints:
            logger.info(endpoint)
        assert len(remote_endpoints) >= 1
        ...
        assert remote_sink_ep.tsep == AVDTP_TSEP_SRC

        set_config = sbc_codec_configuration()
        sbc_codec_config = [ServiceCapabilities(AVDTP_MEDIA_TRANSPORT_SERVICE_CATEGORY), set_config]

        origin_create_stream_func = sink.create_stream
        sink.create_stream = local_create_stream

        stream = await sink.create_stream(sink, source_endpoint, remote_sink_ep, sbc_codec_config)
        ...
        sink.create_stream = origin_create_stream_func

        await bumble_acl_disconnect(shell, dut, device, connection)


async def a2dp_source_case_source_to_sink(hci_port, shell, dut, address, snoop_file) -> None:
    """
    Test All a2dp commands are sent from source to sink
    """
    logger.info('<<< a2dp_source_case_source_to_sink ...')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        def on_avdtp_connection(server):
            nonlocal sink_endpoint
            nonlocal sink
            sink = server
            sink_endpoint = sink.add_sink(codec_capabilities=source_codec_capabilities())
            logger.info("a2dp_source connect successfully")
        ...
        await send_cmd_to_iut(shell, dut, 'a2dp_source configure', 'success to configure')
        for acp_id in sink.streams:
            if acp_id == sink_endpoint.seid:
                stream = sink.streams[acp_id]
                break

        await send_cmd_to_iut(shell, dut, 'a2dp_source establish', 'success to establish')
        assert stream.state == AVDTP_OPEN_STATE

        await send_cmd_to_iut(shell, dut, 'a2dp_source start', 'success to start')
        assert stream.state == AVDTP_STREAMING_STATE

        await send_cmd_to_iut(shell, dut, 'a2dp_source suspend', "success to suspend")
        assert stream.state == AVDTP_OPEN_STATE

        await send_cmd_to_iut(shell, dut, 'a2dp_source release', "success to release")
        assert stream.state in (AVDTP_CLOSING_STATE, AVDTP_IDLE_STATE)
...
class TestA2dpSink:
    def test_a2dp_source_register_ep(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_register_ep {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_register_ep(hci, shell, dut, iut_address, snoop_file))

    def test_a2dp_source_connect(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_connect {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_connect(hci, shell, dut, iut_address, snoop_file))

    def test_a2dp_source_disconnect(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_disconnect {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_disconnect(hci, shell, dut, iut_address, snoop_file))

    def test_a2dp_source_discover_get_cap(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_discover_get_cap {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_discover_get_cap(hci, shell, dut, iut_address, snoop_file))

    def test_a2dp_source_sink_to_source(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_sink_to_source {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_sink_to_source(hci, shell, dut, iut_address, snoop_file))

    def test_a2dp_source_source_to_sink(self, shell: Shell, dut: DeviceAdapter, a2dp_source_dut):
        logger.info(f'test_a2dp_source_case_source_to_sink {a2dp_source_dut}')
        hci, iut_address = a2dp_source_dut
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(a2dp_source_case_source_to_sink(hci, shell, dut, iut_address, snoop_file))
