...
static void stream_aborted(struct bt_a2dp_stream *stream)
{
...
static int cmd_modify_sbc_config(const struct shell *sh, int32_t argc, char *argv[])
{
	uint8_t *codec_ie = (uint8_t *)sbc_cfg.codec_config->codec_ie;

	if (argc % 2 == 1) {
		shell_error(sh, "parameters are wrong");
	}

	for (int i = 0; i < argc; i++) {
		if (!strcmp(argv[i], "sf")) {
			if (!strcmp(argv[++i], "16000")) {
				codec_ie[0U] |= A2DP_SBC_SAMP_FREQ_16000;
			} else if (!strcmp(argv[i + 1], "32000")) {
				codec_ie[0U] |= A2DP_SBC_SAMP_FREQ_32000;
			} else if (!strcmp(argv[i + 1], "44100")) {
				codec_ie[0U] |= A2DP_SBC_SAMP_FREQ_44100;
			} else if (!strcmp(argv[i + 1], "48000")) {
				codec_ie[0U] |= A2DP_SBC_SAMP_FREQ_48000;
			}
		} else if (!strcmp(argv[i], "cm")) {
			if (!strcmp(argv[++i], "1")) {
				codec_ie[0U] |= A2DP_SBC_CH_MODE_MONO;
			} else if (!strcmp(argv[i + 1], "2")) {
				codec_ie[0U] |= A2DP_SBC_CH_MODE_DUAL;
			} else if (!strcmp(argv[i + 1], "4")) {
				codec_ie[0U] |= A2DP_SBC_CH_MODE_STEREO;
			} else if (!strcmp(argv[i + 1], "8")) {
				codec_ie[0U] |= A2DP_SBC_CH_MODE_JOINT;
			}
		} else if (!strcmp(argv[i], "bl")) {
			if (!strcmp(argv[++i], "4")) {
				codec_ie[0U] |= A2DP_SBC_BLK_LEN_4;
			} else if (!strcmp(argv[i + 1], "8")) {
				codec_ie[0U] |= A2DP_SBC_BLK_LEN_8;
			} else if (!strcmp(argv[i + 1], "12")) {
				codec_ie[0U] |= A2DP_SBC_BLK_LEN_12;
			} else if (!strcmp(argv[i + 1], "16")) {
				codec_ie[0U] |= A2DP_SBC_BLK_LEN_16;
			}
		} else if (!strcmp(argv[i], "subs")) {
			if (!strcmp(argv[++i], "4")) {
				codec_ie[1U] |= A2DP_SBC_SUBBAND_4;
			} else if (!strcmp(argv[i + 1], "8")) {
				codec_ie[1U] |= A2DP_SBC_SUBBAND_8;
			}
		} else if (!strcmp(argv[i], "am")) {
			if (!strcmp(argv[++i], "0")) {
				codec_ie[1U] |= A2DP_SBC_ALLOC_MTHD_SNR;
			} else if (!strcmp(argv[i + 1], "1")) {
				codec_ie[1U] |= A2DP_SBC_ALLOC_MTHD_LOUDNESS;
			}
		} else if (!strcmp(argv[i], "min_b")) {
			codec_ie[2U] = strtoul(argv[++i], NULL, 10);
		} else if (!strcmp(argv[i], "max_b")) {
			codec_ie[3U] = strtoul(argv[++i], NULL, 10);
		}
	}
	return 0;
}
...
#define MODIFY_SBC_CONFIG_HELP                                                                     \
	"<"                                                                                        \
	"sf: 16000/32000/44100/48000\n"                                                            \
	"cm: 1(Mono)/2(Dual)/4(Stereo)/8(Joint-Stereo)\n"                                          \
	"bl: 4/8/12/16\n"                                                                          \
	"subs: 4/8\n"                                                                              \
	"am: 0(SNR)/1(Loudness)\n"                                                                 \
	"min_b: 2-250\n"                                                                           \
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(modify_sbc_config, NULL, MODIFY_SBC_CONFIG_HELP, cmd_modify_sbc_config, 1,
		      14),
