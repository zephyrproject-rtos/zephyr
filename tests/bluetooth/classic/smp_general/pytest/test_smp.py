...
async def wait_for_shell_response(dut, regex: list[str] | str, timeout=10):
    found = False
    lines = []
    ...
    result = {message: False for message in messages}
...
async def sm_init_io_cap(
    hci_port, shell, dut, address, log_name, dut_sec, dut_io_cap, bumble_sec, bumble_io_cap
) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{log_name}.log", "wb") as snoop_file:
            ...
            if dut_io_cap == SMP_NO_INPUT_NO_OUTPUT_IO_CAPABILITY and dut_sec > 2:
                ...
                lines = shell.exec_command("smp security_info")
...
async def sm_init_001(hci_port, shell, dut, address) -> None:
...
async def sm_init_002(hci_port, shell, dut, address) -> None:
    await sm_init_io_cap(
        hci_port,
        shell,
        dut,
...
async def sm_init_003(hci_port, shell, dut, address) -> None:
    await sm_init_io_cap(
        hci_port,
        shell,
        dut,
...
async def sm_init_005(hci_port, shell, dut, address) -> None:
...
async def sm_init_006(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
            logger.info('Comparing number')
...
async def sm_init_008(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
...
async def sm_init_011(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_012(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        def __init__(
            self,
            io_capability,
...
async def sm_init_013(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_017(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def get_number(self) -> int | None:
            """
            Return an optional number as an answer to a passkey request.
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell, dut, f"l2cap_br connect {format(l2cap_server_psm, 'x')} sec 3"
            )
...
async def sm_init_018(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
...
async def sm_init_021(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        def __init__(
            self,
            io_capability,
...
async def sm_init_022(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
...
async def sm_init_025(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_028(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_029(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        def __init__(
            self,
            io_capability,
...
async def sm_init_030(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_032(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        def __init__(
            self,
            io_capability,
...
async def sm_init_034(hci_port, shell, dut, address) -> None:
    async with await open_transport_or_link(hci_port) as hci_transport:
        device = SmpDevice.with_hci(
            'Bumble',
            Address('F0:F1:F2:F3:F4:F5'),
...
async def sm_init_035(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def get_string(self, max_length: int) -> str | None:
            """
            Return a string whose utf-8 encoding is up to max_length bytes.
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            found = check_shell_response(
                lines, f"Enter 16 digits wide PIN code for {bumble_address}"
            )
            assert found is True
...
async def sm_init_037(hci_port, shell, dut, address) -> None:
    await sm_init_io_cap(
        hci_port,
        shell,
        dut,
...
async def sm_init_039(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        def __init__(
            self,
            io_capability,
...
async def sm_rsp_001(hci_port, shell, dut, address) -> None:
...
async def sm_rsp_002(hci_port, shell, dut, address) -> None:
    await sm_rsp_io_cap_with_bonding_flag(
        hci_port,
        shell,
        dut,
...
async def sm_rsp_003(hci_port, shell, dut, address) -> None:
    await sm_rsp_io_cap_with_bonding_flag(
        hci_port,
        shell,
        dut,
...
async def sm_rsp_006(hci_port, shell, dut, address) -> None:
    class Delegate(PairingDelegate):
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
            logger.info('Comparing number')
...
async def sm_rsp_sec_level(hci_port, shell, dut, address, log_name, dut_sec, dut_io_cap) -> None:
    class Delegate(PairingDelegate):
        async def confirm(self, auto: bool = False) -> bool:
            """Confirming pairing."""
            logger.info('Confirming pairing')
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{log_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell, dut, f"l2cap_br send 0 {sdu_sent_cnt} {sdu_sent_len}", None
            )
...
async def sm_rsp_008(hci_port, shell, dut, address) -> None:
...
async def sm_rsp_009(hci_port, shell, dut, address) -> None:
    await sm_rsp_sec_level(
        hci_port,
        shell,
        dut,
...
class TestSmpServer:
    def test_sm_init_001(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        asyncio.run(sm_init_001(hci, shell, dut, iut_address))
    ...
    def test_sm_init_002(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_002(hci, shell, dut, iut_address))
    ...
    def test_sm_init_003(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_003(hci, shell, dut, iut_address))
    ...
    def test_sm_init_005(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_005(hci, shell, dut, iut_address))
    ...
    def test_sm_init_006(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_006(hci, shell, dut, iut_address))
    ...
    def test_sm_init_008(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_008(hci, shell, dut, iut_address))
    ...
    def test_sm_init_011(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_011(hci, shell, dut, iut_address))
    ...
    def test_sm_init_012(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_012(hci, shell, dut, iut_address))
    ...
    def test_sm_init_013(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_013(hci, shell, dut, iut_address))
    ...
    def test_sm_init_017(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_017(hci, shell, dut, iut_address))
    ...
    def test_sm_init_018(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_018(hci, shell, dut, iut_address))
    ...
    def test_sm_init_021(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_021(hci, shell, dut, iut_address))
    ...
    def test_sm_init_022(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_022(hci, shell, dut, iut_address))
    ...
    def test_sm_init_025(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_025(hci, shell, dut, iut_address))
    ...
    def test_sm_init_028(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_028(hci, shell, dut, iut_address))
    ...
    def test_sm_init_029(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_029(hci, shell, dut, iut_address))
    ...
    def test_sm_init_030(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_030(hci, shell, dut, iut_address))
    ...
    def test_sm_init_032(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_032(hci, shell, dut, iut_address))
    ...
    def test_sm_init_034(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_034(hci, shell, dut, iut_address))
    ...
    def test_sm_init_035(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_035(hci, shell, dut, iut_address))
    ...
    def test_sm_init_037(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_037(hci, shell, dut, iut_address))
    ...
    def test_sm_init_039(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_init_039(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_001(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_001(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_002(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_002(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_003(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_003(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_006(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_006(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_008(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_008(hci, shell, dut, iut_address))
    ...
    def test_sm_rsp_009(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_rsp_009(hci, shell, dut, iut_address))
