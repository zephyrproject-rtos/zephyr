...
static struct bt_l2cap_br_server l2cap_servers[APPL_L2CAP_CONNECTION_MAX_COUNT];
...
static struct bt_l2cap_br_server *l2cap_alloc_server(uint16_t psm)
{
	ARRAY_FOR_EACH(l2cap_servers, index) {
		if (l2cap_servers[index].server.psm == 0) {
			l2cap_servers[index].server.psm = psm;
			l2cap_servers[index].server.accept = l2cap_accept;
...
static int cmd_l2cap_register(const struct shell *sh, size_t argc, char *argv[])
{
	...
	ARRAY_FOR_EACH(l2cap_servers, index) {
		if (l2cap_servers[index].server.psm == psm) {
	...
	for (index = 2; index < argc; index++) {
		if (!strcmp(argv[index], "sec")) {
			if (index + 1 >= argc) {
				br_server->server.psm = 0U;
				shell_error(sh, "No security level specified");
				return -ENOEXEC;
			}
			break;
		}
	}
	br_server->server.sec_level = strtoul(argv[++index], NULL, 16);

	if (bt_l2cap_br_server_register(&br_server->server) < 0) {
		br_server->server.psm = 0U;
		shell_error(sh, "Unable to register psm");
		return -ENOEXEC;
	}

	shell_print(sh, "L2CAP psm %u registered", br_server->server.psm);
...
static int cmd_l2cap_connect(const struct shell *sh, size_t argc, char *argv[])
{
	...
	br_chan = l2cap_alloc_chan();
	if ((!br_chan) || (br_chan->chan.chan.conn)) {
		bt_shell_error("No channels available");
		br_chan->active = false;
		return -ENOMEM;
	}
	...
	br_chan->chan.required_sec_level = strtoul(argv[++index], NULL, 16);
...
static int cmd_set_security(const struct shell *sh, size_t argc, char *argv[])
{
	...
	ARRAY_FOR_EACH(l2cap_servers, index) {
		if (l2cap_servers[index].server.psm == psm) {
			l2cap_servers[index].server.sec_level = sec;
...
SHELL_STATIC_SUBCMD_SET_CREATE(sys_cmds, SHELL_CMD_ARG(reboot, NULL, HELP_NONE, cmd_reboot, 1, 1),
			       SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(sdp_client_cmds,
			       SHELL_CMD_ARG(ssa_discovery, NULL, "<UUID>", cmd_ssa_discovery, 2,
					     0),
			       SHELL_SUBCMD_SET_END);

SHELL_STATIC_SUBCMD_SET_CREATE(smp_cmds,
			       SHELL_CMD_ARG(security_info, NULL, HELP_NONE, cmd_get_security_info,
					     1, 0),
			       SHELL_SUBCMD_SET_END);
...
SHELL_CMD_REGISTER(sys, &sys_cmds, "sys control cmds", cmd_default_handler);

SHELL_CMD_REGISTER(sdp_client, &sdp_client_cmds, "Bluetooth classic SDP client shell commands",
		   cmd_default_handler);

SHELL_CMD_REGISTER(smp, &smp_cmds, "Bluetooth classic SMP shell commands", cmd_default_handler);
