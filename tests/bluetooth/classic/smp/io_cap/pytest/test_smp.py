...
async def sm_io_cap_init(hci_port, shell, dut, address, log_name, dut_sec, dut_io_cap) -> None:
    class Delegate(PairingDelegate):
        ...
        async def display_number(self, number, digits):
            ...
            if dut_io_cap == SMP_KEYBOARD_ONLY_IO_CAPABILITY and dut_sec > 2:
        ...
        async def compare_numbers(self, number: int, digits: int) -> bool:
            ...
            if dut_io_cap == SMP_DISPLAY_YES_NO_IO_CAPABILITY and dut_sec > 2:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{log_name}.log", "wb") as snoop_file:
            ...
            elif dut_io_cap == SMP_DISPLAY_YES_NO_IO_CAPABILITY and dut_sec > 2:
            ...
            elif dut_io_cap == SMP_DISPLAY_ONLY_IO_CAPABILITY and dut_sec > 2:
...
async def sm_io_cap_init_001(hci_port, shell, dut, address) -> None:
...
async def sm_io_cap_init_002(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_003(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_004(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_005(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_006(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_007(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_008(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_009(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_010(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_011(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_012(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_013(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_014(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_015(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_init_016(hci_port, shell, dut, address) -> None:
    await sm_io_cap_init(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp(hci_port, shell, dut, address, log_name, dut_sec, dut_io_cap) -> None:
    class Delegate(PairingDelegate):
        ...
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
            logger.info('Comparing number')
            if dut_io_cap == SMP_DISPLAY_YES_NO_IO_CAPABILITY:
                if dut_sec > 2:
                    logger.debug(f'Passkey shown on Bumble: {number}')
                    ...
                    await send_cmd_to_iut(shell, dut, "bt auth-pairing-confirm", None)
                    return True
            elif dut_io_cap == SMP_DISPLAY_ONLY_IO_CAPABILITY:
                if not encrption_on:
                    found, _ = await wait_for_shell_response(dut, "Confirm pairing")
                    assert found is True
                    await send_cmd_to_iut(shell, dut, "bt auth-pairing-confirm", None)
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{log_name}.log", "wb") as snoop_file:
            ...
            if (dut_io_cap == SMP_NO_INPUT_NO_OUTPUT_IO_CAPABILITY and dut_sec > 2) or (
                dut_io_cap == SMP_DISPLAY_ONLY_IO_CAPABILITY and dut_sec > 2
            ):
                ...
                # If dut_sec is 1, the security level will be 2 when both devices support SSP.
                sec_check = 2 if dut_sec == 1 else dut_sec
            ...
            try:
                ...
                logger.info(
                    f'Fail to create L2CAP channel, exception type: {type(e)}, '
                    f'reason: {chan_conn_res}'
                )
...
async def sm_io_cap_rsp_001(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_002(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_003(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_004(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_005(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_006(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_007(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_008(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_009(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_010(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_011(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_012(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_013(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_014(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_015(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
async def sm_io_cap_rsp_016(hci_port, shell, dut, address) -> None:
    await sm_io_cap_rsp(
        hci_port,
        shell,
        dut,
...
class TestSmpServer:
    def test_sm_io_cap_init_001(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        asyncio.run(sm_io_cap_init_001(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_002(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_002(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_003(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_003(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_004(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_004(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_005(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_005(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_006(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_006(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_007(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_007(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_008(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_008(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_009(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_009(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_010(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_010(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_011(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_011(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_012(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_012(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_013(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_013(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_014(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_014(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_015(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_015(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_init_016(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_init_016(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_001(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_001(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_002(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_002(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_003(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_003(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_004(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_004(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_005(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_005(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_006(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_006(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_007(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_007(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_008(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_008(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_009(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_009(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_010(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_010(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_011(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_011(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_012(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_012(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_013(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_013(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_014(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_014(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_015(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_015(hci, shell, dut, iut_address))
    ...
    def test_sm_io_cap_rsp_016(self, shell: Shell, dut: DeviceAdapter, smp_initiator_dut):
        ...
        hci, iut_address = smp_initiator_dut
        asyncio.run(sm_io_cap_rsp_016(hci, shell, dut, iut_address))
