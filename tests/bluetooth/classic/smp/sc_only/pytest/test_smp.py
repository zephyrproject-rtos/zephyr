...
async def sm_io_cap_rsp(hci_port, shell, dut, address, log_name, dut_sec) -> None:
    class Delegate(PairingDelegate):
        async def compare_numbers(self, number: int, digits: int) -> bool:
            """Compare two numbers."""
            logger.info('Comparing number')
        ...
        with open(f"bumble_hci_{log_name}.log", "wb") as snoop_file:
            ...
            try:
                ...
                if isinstance(e, HCI_Error):
                    pairing_res = e.error_name
                else:
                    pairing_res = 'unexpected error'
                logger.info(f'Fail to encrypt, exception type: {type(e)}, reason: {pairing_res}')
