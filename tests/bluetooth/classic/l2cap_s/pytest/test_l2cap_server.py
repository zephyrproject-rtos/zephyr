    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            found, _ = await wait_for_shell_response(
                dut, f"Channel {L2CAP_CHAN_IUT_ID} disconnected"
            )
            assert found is True
            await bumble_acl_disconnect(shell, dut, device, connection)
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell,
                dut,
                "l2cap_br search_mtu {L2CAP_CHAN_IUT_ID} local",
                "local mtu = 196",
            )
            await send_cmd_to_iut(
                shell,
                dut,
                "l2cap_br search_mtu {L2CAP_CHAN_IUT_ID} peer",
                "peer mtu = 65535",
            )
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell,
                dut,
                f"l2cap_br search_mtu {L2CAP_CHAN_IUT_ID} local",
                "local mtu = 48",
            )
            await send_cmd_to_iut(
                shell,
                dut,
                f"l2cap_br search_mtu {L2CAP_CHAN_IUT_ID} peer",
                "peer mtu = 65535",
            )
...
async def l2cap_server_case_12(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await connection.create_l2cap_channel(
                spec=ClassicChannelSpec(psm=L2CAP_SERVER_PSM, mtu=30)
            )
            found, _ = await wait_for_shell_response(dut, f"Channel {L2CAP_CHAN_IUT_ID} connected")
            assert found is True
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            try:
                ...
                assert (
                    error.error_code == 0x2
                    and error.error_name == 'CONNECTION_REFUSED_PSM_NOT_SUPPORTED'
                )
...
async def l2cap_server_case_14(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            l2cap_channel1 = await connection.create_l2cap_channel(
            ...
            await send_cmd_to_iut(
                shell, dut, f"l2cap_br send 1 {data} {str(hex(len(data)))[2:]}", None
            )
            await asyncio.sleep(0.5)
...
async def l2cap_server_case_15(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            for _ in range(STRESS_TEST_MAX_COUNT):
                await connection.create_l2cap_channel(spec=ClassicChannelSpec(psm=L2CAP_SERVER_PSM))
                found, _ = await wait_for_shell_response(
                    dut, f"Channel {L2CAP_CHAN_IUT_ID} connected"
                )
                assert found is True
                await send_cmd_to_iut(
                    shell,
...
async def l2cap_server_case_16(hci_port, shell, dut, address) -> None:
...
class TestL2capBRServer:
    ...
    def test_l2cap_server_case_15(self, shell: Shell, dut: DeviceAdapter, l2cap_br_dut):
        ...
        hci, iut_address = l2cap_br_dut
        asyncio.run(l2cap_server_case_15(hci, shell, dut, iut_address))
    ...
    def test_l2cap_server_case_16(self, shell: Shell, dut: DeviceAdapter, l2cap_br_dut):
        ...
        hci, iut_address = l2cap_br_dut
        asyncio.run(l2cap_server_case_16(hci, shell, dut, iut_address))
