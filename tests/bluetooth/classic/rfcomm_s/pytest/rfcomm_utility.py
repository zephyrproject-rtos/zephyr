...
class RFCOMM_MCC_RLS:
    ...
    def from_bytes(data: bytes) -> RFCOMM_MCC_RLS:
        return RFCOMM_MCC_RLS(
            dlci=(data[0] >> 2) & 0x3F, error_flag=(data[1] >> 1) & 0x01, break_flag=data[1] & 0x01
        )
...
class RFCOMM_MCC_RPN:
    ...
    def from_bytes(data: bytes) -> RFCOMM_MCC_RPN:
        return RFCOMM_MCC_RPN(
            dlci=(data[0] >> 2) & 0x3F,
            baud_rate=(data[1] >> 4) & 0x0F,
            data_bits=data[1] & 0x0F,
            stop_bits=(data[2] >> 4) & 0x0F,
            parity=(data[2] >> 3) & 0x01,
            parity_type=data[2] & 0x07,
            flow_control=data[3],
            xon_char=data[4],
            xoff_char=data[5],
            parameter_mask=data[6] | (data[7] << 8),
        )
