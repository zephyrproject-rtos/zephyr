...
# establish_dlc
async def establish_dlc(rfcomm_mux, channel, send_pn=True, support_cfc=True):
    logger.info(f'Establish DLC for channel {channel}...')
    try:
        session = await rfcomm_mux.open_dlc(channel, send_pn=send_pn, support_cfc=support_cfc)
        if not support_cfc or not send_pn:
...
async def tc_rfcomm_s_1(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Reject RFCOMM Session'
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        # Test Start
        logger.info('Step 1: Initialize RFCOMM Session')
        rfcomm_mux = await rfcomm_client.start()

        logger.info('Step 2: Disconnect BR connection')
        await connection.disconnect()
        found, _ = await _wait_for_shell_response(dut, "Disconnected")
        assert found is True, "DUT did not report disconnection"

        logger.info('Step 3: Attempt to establish RFCOMM session')
        try:
            await establish_dlc(rfcomm_mux, channel_9)
            result = False
        except Exception as e:
...
async def tc_rfcomm_s_2(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Forward compatible with Bluetooth devices that do not send PN command'
    logger.info(f'<<< Start {case_name} ...')
    dut_address = address.split(" ")[0]
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info('Step 2: Try to establish DLC without sending PN command')
        ...
        logger.info('Step 4: DUT transfer data after received FCOFF command from Tester')
        ...
        logger.info('Step 5: The blocked data can be sent after FCON command')
        rfcomm_mux.send_fcon_command()
        assert await wait_mux_response(rfcomm_utility.logger_capture, 'Received FCON response')
        assert await wait_mux_response(logger_capture, 'Data send pass')

        logger.info('Step 6: DUT actively disconnects DLC')
        ...
        assert await wait_mux_response(rfcomm_utility.logger_capture, 'DISC received on dlc=18'), (
            "DUT failed to disconnect DLC"
        )


async def tc_rfcomm_s_3(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Respond to Control Commands After DLC Establishment'
    logger.info(f'<<< Start {case_name} ...')
    dut_address = address.split(" ")[0]
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info('Step 2: Establish DLC')
        dlc_9 = await establish_dlc(rfcomm_mux, channel_9)
        assert dlc_9, "Failed to establish DLC"

        logger.info('Step 3: Send RLS command to DUT')
        ...
        logger.info('Step 4: Send RPN command to DUT')
        ...
        logger.info('Step 5: Send Test command to DUT')
        ...
        logger.info('Step 6: Send unsupported command to DUT')
        ...
        logger.info('Step 7: Disconnect DLC')
        await dlc_9.disconnect()
        found, _ = await _wait_for_shell_response(dut, "disconnected")
        assert found, "DUT did not report DLC disconnection"


async def tc_rfcomm_s_4(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Information Transfer with Aggregate Flow Control'
    logger.info(f'<<< Start {case_name} ...')
    dut_address = address.split(" ")[0]
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info('Step 2: Establish DLC with aggregate flow control enabled')
        dlc_9 = await establish_dlc(rfcomm_mux, channel_9, support_cfc=False)
        ...
        logger.info('Step 7: DUT shutdown RFCOMM Session')
...
async def tc_rfcomm_s_5(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Information Transfer with Credit Based Flow Control and Active Disconnection'
    logger.info(f'<<< Start {case_name} ...')
    dut_address = address.split(" ")[0]
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info('Step 3: Transfer data with credit based flow control')
        await send_cmd_to_iut(shell, dut, "rfcomm_s send 9 1")
        assert await wait_mux_response(logger_capture, 'Data send pass'), (
            "Failed to receive data from DUT"
        )

        logger.info('Step 4: DUT actively disconnects DLC')
        await send_cmd_to_iut(shell, dut, "rfcomm_s disconnect 9")
        assert await wait_mux_response(rfcomm_utility.logger_capture, 'DISC received on dlc=18'), (
            "DUT failed to disconnect DLC"
        )

        logger.info('Step 5: DUT shutdown RFCOMM Session')
        await send_cmd_to_iut(shell, dut, "rfcomm_s disconnect 7")
        assert await wait_mux_response(rfcomm_utility.logger_capture, 'DISC received on dlc=0'), (
            "DUT failed to disconnect DLC"
        )
...
async def tc_rfcomm_s_6(hci_port, shell, dut, address, snoop_file) -> None:
    case_name = 'Information Transfer with Credit Based Flow Control and Passive Disconnection'
    logger.info(f'<<< Start {case_name} ...')
    dut_address = address.split(" ")[0]
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        logger.info('Step 4: Tester initiates DLC disconnection')
        await dlc_9.disconnect()
        found, _ = await _wait_for_shell_response(dut, "disconnected")
        ...
        logger.info('Step 5: Tester initiates RFCOMM session shutdown')
...
class TestRFCOMM:
    def test_rfcomm_s_1(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_1(hci, shell, dut, iut_address, snoop_file))

    def test_rfcomm_s_2(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        """Forward compatible with Bluetooth devices that do not send PN command."""
        logger.info(f'RFCOMM-S-2 {device_under_test}')
        hci, iut_address = device_under_test
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_2(hci, shell, dut, iut_address, snoop_file))

    def test_rfcomm_s_3(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        """Respond to Control Commands After DLC Establishment."""
        logger.info(f'RFCOMM-S-3 {device_under_test}')
        hci, iut_address = device_under_test
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_3(hci, shell, dut, iut_address, snoop_file))

    def test_rfcomm_s_4(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        """Information Transfer with Aggregate Flow Control."""
        logger.info(f'RFCOMM-S-4 {device_under_test}')
        hci, iut_address = device_under_test
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_4(hci, shell, dut, iut_address, snoop_file))

    def test_rfcomm_s_5(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        """Information Transfer with Credit Based Flow Control and Active Disconnection."""
        logger.info(f'RFCOMM-S-5 {device_under_test}')
        hci, iut_address = device_under_test
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_5(hci, shell, dut, iut_address, snoop_file))

    def test_rfcomm_s_6(self, shell: Shell, dut: DeviceAdapter, device_under_test):
        """Information Transfer with Credit Based Flow Control and Passive Disconnection."""
        logger.info(f'RFCOMM-S-6 {device_under_test}')
        hci, iut_address = device_under_test
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            asyncio.run(tc_rfcomm_s_6(hci, shell, dut, iut_address, snoop_file))
