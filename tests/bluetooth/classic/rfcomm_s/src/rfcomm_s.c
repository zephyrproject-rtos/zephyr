    ...
    struct bt_rfcomm_dlc **dlc)
{
    ...
    if (9U == server->channel)
    {
        rfcomm_dlc = &rfcomm_dlc_9;
    }
    else if(7U == server->channel)
    {
...
struct bt_rfcomm_server rfcomm_server_9 = {
    .accept = &rfcomm_accept,
};

struct bt_rfcomm_server rfcomm_server_7 = {
    .accept = &rfcomm_accept,
};
...
static int cmd_send(const struct shell *sh, size_t argc, char *argv[])
{
    ...
    {
        shell_print(sh, "Channel %u isn't supported, just support channel 9 and 7", channel);
        return -ENOEXEC;
    }
...
SHELL_STATIC_SUBCMD_SET_CREATE(rfcomm_s_cmds,
	SHELL_CMD_ARG(register, NULL, "register a rfcomm server channel(cid)", cmd_register, 2, 0),
	SHELL_CMD_ARG(connect, NULL, "create rfcomm connection, cmd_connect", cmd_connect, 2, 0),
	SHELL_CMD_ARG(disconnect, NULL, "disconnect current rfcomm connection", cmd_disconnect, 2, 0),
    SHELL_CMD_ARG(send, NULL, "rfcomm_s send <cid> <data>", cmd_send, 3, 0),
...
SHELL_CMD_ARG_REGISTER(rfcomm_s, &rfcomm_s_cmds, "Bluetooth classic rfcomm_s shell commands",
