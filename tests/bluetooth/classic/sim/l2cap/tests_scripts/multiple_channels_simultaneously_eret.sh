...
test_case_name="bluetooth.classic.sim.l2cap.multiple_channels_simultaneously.eret"
...
ARGS=("--peer_bd_address=${BD_ADDR[1]}" \
      "-test=l2cap_server::test_12_eret_mode_multiple_channels_simultaneously")

execute_async "$peripheral_exe" --bt-dev="127.0.0.1:$HCI_PORT_0" "${ARGS[@]}" || exit 1

central_exe="${zephyr_exe}"
ARGS=("--peer_bd_address=${BD_ADDR[0]}" \
      "-test=l2cap_client::test_12_eret_mode_multiple_channels_simultaneously")
