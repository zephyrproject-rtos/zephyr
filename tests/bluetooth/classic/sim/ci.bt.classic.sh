...
run_test_script() {
    ...
    if [[ -z "${ZEPHYR_BASE:-}" ]]; then
        log_info "$test_index/$total_tests $test_id \033[31mFAIL\033[0m (ZEPHYR_BASE not set)"
    ...
    if [[ ! -d "$ZEPHYR_BASE" ]]; then
        log_info "$test_index/$total_tests $test_id \033[31mFAIL\033[0m (ZEPHYR_BASE dir not found)"
        FAILED_TESTS+=("$test_id")
    ...
    # Return to original directory (don't fail if this doesn't work)
    ...
    if [[ $exit_code -eq 0 ]]; then
        log_info "$test_index/$total_tests $test_id \033[32mPASS\033[0m (${duration}s)"
        PASSED_TESTS+=("$test_id")
        TEST_MESSAGES["$test_id"]="Test passed successfully"
        echo "PASSED: $test_id (${duration}s)" >> "$SUMMARY_LOG"
    else
        log_info "$test_index/$total_tests $test_id \033[31mFAIL\033[0m (${duration}s)"
        cat "$log_file" 2>/dev/null || echo "Failed to read log file: $log_file"
        FAILED_TESTS+=("$test_id")
        TEST_MESSAGES["$test_id"]="Test failed with exit code $exit_code"
        echo "FAILED: $test_id (exit code: $exit_code, ${duration}s)" >> "$SUMMARY_LOG"
        HAS_FAILURE=true
    fi
...
main() {
    ...
    for script in "${TEST_SCRIPTS[@]}"; do
        run_test_script "$script" "$test_index" "$total_tests"
        ((test_index++))
    done
