...
allocate_tcp_port() {
    ...
    if command -v flock &>/dev/null; then
        ...
        # Critical section - port allocation
        {
            ...
            #
            # NOTE: Avoid `for p in $(seq ...)` because it materializes the full
            # range (9000-19999 => 11000 items) into a string and can amplify
            # sub-process spawning in worst-case scenarios.
            if [[ -z "${allocated_port}" ]]; then
                ...
                port=${start_port}
                while [[ ${port} -le ${TCP_PORT_RANGE_END} ]]; do
                ...
                if [[ -z "${allocated_port}" ]]; then
                    port=${TCP_PORT_RANGE_START}
                    while [[ ${port} -lt ${start_port} ]]; do
                        if is_port_available "${port}"; then
                            allocated_port="${port}"
                            echo "${allocated_port}" >> "${TCP_PORT_REGISTRY}"
                            break
        ...
        # Critical section - port allocation
        {
            ...
            # See note in flock path: avoid `$(seq ...)` to prevent expanding the
            # full range and creating excessive subprocess work in worst cases.
            if [[ -z "${allocated_port}" ]]; then
                local start_port=$((TCP_PORT_RANGE_START + \
                    RANDOM % (TCP_PORT_RANGE_END - TCP_PORT_RANGE_START)))
                local port
