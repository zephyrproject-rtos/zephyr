...
async def create_controller(index: int, transport_name: str, link, bd_addresses: set):
...
async def async_sim_controllers():
    ...
    if len(sys.argv) < 2:
        logger.error("No transport provided. Usage: controllers.py <transport1> [transport2] ...")
        return
    ...
    try:
        ...
        for index, transport_name in enumerate(sys.argv[1:]):
            transport, controller = await create_controller(
                index, transport_name, link, bd_addresses
            )
