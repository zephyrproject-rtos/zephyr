...
static K_SEM_DEFINE(br_conn_changed_sem, 0, 1);
...
static void br_connected(struct bt_conn *conn, uint8_t conn_err)
{
	...
	if (conn_err == 0 && !atomic_test_bit(test_flags, TEST_FLAG_CONN_CONNECTED)) {
		k_sem_give(&br_conn_changed_sem);
...
static bool peer_device_discovery(bool limited)
{
	...
	timeout_ms = (uint64_t)(timeout * 1000.0);

	err = k_sem_take(&br_discover_sem, K_MSEC(timeout_ms));
...
ZTEST(gap_central, test_02_limited_discovery)
{
	bool found;
	...
	k_sleep(K_SECONDS(CONFIG_BT_LIMITED_DISCOVERABLE_DURATION + 1));
