    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell,
                dut,
                f"l2cap_br connect {format(L2CAP_SERVER_PSM, 'x')} {MODE}",
...
async def l2cap_client_case_26(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell, dut, f"l2cap_br connect 1003 {MODE}", "Channel 1 connected"
            )
...
async def l2cap_client_case_27(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell, dut, f"l2cap_br connect 1002 {MODE}", "Unable to connect to psm"
            )
...
async def l2cap_client_case_30(hci_port, shell, dut, address) -> None:
    ...
    async with await open_transport_or_link(hci_port) as hci_transport:
        ...
        with open(f"bumble_hci_{sys._getframe().f_code.co_name}.log", "wb") as snoop_file:
            ...
            await send_cmd_to_iut(
                shell,
                dut,
                f"l2cap_br connect {format(L2CAP_SERVER_PSM, 'x')} basic mtu {format(0x27, 'x')}",
                f"Channel {L2CAP_CHAN_IUT_ID} disconnected",
            )


async def l2cap_client_case_31(hci_port, shell, dut, address) -> None:
...
class TestL2capBRClient:
    ...
    def test_l2cap_client_case_30(self, shell: Shell, dut: DeviceAdapter, l2cap_br_dut):
        ...
        hci, iut_address = l2cap_br_dut
        asyncio.run(l2cap_client_case_30(hci, shell, dut, iut_address))
    ...
    def test_l2cap_client_case_31(self, shell: Shell, dut: DeviceAdapter, l2cap_br_dut):
        ...
        hci, iut_address = l2cap_br_dut
        asyncio.run(l2cap_client_case_31(hci, shell, dut, iut_address))
