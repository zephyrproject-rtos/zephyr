...
async def wait_for_shell_response(dut, message, timeout=5):
    found = False
    lines = []
    try:
...
async def send_cmd_to_iut(shell, dut, cmd, parse=None, timeout=10):
    found = False
    lines = shell.exec_command(cmd)
    if parse is not None:
        found = any([parse in line for line in lines])
        if found is False:
            found, lines = await wait_for_shell_response(dut, parse, timeout)
    else:
        found = True
    logger.info(f'{lines}')
