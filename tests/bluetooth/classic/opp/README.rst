...
server's response codes at runtime via ``opp_s set_*_rsp`` and its multi-packet
business-card mode via ``opp_s set_pull_multi``:

#. ``5.2`` OBEX connection establishment (error then success; the client never
   sends a Target header, per OPP Spec 5.4).
#. ``5.3`` Object Push (PUT): success/final, continue/non-final,
   unsupported-media, entity-too-large, multiple objects, abort-ongoing.
#. ``5.4`` Business Card Pull (GET): success, continue/non-final (the client
   drives the continuation GET with ``opp_c pull_continue``, since the client
   stack does not auto-continue a GET), not-found, forbidden-name,
   abort-ongoing.
#. ``5.5`` Business Card Exchange (PUT + GET): success and pull-not-found.
#. ``5.6`` Abort with an active request and with no active request.
#. ``5.8`` Service Discovery + PDU allocation on both roles.
#. ``5.7`` OBEX disconnection (error then success).
#. ``5.1`` RFCOMM transport disconnection (runs last, since the transport
