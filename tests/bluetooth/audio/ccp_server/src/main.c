...
static void *ccp_server_test_suite_setup(void)
{
	struct ccp_server_test_suite_fixture *fixture;
...
static void ccp_server_test_suite_before(void *f)
{
	memset(f, 0, sizeof(struct ccp_server_test_suite_fixture));
}

static void ccp_server_test_suite_after(void *f)
{
	struct ccp_server_test_suite_fixture *fixture = f;
	...
	for (size_t i = ARRAY_SIZE(fixture->bearers); i > 0; i--) {
		...
		if (fixture->bearers[index_to_unreg] != NULL) {
			bt_ccp_server_unregister_bearer(fixture->bearers[index_to_unreg]);
		}
...
static void ccp_server_test_suite_teardown(void *f)
{
	free(f);
}

ZTEST_SUITE(ccp_server_test_suite, NULL, ccp_server_test_suite_setup, ccp_server_test_suite_before,
	    ccp_server_test_suite_after, ccp_server_test_suite_teardown);
...
static void register_default_bearer(struct ccp_server_test_suite_fixture *fixture)
{
	...
	err = bt_ccp_server_register_bearer(&register_param, &fixture->bearers[0]);
	zassert_equal(err, 0, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer)
{
	register_default_bearer(fixture);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_multiple_bearers)
{
	if (CONFIG_BT_CCP_SERVER_BEARER_COUNT == 1) {
		ztest_test_skip();
	}

	register_default_bearer(fixture);

	for (int i = 1; i < CONFIG_BT_CCP_SERVER_BEARER_COUNT; i++) {
		...
		err = bt_ccp_server_register_bearer(&register_param, &fixture->bearers[i]);
		zassert_equal(err, 0, "Unexpected return value %d", err);
	}
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer_inval_null_param)
{
	int err;

	err = bt_ccp_server_register_bearer(NULL, &fixture->bearers[0]);
	zassert_equal(err, -EINVAL, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer_inval_null_bearer)
{
	const struct bt_tbs_register_param register_param = {
		.provider_name = "test",
		.uci = "un999",
	...
	err = bt_ccp_server_register_bearer(&register_param, NULL);
	zassert_equal(err, -EINVAL, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer_inval_no_gtbs)
{
	const struct bt_tbs_register_param register_param = {
		.provider_name = "test",
		.uci = "un999",
...
static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer_inval_double_gtbs)
{
	const struct bt_tbs_register_param register_param = {
		.provider_name = "test",
		.uci = "un999",
	...
	err = bt_ccp_server_register_bearer(&register_param, &fixture->bearers[1]);
	zassert_equal(err, -EALREADY, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_register_bearer_inval_cnt)
{
	const struct bt_tbs_register_param register_param = {
		.provider_name = "test",
		.uci = "un999",
	...
	err = bt_ccp_server_register_bearer(&register_param,
					    &fixture->bearers[CONFIG_BT_CCP_SERVER_BEARER_COUNT]);
	zassert_equal(err, -ENOMEM, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_ccp_server_unregister_bearer)
{
	int err;
...
static ZTEST_F(ccp_server_test_suite, test_ccp_server_unregister_bearer_inval_double_unregister)
{
	int err;
...
static ZTEST_F(ccp_server_test_suite, test_ccp_server_unregister_bearer_inval_null_bearer)
{
	int err;
...
static ZTEST_F(ccp_server_test_suite, test_bt_ccp_server_get_bearer_uci)
{
	const char *res_bearer_name;
	int err;
	...
	err = bt_ccp_server_get_bearer_uci(fixture->bearers[0], &res_bearer_name);
	zassert_equal(err, 0, "Unexpected return value %d", err);
...
static ZTEST_F(ccp_server_test_suite, test_bt_ccp_server_get_bearer_uci_inval_not_registered)
{
	const char *res_bearer_name;
	int err;
...
static ZTEST_F(ccp_server_test_suite, test_bt_ccp_server_get_bearer_uci_inval_null_bearer)
{
	const char *res_bearer_name;
	int err;
	...
	err = bt_ccp_server_get_bearer_uci(NULL, &res_bearer_name);
	zassert_equal(err, -EINVAL, "Unexpected return value %d", err);
}

static ZTEST_F(ccp_server_test_suite, test_bt_ccp_server_get_bearer_uci_inval_null_name)
{
	int err;

	register_default_bearer(fixture);

	err = bt_ccp_server_get_bearer_uci(fixture->bearers[0], NULL);
	zassert_equal(err, -EINVAL, "Unexpected return value %d", err);
}
