...
static inline void expect_bt_bap_stream_ops_configured_called(
	...
	const struct bt_bap_codec_pref pref[])
{
	...
	uint8_t reasons[])
{
	...
	for (unsigned int i = 0; i < mock_bap_stream_stopped_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_stopped_cb_fake.arg0_history[i],
			"'%s()' was called with incorrect '%s[%i]' value", func_name, "stream", i);
		zexpect_equal(reasons[i], mock_bap_stream_stopped_cb_fake.arg1_history[i],
