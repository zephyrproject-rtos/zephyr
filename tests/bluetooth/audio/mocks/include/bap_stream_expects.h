...
#define expect_bt_bap_stream_ops_configured_called(_expected_count, _streams, _pref)               \
do {                                                                                               \
	const char *func_name = "bt_bap_stream_ops.configured";                                        \
												                                                   \
	zexpect_call_count(func_name, _expected_count, mock_bap_stream_configured_cb_fake.call_count); \
												                                                   \
    for (int i = 0; i < mock_bap_stream_configured_cb_fake.call_count; i++) {                      \
		IF_NOT_EMPTY(_streams[i], (                                                                \
			zexpect_equal_ptr(_streams[i],                                                         \
...
static inline void expect_bt_bap_stream_ops_qos_set_called(unsigned int expected_count, struct bt_bap_stream *streams[])
{
	const char *func_name = "bt_bap_stream_ops.qos_set";

	zexpect_call_count(func_name, expected_count, mock_bap_stream_qos_set_cb_fake.call_count);

	for (int i = 0; i < mock_bap_stream_qos_set_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_qos_set_cb_fake.arg0_history[i],
...
static inline void expect_bt_bap_stream_ops_enabled_called(unsigned int expected_count, struct bt_bap_stream *streams[])
{
	...
	for (int i = 0; i < mock_bap_stream_enabled_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_enabled_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}
								...
								struct bt_bap_stream *streams)
{
	...
	zexpect_call_count(func_name, expected_count, mock_bap_stream_metadata_updated_cb_fake.call_count);

	for (int i = 0; i < mock_bap_stream_metadata_updated_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_metadata_updated_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}
...
static inline void expect_bt_bap_stream_ops_disabled_called(unsigned int expected_count, struct bt_bap_stream *streams[])
{
	...
	for (int i = 0; i < mock_bap_stream_disabled_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_disabled_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}
...
static inline void expect_bt_bap_stream_ops_released_called(unsigned int expected_count, const struct bt_bap_stream *streams[])
{
...
static inline void expect_bt_bap_stream_ops_started_called(unsigned int expected_count, struct bt_bap_stream *streams[])
{
	...
	for (unsigned int i = 0; i < mock_bap_stream_started_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_started_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}
...
#define expect_bt_bap_stream_ops_stopped_called(_expected_count, _streams, _reasons)            \
do {                                                                                            \
	const char *func_name = "bt_bap_stream_ops.stopped";                                        \
												                                                \
	zexpect_call_count(func_name, _expected_count, mock_bap_stream_stopped_cb_fake.call_count); \
												                                                \
	for (unsigned int i = 0; i < mock_bap_stream_stopped_cb_fake.call_count; i++) {             \
		IF_NOT_EMPTY(_streams[i], (                                                             \
			zexpect_equal_ptr(_streams[i],                                                      \
				mock_bap_stream_stopped_cb_fake.arg0_history[i],                                \
				"'%s()' was called with incorrect '%s[%i]' value",                              \
				func_name, "stream", i);))                                                      \
												                                                \
		IF_NOT_EMPTY(_reasons[i], (                                                             \
			zexpect_equal(_reasons[i],                                                          \
...
static inline void
expect_bt_bap_stream_ops_connected_called(unsigned int expected_count, const struct bt_bap_stream *streams[])
{
	const char *func_name = "bt_bap_stream_ops.connected";

	zexpect_call_count(func_name, expected_count, mock_bap_stream_connected_cb_fake.call_count);

	for (unsigned int i = 0; i < mock_bap_stream_connected_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_connected_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}

static inline void
expect_bt_bap_stream_ops_disconnected_called(unsigned int expected_count, const struct bt_bap_stream *streams[])
{
	const char *func_name = "bt_bap_stream_ops.disconnected";

	zexpect_call_count(func_name, expected_count, mock_bap_stream_disconnected_cb_fake.call_count);

	for (unsigned int i = 0; i < mock_bap_stream_disconnected_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_disconnected_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
}

static inline void expect_bt_bap_stream_ops_recv_called(unsigned int expected_count,
							     struct bt_bap_stream *streams[],
							     ...
							     struct net_buf *buf)
{
	...
	for (unsigned int i = 0; i < mock_bap_stream_recv_cb_fake.call_count; i++) {
		zexpect_equal_ptr(streams[i], mock_bap_stream_recv_cb_fake.arg0_history[i],
				  "'%s()' was called with incorrect '%s[%i]'", func_name, "stream", i);
	}
...
static inline void expect_bt_bap_stream_ops_sent_called(unsigned int expected_count,
							struct bt_bap_stream *streams[])
{
