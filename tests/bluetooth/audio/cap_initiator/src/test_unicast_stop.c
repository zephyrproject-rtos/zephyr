	...
	struct cap_initiator_test_unicast_stop_fixture *fixture)
{
	...
	group_param.params_count = pair_idx;
					   ...
					   size_t index)
{
	...
	if (IS_ENABLED(CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK) &&
	    IS_ENABLED(CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SRC)) {
		conn_index = (index / 2U) % ARRAY_SIZE(fixture->conns);
	} else {
		conn_index = index % ARRAY_SIZE(fixture->conns);
	}
					   ...
					   size_t index)
{
	...
	if (IS_ENABLED(CONFIG_BT_BAP_UNICAST_CLIENT_ASE_SNK) &&
		...
		const size_t conn_index = index % ARRAY_SIZE(fixture->conns);
		const size_t ep_index = index / ARRAY_SIZE(fixture->conns);
...
static void init_default_params(struct cap_initiator_test_unicast_stop_fixture *fixture)
{
	...
	ARRAY_FOR_EACH(fixture->audio_stop_streams, i) {
		if (fixture->audio_stop_streams[i] == NULL) {
			break;
		}

		fixture->audio_stop_param.count++;
	}
...
static void cap_initiator_test_unicast_stop_after(void *f)
{
	...
	if (fixture->unicast_group != NULL) {
		...
		ARRAY_FOR_EACH(cap_stream_ptrs, idx) {
			/* Stop and release only streams that have been configured */
			if (fixture->cap_streams[idx].bap_stream.conn != NULL) {
				cap_stream_ptrs[param.count] = &fixture->cap_streams[idx];
				param.count++;
			}
		}
