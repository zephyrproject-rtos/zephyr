...
ZTEST(audio_ccid_test_suite, test_bt_ccid_alloc_value)
{
	/* Verify that we can properly wrap CCID allocation */
	for (uint16_t i = 0U; i < MAX_CCID_CNT * 2; i++) {
		const int ret = bt_ccid_alloc_value();

		zassert_true(ret == (i % MAX_CCID_CNT), "Unexpected return value %d", ret);
	}
}
...
ZTEST(audio_ccid_test_suite, test_bt_ccid_alloc_value_all_allocated)
{
...
ZTEST(audio_ccid_test_suite, test_bt_ccid_find_attr)
{
	struct bt_gatt_service test_svc = BT_GATT_SERVICE(test_attrs);
