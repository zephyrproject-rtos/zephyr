	...
	void *codec[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_config_fake.call_count; i++) {
		zassert_equal_ptr(conns[i], mock_bap_unicast_server_cb_config_fake.arg0_history[i],
			"'%s()' was called with incorrect 'conn[%i]' value", func_name, i);
		zassert_equal_ptr(eps[i], mock_bap_unicast_server_cb_config_fake.arg1_history[i],
			"'%s()' was called with incorrect 'ep[%i]' value", func_name, i);
		zassert_equal(dirs[i], mock_bap_unicast_server_cb_config_fake.arg2_history[i],
	...
	void *codec[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_reconfig_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_reconfig_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
		zassert_equal(dirs[i], mock_bap_unicast_server_cb_reconfig_fake.arg1_history[i],
			"'%s()' was called with incorrect 'dir[%i]' value", func_name, i);
	}
	...
	void *qos[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_qos_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_qos_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
	...
	void *meta_len[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_enable_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_enable_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
	...
	void *meta_len[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_metadata_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_metadata_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
	...
	struct bt_bap_stream *streams[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_disable_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_disable_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
}
	...
	struct bt_bap_stream *streams[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_release_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_release_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
}
	...
	struct bt_bap_stream *streams[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_start_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_start_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
}
	...
	struct bt_bap_stream *streams[])
{
	...
	for (unsigned int i = 0; i < mock_bap_unicast_server_cb_stop_fake.call_count; i++) {
		zassert_equal_ptr(streams[i], mock_bap_unicast_server_cb_stop_fake.arg0_history[i],
			"'%s()' was called with incorrect 'stream[%i]' value", func_name, i);
	}
}
