...
ZTEST_F(ascs_test_suite, test_sink_ase_read_state_idle)
{
	...
	zassert_equal(0x00U, hdr.ase_state, "unexpected ASE_State 0x%02x", hdr.ase_state);
}
...
ZTEST_F(ascs_test_suite, test_abort_client_operation_if_callback_not_registered)
{
	...
	notify_params = mock_bt_gatt_notify_cb_fake.arg1_val;
	hdr = (void *)notify_params->data;
	zassert_equal(0x01U, hdr->opcode, "unexpected Opcode 0x%02x", hdr->opcode);
	zassert_equal(0x01U, hdr->number_of_ases, "unexpected Number_of_ASEs 0x%02x",
		      hdr->number_of_ases);
	param = (void *)hdr->params;
	zassert_equal(ase_id, param->ase_id, "unexpected ASE_ID 0x%02x", param->ase_id);
	/* Expect Unspecified Error */
	zassert_equal(0x0EU, param->response_code, "unexpected Response_Code 0x%02x",
		      param->response_code);
	zassert_equal(0x00U, param->reason, "unexpected Reason 0x%02x", param->reason);
}
