...
static void cap_handover_broadcast_to_unicast_test_suite_before(void *f)
{
	...
	ARRAY_FOR_EACH_PTR(fixture->eps, ep) {
	...
	ARRAY_FOR_EACH(fixture->cap_streams, i) {
		...
		fixture->unicast_audio_start_stream_params[i].member.member = &fixture->conns[i];

		fixture->unicast_group_stream_params[i].stream = &fixture->cap_streams[i];
		fixture->unicast_group_stream_params[i].qos_cfg = &fixture->unicast_presets[i].qos;

		fixture->unicast_group_stream_pair_params[i].tx_param =
	       ...
	       test_handover_broadcast_to_unicast_inval_stream_state)
{
	...
	/* Attempt to use a stream not in the broadcast source */
	fixture->unicast_audio_start_stream_params[0].stream->bap_stream.ep->status.state =
