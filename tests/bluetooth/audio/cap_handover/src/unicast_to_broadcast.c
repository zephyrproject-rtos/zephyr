...
static void cap_handover_test_suite_before(void *f)
{
	...
	while (stream_cnt < ARRAY_SIZE(fixture->unicast_group_stream_params)) {
		...
		fixture->unicast_group_stream_params[stream_cnt].qos_cfg =
...
static void cap_handover_test_suite_after(void *f)
{
	...
	if (fixture->unicast_group != NULL) {
		...
		bt_cap_initiator_unicast_audio_stop(&param);
	...
	if (mock_unicast_to_broadcast_complete_cb_fake.arg3_history[0] != NULL) {
		...
		bt_cap_initiator_broadcast_audio_stop(broadcast_source);
		bt_cap_initiator_broadcast_audio_delete(broadcast_source);
	}
}
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inactive_adv)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_null_param)
{
	int err = 0;

	err = bt_cap_initiator_unicast_to_broadcast(NULL);
	zassert_equal(err, -EINVAL, "Unexpected return value %d", err);
}

static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_null_unicast_group)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_null_ext_adv)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_ext_adv_no_pa)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_pa_interval)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_broadcast_id)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite,
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_broadcast_stream)
{
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_unique_metadata)
{
	int err = 0;
...
static ZTEST_F(cap_handover_test_suite, test_handover_unicast_to_broadcast_inval_unicast_group)
{
	int err = 0;
