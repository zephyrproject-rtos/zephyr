				       ...
				       const struct bt_bap_broadcast_assistant_add_src_param *param)
{
	...
	state.src_id = inst->recv_state.src_id = 1U;
	state.adv_sid = inst->recv_state.adv_sid = param->adv_sid;
	state.broadcast_id = inst->recv_state.broadcast_id = param->broadcast_id;
