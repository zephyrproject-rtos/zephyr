...
static void bap_broadcast_source_started_cb(struct bt_bap_broadcast_source *source)
{
	...
	ARRAY_FOR_EACH_PTR(broadcast_assistants, assistant) {
		 ...
		 */
		for (uint8_t i = 0U; i < assistant->recv_state.num_subgroups; i++) {
			if (assistant->recv_state.subgroups[i].bis_sync != 0U) {
				receive_state_callback = false;
				break;
			}
			/* Set the BIS sync to any valid value */
			assistant->recv_state.subgroups[i].bis_sync = BIT(i);
		}

		if (receive_state_callback && broadcast_assistant_cb != NULL &&
		    broadcast_assistant_cb->recv_state != NULL) {
				       ...
				       const struct bt_bap_broadcast_assistant_add_src_param *param)
{
	...
	__ASSERT(param->adv_sid == TEST_COMMON_ADV_SID, "Unexpected param->adv_sid: 0x%02X",
		 param->adv_sid);
	__ASSERT(param->addr.type == TEST_COMMON_ADV_TYPE, "Unexpected param->addr.type: 0x%02X",
		 param->addr.type);
	...
	assistant->recv_state.pa_sync_state =
		param->pa_sync ? BT_BAP_PA_STATE_SYNCED : BT_BAP_PA_STATE_NOT_SYNCED;
	assistant->recv_state.num_subgroups = param->num_subgroups;
...
int bt_bap_broadcast_assistant_rem_src(struct bt_conn *conn, uint8_t src_id)
{
	...
	zassert_equal(BT_BAP_PA_STATE_NOT_SYNCED, assistant->recv_state.pa_sync_state,
		      "Invalid sync state");
	for (uint8_t i = 0U; i < assistant->recv_state.num_subgroups; i++) {
		zassert_equal(0U, assistant->recv_state.subgroups[i].bis_sync);
	}
	(void)memset(&assistant->recv_state, 0, sizeof(assistant->recv_state));
	...
	if (broadcast_assistant_cb != NULL) {
		...
		if (broadcast_assistant_cb->recv_state_removed != NULL) {
			broadcast_assistant_cb->recv_state_removed(conn, src_id);
		}
	}
