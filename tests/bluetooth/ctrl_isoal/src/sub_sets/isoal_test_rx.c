...
{
	custom_sink_sdu_alloc_test_output_buffer.out
		[custom_sink_sdu_alloc_test_output_buffer.buffer_size++] = buf;
						 ...
						 struct isoal_sdu_buffer *sdu_buffer)
{
	...
	zassert_true(custom_sink_sdu_alloc_test_output_buffer.pos <
		     ...
		     NULL);
	memcpy(sdu_buffer, custom_sink_sdu_alloc_test_output_buffer.out
				   [custom_sink_sdu_alloc_test_output_buffer.pos++],
	       sizeof(*sdu_buffer));
...
#define ZASSERT_ISOAL_SDU_ALLOC_TEST(_typ, _sink, _pdu)                                            \
	zassert_equal_ptr(_sink, sink_sdu_alloc_test_fake.arg0_##_typ, "\t\t%p != %p", _sink,      \
			  sink_sdu_alloc_test_fake.arg0_##_typ);                                   \
	zassert_equal_ptr(_pdu, sink_sdu_alloc_test_fake.arg1_##_typ, "\t\t%p != %p", _pdu,        \
			  sink_sdu_alloc_test_fake.arg1_##_typ)

FAKE_VALUE_FUNC(isoal_status_t, sink_sdu_emit_test, const struct isoal_sink *,
		const struct isoal_emitted_sdu_frag *, const struct isoal_emitted_sdu *);
 ...
 */
FAKE_VOID_FUNC(sink_sdu_emit_test_handler, struct isoal_sink, struct isoal_emitted_sdu_frag,
...
#define ZASSERT_ISOAL_SDU_EMIT_TEST(_typ, _sink, _state, _frag_sz, _frag_status, _timestamp,       \
				    _seqn, _dbuf, _dbuf_sz, _total_sz, _sdu_status)                \
	zassert_equal_ptr(_sink, sink_sdu_emit_test_fake.arg0_##_typ, "\t\t%p != %p", _sink,       \
	...
	zassert_equal(_seqn, sink_sdu_emit_test_handler_fake.arg1_##_typ.sdu.seqn, "\t\t%d != %d", \
...
#define ZASSERT_ISOAL_SDU_WRITE_TEST(_typ, _frag_buf, _payload_buf, _length)                       \
	zassert_equal_ptr(_frag_buf, sink_sdu_write_test_fake.arg0_##_typ, "\t\t%p != %p",         \
			  _frag_buf, sink_sdu_write_test_fake.arg0_##_typ);                        \
	zassert_equal_ptr(_payload_buf, sink_sdu_write_test_fake.arg1_##_typ, "\t\t%p != %p",      \
			  _payload_buf, sink_sdu_write_test_fake.arg1_##_typ);                     \
	zassert_equal(_length, sink_sdu_write_test_fake.arg2_##_typ, "\t\t%d != %d", _length,      \
					       ...
					       uint32_t group_sync_delay)
{
	...
	PRINT("RX Test Setup:\n\tHandle 0x%04x\n\tRole %s\n\tFraming %s"
	      ...
	      flush_timeout, (iso_interval_int * CONN_INT_UNIT_US), sdu_interval, stream_sync_delay,
	      group_sync_delay);
#endif
	...
	for (int role = 0; role <= 3; role++) {
		...
		for (int i = 0; i < CONFIG_BT_CTLR_ISOAL_SINKS; i++) {
			...
			zassert_equal(isoal_global.sink_state[sink_hdl[i]].sdu_production.mode,
				      ...
				      "Sink %d in role %s enable failed!", i, ROLE_TO_STR(role));
...
ZTEST(test_rx_basics, test_sink_disable)
{
	...
	isoal_test_create_unframed_pdu(PDU_BIS_LLID_COMPLETE_END, &testdata[testdata_indx],
				       (testdata_size - testdata_indx), payload_number,
				       pdu_timestamp, ISOAL_PDU_STATUS_VALID,
...
ZTEST(test_rx_unframed, test_rx_unframed_dbl_pdu)
{
	...
	isoal_test_create_unframed_pdu(PDU_BIS_LLID_START_CONTINUE, &testdata[testdata_indx],
				       (testdata_size - testdata_indx), payload_number,
				       pdu_timestamp, ISOAL_PDU_STATUS_VALID,
				       &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_unframed, test_rx_unframed_single_pdu_err)
{
	...
	isoal_test_create_unframed_pdu(PDU_BIS_LLID_COMPLETE_END, &testdata[testdata_indx],
				       (testdata_size - testdata_indx), payload_number,
				       pdu_timestamp, ISOAL_PDU_STATUS_ERRORS,
				       &rx_pdu_meta_buf.pdu_meta);
	...
	isoal_test_create_unframed_pdu(PDU_BIS_LLID_COMPLETE_END, &testdata[testdata_indx],
				       (testdata_size - testdata_indx), payload_number,
				       pdu_timestamp, ISOAL_PDU_STATUS_LOST_DATA,
				       &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_unframed, test_rx_unframed_dbl_pdu_invalid_llid1)
{
	...
	isoal_test_create_unframed_pdu(
		PDU_BIS_LLID_FRAMED, &testdata[testdata_indx], (testdata_size - testdata_indx),
		payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_unframed, test_rx_unframed_dbl_pdu_invalid_llid2_pdu_err)
{
	...
	isoal_test_create_unframed_pdu(
		PDU_BIS_LLID_FRAMED, &testdata[testdata_indx], (testdata_size - testdata_indx),
		payload_number, pdu_timestamp, ISOAL_PDU_STATUS_ERRORS, &rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_single_pdu_single_sdu)
{
	...
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_single_sdu)
{
	...
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = isoal_test_add_framed_pdu_start(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_dbl_sdu)
{
	...
	pdu_data_loc[2] = isoal_test_add_framed_pdu_start(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_zero_length_sdu)
{
	...
	/* Zero length SDU */
	pdu_data_loc[2] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
	...
	pdu_data_loc[3] = isoal_test_add_framed_pdu_start(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_padding)
{
	...
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[1] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_pdu_err1)
{
	...
	/* PDU with errors */
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_ERRORS,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_dbl_pdu_dbl_sdu_pdu_err2)
{
	...
	/* PDU with errors */
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_LOST_DATA,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[0] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_single_sdu_pdu_err1)
{
	...
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[3] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_trppl_pdu_dbl_sdu_pdu_err1)
{
	...
	isoal_test_create_framed_pdu_base(payload_number, pdu_timestamp, ISOAL_PDU_STATUS_VALID,
					  &rx_pdu_meta_buf.pdu_meta);
	pdu_data_loc[4] = isoal_test_add_framed_pdu_single(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
...
ZTEST(test_rx_framed, test_rx_framed_single_invalid_pdu_single_sdu_hdr_err)
{
	...
	pdu_data_loc[1] = isoal_test_add_framed_pdu_start(
		&testdata[testdata_indx], (testdata_size - testdata_indx), sdu_timeoffset,
		&rx_pdu_meta_buf.pdu_meta);
