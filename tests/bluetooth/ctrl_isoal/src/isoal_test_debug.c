...
void isoal_test_debug_print_rx_pdu(struct isoal_pdu_rx *pdu_meta)
{
	...
	PRINT("\n");
	PRINT("PDU %04u (%10u) | %12s [%10s] %03u: ", (uint32_t)pdu_meta->meta->payload_number,
				   ...
				   const struct isoal_emitted_sdu *sdu)
{
	...
	PRINT("\n");
	PRINT("SDU %04d (%10d) | %12s [%10s] %03d: ", sdu_frag->sdu.seqn, sdu_frag->sdu.timestamp,
	      STATE_TO_STR(sdu_frag->sdu_state), DU_ERR_TO_STR(sdu_frag->sdu.status), len);
	for (int i = 0; i < len; i++) {
		PRINT("%02x ", buf[i]);
	}
	PRINT("\n");

	PRINT("    %17s   %12s [%10s] %03d  ", "Collated-",
	      (sdu->total_sdu_size != len || sdu->collated_status != sdu_frag->sdu.status
		       ? "!! DIFF !!"
		       : ""),
	      DU_ERR_TO_STR(sdu->collated_status), sdu->total_sdu_size);
	PRINT("\n");
	PRINT("\n");
}
...
void isoal_test_debug_print_tx_pdu(struct node_tx_iso *node_tx)
{
	...
	PRINT("\n");
	PRINT("PDU %04u (    %02u    ) | %12s | %03u: ", (uint32_t)node_tx->payload_count,
	      node_tx->sdu_fragments, LLID_TO_STR(pdu->ll_id), pdu->length);

	for (int i = 0; i < pdu->length; i++) {
		if (seg_length == 0 && pdu->ll_id == PDU_BIS_LLID_FRAMED) {
			seg_length = pdu->payload[i + 1];
			PRINT("[%s %s %03u]", pdu->payload[i] & BIT(0) ? "C" : "S",
			      pdu->payload[i] & BIT(1) ? "C" : "-", pdu->payload[i + 1]);
			if ((pdu->payload[i] & BIT(0)) == 0) {
				PRINT("(%8uus)", ((uint32_t)pdu->payload[i + 2] +
						  ((uint32_t)pdu->payload[i + 3] << 8) +
						  ((uint32_t)pdu->payload[i + 4] << 16)));
			}
			PRINT(" / ");
			PRINT("[%02x %02x]", pdu->payload[i], pdu->payload[i + 1]);
			if ((pdu->payload[i] & BIT(0)) == 0) {
				PRINT("(%02x %02x %02x)", (uint32_t)pdu->payload[i + 4],
				      (uint32_t)pdu->payload[i + 3], (uint32_t)pdu->payload[i + 2]);
			}
			PRINT(" : ");
...
void isoal_test_debug_print_tx_sdu(struct isoal_sdu_tx *tx_sdu)
{
	...
	PRINT("\n");
	PRINT("SDU %04u (%10u) | %12s | %03u: ", tx_sdu->packet_sn, tx_sdu->time_stamp,
	      STATE_TO_STR(tx_sdu->sdu_state), len);
	for (int i = 0; i < len; i++) {
		PRINT("%02x ", buf[i]);
	}
	PRINT("\n");
