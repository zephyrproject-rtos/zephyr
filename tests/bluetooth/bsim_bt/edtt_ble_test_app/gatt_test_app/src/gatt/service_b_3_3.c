...
static ssize_t read_value_v6(struct bt_conn *conn, const struct bt_gatt_attr *attr, void *buf, u16_t len, u16_t offset)
{
    ...
    return bt_gatt_attr_read(conn, attr, buf, len, offset, value, sizeof(value_v6_value));
}
 ...
 */
static void value_v6_ccc_cfg_changed(const struct bt_gatt_attr *attr, u16_t value)
{
...
static struct bt_gatt_attr service_b_3_3_attrs[] = {
    BT_GATT_PRIMARY_SERVICE(BT_UUID_SERVICE_B_3, 0x50),
    BT_GATT_CHARACTERISTIC(BT_UUID_VALUE_V6,
        BT_GATT_CHRC_READ | BT_GATT_CHRC_WRITE_WITHOUT_RESP | BT_GATT_CHRC_WRITE | BT_GATT_CHRC_NOTIFY | BT_GATT_CHRC_INDICATE,
        BT_GATT_PERM_READ | BT_GATT_PERM_WRITE,
        read_value_v6, write_value_v6, &value_v6_value, 0x51),
    BT_GATT_CCC(value_v6_ccc_cfg, value_v6_ccc_cfg_changed, 0x53)
};

static struct bt_gatt_service service_b_3_3_svc = BT_GATT_SERVICE(service_b_3_3_attrs);
...
/**
 * @brief Generate notification for 'Value V6' attribute, if notifications are enabled.
 */
void service_b_3_3_value_v6_notify(void)
{
    ...
    bt_gatt_notify(NULL, &service_b_3_3_attrs[1], &value_v6_value, sizeof(value_v6_value));
}
 ...
 * @param err  The error code from the indicate attempt, 0 if no error or BT_GATT_ERR() with a specific ATT error code.
 */
static void value_v6_indicate_cb(struct bt_conn *conn, const struct bt_gatt_attr *attr, u8_t err)
{
    printk("Indication for attribute 'Value V6' %s\n", (err) ? "failed" : "succeded");
}

/**
 * @brief Generate indication for 'Value V6' attribute, if indications are enabled.
 */
void service_b_3_3_value_v6_indicate(void)
{
    if (!value_v6_ind_active)
        return;
    /*
     * NOTE: Zephyr doesn't automatically bump up the attribute pointer for indications as it does for notifications.
     */
