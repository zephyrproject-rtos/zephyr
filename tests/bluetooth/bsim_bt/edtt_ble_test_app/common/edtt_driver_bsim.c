...
static int edtt_autoshutdown = false;
...
static void edptd_create_fifo_if()
{
	...
	fifo_path[TO_DEVICE] = (char*) bs_calloc(pb_com_path_length + 30, sizeof(char));
	fifo_path[TO_BRIDGE] = (char*) bs_calloc(pb_com_path_length + 30, sizeof(char));
	sprintf(fifo_path[TO_DEVICE], "%s/Device%i.PTTin",  pb_com_path, global_device_nbr);
	sprintf(fifo_path[TO_BRIDGE], "%s/Device%i.PTTout", pb_com_path, global_device_nbr);
	...
	if ((fifo[TO_BRIDGE] = open(fifo_path[TO_BRIDGE], O_WRONLY)) == -1) {
	...
	if ((fifo[TO_DEVICE] = open(fifo_path[TO_DEVICE], O_RDONLY)) == -1) {
		bs_trace_error_line("Couldn't create FIFOs for EDTT IF\n");
	}
