...
static struct bt_audio_stream *broadcast_streams[ARRAY_SIZE(broadcast_sink_streams)];
...
/* Create a mask for the maximum BIS we can sync to using the number of broadcast_streams
 * we have. We add an additional 1 since the bis indexes start from 1 and not
 * 0.
 */
static const uint32_t bis_index_mask = BIT_MASK(ARRAY_SIZE(broadcast_streams) + 1U);
...
static void init(void)
{
	...
	if (IS_ENABLED(CONFIG_BT_AUDIO_BROADCAST_SINK)) {
		...
		for (size_t i = 0U; i < ARRAY_SIZE(broadcast_streams); i++) {
			broadcast_streams[i] = &broadcast_sink_streams[i];
