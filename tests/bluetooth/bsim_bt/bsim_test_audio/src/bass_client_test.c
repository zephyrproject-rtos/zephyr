...
#include "../../../../../subsys/bluetooth/host/audio/bass.h"
				      ...
				      const struct bt_bass_recv_state *state)
{
	...
	bin2hex(state->bad_code, BASS_BROADCAST_CODE_SIZE, bad_code,
		sizeof(bad_code));
	printk("BASS recv state: src_id %u, addr %s, sid %u, sync_state %u, "
	       ...
	       state->encrypt_state == BASS_BIG_ENC_STATE_BAD_CODE ? ", bad code" : "", bad_code);
	...
	g_state_synced = state->pa_sync_state == BASS_PA_STATE_SYNCED;
...
static void test_bass_discover(void)
{
	...
	if (err) {
		FAIL("Failed to discover BASS %d", err);
		return;
	}
...
static void test_bass_broadcast_code(void)
{
	...
	g_write_complete = false;
	err = bt_bass_client_broadcast_code(g_conn, g_src_id, broadcast_code);
	if (err) {
