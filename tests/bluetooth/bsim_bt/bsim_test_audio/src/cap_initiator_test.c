...
#define BROADCAST_STREMT_CNT CONFIG_BT_AUDIO_BROADCAST_SRC_STREAM_COUNT
#else
...
static struct bt_audio_lc3_preset unicast_preset_16_2_1 =
	BT_AUDIO_LC3_UNICAST_PRESET_16_2_1(BT_AUDIO_LOCATION_FRONT_LEFT,
					   BT_AUDIO_CONTEXT_TYPE_UNSPECIFIED);

static struct bt_cap_stream unicast_client_streams[CONFIG_BT_AUDIO_UNICAST_CLIENT_ASE_SNK_COUNT];
static struct bt_codec *unicast_remote_codecs[CONFIG_BT_AUDIO_UNICAST_CLIENT_PAC_COUNT];
static struct bt_audio_ep *unicast_sink_eps[CONFIG_BT_AUDIO_UNICAST_CLIENT_ASE_SNK_COUNT];
...
CREATE_FLAG(flag_unicast_stream_qos);
CREATE_FLAG(flag_unicast_stream_enabled);
CREATE_FLAG(flag_unicast_stream_released);
CREATE_FLAG(flag_unicast_audio_started);
...
static void unicast_stream_configured(struct bt_audio_stream *stream,
			      const struct bt_codec_qos_pref *pref)
{
	...
	SET_FLAG(flag_unicast_audio_started);
}

static void unicast_stream_qos_set(struct bt_audio_stream *stream)
{
	printk("QoS set stream %p\n", stream);

	SET_FLAG(flag_unicast_stream_qos);
}

static void unicast_stream_enabled(struct bt_audio_stream *stream)
{
	printk("Enabled stream %p\n", stream);

	SET_FLAG(flag_unicast_stream_enabled);
}

static void unicast_stream_started(struct bt_audio_stream *stream)
{
	printk("Started stream %p\n", stream);
}

static void unicast_stream_metadata_updated(struct bt_audio_stream *stream)
{
	printk("Metadata updated stream %p\n", stream);
}

static void unicast_stream_disabled(struct bt_audio_stream *stream)
{
	printk("Disabled stream %p\n", stream);
}

static void unicast_stream_stopped(struct bt_audio_stream *stream)
{
	printk("Stopped stream %p\n", stream);
}

static void unicast_stream_released(struct bt_audio_stream *stream)
{
	printk("Released stream %p\n", stream);

	SET_FLAG(flag_unicast_stream_released);
}

static struct bt_audio_stream_ops unicast_stream_ops = {
			     ...
			     enum bt_audio_dir dir)
{
	...
	if (index < CONFIG_BT_AUDIO_UNICAST_CLIENT_PAC_COUNT) {
		unicast_remote_codecs[index] = codec;
	}
}
...
static void unicast_audio_start(void)
{
	...
	UNSET_FLAG(flag_unicast_audio_started);
	...
	WAIT_FOR_FLAG(flag_unicast_audio_started);
}
