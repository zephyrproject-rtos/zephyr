...
ZTEST_SUITE(bt_le_oob_set_sc_data_invalid_inputs, NULL, NULL, NULL, NULL, NULL);
 ...
 */
ZTEST(bt_le_oob_set_sc_data_invalid_inputs, test_null_conn_reference)
{
	const struct bt_le_oob_sc_data oobd_local = {0};
	const struct bt_le_oob_sc_data oobd_remote = {0};

	expect_assert();
	bt_le_oob_set_sc_data(NULL, &oobd_local, &oobd_remote);
}
 ...
 */
ZTEST(bt_le_oob_set_sc_data_invalid_inputs, test_dev_ready_flag_not_set)
{
	int err;
