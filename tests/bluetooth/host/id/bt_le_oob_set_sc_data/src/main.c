...
ZTEST_SUITE(bt_le_oob_set_sc_data, NULL, NULL, NULL, NULL, NULL);

/*
 *  Test setting OOB information and verify that input arguments are passed correctly to
 ...
 *
 *  Expected behaviour:
 *   - bt_le_oob_set_sc_data() returns 0 (success)
 */
ZTEST(bt_le_oob_set_sc_data, test_passing_arguments_correctly)
{
	int err;
	...
	expect_single_call_bt_smp_le_oob_set_sc_data(&conn, &oobd_local, &oobd_remote);

	zassert_ok(err, "Unexpected error code '%d' was returned", err);
}

/*
 *  Test setting OOB information and verify it fails when bt_smp_le_oob_set_sc_data() fails.
 *
 *  Constraints:
 *   - Valid references are used for input parameters
 *   - 'BT_DEV_READY' bit isn't set in bt_devs[0].flags
 ...
 *
 *  Expected behaviour:
 *   - bt_le_oob_set_sc_data() returns '-EINVAL' (failure)
 */
ZTEST(bt_le_oob_set_sc_data, test_bt_smp_le_oob_set_sc_data_fails)
{
	int err;
	struct bt_conn conn = {0};
	const struct bt_le_oob_sc_data oobd_local = {0};
