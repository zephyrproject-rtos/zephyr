 ...
 * -ENOTSUP.
 */
static ZTEST(gatt_dynamic_db, test_register_unregister_enotsup_after_bt_enable)
{
	int err;
	...
	zassert_equal(err, 0, "unregister test_svc failed (%d)", err);

	err = bt_gatt_service_unregister(&test1_svc);
	zassert_equal(err, 0, "unregister test1_svc failed (%d)", err);
