...
static struct bt_conn *get_conn_by_addr(const bt_addr_le_t *address)
{
	...
	if (bt_addr_eq(bt_conn_get_dst_br(default_conn), &address->a)) {
		return default_conn;
	}
...
static uint8_t unit_info(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	if (get_conn_by_addr(&cp->address) == NULL) {
		return BTP_STATUS_FAILED;
	}
				     ...
				     struct bt_avrcp_event_data *data)
{
	...
	bt_addr_copy(&ev->address.a, bt_conn_get_dst_br(default_conn));
...
static int encode_folder_item(struct net_buf *buf, const struct folder_item *item)
{
	...
	memcpy(folder_item->name, item->hdr.name, item->hdr.name_len);
...
static uint8_t ca_ct_connect(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	if (conn == NULL) {
		...
		err = bt_sdp_discover(conn, &discov_avrcp_tg);
	}
...
#if defined(CONFIG_BT_AVRCP_TG_COVER_ART)
static const char *ca_tg_strnstr(const char *haystack, const char *needle, size_t len)
{
	size_t needle_len;
				      ...
				      struct image_variant **out_variant)
{
	...
	for (uint8_t i = 0; i < item->num_variants; i++) {
		if ((ca_tg_strnstr(hdr.data, item->variants[i].encoding, hdr.len) != NULL) &&
		    (ca_tg_strnstr(hdr.data, item->variants[i].pixel, hdr.len) != NULL)) {
