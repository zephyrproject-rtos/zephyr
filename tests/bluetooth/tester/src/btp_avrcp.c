				  ...
				  struct net_buf *buf)
{
	...
	struct bt_avrcp_media_attr *dst_attr;
	uint8_t *dst_ptr;
	uint32_t total_len;
	uint16_t attr_len;
	...
	while (buf->len > 0) {
		...
		dst_attr = (struct bt_avrcp_media_attr *)dst_ptr;
		...
		dst_ptr += sizeof(*dst_attr) + attr_len;
	}
...
static void get_folder_items_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	while (buf->len > 0) {
		...
		case BT_AVRCP_ITEM_TYPE_MEDIA_PLAYER: {
			...
			src_item = (struct bt_avrcp_media_player_item *)net_buf_pull_mem(
				buf, sizeof(*src_item));
			...
			dst_item->player_subtype =
				sys_cpu_to_le32(sys_be32_to_cpu(src_item->player_subtype));
			dst_item->play_status = src_item->play_status;
			memcpy(dst_item->feature_bitmask, src_item->feature_bitmask, 16);
			dst_item->charset_id =
				sys_cpu_to_le16(sys_be16_to_cpu(src_item->charset_id));
			...
			item_len += sizeof(*src_item) + name_len;
			break;
		}
		case BT_AVRCP_ITEM_TYPE_FOLDER: {
			...
			src_item = (struct bt_avrcp_folder_item *)net_buf_pull_mem(
				buf, sizeof(*src_item));
		...
		case BT_AVRCP_ITEM_TYPE_MEDIA_ELEMENT: {
			...
			src_item = (struct bt_avrcp_media_element_item *)net_buf_pull_mem(
				buf, sizeof(*src_item));
			...
			src_name = (struct media_element_item_name *)net_buf_pull_mem(
				buf, sizeof(*src_name));
			dst_name = (struct media_element_item_name *)dst_item->data;
			dst_name->charset_id =
				sys_cpu_to_le16(sys_be16_to_cpu(src_name->charset_id));
			...
			src_attrs = (struct media_element_item_attr *)net_buf_pull_mem(
				buf, sizeof(*src_attrs));
			...
			dst_attr = (struct bt_avrcp_media_attr *)dst_attrs->attrs;
			for (uint8_t j = 0; j < src_attrs->num_attrs; j++) {
				uint16_t attr_len;

				if (buf->len < sizeof(*src_attr)) {
					goto done;
				}
				src_attr = (struct bt_avrcp_media_attr *)net_buf_pull_mem(
					buf, sizeof(*src_attr));

				dst_attr->attr_id =
					sys_cpu_to_le32(sys_be32_to_cpu(src_attr->attr_id));
				dst_attr->charset_id =
					sys_cpu_to_le16(sys_be16_to_cpu(src_attr->charset_id));
				attr_len = sys_be16_to_cpu(src_attr->attr_len);
				dst_attr->attr_len = sys_cpu_to_le16(attr_len);
				...
				memcpy(dst_attr->attr_val, net_buf_pull_mem(buf, attr_len),
				       attr_len);
				dst_attr = (struct bt_avrcp_media_attr *)(dst_attr->attr_val +
									  attr_len);
				item_len += sizeof(*src_attr) + attr_len;
			}
			break;
		}
		default:
...
static void get_item_attrs_rsp(struct bt_avrcp_ct *ct, uint8_t tid, struct net_buf *buf)
{
	...
	while (buf->len > 0U) {
		...
		src_attr = net_buf_pull_mem(buf, sizeof(*src_attr));
		dst_attr = (struct bt_avrcp_media_attr *)dst_ptr;
...
static void play_item_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(target_list, child, node) {
				  ...
				  const struct media_attr_list *attrs_list)
{
	...
	If the response containing only one item still exceeds the size which can be handled by the
	CT, then the TG should return less attributes than are requested. */
	if (buf->len == sizeof(struct bt_avrcp_get_folder_items_rsp)) {
		goto encode;
	}

	/* Caculate attribute length if specific attributes are requested */
	for (uint8_t i = 0; i < attrs_list->attr_count; i++) {
		for (uint8_t j = 0; j < item->num_attrs; j++) {
			if (sys_be32_to_cpu(attrs_list->attr_ids[i]) == item->attr[j].attr_id) {
	...
	/* Caculate attribute length if no specific attributes are requested */
	for (uint8_t i = 0; i < item->num_attrs; i++) {
...
static uint8_t encode_get_folder_items(struct net_buf *buf, sys_slist_t *item_list,
				       uint8_t item_mask,
...
static void get_folder_items_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	case BT_AVRCP_SCOPE_MEDIA_PLAYER_LIST:
		...
		for (uint32_t i = start_item; i <= end_item; i++) {
			...
			if (net_buf_tailroom(tx_buf) <
			    sizeof(*player_item) + tg_player_items[i].name_len) {
				status = BT_AVRCP_STATUS_INTERNAL_ERROR;
				goto done;
			}

			num_items++;
			player_item = net_buf_add(tx_buf, sizeof(*player_item) +
			...
			memcpy(&player_item->feature_bitmask[0],
			       &tg_player_items[i].feature_bitmask[0],
	...
	case BT_AVRCP_SCOPE_VFS: {
		...
		status = encode_get_folder_items(tx_buf, &curr_node->child,
	...
	case BT_AVRCP_SCOPE_SEARCH:
		...
		status = encode_get_folder_items(tx_buf, &tg_search_list,
						 1 << BT_AVRCP_ITEM_TYPE_MEDIA_ELEMENT, cmd,
						 &num_items);
		break;

	case BT_AVRCP_SCOPE_NOW_PLAYING:
		status = encode_get_folder_items(tx_buf, &tg_now_playing_list,
						 1 << BT_AVRCP_ITEM_TYPE_MEDIA_ELEMENT, cmd,
						 &num_items);
		break;
...
static void change_path_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	if (cmd->direction == BT_AVRCP_CHANGE_PATH_PARENT) {
		...
		SYS_SLIST_FOR_EACH_CONTAINER(&curr_node->child, child, node) {
			if ((child->item->item_type == BT_AVRCP_ITEM_TYPE_FOLDER) &&
			    (uid == child->item->uid)) {
				join_path(tg_cur_vfs_path, (const char *)child->item->name);
				status = BT_AVRCP_STATUS_OPERATION_COMPLETED;
				break;
			}
		}
...
static void get_item_attrs_req(struct bt_avrcp_tg *tg, uint8_t tid, struct net_buf *buf)
{
	...
	SYS_SLIST_FOR_EACH_CONTAINER(target_list, child, node) {
		if (uid == child->item->uid) {
			if (child->item->item_type == BT_AVRCP_ITEM_TYPE_MEDIA_ELEMENT) {
				struct media_item *item = (struct media_item *)(void *)child->item;
...
rsp:
	...
	len = 0;
	err = bt_obex_add_header_body_or_end_body(tx_buf, ca_tg_mopl,
						  ca_tg_body_len - ca_tg_body_pos,
						  ca_tg_body + ca_tg_body_pos, &len);

	if (bt_obex_has_header(tx_buf, BT_OBEX_HEADER_ID_END_BODY)) {
...
static void ca_tg_get_image_req(struct bt_avrcp_cover_art_tg *tg, bool final, struct net_buf *buf)
{
	...
	if (variant == NULL) {
		rsp_code = BT_OBEX_RSP_CODE_NOT_ACCEPT;
		goto error_rsp;
	}
