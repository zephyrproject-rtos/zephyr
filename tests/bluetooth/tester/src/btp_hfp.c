...
static void hf_ring_indication(struct bt_conn *conn)
{
	ring_alert = true;
	if (hf_accept_call) {
		bt_hfp_hf_accept(get_dialing_call(hfp_hf));
		hf_accept_call = 0;
	}
}
		       ...
		       void *rsp, uint16_t *rsp_len)
{
	...
	case HFP_AG_ANSWER_CALL:
		if (hfp_ag) {
			// bt_hfp_ag_send_call_indicator(hfp_ag, 1);
			// bt_hfp_ag_send_callsetup_indicator(hfp_ag, 0);
			// bt_hfp_ag_call_status_pl(hfp_ag, hfp_ag_call_call_incoming);
	...
	case HFP_TWC_CALL:
		if (hfp_ag) {
			...
			// {
			// 	bt_hfp_ag_send_callheld_indicator(hfp_ag, 1);
			// 	bt_hfp_ag_send_callsetup_indicator(hfp_ag, 0);
			// 	call_held = 1;
			...
			// else
			// {
			// 	bt_hfp_ag_remote_incoming(hfp_ag, "7654321");
	...
	case HFP_SEND_BCC:
		if (hfp_ag) {
			...
			// 	bt_hfp_ag_codec_selector(hfp_ag, 1);
			// }
	...
	case HFP_SEND_BCC_WBS:
		// if (hfp_ag) {
		// 	if (codecs_negotiate_done == 0) {
		// 		bt_hfp_ag_codec_selector(hfp_ag, 2 );
		// 	}
