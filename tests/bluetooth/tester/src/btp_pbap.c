...
NET_BUF_POOL_FIXED_DEFINE(pbap_tx_pool, CONFIG_BT_MAX_CONN, 1024,
...
static uint16_t peer_sdp_rfcomm_channel = 0;
static uint16_t peer_sdp_l2cap_psm = 0;
static uint32_t peer_supported_features = 0;
static uint8_t peer_supported_repositories = 0;
				  ...
				  uint16_t mopl, struct net_buf *buf)
{
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PCE_EV_CONNECTED, ev,
		     sizeof(*ev) + buf->len);
				     ...
				     struct net_buf *buf)
{
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PCE_EV_DISCONNECTED, ev,
		     sizeof(*ev) + buf->len);
...
{
	struct pbap_instance *inst = CONTAINER_OF(pbap_pce, struct pbap_instance, pbap_pce);
	struct btp_pbap_pce_pull_phonebook_ev *ev;
	uint8_t *event_buf;
	int err;
	...
	tester_rsp_buffer_allocate(sizeof(*ev) + buf->len, &event_buf);
	ev = (struct btp_pbap_pce_pull_phonebook_ev *)event_buf;
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PCE_EV_PULL_PHONEBOOK, ev,
		     sizeof(*ev) + buf->len);
...
static void pbap_pce_setpath_cb(struct bt_pbap_pce *pbap_pce, uint8_t rsp_code,
				struct net_buf *buf)
{
	struct pbap_instance *inst = CONTAINER_OF(pbap_pce, struct pbap_instance, pbap_pce);
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PCE_EV_SET_PATH, ev,
		     sizeof(*ev) + buf->len);
...
static void pbap_pce_abort_cb(struct bt_pbap_pce *pbap_pce, uint8_t rsp_code,
			      struct net_buf *buf)
{
	struct pbap_instance *inst = CONTAINER_OF(pbap_pce, struct pbap_instance, pbap_pce);
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PCE_EV_ABORT, ev,
		     sizeof(*ev) + buf->len);
				  ...
				  struct net_buf *buf)
{
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PSE_EV_CONNECT, ev,
		     sizeof(*ev) + buf->len);
...
static void pbap_pse_disconnected_cb(struct bt_pbap_pse *pbap_pse, struct net_buf *buf)
{
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PSE_EV_DISCONNECT, ev,
		     sizeof(*ev) + buf->len);
...
{
	struct pbap_instance *inst = CONTAINER_OF(pbap_pse, struct pbap_instance, pbap_pse);
	struct btp_pbap_pse_pull_phonebook_ev *ev;
	uint8_t *event_buf;
	int err;
	...
	tester_rsp_buffer_allocate(sizeof(*ev) + buf->len, &event_buf);
	ev = (struct btp_pbap_pse_pull_phonebook_ev *)event_buf;
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PSE_EV_PULL_PHONEBOOK, ev,
		     sizeof(*ev) + buf->len);
...
{
	struct pbap_instance *inst = CONTAINER_OF(pbap_pse, struct pbap_instance, pbap_pse);
	struct btp_pbap_pse_set_path_ev *ev;
	uint8_t *event_buf;
	int err;
	...
	tester_rsp_buffer_allocate(sizeof(*ev) + buf->len, &event_buf);
	ev = (struct btp_pbap_pse_set_path_ev *)event_buf;
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PSE_EV_SET_PATH, ev,
		     sizeof(*ev) + buf->len);
...
static void pbap_pse_abort_cb(struct bt_pbap_pse *pbap_pse, struct net_buf *buf)
{
	...
	tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_PSE_EV_ABORT, ev,
		     sizeof(*ev) + buf->len);
...
static uint8_t pce_rfcomm_connect(const void *cmd, uint16_t cmd_len,
				   void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pce_rfcomm_disconnect(const void *cmd, uint16_t cmd_len,
				      void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pce_l2cap_connect(const void *cmd, uint16_t cmd_len,
				  void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pce_l2cap_disconnect(const void *cmd, uint16_t cmd_len,
				     void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pce_connect(const void *cmd, uint16_t cmd_len,
			   void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pce_pull_phonebook(const void *cmd, uint16_t cmd_len,
				  void *rsp, uint16_t *rsp_len)
{
	const struct btp_pbap_pce_pull_phonebook_cmd *cp = cmd;
	struct pbap_instance *inst;
	struct net_buf *buf;
	int err;
	...
	err = bt_pbap_pce_pull_phonebook(&inst->pbap_pce, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
				      ...
				      void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_pbap_pce_pull_vcardlisting(&inst->pbap_pce, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
				    ...
				    void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_pbap_pce_pull_vcardentry(&inst->pbap_pce, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static uint8_t pce_set_path(const void *cmd, uint16_t cmd_len,
			    void *rsp, uint16_t *rsp_len)
{
	const struct btp_pbap_pce_set_path_cmd *cp = cmd;
	struct pbap_instance *inst;
	struct net_buf *buf;
	int err;
	...
	err = bt_pbap_pce_setpath(&inst->pbap_pce, cp->flags, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static int pse_register_rfcomm()
{
	int err;
...
static uint8_t pse_register_l2cap()
{
	int err;
...
static uint8_t pse_connect_rsp(const void *cmd, uint16_t cmd_len,
			       void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_pbap_pse_connect_rsp(&inst->pbap_pse, sys_le16_to_cpu(cp->mopl),
				      cp->rsp_code, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
...
static uint8_t pse_disconnect_rsp(const void *cmd, uint16_t cmd_len,
				  void *rsp, uint16_t *rsp_len)
{
...
static uint8_t pse_pull_phonebook_rsp(const void *cmd, uint16_t cmd_len,
				      void *rsp, uint16_t *rsp_len)
{
	const struct btp_pbap_pse_pull_phonebook_rsp_cmd *cp = cmd;
	struct pbap_instance *inst;
	struct net_buf *buf;
	int err;
	...
	err = bt_pbap_pse_pull_phonebook_rsp(&inst->pbap_pse, cp->rsp_code, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static uint8_t pse_pull_vcard_listing_rsp(const void *cmd, uint16_t cmd_len,
					  void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_pbap_pse_pull_vcardlisting_rsp(&inst->pbap_pse, cp->rsp_code, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static uint8_t pse_pull_vcard_entry_rsp(const void *cmd, uint16_t cmd_len,
					void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_pbap_pse_pull_vcardentry_rsp(&inst->pbap_pse, cp->rsp_code, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static uint8_t pse_set_path_rsp(const void *cmd, uint16_t cmd_len,
				void *rsp, uint16_t *rsp_len)
{
	const struct btp_pbap_pse_set_path_rsp_cmd *cp = cmd;
	struct pbap_instance *inst;
	struct net_buf *buf;
	int err;
	...
	err = bt_pbap_pse_setpath_rsp(&inst->pbap_pse, cp->rsp_code, buf);
	if (err) {
		net_buf_unref(buf);
		return BTP_STATUS_FAILED;
	}
...
static uint8_t pse_abort_rsp(const void *cmd, uint16_t cmd_len,
			     void *rsp, uint16_t *rsp_len)
{
...
static int pbap_sdp_get_repositories(const struct net_buf *buf, uint8_t *repositories)
{
	...
	if ((value.type != BT_SDP_ATTR_VALUE_TYPE_UINT) || (value.uint.size != sizeof(*repositories))) {
		return -EINVAL;
	}
			       ...
			       const struct bt_sdp_discover_params *params)
{
	...
	if (result && result->resp_buf) {
		...
		bt_sdp_get_proto_param(result->resp_buf, BT_SDP_PROTO_RFCOMM, &peer_sdp_rfcomm_channel);
		...
		tester_event(BTP_SERVICE_ID_PBAP, BTP_PBAP_EV_SDP_DISCOVER, 
				&rp, sizeof(rp));
...
static const struct btp_handler handlers[] = {
	...
	{
		.opcode = BTP_PBAP_PCE_PULL_PHONEBOOK,
		.expect_len = BTP_HANDLER_LENGTH_VARIABLE,
	...
	{
		.opcode = BTP_PBAP_PSE_PULL_PHONEBOOK_RSP,
		.expect_len = BTP_HANDLER_LENGTH_VARIABLE,
	...
	{
		.opcode = BTP_PBAP_PSE_SET_PATH_RSP,
		.expect_len = BTP_HANDLER_LENGTH_VARIABLE,
		.func = pse_set_path_rsp,
	},
	{
