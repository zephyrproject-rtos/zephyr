...
#define BTP_A2DP_ENABLE_DELAY_REPORT       0x0C

#define BTP_A2DP_SEND_DELAY_REPORT         0x0D
struct btp_a2dp_send_delay_report {
	uint16_t delay;
} __packed;

#define BTP_A2DP_GET_CONFIGURE             0x0E
...
#define BTP_A2DP_EV_DISCOVER_RSP	   0x81
struct btp_a2dp_discover_rsp {
	int8_t errcode;
} __packed;
...
#define BTP_A2DP_EV_SEND_DELAY_REPORT_RSP  0x8B
struct btp_a2dp_send_delay_report_rsp {
	uint8_t errcode;
} __packed;

#define BTP_A2DP_EV_GET_CONFIG_RSP  0x8C
struct btp_a2dp_get_config_rsp {
	uint8_t errcode;
} __packed;
