...
#ifndef TESTS_BLUETOOTH_TESTER_SRC_BTP_BTP_BIP_H_
#define TESTS_BLUETOOTH_TESTER_SRC_BTP_BTP_BIP_H_
 ...
 * commands
 *
 * Opcodes 0x02, 0x05 and 0x08 are unused: they were reserved during
 * development for RFCOMM/L2CAP listen commands that the tester ends up not
 * needing (the transport servers are registered once by tester_init_bip()).
 * They are left as holes rather than reused so that the opcode of every
 * command already documented in auto-pts doc/btp_bip.txt stays stable.
 */
#define BTP_BIP_READ_SUPPORTED_COMMANDS 0x01
struct btp_bip_read_supported_commands_rp {
	/*
	 * Zero-length array, not a C99 flexible array member: this struct has
	 * no other named member, which a flexible array member would make
	 * ill-formed (C11 6.7.2.1p18). Same shape as btp_gatt.h, btp_l2cap.h,
	 * btp_rfcomm.h and btp_sdp.h.
	 */
	uint8_t data[0];
} __packed;
...
#endif /* TESTS_BLUETOOTH_TESTER_SRC_BTP_BTP_BIP_H_ */
