...
static void send_discovered_event(struct a2dp_connection *conn)
{
	struct btp_a2dp_discovered_ev *ev;
	uint8_t *buf;
	size_t event_size;

	copy_seps_to_discovered_ep();

	event_size = sizeof(*ev) + sizeof(struct bt_a2dp_sep_info) * found_seps_count;

	tester_rsp_buffer_allocate(event_size, &buf);
	ev = (struct btp_a2dp_discovered_ev *)buf;

	bt_addr_copy(&ev->address.a, &conn->address);
	ev->address.type = BTP_BR_ADDRESS_TYPE;
	ev->result = 0;
	ev->len = sizeof(struct bt_a2dp_sep_info) * found_seps_count;
	memcpy(&ev->sep, discovered_ep, ev->len);

	tester_event(BTP_SERVICE_ID_A2DP, BTP_A2DP_EV_DISCOVERED, ev, event_size);

	tester_rsp_buffer_free();
}

static void send_capabilities_event(struct a2dp_connection *conn, struct bt_a2dp_ep_info *info)
{
	...
	losc = sizeof(struct bt_a2dp_media_codec_capabilities) + info->codec_cap.len;

	event_size = sizeof(*ev) + 2 + losc;
	...
	*ptr++ = BT_AVDTP_SERVICE_MEDIA_CODEC;
	*ptr++ = losc;
	*ptr++ = info->sep_info->media_type;
	*ptr++ = info->codec_type;
	if (info->codec_cap.len > 0) {
		memcpy(ptr, info->codec_cap.codec_ie, info->codec_cap.len);
	}

	rsp_code = discover_param.avdtp_version >= AVDTP_VERSION_1_3
			   ? BTP_A2DP_EV_GET_ALL_CAPABILITIES
			   : BTP_A2DP_EV_GET_CAPABILITIES;
	tester_event(BTP_SERVICE_ID_A2DP, rsp_code, ev, event_size);
			      ...
			      struct bt_a2dp_ep **ep)
{
	...
	if (info == NULL) {
		send_discovered_event(conn);
		tester_rsp_buffer_unlock();
		return BT_A2DP_DISCOVER_EP_STOP;
	}

	if (found_seps_count >= BTP_A2DP_MAX_ENDPOINTS) {
		tester_rsp_buffer_unlock();
		return BT_A2DP_DISCOVER_EP_STOP;
	}
	found_seps_count++;

	send_capabilities_event(conn, info);
...
static uint8_t a2dp_stream_config(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	stream_id = 0xFF;
	for (size_t index = 0; index < ARRAY_SIZE(conn->streams); index++) {
		if (!conn->streams[index].in_use) {
			stream = &conn->streams[index].stream;
			stream_id = index;
			conn->streams[index].in_use = true;
			conn->streams[index].stream_id = stream_id;
			break;
		}
	}
...
{
#if defined(CONFIG_BT_A2DP_SINK) || defined(CONFIG_BT_A2DP_SOURCE)
	int err;
#endif
