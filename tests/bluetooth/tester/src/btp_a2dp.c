...
void app_delay_report_rsp(struct bt_a2dp_stream *stream, uint8_t rsp_err_code)
{
	...
	if (rsp_err_code == 0) {
		...
		memset(&ev, 0, sizeof(ev));
		ev.errcode = rsp_err_code;
		tester_event(BTP_SERVICE_ID_A2DP, BTP_A2DP_EV_SEND_DELAY_REPORT_RSP, &ev,
			     sizeof(ev));
	}
}

static uint8_t a2dp_enable_delay_report(const void *cmd, uint16_t cmd_len, void *rsp,
					uint16_t *rsp_len)
{
...
static uint8_t a2dp_send_delay_report(const void *cmd, uint16_t cmd_len, void *rsp,
				      uint16_t *rsp_len)
{
	int err;
...
void app_get_config_rsp(struct bt_a2dp_stream *stream, struct bt_a2dp_codec_cfg *codec_cfg,
			uint8_t rsp_err_code)
{
	struct btp_a2dp_get_config_rsp ev;

	memset(&ev, 0, sizeof(ev));
	ev.errcode = rsp_err_code;
	tester_event(BTP_SERVICE_ID_A2DP, BTP_A2DP_EV_GET_CONFIG_RSP, &ev, sizeof(ev));
}
...
static uint8_t a2dp_get_configure(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
...
#endif
	{
		.opcode = BTP_A2DP_GET_CONFIGURE,
		.expect_len = 0,
		.func = a2dp_get_configure,
	},
