...
static uint8_t br_connect(const struct btp_l2cap_connect_cmd_common *cp, uint16_t options,
			  struct btp_l2cap_connect_rp *rp, uint16_t *rsp_len)
{
	struct bt_conn *conn;
	...
	if (IS_ENABLED(CONFIG_BT_L2CAP_RET_FC)) {
		if (options & BTP_L2CAP_CONNECT_V2_OPT_RET) {
			br_chan->br.rx.mode = BT_L2CAP_BR_LINK_MODE_RET;
			br_chan->br.rx.max_transmit = 3;
		} else if (options & BTP_L2CAP_CONNECT_V2_OPT_FC) {
			br_chan->br.rx.mode = BT_L2CAP_BR_LINK_MODE_FC;
			br_chan->br.rx.max_transmit = 3;
		} else if (options & BTP_L2CAP_CONNECT_V2_OPT_ERET) {
			br_chan->br.rx.mode = BT_L2CAP_BR_LINK_MODE_ERET;
			br_chan->br.rx.max_transmit = 3;
		} else if (options & BTP_L2CAP_CONNECT_V2_OPT_STREAM) {
		...
		if (options & BTP_L2CAP_CONNECT_V2_OPT_EXT_WIN_SIZE) {
			br_chan->br.rx.extended_control = true;
		} else {
			br_chan->br.rx.extended_control = false;
		}

		if (options & BTP_L2CAP_CONNECT_V2_OPT_MODE_OPTIONAL) {
		...
		if (options & BTP_L2CAP_CONNECT_V2_OPT_NO_FCS) {
			br_chan->br.rx.fcs = BT_L2CAP_BR_FCS_NO;
		} else {
			br_chan->br.rx.fcs = BT_L2CAP_BR_FCS_16BIT;
		}

		br_chan->hold_credit = (options & BTP_L2CAP_CONNECT_V2_OPT_HOLD_CREDIT) != 0;
	}
...
static uint8_t _connect(const struct btp_l2cap_connect_cmd_common *cp, uint16_t options, void *rsp,
			uint16_t *rsp_len)
{
	...
	uint8_t i = 0;
	bool ecfc = options & BTP_L2CAP_CONNECT_OPT_ECFC;
	int err;
	...
	if (cp->address.type == BTP_BR_ADDRESS_TYPE) {
		return br_connect(cp, options, rp, rsp_len);
	}
	...
	for (i = 0U; i < cp->num; i++) {
		...
		chan->hold_credit = options & BTP_L2CAP_CONNECT_OPT_HOLD_CREDIT;
...
static uint8_t connect(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	return _connect(&cp->common, (uint16_t)cp->options, rsp, rsp_len);
}
...
static uint8_t connect_v2(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	return _connect(&cp->common, cp->options, rsp, rsp_len);
}
...
static uint8_t cls_send(const void *cmd, uint16_t cmd_len, void *rsp, uint16_t *rsp_len)
{
	...
	err = bt_l2cap_br_cls_send(conn, psm, buf);
	bt_conn_unref(conn);
	if (err < 0) {
