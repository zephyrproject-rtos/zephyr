...
static struct mce_mas_instance *mce_mas_find(const bt_addr_le_t *address, uint8_t instance_id)
{
	...
	ARRAY_FOR_EACH(mce_mas_instances, i) {
		ARRAY_FOR_EACH(mce_mas_instances[i], j) {
			...
			if (bt_addr_eq(bt_conn_get_dst_br(inst->conn), &address->a) &&
			    inst->instance_id == instance_id) {
				return inst;
			}
		}
				 ...
				 struct bt_map_mse_mas **mse_mas)
{
	...
	inst = mse_mas_alloc(conn, mse_server[index].instance_id);
	if (inst == NULL) {
				      ...
				      uint16_t *rsp_len)
{
	...
	inst = mce_mas_alloc(conn, cp->instance_id);
	if (inst == NULL) {
		bt_conn_unref(conn);
		return BTP_STATUS_FAILED;
	}

	if (bt_map_mce_mas_rfcomm_connect(conn, &inst->mce_mas, &mce_mas_cb, cp->channel) < 0) {
		mce_mas_free(inst);
		bt_conn_unref(conn);
		return BTP_STATUS_FAILED;
	}
				     ...
				     uint16_t *rsp_len)
{
	...
	if (bt_map_mce_mas_l2cap_connect(conn, &inst->mce_mas, &mce_mas_cb,
					 sys_le16_to_cpu(cp->psm)) < 0) {
		mce_mas_free(inst);
		bt_conn_unref(conn);
		return BTP_STATUS_FAILED;
	}
				      ...
				      uint16_t *rsp_len)
{
	...
	if (bt_map_mse_mns_rfcomm_connect(conn, &inst->mse_mns, &mse_mns_cb, cp->channel) < 0) {
		mse_mns_free(inst);
		bt_conn_unref(conn);
		return BTP_STATUS_FAILED;
	}
				     ...
				     uint16_t *rsp_len)
{
	...
	if (bt_map_mse_mns_l2cap_connect(conn, &inst->mse_mns, &mse_mns_cb,
					 sys_le16_to_cpu(cp->psm)) < 0) {
		mse_mns_free(inst);
		bt_conn_unref(conn);
		return BTP_STATUS_FAILED;
...
static int mce_mns_register(void)
{
	int err;
	...
	ARRAY_FOR_EACH(mce_mns_instances, i) {
		...
		if (err != 0) {
			LOG_ERR("Failed to register MCE MNS server [%zu] (err %d)", i, err);
			return err;
		}
	}
...
static int mse_mas_register(void)
{
	int err;
	...
	ARRAY_FOR_EACH(mse_mas_instances, i) {
		ARRAY_FOR_EACH(mse_mas_instances[i], j) {
			...
			if (err != 0) {
				LOG_ERR("Failed to register MSE MAS server [%zu][%zu] (err %d)", i,
					j, err);
				return err;
			}
		}
...
uint8_t tester_init_map(void)
{
	...
	/* Register MCE MNS servers */
	err = mce_mns_register();
	if (err != 0) {
		return BTP_STATUS_FAILED;
	}
	...
	/* Register MSE MAS servers */
	err = mse_mas_register();
	if (err != 0) {
		return BTP_STATUS_FAILED;
	}
