...
static void rfcomm_recv(struct bt_rfcomm_dlc *dlci, struct net_buf *buf)
{
	...
	ev.channel = BT_RFCOMM_GET_CHANNEL(dlci->dlci);
	ev.data_length = buf->len;
	memcpy(ev.data, buf->data, ev.data_length);

	tester_event(BTP_SERVICE_ID_RFCOMM, BTP_RFCOMM_EV_DATA_RECEIVED,
		     &ev, sizeof(ev) + ev.data_length);
}
...
static uint8_t rfcomm_conn(const void *cmd, uint16_t cmd_len,
			   void *rsp, uint16_t *rsp_len)
{
