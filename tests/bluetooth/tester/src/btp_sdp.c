...
{
	struct bt_conn *conn;
	struct btp_sdp_search_req_cmd *req;
	static struct bt_uuid_16 uuid_16 = BT_UUID_INIT_16(0);

	req = (struct btp_sdp_search_req_cmd *)cmd;
...
{
	struct bt_conn *conn;
	struct btp_sdp_attr_req_cmd *req;

	req = (struct btp_sdp_attr_req_cmd *)cmd;
	conn = bt_conn_lookup_addr_br(&req->address);
	if (!conn) {
		return BTP_STATUS_FAILED;
	}
...
{
	struct bt_conn *conn;
	struct btp_sdp_search_attr_req_cmd *req;
	static struct bt_uuid_16 uuid_16 = BT_UUID_INIT_16(0);

	req = (struct btp_sdp_search_attr_req_cmd *)cmd;
	conn = bt_conn_lookup_addr_br(&req->address);
	if (!conn) {
		return BTP_STATUS_FAILED;
	}
