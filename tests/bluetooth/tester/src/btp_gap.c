					     ...
					     uint16_t *rsp_len)
{
	...
	int err;
	size_t eir_count = 0;
	static struct bt_data eir[BTP_EIR_DATA_MAX];
	...
	while (data.len >= (sizeof(uint8_t) + sizeof(uint8_t))) {
		...
		if (data.len < eir[eir_count].data_len) {
			LOG_ERR("eir data length exceeds remaining buffer");
			return BTP_STATUS_FAILED;
		}
	...
	err = bt_br_write_eir(eir, eir_count, cp->fec_required);
	if (err < 0) {
