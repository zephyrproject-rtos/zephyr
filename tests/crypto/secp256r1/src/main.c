...
/*
 * Simple test to show support for secp256r1 curve with MbedTLS's PSA Crypto
 * APIs. 2 different options are proposed:
 * - standard PSA Crypto API, which also includes the key management together
 *   with the crypto support;
 * - direct access to the P256-M software optimized version, which does not
 *   provide key management capabilities, but can be used in memory constrained
 *   devices thanks to its reduced footprint.
 * Operations are pretty simple:
 * - generate 2 keys
 * - perform key agreement.
 * The idea is to provide a way to compare memory footprint for the very
 * same kind of implemented feature.
 */
