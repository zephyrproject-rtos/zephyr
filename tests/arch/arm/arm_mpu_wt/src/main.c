...
static const uint32_t test_patterns[] = {
	0x12345678, 0xDEADBEEF, 0xCAFEBABE, 0x87654321,
				...
				void *dst, size_t size)
{
	...
	dma_cfg.user_data = (void *)dma_dev; /* Pass device for debugging */
...
/**
 * @brief Test basic memory region read/write functionality
 */
ZTEST(arm_mpu_wt, test_sram_basic_access)
{
	volatile uint32_t *test_addr;

	test_addr = alloc_test_memory(TEST_MEMORY_SIZE);
	zassert_not_null(test_addr, "Failed to allocate test memory");

	LOG_INF("Testing basic memory access at %p", (void *)test_addr);
	...
	for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
		...
		zassert_equal(read_value, test_patterns[i],
			      "Memory access failed at index %d: wrote 0x%08x, read 0x%08x",
			      i, test_patterns[i], read_value);
	}

	free_test_memory((void *)test_addr);
	LOG_INF("Basic memory access test completed successfully");
}
...
/**
 * @brief Test Write-Through performance with different data patterns
 */
ZTEST(arm_mpu_wt, test_wt_performance)
{
	volatile uint32_t *test_addr;
	uint32_t start_cycles, end_cycles, total_cycles;
	const int iterations = 100;

	/* Allocate space for all patterns */
	test_addr = alloc_test_memory(TEST_MEMORY_SIZE);
	if (test_addr == NULL) {
		ztest_test_skip();
		return;
	...
	LOG_INF("Testing Write-Through performance characteristics");
	LOG_INF("Memory region: %s", get_memory_region_info());
	LOG_INF("Test address: %p", (void *)test_addr);

	/* Sequential write performance test with different patterns */
	start_cycles = k_cycle_get_32();

	for (int iter = 0; iter < iterations; iter++) {
		for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
			/*
			 * With WT cache, each write goes to both cache and memory
			 * Test with actual patterns to simulate real workload
			 */
			test_addr[i] = test_patterns[i];
		}
	}

	/* Single barrier at the end to ensure all writes complete */
	barrier_dsync_fence_full();

	end_cycles = k_cycle_get_32();
	total_cycles = end_cycles - start_cycles;

	LOG_INF("Write performance: %u cycles for %d iterations × %d patterns",
		total_cycles, iterations, TEST_PATTERN_COUNT);
	if (total_cycles > 0) {
		LOG_INF("Average: %u cycles per write",
			total_cycles / (iterations * TEST_PATTERN_COUNT));
	}

	/* Verify final values */
	for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
		zassert_equal(test_addr[i], test_patterns[i],
			      "Final pattern[%d] incorrect: expected 0x%08x, got 0x%08x",
			      i, test_patterns[i], test_addr[i]);
	}
	...
	for (int iter = 0; iter < iterations; iter++) {
		for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
			/* Reads can come from cache - very fast with WT */
			dummy = test_addr[i];
	...
	LOG_INF("Read performance: %u cycles for %d iterations × %d patterns",
		total_cycles, iterations, TEST_PATTERN_COUNT);
	if (total_cycles > 0) {
		LOG_INF("Average: %u cycles per read",
			total_cycles / (iterations * TEST_PATTERN_COUNT));
	}
...
/**
 * @brief Test Write-Through with stack memory using all patterns
 */
ZTEST(arm_mpu_wt, test_wt_stack_access)
{
	uint32_t test_data[TEST_PATTERN_COUNT];
	volatile uint32_t *test_addr = test_data;

	LOG_INF("Testing Write-Through with stack memory at %p", (void *)test_addr);
	LOG_INF("Testing with %d patterns", TEST_PATTERN_COUNT);

	/* Test each pattern */
	for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
		/* WT cache handles write-through automatically */
		test_addr[i] = test_patterns[i];
		LOG_DBG("Wrote pattern[%d] = 0x%08x to stack", i, test_patterns[i]);
	}
	...
	for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
		...
		if (read_value != test_patterns[i]) {
			LOG_ERR("Stack pattern[%d] mismatch: expected 0x%08x, got 0x%08x",
				i, test_patterns[i], read_value);
			all_passed = false;
		} else {
			LOG_DBG("Stack pattern[%d] OK: 0x%08x", i, read_value);
		}

		zassert_equal(read_value, test_patterns[i],
			      "Stack access failed at pattern[%d]: "
			      "wrote 0x%08x, read 0x%08x",
			      i, test_patterns[i], read_value);
	}

	if (all_passed) {
		LOG_INF("Stack memory test PASSED - All %d patterns verified",
			TEST_PATTERN_COUNT);
	}
}
...
/**
 * @brief Test Write-Through functionality in user mode with all patterns
 */
ZTEST_USER(arm_mpu_wt, test_wt_userspace)
{
	volatile uint32_t *test_addr;
	...
	LOG_INF("Testing Write-Through in user mode at %p", test_addr);
	LOG_INF("Memory region: %s", get_memory_region_info());
	LOG_INF("Testing with %d patterns", TEST_PATTERN_COUNT);

	/* Test all patterns in user mode */
	bool all_passed = true;
	...
	for (int i = 0; i < TEST_PATTERN_COUNT; i++) {
		...
		/* Read back - WT cache ensures consistency */
		uint32_t read_value = test_addr[i];

		if (read_value != test_patterns[i]) {
			LOG_ERR("User mode pattern[%d] mismatch: expected 0x%08x, got 0x%08x",
				i, test_patterns[i], read_value);
			all_passed = false;
		} else {
			LOG_DBG("User mode pattern[%d] OK: 0x%08x", i, read_value);
		}

		zassert_equal(read_value, test_patterns[i],
			      "User mode pattern test failed at [%d]: "
			      "expected 0x%08x, got 0x%08x",
			      i, test_patterns[i], read_value);
	}

	if (all_passed) {
		LOG_INF("User mode test PASSED - All %d patterns verified",
			TEST_PATTERN_COUNT);
	}
