...
ZTEST(arm64_mmu_mem_attr, test_robustness_boundary_access)
{
	volatile uint32_t *end_ptr =
		(volatile uint32_t *)((uintptr_t)test_normal_mem + TEST_REGION_SIZE - sizeof(uint32_t));
...
/**
 * @brief Negative test: Write to potentially read-only region
 *
 * Attempts to write to a device memory region which may be read-only.
 * This tests permission fault handling.
 */
ZTEST(arm64_mmu_mem_attr, test_negative_device_write_fault)
{
	/* Try to write to device memory (UART region) */
	volatile uint32_t *device_ptr = (volatile uint32_t *)TEST_DEVICE_ADDR;

	/* Some device registers might be read-only or cause issues on write */
	ztest_set_fault_valid(true);
	
	/* Write to potentially protected device memory */
	*device_ptr = 0xDEADBEEF;  /* May fault depending on device */
	
	/* If we get here, the write was allowed (not all devices fault on write) */
	/* This is OK - not all device memory will fault */
}
