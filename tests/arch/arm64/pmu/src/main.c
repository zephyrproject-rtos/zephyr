...
/* -------------------------------------------------- overflow (sampling API) */

#if defined(CONFIG_PROFILING_PMU_SAMPLING)
ZTEST(pmu_accuracy, test_counter_overflow_numeric)
{
	unsigned int key;
	bool overflowed, not_overflowed;
	...
	/* Preload so ~2048 retired instructions overflow; the >4096-nop sled must. */
	key = irq_lock();
	pmu_counter_clear_overflow(0);
	(void)pmu_counter_write32(0, (uint32_t)(-2048));
	pmu_start();
	work_nops_large();
	pmu_stop();
	overflowed = pmu_counter_overflow(0);
	irq_unlock(key);

	/* With a huge headroom the small sled must NOT overflow. */
	key = irq_lock();
	pmu_counter_clear_overflow(0);
	(void)pmu_counter_write32(0, (uint32_t)(-1000000));
	pmu_start();
	work_nops_small();
	pmu_stop();
	not_overflowed = pmu_counter_overflow(0);
	irq_unlock(key);

	pmu_counter_disable_all();
	k_sched_unlock();

	TC_PRINT("overflow: preload -2048 over %u nops -> %d ; -1000000 over %u nops -> %d\n",
		 NOPS_LARGE, (int)overflowed, NOPS_SMALL, (int)not_overflowed);

	zassert_true(overflowed,
		     "counter did not overflow after %u insns with preload -2048", NOPS_LARGE);
	zassert_false(not_overflowed,
		      "counter overflowed with ~1M headroom over only %u insns", NOPS_SMALL);
}
