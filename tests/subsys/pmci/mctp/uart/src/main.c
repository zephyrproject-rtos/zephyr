...
static void target_reply(struct k_work *item)
{
	 ...
	 */
	mctp_uart_stop_rx(&mctp_uart_endpoint);
...
ZTEST(mctp_uart_test_suite, test_mctp_uart_ping_pong)
{
	...
	TC_PRINT("Sending message \"ping\" to endpoint %u", ENDPOINT_EID);

	rc = mctp_message_tx(mctp_ctx, ENDPOINT_EID, false,
			0, "ping", sizeof("ping"));
