...
static const struct device *i3c_controller = DEVICE_DT_GET(DT_NODELABEL(i3c1));
...
static void target_reply(struct k_work *item)
{
	...
	TC_PRINT("Target replying \"pong\" to endpoint %d", reply->eid);
...
static void rx_message(uint8_t eid, bool tag_owner, uint8_t msg_tag, void *data, void *msg,
		       size_t len)
{
	TC_PRINT("Received message \"%s\" from endpoint %d to %d, msg_tag %d, len %zu", (char *)msg,
...
ZTEST(mctp_i3c_gpio_test_suite, test_mctp_i3c_gpio_ping_pong)
{
	int rc;
	...
	/* At least on NPCX4, target needs to be alive and well when
	 * controller comes online (so it replies controller commands).
	 * Maybe hotjoin could help? Anyway, we defer controlle initialization
	 * to this point so things should work.
	 */
	device_init(i3c_controller);
	device_init(mctp_endpoint0);
	...
	rc = mctp_message_tx(mctp_ctx, mctp_i3c_ctrl.endpoint_ids[0], false,
			0, "ping", sizeof("ping"));
...
ZTEST_SUITE(mctp_i3c_gpio_test_suite, NULL, NULL, NULL, NULL, NULL);
