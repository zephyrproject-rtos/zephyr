...
static void *setup(void)
{
	...
	uint8_t count;
	uint32_t deepest_exit_latency_us = 0U;
	uint32_t shallowest_residency_us = UINT32_MAX;
	uint32_t shallowest_exit_latency_us = 0U;

	count = pm_state_cpu_get_all(0U, &states);
	zassert_true(count > 0, "no PM states defined for CPU 0");

	/* pm_state_cpu_get_all() returns devicetree order, not sorted by depth. */
	for (uint8_t i = 0; i < count; i++) {
		if (states[i].exit_latency_us > deepest_exit_latency_us) {
			deepest_exit_latency_us = states[i].exit_latency_us;
		}
		if (states[i].min_residency_us < shallowest_residency_us) {
			shallowest_residency_us = states[i].min_residency_us;
			shallowest_exit_latency_us = states[i].exit_latency_us;
		}
	}

	/* Overshoot budget: deepest exit latency + two ticks + small margin. */
	overshoot_tolerance_ms = (deepest_exit_latency_us / 1000U) + k_ticks_to_ms_ceil32(2) + 5U;

	/* The shallowest state (lowest residency) sets the entry threshold. */
	lpm_threshold_ms =
		DIV_ROUND_UP(shallowest_residency_us + shallowest_exit_latency_us, 1000U);
