...
ZTEST(pm_event_device, test_request_release)
{
	...
	zassert_equal(effective_uptime_ticks, uptime_ticks + TEST_EVENT_STATE_REQUEST_LATENCY_TICKS + 1);
	test_event_device_request_mock_await_call();
...
ZTEST(pm_event_device, test_schedule_release)
{
	...
	test_event_device_request_mock_expect_call(uptime_ticks, 1);
	effective_uptime_ticks = pm_event_device_schedule_event(test_event_device, &event0, 1, event_uptime_ticks);
...
ZTEST(pm_event_device, test_schedule_release_overlap)
{
	...
	effective_uptime_ticks = pm_event_device_schedule_event(test_event_device, &event0, 1, event0_uptime_ticks);
	zassert_equal(effective_uptime_ticks, event0_uptime_ticks);
	effective_uptime_ticks = pm_event_device_schedule_event(test_event_device, &event1, 2, event1_uptime_ticks);
