...
{
	const struct isr_action_context *context = arg;

	isr_action_ret = z_pm_device_runtime_power_domain_action_run(
		context->dev, PM_DEVICE_ACTION_TURN_OFF);
}
...
static void run_power_domain_action_test(const struct device *dev)
{
	...
	*data = (struct test_pm_data){0};
	...
	zassert_equal(isr_action_ret, -EWOULDBLOCK);
