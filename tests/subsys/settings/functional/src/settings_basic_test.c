...
ZTEST(settings_functional, test_clear_settings)
{
	...
	for (int i = 0; i < sizeof(struct setting_entry) * CONFIG_SETTINGS_TFM_PSA_NUM_ENTRIES /
		...
		status = SETTINGS_PSA_REMOVE(PSA_ID_RANGE_START + i);
