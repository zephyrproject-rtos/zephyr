...
ZTEST(osal, test_thread_self_delete)
{
	...
	worker_ran = 0;
	t = osal_thread_create(prio_worker, "after", (void *)(uintptr_t)9, STACK_SIZE, PRIO);
	zassert_not_null(t);
	osal_delay_ms(20);
...
ZTEST(osal, test_busy_wait)
{
	...
	if (!IS_ENABLED(CONFIG_ARCH_POSIX)) {
		zassert_true(osal_uptime_ms() - t0 >= 18, "busy-wait did not advance uptime");
	}
}
...
ZTEST(osal, test_event_atomic_clear)
{
	 ...
	 */
	zassert_equal(osal_event_wait(atomic_evt, EVT_A, false, false, OSAL_NO_WAIT) & EVT_A, EVT_A,
		      "re-posted bit was lost by a non-atomic clear");
...
ZTEST(osal, test_static_thread_pool_exhaustion)
{
	...
	zassert_equal(created, CONFIG_OSAL_MAX_THREADS, "expected exactly %d threads, created %d",
...
static void run_smp_contention(bool use_crit, osal_spinlock_t lock)
{
	 ...
	 */
	a = k_thread_create(&smp_threads[0], smp_stacks[0], 2048, smp_hammer, (void *)0, NULL, NULL,
			    5, 0, K_FOREVER);
	b = k_thread_create(&smp_threads[1], smp_stacks[1], 2048, smp_hammer, (void *)1, NULL, NULL,
			    5, 0, K_FOREVER);
	 ...
	 */
	zassert_not_equal(smp_cpu[0], smp_cpu[1], "workers did not run on distinct CPUs (%d, %d)",
			  smp_cpu[0], smp_cpu[1]);
	zassert_equal(smp_counter, 2 * SMP_ITERS, "lost updates under contention: %u != %u",
