...
ZTEST(llext, test_detached)
{
	 ...
	 * This doesn't work with the relocatable ELF type, because there
	 * section addresses aren't "real," e.g. they can be 0.
	 */
	elf_shdr_t *text_region = detached_loader->sects + LLEXT_MEM_TEXT;

	zassert_true(text_region->sh_addr >= detached_shdr.sh_addr + detached_shdr.sh_size ||
		     detached_shdr.sh_addr >= text_region->sh_addr + text_region->sh_size);
#endif
