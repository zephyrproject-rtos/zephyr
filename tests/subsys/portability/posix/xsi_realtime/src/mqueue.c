...
ZTEST(xsi_realtime, test_mqueue_open_invalid_attrs_einval)
{
	...
	zassert_equal(mqd, (mqd_t)-1,
		      "mq_open should fail for negative msgsize");
	zassert_equal(errno, EINVAL,
		      "errno should be EINVAL for negative msgsize");
	...
	zassert_equal(mqd, (mqd_t)-1,
		      "mq_open should fail for negative maxmsg");
	zassert_equal(errno, EINVAL,
		      "errno should be EINVAL for negative maxmsg");
	...
	zassert_equal(errno, EINVAL,
		      "errno should be EINVAL for msgsize exceeding limit");
}
...
ZTEST(xsi_realtime, test_mqueue_open_existing_null_attrs)
{
	...
	/* POSIX: If O_CREAT is specified without O_EXCL and queue exists,
	 * O_CREAT has no effect.
	 */
...
ZTEST(xsi_realtime, test_mqueue_open_default_attrs)
{
	...
	zassert_not_equal(mqd, (mqd_t)-1,
			  "mq_open should succeed with default attributes");
