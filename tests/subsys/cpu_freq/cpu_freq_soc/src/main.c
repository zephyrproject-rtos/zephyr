 ...
 * point IPIs which can be used to verify that the current schedule lock is
 * respected--an assumption made by the test.
 */
void give_take_helper(void *p1, void *p2, void *p3)
{
...
ZTEST(cpu_freq_soc, test_soc_pstates)
{
	...
	for (i = 0; i < ARRAY_SIZE(soc_pstates_dt); i++) {
		...
		/*
		 * Verify that the current thread does not migrate before
		 * calling cpu_freq_pstate_set().
		 */
...
#if defined(CONFIG_SMP) && (CONFIG_MP_MAX_NUM_CPUS > 1)
		/*
		 * Verify that the current thread does not migrate after
		 * calling cpu_freq_pstate_set().
		 */
		for (int j = 0; j < 10; j++) {
			k_busy_wait(10000);
