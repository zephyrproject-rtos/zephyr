...
static void test_zdsp_cfft_q15_dc(uint16_t fft_len)
{
	...
	zassert_equal(zdsp_cfft_init_q15(&inst, fft_len), ZDSP_TRANSFORM_STATUS_OK,
...
static void test_zdsp_cfft_q15_tone(uint16_t fft_len)
{
	...
	for (uint16_t n = 0; n < fft_len; n++) {
		...
		buf[2 * n] = zdsp_f32_to_q15_shift(0.4f * cosf(phase), 0);
		buf[2 * n + 1] = zdsp_f32_to_q15_shift(0.4f * sinf(phase), 0);
	}
...
static void test_zdsp_cfft_q15_vs_f32(uint16_t fft_len)
{
	...
	for (uint16_t n = 0; n < fft_len; n++) {
		...
		fbuf[2 * n] = re;
		fbuf[2 * n + 1] = im;
		qbuf[2 * n] = zdsp_f32_to_q15_shift(re, 0);
		qbuf[2 * n + 1] = zdsp_f32_to_q15_shift(im, 0);
	}

	zassert_equal(zdsp_cfft_init_q15(&inst_q15, fft_len), ZDSP_TRANSFORM_STATUS_OK,
		      ASSERT_MSG_INCORRECT_COMP_RESULT);
