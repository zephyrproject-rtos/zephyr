...
ZTEST(host_class, test_class_matching)
{
	...
	ret = usbh_desc_get_iface_info(desc, &filter_valid_vid_pid, 0);
	zassert_ok(ret, "Expecting the class info to be found");

	ret = usbh_desc_get_iface_info(desc, &filter_invalid_pid, 0);
	zassert_ok(ret, "Expecting the class info to be found");

	ret = usbh_desc_get_iface_info(desc, &filter_invalid_vid, 0);
	zassert_ok(ret, "Expecting the class info to be found");
