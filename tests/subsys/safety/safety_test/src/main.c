...
#include <zephyr/safety/safety_test.h>
...
/*
 * Boot-order probes. One, two and three digit priorities with names out of order, so only
 * a correct linker layout runs them as 5, 20, 100.
 */
SAFETY_TEST_DEFINE(boot_alpha, SAFETY_TEST_CAT_STACK, SAFETY_TEST_LEVEL_POST_KERNEL, 100,
		   SAFETY_TEST_FLAG_BOOT_OK, fn_record, "third");
SAFETY_TEST_DEFINE(boot_bravo, SAFETY_TEST_CAT_STACK, SAFETY_TEST_LEVEL_POST_KERNEL, 5,
...
static int hook_cat_rc;
static struct safety_test_stats hook_cat_stats;

static void failure_hook(const struct safety_test_context *ctx,
			 const struct safety_test_result_record *rec)
{
	...
	hook_cat_rc = safety_test_run_category(SAFETY_TEST_CAT_BIT(SAFETY_TEST_CAT_CPU),
					       &hook_cat_stats);
}
...
enum safety_test_action safety_test_safe_state(const struct safety_test_context *ctx,
					       const struct safety_test_result_record *rec)
{
...
/*
 * A sweep started from inside a hook has every test refused, and must still report
 * a defined answer rather than counters built from whatever was on the stack.
 */
ZTEST(safety_test_api, test_category_sweep_from_hook_is_defined)
{
	memset(&hook_cat_stats, 0, sizeof(hook_cat_stats));
	hook_cat_rc = -1;

	zassert_ok(safety_test_run_by_name("rt_ram_fail", 0, NULL));

	zassert_ok(hook_cat_rc, "the sweep itself is a valid call");
	zassert_true(hook_cat_stats.total > 0, "the CPU category is not empty");
	zassert_equal(hook_cat_stats.not_run, hook_cat_stats.total,
		      "a refused test counts as not run");
	zassert_equal(hook_cat_stats.passed, 0);
	zassert_equal(hook_cat_stats.failed, 0);
	zassert_equal(hook_cat_stats.skipped, 0);
	zassert_equal(hook_cat_stats.over_budget, 0,
		      "nothing ran, so nothing can have overrun");
}
...
ZTEST(safety_test_api, test_boot_tests_ran_in_priority_order)
{
	...
	zassert_equal(captured_order[0], 5);
	zassert_equal(captured_order[1], 20);
	zassert_equal(captured_order[2], 100,
...
ZTEST(safety_test_api, test_run_level_is_idempotent_for_boot_tests)
{
	...
	zassert_equal(boot_order[0], 5);
	zassert_equal(boot_order[1], 20);
	zassert_equal(boot_order[2], 100,
		      "a re-run must apply priority too, not section order");
}
...
static bool check_sorted(const struct safety_test *test, void *user_data)
{
	struct sort_state *last = user_data;
	unsigned int level = test->init_level;
	unsigned int priority = test->priority;

	zassert_true(level > last->level || (level == last->level && priority >= last->priority),
		     "%s at level %u priority %u is linked after level %u priority %u",
		     test->name, level, priority, last->level, last->priority);
	last->level = level;
...
ZTEST(safety_test_api, test_section_is_sorted_by_priority)
{
	struct sort_state last = {0};

	safety_test_foreach(check_sorted, &last);
}
