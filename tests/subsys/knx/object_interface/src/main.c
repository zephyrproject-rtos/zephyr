...
knx_u16_be_t object_interfaces_types[TEST_NUMBER_OF_INTERFACE + 1] = {
	KNX_U16_BE(TEST_NUMBER_OF_INTERFACE),
	KNX_U16_BE(OT_DEVICE),
	KNX_U16_BE(OT_ADDR_TABLE)
};

void object_device_properties_written(PropertyID id)         { ARG_UNUSED(id); }
void address_table_properties_written(PropertyID id)         { ARG_UNUSED(id); }
void object_association_table_properties_written(PropertyID id) { ARG_UNUSED(id); }
void object_application_program_properties_written(PropertyID id) { ARG_UNUSED(id); }
void object_group_table_properties_written(PropertyID id)    { ARG_UNUSED(id); }
...
{
	switch (object_index) {
	case 0: return offsetof(struct Memory, addresses);
	case 1: return offsetof(struct Memory, associations);
	default: return 0;
	}
}
...
{
	switch (object_index) {
	case 0: return sizeof(__memory.addresses);
	case 1: return sizeof(__memory.associations);
	default: return 0;
	}
}

void memory_modified(void) { t_memory_modified = true; }
...
ZTEST(oi_read, test_multi_octet_property_is_big_endian_on_the_wire)
{
	...
	zassert_equal(data[1], OT_ADDR_TABLE,
		      "low octet second — got %02x %02x, which is host order",
		      data[0], data[1]);
}
...
ZTEST(oi_access, test_read_only_property_rejects_write)
{
	...
	access_reset();
	zassert_false(interface_write_property(0, PID_MANUFACTURER_ID, 1, 1, (uint8_t *)&val, sizeof(val)),
...
ZTEST(oi_access, test_read_only_with_restrictive_level_rejects_write)
{
	...
	access_reset();
	zassert_false(interface_write_property(0, PID_OBJECT_TYPE, 1, 1, (uint8_t *)&val, sizeof(val)));
...
ZTEST(oi_access, test_writable_property_accepts_write_at_default_level)
{
	...
	zassert_equal(interface_access_level(), 3,
		      "the default level is the least privileged");
...
ZTEST(oi_access, test_load_state_control_respects_access)
{
	...
	access_reset();
	zassert_equal(test_properties_0[4].description.property_id,
		      PID_LOAD_STATE_CONTROL, "test table layout changed");

	test_properties_0[4].description.access = ReadLv3 | WriteLv0;
	zassert_false(interface_write_property(0, PID_LOAD_STATE_CONTROL, 1, 1, unload, sizeof(unload)),
		      "a load control write must honour the access level too");

	test_properties_0[4].description.access = saved;
	zassert_true(interface_write_property(0, PID_LOAD_STATE_CONTROL, 1, 1, unload, sizeof(unload)),
...
ZTEST(oi_desc, test_description_object_type)
{
	...
	interface_read_property_description(0, PID_OBJECT_TYPE, 0,
		&write_enable, &pdt, &max_elem, &access, &idx_found);
...
ZTEST(oi_desc, test_description_unknown_returns_defaults)
{
	...
	interface_read_property_description(0, 0xAB, 0,
		&write_enable, &pdt, &max_elem, &access, &idx_found);
...
ZTEST(oi_load_state, test_empty_pdu_is_refused)
{
	...
	t_load_state = LS_LOADED;
	zassert_false(load_state_machine_event(0, cmd, 0),
		      "there is no event octet to act on");
	zassert_equal(t_load_state, LS_LOADED);
}
...
ZTEST(oi_load_state, test_write_property_forwards_a_valid_event)
{
	...
	t_load_state = LS_UNLOADED;
	interface_reset_access_level();
	zassert_true(interface_write_property(0, PID_LOAD_STATE_CONTROL, 1, 1,
					      cmd, sizeof(cmd)));
	zassert_equal(t_load_state, LS_LOADING);
}
...
ZTEST(oi_load_state, test_write_property_reports_a_refused_event)
{
	...
	t_load_state = LS_LOADING;
	interface_reset_access_level();
	zassert_false(interface_write_property(0, PID_LOAD_STATE_CONTROL, 1, 1,
					       cmd, sizeof(cmd)));
	zassert_equal(t_load_state, LS_LOADING);
}
