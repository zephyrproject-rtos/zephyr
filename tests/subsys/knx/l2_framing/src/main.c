...
uint16_t device_individual_address(void) { return s_own_ia; }
void     device_set_ia_duplication(void) { s_ia_duplication_calls++; }
...
void Ph_Data__txdebug(struct knx_pkt *pkt) { ARG_UNUSED(pkt); }

int knx_l1_send_frame(const uint8_t *frame, uint16_t len)
{
	ARG_UNUSED(frame); ARG_UNUSED(len);
	return 0;
}
...
void N_Data_Individual__ind(struct knx_pkt *pkt) { capture_rx(pkt, ROUTE_INDIVIDUAL); }
void N_Data_Group__ind(struct knx_pkt *pkt) { capture_rx(pkt, ROUTE_GROUP); }
void N_Data_Broadcast__ind(struct knx_pkt *pkt) { capture_rx(pkt, ROUTE_BROADCAST); }
void N_Data_SystemBroadcast__ind(struct knx_pkt *pkt) { capture_rx(pkt, ROUTE_SYSTEM_BROADCAST); }
...
void N_Data_Individual__con(struct knx_pkt *pkt) { capture_con(pkt); }
void N_Data_Group__con(struct knx_pkt *pkt) { capture_con(pkt); }
void N_Data_Broadcast__con(struct knx_pkt *pkt) { capture_con(pkt); }
void N_Data_SystemBroadcast__con(struct knx_pkt *pkt) { capture_con(pkt); }
 ...
 * Total = 8 + LG = 9 octets.
 */
static const uint8_t k_std_frame[9] = {
	0xBC, 0x11, 0x01, 0x08, 0x01, 0xE1, 0x00, 0x81, 0x3A
};
 ...
 */
static const uint8_t k_ext_frame[10] = {
	0x3C, 0xE0, 0x11, 0x01, 0x08, 0x01, 0x01, 0x00, 0x81, 0xBA
};
...
{
	zassert_equal(fcs(k_std_frame, 8), 0x3A,
		      "standard-frame FCS must be 0x3A, got 0x%02x",
		      fcs(k_std_frame, 8));
	zassert_equal(fcs(k_ext_frame, 9), 0xBA,
		      "extended-frame FCS must be 0xBA, got 0x%02x",
		      fcs(k_ext_frame, 9));
}
...
ZTEST(l2_rx_parse, test_extended_hop_count_comes_from_ctrle_not_lg)
{
	...
	f[7] = 0x00;
	f[8] = 0x81; f[9] = 0xAA; f[10] = 0xBB; f[11] = 0xCC;
	f[12] = fcs(f, 12);

	zassert_true(rx_feed(f, sizeof(f)));
	zassert_equal(s_route, ROUTE_GROUP);
	zassert_equal(s_rx.hop_count, 3,
		      "extended hop count must come from CTRLE, got %u",
		      s_rx.hop_count);
...
ZTEST(l2_rx_parse, test_individual_frame_to_own_address_is_accepted)
{
	...
	t_reset();
	memcpy(f, k_std_frame, sizeof(f));
	f[3] = 0x11; f[4] = 0x01;              /* DA = our own IA */
	f[5] = 0x00 | (6u << 4) | 0x01;        /* AT = individual, hop 6, LG 1 */
	f[1] = 0x12; f[2] = 0x02;              /* SA = someone else */
	f[8] = fcs(f, 8);
...
ZTEST(l2_rx_parse, test_broadcast_frame_is_routed_to_broadcast)
{
	...
	t_reset();
	memcpy(f, k_std_frame, sizeof(f));
	f[3] = 0x00; f[4] = 0x00;              /* DA = 0 -> broadcast */
	f[8] = fcs(f, 8);
 ...
 */
static struct knx_pkt *tx_build(uint8_t *lsdu, uint8_t lsdu_len,
				knx_addr_t dst, knx_addr_type_t at,
...
ZTEST(l2_tx_build, test_standard_frame_header_matches_reference)
{
	...
	t_reset();
	pkt = tx_build(lsdu, sizeof(lsdu), 0x0801, KNX_ADDR_TYPE_GROUP,
		       KNX_PRIORITY_LOW, 6);
...
ZTEST(l2_tx_build, test_lsdu_points_at_tpci_after_standard_build)
{
	...
	zassert_equal_ptr(pkt->lsdu, &pkt->buf[6],
			  "pkt->lsdu must point at the TPCI, not at CTRL");
	zassert_equal(pkt->lsdu[0], 0x00, "lsdu[0] must be the TPCI");
	zassert_not_equal(pkt->lsdu[0], pkt->buf[0],
			  "lsdu must not alias the CTRL byte");
	knx_pkt_free(pkt);
}
...
ZTEST(l2_tx_build, test_lsdu_points_at_tpci_after_extended_build)
{
	...
	zassert_equal_ptr(pkt->lsdu, &pkt->buf[7],
			  "extended frames put the TPCI at buf[7]");
...
ZTEST(l2_tx_build, test_ctrl_byte_encodes_priority_and_not_repeated)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(cases); i++) {
		...
		zassert_equal(pkt->buf[0], cases[i].expect,
			      "priority %d: expected CTRL 0x%02x, got 0x%02x",
			      cases[i].prio, cases[i].expect, pkt->buf[0]);
...
ZTEST(l2_tx_build, test_tx_dequeue_next_drains_high_before_low)
{
	...
	zassert_equal(l_tx_dequeue_next(&out), -ENOMSG,
		      "both queues must now report empty");
}
...
ZTEST(l2_tx_build, test_lg_excludes_the_tpci_octet)
{
	...
	zassert_equal(pkt->buf[5] & 0x0Fu, 4,
		      "LG must be lsdu_len - 1 = 4, got %u", pkt->buf[5] & 0x0Fu);
...
ZTEST(l2_tx_build, test_switches_to_extended_above_16_octets)
{
	...
	zassert_equal(pkt->buf[0] & 0x80u, 0u,
		      "extended frames clear CTRL bit 7");
	zassert_equal(pkt->frame_format, KNX_FRAME_FORMAT_EXTENDED);
	zassert_equal(pkt->buf[1], 0x80u | (6u << 4),
		      "CTRLE carries AT and the hop count");
...
ZTEST(l2_tx_build, test_max_frame_stays_in_first_ncn5130_block)
{
	...
	zassert_equal(out->buf[6], KNX_MAX_APDU_OCTETS,
		      "LG must equal the advertised APDU length, got %u",
		      out->buf[6]);
	zassert_true(raw_len <= 63u,
		     "raw_len %u must stay <= 63 so the frame fits NCN5130 block 0",
		     raw_len);
	knx_pkt_free(out);
}
...
ZTEST(l2_repetition, test_repetition_of_preceding_frame_is_dropped)
{
	...
	zassert_equal(s_rx.deliveries, 1,
		      "the repetition must NOT be delivered a second time");
}
...
ZTEST(l2_repetition, test_repetition_flag_is_normalised)
{
	...
	t_reset();
	zassert_true(rx_feed(k_std_frame, sizeof(k_std_frame)));
	zassert_equal(s_rx.repeated, 0,
		      "CTRL bit 5 set (r = 1) means NOT repeated");
	...
	zassert_true(rx_feed(rep, sizeof(rep)));
	zassert_equal(s_rx.deliveries, 1,
		      "a repetition with no preceding frame is still new data");
...
ZTEST(l2_repetition, test_identical_frames_not_marked_repeated_are_both_delivered)
{
	...
	zassert_equal(s_rx.deliveries, 2,
		      "identical frames with r = 1 are distinct telegrams");
}
...
ZTEST(l2_repetition, test_repetition_of_a_different_frame_is_delivered)
{
	...
	zassert_true(rx_feed(rep, sizeof(rep)));
	zassert_equal(s_rx.deliveries, 2,
		      "it repeats a frame that was never the preceding one");
}
...
ZTEST(l2_repetition, test_repetition_after_an_intervening_frame_is_delivered)
{
	...
	zassert_true(rx_feed(rep, sizeof(rep)));
	zassert_equal(s_rx.deliveries, 3,
		      "the reference is the DIRECTLY preceding frame only");
}
