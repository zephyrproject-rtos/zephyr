...
static uint8_t tpci_connect(void)    { return 0x80u; }
static uint8_t tpci_disconnect(void) { return 0x81u; }
...
typedef enum { T_DataIndividual, T_DataConnected, T_Connect,
	       T_Disconnect, T_Ack, T_Nack } TpduType;
...
static uint8_t seq_of_pdu(uint8_t tpci) { return (tpci >> 2) & 0x0Fu; }

/* E04/E05/E06 classification — mirrors T_Data_Individual__ind logic */
typedef enum { E04_CORRECT, E05_REPEAT, E06_WRONG } SeqEvent;
...
ZTEST(l4_tpci_encoding, test_connect_pdu)
{
	...
	zassert_equal(tpci_connect(), 0x80u,
		"T_Connect PDU must be 0x80");
	zassert_equal(pdu_type(0x80u), T_Connect,
		"0x80 must classify as T_Connect");
}

ZTEST(l4_tpci_encoding, test_disconnect_pdu)
{
	/* T_Disconnect_PDU is fixed at 0x81 */
	zassert_equal(tpci_disconnect(), 0x81u,
		"T_Disconnect PDU must be 0x81");
	zassert_equal(pdu_type(0x81u), T_Disconnect,
		"0x81 must classify as T_Disconnect");
}
...
ZTEST(l4_tpci_encoding, test_nak_differs_from_ack)
{
	...
	for (uint8_t seq = 0; seq < 4; seq++) {
		...
		zassert_equal(a ^ n, 0x01u,
			"ACK and NAK must differ only in bit 0 for seq=%u", seq);
	}
}
...
ZTEST(l4_seq_events, test_seqno_full_cycle)
{
	...
	for (int i = 0; i < 16; i++) {
		zassert_equal(classify_seq(rcv, rcv), E04_CORRECT,
			"step %d must be E04", i);
		rcv = (rcv + 1u) & 0x0Fu;
	}
...
ZTEST(l4_ack_nak, test_ack_all_sequences)
{
	for (uint8_t seq = 0; seq < 16; seq++) {
		...
		zassert_equal(pdu_type(b), T_Ack,
			"T_ACK(%u) must classify as T_Ack", seq);
		zassert_equal(seq_of_pdu(b), seq,
			"seq extracted from T_ACK must match %u", seq);
		/* Base pattern: 0xC2 | (seq<<2) — bit 7 and 6 set, bit 0 clear */
		zassert_equal(b & 0xC3u, 0xC2u,
			"T_ACK base pattern 0xC2 for seq=%u", seq);
	}
}
...
ZTEST(l4_ack_nak, test_nak_all_sequences)
{
	for (uint8_t seq = 0; seq < 16; seq++) {
		...
		zassert_equal(pdu_type(b), T_Nack,
			"T_NAK(%u) must classify as T_Nack", seq);
		zassert_equal(seq_of_pdu(b), seq,
			"seq extracted from T_NAK must match %u", seq);
		/* Base pattern: 0xC3 | (seq<<2) — bits 7,6,0 set */
		zassert_equal(b & 0xC3u, 0xC3u,
			"T_NAK base pattern 0xC3 for seq=%u", seq);
	}
}
...
ZTEST(l4_ack_nak, test_ack_nak_bit0_difference_all_sequences)
{
	...
	for (uint8_t seq = 0; seq < 16; seq++) {
		...
		uint8_t n = tpci_nak(seq);
		zassert_equal(a ^ n, 0x01u,
			"ACK^NAK must be 0x01 for seq=%u", seq);
	}
}
