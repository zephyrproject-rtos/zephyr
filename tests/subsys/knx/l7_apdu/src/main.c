...
static int build_prop_response(uint8_t object_index, uint8_t property_id,
...
static int build_propdesc_response(uint8_t obj, uint8_t prop, uint8_t prop_idx,
				   bool write_enable, uint8_t type,
				   uint16_t max_elem, uint8_t access,
				   uint8_t *out)
{
...
static int build_mem_response(uint8_t number, uint16_t addr,
...
ZTEST(l7_groupvalue, test_read_apdu_2bytes)
{
	...
	uint8_t apdu[2] = {
		(A_GroupValue_Read >> 8) & 0xFFu,
		 A_GroupValue_Read & 0xFFu
	};
...
ZTEST(l7_groupvalue, test_write_short_form_1bit_off)
{
	uint8_t apdu[2] = {
		(A_GroupValue_Write >> 8) & 0xFFu,
		(A_GroupValue_Write & 0xFFu) | 0x00u
	};
...
ZTEST(l7_authorize, test_request_apci)
{
	...
	zassert_equal(A_Authorize_Request & 0xFFu, 0xD1u,
		"Authorize_Request APCI[7:0] = 0xD1");
}
...
ZTEST(l7_authorize, test_response_deny_level)
{
	...
	uint8_t apdu[3] = {
		(A_Authorize_Response >> 8) & 0xFFu,
		 A_Authorize_Response & 0xFFu,
		 level
	};
