...
static uint8_t knx_fcs_extended(const uint8_t *buf, uint8_t len)
{
	...
	for (int i = 0; i < len; i++) {
		if (i == 1) continue;  /* skip CTRLE byte */
		crc ^= buf[i];
	}
	return ~crc;
}
 ...
 */
static int build_ncn5130_cmds(const uint8_t *frame_with_fcs, uint8_t total_len,
...
static int encode_groupvalue(uint16_t apci, const uint8_t *data, uint8_t data_bits,
			      uint8_t *out, int out_max)
{
	if (data_bits <= 6) {
...
static int decode_groupvalue_short(const uint8_t *lsdu, uint8_t lsdu_len,
				   uint16_t *apci_out, uint8_t *data_out)
{
	if (lsdu_len != 2) return -1;
...
ZTEST(knx_fcs, test_fcs_known_frame)
{
	...
	uint8_t xor = 0;
	for (int i = 0; i < (int)sizeof(frame); i++) xor ^= frame[i];
