...
void N_Data_Individual__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Individual__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Group__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Group__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Group__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Broadcast__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Broadcast__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_Broadcast__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_SystemBroadcast__req(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_SystemBroadcast__con(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
void N_Data_SystemBroadcast__ind(struct knx_pkt *pkt) { knx_pkt_free(pkt); }
...
uint16_t device_individual_address(void) { return 0x1101; }
uint8_t  device_routing_count(void) { return 6; }
void     device_clear_verify_mode(void) { s_verify_cleared++; }
...
void interface_reset_access_level(void) { s_access_reset++; }

uint16_t address_table_get_group_address(uint16_t tsap) { ARG_UNUSED(tsap); return 0; }
uint16_t address_table_get_tsap(uint16_t addr) { ARG_UNUSED(addr); return 0xFFFF; }
...
#define L7_STUB(name) void name(struct knx_pkt *pkt) { ARG_UNUSED(pkt); s_l7_dispatched++; }
...
bool device_prog_mode(void) { return true; }
...
static void t_event(unsigned ev, knx_addr_t src, uint8_t *data, uint8_t len)
{
...
ZTEST(l4_an210, test_e09_open_wait_is_ignored_not_disconnect)
{
	...
	zassert_equal(__get_state(), OPEN_WAIT,
		      "E09/OPEN_WAIT must stay OPEN_WAIT (AN210), got state %d",
		      __get_state());
	zassert_false(SENT_DISCONNECT(),
		      "E09/OPEN_WAIT must not send T_Disconnect (that is the "
	...
	for (unsigned ev = 0; ev < 28; ev++) {
		for (int st = CLOSED; st <= CONNECTING; st++) {
			...
			zassert_equal(__get_state(), expect,
				      "E%02u from state %d: expected %d, got %d",
				      ev, st, expect, __get_state());
		}
	}
}
...
ZTEST(l4_table, test_e06_connecting_stays_connecting_via_ind)
{
	...
	zassert_equal(__get_state(), CONNECTING,
		      "E06/CONNECTING must remain CONNECTING");
...
ZTEST(l4_table, test_e25_closed_enters_connecting_via_req)
{
	...
	zassert_equal(__get_state(), CONNECTING,
		      "T_Connect.req from CLOSED must enter CONNECTING");
...
ZTEST(l4_actions, test_a1_accepts_connection)
{
	...
	zassert_equal(__get_state(), OPEN_IDLE);
	zassert_equal(_connection_address, OTHER_ADDR,
		      "A1 must adopt the caller's address");
...
ZTEST(l4_actions, test_a2_acks_and_dispatches)
{
	...
	zassert_equal(s_sent[0].dst, CONN_ADDR,
		      "T_ACK goes to the connection address");
}
...
ZTEST(l4_actions, test_a10_rejects_foreign_partner)
{
	...
	zassert_equal(__get_state(), OPEN_IDLE,
		      "A10 must not disturb the existing connection");
	...
	zassert_equal(s_sent[0].dst, OTHER_ADDR,
		      ...
		      "connection address");
	zassert_equal(_connection_address, CONN_ADDR,
		      "A10 must leave connection_address alone");
}
...
ZTEST(l4_actions, test_a5_remote_disconnect)
{
	...
	zassert_true(s_verify_cleared > 0,
		     "closing the connection must clear Verify Mode "
...
ZTEST(l4_actions, test_a7_sends_with_seqnosend)
{
	...
	zassert_equal(s_sent[0].tpci, (uint8_t)(0x40u | (5u << 2)),
		      "A7 must stamp SeqNoSend into the TPCI, got 0x%02x",
		      s_sent[0].tpci);
...
ZTEST(l4_actions, test_a9_retransmits_same_frame)
{
	...
	zassert_equal(s_sent[0].tpci, first_tpci,
		      "A9 must resend with the SAME sequence number");
...
ZTEST(l4_actions, test_a3_acks_repetition_a4_naks)
{
	...
	zassert_equal(__get_state(), OPEN_IDLE);
	zassert_equal(s_l7_dispatched, 0,
		      "A3 must NOT re-deliver a repeated frame to L7");
...
ZTEST(l4_actions, test_e18_gives_up_after_max_retries)
{
	...
	zassert_equal(__get_state(), CLOSED,
		      "E18/OPEN_WAIT must close the connection");
	zassert_true(SENT_DISCONNECT());
}
...
ZTEST(l4_classify, test_foreign_source_is_rejected)
{
	...
	zassert_equal(__get_state(), OPEN_IDLE);
	zassert_equal(s_l7_dispatched, 0,
		      "data from a foreign source must not reach L7");
}
...
ZTEST(l4_actions, test_a2_acks_and_advances_seqnorcv)
{
	...
	zassert_equal(_seqNoRcv, 6,
		      "A2 must advance SeqNoRcv — A3/A4 must not");
}
...
ZTEST(l4_group, test_read_is_answered_once)
{
	...
	zassert_equal(s_read_ind_calls, 1,
		      "and the walk must stop at the ASAP that answered");
