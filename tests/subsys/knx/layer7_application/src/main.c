...
uint16_t device_individual_address(void) { return s_ia; }
void     device_individual_address_set(uint16_t v) { s_ia = v; }
uint8_t  device_routing_count(void) { return 6u; }
uint16_t device_manufacturer_id(void) { return 0x00FAu; } /* this device's real id */
uint8_t  device_serial_number(int i) { return (uint8_t)(0x10 + i); }
uint8_t  device_hardware_type(int i) { return (uint8_t)i; }
uint16_t device_version(void) { return 0x0001u; }
uint16_t device_descriptor(void) { return 0x07B0u; }
void     device_prog_mode_set(bool v) { s_prog_mode = v; }
...
uint8_t interface_read_property(uint8_t object_index, PropertyID property_id,
				 uint32_t start_index, uint32_t *count,
				 uint8_t *data, uint8_t max_size)
{
	ARG_UNUSED(object_index); ARG_UNUSED(property_id); ARG_UNUSED(start_index);
	ARG_UNUSED(data); ARG_UNUSED(max_size);
	*count = 0;
	return 0;
}

bool interface_write_property(uint8_t object_index, PropertyID property_id,
			      uint16_t start_index, uint8_t number_of_elements,
			      const uint8_t *data, uint8_t data_length)
{
	ARG_UNUSED(object_index); ARG_UNUSED(property_id); ARG_UNUSED(start_index);
	ARG_UNUSED(number_of_elements); ARG_UNUSED(data); ARG_UNUSED(data_length);
	return false;
}
...
{
	ARG_UNUSED(object_index); ARG_UNUSED(property_id); ARG_UNUSED(property_index);
	ARG_UNUSED(write_enable); ARG_UNUSED(property_datatype); ARG_UNUSED(max_nr_of_elem);
	ARG_UNUSED(access);
...
{
	ARG_UNUSED(object_index); ARG_UNUSED(property_id); ARG_UNUSED(data_in);
	ARG_UNUSED(data_in_len); ARG_UNUSED(data_out); ARG_UNUSED(max_out);
	return 0;
}
...
ZTEST(l7_user_memory, test_user_memory_bit_write_sets_and_clears_without_disturbing_neighbours)
{
	...
	zassert_equal(__memory.device_control, 0xB4u, "bit 0 must clear back to its original value");
}
...
ZTEST(l7_user_memory, test_user_memory_write_verify_mode_off_sends_no_response)
{
	...
	zassert_equal(__memory.start[0], 0x11u, "the write must still apply even with Verify Mode off");
...
ZTEST(l7_group_read_on_init, test_is_a_noop_when_table_not_loaded)
{
	...
	zassert_equal(s_group_count, 0, "read-on-init must do nothing while the table isn't Loaded");
}

ZTEST(l7_group_read_on_init, test_object_group_table_properties_written_only_triggers_on_reaching_loaded)
{
	reset();
	...
	zassert_equal(s_group_count, 0, "the hook must not fire before the table reaches LS_LOADED");
