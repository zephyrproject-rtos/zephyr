...
#define TEST_PROXY_AGENT_NODE DT_NODELABEL(test_proxy_agent)
...
ZTEST(uart_backend, test_device_not_ready)
{
	...
	ret = api->backend_init(NULL);
	zassert_equal(ret, -EINVAL, "Expected -EINVAL for NULL config, got %d", ret);

	memset(&null_dev_config, 0, sizeof(null_dev_config));
	null_dev_config.dev = NULL;
	ret = api->backend_init((void *)&null_dev_config);
	zassert_equal(ret, -ENODEV, "Expected -ENODEV for NULL device, got %d", ret);
}
...
ZTEST(uart_backend, test_backend_recv)
{
	...
	frame_len += sizeof(uint32_t);
	padded_len =
		((frame_len + config->rx.async_rx_buf_size - 1) / config->rx.async_rx_buf_size) *
		config->rx.async_rx_buf_size;
...
ZTEST(uart_backend, test_recv_sync_recovery)
{
	...
	pos += sizeof(uint32_t);
	padded_len = ((pos + config->rx.async_rx_buf_size - 1) / config->rx.async_rx_buf_size) *
		     config->rx.async_rx_buf_size;
...
static void test_teardown(void *fixture)
{
	...
	memset(config->rx.async_rx_buf, 0,
	       config->rx.async_rx_buf_size * config->rx.async_rx_buf_count);
