...
ZTEST(publish_stats, test_channel_metadata)
{
	...
	zassert_within(k_uptime_ticks(), zbus_chan_publish_time(&chan), clock_window);
	zassert_within(1000, zbus_chan_avg_publish_period(&chan), 50);
	...
	zassert_within(pub_time, zbus_chan_publish_time(&chan), clock_window);
	zassert_within(400, zbus_chan_avg_publish_period(&chan), 50);
	...
	zassert_within(pub_time, zbus_chan_publish_time(&chan), clock_window);
	zassert_within(500, zbus_chan_avg_publish_period(&chan), 50);
}
