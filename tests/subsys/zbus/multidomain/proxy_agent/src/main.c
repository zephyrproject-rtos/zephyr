...
ZTEST(proxy_agent_test, test_message_content_in_retransmissions)
{
	...
	printk("Send count: %zu\n", mock_backend_send_fake.call_count);

	zbus_chan_pub(&test_channel_1, &test_data, K_MSEC(100));

	k_sleep(K_MSEC(get_total_timeout(2) + 5));

	/* Verify multiple sends occurred */
	printk("Send count: %zu\n", mock_backend_send_fake.call_count);
	zassert_true(mock_backend_send_fake.call_count == initial_send_count + 2,
...
ZTEST(proxy_agent_test, test_concurrent_messages)
{
	...
	zassert_true(mock_backend_send_fake.call_count == 5,
		     "All concurrent messages should be sent");
}

ZTEST(proxy_agent_test, test_ack_timing_boundary)
{
	mock_backend_set_auto_ack(false);

	uint32_t test_data = 0xB0DECA11U;

	zbus_chan_pub(&test_channel_1, &test_data, K_MSEC(100));

	k_sleep(K_MSEC(1));
	zassert_true(mock_backend_send_fake.call_count == 1, "Message should be sent initially");
	...
	size_t count_before_ack = mock_backend_send_fake.call_count;

	/* Send ACK just before timeout */
	if (mock_backend_stored_ack_cb) {
		mock_backend_stored_ack_cb(msg_id, mock_backend_stored_ack_user_data);
	}
	...
	zassert_equal(mock_backend_send_fake.call_count, count_before_ack,
		      "No retransmission should occur after ACK received");
}
