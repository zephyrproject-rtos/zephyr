...
ZTEST(sensing, test_open_connections_limit)
{
	...
	sensing_sensor_handle_t handles[CONFIG_SENSING_MAX_DYNAMIC_CONNECTIONS + 1];

	zassert_not_null(sensor);

	/* Allocate all the connection */
	for (int i = 0; i < CONFIG_SENSING_MAX_DYNAMIC_CONNECTIONS; ++i) {
		zassert_ok(sensing_open_sensor(sensor, &cb_list, &handles[i]));
	}

	/* Try to over allocate */
	zassert_equal(-ENOSPC,
		      sensing_open_sensor(sensor, &cb_list,
					  &handles[CONFIG_SENSING_MAX_DYNAMIC_CONNECTIONS]));

	/* Free one connection */
	zassert_ok(sensing_close_sensor(handles[0]));

	/* Allocate one */
	zassert_ok(sensing_open_sensor(sensor, &cb_list,
				       &handles[CONFIG_SENSING_MAX_DYNAMIC_CONNECTIONS]));
}
