...
static void *isotp_implementation_setup(void)
{
	int ret;

	zassume_true(sizeof(random_data) >= sizeof(data_buf) * 2 + 10,
		     "Test data size to small");

	zassume_true(device_is_ready(can_dev), "CAN device not ready");

	ret = can_set_mode(can_dev, CAN_MODE_LOOPBACK);
	zassume_equal(ret, 0, "Configuring loopback mode failed (%d)", ret);
