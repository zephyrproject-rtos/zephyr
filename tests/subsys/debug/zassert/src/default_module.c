...
ZTEST(zassert_default, test_legacy_level_off)
{
	int disabled_count = 0;
	int enabled_count = 0;

	__ASSERT(false, "must not fire");
	__ASSERT_EVAL(disabled_count++, enabled_count++, false, "must not fire");

	zassert_false(harness_fired(), "legacy level 0 assert fired");
	zassert_equal(disabled_count, 1, "disabled expression not evaluated");
	zassert_equal(enabled_count, 0, "enabled expression evaluated");
}
#endif
