...
ZTEST(zassert_inline, test_inline_forced_on)
{
	HARNESS_EXPECT(inline_forced_on_fn());

	if (IS_ENABLED(CONFIG_ASSERT)) {
		zassert_true(harness_fired(), "forced ON did not fire");
		zassert_true(harness_contains("ASSERTION FAIL"), "location not printed");
		zassert_false(harness_contains("1 == 2"), "condition printed at ON");
		zassert_false(harness_contains("forced on"), "message printed at ON");
	} else {
		zassert_false(harness_fired(), "master switch off must silence ZASSERT_LEVEL_ON");
