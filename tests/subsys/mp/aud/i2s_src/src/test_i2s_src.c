...
static struct mp_aud_i2s_src i2s_src;
...
static void i2s_src_before(void *f)
{
	...
	MP_ELEMENT_INIT(&i2s_src, mp_aud_i2s_src_init, TEST_I2S_SRC_ID);
}

ZTEST(aud_i2s_src, test_binds_the_i2s_receiver_from_devicetree)
{
	const struct device *dev = NULL;

	zassert_true(device_is_ready(i2s_rx_dev), "I2S receiver not ready");

	zassert_ok(mp_object_get_properties((struct mp_object *)&i2s_src, MP_PROP_AUD_SRC_DEVICE,
					    &dev, MP_PROP_LIST_END));
	zassert_equal(dev, i2s_rx_dev, "element did not bind the i2s-codec-rx alias");
}

ZTEST(aud_i2s_src, test_slab_property_round_trips)
{
	struct k_mem_slab *slab = NULL;

	zassert_ok(mp_object_set_properties((struct mp_object *)&i2s_src, MP_PROP_AUD_SRC_SLAB_PTR,
					    &test_i2s_slab, MP_PROP_LIST_END));
	zassert_ok(mp_object_get_properties((struct mp_object *)&i2s_src, MP_PROP_AUD_SRC_SLAB_PTR,
					    &slab, MP_PROP_LIST_END));
 ...
 * pipeline.
 */
ZTEST(aud_i2s_src, test_advertises_receiver_capabilities)
{
	struct mp_caps *caps = i2s_src.aud_src.src.srcpad.caps;
 ...
 */
ZTEST(aud_i2s_src, test_queries_the_receive_direction)
{
	struct audio_caps rx_caps;

	zassert_not_null(i2s_src.aud_src.get_audio_caps, "get_audio_caps not bound");

	zassert_ok(i2s_src.aud_src.get_audio_caps(i2s_rx_dev, &rx_caps),
...
ZTEST(aud_i2s_src, test_set_caps_without_a_slab_is_rejected)
{
	...
	zassert_not_equal(i2s_src.aud_src.src.set_caps(&i2s_src.aud_src.src, caps), 0,
 ...
 */
ZTEST(aud_i2s_src, test_codec_narrows_the_advertised_capabilities)
{
	...
	zassert_ok(mp_object_set_properties((struct mp_object *)&i2s_src,
					    MP_PROP_AUD_I2S_SRC_CODEC_DEVICE, fake_codec_dev,
					    MP_PROP_LIST_END));

	structure = mp_caps_get_structure(i2s_src.aud_src.src.srcpad.caps, 0);
...
ZTEST(aud_i2s_src, test_codec_property_round_trips)
{
	...
	zassert_ok(mp_object_get_properties((struct mp_object *)&i2s_src,
					    MP_PROP_AUD_I2S_SRC_CODEC_DEVICE, &read_back,
					    MP_PROP_LIST_END));
	zassert_equal(read_back, fake_codec_dev, "codec property did not round-trip");
}

ZTEST(aud_i2s_src, test_clock_role_round_trips_and_rejects_junk)
{
	enum mp_aud_i2s_codec_clk_role role;

	zassert_ok(mp_object_set_properties((struct mp_object *)&i2s_src, MP_PROP_AUD_I2S_SRC_CLK_ROLE,
					    (void *)MP_AUD_I2S_TARGET_CODEC_CONTROLLER,
					    MP_PROP_LIST_END));
	zassert_ok(mp_object_get_properties((struct mp_object *)&i2s_src, MP_PROP_AUD_I2S_SRC_CLK_ROLE,
					    &role, MP_PROP_LIST_END));
	zassert_equal(role, MP_AUD_I2S_TARGET_CODEC_CONTROLLER, "clock role did not round-trip");

	zassert_not_equal(mp_object_set_properties((struct mp_object *)&i2s_src,
						   MP_PROP_AUD_I2S_SRC_CLK_ROLE, (void *)0xBADU,
						   MP_PROP_LIST_END),
 ...
 */
ZTEST(aud_i2s_src, test_codec_is_configured_for_capture)
{
	struct mp_caps *caps;
	...
	zassert_ok(i2s_src.aud_src.src.set_caps(&i2s_src.aud_src.src, caps),
...
ZTEST_SUITE(aud_i2s_src, NULL, NULL, i2s_src_before, NULL, NULL);
