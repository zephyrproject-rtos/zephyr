...
static int get_source_name(const struct log_link *link, uint16_t source_id, char *buf,
			   size_t *length)
{
	if (length) {
		*length = 0;
...
static int get_levels(const struct log_link *link, uint16_t source_id, uint8_t *level,
