...
#define LOG_COMPRESSED_MSG_LEN_UNALIGN(args_cnt)                                     \
	((args_cnt < 2) ? offsetof(struct log_msg_compressed, arg1) :                \
			  sizeof(struct log_msg_compressed))
#else
#define LOG_COMPRESSED_MSG_LEN_UNALIGN(args_cnt)                                     \
	(offsetof(struct log_msg_compressed, arg0) + (sizeof(uint32_t) * (args_cnt)))
#endif

#define LOG_COMPRESSED_MSG_LEN(args_cnt)                                             \
	ROUND_UP(LOG_COMPRESSED_MSG_LEN_UNALIGN(args_cnt), Z_LOG_MSG_ALIGNMENT)
