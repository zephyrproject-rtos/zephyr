...
{
	zassert_true(LOG_MSG_NARGS_SINGLE_CHUNK == 3, "test assumes following setting");

	u32_t used_slabs = k_mem_slab_num_used_get(&log_msg_pool);
	struct log_msg *p_msg;

	/* allocation of 0 argument fits in single buffer */
	p_msg = log_msg_create_0(my_string);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	p_msg = log_msg_create_1(my_string, 1);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	p_msg = log_msg_create_2(my_string, 1, 2);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	p_msg = log_msg_create_3(my_string, 1, 2, 3);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	p_msg = log_msg_create_4(my_string, 1, 2, 3, 4);
	zassert_equal((used_slabs + 2), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs += 2;
	...
	p_msg = log_msg_create_5(my_string, 1, 2, 3, 4, 5);
	zassert_equal((used_slabs + 2), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs += 2;
	...
	/* allocation of 5 argument fits in 2 buffers */
	p_msg = log_msg_create_6(my_string, 1, 2, 3, 4, 5, 6);
	zassert_equal((used_slabs + 2), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs += 2;
...
void test_log_hexdump_msg(void)
{
	...
	p_msg = log_msg_hexdump_create(data, LOG_MSG_HEXDUMP_BYTES_SINGLE_CHUNK - 4);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	/* allocation of buffer that fits in single buffer */
	p_msg = log_msg_hexdump_create(data, LOG_MSG_HEXDUMP_BYTES_SINGLE_CHUNK);
	zassert_equal((used_slabs + 1), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs++;
	...
	p_msg = log_msg_hexdump_create(data, LOG_MSG_HEXDUMP_BYTES_SINGLE_CHUNK + 1);
	zassert_equal((used_slabs + 2), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs += 2;
	...
	p_msg = log_msg_hexdump_create(data, LOG_MSG_HEXDUMP_BYTES_SINGLE_CHUNK + HEXDUMP_BYTES_CONT_MSG + 1);
	zassert_equal((used_slabs + 3), k_mem_slab_num_used_get(&log_msg_pool), "Expected mem slab allocation.");
	used_slabs += 3;
...
void test_log_hexdump_data_get_single_chunk(void)
{
	...
	p_msg = log_msg_hexdump_create(data, wr_length);

	offset = 0;
	rd_length = wr_length - 1;
	rd_req_length = rd_length;

	log_msg_hexdump_data_get(p_msg,
				 read_data,
				 &rd_length,
				 offset);

	zassert_equal(rd_length, rd_req_length, "Expected to read requested amount of data\n");
	zassert_true(memcmp(&data[offset], read_data, rd_length) == 0, "Expected data.\n");
	...
	log_msg_hexdump_data_get(p_msg,
				 ...
				 &rd_length,
				 offset);
	zassert_equal(rd_length, wr_length, "Expected to read requested amount of data\n");
	zassert_true(memcmp(&data[offset], read_data, rd_length) == 0, "Expected data.\n");

	/* Attempt to read with non zero offset, requested length fits in the buffer */
	offset = 4;
	...
	/* Attempt to read with non zero offset, requested length DOES NOT fit in the buffer */
	offset = 4;
	rd_length = LOG_MSG_HEXDUMP_BYTES_SINGLE_CHUNK; /* requesting read more data */
	rd_req_length = rd_length;
	...
	log_msg_hexdump_data_get(p_msg,
				 ...
				 &rd_length,
				 offset);
	zassert_equal(rd_length, wr_length - offset, "Expected to read requested amount of data\n");
	zassert_true(memcmp(&data[offset], read_data, rd_length) == 0, "Expected data.\n");
...
void test_log_hexdump_data_get_multiple_chunks(void)
{
	...
	zassert_equal(rd_length, 0, "Expected to read requested amount of data\n");
