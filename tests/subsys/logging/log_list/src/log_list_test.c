...
void test_log_list(void)
{
	...
	p_msg = log_list_head_peek(&my_list);
	zassert_true(&msg1 == p_msg, "Unexpected head 0x%08X.\n", p_msg);

	p_msg = log_list_head_get(&my_list);
	zassert_true(&msg1 == p_msg, "Unexpected head 0x%08X.\n", p_msg);
	zassert_true(NULL == log_list_head_peek(&my_list), "Expected empty list.\n");
	...
	p_msg = log_list_head_peek(&my_list);
	zassert_true(&msg2 == p_msg, "Unexpected head 0x%08X.\n", p_msg);
	...
	p_msg = log_list_head_get(&my_list);
	zassert_true(NULL == p_msg, "Expected empty list.\n");
}
...
void test_log_list_multiple_items(void)
{
	...
	for (i = 0; i < 10; i++) {
		zassert_true(&msg[i] == log_list_head_get(&my_list), "Unexpected head.\n");
	}
	zassert_true(NULL == log_list_head_get(&my_list), "Expected empty list.\n");
}
