...
{
	const struct flash_area *fa;
	int slot = fota_http_get_download_slot(0);

	zassert_true(slot >= 0, "no upload slot");
	zassert_ok(flash_area_open(slot, &fa));
	zassert_ok(flash_area_read(fa, boot_get_image_start_offset(slot), readback, len));
	flash_area_close(fa);
...
{
	int slot = fota_http_get_download_slot(image_index);
...
ZTEST(fota_http, test_invalid_params)
{
	...
	zassert_equal(fota_http_get_download_slot(CONFIG_UPDATEABLE_IMAGE_NUMBER), -ENODEV);
}
...
ZTEST(fota_http, test_download_ok)
{
	...
	zassert_equal(fota_http_nvm_bytes_written(), IMAGE_SIZE);
	zassert_not_null(strstr(last_request, "GET /image.bin HTTP/1.1"));
	verify_slot_content(image, IMAGE_SIZE);

	zassert_ok(fota_http_get_image_header(0, &header));
	...
	zassert_equal(fota_http_get_image_header(0, &header), -EIO);
...
ZTEST(fota_http, test_status_error)
{
	...
	zassert_equal(fota_http_nvm_bytes_written(), 0);
}
...
ZTEST(fota_http, test_truncated)
{
	...
	zassert_true(fota_http_nvm_bytes_written() > 0);
	zassert_true(fota_http_nvm_bytes_written() <= 1500);
	zassert_equal(fota_http_apply(0, false), -ENOEXEC);
	zassert_equal(mcuboot_swap_type(), BOOT_SWAP_TYPE_NONE);
}
...
ZTEST(fota_http, test_no_content_length_truncated)
{
	...
	zassert_equal(download(base_url(), false), -EMSGSIZE);
	zassert_true(fota_http_nvm_bytes_written() > 0);
	zassert_true(fota_http_nvm_bytes_written() < IMAGE_SIZE);
	zassert_equal(fota_http_apply(0, false), -ENOEXEC);
}
...
ZTEST(fota_http, test_cancel_and_busy)
{
	...
	zassert_equal(fota_http_apply(0, false), -EBUSY);
...
ZTEST(fota_http, test_resume)
{
	...
	zassert_equal(download(base_url(), false), -EMSGSIZE);
	resumed_from = fota_http_nvm_bytes_written();
...
ZTEST(fota_http, test_resume_without_progress)
{
	if (!IS_ENABLED(CONFIG_FOTA_HTTP_RESUME)) {
		ztest_test_skip();
	}
