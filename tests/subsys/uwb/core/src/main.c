...
/* UWB API test suite – exercises UCI core commands */
...
LOG_MODULE_REGISTER(test_uwb_core, LOG_LEVEL_DBG);

/*
 * Test fixture – init/deinit the full stack around every test
 */

static void *uwb_core_suite_setup(void)
{
	return NULL;
}

static void uwb_core_suite_teardown(void *data)
{
	ARG_UNUSED(data);
}

ZTEST_SUITE(uwb_core_suite, NULL, uwb_core_suite_setup, NULL, NULL, uwb_core_suite_teardown);

ZTEST(uwb_core_suite, test_device_reset)
{
	uwb_status_code_t status = uwb_api_core_device_reset(0);

	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_core_reset failed: 0x%x", status);
}

ZTEST(uwb_core_suite, test_get_device_info)
{
	uwb_device_info_t info = {0};
	uwb_status_code_t status = uwb_api_core_get_device_info(&info);

	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_get_device_info failed: 0x%x",
		      status);

	/* The emulated backend reports version 1.0 for all version fields */
...
ZTEST(uwb_core_suite, test_get_device_capabilities)
{
...
ZTEST(uwb_core_suite, test_get_device_state)
{
	uint8_t device_state = UWB_DEVICE_STATE_NA;
	uwb_status_code_t status = uwb_api_get_device_state(&device_state);

	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_get_device_info failed: 0x%x",
		      status);

	zassert_equal(device_state, UWB_DEVICE_STATE_READY,
		      "Device state not ready without any active sessions");
}

ZTEST(uwb_core_suite, test_session_state)
{
	uint32_t session_id = 0xDEADBEEF;
	uint32_t session_handle = 0;

	uwb_status_code_t status =
		uwb_api_session_init(session_id, UWB_SESSION_TYPE_RANGING, &session_handle);
	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_session_init failed: 0x%x", status);

	uint8_t session_state = UWB_SESSION_STATUS_ERROR;

	status = uwb_api_session_get_state(session_handle, &session_state);
	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_session_get_state failed: 0x%x",
		      status);
	zassert_equal(session_state, UWB_SESSION_STATUS_INITIALIZED, "session state not init: 0x%x",
		      status);

	status = uwb_api_session_deinit(session_handle);
	zassert_equal(status, UWB_STATUS_CODE_SUCCESS, "uwb_api_session_deinit failed: 0x%x",
		      status);

	status = uwb_api_session_get_state(session_handle, &session_state);
	zassert_equal(status, UCI_STATUS_CODE_ERROR_SESSION_NOT_EXIST,
