...
static int cmd_readline_null_buf(const struct shell *sh, size_t argc, char **argv)
{
	...
	return shell_readline(sh, NULL, 10, K_NO_WAIT);
}

static int cmd_readline_zero_len(const struct shell *sh, size_t argc, char **argv)
{
	uint8_t buf[10];
	...
	return shell_readline(sh, buf, 0, K_NO_WAIT);
}
...
SHELL_CMD_REGISTER(test_readline_zero_len, NULL, NULL, cmd_readline_zero_len);
...
ZTEST(sh, test_shell_readline_errors)
{
	...
	test_shell_execute_cmd("test_readline_null_buf", -EINVAL);
	test_shell_execute_cmd("test_readline_zero_len", -EINVAL);
