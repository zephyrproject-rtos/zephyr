...
ZTEST(precision_timing, test_ptp_adapter_derives_legacy_capabilities)
{
	...
	zassert_ok(precision_ptp_clock_init(&adapter, DEVICE_GET(legacy_ptp_clock), local_domain));
	...
	zassert_equal(caps.max_phase_adjust_ns, 0);
	...
	zassert_ok(precision_clock_adjust_phase(precision_clk, 0));
...
ZTEST(precision_timing, test_ptp_adapter_rejects_invalid_arguments)
{
	...
	zassert_equal(precision_ptp_clock_init(NULL, DEVICE_GET(legacy_ptp_clock), local_domain),
		      -EINVAL);
	zassert_equal(precision_ptp_clock_init(&adapter, NULL, local_domain), -EINVAL);
}
...
ZTEST(precision_timing, test_ptp_adapter_reports_missing_legacy_operations)
{
	...
	zassert_ok(
		precision_ptp_clock_init(&adapter, DEVICE_GET(readonly_ptp_clock), local_domain));
	precision_clk = precision_ptp_clock_get(&adapter);
...
static int fake_clock_read(const struct precision_clock *precision_clk,
			   struct precision_time_point *tp)
{
	struct fake_clock_data *data = (struct fake_clock_data *)precision_clk->user_data;
