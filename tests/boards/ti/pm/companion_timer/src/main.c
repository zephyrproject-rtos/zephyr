...
bool correct_state_entered_during_sleep;
...
int main(void)
{
	const struct device *counter_dev = DEVICE_DT_GET(DT_CHOSEN(zephyr_system_timer_companion));
	...
	uint32_t sleep_durations_ms[] = {min_residency_ms + FIRST_SLEEP_OFFSET_MS,
					 min_residency_ms + FIRST_SLEEP_OFFSET_MS +
						 SECOND_SLEEP_OFFSET_MS,
					 min_residency_ms + FIRST_SLEEP_OFFSET_MS +
						 SECOND_SLEEP_OFFSET_MS + THIRD_SLEEP_OFFSET_MS};
	...
	for (int i = 0; i < 3; i++) {
		...
		if (correct_state_entered_during_sleep) {
			LOG_INF("✓ Duration: %u ms (expected: %u ms)", actual_duration_ms,
				sleep_durations_ms[i]);
		} else {
			LOG_ERR("✗ Duration: %u ms (expected: %u ms) - "
