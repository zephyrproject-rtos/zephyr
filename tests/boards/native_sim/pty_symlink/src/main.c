...
#ifdef CONFIG_PTY_INTERACTIVE_TEST
/* Include native simulator header for self-shutdown */
...
ZTEST(pty_uart_comm, test_uart_echo)
{
	...
	while (1) {
		...
		while (uart_poll_in(uart_dev, &c) == 0) {
			...
			if (memcmp(recv_buf + recv_len - 6, "hello\r", 6) != 0) {
				continue;
			}
			...
			/* Trigger a clean shutdown by setting end time to current time */
			nsi_hws_set_end_of_time(nsi_hws_get_time());
			printk("Triggered clean shutdown after "
			       "sending response\n");
		}
