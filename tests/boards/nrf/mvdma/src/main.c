...
static uint32_t *cachealigned_desc_alloc(uint32_t *desc, size_t desc_size)
{
	size_t size = ROUND_UP(desc_size, CONFIG_DCACHE_LINE_SIZE);
	uint32_t *ret = k_aligned_alloc(CONFIG_DCACHE_LINE_SIZE, size);

	zassert_not_null(ret);
	if (desc) {
		memcpy(ret, desc, desc_size);
	}
	return ret;
}

static void cachealigned_desc_free(void *desc)
{
	k_free(desc);
}
		    ...
		    uint32_t *sink_desc, size_t sink_len, bool blocking)
{
	...
	struct nrf_mvdma_ctrl ctrl = NRF_MVDMA_CTRL_INIT(blocking ? NULL : mvdma_handler, &t_int);

	exp_user_data = &t_int;

	t_start = get_ts();
	rv = nrf_mvdma_xfer(&ctrl, &job, true);
	t_dma_setup = get_ts() - t_start - t_delta;
	zassert_equal(rv, 0, "Unexpected rv:%d", rv);

	if (blocking) {
		while (nrf_mvdma_xfer_check(&ctrl) == -EBUSY) {
		}
	} else {
...
static void test_memcpy(void *dst, void *src, size_t len, bool frag_dst, bool frag_src, bool blk)
{
	...
	uint32_t *source_job = cachealigned_desc_alloc(source_job_data, sizeof(source_job_data));
	uint32_t *source_job_frag = cachealigned_desc_alloc(source_job_frag_data,
							    sizeof(source_job_frag_data));
	uint32_t *sink_job = cachealigned_desc_alloc(sink_job_data, sizeof(sink_job_data));
	uint32_t *sink_job_frag = cachealigned_desc_alloc(sink_job_frag_data,
							  sizeof(sink_job_frag_data));

	rv = sys_cache_data_flush_range(src, len);
	zassert_equal(rv, cache_err);

	dma_run(frag_src ? source_job_frag : source_job,
		frag_src ? sizeof(source_job_frag_data) : sizeof(source_job_data),
		frag_dst ? sink_job_frag : sink_job,
		frag_dst ? sizeof(sink_job_frag_data) : sizeof(sink_job_data), blk);
	...
	cachealigned_desc_free(source_job);
	cachealigned_desc_free(source_job_frag);
	cachealigned_desc_free(sink_job);
	cachealigned_desc_free(sink_job_frag);
}
			   ...
			   size_t total_dst, size_t offset_dst)
{
	...
	uint32_t *source_job = cachealigned_desc_alloc(source_job_data, sizeof(source_job_data));
	uint32_t *sink_job = cachealigned_desc_alloc(sink_job_data, sizeof(source_job_data));

	struct nrf_mvdma_jobs_desc job = {
		.source = source_job,
		.source_desc_size = sizeof(source_job_data),
		.sink = sink_job,
		.sink_desc_size = sizeof(sink_job_data),
	};
	struct nrf_mvdma_ctrl ctrl = NRF_MVDMA_CTRL_INIT(mvdma_handler, NULL);
...
ZTEST(nrf_mvdma, test_copy_from_slow_periph_mem_blocking)
{
	copy_from_slow_periph_mem(true);
}

ZTEST(nrf_mvdma, test_copy_from_slow_periph_mem_nonblocking)
{
...
ZTEST(nrf_mvdma, test_copy_to_slow_periph_mem_blocking)
{
	copy_to_slow_periph_mem(true);
}

ZTEST(nrf_mvdma, test_copy_to_slow_periph_mem_nonblocking)
{
...
#define buf2_dst buffer3
	...
	static uint32_t source_job[] __cacheline_aligned = {
		NRF_MVDMA_JOB_DESC(buf2_src, BUF_LEN, NRF_MVDMA_ATTR_DEFAULT, 0),
		NRF_MVDMA_JOB_TERMINATE
	};
	static uint32_t sink_job[] __cacheline_aligned = {
		NRF_MVDMA_JOB_DESC(buf2_dst, BUF_LEN, NRF_MVDMA_ATTR_DEFAULT, 0),
		NRF_MVDMA_JOB_TERMINATE
	};
	static uint32_t source_job_periph_ram[] __cacheline_aligned = {
		NRF_MVDMA_JOB_DESC(buf1_src, BUF_LEN, NRF_MVDMA_ATTR_DEFAULT, 0),
		NRF_MVDMA_JOB_TERMINATE
	};
	static uint32_t sink_job_periph_ram[] __cacheline_aligned = {
	...
	static struct nrf_mvdma_ctrl ctrl;
	static struct nrf_mvdma_ctrl ctrl2;
	int rv1 = 0;
	int rv2 = 0;
	...
	rv = nrf_mvdma_xfer(&ctrl2, &job2, true);
	zassert_true(rv >= 0, "Unexpected rv:%d", rv);

	t5 = get_ts();

	if (blocking) {
		while ((rv1 = nrf_mvdma_xfer_check(&ctrl)) == -EBUSY) {
		}
		t2 = get_ts();
		nrf_mvdma_task_trigger(NRF_MVDMA, NRF_MVDMA_TASK_PAUSE);
		while ((rv2 = nrf_mvdma_xfer_check(&ctrl2)) == -EBUSY) {
		}
		t3 = get_ts();
	} else {
		rv = k_sem_take(&done, K_MSEC(100));
		...
		bool timing)
{
	...
	uint32_t *src_job1 = cachealigned_desc_alloc(src_job1_, sizeof(src_job1_));
	uint32_t *src_job2 = cachealigned_desc_alloc(src_job2_, sizeof(src_job2_));
	uint32_t *src_job3 = cachealigned_desc_alloc(src_job3_, sizeof(src_job3_));
	uint32_t *sink_job1 = cachealigned_desc_alloc(sink_job1_, sizeof(sink_job1_));
	uint32_t *sink_job2 = cachealigned_desc_alloc(sink_job2_, sizeof(sink_job2_));
	uint32_t *sink_job3 = cachealigned_desc_alloc(sink_job3_, sizeof(sink_job3_));

	struct k_sem sem;
	struct nrf_mvdma_jobs_desc job1 = {
		.source = src_job1,
		.source_desc_size = sizeof(src_job1_),
		.sink = sink_job1,
		.sink_desc_size = sizeof(sink_job1_),
	};
	struct nrf_mvdma_jobs_desc job2 = {
		.source = src_job2,
		.source_desc_size = sizeof(src_job2_),
		.sink = sink_job2,
		.sink_desc_size = sizeof(sink_job2_),
	};
	struct nrf_mvdma_jobs_desc job3 = {
		.source = src_job3,
		.source_desc_size = sizeof(src_job3_),
		.sink = sink_job3,
		.sink_desc_size = sizeof(sink_job3_),
	};
	struct nrf_mvdma_ctrl ctrl1;
	struct nrf_mvdma_ctrl ctrl2;
	struct nrf_mvdma_ctrl ctrl3;
	...
	rv = nrf_mvdma_xfer(&ctrl1, &job1, true);
	zassert_equal(rv, 0, "Unexpected rv:%d", rv);
	...
	if (timing) {
		...
		if (job2_blocking) {
			while ((rv = nrf_mvdma_xfer_check(&ctrl2)) == -EBUSY) {
			}
	...
	rv = nrf_mvdma_xfer(&ctrl3, &job3, true);
	zassert_equal(rv, 0, "Unexpected rv:%d", rv);
	...
	rv = job3_blocking ? nrf_mvdma_xfer_check(&ctrl3) : k_sem_take(&sem, K_NO_WAIT);
	...
	cachealigned_desc_free(src_job1);
	cachealigned_desc_free(src_job2);
	cachealigned_desc_free(src_job3);
	cachealigned_desc_free(sink_job1);
	cachealigned_desc_free(sink_job2);
	cachealigned_desc_free(sink_job3);
}

ZTEST(nrf_mvdma, test_concurrent_jobs_check)
{
...
ZTEST(nrf_mvdma, test_peripheral_operation_blocking)
{
	peripheral_operation(true);
}

ZTEST(nrf_mvdma, test_peripheral_operation_nonblocking)
{
...
static void mix_periph_slow_ram(bool blocking)
{
	...
	static uint32_t xfer_data[] __cacheline_aligned = {
...
ZTEST(nrf_mvdma, test_mix_periph_slow_ram_blocking)
{
	mix_periph_slow_ram(true);
}

ZTEST(nrf_mvdma, test_mix_periph_slow_ram_nonblocking)
{
	mix_periph_slow_ram(false);
}

ZTEST(nrf_mvdma, test_simple_xfer)
{
	struct nrf_mvdma_basic_desc desc __aligned(CONFIG_DCACHE_LINE_SIZE) =
	 ...
	 */
	for (int i = 0; i < 2; i++) {
		struct nrf_mvdma_ctrl ctrl = NRF_MVDMA_CTRL_INIT(NULL, NULL);
		...
		t = get_ts();
		rv = nrf_mvdma_basic_xfer(&ctrl, &desc, false);
		t = get_ts() - t;
		zassert_ok(rv);

		k_busy_wait(1000);
		rv = nrf_mvdma_xfer_check(&ctrl);
		zassert_true(rv >= 0);
...
ZTEST(nrf_mvdma, test_simple_zero_fill)
{
	struct nrf_mvdma_basic_desc desc __aligned(CONFIG_DCACHE_LINE_SIZE) =
