...
ZTEST(comparator_runtime_configure, test_comp_config_se_aref)
{
	...
	zassert_true(rc == 0, "Cannot configure comparator.");

	rc = comparator_set_trigger_callback(test_dev, test_callback, NULL);
	zassert_true(rc == 0, "Cannot set callback for comparator.");

	rc = comparator_set_trigger(test_dev, COMPARATOR_TRIGGER_BOTH_EDGES);
	zassert_true(rc == 0, "Cannot set trigger for comparator.");
	...
	k_msleep(10);
	zassert_true(counter == 1, "Comparator was not triggered for first threshold cross");
	gpio_pin_set_dt(&test_pin_2, 0);
	k_msleep(10);
	zassert_true(counter == 2, "Comparator was not triggered for second threshold cross");
...
zassert_true(rc == 0, "Cannot configure comparator.");
	...
	zassert_true(counter == 0, "Comparator was triggered for rising threshold cross");

	gpio_pin_set_dt(&test_pin_1, 0);
	gpio_pin_set_dt(&test_pin_2, 1);
	k_msleep(10);

	zassert_true(counter == 1, "Comparator was not triggered for falling threshold cross");
}
...
ZTEST(comparator_runtime_configure, test_comp_config_diff_rising)
{
	...
	zassert_true(counter == 0, "Comparator was triggered for falling threshold cross");

	gpio_pin_set_dt(&test_pin_1, 1);
	gpio_pin_set_dt(&test_pin_2, 0);
	k_msleep(10);

	zassert_true(counter == 1, "Comparator was not triggered for rising threshold cross");
}
...
ZTEST(comparator_runtime_configure, test_comp_config_diff_none)
{
	...
	zassert_true(counter == 0, "Comparator was not triggered for rising threshold cross");
}
