...
ZTEST(nandflash, test_nandflash)
{
	...
	page_buf = k_malloc(nand.pagesize + nand.oobsize);
	...
	for (i = 0; i < nand.eccbits; i++) {
		printf(" Flip bit %d of data[%d]: 0x%02x -> 0x%02x\n",
		       i & 0x7, i, page_buf[i], page_buf[i] ^ (char)BIT(i & 0x7));
		page_buf[i] ^= (char)BIT(i & 0x7);
	}
	...
	for (i = 0; i < nand.eccbits; i++) {
		printf(" Read raw data[%d]: 0x%02x\n", i, page_buf[i]);
	}

	ret = nand_read_page(dev, page, page_buf, 0);
	zassert_true(ret >= 0, "Failed to read page %d, ret=%d\n", page, ret);

	for (i = 0; i < nand.eccbits; i++) {
		printf(" Read ecc data[%d]: 0x%02x\n", i, page_buf[i]);
	}
