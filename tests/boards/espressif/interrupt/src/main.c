...
ZTEST(esp_interrupt, test_nested_isr)
{
	...
	while(cnt--) {
		...
		zassert_false(counter_set_channel_alarm(timer0, 0, &alarm_a), "Failed to set alarm A");
		zassert_false(counter_set_channel_alarm(timer1, 0, &alarm_b), "Failed to set alarm B");
		zassert_false(counter_set_channel_alarm(timer2, 0, &alarm_c), "Failed to set alarm C");
	...
	zassert_true(error_cnt == 0,  "Errors occured (%d)", error_cnt);
	zassert_true(success_cnt == TEST_CYCLES, "Not all test cycles pssed (%d from %d)", success_cnt, TEST_CYCLES);
}

ZTEST_SUITE(esp_interrupt, NULL,
	    esp_interrupt_suite_setup,
	    NULL,
	    NULL,
	    NULL);
