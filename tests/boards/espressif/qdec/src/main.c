...
ZTEST(qdec_selfloop, test_09_lower_thresh)
{
	static const struct sensor_trigger trig_u0 = {
		.type = SENSOR_TRIG_THRESHOLD,
		.chan = SENSOR_CHAN_ROTATION,
...
ZTEST(qdec_selfloop, test_10_four_units_concurrent)
{
	int32_t counts[4] = {0};
...
ZTEST(qdec_selfloop, test_11_rotation_degrees)
{
	int32_t q;
	int8_t shift;
...
ZTEST(qdec_selfloop, test_12_high_limit_auto_reset)
{
	...
	zassert_equal(read_counts(counts), 0);
	/*
	 * The hardware counter is clamped back to zero on every high-limit
	 * event, but the driver carries each wrap into a software accumulator,
	 * so the reported count is the true pulse total rather than 25 % 10.
	 */
	zassert_equal(counts[2], 25, "expected 25 across two auto-resets, got %d", counts[2]);
	zassert_true(trig_hits[2] >= 2, "expected H_LIM >= 2 hits, got %u", trig_hits[2]);
}

ZTEST(qdec_selfloop, test_13_zero_cross_unit_counts)
{
	int32_t counts[4] = {0};
...
ZTEST(qdec_selfloop, test_14_quadrature_direction)
{
	int32_t counts[4] = {0};
