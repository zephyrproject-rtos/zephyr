...
static void test_done(void *arg, u32_t channel, int error_code)
{
	...
	if (error_code != 0) {
		printk("ERROR: DMA transfer error code = 0x%x\n", error_code);
		k_sem_give(&dma_sem);
...
static int test_task(u32_t chan_id, u32_t blen, u32_t block_count)
{
	...
	if (block_count > ARRAY_SIZE(transfer_blocks)) {
		printk("ERROR: block_count %u is greater than %ld\n",
		       block_count, ARRAY_SIZE(transfer_blocks));
		return -1;
	}

	dma_device = device_get_binding(DMA_DEVICE_NAME);
	if (!dma_device) {
		printk("ERROR: Cannot get dma controller\n");
		return -1;
	}
	...
	default:
		printk("ERROR: Invalid block count %d\n", block_count);
		return -1;
	}
...
void dma_thread(void)
{
	while (1) {
		...
		for (i = 0; i < ARRAY_SIZE(tests); i++) {
			struct dma_test *test = &tests[i];

			k_sleep(SLEEPTIME);

			if (test_task(test->chan_id, test->blen,
				      test->block_count) == 0) {
				printk("Testing %u/%u/%u: DMA Passed\n",
				       test->chan_id, test->blen,
				       test->block_count);
			} else {
				printk("Testing %u/%u/%u: DMA Failed\n",
				       test->chan_id, test->blen,
				       test->block_count);
			}
		}
...
K_THREAD_DEFINE(dma_thread_id, STACKSIZE, dma_thread,
