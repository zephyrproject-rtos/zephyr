...
REFERENCE_OUTPUT = [
        ...
        "ERROR: /i2c@11112222/test-i2c-dev@10 <NULL> is initialized before its dependency /gpio@ffff <init_fn_0> (PRE_KERNEL_1+1 < PRE_KERNEL_1+2)",
        "ERROR: /i2c@11112222/test-i2c-dev@10 <NULL> is initialized before its dependency /i2c@11112222 <init_fn_1> (PRE_KERNEL_1+1 < PRE_KERNEL_1+3)",
        "INFO: /i2c@11112222/test-i2c-dev@11 <NULL> PRE_KERNEL_1+4 > /gpio@ffff <init_fn_0> PRE_KERNEL_1+2",
        "INFO: /i2c@11112222/test-i2c-dev@11 <NULL> PRE_KERNEL_1+4 > /i2c@11112222 <init_fn_1> PRE_KERNEL_1+3",
]
...
def check_file(file_name, expect):
    ...
    if sorted(expect) != sorted(output):
        print(f"Mismatched output from {file_name}")
        print()
        print("expected:")
        print("\n".join(sorted(expect)))
        print()
