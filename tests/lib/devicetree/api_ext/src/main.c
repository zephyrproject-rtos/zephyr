...
ZTEST(devicetree_api_ext, test_dac_dt_spec)
{
	struct dac_dt_spec dac_spec;

	/* DAC_DT_SPEC_GET_BY_NAME */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME(TEST_DAC, ch1);
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME(TEST_DAC, ch2);
	zexpect_equal(dac_spec.channel_id, 20, "");

	/* DAC_DT_SPEC_INST_GET_BY_NAME */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME(0, ch1);
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME(0, ch2);
	zexpect_equal(dac_spec.channel_id, 20, "");

	/* DAC_DT_SPEC_GET_BY_IDX */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX(TEST_DAC, 0);
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX(TEST_DAC, 1);
	zexpect_equal(dac_spec.channel_id, 20, "");

	/* DAC_DT_SPEC_INST_GET_BY_IDX */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX(0, 0);
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX(0, 1);
	zexpect_equal(dac_spec.channel_id, 20, "");

	/* DAC_DT_SPEC_GET_BY_NAME_OR */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME_OR(TEST_DAC, ch1, {0});
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME_OR(TEST_DAC, ch2, {0});
	zexpect_equal(dac_spec.channel_id, 20, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME_OR(TEST_DAC, ch_missing, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_NAME_OR(TEST_MISSING, ch1, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	/* DAC_DT_SPEC_INST_GET_BY_NAME_OR */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME_OR(0, ch1, {0});
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME_OR(0, ch2, {0});
	zexpect_equal(dac_spec.channel_id, 20, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME_OR(0, ch_missing, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_NAME_OR(100, ch1, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	/* DAC_DT_SPEC_GET_BY_IDX_OR */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX_OR(TEST_DAC, 0, {0});
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX_OR(TEST_DAC, 1, {0});
	zexpect_equal(dac_spec.channel_id, 20, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX_OR(TEST_DAC, 100, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_GET_BY_IDX_OR(TEST_MISSING, 0, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	/* DAC_DT_SPEC_INST_GET_BY_IDX_OR */
	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX_OR(0, 0, {0});
	zexpect_equal(dac_spec.channel_id, 10, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX_OR(0, 1, {0});
	zexpect_equal(dac_spec.channel_id, 20, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX_OR(0, 100, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");

	dac_spec = (struct dac_dt_spec)DAC_DT_SPEC_INST_GET_BY_IDX_OR(100, 0, {0});
	zexpect_equal(dac_spec.channel_id, 0, "");
}
