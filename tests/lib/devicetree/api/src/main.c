...
ZTEST(devicetree_api, test_nodelabel_c_token)
{
	...
	const char *test_gpio = STRINGIFY(DT_NODELABEL_C_TOKEN(DT_PHANDLE(TEST_PH, gpios)));
	zassert_equal(test_gpio, "test_gpio_1");
	const char *test_i2c1 = STRINGIFY(DT_NODELABEL_C_TOKEN(DT_PHANDLE_BY_IDX(TEST_PH, phs, 0)));
	zassert_equal(test_i2c1, "test_i2c1");
	const char *test_spi1 = STRINGIFY(DT_NODELABEL_C_TOKEN(DT_PHANDLE_BY_IDX(TEST_PH, phs, 1)));
	zassert_equal(test_spi1, "test_spi1");
	const char *test_phandles = STRINGIFY(DT_NODELABEL_C_TOKEN(TEST_PH));
	zassert_equal(test_phandles, "test_phandles");

	/* DT_NODELABEL_C_TOKEN_BY_IDX */
	const char *test_i2c = STRINGIFY(DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(TEST_PH, phs, 0), 1));
	zassert_equal(test_i2c, "test_i2c");
	const char *test_spi = STRINGIFY(DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(TEST_PH, phs, 1), 1));
