...
ZTEST(zassert_mygroup, test_passing_assert_is_noop)
{
...
ZTEST(zassert_mygroup, test_failing_assert_fires)
{
	HARNESS_EXPECT(ZASSERT(1 == 2, "boom"));

	if (CONFIG_MYGROUP_ZASSERT_LEVEL == ZASSERT_OFF) {
...
ZTEST(zassert_mygroup, test_failing_assert_message)
{
	HARNESS_EXPECT(ZASSERT(1 == 2, "boom %d", harness_counting_arg()));

	if (CONFIG_MYGROUP_ZASSERT_LEVEL == ZASSERT_VERBOSE) {
...
ZTEST(zassert_mygroup, test_failing_assert_no_message)
{
...
ZTEST_SUITE(zassert_mygroup, NULL, NULL, harness_reset, harness_reset, NULL);
