...
struct service {
	...
	onoff_service_notify_fn onoff_notify;
...
static void basic_start(struct onoff_service *srv,
			onoff_service_notify_fn notify)
{
	struct service *sp = CONTAINER_OF(srv, struct service, onoff);

	if (sp->async_onoff) {
		sp->onoff_notify = notify;
	} else {
		sp->active = sp->onoff_request_rv >= 0;
		notify(srv, sp->onoff_request_rv);
	}
}

static void basic_stop(struct onoff_service *srv,
			onoff_service_notify_fn notify)
{
	struct service *sp = CONTAINER_OF(srv, struct service, onoff);
	...
	if (sp->async_onoff) {
		...
		sp->active = false;
		notify(srv, sp->onoff_release_rv);
	}
}

static struct onoff_service_transitions const basic_onoff_transitions = {
	.start = basic_start,
	.stop = basic_stop,
	.flags = 0,
};
...
static void replace_service_onoff(struct onoff_service_transitions *transitions)
{
...
static void test_sync_priority(void)
{
	...
	for (u32_t i = 0; i < ARRAY_SIZE(operation); ++i) {
...
static void test_onoff_success(void)
{
	...
	service.onoff_notify(&service.onoff, 0);
...
static void test_onoff_start_failure(void)
{
	...
	for (u32_t idx = 0; idx < ARRAY_SIZE(operation); ++idx) {
	...
	zassert_equal(service.process_cnt, 0,
		      "unexpected process");
	for (u32_t idx = 0; idx < ARRAY_SIZE(operation); ++idx) {
		zassert_equal(async_notify_fetch_result(np[idx], &res), -EAGAIN,
	...
	/* Fail the start */
	service.async_onoff = false;
	service.onoff_notify(&service.onoff, onoff_res);

	zassert_equal(service.process_cnt, 0,
		      "unexpected process");

	for (u32_t idx = 0; idx < ARRAY_SIZE(operation); ++idx) {
		zassert_equal(async_notify_fetch_result(np[idx], &res), 0,
			      "fetch %u failed", idx);
...
static void onoff_restart_stop(struct onoff_service *srv,
			       onoff_service_notify_fn notify)
{
	struct service *sp = CONTAINER_OF(srv, struct service, onoff);
...
/* Mutate the operation list during a stop to force a restart.  */
static void onoff_stop_failure_stop(struct onoff_service *srv,
				    onoff_service_notify_fn notify)
{
	struct service *sp = CONTAINER_OF(srv, struct service, onoff);
	struct onoff_restart_data *dp = sp->data;
...
static void test_onoff_stop_failure(void)
{
	...
	zassert_equal(async_notify_fetch_result(np[0], &res), 0,
