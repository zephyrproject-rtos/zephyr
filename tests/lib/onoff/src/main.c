...
static void test_client_init_validation(void)
{
	...
	zassert_equal(z_snode_next_peek(&cli.node), NULL,
		      "cli node mismatch");
	zassert_equal(cli.notify.flags, ASYNC_NOTIFY_METHOD_SPINWAIT,
		      "cli spinwait flags");
	...
	zassert_equal(z_snode_next_peek(&cli.node), NULL,
		      "cli signal node");
	zassert_equal(cli.notify.flags, ASYNC_NOTIFY_METHOD_SIGNAL,
		      "cli signal flags");
	...
	zassert_equal(z_snode_next_peek(&cli.node), NULL,
		      "cli callback node");
	zassert_equal(cli.notify.flags, ASYNC_NOTIFY_METHOD_CALLBACK,
		      "cli callback flags");
...
static void test_validate_args(void)
{
	...
	init_spinwait(&cli);
	cli.notify.flags = ASYNC_NOTIFY_METHOD_COMPLETED;
	rc = onoff_request(&srv, &cli);
	zassert_equal(rc, -EINVAL,
...
static void test_orphan_stop(void)
{
	...
	const struct onoff_transitions transitions =
		ONOFF_TRANSITIONS_INITIALIZER(start, stop, reset,
					      ONOFF_START_SLEEPS | ONOFF_STOP_SLEEPS);
...
static void test_cancel_release(void)
{
	static const struct onoff_transitions srv_transitions =
		ONOFF_TRANSITIONS_INITIALIZER(start, stop, NULL, ONOFF_STOP_SLEEPS);
...
{
	static const struct onoff_transitions srv_transitions =
		ONOFF_TRANSITIONS_INITIALIZER(start, stop, reset, ONOFF_RESET_SLEEPS);
	struct onoff_manager srv = ONOFF_MANAGER_INITIALIZER(&srv_transitions);
	int rc;
