 ...
 */
ZTEST(ringbuffer_api, test_ring_buffer_main)
{
	int ret, put_count, i;
	uint32_t getdata[6];
...
/*test cases*/
ZTEST(ringbuffer_api, test_ringbuffer_init)
{
 ...
 * @see ring_buf_item_put, ring_buf_item_get
 */
ZTEST(ringbuffer_api, test_ringbuffer_put_get_thread)
{
	pbuf = &ringbuf;
...
ZTEST(ringbuffer_api, test_ringbuffer_put_get_isr)
{
	pbuf = &ringbuf;
...
ZTEST(ringbuffer_api, test_ringbuffer_put_get_discard)
{
	pbuf = &ringbuf;
	tringbuf_put((const void *)0);
	tringbuf_put((const void *)1);
 ...
 * @see ring_buf_item_put, ring_buf_item_get
 */
ZTEST(ringbuffer_api, test_ringbuffer_pow2_put_get_thread_isr)
{
	pbuf = &ringbuf_pow2;
	tringbuf_put((const void *)0);
	irq_offload(tringbuf_put, (const void *)1);
...
ZTEST(ringbuffer_api, test_byte_put_free)
{
 ...
 * @see ring_buf_item_put, ring_buf_item_get
 */
ZTEST(ringbuffer_api, test_ringbuffer_array_perf)
{
...
ZTEST(ringbuffer_api, test_ringbuffer_partial_putting)
{
...
ZTEST(ringbuffer_api, test_ringbuffer_partial_getting)
{
	uint8_t indata[RINGBUFFER_SIZE];
	uint8_t outdata[RINGBUFFER_SIZE];
	uint32_t len;
...
ZTEST(ringbuffer_api, test_ringbuffer_equal_bufs)
{
	struct ring_buf buf_ii;
	uint8_t *data;
	uint32_t claimed;
...
/**
 * @brief Test zero-copy get/put pointer API
 *
 * @details Exercise the zero-copy interface end to end: put data and read it
 * back through the pointer from ring_buf_get_ptr, consume it, then acquire a
 * write pointer with ring_buf_put_ptr, write and commit. Verify that repeated
 * get_ptr/put_ptr calls return the same pointer and size, that data is not
 * changed before commit, and that size, space and capacity accounting stays
 * consistent across reads, full commits and partial commits.
 *
 * @ingroup lib_ringbuffer_tests
 *
 * @see ring_buf_get_ptr, ring_buf_put_ptr, ring_buf_consume, ring_buf_commit
 */
...
/**
 * @brief Test zero-copy ring_buf_put_ptr with wrap-around
 *
 * @details Fill the buffer through a write pointer, read part of it to move
 * the tail, then verify ring_buf_put_ptr returns contiguous write regions up
 * to the buffer end on the wrap-around and up to the data start once wrapped.
 * Check the returned pointers, sizes and that the buffer reports full with no
 * space left after all commits.
 *
 * @ingroup lib_ringbuffer_tests
 *
 * @see ring_buf_put_ptr, ring_buf_commit
 */
...
/**
 * @brief Test zero-copy ring_buf_get_ptr with wrap-around
 *
 * @details Put and consume data so that the stored contents wrap around the
 * buffer end, then verify ring_buf_get_ptr returns contiguous read regions up
 * to the buffer end and then the remaining wrapped data, with the correct
 * pointers and sizes. Also check it returns 0 on an empty buffer and after
 * ring_buf_reset.
 *
 * @ingroup lib_ringbuffer_tests
 *
 * @see ring_buf_get_ptr, ring_buf_consume, ring_buf_reset
 */
