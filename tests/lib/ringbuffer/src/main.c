...
ZTEST(ringbuffer_api, test_ringbuffer_raw)
{
	...
	ring_buf_consume(&ringbuf_raw, RINGBUFFER_SIZE);
	zassert_true(ring_buf_is_empty(&ringbuf_raw));
}
...
ZTEST(ringbuffer_api, test_size)
{
	...
	ring_buf_put(&ringbuf_raw, buf, sizeof(buf));
	ring_buf_consume(&ringbuf_raw, 3);
	ring_buf_put(&ringbuf_raw, "x", 1);

	size = ring_buf_size_get(&ringbuf_raw);
	zassert_equal(sizeof(buf) - 2, size,
			"wrong size: exp: %zu: actual: %u", sizeof(buf) - 2,
			size);
}
...
ZTEST(ringbuffer_api, test_invalid_size_init)
{
	uint8_t buf[1];
	struct ring_buf rbempty = RING_BUF_INIT(NULL, 0);
	struct ring_buf rb = RING_BUF_INIT(buf, 0);
	struct ring_buf rb2 = RING_BUF_INIT(buf, 1);

	verify_init_state(&rbempty);
	verify_init_state(&rb);
	verify_init_state(&rb2);
}
...
/* Illustrate that snapshot can be used to write to a ring buffer without modifying the original
 * until commit */
ZTEST(ringbuffer_api, test_claim_finish_cancel)
{
	uint8_t ref[20] = {0};
	...
	do_cbvprintf(str_put, (void *)&str_ctx, str, 12);
