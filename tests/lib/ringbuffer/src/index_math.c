...
{
	/* As no data is pushed to the buffer, this is still illegal but allows us
	 * to test index math at the maximum size without needing to allocate a
	 * large buffer
	 */
	ring_buf_init(rb, RING_BUFFER_MAX_SIZE, edge_dummy);
}
...
ZTEST(ringbuffer_index_math, test_max_size_constants)
{
	...
	zassert_equal(RING_BUFFER_MAX_SIZE, UINT32_MAX / 4);
#else
	zassert_equal(RING_BUFFER_MAX_SIZE, UINT16_MAX / 2);
#endif
}
...
ZTEST(ringbuffer_index_math, test_init_at_max_size)
{
	...
	zassert_equal(ring_buf_capacity_get(&rb), RING_BUFFER_MAX_SIZE);
	zassert_equal(ring_buf_space_get(&rb), RING_BUFFER_MAX_SIZE);
...
ZTEST(ringbuffer_index_math, test_fill_completely_max_size)
{
	...
	while ((avail = ring_buf_put_ptr(&rb, &p, 0))) {
		ring_buf_commit(&rb, avail);
		written += avail;
	}
	zassert_equal(written, RING_BUFFER_MAX_SIZE);
	zassert_true(ring_buf_is_full(&rb));
	zassert_equal(ring_buf_size_get(&rb), RING_BUFFER_MAX_SIZE);
	zassert_equal(ring_buf_space_get(&rb), 0);
	zassert_equal(ring_buf_put_ptr(&rb, &p, 0), 0);

	while ((avail = ring_buf_get_ptr(&rb, &p, 0))) {
		ring_buf_consume(&rb, avail);
		read += avail;
	}
	zassert_equal(read, RING_BUFFER_MAX_SIZE);
	zassert_true(ring_buf_is_empty(&rb));
}
...
ZTEST(ringbuffer_index_math, test_index_at_storage_top)
{
	...
	ring_buf_commit(&rb, 1);
	zassert_equal(rb.write_idx, rb.size);
...
ZTEST(ringbuffer_index_math, test_size_get_wrap_branch_at_max)
{
	...
	rb.read_idx = RING_BUFFER_MAX_SIZE - 2;
	rb.write_idx = RING_BUFFER_MAX_SIZE + 3;

	zassert_equal(ring_buf_capacity_get(&rb), RING_BUFFER_MAX_SIZE);
	zassert_equal(ring_buf_size_get(&rb), 5, "expected %u, got %u", 5, ring_buf_size_get(&rb));
...
ZTEST(ringbuffer_index_math, test_commit_consume_full_storage_walk)
{
	...
	/* Force the index math to wrap around the underlying index type and
	 * verify size/capacity tracking
	 */
