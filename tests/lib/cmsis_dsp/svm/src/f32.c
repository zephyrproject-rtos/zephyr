...
#define DECLARE_COMMON_VARS(in_dims, in_param) \
    ...
    const float32_t intercept = params[svec_count + (vec_dims * svec_count)]; \
...
#define DECLARE_POLY_VARS() \
    ...
    const float32_t coeff0 = params[svec_count + (vec_dims * svec_count) + 1]; \
...
static void test_arm_svm_linear_predict_f32(void)
{
    ...
    arm_svm_linear_init_f32(
        &inst, svec_count, vec_dims, intercept, dual_coeff, svec, classes);
