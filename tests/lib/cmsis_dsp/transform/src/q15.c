...
#define SNR_ERROR_THRESH    ((float32_t)5)
    ...
    const q15_t *input, const q15_t *ref, size_t length)
{
    ...
    /* Validate output */
    zassert_true(
        test_snr_error_q15(length, output, scaled_ref, SNR_ERROR_THRESH),
        ASSERT_MSG_SNR_LIMIT_EXCEED);
...
void test_transform_q15(void)
{
    ztest_test_suite(transform_q15,
    ...
    ztest_run_test_suite(transform_q15);
}
