...
static void test_op2(int op, const uint32_t *ref, size_t length)
{
    ...
    for (index = 0; index < NUM_MATRICES; index++)
    {
        ...
        /* Load matrix data */
        memcpy(mat_in1.pData, in_com1, rows * columns * sizeof(float32_t));
        memcpy(mat_in2.pData, in_com2, rows * columns * sizeof(float32_t));
...
DEFINE_TEST_VARIANT3(op2, arm_mat_add_f32, OP2_ADD, ref_add, ARRAY_SIZE(ref_add));
DEFINE_TEST_VARIANT3(op2, arm_mat_sub_f32, OP2_SUB, ref_sub, ARRAY_SIZE(ref_sub));
...
static void test_arm_mat_inverse_f32(void)
{
    ...
    for (index = 0; index < NUM_MATRICES_INV; index++)
    {
        ...
        /* Load matrix data */
        memcpy(mat_in1.pData, input, rows * columns * sizeof(float32_t));
