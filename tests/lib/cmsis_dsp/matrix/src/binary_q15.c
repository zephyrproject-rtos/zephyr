    ...
    const q15_t *ref, size_t length)
{
    ...
    for (index = 0; index < NUM_MATRICES; index++)
    {
        ...
        memcpy(mat_in2.pData, input2, internal * columns * sizeof(q15_t));
    ...
    zassert_true(
        test_near_equal_q15(length, output, ref, ABS_HIGH_ERROR_THRESH_Q15),
    ...
    const q15_t *ref, size_t length)
{
    ...
    for (index = 0; index < NUM_MATRICES; index++)
    {
        ...
        /* Load matrix data */
        memcpy(mat_in1.pData, input1, 2 * rows * internal * sizeof(q15_t));
        memcpy(mat_in2.pData, input2, 2 * internal * columns * sizeof(q15_t));

        /* Run test function */
        switch (op)
        {
            case OP2C_CMPLX_MULT:
                arm_mat_cmplx_mult_q15(&mat_in1, &mat_in2, &mat_out, scratch);
                break;
            default:
                zassert_unreachable("invalid operation");
        }
