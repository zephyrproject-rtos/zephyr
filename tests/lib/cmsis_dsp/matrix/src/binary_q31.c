    ...
    const q31_t *ref, size_t length)
{
    ...
    for (index = 0; index < NUM_MATRICES; index++)
    {
        ...
        memcpy(mat_in2.pData, input2, internal * columns * sizeof(q31_t));

        /* Run test function */
        switch (op)
        {
            case OP2_MULT:
                arm_mat_mult_q31(&mat_in1, &mat_in2, &mat_out);
                break;
            default:
    ...
    const q31_t *ref, size_t length)
{
    ...
    for (index = 0; index < NUM_MATRICES; index++)
    {
        ...
        /* Load matrix data */
        memcpy(mat_in1.pData, input1, 2 * rows * internal * sizeof(q31_t));
        memcpy(mat_in2.pData, input2, 2 * internal * columns * sizeof(q31_t));

        /* Run test function */
        switch (op)
        {
            case OP2C_CMPLX_MULT:
                arm_mat_cmplx_mult_q31(&mat_in1, &mat_in2, &mat_out);
                break;
            default:
                zassert_unreachable("invalid operation");
        }
