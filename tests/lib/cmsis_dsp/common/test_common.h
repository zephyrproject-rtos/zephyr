...
#define ASSERT_MSG_SNR_LIMIT_EXCEED "signal-to-noise ratio error limit exceeded"
...
static inline bool test_near_equal_f64(
    size_t length, const float64_t *a, const float64_t *b, float64_t threshold)
{
    size_t index;
...
static inline bool test_near_equal_f32(
    size_t length, const float32_t *a, const float32_t *b, float32_t threshold)
{
    size_t index;
