...
ZTEST(mheap_api, test_k_aligned_realloc)
{
	void *r;

	/* Adapted from test_k_aligned_alloc */

	r = k_aligned_realloc(NULL, sizeof(void *), 1);
	/* allocation succeeds */
	zassert_not_null(r, "aligned realloc of 1 byte failed");
	/* r is suitably aligned */
	zassert_equal(0, (uintptr_t)r % sizeof(void *),
		"%p not %u-byte-aligned",
		r, sizeof(void *));

	r = k_aligned_realloc(r, sizeof(void *), 64);
	/* allocation succeeds */
	zassert_not_null(r, "aligned realloc of 64 byte failed");
	/* r is suitably aligned */
	zassert_equal(0, (uintptr_t)r % sizeof(void *),
		"%p not %u-byte-aligned",
		r, sizeof(void *));
	...
	/* allocate with > 8 byte alignment */
	r = k_aligned_alloc(16, 32);
	/* allocation succeeds */
	zassert_not_null(r, "16-byte-aligned realloc failed");
	/* r is suitably aligned */
	zassert_equal(0, (uintptr_t)r % 16,
		"%p not 16-byte-aligned", r);

	/* allocate with > 8 byte alignment */
	r = k_aligned_realloc(r, 16, 26);
	/* allocation succeeds */
	zassert_not_null(r, "16-byte-aligned realloc failed");
	/* r is suitably aligned */
	zassert_equal(0, (uintptr_t)r % 16,
