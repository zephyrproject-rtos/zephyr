...
ZTEST(ringbuffer_api, test_index_overflow)
{
	...
	zassert_true(!memcmp(input, res, sizeof(input)), "read != written");
	zassert_true(ring_buffer_size(&rb) == 0, "ring_buffer should be empty");
	zassert_true(ring_buffer_space(&rb) == ring_buffer_capacity(&rb), "buffer should be empty, full space");
}
...
ZTEST(ringbuffer_api, test_stress)
{
	...
	while (read < sizeof(input)) {
		...
		zassert_true(ring_buffer_size(&rb) == to_write, "buffer should contain written data");
		zassert_true(ring_buffer_space(&rb) == ring_buffer_capacity(&rb) - to_write, "buffer should contain written data");
		while (read < written) {
