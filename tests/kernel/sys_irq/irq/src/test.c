...
ZTEST(sys_irq_irq, test__trigger_irq_while_disabled__no_irq)
{
...
ZTEST(sys_irq_irq, test__trigger_irq_while_enabled__irq)
{
...
ZTEST(sys_irq_irq, test__disable_enabled_irq__retval_1)
{
	zassert_ok(sys_irq_enable(INTD0_SYS_IRQN));
	zassert_ok(sys_irq_enable(INTD1_SYS_IRQN));
...
ZTEST(sys_irq_irq, test__clear_cleared_irq__retval_0)
{
	zassert_equal(sys_irq_clear(INTD0_SYS_IRQN), 0);
	zassert_equal(sys_irq_clear(INTD1_SYS_IRQN), 0);
}

ZTEST(sys_irq_irq, test__clear_triggered_irq__retval_1)
{
	zassert_ok(sys_irq_trigger(INTD0_SYS_IRQN));
	zassert_ok(sys_irq_trigger(INTD1_SYS_IRQN));
...
ZTEST_SUITE(sys_irq_irq, NULL, test_init, test_before, NULL, NULL);
