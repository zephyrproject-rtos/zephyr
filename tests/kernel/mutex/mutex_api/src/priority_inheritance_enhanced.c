...
/*
 * test_zz_deadlock_detection_run_last (below) intentionally triggers
 * __ASSERT while mutex_lock (kernel/mutex.c) is held. In
 * CONFIG_ZTEST_FATAL_HOOK builds the caught fatal error aborts the current
 * thread instead of halting, which leaves mutex_lock's spinlock-owner
 * tracking stuck as "held" for the rest of the process
 * (z_assert_can_swap()'s ztest-abort exemption only accounts for one extra
 * nested lock). Any later test that calls k_mutex_lock()/k_mutex_unlock()
 * would then hit a false "Invalid spinlock" assertion. ztest runs unit
 * tests within a suite in name-sorted (not file/declaration) order, so the
 * "zz_..._run_last" name -- not source position -- is what keeps this test
 * after every other test in this suite that touches a mutex.
 */
#if defined(CONFIG_MUTEX_DEADLOCK_DETECT) && Z_MUTEX_PI_ENABLED
/*
 * After test_zz_deadlock_detection_run_last fires __ASSERT, the test
 * function is aborted before it can clean up. This hook runs after the
 * fatal error is caught and restores a clean state so subsequent tests
 * (only ztest framework bookkeeping happens after this point) are not
 * affected by stale threads or locked mutexes left behind by the aborted
 * test.
 */
 ...
 *
 * Named to sort last (see comment above) so no later test in this suite
 * is corrupted by the stuck mutex_lock spinlock state this test leaves
 * behind.
 */
ZTEST(mutex_api_1cpu, test_zz_deadlock_detection_run_last)
{
