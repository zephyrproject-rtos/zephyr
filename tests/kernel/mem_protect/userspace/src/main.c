...
/* Purposely Not marked with qualifier "_app_dmem()" for test */
bool non_app_global;

static void access_non_app_memory(void)
{
	/*
	 * Attempt to access non_app_global which has not
	 * been placed into the _app_dmem shared memory region.
	 * Depends on CONFIG_APPLICATION_MEMORY=n .
	 */
	expect_fault = true;
	expected_reason = REASON_HW_EXCEPTION;
	BARRIER();
	non_app_global = true;
	zassert_unreachable("Thread accessed global not in "
		"memory domain\n");
}
...
void test_main(void)
{
	...
	ztest_test_suite(userspace,
			 ...
			 ztest_user_unit_test(read_kobject_user_pipe),
			 ztest_user_unit_test(access_non_app_memory),
