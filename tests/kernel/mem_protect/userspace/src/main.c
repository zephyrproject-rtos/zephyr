...
void *z_impl_get_tls_pointer(void)
{
	return &(k_current_get()->userspace_local_data);
}

Z_SYSCALL_HANDLER(get_tls_pointer)
{
	return (u32_t)z_impl_get_tls_pointer();
}
...
void stack_buffer_scenarios(k_thread_stack_t *stack_obj, size_t obj_size)
{
	 ...
	 *
	 * FIXME: Should we round up and enforce equal?
	 */
	zassert_true(STEST_STACKSIZE <= (obj_size - K_THREAD_STACK_RESERVED),
		      "bad stack size in object");

	/* todo Test alignment of stack object address */

	/* todo Test K_THREAD_STACK_LEN() */
	...
	/* TODO: Check that the TLS data struct is within the stack object */
	...
	if (z_arch_is_user_context()) {
		...
		for (pos = stack_start; pos < stack_end; pos++) {
			zassert_false(check_perms((void *)pos, 1, 0),
