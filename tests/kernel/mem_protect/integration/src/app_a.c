...
void sample_callback(const struct device *dev, void *context, void *data)
{
	...
	zassert_equal(ret, 0, "k_msgq_put failed with %d", ret);
}
...
static void monitor_entry(void *p1, void *p2, void *p3)
{
	...
	zassert_equal(ret, 0, "couldn't start driver interrupts");

	while (monitor_count < NUM_LOOPS) {
		payload = sys_heap_alloc(&shared_pool,
					 SAMPLE_DRIVER_MSG_SIZE);

		zassert_not_null(payload, "couldn't alloc memory from shared pool");
		...
		zassert_equal(ret, 0, "k_msgq_get() failed with %d", ret);
	...
	zassert_equal(ret, 0, "couldn't disable driver");
...
static void writeback_entry(void *p1, void *p2, void *p3)
{
	...
	zassert_equal(ret, 0, "no more magic!");
...
void app_a_entry(void *p1, void *p2, void *p3)
{
	...
	zassert_not_null(sample_device, "bad sample device");
