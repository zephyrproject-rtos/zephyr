...
/* Macro declarations */
#define SYS_SEM_MAX_VAL  (3U)
#define TOTAL_MAX (4U)
#define PRIO 5
#define LOW_PRIO 8
#define HIGH_PRIO 2

K_THREAD_STACK_ARRAY_DEFINE(multi_stack_give, STACK_NUMS, STACK_SIZE);
K_THREAD_STACK_ARRAY_DEFINE(multi_stack_take, STACK_NUMS, STACK_SIZE);

struct k_thread multi_tid_give[STACK_NUMS];
struct k_thread multi_tid_take[STACK_NUMS];
struct k_sem usage_sem, sync_sem, limit_sem;
...
{
	k_sem_init(&usage_sem, SEM_INIT_VAL, SYS_SEM_MAX_VAL);
	k_sem_init(&sync_sem, SEM_INIT_VAL, SYS_SEM_MAX_VAL);
...
static void multi_thread_sem_give(void *p1, void *p2, void *p3)
{
	...
	if (count < TOTAL_MAX) {
		zassert_equal(k_sem_count_get(&limit_sem), count,
			       "multi get sem error");
	} else {
		zassert_equal(k_sem_count_get(&limit_sem), SYS_SEM_MAX_VAL,
		"count > SYS_SEM_MAX_VAL");
	}
...
static void multi_thread_sem_take(void *p1, void *p2, void *p3)
{
	...
	if (count >= 0) {
		zassert_equal(k_sem_count_get(&limit_sem), count,
			       "multi take sem error");
	} else {
		zassert_equal(k_sem_count_get(&limit_sem), 0,
				"count < SEM_INIT_VAL");
	}
...
{
	k_sem_init(&limit_sem, SEM_INIT_VAL, SYS_SEM_MAX_VAL);
	k_sem_init(&sync_sem, SEM_INIT_VAL, SYS_SEM_MAX_VAL);
