...
/**
 * @brief The system clock ISR must not truncate a >32-bit elapsed-tick delta.
 *
 * Rewind the driver's announce baseline by a >32-bit tick count, then let a real,
 * short timer fire through the real ISR path. A driver whose tick delta computation
 * truncates to 32 bits will report a masked, small elapsed-tick count to the
 * kernel instead of the true one; a correct driver's uptime jumps forward by the
 * full amount.
 */
ZTEST(long_wait, test_driver_isr_tick_truncation)
{
	const sys_clock_ticks_t rewind_ticks = (sys_clock_ticks_t)UINT32_MAX + 100000;

	uint64_t before = k_uptime_ticks();

	/* Arm the near timer first, while the driver's baseline is still accurate,
	 * so its real deadline is programmed into hardware normally. Then rewind
	 * the baseline so the ISR computes an elapsed delta as if rewind_ticks
	 * had genuinely passed.
	 */
	k_timer_start(&near_timer, K_MSEC(20), K_NO_WAIT);
	z_sys_clock_rewind_ticks(rewind_ticks);

	zassert_ok(k_sem_take(&near_sem, K_MSEC(500)),
		   "near timer did not fire after rewinding the driver baseline");

	uint64_t after = k_uptime_ticks();

	k_timer_stop(&near_timer);

	/* Resync before asserting, so a failing assertion below still leaves the
	 * driver in a working state for tests that run after this one.
	 */
	z_sys_clock_resync();

	zassert_true((after - before) > (uint64_t)UINT32_MAX,
		     "driver ISR truncated the elapsed-tick delta: only %llu ticks reported",
