...
/**
 *
 * @brief Calculate absolute difference between arguments
 *
...
void RegressionTask(void)
{
	 ...
	 */
	if (absdiff(TASK_TICKS_TO_SLEEP, task_actual_sleep_ticks) > 1) {
		TC_ERROR("task_sleep() time is %d instead of %d\n",
				 task_actual_sleep_ticks, TASK_TICKS_TO_SLEEP);
		goto error_out;
	}
	...
	/*
	 * Allow up to 1 tick variance as the test may not have put
	 * the fiber to sleep on a tick boundary
	 */
	if (absdiff(FIBER_TICKS_TO_SLEEP, fiber_actual_sleep_ticks) > 1) {
		TC_ERROR("fiber_sleep() time is %d instead of %d\n",
				 fiber_actual_sleep_ticks, FIBER_TICKS_TO_SLEEP);
		goto error_out;
	}
	...
	/* clear the semaphore given by the test fiber */
