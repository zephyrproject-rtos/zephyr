 ...
 *
 * Schedules timers in a ramp of delays to verify that
 * a within a reasonable range of timeouts for a integration
 * test that these times are schedulable correctly.
 *
 * It could be that some timer drivers (infineon) might
 * have difficulty in scheduling some time periods but not others
 * due to implementation details. This test should catch those.
 *
 * The ramp is logrithmic up to what amounts to ~10 seconds of ticks
 */
