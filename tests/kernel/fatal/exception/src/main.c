...
{
	k_thread_create(&alt_thread, alt_stack, K_THREAD_STACK_SIZEOF(alt_stack), entry, NULL, NULL,
			NULL, K_PRIO_COOP(PRIORITY), 0, K_NO_WAIT);
 ...
 *
 * @details
 * Same contract as test_fatal_cpu_exception() but using a second architecture
 ...
 *
 * @details
 * Companion to test_fatal_except_reason_positive() using a negative reason (-2) to
 ...
 *
 * @details
 * Companion to test_fatal_stack_sentinel_timer() that forces the sentinel check via
 ...
 *
 * @details
 * Same as test_fatal_stack_hw_overflow() but the offending thread uses the FPU
