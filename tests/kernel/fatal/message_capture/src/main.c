...
ZTEST(fatal_message_capture, test_fatal_message_capture)
{
	...
	k_thread_create(&crash_thread, crash_stack, K_THREAD_STACK_SIZEOF(crash_stack), oops_entry,
			NULL, NULL, NULL, K_PRIO_COOP(PRIORITY), 0, K_NO_WAIT);
	k_thread_abort(&crash_thread);

	tc_capture_stop();

	zassert_true(tc_capture_contains(EXPECTED_DUMP_STR), "fatal path did not emit \"%s\"",
		     EXPECTED_DUMP_STR);
	zassert_equal(expected_reason, -1, "fatal handler did not observe K_ERR_KERNEL_OOPS");
}
