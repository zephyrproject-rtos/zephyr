...
ZTEST(cleanup_api, test_defer_k_heap_free)
{
	...
	DEFER(k_heap_free)(&_system_heap, my_ptr);
...
ZTEST(cleanup_api, test_defer_k_mem_slab_free)
{
...

	{
		...
		zassert_ok(ret);
		DEFER(k_mem_slab_free)(&test_slabs, ptr);
...
DEFINE_CLASS(foo, struct foo, foo_destructor(_T), foo_constructor(len), size_t len)

ZTEST(cleanup_api, test_custom_class)
{
	CLASS(foo, f)(42);
