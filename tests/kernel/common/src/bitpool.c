...
ZTEST_USER(bitpool, test_bitpool_atomic_cas)
{
	bool res;
	static const size_t calc_bitcnt = BITPOOL_TEST_LONG_BITCNT;
	ATOMIC_DEFINE(tested_atomic_bitmap, BITPOOL_TEST_LONG_BITCNT) = {0};
	const struct bitpool_atomic tested_atomic =
		BITPOOL_ATOMIC_INIT(tested_atomic_bitmap, BITPOOL_TEST_LONG_BITCNT);
	ATOMIC_VAL_DEFINE(old_atomic, BITPOOL_TEST_LONG_BITCNT);
	ATOMIC_VAL_DEFINE(calc_atomic, BITPOOL_TEST_LONG_BITCNT);

	/* Bitpools have embedded spinlocks and can't on the stack. */
	if (IS_ENABLED(CONFIG_KERNEL_COHERENCE)) {
		ztest_test_skip();
	}

	bitpool_atomic_read(&tested_atomic, old_atomic);
	bitpool_copy(old_atomic, calc_atomic, calc_bitcnt);

	bitpool_set_block_to(calc_atomic, 10, calc_bitcnt - 20, true);

	/* CAS */
	res = bitpool_atomic_cas(&tested_atomic, old_atomic, calc_atomic);
	zassert_true(res, "CAS error");

	bitpool_check_ranges(tested_atomic.bitmap, calc_bitcnt, (10, calc_bitcnt - 11));
...
{
	ATOMIC_DEFINE(tested_atomic_bitmap, BITPOOL_TEST_BITCNT) = {0};
	const struct bitpool_atomic tested_atomic =
		BITPOOL_ATOMIC_INIT(tested_atomic_bitmap, BITPOOL_TEST_BITCNT);
	size_t cnt;
	...
	BITPOOL_ATOMIC_OP(&tested_atomic, old, new)
	{
		bitpool_copy(old, new, tested_atomic.bitcnt);
		bitpool_set_block_to(new, 10, 10, true);
	}
	bitpool_check_ranges(tested_atomic.bitmap, BITPOOL_TEST_BITCNT, (10, 19));
	...
	BITPOOL_ATOMIC_OP(&tested_atomic, old, new)
	{
		...
		if (cnt < 3) {
			BITPOOL_ATOMIC_OP(&tested_atomic, iold, inew)
			{
				bitpool_copy(iold, inew, BITPOOL_TEST_BITCNT);
				bitpool_set_bit(inew, cnt);
			}
			bitpool_check_ranges(tested_atomic.bitmap, BITPOOL_TEST_BITCNT,
					     (0, cnt), (10, 19));
		}
	...
	bitpool_check_ranges(tested_atomic.bitmap, BITPOOL_TEST_BITCNT, (0, 2), (10, 19), (15, 15));
}
...
ZTEST_USER(bitpool, test_bitpool_atomic_op_break)
{
	...
	bitpool_check_ranges(tested_atomic.bitmap, BITPOOL_TEST_BITCNT, (0, 0));
