 ...
 * Example usage:
 *
 *    DT_NODE_NODELABEL(DT_NODELABEL(node1), 0) // node1
 *    DT_NODE_NODELABEL(DT_NODELABEL(node1), 2) // node3
 *
 * @param node_id node identifier
 * @param idx the index to get
 * @retval the idx'th node label of the node
 */
#define DT_NODE_NODELABEL(node_id, idx)	DT_CAT3(node_id, _NODELABEL_IDX_, idx)
