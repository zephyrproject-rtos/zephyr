...
#if defined(CONFIG_MMU) && (CONFIG_KERNEL_VM_BASE != CONFIG_SRAM_BASE_ADDRESS)
/* LMAs and VMAs for the kernel are not the same for all sections but are
 * a fixed offset from each other, corresponding to physical vs. virtual memory
 * location.
 */
#define VM_KERNEL	1
#ifdef CONFIG_XIP
...
#elif defined(CONFIG_XIP) || defined(VM_KERNEL)
