 ...
 *
 * These are used when sending data that is classified with different
 * priorities. So some traffic can be marked as high priority and it will
 * be sent first. There is always at least one work queue in the system.
 * Each network packet that is transmitted goes through a work queue
 * thread that will transmit it.
 */
struct net_traffic_class {
	/** Work queue for handling this TX packet queue */
...
struct net_if_dev {
	...
	/** Make sure we initialize TX queue only once. Multiple queue
	 * inits might happen if we have multiple network interfaces
	 * configured.
	 */
	bool tx_queue_init_done;
} __net_if_align;
