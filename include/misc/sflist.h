				       ...
				       sys_sfnode_t *child)
{
	...
	parent->next_and_flags = cur_flags | (u32_t)child;
}
...
{
	__ASSERT_NO_MSG(!(flags & ~SYS_SFLIST_FLAGS_MASK));
...
{
	__ASSERT_NO_MSG(!(flags & ~SYS_SFLIST_FLAGS_MASK));
	node->next_and_flags = (u32_t)(z_sfnode_next_peek(node)) | flags;
}
