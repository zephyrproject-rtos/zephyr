 ...
 */
#define ASYNC_NOTIFY_EXTENSION_POS 2

/*
 * Mask isolating the bits of async_notify::flags that are available
 * for extension.
 */
#define ASYNC_NOTIFY_EXTENSION_MASK (~BIT_MASK(ASYNC_NOTIFY_EXTENSION_POS))
 ...
 * updated so that asynch_notify_fetch_result() returns success.  This
 ...
 * async_notify object at this point, the containing object may have
...
struct async_notify {
	 ...
	 *
	 * Bits below ASYNC_NOTIFY_EXTENSION_POS are initialized by
	 * async notify API init functions like
	 * async_notify_init_callback(), and must not by modified by
	 ...
	 * be zeroed by async_notify_finalize().
	 */
 ...
 * async_notify instance should confirm that the extension bits are
