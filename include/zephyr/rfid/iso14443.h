...
/**
 * @brief Indicates the supported bit rates for communication between PCD and PICC.
 * @note Only present if bit 4 of T0 is set.
 *
 * Bits 0–2 (DS): Supported bit rates PICC → PCD (card to reader)
 * Bits 4–6 (DR): Supported bit rates PCD → PICC (reader to card)
 * Bit 7:         If set, DS and DR must be equal (symmetric communication only)
 *
 * Bit encoding (DS / DR): 0=106, 1=212, 2=424, 3=848 kbit/s
 */
#define RFID_ISO14443A_ATS_TA_PRESENT 0x10U

/**
 * @brief Defines the timing constraints the PCD must respect during communication.
 * @note Only present if bit 5 of T0 is set.
 *
 * Bits 0–3 (FWI):  Frame Waiting Time Integer – maximum time the reader must
 *                  wait for a response from the card.
 *                  FWT = (256 × 16 / fc) × 2^FWI  (fc = 13.56 MHz)
 * Bits 4–7 (SFGI): Start-up Frame Guard Time Integer – minimum delay required
 *                  after ATS before the first command may be sent.
 *                  SFGT = (256 × 16 / fc) × 2^SFGI
 */
#define RFID_ISO14443A_ATS_TB_PRESENT 0x20U

/**
 * @brief Indicates which optional protocol features are supported by the PICC.
 * @note Only present if bit 6 of T0 is set.
 *
 * Bit 0 (NAD): If set, the card supports Node Address (NAD) for multiplexing.
 * Bit 1 (CID): If set, the card supports Card Identifier (CID),
 *              allowing multiple cards to be addressed individually in the field.
 * Bits 2–7:    RFU – reserved for future use, shall be 0.
 */
