...
/**
 * @file
 * @brief Public API for the SCSI subsystem.
 * @ingroup scsi_interface
 */
...
/**
 * @defgroup scsi_interface SCSI Subsystem
 * @ingroup os_services
 * @{
 */
...
#define SCSI_TST_U_RDY	0x00 /**< SCSI Test Unit Ready command opcode. */
#define SCSI_READ10	0x28 /**< SCSI Read (10-byte CDB) command opcode. */
#define SCSI_READ16	0x88 /**< SCSI Read (16-byte CDB) command opcode. */
#define SCSI_WRITE10	0x2A /**< SCSI Write (10-byte CDB) command opcode. */
#define SCSI_WRITE16	0x8A /**< SCSI Write (16-byte CDB) command opcode. */

#define SCSI_MAX_RETRIES	(3)  /**< Maximum number of retries for SCSI commands. */

#define MAX_CDB_SIZE		(16U) /**< Max CDB size in bytes. */

/* SCSI IOCTL Command Codes */
#define SCSI_IOCTL_TEST_UNIT_READY	(0x02) /**< IOCTL to check device ready. */
#define SG_IO				(0x85) /**< IOCTL for SG I/O requests. */

/* Block Storage Generic (BSG) Protocol Constants */
#define BSG_PROTOCOL_SCSI		(0U) /**< BSG protocol for SCSI. */
#define BSG_SUB_PROTOCOL_SCSI_CMD	(0U) /**< BSG sub-protocol for SCSI cmds. */
#define BSG_SUB_PROTOCOL_SCSI_TRANSPORT	(1U) /**< BSG sub-protocol for transport. */

/* Data Transfer Directions for SCSI Generic (SG) Commands */
#define SG_DXFER_NONE		(1) /**< No data transfer. */
#define SG_DXFER_TO_DEV		(2) /**< Data transfer from host to device. */
#define SG_DXFER_FROM_DEV	(3) /**< Data transfer from device to host. */
...
struct scsi_ops {
	 ...
	 */
	int (*exec)(struct scsi_device *sdev, struct scsi_cmd *cmd);
};
 ...
 */
int scsi_write(struct scsi_device *scsi_dev, uint64_t sector, uint32_t count, const uint8_t *buf);
 ...
 * @return 0 if successful, or a negative error code on failure.
 */
int scsi_read(struct scsi_device *scsi_dev, uint64_t sector, uint32_t count, uint8_t *buf);
 ...
 */
int scsi_add_lun_host(struct scsi_host_info *shost, struct lun_info *lun);
 ...
 * @return 0 if successful, or a negative error code on failure.
 */
int scsi_ioctl(struct scsi_device *sdev, int32_t cmd, void *arg);
