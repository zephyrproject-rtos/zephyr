...
#define SENSOR_INFO_NAME(node_id)					\
 ...
 * element in an iterable section be used by the sensor subsystem to identify
 * all sensor devices.
 *
...
#define SENSOR_DEVICE_DT_DEFINE(node_id, init_fn, pm_device,		\
	...
	DEVICE_DT_DEFINE(node_id, init_fn, pm_device,			\
			 ...
			 api_ptr, __VA_ARGS__);				\
									\
	SENSOR_INFO_DEFINE(SENSOR_INFO_NAME(node_id),			\
			   DEVICE_DT_GET(node_id),			\
			   DT_PROP_OR(node_id, vendor, NULL),		\
			   DT_PROP_OR(node_id, model, NULL),		\
