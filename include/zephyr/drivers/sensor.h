...
extern struct rtio __sensor_fetch_get_compat_rtio;
...
#define SENSOR_FETCH_GET_COMPAT_INITIALIZER(_dev, _iodev, _buf)                                    \
	{                                                                                          \
		.dev = _dev,                                                                       \
		.iodev = _iodev,                                                                   \
		.rtio = &__sensor_fetch_get_compat_rtio,                                           \
...
static inline int z_impl_sensor_sample_fetch(const struct device *dev)
{
	...
	if (api->sample_fetch == NULL) {
		return sensor_sample_fetch_compat(dev, SENSOR_CHAN_ALL);
	}
#endif
