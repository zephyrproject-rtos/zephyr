 ...
 */
typedef struct device *(*cs40l5x_io_bus_regmap_get_device)(const struct device *const dev);
 ...
 */
typedef bool (*cs40l5x_io_bus_regmap_is_ready)(const struct device *const dev);
 ...
 */
typedef int (*cs40l5x_io_bus_regmap_read)(const struct device *const dev, const uint32_t addr,
 ...
 */
typedef int (*cs40l5x_io_bus_regmap_write)(const struct device *const dev, const uint32_t *const tx,
					   const uint32_t len);
...
struct cs40l5x_bus_io {
	...
	cs40l5x_io_bus_regmap_get_device get_device;
	/**< Check if control port device is ready */
	cs40l5x_io_bus_regmap_is_ready is_ready;
	/**< Read from the device */
	cs40l5x_io_bus_regmap_read read;
	/**< Write to the device  */
	cs40l5x_io_bus_regmap_write write;
};
