...
/** @brief Byte swapping to change the endianess.
 *
 * When set, endianess on the data pointed to by memory address will be swapped.
 */
 ...
 */
typedef void (*nrf_mvdma_handler_t)(void *user_data, int status);
...
BUILD_ASSERT(COND_CODE_1(CONFIG_DCACHE,
			 (sizeof(struct nrf_mvdma_basic_desc) == CONFIG_DCACHE_LINE_SIZE), (1)));
 ...
 * handler. Alternatively, transfer can be free running and @ref nrf_mvdma_xfer_check is
 * used to poll for transfer completion. When free running mode is used then it is mandatory
 * to poll until @ref nrf_mvdma_xfer_check reports transfer completion due to the power
 * management.
 *
 ...
 */
int nrf_mvdma_basic_xfer(struct nrf_mvdma_ctrl *ctrl, struct nrf_mvdma_basic_desc *desc,
			 bool queue);
 ...
 * retval -EBUSY transfer is not started because MVDMA was busy and @p queue was false.
 */
int nrf_mvdma_xfer(struct nrf_mvdma_ctrl *ctrl, struct nrf_mvdma_jobs_desc *desc, bool queue);
