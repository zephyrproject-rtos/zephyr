 ...
 */
typedef int (*slcd_set_pin_t)(const struct device *dev, uint32_t pin, uint8_t com_mask, bool on);
...
/**
 * @typedef slcd_start_t
 * @brief Callback API to enable/power on the SLCD controller.
 * See slcd_start() for argument description
 */
typedef int (*slcd_start_t)(const struct device *dev);

/**
 * @typedef slcd_stop_t
 * @brief Callback API to disable/power off the SLCD controller.
 * See slcd_stop() for argument description
 */
typedef int (*slcd_stop_t)(const struct device *dev);
 ...
 */
static inline int slcd_set_pin(const struct device *dev, uint32_t pin, uint8_t com_mask, bool on)
{
	const struct slcd_driver_api *api = (const struct slcd_driver_api *)dev->api;
...
/**
 * @brief Enable/power on the SLCD controller.
 *
 * This function enables and powers on the SLCD controller. Once enabled,
 * the controller can be used to display content on the SLCD display.
 *
 * @param dev SLCD controller device instance
 *
 * @retval 0 on success
 ...
 * @retval -errno Negative errno code on other failure
 */
static inline int slcd_start(const struct device *dev)
{
	const struct slcd_driver_api *api = (const struct slcd_driver_api *)dev->api;
...
/**
 * @brief Disable/power off the SLCD controller.
 *
 * This function disables and powers off the SLCD controller. Once disabled,
 * the display content will be cleared and the controller will be in low-power mode.
 *
 * @param dev SLCD controller device instance
 *
 * @retval 0 on success
 ...
 * @retval -errno Negative errno code on other failure
 */
static inline int slcd_stop(const struct device *dev)
{
	const struct slcd_driver_api *api = (const struct slcd_driver_api *)dev->api;
