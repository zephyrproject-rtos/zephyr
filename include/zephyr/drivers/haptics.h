...
/**
 * @typedef haptics_preempt_output_t
 * @brief Preempt the current haptic event and set the haptic device to start output for a new
 * event.
 * @param dev Pointer to the device structure for haptic device instance
 */
typedef int (*haptics_preempt_output_t)(const struct device *dev);
 ...
 * @param dev Pointer to the device structure for haptic device instance
 */
typedef void (*haptics_stop_output_t)(const struct device *dev);
...
__subsystem struct haptics_driver_api {
	...
	haptics_preempt_output_t preempt_output;
};
...
static inline void z_impl_haptics_stop_output(const struct device *dev)
{
	const struct haptics_driver_api *api = (const struct haptics_driver_api *)dev->api;
...
/**
 * @brief Set the haptic device to preempt output for a playback event
 *
 * @param dev Pointer to the device structure for haptic device instance
 *
 * @retval 0 if successful
 * @retval < 0 if failed
 */
__syscall int haptics_preempt_output(const struct device *dev);

static inline int z_impl_haptics_preempt_output(const struct device *dev)
{
	const struct haptics_driver_api *api = (const struct haptics_driver_api *)dev->api;

	return api->preempt_output(dev);
}
