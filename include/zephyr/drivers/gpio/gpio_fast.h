 ...
 *
 * TOOLCHAIN_PRAGMA_UNROLL fully unrolls the loop, producing straight-line
 * NOP instructions with zero loop overhead. The argument can be any
 * compile-time constant expression (including arithmetic).
 *
 * We use this in place of LISTIFY() because then we can compute the NOP
 * count at compile time based on the vendor-specific cycle costs. Otherwise,
 * this has to be a literal, which pushes the math to Kconfig, which is much
