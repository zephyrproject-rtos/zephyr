...
struct vringh {
	...
	/**
	 * @brief Guest notification callback
	 * Called to notify guest about completed buffers.
	 */
	void (*notify)(struct vringh *vr);

	/**
	 * @brief Guest-to-host notification callback
	 * Called when guest sends notification to host.
	 */
	void (*received)(struct vringh *vr);
};
 ...
 * @param notify_callback  Guest notification callback
 *
 * @retval 0        Success
 * @retval -EINVAL  Invalid parameters
 ...
 *
 * @code{.c}
 * void guest_notify(struct vringh *vrh) {
 *     vhost_queue_notify(vrh->dev, vrh->queue_id);
 * }
 *
 * int ret = vringh_init_device(&vrh, vhost_dev, 0, guest_notify);
 * @endcode
 */
