...
#ifdef CONFIG_COUNTER_64BITS_FREQ
#define get_frequency z_impl_counter_get_frequency_64
#else
#define get_frequency z_impl_counter_get_frequency
#endif
...
{
	uint64_t ticks = (us * get_frequency(dev)) / USEC_PER_SEC;
...
{
	return (us * get_frequency(dev)) / USEC_PER_SEC;
}
...
{
	return ((uint64_t)ticks * USEC_PER_SEC) / get_frequency(dev);
}
...
{
	return (ticks * USEC_PER_SEC) / get_frequency(dev);
}
...
{
	return (ns * get_frequency(dev)) / NSEC_PER_SEC;
}
...
{
	return ((uint64_t)ticks * NSEC_PER_SEC) / get_frequency(dev);
}
...
{
	return (ticks * NSEC_PER_SEC) / get_frequency(dev);
}
