...
struct lin_msg {
	...
	union {
		/** Payload data accessed by bytes */
		uint8_t data[LIN_MAX_DLEN];
		/** Payload data accessed by 32-bit words */
...
struct lin_event {
	...
	int status;
	/** Event-specific context information */
	union {
 ...
 * See @a lin_set_event_callback() for argument description.
 */
typedef int (*lin_set_event_callback_t)(const struct device *dev, lin_event_callback_t callback,
					void *user_data);
...
#define LIN_DT_DRIVER_CONFIG_GET(node_id, _min_bitrate, _max_bitrate)                              \
	{                                                                                          \
		...
		.bitrate = DT_PROP_OR(node_id, bitrate, 0),                                        \
	}
...
/**
 * @driver_ops{LIN Controller}
 */
__subsystem struct lin_driver_api {
	/**
	 * @driver_ops_mandatory @copybrief lin_start
	 */
	lin_start_t start;
	/**
	 * @driver_ops_mandatory @copybrief lin_stop
	 */
	lin_stop_t stop;
	/**
	 * @driver_ops_mandatory @copybrief lin_configure
	 */
	lin_configure_t configure;
	/**
	 * @driver_ops_mandatory @copybrief lin_get_config
	 */
	lin_get_config_t get_config;
	/**
	 * @driver_ops_optional @copybrief lin_set_rx_filter
	 */
	lin_set_rx_filter_t set_rx_filter;
	/**
	 * @driver_ops_mandatory @copybrief lin_send
	 */
	lin_send_t send;
	/**
	 * @driver_ops_mandatory @copybrief lin_receive
	 */
	lin_receive_t receive;
	/**
	 * @driver_ops_optional @copybrief lin_response
	 */
	lin_response_t response;
	/**
	 * @driver_ops_optional @copybrief lin_read
	 */
	lin_read_t read;
	/**
	 * @driver_ops_optional @copybrief lin_wakeup_send
	 */
	lin_wakeup_send_t wakeup_send;
	/**
	 * @driver_ops_mandatory @copybrief lin_set_event_callback
	 */
	lin_set_event_callback_t set_event_callback;
};
 ...
 */
static inline int lin_set_event_callback(const struct device *dev, lin_event_callback_t callback,
					 void *user_data)
{
	return DEVICE_API_GET(lin, dev)->set_event_callback(dev, callback, user_data);
}
