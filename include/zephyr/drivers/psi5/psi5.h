...
enum psi5_status {
	...
	PSI5_RX_POWERTRAIN_FRAME,
	PSI5_RX_ERR_SERIAL_FRAME,
	PSI5_RX_ERR_POWERTRAIN_FRAME,
};
 ...
 */
typedef void (*psi5_rx_serial_frame_callback_t)(const struct device *dev, uint8_t channel,
						struct psi5_frame *frame, enum psi5_status status,
						void *user_data);

/**
 * @brief Defines the application callback handler function signature for receiving powertrain
 * frame.
 *
 * @param dev        Pointer to the device structure for the driver instance.
 * @param channel    The hardware channel of the driver instance.
 ...
 * @param user_data  User data provided when the filter was added.
 */
typedef void (*psi5_rx_powertrain_frame_callback_t)(const struct device *dev, uint8_t channel,
...
typedef int (*psi5_add_rx_callback_t)(const struct device *dev, uint8_t channel,
				      ...
				      psi5_rx_powertrain_frame_callback_t powertrain_callback,
				      void *user_data);
...
static inline int z_impl_psi5_add_rx_callback(
	const struct device *dev, uint8_t channel, psi5_rx_serial_frame_callback_t serial_callback,
	psi5_rx_powertrain_frame_callback_t powertrain_callback, void *user_data)
{
	const struct psi5_driver_api *api = (const struct psi5_driver_api *)dev->api;

	if (api->add_rx_callback) {
		return api->add_rx_callback(dev, channel, serial_callback, powertrain_callback,
					    user_data);
	}
