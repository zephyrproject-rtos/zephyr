 ...
 */
typedef void (*psi5_tx_callback_t)(const struct device *dev, uint8_t hw_channel, enum psi5_state,
				   void *user_data);
 ...
 */
typedef void (*psi5_rx_callback_t)(const struct device *dev, uint8_t hw_channel,
 ...
 */
typedef int (*psi5_start_t)(const struct device *dev, uint8_t hw_channel);

/**
 * @brief Callback API upon stopping PSI5
 * See @a psi5_stop() for argument description
 */
typedef int (*psi5_stop_t)(const struct device *dev, uint8_t hw_channel);

/**
 * @brief Callback API upon sending PSI5 frame
 * See @a psi5_transmit() for argument description
 */
typedef int (*psi5_send_t)(const struct device *dev, uint8_t hw_channel, const uint64_t psi5_data,
 ...
 */
typedef int (*psi5_add_rx_callback_t)(const struct device *dev, uint8_t hw_channel,
...
static inline int psi5_start(const struct device *dev, uint8_t hw_channel)
{
...
static inline int psi5_stop(const struct device *dev, uint8_t hw_channel)
{
	const struct psi5_driver_api *api = (const struct psi5_driver_api *)dev->api;
...
static inline int psi5_send(const struct device *dev, uint8_t hw_channel, const uint64_t psi5_data,
			    k_timeout_t timeout, psi5_tx_callback_t callback, void *user_data)
{
	const struct psi5_driver_api *api = (const struct psi5_driver_api *)dev->api;

	if (api->send) {
		return api->send(dev, hw_channel, psi5_data, timeout, callback, user_data);
	}
...
static inline int psi5_add_rx_callback(const struct device *dev, uint8_t hw_channel,
				       psi5_rx_callback_t callback, void *user_data)
{
	const struct psi5_driver_api *api = (const struct psi5_driver_api *)dev->api;
