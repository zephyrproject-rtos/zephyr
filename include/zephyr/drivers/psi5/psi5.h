...
/**
 * @brief Defines the application callback handler function signature for receiving data frame.
 *
 * @param dev        Pointer to the device structure for the driver instance.
 * @param channel    The hardware channel of the driver instance.
 * @param frame      Received frame.
 ...
 * @param user_data  User data provided when the filter was added.
 */
typedef void (*psi5_rx_data_frame_callback_t)(const struct device *dev, uint8_t channel,
					      struct psi5_frame *frame, enum psi5_status status,
					      void *user_data);
...
typedef int (*psi5_add_rx_callback_t)(const struct device *dev, uint8_t channel,
				      ...
				      psi5_rx_data_frame_callback_t data_callback, void *user_data);
 ...
 * @param dev       Pointer to the device structure for the driver instance.
 * @param channel   The hardware channel of the driver instance.
 * @param callback  This function is called by PSI5 driver whenever a serial callback is received.
 * @param callback  This function is called by PSI5 driver whenever a data callback is received.
 * @param user_data User data to pass to callback function.
 * @retval 0 if successful.
 * @retval -EINVAL if an invalid channel is given.
 */
					      ...
					      void *user_data)
{
	...
	if (api->add_rx_callback) {
		return api->add_rx_callback(dev, channel, serial_callback, data_callback,
					    user_data);
	}
