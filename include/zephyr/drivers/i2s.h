...
__subsystem struct i2s_driver_api {
	...
	/**
	 * @driver_ops_optional @copybrief i2s_get_state
	 * See i2s_get_state() for arguments description.
	 */
 ...
 * @retval I2S_STATE_UNKNOWN    Driver does not support state reporting, or
 *                              the requested direction is not available on
 *                              this device.
 */
...
{
	if (DEVICE_API_GET(i2s, dev)->get_state == NULL) {
