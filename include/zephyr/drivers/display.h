 ...
 *
 * @brief Called either in ISR context (if in_isr=true at register time) or in thread context
 * (if in_isr=false, driver will schedule work to call it).
 ...
 *
 * @param dev Pointer to device structure
 * @param cb User callback function pointer, see display_event_cb_t description
 ...
 *
 * @return 0 and a non-zero out_reg_handle on success, otherwise a negative errno code.
