 ...
 * @brief Device driver API for NVMEM providers.
 *
 * Devices whose purpose is to expose NVMEM cells implement this driver API.
 * The NVMEM subsystem dispatches cell accesses to it, next to the memory
 * device APIs it also supports.
 *
 ...
 */
typedef int (*nvmem_provider_api_read)(const struct device *dev, uint32_t addr, size_t off,
 ...
 */
typedef int (*nvmem_provider_api_write)(const struct device *dev, uint32_t addr, size_t off,
					const void *data, size_t len);

/** @driver_ops{NVMEM provider} */
__subsystem struct nvmem_provider_driver_api {
	/** @driver_ops_mandatory @copybrief nvmem_provider_api_read */
	nvmem_provider_api_read read;
	/**
	 * @driver_ops_optional @copybrief nvmem_provider_api_write
