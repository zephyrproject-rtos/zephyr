...
enum stepper_signal_result {
	...
	STEPPER_SIGNAL_TARGET_POSITION_REACHED = 0,
};
 ...
 */
typedef int (*stepper_enable_t)(const struct device *dev, bool enable);
 ...
 */
typedef int (*stepper_move_t)(const struct device *dev, int32_t micro_steps,
 ...
 * @retval  0 Success
 */
__syscall int stepper_enable(const struct device *dev, bool enable);

static inline int z_impl_stepper_enable(const struct device *dev, bool enable)
{
...
static inline int z_impl_stepper_move(const struct device *dev, int32_t micro_steps,
					  struct k_poll_signal *async)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;
 ...
 * @retval       0 Success
 */
__syscall int stepper_set_actual_position(const struct device *dev, const int32_t value);

static inline int z_impl_stepper_set_actual_position(const struct device *dev,
							 const int32_t value)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;
 ...
 * @retval       0 Success
 */
__syscall int stepper_set_target_position(const struct device *dev, const int32_t value,
					      struct k_poll_signal *async);

static inline int z_impl_stepper_set_target_position(const struct device *dev,
							 const int32_t value,
							 struct k_poll_signal *async)
{
	const struct stepper_api *api = (const struct stepper_api *)dev->api;
