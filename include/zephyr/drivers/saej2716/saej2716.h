 ...
 * @param dev       Pointer to the device structure for the driver instance.
 * @param channel   The hardware channel of the driver instance.
 * @param serial_callback  This function is called by SAEJ2716 driver whenever a serial frame is
 * received.
 * @param fast_callback  This function is called by SAEJ2716 driver whenever a fast frame is
 * received.
				  ...
				  saej2716_rx_fast_frame_callback_t fast_callback, void *user_data)
{
	...
	if (api->register_callback) {
		return api->register_callback(dev, channel, serial_callback, fast_callback,
					      user_data);
	}
