...
struct pulse_io_config {
	...
	/**
	 * Line state when idle. On TX this is the level the line rests
	 * at between and after frames. Carrier modulation and
	 * demodulation apply to the non-idle (active) level. RX capture
	 * itself is polarity-agnostic: received symbols and cells carry
	 * explicit levels.
	 */
	bool idle_high;
