 ...
 *
 * @param emulator The device Emulator instance
 ...
 */
typedef int (*emul_espi_api_set_vw)(const struct emul *emulator, enum espi_vwire_signal vw,
				    uint8_t level);
 ...
 * @retval -EIO General input / output error.
 */
typedef int (*emul_espi_api_get_vw)(const struct emul *emulator, enum espi_vwire_signal vw,
				    uint8_t *level);
 ...
 */
typedef uintptr_t (*emul_espi_api_get_acpi_shm)(const struct emul *emulator);
#endif
