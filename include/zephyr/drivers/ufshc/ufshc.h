...
/**
 * @file
 * @brief Public API for UFS Host Controller (UFSHC) drivers.
 * @ingroup ufshc_interface
 */

#ifndef ZEPHYR_INCLUDE_DRIVERS_UFS_UFSHC_H_
#define ZEPHYR_INCLUDE_DRIVERS_UFS_UFSHC_H_
...
/**
 * @defgroup ufshc_interface UFSHC Drivers
...
__subsystem struct ufshc_driver_api {
	 ...
	 *         -EIO: I/O error.
	 */
	int (*phy_initialization)(const struct device *dev);
	 ...
	 *         -EIO: I/O error.
	 */
	int (*link_startup_notify)(const struct device *dev, uint8_t status);
};
 ...
 *         -EIO: I/O error.
 */
__syscall int ufshc_variant_phy_initialization(const struct device *dev);

static inline int z_impl_ufshc_variant_phy_initialization(const struct device *dev)
{
 ...
 *         -EIO: I/O error.
 */
__syscall int ufshc_variant_link_startup_notify(const struct device *dev,
					uint8_t status);

static inline int z_impl_ufshc_variant_link_startup_notify(const struct device *dev,
							   uint8_t status)
{
	const struct ufshc_driver_api *api = (const struct ufshc_driver_api *)dev->api;
...
#endif /* ZEPHYR_INCLUDE_DRIVERS_UFS_UFSHC_H_ */
