...
/** API for registering one asynchronous callback */
typedef int (*cellular_api_subscribe_event)(const struct device *dev, enum cellular_event event,
...
__subsystem struct cellular_driver_api {
	...
	cellular_api_subscribe_event register_event_cb;
};
...
/**
 * @brief Subscribe to one asynchronous cellular event.
 *
 * Call again with @c cb = NULL to remove the subscription.
 *
 * @retval 0         Success
 ...
 */
static inline int cellular_event_subscribe(const struct device *dev, enum cellular_event event,
					   cellular_event_cb_t cb, void *user_data)
{
	const struct cellular_driver_api *api = (const struct cellular_driver_api *)dev->api;
	...
	return api->register_event_cb(dev, event, cb, user_data);
}
