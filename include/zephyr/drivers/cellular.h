 ...
 * @param count In: capacity of @p cells. Out: number of entries written, which
 * on @c -ENOMEM is the capacity that was filled rather than the number measured.
 *
 * @return 0 on success, negative errno value on failure.
 * @retval -ENOSYS API is not supported by cellular network device.
 * @retval -EINVAL @p cells or @p count is NULL.
