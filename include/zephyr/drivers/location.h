			...
			struct location_position *position)
{
	...
	if (api->position_get == NULL) {
		return -ENOSYS;
	}

	return api->position_get(dev, position);
}
			...
			struct location_bearing *bearing)
{
	...
	if (api->bearing_get == NULL) {
		return -ENOSYS;
	}

	return api->bearing_get(dev, bearing);
}
			...
			struct location_speed *speed)
{
	...
	if (api->speed_get == NULL) {
		return -ENOSYS;
	}

	return api->speed_get(dev, speed);
}
			...
			struct location_altitude *altitude)
{
	...
	if (api->altitude_get == NULL) {
		return -ENOSYS;
	}

	return api->altitude_get(dev, altitude);
}
