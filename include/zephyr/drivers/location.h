...
/**
 * @brief Location events contained in location_events_t
 */
...
/**
 * @typedef location_events_t
 * @brief Contains location events represented by single bits
 */
typedef uint8_t location_events_t;
...
typedef void (*location_event_handler_t)(const struct device *dev,
			location_events_t events, void *user_data);
 ...
 * @brief Location API
 */
__subsystem struct location_driver_api {
			...
			struct location_position *position)
{
	...
	if (((const struct location_driver_api *)dev->api)->position_get == NULL) {
		return -ENOSYS;
	}

	return ((const struct location_driver_api *)dev->api)->position_get(dev, position);
}
			...
			struct location_bearing *bearing)
{
	...
	if (((const struct location_driver_api *)dev->api)->bearing_get == NULL) {
		return -ENOSYS;
	}

	return ((const struct location_driver_api *)dev->api)->bearing_get(dev, bearing);
}
			...
			struct location_speed *speed)
{
	...
	if (((const struct location_driver_api *)dev->api)->speed_get == NULL) {
		return -ENOSYS;
	}

	return ((const struct location_driver_api *)dev->api)->speed_get(dev, speed);
}
			...
			struct location_altitude *altitude)
{
	...
	if (((const struct location_driver_api *)dev->api)->altitude_get == NULL) {
		return -ENOSYS;
	}

	return ((const struct location_driver_api *)dev->api)->altitude_get(dev, altitude);
}
			...
			location_event_handler_t handler, void *user_data)
{
	...
	if (((const struct location_driver_api *)dev->api)->event_handler_set == NULL) {
		return -ENOSYS;
	}

	const struct location_driver_api *api = (const struct location_driver_api *)dev->api;

	return api->event_handler_set(dev, handler, user_data);
}
