 ...
 *
 * In this case the device used with the API will function in controller mode,
 * meaning it will control the CLK line on the SPI bus and the chip select,
 * and therefore have a full control over the timing of the transaction.
 */
 ...
 *
 * With this mode, the device used with the API will function in peripheral mode,
 * meaning it will need to wait for it's select line to be addressed,
 ...
 *
 * @details
 * For testing purposes, enable hardware loopback, so that transmit data is
 * fed back to the receiver of the same controller.
...
/**
 * @brief Get SPI polarity and phase mode bitmask from a @ref spi_operation_t
 */
 ...
 */
struct spi_buf {
	/** Valid pointer to a data buffer, or NULL for a NOP indication */
	void *buf;
...
/**
 * @brief SPI scattered buffer array structure
 *
 ...
 *
 * @note In slave mode, data transfer happens when the master asserts CS and
 ...
 *
 * With this API the transfer function will return immediately and
 * report completion through a callback mechanism.
 *
 * @{
 */
 ...
 *       the function of master/slave modes is the same except that this function
 *       will return immediately upon submitting the transfer and get the callback
 *       once finished.
 *
 * @note This function is available only if @kconfig{CONFIG_SPI_ASYNC}
