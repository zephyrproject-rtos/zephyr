...
#define SPI_CS_CONTROL_INIT_NATIVE(node_id)						\
	.lead = DT_PROP_OR(node_id, spi_cs_setup_delay_ns, 0),				\
	.lag = DT_PROP_OR(node_id, spi_cs_hold_delay_ns, 0),
...
{										\
	...
	.active_high = DT_PROP(node_id, spi_cs_high),				\
...
static uint16_t spi_get_word_delay(const struct spi_config *cfg)
{
