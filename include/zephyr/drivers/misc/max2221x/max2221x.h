...
/**
 * @brief Enable the MAX2221x device.
 *
 * @param dev Pointer to the device structure for the driver instance.
 *
 * @return 0 on success, or a negative error code on failure.
...
/**
 * @brief Disable the MAX2221x device.
 *
 * @param dev Pointer to the device structure for the driver instance.
 *
 * @return 0 on success, or a negative error code on failure.
...
/**
 * @brief Enable a channel on the MAX2221x device.
 *
 * @param dev Pointer to the device structure for the driver instance.
...
/**
 * @brief Disable a channel on the MAX2221x device.
 *
 * @param dev Pointer to the device structure for the driver instance.
...
/**
 * @brief Enable a ramp on channel of the MAX2221x device.
 *
 * Example usage:
 * @code
 * #include <zephyr/drivers/misc/max2221x/max2221x.h>
 *
 * // Enable ramp up and ramp down on channel
 * uint8_t ramp_mask = MAX2221X_RAMP_UP_MASK | MAX2221X_RAMP_DOWN_MASK;
 *
 * int ret = max2221x_enable_ramps(dev, channel, ramp_mask);
 * if (ret)
 *     printk("Failed to enable ramp on channel %d: %d\n", channel, ret);
 * else
 *     printk("Ramp enabled on channel %d\n", channel);
 * @endcode
 *
 * @param dev Pointer to the device structure for the driver instance.
 * @param channel The channel to enable the ramp for.
 * @param ramp_mask The ramp mask to enable.
 *
 * @return 0 on success, or a negative error code on failure.
 */
...
/**
 * @brief Disable a ramp on channel of the MAX2221x device.
 *
 * @param dev Pointer to the device structure for the driver instance.
 * @param channel The channel to disable the ramp for.
 * @param ramp_mask The ramp mask to disable.
 *
 * Example usage:
 * @code
 * #include <zephyr/drivers/misc/max2221x/max2221x.h>
 *
 * // Disable ramp up and ramp down on channel
 * uint8_t ramp_mask = MAX2221X_RAMP_UP_MASK | MAX2221X_RAMP_DOWN_MASK;
 *
 * int ret = max2221x_disable_ramps(dev, channel, ramp_mask);
 * if (ret)
 *     printk("Failed to disable ramp on channel %d: %d\n", channel, ret);
 * else
 *     printk("Ramp disabled on channel %d\n", channel);
 * @endcode
 *
 * @return 0 on success, or a negative error code on failure.
 */
 ...
 * #include <zephyr/drivers/misc/max2221x/max2221x.h>
 *
 * // Set rapid fire parameters and start rapid fire mode
 * int ret;
 * ret = max2221x_set_on_time(dev, channel, 1000); // Set on time to 1000 us
 * if (ret) {
 *     printk("Failed to set on time: %d\n", ret);
 *     return ret;
 * }
 * ret = max2221x_set_off_time(dev, channel, 1000); // Set off time to 1000 us
 * if (ret) {
 *     printk("Failed to set off time: %d\n", ret);
 *     return ret;
 * }
 * ret = max2221x_set_stop_state(dev, channel, true); // Set stop state
 * if (ret) {
 *     printk("Failed to set stop state: %d\n", ret);
 *     return ret;
 * }
 * ret = max2221x_set_repetitions(dev, channel, 10); // Set repetitions to 10
 * if (ret) {
 *     printk("Failed to set repetitions: %d\n", ret);
 *     return ret;
 * }
 * ret = max2221x_start_rapid_fire(dev, channel); // Start rapid fire mode
 * if (ret) {
