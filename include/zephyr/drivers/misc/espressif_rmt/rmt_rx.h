 ...
 * @note When CONFIG_ESPRESSIF_RMT_ISR_IRAM_SAFE is enabled, the callback itself and functions called by it should be placed in IRAM.
 ...
 * @retval -ENODEV Create RMT RX channel failed because all RMT channels are used up and no more free one.
 * @retval -ENODEV Create RMT RX channel failed because some feature is not supported by hardware, e.g. DMA feature is not supported by hardware.
 * @retval -ENODEV Create RMT RX channel failed because of other error.
 */
 ...
 * @note If you want this function to work even when the flash cache is disabled, please enable the `CONFIG_ESPRESSIF_RMT_RECV_FUNC_IN_IRAM` option.
 *
 ...
 * @retval -ENODEV Initiate receive job failed because channel is not enabled.
 * @retval -ENODEV Initiate receive job failed because of other error.
 */
int rmt_receive(rmt_channel_handle_t rx_channel, void *buffer, size_t buffer_size, const rmt_receive_config_t *config);
 ...
 */
int rmt_rx_register_event_callbacks(rmt_channel_handle_t rx_channel, const rmt_rx_event_callbacks_t *cbs, void *user_data);
