...
/**
 * @file
 * @brief Espressif RMT RX public API
 */
 ...
 * @note When CONFIG_ESPRESSIF_RMT_RX_ISR_CACHE_SAFE is enabled, the callback itself and functions
...
typedef struct {
	...
	struct {
		 ...
		 * sleep mode. This can save power, but at the expense of more RAM being consumed to
...
typedef struct {
	...
	struct {
		 ...
		 * the data piece by piece, because the user buffer is not sufficient to save all
		 * the data.
		 */
