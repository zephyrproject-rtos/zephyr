...
__subsystem struct dali_driver_api {
	int (*receive)(const struct device *dev, struct dali_frame *frame, k_timeout_t timeout);
 ...
 * @param[out] rx_frame received DALI frame, or event
			       ...
			       k_timeout_t timeout)
{
	...
	__ASSERT_NO_MSG(dev);
	__ASSERT_NO_MSG(rx_frame);
	return api->receive(dev, rx_frame, timeout);
}
 ...
 *
 * @param[in] dev         DALI device
 * @param[in] tx_frame    DALI frame to transmit
 *
 * @retval 0          success
 * @retval -EINVAL    invalid input parameters
