...
/*
 * All the numeric types described in this file are little endian.
 */
...
struct gb_operation_msg_hdr {
	...
	uint16_t operation_id; /* Operation unique id */
	uint8_t type;          /* E.g GB_I2C_TYPE_* or GB_GPIO_TYPE_* */
	uint8_t result;        /* Result of request (in responses only) */
...
struct gb_fw_download_find_firmware_request {
	uint8_t firmware_tag[GB_FIRMWARE_TAG_MAX_SIZE];
} __packed;
...
struct gb_svc_pwrmon_rail_names_get_response {
	uint8_t status;
	uint8_t name[][GB_SVC_PWRMON_RAIL_NAME_BUFSIZE];
} __packed;
...
struct gb_lights_get_channel_config_response {
	...
	uint32_t flags;
	uint32_t color;
	uint8_t color_name[32];
	uint32_t mode;
	uint8_t mode_name[32];
} __packed;
...
struct gb_audio_pcm {
	uint8_t stream_name[GB_AUDIO_PCM_NAME_MAX];
	uint32_t formats; /* GB_AUDIO_PCM_FMT_* */
	uint32_t rates;   /* GB_AUDIO_PCM_RATE_* */
	uint8_t chan_min;
	uint8_t chan_max;
	uint8_t sig_bits; /* number of bits of content */
} __packed;

struct gb_audio_dai {
	uint8_t name[AUDIO_DAI_NAME_MAX];
	uint16_t data_cport;
...
struct gb_audio_ctl_elem_info { /* See snd_ctl_elem_info in Linux source */
	uint8_t type;           /* GB_AUDIO_CTL_ELEM_TYPE_* */
	uint16_t dimen[4];
	union {
...
struct gb_audio_ctl_elem_value { /* See snd_ctl_elem_value in Linux source */
	uint64_t timestamp;      /* XXX needed? */
	union {
		uint32_t integer_value[2]; /* consider CTL_DOUBLE_xxx */
		uint64_t integer64_value[2];
		uint32_t enumerated_item[2];
	} value;
} __packed;

struct gb_audio_control {
	uint8_t name[AUDIO_CONTROL_NAME_MAX];
	uint8_t id;    /* 0-63 */
	uint8_t iface; /* GB_AUDIO_IFACE_* */
	uint16_t data_cport;
	uint32_t access;      /* GB_AUDIO_ACCESS_* */
	uint8_t count;        /* count of same elements */
	uint8_t count_values; /* count of values, max=2 for CTL_DOUBLE_xxx */
	struct gb_audio_ctl_elem_info info;
} __packed;

struct gb_audio_widget {
	uint8_t name[AUDIO_WIDGET_NAME_MAX];
	uint8_t sname[AUDIO_WIDGET_NAME_MAX];
	uint8_t id;
	uint8_t type;  /* GB_AUDIO_WIDGET_TYPE_* */
	uint8_t state; /* GB_AUDIO_WIDGET_STATE_* */
	uint8_t ncontrols;
	struct gb_audio_control ctl[]; /* 'ncontrols' entries */
} __packed;

struct gb_audio_route {
	uint8_t source_id;      /* widget id */
	uint8_t destination_id; /* widget id */
	uint8_t control_id;     /* 0-63 */
	uint8_t index;          /* Selection within the control */
} __packed;
...
struct gb_audio_topology {
	...
	uint8_t num_routes;
	uint32_t size_dais;
	uint32_t size_controls;
	uint32_t size_widgets;
	uint32_t size_routes;
	uint32_t jack_type;
	/*
