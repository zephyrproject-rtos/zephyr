...
struct fota_http_download_params {
...
#if defined(CONFIG_FOTA_HTTP_SHA256_CHECK) || defined(__DOXYGEN__)
	/**
	 * Optional expected SHA-256 digest of the image, 32 bytes. Only
	 * present with @kconfig{CONFIG_FOTA_HTTP_SHA256_CHECK}.
	 */
	const uint8_t *sha256;
#endif
#if defined(CONFIG_FOTA_HTTP_TLS) || defined(__DOXYGEN__)
	/**
	 * Optional TLS security tags to use instead of
	 * @kconfig{CONFIG_FOTA_HTTP_TLS_SEC_TAG}. Only present with
	 * @kconfig{CONFIG_FOTA_HTTP_TLS}.
	 */
	 ...
	 * Only present with @kconfig{CONFIG_FOTA_HTTP_TLS}.
	 */
	 ...
	 * server is fetched again from the start. Only present with
	 * @kconfig{CONFIG_FOTA_HTTP_RESUME}.
	 */
#if defined(CONFIG_FOTA_HTTP_RESUME) || defined(__DOXYGEN__)
	bool resume;
#endif
};
 ...
 * download leaves the slot partially written, which MCUboot rejects, and
 * fota_http_apply() refuses it until a later download succeeds. An image
 * rejected by the SHA-256 or downgrade check is erased.
 *
 ...
 * @retval -ENOTSUP The URL scheme is not supported.
 ...
 * @retval -EILSEQ The SHA-256 digest does not match, the image was erased.
 ...
 *                than the running one and was erased.
 ...
 *
 * Requires @kconfig{CONFIG_FOTA_HTTP_ASYNC}.
 *
 ...
 * @retval -ENOTSUP @kconfig{CONFIG_FOTA_HTTP_ASYNC} is disabled.
 ...
 *
 * The secondary slot must hold an image with a valid MCUboot header, and
 * the last download into it since boot, if any, must have succeeded. The
 ...
 * @retval -ENODEV No slot exists for @p image_index.
 * @retval -EBUSY A download is in progress.
 * @retval -ENOEXEC The secondary slot holds no valid image, or the last
 *                  download into it failed.
...
/**
 * @brief Bytes written to non-volatile memory by the most recent download.
 *
