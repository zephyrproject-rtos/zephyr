...
#define EBU_ADDRSEL0_REGENAB_Pos 0
#define EBU_ADDRSEL0_ALTENAB_Pos 1
#define EBU_ADDRSEL0_WPROT_Pos	 2
...
#define EBU_BUSRCON0_PORTW_Pos	 22
...
#define EBU_BUSWCON0_PORTW_Pos	 22
...
#define XMC4XXX_EBU_SET_ADDRSEL(regenab, altenab, wprot)                                           \
	((regenab) << EBU_ADDRSEL0_REGENAB_Pos | (altenab) << EBU_ADDRSEL0_ALTENAB_Pos |           \
	 (wprot) << EBU_ADDRSEL0_WPROT_Pos)

#define XMC4XXX_EBU_SET_BUSRCON(fetblen, fbbmsel, bfsss, fdbken, bfcmsel, naa, ecse, ebse, dba,    \
				waitinv, bcgen, portw, wait, aap)                                  \
	((fetblen) << EBU_BUSRCON0_FETBLEN_Pos | (fbbmsel) << EBU_BUSRCON0_FBBMSEL_Pos |           \
	 ...
	 (bcgen) << EBU_BUSRCON0_BCGEN_Pos | (portw) << EBU_BUSRCON0_PORTW_Pos |                   \
	 (wait) << EBU_BUSRCON0_WAIT_Pos | (aap) << EBU_BUSRCON0_AAP_Pos)

#define XMC4XXX_EBU_SET_BUSWCON(fetblen, fbbmsel, naa, ecse, ebse, waitinv, bcgen, portw, wait,    \
				aap, lockcs)                                                       \
	((fetblen) << EBU_BUSWCON0_FETBLEN_Pos | (fbbmsel) << EBU_BUSWCON0_FBBMSEL_Pos |           \
	 ...
	 (bcgen) << EBU_BUSWCON0_BCGEN_Pos | (portw) << EBU_BUSWCON0_PORTW_Pos |                   \
	 (wait) << EBU_BUSWCON0_WAIT_Pos | (aap) << EBU_BUSWCON0_AAP_Pos |                         \
	 (lockcs) << EBU_BUSWCON0_LOCKCS_Pos)
...
#define XMC4XXX_EBU_SDRMOD(xba, xopm, coldstart, opmode, caslat, btyp, burstl)                     \
	((xba) << EBU_SDRMOD_XBA_Pos | (xopm) << EBU_SDRMOD_XOPM_Pos |                             \
	 (coldstart) << EBU_SDRMOD_COLDSTART_Pos | (opmode) << EBU_SDRMOD_OPMODE_Pos |             \
	 (caslat) << EBU_SDRMOD_CASLAT_Pos | (btyp) << EBU_SDRMOD_BTYP_Pos |                       \
	 (burstl) << EBU_SDRMOD_BURSTL_Pos)

#define XMC4XXX_EBU_SDRMREF(res_dly, arfsh, selfrex_dly, erfshc, autoselfr, selfrenst, selfrex,    \
			    selfrexst, refreshr, refreshc)                                         \
	((res_dly) << EBU_SDRMREF_RES_DLY_Pos | (arfsh) << EBU_SDRMREF_ARFSH_Pos |                 \
	 ...
	 (autoselfr) << EBU_SDRMREF_AUTOSELFR_Pos | (selfrenst) << EBU_SDRMREF_SELFRENST_Pos |     \
	 (selfrex) << EBU_SDRMREF_SELFREX_Pos | (selfrexst) << EBU_SDRMREF_SELFREXST_Pos |         \
