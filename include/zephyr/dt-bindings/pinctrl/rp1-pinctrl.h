...
/**
 * @file
 * @brief Devicetree pin control helpers for Raspberry Pi RP1.
 */
...
/**
 * @name RP1 Alternate functions
 * @brief Common RP1 alternate function selectors.
 *
 * SoC-specific signal names are defined in the corresponding pin list below.
 * Refer to the RP1 datasheet tables describing GPIO alternate functions for
 * the source of these selector values.
 */
...
/**
 * @brief Encode an RP1 pin number and alternate function into one pinmux value.
 */
#define RP1_PINMUX(pin, func) (((pin) << RP1_PIN_NUM_POS) | ((func) << RP1_ALT_FUNC_POS))

/** @brief Extract the bank field from an encoded RP1 pinmux value. */
#define RP1_GET_BANK(pinctrl) (((pinctrl) >> RP1_BANK_POS) & RP1_BANK_MASK)
/** @brief Extract the pin number from an encoded RP1 pinmux value. */
#define RP1_GET_PIN(pinctrl)  (((pinctrl) >> RP1_PIN_NUM_POS) & RP1_PIN_NUM_MASK)
/** @brief Extract the alternate function from an encoded RP1 pinmux value. */
#define RP1_GET_FUNC(pinctrl) (((pinctrl) >> RP1_ALT_FUNC_POS) & RP1_ALT_FUNC_MASK)

/**
 * @name RP1 GPIO Override
 *
 * Output, input, and IRQ override values for RP1 GPIO control.
 *
 * @{
 */
