 ...
 * INPUTMUX PMUX_SHIFT (20) OR'ed with the per-source selector value.
 *
 * In the devicetree, cookies are used as the "connection" specifier in
 * "mux-states = <&inputmux channel connection>" inside "reference-sources"
 * and "target-sources" child nodes. The channel cell selects the
 * FMEASURE_CH_SEL slot (0 = reference, 1 = target); the same connection
 * cookie is valid for either channel.
 *
 * The connection cookie is also the runtime clock_monitor_set_source()
 * selector for both reference and target sources.
 */
