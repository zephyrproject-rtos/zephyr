...
#if defined(CONFIG_HEAPTRACE) || defined(__DOXYGEN__)

/**
 * @defgroup heaptrace_apis Heap Trace APIs
 * @ingroup heaps
 * @{
 */

/**
 * @brief Maximum thread name length including the terminating '\0'.
 *
 * Sized from CONFIG_THREAD_MAX_NAME_LEN when thread naming is enabled
 * (kernel default 32, 64 under ZTEST), with a fallback large enough for
 * the "<unnamed>:0x20001234" formatted by heaptrace_get_thread_name_by_tid()
 * when it is not (20 characters + NUL for a 32-bit pointer, 36 + NUL on
 * 64-bit systems).
 */
#ifdef CONFIG_THREAD_MAX_NAME_LEN
#define HEAPTRACE_THREAD_NAME_LEN CONFIG_THREAD_MAX_NAME_LEN
#else
#define HEAPTRACE_THREAD_NAME_LEN (sizeof(void *) * 2 + 13)
#endif
 ...
 *
 * @note The resize inference above only works when the pointer is already
 * tracked.  When it is not -- because a filter dropped the original
 * allocation, the record table was full, or heaptrace_reset() ran since
 * -- the alloc half creates a fresh record and the companion free then
 * removes it, leaving the still-live block untracked.  Inferring a
 * resize from an alloc/free pair cannot separate the two cases; an
 * unambiguous fix would be a dedicated HEAP_REALLOC notification from
 * the allocator, which heap_listener does not currently emit.
 *
...
/**
 * @brief Human-readable name for a heap identifier
 *
 * @param heap_id Heap identifier carried by heap events
 *
 * @return "libc" for HEAP_ID_LIBC, "sys_heap" otherwise
 */
const char *heaptrace_heap_id_to_name(uintptr_t heap_id);
