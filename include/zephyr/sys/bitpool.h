 ...
 *
 * @code
 * BITPOOL_ATOMIC_OP(&bitmap, old, new)
 * {
 *    bitpool_copy(old, new, bitmap->bitcnt);
 ...
 * @code
 * bool status = false;
 * ATOMIC_VAL_DEFINE(old, bitmap->bitcnt);
 * ATOMIC_VAL_DEFINE(new, bitmap->bitcnt);
 *
 * while(!status) {
 *   bitpool_atomic_read(&bitmap, old, bitmap->bitcnt);
 *   // Here goes the code from BITPOOL_ATOMIC_OP body
 *   bitpool_copy(old, new, bitmap->bitcnt);
 *   bitpool_set_inv_block(new, 0, 15, 1);
 *   // End of the code from BITPOOL_ATOMIC_OP body
 *   status = bitpool_atomic_cas(&bitmap, old, new);
 * }
 * @endcode
 *
 * @param _bp     Bitpool atomic instance
 ...
 */
#define BITPOOL_ATOMIC_OP(_bp, _old, _new)                                       \
	for (bool __status = false, __first_run = true; __first_run; /* noop */) \
		for (atomic_val_t _old[ATOMIC_BITMAP_SIZE((_bp)->bitcnt)],       \
				  _new[ATOMIC_BITMAP_SIZE((_bp)->bitcnt)];       \
		    __first_run; /* noop */)                                     \
			for (__first_run = false;                                \
			    bitpool_atomic_read(_bp, _old), !__status;           \
			    __status = bitpool_atomic_cas(_bp, _old, _new))

/** @brief Exit from @ref BITPOOL_ATOMIC_OP loop body */
#define BITPOOL_ATOMIC_OP_break break

/**
 * @brief Atomic bitpool instance
 *
 * Instance used by the only really atomic operations implemented in the bitpool module:
 * @ref bitpool_atomic_read, @ref bitpool_atomic_write and @ref bitpool_atomic_cas.
 */
struct bitpool_atomic {
	struct k_spinlock *spinlock; /**< Spinlock for the bitpool atomic operations */
	atomic_t *bitmap;            /**< Pointer to the bitmap */
	size_t bitcnt;               /**< Number of bits in the bitmap */
};

/**
 * @brief Initialize the bitpool atomic structure with spinlock
 *
 * Initialize the bitpool structure giving the external spinlock pointer.
 * The main usage would be to initialize static bitpool structure inside function body,
 * where the spinlock cannot be created inline becouse of its scope.
 *
 * Example of usage:
 * @code
 * void foo(void)
 ...
 *     static ATOMIC_DEFINE(my_bitmap, 32);
 *     static const struct bitpool = BITPOOL_ATOMIC_INIT_SPINLOCK(my_bitmap, 32, &my_spinlock);
 *     ...
 * }
 * @endcode
 */
#define BITPOOL_ATOMIC_INIT_SPINLOCK(_bitmap, _bitcnt, _spinlock) { \
	.spinlock = _spinlock,                                      \
	.bitmap = _bitmap,                                          \
	.bitcnt = _bitcnt,                                          \
}
...
/**
 * @brief Initialize the bitpool atomic structure
 *
 * Initialize the bitpool structure with automatic spinlock.
 * Be aware that the spinlock would be created in the same scope as the bitpool structure.
 * It is not recommended to use this macro in the local scope if the bitpool structure is
 * created with static modifier.
 */
#define BITPOOL_ATOMIC_INIT(_bitmap, _bitcnt) \
	BITPOOL_ATOMIC_INIT_SPINLOCK(_bitmap, _bitcnt, &(struct k_spinlock){})
...
void bitpool_atomic_read_internal(const struct bitpool_atomic *source, atomic_val_t *target);

/**
 * @brief Read atomic bitmap to atomic value copy
 *
 * @param[in]  source The source atomic bitpool instance
 * @param[out] target The memory where to write the data
 */
static inline void bitpool_atomic_read(const struct bitpool_atomic *source, atomic_val_t *target)
{
	if (source->bitcnt <= ATOMIC_BITS) {
		*target = atomic_get(source->bitmap);
	} else {
		bitpool_atomic_read_internal(source, target);
	}
}

void bitpool_atomic_write_internal(const struct bitpool_atomic *target, const atomic_val_t *source);
 ...
 */
static inline void bitpool_atomic_write(const struct bitpool_atomic *target,
					const atomic_val_t *source)
{
	if (target->bitcnt <= ATOMIC_BITS) {
		atomic_set(target->bitmap, *source);
	} else {
		bitpool_atomic_write_internal(target, source);
	}
}

bool bitpool_atomic_cas_internal(const struct bitpool_atomic *target,
 ...
 */
static inline bool bitpool_atomic_cas(const struct bitpool_atomic *target,
				      const atomic_val_t *old_value,
				      const atomic_val_t *new_value)
{
	if (target->bitcnt <= ATOMIC_BITS) {
		return atomic_cas(target->bitmap, *old_value, *new_value);
	} else {
		return bitpool_atomic_cas_internal(target, old_value, new_value);
	}
}
