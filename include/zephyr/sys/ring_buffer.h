/*
 * Copyright (c) 2025 Måns Ansgariusson
 *
...
#define RING_BUFFER_DECLARE(buff, size) RING_BUF_DECLARE(buff, size)
...
#endif
#define RING_BUFFER_MAX_SIZE ((((size_t)1) << (8 * sizeof(ring_buffer_index_t))) - 1)
 ...
 *
 * The number of bytes must be equal to or lower than the preceding @ref ring_buffer_write_.
 *
 * @param  rb Address of ring buffer.
 ...
 *
 * The number of bytes must be less or equal to the preceding @ref ring_buffer_read_ .
