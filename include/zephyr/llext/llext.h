...
/**
 * @brief List of ELF segments stored or referenced in the llext subsystem.
 */
enum llext_mem {
	...
	LLEXT_MEM_COUNT,        /**< Number of segments managed by LLEXT */
};
...
/**
 * @brief Linkable loadable extension structure
 *
...
struct llext {
	...
	/** Lookup table of memory segments */
	void *mem[LLEXT_MEM_COUNT];

	/** Is the memory for this segment allocated on heap? */
	bool mem_on_heap[LLEXT_MEM_COUNT];

	/** Size of each stored segment */
...
/**
 * @brief Add the known memory partitions of the extension to a memory
 * domain.
 *
 * Allows an extension to be executed in user mode threads when memory
 * protection hardware is enabled.
 *
