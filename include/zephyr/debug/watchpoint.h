 ...
 *
 * @retval 0 Success.
 * @retval -EPERM Called from user mode; the descriptor is not accessed.
 ...
 * @retval -ENOTSUP The request or secondary CPU startup configuration is unsupported.
 */
 ...
 * false result for a valid descriptor queried from supervisor mode means
 * the descriptor is fully disarmed.
 *
 ...
 * This function may be called from any supervisor context. Calls from user
 * mode return false without accessing the descriptor.
 *
 * @param wp Watchpoint descriptor.
 *
 * @retval true The watchpoint is arming, armed, or disarming.
 * @retval false The watchpoint is fully disarmed, @p wp is NULL, or the caller
 *         is in user mode.
 */
