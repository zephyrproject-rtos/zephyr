...
struct bt_hci_cp_configure_data_path {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, vs_config);
} __packed;
...
struct bt_hci_std_codecs {
	uint8_t num_codecs;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_std_codec_info, codec_info);
} __packed;
...
struct bt_hci_vs_codecs {
	uint8_t num_codecs;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_vs_codec_info, codec_info);
} __packed;
struct bt_hci_rp_read_codecs {
	uint8_t status;
	/* other fields filled in dynamically */
	FLEXIBLE_ARRAY_DECLARE(uint8_t, codecs);
} __packed;
...
struct bt_hci_std_codecs_v2 {
	uint8_t num_codecs;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_std_codec_info_v2, codec_info);
} __packed;
...
struct bt_hci_vs_codecs_v2 {
	uint8_t num_codecs;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_vs_codec_info_v2, codec_info);
} __packed;
...
struct bt_hci_rp_read_codec_capabilities {
	...
	/* other fields filled in dynamically */
	FLEXIBLE_ARRAY_DECLARE(uint8_t, capabilities);
} __packed;
...
struct bt_hci_cp_read_ctlr_delay {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, codec_config);
} __packed;
...
struct bt_hci_cp_le_set_ext_adv_enable {
	uint8_t  enable;
	uint8_t  set_num;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_ext_adv_set, s);
} __packed;
...
struct bt_hci_cp_le_set_ext_scan_param {
	...
	uint8_t  filter_policy;
	uint8_t  phys;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_ext_scan_phy, p);
} __packed;
...
struct bt_hci_cp_le_ext_create_conn {
	...
	bt_addr_le_t peer_addr;
	uint8_t      phys;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_ext_conn_phy, p);
} __packed;
...
struct bt_hci_cp_le_set_pawr_subevent_data_element {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, subevent_data);
} __packed;

struct bt_hci_cp_le_set_pawr_subevent_data {
	uint8_t adv_handle;
	uint8_t num_subevents;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_cp_le_set_pawr_subevent_data_element, subevents);
} __packed;
...
struct bt_hci_cp_le_set_pawr_response_data {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, response_data);
} __packed;
...
struct bt_hci_cp_le_set_pawr_sync_subevent {
	...
	uint8_t num_subevents;
	FLEXIBLE_ARRAY_DECLARE(uint8_t, subevents);
} __packed;
...
struct bt_hci_cp_le_rx_test_v3 {
	...
	FLEXIBLE_ARRAY_DECLARE(uint8_t, ant_ids);
} __packed;
...
struct bt_hci_cp_le_set_cig_params {
	...
	uint16_t p_latency;
	uint8_t  num_cis;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_cis_params, cis);
} __packed;
...
struct bt_hci_rp_le_set_cig_params {
	...
	uint8_t  cig_id;
	uint8_t  num_handles;
	FLEXIBLE_ARRAY_DECLARE(uint16_t, handle);
} __packed;
...
struct bt_hci_cp_le_set_cig_params_test {
	...
	uint8_t  framing;
	uint8_t  num_cis;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_cis_params_test, cis);
} __packed;
...
struct bt_hci_cp_le_create_cis {
	uint8_t  num_cis;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_cis, cis);
} __packed;
...
struct bt_hci_cp_le_big_create_sync {
	...
	uint16_t sync_timeout;
	uint8_t  num_bis;
	FLEXIBLE_ARRAY_DECLARE(uint8_t, bis);
} __packed;
...
struct bt_hci_evt_le_per_adv_response_report {
	...
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_evt_le_per_adv_response, responses);
} __packed;
...
struct bt_hci_evt_le_advertising_report {
	uint8_t num_reports;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_evt_le_advertising_info, adv_info);
} __packed;
...
struct bt_hci_evt_le_direct_adv_report {
	uint8_t num_reports;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_evt_le_direct_adv_info, direct_adv_info);
} __packed;
...
struct bt_hci_evt_le_ext_advertising_report {
	uint8_t num_reports;
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_evt_le_ext_advertising_info, adv_info);
} __packed;
...
struct bt_hci_evt_le_connectionless_iq_report {
	...
	FLEXIBLE_ARRAY_DECLARE(struct bt_hci_le_iq_sample, sample);
} __packed;
