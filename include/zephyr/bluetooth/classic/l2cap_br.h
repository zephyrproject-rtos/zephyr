 ...
 * It is registered with the help of the bt_l2cap_br_cls_register() API.
 * It's permissible to register multiple instances of this @ref bt_l2cap_br_echo_cb type, in case
 * different modules of an application are interested in tracking the connectionless data channel
 * frames. If a callback is not of interest for an instance, it may be set to NULL and
 * will as a consequence not be used for that instance.
 */
struct bt_l2cap_br_cls_cb {
	/** @brief Register PSM.
	 *
 ...
 * @retval 0 Success.
 * @retval -EINVAL If @p cb is NULL.
 * @retval -EINVAL If @p cb has invalid PSM value.
 * @retval -EINVAL If @p cb has invalid `recv` callback.
 * @retval -EINVAL If @p cb has invalid security level.
 ...
 */
#define BT_L2CAP_CLS_SDU_HDR_SIZE 2

/**
 *  @brief Headroom needed for outgoing L2CAP PDU format within a connectionless data channel.
 */
#define BT_L2CAP_CLS_RESERVE BT_L2CAP_BUF_SIZE(BT_L2CAP_CLS_SDU_HDR_SIZE)
 ...
 *
 * The application is required to have reserved @ref BT_L2CAP_CLS_RESERVE bytes
 ...
 *
 * @retval 0 Success.
 * @retval -EINVAL If @p conn or @p buf is NULL.
 * @retval -EINVAL If @p psm is invalid.
 ...
 * @return -EINVAL If the `buf->user_data_size` is less than the size of `struct closure`.
 */
