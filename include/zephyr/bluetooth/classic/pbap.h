 ...
 *
 *  @param pbap_pse PBAP PSE object, @ref bt_pbap_pse.
 *  @param rsp_code Response code (typically @ref BT_PBAP_RSP_CODE_SUCCESS), @ref
 * bt_pbap_rsp_code.
 *  @param buf Buffer containing optional response headers.
 *
 *  @return 0 on success, negative error code on failure.
 ...
 *  @param nonce Output buffer to store the generated nonce (@ref BT_OBEX_CHALLENGE_TAG_NONCE_LEN
 * bytes).
 *
 *  @return 0 on success, negative error code on failure.
 */
 ...
 *
 *  @param pwd Password string used for authentication (null-terminated).
 *  @param nonce Nonce value received in the authentication challenge (@ref
 * BT_OBEX_CHALLENGE_TAG_NONCE_LEN bytes).
 *  @param rsp_digest Output buffer to store the calculated response digest (@ref
 * BT_OBEX_RESPONSE_TAG_REQ_DIGEST_LEN bytes).
 *
 *  @return 0 on success, negative error code on failure.
 */
 ...
 *
 *  @param nonce Nonce value that was sent in the authentication challenge (@ref
 * BT_OBEX_CHALLENGE_TAG_NONCE_LEN bytes).
 *  @param rsp_digest Response digest received from the authenticating peer (@ref
 * BT_OBEX_RESPONSE_TAG_REQ_DIGEST_LEN bytes).
 *  @param pwd Password string used for authentication (null-terminated).
 *
