...
/** @brief Create an RFCOMM protocol data unit (PDU) buffer for SPP
 *
 * This function allocates a new RFCOMM protocol data unit (PDU) buffer
 * from the specified memory pool, reserving space for RFCOMM and SPP headers.
 * The returned buffer is ready for application data to be added.
 *
 * @note The function must only be called after the connection has been fully
 *       established (i.e., after the `connected` callback has been triggered).
 *       Calling this function on a partially established connection will result
 ...
 *
 * @param spp  Valid SPP connection object obtained after successful connection
 * @param pool Memory pool from which to allocate the PDU buffer
 *
 * @return Pointer to the allocated PDU buffer on success
 * @return NULL if:
 *         - spp is NULL
 *         - The underlying ACL connection is missing
 *         - Unable to allocate buffer from the specified pool
 */
