 ...
 * @param cb       Callback operations structure, @ref bt_opp_client_cb. Must not
 *                 be NULL, although every callback in it is optional.
 ...
 * Push Client. Every callback may be set to NULL, in which case the stack
 * responds automatically: @ref BT_OPP_RSP_CODE_NOT_IMPL for push() and
 * pull_bcard(), @ref BT_OPP_RSP_CODE_SUCCESS for connect(), disconnect() and
 * abort(). The structure must remain valid and constant for the lifetime of
 * the server.
 */
