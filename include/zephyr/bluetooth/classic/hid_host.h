...
struct bt_hid_host_cb {
	 ...
	 *                    when a DATA message is not received.
	 */
	 ...
	 *
	 * The remote HID Device has sent a VIRTUAL_CABLE_UNPLUG control message
	 * and the stack disconnects the HID association. HID spec v1.1.2 Section
	 * 3.1.2.2.3 also requires the bonding to be destroyed; this is left to the
	 * application because bt_br_unpair() would drop the whole ACL and with it
	 * any co-located profile.
	 *
	 * A control transaction that was still outstanding when the unplug arrived
	 * is discarded, so its result callback is never invoked: per Section
	 * 3.1.2.2.3 the recipient discards any pending control transfer. An
	 * application waiting for such a reply has to treat this callback as the
	 * outcome.
	 *
	 * @param hid HID Host instance.
	 */
 ...
 * @retval -EBUSY if an HID association is still active.
 * @retval -EALREADY if the HID Host callbacks are not registered.
 */
 ...
 *
 * @return 0 on success, -ENOTSUP if bt_hid_host_register() has not been called,
 ...
 *
 * Sends a VCU control message and waits for the HID Device to acknowledge it by
 * disconnecting the association, per HID spec v1.1.2 Section 3.1.2.2.3. If the
 * peer does not disconnect, the host starts a fallback teardown after the
 * transaction timeout. Destroying the bonding is left to the application, see
 * the vc_unplug callback.
 *
 * @param hid HID Host instance.
 *
 * @return 0 on success, negative errno on failure.
 ...
 *
 * The returned buffer already reserves headroom for the HID header, so the
 * profile can prepend it without copying. Callers only append the report
 * payload with net_buf_add_*() and must not consume the headroom themselves.
 *
