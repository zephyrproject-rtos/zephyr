...
struct bt_tbs_register_param {
	/** The name of the provide, for example a cellular service provider */
	char *provider_name;
 ...
 * @retval The bearer index if >= 0
 * @retval -EINVAL if @p param contains invalid data
 * @retval -ELAREADY if @p param.gtbs is true and GTBS has already been registered
 * @retval -EAGAIN if @p param.gtbs is false and GTBS has not been registered
 * @retval -ENOMEM if @p param.gtbs is false and no more TBS can be registered (see
 *         @kconfig{CONFIG_BT_TBS_BEARER_COUNT})
 * @retval -ENOEXEC if the service failed to be registered
 */
 ...
 *
 * @retval 0 on success
 * @retval -EINVAL if @p bearer_index is invalid
 * @retval -EALREADY if the bearer identified by @p bearer_index is not registered
 * @retval -ENOEXEC if the service failed to be unregistered
 */
