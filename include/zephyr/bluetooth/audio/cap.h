...
struct bt_cap_initiator_cb {
	 ...
	 * - -EAGAIN if operation failed due to internal errors.
	 *
	 * @param conn Pointer to the connection where the error occurred. NULL if @p err is 0,
	 *             if @p err is -EAGAIN or -ECANCELED.
	 */
 ...
 * @retval -EAGAIN Internal error; try again.
 */
...
struct bt_cap_commander_cb {
	 ...
	 * @param err		0 on success, BT_GATT_ERR() with a
	 *			specific ATT (BT_ATT_ERR_*) error code or -ECANCELED if cancelled
	 *			by bt_cap_commander_cancel() or -EAGAIN on internal errors.
	 */
