...
#else /* !CONFIG_BT_CONN || !CONFIG_BT_HCI_HOST */
...
/* TODO: Introduce Kconfig option to replace BT_BUF_HCI_ACL_RX_COUNT. For now, lets use minimum one
 * Rx buffer for each active ACL connection in addition to one each blocked unreferencing in the
 * Host per active ACL connection.
 *
 * We do not want the use of CONFIG_BT_BUF_EVT_RX_COUNT in the derivation of
 * BT_BUF_HCI_ACL_RX_COUNT.
 */
#define BT_BUF_RX_COUNT ((CONFIG_BT_MAX_CONN + 1U) + BT_BUF_ACL_RX_COUNT)
