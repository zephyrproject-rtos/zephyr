...
#if defined(CONFIG_POSIX_API) || defined(__DOXYGEN__)
...
#if defined(CONFIG_POSIX_SYNCHRONIZED_IO) || defined(__DOXYGEN__)
/**
...
/* stat(): documented in sys/stat.h */
int stat(const char *path, struct stat *buf);

/* mkdir(): documented in sys/stat.h */
 ...
 *
 * @param[out] buf Buffer to receive the null-terminated host name.
...
#if defined(CONFIG_POSIX_C_LIB_EXT) || defined(__DOXYGEN__)
/**
...
extern char *optarg; /**< Argument of the current option parsed by getopt(). */

extern int opterr;   /**< Error flag for getopt(); non-zero enables error messages. */

extern int optind;   /**< Index of the next element of argv to be processed by getopt(). */

extern int optopt;   /**< Unrecognized option character from getopt(). */
#endif
 ...
 * @return 0 on success, or -1 with errno set on failure.
 *
 * @deprecated Obsolescent in POSIX.1-2017; use nanosleep() instead.
 *
 * @posix_func{usleep}
 */
 ...
 * @param[out] buf Buffer to receive the null-terminated value, or NULL.
 * @param len      Size of @p buf in bytes.
 *
...
/* sysconf(): documented in sys/sysconf.h (macro form) and below (function form) */
...
/**
 * @brief Get an identifier for the current host (XSI extension).
 *
