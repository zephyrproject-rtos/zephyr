...
#define MAP_SHARED  0x1    /**< Changes are shared between all mappings of the same object. */

#define MAP_PRIVATE 0x2    /**< Changes are private (copy-on-write). */

#define MAP_FIXED   0x4    /**< Map at the exact address given to mmap(). */

/* for Linux compatibility */

#define MAP_ANONYMOUS 0x20 /**< Anonymous mapping; the file descriptor argument is ignored. */

#define MS_SYNC       0x0  /**< Flush modified pages to the underlying storage synchronously. */

#define MS_ASYNC      0x1  /**< Schedule writes and return immediately. */

#define MS_INVALIDATE 0x2  /**< Invalidate cached data so subsequent reads reflect the file. */

#define MAP_FAILED ((void *)-1) /**< Value returned by mmap() on failure. */

#define MCL_CURRENT 0      /**< Lock all currently mapped pages into memory. */

#define MCL_FUTURE  1      /**< Lock all future mappings into memory. */
 ...
 *
 * @param flags Bitwise OR of either or both of @c MCL_CURRENT and @c MCL_FUTURE.
 *
 * @return 0 on success, or -1 with errno set on failure.
 *
 * @posix_func{mlockall}
