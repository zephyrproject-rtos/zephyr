 ...
 *
 * @return New eventfd file descriptor on success, or -1 with errno set on failure.
 * @ingroup posix_extensions
 * @compat_api{eventfd}
 */
 ...
 * the counter is decremented by 1. If the counter is 0, the call blocks
 * unless the eventfd was created with @c EFD_NONBLOCK, in which case it
 * fails with @c errno set to @c EAGAIN.
 *
 ...
 *
 * Adds @p value to the counter and wakes any thread blocked in
 * eventfd_read(), read(), or poll() on @p fd. If the addition would
 * overflow the counter, the call blocks until a read occurs, unless the
 * eventfd was created with @c EFD_NONBLOCK, in which case it fails with
 * @c errno set to @c EAGAIN. Writing @c UINT64_MAX is invalid.
 *
 * @param fd    File descriptor.
