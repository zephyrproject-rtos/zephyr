...
typedef uint32_t socklen_t; /**< Type for socket address length values. */
struct msghdr;
struct sockaddr;

#define MSG_PEEK     ZSOCK_MSG_PEEK     /**< Peek at incoming data without removing it. */

#define MSG_TRUNC    ZSOCK_MSG_TRUNC    /**< Report the real datagram length even if truncated. */

#define MSG_DONTWAIT ZSOCK_MSG_DONTWAIT /**< Enable non-blocking operation for this call only. */

#define MSG_WAITALL  ZSOCK_MSG_WAITALL  /**< Block until all requested data has been received. */

#define SHUT_RD   ZSOCK_SHUT_RD         /**< Disables further receive operations. */

#define SHUT_WR   ZSOCK_SHUT_WR         /**< Disables further send operations. */

#define SHUT_RDWR ZSOCK_SHUT_RDWR       /**< Disables further send and receive operations. */
#endif
 ...
 * @param[out] addr       Address of the connecting peer, or NULL.
 * @param[in,out] addrlen Size of @p addr on input; actual address size on output.
 *
 ...
 *
 * @param sock            Socket file descriptor.
 * @param[out] addr       Local address bound to the socket.
 * @param[in,out] addrlen Size of @p addr on input; actual address size on output.
 *
 * @return 0 on success, or -1 with errno set on failure.
 *
 ...
 * @param[in,out] optlen Size of @p optval on input; actual size on output.
 *
 * @return 0 on success, or -1 with errno set on failure.
 *
 ...
 * @param[out] src_addr   Sender's address, or NULL.
 * @param[in,out] addrlen Size of @p src_addr on input; actual address size on output.
 *
 ...
 * @param[out] sv Two-element array receiving the socket descriptors.
 *
 * @return 0 on success, or -1 with errno set on failure.
 *
