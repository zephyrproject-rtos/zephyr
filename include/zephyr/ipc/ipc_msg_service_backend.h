...
struct ipc_msg_service_backend {
	...
	/**
	 * @brief Pointer to the function that will be used to query endpoint status.
	 *
	 * @param[in] instance Instance pointer.
	 ...
	 */
	int (*query)(const struct device *instance, void *token, uint8_t query_type,
	 ...
	 * @retval other errno codes depending on the implementation of the backend.
	 */
	int (*send)(const struct device *instance, void *token, uint8_t msg_type,
		    const void *msg_data);
