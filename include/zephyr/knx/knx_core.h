...
#define KNX_ADDR_VAL(addr) \
	(unsigned int)(((addr) >> 12) & 0xF), \
	(unsigned int)(((addr) >> 8) & 0xF), \
 ...
 */
#define KNX_ADDR_TYPE_VAL(type) \
	((type) == KNX_ADDR_TYPE_INDIVIDUAL ? "Individual" : "Group")
 ...
 */
#define KNX_FRAME_VAL(format) \
	((format) == KNX_FRAME_FORMAT_EXTENDED ? "Extended" : "Standard")
...
#define KNX_PRIORITY_VAL(prio) \
	((prio) == KNX_PRIORITY_SYSTEM ? "System" : \
	 ...
	 (prio) == KNX_PRIORITY_REPEATED ? "Repeated" : "Unknown")
...
#define KNX_GROUP_VAL(addr) \
	(unsigned int)(((addr) >> 11) & 0x1F), \
	(unsigned int)(((addr) >> 8) & 0x7), \
	(unsigned int)((addr) & 0xFF)
 ...
 * @endcode
 */
#define KNX_ADDR_AUTO_FMT(type) \
	((type) == KNX_ADDR_TYPE_INDIVIDUAL ? KNX_ADDR_FMT : KNX_GROUP_FMT)
