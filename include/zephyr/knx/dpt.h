 ...
 *
 * KNX Datapoint Type (DPT) encode/decode API.
 *
 * Enabled by CONFIG_KNX_DPT=y.  All DPT families are built by default;
 * individual families can be excluded via CONFIG_KNX_DPT_DISABLE_<n>.
 */
...
void     uint8_to_payload(uint8_t *payload, size_t payload_length, int index,
			   uint8_t value, uint8_t mask);
void     int16_to_payload(uint8_t *payload, size_t payload_length, int index,
			   int16_t value, uint16_t mask);
void     float16_to_payload(uint8_t *payload, size_t payload_length, int index,
...
#ifdef ENABLE_DPT_2
typedef struct { bool control; bool value; } dpt2_t;
uint8_t dpt2_encode(dpt2_t data, uint8_t *payload);
dpt2_t  dpt2_decode(const uint8_t *payload);
#endif

#ifdef ENABLE_DPT_3
typedef struct { bool control; uint8_t step_code; } dpt3_t;
...
#ifdef ENABLE_DPT_10
typedef struct { uint8_t day; uint8_t hour; uint8_t minute; uint8_t second; } dpt10_t;
...
#ifdef ENABLE_DPT_11
typedef struct { uint8_t day; uint8_t month; uint8_t year; } dpt11_t;
...
#ifdef ENABLE_DPT_18
typedef struct { uint8_t scene; bool learn; } dpt18_t;
...
typedef struct { uint8_t red; uint8_t green; uint8_t blue; } dpt232_t;
