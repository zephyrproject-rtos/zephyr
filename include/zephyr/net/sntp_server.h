...
/**
 * @brief Set SNTP time source id, stratum and precision
 *
 * Until this function is called, the server answers queries with the
 * leap indicator set to "clock not synchronized" and stratum 16, telling
 * clients that its time must not be used. Call it after the system clock
 * has been set to a trusted time.
 *
