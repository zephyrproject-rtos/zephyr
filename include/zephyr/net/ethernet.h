...
/* The L2 multicast addresses are tracked only if there is a multicast
 * monitor to feed the list and a receive filter to program.
 */
#if defined(CONFIG_NET_L2_ETHERNET) && defined(NET_IF_MCAST_MONITOR_SUPPORTED) && \
	!defined(CONFIG_NET_RAW_MODE)
 ...
 * address is joined by its first user.
 *
