...
#define Z_NET_ETH_MAC_DEV_CONFIG_NAME(node_id)                                                     \
	_CONCAT(net_eth_mac_dev_config_, DEVICE_DT_NAME_GET(node_id))

/**
 * @brief Obtain the type of the ethernet MAC config for the given DT node identifier.
 *
 * @param node_id Node identifier.
 */
...
/**
 * @brief Create a MAC address configuration struct for the given DT node identifier
 *
 * @param node_id Node identifier.
 */
...
/**
 * @brief Like NET_ETH_MAC_DT_DEFINE for an instance of a DT_DRV_COMPAT
 * compatible
 *
