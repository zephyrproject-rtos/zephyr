...
#define PROMETHEUS_HISTOGRAM_DEFINE(_name, _detail)                                                \
	static STRUCT_SECTION_ITERABLE_ALTERNATE(prometheus_metric, prometheus_histogram,          \
						 _name) = {.base = (void *)(_detail),              \
							   .buckets = NULL,                        \
							   .num_buckets = 0,                       \
							   .sum = 0,                               \
							   .count = 0}
 ...
 */
void prometheus_histogram_observe(struct prometheus_histogram *histogram, double value);
