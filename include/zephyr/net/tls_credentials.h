 ...
 *
 * @retval 0 private key successfully generated.
 * @retval -EEXIST There is already a private key installed at the requested security tag.
 * @retval -EFAULT Key generation failed.
 * @retval -EFBIG Key does not fit in the buffer provided.
 * @retval -EINVAL Invalid input.
 * @retval -ENOTSUP The TLS credentials backend doesn't support the requested keygen operation.
 */
 ...
 * @param type	 The type of public/private key-pair to generate. Set to
 * 		 TLS_CREDENTIAL_KEYGEN_DEFAULT to use the default type for the credentials
 * 		 back-end. Set to TLS_CREDENTIAL_KEYGEN_EXISTING to instead use a pre-existing
 * 		 private key.
 ...
 * @retval -EINVAL Invalid input.
 * @retval -EEXIST Keygen was requested, but there is already a private key installed at the
 *		   requested security tag.
 * @retval -ENOENT Use of existing key was requested, but a private key does not exist at the\
 * 		   requested security tag.
 * @retval -EFBIG Key or CSR does not fit in the buffer provided.
 * @retval -EFAULT CSR or key generation failed. See logs.
 * @retval -ENOTSUP The TLS credentials backend doesn't support the requested keygen or CSR
 * 		    operation.
 */
