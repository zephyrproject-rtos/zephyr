...
/** Maximum number of bytes @ref srtp_stream_protect appends to a packet: the
 *  authentication tag plus the MKI, when configured.
 */
#define SRTP_TRAILER_MAX_LEN (SRTP_AES_GCM_TAG_LEN + SRTP_MKI_MAX_LEN)
 ...
 * with @ref srtp_stream_deinit. @ref srtp_stream_protect and @ref srtp_stream_unprotect
 * serialize on an internal lock; the strictly increasing transmit order
 * required by @ref srtp_stream_protect must be ensured by the caller.
 */
struct srtp_stream {
	/** @cond INTERNAL_HIDDEN */
	uint32_t ssrc;
 ...
 * @ref SRTP_TRAILER_MAX_LEN bytes of growth beyond @p pkt_len.
 *
 ...
 * is released again, so unauthenticated traffic cannot pin the slots, though
 * each spoofed source still costs one session key derivation.
 */
struct srtp_session_ctx {
	...
	/** Packets currently being unprotected against each stream. */
	uint8_t rx_users[CONFIG_SRTP_MAX_RX_STREAMS];
	bool rx_enabled;
	...
	uint8_t rx_master_key[SRTP_AES_256_KEY_LEN];
	uint8_t rx_master_salt[SRTP_MASTER_SALT_LEN];
	uint8_t rx_mki[SRTP_MKI_MAX_LEN];

	/** Protects the receive stream binding table. */
	struct k_mutex lock;
	/** Signals a receive stream hold count reaching zero while
	 *  rtp_session_clear_srtp() drains in-flight packets.
	 */
	struct k_condvar rx_drained;
