 ...
 */
typedef void (*coap_client_tcp_response_cb_t)(const struct coap_client_response_data *data,
					      void *user_data);
 ...
 * @brief Callback to configure the socket before connecting.
 *
 * Called after socket creation but before connect, allowing the application
 * to set socket options (e.g. TLS credentials).
 *
...
/**
 * @brief Connect the CoAP TCP client to a server.
 *
 ...
 * @param proto Protocol to use (IPPROTO_TCP or IPPROTO_TLS_1_2).
 *
 * @return 0 on success, negative error code otherwise.
 */
...
/**
 * @brief Close the CoAP TCP client connection.
 *
 * Cancels any ongoing requests and closes the underlying socket.
 * For graceful shutdown, send a Release signal with
 * @ref coap_client_tcp_release before calling this.
 *
 * @param client Client instance.
 *
 * @return 0 on success, negative error code otherwise.
