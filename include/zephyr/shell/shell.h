 ...
 * @param[out] bytes_to_read	Maximum number of data bytes to receive.
 * @param[in]  bytes_read	Address of area to hold the number of bytes
 * 				received.
 * @param[in]  min_xfer		Minimum number of data bytes to receive.
 * @param[in]  timeout		Waiting period to wait for the data to be
 * 				received, or one of the special values K_NO_WAIT
 * 				and K_FOREVER.
 *
 * @retval 0			At least @a min_xfer bytes of data were
 * 				received.
 * @retval -EPERM		Invoked from invalid thread.
 * @retval -EINVAL		Invalid parameters supplied.
 * @retval -EIO			Returned without waiting; zero data bytes were
 * 				received.
 * @retval -EAGAIN		Waiting period timed out; between zero and
 * 				@a min_xfer minus one data bytes were received.
 */
int shell_bypass_receive(struct shell_bypass *shb, void *data,
			 size_t bytes_to_receive, size_t *bytes_received,
 ...
 * @param[in]  bytes_to_send 	Size of data (in bytes).
 * @param[out] bytes_sent	Address of area to hold the number of bytes
 * 				sent.
 * @param[in]  min_xfer		Minimum number of bytes to write.
 * @param[in]  timeout		Waiting period to wait for the data to be
 * 				written, or one of the special values K_NO_WAIT
 * 				and K_FOREVER.
 *
 * @retval 0			At least min_xfer bytes of data were sent.
 * @retval -EPERM		Invoked from invalid thread.
 * @retval -EIO			Returned without waiting; zero data bytes were
 * 				sent.
 * @retval -EAGAIN		Waiting period timed out; between zero and
 * 				min_xfer minus one data bytes were sent.
 */
int shell_bypass_send(struct shell_bypass *shb, const void *data,
		      size_t bytes_to_send, size_t *bytes_sent,
		      size_t min_xfer, k_timeout_t timeout);
