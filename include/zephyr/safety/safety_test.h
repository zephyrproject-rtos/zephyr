...
/**
 * @brief Report whether logging is usable at a given init level.
 *
 * Below SAFETY_TEST_LEVEL_PRE_KERNEL_2 the system timer has not started, and the log
 * core timestamps every message from it, so a log call there faults. Test functions,
 * failure hooks and safe-state handlers are all handed their init level and should
 * ask this before logging.
 *
 * @param level Init level, from @ref safety_test_context.init_level.
 * @return true when a log call is safe.
 */
static inline bool safety_test_can_log(enum safety_test_init_level level)
{
	return level >= SAFETY_TEST_LEVEL_PRE_KERNEL_2;
}
 ...
 * follows the same convention, so wrapping one is a single line that calls it and
 ...
 *
 * @p ctx reports the level the test ran at, which decides what a hook may do. Below
 * SAFETY_TEST_LEVEL_PRE_KERNEL_2 it must not log: ask safety_test_can_log() first. No
 * driver has initialised before SAFETY_TEST_LEVEL_POST_KERNEL either, so check any
 * device with device_is_ready() rather than assuming.
 *
 ...
 * any level.
 */
typedef void (*safety_test_failure_fn)(const struct safety_test_context *ctx,
...
#define Z_SAFETY_TEST_LVL_EARLY_SAFETY_TEST_LEVEL_EARLY               1
#define Z_SAFETY_TEST_LVL_PRE_KERNEL_1_SAFETY_TEST_LEVEL_PRE_KERNEL_1 1
#define Z_SAFETY_TEST_LVL_PRE_KERNEL_2_SAFETY_TEST_LEVEL_PRE_KERNEL_2 1
#define Z_SAFETY_TEST_LVL_POST_KERNEL_SAFETY_TEST_LEVEL_POST_KERNEL   1
#define Z_SAFETY_TEST_LVL_APPLICATION_SAFETY_TEST_LEVEL_APPLICATION   1

/* Anything but a SAFETY_TEST_LEVEL_* name is a compile error. */
#define Z_SAFETY_TEST_LEVEL_ORD(_level)                                                            \
	COND_CASE_1(Z_SAFETY_TEST_LVL_EARLY_##_level, (0),                                         \
		    Z_SAFETY_TEST_LVL_PRE_KERNEL_1_##_level, (1),                                  \
		    Z_SAFETY_TEST_LVL_PRE_KERNEL_2_##_level, (2),                                  \
		    Z_SAFETY_TEST_LVL_POST_KERNEL_##_level, (3),                                   \
		    Z_SAFETY_TEST_LVL_APPLICATION_##_level, (4),                                   \
		    (ZERO_OR_COMPILE_ERROR(0)))

#define Z_SAFETY_TEST_LEVEL_WORD(_level)                                                           \
	COND_CASE_1(Z_SAFETY_TEST_LVL_EARLY_##_level, (EARLY),                                     \
		    Z_SAFETY_TEST_LVL_PRE_KERNEL_1_##_level, (PRE_KERNEL_1),                       \
		    Z_SAFETY_TEST_LVL_PRE_KERNEL_2_##_level, (PRE_KERNEL_2),                       \
		    Z_SAFETY_TEST_LVL_POST_KERNEL_##_level, (POST_KERNEL),                         \
		    Z_SAFETY_TEST_LVL_APPLICATION_##_level, (APPLICATION),                         \
		    (UNDEFINED))

/* The linker sorts priorities as text, so only plain decimal numbers order correctly. */
#define Z_SAFETY_TEST_PRIO_DIGITS(_prio) ((_prio) < 10 ? 1 : ((_prio) < 100 ? 2 : 3))

/* Must match CREATE_OBJ_LEVEL in the linker script. */
#define Z_SAFETY_TEST_SECTION(_level, _prio)                                                       \
	__attribute__((__section__(".z_safety_test_" STRINGIFY(Z_SAFETY_TEST_LEVEL_WORD(_level))   \
					   "_P_" STRINGIFY(_prio) "_")))
/** @endcond */
 ...
 * @param _prio Order within the level, 0-255, lower first. Must be a plain decimal number.
...
#define SAFETY_TEST_DEFINE_EX(_name, _cat, _level, _prio, _flags, _fn, _desc, _user_data,          \
	...
	BUILD_ASSERT(Z_SAFETY_TEST_LEVEL_ORD(_level) == (_level),                                  \
		     "safety test: init level must be a SAFETY_TEST_LEVEL_* name");                \
	BUILD_ASSERT((unsigned int)(_level) <= SAFETY_TEST_LEVEL_APPLICATION,                      \
		     "safety test: init level out of range");                                      \
	BUILD_ASSERT(sizeof(STRINGIFY(_prio)) - 1 == Z_SAFETY_TEST_PRIO_DIGITS(_prio),             \
		     "safety test: priority must be a plain decimal number");                      \
	...
	Z_DECL_ALIGN(const struct safety_test) Z_SAFETY_TEST_NAME(_name)                           \
		Z_SAFETY_TEST_SECTION(_level, _prio) __used __noasan = {                           \
 ...
 * @kconfig{CONFIG_SAFETY_TEST_DEFAULT_ACTION_HALT},
 * @kconfig{CONFIG_SAFETY_TEST_DEFAULT_ACTION_RESET} or
 * @kconfig{CONFIG_SAFETY_TEST_DEFAULT_ACTION_CONTINUE}.
 *
 ...
 * @p ctx reports the level, which decides what it may do: below
 * SAFETY_TEST_LEVEL_PRE_KERNEL_2 it must not log, so ask safety_test_can_log() first.
 *
 ...
 *
 * @param ctx Execution context of the failed test.
