...
#ifdef PKTBUF_STORAGE_ALIGN
/**
 * @brief Define libmctp TX packet-buffer storage for one binding instance.
 *
 * @param _name Binding instance name.
 */
#define MCTP_I2C_SMBUS_TARGET_TX_STORAGE_DEFINE(_name)		\
	PKTBUF_STORAGE_ALIGN uint8_t _name##_tx_storage[MCTP_PKTBUF_SIZE( \
		MCTP_I2C_SMBUS_MAX_MCTP_BYTES)]

/**
 * @brief Initialize libmctp TX packet-buffer storage for one instance.
 *
 * @param _name Binding instance name.
 */
#define MCTP_I2C_SMBUS_TARGET_TX_STORAGE_INIT(_name)	\
	.tx_storage = _name##_tx_storage,
#else
/**
 * @brief Define libmctp TX packet-buffer storage for one binding instance.
 *
 * This is empty when the libmctp version in use does not expose
...
#define MCTP_I2C_SMBUS_TARGET_DT_DEFINE(_name, _node_id)		\
	...
	MCTP_I2C_SMBUS_TARGET_TX_STORAGE_DEFINE(_name);			\
	static struct mctp_binding_i2c_smbus_target _name = {		\
		.binding = {						\
			...
			MCTP_I2C_SMBUS_TARGET_TX_STORAGE_INIT(_name)	\
		},							\
