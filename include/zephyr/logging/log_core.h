...
#define Z_LOG_LEVEL_ALL_CHECK(_level, _inst, _source)                                              \
	(Z_LOG_CONST_LEVEL_CHECK(_level) &&                                                        \
	 ...
	 COND_CODE_0(CONFIG_USERSPACE, (Z_LOG_DYNAMIC_LEVEL_CHECK(_level, _source)), (true))       \
	)
...
#define Z_LOG_LEVEL_ALL_CHECK_BREAK(_level, _inst, _source)                                        \
	bool dynamic_level_check = true;                                                           \
	IF_ENABLED(CONFIG_USERSPACE,                                                               \
		(if (!k_is_user_context()) {                                                       \
			compiler_barrier();                                                        \
			dynamic_level_check = Z_LOG_DYNAMIC_LEVEL_CHECK((_level), (_source));      \
		})                                                                                 \
	)                                                                                          \
	if (!Z_LOG_LEVEL_ALL_CHECK((_level), (_inst), (_source)) || !dynamic_level_check) {        \
		break;                                                                             \
	}
