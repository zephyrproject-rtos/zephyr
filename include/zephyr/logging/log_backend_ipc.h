 ...
 * @ingroup log_backend_ipc
 */

#ifndef ZEPHYR_INCLUDE_LOGGING_LOG_BACKEND_IPC_H_
#define ZEPHYR_INCLUDE_LOGGING_LOG_BACKEND_IPC_H_
...
/**
 * @defgroup log_backend_ipc IPC service log backend
 * @ingroup log_backend
...
struct log_backend_ipc_data {
	...
	struct log_ipc_service_msg *pkt; /**< Packet being filled in, NULL if there is none. */
	size_t pkt_len;                  /**< Number of payload bytes written to the packet. */
	size_t pkt_cap;                  /**< Payload capacity of the packet. */
#endif
	int status;       /**< Unused. */
	bool panic;       /**< @c true after panic mode is entered on this backend. */
	bool ready;       /**< @c true once bound and the READY message was sent. */
	bool initialized; /**< @c true once the backend is initialized. */
};

/**
 * @brief Static configuration of an IPC service logging backend instance.
 */
struct log_backend_ipc_config {
	const struct device *ipc_instance; /**< IPC service instance device. */
	struct log_backend_ipc_data *data; /**< Runtime state for this instance. */
 ...
 *
 * @param priv Pointer to @ref log_backend_ipc_config (endpoint private data).
 */
...
extern const struct log_backend_api log_backend_ipc_api;
 ...
 */
#define LOG_BACKEND_IPC_DEFINE(_name, ipc_node)                                                    \
	static struct log_backend_ipc_data _name##_data;                                           \
	static const struct log_backend_ipc_config _name##_config;                                 \
	static const struct log_backend_ipc_config _name##_config = {                              \
		...
		.ept_cfg =                                                                         \
			{                                                                          \
				...
				.cb =                                                              \
					{                                                          \
						.bound = log_backend_ipc_bound_cb,                 \
						.received = log_backend_ipc_recv_cb,               \
					},                                                         \
				.priv = (void *)&_name##_config,                                   \
			},                                                                         \
	};                                                                                         \
	LOG_BACKEND_DEFINE(_name, log_backend_ipc_api, true, (void *)&_name##_config)
...
#endif /* ZEPHYR_INCLUDE_LOGGING_LOG_BACKEND_IPC_H_ */
