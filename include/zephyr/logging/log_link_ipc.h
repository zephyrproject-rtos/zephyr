 ...
 * @ingroup log_link_ipc
 */

#ifndef ZEPHYR_INCLUDE_LOGGING_LOG_LINK_IPC_H_
#define ZEPHYR_INCLUDE_LOGGING_LOG_LINK_IPC_H_
...
/**
 * @defgroup log_link_ipc IPC service log link
 * @ingroup log_link
...
#define LOG_LINK_IPC_MPSC_BUFFER_DEFINE(_name, _buffer_size)                                       \
	...
	static const struct mpsc_pbuf_buffer_config mpsc_config_##_name = {                        \
		...
		.flags =                                                                           \
			(IS_ENABLED(CONFIG_LOG_MODE_OVERFLOW) ? MPSC_PBUF_MODE_OVERWRITE : 0) |    \
			(IS_ENABLED(CONFIG_LOG_MEM_UTILIZATION) ? MPSC_PBUF_MAX_UTILIZATION : 0)}; \
 ...
 *                     used when @kconfig{CONFIG_LOG_LINK_IPC_RX_HOLD} is
...
#define LOG_LINK_IPC_DEFINE(_name, _ipc_node, _buffer_size, _mem_access)                           \
	...
	COND_CODE_1(CONFIG_LOG_LINK_IPC_RX_HOLD, \
		(SPSC_DEFINE(msg_buffer_##_name, struct log_link_ipc_msg_buffer, \
			CONFIG_LOG_LINK_IPC_RX_HOLD_QUEUE_SIZE)), \
	...
	static const struct log_link_ipc log_link_ipc_##_name = {                                  \
		...
		.ipc_instance = DEVICE_DT_GET(_ipc_node),                                          \
		.link = &_name,                                                                    \
...
#endif /* ZEPHYR_INCLUDE_LOGGING_LOG_LINK_IPC_H_ */
