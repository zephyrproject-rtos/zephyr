...
#if defined(CONFIG_ZBUS_MSG_BUF_POOL_ISOLATION) || defined(__DOXYGEN__)
	/** Net buf pool for message subscribers and async listeners. It can be either the global or
	 * a separated one.
	 */
	struct net_buf_pool *msg_buf_pool;
#endif /* ZBUS_MSG_BUF_POOL_ISOLATION */
...
#if defined(CONFIG_ZBUS_ASYNC_LISTENER) || defined(__DOXGEN__)
...
struct zbus_async_listener_work {
	struct k_work work;
	struct k_fifo *message_fifo;
	/** Queue to be used in during the publication process */
 ...
 * async listener work item that will handle a FIFO where it will receive the message asynchronously
 ...
 *
 * @param obs The async listener observer's reference.
 * @param queue The work queue to be used for executing the async listener.
 *
...
static inline int zbus_async_listener_set_work_queue(const struct zbus_observer *obs,
						     struct k_work_q *queue)
{
	__ASSERT(obs != NULL, "obs required");
	__ASSERT(obs->type == ZBUS_OBSERVER_ASYNC_LISTENER_TYPE, "async listener required");
	__ASSERT(queue != NULL, "queue required");
					      ...
					      struct net_buf_pool *pool)
{
	...
	chan->data->msg_buf_pool = pool;
}

#endif /* CONFIG_ZBUS_MSG_BUF_POOL_ISOLATION */
