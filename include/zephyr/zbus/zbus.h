...
#define _ZBUS_CHAN_DEFINE(_name, _id, _type, _validator, _user_data)                               \
	...
	_ZBUS_CPP_EXTERN const STRUCT_SECTION_ITERABLE(zbus_channel, _name) = {                    \
		...
		IF_ENABLED(CONFIG_ZBUS_CHANNEL_ID, (.id = _id,)) .message =                           \
							   &_ZBUS_MESSAGE_NAME(_name),             \
			   .message_size = sizeof(_type), .user_data = _user_data,                 \
			   .validator = _validator, .data = &_CONCAT(_zbus_chan_data_, _name),     \
 ...
 * @param _init_val The message initialization.
 */
#define ZBUS_CHAN_ID_DEFINE(_name, _id, _type, _validator, _user_data, _observers, _init_val)      \
	static _type _ZBUS_MESSAGE_NAME(_name) = _init_val;                                        \
 ...
 *
 * @param channel_id Unique channel ID from @ref ZBUS_CHAN_ID_DEFINE
 *
