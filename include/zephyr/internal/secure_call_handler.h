...
#if defined(CONFIG_ARM_SECURE_FIRMWARE) || defined(__DOXYGEN__)
 ...
 *
 * Delegates to the arch backend (arch_secure_call_buffer_accessible()) to check
 * that the entire range is Non-Secure and readable from the current privilege
 * level.
 *
 * @ingroup secure_call
 */
#define Z_SECURE_MEMORY_READ(ptr, size)                                                            \
	Z_SECURE_VERIFY(arch_secure_call_buffer_accessible((const void *)(uintptr_t)(ptr), (size), \
							   false))
 ...
 *
 * Delegates to the arch backend (arch_secure_call_buffer_accessible()) to check
 * that the entire range is Non-Secure and writable from the current privilege
 * level.
 *
 * @ingroup secure_call
 */
