...
/**
 * @file
 * @brief Public API for the queue-centric work queue
 * @ingroup workq_apis
 */
...
/**
 * @brief A work item to be executed by a work queue.
 *
...
/**
 * @brief A queue-centric work queue.
 *
 * All the members are internal and should not be accessed directly.
 */
struct workq {
 ...
 *
 * Sets the queue to the frozen state and aborts the delayed-work timeout so
 * delayed items are not promoted to pending until the queue is thawed. This is
 * orthogonal to open/close: freezing does not reject new submissions.
 *
 ...
 *
 * Clears the frozen state and reschedules the delayed-work timeout. This is
 * orthogonal to open/close and does not change whether the queue accepts new
 * submissions.
 *
...
/**
 * @brief Statically initialize a work queue in the open state.
 *
 * @param obj Name of the @ref workq object being initialized.
 */
 ...
 *
 * @param name Name of the @ref workq object.
 */
 ...
 *
 * The configuration is not copied: @p cfg is applied to the thread when it is
 * started and is read again on every restart. It must therefore remain valid
 * for the entire lifetime of the thread. Pass NULL to use the built-in
 * defaults (no name, default preemptible priority).
 *
...
/** @brief Statically initialize a work queue thread
 *
