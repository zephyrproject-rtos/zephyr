 ...
 * the system resets on wake-up, so this normally does not return. A wake-up
 * source must be configured; @p timeout is propagated to SoC backends that
 * support a timed wake.
 *
 * @param timeout Maximum time to remain off (@ref K_FOREVER for no limit).
 *
 * @retval -ENOTSUP No soft-off state is declared (otherwise does not return).
 */
