 ...
 * @details When resolving a phandle symbol from @p node_id, for
 *          a form like `label : &other {};`, it builds a label
 *          list `[other_label1, other_label2, label, ...]` and
 *          uses @p idx to get the original label.
 *
 * Example devicetree fragment:
 *
 * @code{.dts}
 ...
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 1), 0) // n3
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 0), 1) // n2_1
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 1), 1) // n3_1
 * @endcode
 *
