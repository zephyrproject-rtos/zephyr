 ...
 *
 * The index @p idx for labels is ordered from right to left.
 *
 * Example devicetree fragment:
 *
 * @code{.dts}
 ...
 *
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 0), 0) // n2_1
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 1), 0) // n3_1
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 0), 1) // n2
 *     DT_NODELABEL_C_TOKEN_BY_IDX(DT_PHANDLE_BY_IDX(N1, foo, 1), 1) // n3
 * @endcode
 *
 * @param node_id node identifier
 * @param idx index into @p node_id
 * @return C symbolic name of the node for the given index
 * @note The @p idx retrieves node labels in right-to-left order, as shown
 *       in the example.
 */
