 ...
 */
#define DT_ROMSTART_REGION DT_STRING_TOKEN(DT_CHOSEN(zephyr_romstart), zephyr_memory_region)
...
#error "The chosen romstart property references a node that doesn't have a 'zephyr,memory-region'" \
	"compatible string and property to create a new linker script memory region"
#endif
