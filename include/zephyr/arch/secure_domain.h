 ...
 * (trusted) code can hand control to. arch_secure_domain_swap() applies the
 * domain's entry setup and transfers control into it.
 *
 * This is distinct from the address-attribution API in
 * <zephyr/arch/security_partition.h>: that decides which addresses belong to
 * which domain; this enters a domain. Bus-level access filters are a separate
 * concern.
 *
 * The hand-off is one-way: arch_secure_domain_swap() does not return. Only
 * meaningful from the trusted world.
 */
 ...
 * transfers control to @p domain->entry.
 *
 * All isolation setup must be complete before calling; the hand-off is one-way
 * and the caller does not run again. Only meaningful from the trusted world.
 *
...
/*
 * Backend hooks used by arch_secure_domain_swap(). These are weak so a backend
 * can override the entry sequence for targets that need a non-default one.
 * Application code should call arch_secure_domain_swap(), not these directly.
 */
