 ...
 * trusted and untrusted regions. Each arch backend maps this API onto its
 * own attribution unit; architectures without one provide no-op stubs that
 * return -ENOTSUP.
 *
 * This API covers only the CPU-side attribution unit. Bus-level filters are
 * a separate concern and are not abstracted here.
 *
...
enum arch_security_attr {
	 ...
	 * Where Secure is the hardware default for unattributed addresses,
	 * listing a region as S may be a no-op.
	 */
	 ...
	 * the CPU switches to the trusted world at the designated entry point.
	 * Used for the veneer region that holds the __secure_call entry stubs.
	 *
...
struct arch_security_region {
	/** Region base address. Must satisfy the arch alignment requirements. */
	uintptr_t base;
 ...
 *
 * Configures the arch security attribution unit with the supplied region
 * descriptors. Address ranges not covered by any region default to Secure
 * after arch_security_partition_enable() is called.
 *
 * Regions are installed in the order supplied; on hardware with explicit
 * region priority the first matching region wins.
 *
 ...
 *
 * The attribution unit is bypassed. No-op on architectures without
