 ...
 *
 * @code{.c}
 * static int r_cb(void *const priv, const uint32_t block, const uint16_t size,
 *                 uint8_t buf[static CONFIG_USBD_DFU_TRANSFER_SIZE])
 * {
 *         int len;
 *
 ...
 * }
 *
 * static int w_cb(void *const priv, const uint32_t block, const uint16_t size,
 *                 const uint8_t buf[static CONFIG_USBD_DFU_TRANSFER_SIZE])
 * {
 *         return 0;
 * }
 *
 * static bool next_cb(void *const priv,
 *                     const enum usb_dfu_state state, const enum usb_dfu_state next)
 * {
 *         return true;
 * }
 ...
 * @param ir_cb Image write callback
 * @param iw_cb Image read callback
 */
#define USBD_DFU_DEFINE_IMG(id, iname, ipriv, ir_cb, iw_cb, inext)			\
