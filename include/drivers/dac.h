 ...
 * continuos mode.
 * See dac_start_continious() for argument descriptions.
 */
typedef int (*dac_api_start_continious)(const struct device *dev,
					uint8_t channel);
...
__subsystem struct dac_driver_api {
	...
	dac_api_start_continious start_continious;
 ...
 *
 * @retval 0         On success.
 * @retval -EINVAL   If a parameter with an invalid value haTypes of events passed to callback in UART_ASYNC_API
 */
...
/**
 * @brief Start DAC in continious mode.
 * 
 * This will start the dac driver to send continiously the samples
 ...
 * @retval 0	    If successful, negative errno code otherwise.
 */
static inline int dac_start_continious(const struct device *dev,
					uint8_t channel)
{
#ifdef CONFIG_DAC_CONTINIOUS_API
	const struct dac_driver_api *api =
	...
	return api->start_continious(dev, channel);
#else
	return -ENOTSUP;
#endif
}
