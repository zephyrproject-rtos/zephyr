...
#ifndef ZEPHYR_INCLUDE_DRIVERS_SIP_SVC_PROTO_H_
#define ZEPHYR_INCLUDE_DRIVERS_SIP_SVC_PROTO_H_

/**
 * @file
 * @brief Arm SiP services driver communication protocol
 ...
 *
 * Driver to fill in the SMC/HVC return value in struct sip_svc_response
 ...
 *     - Typical flow, synchronous request. Driver expects EL3/EL2 firmware to
...
/** @brief SiP Services driver communication protocol
 *         request format.
 *
 * request header
 *     - bits [15: 0] Arm SiP services command code
 *     - bits [23:16] Transaction ID (Filled in by sip_svc driver)
 ...
 *     - Dynamic memory address for driver to put the asynchronous response
 *       data. The driver will free this memory space if the client has
 ...
 *
 * priv_data
 *     - Memory address to client context. Driver will pass this address back
 ...
 *       of a asynchronous command transacation.
 *
 * resp_data_size
 *     - This parameter only used by asynchronous command.
...
#endif /* ZEPHYR_INCLUDE_DRIVERS_SIP_SVC_PROTO_H_ */
