...
__subsystem struct gpio_driver_api {
	...
	int (*manage_callback)(const struct device *port,
			       struct gpio_callback *cb,
			       bool set);
	int (*enable_callback)(const struct device *port, gpio_pin_t pin);
	int (*disable_callback)(const struct device *port, gpio_pin_t pin);
	u32_t (*get_pending_int)(const struct device *dev);
};
...
__syscall int gpio_enable_callback(const struct device *port, gpio_pin_t pin);

static inline int z_impl_gpio_enable_callback(const struct device *port,
					      gpio_pin_t pin)
{
	const struct gpio_driver_api *api =
		(const struct gpio_driver_api *)port->driver_api;
...
__syscall int gpio_disable_callback(const struct device *port, gpio_pin_t pin);

static inline int z_impl_gpio_disable_callback(const struct device *port,
					       gpio_pin_t pin)
{
	const struct gpio_driver_api *api =
		(const struct gpio_driver_api *)port->driver_api;
 ...
 */
/* Deprecated in 2.2 release */
__deprecated static inline int gpio_pin_write(const struct device *port,
					      gpio_pin_t pin,
					      u32_t value)
{
 ...
 */
/* Deprecated in 2.2 release */
__deprecated static inline int gpio_pin_read(const struct device *port,
					     gpio_pin_t pin,
					     u32_t *value)
{
