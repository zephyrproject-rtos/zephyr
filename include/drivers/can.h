 ...
 *
 * The message queue's ring buffer contains space for @a nframes CAN frames.
 *
 * @see can_add_rx_msgq()
 *
 * @param name    Name of the message queue.
 * @param nframes Number of CAN frames.
 */
#define CAN_MSGQ_DEFINE(name, nframes) \
	K_MSGQ_DEFINE(name, sizeof(struct zcan_frame), nframes, 4)
 ...
 *
 * @note The message queue must me initialized before calling this function and
 ...
 * @retval -ENOSPC if there are no free filters.
 */
__syscall int can_add_rx_msgq(const struct device *dev, struct k_msgq *msgq,
			      const struct zcan_filter *filter);
 ...
 * can_add_rx_filter() or @a can_add_rx_msgq().
 *
 * @param dev       Pointer to the device structure for the driver instance.
 ...
 *
 * @warning This function is deprecated. Use @a can_add_rx_msgq() along with @a
...
__deprecated static inline int can_attach_msgq(const struct device *dev, struct k_msgq *msg_q,
					       const struct zcan_filter *filter)
{
	return can_add_rx_msgq(dev, msg_q, filter);
}
