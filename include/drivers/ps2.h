 ...
 */
typedef void (*ps2_callback_t)(const struct device *dev, u8_t data);
...
typedef int (*ps2_read_t)(const struct device *dev, u8_t *value);
typedef int (*ps2_write_t)(const struct device *dev, u8_t value);
 ...
 * @retval Negative errno code if failure.
 */
__syscall int ps2_write(const struct device *dev, u8_t value);

static inline int z_impl_ps2_write(const struct device *dev, u8_t value)
{
	const struct ps2_driver_api *api =
 ...
 * @retval Negative errno code if failure.
 */
__syscall int ps2_read(const struct device *dev,  u8_t *value);

static inline int z_impl_ps2_read(const struct device *dev, u8_t *value)
{
	const struct ps2_driver_api *api =
			(const struct ps2_driver_api *)dev->driver_api;
