...
static struct gpio_mmio32_context Z_CONCAT(Z_DEVICE_DT_DEV_NAME(node_id), _ctx);		\
											\
static const struct gpio_mmio32_config Z_CONCAT(Z_DEVICE_DT_DEV_NAME(node_id), _cfg) = {	\
	.common = {									\
		 .port_pin_mask = _mask,						\
	},										\
...
DEVICE_DT_DEFINE(node_id,								\
		    &gpio_mmio32_init,							\
		    NULL,								\
		    &Z_CONCAT(Z_DEVICE_DT_DEV_NAME(node_id), _ctx),			\
		    &Z_CONCAT(Z_DEVICE_DT_DEV_NAME(node_id), _cfg),			\
