...
struct lsm9ds1_config {
    ...
    uint16_t i2c_slave_addr_acclgyro;
    uint16_t i2c_slave_addr_magn;
};
...
struct lsm9ds1_data {
    ...
    uint8_t accl_bandwidth;

    // Gyroscope settings
    uint8_t gyro_scale;
    uint8_t gyro_outputDataRate;
    uint8_t gyro_bandwidth;

    // Magnetometer settings
    uint8_t magn_scale;
    uint8_t magn_outputDataRate;
    uint8_t magn_mode;

    // Temperature sensor settings
    uint8_t temp_oversampleRate;
