...
#define LOG_CONST_ID_GET(_addr) \
	COND_CODE_1(CONFIG_LOG, ((__log_level != 0U ? log_const_source_id(_addr) : 0U)), (0U))
...
#define LOG_DYNAMIC_ID_GET(_addr) \
	COND_CODE_1(CONFIG_LOG, ((__log_level != 0U ? log_dynamic_source_id(_addr) : 0U)), (0U))
...
#define Z_LOG_CONST_LEVEL_CHECK(_level)					    \
	...
	((_level) <= __log_level) &&					    \
	((_level) <= (uint8_t)CONFIG_LOG_MAX_LEVEL)				    \
	)								    \
	))
...
#define LOG_FILTER_AGGR_SLOT_IDX 0U
