...
/** @brief Log link callback.
 *
 * Callback is called when new chunk is received.
 */
...
/** @brief Log link control block. */
struct log_link_ctrl_blk {
	u8_t domain_cnt;	/* Number of domains in the link. */
	u8_t domain_offset;	/* Domains numbering offset. */
	u32_t *filters;		/* Runtime filtering of source from the link. */
	char **domain_names;	/* Domain names. */
};
...
/** @brief Create log link instance.
 *
 * @param _name		Link name. Name is used as log source, domain prefix.
 * @param _api		Link API structure.
 * @param _ctx		User context.
 */
 ...
 * @return link name.
 */
static inline const char *log_link_get_name(const struct log_link *link)
{
	return link->name;
}
