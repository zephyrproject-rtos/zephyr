...
void cstart(unsigned int magic, unsigned int arg)
{
	if (magic == BOOT_MAGIC_STUB16) {
		printf("Alive on SMP CPU in 32 bit protected mode.  Stack ~%xh\n", &magic);
	}
	...
	*/
	if (magic == BOOT_MAGIC_MULTIBOOT) {
		...
		printf("mem_lower %d mem_upper %d mmap_length %d mmap_addr %p\n",
	...
	/* FIXME: I get reboots if I remove this, despite the obvious
	 * fact that the code is never reached (the real HTL loop is
	 * in core64.c.  Figure this out...
	 */
	while(1) {
