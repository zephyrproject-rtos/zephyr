...
/* Suck in the already-compiled stub as a binary blob in its own
 * section.  Easiest way to import it into the link
 */
__asm__(".pushsection .stub32\n"
        ".incbin \"stub32.bin\"\n"
        ".popsection\n");

/* Similarly pull in the 16 bit stub, but this lives in rodata in our
 * image, because we have to copy it to low memory later
 */
extern char _stub16_start, _stub16_end;
		...
		unsigned long err)
{
	 ...
	 */
	return (long)isr_exit_restore_stack((void *)(rsp - 48));
}
...
static long choose_isr_entry(int vector)
{
	 ...
	 */
	const int mask = 0b00100111110100000000;
...
static void init_gdt(void)
{
	...
	for(int i=3; i < sizeof(_shared.gdt)/sizeof(_shared.gdt[0]); i++) {
...
static void smp_init(void)
{
	...
	for (int i = 1; i < CONFIG_MP_NUM_CPUS; i++) {
		_shared.smpinit_stack = init_cpu_stack(i);
...
void _cstart64(int cpu_id)
{
	...
	_apic.SPURIOUS |= 1<<8;
	_apic.LDR = cpu_id << 24;
	_apic.DIVIDE_CONF = APIC_DIVISOR(CONFIG_APIC_TSC_SHIFT);

	printf("Initializing FS/GS segments for local CPU%d\n", cpu_id);
	setup_fg_segs(cpu_id);

	if (cpu_id == 0) {
		printf("Brining up auxilliary CPUs...\n");
		smp_init();
	}
