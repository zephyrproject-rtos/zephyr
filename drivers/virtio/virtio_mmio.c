...
#define VIRTIO_MMIO_QUEUE_INTERRUPT                1
#define VIRTIO_MMIO_DEVICE_CONFIGURATION_INTERRUPT 2
...
static void virtio_mmio_isr(const struct device *dev)
{
	...
	if (isr_status & VIRTIO_MMIO_QUEUE_INTERRUPT) {
		for (int i = 0; i < data->virtqueue_count; i++) {
			const uint16_t used_idx = sys_le16_to_cpu(virtqs[i].used->idx);

			while (virtqs[i].last_used_idx != used_idx) {
				const uint16_t idx = virtqs[i].last_used_idx % virtqs[i].num;
				const uint16_t idx_le = sys_cpu_to_le16(idx);
				const uint16_t chain_head_le = virtqs[i].used->ring[idx_le].id;
				const uint16_t chain_head = sys_le16_to_cpu(chain_head_le);
				const uint32_t used_len =
					sys_le32_to_cpu(virtqs[i].used->ring[idx_le].len);
				 ...
				 */
				while (!last) {
					const uint16_t curr = next;
					const uint16_t curr_le = sys_cpu_to_le16(curr);
	...
	if (isr_status & VIRTIO_MMIO_DEVICE_CONFIGURATION_INTERRUPT) {
...
static bool virtio_mmio_read_driver_feature_bit(const struct device *dev, int bit)
{
	virtio_mmio_write32(dev, VIRTIO_MMIO_DRIVER_FEATURES_SEL, bit / 32);
	/* DRIVER_FEATURES is write-only */
	const uint32_t val = virtio_mmio_read32(dev, VIRTIO_MMIO_DRIVER_FEATURES);
...
static const struct virtio_driver_api virtio_mmio_driver_api = {
	...
	.read_driver_feature_bit = virtio_mmio_read_driver_feature_bit,
