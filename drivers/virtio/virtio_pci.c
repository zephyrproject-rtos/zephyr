...
#define VIRTIO_PCI_QUEUE_INTERRUPT 1
#define VIRTIO_PCI_DEVICE_CONFIGURATION_INTERRUPT 2
...
void virtio_pci_isr(const struct device *dev)
{
	...
	struct virtq *virtqs = data->virtqueues;
	k_spinlock_key_t key = k_spin_lock(&data->isr_lock);

	uint8_t isr_status = *data->isr_status;

	if (isr_status & VIRTIO_PCI_QUEUE_INTERRUPT) {
		for (int i = 0; i < data->virtqueue_count; i++) {
			uint16_t used_idx = sys_le16_to_cpu(virtqs[i].used->idx);

			while (virtqs[i].last_used_idx != used_idx) {
				uint16_t idx = virtqs[i].last_used_idx % virtqs[i].num;
				uint16_t idx_le = sys_cpu_to_le16(idx);
				uint16_t chain_head_le = virtqs[i].used->ring[idx_le].id;
				uint16_t chain_head = sys_le32_to_cpu(chain_head_le);
				uint16_t used_len = sys_le32_to_cpu(
					virtqs[i].used->ring[idx_le].len
				);

				struct virtq_receive_callback_entry *cbe =
					&virtqs[i].recv_cbs[chain_head];
				...
				while (!last) {
					...
					next = virtqs[i].desc[curr_le].next;
					last = !(virtqs[i].desc[curr_le].flags & VIRTQ_DESC_F_NEXT);
					k_stack_push(&virtqs[i].free_desc_stack, curr);
				}

				virtqs[i].last_used_idx++;
			}
		}
	}
	if (isr_status & VIRTIO_PCI_DEVICE_CONFIGURATION_INTERRUPT) {
...
static void virtio_pci_notify_queue(const struct device *dev, uint16_t queue_idx)
{
	...
	data->common_cfg->queue_select = queue_idx;
	 ...
	 */
	size_t notify_off = data->common_cfg->queue_notify_off * data->notify_off_multiplier;
	volatile uint16_t *notify_addr = data->notify_cfg + notify_off;
	...
	const struct device *dev, uint16_t virtqueue_n, struct virtq *virtqueue)
{
	...
	data->common_cfg->queue_select = virtqueue_n;
	barrier_dmem_fence_full();
	...
	const struct device *dev, virtio_enumerate_queues cb, void *opaque)
{
	...
	for (int i = 0; i < queue_count; i++) {
		data->common_cfg->queue_select = i;
		barrier_dmem_fence_full();
...
static bool virtio_pci_read_driver_feature_bit(const struct device *dev, int bit)
{
	uint32_t word_n = bit / 32;
	uint32_t mask = BIT(bit % 32);

	return virtio_pci_read_driver_feature_word(dev, word_n) & mask;
}
...
static int virtio_pci_init_common(const struct device *dev)
{
	...
	if (virtio_pci_read_cap(conf->pcie->bdf, VIRTIO_PCI_CAP_NOTIFY_CFG, &vpnc, sizeof(vpnc))) {
		...
		data->notify_off_multiplier = vpnc.notify_off_multiplier;
	} else {
		LOG_ERR(
	 ...
	 * - initialization is finalized by writing DRIVER_OK bit, which is also done by
	 *   init_virtqueues() from api
	 */
...
static const struct virtio_driver_api virtio_pci_driver_api = {
	...
	.read_driver_feature_bit = virtio_pci_read_driver_feature_bit,
...
#define VIRTIO_PCI_DEFINE(inst)                                                     \
	...
	{                                                                               \
		IRQ_CONNECT(                                                                \
			DT_INST_IRQN(inst), DT_INST_IRQ(inst, priority), virtio_pci_isr, dev, 0 \
		);                                                                          \
