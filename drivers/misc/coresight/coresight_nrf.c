...
#define TS_CLOCKRATE                                                                               \
	(DT_PROP(DT_NODELABEL(hsfll200), clock_frequency) / CONFIG_CORESIGHT_NRF_TSGEN_CLK_DIV)

#define ATBREPLICATOR_IDFILTER_FORWARD_STM BIT(CONFIG_CORESIGHT_NRF_ATB_TRACE_ID_STM_GLOBAL >> 4)
...
int coresight_nrf_init_stm_etr(uintptr_t buf, size_t buf_word_len)
{
...
int coresight_nrf_init_stm_tpiu(void)
{
	mem_addr_t atbfunnel211 = DT_REG_ADDR(DT_NODELABEL(atbfunnel211));
	mem_addr_t atbreplicator210 = DT_REG_ADDR(DT_NODELABEL(atbreplicator210));
	mem_addr_t atbreplicator213 = DT_REG_ADDR(DT_NODELABEL(atbreplicator213));
...
#define CORESIGHT_NRF_INST(inst)                                                                   \
	...
	DEVICE_DT_INST_DEFINE(inst, coresight_nrf_init, NULL, NULL, &coresight_nrf_cfg_##inst,     \
			      POST_KERNEL, CORESIGHT_NRF_INIT_PRIORITY, NULL);
