...
static void nrf_atbfunnel_init(mem_addr_t funnel_addr, uint32_t ch_mask)
{
	...
	sys_write32((funnel_hold_time | (ch_mask & 0xFF)), funnel_addr + ATBFUNNEL_CTRLREG_OFFSET);
				   ...
				   uint32_t ch1_filter)
{
	...
	sys_write32(ch0_filter, replicator_addr + ATBREPLICATOR_IDFILTER0_OFFSET);
	sys_write32(ch1_filter, replicator_addr + ATBREPLICATOR_IDFILTER1_OFFSET);
