			       ...
			       void *user_data)
{
	...
	/* Publish the user pointer first; the receive thread reads it last. */
...
int esp_hosted_mcu_rpc_call(Rpc *req, Rpc *resp, int32_t timeout)
{
	...
	static uint8_t tx[ESP_HOSTED_MCU_FRAME_SIZE] __aligned(ESP_HOSTED_MCU_DMA_ALIGN);
	int ret;
				      ...
				      uint16_t len)
{
	...
	while (body - off >= 2) {
		...
		if (tag == ESP_HOSTED_MCU_PRIV_TAG_FIRMWARE_VERSION &&
			...
			/* Other private tags are not consumed. */
		}
				       ...
				       const struct esp_hosted_mcu_header *hdr)
{
	...
	case ESP_HOSTED_MCU_HCI_IF: {
		/* Load the callback first, so the user pointer cannot be stale. */
		esp_hosted_mcu_rx_cb_t cb = data->if_cb[hdr->if_type];

		compiler_barrier();
		if (cb != NULL) {
			cb(payload, hdr->len, hdr->hci_pkt_type, data->if_user[hdr->if_type]);
		}
		break;
	}
...
static void esp_hosted_mcu_event_task(void *p1, void *p2, void *p3)
{
	 ...
	 *
	 * A transport may hand this buffer straight to DMA, and cache
	 * maintenance around a DMA receive works on whole cache lines. If the
	 * buffer shared a line with another object, invalidating after the
	 * transfer would discard that object's dirty bytes on a write-back
	 * cache, so keep it on its own lines.
	 */
	...
	while (true) {
		if (!cfg->transport->data_ready(dev)) {
			...
			cfg->transport->wait_for_rx(
				dev, K_MSEC(CONFIG_ESP_HOSTED_MCU_EVENT_TASK_POLL_MS));
			continue;
		}
...
static void esp_hosted_mcu_check_fw_version(struct esp_hosted_mcu_data *data)
{
	...
	/*
	 * Give the boot event a chance to land first. Firmware that carries the
	 * version there answers without a round trip, and asking anyway costs
	 * the full request timeout on the coprocessors that never reply.
	 */
	for (k_timepoint_t end = sys_timepoint_calc(K_MSEC(ESP_HOSTED_MCU_BOOT_EVENT_WAIT));
	     data->fw_version == 0 && !sys_timepoint_expired(end);) {
		k_msleep(10);
	}
