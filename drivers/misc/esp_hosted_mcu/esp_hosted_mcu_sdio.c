 ...
 * Exactly one of tx or rx is non-NULL.
 */
static int esp_slc_fifo_xfer(struct esp_hosted_mcu_sdio_data *data, const uint8_t *tx, uint8_t *rx,
			     uint32_t len)
{
	uint32_t remain = len;
	...
	while (remain > 0) {
		...
		if (tx != NULL) {
			ret = sdio_write_addr(&data->func1, addr, (uint8_t *)tx_ptr, chunk);
		} else {
			ret = sdio_read_addr(&data->func1, addr, rx_ptr, chunk);
		}
		...
		tx_ptr += (tx != NULL) ? consumed : 0;
		rx_ptr += (rx != NULL) ? consumed : 0;
		remain -= consumed;
	}
...
static void ESP_HOSTED_MCU_ISR_ATTR esp_hosted_mcu_sdio_card_int(const struct device *sdhc_dev,
								 int reason, const void *user_data)
{
...
static int esp_hosted_mcu_sdio_init(const struct device *dev)
{
	 ...
	 * cycle the SD subsystem performs when it enumerates the card below.
	 * That only restarts it when the host controller acts on the SDHC power
	 * mode and the board routes the coprocessor supply through it, so the
	 * enumeration retry below is what covers a cycle that did nothing.
	 */
	 ...
	 */
	k_timepoint_t ready_end = sys_timepoint_calc(K_MSEC(ESP_SLC_READY_TIMEOUT));
...
static void esp_hosted_mcu_sdio_wait_for_rx(const struct device *dev, k_timeout_t timeout)
{
	const struct esp_hosted_mcu_config *cfg = dev->config;
	const struct esp_hosted_mcu_sdio_config *sdio = cfg->transport_config;
	struct esp_hosted_mcu_sdio_data *data = &esp_hosted_mcu_sdio_data_0;
	...
	k_sem_take(&data->rx_sem, timeout);
}

/*
 * Push one frame to the coprocessor: wait for enough free receive buffers,
 * then write the padded frame over the FIFO window and spend the credits.
 */
static int esp_slc_tx_frame(struct esp_hosted_mcu_sdio_data *data, const uint8_t *tx,
			    uint16_t tx_len)
{
	 ...
	 */
	k_timepoint_t credit_end = sys_timepoint_calc(K_MSEC(ESP_SLC_TX_CREDIT_TIMEOUT));
	...
	while (true) {
		...
		if (sys_timepoint_expired(credit_end)) {
			return -EBUSY;
		}
		k_usleep(100);
	}
	...
	k_mutex_lock(&data->bus_lock, K_FOREVER);
	ret = esp_slc_fifo_xfer(data, tx, NULL, padded);
	if (ret == 0) {
		data->tx_sent_buffers = (data->tx_sent_buffers + need) & ESP_SLC_TX_BUFFER_MASK;
	}
	k_mutex_unlock(&data->bus_lock);
	if (ret != 0) {
					...
					uint16_t tx_len, uint8_t *rx)
{
	...
	if (tx != NULL && tx_len > 0) {
		ret = esp_slc_tx_frame(data, tx, tx_len);
		if (ret != 0) {
			return ret;
		}
	}
	...
	if (ret == 0 && pending > 0) {
		...
		ret = esp_slc_fifo_xfer(data, NULL, rx, pending);
		if (ret == 0) {
