...
{
	K_OOPS(K_SYSCALL_DRIVER_TGPIO(port, get_time));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(current_time, sizeof(uint64_t)));
...
static inline int z_vrfy_tgpio_port_get_cycles_per_second(const struct device *port,
							   uint32_t *cycles)
{
	K_OOPS(K_SYSCALL_DRIVER_TGPIO(port, cyc_per_sec));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(cycles, sizeof(uint32_t)));
...
{
	K_OOPS(K_SYSCALL_DRIVER_TGPIO(port, set_perout));
...
{
	K_OOPS(K_SYSCALL_DRIVER_TGPIO(port, pin_disable));
...
{
	K_OOPS(K_SYSCALL_DRIVER_TGPIO(port, config_ext_ts));
