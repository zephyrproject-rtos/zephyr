...
#define DEV_CFG(_dev)       ((const struct tgpio_config *)(_dev)->config)
...
static int tgpio_intel_get_time(const struct device *dev, uint64_t *current_time)
{
...
static int tgpio_intel_cyc_per_sec(const struct device *dev, uint32_t *cycles)
{
...
static int tgpio_intel_pin_disable(const struct device *dev, uint32_t pin)
{
...
static int tgpio_intel_periodic_output(const struct device *dev, uint32_t pin, uint64_t start_time,
...
static int tgpio_intel_config_external_timestamp(const struct device *dev, uint32_t pin,
...
static int tgpio_intel_read_ts_ec(const struct device *dev, uint32_t pin, uint64_t *timestamp,
...
static int tgpio_init(const struct device *dev)
{
	...
	device_map(&rt->reg_base, cfg->reg_base.phys_addr & ~0xFFU, cfg->reg_base.size,
		   K_MEM_CACHE_NONE);
