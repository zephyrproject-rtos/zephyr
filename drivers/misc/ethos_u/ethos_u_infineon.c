...
#define DT_DRV_COMPAT infineon_ifx_npu
...
/* Current CPU Power Domain - for PDCM dependency configuration */
#if defined(__CORTEX_M) && (__CORTEX_M == 33U)
#define U55_CURRENT_CPU_PD CY_PD_PDCM_SYSCPU /* M33 */
#elif defined(__CORTEX_M) && (__CORTEX_M == 55U)
#define U55_CURRENT_CPU_PD CY_PD_PDCM_APPCPU /* M55 */
#else
/* Default to M55/APPCPU for PSE84 NPU driver */
#define U55_CURRENT_CPU_PD CY_PD_PDCM_APPCPU
#endif
...
static int infineon_npu_pdcm_enable(void)
{
	...
	LOG_DBG("Setting PDCM dependency: U55 depends on CPU PD %d", U55_CURRENT_CPU_PD);

	status = cy_pd_pdcm_set_dependency(CY_PD_PDCM_U55, U55_CURRENT_CPU_PD);
