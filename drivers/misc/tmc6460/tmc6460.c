...
/*
 * Bus abstraction: each transport (SPI or UART) provides an implementation of
 * the following operations. The concrete function-pointer table is selected at
 * device declaration time based on the devicetree bus the instance sits on.
 */
typedef int (*tmc6460_bus_ready_fn)(const struct device *dev);
typedef int (*tmc6460_reg_read_fn)(const struct device *dev, uint16_t reg_addr, uint32_t *reg_val);
typedef int (*tmc6460_reg_write_fn)(const struct device *dev, uint16_t reg_addr, uint32_t reg_val);

struct tmc6460_bus_io {
	tmc6460_bus_ready_fn ready;
	tmc6460_reg_read_fn read;
	tmc6460_reg_write_fn write;
};

struct tmc6460_config {
	/* Bus operations for the transport this instance is wired to */
	const struct tmc6460_bus_io *bus_io;
...
static int tmc6460_spi_ready(const struct device *dev)
{
...
static const struct tmc6460_bus_io tmc6460_bus_io_spi = {
	.ready = tmc6460_spi_ready,
	.read = tmc6460_spi_read,
	.write = tmc6460_spi_write,
};
...
static int tmc6460_uart_ready(const struct device *dev)
{
	const struct tmc6460_config *config = dev->config;
...
static const struct tmc6460_bus_io tmc6460_bus_io_uart = {
	.ready = tmc6460_uart_ready,
	.read = tmc6460_uart_read,
	.write = tmc6460_uart_write,
};
#endif /* CONFIG_TMC6460_UART */
...
static int tmc6460_init(const struct device *dev)
{
	...
	return config->bus_io->ready(dev);
}
...
#define TMC6460_CONFIG_UART(inst)                                                                  \
	.bus_io = &tmc6460_bus_io_uart, .uart = DEVICE_DT_GET(DT_INST_BUS(inst)),                  \
