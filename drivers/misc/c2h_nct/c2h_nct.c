...
#define DT_DRV_COMPAT nuvoton_nct_c2h

/*
 * NCT series enables the Core to access the Host module registers
...
LOG_MODULE_REGISTER(c2h_nct, CONFIG_LOG_DEFAULT_LEVEL);

#define NCT_C2H_TRANSACTION_TIMEOUT_US 200

struct c2h_nct_config {
...
{
	struct c2h_nct_config *cfg = (struct c2h_nct_config *)dev->config;
...
#define NCT_C2H_INIT_FUNC(inst) _CONCAT(c2h_init, inst)
#define NCT_C2H_INIT_FUNC_DECL(inst) \
	static int c2h_init##inst(const struct device *dev)

/* C2H init function implementation */
#define NCT_C2H_INIT_FUNC_IMPL(inst)				              \
	static int c2h_init##inst(const struct device *dev)	                      \
	{								              \
		struct c2h_nct_config *cfg = (struct c2h_nct_config *)dev->config;  \
		struct c2h_reg *const inst_c2h = cfg->inst_c2h;		              \
...
#define NCT_C2H_INIT(inst)						                        \
	NCT_C2H_INIT_FUNC_DECL(inst);								\
									                        \
	static const struct c2h_nct_config c2h_nct_config_##inst = {	                        \
		.inst_c2h = (struct c2h_reg *)DT_INST_REG_ADDR(inst),				\
	};								                        \
									                        \
	DEVICE_DT_INST_DEFINE(inst,					                        \
			      NCT_C2H_INIT_FUNC(inst),						\
			      NULL,					                        \
			      NULL, &c2h_nct_config_##inst,		                        \
			      PRE_KERNEL_1,				                        \
			      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, NULL);                        \
									                        \
									                        \
	NCT_C2H_INIT_FUNC_IMPL(inst)

DT_INST_FOREACH_STATUS_OKAY(NCT_C2H_INIT)
