...
static esp_err_t rmt_tx_create_trans_queue(rmt_tx_channel_t *tx_channel, const rmt_tx_channel_config_t *config)
{
    ...
    for (int i = 0; i < RMT_TX_QUEUE_MAX; i++) {
        ...
        ESP_RETURN_ON_FALSE(tx_channel->trans_queue_structs[i], ESP_ERR_NO_MEM, TAG, "no mem for queue storage");
    ...
    for (int i = 0; i < config->trans_queue_depth; i++) {
        ...
        ESP_RETURN_ON_FALSE(k_msgq_put(&tx_channel->trans_queues[RMT_TX_QUEUE_READY], &p_trans_desc, K_NO_WAIT) == 0,
...
esp_err_t rmt_transmit(rmt_channel_handle_t channel, rmt_encoder_t *encoder, const void *payload, size_t payload_bytes, const rmt_transmit_config_t *config)
{
    ...
    if (k_msgq_put(&tx_chan->trans_queues[RMT_TX_QUEUE_PROGRESS], &t, K_NO_WAIT) == 0) {
        ...
        ESP_RETURN_ON_FALSE(k_msgq_put(&tx_chan->trans_queues[RMT_TX_QUEUE_READY], &t, K_NO_WAIT) == 0,
                            ESP_ERR_INVALID_STATE, TAG, "ready queue full");
    }
...
esp_err_t rmt_tx_wait_all_done(rmt_channel_handle_t channel, k_timeout_t wait_ticks)
{
    ...
    for (size_t i = 0; i < num_trans_inflight; i++) {
        ESP_RETURN_ON_FALSE(k_msgq_get(&tx_chan->trans_queues[RMT_TX_QUEUE_COMPLETE], &t, wait_ticks) == 0,
