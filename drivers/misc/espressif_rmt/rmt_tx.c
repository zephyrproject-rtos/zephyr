	...
	const rmt_tx_channel_config_t *config)
{
	...
	if (tx_channel->base.with_dma) {
		...
		channel_scan_start = RMT_TX_CHANNEL_OFFSET_IN_GROUP
			+ RMT_LL_GET(TX_CANDIDATES_PER_INST) - 1;
...
#if CONFIG_PM
	key = k_spin_lock(&group->spinlock);
	if (tx_channel->base.pm_policy_state_on) {
		tx_channel->base.pm_policy_state_on = false;
	...
	rmt_channel_handle_t *ret_chan)
{
	...
	with_dma = (cfg->dma_dev)
		...
		&& (RMT_TX_CHANNEL_ID(config->gpio_pinmux)
		== RMT_LL_GET(TX_CANDIDATES_PER_INST) - 1);
	/*
	...
	key = k_spin_lock(&group->spinlock);
	rmt_hal_tx_channel_reset(hal, channel_id);
	k_spin_unlock(&group->spinlock, key);
	...
	ret = esp_intr_alloc_intrstatus(soc_rmt_signals[group->group_id].irq, isr_flags,
	...
	rc = rmt_select_periph_clock(&tx_channel->base, config->clk_src, config->resolution_hz);
	if (rc) {
	...
	rmt_sync_manager_handle_t *ret_synchro)
{
	...
	synchro = k_calloc(1, sizeof(rmt_sync_manager_t) + sizeof(rmt_channel_handle_t)
		* config->array_size);
	if (!synchro) {
...
#if CONFIG_ESPRESSIF_RMT_TX_ISR_CACHE_SAFE
...
static size_t IRAM_ATTR rmt_tx_mark_eof(rmt_tx_channel_t *tx_chan, bool need_eof_marker)
{
...
static size_t IRAM_ATTR rmt_encode_check_result(rmt_tx_channel_t *tx_chan, rmt_tx_trans_desc_t *t)
{
	...
	if (encode_state & RMT_ENCODING_COMPLETE) {
		...
		if (!(encode_state & RMT_ENCODING_MEM_FULL)) {
			encoded_symbols += rmt_tx_mark_eof(tx_chan,
				!(encode_state & RMT_ENCODING_WITH_EOF));
		}
	}
	...
	if (t->loop_count != 0) {
		if (unlikely(encoded_symbols > tx_chan->base.mem_block_num
			* SOC_RMT_MEM_WORDS_PER_CHANNEL)) {
			LOG_ERR("encoding artifacts can't exceed hw memory block for loop "
				"transmission");
		}
	}
...
static int rmt_tx_enable(rmt_channel_handle_t channel)
{
	rmt_tx_channel_t *tx_chan = __containerof(channel, rmt_tx_channel_t, base);
	rmt_tx_trans_desc_t *t = NULL;
#if CONFIG_PM || SOC_RMT_SUPPORT_DMA
	rmt_group_t *group = channel->group;
	k_spinlock_key_t key;
#endif
#if SOC_RMT_SUPPORT_DMA
...
static int rmt_tx_disable(rmt_channel_handle_t channel)
{
	...
	if (atomic_cas(&channel->fsm, RMT_FSM_RUN, RMT_FSM_WAIT)) {
		 ...
		 */
		memset(channel->hw_mem_base, 0, channel->mem_block_num
			* SOC_RMT_MEM_WORDS_PER_CHANNEL * sizeof(rmt_symbol_word_t));
...
static void IRAM_ATTR rmt_isr_handle_tx_threshold(rmt_tx_channel_t *tx_chan)
{
...
static void IRAM_ATTR rmt_isr_handle_tx_done(rmt_tx_channel_t *tx_chan)
{
	rmt_channel_t *channel = &tx_chan->base;
	...
	if (atomic_cas(&channel->fsm, RMT_FSM_RUN, RMT_FSM_WAIT)) {
		...
		k_msgq_put(&tx_chan->trans_queues[RMT_TX_QUEUE_COMPLETE], &trans_desc, K_NO_WAIT);
		tx_chan->cur_trans = NULL;
		atomic_set(&channel->fsm, RMT_FSM_ENABLE);
		...
		if (tx_chan->on_trans_done) {
			...
			tx_chan->on_trans_done(channel, &edata, tx_chan->user_data);
		}
	}
...
#if SOC_RMT_SUPPORT_TX_LOOP_COUNT
static void IRAM_ATTR rmt_isr_handle_tx_loop_end(rmt_tx_channel_t *tx_chan)
{
	rmt_channel_t *channel = &tx_chan->base;
	rmt_group_t *group = channel->group;
	rmt_hal_context_t *hal = &group->hal;
	...
	if (trans_desc) {
		...
		if (atomic_cas(&channel->fsm, RMT_FSM_RUN, RMT_FSM_WAIT)) {
			/* move current finished transaction to the complete queue */
			k_msgq_put(&tx_chan->trans_queues[RMT_TX_QUEUE_COMPLETE], &trans_desc,
				K_NO_WAIT);
			tx_chan->cur_trans = NULL;
			atomic_set(&channel->fsm, RMT_FSM_ENABLE);
...
static void IRAM_ATTR rmt_tx_default_isr(void *args)
{
	...
	status = rmt_ll_tx_get_interrupt_status(hal->regs, channel->channel_id);
	rmt_ll_clear_interrupt_status(hal->regs, status);

	/* Tx threshold interrupt */
	if (status & RMT_LL_EVENT_TX_THRES(channel->channel_id)) {
		rmt_isr_handle_tx_threshold(tx_chan);
	}

	/* Tx end interrupt */
	if (status & RMT_LL_EVENT_TX_DONE(channel->channel_id)) {
		rmt_isr_handle_tx_done(tx_chan);
	}

#if SOC_RMT_SUPPORT_TX_LOOP_COUNT
	/* Tx loop end interrupt */
	if (status & RMT_LL_EVENT_TX_LOOP_END(channel->channel_id)) {
