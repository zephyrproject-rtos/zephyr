...
#define RMT_RX_CHANNEL_OFFSET_IN_GROUP \
	(RMT_LL_GET(CHANS_PER_INST) - RMT_LL_GET(TX_CANDIDATES_PER_INST))
...
struct rmt_group_t {
	...
	rmt_tx_channel_t *tx_channels[RMT_LL_GET(TX_CANDIDATES_PER_INST)];
	/**< Array of RMT RX channels */
	rmt_rx_channel_t *rx_channels[RMT_LL_GET(RX_CANDIDATES_PER_INST)];
...
typedef struct {
	...
	struct {
		...
		/**< Indicate whether need to insert an EOF mark
		 * (a special RMT symbol)
		 */
		uint32_t need_eof_mark: 1;
	} flags;
} rmt_tx_trans_desc_t;
...
typedef struct {
	...
	struct {
		/**< Packet is too long, we need to notify the user to process the data piece by
		 * piece, in a ping-pong approach
		 */
