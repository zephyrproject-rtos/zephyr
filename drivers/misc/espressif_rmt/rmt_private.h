...
struct rmt_group_t {
    ...
    struct k_spinlock spinlock; /**< to protect per-group register level concurrent access */
...
struct rmt_channel_t {
    ...
    struct k_spinlock spinlock; /**< prevent channel resource accessing by user and interrupt concurrently */
...
struct rmt_tx_channel_t {
    ...
    struct k_msgq trans_queues[RMT_TX_QUEUE_MAX]; /**< transaction queues */
    struct msg *trans_queue_structs[RMT_TX_QUEUE_MAX]; /**< memory to store the static structure for trans_queues */
 ...
 * @retval -ENODEV if setting clock source failed because the clk_src is different from other RMT channel.
