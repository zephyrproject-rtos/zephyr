...
typedef struct rmt_platform_t {
    unsigned int key;                     /**< platform level mutex lock */
...
int rmt_select_periph_clock(rmt_channel_handle_t chan, rmt_clock_source_t clk_src)
{
    ...
    if (clock_selection_conflict) {
        LOG_ERR("Group clock conflict, already is %d but attempt to %d", group->clk_src, clk_src);
        return -EINVAL;
    }
    ...
    ret = esp_clk_tree_src_get_freq_hz((soc_module_clk_t)clk_src, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED, &periph_src_clk_hz);
    if (ret) {
...
#define ESPRESSIF_RMT_DEVICE(idx)                                               \
DEVICE_DT_INST_DEFINE(idx, NULL, NULL, NULL, NULL,                              \
		      POST_KERNEL, CONFIG_ESPRESSIF_RMT_INIT_PRIORITY, NULL);

DT_INST_FOREACH_STATUS_OKAY(ESPRESSIF_RMT_DEVICE)
