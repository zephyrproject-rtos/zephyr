...
enum seu_commands {
	/** enum for not selected SEU commands */
	NOT_SELECTED = 0,
	/** enum for selected Read SEU commands */
	READ_SEU_ERROR,
	/** enum for selected insert SEU commands */
	INSERT_SEU_ERROR,
	/** enum for selected Read SEU statistics commands */
	READ_SEU_STATS,
	/** enum for selected insert safe SEU commands */
	INSERT_SAFE_SEU_ERROR,
	/** enum for selected insert ECC commands */
	INSERT_ECC_ERROR
};

/** SIP SVC private data struture */
...
static void seu_callback(uint32_t c_token, struct sip_svc_response *response)
{
	...
	struct sip_svc_private_data *priv = (struct sip_svc_private_data *)response->priv_data;
...
static int seu_send_sip_svc(uint32_t *cmd_addr, uint32_t cmd_size, uint32_t *resp_addr,
			    uint32_t resp_size, struct sip_svc_private_data *sip_svc_data)
{
	...
	err = svc_client_open(sip_svc_data->seu_data_ptr);
	if (err) {
	...
	trans_id = sip_svc_send(sip_svc_data->seu_data_ptr->mailbox_smc_dev,
				sip_svc_data->seu_data_ptr->mailbox_client_token, &request,
				seu_callback);
	if (trans_id < 0) {
	...
	err = k_sem_take(&(sip_svc_data->semaphore), K_FOREVER);
	if (err != 0) {
		LOG_ERR("Error in taking semaphore");
	}

sip_svc_client_close:
	err = svc_client_close(sip_svc_data->seu_data_ptr);
	if (err) {
		LOG_ERR("Unregistering & Closing failed");
		return err;
	}

	err = sip_svc_data->status;
...
int read_seu_error(struct seu_intel_socfpga_data *const seu_data, uint32_t *seu_error_count)
{
	...
	struct sip_svc_private_data priv;
	int ret = 0;
