...
#define CS40L26_IRQ_MASK1                                                                          \
	((uint32_t)(CS40L26_DSP_VIRTUAL2_MBOX_WR_MASK1 | CS40L26_AMP_ERR_MASK1 |                   \
		    CS40L26_TEMP_ERR_MASK1 | CS40L26_BST_IPK_FLAG_MASK1 |                          \
		    CS40L26_BST_SHORT_ERR_MASK1 | CS40L26_BST_DCM_UVP_ERR_MASK1 |                  \
...
__maybe_unused static int cs40l26_pm_turn_off(const struct device *const dev)
{
	...
	if (IS_ENABLED(CONFIG_HAPTICS_CS40L26_RESET) && config->reset_gpio.port != NULL) {
		...
		ret1 = gpio_pin_set_dt(&config->reset_gpio, 1);
		if (ret1 >= 0 &&
