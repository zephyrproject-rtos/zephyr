...
static inline void z_vrfy_haptics_stop_output(const struct device *dev)
{
...
static inline int z_vrfy_haptics_preempt_output(const struct device *dev)
{
	Z_OOPS(Z_SYSCALL_DRIVER_HAPTICS(dev, preempt_output));

	return z_impl_haptics_preempt_output(dev);
}

#include <syscalls/haptics_preempt_output_mrsh.c>
