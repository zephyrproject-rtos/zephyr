...
static bool cs40l5x_regmap_is_ready_spi(const struct device *const dev)
{
...
static struct device *cs40l5x_regmap_get_device_spi(const struct device *const dev)
{
...
static int cs40l5x_regmap_read_spi(const struct device *const dev, const uint32_t addr,
...
static int cs40l5x_regmap_write_spi(const struct device *const dev, const uint32_t *const tx,
				    const uint32_t len)
{
	const struct cs40l5x_config *const config = dev->config;
...
const struct cs40l5x_bus_io cs40l5x_bus_io_spi = {
	.is_ready = cs40l5x_regmap_is_ready_spi,
	.get_device = cs40l5x_regmap_get_device_spi,
	.read = cs40l5x_regmap_read_spi,
	.write = cs40l5x_regmap_write_spi,
};
