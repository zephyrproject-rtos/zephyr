...
#define CS40L5X_ANY_DEV_USE_EXTERNAL_BOOST DT_ANY_INST_HAS_BOOL_STATUS_OKAY(external_boost)
#define CS40L5X_ANY_DEV_USE_INTERNAL_BOOST !DT_ALL_INST_HAS_BOOL_STATUS_OKAY(external_boost)
...
static int cs40l5x_write_errata(const struct device *const dev)
{
	...
	if (CS40L5X_ANY_DEV_USE_EXTERNAL_BOOST && config->external_boost) {
...
static int cs40l5x_boost_configuration(const struct device *const dev)
{
	...
	return (!CS40L5X_ANY_DEV_USE_INTERNAL_BOOST || config->external_boost)
...
int cs40l5x_logger(const struct device *const dev, enum cs40l5x_logger logger_state)
{
	...
	if (!IS_ENABLED(CONFIG_HAPTICS_CS40L5X_LOGGER)) {
...
#define HAPTICS_CS40L5X_CONFIG(inst)                                                               \
	...
	.external_boost = DT_INST_PROP(inst, external_boost),                                      \
