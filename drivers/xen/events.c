...
int bind_interdomain_event_channel(domid_t remote_dom, evtchn_port_t remote_port, evtchn_cb_t cb,
				   void *data)
{
	int rc;
...
void notify_evtchn(evtchn_port_t port)
{
	...
	__ASSERT(port < EVTCHN_2L_NR_CHANNELS, "%s: trying to send notify for invalid evtchn #%u\n",
		 __func__, port);
...
{
	__ASSERT(port < EVTCHN_2L_NR_CHANNELS, "%s: trying to bind invalid evtchn #%u\n", __func__,
		 port);
...
{
	__ASSERT(port < EVTCHN_2L_NR_CHANNELS, "%s: trying to unbind invalid evtchn #%u\n",
		 __func__, port);
...
int mask_event_channel(evtchn_port_t port)
{
	...
	__ASSERT(port < EVTCHN_2L_NR_CHANNELS, "%s: trying to mask invalid evtchn #%u\n", __func__,
		 port);
...
int unmask_event_channel(evtchn_port_t port)
{
	...
	__ASSERT(port < EVTCHN_2L_NR_CHANNELS, "%s: trying to unmask invalid evtchn #%u\n",
		 __func__, port);
...
static void events_isr(void *data)
{
	...
	pos_selector = __atomic_exchange_n(&vcpu->evtchn_pending_sel, 0, __ATOMIC_SEQ_CST);
	...
	while (pos_selector) {
		...
		while ((events_pending = get_pending_events(pos_index)) != 0) {
			...
			port = (pos_index * 8 * sizeof(xen_ulong_t)) + event_index;
			process_event(port);
		}
	}
}
...
int xen_events_init(void)
{
	...
	IRQ_CONNECT(DT_IRQ_BY_IDX(DT_INST(0, xen_xen), 0, irq),
		    DT_IRQ_BY_IDX(DT_INST(0, xen_xen), 0, priority), events_isr, NULL,
