...
void *gnttab_get_pages(unsigned int npages)
{
	...
	for (removed = 0; removed < npages; removed++) {
		gfn = xen_virt_to_gfn((char *)page_addr + (removed * XEN_PAGE_SIZE));
	...
	if (ret) {
		...
		if (removed > 0) {
			...
			if (ret) {
				LOG_WRN("gnttab_put_pages failed ret=%d addr=%p", ret, page_addr);
			}
		}
...
int gnttab_put_pages(void *page_addr, unsigned int npages)
{
	...
	pages = k_malloc(sizeof(xen_pfn_t) * npages);
	if (pages == NULL) {
	...
	for (i = 0; i < npages; i++) {
		pages[i] = xen_virt_to_gfn((char *)page_addr + (i * XEN_PAGE_SIZE));
	}
