 ...
 *
 * Host side of the shared-memory mailbox exposed by the wifi4 MAC
 * firmware blob.
 *
 *   shared memory : struct ipc_shared_env_tag ipc_shared_env.
 *   ctrl regs     : BFLB_IPC_REG_BASE APP2EMB_TRIGGER (host->fw doorbell),
 ...
 *
 * The shared-env seeding, the E2A ISR body and the firmware wake after a
 * doorbell trigger are firmware-specific and live in the per-SoC
 * soc/bflb/<series>/wifi/bflb_wifi_ipc.c files.
 */
...
/* The blob provides ipc_shared_env (in its shared-RAM IPC section). */
...
int bflb_wifi_ipc_init(struct bflb_wifi_dev *d)
{
	...
	IRQ_CONNECT(WIFI_IPC_IRQ_NUM, WIFI_IPC_IRQ_PRI, bflb_wifi_ipc_isr, NULL, 0);
	irq_enable(WIFI_IRQ_NUM);
	irq_enable(WIFI_IPC_IRQ_NUM);

	LOG_DBG("ipc init: shared_env=%p", &ipc_shared_env);
	return 0;
}
