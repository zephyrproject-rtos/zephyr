 ...
 *
 * Uses one mbedtls_ccm_context, keyed at PTK install and freed on
 * disconnect, instead of psa_aead_encrypt(): the PSA one-shot API
 * allocates and frees an operation context on every call, and under
 * MBEDTLS_MEMORY_BUFFER_ALLOC_C that per-frame churn fragments the
 * fixed mbedTLS heap until an alloc fails during sustained TX.  Not a
 * PSA bug and not a leak, just fragmentation.
 */
...
#define FC_AAD_BYTE0_MASK   0x8FU /* mute Subtype bits 4-6 for data frames */
			    ...
			    bool has_a4, uint8_t qos_tid)
{
	...
	aad[0] = mac_hdr[0] & FC_AAD_BYTE0_MASK;
			   ...
			   size_t mac_hdr_len, const uint8_t *plain, size_t plain_len, int qos_tid)
{
	...
	/* AAD from the on-air header so Protected is always set in it. */
	aad_len = ccmp_make_aad(aad, out, is_qos, has_a4, tid);
