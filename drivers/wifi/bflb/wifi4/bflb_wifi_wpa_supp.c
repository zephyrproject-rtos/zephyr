...
static int bflb_wpa_supp_get_scan_results(void *if_priv)
{
	...
	for (uint8_t i = 0; i < count; i++) {
		...
		uint8_t res_buf[sizeof(struct wpa_scan_res) + 2 + BFLB_WIFI_SSID_MAX_LEN +
				BFLB_SCAN_RSN_IE_MAX + BFLB_SCAN_WPA_IE_MAX + BFLB_SCAN_RSNXE_MAX]
		...
		*ie_pos++ = ap->ssid_len;
		...
		if (ap->rsnxe_len > 0U) {
			memcpy(ie_pos, ap->rsnxe, ap->rsnxe_len);
			ie_pos += ap->rsnxe_len;
		}
		...
		r->freq = wifi_utils_chan_to_freq(WIFI_FREQ_BAND_2_4_GHZ, ap->channel);
		r->level = ap->rssi;
		r->ie_len = ie_len;
		r->caps = ap->caps;
...
static int bflb_wpa_supp_associate(void *if_priv, struct wpa_driver_associate_params *params)
{
	...
	if (params->freq.freq != 0) {
		...
		ctx->conn_channel = wifi_utils_freq_to_chan(params->freq.freq);
	}
	 ...
	 * The RSNXE is dropped: the SAE bridge only does hunting-and-pecking
	 * and the blob owns the auth frame status code, so advertising H2E
	 * makes the AP reject the association.
	 */
	ctx->assoc_ie_len = 0;
	if (params->wpa_ie != NULL && params->wpa_ie_len <= sizeof(ctx->assoc_ie)) {
		const uint8_t *ie = params->wpa_ie;
		const uint8_t *end = ie + params->wpa_ie_len;

		while ((end - ie) >= 2 && (end - ie) >= (2 + ie[1])) {
			size_t n = 2 + ie[1];

			if (ie[0] != WLAN_EID_RSNX) {
				memcpy(&ctx->assoc_ie[ctx->assoc_ie_len], ie, n);
				ctx->assoc_ie_len += n;
			}
			ie += n;
		}
				 ...
				 size_t key_len, enum key_flag key_flag)
{
	...
	/* No RX-only "next" TK slot in the firmware: skip it so the PTK is
	 * only installed once EAPOL msg 4 has gone out.
	 */
	if (pairwise && (key_flag & KEY_FLAG_NEXT) != 0) {
		return 0;
	}
	...
	/* CCMP only: the firmware's TX path does not offer TKIP or WEP. */
				     ...
				     unsigned char *bssid)
{
	...
	si->data.current_tx_rate = (unsigned long)bflb_wifi_phy_rate_kbps(d->sta_idx);
	if (d->connected_channel > 0U) {
		si->frequency =
			wifi_utils_chan_to_freq(WIFI_FREQ_BAND_2_4_GHZ, d->connected_channel);
	}
...
static int bflb_wpa_supp_get_wiphy(void *if_priv)
{
	...
	for (i = 1;
	     i <= BFLB_WIFI_CHAN_2G4_CNT && band.wpa_supp_n_channels < WPA_SUPP_SBAND_MAX_CHANNELS;
	     i++) {
	...
	band.wpa_supp_n_bitrates = BFLB_WIFI_LEGACY_RATE_CNT;
	for (i = 0; i < BFLB_WIFI_LEGACY_RATE_CNT; i++) {
		band.bitrates[i].wpa_supp_bitrate = bflb_wifi_legacy_rates_100kbps[i];
	}
...
static int bflb_wpa_supp_get_conn_info(void *if_priv, struct wpa_conn_info *info)
{
	const struct bflb_wifi_dev *d = &bflb_wifi;
	...
	/* Taken from the BSS's own beacon, which the scan cached.  TWT is
	 * 11ax only, so it stays false.
	 */
	if (d->connected) {
		(void)bflb_wifi_scan_conn_info(d->connected_bssid, &info->beacon_interval,
					       &info->dtim_period);
	}
