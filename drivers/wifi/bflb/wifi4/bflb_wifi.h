...
/* 2.4 GHz channel plan: channels 1-13 sit at 2407 + 5 * n MHz, channel 14
 * is the 2484 MHz outlier.  Channel 14 is not scanned or configured.
 */
#define BFLB_WIFI_CHAN_2G4_MAX  14U
#define BFLB_WIFI_CHAN_2G4_CNT  13U
#define BFLB_WIFI_FREQ_2G4_BASE 2407U
#define BFLB_WIFI_FREQ_2G4_STEP 5U
#define BFLB_WIFI_FREQ_2G4_CH14 2484U

static inline uint16_t bflb_wifi_chan_to_freq(uint8_t chan)
{
	if ((chan == 0U) || (chan > BFLB_WIFI_CHAN_2G4_MAX)) {
		return 0U;
	}
	if (chan == BFLB_WIFI_CHAN_2G4_MAX) {
		return BFLB_WIFI_FREQ_2G4_CH14;
	}
	return (uint16_t)(BFLB_WIFI_FREQ_2G4_BASE + (BFLB_WIFI_FREQ_2G4_STEP * chan));
}

static inline uint8_t bflb_wifi_freq_to_chan(uint16_t freq)
{
	uint16_t first = bflb_wifi_chan_to_freq(1U);
	uint16_t last = bflb_wifi_chan_to_freq(BFLB_WIFI_CHAN_2G4_CNT);

	if ((freq >= first) && (freq <= last)) {
		return (uint8_t)((freq - BFLB_WIFI_FREQ_2G4_BASE) / BFLB_WIFI_FREQ_2G4_STEP);
	}
	if (freq == BFLB_WIFI_FREQ_2G4_CH14) {
		return BFLB_WIFI_CHAN_2G4_MAX;
	}
	return 0U;
}
