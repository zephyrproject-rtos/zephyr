...
#define BFLB_SCAN_AP_MAX     ((uint8_t)CONFIG_WIFI_BFLB_WIFI4_SCAN_AP_MAX)
#define BFLB_SCAN_RSN_IE_MAX 50U
#define BFLB_SCAN_WPA_IE_MAX 40U
#define BFLB_SCAN_RSNXE_MAX  16U
...
struct bflb_scan_ap {
	...
	uint8_t mfp;              /* wifi_mfp_options */
	uint16_t caps;            /* 802.11 capability field */
	uint16_t beacon_interval; /* TU */
	uint8_t dtim_period;      /* 0 when only a probe response was seen */
	/* Raw security IEs cached verbatim for the supplicant -- hostap
	 * byte-compares the RSN IE and RSNXE during 4-way handshake msg 3
	 * validation.
	 */
	...
	uint8_t wpa_ie_len;
	uint8_t rsnxe[BFLB_SCAN_RSNXE_MAX];
	uint8_t rsnxe_len;
};
...
/* Beacon interval and DTIM period last seen for a BSSID, from its beacon.
 * Returns false when that BSSID is not in the scan cache.
 */
