...
static void bflb_init_chan_config(struct me_chan_config_req *c)
{
	...
	for (uint8_t ch = 1U; ch <= BFLB_WIFI_CHAN_2G4_CNT; ch++) {
		c->chan2G4[ch - 1U].freq = bflb_wifi_chan_to_freq(ch);
...
int bflb_wifi_connect_req(struct bflb_wifi_dev *d, const struct bflb_wifi_connect_params *p)
{
	...
	req.chan.band = 0;
	req.chan.freq = bflb_wifi_chan_to_freq(p->channel);
	if (req.chan.freq == 0U) {
...
int bflb_wifi_disconnect_req(struct bflb_wifi_dev *d)
{
	...
	/* The interface is left in place: connecting removes it before
	 * adding a fresh one anyway, and tearing it down here races the
	 * disconnect the firmware is still processing -- it reschedules a
	 * timer whose owner has already gone.
	 */
	return 0;
}
