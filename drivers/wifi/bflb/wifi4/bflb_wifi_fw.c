...
#define BFLB_KEY_MGMT_FT_PSK     BIT(6)
#define BFLB_KEY_MGMT_PSK_SHA256 BIT(8)
...
#define BFLB_RSN_AKM_FT_PSK     4U
#define BFLB_RSN_AKM_PSK_SHA256 6U
...
#define BFLB_DISCONNECT_IND_WAIT_MS 1000U
...
#define BFLB_SM_FLAG_MFP_IN_USE          BIT(4)
#define BFLB_SM_FLAG_PMF_CAPABLE         BIT(9)
#define BFLB_SM_FLAG_PMF_REQUIRED        BIT(10)
#define BFLB_SM_FLAGS_PMF                                                                          \
	(BFLB_SM_FLAG_MFP_IN_USE | BFLB_SM_FLAG_PMF_CAPABLE | BFLB_SM_FLAG_PMF_REQUIRED)
...
/* Set by a disconnect the FW accepted, cleared when SM_DISCONNECT_IND lands. */
static bool bflb_disconnect_pending;
static K_SEM_DEFINE(bflb_disconnect_ind_sem, 0, 1);

/* The FW's station-manager environment; its first word is the connect
 * request the SM task is currently working on.
 */
extern struct sm_connect_req *sm_env[];
...
static void bridge_sta_config(wifi_connect_parm_t *p)
{
	...
	/* The FW wipes sm_connect_req.flags while matching a beacon against
	 * the connect request and restores only the control-port bits, so
	 * the PMF bits that make it accept an SAE AKM are gone by the next
	 * beacon of the same join scan -- the AP then stops matching and
	 * the join ends in status 14.  Put them back on every match.
	 */
	if (g_supp_ctx.conn_sae && (sm_env[0] != NULL)) {
		sm_env[0]->flags |= BFLB_SM_FLAGS_PMF;
	}
...
static int bridge_parse_wpa_ie(const uint8_t *ie, size_t len, wifi_wpa_ie_t *out)
{
	...
	count = sys_get_le16(pos);
	pos += 2;
	/* Every advertised AKM has to be reported: the FW picks SAE over PSK
	 * from this bitfield, so dropping the trailing suites of a
	 * WPA2/WPA3 transition AP would pin the connection to WPA2.
	 */
	for (i = 0U; (i < count) && ((pos + BFLB_RSN_SUITE_LEN) <= end);
	     i++, pos += BFLB_RSN_SUITE_LEN) {
		switch (pos[3]) {
		...
		case BFLB_RSN_AKM_FT_PSK:
			...
			break;
		case BFLB_RSN_AKM_PSK_SHA256:
			out->key_mgmt |= BFLB_KEY_MGMT_PSK_SHA256;
			break;
...
static void bflb_init_chan_config(struct me_chan_config_req *c)
{
	...
	for (uint8_t ch = 1U; ch <= BFLB_WIFI_CHAN_2G4_CNT; ch++) {
		c->chan2G4[ch - 1U].freq = wifi_utils_chan_to_freq(WIFI_FREQ_BAND_2_4_GHZ, ch);
		c->chan2G4[ch - 1U].band = 0;
		c->chan2G4[ch - 1U].flags = 0;
		c->chan2G4[ch - 1U].tx_power = BFLB_TX_POWER_DBM;
	}
	c->chan2G4_cnt = BFLB_WIFI_CHAN_2G4_CNT;
}
				    ...
				    const struct bflb_wifi_connect_params *p)
{
	...
	if (ap != NULL) {
		...
		/* Without a channel the firmware sweeps all of them looking
		 * for the BSS, which costs seconds on every connect.
		 */
		if (req->chan.freq == BFLB_CHAN_FREQ_ANY) {
			req->chan.freq = ap->center_freq;
		}
	} else {
...
int bflb_wifi_connect_req(struct bflb_wifi_dev *d, const struct bflb_wifi_connect_params *p)
{
	...
	/* Removing a VIF the FW is still tearing down leaves a queued timer
	 * with a wiped owner (FW asserts on timer->cb): wait for the IND.
	 */
	if (bflb_disconnect_pending) {
		bflb_disconnect_pending = false;
		ret = k_sem_take(&bflb_disconnect_ind_sem, K_MSEC(BFLB_DISCONNECT_IND_WAIT_MS));
		if (ret != 0) {
			LOG_WRN("no SM_DISCONNECT_IND before reconnect");
		}
	}
	...
	req.chan.band = 0;
	req.chan.freq = wifi_utils_chan_to_freq(WIFI_FREQ_BAND_2_4_GHZ, p->channel);
	if (req.chan.freq == 0U) {
	...
	/* The FW's join scan only considers an AP's SAE AKM when one of the
	 * PMF flags is set; without them a WPA3-only AP is skipped and the
	 * connect ends in status 14 (JOIN_NETWORK_FAIL).
	 */
	if (p->sae) {
		req.flags |= BFLB_SM_FLAGS_PMF;
	}
...
{
	struct sm_disconnect_req req = {.vif_idx = d->vif_idx};
	...
	/* The interface is left in place: connecting removes it once the
	 * FW has finished the disconnect (see bflb_wifi_connect_req).
	 */
	if ((ret == 0) && (cfm_status == 0U)) {
		k_sem_reset(&bflb_disconnect_ind_sem);
		bflb_disconnect_pending = true;
	}
	return 0;
}
			      ...
			      uint32_t len)
{
	...
	case SM_DISCONNECT_IND: {
		...
		k_sem_give(&bflb_disconnect_ind_sem);
