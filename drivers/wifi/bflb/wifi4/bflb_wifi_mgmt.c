			    ...
			    struct wifi_iface_status *status)
{
	...
	/* The driver only ever joins a BSS as a station, and the SoC is
	 * 2.4 GHz only.  The negotiated link mode is not reported by the
	 * firmware, and management frame protection is never negotiated.
	 */
