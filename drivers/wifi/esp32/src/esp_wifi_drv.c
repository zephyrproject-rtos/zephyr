...
static void esp32_wifi_arm_rssi_threshold(int threshold)
{
	...
	if (threshold < ESP32_WIFI_RSSI_THRESHOLD_MIN) {
		threshold = ESP32_WIFI_RSSI_THRESHOLD_MIN;
	}
...
#if defined(CONFIG_ESP32_WIFI_RRM_SUPPORT)
	/* Larger than the payload buffer, and the supplicant has already taken
	 * the APs it names into its candidate list. Drop it here so it does not
	 * look like a queue problem.
	 */
...
/* The supplicant consumes the report itself, adding the APs it names to its
 * candidate list, so issuing the request is all the driver does. The raw
 * elements it carries are not in the text form the Wi-Fi management layer takes.
 */
