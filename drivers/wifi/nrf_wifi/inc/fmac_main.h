...
/* Settle any pending or running oper-state work before the FMAC device context
 * goes away.
 */
static inline void nrf_wifi_settle_oper_state(struct nrf_wifi_vif_ctx_zep *vif_ctx_zep)
{
#ifdef NRF70_OPER_STATE_TRACKED
	struct k_work_sync sync;

	k_work_cancel_sync(&vif_ctx_zep->nrf_wifi_net_iface_work, &sync);
#else
	ARG_UNUSED(vif_ctx_zep);
#endif /* NRF70_OPER_STATE_TRACKED */
}
