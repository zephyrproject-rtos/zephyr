...
void SimpleLinkWlanEventHandler(SlWlanEvent_t *wlan_event)
{
	...
	case SL_WLAN_EVENT_CONNECT:
		...
		sl_conn.error = 0;
		nwp.cb(NET_EVENT_WIFI_CMD_CONNECT_RESULT, &sl_conn);
		break;
	...
	case SL_WLAN_EVENT_DISCONNECT:
		...
		/* Continue the notification callback chain... */
		nwp.cb(NET_EVENT_WIFI_CMD_DISCONNECT_RESULT, &sl_conn);
		break;
...
void SimpleLinkNetAppEventHandler(SlNetAppEvent_t *netapp_event)
{
	...
	case SL_DEVICE_EVENT_DROPPED_NETAPP_IPACQUIRED:
		...
		nwp.cb(NET_EVENT_IPV4_ADDR_ADD, &sl_conn);
		break;
	...
	case SL_DEVICE_EVENT_DROPPED_NETAPP_IPACQUIRED_V6:
		...
		for (i = 0; i < 3; i++) {
			SYS_LOG_INF("%04x:%04x:",
				    ((sl_conn.ipv6_addr[i] >> 16) & 0xffff),
				    sl_conn.ipv6_addr[i] & 0xffff);
		}
...
int _simplelink_scan(void)
{
	...
	/* Try 'MAX_SCAN_TRIALS' attempts to get results */
	while ((ret == SL_ERROR_WLAN_GET_NETWORK_LIST_EAGAIN) &&
	       (trials++ < CONFIG_WIFI_SIMPLELINK_MAX_SCAN_TRIALS)) {
		/* Wait for the NWP to complete the scan: */
		k_sleep(2000);

		/* Collect results form one-shot scans.*/
		ret = sl_WlanGetNetworkList(0,
					    CONFIG_WIFI_SIMPLELINK_SCAN_COUNT,
					    &nwp.net_entries[0]);
	}

	if (ret <= 0) {
		SYS_LOG_ERR("Unable to retrieve the scan list");
	}
...
int _simplelink_connect(struct wifi_connect_req_params *params)
{
	...
	secParams.Type = (params->security == WIFI_SECURITY_TYPE_PSK ?
			  SL_WLAN_SEC_TYPE_WPA_WPA2 : SL_WLAN_SEC_TYPE_OPEN);
