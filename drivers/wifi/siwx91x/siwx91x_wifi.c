...
static inline int siwx91x_z_to_sl_mode(uint8_t z_mode)
{
	switch (z_mode) {
	case WIFI_STA_MODE:
...
static inline int siwx91x_sl_to_z_mode(sl_wifi_interface_t interface)
{
	switch (interface) {
...
static int siwx91x_ap_enable(const struct device *dev, struct wifi_connect_req_params *params)
{
	...
	case WIFI_SECURITY_TYPE_NONE:
		siwx91x_ap_cfg.security = SL_WIFI_OPEN;
		sl_net_set_credential(siwx91x_ap_cfg.credential_id, default_wifi_ap_credential.type,
				      (const void *)default_wifi_ap_credential.data,
				      default_wifi_ap_credential.data_length);
		break;

	case WIFI_SECURITY_TYPE_WPA_PSK:
		siwx91x_ap_cfg.security = SL_WIFI_WPA;
		break;

	case WIFI_SECURITY_TYPE_PSK:
		siwx91x_ap_cfg.security = SL_WIFI_WPA2;
		break;
	...
	if (params->psk_length) {
		sl_net_set_credential(siwx91x_ap_cfg.credential_id, SL_NET_WIFI_PSK, params->psk,
				      params->psk_length);
	}
...
static int siwx91x_status(const struct device *dev, struct wifi_iface_status *status)
{
	...
	if (FIELD_GET(SIWX91X_INTERFACE_MASK, interface) == SL_WIFI_CLIENT_INTERFACE) {
		...
		sl_wifi_get_operational_statistics(SL_WIFI_CLIENT_INTERFACE,
...
static int siwx91x_mode(const struct device *dev, struct wifi_mode_info *mode)
{
	...
	if (mode->oper == WIFI_MGMT_SET) {
		if (siwx91x_z_to_sl_mode(mode->mode) < 0) {
			return -EINVAL;
		}

		oper_mode = siwx91x_z_to_sl_mode(mode->mode);
		ret = siwx91x_nwp_mode_switch(oper_mode);
		if (ret) {
			return ret;
		}

		sidev->state = WIFI_STATE_INACTIVE;
	} else if (mode->oper == WIFI_MGMT_GET) {
		interface = sl_wifi_get_default_interface();
		if (siwx91x_sl_to_z_mode(FIELD_GET(SIWX91X_INTERFACE_MASK, interface)) < 0) {
			return -EINVAL;
		}

		mode->mode = siwx91x_sl_to_z_mode(FIELD_GET(SIWX91X_INTERFACE_MASK, interface));
	}
