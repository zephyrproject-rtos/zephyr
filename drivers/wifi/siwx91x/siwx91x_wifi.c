				  ...
				  sl_wifi_advanced_scan_configuration_t *advanced_scan_config)
{
	...
	case SL_WIFI_SCAN_TYPE_PASSIVE:
		if (!dwell_time_passive) {
			...
			if (dwell_time_passive < 10 || dwell_time_passive > 1000) {
	...
	case SL_WIFI_SCAN_TYPE_ADV_SCAN:
		if (!dwell_time_active) {
			advanced_scan_config->active_channel_time =
				SL_WIFI_ADV_ACTIVE_SCAN_DURATION;
		} else {
			if (dwell_time_active < 5 || dwell_time_active > 1000) {
				LOG_ERR("Invalid active scan dwell time");
				return -EINVAL;
		...
		if (!dwell_time_passive) {
			advanced_scan_config->passive_channel_time =
				SL_WIFI_ADV_PASSIVE_SCAN_DURATION;
		} else {
			if (dwell_time_passive < 10 || dwell_time_passive > 1000) {
				LOG_ERR("Invalid passive scan dwell time");
				return -EINVAL;
			...
			scan_result_cb_t cb)
{
	...
	sl_wifi_advanced_scan_configuration_t advanced_scan_config = {
		.passive_channel_time = SL_WIFI_ADV_PASSIVE_SCAN_DURATION,
		.trigger_level = SL_WIFI_ADV_SCAN_THRESHOLD,
		.trigger_level_change = SL_WIFI_ADV_RSSI_TOLERANCE_THRESHOLD,
		.enable_multi_probe = SL_WIFI_ADV_MULTIPROBE,
		.enable_instant_scan = SL_WIFI_ENABLE_INSTANT_SCAN,
	};
	sl_wifi_roam_configuration_t roam_configuration = {.trigger_level = SL_WIFI_NEVER_ROAM,
	...
	if (sidev->state == WIFI_STATE_COMPLETED) {
...
#if IS_ENABLED(CONFIG_WIFI_SILABS_SIWX91X_ENABLE_ROAMING)
		roam_configuration.trigger_level = CONFIG_WIFI_SILABS_SIWX91X_ROAMING_TRIGGER_LEVEL;
		roam_configuration.trigger_level_change =
			CONFIG_WIFI_SILABS_SIWX91X_ROAMING_TRIGGER_LEVEL_CHANGE;
#endif
		...
		sl_scan_config.periodic_scan_interval = SL_WIFI_ADV_SCAN_PERIODICITY;
		...
		if (z_scan_config->scan_type == WIFI_SCAN_TYPE_ACTIVE) {
			...
			ret = siwx91x_configure_scan_dwell_time(
				SL_WIFI_SCAN_TYPE_PASSIVE, z_scan_config->dwell_time_active,
				z_scan_config->dwell_time_passive, NULL);
		}
