...
static int siwx91x_set_twt_setup(struct wifi_twt_params *params, sl_wifi_twt_request_t *twt_req)
{
...
static int siwx91x_set_twt_teardown(struct wifi_twt_params *params, sl_wifi_twt_request_t *twt_req)
{
	sl_status_t status = SL_STATUS_OK;
...
static int siwx91x_set_twt(const struct device *dev, struct wifi_twt_params *params)
{
	...
	ret = siwx91x_convert_z_sl_twt_req_type(params->setup_cmd);
	if (ret < 0) {
		params->fail_reason = WIFI_TWT_FAIL_CMD_EXEC_FAIL;
		return ret;
	}
	...
	if (params->operation == WIFI_TWT_SETUP) {
		ret = siwx91x_set_twt_setup(params, &twt_req);
		if (ret < 0) {
			return ret;
		}
	} else if (params->operation == WIFI_TWT_TEARDOWN) {
		ret = siwx91x_set_twt_teardown(params, &twt_req);
		if (ret < 0) {
			return ret;
		}
	}
