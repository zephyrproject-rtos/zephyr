		       ...
		       k_timeout_t timeout)
{
	...
	ret = net_pkt_alloc_buffer_with_reserve(pkt, size, SIWX91X_WIFI_SOCK_TX_HEADROOM,
						proto, timeout);
	return ret;
}
...
void siwx91x_sock_on_join_ipv6(const struct device *dev, struct net_if *iface)
{
	...
	net_if_ipv6_addr_add(iface, &lua, NET_ADDR_AUTOCONF, 0);
	net_if_ipv6_addr_add(iface, &gua, NET_ADDR_AUTOCONF, 0);
	net_if_ipv6_prefix_add(iface, &gua, prefix_len, 0xffffffffU);
	net_if_ipv6_router_add(iface, &gw, true, 0);
}
