			 ...
			 socklen_t addrlen)
{
	...
	ret = bind((int)context->offload_context, (struct sockaddr *)addr, addrlen);
	if (ret) {
		LOG_ERR("bind error %d %s!", ret, socket_message_to_string(ret));
		return ret;
	}
...
static int winc1500_listen(struct net_context *context, int backlog)
{
	...
	if (ret) {
		LOG_ERR("listen error %d %s!", ret, socket_error_string(ret));
		return ret;
	}

	if (k_sem_take(&w1500_data.socket_data[socket].wait_sem, WINC1500_LISTEN_TIMEOUT)) {
		return -ETIMEDOUT;
	}
			    ...
			    void *user_data)
{
	...
	if (ret) {
		LOG_ERR("connect error %d %s!", ret, socket_error_string(ret));
		return ret;
	}
			   ...
			   void *user_data)
{
	...
	if (ret) {
		LOG_ERR("accept error %d %s!", ret, socket_error_string(ret));
		return ret;
	}
			   ...
			   net_context_send_cb_t cb, int32_t timeout, void *user_data)
{
	...
	ret = sendto(socket, buf->data, buf->len, 0, (struct sockaddr *)dst_addr, addrlen);
	if (ret) {
			 ...
			 void *user_data)
{
	...
	if (ret) {
		LOG_ERR("recv error %d %s!", ret, socket_error_string(ret));
		return ret;
	}
