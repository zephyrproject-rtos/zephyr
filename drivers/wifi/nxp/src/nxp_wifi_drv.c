...
static atomic_t s_netif_init_count = ATOMIC_INIT(0);
...
int nxp_wifi_wlan_event_callback(enum wlan_event_reason reason, void *data)
{
	...
	case WLAN_REASON_UAP_SUCCESS:
		...
		if (bss_type == WLAN_BSS_TYPE_UAP) {
			...
			if (net_addr_pton(NET_AF_INET, CONFIG_NXP_WIFI_SOFTAP_IP_BASE,
			    &base_addr) < 0) {
...
#ifdef CONFIG_WIFI_NM_WPA_SUPPLICANT_P2P
		else if (bss_type == WLAN_BSS_TYPE_WIFIDIRECT) {
			LOG_DBG("WLAN: GO Started");
			if (net_addr_pton(NET_AF_INET, CONFIG_NXP_WIFI_P2P_IP_ADDRESS,
			    &dhcps_addr4) < 0) {
				LOG_ERR("Invalid CONFIG_NXP_WIFI_P2P_IP_ADDRESS");
				return 0;
			}
			net_if_ipv4_addr_add(iface, &dhcps_addr4, NET_ADDR_MANUAL, 0);
			net_if_ipv4_set_gw(iface, &dhcps_addr4);
			...
			if (net_addr_pton(NET_AF_INET, CONFIG_NXP_WIFI_P2P_IP_MASK,
			    &netmask_addr) < 0) {
				LOG_ERR("Invalid CONFIG_NXP_WIFI_P2P_IP_MASK");
				return 0;
			}
			net_if_ipv4_set_netmask_by_addr(iface, &dhcps_addr4, &netmask_addr);
			net_if_dormant_off(iface);
			...
			if (net_addr_pton(NET_AF_INET, CONFIG_NXP_WIFI_P2P_IP_BASE,
			    &base_addr) < 0) {
				LOG_ERR("Invalid CONFIG_NXP_WIFI_P2P_IP_BASE");
				return 0;
			}
		}
#endif
...
/**
 * Called from each iface_api.init callback.
...
#ifdef CONFIG_WIFI_NM_WPA_SUPPLICANT_P2P
	if (iface == (struct net_if *)net_get_wfd_interface()) {
		return BIT(WIFI_TYPE_P2P);
	}
#endif
