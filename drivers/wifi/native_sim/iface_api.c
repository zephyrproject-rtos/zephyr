...
{
	memcpy(ctx->ll_addr.addr, ctx->mac_addr, sizeof(ctx->mac_addr));
					...
					int count, uint16_t *vlan_tag, int *status)
{
	...
	if (IS_ENABLED(CONFIG_ETH_NATIVE_TAP_VLAN_TAG_STRIP)) {
...
static int read_data(struct wifi_context *ctx, int fd)
{
	...
	count = wifi_read_data(fd, ctx->recv, sizeof(ctx->recv));
	if (count <= 0) {
		return 0;
	}
...
{
	/* Large enough for the biggest event datagram (assoc with two IE
	 * blobs). A SOCK_DGRAM read truncates anything beyond this.
	 */
	uint8_t evbuf[2 * HOST_WIFI_EVENT_MAX_IES + 64];

	LOG_DBG("Starting RX thread");

	while (1) {
		/* Read whenever the interface is admin-up. This must not depend
		 * on net_if_is_up()/carrier/dormant: EAPOL (the 4-way handshake)
		 * has to be received while the interface is still dormant, before
		 * the connection completes.
		 */
		if (net_if_is_admin_up(ctx->iface)) {
			while (!wifi_wait_data(ctx->dev_fd)) {
				read_data(ctx, ctx->dev_fd);
				k_yield();
			}
		}

		/* Drain events forwarded by the host nl80211 driver (scan done,
		 * and in later stages auth/assoc/etc.). Available once the
		 * supplicant has initialized the host driver.
		 */
		if (ctx->event_fd >= 0) {
			while (!wifi_wait_data(ctx->event_fd)) {
				ssize_t n = wifi_read_data(ctx->event_fd, evbuf, sizeof(evbuf));

				if (n <= 0) {
					break;
				}

				wifi_drv_event_dispatch(ctx, evbuf, (size_t)n);
				k_yield();
			}
		}

		k_sleep(K_MSEC(CONFIG_WIFI_NATIVE_SIM_RX_WAIT_TIME));
	}
}
...
void wifi_if_init(struct net_if *iface)
{
	...
	struct net_linkaddr *ll_addr = wifi_get_mac(ctx);
	...
	/* The host event pipe fd is set once the supplicant initializes the
	 * host driver (wifi_drv_init). Until then the RX thread must not poll it.
	 */
	ctx->event_fd = -1;
	...
	/* Use the same MAC as the host interface. The AP associates and
	 * addresses the STA using the host radio MAC, so the Zephyr interface
	 * must share it; otherwise received data frames (incl. the EAPOL 4-way
	 * handshake) are dropped by the Ethernet L2 as "not for me" and frames
	 * we transmit are rejected by the AP. Fall back to the generated MAC if
	 * the host interface cannot be queried.
	 */
	if (wifi_iface_get_mac(ctx->if_name_host, ctx->mac_addr) < 0) {
		LOG_WRN("Cannot read host MAC for %s, using generated address",
			ctx->if_name_host);
	}

	/* Refresh the link address from the (possibly updated) MAC. */
	ll_addr = wifi_get_mac(ctx);

	net_if_set_link_addr(iface, ll_addr->addr, ll_addr->len,
			     NET_LINK_ETHERNET);

	ctx->dev_fd = wifi_iface_create(ctx->if_name_host);
	if (ctx->dev_fd < 0) {
	...
	/* The simulated radio link is always present, so bring the carrier up
	 * and mark the interface dormant. It leaves dormant (becomes fully up)
	 * only once the supplicant authorizes the controlled port on a
	 * successful connection (see wifi_drv_set_supp_port()).
	 */
		       ...
		       const struct ethernet_config *config)
{
	...
	if (IS_ENABLED(CONFIG_NET_PROMISCUOUS_MODE) &&
		...
		ret = wifi_promisc_mode(context->if_name_host,
...
int wifi_if_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	ret = wifi_write_data(ctx->dev_fd, ctx->send, count);
	if (ret < 0) {
