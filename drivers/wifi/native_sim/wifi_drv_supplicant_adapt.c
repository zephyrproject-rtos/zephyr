...
struct api_ctx {
	...
	/* Posted by the handler thread once it has finished initializing (either
	 * successfully or on failure), so host_wifi_drv_init() can wait for it.
	 */
	sem_t init_sem;
 ...
 */
static int run_on_eloop(struct api_ctx *ctx, int (*fn)(struct api_ctx *ctx, void *arg),
			void *arg)
{
...
static void send_auth(struct api_ctx *ctx, union wpa_event_data *data)
{
	...
	size_t ies_len = NSI_MIN(data->auth.ies_len, HOST_WIFI_EVENT_MAX_IES);
	size_t off = 0;
...
static void send_assoc(struct api_ctx *ctx, union wpa_event_data *data)
{
	...
	size_t req_len = NSI_MIN(data->assoc_info.req_ies_len, HOST_WIFI_EVENT_MAX_IES);
	size_t resp_len = NSI_MIN(data->assoc_info.resp_ies_len, HOST_WIFI_EVENT_MAX_IES);
	size_t off = 0;
...
static void send_assoc_reject(struct api_ctx *ctx, union wpa_event_data *data)
{
	...
	size_t resp_len = NSI_MIN(data->assoc_reject.resp_ies_len, HOST_WIFI_EVENT_MAX_IES);
	size_t off = 0;
			...
			const u8 *ie, size_t ie_in_len)
{
	...
	size_t ie_len = NSI_MIN(ie_in_len, HOST_WIFI_EVENT_MAX_IES);
...
static void send_rx_mgmt(struct api_ctx *ctx, union wpa_event_data *data)
{
	...
	size_t frame_len = NSI_MIN(data->rx_mgmt.frame_len, HOST_WIFI_EVENT_MAX_IES);
	size_t off = 0;
...
static void *host_handler(void *args)
{
	...
	/* The host side nl80211 driver logs through hostap's wpa_printf(), which
	 * is controlled by the wpa_debug_level global and writes to the file set
	 * by wpa_debug_open_file(). Redirect it to a file so that it does not mix
	 * with the Zephyr console output.
	 */
	wpa_debug_level = ctx->wpa_debug_level;

	if (ctx->wpa_debug_file_path != NULL && ctx->wpa_debug_file_path[0] != '\0' &&
	    wpa_debug_open_file(ctx->wpa_debug_file_path) < 0) {
		nsi_print_warning("%s: cannot open %s, logging to stdout\n", __func__,
				  ctx->wpa_debug_file_path);
	}

	if (eloop_init()) {
		nsi_print_warning("%s: Failed to init eloop\n", __func__);
		goto out;
	}
	...
	if (ctx->drv->global_init) {
		...
		if (ctx->drv_global_priv == NULL) {
			nsi_print_warning("%s: driver global_init failed\n", __func__);
			goto out;
		}
	}
	...
	if (ctx->drv_priv == NULL) {
		nsi_print_warning("%s: driver init2 failed for %s\n", __func__,
				  ctx->ifname);
		goto out;
	}
	...
	if (eloop_register_read_sock(ctx->cmd_fd[1], cmd_handler, ctx, NULL)) {
		nsi_print_warning("%s: failed to register command socket\n", __func__);
		goto out;
	}

	ctx->handler_started = true;

	/* Init done: unblock host_wifi_drv_init() before we block in eloop_run(). */
	sem_post(&ctx->init_sem);
...
out:
	if (!ctx->handler_started) {
		/* Init failed before eloop_run(): flag it and unblock the waiter. */
	...
	/* Returning from the pthread start routine terminates the thread; the
	 * value is discarded (the joiner passes NULL for the exit status).
	 */
	return NULL;
}
...
void *host_wifi_drv_init(void *context, const char *iface_name,
			 const char *debug_file_path, int wpa_debug_level)
{
	...
	if (socketpair(AF_UNIX, SOCK_DGRAM, 0, args.event_fd) < 0) {
		nsi_print_warning("%s: socketpair: %s\n", __func__, strerror(errno));
		return NULL;
	}
	...
	/* Keep the thread joinable so deinit can wait for clean shutdown. */
	...
	/* Wait (up to 10 s) for the handler thread to finish initializing. */
	if (clock_gettime(CLOCK_REALTIME, &ts) == 0) {
		ts.tv_sec += 10;
		while (sem_timedwait(&args.init_sem, &ts) != 0 && errno == EINTR) {
		}
	}

	if (!args.handler_started) {
		nsi_print_warning("%s: nl80211 driver failed to start\n", __func__);
		ret = -1;
		goto out;
	}
...
void host_wifi_drv_deinit(void *priv)
{
	...
	if (ret != 0) {
		nsi_print_warning("%s: %s: %s (%d)\n", __func__, "pthread_join",
				  strerror(ret), -ret);
	}

	sem_destroy(&ctx->init_sem);
...
int host_wifi_drv_get_wiphy(void *priv, void **band_arg, int *count)
{
	...
	*count = num_modes;
	...
	for (i = 0; i < num_modes; i++) {
		...
		band->wpa_supp_n_channels = NSI_MIN(modes[i].num_channels,
						    WPA_SUPP_SBAND_MAX_CHANNELS);
		band->wpa_supp_n_bitrates = NSI_MIN(modes[i].num_rates,
						    WPA_SUPP_SBAND_MAX_RATES);
		...
		for (j = 0;
		     j < NSI_MIN(band->wpa_supp_n_bitrates, WPA_SUPP_SBAND_MAX_RATES);
		     j++) {
				 ...
				 bool multicast)
{
	NSI_ARG_UNUSED(priv);
	NSI_ARG_UNUSED(type);
	NSI_ARG_UNUSED(match);
	NSI_ARG_UNUSED(match_len);
	NSI_ARG_UNUSED(multicast);
			    ...
			    unsigned int wait_time, int cookie)
{
	...
	NSI_ARG_UNUSED(no_cck);
	NSI_ARG_UNUSED(offchanok);
	NSI_ARG_UNUSED(cookie);
