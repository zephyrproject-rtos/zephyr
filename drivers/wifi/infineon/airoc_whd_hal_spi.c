...
#if defined(CONFIG_AIROC_WIFI_BUS_SPI_DATA_IRQ_MUX) && defined(CONFIG_BOARD_RPI_PICO_RP2040_W)
#define PICO_BUS_DEVICE DT_PARENT(DT_PARENT(DT_DRV_INST(0)))

#if PICO_BUS_DEVICE == DT_NODELABEL(pio0)
#define PICO_PIO_HOST GPIO_FUNC_PIO0
#elif PICO_BUS_DEVICE == DT_NODELABEL(pio1)
#define PICO_PIO_HOST GPIO_FUNC_PIO1
#elif PICO_BUS_DEVICE == DT_NODELABEL(pio2)
#define PICO_PIO_HOST GPIO_FUNC_PIO2
#endif
...
whd_result_t whd_bus_spi_irq_enable(whd_driver_t whd_driver, whd_bool_t enable)
{
	...
	if (enable) {
		...
		__ASSERT(ret == 0, "gpio_pin_interrupt_configure_dt failed, return code %d", ret);
#if defined(CONFIG_AIROC_WIFI_BUS_SPI_DATA_IRQ_MUX) && defined(CONFIG_BOARD_RPI_PICO_RP2040_W)
		gpio_set_function(gpio->pin, PICO_PIO_HOST);
#endif
	}
