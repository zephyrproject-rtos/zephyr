...
struct renesas_ra_ssie_data {
	...
	struct i2s_buf rx_msgs[CONFIG_I2S_RENESAS_RA_SSIE_TX_BLOCK_COUNT];
	bool tx_configured;
	bool rx_configured;
	...
	transfer_info_t DTC_TRANSFER_INFO_ALIGNMENT rx_transfer_info;
	...
	transfer_info_t DTC_TRANSFER_INFO_ALIGNMENT tx_transfer_info;
					     ...
					     ssi_extended_cfg_t *fsp_ext_cfg)
{
	...
	ssi_clock_div_t bit_clock_div = (ssi_clock_div_t)-1;
	...
	for (size_t i = 0; i < VALID_DIVISOR_COUNT; ++i) {
		div = valid_divisors[i];
		actual_bclk = rate / div;
	...
	if (bit_clock_div == (ssi_clock_div_t)-1) {
		return -EINVAL;
	}
...
static void renesas_ra_ssie_idle_dir_both_handle(const struct device *dev)
{
	...
	if (dev_data->state == I2S_STATE_STOPPING) {
		if (dev_data->active_dir == I2S_DIR_BOTH) {
			...
			if (k_msgq_num_used_get(&dev_data->tx_queue) == 0) {
				...
				/*
				 * If trigger command is I2S_TRIGGER_STOP, stop the transfer
				 * immediately. If trigger command is I2S_TRIGGER_DRAIN, send all
				 * msg in tx queue then stop.
				 */
...
static void renesas_ra_ssie_rx_callback(const struct device *dev)
{
	...
	struct i2s_buf msg_item_rx = {.mem_block = stream->mem_block,
				      .mem_block_len = stream->mem_block_len};
...
static void renesas_ra_ssie_tx_callback(const struct device *dev)
{
	...
	if (dev_data->active_dir == I2S_DIR_TX) {
		if (dev_data->state == I2S_STATE_STOPPING) {
			...
			if (k_msgq_num_used_get(&dev_data->tx_queue) == 0) {
				goto tx_disable;
				/* If trigger STOP, send the current data block and stop the
				 * transfer immediately If trigger DRAIN, send all data in tx queue
...
static int i2s_renesas_ra_ssie_write(const struct device *dev, void *mem_block, size_t size)
{
	...
	ret = k_msgq_put(&dev_data->tx_queue, &msg_item,
			 SYS_TIMEOUT_MS(dev_data->tx_stream.cfg.timeout));
	if (ret < 0) {
		return ret;
	}
...
static int i2s_renesas_ra_ssie_read(const struct device *dev, void **mem_block, size_t *size)
{
	...
	ret = k_msgq_get(&dev_data->rx_queue, &msg_item,
			 (dev_data->state == I2S_STATE_ERROR)
				 ? K_NO_WAIT
				 : SYS_TIMEOUT_MS(dev_data->rx_stream.cfg.timeout));
	if (ret == -ENOMSG) {
		return -EIO;
	}
...
#define RA_SSIE_RX_IRQ_BY_NAME(index, prop)                                                        \
	COND_CODE_1(										\
		DT_INST_PROP(index, full_duplex),				\
		(DT_INST_IRQ_BY_NAME(index, ssi_rxi, prop)),	\
		(DT_INST_IRQ_BY_NAME(index, ssi_rt, prop)))

#define RA_SSIE_TX_IRQ_BY_NAME(index, prop)                                                        \
	COND_CODE_1(							\
		DT_INST_PROP(index, full_duplex),	\
		(DT_INST_IRQ_BY_NAME(index, ssi_txi, prop)),	\
		(DT_INST_IRQ_BY_NAME(index, ssi_rt, prop)))
...
#define SSIE_DTC_TX_SOURCE(index)                                                                  \
	COND_CODE_1(		\
		DT_INST_PROP(index, full_duplex),	\
		(DT_INST_IRQ_BY_NAME(index, ssi_txi, irq)),		\
		(DT_INST_IRQ_BY_NAME(index, ssi_rt, irq)))

#define SSIE_DTC_RX_SOURCE(index)                                                                  \
	COND_CODE_1(	\
		DT_INST_PROP(index, full_duplex),	\
		(DT_INST_IRQ_BY_NAME(index, ssi_rxi, irq)),		\
		(DT_INST_IRQ_BY_NAME(index, ssi_rt, irq)))
...
#define SSIE_RA_IRQ_INIT(index)                                                                    \
	COND_CODE_1(										\
	DT_INST_PROP(index, full_duplex),					\
	(SSIE_RA_FULL_DUPLEX_INIT(index)),					\
...
#define SSIE_RA_INIT(index)                                                                        \
	...
	static struct renesas_ra_ssie_data renesas_ra_ssie_data_##index = {                        \
		...
		.fsp_cfg = {.channel = DT_INST_PROP(index, channel),                               \
			    ...
			    .pcm_width = I2S_WORD_LENGTH_16_BITS,                                  \
			    .word_length = I2S_PCM_WIDTH_16_BITS,                                  \
