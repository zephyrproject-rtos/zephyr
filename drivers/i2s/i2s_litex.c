...
static int queue_get(struct ring_buffer *rb, void **mem_block, size_t *size)
{
	...
	key = irq_lock();
	if (ring_buffer_size(rb) < sizeof(item)) {
		/* Ring buffer is empty */
		irq_unlock(key);
		return -ENOMEM;
	}
	ring_buffer_read(rb, (uint8_t *)&item, sizeof(item));
...
static int queue_put(struct ring_buffer *rb, void *mem_block, size_t size)
{
	...
	key = irq_lock();
	if (ring_buffer_space(rb) < sizeof(item)) {
		/* Ring buffer is full */
		irq_unlock(key);
		return -ENOMEM;
	}

	item.mem_block = mem_block;
	item.size = size;
	ring_buffer_write(rb, (uint8_t *)&item, sizeof(item));
	irq_unlock(key);
	return 0;
}
...
#define I2S_INIT(dir, size)							\
	RING_BUFFER_DEFINE(queue_##dir, size);					\
	static struct i2s_litex_data i2s_litex_data_##dir = {			\
		.dir.mem_block_queue = &queue_##dir,				\
	};									\
										\
