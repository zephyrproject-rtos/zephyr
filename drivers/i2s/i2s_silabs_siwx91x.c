...
#define SIWX91X_I2S_INIT(inst)                                                                     \
	...
	struct i2s_siwx91x_queue_item                                                              \
		tx_ring_buf_##inst[CONFIG_I2S_SILABS_SIWX91X_TX_BLOCK_COUNT + 1];                  \
                                                                                                   \
	BUILD_ASSERT(((I2S0_Type *)DT_INST_REG_ADDR(inst) != I2S0) ||                              \
			     (DT_INST_PROP(inst, silabs_channel_group) < 2),                       \
		     "Invalid channel group!");                                                    \
	BUILD_ASSERT(((I2S0_Type *)DT_INST_REG_ADDR(inst) != I2S1) ||                              \
			     (DT_INST_PROP(inst, silabs_channel_group) < 1),                       \
		     "Invalid channel group!");                                                    \
                                                                                                   \
