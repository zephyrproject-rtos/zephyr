...
#define DT_DRV_COMPAT silabs_i2s
...
static int i2s_siwx91x_queue_put(struct ring_buffer *rb, void *mem_block, size_t size)
{
	uint16_t head_next;
	unsigned int key;
...
static int i2s_siwx91x_queue_get(struct ring_buffer *rb, void **mem_block, size_t *size)
{
	unsigned int key;
				  ...
				  uint32_t block_count, bool is_tx, uint8_t xfer_size)
{
	struct dma_config cfg = {
		...
		.dma_callback = is_tx ? &dma_tx_callback : &dma_rx_callback,
		.user_data = (void *)dev,
	};
					   ...
					   uint32_t blk_size, uint32_t dma_channel, bool is_tx)
{
	...
	if ((ARRAY_INDEX(stream->dma_descriptors, desc) + 1) == 1) {
...
static void dma_rx_callback(const struct device *dma_dev, void *user_data, uint32_t channel,
			    int status)
{
	const struct device *i2s_dev = (const struct device *)user_data;
	const struct i2s_siwx91x_config *cfg = i2s_dev->config;
	struct i2s_siwx91x_data *data = i2s_dev->data;
...
static void i2s_siwx91x_start_tx(const struct device *dev)
{
	...
	if (cfg->channel_group == 0) {
		cfg->reg->CHANNEL_CONFIG[1].I2S_TER_b.TXCHEN = 0;
	} else {
		cfg->reg->CHANNEL_CONFIG[0].I2S_TER_b.TXCHEN = 0;
	}
}
...
static void i2s_siwx91x_start_rx(const struct device *dev)
{
	...
	if (cfg->channel_group == 0) {
		cfg->reg->CHANNEL_CONFIG[1].I2S_RER_b.RXCHEN = 0;
	} else {
		cfg->reg->CHANNEL_CONFIG[0].I2S_RER_b.RXCHEN = 0;
	}
}
...
static int i2s_siwx91x_init(const struct device *dev)
{
	...
	if ((cfg->reg == I2S0) && (cfg->channel_group > 1)) {
		return -EINVAL;
	}

	if ((cfg->reg == I2S1) && (cfg->channel_group != 0)) {
		return -EINVAL;
	}
	...
	k_sem_init(&data->rx.sem, 0, CONFIG_I2S_SIWX91X_RX_BLOCK_COUNT);
	k_sem_init(&data->tx.sem, CONFIG_I2S_SIWX91X_TX_BLOCK_COUNT,
		   CONFIG_I2S_SIWX91X_TX_BLOCK_COUNT);
...
#define SIWX91X_I2S_INIT(inst)                                                                     \
	...
	struct queue_item rx_ring_buf_##inst[CONFIG_I2S_SIWX91X_RX_BLOCK_COUNT + 1];               \
	struct queue_item tx_ring_buf_##inst[CONFIG_I2S_SIWX91X_TX_BLOCK_COUNT + 1];               \
                                                                                                   \
	static struct i2s_siwx91x_data i2s_data_##inst = {                                         \
		.rx =                                                                              \
			{                                                                          \
				...
				.stream_start = i2s_siwx91x_rx_stream_start,                       \
				.queue_drop = i2s_siwx91x_rx_queue_drop,                           \
			},                                                                         \
		.tx =                                                                              \
			{                                                                          \
				...
				.stream_start = i2s_siwx91x_tx_stream_start,                       \
				.queue_drop = i2s_siwx91x_tx_queue_drop,                           \
			},                                                                         \
	};                                                                                         \
