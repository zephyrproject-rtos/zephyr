...
static const char *ns_i2s_active_path(const struct ns_i2s_config *cfg,
				      const struct ns_i2s_data *data,
				      enum i2s_dir dir)
{
	if ((dir == I2S_DIR_RX) && (data != NULL) && (data->cmd_rx_path != NULL)) {
		return data->cmd_rx_path;
	}

	if ((dir == I2S_DIR_TX) && (data != NULL) && (data->cmd_tx_path != NULL)) {
...
static void ns_i2s_stream_ready(struct ns_i2s_stream *stream)
{
	...
	stream->last_error = 0;
	stream->missing_input_file = false;
			       ...
			       struct ns_i2s_stream *stream, enum i2s_dir dir)
{
	...
	if ((stream->fd >= 0) || stream->missing_input_file) {
		return 0;
	}
	...
	if (dir == I2S_DIR_RX) {
		fd = ns_i2s_open_rx_file_bottom(path);
	} else {
		fd = ns_i2s_open_tx_file_bottom(path);
	}
	...
	if (fd < 0) {
		...
		if ((dir == I2S_DIR_RX) && ((fd == -ENOENT) || (fd == -ENOTDIR))) {
			stream->missing_input_file = true;
...
static bool ns_i2s_process_tx(const struct device *dev)
{
	...
	if (ns_i2s_prepare_file(cfg, data, &data->tx, I2S_DIR_TX) < 0) {
...
static bool ns_i2s_process_rx_file(const struct device *dev)
{
	...
	if (ns_i2s_prepare_file(cfg, data, &data->rx, I2S_DIR_RX) < 0) {
...
static int ns_i2s_validate_cfg(const struct ns_i2s_config *cfg, const struct ns_i2s_data *data,
			       enum i2s_dir dir, const struct i2s_config *i2s_cfg)
{
	...
	if (!ns_i2s_path_valid(ns_i2s_active_path(cfg, data, dir))) {
		return -EINVAL;
	}
			    ...
			    const struct i2s_config *i2s_cfg)
{
	...
	ret = ns_i2s_validate_cfg(cfg, data, dir, i2s_cfg);
	if (ret < 0) {
		k_mutex_unlock(&data->lock);
		return ret;
	}
...
static int ns_i2s_trigger(const struct device *dev, enum i2s_dir dir, enum i2s_trigger_cmd cmd)
{
	...
	case I2S_TRIGGER_START:
		...
		if ((dir == I2S_DIR_RX) && !ns_i2s_loopback_enabled(data)) {
			ret = ns_i2s_prepare_file(cfg, data, stream, dir);
			if (ret < 0) {
				k_mutex_unlock(&data->lock);
				return ret;
			}
...
#define NS_I2S_COMMAND_LINE_OPTS(inst) \
	{ \
		...
		.descript = "Path to binary file to be used as native_sim " \
			    ...
			    ". Overrides " \
			    "CONFIG_I2S_NATIVE_SIM_RX_FILE_PATH if set", \
	}, \
	{ \
		...
		.descript = "Path to binary file to be used as native_sim " \
			    ...
			    ". Overrides " \
			    "CONFIG_I2S_NATIVE_SIM_TX_FILE_PATH if set", \
	},

static void ns_i2s_options(void)
{
	static struct args_struct_t ns_i2s_options[] = {
		DT_INST_FOREACH_STATUS_OKAY(NS_I2S_COMMAND_LINE_OPTS) ARG_TABLE_ENDMARKER,
	};
