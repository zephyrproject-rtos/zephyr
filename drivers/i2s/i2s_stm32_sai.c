...
struct stream {
	...
	void (*stream_stop)(struct stream *, const struct device *dev);
...
void HAL_SAI_RxCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	if (HAL_SAI_Receive_DMA(hsai, stream->mem_block, stream->mem_block_len / 2) != HAL_OK) {
...
void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	if (HAL_SAI_Transmit_DMA(hsai, stream->mem_block, stream->mem_block_len / 2) != HAL_OK) {
		LOG_ERR("HAL_SAI_Transmit_DMA: <FAILED>");
	}
...
static int stream_start(struct stream *stream, const struct device *dev, enum i2s_dir dir)
{
	...
	if (dir == I2S_DIR_TX) {
		...
		/* TODO: FIX /2 */
		if (HAL_SAI_Transmit_DMA(hsai, stream->mem_block, stream->mem_block_len / 2) !=
		    HAL_OK) {
			LOG_ERR("HAL_SAI_Transmit_DMA: <FAILED>");
			return -EIO;
		}
		...
		if (HAL_SAI_Receive_DMA(hsai, stream->mem_block, stream->mem_block_len / 2) !=
		    HAL_OK) {
			LOG_ERR("HAL_SAI_Transmit_DMA: <FAILED>");
			return -EIO;
		}
...
static void stream_stop(struct stream *stream, const struct device *dev)
{
	LOG_DBG("Stream Stopped");

	LOG_ERR("RX queue: %d", k_msgq_num_used_get(&stream->queue));
				 ...
				 enum i2s_trigger_cmd cmd)
{
	...
	if (stream->state == I2S_STATE_NOT_READY) {
		LOG_ERR("I2S_STATE_NOT_READY");

	} else if (stream->state == I2S_STATE_READY) {
		LOG_ERR("I2S_STATE_READY");

	} else if (stream->state == I2S_STATE_RUNNING) {
		LOG_ERR("I2S_STATE_RUNNING");

	} else if (stream->state == I2S_STATE_STOPPING) {
		LOG_ERR("I2S_STATE_STOPPING");

	} else if (stream->state == I2S_STATE_ERROR) {
		LOG_ERR("I2S_STATE_ERROR");
	}
