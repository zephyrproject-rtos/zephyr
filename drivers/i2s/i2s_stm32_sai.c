...
static int i2s_stm32_sai_dma_init(const struct device *dev)
{
	...
	LOG_DBG("DMA channel %d configured", stream->dma_channel + STM32_DMA_STREAM_OFFSET);

/* DMA Settings for GPDMA - STM32U5xx & STM32H5xx */
	...
	/* TODO: FIX BASED ON THE I2S CONFIG WORD SIZE */
...
#else
	LOG_ERR("DMA Supports only GPDMA - STM32U5xx");
	return -EIO;
#endif
	return 0;
}
	...
	struct i2s_stm32_sai_data sai##index_data = {                                              \
		...
		COND_CODE_1(DT_INST_DMAS_HAS_NAME(index, tx),                                       \
			...
			(SAI_DMA_CHANNEL_INIT(index, rx, PERIPHERAL, MEMORY))),                      \
                                                                                                   ...
                                                                                                   \
	struct i2s_stm32_sai_cfg sai##index_config = {                                             \
		...
		.mclk_nodiv = DT_INST_PROP(index, no_divider),                                     \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, &i2s_stm32_sai_initialize, NULL, &sai##index_data,            \
