...
void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
	...
	if (stream->state == I2S_STATE_STOPPING) {
		if (k_msgq_num_used_get(&stream->queue) == 0) {
			// LOG_WRN("k_msgq_num_used_get(&stream->queue) == 0");
			stream->state = I2S_STATE_READY;
			goto exit;
		}
	}
	...
	if (ret < 0) {
		stream->state = I2S_STATE_ERROR;
		// LOG_WRN("k_msgq_get(&stream->queue, &item, K_NO_WAIT)");
		goto exit;
	}
...
#define I2S_STM32_SAI_INIT(index)                                                                  \
	...
	struct i2s_stm32_sai_cfg sai_config_##index = {                                            \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(                                            \
			index), /*.irq_config = i2s_stm32_irq_config_func_##index, */              \
	...
	/*	static void i2s_stm32_irq_config_func_##index(const struct device *dev)                 \
	   ...
	   i2s_stm32_isr, DEVICE_DT_INST_GET(index), 0); irq_enable(DT_INST_IRQN(index));          \
		}*/
