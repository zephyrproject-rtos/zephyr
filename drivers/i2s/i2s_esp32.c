...
#if !SOC_GDMA_SUPPORTED
	/* Direct (non-GDMA) peripheral interrupt: multilevel-encoded IRQ. */
	unsigned int irq;
...
#if !SOC_GDMA_SUPPORTED
static void IRAM_ATTR i2s_esp32_isr(const void *arg)
{
	const struct device *dev = (const struct device *)arg;
	const struct i2s_esp32_cfg *const dev_cfg = dev->config;
	const i2s_hal_context_t *hal = &(dev_cfg->hal);
...
#define IRQ_CONFIG_FUNC(idx)                                                                       \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN_BY_IDX(idx, 0), IRQ_DEFAULT_PRIORITY, i2s_esp32_isr,       \
			    DEVICE_DT_INST_GET(idx), ESP_INTR_FLAG_IRAM);                          \
	}
#define IRQ_CONFIG(idx) .irq = DT_INST_IRQN_BY_IDX(idx, 0),                                        \
			.irq_configure = i2s_esp32_##idx##_irq_configure
#else
#define IRQ_CONFIG_FUNC(idx)
