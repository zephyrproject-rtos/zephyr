...
#endif /* SOC_GDMA_SUPPORTED */
{
	...
	if (status & I2S_LL_EVENT_TX_DSCR_ERR) {
		...
		/*
		 * Leave mem_block set. A failed transfer is precisely the case where the
		 * channel may not have stopped, so the block is not this callback's to
		 * give away: i2s_esp32_tx_stop_transfer() below frees it, and only once
		 * dma_stop() has reported success. RX already works this way -- it checks
		 * the status before it hands its block on. On ESP32 and ESP32-S2 the block
		 * joins the deliberate leak documented in that function, which is the same
		 * trade this series already makes there.
		 */
		goto tx_disable;
	}
