				   ...
				   const struct i2s_config *i2s_cfg)
{
	...
	if (i2s_cfg->options & I2S_OPT_LOOPBACK) {
		LOG_DBG("For internal loopback: I2S#_O_SD_GPIO = I2S#_I_SD_GPIO");
		return -EINVAL;
	}
	...
	if ((i2s_cfg->options & I2S_OPT_FRAME_CLK_SLAVE) != 0 &&
		...
		LOG_DBG("I2S_OPT_FRAME_CLK and I2S_OPT_BIT_CLK options must both be"
			" MASTER or SLAVE");
		return -EINVAL;
	}
			       ...
			       const struct i2s_config *i2s_cfg)
{
	...
	case I2S_DIR_RX:
		...
		if (stream) {
			...
			LOG_DBG("I2S_DIR_RX not enabled");
			err = -EINVAL;
		}
#else
		LOG_DBG("I2S_DIR_RX not enabled");
	...
	case I2S_DIR_TX:
		...
		if (stream) {
			...
			LOG_DBG("I2S_DIR_TX not enabled");
			err = -EINVAL;
		}
#else
		LOG_DBG("I2S_DIR_TX not enabled");
...
static int i2s_esp32_read(const struct device *dev, void **mem_block, size_t *size)
{
	...
	if (state == I2S_STATE_NOT_READY) {
		LOG_DBG("RX invalid state: %d", (int)stream->data->state);
		return -EIO;
	}
...
static int i2s_esp32_write(const struct device *dev, void *mem_block, size_t size)
{
	...
	if (state != I2S_STATE_RUNNING && state != I2S_STATE_READY) {
		LOG_DBG("TX Invalid state: %d", (int)state);
		return -EIO;
	}
...
#define I2S_ESP32_DT_INST_SANITY_CHECK(index)                                                      \
	...
	BUILD_ASSERT(!DT_INST_NODE_HAS_PROP(index, interrupts),                                    \
		     "Unexpected property: interrupts");                                           \
	BUILD_ASSERT(!DT_INST_NODE_HAS_PROP(index, interrupt_names),                               \
		     "Unexpected property: interrupt-names")
...
#define I2S_ESP32_DT_INST_SANITY_CHECK(index)                                                      \
	...
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(index, interrupts), "Missing property: interrupts");    \
	BUILD_ASSERT(DT_INST_NODE_HAS_PROP(index, interrupt_names),                                \
