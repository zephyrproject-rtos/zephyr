...
static inline int start_stream(const struct i2s_max32_stream *stream, enum i2s_dir dir)
{
	...
	if (dir == I2S_DIR_RX) {
		Wrap_MXC_I2S_SetDMARxThreshold(stream->i2s.reg, 4);
		Wrap_MXC_I2S_EnableDMARx(stream->i2s.reg);
	} else {
			       ...
			       const struct i2s_config *i2s_cfg)
{
	...
	LOG_DBG("configure with dir=%d, word_size=%u, channels=%u, frame_clk_freq=%u",
