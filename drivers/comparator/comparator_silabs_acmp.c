...
struct acmp_config {
	...
	void (*irq_init)(void);
	int (*input_abus_allocate)(int input);
...
typedef enum acmp_input_port {
	ACMP_INPUT_PORT_A = 0x80,
	ACMP_INPUT_PORT_B = 0x90,
	ACMP_INPUT_PORT_C = 0xA0,
	ACMP_INPUT_PORT_D = 0xB0,
} acmp_input_port_t;

#define ACMP_INPUT_PORT(input) (input & 0xF0)
#define ACMP_INPUT_PIN(input)  (input & 0x0F)
...
static int acmp_init(const struct device *dev)
{
	...
	/* Allocate ABUS for GPIO inputs */
	config->input_abus_allocate(config->input_positive);
	config->input_abus_allocate(config->input_negative);
...
#define ACMP_DEVICE(inst)                                                                          \
	...
	{                                                                                          \
		...
		irq_enable(DT_INST_IRQN(inst));                                                    \
                                                                                                   ...
                                                                                                   \
	static int acmp_input_abus_allocate##inst(int input)                                       \
	{                                                                                          \
		switch (ACMP_INPUT_PORT(input)) {                                                  \
		case ACMP_INPUT_PORT_A:                                                            \
			if (ACMP_INPUT_PIN(input) % 2 == 0) {                                      \
				if ((GPIO->ABUSALLOC & _GPIO_ABUSALLOC_AEVEN0_MASK) ==             \
				    GPIO_ABUSALLOC_AEVEN0_TRISTATE) {                              \
					GPIO->ABUSALLOC |= GPIO_ABUSALLOC_AEVEN0_ACMP##inst;       \
				} else if ((GPIO->ABUSALLOC & _GPIO_ABUSALLOC_AEVEN1_MASK) ==      \
					   GPIO_ABUSALLOC_AEVEN1_TRISTATE) {                       \
					GPIO->ABUSALLOC |= GPIO_ABUSALLOC_AEVEN1_ACMP##inst;       \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			} else {                                                                   \
				if ((GPIO->ABUSALLOC & _GPIO_ABUSALLOC_AODD0_MASK) ==              \
				    GPIO_ABUSALLOC_AODD0_TRISTATE) {                               \
					GPIO->ABUSALLOC |= GPIO_ABUSALLOC_AODD0_ACMP##inst;        \
				} else if ((GPIO->ABUSALLOC & _GPIO_ABUSALLOC_AODD1_MASK) ==       \
					   GPIO_ABUSALLOC_AODD1_TRISTATE) {                        \
					GPIO->ABUSALLOC |= GPIO_ABUSALLOC_AODD1_ACMP##inst;        \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			}                                                                          \
			break;                                                                     \
		case ACMP_INPUT_PORT_B:                                                            \
			if (ACMP_INPUT_PIN(input) % 2 == 0) {                                      \
				if ((GPIO->BBUSALLOC & _GPIO_BBUSALLOC_BEVEN0_MASK) ==             \
				    GPIO_BBUSALLOC_BEVEN0_TRISTATE) {                              \
					GPIO->BBUSALLOC |= GPIO_BBUSALLOC_BEVEN0_ACMP##inst;       \
				} else if ((GPIO->BBUSALLOC & _GPIO_BBUSALLOC_BEVEN1_MASK) ==      \
					   GPIO_BBUSALLOC_BEVEN1_TRISTATE) {                       \
					GPIO->BBUSALLOC |= GPIO_BBUSALLOC_BEVEN1_ACMP##inst;       \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			} else {                                                                   \
				if ((GPIO->BBUSALLOC & _GPIO_BBUSALLOC_BODD0_MASK) ==              \
				    GPIO_BBUSALLOC_BODD0_TRISTATE) {                               \
					GPIO->BBUSALLOC |= GPIO_BBUSALLOC_BODD0_ACMP##inst;        \
				} else if ((GPIO->BBUSALLOC & _GPIO_BBUSALLOC_BODD1_MASK) ==       \
					   GPIO_BBUSALLOC_BODD1_TRISTATE) {                        \
					GPIO->BBUSALLOC |= GPIO_BBUSALLOC_BODD1_ACMP##inst;        \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			}                                                                          \
			break;                                                                     \
		case ACMP_INPUT_PORT_C:                                                            \
		case ACMP_INPUT_PORT_D:                                                            \
			if (ACMP_INPUT_PIN(input) % 2 == 0) {                                      \
				if ((GPIO->CDBUSALLOC & _GPIO_CDBUSALLOC_CDEVEN0_MASK) ==          \
				    GPIO_CDBUSALLOC_CDEVEN0_TRISTATE) {                            \
					GPIO->CDBUSALLOC |= GPIO_CDBUSALLOC_CDEVEN0_ACMP##inst;    \
				} else if ((GPIO->CDBUSALLOC & _GPIO_CDBUSALLOC_CDEVEN1_MASK) ==   \
					   GPIO_CDBUSALLOC_CDEVEN1_TRISTATE) {                     \
					GPIO->CDBUSALLOC |= GPIO_CDBUSALLOC_CDEVEN1_ACMP##inst;    \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			} else {                                                                   \
				if ((GPIO->CDBUSALLOC & _GPIO_CDBUSALLOC_CDODD0_MASK) ==           \
				    GPIO_CDBUSALLOC_CDODD0_TRISTATE) {                             \
					GPIO->CDBUSALLOC |= GPIO_CDBUSALLOC_CDODD0_ACMP##inst;     \
				} else if ((GPIO->CDBUSALLOC & _GPIO_CDBUSALLOC_CDODD1_MASK) ==    \
					   GPIO_CDBUSALLOC_CDODD1_TRISTATE) {                      \
					GPIO->CDBUSALLOC |= GPIO_CDBUSALLOC_CDODD1_ACMP##inst;     \
				} else {                                                           \
					return -EIO;                                               \
				}                                                                  \
			}                                                                          \
	...
	static const struct acmp_config acmp_config##inst = {                                      \
		...
		.input_abus_allocate = acmp_input_abus_allocate##inst,                             \
