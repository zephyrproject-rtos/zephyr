...
static int acmphs_renesas_set_trigger(const struct device *dev, enum comparator_trigger trigger)
{
	...
	case COMPARATOR_TRIGGER_RISING_EDGE:
		...
		data->acmphs.p_reg->CMPCTL |=
			(COMPARATOR_TRIGGER_RISING << R_ACMPHS0_CMPCTL_CEG_Pos);
		break;

	case COMPARATOR_TRIGGER_FALLING_EDGE:
		data->fsp_config.trigger = COMPARATOR_TRIGGER_FALLING;
		data->acmphs.p_reg->CMPCTL |=
			(COMPARATOR_TRIGGER_FALLING << R_ACMPHS0_CMPCTL_CEG_Pos);
		break;

	case COMPARATOR_TRIGGER_BOTH_EDGES:
		data->fsp_config.trigger = COMPARATOR_TRIGGER_BOTH_EDGE;
		data->acmphs.p_reg->CMPCTL |=
			(COMPARATOR_TRIGGER_BOTH_EDGE << R_ACMPHS0_CMPCTL_CEG_Pos);
		break;

	case COMPARATOR_TRIGGER_NONE:
		data->fsp_config.trigger = COMPARATOR_TRIGGER_NO_EDGE;
		data->acmphs.p_reg->CMPCTL |=
			(COMPARATOR_TRIGGER_NO_EDGE << R_ACMPHS0_CMPCTL_CEG_Pos);
		break;
	}
