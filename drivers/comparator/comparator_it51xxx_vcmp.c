...
static int it51xxx_vcmp_set_trigger(const struct device *dev, enum comparator_trigger trigger)
{
	...
	/* Only enable VCMP and VCMP interrupts when the trigger is not none and if a
	 * callback is set.
	 */
	if (data->interrupt_mask && data->user_cb != NULL) {
		/* Enable voltage comparator and interrupt */
