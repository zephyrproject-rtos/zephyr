 ...
 */
static int nxp_slcd_set_pin(const struct device *dev, uint32_t pin, uint8_t com_mask, bool on)
{
	...
	if (com_mask >= (1U << data->back_plane_count)) {
		LOG_ERR("Invalid com_mask: 0x%02x (back_plane_count: %u, max allowed: 0x%02x)",
			com_mask, data->back_plane_count, (1U << data->back_plane_count) - 1);
		return -EINVAL;
	}

	/* Set the front plane segments for the specified pin and COM lines. */
	if ((com_mask & (com_mask - 1U)) == 0U) {
		SLCD_SetFrontPlaneOnePhase(config->base, data->front_plane_pins[pin],
					   __builtin_ctz(com_mask), on);
	} else {
		SLCD_SetFrontPlaneSegments(config->base, data->front_plane_pins[pin],
					   (on ? com_mask : 0));
	}
...
static int nxp_slcd_start(const struct device *dev)
{
	const struct nxp_slcd_config *config = dev->config;
...
static int nxp_slcd_stop(const struct device *dev)
{
	const struct nxp_slcd_config *config = dev->config;
...
static int nxp_slcd_init(const struct device *dev)
{
	...
	if (config->back_plane_count == 0 || config->back_plane_count > 8) {
		LOG_ERR("Invalid back_plane_count: %u (must be 1-8)", config->back_plane_count);
		return -EINVAL;
	}
...
#define NXP_SLCD_DEFINE(inst)                                                                      \
	...
	static struct nxp_slcd_config nxp_slcd_config_##inst = {                                   \
		...
		.duty_cycle =                                                                      \
			NXP_SLCD_GET_DUTY_CYCLE(ARRAY_SIZE(nxp_slcd_back_plane_pins_##inst)),      \
	...
	DEVICE_DT_INST_DEFINE(inst, &nxp_slcd_init, NULL, &nxp_slcd_data_##inst,                   \
			      &nxp_slcd_config_##inst, POST_KERNEL,                                \
			      CONFIG_SLCD_CONTROLLER_INIT_PRIORITY, &nxp_slcd_api);
