...
int nxp_sc18is606_claim(const struct device *dev)
{
	...
	return k_mutex_lock(&data->bridge_lock, K_NO_WAIT);
}
...
static int sc18is606_spi_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if ((config->operation & (SPI_LINES_DUAL | SPI_LINES_QUAD | SPI_LINES_OCTAL)) != 0U) {
...
/*Runtime Power Management  Section*/
static int sc18is606_pm_turn_on(const struct device *dev)
{
	const struct nxp_sc18is606_config *cfg = dev->config;
	int ret;

	ret = pm_device_runtime_get(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device int_gpio run time for turn on (%d)", ret);
		return ret;
	}

	ret = int_gpios_setup(dev);
	if (ret != 0U) {
		LOG_ERR("Could not set up the int_gpios on resume (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_get(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get reset_gpios runtime for resume (%d)", ret);
		return ret;
	}
	...
	/*After reset we dont need the reset pin so release it*/
	ret = pm_device_runtime_put(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not put reset_gpios runtime for resume (%d)", ret);
		return ret;
	}

	/*Release devices needed for configuring*/
	ret = pm_device_runtime_get(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device reset_gpio run time for off (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_put(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not put device reset_gpio for off (%d)", ret);
	}

	return ret;
}

static int sc18is606_pm_turn_off(const struct device *dev)
{
	const struct nxp_sc18is606_config *cfg = dev->config;
	int ret;
	...
	ret = pm_device_runtime_get(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device int_gpio run time for off (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_put(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not put device int_gpio for off (%d)", ret);
	}
	...
	/* We have released everything we can go off now*/

	return ret;
}

static int sc18is606_pm_resume(const struct device *dev)
{
	const struct nxp_sc18is606_config *cfg = dev->config;
	int ret;
	...
	ret = pm_device_runtime_get(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device int_gpio run time for resume (%d)", ret);
		return ret;
	}

	/*Is this necessary ?*/
	ret = gpio_pin_set_dt(&cfg->int_gpios, 1);
	if (ret != 0U) {
		LOG_ERR("Could not set int_gpio dt for resume (%d)", ret);
		return ret;
	}
...
static int sc18is606_pm_suspend(const struct device *dev)
{
	struct nxp_sc18is606_data *data = dev->data;
	const struct nxp_sc18is606_config *cfg = dev->config;
	int ret;
	uint8_t buffer[] = {IDLE_MODE};

	ret = pm_device_runtime_get(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device interrupt run time (%d)", ret);
		return ret;
	}

	ret = gpio_pin_interrupt_configure_dt(&cfg->int_gpios, GPIO_INT_DISABLE);
	if (ret != 0U) {
		LOG_ERR("Could not disable the int_gpio interrupt (%d)", ret);
		return ret;
	}

	ret = gpio_remove_callback_dt(&cfg->int_gpios, &data->int_cb);
	if (ret != 0U) {
		LOG_ERR("Could not remove int_gpio callback (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_put(cfg->int_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not put the int_gpio runtime (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_get(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not get device reset run time (%d)", ret);
		return ret;
	}

	ret = pm_device_runtime_put(cfg->reset_gpios.port);
	if (ret != 0U) {
		LOG_ERR("Could not put the device reset gpio (%d)", ret);
		return ret;
	}

	/* Send IDLE MESSAGE to device */
	ret = nxp_sc18is606_transfer(dev, buffer, sizeof(buffer), NULL, 0, NULL);

	/*TODO Check if I need to do something about the semaphore in suspend*/
	return ret;
}

static int sc18is606_pm_action(const struct device *dev, enum pm_device_action action)
{
	int ret;

	switch (action) {
	case PM_DEVICE_ACTION_SUSPEND:
		ret = sc18is606_pm_suspend(dev);
		break;
	case PM_DEVICE_ACTION_RESUME:
		ret = sc18is606_pm_resume(dev);
		break;
	case PM_DEVICE_ACTION_TURN_OFF:
		ret = sc18is606_pm_turn_off(dev);
		break;
	case PM_DEVICE_ACTION_TURN_ON:
		ret = sc18is606_pm_turn_on(dev);
		break;
	default:
		ret = -EINVAL;
		break;
...
static int sc18is606_init(const struct device *dev)
{
	...
	return pm_device_driver_init(dev, sc18is606_pm_action);
}

static int sc18is606_deinit(const struct device *dev)
{
	int ret;

	/*Use the pm_action callback to move or verify the device into a proper state*/
	ret = pm_device_driver_deinit(dev, sc18is606_pm_action);
	if (ret) {
		LOG_ERR("Error when starting deinit (%d)", ret);
		return ret;
	}

	/*We are now off or suspended so are our dependencies, will be here until new owner shows up
	 *  Do I do something about my semaphore ?
	 */
	return ret;
}
...
#define SPI_SC18IS606_DEFINE(inst)                                                                 \
	static struct nxp_sc18is606_data sc18is606_data_##inst = {                                 \
		.frequency = DT_INST_ENUM_IDX(inst, frequency),					   \
	...
	static const struct nxp_sc18is606_config sc18is606_config_##inst = {                       \
		...
		.int_gpios = GPIO_DT_SPEC_GET_OR(DT_INST_PARENT(inst), int_gpios, {0}),            \
                                                                                                   ...
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(inst, sc18is606_pm_action);                                       \
                                                                                                   \
	DEVICE_DT_INST_DEINIT_DEFINE(inst, sc18is606_init, sc18is606_deinit,                       \
				     PM_DEVICE_DT_INST_GET(inst), &sc18is606_data_##inst,          \
				     &sc18is606_config_##inst, POST_KERNEL,                        \
				     CONFIG_SPI_INIT_PRIORITY, &sc18is606_api);
