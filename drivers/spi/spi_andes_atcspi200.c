...
K_HEAP_DEFINE(chain_block, 2048);
...
{
	struct spi_atcspi200_data *data = user_data;
	const struct device *spi_dev = data->self_dev;
	struct spi_context *ctx = &data->ctx;
...
static int spi_dma_tx_load(const struct device *dev)
{
	...
	data->dma_tx.dma_blk_cfg.dest_address = (uint32_t)SPI_DATA(dev);
	...
	data->dma_tx.dma_cfg.user_data = data;
	...
	if (data->dma_tx.dma_cfg.source_chaining_en) {
		...
		while (remain_len > 0) {
			...
			next_blk_cfg = k_heap_alloc(&chain_block,
						sizeof(struct dma_block_config), K_FOREVER);
			...
			next_blk_cfg->dest_address = (uint32_t)SPI_DATA(dev);
			/* fifo mode NOT USED there */
			if (data->dma_tx.dst_addr_increment) {
...
static int spi_dma_rx_load(const struct device *dev)
{
	...
	data->dma_rx.dma_blk_cfg.source_address = (uint32_t)SPI_DATA(dev);
	...
	if (data->dma_rx.dma_cfg.source_chaining_en) {
		...
		while (remain_len > 0) {
			...
			next_blk_cfg->source_address = (uint32_t)SPI_DATA(dev);
...
static int spi_transfer_dma(const struct device *dev)
{
	...
	CLR_MASK(SPI_TIMIN(dev), 0xff);

	/* Enable END Interrupts */
	sys_write32(IEN_END_MSK, SPI_INTEN(dev));
		 ...
		 && (data->dma_rx.dma_dev != NULL)) {
			...
			if (data->dma_tx.dma_cfg.source_chaining_en) {
				...
				struct dma_block_config *block =
							data->dma_tx.dma_blk_cfg.next_block;

				while (block != NULL) {
					struct dma_block_config *next_block = block->next_block;

					k_heap_free(&chain_block, block);
					block = next_block;
				}
			}
			...
			if (data->dma_rx.dma_cfg.source_chaining_en) {
				...
				struct dma_block_config *block =
							data->dma_rx.dma_blk_cfg.next_block;
