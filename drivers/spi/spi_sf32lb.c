...
static int spi_sf32lb_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if (word_size == 8) {
		top_ctrl |= ((8 - 1) << SPI_TOP_CTRL_DSS_Pos) & SPI_TOP_CTRL_DSS_Msk;//FIELD_PREP(SPI_TOP_CTRL_DSS_Msk, 8U - 1U)
	} else if (word_size == 16) {
		top_ctrl |= ((16 - 1) << SPI_TOP_CTRL_DSS_Pos) & SPI_TOP_CTRL_DSS_Msk;//FIELD_PREP(SPI_TOP_CTRL_DSS_Msk, 16U - 1U)
	} else if (word_size == 24) {
		top_ctrl |= ((24 - 1) << SPI_TOP_CTRL_DSS_Pos) & SPI_TOP_CTRL_DSS_Msk;//FIELD_PREP(SPI_TOP_CTRL_DSS_Msk, 24U - 1U)
	}

	if (SPI_FRAME_FORMAT_TI == (config->operation & SPI_FRAME_FORMAT_TI)) {
		top_ctrl |= (0x00000001U << SPI_TOP_CTRL_FRF_Pos) & SPI_TOP_CTRL_FRF_Msk;//FIELD_PREP(SPI_TOP_CTRL_FRF_Msk, 0x00000001U)
	} else {
		top_ctrl |= (0x00000000U << SPI_TOP_CTRL_FRF_Pos) & SPI_TOP_CTRL_FRF_Msk;//FIELD_PREP(SPI_TOP_CTRL_FRF_Msk, 0x00000000U)
	}
...
static DEVICE_API(spi_driver_api, spi_sf32lb_api) = {
