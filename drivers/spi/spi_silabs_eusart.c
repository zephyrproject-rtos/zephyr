...
#ifdef CONFIG_SPI_SILABS_EUSART_DMA
	struct dma_channel dma_channel_rx;
	struct dma_channel dma_channel_tx;
#endif
};
...
static inline bool is_dma_enabled_instance(const struct device *dev)
{
	...
	return data->dma_channel_rx.dma_dev != NULL;
#else
	return false;
#endif
}
...
static int spi_silabs_eusart_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if (is_dma_enabled_instance(dev)) {
		if (!device_is_ready(data->dma_channel_tx.dma_dev)
		    || data->dma_channel_tx.dma_dev == NULL) {
			return -ENODEV;
		}
		...
		if (data->dma_channel_rx.channel_nb < 0) {
			data->dma_channel_rx.channel_nb = dma_request_channel(data->dma_channel_rx.dma_dev,
									      NULL);
		}
		...
		if (data->dma_channel_tx.channel_nb < 0) {
			data->dma_channel_tx.channel_nb = dma_request_channel(data->dma_channel_tx.dma_dev,
									      NULL);
		}

		if (data->dma_channel_tx.channel_nb < 0) {
			dma_release_channel(data->dma_channel_rx.dma_dev,
					    data->dma_channel_rx.channel_nb);
			LOG_ERR("DMA channel request failed");
			return -EAGAIN;
		}
	}
					   ...
					   size_t total_padded_transaction_size, bool is_tx)
{
	...
	struct dma_channel *channel = is_tx ? &(instance_data->dma_channel_tx) :
					      &(instance_data->dma_channel_rx);
	struct dma_block_config *cur_blk = &(channel->dma_descriptors[0]);
	...
	for (size_t i = 0U; i < buffer_count; i++) {
		size_t buffer_remaining_count = buffer->len;
		...
		current_total_transfer_size += buffer->len;

		/* Create the required number of dma transfer block for this buffer */
		while (block_idx <= CONFIG_SPI_SILABS_EUSART_DMA_MAX_BLOCKS) {
			if (block_idx >= CONFIG_SPI_SILABS_EUSART_DMA_MAX_BLOCKS) {
				LOG_ERR("DMA channel transfer blocks exausted. Can't prepare the "
					"transaction.");
				return -ENOMEM;
			}

			uint32_t buf_index = buffer->len - buffer_remaining_count;
			uint8_t *buffer_start_address = buffer->buf ? 
							&(((uint8_t *)buffer->buf)[buf_index])
							: NULL;

			buffer_remaining_count = spi_silabs_build_dma_block(instance_conf,
									    &(cur_blk[block_idx]),
									    buffer_start_address,
									    buffer_remaining_count,
									    is_tx);
	...
	while (padding_size) {
		...
		padding_size = spi_silabs_build_dma_block(
			instance_conf, &cur_blk[block_idx], NULL, padding_size,
			is_tx);
		block_idx++;
	}

	spi_silabs_build_dma_config(spi_dev, channel, block_idx, is_tx);

	ret = dma_config(channel->dma_dev, channel->channel_nb, &(channel->dma_config));
				...
				int status)
{
	...
	if (status < 0U) {
		dma_stop(instance_data->dma_channel_tx.dma_dev,
				instance_data->dma_channel_tx.channel_nb);
		dma_stop(instance_data->dma_channel_rx.dma_dev,
				instance_data->dma_channel_rx.channel_nb);
	}
...
static int spi_silabs_eusart_xfer_dma(const struct device *dev, const struct spi_config *config)
{
	...
	ret = dma_start(data->dma_channel_rx.dma_dev, data->dma_channel_rx.channel_nb);

	if (ret) {
		goto force_transaction_close;
	}

	ret = dma_start(data->dma_channel_tx.dma_dev, data->dma_channel_tx.channel_nb);

	if (ret) {
		dma_stop(data->dma_channel_rx.dma_dev, data->dma_channel_rx.channel_nb);
		goto force_transaction_close;
	}
