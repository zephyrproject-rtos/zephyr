...
static inline void zynq_qspi_write_txd(const struct device *dev, unsigned int size)
{
	...
	enum xlnx_zynq_qspi_registers reg = reg_lut[reg_idx & 0x3];
...
static int zynq_qspi_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	...
	if (SPI_WORD_SIZE_GET(spi_cfg->operation) != 8) {
		LOG_ERR("Word size of %u not allowed", SPI_WORD_SIZE_GET(spi_cfg->operation));
		return -ENOTSUP;
	}

	if ((spi_cfg->operation & SPI_TRANSFER_LSB) != 0) {
