...
#define DT_DRV_COMPAT andestech_et171
		     ...
		     const struct spi_config *config)
{
	...
	if (spi_context_configured(ctx, config)) {
		if (ctx->config->frequency == config->frequency) {
				...
				uint32_t channel, int status)
{
	...
	struct spi_et171_data *data = spi_dev->data;
	struct spi_context *ctx = &data->ctx;
	int error;
...
static int spi_dma_tx_load(const struct device *dev)
{
	...
	if (ctx->current_tx->buf == NULL) {
		...
		data->dma_tx.dma_blk_cfg.source_address = (uintptr_t)&dummy_rxtx_buffer;
...
#ifdef CONFIG_DCACHE
#ifdef CONFIG_CACHE_MANAGEMENT
		if (IS_ADDR_IN_RAM(data->dma_tx.dma_blk_cfg.source_address)) {
			// DMA_DCACHE_WRITEBACK(cfg_blocks->source_address, tfr_size);
			cache_data_flush_range((void *)data->dma_tx.dma_blk_cfg.source_address,
					       ctx->current_tx->len);
		}
#else
	...
	if (data->dma_tx.dma_cfg.source_chaining_en) {
		...
		while (remain_len > 0) {
			...
			if (current_tx->buf == NULL) {
				dummy_rxtx_buffer = 0;
				/* if tx buff is null, then sends NOP on the line. */
				next_blk_cfg->source_address = (uintptr_t)&dummy_rxtx_buffer;
				next_blk_cfg->source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
			} else {
				next_blk_cfg->source_address = (uintptr_t)current_tx->buf;
#ifdef CONFIG_DCACHE
#ifdef CONFIG_CACHE_MANAGEMENT
				if (IS_ADDR_IN_RAM(next_blk_cfg->source_address)) {
					cache_data_flush_range((void *)next_blk_cfg->source_address,
							       ctx->current_tx->len);
				}
#else
#error "With Cache enable, you may need to do the cache flush before reset."
...
static int spi_dma_rx_load(const struct device *dev)
{
	...
	if (ctx->current_rx->buf == NULL) {
		...
		data->dma_rx.dma_blk_cfg.dest_address = (uintptr_t)&dummy_rxtx_buffer;
...
#ifdef CONFIG_DCACHE
#ifdef CONFIG_CACHE_MANAGEMENT
		if (IS_ADDR_IN_RAM(data->dma_rx.dma_blk_cfg.dest_address)) {
			// DMA_DCACHE_INVALID(cfg_blocks->dest_address, dst_addr + size);
			cache_data_invd_range((void *)data->dma_rx.dma_blk_cfg.dest_address,
					      ctx->current_rx->len);
		}
#else
#error "With Cache enable, you may need to do the cache flush before reset."
	...
	if (data->dma_rx.dma_cfg.source_chaining_en) {
		...
		while (remain_len > 0) {
			...
			if (current_rx->buf == NULL) {
				/* if rx buff is null, then write data to dummy address. */
				next_blk_cfg->dest_address = (uintptr_t)&dummy_rxtx_buffer;
				next_blk_cfg->dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
			} else {
				next_blk_cfg->dest_address = (uintptr_t)current_rx->buf;
#ifdef CONFIG_DCACHE
#ifdef CONFIG_CACHE_MANAGEMENT
				if (IS_ADDR_IN_RAM(next_blk_cfg->dest_address)) {
					cache_data_invd_range((void *)next_blk_cfg->dest_address,
							      ctx->current_rx->len);
				}
#else
#error "With Cache enable, you may need to do the cache flush before reset."
...
static int spi_transfer_dma(const struct device *dev)
{
	...
	// CLR_MASK(SPI_TIMIN(cfg->base), 0xff); // already config, remove
			...
			if (!IS_ALIGN(rx_buf->buf)) {
				if (rx_buf->len <= (2 * cache_line_size)) {
					...
					*FregSize += rx_buf->len;
					...
					// + head
					*FregSize +=
						ROUND_UP((size_t)rx_buf->buf, cache_line_size) -
						(size_t)rx_buf->buf;
					*ExtendCount += 1;
				  ...
				  bool asynchronous, spi_callback_t cb, void *userdata)
{
	...
	if (asynchronous) {
		ret = transceive(dev, config, tx_bufs, &data->aligned_rx_bufs, true, cb, userdata);
	} else {
		ret = transceive(dev, config, tx_bufs, &data->aligned_rx_bufs, false, NULL, NULL);
	}
			 ...
			 const struct spi_buf_set *tx_bufs, const struct spi_buf_set *rx_bufs)
{
	...
	ret = transceive(dev, config, tx_bufs, rx_bufs, false, NULL, NULL);
#else
...
#ifndef CONFIG_DCACHE
	ret = transceive(dev, config, tx_bufs, rx_bufs, true, cb, userdata);
#else
	int iExtendCount = 0, iFregSize = 0;
	int need_align = is_need_alignment(tx_bufs, rx_bufs, &iExtendCount, &iFregSize);
	if (need_align == false) {
...
#define SPI_INIT(n)                                                                                \
	...
	static struct spi_et171_cfg spi_et171_dev_cfg_##n = {                                      \
		...
		.xip = SPI_ROM_CFG_XIP(DT_DRV_INST(n)),                                            \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, spi_et171_init, NULL, &spi_et171_dev_data_##n,                    \
