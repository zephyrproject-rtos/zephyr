...
static int spi_dma_tx_load(const struct device *dev)
{
	...
	if (ctx->current_tx->buf == NULL) {
		...
		if (IS_ADDR_IN_RAM(data->dma_tx.dma_blk_cfg.source_address)) {
			...
			sys_cache_data_flush_range((void *)data->dma_tx.dma_blk_cfg.source_address,
...
static int spi_dma_rx_load(const struct device *dev)
{
	...
	if (ctx->current_rx->buf == NULL) {
		...
		if (IS_ADDR_IN_RAM(data->dma_rx.dma_blk_cfg.dest_address)) {
			...
			sys_cache_data_invd_range((void *)data->dma_rx.dma_blk_cfg.dest_address,
						  (data->dma_rx.dma_blk_cfg.block_size /
						   data->dma_rx.dma_cfg.source_data_size));
		}
#else
#error "With Cache enable, you may need to do the cache flush before reset."
#endif
