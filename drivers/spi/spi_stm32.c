...
static int spi_stm32_shift_m(const struct spi_stm32_config *cfg, struct spi_stm32_data *data)
{
	if (cfg->fifo_enabled) {
#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi)
		return spi_stm32_shift_fifo(cfg->spi, data);
#endif /* DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi) */
	} else {
		uint32_t transfer_dir = ll_get_transfer_direction(cfg->spi);
		...
		if (transfer_dir != STM32_SPI_HALF_DUPLEX_RX) {
			...
			spi_stm32_send_next_frame(cfg->spi, data, 0U);
		}
		...
		if (transfer_dir != STM32_SPI_HALF_DUPLEX_TX) {
			...
			spi_stm32_read_next_frame(cfg->spi, data, 0U);
		}
	}
	return 0;
}
				      ...
				      const uint8_t *tx_buf, uint8_t *rx_buf, uint32_t buf_len)
{
	 ...
	 */
	if (transfer_dir != STM32_SPI_HALF_DUPLEX_RX && LL_SPI_GetMode(spi) == LL_SPI_MODE_MASTER) {
		LL_SPI_StartMasterTransfer(spi);
	}
#else
	LL_SPI_Enable(spi);
			       ...
			       bool write)
{
	...
	bool use_dma = (data->dma_tx.dma_dev != NULL) && (data->dma_rx.dma_dev != NULL);
#else /* CONFIG_SPI_STM32_DMA */
	bool use_dma = false;
#endif /* CONFIG_SPI_STM32_DMA */
	if (cfg->fifo_enabled && !use_dma) {
		...
		if (cfg->fifo_threshold > cfg->fifo_size) {
			LOG_ERR("FIFO threshold is higher than FIFO size.");
			return -EINVAL;
		}

		data->fifo_threshold = cfg->fifo_threshold / dfs;

		if (data->fifo_threshold == 0U &&
		    data->fifo_threshold <= STM32_SPI_FIFO_THRESHOLD_MAX) {
			data->fifo_threshold = 1U;
		}
	} else {
...
static void dma_callback_async(const struct device *dma_dev, const struct device *spi_dev,
...
#define STM32_SPI_INIT(id)							\
	...
	static const struct spi_stm32_config spi_stm32_cfg_##id = {		\
		...
		IF_ENABLED(DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi), (		\
			...
			.fifo_threshold = DT_INST_PROP(id, fifo_threshold),	\
		))								\
