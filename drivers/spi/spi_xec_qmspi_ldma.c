...
#define XEC_QSPI_HW_ERRORS_ALL (XEC_QSPI_HW_ERRORS | XEC_QSPI_HW_ERRORS_LDMA)
...
const uint8_t smode48_tbl[4] = {0x04u, 0x02u, 0x05u, 0x03u};

static void qmspi_set_signalling_mode(struct spi_qmspi_data *qdata, struct qmspi_regs *regs,
				      uint32_t smode)
{
	const uint8_t *ptbl;
	uint32_t m;
	...
	regs->MODE = (regs->MODE & ~(MCHP_QMSPI_M_SIG_MASK)) | (m << MCHP_QMSPI_M_SIG_POS);
}
 ...
 */
static int qmspi_configure(const struct device *dev, const struct spi_config *config)
{
	...
	if (cfg->cs1_freq) {
		...
		regs->MODE_ALT1 =
			(fdiv << MCHP_QMSPI_MA1_CS1_CDIV_POS) & MCHP_QMSPI_MA1_CS1_CDIV_MSK;
 ...
 */
static inline int qmspi_xfr_cm_init(const struct device *dev, const struct spi_buf_set *tx_bufs,
 ...
 */
static int qmspi_xfr_sync(const struct device *dev, const struct spi_config *spi_cfg,
				 ...
				 const struct spi_buf_set *rx_bufs)
{
	...
	regs->STS = 0xffffffffu;
	regs->EXE = MCHP_QMSPI_EXE_START;
	regs->IEN = MCHP_QMSPI_IEN_XFR_DONE | MCHP_QMSPI_IEN_PROG_ERR | MCHP_QMSPI_IEN_LDMA_RX_ERR |
		    MCHP_QMSPI_IEN_LDMA_TX_ERR;

	return 0;
}

/* Wrapper to start asynchronous (interrupts enabled) SPI transaction */
static int qmspi_xfr_async(const struct device *dev, const struct spi_config *config,
			   const struct spi_buf_set *tx_bufs, const struct spi_buf_set *rx_bufs)
{
	struct spi_qmspi_data *qdata = dev->data;
	int err = 0;
...
static int qmspi_transceive(const struct device *dev, const struct spi_config *config,
			    const struct spi_buf_set *tx_bufs, const struct spi_buf_set *rx_bufs,
			    bool asynchronous, spi_callback_t cb, void *user_data)
{
	struct spi_qmspi_data *qdata = dev->data;
	struct spi_context *ctx = &qdata->ctx;
	int err = 0;
...
static int qmspi_transceive_sync(const struct device *dev, const struct spi_config *config,
...
static int qmspi_transceive_async(const struct device *dev, const struct spi_config *config,
				  const struct spi_buf_set *tx_bufs,
...
static int qmspi_release(const struct device *dev, const struct spi_config *config)
{
...
#define XEC_QMSPI_GIRQ_POS(i) MCHP_XEC_ECIA_GIRQ_POS(DT_INST_PROP_BY_IDX(i, girqs, 0))

#define XEC_QMSPI_NVIC_AGGR(i) MCHP_XEC_ECIA_NVIC_AGGR(DT_INST_PROP_BY_IDX(i, girqs, 0))
