				  ...
				  struct spi_pico_pio_data *data)
{
	...
	return ((pio->sm[dev_cfg->pio_sm].addr == data->pio_tx_offset) &&
		pio_sm_is_tx_fifo_empty(pio, dev_cfg->pio_sm));
}
				  ...
				  struct spi_pico_pio_data *data, const struct spi_config *spi_cfg)
{
	...
	if (dev_cfg->sio_gpio.port) {
		...
		data->pio_tx_offset =
			pio_add_program(pio, RPI_PICO_PIO_GET_PROGRAM(spi_sio_mode_0_0_tx));

		data->pio_rx_offset =
			pio_add_program(pio, RPI_PICO_PIO_GET_PROGRAM(spi_sio_mode_0_0_rx));
		...
		pio_sm_set_pindirs_with_mask(pio, dev_cfg->pio_sm,
					     (BIT(clk->pin) | BIT(sio->pin)),
					     (BIT(clk->pin) | BIT(sio->pin)));
		pio_sm_set_pins_with_mask(pio, dev_cfg->pio_sm, 0,
					  BIT(clk->pin) | BIT(sio->pin));
		pio_gpio_init(pio, sio->pin);
		pio_gpio_init(pio, clk->pin);

		pio_sm_init(pio, dev_cfg->pio_sm, data->pio_tx_offset, &sm_config);
		pio_sm_set_enabled(pio, dev_cfg->pio_sm, true);
#else
		...
		pio_sm_set_consecutive_pindirs(pio, dev_cfg->pio_sm, miso->pin, 1, false);
		pio_sm_set_pindirs_with_mask(pio, dev_cfg->pio_sm,
					     (BIT(clk->pin) | BIT(mosi->pin)),
					     (BIT(clk->pin) | BIT(mosi->pin)));
		pio_sm_set_pins_with_mask(pio, dev_cfg->pio_sm, (cpol << clk->pin),
					  BIT(clk->pin) | BIT(mosi->pin));
	...
	while (data->rx_count < chunk_len || data->tx_count < chunk_len) {
		...
		while ((!pio_sm_is_tx_fifo_full(pio, dev_cfg->pio_sm)) &&
			...
			case 4: {
				...
				spi_pico_pio_sm_put32(pio, dev_cfg->pio_sm, txrx);
			} break;
			...
			case 2: {
				...
				spi_pico_pio_sm_put16(pio, dev_cfg->pio_sm, txrx);
			} break;
			...
			case 1: {
				...
				spi_pico_pio_sm_put8(pio, dev_cfg->pio_sm, txrx);
			} break;
		...
		while ((!pio_sm_is_rx_fifo_empty(pio, dev_cfg->pio_sm)) &&
		       data->rx_count < chunk_len && fifo_cnt > 0) {
			switch (data->dfs) {
			case 4: {
				txrx = spi_pico_pio_sm_get32(pio, dev_cfg->pio_sm);
			...
			case 2: {
				txrx = spi_pico_pio_sm_get16(pio, dev_cfg->pio_sm);
			...
			case 1: {
				txrx = spi_pico_pio_sm_get8(pio, dev_cfg->pio_sm);
...
static void spi_pico_pio_txrx_3_wire(const struct device *dev)
{
	...
	if (txbuf) {
		pio_sm_set_enabled(pio, dev_cfg->pio_sm, false);
		pio_sm_set_wrap(pio, dev_cfg->pio_sm,
				data->pio_tx_offset +
					RPI_PICO_PIO_GET_WRAP_TARGET(spi_sio_mode_0_0_tx),
				data->pio_tx_offset + RPI_PICO_PIO_GET_WRAP(spi_sio_mode_0_0_tx));
		pio_sm_clear_fifos(pio, dev_cfg->pio_sm);
		pio_sm_set_pindirs_with_mask(pio, dev_cfg->pio_sm, BIT(sio_pin), BIT(sio_pin));
		pio_sm_restart(pio, dev_cfg->pio_sm);
		pio_sm_clkdiv_restart(pio, dev_cfg->pio_sm);
		pio_sm_exec(pio, dev_cfg->pio_sm, pio_encode_jmp(data->pio_tx_offset));
		pio_sm_set_enabled(pio, dev_cfg->pio_sm, true);
		...
		while ((!pio_sm_is_tx_fifo_empty(pio, dev_cfg->pio_sm)) ||
		       (!spi_pico_pio_sm_complete(dev_cfg))) {
			;
		}
	}
	...
	if (rxbuf) {
		pio_sm_set_enabled(pio, dev_cfg->pio_sm, false);
		pio_sm_set_wrap(pio, dev_cfg->pio_sm, data->pio_rx_wrap_target,
				data->pio_rx_wrap);
		pio_sm_clear_fifos(pio, dev_cfg->pio_sm);
		pio_sm_set_pindirs_with_mask(pio, dev_cfg->pio_sm, 0, BIT(sio_pin));
		pio_sm_restart(pio, dev_cfg->pio_sm);
		pio_sm_clkdiv_restart(pio, dev_cfg->pio_sm);
		pio_sm_put(pio, dev_cfg->pio_sm, (rx_size * data->bits) - 1);
		pio_sm_exec(pio, dev_cfg->pio_sm, pio_encode_jmp(data->pio_rx_offset));
		pio_sm_set_enabled(pio, dev_cfg->pio_sm, true);
...
#define SPI_PICO_PIO_INIT(inst)                                                                    \
	...
	static struct spi_pico_pio_config spi_pico_pio_config_##inst = {                           \
		...
		.pio_sm = DT_INST_REG_ADDR_BY_NAME_OR(inst, sm, DT_INST_REG_ADDR(inst)),           \
