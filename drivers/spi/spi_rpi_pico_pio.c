				  ...
				  struct spi_pico_pio_data *data, const struct spi_config *spi_cfg)
{
	...
	if (data->pio_tx_program) {
		pio_remove_program(data->pio, data->pio_tx_program, data->pio_tx_offset);
		data->pio_tx_program = NULL;
		data->pio_tx_offset = PIO_MAX_OFFSET;
	}

	if (data->pio_rx_program) {
		pio_remove_program(data->pio, data->pio_rx_program, data->pio_rx_offset);
	...
	if (dev_cfg->sio_gpio.port) {
		...
		data->pio_tx_offset = pio_add_program(data->pio, data->pio_tx_program);

		data->pio_rx_program = RPI_PICO_PIO_GET_PROGRAM(spi_sio_mode_0_0_rx);
		data->pio_rx_offset = pio_add_program(data->pio, data->pio_rx_program);
		...
		if (!pio_can_add_program(data->pio, data->pio_tx_program)) {
			return -EBUSY;
		}
