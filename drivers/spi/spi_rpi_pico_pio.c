...
static inline int spi_pico_pio_sm_complete(const struct spi_pico_pio_config *dev_cfg,
					   struct spi_pico_pio_data *data)
{
	return ((dev_cfg->pio->sm[data->pio_sm].addr == data->pio_tx_offset) &&
		pio_sm_is_tx_fifo_empty(dev_cfg->pio, data->pio_sm));
}
				  ...
				  struct spi_pico_pio_data *data, const struct spi_config *spi_cfg)
{
	...
	if (dev_cfg->sio_gpio.port) {
		...
		data->pio_tx_offset = pio_add_program(
			dev_cfg->pio, RPI_PICO_PIO_GET_PROGRAM(spi_sio_mode_0_0_tx));

		data->pio_rx_offset = pio_add_program(
			dev_cfg->pio, RPI_PICO_PIO_GET_PROGRAM(spi_sio_mode_0_0_rx));
		...
		pio_sm_set_pindirs_with_mask(dev_cfg->pio, data->pio_sm,
					     (BIT(clk->pin) | BIT(sio->pin)),
					     (BIT(clk->pin) | BIT(sio->pin)));
		pio_sm_set_pins_with_mask(dev_cfg->pio, data->pio_sm, 0,
					  BIT(clk->pin) | BIT(sio->pin));
		pio_gpio_init(dev_cfg->pio, sio->pin);
		pio_gpio_init(dev_cfg->pio, clk->pin);

		pio_sm_init(dev_cfg->pio, data->pio_sm, data->pio_tx_offset, &sm_config);
		pio_sm_set_enabled(dev_cfg->pio, data->pio_sm, true);
#else
		...
		if (!pio_can_add_program(dev_cfg->pio, program)) {
			return -EBUSY;
		}

		data->pio_tx_offset = pio_add_program(dev_cfg->pio, program);
		sm_config = pio_get_default_sm_config();
		...
		pio_sm_set_consecutive_pindirs(dev_cfg->pio, data->pio_sm, miso->pin, 1, false);
		pio_sm_set_pindirs_with_mask(dev_cfg->pio, data->pio_sm,
					     (BIT(clk->pin) | BIT(mosi->pin)),
					     (BIT(clk->pin) | BIT(mosi->pin)));
		pio_sm_set_pins_with_mask(dev_cfg->pio, data->pio_sm, (cpol << clk->pin),
					  BIT(clk->pin) | BIT(mosi->pin));
		pio_gpio_init(dev_cfg->pio, mosi->pin);
		pio_gpio_init(dev_cfg->pio, miso->pin);
		pio_gpio_init(dev_cfg->pio, clk->pin);
...
static void spi_pico_pio_txrx_4_wire(const struct device *dev)
{
	...
	pio_sm_clear_fifos(dev_cfg->pio, data->pio_sm);

	while (data->rx_count < chunk_len || data->tx_count < chunk_len) {
		/* Fill up fifo with available TX data */
		while ((!pio_sm_is_tx_fifo_full(dev_cfg->pio, data->pio_sm)) &&
			...
			case 4: {
				...
				spi_pico_pio_sm_put32(dev_cfg->pio, data->pio_sm, txrx);
			} break;
			...
			case 2: {
				...
				spi_pico_pio_sm_put16(dev_cfg->pio, data->pio_sm, txrx);
			} break;
			...
			case 1: {
				...
				spi_pico_pio_sm_put8(dev_cfg->pio, data->pio_sm, txrx);
			} break;
		...
		while ((!pio_sm_is_rx_fifo_empty(dev_cfg->pio, data->pio_sm)) &&
		       data->rx_count < chunk_len && fifo_cnt > 0) {
			switch (data->dfs) {
			case 4: {
				txrx = spi_pico_pio_sm_get32(dev_cfg->pio, data->pio_sm);
			...
			case 2: {
				txrx = spi_pico_pio_sm_get16(dev_cfg->pio, data->pio_sm);
			...
			case 1: {
				txrx = spi_pico_pio_sm_get8(dev_cfg->pio, data->pio_sm);
...
static void spi_pico_pio_txrx_3_wire(const struct device *dev)
{
	...
	if (txbuf) {
		pio_sm_set_enabled(dev_cfg->pio, data->pio_sm, false);
		pio_sm_set_wrap(dev_cfg->pio, data->pio_sm,
				data->pio_tx_offset +
					RPI_PICO_PIO_GET_WRAP_TARGET(spi_sio_mode_0_0_tx),
				data->pio_tx_offset + RPI_PICO_PIO_GET_WRAP(spi_sio_mode_0_0_tx));
		pio_sm_clear_fifos(dev_cfg->pio, data->pio_sm);
		pio_sm_set_pindirs_with_mask(dev_cfg->pio, data->pio_sm, BIT(sio_pin),
					     BIT(sio_pin));
		pio_sm_restart(dev_cfg->pio, data->pio_sm);
		pio_sm_clkdiv_restart(dev_cfg->pio, data->pio_sm);
		pio_sm_exec(dev_cfg->pio, data->pio_sm, pio_encode_jmp(data->pio_tx_offset));
		pio_sm_set_enabled(dev_cfg->pio, data->pio_sm, true);
		...
		while ((!pio_sm_is_tx_fifo_empty(dev_cfg->pio, data->pio_sm)) ||
		       (!spi_pico_pio_sm_complete(dev_cfg, data))) {
			;
		}
	}
	...
	if (rxbuf) {
		pio_sm_set_enabled(dev_cfg->pio, data->pio_sm, false);
		pio_sm_set_wrap(dev_cfg->pio, data->pio_sm, data->pio_rx_wrap_target,
				data->pio_rx_wrap);
		pio_sm_clear_fifos(dev_cfg->pio, data->pio_sm);
		pio_sm_set_pindirs_with_mask(dev_cfg->pio, data->pio_sm, 0, BIT(sio_pin));
		pio_sm_restart(dev_cfg->pio, data->pio_sm);
		pio_sm_clkdiv_restart(dev_cfg->pio, data->pio_sm);
		pio_sm_put(dev_cfg->pio, data->pio_sm, (rx_size * data->bits) - 1);
		pio_sm_exec(dev_cfg->pio, data->pio_sm, pio_encode_jmp(data->pio_rx_offset));
		pio_sm_set_enabled(dev_cfg->pio, data->pio_sm, true);
...
#define SPI_PICO_PIO_INIT(inst)                                                                    \
	...
	static struct spi_pico_pio_config spi_pico_pio_config_##inst = {                           \
		...
		.pio = PIO_OBJECT_FROM_DT_NODE(DT_INST_PARENT(inst)),                              \
