...
static int ra_spi_b_configure(const struct device *dev, const struct spi_config *config)
{
	...
	fsp_err = R_SPI_B_CalculateBitrate(config->frequency, data->fsp_config_extend.clock_source,
	...
	if (fsp_err != FSP_SUCCESS) {
		LOG_INF("R_SPI_B_Open error: %d", fsp_err);
	}
...
static const struct spi_driver_api ra_spi_driver_api = {
	.transceive = ra_spi_b_transceive,
#ifdef CONFIG_SPI_ASYNC
...
#define RA_SPI_INIT(index)                                                                         \
	...
	static struct ra_spi_data ra_spi_data_##index = {                                          \
		...
		.fsp_config_extend = {.clock_source = SPI_B_CLOCK_SOURCE_PCLK},                    \
