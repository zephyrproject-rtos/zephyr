...
static void renesas_ra_sci_b_transceive_data_polling(struct spi_renesas_ra_sci_b_data *data)
{
	...
	RP_SCI_B_SPI_StartTransferPolling(&data->fsp_ctrl);
	...
	do {
		...
		RP_SCI_B_SPI_WriteOneBytePolling(&data->fsp_ctrl, tx_byte);
		RP_SCI_B_SPI_ReadOneBytePolling(&data->fsp_ctrl, &rx_byte);
	...
	RP_SCI_B_SPI_EndTransferPolling(&data->fsp_ctrl);
}
#endif
