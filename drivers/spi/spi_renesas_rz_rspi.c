...
static int spi_rz_rspi_configure(const struct device *dev, const struct spi_config *spi_config)
{
	struct spi_rz_rspi_data *data = dev->data;
	const struct spi_rz_rspi_config *config = dev->config;
	...
	if (spi_config->operation & SPI_FRAME_FORMAT_TI) {
		LOG_ERR("TI frame not supported");
		return -ENOTSUP;
	}

	if (IS_ENABLED(CONFIG_SPI_EXTENDED_MODES) &&
	    (spi_config->operation & SPI_LINES_MASK) != SPI_LINES_SINGLE) {
		LOG_DEV_ERR(dev, "Only single line mode is supported");
		return -ENOTSUP;
	}

	/* SPI mode */
	if (spi_config->operation & SPI_OP_MODE_SLAVE) {
	...
	/* SPI bit width */
	spi_width = (spi_bit_width_t)(SPI_WORD_SIZE_GET(spi_config->operation) - 1);
	...
	/* Calculate bitrate */
	if (spi_config->frequency > 0) {
		err = R_RSPI_CalculateBitrate(spi_config->frequency,
...
static int transceive(const struct device *dev, const struct spi_config *spi_config,
...
#endif
	spi_context_lock(spi_ctx, asynchronous, cb, userdata, spi_config);
...
end_transceive:
	...
	ret = spi_rtio_transceive(rtio_ctx, spi_config, tx_bufs, rx_bufs);
#endif /* CONFIG_SPI_RTIO */
...
static int spi_rz_rspi_transceive_sync(const struct device *dev,
				       const struct spi_config *spi_config,
				       const struct spi_buf_set *tx_bufs,
				       const struct spi_buf_set *rx_bufs)
{
	return transceive(dev, spi_config, tx_bufs, rx_bufs, false, NULL, NULL);
}

static int spi_rz_rspi_release(const struct device *dev, const struct spi_config *spi_config)
{
	struct spi_rz_rspi_data *data = dev->data;
...
static int spi_rz_rspi_transceive_async(const struct device *dev,
					const struct spi_config *spi_config,
					const struct spi_buf_set *tx_bufs,
					const struct spi_buf_set *rx_bufs, spi_callback_t cb,
					void *userdata)
{
	return transceive(dev, spi_config, tx_bufs, rx_bufs, true, cb, userdata);
}
