...
#ifdef CONFIG_SPI_ESP32_INTERRUPT
static void IRAM_ATTR spi_esp32_isr(const void *arg)
{
...
#define IRQ_CONFIG_FUNC(idx)                                                                       \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN_BY_IDX(idx, 0), IRQ_DEFAULT_PRIORITY,                     \
			    spi_esp32_isr, DEVICE_DT_INST_GET(idx), ESP_INTR_FLAG_IRAM);           \
