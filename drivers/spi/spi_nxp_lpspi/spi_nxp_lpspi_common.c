...
static inline uint32_t lpspi_set_delays(const struct device *dev, uint32_t prescaled_clock)
{
	...
	if (config->pcs_sck_delay <= minimum_delay_ns) {
		ccr |= LPSPI_CCR_PCSSCK(lpspi_calc_delay_scaler(config->pcs_sck_delay,
							 prescaled_clock, 0));
	}

	if (config->sck_pcs_delay <= minimum_delay_ns) {
		ccr |= LPSPI_CCR_SCKPCS(lpspi_calc_delay_scaler(config->sck_pcs_delay,
							 prescaled_clock, 0));
	}

	if (config->transfer_delay <= (2 * minimum_delay_ns)) {
		ccr |= LPSPI_CCR_DBT(lpspi_calc_delay_scaler(config->transfer_delay,
						      prescaled_clock, 1));
	}
