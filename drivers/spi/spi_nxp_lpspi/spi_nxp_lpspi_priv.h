...
#define DEV_CFG(_dev)  ((const struct spi_nxp_config *)(_dev)->config)
#define DEV_DATA(_dev) ((struct spi_nxp_data *)(_dev)->data)

struct spi_nxp_config {
 ...
 *
 * Should be called by specific driver init function
 */
int spi_nxp_init_common(const struct device *dev);

#define SPI_NXP_LPSPI_IRQ_FUNC_LP_FLEXCOMM(n)                                                      \
	nxp_lp_flexcomm_setirqhandler(DEVICE_DT_GET(DT_INST_PARENT(n)), DEVICE_DT_INST_GET(n),     \
				      LP_FLEXCOMM_PERIPH_LPSPI, lpspi_isr);

#define SPI_NXP_LPSPI_IRQ_FUNC_DISTINCT(n)                                                         \
	IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), lpspi_isr, DEVICE_DT_INST_GET(n),   \
		    0);                                                                            \
	irq_enable(DT_INST_IRQN(n));

#define SPI_NXP_LPSPI_IRQ_FUNC(n) COND_CODE_1(DT_NODE_HAS_COMPAT(DT_INST_PARENT(n),		   \
								  nxp_lp_flexcomm),		   \
						(SPI_NXP_LPSPI_IRQ_FUNC_LP_FLEXCOMM(n)),	   \
						(SPI_NXP_LPSPI_IRQ_FUNC_DISTINCT(n)))
...
#define SPI_NXP_LPSPI_CONFIG_INIT(n)                                                               \
	static const struct spi_nxp_config spi_nxp_config_##n = {                                  \
		DEVICE_MMIO_NAMED_ROM_INIT(reg_base, DT_DRV_INST(n)),                              \
		.clock_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(n)),                                \
		.clock_subsys = (clock_control_subsys_t)DT_INST_CLOCKS_CELL(n, name),              \
		.irq_config_func = spi_nxp_config_func_##n,                                        \
...
#define SPI_NXP_LPSPI_COMMON_INIT(n)                                                               \
	PINCTRL_DT_INST_DEFINE(n);                                                                 \
                                                                                                   \
	static void spi_nxp_config_func_##n(const struct device *dev)                              \
	{                                                                                          \
		SPI_NXP_LPSPI_IRQ_FUNC(n)                                                          \
	}

#define SPI_NXP_LPSPI_COMMON_DATA_INIT(n)                                                          \
	SPI_CONTEXT_INIT_LOCK(spi_nxp_data_##n, ctx),                                              \
		SPI_CONTEXT_INIT_SYNC(spi_nxp_data_##n, ctx),                                      \
