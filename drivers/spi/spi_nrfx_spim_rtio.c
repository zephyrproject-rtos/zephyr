...
static void iodev_end_txn(const struct device *dev, int result)
{
	...
	if (spi_rtio_complete(spi_rtio_ctx, result) == false) {
		if ((spi_cfg->operation & SPI_HOLD_ON_CS) == 0) {
				 ...
				 const struct spi_buf_set *rx_bufs)
{
	...
	return spi_rtio_transceive(dev_config->spi_rtio_ctx, spi_cfg, tx_bufs, rx_bufs);
}
				       ...
				       void *userdata)
{
	...
	return spi_rtio_transceive_cb(dev_config->spi_rtio_ctx,
				      spi_cfg,
				      tx_bufs,
				      rx_bufs,
				      cb,
...
static void driver_api_iodev_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	if (spi_rtio_submit(spi_rtio_ctx, iodev_sqe)) {
