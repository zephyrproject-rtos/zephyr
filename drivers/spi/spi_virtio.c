...
/**
 * @brief Translate the SPI operation word into the virtio mode bits
 *
 * @param op SPI operation word
 * @return mode bits for struct virtio_spi_transfer_head
 */
static uint32_t spi_virtio_mode_bits(spi_operation_t op)
{
	uint32_t mode = 0;
...
/**
 * @brief Check the requested mode bits against the device capabilities
 *
 * @param data driver data with the cached capabilities
 * @param mode requested mode bits
 * @return 0 on success or negative error code on failure
 */
static int spi_virtio_check_mode(const struct spi_virtio_data *data, uint32_t mode)
{
	uint32_t mf = data->mode_func_supported;

	if ((mode & VIRTIO_SPI_CPHA) != 0 && (mf & VIRTIO_SPI_MF_SUPPORT_CPHA_1) == 0) {
		LOG_ERR("device does not support CPHA=1");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_CPHA) == 0 && (mf & VIRTIO_SPI_MF_SUPPORT_CPHA_0) == 0) {
		LOG_ERR("device does not support CPHA=0");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_CPOL) != 0 && (mf & VIRTIO_SPI_MF_SUPPORT_CPOL_1) == 0) {
		LOG_ERR("device does not support CPOL=1");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_CPOL) == 0 && (mf & VIRTIO_SPI_MF_SUPPORT_CPOL_0) == 0) {
		LOG_ERR("device does not support CPOL=0");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_CS_HIGH) != 0 && (mf & VIRTIO_SPI_MF_SUPPORT_CS_HIGH) == 0) {
		LOG_ERR("device does not support an active high chip select");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_MODE_LSB_FIRST) != 0 &&
	    (mf & VIRTIO_SPI_MF_SUPPORT_LSB_FIRST) == 0) {
		LOG_ERR("device does not support LSB first transfers");
		return -ENOTSUP;
	}

	if ((mode & VIRTIO_SPI_MODE_LOOP) != 0 && (mf & VIRTIO_SPI_MF_SUPPORT_LOOPBACK) == 0) {
...
static int spi_virtio_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	...
	uint32_t mode = spi_virtio_mode_bits(op);
	uint8_t nbits;
	int ret;

	if (SPI_OP_MODE_GET(op) == SPI_OP_MODE_PERIPHERAL) {
	...
	if (spi_cfg->peripheral >= data->cs_max_number) {
		LOG_ERR("chip select %u out of range (%u available)", spi_cfg->peripheral,
			data->cs_max_number);
		return -EINVAL;
	}
	...
	ret = spi_virtio_check_mode(data, mode);
	if (ret != 0) {
		return ret;
	}
	...
	data->head.chip_select_id = spi_cfg->peripheral;
...
static int spi_virtio_transfer(const struct device *dev, size_t len, bool last)
{
	...
	/* the device only reads the tx buffers, casting the const away is safe */
	if (spi_context_tx_buf_on(ctx)) {
		bufs[n++] = (struct virtq_buf){.addr = (void *)(uintptr_t)ctx->tx_buf, .len = len};
	} else if (!spi_context_rx_buf_on(ctx)) {
		bufs[n++] = (struct virtq_buf){.addr = (void *)(uintptr_t)spi_virtio_dummy_tx,
					       .len = len};
	} else {
		/* rx only, the device clocks out without a tx buffer */
	}
