...
/* Several cases on the supported data width
 * - For non-H7 compatible:
 *   - Some series only support 8 and 16-bit
 *   - Some series support 4 to 16-bit for all instances
 * - For H7 compatible:
 *   - All series have at least one instance supporting 4 to 32-bit
 *   - Some series have instances supporting only 4 to 16-bit
 *   - Some series have instances supporting only 8 and 16-bit
 */
#if defined(LL_SPI_DATAWIDTH_4BIT)
#define STM32_SPI_DATAWIDTH_MIN	4
#if defined(LL_SPI_DATAWIDTH_32BIT)
#define STM32_SPI_DATAWIDTH_MAX	32
#define STM32_SPI_LIMITED_DATAWIDTH_MAX	16
#else
#define STM32_SPI_DATAWIDTH_MAX	16
#endif

#define STM32_SPI_DATAWIDTH(i, _)								\
	_CONCAT_2(LL_SPI_DATAWIDTH_, UTIL_INC(UTIL_INC(UTIL_INC(UTIL_INC(i)))), BIT)
static const uint32_t table_datawidth[] = {
	LISTIFY(UTIL_DEC(UTIL_DEC(UTIL_DEC(STM32_SPI_DATAWIDTH_MAX))), STM32_SPI_DATAWIDTH, (,))
};

#if defined(IS_SPI_LIMITED_INSTANCE)
/* Even though the RM describes the instances supporting 4 to 16-bit as full-featured, the HAL
 * macro IS_SPI_LIMITED_INSTANCE applies to them as well as to the instances limited to 8 & 16-bit.
 * Device tree property limited-instance only applies to instances limited to 8 and 16-bit, that way
 * the two can be distinguished.
 */
#define IS_WIDTH_SUPPORTED(spi, width, limited)							\
	(!IS_SPI_LIMITED_INSTANCE(spi) ?							\
	(width >= STM32_SPI_DATAWIDTH_MIN) && (width <= STM32_SPI_DATAWIDTH_MAX) :		\
		(limited ? (width == 8) || (width == 16) :					\
		(width >= STM32_SPI_DATAWIDTH_MIN) && (width <= STM32_SPI_LIMITED_DATAWIDTH_MAX)))
#else
#define IS_WIDTH_SUPPORTED(spi, width, limited)							\
	((width >= STM32_SPI_DATAWIDTH_MIN) && (width <= STM32_SPI_DATAWIDTH_MAX))
#endif
...
#define IS_WIDTH_SUPPORTED(spi, width, limited) ((width == 8) || (width == 16))
#endif /* LL_SPI_DATAWIDTH_4BIT */
...
static uint32_t bits2bytes(spi_operation_t operation)
{
...
{
	uint32_t frame_size = bits2bytes(data->ctx.config->operation);
...
#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi)
	if (!IS_WIDTH_SUPPORTED(spi, SPI_WORD_SIZE_GET(config->operation), cfg->limited_instance)) {
#else
	if (!IS_WIDTH_SUPPORTED(spi, SPI_WORD_SIZE_GET(config->operation), false)) {
#endif
		return -ENOTSUP;
	}
...
#define STM32_SPI_INIT(id)							\
	...
	static const struct spi_stm32_config spi_stm32_cfg_##id = {		\
		...
		IF_ENABLED(DT_HAS_COMPAT_STATUS_OKAY(st_stm32h7_spi),		\
			   (.limited_instance =	DT_INST_PROP(id, limited_instance)))	\
	};									\
										\
