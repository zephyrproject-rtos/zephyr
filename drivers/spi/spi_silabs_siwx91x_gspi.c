...
#define GSPI_TX_UDMA_CH_NUM                   11
#define GSPI_RX_UDMA_CH_NUM                   10
#define GSPI_CLK_EN_BIT_FIELD                 0
#define GSPI_CLK_SYNC_BIT_FIELD               1
#define GSPI_DATA_SAMPLE_EDGE_BIT_FIELD       0
#define GSPI_CLK_MODE_CSN0_BIT_FIELD          1
#define GSPI_RD_DATA_SWAP_MNL_BIT_FIELD       4
#define GSPI_FULL_DUPLEX_EN_BIT_FIELD         15
#define GSPI_MANUAL_WR_BIT_FIELD              1
#define GSPI_MANUAL_RD_BIT_FIELD              2
#define USE_PREV_LENGTH_BIT_FIELD             7
#define GSPI_BUSY_BIT_FIELD                   0
#define SPI_HIGH_PERFORMANCE_EN_BIT_FIELD     11
#define GSPI_MANUAL_CSN_BIT_FIELD             0
			       ...
			       spi_callback_t cb, void *userdata)
{
	...
	if (clk_div_factor < 1) {
		sys_set_bit((mem_addr_t)&cfg->reg->GSPI_CLK_CONFIG, GSPI_CLK_EN_BIT_FIELD);
		sys_set_bit((mem_addr_t)&cfg->reg->GSPI_CLK_CONFIG, GSPI_CLK_SYNC_BIT_FIELD);
	}

	/* Configure data sampling edge for high-speed transfers */
	if (bit_rate > GSPI_MAX_BAUDRATE_FOR_POS_EDGE_SAMPLE) {
		sys_set_bit((mem_addr_t)&cfg->reg->GSPI_BUS_MODE, GSPI_DATA_SAMPLE_EDGE_BIT_FIELD);
	}

	/* Set the clock divider factor */
	sys_write32(clk_div_factor, (mem_addr_t)&cfg->reg->GSPI_CLK_DIV);

	/* Configure SPI clock mode */
	if (0 == (spi_cfg->operation & (SPI_MODE_CPOL | SPI_MODE_CPHA))) {
		sys_clear_bit((mem_addr_t)&cfg->reg->GSPI_BUS_MODE, GSPI_CLK_MODE_CSN0_BIT_FIELD);
	} else {
		sys_set_bit((mem_addr_t)&cfg->reg->GSPI_BUS_MODE, GSPI_CLK_MODE_CSN0_BIT_FIELD);
	}

	/*  Update the number of Data Bits */
	sys_write32(SPI_WORD_SIZE_GET(spi_cfg->operation), (mem_addr_t)&cfg->reg->GSPI_WRITE_DATA2);

	/* Swap the read data inside the GSPI controller it-self */
	sys_clear_bit((mem_addr_t)&cfg->reg->GSPI_CONFIG2, GSPI_RD_DATA_SWAP_MNL_BIT_FIELD);

	/* Enable full-duplex mode and manual read/write */
	sys_set_bit((mem_addr_t)&cfg->reg->GSPI_CONFIG1, GSPI_FULL_DUPLEX_EN_BIT_FIELD);
	sys_set_bit((mem_addr_t)&cfg->reg->GSPI_CONFIG1, GSPI_MANUAL_WR_BIT_FIELD);
	sys_set_bit((mem_addr_t)&cfg->reg->GSPI_CONFIG1, GSPI_MANUAL_RD_BIT_FIELD);
	sys_set_bit((mem_addr_t)&cfg->reg->GSPI_WRITE_DATA2, USE_PREV_LENGTH_BIT_FIELD);

	/* Configure FIFO thresholds */
	sys_write32(0, (mem_addr_t)&cfg->reg->GSPI_FIFO_THRLD);
#ifdef CONFIG_SPI_SILABS_SIWX91X_GSPI_DMA
	if (spi_siwx91x_is_dma_enabled_instance(dev)) {
		...
		if (data->dma_rx.chan_nb == GSPI_RX_UDMA_CH_NUM) {
			channel_filter = GSPI_RX_UDMA_CH_NUM;
			data->dma_rx.chan_nb =
				dma_request_channel(data->dma_rx.dma_dev, &channel_filter);
		} else {
			return -EINVAL;
		}
		...
		if (data->dma_tx.chan_nb == GSPI_TX_UDMA_CH_NUM) {
			channel_filter = GSPI_TX_UDMA_CH_NUM;
			data->dma_tx.chan_nb =
				dma_request_channel(data->dma_tx.dma_dev, &channel_filter);
		}

		if (data->dma_tx.chan_nb != GSPI_TX_UDMA_CH_NUM) {
			dma_release_channel(data->dma_rx.dma_dev, data->dma_rx.chan_nb);
			data->dma_rx.chan_nb = -1;
			return -EAGAIN;
		}
	}
#endif /* CONFIG_SPI_SILABS_SIWX91X_GSPI_DMA */
					 ...
					 uint32_t channel, int status)
{
	...
	instance_ctx->asynchronous = false;
}
...
static int gspi_siwx91x_transceive_dma(const struct device *dev, const struct spi_config *config)
{
	...
	uint32_t dma_base_addr = *((uint32_t *)dma_dev->config);
	UDMA0_Type *dma_reg_base = (UDMA0_Type *)dma_base_addr;
	...
	sys_write32(BIT(data->dma_rx.chan_nb), (mem_addr_t)&dma_reg_base->CHNL_ENABLE_SET);
	sys_write32(BIT(data->dma_tx.chan_nb), (mem_addr_t)&dma_reg_base->CHNL_ENABLE_SET);
...
static void gspi_siwx91x_send(const struct gspi_siwx91x_config *cfg, uint32_t data)
{
	...
	sys_write32(data, (mem_addr_t)&cfg->reg->GSPI_WRITE_FIFO[0]);
}

static uint32_t gspi_siwx91x_receive(const struct gspi_siwx91x_config *cfg)
{
	return sys_read32((mem_addr_t)&cfg->reg->GSPI_READ_FIFO[0]);
}
...
static int gspi_siwx91x_shift_frames(const struct device *dev)
{
	...
	while (sys_test_bit((mem_addr_t)&cfg->reg->GSPI_STATUS, GSPI_BUSY_BIT_FIELD)) {
	}
				   ...
				   spi_callback_t cb, void *userdata)
{
	...
	/* Check if the operation is asynchronous and if DMA is enabled */
	if (asynchronous & spi_siwx91x_is_dma_enabled_instance(dev)) {
...
static int gspi_siwx91x_init(const struct device *dev)
{
	...
	sys_set_bit((mem_addr_t)&cfg->reg->GSPI_BUS_MODE, SPI_HIGH_PERFORMANCE_EN_BIT_FIELD);
	sys_clear_bit((mem_addr_t)&cfg->reg->GSPI_CONFIG1, GSPI_MANUAL_CSN_BIT_FIELD);
