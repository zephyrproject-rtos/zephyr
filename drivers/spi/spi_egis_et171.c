		...
		if (!IS_ALIGN(rx_buf->buf)) {
			if (rx_buf->len <= (2 * cache_line_size)) {
				...
				*FregSize += rx_buf->len;
				...
				/* head */
				*FregSize += ROUND_UP((size_t)rx_buf->buf, cache_line_size) -
					     (size_t)rx_buf->buf;
				*ExtendCount += 1;
				/* tail */
				if (((size_t)rx_buf->buf + rx_buf->len) !=
				    ROUND_DOWN((size_t)rx_buf->buf + rx_buf->len,
					       cache_line_size)) {
					*FregSize += ((size_t)rx_buf->buf + rx_buf->len) -
						     ROUND_DOWN((size_t)rx_buf->buf + rx_buf->len,
								cache_line_size);
					*ExtendCount += 1;
				}
...
int transceive_with_extend_buffer(const struct device *dev, const struct spi_config *config,
				  ...
				  const struct spi_buf_set *rx_bufs, int ExtendCount, int FregSize)
{
	int ret = 0;
	struct spi_et171_data *const data = dev->data;
	int cache_line_size = sys_cache_data_line_size_get();
	int iNewCount = rx_bufs->count + ExtendCount;
	int align_buf_size = ROUND_UP(FregSize, cache_line_size);

	ret = allocate_dma_buffers(&data->dma_buf_ctx, iNewCount, align_buf_size, cache_line_size);
	...
	data->dma_buf_ctx.count = iNewCount;
...
#if !defined(CONFIG_DCACHE) || !defined(CONFIG_ANDES_SPI_DMA_MODE)
	ret = transceive(dev, config, tx_bufs, rx_bufs);
#else
	int iExtendCount = 0, iFregSize = 0;
	int need_align = is_need_alignment(tx_bufs, rx_bufs, &iExtendCount, &iFregSize);

	if (need_align == false) {
		ret = transceive(dev, config, tx_bufs, rx_bufs);
	} else {
		ret = transceive_with_extend_buffer(dev, config, tx_bufs, rx_bufs, iExtendCount,
						    iFregSize);
	}
#endif
