					  ...
					  uint32_t ref_hz)
{
	...
	uint32_t clkd = 0;
	uint32_t clkg = 0;
	uint32_t fRatio = ref_hz / speed_hz;

	if ((ref_hz % speed_hz != 0) && (fRatio < OMAP_MCSPI_MAX_DIVIDER)) {
		fRatio++;
	}

	if ((fRatio & (fRatio - 1)) != 0) {
		clkg = 1;
		extclk = (fRatio - 1) >> 4;
		clkd = (fRatio - 1) & 0X0F;
	} else {
...
static int omap_mcspi_transceive_pio(const struct device *dev, size_t count)
{
	...
	uint32_t word_mask = (1ULL << word_size) - 1;
	...
	do {
		...
		if (rx_buf) {
			if (count == 1 && !tx_buf && turbo) {
				...
				read_rx(rx_buf, rx_reg, dfs, word_mask); /* extra rx for turbo */
				rx_buf += dfs;
				count = 0;
			...
			read_rx(rx_buf, rx_reg, dfs, word_mask); /* rx */
			rx_buf += dfs;
		}
	} while (count);
