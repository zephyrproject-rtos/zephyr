...
{
	for (uint32_t divisor = 0; divisor < 15; divisor++) {
		if (speed_hz >= (ref_hz >> divisor)) {
			return divisor;
		}
	}
					  ...
					  uint32_t ref_hz)
{
	...
	uint32_t clkd = 0, clkg, divisor = 0, extclk = 0;
	...
	if (speed_hz < (ref_hz / OMAP_MCSPI_MAX_DIVIDER)) {
		...
		divisor = (ref_hz + speed_hz - 1) / speed_hz;
		speed_hz = ref_hz / divisor;
		clkd = (divisor - 1) & 0xf;
		clkg = 1;
		extclk = (divisor - 1) >> 4;
	}
...
static ALWAYS_INLINE void read_rx(uint8_t *rx_buf, volatile uint32_t *rx_reg, uint8_t dfs)
{
	switch (dfs) {
	case 1: {
		UNALIGNED_PUT(*rx_reg, (uint8_t *)rx_buf);
		break;
	}
	case 2: {
		UNALIGNED_PUT(*rx_reg, (uint16_t *)rx_buf);
		break;
	}
	case 4: {
		UNALIGNED_PUT(*rx_reg, (uint32_t *)rx_buf);
		break;
	}
	default: {
		/* unreachable */
...
static int omap_mcspi_transceive_pio(const struct device *dev, size_t count)
{
	...
	do {
		...
		if (tx_buf) {
			if (omap_mcspi_wait_for_reg_bit(chstat, OMAP_MCSPI_CHSTAT_TXS)) {
		...
		if (rx_buf) {
			if (count == 1 && !tx_buf && turbo) {
				omap_mcspi_channel_enable(dev, false); /* last rx in rx-only */
				...
				read_rx(rx_buf, rx_reg, dfs); /* extra rx for turbo */
				rx_buf += dfs;
				count = 0;
			} else if (count == 0 && !tx_buf) {
				omap_mcspi_channel_enable(dev, false); /* last rx in rx-only */
			}
			...
			read_rx(rx_buf, rx_reg, dfs); /* rx */
			rx_buf += dfs;
		}
	} while (count);

	/* for tx-only mode, be sure all words have shifted out */
	if (!rx_buf) {
		if (omap_mcspi_wait_for_reg_bit(chstat, OMAP_MCSPI_CHSTAT_TXS)) {
			LOG_ERR("TXS timed out");
		} else if (omap_mcspi_wait_for_reg_bit(chstat, OMAP_MCSPI_CHSTAT_EOT)) {
				     ...
				     spi_callback_t callback, void *userdata)
{
	...
	while (spi_context_tx_on(ctx) || spi_context_rx_on(ctx)) {
		...
		if (ret < 0) {
			LOG_ERR("Transaction failed, TX/RX left: %d/%d",
				spi_context_tx_len_left(ctx), spi_context_rx_len_left(ctx));
			goto cleanup;
		}
	}
