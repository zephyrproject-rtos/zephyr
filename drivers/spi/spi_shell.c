...
#define NUM_EMPTY_SPI_THINGS 128
#define NUM_EMPTY_MAPS       128

/* Runs the given fn only if the node_id belongs to a spi device, which is on a okay spi bus. */
...
/* Struct representing either a spi bus or an spi device. */
struct spi_thing {
	/* Device name. Either a spi bus or spi device. */
	const char *name;
	struct spi_dt_spec spec;
};

/* Struct used to map a label to a name of the associated spi_thing. */
struct map {
	/* Either nodelabel or device name of either spi bus or spi device. */
	const char *label;
	/* Device name. This is the same as the name of the associated spi_thing struct. */
	const char *name;
};

#define INST_SPI_THING_AS_SPI_DEV(node_id)                                                         \
	{                                                                                          \
		.name = DEVICE_DT_NAME(node_id),                                                   \
		.spec = SPI_DT_SPEC_GET(node_id, 0, 0),                                            \
	},

#define INST_SPI_THING_AS_SPI_DEV_AS_SPI_BUS(dev)                                                  \
	(struct spi_thing){                                                                        \
		.name = dev->name,                                                                 \
		.spec =                                                                            \
			{                                                                          \
...
/* Instantiate spi thing struct from node_id, if that is a spi device.  */
#define INST_ALL_SPI_DEVICES_AS_SPI_THINGS(node_id)                                                \
	RUN_FN_ON_SPI_DEVICE(node_id, INST_SPI_THING_AS_SPI_DEV(node_id))

/* List of spi things. At compile time we instantiate structs for all spi devices.
 * Additional empty space is reserved for the spi buses, which are added in spi_buses_init at
 * runtime.
 */
static struct spi_thing spi_things[] = {DT_FOREACH_STATUS_OKAY_NODE(
	INST_ALL_SPI_DEVICES_AS_SPI_THINGS) CREATE_NUM_EMPTY_STRUCTS(NUM_EMPTY_SPI_THINGS)};
static size_t num_spi_things = ARRAY_SIZE(spi_things) - NUM_EMPTY_SPI_THINGS;
 ...
 * runtime.
 */
static struct map maps[] = {DT_FOREACH_STATUS_OKAY_NODE(INST_MAPS_FROM_SPI_DEVICE_NODELABELS)
				    DT_FOREACH_STATUS_OKAY_NODE(INST_MAPS_FROM_SPI_DEVICE_NODE_ID)
					    CREATE_NUM_EMPTY_STRUCTS(NUM_EMPTY_MAPS)};
static size_t num_maps = ARRAY_SIZE(maps) - NUM_EMPTY_MAPS;
 ...
 * - add an entry to spi_things array
 * - add an entry to maps array by it's name
...
static int spi_buses_init(void)
{
	...
	while (1) {
		...
		if (num_spi_things == ARRAY_SIZE(spi_things)) {
			printk("ERROR: not enough space in spi_things array\n");
			printk("Increase NUM_EMPTY_SPI_THINGS in spi_shell.c\n");
			break;
		}

		spi_things[num_spi_things++] = INST_SPI_THING_AS_SPI_DEV_AS_SPI_BUS(dev);
		...
		if (num_maps + nl->num_nodelabels > ARRAY_SIZE(maps)) {
			...
			printk("Increase NUM_EMPTY_MAPS in spi_shell.c\n");
			break;
		}
...
SYS_INIT(spi_buses_init, APPLICATION, 0);
 ...
 * the spi_things array for a matching name and return the corresponding spi_dt_spec.
 *
 * @param[in] label
 *
...
static struct spi_dt_spec *find_spec_by_label(const char *label)
{
	...
	for (size_t i = 0; i < num_spi_things; i++) {
		if (strcmp(name, spi_things[i].name) == 0) {
			return &spi_things[i].spec;
		}
	}
...
static void gpio_device_name_get(size_t idx, struct shell_static_entry *entry)
{
...
SHELL_DYNAMIC_CMD_CREATE(dsub_gpio_device_name, gpio_device_name_get);

static void spi_thing_name_get2(size_t idx, struct shell_static_entry *entry)
{
	if (idx >= num_maps) {
		entry->syntax = NULL;
		return;
	...
	entry->handler = NULL;
	entry->help = NULL;
	entry->subcmd = &dsub_gpio_device_name;
}

SHELL_DYNAMIC_CMD_CREATE(dsub_spi_thing_name2, spi_thing_name_get2);

static void spi_thing_name_get(size_t idx, struct shell_static_entry *entry)
{
	if (idx >= num_maps) {
		entry->syntax = NULL;
		return;
...
SHELL_DYNAMIC_CMD_CREATE(dsub_spi_thing_name, spi_thing_name_get);
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	sub_spi_cmds,
	SHELL_CMD_ARG(conf, &dsub_spi_thing_name,
		      "Configure SPI\n"
