 ...
 */
static struct spi_buf_set *copy_and_check(struct spi_buf_set *bufs, struct spi_buf *buf_copy,
					  int writable)
{
	size_t i;
	...
	K_OOPS(K_SYSCALL_MEMORY_ARRAY_READ(bufs->buffers, bufs->count, sizeof(struct spi_buf)));

	/* Not worried about overflow here: _SYSCALL_MEMORY_ARRAY_READ()
	 * takes care of it.
	 */
	bufs->buffers = memcpy(buf_copy, bufs->buffers, bufs->count * sizeof(struct spi_buf));
 ...
 * overflow.
 */
static uint32_t copy_bufs_and_transceive(const struct device *dev, const struct spi_config *config,
					 struct spi_buf_set *tx_bufs, struct spi_buf_set *rx_bufs)
{
	...
	return z_impl_spi_transceive((const struct device *)dev, config, tx_bufs, rx_bufs);
}

static inline int z_vrfy_spi_transceive(const struct device *dev, const struct spi_config *config,
					...
					const struct spi_buf_set *rx_bufs)
{
	...
	if (tx_bufs) {
		const struct spi_buf_set *tx = (const struct spi_buf_set *)tx_bufs;

		K_OOPS(K_SYSCALL_MEMORY_READ(tx_bufs, sizeof(struct spi_buf_set)));
	...
	if (rx_bufs) {
		const struct spi_buf_set *rx = (const struct spi_buf_set *)rx_bufs;

		K_OOPS(K_SYSCALL_MEMORY_READ(rx_bufs, sizeof(struct spi_buf_set)));
	...
	if (spi_cs_is_gpio(&config_copy)) {
		K_OOPS(K_SYSCALL_OBJ(config_copy.cs.gpio.port, K_OBJ_DRIVER_GPIO));
	}

	return copy_bufs_and_transceive((const struct device *)dev, &config_copy, &tx_bufs_copy,
					&rx_bufs_copy);
}
#include <zephyr/syscalls/spi_transceive_mrsh.c>

static inline int z_vrfy_spi_release(const struct device *dev, const struct spi_config *config)
{
	K_OOPS(K_SYSCALL_MEMORY_READ(config, sizeof(*config)));
