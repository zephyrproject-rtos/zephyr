...
static int spi_it51xxx_configure(const struct device *dev, const struct spi_config *spi_cfg)
{
	...
	timeout_us = 8 * USEC_PER_SEC / spi_cfg->frequency;
	timeout_us += CONFIG_SPI_COMPLETION_TIMEOUT_TOLERANCE * USEC_PER_MSEC;
...
static inline void spi_it51xxx_complete(const struct device *dev, const int status)
{
	struct spi_it51xxx_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
...
static inline int spi_it51xxx_tx(const struct device *dev)
{
	const struct spi_it51xxx_config *cfg = dev->config;
	struct spi_it51xxx_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
	...
	do {
		...
		if (k_sem_take(&data->byte_done_lock, data->byte_transfer_timeout) != 0) {
			LOG_ERR("tx: byte transfer is timeout");
			return -ETIMEDOUT;
		}
...
static inline int spi_it51xxx_rx(const struct device *dev)
{
	const struct spi_it51xxx_config *cfg = dev->config;
	struct spi_it51xxx_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
	...
	do {
		...
		if (k_sem_take(&data->byte_done_lock, data->byte_transfer_timeout) != 0) {
			LOG_ERR("rx: byte transfer is timeout");
			return -ETIMEDOUT;
		}
...
static inline int spi_it51xxx_tx_rx(const struct device *dev)
{
	const struct spi_it51xxx_config *cfg = dev->config;
	struct spi_it51xxx_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
	...
	for (size_t i = 0; i < ctx->tx_len; i++) {
		...
		if (k_sem_take(&data->byte_done_lock, data->byte_transfer_timeout) != 0) {
			LOG_ERR("tx/rx: tx byte transfer is timeout");
			return -ETIMEDOUT;
		}
	}
	...
	return spi_it51xxx_rx(dev);
}

static int spi_it51xxx_xfer(const struct device *dev)
{
	const struct spi_it51xxx_config *cfg = dev->config;
	struct spi_it51xxx_data *data = dev->data;
	struct spi_context *ctx = &data->ctx;
	...
	if (!spi_context_tx_on(ctx)) {
		...
		ret = spi_it51xxx_rx(dev);
	} else if (!spi_context_rx_on(ctx)) {
		/* tx only, nothing to rx */
		ret = spi_it51xxx_tx(dev);
	} else {
		ret = spi_it51xxx_tx_rx(dev);
	}

	if (spi_context_total_tx_len(ctx) || spi_context_total_rx_len(ctx)) {
		LOG_WRN("the length of tx (%d) or rx (%d) buffer is not zero",
...
static int spi_it51xxx_init(const struct device *dev)
{
	...
	irq_connect_dynamic(cfg->irq_no, 0, it51xxx_spi_isr, dev, 0);
