 ...
 * @brief
 *
 * A common driver for GD32 pinmux.
 */
...
#include <device.h>
#include <soc.h>
// #include <gd32_ll_bus.h>
// #include <gd32_ll_gpio.h>
...
const struct device * const gpio_ports[GD32_PORTS_MAX] = {
	GPIO_DEVICE(gpioa),
	GPIO_DEVICE(gpiob),
	GPIO_DEVICE(gpioc),
	GPIO_DEVICE(gpiod),
...
static int gd32_pin_configure(uint32_t pin, uint32_t func, uint32_t altf)
{
	const struct device *port_device = gpio_ports[GD32_PORT(pin)];

	/* not much here, on GD32F10x the alternate function is
	...
	return gpio_gd32_configure(port_device, GD32_PIN(pin), func, altf);
}
			       ...
			       size_t list_size, uint32_t base)
{
	...
	if (gd32_dt_pinctrl_remap(pinctrl, list_size, base)) {
	...
	for (int i = 0; i < list_size; i++) {
		...
		if (GD32_DT_PINMUX_FUNC(mux) == ALTERNATE) {
			func = pinctrl[i].pincfg | GD32_MODE_OUTPUT |
							GD32_CNF_ALT_FUNC;
		} else if (GD32_DT_PINMUX_FUNC(mux) == ANALOG) {
			func = pinctrl[i].pincfg | GD32_MODE_INPUT |
							GD32_CNF_IN_ANALOG;
		} else if (GD32_DT_PINMUX_FUNC(mux) == GPIO_IN) {
			func = pinctrl[i].pincfg | GD32_MODE_INPUT;
			pupd = func & (GD32_PUPD_MASK << GD32_PUPD_SHIFT);
			if (pupd == GD32_PUPD_NO_PULL) {
				func = func | GD32_CNF_IN_FLOAT;
			} else {
				func = func | GD32_CNF_IN_PUPD;
			}
		} else {
			/* Not supported */
			__ASSERT_NO_MSG(GD32_DT_PINMUX_FUNC(mux));
		}
#else
		if (GD32_DT_PINMUX_FUNC(mux) < ANALOG) {
			func = pinctrl[i].pincfg | GD32_MODER_ALT_MODE;
		} else if (GD32_DT_PINMUX_FUNC(mux) == ANALOG) {
			func = GD32_MODER_ANALOG_MODE;
		} else {
			/* Not supported */
			__ASSERT_NO_MSG(GD32_DT_PINMUX_FUNC(mux));
		}
		...
		pin = GD32PIN(GD32_DT_PINMUX_PORT(mux),
			       GD32_DT_PINMUX_LINE(mux));
		port_device = gpio_ports[GD32_PORT(pin)];

#ifdef CONFIG_PM_DEVICE_RUNTIME
		ret = pm_device_get(port_device);
#else
		ret = gpio_gd32_clock_request(port_device, true);
		...
		gd32_pin_configure(pin, func, GD32_DT_PINMUX_FUNC(mux));
			   ...
			   size_t list_size, uint32_t base)
{
	...
	remap = GD32_DT_PINMUX_REMAP(pinctrl[0].pinmux);

	for (int i = 1; i < list_size; i++) {
		mux = pinctrl[i].pinmux;
		remap = GD32_DT_PINMUX_REMAP(mux);

		if (GD32_DT_PINMUX_REMAP(mux) != remap) {
			return -EINVAL;
		}
	}
 ...
 * @brief pin setup
 *
 * @param pin  GD32PIN() encoded pin ID
...
int z_pinmux_gd32_set(uint32_t pin, uint32_t func)
{
	...
	if (gpio_gd32_clock_request(port_device, true)) {
		return -EIO;
	}

	return gd32_pin_configure(pin, func, func & GD32_AFR_MASK);
}
		      ...
		      size_t pins)
{
	...
	for (i = 0; i < pins; i++) {
		z_pinmux_gd32_set(pinconf[i].pin_num,
				  pinconf[i].mode);
	}
}
