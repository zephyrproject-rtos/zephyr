...
static int pinmux_mcux_set(const struct device *dev, u32_t pin, u32_t func)
{
...
static int pinmux_mcux_get(const struct device *dev, u32_t pin, u32_t *func)
{
	const struct pinmux_mcux_config *config = dev->config_info;
	PORT_Type *base = config->base;
...
static int pinmux_mcux_pullup(const struct device *dev, u32_t pin, u8_t func)
{
	return -ENOTSUP;
}

static int pinmux_mcux_input(const struct device *dev, u32_t pin, u8_t func)
{
	return -ENOTSUP;
}
