...
struct sent_nxp_s32_config {
	...
	uint8_t hw_channel_map[SRX_CNL_COUNT];
...
static uint8_t sent_nxp_s32_get_logical_channel_id(const struct device *dev, uint8_t hw_channel)
{
	const struct sent_nxp_s32_config *config = dev->config;
	uint8_t channel_id = -EINVAL;

	for (int i = 0; i < config->num_channels; i++) {
		if (config->hw_channel_map[i] == hw_channel) {
			channel_id = i;
			break;
		}
...
static int sent_nxp_s32_start(const struct device *dev, uint8_t hw_channel)
{
	const struct sent_nxp_s32_config *config = dev->config;
	struct sent_nxp_s32_data *data = dev->data;
	struct sent_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];
	uint8_t channel_id = sent_nxp_s32_get_logical_channel_id(dev, hw_channel);
	int err;
	...
	if (err) {
		LOG_ERR("Failed to start SENT %d channel %d", config->ctrl_inst, hw_channel);
		return -EIO;
	}
...
static int sent_nxp_s32_stop(const struct device *dev, uint8_t hw_channel)
{
	const struct sent_nxp_s32_config *config = dev->config;
	struct sent_nxp_s32_data *data = dev->data;
	struct sent_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];
	...
	if (err) {
		LOG_ERR("Failed to stop SENT %d channel %d", config->ctrl_inst, hw_channel);
		return -EIO;
	}
...
static int sent_nxp_s32_add_rx_callback(const struct device *dev, uint8_t hw_channel,
					 sent_rx_callback_t callback, void *user_data)
{
	struct sent_nxp_s32_data *data = dev->data;
	struct sent_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];

	if (sent_nxp_s32_get_logical_channel_id(dev, hw_channel) == -EINVAL) {
		return -EINVAL;
	}
...
#define DEV_SENT_NXP_S32_INIT(n)                                                                   \
	...
	static struct sent_nxp_s32_config sent_nxp_s32_config_##n = {                              \
		...
		.hw_channel_map = {DT_INST_FOREACH_CHILD_STATUS_OKAY_SEP(n, DT_REG_ADDR, (, ))},   \
...
#define SENT_NXP_S32_CALLBACK(n)                                                                   \
	...
	{                                                                                          \
		...
		struct sent_nxp_s32_data *data = dev->data;                                        \
		const struct sent_nxp_s32_config *config = dev->config;                            \
		uint8_t hw_channel = config->hw_channel_map[channel_id];                           \
		struct sent_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];  \
	...
	{                                                                                          \
		...
		struct sent_nxp_s32_rx_callback *rx_callback = &channel_data->rx_callback;         \
                                                                                                   \
		if (rx_callback->callback) {                                                       \
			rx_callback->callback(dev, hw_channel, NULL, SENT_STATE_FMSG_CRC_ERR,      \
	...
	{                                                                                          \
		...
		struct sent_nxp_s32_rx_callback *rx_callback = &channel_data->rx_callback;         \
                                                                                                   \
		if (rx_callback->callback) {                                                       \
			rx_callback->callback(dev, hw_channel, NULL, SENT_STATE_SMSG_CRC_ERR,      \
					      rx_callback->user_data);                             \
		}                                                                                  \
	}
