...
LOG_MODULE_REGISTER(uaol_intel_adsp);
...
static bool uaol_intel_adsp_power_steady(const struct device *dev)
{
	...
	return (ctl.part.cpa & link_mask) == (ctl.part.spa & link_mask);
}
...
static bool uaol_intel_adsp_ic_ready(const struct device *dev)
{
	struct uaol_intel_adsp_data *dp = dev->data;
	union UAOLxIC ic;

	ic.full = sys_read32(UAOLxIC_ADDR(dp));
	return ic.part.icb == 0;
}
...
static int uaol_intel_adsp_send_xhci_msg(const struct device *dev, uint32_t *msg, size_t msg_size)
{
	...
	uint32_t timeout = XHCI_MSG_TIMEOUT_USEC;
	int i, chunk_words;
...
static bool uaol_intel_adsp_ir_ready(const struct device *dev)
{
	struct uaol_intel_adsp_data *dp = dev->data;
	union UAOLxIR ir;

	ir.full = sys_read32(UAOLxIR_ADDR(dp));
	return ir.part.irvi == 1;
}
						    ...
						    uint32_t service_interval_usec)
{
	...
	divisor = sys_gcd(fcadivm, fcadivn);
	fcadivm /= divisor;
	fcadivn /= divisor;
...
static int uaol_intel_adsp_stop(const struct device *dev, int stream)
{
	...
	if (ret) {
		LOG_WRN("Failed to stop stream (ret %d), resetting stream...", ret);
