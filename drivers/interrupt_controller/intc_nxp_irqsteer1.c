...
LOG_MODULE_REGISTER(nxp_irqstr1);

/* used for driver binding */
#define DT_DRV_COMPAT nxp_irqsteer1_intc
...
/* utility macros */
#define DISPATCHER_INST_INDEX(disp) \
	((int32_t)(((const struct irqsteer_config *)disp->dev->config)->inst_index))
...
static int to_zephyr_irq(uint32_t inst_index, uint32_t irq,
			 struct irqsteer_dispatcher *dispatcher)
{
	...
	for (i = dispatcher->master_index - 1; i >= 0; i--) {
		idx -= IRQSTEER_GetMasterIrqCount((int32_t)inst_index, i);
	}
...
static int to_system_irq(uint32_t inst_index, int irq, int master_index)
{
	int i;

	for (i = master_index - 1; i >= 0; i--) {
		irq += IRQSTEER_GetMasterIrqCount((int32_t)inst_index, i);
	}
...
static int from_zephyr_irq(uint32_t inst_index, uint32_t irq, uint32_t master_index)
{
	int i, idx;
					 ...
					 bool enable)
{
	if (enable) {
		...
		if (disp) {
			IRQSTEER_EnableMasterInterrupt(DISPATCHER_INST_INDEX(disp),
						       DISPATCHER_MASTER_INDEX(disp));
		}
	} else {
		if (disp) {
			IRQSTEER_DisableMasterInterrupt(DISPATCHER_INST_INDEX(disp),
							DISPATCHER_MASTER_INDEX(disp));
		}
...
{
	int32_t inst_index = DISPATCHER_INST_INDEX(disp);

	if (enable) {
		IRQSTEER_EnableInterrupt(inst_index, system_irq);
	} else {
		IRQSTEER_DisableInterrupt(inst_index, system_irq);
	}
}
...
static void irqstr_request_irq_unlocked(struct irqsteer_dispatcher *disp,
					uint32_t zephyr_irq)
{
	uint32_t system_irq = from_zephyr_irq(DISPATCHER_INST_INDEX(disp),
...
static void irqsteer_isr_dispatcher(const void *data)
{
	...
	status = IRQSTEER_GetMasterInterruptsStatus((int32_t)(cfg->inst_index),
	...
	for (i = 0; status; i++) {
		...
		if (status & 1) {
			...
			system_irq = to_system_irq(cfg->inst_index, i,
						   dispatcher->master_index);

			/* convert system INTID to a Zephyr INTID */
			zephyr_irq = to_zephyr_irq(cfg->inst_index, system_irq, dispatcher);
...
static int irqsteer_init(const struct device *dev)
{
	...
	IRQSTEER_Init(DT_REG_ADDR(DT_NODELABEL(irqsteer)));
...
static struct irqsteer_config irqsteer_config = {
	.inst_index = DT_REG_ADDR(DT_NODELABEL(irqsteer)),
