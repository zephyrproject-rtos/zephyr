...
static void cavs_ictl_isr(void *arg)
{
	...
	struct cavs_ictl_runtime *context = port->fixed->driver_data;

	const struct cavs_ictl_config *config = port->fixed->config_info;
...
{
	struct cavs_ictl_runtime *context = dev->fixed->driver_data;
