...
struct stm32_exti_data {
	/* per-line callbacks */
	struct __exti_cb cb_list[NUM_EXTI_LINES];
};
...
struct stm32_exti_line_range {
	...
	const struct __exti_cb *cb_list;
};
...
uint32_t stm32_exti_is_pending(uint32_t linenum)
{
	...
	if (linenum < 32U) {
		ret = EXTI_IS_ACTIVE_FLAG_0_31(ll_exti_line);
#ifdef CONFIG_EXTI_STM32_HAS_64_LINES
	} else if (linenum < 64U) {
		ret = EXTI_IS_ACTIVE_FLAG_32_63(ll_exti_line);
#endif /* CONFIG_EXTI_STM32_HAS_64_LINES */
#ifdef CONFIG_EXTI_STM32_HAS_96_LINES
	} else if (linenum < 96U) {
		ret = EXTI_IS_ACTIVE_FLAG_64_95(ll_exti_line);
...
static void stm32_exti_isr(const void *exti_range)
{
	...
	for (uint8_t linenum = range->start;
		...
		if (stm32_exti_is_pending(linenum) != 0) {
			...
			if (!data->cb_list[linenum].cb) {
				continue;
			}

			data->cb_list[linenum].cb(linenum, data->cb_list[linenum].data);
		}
	}
}
...
int stm32_exti_set_irq_callback(uint32_t linenum, stm32_exti_irq_cb_t cb, void *user)
{
	...
	if ((data->cb_list[linenum].cb == cb) && (data->cb_list[linenum].data == user)) {
		return 0;
	}

	/* if callback already exists/maybe-running return busy */
	if (data->cb_list[linenum].cb != NULL) {
		return -EBUSY;
	}

	data->cb_list[linenum].cb = cb;
	data->cb_list[linenum].data = user;
...
void stm32_exti_remove_irq_callback(uint32_t linenum)
{
	...
	data->cb_list[linenum].cb = NULL;
	data->cb_list[linenum].data = NULL;
}
...
static int stm32_exti_init(const struct device *dev)
{
	...
	DT_FOREACH_PROP_ELEM(EXTI_NODE, interrupt_names,
	...
	for (size_t i = 0; i < ARRAY_SIZE(exti_lines_range); i++) {
		...
		exti_lines_range[i].cb_list = exti_data.cb_list + idx;
	}
