			      ...
			      unsigned int prio, uint32_t flags)
{
	...
	uint32_t idx = GIC_IS_ESPI(intid) ? \
							  ((intid - GIC_ESPI_START) / GIC_NUM_INTR_PER_REG) : \
	...
	if (!GIC_IS_SGI(intid)) {
		idx = GIC_IS_ESPI(intid) ? \
						 ((intid - GIC_ESPI_START) / GIC_NUM_CFG_PER_REG) : \
						 (intid / GIC_NUM_CFG_PER_REG);
		shift = (intid & (GIC_NUM_CFG_PER_REG - 1)) * 2;

		val = GIC_IS_ESPI(intid) ? sys_read32(ICFGRnE(base, idx)) : sys_read32(ICFGR(base, idx));
...
bool arm_gic_irq_is_enabled(unsigned int intid)
{
	...
	val = GIC_IS_ESPI(intid) ? \
					  sys_read32(ISENABLERnE(GET_DIST_BASE(intid), idx)) : \
...
bool arm_gic_irq_is_pending(unsigned int intid)
{
	...
	val = GIC_IS_ESPI(intid) ? \
					  sys_read32(ISPENDRnE(GET_DIST_BASE(intid), idx)) : \
