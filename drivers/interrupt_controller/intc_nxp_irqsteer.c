...
static void irqsteer_hw_init(const struct irqsteer_config *cfg)
{
	...
	sys_write32(base + CHAN_MINTDIS(reg_num), 0xFFFFFFFF);

	/* Clear all channel masks */
	for (; reg_index < reg_num; reg_index++) {
		sys_write32(base + CHAN_MASK(reg_index, reg_num), 0);
	}
}
...
static void irqsteer_enable_interrupt(const struct irqsteer_config *cfg, uint32_t irq)
{
	...
	sys_write32(base + CHAN_MASK(reg_index, reg_num), mask);
}
					     ...
					     uint32_t master_index)
{
	...
	sys_write32(base + CHAN_MINTDIS(reg_num), mintdis);
}
						      ...
						      uint32_t master_index)
{
	...
	for (i = 0; i < 2; i++) {
		...
		ch_status = sys_read32(base + CHAN_STATUS(reg_index, reg_num));
		ch_mask = sys_read32(base + CHAN_MASK(reg_index, reg_num));
