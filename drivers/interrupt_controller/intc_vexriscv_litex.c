...
static void vexriscv_litex_irq_handler(const void *device)
{
	...
	DT_FOREACH_STATUS_OKAY(litex_timer0, LITEX_IRQ_ADD);
#endif

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	DT_FOREACH_STATUS_OKAY(litex_uart0, LITEX_IRQ_ADD);
#endif

#ifdef CONFIG_ETH_LITEETH
	DT_FOREACH_STATUS_OKAY(litex_eth0, LITEX_IRQ_ADD);
#endif

#ifdef CONFIG_I2S_LITEX
	DT_FOREACH_STATUS_OKAY(litex_i2s, LITEX_IRQ_ADD);
#endif
