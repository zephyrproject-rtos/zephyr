...
/* Current physical core id, Aff0 of MPIDR (flat single-cluster A53). */
static inline unsigned int this_core(void)
{
	return (unsigned int)MPIDR_AFFLVL(GET_MPIDR(), 0);
}
...
{
	/*
	 * Order the normal-memory stores that motivated this IPI before
	 * the Device-memory store that raises it, or the target can take
	 * the interrupt without being able to observe them yet. Same
	 * barrier as gic_raise_sgi().
	 */
