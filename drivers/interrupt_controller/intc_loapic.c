...
#define LOAPIC_LVT_PENTIUM4 5U	  		/* LO APIC LVT - Pentium4 */
#define LOAPIC_LVT_P6 4U		 		/* LO APIC LVT - P6 */
#define LOAPIC_LVT_P5 3U		    	/* LO APIC LVT - P5 */
...
int z_irq_controller_isr_vector_get(void)
{
	 ...
	 * vectors
	 */
	for (unsigned int block = 7; likely(block > 0U); block--) {
		pReg = x86_read_loapic(LOAPIC_ISR + (block * 0x10U));
		if (pReg != 0U) {
			const unsigned int uvector = (block * 32U) + (find_msb_set(pReg) - 1U);
			return (int)uvector;
		}
