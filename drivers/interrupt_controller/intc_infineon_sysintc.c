...
#define DT_DRV_COMPAT infineon_sysintc

static void sysintc_mux_isr(void)
{
...
#define _SYSINTC_REGISTER_DISPATCHER(node_id)                                                      \
	IRQ_CONNECT(DT_IRQN(node_id), DT_IRQ(node_id, priority), sysintc_mux_isr, NULL, 0)
...
void z_soc_irq_priority_set(unsigned int irq, unsigned int prio, unsigned int flags)
{
	...
	/* System interrupts have no priority of their own; assert it is 0. */
	__ASSERT(prio == 0, "system interrupt priority must be 0");
}

static int sysintc_init(const struct device *dev)
{
	ARG_UNUSED(dev);

	SYSINTC_REGISTER_DISPATCHERS(DT_NODELABEL(cpuss_intc));

	return 0;
}

DEVICE_DT_INST_DEFINE(0, sysintc_init, NULL, NULL, NULL, PRE_KERNEL_1, CONFIG_INTC_INIT_PRIORITY,
		      NULL);

#define SYSINTC_MUX_IRQ_PARENT_ENTRY(node_id)                                                      \
	IRQ_PARENT_ENTRY_DEFINE(CONCAT(sysintc_aggregator_, DT_NODE_CHILD_IDX(node_id)), NULL,     \
				DT_IRQN(node_id), INTC_CHILD_ISR_TBL_OFFSET(node_id),              \
				DT_INTC_GET_AGGREGATOR_LEVEL(node_id));

DT_INST_FOREACH_CHILD_STATUS_OKAY(0, SYSINTC_MUX_IRQ_PARENT_ENTRY);
