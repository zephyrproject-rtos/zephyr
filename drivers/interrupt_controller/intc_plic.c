...
# define PLIC_EN_PER_HART        (PLIC_EN_PER_TARGET * \
				  CONFIG_PLIC_TARGET_INCR_OF_NEXT_HART)
# define PLIC_REG_PER_HART       (PLIC_REG_PER_TARGET * \
				  CONFIG_PLIC_TARGET_INCR_OF_NEXT_HART)
#else
...
int riscv_plic_get_irq(void)
{
	...
	int irq = save_irq[_current_cpu->id];
...
static void plic_irq_handler(const void *arg)
{
	volatile struct plic_regs_t *regs = (volatile struct plic_regs_t *)
		INT_TO_POINTER(PLIC_REG +
			       (_current_cpu->id * PLIC_REG_PER_HART));
