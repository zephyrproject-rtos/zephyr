...
static const struct device *const miwu_devs[] = {DT_INST_FOREACH_STATUS_OKAY(NPCX_MIWU_DEV)};
...
BUILD_ASSERT(offsetof(struct miwu_callback, io_cb.params) + sizeof(struct miwu_io_params) ==
	...
	SYS_SLIST_FOR_EACH_CONTAINER_SAFE(cb_list, cb, tmp, node) {
		...
		if (cb->io_cb.params.cb_type == NPCX_MIWU_CALLBACK_GPIO) {
			if (BIT(cb->io_cb.params.wui.bit) & mask) {
				...
				cb->io_cb.handler(npcx_get_gpio_dev(cb->io_cb.params.gpio_port),
...
#define NPCX_MIWU_INIT(inst)                                                                       \
	...
	struct intc_miwu_data miwu_data_##inst;                                                    \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, NPCX_MIWU_INIT_FUNC(inst), NULL, &miwu_data_##inst,            \
