 ...
 * - No support for interrupt priority filtering.
...
{
	const struct xlnx_intc_config *cfg = dev->config;
...
{
	const struct xlnx_intc_config *cfg = dev->config;
	struct xlnx_intc_data *data = dev->data;
 ...
 */
static inline void intc_irq_acknowledge(const struct device *dev, uint32_t mask)
{
	const struct xlnx_intc_config *cfg = dev->config;

	intc_write(dev, mask & intc_valid_mask(cfg), XIN_IAR_OFFSET);
}
 ...
 */
static inline unsigned int xlnx_intc_intr_get_state(const struct device *dev)
{
...
static void xlnx_intc_dispatch_child_isrs(const struct device *dev, uint32_t irq_mask)
{
	...
	while (irq_mask != 0U) {
		...
		if (entry->isr == z_irq_spurious) {
			LOG_WRN("No handler for INTC line %u (table %u), acking", bitpos,
				table_idx);
...
#define XLNX_INTC_DEVICE_INIT(inst)                                                                \
	...
	BUILD_ASSERT(!DT_INST_PROP(inst, xlnx_is_fast), "xlnx,is-fast is not supported");          \
	BUILD_ASSERT(DT_INST_PROP(inst, xlnx_num_intr_inputs) >= 1 &&                              \
			     DT_INST_PROP(inst, xlnx_num_intr_inputs) <= 32,                       \
		     "xlnx,num-intr-inputs must be 1..32");                                        \
	BUILD_ASSERT(DT_INST_INTC_GET_AGGREGATOR_LEVEL(inst) == 2,                                 \
		     "Only second-level interrupt aggregation is supported");                      \
                                                                                                   \
	...
	static const struct xlnx_intc_config xlnx_intc_config_##inst = {                           \
		...
		.config_func = xlnx_intc_config_irq_##inst,                                        \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, xlnx_intc_initialize, NULL, &xlnx_intc_data_##inst,            \
			      &xlnx_intc_config_##inst, PRE_KERNEL_1, CONFIG_INTC_INIT_PRIORITY,   \
			      &xlnx_intc_apis);
