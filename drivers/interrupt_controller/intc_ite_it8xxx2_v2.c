...
#define IT8XXX2_INTC_ISR(g)   ECREG(IT8XXX2_INTC_BASE_SHIFT(g) + ((g) < 4 ? 0x0 : 0x4))
/* Interrupt enable register */
#define IT8XXX2_INTC_IER(g)   ECREG(IT8XXX2_INTC_BASE_SHIFT(g) + ((g) < 4 ? 0x1 : 0x5))
/* Interrupt edge/level triggered mode register */
#define IT8XXX2_INTC_IELMR(g) ECREG(IT8XXX2_INTC_BASE_SHIFT(g) + ((g) < 4 ? 0x2 : 0x6))
/* Interrupt polarity register */
#define IT8XXX2_INTC_IPOLR(g) ECREG(IT8XXX2_INTC_BASE_SHIFT(g) + ((g) < 4 ? 0x3 : 0x7))
...
uint8_t __soc_ram_code get_irq(void *arg)
{
	...
	if (intc_irq == 0) {
		...
		for (int i = (IT8XXX2_INTC_GROUP_CNT - 1); i >= 0; i--) {
			int_pending = (IT8XXX2_INTC_ISR(i) & IT8XXX2_INTC_IER(i));
			if (int_pending != 0) {
