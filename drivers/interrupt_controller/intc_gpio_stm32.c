...
/**
 * @brief Translate EXTI line to line number
 *
 * @param line LL_EXTI_LINE_n, has to be a power of 2
 *
 * @returns EXTI line number (n) for LL_EXTI_LINE_n define
 */
static inline gpio_pin_t ll_exti_line_to_linenum(stm32_gpio_irq_line_t line)
{
	const gpio_pin_t line_num = LOG2CEIL(line);

	__ASSERT_NO_MSG(IS_POWER_OF_TWO(line) && line_num < NUM_EXTI_LINES);
