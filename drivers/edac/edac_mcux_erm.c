...
#if defined(CONFIG_EDAC_ERROR_INJECT) && CONFIG_EDAC_ERROR_INJECT
#include <fsl_eim.h>
...
static int inject_error_trigger(const struct device *dev)
{
	...
	if (data->eim_channel != data->erm_channel) {
		LOG_WRN("EIM inject error on channel %x but ERM was listening on channel %x set "
			"ERM channel be same with EIM channel",
			data->eim_channel, data->erm_channel);
		data->erm_channel = data->eim_channel;
	}
	enable_ecc(eim_channel_data->ecc_enable);
	EIM_InjectDataBitError(config->eim_base, data->eim_channel, inject_data);
	EIM_EnableErrorInjectionChannels(config->eim_base, eim_channel_data->inject_enable);
...
static void edac_nxp_isr(const struct device *dev)
{
	...
	struct edac_nxp_callback_data cb_data = {
		.corr_err_count = ERM_GetErrorCount(config->erm_base, data->erm_channel),
		.err_syndrome = ERM_GetSyndrome(config->erm_base, data->erm_channel),
	...
	if (kERM_SingleBitCorrectionIntFlag == (status & kERM_SingleBitCorrectionIntFlag)) {
		LOG_ERR("ERM channel %d correctable ECC error detected, address 0x%x, syndrome "
		...
		LOG_ERR("ERM channel %d uncorrectable ECC error detected, address 0x%x",
