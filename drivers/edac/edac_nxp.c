...
#include <zephyr/drivers/edac/edac_nxp.h>
#include <fsl_erm.h>
...
/* NOTE This macro is used to avoid checkpatch error: SPACING: need consistent spacing around '*'
 * (ctx:WxV)  */
#define EIM_CONFIG                                                                                 \
	EIM_Type *eim_base;                                                                        \
...
#define EDAC_NXP_ERM_CHANNELS(n)                                                                   \
	static const int edac_nxp_##n##_channels[] = DT_PROP(DT_INST_PHANDLE(n, erm), channels);
...
#define EDAC_NXP_IRQ_CONFIG(n)                                                                     \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_IRQ_BY_IDX(DT_INST_PHANDLE(n, erm), 0, irq),                        \
			    DT_IRQ_BY_IDX(DT_INST_PHANDLE(n, erm), 0, priority), edac_nxp_isr,     \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		irq_enable(DT_IRQ_BY_IDX(DT_INST_PHANDLE(n, erm), 0, irq));                        \
		IRQ_CONNECT(DT_IRQ_BY_IDX(DT_INST_PHANDLE(n, erm), 1, irq),                        \
			    DT_IRQ_BY_IDX(DT_INST_PHANDLE(n, erm), 1, priority), edac_nxp_isr,     \
			    DEVICE_DT_INST_GET(n), 0);                                             \
...
#define EDAC_NXP_DEVICE_CONFIG_INIT(n)                                                             \
	...
	static const struct edac_nxp_config edac_nxp_##n##_config = {                              \
		...
		EDAC_NXP_EIM_CONFIG_FIELDS(n).erm_channels = edac_nxp_##n##_channels,              \
...
#define EDAC_NXP_DEVICE_INIT(n)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(n, &edac_nxp_init, NULL, &edac_nxp_##n##_data,                       \
			      &edac_nxp_##n##_config, POST_KERNEL,                                 \
