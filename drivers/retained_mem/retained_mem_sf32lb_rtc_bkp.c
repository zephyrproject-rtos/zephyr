...
struct retained_mem_sf32lb_data {
	struct k_mutex lock;
};
#endif
...
static inline void retained_mem_sf32lb_lock_take(const struct device *dev)
{
#ifdef CONFIG_RETAINED_MEM_MUTEXES
	struct retained_mem_sf32lb_data *data = dev->data;
...
static inline void retained_mem_sf32lb_lock_release(const struct device *dev)
{
#ifdef CONFIG_RETAINED_MEM_MUTEXES
	struct retained_mem_sf32lb_data *data = dev->data;
				    ...
				    size_t len)
{
	...
	retained_mem_sf32lb_lock_take(dev);
	memcpy(buf, (void *)(cfg->base + cfg->sub_base + offset), len);
	retained_mem_sf32lb_lock_release(dev);
				     ...
				     size_t len)
{
	...
	retained_mem_sf32lb_lock_take(dev);
	memcpy((void *)(cfg->base + cfg->sub_base + offset), buf, len);
	retained_mem_sf32lb_lock_release(dev);

	return 0;
}

static int retained_mem_sf32lb_clear(const struct device *dev)
{
	const struct retained_mem_sf32lb_config *cfg = dev->config;

	retained_mem_sf32lb_lock_take(dev);
	memset((void *)(cfg->base + cfg->sub_base), 0, cfg->size);
	retained_mem_sf32lb_lock_release(dev);
...
static DEVICE_API(retained_mem, retained_mem_sf32lb_api) = {
	...
	.clear = retained_mem_sf32lb_clear,
};
...
#ifdef CONFIG_RETAINED_MEM_MUTEXES
static struct retained_mem_sf32lb_data retained_mem_sf32lb_data_0;
#endif

DEVICE_DT_INST_DEFINE(0, retained_mem_sf32lb_init, NULL,
		      COND_CODE_1(CONFIG_RETAINED_MEM_MUTEXES,
				   (&retained_mem_sf32lb_data_0),
				   (NULL)),
		      &retained_mem_sf32lb_cfg_0, POST_KERNEL,
		      CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &retained_mem_sf32lb_api);
