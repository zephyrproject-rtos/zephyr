...
#define dai_get_ssp_link(dai) dai->ssp_link
#define dai_get_mn(dai) dai->ssp_link->ssp_plat_data.mn_inst
#define dai_get_ftable(dai) dai->ssp_link->ssp_plat_data.ftable
#define dai_get_fsources(dai) dai->ssp_link->ssp_plat_data.fsources
#define dai_mn_base(dai) dai->ssp_link->ssp_plat_data.mn_inst->base
#define dai_base(dai) dai->ssp_link->ssp_plat_data.base
#define dai_ip_base(dai) dai->ssp_link->ssp_plat_data.ip_base
#define dai_shim_base(dai) dai->ssp_link->ssp_plat_data.shim_base
#define dai_hdamlssp_base(dai) dai->ssp_link->ssp_plat_data.hdamlssp_base
#define dai_i2svss_base(dai) dai->ssp_link->ssp_plat_data.i2svss_base
...
static struct dai_intel_ssp_mn ssp_mn_divider = {
	.base = DT_REG_ADDR_BY_IDX(DT_NODELABEL(ssplink0), 1),
};

#define INTEL_SSP_LINK_INST_DEFINE(node_id) {			\
	.link_index = DT_PROP(node_id, link),					\
	.acquire_count = 0,									\
	.is_power_en = 0,									\
...
static struct intel_ssp_link ssp_link_table[] = {
	DT_FOREACH_STATUS_OKAY(intel_ssp_link, INTEL_SSP_LINK_INST_DEFINE)
};

static uint32_t ssp_get_link_count(void)
{
	return ARRAY_SIZE(ssp_link_table);
}


static struct intel_ssp_link *ssp_get_link(uint32_t link_index)
{
	uint32_t link_count = ssp_get_link_count();
	uint32_t i;

	for (i = 0; i < link_count; i++) {
		if (ssp_link_table[i].link_index == link_index) {
			return &ssp_link_table[i];
		}
	}
...
static void ssp_acquire_link(struct intel_ssp_link *link)
{
	link->acquire_count++;
}

static void ssp_release_link(struct intel_ssp_link *link)
{
...
static bool ssp_link_is_acquired(struct intel_ssp_link *link)
{
...
static void dai_ssp_mn_release_bclk(struct dai_intel_ssp *dp, uint32_t link_index)
{
	struct dai_intel_ssp_mn *mp = dai_get_mn(dp);
	k_spinlock_key_t key;
	bool mn_in_use;
	...
	key = k_spin_lock(&mp->lock);
	mp->bclk_sources[link_index] = MN_BCLK_SOURCE_NONE;
...
static void dai_ssp_mn_reset_bclk_divider(struct dai_intel_ssp *dp, uint32_t link_index)
{
	struct dai_intel_ssp_mn *mp = dai_get_mn(dp);
	k_spinlock_key_t key;
	...
	sys_write32(1, dai_mn_base(dp) + MN_MDIV_M_VAL(link_index));
	sys_write32(1, dai_mn_base(dp) + MN_MDIV_N_VAL(link_index));
						...
						uint32_t link_index)
{
	...
	shim_reg = sys_read32(dai_shim_base(dp) + SHIM_CLKCTL) |
		(link_index < CONFIG_DAI_INTEL_SSP_NUM_BASE ?
			SHIM_CLKCTL_I2SFDCGB(link_index) :
			SHIM_CLKCTL_I2SEFDCGB(link_index -
					      CONFIG_DAI_INTEL_SSP_NUM_BASE));

	sys_write32(shim_reg, dai_shim_base(dp) + SHIM_CLKCTL);

	LOG_INF("link_index %d CLKCTL %08x", link_index, shim_reg);
#endif
}
...
static void dai_ssp_pm_runtime_en_ssp_power(struct dai_intel_ssp *dp, uint32_t link_index)
{
#if CONFIG_DAI_SSP_HAS_POWER_CONTROL
	int ret;

	LOG_INF("SSP link %d", dp->link_index);
#if CONFIG_SOC_INTEL_ACE15_MTPM || CONFIG_SOC_SERIES_INTEL_ADSP_CAVS
	sys_write32(sys_read32(dai_ip_base(dp) + I2SLCTL_OFFSET) | I2SLCTL_SPA(link_index),
		    dai_ip_base(dp) + I2SLCTL_OFFSET);

	/* Check if powered on. */
	ret = dai_ssp_poll_for_register_delay(dai_ip_base(dp) + I2SLCTL_OFFSET,
					      I2SLCTL_CPA(link_index), I2SLCTL_CPA(link_index),
...
#endif
	if (ret) {
		LOG_WRN("SSP link %d: timeout", dp->link_index);
	}
#else
	ARG_UNUSED(dp);
	ARG_UNUSED(link_index);
#endif /* CONFIG_DAI_SSP_HAS_POWER_CONTROL */
}

static void dai_ssp_pm_runtime_dis_ssp_power(struct dai_intel_ssp *dp, uint32_t link_index)
{
#if CONFIG_DAI_SSP_HAS_POWER_CONTROL
	int ret;

	LOG_INF("SSP link %d", link_index);
#if CONFIG_SOC_INTEL_ACE15_MTPM || CONFIG_SOC_SERIES_INTEL_ADSP_CAVS
	sys_write32(sys_read32(dai_ip_base(dp) + I2SLCTL_OFFSET) & (~I2SLCTL_SPA(link_index)),
		    dai_ip_base(dp) + I2SLCTL_OFFSET);
	...
	ret = dai_ssp_poll_for_register_delay(dai_ip_base(dp) + I2SLCTL_OFFSET,
					      I2SLCTL_CPA(link_index), 0,
					      DAI_INTEL_SSP_MAX_SEND_TIME_PER_SAMPLE);
#elif CONFIG_SOC_INTEL_ACE20_LNL
	sys_write32(sys_read32(dai_hdamlssp_base(dp) + I2SLCTL_OFFSET) & (~I2SLCTL_SPA(link_index)),
			dai_hdamlssp_base(dp) + I2SLCTL_OFFSET);
...
#endif
	if (ret) {
		LOG_WRN("SSP link %d: timeout", link_index);
	}
#else
	ARG_UNUSED(dp);
	ARG_UNUSED(link_index);
...
static void dai_ssp_program_channel_map(struct dai_intel_ssp *dp,
		const struct dai_config *cfg, uint32_t link_index)
{
#ifdef CONFIG_SOC_INTEL_ACE20_LNL
	uint16_t pcmsycm = cfg->link_config;
	 /* Set upper slot number from configuration */
	pcmsycm = pcmsycm | (dp->ssp_link->params.tdm_slots - 1) << 4;

	if (DAI_INTEL_SSP_IS_BIT_SET(cfg->link_config, 15)) {
		uint32_t reg_add = dai_ip_base(dp) + 0x1000 * link_index + PCMS0CM_OFFSET;
		/* Program HDA output stream parameters */
		sys_write16((pcmsycm & 0xffff), reg_add);
	} else {
		uint32_t reg_add = dai_ip_base(dp) + 0x1000 * link_index + PCMS1CM_OFFSET;
...
static void ssp_empty_rx_fifo_on_stop(struct dai_intel_ssp *dp)
{
	...
	uint64_t sample_ticks = ssp_link->params.fsync_rate ? 1000000 /
		ssp_link->params.fsync_rate : 0;
	uint32_t retry = DAI_INTEL_SSP_RX_FLUSH_RETRY_MAX;
	uint32_t entries[2];
	uint32_t i, sssr;
...
static int dai_ssp_mclk_prepare_enable(struct dai_intel_ssp *dp)
{
	...
	if (ssp_link->ssp_plat_data.clk_active & SSP_CLK_MCLK_ACTIVE) {
		return 0;
	}

	/* MCLK config */
	ret = dai_ssp_mn_set_mclk(dp, ssp_link->params.mclk_id, ssp_link->params.mclk_rate);
	if (ret < 0) {
		LOG_ERR("invalid mclk_rate = %d for mclk_id = %d", ssp_link->params.mclk_rate,
			ssp_link->params.mclk_id);
	} else {
		ssp_link->ssp_plat_data.clk_active |= SSP_CLK_MCLK_ACTIVE;
	}
...
static void dai_ssp_mclk_disable_unprepare(struct dai_intel_ssp *dp)
{
	...
	dai_ssp_mn_release_mclk(dp, ssp_link->params.mclk_id);
...
static int dai_ssp_bclk_prepare_enable(struct dai_intel_ssp *dp)
{
	...
	if (ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ACTIVE) {
		return 0;
	}
...
#if CONFIG_INTEL_MN
	/* BCLK config */
	ret = dai_ssp_mn_set_bclk(dp, dp->link_index, ssp_link->params.bclk_rate,
				  &mdiv, &need_ecs);
	if (ret < 0) {
		LOG_ERR("invalid bclk_rate = %d for link_index = %d",
			ssp_link->params.bclk_rate, dp->link_index);
		goto out;
	}
#else
	if (ft[DAI_INTEL_SSP_DEFAULT_IDX].freq % ssp_link->params.bclk_rate != 0) {
		LOG_ERR("invalid bclk_rate = %d for link_index = %d",
			ssp_link->params.bclk_rate, dp->link_index);
		ret = -EINVAL;
		goto out;
	...
	mdiv = ft[DAI_INTEL_SSP_DEFAULT_IDX].freq / ssp_link->params.bclk_rate;
#endif
...
out:
	if (!ret) {
		ssp_link->ssp_plat_data.clk_active |= SSP_CLK_BCLK_ACTIVE;
	}
...
#if CONFIG_INTEL_MN
	dai_ssp_mn_release_bclk(dp, dp->link_index);
#endif
	ssp_link->ssp_plat_data.clk_active &= ~SSP_CLK_BCLK_ACTIVE;
}
...
static void dai_ssp_log_ssp_data(struct dai_intel_ssp *dp)
{
	...
	LOG_INF("plat_data base: %u", dp->ssp_link->ssp_plat_data.base);
	LOG_INF("plat_data irq: %u", dp->ssp_link->ssp_plat_data.irq);
	LOG_INF("plat_data fifo playback offset: %u",
		dp->ssp_link->ssp_plat_data.fifo[DAI_DIR_PLAYBACK].offset);
	LOG_INF("plat_data fifo playback handshake: %u",
		dp->ssp_link->ssp_plat_data.fifo[DAI_DIR_PLAYBACK].handshake);
	LOG_INF("plat_data fifo capture offset: %u",
		dp->ssp_link->ssp_plat_data.fifo[DAI_DIR_CAPTURE].offset);
				   ...
				   const void *bespoke_cfg)
{
	...
	ssp->config = *config;
	memcpy(&ssp_link->params, bespoke_cfg, sizeof(struct dai_intel_ipc3_ssp_params));
	...
	sstsa = SSTSA_SSTSA(ssp_link->params.tx_slots);

	/* ssrsa dynamic setting is RTSA, default 2 slots */
	ssrsa = SSRSA_SSRSA(ssp_link->params.rx_slots);
	...
	if (ssp_link->params.clks_control & DAI_INTEL_IPC3_SSP_CLKCTRL_BCLK_IDLE_HIGH) {
	...
	sscr1 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_TINTE) ?
		SSCR1_TINTE : 0;

	/* Peripheral Trailing Byte Interrupts Disable/Enable */
	sscr1 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_PINTE) ?
		SSCR1_PINTE : 0;
	 ...
	 */
	sscr1 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_LBM) ?
		SSCR1_LBM : 0;

	if (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_LBM) {
	 ...
	 * in SSPSP.SCMODE[1:0]
	 */
	sscr2 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_SMTATF) ?
		SSCR2_SMTATF : 0;

	/* Receive data are sampled at the same/opposite clock edge specified
	 * in SSPSP.SCMODE[1:0]
	 */
	sscr2 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_MMRATF) ?
		SSCR2_MMRATF : 0;

	/* Enable/disable the fix for PSP consumer mode TXD wait for frame
	 * de-assertion before starting the second channel
	 */
	sscr2 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_PSPSTWFDFD) ?
		SSCR2_PSPSTWFDFD : 0;

	/* Enable/disable the fix for PSP provider mode FSRT with dummy stop &
	 * frame end padding capability
	 */
	sscr2 |= (ssp_link->params.quirks & DAI_INTEL_IPC3_SSP_QUIRK_PSPSRWFDFD) ?
		SSCR2_PSPSRWFDFD : 0;

	if (!ssp_link->params.mclk_rate ||
	    ssp_link->params.mclk_rate > ft[DAI_INTEL_SSP_MAX_FREQ_INDEX].freq) {
		LOG_ERR("invalid MCLK = %d Hz (valid < %d)", ssp_link->params.mclk_rate,
			ft[DAI_INTEL_SSP_MAX_FREQ_INDEX].freq);
		ret = -EINVAL;
		goto out;
	}

	if (!ssp_link->params.bclk_rate ||
		ssp_link->params.bclk_rate > ssp_link->params.mclk_rate) {
		LOG_ERR("BCLK %d Hz = 0 or > MCLK %d Hz", ssp_link->params.bclk_rate,
			ssp_link->params.mclk_rate);
		ret = -EINVAL;
		goto out;
	}

	/* calc frame width based on BCLK and rate - must be divisible */
	if (ssp_link->params.bclk_rate % ssp_link->params.fsync_rate) {
		LOG_ERR("BCLK %d is not divisible by rate %d", ssp_link->params.bclk_rate,
			ssp_link->params.fsync_rate);
		ret = -EINVAL;
		goto out;
	}

	/* must be enough BCLKs for data */
	bdiv = ssp_link->params.bclk_rate / ssp_link->params.fsync_rate;
	if (bdiv < ssp_link->params.tdm_slot_width * ssp_link->params.tdm_slots) {
		LOG_ERR("not enough BCLKs need %d",
			ssp_link->params.tdm_slot_width * ssp_link->params.tdm_slots);
		ret = -EINVAL;
		goto out;
	}

	/* tdm_slot_width must be <= 38 for SSP */
	if (ssp_link->params.tdm_slot_width > 38) {
		LOG_ERR("tdm_slot_width %d > 38", ssp_link->params.tdm_slot_width);
		ret = -EINVAL;
		goto out;
	}

	bdiv_min = ssp_link->params.tdm_slots *
		   (ssp_link->params.tdm_per_slot_padding_flag ?
		    ssp_link->params.tdm_slot_width : ssp_link->params.sample_valid_bits);
	if (bdiv < bdiv_min) {
	...
	case DAI_INTEL_IPC3_SSP_FMT_I2S:
		...
		sscr0 |= SSCR0_FRDC(ssp_link->params.tdm_slots);
	...
	case DAI_INTEL_IPC3_SSP_FMT_DSP_B:
		...
		sscr0 |= SSCR0_MOD | SSCR0_FRDC(ssp_link->params.tdm_slots);

		/* set asserted frame length */
		frame_len = 1; /* default */

		if (cfs && ssp_link->params.frame_pulse_width > 0 &&
		    ssp_link->params.frame_pulse_width <=
		    DAI_INTEL_IPC3_SSP_FRAME_PULSE_WIDTH_MAX) {
			frame_len = ssp_link->params.frame_pulse_width;
		}

		/* frame_pulse_width must less or equal 38 */
		if (ssp_link->params.frame_pulse_width >
			DAI_INTEL_IPC3_SSP_FRAME_PULSE_WIDTH_MAX) {
		...
		active_tx_slots = POPCOUNT(ssp_link->params.tx_slots);
		active_rx_slots = POPCOUNT(ssp_link->params.rx_slots);
		 ...
		 */
		if (ssp_link->params.tdm_per_slot_padding_flag) {
			frame_end_padding = bdiv - ssp_link->params.tdm_slots *
				ssp_link->params.tdm_slot_width;

			slot_end_padding = ssp_link->params.tdm_slot_width -
				ssp_link->params.sample_valid_bits;
	...
	data_size = ssp_link->params.sample_valid_bits;
	...
	/* setting TFT and RFT */
	switch (ssp_link->params.sample_valid_bits) {
	case 16:
	...
	case 32:
		...
		break;
	default:
		LOG_ERR("sample_valid_bits %d", ssp_link->params.sample_valid_bits);
		ret = -EINVAL;
		goto out;
	}
...
clk:
	...
	case DAI_INTEL_IPC3_SSP_CONFIG_FLAGS_HW_PARAMS:
		if (ssp_link->params.clks_control & DAI_INTEL_IPC3_SSP_CLKCTRL_MCLK_ES) {
			ret = dai_ssp_mclk_prepare_enable(dp);
			if (ret < 0) {
				goto out;
			}

			ssp_link->ssp_plat_data.clk_active |= SSP_CLK_MCLK_ES_REQ;

			LOG_INF("hw_params stage: enabled MCLK clocks for SSP link %d...",
					dp->link_index);
		}

		if (ssp_link->params.clks_control & DAI_INTEL_IPC3_SSP_CLKCTRL_BCLK_ES) {
			...
			ssp_link->ssp_plat_data.clk_active |= SSP_CLK_BCLK_ES_REQ;

			if (enable_sse) {
				/* enable port */
				dai_ssp_update_bits(dp, SSCR0, SSCR0_SSE, SSCR0_SSE);

				LOG_INF("SSE set for SSP%d", dp->dai_index);
			}

			LOG_INF("hw_params stage: enabled BCLK clocks for SSP link %d...",
					dp->link_index);
		}
		break;
	case DAI_INTEL_IPC3_SSP_CONFIG_FLAGS_HW_FREE:
		...
		if (dp->state[DAI_DIR_CAPTURE] != DAI_STATE_PRE_RUNNING ||
		    dp->state[DAI_DIR_PLAYBACK] != DAI_STATE_PRE_RUNNING) {
			LOG_INF("hw_free stage: ignore since link SSP%d still in use",
					dp->link_index);
			break;
		}

		if (ssp_link->params.clks_control & DAI_INTEL_IPC3_SSP_CLKCTRL_BCLK_ES) {
			LOG_INF("hw_free stage: releasing BCLK clocks for SSP link %d...",
					dp->link_index);
			if (ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ACTIVE) {
				dai_ssp_update_bits(dp, SSCR0, SSCR0_SSE, 0);
				LOG_INF("SSE clear for link SSP%d", dp->link_index);
			}
			dai_ssp_bclk_disable_unprepare(dp);
			ssp_link->ssp_plat_data.clk_active &= ~SSP_CLK_BCLK_ES_REQ;
		}
		if (ssp_link->params.clks_control & DAI_INTEL_IPC3_SSP_CLKCTRL_MCLK_ES) {
			LOG_INF("hw_free stage: releasing MCLK clocks for link SSP%d...",
					dp->link_index);
				   ...
				   const struct dai_intel_ipc4_ssp_config *regs)
{
	...
	LOG_ERR("SSP%d configuration:", dp->dai_index);
	...
	dp->ssp_link->params.sample_valid_bits = SSCR0_DSIZE_GET(ssc0);
	if (ssc0 & SSCR0_EDSS) {
		dp->ssp_link->params.sample_valid_bits += 16;
	}

	dp->ssp_link->params.tdm_slots = SSCR0_FRDC_GET(ssc0);
	dp->ssp_link->params.tx_slots = SSTSA_GET(sstsa);
	dp->ssp_link->params.rx_slots = SSRSA_GET(ssrsa);
	dp->ssp_link->params.fsync_rate = cfg->rate;
...
static int dai_ssp_pre_start(struct dai_intel_ssp *dp)
{
	 ...
	 */
	if (!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_MCLK_ES_REQ)) {
		/* MCLK config */
		ret = dai_ssp_mclk_prepare_enable(dp);
		if (ret < 0) {
			return ret;
	...
	if (!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ES_REQ)) {
		ret = dai_ssp_bclk_prepare_enable(dp);
	}
...
static void dai_ssp_post_stop(struct dai_intel_ssp *dp)
{
	...
	if (dp->state[DAI_DIR_PLAYBACK] != DAI_STATE_RUNNING &&
		...
		if (!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ES_REQ)) {
			LOG_INF("releasing BCLK clocks for SSP%d...", dp->link_index);
			dai_ssp_bclk_disable_unprepare(dp);
		}
		if (!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_MCLK_ES_REQ)) {
			LOG_INF("releasing MCLK clocks for SSP%d...", dp->link_index);
			dai_ssp_mclk_disable_unprepare(dp);
		}
	}
}
...
static void dai_ssp_early_start(struct dai_intel_ssp *dp, int direction)
{
	...
	if (!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ES_REQ)) {
		/* enable port */
		LOG_INF("SSP%d: set SSE", dp->link_index);
		dai_ssp_update_bits(dp, SSCR0, SSCR0_SSE, SSCR0_SSE);
	}
...
static void dai_ssp_stop(struct dai_intel_ssp *dp, int direction)
{
	...
	if (dp->state[DAI_DIR_CAPTURE] == DAI_STATE_PRE_RUNNING &&
	    ...
	    COND_CODE_1(CONFIG_INTEL_ADSP_CAVS,
			(!(ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ES_REQ)), (true))) {

		if (!ssp_link_is_acquired(ssp_link)) {
			dai_ssp_update_bits(dp, SSCR0, SSCR0_SSE, 0);
			LOG_INF("%s SSE clear SSP%d", __func__, dp->link_index);
		}
	}
...
static int dai_ssp_config_get(const struct device *dev, struct dai_config *cfg, enum dai_dir dir)
{
	...
	params->rate = ssp_link->params.fsync_rate;

	if (dir == DAI_DIR_PLAYBACK) {
		params->channels = POPCOUNT(ssp_link->params.tx_slots);
	} else {
		params->channels = POPCOUNT(ssp_link->params.rx_slots);
	}

	params->word_size = ssp_link->params.sample_valid_bits;
			      ...
			      const void *bespoke_cfg)
{
	...
	dai_ssp_program_channel_map(dp, cfg, dp->link_index);
	return ret;
}
							   ...
							   enum dai_dir dir, int stream_id)
{
	...
	prop->fifo_address = ssp_link->ssp_plat_data.fifo[array_index].offset;
	prop->dma_hs_id = dp->ssp_link->ssp_plat_data.fifo[array_index].handshake;

	if (ssp_link->ssp_plat_data.clk_active & SSP_CLK_BCLK_ACTIVE) {
		prop->reg_init_delay = 0;
	} else {
		prop->reg_init_delay = ssp_link->params.bclk_delay;
	}
...
static int dai_ssp_probe(struct dai_intel_ssp *dp)
{
	...
	if (!ssp) {
		LOG_ERR("SSP%d: alloc failed", dp->link_index);
		return -ENOMEM;
	}
	...
	if (ssp_link->is_power_en) {
		return 0;
	}

#if CONFIG_INTEL_MN
	/* Reset M/N, power-gating functions need it */
	dai_ssp_mn_reset_bclk_divider(dp, dp->link_index);
#endif

	/* Enable SSP power */
	dai_ssp_pm_runtime_en_ssp_power(dp, dp->link_index);

	/* Disable dynamic clock gating before touching any register */
	dai_ssp_pm_runtime_dis_ssp_clk_gating(dp, dp->link_index);

	ssp_link->is_power_en = true;
...
static int dai_ssp_remove(struct dai_intel_ssp *dp)
{
	...
	if (ssp_link_is_acquired(dp->ssp_link)) {
		k_free(dai_get_drvdata(dp));
		dai_set_drvdata(dp, NULL);
		return 0;
	}

	dai_ssp_pm_runtime_en_ssp_clk_gating(dp, dp->link_index);

	dai_ssp_mclk_disable_unprepare(dp);
	dai_ssp_bclk_disable_unprepare(dp);

	/* Disable SSP power */
	dai_ssp_pm_runtime_dis_ssp_power(dp, dp->link_index);

	k_free(dai_get_drvdata(dp));
	dai_set_drvdata(dp, NULL);

	ssp_link->is_initialized = false;
	ssp_link->is_power_en = false;
...
static int dai_intel_ssp_init_device(const struct device *dev)
{
	...
	dp->ssp_link = ssp_get_link(dp->link_index);
...
#define DAI_INTEL_SSP_DEVICE_INIT(n)						\
	...
	static struct dai_intel_ssp dai_intel_ssp_data_##n = {	\
		.dai_index = DT_INST_REG_ADDR(n),			\
		.link_index = DT_INST_PROP(n, link),		\
	};	\
												\
