...
static int dai_uaol_process_dma_control_data(struct dai_intel_uaol_data *dp,
					     const void *data, size_t size)
{
	size_t i, hop;
	struct ipc4_uaol_tlv *tlv = (struct ipc4_uaol_tlv *)data;
				  ...
				  size_t size)
{
	...
	return dai_uaol_process_dma_control_data(dp, bespoke_cfg, size);
}

static DEVICE_API(dai, dai_intel_uaol_api_funcs) = {
