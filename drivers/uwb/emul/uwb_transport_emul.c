...
/**
 * @file
 * @brief Emulated backend for UWB transport layer (uwb_transport_* APIs)
 *
...
LOG_MODULE_REGISTER(uwb_transport_emul, CONFIG_UWB_LOG_LEVEL);

#define DT_DRV_COMPAT zephyr_uwb_emul
...
/* -----------------------------------------------------------------------
 * Emulated device thread – processes commands and produces responses
 * -----------------------------------------------------------------------
 */
...
static void handle_core_reset(int write_fd)
{
	...
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to CORE_RESET");
}
...
static void handle_core_get_capabilities(int write_fd)
{
	...
	build_rsp(rsp, &rsp_len, UCI_GID_CORE, UCI_MSG_CORE_GET_CAPS_INFO, payload,
		  sizeof(payload));
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to UCI_MSG_CORE_GET_CAPS_INFO");
}
...
static void handle_core_get_device_state(int write_fd)
{
	...
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to UCI_MSG_CORE_GET_CONFIG");
}
...
static void handle_core_get_device_info(int write_fd)
{
	...
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to CORE_GET_DEVICE_INFO");
}
...
static void handle_session_init(uint32_t session_id)
{
	if (g_session_ctx.initialized) {
		...
		k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
		LOG_DBG("emul: responded to SESSION_INIT");
	} else {
...
static void handle_range_start(int write_fd)
{
	...
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to RANGE_START");
}
...
static void handle_range_stop(int write_fd)
{
	...
	k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
	LOG_DBG("emul: responded to RANGE_STOP");
}

static void *emul_device_fn(void *arg)
{
	uint8_t cmd[256];

	LOG_DBG("emul: device thread started");

	while (atomic_get(&emu_running)) {
		...
		if (n <= 0) {
			if (!atomic_get(&emu_running)) {
				break;
			}
			LOG_ERR("emul: pipe read error %zd (errno %d)", n, errno);
			continue;
		}
		...
		if (payload_len > 0) {
			int pn = k_pipe_read(&pipe_host_to_dev, &cmd[UCI_HEADER_SIZE],
						 payload_len, K_FOREVER);
			LOG_HEXDUMP_INF(cmd, payload_len + UCI_HEADER_SIZE, "UCI packet");

			if (pn != (int)payload_len) {
				LOG_ERR("emul: short payload read");
				continue;
			}
		}
		...
		LOG_DBG("emul: rx MT=0x%x GID=0x%x OID=0x%x PL=%u", mt, gid, oid, payload_len);
		...
		if (gid == UCI_GID_CORE && oid == UCI_MSG_CORE_DEVICE_RESET) {
			...
			if (param_id == UWB_CORE_CONFIG_DEVICE_STATE) {
				...
				k_pipe_write(&pipe_dev_to_host, rsp, rsp_len, K_FOREVER);
				LOG_WRN("emul: unknown cmd GID=0x%x OID=0x%x", gid, oid);
			}
	...
	LOG_DBG("emul: device thread exiting");
	return NULL;
}
...
int uwb_transport_open(const struct device *dev)
{
	...
	atomic_set(&emu_running, 1);

	uwb_dev = k_thread_create(&uwb_dev_thread, (k_thread_stack_t *)&uwb_dev_stack,
				  K_THREAD_STACK_SIZEOF(uwb_dev_stack),
				  (k_thread_entry_t)&emul_device_fn, NULL, NULL, NULL,
				  K_PRIO_COOP(5), 0, K_NO_WAIT);
	if (NULL == uwb_dev) {
		atomic_set(&emu_running, 0);
		return -1;
	}

	LOG_INF("%s: Emulated UWB backend ready", __func__);
	return 0;
}
...
int uwb_transport_close(const struct device *dev)
{
	...
	LOG_INF("%s: Emulated UWB backend closed", __func__);
	return 0;
}
