...
#define DT_DRV_COMPAT zephyr_native_uwb_device
...
static void handle_core_reset(int write_fd)
{
	...
	size_t rsp_len;
	uint8_t payload = UCI_STATUS_CODE_OK;
...
static void handle_core_get_device_state(int write_fd)
{
	 ...
	 * phyVerMaj | phyVerMin | vendor_spec_len
	 */
	uint8_t payload[] = {0x00, 1, UWB_CORE_CONFIG_DEVICE_STATE, 1, UWB_DEVICE_STATE_READY};
...
static void handle_session_init(uint32_t session_id)
{
	if (g_session_ctx.initialized) {
		...
		uint8_t rsp[UCI_HEADER_SIZE + 1];
		size_t rsp_len;
		uint8_t payload = UCI_STATUS_CODE_REJECTED;
		...
		uint8_t payload[] = {UCI_STATUS_CODE_OK, 0x00, 0x00, 0x00, 0x01};
		uint8_t ntf_payload[] = {0x00, 0x00, 0x00, 0x01, UWB_SESSION_STATUS_INITIALIZED, 0};
...
static void handle_session_deinit(uint32_t session_handle)
{
	if (!g_session_ctx.initialized || (g_session_ctx.session_handle != session_handle)) {
		...
		uint8_t rsp[UCI_HEADER_SIZE + 1];
		size_t rsp_len;
		uint8_t payload = UCI_STATUS_CODE_ERROR_SESSION_NOT_EXIST;
		...
		size_t rsp_len = sizeof(rsp);
		uint8_t payload = UCI_STATUS_CODE_OK;
		uint8_t ntf_payload[] = {0x00, 0x00, 0x00, 0x01, UWB_SESSION_STATUS_DE_INITIALIZED,
					 0};
...
static void handle_session_get_state(uint32_t session_handle)
{
	if (!g_session_ctx.initialized || (g_session_ctx.session_handle != session_handle)) {
		...
		uint8_t rsp[UCI_HEADER_SIZE + sizeof(uint32_t) + 1 + 1];
		size_t rsp_len = sizeof(rsp);
		uint8_t payload[] = {UCI_STATUS_CODE_OK, UWB_SESSION_STATUS_INITIALIZED};
...
static void *sim_device_fn(void *arg)
{
	...
	while (atomic_get(&sim_running)) {
		...
		if (gid == UCI_GID_CORE && oid == UCI_MSG_CORE_DEVICE_RESET) {
			...
			if (param_id == UWB_CORE_CONFIG_DEVICE_STATE) {
				handle_core_get_device_state(0);
			} else {
				uint8_t rsp[UCI_HEADER_SIZE + 1];
				size_t rsp_len;
				uint8_t payload[] = {UCI_STATUS_CODE_INVALID_PARAM, 1, param_id, 0};
			...
			uint8_t rsp[UCI_HEADER_SIZE + 1];
			size_t rsp_len;
			uint8_t payload = UCI_STATUS_CODE_FAILED;
...
int uwb_transport_open(const struct device *dev)
{
...
int uwb_transport_close(const struct device *dev)
{
	atomic_set(&sim_running, 0);
...
int uwb_transport_uci_read(const struct device *dev, uint8_t *pBuffer, int bytes_to_read)
{
...
int uwb_transport_uci_write(const struct device *dev, uint8_t *pBuffer, uint16_t bytes_to_write)
{
...
static DEVICE_API(uwb_uci, drv) = {
	.open = uwb_transport_open,
	.close = uwb_transport_close,
	.send = uwb_transport_uci_write,
	.recv = uwb_transport_uci_read,
};

#define UCI_DEVICE_INIT(inst)                                                                      \
