...
/**
 * Add hardware filter rule for event triggered when receiving LIN header (slave only)
 *
 ...
 *
 * If the device is master, the header will always transmitted on the the bus, then following with
			     ...
			     k_timeout_t timeout)
{
	...
	if (data->common.config.mode != LIN_MODE_COMMANDER) {
		LOG_ERR("LIN send operation is only allowed in master mode");
		return -EPERM;
	}
 ...
 *
 * If the device is master, the header will always transmitted on the bus, then it will monitor the
 * bus to receive the response from responder.
 */
static int lin_skeleton_receive(const struct device *dev, struct lin_msg *msg, k_timeout_t timeout)
{
	...
	if (data->common.config.mode != LIN_MODE_COMMANDER) {
		LOG_ERR("LIN receive operation is only allowed in master mode");
		return -EPERM;
	}
...
/**
 * Send a LIN response only (slave mode only)
 *
				 ...
				 k_timeout_t timeout)
{
	...
	if (data->common.config.mode != LIN_MODE_RESPONDER) {
		LOG_ERR("LIN response operation is only allowed in slave mode");
		return -EPERM;
	}
...
static int lin_skeleton_read(const struct device *dev, struct lin_msg *msg, k_timeout_t timeout)
{
	...
	if (data->common.config.mode != LIN_MODE_RESPONDER) {
		LOG_ERR("LIN read operation is only allowed in slave mode");
		return -EPERM;
	}
 ...
 * - Slave mode: a LIN sleep command was received.
 * - Master mode: when no bus activity for a certain time. A goto sleep command should be sent to
 ...
 *
 * In case of LIN slave node, it is mandatory to assign callback before starting LIN bus because
