			      ...
			      uint16_t *data)
{
	...
	err = mdio_gpio_transfer(dev, prtad, devad, true, MDIO_OP_C22_READ, 0, data);
			       ...
			       uint16_t data)
{
	...
	err = mdio_gpio_transfer(dev, prtad, devad, true, MDIO_OP_C22_WRITE, data, NULL);
				  ...
				  uint16_t regad, uint16_t *data)
{
	...
	err = mdio_gpio_transfer(dev, prtad, devad, false, MDIO_OP_C45_ADDRESS, regad, NULL);
	if (err) {
		k_sem_give(&dev_data->sem);
	...
	err = mdio_gpio_transfer(dev, prtad, devad, false, MDIO_OP_C45_READ, 0, data);
				   ...
				   uint16_t regad, uint16_t data)
{
	...
	err = mdio_gpio_transfer(dev, prtad, devad, false, MDIO_OP_C45_WRITE, data, NULL);
