			 ...
			 enum mdio_opcode op, bool c45, uint16_t data_in, uint16_t *data_out)
{
	...
	cfg->regs->GMAC_MAN = (c45 ? 0U : GMAC_MAN_CLTTO) | GMAC_MAN_OP(op) | GMAC_MAN_WTN(0x02) |
...
static int mdio_sam_read(const struct device *dev, uint8_t prtad, uint8_t regad, uint16_t *data)
{
	return mdio_transfer(dev, prtad, regad, MDIO_OP_C22_READ, false, 0, data);
}

static int mdio_sam_write(const struct device *dev, uint8_t prtad, uint8_t regad, uint16_t data)
{
	return mdio_transfer(dev, prtad, regad, MDIO_OP_C22_WRITE, false, data, NULL);
}

static int mdio_sam_read_c45(const struct device *dev, uint8_t prtad, uint8_t devad, uint16_t regad,
			     uint16_t *data)
{
	int err;
...
static int mdio_sam_write_c45(const struct device *dev, uint8_t prtad, uint8_t devad,
