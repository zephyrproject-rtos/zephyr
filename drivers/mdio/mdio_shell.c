 ...
 * scan [<dev_addr>]
 */
static int cmd_mdio_scan(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (!dev) {
		shell_error(shell, "MDIO: Device driver %s not found.",
				MDIO_DEVICE);
		return -ENODEV;
	}
	...
	for (int i = 0; i < 32; i++) {
		...
		if (mdio_read(dev, i, dev_addr, &data) >= 0 &&
			data != 0xffff) {
			cnt++;
			shell_print(shell, "Found MDIO device @ 0x%x", i);
		}
	}

	mdio_bus_disable(dev);

	shell_print(shell, "%u devices found on %s", cnt, MDIO_DEVICE);

	return 0;
}

/* mdio write <port_addr> <dev_addr> <data> */
static int cmd_mdio_write(const struct shell *shell, size_t argc, char **argv)
{
	const struct device *dev;
	uint16_t data;
	uint16_t dev_addr;
	...
	if (mdio_write(dev, port_addr, dev_addr, data) < 0) {
		shell_error(shell, "Failed to write to device: %s",
				MDIO_DEVICE);
		mdio_bus_disable(dev);
		return -EIO;
	}
...
static int cmd_mdio_read(const struct shell *shell, size_t argc, char **argv)
{
	const struct device *dev;
	uint16_t data;
	uint16_t dev_addr;
	...
	if (mdio_read(dev, port_addr, dev_addr, &data) < 0) {
		shell_error(shell, "Failed to read from device: %s",
			MDIO_DEVICE);
		mdio_bus_disable(dev);
		return -EIO;
	}
	...
	shell_print(shell, "%x[%x]: 0x%x", port_addr, dev_addr, data);
