...
#define DEV_CFG(dev)  ((const struct ufshc_versal2_config *)(dev)->config)
#define DEV_DATA(dev) ((struct ufshc_versal2_data *)(dev)->data)
...
struct ufshc_versal2_config {
	DEVICE_MMIO_NAMED_ROM(mmio); /**< MMIO ROM entry for the UFSHC register region */
...
struct ufshc_versal2_data {
	DEVICE_MMIO_NAMED_RAM(mmio);      /**< MMIO RAM entry for UFSHC register region */
...
static void ufshc_versal2_initialization(const struct device *dev)
{
	...
	sys_write32((cfg->core_clk_rate / 1000000U),
		    (DEVICE_MMIO_NAMED_GET(dev, mmio) +
		    (uintptr_t)(VERSAL2_UFS_REG_HCLKDIV_OFFSET)));
 ...
 */
static int ufshc_versal2_link_startup_notify(const struct device *dev, uint8_t status)
{
	struct ufshc_versal2_data *drvdata = dev->data;
 ...
 * @return 0 (success) or non-zero value on failure.
 */
static int ufshc_versal2_phy_init(const struct device *dev)
{
	const struct ufshc_versal2_config *cfg = dev->config;
	struct ufshc_versal2_data *drvdata = dev->data;
	struct ufs_host_controller *ufshc = &drvdata->ufshc;
...
static int32_t ufshc_versal2_init(const struct device *dev)
{
	...
	ufshc->mmio_base = DEVICE_MMIO_NAMED_GET(dev, mmio);
