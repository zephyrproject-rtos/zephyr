...
#define DT_DRV_COMPAT st_stm32_ospi_psram_aps6408l
...
enum ospi_memory_type {
	MEMTYPE_MICRON = HAL_OSPI_MEMTYPE_MICRON,
	MEMTYPE_MACRONIX = HAL_OSPI_MEMTYPE_MACRONIX,
	MEMTYPE_APMEMORY = HAL_OSPI_MEMTYPE_APMEMORY,
	MEMTYPE_MACRONIX_RAM = HAL_OSPI_MEMTYPE_MACRONIX_RAM,
	MEMTYPE_HYPERBUS = HAL_OSPI_MEMTYPE_HYPERBUS
};
			      ...
			      uint32_t latency_cycles)
{
	...
	sCommand.DataDtrMode = HAL_OSPI_DATA_DTR_ENABLE;
	sCommand.NbData = 2;
	sCommand.DummyCycles = latency_cycles;
...
static int ap_memory_configure(OSPI_HandleTypeDef *hospi)
{
	...
	uint8_t regW_MR0[2] = {
		(DT_INST_PROP(0, fixed_latency) ? 0x20U : 0x00U) | (read_latency_code << 2)|
			       (DT_INST_PROP(0, drive_strength)), 0x0D};
	...
	if (ap_memory_write_reg(hospi, MR0, regW_MR0) != HAL_OK) {
		return -EIO;
	}

	if (ap_memory_read_reg(hospi, MR0, regR_MR0, read_latency_cycles) != HAL_OK) {
		return -EIO;
	}
...
static int memc_stm32_ospi_psram_init(const struct device *dev)
{
	...
	struct memc_stm32_ospi_psram_data *dev_data = dev->data;
	OSPI_HandleTypeDef *hospi = &dev_data->hospi;
	OSPIM_CfgTypeDef *sOspiManagerCfg = &dev_data->sOspiManagerCfg;
	HAL_OSPI_DLYB_CfgTypeDef *HAL_OSPI_DLYB_Cfg_Struct = &dev_data->HAL_OSPI_DLYB_Cfg_Struct;
...
struct memc_stm32_ospi_psram_config memc_stm32_ospi_config = {
...
struct memc_stm32_ospi_psram_data memc_stm32_ospi_data = {
	.hospi =
		{
			...
			.Init =
				{
					...
					.MemoryType = CONCAT(MEMTYPE_, DT_INST_STRING_UPPER_TOKEN(
									       0, memory_type)),
					...
					.DelayBlockBypass = DT_PROP(STM32_OSPI_NODE, dlyb_bypass)
...
DEVICE_DT_INST_DEFINE(0, &memc_stm32_ospi_psram_init, NULL, &memc_stm32_ospi_data,
		      &memc_stm32_ospi_config, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,
		      NULL);
