...
#define REMAP_IMAGE_DST    DT_NODE_BY_PARTITION_LABEL(image_0)
#define REMAP_IMAGE_SRC    DT_NODE_BY_PARTITION_LABEL(image_1)
...
/* Make sure remapping alignment is a multiple of 1024 */
 ...
 *
 * To make things more safe the image header is not included in the remapped area.
 * This guarantees that correct data will be read from the header even when AHB read is used.
 *
 */
	...
	uint32_t flash_base_ns = flash_base & ~(1 << TZ_BIT_NUM);
	...
	/* remapping is set from the end of MCUboot header to the end of image slot */
	flexspi_addr_map_config_t cfg = {
		.addrStart  = flash_base_ns + slot0_offset + REMAP_ALIGNMENT,
		.addrEnd    = flash_base_ns + slot0_offset + slot0_size,
