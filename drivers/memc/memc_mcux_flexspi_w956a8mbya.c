...
static int memc_flexspi_w956a8mbya_get_vendor_id(const struct device *dev, uint16_t *vendor_id)
{
...
static int memc_flexspi_w956a8mbya_init(const struct device *dev)
{
	...
	if (memc_flexspi_set_device_config(
		    ...
		    sizeof(memc_flexspi_w956a8mbya_lut) / MEMC_FLEXSPI_CMD_SIZE, config->port)) {
...
#define MEMC_FLEXSPI_W956A8MBYA(n)                                                                 \
	static const struct memc_flexspi_w956a8mbya_config memc_flexspi_w956a8mbya_config_##n = {  \
		.port = DT_INST_REG_ADDR(n),                                                       \
		.config = MEMC_FLEXSPI_DEVICE_CONFIG(n),                                           \
	};                                                                                         \
                                                                                                   \
	static struct memc_flexspi_w956a8mbya_data memc_flexspi_w956a8mbya_data_##n = {            \
		.controller = DEVICE_DT_GET(DT_INST_BUS(n)),                                       \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, memc_flexspi_w956a8mbya_init, NULL,                               \
			      &memc_flexspi_w956a8mbya_data_##n,                                   \
			      &memc_flexspi_w956a8mbya_config_##n, POST_KERNEL,                    \
