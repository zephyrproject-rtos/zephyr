...
/**
 * @brief Enable or disable IPED IP-command write encryption (IPWR_EN)
 *
 * IPWR_EN is normally provisioned via BootROM.  This API lets software
 * change it at runtime, e.g. to exercise the CTR write path in samples
 * and tests.
 *
 * @param dev FlexSPI device
 * @param enable true to set IPWR_EN, false to clear it
 * @return 0 on success, -ENOTSUP if IPED is not available or not enabled
 */
int memc_flexspi_iped_ipwr_enable(const struct device *dev, bool enable);
