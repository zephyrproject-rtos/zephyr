...
/*
 * The SEMC_IOCR CSX pin mux enum values are not consecutive and their
 * spacing differs between SoC families, so the chip select has to be mapped
 * to the matching pin mux value instead of being computed from CSX0.
 */
#define MEMC_MCUX_SEMC_SDRAM_CSX_MUX(cs)                                                           \
	((cs) == 0   ? kSEMC_MUXCSX0                                                               \
	 : (cs) == 1 ? kSEMC_MUXCSX1                                                               \
	 : (cs) == 2 ? kSEMC_MUXCSX2                                                               \
		     : kSEMC_MUXCSX3)
...
#define MEMC_MCUX_SEMC_SDRAM_INIT(n)                                                               \
	static const struct memc_mcux_semc_sdram_config memc_mcux_semc_sdram_config_##n = {        \
		...
		.sdram_cfg =                                                                       \
			{                                                                          \
				.csxPinMux = (semc_iomux_pin)MEMC_MCUX_SEMC_SDRAM_CSX_MUX(         \
					DT_INST_PROP(n, cs)),                                      \
