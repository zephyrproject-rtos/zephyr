...
static int memc_flexspi_is66wvq8m4_get_vendor_id(const struct device *dev, uint8_t *vendor_id)
{
...
static int memc_flexspi_is66wvq8m4_update_cfg(const struct device *dev, uint16_t mask,
					      uint16_t set_val)
{
	const struct memc_flexspi_is66wvq8m4_config *config = dev->config;
	struct memc_flexspi_is66wvq8m4_data *data = dev->data;
...
static int memc_flexspi_is66wvq8m4_init(const struct device *dev)
{
	...
	if (memc_flexspi_set_device_config(
		    ...
		    sizeof(memc_flexspi_is66wvq8m4_lut) / MEMC_FLEXSPI_CMD_SIZE, config->port)) {
