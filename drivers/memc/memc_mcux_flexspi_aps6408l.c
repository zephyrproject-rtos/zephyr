	...
	[READ_DATA] =
		{
			...
			FLEXSPI_LUT_SEQ(kFLEXSPI_Command_DUMMY_RWDS_DDR, kFLEXSPI_8PAD, 0x07,
...
static int memc_flexspi_aps6408l_get_vendor_id(const struct device *dev, uint8_t *vendor_id)
{
...
static int memc_flexspi_aps6408l_update_reg(const struct device *dev, uint8_t reg, uint8_t mask,
					    uint8_t set_val)
{
	const struct memc_flexspi_aps6408l_config *config = dev->config;
	struct memc_flexspi_aps6408l_data *data = dev->data;
	...
	transfer.cmdType = kFLEXSPI_Write, transfer.seqIndex = WRITE_REG;
...
static int memc_flexspi_aps6408l_init(const struct device *dev)
{
	...
	if (memc_flexspi_set_device_config(
		    data->controller, &config->config, (const uint32_t *)memc_flexspi_aps6408l_lut,
		    sizeof(memc_flexspi_aps6408l_lut) / MEMC_FLEXSPI_CMD_SIZE, config->port)) {
	...
	if (vendor_id != APM_VENDOR_ID) {
		LOG_WRN("Vendor ID does not match expected value of 0x%0x", APM_VENDOR_ID);
	}
	...
	if (memc_flexspi_aps6408l_update_reg(dev, APS_6408L_MR_4, APS_6408L_WLC_MASK,
					     APS_6408L_WLC_200)) {
...
#define MEMC_FLEXSPI_APS6408L(n)                                                                  \
	static const struct memc_flexspi_aps6408l_config memc_flexspi_aps6408l_config_##n = {     \
		.port = DT_INST_REG_ADDR(n),                                                      \
		.config = MEMC_FLEXSPI_DEVICE_CONFIG(n),                                          \
	};                                                                                        \
                                                                                                  \
	static struct memc_flexspi_aps6408l_data memc_flexspi_aps6408l_data_##n = {               \
		.controller = DEVICE_DT_GET(DT_INST_BUS(n)),                                      \
	};                                                                                        \
                                                                                                  \
	DEVICE_DT_INST_DEFINE(n, memc_flexspi_aps6408l_init, NULL,                                \
			      &memc_flexspi_aps6408l_data_##n, &memc_flexspi_aps6408l_config_##n, \
