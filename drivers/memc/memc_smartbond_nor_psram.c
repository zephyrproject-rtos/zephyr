...
static void memc_automode_configure(void)
{
	...
	QSPIC2_CTRLMODE_REG_SET_FIELD(QSPIC_USE_32BA, reg,
						DT_INST_PROP(0, addr_size));
	QSPIC2_CTRLMODE_REG_SET_FIELD(QSPIC_CLK_MD, reg,
						DT_INST_PROP(0, clock_mode));
	...
	QSPIC2_BURSTCMDB_REG_SET_FIELD(QSPIC_DMY_NUM, reg,
						DT_INST_PROP(0, dummy_bytes_size));
	QSPIC2_BURSTCMDB_REG_SET_FIELD(QSPIC_DAT_RX_MD, reg,
						DT_INST_PROP(0, rx_data_mode));
...
static int memc_smartbond_init(const struct device *dev)
{
	...
	memc_set_status(true, DT_INST_PROP(0, clock_div));
...
#if CONFIG_PM_DEVICE_RUNTIME
		/*
		 * Once the device is configured on applicaton level, prevent the
		 * system from being powered off (a low-power mode is used instead).
		 * As such, registers values are not lost until application suspend
		 * the device, explicitly. When the runtime mode is not enabled,
		 * there is no need to prevent the extended sleep state as accessing
		 * the memory device will block the AMBA bus till transfer is completed.
		 */
