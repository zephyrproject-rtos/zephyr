...
#define SFP_MDAD_NODE(n, i) DT_INST_CHILD(n, sfp_S_mdad_##i)
...
#define SFP_FRAD_NODE(n, i) DT_INST_CHILD(n, sfp_S_frad_##i)
...
#define _QSPI_SFP_FRAD_CFG(i, n)							\
	COND_CODE_1(DT_NODE_EXISTS(SFP_FRAD_NODE(n, i)),				\
		({ /* Get configuration from DT if DT node exists */			\
			.StartAddress = DT_REG_ADDR(SFP_FRAD_NODE(n, i)),		\
			.EndAddress = DT_REG_ADDR(SFP_FRAD_NODE(n, i)) +		\
					DT_REG_SIZE(SFP_FRAD_NODE(n, i)) - 1,		\
			.Valid = true,							\
