...
const uint32_t memc_xspi_w958d6nbkx_lut[][5] = {
	...
	[PSRAM_CMD_MEM_READ] = {
		...
		XSPI_LUT_SEQ(kXSPI_Command_CADDR_DDR, kXSPI_8PAD, 0x10, kXSPI_Command_DUMMY_SDR, kXSPI_8PAD, 6),
	...
	[PSRAM_CMD_REG_READ] = {
		...
		XSPI_LUT_SEQ(kXSPI_Command_CADDR_DDR, kXSPI_8PAD, 0x10, kXSPI_Command_DUMMY_SDR, kXSPI_8PAD, 6), /* Dummy cycle: 2 * 6 + 2 */
		XSPI_LUT_SEQ(kXSPI_Command_READ_DDR, kXSPI_8PAD, 0x08, kXSPI_Command_STOP, kXSPI_1PAD, 0x0),
	},
...
static struct memc_xspi_dev_config dev_configs[] = {
	{
		...
		.lut_count = sizeof(memc_xspi_w958d6nbkx_lut) / sizeof(memc_xspi_w958d6nbkx_lut[0][0])
	},
};

static int xspi_psram_write_reg(const struct device *dev, uint32_t regAddr, uint8_t *data, uint32_t size);
static int xspi_psram_read_reg(const struct device *dev, uint32_t regAddr, uint8_t *data, uint32_t size);

static int memc_mcux_xspi_w958d6nbkx_setup(const struct device *dev, xspi_device_config_t *config)
{
	...
	do {
		...
		if (id != PSRAM_MANUFACTURER_ID_WINBOND) {
			LOG_ERR("Wrong manufacturer ID: 0x%X, expected: 0x%X",
					id, PSRAM_MANUFACTURER_ID_WINBOND);
			ret = -ENODEV;
			break;
		}
...
static int memc_mcux_xspi_psram_setup(const struct device *dev, const char *dev_name_prefix, xspi_device_config_t *config)
{
	int ret = 0;
...
{
	struct memc_mcux_xspi_psram_config *config = (struct memc_mcux_xspi_psram_config *)dev->config;
	struct memc_mcux_xspi_psram_data *data = dev->data;
	const struct device *xspi_dev = data->xspi_dev;
	...
	ret = memc_xspi_set_device_config(xspi_dev, dev_config, xspi_psram_config->lut_array, xspi_psram_config->lut_count);
	if (ret < 0) {
		return ret;
	}
