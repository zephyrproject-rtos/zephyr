...
#define MEMC_MCUX_XSPI_TARGET_GROUP_COUNT 2
#define MEMC_MCUX_XSPI_SFP_FRAD_COUNT 8
...
void memc_mcux_xspi_update_device_addr_mode(const struct device *dev, xspi_device_addr_mode_t addr_mode)
{
...
static int memc_mcux_xspi_init(const struct device *dev)
{
	...
	if (memc_xspi_is_running_xip(dev)) {
		if (!IS_ENABLED(CONFIG_MEMC_MCUX_XSPI_INIT_XIP)) {
			LOG_ERR("XIP active on %s, skipping init\n", dev->name);
			return 0;
		}
	}
	...
	if (memc_xspi_config->mdad_valid) {
		config.ptrIpAccessConfig->ptrSfpMdadConfig = (xspi_sfp_mdad_config_t *)&memc_xspi_config->mdad_configs;
	}
	if (memc_xspi_config->frad_valid) {
		config.ptrIpAccessConfig->ptrSfpFradConfig = (xspi_sfp_frad_config_t *)&memc_xspi_config->frad_configs;
	}
...
#define MCUX_XSPI_MDAD_INIT(idx, n) \
	COND_CODE_1(DT_NODE_EXISTS(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx)), \
		({ \
			.enableDescriptorLock = DT_PROP(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx), enable_descriptor_lock), \
			.maskType = DT_PROP(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx), mask_type), \
			.mask = DT_PROP(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx), mask), \
			.masterIdReference = DT_PROP(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx), master_id_reference), \
			.secureAttribute = DT_PROP(DT_CHILD(DT_DRV_INST(n), mdad_tg ## idx), secure_attribute), \
		}), \
		({ \
			0 \
		}))
...
#define MCUX_XSPI_FRAD_INIT(idx, n) \
	COND_CODE_1(DT_NODE_EXISTS(DT_CHILD(DT_DRV_INST(n), frad_region ## idx)), \
		({ \
			.startAddress = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), start_address), \
			.endAddress = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), end_address), \
			.tg0MasterAccess = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), tg0_master_access), \
			.tg1MasterAccess = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), tg1_master_access), \
			.assignIsValid = true, \
			.descriptorLock = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), descriptor_lock), \
			.exclusiveAccessLock = DT_PROP(DT_CHILD(DT_DRV_INST(n), frad_region ## idx), exclusive_access_lock), \
		}), \
		({ \
			0 \
		}))
...
#define MCUX_XSPI_INIT(n) \
	...
	static const struct memc_mcux_xspi_config memc_mcux_xspi_config_##n = { \
		...
		.mdad_configs = { \
			.tgMdad = { \
				LISTIFY(MEMC_MCUX_XSPI_TARGET_GROUP_COUNT, MCUX_XSPI_MDAD_INIT, (,), n) \
			}, \
		}, \
		.mdad_valid = DT_NODE_EXISTS(DT_CHILD(DT_DRV_INST(n), mdad_tg0)), \
		.frad_configs = { \
			.fradConfig = { \
				LISTIFY(MEMC_MCUX_XSPI_SFP_FRAD_COUNT, MCUX_XSPI_FRAD_INIT, (,), n) \
			}, \
		}, \
