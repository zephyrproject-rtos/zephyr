...
#define REGION_CONFIG(node_id)                                                                     \
	{                                                                                          \
		...
		.addrsel = DT_PROP(node_id, address_select),                                       \
		.busrcon = DT_PROP(node_id, bus_read_configuration) |                              \
			   DT_ENUM_IDX(node_id, device_type) << EBU_BUSRCON0_AGEN_Pos,             \
		.buswcon = DT_PROP(node_id, bus_write_configuration) |                             \
			   DT_ENUM_IDX(node_id, device_type) << EBU_BUSWCON0_AGEN_Pos,             \
		.busrap = DT_PROP(node_id, bus_read_timing),                                       \
		.buswap = DT_PROP(node_id, bus_write_timing),                                      \
	},
