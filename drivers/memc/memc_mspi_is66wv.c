...
#define IS66WV_DEEP_POWER_DOWN 0xB9
...
static int memc_mspi_is66wv_dpd_enter(const struct device *psram)
{
	int ret;

	LOG_DBG("Entering Deep Power Down");

	ret = memc_mspi_is66wv_command_write(psram, IS66WV_DEEP_POWER_DOWN, 0, NULL, 0);
	if (ret) {
		LOG_ERR("Failed to enter DPD");
		return ret;
	}
	/* tDPDIN: 150 µs min to reach DPD power level */
	k_busy_wait(150);

	return 0;
}

static int memc_mspi_is66wv_dpd_exit(const struct device *psram)
{
	const struct memc_mspi_is66wv_config *cfg = psram->config;
	struct memc_mspi_is66wv_data *data = psram->data;
	int ret;

	LOG_DBG("Exiting Deep Power Down");

	/*
	 * DPD exit: assert CE# for tDPDX (200 ns min) via a dummy command.
	 * Device ignores the command content during exit.
	 */
	ret = memc_mspi_is66wv_command_write(psram, 0, 0, NULL, 0);
	if (ret) {
		LOG_ERR("Failed to exit DPD");
		return ret;
	}
	/* tDPDOUT: max 150 µs from CE# high to standby */
	k_busy_wait(150);

	ret = mspi_dev_config(cfg->bus, &cfg->dev_id, MSPI_DEVICE_CONFIG_ALL,
			      (const struct mspi_dev_cfg *)&data->dev_cfg);
	if (ret) {
		LOG_ERR("Failed to reconfigure MSPI after DPD exit");
		return -EIO;
	}
...
static int memc_mspi_is66wv_pm_action(const struct device *psram, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		acquire(psram);
		memc_mspi_is66wv_dpd_enter(psram);
		release(psram);
		break;

	case PM_DEVICE_ACTION_RESUME:
		acquire(psram);
		memc_mspi_is66wv_dpd_exit(psram);
		release(psram);
		break;
...
#define MEMC_MSPI_IS66WV(n)                                                                        \
	static const struct memc_mspi_is66wv_config memc_mspi_is66wv_config_##n = {                \
		.port = 0,                                                                         \
	...
	static struct memc_mspi_is66wv_data memc_mspi_is66wv_data_##n = {                          \
		.lock = Z_SEM_INITIALIZER(memc_mspi_is66wv_data_##n.lock, 0, 1),                   \
	};                                                                                         \
	PM_DEVICE_DT_INST_DEFINE(n, memc_mspi_is66wv_pm_action);                                   \
	DEVICE_DT_INST_DEFINE(n, memc_mspi_is66wv_init, PM_DEVICE_DT_INST_GET(n),                  \
			      &memc_mspi_is66wv_data_##n, &memc_mspi_is66wv_config_##n,            \
			      POST_KERNEL, CONFIG_MEMC_INIT_PRIORITY, &memc_mspi_is66wv_api);
