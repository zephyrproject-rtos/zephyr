...
struct mfd_pca9422_data {
	...
	struct k_sem lock;
	const struct device *dev;
	struct mfd_pca9422_child *children;
	size_t num_children;
};
...
static void mfd_pca9422_isr(const struct device *dev)
{
	...
	POWER_DisableInterrupts(PMC_CTRL_INTRPADEN_MASK);
...
static void mfd_pca9422_work_handler(struct k_work *work)
{
	...
	if (reg_top_int & PCA9422_BIT_SYS_INT) {
		...
		ret = mfd_pca9422_reg_burst_read(data->dev, PCA9422_REG_SUB_INT0, reg_val,
						 4);
		if (ret < 0) {
	...
	if (reg_top_int &
		(PCA9422_BIT_SW_BB_INT | PCA9422_BIT_SW_INT | PCA9422_BIT_LDO_INT)) {
		/* Regulator block interrupt event trigger */
		/* Read sub level interrupt 2 and mask to clear flags */
		ret = mfd_pca9422_reg_burst_read(data->dev, PCA9422_REG_SUB_INT2, reg_val,
						 2);
		if (ret < 0) {
		...
		if (reg_top_int & PCA9422_BIT_SW_BB_INT) {
			...
			ret = mfd_pca9422_reg_burst_read(
				data->dev, PCA9422_REG_INT1, reg_val, 2);
			if (ret < 0) {
...
error:
	 ...
	 */
#if defined(CONFIG_SOC_SERIES_IMXRT5XX)
	POWER_ClearEventFlags(PMC_FLAGS_INTNPADF_MASK);
#elif defined(CONFIG_SOC_SERIES_IMXRT7XX)
...
void mfd_pca9422_set_irqhandler(const struct device *dev, const struct device *child_dev,
				uint8_t child_num, child_isr_t handler)
{
	struct mfd_pca9422_data *data = dev->data;
	struct mfd_pca9422_child *child;

	/* Set child device piont */
	child = &data->children[child_num];

	/* Store the interrupt handler and device node for child device */
	child->dev = child_dev;
	child->child_isr = handler;
	child->child_idx = child_num;
}
...
static int mfd_pca9422_init(const struct device *dev)
{
	...
	val = 0xFFU;
	val = val & ~PCA9422_BIT_ON_SHORT_PUSH; /* for test */
	...
	val = 0xFFU;
	val = val & ~PCA9422_BIT_ON_LONG_PUSH; /* For test */
	...
	val = 0xFFU;
	ret = i2c_reg_write_byte_dt(&config->bus, PCA9422_REG_INT1_MASK, val);
...
#define MFD_PCA9422_CHILD_INIT(child_node_id)                                                      \
	[DT_NODE_CHILD_IDX(child_node_id)] = {                                                     \
		.child_idx = DT_NODE_CHILD_IDX(child_node_id),                                     \
	},

#define MFD_PCA9422_INIT(inst)                                                                     \
                                                                                                   \
	static struct mfd_pca9422_child pca9422_children[PCA9422_DEV_MAX] = {                      \
		DT_INST_FOREACH_CHILD_STATUS_OKAY(inst, MFD_PCA9422_CHILD_INIT)};                  \
                                                                                                   \
	...
	static struct mfd_pca9422_data mfd_pca9422_data_##inst = {                                 \
		.work = Z_WORK_INITIALIZER(mfd_pca9422_work_handler),                              \
		.lock = Z_SEM_INITIALIZER(mfd_pca9422_data_##inst.lock, 1, 1),                     \
		.dev = DEVICE_DT_INST_GET(inst),                                                   \
		.children = pca9422_children,                                                      \
		.num_children = ARRAY_SIZE(pca9422_children),                                      \
	};                                                                                         \
                                                                                                   \
