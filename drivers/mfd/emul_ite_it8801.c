...
static void it8801_emul_backend_reset(const struct emul *target)
{
...
static int it8801_emul_backend_get_reg(const struct emul *target, uint8_t reg,
				       uint8_t *val)
{
	struct it8801_emul_data *data = target->data;
...
static int it8801_emul_backend_set_reg(const struct emul *target, uint8_t reg,
				       uint8_t val)
{
	struct it8801_emul_data *data = target->data;
...
static void it8801_emul_backend_set_read_fail_reg(const struct emul *target,
						  uint16_t reg)
{
	struct it8801_emul_data *data = target->data;
...
static void it8801_emul_backend_set_write_fail_reg(const struct emul *target,
						   uint16_t reg)
{
	struct it8801_emul_data *data = target->data;
...
static const struct it8801_emul_driver_api it8801_emul_backend_api = {
	.reset = it8801_emul_backend_reset,
	.get_reg = it8801_emul_backend_get_reg,
	.set_reg = it8801_emul_backend_set_reg,
	.set_read_fail_reg = it8801_emul_backend_set_read_fail_reg,
	.set_write_fail_reg = it8801_emul_backend_set_write_fail_reg,
};
 ...
 *
 * Failure injection may be configured via the backend API to force the
 * emulator to reject accesses to a specific register.
 */
...
#define IT8801_EMUL_RESET_RULE_BEFORE(inst)                                                        \
	it8801_emul_backend_reset(&EMUL_DT_NAME_GET(DT_DRV_INST(inst)));
...
static int it8801_emul_init(const struct emul *target, const struct device *parent)
{
	...
	it8801_emul_backend_reset(target);
	return 0;
}
...
#define IT8801_EMUL_DEFINE(n)                                                                      \
	...
	EMUL_DT_INST_DEFINE(n, it8801_emul_init, &it8801_emul_data_##n, &it8801_emul_cfg_##n,      \
			    &it8801_emul_bus_api, &it8801_emul_backend_api);
