...
LOG_MODULE_REGISTER(mfd_aw9523b, CONFIG_MFD_LOG_LEVEL);

#define AW9523B_ID_VALUE             0x23
#define AW9523B_RESET_PULSE_WIDTH_US 20
...
static int mfd_aw9523b_hw_reset(const struct device *dev)
{
	...
	if (ret) {
		LOG_ERR("%s: Could not configure reset GPIO: %d", dev->name, ret);
		return ret;
	}

	k_busy_wait(AW9523B_RESET_PULSE_WIDTH_US);

	ret = gpio_pin_set_dt(&config->reset_gpio, 0);
	if (ret) {
		LOG_ERR("%s: Could not clear reset GPIO: %d", dev->name, ret);
		return ret;
	}
#endif
	return 0;
...
static int mfd_aw9523b_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_TURN_ON:
		...
		if (ret) {
			LOG_ERR("%s: I2C ID check failed: %d", dev->name, ret);
			return ret;
		};

		if (reg != AW9523B_ID_VALUE) {
			LOG_ERR("%s: Invalid ID read: %u", dev->name, reg);
			return -EIO;
		}
...
static int mfd_aw9523b_init(const struct device *dev)
{
	...
	int ret = mfd_aw9523b_hw_reset(dev);

	if (ret < 0) {
		LOG_ERR("%s: Could not perform hardware reset: %d", dev->name, ret);
		return ret;
	}
