...
static int mfd_adp5585_software_reset(const struct device *dev)
{
	...
	if (ret == 0) {
		ret = i2c_reg_write_byte_dt(&config->i2c_bus, ADP5585_PIN_CONFIG_A, 0x00U);
	}
	if (ret == 0) {
		ret = i2c_reg_write_byte_dt(&config->i2c_bus, ADP5585_PIN_CONFIG_B, 0x00U);
	}
...
static void mfd_adp5585_work_handler(struct k_work *work)
{
	...
	/* Read Interrput Flag */
	if (ret == 0) {
		ret = i2c_burst_read_dt(&config->i2c_bus, ADP5585_INT_STATUS,
					(uint8_t *)&reg_int_status, 1U);
	}
...
{
	const struct mfd_adp5585_config *config = dev->config;
	const struct mfd_adp5585_data *data = dev->data;
	int ret;
