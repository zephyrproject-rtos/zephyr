...
struct tle9104_config {
	...
	uint16_t diagnostic_filter_time_us;
	uint16_t overcurrent_shutdown_delay_time_us;
	uint16_t overcurrent_shutdown_threshold_ma;
};
...
static int tle9104_clear_diagnostics_internal(const struct device *dev)
{
	...
	result = tle9104_transceive_frame(dev, true, TLE9104REGISTER_DIAGOUT12ON, 0xFF, &read_reg,
					  &temp);
	if (result != 0) {
		return result;
	}

	result = tle9104_transceive_frame(dev, true, TLE9104REGISTER_DIAGOUT34ON, 0xFF, &read_reg,
					  &temp);
	if (result != 0) {
		return result;
	}

	result = tle9104_transceive_frame(dev, true, TLE9104REGISTER_DIAGOFF, 0xFF, &read_reg,
					  &temp);
	if (result != 0) {
		return result;
	}
...
static int tle9104_init(const struct device *dev)
{
	...
	register_cfg = 0x00;
	switch (config->diagnostic_filter_time_us) {
	case 2000:
		tle9104_set_register_bits(&register_cfg, TLE9104_OFFDIAGCFG_DIAGFILTCFG_POS,
					  TLE9104_OFFDIAGCFG_DIAGFILTCFG_LENGTH, 3);
		break;
	case 1000:
		tle9104_set_register_bits(&register_cfg, TLE9104_OFFDIAGCFG_DIAGFILTCFG_POS,
					  TLE9104_OFFDIAGCFG_DIAGFILTCFG_LENGTH, 2);
		break;
	case 500:
		tle9104_set_register_bits(&register_cfg, TLE9104_OFFDIAGCFG_DIAGFILTCFG_POS,
					  TLE9104_OFFDIAGCFG_DIAGFILTCFG_LENGTH, 1);
		break;
	case 200:
		tle9104_set_register_bits(&register_cfg, TLE9104_OFFDIAGCFG_DIAGFILTCFG_POS,
					  TLE9104_OFFDIAGCFG_DIAGFILTCFG_LENGTH, 0);
		break;
	default:
		LOG_ERR("invalid value %i for diagnostic filter time",
			config->diagnostic_filter_time_us);
		return -EINVAL;
	}
	...
	register_cfg = 0x00;
	switch (config->overcurrent_shutdown_delay_time_us) {
	case 30000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 7);
		break;
	case 20000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 6);
		break;
	case 10000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 5);
		break;
	case 5000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 4);
		break;
	case 1000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 3);
		break;
	case 500:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 2);
		break;
	case 120:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 1);
		break;
	case 60:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCFILTCFG_POS,
					  TLE9104_ONDIAGCFG_OCFILTCFG_LENGTH, 0);
		break;
	default:
		LOG_ERR("invalid value %i for overcurrent shutdown delay time",
			config->overcurrent_shutdown_delay_time_us);
		return -EINVAL;
	}
	switch (config->overcurrent_shutdown_threshold_ma) {
	case 4000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCTH_POS,
					  TLE9104_ONDIAGCFG_OCTH_LENGTH, 3);
		break;
	case 3000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCTH_POS,
					  TLE9104_ONDIAGCFG_OCTH_LENGTH, 2);
		break;
	case 2000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCTH_POS,
					  TLE9104_ONDIAGCFG_OCTH_LENGTH, 1);
		break;
	case 1000:
		tle9104_set_register_bits(&register_cfg, TLE9104_ONDIAGCFG_OCTH_POS,
					  TLE9104_ONDIAGCFG_OCTH_LENGTH, 0);
		break;
	default:
		LOG_ERR("invalid value %i for overcurrent shutdown threshold",
			config->overcurrent_shutdown_threshold_ma);
		return -EINVAL;
	}
...
#define TLE9104_INIT(inst)                                                                         \
	static const struct tle9104_config tle9104_##inst##_config = {                             \
		...
		.diagnostic_filter_time_us = DT_INST_PROP(inst, diagnostic_filter_time),           \
		.overcurrent_shutdown_delay_time_us =                                              \
			DT_INST_PROP(inst, overcurrent_shutdown_delay_time),                       \
		.overcurrent_shutdown_threshold_ma =                                               \
			DT_INST_PROP(inst, overcurrent_shutdown_threshold),                        \
	};                                                                                         \
                                                                                                   \
