...
#if CONFIG_MFD_AXP2101_INTERRUPT
...
#define AXP2101_REG_IRQ_STATUS_1	0x49U
	#define AXP2101_SHORT_PRESS_IRQ			BIT(3)
	#define AXP2101_LONG_PRESS_IRQ			BIT(2)
	#define AXP2101_PWR_ON_NEG_EDGE_IRQ		BIT(1)
	#define AXP2101_PWR_ON_POS_EDGE_IRQ		BIT(0)
...
static const uint8_t axp2101_dflt_irq_enable[] = {
	...
	AXP2101_PWR_ON_NEG_EDGE_IRQ | AXP2101_PWR_ON_POS_EDGE_IRQ,
#else
	0x00,
#endif
					     ...
					     uint8_t irq_status[AXP2101_IRQ_STATUS_REG_COUNT])
{
	...
	for (reg = AXP2101_REG_IRQ_STATUS_0; reg <= AXP2101_REG_IRQ_STATUS_2; reg++) {
		ret = i2c_reg_read_byte_dt(i2c, reg, &val);
		if (ret < 0) {
			LOG_ERR("Failed to read IRQ status reg 0x%x", reg);
			return ret;
		}

		ret = i2c_reg_write_byte_dt(i2c, reg, val);
		if (ret < 0) {
			LOG_ERR("Failed to write IRQ status reg 0x%x", reg);
			return ret;
		}

		irq_status[reg - AXP2101_REG_IRQ_STATUS_0] = val;
	}
...
#if CONFIG_MFD_AXP2101_POWER_BUTTON
	if (irq_status_regs[AXP2101_IRQ_STATUS_1_IDX] & AXP2101_PWR_ON_NEG_EDGE_IRQ) {
		ret = input_report_key(dev, INPUT_KEY_POWER, true, true, K_FOREVER);
	}
	if (irq_status_regs[AXP2101_IRQ_STATUS_1_IDX] & AXP2101_PWR_ON_POS_EDGE_IRQ) {
...
#endif /* CONFIG_MFD_AXP2101_INTERRUPT */
...
static int mfd_axp2101_init(const struct device *dev)
{
	...
	/* Disable all interrupts (most are enabled by default) */
	for (i = 0; i < ARRAY_SIZE(axp2101_dflt_irq_enable); i++) {
		ret = i2c_reg_write_byte_dt(&config->i2c, AXP2101_REG_IRQ_ENABLE_0 + i,
					    axp2101_dflt_irq_enable[i]);
		if (ret < 0) {
			LOG_ERR("Failed to disable IRQ reg 0x%x", i);
			return ret;
		}
	}

	/* Clear any pending interrupt (if any) */
	for (i = AXP2101_REG_IRQ_STATUS_0; i <= AXP2101_REG_IRQ_STATUS_2; i++) {
		ret = i2c_reg_write_byte_dt(&config->i2c, i, 0xFF);
		if (ret < 0) {
			LOG_ERR("Unable to clear IRQ status reg 0x%x", i);
			return ret;
		}
	}
...
#define MFD_AXP2101_DEFINE(node)                                                                   \
	static const struct mfd_axp2101_config config##node = {                                    \
		...
		IF_ENABLED(CONFIG_MFD_AXP2101_INTERRUPT,                                           \
			   (.int_gpio = GPIO_DT_SPEC_GET_OR(node, int_gpios, {0}),))               \
	};                                                                                         \
                                                                                                   \
	static struct mfd_axp2101_data data##node;                                                 \
                                                                                                   \
	DEVICE_DT_DEFINE(node, mfd_axp2101_init, NULL, &data##node,                                \
