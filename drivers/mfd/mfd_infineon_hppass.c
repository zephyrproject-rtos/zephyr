...
/*
 * DeepSleep retains the HPPASS config (STT, startup, triggers, infra trims and
 * INTR_MASK) but drops the block out of BLOCK_READY and resets the volatile SAR
 * calibration.  RESUME reloads the calibration and re-runs the AC startup.
 */
static int ifx_hppass_mfd_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_TURN_ON: {
		...
		/* Full rebuild; the NVIC is restored by the platform resume path. */
		ret = ifx_hppass_init_hw(dev);
		if (ret != 0) {
			return ret;
		}
		/* init_hw() never writes INTR_MASK, which the power loss cleared. */
		ifx_hppass_update_hw_mask(dev);
		break;
	}
#endif
