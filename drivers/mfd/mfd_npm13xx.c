...
static void work_callback(struct k_work *work)
{
	 ...
	 * guards only the list add/remove and is deliberately not held here. The
	 * _SAFE walk caches the next node, so a handler may remove its own
	 * callback; removing another callback from a handler is not supported,
	 * same as gpio_fire_callbacks(). Mirrors mfd_npm10xx.
	 */
