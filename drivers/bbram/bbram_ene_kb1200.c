...
static int bbram_kb1200_read(const struct device *dev, size_t offset, size_t size, uint8_t *data)
{
	...
	if (((offset + size) > config->size) || (size < 1)) {
		return -ERANGE;
	}
	memcpy((void *)data, (void *)((uint8_t *)&vbat->BKUPSTS + offset), size);
	return 0;
}
			      ...
			      const uint8_t *data)
{
	...
	memcpy((void *)((uint8_t *)&vbat->BKUPSTS + offset), (void *)data, size);
	return 0;
}
...
#define BBRAM_KB1200_DEVICE(inst)                                                                  \
	...
	static const struct bbram_kb1200_config bbram_cfg_##inst = {                               \
		...
		.size = DT_INST_PROP(inst, bbram_size),                                            \
	};                                                                                         \
