...
#define DRV_CONFIG(dev) ((const struct bbram_it8xxx2_config *)(dev)->config)

static int bbram_it8xxx2_read(const struct device *dev, size_t offset, size_t size, uint8_t *data)
{
	const struct bbram_it8xxx2_config *config = DRV_CONFIG(dev);
...
#define BBRAM_INIT(inst)                                                                           \
	...
	DEVICE_DT_INST_DEFINE(inst, bbram_it8xxx2_init, NULL, &bbram_data_##inst,                  \
			      &bbram_cfg_##inst, PRE_KERNEL_1, CONFIG_BBRAM_INIT_PRIORITY,         \
