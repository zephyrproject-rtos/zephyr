					 ...
					 uint16_t length)
{
	 ...
	 *
	 * When the buffer is word aligned, fetch the word-aligned bulk
	 * directly (a single call for the common multiple-of-4 case) and
	 * copy only the trailing 1-3 bytes through a bounce word. When the
	 * buffer is not word aligned the SDK's 32-bit stores could fault,
	 * so fall back to fetching each word into an aligned bounce
	 * variable and copying the requested bytes. In every case we never
	 * write past buffer[length - 1].
	 */
	if (((uintptr_t)buffer % sizeof(uint32_t)) == 0U) {
		uint16_t aligned = length & ~0x3U;

		if (aligned > 0U) {
			status = TRNG_GetRandomData(config->base, buffer,
						    aligned);
		...
		if (length > 0U) {
			...
			status = TRNG_GetRandomData(config->base, &word,
						    sizeof(word));
			if (status != kStatus_Success) {
				return -EIO;
			}
