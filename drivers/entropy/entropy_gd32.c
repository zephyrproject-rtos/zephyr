...
static int entropy_gd32_recover(void)
{
	/*
...
static int entropy_gd32_wait_drdy(bool allow_yield)
{
	...
	while (SET != trng_flag_get(TRNG_FLAG_DRDY)) {
		...
		if (allow_yield && !k_is_pre_kernel()) {
			k_yield();
		}
	}
...
static int entropy_gd32_fetch_busywait(uint8_t *dst, uint16_t len, bool allow_yield)
{
	while (len > 0U) {
		int ret = entropy_gd32_wait_drdy(allow_yield);
...
static int entropy_gd32_get_entropy(const struct device *dev, uint8_t *buffer, uint16_t length)
{
	...
	return entropy_gd32_fetch_busywait(buffer, length, true);
}
					...
					uint32_t flags)
{
	...
	int ret = entropy_gd32_fetch_busywait(buffer, length, false);
