...
{
	return atomic_cas(&data->modem_usage, 0, 1);
}
...
static bool modem_release(struct sx12xx_data *data)
{
	...
	if (atomic_cas(&data->modem_usage, 1, 2)) {
			      ...
			      int8_t snr)
{
	...
	/* Manually release the modem here
	 * as we need to perform protected operations.
	 */
	if (atomic_cas(&dev_data.modem_usage, 1, 2)) {
		/* We can make two observations here:
		 *  1. lora_recv hasn't already exited due to a timeout.
		 *  2. If lora_recv times out before we raise the signal,
		 *     but while this code is running, it will block on the
		 *     signal again.
		 * This lets us guarantee that the pointers in rx_params are
		 * always valid in this function.
		 */
