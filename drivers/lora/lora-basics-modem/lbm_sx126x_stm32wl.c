...
static void stm32wl_dio1_irq_enable(const struct device *dev, bool enable)
{
	ARG_UNUSED(dev);
...
static void stm32wl_get_pa_cfg(const struct device *dev, int16_t power,
			       ral_sx126x_bsp_tx_cfg_output_params_t *output_params)
{
	const struct stm32wl_config *config = dev->config;
...
static const struct lbm_sx126x_core_ops stm32wl_ops = {
	.irq_init = stm32wl_variant_init,
	.reset = stm32wl_reset,
	.is_busy = stm32wl_is_busy,
	.get_pa_cfg = stm32wl_get_pa_cfg,
	.get_ocp = stm32wl_get_ocp,
	.dio1_irq_enable = stm32wl_dio1_irq_enable,
};

#define STM32WL_DEFINE(node_id)                                                                    \
	static const struct stm32wl_config config_##node_id = {                                    \
		.common = {LBM_SX126X_CONFIG_COMMON(node_id, &stm32wl_ops)},                       \
