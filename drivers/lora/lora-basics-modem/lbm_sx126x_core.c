			       ...
			       ral_sx126x_bsp_tx_cfg_output_params_t *output_params)
{
	...
	config->ops->get_tx_cfg(dev, input_params->system_output_pwr_in_dbm, output_params);
}
...
void lbm_driver_dio1_irq_enable(const struct device *dev)
{
	...
	config->ops->dio1_irq_set(dev, true);
}
...
void lbm_driver_dio1_irq_disable(const struct device *dev)
{
	...
	config->ops->dio1_irq_set(dev, false);
}
...
int lbm_driver_radio_init(const struct device *dev)
{
	...
	ret = config->ops->variant_init(dev);
	if (ret < 0) {
		return ret;
	}
