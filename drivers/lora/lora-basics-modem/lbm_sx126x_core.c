...
{
	const struct lbm_sx126x_core_config *config = dev->config;
	k_timepoint_t expiry = sys_timepoint_calc(timeout);

	do {
		if (!config->ops->is_busy(dev)) {
			return 0;
		}
		k_sleep(K_MSEC(1));
				     ...
				     const uint16_t data_length)
{
	...
	const struct lbm_sx126x_core_config *config = dev->config;
	struct lbm_sx126x_core_data *dev_data = dev->data;
	const struct spi_buf tx_bufs[] = {
		{
			.buf = (void *)command,
			.len = command_length,
			       ...
			       ral_sx126x_bsp_tx_cfg_output_params_t *output_params)
{
	...
	config->ops->get_pa_cfg(dev, input_params->system_output_pwr_in_dbm, output_params);
}
...
void ral_sx126x_bsp_get_ocp_value(const void *context, uint8_t *ocp_in_step_of_2_5_ma)
{
	...
	if (config->ops->get_ocp != NULL) {
		config->ops->get_ocp(dev, ocp_in_step_of_2_5_ma);
	}
}
...
void lbm_driver_dio1_irq_enable(const struct device *dev)
{
	...
	config->ops->dio1_irq_enable(dev, true);
}
...
void lbm_driver_dio1_irq_disable(const struct device *dev)
{
	...
	config->ops->dio1_irq_enable(dev, false);
}
				      ...
				      gpio_callback_handler_t handler)
{
	...
	if (config->ops->dio1_callback_add == NULL) {
		return -ENOTSUP;
	}

	return config->ops->dio1_callback_add(dev, callback, handler);
}
...
int lbm_driver_remove_dio1_gpio_callback(const struct device *dev, struct gpio_callback *callback)
{
	...
	if (config->ops->dio1_callback_remove == NULL) {
		return -ENOTSUP;
	}

	return config->ops->dio1_callback_remove(dev, callback);
}
...
int lbm_sx126x_core_init(const struct device *dev)
{
	 ...
	 */
	if (config->ops->pins_init != NULL) {
		ret = config->ops->pins_init(dev);
		if (ret < 0) {
			return ret;
		}
	}
