...
int lbm_lora_common_init(const struct device *dev)
{
	...
	status = ral_set_dio_irq_params(&config->ralf.ral,
					RAL_IRQ_TX_DONE | RAL_IRQ_RX_DONE | RAL_IRQ_RX_HDR_ERROR |
						RAL_IRQ_RX_CRC_ERROR | RAL_IRQ_CAD_DONE |
						RAL_IRQ_CAD_OK);
	if (status != RAL_STATUS_OK) {
