...
static int sx126x_ensure_device_ready(const struct device *dev, k_timeout_t timeout)
{
	...
	ret = sx126x_wait_device_ready(dev, K_MSEC(5000));
	data->asleep = false;
	return ret;
}
...
static int sx126x_init(const struct device *dev)
{
	...
	/* Enable interrupts */
	gpio_pin_interrupt_configure_dt(&config->dio1, GPIO_INT_ENABLE);
...
#define SX126X_DEFINE(node_id, sx_variant)                                                         \
	static const struct lbm_sx126x_config config_##node_id = {                                 \
		...
		.dio3_tcxo_startup_delay_ms = DT_PROP_OR(node_id, dio3_tcxo_startup_delay_ms, 0),  \
