 ...
 */
#define SX1272_PA_RFO				0
#define SX1272_PA_BOOST				1

#if DT_INST_NODE_HAS_PROP(0, rfo_enable_gpios) &&	\
	DT_INST_NODE_HAS_PROP(0, pa_boost_enable_gpios)
#define SX1272_PA_OUTPUT(power)				\
	((power) > 14 ? SX1272_PA_BOOST : SX1272_PA_RFO)
#elif DT_INST_NODE_HAS_PROP(0, rfo_enable_gpios)
#define SX1272_PA_OUTPUT(power)		SX1272_PA_RFO
#elif DT_INST_NODE_HAS_PROP(0, pa_boost_enable_gpios)
#define SX1272_PA_OUTPUT(power)		SX1272_PA_BOOST
...
BUILD_ASSERT(0, "None of rfo-enable-gpios, pa-boost-enable-gpios and "
	     ...
	     "Look at semtech,sx1272.yaml to fix that.");
#endif

#define SX1272_REG_PA_CONFIG			0x09
#define SX1272_REG_PA_DAC			0x4d
#define SX1272_REG_VERSION			0x42

#define SX1272_PA_CONFIG_MAX_POWER_SHIFT	4
...
#define SX1272_DIO_GPIO_ELEM(idx, inst) \
	{ \
...
#define SX1272_DIO_GPIO_INIT(n) \
	UTIL_LISTIFY(DT_INST_PROP_LEN(n, dio_gpios), SX1272_DIO_GPIO_ELEM, n)

static const struct sx1272_dio sx1272_dios[] = { SX1272_DIO_GPIO_INIT(0) };

#define SX1272_MAX_DIO ARRAY_SIZE(sx1272_dios)

static struct sx1272_data {
	...
	const struct device *dio_dev[SX1272_MAX_DIO];
	struct k_work dio_work[SX1272_MAX_DIO];
} dev_data;
...
static inline void sx1272_antenna_enable(int val)
{
#if DT_INST_NODE_HAS_PROP(0, antenna_enable_gpios)
...
static inline void sx1272_rfi_enable(int val)
{
#if DT_INST_NODE_HAS_PROP(0, rfi_enable_gpios)
...
static inline void sx1272_rfo_enable(int val)
{
#if DT_INST_NODE_HAS_PROP(0, rfo_enable_gpios)
...
static inline void sx1272_pa_boost_enable(int val)
{
#if DT_INST_NODE_HAS_PROP(0, pa_boost_enable_gpios)
...
void SX1272SetAntSwLowPower(bool low_power)
{
	if (low_power) {
		/* force inactive (low power) state of all antenna paths */
		sx1272_rfi_enable(0);
		sx1272_rfo_enable(0);
		sx1272_pa_boost_enable(0);

		sx1272_antenna_enable(0);
	} else {
		sx1272_antenna_enable(1);

		/* rely on SX1272SetAntSw() to configure proper antenna path */
	}
}
...
void SX1272SetAntSw(uint8_t opMode)
{
	...
	case RFLR_OPMODE_TRANSMITTER:
		...
		if (SX1272_PA_OUTPUT(dev_data.tx_power) == SX1272_PA_BOOST) {
			sx1272_rfo_enable(0);
			sx1272_pa_boost_enable(1);
		} else {
			sx1272_pa_boost_enable(0);
			sx1272_rfo_enable(1);
		}
...
static void sx1272_dio_work_handle(struct k_work *work)
{
...
static void sx1272_irq_callback(const struct device *dev,
				struct gpio_callback *cb, uint32_t pins)
{
	...
	for (i = 0; i < SX1272_MAX_DIO; i++) {
		if (dev == dev_data.dio_dev[i] &&
		    pin == sx1272_dios[i].pin) {
...
{
	unsigned int i;
	static struct gpio_callback callbacks[SX1272_MAX_DIO];
	...
	for (i = 0; i < SX1272_MAX_DIO; i++) {
		...
		dev_data.dio_dev[i] = device_get_binding(sx1272_dios[i].port);
		...
		k_work_init(&dev_data.dio_work[i], sx1272_dio_work_handle);

		gpio_pin_configure(dev_data.dio_dev[i], sx1272_dios[i].pin,
				   GPIO_INPUT | GPIO_INT_DEBOUNCE
				   | sx1272_dios[i].flags);

		gpio_init_callback(&callbacks[i],
				   sx1272_irq_callback,
				   BIT(sx1272_dios[i].pin));
...
static int sx1272_transceive(uint8_t reg, bool write, void *data, size_t length)
{
...
int sx1272_read(uint8_t reg_addr, uint8_t *data, uint8_t len)
{
	return sx1272_transceive(reg_addr, false, data, len);
}

int sx1272_write(uint8_t reg_addr, uint8_t *data, uint8_t len)
{
	return sx1272_transceive(reg_addr | BIT(7), true, data, len);
}
...
void SX1272WriteBuffer(uint16_t addr, uint8_t *buffer, uint8_t size)
{
	...
	ret = sx1272_write(addr, buffer, size);
	if (ret < 0) {
...
void SX1272ReadBuffer(uint16_t addr, uint8_t *buffer, uint8_t size)
{
	...
	ret = sx1272_read(addr, buffer, size);
	if (ret < 0) {
...
void SX1272SetRfTxPower(int8_t power)
{
	...
	ret = sx1272_read(SX1272_REG_PA_DAC, &pa_dac, 1);
	if (ret < 0) {
	...
	if (SX1272_PA_OUTPUT(power) == SX1272_PA_BOOST) {
		...
		if (power > 0) {
			...
			pa_config |= 7 << SX1272_PA_CONFIG_MAX_POWER_SHIFT;
	...
	ret = sx1272_write(SX1272_REG_PA_CONFIG, &pa_config, 1);
	if (ret < 0) {
		LOG_ERR("Unable to write PA config");
		return;
	}

	ret = sx1272_write(SX1272_REG_PA_DAC, &pa_dac, 1);
	if (ret < 0) {
...
const struct Radio_s Radio = {
	.Init = SX1272Init,
	.GetStatus = SX1272GetStatus,
	.SetModem = SX1272SetModem,
	.SetChannel = SX1272SetChannel,
	.IsChannelFree = SX1272IsChannelFree,
	.Random = SX1272Random,
	.SetRxConfig = SX1272SetRxConfig,
	.SetTxConfig = SX1272SetTxConfig,
	.CheckRfFrequency = SX1272CheckRfFrequency,
	.TimeOnAir = SX1272GetTimeOnAir,
	.Send = SX1272Send,
	.Sleep = SX1272SetSleep,
	.Standby = SX1272SetStby,
	.Rx = SX1272SetRx,
	.Write = SX1272Write,
	.Read = SX1272Read,
	.WriteBuffer = SX1272WriteBuffer,
	.ReadBuffer = SX1272ReadBuffer,
	.SetMaxPayloadLength = SX1272SetMaxPayloadLength,
	.SetPublicNetwork = SX1272SetPublicNetwork,
	.GetWakeupTime = SX1272GetWakeupTime,
	.IrqProcess = NULL,
	.RxBoosted = NULL,
	.SetRxDutyCycle = NULL,
	.SetTxContinuousWave = SX1272SetTxContinuousWave,
};

static int sx1272_antenna_configure(void)
{
	int ret;
...
static int sx1272_lora_init(const struct device *dev)
{
	...
	ret = sx1272_read(SX1272_REG_VERSION, &regval, 1);
	if (ret < 0) {
		LOG_ERR("Unable to read version info");
		return -EIO;
	}

	ret = sx1272_antenna_configure();
	if (ret < 0) {
		LOG_ERR("Unable to configure antenna");
		return -EIO;
	}

	LOG_INF("SX1272 Version:%02x found", regval);
...
static const struct lora_driver_api sx1272_lora_api = {
...
DEVICE_AND_API_INIT(sx1272_lora, DT_INST_LABEL(0),
		    &sx1272_lora_init, NULL,
