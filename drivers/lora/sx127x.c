...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(semtech_sx1272) + DT_NUM_INST_STATUS_OKAY(semtech_sx1276) <= 1,
...
#define SX127X_PA_OUTPUT(power) ((power) > 14 ? SX127X_PA_BOOST : SX127X_PA_RFO)
...
#define SX127X_PA_OUTPUT(power) DT_INST_ENUM_IDX(0, power_amplifier_output)
#else
...
#define SX127X_DIO_GPIO_LEN(inst) DT_INST_PROP_LEN(inst, dio_gpios)

#define SX127X_DIO_GPIO_ELEM(idx, inst) GPIO_DT_SPEC_INST_GET_BY_IDX(inst, dio_gpios, idx)

#define SX127X_DIO_GPIO_INIT(n) LISTIFY(SX127X_DIO_GPIO_LEN(n), SX127X_DIO_GPIO_ELEM, (,), n)
...
static void sx127x_irq_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	unsigned int i, pin;
	...
	for (i = 0; i < SX127X_MAX_DIO; i++) {
		if (dev == sx127x_dios[i].port && pin == sx127x_dios[i].pin) {
...
void SX127xIoIrqInit(DioIrqHandler **irqHandlers)
{
	...
	for (i = 0; i < SX127X_MAX_DIO; i++) {
		...
		if (!gpio_is_ready_dt(&sx127x_dios[i])) {
			LOG_ERR("GPIO port %s not ready", sx127x_dios[i].port->name);
			return;
		}
		...
		gpio_init_callback(&callbacks[i], sx127x_irq_callback, BIT(sx127x_dios[i].pin));

		if (gpio_add_callback(sx127x_dios[i].port, &callbacks[i]) < 0) {
			LOG_ERR("Could not set gpio callback.");
			return;
		}
		gpio_pin_interrupt_configure_dt(&sx127x_dios[i], GPIO_INT_EDGE_TO_ACTIVE);
	}
}
...
DEVICE_DT_INST_DEFINE(0, &sx127x_lora_init, NULL, NULL, NULL, POST_KERNEL,
