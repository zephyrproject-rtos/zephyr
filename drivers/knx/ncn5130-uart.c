...
#define NCN_TRACE_WORD(w, ok)                                                  \
	LOG_DBG("rx %03x octet=%02x p9=%u calc=%u %s bus=%d ncn=%d",           \
		(unsigned)(w), (unsigned)((w) & 0xFFu),                        \
		(unsigned)(((w) >> 8) & 1u),                                   \
		(unsigned)knx_parity8((uint8_t)((w) & 0xFFu)),                 \
		(ok) ? "OK" : "FLAGGED", (int)atomic_get(&s_bus_state),        \
		(int)atomic_get(&s_ncn_state))
#else
#define NCN_TRACE_WORD(w, ok) do { } while (0)
#endif
...
static void stats_timer_cb(struct k_timer *timer)
{
	...
	LOG_INF("ncn5130 rx stats: words=%u parity_err=%u rx_stopped=%u "
		...
		(unsigned)s_rx_words, (unsigned)s_rx_parity_errors,
		(unsigned)s_rx_stopped_events, (unsigned)s_reset_ind_ignored,
...
static void receive_word(uint16_t word)
{
	...
	if (!s_rx_health_reported && s_rx_words >= NCN_RX_HEALTH_SAMPLE) {
		...
		if (s_rx_parity_errors * 4u >= s_rx_words) {
			LOG_ERR("9th-bit mismatch on %u of the first %u octets. The 9th "
				...
				"9-bit mode requires.",
				(unsigned)s_rx_parity_errors, (unsigned)s_rx_words);
		} else {
	...
	if (!parity_ok) {
		 ...
		 */
		if (bus_state == BUS_FREE &&
		    (byte & CTRL_FRAME_MASK) == CTRL_FRAME_VALUE) {
	...
	if (s_rx_parse_state == NCN_RX_SYSSTAT) {
		...
		LOG_DBG("U_SystemStat.ind status=0x%02x v20v=%d vdd2=%d vbus=%d vfilt=%d xtal=%d tw=%d mode=%u",
			byte,
			(byte & 0x80u) ? 1 : 0,
			(byte & 0x40u) ? 1 : 0,
			(byte & 0x20u) ? 1 : 0,
	...
	if (atomic_get(&s_bus_state) == BUS_TX_IN_PROGRESS) {
		...
		s_tx_stray_bytes++;
		if ((byte & 0x07u) == 0x07u) {
			LOG_DBG("TX window: U_State.ind-shaped stray 0x%02x%s%s%s%s%s",
				byte,
	...
	if ((byte & 0x07u) == 0x07u) {
		LOG_WRN("U_State.ind 0x%02x%s%s%s%s", byte,
			(byte & 0x40u) ? " rx_err"    : "",
			(byte & 0x20u) ? " tx_err"    : "",
			(byte & 0x10u) ? " proto_err" : "",
...
static void uart_cb(const struct device *dev, struct uart_event *evt, void *user_data)
{
	...
	case UART_RX_RDY: {
		...
		const uint16_t *words =
			(const uint16_t *)(evt->data.rx.buf + evt->data.rx.offset);
	...
	case UART_RX_BUF_REQUEST:
		...
		uart_rx_buf_rsp_u16(dev, s_rx_buf[s_rx_buf_idx],
				    ARRAY_SIZE(s_rx_buf[0]));
		break;
	...
	case UART_RX_STOPPED:
		...
		LOG_WRN("UART_RX_STOPPED reason=0x%x (total %u) — re-arming RX",
			evt->data.rx_stop.reason, (unsigned)s_rx_stopped_events);
		...
		s_rx_parse_state = NCN_RX_IDLE;
		s_rx_buf_idx = 0u;
		if (uart_rx_enable_u16(dev, s_rx_buf[0], ARRAY_SIZE(s_rx_buf[0]),
				       SYS_FOREVER_US) != 0) {
...
int knx_l1_send_frame(const uint8_t *frame, uint16_t len)
{
	...
	if (atomic_get(&s_ncn_state) != NCN_STATE_NORMAL) {
		LOG_WRN("knx_l1_send_frame: NCN5130 not in NORMAL state (%d)",
			(int)atomic_get(&s_ncn_state));
		return -EBUSY;
	}
	...
	if (raw_len > 64u) {
		LOG_ERR("knx_l1_send_frame: %u data bytes exceed the 64-byte block this "
	...
	if (atomic_get(&s_tx_echo_remaining) != 0 || s_tx_stray_bytes != 0u) {
		LOG_WRN("TX echo mismatch: len=%u echo_left=%u stray=%u con=%d",
			len, (unsigned)atomic_get(&s_tx_echo_remaining),
			(unsigned)s_tx_stray_bytes, con);
	} else {
...
static int ncn5130_init(const struct device *dev)
{
	...
	/* Length in WORDS for the _u16 variant. */
	ret = uart_rx_enable_u16(s_uart_dev, s_rx_buf[0], ARRAY_SIZE(s_rx_buf[0]),
				 SYS_FOREVER_US);
	if (ret != 0) {
...
DEVICE_DT_INST_DEFINE(0, ncn5130_init, NULL, NULL, &ncn5130_cfg_0,
