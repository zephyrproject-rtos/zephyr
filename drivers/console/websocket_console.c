...
#define SYS_LOG_LEVEL CONFIG_SYS_LOG_WS_CONSOLE_LEVEL
...
#define WS_CONSOLE_STACK_SIZE	CONFIG_WS_CONSOLE_STACK_SIZE
#define WS_CONSOLE_PRIORITY	CONFIG_WS_CONSOLE_PRIO
#define WS_CONSOLE_TIMEOUT	K_MSEC(CONFIG_WS_CONSOLE_SEND_TIMEOUT)
#define WS_CONSOLE_LINES	CONFIG_WS_CONSOLE_LINE_BUF_NUMBERS
#define WS_CONSOLE_LINE_SIZE	CONFIG_WS_CONSOLE_LINE_BUF_SIZE
#define WS_CONSOLE_TIMEOUT	K_MSEC(CONFIG_WS_CONSOLE_SEND_TIMEOUT)
#define WS_CONSOLE_THRESHOLD	CONFIG_WS_CONSOLE_SEND_THRESHOLD
 ...
 * CONFIG_WS_CONSOLE_LINE_BUF_NUMBERS if possible.
 */
struct line_buf {
...
static struct ws_ctx *ws_console;
...
static void ws_end_client_connection(struct ws_ctx *console)
{
	...
	ws_send_data(console, NULL, WS_OPCODE_CLOSE, false, true,
		     NULL);

	ws_rb_init();
}

static int ws_setup_out_pkt(struct ws_ctx *console)
{
	out_pkt = net_app_get_net_pkt(&console->app_ctx, AF_UNSPEC,
				      K_FOREVER);
	if (!out_pkt) {
...
#ifdef CONFIG_WS_CONSOLE_DEBUG_DEEP
...
static bool ws_console_send(struct ws_ctx *console)
{
	...
	if (lb) {
		...
		ret = ws_send_data(console, out_pkt, WS_OPCODE_DATA_TEXT,
				   false, true, NULL);
		if (ret < 0) {
 ...
 */
SYS_INIT(ws_console_init, APPLICATION, CONFIG_WS_CONSOLE_INIT_PRIORITY);
