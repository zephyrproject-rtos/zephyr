...
static inline int z_vrfy_can_add_rx_msgq(const struct device *dev,
					 struct k_msgq *msgq,
					 const struct zcan_filter *filter)
{
	...
	return z_impl_can_add_rx_msgq((const struct device *)dev,
				      (struct k_msgq *)msgq,
				      (const struct zcan_filter *)filter);
}
#include <syscalls/can_add_rx_msgq_mrsh.c>
