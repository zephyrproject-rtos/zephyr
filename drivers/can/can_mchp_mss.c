...
static void mss_can_stop_command(uintptr_t reg_base)
{
...
static void mss_can_stop_command_and_disable_interrupts(uintptr_t reg_base)
{
	mss_can_stop_command(reg_base);
...
static int mss_can_reset(const struct device *dev)
{
	...
	mss_can_stop_command_and_disable_interrupts(reg_base);
			...
			k_timeout_t timeout, can_tx_callback_t callback, void *user_data)
{
	...
	if (likely(data->common.started)) {
		 ...
		 */
		const uint32_t tx_used = data->tx_used;
		const uint32_t index = find_msb_set(~tx_used) - 1;
		const uintptr_t tx_msg_base = reg_base + MSS_CAN_TX_MSG(index);
				 ...
				 void *user_data, const struct can_filter *filter)
{
	...
	do {
		const uintptr_t rx_msg_base = reg_base + MSS_CAN_RX_MSG(index);
		const uint32_t rx_msg_ctrl_cmd = sys_read32(rx_msg_base + MSS_CAN_RX_MSG_CTRL_CMD);
...
static int mss_can_stop(const struct device *dev)
{
	...
	if (data->common.started) {
		...
		mss_can_stop_command_and_disable_interrupts(cfg->reg_base);
		irq_disable(cfg->irq);
...
static int mss_can_recover(const struct device *dev, k_timeout_t timeout)
{
	...
	if (!data->common.started) {
		...
		mss_can_stop_command(reg_base);
		mss_can_write_config(reg_base, mode);
		mss_can_write_command(reg_base, mode);
