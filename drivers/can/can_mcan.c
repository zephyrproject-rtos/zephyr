				 ...
				 struct can_timing *timing_data)
{
	if (timing) {
		...
		__ASSERT_NO_MSG(timing->prescaler <= 0x200 &&
				timing->prescaler > 0);
		can->nbtp = (((u32_t)timing->phase_seg1 - 1UL) & 0xFF) <<
				CAN_MCAN_NBTP_NTSEG1_POS |
			    (((u32_t)timing->phase_seg2 - 1UL) & 0x7F) <<
				CAN_MCAN_NBTP_NTSEG2_POS |
			    (((u32_t)timing->sjw        - 1UL) & 0x7F) <<
				CAN_MCAN_NBTP_NSJW_POS   |
			    (((u32_t)timing->prescaler  - 1UL) & 0x1FF) <<
	...
	if (timing_data) {
		...
		can->dbtp = (((u32_t)timing_data->phase_seg1 - 1UL) & 0x1F) <<
				CAN_MCAN_DBTP_DTSEG1_POS |
			    (((u32_t)timing_data->phase_seg2 - 1UL) & 0x0F)  <<
				CAN_MCAN_DBTP_DTSEG2_POS |
			    (((u32_t)timing_data->sjw        - 1UL) & 0x0F)  <<
				CAN_MCAN_DBTP_DSJW_POS   |
			    (((u32_t)timing_data->prescaler  - 1UL) & 0x1F) <<
		  ...
		  struct can_mcan_data *data)
{
	...
	can->sidfc = ((u32_t)msg_ram->std_filt & CAN_MCAN_SIDFC_FLSSA_MSK) |
		     (ARRAY_SIZE(msg_ram->std_filt) << CAN_MCAN_SIDFC_LSS_POS);
	can->xidfc = ((u32_t)msg_ram->ext_filt & CAN_MCAN_XIDFC_FLESA_MSK) |
		     (ARRAY_SIZE(msg_ram->ext_filt) << CAN_MCAN_XIDFC_LSS_POS);
	can->rxf0c = ((u32_t)msg_ram->rx_fifo0 & CAN_MCAN_RXF0C_F0SA) |
		     (ARRAY_SIZE(msg_ram->rx_fifo0) << CAN_MCAN_RXF0C_F0S_POS);
	can->rxf1c = ((u32_t)msg_ram->rx_fifo1 & CAN_MCAN_RXF1C_F1SA) |
		     (ARRAY_SIZE(msg_ram->rx_fifo1) << CAN_MCAN_RXF1C_F1S_POS);
	can->rxbc = ((u32_t)msg_ram->rx_buffer & CAN_MCAN_RXBC_RBSA);
	can->txefc = ((u32_t)msg_ram->tx_event_fifo & CAN_MCAN_TXEFC_EFSA_MSK) |
		     (ARRAY_SIZE(msg_ram->tx_event_fifo) <<
		     CAN_MCAN_TXEFC_EFS_POS);
	can->txbc = ((u32_t)msg_ram->tx_fifo & CAN_MCAN_TXBC_TBSA) |
	...
	for (u32_t *ptr = (u32_t *)msg_ram;
	     ptr < (u32_t *)msg_ram +
		   sizeof(struct can_mcan_msg_sram) / sizeof(u32_t);
	     ptr++) {
		*ptr = 0;
	}
...
static void can_mcan_get_message(struct can_mcan_data *data,
				 ...
				 volatile u32_t *fifo_status_reg,
				 volatile u32_t *fifo_ack_reg)
{
	u32_t get_idx, filt_idx;
	...
	while ((*fifo_status_reg & CAN_MCAN_RXF0S_F0FL)) {
		...
		if (hdr.xtd) {
			frame.ext_id = hdr.ext_id;
		} else {
			frame.std_id = hdr.std_id;
		}
		frame.fd = hdr.fdf;
		...
		if (data_length <= sizeof(frame.data)) {
			...
			for (src = fifo[get_idx].data_32,
			     dst = frame.data_32,
			     end = dst + CAN_DIV_CEIL(data_length, sizeof(u32_t));
			     dst < end;
			     src++, dst++) {
				*dst = *src;
			}
		  ...
		  can_tx_callback_t callback, void *callback_arg)
{
	...
	if (frame->id_type == CAN_STANDARD_IDENTIFIER) {
		tx_hdr.std_id = frame->std_id;
	} else {
		tx_hdr.ext_id = frame->ext_id;
	}
