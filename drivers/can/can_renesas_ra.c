...
static int can_renesas_ra_start(const struct device *dev)
{
	...
	struct st_canfd_instance_ctrl *p_ctrl = data->fsp_can.p_ctrl;
	struct st_can_cfg *p_cfg = (struct st_can_cfg *)data->fsp_can.p_cfg;
	struct st_canfd_extended_cfg *p_extend = (struct st_canfd_extended_cfg *)p_cfg->p_extend;
	int ret = 0;
	...
	if ((p_ctrl->open != 0) && (FSP_SUCCESS != can_api->close(p_ctrl))) {
		LOG_DBG("CAN close failed");
		ret = -EIO;
		goto end;
	}

	if (FSP_SUCCESS != can_api->open(p_ctrl, p_cfg)) {
	...
	if ((data->common.mode & CAN_MODE_LOOPBACK) != 0) {
		if (FSP_SUCCESS !=
		    can_api->modeTransition(p_ctrl, CAN_OPERATION_MODE_NORMAL,
					    (transceiver_dev != NULL
