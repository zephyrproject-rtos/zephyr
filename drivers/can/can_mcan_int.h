...
#ifdef CONFIG_CAN_STM32FD
struct can_mcan_reg {
	volatile u32_t crel;     /* Core Release Register */
	volatile u32_t endn;     /* Endian Register */
	volatile u32_t cust;     /* Customer Register */
	volatile u32_t dbtp;     /* Data Bit Timing & Prescaler Register */
	volatile u32_t test;     /* Test Register */
	volatile u32_t rwd;      /* RAM Watchdog */
	volatile u32_t cccr;     /* CC Control Register */
	volatile u32_t nbtp;     /* Nominal Bit Timing & Prescaler Register */
	volatile u32_t tscc;     /* Timestamp Counter Configuration */
	volatile u32_t tscv;     /* Timestamp Counter Value */
	volatile u32_t tocc;     /* Timeout Counter Configuration */
	volatile u32_t tocv;     /* Timeout Counter Value */
	u32_t res1[4];           /* Reserved (4) */
	volatile u32_t ecr;      /* Error Counter Register */
	volatile u32_t psr;      /* Protocol Status Register */
	volatile u32_t tdcr;     /* Transmitter Delay Compensation */
	u32_t res2;              /* Reserved (1) */
	volatile u32_t ir;       /* Interrupt Register */
	volatile u32_t ie;       /* Interrupt Enable */
	volatile u32_t ils;      /* Interrupt Line Select */
	volatile u32_t ile;      /* Interrupt Line Enable */
	u32_t res3[8];           /* Reserved (8) */
	volatile u32_t rxgfc;    /* Global Filter Configuration */
	volatile u32_t xidam;    /* Extended ID AND Mask */
	volatile u32_t hpms;     /* High Priority Message Status */
	u32_t res4;              /* Reserved (1) */
	volatile u32_t rxf0s;    /* Rx FIFO 0 Status */
	volatile u32_t rxf0a;    /* Rx FIFO 0 Acknowledge */
	volatile u32_t rxf1s;    /* Rx FIFO 1 Status */
	volatile u32_t rxf1a;    /* Rx FIFO 1 Acknowledge */
	u32_t res5[8];           /* Reserved (8) */
	volatile u32_t txbc;     /* Tx Buffer Configuration */
	volatile u32_t txfqs;    /* Tx FIFO/Queue Status */
	volatile u32_t txbrp;    /* Tx Buffer Request Pending */
	volatile u32_t txbar;    /* Tx Buffer Add Request */
	volatile u32_t txbcr;    /* Tx Buffer Cancellation */
	volatile u32_t txbto;    /* Tx Buffer Transmission */
	volatile u32_t txbcf;    /* Tx Buffer Cancellation Finished */
	volatile u32_t txbtie;   /* Tx Buffer Transmission Interrupt Enable */
	volatile u32_t txcbie;   /* Tx Buffer Cancellation Fi.Interrupt En. */
	volatile u32_t txefs;    /* Tx Event FIFO Status */
	volatile u32_t txefa;    /* Tx Event FIFO Acknowledge */
};
...
struct can_mcan_reg {
	...
	volatile u32_t ile;      /* Interrupt Line Enable */
	u32_t res3[8];           /* Reserved (8) */
	volatile u32_t gfc;      /* Global Filter Configuration */
	volatile u32_t sidfc;    /* Standard ID Filter Configuration */
	volatile u32_t xidfc;    /* Extended ID Filter Configuration */
	volatile u32_t res4;     /* Reserved (1) */
	volatile u32_t xidam;    /* Extended ID AND Mask */
	volatile u32_t hpms;     /* High Priority Message Status */
	volatile u32_t ndata1;   /* New Data 1 */
	volatile u32_t ndata2;   /* New Data 2 */
	volatile u32_t rxf0c;    /* Rx FIFO 0 Configuration */
	volatile u32_t rxf0s;    /* Rx FIFO 0 Status */
	volatile u32_t rxf0a;    /* FIFO 0 Acknowledge */
	volatile u32_t rxbc;     /* Rx Buffer Configuration */
	volatile u32_t rxf1c;    /* Rx FIFO 1 Configuration */
	volatile u32_t rxf1s;    /* Rx FIFO 1 Status */
	volatile u32_t rxf1a;    /* Rx FIFO 1 Acknowledge*/
	volatile u32_t rxesc;    /* Rx Buffer / FIFO Element Size Config */
	volatile u32_t txbc;     /* Buffer Configuration */
	volatile u32_t txfqs;    /* FIFO/Queue Status */
	volatile u32_t txesc;    /* Tx Buffer Element Size Configuration */
	volatile u32_t txbrp;    /* Buffer Request Pending */
	volatile u32_t txbar;    /* Add Request */
	volatile u32_t txbcr;    /* Buffer Cancellation Request */
	volatile u32_t txbto;    /* Tx Buffer Transmission Occurred  */
	volatile u32_t txbcf;    /* Tx Buffer Cancellation Finished */
	volatile u32_t txbtie;   /* Tx Buffer Transmission Interrupt Enable */
	volatile u32_t txbcie;   /* Tx Buffer Cancellation Fin. Interrupt En. */
	volatile u32_t res5[2];  /* Reserved (2) */
	volatile u32_t txefc;    /* Tx Event FIFO Configuration */
	volatile u32_t txefs;    /* Tx Event FIFO Status */
	volatile u32_t txefa;    /* Tx Event FIFO Acknowledge */
};
#endif
