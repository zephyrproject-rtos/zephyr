...
#define DEV_CFG(_dev)  ((const struct xcanfd_cfg *)(_dev)->config)
#define DEV_DATA(_dev) ((struct xcanfd_data *)(_dev)->data)
 ...
 */
static inline uint32_t xcanfd_read32(const struct device *dev, uint32_t offs)
{
 ...
 */
static inline void xcanfd_write32(const struct device *dev, uint32_t offs, uint32_t value)
{
 ...
 */
static void xcanfd_filter_disable(const struct device *dev, uint32_t filter_mask)
{
	uint32_t filters;

	filters = xcanfd_read32(dev, XCANFD_AFR_OFFSET);
	filters &= (~filter_mask);
	xcanfd_write32(dev, XCANFD_AFR_OFFSET, filters);
}

/**
 * @brief Enable specified acceptance filters
 */
static void xcanfd_filter_enable(const struct device *dev, uint32_t filter_mask)
{
	uint32_t filters;
 ...
 */
static int xcanfd_acceptance_filter_set(const struct device *dev, uint32_t filter_index,
					uint32_t mask_value, uint32_t id_value)
{
	...
	enabled_filters = xcanfd_read32(dev, XCANFD_AFR_OFFSET);
	...
	xcanfd_write32(dev, XCANFD_AFMR_ADDR(filter_index), mask_value);
	xcanfd_write32(dev, XCANFD_AFIDR_ADDR(filter_index), id_value);
 ...
 */
static void xcanfd_filter_to_hw_format(const struct can_filter *filter, uint32_t *hw_mask,
				       uint32_t *hw_id)
{
	*hw_id = 0;
	*hw_mask = 0;
	...
	if (filter->flags & CAN_FILTER_IDE) {
		*hw_id |= (filter->id << XCANFD_AFIDR_AIID_EXT_SHIFT) & XCANFD_AFIDR_AIID_EXT_MASK;
		*hw_id |= XCANFD_AFIDR_AIIDE_MASK;
		*hw_mask |=
			(filter->mask << XCANFD_AFMR_AMID_EXT_SHIFT) & XCANFD_AFMR_AMID_EXT_MASK;
 ...
 */
static bool xcanfd_frame_matches_filter(const struct device *dev, const struct can_frame *frame,
					struct xcanfd_filter **matched_filter)
{
	...
	for (int i = 0; i < XCANFD_MAX_FILTERS; i++) {
		...
		masked_filter_id = data->filters[i].filter.id & data->filters[i].filter.mask;
...
static void xcanfd_state_interrupt(const struct device *dev, uint32_t isr)
{
	...
	if (isr & XCANFD_IXR_SLP_MASK) {
		...
		msr = xcanfd_read32(dev, XCANFD_MSR_OFFSET);

		if (!(msr & XCANFD_MSR_SLEEP_MASK)) {
			xcanfd_write32(dev, XCANFD_MSR_OFFSET, msr | XCANFD_MSR_SLEEP_MASK);
		}
		xcanfd_write32(dev, XCANFD_SRR_OFFSET, 0);
	}

	if (isr & XCANFD_IXR_WKUP_MASK) {
		data->state = CAN_STATE_ERROR_ACTIVE;
		xcanfd_write32(dev, XCANFD_SRR_OFFSET, XCANFD_SRR_CEN_MASK);
	}
}
...
static void xcanfd_tx_interrupt(const struct device *dev, uint32_t isr)
{
	...
	xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_TXOK_MASK);

	trr_reg = xcanfd_read32(dev, XCANFD_TRR_OFFSET);
...
/**
 * @brief Parse frame ID from received message
 */
static void xcanfd_parse_frame_id(struct can_frame *frame, uint32_t raw_id_reg)
{
...
/**
 * @brief Parse frame flags from received message
 */
static void xcanfd_parse_frame_flags(struct can_frame *frame, uint32_t raw_id_reg, uint32_t dlc_reg)
{
...
/**
 * @brief Validate DLC value based on frame type
 */
static bool xcanfd_validate_dlc(const struct can_frame *frame)
{
...
/**
 * @brief Read frame data from RX FIFO
 */
static void xcanfd_read_frame_data(const struct device *dev, struct can_frame *frame,
				   uint32_t offset)
{
	...
	if (len > 0 && len <= sizeof(frame->data)) {
		for (int i = 0; i < len; i += 4) {
			reg_data = xcanfd_read32(dev, dw_offset);
...
/**
 * @brief Process a single received CAN frame
 */
static void xcanfd_process_rx_frame(const struct device *dev, uint32_t offset)
{
	struct can_frame frame = {0};
	uint32_t raw_id_reg;
	uint32_t dlc_reg;

	raw_id_reg = xcanfd_read32(dev, XCANFD_FRAME_ID_ADDR(offset));
	dlc_reg = xcanfd_read32(dev, XCANFD_FRAME_DLC_ADDR(offset));

	xcanfd_parse_frame_flags(&frame, raw_id_reg, dlc_reg);
	xcanfd_parse_frame_id(&frame, raw_id_reg);

	frame.dlc = (dlc_reg & XCANFD_DLCR_DLC_MASK) >> XCANFD_DLCR_DLC_SHIFT;

	if (!xcanfd_validate_dlc(&frame)) {
		xcanfd_write32(dev, XCANFD_FSR_OFFSET,
			       xcanfd_read32(dev, XCANFD_FSR_OFFSET) | XCANFD_FSR_IRI_MASK);
		return;
	}

	xcanfd_read_frame_data(dev, &frame, offset);
...
static void xcanfd_rx_interrupt(const struct device *dev, uint32_t isr)
{
	...
	fsr = xcanfd_read32(dev, XCANFD_FSR_OFFSET);
	mask = XCANFD_2_FSR_FL_MASK;

	if (fsr & mask) {
		offset = XCANFD_RXMSG_2_FRAME_ADDR(fsr & XCANFD_2_FSR_RI_MASK);
		xcanfd_process_rx_frame(dev, offset);
	}

	xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_RXOK_MASK);
}

/**
 * @brief Handle arbitration lost or bus-off events for all TX mailboxes.
 */
static void xcanfd_handle_arblost(const struct device *dev, struct xcanfd_data *data)
{
	unsigned int key = irq_lock();

	for (int i = 0; i < XCANFD_MAX_TX_MAILBOXES; i++) {
		if (data->tx_mailboxes[i].in_use && data->tx_mailboxes[i].callback) {
...
/**
 * @brief Get current error state from hardware status register
 */
static enum can_state xcanfd_get_error_state(const struct device *dev)
{
	uint32_t sr = xcanfd_read32(dev, XCANFD_SR_OFFSET);
...
/**
 * @brief Update CAN state and notify application via callback
 *
 * @param dev Pointer to the device structure
 * @param new_state New CAN state to transition to
 *
 * This helper ensures consistent state change handling with proper callback notification
 */
static void xcanfd_update_state(const struct device *dev, enum can_state new_state)
{
	struct xcanfd_data *data = dev->data;
	...
	if (new_state != data->state) {
		...
		if (data->common.state_change_cb) {
			ecr = xcanfd_read32(dev, XCANFD_ECR_OFFSET);
...
/**
 * @brief Handle CAN controller error interrupts and update error statistics.
 */
static void xcanfd_handle_error(const struct device *dev)
{
	enum can_state current_state;
	uint32_t esr = xcanfd_read32(dev, XCANFD_ESR_OFFSET);
	...
	xcanfd_write32(dev, XCANFD_ESR_OFFSET, esr);

	current_state = xcanfd_get_error_state(dev);
	xcanfd_update_state(dev, current_state);
}
...
static void xcanfd_isr(const struct device *dev)
{
	...
	if (data->state == CAN_STATE_STOPPED && !data->common.started) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, 0xFFFFFFFFU);
		return;
	}

	isr = xcanfd_read32(dev, XCANFD_ISR_OFFSET);
	if (!isr) {
		return;
	}

	if (isr & (XCANFD_IXR_SLP_MASK | XCANFD_IXR_WKUP_MASK)) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET,
			       (XCANFD_IXR_SLP_MASK | XCANFD_IXR_WKUP_MASK));
		xcanfd_state_interrupt(dev, isr);
	}

	if (isr & XCANFD_IXR_ARBLST_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_ARBLST_MASK);
		xcanfd_handle_arblost(dev, data);
	}
	...
	if (isr & XCANFD_IXR_ERROR_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_ERROR_MASK);
		xcanfd_handle_error(dev);
	}

	if (isr & XCANFD_IXR_RXOFLW_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_RXOFLW_MASK);
		CAN_STATS_RX_OVERRUN_INC(dev);
	}

	if (isr & XCANFD_IXR_RXFOFLW_1_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_RXFOFLW_1_MASK);
		CAN_STATS_RX_OVERRUN_INC(dev);
	}

	if (isr & XCANFD_IXR_BSOFF_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_BSOFF_MASK);

		xcanfd_update_state(dev, CAN_STATE_BUS_OFF);
		xcanfd_handle_arblost(dev, data);
	}
	...
	if (isr & XCANFD_IXR_RXNEMP_MASK) {
		xcanfd_write32(dev, XCANFD_ICR_OFFSET, XCANFD_IXR_RXNEMP_MASK);
		xcanfd_rx_interrupt(dev, isr);
	}
}
...
static int xcanfd_set_mode(const struct device *dev, can_mode_t mode)
{
	...
	if ((mode & CAN_MODE_FD) != 0) {
		if (IS_ENABLED(CONFIG_CAN_FD_MODE)) {
			brpr = xcanfd_read32(dev, XCANFD_BRPR_OFFSET);
			brpr |= XCANFD_BRPR_TDC_ENABLE_MASK;
			xcanfd_write32(dev, XCANFD_BRPR_OFFSET, brpr);
		} else {
	...
	xcanfd_write32(dev, XCANFD_MSR_OFFSET, msr_reg);
	xcanfd_write32(dev, XCANFD_SRR_OFFSET, XCANFD_SRR_CEN_MASK);
...
static int xcanfd_start(const struct device *dev)
{
	...
	xcanfd_write32(dev, XCANFD_IER_OFFSET, ier);

	xcanfd_filter_disable(dev, XCANFD_AFR_UAF_ALL_MASK);
	if (data->enabled_filters_mask != 0) {
		xcanfd_filter_enable(dev, data->enabled_filters_mask);
	}
...
static int xcanfd_stop(const struct device *dev)
{
	...
	while (k_uptime_delta(&start_time) < XCANFD_TIMEOUT_MS) {
		status_reg = xcanfd_read32(dev, XCANFD_SR_OFFSET);
	...
	if (k_uptime_delta(&start_time) >= XCANFD_TIMEOUT_MS) {
		LOG_ERR("Timeout waiting for configuration mode");
		ret = -ETIMEDOUT;
	}

	xcanfd_write32(dev, XCANFD_IER_OFFSET, 0);
...
static int xcanfd_set_timing_data(const struct device *dev, const struct can_timing *timing)
{
	...
	is_config_mode = xcanfd_read32(dev, XCANFD_SR_OFFSET) & XCANFD_SR_CONFIG_MASK;
	if (!is_config_mode) {
	...
	if (ret == 0) {
		...
		xcanfd_write32(dev, XCANFD_F_BRPR_OFFSET, btr0);
		xcanfd_write32(dev, XCANFD_F_BTR_OFFSET, btr1);
	}
	return 0;
}
 ...
 */
static int xcanfd_set_timing(const struct device *dev, const struct can_timing *timing)
{
	const struct xcanfd_cfg *config = dev->config;
	struct xcanfd_data *data = dev->data;
	...
	if (ret == 0) {
		...
		btr1 = ((calc_timing.prop_seg + calc_timing.phase_seg1 - 1)) & XCANFD_BTR_TS1_MASK;
		btr1 |= (((calc_timing.phase_seg2 - 1) << XCANFD_BTR_TS2_SHIFT) &
			 XCANFD_BTR_TS2_MASK);
		btr1 |= (((calc_timing.sjw - 1) << XCANFD_BTR_SJW_SHIFT) & XCANFD_BTR_SJW_MASK);
		xcanfd_write32(dev, XCANFD_BRPR_OFFSET, btr0);
		xcanfd_write32(dev, XCANFD_BTR_OFFSET, btr1);
	}
	return 0;
}
			    ...
			    struct can_bus_err_cnt *err_cnt)
{
	...
	if (err_cnt) {
		err_cnt->tx_err_cnt =
			xcanfd_read32(dev, XCANFD_ECR_OFFSET) & XCANFD_ECR_TEC_MASK;
		err_cnt->rx_err_cnt =
			((xcanfd_read32(dev, XCANFD_ECR_OFFSET) & XCANFD_ECR_REC_MASK) >>
...
/**
 * @brief Validate CAN frame flags and parameters
 */
static int xcanfd_validate_frame(const struct xcanfd_data *data, const struct can_frame *frame)
{
	if (!frame) {
...
/**
 * @brief Validate DLC for frame type
 */
static int xcanfd_validate_frame_dlc(const struct can_frame *frame)
{
	if (IS_ENABLED(CONFIG_CAN_FD_MODE)) {
		if ((frame->flags & CAN_FRAME_FDF) != 0) {
			if (frame->dlc > CANFD_MAX_DLC) {
...
/**
 * @brief Find available TX mailbox
 */
static int xcanfd_find_free_mailbox(const struct device *dev, struct xcanfd_data *data)
{
	uint32_t trr_reg = xcanfd_read32(dev, XCANFD_TRR_OFFSET);
...
/**
 * @brief Build CAN frame ID register value
 */
static uint32_t xcanfd_build_id_reg(const struct can_frame *frame)
{
	uint32_t id_reg = 0;

	if (frame->flags & CAN_FRAME_IDE) {
		if (frame->id > CAN_EXT_ID_MASK) {
			LOG_ERR("TX: Extended ID 0x%08x exceeds maximum 0x%08x", frame->id,
				CAN_EXT_ID_MASK);
		}
		...
		if (frame->id > CAN_STD_ID_MASK) {
			LOG_ERR("TX: Standard ID 0x%08x exceeds maximum 0x%08x", frame->id,
				CAN_STD_ID_MASK);
		}
...
/**
 * @brief Build CAN frame DLC register value
 */
static uint32_t xcanfd_build_dlc_reg(const struct can_frame *frame)
{
...
/**
 * @brief Write frame data to TX mailbox
 */
static void xcanfd_write_frame_data(const struct device *dev, const struct can_frame *frame,
				    uint32_t frame_offset)
{
	...
	for (uint32_t len = 0; len < nobytes; len += 4) {
		...
		xcanfd_write32(dev,
			       (XCANFD_FRAME_DW_ADDR(frame_offset) + (dwindex * XCANFD_DW_BYTES)),
			       value);
		dwindex++;
	}
}
 ...
 */
static int xcanfd_send(const struct device *dev, const struct can_frame *frame, k_timeout_t timeout,
		       can_tx_callback_t callback, void *user_data)
{
	...
	ret = xcanfd_validate_frame(data, frame);
	if (ret != 0) {
		return ret;
	}

	ret = xcanfd_validate_frame_dlc(frame);
	if (ret != 0) {
		return ret;
	}
	...
	mailbox_id = xcanfd_find_free_mailbox(dev, data);
	if (mailbox_id == -1) {
		k_mutex_unlock(&data->inst_mutex);
		return -EBUSY;
	}
	...
	id_reg = xcanfd_build_id_reg(frame);
	dlc_reg = xcanfd_build_dlc_reg(frame);

	xcanfd_write32(dev, XCANFD_FRAME_ID_ADDR(frame_offset), id_reg);
	xcanfd_write32(dev, XCANFD_FRAME_DLC_ADDR(frame_offset), dlc_reg);

	xcanfd_write_frame_data(dev, frame, frame_offset);

	xcanfd_write32(dev, XCANFD_TRR_OFFSET, BIT(mailbox_id));
 ...
 */
static int xcanfd_add_rx_filter(const struct device *dev, can_rx_callback_t cb, void *cb_arg,
				const struct can_filter *filter)
{
	...
	if (filter->flags & CAN_FILTER_IDE) {
		if (filter->id > CAN_EXT_ID_MASK) {
			LOG_ERR("Extended ID 0x%08x exceeds maximum 0x%08x", filter->id,
				CAN_EXT_ID_MASK);
			return -EINVAL;
		}
		if (filter->mask > CAN_EXT_ID_MASK) {
			LOG_ERR("Extended mask 0x%08x exceeds maximum 0x%08x", filter->mask,
				CAN_EXT_ID_MASK);
			return -EINVAL;
		}
	} else {
		if (filter->id > CAN_STD_ID_MASK) {
			LOG_ERR("Standard ID 0x%03x exceeds maximum 0x%03x", filter->id,
				CAN_STD_ID_MASK);
			return -EINVAL;
		}
		if (filter->mask > CAN_STD_ID_MASK) {
			LOG_ERR("Standard mask 0x%03x exceeds maximum 0x%03x", filter->mask,
				CAN_STD_ID_MASK);
			return -EINVAL;
		}
	}
	...
	ret = xcanfd_acceptance_filter_set(dev, filter_index + 1, hw_mask, hw_id);
	if (ret != 0) {
	...
	xcanfd_filter_enable(dev, BIT(filter_index));
...
static void xcanfd_remove_rx_filter(const struct device *dev, int filter_id)
{
	...
	xcanfd_filter_disable(dev, BIT(filter_id));
...
static int xcanfd_reset(const struct device *dev)
{
	...
	xcanfd_write32(dev, XCANFD_IER_OFFSET, 0);
	xcanfd_write32(dev, XCANFD_ICR_OFFSET, 0xFFFFFFFFU);
	xcanfd_write32(dev, XCANFD_SRR_OFFSET, XCANFD_SRR_RESET_MASK);

	start_time = k_uptime_get();

	while (k_uptime_delta(&start_time) < XCANFD_TIMEOUT_MS) {
		sr_reg = xcanfd_read32(dev, XCANFD_SR_OFFSET);
	...
	if (k_uptime_delta(&start_time) >= XCANFD_TIMEOUT_MS) {
		LOG_ERR("Timeout exceeded while waiting for configuration mode (SR=0x%08x)",
			xcanfd_read32(dev, XCANFD_SR_OFFSET));
		return -ETIMEDOUT;
	}
	xcanfd_write32(dev, XCANFD_SRR_OFFSET, 0);
	data->tx_head = 0;
 ...
 */
static int xcanfd_recover(const struct device *dev, k_timeout_t timeout)
{
	struct xcanfd_data *data = dev->data;
	struct can_bus_err_cnt err_cnt;
	enum can_state current_state;
	...
	xcanfd_get_state(dev, &current_state, &err_cnt);
...
static int xcanfd_init(const struct device *dev)
{
	...
	xcanfd_write32(dev, XCANFD_AFR_OFFSET, 0);

	for (int i = 0; i < XCANFD_MAX_FILTERS; i++) {
		xcanfd_write32(dev, XCANFD_AFMR_ADDR(i), 0);
		xcanfd_write32(dev, XCANFD_AFIDR_ADDR(i), 0);
	}
...
static DEVICE_API(can, xcanfd_driver_api) = {
	...
	.timing_data_min = {.sjw = XCANFD_TIMING_SJW_MIN,
			    .prop_seg = XCANFD_TIMING_PROP_SEG_MIN,
			    .phase_seg1 = XCANFD_TIMING_PHASE_SEG1_MIN,
			    .phase_seg2 = XCANFD_TIMING_PHASE_SEG2_MIN,
			    .prescaler = XCANFD_TIMING_PRESCALER_MIN},
	.timing_data_max = {.sjw = XCANFD_TIMING_SJW_MAX,
			    .prop_seg = XCANFD_TIMING_PROP_SEG_MAX,
			    .phase_seg1 = XCANFD_TIMING_PHASE_SEG1_MAX,
			    .phase_seg2 = XCANFD_TIMING_PHASE_SEG2_MAX,
			    .prescaler = XCANFD_TIMING_PRESCALER_MAX},
...
#define XCANFD_IRQ_CONFIG(n)                                                                       \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ_BY_IDX(n, 0, priority), xcanfd_isr,       \
...
#define xcanfd_INST(n)                                                                             \
	BUILD_ASSERT(DT_INST_REG_ADDR(n) != 0, "Invalid register base address");                   \
	BUILD_ASSERT(DT_INST_PROP_BY_PHANDLE(n, clocks, clock_frequency) != 0,                     \
		     "Invalid CAN core clock frequency");                                          \
	XCANFD_IRQ_CONFIG(n)                                                                       \
	static const struct xcanfd_cfg xcanfd_cfg_##n = {                                          \
		.common = CAN_DT_DRIVER_CONFIG_INST_GET(n, XLNX_CANFD_BUS_SPEED_MIN,               \
							XLNX_CANFD_BUS_SPEED_MAX),                 \
		DEVICE_MMIO_NAMED_ROM_INIT(mmio, DT_DRV_INST(n)),                                 \
		.init_func = xcanfd_config_intr##n,                                                \
		.can_core_clock = DT_INST_PROP_BY_PHANDLE(n, clocks, clock_frequency),             \
	};                                                                                         \
	static struct xcanfd_data xcanfd_data_##n;                                                 \
	CAN_DEVICE_DT_INST_DEFINE(n, xcanfd_init, NULL, &xcanfd_data_##n, &xcanfd_cfg_##n,         \
				  POST_KERNEL, CONFIG_CAN_INIT_PRIORITY, &xcanfd_driver_api);
