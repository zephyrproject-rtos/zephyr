 ...
 */
static int xcanfd_acceptance_filter_set(const struct xcanfd_cfg *config, uint32_t filter_index,
 ...
 */
static void xcanfd_read_frame_data(const struct xcanfd_cfg *config, struct can_frame *frame,
				   uint32_t offset)
{
...
static void xcanfd_process_rx_frame(const struct device *dev, uint32_t offset)
{
	...
	xcanfd_read_frame_data(config, &frame, offset);
 ...
 */
static void xcanfd_handle_error(const struct device *dev, const struct xcanfd_cfg *config)
{
	uint32_t esr = xcanfd_read32(config, XCANFD_ESR_OFFSET);
	...
	xcanfd_write32(config, XCANFD_ESR_OFFSET, esr);
}
...
static void xcanfd_isr(const struct device *dev)
{
	...
	if (data->state == CAN_STATE_STOPPED && !data->common.started) {
		xcanfd_write32(config, XCANFD_ICR_OFFSET, 0xFFFFFFFFU);
		return;
	}
	...
	if (isr & XCANFD_IXR_ERROR_MASK) {
		...
		xcanfd_handle_error(dev, config);
	}

	if (isr & XCANFD_IXR_RXOFLW_MASK) {
		xcanfd_write32(config, XCANFD_ICR_OFFSET, XCANFD_IXR_RXOFLW_MASK);
		CAN_STATS_RX_OVERRUN_INC(dev);
	}

	if (isr & XCANFD_IXR_RXFOFLW_1_MASK) {
		xcanfd_write32(config, XCANFD_ICR_OFFSET, XCANFD_IXR_RXFOFLW_1_MASK);
		CAN_STATS_RX_OVERRUN_INC(dev);
	}
			    ...
			    struct can_bus_err_cnt *err_cnt)
{
	...
	if (err_cnt) {
		err_cnt->tx_err_cnt =
			xcanfd_read32(config, XCANFD_ECR_OFFSET) & XCANFD_ECR_TEC_MASK;
		err_cnt->rx_err_cnt =
			((xcanfd_read32(config, XCANFD_ECR_OFFSET) & XCANFD_ECR_REC_MASK) >>
 ...
 */
static int xcanfd_find_free_mailbox(const struct xcanfd_cfg *config, struct xcanfd_data *data)
{
	uint32_t trr_reg = xcanfd_read32(config, XCANFD_TRR_OFFSET);
 ...
 */
static void xcanfd_write_frame_data(const struct xcanfd_cfg *config, const struct can_frame *frame,
		       ...
		       can_tx_callback_t callback, void *user_data)
{
	...
	mailbox_id = xcanfd_find_free_mailbox(config, data);
	if (mailbox_id == -1) {
		k_mutex_unlock(&data->inst_mutex);
		return -EBUSY;
	}
	...
	xcanfd_write_frame_data(config, frame, frame_offset);
...
#define xcanfd_INST(n)                                                                             \
	...
	DEVICE_DT_INST_DEFINE(n, xcanfd_init, NULL, &xcanfd_data_##n, &xcanfd_cfg_##n,             \
