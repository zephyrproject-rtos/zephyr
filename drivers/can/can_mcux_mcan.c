...
LOG_MODULE_DECLARE(mcux_mcan, CONFIG_CAN_LOG_LEVEL);
			  ...
			  void *callback_arg)
{
	...
	return can_mcan_send(&config->mcan, &data->mcan, &data->msg_ram,
			     msg, timeout, callback_isr, callback_arg);
}

static int mcux_mcan_attach_isr(const struct device *dev,
				...
				const struct zcan_filter *filter)
{
	...
	return can_mcan_attach_isr(&data->mcan, &data->msg_ram,
				   isr, callback_arg, filter);
}

static void mcux_mcan_detach(const struct device *dev, int filter_id)
{
	struct mcux_mcan_data *data = dev->data;

	can_mcan_detach(&data->mcan, &data->msg_ram, filter_id);
}
...
static void mcux_mcan_register_state_change_isr(const struct device *dev,
						can_state_change_isr_t isr)
{
	struct mcux_mcan_data *data = dev->data;

	data->mcan.state_change_isr = isr;
}
...
static const struct can_driver_api mcux_mcan_driver_api = {
	...
	.send = mcux_mcan_send,
	.attach_isr = mcux_mcan_attach_isr,
	.detach = mcux_mcan_detach,
#ifndef CONFIG_CAN_AUTO_BUS_OFF_RECOVERY
	.recover = can_mcan_recover,
#endif /* CONFIG_CAN_AUTO_BUS_OFF_RECOVERY */
	.get_state = mcux_mcan_get_state,
	.register_state_change_isr = mcux_mcan_register_state_change_isr,
