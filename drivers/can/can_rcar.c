...
static void can_rcar_error(const struct device *dev)
{
	...
	if (eifr & RCAR_CAN_EIFR_BEIF) {
		...
		if (ecsr & RCAR_CAN_ECSR_ADEF) {
			...
			sys_write8((uint8_t)~RCAR_CAN_ECSR_ADEF, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_BE0F) {
			CAN_STATS_BIT0_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_BE0F, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_BE1F) {
			CAN_STATS_BIT1_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_BE1F, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_CEF) {
			CAN_STATS_CRC_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_CEF, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_AEF) {
			CAN_STATS_ACK_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_AEF, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_FEF) {
			CAN_STATS_FORM_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_FEF, config->reg_addr + RCAR_CAN_ECSR);
		}
		if (ecsr & RCAR_CAN_ECSR_SEF) {
			CAN_STATS_STUFF_ERROR_INC(dev);
			sys_write8((uint8_t)~RCAR_CAN_ECSR_SEF, config->reg_addr + RCAR_CAN_ECSR);
		}

		sys_write8((uint8_t)~RCAR_CAN_EIFR_BEIF, config->reg_addr + RCAR_CAN_EIFR);
	}
	if (eifr & RCAR_CAN_EIFR_EWIF) {
		LOG_DBG("Error warning interrupt\n");
		/* Clear interrupt condition */
		sys_write8((uint8_t)~RCAR_CAN_EIFR_EWIF, config->reg_addr + RCAR_CAN_EIFR);
		can_rcar_state_change(dev, CAN_STATE_ERROR_WARNING);
	}
	if (eifr & RCAR_CAN_EIFR_EPIF) {
		LOG_DBG("Error passive interrupt\n");
		/* Clear interrupt condition */
		sys_write8((uint8_t)~RCAR_CAN_EIFR_EPIF, config->reg_addr + RCAR_CAN_EIFR);
	...
	if (eifr & RCAR_CAN_EIFR_BORIF) {
		...
		/* Clear interrupt condition */
		sys_write8((uint8_t)~RCAR_CAN_EIFR_BORIF, config->reg_addr + RCAR_CAN_EIFR);
	...
	if (eifr & RCAR_CAN_EIFR_BOEIF) {
		...
		sys_write8(RCAR_CAN_IER_ERSIE, config->reg_addr + RCAR_CAN_IER);
		/* Clear interrupt condition */
		sys_write8((uint8_t)~RCAR_CAN_EIFR_BOEIF, config->reg_addr + RCAR_CAN_EIFR);
		can_rcar_state_change(dev, CAN_STATE_BUS_OFF);
	}
	if (eifr & RCAR_CAN_EIFR_ORIF) {
		LOG_DBG("Receive overrun error interrupt\n");
		CAN_STATS_RX_OVERRUN_INC(dev);
		sys_write8((uint8_t)~RCAR_CAN_EIFR_ORIF, config->reg_addr + RCAR_CAN_EIFR);
	}
	if (eifr & RCAR_CAN_EIFR_OLIF) {
		LOG_DBG("Overload Frame Transmission error interrupt\n");
		sys_write8((uint8_t)~RCAR_CAN_EIFR_OLIF, config->reg_addr + RCAR_CAN_EIFR);
	}
	if (eifr & RCAR_CAN_EIFR_BLIF) {
		LOG_DBG("Bus lock detected interrupt\n");
		sys_write8((uint8_t)~RCAR_CAN_EIFR_BLIF, config->reg_addr + RCAR_CAN_EIFR);
	}
}

static void can_rcar_rx_filter_isr(const struct device *dev, struct can_rcar_data *data,
...
static void can_rcar_rx_isr(const struct device *dev)
{
	...
	frame.dlc = sys_read16(config->reg_addr + RCAR_CAN_MB_60 + RCAR_CAN_MB_DLC_OFFSET) & 0xF;
	...
	if (val & RCAR_CAN_MB_RTR) {
		...
		for (i = 0; i < frame.dlc; i++) {
			frame.data[i] = sys_read8(config->reg_addr + RCAR_CAN_MB_60 +
						  RCAR_CAN_MB_DATA_OFFSET + i);
		}
	}
#if defined(CONFIG_CAN_RX_TIMESTAMP)
	/* read upper byte */
	frame.timestamp = sys_read8(config->reg_addr + RCAR_CAN_MB_60 + RCAR_CAN_MB_TSH_OFFSET)
			  << 8;
	/* and then read lower byte */
	frame.timestamp |= sys_read8(config->reg_addr + RCAR_CAN_MB_60 + RCAR_CAN_MB_TSL_OFFSET);
#endif
...
static void can_rcar_isr(const struct device *dev)
{
	...
	if (isr & RCAR_CAN_ISR_TXFF) {
		...
		while (1) {
			...
			unsent = (unsent & RCAR_CAN_TFCR_TFUST) >> RCAR_CAN_TFCR_TFUST_SHIFT;
	...
	if (isr & RCAR_CAN_ISR_RXFF) {
		...
		while (!(sys_read8(config->reg_addr + RCAR_CAN_RFCR) & RCAR_CAN_RFCR_RFEST)) {
			can_rcar_rx_isr(dev);
		}
				   ...
				   const struct can_timing *timing)
{
	...
	bcr = RCAR_CAN_BCR_TSEG1(timing->phase_seg1 + timing->prop_seg - 1) |
	      RCAR_CAN_BCR_BPR(timing->prescaler - 1) | RCAR_CAN_BCR_SJW(timing->sjw - 1) |
...
static int can_rcar_set_timing(const struct device *dev, const struct can_timing *timing)
{
	const struct can_rcar_cfg *config = dev->config;
	struct can_rcar_data *data = dev->data;
	int ret = 0;
	...
	struct reg_backup regs[3] = {{RCAR_CAN_TCR, 0}, {RCAR_CAN_TFCR, 0}, {RCAR_CAN_RFCR, 0}};
			 ...
			 k_timeout_t timeout, can_tx_callback_t callback, void *user_data)
{
	...
	sys_write16(frame->dlc, config->reg_addr + RCAR_CAN_MB_56 + RCAR_CAN_MB_DLC_OFFSET);

	if ((frame->flags & CAN_FRAME_RTR) == 0) {
		for (i = 0; i < frame->dlc; i++) {
			sys_write8(frame->data[i],
				   config->reg_addr + RCAR_CAN_MB_56 + RCAR_CAN_MB_DATA_OFFSET + i);
		}
	}
...
static inline int can_rcar_add_rx_filter_unlocked(const struct device *dev, can_rx_callback_t cb,
...
static int can_rcar_add_rx_filter(const struct device *dev, can_rx_callback_t cb, void *cb_arg,
...
static int can_rcar_init(const struct device *dev)
{
	...
	/* reset the registers */
	ret = clock_control_off(config->clock_dev, (clock_control_subsys_t)&config->mod_clk);
	if (ret < 0) {
		return ret;
	}
	...
	ret = clock_control_on(config->clock_dev, (clock_control_subsys_t)&config->bus_clk);
	if (ret < 0) {
		return ret;
	}
	...
	sys_write32(RCAR_CAN_FIDCR_IDE, config->reg_addr + RCAR_CAN_FIDCR1);

	/* Enable and configure FIFO mailbox interrupts Rx and Tx */
	sys_write32(RCAR_CAN_MIER1_RXFIE | RCAR_CAN_MIER1_TXFIE, config->reg_addr + RCAR_CAN_MIER1);
