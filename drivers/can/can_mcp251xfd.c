...
static int mcp251xfd_handle_ivmif(const struct device *dev)
{
	...
	if ((tmp & MCP251XFD_REG_BDIAG1_NACKERR) != 0) {
		...
		LOG_INF("setting ack error nom");
	}

	if ((tmp & MCP251XFD_REG_BDIAG1_NBIT0ERR) != 0) {
		CAN_STATS_BIT0_ERROR_INC(dev);
	}

	if ((tmp & MCP251XFD_REG_BDIAG1_NBIT1ERR) != 0) {
		CAN_STATS_BIT1_ERROR_INC(dev);
	}

	if ((tmp & MCP251XFD_REG_BDIAG1_DBIT0ERR) != 0) {
		CAN_STATS_BIT0_ERROR_INC(dev);
	}

	if ((tmp & MCP251XFD_REG_BDIAG1_DBIT1ERR) != 0) {
		CAN_STATS_BIT1_ERROR_INC(dev);
	}
