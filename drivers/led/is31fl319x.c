...
static uint8_t is31fl319x_map_led_to_start_channel(const struct is31fl319x_config *config, uint32_t led)
{
				     ...
				     uint32_t num_channels, const uint8_t *buf)
{
	...
	for (int i = 0; i < num_channels; i++) {
		ret = i2c_reg_write_byte_dt(&config->bus, regs->led_channels[i + start_channel], buf[i]);
		if (ret != 0) {
			break;
		}
	}
