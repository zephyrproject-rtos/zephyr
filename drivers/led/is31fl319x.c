...
#define FEATURE_ID_IS_ADDR         BIT(0) /* The id is the bus address */
#define FEATURE_SET_CURRENT        BIT(1) /* the device supports per-channel current limits */
#define FEATURE_SET_CURRENT_GLOBAL BIT(2) /* the device supports a single global current limit */
...
static int is31fl319x_init(const struct device *dev)
{
	...
	if (model->shutdown_reg != REG_NOT_DEFINED) {
		...
		if (ret != 0) {
			LOG_ERR("%s: failed to set shutdown register", dev->name);
			return ret;
		}
	}
