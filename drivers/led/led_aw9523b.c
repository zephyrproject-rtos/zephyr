/*
 * SPDX-FileCopyrightText: 2026 Smartbox Assistive Technology Ltd.
 *
...
LOG_MODULE_REGISTER(led_aw9523b, CONFIG_LED_LOG_LEVEL);
...
exit_sem_unlock:
	if (ret < 0) {
		LOG_ERR("%s: Turn on sequence failed: %d", dev->name, ret);
	}
...
#define LED_AW9523B_DEFINE(inst)                                                                   \
	...
	BUILD_ASSERT(ARRAY_SIZE(led_aw9523b_channels##inst) > 0,                                   \
		     "LED controller must have at least one child node");                          \
                                                                                                   \
	static const struct led_aw9523b_config led_aw9523b_config##inst = {                        \
		...
		.i_max_idx = DT_INST_ENUM_IDX_OR(inst, current_microamp, 0),                       \
	};                                                                                         \
                                                                                                   \
