 ...
 *
 * The IS31FL3743B is an 11x18 (198 LED) matrix controller with an SPI interface. LEDs are
 * addressed as 11 switch columns (SW1-SW11) driving 18 constant-current sinks (CS1-CS18).
 *
...
#define IS31FL3743B_SPI_OPERATION (SPI_OP_MODE_CONTROLLER | SPI_WORD_SET(8) | SPI_TRANSFER_MSB)
...
/* Toggles the software shutdown bit (disables all current sources). */
static int is31fl3743b_set_shutdown(const struct device *dev, bool shutdown)
{
	struct is31fl3743b_data *data = dev->data;
...
/*	Software shutdown turns off every current source. All registers
 *	retain their contents during this mode and draw a typical 1.3uA.
 *	See the SHUTDOWN MODE section on page 19 of the datasheet.
 */
static int is31fl3743b_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
	case PM_DEVICE_ACTION_SUSPEND:
		return is31fl3743b_set_shutdown(dev, true);
	case PM_DEVICE_ACTION_RESUME:
		return is31fl3743b_set_shutdown(dev, false);
	default:
		return -ENOTSUP;
	}
}
...
#define IS31FL3743B_DEVICE(n)                                                                      \
	...
	static struct is31fl3743b_data is31fl3743b_data_##n;                                       \
                                                                                                   \
	PM_DEVICE_DT_INST_DEFINE(n, is31fl3743b_pm_action);                                        \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, &is31fl3743b_init, PM_DEVICE_DT_INST_GET(n),                      \
			      &is31fl3743b_data_##n,                                               \
