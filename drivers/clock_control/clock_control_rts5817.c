...
static int rlx_enable_syspll(const struct device *dev, const struct clk_rlx *clk)
{
	...
	sys_set_bits(config->syspll_base + R_SYSPLL_CTL, POW_SYSPLL);
	k_busy_wait(20);

	sys_set_bits(config->syspll_base + R_SYSPLL_CTL, PLL_LOAD_EN);
	k_busy_wait(10);

	sys_clear_bits(config->syspll_base + R_SYSPLL_CTL, PLL_LOAD_EN);
	k_busy_wait(70);

	while (!(sys_read32(config->syspll_base + R_SYSPLL_STS) & PLL_CKUSABLE) && timeout--) {
		k_busy_wait(10);
	}
	k_busy_wait(70);
	if (timeout) {
...
static int rlx_disable_syspll(const struct device *dev, const struct clk_rlx *clk)
{
	...
	sys_clear_bits(config->syspll_base + R_SYSPLL_CTL, POW_SYSPLL);
	k_busy_wait(10);
						   ...
						   const struct clk_rlx *clk)
{
	...
	if (sys_read32(config->syspll_base + R_SYSPLL_STS) & PLL_CKUSABLE) {
...
static uint32_t rlx_syspll_get_rate(const struct device *dev, const struct clk_rlx *clk)
{
	...
	clk_data->syspll_frequency = 4000000 * (ssc_n + 2 + ssc_f / 4096); /* Hz */
...
static int rlx_syspll_set_rate(const struct device *dev, const struct clk_rlx *clk, uint32_t rate)
{
	...
	ssc_n = (rate / 4000000) - 2;
	ssc_f = 4096 * (rate % 4000000) / 4000000;

	/* PLL_REG_CCO_SEL=1: use CCO; PLL_REG_CCO_SEL=0: use VCO */
	sys_clear_bits(config->syspll_base + R_SYSPLL_CFG, PLL_REG_CCO_SEL);
	if (ssc_f) {
		sys_clear_bits(config->syspll_base + R_SYSPLL_CFG, PLL_REG_PI_SEL);
		sys_clear_bits(config->syspll_base + R_SYSPLL_CFG, PLL_BYPASS_PI);
	} else {
		sys_set_bits(config->syspll_base + R_SYSPLL_CFG, PLL_REG_PI_SEL);
		sys_set_bits(config->syspll_base + R_SYSPLL_CFG, PLL_BYPASS_PI);
	}
	sys_set_bits(config->syspll_base + R_SYSPLL_CFG, REG_SC_H);
	reg = sys_read32(config->syspll_base + R_SYSPLL_NF_CODE);
	reg = (reg & ~(F_SSC | N_SSC)) | (ssc_n << N_SSC_OFFSET) | (ssc_f << F_SSC_OFFSET);
...
{
	struct rts_fp_clock_data *clk_data = dev->data;
	uint32_t src[] = {240000000, 160000000, 96000000, clk_data->syspll_frequency};
	uint32_t reg = rts_fp_clk_read_reg(dev, clk->clkreg);
...
{
	struct rts_fp_clock_data *clk_data = dev->data;
	uint32_t src[] = {240000000, 96000000, 80000000, clk_data->syspll_frequency};
	uint32_t reg = rts_fp_clk_read_reg(dev, clk->clkreg);
	uint32_t divider, idx;
	uint32_t rate;
...
{
	struct rts_fp_clock_data *clk_data = dev->data;
	uint32_t src[] = {96000000, 120000000, 0, clk_data->syspll_frequency};
	uint32_t reg = rts_fp_clk_read_reg(dev, clk->clkreg);
	uint32_t divider, idx;
	uint32_t rate;
...
{
	uint32_t reg = rts_fp_clk_read_reg(dev, clk->clkreg);
	uint32_t src_clk_frq = 120000000;
	uint32_t divider;
	uint32_t rate;
...
{
	uint32_t reg = rts_fp_clk_read_reg(dev, clk->clkreg);
	uint32_t src_clk_frq = 240000000;
	uint32_t divider;
	uint32_t rate;
						     ...
						     const struct clk_rlx *clk)
{
	...
	if (clk->id == RTS_FP_CLK_TRNG) {
		en_mask = TRNG_CLK_EN_MASK;
	} else if (clk->id == RTS_FP_CLK_I2C_S) {
		en_mask = I2C_S_CLK_EN_MASK;
	} else {
		return -EINVAL;
	}
...
static void rts_fp_clk_enable_uart(const struct device *dev)
{
	...
	if (DT_NODE_HAS_STATUS(DT_INST(0, ns16550), okay)) {
		val = rts_fp_clk_read_reg(dev, R_SYS_UART0_CLK_CFG_REG);
		rts_fp_clk_write_reg(dev, val | 1 << 24, R_SYS_UART0_CLK_CFG_REG);
	}

	if (DT_NODE_HAS_STATUS(DT_INST(1, ns16550), okay)) {
		val = rts_fp_clk_read_reg(dev, R_SYS_UART1_CLK_CFG_REG);
		rts_fp_clk_write_reg(dev, val | 1 << 24, R_SYS_UART1_CLK_CFG_REG);
	}
}
...
DEVICE_DT_DEFINE(DT_DRV_INST(0), rts_fp_clk_init, NULL, &rts_fp_clock_data, &rts_fp_clock_config,
