...
static inline int ifx_configure_eco(const struct device *dev)
{
	...
	ret = Cy_SysClk_EcoConfigure(DT_PROP(DT_NODELABEL(eco), clock_frequency),
				     DT_PROP(DT_NODELABEL(eco), eco_load_capacitance_pf),
				     DT_PROP(DT_NODELABEL(eco), eco_esr_ohms),
				     DT_PROP(DT_NODELABEL(eco), eco_drive_level_uw));
	if (ret != 0) {
		return -EINVAL;
	}
...
static int ifx_clk_init(const struct device *dev)
{
	...
	DT_NODE_HAS_PROP(DT_NODELABEL(clk_slow), clock_div)
	if (DT_PROP(DT_NODELABEL(clk_slow), clock_div) > 0) {
		Cy_SysClk_ClkSlowSetDivider(DT_PROP(DT_NODELABEL(clk_slow), clock_div) - 1);
	}
#endif
