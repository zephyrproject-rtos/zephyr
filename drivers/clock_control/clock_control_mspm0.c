...
static int clock_mspm0_get_rate(const struct device *dev, clock_control_subsys_t sys,
	...
	switch(clockSys->bus){
		...
		case MSPM0_CLOCK_BUS_ULPCLK:
			*rate = CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC / DT_PROP(DT_NODELABEL(clkmux),uclk_div);
			break;
...
DEVICE_DT_DEFINE(DT_NODELABEL(clkmux),
	&clock_mspm0_init,
	NULL, NULL, NULL,
...
#if (DT_NODE_HAS_PROP(DT_NODELABEL(pll), clk2x_div) && DT_NODE_HAS_PROP(DT_NODELABEL(pll), clk0_div))
