...
#define DT_DRV_COMPAT nuvoton_nct_pcc

#include <zephyr/drivers/clock_control.h>
#include <zephyr/dt-bindings/clock/nct_clock.h>

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(clock_control_nct, CONFIG_CLOCK_CONTROL_LOG_LEVEL);

/* Driver config */
struct nct_pcc_config {
...
#define NCT_HFCGCTRL_LOAD     0
#define NCT_HFCGCTRL_LOCK     2
#define NCT_HFCGCTRL_CLK_CHNG 7

/* Clock settings from pcc node */
#define NCT_NODE_PCC_LABEL DT_NODELABEL(pcc)
/* Target OFMCLK freq */
#define OFMCLK      DT_PROP(NCT_NODE_PCC_LABEL, clock_frequency)
/* Core clock prescaler */
#define FPRED_VAL   (DT_PROP(NCT_NODE_PCC_LABEL, core_prescaler) - 1)
/* APB1 clock divider */
#define APB1DIV_VAL (DT_PROP(NCT_NODE_PCC_LABEL, apb1_prescaler) - 1)
/* APB2 clock divider */
#define APB2DIV_VAL (DT_PROP(NCT_NODE_PCC_LABEL, apb2_prescaler) - 1)
/* APB3 clock divider */
#define APB3DIV_VAL (DT_PROP(NCT_NODE_PCC_LABEL, apb3_prescaler) - 1)
/* AHB6 clock divider*/
#define AHB6DIV_VAL (DT_PROP(NCT_NODE_PCC_LABEL, ahb6_prescaler) - 1)
/* FIU clock divider */
#define FIUDIV_VAL  (DT_PROP(NCT_NODE_PCC_LABEL, fiu_prescaler) - 1)
/* I3C clock divider */
#define I3C_TC_DIV_VAL  (DT_PROP(NCT_NODE_PCC_LABEL, i3c_tc_prescaler) - 1)

/* Core domain clock */
#define CORE_CLK           (OFMCLK / DT_PROP(NCT_NODE_PCC_LABEL, core_prescaler))
/* Low Frequency clock */
#define LFCLK              32768
/* FMUL clock */
#if (OFMCLK > MHZ(50))
...
/* Get APB clock freq */
#define NCT_APB_CLOCK(no) (APBSRC_CLK / (APB##no##DIV_VAL + 1))
...
/* PMC multi-registers */
#define NCT_PWDWN_CTL_OFFSET(n) (((n) < 7) ? (0x07 + n) : (0x15 + (n - 7)))
#define NCT_PWDWN_CTL(base, n)  (*(volatile uint8_t *)(base + NCT_PWDWN_CTL_OFFSET(n)))
#define NCT_CLOCK_REG_OFFSET(n)                                                                   \
	(((n) >> NCT_CLOCK_PWDWN_GROUP_OFFSET) & NCT_CLOCK_PWDWN_GROUP_MASK)
#define NCT_CLOCK_REG_BIT_OFFSET(n)                                                               \
	(((n) >> NCT_CLOCK_PWDWN_BIT_OFFSET) & NCT_CLOCK_PWDWN_BIT_MASK)

#define DRV_CONFIG(dev) ((const struct nct_pcc_config *)(dev)->config)

static inline int nct_clock_control_on(const struct device *dev, clock_control_subsys_t sub_system)
{
...
static inline int nct_clock_control_off(const struct device *dev,
...
static int nct_clock_control_get_subsys_rate(const struct device *dev,
					      clock_control_subsys_t sub_system, uint32_t *rate)
{
	ARG_UNUSED(dev);
	uint32_t clk_cfg = (uint32_t)sub_system;
	uint8_t bus_id = ((clk_cfg >> NCT_CLOCK_BUS_OFFSET) & NCT_CLOCK_BUS_MASK);

	switch (bus_id) {
	case NCT_CLOCK_BUS_APB1:
		*rate = NCT_APB_CLOCK(1);
		break;
	case NCT_CLOCK_BUS_APB2:
		*rate = NCT_APB_CLOCK(2);
		break;
	case NCT_CLOCK_BUS_APB3:
		*rate = NCT_APB_CLOCK(3);
		break;
	case NCT_CLOCK_BUS_AHB6:
		*rate = CORE_CLK / (AHB6DIV_VAL + 1);
		break;
	case NCT_CLOCK_BUS_FIU:
		*rate = CORE_CLK / (FIUDIV_VAL + 1);
		break;
	case NCT_CLOCK_BUS_I3C_TC:
		*rate = NCT_APB_CLOCK(3) / (I3C_TC_DIV_VAL + 1);
		break;
	case NCT_CLOCK_BUS_CORE:
		*rate = CORE_CLK;
		break;
	case NCT_CLOCK_BUS_OSC:
		*rate = OFMCLK;
		break;
	case NCT_CLOCK_BUS_LFCLK:
		*rate = LFCLK;
		break;
	case NCT_CLOCK_BUS_FMCLK:
		*rate = FMCLK;
		break;
	case NCT_CLOCK_BUS_USB20_CLK:
		*rate = USB20_CLK;
		break;
	case NCT_CLOCK_BUS_SIO_CLK:
		*rate = SIO_CLK;
		break;
	default:
		*rate = 0U;
...
static struct clock_control_driver_api nct_clock_control_api = {
	.on = nct_clock_control_on,
	.off = nct_clock_control_off,
	.get_rate = nct_clock_control_get_subsys_rate,
};

static int nct_clock_control_init(const struct device *dev)
{
	...
	if (priv->hfcgn != freq_p->hfcgn || priv->hfcgml != freq_p->hfcgml ||
		...
		priv->hfcgctrl |= BIT(NCT_HFCGCTRL_LOAD);
		/* Wait for stable */
		while (sys_test_bit(priv->hfcgctrl, NCT_HFCGCTRL_CLK_CHNG))
			;
	}
...
const struct nct_pcc_config pcc_config = {
	.base_cdcg = DT_INST_REG_ADDR_BY_NAME(0, cdcg),
	.base_pmc = DT_INST_REG_ADDR_BY_NAME(0, pmc),
};

DEVICE_DT_INST_DEFINE(0, &nct_clock_control_init, NULL, NULL, &pcc_config, PRE_KERNEL_1,
		      CONFIG_KERNEL_INIT_PRIORITY_OBJECTS, &nct_clock_control_api);
