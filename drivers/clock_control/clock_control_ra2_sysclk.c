...
static
int sysclk_driver_api_on_off(const struct device *dev,
...
static
int sysclk_driver_api_get_rate(const struct device *dev,
				 clock_control_subsys_t sys,
				 uint32_t *rate)
{
...
sysclk_driver_api_get_status(const struct device *dev,
				clock_control_subsys_t sys)
{
	ARG_UNUSED(sys);
	ARG_UNUSED(dev);
...
static
int sysclk_configure_internal_clk(const struct ra_internal_osc_cfg *icfg,
...
static
int sysclk_driver_api_configure(const struct device *dev,
					  clock_control_subsys_t sys,
					  void *data)
{
	int ret = 0;
...
static
int ra_internal_osc_driver_api_get_rate(const struct device *dev,
				 clock_control_subsys_t sys,
				 uint32_t *rate)
{
	ARG_UNUSED(sys);
...
static
int ra_internal_osc_driver_configure(const struct device *dev,
					  clock_control_subsys_t sys,
					  void *data)
{
