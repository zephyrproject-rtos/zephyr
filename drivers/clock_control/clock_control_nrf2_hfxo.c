...
#define DT_DRV_COMPAT nordic_nrf_hfxo
				...
				const struct nrf_clock_spec *spec)
{
	...
	uint16_t accuracy = spec->accuracy;
	...
	if (accuracy == NRF_CLOCK_CONTROL_ACCURACY_MAX) {
		accuracy = dev_config->max_accuracy;
	}

	if (accuracy < dev_config->max_accuracy) {
...
#ifdef NRF_BICR
	uint32_t bicr_startuptime_us = NRF_BICR->HFXO.STARTUPTIME;

	if (bicr_startuptime_us != BICR_HFXO_STARTUPTIME_TIME_Unconfigured) {
		dev_data->start_up_time = K_USEC(bicr_startuptime_us);
	} else
#endif
	{
		dev_data->start_up_time = K_USEC(1000);
	}
