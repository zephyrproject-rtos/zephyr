...
const fxosc_config_t g_fxoscConfig_BOARD_BootClockRUN = {.freqHz = NXP_FXOSC_FREQ,
...
const clock_pcfs_config_t g_pcfsConfig_BOARD_BootClockRUN = {.maxAllowableIDDchange =
...
static int mc_cgm_init(const struct device *dev)
{
	...
	CLOCK_ProgressiveClockFrequencySwitch(kPLL_PHI0_CLK_to_MUX0,
					      &g_pcfsConfig_BOARD_BootClockRUN);
#endif
