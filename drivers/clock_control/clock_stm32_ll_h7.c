...
#define PLLX_P_FREQ(pllsrc_freq, divm, divn, fracn, divp) \
	(PLLX_VCO_FREQ((pllsrc_freq), (divm), (divn), (fracn)) / (divp))
...
#define PLLP_VALUE \
	PLLX_P_FREQ(PLLSRC_FREQ, STM32_PLL_M_DIVISOR, STM32_PLL_N_MULTIPLIER, \
				     ...
				     unsigned int pllout_div)
{
	...
	return PLLX_P_FREQ(pllsrc_freq, pllm_div, plln_mul_u64, plln_frac, pllout_div);
}
