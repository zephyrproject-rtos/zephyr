 ...
 */
static int clock_control_ft9001_on(const struct device *dev,
...
clock_control_ft9001_get_status(const struct device *dev, clock_control_subsys_t sys)
{
	const struct clock_control_ft9001_config *config = dev->config;
	uint32_t id = POINTER_TO_UINT(sys);
	uint32_t reg_offset = FT9001_CLOCK_REG_OFFSET(id);
