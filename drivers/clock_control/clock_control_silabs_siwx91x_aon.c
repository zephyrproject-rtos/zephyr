...
static const struct {
	...
	{ SIWX91X_CLK_AON_REF_HF, SIWX91X_CLK_GATED,       0 }, /* FSM_NO_CLOCK               */
			   ...
			   struct silabs_siwx91x_clock_control_config *mux)
{
	 ...
	 * k_us_to_cyc_ceil32() → soft 64-bit libgcc (__udivmoddi4), which is
	 * too heavy for clock restore on the log → UART → PM path and can
	 * overflow CONFIG_LOG_PROCESS_THREAD_STACK_SIZE (default 768).
	 */
	switch (mux->clkid) {
	...
	case SIWX91X_CLK_REF_ULP:
		 ...
		 * it is not initialized yet.
		 */
		while (M4CLK->PLL_STAT_REG_b.ULP_REF_CLK_SWITCHED != 1) {
		}
	...
	case SIWX91X_CLK_AON_REF_HF:
		...
		while (cfg->fsm_reg->MCU_FSM_CLKS_REG_b.HF_FSM_CLK_SWITCHED_SYNC != 1) {
		}
	...
	case SIWX91X_CLK_AON_REF_LF:
		...
		while (cfg->aon_reg->MCUAON_KHZ_CLK_SEL_POR_RESET_STATUS_b
		       .AON_KHZ_CLK_SEL_CLOCK_SWITCHED != 1) {
		}
	...
	*rate = 0;
	...
	case SIWX91X_CLK_XTAL_MHZ:
		*rate = DT_PROP(DT_NODELABEL(xtal_mhz), clock_frequency);
		break;
	case SIWX91X_CLK_XTAL_KHZ:
		/* External UULP input rate comes from devicetree. */
							      ...
							      clock_control_subsys_t clk_cfg)
{
	...
	case SIWX91X_CLK_XTAL_MHZ:
		...
		break;
	case SIWX91X_CLK_XTAL_KHZ:
		if (cfg->external_xtal_khz_pin != UINT8_MAX) {
			/* The XTAL gate does not control an external input. */
			val = true;
		} else {
			val = cfg->fsm_reg->MCU_FSM_CLK_ENS_AND_FIRST_BOOTUP_b
...
static int siwx91x_aon_clock_init(const struct device *dev)
{
	...
	/* If a uulp gpio is configured to receive an external 32 kHz oscillator signal, configure
	 * it. This ROM function actually do all needed actions:
	 *  - The UULP GPIO pinctrl to put the gpio pin in clock input mode.
	 *  - The switch that enables the XTAL kHz clock from the UULP GPIO.
	 * TODO: The switch remains to the cmu aon driver but the uulp gpio configuration should be
	 * done in the uulp gpio driver.
	 */
	if (cfg->external_xtal_khz_pin != UINT8_MAX) {
		ret = RSI_CLK_XtalClkConfig(cfg->external_xtal_khz_pin);
		if (ret != RSI_OK) {
			LOG_ERR("UULP GPIO %u oscillator setup failed", cfg->external_xtal_khz_pin);
		}
	}
...
#define SIWX91X_AON_EXTERNAL_XTAL_KHZ_PIN(inst) \
	DT_INST_PROP_OR(inst, silabs_external_xtal_khz_pin, UINT8_MAX)
...
#define SIWX91X_AON_CLOCK_INIT(inst)                                                               \
	...
	static const struct siwx91x_aon_clock_config siwx91x_aon_clock_config_##inst = {           \
		...
		.external_xtal_khz_pin = SIWX91X_AON_EXTERNAL_XTAL_KHZ_PIN(inst), \
