...
static void siwx91x_ulp_clk_wait_switched(const struct device *dev, uint32_t clockid)
{
	const struct siwx91x_ulp_clock_config *config = dev->config;
	ULPCLK_Type *ulpclk = config->ulpclk_reg;

	/*
	 * Spin on HW status bits. Avoid WAIT_FOR() here: it pulls in
	 * k_us_to_cyc_ceil32() → __aeabi_uldivmod / __udivmoddi4, which is too
	 * heavy for this path (log → UART → PM → clock_on/configure). With the
	 * default CONFIG_LOG_PROCESS_THREAD_STACK_SIZE (768), that soft 64-bit
	 * helper overflows the log thread stack. A bare while keeps the restore
	 * path light (and matches Wiseconnect).
	 */
	switch (clockid) {
	case SIWX91X_CLK_ULP_REF_CPU:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_PROC_CLK_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_UART:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_UART_CLK_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_I2S:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_I2S_CLK_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_TIMER:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_TIMER_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_SSI:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_SSI_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_ADC:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_AUXADC_b != 1) {
		}
		break;
	case SIWX91X_CLK_ULP_I2C:
		while (ulpclk->CLOCK_STAUS_REG_b.CLOCK_SWITCHED_I2C_b != 1) {
		}
		break;
	default:
		break;
...
static int siwx91x_ulp_clk_get_div_factor(const struct device *dev, uint32_t clockid)
{
	...
	case SIWX91X_CLK_ULP_REF_AON:
		...
		break;
	default:
		/* Clocks without a programmable divider. */
		break;
	}
				  ...
				  struct silabs_siwx91x_clock_control_config *mux)
{
	...
	siwx91x_ulp_clk_wait_switched(dev, mux->clkid);
...
static int siwx91x_ulp_clock_on(const struct device *dev, clock_control_subsys_t sys)
{
	...
	switch (clockid) {
	case SIWX91X_CLK_ULP_REF_CPU:
		/* ULP_REF_CPU is part of the Misc domain for power */
...
static int siwx91x_ulp_clock_off(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case SIWX91X_CLK_ULP_STATIC_I2S:
		RSI_ULPSS_PeripheralDisable(config->ulpclk_reg, ULP_I2S_CLK);
		RSI_PS_UlpssPeriPowerDown(ULPSS_PWRGATE_ULP_I2S);
		config->ulpclk_reg->ULP_I2S_CLK_GEN_REG_b.ULP_I2S_MASTER_SLAVE_MODE_b = 0;
		break;
	case SIWX91X_CLK_ULP_ADC:
				      ...
				      uint32_t *rate)
{
	...
	*rate = 0;
	...
	div_factor = siwx91x_ulp_clk_get_div_factor(dev, clockid);
	if (div_factor > 1) {
				      ...
				      clock_control_subsys_rate_t raw_rate)
{
	...
	/*
	 * The I2S driver requests a bit-clock rate on SIWX91X_CLK_ULP_I2S.
	 * When that clock is sourced from the I2S PLL, apply the rate there
	 * (PLL programming lives in the HP manager).
	 */
	if (clockid == SIWX91X_CLK_ULP_I2S) {
		return clock_control_set_rate(siwx91x_clock_control_get_device(SIWX91X_CLK_PLL_I2S),
					      (clock_control_subsys_t)SIWX91X_CLK_PLL_I2S,
...
static int siwx91x_ulp_clock_init(const struct device *dev)
{
	 ...
	 * domain in the future. If we don't do this, we will not be able to control the clocks
	 * for the ULP peripherals (clock_control_on() will not work). This is not necessary at
	 * the moment because no one explicitly shuts down the ULP_MISC domain.
	 */
	...
	/* ULP_REF_CPU is the bus clock for the ULP peripherals, and ULP_REF_AON is the bus clock
	 * for the UULP peripherals. These are enabled by default at startup, which is why
	 ...
	 * CPU_LP (HP clock driver). This is not currently used, and WiseConnect uses it only
