...
static
int ra_root_osc_driver_api_on(const struct device *dev,
...
static
int ra_root_osc_driver_api_off(const struct device *dev,
				clock_control_subsys_t sys)
{
	const struct ra_root_osc_cfg * const cfg = dev->config;
	struct ra_root_osc_data *dat = dev->data;
...
static
int ra_root_osc_driver_api_get_rate(const struct device *dev,
				 clock_control_subsys_t sys,
				 uint32_t *rate)
{
	ARG_UNUSED(sys);
...
ra_root_osc_driver_api_get_status(
