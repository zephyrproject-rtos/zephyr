...
static const clock_ip_name_t mcxl_aon_gate_table[] = {
	...
	[MCXL_AON_GATE_KPP]      = kCLOCK_GateAonKPP,
};
...
BUILD_ASSERT(MCXL_AON_GATE_KPP <= MCXL_CLOCK_GATE_IDX_MASK,
...
BUILD_ASSERT(ARRAY_SIZE(mcxl_aon_gate_table) == (MCXL_AON_GATE_KPP + 1),
...
static bool mcxl_aon_get_gate_freq(uint8_t gate_idx, uint32_t *freq)
{
	...
	case MCXL_AON_GATE_SYS:
		*freq = CLOCK_GetAonCoreSysClkFreq();
		return true;
	case MCXL_AON_GATE_KPP:
		*freq = CLOCK_GetFreq(kCLOCK_Fro16k); /* AON KPP runs from FRO16K */
		return true;
	default:
		return false;
	}
