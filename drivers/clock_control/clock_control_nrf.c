...
#define CLOCK_LOG(lvl, dev, subsys, ...)                                              \
	LOG_##lvl("%s: " GET_ARG_N(1, __VA_ARGS__),                                   \
		  get_sub_config(dev, (enum clock_control_nrf_type)subsys)            \
			  ->name COND_CODE_0(NUM_VA_ARGS_LESS_1(__VA_ARGS__),\
...
struct nrf_clock_control_config {
	struct nrf_clock_control_sub_config subsys[CLOCK_CONTROL_NRF_TYPE_COUNT];
};
...
static const struct nrf_clock_control_sub_config *get_sub_config(const struct device *dev,
								 enum clock_control_nrf_type type)
{
	const struct nrf_clock_control_config *config = dev->config;
...
{
	return get_onoff_manager(CLOCK_DEVICE, (enum clock_control_nrf_type)(size_t)sys);
}

static enum clock_control_status get_status(const struct device *dev, clock_control_subsys_t subsys)
{
...
static void clkstarted_handle(const struct device *dev, enum clock_control_nrf_type type)
{
...
static void generic_hfclk_start(void)
{
	...
	if (already_started) {
		...
		clkstarted_handle(CLOCK_DEVICE, CLOCK_CONTROL_NRF_TYPE_HFCLK);
		return;
	}
...
static int stop(const struct device *dev, clock_control_subsys_t subsys, uint32_t ctx)
{
	enum clock_control_nrf_type type = (enum clock_control_nrf_type)(size_t)subsys;
...
static void onoff_start(struct onoff_manager *mgr, onoff_notify_fn notify)
{
	...
	err = async_start(CLOCK_DEVICE, get_subsys(mgr), onoff_started_callback, notify, CTX_ONOFF);
	if (err < 0) {
		notify(mgr, err);
	}
}
...
void z_nrf_clock_control_lf_on(enum nrf_lfclk_start_mode start_mode)
{
	...
	if (atomic_set(&on, 1) == 0) {
		...
		struct onoff_manager *mgr =
			get_onoff_manager(CLOCK_DEVICE, CLOCK_CONTROL_NRF_TYPE_LFCLK);
...
static void hfclkaudio_init(void)
{
	...
	const uint32_t frequency = DT_PROP(DT_NODELABEL(clock), hfclkaudio_frequency);
...
static int clk_init(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), nrfx_isr,
		    nrfx_power_clock_irq_handler, 0);
	...
	for (enum clock_control_nrf_type i = 0; i < CLOCK_CONTROL_NRF_TYPE_COUNT; i++) {
		struct nrf_clock_control_sub_data *subdata = get_sub_data(dev, i);

		err = onoff_manager_init(get_onoff_manager(dev, i), &transitions);
		if (err < 0) {
			return err;
		}
...
DEVICE_DT_DEFINE(DT_NODELABEL(clock), clk_init, NULL, &data, &config, PRE_KERNEL_1,
...
static int cmd_status(const struct shell *sh, size_t argc, char **argv)
{
	...
	struct onoff_manager *hf_mgr =
		get_onoff_manager(CLOCK_DEVICE, CLOCK_CONTROL_NRF_TYPE_HFCLK);
	...
	shell_print(sh, "\t- %srunning (users: %u)", hf_status ? "" : "not ", hf_mgr->refs);
	shell_print(sh, "\t- last start: %u ms (%u ms ago)", (uint32_t)abs_start,
		    (uint32_t)(now - abs_start));
	shell_print(sh, "\t- last stop: %u ms (%u ms ago)", (uint32_t)abs_stop,
		    (uint32_t)(now - abs_stop));
	shell_print(sh, "LF clock:");
	shell_print(sh, "\t- %srunning (users: %u)", lf_status ? "" : "not ", lf_mgr->refs);

	return 0;
}

SHELL_STATIC_SUBCMD_SET_CREATE(subcmds, SHELL_CMD_ARG(status, NULL, "Status", cmd_status, 1, 0),
			       SHELL_SUBCMD_SET_END);

SHELL_COND_CMD_REGISTER(CONFIG_CLOCK_CONTROL_NRF_SHELL, nrf_clock_control, &subcmds,
			"Clock control commands", cmd_status);
