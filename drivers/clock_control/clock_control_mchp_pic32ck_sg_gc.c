...
/* PLL_OUTPUT initialization structure. */
...
static int clock_mchp_off(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case SUBSYS_TYPE_PLL_OUT:
		...
		out_idx = inst % PLLOUT_COUNT;

		pos_en = PLLOUT_EN_BIT_POS(out_idx);
		postdiv_reg = PLLOUT_POSTDIV_REG(oscctrl_regs, pll, out_idx);
		*postdiv_reg &= ~BIT(pos_en);
...
void clock_pll_out_init(const struct device *dev, struct clock_pll_out_init *pll_out_init)
{
	...
	pos_postdiv = PLLOUT_DIV_BIT_POS(out_idx);
	postdiv_reg = PLLOUT_POSTDIV_REG(oscctrl_regs, pll, out_idx);

	val32 = *postdiv_reg & ~(PLLPOSTDIV_Msk << pos_postdiv);
