...
static const struct clock_options {
	...
	/* TODO: use real accuracy values, handle other LFXO configurations */
	{
		.accuracy = 100,
		.precision = 0,
...
static const uint16_t max_accuracy = 25;
...
static void lfclk_work_handler(struct k_work *work)
{
	...
	err = nrfs_clock_lfclk_src_set(clock_options[option_idx].src, dev_data);
	if (err != NRFS_SUCCESS) {
