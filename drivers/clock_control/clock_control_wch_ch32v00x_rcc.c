					    ...
					    uint32_t *rate)
{
	...
	sysclk /= config->clocks_data[CH32_CLKID_CLK_HB].div;

	if (id == CH32V00X_CLOCK_ADC1) {
		sysclk /= config->clocks_data[CH32_CLKID_CLK_ADC].div;
	}
...
{
	uint16_t div = config->clocks_data[CH32_CLKID_CLK_HB].div;
	uint32_t hpre;
	...
	if (div <= 8) {
		...
		hpre = find_msb_set(div - 2) | 0b1000;
	}

	config->rcc_regs->CFGR0 =
		//(config->rcc_regs->CFGR0 & ~RCC_HPRE) | (hpre << find_lsb_set(RCC_HPRE));
		(config->rcc_regs->CFGR0 & ~RCC_HPRE) | (hpre << 4);
}

static void wch_set_adc_prescaler(const struct clock_control_wch_config *config)
{
	uint16_t div = config->clocks_data[CH32_CLKID_CLK_ADC].div;
	uint32_t adcpre = 0;
...
#error "Invalid pll source"
			...
			[CH32_CLKID_CLK_HB] =
				{
					.div = DT_PROP(CH32_GET_CLK(clk_hb), div),
				},
			[CH32_CLKID_CLK_ADC] =
				{
					.div = DT_PROP(CH32_GET_CLK(clk_adc), div),
				},
		},
};
