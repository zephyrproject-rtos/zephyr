...
static const struct clock_options {
	...
	{
		.accuracy = DT_INST_PROP(0, open_loop_accuracy_ppm),
		.mode = FLL16M_MODE_OPEN_LOOP,
	},
	{
		.accuracy = DT_INST_PROP(0, closed_loop_accuracy_ppm),
		.mode = FLL16M_MODE_CLOSED_LOOP,
	},
	{
		.accuracy = DT_INST_PROP(0, bypass_accuracy_ppm),
		.mode = FLL16M_MODE_BYPASS,
	},
};

static const uint16_t max_accuracy = DT_INST_PROP(0, bypass_accuracy_ppm);
...
{
	const struct device *hfxo = DEVICE_DT_GET(DT_INST_PROP(0, bypass_clock));
	struct fll16m_dev_data *dev_data =
