...
static const struct clock_options {
	...
	{
		/* TODO: not to be used when there is no LFXO. */
		.accuracy = 5000,
		.mode = FLL16M_MODE_CLOSED_LOOP,
	},
	{
		/* TODO: HFXO accuracy, get from devicetree. */
		.accuracy = 100,
		.mode = FLL16M_MODE_BYPASS,
	},
};
static const uint16_t max_accuracy = 100;
...
static void fll16m_work_handler(struct k_work *work)
{
	...
	uint8_t option_idx = ARRAY_SIZE(clock_options) - 1;

	active_options = clock_config_update_begin(work);
	while (option_idx) {
		if (active_options & BIT(option_idx)) {
			break;
		}
		--option_idx;
	}

	if (clock_options[option_idx].mode == FLL16M_MODE_BYPASS) {
		int rc;
		...
		activate_fll16m_mode(dev_data, clock_options[option_idx].mode);
	}
}
