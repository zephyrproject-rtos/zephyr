 ...
 */
#define ALIF_CLOCK_CFG_SRC(id) \
	(((id) >> ALIF_CLOCK_SRC_SHIFT) & ALIF_CLOCK_SRC_MASK)

/**
 * @brief Extract clock source mask width from encoded clock ID
 * @param id Clock ID from device tree (encoded with ALIF_CLK_CFG)
 * @return Mask width for source field (0=no source select, 1-3=field width)
 */
#define ALIF_CLOCK_CFG_SRC_MASK(id) \
	(((id) >> ALIF_CLOCK_SRC_MASK_SHIFT) & ALIF_CLOCK_SRC_MASK_MASK)

/**
 * @brief Extract clock source bit position from encoded clock ID
 * @param id Clock ID from device tree (encoded with ALIF_CLK_CFG)
 * @return Bit position for clock source selection (0-31)
 */
#define ALIF_CLOCK_CFG_SRC_BIT_POS(id) \
			...
			clock_control_subsys_t sub_system)
{
	 ...
	 */
	if (ALIF_CLOCK_CFG_SRC_MASK(clk_id)) {
		uint32_t src_bit = ALIF_CLOCK_CFG_SRC_BIT_POS(clk_id);
		uint32_t src_mask = ALIF_CLOCK_CFG_SRC_MASK(clk_id);
		uint32_t src_value = ALIF_CLOCK_CFG_SRC(clk_id);
