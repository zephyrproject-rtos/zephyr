				    ...
				    uint32_t *rate)
{
	...
	switch (id) {
	case SF32LB_PMUC_CLOCK_LRC10:
		*rate = config->lrc10_freq;
		return 0;
	case SF32LB_PMUC_CLOCK_LRC32:
		*rate = config->lrc32_freq;
		return 0;
	default:
		break;
	}
...
#define SF32LB_PMUC_CLK_CONFIG(inst)                                                               \
	static const struct sf32lb_pmuc_clk_config sf32lb_pmuc_clk_config_##inst = {               \
		...
		.lrc10_freq = DT_PROP_BY_IDX(DT_DRV_INST(inst), clock_frequencies,                 \
					     SF32LB_PMUC_CLOCK_LRC10),                             \
		.lrc32_freq = DT_PROP_BY_IDX(DT_DRV_INST(inst), clock_frequencies,                 \
					     SF32LB_PMUC_CLOCK_LRC32),                             \
	}
