...
#define PMUC_LXT32_CR offsetof(PMUC_TypeDef, LXT_CR)
...
#define SF32LB_PMUC_LXT32_FREQ 32768U
...
static int sf32lb_pmuc_clk_on(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case SF32LB_PMUC_CLOCK_LXT32:
		sys_set_bit(config->base + PMUC_LXT32_CR, PMUC_LXT_CR_EN_Pos);
		while (sys_test_bit(config->base + PMUC_LXT32_CR, PMUC_LXT_CR_RDY_Pos) == 0U) {
		}
		return 0;
	default:
		break;
...
static int sf32lb_pmuc_clk_off(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case SF32LB_PMUC_CLOCK_LRC32:
		...
		return 0;
	case SF32LB_PMUC_CLOCK_LXT32:
		sys_clear_bit(config->base + PMUC_LXT32_CR, PMUC_LXT_CR_EN_Pos);
		return 0;
	default:
		break;
	}
