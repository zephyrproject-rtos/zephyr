...
#define CLK32K_XTAL_WAIT             (16U * 1024U)
#define CLK32K_XTAL_MON_WAIT         (64U * 1024U)
#define XEC_CC_DFLT_PLL_LOCK_WAIT_MS 30U
...
#define XEC_HT_CNT_PER_MS      32U /* clk is 32.768KHz */
#define XEC_HT_MAX_DLY_W_30P5_RESO 1998U /* 65536 * 30.5us = ~1998ms */
...
#define XEC_CLK_100KHZ KHZ(100)
#define XEC_CLK_96MHZ  MHZ(96)
#define XEC_CLK_48MHZ  MHZ(48)
#define XEC_CLK_192MHZ MHZ(192)
...
static void ht_clean(const struct device *dev)
{
	...
	sys_write16(0, ht_base + XEC_HT_CR_OFS);
			 ...
			 int (*is_ready)(const struct device *))
{
	...
	sys_write16(0, ht_base + XEC_HT_CR_OFS); /* 30.5 us per count */
			    ...
			    int (*is_ready)(const struct device *))
{
	...
	while (ms > 0) {
		if (ms > XEC_HT_MAX_DLY_W_30P5_RESO) {
			ht_counts = UINT16_MAX;
			ms -= XEC_HT_MAX_DLY_W_30P5_RESO;
		} else {
			ht_counts = ms * XEC_HT_CNT_PER_MS;
			ms = 0;
		}
...
static int xec_cc_xtal_health_check(const struct device *dev)
{
	...
	mem_addr_t pcr_base = drvcfg->pcr_base;
	uint32_t r = 0, pass_msk = 0, err_msk = 0, ms_count = 0;
	int rc = 0;
	...
	while (1) {
		...
		if ((r & BIT(XEC_CC_32K_SR_PULSE_RDY_POS)) != 0) { /* RO counters updated? */
			...
			if ((r & err_msk) != 0) { /* monitor reported a failure */
				rc = -EIO;
				break;
			}
						   ...
						   clock_control_subsys_t sys)
{
	...
	case MCHP_XEC_PCR_CLK_XTAL:
		...
		if (xtal_enabled(dev) != 0) {
			...
			/* 32KHz clock monitor is only present on MEC172x/4x/5x */
			struct xec_pcr_data *data = dev->data;
			uint32_t good = BIT(XEC_CC_32K_SR_PER_PASS_POS) |
					BIT(XEC_CC_32K_SR_DC_PASS_POS) |
					BIT(XEC_CC_32K_SR_VALID_POS);
			uint32_t bad = BIT(XEC_CC_32K_SR_FAIL_POS) |
				       BIT(XEC_CC_32K_SR_PER_OVFL_POS) |
				       BIT(XEC_CC_32K_SR_UNWELL_POS);

			if ((data->clkmon_status != 0) &&
			    (((data->clkmon_status & good) != good) ||
			     ((data->clkmon_status & bad) != 0))) {
				cc_status = CLOCK_CONTROL_STATUS_STARTING;
			}
#endif
		}
		break;
...
int z_mchp_xec_pcr_periph_sleep(uint8_t slp_idx, uint8_t slp_pos, uint8_t slp_en)
{
	...
	if ((slp_idx >= XEC_CC_PCR_MAX_SCR) || (slp_pos > XEC_CC_SCR_BITPOS_MAX)) {
		return -EINVAL;
	}
...
static int xec_clock_control_init(const struct device *dev)
{
	...
	rc = z_mchp_xec_pcr_vb_pll_init();
	if (rc != 0) {
		return rc;
	}

	return soc_xec_pcr_cpu_clk_div_set(drvcfg->core_clk_div);
#endif
}
...
#define XEC_DT_CLK(i, prop, pos) ((DT_INST_PROP_OR(i, prop, 0) & BIT(0)) << (pos))

#define XEC_DT_CLK_FLAGS(i)                                                   \
	((XEC_DT_CLK(i, xtal_single_ended, 0)) |                              \
	 (XEC_DT_CLK(i, pll_src_lock, 1)) |                                   \
	 (XEC_DT_CLK(i, silosc_en_lock, 2)) |                                 \
	 (XEC_DT_CLK(i, internal_osc_disable, 3)) |                           \
	 (XEC_DT_CLK(i, xtal_hi_startup_current_disable, 4)) |                \
...
const struct xec_pcr_config pcr_xec_config = {
	...
	.xtal_mon_max_ms = (uint16_t)DT_INST_PROP(0, xtal_monitor_max_ms),
	.pll_lock_timeout_ms =
	...
	.core_clk_div = (uint8_t)DT_INST_PROP_OR(0, core_clock_div, CONFIG_SOC_MEC_PROC_CLK_DIV),
