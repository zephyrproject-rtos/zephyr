					   ...
					   uint32_t *rate)
{
...
#if DT_NODE_EXISTS(DT_NODELABEL(fcpb))
	case CLOCK_FCPB:
		*rate = fsource0_freq / fcpb_div;
		return 0;
#endif
#if DT_NODE_EXISTS(DT_NODELABEL(fgtm))
	case CLOCK_FGTM:
		*rate = fsource0_freq / fgtm_div;
		return 0;
#endif
#if DT_NODE_EXISTS(DT_NODELABEL(fegtm))
	case CLOCK_FEGTM:
		*rate = fsource0_freq / fegtm_div;
		return 0;
#endif
	case CLOCK_FSTM:
	...
	case CLOCK_FASCLINF:
		*rate = fsource2_freq / fasclinf_div;
		return 0;
	case CLOCK_FASCLINS:
		*rate = fsource1_freq / fasclinsi_div;
		return 0;
	case CLOCK_FQSPI:
		*rate = (CLOCK_SOURCE_IS(fsourceqspi, fsource1) ? fsource1_freq : fsource2_freq) /
			fqspi_div;
		return 0;
	case CLOCK_FI2C:
		*rate = fsource2_freq / fi2c_div;
		return 0;
	default:
		return -EINVAL;
	}
...
/* CCU_TC.012 work-around: turn the PLL off, re-write its default register set,
 * then run the normal init. Used for both the system and peripheral PLLs.
 */
static int clock_control_tc4x_ccu_syspll_setup(void)
{
	int ret;
	...
	return clock_control_tc4x_ccu_syspll_init();
}

static int clock_control_tc4x_ccu_perpll_setup(void)
{
	int ret;
	...
	return clock_control_tc4x_ccu_perpll_init();
}
...
static int clock_control_tc4x_ccu_init(const struct device *dev)
{
	...
	/* We expect that the startup CPU is responsible initialization of the CCU */
	...
	if (CLOCK_SOURCE_IS(fsource0, fpll0)) {
		ret = clock_control_tc4x_ccu_syspll_setup();
		if (ret) {
			return ret;
		}
	}
	...
	if (CLOCK_SOURCE_IS(fsource1, fpll1)) {
		ret = clock_control_tc4x_ccu_perpll_setup();
		if (ret) {
			return ret;
		}
	}
	...
	if (CLOCK_SOURCE_IS(fsource0, fpll0)) {
		...
		/* Neither PLL is the active source; stay on fback. */
	}
...
DEVICE_DT_INST_DEFINE(0, clock_control_tc4x_ccu_init, NULL, NULL, NULL, PRE_KERNEL_1,
