...
static const struct {
	...
	{ SIWX91X_CLK_GSPI,       SIWX91X_CLK_REF_HP,    1 },
	{ SIWX91X_CLK_GSPI,       SIWX91X_CLK_PLL_SOC,   2 },
	{ SIWX91X_CLK_GSPI,       SIWX91X_CLK_PLL_INTF,  4 },
...
static void siwx91x_hp_clk_proc_change_update(void)
{
	uint32_t rate;
	...
	if (IS_ENABLED(CONFIG_CORTEX_M_SYSTICK)) {
		z_sys_clock_hw_cycles_per_sec_update(rate);
	}
}
...
static void siwx91x_hp_clk_wait_switched(const struct device *dev, uint32_t clockid)
{
	const struct siwx91x_hp_clock_config *cfg = dev->config;
	...
	switch (clockid) {
	case SIWX91X_CLK_CPU:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.M4_SOC_CLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_CPU_LP:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.SLEEP_CLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_QSPI:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.QSPI_CLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_QSPI2:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.QSPI_2_CLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_UART0:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.USART1_SCLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_UART1:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.USART2_SCLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_SSI:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.SSI_MST_SCLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_I2S:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.I2S_CLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_GSPI:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.GEN_SPI_MST1_SCLK_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_PIN_OUT:
		while (cfg->m4clk_reg->PLL_STAT_REG_b.MCU_CLKOUT_SWITCHED != 1) {
		}
		break;
	case SIWX91X_CLK_HP_REF_ULP:
				 ...
				 struct silabs_siwx91x_clock_control_config *mux)
{
	 ...
	 * configuration. This is the responsabilty of the peripheral to enable his clock with
	 * clock_control_on() Some exceptions are made for QSPI (because of XIP) and obviously for
	 * the cpu clock.
	 */
	switch (mux->clkid) {
	case SIWX91X_CLK_CPU:
	...
	siwx91x_hp_clk_wait_switched(dev, mux->clkid);
				     ...
				     uint32_t *rate)
{
	...
	*rate = 0;
	...
	case SIWX91X_CLK_PIN_OUT:
		...
		break;
	case SIWX91X_CLK_HP_REF_PLL:
		/* Shared PLL ref mux is SPI-mapped; use the persisted DT/runtime route. */
		for (size_t i = 0; i < data->hp_clk_mux_count; i++) {
			if (data->hp_clk_mux[i].clkid == SIWX91X_CLK_HP_REF_PLL) {
				ref_clkid = data->hp_clk_mux[i].ref_clkid;
				break;
			}
		}
	...
	div_factor = siwx91x_hp_clk_get_div_factor(dev, clockid);
	if (div_factor > 1) {
				     ...
				     clock_control_subsys_rate_t raw_rate)
{
	...
	/*
	 * The I2S driver requests a bit-clock rate on SIWX91X_CLK_I2S. That clock
	 * is sourced from the I2S PLL, so apply the rate there.
	 * TODO: Rework I2S driver.
	 */
	if (clockid == SIWX91X_CLK_I2S) {
		clockid = SIWX91X_CLK_PLL_I2S;
	}
...
#define SIWX91X_HP_CLOCK_INIT(inst)                                                                \
	...
	static struct silabs_siwx91x_clock_control_pll_config pll_cfg_##inst[] = {                 \
		DT_INST_FOREACH_CHILD_STATUS_OKAY(inst, SIWX91X_CLOCK_MANAGER_PLL_INIT)            \
	};                                                                                         \
