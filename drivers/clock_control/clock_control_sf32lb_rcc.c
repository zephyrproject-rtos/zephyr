...
#define SF32LB_SYS_CLK_VALUE_HRC48 0U
#define SF32LB_SYS_CLK_VALUE_HXT48 1U
#define SF32LB_SYS_CLK_VALUE_LPCLK 2U
#define SF32LB_SYS_CLK_VALUE_DLL1  3U

#define SF32LB_PERI_CLK_VALUE_HRC48 0U
#define SF32LB_PERI_CLK_VALUE_HXT48 1U

#define SF32LB_MPI_CLK_VALUE_PERI 0U
#define SF32LB_MPI_CLK_VALUE_DLL1 1U
#define SF32LB_MPI_CLK_VALUE_DLL2 2U

#define SF32LB_USB_CLK_VALUE_SYSCLK 0U
#define SF32LB_USB_CLK_VALUE_DLL2   1U
...
#define SF32LB_SYS_CLK_SRC_VALUE_FROM_IDX(idx)                                                     \
	((idx) == SF32LB_SYS_CLK_IDX_HRC48   ? SF32LB_SYS_CLK_VALUE_HRC48                          \
	 : (idx) == SF32LB_SYS_CLK_IDX_HXT48 ? SF32LB_SYS_CLK_VALUE_HXT48                          \
	 : (idx) == SF32LB_SYS_CLK_IDX_LPCLK ? SF32LB_SYS_CLK_VALUE_LPCLK                          \
					     : SF32LB_SYS_CLK_VALUE_DLL1)

#define SF32LB_PERI_CLK_SRC_VALUE_FROM_IDX(idx)                                                    \
	((idx) == SF32LB_PERI_CLK_IDX_HXT48 ? SF32LB_PERI_CLK_VALUE_HXT48                          \
					    : SF32LB_PERI_CLK_VALUE_HRC48)

#define SF32LB_MPI_CLK_SRC_VALUE_FROM_IDX(idx)                                                     \
	((idx) == SF32LB_MPI_CLK_IDX_DLL2   ? SF32LB_MPI_CLK_VALUE_DLL2                            \
	 : (idx) == SF32LB_MPI_CLK_IDX_DLL1 ? SF32LB_MPI_CLK_VALUE_DLL1                            \
					    : SF32LB_MPI_CLK_VALUE_PERI)

#define SF32LB_USB_CLK_VALUE_FROM_IDX(idx)                                                         \
	((idx) == SF32LB_USB_CLK_IDX_DLL2 ? SF32LB_USB_CLK_VALUE_DLL2 : SF32LB_USB_CLK_VALUE_SYSCLK)
...
#define SF32LB_SYS_CLK_SRC_IDX(inst)                                                               \
	DT_ENUM_IDX_OR(DT_DRV_INST(inst), sifli_sys_clk_src, SF32LB_SYS_CLK_IDX_HXT48)
...
#define SF32LB_SYS_CLK_SRC_VALUE(inst)                                                             \
	SF32LB_SYS_CLK_SRC_VALUE_FROM_IDX(SF32LB_SYS_CLK_SRC_IDX(inst))
#define SF32LB_PERI_CLK_SRC_VALUE(inst)                                                            \
	SF32LB_PERI_CLK_SRC_VALUE_FROM_IDX(SF32LB_PERI_CLK_SRC_IDX(inst))
#define SF32LB_MPI1_CLK_SRC_VALUE(inst)                                                            \
	SF32LB_MPI_CLK_SRC_VALUE_FROM_IDX(SF32LB_MPI1_CLK_SRC_IDX(inst))
#define SF32LB_MPI2_CLK_SRC_VALUE(inst)                                                            \
	SF32LB_MPI_CLK_SRC_VALUE_FROM_IDX(SF32LB_MPI2_CLK_SRC_IDX(inst))
#define SF32LB_USB_CLK_SRC_VALUE(inst) SF32LB_USB_CLK_VALUE_FROM_IDX(SF32LB_USB_CLK_SRC_IDX(inst))
...
#define SF32LB_MPI1_CLK_REQUIRES_DLL2(inst)                                                        \
	(SF32LB_MPI1_CLK_SRC_IDX(inst) == SF32LB_MPI_CLK_IDX_DLL2)
#define SF32LB_MPI1_CLK_REQUIRES_DLL1(inst)                                                        \
	(SF32LB_MPI1_CLK_SRC_IDX(inst) == SF32LB_MPI_CLK_IDX_DLL1)
#define SF32LB_MPI2_CLK_REQUIRES_DLL2(inst)                                                        \
	(SF32LB_MPI2_CLK_SRC_IDX(inst) == SF32LB_MPI_CLK_IDX_DLL2)
#define SF32LB_MPI2_CLK_REQUIRES_DLL1(inst)                                                        \
	(SF32LB_MPI2_CLK_SRC_IDX(inst) == SF32LB_MPI_CLK_IDX_DLL1)
#define SF32LB_USB_CLK_REQUIRES_DLL2(inst) (SF32LB_USB_CLK_SRC_IDX(inst) == SF32LB_USB_CLK_IDX_DLL2)
...
{
	return (config->sys_clk_src == SF32LB_SYS_CLK_VALUE_HXT48) ||
	       (config->peri_clk_src == SF32LB_PERI_CLK_VALUE_HXT48) || (config->dll1_freq != 0U) ||
	       (config->dll2_freq != 0U);
}
...
{
	uint32_t div = config->hdiv;

	if (div == 0U) {
		return sf32lb_get_sys_clk(config);
	}

	return sf32lb_get_sys_clk(config) / div;
}
...
{
	switch (src) {
	case SF32LB_MPI_CLK_VALUE_DLL1:
		return config->dll1_freq;
	case SF32LB_MPI_CLK_VALUE_DLL2:
...
{
	uint32_t src = (config->usb_clk_src == SF32LB_USB_CLK_VALUE_DLL2)
			       ? config->dll2_freq
...
static int clock_control_sf32lb_rcc_init(const struct device *dev)
{
	...
	if (config->dll1_freq != 0U || config->dll2_freq != 0U) {
		...
		val |= FIELD_PREP(HPSYS_RCC_CSR_SEL_SYS_Msk, SF32LB_SYS_CLK_VALUE_HXT48) |
		       FIELD_PREP(HPSYS_RCC_CSR_SEL_PERI_Msk, SF32LB_PERI_CLK_VALUE_HXT48);
