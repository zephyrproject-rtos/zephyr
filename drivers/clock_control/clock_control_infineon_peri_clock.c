...
/* PSOC 4 has no instance/group fields to pack and never calls this helper, so
 * it is built only on the parts that pack the destination.
 */
...
#else
	/* PSOC 4 has no instance/group fields, so the destination is just 0. */
	clk_dst = 0;
#endif
