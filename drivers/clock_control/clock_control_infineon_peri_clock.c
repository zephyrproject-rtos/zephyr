...
static inline en_clk_dst_t ifx_cat1_utils_peri_pclk_build_en_clk_dst(uint8_t output, uint8_t group,
								     uint8_t instance)
{
...
static int ifx_cat1_peri_clock_init(const struct device *dev)
{
	 ...
	 */
	clk_dst = ifx_cat1_utils_peri_pclk_build_en_clk_dst(0, data->clock.group,
							    data->clock.instance);
