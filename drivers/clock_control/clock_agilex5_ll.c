...
void clock_agilex5_ll_init(mm_reg_t base_addr)
{
	...
	clock_agilex5_ll.mainpll_addr =
						clock_agilex5_ll.base_addr + CLKMGR_MAINPLL_OFFSET;

	/* Clock manager peripheral PLL base address. */
	clock_agilex5_ll.peripll_addr =
						clock_agilex5_ll.base_addr + CLKMGR_PERPLL_OFFSET;

	/* Clock manager control module base address. */
	clock_agilex5_ll.ctl_addr =
						clock_agilex5_ll.base_addr + CLKMGR_INTEL_OFFSET;
}
...
uint32_t get_uart_clk(void)
{
	...
	reg = sys_read32(clock_agilex5_ll.mainpll_addr + CLKMGR_MAINPLL_NOCDIV);
...
uint32_t get_mmc_clk(void)
{
	...
	reg = sys_read32(clock_agilex5_ll.ctl_addr + CLKMGR_INTEL_SDMMCCTR);
