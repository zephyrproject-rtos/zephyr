...
typedef struct {
	...
	void *user_data;
	uint32_t flags;
} nrf_clock_control_hfclk_data_t;
...
typedef struct {
	...
	const char *name;
#endif
} nrf_clock_control_hfclk_config_t;
			...
			uint32_t ctx)
{
	...
	err = set_starting_state(&((nrf_clock_control_hfclk_data_t*)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((nrf_clock_control_hfclk_data_t*)dev->data)->cb = cb;
	((nrf_clock_control_hfclk_data_t*)dev->data)->user_data = user_data;

	((nrf_clock_control_hfclk_config_t*)dev->config)->start();
...
static int stop(const struct device *dev, uint32_t ctx)
{
	...
	err = set_off_state(&((nrf_clock_control_hfclk_data_t*)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((nrf_clock_control_hfclk_config_t*)dev->config)->stop();
				   ...
				   void *user_data)
{
	...
	notify(&((nrf_clock_control_hfclk_data_t*)dev->data)->mgr, 0);
}
...
{
	clock_control_cb_t callback = ((nrf_clock_control_hfclk_data_t*)dev->data)->cb;

	((nrf_clock_control_hfclk_data_t*)dev->data)->cb = NULL;
	set_on_state(&((nrf_clock_control_hfclk_data_t*)dev->data)->flags);
	DBG(dev, "Clock started");

	if (callback) {
		callback(dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, ((nrf_clock_control_hfclk_data_t*)dev->data)->user_data);
	}
}
...
static void clock_event_handler()
{
	...
	/* Check needed due to anomaly 201:
		...
		*/
	if (GET_STATUS(((nrf_clock_control_hfclk_data_t*)dev->data)->flags) == CLOCK_CONTROL_STATUS_STARTING) {
...
static void generic_hfclk_start(void)
{
	...
	if (hfclk_users & HF_USER_BT) {
		...
		if (type == NRF_CLOCK_HFCLK_HIGH_ACCURACY) {
			...
			set_on_state(&((nrf_clock_control_hfclk_data_t*)CLOCK_DEVICE_HFCLK->data)->flags);
		}
	}
					    ...
					    clock_control_subsys_t subsys)
{
	...
	return GET_STATUS(((nrf_clock_control_hfclk_data_t*)dev->data)->flags);
}
...
static int clk_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_CLOCK_CONTROL_NRF_DRIVER_CALIBRATION)) {
		nrf_clock_control_hfclk_data_t *data = ((nrf_clock_control_hfclk_data_t*)dev->data);

		z_nrf_clock_calibration_init(&data->mgr);
	}

	err = onoff_manager_init(&((nrf_clock_control_hfclk_data_t*)dev->data)->mgr,
					&transitions);
	if (err < 0) {
		return err;
	}

	((nrf_clock_control_hfclk_data_t*)dev->data)->flags = CLOCK_CONTROL_STATUS_OFF;
...
static nrf_clock_control_hfclk_data_t data;

static const nrf_clock_control_hfclk_config_t config = {
