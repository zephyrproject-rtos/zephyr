...
#define DT_DRV_COMPAT nordic_nrf_hfclk
...
#define CLOCK_DEVICE_HFCLK DEVICE_DT_GET(DT_NODELABEL(clock_hfclk))
...
#define ERR(dev, ...) CLOCK_LOG(ERR, dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, __VA_ARGS__)
#define WRN(dev, ...) CLOCK_LOG(WRN, dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, __VA_ARGS__)
#define INF(dev, ...) CLOCK_LOG(INF, dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, __VA_ARGS__)
#define DBG(dev, ...) CLOCK_LOG(DBG, dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, __VA_ARGS__)

struct nrf_clock_control_hfclk_data {
...
struct nrf_clock_control_hfclk_config {
			...
			uint32_t ctx)
{
	...
	err = set_starting_state(&dev->data->flags, ctx);
	if (err < 0) {
		return err;
	}

	dev->data->cb = cb;
	dev->data->user_data = user_data;
...
static int stop(const struct device *dev, uint32_t ctx)
{
	...
	err = set_off_state(&dev->data->flags, ctx);
	if (err < 0) {
		return err;
	}
...
{
	clock_control_cb_t callback = dev->data->cb;

	dev->data->cb = NULL;
	set_on_state(&dev->data->flags);
	DBG(dev, "Clock started");

	if (callback) {
		callback(dev, CLOCK_CONTROL_NRF_TYPE_HFCLK, dev->data->user_data);
	}
}
...
static void clock_event_handler()
{
	...
	/* Check needed due to anomaly 201:
		...
		*/
	if (GET_STATUS(dev->data->flags) == CLOCK_CONTROL_STATUS_STARTING) {
...
static void generic_hfclk_start(void)
{
	...
	if (hfclk_users & HF_USER_BT) {
		...
		if (type == NRF_CLOCK_HFCLK_HIGH_ACCURACY) {
			 ...
			 */
			set_on_state(&CLOCK_DEVICE_HFCLK->data->flags);
		}
	}
					    ...
					    clock_control_subsys_t subsys)
{
	...
	return GET_STATUS(dev->data->flags);
}
...
static int clk_init(const struct device *dev)
{
	...
	err = onoff_manager_init(&dev->data->mgr,
					&transitions);
	if (err < 0) {
		return err;
	}

	dev->data->flags = CLOCK_CONTROL_STATUS_OFF;
...
static struct nrf_clock_control_hfclk_data data;

static const struct nrf_clock_control_hfclk_config config = {
	.start = generic_hfclk_start,
	.stop = generic_hfclk_stop,
	IF_ENABLED(CONFIG_LOG, (.name = "hfclk",))
};

DEVICE_DT_DEFINE(DT_NODELABEL(clock_hfclk), clk_init, NULL,
		 &data, &config,
