...
static struct cpg_clk_info_table core_props[] = {
	...
	RCAR_CORE_CLK_INFO_ITEM(R8A779G0_CLK_CL16M, RCAR_CPG_NONE, RCAR_CPG_NONE,
				RCAR_CPG_KHZ(16660)),
	RCAR_CORE_CLK_INFO_ITEM(R8A779G0_CLK_SASYNCPERD1, RCAR_CPG_NONE, RCAR_CPG_NONE,
				266666666),
					    ...
					    struct cpg_clk_info_table *clk_info, uint32_t enable)
{
	...
	case R8A779G0_CLK_SD0:
		...
		reg &= ~(1 << R8A779G0_CLK_SD0_STOP_BIT);
		reg |= (!enable << R8A779G0_CLK_SD0_STOP_BIT);
		break;
	case R8A779G0_CLK_SD0H:
		reg = sys_read32(DEVICE_MMIO_GET(dev) + clk_info->offset);
		reg &= ~(1 << R8A779G0_CLK_SD0H_STOP_BIT);
...
int r8a779g0_cpg_mssr_start_stop(const struct device *dev, clock_control_subsys_t sys, bool enable)
{
	...
	if (clk->domain == CPG_MOD) {
		...
		ret = r8a779g0_cpg_core_clock_endisable(dev, clk, enable);
	} else {
		ret = -EINVAL;
	}
...
static int r8a779g0_cpg_mssr_start(const struct device *dev, clock_control_subsys_t sys)
{
	return r8a779g0_cpg_mssr_start_stop(dev, sys, true);
}

static int r8a779g0_cpg_mssr_stop(const struct device *dev, clock_control_subsys_t sys)
{
	return r8a779g0_cpg_mssr_start_stop(dev, sys, false);
}
...
static const struct clock_control_driver_api r8a779g0_cpg_mssr_api = {
	.on = r8a779g0_cpg_mssr_start,
	.off = r8a779g0_cpg_mssr_stop,
...
#define R8A779G0_MSSR_INIT(inst)							\
	...
	DEVICE_DT_INST_DEFINE(inst,						\
			      &r8a779g0_cpg_mssr_init,				\
			      NULL,						\
			      &cpg_mssr##inst##_data,				\
			      &cpg_mssr##inst##_cfg,				\
			      PRE_KERNEL_1,					\
			      CONFIG_CLOCK_CONTROL_INIT_PRIORITY,		\
			      &r8a779g0_cpg_mssr_api);
