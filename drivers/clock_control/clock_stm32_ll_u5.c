...
#ifdef CONFIG_PM
	/* Disable unused clocks that are enabled. These will not be enabled, unless the MCU uses them for PM wakeup purposes. */
	if (!IS_ENABLED(STM32_MSIS_ENABLED) && READ_BIT(RCC->CR, RCC_CR_MSISON)) {
		LL_RCC_MSIS_Disable();
	}

	if (!IS_ENABLED(STM32_HSI_ENABLED) && READ_BIT(RCC->CR, RCC_CR_HSION)) {
		LL_RCC_HSI_Disable();
	}
#endif
