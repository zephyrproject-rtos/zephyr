...
BUILD_ASSERT(STM32_PLL_R_ENABLED == STM32_PLL_DIVR_ENABLED,
	     "For the PLL, both div-r and div-divr must be present if one of them is present");
#endif /* STM32_PLL_ENABLED && RCC_DCKCFGR_PLLDIVR */

#if defined(STM32_PLLI2S_ENABLED) && defined(RCC_DCKCFGR_PLLI2SDIVQ)
BUILD_ASSERT(STM32_PLLI2S_Q_ENABLED == STM32_PLLI2S_DIVQ_ENABLED,
	     "For the PLLI2S, both div-q and div-divq must be present if one of them is present");
#endif /* STM32_PLLI2S_ENABLED && RCC_DCKCFGR_PLLI2SDIVQ */

#if defined(STM32_PLLI2S_ENABLED) && defined(RCC_DCKCFGR_PLLI2SDIVR)
BUILD_ASSERT(STM32_PLLI2S_R_ENABLED == STM32_PLLI2S_DIVR_ENABLED,
	     "For the PLLI2S, both div-r and div-divr must be present if one of them is present");
#endif /* STM32_PLLI2S_ENABLED && RCC_DCKCFGR_PLLI2SDIVR */

#if defined(STM32_PLLSAI_ENABLED)
BUILD_ASSERT(STM32_PLLSAI_Q_ENABLED == STM32_PLLSAI_DIVQ_ENABLED,
	     "For the PLLSAI, both div-q and div-divq must be present if one of them is present");
#endif /* STM32_PLLSAI_ENABLED */

#if defined(STM32_PLLSAI_ENABLED) && defined(RCC_PLLSAICFGR_PLLSAIR)
BUILD_ASSERT(STM32_PLLSAI_R_ENABLED == STM32_PLLSAI_DIVR_ENABLED,
...
void config_pll_sysclock(void)
{
	...
	LL_RCC_PLL_ConfigDomain_SAI(get_pll_source(),
				    ...
				    plldivr(STM32_PLL_DIVR_DIVISOR));
...
#error "PLLI2S doesn't have P output on this SOC"
	...
	LL_RCC_PLLI2S_ConfigDomain_SAI(get_plli2s_source(),
				       ...
				       plli2sdivq(STM32_PLLI2S_DIVQ_DIVISOR));
...
#error "PLLI2S doesn't have Q output on this SOC"
	...
	LL_RCC_PLLI2S_ConfigDomain_SAI(get_plli2s_source(),
				       ...
				       plli2sdivr(STM32_PLLI2S_DIVR_DIVISOR));
