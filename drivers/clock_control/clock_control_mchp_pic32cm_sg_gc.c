...
#define DT_DRV_COMPAT microchip_pic32cm_sg_gc_clock
LOG_MODULE_REGISTER(clock_mchp_pic32cm_sg_gc, CONFIG_CLOCK_CONTROL_LOG_LEVEL);
...
#define GCLK_IO_MIN                0
#define GCLK_IO_MAX                7
#define GCLK_PH_MAX                38
...
/* mclk bus */
#define MMASKREG_MAX (1)

/* XOSC32K instances */
#define INST_XOSC32K_XOSC1K  0
#define INST_XOSC32K_XOSC32K 1

/* GCLKPERIPH ids for clocks */
#define DPLL_GCLKPERIPH_ID    1
#define DFLL48M_GCLKPERIPH_ID 0
...
static int clock_get_rate_dpll(const struct device *dev, uint32_t *freq);
...
static int clock_check_subsys(union clock_mchp_subsys subsys)
{
	...
	if (subsys.val == (uint32_t)CLOCK_CONTROL_SUBSYS_ALL) {
		LOG_ERR("%s: invalid subsys value %d", __func__, subsys.val);
		return -EINVAL;
	}

	/* Check if the specified subsystem was found. */
	if (subsys.bits.type > SUBSYS_TYPE_MAX) {
		LOG_ERR("%s: invalid subsys type %d", __func__, subsys.bits.type);
		return -EINVAL;
	}
	...
	case SUBSYS_TYPE_DFLL48M:
		...
		gclkperiph_max = DFLL48M_GCLKPERIPH_ID;
		break;

	case SUBSYS_TYPE_DPLL:
		inst_max = CLOCK_MCHP_DPLL_ID_MAX;
		gclkperiph_max = DPLL_GCLKPERIPH_ID;
		break;
...
int clock_on_dpll(const struct device *dev)
{
	...
	if (WAIT_FOR(((oscctrl_regs->OSCCTRL_STATUS & OSCCTRL_STATUS_PLL0LOCK_Msk) != 0),
		     TIMEOUT_DPLL_LOCK, NULL) == false) {
		LOG_ERR("%s: DPLL0 lock timed out", __func__);
		return -ETIMEDOUT;
	}
...
int clock_on_dpll_out(const struct device *dev, int inst)
{
	...
	*(&(oscctrl_regs->OSCCTRL_PLL0POSTDIVA) + (inst / PLLOUT_COUNT_PER_REG)) |= BIT(pos_en);

	/* Set dpll_out status as on */
	data->gclkgen_src_on_status |= BIT(CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT0 + inst);

	/* Switch on DPLL, if not already on. */
	if ((data->dpll_on_status == 0) && (data->dpll_on_request != 0)) {
		return clock_on_dpll(dev);
	}
...
static int clock_on(const struct device *dev, const union clock_mchp_subsys subsys)
{
	...
	case SUBSYS_TYPE_DPLL:
		...
		if ((data->gclkgen_src_on_status &
		     (BIT_MASK(5) << CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT0)) != 0) {
			ret_val = clock_on_dpll(dev);
		} else {
			data->dpll_on_request = 1;
		}
		break;
	...
	default:
		LOG_ERR("%s: non-supported clock type %d", __func__, subsys.bits.type);
		ret_val = -ENOTSUP;
		break;
	}
				  ...
				  enum clock_mchp_gclk_src_clock gclkgen_called_src, uint32_t *freq)
{
	...
	if (gclkgen_called_src == gclkgen_src) {
		...
		if ((gclkgen_src > CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT0) &&
		    (gclkgen_src <= CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT4)) {
			LOG_ERR("%s: invalid gclkgen_src %d", __func__, gclkgen_src);
			return -ENOTSUP;
		}
	} else {
		/* Else.. Added to pass compliance check */
	}
	...
	case CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT4:
...
static int clock_get_rate_dpll(const struct device *dev, uint32_t *freq)
{
	...
	if (clock_mchp_get_status(dev, (clock_control_subsys_t)MCHP_CLOCK_DERIVE_ID(
					       ...
					       DPLL_GCLKPERIPH_ID, 0)) != CLOCK_CONTROL_STATUS_ON) {
		*freq = 0;
		return 0;
	}

	ref_clk_type = (oscctrl_regs->OSCCTRL_PLL0CTRL & OSCCTRL_PLL0CTRL_REFSEL_Msk) >>
		       OSCCTRL_PLL0CTRL_REFSEL_Pos;

	switch (ref_clk_type) {
	case OSCCTRL_PLL0CTRL_REFSEL_GCLK_Val:
		src_gclkgen = (config->gclk_regs->GCLK_PCHCTRL[DPLL_GCLKPERIPH_ID] &
			       GCLK_PCHCTRL_GEN_Msk) >>
			      GCLK_PCHCTRL_GEN_Pos;
		ret_val = clock_get_rate_gclkgen(dev, src_gclkgen,
						 CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT0, &src_freq);
		break;
	...
	default:
		LOG_ERR("%s: Unexpected ref_clk_type %d", __func__, ref_clk_type);
		break;
	}
	...
	refdiv = (oscctrl_regs->OSCCTRL_PLL0REFDIV & OSCCTRL_PLL0REFDIV_REFDIV_Msk) >>
		 OSCCTRL_PLL0REFDIV_REFDIV_Pos;
	if (refdiv != 0) {
		src_freq = src_freq / refdiv;
	}

	fbdiv = (oscctrl_regs->OSCCTRL_PLL0FBDIV & OSCCTRL_PLL0FBDIV_FBDIV_Msk) >>
...
static int clock_get_rate_dpll_out(const struct device *dev, uint8_t dpll_out_id, uint32_t *freq)
{
	...
	ret_val = clock_get_rate_dpll(dev, &src_freq);
	if (ret_val == 0) {
		postdiv = *(&oscctrl_regs->OSCCTRL_PLL0POSTDIVA +
			    (dpll_out_id / PLLOUT_COUNT_PER_REG)) &
			  (OSCCTRL_PLL0POSTDIVA_POSTDIV0_Msk
			   << ((dpll_out_id % PLLOUT_COUNT_PER_REG) * PLLOUT_POSTDIV_SPAN));
		if (postdiv != 0) {
...
/* configure DPLL. */
static void clock_configure_dpll(const struct device *dev, void *req_config)
{
	const struct clock_mchp_config *config = dev->config;
	oscctrl_registers_t *oscctrl_regs = config->oscctrl_regs;
	gclk_registers_t *gclk_regs = config->gclk_regs;
	...
	src = dpll_config->src;
	if (src <= CLOCK_MCHP_DPLL_SRC_GCLK11) {
		gclk_regs->GCLK_PCHCTRL[DPLL_GCLKPERIPH_ID] |=
			(GCLK_PCHCTRL_GEN(src) | GCLK_PCHCTRL_CHEN_Msk);
		if (WAIT_FOR(((gclk_regs->GCLK_PCHCTRL[DPLL_GCLKPERIPH_ID] &
	...
	/* Configure feedback division factor */
	val32 = oscctrl_regs->OSCCTRL_PLL0FBDIV & ~(OSCCTRL_PLL0FBDIV_FBDIV_Msk);
	val32 |= OSCCTRL_PLL0FBDIV_FBDIV(dpll_config->feedback_divider_factor);
	oscctrl_regs->OSCCTRL_PLL0FBDIV = val32;

	/* Configure reference division factor */
	val32 = oscctrl_regs->OSCCTRL_PLL0REFDIV & ~(OSCCTRL_PLL0REFDIV_REFDIV_Msk);
	val32 |= OSCCTRL_PLL0REFDIV_REFDIV(dpll_config->ref_division_factor);
	oscctrl_regs->OSCCTRL_PLL0REFDIV = val32;

	/* Configure reference source select and ondemand bit */
	val32 = oscctrl_regs->OSCCTRL_PLL0CTRL &
		~(OSCCTRL_PLL0CTRL_REFSEL_Msk | OSCCTRL_PLL0CTRL_ONDEMAND_Msk);
	val32 |= OSCCTRL_PLL0CTRL_REFSEL(
		(src > CLOCK_MCHP_DPLL_SRC_GCLK11) ? (src - CLOCK_MCHP_DPLL_SRC_GCLK11) : 0);
	val32 |= OSCCTRL_PLL0CTRL_ONDEMAND(dpll_config->on_demand_en);
	oscctrl_regs->OSCCTRL_PLL0CTRL = val32;
}
...
static void clock_configure_dpll_out(const struct device *dev, uint8_t inst, void *req_config)
{
	...
	uint32_t val32, pos_postdiv, reg_offset;

	pos_postdiv = (inst % PLLOUT_COUNT_PER_REG) * PLLOUT_POSTDIV_SPAN;
	reg_offset = inst / PLLOUT_COUNT_PER_REG;

	val32 = *(&oscctrl_regs->OSCCTRL_PLL0POSTDIVA + reg_offset);
	val32 &= ~((BIT_MASK(6) << pos_postdiv));
	val32 |= (dpll_out_config->output_division_factor << pos_postdiv);
	*(&(oscctrl_regs->OSCCTRL_PLL0POSTDIVA) + reg_offset) = val32;
}
...
static int clock_mchp_on(const struct device *dev, clock_control_subsys_t sys)
{
	...
	if (0 != clock_check_subsys(subsys)) {
		LOG_ERR("%s: non-supported clock subsystem", __func__);
		return -ENOTSUP;
	}
	...
	if (status == CLOCK_CONTROL_STATUS_ON) {
		/* clock is already on. */
		LOG_INF("%s: clock is already on", __func__);
		return -EALREADY;
	}
...
static int clock_mchp_off(const struct device *dev, clock_control_subsys_t sys)
{
	...
	case SUBSYS_TYPE_DPLL:
		oscctrl_regs->OSCCTRL_PLL0CTRL &= ~OSCCTRL_PLL0CTRL_ENABLE_Msk;
		data->dpll_on_request = 0;
		data->dpll_on_status = 0;
		break;
	...
	case SUBSYS_TYPE_DPLL_OUT:
		...
		*(&(oscctrl_regs->OSCCTRL_PLL0POSTDIVA) + reg_offset) &= ~BIT(pos_en);
	...
	case SUBSYS_TYPE_GCLKGEN:
		...
		if (inst == CLOCK_MCHP_GCLKGEN_GEN0) {
			/* GCLK GEN0 is always on */
			LOG_ERR("%s: can not turn off GCLK GEN0", __func__);
			return -ENOTSUP;
		}
						       ...
						       clock_control_subsys_t sys)
{
	...
	case SUBSYS_TYPE_DPLL_OUT:
		/* Check if DPLL is enabled */
		if ((oscctrl_regs->OSCCTRL_STATUS & OSCCTRL_STATUS_PLL0LOCK_Msk) != 0) {
			/* Find the bit position for the specified PLLOUT */
			mask = BIT((((inst % PLLOUT_COUNT_PER_REG) + 1) * PLLOUT_POSTDIV_SPAN) - 1);
			ret_status = ((*(&oscctrl_regs->OSCCTRL_PLL0POSTDIVA +
					 (inst / PLLOUT_COUNT_PER_REG)) &
				       mask) != 0)
					     ? CLOCK_CONTROL_STATUS_ON
...
static int clock_mchp_get_rate(const struct device *dev, clock_control_subsys_t sys, uint32_t *freq)
{
	...
	case SUBSYS_TYPE_DPLL:
		ret_val = clock_get_rate_dpll(dev, freq);
		break;
				...
				void *req_config)
{
	...
	case SUBSYS_TYPE_DPLL:
		clock_configure_dpll(dev, req_config);
		break;
...
void clock_dpll_init(const struct device *dev, struct clock_dpll_init *dpll_init)
{
	...
	if ((data->dpll_src_on_status & BIT(src)) == 0) {
		LOG_INF("%s: source clock (driving DPLL_0) is off. (Maximum init retry = %d)",
			__func__, CLOCK_INIT_ITERATION_COUNT);
		return;
	}
	...
	/* Configure reference source select and ondemand bit */
	val32 = oscctrl_regs->OSCCTRL_PLL0CTRL &
		~(OSCCTRL_PLL0CTRL_BWSEL_Msk | OSCCTRL_PLL0CTRL_REFSEL_Msk |
...
void clock_dpll_out_init(const struct device *dev, struct clock_dpll_out_init *dpll_out_init)
{
	...
	if ((data->gclkgen_src_on_status & BIT(CLOCK_MCHP_GCLK_SRC_DPLL0_CLKOUT0 + inst)) != 0) {
		LOG_INF("%s: skipping dpll_0_out_%d_init, as it is already on", __func__, inst);
		return;
	}
	...
	*(&(oscctrl_regs->OSCCTRL_PLL0POSTDIVA) + reg_offset) = val32;
	...
	if ((dpll_out_init->output_en == 0) || (clock_on_dpll_out(dev, inst) != 0)) {
		LOG_INF("%s: skipping dpll_0_out_%d_init, as driving DPLL is off", __func__, inst);
		return;
	}
...
#define CLOCK_MCHP_ITERATE_DPLL_OUT(child)                                                         \
	{                                                                                          \
		struct clock_dpll_out_init dpll_out_init = {0};                                    \
		dpll_out_init.subsys.val = DT_PROP(child, subsystem);                              \
		dpll_out_init.output_en = DT_PROP(child, dpll_output_en);                          \
		dpll_out_init.output_division_factor =                                             \
			DT_PROP(child, dpll_output_division_factor);                               \
		clock_dpll_out_init(dev, &dpll_out_init);                                          \
	}

#define CLOCK_MCHP_PROCESS_DPLL0(node)                                                             \
	{                                                                                          \
		struct clock_dpll_init dpll_init = {0};                                            \
		dpll_init.feedback_divider_factor = DT_PROP(node, dpll_feedback_divider_factor);   \
		dpll_init.ref_division_factor = DT_PROP(node, dpll_ref_division_factor);           \
		dpll_init.bandwidth_sel = DT_ENUM_IDX(node, dpll_bandwidth_sel);                   \
		dpll_init.src = DT_ENUM_IDX(node, dpll_src);                                       \
		dpll_init.on_demand_en = DT_PROP(node, dpll_on_demand_en);                         \
		dpll_init.enable = DT_PROP(node, dpll_en);                                         \
