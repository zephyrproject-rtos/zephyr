...
typedef struct {
	...
	void *user_data;
	uint32_t flags;
} nrf_clock_control_lfclk_data_t;
...
typedef struct {
	...
	const char *name;
#endif
} nrf_clock_control_lfclk_config_t;
					    ...
					    clock_control_subsys_t subsys)
{
	...
	return GET_STATUS(((nrf_clock_control_lfclk_data_t*)dev->data)->flags);
}
...
{
	clock_control_cb_t callback = ((nrf_clock_control_lfclk_data_t*)dev->data)->cb;
	void *user_data = ((nrf_clock_control_lfclk_data_t*)dev->data)->user_data;

	((nrf_clock_control_lfclk_data_t*)dev->data)->cb = NULL;
	set_on_state(&((nrf_clock_control_lfclk_data_t*)dev->data)->flags);
...
static void lfclk_start(void)
{
	...
	struct onoff_manager *mgr;
	(void)nrf_clock_control_get_onoff(CLOCK_DEVICE_HF, &mgr);

	(void)onoff_request(mgr, &lfsynth_cli);
#endif
...
static void lfclk_stop(void)
{
	...
	(void)onoff_cancel_or_release(mgr, &lfsynth_cli);
#endif
}
...
static int stop(const struct device *dev, uint32_t ctx)
{
	...
	err = set_off_state(&((nrf_clock_control_lfclk_data_t*)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((nrf_clock_control_lfclk_config_t*)dev->config)->stop();
                    ...
                    void *user_data, uint32_t ctx)
{
	...
	err = set_starting_state(&((nrf_clock_control_lfclk_data_t*)dev->data)->flags, ctx);
	if (err < 0) {
		return err;
	}

	((nrf_clock_control_lfclk_data_t*)dev->data)->cb = cb;
	((nrf_clock_control_lfclk_data_t*)dev->data)->user_data = user_data;

	((nrf_clock_control_lfclk_config_t*)dev->config)->start();
				   ...
				   void *user_data)
{
	...
	notify(&((nrf_clock_control_lfclk_data_t*)dev->data)->mgr, 0);
}
...
void z_nrf_clock_control_lf_on(enum nrf_lfclk_start_mode start_mode)
{
	...
	if (atomic_set(&on, 1) == 0) {
		int err;
		struct onoff_manager *mgr = 
			&((nrf_clock_control_lfclk_data_t *)CLOCK_DEVICE_LFCLK->data)->mgr;
...
static int clk_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_CLOCK_CONTROL_NRF_DRIVER_CALIBRATION)) {
        nrf_clock_control_lfclk_data_t *data = ((nrf_clock_control_lfclk_data_t*)dev->data);

		z_nrf_clock_calibration_init(&data->mgr);
	}

    err = onoff_manager_init(&((nrf_clock_control_lfclk_data_t*)dev->data)->mgr,
                    &transitions);
    if (err < 0) {
        return err;
    }

    ((nrf_clock_control_lfclk_data_t*)dev->data)->flags = CLOCK_CONTROL_STATUS_OFF;
...
static struct nrf_clock_control_lfclk_data_t data;

static const struct nrf_clock_control_lfclk_config_t config = {
