...
#define RC64MPLL_FREQ		(STM32_HSE_FREQ * DT_PROP(DT_NODELABEL(pll), clock_mult)	\
					 / DT_PROP(DT_NODELABEL(pll), clock_div))
#else
#define RC64MPLL_FREQ		STM32_HSI_FREQ
#endif
...
#define STM32_WL3_CLKROOT_FREQ	RC64MPLL_FREQ
...
#if defined(STM32_SYSCLK_SRC_PLL)
BUILD_ASSERT(DT_SAME_NODE(DT_CLOCKS_CTLR(DT_NODELABEL(pll)), DT_NODELABEL(clk_hse)),
	"the RC64MPLL block can only be driven by HSE in PLL mode");
#endif
...
{
	switch (src_clk) {
	case STM32_SRC_CLK_ROOT:
	case STM32_SRC_CLKSLOWMUX:
	case STM32_SRC_CLK_ROOT_DIV:
...
{
	return LL_RCC_DIRECT_HSE_IsEnabled() ? STM32_HSE_FREQ : RC64MPLL_FREQ;
}
...
static uint32_t get_clk_sys_freq(void)
{
	...
	return RC64MPLL_FREQ >> clksysdiv;
}
