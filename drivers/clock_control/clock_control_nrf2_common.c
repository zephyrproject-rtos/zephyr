...
LOG_MODULE_REGISTER(clock_control_nrf2);
...
static void update_config(struct clock_config_generic *cfg)
{
	...
	/* If the update work is already scheduled (FLAG_UPDATE_NEEDED was set
	 * before the above OR operation) or it is currently executed, it is
	 * not to be submitted again. In the latter case, it will be submitted
	 * by clock_config_update_end().
	 */
...
uint32_t clock_config_update_begin(struct k_work *work)
{
	...
	return cfg->flags_snapshot & BIT_MASK(ONOFF_CNT_MAX);
}
