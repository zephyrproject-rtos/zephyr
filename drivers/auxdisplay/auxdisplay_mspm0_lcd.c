...
/* voltage-source property → LCDVCTL base bits (indexed by MSPM0_LCD_VOLTAGE_SOURCE_*) */
...
/* bias-mode property → LCDVCTL bits (indexed by MSPM0_LCD_BIAS_MODE_*) */
...
static bool mspm0_lcd_write_symbol(const struct device *dev, uint8_t ch)
{
	...
	gap = cfg->panel_config.rotated ? (uint8_t)(cols - 1u - data->cursor_x)
...
static int mspm0_lcd_init(const struct device *dev)
{
	...
	/* LCDCTL0: mux + divider + low-power waveform (always on for static and 1/3-bias).
	 * LCDLP is only valid when LCDBIASSEL=0 (bias_mode != MSPM0_LCD_BIAS_MODE_1_4); hardware
	 * ignores it otherwise, but we set it unconditionally for those modes as it is always
	 * beneficial.
	 */
	uint32_t ctl0 = FIELD_PREP(LCD_LCDCTL0_LCDMXX_MSK, cfg->mux_mode) |
			FIELD_PREP(LCD_LCDCTL0_LCDDIVX_MSK, cfg->freq_divider);

	if (cfg->bias_mode != MSPM0_LCD_BIAS_MODE_1_4) {
		ctl0 |= LCD_LCDCTL0_LCDLP;
	}
	regs->lcdctl0 = ctl0;

	/* LCDVCTL: voltage mode + bias + optional modifier bits. */
	uint32_t vctl = voltage_mode_vctl[cfg->voltage_source] | bias_vctl[cfg->bias_mode];
	...
	if (!WAIT_FOR((regs->lcdmemctl & (LCD_LCDMEMCTL_LCDCLRM | LCD_LCDMEMCTL_LCDCLRBM)) == 0,
		      1000, k_busy_wait(1))) {
		LOG_WRN("LCD memory clear timed out");
	}
...
#define MSPM0_LCD_INIT(n)                                                                          \
	...
	static const struct mspm0_lcd_cfg mspm0_lcd_cfg_##n = {                                    \
		...
		.voltage_boost = DT_INST_PROP(n, voltage_boost),                                   \
		.bias_mode = DT_INST_PROP(n, bias_mode),                                           \
		.voltage_source = DT_INST_PROP(n, voltage_source),                                 \
