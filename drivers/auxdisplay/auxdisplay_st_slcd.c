...
static int auxdisplay_st_slcd_clear(const struct device *dev)
{
	...
	return st_slcd_clear_and_update(dev);
}
...
static int auxdisplay_st_slcd_indicator_set(const struct device *dev, uint8_t index, bool enable)
{
	 ...
	 * data to the glass.
	 */
	return st_slcd_update_request(dev);
}
...
{
	const struct auxdisplay_st_slcd_config *config = dev->config;
	const struct auxdisplay_st_slcd_data *data = dev->data;
	uint16_t pattern;

	if (ascii_char < data->custom_character_slot_used) {
		pattern = data->custom_character_patterns[ascii_char];
	} else {
		pattern = slcd_14seg_convert_ascii_char_to_14seg_pattern(config->panel_config,
									 ascii_char);
	}
...
static int auxdisplay_st_slcd_write(const struct device *dev, const uint8_t *text, uint16_t len)
{
	...
	for (int i = 0, position = data->cursor_y * cols + data->cursor_x;
		...
		if (data->cursor_x < cols - 1) {
			...
			/* The cursor stays at the last available position, and this text is
			 * truncated; the next write operation will write one character at this
			 * position.
			 */
			break;
		}
...
#define AUXDISPLAY_ST_SLCD_DEVICE(inst)                                                            \
	...
	static struct auxdisplay_st_slcd_data auxdisplay_st_slcd_data_##inst = {                   \
		...
		.hlcd.Init.Duty = DT_INST_STRING_TOKEN(inst, lcd_duty),                            \
		.hlcd.Init.Bias = DT_INST_STRING_TOKEN(inst, lcd_bias),                            \
		.hlcd.Init.MuxSegment = DT_INST_STRING_TOKEN(inst, lcd_mux_segment),               \
		.hlcd.Init.VoltageSource = DT_INST_STRING_TOKEN(inst, lcd_voltage_source),         \
		.hlcd.Init.Prescaler = DT_INST_STRING_TOKEN(inst, lcd_prescaler),                  \
		.hlcd.Init.Divider = DT_INST_STRING_TOKEN(inst, lcd_divider),                      \
		.hlcd.Init.Contrast = DT_INST_STRING_TOKEN(inst, lcd_contrast),                    \
		.hlcd.Init.PulseOnDuration = DT_INST_STRING_TOKEN(inst, lcd_pulse_on_duration),    \
		.hlcd.Init.DeadTime = DT_INST_STRING_TOKEN(inst, lcd_dead_time),                   \
		.hlcd.Init.HighDrive = DT_INST_STRING_TOKEN(inst, lcd_high_drive),                 \
		.hlcd.Init.BlinkMode = DT_INST_STRING_TOKEN(inst, lcd_blink_mode),                 \
		.hlcd.Init.BlinkFrequency = DT_INST_STRING_TOKEN(inst, lcd_blink_frequency),       \
	...
	static const struct stm32_pclken auxdisplay_st_slcd_pclken_##inst[] = {                    \
		STM32_DT_INST_CLOCK_INFO_BY_NAME(inst, bus)};                                      \
                                                                                                   \
