...
static fsp_err_t flash_renesas_ra_ospi_b_write_enable(ospi_b_instance_ctrl_t *p_ctrl)
{
	...
	if (WEN_BIT_MASK != (transfer.data & WEN_BIT_MASK)) {
...
static fsp_err_t flash_renesas_ra_ospi_b_setup_calibrate_data(ospi_b_instance_ctrl_t *p_ctrl)
{
	...
	if (memcmp((uint8_t *)APP_ADDRESS(SECTOR_THREE), &autocalibration_data,
		...
		err = R_OSPI_B_Erase(p_ctrl, (uint8_t *)APP_ADDRESS(SECTOR_THREE), SECTOR_SIZE_4K);
		if (err != FSP_SUCCESS) {
			return err;
		}
					 ...
					 const uintptr_t in, void *out)
{
	...
	case FLASH_EX_OP_RESET:
		...
		/* Enable reset */
		transfer =
			(SPI_FLASH_PROTOCOL_EXTENDED_SPI == ospi_b_data->ospi_b_ctrl.spi_protocol)
...
static const struct flash_driver_api flash_renesas_ra_ospi_b_api = {
...
static struct flash_renesas_ra_ospi_b_data ospi_b_data = {
	...
	.ospi_b_high_speed_command_set = {.protocol = SPI_FLASH_PROTOCOL_8D_8D_8D,
					  ...
					  .read_dummy_cycles = DUMMY_CYCLE_READ_MEMORY_OSPI,
					  .program_dummy_cycles = DUMMY_CYCLE_WRITE_OSPI,
					  .status_dummy_cycles = DUMMY_CYCLE_READ_REGISTER_OSPI,
	...
	.ospi_b_config_extend = {.channel = OSPI_B_DEVICE_NUMBER_1,
				 ...
				 .read_dummy_cycles = DUMMY_CYCLE_READ_MEMORY_SPI,
				 .program_dummy_cycles = DUMMY_CYCLE_WRITE_SPI,
