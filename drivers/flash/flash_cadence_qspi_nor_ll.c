...
int cad_qspi_idle(cad_qspi_params_t *cad_params)
{
	if (cad_params == NULL) {
		printk("ERROR: Wrong parameter\n");
		return -EINVAL;
	}
...
int cad_qspi_set_baudrate_div(cad_qspi_params_t *cad_params, uint32_t div)
{
	if (cad_params == NULL) {
		printk("ERROR: Wrong parameter\n");
		return -EINVAL;
	...
	sys_clear_bits(cad_params->reg_base + CAD_QSPI_CFG,
			~CAD_QSPI_CFG_BAUDDIV_MSK);

	sys_set_bits(cad_params->reg_base + CAD_QSPI_CFG,
			CAD_QSPI_CFG_BAUDDIV(div));

	return 0;
}

int cad_qspi_configure_dev_size(cad_qspi_params_t *cad_params, uint32_t addr_bytes,
...
int cad_qspi_set_read_config(cad_qspi_params_t *cad_params,
		uint32_t opcode, uint32_t instr_type,
		...
		uint32_t mode_bit, uint32_t dummy_clk_cycle)
{
	...
	sys_write32(CAD_QSPI_DEV_OPCODE(opcode) |
...
int cad_qspi_set_write_config(cad_qspi_params_t *cad_params,
		uint32_t opcode, uint32_t addr_type,
...
int cad_qspi_timing_config(cad_qspi_params_t *cad_params,
		uint32_t clkphase, uint32_t clkpol, uint32_t csda,
...
int cad_qspi_stig_cmd_helper(cad_qspi_params_t *cad_params, int cs, uint32_t cmd)
{
	uint32_t count = 0;
	...
	/* chip select */
	sys_write32((sys_read32(cad_params->reg_base + CAD_QSPI_CFG) &
	...
	sys_write32(cmd | CAD_QSPI_FLASHCMD_EXECUTE,
			cad_params->reg_base + CAD_QSPI_FLASHCMD);

	do {
		uint32_t reg = sys_read32(cad_params->reg_base +
					CAD_QSPI_FLASHCMD);
	...
	if (count >= CAD_QSPI_COMMAND_TIMEOUT) {
		printk("ERROR: Error sending QSPI command %x, timed out\n",
				cmd);
...
int cad_qspi_stig_cmd(cad_qspi_params_t *cad_params, uint32_t opcode,
		uint32_t dummy)
{
	...
	if (dummy > ((1 << CAD_QSPI_FLASHCMD_NUM_DUMMYBYTES_MAX) - 1)) {
		printk("ERROR: Faulty dummy bytes\n");
		return -1;
	}
...
int cad_qspi_stig_read_cmd(cad_qspi_params_t *cad_params, uint32_t opcode,
		uint32_t dummy, uint32_t num_bytes, uint32_t *output)
{
	if (dummy > ((1 << CAD_QSPI_FLASHCMD_NUM_DUMMYBYTES_MAX) - 1)) {
		printk("ERROR: Faulty dummy byes\n");
		return -1;
	}
	...
	if (cad_qspi_stig_cmd_helper(cad_params,
			cad_params->cad_qspi_cs, cmd)) {
		printk("ERROR: failed to send stig cmd\n");
		return -1;
	}
	...
	if (num_bytes > 4) {
		output[1] = sys_read32(cad_params->reg_base +
...
int cad_qspi_stig_wr_cmd(cad_qspi_params_t *cad_params, uint32_t opcode,
		uint32_t dummy, uint32_t num_bytes, uint32_t *input)
{
	...
	if (num_bytes > 4)
		sys_write32(input[1], cad_params->reg_base +
...
int cad_qspi_stig_addr_cmd(cad_qspi_params_t *cad_params, uint32_t opcode,
...
int cad_qspi_device_bank_select(cad_qspi_params_t *cad_params, uint32_t bank)
{
	int status = 0;
	...
	status = cad_qspi_stig_wr_cmd(cad_params, CAD_QSPI_STIG_OPCODE_WREN_EXT_REG,
			0, 1, &bank);
	if (status != 0)
		return status;
...
int cad_qspi_device_status(cad_qspi_params_t *cad_params, uint32_t *status)
{
	return cad_qspi_stig_read_cmd(cad_params,
		CAD_QSPI_STIG_OPCODE_RDSR, 0, 1, status);
}

#if CAD_QSPI_MICRON_N25Q_SUPPORT
int cad_qspi_n25q_enable(cad_qspi_params_t *cad_params)
{
	cad_qspi_set_read_config(cad_params, QSPI_FAST_READ,
...
int cad_qspi_n25q_wait_for_program_and_erase(cad_qspi_params_t *cad_params,
				int program_only)
{
	...
	while (count < CAD_QSPI_COMMAND_TIMEOUT) {
		...
		if (status != 0) {
			printk("ERROR: Error getting device status\n");
			return -1;
		}
	...
	if (count >= CAD_QSPI_COMMAND_TIMEOUT) {
		printk("ERROR: Timed out waiting for idle\n");
		return -1;
	}
	...
	while (count < CAD_QSPI_COMMAND_TIMEOUT) {
		status = cad_qspi_stig_read_cmd(cad_params, CAD_QSPI_STIG_OPCODE_RDFLGSR,
				0, 1, &flag_sr);
		if (status != 0) {
			printk("ERROR: Error waiting program and erase.\n");
			return status;
		}
	...
	if (count >= CAD_QSPI_COMMAND_TIMEOUT)
		printk("ERROR: Timed out waiting for program and erase\n");

	if ((program_only && CAD_QSPI_STIG_FLAGSR_PROGRAMERROR(flag_sr)) ||
			(!program_only &&
			CAD_QSPI_STIG_FLAGSR_ERASEERROR(flag_sr))) {
		printk("ERROR: Error programming/erasing flash\n");
...
int cad_qspi_indirect_read_start_bank(cad_qspi_params_t *cad_params,
			uint32_t flash_addr, uint32_t num_bytes)
{
	...
	sys_write32(CAD_QSPI_INDRD_START |
...
int cad_qspi_indirect_write_start_bank(cad_qspi_params_t *cad_params,
				uint32_t flash_addr,
				uint32_t num_bytes)
{
	if (cad_params == NULL) {
	...
	sys_write32(CAD_QSPI_INDWR_START |
...
int cad_qspi_indirect_write_finish(cad_qspi_params_t *cad_params)
{
...
int cad_qspi_enable(cad_qspi_params_t *cad_params)
{
	int status;
...
int cad_qspi_enable_subsector_bank(cad_qspi_params_t *cad_params, uint32_t addr)
{
	int status = 0;
	...
	status = cad_qspi_stig_addr_cmd(cad_params,
				CAD_QSPI_STIG_OPCODE_SUBSEC_ERASE, 0, addr);
	if (status != 0)
		return status;
...
int cad_qspi_erase_subsector(cad_qspi_params_t *cad_params, uint32_t addr)
{
	int status = 0;
...
int cad_qspi_erase_sector(cad_qspi_params_t *cad_params, uint32_t addr)
{
	int status = 0;
	...
	status = cad_qspi_stig_addr_cmd(cad_params, CAD_QSPI_STIG_OPCODE_SEC_ERASE,
					0, addr);
	if (status != 0)
		return status;
...
void cad_qspi_calibration(cad_qspi_params_t *cad_params, uint32_t dev_clk,
			uint32_t qspi_clk_mhz)
{
	...
	status = cad_qspi_stig_read_cmd(cad_params, CAD_QSPI_STIG_OPCODE_RDID,
					0, 3, &sample_rdid);
	if (status != 0)
		return;
	...
	do {
		if (status != 0)
			break;
		status = cad_qspi_stig_read_cmd(cad_params,
		...
		sys_write32(CAD_QSPI_RDDATACAP_BYP(1) |
...
int cad_qspi_int_disable(cad_qspi_params_t *cad_params, uint32_t mask)
{
	if (cad_params == NULL) {
		printk("ERROR: Wrong parameter\n");
		return -EINVAL;
...
int cad_qspi_init(cad_qspi_params_t *cad_params, uint32_t clk_phase,
			uint32_t clk_pol, uint32_t csda,
			uint32_t csdads, uint32_t cseot,
			uint32_t cssot, uint32_t rddatacap)
{
	int status = 0;
	...
	if (cad_qspi_idle(cad_params) == 0) {
		printk("ERROR: device not idle\n");
		return -EBUSY;
	}
	status = cad_qspi_timing_config(cad_params, clk_phase, clk_pol,
					csda, csdads, cseot, cssot, rddatacap);

	if (status != 0) {
		printk("ERROR: config set timing failure\n");
		return status;
	}

	sys_write32(CAD_QSPI_REMAPADDR_VALUE_SET(0),
			cad_params->reg_base + CAD_QSPI_REMAPADDR);

	status = cad_qspi_int_disable(cad_params, CAD_QSPI_INT_STATUS_ALL);
	if (status != 0) {
		printk("ERROR: failed disable\n");
		return status;
	}
	...
	if (status != 0) {
		printk("ERROR: failed enable\n");
		return status;
	}

	qspi_desired_clk_freq = 100;
	cad_qspi_calibration(cad_params, qspi_desired_clk_freq, 50000000);

	status = cad_qspi_stig_read_cmd(cad_params, CAD_QSPI_STIG_OPCODE_RDID, 0, 3,
					&rdid);

	if (status != 0) {
		printk("ERROR: Error reading RDID\n");
		return status;
	}
	...
	if (!(((cap_code >> 4) > 0x9) || ((cap_code & 0xf) > 0x9))) {
		...
		printk("INFO: QSPI Capacity: %x\n\n", cad_params->qspi_device_size);

	} else {
		printk("ERROR: Invalid CapacityID encountered: 0x%02x\n",
				cap_code);
		return -1;
	}

	cad_qspi_configure_dev_size(cad_params, QSPI_ADDR_BYTES,
				QSPI_BYTES_PER_DEV,
				QSPI_BYTES_PER_BLOCK);

	printk("INFO: Flash size: %d Bytes\n", cad_params->qspi_device_size);

	return status;
}

int cad_qspi_indirect_page_bound_write(cad_qspi_params_t *cad_params, uint32_t offset,
		uint8_t *buffer, uint32_t len)
{
	...
	write_count = 0;
	sram_partition = CAD_QSPI_SRAMPART_ADDR(sys_read32(cad_params->reg_base +
			 CAD_QSPI_SRAMPART));
	...
	while (write_count < len) {
		write_fill_level = CAD_QSPI_SRAMFILL_INDWRPART(
					sys_read32(cad_params->reg_base +
							CAD_QSPI_SRAMFILL));
...
int cad_qspi_read_bank(cad_qspi_params_t *cad_params, uint8_t *buffer,
...
int cad_qspi_write_bank(cad_qspi_params_t *cad_params, uint32_t offset,
		uint8_t *buffer, uint32_t size)
{
	...
	while (size) {
		status = cad_qspi_indirect_page_bound_write(cad_params, offset,
							buffer, write_size);
		if (status != 0)
			break;
		...
		uint32_t offset, uint32_t  size)
{
	...
	if ((offset >= cad_params->qspi_device_size) ||
			(offset + size - 1 >= cad_params->qspi_device_size) ||
			(size == 0)) {
		printk("ERROR: Invalid read parameter\n");
		return -EINVAL;
	}

	if (CAD_QSPI_INDRD_RD_STAT(sys_read32(cad_params->reg_base +
						CAD_QSPI_INDRD))) {
		printk("ERROR: Read in progress\n");
		return -ENOTBLK;
	}
	...
	for (i = 0; i < bank_count; ++i) {
		status = cad_qspi_device_bank_select(cad_params, CAD_QSPI_BANK_ADDR(
								bank_addr));
		if (status != 0)
			break;

		status = cad_qspi_read_bank(cad_params, read_data,
...
int cad_qspi_erase(cad_qspi_params_t *cad_params, uint32_t offset, uint32_t size)
{
	int status = 0;
	...
	uint32_t offset, uint32_t size)
{
	...
	if (CAD_QSPI_INDWR_RDSTAT(sys_read32(cad_params->reg_base +
						CAD_QSPI_INDWR))) {
		printk("ERROR: QSPI Error: Write in progress\n");
		return -ENOTBLK;
	}
	...
	for (i = 0; i < bank_count; ++i) {
		status = cad_qspi_device_bank_select(cad_params,
				CAD_QSPI_BANK_ADDR(bank_addr));
		if (status != 0)
			break;

		status = cad_qspi_write_bank(cad_params, bank_offset,
					write_data, copy_len);
		if (status != 0)
			break;
...
void cad_qspi_reset(cad_qspi_params_t *cad_params)
{
