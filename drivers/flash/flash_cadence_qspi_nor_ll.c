					    ...
					    uint32_t offset, uint32_t size)
{
	...
	if (size >= sizeof(uint32_t)) {
		level = CAD_QSPI_SRAMFILL_INDRDPART(sys_read32(cad_params->reg_base +
							       CAD_QSPI_SRAMFILL));
				       ...
				       uint32_t offset, uint32_t size)
{
	...
	while (remaining_bytes) {
		...
		byte_transferred = cad_qspi_read_data_using_interrupt(cad_params,
								      read_req->buffer,
								      read_req->offset,
								      remaining_bytes);
				      ...
				      uint32_t offset, uint8_t *buffer, uint32_t len)
{
	...
	while (write_count < write_size) {
		write_fill_level = CAD_QSPI_SRAMFILL_INDWRPART(sys_read32(cad_params->reg_base +
									  CAD_QSPI_SRAMFILL));
						       ...
						       uint32_t len)
{
	...
	while (write_req->requested_bytes > write_req->transferred_bytes) {
		byte_written = cad_qspi_indirect_write_page_data(cad_params,
								 write_req->offset,
								 write_req->buffer,
								 (write_req->requested_bytes -
								 write_req->transferred_bytes));
