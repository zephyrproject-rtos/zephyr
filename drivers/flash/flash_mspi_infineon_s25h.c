...
struct flash_mspi_infineon_s25h_cfg {
	...
	uint16_t jedec_device_id;
	uint8_t jedec_manufacturer_id;

	bool stay_in_startup_mspi_config;
};
					 ...
					 size_t size)
{
	...
	struct mspi_xfer xfer = {
		...
		.packets = &packet,
		.num_packet = 1,
		/* 20 milliseconds + 10 ms per 4KiB; for 256 KiB this results in 660 ms */
		.timeout = (size * 10 / 4096) + 20,
	};
...
static int flash_mspi_infineon_s25h_erase(const struct device *dev, off_t addr, size_t size)
{
	...
	const struct mspi_xfer xfer_erase = {
		...
		.num_packet = 1,
		/* 20 milliseconds + 4 ms per 16KiB; for 256 KiB this results in 84 ms */
		.timeout = (size * 4 / 16384) + 20,
	};
...
{
	const struct flash_mspi_infineon_s25h_cfg *cfg = dev->config;
	uint8_t id[3];
	int ret;
	...
	if (manufacturer_id != cfg->jedec_manufacturer_id || device_id != cfg->jedec_device_id) {
		LOG_ERR("Read JEDEC ids don't match expected ids. The communication is possibly "
...
static int flash_mspi_infineon_s25h_init(const struct device *dev)
{
	...
	/* Switch into 4S-4S-4S mode, if not deactivated */
	if (!config->stay_in_startup_mspi_config) {
...
#define INFINEON_MSPI_FLASH_S25H_CHECK_DEVICETREE_CONFIG(n)                                        \
	...
	INFINEON_MSPI_FLASH_S25H_CHECK_PROP_IS_UNDEFINED(n, memmap_config);                        \
...
#define INFINFEON_MSPI_FLASH_S25H_DEFINE(n)                                                        \
	...
	static const struct flash_mspi_infineon_s25h_cfg flash_mspi_infineon_s25h_cfg_##n = {      \
		...
		.jedec_device_id = DT_PROP(DT_DRV_INST(n), device_id),                             \
		.jedec_manufacturer_id = DT_PROP(DT_DRV_INST(n), manufacturer_id),                 \
		.stay_in_startup_mspi_config = DT_PROP(DT_DRV_INST(n), keep_startup_mspi_config),  \
	};                                                                                         \
