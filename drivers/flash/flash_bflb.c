...
typedef struct {
	char magic[4];
	uint32_t revision;
} __packed bflb_flash_magic_1;

typedef struct {
	char magic[4];
} __packed bflb_flash_magic_2;
...
typedef struct {
	bflb_flash_magic_1 magic_1;
	bflb_flash_magic_2 magic_2;
	bflb_flash_cfg flash_cfg;
	uint32_t flash_cfg_crc;
} __packed bflb_flash_header;
...
static int flash_bflb_send_command(struct flash_bflb_data *data, bflb_flash_command *command)
{
	...
	tmp = flash_bflb_set_command_sahb(data, command, true);
	if (tmp != 0) {
		return tmp;
	}
...
static int flash_bflb_enable_qspi(struct flash_bflb_data *data)
{
	...
	if (data->flash_cfg.qe_read_reg_len == 0) {
		...
		flash_bflb_flash_write_register(data, data->flash_cfg.qe_index, (uint8_t *)&tmp,
	...
	/* qe is a specific value, not encountered in available flash chip configs  */
		...
		if (((tmp >> (data->flash_cfg.qe_bit & 0x08)) & 0xff) ==
	...
	flash_bflb_flash_write_register(data, data->flash_cfg.qe_index, (uint8_t *)&tmp,
	data->flash_cfg.qe_write_reg_len);
	flash_bflb_flash_read_register(data, data->flash_cfg.qe_index, (uint8_t *)&tmp,
	data->flash_cfg.qe_write_reg_len);
...
static void flash_bflb_set_offset(uint32_t base_reg, uint32_t offset)
{
	...
	*((uint32_t *)(base_reg + SF_CTRL_SF_ID0_OFFSET_OFFSET)) = tmp;
}
			     ...
			     size_t length)
{
	...
	flash_bflb_flash_read_register(data, 0, (uint8_t *)(&tmp), 1);
	if ((tmp & 0x1C) != 0) {
		return -EINVAL;
	}
...
static int flash_bflb_config_init(const struct device *dev)
{
	...
	uint32_t img_offset;
	unsigned int locker;
	bflb_flash_header header;

	if (flash_bflb_guard(&flash_bflb_config_init)) {
		return -ENOTSUP;
	}
	...
	flash_bflb_xip_memcpy((uint8_t *)(BFLB_XIP_BASE),
			      (uint8_t *)&header, sizeof(bflb_flash_header));
	...
	tmp = bflb_soft_crc32(0, (uint8_t *)(&header.flash_cfg), sizeof(bflb_flash_cfg));
	if (tmp != header.flash_cfg_crc) {
		LOG_ERR("Flash data crc is incorrect %d vs %d", tmp, img_offset);
		return -EINVAL;
	}
	flash_bflb_xip_memcpy((uint8_t *)&(header.flash_cfg),
			      (uint8_t *)&(data->flash_cfg), sizeof(bflb_flash_cfg));
