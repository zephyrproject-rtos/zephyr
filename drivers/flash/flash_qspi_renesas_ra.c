...
#include "r_qspi.h"
#include "flash_qspi_renesas_ra.h"
...
static struct qspi_flash_ra_data qspi_flash_data = {
	.qspi_cfg = {
		...
		.page_program_command = QSPI_CMD_PAGE_PROGRAM,
		.write_enable_command = QSPI_CMD_WRITE_ENABLE,
		.status_command = QSPI_CMD_READ_STATUS,
...
static int get_flash_status(const struct device *dev)
{
	...
	do {
		...
		if (err != FSP_SUCCESS) {
			LOG_ERR("qspi: status get failed");
			return -EIO;
		}
		--time_out;
...
static int qspi_flash_ra_read_jedec_id(const struct device *dev, uint8_t *id)
{
	...
	if (SPI_FLASH_PROTOCOL_QPI == qspi_data->qspi_cfg.spi_protocol) {
		cmd = QSPI_CMD_READ_ID;
	} else {
		cmd = Ex_SPI_CMD_READ_ID;
	}
	...
	if (err != FSP_SUCCESS) {
		LOG_ERR("qspi: direct read failed");
		err = -EIO;
		goto out;
	}
	...
	if (err != FSP_SUCCESS) {
		LOG_ERR("qspi: failed to get status for QSPI operation");
		err = -EIO;
	}
out:
	release_device(dev);
...
static int qspi_flash_ra_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	if ((rc != 0) || (offset != page_info_start.start_offset)) {
		LOG_ERR("qspi: The offset 0x%lx is not aligned with the starting sector",
			(long)offset);
		return -EINVAL;
	}
	...
	if ((rc != 0) || ((offset + len) != page_info_end.start_offset)) {
		LOG_ERR("qspi: The size %u is not aligned with the ending sector", len);
		return -EINVAL;
	}
...
static int set_qspi_flash_status(const struct device *dev)
{
	...
	if (SET_SREG_VALUE != sreg_data) {
		LOG_ERR("qspi: verify status register data failed");
		return -EIO;
	}
	return ret;
}
...
static int qspi_flash_ra_init(const struct device *dev)
{
	...
	if (ret) {
		LOG_ERR("qspi: failed to configure pins for QSPI");
		return -EIO;
	}
	...
	if (ret) {
		LOG_ERR("qspi: set qspi flash status failed");
		return -EIO;
	}
#if QSPI_ENABLE_QUAD_MODE
	uint8_t data_qpi_en = QSPI_CMD_ENTER_QPI_MODE;
	...
	if (ret) {
		LOG_ERR("qspi: set SpiProtocol failed");
		return -EIO;
	}
#endif
