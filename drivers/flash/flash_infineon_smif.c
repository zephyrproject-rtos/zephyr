...
#define IFX_SMIF_CONTROLLER_INST(n)                                                   \
	...
	BUILD_ASSERT(!DT_INST_PROP(n, not_pre_initialized) ||                             \
			     (DT_INST_NODE_HAS_PROP(n, peri_nr) &&                       \
			      ...
			      DT_INST_NODE_HAS_PROP(n, clock_hf_nr)),                    \
		     "not-pre-initialized SMIF requires peri-nr, peri-group-nr, "         \
		     "peri-slave-nr and clock-hf-nr");                                    \
                                                                                      \
	...
	static const struct ifx_smif_controller_config ifx_smif_controller_config_##n = {   \
		...
		.clk_group_nr = DT_INST_PROP_OR(n, peri_group_nr, 0),                           \
		.clk_slave_nr = DT_INST_PROP_OR(n, peri_slave_nr, 0),                           \
		.clk_hf_nr = DT_INST_PROP_OR(n, clock_hf_nr, 0),                                \
	};                                                                                  \
                                                                                        \
