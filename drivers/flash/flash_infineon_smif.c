 ...
 * "infineon,hw-init", e.g. when the first stage boots from internal RRAM or
...
static int ifx_smif_controller_init(const struct device *dev)
{
	 ...
	 * infineon,hw-init, so that chip-select and data lines for devices the
...
#define IFX_SMIF_HYBRID_REGION(i, child)                                                 \
	{                                                                                    \
		.regionAddress = DT_PROP_BY_IDX(child, infineon_hybrid_region_address, i),       \
		.sectorsCount = DT_PROP_BY_IDX(child, infineon_hybrid_region_sectors, i),        \
		.eraseCmd = DT_PROP_BY_IDX(child, infineon_hybrid_region_erase_command, i),      \
		.eraseSize = DT_PROP_BY_IDX(child, infineon_hybrid_region_erase_size, i),        \
		.eraseTime = DT_PROP_BY_IDX(child, infineon_hybrid_region_erase_time_ms, i),     \
	},
#define IFX_SMIF_HYBRID_PTR(i, child) &child##_hybrid[i],
#define IFX_SMIF_HYBRID_DEFINE(child)                                                   \
	COND_CODE_1(                                                                        \
		DT_NODE_HAS_PROP(child, infineon_hybrid_region_address),                        \
		(static cy_stc_smif_hybrid_region_info_t child##_hybrid[] = {LISTIFY(           \
			 DT_PROP_LEN(child, infineon_hybrid_region_address),                        \
			 IFX_SMIF_HYBRID_REGION, (), child)};                                       \
		 static cy_stc_smif_hybrid_region_info_t *child##_hybrid_ptrs[] = {LISTIFY(     \
			 DT_PROP_LEN(child, infineon_hybrid_region_address), IFX_SMIF_HYBRID_PTR,   \
			 (), child)};),                                                             \
		())
#define IFX_SMIF_HYBRID_CFG(child)                                                      \
	COND_CODE_1(DT_NODE_HAS_PROP(child, infineon_hybrid_region_address),                \
		    (.hybridRegionCount = DT_PROP_LEN(child, infineon_hybrid_region_address),   \
 ...
 * "infineon,protocol" devicetree enum. Only the read command differs between
 ...
 */
#define IFX_SMIF_IS_SINGLE(child) DT_ENUM_HAS_VALUE(child, infineon_protocol, single_spi)
...
#define IFX_SMIF_CHILD_NOR_DEFINE(child)                                               \
	...
	static cy_stc_smif_mem_cmd_t child##_erase_cmd = {                                 \
		.command = DT_PROP(child, infineon_erase_command),                       \
		.cmdWidth = CY_SMIF_WIDTH_SINGLE,                                              \
		.addrWidth = CY_SMIF_WIDTH_SINGLE,                                             \
		.mode = CY_SMIF_NO_COMMAND_OR_MODE,                                            \
		.modeWidth = CY_SMIF_WIDTH_SINGLE,                                             \
	...
	static cy_stc_smif_mem_cmd_t child##_program_cmd = {                               \
		.command = DT_PROP(child, infineon_program_command),                      \
		.cmdWidth = CY_SMIF_WIDTH_SINGLE,                                             \
		.addrWidth = CY_SMIF_WIDTH_SINGLE,                                            \
		.mode = CY_SMIF_NO_COMMAND_OR_MODE,                                           \
		.modeWidth = CY_SMIF_WIDTH_SINGLE,                                            \
	...
	static cy_stc_smif_mem_device_cfg_t child##_dev_cfg = {                           \
		.numOfAddrBytes = DT_PROP(child, infineon_address_bytes),                     \
		...
		.programSize = DT_PROP(child, infineon_page_size),                         \
		...
		.eraseTime = DT_PROP(child, infineon_erase_time_ms),                       \
		.chipEraseTime = DT_PROP(child, infineon_chip_erase_time_ms),           \
		.programTime = DT_PROP(child, infineon_program_time_us),                  \
		IFX_SMIF_HYBRID_CFG(child)                                                    \
	};                                                                                \
	static cy_stc_smif_mem_config_t child##_mem_cfg = {                               \
		.slaveSelect =                                                              \
			(cy_en_smif_slave_select_t)BIT(DT_PROP(child, infineon_chip_select)), \
		.flags = CY_SMIF_FLAG_SMIF_REV_3 | CY_SMIF_FLAG_WR_EN,                        \
		.dataSelect = (cy_en_smif_data_select_t)DT_PROP(child, infineon_data_select), \
...
#define IFX_SMIF_CONTROLLER_INST(n)                                                   \
	...
	static const struct ifx_smif_controller_config ifx_smif_controller_config_##n = {   \
		...
		.input_freq_mhz = DT_INST_PROP(n, infineon_input_frequency_mhz),                \
		.deselect_delay = DT_INST_PROP(n, infineon_deselect_delay),                     \
