...
#include <init.h>
#include <string.h>
#include "spi_flash_mx25r643f_defs.h"
#include "spi_flash_mx25r643f.h"
...
static inline int spi_flash_wb_id(struct device *dev)
{
	...
	if (spi_flash_wb_access(driver_data, MX25R6435F_CMD_RDID,
	...
	if (temp_data != CONFIG_SPI_FLASH_MX25R6435F_DEVICE_ID) {
		return -ENODEV;
	}
...
static inline void wait_for_flash_idle(struct device *dev)
{
	...
	do {
		reg = spi_flash_wb_reg_read(dev, MX25R6435F_CMD_RDSR);
	} while (reg & MX25R6435F_WIP_BIT);
}
			     ...
			     size_t len)
{
	...
	ret = spi_flash_wb_access(driver_data, MX25R6435F_CMD_READ,
			      ...
			      const void *data, size_t len)
{
	...
	/* Assume write protection has been disabled. Note that MX25R6435F
	 * flash automatically turns on write protection at the completion
	 * of each write or erase transaction.
	 */
	ret = spi_flash_wb_access(driver_data, MX25R6435F_CMD_PP,
...
static int spi_flash_wb_write_protection_set(struct device *dev, bool enable)
{
	...
	if (enable) {
		reg = MX25R6435F_CMD_WRDI;
	} else {
		reg = MX25R6435F_CMD_WREN;
	}
					      ...
					      off_t offset, size_t size)
{
	...
	/* write enable */
	spi_flash_wb_reg_write(dev, MX25R6435F_CMD_WREN);

	wait_for_flash_idle(dev);

	switch (size) {
	case MX25R6435F_SECTOR_SIZE:
		erase_opcode = MX25R6435F_CMD_SE;
		break;
	case MX25R6435F_BLOCK32K_SIZE:
		erase_opcode = MX25R6435F_CMD_BE32K;
		break;
	case MX25R6435F_BLOCK_SIZE:
		erase_opcode = MX25R6435F_CMD_BE;
		break;
	case CONFIG_SPI_FLASH_MX25R6435F_FLASH_SIZE:
		erase_opcode = MX25R6435F_CMD_CE;
		need_offset = false;
		break;
	default:
		return -EIO;
...
static int spi_flash_wb_erase(struct device *dev, off_t offset, size_t size)
{
	...
	if ((offset < 0) || ((offset & MX25R6435F_SECTOR_MASK) != 0) ||
	    ((size + offset) > CONFIG_SPI_FLASH_MX25R6435F_FLASH_SIZE) ||
	    ((size & MX25R6435F_SECTOR_MASK) != 0)) {
		return -ENODEV;
	}
	...
	while ((size_remaining >= MX25R6435F_SECTOR_SIZE) && (ret == 0)) {
		if (size_remaining == CONFIG_SPI_FLASH_MX25R6435F_FLASH_SIZE) {
			ret = spi_flash_wb_erase_internal(dev, offset, size);
			break;
		}

		if (size_remaining >= MX25R6435F_BLOCK_SIZE) {
			ret = spi_flash_wb_erase_internal(dev, new_offset,
				MX25R6435F_BLOCK_SIZE);
			new_offset += MX25R6435F_BLOCK_SIZE;
			size_remaining -= MX25R6435F_BLOCK_SIZE;
			continue;
		}

		if (size_remaining >= MX25R6435F_BLOCK32K_SIZE) {
			ret = spi_flash_wb_erase_internal(dev, new_offset,
				MX25R6435F_BLOCK32K_SIZE);
			new_offset += MX25R6435F_BLOCK32K_SIZE;
			size_remaining -= MX25R6435F_BLOCK32K_SIZE;
			continue;
		}

		if (size_remaining >= MX25R6435F_SECTOR_SIZE) {
			ret = spi_flash_wb_erase_internal(dev, new_offset,
				MX25R6435F_SECTOR_SIZE);
			new_offset += MX25R6435F_SECTOR_SIZE;
			size_remaining -= MX25R6435F_SECTOR_SIZE;
			continue;
		}
	}
...
static int spi_flash_wb_configure(struct device *dev)
{
	...
	data->spi = device_get_binding(CONFIG_SPI_FLASH_MX25R6435F_SPI_NAME);
	if (!data->spi) {
		return -EINVAL;
	}

	data->spi_cfg.frequency = CONFIG_SPI_FLASH_MX25R6435F_SPI_FREQ_0;
	data->spi_cfg.operation = SPI_WORD_SET(8);
	data->spi_cfg.slave = CONFIG_SPI_FLASH_MX25R6435F_SPI_SLAVE;

#if defined(CONFIG_SPI_FLASH_MX25R6435F_GPIO_SPI_CS)
	data->cs_ctrl.gpio_dev = device_get_binding(
		CONFIG_SPI_FLASH_MX25R6435F_GPIO_SPI_CS_DRV_NAME);
	if (!data->cs_ctrl.gpio_dev) {
		return -ENODEV;
	}

	data->cs_ctrl.gpio_pin = CONFIG_SPI_FLASH_MX25R6435F_GPIO_SPI_CS_PIN;
	data->cs_ctrl.delay = CONFIG_SPI_FLASH_MX25R6435F_GPIO_CS_WAIT_DELAY;
...
DEVICE_INIT(spi_flash_memory, CONFIG_SPI_FLASH_MX25R6435F_DRV_NAME,
	    spi_flash_init, &spi_flash_memory_data, NULL, POST_KERNEL,
	    CONFIG_SPI_FLASH_MX25R6435F_INIT_PRIORITY);
