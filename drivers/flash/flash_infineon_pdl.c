			   ...
			   size_t remaining_len)
{
	...
	if ((write_offset + remaining_len) > cfg->max_addr) {
		return -EINVAL;
	}
...
static int flash_ifx_read(const struct device *dev, off_t offset, void *data, size_t remaining_len)
{
	...
	if ((read_addr + remaining_len) > cfg->max_addr) {
		return -EINVAL;
	}

	memcpy(data, (const void *)read_addr, remaining_len);
...
DEVICE_DT_INST_DEFINE(0, NULL, NULL, NULL, &ifx_flash_config_parameters, POST_KERNEL,
		      CONFIG_FLASH_INIT_PRIORITY, &flash_api);
