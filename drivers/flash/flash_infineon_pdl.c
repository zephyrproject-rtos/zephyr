...
/* Cache/prefetch invalidation */
#define IFX_FLASH_CACHE_NONE          0
#define IFX_FLASH_CACHE_FLASHC_BUFFER 1
#define IFX_FLASH_CACHE_ICACHE0       2
#define IFX_FLASH_CACHE_INVALIDATE                                                                 \
	DT_INST_ENUM_IDX_OR(0, infineon_cache_invalidate, IFX_FLASH_CACHE_NONE)
...
{
#if IFX_FLASH_CACHE_INVALIDATE == IFX_FLASH_CACHE_FLASHC_BUFFER
	Cy_SysLib_ClearFlashCacheAndBuffer();
#elif IFX_FLASH_CACHE_INVALIDATE == IFX_FLASH_CACHE_ICACHE0
			   ...
			   size_t remaining_len)
{
	...
	__ASSERT((((uintptr_t)src_ptr) & (sizeof(uint32_t) - 1)) == 0,
		 "flash write buffer must be word-aligned");
	...
	while (remaining_len > 0) {
		...
		status = Cy_Flash_ProgramRow(write_offset, (const uint32_t *)src_ptr);
#else
		/* Auto-erases the row as part of the write. */
		status = Cy_Flash_WriteRow(write_offset, (const uint32_t *)src_ptr);
#endif
