...
#ifdef CONFIG_SOC_SERIES_STM32H7RSX
	uintptr_t cur =
		(uintptr_t)regs + offsetof(FLASH_TypeDef, WRPSR);
	uintptr_t prg =
		(uintptr_t)regs + offsetof(FLASH_TypeDef, WRPSRP);
#else
	uintptr_t cur =
		(uintptr_t)regs + offsetof(FLASH_TypeDef, WPSN_CUR1);
	uintptr_t prg =
		(uintptr_t)regs + offsetof(FLASH_TypeDef, WPSN_PRG1);
#endif

#ifdef DUAL_BANK
	if (bank == 1) {
		cur = (uint32_t)regs + offsetof(FLASH_TypeDef, WPSN_CUR2);
		prg = (uint32_t)regs + offsetof(FLASH_TypeDef, WPSN_PRG2);
	}
#endif /* DUAL_BANK */
...
#endif
	uint32_t sectors_per_bank = __builtin_popcount((uint32_t)bank_mask);
...
int flash_stm32_get_wp_sectors(const struct device *dev, uint64_t *protected_sectors)
{
	...
	*protected_sectors = (~cur & mask) >> pos;
	...
	uint64_t proctected_sectors_2 =
		(~regs->WPSN_CUR2 & mask) >> pos;
	uint32_t sectors_per_bank = __builtin_popcount(mask);
