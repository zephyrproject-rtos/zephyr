...
BUILD_ASSERT(((JESD216_DW15_QER_NONE == INST_0_QER)
	      || (JESD216_DW15_QER_S1B6 == INST_0_QER)
	      || (JESD216_DW15_QER_S2B1v6 == INST_0_QER)),
...
static int qspi_nrfx_configure(const struct device *dev)
{
	...
	if (ret == 0) {
		...
		if (JESD216_DW15_QER_S1B6 == INST_0_QER) {
			ret = qspi_rdsr(dev);
			if (ret < 0) {
		...
		} else if (JESD216_DW15_QER_S2B1v6 == INST_0_QER) {
			ret = qspi_rdsr2(dev);
			if (ret < 0) {
