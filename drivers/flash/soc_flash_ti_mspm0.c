...
#define FLASH_MSPM0_WRITE_BLOCK_SIZE DT_PROP_OR(MSPM0_FLASH_NODE, write_block_size, 0)
BUILD_ASSERT(FLASH_MSPM0_WRITE_BLOCK_SIZE != 0, "Flash Write Block size is required");
				...
				const void *buf, size_t len)
{
	...
	if (!IS_PTR_ALIGNED(buf, uint32_t)) {
		is_unaligned = true;
		write_ptr = aligned_data;
	}
	...
	while (len != 0) {
		if (!is_unaligned) {
			write_ptr = (uint32_t *)src;
		} else {
			memcpy(write_ptr, src, FLASH_MSPM0_WRITE_BLOCK_SIZE);
		}
		...
		src += FLASH_MSPM0_WRITE_BLOCK_SIZE;
	}
