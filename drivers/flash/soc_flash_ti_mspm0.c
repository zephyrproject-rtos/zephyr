...
#define MSPM0_FLASH_SIZE	(CONFIG_FLASH_SIZE * 1024)
#define MSPM0_FLASH_PAGE_SIZE	(CONFIG_SOC_FLASH_TI_MSPM0_LAYOUT_PAGE_SIZE)
...
static inline int flash_ti_mspm0_valid_range(off_t offset, size_t len)
{
	...
	if ((offset + len) > MSPM0_FLASH_SIZE) {
		return -EINVAL;
	}
				...
				off_t offset, size_t len)
{
	...
	if (len < MSPM0_FLASH_PAGE_SIZE) {
		LOG_ERR("Erase must be done in page length manner");
		return -EINVAL;
	}

	iter = len / MSPM0_FLASH_PAGE_SIZE;
	...
	do {
		...
		offset = offset + MSPM0_FLASH_PAGE_SIZE;
		iter--;
	} while (iter != 0);
				...
				const void *data, size_t len)
{
	...
	if (len % FLASH_MSPM0_WRITE_BLOCK_SIZE) {
		LOG_DBG("Length must be aligned to write block");
		return -EINVAL;
	}
