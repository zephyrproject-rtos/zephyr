...
static int flash_semihost_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	if (offset < 0 || offset > config->flash_size || (config->flash_size - offset) < len) {
		return -EINVAL;
	}
				...
				size_t len)
{
	...
	if (offset % config->flash_params->write_block_size != 0 ||
		len % config->flash_params->write_block_size != 0) {
		return -EINVAL;
	}
	...
	long semihost_ret = semihost_write(dev_data->fd, data, (long)len);
...
static int flash_semihost_erase(const struct device *dev, off_t offset, size_t size)
{
	...
	if (offset < 0 || offset > config->flash_size || (config->flash_size - offset) < size) {
		return -EINVAL;
	}
...
static int flash_semihost_init(const struct device *dev)
{
	...
	flen = semihost_flen(fd);
	if (flen < 0) {
		semihost_close(fd);
		return -EIO;
	}

	if (flen < config->flash_size) {
		/*
		 * The flash file is smaller than the expected flash size, extend it
		 * with 0xff
		 */
		uint8_t write_buf[ERASE_FILL_BUFFER_SIZE];
		/* Seek to first unprogrammed page */
		flen = ROUND_UP(flen, config->erase_block_size);

		ret = semihost_seek(fd, flen);
		if (ret != 0) {
			semihost_close(fd);
			return -EIO;
		}
		memset(write_buf, 0xff, ERASE_FILL_BUFFER_SIZE);

		while (flen < config->flash_size) {
			size_t chunk = MIN(ERASE_FILL_BUFFER_SIZE, config->flash_size - flen);

			ret = semihost_write(fd, write_buf, (long)chunk);
			if (ret != 0) {
				semihost_close(fd);
				return -EIO;
			}
...
#define FLASH_SEMIHOST_PAGES_LAYOUT_INIT(n)                                                        \
	static const struct flash_pages_layout flash_semihost_layout_##n[] = {                     \
		{                                                                                  \
			.pages_count = DT_REG_SIZE(SOC_NV_FLASH_CHILD_NODE(n)) /                   \
				       DT_PROP(SOC_NV_FLASH_CHILD_NODE(n), erase_block_size),      \
			.pages_size = DT_PROP(SOC_NV_FLASH_CHILD_NODE(n), erase_block_size),       \
		},                                                                                 \
	};

#define FLASH_SEMIHOST_PARAMS_INIT(n)                                                              \
	static const struct flash_parameters flash_semihost_params_##n = {                         \
		.write_block_size = DT_PROP(SOC_NV_FLASH_CHILD_NODE(n), write_block_size),       \
...
#define FLASH_SEMIHOST_DEVICE_INIT(n)                                                              \
	...
	static const struct flash_semihost_config flash_semihost_cfg_##n = {                       \
		...
		.flash_size = DT_REG_SIZE(SOC_NV_FLASH_CHILD_NODE(n)),                             \
		.erase_block_size = DT_PROP(SOC_NV_FLASH_CHILD_NODE(n), erase_block_size),         \
	};                                                                                         \
