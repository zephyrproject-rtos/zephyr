...
static int _nand_read(const struct device *dev, int page, void *buf,
...
static int _nand_write(const struct device *dev, int page, const void *buf,
		       int oob_req, int is_raw, int is_oob)
{
	const struct nand_config *cfg = dev->config;
	struct nand_data *data = dev->data;
...
int z_impl_nand_read_page(const struct device *dev, int page, void *buf, int oob_req)
{
	return _nand_read(dev, page, buf, oob_req, 0, 0);
}

int z_impl_nand_write_page(const struct device *dev, int page, const void *buf, int oob_req)
{
	return _nand_write(dev, page, buf, oob_req, 0, 0);
}

int z_impl_nand_read_page_raw(const struct device *dev, int page, void *buf, int oob_req)
{
	return _nand_read(dev, page, buf, oob_req, 1, 0);
}

int z_impl_nand_write_page_raw(const struct device *dev, int page, const void *buf, int oob_req)
{
	return _nand_write(dev, page, buf, oob_req, 1, 0);
}

int z_impl_nand_read_oob(const struct device *dev, int page, void *buf)
{
	return _nand_read(dev, page, buf, 0, 0, 1);
}

int z_impl_nand_write_oob(const struct device *dev, int page, const void *buf)
{
	return _nand_write(dev, page, buf, 0, 0, 1);
}

int z_impl_nand_erase(const struct device *dev, int block)
{
	const struct nand_config *cfg = dev->config;
	const struct nand_op *ops = cfg->ops;
	struct nand_data *data = dev->data;
