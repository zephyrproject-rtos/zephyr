...
#define DEVICE_NODE DT_CHILD(DT_INST(0, cdns_qspi_nor), flash_0)
...
{
	const struct flash_cad_config *config = DEV_CFG(dev);
...
#define CREATE_FLASH_CADENCE_QSPI_DEVICE(inst)                                                     \
	...
	static const struct flash_parameters flash_cad_parameters_##inst = {			\
	...
	static struct flash_cad_config flash_cad_config_##inst = {                                 \
		...
		.parameters = &flash_cad_parameters_##inst,					\
