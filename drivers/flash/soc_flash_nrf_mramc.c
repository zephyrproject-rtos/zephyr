...
#define DT_DRV_COMPAT nordic_mram_controller
...
/* write block size and erase block size is in the child node of
 * mramc DT_FOREACH_CHILD_STATUS_OKAY is used to get the values
 * from the first child node.
 */
static const uint32_t write_block_size_arr[] = {
	DT_INST_FOREACH_CHILD_STATUS_OKAY(0, _WBS)
};

static const uint32_t erase_block_size_arr[] = {
	DT_INST_FOREACH_CHILD_STATUS_OKAY(0, _EBS)
};

static const uint32_t write_block_size = write_block_size_arr[0];
static const uint32_t erase_block_size = erase_block_size_arr[0];
			 ...
			 const void *data, size_t len)
{
	...
	nrfx_mramc_words_write(addr, data, len / sizeof(uint32_t));
...
static void nrf_mramc_page_layout(const struct device *dev, const struct flash_pages_layout **layout,
				 size_t *layout_size)
{
	ARG_UNUSED(dev);
...
static int mramc_sys_init(const struct device *dev)
{
	...
	if ((erase_block_size % write_block_size) != 0) {
		LOG_ERR("Erase block size (%d) must be a multiple of write block size (%d)",
				erase_block_size, write_block_size);
		return -EINVAL;
	}

	NRF_APPLICATION_ICACHE_S->ENABLE = CACHE_ENABLE_ENABLE_Disabled;
