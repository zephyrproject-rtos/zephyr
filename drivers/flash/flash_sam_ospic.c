...
{
	uint32_t timeout_us = QSPI_SYNC_TIMEOUT;

	while ((qspi->QSPI_SR & QSPI_SR_SYNCBSY_Msk) && timeout_us--) {
		k_usleep(1);
	}
		...
		const struct qspi_mem_op *op, uint32_t offset)
{
	...
	uint32_t channel;
	struct dma_config *dma_cfg = &priv->dma_cfg;
	struct dma_block_config *dma_block_cfg = &priv->dma_block_cfg;
...
static int qspi_receive_data(struct qspi_priv *priv, const struct qspi_mem_op *op)
{
...
static int qspi_send_data(struct qspi_priv *priv, const struct qspi_mem_op *op)
{
	uint32_t timeout_ms = QSPI_TIMEOUT;
	uint32_t offset;
	qspi_registers_t *qspi;
...
static int qspi_transfer(struct qspi_priv *priv, const struct qspi_mem_op *op)
{
	...
	/* Send/Receive data. */
	if (op->data.dir == QSPI_MEM_DATA_IN) {
		ret = qspi_receive_data(priv, op);
	} else {
		ret = qspi_send_data(priv, op);
	}
	if (ret) {
		return ret;
	}
