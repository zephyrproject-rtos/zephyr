...
static struct memc_xspi_dev_config device_configs[] = {
	{
		...
		.xspi_dev_config = {
			...
			.ptrDeviceDdrConfig = &(xspi_device_ddr_config_t) {
		...
		.lut_count = sizeof(flash_xspi_lut) / sizeof(flash_xspi_lut[0][0])
	},
};
...
static int flash_mcux_xspi_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	DCACHE_InvalidateByRange((uint32_t)src, len);
...
static int flash_mcux_xspi_probe(const struct device *dev)
{
	...
	do {
		...
		ret = memc_xspi_set_device_config(xspi_dev, dev_config, flash_dev_config->lut_array, flash_dev_config->lut_count);
	} while (0);
...
static int flash_mcux_xspi_init(const struct device *dev)
{
	...
	return flash_mcux_xspi_enable_quad(dev);
}
...
#define FLASH_MCUX_XSPI_LAYOUT(n) \
	.layout.pages_size = DT_INST_PROP(n, erase_size), \
	.layout.pages_count = DT_INST_PROP(n, size) / DT_INST_PROP(n, erase_size),
#else
#define FLASH_MCUX_XSPI_LAYOUT(n)
#endif
