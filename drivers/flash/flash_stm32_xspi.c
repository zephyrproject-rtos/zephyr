...
#if CONFIG_FLASH_STM32_XSPI_DMA
...
static int flash_stm32_xspi_init(const struct device *dev)
{
	...
	defined(XSPIM) || defined(XSPIM1) || defined(XSPIM2)) && !defined(CONFIG_STM32_XSPIM)
...
#define XSPI_STM32_INIT(inst)									\
	...
	static const struct flash_stm32_xspi_config flash_stm32_xspi_cfg_##inst = {		\
		...
		IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, four_byte_opcodes), (			\
			.four_byte_opcodes = DT_INST_PROP(inst, four_byte_opcodes),		\
		))										\
		IF_ENABLED(DT_INST_NODE_HAS_PROP(st_stm32_ospi_nor, requires_ulbpr), (		\
			.requires_ulbpr = DT_INST_PROP(inst, requires_ulbpr),			\
		))										\
		IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, reset_gpios), (				\
			.reset = GPIO_DT_SPEC_INST_GET(0, reset_gpios),				\
			.reset_gpios_duration = DT_INST_PROP(inst, reset_gpios_duration),	\
		))										\
		IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, sfdp_bfp), (				\
			.sfdp_bfp = DT_INST_PROP(inst, sfdp_bfp),				\
		))										\
	};											\
												\
