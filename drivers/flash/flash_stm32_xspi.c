...
static int stm32_xspi_write_cfg2reg_dummy(XSPI_HandleTypeDef *hxspi, uint8_t nor_mode,
					  uint8_t nor_rate)
{
	...
	if (HAL_XSPI_Transmit(hxspi, &transmit_data, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
...
static int stm32_xspi_config_mem(const struct device *dev)
{
	...
	if (stm32_xspi_write_cfg2reg_dummy(&dev_data->hxspi, XSPI_SPI_MODE, XSPI_STR_TRANSFER) !=
	    0) {
...
static bool stm32_xspi_is_memorymap(const struct device *dev)
{
	...
	return ((READ_BIT(dev_data->hxspi.Instance->CR, XSPI_CR_FMODE) == XSPI_CR_FMODE) ? true
											 : false);
}
...
static int flash_stm32_xspi_read(const struct device *dev, off_t addr, void *data, size_t size)
{
	...
	LOG_DBG("XSPI: read %zu data at 0x%lx", size, (long)(STM32_XSPI_BASE_ADDRESS + addr));
	xspi_lock_thread(dev);
				  ...
				  size_t size)
{
	...
	LOG_DBG("XSPI: write %zu data at 0x%lx", size, (long)(STM32_XSPI_BASE_ADDRESS + addr));
...
static int flash_stm32_xspi_init(const struct device *dev)
{
	...
	LOG_INF("Memory-mapped NOR-flash at 0x%lx (0x%x bytes)", (long)(STM32_XSPI_BASE_ADDRESS),
		dev_cfg->flash_size);
#else
	LOG_INF("NOR external-flash at 0x%lx (0x%x bytes)", (long)(STM32_XSPI_BASE_ADDRESS),
		dev_cfg->flash_size);
#endif /* CONFIG_STM32_MEMMAP*/
	return 0;
}
...
DEVICE_DT_INST_DEFINE(0, &flash_stm32_xspi_init, NULL, &flash_stm32_xspi_dev_data,
		      &flash_stm32_xspi_cfg, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,
