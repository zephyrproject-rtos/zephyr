...
#define MPI_CCRX_CMD_BE_SE_4B MPI_CCRX_IMODE_SINGLE | MPI_CCRX_ADMODE_SINGLE | MPI_CCRX_ADSIZE_N(4U)
...
static __ramfunc int flash_sf32lb_mpi_qspi_nor_write(const struct device *dev, off_t offset,
						     const void *data_, size_t size)
{
	...
	while (size > 0) {
		...
		/* TODO: using page size seems to overrun FIFO? */
		chunk_len = MIN(size, MPI_FIFO_SIZE);
...
static __ramfunc int flash_sf32lb_mpi_qspi_nor_erase(const struct device *dev, off_t offset,
						     size_t size)
{
	struct flash_sf32lb_mpi_qspi_nor_data *data = dev->data;
	uint32_t ccrx;
...
static __ramfunc int flash_sf32lb_mpi_qspi_nor_init(const struct device *dev)
{
	...
	sys_write32(0xFFU, data->mpi + MPI_TIMR);
	sys_write32(0x50005000U, data->mpi + MPI_CIR);
	sys_write32(0xFFU, data->mpi + MPI_ABR1);
	sys_write32(0xFFU, data->mpi + MPI_HRABR);
	...
	/* configure row boundary size */
	val = sys_read32(data->mpi + MPI_DCR);
	val &= ~MPI_DCR_RBSIZE_Msk;
	val |= FIELD_PREP(MPI_DCR_RBSIZE_Msk, data->rb_size);
	sys_write32(val, data->mpi + MPI_DCR);
...
#define QSPI_NEEDS_4B_ADDR(n)                                                                      \
	(DT_REG_SIZE(DT_INST_PHANDLE(n, sifli_mapped_memory)) > QSPI_NOR_MAX_3B_SIZE)
...
#define FLASH_SF32LB_MPI_QSPI_NOR_DEFINE(n)                                                        \
	...
	static const struct flash_sf32lb_mpi_qspi_nor_config config##n = {                         \
		...
		.layout =                                                                          \
			{                                                                          \
				.pages_count =                                                     \
					DT_REG_SIZE(DT_INST_PHANDLE(n, sifli_mapped_memory)) /     \
					SPI_NOR_SECTOR_SIZE,                                       \
	...
	static struct flash_sf32lb_mpi_qspi_nor_data data##n = {                                   \
		.mpi = DT_INST_REG_ADDR(n),                                                        \
		.base = DT_REG_ADDR(DT_INST_PHANDLE(n, sifli_mapped_memory)),                      \
		.size = DT_REG_SIZE(DT_INST_PHANDLE(n, sifli_mapped_memory)),                      \
		.dma = SF32LB_DMA_DT_INST_SPEC_GET(n),                                             \
		.lines = DT_INST_PROP_OR(n, sifli_lines, 1U),                                      \
		.psclr = DT_INST_PROP(n, sifli_psclr),                                             \
		.rb_size = DT_INST_ENUM_IDX(n, sifli_row_boundary),                                \
