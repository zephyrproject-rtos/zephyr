...
#define DT_DRV_COMPAT microchip_qspi_g1_flash
...
#include "spi_nor.h"
#include "jesd216.h"
#include "flash_mchp_qspi_g1.h"

LOG_MODULE_REGISTER(FLASH_SAM_QSPI, CONFIG_FLASH_LOG_LEVEL);

struct flash_mchp_qspi_config {
...
/**
 * struct flash_mchp_nor_data - Structure to describe some SPI flash memory
...
flash_mchp_qspi_get_parameters(const struct device *dev)
{
	return &FLASH_SAM_QSPI_parameters;
}

static int flash_mchp_qspi_get_size(const struct device *dev, uint64_t *size)
{
	struct flash_mchp_nor_data *data = dev->data;
...
{
	const struct flash_mchp_qspi_config *config = dev->config;
	...
	memset(&hqspi.dma_cfg, 0, sizeof(struct dma_config));
	memset(&hqspi.dma_block_cfg, 0, sizeof(struct dma_block_config));
...
{
	struct flash_mchp_nor_data *data = dev->data;
	size_t flash_size = data->size;

	return ((addr >= 0) &&  (flash_size - size) >= addr);
}

static int flash_mchp_qspi_read(const struct device *dev, off_t addr,
...
static int flash_mchp_qspi_erase(const struct device *dev, off_t addr,
				  size_t size)
{
	const struct flash_mchp_qspi_config *config = dev->config;
	struct flash_mchp_nor_data *data = dev->data;
...
static int flash_mchp_qspi_write(const struct device *dev, off_t addr, const void *buf,
				       size_t size)
{
	const struct flash_mchp_qspi_config *config = dev->config;
	struct flash_mchp_nor_data *data = dev->data;
 ...
 */
static int mchp_qspi_read_jedec_id(const struct device *dev, uint8_t *id)
{
	const struct flash_mchp_qspi_config *config = dev->config;
	struct qspi_priv hqspi = {0};
	struct qspi_mem_op op = {0};
 ...
 */
static int mchp_qspi_read_sfdp(const struct device *dev, off_t addr, void *data,
			  size_t size)
{
	const struct flash_mchp_qspi_config *config = dev->config;
	struct qspi_priv hqspi = {0};
...
static int nor_sam_init_params(const struct device *dev)
{
	...
	ret = mchp_qspi_read_sfdp(dev, 0, u.raw, sizeof(u.raw));
	if (ret != 0) {
	...
	while (php != phpe) {
		...
		if (id == JESD216_SFDP_PARAM_ID_BFP) {
			...
			ret = mchp_qspi_read_sfdp(dev, jesd216_param_addr(php),
					     (uint8_t *)u2.dw,
...
static int flash_mchp_nor_init(const struct device *dev)
{
	const struct flash_mchp_qspi_config *config = dev->config;
	struct flash_mchp_nor_data *data = dev->data;
	struct qspi_priv hqspi = {0};
...
static DEVICE_API(flash, flash_mchp_qspi_api) = {
	.read = flash_mchp_qspi_read,
	.write = flash_mchp_qspi_write,
	.erase = flash_mchp_qspi_erase,
	.get_parameters = flash_mchp_qspi_get_parameters,
	.get_size = flash_mchp_qspi_get_size,
#if defined(CONFIG_FLASH_JESD216_API)
	.sfdp_read = mchp_qspi_read_sfdp,
	.read_jedec_id = mchp_qspi_read_jedec_id,
#endif /* CONFIG_FLASH_JESD216_API */
};

#define FLASH_SAM_QSPI(N)									\
	PINCTRL_DT_INST_DEFINE(N);								\
	static const struct flash_mchp_qspi_config						\
		flash_mchp_qspi_config_##N = {							\
	...
	static struct flash_mchp_nor_data flash_mchp_nor_data_##N = {};				\
	DEVICE_DT_INST_DEFINE(N,								\
			      flash_mchp_nor_init,						\
			      NULL,								\
			      &flash_mchp_nor_data_##N,						\
			      &flash_mchp_qspi_config_##N,					\
			      POST_KERNEL,							\
