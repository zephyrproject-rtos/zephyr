...
#define MSPM0_FLASH_COMPAT_NODE(node_id) \
	COND_CODE_1(DT_NODE_HAS_COMPAT(node_id, soc_nv_flash), (node_id), ())

#define MSPM0_FLASH_NODE \
		DT_INST_FOREACH_CHILD_STATUS_OKAY(0, MSPM0_FLASH_COMPAT_NODE)

#define MSPM0_FLASH_SIZE	DT_REG_SIZE(MSPM0_FLASH_NODE)
#define FLASH_MSPM0_BASE_ADDRESS DT_REG_ADDR(MSPM0_FLASH_NODE)
#define FLASH_CMDWAIT_TIMEOUT	500

#define MSPM0_FLASH_ERASE_SIZE	DT_PROP_OR(MSPM0_FLASH_NODE, erase_block_size, 0)
BUILD_ASSERT(MSPM0_FLASH_ERASE_SIZE != 0, "Flash Erase Block size is required");
				...
				off_t offset, size_t len)
{
	...
	if (!IS_ALIGNED(offset, MSPM0_FLASH_ERASE_SIZE)) {
		LOG_ERR("Offset must be aligned to flash erase size");
		return -EINVAL;
	}

	if (!IS_ALIGNED(len, MSPM0_FLASH_ERASE_SIZE)) {
		LOG_ERR("Erase length must be aligned to flash erase size");
		return -EINVAL;
	}
...
#ifdef CONFIG_FLASH_PAGE_LAYOUT
static void flash_ti_mspm0_page_layout(const struct device *dev,
...
#define FLASH_TI_MSPM0_DEVICE_INIT(inst)					    \
	...
	static const struct flash_ti_mspm0_config flash_ti_mspm0_cfg##inst = {	    \
		...
		(.flash_layout =  {						    \
			.pages_count = MSPM0_FLASH_SIZE / MSPM0_FLASH_ERASE_SIZE,   \
			.pages_size = MSPM0_FLASH_ERASE_SIZE,			    \
		},))								    \
	};									    \
										    \
