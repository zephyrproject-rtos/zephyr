...
static int spi_flash_at45_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	const struct spi_flash_at45_config *cfg = dev->config;
	int err;
...
static int perform_write(const struct device *dev, off_t offset, const void *data, size_t len)
{
	...
	int err;
	uint8_t const op_and_addr[] = {
		IS_ENABLED(CONFIG_SPI_FLASH_AT45_USE_READ_MODIFY_WRITE) ? CMD_MODIFY : CMD_WRITE,
		(offset >> 16) & 0xFF,
		(offset >> 8) & 0xFF,
		(offset >> 0) & 0xFF,
	};
...
static int spi_flash_at45_write(const struct device *dev, off_t offset, const void *data,
				size_t len)
{
	const struct spi_flash_at45_config *cfg = dev->config;
	int err = 0;
...
static int spi_flash_at45_erase(const struct device *dev, off_t offset, size_t size)
{
	const struct spi_flash_at45_config *cfg = dev->config;
	int err = 0;
...
static int spi_flash_at45_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct spi_flash_at45_config *dev_config = dev->config;
	...
	case PM_DEVICE_ACTION_SUSPEND:
		acquire(dev);
		power_down_op(dev, dev_config->use_udpd ? CMD_ENTER_UDPD : CMD_ENTER_DPD,
...
static const struct flash_parameters *flash_at45_get_parameters(const struct device *dev)
{
	ARG_UNUSED(dev);
...
#define SPI_FLASH_AT45_INST(idx)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
		...
		inst_##idx##_config =                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  \
			{                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              \
				.bus = SPI_DT_SPEC_INST_GET(                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           \
					...
					0),                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            \
				IF_ENABLED(INST_HAS_RESET_GPIO(idx),			\
			(.reset = &reset_##idx,)) IF_ENABLED(INST_HAS_WP_GPIO(idx),			\
			(.wp = &wp_##idx,)) IF_ENABLED(CONFIG_FLASH_PAGE_LAYOUT, (			     \
			.pages_layout = {				     \
				.pages_count = INST_##idx##_PAGES,	     \
				.pages_size  = DT_INST_PROP(idx, page_size), \
			},)) .chip_size = INST_##idx##_BYTES, .sector_size = DT_INST_PROP(idx, sector_size), .sector_0a_size = DT_INST_PROP(idx, sector_0a_pages) * DT_INST_PROP(idx, page_size), .block_size = DT_INST_PROP(idx, block_size), .page_size = DT_INST_PROP(idx, page_size), .no_chip_erase = DT_INST_PROP(idx, no_chip_erase), .no_sector_erase = DT_INST_PROP(idx, no_sector_erase), .t_enter_dpd = DIV_ROUND_UP(DT_INST_PROP(idx, enter_dpd_delay), NSEC_PER_USEC), .t_exit_dpd = DIV_ROUND_UP(DT_INST_PROP(idx, exit_dpd_delay), NSEC_PER_USEC), .use_udpd = DT_INST_PROP(idx, use_udpd), .jedec_id = DT_INST_PROP(idx, jedec_id), \
	};                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             \
	...
	PM_DEVICE_DT_INST_DEFINE(idx, spi_flash_at45_pm_action);                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       \
	DEVICE_DT_INST_DEFINE(idx, spi_flash_at45_init, PM_DEVICE_DT_INST_GET(idx),                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    \
			      &inst_##idx##_data, &inst_##idx##_config, POST_KERNEL,                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   \
			      CONFIG_SPI_FLASH_AT45_INIT_PRIORITY, &spi_flash_at45_api);
