...
static int spi_nor_erase(struct device *dev, off_t addr, size_t size)
{
	...
	while (size) {
		...
		if (size == (params->sector_size * params->n_sectors)) {
		...
		} else if ((DT_INST_0_JEDEC_SPI_NOR_ERASE_BLOCK_SIZE == SZ_64K)
			  && (size >= SZ_64K)
			  && ((addr & MASK_64K) == 0)) {
			/* 64 KiB block erase */
			...
			addr += SZ_64K;
			size -= SZ_64K;
		} else if ((DT_INST_0_JEDEC_SPI_NOR_ERASE_BLOCK_SIZE == SZ_32K)
			  && (size >= SZ_32K)
			  && ((addr & MASK_32K) == 0)) {
			/* 32 KiB block erase */
...
static const struct flash_pages_layout dev_layout = {
	.pages_count = DT_INST_0_JEDEC_SPI_NOR_SIZE / 8 / DT_INST_0_JEDEC_SPI_NOR_ERASE_BLOCK_SIZE,
	.pages_size = DT_INST_0_JEDEC_SPI_NOR_ERASE_BLOCK_SIZE,
};
...
#endif
	.write_block_size = DT_INST_0_JEDEC_SPI_NOR_WRITE_BLOCK_SIZE,
};
