...
static int flash_valid_range(off_t offset, size_t len)
{
	...
	} else if ((offset + len) > SOC_NV_FLASH_SIZE) {
		LOG_WRN("0x%lx: ends past the end of flash", (long)offset);
		ret = -EINVAL;
	} else {
		/* All checks passed; ret remains FLASH_MCHP_SUCCESS */
	}
 ...
 *
 * @return 0 on success, or a negative error code if a write fails.
 */
 ...
 *
 * @return 0 on success, -EINVAL for alignment errors, or a negative error code on failure.
 */
