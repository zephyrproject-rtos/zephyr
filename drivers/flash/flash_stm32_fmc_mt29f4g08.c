...
int flash_mt29f4g08_ex_op(const struct device *dev, uint16_t code, const uintptr_t in, void *out)
{
	...
	case FLASH_IS_BAD_BLOCK: {
		...
		if (size_mul_overflow(block, config->block_size, &offset) ||
		    (offset >= config->flash_size)) {
			*(int *)out = FLASH_BLOCK_BAD;
			return -EINVAL;
		}
		...
		ret = flash_stm32_fmc_nand_read_spare_area(config->controller, &address,
							   spare_area);
		if (ret == 0) {
			*(int *)out = (spare_area[0] != 0xFF) ? FLASH_BLOCK_BAD : FLASH_BLOCK_GOOD;
		} else {
			*(int *)out = FLASH_BLOCK_BAD;
		}
		break;
