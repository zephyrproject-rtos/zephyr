...
static inline int z_vrfy_flash_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	K_OOPS(K_SYSCALL_DRIVER_FLASH(dev, read));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(data, len));
	return z_impl_flash_read((const struct device *)dev, offset, (void *)data, len);
}
#include <zephyr/syscalls/flash_read_mrsh.c>

static inline int z_vrfy_flash_write(const struct device *dev, off_t offset, const void *data,
				     size_t len)
{
	K_OOPS(K_SYSCALL_DRIVER_FLASH(dev, write));
	K_OOPS(K_SYSCALL_MEMORY_READ(data, len));
	return z_impl_flash_write((const struct device *)dev, offset, (const void *)data, len);
}
#include <zephyr/syscalls/flash_write_mrsh.c>

static inline int z_vrfy_flash_erase(const struct device *dev, off_t offset, size_t size)
{
...
int z_vrfy_flash_fill(const struct device *dev, uint8_t val, off_t offset, size_t size)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_FLASH));
...
static inline int z_vrfy_flash_get_page_info_by_offs(const struct device *dev, off_t offs,
						     struct flash_pages_info *info)
{
	K_OOPS(K_SYSCALL_DRIVER_FLASH(dev, page_layout));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(info, sizeof(struct flash_pages_info)));
	return z_impl_flash_get_page_info_by_offs((const struct device *)dev, offs,
						  (struct flash_pages_info *)info);
}
#include <zephyr/syscalls/flash_get_page_info_by_offs_mrsh.c>

static inline int z_vrfy_flash_get_page_info_by_idx(const struct device *dev, uint32_t idx,
						    struct flash_pages_info *info)
{
	K_OOPS(K_SYSCALL_DRIVER_FLASH(dev, page_layout));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(info, sizeof(struct flash_pages_info)));
	return z_impl_flash_get_page_info_by_idx((const struct device *)dev, idx,
						 (struct flash_pages_info *)info);
}
...
static inline int z_vrfy_flash_sfdp_read(const struct device *dev, off_t offset, void *data,
					 size_t len)
{
...
static inline int z_vrfy_flash_read_jedec_id(const struct device *dev, uint8_t *id)
{
...
static inline int z_vrfy_flash_ex_op(const struct device *dev, uint16_t code, const uintptr_t in,
				     void *out)
{
