...
static int cdns_nand_device_info(struct cadence_nand_params *params)
{
	...
	LOG_INF("Number of LUMs %x", params->nluns);
...
static int cdns_nand_set_opr_mode(uintptr_t base_address, uint8_t opr_mode)
{
	...
	if (opr_mode == CNF_OPR_WORK_MODE_SDR) {
		...
		LOG_ERR("Driver Operation work mode not supported");
		return -ENOTSUP;
	}
...
static struct cdns_cdma_command_descriptor *cdma_allocate_desc_memory(uint32_t page_count,
								      uint32_t max_page_desc)
{
	struct cdns_cdma_command_descriptor *cdma_desc;

	if (page_count > max_page_desc) {
		page_count = max_page_desc;
	}

	cdma_desc = k_malloc(sizeof(struct cdns_cdma_command_descriptor) * page_count);
	if (cdma_desc == NULL) {
		LOG_ERR("Memory allocation error occurred %s", __func__);
		return NULL;
	}

	memset(cdma_desc, 0, sizeof(struct cdns_cdma_command_descriptor) * page_count);

	return cdma_desc;
}

static int cdns_cdma_desc_transfer_finish(struct cadence_nand_params *params,
					  struct cdns_cdma_command_descriptor *cdma_desc,
					  uint32_t page_count, uint32_t max_page_desc,
					  uint32_t ctype, uint32_t cond_start, char *buffer)
{
...
#else
		ret = cdns_transfer_complete(cdma_desc);
#endif
		if (ret != 0) {
			k_free(cdma_desc);
			return ret;
				...
				char *buffer, uint32_t page_count)
{
	...
	cdma_desc = cdma_allocate_desc_memory(page_count, CONFIG_FLASH_CDNS_CDMA_PAGE_COUNT);

	if (cdma_desc == NULL) {
		LOG_ERR("Memory allocation error occurred %s", __func__);
		return -ENOSR;
	}
	ret = cdns_cdma_desc_transfer_finish(params, cdma_desc, page_count,
					     CONFIG_FLASH_CDNS_CDMA_PAGE_COUNT, CNF_CMD_WR,
					     start_page_number, buffer);
			       ...
			       char *buffer, uint32_t page_count)
{
	...
	ret = cdns_cdma_desc_transfer_finish(params, cdma_desc, page_count,
					     CONFIG_FLASH_CDNS_CDMA_PAGE_COUNT, CNF_CMD_RD,
					     start_page_number, buffer);
				...
				uint32_t block_count)
{
	...
	cdma_desc = cdma_allocate_desc_memory(block_count, CONFIG_FLASH_CDNS_CDMA_BLOCK_COUNT);

	if (cdma_desc == NULL) {
		LOG_ERR("Memory allocation error occurred %s", __func__);
		return -ENOSR;
	}
	ret = cdns_cdma_desc_transfer_finish(params, cdma_desc, block_count,
...
/**
 * Perform buffer copying from/to SDMA regs in the Cadence NAND controller.
 *
 ...
 * @param dir The direction of the copy operation (read or write).
 */
static void sdma_buffer_copy(uint32_t sdma_base_address, uint8_t *buffer, uint32_t data_length,
			     uint8_t dir)
{
	uint32_t index;
				  ...
				  void *data_buffer)
{
	...
	sdma_buffer_copy(params->sdma_base, data_buffer, params->page_size, CDNS_READ);
	return 0;
}
		   ...
		   uint32_t size)
{
	...
	if ((size == 0) || (offset >= params->device_size) ||
	    (((offset + size) - 1) >= params->device_size)) {
		LOG_ERR("Wrong offset value passed!!");
		return -EINVAL;
	}
	...
	if ((check_page_last == 1) && (check_page_first == 1)) {
		...
		ret = cdns_nand_pio_read_write(params, start_page_number, (char *)buffer,
					       page_count, CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, start_page_number, (char *)buffer,
					       page_count, CDNS_READ);
#endif
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, start_page_number, first_end_page, page_count);
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, start_page_number, first_end_page,
					       page_count, CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, start_page_number, first_end_page,
					       page_count, CDNS_READ);
#endif
		if (ret != 0) {
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, end_page_number, first_end_page, 1);
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, end_page_number, first_end_page, 1,
					       CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, end_page_number, first_end_page, 1,
					       CDNS_READ);
#endif
		if (ret < 0) {
			k_free(first_end_page);
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, start_page_number, (char *)buffer,
					  (--page_count));
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, start_page_number, (char *)buffer,
					       (--page_count), CDNS_READ);
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, start_page_number, first_end_page, 1);
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, start_page_number, first_end_page, 1,
					       CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, start_page_number, first_end_page, 1,
					       CDNS_READ);
#endif
		if (ret < 0) {
			k_free(first_end_page);
		...
		r_bytes = (offset + size) % (params->page_size);
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, (++start_page_number),
					  ((char *)buffer + bytes_dif), (--page_count));
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, (++start_page_number),
					       ((char *)buffer + bytes_dif), (--page_count),
					       CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, (++start_page_number),
					       ((char *)buffer + bytes_dif), (--page_count),
					       CDNS_READ);
#endif
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, end_page_number, last_end_page, 1);
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, end_page_number, last_end_page, 1,
					       CDNS_READ);
#elif CONFIG_CDNS_NAND_GENERIC_MODE
		ret = cdns_nand_gen_read_write(params, end_page_number, last_end_page, 1,
					       CDNS_READ);
#endif
		if (ret != 0) {
			k_free(last_end_page);
...
#if CONFIG_CDNS_NAND_CDMA_MODE
		ret = cdns_nand_cdma_read(params, (++start_page_number),
					  ((char *)buffer + bytes_dif), (page_count - 2));
#elif CONFIG_CDNS_NAND_PIO_MODE
		ret = cdns_nand_pio_read_write(params, (++start_page_number),
					       ((char *)buffer + bytes_dif), (page_count - 2),
					       CDNS_READ);
		    ...
		    uint32_t len)
{
	...
	if ((len == 0) || (offset >= params->device_size) ||
	    (((offset + len) - 1) >= params->device_size)) {
		LOG_ERR("Wrong offset value passed!!");
		return -EINVAL;
	}
