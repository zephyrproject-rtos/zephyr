...
static inline int spi_flash_reg_read(struct device *dev, u8_t cmd_id)
{
	...
	if (spi_transceive(data->spi_dev, &data->config, &desc, &desc) != 0) {
		return -EIO;
	}
			       ...
			       void *ptr, size_t len)
{
	...
	struct spi_buf_set txdesc = {
		.buffers = txbufs,
		.count = 2,
	};
	struct spi_buf_set rxdesc = {
		.buffers = rxbufs,
		.count = 2,
	};

	if (spi_transceive(data->spi_dev, &data->config, &txdesc, &rxdesc) != 0) {
		return -EIO;
	}
			  ...
			  size_t len)
{
	...
	hdr[i++] = offset >> 8;
	hdr[i++] = offset;
	r = spi_transceive(data->spi_dev, &data->config, &txdesc, &rxdesc);
	k_sem_give(&data->sem);
			      ...
			      const void *ptr, size_t len)
{
	...
	r = spi_transceive(data->spi_dev, &data->config, &txdesc, NULL);
	if (r == 0) {
		wait_for_flash_idle(dev);
	}
				      ...
				      u8_t opcode, off_t offset)
{
	...
	wait_for_flash_idle(dev);
	return spi_transceive(data->spi_dev, &data->config, &txdesc, NULL);
}
