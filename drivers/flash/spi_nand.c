...
static int spi_nand_wait_until_ready(const struct device *dev)
{
	...
	uint8_t reg = 0;
	struct spi_nand_data *data = dev->data;
	int timeout_ms = data->program_timeout;
			 ...
			 size_t size)
{
	...
	/* addr must be aligned with page */
	if ((addr % data->page_size) != 0) {
		return -EINVAL;
	}
...
out0:
	...
	if (onfi_table[ONFI_SIG_0] == 'O' && onfi_table[ONFI_SIG_1] == 'N'
		...
		data->program_timeout = onfi_table[ONFI_BE_TIME_135] + (onfi_table[ONFI_BE_TIME_136] << 8);
		...
		if (onfi_table[ONFI_DATA_REC_167] & 0x01) {
