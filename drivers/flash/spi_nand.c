			 ...
			 size_t size)
{
	...
	if (addr & (data->sub_page_size - 1)) {
		return -EINVAL;
	}

	/* size must be a multiple of sub-page */
	if ((size % data->sub_page_size) != 0) {
		return -EINVAL;
	}
...
out0:
	...
	if (onfi_table[0] == 'O' && onfi_table[1] == 'N'
		...
		data->sub_page_size = onfi_table[86] + (onfi_table[87] << 8) + (onfi_table[88] << 16);
...
flash_nand_get_parameters(const struct device *dev)
{
	ARG_UNUSED(dev);
	struct spi_nand_data *data = dev->data;
	const struct flash_parameters *flash_nand_parameters;

	flash_nand_parameters->write_block_size = data->sub_page_size;
