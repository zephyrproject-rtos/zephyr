...
static int flash_it51xxx_read(const struct device *dev, off_t offset, void *dst_data, size_t len)
{
	...
	if (!is_valid_range(offset, len) || (len > FLASH_READ_MAX_SZ)) {
		LOG_ERR("Out of boundaries: FLASH_SIZE=%#x, read max size=%u, offset=%#lx, len=%u",
			FLASH_SIZE, FLASH_READ_MAX_SZ, offset, len);
		return -EINVAL;
	}
	...
	sys_write8(m1kflhctrl5_cmd | M1K_READ_BCNT(FIELD_GET(GENMASK(9, 8), len)),
		   SMFI_M1KFLHCTRL5);
	/* M1K-READ byte count[7:0] */
	sys_write8(FIELD_GET(GENMASK(7, 0), len), SMFI_M1KFLHCTRL4);
			       ...
			       size_t len)
{
	...
	if (!is_valid_range(offset, len) || (len > FLASH_WRITE_MAX_SZ)) {
		LOG_ERR("Out of boundaries: FLASH_SIZE=%#x, write max size=%u, offset=%#lx, len=%u",
			FLASH_SIZE, FLASH_WRITE_MAX_SZ, offset, len);
		return -EINVAL;
	}
	...
	sys_write8(m1kflhctrl7_cmd | M1K_PROG_BCNT(FIELD_GET(GENMASK(9, 8), len)),
		   SMFI_M1KFLHCTRL7);
	/* M1K-PROG byte count[7:0] */
	sys_write8(FIELD_GET(GENMASK(7, 0), len), SMFI_M1KFLHCTRL6);
...
static int flash_it51xxx_erase(const struct device *dev, off_t offset, size_t len)
{
	...
	flash_set_m1k_erase_uba(dev, offset + len);
