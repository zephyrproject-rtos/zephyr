...
#ifndef ZEPHYR_DRIVERS_FLASH_NCT_QSPI_H_
#define ZEPHYR_DRIVERS_FLASH_NCT_QSPI_H_
...
#define NCT_TRANSCEIVE_ACCESS_WRITE BIT(0)
#define NCT_TRANSCEIVE_ACCESS_READ  BIT(1)
#define NCT_TRANSCEIVE_ACCESS_ADDR  BIT(2)

/* Valid value of Dn_NADDRB that sets the number of address bytes in a transaction */
#define NCT_DEV_NUM_ADDR_1BYTE 1
#define NCT_DEV_NUM_ADDR_2BYTE 2
#define NCT_DEV_NUM_ADDR_3BYTE 3
#define NCT_DEV_NUM_ADDR_4BYTE 4

/* Transceive operation configuration for a SPI device */
struct nct_transceive_cfg {
	uint8_t opcode;
	uint8_t *tx_buf;
	size_t  tx_count;
	uint8_t *rx_buf;
 ...
 */
typedef int (*qspi_nct_transceive)(const struct device *dev,
				struct nct_transceive_cfg *cfg, uint32_t flags);

/**
 * @brief Lock the mutex of nct qspi bus controller and apply its configuration
 *
 * @param dev Pointer to the device structure for qspi bus controller instance.
 * @param cfg Pointer to the configuration for the device on qspi bus.
 * @param operation Qspi bus operation for the device.
 */
typedef void (*qspi_nct_mutex_lock_configure)(const struct device *dev,
					const struct nct_qspi_cfg *cfg,
					const uint32_t operation);

/**
 * @brief Unlock the mutex of nct qspi bus controller.
 *
 * @param dev Pointer to the device structure for qspi bus controller instance.
 */
typedef void (*qspi_nct_mutex_unlock)(const struct device *dev);

struct nct_qspi_ops {
	qspi_nct_mutex_lock_configure lock_configure;
	qspi_nct_mutex_unlock unlock;
	qspi_nct_transceive transceive;
};

/* Device data */
struct nct_qspi_data {
	/* mutex of qspi bus controller */
	struct k_sem lock_sem;
	/* qspi operation interface */
	struct nct_qspi_ops *qspi_ops;
	/* Current device configuration on QSPI bus */
	const struct nct_qspi_cfg *cur_cfg;
...
#endif /* ZEPHYR_DRIVERS_FLASH_NCT_QSPI_H_ */
