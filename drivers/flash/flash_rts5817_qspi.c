...
/* Bus address 0x01800000 maps to flash address 0x1100. */
#define FLASH_MAP_OFFSET 0x1100
				  ...
				  off_t addr, size_t len, uint8_t *data)
{
	...
	/* Check alignment */
	if ((uint32_t)data & (CONFIG_DCACHE_LINE_SIZE - 1)) {
		LOG_ERR("Read buffer address is not aligned to %d", CONFIG_DCACHE_LINE_SIZE);
		return -EINVAL;
	}
...
{
	uint8_t temp[CONFIG_DCACHE_LINE_SIZE] __aligned(CONFIG_DCACHE_LINE_SIZE);
	int ret;
...
static int rts_fp_qspi_write_status(const struct device *dev, uint8_t cmd, uint8_t wren_cmd,
				    uint8_t *value, uint8_t len)
{
	sys_cache_data_flush_range(value, len);
	/* Write enable */
	rts_fp_qspi_write_reg(dev, wren_cmd, R_SPI_SUB1_COMMAND);
	rts_fp_qspi_write_reg(dev, 0x0, R_SPI_SUB1_ADDR);
	rts_fp_qspi_write_reg(dev, SPI_C_MODE0, R_SPI_SUB1_MODE);
...
static int rts_fp_flash_write_status_volatile(const struct device *dev, uint8_t *value)
{
	struct flash_rts5817_qspi_data *dev_data = dev->data;
	int ret;
	...
	switch (dev_data->flash->str_rw_type) {
	case SREG1_R1_W1:
		return rts_fp_qspi_write_status(dev, SF_WRSR, dev_data->flash->sr_wren_command,
						value, 1);
	case SREG2_R2_W1:
		return rts_fp_qspi_write_status(dev, SF_WRSR, dev_data->flash->sr_wren_command,
						value, 2);
	case SREG2_R2_W2:
		ret = rts_fp_qspi_write_status(dev, SF_WRSR, dev_data->flash->sr_wren_command,
					       value, 1);
		if (ret) {
			return ret;
		}
		return rts_fp_qspi_write_status(dev, SF_WRSR_H, dev_data->flash->sr_wren_command,
						&value[1], 1);
	default:
		return -ENOTSUP;
	}
...
static int rts_fp_flash_write_status_non_volatile(const struct device *dev, uint8_t *value)
{
	struct flash_rts5817_qspi_data *dev_data = dev->data;
	int ret;
	...
	switch (dev_data->flash->str_rw_type) {
	case SREG1_R1_W1:
		return rts_fp_qspi_write_status(dev, SF_WRSR, SF_WREN, value, 1);
	case SREG2_R2_W1:
		return rts_fp_qspi_write_status(dev, SF_WRSR, SF_WREN, value, 2);
	case SREG2_R2_W2:
		ret = rts_fp_qspi_write_status(dev, SF_WRSR, SF_WREN, value, 1);
		if (ret) {
			return ret;
		}
		return rts_fp_qspi_write_status(dev, SF_WRSR_H, SF_WREN, &value[1], 1);
	default:
		return -ENOTSUP;
	}
}
...
static int rts_fp_flash_soft_write_protect(const struct device *dev, bool enable)
{
	...
	return rts_fp_flash_write_status_volatile(dev, status_reg);
}
...
static int rts_fp_flash_quad_cfg(const struct device *dev, bool enable)
{
	...
	return rts_fp_flash_write_status_non_volatile(dev, status_reg);
}
...
static void __attribute((aligned(CONFIG_ICACHE_LINE_SIZE), noinline))
...
static int flash_rts5817_qspi_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	uint8_t temp_buf[CONFIG_DCACHE_LINE_SIZE] __aligned(CONFIG_DCACHE_LINE_SIZE);
	uint16_t temp_len;
	int ret;
	...
	temp_len = CONFIG_DCACHE_LINE_SIZE - ((uint32_t)data % CONFIG_DCACHE_LINE_SIZE);
	if (temp_len != CONFIG_DCACHE_LINE_SIZE) {
		if (len < temp_len) {
			temp_len = len;
		}
	...
	temp_len = ROUND_DOWN(len, CONFIG_DCACHE_LINE_SIZE);
	if (temp_len) {
		ret = rts_fp_qspi_read_align(dev, dev_data->read_mode, dev_data->read_cmd, offset,
					     temp_len, data);
		if (ret) {
				    ...
				    const void *data)
{
	...
	sys_cache_data_invd_range((void *)(CONFIG_FLASH_BASE_ADDRESS - FLASH_MAP_OFFSET + offset),
				  len);
...
static int flash_rts5817_ex_op_write_status(const struct device *dev, uint8_t *sr)
{
	...
	return rts_fp_flash_write_status_volatile(dev, sr);
}
...
static int rts_fp_qspi_resume(const struct device *dev)
{
	const struct flash_rts5817_qspi_config *dev_cfg = dev->config;
	struct flash_rts5817_qspi_data *dev_data = dev->data;
	clock_control_subsys_rate_t rate = (clock_control_subsys_rate_t)MHZ(120);
...
static int rts_fp_qspi_suspend(const struct device *dev)
{
	const struct flash_rts5817_qspi_config *dev_cfg = dev->config;

	syscon_update_bits(dev_cfg->syscon_sys, R_AL_DUMMY0, ECO_CACHESPI_HOLD_PU_EN, 0);
	syscon_update_bits(dev_cfg->syscon_sys, R_AL_DUMMY1, ECO_CACHESPI_HOLD_PD_EN,
			   ECO_CACHESPI_HOLD_PD_EN);
	syscon_update_bits(dev_cfg->syscon_sys, R_AL_DUMMY0, ECO_CACHESPI_HOLD_PUPD_CTRL_SWITCH,
			   ECO_CACHESPI_HOLD_PUPD_CTRL_SWITCH);
	k_busy_wait(10);
	syscon_update_bits(dev_cfg->syscon_sys, R_AL_DUMMY0, ECO_CACHESPI_HOLD_PUPD_CTRL_SWITCH, 0);
	k_busy_wait(100);

	return 0;
}

static int rts_fp_qspi_pm_action(const struct device *dev, enum pm_device_action action)
{
	int ret;

	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		ret = rts_fp_qspi_resume(dev);
		break;
	case PM_DEVICE_ACTION_SUSPEND:
		ret = rts_fp_qspi_suspend(dev);
		break;
	default:
		return -ENOTSUP;
	}
...
static const struct flash_rts5817_qspi_config flash_rts5817_qspi_cfg = {
	...
	.syscon_sys = DEVICE_DT_GET(DT_INST_PHANDLE(0, syscon_sys)),
};

static struct flash_rts5817_qspi_data flash_rts5817_qspi_dev_data;

PM_DEVICE_DT_INST_DEFINE(0, rts_fp_qspi_pm_action);
DEVICE_DT_INST_DEFINE(0, &flash_rts5817_qspi_init, PM_DEVICE_DT_INST_GET(0),
		      &flash_rts5817_qspi_dev_data, &flash_rts5817_qspi_cfg, POST_KERNEL,
		      CONFIG_FLASH_INIT_PRIORITY, &flash_rts5817_qspi_driver_api);
