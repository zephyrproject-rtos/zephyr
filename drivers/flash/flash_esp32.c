...
static bool aligned_flash_erase(size_t addr, size_t size)
{
	...
	LOG_INF("%s: forcing unaligned erase on sector Offset: "
	...
	while (current_addr < end_addr) {
		...
		if (preserve_head || preserve_tail) {
			...
			if (flash_esp32_read_check_enc(current_addr, erase_aux_buf, sector_size) !=
			    ESP_OK) {
				LOG_ERR("%s: Flash read failed at 0x%08lx", __func__,
			...
			LOG_INF("%s: partial sector erase from: 0x%08lx to: 0x%08lx Length: 0x%x",
				__func__, (uintptr_t)(current_addr + erase_start),
				(uintptr_t)(current_addr + erase_end), erase_end - erase_start);

			/* Erase full sector */
			if (esp_flash_erase_region(NULL, current_addr, sector_size) != ESP_OK) {
				LOG_ERR("%s: Flash erase failed at 0x%08lx", __func__,
					(uintptr_t)current_addr);
				return false;
			}
			...
			if (preserve_head) {
				...
				if (!aligned_flash_write(current_addr, erase_aux_buf, erase_start,
							 false)) {
					LOG_ERR("%s: Flash write failed at 0x%08lx", __func__,
						(uintptr_t)current_addr);
					return false;
			...
			if (preserve_tail) {
				...
				if (!aligned_flash_write(current_addr + erase_end,
							 &erase_aux_buf[erase_end],
			...
			LOG_INF("%s: sectors erased from: 0x%08lx length: 0x%x", __func__,
