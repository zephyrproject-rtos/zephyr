...
{
	int err;
	uint8_t cmd[2] = {DEV_READ_SR, DEV_DUMMY_BYTE}, sr[2];
...
{
	int err;
	uint8_t cmd[1] = {DEV_READ_DEVICE_INFO}, info[3];
	const struct flash_at25xv021a_config *config = dev->config;
	const struct spi_buf tx_buf = {.buf = cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
{
	int err;
	uint8_t cmd[1] = {DEV_WRITE_ENABLE}, status;
	const struct flash_at25xv021a_config *config = dev->config;
	const struct spi_buf tx_buf = {.buf = &cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
{
	int err;
	uint8_t cmd[2] = {DEV_WRITE_SR, DEV_HW_LOCK}, status;
	const struct flash_at25xv021a_config *config = dev->config;
	const struct spi_buf tx_buf = {.buf = &cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
{
	int err;
	uint8_t cmd[2] = {DEV_WRITE_SR, DEV_HW_UNLOCK}, status;
	const struct flash_at25xv021a_config *config = dev->config;
	const struct spi_buf tx_buf = {.buf = &cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
static int flash_at25xv021a_global_protection(const struct device *dev, uint8_t protection_cmd)
{
	int err;
	const struct flash_at25xv021a_config *config = dev->config;
	uint8_t cmd[2] = {DEV_WRITE_SR, protection_cmd}, expected, status;
	const struct spi_buf tx_buf = {.buf = &cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
						...
						uint8_t protection_cmd)
{
	...
	unexpected = (protection_cmd == DEV_PROTECT) ? 0 : DEV_SR_SWP;
...
{
	int err;
	uint8_t cmd[1] = {DEV_CHIP_ERASE}, status;
	const struct flash_at25xv021a_config *config = dev->config;
	const struct spi_buf tx_buf = {.buf = &cmd, .len = ARRAY_SIZE(cmd)};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
static int flash_at25xv021a_process_erase(const struct device *dev, off_t *offset)
{
	int err;
	const struct flash_at25xv021a_config *config = dev->config;
	off_t page_addr = *offset / config->page_size;
...
#define ASSERT_TIMEOUTS(timeout, timeout_erase)                                                    \
	BUILD_ASSERT((timeout > 0) && (timeout_erase > 0), "Timeouts must be positive integers")

#define SPI_OP (SPI_OP_MODE_MASTER | SPI_TRANSFER_MSB | SPI_WORD_SET(8))

#define PAGE_SIZE_IF_WRITEABLE(inst)                                                               \
	COND_CODE_0(DT_ALL_INST_HAS_BOOL_STATUS_OKAY(read_only),	\
...
#define WP_GPIO_IF_PROVIDED(inst)                                                                  \
	IF_ENABLED(DT_NODE_HAS_PROP(DT_INST(inst, atmel_at25xv021a), wp_gpios),	\
