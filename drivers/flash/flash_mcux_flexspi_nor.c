		...
		const void *buffer, size_t len)
{
...
#endif
#if defined(CONFIG_HAS_MCUX_CACHE) || defined(CONFIG_MEMC_MCUX_FLEXSPI_IPED)
...
#ifdef CONFIG_MEMC_MCUX_FLEXSPI_IPED
	{
		...
		if (memc_flexspi_iped_any_overlap(&data->controller, data->port, offset, len) &&
		    !memc_flexspi_iped_ipwr_enabled(&data->controller)) {
			LOG_ERR("Write overlaps IPED region, but IPWR_EN is not set — "
