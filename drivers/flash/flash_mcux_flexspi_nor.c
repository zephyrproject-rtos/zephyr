...
static int flash_flexspi_nor_is25_clear_dummy_cycles(struct flash_flexspi_nor_data *data,
			uint32_t (*flexspi_lut)[MEMC_FLEXSPI_CMD_PER_SEQ])
{
	int ret;
	 ...
	 */
	const int supports_rderp = resp_data & 0xF0 && !(resp_data & 0x01);

	uint32_t read_params = supports_rderp
		? 0 	 /* With extended read register  */
		: 0xE0U; /* Without extended read register */
