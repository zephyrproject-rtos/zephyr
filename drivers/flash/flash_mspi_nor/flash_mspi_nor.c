		    ...
		    size_t size)
{
	...
	if ((FLASH_DATA(dev)->quirks != NULL) &&
	    (FLASH_DATA(dev)->quirks->pre_read != NULL)) {
		rc = FLASH_DATA(dev)->quirks->pre_read(dev, addr, dest, size);
		if (rc < 0) {
			return rc;
		}
	}
	...
	if ((FLASH_DATA(dev)->quirks != NULL) &&
	    (FLASH_DATA(dev)->quirks->post_read != NULL)) {
		rc = FLASH_DATA(dev)->quirks->post_read(dev, addr, dest, size);
		if (rc < 0) {
			return rc;
		}
	}
		     ...
		     size_t size)
{
	...
	while (size > 0) {
		...
		if ((FLASH_DATA(dev)->quirks != NULL) &&
		    (FLASH_DATA(dev)->quirks->pre_write != NULL)) {
			rc = FLASH_DATA(dev)->quirks->pre_write(dev, addr, src, to_write);
			if (rc < 0) {
				break;
			}
		}
		...
		if ((FLASH_DATA(dev)->quirks != NULL) &&
		    (FLASH_DATA(dev)->quirks->post_write != NULL)) {
			rc = FLASH_DATA(dev)->quirks->post_write(dev, addr, src, to_write);
			if (rc < 0) {
				break;
			}
		}
...
static int flash_chip_init(const struct device *dev)
{
	...
	for (i = 0; i < vendor_count; i++) {
		if (vendors[i]->probe_dev(dev, FLASH_DATA(dev),
...
#define FLASH_DATA_INIT(inst, flash_macro)                                      \
	.flash_data = COND_CODE_1(DT_CAT(flash_macro, _SUPPORTED),              \
		(&DT_CAT(flash_macro, _DATA)), (&flash_dev_##inst))
