...
#define CONFIG_SPI_NAND_BCH_HEAP_SIZE 512000
#endif
...
do {															\
		(buf) = k_heap_alloc(&bch_heap, size, K_NO_WAIT);		\
		if (!(buf)) {											\
			bch_free(bch);                                      \
			return -ENOMEM;								\
		}														\
...
#define CALLOC_BCH(buf, num, size)                                   \
	do {                                                             \
		size_t _total = (num) * (size);                              \
		(buf) = k_heap_alloc(&bch_heap, _total, K_NO_WAIT);          \
		if (!(buf)) {                                                \
			bch_free(bch);                                           \
			return -ENOMEM;       		                             \
		}                                                            \
...
static unsigned int *build_generator_poly(struct bch_code *bch)
{
	...
	MALLOC_BCH(x, (bch->m * bch->t + 1) * sizeof(*x));
	MALLOC_BCH(g, (bch->ecc_words + 1)  * sizeof(*g));
...
struct bch_code *bch_init(int m, int t)
{
	...
	MALLOC_BCH(bch, sizeof(struct bch_code));
	MALLOC_BCH(bch->a_pow, (bch->n + 1) * sizeof(*bch->a_pow));
	MALLOC_BCH(bch->a_log, (bch->n + 1) * sizeof(*bch->a_log));
	MALLOC_BCH(bch->mod_tab, bch->ecc_words * 16 * 4 * sizeof(*bch->mod_tab));
	...
	MALLOC_BCH(bch->input_data, (1 << m) / 8);
