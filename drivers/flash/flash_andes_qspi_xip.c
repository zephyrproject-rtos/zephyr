...
#define MCACHE_CTL_IPREF_EN BIT(9)
#define MCACHE_CTL_DPREF_EN BIT(10)
...
static __ramfunc unsigned int prepare_for_ramfunc(void)
{
	unsigned int key;
...
static __ramfunc void cleanup_after_ramfunc(unsigned int key)
{
	csr_set(NDS_MMISC_CTL, MMISC_CTL_BRPE_EN);
	irq_unlock(key);
}

static __ramfunc void prepare_for_flashing(void)
{
	/* Disable prefetching to avoid any flash reads by hardware while working on flash. */
	csr_clear(NDS_MCACHE_CTL, MCACHE_CTL_DPREF_EN);
	csr_clear(NDS_MCACHE_CTL, MCACHE_CTL_IPREF_EN);

	/* Finish all potetnial pending accesses to flash. */
...
static __ramfunc void cleanup_after_flashing(const struct device *dev, off_t addr, size_t size)
{
	...
	/* Make sure all flash operations are finished. */
	__asm__ volatile("fence.i" ::: "memory");
	...
	/* Restore prefetching */
	csr_set(NDS_MCACHE_CTL, MCACHE_CTL_IPREF_EN);
	csr_set(NDS_MCACHE_CTL, MCACHE_CTL_DPREF_EN);
}
