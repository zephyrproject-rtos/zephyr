						 ...
						 size_t length)
{
	...
	regs->TFMAT = regs->TFMAT & ~TFMAT_DATA_MERGE_MSK;
	regs->TCTRL = tctrl;
...
static __ramfunc int do_write(const struct device *dev, off_t addr, const void *src, size_t size)
{
	...
	do {
		...
		ret = flash_andes_qspi_xip_cmd_addr_write(dev, SPI_NOR_CMD_PP_1_1_4, addr, src,
							  to_write);

		flash_andes_qspi_xip_wait_until_ready(dev);
		cache_data_invd_range((void *)(addr + config->mapped_base), to_write);
		cache_instr_invd_range((void *)(addr + config->mapped_base), to_write);
				      ...
				      size_t size)
{
	...
	printk("DN write: offset: 0x%08lx, size: 0x%08x\n", addr, size);
...
static __ramfunc int do_erase(const struct device *dev, off_t addr, size_t size)
{
	...
	while (size > 0) {
		...
		ret = flash_andes_qspi_xip_cmd_addr_write(dev, SPI_NOR_CMD_SE, addr, NULL, 0);
		flash_andes_qspi_xip_wait_until_ready(dev);
		cache_data_invd_range((void *)(addr + config->mapped_base), SPI_NOR_SECTOR_SIZE);
		cache_instr_invd_range((void *)(addr + config->mapped_base), SPI_NOR_SECTOR_SIZE);
...
static int flash_andes_qspi_xip_erase(const struct device *dev, off_t addr, size_t size)
{
	...
	printk("DN erase: offset: 0x%08lx, size: 0x%08x\n", addr, size);
