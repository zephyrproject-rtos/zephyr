			  ...
			  size_t *layout_size)
{
	...
	if (flash_data->FlashRegion == DATA_FLASH) {
		flash_ra_layout[0].pages_count =
			flash_data->area_size / flash_data->erase_block_size;
		flash_ra_layout[0].pages_size = flash_data->erase_block_size;
...
static int flash_ra_init(const struct device *dev)
{
	...
	const struct flash_hp_ra_config *cfg = dev->config;
	struct flash_hp_ra_data *flash_data = dev->data;
	...
	if (cfg->flash_ra_parameters.write_block_size == UNIT_CODE_FLASH_PROGRAMMING) {
