...
#ifndef ZEPHYR_DRIVERS_FLASH_MSPI_NOR_QUIRKS_INFINEON_H_
#define ZEPHYR_DRIVERS_FLASH_MSPI_NOR_QUIRKS_INFINEON_H_
...
static int infineon_s28hx512t_read_register(const struct device *dev, uint32_t reg, uint8_t *value)
{
	...
	set_up_xfer_with_addr(dev, MSPI_RX, reg, MSPI_PIO);
...
static int infineon_s28hx512t_write_register(const struct device *dev, uint32_t reg, uint8_t value)
{
	...
	set_up_xfer_with_addr(dev, MSPI_TX, reg, MSPI_PIO);
	dev_data->packet.data_buf = &value;
	dev_data->packet.num_bytes = 1;
...
static int infineon_s28hx512t_pre_init(const struct device *dev)
{
	...
	/* default mode is 3 byte addressing mode in 1S-1S-1S */
...
#define FLASH_HAS_CADENCE_PARENT(node, ...)                                                        \
	IF_ENABLED(DT_NODE_HAS_COMPAT(DT_PARENT(node), cdns_xspi_nor), (1))

#define FLASH_ANY_HAS_CADENCE_PARENT                                                               \
	UTIL_NOT(IS_EMPTY(                                                                         \
		DT_FOREACH_STATUS_OKAY_VARGS(infineon_s28hx512t, FLASH_HAS_CADENCE_PARENT)))

#if FLASH_ANY_HAS_CADENCE_PARENT
...
static int infineon_s28hx512t_configure_rd_delay_cadence_mspi(const struct device *dev)
{
	...
	struct mspi_cadence_timing_cfg timing;
	uint8_t max = 0xff;
	uint8_t min = 0x00;
	int rc = 0;

	if (!dev_config->jedec_id_specified) {
		LOG_ERR("jedec-id must be specified in DT to calibrate read delay");
		return -EINVAL;
	}

	for (int8_t rd_delay = 0xf; rd_delay >= 0x0; rd_delay--) {
		timing.rd_delay = rd_delay;

		rc = mspi_timing_config(dev_config->bus, &dev_config->mspi_id,
					MSPI_CADENCE_TIMING_PARAM_RD_DELAY, &timing);
		if (rc < 0) {
	...
	timing.rd_delay = (max + min) / 2;
	LOG_INF("setting read delay as 0x%x", timing.rd_delay);
...
#endif /* FLASH_ANY_HAS_CADENCE_PARENT */
...
#endif /* ZEPHYR_DRIVER_FLASH_MSPI_NOR_QUIRKS_INFINEON_H_ */
