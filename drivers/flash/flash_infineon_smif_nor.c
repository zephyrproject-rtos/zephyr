 ...
 * "hybrid-region-*" arrays has non-uniform sectors, so describe each
...
#define IFX_SMIF_NOR_HYBRID_LAYOUT_ENTRY(node_id, prop, idx)                                       \
	{                                                                                          \
		.pages_count = DT_PROP_BY_IDX(node_id, hybrid_region_sectors, idx),       \
		.pages_size = DT_PROP_BY_IDX(node_id, hybrid_region_erase_size, idx),     \
	}
...
#define IFX_SMIF_NOR_LAYOUT_DEFINE(n)                                                              \
	static const struct flash_pages_layout ifx_smif_nor_layout_##n[] = {                       \
		COND_CODE_1(DT_INST_NODE_HAS_PROP(n, hybrid_region_sectors),              \
			    (DT_INST_FOREACH_PROP_ELEM_SEP(n, hybrid_region_sectors,      \
...
#define IFX_SMIF_NOR_INST(n)                                                                       \
	...
	static const struct ifx_smif_nor_config ifx_smif_nor_config_##n = {                        \
		...
		.mem_num = DT_INST_PROP(n, chip_select),                                  \
