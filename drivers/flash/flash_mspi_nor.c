...
#define DTS_RX_DUMMY_NOT_SPECIFIED 0xFF
		    ...
		    size_t size)
{
	...
	if (dev_config->dts_rx_dummy != DTS_RX_DUMMY_NOT_SPECIFIED) {
		dev_data->xfer.rx_dummy = dev_config->dts_rx_dummy;
	} else {
		/* Since it's not yet possible to specify mode bits with MSPI
		 * API, treat mode bit cycles as just dummy.
		 */
		dev_data->xfer.rx_dummy = dev_data->cmd_info.read_mode_bit_cycles
...
static int flash_chip_init(const struct device *dev)
{
	...
	/* Read the SFDP signature to test if communication with the flash chip
	 * can be successfully performed after switching to target IO mode.
	 */
...
#define FLASH_MSPI_NOR_INST(inst)						\
	...
	BUILD_ASSERT(DT_INST_PROP_OR(inst, rx_dummy, 0)				\
		     != DTS_RX_DUMMY_NOT_SPECIFIED,				\
		"Unsupported number of RX dummy clocks specified for "		\
