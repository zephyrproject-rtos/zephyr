...
static int upipe_set_channel(const struct device *dev, u16_t channel)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(channel);
...
static int upipe_set_pan_id(const struct device *dev, u16_t pan_id)
{
	u8_t pan_id_le[2];
...
static int upipe_set_short_addr(const struct device *dev, u16_t short_addr)
{
...
static int upipe_set_txpower(const struct device *dev, s16_t dbm)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(dbm);
