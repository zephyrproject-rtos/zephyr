...
/* B91 data structure */
static struct  b91_data_t b91_data;
...
static int b91_set_pan_id(uint16_t pan_id)
{
	...
	memcpy(b91_data.filter_pan_id, pan_id_le, B91_PAN_ID_SIZE);
...
static int b91_set_short_addr(uint16_t short_addr)
{
	...
	memcpy(b91_data.filter_short_addr, short_addr_le, B91_SHORT_ADDRESS_SIZE);
...
{
	memcpy(b91_data.filter_ieee_addr, ieee_addr, B91_IEEE_ADDRESS_SIZE);
...
static bool b91_run_filter(uint8_t *rx_buffer)
{
	...
	if (memcmp(&rx_buffer[B91_PAN_ID_OFFSET], b91_data.filter_pan_id,
		   B91_PAN_ID_SIZE) != 0 &&
	...
	case B91_DEST_ADDR_TYPE_SHORT:
		...
		if (memcmp(&rx_buffer[B91_DEST_ADDR_OFFSET], B91_BROADCAST_ADDRESS,
		    ...
		    memcmp(&rx_buffer[B91_DEST_ADDR_OFFSET], b91_data.filter_short_addr,
			   B91_SHORT_ADDRESS_SIZE) != 0) {
			return false;
		}
		break;
	...
	case B91_DEST_ADDR_TYPE_IEEE:
		/* If not broadcast, check if length and address matches */
		if ((net_if_get_link_addr(b91_data.iface)->len != B91_IEEE_ADDRESS_SIZE) ||
		    memcmp(&rx_buffer[B91_DEST_ADDR_OFFSET], b91_data.filter_ieee_addr,
...
{
	struct b91_data_t *b91 = dev->data;
...
static void b91_update_rssi_and_lqi(struct net_pkt *pkt)
{
	...
	rssi = ((signed char)(b91_data.rx_buffer[b91_data.
						 rx_buffer[B91_LENGTH_OFFSET] + B91_RSSI_OFFSET])) - 110;
...
static void b91_set_tx_payload(uint8_t *payload, uint8_t payload_len)
{
	...
	b91_data.tx_buffer[0] = rf_tx_dma_len & 0xff;
	b91_data.tx_buffer[1] = (rf_tx_dma_len >> 8) & 0xff;
	b91_data.tx_buffer[2] = (rf_tx_dma_len >> 16) & 0xff;
	b91_data.tx_buffer[3] = (rf_tx_dma_len >> 24) & 0xff;
	b91_data.tx_buffer[4] = payload_len + 2;
	memcpy(b91_data.tx_buffer + B91_PAYLOAD_OFFSET, payload, payload_len);
}

/* Enable ack handler */
static void b91_handle_ack_en()
{
	b91_data.ack_handler_en = true;
}

/* Disable ack handler */
static void b91_handle_ack_dis()
{
	b91_data.ack_handler_en = false;
}

/* Handle acknowledge packet */
static void b91_handle_ack()
{
	struct net_pkt *ack_pkt;

	/* allocate ack packet */
	ack_pkt = net_pkt_alloc_with_buffer(b91_data.iface, B91_ACK_FRAME_LEN,
	...
	/* update packet data */
	if (net_pkt_write(ack_pkt, b91_data.rx_buffer + B91_PAYLOAD_OFFSET,
			  B91_ACK_FRAME_LEN) < 0) {
	...
	/* handle ack */
	if (ieee802154_radio_handle_ack(b91_data.iface, ack_pkt) != NET_OK) {
		LOG_INF("ACK packet not handled - releasing.");
	}

	/* release ack_wait semaphore */
	k_sem_give(&b91_data.ack_wait);
...
static void b91_send_ack(uint8_t seq_num)
{
	...
	rf_tx_pkt(b91_data.tx_buffer);
}

/* RX IRQ handler */
static void b91_rf_rx_isr()
{
	uint8_t status;
	uint8_t length;
	uint8_t *payload;
	...
	/* check CRC */
	if (rf_zigbee_packet_crc_ok(b91_data.rx_buffer)) {
		/* get payload length */
		if (IS_ENABLED(CONFIG_IEEE802154_RAW_MODE) ||
		    IS_ENABLED(CONFIG_NET_L2_OPENTHREAD)) {
			length = b91_data.rx_buffer[B91_LENGTH_OFFSET];
		} else {
			length = b91_data.rx_buffer[B91_LENGTH_OFFSET] - B91_FCS_LENGTH;
		}
		...
		pkt = net_pkt_alloc_with_buffer(b91_data.iface, length, AF_UNSPEC, 0, K_NO_WAIT);
		if (!pkt) {
		...
		status = net_recv_data(b91_data.iface, pkt);
		if (status < 0) {
...
/* TX IRQ handler */
static void b91_rf_tx_isr()
{
	/* clear irq status */
	rf_clr_irq_status(FLD_RF_IRQ_TX);

	/* release tx semaphore */
	k_sem_give(&b91_data.tx_wait);
...
static int b91_init(const struct device *dev)
{
	...
	rf_set_rx_dma(b91_data.rx_buffer, 3, B91_TRX_LENGTH);
	rf_set_rxmode();

	/* init IRQs */
	IRQ_CONNECT(IRQ15_ZB_RT, B91_RF_IRQ_PRIORITY, b91_rf_isr, 0, 0);
	riscv_plic_irq_enable(IRQ15_ZB_RT);
	riscv_plic_set_priority(IRQ15_ZB_RT, B91_RF_IRQ_PRIORITY);
	rf_set_irq_mask(FLD_RF_IRQ_RX | FLD_RF_IRQ_TX);

	/* init data variables */
	b91_data.is_started = true;
	b91_data.ack_handler_en = false;
...
NET_DEVICE_INIT(b91_154_radio, CONFIG_IEEE802154_B91_DRV_NAME,
		b91_init, NULL, &b91_data, NULL,
		CONFIG_IEEE802154_B91_INIT_PRIO,
		&b91_radio_api, L2,
		L2_CTX_TYPE, MTU);
#else
DEVICE_AND_API_INIT(b91_154_radio, CONFIG_IEEE802154_B91_DRV_NAME,
		    b91_init, &b91_data, NULL,
