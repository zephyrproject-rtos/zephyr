...
{
	if ((pkt->ieee802154_fc_is_set && (!pkt->ieee802154_ie_is_updated)) ||
	    ((!pkt->ieee802154_fc_is_set) && pkt->ieee802154_ie_is_updated)) {
		/* The radio driver can't handle such frames */
		return false;
	}
	...
	nrf_802154_transmit_metadata_t metadata = {
		.frame_props = {
			...
			.dynamic_data_is_set = pkt->ieee802154_fc_is_set,
		},
		.cca = cca,
	};
...
/* This function cannot be used in the serialized version yet. */
	...
	net_pkt_set_ieee802154_fc_is_set(pkt, nrf5_radio->tx_frame_is_dynamic_data_set);
	net_pkt_set_ieee802154_ie_is_updated(pkt, nrf5_radio->tx_frame_is_dynamic_data_set);
...
void nrf_802154_transmitted_raw(const uint8_t *frame,
				const nrf_802154_transmit_done_metadata_t *metadata)
{
	...
	nrf5_data.tx_frame_is_dynamic_data_set = metadata->frame_props.dynamic_data_is_set;
