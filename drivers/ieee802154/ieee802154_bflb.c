...
/* TX/ACK timeout constants. Sized for the BL616 coexistence arbiter,
 * which is the slowest path; only hit on failure so the margin is harmless.
 */
#define IEEE802154_BFLB_TX_TIMEOUT  K_MSEC(200)
#define IEEE802154_BFLB_ACK_TIMEOUT K_MSEC(100)

/* IEEE 802.15.4-2020 constants for the O-QPSK PHY at 2450 MHz.
 * Symbol period is 16 us, so us <-> symbols is a shift by 4.
 */
#define IEEE802154_BFLB_US_PER_SYMBOL_BITS 4
/* macSifsPeriod, in symbols. */
#define IEEE802154_BFLB_AIFS_SYMBOLS 12
/* Frame Control field bit 5: Ack Request. */
#define IEEE802154_BFLB_FRAME_IS_ACK_REQ(fcf0) (((fcf0) & BIT(5)) != 0)
			      ...
			      struct net_pkt *pkt, struct net_buf *frag)
{
	 ...
	 */
	if (IEEE802154_BFLB_FRAME_IS_ACK_REQ(data[0])) {
...
static int bflb_ieee802154_init(const struct device *dev)
{
	...
	lmac154_fptClear();
	lmac154_setEnhAckWaitTime((IEEE802154_BFLB_AIFS_SYMBOLS + (6 + 39) * 2 + 20)
				  << IEEE802154_BFLB_US_PER_SYMBOL_BITS);
	lmac154_setRxStateWhenIdle(true);
	lmac154_setFramePendingMode(LMAC154_FPT_ANY);

	now_us = k_ticks_to_us_floor64(k_uptime_ticks());
	zb_timer_cfg((uint32_t)(now_us >> IEEE802154_BFLB_US_PER_SYMBOL_BITS));
