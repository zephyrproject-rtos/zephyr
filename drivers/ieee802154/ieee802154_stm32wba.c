...
static inline const struct device *stm32wba_802154_get_device(void)
{
	...
	while (1) {
		...
		if (net_pkt_write(pkt, rx_frame->psdu, pkt_len) != 0) {
			...
			if (net_recv_data(stm32wba_radio->iface, pkt) < 0) {
				...
				net_pkt_unref(pkt);
			} else if (LOG_LEVEL >= LOG_LEVEL_DBG) {
...
static void stm32wba_802154_receive_failed(stm32wba_802154_rx_error_t error)
{
					      ...
					      const struct ieee802154_stm32wba_config *config)
{
	...
	case IEEE802154_STM32WBA_CONFIG_CCA_THRESHOLD:
		...
		if (st_ral_set_cca_energy_detect_threshold(config->cca_thr) != 0) {
			return -EIO;
		}
		break;
	...
	case IEEE802154_STM32WBA_CONFIG_ANTENNA_DIV:
		...
		if (st_ral_set_ant_div_enable(config->ant_div) != 0) {
			return -EIO;
		}
		break;
				      ...
				      struct ieee802154_stm32wba_attr_value *value)
{
	...
	case IEEE802154_STM32WBA_ATTR_CCA_THRESHOLD:
		...
		if (st_ral_get_cca_energy_detect_threshold(&l_cca_thr) != 0) {
			return -ENOENT;
		}
	...
	case IEEE802154_STM32WBA_ATTR_RAND_NUM:
		...
		if (st_ral_mac_gen_rnd_num(&l_rand_num, 1, true) != 0) {
			return -ENOENT;
		}
					     ...
					     energy_scan_done_cb_t done_cb)
{
	...
	if (stm32wba_802154_data.energy_scan_done_cb == NULL) {
		...
		if (st_ral_energy_detection(duration)) {
...
static bool stm32wba_802154_transmit(struct net_pkt *pkt, 
...
static int stm32wba_802154_start(const struct device *dev)
{
	...
	st_ral_tx_power_set(stm32wba_802154_data.txpwr);
...
static void stm32wba_802154_iface_init(struct net_if *iface)
{
	...
	(void)st_ral_set_max_csma_backoff(MAX_CSMA_BACKOFF);
	(void)st_ral_set_max_frame_retries(MAX_FRAME_RETRY);
	(void)st_ral_set_cca_energy_detect_threshold(CCA_THRESHOLD);
#endif
...
static int stm32wba_802154_set_ack_fpb(const struct ieee802154_config *config)
{
	...
	if (config->ack_fpb.extended) {
		if (st_ral_pending_bit_for_ext_addr_set((const uint8_t *)config->ack_fpb.addr)) {
		...
		if (st_ral_pending_bit_for_short_addr_set((const uint16_t)shortAddr)) {
...
static int stm32wba_802154_clear_ack_fpb(const struct ieee802154_config *config)
{
	...
	if (config->ack_fpb.extended) {
		if (st_ral_pending_bit_for_ext_addr_clear((const uint8_t *)config->ack_fpb.addr)) {
		...
		if (st_ral_pending_bit_for_short_addr_clear((const uint16_t)shortAddr)) {
...
void stm32wba_802154_energy_scan_done(int8_t rssiResult)
{
	if (stm32wba_802154_data.energy_scan_done_cb != NULL) {
		...
		stm32wba_802154_data.energy_scan_done_cb = NULL;
		callback(stm32wba_802154_get_device(), rssiResult);
	}
}
