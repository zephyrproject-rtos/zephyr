...
#include "Phy.h"
// TODO #include "PhyInterface.h"
#include "fsl_component_messaging.h"
#include "fsl_component_timer_manager.h"
// TODO #include "fwk_platform_ot.h"
...
void mcxw_get_eui64(uint8_t *eui64)
{
    ...
    // TODO PLATFORM_GetIeee802_15_4Addr(aIeeeEui64);
...
static int mcxw_set_extended_address(const struct device *dev,
...
static int mcxw_filter(const struct device *dev, bool set,
...
{
    struct mcxw_context *mcxw_radio = dev->data;
    __ASSERT(mcxw_radio->state == RADIO_STATE_DISABLED, "mcux_start");
...
{
    struct mcxw_context *mcxw_radio = dev->data;
    __ASSERT(mcxw_radio->state != RADIO_STATE_DISABLED, "mcux_stop");
...
// otPlatRadioSleep
void mcxw_radio_sleep()
{
    __ASSERT_NO_MSG(((mcxw_ctx.state != RADIO_STATE_TRANSMIT) && (mcxw_ctx.state != RADIO_STATE_DISABLED)));
...
// otPlatRadioTransmit
static int mcxw_tx(const struct device *dev,
		   enum ieee802154_tx_mode mode,
		   struct net_pkt *pkt,
		   struct net_buf *frag)
{
    struct mcxw_context *mcxw_radio = dev->data;
    /* tx_data bufer has reserved memory for both macToPdDataMessage_t and actual data frame after */
    ...
    __ASSERT(mcxw_radio->state != RADIO_STATE_DISABLED, "mcxw_tx: radio disabled");
    ...
    mcxw_radio->tx_frame.length = payload_len;
    ...
    /*  tx_frame.psdu will point to tx_frame.tx_data data buffer after macToPdDataMessage_t structure */
    ...
    switch(mode)
    {
        ...
        case IEEE802154_TX_MODE_TXTIME_CCA:
            ...
            msg->msgData.dataReq.startTime = rf_adjust_tstamp_from_app(mcxw_radio->tx_frame.tx_delay);
        ...
        if (!net_pkt_ieee802154_frame_secured(pkt))
        {
            ...
            if (!net_pkt_ieee802154_mac_hdr_rdy(pkt))
            {
                ...
                if (mcxw_radio->csl_period)
                {
                    ...
                    SetCslIe(mcxw_radio->tx_frame.psdu, mcxw_radio->tx_frame.length,
                            mcxw_radio->csl_period, rf_compute_csl_phase(hdrTimeUs));
                }
#endif
            }
        }
...
void mcxw_rx_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    while(true)
    {
        ...
        // free phy allocated buffer and network packet
        MSG_Free(rx_frame.phy_buffer);
...
static int mcxw_energy_scan(const struct device *dev,
				  uint16_t duration,
				  energy_scan_done_cb_t done_cb)
{
    ...
    __ASSERT_NO_MSG(((mcxw_radio->state != RADIO_STATE_TRANSMIT) && (mcxw_radio->state != RADIO_STATE_DISABLED)));
...
// otPlatRadioConfigureEnhAckProbing - TODO check if correct
...
/*
 * Compute the CSL Phase for the aTimeUs - i.e. the time from the aTimeUs to mcxw_ctx.csl_sample_time.
 * The assumption is that mcxw_ctx.csl_sample_time > aTimeUs. Since the time is kept with a limited timer in reality
 * it means that sometimes mcxw_ctx.csl_sample_time < aTimeUs, when the timer overflows. Therefore the formula should be:
 *
...
static uint16_t rf_compute_csl_phase(uint32_t aTimeUs)
{
    ...
    uint32_t cslPhaseUs  = (cslPeriodUs - (aTimeUs % cslPeriodUs) + (mcxw_ctx.csl_sample_time % cslPeriodUs)) % cslPeriodUs;
...
// TBD: Should MLMECcaRequest be called instead?
...
phyStatus_t PD_MAC_SapHandler(void *pMsg, instanceId_t instance)
{
    ...
    // TODO PWR_DisallowDeviceToSleep();
    ...
    case gPdDataCnf_c:
        ...
        {
            SetFrameCounter(mcxw_ctx.tx_frame.psdu, mcxw_ctx.tx_frame.length, pDataMsg->fc);
            mcxw_ctx.tx_frame.hdr_updated = true;
            // when should mcxw_ctx.tx_frame.sec_processed be set to true?
        }
#endif
        ...
        memcpy(mcxw_ctx.rx_ack_frame.psdu, pDataMsg->msgData.dataCnf.ackData, mcxw_ctx.rx_ack_frame.length);
    ...
    case gPlmeTimeoutInd_c:
        if (RADIO_STATE_TRANSMIT == mcxw_ctx.state)
        {
            ...
            if (IsKeyIdMode1(mcxw_ctx.tx_frame.psdu, mcxw_ctx.tx_frame.length) &&
                !mcxw_ctx.tx_frame.sec_processed && !mcxw_ctx.tx_frame.hdr_updated)
            {
                SetFrameCounter(mcxw_ctx.tx_frame.psdu, mcxw_ctx.tx_frame.length, pPlmeMsg->fc);
                mcxw_ctx.tx_frame.hdr_updated = true;
                // when should mcxw_ctx.tx_frame.sec_processed be set to true?
            }
#endif
...
static int mcxw_configure(const struct device *dev,
			  ...
			  const struct ieee802154_config *config)
{
    ...
    case IEEE802154_CONFIG_ACK_FPB:
        if(config->ack_fpb.enabled)
            return mcxw_src_match_entry(config->ack_fpb.extended,
                                        config->ack_fpb.addr);
        else
            return mcxw_src_clear_entry(config->ack_fpb.extended,
                                        config->ack_fpb.addr);
	...
	case IEEE802154_CONFIG_RX_SLOT:
        mcxw_receive_at(config->rx_slot.channel,
                config->rx_slot.start / NSEC_PER_USEC,
	...
	case IEEE802154_CONFIG_EVENT_HANDLER:
        // TODO
		// mcxw_data.event_handler = config->event_handler;
		break;
...
static int mcxw_attr_get(const struct device *dev,
...
static int mcxw_init(const struct device *dev)
{
    ...
    // Register Phy Data Service Access Point and Phy Layer Management Entities Service Access Point handlers */
    Phy_RegisterSapHandlers((PD_MAC_SapHandler_t)PD_MAC_SapHandler, (PLME_MAC_SapHandler_t)PLME_MAC_SapHandler,
                            ot_phy_ctx);
    ...
    k_msgq_init(&mcxw_radio->rx_msgq, mcxw_radio->rx_msgq_buffer, sizeof(struct mcxw_rx_frame), NMAX_RXRING_BUFFERS);
    ...
    k_thread_create(&mcxw_radio->rx_thread, mcxw_radio->rx_stack,
        CONFIG_IEEE802154_MCXW_RX_STACK_SIZE,
        mcxw_rx_thread, mcxw_radio, NULL, NULL,
...
static void mcxw_iface_init(struct net_if *iface)
{
	...
	net_if_set_link_addr(iface, mcxw_radio->mac, sizeof(mcxw_radio->mac),
                        NET_LINK_IEEE802154);
...
NET_DEVICE_DT_INST_DEFINE(
    ...
    &mcxw_ctx,
    NULL,
    CONFIG_IEEE802154_MCXW_INIT_PRIO,
    &mcxw71_radio_api,
    L2,
    L2_CTX_TYPE,
    MTU);
