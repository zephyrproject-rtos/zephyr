...
#define DEFINE_SREG_READ(__reg_name, __reg_addr)			\
	...
	{								\
		u8_t val;						\
									\
		if (_cc2520_access(spi, true, CC2520_INS_MEMRD,		\
				   __reg_addr, &val, 1)) {		\
			return val;					\
		}							\
									\
...
#define DEFINE_FREG_READ(__reg_name, __reg_addr)			\
	...
	{								\
		u8_t val;						\
									\
		if (_cc2520_access(spi, true, CC2520_INS_REGRD,		\
				   __reg_addr, &val, 1)) {		\
			return val;					\
		}							\
									\
...
#define DEFINE_FREG_WRITE(__reg_name, __reg_addr)			\
	static inline bool write_reg_##__reg_name(struct spi_config *spi, \
						  u8_t val)		\
	{								\
		return _cc2520_access(spi, false, CC2520_INS_REGWR,	\
				      __reg_addr, &val, 1);		\
	}
...
#define DEFINE_MEM_WRITE(__mem_name, __addr, __sz)			\
	static inline bool write_mem_##__mem_name(struct spi_config *spi, \
						  u8_t *buf)		\
	{								\
		return _cc2520_access(spi, false, CC2520_INS_MEMWR,	\
				      __addr, buf, __sz);		\
...
static inline bool _cc2520_command_strobe(struct spi_config *spi,
					  u8_t instruction)
{
	return _cc2520_access(spi, false, instruction, 0, NULL, 0);
}

static inline bool _cc2520_command_strobe_snop(struct spi_config *spi,
					       u8_t instruction)
{
	u8_t snop[1] = { CC2520_INS_SNOP };

	return _cc2520_access(spi, false, instruction, 0, snop, 1);
}
