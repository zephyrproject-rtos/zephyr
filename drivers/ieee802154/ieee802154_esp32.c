...
static struct esp32_data esp32_data;
...
{
	struct esp32_data *data = dev->data;
	int err;
...
static int esp32_filter(const struct device *dev, bool set,
...
static int handle_ack(struct esp32_data *data)
{
	uint8_t ack_len;
...
/* override weak function in components/ieee802154/esp_ieee802154.c of ESP-IDF */
void esp_ieee802154_transmit_done(const uint8_t *tx_frame, const uint8_t *ack_frame,
		    ...
		    struct net_buf *frag)
{
	...
	case IEEE802154_TX_MODE_TXTIME_CCA:
		...
		err = esp_ieee802154_transmit_at(frag->data,
	...
	LOG_HEXDUMP_DBG(frag->data, frag->len, "TX buffer:");
...
static int esp32_start(const struct device *dev)
{
	...
	return esp_ieee802154_enable() == 0 ? 0 : -EIO;
}
			   ...
			   const struct ieee802154_config *config)
{
	...
	/* configure currently not supported */

	return -ENOTSUP; /* TODO: Return -ENOSYS? */
}
