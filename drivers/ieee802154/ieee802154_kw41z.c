...
{
	return (ZLL->SEQ_STATE & ZLL_SEQ_STATE_SEQ_STATE_MASK) >> ZLL_SEQ_STATE_SEQ_STATE_SHIFT;
}
...
static inline void kw41z_set_seq_state(uint8_t state)
{
	...
	ZLL->PHY_CTRL = (ZLL->PHY_CTRL & ~ZLL_PHY_CTRL_XCVSEQ_MASK) | ZLL_PHY_CTRL_XCVSEQ(state);
}
...
static void kw41z_phy_abort(void)
{
	...
	/* Stop timers */
	ZLL->PHY_CTRL &= ~(ZLL_PHY_CTRL_TMR1CMP_EN_MASK | ZLL_PHY_CTRL_TMR2CMP_EN_MASK |
...
static void kw41z_isr_timeout_cleanup(void)
{
	 ...
	 * and timeout
	 */
	ZLL->PHY_CTRL &= ~(ZLL_PHY_CTRL_TMR3CMP_EN_MASK | ZLL_PHY_CTRL_TC3TMOUT_MASK |
			   ZLL_PHY_CTRL_XCVSEQ_MASK);

	/* Mask SEQ, RX, TX and CCA interrupts */
	ZLL->PHY_CTRL |= ZLL_PHY_CTRL_CCAMSK_MASK | ZLL_PHY_CTRL_RXMSK_MASK |
			 ZLL_PHY_CTRL_TXMSK_MASK | ZLL_PHY_CTRL_SEQMSK_MASK;
...
static void kw41z_isr_seq_cleanup(void)
{
	...
	irqsts &= ~(ZLL_IRQSTS_TMR1IRQ_MASK | ZLL_IRQSTS_TMR2IRQ_MASK | ZLL_IRQSTS_TMR3IRQ_MASK |
...
static int kw41z_set_ieee_addr(const struct device *dev, const uint8_t *ieee_addr)
{
	uint32_t val;
...
static int kw41z_filter(const struct device *dev, bool set, enum ieee802154_filter_type type,
...
static int kw41z_set_txpower(const struct device *dev, int16_t dbm)
{
	if (dbm < KW41Z_OUTPUT_POWER_MIN) {
		LOG_INF("TX-power %d dBm below min of %d dBm, using %d dBm", dbm,
			KW41Z_OUTPUT_POWER_MIN, KW41Z_OUTPUT_POWER_MIN);
		dbm = KW41Z_OUTPUT_POWER_MIN;
	} else if (dbm > KW41Z_OUTPUT_POWER_MAX) {
		LOG_INF("TX-power %d dBm above max of %d dBm, using %d dBm", dbm,
			KW41Z_OUTPUT_POWER_MAX, KW41Z_OUTPUT_POWER_MAX);
...
static inline void kw41z_rx(struct kw41z_context *kw41z, uint8_t len)
{
	...
	pkt = net_pkt_rx_alloc_with_buffer(kw41z->iface, pkt_len, AF_UNSPEC, 0, K_NO_WAIT);
	if (!pkt) {
...
static void handle_ack(struct kw41z_context *kw41z, uint8_t seq_number)
{
	...
	ack_pkt =
		net_pkt_rx_alloc_with_buffer(kw41z->iface, ACK_FRAME_LEN, AF_UNSPEC, 0, K_NO_WAIT);
	if (!ack_pkt) {
	...
	/* Re-create ACK frame. */
	ack_psdu[0] = kw41z_context_data.frame_pending ? ACK_FRAME_TYPE | ACK_FRAME_PENDING_BIT
						       : ACK_FRAME_TYPE;
	ack_psdu[1] = 0;
...
static int kw41z_tx(const struct device *dev, enum ieee802154_tx_mode mode, struct net_pkt *pkt,
		    struct net_buf *frag)
{
	struct kw41z_context *kw41z = dev->data;
	...
	memcpy(((uint8_t *)ZLL->PKT_BUFFER_TX) + 1, (void *)frag->data, payload_len);
#else /* CONFIG_SOC_MKW40Z4 */
	((uint8_t *)ZLL->PKT_BUFFER)[0] = payload_len + KW41Z_FCS_LENGTH;
	memcpy(((uint8_t *)ZLL->PKT_BUFFER) + 1, (void *)frag->data, payload_len);
#endif
	...
	if (ieee802154_is_ar_flag_set(frag)) {
		tx_timeout = kw41z->tx_warmup_time + KW41Z_SHR_PHY_TIME +
			     payload_len * KW41Z_PER_BYTE_TIME + 10 + KW41Z_ACK_WAIT_TIME;

		LOG_DBG("AUTOACK ENABLED: len: %d, timeout: %d, seq: %d", payload_len, tx_timeout,
			frag->data[2]);
		...
		LOG_DBG("AUTOACK DISABLED: len: %d, seq: %d", payload_len, frag->data[2]);
...
static void kw41z_isr(int unused)
{
	...
	LOG_DBG("ENTRY: irqsts: 0x%08X, PHY_CTRL: 0x%08X, "
		...
		irqsts, (unsigned int)ZLL->PHY_CTRL, (unsigned int)seq_state,
		(unsigned int)ZLL->SEQ_CTRL_STS,
		(unsigned int)(ZLL->EVENT_TMR >> ZLL_EVENT_TMR_EVENT_TMR_SHIFT), state);
	...
	if (irqsts & ZLL_IRQSTS_FILTERFAIL_IRQ_MASK) {
		LOG_DBG("Incoming RX failed packet filtering rules: "
			...
			(unsigned int)ZLL->FILTERFAIL_CODE, irqsts, (unsigned int)ZLL->PHY_CTRL,
		...
		KW_DBG_TRACE(KW41_DBG_TRACE_WTRM, irqsts, (unsigned int)ZLL->PHY_CTRL, seq_state);

		if (rx_len > IEEE802154_ACK_LENGTH) {

			LOG_DBG("WMRK irq: seq_state: 0x%08x, rx_len: %d", seq_state, rx_len);
			/*
	...
	if ((state != KW41Z_STATE_IDLE) && (irqsts & ZLL_IRQSTS_SEQIRQ_MASK)) {
		 ...
		 */
			 ...
			 (!(irqsts & ZLL_IRQSTS_RXIRQ_MASK)) && (state != KW41Z_STATE_TX)) {

			LOG_DBG("a) TMR3 timeout: irqsts: 0x%08X, "
				"seq_state: 0x%08X, PHY_CTRL: 0x%08X, "
				"state: %d",
				irqsts, seq_state, (unsigned int)ZLL->PHY_CTRL, state);

			KW_DBG_TRACE(KW41_DBG_TRACE_TMR3, irqsts, (unsigned int)ZLL->PHY_CTRL,
				     seq_state);
			...
			switch (state) {
			case KW41Z_STATE_RX:
				LOG_DBG("RX seq done: SEQ_STATE: 0x%08X", (unsigned int)seq_state);

				KW_DBG_TRACE(KW41_DBG_TRACE_RX, irqsts, (unsigned int)ZLL->PHY_CTRL,
					     seq_state);

				kw41z_tmr3_disable();

				rx_len = (ZLL->IRQSTS & ZLL_IRQSTS_RX_FRAME_LENGTH_MASK) >>
					 ZLL_IRQSTS_RX_FRAME_LENGTH_SHIFT;
			...
			case KW41Z_STATE_TX:
				LOG_DBG("TX seq done");
				KW_DBG_TRACE(KW41_DBG_TRACE_TX, irqsts, (unsigned int)ZLL->PHY_CTRL,
					     seq_state);
		...
		if ((irqsts & ZLL_IRQSTS_TMR3IRQ_MASK) && (!(irqsts & ZLL_IRQSTS_TMR3MSK_MASK))) {
...
static int kw41z_init(const struct device *dev)
{
	...
	ZLL->PHY_CTRL = ZLL_PHY_CTRL_CCATYPE(KW41Z_CCA_MODE1) | ZLL_PHY_CTRL_CRC_MSK_MASK |
			ZLL_PHY_CTRL_PLL_UNLOCK_MSK_MASK |
			/*ZLL_PHY_CTRL_FILTERFAIL_MSK_MASK	|*/
			ZLL_PHY_CTRL_RX_WMRK_MSK_MASK | ZLL_PHY_CTRL_CCAMSK_MASK |
			ZLL_PHY_CTRL_RXMSK_MASK | ZLL_PHY_CTRL_TXMSK_MASK |
			ZLL_PHY_CTRL_CCABFRTX_MASK | ZLL_PHY_CTRL_SEQMSK_MASK;
	...
	ZLL->RX_FRAME_FILTER = ZLL_RX_FRAME_FILTER_FRM_VER_FILTER(3) |
			       ZLL_RX_FRAME_FILTER_CMD_FT_MASK | ZLL_RX_FRAME_FILTER_DATA_FT_MASK |
			       ZLL_RX_FRAME_FILTER_ACK_FT_MASK | ZLL_RX_FRAME_FILTER_BEACON_FT_MASK;
	...
	kw41z->rx_warmup_time = (XCVR_TSM->END_OF_SEQ & XCVR_TSM_END_OF_SEQ_END_OF_RX_WU_MASK) >>
				XCVR_TSM_END_OF_SEQ_END_OF_RX_WU_SHIFT;
	kw41z->tx_warmup_time = (XCVR_TSM->END_OF_SEQ & XCVR_TSM_END_OF_SEQ_END_OF_TX_WU_MASK) >>
...
static int kw41z_configure(const struct device *dev, enum ieee802154_config_type type,
...
NET_DEVICE_DT_INST_DEFINE(0, kw41z_init,                     /* Initialization Function */
