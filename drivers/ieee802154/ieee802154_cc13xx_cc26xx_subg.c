...
static uint32_t ieee802154_cc13xx_overrides_sub_ghz[] = {
	...
	(uint32_t)0x00F788D3, (uint32_t)0xFFFFFFFF};
static uint32_t rf_prop_overrides_tx_std[] = {
	...
	HW_REG_OVERRIDE(0x60A8, 0x0401), (uint32_t)0xFFFFFFFF};
static uint32_t rf_prop_overrides_tx_20[] = {
	...
	HW_REG_OVERRIDE(0x60A8, 0x0001), (uint32_t)0xFFFFFFFF};
...
static void cmd_prop_tx_adv_callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
	...
	LOG_DBG("ch: %u cmd: %04x cs st: %04x tx st: %04x e: 0x%" PRIx64, ch, op->commandNo,
		op->status, drv_data->cmd_prop_tx_adv.status, e);
}
...
static void drv_rx_done(struct ieee802154_cc13xx_cc26xx_subg_data *drv_data)
{
	...
	for (int i = 0; i < CC13XX_CC26XX_NUM_RX_BUF; i++) {
		if (drv_data->rx_entry[i].status == DATA_ENTRY_FINISHED) {
			...
			LOG_DBG("Received: len = %u, rssi = %d status = %u", len, rssi, status);
...
static void cmd_prop_rx_adv_callback(RF_Handle h, RF_CmdHandle ch, RF_EventMask e)
{
	...
	if (op->status == PROP_ERROR_RXBUF || op->status == PROP_ERROR_RXFULL ||
...
static int drv_start_rx(const struct device *dev)
{
	...
	cmd_handle = RF_postCmd(drv_data->rf_handle, (RF_Op *)&drv_data->cmd_prop_rx_adv,
...
static int ieee802154_cc13xx_cc26xx_subg_set_channel(const struct device *dev, uint16_t channel)
{
	struct ieee802154_cc13xx_cc26xx_subg_data *drv_data = dev->data;
	uint16_t freq, fract;
	RF_EventMask events;
	...
	events = RF_runCmd(drv_data->rf_handle, (RF_Op *)&drv_data->cmd_fs, RF_PriorityNormal, NULL,
			   0);
...
static int ieee802154_cc13xx_cc26xx_subg_filter(const struct device *dev, bool set,
...
static int ieee802154_cc13xx_cc26xx_subg_set_txpower(const struct device *dev, int16_t dbm)
{
	struct ieee802154_cc13xx_cc26xx_subg_data *drv_data = dev->data;
...
static int ieee802154_cc13xx_cc26xx_subg_tx(const struct device *dev, enum ieee802154_tx_mode mode,
...
static int ieee802154_cc13xx_cc26xx_subg_configure(const struct device *dev,
...
static const struct ieee802154_radio_api ieee802154_cc13xx_cc26xx_subg_radio_api = {
...
static int ieee802154_cc13xx_cc26xx_subg_init(const struct device *dev)
{
	...
	drv_data->rf_handle =
		RF_open(&drv_data->rf_object, &rf_mode,
			(RF_RadioSetup *)&ieee802154_cc13xx_subg_radio_div_setup, &rf_params);
	...
	.cmd_prop_rx_adv =
		{
			...
			.pOutput = (uint8_t *)&ieee802154_cc13xx_cc26xx_subg_data
			...
			.csConf =
				{
					 ...
					 */
					.bEnaRssi = true,
					/* Abort as soon as any energy above the ED threshold is
					   detected. */
					.busyOp = true,
