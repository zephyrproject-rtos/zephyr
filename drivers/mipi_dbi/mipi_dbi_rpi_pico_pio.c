...
#define SM_SEND_BIT_DURATION_US(pio_clk_div)                                                       \
	MAX(((1e6 * CYCLES_PER_BIT * pio_clk_div) / CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC), 1)
...
static int mipi_dbi_pio_configure(const struct device *dev)
{
	...
	mipi_dbi_pio_configure_program(dev_cfg->splits, dev_cfg->split_count);
	...
	for (int i = 0; i < dev_cfg->split_count; ++i) {
		...
		sm_config_set_clkdiv_int_frac(&sm_config, dev_cfg->pio_clock_div, 0);
...
static bool mipi_dbi_pico_pio_all_fifos_empty(struct mipi_dbi_pico_pio_data *data,
					      const struct mipi_dbi_pico_pio_config *config)
{
	bool empty = true;

	for (int i = 0; i < config->split_count; ++i) {
		empty &= pio_sm_is_tx_fifo_empty(data->pio, config->splits[i].sm.sm);
	}
	return empty;
}

static void
mipi_dbi_pico_pio_wait_end_of_transmission(struct mipi_dbi_pico_pio_data *data,
					   const struct mipi_dbi_pico_pio_config *config)
{
	while (!mipi_dbi_pico_pio_all_fifos_empty(data, config)) {
	};
	/*
	 ...
	 */
	k_busy_wait(SM_SEND_BIT_DURATION_US(config->pio_clock_div));
}
					  ...
					  size_t len)
{
	...
	case MIPI_DBI_MODE_8080_BUS_16_BIT:
		...
		if (cmd_present) {
			...
			for (int i = 0; i < config->split_count; ++i) {
				pio_sm_put_blocking(data->pio, config->splits[i].sm.sm,
						    (uint32_t)cmd);
			}
			pio_enable_sm_mask_in_sync(data->pio, data->sm_mask);
			mipi_dbi_pico_pio_wait_end_of_transmission(data, config);
		...
		if (len > 0) {
			...
			for (int i = 0; i < config->split_count; ++i) {
				WRITE_BIT(dma_channel_mask, config->splits[i].dma.channel, true);
				config->splits[i].dma.head_block.block_size = len;
				config->splits[i].dma.head_block.source_address =
					(uint32_t)&data_buf[0];
				dma_config(config->dev_dma, config->splits[i].dma.channel,
					   &config->splits[i].dma.config);
				dma_start(config->dev_dma, config->splits[i].dma.channel);
			}
...
#define DATA_PIN_SPLIT_GET_BY_IDX(node_id, prop, idx)                                              \
	{.pin_base = DT_PHA_BY_IDX(node_id, prop, idx, base_pin),                                  \
	 .pin_count = DT_PHA_BY_IDX(node_id, prop, idx, consecutive_pins),                         \
	 .pin_discarded = DT_FOREACH_PROP_ELEM_SEP_VARGS(node_id, prop, DISCARDED_PINS, (+), idx)}
...
#define PIO_MIPI_DBI_INIT(n)                                                                       \
	...
	K_MSGQ_DEFINE(dma_msgq##n, sizeof(int), MIPI_DBI_MAX_SPLITS, 4);                           \
                                                                                                   \
	static struct mipi_dbi_pico_pio_split splits_##n[] = {DT_INST_FOREACH_PROP_ELEM_SEP(       \
		n, data_pin_splits, DATA_PIN_SPLIT_GET_BY_IDX, (,))};                              \
                                                                                                   \
