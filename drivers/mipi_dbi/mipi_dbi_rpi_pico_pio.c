...
#define BIT_COUNT(value, bit_count) (value | SET_BITS(bit_count, 0))
...
#define BASE_SM_INSTRUCTION(n, idx, cpc, pc, ss, opt)                                              \
	static uint16_t inst_##n##_program_##idx##_instructions[] = {                              \
		DELAY_SIDESET(0x90e0, opt, ss, SIDE(1, 1, 1, cpc, n), 0),                          \
		0x6020,                                                                            \
		DELAY_SIDESET(BIT_COUNT(0x7040, pc), opt, ss, SIDE(1, 1, 0, cpc, n), 0),           \
		0x006b,                                                                            \
		DELAY_SIDESET(0xb042, opt, ss, SIDE(1, 0, 0, cpc, n), 0),                          \
		DELAY_SIDESET(0x90e0, opt, ss, SIDE(1, 0, 0, cpc, n), 1),                          \
		DELAY_SIDESET(BIT_COUNT(0x7000, pc), 1, ss, SIDE(0, 0, 0, cpc, n), 1),             \
		DELAY_SIDESET(0x102c, opt, ss, SIDE(1, 0, 0, cpc, n), 1),                          \
		0x0049,                                                                            \
		DELAY_SIDESET(0x90e0, opt, ss, SIDE(1, 1, 0, cpc, n), 1),                          \
		DELAY_SIDESET(BIT_COUNT(0x7000, pc), opt, ss, SIDE(0, 1, 0, cpc, n), 0),           \
		0x0049,                                                                            \
		DELAY_SIDESET(0xd000, opt, ss, SIDE(1, 1, 0, cpc, n), 1),                          \
	};                                                                                         \
...
#define SM_INSTRUCTION(n, idx, pin_count, pin_discarded)                                           \
	static uint16_t inst_##n##_program_##idx##_instructions[] = {                              \
		BIT_COUNT(0x6020, (pin_count) + (pin_discarded)),                                  \
		0x0303,                                                                            \
		0x0243,                                                                            \
		BIT_COUNT(0x6160, pin_discarded),                                                  \
		BIT_COUNT(0x6000, pin_count),                                                      \
		0x0023,                                                                            \
	};                                                                                         \
