		...
		if (lcdif_data->height > 0) {
			if (lcdif_data->heightEachWrite > lcdif_data->height) {
				...
				if (lcdif_data->dsiFormat == kDSI_DbiRGB565) {
					DSI_SetDbiPixelPayloadSize(mipi_base,
												height * lcdif_data->width);
				} else if (lcdif_data->dsiFormat == kDSI_DbiRGB332) {
					/* 2 pixel sent in 1 cycle. */
					DSI_SetDbiPixelPayloadSize(mipi_base,
												(height * lcdif_data->width) >> 1U);
				} else {
					/* 2 pixels sent in 3 cycles. */
					DSI_SetDbiPixelPayloadSize(mipi_base,
												(height * lcdif_data->width * 3U) >> 1);
				}
				LCDIF_SetFrameBufferPosition(config->base, 0U, 0U, 0U,
												lcdif_data->width, height);
			}

			LCDIF_DbiSelectArea(config->base, 0, 0,
								lcdif_data->heightSent,
								lcdif_data->width - 1U,
						   ...
						   enum display_pixel_format pixfmt)
{
	...
	if (lcdif_data->dsiFormat == kDSI_DbiRGB565) {
		...
		/* 2 pixels sent in 3 cycles.
		 * kDSI_DbiRGB888, kDSI_DbiRGB444 and kDSI_DbiRGB666
		 */
		payloadBytePerPixel = 3U;
	}

	/* If the whole update payload exceeds the DSI max payload size,
	 * send the payload in multiple times.
	 */
	if ((desc->width * desc->height * payloadBytePerPixel) > MIPI_DSI_MAX_PAYLOAD_SIZE) {
		/* Calculate how may lines to send each time. Make sure each time the buffer
		 * address meets the align requirement.
		 */
		lcdif_data->heightEachWrite = MIPI_DSI_MAX_PAYLOAD_SIZE / desc->width /
			payloadBytePerPixel;

		while (((lcdif_data->heightEachWrite * bypes_per_pixel * desc->pitch)
				 & (LCDIF_FB_ALIGN - 1U)) != 0U) {
			lcdif_data->heightEachWrite--;
		}

		/* Point the data to the next piece. */
		lcdif_data->data = (uint8_t *)((uint32_t)framebuf +
			(lcdif_data->heightEachWrite * bypes_per_pixel * desc->pitch));
	}

	/* Set payload size. */
	DSI_SetDbiPixelPayloadSize(mipi_base,
		(lcdif_data->heightEachWrite * desc->width * payloadBytePerPixel) >> 1U);
	...
	LCDIF_DbiSelectArea(config->base, 0, 0, 0, desc->width - 1U,
						lcdif_data->heightEachWrite - 1U, false);
