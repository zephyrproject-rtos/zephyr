...
static int regulator_npm13xx_set_buck_pin_ctrl(const struct device *dev, uint8_t chan, uint8_t pin,
					       uint8_t inv, enum npm13xx_gpio_type type)
{
	const struct regulator_npm13xx_config *config = dev->config;
	uint8_t ctrl;
	uint8_t mask;
...
static int regulator_npm13xx_set_ldsw_pin_ctrl(const struct device *dev, uint8_t chan, uint8_t pin,
					       uint8_t inv, enum npm13xx_gpio_type type)
{
	const struct regulator_npm13xx_config *config = dev->config;
	uint8_t ctrl;
...
int regulator_npm13xx_set_pin_ctrl(const struct device *dev, const struct gpio_dt_spec *spec,
				   enum npm13xx_gpio_type type)
{
	...
	switch (config->source) {
	case NPM13XX_SOURCE_BUCK1:
		return regulator_npm13xx_set_buck_pin_ctrl(dev, 0, spec->pin, inv, type);
	case NPM13XX_SOURCE_BUCK2:
		return regulator_npm13xx_set_buck_pin_ctrl(dev, 1, spec->pin, inv, type);
	case NPM13XX_SOURCE_LDO1:
		return regulator_npm13xx_set_ldsw_pin_ctrl(dev, 0, spec->pin, inv, type);
	case NPM13XX_SOURCE_LDO2:
		return regulator_npm13xx_set_ldsw_pin_ctrl(dev, 1, spec->pin, inv, type);
	default:
		return -ENODEV;
	}
}
