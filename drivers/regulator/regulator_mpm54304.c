...
static int regulator_mpm54304_enable(const struct device *dev)
{
	...
	return i2c_reg_update_byte_dt(&config->bus, MPM54304_REG_EN,
				      config->enable_mask, 0xFF);
}
...
static int regulator_mpm54304_init(const struct device *dev)
{
	...
	LOG_INF("vendor id: 0x%x", value >> 4);
...
#define REGULATOR_MPM54304_DEFINE(node_id, id, child_name)					\
	...
	DEVICE_DT_DEFINE(node_id, regulator_mpm54304_init, NULL,				\
			 &regulator_mpm54304_data_##id,						\
			 &regulator_mpm54304_config_##id,					\
			 POST_KERNEL,								\
			 CONFIG_REGULATOR_MPM54304_INIT_PRIORITY,				\
			 &mpm54304_api);

#define REGULATOR_MPM54304_DEFINE_COND(inst, child)						\
	IF_ENABLED(DT_NODE_EXISTS(DT_INST_CHILD(inst, child)),					\
		    (REGULATOR_MPM54304_DEFINE(DT_INST_CHILD(inst, child),			\
					       child##inst, child)))

#define REGULATOR_MPM54304_DEFINE_ALL(inst)							\
	REGULATOR_MPM54304_DEFINE_COND(inst, buck1)						\
	REGULATOR_MPM54304_DEFINE_COND(inst, buck2)						\
	REGULATOR_MPM54304_DEFINE_COND(inst, buck3)						\
	REGULATOR_MPM54304_DEFINE_COND(inst, buck4)
