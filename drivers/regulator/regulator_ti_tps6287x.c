...
#define TPS6287X_INIT_RETRY_WAIT_US 100U /* Wait 100 µs before another retry. */
					   ...
					   int32_t *volt_uv)
{
	...
	for (uint8_t i = 0U; i < ARRAY_SIZE(voltage_ranges); i++) {
		...
		if (idx < count) {
			return linear_range_get_value(&voltage_ranges[i], idx, volt_uv);
		}
		idx -= count;
	}
...
static int regulator_tps6287x_init(const struct device *dev)
{
	...
	for (int i = 0; i < TPS6287X_MAX_INIT_RETRY && rc != 0; i++) {
		rc = i2c_reg_read_byte_dt(&cfg->i2c, TPS6287X_REG_STATUS, &status);
		if (rc < 0) {
			k_busy_wait(TPS6287X_INIT_RETRY_WAIT_US);
		}
	}
