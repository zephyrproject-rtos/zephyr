...
static int regulator_max20362_set_dvs_source(const struct device *dev, uint8_t source)
{
	const struct regulator_max20362_common_config *config = dev->config;
...
static DEVICE_API(regulator_parent, parent_api) = {
	.dvs_state_set = regulator_max20362_set_dvs_source,
};
...
#define REGULATOR_MAX20362_DEFINE_ALL(inst)                                                        \
	...
	static const uint32_t rr_voltages_##inst[] = DT_INST_PROP_OR(inst, rr_microvolt, {0});     \
	...
	DEVICE_DT_INST_DEFINE(inst, regulator_max20362_common_init, NULL, &common_data_##inst,     \
			      ...
			      CONFIG_REGULATOR_ADI_MAX20362_COMMON_INIT_PRIORITY, &parent_api);    \
                                                                                                   \
