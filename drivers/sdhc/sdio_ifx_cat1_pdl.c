...
static int ifx_cat1_sdio_set_io(const struct device *dev, struct sdhc_io *ios)
{
	...
	// TODO: SREP Update to support chaning clock config ??
	return 0;
}
...
static int ifx_cat1_sdio_card_busy(const struct device *dev)
{
	...
	//     result (_CYHAL_SDXX_NOT_RUNNING != sdxx->data_transfer_status);
	return 0;
}
...
static cy_rslt_t sdio_host_send_cmd(SDHC_Type *base, ifx_sdio_host_command_t command,
...
#define _CYHAL_SDIO_TRANSFER_TRIES (50U)
	...
	do {
		...
		if (Cy_SD_Host_GetNormalInterruptStatus(base) & CY_SD_HOST_ERR_INTERRUPT) {
			...
			/* Usually it is enough ~ 15 us for reset to complete on 100 MHz HF clock
			 * for any supported compiler / optimization. Making max time much greater
			 * to cover different frequencies and unusual cases. Timeout and SW_RST_R
			 * check will be removed after DRIVERS-5769 is resolved. */
			uint32_t timeout_us = 1000;
		...
		// 	/*  Enabling command complete interrupt mask if corresponding event was
		// 	 * enabled by user _cyhal_sdio_irq_handler will disable
		// 	 * CY_SD_HOST_CMD_COMPLETE mask once interrupt is generated. */
		...
		if (CY_RSLT_SUCCESS == result) {
			...
			while (retry > 0UL) {
				...
				if (CY_SD_HOST_CMD_COMPLETE ==
				    (CY_SD_HOST_CMD_COMPLETE &
				     Cy_SD_Host_GetNormalInterruptStatus(base))) {
					// TODO: SREP
					// sdxx->data_transfer_status &=
					// ~_CYHAL_SDXX_WAIT_CMD_COMPLETE;

					/* Clear interrupt flag */
					Cy_SD_Host_ClearNormalInterruptStatus(
						base, CY_SD_HOST_CMD_COMPLETE);
	...
	// Cy_SD_Host_PollCmdComplete(base);
...
static cy_rslt_t sdxx_prepare_for_transfer(SDHC_Type *base)
{
	uint32_t activated_cmd_complete = 0;
	// if (sdxx->irq_cause & (sdxx->is_sdio ? CYHAL_SDIO_CMD_COMPLETE :
	// CYHAL_SDHC_CMD_COMPLETE)) {
	// 	/* Activate CY_SD_HOST_CMD_COMPLETE interrupt mask only if user enabled callback for
	// 	 * that event */
...
static cy_en_sd_host_status_t sdxx_pollcmdcomplete(SDHC_Type *base, uint32_t *time_used_ms)
{
	cy_en_sd_host_status_t result = CY_SD_HOST_ERROR_TIMEOUT;
	// uint32_t retry = _CYHAL_SDHC_RETRY_TIMES * _CYHAL_SDIO_CMD_CMPLT_DELAY_US;
	uint32_t retry = 1000 * 5;
	...
	if (time_used_ms != NULL) {
		// *time_used_ms = (((_CYHAL_SDHC_RETRY_TIMES * _CYHAL_SDIO_CMD_CMPLT_DELAY_US) -
		// retry) /1000);
		*time_used_ms = (((1000 * 5) - retry) / 1000);
	}
						       ...
						       uint16_t length)
{
	...
	do {
		...
		if (CY_SD_HOST_SUCCESS == result) {
			result = (cy_rslt_t)sdxx_pollcmdcomplete(base, NULL);
		}
						      ...
						      uint16_t length, uint32_t *response)
{
	...
	do {
		...
		if (CY_SD_HOST_SUCCESS == result) {
			...
			//  result = sdxx_waitfor_transfer_complete(sdxx);
...
#define IFX_CAT1_SDHC_INIT(n)                                                                      \
	...
	DEVICE_DT_INST_DEFINE(n, &ifx_cat1_sdio_init, NULL, &ifx_cat1_sdio_##n##_data,             \
			      &ifx_cat1_sdio_##n##_config, PRE_KERNEL_1, CONFIG_SDHC_INIT_PRIORITY, \
			      &ifx_cat1_sdio_api);
