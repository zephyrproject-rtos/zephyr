...
static void sdhc_max32_init_props(const struct device *dev)
{
	...
	sdhc_data->props.host_caps.vol_180_support = true;
...
static int sdhc_max32_execute_tuning(const struct device *dev)
{
	return ENOTSUP;
}
...
#define DEFINE_SDHC_MAX32(_num)                                                                    \
	...
	static const struct sdhc_max32_config sdhc_max32_config_##_num = {                         \
		...
		.bus_volt = MXC_SDHC_Bus_Voltage_1_8 + DT_ENUM_IDX_OR(_num, sd_bus_voltage, 0),    \
