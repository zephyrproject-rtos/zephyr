...
static int sdhc_esp32_controller_init(const struct device *dev)
{
	...
	/*
	 * Attach interrupt handler. IRQ_CONNECT cannot fail, so the rollback the
	 * allocating call needed is gone; kept here for reference in case a
	 * failable step is ever put back on this path:
	 *
...
static int sdhc_esp32_init(const struct device *dev)
{
	...
	if (ret != 0) {
		k_msgq_purge(data->s_host_ctx.event_queue);
		/* Was: irq_matrix_disable(cfg->irq, cfg->source); */
		irq_disable(cfg->irq);
		return ret;
	}
