...
static int xlnx_sdhc_host_props(const struct device *dev, struct sdhc_host_props *props)
{
	...
	props->host_caps_extra.bus_4_bit_support = XLNX_SDHC_GET_HOST_PROP_BIT(cap,
			XLNX_SDHC_4BIT_SUPPORT);

	if ((cap & CHECK_BITS(XLNX_SDHC_SDR400_SUPPORT)) != 0U) {
		props->host_caps_extra.hs400_support = (uint8_t)config->hs400_mode;
		dev_data->has_phy = true;
	}
	props->host_caps_extra.hs200_support = (uint8_t)config->hs200_mode;
