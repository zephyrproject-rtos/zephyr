...
uint8_t sdhc_standard_read8(const struct device *dev, uint32_t reg)
{
	...
	if (cfg->ops != NULL && cfg->ops->read8 != NULL) {
		return cfg->ops->read8(dev, reg);
	}
#endif
	base = SDHC_STANDARD_DEV_MMIO_GET(dev);
...
void sdhc_standard_write8(const struct device *dev, uint32_t reg, uint8_t value)
{
	...
	if (cfg->ops != NULL && cfg->ops->write8 != NULL) {
	...
	base = SDHC_STANDARD_DEV_MMIO_GET(dev);
	reg_value = sys_read32(base + reg_addr);
	reg_value &= ~(0x000000ffU << (8U * reg_byte_offset));
	reg_value |= ((uint32_t)value << (8U * reg_byte_offset));
	sys_write32(reg_value, base + reg_addr);
}
...
uint16_t sdhc_standard_read16(const struct device *dev, uint32_t reg)
{
	...
	if (cfg->ops != NULL && cfg->ops->read16 != NULL) {
...
void sdhc_standard_write16(const struct device *dev, uint32_t reg, uint16_t value)
{
	...
	if (cfg->ops != NULL && cfg->ops->write16 != NULL) {
	...
	base = SDHC_STANDARD_DEV_MMIO_GET(dev);
	reg_value = sys_read32(base + reg_addr);
	reg_value &= ~(0x0000ffffU << (8U * reg_byte_offset));
	reg_value |= ((uint32_t)value << (8U * reg_byte_offset));
	sys_write32(reg_value, base + reg_addr);
}
...
uint32_t sdhc_standard_read32(const struct device *dev, uint32_t reg)
{
	...
	if (cfg->ops != NULL && cfg->ops->read32 != NULL) {
...
void sdhc_standard_write32(const struct device *dev, uint32_t reg, uint32_t value)
{
	...
	if (cfg->ops != NULL && cfg->ops->write32 != NULL) {
...
uint64_t sdhc_standard_read64(const struct device *dev, uint32_t reg)
{
	...
	if (cfg->ops != NULL && cfg->ops->read64 != NULL) {
...
void sdhc_standard_write64(const struct device *dev, uint32_t reg, uint64_t value)
{
	...
	if (cfg->ops != NULL && cfg->ops->write64 != NULL) {
		cfg->ops->write64(dev, reg, value);
		return;
	}
#endif
	base = SDHC_STANDARD_DEV_MMIO_GET(dev);
	sys_write32(value & 0xffffffff, base + reg);
	sys_write32((value >> 32) & 0xffffffff, base + reg + 4);
}

void sdhc_standard_capabilities_init(const struct device *dev)
{
	__ASSERT_NO_MSG(dev != NULL);
	...
	LOG_DBG("SDHC standard capabilities: 0x%016" PRIx64, val);
