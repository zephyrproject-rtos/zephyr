...
static void ra_sdmmc_accs_isr(const struct device *dev)
{
	ARG_UNUSED(dev);
	sdhimmc_accs_isr();
}

static void ra_sdmmc_card_isr(const struct device *dev)
{
	ARG_UNUSED(dev);
	sdhimmc_card_isr();
}

static void ra_sdmmc_dma_req_isr(const struct device *dev)
{
	ARG_UNUSED(dev);
	sdhimmc_dma_req_isr();
}

void ra_sdio_int_isr(const struct device *dev)
{
	...
	if (info1 & R_SDHI0_SDIO_INFO1_EXWT_Msk) { /* EXWT: CMD53 transfer complete */
		priv->sdmmc_event.transfer_completed = true;
		k_sem_give(&priv->sdmmc_event.transfer_sem);
	}

	if (info1 & R_SDHI0_SDIO_INFO1_IOIRQ_Msk) {
		/*
		 * Mask IOIRQ before clearing the ICU: if the card interrupt line
		 * is still asserted the peripheral re-sets R_SDHI0_SDIO_INFO1_MASK_IOIRQM_Msk
		 * immediately, which would re-latch the ICU and storm the ISR.  The upper layer
		 * re-enables via sdhc_enable_interrupt() after servicing the card.
		 */
		priv->sdmmc_ctrl.p_reg->SDIO_INFO1_MASK |= R_SDHI0_SDIO_INFO1_MASK_IOIRQM_Msk;
		if (priv->cb) {
				    ...
				    void *user_data)
{
	...
	if (sources & SDHC_INT_SDIO) {
		priv->sdmmc_ctrl.p_reg->SDIO_MODE |= R_SDHI0_SDIO_MODE_INTEN_Msk;
		priv->sdmmc_ctrl.p_reg->SDIO_INFO1_MASK &=
			~(R_SDHI0_SDIO_INFO1_MASK_EXWTM_Msk | R_SDHI0_SDIO_INFO1_MASK_EXPUB52M_Msk |
			  R_SDHI0_SDIO_INFO1_MASK_IOIRQM_Msk);
	}

	if (sources & SDHC_INT_INSERTED) {
		priv->sdmmc_ctrl.p_reg->SD_INFO1 = ~R_SDHI0_SD_INFO1_SDD3IN_Msk;
		priv->sdmmc_ctrl.p_reg->SD_INFO1_MASK &= ~R_SDHI0_SD_INFO1_MASK_SDD3INM_Msk;
	}

	if (sources & SDHC_INT_REMOVED) {
		priv->sdmmc_ctrl.p_reg->SD_INFO1 = ~R_SDHI0_SD_INFO1_SDD3RM_Msk;
		priv->sdmmc_ctrl.p_reg->SD_INFO1_MASK &= ~R_SDHI0_SD_INFO1_MASK_SDD3RMM_Msk;
	}
...
static int sdhc_ra_reset(const struct device *dev)
{
	...
	/* Reset SDHI. */
	((R_SDHI0_Type *)cfg->regs)->SOFT_RST &= ~R_SDHI0_SOFT_RST_SDRST_Msk;
	((R_SDHI0_Type *)cfg->regs)->SOFT_RST |= R_SDHI0_SOFT_RST_SDRST_Msk;
