...
#define DT_DRV_COMPAT microchip_sama7g5_sdhci
...
struct sd_request {
	uint32_t flags;
#define REQ_RSP_PRESENT	(1 << 0)
#define REQ_RSP_136	(1 << 1)
#define REQ_RSP_DATA	(1 << 2)
#define REQ_RSP_TRFC	(1 << 3)
#define REQ_USE_ADMA	(1 << 4)
#define REQ_IS_READ	(1 << 5)
...
struct sam_sdmmc_config {
	...
	uint8_t  no_18v;
	uint8_t  voltage_300;
	uint8_t  rstn_power_en;
	uint8_t  auto_cmd12;
	uint8_t  auto_cmd23;
	uint8_t  mmc_hs200_18v;
	uint8_t  mmc_hs400_18v;
	uint32_t max_bus_clk;
...
static int sdmmc_set_clock(sdmmc_registers_t *sdmmc, uint32_t base_clk, uint32_t clock)
{
	...
	if (mult_clk && ((mult_clk / (divider + 1)) <= clock)) {
...
static int sam_sdmmc_isr(const struct device *dev)
{
	...
	if (req) {
		if (!(req->flags & REQ_RSP_MASK) || req->reset_mask) {
...
static int sam_sdmmc_reset(const struct device *dev)
{
	...
	/* Set max timeout */
	sdmmc->SDMMC_TCR = SDMMC_TCR_DTCVAL(0xE);
...
static int sam_sdmmc_set_io(const struct device *dev, struct sdhc_io *ios)
{
	...
	if (ios->clock != io_cfg->clock) {
		if ((config->max_bus_clk) &&
		    (ios->clock > config->max_bus_clk)) {
			ret = sdmmc_set_clock(sdmmc, config->base_clk, config->max_bus_clk);
		} else {
	...
	if (ios->bus_mode != io_cfg->bus_mode) {
		...
		} else if (ios->bus_mode == SDHC_BUSMODE_PUSHPULL) {
	...
	if (ios->timing != io_cfg->timing) {
		if (ios->timing == SDHC_TIMING_LEGACY) {
			sdmmc_high_speed(sdmmc, 0);
		} else if (ios->timing == SDHC_TIMING_HS) {
			sdmmc_high_speed(sdmmc, 1);
		} else {
			/* UHS-I mode is TBD */
			return -ENOTSUP;
...
static int sam_sdmmc_get_host_props(const struct device *dev, struct sdhc_host_props *props)
{
	...
	if (config->voltage_300) {
		props->host_caps.vol_300_support = true;
	}
	if (!config->no_18v) {
...
#define SAM_SDMMC_INIT(N)									\
	...
	static const struct sam_sdmmc_config sdmmc_##N##_config = {				\
		...
		.voltage_300     = DT_INST_PROP(N, voltage_300),				\
		...
		.max_bus_clk     = DT_INST_PROP(N, max_bus_clk),				\
