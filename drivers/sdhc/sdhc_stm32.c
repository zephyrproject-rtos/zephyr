				  ...
				  struct sdhc_data *data)
{
	...
	if (!IS_ENABLED(CONFIG_SDHC_STM32_POLLING_SUPPORT)) {
		dev_data->sdio_dma_buf =
			k_aligned_alloc(CONFIG_SDHC_BUFFER_ALIGNMENT,
	...
	if (direction == SDIO_IO_WRITE) {
		...
		if (IS_ENABLED(CONFIG_SDHC_STM32_POLLING_SUPPORT)) {
			...
			memcpy(dev_data->sdio_dma_buf, data->data,
					dev_data->total_transfer_bytes);
			sys_cache_data_flush_range((uint32_t *)dev_data->sdio_dma_buf,
						dev_data->total_transfer_bytes);
				  ...
				  struct sdhc_data *data)
{
	...
	k_mutex_lock(&dev_data->bus_mutex, K_MSEC(cmd->timeout_ms));
...
static int sdhc_stm32_set_io(const struct device *dev, struct sdhc_io *ios)
{
	...
	if ((ios->bus_width != 0) && (host_io->bus_width != ios->bus_width)) {
		uint32_t bus_width_reg_value = SDMMC_BUS_WIDE_1B;
...

{
	...
	if (__HAL_SDIO_GET_FLAG(config->hsd, SDMMC_FLAG_DATAEND)) {
		sys_cache_data_invd_range((uint32_t *)data->sdio_dma_buf,
...
#define STM32_SDHC_IRQ_HANDLER_DECL(index) static void sdhc_stm32_irq_config_func_##index(void)
