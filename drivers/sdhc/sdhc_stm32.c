...
{
	uint32_t errors = HAL_SDIO_GetError(hsd);

	__ASSERT(errors != HAL_SDIO_ERROR_NONE, "sdhc_stm32_log_err_type called with no error");
...
static int sdhc_stm32_reset(const struct device *dev)
{
	...
	config->hsd->ErrorCode = HAL_SDIO_ERROR_NONE;
	config->hsd->State = HAL_SDIO_STATE_RESET;
