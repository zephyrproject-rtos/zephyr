...
struct sdhc_stm32_config {
	SDMMC_TypeDef *Instance; /* Base address of the SDIO/SDMMC peripheral */
...
{
	uint32_t error_code = dev_data->ErrorCode;
...
static int sdhc_stm32_sd_init(const struct device *dev)
{
	...
	if (clock_control_get_rate(clk, (clock_control_subsys_t)(uintptr_t)&config->pclken[0],
				   &sdmmc_clk_rate) < 0) {
...
static int sdhc_stm32_card_busy(const struct device *dev)
{
	...
	return dev_data->State == SDMMC_STATE_BUSY;
}
...
static void sdhc_stm32_clear_icr_flags(SDMMC_TypeDef *Instance)
{
	...
	if ((Instance->STA & SDMMC_STA_DCRCFAIL) != 0U) {
		icr_clear_flag |= SDMMC_ICR_DCRCFAILC;
	}

	if ((Instance->STA & SDMMC_STA_DTIMEOUT) != 0U) {
		icr_clear_flag |= SDMMC_ICR_DTIMEOUTC;
	}

	if ((Instance->STA & SDMMC_STA_TXUNDERR) != 0U) {
		icr_clear_flag |= SDMMC_ICR_TXUNDERRC;
	}

	if ((Instance->STA & SDMMC_STA_RXOVERR) != 0U) {
