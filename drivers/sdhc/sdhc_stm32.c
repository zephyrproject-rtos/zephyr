				  ...
				  struct sdhc_data *data)
{
	...
	if (is_block_mode) {
		...
		config->hsd->block_size = 0;
	}

	dev_data->total_transfer_bytes = data->blocks * data->block_size;
#ifndef CONFIG_SDHC_STM32_POLLING_SUPPORT
	dev_data->sdio_dma_buf =
		k_aligned_alloc(CONFIG_SDHC_BUFFER_ALIGNMENT, data->blocks * data->block_size);
	...
	if (direction == SDIO_IO_WRITE) {
		if (data->data) {
#ifdef CONFIG_SDHC_STM32_POLLING_SUPPORT
			...
			arch_dcache_flush_range((uint32_t *)dev_data->sdio_dma_buf,
						dev_data->total_transfer_bytes);
...
static void sdhc_stm32_init_props(const struct device *dev)
{
	...
	props->host_caps.sdr50_support = false;
	props->host_caps.sdio_async_interrupt_support = false;
	props->is_spi = false;
}
...

{
	...
	if (__HAL_SDIO_GET_FLAG(config->hsd, SDMMC_FLAG_DATAEND)) {
		arch_dcache_invd_range((uint32_t *)data->sdio_dma_buf, data->total_transfer_bytes);
	}

	icr_clear_flag |= (SDMMC1->STA & SDMMC_STA_DCRCFAIL) ? SDMMC_ICR_DCRCFAILC : 0;
	icr_clear_flag |= (SDMMC1->STA & SDMMC_STA_DTIMEOUT) ? SDMMC_ICR_DTIMEOUTC : 0;
	icr_clear_flag |= (SDMMC1->STA & SDMMC_STA_TXUNDERR) ? SDMMC_ICR_TXUNDERRC : 0;
	icr_clear_flag |= (SDMMC1->STA & SDMMC_STA_RXOVERR) ? SDMMC_ICR_RXOVERRC : 0;
	if (icr_clear_flag) {
		LOG_ERR("SDMMC interrupt err flag raised: 0x%08X\n", icr_clear_flag);
		SDMMC1->ICR = icr_clear_flag;
	}
...
#define STM32_SDHC_IRQ_HANDLER_FUNCTION(index)                                                     \
