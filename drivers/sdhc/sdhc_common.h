...
/**
 * @file sdhc_common.h
 * @brief Common utilities shared by SDHC vendor drivers.
 *
 * This header provides shared configuration structures and helpers that
 * vendor-specific SDHC drivers are expected to use for consistent behavior
 * across the subsystem.  The set of utilities offered here will grow over
 * time; drivers should prefer the helpers in this file over re-implementing
 * equivalent logic locally.
 */
...
struct sdhc_common_config {
	uint32_t max_current_330;       /**< Max current (mA) at 3.3V */
	uint32_t max_current_300;       /**< Max current (mA) at 3.0V */
	uint32_t max_current_180;       /**< Max current (mA) at 1.8V */
	uint32_t min_bus_freq;          /**< Min bus frequency (Hz) */
	uint32_t max_bus_freq;          /**< Max bus frequency (Hz) */
...
/**
 * @brief Initialize host properties from DT-derived configuration.
 *
 * Populates @p props with values sourced from @p cfg, which is generated
 * from the device tree node of the host controller. Typically called once
 * during driver initialization before passing @p props to the SDHC subsystem.
 *
 * @param props Output structure to be populated.
 * @param cfg   DT-derived configuration for the host controller.
 */
