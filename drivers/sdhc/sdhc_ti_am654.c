...
struct ti_am654_adma2_descriptor_table {
	bool valid: 1;
	bool end: 1;
	bool interrupt: 1;
	uint8_t action: 3;
...
struct ti_am654_data {
	...
	struct ti_am654_adma2_descriptor_table descs[MAX(CONFIG_SDHC_TI_AM654_ADMA_DESC_LEN, 1)];
					    ...
					    bool is_write)
{
	...
	uint32_t length_left = dat->block_size * dat->blocks;
	int i = 0;
	...
	while (length_left > TI_AM654_ADMA2_DESC_LENGTH_MAX) {
		...
		data->descs[i].action = TI_AM654_ADMA2_DESC_ACTION_TRAN;
		data->descs[i].length_hi =
			FIELD_GET(GENMASK(25, 16), TI_AM654_ADMA2_DESC_LENGTH_MAX);
		data->descs[i].length_lo =
			FIELD_GET(GENMASK(15, 0), TI_AM654_ADMA2_DESC_LENGTH_MAX);
		data->descs[i].addr_lo = FIELD_GET(GENMASK64(31, 0), address);
		data->descs[i].addr_hi = FIELD_GET(GENMASK64(63, 32), address);
	...
	data->descs[i].interrupt = false;
	data->descs[i].action = TI_AM654_ADMA2_DESC_ACTION_TRAN;
	data->descs[i].length_hi = FIELD_GET(GENMASK(25, 16), length_left);
	data->descs[i].length_lo = FIELD_GET(GENMASK(15, 0), length_left);
	data->descs[i].addr_lo = FIELD_GET(GENMASK64(31, 0), address);
	data->descs[i].addr_hi = FIELD_GET(GENMASK64(63, 32), address);

	sys_cache_data_flush_range(data->descs, sizeof(data->descs[0]) * (i + 1));
	sys_cache_data_flush_and_invd_range(dat->data, dat->blocks * dat->block_size);
