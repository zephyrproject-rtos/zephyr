...
static int sdhc_spi_rx(const struct device *spi_dev, struct spi_config *spi_cfg, uint8_t *buf,
		       int len)
{
#if ANY_INST_REQUIRES_EXPLICIT_FF
	struct spi_buf tx_bufs[] = {{.buf = (uint8_t *)sdhc_ones, .len = len}};
...
static int sdhc_spi_wait_unbusy(const struct device *dev, int timeout_ms, int interval_ticks)
{
	const struct sdhc_spi_config *config = dev->config;
	struct sdhc_spi_data *data = dev->data;
	int ret;
...
static int sdhc_spi_response_get(const struct device *dev, struct sdhc_command *cmd, int rx_len)
{
	const struct sdhc_spi_config *config = dev->config;
	...
	if (response == end) {
		...
		while (timeout > 0) {
			ret = sdhc_spi_rx(config->spi_dev, dev_data->spi_cfg, response, 1);
			if (ret < 0) {
				return ret;
			}
			if (*response != 0xff) {
	...
	case SD_SPI_RSP_TYPE_R1b:
		...
		if (response == end) {
			...
			ret = sdhc_spi_wait_unbusy(dev, SPI_R1B_TIMEOUT_MS, 1000);
		}
		break;
...
static int sdhc_skip(const struct device *dev, uint8_t skip_val)
{
	...
	do {
		ret = sdhc_spi_rx(config->spi_dev, data->spi_cfg, &buf, sizeof(buf));
		if (ret) {
			return ret;
		}
...
static int sdhc_spi_read_data(const struct device *dev, struct sdhc_data *data)
{
	...
	while (remaining--) {
		ret = spi_transceive(config->spi_dev, dev_data->spi_cfg, tx_ptr, &rx);
		if (ret) {
			LOG_ERR("Data write failed");
			return ret;
		}
		/* Read CRC16 plus one end byte */
		ret = sdhc_spi_rx(config->spi_dev, dev_data->spi_cfg, crc, sizeof(crc));
		if (crc16_itu_t(0, read_location, data->block_size) != sys_get_be16(crc)) {
			/* Bad CRC */
...
static int sdhc_spi_write_data(const struct device *dev, struct sdhc_data *data)
{
	...
	while (remaining--) {
		...
		sys_put_be16(crc16_itu_t(0, write_location, data->block_size), crc);
		ret = spi_write(config->spi_dev, dev_data->spi_cfg, &tx);
		if (ret) {
			return ret;
		}
		/* Read back the data response token from the card */
		ret = sdhc_spi_rx(config->spi_dev, dev_data->spi_cfg, &resp, sizeof(resp));
		if (ret) {
			return ret;
		}
...
static int sdhc_spi_request(const struct device *dev, struct sdhc_command *cmd,
	...
	if (data == NULL) {
		...
		do {
			...
			if (ret || (cmd->opcode == SD_READ_MULTIPLE_BLOCK)) {
				 ...
				 */
				stop_ret = sdhc_spi_send_cmd(dev, (struct sdhc_command *)&stop_cmd,
							     false);
				while ((stop_ret != 0) && (stop_retries > 0)) {
					/* Retry stop command */
					ret = stop_ret = sdhc_spi_send_cmd(
						dev, (struct sdhc_command *)&stop_cmd, false);
					stop_retries--;
				}
			}
		} while ((ret != 0) && (retries > 0));
	...
	if (ios->clock != data->spi_cfg->frequency) {
		...
		if (ios->clock != 0) {
			if (data->spi_cfg == &data->cfg_a) {
				...
				memcpy(&data->cfg_b, &data->cfg_a, sizeof(struct spi_config));
				data->spi_cfg = &data->cfg_b;
			} else {
				data->cfg_b.frequency = ios->clock;
				memcpy(&data->cfg_a, &data->cfg_b, sizeof(struct spi_config));
