...
void sdhc_emul_core_build_ext_csd(struct sdhc_emul_card *card)
{
	...
	card->ext_csd[MMC_EXT_CSD_SEC_COUNT + 0] = card->n_blocks & 0xFF;
	card->ext_csd[MMC_EXT_CSD_SEC_COUNT + 1] = (card->n_blocks >> 8) & 0xFF;
	card->ext_csd[MMC_EXT_CSD_SEC_COUNT + 2] = (card->n_blocks >> 16) & 0xFF;
	card->ext_csd[MMC_EXT_CSD_SEC_COUNT + 3] = (card->n_blocks >> 24) & 0xFF;
	card->ext_csd[MMC_EXT_CSD_REV] = MMC_5_1;
	card->ext_csd[MMC_EXT_CSD_STRUCTURE] = 2;
	card->ext_csd[MMC_EXT_CSD_CARD_TYPE] = (1 << 0) | (1 << 1) | (1 << 4) | (1 << 5);
}
...
static void sdhc_emul_cmd0(struct sdhc_emul_card *card)
{
	...
	card->ext_csd[MMC_EXT_CSD_BUS_WIDTH] = 0;
	card->ext_csd[MMC_EXT_CSD_HS_TIMING] = 0;
}
		       ...
		       struct sdhc_data *data)
{
	...
	if (cmd->opcode == SD_APP_SEND_OP_COND) {
		cmd->response[0] = card->ocr | SD_OCR_PWR_BUSY_FLAG;
		card->state = SDHC_STATE_READY;
		return 0;
	}
	if (cmd->opcode == SD_APP_SEND_SCR) {
		...
		scr[0] = 0x02;
		scr[1] = 0x25;
		cmd->response[0] = (card->state << SD_R1_CURRENT_STATE_OFFSET);
		data->bytes_xfered = 8;
		return 0;
	}
	return -ENOTSUP;
				    ...
				    struct sdhc_command *cmd)
{
	if (card->type == SDHC_EMUL_TYPE_SDIO) {
		cmd->response[0] = (card->sdio_fn_count << 28) | SD_OCR_PWR_BUSY_FLAG;
	} else if (card->type == SDHC_EMUL_TYPE_EMMC) {
		if (cmd->arg & 0x8000) {
			card->state = SDHC_STATE_SLEEP;
		} else {
			card->state = SDHC_STATE_STANDBY;
		}
	} else {
			    ...
			    struct sdhc_command *cmd)
{
	...
	if (index == MMC_EXT_CSD_HS_TIMING && value == 0x03 && !card->hs200_mode) {
		return -EINVAL;
	}
...
{
	if (card->state == SDHC_STATE_STANDBY) {
		card->state = SDHC_STATE_TRANSFER;
	} else if (card->state == SDHC_STATE_TRANSFER) {
		card->state = SDHC_STATE_STANDBY;
	}
	return 0;
}
				...
				struct sdhc_data *data)
{
	...
	if (card->state != SDHC_STATE_TRANSFER) {
		return -EINVAL;
	}
			   ...
			   struct sdhc_data *data)
{
	...
	case SD_ALL_SEND_CID:
		card->state = SDHC_STATE_IDENT;
		memcpy(cmd->response, card->cid, 16);
		break;
	case SD_SEND_RELATIVE_ADDR:
		card->rca = 0x1234;
		cmd->response[0] = (card->rca << SD_R6_RCA_OFFSET) |
				 (card->state << SD_R1_CURRENT_STATE_OFFSET);
		card->state = SDHC_STATE_STANDBY;
		break;
