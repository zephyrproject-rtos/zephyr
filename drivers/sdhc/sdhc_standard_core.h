...
/**
 * @file sdhc_standard_core.h
 * @brief Standard SD Host Controller core for specification-compliant drivers.
 *
 * This header provides the register map, bit definitions, shared
 * config/data structures, and initialization helpers for host controllers that
 * conform to the SD Host Controller Simplified Specification.
 *
 * Vendor drivers built on top of this core are expected to use the structures
 * and helpers defined here rather than duplicating equivalent logic.  The API
 * surface will grow as additional common functionality is extracted from vendor
 * drivers; always check this header before adding new controller-level code to
 * a vendor driver.
 */
...
/**
 * @brief Initialize the SDHC standard capabilities.
 *
 * Reads and parses the hardware capability registers of the standard
 * host controller and populates the capability fields for the device.
 *
 * @param dev Pointer to the host controller device.
 */
