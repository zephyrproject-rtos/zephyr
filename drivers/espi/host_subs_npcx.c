...
#if defined(CONFIG_ESPI_NPCX_i8042_KBC_AUX_VWIRE_IRQ_WORKAROUND)
								...
								uint32_t *data)
{
	if (op >= E8042_START_OPCODE && op <= E8042_MAX_OPCODE) {
...
#ifndef CONFIG_ESPI_NPCX_i8042_KBC_AUX_VWIRE_IRQ_WORKAROUND
		...
		case E8042_OBF_HAS_CHAR:
			 ...
			 * the data
			 */
#ifdef CONFIG_ESPI_NPCX_i8042_KBC_AUX_VWIRE_IRQ_WORKAROUND
			...
			*data = (IS_BIT_SET(inst_kbc->HIKMST, NPCX_HIKMST_OBF) ||
				 IS_BIT_SET(inst_espi->STATUS_IMG, NPCX_STATUS_IMG_VWIRE_AVAIL));
#else
			*data = IS_BIT_SET(inst_kbc->HIKMST, NPCX_HIKMST_OBF);
#endif
			break;
