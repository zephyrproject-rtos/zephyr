...
/* Busy-wait for a VWire write. It should be within nanoseconds,
 * so this only guards a stuck bus and does not need tuning.
 */
#define ESPI_NXP_VWIRE_DONE_TIMEOUT_US 1000U

/* KBC IRQ numbers for IRQPUSH */
#define KBC_IRQ_KEYBOARD 1U
#define KBC_IRQ_MOUSE    12U

/* ESPICAP.SPICAP field encodings: supported SPI I/O mode. */
#define ESPI_NXP_SPICAP_SINGLE 0U
#define ESPI_NXP_SPICAP_DUAL   1U
#define ESPI_NXP_SPICAP_QUAD   3U

/* ESPICAP.MAXSPD field encodings: maximum supported eSPI frequency. */
#define ESPI_NXP_MAXSPD_20MHZ 0U
#define ESPI_NXP_MAXSPD_25MHZ 1U
#define ESPI_NXP_MAXSPD_33MHZ 2U
#define ESPI_NXP_MAXSPD_50MHZ 3U
#define ESPI_NXP_MAXSPD_66MHZ 4U

/* Convert a byte-size devicetree property into its ESPICAP field encoding.
 *   OOBMX/MEMMX: 64B=1, 128B=2, 256B=3            -> LOG2(bytes) - 5
 *   TRGT_REQ:    64B=1 ... 4096B=7                -> LOG2(bytes) - 5
 *   FLASHMX:     64B=0, 128B=1, 256B=2, 512B=3    -> LOG2(bytes) - 6
 *   TAFERA:      2KB=0, 4KB=1, 8KB=2, 16KB=3      -> LOG2(kb) - 1
 */
#define ESPI_NXP_OOBMX_ENC(bytes)   (LOG2(bytes) - 5)
#define ESPI_NXP_MEMMX_ENC(bytes)   (LOG2(bytes) - 5)
#define ESPI_NXP_REQMX_ENC(bytes)   (LOG2(bytes) - 5)
#define ESPI_NXP_FLASHMX_ENC(bytes) (LOG2(bytes) - 6)
#define ESPI_NXP_TAFERA_ENC(kb)     (LOG2(kb) - 1)

/* Per-port RAM buffer size encoding: 4B=0, 8B=1 ... 512B=7. */
#define ESPI_NXP_RAMSIZE_ENC(bytes) (LOG2(bytes) - 2)
...
struct espi_nxp_config {
	...
	uint8_t port_count;
	uint8_t max_oob_payload;
	uint8_t max_flash_payload;
	uint8_t max_peripheral_payload;
	uint8_t max_taf_read_req;
	uint8_t taf_erase_size;
};
...
struct espi_nxp_data {
	...
	struct k_sem oob_rx_ready;
#endif
#ifdef CONFIG_ESPI_TAF_NXP
	struct espi_nxp_taf_req taf_req;
#endif
};
...
static int espi_nxp_configure(const struct device *dev, struct espi_cfg *cfg)
{
	const struct espi_nxp_config *config = dev->config;
	ESPI_Type *base = config->base;
	uint32_t cap = base->ESPICAP;

	if (cfg == NULL) {
		return -EINVAL;
	}

	cap &= ~ESPI_ESPICAP_SPICAP_MASK;
	if (cfg->io_caps & ESPI_IO_MODE_QUAD_LINES) {
		cap |= ESPI_ESPICAP_SPICAP(ESPI_NXP_SPICAP_QUAD);
	} else if (cfg->io_caps & ESPI_IO_MODE_DUAL_LINES) {
		cap |= ESPI_ESPICAP_SPICAP(ESPI_NXP_SPICAP_DUAL);
	} else {
		/* Zero means single mode. */
	}

	cap &= ~ESPI_ESPICAP_MAXSPD_MASK;
	if (cfg->max_freq >= 66) {
		cap |= ESPI_ESPICAP_MAXSPD(ESPI_NXP_MAXSPD_66MHZ);
	} else if (cfg->max_freq >= 50) {
		cap |= ESPI_ESPICAP_MAXSPD(ESPI_NXP_MAXSPD_50MHZ);
	} else if (cfg->max_freq >= 33) {
		cap |= ESPI_ESPICAP_MAXSPD(ESPI_NXP_MAXSPD_33MHZ);
	} else if (cfg->max_freq >= 25) {
		cap |= ESPI_ESPICAP_MAXSPD(ESPI_NXP_MAXSPD_25MHZ);
	} else {
		/* Zero means 20 MHz. */
	}

	cap &= ~(ESPI_ESPICAP_OOBOK_MASK | ESPI_ESPICAP_SAF_MASK);
	if (cfg->channel_caps & ESPI_CHANNEL_OOB) {
		cap |= ESPI_ESPICAP_OOBOK_MASK;
	}
#ifdef CONFIG_ESPI_TAF_NXP
	if (cfg->channel_caps & ESPI_CHANNEL_FLASH) {
		cap |= ESPI_ESPICAP_SAF_MASK;
	}
#endif
	cap |= ESPI_ESPICAP_ALPIN_MASK;
	base->ESPICAP = cap;
...
{
	const struct espi_nxp_config *config = dev->config;
	uint32_t cfg = config->base->ESPICFG;

	switch (ch) {
	case ESPI_CHANNEL_PERIPHERAL:
		return (cfg & ESPI_ESPICFG_MEMENA_MASK) != 0U;
	case ESPI_CHANNEL_VWIRE:
		return (cfg & ESPI_ESPICFG_VWOK_MASK) != 0U;
	case ESPI_CHANNEL_OOB:
		return (cfg & ESPI_ESPICFG_OOBOK_MASK) != 0U;
	case ESPI_CHANNEL_FLASH:
		return (cfg & ESPI_ESPICFG_FLSHOK_MASK) != 0U;
	default:
		return false;
	}
}
				...
				enum espi_vwire_signal signal, uint8_t level)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(wirewo_map); i++) {
		if (wirewo_map[i].signal == signal) {
			...
			/* Reject if a previous VWire is still in flight. */
			if ((config->base->WIREWO & ESPI_WIREWO_DONE_MASK) == 0U) {
				irq_unlock(key);
				return -EBUSY;
			}
			...
			/* Confirm VWire message delivery before returning. */
			if (!WAIT_FOR((config->base->WIREWO & ESPI_WIREWO_DONE_MASK) != 0U,
				      ESPI_NXP_VWIRE_DONE_TIMEOUT_US, k_busy_wait(1))) {
				ret = -ETIMEDOUT;
			}
			      ...
			      struct espi_oob_packet *pckt)
{
	...
	if ((drv->oob_port == PORT_NONE) || (pckt == NULL)) {
		return -EINVAL;
	}

	if ((pckt->len == 0U) || (pckt->buf == NULL)) {
		return -EINVAL;
	}

	status_t status = ESPI_SendOOB(base, &drv->handle,
				 ...
				 struct espi_oob_packet *pckt)
{
	...
	ret = k_sem_take(&drv->oob_rx_ready, K_MSEC(CONFIG_ESPI_NXP_TIMEOUT_MS));
	if (ret == -EAGAIN) {
		LOG_ERR("OOB receive timeout");
		return -ETIMEDOUT;
	}

	len = ESPI_GetPortMsgLen(base, drv->oob_port);
	if (len > pckt->len) {
		LOG_ERR("OOB rx buffer too small: %u > %u, message dropped",
			len, pckt->len);
		espi_nxp_set_sstcl(base, drv->oob_port, kESPI_SSTCL_OOBRecvEmpty);
		return -EMSGSIZE;
	}

	memcpy(pckt->buf, ESPI_GetPortRamBuffer(base, drv->oob_port), len);
	pckt->len = len;
	espi_nxp_set_sstcl(base, drv->oob_port, kESPI_SSTCL_OOBRecvEmpty);
			       ...
			       espi_flash_request_t *req, void *userData)
{
	...
	if (req->length == 0U && req->readStart) {
		LOG_INF("First request is zero length, ignoring");
		return;
	}

	data->taf_req.addr = req->addr;
	data->taf_req.len = req->length;
	data->taf_req.port = handle->safPort;
	data->taf_req.tag = req->tag;
	data->taf_req.type = (uint8_t)req->type;
	...
	espi_nxp_fire_callbacks(dev, (struct espi_event){
		...
		.evt_data    = (uint32_t)(uintptr_t)&data->taf_req,
	});
}

#endif /* CONFIG_ESPI_TAF_NXP */
			       ...
			       void *userData)
{
	...
	if (status & kESPI_BusResetFlag) {
		...
		data->wirero_prev = 0U;
		/* kESPI_InResetFlag
		 * 0 - Not in reset(RESET pin is high),
		 * 1 - In reset(RESET pin is low).
		 */
	...
	if (status & kESPI_Port80InterruptFlag) {
		...
		/* If port80 is not enabled, code should not enter here. Ignore the return value. */
		(void)ESPI_GetPort80Status(base, &p80);
		espi_nxp_fire_callbacks(dev, (struct espi_event){
			     ...
			     uint32_t port, uint32_t status, void *userData)
{
	...
	if (status & ESPI_STAT_INTERR_MASK) {
		...
		LOG_ERR("eSPI port %u error: %d (status=0x%08x)", port, err,
			status);
	}
	...
	if ((int8_t)port == data->acpi_port) {
		uint32_t idx, pdata, evt_data;

		ESPI_GetEndpointData(base, port, &idx, &pdata);
		/* ACPI-style Endpoint. A command write raises the port INTSPC0
		 * interrupt, while a data write raises the write interrupt.
		 */
		struct espi_evt_data_acpi acpi_evt = {
			.type = (status & ESPI_STAT_INTSPC0_MASK) ?
				ESPI_EVENT_DATA_ACPI_TYPE_HOST_TO_EC_CMD :
				ESPI_EVENT_DATA_ACPI_TYPE_HOST_TO_EC_DATA,
			.data = pdata & 0xFFU,
		};

		memcpy(&evt_data, &acpi_evt, sizeof(evt_data));
		espi_nxp_fire_callbacks(dev, (struct espi_event){
			.evt_type    = ESPI_BUS_PERIPHERAL_NOTIFICATION,
...
{
	const struct espi_nxp_config *config = dev->config;
	espi_port_config_t espi_ports_cfg[ESPI_PORT_COUNT] = {0};
	struct espi_nxp_data *data = dev->data;
	ESPI_Type *base = config->base;
...
#endif
#ifdef CONFIG_ESPI_OOB_CHANNEL
	k_sem_init(&data->oob_rx_ready, 0, 1);
#endif

	if (!device_is_ready(config->clock_dev)) {
		LOG_ERR("eSPI clock controller is not ready");
		return -ENODEV;
	}
	...
	for (uint8_t i = 0; i < config->port_count; i++) {
		...
		espi_ports_cfg[i] = (espi_port_config_t){
			.type       = p->type,
	...
	espi_cfg.portConfig     = espi_ports_cfg;
	...
	espi_cfg.enableSAF      = IS_ENABLED(CONFIG_ESPI_TAF_NXP) &&
				  (data->flash_port != PORT_NONE);
	espi_cfg.maxOOBPayloadSize   = config->max_oob_payload;
	espi_cfg.maxFlashPayloadSize = config->max_flash_payload;
	espi_cfg.maxPayloadSize      = config->max_peripheral_payload;
	espi_cfg.maxSAFRxReqSize     = config->max_taf_read_req;
	espi_cfg.SAFEraseSize        = config->taf_erase_size;
...
#ifdef CONFIG_ESPI_TAF_NXP
	/* Parent owns the HAL flash handle; each TAF child driver registers its
	 * own notification callback and derives base from its DT config.
	 */
	ESPI_FlashCreateHandle(base, &data->handle, espi_nxp_flash_ops, UINT32_MAX);
#endif
...
#define ESPI_NXP_INIT(n) \
	...
	static const struct espi_nxp_port_cfg \
		espi_nxp_ports_##n[] = { \
		DT_INST_FOREACH_CHILD_STATUS_OKAY(n, ESPI_NXP_PORT_CFG) \
	}; \
	\
	static const struct espi_nxp_config espi_nxp_config_##n = { \
		...
		.max_oob_payload = ESPI_NXP_OOBMX_ENC( \
			DT_INST_PROP(n, max_oob_payload_size)), \
		.max_flash_payload = ESPI_NXP_FLASHMX_ENC( \
			DT_INST_PROP(n, max_flash_payload_size)), \
		.max_peripheral_payload = ESPI_NXP_MEMMX_ENC( \
			DT_INST_PROP(n, max_peripheral_payload_size)), \
		.max_taf_read_req = ESPI_NXP_REQMX_ENC( \
			DT_INST_PROP(n, max_taf_read_req_size)), \
		.taf_erase_size = ESPI_NXP_TAFERA_ENC( \
			DT_INST_PROP(n, taf_erase_size)), \
	}; \
	\
