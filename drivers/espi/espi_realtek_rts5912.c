...
static int espi_send_vw_event(uint8_t index, uint8_t data, const struct device *dev)
{
	...
	if (!WAIT_FOR(!(espi_reg->EVSTS & ESPI_EVSTS_TXFULL), ESPI_VW_EVENT_TIMEOUT_CNT,
		      k_busy_wait(1))) {
		return -EBUSY;
	}
	...
	if (!WAIT_FOR(!(espi_reg->EVSTS & ESPI_EVSTS_TXFULL), ESPI_VW_EVENT_RETRY_CNT, NULL)) {
		return -EBUSY;
	}
