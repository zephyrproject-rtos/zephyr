...
static bool vw_set_irq1(uint8_t level, const struct espi_rts5912_config *const espi_config);
...
static void kbc_obe_isr(const struct device *dev)
{
	...
	if (prev_irq1 == 1) {
		if (!vw_set_irq1(0x0, espi_config)) {
			prev_irq1 = 0;
		}
	}

	if ((kbc_reg->STS & KBC_STS_OBF_Msk)) {
			 ...
			 const struct espi_rts5912_config *const espi_config)
{
	...
	if (kbc_int_en) {  //Check IRQ1 Interupt En or not
		kbdata_with_irq1(exData, espi_config);   //Gen IRQ1-Level High to VW ch
	} else {
		kbc_reg->OB = exData;
	}
...
static int espi_kbc_setup(const struct device *dev)
{
	...
	kbc_reg->INTEN = (KBC_INTEN_IBFINTEN_Msk | KBC_INTEN_OBFINTEN_Msk);
