			     ...
			     struct espi_oob_packet *pckt)
{
	...
	if (regs->OOBTXSTS & err_mask) {
		LOG_ERROR("Tx failed %x", regs->OOBTXSTS);
...
static void espi_pc_isr(const struct device *dev)
{
	...
	if (status & MCHP_ESPI_PC_STS_BUS_ERR) {
		LOG_ERROR("%s bus error", __func__);
...
static int espi_xec_init(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERROR("XEC eSPI V2 pinctrl setup failed (%d)", ret);
		return ret;
	}
	...
	if (ret) {
		LOG_ERROR("XEX eSPI V2 register VW handlers error %d", ret);
		return ret;
	}
