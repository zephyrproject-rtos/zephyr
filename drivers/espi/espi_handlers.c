...
static inline int z_vrfy_espi_config(const struct device *dev, struct espi_cfg *cfg)
{
	struct espi_cfg cfg_copy;

	K_OOPS(K_SYSCALL_DRIVER_ESPI(dev, config));
	K_OOPS(k_usermode_from_copy(&cfg_copy, cfg, sizeof(struct espi_cfg)));

	return z_impl_espi_config(dev, &cfg_copy);
}
#include <zephyr/syscalls/espi_config_mrsh.c>

static inline bool z_vrfy_espi_get_channel_status(const struct device *dev, enum espi_channel ch)
{
...
static inline int z_vrfy_espi_send_vwire(const struct device *dev, enum espi_vwire_signal signal,
					 uint8_t level)
{
...
static inline int z_vrfy_espi_receive_vwire(const struct device *dev, enum espi_vwire_signal signal,
					    uint8_t *level)
{
	int ret;
	uint8_t level_copy;
					   ...
					   struct espi_request_packet *req)
{
	...
	K_OOPS(k_usermode_from_copy(&req_copy, req, sizeof(struct espi_request_packet)));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(req_copy.data, req_copy.len));

	ret = z_impl_espi_read_request(dev, &req_copy);

	K_OOPS(k_usermode_to_copy(req, &req_copy, sizeof(struct espi_request_packet)));
...
static inline int z_vrfy_espi_send_oob(const struct device *dev, struct espi_oob_packet *pckt)
{
	int ret;
	...
	K_OOPS(k_usermode_from_copy(&pckt_copy, pckt, sizeof(struct espi_oob_packet)));
...
static inline int z_vrfy_espi_receive_oob(const struct device *dev, struct espi_oob_packet *pckt)
{
	int ret;
	struct espi_oob_packet pckt_copy;
	...
	K_OOPS(k_usermode_to_copy(pckt, &pckt_copy, sizeof(struct espi_oob_packet)));

	return ret;
}
#include <zephyr/syscalls/espi_receive_oob_mrsh.c>

static inline int z_vrfy_espi_read_flash(const struct device *dev, struct espi_flash_packet *pckt)
{
	int ret;
	struct espi_flash_packet pckt_copy;

	K_OOPS(K_SYSCALL_DRIVER_ESPI(dev, flash_read));
	K_OOPS(k_usermode_from_copy(&pckt_copy, pckt, sizeof(struct espi_flash_packet)));
	K_OOPS(K_SYSCALL_MEMORY_WRITE(pckt->buf, pckt->len));

	ret = z_impl_espi_read_flash(dev, pckt);
	K_OOPS(k_usermode_to_copy(pckt, &pckt_copy, sizeof(struct espi_flash_packet)));

	return ret;
}
#include <zephyr/syscalls/espi_read_flash_mrsh.c>

static inline int z_vrfy_espi_write_flash(const struct device *dev, struct espi_flash_packet *pckt)
{
	int ret;
	struct espi_flash_packet pckt_copy;
...
static inline int z_vrfy_espi_flash_erase(const struct device *dev, struct espi_flash_packet *pckt)
{
	int ret;
	struct espi_flash_packet pckt_copy;
