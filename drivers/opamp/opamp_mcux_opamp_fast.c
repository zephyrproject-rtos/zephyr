...
static int mcux_opamp_fast_set_gain(const struct device *dev, const struct opamp_gain_cfg *cfg)
{
	const struct mcux_opamp_fast_config *config = dev->config;
	OPAMP_Type *base = config->base;

	if (cfg->inverting_gain <= 2) {
		OPAMP_SetCompensationCapcitor(base, kOPAMP_FitGain2x);
	} else if (cfg->inverting_gain <= 4) {
		OPAMP_SetCompensationCapcitor(base, kOPAMP_FitGain4x);
	} else if (cfg->inverting_gain <= 8) {
		OPAMP_SetCompensationCapcitor(base, kOPAMP_FitGain8x);
	} else {
		OPAMP_SetCompensationCapcitor(base, kOPAMP_FitGain16x);
	}
...
#define OPAMP_MCUX_OPAMP_FAST_DEFINE(inst)							\
	static const struct mcux_opamp_fast_config _CONCAT(config, inst) = {			\
		...
		.is_reference_source = DT_INST_PROP(inst, is_reference_source),			\
												...
												\
	DEVICE_DT_INST_DEFINE(inst, mcux_opamp_fast_init, NULL, NULL, &_CONCAT(config, inst),	\
