...
/**
 * Waits for the IRQ_OUT pin to go low.
 *
 ...
 * the wait.
 *
 * irq_out: Pointer to the GPIO specification for the IRQ_OUT pin.
 * data: Pointer to the RFID data structure containing the semaphore.
 */
					    ...
					    struct rfid_cr95hf_data *data, k_timeout_t timeout)
{
	...
	int pin_state = gpio_pin_get_dt(irq_out);
...
static inline void rfid_cr95hf_IRQ_IN_pulse(const struct gpio_dt_spec *irq_in)
{
	...
	k_sleep(CR95HF_IRQ_IN_PULSE_T0);
	gpio_pin_set_dt(irq_in, 1);
	k_sleep(CR95HF_IRQ_IN_PULSE_T1);
	gpio_pin_set_dt(irq_in, 0);
	k_sleep(CR95HF_IRQ_IN_PULSE_T3);
}

/**
 * Transceive data using SPI communication.
 *
...
{
	const struct rfid_cr95hf_config *config = dev->config;
	struct rfid_cr95hf_data *data = dev->data;
	int err;

	LOG_DBG("snd_buffer[1]: %02X len: %zu",
		(data->spi_snd_buffer.len > 1U) ? data->snd_buffer[1] : 0U,
		data->spi_snd_buffer.len);

	err = spi_transceive_dt(&config->spi, &data->spi_snd_buffer_arr, &data->spi_rcv_buffer_arr);
	if (err) {
		return err;
	}
...
/**
 * Initialize the SPI configuration for the RFID CR95HF.
 *
...
/**
 * Polls the CR95HF device for readiness to read or send data.
 *
 ...
 *
 * dev: The device structure representing the CR95HF device.
 * ready_read: Flag indicating if the function should check
 * for readiness to read data.
 * ready_send: Flag indicating if the function should check
...
/**
 *  Sets the device to tag detector mode
 *
...
static int rfid_cr95hf_load_protocol(const struct device *dev, rfid_proto_t proto, rfid_mode_t mode)
{
	...
	rfid_mode_t tx_mode = mode & RFID_MODE_TX_MASK;
	rfid_mode_t rx_mode = mode & RFID_MODE_RX_MASK;

	if (sys_count_bits(&tx_mode, 4) > 1U || sys_count_bits(&rx_mode, 4) > 1U) {
		return -EINVAL;
	}
				     ...
				     uint16_t *rx_len)
{
	...
	if (tx_len > (CR95HF_SND_BUF_SIZE - 4U)) {
		return -EMSGSIZE;
	}

	data->snd_buffer[0] = 0x00; /* Control Byte: Send */
	data->snd_buffer[1] = 0x04; /* cmd: SendRecev */
	data->snd_buffer[2] = (uint8_t)(tx_len + 1U);
	data->snd_buffer[tx_len + 3U] = tx_last_bits | (data->hw_tx_crc ? 0x20 : 0x00);
	memcpy(data->snd_buffer + 3, tx_data, tx_len);
	data->spi_snd_buffer.len = tx_len + 4U;
	data->spi_rcv_buffer.len = 0;
	...
	uint16_t out_len = data->spi_rcv_buffer.len;
	...
	memcpy(rx_data, data->rcv_buffer, out_len);
	*rx_len = out_len;
	return 0;
}
...
#define RFID_DEVICE_SPI(n)                                                                         \
	extern struct rfid_cr95hf_data rfid_device_prv_data_##n;                                   \
	...
	static const struct rfid_cr95hf_config rfid_device_prv_config_##n = {                      \
	...
	struct rfid_cr95hf_data rfid_device_prv_data_##n = {                                       \
...
#define RFID_DEVICE_UART(n) "CR95hf on UART is not yet implemented"

#define CR95HF_DEFINE(n)                                                                           \
	COND_CODE_1(DT_INST_ON_BUS(n, spi),	\
		    (RFID_DEVICE_SPI(n)),	\
		    ())                                                      \
	COND_CODE_1(DT_INST_ON_BUS(n, uart),	\
		    (RFID_DEVICE_UART(n)),	\
		    ())
