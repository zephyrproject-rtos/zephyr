...
static int pd_intel_adsp_set_power_enable(struct pg_bits *bits, bool power_enable)
{
	...
	if (power_enable) {
		...
		if (!WAIT_FOR(sys_read16((mem_addr_t)ACE_PWRSTS) & BIT(bits->CPA_bit), 10000,
			      k_busy_wait(1))) {
			return -EIO;
		}
	} else {
...
#define POWER_DOMAIN_DEVICE(id)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(id, pd_intel_adsp_init, PM_DEVICE_DT_INST_GET(id), &pd_pg_reg##id,   \
			      NULL, POST_KERNEL, CONFIG_POWER_DOMAIN_INTEL_ADSP_INIT_PRIORITY,     \
			      NULL);
