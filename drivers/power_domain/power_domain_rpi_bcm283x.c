...
static int rpi_bcm283x_power_on(const struct device *dev)
{
	...
	struct rpi_bcm283x_power_data *power = dev->data;
	int ret;
...
{
	__maybe_unused struct rpi_bcm283x_power_data *power = dev->data;

	LOG_DBG("PM action %d on domain 0x%08x", action, power->dev_id);
...
static int rpi_bcm283x_power_init(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Failed to initialize PM (%d)", ret);
		return ret;
	}
...
#define RPI_POWER_DOMAIN_INIT(n)                                                                   \
	static struct rpi_bcm283x_power_data rpi_bcm283x_power_data_##n = {                        \
		.dev_id = DT_INST_PROP(n, power_domain_id),                                        \
	};                                                                                         \
	                                                                                           \
	...
	PM_DEVICE_DT_INST_DEFINE(n, rpi_bcm283x_pm_action);                                        \
	                                                                                           \
	DEVICE_DT_INST_DEFINE(n, rpi_bcm283x_power_init, PM_DEVICE_DT_INST_GET(n),                 \
			      &rpi_bcm283x_power_data_##n, &rpi_bcm283x_power_config_##n,          \
			      POST_KERNEL, CONFIG_POWER_DOMAIN_RASPBERRYPI_BCM283X_INIT_PRIORITY,  \
			      NULL);
