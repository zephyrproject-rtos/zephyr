...
#define SEMA42_GATE_RESET_ALL (0x40U)
...
{
	const struct nxp_sema42_config *cfg = (const struct nxp_sema42_config *)dev->config;
	SEMA42_Type *base = (SEMA42_Type *)DEVICE_MMIO_GET(dev);
	...
	/* Reset all gates.
	 * First write: set RSTGDP = SEMA42_GATE_RESET_PATTERN_1
	 * Second write: set RSTGDP = SEMA42_GATE_RESET_PATTERN_2, and RSTGTN = 0x40
	 */
	base->RSTGT_W = SEMA42_RSTGT_W_RSTGDP(SEMA42_GATE_RESET_PATTERN_1);
	base->RSTGT_W = SEMA42_RSTGT_W_RSTGDP(SEMA42_GATE_RESET_PATTERN_2) |
					      SEMA42_RSTGT_W_RSTGTN(SEMA42_GATE_RESET_ALL);
