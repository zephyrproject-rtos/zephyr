...
static int eeprom_stm32_read(const struct device *dev, off_t offset, void *buf, size_t len)
{
...
static int eeprom_stm32_write(const struct device *dev, off_t offset, const void *buf, size_t len)
{
	const struct eeprom_stm32_config *config = dev->config;
