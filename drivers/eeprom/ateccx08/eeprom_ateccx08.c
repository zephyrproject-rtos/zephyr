...
static size_t eeprom_ateccx08_size(const struct device *dev)
{
	...
	return atecc_get_zone_size(at_data->std_zone, at_data->std_slot);
}
				 ...
				 size_t len)
{
	...
	return atecc_write_bytes(dev, at_data->std_zone, at_data->std_slot, offset, data, len);
}
...
static int eeprom_ateccx08_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	...
	return atecc_read_bytes(dev, at_data->std_zone, at_data->std_slot, offset, data, len);
}
