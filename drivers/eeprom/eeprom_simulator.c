...
#else
static u8_t mock_eeprom[DT_INST_REG_SIZE(0)];
...
static int eeprom_mock_init(struct device *dev)
{
	...
	if (ftruncate(eeprom_fd, DT_INST_REG_SIZE(0)) == -1) {
	...
	mock_eeprom = mmap(NULL, DT_INST_REG_SIZE(0),
...
static void eeprom_native_posix_cleanup(void)
{
	if ((mock_eeprom != MAP_FAILED) && (mock_eeprom != NULL)) {
		munmap(mock_eeprom, DT_INST_REG_SIZE(0));
	}
