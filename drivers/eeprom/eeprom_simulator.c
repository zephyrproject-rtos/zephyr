...
static int eeprom_range_is_valid(const struct device *dev, off_t offset, size_t len)
{
...
static int eeprom_sim_read(const struct device *dev, off_t offset, void *data, size_t len)
{
	if (!len) {
		return 0;
	}
	...
	STATS_INCN(eeprom_sim_stats, eeprom_read_time_us, CONFIG_EEPROM_SIMULATOR_MIN_READ_TIME_US);
#endif

	return 0;
}

static int eeprom_sim_write(const struct device *dev, off_t offset, const void *data, size_t len)
{
	const struct eeprom_sim_config *config = dev->config;
	...
	if (eeprom_sim_thresholds.max_write_calls != 0) {
		if (eeprom_sim_stats.eeprom_write_calls > eeprom_sim_thresholds.max_write_calls) {
			goto end;
...
static int eeprom_sim_init(const struct device *dev)
{
	...
	STATS_INIT_AND_REG(eeprom_sim_thresholds, STATS_SIZE_32, "eeprom_sim_thresholds");
