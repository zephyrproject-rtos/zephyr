		    ...
		    int32_t max, uint32_t step, int32_t def)
{
	...
	sys_slist_append(&vdev->ctrls, &ctrl->node);
...
static int video_find_ctrl(const struct device *dev, uint32_t id, struct video_ctrl **ctrl)
{
	...
	while (vdev) {
		SYS_SLIST_FOR_EACH_CONTAINER(&vdev->ctrls, *ctrl, node) {
			if ((*ctrl)->id == id) {
				return 0;
			}
		}
...
int video_set_ctrl(const struct device *dev, struct video_control *control)
{
	...
	if (DEVICE_API_GET(video, ctrl->dev)->set_ctrl == NULL) {
		return -ENOSYS;
	}

	/* Call driver's set_ctrl */
	ret = DEVICE_API_GET(video, ctrl->dev)->set_ctrl(ctrl->dev, control);
	if (ret) {
		return ret;
	}
