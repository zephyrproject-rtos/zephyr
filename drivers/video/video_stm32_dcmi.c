				       ...
				       enum video_buf_type type)
{
	...
	/* check for snapshot mode and if on simply bail */
				    ...
				    k_timeout_t timeout)
{
	...
	if (data->snapshot_mode) {
		/* first see if maybe we were already called and have one active */
		*vbuf = k_fifo_get(&data->fifo_out, K_NO_WAIT);
		if (*vbuf != NULL) {
			return 0;
		}

		/* next see if maybe we were already called and have one active */
		if (data->vbuf == NULL) {
			data->vbuf = k_fifo_get(&data->fifo_in, K_NO_WAIT);
			if (data->vbuf == NULL) {
				return -ENOMEM;
...
static inline int video_stm32_dcmi_get_snapshot_mode(const struct device *dev, bool *snapshot_mode)
{
	struct video_stm32_dcmi_data *data = dev->data;
...
static inline int video_stm32_dcmi_set_snapshot_mode(const struct device *dev, bool snapshot_mode)
{
	struct video_stm32_dcmi_data *data = dev->data;

	data->snapshot_mode = snapshot_mode;
	return 0;
}
...
static DEVICE_API(video, video_stm32_dcmi_driver_api) = {
	...
	.get_snapshot_mode = video_stm32_dcmi_get_snapshot_mode,
	.set_snapshot_mode = video_stm32_dcmi_set_snapshot_mode,
};
...
static int video_stm32_dcmi_init(const struct device *dev)
{
	...
	#if CONFIG_VIDEO_BUFFER_POOL_NUM_MAX == 1
	LOG_INF("Only one buffer so snapshot mode only");
