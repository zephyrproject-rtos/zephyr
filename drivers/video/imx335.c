...
static const struct imx335_reg imx335_fixed_regs[] = {
	{0x3288, 0x21},
	{0x328a, 0x02},
	{0x3414, 0x05},
	{0x3416, 0x18},
...
#define IMX335_INCK_74MHZ	0
#define IMX335_INCK_27MHZ	1
#define IMX335_INCK_24MHZ	2
#define IMX335_INCK_18MHZ	3
#define IMX335_INCK_6MHZ	4
static const struct imx335_inck imx335_inck_cfg[] = {
	{	/* 74MHz */
		.bcwait_time = 0xb6,
		.cpwait_time = 0x7f,
		.incksel1 = 0x80,
...
static int imx335_set_fmt(const struct device *dev, enum video_endpoint_id ep,
			  struct video_format *fmt)
{
	/*
...
static int imx335_get_fmt(const struct device *dev, enum video_endpoint_id ep,
			  struct video_format *fmt)
{
...
static int imx335_get_caps(const struct device *dev, enum video_endpoint_id ep,
...
static int imx335_set_stream(const struct device *dev, bool enable)
{
...
{
	const struct imx335_config *cfg = dev->config;
	const struct imx335_inck *regs;
	int ret;

	switch (rate) {
	case MHZ(74):
		regs = &imx335_inck_cfg[IMX335_INCK_74MHZ];
		break;
	case MHZ(27):
		regs = &imx335_inck_cfg[IMX335_INCK_27MHZ];
		break;
	case MHZ(24):
		regs = &imx335_inck_cfg[IMX335_INCK_24MHZ];
		break;
	case MHZ(18):
		regs = &imx335_inck_cfg[IMX335_INCK_18MHZ];
		break;
	case MHZ(6):
		regs = &imx335_inck_cfg[IMX335_INCK_6MHZ];
		break;
	default:
	...
	ret = video_write_cci_reg(&cfg->i2c, IMX335_BCWAIT_TIME, regs->bcwait_time);
	if (ret) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, IMX335_CPWAIT_TIME, regs->cpwait_time);
	if (ret) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, IMX335_INCKSEL1, regs->incksel1);
	if (ret) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, IMX335_INCKSEL2_PLL_IF_GC, regs->incksel2);
	if (ret) {
		return ret;
	}

	ret = video_write_cci_reg(&cfg->i2c, IMX335_INCKSEL3, regs->incksel3);
	if (ret) {
		return ret;
	}

	return video_write_cci_reg(&cfg->i2c, IMX335_INCKSEL4, regs->incksel4);
}
...
static int imx335_init(const struct device *dev)
{
	...
	for (i = 0; imx335_fixed_regs[i].addr != 0; i++) {
		ret = video_write_cci_reg(&cfg->i2c, IMX335_REG8(imx335_fixed_regs[i].addr),
					  imx335_fixed_regs[i].data);
		if (ret) {
			LOG_ERR("Unable to initialize the sensor");
			return ret;
		}
	...
	/* TODO - Allow support of other input clock */
