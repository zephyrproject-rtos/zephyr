...
#define MIPID02_CLK_LANE_REG1_UI_SHIFT	2
#define MIPID02_CLK_LANE_REG1_UI_MASK	0x3f
...
static const struct mipid02_format_desc *mipid02_get_format_desc(uint32_t pixelformat)
{
	...
	for (i = 0; i < ARRAY_SIZE(mipid02_supported_formats); i++) {
...
static int mipid02_set_fmt(const struct device *dev, enum video_endpoint_id ep,
			   struct video_format *fmt)
{
	...
	ret = video_set_format(cfg->source_dev, 0, &sensor_fmt);
	if (ret < 0) {
		return ret;
	}
...
static int mipid02_get_fmt(const struct device *dev, enum video_endpoint_id ep,
			   struct video_format *fmt)
{
	const struct mipid02_config *cfg = dev->config;
	struct mipid02_data *drv_data = dev->data;
	...
	if (!drv_data->desc) {
		ret = video_get_format(cfg->source_dev, 0, fmt);
		if (ret < 0) {
			return ret;
		}
...
static int mipid02_get_caps(const struct device *dev, enum video_endpoint_id ep,
			    struct video_caps *caps)
{
	...
	ret = video_get_caps(cfg->source_dev, 0, caps);
	if (ret < 0) {
		return ret;
	}
	...
	ind = 0;
	for (i = 0; caps->format_caps[i].pixelformat; i++) {
		const struct mipid02_format_desc *desc =
...
static int mipid02_set_stream(const struct device *dev, bool enable)
{
	const struct mipid02_config *cfg = dev->config;
	struct mipid02_data *drv_data = dev->data;
	...
	/* Get pixelrate information from source */
	ret = video_get_ctrl(cfg->source_dev, &ctrl);
	if (ret < 0) {
		return ret;
	}
...
static int mipid02_set_frmival(const struct device *dev, enum video_endpoint_id ep,
...
static int mipid02_get_frmival(const struct device *dev, enum video_endpoint_id ep,
			       struct video_frmival *frmival)
{
	const struct mipid02_config *cfg = dev->config;
...
static int mipid02_enum_frmival(const struct device *dev, enum video_endpoint_id ep,
