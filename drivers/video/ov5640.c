...
static const struct ov5640_reg mipi_low_res_params[] = {
...
static const struct ov5640_reg mipi_hd_res_params[] = {
...
static const struct ov5640_mode_config mipi_modes[] = {
	{
		.width = 640,
		.height = 480,
		.array_size_res_params = ARRAY_SIZE(mipi_low_res_params),
		.res_params = mipi_low_res_params,
	...
	},
	{
		.width = 1280,
		.height = 720,
		.array_size_res_params = ARRAY_SIZE(mipi_hd_res_params),
		.res_params = mipi_hd_res_params,
...
static const struct video_format_cap mipi_fmts[] = {
...
static inline bool ov5640_is_dvp(const struct device *dev)
{
	...
	return strcmp(cfg->bus_type, "parallel") == 0;
}
			  ...
			  struct video_format *fmt)
{
	...
	if (ov5640_is_dvp(dev)) {
		...
		fmts = mipi_fmts;
		modes = mipi_modes;
		num_fmts = ARRAY_SIZE(mipi_fmts);
		array_size_modes = ARRAY_SIZE(mipi_modes);
	}
...
{
	caps->format_caps = ov5640_is_dvp(dev) ? dvp_fmts : mipi_fmts;
	return 0;
}
...
static int ov5640_set_ctrl_test_pattern(const struct device *dev, uint8_t value)
{
	const struct ov5640_config *cfg = dev->config;
...
{
	const struct ov5640_config *cfg = dev->config;
	int ret = ov5640_modify_reg(&cfg->i2c, HZ5060_CTRL01_REG, BIT(7),
				    value == VIDEO_CID_POWER_LINE_FREQUENCY_AUTO ? 0 : BIT(7));

	if (ret) {
		return ret;
	}

	return ov5640_modify_reg(&cfg->i2c, HZ5060_CTRL00_REG, BIT(2),
				 value == VIDEO_CID_POWER_LINE_FREQUENCY_50HZ ? BIT(2) : 0);
}
			       ...
			       struct video_frmival_enum *fie)
{
	...
	for (i = 0; i < ARRAY_SIZE(mipi_modes); i++) {
		if (fie->width == mipi_modes[i].width && fie->height == mipi_modes[i].height) {
			break;
		}
	}

	if (i == ARRAY_SIZE(mipi_modes) || fie->index > ARRAY_SIZE(ov5640_frame_rates) ||
	    ov5640_frame_rates[fie->index] > mipi_modes[i].max_frmrate) {
		return -EINVAL;
	}
