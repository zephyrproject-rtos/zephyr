...
static int ov2640_set_brightness(const struct device *dev, int level)
{
	...
	ret = ov2640_set_level(dev, level, NUM_BRIGHTNESS_LEVELS, ARRAY_SIZE(brightness_regs[0]),
			       brightness_regs);
...
static int ov2640_set_saturation(const struct device *dev, int level)
{
	...
	ret = ov2640_set_level(dev, level, NUM_SATURATION_LEVELS, ARRAY_SIZE(saturation_regs[0]),
			       saturation_regs);
...
static int ov2640_set_contrast(const struct device *dev, int level)
{
	...
	ret = ov2640_set_level(dev, level, NUM_CONTRAST_LEVELS, ARRAY_SIZE(contrast_regs[0]),
			       contrast_regs);
...
static int ov2640_set_fmt(const struct device *dev, enum video_endpoint_id ep,
			  struct video_format *fmt)
{
...
static int ov2640_get_fmt(const struct device *dev, enum video_endpoint_id ep,
			  struct video_format *fmt)
{
	struct ov2640_data *drv_data = dev->data;
...
static int ov2640_get_caps(const struct device *dev, enum video_endpoint_id ep,
...
static int ov2640_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	int ret = 0;
