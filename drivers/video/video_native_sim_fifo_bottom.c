...
int video_nsi_fifo_read_bottom(int fd, uint8_t *buf, size_t frame_size, size_t *offset)
{
	while (*offset < frame_size) {
		...
		if (ret == 0) {
			/*
			 * All the writers closed the FIFO. Drop the partial frame so that
			 * the next writer to attach is guaranteed to be frame aligned. The
			 * file descriptor is kept open: a new writer can attach to it.
			 */
			bool partial = (*offset > 0);

			*offset = 0;

			return partial ? VIDEO_NSI_FIFO_WRITER_GONE_PARTIAL
				       : VIDEO_NSI_FIFO_WRITER_GONE;
		}
		...
		if ((errno == EAGAIN) || (errno == EWOULDBLOCK)) {
			return VIDEO_NSI_FIFO_NO_FRAME;
		}

		nsi_print_warning("Error reading the video FIFO (%s)\n", strerror(errno));
		return -nsi_errno_to_mid(errno);
	}

	*offset = 0;

	return VIDEO_NSI_FIFO_FRAME;
}
