...
static void emul_rx_worker(struct k_work *work)
{
	...
	/* Alternative to a loop: call the worker from the worker again */
	while ((vbuf = k_fifo_get(&data->fifo_in, K_NO_WAIT)) != NULL) {
		 ...
		 * buffer memory using DMA. The vbuf->size is checked in emul_rx_enqueue().
		 */
...
#define EMUL_RX_DEFINE(n)                                                                          \
	static const struct emul_rx_config emul_rx_conf_##n = {                                    \
		.source_dev = DEVICE_DT_GET(                                                       \
			DT_REMOTE_DEVICE(DT_CHILD(DT_INST_CHILD(n, port), endpoint))),             \
	};                                                                                         \
                                                                                                   \
