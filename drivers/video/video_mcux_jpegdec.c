...
static int mcux_jpegdec_set_fmt(const struct device *dev, struct video_format *fmt)
{
	 ...
	 * input frame buffer. So only validate the input size and output format if
	...
	if (data->first_frame_rcv) {
		...
		if ((!is_input) && (fmt->pixelformat != data->m2m.out.fmt.pixelformat)) {
...
static int mcux_jpegdec_enqueue(const struct device *dev, struct video_buffer *vbuf)
{
	...
	if (vbuf->type == VIDEO_BUF_TYPE_INPUT) {
		...
		if (!data->first_frame_rcv) {
			...
			/*
			 * For NV12 2-plane format, the Y plane is 1BPP and UV plane is
			 * 0.5BPP. But the same pitch configuration applies to the 2 planes, so
