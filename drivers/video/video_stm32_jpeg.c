...
/* FIXME - this depends on the JPEG quality */
#define JPEG_ESTIMATED_COMPRESSION_RATIO	2
...
static int stm32_jpeg_set_fmt(const struct device *dev, struct video_format *fmt)
{
	...
	/* FIXME - we need to approximate the JPEG compression ratio depending on quality */
	fmt->size = fmt->width * fmt->height / JPEG_ESTIMATED_COMPRESSION_RATIO;
