...
static const struct ov5642_mode_config * const dvp_mode_from_res(enum ov5642_res res)
{
	return &dvp_modes[res];
}
...
static int ov5642_set_fmt(const struct device *dev, struct video_format *fmt)
{
	...
	drv_data->cur_mode = dvp_mode_from_res((enum ov5642_res)i);
			     ...
			     enum video_buf_type type)
{
	...
	int ret;
	ret = video_write_cci_reg(&cfg->i2c, OV5642_REG8(SYS_CTRL0_REG), val);
	if (ret) {
...
static int ov5642_set_ctrl_hue(const struct device *dev, int value)
{
	...
	int ret;
	ret = video_modify_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL0_REG), BIT(0),
				   BIT(0));
	if (ret) {
		return ret;
	}
	...
	struct video_reg16 hue_params[] = {{SDE_CTRL1_REG, abs(cos_coef)},
					  {SDE_CTRL2_REG, abs(sin_coef)}};

	ret = video_modify_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL10_REG), 0x7F,
				   sign);
	if (ret < 0) {
		return ret;
	}
...
static int ov5642_set_ctrl_saturation(const struct device *dev, int value)
{
	...
	struct video_reg16 saturation_params[] = {{SDE_CTRL3_REG, value},
						 {SDE_CTRL4_REG, value}};
...
static int ov5642_set_ctrl_contrast(const struct device *dev, int value)
{
	const struct ov5642_config *cfg = dev->config;
	int ret;
	ret = video_modify_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL0_REG), BIT(2),
				   BIT(2));
	if (ret) {
		return ret;
	}

	ret = video_modify_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL8_REG), BIT(2),
				   BIT(2));
	if (ret < 0) {
		return ret;
	}

	return video_write_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL8_REG), value);
}
...
static int ov5642_set_ctrl_brightness(const struct device *dev, int value)
{
	...
	ret = video_modify_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL10_REG), BIT(2),
				   value >= 0 ? 0 : BIT(2));
	if (ret < 0) {
		return ret;
	}

	return video_write_cci_reg(&cfg->i2c, OV5642_REG8(SDE_CTRL9_REG),
				   (abs(value) << 4) & 0xf0);
}
...
static int ov5642_set_ctrl(const struct device *dev, uint32_t id)
{
	...
	case VIDEO_CID_HFLIP:
		return video_modify_cci_reg(&cfg->i2c,
				OV5642_REG8(TIMING_TC_REG18_REG),
				BIT(6),
				ctrls->hflip.val ? BIT(6) : 0);
	...
	case VIDEO_CID_EXPOSURE:
		return video_modify_cci_reg(&cfg->i2c,
				OV5642_REG8(AEC_PK_MANUAL),
				BIT(0),
				ctrls->ae.val ? BIT(0) : 0);
	case VIDEO_CID_WHITE_BALANCE_TEMPERATURE:
		return video_modify_cci_reg(&cfg->i2c,
				 OV5642_REG8(ISP_CTRL01_REG),
				 BIT(0),
...
static int ov5642_init(const struct device *dev)
{
	...
	ret = video_read_cci_reg(&cfg->i2c, OV5642_REG16(CHIP_ID_REG), &tmp);
	if (ret) {
	...
	if (cfg->bus_type == VIDEO_BUS_TYPE_PARALLEL) {
		ret = video_write_cci_reg(&cfg->i2c, OV5642_REG8(IO_MIPI_CTRL00_REG),
