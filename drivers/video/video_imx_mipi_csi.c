...
static int64_t csi_calc_link_freq(struct csi_data *csi)
{
	...
	link_freq =
		(uint64_t)video_get_csi_link_freq(csi->sensor, csi->csi_fmt.bpp, csi->num_lanes);
	if (link_freq < 0) {
		return -EINVAL;
	}
...
static int csi_configure_phy(struct csi_data *csi)
{
	...
	LOG_INF("Configuring DPHY: lanes=%u, hs_clk_rate=%llu Hz", opts.lanes, opts.hs_clk_rate);
...
static int csi_hw_init(struct csi_data *csi)
{
	...
	phy_stopstate_mask = MIPI_CSI2RX_DPHY_STOPSTATE_CLK_LANE | GENMASK(csi->num_lanes - 1, 0);
	...
	LOG_ERR("Timeout waiting for lanes stop state (got 0x%08x, expected 0x%08x)", val,
		phy_stopstate_mask);
...
static int csi_set_stream(const struct device *dev, bool enable, enum video_buf_type type)
{
	struct csi_data *csi = CSI_DATA(dev);
	int ret = 0;
...
static int csi_set_frmival(const struct device *dev, struct video_frmival *frmival)
{
	...
	LOG_INF("Frame interval set: %u/%u (%u fps)", frmival->numerator, frmival->denominator,
...
#define CSI_DEVICE_INIT(inst)                                                                      \
	...
	static const struct csi_config csi_config_##inst = {                                       \
		...
		.num_lanes = DT_PROP_LEN(DT_INST_ENDPOINT_BY_ID(inst, 0, 0), data_lanes),          \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, csi_init, NULL, &csi_data_##inst, &csi_config_##inst,          \
