...
#define DT_DRV_COMPAT nxp_imx_isi_channel
...
/*
 * Note: this matches the original single-channel driver behavior.
 * If per-channel flip is required later, move this into channel data.
 */
static isi_flip_mode_t flip = kISI_FlipDisable;

struct video_mcux_isi_channel_config {
	const struct device *core_dev;
...
struct video_mcux_isi_channel_data {
	const struct device *dev;

	struct k_queue q_in;
	struct k_queue q_out;

	/* Buffers primed before streaming starts (2 buffers for HW double-buffer). */
	struct k_queue q_active;
...
static ISI_Type *isi_base_from_core(const struct device *dev)
{
	const struct video_mcux_isi_channel_config *cfg = dev->config;
	const struct video_mcux_isi_core_data *core = video_mcux_isi_core_get_data(cfg->core_dev);
...
{
	struct video_mcux_isi_channel_data *data = dev->data;
	const struct video_mcux_isi_channel_config *cfg = dev->config;
	...
	ISI_Type *base = isi_base_from_core(dev);
	...
	if (in_is_yuv == out_is_yuv) {
		ISI_EnableColorSpaceConversion(base, cfg->channel, false);
		return 0;
	}

	if (out_is_yuv) {
		ISI_SetColorSpaceConversionConfig(base, cfg->channel, &csc_rgb2yuv);
		ISI_EnableColorSpaceConversion(base, cfg->channel, true);
		return 0;
	}

	/* out is RGB */
	ISI_SetColorSpaceConversionConfig(base, cfg->channel, &csc_yuv2rgb);
	ISI_EnableColorSpaceConversion(base, cfg->channel, true);
...
static void isi_return_buffer(struct video_mcux_isi_channel_data *data, struct video_buffer *vbuf)
{
	if (vbuf == NULL) {
		return;
	}
	...
	k_queue_append(&data->q_out, vbuf);
}

static void isi_return_all_buffers(struct video_mcux_isi_channel_data *data)
{
	...
	vbuf = k_queue_get(&data->q_active, K_NO_WAIT);
	while (vbuf != NULL) {
	...
	vbuf = k_queue_get(&data->q_in, K_NO_WAIT);
	while (vbuf != NULL) {
		if (vbuf != drop && vbuf != active0 && vbuf != active1) {
			isi_return_buffer(data, vbuf);
		}
...
static int isi_channel_configure(const struct device *dev)
{
	...
	ISI_Init(base, cfg->channel);
	ISI_SetConfig(base, cfg->channel, &data->isi_config);
	ISI_SetScalerConfig(base, cfg->channel, data->in_fmt.width, data->in_fmt.height, scale_w,
			    scale_h);
	...
	if (hw_crop_en) {
		...
		ISI_SetCropConfig(base, cfg->channel, &crop_cfg);
		ISI_EnableCrop(base, cfg->channel, true);
	} else {
		ISI_EnableCrop(base, cfg->channel, false);
	}

	ISI_SetFlipMode(base, cfg->channel, flip);
...
static int video_mcux_isi_set_fmt(const struct device *dev, struct video_format *fmt)
{
	...
	ret = video_set_format(cfg->source_dev, &in_fmt);
	if (ret) {
		return ret;
	}

	ret = video_get_format(cfg->source_dev, &data->in_fmt);
	if (ret) {
		return ret;
	}
	...
	/* Return effective output format to the caller. */
	*fmt = out_fmt;
...
static int video_mcux_isi_stream_start(const struct device *dev)
{
	...
	if (data->drop_vbuf == NULL || data->active_primed_cnt != ISI_MAX_ACTIVE_BUF) {
		LOG_ERR("%s: requires 1 drop buffer + %u active buffers", dev->name,
			ISI_MAX_ACTIVE_BUF);
		return -ENOMEM;
	}

	vb0 = k_queue_get(&data->q_active, K_NO_WAIT);
	vb1 = k_queue_get(&data->q_active, K_NO_WAIT);
	if (vb0 == NULL || vb1 == NULL) {
		LOG_ERR("%s: requires %u active buffers", dev->name, ISI_MAX_ACTIVE_BUF);
		return -ENOMEM;
	}
	...
	ISI_SetOutputBufferAddr(base, cfg->channel, 0, (uint32_t)(uintptr_t)vb0->buffer, 0, 0);
	ISI_SetOutputBufferAddr(base, cfg->channel, 1, (uint32_t)(uintptr_t)vb1->buffer, 0, 0);

	data->buffer_index = 0U;
	data->streaming = true;

	ISI_ClearInterruptStatus(base, cfg->channel, (uint32_t)kISI_FrameReceivedInterrupt);
	ISI_EnableInterrupts(base, cfg->channel, (uint32_t)kISI_FrameReceivedInterrupt);
	irq_enable(cfg->irq);
	ISI_Start(base, cfg->channel);

	ret = video_stream_start(cfg->source_dev, VIDEO_BUF_TYPE_OUTPUT);
	if (ret) {
		ISI_Stop(base, cfg->channel);
		ISI_DisableInterrupts(base, cfg->channel, (uint32_t)kISI_FrameReceivedInterrupt);
...
static int video_mcux_isi_enqueue(const struct device *dev, struct video_buffer *vbuf)
{
	...
	if (!data->streaming) {
		if (data->drop_vbuf == NULL) {
			data->drop_vbuf = vbuf;
			LOG_DBG("%s: drop frame buffer set: %p", dev->name, vbuf->buffer);
			return 0;
		}

		/* Prime the two active HW buffers before streaming starts. */
		if (data->active_primed_cnt < ISI_MAX_ACTIVE_BUF) {
			k_queue_append(&data->q_active, vbuf);
			data->active_primed_cnt++;
			LOG_DBG("%s: active buffer primed (%u/%u): %p", dev->name,
				data->active_primed_cnt, ISI_MAX_ACTIVE_BUF, vbuf->buffer);
			return 0;
		}
	}

	k_queue_append(&data->q_in, vbuf);
				  ...
				  k_timeout_t timeout)
{
	...
	*vbuf = k_queue_get(&data->q_out, timeout);
	if (*vbuf == NULL) {
		return -EAGAIN;
	}
...
static int video_mcux_isi_flush(const struct device *dev, bool cancel)
{
	...
	if (!cancel) {
		while (!k_queue_is_empty(&data->q_in)) {
			k_sleep(K_MSEC(1));
		}
		return 0;
	}
...
static int video_mcux_isi_get_selection(const struct device *dev, struct video_selection *sel)
{
	...
	switch (sel->target) {
	case VIDEO_SEL_TGT_CROP:
		sel->rect = data->crop_en ? data->crop : (struct video_rect){0};
		return 0;
	case VIDEO_SEL_TGT_COMPOSE:
		sel->rect = data->compose_en ? data->compose : (struct video_rect){0};
		return 0;
	default:
		return -ENOTSUP;
	}
...
static void video_mcux_isi_isr(const struct device *dev)
{
	...
	int_status = ISI_GetInterruptStatus(base, cfg->channel);
	ISI_ClearInterruptStatus(base, cfg->channel, int_status);
	...
	ISI_SetOutputBufferAddr(base, cfg->channel, data->buffer_index, buffer_addr, 0, 0);
	...
	if ((now_ms - data->last_stat_ms) >= 1000U) {
		if (data->drop_cnt) {
			LOG_INF("%s dropped %u frame(s) (no input buffer)", dev->name,
				data->drop_cnt);
			data->drop_cnt = 0U;
		}
...
static int video_mcux_isi_init(const struct device *dev)
{
	...
	if (!device_is_ready(cfg->core_dev)) {
		LOG_ERR("%s: ISI core device not ready", dev->name);
		return -ENODEV;
	}

	k_queue_init(&data->q_in);
	k_queue_init(&data->q_out);
	k_queue_init(&data->q_active);
	...
	data->isi_config.sourcePort = cfg->input_port;
	data->isi_config.mipiChannel = 0;

	data->scale_width = 0U;
	data->scale_height = 0U;
...
#define ISI_CORE_NODE(inst)									   \
	COND_CODE_1(DT_NODE_HAS_COMPAT(DT_INST_PARENT(inst), nxp_imx_isi),			   \
		(DT_INST_PARENT(inst)), (DT_PARENT(DT_INST_PARENT(inst))))

#define ISI_CORE_DEV(inst) DEVICE_DT_GET(ISI_CORE_NODE(inst))

#define ISI_PARENT_INPUT_PORT(inst) DT_PROP_OR(ISI_CORE_NODE(inst), input_port, 0)
...
#define VIDEO_MCUX_ISI_CHANNEL_DEVICE(inst)                                                        \
	...
	DEVICE_DT_INST_DEFINE(inst, &video_mcux_isi_channel_init_##inst, NULL,                     \
			      ...
			      &video_mcux_isi_channel_config_##inst, POST_KERNEL,                  \
			      CONFIG_VIDEO_INIT_PRIORITY, &video_mcux_isi_driver_api);             \
