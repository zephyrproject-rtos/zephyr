...
static int emul_imager_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	case VIDEO_CID_CAMERA_EXPOSURE:
		return emul_imager_write_reg(dev, EMUL_IMAGER_REG_EXPOSURE, (uint32_t)value);
	case VIDEO_CID_CAMERA_GAIN:
		return emul_imager_write_reg(dev, EMUL_IMAGER_REG_GAIN, (uint32_t)value);
	case VIDEO_CID_CAMERA_TEST_PATTERN:
		return emul_imager_write_reg(dev, EMUL_IMAGER_REG_PATTERN, (uint32_t)value);
	default:
		return -ENOTSUP;
	}
}
