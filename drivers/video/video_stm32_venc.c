...
#define EWL_HEAP_ALIGNED_ALLOC(size)				\
	mem_attr_heap_aligned_alloc((ATTR_SW_ALLOC_NON_CACHE <<	\
				    DT_MEM_SW_ATTR_SHIFT), ALIGNMENT_INCR, size)
#define EWL_HEAP_ALIGNED_FREE(block) mem_attr_heap_free(block)
...
EWLHwConfig_t EWLReadAsicConfig(void)
{
	...
	LOG_DBG("EWLReadAsicConfig:\n");
	LOG_DBG("    maxEncodedWidth   = %d\n", cfg_info.maxEncodedWidth);
	LOG_DBG("    h264Enabled       = %d\n", cfg_info.h264Enabled);
	LOG_DBG(         "    jpegEnabled       = %d\n", cfg_info.jpegEnabled);
	LOG_DBG(         "    vp8Enabled        = %d\n", cfg_info.vp8Enabled);
	LOG_DBG(         "    vsEnabled         = %d\n", cfg_info.vsEnabled);
	LOG_DBG(         "    rgbEnabled        = %d\n", cfg_info.rgbEnabled);
	LOG_DBG(         "    searchAreaSmall   = %d\n", cfg_info.searchAreaSmall);
	LOG_DBG(         "    scalingEnabled    = %d\n", cfg_info.scalingEnabled);
	LOG_DBG(         "    address64bits     = %d\n", cfg_info.addr64Support);
	LOG_DBG(         "    denoiseEnabled    = %d\n", cfg_info.dnfSupport);
	LOG_DBG(         "    rfcEnabled        = %d\n", cfg_info.rfcSupport);
	LOG_DBG(         "    instanctEnabled   = %d\n", cfg_info.instantSupport);
	LOG_DBG(         "    busType           = %d\n", cfg_info.busType);
	LOG_DBG(         "    synthesisLanguage = %d\n", cfg_info.synthesisLanguage);
...
/* #define _LL_VENC_WriteRegister(swreg,val) {if (val) LOG_DBG("'swreg%d' : 0x%08x,\n",swreg,val);LL_VENC_WriteRegister(swreg,val);} */
#define _LL_VENC_WriteRegister(swreg,val) LL_VENC_WriteRegister(swreg,val)

void EWLWriteReg(const void *inst, u32 offset, u32 val)
{
	/*LOG_DBG("%s> %d at offset 0x%02X --> %08X\n",
		__func__, offset / 4, offset, val);*/

	/* offset is defined in the EWL API in bytes. convert to number of registers by shifting */
	_LL_VENC_WriteRegister((offset >> 2), val);
}
...
i32 EWLMallocLinear(const void *instance, u32 size, EWLLinearMem_t *info)
{
	...
	inst->chunks[inst->totalChunks] = (u32 *)EWL_HEAP_ALIGNED_ALLOC(size_aligned);
	if (!inst->chunks[inst->totalChunks]) {
		LOG_DBG("EWLMallocLinear unable to allocate %8d bytes\n", size_aligned);
		return EWL_ERROR;
	}
	...
	if (!info->virtualAddress)  {
		LOG_DBG("EWLMallocLinear unable to get chunk for %8d bytes\n", size_aligned);
		return EWL_ERROR;
	}
...
#ifdef TRACE_EWL
	mem_counter += size;
	LOG_DBG("EWLMallocLinear allocated %8d bytes --> %p / 0x%x. Total : %d\n", size_aligned, info->virtualAddress,
...
#ifdef TRACE_EWL
	mem_counter += n;
	LOG_DBG("EWLmalloc %8d bytes --> %p, total : %d\n", n, p, mem_counter);
#endif /* TRACE_EWL */
...
void *EWLcalloc(u32 n, u32 s)
{
	...
	if (!p) {
		LOG_ERR("alloc failed for size=%d\n", n * s);
		return NULL;
	}
...
i32 EWLWaitHwRdy(const void *inst, u32 *slicesReady)
{
	...
	do {
		...
		LOG_DBG("EWLWaitHw: IRQ stat = %08x\n", irq_stats);
	...
	LOG_DBG("EWLWaitHw: encoding = %d ms\n", k_ticks_to_ms_ceil32(sys_clock_tick_get_32() - start));

	if (slicesReady != NULL)
		LOG_DBG("EWLWaitHw: slicesReady = %d\n", *slicesReady);
	...
	if (k_sem_take(&enc->complete, K_MSEC(EWL_TIMEOUT))) {
		...
		printf("Interrupt timeout VENC status=0x%x\n", irq_status);
		LOG_DBG("%s<\n", __func__);
		return EWL_OK;
	}

	LOG_DBG("EWLWaitHw: encoding = %ld ms\n", k_ticks_to_ms_ceil32(sys_clock_tick_get_32() - start));
...
{
	const struct device *venc_clock = DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE);

	LOG_DBG("%s>\n", __func__);

	if (!device_is_ready(venc_clock)) {
...
int encoder_prepare(struct video_stm32_venc_data *data)
{
	...
	if (ret != H264ENC_OK) {
		LOG_ERR("H264EncGetCodingCtrl error=%d\n", ret);
		return -1;
	}
#ifdef VENC_HW_HANDSHAKE
...
#else
	codingctrl_cfg.enableCabac = 0;
	codingctrl_cfg.quarterPixelMv = 0;
	codingctrl_cfg.inputLineBufEn = 0;
	codingctrl_cfg.inputLineBufLoopBackEn = 0;
	codingctrl_cfg.inputLineBufDepth = 0;
	codingctrl_cfg.inputLineBufHwModeEn = 0;
#endif
	ret = H264EncSetCodingCtrl(data->encoder, &codingctrl_cfg);
	if (ret != H264ENC_OK) {
		LOG_ERR("H264EncSetCodingCtrl error=%d\n", ret);
		return -1;
	}
	...
	if (ret != H264ENC_OK) {
		LOG_ERR("H264EncGetRateCtrl error=%d\n", ret);
		return -1;
	}
#ifdef VENC_RATECTRL_VBR
	/* Variable bitrate */
	ratectrl_cfg.pictureRc = 1;
	ratectrl_cfg.mbRc = 1;
	ratectrl_cfg.pictureSkip = 0;
	ratectrl_cfg.hrd = 0;
	ratectrl_cfg.qpHdr = VENC_RATECTRL_QP;
	ratectrl_cfg.qpMin = 10;
	ratectrl_cfg.qpMax = 51;
	ratectrl_cfg.gopLen = VENC_RATECTRL_FPS;
	ratectrl_cfg.bitPerSecond = ((data->width * data->height * 12) * VENC_RATECTRL_FPS) / 30;
	...
	ratectrl_cfg.hrd = 0;
	ratectrl_cfg.qpHdr = VENC_RATECTRL_QP;
	ratectrl_cfg.qpMin = VENC_RATECTRL_QP;
	ratectrl_cfg.qpMax = VENC_RATECTRL_QP;
#endif
	ret = H264EncSetRateCtrl(data->encoder, &ratectrl_cfg);
	if (ret != H264ENC_OK) {
		LOG_ERR("H264EncSetRateCtrl error=%d\n", ret);
		return -1;
	}
...
static int encoder_start(struct video_stm32_venc_data *data)
{
	...
	encIn.pOutBuf = (u32 *)output->buffer;
	encIn.busOutBuf = (uint32_t) encIn.pOutBuf;
	encIn.outBufSize = ALIGNED_SIZE(output->size);

	/* create stream */
	ret = H264EncStrmStart(data->encoder, &encIn, &encOut);
	if (ret != H264ENC_OK) {
		LOG_ERR("H264EncStrmStart error=%d\n", ret);
		return -1;
	}

	output->bytesused = encOut.streamSize;
	LOG_DBG("SPS/PPS generated, size= %d\n", output->bytesused);

	LOG_DBG("%s> %p\n", __func__, &data->fifo_output_out);
...
static int encode_frame(struct video_stm32_venc_data *data)
{
	...
	output->bytesused = encOut.streamSize;
	LOG_DBG("output=%p, encOut.streamSize=%d\n", output, encOut.streamSize);
	...
	case H264ENC_FUSE_ERROR:
		LOG_ERR("H264EncStrmEncode error=%d\n", ret);

		LOG_ERR("DCMIPP and VENC desync at frame %d, restart the video\n", data->frame_nb);
		encoder_end(data);
	...
	default:
		LOG_ERR("H264EncStrmEncode error=%d\n", ret);
		LOG_ERR("error encoding frame %d\n", data->frame_nb);
				  ...
				  struct video_buffer *vbuf)
{
	...
	if (vbuf->type == VIDEO_BUF_TYPE_INPUT) {
		...
		LOG_DBG("%s put %p in fifo_input\n", __func__, vbuf);
		ret = encode_frame(data);
	} else {
		k_fifo_put(&data->fifo_output_in, vbuf);
		LOG_DBG("%s put %p in fifo_output_in\n", __func__, vbuf);
	}
...
static int video_stm32_venc_init(const struct device *dev)
{
	...
	LOG_DBG("CPU frequency    : %d\n", HAL_RCC_GetCpuClockFreq() / 1000000);
	LOG_DBG("sysclk frequency : %d\n", HAL_RCC_GetSysClockFreq() / 1000000);
	LOG_DBG("pclk5 frequency  : %d\n", HAL_RCC_GetPCLK5Freq() / 1000000);
