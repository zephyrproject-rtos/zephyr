...
#define VIDEO_SW_GENERATOR_JPEG_FORMAT_CAP                                                         \
	{                                                                                          \
...
static const struct video_format_cap fmts[] = {
	...
	VIDEO_SW_GENERATOR_JPEG_FORMAT_CAP,
	{0},
};
...
static int video_sw_generator_set_fmt(const struct device *dev, struct video_format *fmt)
{
	...
	/* Special handling for JPEG format */
...
static int video_sw_generator_fill_jpeg(const struct device *const dev, struct video_buffer *vbuf,
					bool hflip)
{
	uint8_t *frame = jpeg_frame_buffers[hflip ? 1U : 0U];

	/* Check if buffer is large enough */
	if (vbuf->size < JPEG_FRAME_SIZE) {
		LOG_ERR("Buffer too small for JPEG frame (need %d, have %zu)", JPEG_FRAME_SIZE,
			vbuf->size);
		return -ENOMEM;
	}

	/* Copy the JPEG frame data to the video buffer */
	memcpy(vbuf->buffer, frame, JPEG_FRAME_SIZE);
	vbuf->bytesused = JPEG_FRAME_SIZE;
...
static int video_sw_generator_fill(const struct device *const dev, struct video_buffer *vbuf)
{
	...
	/* Handle JPEG format specially */
	if (data->fmt.pixelformat == VIDEO_PIX_FMT_JPEG) {
		return video_sw_generator_fill_jpeg(dev, vbuf, hflip);
	}
...
static int video_sw_generator_init_controls(const struct device *dev)
{
	...
	ret = video_init_menu_ctrl(&data->ctrls.test_pattern, dev, VIDEO_CID_TEST_PATTERN, 0,
				   test_pattern_menu);
	if (ret < 0) {
		return ret;
	}
