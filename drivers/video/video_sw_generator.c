					   ...
					   struct video_frmival_enum *fie)
{
	...
	while (fmts[i].pixelformat && (fmts[i].pixelformat != fie->pixelformat)) {
		i++;
	}

	if ((i == ARRAY_SIZE(fmts)) || (fie->width > fmts[i].width_max) ||
	    (fie->width < fmts[i].width_min) || (fie->height > fmts[i].height_max) ||
	    (fie->height < fmts[i].height_min)) {
		return -EINVAL;
	}
