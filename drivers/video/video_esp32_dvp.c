/*
 * Copyright (c) 2024 espros photonics Co., Ltd.
 *
...
struct video_esp32_data {
	...
	uint8_t dma_buffer[VIDEO_ESP32_DMA_BUFFER_MAX_SIZE];
	struct dma_block_config dma_blocks[CONFIG_DMA_ESP32_DESCRIPTOR_NUM];
};

struct video_esp32_dma_data {
	struct k_fifo *fifo_out;
			       ...
			       struct video_format *fmt)
{
	...
	ret = video_get_format(cfg->sensor_dev, ep, fmt);
	if (ret) {
		LOG_ERR("Failed to get format from sensor");
		return ret;
	}
			       ...
			       struct video_format *fmt)
{
	...
	return video_set_format(cfg->sensor_dev, ep, fmt);
}
...
static int video_esp32_set_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	return video_set_ctrl(cfg->sensor_dev, cid, value);
}
...
static int video_esp32_get_ctrl(const struct device *dev, unsigned int cid, void *value)
{
	...
	return video_get_ctrl(cfg->sensor_dev, cid, value);
}
