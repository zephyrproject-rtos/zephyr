...
static isi_flip_mode_t video_mcux_isi_get_flip(const struct video_mcux_isi_ctrls *ctrls)
{
	bool hflip = ctrls->hflip.val != 0;
	bool vflip = ctrls->vflip.val != 0;

	if (hflip && vflip) {
		return kISI_FlipBoth;
	} else if (hflip) {
		return kISI_FlipHorizontal;
	} else if (vflip) {
		return kISI_FlipVertical;
	}
	return kISI_FlipDisable;
}
...
static int isi_channel_configure(const struct device *dev)
{
	...
	ISI_SetFlipMode(base, cfg->channel, video_mcux_isi_get_flip(&data->ctrls));
...
static int video_mcux_isi_set_selection(const struct device *dev, struct video_selection *sel)
{
	...
	case VIDEO_SEL_TGT_CROP:
		/* Validate against CROP_BOUND (full input frame size) */
		if (sel->rect.left + sel->rect.width > data->in_fmt.width ||
		    sel->rect.top + sel->rect.height > data->in_fmt.height) {
			return -EINVAL;
		}
		data->crop = sel->rect;
	...
	case VIDEO_SEL_TGT_COMPOSE:
		/* Validate against COMPOSE_BOUND (full output format size) */
		if (sel->rect.left + sel->rect.width > data->fmt.width ||
		    sel->rect.top + sel->rect.height > data->fmt.height) {
			return -EINVAL;
		}
...
static int video_mcux_isi_get_selection(const struct device *dev, struct video_selection *sel)
{
	...
	case VIDEO_SEL_TGT_CROP_BOUND:
		/* Maximum crop = full input frame from upstream source */
		sel->rect.left   = 0;
		sel->rect.top    = 0;
		sel->rect.width  = data->in_fmt.width;
		sel->rect.height = data->in_fmt.height;
		return 0;
	case VIDEO_SEL_TGT_COMPOSE:
		sel->rect = data->compose_en ? data->compose : (struct video_rect){0};
		return 0;
	case VIDEO_SEL_TGT_COMPOSE_BOUND:
		/* Maximum compose = full output format size */
		sel->rect.left   = 0;
		sel->rect.top    = 0;
