...
/* Check if the SOC supports CSI & PixelPipes by searching for HAL macros */
#if defined(HAL_DCMIPP_CSI_ERROR_NONE)
...
void HAL_DCMIPP_PIPE_FrameEventCallback(DCMIPP_HandleTypeDef *hdcmipp, uint32_t Pipe)
{
	...
	if (Pipe == DCMIPP_PIPE0) {
		...
		if (ret != HAL_OK) {
			LOG_DBG("Failed to read counter - buffer in error");
...
static int stm32_dcmipp_stream_enable(const struct device *dev)
{
	...
	input_fmt = stm32_dcmipp_get_input_info(dcmipp->sensor_fmt.pixelformat);
	if (input_fmt == NULL) {
	...
	if (dcmipp->enabled_pipe == 0) {
		ret = video_set_format(config->sensor_dev, &dcmipp->sensor_fmt);
		if (ret < 0) {
			goto out;
		}
...
static int stm32_dcmipp_set_frmival(const struct device *dev, struct video_frmival *frmival)
{
	...
	return video_set_frmival(config->sensor_dev, frmival);
}
...
static int stm32_dcmipp_enum_frmival(const struct device *dev, struct video_frmival_enum *fie)
{
	...
	return video_enum_frmival(config->sensor_dev, fie);
}
