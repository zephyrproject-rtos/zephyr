				    ...
				    struct video_frmival_enum *fie)
{
	...
	if (fie->type == VIDEO_FRMIVAL_TYPE_DISCRETE) {
		est_pixel_rate = mipi_csi2rx_estimate_pixel_rate(&cur_frmival, &fie->discrete,
		...
		est_pixel_rate = mipi_csi2rx_estimate_pixel_rate(&cur_frmival, &fie->stepwise.min,
								 &cur_fmt, &fie_fmt, cur_pixel_rate,
								 drv_data->csi2rxConfig.laneNum);
		if (est_pixel_rate > MAX_SUPPORTED_PIXEL_RATE) {
			return -EINVAL;
		...
		est_pixel_rate = mipi_csi2rx_estimate_pixel_rate(&cur_frmival, &fie->stepwise.max,
								 &cur_fmt, &fie_fmt, cur_pixel_rate,
								 drv_data->csi2rxConfig.laneNum);
		if (est_pixel_rate > MAX_SUPPORTED_PIXEL_RATE) {
			fie->stepwise.max.denominator =
				...
				(mipi_csi2rx_cal_frame_size(&fie_fmt) * cur_pixel_rate *
