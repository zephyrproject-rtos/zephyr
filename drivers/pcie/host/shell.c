...
static void show_msi(const struct shell *sh, pcie_bdf_t bdf)
{
	...
	if (msi) {
		...
		shell_fprintf(sh, SHELL_NORMAL, "    MSI support%s%s\n",
			      ...
			      (data & PCIE_MSI_MCR_EN) ? ", enabled" : ", disabled");
	}
	...
	if (msi) {
		...
		table_size = ((data & PCIE_MSIX_MCR_TSIZE) >> PCIE_MSIX_MCR_TSIZE_SHIFT) + 1;

		shell_fprintf(sh, SHELL_NORMAL, "    MSI-X support%s table size %d\n",
			      (data & PCIE_MSIX_MCR_EN) ? ", enabled" : ", disabled", table_size);

		offset = pcie_conf_read(bdf, msi + PCIE_MSIX_TR);
		bir = offset & PCIE_MSIX_TR_BIR;
		offset &= PCIE_MSIX_TR_OFFSET;

		shell_fprintf(sh, SHELL_NORMAL, "\tTable offset 0x%x BAR %d\n", offset, bir);

		offset = pcie_conf_read(bdf, msi + PCIE_MSIX_PBA);
		bir = offset & PCIE_MSIX_PBA_BIR;
		offset &= PCIE_MSIX_PBA_OFFSET;

		shell_fprintf(sh, SHELL_NORMAL, "\tPBA offset 0x%x BAR %d\n", offset, bir);
	}
#endif
}
...
static void show_bars(const struct shell *sh, pcie_bdf_t bdf)
{
	...
	for (bar = PCIE_CONF_BAR0; bar <= PCIE_CONF_BAR5; ++bar) {
		...
		shell_fprintf(sh, SHELL_NORMAL, "    bar %d: %s%s", bar - PCIE_CONF_BAR0,
		...
		if (PCIE_CONF_BAR_64(data)) {
			++bar;
			shell_fprintf(sh, SHELL_NORMAL, "%08x", pcie_conf_read(bdf, bar));
		}
...
static void show_vc(const struct shell *sh, pcie_bdf_t bdf)
{
	...
	shell_fprintf(sh, SHELL_NORMAL,
		      ...
		      regs.cap_reg_1.vc_count + 1, regs.cap_reg_1.lpvc_count,
		      regs.cap_reg_1.pat_entry_size, regs.cap_reg_2.vca_cap,
	...
	for (idx = 0; idx < regs.cap_reg_1.vc_count + 1; idx++) {
		shell_fprintf(sh, SHELL_NORMAL,
			      ...
			      idx, res_regs[idx].cap_reg.pa_cap, res_regs[idx].cap_reg.rst,
...
static void pcie_dump(const struct shell *sh, pcie_bdf_t bdf)
{
	for (int i = 0; i < 16; i++) {
		...
		for (int j = 0; j < 4; j++) {
			shell_fprintf(sh, SHELL_NORMAL, "%02x ", (uint8_t)val);
			val >>= 8;
		}
...
static void show(const struct shell *sh, pcie_bdf_t bdf, bool details, bool dump)
{
	...
	shell_fprintf(sh, SHELL_NORMAL, "%d:%x.%d ID %x:%x ", PCIE_BDF_TO_BUS(bdf),
		      PCIE_BDF_TO_DEV(bdf), PCIE_BDF_TO_FUNC(bdf), PCIE_ID_TO_VEND(data),
		      PCIE_ID_TO_DEV(data));

	data = pcie_conf_read(bdf, PCIE_CONF_CLASSREV);
	shell_fprintf(sh, SHELL_NORMAL, "class %x subclass %x prog i/f %x rev %x",
		      PCIE_CONF_CLASSREV_CLASS(data), PCIE_CONF_CLASSREV_SUBCLASS(data),
		      PCIE_CONF_CLASSREV_PROGIF(data), PCIE_CONF_CLASSREV_REV(data));
	...
	if (PCIE_CONF_TYPE_BRIDGE(data)) {
		...
		if (irq != PCIE_CONF_INTR_IRQ_NONE) {
			shell_fprintf(sh, SHELL_NORMAL, "    wired interrupt on IRQ %d\n", irq);
		}
	}
