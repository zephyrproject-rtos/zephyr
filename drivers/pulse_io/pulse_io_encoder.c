			  ...
			  size_t *produced)
{
	...
	for (size_t i = 0; i < nbytes; i++) {
		...
		for (int bit = 0; bit < 8; bit++) {
			...
			/*
			 * Match both halves of the pair: protocols like NEC
			 * use identical marks and encode the bit in the
			 * space alone.
			 */
			if (within(sym[0].duration, tmpl->one[0].duration, tolerance_ticks) &&
			    within(sym[1].duration, tmpl->one[1].duration, tolerance_ticks)) {
				b |= (uint8_t)(1U << pos);
			} else if (within(sym[0].duration, tmpl->zero[0].duration,
					  tolerance_ticks) &&
				   within(sym[1].duration, tmpl->zero[1].duration,
					  tolerance_ticks)) {
