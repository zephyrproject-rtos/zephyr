...
static int crypto_aes_cbc_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	...
	do {
		...
		if (session->op == CRYPTO_CIPHER_OP_DECRYPT) {
			...
			iv = &pkt->in_buf[inp_bytes - AES_BLOCK_SIZE];
		}

		/* read the dataout */
		ret = DL_AES_getDataOut(config->regs, &pkt->out_buf[out_bytes]);
		if (ret != DL_AES_STATUS_SUCCESS) {
			break;
		}
		out_bytes += AES_BLOCK_SIZE;

	} while (out_bytes < data_len);
			     ...
			     enum cipher_algo algo, enum cipher_mode mode, enum cipher_op op)
{
	...
	if (session == NULL) {
		LOG_INF("All %d session(s) in use", CONFIG_CRYPTO_MSPM0_MAX_SESSION);
		ret = -EINVAL;
		goto out;
	}
