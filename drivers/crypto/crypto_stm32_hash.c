...
#if DT_INST_PROP(0, st_has_sha3_algorithm)
...
static int stm32_hash_handler(struct hash_ctx *ctx, struct hash_pkt *pkt, bool finish)
{
...
#if defined(STM32_HASH_USE_ISR)
	/*
	 * Unlike the legacy HAL (one _Start_IT() entry point per algorithm),
	 * HAL2's HAL_HASH_Compute_IT() is generic: the algorithm was already
	 * selected above via HAL_HASH_SetConfig(), so every algorithm exposed
	 * on this node can go through the interrupt-driven path.
	 */
	k_sem_reset(&data->complete_sem);

	status = HAL_HASH_Compute_IT(&data->hhash, pkt->in_buf, pkt->in_len, pkt->out_buf,
				     UINT32_MAX, &digest_size);
	if (status == HAL_OK) {
		/*
		 * HAL_HASH_Compute_IT() arms the peripheral and returns
		 * immediately; completion (or error) is signalled asynchronously
		 * from the HASH ISR via HAL_HASH_DigestCpltCallback()/
		 * HAL_HASH_ErrorCallback(), both of which give complete_sem.
		 */
		if (k_sem_take(&data->complete_sem, K_MSEC(1000)) != 0) {
			status = HAL_TIMEOUT;
		} else if (HAL_HASH_GetState(&data->hhash) != HAL_HASH_STATE_CONFIGURED) {
			status = HAL_ERROR;
		} else {
			k_sem_give(&data->device_sem);
			UNUSED(digest_size);
	...
	if ((session->algo == CRYPTO_HASH_ALGO_SHA224) ||
		...
		switch (session->algo) {
		case CRYPTO_HASH_ALGO_SHA224:
			status = HAL_HASHEx_SHA224_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							    pkt->in_len, pkt->out_buf);
			break;
		case CRYPTO_HASH_ALGO_SHA256:
			status = HAL_HASHEx_SHA256_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							    pkt->in_len, pkt->out_buf);
			break;
#if defined(STM32_HASH_USE_SHA3)
		case CRYPTO_HASH_ALGO_SHA3_224:
			status = HAL_HASHEx_SHA3_224_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							      pkt->in_len, pkt->out_buf);
			break;
		case CRYPTO_HASH_ALGO_SHA3_256:
			status = HAL_HASHEx_SHA3_256_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							      pkt->in_len, pkt->out_buf);
			break;
		case CRYPTO_HASH_ALGO_SHA3_384:
			status = HAL_HASHEx_SHA3_384_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							      pkt->in_len, pkt->out_buf);
			break;
		case CRYPTO_HASH_ALGO_SHA3_512:
			status = HAL_HASHEx_SHA3_512_Start_IT(&data->hhash, (uint8_t *)pkt->in_buf,
							      pkt->in_len, pkt->out_buf);
			break;
#endif /* STM32_HASH_USE_SHA3 */
		default:
	...
	switch (session->algo) {
	case CRYPTO_HASH_ALGO_SHA224:
		status = HAL_HASHEx_SHA224_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						 pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
	case CRYPTO_HASH_ALGO_SHA256:
		status = HAL_HASHEx_SHA256_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						 pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
#if defined(STM32_HASH_USE_SHA3)
	case CRYPTO_HASH_ALGO_SHA3_224:
		status = HAL_HASHEx_SHA3_224_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						   pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
	case CRYPTO_HASH_ALGO_SHA3_256:
		status = HAL_HASHEx_SHA3_256_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						   pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
	case CRYPTO_HASH_ALGO_SHA3_384:
		status = HAL_HASHEx_SHA3_384_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						   pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
	case CRYPTO_HASH_ALGO_SHA3_512:
		status = HAL_HASHEx_SHA3_512_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						   pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
#endif /* STM32_HASH_USE_SHA3 */
#if DT_INST_PROP(0, st_has_sha384_algorithm)
	case CRYPTO_HASH_ALGO_SHA384:
		status = HAL_HASHEx_SHA384_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						 pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
#endif /* DT_INST_PROP(0, st_has_sha384_algorithm) */
#if DT_INST_PROP(0, st_has_sha512_algorithm)
	case CRYPTO_HASH_ALGO_SHA512:
		status = HAL_HASHEx_SHA512_Start(&data->hhash, (uint8_t *)pkt->in_buf,
						 pkt->in_len, pkt->out_buf, HAL_MAX_DELAY);
		break;
#endif /* DT_INST_PROP(0, st_has_sha512_algorithm) */
	default:
...
/*
 * HAL2 renamed the digest-complete callback from HAL_HASH_DgstCpltCallback()
 * to HAL_HASH_DigestCpltCallback(); HAL_HASH_ErrorCallback() kept its name.
 */
#if defined(CONFIG_STM32_HAL2)
void HAL_HASH_DigestCpltCallback(stm32_hash_handle_t *hhash)
#else
void HAL_HASH_DgstCpltCallback(stm32_hash_handle_t *hhash)
#endif /* CONFIG_STM32_HAL2 */
{
	ARG_UNUSED(hhash);
...
void HAL_HASH_ErrorCallback(stm32_hash_handle_t *hhash)
{
	ARG_UNUSED(hhash);
	k_sem_give(&crypto_stm32_hash_dev_data.complete_sem);
}
