...
#include "crypto_stm32_sha_priv.h"

#define LOG_LEVEL CONFIG_CRYPTO_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(crypto_stm32_sha);

#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32_hash)
#define DT_DRV_COMPAT st_stm32_hash
#else
#error No STM32 HW Hash Accelerator in device tree
#endif

#if DT_HAS_COMPAT_STATUS_OKAY(st_stm32_hash)
#define STM32_HASH_TYPEDEF HASH_TypeDef
#endif

#define CRYPTO_HASH_MAX_SESSIONS CONFIG_CRYPTO_HASH_MAX_SESSIONS

static struct crypto_stm32_sha_session crypto_stm32_sha_sessions[CRYPTO_HASH_MAX_SESSIONS];

static int crypto_stm32_sha_get_unused_session_index(const struct device *dev)
{
	int i;
	struct crypto_stm32_sha_data *data = CRYPTO_STM32_SHA_DATA(dev);

	k_sem_take(&data->session_sem, K_FOREVER);

	for (i = 0; i < CRYPTO_HASH_MAX_SESSIONS; i++) {
		if (!crypto_stm32_sha_sessions[i].in_use) {
			crypto_stm32_sha_sessions[i].in_use = true;
...
static int stm32_hash_handler(struct hash_ctx *ctx, struct hash_pkt *pkt, bool finish)
{
	...
	struct crypto_stm32_sha_data *data = CRYPTO_STM32_SHA_DATA(dev);
	struct crypto_stm32_sha_session *session = CRYPTO_STM32_SHA_SESSN(ctx);
	...
	case CRYPTO_HASH_ALGO_SHA224:
		status = HAL_HASHEx_SHA224_Start(
			&data->hhash,
			pkt->in_buf,
			pkt->in_len,
			pkt->out_buf,
...
static int stm32_hash_begin_session(const struct device *dev,
				    struct hash_ctx *ctx,
				    enum hash_algo algo)
{
	int ctx_idx;
	struct crypto_stm32_sha_session *session;

	ctx_idx = crypto_stm32_sha_get_unused_session_index(dev);
	if (ctx_idx < 0) {
		LOG_ERR("No free session for now");
		return -ENOSPC;
	}

	session = &crypto_stm32_sha_sessions[ctx_idx];
...
static int stm32_hash_free_session(const struct device *dev,
				   struct hash_ctx *ctx)
{
	struct crypto_stm32_sha_session *session = CRYPTO_STM32_SHA_SESSN(ctx);
...
static int crypto_stm32_hash_init(const struct device *dev)
{
	...
	const struct crypto_stm32_sha_config *cfg = CRYPTO_STM32_SHA_CFG(dev);
	struct crypto_stm32_sha_data *data = CRYPTO_STM32_SHA_DATA(dev);
	...
	if (HAL_HASH_DeInit(&data->hhash) != HAL_OK) {
	...
	LOG_INF("STM32 HASH driver initialized");
	return 0;
}
...
static struct crypto_stm32_sha_data crypto_stm32_hash_dev_data = {0};

static const struct crypto_stm32_sha_config crypto_stm32_hash_dev_config = {
...
DEVICE_DT_INST_DEFINE(0, crypto_stm32_hash_init, NULL,
		      &crypto_stm32_hash_dev_data, &crypto_stm32_hash_dev_config,
