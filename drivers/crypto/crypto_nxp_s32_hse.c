...
#define CRYPTO_NXP_S32_HSE_SERVICE_TIMEOUT 1000000000
						     ...
						     struct cipher_ctx *ctx)
{
	...
	session->key_infor.keyBitLen = CONFIG_CRYPTO_NXP_S32_HSE_AES_KEY_SIZE;
...
static int crypto_nxp_s32_hse_cipher_free_session(const struct device *dev, struct cipher_ctx *ctx)
{
	...
	memset(&session->key_infor, 0, sizeof(hseKeyInfo_t));
	free_session(dev, session);
	k_mutex_unlock(&session->crypto_lock);
