...
{
	int retry_count = 0;
	struct ataes132a_device_data *data = dev->fixed->driver_data;
	const struct ataes132a_device_config *cfg = dev->fixed->config_info;
	uint8_t count;
	uint8_t status;
	uint8_t crc[2];
	int i, i2c_return;
...
{
	struct ataes132a_device_data *ataes132a = dev->fixed->driver_data;
	const struct ataes132a_device_config *cfg = dev->fixed->config_info;
	uint32_t i2c_cfg;
