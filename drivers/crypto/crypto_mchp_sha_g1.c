				      ...
				      int len)
{
	...
	while (len >= sizeof(uint32_t)) {
		v = data[i * sizeof(uint32_t) + 3];
		v = (v << BITS_PER_BYTE) | data[i * sizeof(uint32_t) + 2];
		v = (v << BITS_PER_BYTE) | data[i * sizeof(uint32_t) + 1];
		v = (v << BITS_PER_BYTE) | data[i * sizeof(uint32_t) + 0];
	...
	if (len > 0) {
		v = data[i * sizeof(uint32_t) + 0];
		if (len > 1) {
			v |= (uint32_t)data[i * sizeof(uint32_t) + 1] << 8;
		}
		if (len > 2) {
			v |= (uint32_t)data[i * sizeof(uint32_t) + 2] << 16;
		}
		if (i < ARRAY_SIZE(regs->SHA_IDATAR)) {
			regs->SHA_IDATAR[i] = v;
		} else {
...
static inline void mchp_sha_get_output(sha_registers_t *const regs, unsigned char *data, int len)
{
	...
	for (i = 0; i < (len / sizeof(uint32_t)) && i < ARRAY_SIZE(regs->SHA_IODATAR); i++) {
		v = regs->SHA_IODATAR[i];
		data[i * sizeof(uint32_t)] = (uint8_t)(v);
		data[i * sizeof(uint32_t) + 1] = (uint8_t)(v >> 8);
		data[i * sizeof(uint32_t) + 2] = (uint8_t)(v >> 16);
		data[i * sizeof(uint32_t) + 3] = (uint8_t)(v >> 24);
	}
}
			    ...
			    const unsigned char *data, unsigned int len, unsigned char *digest)
{
	...
	while (len - processed >= block_size) {
		...
		mchp_sha_set_input(regs, &data[processed], block_size);
	...
	if (len - processed) {
		mchp_sha_set_input(regs, &data[processed], len - processed);
		ret = mchp_sha_wait_data_rdy(regs);
		if (ret < 0) {
			return ret;
		}
...
static int mchp_sha_handler(struct hash_ctx *ctx, struct hash_pkt *pkt, bool finish)
{
	...
	ret = mchp_sha_process(cfg->regs, session->algo_cfg, pkt->in_buf, pkt->in_len,
			       inplace_ops ? (uint8_t *)pkt->in_buf : pkt->out_buf);
