					 ...
					 struct cipher_pkt *pkt)
{
	...
	if (pkt->out_buf_max < pkt->in_len) {
		LOG_ERR("Output buffer too small: %d < %d", pkt->out_buf_max, pkt->in_len);
		return -EINVAL;
	}
...
static int opentitan_aes_cbc_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *iv)
{
	...
	/*
	 * Unless the session opts out with CAP_NO_IV_PREFIX, the API convention is
	 * that the IV travels with the ciphertext: an encrypt op prepends it to
	 * out_buf, and a decrypt op reads it back from the first block of in_buf
	 * (in which case the 'iv' argument is ignored).
	 */
	const bool prefix_iv = (ctx->flags & CAP_NO_IV_PREFIX) == 0;
	const bool encrypt = (sess->dir == CRYPTO_CIPHER_OP_ENCRYPT);

	/* Payload size, i.e. in_len minus any IV prefix carried on the input. */
	const int data_len = (prefix_iv && !encrypt) ? pkt->in_len - 16 : pkt->in_len;
	const int out_len = (prefix_iv && encrypt) ? data_len + 16 : data_len;

	if (data_len == 0) {
		LOG_ERR("CBC input %d holds an IV prefix but no ciphertext", pkt->in_len);
		return -EINVAL;
	}

	if (pkt->out_buf_max < out_len) {
		LOG_ERR("Output buffer too small: %d < %d", pkt->out_buf_max, out_len);
		return -EINVAL;
	}
	...
	uint32_t num_blocks = data_len / 16;
	...
	for (uint32_t i = 0; i < num_blocks; i++) {
		...
		aes_read_block(base, out + i * 16);

		if (i + 2 < num_blocks) {
			aes_write_block(base, in + (i + 2) * 16);
		}
	}
...
static int opentitan_aes_ctr_op(struct cipher_ctx *ctx, struct cipher_pkt *pkt, uint8_t *ctr)
{
	...
	/*
	 * The counter block is a split counter: the caller owns only the leading
	 * IV bytes (ivlen = 128 bits - ctr_len), and the trailing counter half is
	 * ours to manage and starts at zero. Reading a full block from 'ctr' would
	 * run off the end of the caller's buffer.
	 */
	uint32_t ctr_len_bits = ctx->mode_params.ctr_info.ctr_len;
	...
	uint8_t counter_blk[16] = {0};

	memcpy(counter_blk, ctr, 16u - (ctr_len_bits / 8u));
	...
	for (uint32_t i = 0; i < 4; i++) {
		iv_words[i] = sys_get_le32(counter_blk + i * 4);
	}
	aes_write_iv(base, iv_words);
...
/*
 * CAP_NO_IV_PREFIX is advertised as supported, not as mandatory: CBC follows
 * the default IV-prefix convention unless the session opts out via ctx->flags.
 */
