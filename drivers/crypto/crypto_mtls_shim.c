...
K_MEM_POOL_DEFINE(mtls_shim_session_pool,
		  sizeof(struct mtls_shim_session),
		  sizeof(struct mtls_shim_session),
				  ...
				  enum cipher_op op_type)
{
	...
	r = mbedtls_cipher_setkey(&session->cipher,
				  ctx->key.bit_stream, 128, op);
	if (r != 0) {
		mtls_free_session(session);
		return -EINVAL;
	}
				    ...
				    struct cipher_ecc_pkt *pkt)
{
	...
	r = mbedtls_ecp_point_read_binary(&grp, &pt,
					  pkt->public_key,
					  pkt->public_key_len);
	if (r != 0) {
				   ...
				   struct cipher_ecc_pkt *pkt)
{
	...
	r = mbedtls_ecp_point_read_binary(&keypair.grp,
					  &Q, pkt->public_key,
					  pkt->public_key_len);
	if (r != 0) {
		goto free_Q;
			      ...
			      enum cipher_op op_type)
{
	...
	switch (algo) {
	case CRYPTO_CIPHER_ALGO_AES:
		return mtls_session_setup_aes(ctx, mode, algo, op_type);
#if defined(CONFIG_MBEDTLS_ENABLE_ECC_DH)
	case CRYPTO_CIPHER_ALGO_ECC:
