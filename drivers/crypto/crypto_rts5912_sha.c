...
K_MUTEX_DEFINE(crypto_rts5912_in_use);
...
static int rts5912_sha256_process(const struct device *dev, uint8_t *input, size_t blk_size)
{
	...
	k_mutex_lock(&crypto_rts5912_in_use, K_FOREVER);
	for (uint32_t i = 0; i < 8; i++) {
		rts5912_sha256_ctx->state[i] = sha2_regs->digest[i << 1];
	}
	k_mutex_unlock(&crypto_rts5912_in_use);
	return 0;
}
				      ...
				      enum hash_algo algo)
{
	...
	if (k_sem_take(&(rts5912_sha256_ctx->in_use), K_FOREVER) != 0) {
		printk("Crypto driver is busy!");
		return -EBUSY;
	}
...
static int rts5912_hash_free_session(const struct device *dev, struct hash_ctx *ctx)
{
	...
	k_sem_give(&(rts5912_sha256_ctx->in_use));
...
static int rts5912_sha_init(const struct device *dev)
{
	...
	k_sem_init(&(rts5912_sha256_ctx->in_use), 1, 1);
	...
	rts5912_hash_free_session(dev, pkt.ctx);
