...
struct dma_npcx_ch_data {
	int channel_active; /* busy is 1 and idle is 0 */
...
static void dma_npcx_isr(const struct device *dev)
{
	...
	for (int channel = 0; channel < dev_cfg->max_channel; channel++) {
		struct gdma_reg *const inst = HAL_INSTANCE(dev, channel);
...
static int dma_npcx_configure(const struct device *dev, uint32_t channel, struct dma_config *cfg)
{
	...
	if (channel >= dev_cfg->max_channel) {
	...
	k_mutex_lock(&channel_data->ch_mutex, K_FOREVER);

	if (channel_data->channel_active == 1) {
		LOG_ERR("Channel %x is busy", channel);
		ret = -EBUSY;
		goto dma_config_exit;
	}
	...
	ret = dma_set_controller(dev, channel, cfg);

dma_config_exit:
	k_mutex_unlock(&channel_data->ch_mutex);
...
static int dma_npcx_start(const struct device *dev, uint32_t channel)
{
	...
	ret = dma_trans_start(dev, channel);
...
static int dma_npcx_init(const struct device *dev)
{
	...
	/* default as below  */
	for (uint8_t ch = 0; ch < dev_cfg->max_channel; ch++) {
		/* power up both channel when init */
		dma_set_power_down(dev, ch, 0);

		channel_data = &dev_data->channels[ch];
		k_mutex_init(&channel_data->ch_mutex);
	}
...
#define DMA_NPCX_INIT(n)                                                                           \
	...
	static struct dma_npcx_ch_data dma_npcx##n##_channels[DT_INST_PROP(n, dma_channels)];    \
	static struct dma_npcx_dev_data dma_npcx_data_##n = {                                      \
		.channels = dma_npcx##n##_channels,                                                \
	};                                                                                         \
	static const struct dma_npcx_config npcx_config_##n = {                                    \
		.reg_base = &gdma_reg_ch_base_##n[0],                                              \
		.max_channel = DT_INST_PROP(n, dma_channels),                                      \
