...
static void rz_dma_int_isr(const struct device *dev)
{
	struct dma_renesas_rz_data *data = dev->data;
	dma_extended_cfg_t *p_extend =
		(dma_extended_cfg_t *)data->channels[data->curr_channel].fsp_cfg.p_extend;

#ifdef CONFIG_CPU_CORTEX_A
	transfer_info_t *p_info = data->channels[data->curr_channel].fsp_cfg.p_info;
...
#define RZ_DMA_IRQ_CONFIGURE(n, inst)                                                              \
	IRQ_CONNECT(DT_INST_IRQ_BY_IDX(inst, n, irq), DT_INST_IRQ_BY_IDX(inst, n, priority),       \
		    rz_dma_int_isr, DEVICE_DT_INST_GET(inst),                                      \
