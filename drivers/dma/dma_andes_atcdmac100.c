...
#define DT_DRV_COMPAT andestech_atcdmac100

#define LOG_LEVEL CONFIG_DMA_LOG_LEVEL
#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(dma_andes_atcdmac100);

#define ATCDMAC100_MAX_CHAN	8

#define DMA_ID_REV_REG(dev)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x00)

#define DMA_CFG_REG(dev)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x10)

#define DMA_CTRL_REG(dev)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x20)				

#define DMA_INT_STATUS(dev)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x30)

#define DMA_CH_ENABLE(dev)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x34)		

#define DMA_CH_OFFSET(ch)	(ch * 0x14)
#define DMA_ABORT(dev)				\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x40)
#define DMA_CH_CTRL(dev, ch)		\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x44 + DMA_CH_OFFSET(ch))
#define DMA_CH_SRC_ADDR_L(dev, ch)	\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x48 + DMA_CH_OFFSET(ch))
#define DMA_CH_DST_ADDR_L(dev, ch)	\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x4C + DMA_CH_OFFSET(ch))
#define DMA_CH_TRANSIZE(dev, ch)	\
		(((struct dma_atcdmac100_cfg *)dev->config)->base + 0x50 + DMA_CH_OFFSET(ch))
...
#define DMA_WIDTH_RESERVED	(3)

/* DMA Channel Control Register Definition */
#define DMA_CH_CTRL_SRCREQB5_MASK		BIT(31)
#define DMA_CH_CTRL_DSTREQB5_MASK		BIT(30)
#define DMA_CH_CTRL_PRIORITY_HIGH		BIT(29)
#define DMA_CH_CTRL_SBSIZE_MASK			GENMASK(24, 22)
#define DMA_CH_CTRL_SBSIZE(n)			FIELD_PREP(DMA_CH_CTRL_SBSIZE_MASK, (n))
#define DMA_CH_CTRL_SWIDTH_MASK			GENMASK(21, 20)
#define DMA_CH_CTRL_SWIDTH(n)			FIELD_PREP(DMA_CH_CTRL_SWIDTH_MASK, (n))
#define DMA_CH_CTRL_DWIDTH_MASK			GENMASK(19, 18)
...
struct dma_atcdmac100_data {
	struct dma_chan_data chan[ATCDMAC100_MAX_CHAN];
	struct k_spinlock lock;
};

/* Device constant configuration parameters */
struct dma_atcdmac100_cfg {
	atcdmac100_cfg_func_t irq_config;
	uint32_t base;
	uint32_t irq_num;
};

static struct __aligned(64)
	chain_block dma_chain[ATCDMAC100_MAX_CHAN][8];

static void dma_atcdmac100_isr(const struct device *dev)
{
	uint32_t int_status, int_ch_status, channel;
	struct dma_atcdmac100_data *const data = dev->data;
				 ...
				 struct dma_config *cfg)
{
	...
	/* dma_slot is effective the req/sel pair number for a particular IP (e.g. SPI, UART, I2C) */
...
static int dma_atcdmac100_reload(const struct device *dev, uint32_t channel,
...
static int dma_atcdmac100_transfer_start(const struct device *dev,
					 uint32_t channel)
{
	struct dma_atcdmac100_data *const data = dev->data;
...
static int dma_atcdmac100_transfer_stop(const struct device *dev,
					uint32_t channel)
{
	struct dma_atcdmac100_data *const data = dev->data;
	k_spinlock_key_t key;
...
static int dma_atcdmac100_init(const struct device *dev)
{
	const struct dma_atcdmac100_cfg *const config = (struct dma_atcdmac100_cfg *)dev->config;
	uint32_t ch_num;
...
static int dma_atcdmac100_get_status(const struct device *dev,
				     uint32_t channel,
...
static const struct dma_driver_api dma_atcdmac100_api = {
	.config = dma_atcdmac100_config,
	.reload = dma_atcdmac100_reload,
	.start = dma_atcdmac100_transfer_start,
	.stop = dma_atcdmac100_transfer_stop,
	.get_status = dma_atcdmac100_get_status
};

#define ATCDMAC100_INIT(n)						\
									\
	static void dma_atcdmac100_irq_config_##n(void);		\
									\
	static const struct dma_atcdmac100_cfg dma_config_##n = {	\
		.irq_config = dma_atcdmac100_irq_config_##n,		\
		.base = DT_INST_REG_ADDR(n),				\
		.irq_num = DT_INST_IRQN(n),				\
	};								\
									\
	static struct dma_atcdmac100_data dma_data_##n;			\
									\
...
DT_INST_FOREACH_STATUS_OKAY(ATCDMAC100_INIT)
