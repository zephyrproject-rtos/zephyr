					     ...
					     uint32_t block_size, LDMA_Descriptor_t *desc)
{
	...
	desc->xfer.size = LOG2(src_dsize);
	if (desc->xfer.size < ldmaCtrlSizeByte || desc->xfer.size > ldmaCtrlSizeWord) {
		return -ENOTSUP;
	}

	if (block_size % src_dsize) {
		desc->xfer.xferCnt = block_size / src_dsize;
	} else {
		desc->xfer.xferCnt = block_size / src_dsize - 1;
	}
					  ...
					  struct dma_block_config *block, LDMA_Descriptor_t *desc)
{
	...
	ret = dma_silabs_get_transfer_parameter(config->source_data_size, config->dest_data_size,
						block->block_size, desc);
	if (ret) {
		return ret;
	}
...
{
	LDMA_Descriptor_t *head_desc, *temp_desc;
	int ret;

	head_desc = desc;
	while (desc) {
		temp_desc = LDMA_DESCRIPTOR_LINKABS_LINKADDR_TO_ADDR(desc->xfer.linkAddr);
					   ...
					   struct dma_silabs_channel *chan_conf)
{
	...
	LDMA_Descriptor_t *desc, *temp_desc;
	int blockcount = 0;
	int ret;
				...
				struct dma_config *config)
{
	...
	chan_conf->complete_callback_en = 1;
