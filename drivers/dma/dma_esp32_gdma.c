...
#define DMA_IRQ_CONNECT(n, idx)                                                                    \
	IRQ_CONNECT(DT_INST_IRQN_BY_IDX(idx, n), IRQ_DEFAULT_PRIORITY, DMA_GET_HANDLER_NAME(n),     \
		    DEVICE_DT_INST_GET(idx), ESP_INTR_FLAG_IRAM);                                  \
	irq_enable(DT_INST_IRQN_BY_IDX(idx, n));
