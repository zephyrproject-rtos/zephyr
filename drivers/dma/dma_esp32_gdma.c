...
static int dma_esp32_config_descriptor(struct dma_esp32_channel *dma_channel,
					struct dma_block_config *block, bool tx_dir)
{
	if (!block) {
	...
	for (int i = 0; i < CONFIG_DMA_ESP32_MAX_DESCRIPTOR_NUM; ++i) {
		...
		if (block_size > DMA_DESCRIPTOR_BUFFER_MAX_SIZE) {
			buffer_size = DMA_DESCRIPTOR_BUFFER_MAX_SIZE;
		} else {
				...
				struct dma_config *config_dma)
{
	...
	return dma_esp32_config_descriptor(dma_channel, config_dma->head_block, true);
}
