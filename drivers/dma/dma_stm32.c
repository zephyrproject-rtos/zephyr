			       ...
			       struct dma_config *config)
{
	...
	source_periph = stream->direction == MEMORY_TO_PERIPHERAL;
	ret = dma_stm32_width_config(config, source_periph, dma,
				     &DMA_InitStruct, id);
	if (ret < 0) {
		return ret;
	}
	...
	DMA_InitStruct.MemBurst = stm32_dma_get_mburst(config, source_periph);
...
const struct dma_stm32_config dma_stm32_config_##index = {		\
	...
	.support_m2m = DT_DMA_##index##_ST_MEM2MEM,			\
};									\
									\
