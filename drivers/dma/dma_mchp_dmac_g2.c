...
static void dma_mchp_isr_shared(const struct device *dev)
{
	...
	/* Ignore non TERR/TCMPL interrupts */
	if ((int_pend & (DMAC_INTPEND_TERR_Msk | DMAC_INTPEND_TCMPL_Msk)) == 0) {
		return;
	}

	/* Convert INTPEND flags to CHINTFLAG format for common handler */
	ch_int_flag = 0;
	if (int_pend & DMAC_INTPEND_TERR_Msk) {
		ch_int_flag |= DMAC_CHINTFLAG_TERR_Msk;
	}
	if (int_pend & DMAC_INTPEND_TCMPL_Msk) {
		ch_int_flag |= DMAC_CHINTFLAG_TCMPL_Msk;
	}
