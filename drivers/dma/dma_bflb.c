...
#define BFLB_DMA_CH_NB 8
#elif defined(CONFIG_SOC_SERIES_BL61X) || defined(CONFIG_SOC_SERIES_BL60X)
#define BFLB_DMA_CH_NB 4
#else
...
static size_t dma_bflb_get_transfer_size(const struct device *dev, uint32_t channel)
{
	...
	switch ((control & DMA_SWIDTH_MASK) >> DMA_SWIDTH_SHIFT) {
			   ...
			   struct dma_config *config)
{
	...
	ch_config = sys_read32(cfg->base_reg + DMA_CxCONFIG_OFFSET + BFLB_DMA_CH_OFFSET(channel));
	...
	ch_config |= (block->source_address << DMA_SRCPERIPHERAL_SHIFT) & DMA_SRCPERIPHERAL_MASK;
	ch_config |= (block->dest_address << DMA_DSTPERIPHERAL_SHIFT) & DMA_DSTPERIPHERAL_MASK;
	...
	sys_write32(ch_config, cfg->base_reg + DMA_CxCONFIG_OFFSET + BFLB_DMA_CH_OFFSET(channel));
...
#ifdef CONFIG_SOC_SERIES_BL61X
	/* on BL61x, we must clean the cache to update the peripheral memory data */
	__asm__ volatile (
		"fence\n"
		/* th.dcache.ciall*/
		".insn 0x30000B\n"
			       ...
			       struct dma_status *stat)
{
	...
	stat->pending_length = control & DMA_TRANSFERSIZE_MASK;
	...
	switch (((config & DMA_FLOWCNTRL_MASK) >> DMA_FLOWCNTRL_SHIFT) & BFLB_DMA_FLOW_P_P) {
