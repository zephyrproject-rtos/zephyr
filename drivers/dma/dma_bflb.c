...
#ifdef CONFIG_SOC_SERIES_BL61X
/* Cache lines are 32-bytes long */
#define BL61X_CACHE_LINE_MASK 0xFFFFFFE0
#define BL61X_CACHE_LINE_SIZE 32

static void dma_bflb_invalidate_cache_line(uintptr_t address_in)
{
	register uintptr_t address __asm__("a3") = address_in & BL61X_CACHE_LINE_MASK;
...
static int dma_bflb_start(const struct device *dev, uint32_t channel)
{
	...
	for (uintptr_t i = addr; i < addr + pending_length; i += BL61X_CACHE_LINE_SIZE) {
		dma_bflb_invalidate_cache_line(i);
	}
#endif
