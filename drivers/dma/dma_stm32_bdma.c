						   ...
						   DMA_InitTypeDef *hal_config)
{
	...
	switch (cfg->channel_direction) {
	case MEMORY_TO_PERIPHERAL:
		mem_adj = block->source_addr_adj;
		periph_adj = block->dest_addr_adj;
		break;
	case PERIPHERAL_TO_MEMORY:
		mem_adj = block->dest_addr_adj;
		periph_adj = block->source_addr_adj;
		break;
	case MEMORY_TO_MEMORY:
		mem_adj = block->dest_addr_adj;
		periph_adj = block->source_addr_adj;
	...
	ret = bdma_stm32_hal_map_addr_adj(periph_adj,
	...
	return bdma_stm32_hal_map_addr_adj(mem_adj,
					   &hal_config->MemInc,
					   DMA_MINC_ENABLE,
					   DMA_MINC_DISABLE);
}
