...
static struct __aligned(64) chain_block
	dma_chain[ATCDMAC100_MAX_CHAN][sizeof(struct chain_block) * 16];
...
static int dma_atcdmac300_config(const struct device *dev, uint32_t channel, struct dma_config *cfg)
{
	struct dma_atcdmac300_data *const data = dev->data;
	...
	ch_ctrl |= DMA_CH_CTRL_SWIDTH(src_width) | DMA_CH_CTRL_DWIDTH(dst_width) |
	...
	sys_write32(cfg_blocks->source_address, DMA_CH_SRC_ADDR_L(dev, channel));
	sys_write32(0, DMA_CH_SRC_ADDR_H(dev, channel));
	sys_write32(cfg_blocks->dest_address, DMA_CH_DST_ADDR_L(dev, channel));
	...
	if (cfg->dest_chaining_en == 1 && cfg_blocks->next_block) {
		...
		for (cfg_blocks = cfg_blocks->next_block; cfg_blocks != NULL;
			...
			ch_ctrl &= ~(DMA_CH_CTRL_SRCADDRCTRL_MASK | DMA_CH_CTRL_DSTADDRCTRL_MASK);
...
static int dma_atcdmac300_reload(const struct device *dev, uint32_t channel, uint32_t src,
...
static int dma_atcdmac300_transfer_start(const struct device *dev, uint32_t channel)
{
	struct dma_atcdmac300_data *const data = dev->data;
...
static int dma_atcdmac300_transfer_stop(const struct device *dev, uint32_t channel)
{
	struct dma_atcdmac300_data *const data = dev->data;
	k_spinlock_key_t key;
...
static int dma_atcdmac300_get_status(const struct device *dev, uint32_t channel,
...
static const struct dma_driver_api dma_atcdmac300_api = {.config = dma_atcdmac300_config,
