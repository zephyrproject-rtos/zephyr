...
static void rza2m_dma_set_dmars(const struct device *dev, int ch, uint32_t value)
{
	...
	/* DMARSn/nS is shared by channel pair */
...
static void rza2m_dma_set_dctrl(const struct device *dev, int ch, uint32_t value)
{
	...
	/* DCTRL is shared by the whole 0-7 or 8-15 channel group */
	key = k_spin_lock(&data->lock);
...
static int dma_rza2m_validate_config(struct dma_config *dma_cfg, uint32_t ch)
{
	...
	uint16_t dmars = RZA2M_DMA_SLOT_DMARS(dma_cfg->dma_slot);

	if ((dmars == RZA2M_DMA_DMARS_EXT_DREQ0) && (ch != 0)) {
		LOG_ERR("External request (DREQ0) is only supported on channel 0");
		return -EINVAL;
	}

	if ((dma_cfg->channel_direction != MEMORY_TO_MEMORY) && (dmars == 0U)) {
...
static int dma_rza2m_config(const struct device *dev, uint32_t channel, struct dma_config *dma_cfg)
{
	...
	ret = dma_rza2m_validate_config(dma_cfg, channel);
	if (ret < 0) {
		return ret;
	}
...
static int dma_rza2m_resume(const struct device *dev, uint32_t ch)
{
	...
	if (!rza2m_dma_get_ch_sus(dev, ch)) {
		return -EINVAL;
	}
