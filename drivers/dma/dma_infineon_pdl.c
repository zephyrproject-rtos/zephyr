...
struct ifx_cat1_dma_channel {
	 ...
	 * suspended (see ifx_cat1_dma_trig()). resume() re-issues only a trigger
...
int ifx_cat1_dma_trig(const struct device *dev, uint32_t channel)
{
	 ...
	 * meant to be paused. Record that a trigger was deferred so resume()
	 * knows there is a withheld segment to re-arm.
	 */
...
static int ifx_cat1_dma_resume(const struct device *dev, uint32_t channel)
{
	 ...
	 * suspend (nothing was deferred), the transfer is already finished:
	 * re-enabling and re-triggering would issue a software trigger with no
	 * valid current descriptor and raise a CURR_PTR_NULL error.
	 */
	 ...
	 * re-issue the withheld software trigger so the transfer continues,
