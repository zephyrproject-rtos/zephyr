 ...
 *
 *   - SDRAM appears at a VC-bus alias that differs by SoC, so each SoC
 *     dts states its own in brcm,sdram-bus-alias. Matches the Linux
 *     `dma-ranges` in each SoC's dtsi:
 ...
 */
#define DMA_BUS_ALIAS   ((uint32_t)DT_INST_PROP(0, brcm_sdram_bus_alias))
