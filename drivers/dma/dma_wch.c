...
static int dma_wch_config(const struct device *dev, uint32_t ch, struct dma_config *dma_cfg)
{
	...
	if (dma_cfg->channel_direction == MEMORY_TO_PERIPHERAL) {
		cfgr |= dma_width_index(dma_cfg->source_data_size) * DMA_CFGR1_MSIZE_0;
		cfgr |= dma_width_index(dma_cfg->dest_data_size) * DMA_CFGR1_PSIZE_0;
		...
		cfgr |= dma_width_index(dma_cfg->source_data_size) * DMA_CFGR1_PSIZE_0;
		cfgr |= dma_width_index(dma_cfg->dest_data_size) * DMA_CFGR1_MSIZE_0;
