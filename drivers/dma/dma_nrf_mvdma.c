...
static struct nrf_mvdma_ctrl *curr_ctrl;
static NRF_MVDMA_Type *reg = (NRF_MVDMA_Type *)DT_REG_ADDR(MVDMA_NODE());

static uint32_t dummy_jobs[] __cacheline_aligned = {
...
static int xfer(struct nrf_mvdma_ctrl *ctrl, uint32_t src, uint32_t sink, bool queue)
{
	int rv, key;
	bool int_en = true;
	...
	if (nrf_mvdma_activity_check(reg) || (curr_ctrl && curr_ctrl->handler)) {
		...
		if (curr_ctrl != NULL) {
			sys_snode_t *node;
			struct nrf_mvdma_ctrl *prev;

			curr_ctrl->handler = (nrf_mvdma_handler_t)1;
			while ((node = sys_slist_get(&list)) != NULL) {
				prev = CONTAINER_OF(node, struct nrf_mvdma_ctrl, node);
				prev->handler = (nrf_mvdma_handler_t)1;
			}
		}
...
int nrf_mvdma_basic_xfer(struct nrf_mvdma_ctrl *ctrl, struct nrf_mvdma_basic_desc *desc, bool queue)
{
...
int nrf_mvdma_xfer_check(const struct nrf_mvdma_ctrl *ctrl)
{
	...
	if (!nrf_mvdma_activity_check(reg)) {
		curr_ctrl = NULL;
...
{
	int key;
	struct nrf_mvdma_ctrl *ctrl = curr_ctrl;
	sys_snode_t *node;
	bool int_dis = true;
	...
	if (node) {
		struct nrf_mvdma_ctrl *next = CONTAINER_OF(node, struct nrf_mvdma_ctrl, node);
	...
	if (ctrl->handler) {
		...
		ctrl->handler = (nrf_mvdma_handler_t)1;
	}
}
...
SYS_INIT(nrf_mvdma_init, PRE_KERNEL_1, CONFIG_DMA_INIT_PRIORITY);
