...
static void dma_sf32lb_isr(const struct device *dev)
{
	...
	for (uint8_t channel = 0U; channel < config->channels; channel++) {
		int status;

		if ((isr & DMAC_IFCR_CTCIF(channel)) != 0U) {
			status = DMA_STATUS_COMPLETE;
		} else if ((isr & DMAC_IFCR_CTEIF(channel)) != 0U) {
			status = -EIO;
		} else {
			continue;
		}

		data->channels[channel].callback(dev, data->channels[channel].user_data, channel,
						 status);
	}

	sys_write32(isr, config->dmac + DMAC_ISR);
}
			     ...
			     struct dma_config *config_dma)
{
	...
	if (config_dma->head_block->block_size > DMAC_MAX_LEN) {
		LOG_ERR("Block size exceeds maximum");
		return -EINVAL;
	}

	if (config_dma->dma_slot >= config->requests) {
		LOG_ERR("Invalid DMA slot");
		return -EINVAL;
	}
	...
	if (channel < 4U) {
		...
		cselrx |= FIELD_PREP(DMAC_CSELR1_C1S_Msk << (channel * 8U), config_dma->dma_slot);
	...
	data->channels[channel].callback = config_dma->dma_callback;
	data->channels[channel].user_data = config_dma->user_data;
	data->channels[channel].direction = config_dma->channel_direction;
			     ...
			     size_t size)
{
	...
	switch (data->channels[channel].direction) {
	case MEMORY_TO_MEMORY:
	case PERIPHERAL_TO_MEMORY:
		cparx = src;
		cm0arx = dst;
...
{
	const struct dma_sf32lb_config *cfg = dev->config;
	...
	for (uint8_t channel = 0U; channel < cfg->channels; channel++) {
		uint32_t ccrx;

		ccrx = sys_read32(cfg->dmac + DMAC_CCRX(channel));
		ccrx &= ~(DMAC_CCR1_EN | DMAC_CCR1_TCIE | DMAC_CCR1_HTIE | DMAC_CCR1_TEIE);
		sys_write32(ccrx, cfg->dmac + DMAC_CCRX(channel));
	}
...
#define DMA_SF32LB_IRQ_CONFIGURE(n, inst)                                                          \
	IRQ_CONNECT(DT_INST_IRQ_BY_IDX(inst, n, irq), DT_INST_IRQ_BY_IDX(inst, n, priority),       \
		    dma_sf32lb_isr, DEVICE_DT_INST_GET(inst), 0);                                  \
