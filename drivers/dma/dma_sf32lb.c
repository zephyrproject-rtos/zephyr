...
printk("dma_sf32lb_isr");
	...
	sys_clear_bit(config->dmac + DMAC_CCRX(channel), DMAC_CCR1_EN_Pos);
			     ...
			     struct dma_config *config_dma)
{
	...
	ccrx &= ~(DMAC_CCR1_TCIE | DMAC_CCR1_HTIE | DMAC_CCR1_TEIE | DMAC_CCR1_DIR_Msk | DMAC_CCR1_CIRC_Msk | DMAC_CCR1_EN_Msk |
				 ...
				 struct dma_status *stat)
{
	...
	stat->busy = atomic_test_bit(data->status, channel);//printk("stat->busy[%d]:%d\n", channel, stat->busy);
