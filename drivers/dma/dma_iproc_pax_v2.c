			      ...
			      struct dma_iproc_pax_ring_data *ring)
{
	...
	if (!timeout) {
		LOG_ERROR("[ring %d]: not recvd write sync!\n", ring->idx);
		ret = -ETIMEDOUT;
	}
				     ...
				     uint32_t *non_hdr_bd_count)
{
	...
	case PERIPHERAL_TO_MEMORY:
		...
		break;
	default:
		LOG_ERROR("not supported transfer direction");
		return -EINVAL;
	}
				   ...
				   struct dma_config *cfg)
{
	...
	if (cfg->block_count >= RM_V2_MAX_BLOCK_COUNT) {
		LOG_ERROR("Dma block count[%d] supported exceeds limit[%d]\n", cfg->block_count,
