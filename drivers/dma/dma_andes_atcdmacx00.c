...
static struct chain_block __aligned(64)
	...
	DT_HAS_COMPAT_STATUS_OKAY(andestech_dlm))

#if (CONFIG_MP_MAX_NUM_CPUS > 4)
#error "DMA only support local memory with CPU less than four cores."
#endif

#define MEM_VIA_LMACCESSPORT(addr)			(mem_via_lmaccessport(addr))

#if (DT_HAS_COMPAT_STATUS_OKAY(andestech_ilm))
const uint32_t ilm_lmap[5][2] = {
	{DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_ilm), 0),
	 DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_ilm), 0)},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 1),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_ilm), 1)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 1),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_ilm), 1)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 2),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_ilm), 2)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 2),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_ilm), 2)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 3),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_ilm), 3)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 3),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_ilm), 3)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 4),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_ilm), 4)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_ilm), 4),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_ilm), 4)), (0))}};
#endif

#if (DT_HAS_COMPAT_STATUS_OKAY(andestech_dlm))
const uint32_t dlm_lmap[5][2] = {
	{DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_dlm), 0),
	 DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_dlm), 0)},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 1),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_dlm), 1)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 1),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_dlm), 1)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 2),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_dlm), 2)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 2),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_dlm), 2)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 3),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_dlm), 3)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 3),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_dlm), 3)), (0))},
	{COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 4),
			(DT_REG_ADDR_BY_IDX(DT_INST(0, andestech_dlm), 4)), (0)),
	 COND_CODE_1(DT_REG_HAS_IDX(DT_INST(0, andestech_dlm), 4),
			(DT_REG_SIZE_BY_IDX(DT_INST(0, andestech_dlm), 4)), (0))}};
#endif

uint32_t mem_via_lmaccessport(uint32_t address)
{
...
#if (DT_HAS_COMPAT_STATUS_OKAY(andestech_dlm))
	if (IS_ENABLED(CONFIG_SOC_ANDES_V5_LM_SLVP)) {
		if ((addr >= dlm_lmap[0][0]) && (addr < (dlm_lmap[0][0] + dlm_lmap[0][1]))) {
			if (dlm_lmap[arch_proc_id() + 1][0] == 0) {
				LOG_ERR("Local memory access port doesn't support this core");
			} else {
				offset = dlm_lmap[arch_proc_id() + 1][0] - dlm_lmap[0][0];
			}
		}
	} else {
		LOG_ERR("Doesn't support local memory without local memory access port");
	}
#endif
#if (DT_HAS_COMPAT_STATUS_OKAY(andestech_ilm))
	if (IS_ENABLED(CONFIG_SOC_ANDES_V5_LM_SLVP)) {
		if ((addr >= ilm_lmap[0][0]) && (addr < (ilm_lmap[0][0] + ilm_lmap[0][1]))) {
			if (ilm_lmap[arch_proc_id() + 1][0] == 0) {
				LOG_ERR("Local memory access port doesn't support this core");
			} else {
				offset = ilm_lmap[arch_proc_id() + 1][0] - ilm_lmap[0][0];
			}
		}
	} else {
		LOG_ERR("Doesn't support local memory without local memory access port");
...
#else
#define MEM_VIA_LMACCESSPORT(addr)			(addr)
#endif
				 ...
				 struct dma_config *cfg)
{
	...
	sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK(31, 0),
	...
	if (data->version == ATCDMAC300_VERSION) {
		sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK64(63, 32),
							(cfg_blocks->source_address))),
...
#if (DT_HAS_COMPAT_STATUS_OKAY(andestech_ilm) ||	\
	DT_HAS_COMPAT_STATUS_OKAY(andestech_dlm))
		LOG_ERR("DMA chain transfer doesn't support local mamory.");
		ret = -EINVAL;
		goto end;
#endif
...
#endif
{
	...
	/* Set source and destination address */
	sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK(31, 0), (src))),
							DMA_CH_SRC_ADDR(dev, channel));
	sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK(31, 0), (dst))),
							DMA_CH_DST_ADDR(dev, channel));

	if (data->version == ATCDMAC300_VERSION) {
		sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK64(63, 32), (src))),
								DMA_CH_SRC_ADDR_H(dev, channel));
		sys_write32(MEM_VIA_LMACCESSPORT((uint32_t)FIELD_GET(GENMASK64(63, 32), (dst))),
								DMA_CH_DST_ADDR_H(dev, channel));
	}
