...
struct infineon_dmac_channel {
	...
	struct dma_block_config *block_chain;
...
static int infineon_dmac_cfg_desc(const struct device *dev, uint32_t channel,
				  ...
				  const struct dma_config *cfg, bool is_last_block,
				  enum dma_channel_direction direction)
{
	const struct infineon_dmac_config *config = dev->config;
...
static int infineon_dmac_config(const struct device *dev, uint32_t channel, struct dma_config *cfg)
{
	...
	ch->block_chain = cfg->head_block;
	ch->total_blocks = block_count;
	ch->blocks_transferred = 0;

	/* Setup descriptors in a loop */
	struct dma_block_config *block = cfg->head_block;
	bool use_ping = true; /* Start with PING descriptor */
	uint32_t descs_to_setup = MIN(block_count, MAX_DESC_AVAIL);

	for (uint32_t i = 0; i < descs_to_setup && block != NULL; i++) {
		cy_en_dmac_descriptor_t desc_type;
		bool is_last_block = (i + 1 == block_count);

		if (use_ping) {
			desc_type = CY_DMAC_DESCRIPTOR_PING;
		} else {
			desc_type = CY_DMAC_DESCRIPTOR_PONG;
		}

		ret = infineon_dmac_cfg_desc(dev, channel, desc_type, block, cfg, is_last_block,
					     ch->direction);
		if (ret != 0) {
			infineon_dmac_cleanup_channel(dev, channel);
			return ret;
...
static void infineon_dmac_isr(const struct device *dev, uint32_t channel)
{
	...
	is_last_block = ((ch->blocks_transferred + 1) == ch->total_blocks);
	...
	if (ch->total_blocks >= 2 && ch->blocks_transferred < ch->total_blocks) {
		...
		struct dma_block_config *next_block = ch->block_chain;

		for (uint32_t block = 0; (block < ch->blocks_transferred) && (next_block != NULL);
		     block++) {
			next_block = next_block->next_block;
		}

		if (next_block != NULL) {
			/* Determine which descriptor to reconfigure */
			...
			ret = infineon_dmac_cfg_desc(dev, channel, reconfigure_desc, next_block,
						     ch->config, is_last_block, ch->direction);
	...
	if (ch->blocks_transferred < ch->total_blocks) {
		if (ch->complete_callback_en) {
			callback(dev, ch->user_data, channel, 0);
		}
		return;
	}
