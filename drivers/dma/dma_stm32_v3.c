...
static uint32_t dma_stm32_id_to_channel(uint32_t id)
{
...
{
	mem_addr_t dma_channel_ccr_reg =
		(mem_addr_t) &
		((DMA_Channel_TypeDef *)((uint32_t)dma + LL_DMA_CH_OFFSET_TAB[channel]))->CCR;
	sys_clear_bits(dma_channel_ccr_reg, DMA_CCR_TCIE | DMA_CCR_HTIE | DMA_CCR_USEIE |
						    DMA_CCR_ULEIE | DMA_CCR_DTEIE | DMA_CCR_SUSPIE);
}

static void dma_stm32_enable_it(const DMA_TypeDef *dma, uint32_t channel)
{
	mem_addr_t dma_channel_ccr_reg =
		(mem_addr_t) &
		((DMA_Channel_TypeDef *)((uint32_t)dma + LL_DMA_CH_OFFSET_TAB[channel]))->CCR;
...
static void dma_stm32_dump_channel_irq(const DMA_TypeDef *dma, uint32_t channel)
{
...
static void dma_stm32_clear_channel_irq(const DMA_TypeDef *dma, uint32_t channel)
{
	mem_addr_t dma_channel_cfcr_reg =
		(mem_addr_t) &
		((DMA_Channel_TypeDef *)((uint32_t)dma + LL_DMA_CH_OFFSET_TAB[channel]))->CFCR;
	sys_set_bits(dma_channel_cfcr_reg, DMA_CFCR_TCF | DMA_CFCR_HTF | DMA_CFCR_DTEF |
						   DMA_CFCR_ULEF | DMA_CFCR_USEF | DMA_CFCR_TOF |
						   DMA_CFCR_SUSPF);
}

static int dma_stm32_disable_channel(const DMA_TypeDef *dma, uint32_t channel)
{
