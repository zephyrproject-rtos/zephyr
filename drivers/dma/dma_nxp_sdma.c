...
AT_NONCACHEABLE_SECTION_ALIGN(static sdma_context_data_t sdma_contexts[FSL_FEATURE_SDMA_MODULE_CHANNEL], 4);
...
struct sdma_dev_data {
	...
	sdma_buffer_descriptor_t bd_pool[FSL_FEATURE_SDMA_MODULE_CHANNEL][DMA_NXP_SDMA_BD_COUNT] __aligned(64);
};

static void dma_nxp_sdma_init_stat(struct sdma_channel_data *chan_data)
{
...
static void dma_nxp_sdma_isr(const void *data)
{
	...
	while (val) {
		if ((val & 0x1) != 0) {
			...
			SDMA_HandleIRQ(&dev_data->chan[i].handle);
		}
		i++;
		val >>= 1;
	}
	...
	switch (config->channel_direction) {
		...
		case PERIPHERAL_TO_PERIPHERAL:
			*type = kSDMA_PeripheralToPeripheral;
			break;
		default:
			LOG_ERR("channel direction not supported %d", config->channel_direction);
			return;
	}
	LOG_DBG("sdma_set_transfer_type() dir %d type = %d", config->channel_direction, *type);
}

void sdma_set_peripheral_type(struct dma_config *config, sdma_peripheral_t *type)
{
	/* only support normal peripheral */
			       ...
			       struct dma_config *config)
{
	...
	dma_nxp_sdma_init_stat(chan_data);
...
static int dma_nxp_sdma_start(const struct device *dev, uint32_t channel)
{
	...
	if (channel >= FSL_FEATURE_SDMA_MODULE_CHANNEL) {
		LOG_ERR("dma_nxp_sdma_start() invalid channel %d", channel);
		return -EINVAL;
	}
...
static int dma_nxp_sdma_stop(const struct device *dev, uint32_t channel)
{
	...
	if (channel >= FSL_FEATURE_SDMA_MODULE_CHANNEL) {
		LOG_ERR("dma_nxp_sdma_stop() invalid channel %d", channel);
		return -EINVAL;
	}
