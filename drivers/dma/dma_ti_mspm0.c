...
static inline int dma_ti_mspm0_get_dmasz(uint32_t size, uint8_t source_data_size,
					 uint8_t dest_data_size, uint8_t direction)
{
	if (direction == PERIPHERAL_TO_MEMORY) {
		return size / source_data_size;
	}
	return size / dest_data_size;
}
				  ...
				  struct dma_config *config)
{
	...
	K_SPINLOCK(&dma_data->lock) {
		...
		cfg->regs->dmachan[channel].dmasz =
			dma_ti_mspm0_get_dmasz(b_cfg->block_size, config->source_data_size,
					       config->dest_data_size, config->channel_direction);
			       ...
			       uint32_t src_addr, uint32_t dest_addr, size_t size)
{
	...
	cfg->regs->dmachan[channel].dmasz = dma_ti_mspm0_get_dmasz(
		size, data->source_data_size, data->dest_data_size, data->direction);
						 ...
						 uint32_t channel)
{
	...
	cfg->regs->dmachan[channel].dmasz = dma_ti_mspm0_get_dmasz(
		block->block_size, data->source_data_size, data->dest_data_size, data->direction);
	cfg->regs->dmachan[channel].dmactl |= DMA_MSPM0_CTL_DMAEN;
