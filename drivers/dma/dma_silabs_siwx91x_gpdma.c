				     ...
				     const RSI_GPDMA_DESC_T *xfer_cfg, uint32_t channel)
{
	...
	for (int i = 0; i < config->block_count; i++) {
		...
		ret = sys_mem_blocks_alloc_contiguous(data->desc_pool, 1, (void **)&cur_desc);
...
free_desc:
	...
	sys_mem_blocks_free_contiguous(data->desc_pool, (void *)data->chan_info[channel].desc,
				       block_alloc_count);
...
static void siwx91x_gpdma_isr(const struct device *dev)
{
	...
	if (channel_int_status & done_mask) {
		key = k_spin_lock(&data->desc_pool_lock);
		sys_mem_blocks_free_contiguous(data->desc_pool,
