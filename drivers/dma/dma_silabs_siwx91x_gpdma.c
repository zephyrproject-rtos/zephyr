...
static void siwx91x_gpdma_free_desc(sys_mem_blocks_t *mem_block, void *block)
{
	RSI_GPDMA_DESC_T *cur_desc = (RSI_GPDMA_DESC_T *)block;
...
free_desc:
	key = k_spin_lock(&data->desc_pool_lock);
	siwx91x_gpdma_free_desc(data->desc_pool, (void *)data->chan_info[channel].desc);
	k_spin_unlock(&data->desc_pool_lock, key);
	return -EINVAL;
}
...
static void siwx91x_gpdma_isr(const struct device *dev)
{
	...
	if (channel_int_status & done_mask) {
		key = k_spin_lock(&data->desc_pool_lock);
		siwx91x_gpdma_free_desc(data->desc_pool,
					       (void *)data->chan_info[channel].desc);
		k_spin_unlock(&data->desc_pool_lock, key);
