...
static int dma_mchp_validate(const struct device *dev, uint32_t channel, struct dma_config *config)
{
	...
	if (channel >= dev_data->dma_ctx.dma_channels) {
		...
		ret = -EINVAL;
	} else if (dmac_ch_get_state(DMAC_REG, channel) == DMA_MCHP_CH_ACTIVE) {
		LOG_ERR("DMA channel %d is already in use", channel);
		ret = -EBUSY;
	} else if (config->source_data_size != config->dest_data_size) {
...
static int dma_mchp_start(const struct device *dev, uint32_t channel)
{
	...
	else if (dev_data->dma_channel_config[channel].is_configured != true) {
			   ...
			   size_t size)
{
	...
	/* Check if the channel is configured */
		...
		if (ret < 0) {
			LOG_DBG("Reload channel %d for %08X to %08X (%u) failed!", channel, src,
				dst, size);
		} else {
