...
LOG_MODULE_REGISTER(hwinfo_nxp_rstctl, CONFIG_HWINFO_LOG_LEVEL);

/* The reset source bits are defined in the rstctl_reset_source_t enum. */
#define NXP_RESET_POR_FLAG      RSTCTL3_SYSRSTSTAT_VDD_POR_MASK
#define NXP_RESET_PIN_FLAG      RSTCTL3_SYSRSTSTAT_RESETN_RESET_MASK
#define NXP_RESET_DEBUG_FLAG    RSTCTL3_SYSRSTSTAT_ISP_AP_RESET_MASK
#define NXP_RESET_SOFTWARE_FLAG RSTCTL3_SYSRSTSTAT_ITRC_SW_RESET_MASK
#define NXP_RESET_CPU_FLAG                                                         \
 ...
 *
 * Translate bitmask from NXP RSTCTL reset source bitmask to Zephyr
 * hwinfo reset source bitmask.
 *
 * @param NXP RSTCTL reset source mask.
 * @retval Zephyr hwinfo reset source mask.
 */
static uint32_t hwinfo_nxp_rstctl_xlate_reset_sources(uint32_t sources)
{
	uint32_t mask = 0;

	if (sources & NXP_RESET_POR_FLAG) {
		mask |= RESET_POR;
	}

	if (sources & NXP_RESET_PIN_FLAG) {
		mask |= RESET_PIN;
	}

	if (sources & NXP_RESET_DEBUG_FLAG) {
		mask |= RESET_DEBUG;
	}

	if (sources & NXP_RESET_SOFTWARE_FLAG) {
		mask |= RESET_SOFTWARE;
	}

	if (sources & NXP_RESET_CPU_FLAG) {
		mask |= RESET_SOFTWARE;
	}

	if (sources & NXP_RESET_WATCHDOG_FLAG) {
		mask |= RESET_WATCHDOG;
	}
...
int z_impl_hwinfo_get_reset_cause(uint32_t *cause)
{
	...
	*cause = hwinfo_nxp_rstctl_xlate_reset_sources(sources);
