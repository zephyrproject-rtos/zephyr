...
struct ufsdisk_data {
	...
	uint8_t card_buffer[CONFIG_UFS_BUFFER_SIZE]
		__aligned(CONFIG_UFSHC_BUFFER_ALIGNMENT);
					   /**< Static buffer for unaligned I/O operations. */
	enum ufsdisk_status status;        /**< Current status of the UFS disk. */
};

/**
 * @brief Configuration structure for a UFS disk.
 */
struct ufsdisk_config {
	const struct device *host_controller; /**< UFS Host Controller device. */
	const char *name;                     /**< Disk name for registration. */
...
static int32_t disk_ufs_access_init(struct disk_info *disk)
{
	...
	const struct ufsdisk_config *cfg = dev->config;
	struct ufsdisk_data *data = dev->data;
	int32_t err;
	...
	if ((data->ufshc != NULL) && (data->ufshc->is_initialized)) {
		...
		err = ufs_init(cfg->host_controller, &data->ufshc);
		if (err != 0) {
	...
	data->sdev = scsi_device_lookup_by_host(data->ufshc->host, cfg->lun);
	if (data->sdev == NULL) {
		LOG_ERR("SCSI device for lun:%d is NULL", cfg->lun);
		err = -ENOTSUP;
		goto out;
	}
	...
	if (err != 0) {
		LOG_ERR("Failed to execute TUR, lun:%d", cfg->lun);
		goto out;
	}
				    ...
				    uint32_t num_blocks)
{
	...
	const struct device *dev = disk->dev;
	struct ufsdisk_data *disk_data = dev->data;
...
static int32_t disk_ufs_access_ioctl(struct disk_info *disk, uint8_t cmd, void *buff)
{
	...
	case DISK_IOCTL_CTRL_DEINIT:
		...
		/* mark the disk as uninitialized */
...
/**
 * @brief Initialize UFS disk device.
 *
 * This function initializes a UFS disk device and registers it
 * with the disk access subsystem.
 *
 * @param dev Pointer to the device structure.
 *
 * @return 0 on success, negative error code on failure.
 */
static int disk_ufs_init(const struct device *dev)
{
	const struct ufsdisk_config *config = dev->config;
	struct ufsdisk_data *data = dev->data;

	data->status = UFS_UNINIT;
	data->info.name = config->name;
	data->info.ops = &ufs_disk_ops;
	data->info.dev = dev;

	return disk_access_register(&data->info);
}

#define DISK_ACCESS_UFS_INIT(n)						\
	static const struct ufsdisk_config ufsdisk_config_##n = {	\
		.host_controller = DEVICE_DT_GET(DT_INST_PARENT(n)),	\
		.name = DT_INST_PROP(n, disk_name),			\
		.lun = DT_INST_PROP(n, lun),				\
	};								\
	static struct ufsdisk_data ufsdisk_data_##n;			\
	DEVICE_DT_INST_DEFINE(n, &disk_ufs_init, NULL,			\
		&ufsdisk_data_##n, &ufsdisk_config_##n,			\
		POST_KERNEL, CONFIG_UFSDISK_INIT_PRIORITY, NULL);

DT_INST_FOREACH_STATUS_OKAY(DISK_ACCESS_UFS_INIT)
