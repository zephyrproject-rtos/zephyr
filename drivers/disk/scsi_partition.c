...
{
	uint8_t sector[SCSI_PART_MAX_SECTOR_SIZE];
	uint32_t base = 0U;
	uint32_t parts = 0U;
	uint32_t block_size;
	uint64_t disk_sectors;
	...
	ret = scsi_io_read(sdev, 0U, 1U, sector, block_size);
	if (ret != 0) {
		return ret;
	}

	if (scsi_part_has_fat_bpb(sector, block_size)) {
		*lba_offset = 0U;
	...
	ret = scsi_part_mbr_fat_partition(sector, &base, &parts);
	if (ret == 0) {
		ret = scsi_io_read(sdev, base, 1U, sector, block_size);
		if (ret != 0) {
			return ret;
		}
		if (!scsi_part_has_fat_bpb(sector, block_size)) {
