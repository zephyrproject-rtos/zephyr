...
{
	return nvme_namespace_get_num_sectors(ns) * nvme_namespace_get_sector_size(ns);
}
...
const struct nvme_namespace_data *nvme_namespace_get_data(struct nvme_namespace *ns)
{
	return &ns->data;
}
...
int nvme_namespace_construct(struct nvme_namespace *ns, uint32_t id, struct nvme_controller *ctrlr)
{
	...
	nvme_ctrlr_cmd_identify_namespace(ctrlr, id, &ns->data, nvme_completion_poll_cb, &status);
	...
	if (flbas_fmt > ns->data.nlbaf) {
		LOG_DBG("NS id %d: lba format %d exceeds number supported (%d)", id, flbas_fmt,
			ns->data.nlbaf + 1);
		return -EIO;
	}
	...
	snprintf(ns->name, NVME_NAMESPACE_NAME_MAX_LENGTH, "nvme%dn%d", ctrlr->id, ns->id - 1);
