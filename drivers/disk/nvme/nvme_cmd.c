...
void nvme_completion_print(const struct nvme_completion *cpl)
{
	...
	LOG_DBG("%s (%02x/%02x) crd:%x m:%x dnr:%x p:%d "
		"sqid:%d cid:%d cdw0:%x\n",
		get_status_string(sct, sc), sct, sc, crd, m, dnr, p, cpl->sqid, cpl->cid,
		cpl->cdw0);
}
...
static void nvme_cmd_register_request(struct nvme_request *request)
{
	...
	if (!k_work_delayable_remaining_get(&request_timer)) {
		k_work_reschedule(&request_timer, K_SECONDS(CONFIG_NVME_REQUEST_TIMEOUT));
	}
}
...
static void request_timeout(struct k_work *work)
{
	...
	SYS_DLIST_FOR_EACH_CONTAINER_SAFE(&pending_request, request, next, node) {
		if ((int32_t)(request->req_start + CONFIG_NVME_REQUEST_TIMEOUT - current) > 0) {
			break;
		}

		LOG_WRN("Request %p CID %u timed-out", request, request->cmd.cdw0.cid);
	...
	if (request) {
		k_work_reschedule(&request_timer, K_SECONDS(request->req_start +
							    CONFIG_NVME_REQUEST_TIMEOUT - current));
	}
}
...
static void nvme_cmd_request_complete(struct nvme_request *request, struct nvme_completion *cpl)
{
	bool error, retriable, retry;

	error = nvme_completion_is_error(cpl);
	retriable = nvme_completion_is_retry(cpl);
	retry = error && retriable && request->retries < CONFIG_NVME_RETRY_COUNT;
	...
	if (error && (!retriable || (request->retries >= CONFIG_NVME_RETRY_COUNT))) {
		LOG_DBG("CMD error");
	...
	if (retry) {
		...
		LOG_DBG("Request %p CMD complete on %p/%p", request, request->cb_fn,
			request->cb_arg);
...
int nvme_cmd_qpair_setup(struct nvme_cmd_qpair *qpair, struct nvme_controller *ctrlr, uint32_t id)
{
	const struct nvme_controller_config *nvme_ctrlr_cfg = ctrlr->dev->config;
	...
	qpair->sq_tdbl_off = nvme_mmio_offsetof(doorbell) + (qpair->id << (ctrlr->dstrd + 1));
	qpair->cq_hdbl_off = nvme_mmio_offsetof(doorbell) + (qpair->id << (ctrlr->dstrd + 1)) +
			     (1 << ctrlr->dstrd);

	if (!pcie_msi_vector_connect(nvme_ctrlr_cfg->pcie->bdf, &ctrlr->vectors[qpair->vector],
	...
	LOG_DBG("CMD Qpair created ID %u, %u entries - cmd/cpl addr "
		...
		qpair->id, qpair->num_entries, qpair->cmd_bus_addr, qpair->cpl_bus_addr,
		qpair->sq_tdbl_off, qpair->cq_hdbl_off);
...
static int nvme_cmd_qpair_fill_prp_list(struct nvme_cmd_qpair *qpair, struct nvme_request *request,
					int n_prp)
{
	...
	request->cmd.dptr.prp1 = (uint64_t)sys_cpu_to_le64(p_addr);
	request->cmd.dptr.prp2 = (uint64_t)sys_cpu_to_le64(&prp_list->prp);
...
static int nvme_cmd_qpair_fill_dptr(struct nvme_cmd_qpair *qpair, struct nvme_request *request)
{
	switch (request->type) {
	...
	case NVME_REQUEST_VADDR:
		...
		n_prp = compute_n_prp((uintptr_t)request->payload, request->payload_size);
		if (n_prp <= 2) {
			request->cmd.dptr.prp1 = (uint64_t)sys_cpu_to_le64(request->payload);
			if (n_prp == 2) {
				request->cmd.dptr.prp2 = (uint64_t)sys_cpu_to_le64(
					NVME_PRP_NEXT_PAGE((uintptr_t)request->payload));
			} else {
...
int nvme_cmd_qpair_submit_request(struct nvme_cmd_qpair *qpair, struct nvme_request *request)
{
	mm_reg_t regs = DEVICE_MMIO_GET(qpair->ctrlr->dev);
	int ret;
	...
	request->cmd.cdw0.cid = sys_cpu_to_le16((uint16_t)(request - request_pool));
	...
	memcpy(&qpair->cmd[qpair->sq_tail], &request->cmd, sizeof(request->cmd));
	...
	LOG_DBG("Request %p %llu submitted: CID %u - sq_tail %u", request, qpair->num_cmds,
		request->cmd.cdw0.cid, qpair->sq_tail - 1);
	return 0;
}
