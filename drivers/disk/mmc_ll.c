			...
			unsigned int r_type, unsigned int *r_data)
{
	...
	memset((void *)&cmd, 0, sizeof(struct mmc_cmd));
	...
	if (ret != 0) {
		printk("VERBOSE: Send command %u error: %d\n", idx, ret);
	}
...
static int mmc_device_state(void)
{
	...
	do {
		...
		if (retries == 0) {
			printk("ERROR: CMD13 failed after %d retries\n",
...
static int mmc_sd_switch(unsigned int bus_width)
{
	...
	do {
		...
		if ((ret != 0) && (retries == 0)) {
			printk("ERROR: ACMD51 failed after %d retries (ret=%d)\n",
...
static int mmc_set_ios(unsigned int clk, unsigned int bus_width)
{
	...
	if (mmc_dev_info->mmc_dev_type != MMC_IS_EMMC) {
		if (width == MMC_BUS_WIDTH_8) {
			printk("WARNING: Wrong bus config for SD-card, force to 4\n");
		...
		printk("VERBOSE: Wrong MMC type or spec version\n");
	}
...
static int mmc_fill_device_info(void)
{
	...
	case MMC_IS_SD:
		...
		__ASSERT_NO_MSG(c_size != 0xFFFU);

		mmc_dev_info->device_size = (c_size + 1U) *
					    BIT64(mmc_csd.c_size_mult + 2U) *
					    mmc_dev_info->block_size;

		break;

	case MMC_IS_SD_HC:
		__ASSERT_NO_MSG(mmc_csd.csd_structure == 1U);
...
static int sd_send_op_cond(void)
{
	...
	printk("ERROR: ACMD41 failed after %d retries\n", SEND_OP_COND_MAX_RETRIES);
...
static int mmc_send_op_cond(void)
{
	...
	printk("ERROR: CMD1 failed after %d retries\n", SEND_OP_COND_MAX_RETRIES);
...
size_t mmc_erase_blocks(int lba, size_t size)
{
	...
	__ASSERT_NO_MSG(ops != NULL);
	__ASSERT_NO_MSG((size != 0U) && ((size & MMC_BLOCK_MASK) == 0U));
