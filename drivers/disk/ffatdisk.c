...
static void ffat_write_bs(const struct device *dev, const uint8_t *const buf)
{
	const struct ffatdisk_config *config = dev->config;
	const struct fat_boot_sector *const my_fat_bs = config->fat_bs;
	const struct fat_boot_sector *const fat_bs = (void *)buf;

	LOG_DBG("Write boot sector");
	if (config->fat32) {
		if (my_fat_bs->ebpb32.vol_id == fat_bs->ebpb32.vol_id) {
			LOG_INF("FAT32 CHKDSK 0x%02x", fat_bs->ebpb32.reserved1);
		}
	} else {
		if (my_fat_bs->ebpb16.vol_id == fat_bs->ebpb16.vol_id) {
			LOG_INF("FAT16 CHKDSK 0x%02x", fat_bs->ebpb16.reserved1);
		}
	}
}
			   ...
			   const uint8_t *const buf, const uint32_t sector)
{
	...
	if (ffat_get_file_and_block(dev, sector, &f_block) != NULL) {
		LOG_DBG("Overwriting file block %u, sector %u (%u)",
			f_block, sector, d_sector);
	}

	/*
	 * If there are free clusters on the volume, the file system driver can
	 * write to any of the free clusters and we cannot determine the sector
	 * number of the file. Therefore, there should be only one file with
	 * write callback. To get the exact sector (or block) number and other
	 * metadata, backends need to encapsulate the payload, which we do not
	 * care about here. For the backends that cannot encapsulate the
	 * payload, we use an internal counter that is reset on each write to
	 * FAT sectors.
	 */
	...
	if (f != NULL && f->wr_cb != NULL) {
		f->wr_cb(f, data->wblock_counter, buf, byts_per_sec);
		LOG_DBG("Write file block %u, sector %u (%u)",
			data->wblock_counter, sector, d_sector);
	}
