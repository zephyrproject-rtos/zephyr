			   ...
			   const struct mspi_xfer *req)
{
	...
	if (dev_data->standard_spi &&
	    (req->cmd_length != 0 || req->addr_length != 0)) {
		if (req->rx_dummy % 8) {
			LOG_ERR("Unsupported RX dummy cycles: %u",
				req->rx_dummy);
			return -EINVAL;
		}
	} else {
	...
	if (req->tx_dummy > SPI_CTRLR0_WAIT_CYCLES_MAX) {
		LOG_ERR("Invalid TX dummy cycles: %u", req->tx_dummy);
		return -EINVAL;
	}
