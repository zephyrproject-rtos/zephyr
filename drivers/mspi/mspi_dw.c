...
static void cleanup_async_on_error(const struct device *dev)
{
	...
	LOG_DBG("Cleaning MSPI setup due to error");
...
static void handle_end_of_xfer(const struct device *dev)
{
	struct mspi_dw_data *dev_data = dev->data;
	int rc;
...
static int start_next_packet(const struct device *dev, k_timeout_t timeout)
{
	...
	if ((dev_data->xfer.timeout > 0) && (dev_data->xfer.async)) {
		k_timer_start(&dev_data->async_timeout_timer, K_MSEC(dev_data->xfer.timeout),
				K_NO_WAIT);
	}
			  ...
			  const struct mspi_xfer *req)
{
	...
	/*
	 * For async transactions, hold ctx_lock until completion or
	 * when error occurs - handled by cleanup_async_on_error()
	 */
	if (req->async) {
		return rc;
	}

	/* For sync transactions or async errors, release lock and cleanup */
	k_sem_give(&dev_data->ctx_lock);
...
#define MSPI_DW_INST(inst)						\
	...
	IF_ENABLED(CONFIG_MSPI_DMA,					\
		  (MSPI_DW_DMA_XFER_LIST_DEFINE(inst);))		\
	...
	IF_ENABLED(CONFIG_MSPI_DMA,					\
		(.dma_transfer_list = MSPI_DW_DMA_XFER_LIST_GET(inst),  \
		 .dma_joblist = MSPI_DW_DMA_JOBLIST_GET(inst),))	\
