...
static int mspi_ambiq_config(const struct mspi_dt_spec *spec)
{
	...
	if (config->op_mode != MSPI_OP_MODE_CONTROLLER) {
		LOG_INST_ERR(cfg->log, "%u, only support MSPI controller.", __LINE__);
		return -ENOTSUP;
	}
				 ...
				 const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (param_mask == MSPI_DEVICE_CONFIG_NONE && !cfg->mspicfg.sw_multi_periph) {
		...
		hal_dev_cfg.ui16ReadInstr = dev_cfg->read_cmd;
		hal_dev_cfg.ui16WriteInstr = dev_cfg->write_cmd;
		...
		ret = am_hal_mspi_control(data->mspiHandle, AM_HAL_MSPI_REQ_RXCFG,
					  &data->hal_rx_cfg);
		if (ret) {
			LOG_INST_ERR(cfg->log, "%u, failed AM_HAL_MSPI_REQ_RXCFG, code:%d.",
				     __LINE__, ret);
			ret = -EHOSTDOWN;
			goto e_return;
		}
				 ...
				 const struct mspi_xip_cfg *xip_cfg)
{
	...
	if (ret) {
		LOG_INST_ERR(cfg->log, "%u, fail to configure XIP REQ config, code:%d.", __LINE__,
			     ret);
		return -EHOSTDOWN;
	}
	...
	if (ret) {
		LOG_INST_ERR(cfg->log, "%u, fail to configure XIP MISC config, code:%d.", __LINE__,
			     ret);
		return -EHOSTDOWN;
	}
				      ...
				      const struct mspi_scramble_cfg *scramble_cfg)
{
	...
	hal_xip_cfg.scramblingEndAddr = hal_xip_cfg.scramblingStartAddr + scramble_cfg->size;
				    ...
				    void *timing_cfg)
{
	...
	am_hal_mspi_timing_scan_t timing = {0};
	int ret = 0;
...
#define MSPI_HAL_CFG_CONFIG(n)                                                                     \
	{                                                                                          \
		.ui32TCBSize = DT_INST_PROP_OR(n, cmdq_buffer_size, 1024) / 4,                     \
		.pTCB = (uint32_t *)mspi_ambiq_cmdq##n,                                            \
		.bClkonD4 = 0,                                                                     \
	}
...
#define MSPI_HAL_RX_CFG(n)                                                                         \
	{                                                                                          \
		.ui8DQSturn = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 0),                          \
		.bRxHI = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 1),                               \
		.bTaForth = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 2),                            \
		.bHyperIO = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 3),                            \
		.ui8RxSmp = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 4),                            \
		.bRBX = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 5),                                \
		.bWBX = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 6),                                \
		.bSCLKRxHalt = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 7),                         \
		.bRxCapEXT = DT_INST_PROP_BY_IDX(n, ambiq_rx_config, 8),                           \
...
#define AMBIQ_MSPI_DEFINE(n)                                                                       \
	...
	static struct mspi_ambiq_data mspi_ambiq_data##n = {                                       \
		.mspiHandle = NULL,                                                                \
		.hal_config_cfg = MSPI_HAL_CFG_CONFIG(n),                                          \
