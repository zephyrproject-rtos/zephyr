 ...
 * checked for undesireable values. Changing any of these values might not be
 * actually supported in the end!
 */
#define CADENCE_MSPI_IMPLEMENTED_DEV_CONFIG_PARAMS                                                 \
	(MSPI_DEVICE_CONFIG_BREAK_TIME | MSPI_DEVICE_CONFIG_MEM_BOUND |                            \
	 MSPI_DEVICE_CONFIG_ENDIAN | MSPI_DEVICE_CONFIG_CE_POL | MSPI_DEVICE_CONFIG_CMD_LEN |      \
	 MSPI_DEVICE_CONFIG_DQS | MSPI_DEVICE_CONFIG_DATA_RATE | MSPI_DEVICE_CONFIG_CE_NUM |       \
	 MSPI_DEVICE_CONFIG_IO_MODE | MSPI_DEVICE_CONFIG_CPP | MSPI_DEVICE_CONFIG_FREQUENCY |      \
	 MSPI_DEVICE_CONFIG_CE_POL)

/*
 * Non-implemented dev_cfg_bits
 *
 * These configuration requests are not implemented by the driver. For
 * compatibility no error is returned, if especially MSPI_DEVICE_CONFIG_ALL was
 * requested.
 */
#define CADENCE_MSPI_IGNORED_DEV_CONFIG_PARAMS                                                     \
	(MSPI_DEVICE_CONFIG_ALL & ~CADENCE_MSPI_IMPLEMENTED_DEV_CONFIG_PARAMS)
...
#define CADENCE_MSPI_SRAM_FILL_MAX_AMOUNT_OF_WORDS  255
