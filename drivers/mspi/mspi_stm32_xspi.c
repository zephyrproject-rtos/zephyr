...
static int mspi_stm32_xspi_memmap_on(const struct device *controller)
{
	...
	XSPI_RegularCmdTypeDef s_command = mspi_stm32_xspi_prepare_cmd(dev_data->dev_cfg.io_mode, dev_data->dev_cfg.data_rate);
...
static inline int mspi_stm32_xspi_context_lock(struct mspi_context *ctx, const struct mspi_dev_id *req,
			     ...
			     uint8_t access_mode)
{
	...
	if (dev_data->xip_cfg.enable) {
		...
		if ((packet->cmd == MSPI_STM32_CMD_WREN) || (packet->cmd == MSPI_STM32_OCMD_WREN) ||
		   ...
		   ((mspi_stm32_xspi_hal_address_size(dev_data->dev_cfg.addr_length) == HAL_XSPI_ADDRESS_24_BITS) && (dev_data->dev_cfg.io_mode == MSPI_IO_MODE_SINGLE))) {
...
static int mspi_stm32_xspi_status_reg(const struct device *controller, const struct mspi_xfer *xfer)
{
	...
	ret = mspi_stm32_xspi_wait_auto_polling(controller,
...
static int mspi_stm32_xspi_mem_ready(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	return mspi_stm32_xspi_wait_auto_polling(dev,
		MSPI_STM32_MEM_RDY_MATCH,
		MSPI_STM32_MEM_RDY_MASK,
		HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
}
...
static int mspi_stm32_xspi_write_enable(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	struct mspi_stm32_data *dev_data = dev->data;
	XSPI_RegularCmdTypeDef s_command = mspi_stm32_xspi_prepare_cmd(cfg_mode, cfg_rate);
...
static int mspi_stm32_xspi_write_cfg2reg_io(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate,
				       uint8_t op_enable)
{
	int ret = 0;
	struct mspi_stm32_data *dev_data = dev->data;
	...
	if (HAL_XSPI_Command(&dev_data->hmspi.xspi, &s_command, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
		LOG_ERR("Write Flash configuration reg2 failed");
		return -EIO;
	}

	if (HAL_XSPI_Transmit(&dev_data->hmspi.xspi, &op_enable, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
		LOG_ERR("Write Flash configuration reg2 failed");
		return -EIO;
	}
...
static int mspi_stm32_xspi_read_cfg2reg(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate,
				   uint8_t *value)
{
	int ret = 0;
	struct mspi_stm32_data *dev_data = dev->data;
	...
	if (cfg_mode == MSPI_IO_MODE_SINGLE) {
		...
		s_command.DummyCycles = MSPI_STM32_DUMMY_REG_OCTAL_DTR;
	} else {
		s_command.DummyCycles = MSPI_STM32_DUMMY_REG_OCTAL;
	}
...
static int mspi_stm32_xspi_config_mem(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	if (mspi_stm32_xspi_write_cfg2reg_dummy(dev, MSPI_IO_MODE_SINGLE, MSPI_DATA_RATE_SINGLE) != 0) {
			     ...
			     const struct mspi_dev_cfg *dev_cfg)
{
	...
	/* Simple assignments (no validation needed) */
 ...
 */
static int mspi_stm32_xspi_dev_config(const struct device *controller, const struct mspi_dev_id *dev_id,
				 const enum mspi_dev_cfg_mask param_mask,
				 const struct mspi_dev_cfg *dev_cfg)
{
	int ret = 0;
 ...
 */
static int mspi_stm32_xspi_xip_config(const struct device *controller, const struct mspi_dev_id *dev_id,
...
static int mspi_stm32_xspi_get_channel_status(const struct device *controller, uint8_t ch)
{
	...
	if (mspi_stm32_xspi_is_inp(controller) || (HAL_XSPI_GET_FLAG(&dev_data->hmspi.xspi, HAL_XSPI_FLAG_BUSY) == SET)) {
		ret = -EBUSY;
	}
...
static int mspi_stm32_xspi_pio_dma_transceive(const struct device *controller, const struct mspi_xfer *xfer)
{
	int ret = 0;
	uint32_t packet_idx;
	struct mspi_stm32_data *dev_data = controller->data;
	...
	while (ctx->packets_left > 0) {
		...
		packet = (struct mspi_xfer_packet *)&ctx->xfer.packets[packet_idx];
		if (!dev_cfg->use_dma) {
			ret = mspi_stm32_xspi_access(controller, packet, (ctx->xfer.async == true) ? MSPI_ACCESS_ASYNC : MSPI_ACCESS_SYNC);
		} else {
 ...
 */
static int mspi_stm32_xspi_transceive(const struct device *controller, const struct mspi_dev_id *dev_id,
				 const struct mspi_xfer *xfer)
{
	...
	if (((xfer->packets->cmd == MSPI_STM32_OCMD_RDSR) || (xfer->packets->cmd == MSPI_STM32_CMD_RDSR))) {
		return mspi_stm32_xspi_status_reg(controller, xfer);
	}

	return mspi_stm32_xspi_pio_dma_transceive(controller, xfer);
...
static int mspi_config_clocks(const struct mspi_stm32_conf *dev_cfg,
...
static int mspi_hal_init(const struct mspi_stm32_conf *dev_cfg,
			 struct mspi_stm32_data *dev_data,
			 uint32_t ahb_clock_freq)
{
...
static int mspi_stm32_xspi_config(const struct mspi_dt_spec *spec)
{
	...
	ret = mspi_config_clocks(dev_cfg, dev_data, &ahb_clock_freq);
	if (ret) {
		return ret;
	}
...
#define STM32_SMPI_IRQ_HANDLER(index)								\
	...
	{											\
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), mspi_stm32_xspi_isr,	\
