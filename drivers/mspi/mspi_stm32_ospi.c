...
#define STM32_OSPI_DLYB_BYPASSED DT_PROP(DT_DRV_INST(0), dlyb_bypass)

#define DT_OSPI_IO_PORT_PROP_OR(prop, default_value)					\
	COND_CODE_1(DT_NODE_HAS_PROP(DT_DRV_INST(0), prop),				\
		    (_CONCAT(HAL_OSPIM_, DT_STRING_TOKEN(DT_DRV_INST(0), prop))),	\
		    ((default_value)))

#define DT_OSPI_PROP_OR(prop, default_value) \
	DT_PROP_OR(DT_DRV_INST(0), prop, default_value)

LOG_MODULE_REGISTER(ospi_stm32, CONFIG_MSPI_LOG_LEVEL);

#include "mspi_stm32.h"

static inline int mspi_stm32_ospi_context_lock(struct mspi_context *ctx,
 ...
 */
static inline bool mspi_stm32_ospi_is_inp(const struct device *controller)
{
...
static OSPI_RegularCmdTypeDef mspi_stm32_ospi_prepare_cmd(uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	cmd_tmp.InstructionDtrMode =
		((cfg_rate == MSPI_DATA_RATE_DUAL) ? HAL_OSPI_INSTRUCTION_DTR_ENABLE
...
static bool mspi_stm32_ospi_is_memorymap(const struct device *dev)
{
	...
	return ((READ_BIT(dev_data->hmspi.ospi.Instance->CR,
...
static int mspi_stm32_ospi_memmap_on(const struct device *controller)
{
	...
	ret = HAL_OSPI_Command(&dev_data->hmspi.ospi, &s_command, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
	if (ret != HAL_OK) {
	...
	ret = HAL_OSPI_MemoryMapped(&dev_data->hmspi.ospi, &s_MemMappedCfg);
	if (ret != HAL_OK) {
			     ...
			     uint8_t access_mode)
{
	...
	if (dev_data->xip_cfg.enable) {
		if (mspi_stm32_ospi_is_memorymap(dev) && packet->dir == MSPI_TX) {

			hal_ret = mspi_stm32_ospi_memmap_off(dev);
			if (hal_ret != 0) {
			...
			if (!mspi_stm32_ospi_is_memorymap(dev)) {
				hal_ret = mspi_stm32_ospi_memmap_on(dev);
				if (hal_ret != 0) {
...
static int mspi_stm32_ospi_status_reg(const struct device *controller, const struct mspi_xfer *xfer)
{
	...
	ret = mspi_stm32_ospi_wait_auto_polling(controller,
...
static int mspi_stm32_ospi_mem_ready(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	return mspi_stm32_ospi_wait_auto_polling(dev,
		MSPI_STM32_MEM_RDY_MATCH,
		MSPI_STM32_MEM_RDY_MASK,
		HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
}
					  ...
					  uint8_t cfg_rate)
{
	...
	s_command.DummyCycles = 0U;
	s_command.NbData = (cfg_mode == MSPI_IO_MODE_SINGLE)
				   ? 1U
...
static void mspi_stm32_ospi_dma_callback(const struct device *dev, void *arg, uint32_t channel,
					 int status)
{
 ...
 */
static inline int mspi_stm32_ospi_verify_device(const struct device *controller,
				      ...
				      const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (mspi_stm32_ospi_dev_cfg_save(controller, param_mask, dev_cfg)) {
					  ...
					  const struct mspi_xfer *xfer)
{
	...
	while (ctx->packets_left > 0) {
		...
		ret = mspi_stm32_ospi_access(controller, packet,
				 ...
				 const struct mspi_dev_id *dev_id, const struct mspi_xfer *xfer)
{
	 ...
	 */
	if ((xfer->packets->cmd == MSPI_STM32_OCMD_RDSR) ||
	...
	} else if (xfer->xfer_mode == MSPI_PIO) {
...
#define MSPI_STM32_INIT(index)             \
	...
	static struct mspi_stm32_data mspi_stm32_dev_data_##index = {     \
		...
		OSPI_DMA_CHANNEL(DT_DRV_INST(index), tx_rx)};  \
         \
	DEVICE_DT_INST_DEFINE(index, &mspi_stm32_ospi_init, NULL, &mspi_stm32_dev_data_##index,\
