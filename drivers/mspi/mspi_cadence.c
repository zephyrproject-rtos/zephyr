...
#define DT_DRV_COMPAT cdns_xspi_nor
 ...
 */
static int mspi_cadence_wait_for_idle(const struct device *controller)
{
...
static int mspi_cadence_check_transfer_package(const struct mspi_xfer *request, uint32_t index)
{
	...
	if (((uint64_t)packet->address) >> (8 * request->addr_length)) {
...
static int mspi_cadence_init(const struct device *dev)
{
	...
	timing_config_reg |=
		FIELD_PREP(CADENCE_MSPI_DEV_DELAY_REG_D_NSS_MASK, CADENCE_MSPI_DEFAULT_DELAY);

	timing_config_reg &= ~CADENCE_MSPI_DEV_DELAY_REG_D_BTWN_MASK;
	timing_config_reg |=
		FIELD_PREP(CADENCE_MSPI_DEV_DELAY_REG_D_BTWN_MASK, CADENCE_MSPI_DEFAULT_DELAY);

	timing_config_reg &= ~CADENCE_MSPI_DEV_DELAY_REG_D_AFTER_MASK;
	timing_config_reg |=
		FIELD_PREP(CADENCE_MSPI_DEV_DELAY_REG_D_AFTER_MASK, CADENCE_MSPI_DEFAULT_DELAY);

	timing_config_reg &= ~CADENCE_MSPI_DEV_DELAY_REG_D_INIT_MASK;
	timing_config_reg |=
		FIELD_PREP(CADENCE_MSPI_DEV_DELAY_REG_D_INIT_MASK, CADENCE_MSPI_DEFAULT_DELAY);
	...
	ind_ahb_addr_trigger &= ~CADENCE_MSPI_IND_AHB_ADDR_TRIGGER_REG_ADDR_MASK;
				       ...
				       uint32_t index, const uint64_t start_time)
{
	...
	while (remaining_bytes > 0) {
		...
		uint32_t current_sram_fill =
			sys_read32(base_address + CADENCE_MSPI_SRAM_FILL_OFFSET);
		uint32_t currently_used_write_words =
			FIELD_GET(CADENCE_MSPI_SRAM_FILL_REG_INDAC_WRITE_MASK, current_sram_fill);
		uint32_t total_available_write_words = CADENCE_MSPI_SRAM_FILL_MAX_AMOUNT_OF_WORDS -
						       config->sram_allocated_for_read;
		free_words = total_available_write_words - currently_used_write_words;
...
static int mspi_cadence_dev_config(const struct device *controller,
				   ...
				   const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (param_mask & MSPI_DEVICE_CONFIG_CE_POL) {
		if (dev_cfg->ce_polarity != MSPI_CE_ACTIVE_LOW) {
	...
	if (param_mask & MSPI_DEVICE_CONFIG_DATA_RATE) {
		uint32_t rd_config =
			sys_read32(base_addr + CADENCE_MSPI_DEV_INSTR_RD_CONFIG_OFFSET);
	...
	if (param_mask & MSPI_DEVICE_CONFIG_CE_NUM) {
		...
		num = ~BIT(dev_cfg->ce_num) & BIT_MASK(4);

		config_reg &= ~CADENCE_MSPI_CONFIG_REG_PERIPH_CS_LINES_MASK;
		config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_PERIPH_CS_LINES_MASK, num);
		sys_write32(config_reg, base_addr + CADENCE_MSPI_CONFIG_OFFSET);
	}
	if (param_mask & MSPI_DEVICE_CONFIG_IO_MODE) {
		ret = mspi_cadence_set_opcode_lines(base_addr, dev_cfg->io_mode);
		if (ret) {
			goto exit;
		}
		ret = mspi_cadence_set_data_lines(base_addr, dev_cfg->io_mode);
		if (ret) {
			goto exit;
		}
		ret = mspi_cadence_set_addr_lines(base_addr, dev_cfg->io_mode);
		if (ret) {
			goto exit;
		}
	}
	if (param_mask & MSPI_DEVICE_CONFIG_CPP) {
		config_reg &= ~CADENCE_MSPI_CONFIG_REG_SEL_CLK_POL_BIT;
		config_reg &= ~CADENCE_MSPI_CONFIG_REG_SEL_CLK_PHASE_BIT;
		switch (dev_cfg->cpp) {
		case MSPI_CPP_MODE_0:
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_POL_BIT, 0);
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_PHASE_BIT, 0);
			break;
		case MSPI_CPP_MODE_1:
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_POL_BIT, 0);
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_PHASE_BIT, 1);
			break;
		case MSPI_CPP_MODE_2:
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_POL_BIT, 1);
			config_reg |= FIELD_PREP(CADENCE_MSPI_CONFIG_REG_SEL_CLK_PHASE_BIT, 0);
			break;
	...
	if (param_mask & MSPI_DEVICE_CONFIG_FREQUENCY) {
		...
		/*
		 * This is underflow-safe (assuming the reference frequency is set)
		 * since required_div will be at least 1 leading to the second
		 * DIV_ROUND_UP also returning at least 1 before being subtracted 1
		 * resulting in 0.
		 */
		uint32_t required_div = DIV_ROUND_UP(config->reference_frequency, dev_cfg->freq);
		uint32_t actual_div = DIV_ROUND_UP(required_div, 2) - 1;

		if (actual_div > 15) {
			LOG_ERR("Requested a frequency that is too low to achieve by dividing the "
				"reference clock");
			ret = -ENOTSUP;
			goto exit;
		}
...
static int mspi_cadence_get_channel_status(const struct device *controller, uint8_t channel)
{
	ARG_UNUSED(channel);
