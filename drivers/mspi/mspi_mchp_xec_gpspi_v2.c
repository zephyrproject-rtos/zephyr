 ...
 * GPSPI hardware has a single chip-select line, so num_periph is fixed at 1 in
 * MSPI_MCHP_XEC_GPSPI_MSPI_CFG() rather than taken from the DT child count; there is
 * nothing left for this function to check it against, and software-multiperipheral
 * (only meaningful for 2+ child devices) is likewise left false.
 */
