...
#define VENDOR_SPECIFIC_DATA_GET(inst) NULL
...
#define MSPI_DT_DRV_COMPAT nordic_nrf_qspi_v2
#include <nrf.h>
...
#define VENDOR_SPECIFIC_DATA_DEFINE(inst) \
	...
	static nordic_qspi_vendor_data_t mspi_dw_##inst##_vendor_data = { \
		.transfer_list = &mspi_dw_##inst##_transfer_list, \
		.joblist = &mspi_dw_##inst##_joblist[0] \
	};

#define VENDOR_SPECIFIC_DATA_GET(inst) &mspi_dw_##inst##_vendor_data
...
static inline void vendor_specific_start_dma_xfer(const struct device *dev)
{
	...
	nordic_qspi_vendor_data_t *vendor_data = (nordic_qspi_vendor_data_t *)
