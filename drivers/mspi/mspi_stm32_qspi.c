...
static int mspi_context_lock(struct mspi_stm32_context *ctx, const struct mspi_dev_id *req,
...
static int mspi_stm32_qspi_config(const struct mspi_dt_spec *spec)
{
	...
	hmspi->Init.SampleShifting = QSPI_SAMPLE_SHIFTING_HALFCYCLE;
 ...
 */
static int mspi_stm32_qspi_verify_device(const struct device *controller,
				      ...
				      const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (data->dev_id != dev_id) {
		...
		ret = mspi_stm32_qspi_verify_device(controller, dev_id);
		if (ret != 0) {
			goto e_return;
		}
	}
...
#define MSPI_QSPI_CONFIG(n)                                                                    \
	{                                                                                      \
		...
		.max_freq = DT_INST_PROP_OR(n, clock_frequency, MSPI_MAX_FREQ),             \
