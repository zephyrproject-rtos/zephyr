...
static int mspi_stm32_qspi_memmap_on(const struct device *controller)
{
	...
	if (dev_data->dev_cfg.read_cmd != 0) {
		...
		if (dev_data->dev_cfg.addr_length == 4) {
			s_command.Instruction = SPI_NOR_CMD_READ_FAST_4B;
		} else {
			s_command.Instruction = SPI_NOR_CMD_READ_FAST;
		}
	}
...
{
	return ((packet->cmd == SPI_NOR_CMD_WREN) || (packet->cmd == SPI_NOR_CMD_SE) ||
		(packet->cmd == SPI_NOR_CMD_SE_4B) || (packet->cmd == SPI_NOR_CMD_RDSR) ||
		(packet->dir == MSPI_TX));
}
					    ...
					    uint8_t access_mode)
{
	...
	if (packet->dir == MSPI_RX) {
		...
		case MSPI_ACCESS_SYNC:
			ret = HAL_QSPI_Receive(&dev_data->hmspi.qspi, packet->data_buf,
		...
		switch (access_mode) {
		case MSPI_ACCESS_SYNC:
			ret = HAL_QSPI_Transmit(&dev_data->hmspi.qspi, packet->data_buf,
						HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
			return ret;
		case MSPI_ACCESS_ASYNC:
	...
	if ((ret != HAL_OK) || (access_mode == MSPI_ACCESS_SYNC)) {
		pm_policy_state_lock_put(PM_STATE_SUSPEND_TO_IDLE, PM_ALL_SUBSTATES);
		(void)pm_device_runtime_put(dev);
		if (ret != HAL_OK) {
			LOG_ERR("Failed to start %s transfer: %d",
				(packet->dir == MSPI_RX) ? "receive" : "transmit", ret);
			return -EIO;
		}
	}
				  ...
				  uint8_t access_mode)
{
	...
	if (cmd.Instruction == SPI_NOR_CMD_WREN) {
	...
	ret = mspi_stm32_qspi_execute_transfer(dev, packet, access_mode);
 ...
 */
static int mspi_stm32_qspi_pm_action(const struct device *dev,
				     enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		{
			bool xip_enabled = dev_data->xip_cfg.enable;
			int ret;
			...
			if (xip_enabled || k_mutex_lock(&dev_data->lock, K_NO_WAIT) != 0) {
