...
static bool stm32_xspi_is_memorymap(const struct device *dev)
{
	...
	return READ_BIT(dev_data->hmspi.xspi.Instance->CR, XSPI_CR_FMODE) == XSPI_CR_FMODE;
}
 ...
 *
 */
static int mspi_stm32_access(const struct device *dev, struct mspi_xfer_packet *packet,
			     uint8_t access_mode)
{
	int ret;
	struct mspi_stm32_data *dev_data = dev->data;
...
indirect:
	...
	ret = HAL_XSPI_Command(&dev_data->hmspi.xspi, &cmd, HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
	if (ret != HAL_OK) {
	...
	if (packet->dir == MSPI_RX) {
		...
		case MSPI_ACCESS_SYNC:
			ret = HAL_XSPI_Receive(&dev_data->hmspi.xspi, packet->data_buf,
						   HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
			goto e_access;
		case MSPI_ACCESS_ASYNC:
			ret = HAL_XSPI_Receive_IT(&dev_data->hmspi.xspi, packet->data_buf);
			break;
		case MSPI_ACCESS_DMA:
			uint8_t *dma_buf = k_aligned_alloc(CONFIG_MSPI_BUFFER_ALIGNMENT, packet->num_bytes);
			if (!dma_buf) {
				LOG_ERR("DMA buffer allocation failed");
				return -ENOMEM;
			}

			ret = HAL_XSPI_Receive_DMA(&dev_data->hmspi.xspi, dma_buf);
			if (ret == 0) {
				if (k_sem_take(&dev_data->sync, K_FOREVER)) {
					LOG_ERR("%d: Failed to take sem", dev_data->hmspi.xspi.ErrorCode);
					k_free(dma_buf);
					return -EIO;
				}
		...
		switch (access_mode) {
		case MSPI_ACCESS_SYNC:
			ret = HAL_XSPI_Transmit(&dev_data->hmspi.xspi, packet->data_buf,
						    HAL_XSPI_TIMEOUT_DEFAULT_VALUE);
			goto e_access;
		case MSPI_ACCESS_ASYNC:
			ret = HAL_XSPI_Transmit_IT(&dev_data->hmspi.xspi, packet->data_buf);
			break;
		case MSPI_ACCESS_DMA:
			ret = HAL_XSPI_Transmit_DMA(&dev_data->hmspi.xspi, packet->data_buf);
			break;
		default:
			/* Not correct */
			ret = HAL_BUSY;
	...
	if (ret != HAL_OK) {
		LOG_ERR("%d: Failed to access data", ret);
		return -EIO;
	}
...
static int mspi_stm32_status_reg(const struct device *controller, const struct mspi_xfer *xfer)
{
	...
	if (stm32_xspi_is_memorymap(controller)) {
		...
		if (ret != 0) {
			LOG_ERR("Failed to abort memory-mapped access before sending status command");
		return  -1;
		}
	}
					  ...
					  uint8_t cfg_rate)
{
	...
	if (HAL_XSPI_Transmit(&dev_data->hmspi.xspi, &transmit_data, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) !=
	    HAL_OK) {
				   ...
				   uint8_t *value)
{
	...
	if (HAL_XSPI_Receive(&dev_data->hmspi.xspi, value, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK) {
		LOG_ERR("Write Flash configuration reg2 failed");
		return -EIO;
	}
...
static int mspi_stm32_get_channel_status(const struct device *controller, uint8_t ch)
{
	...
	if (mspi_is_inp(controller) || (HAL_XSPI_GET_FLAG(&dev_data->hmspi.xspi, HAL_XSPI_FLAG_BUSY) == SET)) {
		ret = -EBUSY;
	}
...
static int mspi_stm32_pio_dma_transceive(const struct device *controller, const struct mspi_xfer *xfer)
{
	int ret = 0;
	uint32_t packet_idx;
	struct mspi_stm32_data *dev_data = controller->data;
				 ...
				 const struct mspi_xfer *xfer)
{
	...
	if (((xfer->packets->cmd == MSPI_STM32_OCMD_RDSR) || (xfer->packets->cmd == MSPI_STM32_CMD_RDSR))) {
		return mspi_stm32_status_reg(controller, xfer);
	}

	return mspi_stm32_pio_dma_transceive(controller, xfer);
...
static int mspi_stm32_config(const struct mspi_dt_spec *spec)
{
	...
	} else if (dev_data->hmspi.xspi.Instance == XSPI2) {
		mspi_mgr_cfg.IOPort = HAL_XSPIM_IOPORT_2;
	}
	mspi_mgr_cfg.nCSOverride = HAL_XSPI_CSSEL_OVR_DISABLED;
	mspi_mgr_cfg.Req2AckTime = 1;

	if (HAL_XSPIM_Config(&dev_data->hmspi.xspi, &mspi_mgr_cfg, HAL_XSPI_TIMEOUT_DEFAULT_VALUE) !=
	    HAL_OK) {
	...
	if (dev_cfg->use_dma) {
		/* Configure/enable the DMA channels after XSPI config */
		...
		__HAL_LINKDMA(&dev_data->hmspi.xspi, hdmatx, hdma_tx);
		...
		__HAL_LINKDMA(&dev_data->hmspi.xspi, hdmarx, hdma_rx);
