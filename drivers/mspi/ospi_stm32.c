...
static bool mspi_stm32_is_memorymap(const struct device *dev)
{
...
static int mspi_stm32_memmap_on(const struct device *controller)
{
	...
	ret = HAL_OSPI_Command(&dev_data->hmspi, &s_command, HAL_OSPI_TIMEOUT_DEFAULT_VALUE);
	if (ret != HAL_OK) {
...
static void ospi_dma_callback(const struct device *dev, void *arg, uint32_t channel, int status)
{
...
static int mspi_stm32_get_channel_status(const struct device *controller, uint8_t ch)
{
	...
	if (mspi_is_inp(controller) ||__HAL_OSPI_GET_FLAG(&dev_data->hmspi.ospi, HAL_OSPI_FLAG_BUSY) == SET) {
		ret = -EBUSY;
	}
...
{
	struct mspi_stm32_data *dev_data = controller->data;
	struct mspi_stm32_conf *dev_conf = controller->config;
