				      ...
				      enum mspi_io_mode io_mode)
{
	...
	/* Pre-calculate total instruction count. */
	dummy = (pkt->dir == MSPI_RX) ? xfer->rx_dummy : xfer->tx_dummy;

	/* The dummy count is encoded in a single 8-bit LUT operand. */
	if (dummy > UINT8_MAX) {
		LOG_ERR("dummy %u exceeds LUT operand max (255)", dummy);
		return -EINVAL;
	}

	/* SPI NOR addresses are at most 4 bytes (32-bit). */
	if (xfer->addr_length > 4) {
		LOG_ERR("addr_length %u not supported (max 4)", xfer->addr_length);
		return -EINVAL;
	}

	n += (xfer->cmd_length > 0) ? 1 : 0;
	n += (xfer->addr_length > 0) ? 1 : 0;
	n += (dummy > 0) ? 1 : 0;
	n += (pkt->num_bytes > 0) ? 1 : 0;
	n += 1; /* STOP */
			 ...
			 void *buf, uint32_t len)
{
	...
	for (uint32_t off = 0; left > 0; off += rx_fifo_size) {
		...
		if (!WAIT_FOR(!(base->SR & QuadSPI_SR_BUSY_MASK),
			      CONFIG_MSPI_NXP_QSPI_BUSY_TIMEOUT_US, k_busy_wait(1))) {
			LOG_ERR("IP read timed out");
			return -EIO;
		}
			  ...
			  const void *buf, uint32_t len)
{
	...
	uint32_t tx_words[FSL_FEATURE_QSPI_TXFIFO_DEPTH];

	if (total_words > FSL_FEATURE_QSPI_TXFIFO_DEPTH) {
		LOG_ERR("Write too large: %u bytes (max %u)",
			len, FSL_FEATURE_QSPI_TXFIFO_DEPTH * 4U);
		return -EINVAL;
	}
	...
	for (uint32_t i = 0; i < total_words; i++) {
		if (!WAIT_FOR(!(QSPI_GetStatusFlags(base) & (uint32_t)kQSPI_TxBufferFull),
			      CONFIG_MSPI_NXP_QSPI_BUSY_TIMEOUT_US, k_busy_wait(1))) {
			LOG_ERR("TX FIFO prefill timed out");
			return -EIO;
		}
...
static int mspi_nxp_qspi_config(const struct mspi_dt_spec *spec)
{
	...
	if (config->duplex != MSPI_HALF_DUPLEX) {
		LOG_ERR("Only half-duplex transfers are supported");
		return -ENOTSUP;
	}

	/* Software-managed multi-peripheral operation is not implemented. */
	if (config->num_periph > 1) {
		LOG_ERR("Multiple peripherals not supported (num_periph=%u)",
			config->num_periph);
		return -ENOTSUP;
	}
...
{
	const struct mspi_nxp_qspi_config *qspi_cfg = dev->config;
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_RX_DUMMY) && (cfg->rx_dummy > UINT8_MAX)) {
		LOG_ERR("rx_dummy %u exceeds LUT operand max (255)", cfg->rx_dummy);
		return -EINVAL;
	}

	if ((param_mask & MSPI_DEVICE_CONFIG_TX_DUMMY) && (cfg->tx_dummy > UINT8_MAX)) {
		LOG_ERR("tx_dummy %u exceeds LUT operand max (255)", cfg->tx_dummy);
		return -EINVAL;
	}

	if ((param_mask & MSPI_DEVICE_CONFIG_CMD_LEN) && (cfg->cmd_length > 1)) {
		LOG_ERR("Unsupported cmd length: %u (max 1)", cfg->cmd_length);
		return -ENOTSUP;
	}
	...
	/* Single-peripheral fast path, acquire the bus without reconfiguring.
	 * Multi-peripheral switching is not implemented in this driver.
	 */
	if (param_mask == MSPI_DEVICE_CONFIG_NONE && (!qspi_cfg->mspicfg.sw_multi_periph)) {
		return 0;
	}
	...
	/* The settings below are recorded for introspection and future use but
	 * are not consumed by this driver's transfer path.
	 */
...
static int mspi_nxp_qspi_get_channel_status(const struct device *dev, uint8_t ch)
{
	...
	/* Do not release the bus while a transfer is still in flight. */
	if (cfg->base->SR & QuadSPI_SR_BUSY_MASK) {
		return -EBUSY;
	}
...
#define NXP_QSPI_INIT(n)                                                      \
	...
	static const struct mspi_nxp_qspi_config mspi_nxp_qspi_cfg_##n = {    \
		...
		.mspicfg = {                                                   \
			...
			.sw_multi_periph =                                     \
				DT_INST_PROP(n, software_multiperipheral),     \
		},                                                             \
