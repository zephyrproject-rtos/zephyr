...
#define SPIN_TABLE_RELEASE_ADDR(node_id)                                           \
	COND_CODE_1(CPU_IS_SPIN_TABLE(node_id),                                    \
		((((uint64_t)DT_PROP_BY_IDX(node_id, cpu_release_addr, 0) << 32) | \
		  (uint64_t)DT_PROP_BY_IDX(node_id, cpu_release_addr, 1))), (0))
...
static const uint64_t release_addr[] = {
	DT_FOREACH_CPU_STATUS_OKAY_SEP(SPIN_TABLE_RELEASE_ADDR, (,))};
...
int pm_system_reset(uint8_t reset_type __unused)
{
