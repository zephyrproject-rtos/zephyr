...
#define PRECISION_PTP_MIN_RATE_PPB (-999999999)

static int precision_ptp_read(const struct precision_clock *precision_clk,
			      struct precision_time_point *tp)
{
	struct precision_ptp_clock_adapter *adapter =
		(struct precision_ptp_clock_adapter *)precision_clk->user_data;
...
static int precision_ptp_set(const struct precision_clock *precision_clk,
...
static int precision_ptp_adjust_phase(const struct precision_clock *precision_clk,
...
static int precision_ptp_adjust_rate(const struct precision_clock *precision_clk, int32_t rate_ppb)
{
	struct precision_ptp_clock_adapter *adapter =
		(struct precision_ptp_clock_adapter *)precision_clk->user_data;
	const struct ptp_clock_driver_api *api;
...
static int precision_ptp_get_caps(const struct precision_clock *precision_clk,
...
static uint32_t precision_ptp_cap_flags(uint32_t ptp_flags)
{
	uint32_t flags = 0U;
 ...
 * ptp_clock_rate_adjust() already rely on. Adjustment limits stay unset
 * because a legacy driver cannot describe them.
 */
static uint32_t precision_ptp_legacy_cap_flags(const struct ptp_clock_driver_api *api)
{
	uint32_t flags = 0U;
...
int precision_ptp_clock_init(struct precision_ptp_clock_adapter *adapter,
			     const struct device *ptp_clock, struct precision_time_domain domain)
{
	...
	caps = (struct precision_clock_caps){
		.flags = precision_ptp_legacy_cap_flags(api),
		.resolution_ns = 1,
	...
	if ((caps.flags & PRECISION_CLOCK_CAP_ADJUST_RATE) != 0U) {
		 ...
		 */
		caps.min_rate_ppb = PRECISION_PTP_MIN_RATE_PPB;
	...
	if ((api != NULL) && (api->get_caps != NULL)) {
		...
		if (ret == 0) {
			caps = (struct precision_clock_caps){
				.flags = precision_ptp_cap_flags(ptp_caps.flags),
