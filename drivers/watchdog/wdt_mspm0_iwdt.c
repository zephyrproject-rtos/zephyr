...
struct wwdt_mspm0_iwdt_config {
	struct lfss_iwdt_regs *base;
};

struct wwdt_mspm0_iwdt_data {
...
static int wwdt_mspm0_iwdt_calculate_timeout(const struct wdt_timeout_cfg *cfg,
					     struct wwdt_mspm0_iwdt_data *data)
{
	static const struct {
...
static int wwdt_mspm0_iwdt_setup(const struct device *dev, uint8_t options)
{
	const struct wwdt_mspm0_iwdt_config *config = dev->config;
	struct wwdt_mspm0_iwdt_data *data = dev->data;
...
static int wwdt_mspm0_iwdt_disable(const struct device *dev)
{
	const struct wwdt_mspm0_iwdt_config *config = dev->config;
	struct wwdt_mspm0_iwdt_data *data = dev->data;
	struct lfss_iwdt_regs *base = config->base;
...
static int wwdt_mspm0_iwdt_install_timeout(const struct device *dev,
					   const struct wdt_timeout_cfg *cfg)
{
	...
	ret = wwdt_mspm0_iwdt_calculate_timeout(cfg, data);
	if (ret == 0) {
...
static int wwdt_mspm0_iwdt_feed(const struct device *dev, int channel_id)
{
	const struct wwdt_mspm0_iwdt_config *config = dev->config;
	struct wwdt_mspm0_iwdt_data *data = dev->data;
...
static int wwdt_mspm0_iwdt_init(const struct device *dev)
{
	struct wwdt_mspm0_iwdt_data *data = dev->data;
...
static DEVICE_API(wdt, wwdt_mspm0_iwdt_driver_api) = {
	.setup = wwdt_mspm0_iwdt_setup,
	.disable = wwdt_mspm0_iwdt_disable,
	.install_timeout = wwdt_mspm0_iwdt_install_timeout,
	.feed = wwdt_mspm0_iwdt_feed,
};

#define WWDT_MSPM0_IWDT_INIT(index)                                                                \
	static const struct wwdt_mspm0_iwdt_config wwdt_mspm0_iwdt_cfg_##index = {                 \
		.base = (struct lfss_iwdt_regs *)DT_INST_REG_ADDR(index),                          \
	};                                                                                         \
                                                                                                   \
	static struct wwdt_mspm0_iwdt_data wwdt_mspm0_iwdt_data_##index;                           \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(index, wwdt_mspm0_iwdt_init, NULL, &wwdt_mspm0_iwdt_data_##index,    \
			      &wwdt_mspm0_iwdt_cfg_##index, POST_KERNEL,                           \
			      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &wwdt_mspm0_iwdt_driver_api);

DT_INST_FOREACH_STATUS_OKAY(WWDT_MSPM0_IWDT_INIT)
