...
{
	/*
	 * PER field encoding: index i maps to register value (i << 4).
	 * Descending exponent order so per_exp[0] is the longest period.
	 */
	static const uint8_t per_exp[] = { 25, 21, 18, 15, 12, 10, 8, 6 };
	...
	uint64_t abs_ticks = IWDT_MAX_CLKDIV * (uint64_t)BIT(per_exp[0]);
	...
	/* Iterate from shortest period up; first entry whose max timeout covers max_ms wins. */
	for (int i = (int)ARRAY_SIZE(per_exp) - 1; i >= 0; i--) {
		uint64_t max_ticks = IWDT_MAX_CLKDIV * (uint64_t)BIT(per_exp[i]);
	...
	data->per = (uint8_t)(lut_idx << 4);

	/* Walk CLKDIV 0..7 to find the smallest divider whose timeout >= max_ms. */
	uint8_t clkdiv;

	for (clkdiv = 0; clkdiv < IWDT_MAX_CLKDIV; clkdiv++) {
		uint64_t ticks =
			(uint64_t)(clkdiv + 1U) * BIT(per_exp[lut_idx]);
	...
	data->clock_divider = MIN(clkdiv, IWDT_MAX_CLKDIV - 1U);
