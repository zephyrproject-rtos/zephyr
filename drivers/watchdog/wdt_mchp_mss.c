...
static int wdt_mchp_mss_setup(const struct device *dev, uint8_t options)
{
	...
	regval &= ~(
		CONTROL_ENABLE_FORBIDDEN_MASK |
		CONTROL_INTEN_SLEEP_MASK |
...
static int wdt_mchp_mss_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	...
	if ((sys_read32(devcfg->wdt_base_addr + CORE_WDT_STATUS) &
		STATUS_LOCKED_MASK) != 0) {
		return -EBUSY;
	}
...
#define WDT_MCHP_MSS_WDT_DEVICE(idx)	\
	...
	\
	static struct mss_wdt_data wdt_##idx##_data = {	\
		.already_programmed = false,	\
		.running = false	\
	};	\
	\
	DEVICE_DT_DEFINE(DT_NODELABEL(wdt##idx), wdt_mchp_mss_init,	\
	NULL, &wdt_##idx##_data, &wdt_##idx##_config, PRE_KERNEL_1,	\
	CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &wdt_mchp_mss_driver_api)
