...
struct wdt_nrfx_data {
	/* Each activated and not reloaded channel can generate watchdog
	 * interrupt.
	 * Array m_callbacks provides storing callbacks for each channel.
	 */
	wdt_callback_t m_callbacks[NRF_WDT_CHANNEL_NUMBER];

	/* The m_allocated_channels variable stores number of currently
	 * allocated and activated watchdog channels.
	 */
	u8_t m_allocated_channels;

	/* The m_timeout variable stores watchdog timeout value in millisecond
	 * units.
	 * It is used to check whether installing watchdog provide the same
	 * timeout value in the window configuration.
	 */
	u32_t m_timeout;
};
...
static int wdt_nrf_setup(struct device *dev, u8_t options)
{
	...
	nrf_wdt_reload_value_set(
			...
			(uint32_t)(((uint64_t)get_dev_data(dev)->m_timeout *
				    32768U) / 1000));
