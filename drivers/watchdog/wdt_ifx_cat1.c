...
#elif defined(CY_IP_MXS22SRSS) && (IFX_WDT_MATCH_BITS == 22)
/* ILO, PILO, BAK all run at 32768 Hz - Period is ~0.030518 ms */
...
#if defined(CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 2)
uint32_t ifx_wdt_timeout_to_match(uint16_t timeout_ms)
{
...
#if defined(CY_IP_MXS40SRSS) && (CY_IP_MXS40SRSS_VERSION >= 2)
	Cy_WDT_SetUpperLimit(IFX_wdt_timeout_to_match(wdt_initial_timeout_ms));
#else
	Cy_WDT_SetMatch(
