...
static int nxp_ewm_setup(const struct device *dev, uint8_t options)
{
	...
	if (config->clk_sel >= 0 && config->clk_sel <= 3) {
...
static int nxp_ewm_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	struct nxp_ewm_data *data = dev->data;
	...
	if (cfg && cfg->window.max <= NXP_EWM_MAX_TIMEOUT_WINDOW &&
	    cfg->window.min <= cfg->window.max && cfg->window.max > 0 && cfg->window.min >= 0) {
...
#define WDT_EWM_INIT(n)                                                                            \
	...
	static const struct nxp_ewm_config nxp_ewm_config_##n = {                                  \
		...
		.is_input_active_high = DT_INST_PROP(n, input_trigger_active_high),                \
		.clk_divider = DT_INST_PROP_OR(n, clk_divider, 0),                                 \
		.clk_sel = DT_INST_PROP_OR(n, clk_sel, 0)};                                        \
                                                                                                   \
	static struct nxp_ewm_data nxp_ewm_data_##n;                                               \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, nxp_ewm_init, NULL, &nxp_ewm_data_##n, &nxp_ewm_config_##n,       \
			      POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &nxp_ewm_api);      \
                                                                                                   \
	static void nxp_ewm_config_func_##n(const struct device *dev)                              \
	{                                                                                          \
		ARG_UNUSED(dev);                                                                   \
                                                                                                   \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), nxp_ewm_isr,                \
