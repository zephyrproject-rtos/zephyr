...
struct iwdt_config {
	/* Devicetree-fixed timing, used only if setup() is called without install_timeout() first. */
...
#define IWDT_RA_SUPPORTED_FLAGS (WDT_FLAG_RESET_NONE | WDT_FLAG_RESET_SOC)
	...
	if (z_cfg->callback != NULL) {
		LOG_ERR("IWDT NMI is not enabled, IWDT callback Wont be called");
		return -ENOTSUP;
	}
#endif /* !CONFIG_WDT_RENESAS_RA_IWDT_NMI */
	...
	data->reset_control = z_cfg->callback != NULL ? WDT_RESET_CONTROL_NMI : WDT_RESET_CONTROL_RESET;
...
#error "CONFIG_WDT_RENESAS_RA_IWDT_REGISTER_START_MODE is selected, but this SoC's " \
	"IWDT does not implement register start mode: window, timeout and clock " \
	"division will be ignored at runtime"
#endif
