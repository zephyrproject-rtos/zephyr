...
static int wwdt_mspm0_feed(const struct device *dev, int channel_id)
{
	...
	((const struct wwdt_mspm0_config *)dev->config)->base->WWDTCNTRST = WWDT_CNTRST_KEY;
...
{
	struct wwdt_mspm0_regs *base = ((const struct wwdt_mspm0_config *)dev->config)->base;
...
DT_INST_FOREACH_STATUS_OKAY(WWDT_MSP_INIT)
