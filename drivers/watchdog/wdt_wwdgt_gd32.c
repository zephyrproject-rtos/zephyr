...
LOG_MODULE_REGISTER(wdt_wwdgt_gd32);

#define WWDGT_NODE DT_NODELABEL(wwdgt)

#define WWDGT_PRESCALER_EXPONENT_MAX    (3U)
 ...
 * @param prescaler_exp exponent part of prescaler
 *
 ...
 */
static inline uint32_t gd32_wwdgt_calc_ticks(uint32_t timeout, uint32_t prescaler_exp)
{
	const uint32_t pclk = rcu_clock_freq_get(CK_APB1);

	return ((timeout * pclk)
		/ (WWDGT_INTERNAL_DIVIDER * (1 << prescaler_exp) * MSEC_PER_SEC) - 1)
	       + WWDGT_COUNTER_MIN;
}
 ...
 *
 * @param window Pointer to timeout window struct.
 * @param counter Pointer to the storage of counter value.
 * @param window Pointer to the storage of window value.
 * @param prescaler Pointer to the storage of prescaler value.
 *
 * @return 0 on success, -EINVAL if the window-max is out of range
 */
static int gd32_wwdgt_convert_window(const struct wdt_window *win,
				     uint32_t *counter,
				     uint32_t *window,
				     uint32_t *prescaler)
{
	for (uint32_t shift = 0U; shift <= WWDGT_PRESCALER_EXPONENT_MAX; shift++) {
		uint32_t window_max_count = gd32_wwdgt_calc_ticks(win->max, shift);

		if (window_max_count <= WWDGT_COUNTER_MAX) {
			*counter = window_max_count;
			*prescaler = CFG_PSC(shift);
			if (win->min == 0U) {
				*window = window_max_count;
			} else {
				*window = gd32_wwdgt_calc_ticks(win->min, shift);
			}
				      ...
				      const struct wdt_timeout_cfg *config)
{
	...
	if (gd32_wwdgt_convert_window(&config->window, &counter, &window, &prescaler) != 0) {
...
{
	rcu_periph_clock_enable(DT_PROP(WWDGT_NODE, rcu_periph_clock));
...
DEVICE_DT_INST_DEFINE(0, gd32_wwdgt_init, NULL, &wwdgt_data, NULL,
