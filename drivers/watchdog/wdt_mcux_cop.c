...
static int mcux_cop_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	...
	if (config->clk_source < kCOP_LpoClock && config->clk_source > kCOP_BusClock) {
...
#endif
	data->cop_config.clockSource = config->clock_source;
...
static const struct mcux_cop_config mcux_cop_config = {
	.base = (COP_Type *)DT_INST_REG_ADDR(0),
