...
static void wdt_esp32_isr(const void *arg);
...
static int wdt_esp32_init(const struct device *dev)
{
	...
	IRQ_CONNECT(config->irq, IRQ_DEFAULT_PRIORITY, wdt_esp32_isr, dev, ESP_INTR_FLAG_IRAM);
...
static void IRAM_ATTR wdt_esp32_isr(const void *arg)
{
