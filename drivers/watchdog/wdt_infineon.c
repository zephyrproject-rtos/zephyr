...
static int ifx_cat1_wdt_setup(const struct device *dev, uint8_t options)
{
	...
	dev_data->wdt_ignore_bits = ifx_wdt_timeout_to_ignore_bits(&dev_data->timeout);
	dev_data->wdt_rounded_timeout_ms = dev_data->timeout;
	/* ifx_wdt_timeout_to_ignore_bits() returns the number of most-significant
	 * timer bits to ignore, which is exactly the argument Cy_WDT_SetIgnoreBits()
	 * expects. The PDL converts it to the hardware IGNORE_BITS_ABOVE field
	 * internally, so no manual match-bit conversion is needed here.
	 */
	Cy_WDT_SetIgnoreBits(dev_data->wdt_ignore_bits);
