 ...
 */
static int ot_aontimer_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
...
{
	IRQ_CONNECT(DT_INST_IRQ_BY_IDX(0, 0, irq), DT_INST_IRQ_BY_IDX(0, 0, priority), wdt_ot_isr,
...
DEVICE_DT_INST_DEFINE(0, ot_aontimer_init, NULL, &ot_aontimer_data, &ot_aontimer_cfg, PRE_KERNEL_1,
		      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &ot_aontimer_api);
