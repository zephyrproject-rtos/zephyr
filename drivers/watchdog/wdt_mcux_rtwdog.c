...
static int mcux_rtwdog_install_timeout(const struct device *dev, const struct wdt_timeout_cfg *cfg)
{
	...
	timeoutCycle = MSEC_TO_RTWDOG_TICKS(clock_freq, clk_divider, cfg->window.max);
	if (timeoutCycle > UINT16_MAX) {
...
static void mcux_rtwdog_isr(const struct device *dev)
{
	...
	flags = RTWDOG_GetStatusFlags(base) & (uint32_t)kRTWDOG_InterruptFlag;
