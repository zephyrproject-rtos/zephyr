...
static int scmi_core_protocol_setup(const struct device *transport)
{
	...
	STRUCT_SECTION_FOREACH(scmi_protocol, it) {
		...
		if (IS_ENABLED(CONFIG_ARM_SCMI_TRANSPORT_HAS_STATIC_CHANNELS)) {
			/* for transports with static channels TX needs to be
			 * bound at compilation time.
			 */
			if (!it->tx) {
				return -EINVAL;
			}
		} else {
			/* can we bind TX channel dynamically? */
			it->tx = scmi_transport_request_channel(transport,
								it->id, true);
			if (!it->tx) {
				return -EINVAL;
			}
