...
static int scmi_core_handle_notification(int hdr)
{
	...
	STRUCT_SECTION_START_EXTERN(scmi_protocol);
	STRUCT_SECTION_END_EXTERN(scmi_protocol);

	for (struct scmi_protocol *it = STRUCT_SECTION_START(scmi_protocol);
			it < STRUCT_SECTION_END(scmi_protocol); it++) {
		events = it->events;
...
static void scmi_core_reply_cb(struct scmi_channel *chan, int hdr)
{
	...
	case SCMI_DELAYED_REPLY:
		/* Avoid deadlock: unblock waiters even if delayed reply handling is TBD. */
		if (!k_is_pre_kernel()) {
			k_sem_give(&chan->sem);
		}
		break;
