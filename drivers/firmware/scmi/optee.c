 ...
 *
 * @tee_dev: Pointer to the underlying TEE device controller
				 ...
				 bool tx)
{
	...
	const struct device *const tee_dev = DEVICE_DT_GET(DT_INST_PHANDLE(0, linaro_optee));
	struct scmi_optee_channel *optee_chan = chan->data;
	int ret;

	if (!device_is_ready(tee_dev)) {
		LOG_ERR("OP-TEE device is not ready");
		return -ENODEV;
	}

	optee_chan->tee_dev = tee_dev;
...
static int invoke_process_msg_channel(struct scmi_optee_channel *optee_chan,
				      size_t msg_size)
{
	struct tee_invoke_func_arg arg = {
	...
	ret = tee_invoke_func(optee_chan->tee_dev, &arg, 3, param);
	if (ret < 0 || arg.ret != 0) {
				   ...
				   struct scmi_message *msg, bool use_polling)
{
	...
	ret = invoke_process_msg_channel(optee_chan, cmd_size);
	if (ret < 0) {
								   ...
								   struct scmi_message *msg)
{
	...
	if (optee_chan->rx_len < sizeof(struct scmi_optee_msg_layout)) {
		LOG_ERR("Invalid SCMI OP-TEE response length: %u", optee_chan->rx_len);
		return -EIO;
	}
	...
	if (msg->content && msg->len > 0 && rx_payload_len > 0) {
		msg->len = MIN(msg->len, rx_payload_len);
		memcpy(msg->content, layout->payload, msg->len);
	} else {
		msg->len = 0;
	}
