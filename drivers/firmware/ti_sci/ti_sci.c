						 ...
						 size_t rx_message_size)
{
	...
	hdr = (struct ti_sci_msg_hdr *)req_buf;
	hdr->seq = data->seq;
	hdr->type = msg_type;
...
static int ti_sci_get_response(const struct device *dev, struct ti_sci_xfer *xfer)
{
	...
	if (k_sem_take(&response_ready_sem, K_MSEC(dev_data->desc.max_rx_timeout_ms * 1000)) != 0) {
...
static int ti_sci_do_xfer(const struct device *dev, struct ti_sci_xfer *xfer)
{
	...
	if (xfer->rx_message.size) {
		return ti_sci_get_response(dev, xfer);
	}
...
int ti_sci_cmd_proc_request(const struct device *dev, uint8_t proc_id)
{
	...
	xfer = ti_sci_setup_one_xfer(dev, TISCI_MSG_PROC_REQUEST, TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (uint32_t *)&req, sizeof(req), NULL, 0);
	if (!xfer) {
		LOG_ERR("Failed to setup transfer");
		return -EINVAL;
	}
...
int ti_sci_cmd_get_revision(const struct device *dev)
{
	...
	xfer = ti_sci_setup_one_xfer(dev, TI_SCI_MSG_VERSION, TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     &hdr, sizeof(struct ti_sci_msg_hdr), &rev_info,
				     sizeof(rev_info));
...
int ti_sci_cmd_query_msmc(const struct device *dev, uint64_t *msmc_start, uint64_t *msmc_end)
{
	...
	xfer = ti_sci_setup_one_xfer(dev, TISCI_MSG_QUERY_MSMC, TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     (uint32_t *)&req, sizeof(req), &resp, sizeof(resp));
	if (!xfer) {
...
static int ti_sci_cmd_rm_irq_set_raw(const struct device *dev,
				     struct tisci_msg_rm_irq_set_req_wrapper *req)
{
	struct ti_sci_xfer *xfer;
	struct tisci_msg_rm_irq_set_resp resp;

	xfer = ti_sci_setup_one_xfer(dev, TISCI_MSG_RM_IRQ_SET, TI_SCI_FLAG_REQ_ACK_ON_PROCESSED,
				     req, sizeof(*req), &resp, sizeof(resp));
	int ret = ti_sci_do_xfer(dev, xfer);
	...
	if ((resp.hdr.flags & TI_SCI_FLAG_RESP_GENERIC_ACK) != TI_SCI_FLAG_RESP_GENERIC_ACK) {
		return -EIO;
	}
...
int ti_sci_cmd_rm_irq_set(const struct device *dev, struct tisci_msg_rm_irq_set_req *req)
{
	if (!dev || !req) {
		return -EINVAL;
	}
	...
	return ti_sci_cmd_rm_irq_set_raw(dev, &req_wrapped);
}

static int ti_sci_cmd_rm_irq_release_raw(const struct device *dev,
					 struct tisci_msg_rm_irq_release_req_wrapper *req)
{
	struct ti_sci_xfer *xfer;
	struct tisci_msg_rm_irq_release_resp resp;

	xfer = ti_sci_setup_one_xfer(dev, TISCI_MSG_RM_IRQ_RELEASE,
				     TI_SCI_FLAG_REQ_ACK_ON_PROCESSED, req, sizeof(*req), &resp,
				     sizeof(resp));
	int ret = ti_sci_do_xfer(dev, xfer);
...
int ti_sci_cmd_rm_irq_release(const struct device *dev, struct tisci_msg_rm_irq_release_req *req)
{
	if (!dev || !req) {
		return -EINVAL;
	}
	...
	return ti_sci_cmd_rm_irq_release_raw(dev, &req_wrapped);
}
