				...
				int32_t timeout)
{
	...
	sf32lb_dma_reload_dt(&config->tx_dma, (uintptr_t)(config->base + UART_TDR),
			     (uint32_t)data->async.tx.buf, data->async.tx.len);
...
static int uart_sf32lb_init(const struct device *dev)
{
	...
	//rx_dma_blk.source_address = (uint32_t)(config->base + UART_RDR);
	rx_dma_blk.source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
	//rx_dma_blk.dest_address = (uint32_t)data->async.rx.buf;
	...
	//tx_dma_blk.source_address = (uint32_t)buf;
	tx_dma_blk.source_addr_adj = DMA_ADDR_ADJ_INCREMENT;
	//tx_dma_blk.dest_address = (uint32_t)(config->base + UART_TDR);
