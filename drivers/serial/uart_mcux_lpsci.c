...
static void mcux_lpsci_poll_out(const struct device *dev, unsigned char c)
{
	...
	while (!(LPSCI_GetStatusFlags(config->base) & kLPSCI_TxDataRegEmptyFlag)) {
	}
...
static int mcux_lpsci_err_check(const struct device *dev)
{
	...
	LPSCI_ClearStatusFlags(config->base, kLPSCI_RxOverrunFlag | kLPSCI_ParityErrorFlag |
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int mcux_lpsci_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	const struct mcux_lpsci_config *config = dev->config;
	uint8_t num_tx = 0U;

	while ((len - num_tx > 0) &&
	       (LPSCI_GetStatusFlags(config->base) & kLPSCI_TxDataRegEmptyFlag)) {
...
static int mcux_lpsci_fifo_read(const struct device *dev, uint8_t *rx_data, const int len)
{
	const struct mcux_lpsci_config *config = dev->config;
	uint8_t num_rx = 0U;

	while ((len - num_rx > 0) &&
	       (LPSCI_GetStatusFlags(config->base) & kLPSCI_RxDataRegFullFlag)) {
...
static int mcux_lpsci_irq_rx_pending(const struct device *dev)
{
	...
	return (LPSCI_GetEnabledInterrupts(config->base) & mask) && mcux_lpsci_irq_rx_full(dev);
}

static void mcux_lpsci_irq_err_enable(const struct device *dev)
{
	const struct mcux_lpsci_config *config = dev->config;
	uint32_t mask = kLPSCI_NoiseErrorInterruptEnable | kLPSCI_FramingErrorInterruptEnable |
...
{
	return (mcux_lpsci_irq_tx_ready(dev) || mcux_lpsci_irq_rx_pending(dev));
}
...
static void mcux_lpsci_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
					void *cb_data)
{
...
#define MCUX_LPSCI_INIT(n)                                                                         \
	...
	static const struct mcux_lpsci_config mcux_lpsci_##n##_config;                             \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, mcux_lpsci_init, NULL, &mcux_lpsci_##n##_data,                    \
			      &mcux_lpsci_##n##_config, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY, \
