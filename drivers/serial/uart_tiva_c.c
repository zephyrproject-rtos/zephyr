...
/*
 * Bounded retry count while waiting for a UART peripheral to become ready.
 */
#define TIVA_C_UART_READY_RETRIES 10000U
...
static int uart_tiva_c_init(const struct device *dev)
{
	...
	uint32_t retries = TIVA_C_UART_READY_RETRIES;
	int ret;
	...
	while (!SysCtlPeripheralReady(TIVA_C_SYSCTL_PERIPH_UART(cfg->uart_index))) {
		if (retries-- == 0U) {
			return -ETIMEDOUT;
		}
	}
