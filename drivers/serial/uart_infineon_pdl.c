 ...
 * suspend. poll_in/out hold no ref (must not sleep) and instead guard against a
 * suspended block. The atomic per-slot bit keeps get/put race-free between the
 * opening thread and the closing ISR/DMA callback (which uses the async put);
 * get() returns whether this call actually took the ref so an error unwind only
 * releases what it acquired and never another session's ref. block_power adds a
				     ...
				     bool block_power)
{
	...
	/* Callable from an ISR (irq_tx/rx_enable): get() returns -EWOULDBLOCK
	 * there rather than blocking. Don't mark the slot held on failure, or
	 * the matching put would underflow the usage count.
	 */
...
static void ifx_cat1_uart_poll_out(const struct device *dev, unsigned char c)
{
	...
	/*
	 * A PM suspend may have clock-gated the block. The fatal error handler
	 * prints synchronously, so re-enable it here to keep the crash output.
	 */
	if (!data->scb_powered) {
		Cy_SCB_UART_Enable(config->reg_addr);
