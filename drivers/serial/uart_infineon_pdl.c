...
/* Called from the SCB ISR for every RX_NOT_EMPTY interrupt.
 *
 * The DMA drains the RX FIFO on the SCB RX trigger, but INTR_RX.NOT_EMPTY is latched when the
 * byte is written into the FIFO, so it still reports the arrival. The bit is only re-armed by
 * the clear in the ISR, so bytes arriving while it is already set are coalesced into a single
 * interrupt; the last byte of a burst always leaves the bit set and therefore always produces
 * an interrupt. That makes this a reliable timestamp for "the line was last active".
 *
...
#if defined(CONFIG_INFINEON_INTC_SYSINTC) && defined(CONFIG_UART_INTERRUPT_DRIVEN)
	irq_enable(config->irq_num);
#endif /* CONFIG_INFINEON_INTC_SYSINTC && CONFIG_UART_INTERRUPT_DRIVEN */
