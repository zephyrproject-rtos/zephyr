...
#include <zephyr/drivers/serial/uart_intel_lw.h>
		...
		/* flow control requires interrupt to work properly. */
		#error "uart_intel_lw.c: Must enable UART_INTERRUPT_DRIVEN for line control!"
	#endif
#endif
 ...
 * uart_drv_cmd with CMD_POLL_ASSERT_RTS before polling in.
 ...
 * Polling for data under normal operation, might cause unexpected behaviour.
 * If users wish to poll for data, please use the api:
 * uart_drv_cmd with CMD_POLL_ASSERT_RTS before polling out.
 * Then use CMD_POLL_DEASSERT_RTS to resume normal operation after polling.
...
static void uart_intel_lw_poll_out(const struct device *dev, unsigned char c)
{
	...
	do {
		...
		status = sys_read32(config->base + INTEL_LW_UART_STATUS_REG_OFFSET);
	} while ((status & INTEL_LW_UART_STATUS_TRDY_MSK) == 0);
				  ...
				  int size)
{
	...
	for (i = 0; i < size; i++) {
		if (data->status_act & INTEL_LW_UART_STATUS_TRDY_MSK) {
			sys_write32(tx_data[i], config->base
				  ...
				  const int size)
{
	...
	for (i = 0; i < size; i++) {
		if (data->status_act & INTEL_LW_UART_STATUS_RRDY_MSK) {
			rx_data[i] = sys_read32(config->base +
