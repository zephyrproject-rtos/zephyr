...
static inline int z_vrfy_uart_tx(const struct device *dev, const u8_t *buf,
...
static inline int z_vrfy_uart_rx_enable(const struct device *dev, u8_t *buf,
					size_t len, s32_t timeout)
{
...
static inline int z_vrfy_uart_drv_cmd(const struct device *dev, u32_t cmd,
				      u32_t p)
{
