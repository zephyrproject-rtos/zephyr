...
static int uart_mcux_configure(const struct device *dev, const struct uart_config *cfg)
{
...
static int uart_mcux_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct uart_mcux_data *data = dev->data;
...
static int uart_mcux_err_check(const struct device *dev)
{
	...
	UART_ClearStatusFlags(config->base,
			      kUART_RxOverrunFlag | kUART_ParityErrorFlag | kUART_FramingErrorFlag);

	return err;
}

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int uart_mcux_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	const struct uart_mcux_config *config = dev->config;
	uint8_t num_tx = 0U;
...
static int uart_mcux_fifo_read(const struct device *dev, uint8_t *rx_data, const int len)
{
	const struct uart_mcux_config *config = dev->config;
	uint8_t num_rx = 0U;
...
static int uart_mcux_irq_rx_pending(const struct device *dev)
{
	...
	return (UART_GetEnabledInterrupts(config->base) & mask) && uart_mcux_irq_rx_full(dev);
}

static void uart_mcux_irq_err_enable(const struct device *dev)
{
	const struct uart_mcux_config *config = dev->config;
	uint32_t mask = kUART_NoiseErrorInterruptEnable | kUART_FramingErrorInterruptEnable |
...
static void uart_mcux_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				       void *cb_data)
{
	struct uart_mcux_data *data = dev->data;
...
#define UART_MCUX_IRQ_INIT(n, name)                                                                \
	do {                                                                                       \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(n, name, irq),                                     \
			    DT_INST_IRQ_BY_NAME(n, name, priority), uart_mcux_isr,                 \
...
#define UART_MCUX_IRQ_CFG_FUNC_INIT(n) .irq_config_func = uart_mcux_config_func_##n
#define UART_MCUX_INIT_CFG(n)          UART_MCUX_DECLARE_CFG(n, UART_MCUX_IRQ_CFG_FUNC_INIT(n))
#else
#define UART_MCUX_CONFIG_FUNC(n)
#define UART_MCUX_IRQ_CFG_FUNC_INIT
#define UART_MCUX_INIT_CFG(n) UART_MCUX_DECLARE_CFG(n, UART_MCUX_IRQ_CFG_FUNC_INIT)
#endif
...
#define UART_MCUX_INIT(n)                                                                          \
	...
	PM_DEVICE_DT_INST_DEFINE(n, uart_mcux_pm_action);                                          \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, uart_mcux_init, PM_DEVICE_DT_INST_GET(n), &uart_mcux_##n##_data,  \
			      &uart_mcux_##n##_config, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,  \
