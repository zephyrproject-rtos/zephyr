...
struct uart_mspm0_line_config {
	...
	uint32_t word_length;  /* LCRH WLEN field */
	uint32_t stop_bits;    /* LCRH STP2 field */
	uint32_t flow_control; /* CTL0 RTSEN/CTSEN bits */
};
...
static int uart_mspm0_install_configuration(const struct device *dev)
{
	...
	uart_mspm0_configure_line(config->regs, data->uart_config.parity,
				  data->uart_config.word_length, data->uart_config.stop_bits,
				  data->uart_config.flow_control);
...
static int uart_mspm0_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	data->uart_config.stop_bits = value;
	...
	data->uart_config.word_length = value;
	...
	data->uart_config.flow_control = value;
...
static int uart_mspm0_config_get(const struct device *dev, struct uart_config *cfg)
{
	...
	ret = uart_mspm0_translate_out(uart_data_bits_to_mspm0, ARRAY_SIZE(uart_data_bits_to_mspm0),
				       data->uart_config.word_length, &cfg->data_bits);
	if (ret != 0) {
		return ret;
	}
	...
	ret = uart_mspm0_translate_out(uart_flow_control_to_mspm0,
				       ...
				       data->uart_config.flow_control, &cfg->flow_ctrl);
	if (ret != 0) {
		return ret;
	}
...
#define MSPM0_UART_INIT_FN(index)								\
	...
	static struct uart_mspm0_data uart_mspm0_data_##index = {                               \
		...
		.uart_config = {                                                                \
				.parity = 0,                                                    \
				.word_length = UART_LCRH_WLEN_DATABIT8,                         \
				.stop_bits = UART_LCRH_STP2_DISABLE,                            \
				.flow_control = (DT_INST_PROP(index, hw_flow_control)            \
