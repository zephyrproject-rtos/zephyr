...
#define HAS_RTO 1
#if HAS_LPUART
#define UART_INSTANCE_HAS_RTO_SET(usart)                                                           \
	(!IS_LPUART_INSTANCE(usart) && (LL_USART_GetRxTimeout(usart) != 0))
#else
#define UART_INSTANCE_HAS_RTO_SET(usart) (LL_USART_GetRxTimeout(usart) != 0)
#endif
#else
#define HAS_RTO 0
#endif
...
#endif /* USART_SR_RXNE */
#if HAS_RTO
	} else if (UART_INSTANCE_HAS_RTO_SET(config->usart) && LL_USART_IsEnabledIT_RTO(usart) &&
		   LL_USART_IsActiveFlag_RTO(usart)) {
...
#if HAS_RTO
	if (UART_INSTANCE_HAS_RTO_SET(usart)) {
...
static void set_timeout_itr(USART_TypeDef *usart, uint32_t baudrate, int32_t timeout)
{
	...
	LOG_DBG("timeout: %d us / %d bits", timeout, (uint32_t)timeout_in_bits);
