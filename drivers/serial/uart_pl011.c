...
static int pl011_set_baudrate(const struct device *dev, uint32_t clk, uint32_t baudrate)
{
...
static int pl011_runtime_configure_internal(const struct device *dev, const struct uart_config *cfg,
					    bool disable)
{
	struct pl011_data *data = dev->data;
	uint32_t lcrh;
...
static int pl011_runtime_configure(const struct device *dev, const struct uart_config *cfg)
{
	return pl011_runtime_configure_internal(dev, cfg, true);
}

static int pl011_runtime_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct pl011_data *data = dev->data;
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int pl011_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	uint8_t num_tx = 0U;
...
static int pl011_fifo_read(const struct device *dev, uint8_t *rx_data, const int len)
{
	uint8_t num_rx = 0U;
...
static int pl011_irq_rx_ready(const struct device *dev)
{
	...
	return ((get_uart(dev)->imsc & PL011_IMSC_RXIM) && (!(get_uart(dev)->fr & PL011_FR_RXFE)));
}
...
static void pl011_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				   void *cb_data)
{
	struct pl011_data *data = dev->data;
