...
#define GET_GECKO_USART_CLOCK(id) .clock = CLOCK_USART(DT_INST_PROP(id, peripheral_id)),
#define GET_GECKO_UART_CLOCK(id)  .clock = CLOCK_UART(DT_INST_PROP(id, peripheral_id)),
#else
#if (USART_COUNT == 1)
...
#define CLOCK_UART(ref)           (((ref) == UART0) ? cmuClock_UART0 : ((ref) == UART1) ? cmuClock_UART1 : -1)
#define GET_GECKO_USART_CLOCK(id) .clock = CLOCK_USART((USART_TypeDef *)DT_INST_REG_ADDR(id)),
#define GET_GECKO_UART_CLOCK(id)  .clock = CLOCK_UART((USART_TypeDef *)DT_INST_REG_ADDR(id)),
#endif /* DT_NODE_HAS_PROP(id, peripheral_id) */

/* Helper define to determine if SOC supports hardware flow control */
#if ((_SILICON_LABS_32B_SERIES > 0) ||                                                             \
     (defined(_USART_ROUTEPEN_RTSPEN_MASK) && defined(_USART_ROUTEPEN_CTSPEN_MASK)))
...
#define UART_GECKO_UART_HW_FLOW_CONTROL_ENABLED DT_INST_FOREACH_STATUS_OKAY(HAS_HFC_OR) 0

#undef DT_DRV_COMPAT
#define DT_DRV_COMPAT silabs_gecko_usart

/* Has any enabled usart instance hw-flow-control enabled? */
#define UART_GECKO_USART_HW_FLOW_CONTROL_ENABLED DT_INST_FOREACH_STATUS_OKAY(HAS_HFC_OR) 0

#if UART_GECKO_USART_HW_FLOW_CONTROL_ENABLED || UART_GECKO_UART_HW_FLOW_CONTROL_ENABLED
#define UART_GECKO_HW_FLOW_CONTROL
#endif

/* Sanity check for hardware flow control */
#if defined(UART_GECKO_HW_FLOW_CONTROL) && (!(defined(HW_FLOWCONTROL_IS_SUPPORTED_BY_SOC)))
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int uart_gecko_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	const struct uart_gecko_config *config = dev->config;
	uint8_t num_tx = 0U;
...
static int uart_gecko_fifo_read(const struct device *dev, uint8_t *rx_data, const int len)
{
	const struct uart_gecko_config *config = dev->config;
	uint8_t num_rx = 0U;
...
static int uart_gecko_irq_rx_ready(const struct device *dev)
{
	...
	return (config->base->IEN & mask) && uart_gecko_irq_rx_full(dev);
}
...
static void uart_gecko_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
					void *cb_data)
{
...
static void uart_gecko_init_pins(const struct device *dev)
{
	...
	config->base->ROUTE = USART_ROUTE_RXPEN | USART_ROUTE_TXPEN | (config->loc << 8);
	...
	if (config->hw_flowcontrol) {
		GPIO_PinModeSet(config->pin_rts.port, config->pin_rts.pin, config->pin_rts.mode,
				config->pin_rts.out);
		GPIO_PinModeSet(config->pin_cts.port, config->pin_cts.pin, config->pin_cts.mode,
				config->pin_cts.out);
		...
		config->base->ROUTELOC1 = (config->loc_rts << _USART_ROUTELOC1_RTSLOC_SHIFT) |
...
static int uart_gecko_init(const struct device *dev)
{
	...
	usartInit.hwFlowControl =
		config->hw_flowcontrol ? usartHwFlowControlCtsAndRts : usartHwFlowControlNone;
#endif
...
#define GECKO_UART_IRQ_HANDLER_FUNC(idx) .irq_config_func = uart_gecko_config_func_##idx,
#define GECKO_UART_IRQ_HANDLER(idx)                                                                \
	static void uart_gecko_config_func_##idx(const struct device *dev)                         \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(idx, rx, irq),                                     \
			    DT_INST_IRQ_BY_NAME(idx, rx, priority), uart_gecko_isr,                \
			    DEVICE_DT_INST_GET(idx), 0);                                           \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(idx, tx, irq),                                     \
			    DT_INST_IRQ_BY_NAME(idx, tx, priority), uart_gecko_isr,                \
			    DEVICE_DT_INST_GET(idx), 0);                                           \
                                                                                                   \
...
#else
#define GECKO_UART_RX_TX_PIN_LOCATIONS(idx) .loc = DT_INST_PROP_BY_IDX(idx, location_rx, 0),
#define VALIDATE_GECKO_UART_RX_TX_PIN_LOCATIONS(idx)                                               \
...
#define PIN_UART_RXD(idx)                                                                          \
	{DT_INST_PROP_BY_IDX(idx, location_rx, 1), DT_INST_PROP_BY_IDX(idx, location_rx, 2),       \
	 gpioModeInput, 1}
#define PIN_UART_TXD(idx)                                                                          \
	{DT_INST_PROP_BY_IDX(idx, location_tx, 1), DT_INST_PROP_BY_IDX(idx, location_tx, 2),       \
	 gpioModePushPull, 1}

#define GECKO_UART_RX_TX_PINS(idx) .pin_rx = PIN_UART_RXD(idx), .pin_tx = PIN_UART_TXD(idx),
...
#define GECKO_UART_RTS_CTS_PINS(idx) .pin_rts = PIN_UART_RTS(idx), .pin_cts = PIN_UART_CTS(idx),

#define GECKO_UART_HW_FLOW_CONTROL(idx) .hw_flowcontrol = DT_INST_PROP(idx, hw_flow_control),
...
#define GECKO_UART_INIT(idx)                                                                       \
	...
	static const struct uart_gecko_config uart_gecko_cfg_##idx = {                             \
		...
		GET_GECKO_UART_CLOCK(idx).baud_rate = DT_INST_PROP(idx, current_speed),            \
		GECKO_UART_HW_FLOW_CONTROL(idx) GECKO_UART_RX_TX_PINS(idx)                         \
			GECKO_UART_RTS_CTS_PINS(idx) GECKO_UART_RX_TX_PIN_LOCATIONS(idx)           \
				GECKO_UART_RTS_CTS_PIN_LOCATIONS(idx)                              \
					GECKO_UART_IRQ_HANDLER_FUNC(idx)};                         \
                                                                                                   \
	static struct uart_gecko_data uart_gecko_data_##idx;                                       \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(idx, uart_gecko_init, NULL, &uart_gecko_data_##idx,                  \
			      &uart_gecko_cfg_##idx, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,    \
...
#define GECKO_USART_IRQ_HANDLER_FUNC(idx) .irq_config_func = usart_gecko_config_func_##idx,
...
#define GECKO_USART_INIT(idx)                                                                      \
	...
	static const struct uart_gecko_config usart_gecko_cfg_##idx = {                            \
		...
		.base = (USART_TypeDef *)DT_INST_REG_ADDR(idx),                                    \
		GET_GECKO_USART_CLOCK(idx).baud_rate = DT_INST_PROP(idx, current_speed),           \
		GECKO_USART_IRQ_HANDLER_FUNC(idx)};                                                \
                                                                                                   \
	static struct uart_gecko_data usart_gecko_data_##idx;                                      \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(idx, uart_gecko_init, NULL, &usart_gecko_data_##idx,                 \
			      &usart_gecko_cfg_##idx, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,   \
			      &uart_gecko_driver_api);                                             \
                                                                                                   \
