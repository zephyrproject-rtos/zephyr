 ...
 */
static bool eusart_pm_lock_get(const struct device *dev,
					   enum eusart_pm_lock lock)
{
#ifdef CONFIG_PM
 ...
 */
static bool eusart_pm_lock_put(const struct device *dev,
					   enum eusart_pm_lock lock)
{
#ifdef CONFIG_PM
	struct eusart_data *data = dev->data;
...
static void eusart_irq_tx_enable(const struct device *dev)
{
	...
	(void)eusart_pm_lock_get(dev, EUSART_PM_LOCK_TX);
...
static void eusart_irq_tx_disable(const struct device *dev)
{
	...
	EUSART_IntClear(config->eusart, EUSART_IEN_TXFL | EUSART_IEN_TXC);
	(void)eusart_pm_lock_put(dev, EUSART_PM_LOCK_TX);
}
...
static void eusart_irq_rx_enable(const struct device *dev)
{
	...
	(void)eusart_pm_lock_get(dev, EUSART_PM_LOCK_RX);
...
static void eusart_irq_rx_disable(const struct device *dev)
{
	...
	EUSART_IntClear(config->eusart, EUSART_IEN_RXFL);
	(void)eusart_pm_lock_put(dev, EUSART_PM_LOCK_RX);
}
...
#ifdef CONFIG_UART_SILABS_EUSART_ASYNC
static inline void eusart_async_user_callback(struct eusart_data *data,
...
static inline void eusart_async_timer_start(struct k_work_delayable *work,
	int32_t timeout)
{
...
static inline void eusart_async_evt_rx_rdy(struct eusart_data *data)
{
	struct uart_event event = {
		.type = UART_RX_RDY,
...
static inline void eusart_async_evt_tx_done(struct eusart_data *data)
{
	struct uart_event event = {
		.type = UART_TX_DONE,
...
static inline void eusart_async_evt_tx_abort(struct eusart_data *data)
{
	struct uart_event event = {
...
static inline void eusart_async_evt_rx_err(struct eusart_data *data, int err_code)
{
	struct uart_event event = {
...
static inline void async_evt_rx_buf_release(struct eusart_data *data)
{
...
static inline void async_evt_rx_buf_request(struct eusart_data *data)
{
	struct uart_event evt = {
...
void eusart_dma_rx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
				  int status)
{
...
void eusart_dma_tx_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
				  int status)
{
	const struct device *uart_dev = user_data;
	struct eusart_data *data = uart_dev->data;
...
static int eusart_async_tx(const struct device *dev, const uint8_t *tx_data,
...
static int eusart_async_rx_enable(const struct device *dev, uint8_t *rx_buf,
					      size_t buf_size, int32_t timeout)
{
	const struct eusart_config *config = dev->config;
	struct eusart_data *data = dev->data;
...
static void eusart_async_rx_timeout(struct k_work *work)
{
	...
	struct eusart_data *data = CONTAINER_OF(rx_channel,
			struct eusart_data, dma_rx);
...
static void eusart_async_tx_timeout(struct k_work *work)
{
	...
	struct uart_dma_channel *tx_channel = CONTAINER_OF(dwork,
			struct uart_dma_channel, timeout_work);
	struct eusart_data *data = CONTAINER_OF(tx_channel,
			struct eusart_data, dma_tx);
...
static int eusart_async_init(const struct device *dev)
{
	...
	k_work_init_delayable(&data->dma_rx.timeout_work,
		eusart_async_rx_timeout);
	k_work_init_delayable(&data->dma_tx.timeout_work,
		eusart_async_tx_timeout);
...
static inline EUSART_Parity_TypeDef eusart_cfg2ll_parity(enum uart_config_parity parity)
{
	switch (parity) {
...
static inline EUSART_Stopbits_TypeDef eusart_cfg2ll_stopbits(enum uart_config_stop_bits sb)
{
	switch (sb) {
...
static inline EUSART_Databits_TypeDef eusart_cfg2ll_databits(enum uart_config_data_bits db,
...
static inline EUSART_HwFlowControl_TypeDef eusart_cfg2ll_hwctrl(enum uart_config_flow_control fc)
{
...
static void eusart_configure_peripheral(const struct device *dev, bool enable)
{
	...
	eusartInit.parity = eusart_cfg2ll_parity(data->uart_cfg->parity);
	eusartInit.stopbits = eusart_cfg2ll_stopbits(data->uart_cfg->stop_bits);
	eusartInit.databits =
		eusart_cfg2ll_databits(data->uart_cfg->data_bits, data->uart_cfg->parity);
	advancedSettings.hwFlowControl = eusart_cfg2ll_hwctrl(data->uart_cfg->flow_ctrl);
