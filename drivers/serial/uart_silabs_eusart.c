...
void eusart_dma_rx_cb(const struct device *dma_dev, void *user_data, uint32_t channel, int status)
{
...
void eusart_dma_tx_cb(const struct device *dma_dev, void *user_data, uint32_t channel, int status)
{
	const struct device *uart_dev = user_data;
	struct eusart_data *data = uart_dev->data;
...
static void eusart_async_rx_timeout(struct k_work *work)
{
	...
	struct uart_dma_channel *rx_channel =
		CONTAINER_OF(dwork, struct uart_dma_channel, timeout_work);
...
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct uart_dma_channel *tx_channel =
		CONTAINER_OF(dwork, struct uart_dma_channel, timeout_work);
