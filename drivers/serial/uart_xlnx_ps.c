...
static void set_baudrate(struct device *dev, u32_t baud_rate)
{
	...
	if ((baud != 0) && (clk_freq != 0)) {
		 ...
		 * -> This function is always called from a context in which the
		 * receiver/transmitter is disabled, the baud rate can be changed
		 * safely at this time.
		 */
 ...
 *
 * @param mode_reg Mode Register contents to which the parity
 *                 configuration shall be added.
 * @param parity   Enumeration value to be converted to a bit mask.
 *
 * @return Mode Register contents to which the parity setting has
 *         been added.
 */
static inline u32_t uart_xlnx_ps_cfg2ll_parity(
	u32_t mode_reg,
 ...
 * this function already masked out by the caller.
 *
 * @param mode_reg Mode Register contents to which the stop bit
 *                 configuration shall be added.
 * @param stopbits Enumeration value to be converted to a bit mask.
 *
 * @return Mode Register contents to which the stop bit setting
 *         has been added.
 */
static inline u32_t uart_xlnx_ps_cfg2ll_stopbits(
	u32_t mode_reg,
 ...
 * this function already masked out by the caller.
 *
 * @param mode_reg Mode Register contents to which the data bit
 *                 configuration shall be added.
 * @param databits Enumeration value to be converted to a bit mask.
 *
 * @return Mode Register contents to which the data bit setting
 *         has been added.
 */
static inline u32_t uart_xlnx_ps_cfg2ll_databits(
	u32_t mode_reg,
 ...
 *
 * @param modemcr_reg Modem Control Register contents to which
 *                    the flow control configuration shall be added.
 * @param hwctrl Enumeration value to be converted to a bit mask.
 *
 * @return Modem Control Register contents to which the data bit
 *         setting has been added.
 */
static inline u32_t uart_xlnx_ps_cfg2ll_hwctrl(
	u32_t modemcr_reg,
	enum uart_config_flow_control hwctrl)
{
	...
	if (UART_CFG_FLOW_CTRL_RTS_CTS == hwctrl) {
		modemcr_reg |= XUARTPS_MODEMCR_FCM_RTS_CTS;
	} else { /* effectively: UART_CFG_FLOW_CTRL_NONE */
	...
	const struct uart_config *cfg)
{
	...
	/* Controller supports only RTS/CTS flow control */
	...
	/* Assemble the updated registers */

	mode_reg    = uart_xlnx_ps_cfg2ll_parity(mode_reg, cfg->parity);
	mode_reg    = uart_xlnx_ps_cfg2ll_stopbits(mode_reg, cfg->stop_bits);
	mode_reg    = uart_xlnx_ps_cfg2ll_databits(mode_reg, cfg->data_bits);
	modemcr_reg = uart_xlnx_ps_cfg2ll_hwctrl(modemcr_reg, cfg->flow_ctrl);
	...
	u32_t modemcr_reg)
{
	...
	if (XUARTPS_MODEMCR_FCM_RTS_CTS
		== (modemcr_reg & XUARTPS_MODEMCR_FCM_MASK)) {
