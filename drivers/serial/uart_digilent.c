...
#define DT_DRV_COMPAT digi_16550
...
#define DEV_CFG(dev) \
	((const struct uart_digi_ns16550_device_config * const) \
	 (dev)->config)
#define DEV_DATA(dev) \
	((struct uart_digi_ns16550_dev_data_t *)(dev)->data)
...
#endif
#endif /* UART_DIGI_NS16550_ACCESS_IOPORT */

#ifdef CONFIG_UART_DIGI_NS16550_ACCESS_WORD_ONLY
#undef INBYTE
...
/* device config */
struct uart_digi_ns16550_device_config {
#ifndef UART_DIGI_NS16550_ACCESS_IOPORT
	DEVICE_MMIO_ROM;
#else
	uint32_t port;
#endif
...
struct uart_digi_ns16550_dev_data_t {
#ifndef UART_DIGI_NS16550_ACCESS_IOPORT
	DEVICE_MMIO_RAM;
#endif
...
static const struct uart_driver_api uart_digi_ns16550_driver_api;
...
{
	const struct uart_digi_ns16550_device_config * const dev_cfg = DEV_CFG(dev);
...
static int uart_digi_ns16550_configure(const struct device *dev,
...
#ifdef CONFIG_UART_DIGI_NS16750
...
static int uart_digi_ns16550_config_get(const struct device *dev,
				   struct uart_config *cfg)
{
	struct uart_digi_ns16550_dev_data_t *data = DEV_DATA(dev);
 ...
 */
static int uart_digi_ns16550_init(const struct device *dev)
{
	int ret;

	ret = uart_digi_ns16550_configure(dev, &DEV_DATA(dev)->uart_config);
	if (ret != 0) {
		return ret;
	}
 ...
 */
static int uart_digi_ns16550_poll_in(const struct device *dev, unsigned char *c)
{
	int ret = -1;
 ...
 */
static void uart_digi_ns16550_poll_out(const struct device *dev,
					   unsigned char c)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 */
static int uart_digi_ns16550_err_check(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 */
static int uart_digi_ns16550_fifo_fill(const struct device *dev,
 ...
 */
static int uart_digi_ns16550_fifo_read(const struct device *dev, uint8_t *rx_data,
				  const int size)
{
	int i;
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return N/A
 */
static void uart_digi_ns16550_irq_tx_enable(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return N/A
 */
static void uart_digi_ns16550_irq_tx_disable(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 */
static int uart_digi_ns16550_irq_tx_ready(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 */
static int uart_digi_ns16550_irq_tx_complete(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return N/A
 */
static void uart_digi_ns16550_irq_rx_enable(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return N/A
 */
static void uart_digi_ns16550_irq_rx_disable(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return 1 if an IRQ is ready, 0 otherwise
 */
static int uart_digi_ns16550_irq_rx_ready(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return 1 if an IRQ is ready, 0 otherwise
 */
static void uart_digi_ns16550_irq_err_disable(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 */
static int uart_digi_ns16550_irq_is_pending(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return Always 1
 */
static int uart_digi_ns16550_irq_update(const struct device *dev)
{
	k_spinlock_key_t key = k_spin_lock(&DEV_DATA(dev)->lock);
 ...
 * @return N/A
 */
static void uart_digi_ns16550_irq_callback_set(const struct device *dev,
 ...
 * @return N/A
 */
static void uart_digi_ns16550_isr(const struct device *dev)
{
	struct uart_digi_ns16550_dev_data_t * const dev_data = DEV_DATA(dev);
...
#ifdef CONFIG_UART_DIGI_NS16550_LINE_CTRL
 ...
 * @return 0 if successful, failed otherwise
 */
static int uart_digi_ns16550_line_ctrl_set(const struct device *dev,
...
#endif /* CONFIG_UART_DIGI_NS16550_LINE_CTRL */

static const struct uart_driver_api uart_digi_ns16550_driver_api = {
	.poll_in = uart_digi_ns16550_poll_in,
	.poll_out = uart_digi_ns16550_poll_out,
	.err_check = uart_digi_ns16550_err_check,
	.configure = uart_digi_ns16550_configure,
	.config_get = uart_digi_ns16550_config_get,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN

	.fifo_fill = uart_digi_ns16550_fifo_fill,
	.fifo_read = uart_digi_ns16550_fifo_read,
	.irq_tx_enable = uart_digi_ns16550_irq_tx_enable,
	.irq_tx_disable = uart_digi_ns16550_irq_tx_disable,
	.irq_tx_ready = uart_digi_ns16550_irq_tx_ready,
	.irq_tx_complete = uart_digi_ns16550_irq_tx_complete,
	.irq_rx_enable = uart_digi_ns16550_irq_rx_enable,
	.irq_rx_disable = uart_digi_ns16550_irq_rx_disable,
	.irq_rx_ready = uart_digi_ns16550_irq_rx_ready,
	.irq_err_enable = uart_digi_ns16550_irq_err_enable,
	.irq_err_disable = uart_digi_ns16550_irq_err_disable,
	.irq_is_pending = uart_digi_ns16550_irq_is_pending,
	.irq_update = uart_digi_ns16550_irq_update,
	.irq_callback_set = uart_digi_ns16550_irq_callback_set,

#endif

#ifdef CONFIG_UART_DIGI_NS16550_LINE_CTRL
	.line_ctrl_set = uart_digi_ns16550_line_ctrl_set,
#endif

};

#include <uart_digilent_port_0.h>
