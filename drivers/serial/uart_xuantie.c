...
#define DEV_UART_HANDLE(dev)						\
	((csi_uart_t *)&(((const struct uart_xuantie_device_config * const)(dev)->config)->uart_handle))
...
static csi_error_t _csi_uart_format(csi_uart_t *uart,  csi_uart_data_bits_t data_bits,
...
#if 0
static uint8_t _csi_uart_getc(csi_uart_t *uart)
{
...
static int uart_xuantie_irq_rx_ready(const struct device *dev)
{
	...
	int ret = ((intr_state == DW_UART_IIR_IID_RECV_DATA_AVAIL) || (intr_state == DW_UART_IIR_IID_CHARACTER_TIMEOUT)) ? 1 : 0;
#endif
 ...
 */
static void uart_xuantie_irq_callback_set(const struct device *dev,
