					     ...
					     uint32_t channel, int status)
{
	...
	if (data->async.rx_counter < data->async.rx_len &&
		...
		data->async.rx_dma_offset = data->async.rx_counter;
...
static void uart_esp32_async_rx_timeout(struct k_work *work)
{
	...
	size_t snapshot_dma_offset = data->async.rx_dma_offset;
	size_t rx_count;
	unsigned int key;

	/*
	 * Get actual transferred bytes from DMA descriptor.
	 * In DMA mode, data goes directly to memory via UHCI+GDMA. The UART
	 * FIFO interrupt only schedules this work; the DMA descriptor counts
	 * actual bytes. A partial, forever-timeout completion may have reloaded
	 * the DMA into the middle of the buffer.
	 */
	if (dma_get_status(config->dma_dev, config->rx_dma_channel, &dma_status) == 0) {
		rx_count = snapshot_dma_offset + dma_status.total_copied;
	} else {
	...
	if (data->async.rx_generation != snapshot_generation ||
	    data->async.rx_dma_offset != snapshot_dma_offset ||
...
static int uart_esp32_async_rx_disable(const struct device *dev)
{
	...
	rx_count = data->async.rx_counter;
	if (dma_get_status(config->dma_dev, config->rx_dma_channel, &dma_status) == 0) {
		rx_count = MAX(rx_count, data->async.rx_dma_offset + dma_status.total_copied);
	}
	if (rx_count > data->async.rx_len) {
		rx_count = data->async.rx_len;
	}
