...
static int rts5912_uart_init(const struct device *dev)
{
	...
	const uint32_t uart_reg = uart_cfg->port;
	uint32_t iir = sys_read32(uart_reg + REG_IIR) & 0x0f;
	...
	while (iir != IIR_NIP) {
		...
		case IIR_RBRF: /* Received data available*/
			sys_write32(sys_read32(uart_reg + REG_IIR) | IIR_THRE, uart_reg + REG_IIR);
			break;
		case IIR_MASK:
			while (1) {
		...
		iir = sys_read32(uart_reg + REG_IIR) & 0x0f;
	}
