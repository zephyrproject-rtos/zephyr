...
#define ADDR_IIR                             (uart_reg + REG_IIR)
...
static int rts5912_uart_init(const struct device *dev)
{
	...
	while (wf_cycle_count > (wf_now - wf_start)) {
		uint32_t iir = sys_read32(ADDR_IIR) & IIR_MASK;
		...
		case IIR_RBRF: /* Received data available*/
			sys_write32(sys_read32(ADDR_IIR) | IIR_THRE, ADDR_IIR);
			break;
		case IIR_BUSY:
			while (wf_cycle_count > (wf_now - wf_start)) {
		...
		iir = sys_read32(ADDR_IIR) & IIR_MASK;
		if (iir == IIR_NIP) {
			break;
		}
		k_busy_wait(10);
