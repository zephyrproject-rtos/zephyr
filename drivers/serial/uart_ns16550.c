...
#define REG_RDR 0x00U  /* Receiver data reg.             */
...
#define REG_IIR 0x02U  /* Interrupt ID reg.              */
#define REG_FCR 0x02U  /* FIFO control reg.              */
#define REG_LCR 0x03U  /* Line control reg.              */
#define REG_MDC 0x04U  /* Modem control reg.             */
#define REG_LSR 0x05U  /* Line status reg.               */
#define REG_MSR 0x06U  /* Modem status reg.              */
#define REG_DLF 0xC0U  /* Divisor Latch Fraction         */
#define REG_PCP 0x200U /* PRV_CLOCK_PARAMS (Apollo Lake) */
...
static void set_baud_rate(const struct device *dev, uint32_t baud_rate)
{
	...
	if ((baud_rate != 0U) && (dev_cfg->sys_clk_freq != 0U)) {
		...
		OUTBYTE(BRDL(dev), (uint8_t)(divisor & 0xffU));
		OUTBYTE(BRDH(dev), (uint8_t)((divisor >> 8) & 0xffU));
...
static int uart_ns16550_poll_in(const struct device *dev, unsigned char *c)
{
	...
	if ((INBYTE(LSR(dev)) & LSR_RXRDY) != 0U) {
		/* got a character */
		*c = INBYTE(RDR(dev));
		ret = 0;
	}
					   ...
					   unsigned char c)
{
	...
	while ((INBYTE(LSR(dev)) & LSR_THRE) == 0U) {
	}
