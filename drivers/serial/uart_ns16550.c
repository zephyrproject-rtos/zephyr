...
#define THR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_THR * UART_REG_ADDR_INTERVAL)
#define RDR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_RDR * UART_REG_ADDR_INTERVAL)
#define BRDL(dev) \
	(DEV_DATA(dev)->devconf.port + REG_BRDL * UART_REG_ADDR_INTERVAL)
#define BRDH(dev) \
	(DEV_DATA(dev)->devconf.port + REG_BRDH * UART_REG_ADDR_INTERVAL)
#define IER(dev) \
	(DEV_DATA(dev)->devconf.port + REG_IER * UART_REG_ADDR_INTERVAL)
#define IIR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_IIR * UART_REG_ADDR_INTERVAL)
#define FCR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_FCR * UART_REG_ADDR_INTERVAL)
#define LCR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_LCR * UART_REG_ADDR_INTERVAL)
#define MDC(dev) \
	(DEV_DATA(dev)->devconf.port + REG_MDC * UART_REG_ADDR_INTERVAL)
#define LSR(dev) (\
	DEV_DATA(dev)->devconf.port + REG_LSR * UART_REG_ADDR_INTERVAL)
#define MSR(dev) \
	(DEV_DATA(dev)->devconf.port + REG_MSR * UART_REG_ADDR_INTERVAL)
#define DLF(dev) (DEV_DATA(dev)->devconf.port + REG_DLF)
#define PCP(dev) (DEV_DATA(dev)->devconf.port + REG_PCP)
...
static int uart_ns16550_init(struct device *dev)
{
	...
	{
		/*
 		* Set port to DEV_DATA, but not overwrite for PCIE enabled
 		* set by pcie_get_mbar() via _configure() above
 		*/
		DEV_DATA(dev)->devconf.port = DEV_CFG(dev)->devconf.port;
	}
