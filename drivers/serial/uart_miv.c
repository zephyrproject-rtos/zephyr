...
#define DEV_UART(dev)                                                                              \
	((struct uart_miv_regs_t *)((const struct uart_miv_device_config *const)(dev)->config)     \
		 ->uart_addr)

static void uart_miv_poll_out(const struct device *dev, unsigned char c)
{
...
static int uart_miv_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	volatile struct uart_miv_regs_t *uart = DEV_UART(dev);
	int i;
...
static int uart_miv_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	volatile struct uart_miv_regs_t *uart = DEV_UART(dev);
	int i;
...
static void uart_miv_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				      void *cb_data)
{
	struct uart_miv_data *data = dev->data;
...
BUILD_ASSERT(DT_NUM_INST_STATUS_OKAY(DT_DRV_COMPAT) <= 1, "unsupported uart_miv instance");
...
DEVICE_DT_INST_DEFINE(0, uart_miv_init, NULL, &uart_miv_data_0, &uart_miv_dev_cfg_0, PRE_KERNEL_1,
		      CONFIG_SERIAL_INIT_PRIORITY, (void *)&uart_miv_driver_api);
...
static void uart_miv_irq_cfg_func_0(const struct device *dev)
{
	...
	k_thread_create(&rx_thread, rx_stack, 500, uart_miv_rx_thread, data, NULL, NULL,
