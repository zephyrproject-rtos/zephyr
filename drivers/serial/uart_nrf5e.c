...
static void uart_nrf5_fifo_rx_setup(struct device *dev);
static void uart_nrf5_pump_tx_fifo(struct device *dev);

static void uart_nrf5_mask(struct device *dev, int mask)
{
...
static int uart_nrf5_init(struct device *dev)
{
	...
	uart_nrf5_mask(dev, 0);
	uart_nrf5_fifo_rx_setup(dev);
#endif
...
#else
	if (uart_nrf5_fifo_read(dev, c, 1)) {
		return 0;
	}
	return -1;
#endif
					...
					unsigned char c)
{
	...
	uart_nrf5_mask(dev, 1);
	struct uart_nrf5_dev_data_t * const dev_data = DEV_DATA(dev);

	while (!uart_nrf5_fifo_fill(dev, &c, 1)) {
		if (dev_data->in_interrupt) {
			uart_nrf5_pump_tx_fifo(dev);
		}
	}
...
static void uart_nrf5_fifo_tx_start(struct device *dev)
{
	struct uart_nrf5_dev_data_t * const dev_data = DEV_DATA(dev);
	volatile struct _uart *uart = UART_STRUCT(dev);
...
static int uart_nrf5_fifo_read(struct device *dev, u8_t *rx_data, int size)
{
	struct uart_nrf5_dev_data_t * const dev_data = DEV_DATA(dev);
	u8_t num_rx = 0;
...
static void uart_nrf5_pump_rx_fifo(struct device *dev)
{
	volatile struct _uart *uart = UART_STRUCT(dev);
	struct uart_nrf5_dev_data_t * const dev_data = DEV_DATA(dev);
...
void uart_nrf5_isr(void *arg)
{
	...
	uart_nrf5_pump_tx_fifo(dev);
	uart_nrf5_pump_rx_fifo(dev);

	if (dev_data->cb &&
		((dev_data->rx_irq_enable && uart_nrf5_irq_rx_ready(dev)) ||
		 (dev_data->tx_irq_enable && uart_nrf5_irq_tx_ready(dev))) {
		dev_data->cb(dev);
	}
...
static const struct uart_driver_api uart_nrf5_driver_api = {
	/** Console I/O function */
	.poll_in		= uart_nrf5_poll_in,
	/** Console I/O function */
	.poll_out		= uart_nrf5_poll_out,
	/** Console I/O function */
	.err_check		= uart_nrf5_err_check,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	/** IRQ FIFO fill function */
	.fifo_fill		= uart_nrf5_fifo_fill,
	/** IRQ FIFO read function */
	.fifo_read		= uart_nrf5_fifo_read,
	 /** IRQ transfer enabling function */
	.irq_tx_enable		= uart_nrf5_irq_tx_enable,
	/** IRQ transfer disabling function */
	.irq_tx_disable		= uart_nrf5_irq_tx_disable,
	/** IRQ transfer ready function */
	.irq_tx_ready		= uart_nrf5_irq_tx_ready,
	/** IRQ receiver enabling function */
	.irq_rx_enable		= uart_nrf5_irq_rx_enable,
	/** IRQ receiver disabling function */
	.irq_rx_disable		= uart_nrf5_irq_rx_disable,
	/** IRQ transfer complete function */
	.irq_tx_complete	= uart_nrf5_irq_tx_complete,
	/** IRQ receiver ready function */
	.irq_rx_ready		= uart_nrf5_irq_rx_ready,
	/** IRQ error enabling function */
	.irq_err_enable		= uart_nrf5_irq_err_enable,
	/** IRQ error disabling function */
	.irq_err_disable	= uart_nrf5_irq_err_disable,
	/** IRQ pending status function */
	.irq_is_pending		= uart_nrf5_irq_is_pending,
	/** IRQ interrupt update function */
	.irq_update		= uart_nrf5_irq_update,
	/** Set the callback function */
	.irq_callback_set	= uart_nrf5_irq_callback_set,
#endif /* CONFIG_UART_INTERRUPT_DRIVEN */
};
