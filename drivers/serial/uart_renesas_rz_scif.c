...
static int uart_rz_scif_apply_config(const struct device *dev)
{
	...
	memcpy(fsp_extend->p_baud_setting, &baud_setting, sizeof(scif_baud_setting_t));
	...
	case UART_CFG_FLOW_CTRL_NONE:
		fsp_extend->flow_control = SCIF_UART_FLOW_CONTROL_NONE;
		fsp_extend->uart_mode = SCIF_UART_MODE_RS232;
		fsp_extend->rs485_setting.enable = 0;
		break;
	case UART_CFG_FLOW_CTRL_RTS_CTS:
		fsp_extend->flow_control = SCIF_UART_FLOW_CONTROL_AUTO;
		fsp_extend->uart_mode = SCIF_UART_MODE_RS232;
		fsp_extend->rs485_setting.enable = 0;
		break;
	default:
