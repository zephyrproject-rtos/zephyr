...
struct uart_ra_cfg {
	...
	struct clock_control_ra_subsys_cfg clock_id;
...
static int uart_ra_init(const struct device *dev)
{
	...
	ret = clock_control_on(config->clock_dev, (void *)&config->clock_id);
	if (ret < 0) {
		return ret;
	}

	ret = clock_control_get_rate(config->clock_dev, (void *)&config->clock_id, &data->clk_rate);
	if (ret < 0) {
		return ret;
	}
...
#define UART_RA_INIT_CFG(n)                                                                        \
	...
	static const struct uart_ra_cfg uart_ra_cfg_##n = {                                        \
		...
		.clock_id = {                                                                      \
			DT_CLOCKS_CELL_BY_IDX(DT_INST_PARENT(n), 0, mstp),                         \
			DT_CLOCKS_CELL_BY_IDX(DT_INST_PARENT(n), 0, stop_bit),                     \
		},                                                                                 \
