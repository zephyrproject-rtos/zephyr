...
#define INSTANCE_HAS_PROP(unused, prefix, i, prop) UARTE_PROP(prefix##i, prop)

#if UARTE_FOR_EACH_INSTANCE(INSTANCE_HAS_PROP, (+), (0), short_endtx_stoptx)
#define UARTE_HAS_ENDTX_STOPTX_SHORT 1
#endif

#if UARTE_FOR_EACH_INSTANCE(INSTANCE_HAS_PROP, (+), (0), frame_timeout)
...
#endif
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	/* Bigger than 1 in case of high speed instances. Baudrate is adjusted by that ratio. */
...
static int baudrate_set(const struct device *dev, uint32_t baudrate)
{
	...
	/* Baudrate needs to be adjusted for instances that are clocked by the higher clock. */
	nrf_baudrate = nrf_baudrate / config->baudrate_div;
...
static int uarte_enable(const struct device *dev, uint32_t act_mask, uint32_t sec_mask)
{
	struct uarte_nrfx_data *data = dev->data;
	int err;
	...
	err = pins_state_change(dev, true);
	if (err < 0) {
		return err;
	}
#ifdef UARTE_ANY_ASYNC
...
static void uarte_disable_locked(const struct device *dev, uint32_t dis_mask, uint32_t sec_mask)
{
	...
	(void)pins_state_change(dev, false);
				...
				int32_t timeout)
{
	...
	if (cfg->flags & UARTE_CFG_FLAG_LOW_POWER) {
		...
		ret = uarte_enable(dev, UARTE_FLAG_LOW_POWER_RX, UARTE_FLAG_LOW_POWER_TX);
		irq_unlock(key);
	}
...
#define UARTE_IS_CACHEABLE(idx) DMM_IS_REG_CACHEABLE(UARTE(idx))

#define UARTE_GET_BAUDRATE_DIV(idx)						\
	COND_CODE_0(UARTE_HAS_PROP(idx, base_frequency), (1),			\
		    ((UARTE_PROP(idx, base_frequency) / 16)))
...
#define UART_NRF_UARTE_DEVICE(idx)					       \
	...
	static const struct uarte_nrfx_config uarte_##idx##z_config = {	       \
		COND_CODE_1(CONFIG_UART_USE_RUNTIME_CONFIGURE,		       \
			(.baudrate_div = UARTE_GET_BAUDRATE_DIV(idx),),	       \
		    (IF_ENABLED(UARTE_HAS_FRAME_TIMEOUT,		       \
			(.baudrate = UARTE_PROP(idx, current_speed),))	       \
