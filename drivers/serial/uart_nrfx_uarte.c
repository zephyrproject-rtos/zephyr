...
#define UARTE_PINS_CROSS_DOMAIN(unused, prefix, idx, _)			\
	COND_CODE_1(DT_NODE_HAS_STATUS_OKAY(UARTE(prefix##idx)),	\
		   (UARTE_HAS_PROP(idx, cross_domain_default_port)),	\
		   (0))

#if (UARTE_FOR_EACH_INSTANCE(UARTE_PINS_CROSS_DOMAIN, (||), (0))) && defined(CONFIG_NRF_SYS_EVENT)
...
#define UART_NRF_UARTE_DEVICE(idx)					       \
	...
	static const struct uarte_nrfx_config uarte_##idx##z_config = {	       \
		...
		IF_ENABLED(UTIL_AND(					       \
			...
			(.cross_domain = true,				       \
			 .default_port =				       \
				UARTE_PROP(idx, cross_domain_default_port),))  \
	};								       \
