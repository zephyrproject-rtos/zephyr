...
static bool usart_gd32_tx_stop(const struct device *dev, bool stop_dma, uint8_t **buf,
			       size_t *sent);
static void usart_gd32_tx_complete(const struct device *dev, enum uart_event_type type,
...
static void usart_gd32_isr(const struct device *dev)
{
	...
	if (data->dma_tx.busy && data->dma_tx.dma_done &&
	    usart_interrupt_flag_get(cfg->reg, USART_INT_FLAG_TC) == SET) {
		uint8_t *buf;
		size_t sent;

		if (usart_gd32_tx_stop(dev, true, &buf, &sent)) {
			usart_gd32_tx_complete(dev, UART_TX_DONE, buf, sent);
		}
	}
#endif /* CONFIG_UART_ASYNC_API */
...
/*
 * Tear down an in-flight TX and take ownership of its completion. The transfer
 * can be ended by the USART TC interrupt, by a DMA error callback or by
 * uart_tx_abort() from a thread, so the teardown runs with interrupts locked
 * and only the first caller gets true. Any path that loses the race sees
 * busy == false and must not emit an event.
 */
static bool usart_gd32_tx_stop(const struct device *dev, bool stop_dma, uint8_t **buf, size_t *sent)
{
	const struct gd32_usart_config *const cfg = dev->config;
	struct gd32_usart_data *const data = dev->data;
	...
	if (data->dma_tx.dma_done) {
		*sent = data->dma_tx.len;
	} else if (dma_get_status(cfg->dma_tx.dev, cfg->dma_tx.channel, &status) == 0) {
				 ...
				 int status)
{
	...
	if (status != 0) {
		...
		if (usart_gd32_tx_stop(dev, true, &buf, &sent)) {
			usart_gd32_tx_complete(dev, UART_TX_ABORTED, buf, sent);
		}
	...
	/*
	 * The DMA has written the last byte to TDATA, but the shift register is
	 * still busy. Wait for the USART TC flag before reporting UART_TX_DONE.
	 */
	key = irq_lock();

	if (data->dma_tx.busy) {
		data->dma_tx.dma_done = true;
...
/* Called with interrupts locked; do not accept a new RX until all buffers are released. */
static void usart_gd32_rx_stop(const struct device *dev, size_t len, int reason)
{
	const struct gd32_usart_config *const cfg = dev->config;
	struct gd32_usart_data *const data = dev->data;
	uint8_t *buf = data->dma_rx.buf;
	uint8_t *next_buf = data->dma_rx.next_buf;
	struct uart_event rdy_evt = {
		.type = UART_RX_RDY,
		.data.rx.buf = buf,
		.data.rx.offset = data->dma_rx.offset,
		.data.rx.len = len - MIN(len, data->dma_rx.offset),
	};
	...
	data->rx_notifying = true;
	data->rx_disable_pending = false;
	...
	if (reason != 0) {
		struct uart_event stopped_evt = {
			...
			.data.rx_stop.data = rdy_evt.data.rx,
		};

		usart_gd32_async_callback(dev, &stopped_evt);
	}
	if (rdy_evt.data.rx.len != 0U) {
				 ...
				 int status)
{
	...
	if (status != 0) {
		usart_gd32_rx_stop(dev, usart_gd32_rx_received_len(dev), UART_ERROR_OVERRUN);
		goto unlock;
	}
	...
	buf = data->dma_rx.buf;
	rdy_evt = (struct uart_event){
		.type = UART_RX_RDY,
		.data.rx.buf = buf,
		.data.rx.len = data->dma_rx.len - data->dma_rx.offset,
		.data.rx.offset = data->dma_rx.offset,
	};
	...
	/* Commit the buffer handoff before callbacks can supply another buffer. */
	if (next_buf != NULL) {
	...
	/* A reentrant disable must not release the new buffer ahead of the old one. */
	data->rx_notifying = true;
	if (rdy_evt.data.rx.len != 0U) {
		usart_gd32_async_callback(dev, &rdy_evt);
	}
	...
	if (!data->dma_rx.busy) {
		usart_gd32_rx_disabled(dev);
	} else if (data->rx_disable_pending) {
		usart_gd32_rx_stop(dev, usart_gd32_rx_received_len(dev), 0);
	} else {
...
static int usart_gd32_rx_enable(const struct device *dev, uint8_t *buf, size_t len, int32_t timeout)
{
	...
	key = irq_lock();
	if (data->dma_rx.busy || data->rx_notifying) {
		ret = -EBUSY;
		goto unlock;
	}
...
static int usart_gd32_rx_disable(const struct device *dev)
{
	...
	key = irq_lock();
	if (!data->dma_rx.busy || data->rx_disable_pending) {
		irq_unlock(key);
		return -EFAULT;
	}
	...
	if (data->rx_notifying) {
		data->rx_disable_pending = true;
	} else {
		usart_gd32_rx_stop(dev, usart_gd32_rx_received_len(dev), 0);
	}
