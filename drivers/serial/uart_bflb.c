...
static uint32_t uart_bflb_get_clock(void)
{
	...
	if (tmpVal == 0) {
		...
		if (tmpVal == 0) {
			...
			/* Crystal clock */
			tmpVal = uart_bflb_get_crystal_frequency()
			       / (hclk_divider + 1);
			return (tmpVal / (uart_divider + 1));
		} else if (tmpVal > 1) {
			/* PLL Clock */
			tmpVal = uart_bflb_get_PLL_frequency()
			       / (hclk_divider + 1);
			return (tmpVal / (uart_divider + 1));
...
static int uart_bflb_configure(const struct device *dev)
{
	...
	tmpVal |= (cfg->tx_fifo_threshold << UART_TX_FIFO_TH_SHIFT)
	       &  UART_TX_FIFO_TH_MASK;
	tmpVal |= (cfg->rx_fifo_threshold << UART_RX_FIFO_TH_SHIFT)
