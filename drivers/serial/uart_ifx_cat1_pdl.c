...
static int ifx_cat1_uart_async_dma_config_buffer(const struct device *dev, bool tx)
{
	...
	ret = dma_config(dma_stream->dev_dma, dma_stream->dma_channel, &dma_stream->dma_cfg);

	if (!ret) {
		ret = dma_start(dma_stream->dev_dma, dma_stream->dma_channel);
	}
...
{
	struct ifx_cat1_uart_data *const data = dev->data;
	const struct device *dev_dma = data->async.dma_tx.dev_dma;
	int err;
...
static int ifx_cat1_uart_async_tx_abort(const struct device *dev)
{
	...
	err = dma_stop(data->async.dma_tx.dev_dma, data->async.dma_tx.dma_channel);
	if (err) {
		LOG_ERR("Error stopping Tx DMA (%d)", err);
		goto unlock;
	}

	err = dma_get_status(data->async.dma_tx.dev_dma, data->async.dma_tx.dma_channel, &stat);
	if (err) {
		LOG_ERR("Error stopping Tx DMA (%d)", err);
		goto unlock;
	}
				 ...
				 int status)
{
	...
	if (status == 0) {
		...
		k_work_cancel_delayable(&data->async.dma_tx.timeout_work);
		dma_stop(data->async.dma_tx.dev_dma, data->async.dma_tx.dma_channel);
					...
					enum uart_rx_stop_reason reason)
{
	...
	if (dma_get_status(data->async.dma_rx.dev_dma, data->async.dma_rx.dma_channel, &stat) ==
	    0) {
					 ...
					 size_t rx_data_size, int32_t timeout)
{
	...
	if (data->async.dma_rx.dev_dma == NULL) {
		return -ENODEV;
	}

	err = dma_get_status(data->async.dma_rx.dev_dma, data->async.dma_rx.dma_channel,
			     &dma_status);
	if (err) {
		return err;
	}
				...
				int status)
{
	...
	if (status == 0) {
		...
		if (!data->async.rx_next_buf) {
			dma_stop(data->async.dma_rx.dev_dma, data->async.dma_rx.dma_channel);
...
static void ifx_cat1_uart_async_rx_timeout(struct k_work *work)
{
	...
	if (dma_get_status(dma_rx->dev_dma, dma_rx->dma_channel, &stat) == 0) {
...
static int ifx_cat1_uart_init(const struct device *dev)
{
	...
	if (data->async.dma_rx.dev_dma != NULL) {
		if (!device_is_ready(data->async.dma_rx.dev_dma)) {
			return -ENODEV;
		}
	...
	if (data->async.dma_tx.dev_dma != NULL) {
		if (!device_is_ready(data->async.dma_tx.dev_dma)) {
			return -ENODEV;
		}
