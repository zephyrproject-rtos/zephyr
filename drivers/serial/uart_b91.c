...
/* Stop bits length enums */
typedef enum {
	UART_STOP_BIT_1         = 0,
	UART_STOP_BIT_1P5       = BIT(4),
	UART_STOP_BIT_2         = BIT(5),
} uart_stop_bit_e;

/* rxtimeout_h register enums */
enum {
	UART_ERR_IRQ_MASK = BIT(7),
};
...
static void uart_b91_cal_div_and_bwpc(uint32_t baudrate,        /* baut rate of UART. */
				      uint32_t pclk,            /* system clock */
				      uint16_t *div,            /* uart clock divider */
				      uint8_t *bwpc)            /* bitwidth, should be more than 2 */
{
	uint8_t i = 0, j = 0;
	uint32_t primeInt = 0;
	uint8_t primeDec = 0;
...
static void uart_b91_init(volatile struct uart_b91_t *uart,     /* UART instance */
			  uint16_t div,                         /* uart clock divider */
			  uint8_t bwpc,                         /* bitwidth, should be more than 2 */
			  uart_parity_e parity,                 /* parity type for UART interface */
			  uart_stop_bit_e stop_bit)             /* length of stop bit for UART interface. */
{
	/* config clock */
	div = div | FLD_UART_CLK_DIV_EN;
	uart->ctrl0 = bwpc;
	uart->clk_div_l = div & 0xFFu;
	uart->clk_div_h = (div >> 8u) & 0xFFu;
			      ...
			      const struct uart_config *cfg)
{
	...
	/* UART configure */
	uart_b91_cal_div_and_bwpc(cfg->baudrate, sys_clk.pclk * 1000 * 1000, &div, &bwpc);
	uart_b91_init(uart, div, bwpc, parity, stop_bits);
...
static int uart_b91_driver_init(const struct device *dev)
{
	...
	uart_b91_cal_div_and_bwpc(cfg->baud_rate, sys_clk.pclk * 1000 * 1000, &div, &bwpc);
	uart_b91_init(uart, div, bwpc, UART_PARITY_NONE, UART_STOP_BIT_1);
...
static void uart_b91_poll_out(const struct device *dev, uint8_t c)
{
	...
	if (data->tx_byte_index == 0) {
		uart->data_buf0 = c;
	} else if (data->tx_byte_index == 1) {
		uart->data_buf1 = c;
	} else if (data->tx_byte_index == 2) {
		uart->data_buf2 = c;
	} else if (data->tx_byte_index == 3) {
		uart->data_buf3 = c;
	}

	data->tx_byte_index++;
	data->tx_byte_index &= 0x03;
}
...
static int uart_b91_poll_in(const struct device *dev, unsigned char *c)
{
	...
	if (data->rx_byte_index == 0) {
		*c = uart->data_buf0;
	} else if (data->rx_byte_index == 1) {
		*c = uart->data_buf1;
	} else if (data->rx_byte_index == 2) {
		*c = uart->data_buf2;
	} else if (data->rx_byte_index == 3) {
		*c = uart->data_buf3;
	}

	data->rx_byte_index++;
	data->rx_byte_index &= 0x03;
			      ...
			      const int size)
{
	...
	for (i = 0; i < size; i++) {
		...
		uart_b91_poll_in(dev, &rx_data[i]);
	}
...
static void uart_b91_irq_tx_enable(const struct device *dev)
{
	...
	uart->ctrl3 = (uart->ctrl3 & (~FLD_UART_TX_IRQ_TRIQ_LEV)) |
		      (0x01 << FLD_UART_TX_IRQ_TRIQ_LEV_OFFSET);
...
static void uart_b91_irq_rx_enable(const struct device *dev)
{
	...
	uart->ctrl3 = (uart->ctrl3 & (~FLD_UART_RX_IRQ_TRIQ_LEV)) |
		      (0x01 << FLD_UART_RX_IRQ_TRIQ_LEV_OFFSET);
...
static void uart_b91_irq_err_enable(const struct device *dev)
{
	...
	uart->rxtimeout_h |= UART_ERR_IRQ_MASK;
}
