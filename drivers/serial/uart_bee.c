...
static int uart_bee_configure(const struct device *dev, const struct uart_config *cfg)
{
	...
	if (baudrate_idx < 0) {
		LOG_ERROR("Unsupported baudrate: %d", cfg->baudrate);
		return -ENOTSUP;
	}

	wordlen = uart_bee_cfg2mac_data_bits(cfg->data_bits);
	if (wordlen < 0) {
		LOG_ERROR("Unsupported data_bits: %d", cfg->data_bits);
		return -ENOTSUP;
	}

	stopbits = uart_bee_cfg2mac_stopbits(cfg->stop_bits);
	if (stopbits < 0) {
		LOG_ERROR("Unsupported stop_bits: %d", cfg->stop_bits);
		return -ENOTSUP;
	}

	parity = uart_bee_cfg2mac_parity(cfg->parity);
	if (parity < 0) {
		LOG_ERROR("Unsupported parity: %d", cfg->parity);
		return -ENOTSUP;
	}
...
{
	LOG_ERROR("Unsupported line_ctrl_set function");
	return -ENOTSUP;
}

int uart_bee_line_ctrl_get(const struct device *dev, uint32_t ctrl, uint32_t *val)
{
	LOG_ERROR("Unsupported line_ctrl_get function");
	return -ENOTSUP;
}
#endif
...
{
	LOG_ERROR("Unsupported drv_cmd function");
	return -ENOTSUP;
}
#endif
