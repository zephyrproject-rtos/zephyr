...
/* Driver dts compatibility: telink,tlx_uart */
...
/* tlx UART registers structure */
struct __packed uart_tlx_t {
...
/* tlx UART data structure */
struct uart_tlx_data {
...
/* tlx UART config structure */
struct uart_tlx_config {
...
/* Get tx fifo count */
static inline uint8_t uart_tlx_get_tx_bufcnt(volatile struct uart_tlx_t *uart)
{
	return (uart->bufcnt & FLD_UART_TX_BUF_CNT) >> FLD_UART_TX_BUF_CNT_OFFSET;
}

/* Get rx fifo count */
static inline uint8_t uart_tlx_get_rx_bufcnt(volatile struct uart_tlx_t *uart)
{
	return (uart->bufcnt & FLD_UART_RX_BUF_CNT) >> FLD_UART_RX_BUF_CNT_OFFSET;
}

/* Check for prime */
static uint8_t uart_tlx_is_prime(uint32_t n)
{
	uint32_t i = 5;
...
static void uart_tlx_irq_handler(const struct device *dev)
{
			      ...
			      const struct uart_config *cfg)
{
	...
	/* UART configure */
	uart_tlx_cal_div_and_bwpc(cfg->baudrate, sys_clk.pclk * 1000 * 1000, &divider, &bwpc);
	uart_tlx_init(uart, divider, bwpc, parity, stop_bits);
...
static void uart_tlx_poll_out(const struct device *dev, uint8_t c)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
	struct uart_tlx_data *data = dev->data;

	while (uart_tlx_get_tx_bufcnt(uart) >= UART_TX_BUF_CNT) {
	}
...
static int uart_tlx_poll_in(const struct device *dev, unsigned char *c)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
	struct uart_tlx_data *data = dev->data;

	if (uart_tlx_get_rx_bufcnt(uart) == 0) {
		return -1;
	}
...
static int uart_tlx_err_check(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static void uart_tlx_irq_tx_enable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static void uart_tlx_irq_tx_disable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static int uart_tlx_irq_tx_ready(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static int uart_tlx_irq_tx_complete(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);

	return (uart_tlx_get_tx_bufcnt(uart) == 0) ? 1 : 0;
}

/* API implementation: irq_rx_enable */
static void uart_tlx_irq_rx_enable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static void uart_tlx_irq_rx_disable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static int uart_tlx_irq_rx_ready(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);

	return (uart_tlx_get_rx_bufcnt(uart) > 0) ? 1 : 0;
}

/* API implementation: irq_err_enable */
static void uart_tlx_irq_err_enable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static void uart_tlx_irq_err_disable(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static int uart_tlx_irq_is_pending(const struct device *dev)
{
	volatile struct uart_tlx_t *uart = GET_UART(dev);
...
static int uart_tlx_irq_update(const struct device *dev)
{
	ARG_UNUSED(dev);
...
static const struct uart_driver_api uart_tlx_driver_api = {
	.poll_in = uart_tlx_poll_in,
	.poll_out = uart_tlx_poll_out,
	.err_check = uart_tlx_err_check,
	.configure = uart_tlx_configure,
	.config_get = uart_tlx_config_get,
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = uart_tlx_fifo_fill,
	.fifo_read = uart_tlx_fifo_read,
	.irq_tx_enable = uart_tlx_irq_tx_enable,
	.irq_tx_disable = uart_tlx_irq_tx_disable,
	.irq_tx_ready = uart_tlx_irq_tx_ready,
	.irq_tx_complete = uart_tlx_irq_tx_complete,
	.irq_rx_enable = uart_tlx_irq_rx_enable,
	.irq_rx_disable = uart_tlx_irq_rx_disable,
	.irq_rx_ready = uart_tlx_irq_rx_ready,
	.irq_err_enable = uart_tlx_irq_err_enable,
	.irq_err_disable = uart_tlx_irq_err_disable,
