...
static int usart_sam_configure(const struct device *dev, const struct uart_config *cfg)
{
	int retval;
	...
	if (cfg->data_bits == UART_CFG_DATA_BITS_9 || cfg->stop_bits == UART_CFG_STOP_BITS_0_5 ||
...
static int usart_sam_config_get(const struct device *dev, struct uart_config *cfg)
{
	struct usart_sam_dev_data *const dev_data = dev->data;
...
static int usart_sam_fifo_fill(const struct device *dev, const uint8_t *tx_data, int size)
{
	const struct usart_sam_dev_cfg *config = dev->config;
...
static int usart_sam_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	const struct usart_sam_dev_cfg *config = dev->config;
...
static int usart_sam_irq_tx_ready(const struct device *dev)
{
	 ...
	 */
	return (usart->US_CSR & US_CSR_TXRDY && usart->US_IMR & US_IMR_TXRDY);
}
...
static int usart_sam_irq_tx_complete(const struct device *dev)
{
	...
	return (usart->US_CSR & US_CSR_TXRDY && usart->US_CSR & US_CSR_TXEMPTY);
}
...
static void usart_sam_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				       void *cb_data)
{
	struct usart_sam_dev_data *const dev_data = dev->data;
...
#define USART_SAM_CONFIG_FUNC(n)                                                                   \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), usart_sam_isr,              \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		irq_enable(DT_INST_IRQN(n));                                                       \
	}
#define USART_SAM_IRQ_CFG_FUNC_INIT(n) .irq_config_func = usart##n##_sam_irq_config_func
#define USART_SAM_INIT_CFG(n)          USART_SAM_DECLARE_CFG(n, USART_SAM_IRQ_CFG_FUNC_INIT(n))
#else
#define USART_SAM_CONFIG_FUNC(n)
#define USART_SAM_IRQ_CFG_FUNC_INIT
#define USART_SAM_INIT_CFG(n) USART_SAM_DECLARE_CFG(n, USART_SAM_IRQ_CFG_FUNC_INIT)
#endif
...
#define USART_SAM_INIT(n)                                                                          \
	...
	static const struct usart_sam_dev_cfg usart##n##_sam_config;                               \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, usart_sam_init, NULL, &usart##n##_sam_data,                       \
			      &usart##n##_sam_config, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,   \
