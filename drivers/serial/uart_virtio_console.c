		...
		for (int i = 0; i < CONFIG_UART_VIRTIO_CONSOLE_RX_CONTROL_BUFSIZE; i++) {
			...
			case VIRTIO_CONSOLE_PORT_NAME:
				LOG_INF("port %u is named \"%.*s\"", data->rx_ctlbuf[i].port,
					ARRAY_SIZE(data->rx_ctlbuf[i].name),
...
static int virtconsole_init(const struct device *dev)
{
	...
	for (int i = 0; i < 3; i++) { /* There are three feature bits to check */
		data->features |= virtio_read_device_feature_bit(config->vdev, i) << i;
	}
	if (virtio_write_driver_feature_bit(config->vdev, VIRTIO_CONSOLE_F_EMERG_WRITE, 1)) {
		LOG_WRN("could not set feature bit");
	}
	size_t n_queues = 2;

	data->virtio_devcfg = virtio_get_device_specific_config(config->vdev);
	__maybe_unused bool multiport = IS_BIT_SET(data->features, VIRTIO_CONSOLE_F_MULTIPORT);
