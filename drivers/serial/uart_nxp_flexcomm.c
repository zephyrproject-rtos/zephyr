...
#define DT_DRV_COMPAT nxp_flexcomm_uart

/** @file
 * @brief UART driver for NXP Flexcomm USART.
 */
...
#define NXP_FLEXCOMM_UART_IS_WAKEUP \
	(IS_ENABLED(CONFIG_PM) && \
	 DT_ANY_INST_HAS_BOOL_STATUS_OKAY(wakeup_source))

#ifdef CONFIG_UART_ASYNC_API
struct nxp_flexcomm_uart_dma_config {
...
struct nxp_flexcomm_config {
	USART_Type *base;
	const struct device *clock_dev;
	clock_control_subsys_t clock_subsys;
	uint32_t baud_rate;
	uint8_t parity;
#ifdef CONFIG_UART_NXP_FLEXCOMM_ISR_SUPPORT
	void (*irq_config_func)(const struct device *dev);
#endif
	const struct pinctrl_dev_config *pincfg;
#ifdef CONFIG_UART_ASYNC_API
	struct nxp_flexcomm_uart_dma_config tx_dma;
	struct nxp_flexcomm_uart_dma_config rx_dma;
...
#endif
#if NXP_FLEXCOMM_UART_IS_WAKEUP
...
struct nxp_flexcomm_uart_tx_data {
...
struct nxp_flexcomm_uart_rx_data {
	uint8_t *xfer_buf;
	size_t xfer_len;
	struct dma_block_config active_block;
...
struct nxp_flexcomm_data {
	...
	void *async_cb_data;
	struct nxp_flexcomm_uart_tx_data tx_data;
	struct nxp_flexcomm_uart_rx_data rx_data;
#endif
...
#ifdef CONFIG_PM_POLICY_DEVICE_CONSTRAINTS
static void nxp_flexcomm_pm_policy_state_lock_get(const struct device *dev)
{
	struct nxp_flexcomm_data *data = dev->data;
...
static void nxp_flexcomm_pm_unlock_if_idle(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
...
static void nxp_flexcomm_pm_policy_state_lock_put(const struct device *dev)
{
	struct nxp_flexcomm_data *data = dev->data;
...
static int nxp_flexcomm_poll_in(const struct device *dev, unsigned char *c)
{
	const struct nxp_flexcomm_config *config = dev->config;
...
static void nxp_flexcomm_poll_out(const struct device *dev,
					     unsigned char c)
{
	const struct nxp_flexcomm_config *config = dev->config;
...
static int nxp_flexcomm_err_check(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t flags = USART_GetStatusFlags(config->base);
	int err = 0;
...
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
static int nxp_flexcomm_fifo_fill(const struct device *dev,
...
static int nxp_flexcomm_fifo_read(const struct device *dev, uint8_t *rx_data,
				   const int len)
{
	const struct nxp_flexcomm_config *config = dev->config;
	int num_rx = 0U;
...
static void nxp_flexcomm_irq_tx_enable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	 ...
	 */
#ifdef CONFIG_PM_POLICY_DEVICE_CONSTRAINTS
	nxp_flexcomm_pm_policy_state_lock_get(dev);
#endif

	USART_EnableInterrupts(config->base, mask);
}

static void nxp_flexcomm_irq_tx_disable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t mask = kUSART_TxLevelInterruptEnable;

#ifdef CONFIG_PM_POLICY_DEVICE_CONSTRAINTS
	nxp_flexcomm_pm_policy_state_lock_put(dev);
#endif

	USART_DisableInterrupts(config->base, mask);
}

static int nxp_flexcomm_irq_tx_complete(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
...
static int nxp_flexcomm_irq_tx_ready(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t mask = kUSART_TxLevelInterruptEnable;
	uint32_t flags = USART_GetStatusFlags(config->base);
...
static void nxp_flexcomm_irq_rx_enable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
...
static void nxp_flexcomm_irq_rx_disable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t mask = kUSART_RxLevelInterruptEnable;
...
static int nxp_flexcomm_irq_rx_full(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t flags = USART_GetStatusFlags(config->base);
...
static int nxp_flexcomm_irq_rx_pending(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t mask = kUSART_RxLevelInterruptEnable;

	return (USART_GetEnabledInterrupts(config->base) & mask)
		&& nxp_flexcomm_irq_rx_full(dev);
}

static void nxp_flexcomm_irq_err_enable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
...
static void nxp_flexcomm_irq_err_disable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	uint32_t mask = kUSART_NoiseErrorInterruptEnable |
			kUSART_FramingErrorInterruptEnable |
...
static int nxp_flexcomm_irq_is_pending(const struct device *dev)
{
	return (nxp_flexcomm_irq_tx_ready(dev)
		|| nxp_flexcomm_irq_rx_pending(dev));
}

static int nxp_flexcomm_irq_update(const struct device *dev)
{
	return 1;
}

static void nxp_flexcomm_irq_callback_set(const struct device *dev,
...
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
static int nxp_flexcomm_uart_configure(const struct device *dev, const struct uart_config *cfg)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
...
static int nxp_flexcomm_uart_config_get(const struct device *dev,
...
static void nxp_flexcomm_async_user_callback(const struct device *dev,
					      struct uart_event *evt)
{
	const struct nxp_flexcomm_data *data = dev->data;
...
static int nxp_flexcomm_uart_callback_set(const struct device *dev,
...
static int nxp_flexcomm_uart_tx(const struct device *dev, const uint8_t *buf,
...
static int nxp_flexcomm_uart_tx_abort(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
	int ret = 0;
	...
	nxp_flexcomm_async_user_callback(dev, &tx_abort_event);

	return ret;
}

static int nxp_flexcomm_uart_rx_enable(const struct device *dev, uint8_t *buf,
					const size_t len, const int32_t timeout)
{
	...
	nxp_flexcomm_async_user_callback(dev, &rx_buf_request);

	return ret;
}

static void nxp_flexcomm_uart_rx_update(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
	...
	if (total_rx_receive_len > data->rx_data.offset) {
		...
		nxp_flexcomm_async_user_callback(dev, &rx_rdy_event);
	}
...
static int nxp_flexcomm_uart_rx_disable(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
	int ret = 0;
	...
	nxp_flexcomm_uart_rx_update(dev);
	...
	nxp_flexcomm_async_user_callback(dev, &current_buffer_release_event);
	...
	if (data->rx_data.next_xfer_buf) {
		...
		nxp_flexcomm_async_user_callback(dev, &next_buffer_release_event);
	}
	...
	nxp_flexcomm_async_user_callback(dev, &rx_disabled_event);

	return ret;
}

static int nxp_flexcomm_uart_rx_buf_rsp(const struct device *dev, uint8_t *buf, size_t len)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
...
static void nxp_flexcomm_uart_dma_tx_callback(const struct device *dma_device, void *cb_data,
...
static void nxp_flexcomm_uart_dma_rx_callback(const struct device *dma_device, void *cb_data,
				uint32_t channel, int status)
{
	/* DMA callback data was configured during driver init as UART device ptr */
	struct device *dev = (struct device *)cb_data;
	...
	if (data->rx_data.next_xfer_buf) {
		...
		nxp_flexcomm_uart_rx_disable(dev);
	}
 ...
 */
static uint32_t nxp_flexcomm_uart_calc_inmux_connection(uint8_t channel, DMA_Type *base)
{
...
static int nxp_flexcomm_uart_async_init(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
	...
	INPUTMUX_EnableSignal(INPUTMUX,
		nxp_flexcomm_uart_calc_inmux_connection(config->rx_dma.channel,
					config->rx_dma.base), true);
	INPUTMUX_EnableSignal(INPUTMUX,
		nxp_flexcomm_uart_calc_inmux_connection(config->tx_dma.channel,
...
#ifdef CONFIG_UART_NXP_FLEXCOMM_ISR_SUPPORT
static void nxp_flexcomm_isr(const struct device *dev)
{
	struct nxp_flexcomm_data *data = dev->data;
	...
	if (data->async_callback) {
		...
		if (config->base->FIFOINTSTAT & USART_FIFOINTSTAT_TXLVL_MASK) {
			...
			nxp_flexcomm_async_user_callback(dev, &tx_done_event);
...
#endif /* CONFIG_UART_NXP_FLEXCOMM_ISR_SUPPORT */

static int nxp_flexcomm_init_common(const struct device *dev)
{
	const struct nxp_flexcomm_config *config = dev->config;
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	struct nxp_flexcomm_data *data = dev->data;
...
#ifdef CONFIG_UART_ASYNC_API
	err = nxp_flexcomm_uart_async_init(dev);
	if (err) {
		return err;
	}
#endif
...
#if NXP_FLEXCOMM_UART_IS_WAKEUP
static void nxp_flexcomm_pm_prepare_wake(const struct device *dev,
...
static void nxp_flexcomm_pm_restore_wake(const struct device *dev,
					  enum pm_state state, uint8_t substate)
{
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
...
#endif /* NXP_FLEXCOMM_UART_IS_WAKEUP */

static uint32_t usart_intenset;
static int nxp_flexcomm_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct nxp_flexcomm_config *config = dev->config;
	int ret;
	...
	case PM_DEVICE_ACTION_TURN_ON:
		ret = nxp_flexcomm_init_common(dev);
		if (ret) {
			return ret;
		}
...
static int nxp_flexcomm_init(const struct device *dev)
{
#if NXP_FLEXCOMM_UART_IS_WAKEUP || defined(CONFIG_PM_POLICY_DEVICE_CONSTRAINTS)
	const struct nxp_flexcomm_config *config = dev->config;
	struct nxp_flexcomm_data *data = dev->data;
#endif
	 ...
	 */
	return pm_device_driver_init(dev, nxp_flexcomm_pm_action);
}

static DEVICE_API(uart, nxp_flexcomm_driver_api) = {
	.poll_in = nxp_flexcomm_poll_in,
	.poll_out = nxp_flexcomm_poll_out,
	.err_check = nxp_flexcomm_err_check,
#ifdef CONFIG_UART_USE_RUNTIME_CONFIGURE
	.configure = nxp_flexcomm_uart_configure,
	.config_get = nxp_flexcomm_uart_config_get,
#endif
#ifdef CONFIG_UART_INTERRUPT_DRIVEN
	.fifo_fill = nxp_flexcomm_fifo_fill,
	.fifo_read = nxp_flexcomm_fifo_read,
	.irq_tx_enable = nxp_flexcomm_irq_tx_enable,
	.irq_tx_disable = nxp_flexcomm_irq_tx_disable,
	.irq_tx_complete = nxp_flexcomm_irq_tx_complete,
	.irq_tx_ready = nxp_flexcomm_irq_tx_ready,
	.irq_rx_enable = nxp_flexcomm_irq_rx_enable,
	.irq_rx_disable = nxp_flexcomm_irq_rx_disable,
	.irq_rx_ready = nxp_flexcomm_irq_rx_full,
	.irq_err_enable = nxp_flexcomm_irq_err_enable,
	.irq_err_disable = nxp_flexcomm_irq_err_disable,
	.irq_is_pending = nxp_flexcomm_irq_is_pending,
	.irq_update = nxp_flexcomm_irq_update,
	.irq_callback_set = nxp_flexcomm_irq_callback_set,
#endif
#ifdef CONFIG_UART_ASYNC_API
	.callback_set = nxp_flexcomm_uart_callback_set,
	.tx = nxp_flexcomm_uart_tx,
	.tx_abort = nxp_flexcomm_uart_tx_abort,
	.rx_enable = nxp_flexcomm_uart_rx_enable,
	.rx_disable = nxp_flexcomm_uart_rx_disable,
	.rx_buf_rsp = nxp_flexcomm_uart_rx_buf_rsp,
#endif
};


#ifdef CONFIG_UART_NXP_FLEXCOMM_ISR_SUPPORT
#define UART_NXP_FLEXCOMM_IRQ_CFG_FUNC(n)					\
	static void nxp_flexcomm_irq_config_func_##n(const struct device *dev)	\
	{									\
		IRQ_CONNECT(DT_INST_IRQN(n),					\
			    DT_INST_IRQ(n, priority),				\
			    nxp_flexcomm_isr, DEVICE_DT_INST_GET(n), 0);	\
										\
		irq_enable(DT_INST_IRQN(n));					\
	}
#define UART_NXP_FLEXCOMM_IRQ_CFG_FUNC_INIT(n)					\
	.irq_config_func = nxp_flexcomm_irq_config_func_##n,
#else
#define UART_NXP_FLEXCOMM_IRQ_CFG_FUNC(n)
#define UART_NXP_FLEXCOMM_IRQ_CFG_FUNC_INIT(n)
#endif /* CONFIG_UART_NXP_FLEXCOMM_ISR_SUPPORT */
...
#ifdef CONFIG_PM_POLICY_DEVICE_CONSTRAINTS
#define UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_DEFINE(n)				\
static void nxp_flexcomm_##n##_pm_unlock(struct k_work *work)			\
{										\
	const struct device *dev = DEVICE_DT_INST_GET(n);			\
										\
	nxp_flexcomm_pm_unlock_if_idle(dev);					\
}
#define UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_BIND(n)				\
	.pm_unlock_work_fn = nxp_flexcomm_##n##_pm_unlock,
#else
#define UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_DEFINE(n)
#define UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_BIND(n)
#endif /* CONFIG_PM_POLICY_DEVICE_CONSTRAINTS */
...
#ifdef CONFIG_UART_ASYNC_API
#define UART_NXP_FLEXCOMM_TX_TIMEOUT_FUNC(n)					\
	static void nxp_flexcomm_uart_##n##_tx_timeout(struct k_work *work)	\
	{									\
		nxp_flexcomm_uart_tx_abort(DEVICE_DT_INST_GET(n));		\
	}
#define UART_NXP_FLEXCOMM_RX_TIMEOUT_FUNC(n)					\
	static void nxp_flexcomm_uart_##n##_rx_timeout(struct k_work *work)	\
	{									\
		nxp_flexcomm_uart_rx_update(DEVICE_DT_INST_GET(n));			\
	}

DT_INST_FOREACH_STATUS_OKAY(UART_NXP_FLEXCOMM_TX_TIMEOUT_FUNC);
DT_INST_FOREACH_STATUS_OKAY(UART_NXP_FLEXCOMM_RX_TIMEOUT_FUNC);

#define UART_NXP_FLEXCOMM_ASYNC_CFG(n)						\
	.tx_dma = {								\
		...
		.cfg = {							\
			...
			.block_count = 1,					\
			.head_block =						\
				&nxp_flexcomm_##n##_data.tx_data.active_block,	\
			.channel_direction = MEMORY_TO_PERIPHERAL,		\
			.dma_callback = nxp_flexcomm_uart_dma_tx_callback,	\
	...
	.rx_dma = {								\
		...
		.cfg = {							\
			...
			.block_count = 1,					\
			.head_block =						\
				&nxp_flexcomm_##n##_data.rx_data.active_block,	\
			.channel_direction = PERIPHERAL_TO_MEMORY,		\
			.dma_callback = nxp_flexcomm_uart_dma_rx_callback,	\
			.user_data = (void *)DEVICE_DT_INST_GET(n)		\
		},								\
		.base = (DMA_Type *)						\
				DT_REG_ADDR(DT_INST_DMAS_CTLR_BY_NAME(n, rx)),	\
	},									\
	.rx_timeout_func = nxp_flexcomm_uart_##n##_rx_timeout,			\
	.tx_timeout_func = nxp_flexcomm_uart_##n##_tx_timeout,
#else
#define UART_NXP_FLEXCOMM_ASYNC_CFG(n)
#endif /* CONFIG_UART_ASYNC_API */
...
#define UART_NXP_FLEXCOMM_WAKEUP_CFG_DEFINE(n)					\
static void serial_nxp_flexcomm_##n##_wakeup_cfg(void)				\
{										\
	IF_ENABLED(DT_INST_PROP(n, wakeup_source), (				\
		NXP_ENABLE_WAKEUP_SIGNAL(DT_INST_IRQN(n));			\
	))									\
}
#define UART_NXP_FLEXCOMM_WAKEUP_CFG_BIND(n)					\
		.wakeup_cfg = serial_nxp_flexcomm_##n##_wakeup_cfg,

#define UART_NXP_FLEXCOMM_LP_STATES_DEFINE(n)	\
	PM_STATE_CONSTRAINTS_LIST_DEFINE(DT_DRV_INST(n), low_power_states);	\

#define UART_NXP_FLEXCOMM_LP_STATES_BIND(n)	\
	.lp_states = PM_STATE_CONSTRAINTS_GET(DT_DRV_INST(n), low_power_states),\

#define UART_NXP_FLEXCOMM_PM_HANDLES_DEFINE(n)					\
static void serial_nxp_flexcomm_##n##_pm_entry(enum pm_state state, uint8_t substate) \
{										\
	IF_ENABLED(DT_INST_PROP(n, wakeup_source), (				\
		nxp_flexcomm_pm_prepare_wake(DEVICE_DT_INST_GET(n), state, substate); \
	))									\
}										\
										\
static void serial_nxp_flexcomm_##n##_pm_exit(enum pm_state state, uint8_t substate) \
{										\
	IF_ENABLED(DT_INST_PROP(n, wakeup_source), (				\
		nxp_flexcomm_pm_restore_wake(DEVICE_DT_INST_GET(n), state, substate); \
	))									\
}
#define UART_NXP_FLEXCOMM_PM_HANDLES_BIND(n)					\
	.pm_handles = {								\
			.substate_entry = serial_nxp_flexcomm_##n##_pm_entry,	\
			.substate_exit = serial_nxp_flexcomm_##n##_pm_exit,	\
			.report_substate = true,				\
	},
#define UART_NXP_FLEXCOMM_LP_CLK_SUBSYS(n)					\
...
#define UART_NXP_FLEXCOMM_INIT_CFG(n)						\
static const struct nxp_flexcomm_config nxp_flexcomm_##n##_config = {		\
	...
	UART_NXP_FLEXCOMM_LP_STATES_BIND(n)					\
	UART_NXP_FLEXCOMM_IRQ_CFG_FUNC_INIT(n)					\
	UART_NXP_FLEXCOMM_ASYNC_CFG(n)						\
	UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_BIND(n)				\
	UART_NXP_FLEXCOMM_WAKEUP_CFG_BIND(n)					\
	UART_NXP_FLEXCOMM_LP_CLK_SUBSYS(n)					\
};

#define UART_NXP_FLEXCOMM_INIT_DATA(n)						\
static struct nxp_flexcomm_data nxp_flexcomm_##n##_data = {			\
	UART_NXP_FLEXCOMM_PM_HANDLES_BIND(n)					\
};

#define UART_NXP_FLEXCOMM_INIT(n)						\
										\
	PINCTRL_DT_INST_DEFINE(n);						\
										\
	UART_NXP_FLEXCOMM_PM_UNLOCK_FUNC_DEFINE(n)				\
	UART_NXP_FLEXCOMM_WAKEUP_CFG_DEFINE(n)					\
	UART_NXP_FLEXCOMM_PM_HANDLES_DEFINE(n)					\
	PM_DEVICE_DT_INST_DEFINE(n, nxp_flexcomm_pm_action);			\
										\
	UART_NXP_FLEXCOMM_INIT_DATA(n)						\
										\
	UART_NXP_FLEXCOMM_IRQ_CFG_FUNC(n)					\
										\
	UART_NXP_FLEXCOMM_LP_STATES_DEFINE(n)					\
										\
	UART_NXP_FLEXCOMM_INIT_CFG(n)						\
										\
										\
	DEVICE_DT_INST_DEFINE(n,						\
			    &nxp_flexcomm_init,				\
			    PM_DEVICE_DT_INST_GET(n),				\
			    &nxp_flexcomm_##n##_data,				\
			    &nxp_flexcomm_##n##_config,			\
			    PRE_KERNEL_1,					\
			    CONFIG_SERIAL_INIT_PRIORITY,			\
			    &nxp_flexcomm_driver_api);

DT_INST_FOREACH_STATUS_OKAY(UART_NXP_FLEXCOMM_INIT)
