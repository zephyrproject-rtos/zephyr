...
static void uart_cdns_irq_update(const struct device *dev)
{
	uintptr_t reg_base = DEVICE_MMIO_GET(dev);

	/* Clear RX/TX interrupts */
	sys_write32(CDNS_IXR_TTRIG | CDNS_IXR_TXEMPTY | CDNS_IXR_RTRIG,
		    reg_base + CDNS_ISR_OFFSET);
}
...
static DEVICE_API(uart, uart_cdns_driver_api) = {
	...
	.irq_update = uart_cdns_irq_update,
