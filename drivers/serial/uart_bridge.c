				  ...
				  const struct device *bridge_dev)
{
	...
	rb_len = ring_buf_put_ptr(&own_data->rb, &recv_buf);
	if (rb_len == 0) {
	...
	ring_buf_commit(&own_data->rb, recv_len);
				  ...
				  const struct device *bridge_dev)
{
	...
	rb_len = ring_buf_get_ptr(&peer_data->rb, &send_buf);
	if (rb_len == 0) {
	...
	ring_buf_consume(&peer_data->rb, sent_len);
