...
#define DT_DRV_COMPAT nuvoton_npcm_usbd
...
LOG_MODULE_REGISTER(usb_dc_npcm);
...
#define NPCM_USB_PHY_TIMEOUT            (50)
/* Timeout for USB Write Data to Host. (Unit:ms) */
#define NPCM_USB_WRITE_TIMEOUT          (200)
...
static inline const struct device *npcm_usbd_device_get(void)
{
...
static inline void usbd_config_ep(uint32_t ep, uint32_t ep_type, uint32_t ep_dir)
{
	if (ep_type == USBD_EP_CFG_TYPE_BULK) {
		/* manual mode */
		USBD->EP[ep - 1].USBD_EPRSPCTL = (BIT(NPCM_USBD_EPRSPCTL_FLUSH) | (0x01 << 1));
	} else if (ep_type == USBD_EP_CFG_TYPE_INT) {
		/* manual mode */
		USBD->EP[ep - 1].USBD_EPRSPCTL = (BIT(NPCM_USBD_EPRSPCTL_FLUSH) | (0x01 << 1));
	} else if (ep_type == USBD_EP_CFG_TYPE_ISO) {
		/* fly mode */
		USBD->EP[ep - 1].USBD_EPRSPCTL = (BIT(NPCM_USBD_EPRSPCTL_FLUSH) | (0x10 << 1));
	}
...
static inline bool usbd_ep_is_enabled(uint8_t ep_idx)
{
	if (ep_idx == 0) {
		...
		return IS_BIT_SET(USBD->EP[ep_idx - 1].USBD_EPCFG, NPCM_USBD_EPCFG_EPEN);
	}
...
static inline void usbd_flush_all_ep(void)
{
	...
	for (i = EPA; i <= EPL; i++) {
		USBD->EP[i].USBD_EPRSPCTL = BIT(NPCM_USBD_EPRSPCTL_FLUSH) | BIT(1);
	}
}
...
static int usbd_enable_phy(void)
{
	...
	/* Enable USB PHY */
	USBD->USBD_PHYCTL |= BIT(NPCM_USBD_PHYCTL_PHYEN);

	/* Wait PHY clock ready */
	st = k_uptime_get();
	while (1) {
		if (k_uptime_get() - st > NPCM_USB_PHY_TIMEOUT) {
...
static void usb_dc_cep_isr(void)
{
	...
	/* Setup Token */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_SETUPTKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_SETUPTKIF);
		return;
	}

	/* Setup Packet */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_SETUPPKIF)) {
		LOG_DBG("\r\n\r\nSETUP pkt");
	...
	/* Control OUT Token */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_OUTTKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_OUTTKIF);
	...
	/* Control IN Token */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_INTKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_INTKIF);
	...
	/* Ping Token */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_PINGIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_PINGIF);
		return;
	}

	/* Data Packet Transmitted and Acked */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_TXPKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_TXPKIF);
	...
	/* Host received */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_RXPKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_RXPKIF);
	...
	/* NAK sent */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_NAKIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_NAKIF);
		return;
	}

	/* STALL Sent */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_STALLIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_STALLIF);
		return;
	}

	/* USB Error */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_ERRIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_ERRIF);
		return;
	}

	/* Status Completion */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_STSDONEIF)) {
		/* Enable CEP Interrupt */
		USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_SETUPPKIEN);
	...
	/* Control Buffer Full */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_BUFFULLIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_BUFFULLIF);
		return;
	}

	/* Control Buffer Empty */
	if (IS_BIT_SET(IrqSt, NPCM_USBD_CEPINTSTS_BUFEMPTYIF)) {
		/* Clear CEP interrupt flag */
		USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_BUFEMPTYIF);
		return;
	}
}
...
static void usb_dc_isr(void)
{
	...
	/* USB interrupt */
	if (IS_BIT_SET(IrqStL, NPCM_USBD_GINTSTS_USBIF)) {
		/* Bus Status */
		IrqSt = USBD->USBD_BUSINTSTS & USBD->USBD_BUSINTEN;

		/* SOF */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_SOFIF)) {
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_SOFIF);
		}

		/* Reset */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_RSTIF)) {
			LOG_DBG("reset_isr");
			...
			if (IS_BIT_SET(USBD->USBD_OPER, NPCM_USBD_OPER_CURSPD)) {
				/* high speed */
				LOG_DBG("hs");
			} else {
				/* full speed */
			...
			USBD->USBD_BUSINTEN = BIT(NPCM_USBD_BUSINTEN_RESUMEIEN) |
					      BIT(NPCM_USBD_BUSINTEN_SUSPENDIEN) |
					      BIT(NPCM_USBD_BUSINTEN_RSTIEN);

			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_RSTIF);
			/* Clear all CEP interrupt flag */
			USBD->USBD_CEPINTSTS = 0x1FFC;
		}

		/* Resume */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_RESUMEIF)) {
			/* Callback function */
			dev_data.status_cb(USB_DC_RESUME, NULL);

			USBD->USBD_BUSINTEN = BIT(NPCM_USBD_BUSINTEN_RSTIEN) |
					      BIT(NPCM_USBD_BUSINTEN_SUSPENDIEN);
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_RESUMEIF);
			LOG_DBG("RS");
		}

		/* Suspend Request */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_SUSPENDIF)) {
			/* Callback function */
			dev_data.status_cb(USB_DC_SUSPEND, NULL);

			USBD->USBD_BUSINTEN = BIT(NPCM_USBD_BUSINTEN_RSTIEN) |
					      BIT(NPCM_USBD_BUSINTEN_RESUMEIEN);
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_SUSPENDIF);
			LOG_DBG("SP");
		}

		/* High speed */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_HISPDIF)) {
			/* Enable CEP Interrupt */
			USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_SETUPPKIEN);
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_HISPDIF);
		}

		/* DMA Completion */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_DMADONEIF)) {
			if (IS_BIT_SET(USBD->USBD_DMACTL, NPCM_USBD_DMACTL_DMARD)) {
				/* DMA read */
			} else {
				/* DMA write */
			}
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_DMADONEIF);
		}

		/* PHY clock is usable */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_PHYCLKVLDIF)) {
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_PHYCLKVLDIF);
		}

		/* Hot-Plug */
		if (IS_BIT_SET(IrqSt, NPCM_USBD_BUSINTSTS_VBUSDETIF)) {
			if (IS_BIT_SET(USBD->USBD_PHYCTL, NPCM_USBD_PHYCTL_VBUSDET)) {
				/* USB Plug In */
				/* Enable USB */
				USBD->USBD_PHYCTL |= BIT(NPCM_USBD_PHYCTL_PHYEN) |
						     BIT(NPCM_USBD_PHYCTL_DPPUEN);
			} else {
				/* USB Un-plug */
				/* Disable USB */
				USBD->USBD_PHYCTL &= ~BIT(NPCM_USBD_PHYCTL_DPPUEN);
			}
			/* Clear Bus interrupt flag */
			USBD->USBD_BUSINTSTS = BIT(NPCM_USBD_BUSINTSTS_VBUSDETIF);
		}
	}

	/* Endpoint interrupt */
	if (IS_BIT_SET(IrqStL, NPCM_USBD_GINTSTS_CEPIF)) {
		/* CEP interrupt */
		usb_dc_cep_isr();
	} else {
		/* Other endpoints interrupt */
		for (ep_idx = EPA; ep_idx <= EPL; ep_idx++) {
			if (IS_BIT_SET(IrqStL, (NPCM_USBD_GINTSTS_EPAIF + ep_idx))) {
...
{
	const struct device *dev = npcm_usbd_device_get();
	const struct npcm_usbd_config *config = dev->config;
	int32_t ret;
	...
	USBD->USBD_GINTEN = BIT(NPCM_USBD_GINTEN_USBIEN);
	/* Enable BUS interrupt */
	USBD->USBD_BUSINTEN = BIT(NPCM_USBD_BUSINTEN_RESUMEIEN) |
			      BIT(NPCM_USBD_BUSINTEN_SUSPENDIEN) |
			      BIT(NPCM_USBD_BUSINTEN_RSTIEN);
	...
	/* HSUSBD_Start */
	USBD->USBD_OPER = BIT(NPCM_USBD_OPER_HISPDEN);       /* high-speed */
...
int usb_dc_ep_clear_stall(const uint8_t ep)
{
	...
	if (type == USB_DC_EP_CONTROL) {
		...
		USBD->EP[ep_idx - 1].USBD_EPRSPCTL = BIT(NPCM_USBD_EPRSPCTL_TOGGLE) | BIT(1);
	}
...
int usb_dc_ep_is_stalled(const uint8_t ep, uint8_t *const stalled)
{
	...
	if (type == USB_DC_EP_CONTROL) {
		*stalled = USBD->USBD_CEPCTL & BIT(NPCM_USBD_CEPCTL_STALLEN);
	} else {
		*stalled = USBD->EP[ep_idx - 1].USBD_EPRSPCTL & BIT(NPCM_USBD_EPRSPCTL_HALT);
	}
	LOG_DBG("ep 0x%x", ep);
	return 0;
}
...
int usb_dc_ep_enable(const uint8_t ep)
{
	...
	if (type == USB_DC_EP_CONTROL) {
		...
		USBD->USBD_GINTEN |= BIT(NPCM_USBD_GINTEN_CEPIEN);
		/* Enable CEP Interrupt */
		USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_SETUPPKIEN);
	} else {
		/* Enable USB EP global interrupt */
		USBD->USBD_GINTEN |= (BIT(ep_idx) << 1);
		USBD->EP[ep_idx - 1].USBD_EPCFG |= BIT(NPCM_USBD_EPCFG_EPEN);

		if (USB_EP_GET_DIR(ep) == USB_EP_DIR_OUT) {
			USBD->EP[ep_idx - 1].USBD_EPINTEN = BIT(NPCM_USBD_EPINTEN_RXPKIEN) |
							    BIT(NPCM_USBD_EPINTEN_SHORTRXIEN);
		}
	}
...
int usb_dc_ep_flush(const uint8_t ep)
{
	...
	if (type == USB_DC_EP_CONTROL) {
		USBD->USBD_CEPCTL |= BIT(NPCM_USBD_CEPCTL_FLUSH);
	} else {
		USBD->EP[ep_idx - 1].USBD_EPRSPCTL |= BIT(NPCM_USBD_EPRSPCTL_FLUSH) | BIT(1);
	}
		    ...
		    const uint32_t data_len, uint32_t *const ret_bytes)
{
	...
	if (type == USB_DC_EP_CONTROL) {
		if (packet_len > 0) {
			...
			st = k_uptime_get();
			while (1) {
				if ((k_uptime_get() - st) > NPCM_USB_WRITE_TIMEOUT) {
					LOG_ERR("Timeout: USB Write Data!");
					return -ETIMEDOUT;
				}

				if ((USBD->USBD_CEPINTSTS & BIT(NPCM_USBD_CEPINTSTS_TXPKIF))
				    != 0) {
					/* Clear CEP data transmitted interrupt flag */
					USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_TXPKIF);
					break;
			...
			if (data_len > packet_len) {
				...
				USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_INTKIEN);
			} else {
				/* Status Stage */
				USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_SETUPPKIF);
				USBD->USBD_CEPINTSTS = BIT(NPCM_USBD_CEPINTSTS_STSDONEIF);
				USBD_SET_CEP_STATE(USBD_CEPCTL_NAKCLR);
				USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_STSDONEIEN);
			}
		} else {
			/* data_len=0 */
		...
		/* packet end */
		USBD->EP[ep_idx - 1].USBD_EPRSPCTL = BIT(NPCM_USBD_EPRSPCTL_SHORTTXEN) | BIT(1);
		...
		USBD->EP[ep_idx - 1].USBD_EPINTEN = BIT(NPCM_USBD_EPINTEN_INTKIEN);
	}
		...
		if (ep_status == USB_DC_EP_SETUP) {
			if (REQTYPE_GET_DIR(dev_data.req_type) == REQTYPE_DIR_TO_HOST) {
				...
				if (dev_data.req_len == 0) {
					...
					USBD->USBD_CEPINTEN = BIT(NPCM_USBD_CEPINTEN_RXPKIEN);
				}
			}
		} else {
			if (dev_data.req_len == 0) {
...
#define USB_DC_NPCM_INIT(inst)									   \
	PINCTRL_DT_INST_DEFINE(inst);                                                              \
                                                                                                   \
	static const struct npcm_usbd_config npcm_usbd_config_##inst = {			   \
	...
	DEVICE_DT_INST_DEFINE(inst, NULL, NULL, NULL,						   \
			      &npcm_usbd_config_##inst, POST_KERNEL,				   \
