...
static void usb_dw_reg_dump(void)
{
	...
	LOG_DBG("USB registers:  GOTGCTL : 0x%x  GOTGINT : 0x%x  GAHBCFG : "
		"0x%x",
		base->gotgctl, base->gotgint, base->gahbcfg);
	LOG_DBG("  GUSBCFG : 0x%x  GINTSTS : 0x%x  GINTMSK : 0x%x", base->gusbcfg, base->gintsts,
		base->gintmsk);
	LOG_DBG("  DCFG    : 0x%x  DCTL    : 0x%x  DSTS    : 0x%x", base->dcfg, base->dctl,
		base->dsts);
	LOG_DBG("  DIEPMSK : 0x%x  DOEPMSK : 0x%x  DAINT   : 0x%x", base->diepmsk, base->doepmsk,
		base->daint);
	LOG_DBG("  DAINTMSK: 0x%x  GHWCFG1 : 0x%x  GHWCFG2 : 0x%x", base->daintmsk, base->ghwcfg1,
		base->ghwcfg2);
	LOG_DBG("  GHWCFG3 : 0x%x  GHWCFG4 : 0x%x", base->ghwcfg3, base->ghwcfg4);

	for (i = 0U; i < USB_DW_OUT_EP_NUM; i++) {
		LOG_DBG("\n  EP %d registers:    DIEPCTL : 0x%x    DIEPINT : "
			"0x%x",
			i, base->in_ep[i].diepctl, base->in_ep[i].diepint);
		LOG_DBG("    DIEPTSIZ: 0x%x    DIEPDMA : 0x%x    DOEPCTL : "
			"0x%x",
			base->in_ep[i].dieptsiz, base->in_ep[i].diepdma, base->out_ep[i].doepctl);
		LOG_DBG("    DOEPINT : 0x%x    DOEPTSIZ: 0x%x    DOEPDMA : "
			"0x%x",
			base->out_ep[i].doepint, base->out_ep[i].doeptsiz, base->out_ep[i].doepdma);
	}
}
...
static int usb_dw_ep_set(uint8_t ep, uint32_t ep_mps, enum usb_dc_ep_transfer_type ep_type)
{
	...
	if (!ep_idx) {
		...
		*p_depctl &= ~USB_DWC2_DEPCTL0_MPS_MASK;
		...
		if (ep_mps > (USB_DWC2_DEPCTL_MPS_MASK >> USB_DWC2_DEPCTL_MPS_POS)) {
			return -EINVAL;
		}
...
static int usb_dw_tx(uint8_t ep, const uint8_t *const data, uint32_t data_len)
{
	...
	if (data_len != 0U) {
		...
		if (ep_idx == USB_DW_IN_EP_0) {
			max_pkt_cnt =
				USB_DWC2_DIEPTSIZ0_PKT_CNT_MASK >> USB_DWC2_DEPTSIZ_PKT_CNT_POS;
			max_xfer_size =
				USB_DWC2_DEPTSIZ0_XFER_SIZE_MASK >> USB_DWC2_DEPTSIZ_XFER_SIZE_POS;
		} else {
			max_pkt_cnt =
...
static int usb_dw_init(void)
{
	...
	base->gusbcfg =
		(base->gusbcfg & ~USB_DWC2_GUSBCFG_PHYIF_16_BIT) | USB_DWC2_GUSBCFG_PHYIF_16_BIT;
	...
	base->gintmsk = USB_DWC2_GINTSTS_OEPINT | USB_DWC2_GINTSTS_IEPINT |
