...
LOG_MODULE_REGISTER(usb_dc_mcux_ip3511fs);
...
#define NUM_OF_EP_MAX           DT_USBD_MCUX_IP3511FS_NUM_BIDIR_EP

/* The minimum value is 1 */
#define EP_BUF_NUMOF_BLOCKS     ((NUM_OF_EP_MAX + 3) / 4)

/* The max MPS is 1023 for FS, 1024 for HS.
 * IP3511 controller need the 64-bytes aligned buffer.
 */
K_MEM_POOL_DEFINE(ep_buf_pool, 64, 1024, EP_BUF_NUMOF_BLOCKS, 64);

static usb_ep_ctrl_data_t s_ep_ctrl[NUM_OF_EP_MAX];
static usb_device_struct_t dev_data;

/* IP3511 device driver interface */
static const usb_device_controller_interface_struct_t ip3511_iface = {
	USB_DeviceLpc3511IpInit, USB_DeviceLpc3511IpDeinit,
	USB_DeviceLpc3511IpSend, USB_DeviceLpc3511IpRecv,
	USB_DeviceLpc3511IpCancel, USB_DeviceLpc3511IpControl
};
...
int usb_dc_attach(void)
{
	...
	dev_data.interface = &ip3511_iface;
	...
	IRQ_CONNECT(DT_USBD_MCUX_IP3511FS_IRQ, DT_USBD_MCUX_IP3511FS_IRQ_PRI,
		    usb_isr_handler, 0, 0);
	irq_enable(DT_USBD_MCUX_IP3511FS_IRQ);
...
int usb_dc_ep_configure(const struct usb_dc_ep_cfg_data *const cfg)
{
	...
	if (dev_data.eps[ep_abs_idx].ep_enabled) {
		status = dev_data.interface->deviceControl(
			...
			(void *)(&cfg->ep_addr));
		if (kStatus_USB_Success != status) {
			LOG_ERR("Failed to de-initialize endpoint");
			return -EIO;
		}
	}
	status = dev_data.interface->deviceControl(dev_data.controllerHandle,
...
int usb_dc_ep_enable(const u8_t ep)
{
	...
	if (dev_data.eps[ep_abs_idx].ep_occupied) {
		LOG_WRN("Endpoint 0x%02x already enabled", ep);
		return -EBUSY;
	}
		    ...
		    const u32_t data_len, u32_t *const ret_bytes)
{
	...
	memcpy(buffer, data, len_to_send);
