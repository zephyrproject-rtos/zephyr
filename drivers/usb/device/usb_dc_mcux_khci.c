...
LOG_MODULE_REGISTER(usb_dc_mcux_khci);
...
#define NUM_OF_EP_MAX		DT_USBD_MCUX_KHCI_NUM_BIDIR_EP
...
int usb_dc_attach(void)
{
	...
	s_Device.interface = (const usb_device_controller_interface_struct_t *)(&s_UsbDeviceKhciInterface);
	if (kStatus_USB_Success != s_Device.interface->deviceInit(CONTROLLER_ID, &s_Device, &s_Device.controllerHandle)) {
		return -EINVAL;
	}

	/* Connect and enable USB interrupt */
	IRQ_CONNECT(DT_USBD_MCUX_KHCI_IRQ, DT_USBD_MCUX_KHCI_IRQ_PRI,
		    usb_isr_handler, 0, 0);
	irq_enable(DT_USBD_MCUX_KHCI_IRQ);
...
static void usb_isr_handler(void)
{
	USB_DeviceKhciIsrFunction(&s_Device);
}
