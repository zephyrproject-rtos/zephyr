...
static void vrt_assemble_frame(const struct device *dev)
{
	...
	SYS_DLIST_FOR_EACH_CONTAINER(&data->ctrl_xfers, tmp, node) {
		...
		if (interval) {
			...
			LOG_DBG("Interrupt transfer s.f. %u f.n. %u interval %u", tmp->start_frame,
				priv->frame_number, interval);
		}
			   ...
			   struct uhc_transfer *const xfer)
{
	...
	if (interval) {
		...
		LOG_DBG("New interrupt transfer s.f. %u f.n. %u interval %u", xfer->start_frame,
			priv->frame_number, interval);
	}
