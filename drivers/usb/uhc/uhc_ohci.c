...
/* wHubCharacteristics: logical power switching and over-current protection modes */
#define OHCI_RH_CHAR_LPSM_MASK GENMASK(1, 0)
#define OHCI_RH_CHAR_OCPM_MASK GENMASK(4, 3)
...
static const uint8_t ohci_rh_config_desc[] = {
	...
	0U,
	/* Interface */
	9U, USB_DESC_INTERFACE, 0U, 0U, 1U, USB_HUB_CLASS_CODE, 0x00U, 0x00U, 0U,
...
static size_t ohci_rh_hub_desc(const struct device *dev, uint8_t *const buf, const size_t size)
{
	...
	if ((rh_desc_a & OHCI_RHDA_NPS) != 0U) {
		chars = FIELD_PREP(OHCI_RH_CHAR_LPSM_MASK, 2U);
	} else {
		chars = FIELD_PREP(OHCI_RH_CHAR_LPSM_MASK,
				   (rh_desc_a & OHCI_RHDA_PSM) != 0U ? 1U : 0U);
	}

	if ((rh_desc_a & OHCI_RHDA_NOCP) != 0U) {
		chars |= FIELD_PREP(OHCI_RH_CHAR_OCPM_MASK, 2U);
	} else {
		chars |= FIELD_PREP(OHCI_RH_CHAR_OCPM_MASK,
				    (rh_desc_a & OHCI_RHDA_OCPM) != 0U ? 1U : 0U);
	}

	buf[0] = (uint8_t)len;
	buf[1] = USB_HUB_DESCRIPTOR_TYPE;
...
static void ohci_rh_control(const struct device *dev, struct uhc_transfer *const xfer)
{
	...
	if (type == USB_REQTYPE_TYPE_STANDARD) {
		...
		case USB_HCREQ_GET_STATUS:
			...
			sys_put_le16((status & OHCI_RHS_OCI) != 0U ? USB_HUB_STATUS_OVER_CURRENT
								   : 0U,
				     &buf[0]);
