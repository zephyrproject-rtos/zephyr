...
{
	/* Set FIFO top */
	dwc2_ll_gdfifocfg_set_ep_info_base_addr(dwc2, fifo->top);
	dwc2_ll_gdfifocfg_set_gdfifo_cfg(dwc2, fifo->top);
	...
	dwc2_ll_gnptxfsiz_set_nptx_fifo_start_addr(dwc2, fifo->top);
	dwc2_ll_gnptxfsiz_set_nptx_fifo_depth(dwc2, fifo->nptxfsiz);

	fifo->top -= fifo->ptxfsiz;

	dwc2_ll_hptxfsiz_set_host_tx_fifo_start_addr(dwc2, fifo->top);
...
static int uhc_dwc2_init_controller(const struct device *dev)
{
	...
	return uhc_dwc2_reset_core_soft(dev);
}
...
static inline void _buffer_exec_proceed(pipe_t *pipe)
{
	dma_buffer_t *buffer = pipe->buffer;
	...
	__ASSERT(buffer->flags.ctrl.cur_stg != 2,
			"_buffer_exec: Invalid control stage: %d", buffer->flags.ctrl.cur_stg);
		...
		switch (ret) {
			case UHC_PORT_EVENT_CONNECTION: {
				...
				if (uhc_dwc2_port_debounce(dev)) {
					...
					LOG_WRN("Port connection event occurred, but port is not connected after debounce");
					LOG_ERR("Is not implemented yet");
				}
				break;
			}
...
static inline void uhc_dwc2_flush_pipes(const struct device *dev)
{
	...
	pipe_t *pipe = (pipe_t*)priv->ctrl_pipe_hdl;
...
static inline void _buffer_fill_ctrl(dma_buffer_t *buffer, struct uhc_transfer *const xfer)
{
	...
	/* TODO: Check that pipe and port are in the correct state to receive XFERs */
	/*
	* Hint:
	* priv->dynamic.port_state == UHC_PORT_STATE_ENABLED
	* pipe->state == UHC_PIPE_STATE_ACTIVE
...
// TODO: IRAM_ATTR
static void _buffer_fill(pipe_t *pipe, struct uhc_transfer *const xfer)
{
	/* TODO: pending xfer? */

	/*
	* Hint:
	* Get an XFER from the pending tailq
	*/
...
static inline void _buffer_exec(pipe_t *pipe)
{
	...
	/* TODO: Verify the logic of xfer->ep in the usbh */
	bool is_in = false;
	...
	if (USB_EP_GET_IDX(xfer->ep) == 0) {
		...
		dwc2_ll_hcchar_set_dir(pipe->chan_obj->regs, is_in);
	}
	...
	if (USB_EP_DIR_IS_OUT(xfer->ep) == 0) {
		/* TODO: sync CACHE */
	}
				...
				pipe_hdl_t pipe_hdl, struct uhc_transfer *const xfer)
{
	...
	pipe_t *pipe = (pipe_t *)pipe_hdl;
	/* TODO: Add the XFER to the pipe's pending tailq */
	pipe->num_xfer_pending++;

	if (_buffer_can_fill(pipe)) {
	    _buffer_fill(pipe, xfer);
	}
