...
typedef enum {
	...
	/* A device is connected to the port but has not been reset. SOF/keep alive aren't being sent */
...
typedef enum {
	...
	/* A channel event has occurred. Call the the channel event handler instead */
...
typedef struct {
	union {
		struct {
			...
			uint32_t ls_via_fs_hub: 1;		/**< EP is on a LS device that is routed via FS hub */
			uint32_t reserved2: 2;
		};
		uint32_t val;
	};
	struct {
		unsigned int interval;				/**< Interval in frames (FS) or microframes (HS) */
		uint32_t offset;					/**< Offset of this channel in the periodic scheduler */
		bool is_hs;							/**< EP is HighSpeed */
...
typedef struct {
	union {
		struct {
			uint32_t active: 1;             /**< Debugging bit to indicate channel is enabled */
		...
		uint32_t val;
	} flags;                                 /**< Flags for channel's status and information */
	const struct usb_dwc2_host_chan *regs;   /**< Pointer to the Host channel's register set */
 ...
 */
typedef struct {
	struct uhc_transfer *xfer;		/**< Pointer to the transfer associated with the buffer */
	union {
		struct {
			...
			uint32_t cur_stg: 2;			/**< Index of the current stage */
	...
	union {
		struct {
			...
			uint32_t was_canceled: 1;		/**< Buffer was done due to a cancellation */
			uint32_t reserved6: 6;
			uint32_t reserved8_1: 8;
			uint32_t pipe_event: 8;		/**< The pipe event when the buffer was done */
...
struct pipe_obj_s {
	...
	union {
		struct {
			...
			uint32_t has_xfer: 1;		/**< Indicates that XFER is: pending, in-flight, or done */
			uint32_t event_pending: 1;	/**< Indicates that a pipe event is pending */
...
struct uhc_dwc2_data_s {
	...
	struct {
		...
		uhc_dwc2_channel_t **hdls;		/**< Handles of each channel */
	} channels;							/* Data, that is used in single thread */

	struct {
		union {
			struct {
				uint32_t lock_enabled: 1;			/**< Debounce lock flag */
				uint32_t fifo_sizes_set: 1;			/**< FIFO sizes have been applied */
				uint32_t periodic_sched_enabled: 1;	/**< Periodic scheduling is enabled */
				uint32_t event_pending: 1;			/**< Port has event is pending */
