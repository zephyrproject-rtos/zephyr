...
static inline int ch_process_control(const struct device *dev, struct uhc_dwc2_channel *ch)
{
	...
	sys_write32(dma_addr, (mem_addr_t)&ch->regs->hcdma);
...
static int ch_start_control(const struct device *dev, struct uhc_dwc2_channel *ch)
{
	struct uhc_transfer *const xfer = ch->xfer;
	const struct usb_setup_packet *setup = (const struct usb_setup_packet *)xfer->setup_pkt;
	uint32_t pkt_cnt;
...
static int ch_start_bulk(const struct device *dev, struct uhc_dwc2_channel *ch)
{
	struct uhc_transfer *const xfer = ch->xfer;
	uint32_t pkt_cnt;
	uint32_t dma_addr = 0;
...
static int ch_reinit(const struct device *dev, struct uhc_dwc2_channel *ch)
{
	uint32_t hcchar;
	int ret = 0;
