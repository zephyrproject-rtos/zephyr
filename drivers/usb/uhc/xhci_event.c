			    ...
			    int br, uint32_t cc, uint32_t lenfield)
{
	...
	if (slot->udev != NULL && xfer->udev != slot->udev) {
		LOG_WRN("bulk DCI%u giveback ignored (stale xfer udev)", (unsigned int)dci);
...
void xhci_handle_event(struct uhc_dwc3_data *priv, struct xhci_trb *evt)
{
	...
	case XHCI_TRB_TRANSFER_EVENT: {
		...
		if (ep_id_evt == (uint32_t)XHCI_DCI_DEFAULT_CONTROL) {
			...
			if (slot->udev != NULL &&
			    slot->bulk_active_xfer[ep_id_evt]->udev != slot->udev) {
				LOG_WRN("bulk DCI%u ignore xfer evt (udev stale, slot reuse?)",
					(unsigned int)ep_id_evt);
				slot->bulk_active_xfer[ep_id_evt] = NULL;
				break;
			}
