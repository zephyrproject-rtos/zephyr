			      ...
			      struct uhc_transfer *const xfer)
{
	...
	const uint16_t mps = MIN(USB_MPS_EP_SIZE(xfer->mps), MAX3421E_MAX_EP_SIZE);
	int ret;
				 ...
				 const uint8_t hrsl)
{
	...
	if (buf != NULL && xfer->stage == UHC_CONTROL_STAGE_DATA) {
		...
		return max3421e_xfer_data(dev, xfer);
	}
...
static int max3421e_hrslt_success(const struct device *dev)
{
	...
	case MAX3421E_HXFR_TYPE_BULKIN: {
		const uint16_t mps = MIN(USB_MPS_EP_SIZE(xfer->mps),
					 MAX3421E_MAX_EP_SIZE);
		...
		if (bc < mps || !net_buf_tailroom(buf)) {
