...
static struct uhc_transfer *peek_next_ctrl_ep0(const struct device *dev)
{
	...
	for (sys_dnode_t *node = sys_dlist_peek_head(&data->ctrl_xfers); node != NULL;
...
{
	for (sys_dnode_t *node = sys_dlist_peek_head(list); node != NULL;
...
static bool stm32_ctrl_handle_wait_or_error(const struct device *dev,
					...
					struct stm32_pipe *p)
{
	...
	if (stm32_ctrl_handle_wait_or_error(dev, d, p, xfer, (uint8_t)hc, urb_state, hal_cnt)) {
		return;
	}
...
static void stm32_schedule_nonctrl_fill(const struct device *dev, struct stm32_otghs_data *d)
{
	...
	while (stm32_count_active_nonctrl(d) < STM32_OTGHS_MAX_INFLIGHT_BULK) {
		...
		for (sys_dnode_t *node = sys_dlist_peek_head(&ud->bulk_xfers); node != NULL;
...
static int stm32_driver_preinit(const struct device *dev)
{
	...
	k_thread_create(&d->thread, cfg->stack, cfg->stack_size, stm32_otghs_thread,
			(void *)dev, NULL, NULL, cfg->thread_prio, 0, K_NO_WAIT);
...
#define STM32_OTGHS_INST_DEFINE(n)                                                                 \
	static void stm32_otghs_irq_connect_##n(void)                                            \
	{                                                                                          \
		...
		irq_enable(DT_INST_IRQN(n));                                                       \
                                                                                                   ...
                                                                                                   \
	static K_KERNEL_STACK_DEFINE(stm32_otghs_stack_##n, STM32_OTGHS_STACK_SIZE_DEFAULT);     \
                                                                                                   \
	...
	static const struct stm32_otghs_config stm32_otghs_cfg_##n = {                             \
		...
		.irq_connect = stm32_otghs_irq_connect_##n,                                        \
		...
		.thread_prio = K_PRIO_PREEMPT(STM32_OTGHS_THREAD_PRIO_DEFAULT),                    \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, stm32_driver_preinit, NULL, &stm32_uhc_data_##n,                \
