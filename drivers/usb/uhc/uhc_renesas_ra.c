...
#define UHC_RENESAS_RA_NUM_DEVICE           1
...
static int uhc_renesas_ra_edpt_open(const struct device *dev, uint8_t dev_addr, uint8_t ep_type,
				    uint8_t ep, uint8_t max_packet_size)
{
	struct uhc_renesas_ra_data *priv = uhc_get_private(dev);
	int ret;

	if (dev_addr > UHC_RENESAS_RA_NUM_DEVICE) {
		LOG_ERR("Exceed maximum device");
		return -EINVAL;
	}
...
static int uhc_renesas_ra_schedule_xfer(const struct device *dev)
{
	...
	if (!atomic_test_bit(&priv->ep, priv->last_xfer->ep)) {
		const uint8_t ep_type = priv->last_xfer->attrib & USB_EP_TRANSFER_TYPE_MASK;

		ret = uhc_renesas_ra_edpt_open(dev, priv->last_xfer->addr, ep_type,
...
static int uhc_renesas_ra_event_xfer_complete(const struct device *dev, usb_event_t *hal_evt)
{
	...
	if (hal_evt->result != USB_XFER_RESULT_SUCCESS) {
