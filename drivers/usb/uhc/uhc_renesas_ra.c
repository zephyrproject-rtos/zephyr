...
{
	struct uhc_renesas_ra_data *priv = uhc_get_private(dev);
	const struct usb_host_ep *host_ep = USB_EP_DIR_IS_IN(xfer->ep)
						    ? &xfer->udev->ep_in[USB_EP_GET_IDX(xfer->ep)]
						    : &xfer->udev->ep_out[USB_EP_GET_IDX(xfer->ep)];
	const uint8_t ep_type = host_ep->desc->Attributes.transfer;
	int ret;

	if (ep_type == USB_EP_TYPE_ISO || ep_type == USB_EP_TYPE_INTERRUPT) {
		if (!atomic_test_bit(&priv->ep, xfer->ep)) {
			ret = uhc_renesas_ra_edpt_open(dev, xfer->udev->addr,
						       host_ep->desc->bmAttributes, xfer->ep,
						       xfer->mps);
			if (ret != 0) {
				return ret;
			}
...
static int uhc_ra_driver_preinit(const struct device *dev)
{
	...
	if (priv->uhc_cfg.hs_irq != BSP_IRQ_DISABLED) {
		R_ICU->IELSR[priv->uhc_cfg.hs_irq] = ELC_EVENT_USBHS_USB_INT_RESUME;
	}
...
#define USB_MODULE_NUMBER(n) ((DT_REG_ADDR(n)) == R_USB_HS0_BASE ? 1 : 0)
