				       ...
				       usb_status_t status)
{
	...
	if (status == kStatus_USB_Success) {
		...
		if ((xfer->ep == 0) && (transfer->setupPacket->bRequest == USB_SREQ_SET_ADDRESS)) {
			...
			/* set address to let USB_HostHelperGetPeripheralInformation get right value
			 */
			xfer->addr = (uint8_t)transfer->setupPacket->wValue;
			mcux_ep_handle = uhc_mcux_init_hal_ep(dev, xfer);
			if (mcux_ep_handle != NULL) {
				uhc_mcux_control(dev, kUSB_HostUpdateControlEndpointAddress,
						 mcux_ep_handle);
			}
		}
	}
...
static int uhc_mcux_enqueue(const struct device *dev, struct uhc_transfer *const xfer)
{
	...
	mcux_xfer = uhc_mcux_hal_init_transfer(dev, mcux_ep_handle, xfer);
	if (mcux_xfer == NULL) {
		return -ENOMEM;
	}
...
static int uhc_mcux_dequeue(const struct device *dev, struct uhc_transfer *const xfer)
{
	/* TODO */
	return 0;
}
