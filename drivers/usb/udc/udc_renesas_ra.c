...
struct udc_renesas_ra_data {
	...
	struct k_msgq msgq;
	struct udc_renesas_ra_evt msg_buf[CONFIG_UDC_RENESAS_RA_MAX_QMESSAGES];
...
static void udc_renesas_ra_event_handler(usbd_callback_arg_t *p_args)
{
	...
	default:
		...
		k_msgq_put(&priv->msgq, &evt, K_NO_WAIT);
		break;
	}
}
...
static ALWAYS_INLINE void renesas_ra_thread_handler(void *const arg)
{
	...
	while (true) {
		...
		k_msgq_get(&data->msgq, &evt, K_FOREVER);
		switch (evt.type) {
				     ...
				     struct net_buf *buf)
{
	...
	k_msgq_put(&data->msgq, &evt, K_NO_WAIT);
...
static int udc_renesas_ra_driver_preinit(const struct device *dev)
{
	...
	k_msgq_init(&priv->msgq, (char *)priv->msg_buf, sizeof(struct udc_renesas_ra_evt),
		    CONFIG_UDC_RENESAS_RA_MAX_QMESSAGES);
...
static int udc_renesas_ra_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		if (priv->udc_cfg.module_number == 1) {
