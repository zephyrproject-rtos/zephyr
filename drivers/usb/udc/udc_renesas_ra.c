...
static void udc_event_handler(usbd_callback_arg_t *p_args)
{
	struct udc_renesas_ra_evt evt = {
		.type = UDC_RENESAS_RA_EVT_HAL,
...
void usb_interrupt_handler(void *arg)
{
	ARG_UNUSED(arg);
	usb_device_isr();
}
...
static int usbd_ctrl_feed_dout(const struct device *dev, const size_t length)
{
	...
	return FSP_SUCCESS == R_USBD_XferStart(&data->udc, cfg->addr, buf->data, buf->size) ? 0
											    : -EIO;
}
...
static int udc_event_xfer_setup(const struct device *dev, struct udc_renesas_ra_evt *evt)
{
	...
	memcpy(buf->data, setup_packet, sizeof(struct usb_setup_packet));
...
static void udc_event_xfer_ctrl_in(const struct device *dev, struct net_buf *const buf)
{
	...
	/* Clear setup endpoint. Send a ZLP */
...
static void udc_event_xfer_out(const struct device *dev, struct net_buf *const buf, uint32_t len)
{
	net_buf_add(buf, len);
	udc_submit_ep_event(dev, buf, 0);
}

static void udc_event_xfer_in(const struct device *dev, struct net_buf *const buf)
{
	udc_submit_ep_event(dev, buf, 0);
}
...
static void udc_event_xfer_complete(const struct device *dev, struct udc_renesas_ra_evt *evt)
{
	...
	unsigned int ep = evt->hal_evt.xfer_complete.ep_addr;
	unsigned int result = evt->hal_evt.xfer_complete.result;
	unsigned int len = evt->hal_evt.xfer_complete.len;
	...
	if (result == USBD_XFER_RESULT_SUCCESS) {
		...
		if (ep == USB_CONTROL_EP_IN) {
			...
			if (USB_EP_DIR_IS_OUT(ep)) {
				udc_event_xfer_out(dev, buf, len);
			} else {
				udc_event_xfer_in(dev, buf);
			}
		}
	} else {
...
static void udc_fake_status_in(const struct device *dev, struct udc_ep_config *const cfg)
{
	struct udc_renesas_ra_evt evt = {
		.type = UDC_RENESAS_RA_EVT_STATUS,
				     ...
				     struct net_buf *buf)
{
	...
	if (cfg->addr == USB_CONTROL_EP_IN && buf->len == 0) {
		udc_fake_status_in(dev, cfg);
		return 0;
	}

	struct udc_renesas_ra_evt evt = {
		.type = UDC_RENESAS_RA_EVT_XFER,
		.ep = cfg->addr,
	};
...
static int udc_renesas_ra_ep_enable(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	usbd_desc_endpoint_t ep_desc = {
		...
		.bEndpointAddress = cfg->addr,
		.bmAttributes = cfg->attributes,
		.wMaxPacketSize = cfg->mps,
		.bInterval = cfg->interval,
	};
...
{
	if (addr != R_USB_FS0->USBADDR_b.USBADDR) {
...
#define USBHS_BASE_ADDR         (0x40252000)
#define USB_MODULE_NUMBER(addr) ((addr == USBHS_BASE_ADDR) ? 1 : 0)
...
#define RENESAS_RA_USB_IRQ_CONFIG_FUNC(n)                                                          \
	...
	{                                                                                          \
		...
		IRQ_CONNECT(DT_IRQ_BY_NAME(DT_NODELABEL(usbhs), usbhs_ir, irq),                    \
			    ...
			    usb_interrupt_handler, DEVICE_DT_INST_GET(n), 0);                      \
		return 0;                                                                          \
	}
...
#define UDC_RENESAS_RA_DEVICE_DEFINE(n)                                                            \
	...
	static struct udc_renesas_ra_data udc_priv_##n = {                                         \
		.udc_cfg =                                                                         \
			{                                                                          \
				.module_number =                                                   \
					USB_MODULE_NUMBER(DT_REG_ADDR(DT_INST_PARENT(n))),         \
				.usb_speed = DT_ENUM_IDX_OR(DT_INST_PARENT(n), maximum_speed,      \
							    UDC_BUS_SPEED_HS),                     \
				.p_callback = udc_event_handler,                                   \
			},                                                                         \
	};                                                                                         \
                                                                                                   \
