...
#define IT51XXX_UDC_EXTEND_CTRL_ENABLED     DT_ALL_INST_HAS_PROP_STATUS_OKAY(ite_extend_ctrl)
...
struct it51xxx_config {
	...
	uint8_t num_of_in_eps;
	uint8_t num_of_out_eps;
	uint8_t num_of_eps;
...
static inline size_t it51xxx_udc_get_rx_len(const struct device *dev, const uint8_t ep)
{
	const struct it51xxx_config *config = dev->config;
	mem_addr_t rx_fifo_base = config->base + epn_rx_fifo_base[USB_EP_GET_IDX(ep)];

	return sys_read8(rx_fifo_base + UDC63_EPN_RX_FIFO_COUNT_LSB) +
				...
				bool enable)
{
	...
	WRITE_BIT(epn_ctrl_val, u32_count_trailing_zeros(mask), enable);
...
static void it51xxx_udc_ip_init(const struct device *dev)
{
	const struct it51xxx_config *config = dev->config;
...
static void it51xxx_xfer_in_data(const struct device *dev, uint8_t ep, struct net_buf *buf)
{
	...
	LOG_DBG("written %d bytes to endpoint %#x tx fifo", len, ep);
}

static int it51xxx_xfer_out_data(const struct device *dev, const uint8_t ep,
				 struct net_buf *const buf, const size_t rx_len)
{
	const uint8_t ep_idx = USB_EP_GET_IDX(ep);
	const struct it51xxx_config *config = dev->config;
	mem_addr_t rx_fifo_base = config->base + epn_rx_fifo_base[ep_idx];
...
static inline int work_handler_setup(const struct device *dev, uint8_t ep)
{
	...
	len = it51xxx_udc_get_rx_len(dev, ep);
...
static void it51xxx_udc_xfer_done(const struct device *dev, const uint8_t ep_idx)
{
	const struct it51xxx_config *config = dev->config;
	mem_addr_t ctrl_base = config->base + epn_ctrl_base[ep_idx];
	uint8_t epn_ctrl_val = sys_read8(ctrl_base + UDC40_EPN_CTRL);
	uint8_t xfer_type = TRANSACTION_TYPE(sys_read8(ctrl_base + UDC42_EPN_TRANSACTION_TYPE));
	uint8_t ep = (xfer_type == IT51XXX_XFER_TYPE_IN ? USB_EP_DIR_IN : USB_EP_DIR_OUT) | ep_idx;
...
static void it51xxx_udc_isr(const void *arg)
{
	...
	if (int_sts & TRANS_DONE) {
		...
		for (uint8_t ep_idx = 0; ep_idx < config->num_of_eps; ep_idx++) {
			it51xxx_udc_xfer_done(dev, ep_idx);
		}
		return;
	}
}
...
BUILD_ASSERT(DT_ALL_INST_HAS_PROP_STATUS_OKAY(ite_extend_ctrl) ||
...
#define IT51XXX_USB_DEVICE_DEFINE(n)                                                               \
	...
	PINCTRL_DT_INST_DEFINE(n);                                                                 \
                                                                                                   \
	static struct udc_ep_config ep_cfg_out_##n[IT51XXX_NUM_OF_EPS(n)];                         \
	static struct udc_ep_config ep_cfg_in_##n[IT51XXX_NUM_OF_EPS(n)];                          \
                                                                                                   \
	static void it51xxx_udc_config_func_##n(void)                                              \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(n, usb, irq), 0, it51xxx_udc_isr,                  \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		IRQ_CONNECT(DT_INST_IRQ_BY_NAME(n, wu, irq), 0, it51xxx_wu_isr,                    \
			    DEVICE_DT_INST_GET(n), 0);                                             \
	};                                                                                         \
                                                                                                   \
	static struct it51xxx_config udc_cfg_##n = {                                               \
		...
		.irq_config_func = it51xxx_udc_config_func_##n,                                    \
		.usb_irq = DT_INST_IRQ_BY_NAME(n, usb, irq),                                       \
		.wu_irq = DT_INST_IRQ_BY_NAME(n, wu, irq),                                         \
