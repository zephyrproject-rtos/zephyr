...
static void start_rx_dma(const struct usb_smartbond_dma_config *dma_cfg, volatile void *src,
			 void *dst, uint16_t size)
{
	if (dma_reload(dma_cfg->rx_dev, dma_cfg->rx_chan, (uint32_t)src, (uint32_t)dst, size) < 0) {
...
static void start_rx_packet(struct usb_smartbond_data *data, struct smartbond_ep_state *ep_state)
{
	...
	if (mps > config->dma_min_transfer_size) {
		if (try_allocate_dma(data, ep_state)) {
			start_rx_dma(&config->dma_cfg, &regs->rxd, net_buf_tail(buf), mps);
...
static void start_tx_dma(const struct usb_smartbond_dma_config *dma_cfg, void *src,
			 volatile void *dst, uint16_t size)
{
	if (dma_reload(dma_cfg->tx_dev, dma_cfg->tx_chan, (uint32_t)src, (uint32_t)dst, size) < 0) {
...
static void start_tx_packet(struct usb_smartbond_data *data, struct smartbond_ep_state *ep_state)
{
	...
	if (ep != USB_CONTROL_EP_IN && size > config->dma_min_transfer_size &&
		...
		start_tx_dma(&config->dma_cfg, buf->data, &regs->txd, size);
	} else {
