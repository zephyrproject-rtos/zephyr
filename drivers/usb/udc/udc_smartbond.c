...
#define EP_MAX DT_INST_PROP(0, num_bidir_endpoints)
_Static_assert(EP_MAX > 0 && EP_MAX <= 4, "num-bidir-endpoints out of range");
...
#define EP0_OUT_STATE(dev_state) (&dev_state->ep_state[0][0])
#define EP0_IN_STATE(dev_state)  (&dev_state->ep_state[1][0])
...
static struct smartbond_ep_state *usb_dc_get_ep_state(struct usb_dc_state *dev_state, uint8_t ep)
{
...
static struct smartbond_ep_state *usb_dc_get_ep_out_state(struct usb_dc_state *dev_state,
							  uint8_t ep_idx)
{
	return ep_idx < EP_MAX ? &dev_state->ep_state[0][ep_idx] : NULL;
}

static struct smartbond_ep_state *usb_dc_get_ep_in_state(struct usb_dc_state *dev_state,
							 uint8_t ep_idx)
{
	return ep_idx < EP_MAX ? &dev_state->ep_state[1][ep_idx] : NULL;
}

static void set_nfsr(struct usb_dc_state *dev_state, uint8_t val)
{
	dev_state->nfsr = val;
	/*
...
static void fill_tx_fifo(struct smartbond_ep_state *ep_state)
{
	...
	const struct udc_ep_config *ep_config = &ep_state->config;
	const uint16_t mps = udc_mps_ep_size(ep_config);
	const uint8_t ep_idx = USB_EP_GET_IDX(ep_config->addr);
...
static bool try_allocate_dma(struct usb_dc_state *dev_state, struct smartbond_ep_state *ep_state)
{
	struct udc_ep_config *ep_config = &ep_state->config;
	const uint8_t ep = ep_config->addr;
	uint8_t ep_idx = USB_EP_GET_IDX(ep);
...
static void start_rx_packet(struct usb_dc_state *dev_state, struct smartbond_ep_state *ep_state)
{
	struct udc_ep_config *ep_config = &ep_state->config;
	const struct udc_smartbond_config *cfg = dev_state->dev->config;
	const uint8_t ep = ep_config->addr;
	struct smartbond_ep_reg_set *regs = ep_state->regs;
	struct net_buf *buf = ep_state->buf;
	uint8_t ep_idx = USB_EP_GET_IDX(ep);
	...
	if (mps > cfg->dma_min_transfer_size) {
		if (try_allocate_dma(dev_state, ep_state)) {
			start_rx_dma(&regs->rxd, net_buf_tail(buf), mps);
...
static void start_tx_packet(struct usb_dc_state *dev_state, struct smartbond_ep_state *ep_state)
{
	const struct udc_smartbond_config *cfg = dev_state->dev->config;
	struct smartbond_ep_reg_set *regs = ep_state->regs;
	struct udc_ep_config *ep_config = &ep_state->config;
	...
	if (ep_config->stat.data1) {
		txc |= USB_USB_TXC1_REG_USB_TOGGLE_TX_Msk;
	}

	if (ep != USB_CONTROL_EP_IN && size > cfg->dma_min_transfer_size &&
	    (uint32_t)(buf->data) >= CONFIG_SRAM_BASE_ADDRESS &&
	    try_allocate_dma(dev_state, ep_state)) {
		start_tx_dma(buf->data, &regs->txd, size);
	} else {
...
static void handle_ep0_rx(struct usb_dc_state *dev_state)
{
	int fifo_bytes;
	uint32_t rxs0 = USB->USB_RXS0_REG;
	struct smartbond_ep_state *ep0_out_state = EP0_OUT_STATE(dev_state);
	...
	if (rxs0 & USB_USB_RXS0_REG_USB_SETUP_Msk) {
		ep0_in_state = EP0_IN_STATE(dev_state);
		ep0_in_config = &ep0_in_state->config;
		ep0_out_state->last_packet_size = 0;
		read_rx_fifo(ep0_out_state, dev_state->setup_buffer, EP0_FIFO_SIZE);
		...
		LOG_DBG("Setup %02x %02x %02x %02x %02x %02x %02x %02x", dev_state->setup_buffer[0],
			dev_state->setup_buffer[1], dev_state->setup_buffer[2],
			dev_state->setup_buffer[3], dev_state->setup_buffer[4],
			dev_state->setup_buffer[5], dev_state->setup_buffer[6],
			dev_state->setup_buffer[7]);
		REG_CLR_BIT(USB_MAMSK_REG, USB_M_EP0_NAK);
		REG_CLR_BIT(USB_RXC0_REG, USB_RX_EN);
		(void)USB->USB_EP0_NAK_REG;
		k_work_submit_to_queue(udc_get_work_q(), &dev_state->ep0_setup_work);
	} else {
		(void)USB->USB_EP0_NAK_REG;
			...
			if (rxs0 & USB_USB_RXS0_REG_USB_RX_LAST_Msk) {
				...
				if (ep0_out_state->last_packet_size < EP0_FIFO_SIZE ||
					...
					k_work_submit_to_queue(udc_get_work_q(),
							       &dev_state->ep0_rx_work);
				} else {
					start_rx_packet(dev_state, ep0_out_state);
				}
			}
		}
	}
...
{
	struct usb_dc_state *dev_state = dev->data;
	struct smartbond_ep_state *ep_state = usb_dc_get_ep_in_state(dev_state, USB_EP_GET_IDX(ep));
	struct net_buf *buf;
	...
	if (buf) {
		...
		start_tx_packet(dev_state, ep_state);
...
static int udc_smartbond_ep_rx(const struct device *dev, uint8_t ep)
{
	...
	struct smartbond_ep_state *ep_state =
		usb_dc_get_ep_out_state(dev_state, USB_EP_GET_IDX(ep));
	struct net_buf *buf;
	...
	if (buf) {
		...
		start_rx_packet(dev_state, ep_state);
 ...
 * but usually it is offloaded to a thread or work queue to handle transfers
 * in a single location. Please refer to existing driver implementations
 * for examples.
 */
static int udc_smartbond_ep_enqueue(const struct device *dev, struct udc_ep_config *const ep_config,
				    struct net_buf *buf)
{
	...
	udc_buf_put(ep_config, buf);

	if (ep_config->stat.halted) {
		/*
 ...
 */
static int udc_smartbond_ep_dequeue(const struct device *dev, struct udc_ep_config *const ep_config)
{
	const uint8_t ep = ep_config->addr;
	unsigned int lock_key;
	struct net_buf *buf;
 ...
 */
int udc_smartbond_ep_enable(const struct device *dev, struct udc_ep_config *const ep_config)
{
	const uint8_t ep = ep_config->addr;
	struct smartbond_ep_state *ep_state;
	bool iso = (ep_config->attributes & USB_EP_TRANSFER_TYPE_MASK) == USB_EP_TYPE_ISO;
	uint8_t ep_idx = USB_EP_GET_IDX(ep);
	...
	ep_state = (struct smartbond_ep_state *)(ep_config);
	if (USB_EP_DIR_IS_IN(ep)) {
...
/*
 * Opposite function to udc_smartbond_ep_enable(). udc_ep_disable_internal()
 * may be used by the driver to disable control endpoints.
 */
static int udc_smartbond_ep_disable(const struct device *dev, struct udc_ep_config *const ep_config)
{
	struct usb_dc_state *dev_state = dev->data;
	const uint8_t ep = ep_config->addr;
	struct smartbond_ep_state *ep_state;
	...
	ep_state = usb_dc_get_ep_state(dev_state, ep);
	if (USB_EP_DIR_IS_IN(ep)) {
		ep_state->regs->txc =
...
static int udc_smartbond_ep_set_halt(const struct device *dev,
				     struct udc_ep_config *const ep_config)
{
	struct smartbond_ep_state *ep_state = (struct smartbond_ep_state *)(ep_config);
	struct net_buf *buf;
	const uint8_t ep = ep_config->addr;

	LOG_DBG("Set halt ep 0x%02x", ep);

	ep_config->stat.halted = 1;
	if (ep_config->addr == USB_CONTROL_EP_IN) {
 ...
 */
static int udc_smartbond_ep_clear_halt(const struct device *dev,
				       struct udc_ep_config *const ep_config)
{
	const uint8_t ep = ep_config->addr;
	struct smartbond_ep_state *ep_state = (struct smartbond_ep_state *)(ep_config);
	...
	ep_config->stat.data1 = 0;
	ep_config->stat.halted = 0;
...
static int udc_smartbond_host_wakeup(const struct device *dev)
{
	...
	if (dev_state->nfsr == NFSR_NODE_SUSPEND) {
		/*
		 * Enter fake state that will use FRAME interrupt to wait before
		 * going operational.
		 */
		set_nfsr(dev_state, NFSR_NODE_WAKING);
...
static uint32_t check_reset_end(struct usb_dc_state *dev_state, uint32_t alt_ev)
{
	if (dev_state->nfsr == NFSR_NODE_RESET) {
		...
		if (GET_BIT(alt_ev, USB_USB_ALTEV_REG_USB_RESET) == 0) {
			...
			if (dev_state->ep_state[0][0].buf != NULL) {
				USB->USB_MAMSK_REG |= USB_USB_MAMSK_REG_USB_M_EP0_RX_Msk;
			}
			LOG_DBG("Set operational %02x", USB->USB_MAMSK_REG);
			set_nfsr(dev_state, NFSR_NODE_OPERATIONAL);
		}
	}
	return alt_ev;
}

void handle_ep0_tx(struct usb_dc_state *dev_state)
{
	uint32_t txs0;
	struct smartbond_ep_state *ep0_in_state = EP0_IN_STATE(dev_state);
	...
	if (GET_BIT(txs0, USB_USB_TXS0_REG_USB_TX_DONE)) {
		...
		if (start_next_packet) {
			start_tx_packet(dev_state, ep0_in_state);
		}
	}
}

static void handle_epx_rx_ev(struct usb_dc_state *dev_state, uint8_t ep_idx)
{
	uint32_t rxs;
	int fifo_bytes;
	struct smartbond_ep_state *ep_state = usb_dc_get_ep_out_state(dev_state, ep_idx);
	const struct udc_smartbond_config *cfg = dev_state->dev->config;
	struct smartbond_ep_reg_set *regs = ep_state->regs;
	struct udc_ep_config *ep_config = &ep_state->config;
	struct net_buf *buf = ep_state->buf;
	...
	do {
		...
		if (GET_BIT(rxs, USB_USB_RXS1_REG_USB_RX_LAST)) {
			if (!ep_state->iso &&
			    GET_BIT(rxs, USB_USB_RXS1_REG_USB_TOGGLE_RX) != ep_config->stat.data1) {
				/* Toggle bit does not match discard packet */
				regs->rxc |= USB_USB_RXC1_REG_USB_FLUSH_Msk;
				ep_state->last_packet_size = 0;
				...
				ep_config->stat.data1 ^= 1;
				REG_SET_VAL(USB_TXC0_REG, USB_TOGGLE_TX0, ep_config->stat.data1);
				net_buf_add(buf, ep_state->last_packet_size);

				if (net_buf_tailroom(buf) == 0 ||
				    ep_state->last_packet_size < udc_mps_ep_size(ep_config) ||
				    ep_state->iso) {
					buf = udc_buf_get(dev_state->dev, ep_config->addr);
					if (unlikely(buf == NULL)) {
						LOG_ERR("ep 0x%02x queue is empty",
							ep_config->addr);
						break;
					}
					ep_config->stat.busy = 0;
					udc_submit_ep_event(dev_state->dev, buf, 0);
					break;
				}
				start_rx_packet(dev_state, ep_state);
			}
		}
	} while (fifo_bytes > cfg->fifo_read_threshold);
}

static void handle_rx_ev(struct usb_dc_state *dev_state)
{
	if (USB->USB_RXEV_REG & BIT(0)) {
		handle_epx_rx_ev(dev_state, 1);
	}

	if (USB->USB_RXEV_REG & BIT(1)) {
		handle_epx_rx_ev(dev_state, 2);
	}

	if (USB->USB_RXEV_REG & BIT(2)) {
		handle_epx_rx_ev(dev_state, 3);
	}
}

static void handle_epx_tx_ev(struct usb_dc_state *dev_state, struct smartbond_ep_state *ep_state)
{
	uint32_t txs;
	struct smartbond_ep_reg_set *regs = ep_state->regs;
	struct udc_ep_config *ep_config = &ep_state->config;
		...
		if (GET_BIT(txs, USB_USB_TXS1_REG_USB_ACK_STAT)) {
			...
			if (buf->len == 0) {
				if (udc_ep_buf_has_zlp(buf)) {
					...
					udc_ep_set_busy(dev_state->dev, ep, false);
					buf = udc_buf_get(dev_state->dev, ep);

					udc_submit_ep_event(dev_state->dev, buf, 0);
					udc_smartbond_ep_tx(dev_state->dev, ep);
				}
				return;
			}
...
static void handle_tx_ev(struct usb_dc_state *dev_state)
{
	if (USB->USB_TXEV_REG & BIT(0)) {
		handle_epx_tx_ev(dev_state, usb_dc_get_ep_in_state(dev_state, 1));
	}
	if (USB->USB_TXEV_REG & BIT(1)) {
		handle_epx_tx_ev(dev_state, usb_dc_get_ep_in_state(dev_state, 2));
	}
	if (USB->USB_TXEV_REG & BIT(2)) {
		handle_epx_tx_ev(dev_state, usb_dc_get_ep_in_state(dev_state, 3));
	}
}

static void handle_epx_tx_warn_ev(struct usb_dc_state *dev_state, uint8_t ep_idx)
{
	fill_tx_fifo(usb_dc_get_ep_in_state(dev_state, ep_idx));
}

static void handle_fifo_warning(struct usb_dc_state *dev_state)
{
	uint32_t fifo_warning = USB->USB_FWEV_REG;

	if (fifo_warning & BIT(0)) {
		handle_epx_tx_warn_ev(dev_state, 1);
	}

	if (fifo_warning & BIT(1)) {
		handle_epx_tx_warn_ev(dev_state, 2);
	}

	if (fifo_warning & BIT(2)) {
		handle_epx_tx_warn_ev(dev_state, 3);
	}
...
static void handle_ep0_nak(struct usb_dc_state *dev_state)
{
	...
	if (REG_GET_BIT(USB_EPC0_REG, USB_STALL)) {
		...
		if (REG_GET_BIT(USB_RXC0_REG, USB_RX_EN) == 0 &&
			...
			(void)USB->USB_EP0_NAK_REG;
			k_work_submit_to_queue(udc_get_work_q(), &dev_state->ep0_tx_work);
		}
	}
}
...
static void handle_bus_reset(struct usb_dc_state *dev_state)
{
	uint32_t alt_ev;
	...
	USB->USB_TXMSK_REG = 0;
	USB->USB_RXMSK_REG = 0;
	set_nfsr(dev_state, NFSR_NODE_RESET);

	for (int i = 0; i < EP_MAX; ++i) {
		dev_state->ep_state[1][i].buf = NULL;
		dev_state->ep_state[1][i].config.stat.busy = 0;
	}

	LOG_INF("send USB_DC_RESET");
	udc_submit_event(dev_state->dev, UDC_EVT_RESET, 0);
	...
	check_reset_end(dev_state, alt_ev);
	empty_ep0_queues(dev_state->dev);
}

static void usb_clock_on(struct usb_dc_state *dev_state)
{
...
static void usb_clock_off(struct usb_dc_state *dev_state)
{
...
static void handle_alt_ev(struct usb_dc_state *dev_state)
{
	...
	if (GET_BIT(alt_ev, USB_USB_ALTEV_REG_USB_RESET) && dev_state->nfsr != NFSR_NODE_RESET) {
		...
		if (USB->USB_NFSR_REG == NFSR_NODE_SUSPEND) {
			...
			for (int ep_idx = 1; ep_idx < EP_MAX; ++ep_idx) {
				ep_state = usb_dc_get_ep_out_state(dev_state, ep_idx);
				if (!ep_state->config.stat.halted) {
					start_rx_packet(dev_state, ep_state);
				}
			}
			udc_submit_event(dev_state->dev, UDC_EVT_RESUME, 0);
		}
	} else if (GET_BIT(alt_ev, USB_USB_ALTEV_REG_USB_SD3)) {
		set_nfsr(dev_state, NFSR_NODE_SUSPEND);
		USB->USB_ALTMSK_REG =
			USB_USB_ALTMSK_REG_USB_M_RESET_Msk | USB_USB_ALTMSK_REG_USB_M_RESUME_Msk;
		usb_clock_off(dev_state);
		udc_submit_event(dev_state->dev, UDC_EVT_SUSPEND, 0);
	}
}
...
static void udc_smartbond_isr(const struct device *dev)
{
	...
	if (GET_BIT(int_status, USB_USB_MAEV_REG_USB_FRAME)) {
		if (dev_state->nfsr == NFSR_NODE_RESET) {
			 ...
			 * actual SOF.
			 */
			check_reset_end(dev_state, USB_USB_ALTEV_REG_USB_RESET_Msk);
 ...
 */
static void usb_change_state(struct usb_dc_state *dev_state, bool attached, bool vbus_present)
{
	if (dev_state->attached == attached && dev_state->vbus_present == vbus_present) {
		return;
	}

	if (vbus_present != dev_state->vbus_present && attached) {
		udc_submit_event(dev_state->dev,
...
static void usb_dc_smartbond_vbus_isr(struct usb_dc_state *dev_state)
{
	LOG_DBG("VBUS_ISR");

	CRG_TOP->VBUS_IRQ_CLEAR_REG = 1;
	usb_change_state(dev_state, dev_state->attached,
...
static int usb_dc_smartbond_alloc_status_out(const struct device *dev)
{
	...
	struct udc_ep_config *ep_config = udc_get_ep_cfg(dev, USB_CONTROL_EP_OUT);
	struct net_buf *buf;
...
{
	struct usb_dc_state *dev_state = dev->data;
	struct smartbond_ep_state *ep_state = EP0_OUT_STATE(dev_state);
	struct udc_ep_config *ep_config = &ep_state->config;
	struct net_buf *buf;
...
{
	struct usb_dc_state *dev_state = CONTAINER_OF(item, struct usb_dc_state, ep0_rx_work);
	const uint8_t ep = USB_CONTROL_EP_OUT;
	struct net_buf *buf;
	const struct device *dev = dev_state->dev;
	unsigned int lock_key;
...
{
	struct usb_dc_state *dev_state = CONTAINER_OF(item, struct usb_dc_state, ep0_tx_work);
	struct net_buf *buf;
	const struct device *dev = dev_state->dev;
	const uint8_t ep = USB_CONTROL_EP_IN;
	unsigned int lock_key;
	...
	buf = udc_buf_peek(dev, ep);
	__ASSERT(buf == EP0_IN_STATE(dev_state)->buf, "TX work without buffer %p %p", buf,
		 EP0_IN_STATE(dev_state)->buf);
	...
	__ASSERT(buf == EP0_IN_STATE(dev_state)->buf, "Internal error");
...
{
	struct usb_dc_state *dev_state = CONTAINER_OF(item, struct usb_dc_state, ep0_setup_work);
	struct net_buf *buf;
	int err;
	const struct device *dev = dev_state->dev;
	...
	net_buf_add_mem(buf, dev_state->setup_buffer, 8);
	ep0_out_state = EP0_OUT_STATE(dev_state);
	ep0_out_state->last_packet_size = 0;
...
{
	struct usb_dc_state *dev_state = dev->data;
	const struct udc_smartbond_config *cfg = dev->config;

	LOG_DBG("Enable UDC");

	usb_change_state(dev_state, true, dev_state->vbus_present);
...
static int udc_smartbond_disable(const struct device *dev)
{
	...
	usb_change_state(dev_state, false, dev_state->vbus_present);

	irq_disable(cfg->udc_irq);
...
static int udc_smartbond_init(const struct device *dev)
{
	...
	k_work_init(&dev_state->ep0_setup_work, handle_ep0_setup_work);
	k_work_init(&dev_state->ep0_rx_work, handle_ep0_rx_work);
	k_work_init(&dev_state->ep0_tx_work, handle_ep0_tx_work);

	data->caps.rwup = true;
	data->caps.mps0 = UDC_MPS0_8;

	for (int i = 0; i < EP_MAX; i++) {
		dev_state->ep_state[0][i].config.caps.out = 1;
		if (i == 0) {
			dev_state->ep_state[0][i].config.caps.control = 1;
			dev_state->ep_state[0][i].config.caps.mps = 8;
		} else {
			dev_state->ep_state[0][i].config.caps.bulk = 1;
			dev_state->ep_state[0][i].config.caps.interrupt = 1;
			dev_state->ep_state[0][i].config.caps.iso = 1;
			dev_state->ep_state[0][i].config.caps.mps = mps;
		}
		dev_state->ep_state[0][i].config.addr = USB_EP_DIR_OUT | i;
		err = udc_register_ep(dev, &dev_state->ep_state[0][i].config);
		if (err != 0) {
			LOG_ERR("Failed to register endpoint");
			return err;
		}
		dev_state->ep_state[0][i].regs = reg_set + i;
	}

	for (int i = 0; i < EP_MAX; i++) {
		dev_state->ep_state[1][i].config.caps.in = 1;
		if (i == 0) {
			dev_state->ep_state[1][i].config.caps.control = 1;
			dev_state->ep_state[1][i].config.caps.mps = 8;
		} else {
			dev_state->ep_state[1][i].config.caps.bulk = 1;
			dev_state->ep_state[1][i].config.caps.interrupt = 1;
			dev_state->ep_state[1][i].config.caps.iso = 1;
			dev_state->ep_state[1][i].config.caps.mps = mps;
		}

		dev_state->ep_state[1][i].config.addr = USB_EP_DIR_IN | i;
		err = udc_register_ep(dev, &dev_state->ep_state[1][i].config);
		if (err != 0) {
			LOG_ERR("Failed to register endpoint");
			return err;
		}
		dev_state->ep_state[1][i].regs = reg_set + i;
	}
	...
	NVIC_SetPendingIRQ(cfg->vbus_irq);
	irq_enable(cfg->vbus_irq);
...
#define UDC_SMARTBOND_DEVICE_DEFINE(n)                                                             \
                                                                                                   \
	static const struct udc_smartbond_config udc_smartbond_config_##n = {                      \
		.udc_irq = UDC_IRQ(n),                                                             \
		.vbus_irq = VBUS_IRQ(n),                                                           \
		...
		.fifo_read_threshold = FIFO_READ_THRESHOLD(n),                                     \
                                                                                                   ...
                                                                                                   \
	static struct usb_dc_state udc_state_##n = {                                               \
		.udc_data =                                                                        \
			{                                                                          \
				.mutex = Z_MUTEX_INITIALIZER(udc_state_##n.udc_data.mutex),        \
			},                                                                         \
	};                                                                                         \
                                                                                                   \
	...
	{                                                                                          \
		...
		return 0;                                                                          \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, udc_smartbond_driver_preinit_##n, NULL, &udc_state_##n,           \
			      &udc_smartbond_config_##n, POST_KERNEL,                              \
