...
static uint8_t udc_get_bd_size(const uint16_t mps)
{
	...
	case 1023:
		...
		break;
	default:
		__ASSERT(err == 0, "Wrong maximum packet size value");
		bd_size = 0;
	}
...
static int udc_handle_evt_din(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	if (buf == NULL) {
		...
		if (ep_cfg->addr == USB_CONTROL_EP_IN) {
			status_in = udc_ctrl_stage_is_status_in(dev);
			no_data_stage = udc_ctrl_stage_is_no_data(dev);
			status_out = udc_ctrl_stage_is_status_out(dev);

 			if (status_in || no_data_stage) {
 ...
 */
static ALWAYS_INLINE void udc_submit_error_event(const struct device *const dev, int err)
{
    int submit_err = 0;
    do {
 ...
 */
static ALWAYS_INLINE void udc_handle_xfer_finished(const struct device *const dev)
{
    struct udc_mchp_data *const priv = udc_get_private(dev);
 ...
 * @param dev  Pointer to the USB device structure.
 */
static ALWAYS_INLINE void udc_handle_setup(const struct device *const dev)
{
    int err = 0;
 ...
 */
static ALWAYS_INLINE void udc_handle_ep_isr(const struct device *dev, const uint8_t idx)
{
