...
#ifndef ZEPHYR_DRIVERS_USB_UDC_DWC2_STM32F7_HSOTG_H
#define ZEPHYR_DRIVERS_USB_UDC_DWC2_STM32F7_HSOTG_H
...
/* STM32F7 OTG_HS device quirk configuration */
struct udc_dwc2_stm32f7_config {
...
static inline int udc_dwc2_stm32f7_enable_clk(const struct udc_dwc2_stm32f7_config *const cfg)
{
	int ret;
 ...
 * macros.
 */
#define UDC_DWC2_STM32_HSE_HZ	DT_PROP(DT_NODELABEL(clk_hse), clock_frequency)

#if UDC_DWC2_STM32_HSE_HZ == 12000000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x0UL << 1)
#elif UDC_DWC2_STM32_HSE_HZ == 12500000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x2UL << 1)
#elif UDC_DWC2_STM32_HSE_HZ == 16000000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x3UL << 1)
#elif UDC_DWC2_STM32_HSE_HZ == 24000000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x4UL << 1)
#elif UDC_DWC2_STM32_HSE_HZ == 25000000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x5UL << 1)
#elif UDC_DWC2_STM32_HSE_HZ == 32000000
#define UDC_DWC2_USBPHYC_PLLSEL		(0x7UL << 1)
#else
#error "Unsupported HSE frequency for the STM32F7 USBPHYC"
#endif

/* High-Speed PHY tuning value (from the STM32F7 HAL LL USB header) */
#define UDC_DWC2_USBPHYC_TUNE_VALUE	0x00000F13UL
 ...
 */
static inline int udc_dwc2_stm32f7_enable_phy(const struct device *const dev,
					      const struct udc_dwc2_stm32f7_config *const cfg)
{
	...
	USB_HS_PHYC->USB_HS_PHYC_PLL = UDC_DWC2_USBPHYC_PLLSEL;
	USB_HS_PHYC->USB_HS_PHYC_TUNE |= UDC_DWC2_USBPHYC_TUNE_VALUE;
...
static inline void udc_dwc2_stm32f7_disable_phy(const struct device *const dev,
						const struct udc_dwc2_stm32f7_config *const cfg)
{
	struct usb_dwc2_reg *const base = dwc2_get_base(dev);
...
#define UDC_DWC2_STM32F7_PHY_IS_ULPI(n)							\
	COND_CODE_1(DT_INST_NODE_HAS_PROP(n, phys),					\
		(DT_NODE_HAS_COMPAT(DT_INST_PHANDLE(n, phys), usb_ulpi_phy)),		\
		(0))

#define UDC_DWC2_STM32F7_PHY_RESET(n)							\
	COND_CODE_1(UDC_DWC2_STM32F7_PHY_IS_ULPI(n),					\
...
#define QUIRK_STM32F7_HSOTG_DEFINE(n)							\
	static const struct stm32_pclken pclken_##n[] = STM32_DT_INST_CLOCKS(n);	\
											\
	static const struct udc_dwc2_stm32f7_config udc_dwc2_stm32f7_config_##n = {	\
		.clk_dev = DEVICE_DT_GET(STM32_CLOCK_CONTROL_NODE),			\
		.pclken = pclken_##n,							\
		.pclken_len = DT_INST_NUM_CLOCKS(n),					\
		.phy_reset_gpio = UDC_DWC2_STM32F7_PHY_RESET(n),			\
	};										\
											\
	static int udc_dwc2_stm32f7_pre_enable_##n(const struct device *dev)		\
	{										\
		int ret = udc_dwc2_stm32f7_enable_clk(&udc_dwc2_stm32f7_config_##n);	\
											\
		if (ret) {								\
			return ret;							\
		}									\
											\
		return udc_dwc2_stm32f7_enable_phy(dev, &udc_dwc2_stm32f7_config_##n);	\
	}										\
											\
	static int udc_dwc2_stm32f7_shutdown_##n(const struct device *dev)		\
	{										\
		udc_dwc2_stm32f7_disable_phy(dev, &udc_dwc2_stm32f7_config_##n);	\
											\
		return 0;								\
	}										\
											\
	const struct dwc2_vendor_quirks dwc2_vendor_quirks_##n = {			\
		.pre_enable = udc_dwc2_stm32f7_pre_enable_##n,				\
		.shutdown = udc_dwc2_stm32f7_shutdown_##n,				\
	};
...
#endif /* ZEPHYR_DRIVERS_USB_UDC_DWC2_STM32F7_HSOTG_H */
