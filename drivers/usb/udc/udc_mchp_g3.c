...
LOG_MODULE_REGISTER(udc_mchp_g3, CONFIG_UDC_DRIVER_LOG_LEVEL);

#define DT_DRV_COMPAT microchip_usb_g3
...
/*
 * Buffer descriptor for OUT endpoints (bank 0) in the Microchip G3 USB
 * device controller.
 *
...
/*
 * Buffer descriptor for IN endpoints (bank 1) in the Microchip G3 USB
 * device controller.
 *
 * Mirrors the hardware register layout:
 *   - 0x10: Buffer address
 ...
 * in the Microchip G3 USB device controller.
 *
...
/*
 * Performs initial setup of the Microchip USB device g3 controller driver.
 *
...
#define UDC_MCHP_THREAD_DEFINE(n)                                                                  \
	K_THREAD_STACK_DEFINE(udc_mchp_stack_##n, CONFIG_UDC_MCHP_G3_STACK_SIZE);                  \
                                                                                                   \
	...
	{                                                                                          \
		...
		k_thread_create(&priv->thread_data, udc_mchp_stack_##n,                            \
				...
				(void *)dev, NULL, NULL,                                           \
				K_PRIO_COOP(CONFIG_UDC_MCHP_G3_THREAD_PRIORITY), K_ESSENTIAL,      \
				K_NO_WAIT);                                                        \
