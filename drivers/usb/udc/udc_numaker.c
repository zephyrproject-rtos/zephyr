...
#define NUMAKER_XUSBD_TYPE              USBD_T
#define NUMAKER_XUSBD_EP_TYPE           USBD_EP_T
#define NUMAKER_XUSBD_SPEED_IDX_DEFAULT 1
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
#define DT_DRV_COMPAT                   nuvoton_numaker_hsusbd
#define NUMAKER_XUSBD_TYPE              HSUSBD_T
#define NUMAKER_XUSBD_EP_TYPE           HSUSBD_EP_T
#define NUMAKER_XUSBD_SPEED_IDX_DEFAULT 2
#endif

/* For bus reset, keep 'SE0' (USB spec: SE0 >= 2.5 ms) */
#define NUMAKER_XUSBD_BUS_RESET_DRV_SE0_US 3000

/* For bus resume, generate 'K' (USB spec: 'K' >= 1 ms) */
#define NUMAKER_XUSBD_BUS_RESUME_DRV_K_US 1500

/* Reserve DMA buffer for Setup/CTRL OUT/CTRL IN, required to be 8-byte aligned */
#define NUMAKER_XUSBD_DMABUF_SIZE_SETUP   8
#define NUMAKER_XUSBD_DMABUF_SIZE_CTRLOUT 64
#define NUMAKER_XUSBD_DMABUF_SIZE_CTRLIN  64

#if DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_usbd)

/* For USBD, enable USB PHY needn't wait. */

/* For USBD, not support DMA */
...
enum numaker_xusbd_msg_type {
	/* Device plug-in */
	NUMAKER_XUSBD_MSG_TYPE_ATTACH,
	/* Bus reset */
	NUMAKER_XUSBD_MSG_TYPE_RESET,
	/* Bus resume */
	NUMAKER_XUSBD_MSG_TYPE_RESUME,
	/* Setup packet received */
	NUMAKER_XUSBD_MSG_TYPE_SETUP,
	/* OUT transaction for specific EP completed */
	NUMAKER_XUSBD_MSG_TYPE_OUT,
	/* IN transaction for specific EP completed */
	NUMAKER_XUSBD_MSG_TYPE_IN,
	/* Re-activate queued transfer for specific EP */
	NUMAKER_XUSBD_MSG_TYPE_XFER,
	/* S/W reconnect */
	NUMAKER_XUSBD_MSG_TYPE_SW_RECONN,
};

struct numaker_xusbd_msg {
	enum numaker_xusbd_msg_type type;
	union {
		struct {
...
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
	uint32_t ep_hw_cfg; /* BSP HSUSBD driver EP configuration */
...
struct udc_numaker_data {
	...
	struct numaker_xusbd_ep_mgmt ep_mgmt; /* EP management */

	struct numaker_xusbd_ep *ep_pool;
...
static inline void numaker_xusbd_sw_connect(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static inline void numaker_xusbd_sw_disconnect(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static inline void numaker_xusbd_sw_reconnect(const struct device *dev)
{
	/* Keep SE0 to trigger bus reset */
	numaker_xusbd_sw_disconnect(dev);
	k_sleep(K_USEC(NUMAKER_XUSBD_BUS_RESET_DRV_SE0_US));
	numaker_xusbd_sw_connect(dev);
}

static inline void numaker_xusbd_reset_addr(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
...
static inline void numaker_xusbd_set_addr(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	struct udc_numaker_data *priv = udc_get_private(dev);
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static inline NUMAKER_XUSBD_EP_TYPE *numaker_xusbd_ep_base(const struct device *dev,
							   uint32_t ep_hw_idx)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static inline void numaker_xusbd_ep_sync_udc_halt(struct numaker_xusbd_ep *ep_cur, bool stalled)
{
...
static inline void numaker_xusbd_ep_set_stall(struct numaker_xusbd_ep *ep_cur)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	NUMAKER_XUSBD_EP_TYPE *ep_base = numaker_xusbd_ep_base(dev, ep_cur->ep_hw_idx);
	...
	numaker_xusbd_ep_sync_udc_halt(ep_cur, true);
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
...
static inline void numaker_xusbd_ep_clear_stall_n_data_toggle(struct numaker_xusbd_ep *ep_cur)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	...
	ep_base->CFGP &= ~USBD_CFGP_SSTALL_Msk;
	numaker_xusbd_ep_sync_udc_halt(ep_cur, false);
...
static int numaker_xusbd_send_msg(const struct device *dev, const struct numaker_xusbd_msg *msg)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	int err;
	...
	if (err < 0) {
		...
		struct numaker_xusbd_msg msg_reconn = {
			.type = NUMAKER_XUSBD_MSG_TYPE_SW_RECONN,
		};
...
static int numaker_xusbd_enable_usb_phy(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static int numaker_xusbd_hw_setup(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	int err;
	...
	base->PHYCTL = 0;
#endif
	err = numaker_xusbd_enable_usb_phy(dev);
	if (err < 0) {
...
static void numaker_xusbd_hw_shutdown(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	struct numaker_scc_subsys scc_subsys;
...
static void numaker_xusbd_vbus_plug_th(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	struct numaker_xusbd_msg msg = {0};
	...
	msg.type = NUMAKER_XUSBD_MSG_TYPE_ATTACH;
	numaker_xusbd_send_msg(dev, &msg);

	LOG_DBG("USB plug-in");
}

/* Interrupt top half processing for vbus unplug */
static void numaker_xusbd_vbus_unplug_th(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static void numaker_xusbd_bus_reset_th(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	struct udc_numaker_data *priv = udc_get_private(dev);
	struct numaker_xusbd_ep *ep_cur = priv->ep_pool;
	struct numaker_xusbd_ep *ep_end = priv->ep_pool + priv->ep_pool_size;
	NUMAKER_XUSBD_EP_TYPE *ep_base;
	struct numaker_xusbd_msg msg = {0};
	...
	for (; ep_cur != ep_end; ep_cur++) {
		ep_base = numaker_xusbd_ep_base(dev, ep_cur->ep_hw_idx);
	...
	numaker_xusbd_reset_addr(dev);

	/* Message for bottom-half processing */
	msg.type = NUMAKER_XUSBD_MSG_TYPE_RESET;
	numaker_xusbd_send_msg(dev, &msg);
...
static void numaker_xusbd_bus_suspend_th(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static void numaker_xusbd_bus_resume_th(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	struct numaker_xusbd_msg msg = {0};
	...
	/* Message for bottom-half processing */
	msg.type = NUMAKER_XUSBD_MSG_TYPE_RESUME;
	numaker_xusbd_send_msg(dev, &msg);
...
static void numaker_xusbd_setup_copy_to_user(const struct device *dev, uint8_t *usrbuf);

#if DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_usbd)
/* Interrupt top half processing for Setup packet */
static void numaker_usbd_setup_th(const struct device *dev)
{
	NUMAKER_XUSBD_EP_TYPE *ep0_base = numaker_xusbd_ep_base(dev, EP0);
	NUMAKER_XUSBD_EP_TYPE *ep1_base = numaker_xusbd_ep_base(dev, EP1);
	struct numaker_xusbd_msg msg = {0};
	 ...
	 * CTRL OUT EP
	 */
	msg.type = NUMAKER_XUSBD_MSG_TYPE_SETUP;
	numaker_xusbd_setup_copy_to_user(dev, msg.setup.packet);
	numaker_xusbd_send_msg(dev, &msg);
}
...
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	NUMAKER_XUSBD_EP_TYPE *ep_base = numaker_xusbd_ep_base(dev, ep_hw_idx);
	uint8_t ep_dir;
	uint8_t ep_idx;
	uint8_t ep;
	struct numaker_xusbd_msg msg = {0};
	...
	if (ep == USB_EP_GET_ADDR(0, USB_EP_DIR_OUT)) {
		struct numaker_xusbd_ep *ep_ctrlout = priv->ep_pool + 0;
	...
	if (USB_EP_DIR_IS_OUT(ep)) {
		msg.type = NUMAKER_XUSBD_MSG_TYPE_OUT;
		msg.out.ep = ep;
	} else {
		msg.type = NUMAKER_XUSBD_MSG_TYPE_IN;
		msg.in.ep = ep;
	}
	numaker_xusbd_send_msg(dev, &msg);
}
 ...
 */
static int numaker_xusbd_ep_xfer_user_dma(struct numaker_xusbd_ep *ep_cur, uint8_t *usrbuf,
					  uint32_t *size_p)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
	...
	if (USB_EP_DIR_IS_IN(ep_cur->addr)) {
		sys_cache_data_flush_range(usrbuf, *size_p);
	} else {
		sys_cache_data_invd_range(usrbuf, *size_p);
	}
#endif
 ...
 * size_p holds size to copy/copied on input/output
 */
static int numaker_xusbd_ep_copy_to_user(struct numaker_xusbd_ep *ep_cur, uint8_t *usrbuf,
...
#if defined(CONFIG_UDC_NUMAKER_DMA)
		int err = numaker_xusbd_ep_xfer_user_dma(ep_cur, usrbuf, size_p);
 ...
 * size_p holds size to copy/copied on input/output
 */
static int numaker_xusbd_ep_copy_from_user(struct numaker_xusbd_ep *ep_cur, const uint8_t *usrbuf,
					   uint32_t *size_p)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
...
#if defined(CONFIG_UDC_NUMAKER_DMA)
		int err = numaker_xusbd_ep_xfer_user_dma(ep_cur, (uint8_t *)usrbuf, size_p);
...
static void numaker_xusbd_ep_config_dmabuf(struct numaker_xusbd_ep *ep_cur, uint32_t dmabuf_base,
					   uint32_t dmabuf_size)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
...
static void numaker_xusbd_ep_abort(struct numaker_xusbd_ep *ep_cur, bool excl_ctrl)
{
	struct udc_ep_config *ep_cfg;
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
...
static void numaker_xusbd_ep_config_major(struct numaker_xusbd_ep *ep_cur,
...
static void numaker_xusbd_ep_enable(struct numaker_xusbd_ep *ep_cur)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
	...
	numaker_xusbd_ep_abort(ep_cur, false);
	numaker_xusbd_ep_clear_stall_n_data_toggle(ep_cur);
...
static void numaker_xusbd_ep_disable(struct numaker_xusbd_ep *ep_cur)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
	NUMAKER_XUSBD_TYPE *base = config->base;
...
static void udc_numaker_ep_trigger(struct numaker_xusbd_ep *ep_cur, uint32_t len)
{
	struct udc_ep_config *ep_cfg;
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
...
static struct numaker_xusbd_ep *numaker_xusbd_ep_mgmt_alloc_ep(const struct device *dev)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	struct numaker_xusbd_ep_mgmt *ep_mgmt = &priv->ep_mgmt;
	struct numaker_xusbd_ep *ep_cur = NULL;
 ...
 */
static int numaker_xusbd_ep_mgmt_alloc_dmabuf(const struct device *dev, uint32_t size,
...
static void numaker_xusbd_ep_mgmt_init(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	struct udc_numaker_data *priv = udc_get_private(dev);
	NUMAKER_XUSBD_TYPE *base = config->base;
	...
	struct numaker_xusbd_ep *ep_cur;
	struct numaker_xusbd_ep *ep_end;
	...
	ep_mgmt->dmabuf_pos += NUMAKER_XUSBD_DMABUF_SIZE_SETUP;
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
	...
	numaker_xusbd_ep_config_dmabuf(ep_cur, ep_mgmt->dmabuf_pos,
				       NUMAKER_XUSBD_DMABUF_SIZE_CTRLOUT);
	ep_mgmt->dmabuf_pos += NUMAKER_XUSBD_DMABUF_SIZE_CTRLOUT;
	ep_cur->mps_valid = true;
	ep_cur->mps = NUMAKER_XUSBD_DMABUF_SIZE_CTRLOUT;
	...
	numaker_xusbd_ep_config_dmabuf(ep_cur, ep_mgmt->dmabuf_pos,
				       NUMAKER_XUSBD_DMABUF_SIZE_CTRLIN);
	ep_mgmt->dmabuf_pos += NUMAKER_XUSBD_DMABUF_SIZE_CTRLIN;
	ep_cur->mps_valid = true;
	ep_cur->mps = NUMAKER_XUSBD_DMABUF_SIZE_CTRLIN;
}

/* Find EP H/W context by EP address */
static struct numaker_xusbd_ep *numaker_xusbd_ep_mgmt_find_ep(const struct device *dev,
							      const uint8_t ep)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	struct numaker_xusbd_ep *ep_cur = priv->ep_pool;
...
static struct numaker_xusbd_ep *numaker_xusbd_ep_mgmt_bind_ep(const struct device *dev,
							      const uint8_t ep)
{
	struct numaker_xusbd_ep *ep_cur = numaker_xusbd_ep_mgmt_find_ep(dev, ep);

	if (!ep_cur) {
		ep_cur = numaker_xusbd_ep_mgmt_alloc_ep(dev);
...
static int numaker_xusbd_xfer_out(const struct device *dev, uint8_t ep, bool strict)
{
	struct net_buf *buf;
	struct numaker_xusbd_ep *ep_cur;
	struct udc_ep_config *ep_cfg;
	...
	/* Bind EP H/W context to EP address */
	ep_cur = numaker_xusbd_ep_mgmt_bind_ep(dev, ep);
	if (!ep_cur) {
...
static int numaker_xusbd_xfer_in(const struct device *dev, uint8_t ep, bool strict)
{
	int err;
	struct net_buf *buf;
	struct numaker_xusbd_ep *ep_cur;
	...
	data_len = buf->len;
	if (data_len) {
		err = numaker_xusbd_ep_copy_from_user(ep_cur, buf->data, &data_len);
		if (err < 0) {
...
static int numaker_xusbd_ctrl_feed_dout(const struct device *dev, const size_t length)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	struct udc_ep_config *ep_cfg;
	struct net_buf *buf;
	...
	return numaker_xusbd_xfer_out(dev, ep_cfg->addr, true);
}

/* Message handler for device plug-in */
static int numaker_xusbd_msg_handle_attach(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	int err;

	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_ATTACH);
...
static int numaker_xusbd_msg_handle_reset(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	int err;

	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_RESET);
...
static int numaker_xusbd_msg_handle_resume(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	int err;

	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_RESUME);
...
static int numaker_xusbd_msg_handle_setup(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	int err;
	uint8_t ep;
	struct numaker_xusbd_ep *ep_cur;
	...
	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_SETUP);
	...
	numaker_xusbd_setup_copy_to_user(dev, data_ptr);
	net_buf_add(buf, 8);
	...
	if (udc_ctrl_stage_is_data_out(dev)) {
		...
		err = numaker_xusbd_ctrl_feed_dout(dev, udc_data_stage_length(buf));
		if (err == -ENOMEM) {
...
static int numaker_xusbd_msg_handle_out(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	int err;
	uint8_t ep;
	...
	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_OUT);
	...
	data_ptr = net_buf_tail(buf);
	err = numaker_xusbd_ep_copy_to_user(ep_cur, data_ptr, &data_len, &data_rmn);
	if (err < 0) {
...
next_xfer:
	...
	numaker_xusbd_xfer_out(dev, ep, false);

	return 0;
}

/* Message handler for DATA IN transaction completed */
static int numaker_xusbd_msg_handle_in(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	int err;
	uint8_t ep;
	struct numaker_xusbd_ep *ep_cur;
	...
	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_IN);
	...
	if (ep == USB_CONTROL_EP_IN) {
		...
		if (udc_ctrl_stage_is_status_out(dev)) {
			...
			err = numaker_xusbd_ctrl_feed_dout(dev, 0);
			if (err < 0) {
...
xfer_next:
	...
	numaker_xusbd_xfer_in(dev, ep, false);

	return 0;
}

/* Message handler for queued transfer re-activated */
static int numaker_xusbd_msg_handle_xfer(const struct device *dev, struct numaker_xusbd_msg *msg)
{
	uint8_t ep;

	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_XFER);
...
static int numaker_xusbd_msg_handle_sw_reconn(const struct device *dev,
					      struct numaker_xusbd_msg *msg)
{
	__ASSERT_NO_MSG(msg->type == NUMAKER_XUSBD_MSG_TYPE_SW_RECONN);

	/* S/W reconnect for error recovery */
	numaker_xusbd_sw_reconnect(dev);

	return 0;
}

static void numaker_xusbd_msg_handler(const struct device *dev)
{
	struct udc_numaker_data *priv = udc_get_private(dev);
	int err;
	struct numaker_xusbd_msg msg;
	...
	while (true) {
		...
		switch (msg.type) {
		case NUMAKER_XUSBD_MSG_TYPE_ATTACH:
			err = numaker_xusbd_msg_handle_attach(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_RESUME:
			err = numaker_xusbd_msg_handle_resume(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_RESET:
			err = numaker_xusbd_msg_handle_reset(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_SETUP:
			err = numaker_xusbd_msg_handle_setup(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_OUT:
			err = numaker_xusbd_msg_handle_out(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_IN:
			err = numaker_xusbd_msg_handle_in(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_XFER:
			err = numaker_xusbd_msg_handle_xfer(dev, &msg);
			break;

		case NUMAKER_XUSBD_MSG_TYPE_SW_RECONN:
			err = numaker_xusbd_msg_handle_sw_reconn(dev, &msg);
			break;
...
static void numaker_xusbd_isr(const struct device *dev)
{
	const struct udc_numaker_config *config = dev->config;
	struct udc_numaker_data *priv = udc_get_private(dev);
	NUMAKER_XUSBD_TYPE *base = config->base;
	...
	if (usbd_intsts & USBD_INTSTS_FLDET) {
		if (base->VBUSDET & USBD_VBUSDET_VBUSDET_Msk) {
			/* USB plug-in */
			numaker_xusbd_vbus_plug_th(dev);
		} else {
			/* USB unplug */
			numaker_xusbd_vbus_unplug_th(dev);
		}
	}
	...
	if (usbd_intsts & USBD_INTSTS_BUS) {
		...
		if (usbd_bus_state & USBD_STATE_USBRST) {
			numaker_xusbd_bus_reset_th(dev);
		}

		/* Bus suspend */
		if (usbd_bus_state & USBD_STATE_SUSPEND) {
			numaker_xusbd_bus_suspend_th(dev);
		}

		/* Bus resume */
		if (usbd_bus_state & USBD_STATE_RESUME) {
			numaker_xusbd_bus_resume_th(dev);
		}
	}
	...
	if (gintsts_ep) {
		...
		while (gintsts_ep_iter) {
			...
			HSUSBD_EP_T *ep_base = numaker_xusbd_ep_base(dev, ep_hw_idx);
				  ...
				  struct net_buf *buf)
{
	...
	if (!ep_cfg->stat.halted) {
		msg.type = NUMAKER_XUSBD_MSG_TYPE_XFER;
...
static int udc_numaker_ep_dequeue(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	/* Bind EP H/W context to EP address */
	ep_cur = numaker_xusbd_ep_mgmt_bind_ep(dev, ep_cfg->addr);
	if (!ep_cur) {
...
static int udc_numaker_ep_set_halt(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	/* Set EP to stalled */
	numaker_xusbd_ep_set_stall(ep_cur);
...
static int udc_numaker_ep_enable(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	if (!ep_cur->dmabuf_valid || ep_cur->dmabuf_size < ep_cfg->mps) {
		/* Allocate DMA buffer */
		err = numaker_xusbd_ep_mgmt_alloc_dmabuf(dev, ep_cfg->mps, &dmabuf_base,
							 &dmabuf_size);
		if (err < 0) {
		...
		/* Configure EP DMA buffer */
		numaker_xusbd_ep_config_dmabuf(ep_cur, dmabuf_base, dmabuf_size);
	}

	/* Configure EP majorly */
	numaker_xusbd_ep_config_major(ep_cur, ep_cfg);

	/* Enable EP */
	numaker_xusbd_ep_enable(ep_cur);
...
static int udc_numaker_ep_disable(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	/* Disable EP */
	numaker_xusbd_ep_disable(ep_cur);
...
static int udc_numaker_host_wakeup(const struct device *dev)
{
	...
	k_sleep(K_USEC(NUMAKER_XUSBD_BUS_RESUME_DRV_K_US));
	base->ATTR ^= USBD_ATTR_RWAKEUP_Msk;
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
...
static int udc_numaker_init(const struct device *dev)
{
	...
	/* Initialize UDC H/W */
	err = numaker_xusbd_hw_setup(dev);
	if (err < 0) {
	...
	/* Initialize all EP H/W contexts */
	numaker_xusbd_ep_mgmt_init(dev);
...
#define UDC_NUMAKER_DEVICE_DEFINE(inst)                                                            \
	...
	static const struct udc_numaker_config udc_numaker_config_##inst = {                       \
		...
		.base = (NUMAKER_XUSBD_TYPE *)DT_INST_REG_ADDR(inst),                              \
		...
		.pincfg = NUMAKER_XUSBD_PINCTRL_DEV_CONFIG_GET(inst),                              \
		...
		.speed_idx = DT_ENUM_IDX_OR(DT_DRV_INST(inst), maximum_speed,                      \
					    NUMAKER_XUSBD_SPEED_IDX_DEFAULT),                      \
	};                                                                                         \
                                                                                                   \
	static struct numaker_xusbd_ep                                                             \
		numaker_xusbd_ep_pool_##inst[DT_INST_PROP(inst, num_bidir_endpoints)];             \
                                                                                                   \
	K_MSGQ_DEFINE(numaker_xusbd_msgq_##inst, sizeof(struct numaker_xusbd_msg),                 \
		      CONFIG_UDC_NUMAKER_MSG_QUEUE_SIZE, 4);                                       \
                                                                                                   \
	static struct udc_numaker_data udc_priv_##inst = {                                         \
		.msgq = &numaker_xusbd_msgq_##inst,                                                \
		.ep_pool = numaker_xusbd_ep_pool_##inst,                                           \
