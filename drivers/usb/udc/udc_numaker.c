...
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
static inline HSUSBD_EP_T *numaker_usbd_ep_base(const struct device *dev, uint32_t ep_hw_idx)
#endif
{
	const struct udc_numaker_config *config = dev->config;
	USBD_T *base = config->base;
...
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
	HSUSBD_EP_T *ep0_base = numaker_usbd_ep_base(dev, EP0);
	HSUSBD_EP_T *ep1_base = numaker_usbd_ep_base(dev, EP1);
#endif
	struct numaker_usbd_msg msg = {0};
 ...
 */
static int numaker_usbd_ep_xfer_user_dma(struct numaker_usbd_ep *ep_cur, uint8_t *usrbuf,
					 uint32_t *size_p)
{
	const struct device *dev = ep_cur->dev;
	const struct udc_numaker_config *config = dev->config;
...
#if defined(CONFIG_UDC_NUMAKER_DMA)
		int err = numaker_usbd_ep_xfer_user_dma(ep_cur, usrbuf, size_p);
...
#if defined(CONFIG_UDC_NUMAKER_DMA)
		int err = numaker_usbd_ep_xfer_user_dma(ep_cur, (uint8_t *)usrbuf, size_p);
...
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
	err = numaker_hsusbd_ep_copy_to_user(ep_cur, usrbuf, size_p);
	if (err < 0) {
		return err;
	}
#endif
...
#elif DT_HAS_COMPAT_STATUS_OKAY(nuvoton_numaker_hsusbd)
	err = numaker_hsusbd_ep_copy_from_user(ep_cur, usrbuf, size_p);
	if (err < 0) {
		return err;
	}
#endif
