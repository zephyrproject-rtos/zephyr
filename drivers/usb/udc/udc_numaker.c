...
enum numaker_usbd_msg_type {
	/* UDC bus event */
	NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT,
...
static inline struct udc_ep_config *numaker_usbd_ep_get_udc_config(struct numaker_usbd_ep *ep_cur)
{
...
{
	struct udc_ep_config *ep_cfg = numaker_usbd_ep_get_udc_config(ep_cur);
...
/* Message handler for UDC bus event */
static int numaker_usbd_msg_handle_udc_bus_event(const struct device *dev,
						 struct numaker_usbd_msg *msg)
{
	__ASSERT_NO_MSG(msg->type == NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT);

	/* Interrupt bottom half processing for bus reset */
	if (msg->udc_bus_event.type == UDC_EVT_RESET) {
		numaker_usbd_bus_reset_bh(dev);
	}

	return udc_submit_event(dev, msg->udc_bus_event.type, 0);
}
...
static void numaker_usbd_msg_handler(const struct device *dev)
{
	...
	while (true) {
		...
		switch (msg.type) {
		case NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT:
			err = numaker_usbd_msg_handle_udc_bus_event(dev, &msg);
			break;
...
static void numaker_udbd_isr(const struct device *dev)
{
	...
	if (usbd_intsts & USBD_INTSTS_FLDET) {
		...
		if (base->VBUSDET & USBD_VBUSDET_VBUSDET_Msk) {
			...
			msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
			msg.udc_bus_event.type = UDC_EVT_VBUS_READY;
			...
			/* Message for bottom-half processing */
			msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
			msg.udc_bus_event.type = UDC_EVT_VBUS_REMOVED;
			numaker_usbd_send_msg(dev, &msg);
	...
	if (usbd_intsts & USBD_INTSTS_BUS) {
		...
		if (usbd_bus_state & USBD_STATE_USBRST) {
			...
			/* Message for bottom-half processing */
			msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
			msg.udc_bus_event.type = UDC_EVT_RESET;
			numaker_usbd_send_msg(dev, &msg);
		...
		if (usbd_bus_state & USBD_STATE_SUSPEND) {
			...
			/* Message for bottom-half processing */
			msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
			msg.udc_bus_event.type = UDC_EVT_SUSPEND;
			numaker_usbd_send_msg(dev, &msg);
		...
		if (usbd_bus_state & USBD_STATE_RESUME) {
			...
			/* Message for bottom-half processing */
			msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
			msg.udc_bus_event.type = UDC_EVT_RESUME;
			numaker_usbd_send_msg(dev, &msg);
	...
	if (usbd_intsts & USBD_INTSTS_SOFIF_Msk) {
		...
		/* Message for bottom-half processing */
		msg.type = NUMAKER_USBD_MSG_TYPE_UDC_BUS_EVENT;
		msg.udc_bus_event.type = UDC_EVT_SOF;
		numaker_usbd_send_msg(dev, &msg);
	}
