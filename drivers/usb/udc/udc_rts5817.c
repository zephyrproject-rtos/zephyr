...
static const uint32_t ep_int_field_mask[] = {
...
#ifdef CONFIG_UDC_RTS5817_USE_RC_CLOCK
static void rts_switch_to_rc(const struct device *dev)
{
...
static void rts_usb_trg_rsume(const struct device *dev)
{
	const struct udc_rts_config *cfg = dev->config;
	mem_addr_t usb_regs = cfg->u2sie_sys_base;
...
static int rts_init_ep_cfg_in(const struct device *dev)
{
	...
	for (int i = 0, n = 0; i < MAX_NUM_EP; i++) {
		if (i == 0) {
			...
			continue;
		} else if ((ep_lnum_table[i] == EPC) || (ep_lnum_table[i] == EPD)) {
...
static int udc_rts_driver_preinit(const struct device *dev)
{
	...
	LOG_DBG("Device %p (max. speed %d)", dev, config->speed_idx);
