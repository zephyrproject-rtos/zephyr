...
void nrf_usbd_legacy_transfer_out_drop(nrf_usbd_common_ep_t ep);
void nrf_usbd_legacy_setup_data_clear(void);
size_t nrf_usbd_legacy_epout_size_get(nrf_usbd_common_ep_t ep);
bool nrf_usbd_legacy_suspend_check(void);
...
void nrf_usbd_legacy_ep_abort(nrf_usbd_common_ep_t ep)
{
...
void nrf_usbd_legacy_enable(void)
{
...
void nrf_usbd_legacy_disable(void)
{
...
void nrf_usbd_legacy_start(bool enable_sof)
{
	m_bus_suspend = false;
...
bool nrf_usbd_legacy_suspend(void)
{
...
bool nrf_usbd_legacy_wakeup_req(void)
{
	bool started = false;
	unsigned int irq_lock_key = irq_lock();
...
bool nrf_usbd_legacy_ep_enable_check(nrf_usbd_common_ep_t ep)
{
	int ep_in = NRF_USBD_COMMON_EP_IS_IN(ep);
	int ep_num = NRF_USBD_COMMON_EP_NUM(ep);
...
void nrf_usbd_legacy_ep_enable(nrf_usbd_common_ep_t ep)
{
	int ep_in = NRF_USBD_COMMON_EP_IS_IN(ep);
	int ep_num = NRF_USBD_COMMON_EP_NUM(ep);
...
void nrf_usbd_legacy_ep_disable(nrf_usbd_common_ep_t ep)
{
	int ep_in = NRF_USBD_COMMON_EP_IS_IN(ep);
	int ep_num = NRF_USBD_COMMON_EP_NUM(ep);
...
nrfx_err_t nrf_usbd_legacy_ep_transfer(nrf_usbd_common_ep_t ep)
{
	nrfx_err_t ret;
...
void nrf_usbd_legacy_ep_stall(nrf_usbd_common_ep_t ep)
{
...
bool nrf_usbd_legacy_ep_stall_check(nrf_usbd_common_ep_t ep)
{
	int ep_in = NRF_USBD_COMMON_EP_IS_IN(ep);
	int ep_num = NRF_USBD_COMMON_EP_NUM(ep);
...
void nrf_usbd_legacy_ep_stall_clear(nrf_usbd_common_ep_t ep)
{
	__ASSERT_NO_MSG(!NRF_USBD_COMMON_EP_IS_ISO(ep));
...
void nrf_usbd_legacy_ep_dtoggle_clear(nrf_usbd_common_ep_t ep)
{
	__ASSERT_NO_MSG(!NRF_USBD_COMMON_EP_IS_ISO(ep));
...
void nrf_usbd_legacy_setup_get(nrf_usbd_common_setup_t *p_setup)
{
...
void nrf_usbd_legacy_setup_clear(void)
{
	LOG_DBG(">> ep0status >>");
	NRF_USBD->TASKS_EP0STATUS = 1;
}

void nrf_usbd_legacy_setup_stall(void)
{
	LOG_DBG("Setup stalled.");
	NRF_USBD->TASKS_EP0STALL = 1;
}

nrf_usbd_common_ep_t nrf_usbd_legacy_last_setup_dir_get(void)
{
...
static void udc_nrf_thread_handler(const struct device *dev)
{
	...
	if (evt & BIT(NRF_USBD_COMMON_EVT_WUREQ)) {
	...
	if (evt & BIT(UDC_NRF_EVT_EP_FINISHED)) {
		uint32_t eps = atomic_clear(&xfer_finished) & UINT32_MAX;
	...
	if (evt & BIT(UDC_NRF_EVT_XFER)) {
		uint32_t eps = atomic_clear(&xfer_new) & UINT32_MAX;
