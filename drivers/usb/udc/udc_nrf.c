...
K_MSGQ_DEFINE(drv_msgq, sizeof(struct udc_nrf_evt), CONFIG_UDC_NRF_MAX_QMESSAGES, sizeof(uint32_t));
...
{
	if (nrf_usbd_common_last_setup_dir_get() == USB_CONTROL_EP_OUT && udc_nrf_setup_rcvd) {
...
static void udc_event_xfer_in_next(const struct device *dev, const uint8_t ep)
{
	...
	if (buf != NULL) {
		nrf_usbd_common_transfer_t xfer = {
			.p_data = {.tx = buf->data},
			.size = buf->len,
			.flags = udc_ep_buf_has_zlp(buf) ? NRF_USBD_COMMON_TRANSFER_ZLP_FLAG : 0,
		};
		nrfx_err_t err;
...
static void udc_event_xfer_in(const struct device *dev, nrf_usbd_common_evt_t const *const event)
{
...
static void udc_event_xfer_out(const struct device *dev, nrf_usbd_common_evt_t const *const event)
{
	uint8_t ep = event->data.eptransfer.ep;
...
static int udc_nrf_ep_enqueue(const struct device *dev, struct udc_ep_config *cfg,
			      struct net_buf *buf)
{
	struct udc_nrf_evt evt = {
		.type = UDC_NRF_EVT_XFER,
...
static int udc_nrf_ep_dequeue(const struct device *dev, struct udc_ep_config *cfg)
{
...
static int udc_nrf_ep_enable(const struct device *dev, struct udc_ep_config *cfg)
{
	uint16_t mps;
...
static int udc_nrf_ep_disable(const struct device *dev, struct udc_ep_config *cfg)
{
	__ASSERT_NO_MSG(cfg);
...
static int udc_nrf_ep_set_halt(const struct device *dev, struct udc_ep_config *cfg)
{
...
static int udc_nrf_ep_clear_halt(const struct device *dev, struct udc_ep_config *cfg)
{
...
static int udc_nrf_enable(const struct device *dev)
{
	...
	if (udc_ep_enable_internal(dev, USB_CONTROL_EP_OUT, USB_EP_TYPE_CONTROL, UDC_NRF_EP0_SIZE,
				   0)) {
		LOG_ERR("Failed to enable control endpoint");
		return -EIO;
	}

	if (udc_ep_enable_internal(dev, USB_CONTROL_EP_IN, USB_EP_TYPE_CONTROL, UDC_NRF_EP0_SIZE,
				   0)) {
		LOG_ERR("Failed to enable control endpoint");
		return -EIO;
	}
...
static int udc_nrf_init(const struct device *dev)
{
	 ...
	 */
	IRQ_CONNECT(USBREGULATOR_IRQn, DT_IRQ(DT_INST(0, nordic_nrf_clock), priority), nrfx_isr,
		    nrfx_usbreg_irq_handler, 0);
	irq_enable(USBREGULATOR_IRQn);
#endif

	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), nrfx_isr,
		    nrf_usbd_common_irq_handler, 0);
...
static int udc_nrf_driver_init(const struct device *dev)
{
	...
	k_thread_create(&drv_stack_data, drv_stack, K_KERNEL_STACK_SIZEOF(drv_stack),
			udc_nrf_thread, (void *)dev, NULL, NULL, K_PRIO_COOP(8), 0, K_NO_WAIT);
...
static const struct udc_nrf_config udc_nrf_cfg = {
	.clock = COND_CODE_1(NRF_CLOCK_HAS_HFCLK192M,
		 ...
		 .pwr = {
			 .dcdcen = (DT_PROP(DT_INST(0, nordic_nrf5x_regulator),
			...
			(DT_NODE_HAS_STATUS_OKAY(DT_INST(0, nordic_nrf53x_regulator_hv)))),
				 ...
				 .evt = {.handler = udc_nrf_power_handler},
};
...
DEVICE_DT_INST_DEFINE(0, udc_nrf_driver_init, NULL, &udc_nrf_data, &udc_nrf_cfg, POST_KERNEL,
		      CONFIG_KERNEL_INIT_PRIORITY_DEVICE, &udc_nrf_api);
