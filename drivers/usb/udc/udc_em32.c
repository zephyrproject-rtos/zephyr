...
struct udc_em32_data {
	/* clock device reference from DTS "clocks" property */
...
#define UDC_EM32_DEVICE_DEFINE(inst)                                                               \
	...
	static struct udc_em32_data em32_udc_priv_##inst = {                                       \
		...
		.clk_id_udc = DT_INST_CLOCKS_CELL_BY_NAME(inst, udc, clk_id),                      \
		.clk_id_atrim = DT_INST_CLOCKS_CELL_BY_NAME(inst, atrim, clk_id),                  \
		.clk_id_aip = DT_INST_CLOCKS_CELL_BY_NAME(inst, aip, clk_id),                      \
		.xfer_type = 0,                                                                    \
		.is_ep0_out_pkg = 0,                                                               \
		.is_pending_pkg = 0,                                                               \
		.is_ep0_in_en = 0,                                                                 \
