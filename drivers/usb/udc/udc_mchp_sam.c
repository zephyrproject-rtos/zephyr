...
#ifdef CONFIG_UDC_MCHP_SAM_RAW_DEBUG
#undef LOG_DBG
#define LOG_DBG(fmt, args...) printk(fmt "\n", ##args)
#endif
...
static inline udphs_registers_t *base_reg(const struct udc_sam_config *config)
{
	return config->base;
}

static inline udphs_ept_registers_t *ep_reg(const struct udc_sam_config *config, const uint8_t ep)
{
	udphs_registers_t *const udphs = config->base;

	return &udphs->UDPHS_EPT[ep];
}

static inline udphs_dma_registers_t *dma_reg(const struct udc_sam_config *config, const uint8_t dma)
{
	udphs_registers_t *const udphs = config->base;
...
static inline uint8_t *fifo_addr(const struct udc_sam_config *config, const uint8_t ep)
{
	return config->fifo + (ep << 16);
}

static inline int ep_banks(const struct udc_sam_config *config, const uint8_t ep)
{
	const struct udphs_ep_desc *ep_desc = &config->ep_desc[ep];

	return ep_desc->nr_banks;
}

static inline int ep_size(const struct udc_sam_config *config, const uint8_t ep)
{
	const struct udphs_ep_desc *ep_desc = &config->ep_desc[ep];

	return 1 << ep_desc->ep_size;
}

static inline int ep_can_dma(const struct udc_sam_config *config, const uint8_t ep)
{
	const struct udphs_ep_desc *ep_desc = &config->ep_desc[ep];
...
static void udphs_speed_mode(udphs_registers_t *const udphs, int mode)
{
...
static void udphs_reset_ep(udphs_registers_t *const udphs, int ep)
{
	udphs_ept_registers_t *const ept = &udphs->UDPHS_EPT[ep];

	ept->UDPHS_EPTCTLDIS = UDPHS_EPTCTLDIS_Msk;
	ept->UDPHS_EPTCFG = 0;
	ept->UDPHS_EPTCLRSTA = UDPHS_EPTCLRSTA_TOGGLESQ_Msk |
			       UDPHS_EPTCLRSTA_FRCESTALL_Msk;
	udphs->UDPHS_EPTRST = UDPHS_EPTRST_EPT_0_Msk << ep;
	udphs->UDPHS_IEN &= ~(UDPHS_IEN_EPT_0_Msk << ep);
}
...
static void udphs_clear_ep_status(udphs_registers_t *const udphs, int ep)
{
	udphs->UDPHS_EPTRST = UDPHS_EPTRST_EPT_0_Msk << ep;
}

static int udphs_config_ep(udphs_registers_t *const udphs, int ep, struct udphs_ep_config *cfg)
{
	udphs_ept_registers_t *const ept =  &udphs->UDPHS_EPT[ep];
	...
	if (cfg->dma) {
		udphs->UDPHS_IEN |= UDPHS_IEN_DMA_1_Msk << (ep - 1);
	}
	udphs->UDPHS_IEN |= UDPHS_IEN_EPT_0_Msk << ep;
...
static int udphs_ep_dma_start(udphs_ept_registers_t *const ept, udphs_dma_registers_t *const dma,
			      struct udphs_dma_request *req)
{
	uint32_t ctrl;

	if (req->len > MAX_DMA_LEN) {
		return -1;
	}
...
{
	const struct udc_sam_config *config = dev->config;
	uint8_t ep = EP(ep_cfg->addr);
	udphs_ept_registers_t *const ept = ep_reg(config, ep);
	unsigned int lock_key;

	LOG_DBG("Prep OUT ep%02x %u %s", ep_cfg->addr, buf->size,
					 ep_can_dma(config, ep) ? "dma" : "fifo");

	if (ep_can_dma(config, ep)) {
		udphs_dma_registers_t *const dma = dma_reg(config, DMA(ep_cfg->addr));
		struct udphs_dma_request req = {0};

		req.buf = buf->data;
		req.len = MIN(buf->size, MAX_DMA_LEN);

		lock_key = irq_lock();
		udphs_ep_dma_start(ept, dma, &req);
	} else {
		lock_key = irq_lock();
		udphs_ep_fifo_out(ept, ENABLE);
	}
		       ...
		       struct net_buf *const buf, struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_can_dma(config, ep)) {
		...
		req.is_in = 1;
		req.zero = udc_ep_buf_has_zlp(buf);
		req.buf = buf->data;
		req.len = MIN(buf->len, MAX_DMA_LEN);

		lock_key = irq_lock();
		udphs_ep_dma_start(ept, dma, &req);
	} else {
		lock_key = irq_lock();
		udphs_ep_fifo_in(ept, ENABLE);
	}
...
static int udc_handle_evt_setup(const struct device *dev, char *setup)
{
	...
	buf = udc_ctrl_alloc(dev, USB_CONTROL_EP_OUT, SETUP_PKT_SIZE);
	if (buf == NULL) {
		return -ENOMEM;
	}

	net_buf_add_mem(buf, setup, SETUP_PKT_SIZE);
...
{
	struct udc_sam_data *const priv = udc_get_private(dev);
	udphs_dma_registers_t *const dma = dma_reg(dev->config, chan);
	int ep = chan + 1;
	udphs_ept_registers_t *const ept = ep_reg(dev->config, ep);
	struct udc_ep_config *ep_cfg;
	struct net_buf *buf;
	...
	if (status & UDPHS_DMASTATUS_CHANN_ENB_Msk) {
		LOG_ERR("DMA_CHANN_ENB is set after transfer is finished!");
		LOG_ERR("DMA%d, s/%08x c/%08x", chan + 1, status, ctrl);
	}
	...
	if ((status & ctrl) & (UDPHS_DMASTATUS_END_BF_ST_Msk | UDPHS_DMASTATUS_END_TR_ST_Msk)) {
		if (ctrl & UDPHS_DMACONTROL_END_TR_EN_Msk) { /* Is OUT */
			...
			buf = udc_buf_peek(ep_cfg);
			if (buf == NULL) {
				LOG_ERR("No buffer for ep%02x", ep);
				udc_submit_event(dev, UDC_EVT_ERROR, -ENOBUFS);
				return -1;
			}

			size = MIN(net_buf_tailroom(buf), MAX_DMA_LEN);
			net_buf_add(buf, size - BUFF_COUNT(status));
			size = MIN(net_buf_tailroom(buf), MAX_DMA_LEN);
			...
			ep_cfg = udc_get_ep_cfg(dev, ep | USB_EP_DIR_IN);
			...
			if (buf->len == 0) {
				...
				atomic_set_bit(&priv->xfer_finished,
					       udc_ep_to_bnum(ep | USB_EP_DIR_IN));
...
static void ALWAYS_INLINE handle_ep_irq(const struct device *dev, const uint8_t ep)
{
	const struct udc_sam_config *config = dev->config;
	struct udc_sam_data *const priv = udc_get_private(dev);
	udphs_ept_registers_t *const ept = ep_reg(config, ep);
	...
	LOG_DBG(" isr ep%d, s/%08x c/%08x", ep, status, ctrl);
	...
	while ((ctrl & UDPHS_EPTCTL_TXRDY_Msk) && !(status & UDPHS_EPTSTA_TXRDY_Msk)) {
		...
		if (buf->len == 0) {
			...
			memcpy(fifo_addr(config, ep), buf->data, size);
			ept->UDPHS_EPTSETSTA = UDPHS_EPTSETSTA_TXRDY_Msk;
			net_buf_pull(buf, size);
		}
	...
	if ((status & ctrl) & UDPHS_EPTSTA_RXRDY_TXKL_Msk) {
		...
		while (BUSY_BANKS(status) > 0) {
			size = MIN(net_buf_tailroom(buf), BYTE_COUNT(status));

			if (size > 0) {
				memcpy(buf->data, fifo_addr(config, ep), size);
	...
	/* Is SETUP */
	if ((ep == 0) && ((status & ctrl) & UDPHS_EPTSTA_RX_SETUP_Msk)) {
		if (BYTE_COUNT(status) != SETUP_PKT_SIZE) {
		...
		memcpy(priv->setup, fifo_addr(config, ep), SETUP_PKT_SIZE);
...
{
	struct udc_sam_data *const priv = udc_get_private(dev);
	udphs_registers_t *const udphs = base_reg(dev->config);
...
static int udc_sam_ep_dequeue(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_can_dma(config, ep)) {
		udphs_registers_t *const udphs = base_reg(config);
		udphs_dma_registers_t *const dma = dma_reg(config, DMA(ep_cfg->addr));

		if (udphs_ep_dma_stop(dma)) {
			LOG_ERR("Failed to stop EP %02x DMA", ep_cfg->addr);
		}
		udphs_clear_ep_status(udphs, ep);
	} else {
		if (USB_EP_DIR_IS_IN(ep_cfg->addr)) {
			udphs_ep_fifo_in(ept, DISABLE);
		} else {
			udphs_ep_fifo_out(ept, DISABLE);
		}
	}
...
static int udc_sam_ep_enable(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	LOG_DBG("Enable ep%02x %s", ep_cfg->addr, ep_can_dma(dev->config, ep) ? "dma" : "fifo");

	udphs_reset_ep(udphs, ep);
	...
	cfg.banks_num = ep_banks(dev->config, ep);
	cfg.trans_num = 0;
	cfg.dma = ep_can_dma(dev->config, ep);

	lock_key = irq_lock();

	if (udphs_config_ep(udphs, ep, &cfg)) {
		LOG_ERR("Failed to config ep %d", ep);
		ret = -EINVAL;
	}
...
{
	udphs_ept_registers_t *const ept = ep_reg(dev->config, EP(ep_cfg->addr));
	unsigned int lock_key;
...
static int udc_sam_ep_clear_halt(const struct device *dev, struct udc_ep_config *const ep_cfg)
{
	...
	if (ep_cfg->stat.halted == false) {
		return 0;
	}

	lock_key = irq_lock();

	udphs_ep_clear_halt(ept);
	ep_cfg->stat.halted = false;

	irq_unlock(lock_key);

	if (EP(ep_cfg->addr) != 0 && !udc_ep_is_busy(ep_cfg)) {
...
static int udc_sam_driver_preinit(const struct device *dev)
{
	...
	for (i = 0; i < UDPHS_EPT_NUMBER / 2; i++) {
		...
		if (i == 0) {
			...
			config->ep_cfg_out[i].caps.mps = ep_size(config, i);
			config->ep_cfg_out[i].addr = USB_EP_DIR_OUT | i;
		} else {
			config->ep_cfg_out[i].caps.bulk = 1;
			config->ep_cfg_out[i].caps.interrupt = 1;
			config->ep_cfg_out[i].caps.iso = 1;
			config->ep_cfg_out[i].caps.mps = ep_size(config, i * 2 - 1);
	...
	for (i = 0; i < UDPHS_EPT_NUMBER / 2; i++) {
		...
		if (i == 0) {
			...
			config->ep_cfg_in[i].caps.mps = ep_size(config, i);
		} else {
			config->ep_cfg_in[i].caps.bulk = 1;
			config->ep_cfg_in[i].caps.interrupt = 1;
			config->ep_cfg_in[i].caps.iso = 1;
			config->ep_cfg_in[i].caps.mps = ep_size(config, i * 2);
		}
