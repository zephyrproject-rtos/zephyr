...
const static struct device *udc_max32_dev;

struct udc_max32_config {
	mxc_usbhs_regs_t *regs;
...
static void udc_event_xfer_in_callback(void *cbdata)
{
	...
	buf = udc_buf_get(req_cb_data->dev, req_cb_data->ep);

	udc_ep_set_busy(req_cb_data->dev, req_cb_data->ep, false);
...
static void udc_event_xfer_in(const struct device *dev, const uint8_t ep)
{
	...
	MXC_USB_Req_t *ep_request = priv->ep_request[USB_EP_GET_IDX(ep)];
	int ret = 0;
	...
	if (buf != NULL) {
		if (buf->len == 0 && ep == USB_CONTROL_EP_IN) {
			buf = udc_buf_get(dev, ep);
			udc_event_xfer_ctrl_status(dev, buf);
			return;
		}

		struct req_cb_data *req_cb_data = k_malloc(sizeof(struct req_cb_data));

		req_cb_data->dev = dev;
		req_cb_data->ep = ep;

		ep_request->ep = USB_EP_GET_IDX(ep);
		...
		if (ret != 0) {
			...
			LOG_ERR("ep 0x%02x error: %x", ep, ret);
 ...
 * caller from the UDC_MAX32_DEVICE_DEFINE macro.
 */
...
static int udc_max32_ep_enable(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	priv->ep_request[USB_EP_GET_IDX(cfg->addr)] = k_malloc(sizeof(MXC_USB_Req_t));
...
static int udc_max32_ep_disable(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	k_free(priv->ep_request[USB_EP_GET_IDX(cfg->addr)]);
...
static int udc_max32_ep_clear_halt(const struct device *dev, struct udc_ep_config *const cfg)
{
	...
	if (udc_buf_peek(dev, cfg->addr) != NULL) {
		struct udc_max32_evt evt = {
			.type = UDC_MAX32_EVT_XFER,
			.ep = cfg->addr,
		};
...
static int udc_max32_set_address(const struct device *dev, const uint8_t addr)
{
	...
	if (ret != 0) {
		LOG_ERR("MXC_USB_SetFuncAddr failed(0x%02x), %d", addr, ret);
		return -EINVAL;
	}
...
{
	struct udc_max32_data *priv = (struct udc_max32_data *)cbdata;

	switch (event) {
	case MAXUSB_EVENT_NOVBUS:
		LOG_DBG("NOVBUS event occurred\n");
		priv->vbus_detected = false;
		break;
	case MAXUSB_EVENT_VBUS:
		LOG_DBG("VBUS event occurred\n");
		priv->vbus_detected = true;
		break;
	case MAXUSB_EVENT_SUSP:
		LOG_DBG("SUSP event occurred\n");
		udc_set_suspended(udc_max32_dev, true);
		udc_submit_event(udc_max32_dev, UDC_EVT_SUSPEND, 0);
		break;
	case MAXUSB_EVENT_DPACT:
		LOG_DBG("DPACT event occurred\n");
		udc_set_suspended(udc_max32_dev, false);
		udc_submit_event(udc_max32_dev, UDC_EVT_SOF, 0);
		break;
	case MAXUSB_EVENT_BRST:
		LOG_DBG("BRST event occurred\n");
		udc_set_suspended(udc_max32_dev, false);
		udc_submit_event(udc_max32_dev, UDC_EVT_RESET, 0);
		break;
...
static int udc_max32_enable(const struct device *dev)
{
	...
	if (!priv->vbus_detected) {
		return -1;
	}

	MXC_USB_EventClear(MAXUSB_EVENT_SUDAV);
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_SUDAV, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
	...
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_SUSP, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
	...
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_DPACT, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
	...
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_BRST, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
...
static int udc_max32_init(const struct device *dev)
{
	...
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_NOVBUS, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
		LOG_ERR("Failed to enable NOVBUS event.");
		return ret;
	}
	ret = MXC_USB_EventEnable(MAXUSB_EVENT_VBUS, udc_max32_event_callback,
				  udc_get_private(dev));
	if (ret != 0) {
	...
	priv->ep_request[0] = k_malloc(sizeof(MXC_USB_Req_t));
...
#define UDC_MAX32_DEVICE_DEFINE(n)                                                                 \
	...
	static const struct udc_max32_config udc_max32_config_##n = {                              \
		.regs = (mxc_usbhs_regs_t *)DT_INST_REG_ADDR(n),                                   \
