...
static void udc_event_xfer_in(const struct device *dev, const uint8_t ep)
{
	...
	struct req_cb_data *req_cb_data = &priv->req_cb_data[USB_EP_GET_IDX(ep)];
	struct net_buf *buf;
	int ret;
	...
	if (buf == NULL) {
		LOG_ERR("Failed to peek net_buf for ep 0x%02x", ep);
		return;
	}
