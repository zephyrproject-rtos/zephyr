...
struct dwc2_vendor_quirks {
	...
	/* Translate a CPU buffer pointer to the address the controller's DMA
	 * master must be programmed with (e.g. a SoC bus address that differs
	 * from the CPU physical address). NULL == identity.
	 */
	uint32_t (*dma_addr)(const struct device *dev, void *buf);
};
...
/* Not generated by DWC2_QUIRK_FUNC_DEFINE() because the signature differs:
 * returns the (possibly translated) DMA bus address for a CPU buffer.
 */
static inline uint32_t dwc2_quirk_dma_addr(const struct device *dev, void *buf)
{
	const struct udc_dwc2_config *const config = dev->config;
	const struct dwc2_vendor_quirks *const quirks = config->quirks;

	if (quirks != NULL && quirks->dma_addr != NULL) {
		return quirks->dma_addr(dev, buf);
	}
