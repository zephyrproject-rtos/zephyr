...
size_t udc_out_ep_alloc_size(const struct device *dev, uint8_t ep, size_t expected_len)
{
	struct udc_ep_config *cfg = udc_get_ep_cfg(dev, ep);
	/* Return early if the parameters are invalid */
	if ((cfg == NULL) || (cfg->mps == 0)) {
		return 0;
	}
	/* The controller allows receiving max packet size transfers at a time.
	 * When DMA moves the data directly to memory, that memory space must be large enough
	 * to accommodate the maximum packet size.
	 * TODO: This behavior can be made driver-specific in the future to optimize memory usage
	 * on the lower end targets that don't require such extra allocations.
	 */
	return ROUND_UP(MAX(1U, expected_len), cfg->mps);
}
