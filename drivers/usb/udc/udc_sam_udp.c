 ...
 *
 * SAM UDP hardware has 8 endpoints (EP0-EP7). EP0 is always control.
 * EP1-EP7 can be configured as either IN or OUT, not both.
 *
 ...
 */
static const uint8_t in_ep_hw_map[] = {0, 1, 3, 5, 7};   /* 5 IN endpoints */
static const uint8_t out_ep_hw_map[] = {0, 2, 4, 6};     /* 4 OUT endpoints */
...
static void sam_udp_isr_dbg_dev(Udp *regs, uint32_t isr)
{
	uint32_t faddr = regs->UDP_FADDR;
	uint32_t glb_stat = regs->UDP_GLB_STAT;
	uint32_t imr = regs->UDP_IMR;
	uint32_t csr0 = regs->UDP_CSR[0];
...
static void sam_udp_isr_dbg_ep(Udp *regs, uint8_t ep_idx)
{
	uint32_t csr = regs->UDP_CSR[ep_idx];
...
#define SAM_UDP_ISR_DBG_DEV(regs, isr) sam_udp_isr_dbg_dev(regs, isr)
#define SAM_UDP_ISR_DBG_EP(regs, ep_idx) sam_udp_isr_dbg_ep(regs, ep_idx)

#else /* !CONFIG_UDC_DRIVER_LOG_LEVEL_DBG */

#define SAM_UDP_ISR_DBG_DEV(regs, isr)
#define SAM_UDP_ISR_DBG_EP(regs, ep_idx)
...
static inline Udp *udc_sam_udp_get_regs(const struct device *dev)
{
 ...
 */
static inline uint8_t ep_to_bit(uint8_t ep_addr)
{
	if (USB_EP_DIR_IS_IN(ep_addr)) {
		return USB_EP_GET_IDX(ep_addr) + 8;
	}
...
static inline void reset_endpoint(Udp *regs, uint8_t ep)
{
	regs->UDP_RST_EP |= BIT(ep);
	while (!(regs->UDP_RST_EP & BIT(ep))) {
		;
	}
	regs->UDP_RST_EP &= ~BIT(ep);
}

static inline void reset_all_endpoints(Udp *regs)
{
 ...
 */
static inline bool ep_has_dual_bank(uint8_t hw_ep)
{
	return hw_ep != 0 && hw_ep != 3;
}

static inline void clear_csr_bits(Udp *regs, uint8_t ep, uint32_t bits)
{
	...
	while ((regs->UDP_CSR[ep] & bits) != 0) {
		;
	}
}

static inline void set_csr_bits(Udp *regs, uint8_t ep, uint32_t bits)
{
	volatile uint32_t csr;
	...
	while ((regs->UDP_CSR[ep] & bits) != bits) {
		;
	}
}
 ...
 */
static void cancel_pending_tx(Udp *regs, uint8_t hw_ep)
{
	uint32_t csr = regs->UDP_CSR[hw_ep];

	if (!(csr & UDP_CSR_TXPKTRDY)) {
		/* TXPKTRDY not set - just reset endpoint */
		reset_endpoint(regs, hw_ep);
		return;
	}
	...
	if (ep_has_dual_bank(hw_ep)) {
		 ...
		 */
		clear_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
		set_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
		clear_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
	} else {
...
static void udc_sam_udp_write_fifo(Udp *regs, uint8_t ep,
				   const uint8_t *data, uint16_t len)
{
	for (uint16_t i = 0; i < len; i++) {
		regs->UDP_FDR[ep] = data[i];
	}
}
 ...
 */
static uint16_t fill_tx_bank(const struct device *dev, uint8_t hw_ep,
			     struct net_buf *buf)
{
	Udp *regs = udc_sam_udp_get_regs(dev);
	...
	udc_sam_udp_write_fifo(regs, hw_ep, buf->data, len);
...
static uint16_t udc_sam_udp_read_fifo(Udp *regs, uint8_t ep,
				      uint8_t *data, uint16_t max_len)
{
	uint16_t count = (regs->UDP_CSR[ep] & UDP_CSR_RXBYTECNT_Msk) >>
			 UDP_CSR_RXBYTECNT_Pos;

	count = MIN(count, max_len);

	for (uint16_t i = 0; i < count; i++) {
		data[i] = regs->UDP_FDR[ep];
	}
...
/*
 * Suspend/Resume Clock Management
 *
 * Per datasheet 40.6.3.6, during suspend we can disable UDPCK and MCK
 * to reduce power. The resume (WAKEUP) is detected asynchronously.
 *
 * Per datasheet 40.6.3.7, MCK must be enabled BEFORE any UDP register
 * access (including clearing WAKEUP in UDP_ICR).
 */
static void udc_sam_udp_suspend_clocks(const struct device *dev)
{
	const struct udc_sam_udp_config *config = dev->config;
	...
	/* Disable MCK for UDP (must be last per datasheet warning) */
	clock_control_off(config->clock_dev,
			  (clock_control_subsys_t)config->clock_cfg);

	LOG_DBG("USB clocks suspended");
}
				  ...
				  struct net_buf *const buf)
{
	...
	if (USB_EP_DIR_IS_IN(cfg->addr)) {
		if (buf == udc_buf_peek(cfg)) {
			Udp *regs = udc_sam_udp_get_regs(dev);
			uint8_t hw_ep = ep_addr_to_hw_ep(cfg->addr);
			uint16_t len;
				   ...
				   struct udc_ep_config *const cfg)
{
	...
	set_csr_bits(regs, hw_ep, UDP_CSR_FORCESTALL);
				     ...
				     struct udc_ep_config *const cfg)
{
	...
	clear_csr_bits(regs, hw_ep, UDP_CSR_FORCESTALL);
	clear_csr_bits(regs, hw_ep, UDP_CSR_STALLSENT);
				 ...
				 struct udc_ep_config *const cfg)
{
	...
	set_csr_bits(regs, hw_ep, ep_type | UDP_CSR_EPEDS);

	regs->UDP_IER = BIT(hw_ep);

	LOG_DBG("EP%d enabled: CSR=0x%08x", hw_ep, regs->UDP_CSR[hw_ep]);
				  ...
				  struct udc_ep_config *const cfg)
{
	...
	regs->UDP_IDR = BIT(hw_ep);

	clear_csr_bits(regs, hw_ep, UDP_CSR_EPEDS);
...
static int udc_sam_udp_set_address(const struct device *dev, const uint8_t addr)
{
	 ...
	 */
	regs->UDP_FADDR = UDP_FADDR_FEN | UDP_FADDR_FADD(addr);
...
static int udc_sam_udp_enable(const struct device *dev)
{
	...
	regs->UDP_TXVC = UDP_TXVC_PUON | UDP_TXVC_TXVDIS;

	LOG_DBG("UDP_TXVC: 0x%08x", regs->UDP_TXVC);
...
static int udc_sam_udp_init(const struct device *dev)
{
	...
	/* Clock already enabled in preinit */
...
static int udc_sam_udp_ctrl_feed_dout(const struct device *dev,
				      const size_t length)
{
	struct udc_ep_config *const ep_cfg = udc_get_ep_cfg(dev,
							   USB_CONTROL_EP_OUT);
	struct net_buf *buf;
...
static void udc_sam_udp_isr_handle_setup(const struct device *dev)
{
	...
	/* Set DIR bit based on data direction before clearing RXSETUP */
	if (priv->setup[0] & USB_EP_DIR_IN) {
		LOG_DBG("Setting DIR bit for IN transfer");
		set_csr_bits(regs, 0, UDP_CSR_DIR);
	}

	/* Clear RXSETUP - must be done after reading FIFO and setting DIR */
	clear_csr_bits(regs, 0, UDP_CSR_RXSETUP);

	LOG_DBG("RXSETUP cleared, CSR=0x%08x", regs->UDP_CSR[0]);
 ...
 */
static void udc_sam_udp_handle_in(const struct device *dev, uint8_t ep_addr)
{
	Udp *regs = udc_sam_udp_get_regs(dev);
	struct udc_sam_udp_data *priv = udc_get_private(dev);
	struct udc_ep_config *cfg = udc_get_ep_cfg(dev, ep_addr);
	uint8_t hw_ep = ep_addr_to_hw_ep(ep_addr);
	struct net_buf *buf;
	...
	if (priv->ep_data[hw_ep].tx_banks > 0) {
		set_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
		LOG_DBG("IN ep 0x%02x: commit pre-filled bank", ep_addr);
	}
	...
	while (buf != NULL && buf->len > 0 &&
		 ...
		 */
		set_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
		LOG_DBG("IN ep 0x%02x: filled bank, %u bytes", ep_addr, len);
	}
	...
	while (buf != NULL && buf->len == 0 &&
		...
		while (buf != NULL && buf->len > 0 &&
			...
			set_csr_bits(regs, hw_ep, UDP_CSR_TXPKTRDY);
			LOG_DBG("IN ep 0x%02x: next buf, %u bytes", ep_addr, len);
		}
	}

	/* Clear TXCOMP after TXPKTRDY per datasheet 40.6.2.2 */
	clear_csr_bits(regs, hw_ep, UDP_CSR_TXCOMP);
}
 ...
 * RX_DATA_BK1."
 */
static uint32_t sam_udp_get_out_bank(struct udc_sam_udp_data *priv,
				     uint8_t hw_ep, uint32_t csr)
{
 ...
 */
static void sam_udp_update_next_bank(struct udc_sam_udp_data *priv,
				     uint8_t hw_ep, uint32_t bank_flag)
{
 ...
 */
static void sam_udp_handle_out_isr(const struct device *dev, uint8_t hw_ep)
{
	Udp *regs = udc_sam_udp_get_regs(dev);
	struct udc_sam_udp_data *priv = udc_get_private(dev);
	uint8_t ep_addr = USB_EP_DIR_OUT | hw_ep;
	struct udc_ep_config *cfg = udc_get_ep_cfg(dev, ep_addr);
	struct net_buf *buf;
	uint32_t csr;
	uint32_t bank_flag;
	...
	/* Read data from FIFO */
	len = udc_sam_udp_read_fifo(regs, hw_ep, net_buf_tail(buf),
				    net_buf_tailroom(buf));
	net_buf_add(buf, len);

	LOG_DBG("ISR OUT ep 0x%02x len %u bank %d",
		ep_addr, len, (bank_flag == UDP_CSR_RX_DATA_BK0) ? 0 : 1);

	/* Clear the bank flag and update alternation counter */
	clear_csr_bits(regs, hw_ep, bank_flag);
				       ...
				       const uint8_t ep_addr)
{
	...
	if (bank_flag == 0) {
		LOG_DBG("OUT ep 0x%02x no data in bank", ep_addr);
		regs->UDP_IER = BIT(hw_ep);
	...
	if (buf == NULL) {
		 ...
		 */
		LOG_DBG("OUT ep 0x%02x no buffer - NAK until ready", ep_addr);
...
static ALWAYS_INLINE void sam_udp_thread_handler(const struct device *const dev)
{
	...
	/* Process SETUP packet - priority over OUT data */
...
static void udc_sam_udp_isr_handler(const struct device *dev)
{
	...
	/*
	 * Fallback clock re-enable for suspended state.
	 *
	 * Per datasheet 40.6.3.7: MCK must be enabled BEFORE any UDP
	 * register access. Since we need to read UDP_ISR below, we must
	 * re-enable clocks here if suspended. The WAKEUP interrupt is
	 * detected asynchronously even with clocks disabled.
	 *
	 * The proper clock re-enable is done in the WAKEUP/RXRSM handler
	 * per the datasheet sequence, but this fallback ensures we can
	 * safely read UDP registers in all cases.
	 */
	if (priv->suspended) {
		udc_sam_udp_resume_clocks(dev);
	}
	...
	if (status & UDP_ISR_ENDBUSRES) {
		LOG_INF("Bus reset, ISR=0x%08x", isr);
		...
		set_csr_bits(regs, 0, UDP_CSR_EPTYPE_CTRL | UDP_CSR_EPEDS);

		/* Enable EP0 and suspend/wakeup interrupts */
		regs->UDP_IER = UDP_ISR_EP0INT | UDP_ISR_RXSUSP | UDP_ISR_WAKEUP;

		/* Enable transceiver - clear TXVDIS, keep PUON */
		regs->UDP_TXVC = UDP_TXVC_PUON;

		LOG_INF("EP0 CSR=0x%08x, TXVC=0x%08x", regs->UDP_CSR[0], regs->UDP_TXVC);
	...
	if (status & UDP_ISR_RXSUSP) {
		...
		udc_sam_udp_suspend_clocks(dev);
	...
	if (status & (UDP_ISR_RXRSM | UDP_ISR_WAKEUP)) {
		...
		regs->UDP_ICR = UDP_ISR_RXRSM | UDP_ISR_WAKEUP;
...
static int udc_sam_udp_driver_preinit(const struct device *dev)
{
	...
	err = udc_sam_udp_enable_usb_clock(dev);
	if (err < 0) {
		LOG_ERR("Failed to enable USB clock: %d", err);
	...
	/*
	 * Register EP0 (control, bidirectional)
	 */
 ...
 *
 * Without this reservation, lower-priority transfers (bulk/interrupt with
 * 64-byte MPS) may claim these endpoints before ISO transfers that require
 * the larger buffer size, causing ISO endpoint allocation to fail.
 *
