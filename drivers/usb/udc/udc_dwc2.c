...
static int udc_dwc2_init_controller(const struct device *dev)
{
	...
	if (priv->dynfifosizing) {
		...
		/* Get available PSRAM size and calculate max allocatable RX fifo size */
		val = sys_read32((mem_addr_t)&base->gdfifocfg);
		psram_size = usb_dwc2_get_gdfifocfg_gdfifocfg(val);
		max_rxfifo = ((psram_size * MAX_RXFIFO_GDFIFO_PERCENTAGE) / 100);
