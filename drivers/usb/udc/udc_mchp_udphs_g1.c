...
#define DT_DRV_COMPAT microchip_udphs_g1_udc
...
LOG_MODULE_REGISTER(udc_mchp_udphs_g1, CONFIG_UDC_DRIVER_LOG_LEVEL);
...
struct udphs_ep_config {
	...
	uint32_t dir:1;
	uint32_t size:3;
	uint32_t banks_num:2;
	uint32_t trans_num:2;
	uint32_t dma:1;
};
...
static const struct udphs_ep_desc sam_ep_desc[UDPHS_EPT_NUMBER] = {
	...
	{ .nr_banks = 3, .ep_size = SZ_1024, .can_dma = 1, .high_bw = 1 }, /* ep 1 */
	{ .nr_banks = 3, .ep_size = SZ_1024, .can_dma = 1, .high_bw = 1 }, /* ep 2 */
	{ .nr_banks = 2, .ep_size = SZ_1024, .can_dma = 1 },               /* ep 3 */
	{ .nr_banks = 2, .ep_size = SZ_512,  .can_dma = 1 },               /* ep 4 */
	{ .nr_banks = 2, .ep_size = SZ_512,  .can_dma = 1 },               /* ep 5 */
	{ .nr_banks = 2, .ep_size = SZ_512,  .can_dma = 1 },               /* ep 6 */
	{ .nr_banks = 2, .ep_size = SZ_512,  .can_dma = 1 },               /* ep 7 */
...
static inline bool ep_can_dma(const struct device *const dev, uint8_t idx)
{
	...
	return config->dma_enable && ep_desc->can_dma;
}
...
#ifdef CONFIG_UDC_MCHP_UDPHS_G1_BURST_LOCK
...
static int sam_handle_evt_finished(const struct device *dev,
...
static ALWAYS_INLINE void udc_thread_handler(const struct device *const dev)
{
	...
	if (evt & BIT(UDC_THREAD_EVT_XFER_FINISHED)) {
		...
		while (eps) {
			...
			ret = sam_handle_evt_finished(dev, ep_cfg);
			if (ret) {
...
static int ALWAYS_INLINE dma_in(const struct device *dev, const uint8_t chan, uint32_t status)
{
	struct udc_sam_data *const priv = udc_get_private(dev);
	udphs_dma_registers_t *const dma = dma_reg(dev, chan);
	uint8_t idx = chan + 1;
...
static int ALWAYS_INLINE handle_dma_irq(const struct device *dev, const uint8_t chan)
{
	...
	if (status & UDPHS_DMASTATUS_CHANN_ENB_Msk) {
		/*
		 * The END_TR_ST interrupt comes from the last transmission,
		 * just ignore it.
		 */
		return 0;
	}
	...
	if (!((status & ctrl) &
	      (UDPHS_DMASTATUS_END_BF_ST_Msk | UDPHS_DMASTATUS_END_TR_ST_Msk))) {
		return 0;
	}

	if (ctrl & UDPHS_DMACONTROL_END_TR_EN_Msk) { /* Is OUT */
		return dma_out(dev, chan, status);
	} else { /* Is IN */
		return dma_in(dev, chan, status);
	}
}
...
static void sam_isr_handler(const struct device *dev)
{
	...
	if (status & UDPHS_INTSTA_INT_SOF_Msk) {
		udphs->UDPHS_CLRINT = UDPHS_CLRINT_INT_SOF_Msk;
...
static int udc_sam_test_mode(const struct device *dev, const uint8_t mode, const bool dryrun)
{
	udphs_registers_t *const udphs = base_reg(dev);
	udphs_ept_registers_t *const ept = ep_reg(dev, 0);

	static const char test_packet[] = {
		/* JKJKJKJK * 9 */
	...
	case USB_SFS_TEST_MODE_J:
		/* Test_J */
		udphs->UDPHS_TST = UDPHS_TST_TST_J_1;
		LOG_DBG("Entering Test_J mode...");
		break;
	case USB_SFS_TEST_MODE_K:
		/* Test_K */
		udphs->UDPHS_TST = UDPHS_TST_TST_K_1;
		LOG_DBG("Entering Test_K mode...");
		break;
	case USB_SFS_TEST_MODE_SE0_NAK:
		/*
		...
		ept->UDPHS_EPTCFG = UDPHS_EPTCFG_BK_NUMBER_1 |
				    UDPHS_EPTCFG_EPT_TYPE_BULK |
				    UDPHS_EPTCFG_EPT_DIR_1 |
				    UDPHS_EPTCFG_EPT_SIZE_64;
		if ((ept->UDPHS_EPTCFG & UDPHS_EPTCFG_EPT_MAPD_Msk) == 0) {
			ept->UDPHS_EPTSETSTA = UDPHS_EPTSETSTA_FRCESTALL_Msk;
			LOG_ERR("Test_SE0_NAK: ep0 not mapped");
		} else {
			ept->UDPHS_EPTCTLENB = UDPHS_EPTCTLENB_EPT_ENABL_Msk;
			LOG_DBG("Entering Test_SE0_NAK mode...");
		}
	case USB_SFS_TEST_MODE_PACKET:
		/* Test_Packet */
		ept->UDPHS_EPTCFG = UDPHS_EPTCFG_BK_NUMBER_1 |
		...
		if ((ept->UDPHS_EPTCFG & UDPHS_EPTCFG_EPT_MAPD_Msk) == 0) {
			ept->UDPHS_EPTSETSTA = UDPHS_EPTSETSTA_FRCESTALL_Msk;
			LOG_ERR("Test_Packet: ep0 not mapped");
		} else {
			ept->UDPHS_EPTCTLENB = UDPHS_EPTCTLENB_EPT_ENABL_Msk;
			udphs->UDPHS_TST = UDPHS_TST_TST_PKT_1;
			memcpy(fifo_addr(dev, 0), test_packet, sizeof(test_packet));
			ept->UDPHS_EPTSETSTA = UDPHS_EPTSETSTA_TXRDY_Msk;
			LOG_DBG("Entering Test_Packet mode...");
		}
		break;
	default:
		return -EINVAL;
...
static int udc_sam_enable(const struct device *dev)
{
	...
	/*
	 * Added delay between clock start and register access;
	 * otherwise, the EP cannot be mapped successfully.
	 */
	k_usleep(10);
...
static int udc_sam_driver_preinit(const struct device *dev)
{
	...
	if (config->num_of_eps > UDPHS_EPT_NUMBER) {
		LOG_ERR("Incorrect eps number %d, max is %d", config->num_of_eps, UDPHS_EPT_NUMBER);
		return -EINVAL;
	}
	...
	for (i = 0; i < config->num_of_eps / 2; i++) {
...
#define UDC_SAM_DEVICE_DEFINE(n)							\
	...
	K_THREAD_STACK_DEFINE(udc_sam_stack_##n,					\
			      CONFIG_UDC_MCHP_UDPHS_G1_STACK_SIZE);			\
											\
	...
	{										\
		...
		k_thread_create(&priv->thread_data,					\
				...
				udc_sam_thread_##n,					\
				(void *)dev, NULL, NULL,				\
				K_PRIO_COOP(CONFIG_UDC_MCHP_UDPHS_G1_THREAD_PRI),	\
				K_ESSENTIAL,						\
				K_NO_WAIT);						\
	...
	static struct udc_ep_config							\
		ep_cfg_out_##n[UDPHS_EPT_NUMBER];					\
	static struct udc_ep_config							\
		ep_cfg_in_##n[UDPHS_EPT_NUMBER];					\
											\
