...
struct udc_dwc3_config {
	...
	/* Setup packet always ready to receive a setup packet from the host */
				      ...
				      struct udc_dwc3_ep_data *const ep_data)
{
	...
	LOG_INF("cmd: configuring endpoint 0x%02x with wMaxPacketSize=%u",
...
static void udc_dwc3_trb_ctrl_setup(const struct device *const dev)
{
	...
	trb[0].addr_lo = LO32((uintptr_t)cfg->setup_packet);
	trb[0].addr_hi = HI32((uintptr_t)cfg->setup_packet);
	trb[0].status = sizeof(*cfg->setup_packet);
	trb[0].ctrl = UDC_DWC3_TRB_CTRL_TRBCTL_CONTROL_SETUP |
				 ...
				 const uint32_t ctrl)
{
	...
	LOG_DBG("TRB_CONTROL_OUT size=%u ctrl=0x%x", buf->size, ctrl);
...
static void udc_dwc3_trb_ctrl_in(const struct device *const dev, struct net_buf *const buf,
				 const uint32_t ctrl)
{
	const struct udc_dwc3_config *const cfg = dev->config;
	...
	LOG_DBG("TRB_CONTROL_IN size=%u ctrl=0x%x", buf->len, ctrl);
...
static void udc_dwc3_on_usb_reset(const struct device *const dev)
{
	...
	udc_ep_cancel_queued(dev, &cfg->ep_data_out[0].cfg);
	udc_ep_cancel_queued(dev, &cfg->ep_data_in[0].cfg);
	udc_ep_set_busy(&cfg->ep_data_out[0].cfg, false);
	udc_ep_set_busy(&cfg->ep_data_in[0].cfg, false);

	/* Prepare the upcoming next setup request */
	udc_dwc3_trb_ctrl_setup(dev);
...
static struct net_buf *udc_dwc3_buf_get(struct udc_dwc3_ep_data *const ep_data)
{
	...
	if (buf == NULL) {
		LOG_ERR("Failed to get a buffer from endpoint 0x%02x queue",
			ep_data->cfg.addr);
...
static struct net_buf *udc_dwc3_buf_peek(const struct device *const dev, const uint8_t ep_addr)
{
	...
	/* Used when submitting a buffer to the hardware: mark as busy */
...
static void udc_dwc3_on_ctrl_in(const struct device *const dev)
{
	...
	if (trb_trbctl == UDC_DWC3_TRB_CTRL_TRBCTL_CONTROL_STATUS_2 ||
		...
		/* End of transfer, prepare the next one */
		udc_dwc3_trb_ctrl_setup(dev);
...
static void udc_dwc3_on_ctrl_out(const struct device *const dev)
{
	...
	struct udc_dwc3_ep_data *const ep_data = &cfg->ep_data_out[0];
	uint32_t trb_trbctl = ep_data->trb_buf[0].ctrl & UDC_DWC3_TRB_CTRL_TRBCTL_MASK;
	uint32_t trb_status = ep_data->trb_buf[0].status;
	struct net_buf *buf;

	if (trb_trbctl == UDC_DWC3_TRB_CTRL_TRBCTL_CONTROL_SETUP) {
		LOG_HEXDUMP_DBG(cfg->setup_packet, sizeof(*cfg->setup_packet), "SETUP received");
		udc_setup_received(dev, cfg->setup_packet);
		 ...
		 * issue a reset.
		 */
		if (cfg->setup_packet->bmRequestType == USB_REQTYPE_TYPE_STANDARD &&
		    cfg->setup_packet->bRequest == USB_SREQ_SET_ADDRESS) {
			udc_dwc3_set_address(dev, cfg->setup_packet->wValue);
		}
...
static void udc_dwc3_irq_handler(void *ptr)
{
	...
	/* TODO: IRQ processing directly here */
...
static void udc_dwc3_next_ctrl_in(const struct device *const dev)
{
	...
	buf = udc_dwc3_buf_peek(dev, USB_CONTROL_EP_IN);
	if (buf == NULL) {
		return;
	}
...
static void udc_dwc3_next_ctrl_out(const struct device *const dev)
{
	...
	buf = udc_dwc3_buf_peek(dev, USB_CONTROL_EP_OUT);
	if (buf == NULL) {
		return;
	}
	...
	bi = udc_get_buf_info(buf);
	if (bi->setup) {
		LOG_DBG("Setup buffer queued by the stack");
	} else if (bi->data) {
		...
		LOG_ERR("Unknown buffer OUT type");
		udc_submit_ep_event(dev, buf, -EINVAL);
	}
}

static int udc_dwc3_ep_enqueue(const struct device *const dev, struct udc_ep_config *const ep_cfg,
			       struct net_buf *buf)
{
	...
	LOG_DBG("Queue ep 0x%02x buf %p (%s)",
		ep_data->cfg.addr, buf,
		bi->setup ? "setup" : bi->data ? "data" : bi->status ? "status" : "?");

	udc_buf_put(ep_cfg, buf);

	switch (ep_data->cfg.addr) {
	case USB_CONTROL_EP_IN:
		udc_dwc3_next_ctrl_in(dev);
		break;
	case USB_CONTROL_EP_OUT:
		udc_dwc3_next_ctrl_out(dev);
		break;
	default:
...
static int udc_dwc3_ep_set_halt(const struct device *const dev, struct udc_ep_config *const ep_cfg)
{
	const struct udc_dwc3_config *const cfg = dev->config;
	...
	case USB_CONTROL_EP_OUT:
		...
		/* Rearm for setup buffer reception */
		udc_dwc3_trb_ctrl_setup(dev);
		break;
	default:
		udc_dwc3_depcmd_end_xfer(dev, ep_data, UDC_DWC3_DEPCMD_HIPRI_FORCERM);
...
static int udc_dwc3_ep_clear_halt(const struct device *const dev, struct udc_ep_config *const ep_cfg)
{
	struct udc_dwc3_ep_data *const ep_data = CONTAINER_OF(ep_cfg, struct udc_dwc3_ep_data, cfg);
...
static void udc_dwc3_event_worker(struct k_work *const work)
{
	...
	struct udc_dwc3_data *const priv = CONTAINER_OF(dwork, struct udc_dwc3_data, dwork);
	const struct device *const dev = priv->dev;
	const struct udc_dwc3_config *const cfg = dev->config;
	mm_reg_t base = DEVICE_MMIO_NAMED_GET(dev, base);

	/* TODO: convert to IRQ-based driver */
	...
	k_work_reschedule(&priv->dwork, K_MSEC(100));
}
...
#define UDC_DWC3_DEVICE_DEFINE(n)						\
	...
	static struct udc_dwc3_ep_data udc_dwc3_ep_data_o##n			\
		[DT_INST_PROP(n, num_out_endpoints)];				\
										\
	static __nocache struct usb_setup_packet udc_dwc3_setup_packet_##n;	\
										\
	static const struct udc_dwc3_config udc_dwc3_config_##n = {		\
		...
		.setup_packet = &udc_dwc3_setup_packet_##n,			\
