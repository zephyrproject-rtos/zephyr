...
const static struct device *udc_ambiq_dev;
static bool udc_ambiq_ctrl_pending_in_ack;
static bool udc_ambiq_ctrl_pending_setup;
static uint8_t udc_ambiq_ctrl_pending_setup_buffer[8] = {0};
static bool udc_ambiq_ctrl_setup_recv_at_status_in;
...
static void udc_evt_callback(am_hal_usb_dev_event_e dev_state)
{
	const struct device *dev = udc_ambiq_dev;
	struct udc_ambiq_data *priv = udc_get_private(dev);
...
static void udc_ep0_setup_callback(uint8_t *usb_setup)
{
	 ...
	 */
	if (udc_ambiq_ctrl_pending_in_ack) {
		udc_ambiq_ctrl_pending_setup = true;
		memcpy(udc_ambiq_ctrl_pending_setup_buffer, usb_setup, 8);
		return;
	}
	 ...
	 */
	udc_ambiq_ctrl_setup_recv_at_status_in = udc_ctrl_stage_is_status_in(dev);
	memcpy(priv->setup, usb_setup, sizeof(struct usb_setup_packet));
	k_msgq_put(&drv_msgq, &evt, K_NO_WAIT);
}

static void udc_ep_xfer_complete_callback(uint8_t ep_addr, uint16_t xfer_len,
				...
				struct net_buf *buf)
{
	...
	if (ep_cfg->addr == USB_CONTROL_EP_IN && buf->len == 0 && udc_ambiq_ctrl_pending_in_ack) {
		udc_ambiq_ctrl_pending_in_ack = false;
		udc_ep_xfer_complete_callback(USB_CONTROL_EP_IN, 0, 0, NULL);
		return 0;
	}
	k_msgq_put(&drv_msgq, &evt, K_NO_WAIT);
...
static int usb_power_rails_set(bool on)
{
#define USB_DEV_NODE DT_NODELABEL(zephyr_udc0)
#if DT_NODE_HAS_STATUS(USB_DEV_NODE, okay) && DT_NODE_HAS_PROP(USB_DEV_NODE, vddusb33_gpios) &&    \
	DT_NODE_HAS_PROP(USB_DEV_NODE, vddusb0p9_gpios)
	int ret = 0;
	struct gpio_dt_spec usb_vddusb33_gpio = GPIO_DT_SPEC_GET(USB_DEV_NODE, vddusb33_gpios);
	struct gpio_dt_spec usb_vddusb0p9_gpio = GPIO_DT_SPEC_GET(USB_DEV_NODE, vddusb0p9_gpios);

	/* Enable USB IO */
	ret = gpio_pin_configure_dt(&usb_vddusb33_gpio, GPIO_OUTPUT);
	if (ret) {
		return ret;
	}
	ret = gpio_pin_configure_dt(&usb_vddusb0p9_gpio, GPIO_OUTPUT);
	if (ret) {
		return ret;
	}

	/* power rails set */
	ret = gpio_pin_set_dt(&usb_vddusb33_gpio, on == true ? 1 : 0);
	if (ret) {
		return ret;
	}
	ret = gpio_pin_set_dt(&usb_vddusb0p9_gpio, on == true ? 1 : 0);
	if (ret) {
		return ret;
	}
...
static int udc_ambiq_init(const struct device *dev)
{
	...
	am_hal_usb_register_dev_evt_callback(priv->usb_handle, udc_evt_callback);
	am_hal_usb_register_ep0_setup_received_callback(priv->usb_handle, udc_ep0_setup_callback);
	am_hal_usb_register_ep_xfer_complete_callback(priv->usb_handle,
						      udc_ep_xfer_complete_callback);
...
static void ambiq_handle_evt_setup(const struct device *dev)
{
	...
	if (udc_ctrl_stage_is_data_out(dev)) {
		...
		udc_ambiq_ctrl_pending_in_ack = true;
		if (err == -ENOMEM) {
...
{
	struct udc_data *data = udc_ambiq_dev->data;
	struct net_buf *buf;
	...
	if (cfg->addr == USB_CONTROL_EP_IN) {
		...
		if (udc_ambiq_ctrl_setup_recv_at_status_in && (buf->len == 0)) {
			udc_ambiq_ctrl_setup_recv_at_status_in = false;
			net_buf_unref(buf);
			return;
		}
		udc_ambiq_ctrl_setup_recv_at_status_in = false;
		 ...
		 */
		if (udc_ambiq_rx_status_in_completed && udc_ambiq_ctrl_pending_setup) {
			udc_ambiq_ctrl_pending_setup = false;
			udc_ep0_setup_callback(udc_ambiq_ctrl_pending_setup_buffer);
		}
	} else {
		udc_submit_ep_event(dev, buf, 0);
	}
