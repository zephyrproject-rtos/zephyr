...
#else
	err = nrf_clock_control_request(DEVICE_DT_GET_ONE(nordic_nrf_clock_xo24m), NULL, &pclk24m_cli);
#endif
	if (err < 0) {
...
#else
	err = nrf_clock_control_cancel_or_release(DEVICE_DT_GET_ONE(nordic_nrf_clock_xo24m), NULL,
											  &pclk24m_cli);
#endif
	if (err < 0) {
