...
#endif /* MBOX_ESP32_HPCORE */
#endif /* !MBOX_ESP32_LPCORE */
	} else {
#if defined(MBOX_ESP32_LPCORE)
		/*
		 * The LP core has no runtime handler registration: the ISR is
		 * ulp_lp_core_lp_pmu_intr_handler() above, which overrides the weak
		 * symbol the hal_espressif vector table already points at. All this
		 * needs is the device pointer that handler runs on.
		 *
		 * Was: ulp_lp_core_intr_set_handler(DT_INST_IRQ_BY_IDX(0, 1, source),
		 *                                   (void (*)(void *))esp32_mbox_isr,
		 *                                   (void *)dev);
		 */
		s_mbox_dev = dev;
...
/*
 * Only the HP image connects interrupts. An LP-core image has no ISR table at all
 * (GEN_ISR_TABLES=n) and its mbox node carries a single LP-core interrupt index, so
 * neither IRQ_CONNECT nor the "core 1" second entry exists there.
 */
#if !defined(MBOX_ESP32_LPCORE)
#define ESP32_MBOX_IRQ(idx)                                                                        \
	...
	{                                                                                          \
		/*                                                                                 \
		 * FROM_CPU interrupt for core 0: level-2 leaf under the INTMUX                    \
		 * aggregator. Connect at the multilevel-encoded IRQ; the SoC backend             \
		 * (z_soc_irq_*) decodes the (source, CPU line) pair and routes the matrix.        \
		 */                                                                                \
		IRQ_CONNECT(DT_INST_IRQN_BY_IDX(idx, 0), IRQ_DEFAULT_PRIORITY, esp32_mbox_isr,      \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN_BY_IDX(idx, 1), IRQ_DEFAULT_PRIORITY, esp32_mbox_isr,      \
			    DEVICE_DT_INST_GET(idx), ESP_INTR_FLAG_IRAM);                          \
