...
struct mbox_mhuv3_data {
	...
	/** Array of channels */
	struct mbox_mhuv3_channel *channels;
};
 ...
 *
 * @return -EIO to indicate an I/O error as transmission is not supported.
 */
							     ...
							     uint32_t doorbell)
{
	...
	return &data->channels[ext->base_ch_idx + channel * MHUV3_FLAG_BITS + doorbell];
}
...
static int mbox_mhuv3_dbe_channels_init(const struct device *dev, uint32_t *base_ch_idx)
{
	...
	*base_ch_idx += ext->num_chans * MHUV3_FLAG_BITS;
...
static struct mbox_mhuv3_channel *mbox_mhuv3_dbe_chan_from_comb_irq_get(const struct device *dev)
{
	...
	for (uint32_t i = 0; i < MHUV3_DBCH_CMB_INT_ST_REG_CNT; i++) {
		...
		return &data->channels[channel * MHUV3_FLAG_BITS + doorbell];
	}
 ...
 *
 * This function initializes the Doorbell Extension (DBE) by allocating memory for
 * the extension structure, reading the number of channels from the device's control
 * registers, and setting up necessary handlers. If the feature is not supported,
 * it returns early. It also updates the device's data with the number of channels
 * available and the extension's function pointers.
 *
 * @param dev Pointer to the device structure.
 *
...
static int mbox_mhuv3_dbe_init(const struct device *dev)
{
	const struct mbox_mhuv3_config *cfg = dev->config;
	struct mbox_mhuv3_data *data = dev->data;
	struct mbox_mhuv3_extension *ext;
	...
	ext->num_chans = read_bitmask32((mem_addr_t)&cfg->ctrl->dbch_cfg0, DBCH_CFG0_NUM_DBCH) + 1;
...
static int mbox_mhuv3_initialize_channels(const struct device *dev)
{
	...
	data->channels = k_malloc(sizeof(*data->channels) * data->num_chans);
	if (!data->channels) {
 ...
 *
 * @return Always returns -EIO as callback registration for a sender device is not supported.
 */
