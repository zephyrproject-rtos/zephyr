...
/* IPC_STRUCT registers, relative to a channel's base. */
...
/* IPC_INTR_STRUCT registers, relative to an interrupt structure's base. */
/*
...
static void ifx_mbox_handle_channel(const struct device *dev, uint32_t channel)
{
	...
	if ((sys_read32(intr + IFX_IPC_INTR) &
	     FIELD_PREP(IFX_IPC_INTR_NOTIFY_MASK, ch_mask)) == 0U) {
		return;
	}
...
/* Structure counts and availability bitmasks default to the maximum 16-entry
 * range when the IPC node omits them.
 */
#define IFX_MBOX_CH_COUNT(idx, n) \
	DT_PROP_OR(IFX_MBOX_IPC_NODE(idx, n), channel_count, 16)
#define IFX_MBOX_INTR_COUNT(idx, n) \
	DT_PROP_OR(IFX_MBOX_IPC_NODE(idx, n), interrupt_count, 16)
#define IFX_MBOX_CH_AVAIL(idx, n) \
	DT_PROP_OR(IFX_MBOX_IPC_NODE(idx, n), channel_mask, 0xFFFF)
#define IFX_MBOX_INTR_AVAIL(idx, n) \
	DT_PROP_OR(IFX_MBOX_IPC_NODE(idx, n), interrupt_mask, 0xFFFF)
