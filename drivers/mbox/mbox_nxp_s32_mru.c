...
static inline uintptr_t get_mbox_addr(const struct device *dev, uint32_t channel, uint32_t mbox)
{
	const struct nxp_s32_mru_config *cfg = dev->config;

	return ((uintptr_t)cfg->base + (channel + 1) * MRU_CHANNEL_OFFSET + mbox * MRU_MBOX_SIZE);
}

static int nxp_s32_mru_send(const struct device *dev, uint32_t channel, const struct mbox_msg *msg)
{
	const struct nxp_s32_mru_config *cfg = dev->config;
...
static int nxp_s32_mru_set_enabled(const struct device *dev, uint32_t channel, bool enable)
{
	struct nxp_s32_mru_data *data = dev->data;
	const struct nxp_s32_mru_config *cfg = dev->config;
	...
	if (enable) {
		...
		*ch_cfg[channel].ChCFG0Add =
			RTU_MRU_CH_CFG0_IE(1) | RTU_MRU_CH_CFG0_MBE0(1) | RTU_MRU_CH_CFG0_CHE(1);
	} else {
		/*
		 * Disable interrupts and mailboxes on this channel, making
		 * the channel's registers not writable afterwards
		 */
		*ch_cfg[channel].ChCFG0Add = RTU_MRU_CH_CFG0_IE(0) | RTU_MRU_CH_CFG0_MBE0(0);
	}
	...
	static struct nxp_s32_mru_config nxp_s32_mru_##n##_config = {                              \
		...
		.hw_cfg = {                                                                        \
			...
			.ChannelCfg = COND_CODE_0(MRU_RX_CHANNELS(n),			\
				 ...
				 .NOTIFYAdd = {(const volatile uint32 *)&MRU_BASE(n)->NOTIFY[0],   \
					  ...
					  (NULL), (nxp_s32_mru_##n##_init_irq)), \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, nxp_s32_mru_init, NULL, &nxp_s32_mru_##n##_data,                  \
			      &nxp_s32_mru_##n##_config, POST_KERNEL, CONFIG_MBOX_INIT_PRIORITY,   \
