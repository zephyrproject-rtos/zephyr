...
static int rpi_pico_mbox_mtu_get(const struct device *dev)
{
	...
	return DT_INST_PROP(0, fifo_depth);
}
				...
				uint32_t channel, bool enable)
{
	...
	if (enable) {
		irq_enable(DT_INST_IRQ_BY_NAME(0, mbox0, irq));
	} else {
		irq_disable(DT_INST_IRQ_BY_NAME(0, mbox0, irq));
	}
...
static int rpi_pico_mbox_init(const struct device *dev)
{
	...
	IRQ_CONNECT(DT_INST_IRQ_BY_NAME(0, mbox0, irq),
		DT_INST_IRQ_BY_NAME(0, mbox0, priority),
