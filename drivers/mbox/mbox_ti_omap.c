...
#define DEV_REG_BASE(dev) \
	((struct omap_mailbox_regs *)DEVICE_MMIO_NAMED_GET(dev, reg_base))
...
static void omap_mailbox_isr(const struct device *dev)
{
	...
	uint32_t irq_enabled = regs->irq_regs[cfg->user_id].enable_set;
...
#define MAILBOX_INSTANCE_DEFINE(idx)						\
	...
	{									\
		...
		IRQ_CONNECT(DT_INST_IRQN(idx), DT_INST_IRQ(idx, priority),	\
			    omap_mailbox_isr, DEVICE_DT_INST_GET(idx), 0);	\
		return 0;							\
	}									\
	DEVICE_DT_INST_DEFINE(idx,						\
			      omap_mailbox_##idx##_init,			\
			      NULL,						\
			      &omap_mailbox_##idx##_data,			\
			      &omap_mailbox_##idx##_config,			\
			      PRE_KERNEL_2,					\
