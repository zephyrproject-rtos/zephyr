...
struct mbox_rz_mhu_data {
	struct rz_mhu_channel *channels;
	/* Per-channel callbacks, sized by the "channels-count" property */
	mbox_callback_t *cb;
	void **user_data;
...
{
	const struct rz_mhu_irq_ctx *ctx = context;
				   ...
				   bool enabled)
{
	...
	WRITE_BIT(data->enabled_channel_mask, channel_id, enabled);
...
#define MHU_RZ_INIT(inst)                                                                          \
	BUILD_ASSERT(DT_INST_PROP(inst, channels_count) ==                                         \
			     DT_INST_PROP_LEN(inst, interrupt_names),                              \
		     "channels-count must match the number of interrupt-names entries");           \
	static const mhu_ns_extended_cfg_t g_mhu_ns##inst##_cfg_extend = {                         \
		.p_reg = (void *)DT_INST_REG_ADDR(inst),                                           \
	};                                                                                         \
	static struct rz_mhu_channel rz_mhu_channels_##inst[] = {DT_INST_FOREACH_PROP_ELEM_VARGS(  \
		inst, interrupt_names, RZ_MHU_INSTANCES_BY_IDX, inst)};                            \
	static struct rz_mhu_irq_ctx rz_mhu_irq_ctx_##inst[] = {                                   \
		DT_INST_FOREACH_PROP_ELEM(inst, interrupt_names, RZ_MHU_IRQ_CTX_BY_IDX)};          \
	static mbox_callback_t mbox_rz_mhu_cb_##inst[DT_INST_PROP(inst, channels_count)];          \
	static void *mbox_rz_mhu_user_data_##inst[DT_INST_PROP(inst, channels_count)];             \
	...
	static struct mbox_rz_mhu_data mbox_rz_mhu_data_##inst = {                                 \
		...
		.cb = mbox_rz_mhu_cb_##inst,                                                       \
		.user_data = mbox_rz_mhu_user_data_##inst,                                         \
	};                                                                                         \
