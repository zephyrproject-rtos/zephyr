				 ...
				 uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	if (channel == RENESAS_RZ_PWM_GPT_IO_A) {
		pin = GPT_IO_PIN_GTIOCA;
		fsp_cfg_extend->gtioca.output_enabled = true;
	} else if (channel == RENESAS_RZ_PWM_GPT_IO_B) {
		pin = GPT_IO_PIN_GTIOCB;
		fsp_cfg_extend->gtiocb.output_enabled = true;
	} else {
		LOG_ERR("Valid only for RENESAS_RZ_PWM_GPT_IO_A and RENESAS_RZ_PWM_GPT_IO_B pins");
		return -EINVAL;
	}
					 ...
					 uint64_t *cycles)
{
	...
	if (!(channel == RENESAS_RZ_PWM_GPT_IO_A || channel == RENESAS_RZ_PWM_GPT_IO_B)) {
		LOG_ERR("Valid only for RENESAS_RZ_PWM_GPT_IO_A and RENESAS_RZ_PWM_GPT_IO_B pins");
		return -EINVAL;
	}
...
static void fsp_callback(timer_callback_args_t *p_args)
{
	...
	if (p_args->event == TIMER_EVENT_CAPTURE_A) {
		...
		if (data->capture.capture_channel == RENESAS_RZ_PWM_GPT_IO_A) {
			data->capture.callback(dev, GPT_IO_PIN_GTIOCA, 0, 0, -ECANCELED,
					       data->capture.user_data);
		} else if (data->capture.capture_channel == RENESAS_RZ_PWM_GPT_IO_B) {
