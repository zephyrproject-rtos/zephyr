...
static inline bool is_ifx_tcpwm_pwm_running(const struct ifx_tcpwm_pwm_config *config)
{
...
#else
	Cy_TCPWM_TriggerCaptureOrSwap_Single(config->reg_base, BIT(config->tcpwm_index));
#endif
}
...
#else
	Cy_TCPWM_TriggerStart_Single(config->reg_base, BIT(config->tcpwm_index));
#endif
}
				    ...
				    pwm_flags_t flags)
{
	...
	if (is_ifx_tcpwm_pwm_running(config)) {
