...
static const nrfx_pwm_t *pwm_instances[] = {
 ...
 * nrfx_pwm_ftpan_109_handler for all active PWM instances.
 */
void CONCAT(nrfx_egu_, idx, _irq_handler)(void)
{
	for (int i = 0; i < ARRAY_SIZE(pwm_instances); i++) {
		nrfx_pwm_ftpan_109_handler(pwm_instances[i]);
	}
}
...
#define _EGU_IRQ_CONNECT(idx)				       \
	IRQ_CONNECT(DT_IRQN(DT_NODELABEL(egu##idx)),	       \
		    ...
		    NULL, nrfx_egu_##idx##_irq_handler, 0)
#else
