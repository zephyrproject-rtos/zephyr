			    ...
			    pwm_flags_t flags)
{
	 ...
	 */
	const struct pwm_nrfx_config *config = dev->fixed->config_info;
	struct pwm_nrfx_data *data = dev->fixed->driver_data;
	uint8_t channel;
	bool was_stopped;
...
static int pwm_nrfx_init(struct device *dev)
{
	...
	if (result != NRFX_SUCCESS) {
		LOG_ERR("Failed to initialize device: %s", dev->fixed->name);
		return -EBUSY;
	}
