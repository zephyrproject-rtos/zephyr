...
static int pwm_it51xxx_get_cycles_per_sec(const struct device *dev,
					  uint32_t channel, uint64_t *cycles)
{
	 ...
	 * the prescaler clock source (cycles) from 9.2MHz to 32.768kHz. In order
...
static int pwm_it51xxx_set_cycles(const struct device *dev,
...
#define PWM_IT51XXX_INIT(inst)								\
	...
	static const struct pwm_it51xxx_cfg pwm_it51xxx_cfg_##inst = {			\
		...
		.channel = DT_PROP(DT_INST(inst, ite_it51xxx_pwm), channel),		\
	...
	DEVICE_DT_INST_DEFINE(inst,							\
			      &pwm_it51xxx_init,					\
			      NULL,							\
			      &pwm_it51xxx_data_##inst,					\
			      &pwm_it51xxx_cfg_##inst,					\
			      PRE_KERNEL_1,						\
