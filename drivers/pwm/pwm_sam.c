...
#define DEV_CFG(dev) \
	((const struct sam_pwm_config * const)(dev)->fixed->config_info)

static int sam_pwm_get_cycles_per_sec(struct device *dev, uint32_t pwm,
				      uint64_t *cycles)
{
...
static int sam_pwm_pin_set(struct device *dev, uint32_t ch,
