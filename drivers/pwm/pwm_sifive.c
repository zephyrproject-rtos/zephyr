...
static int pwm_sifive_set_cycles(const struct device *dev, uint32_t channel, uint32_t period_cycles,
				 uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	sys_set_mask(PWM_REG(config, REG_PWMCFG), SF_PWMSCALEMASK, SF_PWMSCALE(pwmscale));
	...
	sys_write32((pulse_cycles >> pwmscale), PWM_REG(config, REG_PWMCMP(channel)));

	LOG_DBG("channel: %d, pwmscale: %d, pwmcmp0: %d, pwmcmp%d: %d", channel, pwmscale,
		(period_cycles >> pwmscale), channel, (pulse_cycles >> pwmscale));

	return 0;
}

static int pwm_sifive_get_cycles_per_sec(const struct device *dev, uint32_t channel,
					 uint64_t *cycles)
{
...
#define PWM_SIFIVE_INIT(n)                                                                         \
	...
	DEVICE_DT_INST_DEFINE(n, pwm_sifive_init, NULL, &pwm_sifive_data_##n, &pwm_sifive_cfg_##n, \
			      POST_KERNEL, CONFIG_PWM_INIT_PRIORITY, &pwm_sifive_api);
