	...
	for (ch = 0; ch < data->port_count; ch++) {
		...
		if (data->capture[ch].skip_irq < SKIP_IRQ_NUM - 1) {
			...
			if (data->capture[ch].CC_polarity) {
				RTIM_CCxPolarityConfig(config->pwm_timer,
						       TIM_CCPolarity_High, ch);
			} else {
				RTIM_CCxPolarityConfig(config->pwm_timer,
						       TIM_CCPolarity_Low, ch);
			}
			...
			if (data->capture[ch].callback) {
				data->capture[ch].callback(dev, ch,
							   data->capture[ch].period,
...
int pwm_ameba_init(const struct device *dev)
{
	...
	RTIM_TimeBaseInit(config->pwm_timer, &TIM_InitStruct, config->irq_source,
			  NULL, (uint32_t)NULL);
#ifdef CONFIG_PWM_CAPTURE
