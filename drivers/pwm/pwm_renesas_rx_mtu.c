...
static int pwm_rx_set_counter_clear(const struct device *dev, int counter_clear_channel)
{
	const struct pwm_renesas_rx_config *config = dev->config;
...
#ifdef CONFIG_PWM_CAPTURE
static void mtu_rx_set_tior(const struct device *dev, uint32_t channel, uint8_t state)
{
	const struct pwm_renesas_rx_config *config = dev->config;
...
static void mtu_set_nfcr(const struct device *dev, uint8_t channel)
{
	const struct pwm_renesas_rx_config *config = dev->config;
...
#ifdef CONFIG_PWM_CAPTURE
static uint16_t mtu_get_counter(const struct device *dev, uint8_t channel)
{
	const struct pwm_renesas_rx_config *config = dev->config;

	return config->reg.tgr[channel];
}
#endif /* CONFIG_PWM_CAPTURE */
					    ...
					    void *user_data)
{
	...
	*config->reg.tmdr = TMDR_MD_PWM_NORMAL_MODE;
	...
	/* Set noise filter */
	mtu_set_nfcr(dev, channel);

	mtu_rx_set_tior(dev, channel, state);
...
{
	struct pwm_renesas_rx_data *data = dev->data;
	uint16_t counter = mtu_get_counter(dev, data->capture.channel);
