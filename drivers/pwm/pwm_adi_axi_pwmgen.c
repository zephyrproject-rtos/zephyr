...
static inline void pwmgen_write(const struct device *dev, uint32_t reg,
				uint32_t val)
{
...
static int pwmgen_set_cycles(const struct device *dev, uint32_t channel,
...
static int pwmgen_get_cycles_per_sec(const struct device *dev,
...
static int pwmgen_init(const struct device *dev)
{
	...
	if (scratch != AXI_PWMGEN_SCRATCH_TEST) {
		LOG_ERR("scratchpad mismatch: wrote 0x%08x, read 0x%08x",
			AXI_PWMGEN_SCRATCH_TEST, scratch);
		return -EIO;
	}

	LOG_INF("AXI PWMGEN v%d.%d.%c — %u channel(s), ref_clk=%u Hz",
		version >> 16, (version >> 8) & 0xff, version & 0xff,
		data->npwm, cfg->ref_clock_hz);
