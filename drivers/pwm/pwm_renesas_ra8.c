...
static const char *const ra8_pwm_errmsg[] = {
	[FSP_ERR_ASSERTION] = "[FSP_ERR_ASSERTION] A required input pointer is NULL "
			      "OR the source divider is invalid",
	[FSP_ERR_ALREADY_OPEN] = "[FSP_ERR_ALREADY_OPEN] Module is already open",
	[FSP_ERR_IRQ_BSP_DISABLED] =
		"[FSP_ERR_IRQ_BSP_DISABLED] ISR must be enabled to use callback",
	[FSP_ERR_INVALID_MODE] =
		"[FSP_ERR_INVALID_MODE] Triangle wave PWM is only supported if "
		"GPT_CFG_OUTPUT_SUPPORT_ENABLE is 2 "
		"OR Selected channel does not support external count sources "
		"OR External and event count sources not are available in this mode",
	[FSP_ERR_NOT_OPEN] = "[FSP_ERR_NOT_OPEN] The instance is not opened",
	[FSP_ERR_INVALID_ARGUMENT] = "[FSP_ERR_INVALID_ARGUMENT] Duty cycle is larger than period",
	[FSP_ERR_UNSUPPORTED] = "[FSP_ERR_UNSUPPORTED] PLL GPT_CFG_OUTPUT_SUPPORT_ENABLE is 0"};

#define ZEPHYR_ERROR_LOG(err)                                                                      \
	{                                                                                          \
		if (err == FSP_SUCCESS) {                                                          \
			(void)0;                                                                   \
		} else {                                                                           \
			LOG_ERR("%s: %s", __func__, ra8_pwm_errmsg[err]);                          \
		}                                                                                  \
	}
...
static uint32_t pwm_ra8_gtior_calculate(timer_cfg_t const *const p_cfg,
					gpt_pin_level_t const stop_level)
{
	...
	if (TIMER_MODE_PWM == p_cfg->mode) {
		...
		/* In one-shot mode, the output pin goes high after the first compare match (one
		 * cycle after the timer starts counting).
		 */
	...
	if (0 == p_extend->gtior_setting.gtior) {
		...
		if (p_extend->gtioca.output_enabled) {
			uint32_t gtioca_gtior =
				pwm_ra8_gtior_calculate(p_cfg, p_extend->gtioca.stop_level);
		...
		if (p_extend->gtiocb.output_enabled) {
			uint32_t gtiocb_gtior =
				pwm_ra8_gtior_calculate(p_cfg, p_extend->gtiocb.stop_level);
				     ...
				     pwm_capture_callback_handler_t cb, void *user_data)
{
	...
	if (data->capture.continuous == true) {
...
static void fsp_callback(timer_callback_args_t *p_args)
{
	...
	R_GPT_InfoGet(&data->fsp_ctrl, &info);
	...
	if (TIMER_EVENT_CAPTURE_A == p_args->event) {
		if (p_args->capture != 0U) {
			if (true == data->capture.is_pulse_capture) {
				data->capture.pulse =
