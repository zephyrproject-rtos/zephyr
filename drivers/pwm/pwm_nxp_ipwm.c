...
static void nxp_ipwm_apply_config(const struct device *dev, pwm_flags_t flags)
{
	...
	/*
	 * Optionally keep the counter running in wait mode. The idle thread executes WFI as
	 * soon as no thread is runnable, which would otherwise gate the counter
	 * and stop the output whenever the system goes idle.
	 */
	pwm_config.enableWaitMode = config->run_in_wait;
...
static int nxp_ipwm_init(const struct device *dev)
{
	...
	/* Ungate the peripheral clock before touching any PWM register. */
	err = clock_control_on(config->clock_dev, config->clock_subsys);
	if (err) {
		LOG_ERR("%s: cannot ungate the PWM clock (%d)", dev->name, err);
		return err;
	}
...
#define NXP_IPWM_INIT(n)								\
	...
	static const struct nxp_ipwm_config nxp_ipwm_config_##n = {			\
		...
		.run_in_wait = DT_INST_PROP(n, run_in_wait),				\
	};										\
											\
