...
struct pwm_nxp_emios_config {
	...
	const struct pwm_nxp_emios_cnt_bus_uc_cfg *cnt_bus_uc_cfg;
	const struct pwm_nxp_emios_pwm_uc_cfg *pwm_uc_cfg;
					  ...
					  pwm_flags_t flags)
{
	...
	/*
	 * Lock to protect concurrent access to the shared counter bus channel.
	 * Multiple PWM channels may share the same counter bus (e.g. BUS_A),
	 * so cnt_ch_data->ch_period and UC[bus_id] registers must be updated
	 * atomically with respect to other channels on the same bus.
	 */
