...
static int mcux_tpm_set_cycles(const struct device *dev, uint32_t channel, uint32_t period_cycles,
...
static int mcux_tpm_get_cycles_per_sec(const struct device *dev, uint32_t channel, uint64_t *cycles)
{
	const struct mcux_tpm_config *config = dev->config;
	struct mcux_tpm_data *data = dev->data;
...
#define TPM_DEVICE(n)                                                                             \
	...
	DEVICE_DT_INST_DEFINE(n, &mcux_tpm_init, NULL, &mcux_tpm_data_##n, &mcux_tpm_config_##n,  \
			      POST_KERNEL, CONFIG_PWM_INIT_PRIORITY, &mcux_tpm_driver_api);
