...
#include <zephyr/drivers/misc/renesas_elc/renesas_elc.h>
#include "r_gpt.h"
#include "r_gpt_cfg.h"
...
#define PWM_RA8_INIT(index)                                                                        \
	...
	static struct pwm_renesas_ra_data pwm_renesas_ra_data_##index = {                          \
		...
		.start_renesas_elc =                                                               \
			{                                                                          \
				.dev = RENESAS_ELC_DT_SPEC_DEVICE_INST_GET_BY_NAME_OR_NULL(index,  \
											   start), \
				.peripheral = RENESAS_ELC_DT_SPEC_PERIPHERAL_INST_GET_BY_NAME_OR(  \
					index, start, 0),                                          \
				.event = RENESAS_ELC_DT_SPEC_EVENT_INST_GET_BY_NAME_OR(index,      \
										       start, 0),  \
			},                                                                         \
		.stop_renesas_elc =                                                                \
			{                                                                          \
