				       ...
				       uint32_t pulse_cycles, pwm_flags_t flags)
{
	...
	} else if (duty_cycle == period_cycles && pulse_cycles == 0) {
		polarity = FLEXIO_PWM_ACTIVE_HIGH;
		is_only_low_or_high = true;
	} else {
		/*
		 * The duty cycle is in between 0% and 100%
		 * so there is no need to force the line to be
		 * low or high, the polarity can stay as
		 * configured and the is_only_low_or_high
		 * can remain false.
		 */
	}
