					      ...
					      bool is_inverted)
{
	...
	if (is_channel_b) {
		new_reg = reg & ~RZA2M_GTUDDTYC_OBDTY_MASK;
		if (period_cycles == pulse_cycles) {
			/* 100% */
			if (is_inverted) {
				new_reg |= RZA2M_GTUDDTYC_OBDTY_0;
			} else {
				new_reg |= RZA2M_GTUDDTYC_OBDTY_100;
			}
		...
		new_reg = reg & ~RZA2M_GTUDDTYC_OADTY_MASK;
		if (period_cycles == pulse_cycles) {
			/* 100% */
			if (is_inverted) {
				new_reg |= RZA2M_GTUDDTYC_OADTY_0;
			} else {
				new_reg |= RZA2M_GTUDDTYC_OADTY_100;
			}
		} else if (pulse_cycles == 0) {
			/* 0% */
			if (is_inverted) {
					   ...
					   void *user_data)
{
	...
	renesas_rza2m_pwm_write_32(dev, RZA2M_GTPR_OFFSET, 0xffffffff);
	...
	if (capture->capture_channel == RZ_PWM_GPT_IO_B) {
		...
		capture->start_source = capture->inverted ? RZA2M_GT_BF : RZA2M_GT_BR;
		...
		if (capture->type_flag == PWM_CAPTURE_TYPE_PERIOD) {
			...
			capture->capture_source = capture->inverted ? RZA2M_GT_BR : RZA2M_GT_BF;
		} else if (capture->type_flag == PWM_CAPTURE_TYPE_BOTH) {
			capture->capture_source = RZA2M_GT_BR | RZA2M_GT_BF;
		} else {
			return -EINVAL;
		}
	} else {
		renesas_rza2m_pwm_write_32(dev, RZA2M_GTBER_OFFSET, RZA2M_GTBER_CCRA_1_BUF);
		capture->start_source = capture->inverted ? RZA2M_GT_AF : RZA2M_GT_AR;

		if (capture->type_flag == PWM_CAPTURE_TYPE_PERIOD) {
			capture->capture_source = capture->start_source;
		} else if (capture->type_flag == PWM_CAPTURE_TYPE_PULSE) {
			capture->capture_source = capture->inverted ? RZA2M_GT_AR : RZA2M_GT_AF;
		} else if (capture->type_flag == PWM_CAPTURE_TYPE_BOTH) {
			capture->capture_source = RZA2M_GT_AR | RZA2M_GT_AF;
		} else {
			return -EINVAL;
		}
	}
...
static inline void pwm_rza2m_gpt_cmn_isr(const struct device *dev, enum rza2m_gpt_event event)
{
	struct pwm_rza2m_data *data = dev->data;
	struct pwm_rza2m_gpt_capture_data *capture = &data->capture;
	uint32_t reg;
	...
	if (event == RZA2M_GPT_EVENT_CAPTURE_A) {
		reg = renesas_rza2m_pwm_read_32(dev, RZA2M_GTCCRA_OFFSET);
		if (reg == 0) {
			return;
		}
		...
		if (capture->type_flag == PWM_CAPTURE_TYPE_BOTH) {
			...
			if (capture->capture_both_event_count ==
				...
				capture->callback(dev, RZ_PWM_GPT_IO_A, capture->period,
						  capture->pulse, 0, capture->user_data);
			}
		} else if (capture->type_flag == PWM_CAPTURE_TYPE_PULSE) {
			capture->pulse = (capture->overflows * period_cyc) + reg;
			capture->callback(dev, RZ_PWM_GPT_IO_A, 0, capture->pulse, 0,
					  capture->user_data);
		} else {
			capture->period = (capture->overflows * period_cyc) + reg;
			capture->callback(dev, RZ_PWM_GPT_IO_A, capture->period, 0, 0,
					  capture->user_data);
		}

		data->capture.overflows = 0;

		if (!capture->continuous) {
			pwm_rza2m_gpt_disable_capture(dev, RZ_PWM_GPT_IO_A);
		}
	} else if (event == RZA2M_GPT_EVENT_CAPTURE_B) {
		reg = renesas_rza2m_pwm_read_32(dev, RZA2M_GTCCRB_OFFSET);
		if (reg == 0) {
			return;
		}
		...
		if (capture->type_flag == PWM_CAPTURE_TYPE_BOTH) {
			...
			if (capture->capture_both_event_count ==
				...
				data->capture.capture_both_event_count = 0;
				capture->period = (capture->overflows * period_cyc) + reg;
				capture->callback(dev, RZ_PWM_GPT_IO_B, capture->period,
						  capture->pulse, 0, capture->user_data);
			}
		} else if (capture->type_flag == PWM_CAPTURE_TYPE_PULSE) {
			capture->pulse = (capture->overflows * period_cyc) + reg;
			capture->callback(dev, RZ_PWM_GPT_IO_B, 0, capture->pulse, 0,
					  capture->user_data);
		} else {
			capture->period = (capture->overflows * period_cyc) + reg;
			capture->callback(dev, RZ_PWM_GPT_IO_B, capture->period, 0, 0,
					  capture->user_data);
		}
		...
		if (!capture->continuous) {
			pwm_rza2m_gpt_disable_capture(dev, RZ_PWM_GPT_IO_B);
		}
	} else if (event == RZA2M_GPT_EVENT_CYCLE_END) {
		capture->overflows++;
	} else {
		capture->callback(dev, capture->capture_channel, 0, 0, -ECANCELED,
				  capture->user_data);
	}
}
...
{
	pwm_rza2m_gpt_cmn_isr(dev, RZA2M_GPT_EVENT_CAPTURE_A);
}

static void pwm_rza2m_gpt_ccmpb_isr(const struct device *dev)
{
	pwm_rza2m_gpt_cmn_isr(dev, RZA2M_GPT_EVENT_CAPTURE_B);
}

static void pwm_rza2m_gpt_ovf_isr(const struct device *dev)
{
	pwm_rza2m_gpt_cmn_isr(dev, RZA2M_GPT_EVENT_CYCLE_END);
}
