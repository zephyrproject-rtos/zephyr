...
static const struct gpio_kscan_cfg gpio_kscan_it8xxx2_cfg_##inst = {      \
	...
	.reg_ksi_kso_goen = (uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 0),  \
	.reg_ksi_kso_gctrl = (uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 1), \
	.reg_ksi_kso_gdat = (uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 2),  \
	.reg_ksi_kso_gdmr = (uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 3),  \
	.reg_ksi_kso_gpod = (uint8_t *)DT_INST_REG_ADDR_BY_IDX(inst, 4),  \
};                                                                        \
								          \
