...
static int gpio_esp32_config(struct device *dev,
			     ...
			     gpio_flags_t flags)
{
	struct gpio_esp32_data *data = dev->fixed->driver_data;
...
{
	struct gpio_esp32_data *data = port->fixed->driver_data;
...
{
	struct gpio_esp32_data *data = device->fixed->driver_data;
