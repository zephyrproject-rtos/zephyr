...
#define ESP_SOC_GPIO_INIT(_id)                                                                     \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_IRQ_BY_IDX(DT_NODELABEL(gpio##_id), 0, irq),                        \
			    DT_IRQ_BY_IDX(DT_NODELABEL(gpio##_id), 0, priority), gpio_esp32_isr,   \
			    DEVICE_DT_INST_GET(_id),                                               \
			    DT_IRQ_BY_IDX(DT_NODELABEL(gpio##_id), 0, flags));                     \
		irq_matrix_enable(DT_IRQ_BY_IDX(DT_NODELABEL(gpio##_id), 0, irq),                  \
				  DT_IRQ_BY_IDX(DT_NODELABEL(gpio##_id), 0, source));              \
	}                                                                                          \
                                                                                                   \
