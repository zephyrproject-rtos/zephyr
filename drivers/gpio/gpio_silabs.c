...
#define GPIO_CONTROLLER_INIT(n)                                                                    \
	static void gpio_silabs_irq_connect_##n(const struct device *dev)                          \
	{                                                                                          \
		IRQ_CONNECT(GPIO_EVEN_IRQn,                                                        \
			    DT_IRQ_BY_NAME(DT_INST(n, silabs_gpio), gpio_even, priority),          \
			    gpio_silabs_common_isr, DEVICE_DT_GET(DT_INST(n, silabs_gpio)), 0);    \
		IRQ_CONNECT(GPIO_ODD_IRQn,                                                         \
			    DT_IRQ_BY_NAME(DT_INST(n, silabs_gpio), gpio_odd, priority),           \
			    gpio_silabs_common_isr, DEVICE_DT_GET(DT_INST(n, silabs_gpio)), 0);    \
		irq_enable(GPIO_EVEN_IRQn);                                                        \
		irq_enable(GPIO_ODD_IRQn);                                                         \
	}                                                                                          \
	static const struct gpio_silabs_common_config gpio_silabs_common_config_##n = {            \
		.irq_connect = gpio_silabs_irq_connect_##n,                                        \
		.clock = DEVICE_DT_GET(DT_CLOCKS_CTLR(DT_INST_PARENT(n))),                         \
		.clock_cfg = SILABS_DT_CLOCK_CFG(DT_INST_PARENT(n)),                               \
	};                                                                                         \
	DEVICE_DT_DEFINE(DT_INST(n, silabs_gpio), gpio_silabs_common_init, NULL,                   \
			 &gpio_silabs_common_data_inst, &gpio_silabs_common_config_##n,            \
