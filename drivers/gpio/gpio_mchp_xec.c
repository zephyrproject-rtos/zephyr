...
{
	const struct gpio_xec_config *config = dev->fixed->config_info;
...
{
	const struct gpio_xec_config *config = dev->fixed->config_info;
	struct gpio_xec_data *drv_data = dev->fixed->driver_data;
	__IO uint32_t *current_pcr1;
	uint32_t pcr1 = 0U;
	uint32_t mask = 0U;
...
{
	struct gpio_xec_data *data = dev->fixed->driver_data;
