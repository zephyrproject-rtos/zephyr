...
struct gpio_rz_config {
	...
	const struct gpio_rz_irq_info *irq_info;
	const uint8_t irq_info_size;
};
						 ...
						 gpio_flags_t *flags)
{
	...
	/* Covert the pin config to flags */
...
static int gpio_rz_int_disable(const struct gpio_rz_irq_info *irq_info)
{
#if defined(CONFIG_RENESAS_RZ_TINT)
...
static int gpio_rz_int_enable(const struct gpio_rz_irq_info *irq_info, uint8_t irq_type)
{
	int ret = 0;
...
static const struct gpio_rz_irq_info *gpio_rz_get_irq_info(const struct device *dev, gpio_pin_t pin)
{
	const struct gpio_rz_config *config = dev->config;
	const struct gpio_rz_irq_info *irq_info = config->irq_info;
					   ...
					   enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	const struct gpio_rz_irq_info *irq_info = gpio_rz_get_irq_info(dev, pin);
