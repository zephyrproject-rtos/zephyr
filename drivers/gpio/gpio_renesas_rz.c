			       ...
			       uint8_t int_num, gpio_pin_t pin)
{
	...
	GPIO_RZ_IOPORT_TINT_SELECT_SOURCE_REG_CLEAR(int_num);
	...
	*titsr &= ~(0x3 << GPIO_RZ_TITSR_OFFSET(int_num));
	...
	if (data->irq_set_edge & BIT(int_num)) {
		GPIO_RZ_IOPORT_TINT_STATUS_REG_CLEAR(int_num);
			      ...
			      uint8_t int_num, uint8_t irq_type, gpio_pin_t pin)
{
	...
	*tssr |= (GPIO_RZ_TSSR_VAL(gpio_config->port_num, pin)) << GPIO_RZ_TSSR_OFFSET(int_num);
	...
	GPIO_RZ_IOPORT_TINT_SELECT_SOURCE_REG_CLEAR(int_num);
	GPIO_RZ_IOPORT_TINT_SELECT_SOURCE_REG_SET(int_num);
...
#if GPIO_RZ_IOPORT_TINT_SPURIOUS_HANDLE
	if (!(*GPIO_RZ_IOPORT_TINT_STATUS_REG_GET & BIT(irq))) {
		LOG_DEV_DBG(dev, "tint:%u spurious irq, status 0", irq);
		return;
	}
#endif /* GPIO_RZ_IOPORT_TINT_SPURIOUS_HANDLE */

	if (int_data->irq_set_edge & BIT(irq)) {
		GPIO_RZ_IOPORT_TINT_STATUS_REG_CLEAR(irq);
	}
