			       ...
			       gpio_pin_t pin, gpio_flags_t flags)
{
	...
	ret = mec_gpio_pin_num(config->port_num, pin, &pin_num);
	if (ret != MEC_RET_OK) {
		return -EINVAL;
	}

	ret = mec_gpio_port_pin_valid(config->port_num, pin);
	if (ret != MEC_RET_OK) {
		return -EINVAL;
	}
	...
	if (flags == GPIO_DISCONNECTED) {
		ret = mec_gpio_set_property(pin_num, MEC_GPIO_PWRGT_PROP_ID,
	...
	ret = mec_gpio_set_props(pin_num, cfg_props_init, ARRAY_SIZE(cfg_props_init));
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
	...
	if (flags & GPIO_OUTPUT) {
		...
		if (flags & GPIO_OUTPUT_INIT_HIGH) {
			...
			mec_gpio_pad_in(pin_num, &props[idx].val);
		}
		idx++;
	}
	...
	ret = mec_gpio_set_props(pin_num, props, idx);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}

	/* make output state in control read-only in control and read-write in parallel reg */
	ret = mec_gpio_set_property(pin_num, MEC_GPIO_OSEL_PROP_ID, MEC_GPIO_PROP_OSEL_PAROUT);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
...
static void gpio_mec5_intr_en(uint8_t port, gpio_pin_t pin, enum gpio_int_mode mode)
{
	...
	mec_gpio_port_pin_ia_enable(port, pin, en);
}
					     ...
					     enum gpio_int_trig trig)
{
	...
	mec_gpio_pin_num(config->port_num, pin, &pin_num);
	ret = mec_gpio_set_props(pin_num, icfg_props_init, ARRAY_SIZE(icfg_props_init));
	if (ret) {
		return -EIO;
	}

	idet_curr = MEC_GPIO_PROP_IDET_DIS;
	ret = mec_gpio_get_property(pin_num, MEC_GPIO_IDET_PROP_ID, &idet_curr);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
	...
	ret = mec_gpio_set_property(pin_num, MEC_GPIO_IDET_PROP_ID, idet);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}

	ret = mec_gpio_pin_ia_status_clr(pin_num);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
					 ...
					 uint32_t value)
{
	...
	int ret = mec_gpio_parout_port_mask(config->port_num, value, (const uint32_t)mask);
...
static int gpio_mec5_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	...
	int ret = mec_gpio_parout_port_set_bits(config->port_num, (const uint32_t)mask);
...
static int gpio_mec5_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	...
	int ret = mec_gpio_parout_port_mask(config->port_num, 0u, (const uint32_t)mask);
...
static int gpio_mec5_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	...
	if (mec_gpio_parout_port_xor(config->port_num, mask) != MEC_RET_OK) {
		return -EIO;
	}
...
static int gpio_mec5_port_get_raw(const struct device *dev, uint32_t *value)
{
	...
	if (mec_gpio_parin_port(config->port_num, value) != MEC_RET_OK) {
		return -EIO;
	}
				   ...
				   gpio_port_pins_t *inputs, gpio_port_pins_t *outputs)
{
	...
	ret = mec_gpio_port_valid_mask(config->port_num, &valid_msk);
	if (ret != MEC_RET_OK) {
		return -EIO;
	}
	...
	*outputs = 0u;
	for (uint8_t pin_pos = 0; pin_pos < 32; pin++) {
		...
		if ((map & BIT(pin_pos)) && (valid_msk & BIT(pin_pos))) {
			mec_gpio_pin_num(config->port_num, pin, &pin_num);
			mec_gpio_get_property(pin_num, MEC_GPIO_PWRGT_PROP_ID, &pwr_gate);
			mec_gpio_get_property(pin_num, MEC_GPIO_DIR_PROP_ID, &dir);
			mec_gpio_get_property(pin_num, MEC_GPIO_INPAD_DIS_PROP_ID, &in_pad_dis);
...
int gpio_mec5_get_config(const struct device *port, gpio_pin_t pin, gpio_flags_t *flags)
{
	...
	const struct gpio_mec5_config *config = port->config;
	int ret = mec_gpio_port_pin_valid(config->port_num, pin);

	if (ret != MEC_RET_OK) {
		return -EINVAL;
	}

	uint32_t pin_ctrl = mec_gpio_port_get_ctrl_nc(config->port, pin);
	uint32_t pin_flags = 0u;
	uint8_t prop = MEC_GPIO_PROP_DIR_IN;

	mec_gpio_get_ctrl_property(pin_ctrl, MEC_GPIO_DIR_PROP_ID, &prop);

	if (prop == MEC_GPIO_PROP_DIR_OUT) {
		pin_flags |= GPIO_OUTPUT;
		mec_gpio_get_ctrl_property(pin_ctrl, MEC_GPIO_CTRL_OUT_VAL_ID, &prop);
		if (prop != 0) {
		...
		mec_gpio_get_ctrl_property(pin_ctrl, MEC_GPIO_OBUFT_PROP_ID, &prop);
		if (prop == MEC_GPIO_PROP_OPEN_DRAIN) {
			pin_flags |= GPIO_OPEN_DRAIN;
		}
	} else {
		prop = MEC_GPIO_PROP_INPAD_DIS;
		mec_gpio_get_ctrl_property(pin_ctrl, MEC_GPIO_INPAD_DIS_PROP_ID, &prop);
...
static void gpio_mec5_port_isr(const struct device *dev)
{
	 ...
	 */
	mec_gpio_port_ia_result(config->port_num, &girq_result);

	/* Clear source register in aggregator before firing callbacks */
	mec_gpio_port_ia_status_clr_mask(config->port_num, girq_result);
...
#define MEC5_GPIO_PORT(n)						\
	...
	{								\
		...
		const struct gpio_mec5_config *config = dev->config;	\
									\
		mec_gpio_port_ia_ctrl(config->port_num, 1u);		\
									\
