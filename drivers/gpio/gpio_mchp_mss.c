...
#define DEV_GPIO_CFG(dev)  ((const struct mss_gpio_config *const)(dev)->config)
#define DEV_GPIO(dev)      ((volatile struct mss_gpio_t *)(DEV_GPIO_CFG(dev))->gpio_base_addr)
#define DEV_GPIO_DATA(dev) ((struct mss_gpio_data *)(dev)->data)

static int mss_gpio_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
...
static int mss_gpio_port_toggle_bits(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct mss_gpio_t *gpio = DEV_GPIO(dev);
...
static int mss_gpio_port_get_raw(const struct device *dev, gpio_port_value_t *value)
{
	volatile struct mss_gpio_t *gpio = DEV_GPIO(dev);
...
static int mss_gpio_port_set_masked_raw(const struct device *dev, gpio_port_pins_t mask,
...
static int mss_gpio_port_set_bits_raw(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct mss_gpio_t *gpio = DEV_GPIO(dev);
...
static int mss_gpio_port_clear_bits_raw(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct mss_gpio_t *gpio = DEV_GPIO(dev);
...
static int mss_gpio_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
...
static int mss_gpio_manage_callback(const struct device *dev, struct gpio_callback *callback,
				    bool set)
{
