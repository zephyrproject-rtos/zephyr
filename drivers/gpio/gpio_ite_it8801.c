...
void it8801_gpio_alert_handler(const struct device *dev)
{
	...
	/*
	 * When the irq_gpios of the mfd is triggered, if the passed dev
	 * does not match the dev stored in the data struct of GPIO,
	 * further execution will be halted.
	 */
	if (dev != data->gpio_dev) {
		return;
	}
