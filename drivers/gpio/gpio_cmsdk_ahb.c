...
static int gpio_cmsdk_ahb_port_get_raw(const struct device *dev, uint32_t *value)
{
...
static int gpio_cmsdk_ahb_port_set_masked_raw(const struct device *dev, uint32_t mask,
					      uint32_t value)
{
	const struct gpio_cmsdk_ahb_cfg *const cfg = dev->config;
...
static int gpio_cmsdk_ahb_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_cmsdk_ahb_cfg *const cfg = dev->config;
...
static int gpio_cmsdk_ahb_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_cmsdk_ahb_cfg *const cfg = dev->config;
...
static int gpio_cmsdk_ahb_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	const struct gpio_cmsdk_ahb_cfg *const cfg = dev->config;
...
static int cmsdk_ahb_gpio_config(const struct device *dev, uint32_t mask, gpio_flags_t flags)
{
	const struct gpio_cmsdk_ahb_cfg *const cfg = dev->config;
 ...
 */
static int gpio_cmsdk_ahb_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	return cmsdk_ahb_gpio_config(dev, BIT(pin), flags);
}

static int gpio_cmsdk_ahb_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
...
static int gpio_cmsdk_ahb_manage_callback(const struct device *dev, struct gpio_callback *callback,
					  bool set)
{
	struct gpio_cmsdk_ahb_dev_data *data = dev->data;
...
#define CMSDK_AHB_GPIO_DEVICE(n)                                                                   \
	...
	static struct gpio_cmsdk_ahb_dev_data gpio_cmsdk_port_##n##_data;                          \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, gpio_cmsdk_ahb_init, NULL, &gpio_cmsdk_port_##n##_data,           \
			      &gpio_cmsdk_port_##n##_config, PRE_KERNEL_1,                         \
			      CONFIG_GPIO_INIT_PRIORITY, &gpio_cmsdk_ahb_drv_api_funcs);           \
                                                                                                   \
	static void gpio_cmsdk_port_##n##_config_func(const struct device *dev)                    \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), gpio_cmsdk_ahb_isr,         \
