...
static int gpio_adp5585_port_read(const struct device *dev, gpio_port_value_t *value)
{
	...
	uint8_t gpi_status_buf[] = { ADP5585_GPI_STATUS_A,
			0x00, 0x00 };

	ret = i2c_write_read_dt(&parent_cfg->i2c_bus, gpi_status_buf, 1U,
			gpi_status_buf + 1, 2U);
	if (ret) {
		goto out;
	}
	input_data = sys_le16_to_cpu(*((uint16_t*)(gpi_status_buf + 1)));
	*value = input_data;
