...
static int pin_is_valid(gpio_pin_t pin, uint8_t num_pins)
{
	if (pin >= num_pins) {
		return -EINVAL;
	}
...
{
	const struct gpio_rts5912_config *config = (const struct gpio_rts5912_config *)port->config;
	volatile uint32_t *gcr = &(config->reg_base[pin]);

	int err = pin_is_valid(pin, config->num_pins);
	if (err) {
		return err;
	}
...
{
	const struct gpio_rts5912_config *config = (const struct gpio_rts5912_config *)port->config;
	struct gpio_rts5912_data *data = (struct gpio_rts5912_data *)port->data;
	volatile uint32_t *gcr = &(config->reg_base[pin]);
	uint32_t cfg_val = *gcr;
	int err = 0;

	err = pin_is_valid(pin, config->num_pins);
	if (err) {
		return err;
	}
					  ...
					  gpio_flags_t *flags)
{
	...
	if (*gcr & GPIO_GCR_DIR_Msk) {
		if (data->out_init & (0x1UL << pin)) {
			cfg_flag |= GPIO_OUTPUT_INIT_HIGH;
		} else {
			cfg_flag |= GPIO_OUTPUT_INIT_LOW;
		}
