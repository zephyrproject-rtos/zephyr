...
{
	volatile uint32_t *gcr = (volatile uint32_t *)RTS5912_GPIOA_REG_BASE;

	gcr[pin_num] &= ~GPIO_GCR_MFCTRL_Msk;
	gcr[pin_num] |= GPIO_GCR_INTSTS_Msk;
	gcr[pin_num] |= 0x1 << GPIO_GCR_INTCTRL_Pos;
	gcr[pin_num] |= GPIO_GCR_INTEN_Msk;
	gcr[pin_num] |= GPIO_GCR_INTSTS_Msk;
}
...
int gpio_rts5912_get_pin_num(const struct gpio_dt_spec *gpio)
{
	...
	volatile uint32_t gcr = (uint32_t)config->reg_base;
