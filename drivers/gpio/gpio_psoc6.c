...
static int gpio_psoc6_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (flags & GPIO_OUTPUT) {
		if (flags & GPIO_SINGLE_ENDED) {
			drv_mode = (flags & GPIO_LINE_OPEN_DRAIN) ? CY_GPIO_DM_OD_DRIVESLOW_IN_OFF
	...
	LOG_DBG("P: 0x%08x, Pin: %d, Mode: 0x%08x, Val: 0x%02x", (unsigned int)port, pin, drv_mode,
		pin_val);

	return 0;
}

static int gpio_psoc6_port_get_raw(const struct device *dev, uint32_t *value)
{
	const struct gpio_psoc6_config *const cfg = dev->config;
	GPIO_PRT_Type *const port = cfg->regs;
...
static int gpio_psoc6_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
	const struct gpio_psoc6_config *const cfg = dev->config;
	GPIO_PRT_Type *const port = cfg->regs;
...
static int gpio_psoc6_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_psoc6_config *const cfg = dev->config;
	GPIO_PRT_Type *const port = cfg->regs;
...
static int gpio_psoc6_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_psoc6_config *const cfg = dev->config;
	GPIO_PRT_Type *const port = cfg->regs;
...
static int gpio_psoc6_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	const struct gpio_psoc6_config *const cfg = dev->config;
	GPIO_PRT_Type *const port = cfg->regs;
...
static int gpio_psoc6_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
...
static int gpio_psoc6_manage_callback(const struct device *port, struct gpio_callback *callback,
				      bool set)
{
