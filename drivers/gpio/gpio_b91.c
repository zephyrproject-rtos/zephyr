...
#define IS_INST_IRQ_EN(inst)    (DT_INST_IRQ_HAS_IDX(inst, 0) != 0) && \
	(DT_INST_IRQ_HAS_IDX(inst, 1) == 0)
...
{
	gpio_irq_type_e irq = GET_IRQ_NUM(dev);
...
static inline void gpio_b91_irq_status_clr(gpio_irq_type_e irq)
{
...
static void gpio_b91_up_down_res_set(volatile struct gpio_b91_t *gpio,
				     gpio_pin_t pin,
				     gpio_b91_pull_type_e up_down_res)
{
	uint8_t val;
	uint8_t mask;
	uint8_t analog_reg;
					...
					gpio_flags_t flags)
{
	if ((flags & GPIO_PULL_UP) != 0) {
		/* b91 supports two types of pull-up resistors: 1M and 10,
		   however, Zephyr supports just only one 'pull-up' */
				  ...
				  gpio_flags_t flags)
{
	...
	/* Setting Drive Strength is not supported by Zephyr */
...
/* Initialize interrupt handler for IRQ */
static void irq_connect(void)
{
	/* Check that instance 0 (e.g. port A) has interrupt enabled */
    #if IS_INST_IRQ_EN(0)
	...
	/* Check that instance 1 (e.g. port B) has interrupt enabled */
    #if IS_INST_IRQ_EN(1)
	...
	/* Check that instance 2 (e.g. port C) has interrupt enabled */
    #if IS_INST_IRQ_EN(2)
	...
	/* Check that instance 3 (e.g. port D) has interrupt enabled */
    #if IS_INST_IRQ_EN(3)
	...
	/* Check that instance 4 (e.g. port E) has interrupt enabled */
    #if IS_INST_IRQ_EN(4)
