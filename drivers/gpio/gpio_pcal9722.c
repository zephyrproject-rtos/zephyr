...
static __maybe_unused int gpio_pcal9722_pin_interrupt_configure(const struct device *dev,
								gpio_pin_t pin,
...
#define GPIO_PCAL9722_INIT(n)                                                                      \
	static const struct pcal9722_config pcal9722_cfg_##n = {                                   \
		...
		.interrupt_enabled = DT_INST_NODE_HAS_PROP(n, irq_gpios),                          \
		.gpio_int = GPIO_DT_SPEC_INST_GET_OR(n, irq_gpios, {0}),                           \
