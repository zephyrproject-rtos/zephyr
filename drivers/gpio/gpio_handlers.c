...
static inline int z_vrfy_gpio_enable_callback(const struct device *port,
					      gpio_pin_t pin)
{
	Z_OOPS(Z_SYSCALL_DRIVER_GPIO(port, enable_callback));

	return z_impl_gpio_enable_callback((const struct device *)port, pin);
}
#include <syscalls/gpio_enable_callback_mrsh.c>

static inline int z_vrfy_gpio_disable_callback(const struct device *port,
					       gpio_pin_t pin)
{
	Z_OOPS(Z_SYSCALL_DRIVER_GPIO(port, disable_callback));

	return z_impl_gpio_disable_callback((const struct device *)port, pin);
}
