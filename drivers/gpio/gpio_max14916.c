...
static int max14916_pars_spi_diag(const struct device *dev, uint8_t *rx_diag_buff, uint8_t rw)
{
	...
	if (rx_diag_buff[0]) {
		...
		data->glob.interrupt.reg_bits.SHT_VDD_FLT = 0x1 & (rx_diag_buff[0] >> 5);
		data->glob.interrupt.reg_bits.OW_ON_FLT = 0x1 & (rx_diag_buff[0] >> 4);
		data->glob.interrupt.reg_bits.OW_OFF_FLT = 0x1 & (rx_diag_buff[0] >> 3);
		data->glob.interrupt.reg_bits.CURR_LIM = 0x1 & (rx_diag_buff[0] >> 2);
		data->glob.interrupt.reg_bits.OVER_LD_FLT = 0x1 & (rx_diag_buff[0] >> 1);

		if (0x1 & (rx_diag_buff[0] >> 0))
	...
	if (rw == MAX149x6_WRITE && (rx_diag_buff[1] & 0x0f)) {
		...
		LOG_ERR("[DIAG] Flt1[%x] Flt2[%x] Flt3[%x]"
			...
			0x1 & (rx_diag_buff[1] >> 0), 0x1 & (rx_diag_buff[1] >> 1),
			0x1 & (rx_diag_buff[1] >> 2), 0x1 & (rx_diag_buff[1] >> 3),
			0x1 & (rx_diag_buff[1] >> 4), 0x1 & (rx_diag_buff[1] >> 5),
			0x1 & (rx_diag_buff[1] >> 6), 0x1 & (rx_diag_buff[1] >> 7));
...
static int max14916_reg_trans_spi_diag(const struct device *dev, uint8_t addr,
							uint8_t tx, uint8_t rw)
{
...
#define max14916_reg_write(dev, addr, val)  max14916_reg_trans_spi_diag(dev, addr, val,\
										MAX149x6_WRITE)
...
static int gpio_max14916_diag_chan_get(const struct device *dev)
{
	...
	data->glob.interrupt.reg_raw = max149x6_reg_transceive(dev, MAX14916_INT_REG, 0,
								NULL, MAX149x6_READ);

	if (data->glob.interrupt.reg_raw) {
		if (data->glob.interrupt.reg_bits.OVER_LD_FLT) {
			data->chan.ovr_ld = max149x6_reg_transceive(dev,
						MAX14916_OVR_LD_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.CURR_LIM) {
			data->chan.curr_lim = max149x6_reg_transceive(dev,
						MAX14916_CURR_LIM_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.OW_OFF_FLT) {
			data->chan.ow_off = max149x6_reg_transceive(dev,
						MAX14916_OW_OFF_FLT_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.OW_ON_FLT) {
			data->chan.ow_on = max149x6_reg_transceive(dev,
						MAX14916_OW_ON_FLT_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.SHT_VDD_FLT) {
			data->chan.sht_vdd = max149x6_reg_transceive(dev,
						MAX14916_SHT_VDD_FLT_REG, 0, NULL, MAX149x6_READ);
		}

		if (data->glob.interrupt.reg_bits.SUPPLY_ERR) {
			data->glob.glob_err.reg_raw = max149x6_reg_transceive(dev,
						MAX14916_GLOB_ERR_REG, 0, NULL, MAX149x6_READ);
...
#define GPIO_MAX14906_DEVICE(id)								\
	static const struct max14916_config max14916_##id##_cfg = {				\
		...
		.config1.reg_bits.FLED_SET = IS_ENABLED(CONFIG_GPIO_MAX14916_FLED_SET),		\
		.config1.reg_bits.SLED_SET = IS_ENABLED(CONFIG_GPIO_MAX14916_SLED_SET),		\
		.config1.reg_bits.FLED_STRETCH = CONFIG_GPIO_MAX14916_FLED_STRETCH,		\
		.config1.reg_bits.FFILTER_EN = IS_ENABLED(CONFIG_GPIO_MAX14916_FFILTER_EN),	\
		.config1.reg_bits.FILTER_LONG = IS_ENABLED(CONFIG_GPIO_MAX14916_FILTER_LONG),	\
		.config1.reg_bits.FLATCH_EN = IS_ENABLED(CONFIG_GPIO_MAX14916_FLATCH_EN),	\
		.config1.reg_bits.LED_CURR_LIM = IS_ENABLED(CONFIG_GPIO_MAX14916_LED_CUR_LIM),	\
		.config2.reg_bits.VDD_ON_THR = IS_ENABLED(CONFIG_GPIO_MAX14916_VDD_ON_THR),	\
		.config2.reg_bits.SYNCH_WD_EN = IS_ENABLED(CONFIG_GPIO_MAX14916_SYNCH_WD_EN),	\
		.config2.reg_bits.SHT_VDD_THR = CONFIG_GPIO_MAX14916_SHT_VDD_THR,		\
		.config2.reg_bits.OW_OFF_CS = CONFIG_GPIO_MAX14916_OW_OFF_CS,			\
		.config2.reg_bits.WD_TO = CONFIG_GPIO_MAX14916_WD_TO,				\
