...
static int gpio_dw_port_set_bits_raw(const struct device *port, u32_t mask);
static int gpio_dw_port_clear_bits_raw(const struct device *port, u32_t mask);
...
static int gpio_dw_port_get_raw(const struct device *port, u32_t *value)
{
	struct gpio_dw_runtime *context = port->driver_data;
	u32_t base_addr = dw_base_to_block_base(context->base_addr);
	u32_t port_base_addr = context->base_addr;
...
static int gpio_dw_port_toggle_bits(const struct device *port, u32_t mask)
{
	struct gpio_dw_runtime *context = port->driver_data;
	u32_t base_addr = dw_base_to_block_base(context->base_addr);
	u32_t port_base_addr = context->base_addr;
...
static inline int gpio_dw_enable_callback(const struct device *port,
					  gpio_pin_t pin)
{
	struct gpio_dw_runtime *context = port->driver_data;
	u32_t base_addr = dw_base_to_block_base(context->base_addr);
...
static inline int gpio_dw_disable_callback(const struct device *port,
					   gpio_pin_t pin)
{
	struct gpio_dw_runtime *context = port->driver_data;
	u32_t base_addr = dw_base_to_block_base(context->base_addr);
...
static u32_t gpio_dw_get_power_state(const struct device *port)
{
	struct gpio_dw_runtime *context = port->driver_data;
...
*/
static int gpio_dw_device_ctrl(const struct device *port, u32_t ctrl_command,
