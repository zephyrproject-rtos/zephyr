...
static int max14906_pars_spi_diag(const struct device *dev, uint8_t *rx_diag_buff, uint8_t rw)
{
	...
	if (rx_diag_buff[0]) {
		...
		data->glob.interrupt.reg_bits.SHT_VDD_FAULT = 0x1 & (rx_diag_buff[0] >> 5);
		data->glob.interrupt.reg_bits.ABOVE_VDD_FAULT = 0x1 & (rx_diag_buff[0] >> 4);
		data->glob.interrupt.reg_bits.OW_OFF_FAULT = 0x1 & (rx_diag_buff[0] >> 3);
		data->glob.interrupt.reg_bits.CURR_LIM = 0x1 & (rx_diag_buff[0] >> 2);
		data->glob.interrupt.reg_bits.OVER_LD_FAULT = 0x1 & (rx_diag_buff[0] >> 1);

		uint8_t globlf = 0x1 & (rx_diag_buff[0] >> 0);
	...
	if (rw == MAX149x6_WRITE && (rx_diag_buff[1] & 0x0f)) {
		...
		LOG_ERR("[DIAG] Flt1[%x] Flt2[%x] Flt3[%x] Flt4[%x]", 0x1 & (rx_diag_buff[1] >> 0),
								      0x1 & (rx_diag_buff[1] >> 1),
								      0x1 & (rx_diag_buff[1] >> 2),
								      0x1 & (rx_diag_buff[1] >> 3));
...
static int max14906_reg_trans_spi_diag(const struct device *dev, uint8_t addr,
...
static int gpio_max14906_diag_chan_get(const struct device *dev)
{
	...
	data->glob.interrupt.reg_raw = max149x6_reg_transceive(dev, MAX14906_INT_REG,
	...
	if (data->glob.interrupt.reg_raw) {
		if (data->glob.interrupt.reg_bits.OVER_LD_FAULT ||
			...
			data->chan.ovr_ld.reg_raw = max149x6_reg_transceive(dev,
						MAX14906_OVR_LD_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.OW_OFF_FAULT ||
		    data->glob.interrupt.reg_bits.ABOVE_VDD_FAULT)  {
			data->chan.opn_wir.reg_raw = max149x6_reg_transceive(dev,
						MAX14906_OPN_WIR_FLT_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.SHT_VDD_FAULT) {
			data->chan.sht_vdd.reg_raw = max149x6_reg_transceive(dev,
						MAX14906_SHT_VDD_FLT_REG, 0, NULL, MAX149x6_READ);
		}
		if (data->glob.interrupt.reg_bits.DE_MAG_FAULT) {
			data->chan.doi_level.reg_raw = max149x6_reg_transceive(dev,
						MAX14906_DOILEVEL_REG, 0, NULL, MAX149x6_READ);
		...
		if (data->glob.interrupt.reg_bits.SUPPLY_ERR) {
			data->glob.glob_err.reg_raw = max149x6_reg_transceive(dev,
						MAX14906_GLOB_ERR_REG, 0, NULL, MAX149x6_READ);
	...
	ret = data->chan.doi_level.reg_raw |
...
#define GPIO_MAX14906_DEVICE(id)								\
	static const struct max14906_config max14906_##id##_cfg = {				\
		...
		.config1.reg_bits.FLED_SET = IS_ENABLED(CONFIG_GPIO_MAX14906_FLED_SET),		\
		.config1.reg_bits.SLED_SET = IS_ENABLED(CONFIG_GPIO_MAX14906_SLED_SET),		\
		.config1.reg_bits.FLED_STRETCH = CONFIG_GPIO_MAX14906_FLED_STRETCH,		\
		.config1.reg_bits.FFILTER_EN = IS_ENABLED(CONFIG_GPIO_MAX14906_FFILTER_EN),	\
		.config1.reg_bits.FILTER_LONG = IS_ENABLED(CONFIG_GPIO_MAX14906_FILTER_LONG),	\
		.config1.reg_bits.FLATCH_EN = IS_ENABLED(CONFIG_GPIO_MAX14906_FLATCH_EN),	\
		.config1.reg_bits.LED_CURR_LIM = IS_ENABLED(CONFIG_GPIO_MAX14906_LED_CUR_LIM),	\
		.config2.reg_bits.VDD_ON_THR = IS_ENABLED(CONFIG_GPIO_MAX14906_VDD_ON_THR),	\
		.config2.reg_bits.SYNCH_WD_EN = IS_ENABLED(CONFIG_GPIO_MAX14906_SYNCH_WD_EN),	\
		.config2.reg_bits.SHT_VDD_THR = CONFIG_GPIO_MAX14906_SHT_VDD_THR,		\
		.config2.reg_bits.OW_OFF_CS = CONFIG_GPIO_MAX14906_OW_OFF_CS,			\
		.config2.reg_bits.WD_TO = CONFIG_GPIO_MAX14906_WD_TO,				\
