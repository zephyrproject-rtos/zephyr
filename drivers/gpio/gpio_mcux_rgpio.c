...
#define DEV_CFG(_dev)  ((const struct mcux_rgpio_config *)(_dev)->config)
...
static int mcux_rgpio_configure(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
...
static int mcux_rgpio_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
	RGPIO_Type *base = (RGPIO_Type *)DEVICE_MMIO_NAMED_GET(dev, reg_base);
...
static int mcux_rgpio_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	RGPIO_Type *base = (RGPIO_Type *)DEVICE_MMIO_NAMED_GET(dev, reg_base);
...
static int mcux_rgpio_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	RGPIO_Type *base = (RGPIO_Type *)DEVICE_MMIO_NAMED_GET(dev, reg_base);
...
static int mcux_rgpio_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	RGPIO_Type *base = (RGPIO_Type *)DEVICE_MMIO_NAMED_GET(dev, reg_base);
...
static int mcux_rgpio_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
					      enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	} else if ((mode == GPIO_INT_MODE_EDGE) && (trig == GPIO_INT_TRIG_LOW)) {
...
static int mcux_rgpio_manage_callback(const struct device *dev, struct gpio_callback *callback,
				      bool set)
{
...
#define MCUX_RGPIO_PIN_INIT(n)                                                                \
	.pin_muxes = mcux_rgpio_pinmux_##n, .mux_count = DT_PROP_LEN(DT_DRV_INST(n), pinmux),

#define MCUX_RGPIO_IRQ_INIT(n, i)                                                              \
	do {                                                                                   \
		IRQ_CONNECT(DT_INST_IRQ_BY_IDX(n, i, irq), DT_INST_IRQ_BY_IDX(n, i, priority), \
			    mcux_rgpio_port_isr, DEVICE_DT_INST_GET(n), 0);                    \
                                                                                               \
...
#define MCUX_RGPIO_INIT(n)                                                                      \
	...
	static struct mcux_rgpio_data mcux_rgpio_##n##_data;                                    \
                                                                                                \
	DEVICE_DT_INST_DEFINE(n, mcux_rgpio_##n##_init, NULL, &mcux_rgpio_##n##_data,           \
			      &mcux_rgpio_##n##_config, POST_KERNEL, CONFIG_GPIO_INIT_PRIORITY, \
