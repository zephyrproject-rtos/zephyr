...
static u32_t get_port_pcr_irqc_value_from_flags(const struct device *dev,
						u32_t pin,
...
static int gpio_rv32m1_port_get_raw(const struct device *dev, u32_t *value)
{
	const struct gpio_rv32m1_config *config = dev->config_info;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_port_set_bits_raw(const struct device *dev, u32_t mask)
{
	const struct gpio_rv32m1_config *config = dev->config_info;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_port_toggle_bits(const struct device *dev, u32_t mask)
{
	const struct gpio_rv32m1_config *config = dev->config_info;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_enable_callback(const struct device *dev,
				       gpio_pin_t pin)
{
	struct gpio_rv32m1_data *data = dev->driver_data;
...
static int gpio_rv32m1_disable_callback(const struct device *dev,
					gpio_pin_t pin)
{
	struct gpio_rv32m1_data *data = dev->driver_data;
