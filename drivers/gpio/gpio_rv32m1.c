...
static int gpio_rv32m1_configure(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if ((flags & GPIO_INT_ENABLE) && ((config->flags & GPIO_INT_ENABLE) == 0U)) {
		return -ENOTSUP;
	}
...
static int gpio_rv32m1_port_set_masked_raw(const struct device *dev, uint32_t mask, uint32_t value)
{
	const struct gpio_rv32m1_config *config = dev->config;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_port_set_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_rv32m1_config *config = dev->config;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_port_clear_bits_raw(const struct device *dev, uint32_t mask)
{
	const struct gpio_rv32m1_config *config = dev->config;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_port_toggle_bits(const struct device *dev, uint32_t mask)
{
	const struct gpio_rv32m1_config *config = dev->config;
	GPIO_Type *gpio_base = config->gpio_base;
...
static int gpio_rv32m1_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
					       enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	const struct gpio_rv32m1_config *config = dev->config;
	PORT_Type *port_base = config->port_base;
...
static int gpio_rv32m1_manage_callback(const struct device *dev, struct gpio_callback *callback,
				       bool set)
{
