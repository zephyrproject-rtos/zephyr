...
static int gpio_npm6001_port_get_raw(const struct device *dev, uint32_t *value)
{
	...
	ret = i2c_write_read_dt(&config->bus, &reg, sizeof(reg), &val, sizeof(val));
	if (ret < 0) {
		return ret;
	}
...
static int gpio_npm6001_port_set_bits_raw(const struct device *dev, gpio_port_pins_t pins)
{
	const struct gpio_npm6001_config *config = dev->config;
...
static int gpio_npm6001_port_clear_bits_raw(const struct device *dev, gpio_port_pins_t pins)
{
	const struct gpio_npm6001_config *config = dev->config;
...
static inline int gpio_npm6001_configure(const struct device *dev, gpio_pin_t pin,
					 gpio_flags_t flags)
{
	const struct gpio_npm6001_config *config = dev->config;
	...
	if ((flags & GPIO_OUTPUT) != 0U) {
		...
		buf[1] |= NPM6001_CONF_DIRECTION_OUTPUT | NPM6001_CONF_INPUT_ENABLED;
		...
		if ((flags & NPM6001_GPIO_DRIVE_MSK) == NPM6001_GPIO_DRIVE_HIGH) {
		...
		if ((flags & GPIO_OUTPUT_INIT_HIGH) != 0U) {
			...
			ret = gpio_npm6001_port_set_bits_raw(dev, (gpio_port_pins_t)BIT(pin));
			if (ret < 0) {
				return ret;
			}
			...
			ret = gpio_npm6001_port_clear_bits_raw(dev, (gpio_port_pins_t)BIT(pin));
			if (ret < 0) {
				return ret;
			}
		}
		...
		if ((flags & NPM6001_GPIO_SENSE_MSK) == NPM6001_GPIO_SENSE_CMOS) {
...
static int gpio_npm6001_port_set_masked_raw(const struct device *dev, gpio_port_pins_t mask,
					    gpio_port_value_t value)
{
	...
	return gpio_npm6001_port_clear_bits_raw(dev, mask & (~value & NPM6001_PIN_MSK));
}

static int gpio_npm6001_port_toggle_bits(const struct device *dev, gpio_port_pins_t pins)
{
	uint32_t val;
	int ret;
	...
	return gpio_npm6001_port_set_masked_raw(dev, pins, ~val & NPM6001_PIN_MSK);
}
...
#define GPIO_NPM6001_DEFINE(n)                                                       \
	...
	static struct gpio_npm6001_data gpio_npm6001_data##n;                        \
                                                                                     \
	DEVICE_DT_INST_DEFINE(n, gpio_npm6001_init, NULL, &gpio_npm6001_data##n,     \
			      &gpio_npm6001_config##n, POST_KERNEL,                  \
			      CONFIG_GPIO_NPM6001_INIT_PRIORITY, &gpio_npm6001_api);
