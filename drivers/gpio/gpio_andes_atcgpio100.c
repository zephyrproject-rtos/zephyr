...
#define GPIO_BASE(dev) ((const struct gpio_atcgpio100_config *const)(dev)->config)->base
...
static int gpio_atcgpio100_config(const struct device *port, gpio_pin_t pin, gpio_flags_t flags)
{
...
static int gpio_atcgpio100_port_get_raw(const struct device *port, gpio_port_value_t *value)
{
	*value = sys_read32(GPIO_DIN(port));
	return 0;
}

static int gpio_atcgpio100_set_masked_raw(const struct device *port, gpio_port_pins_t mask,
...
static int gpio_atcgpio100_set_bits_raw(const struct device *port, gpio_port_pins_t pins)
{
	sys_write32(pins, GPIO_DSET(port));
	return 0;
}

static int gpio_atcgpio100_clear_bits_raw(const struct device *port, gpio_port_pins_t pins)
{
	sys_write32(pins, GPIO_DCLR(port));
	return 0;
}

static int gpio_atcgpio100_toggle_bits(const struct device *port, gpio_port_pins_t pins)
{
	struct gpio_atcgpio100_data *const data = port->data;
	uint32_t port_value;
...
static int gpio_atcgpio100_pin_interrupt_configure(const struct device *port, gpio_pin_t pin,
...
static int gpio_atcgpio100_port_get_dir(const struct device *port, gpio_port_pins_t map,
