				      ...
				      gpio_flags_t flags)
{
	...
	if ((flags & GPIO_OUTPUT) != 0U) {
		PCU_INOUT_e mode = (flags & GPIO_OPEN_DRAIN) != 0U ? PCU_INOUT_OUTPUT_OPEN_DRAIN
								    : PCU_INOUT_OUTPUT_PUSH_PULL;

		ret = HAL_PCU_SetInOutMode(port, (PCU_PIN_ID_e)pin, mode);
		if (ret != HAL_ERR_OK) {
			return -EINVAL;
		}
		...
		ret = HAL_PCU_SetInOutMode(port, (PCU_PIN_ID_e)pin, PCU_INOUT_INPUT);
		if (ret != HAL_ERR_OK) {
			return -EINVAL;
		}
	} else {
...
static int gpio_abov32_port_get_raw(const struct device *dev, gpio_port_value_t *value)
{
	...
	for (uint32_t pin = 0U; pin < PCU_PIN_ID_MAX; pin++) {
		...
		ret = HAL_PCU_GetInputValue(config->port_id, (PCU_PIN_ID_e)pin, &level);
		if (ret == HAL_ERR_OK && level == PCU_PORT_HIGH) {
