...
/* gpio group a, b, c, d, e, f, g, h, i, j, k ,l ,m, ksi, ksoh, and ksol */
#define GPIO_GROUP_COUNT 16
#define GPIO_GROUP_INDEX(label) DT_PROP(DT_NODELABEL(label), index)
...
static const struct gpio_ite_cfg gpio_ite_cfg_##inst = {           \
	...
	.index = DT_PROP(DT_INST(inst, ite_it8xxx2_gpio), index),  \
