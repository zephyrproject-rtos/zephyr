...
static int gpio_cc32xx_port_get_raw(const struct device *port, u32_t *value)
{
	const struct gpio_cc32xx_config *gpio_config = DEV_CFG(port);
	unsigned long port_base = gpio_config->port_base;
...
static int gpio_cc32xx_port_toggle_bits(const struct device *port, u32_t mask)
{
	const struct gpio_cc32xx_config *gpio_config = DEV_CFG(port);
	unsigned long port_base = gpio_config->port_base;
	long value;
...
static int gpio_cc32xx_enable_callback(const struct device *dev,
				       gpio_pin_t pin)
{
	struct gpio_cc32xx_data *data = DEV_DATA(dev);
...
static int gpio_cc32xx_disable_callback(const struct device *dev,
					gpio_pin_t pin)
{
	struct gpio_cc32xx_data *data = DEV_DATA(dev);
...
static void gpio_cc32xx_port_isr(void *arg)
{
	...
	gpio_fire_callbacks(&data->callbacks, (const struct device *)dev,
			     enabled_int);
}
