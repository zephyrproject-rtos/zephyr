...
static int gpio_ite_configure(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if (gpio_config->kbs_ctrl) {
		...
		if (flags & GPIO_PULL_UP) {
			ksxgctrlr = (ksxgctrlr | KSIX_KSOX_GPIO_PULLUP) & ~KSIX_KSOX_GPIO_PULLDOWN;
		} else if (flags & GPIO_PULL_DOWN) {
			ksxgctrlr = (ksxgctrlr | KSIX_KSOX_GPIO_PULLDOWN) & ~KSIX_KSOX_GPIO_PULLUP;
		} else {
			/* No pull up/down */
...
{
	struct gpio_ite_data *const data =
		CONTAINER_OF(work, struct gpio_ite_data, interrupt_worker);
	gpio_port_value_t value;
...
static int gpio_ite_init(const struct device *dev)
{
	...
	data->instance = dev;
	k_work_init(&data->interrupt_worker, gpio_ite_interrupt_worker);
	k_spin_unlock(&data->lock, key);
