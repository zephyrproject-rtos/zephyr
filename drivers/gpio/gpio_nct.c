...
#define DT_DRV_COMPAT nuvoton_nct_gpio
...
LOG_MODULE_REGISTER(gpio_nct, LOG_LEVEL_ERR);

/* GPIO module instances */
#define NCT_GPIO_DEV(inst) DEVICE_DT_INST_GET(inst),
static const struct device *gpio_devs[] = {
	DT_INST_FOREACH_STATUS_OKAY(NCT_GPIO_DEV)
};

/* Driver config */
struct gpio_nct_config {
	...
	struct nct_wui wui_maps[];
};

/* Driver data */
struct gpio_nct_data {
...
#define HAL_INSTANCE(dev)                                                                          \
        ((struct gpio_reg *)((const struct gpio_nct_config *)(dev)->config)->base)
...
{
	const struct gpio_nct_config *const config = dev->config;
	const struct nct_wui *io_wui = &config->wui_maps[pin];

	if (io_wui->table == NCT_MIWU_TABLE_NONE) {
	 ...
	 */
	if (pin < NCT_GPIO_PORT_PIN_NUM && !nct_miwu_irq_get_state(io_wui)) {
		nct_miwu_io_enable(io_wui);
	}
}
...
void nct_gpio_disable_io_pads(const struct device *dev, int pin)
{
	 ...
	 */
	if (pin < NCT_GPIO_PORT_PIN_NUM && !nct_miwu_irq_get_state(io_wui)) {
		nct_miwu_io_disable(io_wui);
	}
}

/* GPIO api functions */
static int gpio_nct_config(const struct device *dev,
			     gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if ((flags & GPIO_OPEN_DRAIN) && (flags & GPIO_OUTPUT)) {
		if ((flags & GPIO_PULL_UP)) {
			inst_scfg->DEVALTCX |= BIT(NCT_DEVALTCX_GPIO_PULL_EN);
		}
	}
...
static int gpio_nct_pin_get_config(const struct device *port, gpio_pin_t pin,
...
static int gpio_nct_port_get_raw(const struct device *dev,
...
static int gpio_nct_port_set_masked_raw(const struct device *dev,
...
static int gpio_nct_port_set_bits_raw(const struct device *dev,
...
static int gpio_nct_port_clear_bits_raw(const struct device *dev,
						gpio_port_value_t mask)
{
	struct gpio_reg *const inst = HAL_INSTANCE(dev);
...
static int gpio_nct_port_toggle_bits(const struct device *dev,
						gpio_port_value_t mask)
{
	struct gpio_reg *const inst = HAL_INSTANCE(dev);
...
static int gpio_nct_pin_interrupt_configure(const struct device *dev,
					     gpio_pin_t pin,
					     ...
					     enum gpio_int_trig trig)
{
	...
	if (config->wui_maps[pin].table == NCT_MIWU_TABLE_NONE) {
	...
	nct_miwu_irq_disable(&config->wui_maps[pin]);
	...
	if (mode != GPIO_INT_MODE_DISABLED) {
		...
		if (mode == GPIO_INT_MODE_EDGE) {
			miwu_mode = NCT_MIWU_MODE_EDGE;
		} else {
			miwu_mode = NCT_MIWU_MODE_LEVEL;
		}

		/* Determine trigger mode is low, high or both? */
		if (trig == GPIO_INT_TRIG_LOW) {
			miwu_trig = NCT_MIWU_TRIG_LOW;
		} else if (trig == GPIO_INT_TRIG_HIGH) {
			miwu_trig = NCT_MIWU_TRIG_HIGH;
		} else if (trig == GPIO_INT_TRIG_BOTH) {
			miwu_trig = NCT_MIWU_TRIG_BOTH;
		} else {
		...
		ret = nct_miwu_interrupt_configure(&config->wui_maps[pin],
						miwu_mode, miwu_trig);
		if (ret != 0) {
		...
		nct_miwu_irq_enable(&config->wui_maps[pin]);
	}

	return 0;
}

static int gpio_nct_manage_callback(const struct device *dev,
				      struct gpio_callback *callback, bool set)
{
	...
	nct_miwu_init_gpio_callback(miwu_cb, &config->wui_maps[pin],
			config->port);

	/* Insert or remove a IO callback which being called in MIWU ISRs */
	return nct_miwu_manage_callback(miwu_cb, set);
}

/* GPIO driver registration */
static const struct gpio_driver_api gpio_nct_driver = {
	.pin_configure = gpio_nct_config,
#ifdef CONFIG_GPIO_GET_CONFIG
	.pin_get_config = gpio_nct_pin_get_config,
#endif
	.port_get_raw = gpio_nct_port_get_raw,
	.port_set_masked_raw = gpio_nct_port_set_masked_raw,
	.port_set_bits_raw = gpio_nct_port_set_bits_raw,
	.port_clear_bits_raw = gpio_nct_port_clear_bits_raw,
	.port_toggle_bits = gpio_nct_port_toggle_bits,
	.pin_interrupt_configure = gpio_nct_pin_interrupt_configure,
	.manage_callback = gpio_nct_manage_callback,
};

int gpio_nct_init(const struct device *dev)
{
	ARG_UNUSED(dev);
...
#define NCT_GPIO_DEVICE_INIT(inst)                                             \
	static const struct gpio_nct_config gpio_nct_cfg_##inst = {        \
		.common = {						           \
			.port_pin_mask =                                           \
			GPIO_PORT_PIN_MASK_FROM_NGPIOS(NCT_GPIO_PORT_PIN_NUM), \
		},                                                                 \
		.base = DT_INST_REG_ADDR(inst),                                    \
		.port = inst,                                                      \
		.wui_maps = NCT_DT_WUI_ITEMS_LIST(inst),                        \
	};                                                                         \
	BUILD_ASSERT(NCT_DT_WUI_ITEMS_LEN(inst) == NCT_GPIO_PORT_PIN_NUM,    \
			"size of prop. wui-maps must equal to pin number!");   \
	static struct gpio_nct_data gpio_nct_data_##inst;		   \
										   \
	DEVICE_DT_INST_DEFINE(inst,					       \
			    ...
			    &gpio_nct_driver);
DT_INST_FOREACH_STATUS_OKAY(NCT_GPIO_DEVICE_INIT)
