...
const char *const gpio_pca_series_part_name[] = {
	"pca9538", "pca9539", "pca9554", "pca9555", "pcal6524", "pcal6534",
};
...
enum gpio_pca_series_reg_type {          /** Type0 Type1 Type2 Type3 */
	...
	/*  PCA_REG_TYPE_1B_POLARITY_INVERSION,		      x     x     x     x   * (unused,
	 * omitted) */
...
static inline int gpio_pca_series_reg_cache_reset(const struct device *dev)
{
	...
	for (int reg_type = 0; reg_type < PCA_REG_TYPE_COUNT; ++reg_type) {
		...
		if (reg_addr == PCA_REG_INVALID) {
			const uint8_t reset_value_0[] = {0x00, 0x00, 0x00, 0x00,
							 0x00, 0x00, 0x00, 0x00};
						   ...
						   const uint8_t *buf)
{
	...
	LOG_DBG("cache update type %d len %d from %s", reg_type, size, (buf ? "buffer" : "device"));
...
static inline struct gpio_pca_series_reg_cache_mini *
gpio_pca_series_reg_cache_mini_get(const struct device *dev)
{
	struct gpio_pca_series_data *data = dev->data;
...
{
	const struct gpio_pca_series_config *cfg = dev->config;
	struct gpio_pca_series_reg_cache_mini *cache = gpio_pca_series_reg_cache_mini_get(dev);
	int ret = 0;

	ret = gpio_pca_series_reg_read(dev, PCA_REG_TYPE_1B_OUTPUT_PORT, (uint8_t *)&cache->output);
	if (ret) {
	...
	enum gpio_pca_series_reg_type input_reg = cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG
							  ? PCA_REG_TYPE_1B_INPUT_STATUS
							  : PCA_REG_TYPE_1B_INPUT_PORT;
...
#ifdef CONFIG_GPIO_PCA_SERIES_CACHE_ALL
	LOG_WRN("cache base addr: 0x%p size: 0x%2.2x", data->cache, cfg->part_cfg->cache_size);
#else
...
#ifdef CONFIG_GPIO_PCA_SERIES_CACHE_ALL
		if (reg != PCA_REG_INVALID && cache != PCA_REG_INVALID) {
			LOG_WRN("%.2d\t%-24s\t0x%2.2x\t0x%16.16x\t0x%2.2x\t0x%16.16x\t", reg_type,
				gpio_pca_series_reg_name[reg_type], reg, *reg_val_p, cache,
				*cache_val_p);
			 ...
			 * while the actual register is not present. Account for that here:
			 */
			LOG_WRN("%.2d\t%-24s\tNone\tNone\t\t\t0x%2.2x\t0x%16.16x\t", reg_type,
				gpio_pca_series_reg_name[reg_type], cache, *cache_val_p);
		} else {
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
...
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
				,
				reg_type, gpio_pca_series_reg_name[reg_type], reg, *reg_val_p);
#ifdef CONFIG_GPIO_PCA_SERIES_CACHE_ALL
		}
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
	}
...
void gpio_pca_series_cache_test(const struct device *dev)
{
	...
	for (int reg_type = 0; reg_type < PCA_REG_TYPE_COUNT; ++reg_type) {
		...
		if (cache_offset != expected_offset) {
			LOG_ERR("reg %d cache offset 0x%2.2x error, expected 0x%2.2x", reg_type,
...
#ifdef CONFIG_GPIO_PCA_SERIES_INTERRUPT
/** Forward declaration */
static inline void gpio_pca_series_interrupt_handler_standard(const struct device *dev,
 ...
 */
static int gpio_pca_series_pin_configure(const struct device *dev, gpio_pin_t pin,
					 gpio_flags_t flags)
{
	const struct gpio_pca_series_config *cfg = dev->config;
	struct gpio_pca_series_data *data = dev->data;
	...
	if ((flags & GPIO_SINGLE_ENDED) && (cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG) == 0U) {
		return -ENOTSUP;
	}
	...
	if (cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG) {
		...
		ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_1B_OUTPUT_CONFIG,
						(uint8_t *)&reg_value);
	}
	...
	if ((cfg->part_cfg->flags & PCA_HAS_PULL)) {
		if ((flags & GPIO_PULL_UP) || (flags & GPIO_PULL_DOWN)) {
			...
			reg_value = sys_cpu_to_le32(reg_value);
			ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_1B_PULL_SELECT,
							(uint8_t *)&reg_value);
		}
	...
	if ((flags & GPIO_OUTPUT_INIT_HIGH) || (flags & GPIO_OUTPUT_INIT_LOW)) {
		...
		gpio_pca_series_reg_cache_mini_get(dev)->output = sys_le32_to_cpu(reg_value);
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
	}
	...
	reg_value = sys_cpu_to_le32(reg_value);
	ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_1B_CONFIGURATION, (uint8_t *)&reg_value);
 ...
 */
static int gpio_pca_series_port_read_standard(const struct device *dev, gpio_port_value_t *value)
{
	struct gpio_pca_series_data *data = dev->data;
	uint32_t input_data;
	int ret = 0;
	...
	/* Read Input Register */
	ret = gpio_pca_series_reg_read(dev, PCA_REG_TYPE_1B_INPUT_PORT, (uint8_t *)&input_data);
	if (ret) {
	...
	LOG_DBG("dev %s standard_read return %d result 0x%8.8x", dev->name, ret, (uint32_t)*value);
	return ret;
}
 ...
 *             -EWOULDBLOCK if called from ISR context
 */
static int gpio_pca_series_port_read_extended(const struct device *dev, gpio_port_value_t *value)
{
	const struct gpio_pca_series_config *cfg = dev->config;
	struct gpio_pca_series_data *data = dev->data;
	uint32_t input_data;
	...
	ret = gpio_pca_series_reg_read(dev, PCA_REG_TYPE_1B_INPUT_STATUS, (uint8_t *)&input_data);
	if (ret) {
		LOG_ERR("port read error %d", ret);
	} else {
		*value = sys_le32_to_cpu(input_data);
	...
	k_sem_give(&data->lock);
	LOG_DBG("dev %s extended_read return %d result 0x%8.8x", dev->name, ret, (uint32_t)*value);
	return ret;
}

static int gpio_pca_series_port_write(const struct device *dev, gpio_port_pins_t mask,
				      gpio_port_value_t value, gpio_port_value_t toggle)
{
	...
	LOG_DBG("dev %s write mask 0x%8.8x value 0x%8.8x toggle 0x%8.8x", dev->name, (uint32_t)mask,
		(uint32_t)value, (uint32_t)toggle);
	k_sem_take(&data->lock, K_FOREVER);

#ifdef CONFIG_GPIO_PCA_SERIES_CACHE_ALL
	/** get output register old value from reg cache */
	ret = gpio_pca_series_reg_cache_read(dev, PCA_REG_TYPE_1B_OUTPUT_PORT, (uint8_t *)&out_old);
	if (ret) {
							    ...
							    enum gpio_int_trig trig)
{
	...
	ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_2B_INTERRUPT_EDGE, (uint8_t *)&int_edge);
	if (ret) {
		goto out;
	}
	...
	input_latch = ~int_mask;
	input_latch = sys_cpu_to_le32(input_latch);
	ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_1B_INPUT_LATCH, (uint8_t *)&input_latch);
	if (ret) {
		goto out;
	}
	/** update interrupt mask register */
	int_mask = sys_cpu_to_le32(int_mask);
	ret = gpio_pca_series_reg_write(dev, PCA_REG_TYPE_1B_INTERRUPT_MASK, (uint8_t *)&int_mask);
	if (ret) {
		goto out;
	}
...
static int gpio_pca_series_manage_callback(const struct device *dev, struct gpio_callback *callback,
					   bool set)
{
	struct gpio_pca_series_data *data = dev->data;
						       ...
						       gpio_port_value_t *input_value)
{
	...
	int_status =
		(int_rise & transitioned_pins & input) | (int_fall & transitioned_pins & (~input));
