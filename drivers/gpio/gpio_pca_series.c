						 ...
						 uint8_t *buf)
{
	...
	LOG_DBG("cache read type %d len %d mem addr 0x%8.8lx", reg_type, size, (size_t)src);
	memcpy(buf, src, size);
	return ret;
}
						   ...
						   const uint8_t *buf)
{
	...
	LOG_DBG("cache write mem addr 0x%8.8lx len %d", (size_t)dst, size);
	...
	const struct device *dev)
{
	...
	LOG_DBG("mini cache addr 0x%8.8lx", (size_t)cache);
	return cache;
}
...
#ifdef CONFIG_GPIO_PCA_SERIES_CACHE_ALL
	LOG_WRN("cache base addr: 0x%8.8x size: 0x%2.2x",
		(size_t)data->cache, cfg->part_cfg->cache_size);
#else
	LOG_WRN("cache base addr: 0x%8.8x", (size_t)data->cache);
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
...
#else  /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
	LOG_DBG("access address 0x%8.8lx", (size_t)data->cache);
	out_old = gpio_pca_series_reg_cache_mini_get(dev)->output;
#endif /* CONFIG_GPIO_PCA_SERIES_CACHE_ALL */
