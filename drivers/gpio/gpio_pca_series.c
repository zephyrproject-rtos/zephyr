 ...
 *             -EIO if i2c error
 */
static int gpio_pca_series_pin_config_drive_strength(
		...
		enum gpio_pca_series_drive_strength strength)
{
	...
	ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_2b_output_drive_strength,
						 (uint8_t *)&reg_value);
	if (ret) {
		goto out;
	}
	reg_value = (reg_value & (~reg_cfg_mask)) | (strength << reg_cfg_shift);
	ret = gpio_pca_series_reg_write(dev, gpio_pca_series_2b_output_drive_strength,
					(uint8_t *)&reg_value);
		...
		gpio_pin_t pin, gpio_flags_t flags)
{
	...
	if ((flags & GPIO_SINGLE_ENDED) != 0) {
		if (cfg->part_cfg->flags & PCA_HAS_OUT_CONFIG) {
			...
			ret = gpio_pca_series_reg_read_cache(dev,
				gpio_pca_series_1b_output_config, (uint8_t *)&reg_value);
	...
	if (((flags & GPIO_PULL_UP) != 0) || ((flags & GPIO_PULL_DOWN) != 0)) {
		if (cfg->part_cfg->flags & PCA_HAS_PULL) {
			...
			ret = gpio_pca_series_reg_read_cache(dev,
				gpio_pca_series_1b_pull_select, (uint8_t *)&reg_value);
			...
			ret = gpio_pca_series_reg_read_cache(dev,
				gpio_pca_series_1b_pull_enable, (uint8_t *)&reg_value);
			reg_value &= (~BIT(pin)); /* clear bit to disable pull */
			ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_pull_enable,
							(uint8_t *)&reg_value);
		} else {
			ret = -ENOTSUP;
			goto out;
	...
	if (flags & GPIO_INPUT) {
		...
		ret = gpio_pca_series_reg_read_cache(dev,
			gpio_pca_series_1b_configuration, (uint8_t *)&reg_value);
		...
		if (flags & GPIO_OUTPUT_INIT_HIGH) {
			out = out_old | BIT(pin);
			ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_output_port,
							(uint8_t *)&out);
		}
		...
		/* configure gpio_pca_series_1b_configuration */
		ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_1b_configuration,
							 (uint8_t *)&reg_value);
		reg_value &= (~BIT(pin)); /* clear bit to set output */
		ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_configuration,
						(uint8_t *)&reg_value);
	}
		...
		const struct device *dev, gpio_port_value_t *value)
{
	...
	ret = gpio_pca_series_reg_read(dev,
		gpio_pca_series_1b_input_port, (uint8_t *)&input_data);
	if (ret) {
		goto out;
	}
	...
	enum gpio_int_trig trig)
{
	...
	ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_2b_interrupt_edge,
						 (uint8_t *)&buf);
	if (ret) {
	...
	if (cfg->part_cfg->flags & PCA_HAS_INT_MASK) {
		ret = gpio_pca_series_reg_read_cache(dev, gpio_pca_series_1b_interrupt_mask,
							 (uint8_t *)&int_mask);
		if (ret) {
			goto out;
		}
	...
	ret = gpio_pca_series_reg_update_cache(dev, gpio_pca_series_2b_interrupt_edge,
						   (uint8_t *)&buf);
	if (ret) {
		LOG_ERR("get current interrupt config fail [%d]", ret);
		goto out;
	...
	if (cfg->part_cfg->flags & PCA_HAS_LATCH) {
		...
		ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_input_latch,
						(uint8_t *)&input_latch);
		if (ret) {
			goto out;
		}
	...
	if (cfg->part_cfg->flags & PCA_HAS_INT_MASK) {
		ret = gpio_pca_series_reg_write(dev, gpio_pca_series_1b_interrupt_mask,
						(uint8_t *)&int_mask);
		if (ret) {
			goto out;
		}
...
static void gpio_pca_series_interrupt_handler_standard(const struct device *dev)
{
	...
	ret = gpio_pca_series_reg_read(dev, gpio_pca_series_1b_input_port, (uint8_t *)&input);
	if (ret != 0) {
		goto out;
	}
	if (cfg->part_cfg->flags & PCA_HAS_INT_MASK) {
...
#define GPIO_PCA_SERIES_DEVICE_INSTANCE(inst)                                              \
	static const struct gpio_pca_series_config gpio_pca_series_##inst##_cfg = {            \
		.common =                                                                          \
			{                                                                              \
				.port_pin_mask = GPIO_PCA_GET_PIN_MASK_BY_PART_NO(                         \
					DT_INST_ENUM_IDX(inst, part_no)),                                      \
			},                                                                             \
