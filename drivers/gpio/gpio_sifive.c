...
#define DEV_GPIO_CFG(dev)  ((const struct gpio_sifive_config *const)(dev)->config)
#define DEV_GPIO(dev)      ((volatile struct gpio_sifive_t *)(DEV_GPIO_CFG(dev))->gpio_base_addr)
#define DEV_GPIO_DATA(dev) ((struct gpio_sifive_data *)(dev)->data)
 ...
 */
static int gpio_sifive_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags)
{
	volatile struct gpio_sifive_t *gpio = DEV_GPIO(dev);
...
static int gpio_sifive_port_get_raw(const struct device *dev, gpio_port_value_t *value)
{
	volatile struct gpio_sifive_t *gpio = DEV_GPIO(dev);
...
static int gpio_sifive_port_set_masked_raw(const struct device *dev, gpio_port_pins_t mask,
...
static int gpio_sifive_port_set_bits_raw(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct gpio_sifive_t *gpio = DEV_GPIO(dev);
...
static int gpio_sifive_port_clear_bits_raw(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct gpio_sifive_t *gpio = DEV_GPIO(dev);
...
static int gpio_sifive_port_toggle_bits(const struct device *dev, gpio_port_pins_t mask)
{
	volatile struct gpio_sifive_t *gpio = DEV_GPIO(dev);
...
static int gpio_sifive_pin_interrupt_configure(const struct device *dev, gpio_pin_t pin,
					       enum gpio_int_mode mode, enum gpio_int_trig trig)
{
	...
	case GPIO_INT_MODE_EDGE:
		__ASSERT_NO_MSG(GPIO_INT_TRIG_BOTH == (GPIO_INT_LOW_0 | GPIO_INT_HIGH_1));
...
static int gpio_sifive_manage_callback(const struct device *dev, struct gpio_callback *callback,
				       bool set)
{
	struct gpio_sifive_data *data = DEV_GPIO_DATA(dev);
...
DEVICE_DT_INST_DEFINE(0, gpio_sifive_init, NULL, &gpio_sifive_data0, &gpio_sifive_config0,
		      PRE_KERNEL_1, CONFIG_GPIO_INIT_PRIORITY, &gpio_sifive_driver);

#define IRQ_INIT(n)                                                                \
	IRQ_CONNECT(DT_INST_IRQN_BY_IDX(0, n), DT_INST_IRQ_BY_IDX(0, n, priority), \
		    gpio_sifive_irq_handler, DEVICE_DT_INST_GET(0), 0);
