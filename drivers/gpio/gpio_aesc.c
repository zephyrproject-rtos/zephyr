...
static const struct gpio_driver_api gpio_aesc_driver_api = {
...
#define GPIO_AESC_INIT_FUNC(no)						      \
	static void gpio_aesc_bank_##no##_config(const struct device *dev)    \
	{								      \
		volatile struct gpio_aesc_regs *regs = DEV_GPIO(dev);	      \
		ARG_UNUSED(regs);					      \
	}

#define AESC_GPIO_INIT(no)						      \
	GPIO_AESC_INIT_FUNC(no);					      \
	static struct gpio_aesc_data gpio_aesc_dev_data_##no;		      \
	static struct gpio_aesc_config gpio_aesc_dev_cfg_##no = {	      \
		.bank_config = gpio_aesc_bank_##no##_config,		      \
		.common = {						      \
