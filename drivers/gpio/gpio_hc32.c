 ...
 */
static int gpio_hc32_configure(const struct device *port, gpio_pin_t pin, gpio_flags_t flags)
{
	int intr_trigger;
...
static int gpio_hc32_get_config(const struct device *port, gpio_pin_t pin, gpio_flags_t *flags)
{
	const struct gpio_hc32_config *cfg = port->config;
