...
static const struct device *gpio_devs[] = {
	DT_INST_FOREACH_STATUS_OKAY(NPCM_GPIO_DEV)
};
...
void npcm_gpio_enable_io_pads(const struct device *dev, int pin)
{
	...
	const struct npcm_wui *io_wui = &config->wui_maps[pin];

	if (io_wui->table == NPCM_MIWU_TABLE_NONE) {
	 ...
	 */
	if (pin < NPCM_GPIO_PORT_PIN_NUM && !npcm_miwu_irq_get_state(io_wui)) {
...
static int gpio_npcm_port_get_raw(const struct device *dev,
...
static int gpio_npcm_port_set_masked_raw(const struct device *dev,
...
static int gpio_npcm_port_set_bits_raw(const struct device *dev,
...
static int gpio_npcm_port_clear_bits_raw(const struct device *dev,
						gpio_port_value_t mask)
{
	struct gpio_reg *const inst = HAL_INSTANCE(dev);
...
static int gpio_npcm_port_toggle_bits(const struct device *dev,
						gpio_port_value_t mask)
{
	struct gpio_reg *const inst = HAL_INSTANCE(dev);
					     ...
					     enum gpio_int_trig trig)
{
	...
	if (config->wui_maps[pin].table == NPCM_MIWU_TABLE_NONE) {
	...
	npcm_miwu_irq_disable(&config->wui_maps[pin]);
	...
	if (mode != GPIO_INT_MODE_DISABLED) {
		...
		ret = npcm_miwu_interrupt_configure(&config->wui_maps[pin],
						miwu_mode, miwu_trig);
		if (ret != 0) {
		...
		npcm_miwu_irq_enable(&config->wui_maps[pin]);
	}
				      ...
				      struct gpio_callback *callback, bool set)
{
	...
	npcm_miwu_init_gpio_callback(miwu_cb, &config->wui_maps[pin],
			config->port);
...
static const struct gpio_driver_api gpio_npcm_driver = {
	.pin_configure = gpio_npcm_config,
#ifdef CONFIG_GPIO_GET_CONFIG
...
#define NPCM_GPIO_DEVICE_INIT(inst)                                             \
	static const struct gpio_npcm_config gpio_npcm_cfg_##inst = {        \
		...
		.port = inst,                                                      \
		.wui_maps = NPCM_DT_WUI_ITEMS_LIST(inst),                        \
	};                                                                         \
	BUILD_ASSERT(NPCM_DT_WUI_ITEMS_LEN(inst) == NPCM_GPIO_PORT_PIN_NUM,    \
