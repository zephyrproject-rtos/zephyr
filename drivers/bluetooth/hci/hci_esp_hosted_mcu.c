...
#define DT_DRV_COMPAT espressif_esp_hosted_mcu_bt_hci
...
/* One inbound packet: ACL, event or discardable event, plus the H4 type byte. */
#define HCI_ESP_HOSTED_MCU_RX_MTU (MAX(BT_BUF_RX_SIZE, CONFIG_BT_BUF_EVT_DISCARDABLE_SIZE) + 1)

struct hci_esp_hosted_mcu_rx_frame {
	void *fifo_reserved;
	uint16_t len;
	uint8_t data[HCI_ESP_HOSTED_MCU_RX_MTU];
};
...
static bool hci_esp_hosted_mcu_evt_discardable(const uint8_t *evt_data, size_t remaining)
{
	...
	case BT_HCI_EVT_LE_EXT_ADVERTISING_REPORT: {
		const struct bt_hci_evt_le_ext_advertising_report *ext_adv =
			(const void *)&evt_data[sizeof(struct bt_hci_evt_hdr) + 1];
		uint16_t adv_evt_type;

		if (remaining < sizeof(struct bt_hci_evt_hdr) + 1 + sizeof(*ext_adv) +
					sizeof(ext_adv->adv_info[0])) {
			return false;
		}

		adv_evt_type = sys_le16_to_cpu(ext_adv->adv_info[0].evt_type);

		/*
		 * Extended reports may be fragments of a larger report that the
		 * host reassembles, so only a single complete legacy PDU report
		 * is safe to drop.
		 */
				  ...
				  void *user_data)
{
	...
	if (len < 1 || len > HCI_ESP_HOSTED_MCU_RX_MTU) {
		LOG_WRN("Bad frame length %u, dropping", len);
		return;
	}
...
#define HCI_ESP_HOSTED_MCU_INIT(inst)                                                              \
	...
	DEVICE_DT_INST_DEFINE(inst, NULL, NULL, &hci_esp_hosted_mcu_data_##inst,                   \
			      ...
			      CONFIG_ESP_HOSTED_MCU_INIT_PRIORITY, &hci_esp_hosted_mcu_api);
