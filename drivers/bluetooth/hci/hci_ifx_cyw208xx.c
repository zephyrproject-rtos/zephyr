...
#define HCI_BRCM_SET_LOCAL_DEV_ADDR     (0xFC01)
...
static int cyw208xx_setup(const struct device *dev, const struct bt_hci_setup_params *params)
{
	...
	/* Set public address */
	buf = bt_hci_cmd_create(HCI_BRCM_SET_LOCAL_DEV_ADDR, BTM_SET_LOCAL_DEV_ADDR_LENGTH);
	if (buf == NULL) {
		LOG_ERR("Unable to allocate command buffer");
		return -ENOMEM;
	}
	 ...
	 * the HCI command HCI_BRCM_SET_LOCAL_DEV_ADDR. So
	 *
	...
	err = bt_hci_cmd_send_sync(HCI_BRCM_SET_LOCAL_DEV_ADDR, buf, NULL);
	if (err) {
