...
LOG_MODULE_REGISTER(bt_hci_driver_b91, LOG_LEVEL);
...
static void hci_b91_host_rcv_pkt(uint8_t *data, uint16_t len)
{
	...
	LOG_HEXDUMP_DBG((const uint8_t *) data, len, "host packet data:");
...
static int bt_b91_send(struct net_buf *buf)
{
	...
	LOG_HEXDUMP_DBG((const uint8_t *) buf->data, buf->len, "Final HCI buffer:");
