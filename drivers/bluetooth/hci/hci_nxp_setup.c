...
#define FW_UPLOAD_CHANGE_TIMEOUT_RETRY_COUNT 			6U
...
static int bt_nxp_set_calibration_data_annex55(void)
{
	...
	const uint8_t hci_cal_data_annex55[HCI_CMD_STORE_BT_CAL_DATA_PARAM_LENGTH] = {
		    ...
		    0x00U,                            //  Pointer For Next Annex[0] : 0x00000000
		    0x00U,                            //  Pointer For Next Annex[1] : 0x00000000
		    0x00U,                            //  Pointer For Next Annex[2] : 0x00000000
		    0x00U,                            //  Pointer For Next Annex[3] : 0x00000000
		    0x01U,                            //  Annex Version : 0x01
		    0x81U,                            //  External Xtal Calibration Value : 0x7d
		    0x0DU,                            //  Initial TX Power : 13
		    0x07U, 							  //  Front End Loss : 0x07
		    0x28U,                            //  BT Options :
		                                      //              BIT[0] Force Class 2 operation = 0
		                                      //              BIT[1] Disable Pwr Control for class 2= 0
		                                      //              BIT[2] MiscFlag(to indicate external XTAL) = 0
		    ...
		    0x00U,                            //  Pointer For Next Annex[3] : 0x00000000
		    0x01U,                            //  Annex Version : 0x01
		    0x00U,                            //  External Xtal Calibration Value
		    0x03U,                            //  Initial TX Power : 0x03
		    0x03U, 							  //  Front End Loss : 0x03
		    0x00U,                            //  BT Options :
		                                      //              BIT[0] Force Class 2 operation = 0
		                                      //              BIT[1] Disable Pwr Control for class 2= 0
		                                      ...
		                                      //              BIT[7:6] Reserved
		    0x00U,                            //  AOANumberOfAntennas: 0x00
		    0xBAU,                            //  RSSI Golden Low : 0
		    0xCEU,                            //  RSSI Golden High : 0
		    0xC0U,                            //  UART Baud Rate[0] : 0x002DC6C0(3000000)
		    0xC6U,                            //  UART Baud Rate[1] : 0x002DC6C0(3000000)
		    0x2DU,                            //  UART Baud Rate[2] : 0x002DC6C0(3000000)
		    0x00U,                            //  UART Baud Rate[3] : 0x002DC6C0(3000000)
...
static int bt_nxp_set_calibration_data_annex100(void)
{
	...
	const uint8_t hci_cal_data_annex100[HCI_CMD_STORE_BT_CAL_DATA_PARAM_ANNEX100_LENGTH] = {
		...
		0x01,						// Ext PA Present (1 bit) + Ext. PA Gain (7 bits)
		0x00,						// Ext Antenna Gain Present (1 bit) + Ext. Antenna Gain Val (4 bits)
		0x04, 0x00, 				// BT / LE Ext PA FEM CTRL Bit-mask (16 bits=16 RF Control)
		0x01, 						// Ext LNA Present (1 bit) + Ext LNA Gain (7 bits)
		0x00, 						// Reserved
		0x04, 0x00					// BT / LE Ext LNA FEM CTRL Bit-mask (16 bits=16 RF Control)
#elif defined (CONFIG_BT_NXP_IW416)
	    0x64U, 						// Annex Type : 0x64
	    0x83,  						// Checksum
	    0x10, 0x00, 				// Length
	    ...
	    0x01,						// Ext PA Present (1 bit) + Ext. PA Gain (7 bits)
	    0x00,						// Ext Antenna Gain Present (1 bit) + Ext. Antenna Gain Val (4 bits)
	    0x0C, 0x00, 				// BT / LE Ext PA FEM CTRL Bit-mask (16 bits=16 RF Control)
	    0x01, 						// Ext LNA Present (1 bit) + Ext LNA Gain (7 bits)
	    0x00, 						// Reserved
	    0x0C, 0x00					// BT / LE Ext LNA FEM CTRL Bit-mask (16 bits=16 RF Control)
#else
#error "BT Calibration data (annex-100) is not given for selected chipset"
	...
	if (IS_ENABLED(CONFIG_BT_HCI_HOST)) {
		...
		net_buf_add_mem(buf, hci_cal_data_annex100, HCI_CMD_STORE_BT_CAL_DATA_PARAM_ANNEX100_LENGTH);
