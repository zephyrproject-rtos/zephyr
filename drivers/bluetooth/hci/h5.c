...
LOG_MODULE_REGISTER(bt_driver, LOG_LEVEL);
...
static void bt_uart_isr(const struct device *unused, void *user_data)
{
	...
	while (uart_irq_update(h5_dev) &&
		...
		case END:
			...
			LOG_DBG("Received full packet: type %u",
			       H5_HDR_PKT_TYPE(hdr));
...
static void rx_thread(void)
{
	...
	while (true) {
		...
		if (!memcmp(buf->data, sync_req, sizeof(sync_req))) {
			...
			LOG_DBG("Finished H5 configuration, tx_win %u",
			       h5.tx_win);
		} else {
			LOG_ERR("Not handled yet %x %x",
