...
static int rsi_bt_driver_send_tx_pwr_vs_cmd(const struct device *dev);
...
{
	struct net_buf *buf;
	uint8_t le_tx_power_index = RSI_BLE_PWR_INX;
	uint8_t protocol_mode = BT_LE_MODE;
	int err;
...
{
	int ret = rsi_bt_driver_send_tx_pwr_vs_cmd(dev);

	if (ret < 0) {
		LOG_ERR("Failed to send RF power config command: %d", ret);
		return ret;
	}
	return 0;
}
