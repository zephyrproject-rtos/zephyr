...
static int siwx91x_bt_send_raw(const struct device *dev, const uint8_t *pkt, size_t len)
{
	...
	(void)memcpy(&hci->rsi_data_packet, pkt, len);
	err = rsi_bt_driver_send_cmd(RSI_BLE_REQ_HCI_RAW, &hci->rsi_data_packet, NULL);
	if (err == RSI_ERROR_RESPONSE_TIMEOUT) {
		/* The SDK handed the command to the network processor and then
		 * gave up waiting for the response, which may still arrive: the
		 * outcome is for the HCI response, or the caller's own timeout,
		 * to tell. A failure return would mean that nothing was sent.
		 */
		LOG_WRN("BT command response timeout in the SDK");
	} else if (err != 0) {
		/* TODO SILABS ZEPHYR Convert to errno. A common function from rsi/sl_status should
		 * be introduced
		 */
		LOG_ERR("BT command send failure: %d", err);
		return -EIO;
	}
