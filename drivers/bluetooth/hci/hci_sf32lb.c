...
static inline void process_tx(const struct device *dev)
{
	...
	while (h4->tx.buf) {
		LOG_DBG("%s data %p, type %d len %d", __func__, (void *)h4->tx.buf->data,
			h4->tx.buf->data[0], h4->tx.buf->len);
		sf32lb_hci_trace(h4->tx.buf->data[0], &h4->tx.buf->data[1], h4->tx.buf->len - 1,
				 H4TL_PACKET_HOST);
		bytes = ipc_queue_write(h4->ipc_port, h4->tx.buf->data, h4->tx.buf->len,
					IPC_TIMEOUT_MS);
		if (unlikely(bytes < 0)) {
...
static inline void process_rx(const struct device *dev)
{
	...
	LOG_DBG("remaining %u discard %u have_hdr %u rx.buf %p len %u", h4->rx.remaining,
		h4->rx.discard, h4->rx.have_hdr, (void *)h4->rx.buf,
