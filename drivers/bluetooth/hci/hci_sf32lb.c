...
#define SF32LB_PACKET_HOST 0x61
#define SF32LB_PACKET_CTRL 0x62
...
static void zbt_config_mailbox(const struct device *dev)
{
	...
	__ASSERT(IPC_QUEUE_INVALID_HANDLE != hci->ipc_port, "Invalid Handle");
	if (ipc_queue_open(hci->ipc_port)) {
		__ASSERT(0, "Could not open IPC");
	}
}

void sf32lb_hci_trace(uint8_t type, const uint8_t *data, uint16_t len, uint8_t direction)
{
	/*TODO: Add HCI trace functions*/
}
	...
	if (!hci->rx.remaining) {
		if (hci->rx.evt.evt == BT_HCI_EVT_LE_META_EVENT &&
		    ((hci->rx.hdr[sizeof(*hdr)] == BT_HCI_EVT_LE_ADVERTISING_REPORT) ||
		     (hci->rx.hdr[sizeof(*hdr)] == BT_HCI_EVT_LE_EXT_ADVERTISING_REPORT))) {
...
static struct net_buf *get_rx(struct bt_sf32lb_data *hci, k_timeout_t timeout)
{
	LOG_DBG("type 0x%02x, evt 0x%02x", hci->rx.type, hci->rx.evt.evt);

	switch (hci->rx.type) {
	case BT_HCI_H4_EVT:
		return bt_buf_get_evt(hci->rx.evt.evt, hci->rx.discardable, timeout);
	case BT_HCI_H4_ACL:
...
static void rx_thread(void *p1, void *p2, void *p3)
{
	...
	struct bt_sf32lb_data *data = dev->data;
	...
	while (1) {
		...
		while (len) {
			...
			while (buf) {
				...
				if (hci->recv) {
					sf32lb_hci_trace(buf->data[0], &buf->data[1], buf->len - 1,
							 SF32LB_PACKET_CTRL);
					hci->recv(dev, buf);
				} else {
					net_buf_unref(buf);
...
static inline void read_payload(const struct device *dev)
{
	...
	if (!hci->rx.buf) {
		...
		hci->rx.buf = get_rx(hci, K_NO_WAIT);
		if (!hci->rx.buf) {
		...
		if (buf_tailroom < hci->rx.remaining) {
...
static inline void process_tx(const struct device *dev)
{
	...
	while (hci->tx.buf) {
		LOG_DBG("%s data %p, type %d len %d", __func__, (void *)hci->tx.buf->data,
			hci->tx.buf->data[0], hci->tx.buf->len);
		sf32lb_hci_trace(hci->tx.buf->data[0], &hci->tx.buf->data[1], hci->tx.buf->len - 1,
				 SF32LB_PACKET_HOST);
		...
		if (unlikely(bytes < 0)) {
			...
			LOG_DBG("%s bytes %d", __func__, bytes);
		...
		LOG_DBG("%s net_buf_unref", __func__);
