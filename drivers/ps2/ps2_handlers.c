...
static inline int z_vrfy_ps2_config(const struct device *dev, ps2_callback_t callback_isr)
{
	K_OOPS(K_SYSCALL_DRIVER_PS2(dev, config));
	K_OOPS(K_SYSCALL_VERIFY_MSG(callback_isr == NULL, "callback not be set from user mode"));
