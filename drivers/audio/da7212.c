...
LOG_MODULE_REGISTER(dialog_da7212, CONFIG_AUDIO_CODEC_LOG_LEVEL);

#define DT_DRV_COMPAT	dialog_da7212
...
static int da7212_protocol_config(const struct device *dev, audio_dai_type_t dai_type)
{
	...
	case AUDIO_DAI_TYPE_RIGHT_JUSTIFIED:
		proto = DIALOG7212_DAI_FORMAT_RIGTH_JUSTIFIED;
		break;
...
{
	uint8_t vol = (uint8_t)CLAMP(volume, 0, 0x3F);
				...
				audio_channel_t channel, bool mute)
{
	 ...
	 * 0x80 (EN) to unmute
	 */
	uint8_t regValue = mute ? 0xC0U : 0x80U;

	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
	case AUDIO_CHANNEL_HEADPHONE_LEFT:
		da7212_update_reg(dev, DIALOG7212_HP_L_CTRL, 0xC0U, regValue);
		return 0;
	case AUDIO_CHANNEL_FRONT_RIGHT:
	case AUDIO_CHANNEL_HEADPHONE_RIGHT:
		da7212_update_reg(dev, DIALOG7212_HP_R_CTRL, 0xC0U, regValue);
		return 0;
	case AUDIO_CHANNEL_ALL:
		da7212_update_reg(dev, DIALOG7212_HP_L_CTRL, 0xC0U, regValue);
		da7212_update_reg(dev, DIALOG7212_HP_R_CTRL, 0xC0U, regValue);
		da7212_update_reg(dev, DIALOG7212_LINE_CTRL, 0xC0U, regValue);
		return 0;
	default:
		return -EINVAL;
	}
...
static int da7212_in_volume_config(const struct device *dev,
			audio_channel_t channel, int volume)
{
	uint8_t vol = (uint8_t)CLAMP(volume, 0, 0x7F);
				...
				audio_channel_t channel, bool mute)
{
	...
	if (channel == AUDIO_CHANNEL_ALL) {
		da7212_update_reg(dev, DIALOG7212_ADC_L_CTRL, 0xC0U, regValue);
		da7212_update_reg(dev, DIALOG7212_ADC_R_CTRL, 0xC0U, regValue);
		return 0;
	}

	da7212_update_reg(dev, reg, 0xC0U, regValue);
			...
			audio_channel_t channel, uint32_t input)
{
	...
	switch (input) {
	case 0: /* AUX */
		l_sel = DIALOG7212_MIXIN_L_SELECT_AUX_L_SEL_MASK;
		r_sel = DIALOG7212_MIXIN_R_SELECT_AUX_R_SEL_MASK;
		break;
	default:
		/* For other inputs, keep AUX as default to have a working path */
		l_sel = DIALOG7212_MIXIN_L_SELECT_AUX_L_SEL_MASK;
		r_sel = DIALOG7212_MIXIN_R_SELECT_AUX_R_SEL_MASK;
		break;
	}
	...
	switch (channel) {
	case AUDIO_CHANNEL_FRONT_LEFT:
		da7212_write_reg(dev, DIALOG7212_MIXIN_L_SELECT, l_sel);
		break;
	case AUDIO_CHANNEL_FRONT_RIGHT:
		da7212_write_reg(dev, DIALOG7212_MIXIN_R_SELECT, r_sel);
		break;
	case AUDIO_CHANNEL_ALL:
		da7212_write_reg(dev, DIALOG7212_MIXIN_L_SELECT, l_sel);
		da7212_write_reg(dev, DIALOG7212_MIXIN_R_SELECT, r_sel);
...
static void da7212_configure_output(const struct device *dev)
{
	...
	da7212_write_reg(dev, DIALOG7212_DAC_L_CTRL,
		(uint8_t)(DIALOG7212_DAC_L_CTRL_ADC_EN_MASK |
			DIALOG7212_DAC_L_CTRL_ADC_RAMP_EN_MASK));
	da7212_write_reg(dev, DIALOG7212_DAC_R_CTRL,
		(uint8_t)(DIALOG7212_DAC_R_CTRL_ADC_EN_MASK |
	...
	da7212_write_reg(dev, DIALOG7212_DAC_L_GAIN, (uint8_t)(0x67));
	da7212_write_reg(dev, DIALOG7212_DAC_R_GAIN, (uint8_t)(0x67));
...
static void da7212_configure_input(const struct device *dev)
{
	 ...
	 * EN | SMALL_SWIT_CH_FREQ_EN | MCHANGE_OUTPUT | MOD_CPVDD_1 | ANALOG_VLL_LV_BOOSTS_CP
	 */
	da7212_write_reg(dev, DIALOG7212_CP_CTRL,
		(uint8_t)(DIALOG7212_CP_CTRL_EN_MASK |
				  DIALOG7212_CP_CTRL_SMALL_SWIT_CH_FREQ_EN_MASK |
	...
	/* AUX_L_CTRL = 0xB4: AMP_EN | AMP_RAMP_EN | AMP_ZC_EN | AMP_ZC_SEL_INPUT_AUX_L_IF */
	...
	/* AUX_R_CTRL = 0xB0: AMP_EN | AMP_RAMP_EN | AMP_ZC_EN */
	...
	/* MIC_1_CTRL = 0x04 (MIC_1_P), MIC_2_CTRL = 0x04 (MIC_2_P) */
	...
	/* MIXIN_L/R_CTRL = 0x88: AMP_EN | AMP_MIX_EN */
	...
	/* ADC_L_CTRL = 0xA0: ADC_EN | ADC_RAMP_EN (mirror to R for stereo) */
...
static int da7212_configure(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	da7212_write_reg(dev, DIALOG7212_DIG_ROUTING_DAI,
			(uint8_t)DIALOG7212_DIG_ROUTING_DAI_L_SRC_ADC_LEFT);
	...
	da7212_write_reg(dev, DIALOG7212_PLL_INTEGER, 0x20);
