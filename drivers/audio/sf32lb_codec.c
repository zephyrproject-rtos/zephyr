...
static void config_analog_dac_path(AUDCODEC_TypeDef *reg,
			const struct sf32lb_codec_dac_clk *clk)
{
...
static void config_analog_adc_path(AUDCODEC_TypeDef *reg,
			const struct sf32lb_codec_adc_clk *clk)
{
...
static void config_tx_channel(AUDCODEC_TypeDef *reg, int channel,
			struct sf32lb_codec_dac_cfg *cfg)
{
...
static inline void close_analog_adc_path(AUDCODEC_TypeDef *reg)
{
	reg->ADC1_CFG2 &= ~AUDCODEC_ADC1_CFG2_EN;
	reg->ADC2_CFG2 &= ~AUDCODEC_ADC2_CFG2_EN;
	reg->ADC_ANA_CFG &= ~AUDCODEC_ADC_ANA_CFG_MICBIAS_EN;
...
static inline void close_analog_dac_path(AUDCODEC_TypeDef *reg)
{
	reg->DAC1_CFG |= AUDCODEC_DAC1_CFG_SR;
	reg->DAC2_CFG |= AUDCODEC_DAC2_CFG_SR;
	k_busy_wait(10);
...
static inline void clear_dac_channel(AUDCODEC_TypeDef *reg)
{
...
static inline void clear_adc_channel(AUDCODEC_TypeDef *reg)
{
...
static inline void disable_adc(AUDCODEC_TypeDef *reg)
{
	reg->CFG &= (~AUDCODEC_CFG_ADC_ENABLE);
}

static inline void disable_dac(AUDCODEC_TypeDef *reg)
{
	reg->CFG &= (~AUDCODEC_CFG_DAC_ENABLE);
}

static void config_dac_path_volume(AUDCODEC_TypeDef *reg, int channel, int volume)
{
...
void mute_dac_path(AUDCODEC_TypeDef *reg, int mute)
{
...
static void config_rx_channel(AUDCODEC_TypeDef *reg, int channel,
		struct sf32lb_codec_adc_cfg *cfg)
{
...
static inline void refgen_init(AUDCODEC_TypeDef *reg)
{
	reg->BG_CFG0 &= ~AUDCODEC_BG_CFG0_EN_SMPL;
...
static void pll_turn_off(AUDCODEC_TypeDef *reg)
{
	/* turn off pll */
...
static void pll_turn_on(AUDCODEC_TypeDef *reg)
{
	/* turn on bandgap */
...
static int pll_update_freq(AUDCODEC_TypeDef *reg, uint8_t type)
{
	reg->PLL_CFG2 |= AUDCODEC_PLL_CFG2_RSTB;
	/* wait 50us */
	k_busy_wait(50);
...
static inline void wait_pll_done(AUDCODEC_TypeDef *reg)
{
	while (!(reg->PLL_CAL_CFG & AUDCODEC_PLL_CAL_CFG_DONE_Msk)) {
	}
}

static inline void fix_pll_vco_table(struct pll_vco_t *vco,
				uint32_t delta_cnt,
...
static inline void adjust_pll_vco(AUDCODEC_TypeDef *reg, struct pll_vco_t *vco)
{
	uint32_t pll_cnt = 0;
	uint32_t xtal_cnt = 0;
	uint32_t fc_vco = 16;
	...
	fix_pll_vco_table(vco, delta_cnt, delta_cnt_min, delta_cnt_max,
		fc_vco_min, fc_vco_max, fc_vco);
}

static void pll_calibration(AUDCODEC_TypeDef *reg)
{
	pll_turn_on(reg);
 ...
 * @return
 */
static int pll_enable(AUDCODEC_TypeDef *reg, uint32_t freq, uint8_t type)
{
	uint8_t freq_type;
...
static int bf0_update_pll(AUDCODEC_TypeDef *reg, uint32_t freq, uint8_t type)
{
	uint8_t freq_type;
	uint8_t test_result = -1;
	uint8_t vco_index = 0;
...
static int codec_write(const struct device *dev, uint8_t *data, size_t size)
{
	...
	__ASSERT(dev_data->tx_buf && dev_data->tx_write_ptr, "write buf err");
	__ASSERT(size <= dev_data->tx_half_dma_size, "tx size err");
...
static int codec_configure(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	AUDCODEC_TypeDef *reg = sf32lb_cfg->reg;
...
{
	const struct device *dev = codec_dev;
	struct sf32lb_audcodec_data *data = dev->data;
...
static int codec_stop(const struct device *dev, audio_dai_dir_t dir)
{
	...
	if (stop_tx) {
		...
		mute_dac_path(cfg->reg, 1); /* avoid pop noise */
...
static void config_audcodec_dma(const struct sf32lb_dma_dt_spec *spec, uint8_t is_tx)
{
	int ret;
	...
	config_dma.complete_callback_en = 1U;
...
static int codec_driver_init(const struct device *dev)
{
	...
	config_audcodec_dma(&cfg->dma_tx, 1);
	config_audcodec_dma(&cfg->dma_rx, 0);
...
#define SF32LB_AUDIO_CODEC_DEFINE(n)\
	static const struct sf32lb_codec_driver_config config##n = {               \
		.reg = (AUDCODEC_TypeDef *)DT_INST_REG_ADDR_BY_IDX(n, 0),          \
		...
		.pa_power_dt = GPIO_DT_SPEC_GET(DT_ALIAS(pa_power), gpios),        \
	};                                                                         \
                                                                                   \
