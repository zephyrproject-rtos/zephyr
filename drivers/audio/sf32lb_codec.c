...
void mute_dac_path(const struct device *dev, uintptr_t reg, int mute)
{
	uint32_t reg_val;
...
static void sf32lb_codec_set_dac_volume(const struct device *dev, uint8_t volume)
{
	...
	config_dac_path_volume(cfg->reg, gain * 2);
...
{
	LOG_ERR("start_output is not supported, please use start function for this device");
	ARG_UNUSED(dev);
}

static void codec_stop_output(const struct device *dev)
{
	LOG_ERR("stop_output is not supported, please use stop function for this device");
	ARG_UNUSED(dev);
}
