					 ...
					 const struct i2s_config *i2s_cfg, i2s_config_t *fsl_cfg)
{
	...
	switch (i2s_cfg->options & (I2S_OPT_BIT_CLK_SLAVE | I2S_OPT_FRAME_CLK_SLAVE)) {
...
static void i2s_mcux_config_dma_blocks(const struct device *dev, enum i2s_dir dir, uint32_t *buffer,
				       size_t block_size)
{
	const struct i2s_mcux_config *cfg = dev->config;
	struct i2s_mcux_data *dev_data = dev->data;
	...
	case I2S_STATE_RUNNING:
		...
		if (stream->state == I2S_STATE_RUNNING) {
			...
			if (ret != 0) {
				...
				if (ret != 0) {
					LOG_ERR("buffer %p -> in_queue %p err %d", buffer,
						&stream->in_queue, ret);
				}
				dma_start(stream->dev_dma, stream->channel);
			}
...
static int i2s_mcux_tx_stream_start(const struct device *dev)
{
	...
	i2s_mcux_config_dma_blocks(dev, I2S_DIR_TX, (uint32_t *)queue_entry.mem_block,
				   queue_entry.size);
...
static int i2s_mcux_rx_stream_start(const struct device *dev)
{
	...
	for (int i = 0; i < NUM_RX_DMA_BLOCKS; i++) {
		ret = k_mem_slab_alloc(stream->cfg.mem_slab, &buffer[i], K_NO_WAIT);
		if (ret != 0) {
	...
	i2s_mcux_config_dma_blocks(dev, I2S_DIR_RX, (uint32_t *)buffer, stream->cfg.block_size);
...
static int i2s_mcux_init(const struct device *dev)
{
	...
	k_msgq_init(&data->tx.in_queue, (char *)data->tx_in_msgs, sizeof(struct i2s_txq_entry),
		    CONFIG_I2S_MCUX_FLEXCOMM_TX_BLOCK_COUNT);
	k_msgq_init(&data->rx.in_queue, (char *)data->rx_in_msgs, sizeof(void *),
