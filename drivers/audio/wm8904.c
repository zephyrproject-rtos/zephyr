...
{
	LOG_DBG("Initializing codec.");

	const struct wm8904_driver_config *const dev_cfg = DEV_CFG(dev);

	int error = pinctrl_apply_state(dev_cfg->pincfg, PINCTRL_STATE_DEFAULT);

	if (error) {
		LOG_ERR("pinctrl_apply_state fail: %d", error);
		return error;
	}

	error = i2c_recover_bus(dev_cfg->i2c.bus);
	if (error) {
		LOG_ERR("i2c_recover_bus fail: %d", error);
		return error;
	}
...
#define WM8904_INIT(n)                                                                             \
	...
	static const struct wm8904_driver_config wm8904_device_config_##n = {                      \
		...
		.mclk_name = (clock_control_subsys_t)DT_INST_CLOCKS_CELL_BY_NAME(n, mclk, name)};  \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, &wm8904_initialize, NULL, NULL, &wm8904_device_config_##n,        \
