...
{
	/* Supported Fsref values per datasheet, in Hz. */
	static const uint32_t fsref_tbl[] = { 48000, 44100 };
	uint64_t num;		/* K*10000 numerator */
	uint64_t k10k;		/* K scaled by 10000 */
	uint64_t vco_hz;	/* PLL VCO frequency */
	uint32_t fsref;		/* audio reference frequency */
	uint32_t mclk_p;	/* MCLK after P divider */
	uint32_t div_x2;	/* 2 * (Fsref / Fs) */
	uint32_t q;		/* PLL-bypass divider */
	uint32_t j;		/* integer part of K */
	uint32_t d;		/* fractional part of K */
	int8_t fs_div;		/* FS_DIV encoding */
	int fi;			/* fsref_tbl index */
	uint8_t p;		/* MCLK divider */
			   ...
			   struct audio_codec_cfg *cfg)
{
	...
	if (cfg->dai_cfg.i2s.format &
	    (I2S_FMT_CLK_FORMAT_MASK | I2S_FMT_DATA_ORDER_LSB)) {
		return -EINVAL;
	}
...
static void aic26_start_output(const struct device *dev)
{
	...
	pwr_val = AIC26_DAODRC;

	if (data->route != AUDIO_ROUTE_PLAYBACK_CAPTURE) {
		pwr_val |= AIC26_ADPWDN;
	}
