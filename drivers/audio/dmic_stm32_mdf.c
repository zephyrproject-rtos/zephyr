 ...
 * This avoids to need to compute it at runtime.
 */
...
static int dmic_stm32_mdf_configure(const struct device *dev, struct dmic_cfg *cfg)
{
	...
	if (channel->req_num_chan > MDF_MAX_SITFS || channel->req_chan_map_hi != 0) {
		LOG_ERR("DMIC only supports %u channels or less", MDF_MAX_SITFS);
		return -ENOTSUP;
	}
	...
	if (stream->pcm_width > MDF_DATA_RES) {
		LOG_ERR("DMIC only supports max %u-bit resolution samples", MDF_DATA_RES);
		return -ENOTSUP;
	}

	/* Ensure block size alignment with PCM sample size */
	if ((stream->block_size % DIV_ROUND_UP(stream->pcm_width, 8)) != 0) {
		LOG_ERR("Block size is not aligned with sample width");
		return -EINVAL;
	}
...
#define RESHAPE_FILTER(flt)                                                                        \
	.ReshapeFilter = {                                                                         \
		...
		.DecimationRatio = DT_PROP_OR(flt, st_reshape_filter_decimation, 0),               \
	},

#define HIGHPASS_FILTER(flt)                                                                       \
	.HighPassFilter = {                                                                        \
		.Activation = DT_NODE_HAS_PROP(flt, st_highpass_filter_cutoff),                    \
		.CutOffFrequency = DT_PROP_OR(flt, st_highpass_filter_cutoff, 0) <<                \
...
#define MDF_FILTERS_DEFINE(flt)                                                                    \
	...
	static const struct dmic_stm32_mdf_flt_cfg dmic_stm32_mdf_cfg_##flt = {                    \
		...
		.parent = DEVICE_DT_GET(DT_PARENT(flt)),                                           \
                                                                                                   ...
                                                                                                   \
	BUILD_ASSERT(DT_PROP(flt, gain) % 3 == 0, "gain must be a multiple of 3 dB");              \
                                                                                                   \
	static const MDF_FilterConfigTypeDef config_##flt = {                                      \
		.DataSource = MDF_DATA_SOURCE_BSMX,                                                \
		.Delay = CONFIG_DMIC_STM32_MDF_DELAY,                                              \
		...
		.DiscardSamples = CONFIG_DMIC_STM32_MDF_DISCARD_SAMPLES,                           \
	};                                                                                         \
                                                                                                   \
	static struct dmic_stm32_mdf_data dmic_stm32_mdf_data_##flt = {                            \
		.hmdf = {                                                                          \
			...
			.Init = {                                                                  \
				...
				.FilterBistream = UTIL_CAT(UTIL_CAT(MDF_BITSTREAM,                 \
								    DT_NODE_CHILD_IDX(flt)),       \
							   _FALLING),                              \
			},                                                                         \
		},                                                                                 \
