 ...
 *
 * It is a register byte-store, not a behavioural model: it verifies that the driver
 * emits the writes it is expected to, not that those values suit real silicon.
 */
...
struct es8311_emul_data {
	...
	/*
	 * Break only reads. A read here is a write-then-read with a repeated start, which
	 * can fail while a plain register write still works, and that is the failure that
	 * turns a read-modify-write mute into no mute.
	 */
	bool fail_reads;
	/*
	 * Break the write to THIS register. Breaking transfer number N counts reads too,
	 * so one added register read upstream re-aims the fault at a different write.
	 * Negative when disarmed: 0x00 is a real register and cannot be the sentinel.
	 */
	int fail_write_reg;
	/*
				...
				int addr)
{
	...
	if (num_msgs == 1) {
		...
		/*
		 * No register-file reset is modelled: RST_DIG is documented as "reset digital
		 * except control port block", and the control port block is where the registers
		 * live, so no value of 0x00 resets the register file on the part. 0x00 is
		 * stored like any other register.
		 */
...
/*
 * Put the emulated part back as it comes up: a zeroed register file carrying the chip
 * identity, every fault hook disarmed, an empty write log. Anything left armed or dirty
 * becomes the next case's starting state, and that case can pass because of it. This
 * cannot reach the driver's cached properties; the fixture resets those through the API.
 */
