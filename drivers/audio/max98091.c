...
#define DEV_CFG(dev) ((const struct max98091_config *const)dev->config)

static void max98091_write_reg(const struct device *dev, uint8_t reg, uint8_t val)
{
	const struct max98091_config *const dev_cfg = DEV_CFG(dev);
	uint8_t data[2];
	int ret;
...
static void max98091_read_reg(const struct device *dev, uint8_t reg, uint8_t *val)
{
	...
	if (ret == 0) {
		*val = value;
		...
		LOG_ERR("i2c read to codec error %d", ret);
	}
}
...
static void max98091_update_reg(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
	...
	max98091_read_reg(dev, reg, &old_val);

	new_val = (old_val & ~mask) | (val & mask);

	if (new_val != old_val) {
		max98091_write_reg(dev, reg, new_val);
	}
}
...
static int max98091_init(const struct device *dev)
{
	...
	if (!device_is_ready(cfg_tan->i2c.bus)) {
