...
#include <zephyr/drivers/audio/cs35l56.h>
...
enum cs35l56_supply {
	CS35L56_SUPPLY_VDD_P,
	CS35L56_SUPPLY_VDD_IO,
	CS35L56_SUPPLY_VDD_A,
	CS35L56_SUPPLY_VDD_B,
	CS35L56_SUPPLY_VDD_AMP,
	CS35L56_NUM_SUPPLIES,
};

static const char *const cs35l56_supply_names[CS35L56_NUM_SUPPLIES] = {
	[CS35L56_SUPPLY_VDD_P] = "VDD_P",     [CS35L56_SUPPLY_VDD_IO] = "VDD_IO",
	[CS35L56_SUPPLY_VDD_A] = "VDD_A",     [CS35L56_SUPPLY_VDD_B] = "VDD_B",
	[CS35L56_SUPPLY_VDD_AMP] = "VDD_AMP",
};
...
struct cs35l56_config {
	...
	const struct device *supplies[CS35L56_NUM_SUPPLIES];
...
static int cs35l56_route_input(const struct device *dev, audio_channel_t channel, uint32_t input)
{
	...
	switch ((int)channel) {
	case CS35L56_CHANNEL_ASP1_TX1:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL1,
				     CS35L56_ASP1_TX1_SLOT, input);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_TX1_EN,
				      CS35L56_ASP1_TX1_EN);

	case CS35L56_CHANNEL_ASP1_TX2:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL1,
				     CS35L56_ASP1_TX2_SLOT, input << CS35L56_ASP1_TX2_SHIFT);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_TX2_EN,
				      CS35L56_ASP1_TX2_EN);

	case CS35L56_CHANNEL_ASP1_TX3:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL1,
				     CS35L56_ASP1_TX3_SLOT, input << CS35L56_ASP1_TX3_SHIFT);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_TX3_EN,
				      CS35L56_ASP1_TX3_EN);

	case CS35L56_CHANNEL_ASP1_TX4:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL1,
				     CS35L56_ASP1_TX4_SLOT, input << CS35L56_ASP1_TX4_SHIFT);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_TX4_EN,
				      CS35L56_ASP1_TX4_EN);
...
static int cs35l56_route_output(const struct device *dev, audio_channel_t channel, uint32_t output)
{
	...
	case AUDIO_CHANNEL_FRONT_LEFT:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL5,
				     CS35L56_ASP1_RX1_SLOT, output);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_RX1_EN,
				      CS35L56_ASP1_RX1_EN);

	case AUDIO_CHANNEL_FRONT_RIGHT:
		ret = cs35lxx_update(&config->io_bus, CS35L56_ASP1_FRAME_CONTROL5,
				     CS35L56_ASP1_RX2_SLOT, output << CS35L56_ASP1_RX2_SHIFT);
		if (ret < 0) {
			return ret;
		}

		return cs35lxx_update(&config->io_bus, CS35L56_ASP1_ENABLES1, CS35L56_ASP1_RX2_EN,
				      CS35L56_ASP1_RX2_EN);
				 ...
				 audio_property_value_t audio_val)
{
	...
	return cs35lxx_write(&config->io_bus, CS35L56_MAIN_RENDER_USER_VOLUME,
			       ...
			       audio_property_value_t audio_val)
{
	...
	return cs35lxx_write(&config->io_bus, CS35L56_MAIN_RENDER_USER_MUTE,
...
static void cs35l56_stop_output(const struct device *dev)
{
	...
	(void)cs35lxx_write(&config->io_bus, CS35L56_DSP_VIRTUAL1_MBOX_1,
...
static int cs35l56_asp1_set_clks(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	ret = cs35lxx_write(&config->io_bus, CS35L56_ASP1_CONTROL1, asp1_bclk_freq);
	if (ret < 0) {
		return ret;
	}
	...
	return cs35lxx_update(&config->io_bus, CS35L56_ASP1_CONTROL2, CS35L56_BCLK_FSYNC_MASK,
			      clk_opt);
}
...
static int cs35l56_asp1_set_word(const struct device *dev, struct audio_codec_cfg *cfg)
{
	...
	case AUDIO_ROUTE_PLAYBACK_CAPTURE:
		ret = cs35lxx_write(&config->io_bus, CS35L56_ASP1_DATA_CONTROL1, i2s.word_size);
		if (ret < 0) {
			return ret;
		}
	...
	case AUDIO_ROUTE_PLAYBACK:
		ret = cs35lxx_write(&config->io_bus, CS35L56_ASP1_DATA_CONTROL5, i2s.word_size);
		if (ret < 0) {
			return ret;
		}
	...
	return cs35lxx_update(&config->io_bus, CS35L56_ASP1_CONTROL2,
...
{
	__maybe_unused const struct cs35l56_config *config = dev->config;
	int ret;
...
static int cs35l56_wait_for_rom_boot(const struct device *dev)
{
	...
	do {
		ret = cs35lxx_read(&config->io_bus, CS35L56_HALO_STATE, &val);
		if (ret < 0) {
			return ret;
		}
...
static int cs35l56_reset(const struct device *dev)
{
	...
	if (config->reset_gpio.port != NULL) {
		 ...
		 * reset conditions.
		 */
		ret = cs35lxx_write(&config->io_bus, CS35L56_SW_RESET_SFT_RESET_REG, 0x5A000000);
		if (ret < 0) {
			return ret;
		}
	}
...
static int cs35l56_check_ids(const struct device *dev)
{
	...
	ret = cs35lxx_read(&config->io_bus, CS35L56_SW_RESET_DEVID_REG, &val);
	if (ret < 0) {
		return ret;
	}
	...
	ret = cs35lxx_read(&config->io_bus, CS35L56_SW_RESET_REVID_REG, &val);
	if (ret < 0) {
		return ret;
	}
...
static int cs35l56_enable_regulator(const struct device *dev, const int i)
{
	const struct cs35l56_config *config = dev->config;
	...
	if (regulator == NULL) {
		LOG_INST_DBG(config->log, "%s regulator not provided", cs35l56_supply_names[i]);
		return 0;
	}

	if (!device_is_ready(regulator)) {
		LOG_INST_ERR(config->log, "%s regulator device %s is not ready",
			     cs35l56_supply_names[i], regulator->name);
		return -ENODEV;
	}
	...
	if (ret < 0) {
		LOG_INST_ERR(config->log, "Failed to enable %s regulator: %d",
			     cs35l56_supply_names[i], ret);
	}

	return ret;
}

static void cs35l56_disable_regulator(const struct device *dev, const int i)
{
	const struct cs35l56_config *config = dev->config;
	const struct device *regulator = config->supplies[i];
...
static void cs35l56_disable_regulators(const struct device *dev)
{
	for (int i = 0; i < ARRAY_SIZE(cs35l56_supply_names); i++) {
		cs35l56_disable_regulator(dev, i);
	}
}
...
static int cs35l56_init_regulators(const struct device *dev)
{
	...
	for (int i = 0; i < ARRAY_SIZE(cs35l56_supply_names); i++) {
		ret = cs35l56_enable_regulator(dev, i);
		if (ret < 0) {
			return ret;
		}
	}
...
static int cs35l56_init(const struct device *dev)
{
	...
	ret = cs35lxx_update(&config->io_bus, CS35L56_BLOCK_ENABLES2, CS35L56_ASP_EN,
			     CS35L56_ASP_EN);
	if (ret < 0) {
		goto error_regulators;
	}
...
error_regulators:
	cs35l56_disable_regulators(dev);
...
#define CS35L56_SUPPLIES(inst)                                                                     \
	[CS35L56_SUPPLY_VDD_P] = DEVICE_DT_GET_OR_NULL(DT_INST_PHANDLE(inst, vdd_p_supply)),       \
	[CS35L56_SUPPLY_VDD_IO] = DEVICE_DT_GET_OR_NULL(DT_INST_PHANDLE(inst, vdd_io_supply)),     \
	[CS35L56_SUPPLY_VDD_A] = DEVICE_DT_GET_OR_NULL(DT_INST_PHANDLE(inst, vdd_a_supply)),       \
	[CS35L56_SUPPLY_VDD_B] = DEVICE_DT_GET_OR_NULL(DT_INST_PHANDLE(inst, vdd_b_supply)),       \
	[CS35L56_SUPPLY_VDD_AMP] = DEVICE_DT_GET_OR_NULL(DT_INST_PHANDLE(inst, vdd_amp_supply))

#define CS35L56_CONFIG(inst, id)                                                                   \
	{                                                                                          \
		LOG_INSTANCE_PTR_INIT(log, DT_NODE_FULL_NAME_TOKEN(DT_DRV_INST(inst)), inst)       \
			CS35L56_BUS(inst),                                                         \
		.supplies = {CS35L56_SUPPLIES(inst)},                                              \
		.reset_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, reset_gpios, {0}),                    \
		.device_id = id,                                                                   \
	}

#define AUDIO_CODEC_CS35L56_DEFINE(inst, name, id)                                                 \
	LOG_INSTANCE_REGISTER(DT_NODE_FULL_NAME_TOKEN(DT_DRV_INST(inst)), inst,                    \
			      CONFIG_AUDIO_CODEC_LOG_LEVEL);                                       \
	static const struct cs35l56_config name##_config_##inst = CS35L56_CONFIG(inst, id);        \
