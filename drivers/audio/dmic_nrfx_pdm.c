...
static bool check_pdm_frequencies(const struct dmic_nrfx_pdm_drv_cfg *drv_cfg,
				  nrfx_pdm_config_t *config, const struct dmic_cfg *pdm_cfg,
				  uint8_t ratio, uint32_t *best_diff, uint32_t *best_rate,
				  uint32_t *best_freq)
{
	...
	if (IS_ENABLED(CONFIG_SOC_SERIES_NRF53X)) {
		...
		} freqs[] = {{1000000, NRF_PDM_FREQ_1000K}, {1032000, NRF_PDM_FREQ_1032K},
...
static int dmic_nrfx_pdm_configure(const struct device *dev, struct dmic_cfg *config)
{
	struct dmic_nrfx_pdm_drv_data *drv_data = dev->data;
...
#define PDM_NRFX_DEVICE(idx)                                                                       \
	...
	DEVICE_DT_DEFINE(PDM(idx), pdm_nrfx_init##idx, NULL, &dmic_nrfx_pdm_data##idx,             \
			 &dmic_nrfx_pdm_cfg##idx, POST_KERNEL, CONFIG_AUDIO_DMIC_INIT_PRIORITY,    \
			 &dmic_ops);
