...
int nsos_adapt_host_ifindex(const char *name)
{
	...
	if (name != NULL && name[0] != '\0') {
		...
		if (ifindex == 0) {
			return -nsi_errno_to_mid(ENODEV);
		}
	}
	return (int)ifindex;
}
			  ...
			  const void *nsos_mid_optval, size_t nsos_mid_optlen)
{
	...
	case NSOS_MID_IPPROTO_IPV6:
		switch (nsos_mid_optname) {
		case NSOS_MID_IPV6_MULTICAST_HOPS:
			return nsos_adapt_setsockopt_int(fd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS,
							 nsos_mid_optval, nsos_mid_optlen);
...
/* Counts set bits, assuming the host netmask is contiguous (always true for a
...
int nsos_adapt_get_ifaddrs(int ifindex, struct nsos_mid_ifaddr *addrs, size_t *count)
{
	...
	/* Mirror only the represented host interface, and only up to capacity so
	 * the caller can detect and report truncation.
	 */
	for (struct ifaddrs *ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
