...
static int mec5_config_pin(uint32_t pinmux, uint32_t altf)
{
	...
	ret = mec_gpio_pin_num(MCHP_XEC_PINMUX_PORT(pinmux), MCHP_XEC_PINMUX_PIN(pinmux), &pin);
	if (ret) {
		return -EINVAL;
	}

	ret = mec_gpio_set_props(pin, cfg1, ARRAY_SIZE(cfg1));
	if (ret) {
		return -EIO;
	}
	...
	ret = mec_gpio_set_props(pin, cfg2, idx);
	if (ret) {
		return -EIO;
	}

	/* make output state in control read-only in control and read-write in parallel reg */
	ret = mec_gpio_set_property(pin, MEC_GPIO_OSEL_PROP_ID, MEC_GPIO_PROP_OSEL_PAROUT);
	if (ret) {
		return -EIO;
	}
