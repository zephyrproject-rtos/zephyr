...
static const uint32_t sam_port_addrs[] = {
	SAM_PORT_ADDR_OR_NONE(porta) SAM_PORT_ADDR_OR_NONE(portb) SAM_PORT_ADDR_OR_NONE(portc)
		SAM_PORT_ADDR_OR_NONE(portd) SAM_PORT_ADDR_OR_NONE(porte)
...
static void pinctrl_configure_pin(pinctrl_soc_pin_t pin)
{
	...
	if (port_func == SAM_PINMUX_FUNC_periph) {
		soc_pin.flags |=
			(SAM_PINMUX_PERIPH_GET(pin) << SOC_PORT_FUNC_POS) | SOC_PORT_PMUXEN_ENABLE;
	}
