...
static void pinctrl_configure_pin(pinctrl_soc_pin_t pincfg)
{
	...
	if (pincfg.pin >= RTS_FP_PIN_AL0 && pincfg.pin <= RTS_FP_PIN_AL2) {
		...
		value &= ~((BIT(pincfg.pin - RTS_FP_PIN_AL0 + AL_GPIO_PU_CTRL_OFFSET)) |
			   (BIT(pincfg.pin - RTS_FP_PIN_AL0 + AL_GPIO_PD_CTRL_OFFSET)) |
			   (BIT(pincfg.pin - RTS_FP_PIN_AL0 + AL_GPIO_SEL_OFFSET)));

		if (pincfg.pin == RTS_FP_PIN_AL0) {
			value &= ~(CS1_BRIDGE_EN_MASK);
		}
