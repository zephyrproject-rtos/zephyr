/*
 * SPDX-FileCopyrightText: 2026 ABOV Semiconductor Co., Ltd.
 *
...
static void pinctrl_configure_pin(pinctrl_soc_pin_t pin)
{
	 ...
	 *    chip (confirmed against the datasheet), so the pin's OTYPE
	 *    encoding only matters for plain-GPIO pins, not here.
	 */
	if (pin_num < 8U) {
