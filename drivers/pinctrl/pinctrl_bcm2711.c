...
#define BCM2711_PINCTRL_BASE_ADDR DT_REG_ADDR(DT_NODELABEL(pinctrl))
...
static void bcm2711_pinctrl_set_func(uintptr_t base, uint32_t pin, uint32_t func)
{
...
static void bcm2711_pinctrl_set_pull(uintptr_t base, uint32_t pin, uint32_t pull)
{
...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	if (IS_ENABLED(CONFIG_MMU)) {
		...
		base = BCM2711_PINCTRL_BASE_ADDR;
	}
