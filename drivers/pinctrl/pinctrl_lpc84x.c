...
static int lpc_pinctrl_apply_swm(SWM_Type *base, pinctrl_soc_pin_t pin)
{
...
static int lpc_pinctrl_apply_iocon(IOCON_Type *base, pinctrl_soc_pin_t pin)
{
	uint8_t cfg = LPC84X_IOCON_CFG(pin.iocon_cfg);
	uint8_t iocon_idx = LPC84X_IOCON_INDEX(pin.iocon_cfg);

	if (iocon_idx >= IOCON_PIO_COUNT) {
		return -EINVAL;
	}
...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	for (uint8_t i = 0; i < pin_cnt; i++) {
		...
		ret = lpc_pinctrl_apply_swm(swm_base, pins[i]);
		if (ret < 0) {
			return ret;
		}
		ret = lpc_pinctrl_apply_iocon(iocon_base, pins[i]);
		if (ret < 0) {
			return ret;
		}
	}
