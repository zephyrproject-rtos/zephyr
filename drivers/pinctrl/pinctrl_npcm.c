...
#define NPCM_PINMUX_INIT(node_id, prop)                                                            \
	COND_CODE_1( \
		DT_PROP(node_id, nuvoton_pinmux_group_def), \
...
struct npcm_periph_pupd_cfg {
	...
	uint16_t pupd_type0: 2; /* [0]=pull-up, [1]=pull-down */
	uint16_t pupd_reg1: 11; /* Pull-up/down config reg field index1 */
	uint16_t pupd_type1: 2; /* [0]=pull-up, [1]=pull-down */
} __packed;

#define NPCM_PIN_PERIPH_PUPD_GROUP_RDATA(pin, sup, reg0, type0, reg1, type1)                       \
	{                                                                                          \
		.id = pin,                                                                         \
		.pupd_sup = sup,                                                                   \
		.pupd_reg0 = reg0,                                                                 \
...
#define NPCM_PIN_PERIPH_PUPD_TYPE0(node_id, prop) DT_PROP_BY_IDX(node_id, prop, 0)
#define NPCM_PIN_PERIPH_PUPD_REG1(node_id, prop)  DT_PROP_BY_IDX(node_id, prop, 1)
#define NPCM_PIN_PERIPH_PUPD_TYPE1(node_id, prop) DT_PROP_BY_IDX(node_id, prop, 1)
#define NPCM_PIN_PERIPH_PUPD_GROUP_INIT(node_id)                                                   \
	NPCM_PIN_PERIPH_PUPD_GROUP_RDATA(NPCM_PINMUX_ID(node_id, nuvoton_id),                      \
					 NPCM_PIN_PERIPH_PUPD_SUP(node_id, nuvoton_pupd_sup),      \
					 NPCM_PIN_PERIPH_PUPD_REG0(node_id, nuvoton_pupd_reg),     \
					 NPCM_PIN_PERIPH_PUPD_TYPE0(node_id, nuvoton_pupd_type),   \
					 NPCM_PIN_REG_NULL, 0)
#define NPCM_PIN_PERIPH_PUPD_GROUP_EXT_INIT(node_id)                                               \
	NPCM_PIN_PERIPH_PUPD_GROUP_RDATA(NPCM_PINMUX_ID(node_id, nuvoton_id),                      \
					 ...
					 NPCM_PIN_PERIPH_PUPD_REG0(node_id, nuvoton_pupd_reg),     \
					 NPCM_PIN_PERIPH_PUPD_TYPE0(node_id, nuvoton_pupd_type),   \
					 NPCM_PIN_PERIPH_PUPD_REG1(node_id, nuvoton_pupd_reg),     \
					 NPCM_PIN_PERIPH_PUPD_TYPE1(node_id, nuvoton_pupd_type))

#define NPCM_PIN_PERIPH_PUPD_INIT(node_id, prop)                                                   \
	COND_CODE_1( \
		DT_PROP(node_id, nuvoton_pinmux_group_def), \
		(COND_CODE_1( \
			DT_NODE_HAS_PROP(node_id, nuvoton_pupd_reg), \
			(COND_CODE_1( \
				DT_PROP(node_id, nuvoton_pupd_config_extend), \
...
struct npcm_pin_lv_cfg {
	...
	uint16_t level: 1;   /* 0 = normal 3.3V, 1 = low-voltage 1.8V */
} __packed;
...
#define NPCM_PIN_LV_INIT(node_id, prop)                                                            \
	COND_CODE_1( \
		DT_PROP(node_id, nuvoton_pinmux_group_def), \
		(COND_CODE_1( \
			DT_NODE_HAS_PROP(node_id, nuvoton_lv_reg), \
			(NPCM_PIN_LV_GROUP_RDATA( \
				NPCM_PINMUX_ID(node_id, nuvoton_id), \
				NPCM_PIN_LV_REG(node_id, nuvoton_lv_reg), \
				NPCM_PIN_LV_SUP(node_id, nuvoton_lv_sup), \
				NPCM_PIN_LV_LOW_LEVEL(node_id, nuvoton_lv_def) \
				) \
			), \
			()) \
		), \
...
static void npcm_periph_pupd_reg_set(uint16_t bias_type_set, uint16_t io, uint8_t reg_idx)
{
	...
	if (reg_idx == 0) {
		...
		pupd_type = npcm_periph_pupd_table[io].pupd_type0;
	} else if (reg_idx == 1) {
		pupd_reg = npcm_periph_pupd_table[io].pupd_reg1;
		pupd_type = npcm_periph_pupd_table[io].pupd_type1;
	} else {
		/* Invalid index */
...
static int npcm_volt_level_configure(const struct npcm_pinctrl *pin)
{
	...
	if (npcm_pin_lv_table[i].lv_sup & BIT(pin->props.group)) {
		/* Set different level */
		if (npcm_pin_lv_table[i].level != set_level) {
			if (set_level == NPCM_IO_VOLT_LV_1V8) {
			...
			/* Record pin level */
			npcm_pin_lv_table[i].level = set_level;
		}
	}
