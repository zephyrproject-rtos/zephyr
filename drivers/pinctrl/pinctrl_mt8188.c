...
static int pinctrl_set_func(uint16_t pin, uint16_t func)
{
	...
	val = sys_read32(pinctrl_base + offset);
	val &= ~(to_mode_mask(pin));
	val |= ((uint32_t)func) << to_mode_shift(pin);

	sys_write32(val, (pinctrl_base + offset));

	return 0;
}

static int pinctrl_mt8188_init(void)
{
	device_map(&pinctrl_base, DT_INST_REG_ADDR(0), DT_INST_REG_SIZE(0), K_MEM_CACHE_NONE);

	return 0;
}

/*
 * Priority 0 so the window is mapped before any device applies its pin
 * configuration: pinctrl_configure_pins() runs from driver init at this same
 * level, and the drivers that call it are all at a later priority.
 */
SYS_INIT(pinctrl_mt8188_init, PRE_KERNEL_1, 0);
