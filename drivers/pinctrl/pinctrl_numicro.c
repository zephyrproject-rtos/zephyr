...
static const uint32_t gpio_port_addrs[] = {
	NUMICRO_PORT_ADDR_OR_NONE(gpioa) NUMICRO_PORT_ADDR_OR_NONE(gpiob)
		NUMICRO_PORT_ADDR_OR_NONE(gpioc) NUMICRO_PORT_ADDR_OR_NONE(gpiod)
			NUMICRO_PORT_ADDR_OR_NONE(gpioe) NUMICRO_PORT_ADDR_OR_NONE(gpiof)
				NUMICRO_PORT_ADDR_OR_NONE(gpiog) NUMICRO_PORT_ADDR_OR_NONE(gpioh)};
...
static int gpio_configure(const pinctrl_soc_pin_t *pin)
{
	...
	port->DBEN = (port->DBEN & ~BIT(pin_idx)) | ((pin->input_debounce ? 1 : 0) << pin_idx);
	port->SMTEN = (port->SMTEN & ~BIT(pin_idx)) | ((pin->schmitt_trigger ? 1 : 0) << pin_idx);
	port->DINOFF = (port->SMTEN & ~DINOFF_MASK(pin_idx)) |
		       ((pin->input_disable ? 1 : 0) << DINOFF_PIN_SHIFT(pin_idx));
	port->PUSEL = (port->PUSEL & ~PUSEL_MASK(pin_idx)) | (bias << PUSEL_PIN_SHIFT(pin_idx));
...
int pinctrl_configure_pins(const pinctrl_soc_pin_t *pins, uint8_t pin_cnt, uintptr_t reg)
{
	...
	for (uint8_t i = 0U; i < pin_cnt; i++) {
		...
		REG_MFP(port, pin) = (REG_MFP(port, pin) & ~MFP_CTL(pin, 0xf)) | MFP_CTL(pin, mfp);
