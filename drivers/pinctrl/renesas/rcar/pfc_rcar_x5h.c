...
/* Protect per-pin configuration sequence */
...
static void pfc_rcar_set_mode(uintptr_t pfc_base, uint8_t bit, bool peripheral)
{
...
static void pfc_rcar_set_func(uintptr_t pfc_base, const struct rcar_pin_func *rcar_func)
{
	uint32_t reg, val;
...
static const struct pfc_drive_reg *pfc_rcar_get_drive_reg(uint16_t pin, uint8_t *bit)
{
...
static int pfc_rcar_set_bias(uintptr_t pfc_base, uint16_t pin, uint16_t flags)
{
	...
	puen_val = sys_read32(pfc_base + bias_reg->puen);
	pfc_rcar_write(pfc_base, bias_reg->puen, puen_val & ~BIT(bit));
	if (!(flags & RCAR_PIN_FLAGS_PUEN)) {
		return 0;
	}

	/* Select either pull-up or pull-down */
	pud_val = sys_read32(pfc_base + bias_reg->pud);
	if (flags & RCAR_PIN_FLAGS_PUD) {
		pud_val |= BIT(bit);
	} else {
		pud_val &= ~BIT(bit);
	}
	pfc_rcar_write(pfc_base, bias_reg->pud, pud_val);
...
static int pinctrl_configure_pin(const pinctrl_soc_pin_t *pin)
{
	 ...
	 *
	 * For Peripheral mode setting, the documented sequence is as follow :
	 * - If the pin is currently in GPIO mode, we must select the function first and only
	 *   then switch to function mode.
	 * - If the pin is already in function mode and we are changing to another function,
	 *   we must switch back to GPIO mode first, then select the new function, then switch
	 *   to function mode again.
	 *
	 * Since the reset/initial mode value is group-dependent, the driver also cannot assume
	 * that the pin starts in GPIO mode. Thus, let's normalize the direct-MMIO path to the
	 * documented sequence as follow for any function state:
	 *   [1] Set GPIO mode
	 *   [2] Set function ID i.e. Program ALTSEL
	 *   [3] Set Peripheral mode
	 */
	...
	if (pin->drive_strength_microamp != 0) {
		ret = pfc_rcar_set_drive_strength(pfc_base, pin_id, pin->drive_strength_microamp);
	}
...
static int pinctrl_configure_pin(const pinctrl_soc_pin_t *pin)
{
	...
	/*
	 * Peripheral function or GPIO mode.
	 * The SCMI "no function" value is treated by this platform (SCP FW) as the
	 * GPIO/no-peripheral mux state. Pinctrl selects the no-function/GPIO mux state,
	 * while direction/value are expected to be handled later by the R-Car GPIO driver.
	 */
	settings.function = (pin->flags & RCAR_PIN_FLAGS_FUNC_SET) ?
			     pin->func.func : SCMI_PINCTRL_NO_FUNCTION;
	...
	if (pin->flags & RCAR_PIN_FLAGS_PUEN) {
		settings.config[config_cnt++] = SCMI_PINCTRL_PULL_MODE;
		settings.config[config_cnt++] = (pin->flags & RCAR_PIN_FLAGS_PUD) ? 1 : 2;
	}

	/*
	 * Attributes.
	 * Function ID is always valid :
	 * - For peripheral mode, ID is in range [0 ; 15].
	 * - For GPIO mode, ID = SCMI_PINCTRL_NO_FUNCTION.
	 */
	settings.attributes =
		SCMI_PINCTRL_CONFIG_ATTRIBUTES(1, config_cnt >> 1, SCMI_PINCTRL_SELECTOR_PIN);
