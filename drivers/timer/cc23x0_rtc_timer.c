...
{
	sys_clock_ticks_t elapsed_ticks = get_elapsed_ticks_rtc(HWREG(RTC_BASE + RTC_O_TIME8U)) /
...
static void rtc_isr(const void *arg)
{
	...
	sys_clock_ticks_t elapsed_ticks = get_elapsed_ticks_rtc(current_rtc_count);
