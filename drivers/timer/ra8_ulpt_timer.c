...
#define SYS_CLOCK_HW_CYCLES_PER_SEC (32768U)
...
uint32_t sys_clock_cycle_get_32(void)
{
	...
	uint32_t lp_time = accumulated_ulpt_cnt;

	/* convert lptim count in a nb of hw cycles with precision */
	uint64_t ret =
		((uint64_t)lp_time * sys_clock_hw_cycles_per_sec()) / ulpt_info.clock_frequency;
...
static int sys_clock_driver_init(void)
{
	...
	(void)R_ULPT_Start(&g_ulpt_ctrl);
	err = R_ULPT_InfoGet(&g_ulpt_ctrl, &ulpt_info);
	if (err != 0) {
		return err;
	}
