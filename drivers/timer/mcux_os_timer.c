 ...
 * than that window is passed before it takes effect. The window is counted in
 * OSTimer cycles, so the lead does not scale with the rate.
 */
#define TIMER_CORE_ALARM_LEAD_CYCLES 16U
...
static inline void timer_driver_set_compare(uint64_t cycles)
{
	...
	/* A new deadline supersedes both the overflow remainder and the wait
	 * forever state carried from the last one, so the companion recomputes
	 * from MATCH rather than resuming a chain the kernel has just replaced.
	 * sys_clock_no_timeout() sets wait_forever after calling this, so its
	 * own arming is not cleared here.
	 */
