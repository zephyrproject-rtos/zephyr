...
static void timer_driver_set_reload(uint32_t cycles)
{
	...
	for (uint32_t retry = 0U; retry < PIT_ARM_RETRY_MAX; retry++) {
		...
		piv = MIN(cpiv + cycles, PIT_MAX_PIV);
	...
	/* Sync data->piv, without this the cycle count would drift silently when assert is off. */
	data->piv = piv;
...
static void mchp_pit_isr(const void *arg)
{
	...
	data->accumulated_cycles += elapsed_ticks * (data->piv + 1);
