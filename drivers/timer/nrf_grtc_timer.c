...
static uint64_t cc_val; /* Value that is expected to be in CC register. */
...
static void sys_clock_timeout_handler(int32_t id, uint64_t unused, void *p_context)
{
	ARG_UNUSED(id);
...
#else
	err_code = nrfx_grtc_channel_cb_alloc(&system_clock_channel_data.channel,
				      sys_clock_timeout_handler, NULL);
	if (err_code != NRFX_SUCCESS) {
		return -ENOMEM;
	}
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	if (in_announce) {
		 ...
		 */
		cc_val = last_count + last_elapsed + cyc;
