...
static void lptimer_enable_event(const struct device *dev, bool enable)
{
...
static void lptimer_set_delay(const struct device *dev, uint32_t delay)
{
	const struct ifx_cat1_lptimer_config *config = dev->config;
	struct ifx_cat1_lptimer_data *data = dev->data;
	...
	Cy_MCWDT_ClearInterrupt(config->reg_addr, CY_MCWDT_CTR1);
	c0_old_match = (uint16_t)Cy_MCWDT_GetMatch(config->reg_addr, CY_MCWDT_COUNTER0);
	...
	c0_match = (uint16_t)(c0_current_ticks + delay);
	c1_current_ticks = (uint16_t)Cy_MCWDT_GetCount(config->reg_addr, CY_MCWDT_COUNTER1);
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	cycles_per_tick = config->clock_frequency / CONFIG_SYS_CLOCK_TICKS_PER_SEC;

	/* Get current cycle count from the free-running counter */
	key = k_spin_lock(&lock);
	current_cycles = Cy_MCWDT_GetCount(config->reg_addr, CY_MCWDT_COUNTER2);
...
uint32_t sys_clock_cycle_get_32(void)
{
	...
	cycles = Cy_MCWDT_GetCount(config->reg_addr, CY_MCWDT_COUNTER2);
...
static void lptimer_isr(const struct device *dev)
{
	...
	Cy_MCWDT_ClearInterrupt(config->reg_addr, LPTIMER_COUNTERS);
	...
	if ((data->isr_instruction & LPTIMER_ISR_CALL_USER_CB_MASK) != 0) {
		...
		uint32_t current_cycles = Cy_MCWDT_GetCount(config->reg_addr, CY_MCWDT_COUNTER2);
		uint32_t cycles_per_tick = config->clock_frequency / CONFIG_SYS_CLOCK_TICKS_PER_SEC;
		uint32_t delta_cycles;
		uint32_t delta_ticks;
		k_spinlock_key_t key = k_spin_lock(&lock);
...
static int lptimer_init(const struct device *dev)
{
	...
	if (rslt == CY_RSLT_SUCCESS) {
		Cy_MCWDT_Enable(config->reg_addr, LPTIMER_COUNTERS, LPTIMER_RESET_TIME_US);
	} else {
		Cy_MCWDT_Disable(config->reg_addr, LPTIMER_COUNTERS, LPTIMER_RESET_TIME_US);
