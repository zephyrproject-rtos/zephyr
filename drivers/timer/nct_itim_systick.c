...
#define DT_DRV_COMPAT nuvoton_nct_itim_systick

/**
 * @file
 * @brief Nuvoton NCT kernel device driver for  "system clock driver" interface
 *
 * This file contains a kernel device driver implemented by the internal
 * 32-bit timers in Nuvoton NCT series.
 *
 */
...
LOG_MODULE_REGISTER(nct_itim_systick, LOG_LEVEL_ERR);

#define NCT_ITIM32_MAX_CNT 0xffffffff
...
static const struct nct_clk_cfg itim_clk_cfg = DT_INST_PHA(0, clocks, clk_cfg);

static struct k_spinlock lock;

/*
 * This local variable holds the amount of SysTick HW cycles elapsed
 * and it is updated in nct_itim32_sys_isr() and sys_clock_set_timeout().
 *
 * Note:
 *  At an arbitrary point in time the "current" value of the SysTick
 *  HW timer is calculated as:
 *
 * t = sys_cycle_count + nct_itim_elapsed();
 */
 ...
 * in nct_itim_elapsed() function, as well as in the updates to
 * sys_cycle_count.
 *
 * Note:
...
static inline uint64_t nct_itim_elapsed(void)
{
	...
	if ((itsts & BIT(NCT_ITCTS_TO_STS))
		 ...
		 */
		sys_tmr->ITCTS |= BIT(NCT_ITCTS_TO_STS);
	}

	return (last_timeout_cycs - val2) + overflow_sys_cycs;
}

static inline void nct_itim_sys_enable(void)
{
	/* Enable sys timer and wait for it to take effect */
	sys_tmr->ITCTS |= BIT(NCT_ITCTS_ITEN);

	/* Wait until sys timer enable */
	while (!IS_BIT_SET(sys_tmr->ITCTS, NCT_ITCTS_ITEN));
}

static inline void nct_itim_sys_disable(void)
{
...
static int nct_itim_start_sys_tmr_by_tick(int32_t ticks)
{
	...
	elapsed_cycles = nct_itim_elapsed();
	...
	if (ticks == K_TICKS_FOREVER) {
		sys_cycs_timeout = NCT_ITIM32_MAX_CNT;
	}else {
		if (ticks <= 0) {
			ticks = 1;
		}
		...
		if (unlikely(next_cycs <= sys_cycle_count)) {
			...
			sys_cycs_timeout = CLAMP(dcycles, 1, NCT_ITIM32_MAX_CNT);
		}
	}
	...
	if (IS_BIT_SET(sys_tmr->ITCTS, NCT_ITCTS_ITEN)) {
		nct_itim_sys_disable();

		if (sys_tmr->ITCTS & BIT(NCT_ITCTS_TO_STS)) {
			sys_tmr->ITCTS |= BIT(NCT_ITCTS_TO_STS);
		}
	}
...
static void nct_itim_sys_isr(const struct device *dev)
{
	...
	/* Suppose NCT_ITCTS_TO_STS bit clear by nct_itim_elapsed()  */
	sys_cycle_count += nct_itim_elapsed();
	overflow_sys_cycs = 0;
...
void sys_clock_set_timeout(int32_t ticks, bool idle)
{
	...
	nct_itim_start_sys_tmr_by_tick(ticks);
}
...
static void nct_itim_init_global_value(void)
{
	/* setup timeout cycles */
	if (!IS_ENABLED(CONFIG_TICKLESS_KERNEL)) {
		last_timeout_cycs = SYS_CYCLES_PER_TICK;
        } else {
		last_timeout_cycs = NCT_ITIM32_MAX_CNT;
	}
...
static int sys_clock_driver_init(void)
{
	...
	/*
	 * In nct series, we use ITIM32 as system kernel timer. Its source
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
					nct_itim_sys_isr, NULL, 0);
	/* Enable sys timer interrupt */
	irq_enable(DT_INST_IRQN(0));

	nct_itim_init_global_value();
