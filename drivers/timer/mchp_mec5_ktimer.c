...
{
	uint32_t ccr = mec_rtimer_count(rtimer);

	if ((ccr == 0) && mec_rtimer_is_started(rtimer)) {
		ccr = cached_icr;
	}
...
void sys_clock_set_timeout(int32_t n, bool idle)
{
	...
	mec_rtimer_stop(rtimer);
	mec_rtimer_status_clear_all(rtimer);
	...
	mec_rtimer_stop_and_load(rtimer, temp, RTIMER_START_VAL);
...
static void mec5_ktimer_isr(const void *arg)
{
	...
	mec_rtimer_stop_and_load(rtimer, MAX_TICKS * CYCLES_PER_TICK, RTIMER_START_VAL);
...
static void mec5_ktimer_isr(const void *arg)
{
	...
	/* Restart the timer as early as possible to minimize drift... */
	mec_rtimer_stop_and_load(rtimer, cached_icr, RTIMER_START_VAL);
...
void arch_busy_wait(uint32_t usec_to_wait)
{
	...
	uint32_t start = mec_btimer_count(btimer);

	for (;;) {
		uint32_t curr = mec_btimer_count(btimer);
...
static int config_custom_busy_wait(void)
{
	...
	mec_btimer_init(btimer, MEC5_BTIMER_FDIV, count, bflags);
...
static int sys_clock_driver_init(void)
{
	...
	mec_rtimer_init(rtimer, rtmr_cfg, cached_icr);
	...
	mec_rtimer_start(rtimer);
	while (!mec_rtimer_is_counting(rtimer)) {
		;
	}
