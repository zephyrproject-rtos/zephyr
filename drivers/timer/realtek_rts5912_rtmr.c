...
BUILD_ASSERT(IS_ENABLED(CONFIG_TICKLESS_KERNEL),
	     "RTS5912 RTOS Timer driver only support tickless kernel");
BUILD_ASSERT(CONFIG_SYS_CLOCK_HW_CYCLES_PER_SEC == 32768,
	     "RTS5912 RTOS Timer frequency must be 32768");
BUILD_ASSERT(CONFIG_SYS_CLOCK_TICKS_PER_SEC == 32768, "System clock tick must be 32768");
...
static int sys_clock_driver_init(void)
{
	...
	SSCON_REG->PERICLKPWR1 |= (0x1 << SYSTEM_PERICLKPWR1_SLWTMR0CLKPWR_Pos); 

	/* Enable SLWTMR0 */
	SLWTMR_REG->LDCNT = 0xfffffffful;
