					    ...
					    uint32_t ticks, void *user_data)
{
	...
	__ASSERT(ret == 1, "submit to queue failed");
}
...
static int dali_pwm_receive_init(const struct device *dev)
{
	...
	data->rx.ticks.half_bit.min = dali_pwm_us_to_pwm_cycles(
		DALI_RX_BIT_TIME_HALF_MIN_US - config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.half_bit.max = dali_pwm_us_to_pwm_cycles(
		DALI_RX_BIT_TIME_HALF_MAX_US + config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.full_bit.min = dali_pwm_us_to_pwm_cycles(
		DALI_RX_BIT_TIME_FULL_MIN_US - config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.full_bit.max = dali_pwm_us_to_pwm_cycles(
		DALI_RX_BIT_TIME_FULL_MAX_US + config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.stop_bit = dali_pwm_us_to_pwm_cycles(
		DALI_RX_BIT_TIME_STOP_US - config->rx_grey_area_us, receive_cycles);
	data->rx.ticks.destroy_1.min =
		dali_pwm_us_to_pwm_cycles(DALI_TX_DESTROY_1_MIN_US, receive_cycles);
	data->rx.ticks.destroy_1.max = dali_pwm_us_to_pwm_cycles(
		DALI_TX_DESTROY_1_MAX_US - config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.destroy_2.min = dali_pwm_us_to_pwm_cycles(
		DALI_TX_DESTROY_2_MIN_US + config->rx_max_latency_us, receive_cycles);
	data->rx.ticks.destroy_2.max =
		dali_pwm_us_to_pwm_cycles(DALI_TX_DESTROY_2_MAX_US, receive_cycles);
	data->rx.ticks.destroy_3.min =
		dali_pwm_us_to_pwm_cycles(DALI_TX_DESTROY_3_MIN_US, receive_cycles);
	data->rx.ticks.failure =
		dali_pwm_us_to_pwm_cycles(DALI_FAILURE_CONDITION_US, receive_cycles);
	data->rx.ticks.flank_shift = dali_pwm_us_to_pwm_cycles(config->rx_shift_us, receive_cycles);
	data->rx.ticks.tx_rx_propagation =
		dali_pwm_us_to_pwm_cycles(config->tx_rx_propagation_max_us, receive_cycles);
...
static int dali_pwm_transmit_init(const struct device *dev)
{
	...
	data->tx.pwm_ticks.half_bit =
		dali_pwm_us_to_pwm_cycles(DALI_TX_HALF_BIT_US, transmit_cycles);
	data->tx.pwm_ticks.flank_shift =
		dali_pwm_us_to_pwm_cycles(config->tx_shift_us, transmit_cycles);
	...
	data->tx.counter_ticks.half_bit =
		dali_pwm_us_to_pwm_cycles(DALI_TX_HALF_BIT_US, receive_cycles);
	data->tx.counter_ticks.flank_shift =
		dali_pwm_us_to_pwm_cycles(config->tx_shift_us, receive_cycles);
	data->tx.counter_ticks.tx_prog_delay =
		dali_pwm_us_to_pwm_cycles(config->tx_prog_delay_us, receive_cycles);
