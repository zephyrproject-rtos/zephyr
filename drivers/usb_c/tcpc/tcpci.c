...
int tcpci_get_snk_ctrl(const struct i2c_dt_spec *bus)
{
	uint8_t pwrsts;
	int ret;

	ret = tcpci_read_reg8(bus, TCPC_REG_POWER_STATUS, &pwrsts);
	if (ret != 0) {
		return ret;
	}

	return (pwrsts & TCPC_REG_POWER_STATUS_SINKING_VBUS) ? true : false;
}

int tcpci_get_src_ctrl(const struct i2c_dt_spec *bus)
{
	uint8_t pwrsts;
	int ret;
	...
	return (pwrsts & TCPC_REG_POWER_STATUS_SOURCING_VBUS) ? true : false;
}
