...
struct numaker_tcpc_data {
	...
	/* Mark the end of zero-initialized fields in tcpc_init(). The
	 * fields following the marker will persist across tcpc_init().
	 */
	uint32_t zero_end_marker;
 ...
 *
 * @retval 1 Dead Battery mode is enabled
 * @retval 0 Dead Battery mode is not enabled
 */
static int numaker_utcpd_deadbattery_query_enable(const struct device *dev)
{
	const struct numaker_tcpc_config *const config = dev->config;
	UTCPD_T *utcpd_base = config->utcpd_base;
	uint32_t phyctl = NUMAKER_UTCPD_REG_READ_BY_NAME(dev, PHYCTL);

	return (phyctl & UTCPD_PHYCTL_DBCTL_Msk) ? 0 : 1;
}
...
static int numaker_tcpc_init_recycle(const struct device *dev)
{
	...
	/* Zero-initialize would be fine for most fields. */
	memset(data, 0x00, offsetof(struct numaker_tcpc_data, zero_end_marker));
...
static int numaker_tcpc_set_cc(const struct device *dev, enum tc_cc_pull pull)
{
	 ...
	 */
	rc = numaker_utcpd_deadbattery_query_enable(dev);
	if (rc < 0) {
		return rc;
	}
	if (rc == 1) {
