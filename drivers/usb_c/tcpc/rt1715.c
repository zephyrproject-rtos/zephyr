...
static const struct tcpc_driver_api rt1715_driver_api = {
...
void rt1715_alert_work_cb(struct k_work *work)
{
	...
	while (alert_reg != 0) {
		...
		if (alert_type == TCPC_ALERT_HARD_RESET_RECEIVED) {
			...
			rt1715_tcpc_rx_fifo_enqueue(dev);
...
void rt1715_init_work_cb(struct k_work *work)
{
	...
	/* Resets the chip */
	tcpci_write_reg8(&cfg->bus, RT1715_REG_SW_RST, RT1715_REG_SW_RST_EN);
	k_msleep(5);

	res = tcpci_tcpm_get_status_register(&cfg->bus, TCPC_POWER_STATUS, (uint16_t *)&power_reg);
	if (res != 0 || (power_reg & TCPC_REG_POWER_STATUS_UNINIT)) {
		...
		k_work_schedule_for_queue(&k_sys_work_q, &data->init_dwork,
					  K_MSEC(CONFIG_USBC_TCPC_RT1715_INIT_DELAY));
	...
	/* Exit shutdown mode */
	tcpci_update_reg8(&cfg->bus, RT1715_REG_LP_CTRL, RT1715_REG_LP_CTRL_SHUTDOWN_OFF,
			  RT1715_REG_LP_CTRL_SHUTDOWN_OFF);

	/* Enable ext messages */
	tcpci_update_reg8(&cfg->bus, RT1715_REG_LP_CTRL, RT1715_REG_LP_CTRL_ENEXTMSG,
			  RT1715_REG_LP_CTRL_ENEXTMSG);

	/* Disable idle mode */
	tcpci_update_reg8(&cfg->bus, RT1715_REG_LP_CTRL, RT1715_REG_LP_CTRL_AUTOIDLE_EN, 0);
