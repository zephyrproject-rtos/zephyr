...
static int reset_mss_line_assert(const struct device *dev, uint32_t id)
{
	...
	/* Turn off clock */
	sys_clear_bit(config->base + SUBBLK_CLOCK_CR_OFFSET, RESET_MSS_REG_BIT(id));

	/* Hold in reset */
	sys_set_bit(config->base + SOFT_RESET_CR_OFFSET, RESET_MSS_REG_BIT(id));
...
static int reset_mss_line_deassert(const struct device *dev, uint32_t id)
{
	...
	/* Turn on clock */
	sys_set_bit(config->base + SUBBLK_CLOCK_CR_OFFSET, RESET_MSS_REG_BIT(id));

	/* Remove soft reset */
	sys_clear_bit(config->base + SOFT_RESET_CR_OFFSET, RESET_MSS_REG_BIT(id));
