...
static bool pio_i2c_check_error(PIO pio, uint sm)
{
...
static inline uint8_t pio_i2c_get(PIO pio, uint sm)
{
...
static int get_pio_idx(PIO pio)
{
	return pio == pio0 ? 0 : 1;
}
#elif NUM_PIOS == 3
...
static uint8_t load_program_once(PIO pio)
{
	...
	offset = i2c_pio_program_offset[pionum];
