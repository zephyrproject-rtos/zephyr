...
#define I2C_CFG(dev)                    ((struct i2c_b91_cfg *) ((dev)->config))

/* I2C data structure */
struct i2c_b91_data {
	struct k_sem mutex;
};
#define I2C_DATA(dev)                   ((struct i2c_b91_data *) ((dev)->data))
			    ...
			    uint16_t addr)
{
	int status = 0;
	uint8_t send_stop = 0;
	struct i2c_b91_data *data = I2C_DATA(dev);
	...
	for (int i = 0; i < num_msgs; i++) {
		...
		if (msgs[i].flags & I2C_MSG_ADDR_10_BITS) {
			LOG_ERR("10-bits address is supported");
...
{
	int status = 0;
	struct i2c_b91_cfg *cfg = I2C_CFG(dev);
	struct i2c_b91_data *data = I2C_DATA(dev);
...
#define I2C_B91_INIT(inst)					  \
	...
	static struct i2c_b91_cfg i2c_b91_cfg_##inst = {	  \
		.bitrate = DT_INST_PROP(inst, clock_frequency)	  \
								  ...
								  \
	DEVICE_DT_INST_DEFINE(0, i2c_b91_init,			  \
			      NULL,		  \
			      &i2c_b91_data_##inst,		  \
			      &i2c_b91_cfg_##inst,		  \
			      POST_KERNEL,			  \
