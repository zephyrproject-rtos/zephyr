...
static int i2c_rza2m_riic_wait_for_clear(const struct device *dev, uint32_t offs, uint16_t mask)
{
	...
	while ((i2c_rza2m_riic_read(dev, offs) & mask) && (timeout < 10)) {
...
static int i2c_rza2m_riic_wait_for_state(const struct device *dev, uint8_t mask, bool forever)
{
	...
	while ((i2c_rza2m_riic_read(dev, RIIC_IER) != int_backup) && (timeout < 10)) {
		k_msleep(1);
		timeout++;
	}
				   ...
				   uint16_t addr)
{
	...
	while ((i2c_rza2m_riic_read(dev, RIIC_CR2) & RIIC_CR2_BBSY) && (timeout < 10)) {
		k_busy_wait(USEC_PER_MSEC);
		timeout++;
	}
	if (timeout == 10) {
	...
	do {
		if (msgs->flags & I2C_MSG_RESTART) {
			...
			/* Send slave address */
			if (i2c_rza2m_riic_set_addr(dev, addr, msgs->flags)) {
				i2c_rza2m_riic_finish(dev);
		...
		/* Transfer data */
		if (msgs->len) {
			ret = i2c_rza2m_riic_transfer_msg(dev, msgs);
			if (ret != 0) {
				break;
			}
		}
					      ...
					      struct i2c_rza2m_riic_clk_settings *clk_cfg)
{
	...
	for (int temp_divider = 0; temp_divider <= 7; ++temp_divider) {
		...
		uint32_t total_brl_brh = ceil(
			((1 / (double)config->bitrate) - (rise_time_s + fall_time_s)) * divided_p0 -
			(2 * constant_add));
...
static int i2c_rza2m_riic_configure(const struct device *dev, uint32_t dev_config)
{
	...
	/* Ensure the HW is in master mode and does not behave as a slave to another master on the
	 * same channel.
	 */
...
#define I2C_RZA2M_RIIC_INIT(n)                                                                     \
	...
	static const struct i2c_rza2m_riic_config i2c_rza2m_riic_config_##n = {                    \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),                                         \
                                                                                                   ...
                                                                                                   \
	struct i2c_rza2m_riic_clk_settings i2c_rza2m_riic_clk_settings_##n;                        \
	static struct i2c_rza2m_riic_data i2c_rza2m_riic_data_##n = {                              \
		.clk_settings = &i2c_rza2m_riic_clk_settings_##n,                                  \
	};                                                                                         \
                                                                                                   \
