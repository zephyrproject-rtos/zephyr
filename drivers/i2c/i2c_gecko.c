...
#define PIN_I2C_SDA(n) { DT_INST_PROP_BY_IDX(n, location_sda, 1), \
			 DT_INST_PROP_BY_IDX(n, location_sda, 2), gpioModeWiredAnd, 1 }
#define PIN_I2C_SCL(n) { DT_INST_PROP_BY_IDX(n, location_scl, 1), \
			 DT_INST_PROP_BY_IDX(n, location_scl, 2), gpioModeWiredAnd, 1 }

#ifdef CONFIG_SOC_GECKO_HAS_INDIVIDUAL_PIN_LOCATION
#define I2C_GECKO_CONFIG(n)					      \
	static const struct i2c_gecko_config i2c_gecko_config_##n = { \
		.base = (I2C_TypeDef *)DT_INST_REG_ADDR(n),	      \
		.clock = cmuClock_I2C##n,			      \
		.pin_sda = PIN_I2C_SDA(n),			      \
		.pin_scl = PIN_I2C_SCL(n),			      \
		.loc_sda = DT_INST_PROP_BY_IDX(n, location_sda, 0),   \
		.loc_scl = DT_INST_PROP_BY_IDX(n, location_scl, 0),   \
		.bitrate = DT_INST_PROP(n, clock_frequency),	      \
	};
#else
#define I2C_GECKO_CONFIG(n)									    \
	BUILD_ASSERT(										    \
		DT_INST_PROP_BY_IDX(n, location_sda, 0) == DT_INST_PROP_BY_IDX(n, location_scl, 0), \
		"I2C DTS location-* properties must have identical value"			    \
		);										    \
	static const struct i2c_gecko_config i2c_gecko_config_##n = {				    \
		.base = (I2C_TypeDef *)DT_INST_REG_ADDR(n),					    \
		.clock = cmuClock_I2C##n,							    \
		.pin_sda = PIN_I2C_SDA(n),							    \
		.pin_scl = PIN_I2C_SCL(n),							    \
		.loc = DT_INST_PROP_BY_IDX(n, location_scl, 0),					    \
		.bitrate = DT_INST_PROP(0, clock_frequency),					    \
	};
#endif

#define I2C_GECKO_DEVICE(n)								 \
	I2C_GECKO_CONFIG(n);								 \
	static struct i2c_gecko_data i2c_gecko_data_##n;				 \
	DEVICE_AND_API_INIT(i2c_gecko_##n, DT_INST_LABEL(n),				 \
			    &i2c_gecko_init, &i2c_gecko_data_##n, &i2c_gecko_config_##n, \
			    POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,		 \
			    &i2c_gecko_driver_api);

DT_INST_FOREACH_STATUS_OKAY(I2C_GECKO_DEVICE)
