...
static int i2c_rts5912_recover_bus(const struct device *dev)
{
	...
	while (test_bit_enable_sdarecov(reg_base) && --retry_counter) {
		;
	}

	/* Check if bus is not clear */
	ret = test_bit_status_sdanotrecov(reg_base);
	read_clr_intr(reg_base);
	read_clr_tx_abrt(reg_base);
	if (ret) {
		LOG_ERR("Bus Recovery Fail on %s", dev->name);
		return -ETIME;
	}
	return 0;
}
...
static void i2c_rts5912_isr(const struct device *port)
{
	...
	if (bus->cb != NULL) {
		/* Async transfer */
		if (bus->state & I2C_RTS5912_STUCK) {
			LOG_DBG("ETIME\r\n");
			i2c_rts5912_async_done(port, bus, -ETIME);
		} else if (bus->state & I2C_RTS5912_NACK) {
			LOG_DBG("ENACK\r\n");
			i2c_rts5912_async_done(port, bus, -ENXIO);
		} else if (bus->state & I2C_RTS5912_CMD_ERROR) {
			LOG_DBG("EIO\r\n");
			i2c_rts5912_async_done(port, bus, -EIO);
				...
				uint16_t slave_address)
{
	...
	ret = k_mutex_lock(&bus->bus_mutex, K_FOREVER);
	if (ret != 0) {
		return ret;
	}
	...
	while (msg_left > 0) {
		...
		if ((bus->state & I2C_RTS5912_NACK) || (bus->state & I2C_RTS5912_CMD_ERROR)) {
			ret = -EIO;
			break;
		}
