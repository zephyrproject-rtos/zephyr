...
#define I2C_REG_CSA      0x1210
#define I2C_REG_CCTR     0x1214
#define I2C_REG_CSR      0x1218
#define I2C_REG_CRXDATA  0x121C
#define I2C_REG_CTXDATA  0x1220
#define I2C_REG_CTPR     0x1224
#define I2C_REG_CCR      0x1228
#define I2C_REG_CFIFOCTL 0x1238
#define I2C_REG_CFIFOSR  0x123C

/* Target registers */
#define I2C_REG_TOAR     0x1250
#define I2C_REG_TCTR     0x1258
#define I2C_REG_TRXDATA  0x1260
#define I2C_REG_TTXDATA  0x1264
#define I2C_REG_TACKCTL  0x1268
#define I2C_REG_TFIFOCTL 0x126C
#define I2C_REG_TFIFOSR  0x1270

/* GPRCM: Power / Reset */
#define I2C_PWREN_UNLOCK_KEY       ((uint32_t)0x26000000U)
#define I2C_PWREN_ENABLE           BIT(0)
#define I2C_RSTCTL_UNLOCK_KEY      ((uint32_t)0xB1000000U)
#define I2C_RSTCTL_CLEAR_RESETSTKY BIT(1)
#define I2C_RSTCTL_ASSERT_RESET    BIT(0)

/* Clock */
#define I2C_CLKSEL_MFCLK_SELECT_MASK  BIT(2)
#define I2C_CLKSEL_MFCLK_SELECT       BIT(2)
#define I2C_CLKSEL_BUSCLK_SELECT_MASK BIT(3)
#define I2C_CLKSEL_BUSCLK_SELECT      BIT(3)
#define I2C_CLKDIV_DIVIDE_RATIO_MASK  GENMASK(2, 0)
#define I2C_CLKDIV_RATIO_DIVIDE_BY_1  0

#define I2C_CLKCFG_BLOCK_ASYNC      BIT(8)
#define I2C_CLKCFG_BLOCK_ASYNC_MASK BIT(8)

/* Glitch filter */
#define I2C_GFCTL_ANALOG_GLITCH_FILTER_MASK BIT(8)

/* Timeout */
#define I2C_TIMEOUT_CTL_LOAD_VALUE_MASK     GENMASK(7, 0)
#define I2C_TIMEOUT_CTL_COUNTER_ENABLE      BIT(15)
#define I2C_TIMEOUT_CTL_COUNTER_ENABLE_MASK BIT(15)

/* Controller: CSA (Target Address) - aliases for macro construction */
#define I2C_CSA_SADDR_MASK I2C_TSA_SADDR_MASK
#define I2C_CSA_DIR_MASK   I2C_TSA_DIR_MASK
#define I2C_CSA_MMODE_MASK I2C_TSA_MMODE_MASK

/* Controller: CSA (Target Address) */
#define I2C_TSA_SADDR_MASK   GENMASK(10, 1)
#define I2C_TSA_DIR_MASK     BIT(0)
#define I2C_TSA_DIR_TRANSMIT 0
#define I2C_TSA_DIR_RECEIVE  BIT(0)
#define I2C_TSA_MMODE_MASK   BIT(15)
#define I2C_TSA_MMODE_MODE7  0
#define I2C_TSA_MMODE_MODE10 BIT(15)

/* Controller: CCTR (Control) */
#define I2C_CCTR_BURSTRUN_MASK   BIT(0)
#define I2C_CCTR_BURSTRUN_ENABLE BIT(0)
#define I2C_CCTR_START_MASK      BIT(1)
#define I2C_CCTR_START_DISABLE   0
#define I2C_CCTR_START_ENABLE    BIT(1)
#define I2C_CCTR_STOP_MASK       BIT(2)
#define I2C_CCTR_STOP_DISABLE    0
#define I2C_CCTR_STOP_ENABLE     BIT(2)
#define I2C_CCTR_ACK_MASK        BIT(3)
#define I2C_CCTR_ACK_DISABLE     0
#define I2C_CCTR_ACK_ENABLE      BIT(3)
#define I2C_CCTR_MBLEN_MASK      GENMASK(27, 16)

/* Controller: CSR (Status) */
#define I2C_CSRERR_MASK BIT(1)

/* Controller: RX/TX data */
#define I2C_CRXDATA_VALUE_MASK GENMASK(7, 0)

/* Controller: CCR (Configuration) */
#define I2C_CCR_ACTIVE_MASK            BIT(0)
#define I2C_CCR_ACTIVE_ENABLE          BIT(0)
#define I2C_CCR_CLKSTRETCH_ENABLE      BIT(2)
#define I2C_CCR_CLKSTRETCH_ENABLE_MASK BIT(2)

/* Controller: CFIFOCTL / CFIFOSR */
#define I2C_CFIFOCTL_TXTRIG_MASK      GENMASK(2, 0)
#define I2C_CFIFOCTL_TXTRIG_LEVEL_1   FIELD_PREP(GENMASK(2, 0), 1)
#define I2C_CFIFOCTL_TXFLUSH_MASK     BIT(7)
#define I2C_CFIFOCTL_TXFLUSH          BIT(7)
#define I2C_CFIFOCTL_RXTRIG_MASK      GENMASK(10, 8)
#define I2C_CFIFOCTL_RXTRIG_LEVEL_1   FIELD_PREP(GENMASK(10, 8), 0)
#define I2C_CFIFOSR_TXFIFOCNT_MASK    GENMASK(11, 8)
#define I2C_CFIFOSR_TXFIFOCNT_MINIMUM 0
#define I2C_CFIFOSR_RXFIFOCNT_MASK    GENMASK(3, 0)
#define I2C_CFIFOSR_RXFIFOCNT_MINIMUM 0

/* Target: TOAR (Own Address) */
#define I2C_TOAR_OAR_MASK     GENMASK(9, 0)
#define I2C_TOAR_SMODE_MASK   BIT(15)
#define I2C_TOAR_SMODE_MODE7  0
#define I2C_TOAR_SMODE_MODE10 BIT(15)

/* Target: TCTR (Control) */
#define I2C_TCTR_ACTIVE_MASK                 BIT(0)
#define I2C_TCTR_ACTIVE_ENABLE               BIT(0)
#define I2C_TCTR_SCLKSTRETCH_ENABLE          BIT(2)
#define I2C_TCTR_SCLKSTRETCH_ENABLE_MASK     BIT(2)
#define I2C_TCTR_TXEMPTY_ON_TREQ_ENABLE      BIT(3)
#define I2C_TCTR_TXEMPTY_ON_TREQ_ENABLE_MASK BIT(3)
#define I2C_TCTR_TXTRIG_TXMODE_ENABLE        BIT(4)
#define I2C_TCTR_TXTRIG_TXMODE_ENABLE_MASK   BIT(4)
#define I2C_TCTR_SWUEN_MASK                  BIT(10)

/* Target: TACKCTL */
#define I2C_TACKCTL_ACKOVAL_MASK    BIT(1)
#define I2C_TACKCTL_ACKOVAL_DISABLE 0      /* ACK  */
#define I2C_TACKCTL_ACKOVAL_ENABLE  BIT(1) /* NACK */

/* Target: TFIFOCTL / TFIFOSR */
#define I2C_TFIFOCTL_TXTRIG_MASK      GENMASK(2, 0)
#define I2C_TFIFOCTL_TXFLUSH_MASK     BIT(7)
#define I2C_TFIFOCTL_TXFLUSH          BIT(7)
#define I2C_TFIFOCTL_RXTRIG_MASK      GENMASK(10, 8)
#define I2C_TFIFOSR_TXFIFOCNT_MASK    GENMASK(11, 8)
#define I2C_TFIFOSR_TXFIFOCNT_MINIMUM 0
#define I2C_TFIFOSR_RXFIFOCNT_MASK    GENMASK(3, 0)
#define I2C_TFIFOSR_RXFIFOCNT_MINIMUM 0

/* Target: RX/TX data */
#define I2C_TRXDATA_VALUE_MASK GENMASK(7, 0)

/* CPU_INT: IIDX values (pending interrupt index) */
#define I2C_CPU_INT_IIDX_STAT_CRXDONEFG    0x00000001U
#define I2C_CPU_INT_IIDX_STAT_CTXDONEFG    0x00000002U
#define I2C_CPU_INT_IIDX_STAT_CRXFIFOTRG   0x00000003U
#define I2C_CPU_INT_IIDX_STAT_CTXFIFOTRG   0x00000004U
#define I2C_CPU_INT_IIDX_STAT_CNACKFG      0x00000008U
#define I2C_CPU_INT_IIDX_STAT_CSTOPFG      0x0000000AU
#define I2C_CPU_INT_IIDX_STAT_CARBLOSTFG   0x0000000BU
#define I2C_CPU_INT_IIDX_STAT_TIMEOUTA_EVT 0x0000000FU
#define I2C_CPU_INT_IIDX_STAT_TRXDONEFG    0x00000011U
#define I2C_CPU_INT_IIDX_STAT_TTXEMPTY     0x00000016U
#define I2C_CPU_INT_IIDX_STAT_TSTARTFG     0x00000017U
#define I2C_CPU_INT_IIDX_STAT_TSTOPFG      0x00000018U

/* CPU_INT: IMASK bit positions (interrupt enable/mask) */
#define I2C_CPU_INT_IMASK_CRXDONE_SET    BIT(0)
#define I2C_CPU_INT_IMASK_CTXDONE_SET    BIT(1)
#define I2C_CPU_INT_IMASK_CRXFIFOTRG_SET BIT(2)
#define I2C_CPU_INT_IMASK_CTXFIFOTRG_SET BIT(3)
#define I2C_CPU_INT_IMASK_CNACK_SET      BIT(7)
#define I2C_CPU_INT_IMASK_CSTOP_SET      BIT(9)
#define I2C_CPU_INT_IMASK_CARBLOST_SET   BIT(10)
#define I2C_CPU_INT_IMASK_TIMEOUT_A_SET  BIT(14)
#define I2C_CPU_INT_IMASK_TRXDONE_SET    BIT(16)
#define I2C_CPU_INT_IMASK_TTXEMPTY_SET   BIT(21)
#define I2C_CPU_INT_IMASK_TSTART_SET     BIT(22)
#define I2C_CPU_INT_IMASK_TSTOP_SET      BIT(23)
...
/* CLKCFG config values */
#define I2C_CLKCFG_BLOCK_ASYNC BIT(8)

/* Local macro definitions for register values */
#define I2C_CONTROLLER_STOP_ENABLE              I2C_CCTR_STOP_ENABLE
#define I2C_CONTROLLER_STOP_DISABLE             I2C_CCTR_STOP_DISABLE
#define I2C_CONTROLLER_START_ENABLE             I2C_CCTR_START_ENABLE
#define I2C_CONTROLLER_START_DISABLE            I2C_CCTR_START_DISABLE
#define I2C_CONTROLLER_ACK_ENABLE               I2C_CCTR_ACK_ENABLE
#define I2C_CONTROLLER_ACK_DISABLE              I2C_CCTR_ACK_DISABLE
#define I2C_CONTROLLER_DIRECTION_TX             I2C_TSA_DIR_TRANSMIT
#define I2C_CONTROLLER_DIRECTION_RX             I2C_TSA_DIR_RECEIVE
#define I2C_TX_FIFO_LEVEL_BYTES_1               I2C_CFIFOCTL_TXTRIG_LEVEL_1
#define I2C_RX_FIFO_LEVEL_BYTES_1               I2C_CFIFOCTL_RXTRIG_LEVEL_1
#define I2C_TARGET_RESPONSE_OVERRIDE_VALUE_ACK  I2C_TACKCTL_ACKOVAL_DISABLE
#define I2C_TARGET_RESPONSE_OVERRIDE_VALUE_NACK I2C_TACKCTL_ACKOVAL_ENABLE
#define I2C_CONTROLLER_STATUS_ERROR             I2C_CSRERR_MASK
#define I2C_CLOCK_DIVIDE_1                      I2C_CLKDIV_RATIO_DIVIDE_BY_1

/* Startup delay in cycles */
#define POWER_STARTUP_DELAY 16

/* Helper macro for register field update */
#define I2C_UPDATE_REG(base, reg, bit, value)                                                      \
	sys_write32((sys_read32((base) + I2C_REG_##reg) & ~(I2C_##reg##_##bit##_MASK)) |           \
			    ((value) & I2C_##reg##_##bit##_MASK),                                  \
		    (base) + I2C_REG_##reg)
...
#define TI_MSPM0_CONTROLLER_INTERRUPTS                                                             \
	(I2C_CPU_INT_IMASK_CARBLOST_SET | I2C_CPU_INT_IMASK_CNACK_SET |                            \
	 I2C_CPU_INT_IMASK_CRXFIFOTRG_SET | I2C_CPU_INT_IMASK_CRXDONE_SET |                        \
	 I2C_CPU_INT_IMASK_CTXDONE_SET | I2C_CPU_INT_IMASK_TIMEOUT_A_SET |                         \
	 I2C_CPU_INT_IMASK_CSTOP_SET)

/*
 * Mask covering all possible controller interrupt sources, including
 * I2C_CPU_INT_IMASK_CTXFIFOTRG_SET which is dynamically
...
#define TI_MSPM0_CONTROLLER_INTERRUPTS_ALL                                                         \
	(TI_MSPM0_CONTROLLER_INTERRUPTS | I2C_CPU_INT_IMASK_CTXFIFOTRG_SET)

#define TI_MSPM0_TARGET_INTERRUPTS                                                                 \
	(I2C_CPU_INT_IMASK_TRXDONE_SET | I2C_CPU_INT_IMASK_TTXEMPTY_SET |                          \
	 I2C_CPU_INT_IMASK_TSTART_SET | I2C_CPU_INT_IMASK_TSTOP_SET |                              \
	 I2C_CPU_INT_IMASK_TIMEOUT_A_SET)
...
struct i2c_mspm0_data {
	...
	/* Direct user messages, consumed by ISR */
	struct i2c_msg *msgs;
	uint8_t num_msgs;
	uint8_t msg_idx;
							  ...
							  uint32_t ack, uint32_t addr_mode)
{
	...
	I2C_UPDATE_REG(base, CSA, SADDR, FIELD_PREP(I2C_TSA_SADDR_MASK, targetAddr));
	I2C_UPDATE_REG(base, CSA, DIR, direction);
	I2C_UPDATE_REG(base, CSA, MMODE, addr_mode);

	/*
	 * Set control register:
	 * I2C_UPDATE_REG can't be used here as the bits of this register directly
	 * control the transfer and have to be updated all at once.
	 */
	uint32_t cctr = 0;

	cctr |= FIELD_PREP(I2C_CCTR_MBLEN_MASK, length);
	cctr |= I2C_CCTR_BURSTRUN_ENABLE;
	cctr |= start;
	cctr |= stop;
	cctr |= ack;

	sys_write32(cctr, base + I2C_REG_CCTR);
}

/* Timeout helpers (if CONFIG_MSPM0_I2C_SCL_LOW_TIMEOUT_MS != 0) */
static inline void i2c_enable_timeout_a(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TIMEOUT_CTL, COUNTER_ENABLE, I2C_TIMEOUT_CTL_COUNTER_ENABLE);
}

static inline void i2c_set_timeout_a_count(mm_reg_t base, uint32_t count)
{
	I2C_UPDATE_REG(base, TIMEOUT_CTL, LOAD_VALUE, count);
}

/* Interrupt helpers */
static inline void i2c_enable_interrupt(mm_reg_t base, uint32_t interruptMask)
{
	sys_write32(sys_read32(base + I2C_REG_CPU_INT_IMASK) | interruptMask,
		    base + I2C_REG_CPU_INT_IMASK);
}
...
{
	I2C_UPDATE_REG(base, TCTR, SWUEN, 0);
}
...
{
	I2C_UPDATE_REG(base, TCTR, ACTIVE, I2C_TCTR_ACTIVE_ENABLE);
}

static inline void i2c_disable_target(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TCTR, ACTIVE, 0);
}

static inline void i2c_set_target_own_address(mm_reg_t base, uint32_t addr, uint32_t addr_mode)
{
	I2C_UPDATE_REG(base, TOAR, OAR, addr);
	I2C_UPDATE_REG(base, TOAR, SMODE, addr_mode);
}

static inline void i2c_enable_target_clock_stretching(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TCTR, SCLKSTRETCH_ENABLE, I2C_TCTR_SCLKSTRETCH_ENABLE);
}

static inline void i2c_enable_target_tx_trigger_in_tx_mode(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TCTR, TXTRIG_TXMODE_ENABLE, I2C_TCTR_TXTRIG_TXMODE_ENABLE);
}

static inline void i2c_enable_target_tx_empty_on_tx_request(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TCTR, TXEMPTY_ON_TREQ_ENABLE, I2C_TCTR_TXEMPTY_ON_TREQ_ENABLE);
}

static inline void i2c_set_target_tx_fifo_threshold(mm_reg_t base, uint32_t level)
{
	I2C_UPDATE_REG(base, TFIFOCTL, TXTRIG, level);
}

static inline void i2c_set_target_rx_fifo_threshold(mm_reg_t base, uint32_t level)
{
	I2C_UPDATE_REG(base, TFIFOCTL, RXTRIG, level);
}

static inline void i2c_flush_target_tx_fifo(mm_reg_t base)
{
	I2C_UPDATE_REG(base, TFIFOCTL, TXFLUSH, I2C_TFIFOCTL_TXFLUSH);

	while ((sys_read32(base + I2C_REG_TFIFOSR) & I2C_TFIFOSR_TXFIFOCNT_MASK) !=
	       FIELD_PREP(I2C_TFIFOSR_TXFIFOCNT_MASK, I2C_FIFO_DEPTH)) {
		/* Wait */
	}

	I2C_UPDATE_REG(base, TFIFOCTL, TXFLUSH, 0);
}

static inline bool i2c_is_target_rx_fifo_empty(mm_reg_t base)
{
	return ((sys_read32(base + I2C_REG_TFIFOSR) & I2C_TFIFOSR_RXFIFOCNT_MASK) ==
		I2C_TFIFOSR_RXFIFOCNT_MINIMUM);
}

static inline uint8_t i2c_receive_target_data(mm_reg_t base)
{
	return (uint8_t)(sys_read32(base + I2C_REG_TRXDATA) & I2C_TRXDATA_VALUE_MASK);
}

static inline void i2c_transmit_target_data(mm_reg_t base, uint8_t data)
{
	sys_write32(data, base + I2C_REG_TTXDATA);
}

static inline bool i2c_is_target_tx_fifo_full(mm_reg_t base)
{
	return ((sys_read32(base + I2C_REG_TFIFOSR) & I2C_TFIFOSR_TXFIFOCNT_MASK) ==
		I2C_TFIFOSR_TXFIFOCNT_MINIMUM);
}
...
{
	I2C_UPDATE_REG(base, TACKCTL, ACKOVAL, value);
}
#endif /* CONFIG_I2C_TARGET */
...
static int i2c_mspm0_configure(const struct device *dev, uint32_t dev_config)
{
	...
	/* Disable Async clock request */
	i2c_disable_async_clk_req(base);
...
static int i2c_mspm0_reset_controller(const struct device *dev)
{
	...
	/* Wait for peripheral power to stabilize (POST_KERNEL, so k_busy_wait is available) */
	k_busy_wait(k_cyc_to_us_ceil32(POWER_STARTUP_DELAY));
...
/* Start transfer for msgs[msg_idx] */
static void i2c_mspm0_start_dispatch(const struct device *dev)
{
	const struct i2c_mspm0_config *config = dev->config;
	struct i2c_mspm0_data *data = dev->data;
	const mm_reg_t base = config->base;
	const struct i2c_msg *msg = &data->msgs[data->msg_idx];
	uint32_t addr_mode =
		(msg->flags & I2C_MSG_ADDR_10_BITS) ? I2C_TSA_MMODE_MODE10 : I2C_TSA_MMODE_MODE7;
	uint32_t stop = (msg->flags & I2C_MSG_STOP) ? I2C_CONTROLLER_STOP_ENABLE
	...
	if ((msg->flags & I2C_MSG_RW_MASK) == I2C_MSG_READ) {
		...
		i2c_start_controller_transfer_advanced(
			base, data->addr, I2C_CONTROLLER_DIRECTION_RX, msg->len,
		...
		data->transfer_count = i2c_fill_controller_tx_fifo(base, msg->buf, msg->len);

		if (data->transfer_count < data->transfer_len) {
			i2c_enable_interrupt(base, I2C_CPU_INT_IMASK_CTXFIFOTRG_SET);
		} else {
			i2c_disable_interrupt(base, I2C_CPU_INT_IMASK_CTXFIFOTRG_SET);
		}

		data->state = I2C_MSPM0_TX_STARTED;
		i2c_start_controller_transfer_advanced(
			base, data->addr, I2C_CONTROLLER_DIRECTION_TX, msg->len,
...
static int i2c_mspm0_target_register(const struct device *dev, struct i2c_target_config *cfg)
{
	...
	i2c_set_target_own_address(base, data->target_config->address,
				   (data->target_config->flags & I2C_TARGET_FLAGS_ADDR_10_BITS)
					   ? I2C_TOAR_SMODE_MODE10
					   : I2C_TOAR_SMODE_MODE7);

	i2c_clear_interrupt_status(base, I2C_CPU_INT_IMASK_TTXEMPTY_SET);
...
static void i2c_mspm0_isr_target(const struct device *dev)
{
	...
	case I2C_CPU_INT_IIDX_STAT_TRXDONEFG:
		...
		break;
	case I2C_CPU_INT_IIDX_STAT_TTXEMPTY:
		if (data->state == I2C_MSPM0_TARGET_STARTED) {
...
static inline void i2c_mspm0_isr_controller(const struct device *dev)
{
	...
	switch (i2c_get_pending_interrupt(base)) {
	case I2C_CPU_INT_IIDX_STAT_CRXDONEFG:
		/*
		...
		/* Advance the state machine to the next message */
		data->msg_idx++;
		if (data->msg_idx < data->num_msgs) {
			i2c_mspm0_start_dispatch(dev);
		} else {
			/* All messages complete: wake the transfer thread */
			data->state = I2C_MSPM0_IDLE;
			k_sem_give(&data->device_sync_sem);
		}
		break;
	case I2C_CPU_INT_IIDX_STAT_CTXDONEFG:
		i2c_disable_interrupt(base, I2C_CPU_INT_IMASK_CTXFIFOTRG_SET);
		...
		break;
	case I2C_CPU_INT_IIDX_STAT_CRXFIFOTRG:
		...
		break;
	case I2C_CPU_INT_IIDX_STAT_CTXFIFOTRG:
		...
		break;
	case I2C_CPU_INT_IIDX_STAT_CNACKFG:
		...
		break;
	case I2C_CPU_INT_IIDX_STAT_CARBLOSTFG:
		/*
	...
	case I2C_CPU_INT_IIDX_STAT_TIMEOUTA_EVT:
		...
		i2c_flush_controller_tx_fifo(base);
		__fallthrough;
	case I2C_CPU_INT_IIDX_STAT_CSTOPFG:
		k_sem_give(&data->i2c_lock);
	default:
		break;
	}
...
#define TX_FIFO_THRESHOLD(index)                                                                   \
	FIELD_PREP(I2C_CFIFOCTL_TXTRIG_MASK, DT_INST_PROP(index, controller_tx_fifo_threshold))

#define RX_FIFO_THRESHOLD(index)                                                                   \
	FIELD_PREP(I2C_CFIFOCTL_RXTRIG_MASK, DT_INST_PROP(index, controller_rx_fifo_threshold) - 1)
