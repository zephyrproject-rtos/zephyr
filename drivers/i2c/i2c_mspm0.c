...
struct i2c_mspm0_config {
	I2C_Regs *base;
	uint32_t bitrate;
	DL_I2C_ClockConfig g_I2C_clock_config;
...
struct i2c_mspm0_data {
	...
	uint16_t secondary_addr;	/* Secondary slave address */
	uint16_t secondary_dont_care_mask; /* Secondary don't care mask. Muxed with
...
static int i2c_mspm0_configure(const struct device *dev, uint32_t dev_config)
{
	 ...
	 * This function works out to be ceil(clock_rate/(desired_speed*10)) - 1
	 */
...
static int i2c_mspm0_reset_peripheral_target(const struct device *dev) {
	...
	DL_I2C_setClockConfig(config->base,
			(DL_I2C_ClockConfig *)&config->g_I2C_clock_config);
...
#ifdef CONFIG_I2C_MSPM0_MULTI_TARGET_ADDRESS
	if(data->secondary_addr != 0x00) {
		DL_I2C_setTargetOwnAddressAlternate(config->base, data->secondary_addr);
		DL_I2C_setTargetOwnAddressAlternateMask(config->base, data->secondary_dont_care_mask);
	...
	if (data->is_target == true) {
		...
		if (target_config != data->target_config) {
			...
			if (target_config->flags & I2C_TARGET_FLAGS_SECONDARY_ADDR) {
				...
				data->secondary_addr = target_config->address & 0xFF;
				data->secondary_dont_care_mask = (0x0000FF00 & (uint32_t) target_config->address) >> 8;
				DL_I2C_setTargetOwnAddressAlternate(config->base, data->secondary_addr);
				DL_I2C_setTargetOwnAddressAlternateMask(config->base, data->secondary_dont_care_mask);
				DL_I2C_enableTargetOwnAddressAlternate(config->base);
			} else {
...
#define MSP_I2C_INIT_FN(index)					\
	...
	static struct i2c_mspm0_data i2c_mspm0_data_##index = { 	\
		.secondary_addr = 0x00,															\
		.i2c_busy_sem = Z_SEM_INITIALIZER(i2c_mspm0_data_##index.i2c_busy_sem, 0, 1),	\
	};						\
						\
