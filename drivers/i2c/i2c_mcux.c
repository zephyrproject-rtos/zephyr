...
#define DEV_BASE(dev) ((I2C_Type *)((const struct i2c_mcux_config *const)(dev)->config)->base)
...
static int i2c_mcux_configure(const struct device *dev, uint32_t dev_config_raw)
{
...
static void i2c_mcux_master_transfer_callback(I2C_Type *base, i2c_master_handle_t *handle,
...
static int i2c_mcux_transfer(const struct device *dev, struct i2c_msg *msgs, uint8_t num_msgs,
			     uint16_t addr)
{
	I2C_Type *base = DEV_BASE(dev);
	struct i2c_mcux_data *data = dev->data;
	...
	for (int i = 0; i < num_msgs; i++) {
		...
		transfer.direction = (msgs->flags & I2C_MSG_READ) ? kI2C_Read : kI2C_Write;
...
static int i2c_mcux_init(const struct device *dev)
{
	...
	I2C_MasterTransferCreateHandle(base, &data->handle, i2c_mcux_master_transfer_callback,
				       (void *)dev);
...
#define I2C_DEVICE_INIT_MCUX(n)                                                                \
	...
	static struct i2c_mcux_data i2c_mcux_data_##n;                                         \
                                                                                               \
	I2C_DEVICE_DT_INST_DEFINE(n, i2c_mcux_init, NULL, &i2c_mcux_data_##n,                  \
				  &i2c_mcux_config_##n, POST_KERNEL, CONFIG_I2C_INIT_PRIORITY, \
				  &i2c_mcux_driver_api);                                       \
                                                                                               \
	static void i2c_mcux_config_func_##n(const struct device *dev)                         \
	{                                                                                      \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), i2c_mcux_isr,           \
