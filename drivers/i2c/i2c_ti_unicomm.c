...
struct i2c_ti_unicomm_config {
	...
	const struct mspm0_sys_clock *clock_subsys; /* Clock subsystem for clock_control */
...
static int i2c_ti_unicomm_init(const struct device *dev)
{
	...
	/* Select I2C Controller mode */
	sys_write32(UNICOMM_IPMODE_I2CC, cfg->unicomm_base + UNICOMM_IPMODE);
						 ...
						 struct i2c_target_config *target_cfg)
{
	...
	/* Clock config */
	sys_write32(UNICOMM_IPMODE_I2CT, cfg->unicomm_base + UNICOMM_IPMODE);
...
static inline int i2c_ti_unicomm_target_unregister(const struct device *dev,
						   struct i2c_target_config *target_cfg)
{
	const struct i2c_ti_unicomm_config *cfg = dev->config;
	struct i2c_ti_unicomm_data *data = dev->data;

	/* Disable I2CT interrupts */
	sys_write32(0, cfg->unicomm_i2ct_base + UNICOMM_I2CT_CPU_INT_IMASK);
		...
		case I2CT_IIDX_TARGET_RXFIFO_TRIGGER:
			...
			while (!I2CT_IS_RX_FIFO_EMPTY(cfg->unicomm_i2ct_base)) {
				ret = data->target_cfg->callbacks->write_received(
					data->target_cfg,
					sys_read32(cfg->unicomm_i2ct_base + UNICOMM_I2CT_RXDATA));
				if (ret < 0) {
					i2ct_send_nack(cfg->unicomm_i2ct_base);
					break;
				}
...
static DEVICE_API(i2c, i2c_ti_unicomm_api) = {.configure = i2c_ti_unicomm_configure,
					      ...
					      .target_unregister = i2c_ti_unicomm_target_unregister
#endif
};
...
#define I2C_TI_UNICOMM_INIT(index)                                                                 \
	...
	{                                                                                          \
		...
		irq_enable(DT_INST_IRQN(index));                                                   \
                                                                                                   ...
                                                                                                   \
	static const struct mspm0_sys_clock i2c_ti_unicomm_clock_subsys_##index =                  \
