...
#define DT_DRV_COMPAT litex_i2c_litei2c
			      ...
			      uint16_t addr)
{
	...
	litex_write8(1, config->master_enable_addr);
...
transfer_end:
	...
	litex_write8(0, config->master_enable_addr);
...
#define I2C_LITEX_INIT(n)                                                                          \
	static const struct i2c_litex_litei2c_config i2c_litex_litei2c_config_##n = {              \
		...
		.master_enable_addr = DT_INST_REG_ADDR_BY_NAME(n, master_enable),                  \
