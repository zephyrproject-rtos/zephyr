				    ...
				    struct ra_iic_master_bitrate *result)
{
	...
	const double rise_time_ns = config->rise_time_ns / RA_IIC_MASTER_DIV_TIME_NS;
	const double fall_time_ns = config->fall_time_ns / RA_IIC_MASTER_DIV_TIME_NS;
	...
	result->bitrate = 1 / ((total_brl_brh + 2 * constant_add) / divided_pclk + rise_time_ns +
			       fall_time_ns);
	result->duty = 100 * ((rise_time_ns + ((brh + constant_add) / divided_pclk)) /
	...
	LOG_DBG("%s: [input] total_brl_brh[%d] brh[%d] divider[%d]"
		" [output] bitrate[%u] duty[%u] divider[%u] brh[%u] brl[%u] duty_error_percent[%u]\n",
					  ...
					  iic_master_clock_settings_t *clk_cfg)
{
	...
	for (int temp_divider = 0; temp_divider <= 7; ++temp_divider) {
		...
		uint32_t total_brl_brh = ceil(
			((1 / (double)requested_bitrate) - (rise_time_ns + fall_time_ns)) *
					divided_pclk -
				(2 * constant_add));
...
#define I2C_RA_IIC_INIT(index)                                                                     \
	...
	static const struct i2c_ra_iic_config i2c_ra_iic_config_##index = {                        \
		...
		.rise_time_ns = DT_INST_PROP(index, rise_time_ns),                                 \
		.fall_time_ns = DT_INST_PROP(index, fall_time_ns),                                 \
