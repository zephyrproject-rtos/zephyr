				    ...
				    struct ra_iic_master_bitrate *result)
{
	...
	result->bitrate = 1 / ((total_brl_brh + 2 * constant_add) / divided_pclk + rise_time_s +
			       fall_time_s);
	result->duty = 100 * ((rise_time_s + ((brh + constant_add) / divided_pclk)) /
					  ...
					  iic_master_clock_settings_t *clk_cfg)
{
	...
	for (int temp_divider = 0; temp_divider <= 7; ++temp_divider) {
		...
		uint32_t total_brl_brh = ceil(
			((1 / (double)requested_bitrate) - (rise_time_s + fall_time_s)) *
					divided_pclk -
				(2 * constant_add));
