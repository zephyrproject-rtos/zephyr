...
static void i2c_ra_iic_slave_callback(i2c_slave_callback_args_t *p_args)
{
	...
	case I2C_SLAVE_EVENT_RX_REQUEST:
		...
		if (err == 0) {
			...
			fsp_err = R_IIC_SLAVE_Read(&data->slave_ctrl, data->slave_buf, 0);
			__ASSERT_NO_MSG(fsp_err == FSP_SUCCESS);
			LOG_DBG("I2C target does not want to receive data."
				"Send a NACK to master to terminate the transaction.");
		}
	...
	case I2C_SLAVE_EVENT_TX_REQUEST:
		...
		if (err == 0) {
			...
			LOG_DBG("I2C target doesn't provide new data. The I2C bus will be left "
				"floating, MASTER will read the value 0xFF for the remaining "
				"bytes.");
		}
		data->transaction_completed = false;
		break;
