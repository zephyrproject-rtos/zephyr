...
static int rv32m1_lpi2c_configure(const struct device *dev, uint32_t dev_config)
{
...
static void rv32m1_lpi2c_master_transfer_callback(LPI2C_Type *base, lpi2c_master_handle_t *handle,
...
static int rv32m1_lpi2c_transfer(const struct device *dev, struct i2c_msg *msgs, uint8_t num_msgs,
				 uint16_t addr)
{
	const struct rv32m1_lpi2c_config *config = dev->config;
	...
	for (int i = 0; i < num_msgs; i++) {
		...
		transfer.direction = (msgs->flags & I2C_MSG_READ) ? kLPI2C_Read : kLPI2C_Write;
		...
		/* Start the transfer */
		status = LPI2C_MasterTransferNonBlocking(config->base, &data->handle, &transfer);
		...
		if (data->completion_status != kStatus_Success) {
			LOG_DBG("Transfer failed (status %d)", data->completion_status);
...
static int rv32m1_lpi2c_init(const struct device *dev)
{
	...
	LPI2C_MasterTransferCreateHandle(config->base, &data->handle,
					 rv32m1_lpi2c_master_transfer_callback, data);
