...
static const struct device *const fsmbm_devices[] = {DT_INST_FOREACH_STATUS_OKAY(KB106X_FSMBM_DEV)};
static void i2c_kb106x_isr_wrap(void)
{
...
static void kb106x_fsmbm_irq_init(void)
{
	if (init_irq) {
		init_irq = false;
...
#define I2C_KB106X_DEVICE(inst)                                                                    \
	...
	DEVICE_DT_INST_DEFINE(inst, &i2c_kb106x_init, NULL, &i2c_kb106x_data_##inst,               \
			      &i2c_kb106x_config_##inst, PRE_KERNEL_1,                             \
