...
static int i2c_nrfx_twim_transfer(const struct device *dev, struct i2c_msg *msgs, uint8_t num_msgs,
				  uint16_t addr)
{
	...
	nrfx_twim_xfer_desc_t cur_xfer = {.address = addr};
	...
	for (size_t i = 0; i < num_msgs; i++) {
		...
		if (concat_next || (msg_buf_used != 0) || !dma_accessible) {
			...
			if (!(msgs[i].flags & I2C_MSG_READ)) {
				memcpy(msg_buf + msg_buf_used, msgs[i].buf, msgs[i].len);
			}
		...
		cur_xfer.type =
			(msgs[i].flags & I2C_MSG_READ) ? NRFX_TWIM_XFER_RX : NRFX_TWIM_XFER_TX;

		if (cur_xfer.primary_length > dev_config->max_transfer_size) {
			LOG_ERR("Trying to transfer more than the maximum size "
				"for this device: %d > %d",
				cur_xfer.primary_length, dev_config->max_transfer_size);
			return -ENOSPC;
		}

		nrfx_err_t res = nrfx_twim_xfer(
			&dev_config->twim, &cur_xfer,
			(msgs[i].flags & I2C_MSG_STOP) ? 0 : NRFX_TWIM_FLAG_TX_NO_STOP);
		if (res != NRFX_SUCCESS) {
		...
		if ((msgs[i].flags & I2C_MSG_READ) && cur_xfer.p_primary_buf == msg_buf) {
			int j = i;

			while (msg_buf_used >= msgs[j].len) {
				msg_buf_used -= msgs[j].len;
				memcpy(msgs[j].buf, msg_buf + msg_buf_used, msgs[j].len);
				j--;
			}
		}
...
static int i2c_nrfx_twim_configure(const struct device *dev, uint32_t i2c_config)
{
	const struct i2c_nrfx_twim_config *dev_config = dev->config;
	...
	case I2C_SPEED_STANDARD:
		nrf_twim_frequency_set(dev_config->twim.p_twim, NRF_TWIM_FREQ_100K);
		break;
	case I2C_SPEED_FAST:
		nrf_twim_frequency_set(dev_config->twim.p_twim, NRF_TWIM_FREQ_400K);
		break;
#if NRF_TWIM_HAS_1000_KHZ_FREQ
	case I2C_SPEED_FAST_PLUS:
		nrf_twim_frequency_set(dev_config->twim.p_twim, NRF_TWIM_FREQ_1000K);
		break;
#endif
	default:
...
static int twim_nrfx_pm_action(const struct device *dev, enum pm_device_action action)
{
	const struct i2c_nrfx_twim_config *dev_config = dev->config;
...
static int i2c_nrfx_twim_init(const struct device *dev)
{
	...
	if (nrfx_twim_init(&dev_config->twim, &dev_config->twim_config, event_handler, dev_data) !=
	    NRFX_SUCCESS) {
...
#define I2C_NRFX_TWIM_DEVICE(idx)                                                                  \
	...
	I2C_DEVICE_DT_DEFINE(I2C(idx), i2c_nrfx_twim_init, PM_DEVICE_DT_GET(I2C(idx)),             \
