	...
	if (isr & I2C_ISR_NACKF) {
		...
		*data->current.buf = regs->RXDR;
		...
		if (data->current.len == 0) {
			...
			if (data->current.continue_in_next == false) {
		...
		regs->TXDR = *data->current.buf;
		data->current.len--;
		data->current.buf++;

	} else if (isr & I2C_ISR_TC) {
		/* Transfer Complete, no reload this time so either do stop now or
		 * restart in thread
		 */
			      ...
			      uint8_t *next_msg_flags, uint16_t slave)
{
	...
	uint32_t irq_enable_flags = I2C_CR1_ERRIE | I2C_CR1_STOPIE | I2C_CR1_TCIE | I2C_CR1_NACKIE;

#ifdef CONFIG_I2C_STM32_V2_DMA
	dma_xfer_start(dev, msg);
#else
	/* If not using DMA, also enable RX and TX empty interrupts */
	irq_enable_flags |= I2C_CR1_TXIE | I2C_CR1_RXIE;
#endif
	/* Commit to I2C controller */
	regs->CR2 = cr2;

	/* Enable interrupts */
	regs->CR1 |= irq_enable_flags;
