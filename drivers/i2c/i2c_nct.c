...
#define DT_DRV_COMPAT nuvoton_nct_i2c
...
LOG_MODULE_REGISTER(i2c_nct, LOG_LEVEL_ERR);
...
/* I2C operation state */
enum i2c_nct_oper_state {
	I2C_NCT_OPER_STA_IDLE,
	I2C_NCT_OPER_STA_START,
	I2C_NCT_OPER_STA_WRITE,
	I2C_NCT_OPER_STA_READ,
	I2C_NCT_OPER_STA_QUICK,
};

/* Device configuration */
struct i2c_nct_config {
...
struct i2c_nct_data {
	...
	enum i2c_nct_oper_state ctrl_oper_state;
	enum i2c_nct_oper_state target_oper_state;
	uint32_t bitrate;
	uint32_t source_clk;
	uint16_t rx_cnt;
	uint16_t tx_cnt;
...
#define I2C_INSTANCE(dev) ((struct i2c_reg *)((const struct i2c_nct_config *)(dev)->config)->base)
 ...
 */
static inline void i2c_nct_start(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);

	inst->SMBnCTL1 = (inst->SMBnCTL1 & ~0x13) | BIT(NCT_SMBnCTL1_START);
}

static inline void i2c_nct_stop(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);

	(inst)->SMBnCTL1 = ((inst)->SMBnCTL1 & ~0x13) | BIT(NCT_SMBnCTL1_STOP);
}

static inline void i2c_nct_enable_stall(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);

	inst->SMBnCTL1 = (inst->SMBnCTL1 & ~0x13) | BIT(NCT_SMBnCTL1_STASTRE);
}

static inline void i2c_nct_disable_stall(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
...
static inline void i2c_nct_nack(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);

	inst->SMBnCTL1 = (inst->SMBnCTL1 & ~0x13) | BIT(NCT_SMBnCTL1_ACK);
}

static void i2c_nct_start_DMA(const struct device *dev,
				  uint32_t addr, uint16_t len)
{
	...
	/* clear interrupt bit */
	inst->DMA_CTRL = BIT(NCT_DMA_CTRL_DMA_INT_CLR);

	/* Set DMA enable */
	inst->DMA_CTRL = BIT(NCT_DMA_CTRL_DMA_EN);
}

/* return Negative Acknowledge when DMA received last byte. */
static void i2c_nct_DMA_lastbyte(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);

	inst->DMA_CTRL = (inst->DMA_CTRL & ~BIT(NCT_DMA_CTRL_DMA_INT_CLR)) |
			 BIT(NCT_DMA_CTRL_LAST_PEC);
}

static uint16_t i2c_nct_get_dma_cnt(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
...
void Set_Cumulative_ClockLow_Timeout(const struct device *dev, uint8_t interval_ms)
{
	...
	inst->SMBnCTL2 |= BIT(NCT_SMBnCTL2_ENABLE);
	inst->TIMEOUT_EN = (div << NCT_TIMEOUT_EN_TO_CKDIV);
	inst->TIMEOUT_ST |= BIT(NCT_TIMEOUT_ST_T_OUTST1_EN);
	inst->TIMEOUT_ST = BIT(NCT_TIMEOUT_ST_T_OUTST1);
...
void Set_Cumulative_ClockCycle_Timeout(const struct device *dev, uint8_t interval_ms)
{
	...
	inst->SMBnCTL2 |= BIT(NCT_SMBnCTL2_ENABLE);
	inst->TIMEOUT_EN = (div << NCT_TIMEOUT_EN_TO_CKDIV);
	inst->TIMEOUT_ST |= BIT(NCT_TIMEOUT_ST_T_OUTST2_EN);
	inst->TIMEOUT_ST = BIT(NCT_TIMEOUT_ST_T_OUTST2);
	inst->TIMEOUT_CTL1 = interval_ms;
}

static void i2c_nct_reset_module(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	struct i2c_nct_data *const data = dev->data;
	uint32_t ctl1_tmp;
	...
	inst->SMBnCTL1 = (ctl1_tmp & (BIT(NCT_SMBnCTL1_INTEN) | BIT(NCT_SMBnCTL1_EOBINTE) |
				      BIT(NCT_SMBnCTL1_GCMEN) | BIT(NCT_SMBnCTL1_NMINTE)));
	inst->TIMEOUT_EN = timeout_en_tmp;

	data->ctrl_oper_state = I2C_NCT_OPER_STA_IDLE;
}

static void i2c_nct_abort_data(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	...
	inst->SMBnST = (BIT(NCT_SMBnST_STASTR) |
			BIT(NCT_SMBnST_NEGACK) |
			BIT(NCT_SMBnST_BER));

	/* Wait till STOP condition is generated */
	while (--timeout) {
		if ((inst->SMBnCTL1 & BIT(NCT_SMBnCTL1_STOP)) == 0x00) {
			break;
		}
	}

	/* Clear BB (BUS BUSY) bit */
	inst->SMBnCST = BIT(NCT_SMBnCST_BB);
}
...
static void i2c_nct_set_baudrate(const struct device *dev, uint32_t bus_freq)
{
	uint32_t reg_tmp;
	const struct i2c_nct_config *const config = dev->config;
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	...
	if (bus_freq < 400000) {
		...
		inst->SMBnCTL3 &= ~BIT(NCT_SMBnCTL3_400K_MODE);
		SET_FIELD(inst->SMBnCTL2, NCT_SMBnCTL2_SCLFRQ60_FIELD, reg_tmp & 0x7f);
		SET_FIELD(inst->SMBnCTL3, NCT_SMBnCTL3_SCLFRQ87_FIELD, reg_tmp >> 7);
		...
		inst->SMBnCTL3 |= BIT(NCT_SMBnCTL3_400K_MODE);
		SET_FIELD(inst->SMBnCTL2, NCT_SMBnCTL2_SCLFRQ60_FIELD, 0);
		SET_FIELD(inst->SMBnCTL3, NCT_SMBnCTL3_SCLFRQ87_FIELD, 0);
...
static void i2c_nct_notify(const struct device *dev, int error)
{
...
static int i2c_nct_wait_completion(const struct device *dev)
{
	struct i2c_nct_data *const data = dev->data;
	int ret;
...
static int i2c_nct_mutex_lock(const struct device *dev, k_timeout_t timeout)
{
	struct i2c_nct_data *const data = dev->data;
...
static void i2c_nct_mutex_unlock(const struct device *dev)
{
	struct i2c_nct_data *const data = dev->data;
...
static void i2c_nct_ctrl_isr(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	struct i2c_nct_data *const data = dev->data;
	...
	/* --------------------------------------------- */
#if (CONFIG_CONTROLLER_HW_TIMEOUT_EN == 'Y')
	if (inst->TIMEOUT_ST & BIT(NCT_TIMEOUT_ST_T_OUTST1)) {
		/* clear timeout flag */
		inst->TIMEOUT_ST = BIT(NCT_TIMEOUT_ST_T_OUTST1);
		i2c_nct_reset_module(dev);
		i2c_nct_notify(dev, -ETIMEDOUT);
	}
	if (inst->TIMEOUT_ST & BIT(NCT_TIMEOUT_ST_T_OUTST2)) {
		/* clear timeout flag */
		inst->TIMEOUT_ST = BIT(NCT_TIMEOUT_ST_T_OUTST2);
		i2c_nct_reset_module(dev);
		i2c_nct_notify(dev, -ETIMEDOUT);
	...
	/* NACK occurred                                 */
	/* --------------------------------------------- */
	if (inst->SMBnST & BIT(NCT_SMBnST_NEGACK)) {
		i2c_nct_abort_data(dev);
		/* Clear DMA flag */
		inst->DMA_CTRL = BIT(NCT_DMA_CTRL_DMA_INT_CLR);
		data->ctrl_oper_state = I2C_NCT_OPER_STA_IDLE;
		i2c_nct_notify(dev, -ENXIO);
	}

	/* --------------------------------------------- */
	/* BUS ERROR occurred                            */
	/* --------------------------------------------- */
	if (inst->SMBnST & BIT(NCT_SMBnST_BER)) {
		i2c_nct_abort_data(dev);
		data->ctrl_oper_state = I2C_NCT_OPER_STA_IDLE;
		i2c_nct_reset_module(dev);
	...
	if (inst->SMBnST & BIT(NCT_SMBnST_SDAST)) {
		if (data->ctrl_oper_state == I2C_NCT_OPER_STA_START) {
			if (data->tx_cnt == 0 && data->rx_cnt == 0) {
				/* Quick command (SMBUS protocol) */
				data->ctrl_oper_state = I2C_NCT_OPER_STA_QUICK;
				i2c_nct_enable_stall(dev);
				/* quick read or quick write is determined by address */
				inst->SMBnSDA = data->dev_addr;
			} else if (data->tx_cnt == 0 && data->rx_cnt > 0) {
				/* receive mode */
				data->ctrl_oper_state = I2C_NCT_OPER_STA_READ;
				i2c_nct_enable_stall(dev);
				/* send read address */
				inst->SMBnSDA = data->dev_addr | 0x1;
			} else if (data->tx_cnt > 0) {
				/* transmit mode */
				data->ctrl_oper_state = I2C_NCT_OPER_STA_WRITE;
				/* send write address */
				inst->SMBnSDA = data->dev_addr & 0xFE;
			}
		} else if (data->ctrl_oper_state == I2C_NCT_OPER_STA_WRITE) {
			/* Set DMA register to send data */
			i2c_nct_start_DMA(dev, (uint32_t)data->tx_buf, data->tx_cnt);
		} else {
			/* Error */
		}
	}
	...
	/* stall occurred                                */
	/* --------------------------------------------- */
	if (inst->SMBnST & BIT(NCT_SMBnST_STASTR)) {
		if (data->ctrl_oper_state == I2C_NCT_OPER_STA_READ) {
			/* Set DMA register to read data */
			i2c_nct_DMA_lastbyte(dev);
			i2c_nct_start_DMA(dev, (uint32_t)data->rx_buf, data->rx_cnt);
		} else if (data->ctrl_oper_state == I2C_NCT_OPER_STA_QUICK) {
			i2c_nct_stop(dev);
			data->ctrl_oper_state = I2C_NCT_OPER_STA_IDLE;
			i2c_nct_notify(dev, 0);
		} else {
			/* error */
		}

		i2c_nct_disable_stall(dev);
		/* clear STASTR flag */
		inst->SMBnST = BIT(NCT_SMBnST_STASTR);
	}
	...
	/* DMA IRQ occurred                              */
	/* --------------------------------------------- */
	if (inst->DMA_CTRL & BIT(NCT_DMA_CTRL_DMA_IRQ)) {
		if (data->ctrl_oper_state == I2C_NCT_OPER_STA_WRITE) {
			/* Transmit mode */
			if (data->rx_cnt == 0) {
			...
			i2c_nct_stop(dev);
			data->ctrl_oper_state = I2C_NCT_OPER_STA_IDLE;
			data->rx_cnt = i2c_nct_get_DMA_cnt(dev);
			i2c_nct_notify(dev, 0);
		}
		/* Clear DMA flag */
		inst->DMA_CTRL = BIT(NCT_DMA_CTRL_DMA_INT_CLR);
...
static void i2c_nct_target_isr(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	struct i2c_nct_data *const data = dev->data;
	...
	/* --------------------------------------------- */
	if (inst->SMBnST & BIT(NCT_SMBnST_NEGACK)) {
		inst->SMBnST = BIT(NCT_SMBnST_NEGACK);
	}
	...
	/* --------------------------------------------- */
	if (inst->SMBnST & BIT(NCT_SMBnST_BER)) {
		if (data->target_oper_state != I2C_NCT_OPER_STA_QUICK) {
			LOG_ERR("target: bus error");
		}
		data->target_oper_state = I2C_NCT_OPER_STA_START;
		/* clear BER */
		inst->SMBnST = BIT(NCT_SMBnST_BER);
		i2c_nct_reset_module(dev);
	}
	...
	if (inst->SMBnST & BIT(NCT_SMBnST_NMATCH)) {
		if (inst->SMBnST & BIT(NCT_SMBnST_XMIT)) {
			/* target received Read-Address */
			if (data->target_oper_state != I2C_NCT_OPER_STA_START) {
			...
			i2c_nct_start_DMA(dev, (uint32_t)data->rx_buf, sizeof(data->rx_buf));
...
static void i2c_set_target_addr(const struct device *dev, uint8_t target_addr)
{
	...
	/* set target addr 1 */
	inst->SMBnADDR1 = (target_addr | BIT(NCT_SMBnADDR_SAEN));

	/* Enable I2C address match interrupt */
	inst->SMBnCTL1 |= BIT(NCT_SMBnCTL1_NMINTE);
}

static int i2c_nct_target_register(const struct device *dev,
...
static int i2c_nct_target_unregister(const struct device *dev,
...
static void i2c_nct_isr(const struct device *dev)
{
	struct i2c_reg *const inst = I2C_INSTANCE(dev);
	struct i2c_nct_data *data = dev->data;
...
static int i2c_nct_init(const struct device *dev)
{
	const struct i2c_nct_config *const config = dev->config;
	struct i2c_nct_data *const data = dev->data;
	const struct device *const clk_dev = DEVICE_DT_GET(DT_NODELABEL(pcc));
	...
	/* reset data */
	gdma_memset_u8((uint8_t *)data, 0, sizeof(struct i2c_nct_data));

	/* Set default baud rate for i2c*/
	data->bitrate = config->default_bitrate;
	LOG_DBG("bitrate: %d", data->bitrate);
	i2c_nct_set_baudrate(dev, data->bitrate);

	/* Enable I2C module */
	inst->SMBnCTL2 |= BIT(NCT_SMBnCTL2_ENABLE);
	/* Enable I2C interrupt */
	inst->SMBnCTL1 |= BIT(NCT_SMBnCTL1_INTEN);
...
static int i2c_nct_configure(const struct device *dev, uint32_t dev_config)
{
	struct i2c_nct_data *const data = dev->data;
...
static int i2c_nct_combine_msg(const struct device *dev,
				...
				uint8_t num_msgs, uint16_t addr)
{
	...
	data->ctrl_oper_state = I2C_NCT_OPER_STA_START;
	data->err_code = 0;
	...
	ret = i2c_nct_wait_completion(dev);
...
static const struct i2c_driver_api i2c_nct_driver_api = {
	.configure = i2c_nct_configure,
	.transfer = i2c_nct_transfer,
	.target_register = i2c_nct_target_register,
	.target_unregister = i2c_nct_target_unregister,
};


/* I2C controller init macro functions */
#define I2C_NCT_CTRL_INIT_FUNC(inst) _CONCAT(i2c_nct_init_, inst)
#define I2C_NCT_CTRL_INIT_FUNC_DECL(inst)					 \
	static int i2c_nct_init_##inst(const struct device *dev)
#define I2C_NCT_CTRL_INIT_FUNC_IMPL(inst)					 \
	static int i2c_nct_init_##inst(const struct device *dev)		 \
	{									 \
		int ret;							 \
										 \
		ret = i2c_nct_init(dev);					 \
...
#define I2C_NCT_CTRL_INIT(inst)						 \
	PINCTRL_DT_INST_DEFINE(inst);						 \
										 \
	I2C_NCT_CTRL_INIT_FUNC_DECL(inst);					 \
										 \
	static const struct i2c_nct_config i2c_nct_cfg_##inst = {		 \
		...
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(inst),			 \
										 ...
										 \
	static struct i2c_nct_data i2c_nct_data_##inst;			 \
										 \
	DEVICE_DT_INST_DEFINE(inst,						 \
			      I2C_NCT_CTRL_INIT_FUNC(inst),			 \
			      NULL,						 \
			      &i2c_nct_data_##inst, &i2c_nct_cfg_##inst,	 \
			      PRE_KERNEL_1, CONFIG_I2C_INIT_PRIORITY,		 \
			      &i2c_nct_driver_api);				 \
										 \
	I2C_NCT_CTRL_INIT_FUNC_IMPL(inst)

DT_INST_FOREACH_STATUS_OKAY(I2C_NCT_CTRL_INIT)
