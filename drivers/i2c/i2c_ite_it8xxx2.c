...
#define DT_DRV_COMPAT it8xxx2_i2c
...
#define DEV_CFG(dev) \
		((const struct i2c_it8xxxx_config * const)(dev)->config)
#define DEV_DATA(dev) \
		((struct i2c_it8xxxx_data * const)(dev)->data)

#define I2C_STANDARD_PORT_COUNT 3
/* Default PLL frequency. */
#define PLL_CLOCK 48000000

struct i2c_it8xxxx_config {
	uint8_t *base;
...
{
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
...
static void i2c_enhanced_port_set_frequency(const struct device *dev,
				int freq_khz)
{
	struct i2c_it8xxxx_data *data = DEV_DATA(dev);
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
	uint8_t *base = config->base;
	int clk_div, psr;
...
static int enhanced_i2c_error(const struct device *dev)
{
	...
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
	uint8_t *base = config->base;
	int i2c_str = IT83XX_I2C_STR(base);
...
static int i2c_it8xxx2_read_msg(const struct device *dev)
{
	struct i2c_it8xxxx_data *data = DEV_DATA(dev);
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
	uint8_t *base = config->base;
...
static int i2c_it8xxx2_write_msg(const struct device *dev)
{
	struct i2c_it8xxxx_data *data = DEV_DATA(dev);
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
	uint8_t *base = config->base;
		...
		if (IT83XX_SMB_HOSTA(base) & HOSTA_ANY_ERROR) {
			...
			if (!data->stop) {
				if (data->msgs->flags & I2C_MSG_READ)
					return i2c_it8xxx2_read_msg(dev);
				else
					return i2c_it8xxx2_write_msg(dev);
			}
			/* wait finish */
...
static void i2c_it8xxxx_isr(void *arg)
{
...
static int i2c_it8xxxx_init(const struct device *dev)
{
	struct i2c_it8xxxx_data *data = DEV_DATA(dev);
	const struct i2c_it8xxxx_config *config = DEV_CFG(dev);
	uint8_t *base = config->base;
...
static const struct i2c_driver_api i2c_it8xxxx_driver_api = {
	.configure = i2c_it8xxx2_configure,
	.transfer = i2c_it8xxx2_transfer,
};

#define I2C_ITE_IT8XXX2_INIT(idx)                                              \
	static void i2c_it8xxxx_config_func_##idx(void);                       \
									       \
	static const struct i2c_it8xxxx_config i2c_it8xxxx_cfg_##idx = {       \
		.base = (uint8_t *)(DT_INST_REG_ADDR(idx)),                    \
		.irq_config_func = i2c_it8xxxx_config_func_##idx,              \
		.bitrate = DT_INST_PROP(idx, clock_frequency),                 \
		.i2c_irq_base = DT_INST_IRQN(idx),                             \
		.port = DT_INST_PROP(idx, port_num),                           \
	};                                                                     \
	\
	static struct i2c_it8xxxx_data i2c_it8xxxx_data_##idx;	               \
	\
	DEVICE_AND_API_INIT(i2c_it8xxx2_##idx,				       \
			DT_INST_LABEL(idx),                                    \
			&i2c_it8xxxx_init, &i2c_it8xxxx_data_##idx,	       \
			&i2c_it8xxxx_cfg_##idx, POST_KERNEL,		       \
			CONFIG_KERNEL_INIT_PRIORITY_DEVICE,                    \
			&i2c_it8xxxx_driver_api);                              \
	\
	static void i2c_it8xxxx_config_func_##idx(void)                        \
	{                                                                      \
