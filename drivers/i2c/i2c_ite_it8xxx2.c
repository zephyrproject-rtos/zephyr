...
static int i2c_it8xxx2_init(const struct device *dev)
{
	...
	*reg &= ~reg_mask;
	...
	pinmux_pin_set(config->alts_list[0].pinctrls,
		       config->alts_list[0].pin,
		       config->alts_list[0].alt_fun);
	/* The pin is set to I2C alternate function of SDA */
	pinmux_pin_set(config->alts_list[1].pinctrls,
		       config->alts_list[1].pin,
		       config->alts_list[1].alt_fun);
...
static int i2c_it8xxx2_recover_bus(const struct device *dev)
{
	...
	pinmux_pin_input_enable(config->alts_list[0].pinctrls,
				config->alts_list[0].pin,
				PINMUX_OUTPUT_ENABLED);
	/* Set SDA of I2C as GPIO pin */
	pinmux_pin_input_enable(config->alts_list[1].pinctrls,
				config->alts_list[1].pin,
				PINMUX_OUTPUT_ENABLED);

	/* Pull SCL and SDA pin to high */
	gpio_pin_set(config->gpio_dev, config->alts_list[0].pin, 1);
	gpio_pin_set(config->gpio_dev, config->alts_list[1].pin, 1);
	k_msleep(1);

	/* Start condition */
	gpio_pin_set(config->gpio_dev, config->alts_list[1].pin, 0); /* SDA */
	k_msleep(1);
	gpio_pin_set(config->gpio_dev, config->alts_list[0].pin, 0); /* SCL */
	k_msleep(1);
	...
	for (i = 0; i < 9; i++) {
		...
		k_msleep(1);
		/* SCL */
		gpio_pin_set(config->gpio_dev, config->alts_list[0].pin, 0);
		k_msleep(1);
	}
	/* SDA */
	gpio_pin_set(config->gpio_dev, config->alts_list[1].pin, 0);
	k_msleep(1);

	/* Stop condition */
	gpio_pin_set(config->gpio_dev, config->alts_list[0].pin, 1); /* SCL */
	k_msleep(1);
	gpio_pin_set(config->gpio_dev, config->alts_list[1].pin, 1); /* SDA */
	k_msleep(1);
