...
#define I2C_SF32LB_DEFINE(n)                                                                       \
	IF_ENABLED(DT_INST_NODE_HAS_PROP(n, dmas),                                                 \
		(BUILD_ASSERT(IS_ENABLED(CONFIG_DMA),                                              \
		    "DMA support is not enabled but dmas property is set in DT");))                \
	...
	static const struct i2c_sf32lb_config i2c_sf32lb_config_##n = {                            \
		...
		IF_ENABLED(CONFIG_DMA, (.dma_tx = SF32LB_DMA_DT_INST_SPEC_GET_BY_NAME(n, tx),))    \
		IF_ENABLED(CONFIG_DMA, (.dma_rx = SF32LB_DMA_DT_INST_SPEC_GET_BY_NAME(n, rx),))    \
	};                                                                                         \
