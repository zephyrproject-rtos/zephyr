				...
				virtq_receive_callback cb)
{
	...
	uint16_t nbufs = 0;
	uint16_t readable;
	bool is_read = (msg->flags & I2C_MSG_RW_MASK) == I2C_MSG_READ;
	uint32_t flags = 0;
	...
	/*
	 * Every message but the last carries FAIL_NEXT, so a failure partway
	 * through takes the rest of the transfer with it instead of letting
	 * later messages land on a chip that never saw the earlier ones.
	 */
	if (!last) {
			       ...
			       uint16_t addr)
{
	...
	for (uint16_t base = 0; base < num_msgs; base += cfg->max_msgs) {
		uint16_t batch = MIN(cfg->max_msgs, num_msgs - base);
		uint16_t queued = 0;
		...
		for (uint16_t i = 0; i < batch; i++) {
			 ...
			 * or a failure would stop propagating at the seam.
			 */
...
static int i2c_virtio_init(const struct device *dev)
{
	...
	ret = virtio_init_virtqueues(cfg->vdev, 1, i2c_virtio_enum_queues_cb, (void *)cfg);
	if (ret != 0) {
