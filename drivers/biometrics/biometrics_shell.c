...
static int cmd_biometric_enroll_capture(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = biometric_enroll_capture(dev, timeout);
	if (ret < 0) {
		shell_error(sh, "Capture failed [%d]", ret);
		return ret;
	}

	shell_print(sh, "Sample captured");
	return 0;
}
...
static int cmd_biometric_match(const struct shell *sh, size_t argc, char **argv)
{
	...
	ret = biometric_match(dev, mode, (uint16_t)template_id, timeout);
	if (ret < 0) {
		if (ret == -ENOENT) {
	...
	shell_print(sh, "Match, score: %d", ret);
