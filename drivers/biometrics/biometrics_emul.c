...
#define BIOMETRICS_EMUL_DEFAULT_SECURITY  5000
				 ...
				 uint16_t template_id, k_timeout_t timeout)
{
	...
	if (mode == BIOMETRIC_MATCH_VERIFY) {
		ret = biometrics_emul_match_verify(data, template_id);
	} else {
		ret = biometrics_emul_match_identify(data);
	}

	LOG_INF("Match result: %d (mode=%d, template_id=%u)", ret, mode, template_id);
