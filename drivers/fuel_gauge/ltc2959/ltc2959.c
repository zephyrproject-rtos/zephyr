...
/* ACR fixed LSB: 533 nAh = 0.533 µAh */
...
static inline uint32_t ltc2959_counts_to_uah(uint32_t counts)
{
	/* µAh = round(counts * 533 / 1000) */
	uint64_t prod = (uint64_t)counts * (uint64_t)LTC2959_ACR_UAH_NUM;

	return u64_div_round_closest_u32_sat(prod, LTC2959_ACR_UAH_DEN);
}

static inline uint32_t ltc2959_uah_to_counts(uint32_t uah)
{
	/* counts = round(µAh * 1000 / 533) */
	uint64_t prod = (uint64_t)uah * (uint64_t)LTC2959_ACR_UAH_DEN;

	return u64_div_round_closest_u32_sat(prod, (uint32_t)LTC2959_ACR_UAH_NUM);
}
...
static int ltc2959_set_gpio_threshold_uv(const struct device *dev, bool high, int32_t value_uv)
{
	...
	case LTC2959_GPIO_MODE_BIPOLAR: {
		int16_t raw_bp =
			(int16_t)(((int64_t)value_uv * 32768) / LTC2959_GPIO_BIPOLAR_UV_SF);
		uint8_t buf[2];
	...
	case LTC2959_GPIO_MODE_UNIPOLAR: {
		uint16_t raw_up =
			(uint16_t)(((int64_t)value_uv * 32768) / LTC2959_GPIO_UNIPOLAR_UV_SF);
		uint8_t buf[2];
			    ...
			    union fuel_gauge_prop_val *val)
{
	...
	case FUEL_GAUGE_REMAINING_CAPACITY:
		...
		val->remaining_capacity = ltc2959_counts_to_uah(acr);
		break;
			    ...
			    union fuel_gauge_prop_val val)
{
	...
	case FUEL_GAUGE_REMAINING_CAPACITY:
		uint32_t counts = ltc2959_uah_to_counts(val.remaining_capacity);
