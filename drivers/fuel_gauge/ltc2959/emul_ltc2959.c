				     ...
				     int addr)
{
	...
	case 1: {
		...
		for (size_t i = 1; i < m->len; i++, reg++) {
			int rc = emul_ltc2959_reg_write(target, reg, m->buf[i]);
			if (rc < 0) {
				return rc; /* rely on reg_write to flag RO/unknown regs */
			}
		}
		return 0;
	}
	...
	case 2: {
		...
		if (m1->flags & I2C_MSG_READ) {
			...
			for (size_t i = 0; i < m1->len; i++) {
				int val;
				int rc = emul_ltc2959_reg_read(target, reg + i, &val);
				if (rc < 0) {
					return rc; /* rely on reg_read to flag bad regs */
				}
			...
			for (size_t i = 0; i < m1->len; i++) {
				int rc = emul_ltc2959_reg_write(target, reg + i, m1->buf[i]);
				if (rc < 0) {
					return rc;
				}
			}
