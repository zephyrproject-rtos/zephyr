...
static int ltc2959_emul_transfer_i2c(const struct emul *target,
				     struct i2c_msg *msgs,
				     int num_msgs, int addr)
{
	...
	switch (num_msgs) {
	case 1: {
		/* Combined register write: [reg, value] */
		...
		if (msg->len != 2) {
			LOG_ERR("Invalid single-message write length: %d", msg->len);
			return -EIO;
		}

		uint8_t reg = msg->buf[0];
		uint8_t val = msg->buf[1];

		return emul_ltc2959_reg_write(target, reg, val);
	}
	...
	case 2: {
		...
		if (msg0->flags & I2C_MSG_READ || msg0->len != 1) {
			LOG_ERR("Invalid first msg (flags=0x%x, len=%d)", msg0->flags, msg0->len);
			return -EIO;
		}
		...
		if (msg1->flags & I2C_MSG_READ) {
			int val;
			int rc = emul_ltc2959_reg_read(target, reg, &val);
			...
			switch (msg1->len) {
			case 1:
				msg1->buf[0] = (uint8_t)(val & 0xFF);
				break;
			case 2:
				sys_put_le16((uint16_t)val, msg1->buf);
				break;
			default:
				LOG_ERR("Invalid read length: %d", msg1->len);
				return -EIO;
			}
			return 0;
		}
		switch (msg1->len) {
		case 1: {
			uint8_t val = msg1->buf[0];

			return emul_ltc2959_reg_write(target, reg, val);
		}
		case 2: {
			uint16_t val = sys_get_le16(msg1->buf);
			int rc = emul_ltc2959_reg_write(target, reg, val & 0xFF);

			if (rc < 0) {
				return rc;
			}

			return emul_ltc2959_reg_write(target, reg + 1, val >> 8);
		}
		default:
			LOG_ERR("Invalid write length: %d", msg1->len);
			return -EIO;
		}
	}
	default:
...
EMUL_DT_INST_DEFINE(n, ltc2959_emul_init, &ltc2959_emul_data_##n,                      \
&ltc2959_emul_cfg_##n, &ltc2959_emul_api_i2c, NULL)
