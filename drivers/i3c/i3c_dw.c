...
#define DEVICE_ADDR_STATIC_MASK        GENMASK(7, 0)
...
#define INTR_SLAVE_MASK                                                                            \
	(INTR_TRANSFER_ERR_STAT | INTR_IBI_UPDATED_STAT | INTR_READ_REQ_RECV_STAT |                \
	 INTR_DEFSLV_STAT | INTR_DYN_ADDR_ASSGN_STAT | INTR_RX_THLD_STAT)
...
#define SLV_INTR_REQ_SIR_CTRL(x)        ((x)&GENMASK(2, 1))
	...
	for (i = 0; i < nresp; i++) {
		...
		if (!(sys_read32(config->regs + PRESENT_STATE) & PRESENT_STATE_CURRENT_MASTER)) {
			...
			for (i = 0; i < cmd->rx_len; i++) {
				...
				/*
				 * TODO: verify how byte size (lol) data is written to a dword size
				 * buffer over i3c in target mode
				 */
				read_rx_fifo(dev, &rx_data, sizeof(rx_data));
 ...
 */
static int get_i3c_addr_pos(const struct device *dev, uint8_t addr)
{
	struct dw_i3c_data *data = dev->data;
	struct dw_i3c_i2c_dev_data *dw_i3c_device_data;
	struct i3c_device_desc *desc = i3c_dev_list_i3c_addr_find(&data->common.attached_dev, addr);
...
static int get_i2c_addr_pos(const struct device *dev, uint16_t addr)
{
	...
	struct i3c_i2c_device_desc *desc =
		i3c_dev_list_i2c_addr_find(&data->common.attached_dev, addr);
	...
	for (i = 0; i < num_msgs; i++) {
		...
		if (msgs[i].flags & I3C_MSG_READ) {
			...
			if (msgs[i].hdr_mode == I3C_MSG_HDR_DDR) {
				...
				rd_speed = target->data_speed.maxrd;
			}
			...
			if (msgs[i].hdr_mode == I3C_MSG_HDR_DDR) {
				...
				wr_speed = target->data_speed.maxwr;
			}
			       ...
			       struct i2c_msg *msgs, uint8_t num_msgs)
{
	...
	for (i = 0; i < num_msgs; i++) {
		...
		if (msgs[i].flags & I3C_MSG_READ) {
			uint8_t rd_speed = target->lvr & I3C_LVR_I2C_MODE(I3C_LVR_I2C_FM_MODE)
			...
			uint8_t wr_speed = target->lvr & I3C_LVR_I2C_MODE(I3C_LVR_I2C_FM_MODE)
						   ? COMMAND_PORT_SPEED_I2C_FM
						   : COMMAND_PORT_SPEED_I2C_FMP;
...
static int dw_i3c_target_ibi_raise_hj(const struct device *dev)
{
	...
	/*
	 * TODO: How to manually raise an IBI HJ, should i set HJ EN Mask to 0 in init, and then set
	 * to 1 here?
	 */
	return 0;
}
...
static int dw_i3c_target_ibi_raise_tir(const struct device *dev, struct i3c_ibi *request)
{
	...
	if (sys_read32(config->regs + SLV_CHAR_CTRL) & SLV_CHAR_CTRL_IBI_PAYLOAD) {
		...
		sys_write32(config->regs + SLV_SIR_DATA, tir_data);
	}

	/* kick off the ibi tir request */
	slv_intr_req |= SLV_INTR_REQ_SIR;
	sys_write32(config->regs + SLV_INTR_REQ, slv_intr_req);
...
static int i3c_dw_irq(const struct device *dev)
{
	...
	if (!(present_state & PRESENT_STATE_CURRENT_MASTER)) {
		...
		/* Data has been written to the rx fifo by a controller while in target mode*/
		if (status & INTR_RX_THLD_STAT) {
			/* while the rx fifo buffer is not empty */
			while (DATA_BUFFER_STATUS_LEVEL_RX(
				       sys_read32(config->regs + DATA_BUFFER_STATUS_LEVEL)) > 0) {
				uint8_t rx_data;
				/*
				 * TODO: verify how byte size (lol) data is written to a dword size
				 * buffer over i3c in target mode
			...
			sys_write32(INTR_RX_THLD_STAT, config->regs + INTR_STATUS);
		}
		/* Read Requested when the CMDQ is empty*/
		if (status & INTR_READ_REQ_RECV_STAT) {
			if (target_cb != NULL && target_cb->write_received_cb != NULL) {
				/* TODO: how should the first byte be handled? */
...
static void init_scl_timing(const struct device *dev)
{
	const struct dw_i3c_config *config = dev->config;
	struct dw_i3c_data *data = dev->data;
	uint32_t scl_timing, hcnt, lcnt;

	uint64_t core_rate = config->core_clk;

	/* I3C_OD */
	hcnt = DIV_ROUND_UP(data->od_thigh_max_ns * core_rate, I3C_PERIOD_NS) - 1;
	if (hcnt < SCL_I3C_TIMING_CNT_MIN) {
		hcnt = SCL_I3C_TIMING_CNT_MIN;
	} else if (hcnt > SCL_I3C_TIMING_CNT_MAX) {
		hcnt = SCL_I3C_TIMING_CNT_MAX;
	}
	lcnt = DIV_ROUND_UP(data->od_tlow_min_ns * core_rate, I3C_PERIOD_NS);
	if (lcnt < SCL_I3C_TIMING_CNT_MIN) {
		lcnt = SCL_I3C_TIMING_CNT_MIN;
	} else if (lcnt > SCL_I3C_TIMING_CNT_MAX) {
		lcnt = SCL_I3C_TIMING_CNT_MAX;
	}
	...
	/* I3C_PP */
	hcnt = DIV_ROUND_UP(I3C_BUS_THIGH_MAX_NS * core_rate, I3C_PERIOD_NS) - 1;
	if (hcnt < SCL_I3C_TIMING_CNT_MIN) {
		hcnt = SCL_I3C_TIMING_CNT_MIN;
	} else if (hcnt > SCL_I3C_TIMING_CNT_MAX) {
		hcnt = SCL_I3C_TIMING_CNT_MAX;
	...
	/* I2C FM+ */
	lcnt = DIV_ROUND_UP(I3C_BUS_I2C_FMP_TLOW_MIN_NS * core_rate, I3C_PERIOD_NS);
	...
	/* I2C FM */
	lcnt = DIV_ROUND_UP(I3C_BUS_I2C_FM_TLOW_MIN_NS * core_rate, I3C_PERIOD_NS);
...
static void enable_intr(const struct device *dev)
{
	const struct dw_i3c_config *config = dev->config;
	uint32_t thld_ctrl;
	...
	sys_write32(INTR_MASTER_MASK, config->regs + INTR_STATUS_EN);
	sys_write32(INTR_MASTER_MASK, config->regs + INTR_SIGNAL_EN);
}
...
static int dw_i3c_do_ccc(const struct device *dev, struct i3c_ccc_payload *payload)
{
	...
	do {
		...
		if (!i3c_ccc_is_payload_broadcast(payload)) {
			...
			if (pos < 0) {
				LOG_DBG("%s: Invalid Slave address", dev->name);
				return pos;
			}
		} else {
			pos = 0;
		...
		/* last command queue with multiple targets must have TOC and ROC set*/
		if (i3c_ccc_is_payload_broadcast(payload) || (i == payload->targets.num_targets)) {
			cmd->cmd_lo |= COMMAND_PORT_TOC | COMMAND_PORT_ROC;
		}

		if (payload->targets.payloads->rnw) {
		...
		i++;
	} while (i < payload->targets.num_targets);

	start_xfer(dev);

	ret = k_sem_take(&data->sem_xfer, K_MSEC(1000));
	if (ret) {
		LOG_DBG("%s: Semaphore err (%d)", dev->name, ret);
		k_mutex_unlock(&data->mt);
		return ret;
	}
...
static int add_slave_from_daa(const struct device *dev, int32_t pos)
{
	...
	dyn_addr = DEV_ADDR_TABLE_DYNAMIC_ADDR(tmp);
...
static int dw_i3c_target_tx_write(const struct device *dev, uint8_t *buf, uint16_t len)
{
	...
	cmd->cmd_hi = COMMAND_PORT_ARG_DATA_LEN(len) | COMMAND_PORT_TRANSFER_ARG;
	cmd->cmd_lo = COMMAND_PORT_TID(0) | COMMAND_PORT_ROC;
	cmd->buf = buf;
	cmd->tx_len = len;
