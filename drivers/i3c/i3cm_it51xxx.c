...
static void it51xxx_i3cm_isr(const struct device *dev)
{
	...
	if (int_status & TRANSFER_END) {
		...
		case IT51XXX_I3CM_MSG_ERROR:
			LOG_DEV_ERR(dev, "isr: message status error");
			data->error_is_detected = true;
			break;
		case IT51XXX_I3CM_MSG_DAA:
			LOG_DEV_DBG(dev, "isr: daa finished");
			break;
		case IT51XXX_I3CM_MSG_IDLE:
			LOG_DEV_WRN(dev, "isr: end transfer occurs but bus is in idle");
			break;
		default:
			LOG_DEV_ERR(dev, "isr: unknown message status(%d)", data->msg_state);
			break;
		}
