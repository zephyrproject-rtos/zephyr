...
static inline int it51xxx_set_i2c_clock(const struct device *dev)
{
	...
	if (t_high > 0xFFFF || t_low > 0xFFFF) {
		LOG_DEV_ERR(dev, "invalid t_high(0x%x) or t_low(0x%x) setting", t_high, t_low);
	}
	...
	LOG_DEV_DBG(dev, "i2c: t_high 0x%x, t_low 0x%x t_hddat 0x%x", t_high, t_low, t_hddat);
	LOG_DEV_DBG(dev, "i2c: high period: %dns, low period: %dns", t_high_period_ns,
		    t_low_period_ns);
	LOG_DEV_INF(dev, "i2c: freq: %dHz -> %dHz", config_cntlr->scl.i2c,
...
static inline int it51xxx_set_i3c_clock(const struct device *dev)
{
	...
	LOG_DEV_DBG(dev, "i3c: pphigh: %d, pplow %d, odhigh: %d, odlow %d, hddat %d", pphigh, pplow,
		    odhigh, odlow, hddat);
	LOG_DEV_INF(dev, "i3c: pp_freq: %dHz -> %dHz, od_freq %dHz -> %dHz", pp_freq,
					 ...
					 uint8_t num_msgs, uint16_t addr)
{
	...
	if ((msgs[data->curr_msg.curr_idx].flags & I2C_MSG_RW_MASK) == I2C_MSG_READ) {
		size_t data_count;

		data_count = it51xxx_get_received_data_count(dev);
		memcpy(msgs[data->curr_msg.curr_idx].buf, data->dlm_data.rx_data,
		       MIN(msgs[data->curr_msg.curr_idx].len, data_count));
		LOG_DEV_DBG(dev, "i2c: private rx %d bytes", data_count);
		LOG_HEXDUMP_DBG(msgs[data->curr_msg.curr_idx].buf,
				msgs[data->curr_msg.curr_idx].len, "i2c: private xfer rx:");
	}

	if (data->curr_msg.curr_idx != data->curr_msg.num_msgs - 1) {
		if ((msgs[data->curr_msg.curr_idx].flags & I2C_MSG_RW_MASK) == I2C_MSG_WRITE &&
		    msgs[data->curr_msg.curr_idx + 1].flags & I2C_MSG_READ) {
			size_t data_count;

			data_count = it51xxx_get_received_data_count(dev);
			memcpy(msgs[data->curr_msg.curr_idx + 1].buf, data->dlm_data.rx_data,
			       MIN(msgs[data->curr_msg.curr_idx + 1].len, data_count));
			LOG_DEV_DBG(dev, "i2c: private tx-then-rx %d bytes", data_count);
			LOG_HEXDUMP_DBG(msgs[data->curr_msg.curr_idx + 1].buf,
					msgs[data->curr_msg.curr_idx + 1].len,
				 ...
				 struct i3c_msg *msgs, uint8_t num_msgs)
{
	...
	if ((msgs[data->curr_msg.curr_idx].flags & I3C_MSG_RW_MASK) == I3C_MSG_WRITE) {
		data->curr_msg.i3c_msgs[data->curr_msg.curr_idx].num_xfer =
			data->curr_msg.i3c_msgs[data->curr_msg.curr_idx].len;
	}

	if ((msgs[data->curr_msg.curr_idx].flags & I3C_MSG_RW_MASK) == I3C_MSG_READ) {
		size_t data_count;

		data_count = it51xxx_get_received_data_count(dev);
		msgs[data->curr_msg.curr_idx].num_xfer = data_count;
		memcpy(msgs[data->curr_msg.curr_idx].buf, data->dlm_data.rx_data,
		       MIN(msgs[data->curr_msg.curr_idx].len, data_count));
		LOG_DEV_DBG(dev, "i3c: private rx %d bytes", data_count);
		LOG_HEXDUMP_DBG(msgs[data->curr_msg.curr_idx].buf,
				msgs[data->curr_msg.curr_idx].len, "i3c: private xfer rx:");
	}

	if (data->curr_msg.curr_idx != data->curr_msg.num_msgs - 1) {
		if ((msgs[data->curr_msg.curr_idx].flags & I3C_MSG_RW_MASK) == I3C_MSG_WRITE &&
		    msgs[data->curr_msg.curr_idx + 1].flags & I3C_MSG_READ) {
			size_t data_count;

			data_count = it51xxx_get_received_data_count(dev);
			msgs[data->curr_msg.curr_idx + 1].num_xfer = data_count;
			memcpy(msgs[data->curr_msg.curr_idx + 1].buf, data->dlm_data.rx_data,
			       MIN(msgs[data->curr_msg.curr_idx + 1].len, data_count));
			LOG_DEV_DBG(dev, "i3c: private tx-then-rx %d bytes", data_count);
			LOG_HEXDUMP_DBG(msgs[data->curr_msg.curr_idx + 1].buf,
					msgs[data->curr_msg.curr_idx + 1].len,
...
static int it51xxx_i3cm_ibi_enable(const struct device *dev, struct i3c_device_desc *target)
{
	...
	if (ret != 0) {
		LOG_DEV_ERR(dev, "failed to send ibi enec for 0x%x(%d)", target->dynamic_addr, ret);
		return ret;
	}
...
static int it51xxx_i3cm_ibi_disable(const struct device *dev, struct i3c_device_desc *target)
{
	...
	if (ret != 0) {
		LOG_DEV_ERR(dev, "failed to send ibi disec for 0x%x(%d)", target->dynamic_addr,
			    ret);
	}
	return ret;
}
...
static void it51xxx_process_ibi_payload(const struct device *dev)
{
	...
	if (i3c_ibi_has_payload(target)) {
		...
		if (payload_sz > sizeof(payload)) {
			LOG_DEV_WRN(dev, "ibi payloads(%d) is too much", payload_sz);
		}
		memcpy(payload, data->dlm_data.rx_data, MIN(payload_sz, sizeof(payload)));
	}
