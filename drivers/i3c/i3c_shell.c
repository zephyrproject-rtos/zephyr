...
/* i3c speed <device> <speed> <od high_ns> <od low_ns> */
static int cmd_i3c_speed(const struct shell *sh, size_t argc, char **argv)
{
	...
	if (argc == 5) {
		...
		config.scl_od.high_ns = strtol(argv[ARGV_DEV + 2], NULL, 10);
		config.scl_od.low_ns = strtol(argv[ARGV_DEV + 3], NULL, 10);
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(speed, &dsub_i3c_device_name,
		      ...
		      "Usage: speed <device> <speed> <od high_ns> <od low_ns>",
		      cmd_i3c_speed, 3, 2),
