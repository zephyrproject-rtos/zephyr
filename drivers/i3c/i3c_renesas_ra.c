						...
						i3c_slave_info_t const *daa_rx)
{
	...
	uint64_t pid = ((uint64_t)daa_rx->pid[0] << 40U) | ((uint64_t)daa_rx->pid[1] << 32U) |
		       ((uint64_t)daa_rx->pid[2] << 24U) | ((uint64_t)daa_rx->pid[3] << 16U) |
		       ((uint64_t)daa_rx->pid[4] << 8U) | ((uint64_t)daa_rx->pid[5] << 0U);
...
static int i3c_renesas_ra_attach_i2c_device(const struct device *dev,
					    struct i3c_i2c_device_desc *target)
{
	...
	/* Create scheme for saving device in DAT */
	device_table.static_address = (uint8_t)target->addr;
	device_table.device_protocol = I3C_DEVICE_PROTOCOL_I2C;
	device_table.dynamic_address = 0x00;
	device_table.ibi_accept = false;
	device_table.ibi_payload = false;
	device_table.master_request_accept = false;

	target_index = i3c_renesas_ra_device_index_request(dev, (uint8_t)target->addr, true);
	if (target_index < 0) {
		ret = -ERANGE;
		goto attach_i2c_exit;
	}
...
attach_i2c_exit:
	...
	if (ret == 0) {
		LOG_DBG("Attach I2C Legaxy SA[0x%02X] to DAT%d ", target->addr, target_index);
	}

	return ret;
}

static int i3c_renesas_ra_detach_i2c_device(const struct device *dev,
					    struct i3c_i2c_device_desc *target)
{
	struct i3c_renesas_ra_data *data = dev->data;
	fsp_err_t fsp_err = FSP_SUCCESS;
	...
	target_index = i3c_renesas_ra_device_index_find(dev, (uint8_t)target->addr, true);
	if (target_index < 0) {
		ret = -ERANGE;
		goto detach_i2c_exit;
	}
...
detach_i2c_exit:
	...
	if (ret == 0) {
		LOG_DBG("Detach I2C Legaxy SA[0x%02X] from Device Table", target->addr);
	}
...
static void i3c_renesas_ra_iodev_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	return i3c_iodev_submit_fallback(dev, iodev_sqe);
}
...
static DEVICE_API(i3c, i3c_renesas_ra_api) = {
	...
	.attach_i2c_device = i3c_renesas_ra_attach_i2c_device,
	.detach_i2c_device = i3c_renesas_ra_detach_i2c_device,
...
#ifdef CONFIG_I3C_RTIO
	.iodev_submit = i3c_renesas_ra_iodev_submit,
#endif /* CONFIG_I3C_RTIO */
};
