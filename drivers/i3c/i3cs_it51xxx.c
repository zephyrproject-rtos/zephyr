...
LOG_MODULE_REGISTER(i3cs_it51xxx, CONFIG_I3C_IT51XXX_LOG_LEVEL);
...
#define IT51XXX_I3C_IBI_TIMEOUT K_MSEC(1000)
enum it51xxx_i3cs_event_type {
	i3cs_event_normal_mode = 0,
	i3cs_event_ibi,
	i3cs_event_control_request,
	i3cs_event_hot_join,
};
...
struct it51xxx_i3cs_data {
	...
	struct k_mutex lock;
	/* align the 2048 bytes to ensure that the MSB (bits [17:11]) of the TX and RX FIFO
...
static inline bool rx_direct_mode_is_enabled(const struct device *dev)
{
	...
	return (sizeof(data->fifo.rx_data) == 4096);
}
static inline bool tx_direct_mode_is_enabled(const struct device *dev)
{
	struct it51xxx_i3cs_data *data = dev->data;
	return (sizeof(data->fifo.tx_data) == 4096);
}
...
{
	const struct it51xxx_i3cs_config *cfg = dev->config;
	/* The maximum value that can be set for MRL is 0xFFF. Setting this value does not affect
	 * private read operation when the data size exceeds this limit.
	 */
	sys_write8((value <= 0xFFF) ? BYTE_0(value) : 0, cfg->base + I3CS68_MRL_SET_BY_CTRL_LB);
	sys_write8((value <= 0xFFF) ? BYTE_1(value) & 0xF : 0,
		   cfg->base + I3CS69_MRL_SET_BY_CTRL_HB);
}
static inline void set_mwl_value(const struct device *dev, const uint16_t value)
{
	const struct it51xxx_i3cs_config *cfg = dev->config;
	/* The maximum value that can be set for MWL is 0xFFF. Setting this value does not affect
	 * private write operation when the data size exceeds this limit.
	 */
	sys_write8((value <= 0xFFF) ? BYTE_0(value) : 0, cfg->base + I3CS6A_MWL_SET_BY_CTRL_LB);
	sys_write8((value <= 0xFFF) ? BYTE_1(value) & 0xF : 0,
		   cfg->base + I3CS6B_MWL_SET_BY_CTRL_HB);
}
static int it51xxx_i3cs_prepare_tx_fifo(const struct device *dev, uint8_t *buf, uint16_t len)
{
	...
	if (tx_count) {
		LOG_DEV_WRN(dev, "dropped the remaining %d bytes in the tx fifo", tx_count);
	}
	/* flush tx fifo */
...
static int it51xxx_i3cs_target_register(const struct device *dev, struct i3c_target_config *cfg)
{
	struct it51xxx_i3cs_data *data = dev->data;
	if (!data->target_config) {
		...
		LOG_DEV_WRN(dev, "the target has already been registered");
	}
	return 0;
}
static int it51xxx_i3cs_target_unregister(const struct device *dev, struct i3c_target_config *cfg)
{
	struct it51xxx_i3cs_data *data = dev->data;
	if (data->target_config) {
		...
		LOG_DEV_WRN(dev, "the target has not been registered");
	}
	return 0;
}
					...
					uint8_t hdr_mode)
{
	...
	int ret = 0;
	if (!buf || len == 0) {
		LOG_DEV_ERR(dev, "null buffer or zero length");
		return -EINVAL;
	}
	if (hdr_mode != 0) {
	...
	if (len > sizeof(data->fifo.tx_data)) {
		LOG_DEV_ERR(dev, "invalid tx length(%d)", len);
		return -ENOSPC;
	}
...
{
	struct it51xxx_i3cs_data *data = dev->data;
	if (k_sem_take(&data->ibi_sync_sem, IT51XXX_I3C_IBI_TIMEOUT) != 0) {
		LOG_DEV_ERR(dev, "ibi event transmission timed out");
		return -ETIMEDOUT;
	}
	return 0;
}
static int it51xxx_i3cs_target_ibi_raise(const struct device *dev, struct i3c_ibi *request)
{
	...
	case I3C_IBI_TARGET_INTR:
		if (it51xxx_i3cs_is_ibi_disable(dev) || !it51xxx_i3cs_dynamic_addr_valid(dev)) {
			LOG_DEV_ERR(dev, "ibi is disabled or dynamic address is invalid");
			ret = -EINVAL;
			goto out;
		}
		if (request->payload_len > sizeof(data->fifo.tx_data) + 1) {
			LOG_DEV_ERR(dev, "payload too large for ibi tir");
			ret = -ENOMEM;
			goto out;
		}
		if (config_target->bcr & I3C_BCR_IBI_PAYLOAD_HAS_DATA_BYTE &&
		    request->payload_len == 0) {
			LOG_DEV_ERR(dev, "ibi should be with payload");
			ret = -EINVAL;
			goto out;
		}
		if (!(config_target->bcr & I3C_BCR_IBI_PAYLOAD_HAS_DATA_BYTE) &&
		    request->payload_len != 0) {
			LOG_DEV_ERR(dev, "ibi should not be with payload");
			ret = -EINVAL;
			goto out;
		}
		if (request->payload_len == 0) {
			LOG_DEV_DBG(dev, "send ibi without payload");
			sys_write8(I3CS_EVENT_SELECT(i3cs_event_ibi), cfg->base + I3CS0C_CONTROL_0);
		} else {
			...
			if (request->payload_len > 1) {
				if (FIELD_GET(IBI_MDB_GROUP_MASK, request->payload[0]) ==
					 ...
					 * exceeds 1.
					 */
					LOG_DEV_ERR(dev, "unsupported multiple payloads with "
				...
				sys_write8(EXTENDED_IBI_DATA | I3CS_EVENT_SELECT(i3cs_event_ibi),
					   cfg->base + I3CS0C_CONTROL_0);
			} else {
				sys_write8(I3CS_EVENT_SELECT(i3cs_event_ibi),
					   cfg->base + I3CS0C_CONTROL_0);
			}
		}
		break;
	case I3C_IBI_HOTJOIN:
		if (it51xxx_i3cs_is_hj_disable(dev) || it51xxx_i3cs_dynamic_addr_valid(dev)) {
			LOG_DEV_ERR(dev, "hj is disabled or dynamic address is already assigned");
			ret = -EINVAL;
			goto out;
		}
		sys_write8(I3CS_EVENT_SELECT(i3cs_event_hot_join), cfg->base + I3CS0C_CONTROL_0);
		break;
	case I3C_IBI_CONTROLLER_ROLE_REQUEST:
		...
		goto out;
	default:
		LOG_DEV_ERR(dev, "invalid ibi type(0x%x)", request->ibi_type);
		ret = -EINVAL;
		goto out;
	}
	if (it51xxx_i3cs_wait_to_complete(dev) != 0) {
		LOG_DEV_WRN(dev, "failed to issue ibi. maybe the controller is offline");
		sys_write8(I3CS_EVENT_SELECT(i3cs_event_normal_mode), cfg->base + I3CS0C_CONTROL_0);
	}
out:
	k_mutex_unlock(&data->lock);
	return ret;
...
static int it51xxx_i3cs_set_fifo_address(const struct device *dev)
{
	...
	if (sizeof(data->fifo.rx_data) <= 128 && sizeof(data->fifo.tx_data) <= 128) {
		if (FIFO_ADDR_HB(((uint32_t)&data->fifo.tx_data)) !=
			...
			LOG_DEV_ERR(dev, "the msb of tx and rx fifo address should be the same");
			return -EINVAL;
		}
	...
	if (!rx_direct_mode_is_enabled(dev) || !tx_direct_mode_is_enabled(dev)) {
		...
		LOG_DEV_ERR(dev, "tx or rx fifo size is invalid for direct mode");
		return -EINVAL;
	}
	LOG_DEV_DBG(dev, "direct mode is enabled");
...
static int it51xxx_i3cs_init(const struct device *dev)
{
	...
	LOG_DEV_DBG(dev, "select io channel %d", cfg->io_channel);
	/* set extern enable bit */
	if (cfg->extern_enable.bit_mask > 7) {
		LOG_DEV_ERR(dev, "invalid bit mask %d for extern enable setting",
	...
	if (config_target->pid_random) {
		...
		LOG_DEV_INF(dev, "set pid random value: %#llx", config_target->pid);
	}
	if (I3C_BCR_DEVICE_ROLE(config_target->bcr) == I3C_BCR_DEVICE_ROLE_I3C_CONTROLLER_CAPABLE) {
		LOG_DEV_ERR(dev, "i3cs doesn't support controller capability");
		return -ENOTSUP;
	}
	sys_write8(config_target->bcr, cfg->base + I3CS72_BCR);
	sys_write8(config_target->dcr, cfg->base + I3CS71_DCR);
	LOG_DEV_INF(dev, "tx fifo size(%d), address(0x%x)", sizeof(data->fifo.tx_data),
		    (uint32_t)&data->fifo.tx_data);
	LOG_DEV_INF(dev, "rx fifo size(%d), address(0x%x)", sizeof(data->fifo.rx_data),
	...
	for (uint8_t i = 0; i <= ARRAY_SIZE(fifo_size_table); i++) {
		if (i == ARRAY_SIZE(fifo_size_table)) {
			LOG_DEV_ERR(dev, "unknown rx fifo size %d", sizeof(data->fifo.rx_data));
			return -ENOTSUP;
		}
	...
	for (uint8_t i = 0; i <= ARRAY_SIZE(fifo_size_table); i++) {
		if (i == ARRAY_SIZE(fifo_size_table)) {
			LOG_DEV_ERR(dev, "unknown tx fifo size %d", sizeof(data->fifo.tx_data));
			return -ENOTSUP;
		}
...
static void it51xxx_i3cs_check_errwarn(const struct device *dev)
{
	...
	if (errwarn0_val & INVALID_START) {
		LOG_DEV_ERR(dev, "isr: invalid start");
	}
	if (errwarn0_val & CONTROLLER_TERMINATED) {
		LOG_DEV_WRN(dev, "isr: terminated by controller, flush the remaining %d bytes",
			    tx_byte_cnt_in_fifo(dev));
		sys_write8(sys_read8(cfg->base + I3CS2C_DATA_CTRL_0) | FLUSH_TX_FIFO,
			   cfg->base + I3CS2C_DATA_CTRL_0);
	}
	if (errwarn0_val & TX_FIFO_UNDERRUN) {
		LOG_DEV_ERR(dev, "isr: the tx fifo is underrun");
	}
	if (errwarn0_val & RX_FIFO_OVERRUN) {
		LOG_DEV_ERR(dev, "isr: the rx fifo is overrun");
	}
	if (errwarn1_val & S0_OR_S1_ERROR) {
		LOG_DEV_ERR(dev, "isr: s0 or s1 error is detected");
	}
	if (errwarn1_val & SDR_PARITY_ERROR) {
		LOG_DEV_ERR(dev, "isr: sdr parity error");
	}
	LOG_DEV_DBG(dev, "isr: error/warning is detected(0x%x, 0x%x)", errwarn0_val, errwarn1_val);
...
static void it51xxx_i3cs_process_rx_fifo(const struct device *dev, const bool ccc)
{
	...
	if (rx_direct_mode_is_enabled(dev)) {
		...
		if (dfifo_status & I3CS_DIRECT_RX_DONE) {
			...
			LOG_DEV_WRN(dev, "isr: rx pending, but rx not completed");
			return;
		}
...
static void it51xxx_i3cs_isr(const struct device *dev)
{
	...
	LOG_DEV_DBG(dev, "isr: interrupt status 0x%x 0x%x", int_status_1, int_status_2);
	if (int_status_1 & INT_DYN_ADDR_CHANGE) {
		if (it51xxx_i3cs_dynamic_addr_valid(dev)) {
			...
			LOG_DEV_DBG(dev, "dynamic address is assigned");
		} else {
			if (data->target_config) {
				data->target_config->address = 0;
			}
			LOG_DEV_DBG(dev, "dynamic address is reset");
		}
	}
	...
	if (int_status_2 & INT_TARGET_RST) {
		LOG_DEV_INF(dev, "isr: target reset pattern is detected");
	}
