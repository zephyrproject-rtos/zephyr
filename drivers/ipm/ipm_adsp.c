...
BUILD_ASSERT(IPM_ADSP_MAILBOX_OUT_SIZE >= IPM_ADSP_MAX_DATA_SIZE);

/* Mailbox Host -> ADSP */
#define IPM_ADSP_MAILBOX_IN		MAILBOX_HOSTBOX_BASE
#define IPM_ADSP_MAILBOX_IN_SIZE	MAILBOX_HOSTBOX_SIZE

BUILD_ASSERT(IPM_ADSP_MAILBOX_IN_SIZE >= IPM_ADSP_MAX_DATA_SIZE);
...
{
	const struct ipm_adsp_config *config = dev->config_info;
