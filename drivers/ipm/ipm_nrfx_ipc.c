...
static void nrfx_ipc_handler(uint8_t event_idx, void *p_context)
{
	if (nrfx_ipm_data.callback) {
		__ASSERT(event_idx < NRFX_IPC_ID_MAX_VALUE, "Illegal event_idx: %d", event_idx);
		nrfx_ipm_data.callback(DEVICE_DT_INST_GET(0), nrfx_ipm_data.user_data, event_idx,
				       NULL);
	}
}

static int ipm_nrf_send(const struct device *dev, int wait, uint32_t id, const void *data, int size)
{
...
static void ipm_nrf_register_callback(const struct device *dev, ipm_callback_t cb, void *user_data)
{
...
DEVICE_DT_INST_DEFINE(0, ipm_nrf_init, NULL, NULL, NULL, PRE_KERNEL_1,
		      CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &ipm_nrf_driver_api);
...
static void vipm_dispatcher(uint8_t event_idx, void *p_context)
{
	...
	if (nrfx_vipm_data.callback[event_idx] != NULL) {
		nrfx_vipm_data.callback[event_idx](nrfx_vipm_data.ipm_device[event_idx],
