...
static void mcux_mailbox_isr(void *arg)
{
	...
	struct mcux_mailbox_data *data = dev->fixed->driver_data;
	const struct mcux_mailbox_config *config = dev->fixed->config_info;
...
{
	const struct mcux_mailbox_config *config = d->fixed->config_info;
...
static void mcux_mailbox_ipm_register_callback(struct device *d,
					       ...
					       void *context)
{
	struct mcux_mailbox_data *driver_data = d->fixed->driver_data;
