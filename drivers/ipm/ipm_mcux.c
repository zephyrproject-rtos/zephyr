...
static int mcux_mailbox_ipm_send(const struct device *d, int wait, uint32_t id, const void *data,
				 int size)
{
...
static void mcux_mailbox_ipm_register_callback(const struct device *d, ipm_callback_t cb,
					       void *context)
{
...
DEVICE_DT_INST_DEFINE(0, &mcux_mailbox_init, NULL, &mcux_mailbox_0_data, &mcux_mailbox_0_config,
		      PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &mcux_mailbox_driver_api);
