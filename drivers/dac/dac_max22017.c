...
static int max22017_reg_read(const struct device *dev, uint8_t addr,
			uint32_t *value)
{
	int ret;
	...
	*value = 0;
	...
	uint8_t crc_in[] = { addr, ((uint8_t *)value)[1], ((uint8_t *)value)[2] };

	ret = crc8(crc_in, 3, MAX22017_CRC_POLY, 0, true);

	if (ret != ((uint8_t *)value)[3]) {
		LOG_ERR("Reg read: CRC Mismatch calculated / read: %x / %x",
				ret, ((uint8_t *)value)[3]);
		return -EINVAL;
	}
...
static int max22017_reg_write(const struct device *dev, uint8_t addr,
			uint16_t value)
{
	uint8_t crc;
	size_t crc_len = 0;
...
static int max22017_write_value(const struct device *dev, uint8_t channel,
				uint32_t value)
{
	int ret;
	uint32_t ao_sta;
	...
	if (config->latch_mode[channel]) {
		if (config->gpio_ldac.port) {
			...
			ret = max22017_reg_write(dev, MAX22017_AO_CMD_OFF,
...
static void max22017_isr(const struct device *dev, struct gpio_callback *gpio_cb,
					 uint32_t pins)
{
	struct max22017_data *data = CONTAINER_OF(gpio_cb, struct max22017_data,
							  callback_int);
	int ret;
...
{
	struct max22017_data *data = CONTAINER_OF(work, struct max22017_data,
							  int_work);
	...
	if (ret)
		LOG_ERR("Conversion failure on channels: %s %s",
				(ret & BIT(0)) ? "0" : "", (ret & BIT(1)) ? "1" : "");

	ret = FIELD_GET(MAX22017_GEN_INT_OPENWIRE_DTCT_INT, gen_int);
	if (ret)
		LOG_ERR("Openwire detected on channels: %s %s",
				(ret & BIT(0)) ? "0" : "", (ret & BIT(1)) ? "1" : "");
	...
	if (ret)
		LOG_ERR("Thermal shutdown AO channels: %s %s",
				(ret & BIT(0)) ? "0" : "", (ret & BIT(1)) ? "1" : "");

	ret = FIELD_GET(MAX22017_GEN_INT_THWRNG_INT, gen_int);
	if (ret)
		LOG_ERR("Thermal warning AO channels: %s %s",
				(ret & BIT(0)) ? "0" : "", (ret & BIT(1)) ? "1" : "");

	ret = FIELD_GET(MAX22017_GEN_INT_OVC_INT, gen_int);
	if (ret)
		LOG_ERR("Over current on channels: %s %s",
				(ret & BIT(0)) ? "0" : "", (ret & BIT(1)) ? "1" : "");
	...
	if (ret) {
		LOG_ERR("GPI Interrupt: %d", ret);
#ifdef CONFIG_GPIO_MAX22017
		uint32_t gpi_sta, lsb;
...
int adi_max22017_gpio_pin_interrupt_configure(const struct device *dev,
					     gpio_pin_t pin,
					     enum gpio_int_mode mode,
					     enum gpio_int_trig trig)
{
	int ret;
	uint32_t gpio_int, gen_int_en;
	struct max22017_data *data = dev->data;
	...
	ret = max22017_reg_write(dev, MAX22017_GEN_INTEN_OFF, gen_int_en |
		FIELD_PREP(MAX22017_GEN_INTEN_GPI_INTEN, 1));
fail:
	k_sem_give(&data->lock);
	return ret;
}
...
int adi_max22017_gpio_set_pin_value(const struct device *dev, uint8_t pin,
		bool value)
{
	int ret;
	uint32_t gpio_data;
...
int adi_max22017_gpio_get_pin_value(const struct device *dev, uint8_t pin,
		bool *value)
{
	int ret;
	uint32_t gpio_data;
...
int adi_max22017_gpio_port_get_raw(const struct device *dev,
		gpio_port_value_t *value)
{
	...
	ret = max22017_reg_read(dev, MAX22017_GEN_GPIO_DATA_OFF, value);
	if (ret)
		goto fail;
...
int adi_max22017_gpio_port_set_masked_raw(const struct device *dev,
		gpio_port_pins_t mask,
		gpio_port_value_t value)
{
	int ret;
	uint32_t gpio_data, tmp_val;
	struct max22017_data *data = dev->data;
...
int adi_max22017_gpio_port_toggle_bits(const struct device *dev,
...
int adi_max22017_manage_cb(const struct device *dev,
...
static int max22017_init(const struct device *dev)
{
	...
	if (config->gpio_int.port) {
		...
		gpio_init_callback(&data->callback_int, max22017_isr,
	...
	LOG_INF("MAX22017 version: 0x%lx 0x%lx",
			FIELD_GET(MAX22017_GEN_ID_PROD_ID, version),
...
#define INST_DT_MAX22017(index)                                                  \
	static const struct max22017_config max22017_config_##index = {              \
		.spi = SPI_DT_SPEC_INST_GET(index, SPI_OP_MODE_MASTER |                  \
						SPI_WORD_SET(8U), 0U),                   \
	...
	static struct max22017_data max22017_data_##index;                           \
                                                                                 \
	DEVICE_DT_INST_DEFINE(index, max22017_init, NULL,                            \
			    &max22017_data_##index,                                          \
				&max22017_config_##index,                                        \
				POST_KERNEL,                                                     \
				CONFIG_DAC_MAX22017_INIT_PRIORITY,                               \
				&max22017_driver_api);
