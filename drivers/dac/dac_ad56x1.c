...
#define DAC_AD56X1_INST_DEFINE(index, name, res)                                                   \
	static struct ad56x1_data data_##name##_##index;                                           \
	static const struct ad56x1_config config_##name##_##index = {                              \
		.bus = SPI_DT_SPEC_INST_GET(                                                       \
			index, SPI_OP_MODE_MASTER | SPI_MODE_CPHA | SPI_WORD_SET(8), 0),           \
		.resolution = res};                                                                \
	DEVICE_DT_INST_DEFINE(index, ad56x1_init, NULL, &data_##name##_##index,                    \
			      &config_##name##_##index, POST_KERNEL,                               \
			      CONFIG_DAC_AD56X1_INIT_PRIORITY, &ad56x1_driver_api);
