...
/* Registers at address <= 0x13 have 8-bit data, above have 16-bit data */
#define AD5529R_REG_8BIT_UPPER_BOUND 0x013
...
enum ad5529r_output_range {
	...
	AD5529R_RANGE_PM_5V = 0x4,
	AD5529R_RANGE_PM_10V = 0x5,
	AD5529R_RANGE_PM_15V = 0x6,
	AD5529R_RANGE_PM_20V = 0x7,
};

/* Timing constants (from Linux driver) */
#define AD5529R_RESET_PULSE_US 1000
#define AD5529R_RESET_WAIT_US  10000
...
static int ad5529r_write_reg(const struct device *dev, uint16_t addr, uint16_t value)
{
	...
	if (addr <= AD5529R_REG_8BIT_UPPER_BOUND) {
		...
		buffer_tx[2] = (uint8_t)value;
		len = 3;
	} else {
		/* 16-bit: start at addr+1 for address decrement mode */
		instruction = AD5529R_INST_WRITE | AD5529R_DEVICE_ID(config->device_id) |
			      ((addr + 1) & AD5529R_ADDR_MASK);
		sys_put_be16(instruction, buffer_tx);
...
static int ad5529r_write_reg_verify(const struct device *dev, uint16_t addr, uint16_t value)
{
	uint16_t readback;
	int ret;

	ret = ad5529r_write_reg(dev, addr, value);
	if (ret != 0) {
		return ret;
	}

	ret = ad5529r_read_reg(dev, addr, &readback);
	if (ret != 0) {
		return ret;
	}

	if (readback != value) {
		LOG_ERR("reg 0x%03X readback mismatch: wrote 0x%04X, read 0x%04X", addr, value,
			readback);
		return -EIO;
	}

	LOG_DBG("reg 0x%03X verified: 0x%04X", addr, readback);
...
static int ad5529r_software_reset(const struct device *dev)
{
	...
	/* Restore SDO_ENABLE and clear reset bits */
	ret = ad5529r_write_reg(dev, AD5529R_REG_INTERFACE_CONFIG_A, AD5529R_SDO_ACTIVE);
	if (ret != 0) {
		LOG_ERR("failed to restore interface config: %d", ret);
		return ret;
	}
				 ...
				 const struct dac_channel_cfg *channel_cfg)
{
	...
	ret = ad5529r_write_reg_verify(dev, AD5529R_REG_OUT_RANGE_CH(channel_cfg->channel_id),
				       config->output_range);
	if (ret != 0) {
	...
	/* Enable the channel output (default is all disabled) */
	data->channel_setup_mask |= BIT(channel_cfg->channel_id);

	ret = ad5529r_write_reg_verify(dev, AD5529R_REG_OUT_OPERATING_MODE,
				       data->channel_setup_mask);
	if (ret != 0) {
		LOG_ERR("failed to enable channel %u output: %d", channel_cfg->channel_id, ret);
		return ret;
	}
...
static int ad5529r_write_value(const struct device *dev, uint8_t channel, uint32_t value)
{
	...
	ret = ad5529r_write_reg_verify(dev, AD5529R_REG_DAC_INPUT_A_CH(channel), (uint16_t)value);
	if (ret != 0) {
...
static int ad5529r_init(const struct device *dev)
{
	...
	/* Scratch pad write/readback test to verify SPI write path */
	uint16_t scratch;

	ret = ad5529r_write_reg(dev, AD5529R_REG_SCRATCH_PAD, 0xA5);
	if (ret != 0) {
		LOG_ERR("failed to write scratch pad: %d", ret);
		return ret;
	}

	ret = ad5529r_read_reg(dev, AD5529R_REG_SCRATCH_PAD, &scratch);
	if (ret != 0) {
		LOG_ERR("failed to read scratch pad: %d", ret);
		return ret;
	}

	if (scratch != 0xA5) {
		LOG_ERR("scratch pad mismatch: wrote 0xA5, read 0x%02X", scratch);
		return -EIO;
	}
