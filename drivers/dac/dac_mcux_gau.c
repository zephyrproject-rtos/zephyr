...
static inline dac_channel_id_t convert_channel_id(uint8_t channel_id)
{
	switch (channel_id) {
	case 0:
		return kDAC_ChannelA;
	case 1:
		return kDAC_ChannelB;
	default:
				     ...
				     const struct dac_channel_cfg *channel_cfg)
{
	...
	DAC_SetChannelConfig(config->base, (uint32_t)convert_channel_id(channel_cfg->channel_id),
			     &dac_channel_config);

	return 0;
};

static int nxp_gau_dac_write_value(const struct device *dev, uint8_t channel, uint32_t value)
{
	const struct nxp_gau_dac_config *config = dev->config;

	DAC_SetChannelData(config->base, (uint32_t)convert_channel_id(channel), (uint16_t)value);
	return 0;
};
...
#define NXP_GAU_DAC_INIT(inst)                                                                   \
	...
	const struct nxp_gau_dac_config nxp_gau_dac_##inst##_config = {                          \
		...
		.output_range = DT_INST_ENUM_IDX(inst, nxp_output_voltage_range),                \
	};                                                                                       \
                                                                                                 \
	DEVICE_DT_INST_DEFINE(inst, &nxp_gau_dac_init, NULL, NULL, &nxp_gau_dac_##inst##_config, \
			      POST_KERNEL, CONFIG_DAC_INIT_PRIORITY, &nxp_gau_dac_driver_api);
