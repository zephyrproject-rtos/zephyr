...
static void dac_wait_ready(dac_registers_t *dac_reg, uint8_t channel_id)
{
	...
	if ((dac_reg->DAC_CTRLA & DAC_CTRLA_ENABLE_Msk) == DAC_CTRLA_ENABLE_Msk) {
		...
		if (channel_id == DAC_CHANNELS_ALL) {
			...
			if (WAIT_FOR(((dac_reg->DAC_STATUS & DAC_STATUS_READY0_Msk) ==
				      DAC_STATUS_READY0_Msk),
				     TIMEOUT_VALUE_US, k_busy_wait(DELAY_US)) == false) {

				LOG_ERR("Timeout waiting for DAC_STATUS_READY0");
			}

			/* Wait for synchronization channel 1 */
			if (WAIT_FOR(((dac_reg->DAC_STATUS & DAC_STATUS_READY1_Msk) ==
				      DAC_STATUS_READY1_Msk),
				     TIMEOUT_VALUE_US, k_busy_wait(DELAY_US)) == false) {

				LOG_ERR("Timeout waiting for DAC_STATUS_READY1");
			}
		} else {
			...
			/* Wait for synchronization of selected channel */
			if (WAIT_FOR(((dac_reg->DAC_STATUS & mask) == mask), TIMEOUT_VALUE_US,
				     k_busy_wait(DELAY_US)) == false) {

				LOG_ERR("Timeout waiting for DAC_STATUS_READYx (mask=0x%x)", mask);
			}
		}
	}
}
