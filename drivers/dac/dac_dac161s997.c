...
enum dac161s997_reg_status {
	DAC161S997_REG_STATUS_CURR_LOOP_STS,
	DAC161S997_REG_STATUS_LOOP_STS,
	DAC161S997_REG_STATUS_SPI_TIMEOUT_ERR,
	DAC161S997_REG_STATUS_FERR_STS,
	DAC161S997_REG_STATUS_ERRLVL_PIN,
	DAC161S997_REG_STATUS_DAC_RES,
};
...
static const struct {
	uint8_t status_bit;
	const char *str;
} dac161s997_status_strings[] = {
	{BIT(DAC161S997_REG_STATUS_CURR_LOOP_STS), "CURR_LOOP_STS"},
	{BIT(DAC161S997_REG_STATUS_LOOP_STS), "LOOP_STS"},
	{BIT(DAC161S997_REG_STATUS_SPI_TIMEOUT_ERR), "SPI_TIMEOUT_ERR"},
	{BIT(DAC161S997_REG_STATUS_FERR_STS), "FERR_STS"},
};
...
static void dac161s997_gpio_errb_work_handler(struct k_work *work)
{
	...
	ret = dac161s997_read_reg(dev, DAC161S997_REG_STATUS, &status);
	if (ret != 0) {
		goto unlock;
	}

	LOG_WRN("STATUS errors:");
	for (int i = 0; i < ARRAY_SIZE(dac161s997_status_strings); ++i) {
		if ((status & dac161s997_status_strings[i].status_bit) != 0) {
			LOG_WRN(" %s", dac161s997_status_strings[i].str);
		}
	}
				    ...
				    gpio_port_pins_t pins)
{
	...
	LOG_WRN("ERRB interrupt");

	k_work_submit(&data->gpio_errb_work);
}
...
static int dac161s997_init(const struct device *dev)
{
	...
	if (!((status >> DAC161S997_REG_STATUS_DAC_RES & BIT_MASK(3)) == BIT_MASK(3))) {
		LOG_ERR("Unexpected DAC resolution value: 0x%02x", status);
		return ret;
	}
...
#define DAC_DAC161S997_INIT(n)                                                                     \
	...
	DEVICE_DT_INST_DEFINE(n, dac161s997_init, NULL, &dac161s997_data_##n,                      \
			      &dac161s997_config_##n, POST_KERNEL,                                 \
			      CONFIG_DAC_DAC161S997_INIT_PRIORITY, &dac161s997_driver_api);
