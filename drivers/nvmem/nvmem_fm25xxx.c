...
LOG_MODULE_REGISTER(nvmem_fm25xxx, CONFIG_NVMEM_LOG_LEVEL);
...
static uint8_t nvmem_fm25xxx_size_to_addr_bytes(size_t size)
{
    if (size <= 500) {
        return 1;
    } else if (size <= 64000) {
...
static int nvmem_fm25xxx_set_enable_write(const struct device *dev, bool enable_writes)
{
...
static int nvmem_fm25xxx_read(const struct device *dev, off_t offset, void *data, size_t len)
{
    int ret;
    const struct fm25xxx_config *config = dev->config;
    ...
    size_t addr_bytes = nvmem_fm25xxx_size_to_addr_bytes(config->size);
...
static int nvmem_fm25xxx_write(const struct device *dev, off_t offset, const void *buf, size_t len)
{
    int ret;
    const struct fm25xxx_config *config = dev->config;
    ...
    ret = nvmem_fm25xxx_set_enable_write(dev, true);
    if (ret != 0) {
    ...
    ret = nvmem_fm25xxx_set_enable_write(dev, false);
    if (ret != 0) {
        k_sem_give(&dev_data->lock);
...
static size_t nvmem_fm25xxx_get_size(const struct device *dev)
{
    const struct fm25xxx_config *config = dev->config;
...
static const struct nvmem_info *nvmem_fm25xxx_get_info(const struct device *dev)
{
...
static int nvmem_fm25xxx_init(const struct device *dev)
{
    const struct fm25xxx_config *config = dev->config;
    struct fm25xxx_data *data = dev->data;
...
static DEVICE_API(nvmem, nvmem_fm25xxx_api) = {
    .read = &nvmem_fm25xxx_read,
    .write = &nvmem_fm25xxx_write,
    .get_size = &nvmem_fm25xxx_get_size,
    .get_info = &nvmem_fm25xxx_get_info,
};

#define NVMEM_FM25XXX_INIT(inst)								\
	static struct fm25xxx_data fm25xxx_data_##inst = {					\
		.live_info = {									\
			.type = NVMEM_TYPE_FRAM,						\
	...
	static const struct fm25xxx_config fm25xxx_config_##inst = {				\
		...
		.size = DT_INST_PROP(inst, size),						\
												...
												\
	DEVICE_DT_INST_DEFINE(inst, nvmem_fm25xxx_init, NULL, &fm25xxx_data_##inst,		\
		&fm25xxx_config_##inst, POST_KERNEL, CONFIG_NVMEM_MODEL_INIT_PRIORITY,		\
		&nvmem_fm25xxx_api);

DT_INST_FOREACH_STATUS_OKAY(NVMEM_FM25XXX_INIT)
