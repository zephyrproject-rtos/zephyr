...
#include <zephyr/drivers/modem/ublox-sara-n310.h>
...
#if DT_INST_NODE_HAS_PROP(0, mdm_last_gasp_gpios)
static const struct gpio_dt_spec last_gasp_gpio = GPIO_DT_SPEC_INST_GET(0, mdm_last_gasp_gpios);
#endif
...
{
	mdata.network_state = ATOI(argv[0], 0, "stat");
...
{
  mdata.signal_quality.rssi = ATOI(argv[0], 0, "rssi");
  mdata.signal_quality.quality = ATOI(argv[1], 0, "quality");

  LOG_DBG("CSQ:%d, %d", mdata.signal_quality.rssi, mdata.signal_quality.quality);
  return 0;
}

MODEM_CMD_DEFINE(on_cmd_uantr)
{
  int antenna_id;
  int antenna_load;

  antenna_id = ATOI(argv[0], 0, "id");
  antenna_load = ATOI(argv[1], 0, "load");

  mdata.antenna_load = antenna_load;
  LOG_DBG("UANTR:%d, %d", antenna_id, antenna_load);
  return 0;
}

/* CMD: +UJAD */
MODEM_CMD_DEFINE(on_cmd_ujad)
{
	mdata.jamming_state = ATOI(argv[0], 0, "active");

	LOG_DBG("UJAD:%d", mdata.jamming_state);
	return 0;
}

/* CMD: +UULGASP */
MODEM_CMD_DEFINE(on_cmd_last_gasp)
{
    int result = ATOI(argv[0], 0, "result");
	int bearer = ATOI(argv[1], 0, "bearer");

	LOG_DBG("UULGASP:%d, %d", result, bearer);
	return 0;
}
...
static const struct modem_cmd response_cmds[] = {
	...
	MODEM_CMD("+UANTR: ", on_cmd_uantr, 2U, ","),
};

/* unsolicited command handling */
static const struct modem_cmd unsol_cmds[] = {
	MODEM_CMD("+UJAD: ", on_cmd_ujad, 2U, ","),
	MODEM_CMD("+UULGASP: ", on_cmd_last_gasp, 2U, ","),
				...
				k_timeout_t timeout)
{
	...
	modem_context_sprint_ip_addr(dst_addr, ip_buf, 256);

	snprintk(send_buf, sizeof(send_buf),
		 "AT+NSOSTF=%d,\"%s\",%u,%d,%zu,\"%s\"", sock->id,
			 ip_buf, dst_port, release_flag,
...
#define SARA_N310_SOCKET_PRIORITY 40

BUILD_ASSERT(SARA_N310_SOCKET_PRIORITY < CONFIG_NET_SOCKETS_TLS_PRIORITY,
	     "SARA_N310_SOCKET_PRIORITY must be < than NET_SOCKETS_TLS_PRIORITY");

NET_SOCKET_REGISTER(ublox_sara_n310, 40, AF_UNSPEC,
...
static int turn_on_module(void)
{
	...
	gpio_pin_set_dt(&power_gpio, MDM_POWER_DISABLE);
...
{
  int ret = 0;
  /* Max of 256 ASCII characters for text parameter */
  char buf[sizeof("AT+ULGASP=2,last gasp,0,,17,xxx.xxx.xxx.xxx:12345,,1,##,#")] = { 0 };

  if(strlen(text) > 256) {
		LOG_ERR("Parameter text is longer than maximum value of 256");
		return -EINVAL;
  }

  snprintk(buf, sizeof(buf), "AT+ULGASP=1,%s,0,,17,%s,,1,%d,%d",
		   text,
		   ip_addr,
		   tx_cnt, shutdown);
...
{
#if DT_INST_NODE_HAS_PROP(0, mdm_last_gasp_gpios)
  /* Last gasp is triggered on falling edge */
  gpio_pin_set_dt(&last_gasp_gpio, MDM_POWER_ENABLE);
  k_sleep(K_MSEC(10));
  gpio_pin_set_dt(&last_gasp_gpio, MDM_POWER_DISABLE);

  return 0;
#else
  LOG_WRN("mdm-last-gasp-gpios not set in devicetree");
...
int n310_modem_reset(void)
{
	...
	static const struct setup_cmd setup_cmds[] = {
		...
		/* enable smart jamming detection URC for debugging */
		SETUP_CMD_NOHANDLE("AT+UJAD=1"),
		/* enable PDP context */
		SETUP_CMD_NOHANDLE("AT+CIPCA=1"),
		/* set default PDP type */
		SETUP_CMD_NOHANDLE("AT+CFGDFTPDN=3"),				
		/* set GPIO3 pin for last gasp */
		SETUP_CMD_NOHANDLE("AT+UGPIOC=24,19"),
		...
		/* Disregard any optional URC parameters */
...
static int n310_driver_init(const struct device *device)
{
	...
	/* GPIO setup */
	ret = gpio_pin_configure_dt(&power_gpio, GPIO_OUTPUT | GPIO_OPEN_DRAIN);
	if (ret < 0) {
...
#if DT_INST_NODE_HAS_PROP(0, mdm_last_gasp_gpios)
	ret = gpio_pin_configure_dt(&last_gasp_gpio, GPIO_OUTPUT);
	if (ret < 0) {
		LOG_ERR("Failed to configure %s pin", "vint");
		return ret;
	}
	...
	mdata.context.driver_data = &mdata;
