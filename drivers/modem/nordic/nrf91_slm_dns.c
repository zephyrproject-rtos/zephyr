...
static void nrf91_slm_xgetaddrinfo_callback(struct modem_chat *chat, char **argv, uint16_t argc,
					    void *user_data)
{
...
static int nrf91_slm_xgetaddrinfo(struct nrf91_slm_data *data, const char *hostname, int family)
{
	...
	static const char match[] = "#XGETADDRINFO: ";
	int ret;
	...
	ret = modem_chat_script_chat_set_request(&data->sock_script_chat, request);
	if (ret < 0) {
		return ret;
	}

	ret = modem_chat_match_set_match(&data->sock_match, match);
	if (ret < 0) {
		return ret;
	}
	...
	modem_chat_match_set_callback(&data->sock_match, nrf91_slm_xgetaddrinfo_callback);

	/* Run script */
	modem_chat_script_set_timeout(&data->sock_script, 120000);
			  ...
			  const struct zsock_addrinfo *hints, struct zsock_addrinfo **res)
{
	...
	if (port > 0U) {
		if (data->dns_result.ai_family == AF_INET) {
