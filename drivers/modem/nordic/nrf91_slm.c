				     ...
				     const struct modem_chat_script *script)
{
	...
	if (ret < 0) {
		...
		LOG_DBG("rescheduled %s (%d)", script->name, ret);
	}
}
...
static void nrf91_slm_chat_on_iccid(struct modem_chat *chat, char **argv, uint16_t argc,
				    void *user_data)
{
	struct nrf91_slm_data *data = (struct nrf91_slm_data *)user_data;
...
static void nrf91_slm_chat_on_imsi(struct modem_chat *chat, char **argv, uint16_t argc,
				   void *user_data)
{
	struct nrf91_slm_data *data = (struct nrf91_slm_data *)user_data;
...
MODEM_CHAT_MATCH_DEFINE(qccid_match __maybe_unused, "+QCCID: ", "", nrf91_slm_chat_on_iccid);
MODEM_CHAT_MATCH_DEFINE(iccid_match __maybe_unused, "+ICCID: ", "", nrf91_slm_chat_on_iccid);
MODEM_CHAT_MATCH_DEFINE(cimi_match __maybe_unused, "", "", nrf91_slm_chat_on_imsi);
MODEM_CHAT_MATCH_DEFINE(cgmi_match __maybe_unused, "", "", nrf91_slm_chat_on_cgmi);
MODEM_CHAT_MATCH_DEFINE(cgmr_match __maybe_unused, "", "", nrf91_slm_chat_on_cgmr);

MODEM_CHAT_MATCHES_DEFINE(unsol_matches, MODEM_CHAT_MATCH("+CREG: ", ",", nrf91_slm_chat_on_cxreg),
...
MODEM_CHAT_SCRIPT_DEFINE(nordic_nrf91_slm_dial_chat_script, nordic_nrf91_slm_dial_chat_script_cmds,
			 dial_abort_matches, nrf91_slm_chat_callback_handler, 10);
...
static void nrf91_slm_init_socket_script(const struct device *dev)
{
	struct nrf91_slm_data *data = dev->data;

	modem_chat_match_init(&data->sock_match);
	modem_chat_match_set_separators(&data->sock_match, ",*");

	modem_chat_script_chat_init(&data->sock_script_chat);
	modem_chat_script_chat_set_response_matches(&data->sock_script_chat, &data->sock_match, 1);

	modem_chat_script_init(&data->sock_script);
	modem_chat_script_set_name(&data->sock_script, "socket");
	modem_chat_script_set_script_chats(&data->sock_script, &data->sock_script_chat, 1);
	modem_chat_script_set_abort_matches(&data->sock_script, abort_matches,
					    sizeof(abort_matches));
...
{
	struct nrf91_slm_data *data = (struct nrf91_slm_data *)dev->data;
	struct nrf91_slm_config *config = (struct nrf91_slm_config *)dev->config;
	int ret;
	...
	nrf91_slm_init_socket_script(dev);
