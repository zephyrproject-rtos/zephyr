...
struct modem_cellular_event_sub {
...
struct modem_cellular_data {
	...
	struct modem_cellular_event_sub subs[CONFIG_MODEM_CELLULAR_MAX_EVENT_SUBSCRIBERS];
};
...
{
	for (size_t i = 0; i < ARRAY_SIZE(data->subs); i++) {
		if ((data->subs[i].evt == evt) && data->subs[i].cb) {
			data->subs[i].cb(data->dev, evt, payload, data->subs[i].user_data);
		}
	}
}
...
static int append_apn_cmd(struct modem_cellular_data *data, uint8_t *steps, const char *fmt, ...)
{
	...
	va_start(ap, fmt);
	n = vsnprintk(data->apn_buf[*steps], MODEM_CELLULAR_APN_BUF_SIZE, fmt, ap);
	va_end(ap);
...
static int modem_cellular_register_event_cb(const struct device *dev,
					    enum cellular_event evt,
					    cellular_event_cb_t cb,
					    void *user_data)
{
	...
	/* right now the driver only supports this one event */
	...
	if (cb == NULL) {
		for (size_t i = 0; i < ARRAY_SIZE(data->subs); i++) {
			if ((data->subs[i].evt == evt) && data->subs[i].cb != NULL) {
				data->subs[i].cb = NULL;
				ret = 0;
				break;
			}
		...
		for (size_t i = 0; i < ARRAY_SIZE(data->subs); i++) {
			if ((data->subs[i].cb == NULL) || (data->subs[i].cb == cb)) {
				data->subs[i].evt = evt;
				data->subs[i].cb = cb;
				data->subs[i].user_data = user_data;
				ret = 0;
				break;
			}
		}
...
static DEVICE_API(cellular, modem_cellular_api) = {
	...
	.register_event_cb = modem_cellular_register_event_cb,
};
