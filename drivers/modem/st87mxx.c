...
struct getfix_params_t {
	st87mxx_get_pos_callback_t *get_pos_callback_func;
	uint32_t timeout;
};

struct getfix_vars_t {
	struct getfix_params_t params;
...
struct wscan_vars_t {
	st87mxx_wifiscan_params_t params;
	wscan_state_t state;
	uint16_t iter_cnt;
};

struct getrssi_params_t {
	st87mxx_get_rssi_callback_t *get_rssi_callback_func;
};

struct st87mxx_app_srv_data_t {
	struct k_sem sequence_sem;
	sequence_state_t sequence_state;
	struct getfix_vars_t getfix_vars;
	struct wscan_vars_t wscan_vars;
	struct getrssi_params_t rssi_data;
};

struct k_timer st87mxx_sequence_timer;
static struct st87mxx_app_srv_data_t app_srv_data;
...
static int st87mxx_create_socket(struct modem_socket *sock, const struct net_sockaddr *addr)
{
	...
	if (addr->sa_family == NET_AF_INET6) {
		LOG_INF("addr->sa_family: NET_AF_INET6");
		ip_mode = 1;
	} else if (addr->sa_family == NET_AF_INET) {
		LOG_INF("addr->sa_family: NET_AF_INET");
		ip_mode = 0;
	} else {
	}
				...
				const struct net_sockaddr *dest_addr, net_socklen_t addrlen)
{
	...
	/* Get the destination port. */
	if (dest_addr->sa_family == NET_AF_INET6) {
		dst_port = ntohs(net_sin6(dest_addr)->sin6_port);
	} else if (dest_addr->sa_family == NET_AF_INET) {
...
st87mxx_get_pos_callback_t *get_pos_callback_func, uint32_t timeout)
{
	...
	if (timeout == 0) {
		LOG_ERR("st87mxx_gnss_getfix: bad parameter");
		ret = EINVAL;
		goto end;
	}
	else if (app_srv_data.sequence_state == SEQUENCE_ONGOING) {
		LOG_ERR("st87mxx_gnss_getfix: sequence already on-going");
		ret = ECANCELED;
		goto end;
	}
	...
	LOG_DBG("st87mxx_gnss_getfix: sending init");
...
int st87mxx_wifiscan(st87mxx_wifiscan_params_t *wscan_params)
{
	...
	struct modem_cmd data_cmd[] = { MODEM_CMD_ARGS_MAX("#WSCAN:", on_cmd_wscan, 3U, 4U, ","),
	...
	{
		LOG_ERR("st87mxx_wifiscan_start: sequence already on-going");
		ret = ECANCELED;
		goto end;
	}
	...
	/* Initializations */
	app_srv_data.sequence_state = SEQUENCE_ONGOING;
	memcpy(&app_srv_data.wscan_vars.params, wscan_params, sizeof(st87mxx_wifiscan_params_t));
	app_srv_data.wscan_vars.iter_cnt = 0;

	/* Initiate timeout timer */
	k_timer_init(&st87mxx_sequence_timer, sequence_timer_timeout_func, NULL);
	k_timer_start(&st87mxx_sequence_timer, K_SECONDS(app_srv_data.wscan_vars.params.timeout), K_NO_WAIT);
	...
	snprintk(buf_wscan, sizeof(buf_wscan), "AT#WSCAN=5,%d", app_srv_data.wscan_vars.params.nb_scan_iterations);
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL,
			0, buf_wscan, NULL, K_NO_WAIT);
	...
	snprintk(buf_wscan, sizeof(buf_wscan), "AT#WSCAN=1,%s", app_srv_data.wscan_vars.params.channel_list);
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler,
...
int st87mxx_getrssi(st87mxx_get_rssi_callback_t *get_rssi_callback_func)
{
	...
	if (app_srv_data.sequence_state == SEQUENCE_ONGOING) {
		LOG_ERR("st87mxx_getrssi: sequence already on-going");
		ret = ECANCELED;
		goto end;
	}
	...
	/* Initiate timeout timer */
	k_timer_init(&st87mxx_sequence_timer, sequence_timer_timeout_func, NULL);
	k_timer_start(&st87mxx_sequence_timer, K_SECONDS(ST87MXX_AT_DEFAULT_TIMEOUT_S), K_NO_WAIT);
...
int st87mxx_app_services_getstate(st87mxx_state_t * state)
{
	...
	{
		state->sequence_state = app_srv_data.sequence_state;
	}
