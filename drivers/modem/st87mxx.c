...
static const struct setup_cmd init_cmds[] = {
	...
	SETUP_CMD("AT#TEMPLIMIT=-40, 85, "STRINGIFY(TEMP_LOW_SHUTDOWN)", \
		"STRINGIFY(TEMP_HIGH_SHUTDONW)", \
		0,"STRINGIFY(TEMP_SHUTDOWN),"", NULL, 0U, ""),
	SETUP_CMD("AT#VBATLIMIT=2200, 3000, "STRINGIFY(VBAT_LOW_SHUTDOWN)", \
		"STRINGIFY(VBAT_HIGH_SHUTDOWN)", "STRINGIFY(VBAT_SHUTDOWN)", 0, 0","", NULL, 0U, ""),
	...
	SETUP_CMD("AT#SLEEPMODE=1,"STRINGIFY(HOLD_TIME)", "STRINGIFY(AWAKE_TIME), "", NULL, 0U, ""),
	...
	SETUP_CMD("AT#NVMWR="STRINGIFY(ST87MXX_COLD_VERSION_NVM_PAGE)", \
		...
		"STRINGIFY(ST87MXX_COLD_CONFIG_VERSION), \
		"", NULL, 0U, ""),
...
static int st87mxx_gpio_init(void)
{
	...
	{
		if (gpio_pin_interrupt_configure_dt(mdata.ring_gpio, GPIO_INT_EDGE_TO_ACTIVE) < 0) 
		{
			ret = -1;
		} 
		else 
		{
			gpio_init_callback(&ring_gpio_callback_data, ring_pin_cb, BIT(mdata.ring_gpio->pin));
...
static int st87mxx_tcp_connect(struct modem_socket *sock, const struct sockaddr *addr)
{
	...
	uint16_t dst_port = 0;
	char ip_str[NET_IPV6_ADDR_LEN];
	char send_buf[sizeof("AT#TCPCONNECT=#,#,#xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxx.xxx.xxx.xxx#,####")] = { 0 };
	...
	if (sock->type == SOCK_STREAM)
	{
		...
		ret = snprintk(send_buf, sizeof(send_buf), "AT#TCPCONNECT=%d,%d,%s,%d", mdata.context_id,
						sock->id, ip_str, dst_port);
				...
				const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	int ret;
	struct modem_socket *sock = (struct modem_socket *)obj;
	char send_buf_udp[sizeof("AT#IPSENDUDP=##,##,#xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxx.xxx.xxx.xxx#,##,##,##,####")] = { 0 };
	...
	ret = snprintk(send_buf_udp, sizeof(send_buf_udp), "AT#IPSENDUDP=%d,%d,%s,%d,%d,%d,%zu", mdata.context_id, sock->id, ip_str, dst_port,0, 1, len);
	if (ret < 0)
	{
		LOG_ERR("Failed to build send command!!");
		errno = ENOMEM;
