...
static int ubx_m8_init(const struct device *dev)
{
	...
	err = ubx_iface_init(dev, u_blox_m8_unsol_messages,
	...
	err = ubx_iface_msg_send(dev, &stop_gnss,
	...
	err = ubx_iface_msg_send(dev, &start_gnss,
...
static int ubx_m8_set_fix_rate(const struct device *dev, uint32_t fix_interval_ms)
{
	...
	return ubx_iface_msg_payload_send(dev, UBX_CLASS_ID_CFG, UBX_MSG_ID_CFG_RATE,
...
static int ubx_m8_set_navigation_mode(const struct device *dev, enum gnss_navigation_mode mode)
{
	...
	return ubx_iface_msg_payload_send(dev, UBX_CLASS_ID_CFG, UBX_MSG_ID_CFG_NAV5,
...
#define UBX_M8(inst)										   \
	...
	BUILD_ASSERT((DT_INST_PROP(inst, fix_rate) >= 50) &&					   \
		     ...
		     " and must fit in 16-bits.");						   \
												   \
	static const struct ubx_iface_config ubx_m8_cfg_##inst = {				   \
		...
		.fix_rate_ms = DT_INST_PROP(inst, fix_rate),					   \
												   ...
												   \
	static struct ubx_iface_data ubx_m8_data_##inst;					   \
												   \
