...
static void quectel_lcx6g_init_pair_script(const struct device *dev)
{
	...
	modem_chat_script_chat_set_matches(&data->pair_script_chat, &data->pair_match, 1);

	modem_chat_script_init(&data->pair_script);
	modem_chat_script_set_name(&data->pair_script, "pair");
	modem_chat_script_set_chats(&data->pair_script, &data->pair_script_chat, 1);
	modem_chat_script_set_matches(&data->pair_script, &data->pair_match, 1);
