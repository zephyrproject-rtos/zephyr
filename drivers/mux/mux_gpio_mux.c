...
static int gpiomux_configure(const struct device *dev, struct mux_control *mux, uint32_t state)
{
	...
	if (!data->allow_use) {
		LOG_ERR("Cannot configure %s - GPIOs not initialized", dev->name);
		return -ENODEV;
	}
