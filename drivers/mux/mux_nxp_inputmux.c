				  ...
				  struct mux_control *mux, uint32_t state)
{
	...
	uint32_t offset = mux->cells[0];
	uint32_t mask   = mux->cells[1];
	volatile uint32_t *reg = (uint32_t *)((uint32_t)config->base + offset);
	uint32_t temp = *reg;
	int err;
...
static int nxp_inputmux_state_get(const struct device *dev,
				  struct mux_control *mux, uint32_t *state)
{
	if (mux->len < 2) {
		return -EINVAL;
...
static DEVICE_API(mux_control, nxp_inputmux_driver_api) = {
	.state_get = nxp_inputmux_state_get,
	.configure = nxp_inputmux_configure,
};

#define NXP_INPUTMUX_INIT(inst)								\
	static const struct nxp_inputmux_config nxp_inputmux_cfg_##inst = {		\
