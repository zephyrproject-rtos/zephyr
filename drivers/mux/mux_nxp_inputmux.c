...
static int nxp_inputmux_configure(const struct device *dev, mux_control_t mux)
{
	if (!mux->has_state_cell) {
		/* following cast would be buffer overrun without a state cell */
		return -EINVAL;
	}
	const struct inputmux_control *control = (const struct inputmux_control *)mux->control;
