			    ...
			    const struct mux_control *control, uint32_t state)
{
	...
	if ((state & ~NXP_XBAR_SEL_MASK) != 0U) {
		LOG_ERR("xbar input %u exceeds SEL field width", state);
		return -EINVAL;
	}
...
static int mux_nxp_xbar_get_state(const struct device *dev,
				  const struct mux_control *control, uint32_t *state)
{
	uint32_t base = DEVICE_MMIO_GET(dev);
	uint32_t output;
	...
	/* Same fixed-length addressing as mux_nxp_xbar_set(). */
	output = control->cells[0];
	...
	*state = (sel_reg_val >> shift) & NXP_XBAR_SEL_MASK;
...
static DEVICE_API(mux_control, mux_nxp_xbar_driver_api) = {
	...
	.get_state = mux_nxp_xbar_get_state,
};
