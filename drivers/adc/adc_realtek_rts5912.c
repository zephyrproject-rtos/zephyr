...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	const struct adc_rts5912_config *const devcfg = adc_dev->config;
	ADC_Type *regs = devcfg->regs;

	data->repeat_buffer = data->buffer;

	regs->CTRL |= ADC_CTRL_SGLDNINTEN_Msk;
	regs->CTRL |= ADC_CTRL_START_Msk;
}
				     ...
				     const struct adc_channel_cfg *channel_cfg)
{
	...
	regs->CHCTRL |= ((0x01ul << channel_id) | (ADC_CHCTRL_LPFBP_Msk << channel_id));
...
static int adc_rts5912_enable(const struct device *dev)
{
	...
	regs->CTRL |= ADC_CTRL_EN_Msk;
	while ((k_uptime_get() - st) < RTS5912_ADC_ENABLE_TIMEOUT) {
		if (regs->STS & ADC_STS_RDY_Msk) {
			return 0;
		}
...
static DEVICE_API(adc, adc_rts5912_api) = {
...
#define DEV_CONFIG_CLK_DEV_INIT(n)                                                                 \
	...
	.sccon_cfg = {                                                                             \
		...
		.clk_idx = DT_INST_CLOCKS_CELL_BY_NAME(n, adc, clk_idx),                           \
	}
...
static struct adc_rts5912_data adc_rts5912_dev_data_0 = {
	ADC_CONTEXT_INIT_TIMER(adc_rts5912_dev_data_0, ctx),
	ADC_CONTEXT_INIT_LOCK(adc_rts5912_dev_data_0, ctx),
...
DEVICE_DT_INST_DEFINE(0, adc_rts5912_init, NULL, &adc_rts5912_dev_data_0, &adc_rts5912_dev_cfg_0,
		      PRE_KERNEL_1, CONFIG_ADC_INIT_PRIORITY, &adc_rts5912_api);
