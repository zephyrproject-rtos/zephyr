...
#if ADS1X4S0X_HAS_24_BIT_DEV && ADS1X4S0X_HAS_16_BIT_DEV
	const struct ads1x4s0x_config *config = dev->config;
...
#define ADC_ADS1X4S0X_INST_DEFINE(n, name, ch, res)                                                \
	...
	static const struct ads1x4s0x_config config_##name##_##n = {                               \
		...
		IF_ENABLED(CONFIG_ADC_ASYNC, (.stack = thread_stack_##n,)) .gpio_reset =           \
			   ...
			   .vbias_level = DT_INST_PROP(n, vbias_level), .resolution = res,         \
			   .channels = ch,                                                         \
	};                                                                                         \
