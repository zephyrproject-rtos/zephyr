...
static int mcp342x_channel_setup(const struct device *dev, const struct adc_channel_cfg *cfg)
{
	...
	if (cfg->differential && cfg->input_positive != cfg->input_negative) {
		LOG_ERR("Invalid channel config");
		return -EINVAL;
	}
	if (cfg->input_positive == 0) {
		reg_config->channelSelection = MUX_CH1;
	} else if (cfg->input_positive == 1) {
		reg_config->channelSelection = MUX_CH2;
	} else if (cfg->input_positive == 2) {
		reg_config->channelSelection = MUX_CH3;
	} else if (cfg->input_positive == 3) {
		reg_config->channelSelection = MUX_CH4;
	} else {
		LOG_ERR("Invalid channel config");
		return -EINVAL;
	}
...
static int mcp342x_getConversionTime_us(uint8_t resolution)
{
	switch (resolution) {
	case 12:
...
static void mcp342x_perform_read(const struct device *dev)
{
	...
	while (data->channels) {
		 ...
		 */
		k_usleep(mcp342x_getConversionTime_us(data->resolution));
		do {
			k_yield();
