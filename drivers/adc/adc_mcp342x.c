...
static int mcp342x_read_register(const struct device *dev, mcp342x_reg_data_t *data,
				 mcp342x_reg_config_t *config_data)
{
	int ret;
...
static int mcp342x_channel_setup(const struct device *dev, const struct adc_channel_cfg *cfg)
{
	...
	reg_config->channel_selection = cfg->channel_id;
...
static int mcp342x_start_read(const struct device *dev, const struct adc_sequence *seq)
{
	...
	if ((data->configured_channels & seq->channels) != seq->channels) {
		LOG_ERR("not all channels configured before being sampled");
		return -EINVAL;
	}
...
static int mcp342x_read_async(const struct device *dev, const struct adc_sequence *seq,
...
{
	return mcp342x_read_async(dev, seq, NULL);
}
...
static void mcp342x_perform_read(const struct device *dev)
{
	...
	while (data->sample_channels) {
		 ...
		 */
		ch = find_lsb_set(data->sample_channels) - 1;
		reg = data->reg_config[ch];

		switch (data->resolution) {
		case 12:
			reg.sample_rate = SAMPLERATE_240SPS_12BITS;
			break;
		case 14:
			reg.sample_rate = SAMPLERATE_60SPS_14BITS;
			break;
		case 16:
			reg.sample_rate = SAMPLERATE_15SPS_16BITS;
			break;
		...
		*data->buffer++ = res;
		...
		WRITE_BIT(data->sample_channels, ch, 0);
	}
