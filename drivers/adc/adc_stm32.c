...
{
	const struct adc_stm32_cfg *config = dev->fixed->config_info;
...
{
	const struct adc_stm32_cfg *config = dev->fixed->config_info;
	struct adc_stm32_data *data = dev->fixed->driver_data;
	ADC_TypeDef *adc = (ADC_TypeDef *)config->base;
	uint8_t resolution;
	int err;
...
static void adc_stm32_isr(void *arg)
{
	...
	struct adc_stm32_data *data = (struct adc_stm32_data *)dev->fixed->driver_data;
	const struct adc_stm32_cfg *config =
		(const struct adc_stm32_cfg *)dev->fixed->config_info;
