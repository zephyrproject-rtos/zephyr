...
#if ANY_ADC_HAS_CHANNEL_PRESELECTION && !defined(ADC_STM32_INJECTED_CHANNELS)
...
{
	return adc_stm32_read(dev, sequence, NULL);
}
...
static int adc_stm32_read_async(const struct device *dev,
				 ...
				 struct k_poll_signal *async)
{
	return adc_stm32_read(dev, sequence, async);
}
#endif
...
#if ANY_ADC_HAS_CHANNEL_PRESELECTION && CONFIG_ADC_STM32_INJECTED_CHANNELS
	err = adc_stm32_preselection_setup(dev, channel_cfg->channel_id);
	if (err < 0) {
		LOG_ERR("Errot setting preselection register");
		return err;
	}
