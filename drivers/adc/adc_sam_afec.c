				 ...
				 const struct adc_channel_cfg *channel_cfg)
{
	 ...
	 */
	afec->AFEC_EMR &= ~(AFEC_EMR_SIGNMODE(AFEC_EMR_SIGNMODE_SE_UNSG_DF_SIGN_Val));
#endif
...
static int adc_sam_init(const struct device *dev)
{
	...
	afec->AFEC_MR = AFEC_MR_TRGEN_DIS | AFEC_MR_SLEEP_NORMAL | AFEC_MR_FWUP_OFF |
			AFEC_MR_FREERUN_OFF | AFEC_MR_PRESCAL(CONF_ADC_PRESCALER) |
			AFEC_MR_STARTUP_SUT96 | AFEC_MR_ONE | AFEC_MR_USEQ_NUM_ORDER;
...
#ifdef AFEC_11147
			 | AFEC_ACR_PGA0EN | AFEC_ACR_PGA1EN
#endif
		;
...
#define ADC_SAM_INIT(n)                                                                     \
	...
	static struct adc_sam_data adc##n##_sam_data = {                                    \
		...
		ADC_CONTEXT_INIT_SYNC(adc##n##_sam_data, ctx),                              \
                                                                                            ...
                                                                                            \
	DEVICE_DT_INST_DEFINE(n, adc_sam_init, NULL, &adc##n##_sam_data, &adc##n##_sam_cfg, \
