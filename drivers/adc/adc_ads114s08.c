...
#define ADS114S0X_CLK_FREQ_IN_MHZ			    4096
...
/*
 * According to the datasheet this should be 4096 * 1/fclk = 1us,
 * but it actually takes more time for the ADC to report ready in
 * its status register.
 */
#define ADS114S0X_RESET_DELAY_TIME_IN_US 1000

#define ADS114S0X_RESET_LOW_TIME_IN_US                                                             \
	(ADS114S0X_RESET_LOW_TIME_IN_CLOCK_CYCLES / ADS114S0X_CLK_FREQ_IN_MHZ)
#define ADS114S0X_START_SYNC_PULSE_DURATION_IN_US                                                  \
	(ADS114S0X_START_SYNC_PULSE_DURATION_IN_CLOCK_CYCLES / ADS114S0X_CLK_FREQ_IN_MHZ)
#define ADS114S0X_SETUP_TIME_IN_US                                                                 \
	(ADS114S0X_SETUP_TIME_IN_CLOCK_CYCLES / ADS114S0X_CLK_FREQ_IN_MHZ)
				   ...
				   const struct adc_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->differential) {
		...
		if (channel_cfg->channel_id >= ADS114S0X_INPUT_SELECTION_AINCOM) {
...
static int ads114s0x_init(const struct device *dev)
{
	...
	k_thread_name_set(tid, "adc_ads114");
#endif
...
#define ADC_ADS114S0X_INST_DEFINE(n)                                                               \
	...
	static const struct ads114s0x_config config_##n = {                                        \
		...
		.gpio_reset = GPIO_DT_SPEC_GET_BY_IDX_OR(DT_DRV_INST(n), reset_gpios, 0, {0}),     \
		.gpio_data_ready = GPIO_DT_SPEC_GET_BY_IDX(DT_DRV_INST(n), drdy_gpios, 0),         \
		.gpio_start_sync = GPIO_DT_SPEC_GET_BY_IDX_OR(                                     \
			DT_DRV_INST(n), start_sync_gpios, 0, {0}),                                 \
	};                                                                                         \
