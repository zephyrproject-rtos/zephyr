...
int avg_filter_values[] = {2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096};
...
#endif
	uint8_t access[18] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFE, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	case 2:
		...
		data_be = ((uint32_t)reg_val_x[0] << 8) | (uint32_t)reg_val_x[1];
		break;
	case 3:
		data_be = ((uint32_t)reg_val_x[0] << 16) | ((uint32_t)reg_val_x[1] << 8)
				| (uint32_t)reg_val_x[2];
		break;
	default:
		data_be = ((uint32_t)reg_val_x[0] << 8) | (uint32_t)reg_val_x[1];
		break;
...
int ad405x_set_sample_rate(const struct device *dev,
			   enum ad405x_sample_rate rate)
{
	struct adc_ad405x_data *data = dev->data;
	int ret;
...
int ad405x_set_averaging_filter_length(const struct device *dev,
...
#define AD405X_CONFIG(inst, chipid)                        \
		...
		.active_mode = AD405X_AVERAGING_MODE_OP,                            \
...
#define ADC_AD405X_DEFINE(inst, chipid)								\
	...
	DEVICE_DT_INST_DEFINE(inst, adc_ad405x_init, NULL,				\
				&ad405x_data_##inst##chipid, &ad405x_config_##inst##chipid,\
				POST_KERNEL, CONFIG_ADC_INIT_PRIORITY, &ad405x_api_funcs);\
