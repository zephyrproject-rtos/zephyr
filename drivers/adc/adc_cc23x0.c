...
struct adc_cc23x0_data {
	...
	uint8_t mem_index[ADC_CC23X0_CH_COUNT];
	uint8_t ch_count;
	uint8_t mem_count;
	uint16_t *buffer;
...
static void adc_cc23x0_isr(const struct device *dev)
{
	...
	for (i = 0 ; i < ADC_CC23X0_MEM_COUNT ; i++) {
		if (int_flags & ADC_CC23X0_INT_MEMRES(i)) {
			ch = data->ch_sel[i];
			...
			data->channels &= ~BIT(ch);

			ADCClearInterrupt(ADC_CC23X0_INT_MEMRES(i));

			LOG_DBG("Mem %u, Ch %u, Val %d", i, ch, adc_val);
				  ...
				  struct k_poll_signal *sig)
{
	...
	uint8_t ch = ADC_CC23X0_CH_UNDEF;
	uint8_t mem_index = 0;
	size_t exp_size;
	int ret;
	int i;
	...
	if (data->ch_count == 1) {
		...
		data->ch_sel[mem_index] = ch;
		data->mem_index[ch] = mem_index;

		/* Set input channel, memory range, and mode */
		ADCSetInput(data->ref_volt[ch], ch, mem_index);
		ADCSetMemctlRange(mem_index, mem_index);
				    ...
				    const struct adc_channel_cfg *channel_cfg)
{
	...
	if (data->mem_count++ > ADC_CC23X0_MEM_MAX) {
		LOG_ERR("%u channels are already configured", ADC_CC23X0_MEM_COUNT);
		return -ENOMEM;
	}
