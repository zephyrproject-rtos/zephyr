...
static void adc_cc23x0_isr(const struct device *dev)
{
	...
	for (i = 0 ; i < ADC_CC23X0_MEM_COUNT ; i++) {
		if (int_flags & ADC_CC23X0_INT_MEMRES(i)) {
			...
			LOG_DBG("Mem %d, Ch %d, Val %d", i, ch, adc_val);
		}
	}

	if (data->channels) {
		/*
		 * Set adjustment offset for the next channel. This adjustment might not be done
		 * for some channels if several results are processed through a single interrupt
		 * (i.e. several memres flags are set when the ISR triggers).
		 */
		ch = find_lsb_set(data->channels) - 1;
		ADCSetAdjustmentOffset(data->ref_volt[ch]);
		LOG_DBG("Next Ch %d", ch);
	} else {
