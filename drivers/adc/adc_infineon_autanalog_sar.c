 ...
 * reference channels that share one slot; this routine looks that slot up and writes it
 * into the runtime PDL entry, overriding the DT sample-time-sel property (which is only
 * meaningful in static sample-time mode).  In static mode the entries keep the slot
