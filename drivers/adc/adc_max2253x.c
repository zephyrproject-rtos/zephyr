...
static void max2253x_stream_irq_handler(const struct device *dev)
{
	...
	/* Callback SQE */
	rtio_sqe_prep_callback(sqes[2], max2253x_process_sample_cb, (void *)dev, current_sqe);
...
#if CONFIG_ADC_MAX2253X_TRIGGER
				       ...
				       uint32_t pins)
{
	...
	const struct adc_read_config *read_config = data->sqe->sqe.iodev->data;
