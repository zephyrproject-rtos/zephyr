...
static inline int _ti_adc108s102_sampling(struct device *dev)
{
	...
	return spi_transceive(&adc->spi, &tx, &rx);
}
...
static int ti_adc108s102_init(struct device *dev)
{
	...
	adc->spi.dev = device_get_binding(
		CONFIG_ADC_TI_ADC108S102_SPI_PORT_NAME);
	if (!adc->spi.dev) {
		return -EINVAL;
	}

	adc->spi.operation = SPI_WORD_SET(16);
	adc->spi.frequency = CONFIG_ADC_TI_ADC108S102_SPI_FREQ;
	adc->spi.slave = CONFIG_ADC_TI_ADC108S102_SPI_SLAVE;
