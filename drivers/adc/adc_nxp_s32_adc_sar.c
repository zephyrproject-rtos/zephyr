...
#define ADC_NXP_S32_GROUPCHAN_2_PHYCHAN(group, channel) (ADC_SAR_IP_HW_REG_SIZE * group + channel)
...
static int adc_nxp_s32_init(const struct device *dev)
{
	...
	Adc_Sar_Ip_EnableNotifications(
		config->instance, config->callback_select ? ADC_SAR_IP_NOTIF_FLAG_NORMAL_ENDCHAIN
					...
					const struct adc_sequence *sequence)
{
	...
	for (int i = 0; i < data->num_channels; i++) {
		...
		if (mask) {
			Adc_Sar_Ip_EnableChannelNotifications(config->instance, channel,
							      ADC_SAR_IP_CHAN_NOTIF_EOC);
			Adc_Sar_Ip_EnableChannel(config->instance, ADC_SAR_IP_CONV_CHAIN_NORMAL,
						 channel);
		} else {
			Adc_Sar_Ip_DisableChannelNotifications(config->instance, channel,
							       ADC_SAR_IP_CHAN_NOTIF_EOC);
			Adc_Sar_Ip_DisableChannel(config->instance, ADC_SAR_IP_CONV_CHAIN_NORMAL,
						  channel);
		}
	}
...
{
	struct adc_nxp_s32_data *const data = CONTAINER_OF(ctx, struct adc_nxp_s32_data, ctx);
...
static int adc_nxp_s32_read_async(const struct device *dev, const struct adc_sequence *sequence,
...
static int adc_nxp_s32_read(const struct device *dev, const struct adc_sequence *sequence)
{
...
#define ADC_NXP_S32_DRIVER_API(n)                                                                  \
	static const struct adc_driver_api adc_nxp_s32_driver_api_##n = {                          \
		...
		IF_ENABLED(CONFIG_ADC_ASYNC, (.read_async = adc_nxp_s32_read_async,)) .ref_internal = \
