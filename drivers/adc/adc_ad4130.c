...
struct adc_ad4130_data {
	...
	struct ad4130_channel_config channel_setup_cfg[8];
...
static int ad4130_get_reg_size(unsigned int reg_addr, unsigned int *reg_size)
{
...
static int ad4130_reg_write(const struct device *dev, unsigned int reg_addr, unsigned int val)
{
	...
	switch (reg_size) {
	case 1:
		reg_write_tx_buf[1] = val & 0xFF;
		break;
	case 2:
		reg_write_tx_buf[1] = (val >> 8) & 0xFF;
		reg_write_tx_buf[2] = val & 0xFF;
		break;
	case 3:
		reg_write_tx_buf[1] = (val >> 16) & 0xFF;
		reg_write_tx_buf[2] = (val >> 8) & 0xFF;
		reg_write_tx_buf[3] = val & 0xFF;
		break;
	default:
		return -EINVAL;
	}
...
static int ad4130_reg_read(const struct device *dev, unsigned int reg_addr, unsigned int *val)
{
	...
	const struct spi_buf rx_buf = {.buf = reg_read_rx_buf, .len = reg_size};
	const struct spi_buf_set tx = {.buffers = &tx_buf, .count = 1};
...
{
	return ad4130_reg_write_msk(dev, AD4130_CHANNEL_X_REG(channel_id),
				    FIELD_PREP(AD4130_CHANNEL_EN_MASK, enable ? 1 : 0),
...
static int ad4130_init(const struct device *dev)
{
	...
	k_tid_t tid = k_thread_create(
		...
		CONFIG_ADI_AD4130_ADC_ACQUISITION_THREAD_INIT_PRIO, 0, K_NO_WAIT);
