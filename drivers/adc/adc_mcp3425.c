/*
 * Driver for Microchip MCP3425 ADC
 *
 * Copyright (C) 2023 Alva Technology <d.vanlent@alvatechnology.nl>
 *
...
static int mcp3425_channel_setup(const struct device *dev, const struct adc_channel_cfg *cfg)
{
...
static int mcp3425_start_read(const struct device *dev, const struct adc_sequence *seq)
{
...
static int mcp3425_read_async(const struct device *dev, const struct adc_sequence *seq,
...
static int mcp3425_read(const struct device *dev, const struct adc_sequence *seq)
{
...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	setup_config_reg |= (MCP3425_READY_BIT);
...
#define MCP3425_INIT(n)                                                                            \
	...
	K_THREAD_DEFINE(inst_##n##_thread, CONFIG_ADC_MCP3425_ACQUISITION_THREAD_STACK_SIZE,       \
			mcp3425_acq_thread_fn, &inst_##n##_data, NULL, NULL,                       \
			CONFIG_ADC_MCP3425_ASYNC_THREAD_INIT_PRIO, 0, 0);                          \
	static const struct mcp3425_config inst_##n##_config = {                                   \
	...
	DEVICE_DT_INST_DEFINE(n, &mcp3425_init, NULL, &inst_##n##_data, &inst_##n##_config,        \
