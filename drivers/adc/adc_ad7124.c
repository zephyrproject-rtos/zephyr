...
struct ad7124_spi_ready_config {
	uint16_t spi_rdy_poll_cnt;
	bool check_ready;
};
...
struct adc_ad7124_data {
	...
	struct ad7124_spi_ready_config spi_ready;
...
static int adc_ad7124_wait_for_spi_ready(const struct device *dev, uint16_t spi_ready_try_count)
{
	int ret = 0;
	uint32_t read_val = 0;
	bool ready = false;
			       ...
			       enum ad7124_register_lengths len, uint32_t *val)
{
	...
	uint8_t crc_buffer[AD7124_MAX_RETURNED_DATA_SIZE] = {0};
	uint8_t crc_check;

	if (reg != AD7124_ERROR && data->spi_ready.check_ready) {
		ret = adc_ad7124_wait_for_spi_ready(dev, data->spi_ready.spi_rdy_poll_cnt);
		if (ret) {
			return ret;
		}
	}
	...
	if (data->crc_enable == AD7124_ENABLE_CRC) {
		crc_buffer[0] = AD7124_COMM_REG_WEN | AD7124_COMM_REG_RD | AD7124_COMM_REG_RA(reg);
				...
				enum ad7124_register_lengths len, uint32_t val)
{
	...
	struct spi_buf tx_buf[] = {{
		.buf = buffer_tx,
		.len = ((data->crc_enable != AD7124_DISABLE_CRC) ? len + 1 : len) + 1,
	}};
...
int adc_ad7124_wait_to_power_up(const struct device *dev, uint16_t spi_ready_try_count)
{
	int ret = 0;
	uint32_t read_val = 0;
...
int adc_ad7124_reset(const struct device *dev)
{
	...
	ret = adc_ad7124_wait_to_power_up(dev, data->spi_ready.spi_rdy_poll_cnt);
	if (ret) {
		return ret;
	}
...
static int adc_ad7124_update_spi_check_ready(const struct device *dev)
{
	...
	if (reg_temp & AD7124_ERREN_REG_SPI_IGNORE_ERR_EN) {
		data->spi_ready.check_ready = true;
	} else {
		data->spi_ready.check_ready = false;
	}
...
static int adc_ad7124_wait_for_conv_ready(const struct device *dev, uint16_t spi_ready_try_count)
{
	int ret = 0;
	uint32_t read_val = 0;
	bool ready = false;
...
static int adc_ad7124_perform_read(const struct device *dev)
{
	int ret;
	struct adc_ad7124_data *data = dev->data;
	uint16_t active_channels = data->channels;
	uint16_t ch_idx = -1;
	uint16_t adc_ch_id = 0;

	k_sem_take(&data->acquire_signal, K_FOREVER);

	get_next_ch_idx(active_channels, ch_idx, &ch_idx);

	do {
		ret = adc_ad7124_wait_for_conv_ready(dev, data->spi_ready.spi_rdy_poll_cnt);
		if (ret) {
	...
	} while (ch_idx != adc_ch_id);

	data->buffer++;

	while (get_next_ch_idx(active_channels, ch_idx, &ch_idx)) {
		ret = adc_ad7124_wait_for_conv_ready(dev, data->spi_ready.spi_rdy_poll_cnt);
		if (ret) {
			LOG_ERR("waiting for conversion ready failed");
			adc_context_complete(&data->ctx, ret);
