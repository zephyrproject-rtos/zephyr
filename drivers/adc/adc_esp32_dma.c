...
static IRAM_ATTR void adc_esp32_dma_intr_handler(void *arg)
{
	...
	if (i2s_ll_get_intr_status(I2S_LL_GET_HW(0)) & ADC_HAL_DMA_INTR_MASK) {
		i2s_ll_clear_intr_status(I2S_LL_GET_HW(0), ADC_HAL_DMA_INTR_MASK);
#elif defined(CONFIG_SOC_SERIES_ESP32S2)
...
int adc_esp32_dma_init(const struct device *dev)
{
	...
	i2s_ll_enable_clock(I2S_LL_GET_HW(0));
