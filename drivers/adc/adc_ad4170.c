				     ...
				     struct ad4170_channel_config *new_cfg)
{
	...
	if (cfg->acquisition_time != ADC_ACQ_TIME_DEFAULT) {
		LOG_ERR("Unsupported acquisition time %d", cfg->acquisition_time);
		return -EINVAL;
	}
...
#define AD4170_ADC_INIT(compat, inst, id)                                                          \
	static const struct ad4170_config ad4170_config_##inst = {                                 \
		...
		.mclk_hz = DT_INST_PROP_OR(inst, mclk_hz, AD4170_INT_CLOCK_16MHZ),                 \
		.chip_id = id,                                                                     \
	};                                                                                         \
	static struct adc_ad4170_data ad4170_data_##inst = {                                       \
		ADC_CONTEXT_INIT_LOCK(ad4170_data_##inst, ctx),                                    \
		ADC_CONTEXT_INIT_TIMER(ad4170_data_##inst, ctx),                                   \
		ADC_CONTEXT_INIT_SYNC(ad4170_data_##inst, ctx),                                    \
	};                                                                                         \
	DEVICE_DT_DEFINE(DT_INST_AD4170(inst, compat), &ad4170_init, NULL, &ad4170_data_##inst,    \
			 &ad4170_config_##inst, POST_KERNEL, CONFIG_ADC_INIT_PRIORITY,             \
			 &adc_ad4170_driver_api);
