...
static int reference_set(nrf_saadc_channel_config_t *ch_cfg,
...
static int acq_time_set(nrf_saadc_channel_config_t *ch_cfg, uint16_t acq_time)
{
	switch (acq_time) {
...
static void adc_context_start_sampling(struct adc_context *ctx)
{
	...
	if (ctx->sequence.calibrate) {
		nrfx_saadc_offset_calibrate(irq_nrfx_event_handler);
	} else {
