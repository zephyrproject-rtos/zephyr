...
/* Helper macro to convert number of samples to the byte representation. */
#define samples_to_bytes(number_of_samples) (number_of_samples * 2)
...
{
	static const nrf_saadc_input_t range_allowed =
#if (NRF_SAADC_HAS_AIN_AS_PIN)
		ARRAY_SIZE(saadc_psels);
#else
		SAADC_CH_PSELP_PSELP_VDD;
#endif

	nrf_saadc_input_t analog_input = channel_cfg->input_positive;

	if ((analog_input > range_allowed)
#if defined(SAADC_CH_PSELP_PSELP_VDDHDIV5)
	   && (analog_input != SAADC_CH_PSELP_PSELP_VDDHDIV5)
#endif
	) {
		LOG_ERR("Invalid analog positive input number: %d", analog_input);
		return -EINVAL;
	}
#if (NRF_SAADC_HAS_AIN_AS_PIN)
	else if (analog_input < NRF_SAADC_AIN0) {
		LOG_ERR("Invalid analog positive input number: %d", analog_input);
		return -EINVAL;
	}

	*pin_p = saadc_psels[analog_input];
#else
	*pin_p = analog_input;
#endif
	...
	if (channel_cfg->differential) {
		analog_input = channel_cfg->input_negative;
#if (NRF_SAADC_HAS_AIN_AS_PIN)
#if defined(CONFIG_NRF_PLATFORM_HALTIUM)
		if ((analog_input > NRF_SAADC_AIN7) !=
...
#endif
			LOG_ERR("Invalid analog negative input number: %d", analog_input);
			return -EINVAL;
		}
#if (NRF_SAADC_HAS_AIN_AS_PIN)
		else if (analog_input < NRF_SAADC_AIN0) {
			LOG_ERR("Analog negative input not configured.");
			return -EINVAL;
		}
		*pin_n = saadc_psels[analog_input];
#else
		*pin_n = analog_input;
#endif
	} else {
				  ...
				  const struct adc_channel_cfg *channel_cfg)
{
	...
	if (err != 0) {
		LOG_ERR("Cannot assign inputs: %d", err);
		return err;
	}

	err = gain_set(ch_cfg, channel_cfg->gain);
	if (err != 0) {
		LOG_ERR("Cannot set gain: %d", err);
		return err;
	}

	err = reference_set(ch_cfg, channel_cfg->reference);
	if (err != 0) {
		LOG_ERR("Cannot set reference: %d", err);
		return err;
	}

	err = acq_time_set(ch_cfg, channel_cfg->acquisition_time);
	if (err != 0) {
		LOG_ERR("Cannot set acquisition time: %d", err);
		return err;
	}
...
static void adc_context_update_buffer_pointer(struct adc_context *ctx, bool repeat)
{
	if (!repeat) {
#if defined(ADC_BUFFER_IN_RAM)
		m_data.user_buffer = (uint16_t *)m_data.user_buffer + m_data.size;
#else
		nrf_saadc_value_t *buffer = (uint16_t *)m_data.p_buffer + m_data.size;

		nrfx_saadc_buffer_set(buffer, m_data.size);
#endif
	}
}
...
static int get_oversampling(const struct adc_sequence *sequence, uint8_t active_channels_cnt,
			    nrf_saadc_oversample_t *oversampling)
{
	if ((active_channels_cnt > 1) && (sequence->oversampling > 0)) {
		LOG_ERR(
...
static int check_buffer_size(const struct adc_sequence *sequence, uint8_t active_channels_cnt)
{
	size_t needed_buffer_size;

	needed_buffer_size = samples_to_bytes(active_channels_cnt);
...
static void correct_single_ended(const struct adc_sequence *sequence)
{
	...
	int16_t *sample = (int16_t *)m_data.p_buffer;
		      ...
		      const struct adc_sequence *sequence)
{
	...
	nrf_saadc_oversample_t oversampling;
	uint8_t active_channels_cnt = 0U;
	...
	error = get_oversampling(sequence, active_channels_cnt, &oversampling);
	if (error) {
		return error;
	}
	...
	error = check_buffer_size(sequence, active_channels_cnt);
	if (error) {
		return error;
	}
	...
	m_data.active_channels_cnt = active_channels_cnt;

	nrfx_saadc_buffer_set(m_data.samples_buffer, active_channels_cnt);
#else
	 ...
	 */
	nrfx_saadc_buffer_set(sequence->buffer, active_channels_cnt);
#endif
...
static void event_handler(const nrfx_saadc_evt_t *event)
{
	...
	m_data.p_buffer = event->data.done.p_buffer;
	m_data.size = event->data.done.size;
	...
	if (event->type == NRFX_SAADC_EVT_DONE) {
		...
		memcpy(m_data.user_buffer, m_data.samples_buffer,
		       samples_to_bytes(m_data.active_channels_cnt));
#endif
		...
		LOG_ERR("%s:%d: Unknown event type: %d", __func__, __LINE__, event->type);
	}
}
