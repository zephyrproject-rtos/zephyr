...
#if !defined(CONFIG_HAS_NORDIC_DMM)
		nrfx_err_t nrfx_err =
			nrfx_saadc_buffer_set(m_data.samples_buffer, m_data.active_channels_cnt);
		      ...
		      const struct adc_sequence *sequence)
{
	...
	error = dmm_buffer_in_prepare(m_data.mem_reg, m_data.user_buffer,
				      (m_data.internal_timer_enabled
					       ? (samples_to_bytes(active_channels_cnt) *
	...
	nrfx_err = nrfx_saadc_buffer_set(
		...
		(m_data.internal_timer_enabled
			 ? (active_channels_cnt * (1 + sequence->options->extra_samplings))
			 : active_channels_cnt));
	if (nrfx_err != NRFX_SUCCESS) {
		LOG_ERR("Failed to set buffer: %08x", nrfx_err);
		return -EINVAL;
