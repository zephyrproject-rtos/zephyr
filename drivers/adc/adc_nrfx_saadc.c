...
static int saadc_pm_hook(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		...
		LOG_DBG("SAADC disabled");
		return 0;

	case PM_DEVICE_ACTION_RESUME:
		nrfy_saadc_enable(NRF_SAADC);
		LOG_DBG("SAADC enabled");
		return 0;
...
static int pin_assign(nrf_saadc_input_t *pin_p,
		      ...
		      const struct adc_channel_cfg *channel_cfg)
{
	...
	if (analog_input > range_allowed) {
		...
		LOG_ERR("Analog positive input pin not configured.");
		return -EINVAL;
	}
#if (NRF_SAADC_HAS_AIN_AS_PIN)
...
#if (NRF_SAADC_HAS_AIN_AS_PIN)
#if defined(CONFIG_NRF_PLATFORM_HALTIUM)
		if (analog_input > NRF_SAADC_AIN13) {
#else
		if (analog_input > NRF_SAADC_AIN7) {
#endif
#else
#if defined(SAADC_CH_PSELP_PSELP_VDDHDIV5)
		if (analog_input > SAADC_CH_PSELP_PSELP_VDDHDIV5) {
#else
		if (analog_input > SAADC_CH_PSELP_PSELP_VDD) {
#endif
#endif
				  ...
				  const struct adc_channel_cfg *channel_cfg)
{
	...
	err = pin_assign(&cfg.pin_p, &cfg.pin_n, channel_cfg);
	if (err != 0) {
		LOG_ERR("Cannot assign pins: %d", err);
		return -EINVAL;
	}
...
static void adc_context_update_buffer_pointer(struct adc_context *ctx, bool repeat)
{
	if (!repeat) {
		...
		nrf_saadc_value_t *buffer =
			(uint8_t *)m_data.event->data.done.p_buffer +
			samples_to_bytes(&ctx->sequence, m_data.event->data.done.size);
...
static int get_oversampling(const struct adc_sequence *sequence, uint8_t channels_cnt,
			    nrf_saadc_oversample_t *oversampling)
{
	if ((channels_cnt > 1) && (sequence->oversampling > 0)) {
		LOG_ERR(
...
static int check_buffer_size(const struct adc_sequence *sequence, uint8_t channels_cnt)
{
	size_t needed_buffer_size;

	needed_buffer_size = samples_to_bytes(sequence, channels_cnt);
		      ...
		      const struct adc_sequence *sequence)
{
	...
	channels_cnt = POPCOUNT(nrfx_saadc_channels_configured_get() & selected_channels);
	if (channels_cnt == 0) {
...
static void event_handler(const nrfx_saadc_evt_t *event)
{
	...
	if (m_data.event->type == NRFX_SAADC_EVT_DONE) {
#if defined(CONFIG_PM_DEVICE_RUNTIME)
		...
		memcpy(m_data.user_buffer, m_data.samples_buffer,
		       samples_to_bytes(&m_data.ctx.sequence, m_data.channels_cnt));
#endif
		...
		LOG_ERR("%s:%d: Unknown event type: %d", __func__, __LINE__, m_data.event->type);
	}
}
