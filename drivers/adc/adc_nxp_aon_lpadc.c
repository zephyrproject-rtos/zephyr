...
/* Conversion commands implemented by the hardware, i.e. the upper bound on the
 * number of logical channels. How many of them an instance actually reserves is
 * derived from devicetree, see LPADC_AON_MAX_CHANNELS().
 */
...
struct adc_aon_lpadc_data {
	...
	/* Physical input (CMDL[ADCH]) selected for each logical channel. Sized
	 * per instance from the channel nodes present in devicetree.
	 */
				       ...
				       const struct adc_channel_cfg *channel_cfg)
{
	...
	/* The reference is selected for the whole converter through CFG[REFSEL], never
	 * per command, and both REFSEL settings select VDD_ANA_LPADC, the externally
	 * supplied analog rail of the converter. A channel can only ask for that rail.
	 */
...
static int adc_aon_lpadc_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	if ((sequence->channels & ~BIT_MASK(data->max_channels)) != 0U) {
		LOG_ERR("Channel id out of range (max %u)", data->max_channels - 1U);
		return -EINVAL;
	}
	 ...
	 */
	for (int8_t ch = (int8_t)data->max_channels - 1; ch >= 0; ch--) {
		uint8_t command;
...
/*
 * Logical channels are hardware command slots. Size the per-instance input
 ...
 */
#define LPADC_AON_CHANNEL_ID_BIT(node)                                                             \
	IF_ENABLED(DT_NODE_HAS_PROP(node, reg), (BIT(DT_REG_ADDR_RAW(node)) |))
#define LPADC_AON_CHANNEL_ID_MASK(n)                                                               \
	(DT_INST_FOREACH_CHILD_STATUS_OKAY(n, LPADC_AON_CHANNEL_ID_BIT) 0)
#define LPADC_AON_MAX_CHANNELS(n)                                                                  \
	(LPADC_AON_CHANNEL_ID_MASK(n) == 0U ? LPADC_AON_CMD_COUNT                                  \
					    : LOG2(LPADC_AON_CHANNEL_ID_MASK(n)) + 1)
...
#define ADC_AON_LPADC_INIT(n)                                                                      \
	...
	static const struct adc_aon_lpadc_config adc_aon_lpadc_config_##n = {                      \
		...
		.pincfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),                                       \
                                                                                                   ...
                                                                                                   \
	static uint8_t adc_aon_lpadc_channel_input_##n[LPADC_AON_MAX_CHANNELS(n)];                 \
                                                                                                   \
	static struct adc_aon_lpadc_data adc_aon_lpadc_data_##n = {                                \
		...
		.channel_input = adc_aon_lpadc_channel_input_##n,                                  \
		.max_channels = LPADC_AON_MAX_CHANNELS(n),                                         \
	};                                                                                         \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, adc_aon_lpadc_init, NULL, &adc_aon_lpadc_data_##n,                \
			      ...
			      &adc_aon_lpadc_driver_api);                                          \
                                                                                                   \
	BUILD_ASSERT(LPADC_AON_MAX_CHANNELS(n) <= LPADC_AON_CMD_COUNT,                             \
