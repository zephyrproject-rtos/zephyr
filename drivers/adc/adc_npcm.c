...
#define HAL_INSTANCE(dev) ((struct adc_reg *)((const struct adc_npcm_config *)(dev)->config)->base)

static int adc_npcm_validate_buffer_size(const struct device *dev,
...
static void npcm_adc_start_convert(const struct device *dev)
{
	...
	adc_regs->ADCACTRL1 &= ~MaskBit(NPCM_ACTRL1_PWCTRL);
	...
	if (IsDiode) {
		adc_regs->DSADCCTRL0 &= ~MaskBit(NPCM_CTRL0_VNT);
		...
		case CHEN20_TD1P:
			...
			adc_regs->ADCTM &= ~(0x03 << NPCM_TM_T_MODE2);
			break;
		case CHEN21_TD0P:
			adc_regs->DSADCCTRL0 |= 0x02;
			adc_regs->ADCTM &= ~(0x03 << NPCM_TM_T_MODE3);
			break;
		case CHEN22_TD3P:
			adc_regs->DSADCCTRL0 |= 0x03;
			adc_regs->ADCTM &= ~(0x03 << NPCM_TM_T_MODE4);
			break;
		case CHEN23_TD4P:
			adc_regs->DSADCCTRL0 |= 0x04;
			adc_regs->ADCTM &= ~(0x03 << NPCM_TM_T_MODE5);
			break;
		default:
			break;
		}
	...
	adc_regs->DSADCCFG |= (MaskBit(NPCM_CFG_ICEN) | MaskBit(NPCM_CFG_START));
}
...
static void adc_npcm_isr(const struct device *dev)
{
	...
	result &= ~MaskBit(NPCM_TCHNDATA_NEW);
...
static int adc_npcm_start_read(const struct device *dev,
				  const struct adc_sequence *sequence)
{
	const struct adc_npcm_config *config = dev->config;
	struct adc_npcm_data *const data = dev->data;
	...
	/* Fixed 10 bit resolution of npcm ADC */
	if (sequence->resolution != 10) {
		LOG_ERR("Unfixed 10 bit ADC resolution");
		return -ENOTSUP;
	}

	error = adc_npcm_validate_buffer_size(dev, sequence);
	if (error) {
...
{
	struct adc_npcm_data *const data =
		CONTAINER_OF(ctx, struct adc_npcm_data, ctx);
	...
	if (!data->channels) {
		...
		npcm_adc_start_convert(data->adc_dev);
	}
}
...
static int adc_npcm_read(const struct device *dev,
			    const struct adc_sequence *sequence)
{
	struct adc_npcm_data *const data = dev->data;
	int error;
...
static int adc_npcm_read_async(const struct device *dev,
				  const struct adc_sequence *sequence,
...
#define NPCM_ADC_INIT(n)							\
										\
	static void adc_npcm_irq_cfg_func_##n(void)				\
	{									\
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority),		\
			    adc_npcm_isr, DEVICE_DT_INST_GET(n), 0);		\
		irq_enable(DT_INST_IRQN(n));					\
	}									\
										\
	static const struct adc_driver_api adc_npcm_driver_api_##n = {		\
	...
	PINCTRL_DT_INST_DEFINE(n);						\
										\
	static const struct adc_npcm_config adc_npcm_cfg_##n = {		\
		.base = DT_INST_REG_ADDR(n),					\
		.channel_count = DT_INST_PROP(n, channel_count),		\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),			\
		.irq_cfg_func = adc_npcm_irq_cfg_func_##n,			\
	};									\
	static struct adc_npcm_data adc_npcm_data_##n = {			\
		ADC_CONTEXT_INIT_TIMER(adc_npcm_data_##n, ctx),			\
		ADC_CONTEXT_INIT_LOCK(adc_npcm_data_##n, ctx),			\
		ADC_CONTEXT_INIT_SYNC(adc_npcm_data_##n, ctx),			\
	};									\
	DEVICE_DT_INST_DEFINE(n,						\
			     adc_npcm_init, NULL,				\
			     &adc_npcm_data_##n, &adc_npcm_cfg_##n,		\
			     PRE_KERNEL_1, CONFIG_ADC_INIT_PRIORITY,		\
			     &adc_npcm_driver_api_##n);
