					     ...
					     lpadc_conv_command_config_t *cmd)
{
	...
	uint32_t sample_time_array[] = {3, 5, 7, 11, 19, 35, 67, 131};
	 ...
	 */
		...
		for (uint8_t index = 0; index < sample_time_array_size; ++index) {
			sample_time_array[index] =
				(uint32_t)(sample_time_array[index] * adc_cycle_ns);
		}
		...
		if (sample_time_array[0] > acquisition_time_value) {
			...
			left_gap = abs((int)(sample_time_array[array_left_index] -
					     acquisition_time_value));
			right_gap = abs((int)(sample_time_array[array_right_index] -
					      acquisition_time_value));
