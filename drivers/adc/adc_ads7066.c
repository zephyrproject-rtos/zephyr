...
static int ads7066_reset(const struct device *dev);
...
static int ads7066_read_samples(const struct device *dev)
{
	...
	if (channel == data->channel) {
		/*
		 * The next conversion result would come from the last transceive which could have
		 * been a long time ago, therefore we must "flush" the previous conversion in order
		 * not to read an old conversion result.
		 */
...
static int adc_ads7066_configure(const struct device *dev)
{
	...
	err = ads7066_set_register_bits(dev, ADS7066_REG_GENERAL_CFG, ADS7066_CFG_CAL, &options);
	if (err) {
		return err;
	}
...
static int ads7066_reset(const struct device *dev)
{
	...
	data->auto_reg_channels_sel = 0;
	data->channel = 0;

	return adc_ads7066_configure(dev);
}
...
static int adc_ads7066_init(const struct device *dev)
{
	...
	/*
	 * It seems that sometimes (rarely) the ADC gets stuck with invalid conversion results and
	 * status flags after calibration. As a mitigation we try to read a conversion after
	 * calibration with status flags enabled and if an error is detected, we reset the ADC.
	 */
	err = ads7066_sanity_check(dev);
	if (err) {
		LOG_ERR("failed sanity check (%d), attempting reset", err);

		err = ads7066_reset(dev);
		if (err) {
			LOG_ERR("failed to reset the ADC (%d)", err);
			return err;
		}
