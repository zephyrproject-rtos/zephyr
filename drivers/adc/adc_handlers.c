					   ...
					   const struct adc_channel_cfg *user_channel_cfg)
{
	...
	K_OOPS(k_usermode_from_copy(&channel_cfg, (struct adc_channel_cfg *)user_channel_cfg,
				    sizeof(struct adc_channel_cfg)));

	return z_impl_adc_channel_setup((const struct device *)dev, &channel_cfg);
}
#include <zephyr/syscalls/adc_channel_setup_mrsh.c>

static bool copy_sequence(struct adc_sequence *dst, struct adc_sequence_options *options,
...

{
	...
	K_OOPS(K_SYSCALL_VERIFY_MSG(
		copy_sequence(&sequence, &options, (struct adc_sequence *)user_sequence),
