...
struct mcp320x_data {
	...
	K_KERNEL_STACK_MEMBER(stack, CONFIG_ADC_MCP320X_ACQUISITION_THREAD_STACK_SIZE);
};
...
static int mcp320x_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	const struct mcp320x_config *config = dev->config;
	struct mcp320x_data *data = dev->data;
	int err;
...
static int mcp320x_read_async(const struct device *dev, const struct adc_sequence *sequence,
...
static int mcp320x_read(const struct device *dev, const struct adc_sequence *sequence)
{
...
static int mcp320x_read_channel(const struct device *dev, uint8_t channel, uint16_t *result)
{
	const struct mcp320x_config *config = dev->config;
	struct mcp320x_data *data = dev->data;
	uint8_t tx_bytes[2];
	uint8_t rx_bytes[2];
	int err;
	const struct spi_buf tx_buf[2] = {{.buf = tx_bytes, .len = sizeof(tx_bytes)},
					  {.buf = NULL, .len = 1}};
	const struct spi_buf rx_buf[2] = {{.buf = NULL, .len = 1},
					  {.buf = rx_bytes, .len = sizeof(rx_bytes)}};
...
static int mcp320x_init(const struct device *dev)
{
	...
	k_tid_t tid =
		k_thread_create(&data->thread, data->stack, K_KERNEL_STACK_SIZEOF(data->stack),
				mcp320x_acquisition_thread, data, NULL, NULL,
				CONFIG_ADC_MCP320X_ACQUISITION_THREAD_PRIO, 0, K_NO_WAIT);
