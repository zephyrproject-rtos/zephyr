...
struct ads79xx_config {
	...
	uint8_t resolution;
	uint8_t range;
	bool power_down_between_samples;
};
...
static int ads79xx_spi_transfer(const struct device *dev, uint16_t tx_word, uint16_t *rx_word)
{
	...
	int ret = spi_transceive_dt(&cfg->spi, &txs, &rxs);
	if (ret) {
		return ret;
	}
...
static inline uint16_t ads79xx_manual_command(const struct ads79xx_config *cfg, int ch)
{
	...
	if (cfg->power_down_between_samples) {
		cmd |= ADS79xx_CR_POWERDOWN;
	}
...
{
	uint16_t rx;
	int ret = ads79xx_spi_transfer(dev, ads79xx_ctrl_auto1_prog_entry(), &rx);
	if (ret) {
		return ret;
	}
...
{
	const struct ads79xx_config *cfg = dev->config;
	uint16_t w = ads79xx_ctrl_auto1(reset_counter, false, (cfg->range == 2),
					cfg->power_down_between_samples, 0);
	uint16_t rx;
...
static int ads79xx_start_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	int ret = ads79xx_validate_sequence(dev, sequence);
	if (ret) {
			      ...
			      struct k_poll_signal *async)
{
	...
	int ret = ads79xx_start_read(dev, sequence);
...
static void ads79xx_acquisition_thread(void *p1, void *p2, void *p3)
{
	...
	/* Prime the ads79xx */
	uint16_t dummy;
	int ret = ads79xx_spi_transfer(dev, ads79xx_manual_command(cfg, 0), &dummy);
	if (ret) {
...
#define ADS79XX_INIT(t, n, chan, res)                                                              \
	...
	static const struct ads79xx_config ads##t##_cfg_##n = {                                    \
		...
		.power_down_between_samples = DT_INST_PROP_OR(n, ti_powerdown_between_samples, 0), \
	};                                                                                         \
