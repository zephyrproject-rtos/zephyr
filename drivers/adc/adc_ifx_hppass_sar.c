 ...
 *
 * Zephyr driver file for the HPPASS SAR ADC used in the Infineon PSOC C3 series.
 */
...
static int ifx_hppass_sar_configure_group(uint32_t channels, uint32_t group)
{
	...
	if ((channels & ADC_SAMPLER_12_CHANNEL_GROUP) != 0) {
		...
		for (int i = 0; i < 4; i++) {
			if (channels & (1 << (HPPASS_SAR_DIR_CHANNEL_CNT + i))) {
				group_cfg.muxSampMsk |= (1 << 0); /* Enable sampler 12 */
				group_cfg.muxChanIdx[0] = i;
				break;
			}
		}
	}
	if ((channels & ADC_SAMPLER_13_CHANNEL_GROUP) != 0) {
		...
		for (int i = 0; i < 4; i++) {
			if (channels & (1 << (16 + i))) {
				group_cfg.muxSampMsk |= (1 << 1); /* Enable sampler 13 */
				group_cfg.muxChanIdx[1] = i;
				break;
			}
		}
	}
	if ((channels & ADC_SAMPLER_14_CHANNEL_GROUP) != 0) {
		group_cfg.muxChanIdx[2] = 0;
		for (int i = 0; i < 4; i++) {
			if (channels & (1 << (20 + i))) {
				group_cfg.muxSampMsk |= (1 << 2); /* Enable sampler 14 */
				group_cfg.muxChanIdx[2] = i;
				break;
			}
		}
	}
	if ((channels & ADC_SAMPLER_15_CHANNEL_GROUP) != 0) {
		group_cfg.muxChanIdx[3] = 0;
		for (int i = 0; i < 4; i++) {
			if (channels & (1 << (24 + i))) {
				group_cfg.muxSampMsk |= (1 << 3); /* Enable sampler 15 */
				group_cfg.muxChanIdx[3] = i;
				break;
			}
		}
	}
...
/**
 * @brief Read results specified group of channels
 *
 ...
 * started.
 */
static void _ifx_init_pdl_struct(struct ifx_hppass_sar_adc_data *data,
...
static void _ifx_init_channel_cfg(struct ifx_hppass_sar_adc_data *data)
{
	for (int i = 0; i < HPPASS_SAR_ADC_MAX_CHANNELS; i++) {
		...
		if (data->hppass_sar_chan_obj[i].enabled) {
			data->hppass_sar_obj.chan[i] =
				&data->hppass_sar_chan_obj[i].pdl_channel_cfg;
		} else {
					    ...
					    const struct adc_channel_cfg *channel_cfg)
{
	...
	data->hppass_sar_chan_obj[channel_cfg->channel_id].channel_id = channel_cfg->channel_id;
...
static int ifx_hppass_sar_adc_init(const struct device *dev)
{
	...
	LOG_ERR("Debug: dir_samp_en_mask = 0x%04X", cfg->dir_samp_en_mask);
	LOG_ERR("Debug: mux_samp_en_mask = 0x%04X", cfg->mux_samp_en_mask);
	 ...
	 */
	_ifx_init_pdl_struct(data, cfg);
	_ifx_init_channel_cfg(data);
...
#define IFX_HPPASS_SAR_ADC_INIT(n)                                                                 \
	...
	static const struct ifx_hppass_sar_adc_config ifx_hppass_sar_adc_config_##n = {            \
		...
		.vref_mv = DT_INST_PROP_OR(n, vref_mv, 0)};                                        \
