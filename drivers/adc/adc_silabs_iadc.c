				  ...
				  size_t data_size)
{
	...
	needed_buffer_size = active_channels * data_size;
 ...
 * +------------+-------------+---------------+----------------+-------------+----------------+
 * | Alignment  | Oversample  | Digital Avg   | Num Samples   | Output Res  | DATA Mask     |
						  ...
						  struct iadc_data *data)
{
	...
	const static struct oversampling_table {
		...
		{ 0, _IADC_CFG_OSRHS_HISPD2, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT12, 12, 0xFFF0 }, /* default */
		{ 1, _IADC_CFG_OSRHS_HISPD2, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT12, 12, 0xFFF0 }, /* default */
		{ 2, _IADC_CFG_OSRHS_HISPD4, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT16, 13, 0xFFF8 }, /* 4x oversampling*/
		{ 3, _IADC_CFG_OSRHS_HISPD8, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT16, 14, 0xFFFC }, /* 8x oversampling*/
		{ 4, _IADC_CFG_OSRHS_HISPD16, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT16, 15, 0xFFFE }, /* 16x oversampling*/
		{ 5, _IADC_CFG_OSRHS_HISPD32, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT16, 16, 0xFFFF }, /* 32x oversampling*/
		{ 6, _IADC_CFG_OSRHS_HISPD64, _IADC_CFG_DIGAVG_AVG1,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT20, 17, 0xFFFF8 }, /* 64x oversampling*/
		{ 7, _IADC_CFG_OSRHS_HISPD64, _IADC_CFG_DIGAVG_AVG2,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT20, 18, 0xFFFFC }, /* 128x oversampling*/
		{ 8, _IADC_CFG_OSRHS_HISPD64, _IADC_CFG_DIGAVG_AVG4,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT20, 19, 0xFFFFE }, /* 256x oversampling*/
		{ 9, _IADC_CFG_OSRHS_HISPD64, _IADC_CFG_DIGAVG_AVG8,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT20, 20, 0xFFFFF }, /* 512x oversampling*/
		{ 10, _IADC_CFG_OSRHS_HISPD64, _IADC_CFG_DIGAVG_AVG16,
		 _IADC_SCANFIFOCFG_ALIGNMENT_RIGHT20, 21, 0xFFFFF }, /* 1024x oversampling*/
	};

	if (!sequence->oversampling) {
		/* Default to 2x oversampling (as in the default config)*/
		data->oversampling = _IADC_CFG_OSRHS_HISPD2;
		if (!sequence->resolution) {
			/* For compliance with adc_api test, a resolution of 0 is considered as an
			 * error
			 */
			return -EINVAL;
			/* Default to 12 bit resolution (as in the default config)*/
			...
			if (res == 12) {
				...
				if (IS_ENABLED(IADC_NO_EXTENDED_ALIGN)) {
					LOG_ERR("Unsupported resolution %d", res);
					return -EINVAL;
				}
				LOG_DBG("Using %d bit resolution but only 12 bit of accuracy since "
					"no oversampling is activated",
					res);
		...
		/* Max general samples averaging is 2^oversampling = 1024 */
		if (ospl < 0 || ospl > 10) {
		...
		if (!sequence->resolution) {
			data->alignment = ospl_table[ospl].alignment;
			data->resolution = ospl_table[ospl].resolution;
			data->data_mask = ospl_table[ospl].data_mask;
		} else {
			res = sequence->resolution;
			...
			if (ospl_table[ospl].resolution != res) {
				LOG_DBG("Non optimal resolution %d for given oversampling %d", res,
					ospl);
			}

			for (int i = 0; i < ARRAY_SIZE(ospl_table); i++) {
				if (ospl_table[i].resolution == res) {
					data->alignment = ospl_table[i].alignment;
					data->resolution = res;
					data->data_mask = ospl_table[i].data_mask;
					break;
				}
			}
		}
