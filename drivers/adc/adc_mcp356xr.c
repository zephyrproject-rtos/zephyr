...
} adc_mcp356xr_register_address_t;

typedef struct {
	adc_mcp356xr_command_type_t type;
	union {
		adc_mcp356xr_fast_command_t fast_command;
		adc_mcp356xr_register_address_t register_address;
	};
} adc_mcp356xr_command_t;
...
#define RCP356X_STATUS_ADDRESS_ACK_FIELD_VALUE(address) ((address << 1) | (~address & 0x1))
...
enum {
	...
	_MCP356XR_REGISTER_CONFIG0_CLK_SEL_EXTERNAL_CLOCK = 1, /* also selects the external clock */
...
enum {
	...
	_MCP356XR_REGISTER_CONFIG0_ADC_MODE_SHUTDOWN = 1, /* also causes ADC to shutdown */
...
#define _REGISTER_INDEX(register_address)                                                          \
	((register_address >= MCP356XR_REGISTER_ADDRESS_CONFIG0)                                   \
		 ...
		 : MCP356XR_REGISTER_ADDRESS_MUX)
#define REGISTER_INDEX(register_name) _REGISTER_INDEX(REGISTER_ADDRESS(register_name))
...
#ifdef CONFIG_ADC_MCP356XR_USE_READ_CRC
	if (crc != NULL) {
		*crc = ((uint16_t)crc_buffer[0]) << 8 | crc_buffer[1];
	}
#else
	(void)crc;
#endif
...
static int adc_mcp356xr_send_command(const struct device *dev, adc_mcp356xr_command_t command,
				     uint8_t *status_flags, uint8_t *data, size_t length)
{
	...
	if (MCP356XR_STATUS_FIELD_GET(ADDRESS_ACK, status_byte) !=
	    RCP356X_STATUS_ADDRESS_ACK_FIELD_VALUE(config->address)) {
		return -ENOMSG;
	}
...
static int adc_mcp356xr_send_fast_command(const struct device *dev,
					  adc_mcp356xr_fast_command_t fast_command)
{
	adc_mcp356xr_command_t command = {
...
static int adc_mcp356xr_incremental_write(const struct device *dev,
					  adc_mcp356xr_register_address_t start_register_address,
...
static int adc_mcp356xr_read(const struct device *dev, const struct adc_sequence *sequence)
{
	...
	err = ads114s0x_adc_start_read(dev, sequence);
...
static const struct adc_driver_api adc_mcp356xr_api = {
