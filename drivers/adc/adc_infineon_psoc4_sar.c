				   ...
				   const struct adc_channel_cfg *channel_cfg)
{
	...
	if (channel_cfg->differential) {
		...
		} else if (data->channel_cfg[ch].vminus_routing == PSOC4_ADC_ROUTE_AMUXBUS_A ||
			   data->channel_cfg[ch].vminus_routing == PSOC4_ADC_ROUTE_AMUXBUS_B) {

			/* Calculate GPIO port and pin number within that port */
			uint32_t port = psoc4_get_gpio_port(channel_cfg->input_negative,
							    data->channel_cfg[ch].vminus_routing);
			uint32_t gpio_pin =
				channel_cfg->input_negative % PSOC4_ADC_GPIO_PINS_PER_PORT;

			GPIO_PRT_Type *port_base = Cy_GPIO_PortToAddr(port);

			if (!port_base) {
				LOG_ERR("Channel %u: Cannot get GPIO port %u for AMUXBUS negative "
					"input pin %u",
					ch, port, channel_cfg->input_negative);
				return -EINVAL;
			}

			/* Configure AMUXBUS splitter cells */
			psoc4_configure_amuxbus_splitter(data->channel_cfg[ch].vminus_routing,
							 port);

			Cy_GPIO_SetDrivemode(port_base, gpio_pin, CY_GPIO_DM_ANALOG);

			/* Configure HSIOM to connect GPIO pin to AMUXBUS */
			en_hsiom_sel_t hsiom_sel =
				(data->channel_cfg[ch].vminus_routing == PSOC4_ADC_ROUTE_AMUXBUS_A)
					? HSIOM_SEL_AMUXA
					: HSIOM_SEL_AMUXB;
			Cy_GPIO_SetHSIOM(port_base, gpio_pin, hsiom_sel);
				   ...
				   const struct psoc4_adc_config *cfg)
{
	...
	for (int ch = 0; ch < CY_SAR_SEQ_NUM_CHANNELS; ch++) {
		if (sequence->channels & BIT(ch)) {
			...
			/* Enable SARSEQ hardware control for multi-channel sequences */
			if (ch_cfg->vplus_routing == PSOC4_ADC_ROUTE_SARMUX) {
				/* SARMUX pins 0-7: Enable hardware control for the pin */
				hw_ctrl_mask |= SAR_MUX_SWITCH_HW_CTRL_MUX_HW_CTRL_P0_Msk
						<< ch_cfg->vplus;
			} else if (ch_cfg->vplus_routing == PSOC4_ADC_ROUTE_AMUXBUS_A) {
				hw_ctrl_mask |= SAR_MUX_SWITCH_HW_CTRL_MUX_HW_CTRL_AMUXBUSA_Msk;
			} else if (ch_cfg->vplus_routing == PSOC4_ADC_ROUTE_AMUXBUS_B) {
				hw_ctrl_mask |= SAR_MUX_SWITCH_HW_CTRL_MUX_HW_CTRL_AMUXBUSB_Msk;
			}
