...
static int start_read_stream(const struct device *dev, const struct adc_sequence *seq)
{
	...
	if (seq->buffer_size < (num_of_sample * num_of_sample_channels)) {
		return -ENOMEM;
	}
...
uint8_t adc_max32_convert_vref_to_shift(uint16_t vref_mv)
{
	uint8_t count = 1;
			  ...
			  uint16_t max_count, void *data_out)
{
	...
	data->shift = adc_max32_convert_vref_to_shift(enc_data->vref_mv);
	...
	while (count < max_count && buffer < buffer_end) {
		...
		adc_max32_convert_q31(&data->readings[count].value, buffer,
