...
{
	/* Late if (now - target) % top_value < guard_period. */
	return now - target < guard;
}
...
{
	/* Late if (now - target) % top_value < guard_period. */
	return ((now - target) & OSTIMER_COUNTER_MASK) < guard;
}
#endif
				 ...
				 const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if (alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) {
		...
		target = (upper | ticks32) & OSTIMER_COUNTER_MASK;

		if ((0U != data->guard_period) &&
		     ostimer_is_late_32((uint32_t)now, (uint32_t)target, data->guard_period)) {
			if (alarm_cfg->flags & COUNTER_ALARM_CFG_EXPIRE_WHEN_LATE) {
				 ...
				 */
				ostimer_set_alarm_cb_32(data,
					alarm_cfg->callback, alarm_cfg->user_data);
				NVIC_SetPendingIRQ(config->irqn);
			}
			return -ETIME;
		}
	} else {
		target = (now + alarm_cfg->ticks) & OSTIMER_COUNTER_MASK;
	}
				    ...
				    const struct counter_alarm_cfg_64 *alarm_cfg)
{
	...
	if (alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) {
		target = alarm_cfg->ticks & OSTIMER_COUNTER_MASK;

		if ((0U != data->guard_period_64) &&
		     ostimer_is_late_64(now, target, data->guard_period_64)) {
			if (alarm_cfg->flags & COUNTER_ALARM_CFG_EXPIRE_WHEN_LATE) {
				ostimer_set_alarm_cb_64(data,
					alarm_cfg->callback, alarm_cfg->user_data);
				NVIC_SetPendingIRQ(config->irqn);
			}
			return -ETIME;
