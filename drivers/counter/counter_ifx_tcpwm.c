...
struct ifx_tcpwm_counter_data {
	...
	/* Default value of the timer/counter. The cyhal_timer_reset() function will also
	 * change counter to this value when called.
	 */
	uint32_t value;
...
static void timer_enable_event(const struct device *dev, counter_event_t event, bool enable)
{
...
static void counter_isr_handler(const struct device *dev)
{
	...
	if ((data->alarm_cfg.callback != NULL) &&
		...
		timer_enable_event(dev, COUNTER_IRQ_CAPTURE_COMPARE, false);
...
static void set_int_pending(const struct device *dev)
{
	__ASSERT_NO_MSG(dev != NULL);

	const struct ifx_tcpwm_counter_config *const config = dev->config;

	timer_enable_event(dev, COUNTER_IRQ_CAPTURE_COMPARE, true);
...
static int ifx_tcpwm_counter_init(const struct device *dev)
{
	...
	data->alarm_irq_flag = false;
	data->period = config->counter_info.max_top_value;
	data->compare_value = 0;
	data->value = 0;

	/* Configure timer */
	counter_config.period = data->period;
					   ...
					   const struct counter_top_cfg *cfg)
{
	...
	ticks_gt_period = cfg->ticks > data->period;
	...
	if ((ticks_gt_period == false) ||
		...
		if (cfg->callback != NULL) {
			timer_enable_event(dev, COUNTER_IRQ_TERMINAL_COUNT, true);
		}
	}
	return 0;
}
				       ...
				       const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if (absolute) {
		...
		compare_value = ticks_add(IFX_TCPWM_Counter_GetCounter(config->reg_base),
	...
	uint32_t diff = ticks_sub((compare_value - 1), curr, top_val);
