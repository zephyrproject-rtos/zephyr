 ...
 *
 * Based on counter_mcux_rtc.c which is:
...
LOG_MODULE_REGISTER(counter_mcux_lpc_rtc, CONFIG_COUNTER_LOG_LEVEL);
...
static inline uint32_t mcux_lpc_rtc_read(const struct device *dev)
{
	const struct counter_config_info *info = dev->config;
	const struct mcux_lpc_rtc_config *config =
		CONTAINER_OF(info, struct mcux_lpc_rtc_config, info);

	return RTC_GetSecondsTimerCount(config->base);
}
				      ...
				      const struct counter_top_cfg *cfg)
{
	...
	if (cfg->ticks != info->max_top_value) {
		LOG_ERR("Max top value can only be set to 0x%x.", info->max_top_value);
		return -ENOTSUP;
	}
...
static uint32_t mcux_lpc_rtc_get_pending_int(const struct device *dev)
{
	...
	return RTC_GetStatusFlags(config->base) & kRTC_AlarmInterruptEnable;
}
...
static uint32_t mcux_lpc_rtc_get_max_relative_alarm(const struct device *dev)
{
	const struct counter_config_info *info = dev->config;
...
static void mcux_lpc_rtc_isr(const struct device *dev)
{
	...
	if ((RTC_GetStatusFlags(config->base) & kRTC_AlarmFlag) &&
	    (data->alarm_callback)) {
...
static const struct counter_driver_api mcux_lpc_rtc_driver_api = {
	...
	.get_max_relative_alarm = mcux_lpc_rtc_get_max_relative_alarm,
};

static struct mcux_lpc_rtc_data mcux_lpc_rtc_data_0;

static void mcux_lpc_rtc_irq_config_0(const struct device *dev);

static struct mcux_lpc_rtc_config mcux_lpc_rtc_config_0 = {
	.base = (RTC_Type *)DT_INST_REG_ADDR(0),
	.irq_config_func = mcux_lpc_rtc_irq_config_0,
	.info = {
...
DEVICE_AND_API_INIT(lpc_rtc, DT_INST_LABEL(0), &mcux_lpc_rtc_init,
		    &mcux_lpc_rtc_data_0, &mcux_lpc_rtc_config_0.info,
		    POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEVICE,
		    &mcux_lpc_rtc_driver_api);

static void mcux_lpc_rtc_irq_config_0(const struct device *dev)
{
	IRQ_CONNECT(DT_INST_IRQN(0),
		    DT_INST_IRQ(0, priority),
		    mcux_lpc_rtc_isr, DEVICE_GET(lpc_rtc), 0);
