...
#define COUNTER_RTS5912_REG_BASE(_dev)                                                             \
	((SLWTMR_Type *)((const struct counter_rts5912_config *const)_dev->config)->base_address)
...
{
	SLWTMR_Type *counter = COUNTER_RTS5912_REG_BASE(dev);
...
static int counter_rts5912_stop(const struct device *dev)
{
	...
	counter->INTSTS |= SLWTMR_INTSTS_STS;
...
static int counter_rts5912_init(const struct device *dev)
{
	...
	ret = clock_control_on(counter_cfg->clk_dev,
			       (clock_control_subsys_t)&counter_cfg->sccon_cfg);
	if (ret != 0) {
		return ret;
	}
#endif
