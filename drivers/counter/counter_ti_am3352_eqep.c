...
#define TI_EQEP_INT_SELI BIT(9)  /* Strobe Event */
#define TI_EQEP_INT_PCMI BIT(8)  /* Position Counter Compare */
#define TI_EQEP_INT_PCOI BIT(6)  /* Position Counter Overflow */
#define TI_EQEP_INT_PCUI BIT(5)  /* Position Counter Underflow */
#define TI_EQEP_INT_QDCI BIT(3)  /* Quadrature Direction Change */
#define TI_EQEP_INT_GLOB BIT(0)  /* Global Interrupt */
...
static bool ti_eqep_is_counting_up(const struct device *dev)
{
	...
	if (data->source == TI_EQEP_SRC_UP) {
		return true;
	} else if (data->source == TI_EQEP_SRC_DOWN) {
		return false;
	}
...
static ALWAYS_INLINE bool ti_eqep_is_late(const struct device *dev, uint32_t ticks, uint32_t now)
{
	...
	if (ti_eqep_is_counting_up(dev)) {
		window_start = now + 1;
		window_end = (now + guard) % top;

	} else {
		window_start = (now + top - guard) % top;
		window_end = now - 1;
	}

	return (ticks >= window_start && ticks <= window_end);
}
		      ...
		      const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if (chan >= TI_EQEP_ALARM_CHAN_NUM) {
		LOG_ERR("Invalid channel number %u", chan);
		return -EINVAL;
	}

	if (alarm_cfg->callback == NULL) {
		LOG_ERR("No callback provided for alarm");
		return -EINVAL;
	}

	if (data->alarm_callback[chan] != NULL) {
		LOG_ERR("An alarm for this channel is already set");
		return -EBUSY;
	}
