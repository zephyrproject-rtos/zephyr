...
static void ti_eqep_isr(const struct device *dev)
{
	...
	if (flg & (TI_EQEP_INT_PCOI | TI_EQEP_INT_PCUI)) {
		...
		void *user_data = data->alarm_user_data[chan_id];
		...
		if (cb != NULL) {
			cb(dev, chan_id, regs->QPOSCMP, user_data);
		}
		...
		if (cb != NULL) {
			cb(dev, chan_id, regs->QPOSSLAT, user_data);
		}
		...
		if (cb != NULL) {
			cb(dev, chan_id, regs->QPOSILAT, user_data);
		}
		...
		if (cb != NULL) {
			cb(dev, chan_id, regs->QPOSLAT, user_data);
		}
