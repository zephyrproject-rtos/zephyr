...
#define DEV_REGS(dev) ((struct ti_eqep_regs *)DEVICE_MMIO_GET(dev))
		      ...
		      const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if (chan_id >= TI_EQEP_ALARM_CHAN_NUM) {
	...
	if (chan_id == TI_EQEP_ALARM_CHAN_COMPARE) {
...
static void ti_eqep_isr(const struct device *dev)
{
	...
	if (flg & (TI_EQEP_INT_PCOI | TI_EQEP_INT_PCUI)) {
		...
		uint8_t chan_id = TI_EQEP_ALARM_CHAN_COMPARE;
		counter_alarm_callback_t cb = data->alarm_callback[chan_id];
		...
		if (cb != NULL) {
			cb(dev, 0, regs->QPOSCMP, data->alarm_user_data[chan_id]);
		}

		/* clear compare interrupt */
		regs->INTCLR |= TI_EQEP_INT_PCMI;
	} else if (flg & TI_EQEP_INT_SELI) {
		uint8_t chan_id = TI_EQEP_ALARM_CHAN_STROBE;
		counter_alarm_callback_t cb = data->alarm_callback[chan_id];
		...
		if (cb != NULL) {
			cb(dev, 0, regs->QPOSSLAT, data->alarm_user_data[chan_id]);
		}

		/* clear strobe interrupt */
		regs->INTCLR |= TI_EQEP_INT_SELI;
	} else if (flg & TI_EQEP_INT_IELI) {
		uint8_t chan_id = TI_EQEP_ALARM_CHAN_INDEX;
		counter_alarm_callback_t cb = data->alarm_callback[chan_id];
		...
		if (cb != NULL) {
			cb(dev, 0, regs->QPOSILAT, data->alarm_user_data[chan_id]);
		}

		/* clear index interrupt */
		regs->INTCLR |= TI_EQEP_INT_IELI;
	} else if (flg & TI_EQEP_INT_UTOI) {
		uint8_t chan_id = TI_EQEP_ALARM_CHAN_TIMEOUT;
		counter_alarm_callback_t cb = data->alarm_callback[chan_id];
		...
		if (cb != NULL) {
			cb(dev, 0, regs->QPOSLAT, data->alarm_user_data[chan_id]);
		}
...
static int z_impl_ti_eqep_configure_capture(const struct device *dev,
					    const struct ti_eqep_cap_cfg *cap_cfg)
{
	...
	if (cap_cfg->enable) {
		if (qcapctl & TI_EQEP_QCAPCTL_CEN) {
			LOG_ERR("capture unit must be disable before changing prescaler");
			return -ENOTSUP;
		}
...
static void z_impl_ti_eqep_configure_qep(const struct device *dev,
...
static void z_impl_ti_eqep_configure_decoder(const struct device *dev,
					     const struct ti_eqep_dec_cfg *dec_cfg)
{
	...
	if (dec_cfg->rising_edge_only) {
		/* rising edge only - 1x (2x for quad mode) */
		qdecctl |= TI_EQEP_QDECCTL_XCR;
	} else {
		/* both rising and falling - 2x (4x for quad mode) */
		qdecctl &= ~TI_EQEP_QDECCTL_XCR;
	}
...
static int z_impl_ti_eqep_get_latched_capture_values(const struct device *dev, uint32_t *timer,
...
#define TI_EQEP_INIT(n)                                                                            \
	...
	DEVICE_DT_INST_DEFINE(n, ti_eqep_init, NULL, &ti_eqep_data_##n, &ti_eqep_config_##n,       \
			      POST_KERNEL, CONFIG_PWM_INIT_PRIORITY, &ti_eqep_api);
