 ...
 * counter. Remaining TC channels could be used in the future to provide
 * additional functionality, e.g. input clock divider, 32-bit counter option,
 * configured via DT properties.
 *
 * Remarks:
 * - The driver is not thread safe.
 * - The driver does not implement guard periods. Use at your own risk or
 *   submit a patch.
 */
...
static int counter_sam_tc_set_alarm(const struct device *dev,
				    uint8_t chan_id,
				    const struct counter_alarm_cfg *alarm_cfg)
{
	...
	uint32_t top_value = tc_ch->TC_RC != 0 ? tc_ch->TC_RC : UINT16_MAX + 1;

	if (alarm_cfg->ticks > top_value) {
		return -EINVAL;
	}
	...
	if ((alarm_cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE) != 0) {
		...
		alarm_value = (tc_ch->TC_CV + alarm_cfg->ticks) % top_value;
	}
