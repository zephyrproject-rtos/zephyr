...
static int rv3028_counter_reset(const struct device *dev)
{
	const struct rv3028_counter_config *config = dev->config;
	int err;
...
static int rv3028_counter_set_alarm(const struct device *dev, uint8_t chan_id,
				    const struct counter_alarm_cfg *cfg)
{
	...
	if (cfg->flags & COUNTER_ALARM_CFG_EXPIRE_WHEN_LATE) {
		LOG_WRN("Cannot use COUNTER_ALARM_CFG_EXPIRE_WHEN_LATE - HW limitation");
		return -ENOTSUP;
	}

	if (!(cfg->flags & COUNTER_ALARM_CFG_ABSOLUTE)) {
		LOG_WRN("Cannot use relative alarm - HW limitation");
		return -ENOTSUP;
	}
...
static int rv3028_counter_cancel_alarm(const struct device *dev, uint8_t chan_id)
{
	struct rv3028_counter_data *data = dev->data;
...
static int rv3028_counter_set_top_value(const struct device *dev, const struct counter_top_cfg *cfg)
{
	const struct rv3028_counter_config *config = dev->config;
	struct rv3028_counter_data *data = dev->data;
	uint8_t timer[2];
...
static uint32_t rv3028_counter_get_freq(const struct device *dev)
{
	const struct rv3028_counter_config *config = dev->config;
	uint8_t control;
	uint32_t freq;
