...
{
#if defined(CONFIG_GIC)
	/* If ever build this driver on GIC-based parts, keep the original
	 * peripheral-flag-based pending behavior unless you have a reliable
	 * GIC pending query API available on that platform.
	 */
	ARG_UNUSED(irq);
	return false;
#else
...
static int mcux_lptmr_init(const struct device *dev)
{
	...
	lptmr_config.enableFreeRunning = config->freerunning;
...
#define COUNTER_MCUX_LPTMR_DEVICE_INIT(n)					\
	...
	static struct mcux_lptmr_config mcux_lptmr_config_##n = {		\
		...
		.freerunning = MCUX_LPTMR_IS_FREE_RUN(n),			\
