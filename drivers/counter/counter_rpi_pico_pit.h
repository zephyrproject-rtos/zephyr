...
/**
 * @brief Function to get whether the PIT channel has a pending interrupt
 *
 * @param dev Device pointer to the RPI Pico PIT Controller
 * @param slice Slice number of the RPI Pico Pit Channel for whicht the interrupt status is checked
 *
 * @return 1 if interrupt is pending, 0 otherwise.
 */
uint32_t counter_rpi_pico_pit_get_pending_int(const struct device *dev, uint32_t slice);

/**
 * @brief Function to allow RPI Pico PIT channels to register callbacks with their controller
 *
 * @param dev Device pointer to the RPI Pico PIT Controller
...
/**
 * @brief Function to get clock frequency from the controller
 *
 * @param dev Device pointer to the RPI Pico PIT Controller
