 ...
 *
 * @retval retruns unix time.
 */
static uint32_t decode_rtc(const struct device *dev)
{
	struct mcp7940n_data *data = dev->data;
	time_t time_unix = 0;
	struct tm time = { 0 };

	time.tm_sec = data->registers.rtc_sec.sec_one + data->registers.rtc_sec.sec_ten * 10;
	time.tm_min = data->registers.rtc_min.min_one + data->registers.rtc_min.min_ten * 10;
	time.tm_hour = data->registers.rtc_hours.hr_one + data->registers.rtc_hours.hr_ten * 10;
	time.tm_mday = data->registers.rtc_date.date_one + data->registers.rtc_date.date_ten * 10;
	time.tm_wday = data->registers.rtc_weekday.weekday;
	/* tm struct starts months at 0, mcp7940n starts at 1 */
	time.tm_mon =
		data->registers.rtc_month.month_one + data->registers.rtc_month.month_ten * 10 - 1;
	/* tm struct uses years since 1900 but unix time uses years since
	 * 1970
	 */
	time.tm_year = data->registers.rtc_year.year_one + data->registers.rtc_year.year_ten * 10 +
		UNIX_YEAR_OFFSET;
 ...
 * @param time_buffer tm struct containing time to be encoded into mcp7940n
 * registers.
 * @param id alarm ID, can be 0 or 1 for MCP7940N.
 *
 * @retval return 0 on success, or a negative error code from invalid
 * parameter.
 */
static int encode_alarm(const struct device *dev, struct tm *time_buffer, uint8_t id)
{
	struct mcp7940n_data *data = dev->data;
	uint8_t month;
 ...
 * @param dev the MCP7940N device pointer.
 * @param addr register address.
 * @param value pointer to uint8_t value that will be written to the register.
 *
 * @retval return 0 on success, or a negative error code from an I2C
...
static int write_register(const struct device *dev, enum mcp7940n_register addr, uint8_t *value)
{
	...
	const struct mcp7940n_config *cfg = dev->config;
	int rc = 0;
	uint8_t time_data[2]; /* register byte + value byte*/

	if (addr >= REG_INVAL) {
		return -EINVAL;
	}

	/* Load register address into first byte then fill in data value */
	time_data[0] = addr;
	time_data[1] = *value;
...
static int write_data_block(const struct device *dev, enum mcp7940n_register addr, uint8_t size)
{
	...
	int rc = 0;
	uint8_t byte;
	uint8_t time_data[MAX_WRITE_SIZE + 1]; /* Max block size + register byte */
...
/** @brief If the time is never set then the device defaults to 1st January
 * 1970 but with the wrong weekday set. This function ensures the weekday is
 * correct
 *
 * @param dev the MCP7940N device pointer.
...
/** @brief This interrupt handler checks the interrupt flag (IF) of a given alarm. A
 * flag that is set will be cleared and then a callback will be fired.
 *
 * @param dev the MCP7940N device pointer.
...
static int mcp7940n_counter_set_alarm(const struct device *dev, uint8_t id,
				      const struct counter_alarm_cfg *alarm_cfg)
{
	...
	encode_alarm(dev, &time_buffer, id);
	...
	data->counter_ticks[id] = current_time;
	data->alarm_user_data[id] = alarm_cfg->user_data;
...
static int mcp7940n_counter_cancel_alarm(const struct device *dev, uint8_t chan_id)
{
	struct mcp7940n_data *data = dev->data;
	int rc = 0;
 ...
 * the alarm ID.
 */
static int mcp7940n_counter_get_pending_int(const struct device *dev)
{
	struct mcp7940n_data *data = dev->data;
...
out:
	...
	if (rc == 0) {
		rc = interrupt_pending;
	}
	return rc;
}

static int mcp7940n_counter_get_top_value(const struct device *dev)
{
	return UINT32_MAX;
}
...
static int mcp7940n_init(const struct device *dev)
{
	...
	if (cfg->int_gpios.ctrl != NULL) {
		data->gpio_int = device_get_binding(cfg->int_gpios.ctrl);

		if (!data->gpio_int) {
			rc = -ENOTSUP;
			LOG_ERR("GPIO interrupt not found");
		} else {
			data->mcp7940n = dev;
			k_work_init(&data->alarm_work, mcp7940n_work_handler);

			gpio_pin_configure(data->gpio_int, cfg->int_gpios.pin,
					   GPIO_INPUT | cfg->int_gpios.flags);
			gpio_pin_interrupt_configure(data->gpio_int, cfg->int_gpios.pin,
					   GPIO_INT_EDGE_TO_ACTIVE);

			gpio_init_callback(&data->int_callback, mcp7940n_init_cb,
					   BIT(cfg->int_gpios.pin));

			gpio_add_callback(data->gpio_int, &data->int_callback);

			/* Configure interrupt polarity */
			if ((cfg->int_gpios.flags & GPIO_ACTIVE_LOW) == GPIO_ACTIVE_LOW) {
...
static const struct mcp7940n_config mcp7940n_0_config = {
	.generic = {
...
static struct mcp7940n_data mcp7940n_0_data;

DEVICE_DT_INST_DEFINE(0, mcp7940n_init, NULL,
		    &mcp7940n_0_data,
		    &mcp7940n_0_config,
		    POST_KERNEL,
