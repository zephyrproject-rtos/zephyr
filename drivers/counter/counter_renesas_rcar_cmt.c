...
static int rcar_cmt_set_value(const struct device *dev, uint32_t ticks)
{
	if (dev == NULL) {
		return -EINVAL;
	}

	return set_cmcnt(dev, ticks);
}
...
static int rcar_cmt_start(const struct device *dev)
{
	...
	/* Clear compare match flag to ensure the counter running */
	value &= ~BIT_CMF;
	sys_write32(value, cmcsr);
...
static void rcar_cmt_irq_handler(const struct rcar_cmt_irq_param *param)
{
	...
	if ((value & BIT_CMF) != 0) { /* Compare match flag */
		...
		K_SPINLOCK(&data->lock) {
			callback = data->cbs.alarm_callback_func;
			user_data = data->cbs.user_data;
			data->cbs.alarm_callback_func = NULL;
			data->cbs.user_data = NULL;
			data->cbs.is_active = false;
		}

		/* In a match event, both match flag and overflow flag are set.
		 * Clear these flags and disable CMT interrupt
		 */
		value = sys_read32(cmcsr);
		value &= ~(BIT_CMF | BIT_OVF | BIT_CMR_MASK);
		sys_write32(value, cmcsr);
	...
	/* Overflow only ISR */
	else if ((value & BIT_OVF) != 0) { /* Overflow flag */
		/* Run callback */
