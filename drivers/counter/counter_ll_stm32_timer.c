...
#if STM32_DT_INST_PARENT_DEV_DOMAIN_CLOCK_SUPPORT
#define STM32_TIM_CNT_DOMAIN_CLOCK_SUPPORT 1
#else
#define STM32_TIM_CNT_DOMAIN_CLOCK_SUPPORT 0
#endif
...
static int counter_stm32_init_timer(const struct device *dev)
{
	...
	if (IS_ENABLED(STM32_TIM_CNT_DOMAIN_CLOCK_SUPPORT) && (cfg->pclk_len > 1)) {
		...
		if (clock_control_get_rate(clk,
					(clock_control_subsys_t) &cfg->pclken[1], &tim_clk) < 0) {
			LOG_ERR("Timer clock rate get error.");
		}
	} else if (clock_control_get_rate(clk,
				   (clock_control_subsys_t) &cfg->pclken[0], &tim_clk) < 0) {
		LOG_ERR("Timer clock rate get error.");
	}
