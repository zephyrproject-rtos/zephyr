...
static void counter_esp32_isr(const void *arg);
...
static int counter_esp32_init(const struct device *dev)
{
	...
	IRQ_CONNECT(cfg->irq, IRQ_DEFAULT_PRIORITY, counter_esp32_isr, dev, ESP_INTR_FLAG_IRAM);
	irq_enable(cfg->irq);
...
static void IRAM_ATTR counter_esp32_isr(const void *arg)
{
