...
static volatile uint32_t *counter_smartbond_get_irq_reg(volatile void *timer)
{
	return (timer) == TIMER ? &((TIMER_Type *)timer)->TIMER_CLEAR_IRQ_REG :
		&((TIMER2_Type *)timer)->TIMER2_CLEAR_IRQ_REG;
}
				       ...
				       const struct counter_alarm_cfg *alarm_cfg)
{
	...
	__IOM uint32_t *timer_clear_irq_reg = counter_smartbond_get_irq_reg(timer);
...
#define COUNTER_DEVICE_INIT(idx)                                                                   \
	...
	static const struct counter_smartbond_config counter##idx##_config = {                     \
		...
		.irqn = DT_IRQN(TIMERN(idx)),                                                      \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(idx, counter_smartbond_init_timer, NULL, &counter##idx##_data,       \
