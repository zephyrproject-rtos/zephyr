...
{
	struct imx_epit_config *config = (struct imx_epit_config *)get_epit_config(dev);
...
static int imx_epit_set_top_value(const struct device *dev, const struct counter_top_cfg *cfg)
{
	EPIT_Type *base = get_epit_config(dev)->base;
	struct imx_epit_data *driver_data = dev->data;
	...
	EPIT_SetOverwriteCounter(base, !(cfg->flags & COUNTER_TOP_CFG_DONT_RESET));
