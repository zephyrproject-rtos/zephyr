...
static inline uint64_t z_vrfy_counter_us_to_ticks_64(const struct device *dev,
					      uint64_t us)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_us_to_ticks_64((const struct device *)dev,
					     (uint64_t)us);
}
#include <zephyr/syscalls/counter_us_to_ticks_64_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_us_64(const struct device *dev,
					      uint64_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_us_64((const struct device *)dev,
					     (uint64_t)ticks);
}
#include <zephyr/syscalls/counter_ticks_to_us_64_mrsh.c>

static inline uint32_t z_vrfy_counter_ns_to_ticks(const struct device *dev,
					  uint64_t ns)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ns_to_ticks((const struct device *)dev,
					  (uint64_t)ns);
}
#include <zephyr/syscalls/counter_ns_to_ticks_mrsh.c>

static inline uint64_t z_vrfy_counter_ns_to_ticks_64(const struct device *dev,
					     uint64_t ns)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ns_to_ticks_64((const struct device *)dev,
					     (uint64_t)ns);
}
#include <zephyr/syscalls/counter_ns_to_ticks_64_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_ns(const struct device *dev,
					  uint32_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_ns((const struct device *)dev,
					  (uint32_t)ticks);
}
#include <zephyr/syscalls/counter_ticks_to_ns_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_ns_64(const struct device *dev,
					     uint64_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_ns_64((const struct device *)dev,
					     (uint64_t)ticks);
}
...
static inline int z_vrfy_counter_set_value(const struct device *dev,
					   uint32_t ticks)
{
	K_OOPS(K_SYSCALL_DRIVER_COUNTER(dev, set_value));
	return z_impl_counter_set_value((const struct device *)dev,
					ticks);
}
...
static inline int z_vrfy_counter_set_value_64(const struct device *dev,
					      uint64_t ticks)
{
...
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_get_max_top_value_64((const struct device *)dev);
}
#include <zephyr/syscalls/counter_get_max_top_value_64_mrsh.c>

static inline int z_vrfy_counter_set_top_value_64(const struct device *dev,
...
static inline int z_vrfy_counter_set_channel_alarm_64(const struct device *dev,
					       uint8_t chan_id,
					       const struct counter_alarm_cfg_64 *alarm_cfg)
{
	...
	K_OOPS(K_SYSCALL_VERIFY_MSG(cfg_copy.callback == NULL,
				    "callbacks may not be set from user mode"));
	return z_impl_counter_set_channel_alarm_64((const struct device *)dev,
						   (uint8_t)chan_id,
...
#ifdef CONFIG_COUNTER_64BITS_TICKS
static inline uint64_t z_vrfy_counter_get_guard_period_64(const struct device *dev,
						    uint32_t flags)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_get_guard_period_64((const struct device *)dev,
						  flags);
}
#include <zephyr/syscalls/counter_get_guard_period_64_mrsh.c>

static inline int z_vrfy_counter_set_guard_period_64(const struct device *dev,
					      uint64_t ticks, uint32_t flags)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_set_guard_period_64((const struct device *)dev,
						  ticks, flags);
}
