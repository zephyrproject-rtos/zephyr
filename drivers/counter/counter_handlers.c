...
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_us_to_ticks_64((const struct device *)dev, (uint64_t)us);
}
#include <zephyr/syscalls/counter_us_to_ticks_64_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_us_64(const struct device *dev, uint64_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_us_64((const struct device *)dev, (uint64_t)ticks);
}
#include <zephyr/syscalls/counter_ticks_to_us_64_mrsh.c>

static inline uint32_t z_vrfy_counter_ns_to_ticks(const struct device *dev, uint64_t ns)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ns_to_ticks((const struct device *)dev, (uint64_t)ns);
}
#include <zephyr/syscalls/counter_ns_to_ticks_mrsh.c>

static inline uint64_t z_vrfy_counter_ns_to_ticks_64(const struct device *dev, uint64_t ns)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ns_to_ticks_64((const struct device *)dev, (uint64_t)ns);
}
#include <zephyr/syscalls/counter_ns_to_ticks_64_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_ns(const struct device *dev, uint32_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_ticks_to_ns((const struct device *)dev, (uint32_t)ticks);
}
#include <zephyr/syscalls/counter_ticks_to_ns_mrsh.c>

static inline uint64_t z_vrfy_counter_ticks_to_ns_64(const struct device *dev, uint64_t ticks)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
...
static inline int z_vrfy_counter_set_channel_alarm_64(const struct device *dev, uint8_t chan_id,
...
static inline int z_vrfy_counter_set_guard_period(const struct device *dev, uint32_t ticks,
						  uint32_t flags)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
...
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
	return z_impl_counter_get_guard_period_64((const struct device *)dev, flags);
}
#include <zephyr/syscalls/counter_get_guard_period_64_mrsh.c>

static inline int z_vrfy_counter_set_guard_period_64(const struct device *dev, uint64_t ticks,
						     uint32_t flags)
{
	K_OOPS(K_SYSCALL_OBJ(dev, K_OBJ_DRIVER_COUNTER));
