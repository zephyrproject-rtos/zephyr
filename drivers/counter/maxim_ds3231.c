...
static int sc_ctrl(struct device *dev,
		   ...
		   uint8_t clear)
{
	struct ds3231_data *data = dev->fixed->driver_data;
	const struct ds3231_config *cfg = dev->fixed->config_info;
...
static void alarm_worker(struct k_work *work)
{
	...
	const struct ds3231_config *cfg = ds3231->fixed->config_info;
...
int z_vrfy_maxim_ds3231_get_syncpoint(struct device *dev,
				      struct maxim_ds3231_syncpoint *syncpoint)
{
