...
static uint32_t mspm_ticks_add(uint32_t val1, uint32_t val2, uint32_t top)
{
	uint32_t to_top;
...
static uint32_t mspm_ticks_sub(uint32_t val, uint32_t old, uint32_t top)
{
	if (likely(IS_BIT_MASK(top))) {
		return (val - old) & top;
	}
				   ...
				   const struct counter_alarm_cfg *alarm_cfg)
{
	...
	if (absolute) {
		...
		val = mspm_ticks_add(now, val, top);
	}

	/* Lock interrupts: CC write, late-check, and IMASK enable must be
	 * atomic to prevent a missed alarm if the counter reaches val between
	 * the CC write and the IMASK enable.
	 */
	uint32_t key = irq_lock();

	mspm_timer_write_cc(base, chan_id, val);

	diff = mspm_ticks_sub(val - 1U, base->COUNTERREGS.CTR, top);
	if (diff > max_rel_val) {
		if (absolute) {
			err = -ETIME;
		}
