...
static int tc_counter_32bit_mode_init(tc_count32_registers_t *const p_regs,
				      uint32_t ctrla_reg_value, uint32_t evctrl_reg_value)
{
	int ret_val;
...
static int tc_counter_16bit_mode_init(tc_count16_registers_t *const p_regs,
				      uint32_t ctrla_reg_value, uint32_t evctrl_reg_value)
{
	int ret_val;
...
static int tc_counter_8bit_mode_init(tc_count8_registers_t *const p_regs, uint32_t ctrla_reg_value,
				     uint32_t evctrl_reg_value)
{
	int ret_val;
...
static int tc_counter_init(const void *tc_regs, uint32_t prescaler, const uint32_t max_bit_width)
{
	...
	case COUNTER_BIT_MODE_32: {
		...
		ret_val = tc_counter_32bit_mode_init(p_regs, ctrla_reg_value, evctrl_reg_value);
		break;
	}
	case COUNTER_BIT_MODE_16: {
		tc_count16_registers_t *p_regs = &(((tc_registers_t *)tc_regs)->COUNT16);

		ret_val = tc_counter_16bit_mode_init(p_regs, ctrla_reg_value, evctrl_reg_value);
		break;
	}
	case COUNTER_BIT_MODE_8: {
		tc_count8_registers_t *p_regs = &(((tc_registers_t *)tc_regs)->COUNT8);

		ret_val = tc_counter_8bit_mode_init(p_regs, ctrla_reg_value, evctrl_reg_value);
		break;
	}
	default:
