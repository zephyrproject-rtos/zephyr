/* counter_xec.c - Microchip XEC counter driver */
...
static int counter_xec_stop(struct device *dev)
{
	...
	counter->CTRL &= ~MCHP_BTMR_CTRL_HALT;
	counter->CTRL &= ~MCHP_BTMR_CTRL_RELOAD;
...
static void counter_xec_isr(struct device *dev)
{
	...
	if (!(MCHP_GIRQ_RESULT(counter_cfg->girq_id) & counter_cfg->girq_src)) {
		return;
	}

	MCHP_GIRQ_SRC(counter_cfg->girq_id) &= ~counter_cfg->girq_src;
...
static int counter_xec_init(struct device *dev)
{
	...
	MCHP_GIRQ_ENSET(counter_cfg->girq_id) |= counter_cfg->girq_src;
	MCHP_GIRQ_BLK_SETEN(counter_cfg->girq_id);
...
static struct counter_xec_config counter_xec_dev_config_0 = {
	...
	.girq_id = MCHP_B16TMR0_GIRQ_POS,
	.girq_src = MCHP_B16TMR0_GIRQ_VAL,
};
...
static struct counter_xec_config counter_xec_dev_config_1 = {
	...
	.girq_id = MCHP_B16TMR1_GIRQ_POS,
	.girq_src = MCHP_B16TMR1_GIRQ_VAL,
};
...
#if defined(DT_COUNTER_MCHP_XEC_2)
...
static struct counter_xec_config counter_xec_dev_config_2 = {
	.info = {
		.max_top_value = DT_COUNTEX_MCHP_XEC_2_MAX_VALUE,
		.freq = DT_COUNTER_MCHP_XEC_2_CLOCK_FREQUENCY /
			(1 << DT_COUNTER_MCHP_XEC_2_PRESCALER),
		.flags = 0,
		.channels = 1,
	},

	.config_func = counder_xec_irq_config_2,
	.base_address = DT_COUNTER_MCHP_XEC_2_BASE_ADDR,
	.prescaler = DT_COUNTER_MCHP_XEC_2_PRESCALER,
	.girq_id = MCHP_B32TMR1_GIRQ,
	.girq_src = MCHP_B32TMR1_GIRQ_VAL,
};

DEVICE_AND_API_INIT(counter_xec_2, DT_COUNTER_MCHP_XEC_2_LABEL,
...
static void counder_xec_irq_config_2(void)
{
	IRQ_CONNECT(DT_COUNTER_MCHP_XEC_2_IRQ,
		    DT_COUNTER_MCHP_XEC_2_IRQ_PRIORITY,
		    counter_xec_isr, DEVICE_GET(counter_xec_2), 0);
	irq_enable(DT_COUNTER_MCHP_XEC_2_IRQ);
}

#endif /* DT_COUNTER_MCHP_XEC_2 */
