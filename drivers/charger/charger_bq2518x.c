...
static int bq2518x_charge_enable(const struct device *dev, const bool enable)
{
	...
	ret = i2c_reg_update_byte_dt(&cfg->i2c, BQ2518X_ICHG_CTRL,
				     BQ2518X_ICHG_CHG_DIS, value);
	if (ret < 0) {
		return ret;
	}
...
static int bq2518x_set_charge_current(const struct device *dev,
...
static int bq2518x_get_charge_current(const struct device *dev,
				      uint32_t *const_charge_current_ua)
{
	const struct bq2518x_config *cfg = dev->config;
	uint8_t val;
...
static int bq2518x_get_online(const struct device *dev,
...
static int bq2518x_get_status(const struct device *dev,
...
static int bq2518x_init(const struct device *dev)
{
	...
	ret = i2c_reg_update_byte_dt(&cfg->i2c, BQ2518X_IC_CTRL,
				     BQ2518X_WATCHDOG_SEL_1_MSK,
