...
static int pf1550_set_enabled(const struct device *dev, bool enable)
{
	...
	return i2c_reg_update_byte_dt(&config->bus, CHARGER_CHG_OPER, CHG_OPER_CHG_OPER_MASK,
				      enable ? 2 : 0);
}
...
static int pf1550_led_config(const struct device *dev)
{
	...
	val = (cfg->enabled ? LED_PWM_LED_EN : 0) | (cfg->duty / 3);
	...
	val = (cfg->manual ? LED_CNFG_LEDOVRD : 0) | (cfg->behaviour ? LED_CNFG_LED_CFG : 0) |
	      cfg->frequency;
...
static int pf1550_init_properties(const struct device *dev)
{
	...
	static struct charger_pf1550_led_config led_config = {
		...
		.behaviour = ON_DURING_CHARGING_FLASH_DURING_FAULT,
	};
