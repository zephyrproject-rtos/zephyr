...
static int pf1550_led_config(const struct device *dev)
{
	...
	val = (cfg->manual ? LED_CNFG_LEDOVRD : 0) |
	      (cfg->behaviour == FLASH_DURING_CHARGING_ON_DURING_FAULT ? LED_CNFG_LED_CFG : 0) |
	      cfg->frequency;
...
#define PF1550_DEFINE(inst)                                                                        \
	static struct charger_pf1550_led_config charger_pf1550_led_config_##inst = {               \
		.frequency = DT_INST_PROP_BY_IDX(inst, pf1550_led_frequency_duty, 0),              \
		.duty = DT_INST_PROP_BY_IDX(inst, pf1550_led_frequency_duty, 1),                   \
		.behaviour = DT_INST_PROP(inst, pf1550_led_behaviour),                             \
	};                                                                                         \
