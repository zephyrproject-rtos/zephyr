...
static int bq2429x_reg_read(const struct device *dev, uint8_t reg, uint8_t *val)
{
...
static int bq2429x_reg_update(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
	const struct bq2429x_config *config = dev->config;
...
static int bq2429x_test_bit(const struct device *dev, uint8_t reg, const uint8_t bit, bool *set)
{
	...
	ret = bq2429x_reg_read(dev, reg, &value);
	if (ret < 0) {
		return ret;
	}
...
static int bq2429x_field_read(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t *value)
{
	...
	ret = bq2429x_reg_read(dev, reg, &tmp);
	if (ret < 0) {
		return ret;
	}
...
{
	return bq2429x_reg_update(dev, reg, mask, FIELD_PREP(mask, value));
}
...
static int bq2429x_identify(const struct device *dev)
{
	...
	ret = bq2429x_reg_read(dev, BQ2429X_REG_VENDOR, &reg);
	if (ret < 0) {
...
static int bq2429x_get_health(const struct device *dev, enum charger_health *health)
{
	 ...
	 */
	ret = bq2429x_reg_read(dev, BQ2429X_REG_FAULT, &fault);
	if (ret < 0) {
		return ret;
	}
