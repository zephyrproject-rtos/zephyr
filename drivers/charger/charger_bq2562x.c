...
static int bq2562x_get_ichrg_curr(const struct device *dev, uint32_t *current_ua)
{
	...
	*current_ua = ((ichg[1] << 8) | ichg[0]) >> BQ2562X_ICHG_I_MOVE_STEP;
...
static int bq2562x_set_ichrg_curr(const struct device *dev, int chrg_curr)
{
	...
	chrg_curr = ((chrg_curr / BQ2562X_ICHG_I_STEP_uA) << BQ2562X_ICHG_I_MOVE_STEP);
	...
	bq2562x_set_charge_enable(dev, 1);
...
static int bq2562x_get_chrg_volt(const struct device *dev, uint32_t *voltage_uv)
{
	...
	*voltage_uv = ((chrg_volt[1] << 8) | chrg_volt[0]) >> BQ2562X_VREG_V_MOVE_STEP;
...
static int bq2562x_set_chrg_volt(const struct device *dev, int chrg_volt)
{
	...
	chrg_volt = (chrg_volt / BQ2562X_VREG_V_STEP_uV) << BQ2562X_VREG_V_MOVE_STEP;
...
static int bq2562x_get_input_curr_lim(const struct device *dev, uint32_t *current_ua)
{
	...
	*current_ua = ((ilim[1] << 8) | ilim[0]) >> BQ2562X_IINDPM_I_MOVE_STEP;
...
static int bq2562x_set_input_curr_lim(const struct device *dev, int iindpm)
{
	...
	iindpm = (iindpm / BQ2562X_IINDPM_I_STEP_uA) << BQ2562X_IINDPM_I_MOVE_STEP;
...
static int bq2562x_get_input_volt_lim(const struct device *dev, uint32_t *voltage_uv)
{
	...
	*voltage_uv = ((vlim[1] << 8) | vlim[0]) >> BQ2562X_VINDPM_V_MOVE_STEP;
...
static int bq2562x_set_input_volt_lim(const struct device *dev, int vindpm)
{
	...
	vindpm = (vindpm / BQ2562X_VINDPM_V_STEP_uV) << BQ2562X_VINDPM_V_MOVE_STEP;
...
static int bq2562x_set_min_sys_volt(const struct device *dev, int vsysmin)
{
	...
	vsysmin = (vsysmin / BQ2562X_VSYSMIN_V_MOVE_STEP_uV) << BQ2562X_VSYSMIN_V_MOVE_STEP;
...
static int bq2562x_get_prechrg_curr(const struct device *dev, uint32_t *current_ua)
{
	...
	*current_ua = ((prechrg_curr[1] << 8) | prechrg_curr[0]) >> BQ2562X_PRECHRG_I_MOVE_STEP;
...
static int bq2562x_set_prechrg_curr(const struct device *dev, int pre_current)
{
	...
	pre_current = (pre_current / BQ2562X_PRECHRG_I_STEP_uA) << BQ2562X_PRECHRG_I_MOVE_STEP;
...
static int bq2562x_get_term_curr(const struct device *dev, uint32_t *current_ua)
{
	...
	*current_ua = ((iterm[1] << 8) | iterm[0]) >> BQ2562X_TERMCHRG_I_MOVE_STEP;
...
static int bq2562x_set_term_curr(const struct device *dev, int term_current)
{
	...
	term_current = (term_current / BQ2562X_TERMCHRG_I_STEP_uA) << BQ2562X_TERMCHRG_I_MOVE_STEP;
...
static int bq2562x_get_vbat_adc(const struct device *dev, uint32_t *vbat)
{
	...
	*vbat = ((vbat_adc[1] << 8) | vbat_adc[0]) >> BQ2562X_ADC_VBAT_MOVE_STEP;
...
static int bq2562x_get_vbus_adc(const struct device *dev, uint32_t *volt)
{
	...
	*volt = ((vbus_adc[1] << 8) | vbus_adc[0]) >> BQ2562X_ADC_VBUS_MOVE_STEP;
...
static int bq2562x_get_ibat_adc(const struct device *dev, int32_t *ibat)
{
	...
	if (temp & BIT(15)) {
		temp = ~temp + 1;
		*ibat = (temp >> BQ2562X_ADC_IBAT_MOVE_STEP) * BQ2562X_ADC_IBAT_STEP_uV * -1;
	} else {
		*ibat = (temp >> BQ2562X_ADC_IBAT_MOVE_STEP) * BQ2562X_ADC_IBAT_STEP_uV;
	}
...
static int bq2562x_get_ibus_adc(const struct device *dev, int32_t *ibus)
{
	...
	if (temp & BIT(15)) {
		temp = ~temp + 1;
		*ibus = (temp >> BQ2562X_ADC_IBUS_MOVE_STEP) * BQ2562X_ADC_CURR_STEP_uA * -1;
	} else {
		*ibus = (temp >> BQ2562X_ADC_IBUS_MOVE_STEP) * BQ2562X_ADC_CURR_STEP_uA;
	}
...
static int bq2562x_get_health(const struct device *dev, enum charger_health *health)
{
	...
	*health = CHARGER_HEALTH_UNKNOWN;
	...
	} else if (fault & (BQ2562X_OTG_FAULT_STAT |
			    BQ2562X_SYS_FAULT_STAT |
			    BQ2562X_BAT_FAULT_STAT |
			    BQ2562X_VBUS_FAULT_STAT)) {
...
#define BQ2562X_INIT(inst)                                                                         \
	...
	static struct bq2562x_data bq2562x_data_##inst = {                                         \
		.constant_charge_current_max_ua = DT_INST_PROP(inst,                               \
							constant_charge_current_max_microamp),     \
		.constant_charge_voltage_max_uv = DT_INST_PROP(inst,                               \
		...
		.thermal_regulation_threshold = DT_INST_PROP(inst,                                 \
							ti_thermal_regulation_threshold),          \
		.switching_converter_freq = DT_INST_PROP(inst, ti_switching_converter_freq),       \
		.switching_converter_strength = DT_INST_PROP(inst,                                 \
							ti_switching_converter_strength),          \
	};                                                                                         \
                                                                                                   \
