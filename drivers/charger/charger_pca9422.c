...
static int pca9422_charger_set_topoff_current(const struct device *dev, uint32_t current_ua)
{
	...
	return mfd_pca9422_reg_update_byte(config->mfd, PCA9422_REG_CHARGER_CNTL_4,
...
static int pca9422_charger_set_config(const struct device *dev)
{
	...
	return pca9422_charger_set_vsys_regulation_voltage(dev, config->vsys_reg_uv);
}
...
static void pca9422_charger_isr(const struct device *dev)
{
	...
	if (ret == 0) {
		/* Check interrupt event */
		LOG_INF("%s: int_device_mask[0]=0x%x,  [1]=0x%x\n", __func__, mask_val[0],
			mask_val[1]);
		LOG_INF("%s: int_charger_mask[0]=0x%x, [1]=0x%x, [2]=0x%x, [3]=0x%x\n", __func__,
