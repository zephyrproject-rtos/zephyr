...
LOG_MODULE_REGISTER(pca9422_charger);
...
#define PCA9422_CHAREGER_UNLOCK 0x30U
...
static int pca9422_charger_set_enabled(const struct device *dev, bool enable)
{
	...
	return mfd_pca9422_reg_update_byte(config->mfd, PCA9422_REG_CHARGER_CNTL_1,
				    ...
				    const union charger_propval *val)
{
	...
	switch (prop) {
	case CHARGER_PROP_CONSTANT_CHARGE_CURRENT_UA:
		ret = pca9422_charger_set_constant_charge_current(dev,
								  val->const_charge_current_ua);
		break;
	case CHARGER_PROP_CONSTANT_CHARGE_VOLTAGE_UV:
		ret = pca9422_charger_set_constant_charge_voltage(dev,
								  val->const_charge_voltage_uv);
		break;
	case CHARGER_PROP_INPUT_REGULATION_CURRENT_UA:
		ret = pca9422_charger_set_input_regulation_current(
			dev, val->input_current_regulation_current_ua);
		break;
	case CHARGER_PROP_CHARGE_TERM_CURRENT_UA:
		ret = pca9422_charger_set_topoff_current(dev, val->charge_term_current_ua);
		break;
	default:
		ret = -ENOTSUP;
		break;
...
static void pca9422_charger_isr(const struct device *dev)
{
	...
	if (ret == 0) {
		...
		printf("%s: int_device[0]=0x%x,  [1]=0x%x\n", __func__, int_val[0], int_val[1]);
		printf("%s: int_charger[0]=0x%x, [1]=0x%x, [2]=0x%x, [3]=0x%x\n", __func__,
	...
	if (ret == 0) {
		/* Check interrupt event */
		printf("%s: int_device_mask[0]=0x%x,  [1]=0x%x\n", __func__, mask_val[0],
		       mask_val[1]);
		printf("%s: int_charger_mask[0]=0x%x, [1]=0x%x, [2]=0x%x, [3]=0x%x\n", __func__,
...
static int pca9422_charger_init(const struct device *dev)
{
	...
	child_idx = DT_NODE_CHILD_IDX(DT_NODELABEL(charger));
	mfd_pca9422_set_irqhandler(config->mfd, dev, child_idx, pca9422_charger_isr);
...
#define CHARGER_PCA9422_DEFINE(inst)                                                               \
	...
	DEVICE_DT_INST_DEFINE(inst, &pca9422_charger_init, NULL, &charger_pca9422_data_##inst,     \
			      ...
			      CONFIG_MFD_INIT_PRIORITY, &pca9422_charger_driver_api);
