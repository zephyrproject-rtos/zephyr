...
{
	k_sem_take(&((struct rv8803_data *)dev->data)->lock, K_FOREVER);
}

static void rv8803_unlock_sem(const struct device *dev)
{
	k_sem_give(&((struct rv8803_data *)dev->data)->lock);
}
...
{
#ifndef RV8803_INT_GPIOS_IN_USE
	ARG_UNUSED(dev);
	ARG_UNUSED(id);
	ARG_UNUSED(callback);
...
#define RV8803_INIT(inst)                                                                          \
	static const struct rv8803_config rv8803_config_##inst = {                                 \
		...
		IF_ENABLED(RV8803_INT_GPIOS_IN_USE,                                                \
