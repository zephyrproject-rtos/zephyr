					 ...
					 sl_calendar_datetime_config_t *cldr)
{
	...
	cldr->MilliSeconds = tm->tm_nsec / 1000000;
}
					 ...
					 struct rtc_time *tm)
{
	...
	tm->tm_nsec = cldr->MilliSeconds * 1000000;
}

static int silabs_siwx91x_rtc_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	sl_calendar_datetime_config_t siwx91x_time = {0};
...
static int silabs_siwx91x_rtc_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	sl_calendar_datetime_config_t siwx91x_time = {0};
	struct siwx91x_rtc_data *data = dev->data;
	int ret;
...
static DEVICE_API(rtc, siwx91x_rtc_driver_api) = {
	.set_time = silabs_siwx91x_rtc_set_time,
	.get_time = silabs_siwx91x_rtc_get_time,
};
