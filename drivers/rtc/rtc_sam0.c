...
#ifdef CONFIG_RTC_ALARM
static uint32_t rtc_sam0_datetime_from_tm(const struct rtc_time *timeptr, uint32_t mask)
{
	uint32_t datetime = 0;
...
static void rtc_sam0_tm_from_datetime(struct rtc_time *timeptr, uint32_t mask,
...
static uint32_t rtc_sam0_alarm_msk_from_mask(uint32_t mask)
{
...
static uint32_t rtc_sam0_mask_from_alarm_msk(uint32_t alarm_mask)
{
	uint32_t mask = 0;
				   ...
				   const struct rtc_time *timeptr)
{
	...
	LOG_INF("S: datetime: %d, mask: %d", datetime, alarm_msk);
				   ...
				   struct rtc_time *timeptr)
{
	...
	LOG_INF("G: datetime: %d, mask: %d", datetime.reg, alarm_msk);
...
static int rtc_sam0_set_calibration(const struct device *dev, int32_t calibration)
{
	const struct rtc_sam0_config *cfg = dev->config;
	RtcMode2 *regs = &cfg->regs->MODE2;
	int32_t correction = (int32_t)((float)cfg->cal_constant
			   * (float)calibration
			   * (float)1e-9)
			   ;
...
static int rtc_sam0_get_calibration(const struct device *dev, int32_t *calibration)
{
	...
	if (correction == 0) {
		...
		*calibration = (correction * 1e9) / cfg->cal_constant;
	}
...
#define RTC_SAM0_OSC32KCTRL(n)								\
	...
	(										\
		.has_osc32kctrl = true,							\
		.osc32_src = DT_INST_ASSIGNED_CLOCKS_CELL_BY_NAME(n, osc32kctrl, src)	\
	),										\
	(										\
