				       ...
				       rtc_alarm_callback callback, void *user_data)
{
	...
	if (callback == NULL) {
		...
		i2c_reg_read_byte_dt(&config->i2c, MAX31329_REG_INT_EN, &rtc_data);
		rtc_data &= ~BIT(id ? 1 : 0);

		ret = i2c_reg_write_byte_dt(&config->i2c, MAX31329_REG_INT_EN, rtc_data);
		if (ret) {
			LOG_ERR("Error when Disabling interrupts: %i", ret);
			return ret;
		}
		...
		/* Set intA as Alarm1 interrupts: Note Table 2 in data sheet */
		rtc_data = 0x00;
		ret = i2c_reg_write_byte_dt(&config->i2c, MAX31329_REG_RTC_CONFIG2, rtc_data);
		if (ret) {
			LOG_ERR("Error when setting intB: %i", ret);
			return ret;
		}
