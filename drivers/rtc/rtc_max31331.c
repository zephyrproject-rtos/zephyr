				       ...
				       const struct rtc_time *timeptr)
{
	...
	if (id == 1) {
		...
		if ((mask & RTC_ALARM_TIME_MASK_WEEKDAY) &&
			...
			raw_time[3] = ((bin2bcd(timeptr->tm_wday) & ALARM_1_DAY_DATE_MASK) |
			...
			raw_time[3] = (((bin2bcd(timeptr->tm_mday) & ALARM_1_DAY_DATE_FIELD_MASK) &
					~ALARM_1_DAY_DATE_OP_MASK) |
		...
		if (timeptr->tm_wday >= 0 && timeptr->tm_wday <= 6) {
			...
			if (mask & RTC_ALARM_TIME_MASK_WEEKDAY) {
				raw_time[2] = ((bin2bcd(timeptr->tm_wday) & ALARM_2_DAY_DATE_MASK) |
...
#define RTC_MAX31331_DEFINE(inst)                                                                  \
	...
	static const struct rtc_max31331_config rtc_max31331_config##inst = {                      \
		...
		IF_ENABLED(CONFIG_RTC_ALARM, (.inta_gpios =	\
			GPIO_DT_SPEC_INST_GET_OR(inst, interrupt_gpios, {0}))) };    \
                                                                                                   \
