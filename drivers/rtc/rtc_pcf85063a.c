...
static int pcf85063a_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	...
	/* Build register sequence Seconds..Years (registers 04h..0Ah)
	 * Seconds register bit7 = OS flag -> ensure cleared (0) when setting time.
	 *
	 * rtc_time fields follow struct tm semantics:
	 *  - tm_sec   0-59
	 *  - tm_min   0-59
	 *  - tm_hour  0-23
	 *  - tm_mday  1-31
	 *  - tm_mon   0-11  (device expects 1-12)
	 *  - tm_year  years since 1900
	 *  - tm_wday  0-6 (Sunday=0)
	 */

	raw_time[0] = bin2bcd(timeptr->tm_sec) & 0x7F;
	raw_time[1] = bin2bcd(timeptr->tm_min);
	raw_time[2] = bin2bcd(timeptr->tm_hour);
	raw_time[3] = bin2bcd(timeptr->tm_mday);
	raw_time[4] = timeptr->tm_wday & 0x07;
	raw_time[5] = bin2bcd((timeptr->tm_mon + 1));
...
static int pcf85063a_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	...
	timeptr->tm_wday = (int)(raw_time[4] & PCF85063A_WEEKDAYS_MASK);
	timeptr->tm_mon = (int)bcd2bin(raw_time[5] & PCF85063A_MONTHS_MASK) - 1;
	timeptr->tm_year = (int)bcd2bin(raw_time[6]);
				    ...
				    struct rtc_time *timeptr)
{
	...
	*mask = 0U;
	...
	if (!(regs[0] & BIT(7))) {
		timeptr->tm_min = bcd2bin(regs[0] & PCF85063A_MINUTES_MASK);
		*mask |= RTC_ALARM_TIME_MASK_MINUTE;
	}

	if (!(regs[1] & BIT(7))) {
		timeptr->tm_hour = bcd2bin(regs[1] & PCF85063A_HOURS_MASK);
		*mask |= RTC_ALARM_TIME_MASK_HOUR;
	}

	if (!(regs[2] & BIT(7))) {
		timeptr->tm_mday = bcd2bin(regs[2] & PCF85063A_DAYS_MASK);
		*mask |= RTC_ALARM_TIME_MASK_MONTHDAY;
	}

	if (!(regs[3] & BIT(7))) {
		timeptr->tm_wday = bcd2bin(regs[3] & PCF85063A_WEEKDAYS_MASK);
...
static int pcf85063a_alarm_is_pending(const struct device *dev, uint16_t id)
{
	...
	if (reg & BIT(6)) {
		/* Clear AF by writing back Control_2 with AF bit cleared (write 0 clears the flag).
		 */
