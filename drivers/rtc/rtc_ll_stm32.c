...
static int rtc_stm32_set_calibration(const struct device *dev, int32_t calibration)
{
	...
	sys_modify_bits((mem_addr_t)&RTC->CALR, RTC_CALR_CALP | RTC_CALR_CALM, calp | calm);
...
static int rtc_stm32_get_calibration(const struct device *dev, int32_t *calibration)
{
	...
	bool calp_enabled = sys_test_bits((mem_addr_t)&calr, RTC_CALR_CALP);
	uint32_t calm = sys_test_bits((mem_addr_t)&calr, RTC_CALR_CALM);
