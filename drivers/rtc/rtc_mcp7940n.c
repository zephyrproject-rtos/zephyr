...
#define MCP7940N_YEAR_MAX    (MCP7940N_YEAR_OFFSET + 99)

/* Largest block size */
#define MAX_WRITE_SIZE (RTC_TIME_REGISTERS_SIZE)

/* Byte offsets into struct mcp7940n_time_registers / the RTC register file.
 * The MCP7940N packs each field's value into BCD nibbles alongside control
 * bits in the same byte; bitfields are not used here since their bit order
 * within a byte is implementation-defined by the C standard.
 */
#define MCP7940N_REG_SEC_OFFSET     0
#define MCP7940N_REG_MIN_OFFSET     1
#define MCP7940N_REG_HOUR_OFFSET    2
#define MCP7940N_REG_WDAY_OFFSET    3
#define MCP7940N_REG_DATE_OFFSET    4
#define MCP7940N_REG_MONTH_OFFSET   5
#define MCP7940N_REG_YEAR_OFFSET    6

#define MCP7940N_SEC_START_OSC_BIT  BIT(7)
#define MCP7940N_HOUR_12HR_BIT      BIT(6)
#define MCP7940N_WDAY_VBATEN_BIT    BIT(3)
#define MCP7940N_WDAY_OSCRUN_BIT    BIT(5)
#define MCP7940N_WDAY_MASK          GENMASK(2, 0)
...
struct mcp7940n_data {
	struct k_sem lock;
	uint8_t registers[RTC_TIME_REGISTERS_SIZE];
};
...
static void decode_rtc(const struct device *dev, struct rtc_time *timeptr)
{
	...
	uint8_t *reg = data->registers;

	timeptr->tm_sec = bcd2bin(reg[MCP7940N_REG_SEC_OFFSET] & 0x7F);
	timeptr->tm_min = bcd2bin(reg[MCP7940N_REG_MIN_OFFSET] & 0x7F);
	timeptr->tm_hour = bcd2bin(reg[MCP7940N_REG_HOUR_OFFSET] & 0x3F);
	timeptr->tm_mday = bcd2bin(reg[MCP7940N_REG_DATE_OFFSET] & 0x3F);
	timeptr->tm_wday = reg[MCP7940N_REG_WDAY_OFFSET] & MCP7940N_WDAY_MASK;
	/* rtc_time starts months at 0, mcp7940n starts at 1 */
	timeptr->tm_mon = bcd2bin(reg[MCP7940N_REG_MONTH_OFFSET] & 0x1F) - 1;
	timeptr->tm_year = bcd2bin(reg[MCP7940N_REG_YEAR_OFFSET]) + MCP7940N_YEAR_OFFSET;
}
...
static int encode_rtc(const struct device *dev, const struct rtc_time *timeptr)
{
	...
	if (timeptr->tm_year < MCP7940N_YEAR_OFFSET || timeptr->tm_year > MCP7940N_YEAR_MAX) {
		return -EINVAL;
	}
	year_since_epoch = timeptr->tm_year - MCP7940N_YEAR_OFFSET;

	reg[MCP7940N_REG_SEC_OFFSET] = MCP7940N_SEC_START_OSC_BIT | bin2bcd(timeptr->tm_sec);
	reg[MCP7940N_REG_MIN_OFFSET] = bin2bcd(timeptr->tm_min);
	reg[MCP7940N_REG_HOUR_OFFSET] = (reg[MCP7940N_REG_HOUR_OFFSET] & MCP7940N_HOUR_12HR_BIT) |
					 bin2bcd(timeptr->tm_hour);
	reg[MCP7940N_REG_WDAY_OFFSET] = (reg[MCP7940N_REG_WDAY_OFFSET] & ~MCP7940N_WDAY_MASK) |
					 (timeptr->tm_wday & MCP7940N_WDAY_MASK);
	reg[MCP7940N_REG_DATE_OFFSET] = bin2bcd(timeptr->tm_mday);
	reg[MCP7940N_REG_MONTH_OFFSET] = bin2bcd(month);
	reg[MCP7940N_REG_YEAR_OFFSET] = bin2bcd(year_since_epoch);
...
static int write_data_block(const struct device *dev, enum mcp7940n_register addr, uint8_t size)
{
	...
	if (addr == REG_RTC_SEC) {
		write_block_start = data->registers;
	} else {
		return -EINVAL;
	}
...
static int mcp7940n_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	...
	if (!(data->registers[MCP7940N_REG_WDAY_OFFSET] & MCP7940N_WDAY_OSCRUN_BIT)) {
...
static int mcp7940n_init(const struct device *dev)
{
	...
	/* Set 24-hour time */
	data->registers[MCP7940N_REG_HOUR_OFFSET] &= ~MCP7940N_HOUR_12HR_BIT;
	rc = write_register(dev, REG_RTC_HOUR, data->registers[MCP7940N_REG_HOUR_OFFSET]);
	if (rc < 0) {
		goto out;
	}

	/* Enable battery backup*/
	data->registers[MCP7940N_REG_WDAY_OFFSET] |= MCP7940N_WDAY_VBATEN_BIT;
	rc = write_register(dev, REG_RTC_WDAY, data->registers[MCP7940N_REG_WDAY_OFFSET]);
	if (rc < 0) {
		goto out;
	}
