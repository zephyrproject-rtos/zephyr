...
#define DT_DRV_COMPAT nuvoton_nct_rtc
...
#include <zephyr/drivers/misc/c2h_nct/c2h_nct.h>
#include <zephyr/kernel.h>
#include <soc.h>
#include "rtc_utils.h"

#include <zephyr/logging/log.h>
LOG_MODULE_REGISTER(rtc_nct);

/* RTC support 2000 ~ 2099 */
#define NCT_RTC_YEAR_MIN 2000
#define NCT_RTC_YEAR_MAX 2099
 ...
 * Use NCT_YEAR_GAP when dealing with the year calculation.
 */
#define NCT_YEAR_GAP 100
/* struct tm start time:   1st, Jan, 1900 */
#define TM_YEAR_REF 1900
#define DELAY_COUNT 500
#define TIMER_DELAY_COUNT 10

struct rtc_nct_config {
	struct c2h_reg *const inst_c2h;
	const struct nct_wui rtcwk;
...
static int rtc_nct_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	struct rtc_nct_data *data = dev->data;
	const struct rtc_nct_config *cfg = dev->config;
	const struct device *c2h_dev = cfg->c2h_dev;
	uint32_t real_year = timeptr->tm_year + TM_YEAR_REF;
	uint8_t i, val;

	if ((real_year < NCT_RTC_YEAR_MIN) || (real_year > NCT_RTC_YEAR_MAX)) {
	...
	/* Set year */
	rtc_write_offset(c2h_dev, RTC_YEAR, bin2bcd((timeptr->tm_year - NCT_YEAR_GAP)));

	/*Set month */
	/* tm Month = 0 - 11, Jan in NCT RTCMONTH reg starts from 1 */
	rtc_write_offset(c2h_dev, RTC_MONTH, bin2bcd(timeptr->tm_mon + 1));

	/* Set weekdays */
	/* tm Sunday = 0, Sunday in NCT RTCWEEKDAY reg starts from 1 */
...
static int rtc_nct_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	struct rtc_nct_data *data = dev->data;
	const struct rtc_nct_config *cfg = dev->config;
	const struct device *c2h_dev = cfg->c2h_dev;
	...
	/* Get year */
	timeptr->tm_year = bcd2bin(rtc_read_offset(c2h_dev, RTC_YEAR)) + NCT_YEAR_GAP;
...
#ifdef CONFIG_RTC_ALARM
static int rtc_nct_alarm_get_supported_fields(const struct device *dev, uint16_t id,
						  uint16_t *mask)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(id);
...
static int rtc_nct_alarm_set_time(const struct device *dev, uint16_t id, uint16_t mask,
				      const struct rtc_time *timeptr)
{
	...
	rtc_nct_alarm_get_supported_fields(dev, 0, &mask_capable);
...
static int rtc_nct_alarm_get_time(const struct device *dev, uint16_t id, uint16_t *mask,
...
static int rtc_nct_alarm_is_pending(const struct device *dev, uint16_t id)
{
	struct rtc_nct_data *data = dev->data;
	int ret;
...
static int rtc_nct_alarm_set_callback(const struct device *dev, uint16_t id,
...
static const struct rtc_driver_api rtc_nct_driver_api = {
	.set_time = rtc_nct_set_time,
	.get_time = rtc_nct_get_time,
#ifdef CONFIG_RTC_ALARM
	.alarm_get_supported_fields = rtc_nct_alarm_get_supported_fields,
	.alarm_set_time = rtc_nct_alarm_set_time,
	.alarm_get_time = rtc_nct_alarm_get_time,
	.alarm_is_pending = rtc_nct_alarm_is_pending,
	.alarm_set_callback = rtc_nct_alarm_set_callback,
#endif /* CONFIG_RTC_ALARM */
};

static void rtc_nct_isr(const struct device *dev, struct nct_wui *wui)
{
	const struct rtc_nct_config *cfg = dev->config;
	const struct device *c2h_dev = cfg->c2h_dev;

#ifdef CONFIG_RTC_ALARM
	struct rtc_nct_data *const data = dev->data;
#endif
	ARG_UNUSED(wui);
...
static int rtc_nct_init(const struct device *dev)
{
	struct rtc_nct_config *cfg = (struct rtc_nct_config *)dev->config;
	...
	nct_miwu_init_dev_callback(&rtc_miwu_cb, &cfg->rtcwk, rtc_nct_isr,
			dev);

	nct_miwu_manage_callback(&rtc_miwu_cb, true);

	/*
	 * Configure the rtc wake-up event
	 */
	nct_miwu_interrupt_configure(&cfg->rtcwk,
			NCT_MIWU_MODE_EDGE, NCT_MIWU_TRIG_HIGH);

	nct_miwu_irq_enable(&cfg->rtcwk);
...
#define NCT_RTC_INIT(inst)						                        \
									                        \
	static const struct rtc_nct_config rtc_nct_config_##inst = {	                        \
		.inst_c2h = (struct c2h_reg *)DT_REG_ADDR(DT_NODELABEL(c2h)),			\
		.rtcwk = NCT_DT_WUI_ITEM_BY_NAME(inst, rtc_wk),				\
		.c2h_dev = DEVICE_DT_GET(DT_NODELABEL(c2h))					\
	};								                        \
									                        \
	static struct rtc_nct_data rtc_nct_data_##inst;                                       \
												\
	DEVICE_DT_INST_DEFINE(inst,					                        \
			      rtc_nct_init,							\
			      NULL,					                        \
			      &rtc_nct_data_##inst, &rtc_nct_config_##inst,			\
			      PRE_KERNEL_2,				                        \
			      CONFIG_RTC_INIT_PRIORITY, &rtc_nct_driver_api);			\

DT_INST_FOREACH_STATUS_OKAY(NCT_RTC_INIT)
