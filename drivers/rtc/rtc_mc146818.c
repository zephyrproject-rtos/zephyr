...
#define RTC_STD_INDEX(dev)  PLATFORM_MMIO_GET(dev)
#define RTC_STD_TARGET(dev) (PLATFORM_MMIO_GET(dev) + 1)
...
#define RTC_MC146818_DEV_CFG(inst)						\
	struct rtc_mc146818_data rtc_mc146818_data##inst = {\
		PLATFORM_RAM_INIT(DT_DRV_INST(inst))\
	};			\
	static const struct  rtc_mc146818_cfg rtc_mc146818_cfg##inst = {	\
		PLATFORM_ROM_INIT(DT_DRV_INST(inst))				\
	};									\
										\
	RTC_MC146818_INIT_FN_DEFINE(inst)					\
										\
	PLATFORM_DEV_DT_INST_DEFINE(inst, rtc_mc146818_init##inst,		\
