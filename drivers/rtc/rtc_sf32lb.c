...
static int rtc_sf32lb_set_time(const struct device *dev, const struct rtc_time *timeptr)
{
	...
	tr = FIELD_PREP(RTC_DR_YT_Msk | RTC_DR_YU_Msk, bin2bcd(timeptr->tm_hour)) |
	...
	if (timeptr->tm_year < 100) { /* 20th centry */
		dr |= RTC_DR_CB;
		dr |= FIELD_PREP(RTC_DR_YT_Msk | RTC_DR_YU_Msk, bin2bcd(timeptr->tm_year)) |
		       ...
		       FIELD_PREP(RTC_DR_DT_Msk | RTC_DR_DU_Msk, timeptr->tm_mday) |
...
static int rtc_sf32lb_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	...
	if (reg & RTC_DR_CB) { /* 20th centry */
		timeptr->tm_year =
			bcd2bin(FIELD_GET(RTC_DR_YT_Msk | RTC_DR_YU_Msk, reg));
	} else {
		timeptr->tm_year =
			bcd2bin(FIELD_GET(RTC_DR_YT_Msk | RTC_DR_YU_Msk, reg)) + 100;
	}
	...
	timeptr->tm_wday = bcd2bin(FIELD_GET(RTC_DR_WD_Msk, reg));

	return 0;
}

static DEVICE_API(rtc_driver_api, rtc_sf32lb_driver_api) = {
