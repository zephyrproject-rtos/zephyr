...
#define COUNTER_RTC_ALARMS_SZ(n)    MAX(COUNTER_RTC_ALARMS_COUNT(n), 1)

#if defined(CONFIG_RTC_ALARM)

/* Per-instance static storage for alarm context, sized from DT */
#define COUNTER_RTC_DECLARE_ALARM_STORAGE(n)                                                \
	static rtc_alarm_callback counter_rtc_alarm_callback_arr_##n[COUNTER_RTC_ALARMS_SZ(n)]; \
	static void *counter_rtc_alarm_user_data_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];             \
	static uint16_t counter_rtc_alarm_mask_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];               \
	static struct rtc_time counter_rtc_alarm_time_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];        \
	static bool counter_rtc_alarm_pending_arr_##n[COUNTER_RTC_ALARMS_SZ(n)];
