...
{
	struct sgp40_data *data = dev->data;
	enum sensor_attribute_sgp40 attr_p = (enum sensor_attribute_sgp40) attr;

	if (attr_p != SENSOR_ATTR_SGP40_TEMPERATURE &&
		attr_p != SENSOR_ATTR_SGP40_HUMIDITY) {
		return -ENOTSUP;
	}
	...
	if (attr_p == SENSOR_ATTR_SGP40_HUMIDITY) {
		int16_t rh_ticks;
		int32_t tmp;
