...
struct versal_sysmon_cfg {
	...
	const struct sysmon_chan_data *chan;
...
static void sysmon_processed_to_supply_raw(int val, uint32_t reg_val, uint32_t *raw)
{
	int exponent, format;
	int32_t mantissa;
	uint32_t cfg;
...
static int sysmon_find_chan_idx_by_name(const char *config_chan_names, uint8_t *match_idx)
{
...
static int versal_sysmon_find_chan(const struct device *dev, enum sensor_channel chan)
{
	const struct versal_sysmon_cfg *cfg = dev->config;
	struct sysmon_data *data = dev->data;

	for (int i = 0; i < cfg->num_channels; i++) {
		if (data->match_idx[i] == chan) {
...
{
	struct sysmon_data *data = dev->data;
	uint32_t chan_id, mask, alarm_chan;
	int ret;
	unsigned int key;

	ret = sysmon_trigger_chan_to_id(trig, &chan_id, dev, &alarm_chan);
	if (ret) {
...
static int sysmon_supply_attr_set(const struct device *dev, enum sensor_attribute attr,
...
static void versal_sysmon_events_work_handler(struct k_work *work)
{
	...
	uint32_t events = (uint32_t)atomic_clear(&data->pending_events);

	versal_sysmon_handle_events(dev, events);
	k_work_reschedule(&data->unmask_work, K_MSEC(1));
}

static int sysmon_temp_attr_set(const struct device *dev, enum sensor_attribute attr,
				const struct sensor_value *val)
{
	uint32_t offset;
	int32_t raw, temp_val = 0, temp_mc;
	struct sysmon_data *data = dev->data;
	...
	case SENSOR_ATTR_LOWER_THRESH:
		...
		sysmon_millicelsius_to_raw(&raw, temp_mc);
		sys_write32(raw, DEVICE_MMIO_GET(dev) + offset);
		break;
				  ...
				  enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((uint32_t)chan == (uint32_t)VERSAL_SYSMON_DIE_TEMP) {
		return sysmon_temp_attr_set(dev, attr, val);
	}

	chan_num = versal_sysmon_find_chan(dev, chan);
	if (chan_num < 0) {
		return chan_num;
	}

	return sysmon_supply_attr_set(dev, attr, chan_num, val);
}

static int sysmon_supply_attr_get(const struct device *dev, enum sensor_attribute attr,
				  uint8_t chan, struct sensor_value *val)
{
	...
	case SENSOR_ATTR_ALERT:
		reg_index = chan / 32;
		offset = (reg_index * 4) + SYSMON_ALARM_REG;
		val->val1 = !!(sys_read32(DEVICE_MMIO_GET(dev) + offset) & BIT(chan % 32U));
		val->val2 = 0;
		break;
...
static int sysmon_temp_attr_get(const struct device *dev, enum sensor_attribute attr,
				struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LOWER_THRESH:
		offset = SYSMON_TEMP_LOWER_THRESH;
		raw = sys_read32(DEVICE_MMIO_GET(dev) + offset);
		sysmon_raw_to_millicelsius(raw, &temp_val);
...
static int versal_sysmon_attr_get(const struct device *dev, enum sensor_channel chan,
				  enum sensor_attribute attr, struct sensor_value *val)
{
	...
	if ((uint32_t)chan == (uint32_t)VERSAL_SYSMON_DIE_TEMP) {
		return sysmon_temp_attr_get(dev, attr, val);
	}
	...
	return sysmon_supply_attr_get(dev, attr, chan_num, val);
}
				     ...
				     struct sensor_value *val)
{
	...
	if ((uint32_t)chan == (uint32_t)VERSAL_SYSMON_DIE_TEMP) {
		...
		channel_address = versal_sysmon_find_chan(dev, chan);
...
static int versal_sysmon_init(const struct device *dev)
{
	...
	for (uint8_t i = 0U; i < cfg->num_channels; i++) {
		if (sysmon_find_chan_idx_by_name(cfg->chan[i].name, &data->match_idx[i]) != 0) {
			return -EINVAL;
		}
	}
...
#define VERSAL_SYSMON_INTR_CONFIG(inst)                                                            \
	...
	{                                                                                          \
		...
		atomic_or(&data->pending_events, pending);                                         \
...
#define VERSAL_SYSMON_INIT(inst)                                                                   \
	...
	static const struct sysmon_chan_data chans_##inst[] = {                                    \
		DT_INST_FOREACH_CHILD(inst, SUPPLY_CHAN_DATA_INIT)};                               \
                                                                                                   \
	static uint8_t match_idx_##inst[ARRAY_SIZE(chans_##inst)];                                 \
	BUILD_ASSERT(DT_INST_PROP(inst, xlnx_numchannels) == ARRAY_SIZE(chans_##inst),             \
		     "xlnx,numchannels must equal the number of channel child nodes");             \
                                                                                                   \
