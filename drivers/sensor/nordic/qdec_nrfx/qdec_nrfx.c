...
static inline void qdec_nrfx_encode_angle_q31(int32_t acc, int32_t steps,
					      struct sensor_q31_data *out)
{
	int64_t angle_q31;

	angle_q31 = (int64_t)acc * FULL_ANGLE * (INT64_C(1) << (31 - QDEC_ANGLE_SHIFT));
	angle_q31 /= steps;

	out->readings[0].angle = (q31_t)angle_q31;
}
				    ...
				    uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	case SENSOR_CHAN_ROTATION:
		...
		qdec_nrfx_encode_angle_q31(edata->acc, edata->header.steps, out);
