...
static inline void qdec_nrfx_encode_angle_q31(struct sensor_value *val, struct sensor_q31_data *out)
{
	int64_t micro, q;
...
static void qdec_nrfx_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	struct qdec_nrfx_edata *edata = NULL;
	...
	edata->header.steps = config->steps;
				    ...
				    uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	if (edata->acc != 0 && edata->header.steps > 0) {
		val.val1 = (edata->acc * FULL_ANGLE) / edata->header.steps;
		val.val2 = (edata->acc * FULL_ANGLE) - (val.val1 * edata->header.steps);
		if (val.val2 != 0) {
			val.val2 *= 1000000;
			val.val2 /= edata->header.steps;
		}
	}
	...
	case SENSOR_CHAN_ROTATION:
		...
		qdec_nrfx_encode_angle_q31(&val, out);
