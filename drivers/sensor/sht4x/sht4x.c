...
{
	struct sht4x_data *data = dev->data;
	enum sensor_attribute_sht4x attr_p = (enum sensor_attribute_sht4x) attr;

	if (attr_p != SENSOR_ATTR_SHT4X_HEATER_POWER &&
		attr_p != SENSOR_ATTR_SHT4X_HEATER_DURATION) {
		return -ENOTSUP;
	}
	...
	if (attr_p == SENSOR_ATTR_SHT4X_HEATER_DURATION) {
