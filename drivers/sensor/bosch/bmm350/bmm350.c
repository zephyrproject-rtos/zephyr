	...
	if (lsb_msb) {
		...
		if (otp_err != BMM350_OTP_STATUS_NO_ERROR) {
			...
			case BMM350_OTP_STATUS_BOOT_ERR:
				ret = BMM350_E_OTP_BOOT;
				break;
			case BMM350_OTP_STATUS_PAGE_RD_ERR:
				ret = BMM350_E_OTP_PAGE_RD;
				break;
			case BMM350_OTP_STATUS_PAGE_PRG_ERR:
				ret = BMM350_E_OTP_PAGE_PRG;
				break;
			case BMM350_OTP_STATUS_SIGN_ERR:
				ret = BMM350_E_OTP_SIGN;
				break;
			case BMM350_OTP_STATUS_INV_CMD_ERR:
				ret = BMM350_E_OTP_INV_CMD;
				break;
			default:
				ret = BMM350_E_OTP_UNDEFINED;
				break;
			}
			return ret;
		}
 ...
 */
static int8_t bmm350_get_pmu_cmd_status_0(struct bmm350_pmu_cmd_status_0 *pmu_cmd_stat_0,
 ...
 */
static int8_t set_powermode(enum bmm350_power_modes powermode, const struct device *dev)
{
	/* Variable to store the function result */
	int8_t ret = 0;
	uint8_t rx_buf[3] = {0x00};
 ...
 */
static int8_t bmm350_set_powermode(enum bmm350_power_modes powermode, const struct device *dev)
{
	/* Variable to store the function result */
	int8_t ret = 0;
	uint8_t rx_buf[3] = {0x00};
	...
	if (ret == BMM350_OK) {
		...
		if (ret == BMM350_OK) {
			...
			if (ret == BMM350_OK) {
				ret = set_powermode(powermode, dev);
			}
		}
	}
...
static int8_t bmm350_magnetic_reset_and_wait(const struct device *dev)
{
	...
	/* Read PMU CMD status */
	ret = bmm350_get_pmu_cmd_status_0(&pmu_cmd_stat_0, dev);
	...
	if ((ret == BMM350_OK) && (pmu_cmd_stat_0.pwr_mode_is_normal == BMM350_ENABLE)) {
		...
		ret = bmm350_set_powermode(BMM350_SUSPEND_MODE, dev);
	...
	if ((ret == BMM350_OK) && (restore_normal == BMM350_ENABLE)) {
		ret = bmm350_set_powermode(BMM350_NORMAL_MODE, dev);
...
static int8_t read_out_raw_data(int32_t *out_data, const struct device *dev)
{
	int8_t rslt = BMM350_OK;
	int32_t temp = 0;
	...
	if (out_data != NULL) {
		...
		if (rslt == BMM350_OK) {
			...
			out_data[0] = (
				(raw_data.raw_xdata * BMM350_LSB_TO_UT_XY_COEFF) /
				BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[1] = (
				(raw_data.raw_ydata * BMM350_LSB_TO_UT_XY_COEFF) /
				BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[2] = (
				(raw_data.raw_zdata * BMM350_LSB_TO_UT_Z_COEFF) /
				BMM350_LSB_TO_UT_COEFF_DIV);
			out_data[3] = (
				(raw_data.raw_data_t *BMM350_LSB_TO_UT_TEMP_COEFF) /
				BMM350_LSB_TO_UT_COEFF_DIV);
...
static int bmm350_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (bmm350_get_compensated_mag_xyz_temp_data_fixed(&mag_temp_data, dev) < 0) {
			   ...
			   const struct sensor_value *osr)
{
	...
	if (odr) {
		...
		if (bmm350_set_odr_performance((enum bmm350_data_rates)odr_bits, BMM350_AVERAGING_2,
					       dev) < 0) {
	...
	if (osr) {
		if (osr->val1 == 0) {
			if (bmm350_set_powermode(BMM350_SUSPEND_MODE, dev) < 0) {
				LOG_ERR("bmm350_set_powermode suspend failed");
				return -EIO;
			}

		} else {
			if (bmm350_set_powermode(BMM350_NORMAL_MODE, dev) < 0) {
				LOG_ERR("bmm350_set_powermode normal failed");
				return -EIO;
			}
		}
	}
...
#ifdef CONFIG_BMM350_TRIGGER
#define BMM350_INT_CFG(inst) .drdy_int = GPIO_DT_SPEC_INST_GET(inst, drdy_gpios),
#else
