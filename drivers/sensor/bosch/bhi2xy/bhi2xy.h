...
struct bhi2xy_data {
	...
	// current ranges for accel and gyro, obtained during init
	// used for unit conversions
	uint16_t acc_range;
	uint16_t gyro_range;
	// there are a lot of virtual sensors, we reserve data for some
	// of the most probably used ones so far, but can be extended later
	int16_t acc[3], gyro[3], mag[3];
	int16_t euler[3];      // orientation
	int16_t grv[4], rv[4]; // game rotation vector (relative orientation) and
