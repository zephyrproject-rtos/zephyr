...
int bma280_attr_set(const struct device *dev, enum sensor_channel chan, enum sensor_attribute attr,
		    const struct sensor_value *val)
{
	...
	if (attr == SENSOR_ATTR_SLOPE_TH) {
		...
		if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_SLOPE_TH, (uint8_t)slope_th) <
		    0) {
			LOG_DBG("Could not set slope threshold");
			return -EIO;
		}
	} else if (attr == SENSOR_ATTR_SLOPE_DUR) {
		if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_5, BMA280_SLOPE_DUR_MASK,
...
static void bma280_gpio_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
...
static void bma280_thread_cb(const struct device *dev)
{
	...
	if (status & BMA280_BIT_DATA_INT_STATUS && drv_data->data_ready_handler != NULL &&
	    err == 0) {
	...
	if (status & BMA280_BIT_SLOPE_INT_STATUS && drv_data->any_motion_handler != NULL &&
	    err == 0) {
...
int bma280_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
		       sensor_trigger_handler_t handler)
{
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_1, BMA280_BIT_DATA_EN,
					   0) < 0) {
		...
		if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_0, BMA280_SLOPE_EN_XYZ,
					   0) < 0) {
			LOG_DBG("Could not disable data ready interrupt");
			return -EIO;
		}
...
int bma280_init_interrupt(const struct device *dev)
{
	...
	if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_INT_RST_LATCH,
				  BMA280_BIT_INT_LATCH_RESET | BMA280_INT_MODE_LATCH) < 0) {
	...
	gpio_init_callback(&drv_data->gpio_cb, bma280_gpio_callback, BIT(config->int1_gpio.pin));
	...
	if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_MAP_1, BMA280_INT_MAP_1_BIT_DATA,
	...
	if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_MAP_0, BMA280_INT_MAP_0_BIT_SLOPE,
				   BMA280_INT_MAP_0_BIT_SLOPE) < 0) {
		LOG_DBG("Could not map any-motion interrupt pin");
		return -EIO;
	}

	if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_1, BMA280_BIT_DATA_EN, 0) < 0) {
		LOG_DBG("Could not disable data ready interrupt");
		return -EIO;
	}

	/* disable any-motion interrupt */
	if (i2c_reg_update_byte_dt(&config->i2c, BMA280_REG_INT_EN_0, BMA280_SLOPE_EN_XYZ, 0) < 0) {
		LOG_DBG("Could not disable data ready interrupt");
		return -EIO;
	}
	...
	k_thread_create(&drv_data->thread, drv_data->thread_stack, CONFIG_BMA280_THREAD_STACK_SIZE,
			bma280_thread, drv_data, NULL, NULL,
			K_PRIO_COOP(CONFIG_BMA280_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_BMA280_TRIGGER_GLOBAL_THREAD)
