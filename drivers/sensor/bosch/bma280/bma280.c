...
static int bma280_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	 ...
	 */
	if (i2c_burst_read_dt(&config->i2c, BMA280_REG_ACCEL_X_LSB, buf, 6) < 0) {
	...
	if (i2c_reg_read_byte_dt(&config->i2c, BMA280_REG_TEMP, (uint8_t *)&drv_data->temp_sample) <
	    0) {
...
static void bma280_channel_accel_convert(struct sensor_value *val, int64_t raw_val)
{
	/*
	 * accel_val = (sample * BMA280_PMU_FULL_RAGE) /
	 *             (2^data_width * 10^6)
	 */
	raw_val = (raw_val * BMA280_PMU_FULL_RANGE) / (1 << (8 + BMA280_ACCEL_LSB_BITS));
...
static int bma280_channel_get(const struct device *dev, enum sensor_channel chan,
...
int bma280_init(const struct device *dev)
{
	...
	/* read device ID */
	if (i2c_reg_read_byte_dt(&config->i2c, BMA280_REG_CHIP_ID, &id) < 0) {
	...
	if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_PMU_BW, BMA280_PMU_BW) < 0) {
		LOG_DBG("Could not set data filter bandwidth");
		return -EIO;
	}

	/* set g-range */
	if (i2c_reg_write_byte_dt(&config->i2c, BMA280_REG_PMU_RANGE, BMA280_PMU_RANGE) < 0) {
