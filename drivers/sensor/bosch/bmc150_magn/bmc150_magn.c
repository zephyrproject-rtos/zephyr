...
static const struct {
	int freq;
	uint8_t reg_val;
} bmc150_magn_samp_freq_table[] = {{2, 0x01},  {6, 0x02},  {8, 0x03},  {10, 0x00},
...
static const struct bmc150_magn_preset {
	...
	uint8_t rep_z;
	uint8_t odr;
} bmc150_magn_presets_table[] = {[LOW_POWER_PRESET] = {3, 3, 10},
				 [REGULAR_PRESET] = {9, 15, 10},
				 [ENHANCED_REGULAR_PRESET] = {15, 27, 10},
				 [HIGH_ACCURACY_PRESET] = {47, 83, 20}};

static int bmc150_magn_set_power_mode(const struct device *dev, enum bmc150_magn_power_modes mode,
				      int state)
{
	...
	case BMC150_MAGN_POWER_MODE_SUSPEND:
		if (i2c_reg_update_byte_dt(&config->i2c, BMC150_MAGN_REG_POWER,
					   BMC150_MAGN_MASK_POWER_CTL, !state) < 0) {
			return -EIO;
		}
	...
	for (i = 0U; i < ARRAY_SIZE(bmc150_magn_samp_freq_table); ++i) {
		if (val <= bmc150_magn_samp_freq_table[i].freq) {
			return i2c_reg_update_byte_dt(
				&config->i2c, BMC150_MAGN_REG_OPMODE_ODR, BMC150_MAGN_MASK_ODR,
				bmc150_magn_samp_freq_table[i].reg_val << BMC150_MAGN_SHIFT_ODR);
		}
	}
...
static int bmc150_magn_read_rep_xy(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, BMC150_MAGN_REG_REP_XY, &reg_val) < 0) {
		return -EIO;
	}
...
static int bmc150_magn_read_rep_z(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, BMC150_MAGN_REG_REP_Z, &reg_val) < 0) {
		return -EIO;
	}
...
static int bmc150_magn_compute_max_odr(const struct device *dev, int rep_xy, int rep_z,
				       int *max_odr)
{
	struct bmc150_magn_data *data = dev->data;
...
static int bmc150_magn_read_odr(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, BMC150_MAGN_REG_OPMODE_ODR, &reg_val) < 0) {
		return -EIO;
	}
...
static int bmc150_magn_write_rep_xy(const struct device *dev, int val)
{
	...
	if (i2c_reg_update_byte_dt(&config->i2c, BMC150_MAGN_REG_REP_XY,
					 ...
					 uint16_t rhall, bool is_x)
{
	...
	val = ((int16_t)(((uint16_t)((((int32_t)tregs->xyz1) << 14) / rhall)) -
			 ((uint16_t)0x4000)));
	val = ((int16_t)((((int32_t)xy) *
			  ((((((((int32_t)tregs->xy2) * ((((int32_t)val) * ((int32_t)val)) >> 7)) +
			       (((int32_t)val) * ((int32_t)(((int16_t)tregs->xy1) << 7)))) >>
			      9) +
			     ((int32_t)0x100000)) *
					...
					uint16_t rhall)
{
	...
	val = (((((int32_t)(z - tregs->z4)) << 15) -
		((((int32_t)tregs->z3) * ((int32_t)(((int16_t)rhall) - ((int16_t)tregs->xyz1)))) >>
		 2)) /
	       (tregs->z2 +
		((int16_t)(((((int32_t)tregs->z1) * ((((int16_t)rhall) << 1))) + (1 << 15)) >>
			   16))));

	return val;
}

static int bmc150_magn_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	struct bmc150_magn_data *data = dev->data;
	const struct bmc150_magn_config *config = dev->config;
	...
	if (i2c_burst_read_dt(&config->i2c, BMC150_MAGN_REG_X_L, (uint8_t *)values,
			      sizeof(values)) < 0) {
...
static int bmc150_magn_channel_get(const struct device *dev, enum sensor_channel chan,
...
#if defined(BMC150_MAGN_SET_ATTR_REP)
static inline int bmc150_magn_attr_set_rep(const struct device *dev, enum sensor_channel chan,
...
#if defined(BMC150_MAGN_SET_ATTR)
static int bmc150_magn_attr_set(const struct device *dev, enum sensor_channel chan,
...
static int bmc150_magn_init_chip(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, BMC150_MAGN_REG_CHIP_ID, &chip_id) < 0) {
	...
	if (i2c_reg_write_byte_dt(&config->i2c, BMC150_MAGN_REG_REP_XY,
				  BMC150_MAGN_REPXY_TO_REGVAL(preset.rep_xy)) < 0) {
		LOG_ERR("failed to set REP XY to %d", preset.rep_xy);
		goto err_poweroff;
	}

	if (i2c_reg_write_byte_dt(&config->i2c, BMC150_MAGN_REG_REP_Z,
	...
	if (i2c_burst_read_dt(&config->i2c, BMC150_MAGN_REG_TRIM_START, (uint8_t *)&data->tregs,
			      sizeof(data->tregs)) < 0) {
