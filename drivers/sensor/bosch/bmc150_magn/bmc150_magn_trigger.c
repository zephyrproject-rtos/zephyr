...
{
	const struct bmc150_magn_config *const cfg = dev->config;

	gpio_pin_interrupt_configure_dt(&cfg->int_gpio,
					enable ? GPIO_INT_EDGE_TO_ACTIVE : GPIO_INT_DISABLE);
}

int bmc150_magn_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
			    sensor_trigger_handler_t handler)
{
	struct bmc150_magn_data *data = dev->data;
	const struct bmc150_magn_config *const config = dev->config;
	uint8_t state;
	...
	if (trig->type == SENSOR_TRIG_DATA_READY) {
		...
		if (i2c_reg_update_byte_dt(&config->i2c, BMC150_MAGN_REG_INT_DRDY,
					   BMC150_MAGN_MASK_DRDY_EN,
					   state << BMC150_MAGN_SHIFT_DRDY_EN) < 0) {
...
static void bmc150_magn_gpio_drdy_callback(const struct device *dev, struct gpio_callback *cb,
					   uint32_t pins)
{
	struct bmc150_magn_data *data = CONTAINER_OF(cb, struct bmc150_magn_data, gpio_cb);
...
static void bmc150_magn_thread_main(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		while (i2c_reg_read_byte_dt(&config->i2c, BMC150_MAGN_REG_INT_STATUS, &reg_val) <
		       0) {
...
static int bmc150_magn_set_drdy_polarity(const struct device *dev, int state)
{
	...
	return i2c_reg_update_byte_dt(&config->i2c, BMC150_MAGN_REG_INT_DRDY,
...
int bmc150_magn_init_interrupt(const struct device *dev)
{
	...
	if (i2c_reg_update_byte_dt(&config->i2c, BMC150_MAGN_REG_INT_DRDY, BMC150_MAGN_MASK_DRDY_EN,
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_BMC150_MAGN_TRIGGER_THREAD_STACK,
			bmc150_magn_thread_main, data, NULL, NULL, K_PRIO_COOP(10), 0, K_NO_WAIT);
