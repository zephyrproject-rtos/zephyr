...
int bma4xx_reset(const struct device *dev)
{
	...
	__ASSERT(status == 0, "%s could not issue soft reset command", __func__);

	status |= bma4xx->hw_ops->read_reg(dev, BMA4XX_REG_EVENT, &value);
	__ASSERT(status == 0, "%s could not read reg event", __func__);

	if (FIELD_GET(BMA4XX_BIT_EVENT_POR_DETECTED, value) != 1) {
		LOG_ERR("Unexpected soft-reset done value");
		return -EINVAL;
	}

	status |= bma4xx->hw_ops->read_reg(dev, BMA4XX_REG_CHIP_ID, &value);
	__ASSERT(status == 0, "%s could not read chip id", __func__);

	LOG_DBG("chip_id is 0x%02x", value);

	if (value != BMA4XX_CHIP_ID_BMA422) {
...
static uint16_t bma4xx_compute_fifo_wm(const struct bma4xx_config *new_cfg)
{
	...
	bma4xx_accel_reg_to_hz(new_cfg->accel_odr, &val);
