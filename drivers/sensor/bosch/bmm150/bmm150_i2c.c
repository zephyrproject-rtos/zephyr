...
static int bmm150_reg_read_i2c(const union bmm150_bus *bus, uint8_t start, uint8_t *buf, int size)
{
	return i2c_burst_read_dt(&bus->i2c, start, buf, size);
}

static int bmm150_reg_write_i2c(const union bmm150_bus *bus, uint8_t reg, uint8_t val)
{
