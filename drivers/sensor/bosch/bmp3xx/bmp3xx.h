...
#ifndef ZEPHYR_BMP3XX_H
#define ZEPHYR_BMP3XX_H
...
typedef int (*bmp3xx_reg_read_fn)(const union bmp3xx_bus *bus, uint8_t start, uint8_t *buf,
				  int size);
typedef int (*bmp3xx_reg_write_fn)(const union bmp3xx_bus *bus, uint8_t reg, uint8_t val);
...
#ifdef BMP3XX_USE_SPI_BUS
#define BMP3XX_SPI_OPERATION (SPI_WORD_SET(8) | SPI_TRANSFER_MSB | SPI_MODE_CPOL | SPI_MODE_CPHA)
...
#define BMP3XX_PWR_CTRL_ON								    \
	(BMP3XX_PWR_CTRL_PRESS_EN | BMP3XX_PWR_CTRL_TEMP_EN | BMP3XX_PWR_CTRL_MODE_NORMAL)
