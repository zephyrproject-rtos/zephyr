					       ...
					       struct bma400_runtime_config *cfg)
{
	...
	if (cfg->fifo_wm_en || cfg->fifo_full_en) {
		res = dev_data->hw_ops->write_reg(dev, BMA400_REG_CMD, BMA400_FIFO_FLUSH_CMD);
		if (res) {
			LOG_ERR("%s could not flush the FIFO", __func__);
			return res;
		}
...
int bma400_stream_configure(const struct device *dev, struct bma400_runtime_config *cfg)
{
	...
	res = dev_data->hw_ops->write_reg(dev, BMA400_REG_INT1_MAP,
	...
	/* Set motion detection duration */
	res = dev_data->hw_ops->write_reg(dev, BMA400_REG_GEN2INT_CONFIG3,
					  (cfg->motion_duration >> 8) & 0xFF);
	if (res) {
		LOG_ERR("%s could not write BMA400_REG_GEN2INT_CONFIG3", __func__);
		return res;
	}
	res = dev_data->hw_ops->write_reg(dev, BMA400_REG_GEN2INT_CONFIG31,
					  cfg->motion_duration & 0xFF);
	if (res) {
...
static int bma400_attr_set_motion_duration(const struct sensor_value *val,
					   struct bma400_runtime_config *new_config)
{
	if (val->val2 != 0 || val->val1 > 65535) {
		return -ERANGE;
	}
	new_config->motion_duration = (uint16_t)val->val1;
	return 0;
}
					    ...
					    struct bma400_runtime_config *new_config)
{
	...
	ms2 = sensor_value_to_float(val);
	reg_float = ms2 * 1000.0f / (9.80665f * 8.0f);
	if (reg_float > 255.0f || reg_float < 0.0f) {
		return -ERANGE;
	}
	new_config->motion_threshold = (uint8_t)(reg_float + 0.5f);
	return 0;
}
#endif /* CONFIG_BMA400_STREAM */
				 ...
				 struct bma400_runtime_config *new_config)
{
	...
	status = bma400_fs_to_reg(sensor_ms2_to_g(val) * 1000000, &reg_val);
	if (status < 0) {
		return status;
	}
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			...
			if (val->val1 < 0 || val->val2 < 0) {
				LOG_ERR("Not valid threshold value");
				return -EINVAL;
			}
			   ...
			   enum sensor_attribute attr, struct sensor_value *val)
{
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			...
			uint32_t range_ug = fs_to_reg_map[(reg_val & BMA400_ACCEL_RANGE_MSK) >>
							  BMA400_ACCEL_RANGE_POS];
			sensor_g_to_ms2(range_ug / 1000000, val);
		} else if (attr == SENSOR_ATTR_CONFIGURATION) {
...
#define BMA400_SPI_OPERATION (SPI_WORD_SET(8) | SPI_TRANSFER_MSB | SPI_OP_MODE_CONTROLLER)
...
#define BMA400_CFG_STREAM(inst)                                                                    \
	...
	.motion_detection_en = false, .motion_threshold = DT_INST_PROP(inst, motion_threshold),    \
...
#define BMA400_INIT(inst)                                                                          \
	...
	static const struct bma400_config bma400_config_##inst = {                                 \
		BMA400_DEFINE_BUS(inst)                                                            \
			IF_ENABLED(CONFIG_BMA400_STREAM, ( \
