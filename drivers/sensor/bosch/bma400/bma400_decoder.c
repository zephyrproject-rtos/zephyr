					  ...
					  uint16_t *frame_count)
{
	...
	while (buffer < end) {
		...
		const bool is_empty = (buffer[0] == BMA400_FIFO_EMPTY_FRAME);
		const bool is_sensortime = (buffer[0] == BMA400_FIFO_SENSOR_TIME);
		const bool is_ctrl = (buffer[0] == BMA400_FIFO_CONTROL_FRAME);
		const bool has_accel = (buffer[0] & BMA400_FIFO_DATA_EN_MASK) != 0;

		if (is_empty) {
			size += BMA400_FIFO_CF_LENGTH;
		} else if (is_sensortime) {
			size += BMA400_FIFO_ST_LENGTH;
		} else if (is_ctrl) {
			size += BMA400_FIFO_CF_LENGTH;
		} else if (has_accel) {
...
static void bma400_convert_raw_accel_to_q31(int16_t raw_val, q31_t *out)
{
	...
	int16_t value = (int16_t)(raw_val << 4) >> 4;
			      ...
			      uint16_t max_count, void *data_out)
{
	...
	const uint8_t *buffer_end =
		buffer + sizeof(struct bma400_fifo_data) + 1 + edata->fifo_count;
	...
	while (accel_frame_count < max_count && buffer < buffer_end) {
		...
		if (is_empty) {
			...
			frame_end += BMA400_FIFO_ST_LENGTH + BMA400_FIFO_HEADER_LENGTH;
		} else if (is_ctrl) {
			frame_end += BMA400_FIFO_CF_LENGTH + BMA400_FIFO_HEADER_LENGTH;
		} else if (has_accel) {
