...
static void bme680_set_mem_page_cb(struct rtio *ctx, const struct rtio_sqe *sqe, void *arg0)
{
	...
	uint8_t reg_addr_wr = BME680_REG_STATUS & BME680_SPI_WRITE_MSK;
								...
								uint8_t addr)
{
	...
	uint8_t reg_addr = BME680_REG_STATUS | BME680_SPI_READ_BIT;
	rtio_sqe_prep_tiny_write(write_reg_sqe, iodev, RTIO_PRIO_NORM, &reg_addr, 1, NULL);
	write_reg_sqe->flags |= RTIO_SQE_TRANSACTION;

	rtio_sqe_prep_read(read_status_sqe, iodev, RTIO_PRIO_NORM, &data->status, 1, (void *)dev);
