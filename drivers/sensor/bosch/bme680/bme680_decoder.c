...
LOG_MODULE_REGISTER(BME680_DECODER, CONFIG_SENSOR_LOG_LEVEL);
								...
								struct bme680_compensated_data *data)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		adc_temp = sys_get_be24(&raw_data->buf[4]) >> 4;
	...
	case SENSOR_CHAN_PRESS:
		...
		adc_temp = sys_get_be24(&raw_data->buf[4]) >> 4;
		bme680_calc_temp(data, comp_param, &t_fine, adc_temp);
		adc_press = sys_get_be24(&raw_data->buf[1]) >> 4;
	...
	case SENSOR_CHAN_HUMIDITY:
		...
		adc_temp = sys_get_be24(&raw_data->buf[4]) >> 4;
		bme680_calc_temp(data, comp_param, &t_fine, adc_temp);
		adc_hum = sys_get_be16(&raw_data->buf[7]);
				 ...
				 uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP: {
		...
		readq = result.temp * pow(2, 31 - BME680_TEMP_SHIFT);
		convq = BME680_TEMP_CONV * pow(2, 31 - BME680_TEMP_SHIFT);
	...
	case SENSOR_CHAN_PRESS: {
		...
		readq = result.press;
		convq = BME680_PRESS_CONV_KPA * pow(2, 31 - BME680_PRESS_SHIFT);
