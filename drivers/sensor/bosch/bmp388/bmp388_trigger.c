...
static K_THREAD_STACK_DEFINE(bmp388_thread_stack, CONFIG_BMP388_THREAD_STACK_SIZE);
...
{
	struct bmp388_data *data = CONTAINER_OF(work, struct bmp388_data, work);

	bmp388_handle_interrupts(data->dev);
}
#endif

static void bmp388_gpio_callback(const struct device *port, struct gpio_callback *cb, uint32_t pin)
{
	struct bmp388_data *data = CONTAINER_OF(cb, struct bmp388_data, gpio_cb);
		       ...
		       sensor_trigger_handler_t handler)
{
	...
	if (bmp388_reg_field_update(dev, BMP388_REG_INT_CTRL, BMP388_INT_CTRL_DRDY_EN_MASK,
...
int bmp388_trigger_mode_init(const struct device *dev)
{
	...
	k_thread_create(&bmp388_thread, bmp388_thread_stack, CONFIG_BMP388_THREAD_STACK_SIZE,
			bmp388_thread_main, (void *)dev, NULL, NULL,
			K_PRIO_COOP(CONFIG_BMP388_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_BMP388_TRIGGER_GLOBAL_THREAD)
	...
	gpio_init_callback(&data->gpio_cb, bmp388_gpio_callback, BIT(cfg->gpio_int.pin));
