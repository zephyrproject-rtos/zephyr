				...
				uint8_t reg_addr, int16_t *val)
{
	int ret;
	uint8_t i2c_data[2];
	const struct bq34110_config *config = dev->config;

	ret = i2c_write_read_dt(&config->i2c, &reg_addr, sizeof(reg_addr),
				&i2c_data, 2);
	if (ret < 0) {
			       ...
			       struct sensor_value *val)
{
	...
	case SENSOR_CHAN_GAUGE_TEMP:
		...
		int_temp = (data->internal_temperature * 0.1);
		int_temp = int_temp - ZERO_DEG_CELSIUS_IN_KELVIN;
				 ...
				 uint16_t *val, uint8_t read_addr)
{
	...
	ret = i2c_write_read_dt(&config->i2c, &read_addr, sizeof(read_addr),
				&read_data, sizeof(read_data));
	if (ret < 0) {
		LOG_ERR("Failed to write read");
		return ret;
	}
			       ...
			       uint8_t data[], uint8_t len)
{
	int ret;
	uint16_t datasum;
	uint8_t i, df_addr_data[3],
		datasum_len[3], df_write_data[len + 1];
	const struct bq34110_config *config = dev->config;

	df_addr_data[0] = MANUFACTURER_ACCESS_CONTROL;
	df_addr_data[1] = (uint8_t)(df_addr & 0x00FF);
	df_addr_data[2] = (uint8_t)((df_addr >> 8) & 0x00FF);

	ret = i2c_write_dt(&config->i2c, df_addr_data, sizeof(df_addr_data));
	if (ret < 0) {
		LOG_ERR("Failed to write MAC");
		return ret;
	}

	df_write_data[0] = MAC_DATA;
	for (i = 1; i <= len; i++) {
		df_write_data[i] = data[i-1];
	}

	ret = i2c_write_dt(&config->i2c, df_write_data, sizeof(df_write_data));
	if (ret < 0) {
		LOG_ERR("Failed to write MAC");
		return ret;
	}

	datasum = df_addr_data[1] + df_addr_data[2];
	for (i = 1; i <= len; i++) {
		datasum += df_write_data[i];
	}

	datasum_len[0] = MAC_DATA_SUM;
	datasum_len[1] = (uint8_t)~datasum;
	datasum_len[2] = 4 + len;
	ret = i2c_write_dt(&config->i2c, datasum_len, sizeof(datasum_len));
	if (ret < 0) {
...
#define BQ34110_INIT(index)						\
	...
	static const struct bq34110_config bq34110_config_##index = {	\
		...
		.design_capacity = DT_INST_PROP(index, design_capacity),\
									...
									\
	SENSOR_DEVICE_DT_INST_DEFINE(index, bq34110_gauge_init,		\
			      NULL,					\
