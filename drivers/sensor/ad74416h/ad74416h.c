...
static int ad74416h_access(const struct device *dev, uint8_t reg, uint16_t val_in, uint16_t *val_out) {
...
static int ad74416h_gpio_config(const struct device *dev, gpio_pin_t pin, gpio_flags_t flags) {
    if (flags & GPIO_OUTPUT) {
        ...
        return ad74416h_access(dev, AD74416H_REG_DO_EXT_CONFIG(pin), 0x0001, NULL); // Sourcing
    } else {
        return ad74416h_access(dev, AD74416H_REG_CH_FUNC_SETUP(pin), 0x0008, NULL); // Digital In
    }
}
...
static int ad74416h_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) {
    ...
    if (chan == SENSOR_CHAN_AMBIENT_TEMP) {
        ...
        double celsius = (-3.9083e-3 + sqrt(pow(3.9083e-3, 2) - 4 * -5.775e-7 * (1 - R / 1000.0))) / (2 * -5.775e-7);
...
static int ad74416h_init(const struct device *dev) {
    ...
    if (config->adc_rdy_gpio.port) {
        ...
        gpio_init_callback(&data->adc_rdy_cb, ad74416h_rdy_handler, BIT(config->adc_rdy_gpio.pin));
...
#define AD74416H_DEFINE(n) \
    ...
    DEVICE_DT_INST_DEFINE(n, ad74416h_init, NULL, &ad74416h_data_##n, \
                         ...
                         CONFIG_SENSOR_INIT_PRIORITY, &ad74416h_sensor_api);
