...
static int bmi088_sample_fetch(const struct device *dev, enum sensor_channel chan) {
    ...
    uint8_t status = 0;
    while ((status & BMI088_STATUS_MASK) == 0) {

        if (bmi088_byte_read(dev, BMI088_ACC_STATUS, &status) < 0) {
            return -EIO;
        }
    }

    if (bmi088_read(dev, RATE_X_LSB, data->sample.acc, BMI088_SAMPLE_SIZE) < 0) {
        return -EIO;
    }
...
static int bmi088_channel_get(const struct device *dev, enum sensor_channel chan, struct sensor_value *val) {
    ...
    if (bmi088_byte_read(dev, ACC_RANGE, &range_value) < 1) {
        return -EIO;
    }
    ...
    switch (chan) {
        ...
        case SENSOR_CHAN_ACCEL_Z: {
            struct bmi088_data *data = to_data(dev);
            *val = bmi088_channel_convert(chan, scale, data->sample.acc);
            return 0;
        }
        case SENSOR_CHAN_ACCEL_XYZ: {
            struct bmi088_data *data = to_data(dev);
            val[0] = bmi088_channel_convert(SENSOR_CHAN_ACCEL_X, scale, data->sample.acc);
            val[1] = bmi088_channel_convert(SENSOR_CHAN_ACCEL_Y, scale, data->sample.acc);
            val[2] = bmi088_channel_convert(SENSOR_CHAN_ACCEL_Z, scale, data->sample.acc);
            return 0;
        }
        default:
...
static int bmi088_init(const struct device *dev) {
    ...
    if (bmi088_reg_field_update(dev, ACC_RANGE,0, BIT(0) | BIT(1) ,BMI088_DEFAULT_RANGE) < 0) {
        LOG_DBG("Cannot set default range for accelerometer.");
        return -EIO;
    }

    // Switch acc to normal mode
    if (bmi088_byte_write(dev, ACC_PWR_CTRL, ACC_NORMAL_MODE) < 0) {
