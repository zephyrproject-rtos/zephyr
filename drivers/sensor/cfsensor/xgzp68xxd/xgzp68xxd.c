...
/*
 * Copyright (c) 2026 Kristian Angelov
 *
				 ...
				 struct sensor_value *val)
{
	...
	case SENSOR_CHAN_PRESS:
		sensor_value_from_micro(val,
					((int64_t)(data->raw_pressure) * 1000) / config->k_value);
		return 0;
	case SENSOR_CHAN_AMBIENT_TEMP:
		sensor_value_from_milli(val, (data->raw_temperature * 1000) / XGZP68XXD_TEMP_SCALE);
		return 0;
	default:
		return -ENOTSUP;
	}
...
#define XGZP68XXD_DEFINE(inst)                                                                     \
	static struct xgzp68xxd_data xgzp68xxd_data##inst;                                         \
                                                                                                   \
