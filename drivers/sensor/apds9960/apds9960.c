...
{
	const struct apds9960_config *config = dev->fixed->config_info;
	struct apds9960_data *data = dev->fixed->driver_data;
	uint8_t tmp;
...
{
	const struct apds9960_config *config = dev->fixed->config_info;
	struct apds9960_data *drv_data = dev->fixed->driver_data;
