...
struct pcnt_esp32_config {
	...
	/* Not trigger-gated: the ISR always runs the software accumulator. */
...
/*
 * Read a unit's wide count: the 16-bit hardware register plus the software
 * accumulator maintained by the ISR on every high/low limit wrap.
 *
 * The count register and the interrupt status live in different registers, so
 * they cannot be sampled in a single instruction. A wrap that happens after
 * the count is read but before the ISR has run would otherwise be missed for
 * one limit period. Detect that pending event here and compensate.
 *
 * The event may also be raised between the two reads below, in which case the
 * register value already belongs to the new period and must not be adjusted.
 * Distinguish the two by requiring the count to still be on the far side of
 * half the limit, which holds as long as the counter does not wrap twice
 * between the reads.
 */
static int32_t pcnt_esp32_read_count(const struct device *dev,
				     struct pcnt_esp32_unit_config *unit_config)
{
	...
	raw = pcnt_ll_get_count(data->hal.dev, u);

	if (pcnt_ll_get_intr_status(data->hal.dev) & BIT(u)) {
		uint32_t unit_status = pcnt_ll_get_unit_status(data->hal.dev, u);

		if ((unit_status & PCNT_INTR_L_LIM) && raw >= unit_config->l_limit_eff / 2) {
			raw += unit_config->l_limit_eff;
		} else if ((unit_status & PCNT_INTR_H_LIM) && raw <= unit_config->h_limit_eff / 2) {
			raw += unit_config->h_limit_eff;
		}
	}

	raw += unit_config->accum;
...
static int pcnt_esp32_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	for (uint8_t i = 0; i < config->unit_len; i++) {
		...
		unit_config->count_val_acc = pcnt_esp32_read_count(dev, unit_config);
	}
...
static void pcnt_esp32_isr(const struct device *dev);
...
int pcnt_esp32_init(const struct device *dev)
{
	...
	for (uint8_t i = 0; i < config->unit_len; i++) {
		...
		unit_config->h_limit_eff =
			unit_config->h_limit_set ? unit_config->h_limit : INT16_MAX;
		unit_config->l_limit_eff =
			unit_config->l_limit_set ? unit_config->l_limit : INT16_MIN;
		 ...
		 */
		pcnt_ll_set_high_limit_value(data->hal.dev, u, unit_config->h_limit_eff);
		pcnt_ll_set_low_limit_value(data->hal.dev, u, unit_config->l_limit_eff);

		/*
		 * The limit events drive the software accumulator that extends the
		 * 16-bit counter, so enable them unconditionally rather than only
		 * when a DT limit is present.
		 */
	...
	/*
	 * The interrupt is always required: it maintains the software accumulator
	 * that extends the 16-bit hardware counter, independently of whether any
	 * threshold trigger is registered.
	 */
	/* Pre-multilevel path, kept for reference:
	 *
...
static void IRAM_ATTR pcnt_esp32_isr(const struct device *dev)
{
	...
	for (uint8_t i = 0; i < config->unit_len; i++) {
		...
		/*
		 * The counter is clamped back to zero on a limit event, so carry
		 * the limit into the accumulator to keep the reported count
		 * monotonic across the 16-bit wrap.
		 */
		if (unit_status & PCNT_INTR_L_LIM) {
			unit_config->accum += unit_config->l_limit_eff;
		} else if (unit_status & PCNT_INTR_H_LIM) {
			unit_config->accum += unit_config->h_limit_eff;
		}
				  ...
				  sensor_trigger_handler_t handler)
{
	...
	if (trig->chan != SENSOR_CHAN_ROTATION && trig->chan != SENSOR_CHAN_ENCODER_COUNT &&
...
static void pcnt_esp32_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	for (uint8_t i = 0; i < config->unit_len; i++) {
		struct pcnt_esp32_unit_config *unit_config = &config->unit_config[i];
		int32_t raw = pcnt_esp32_read_count(dev, unit_config);
...
#define UNIT_CONFIG(node_id)                                                                       \
	{                                                                                          \
		...
		.filter = CLAMP(DT_PROP_OR(node_id, filter, 0), 0, 1023),                          \
