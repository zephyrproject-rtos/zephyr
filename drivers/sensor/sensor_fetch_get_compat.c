...
RTIO_DEFINE(__sensor_fetch_get_compat_rtio, 8, 8);
...
int sensor_sample_fetch_compat(const struct device *dev, enum sensor_channel chan)
{
			      ...
			      struct sensor_value *val)
{
	...
	float valf = Z_SHIFT_Q31_TO_F32(data->readings[0].value, data->shift);
	sensor_value_from_float(val, valf);
