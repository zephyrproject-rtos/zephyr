...
static int mcp9843_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	buf[0] &= 0x1F; /* mask out flags */

	if ((buf[0] & 0x10) == 0x10) {
		/* negative temperature */
		buf[0] &= 0x0F;
		data->temp = ((buf[0] * 16 * 16) + buf[1]) - (256 * 16);
	} else {
		/* positive temperature */
		data->temp = (buf[0] * 16 * 16) + buf[1];
	}
			       ...
			       struct sensor_value *val)
{
	...
	if (data->temp == INT32_MIN) {
		return -EINVAL;
	}

	val->val1 = data->temp / 16;
	val->val2 = (data->temp * 1000000 / 16) % 1000000;
