...
#define TACH_CTRL_EDGES  (CONFIG_TACH_XEC_EDGES << MCHP_TACH_CTRL_NUM_EDGES_POS)
...
int tach_xec_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	while (poll_count < PIN_STS_TIMEOUT) {
		...
		if (tach->STATUS & MCHP_TACH_STS_CNT_RDY) {
			data->count = tach->CONTROL >> MCHP_TACH_CTRL_COUNTER_POS;
			break;
		}
...
static void tach_xec_sleep_clr(const struct device *dev)
{
	...
	struct pcr_regs *const pcr =
		(struct pcr_regs *const)(DT_REG_ADDR_BY_IDX(DT_NODELABEL(pcr), 0));
...
static int tach_xec_init(const struct device *dev)
{
	...
	tach->CONTROL = MCHP_TACH_CTRL_READ_MODE_100K_CLOCK | TACH_CTRL_EDGES |
