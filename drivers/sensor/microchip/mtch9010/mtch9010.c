...
static int mtch9010_init(const struct device *dev)
{
	...
	if (config->uart_init) {
		...
		if (config->mode == MTCH9010_CAPACITIVE) {
			mtch9010_command_send(dev, MTCH9010_CMD_STR_CAPACITIVE_MODE);
		} else if (config->mode == MTCH9010_CONDUCTIVE) {
			mtch9010_command_send(dev, MTCH9010_CMD_STR_CONDUCTIVE_MODE);
		} else {
		...
		mtch9010_command_send(dev, temp_buffer);

		/* Extended Mode Enable */
		if (config->extended_mode_enable) {
			/* Extended Mode */
			mtch9010_command_send(dev, MTCH9010_CMD_STR_EXTENDED_MODE_EN);
			LOG_INST_DBG(config->log, "Extended mode is enabled");

			if (config->format == MTCH9010_OUTPUT_FORMAT_DELTA) {
				mtch9010_command_send(dev, MTCH9010_CMD_STR_EXTENDED_FORMAT_DELTA);
			} else if (config->format == MTCH9010_OUTPUT_FORMAT_CURRENT) {
				mtch9010_command_send(dev,
						      MTCH9010_CMD_STR_EXTENDED_FORMAT_CURRENT);
			} else if (config->format == MTCH9010_OUTPUT_FORMAT_BOTH) {
				mtch9010_command_send(dev, MTCH9010_CMD_STR_EXTENDED_FORMAT_BOTH);
			} else if (config->format == MTCH9010_OUTPUT_FORMAT_MPLAB_DATA_VISUALIZER) {
				mtch9010_command_send(dev,
						      MTCH9010_CMD_STR_EXTENDED_FORMAT_MPLAB_DV);
			} else {
			...
			mtch9010_command_send(dev, MTCH9010_CMD_STR_EXTENDED_MODE_DIS);
		...
		if (config->ref_mode == MTCH9010_REFERENCE_CURRENT_VALUE) {
			...
			/* No Averaging */
			mtch9010_command_send(dev, MTCH9010_CMD_STR_REF_MODE_CURRENT_VALUE);
			...
			while (samplingCount < CONFIG_MTCH9010_REFERENCE_AVERAGING_COUNT) {
				mtch9010_command_send(dev, MTCH9010_CMD_STR_REF_MODE_REPEAT_MEAS);

				bool fail_safe_cmd = false;
			...
			if (!fail_safe_cmd) {
				...
				if (calc_ref_val > MTCH9010_MAX_RESULT) {
					LOG_INST_ERR(config->log,
						     "Computed reference out of range");
					return -EIO;
				}
				...
				LOG_INST_DBG(config->log, "Average reference value = %u",
					     calc_ref_val);

				/* Set the average value as a custom value */
				mtch9010_command_send(dev, MTCH9010_CMD_STR_REF_MODE_CUSTOM);

				/* Send the computed reference */
				snprintf(temp_buffer, MTCH9010_INTERNAL_BUFFER_SIZE, "%u",
					 data->reference);
				mtch9010_command_send(dev, temp_buffer);
			} else {
				/* A communication error occurred during averaging */
				LOG_INST_ERR(config->log,
					     "Communication error during averaging - using "
					     "single sample fallback");
				mtch9010_command_send(dev, MTCH9010_CMD_STR_REF_MODE_CURRENT_VALUE);

				/* Set to 0, since we can't trust the results */
				data->reference = 0;
			}
#endif
...
static int mtch9010_command_send(const struct device *dev, const char *str)
{
	...
	if (mtch9010_timeout_receive(dev, &temp_buffer[0], sizeof(temp_buffer),
				     CONFIG_MTCH9010_UART_COMMAND_TIMEOUT_MS) == 0) {
	...
	if (gpio_is_ready_dt(&config->heartbeat_gpio)) {
		...
		if (rtn == 0) {
			...
			if (rtn == 0) {
				...
				if (rtn < 0) {
					LOG_INST_ERR(config->log,
						     "Unable to configure interrupt; code %d", rtn);
				} else {
			...
			LOG_INST_ERR(config->log, "Unable to enable interrupt code %d", rtn);
		}
#endif
	} else {
...
static int mtch9010_device_reset(const struct device *dev)
{
	...
	k_msleep(CONFIG_MTCH9010_RESET_TIME_MS);
				...
				struct mtch9010_result_t *result)
{
	...
	case MTCH9010_OUTPUT_FORMAT_CURRENT:
		/* Data: <val>\n\r */
		temp = strtol(buffer, &endOfString, 10);
	...
	case MTCH9010_OUTPUT_FORMAT_BOTH:
		...
		if (*endOfString == ' ') {
			...
			temp = strtol(endOfString, &endOfString, 10);
...
static int mtch9010_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_MTCH9010_MEAS_DELTA: {
		...
		if (config->sleep_time != 0) {
			...
			k_msleep(CONFIG_MTCH9010_WAKE_PULSE_WIDTH_MS);
		...
		for (uint8_t i = 0; i < MTCH9010_INTERNAL_BUFFER_SIZE; i++) {
			temp_buffer[i] = '\0';
		}
...
#ifdef CONFIG_MTCH9010_HEARTBEAT_MONITORING_ENABLE
		if (k_sem_take(&data->heartbeat_sem,
			       K_MSEC(CONFIG_MTCH9010_UART_COMMAND_TIMEOUT_MS)) < 0) {
			return -EBUSY;
		}

		/* Compute the last access time */
		int64_t timeDiff = k_uptime_delta(&data->last_heartbeat);

		k_sem_give(&data->heartbeat_sem);

		if (timeDiff < CONFIG_MTCH9010_ERROR_PERIOD_MS) {
...
#define MTCH9010_DEFINE(inst)                                                                      \
	...
	static const struct mtch9010_config_t mtch9010_config_##inst = {                           \
		...
		.uart_dev = DEVICE_DT_GET_OR_NULL(DT_PARENT(DT_DRV_INST(inst))),                   \
