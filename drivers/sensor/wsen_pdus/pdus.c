...
LOG_MODULE_REGISTER(PDUS, CONFIG_SENSOR_LOG_LEVEL);
...
static int pdus_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (WE_SUCCESS != PDUS_getPressureAndTemperature_float(&data->sensor_interface,
			    ...
			    struct sensor_value *value)
{
	...
	if (chan == SENSOR_CHAN_PRESS) {
		...
		value->val2 = (((int32_t)(data->pressure_k_pa * 1000)) % 1000) * (1000000 / 1000);
	} else if (chan == SENSOR_CHAN_AMBIENT_TEMP) {
		value->val1 = (int32_t)data->temperature_deg_c;
		value->val2 =
			(((int32_t)(data->temperature_deg_c * 1000)) % 1000) * (1000000 / 1000);
	} else {
		return -ENOTSUP;
	}
