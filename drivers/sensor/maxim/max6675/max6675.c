...
static int max6675_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	int ret;
	uint8_t buf_rx[2];
	const struct spi_buf rx_buf = {.buf = buf_rx, .len = ARRAY_SIZE(buf_rx)};
	const struct spi_buf_set rx_bufs = {.buffers = &rx_buf, .count = 1U};
