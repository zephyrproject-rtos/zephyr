...
{
	struct adc_cmp_npcx_data *data = CONTAINER_OF(item, struct adc_cmp_npcx_data, work);
...
static int adc_cmp_npcx_init(const struct device *dev)
{
	...
	ret = adc_npcx_threshold_ctrl_set_param(config->adc, config->th_sel, &param);
	if (ret) {
		goto init_error;
	}
	...
	if (config->thr_mv != ADC_CMP_NPCX_UNDEFINED) {
		...
		ret = adc_npcx_threshold_mv_to_thrval(config->adc, config->thr_mv, &param.val);
		if (ret) {
			goto init_error;
		}
...
init_error:
	if (ret) {
		LOG_ERR("Error setting parameter %d - value %d", (uint32_t)param.type, param.val);
	}

	return ret;
}

static int adc_cmp_npcx_set_threshold(const struct device *dev, bool is_upper, bool is_mv,
				      uint32_t value)
{
	const struct adc_cmp_npcx_config *const config = dev->config;
	struct adc_npcx_threshold_param param;
	...
	param.type = ADC_NPCX_THRESHOLD_PARAM_L_H;
	param.val =
		is_upper ? ADC_NPCX_THRESHOLD_PARAM_L_H_HIGHER : ADC_NPCX_THRESHOLD_PARAM_L_H_LOWER;
...
static int adc_cmp_npcx_attr_set(const struct device *dev, enum sensor_channel chan,
				 enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_UPPER_VOLTAGE_THRESH:
		ret = adc_cmp_npcx_set_threshold(
			dev,
			/* Is upper? */
	...
	case SENSOR_ATTR_ALERT:
		ret = adc_npcx_threshold_ctrl_enable(config->adc, config->th_sel, !!val->val1);
		break;
	default:
		ret = -ENOTSUP;
	}
...
static int adc_cmp_npcx_attr_get(const struct device *dev, enum sensor_channel chan,
				 enum sensor_attribute attr, struct sensor_value *val)
{
	return -ENOTSUP;
}

static int adc_cmp_npcx_trigger_set(const struct device *dev, const struct sensor_trigger *trig,
				    sensor_trigger_handler_t handler)
{
	const struct adc_cmp_npcx_config *const config = dev->config;
	struct adc_cmp_npcx_data *data = dev->data;
	...
	param.type = ADC_NPCX_THRESHOLD_PARAM_WORK;
	param.val = (uint32_t)&data->work;
	return adc_npcx_threshold_ctrl_set_param(config->adc, config->th_sel, &param);
}

static int adc_cmp_npcx_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	return -ENOTSUP;
}

static int adc_cmp_npcx_channel_get(const struct device *dev, enum sensor_channel chan,
...
#define NPCX_ADC_CMP_INIT(inst)                                                                   \
	...
	static const struct adc_cmp_npcx_config adc_cmp_npcx_config_##inst = {                    \
		...
		.thr_mv = DT_INST_PROP_OR(inst, threshold_mv, ADC_CMP_NPCX_UNDEFINED),            \
		.comparison = DT_INST_STRING_TOKEN_OR(inst, comparison, ADC_CMP_NPCX_UNDEFINED)}; \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, adc_cmp_npcx_init, NULL, &adc_cmp_npcx_data_##inst,    \
				     &adc_cmp_npcx_config_##inst, POST_KERNEL,                    \
				     CONFIG_SENSOR_INIT_PRIORITY, &adc_cmp_npcx_api);             \
