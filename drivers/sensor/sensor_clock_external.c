...
LOG_MODULE_REGISTER(sensor_clock, LOG_LEVEL_WRN);
...
{
	int ret = counter_start(external_sensor_clock);

	if (ret != 0) {
		LOG_WRN("Failed to start sensor clock counter: %d\n", ret);
		return ret;
	}

	freq = counter_get_frequency(external_sensor_clock);
	if (freq == 0) {
		LOG_WRN("Sensor clock %s has no fixed frequency\n", external_sensor_clock->name);
		return -EINVAL;
	}
