...
static int adxl362_reg_access(struct spi_config *spi, u8_t cmd,
				  ...
				  u8_t register_address, u8_t count)
{
	...
	return adxl362_reg_access(&adxl362_data->spi,
				  ADXL362_WRITE_REG,
				  register_address,
				  &register_value,
				  count);
...
static int adxl362_init(struct device *dev)
{
	...
	data->spi.dev = device_get_binding(CONFIG_ADXL362_SPI_DEV_NAME);
	if (!data->spi.dev) {
	...
	data->spi.slave = CONFIG_ADXL362_SPI_DEV_SLAVE;
