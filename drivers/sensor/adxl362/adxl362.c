...
{
	struct adxl362_data *adxl362_data = dev->fixed->driver_data;
...
int adxl362_reg_write_mask(struct device *dev, uint8_t register_address,
...
{
	struct adxl362_data *data = dev->fixed->driver_data;
	int16_t buf[4];
	int ret;
...
{
	const struct adxl362_config *config = dev->fixed->config_info;
	struct adxl362_data *data = dev->fixed->driver_data;
	uint8_t value;
	int err;
