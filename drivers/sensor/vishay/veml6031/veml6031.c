...
static int veml6031_read16(const struct device *dev, uint8_t cmd, uint8_t *data)
{
	...
	ret = i2c_burst_read_dt(&conf->bus, cmd, recv_buf, ARRAY_SIZE(recv_buf));
	if (ret < 0) {
		return ret;
	}
...
static int veml6031_write16(const struct device *dev, uint8_t cmd, uint8_t *data)
{
	...
	return i2c_burst_write_dt(&conf->bus, cmd, send_buf, ARRAY_SIZE(send_buf));
}
...
static int veml6031_fetch(const struct device *dev)
{
	...
	ret = veml6031_read16(dev, VEML6031_CMDCODE_ALS_DATA_L, counts);
	if (ret < 0) {
		return ret;
	}
	data->als_data = counts[1] << 8 | counts[0];

	ret = veml6031_read16(dev, VEML6031_CMDCODE_IR_DATA_L, counts);
	if (ret < 0) {
		return ret;
	}
