 ...
 */
static struct veml6031_it_data veml6031_it_values[] = {
 ...
 */
static const float veml6031_resolution[VEML6031_DIV4_COUNT]
				      [VEML6031_GAIN_COUNT]
				      [VEML6031_IT_COUNT] = {
	/*3.125ms   6.25ms   12.5ms     25ms     50ms    100ms    200ms     400ms IT */
	/* size 4/4 */
	{
	{0.8704f, 0.4352f, 0.2176f, 0.1088f, 0.0544f, 0.0272f, 0.0136f, 0.0068f}, /* Gain 1    */
	{0.4352f, 0.2176f, 0.1088f, 0.0544f, 0.0272f, 0.0136f, 0.0068f, 0.0034f}, /* Gain 2    */
	{1.3188f, 0.6504f, 0.3297f, 0.1648f, 0.0824f, 0.0412f, 0.0206f, 0.0103f}, /* Gain 0.66 */
	{1.7408f, 0.8704f, 0.4352f, 0.2176f, 0.1088f, 0.0544f, 0.0272f, 0.0136f}, /* Gain 0.5  */
	},
	{
	/* size 1/4 */
	{3.4816f, 1.7408f, 0.8704f, 0.4352f, 0.2176f, 0.1088f, 0.0544f, 0.0272f}, /* Gain 1    */
	{1.7408f, 0.8704f, 0.4352f, 0.2176f, 0.1088f, 0.0544f, 0.0272f, 0.0136f}, /* Gain 2    */
	{5.2752f, 2.6376f, 1.3188f, 0.6594f, 0.3297f, 0.1648f, 0.0824f, 0.0412f}, /* Gain 0.66 */
	{6.9632f, 3.4816f, 1.7408f, 0.8704f, 0.4352f, 0.2176f, 0.1088f, 0.0544f}, /* Gain 0.5  */
	},
};
...
struct veml6031_data {
	...
	enum veml6031_it	it;	/* ALS integration time */
...
{
	k_sleep(K_USEC(veml6031_it_values[data->it].us));
}
...
static int veml6031_write(const struct device *dev, uint8_t cmd, uint8_t data)
{
	const struct veml6031_config *conf = dev->config;
	uint8_t send_buf[2];
...
static int veml6031_write_conf(const struct device *dev)
{
	...
	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_CONF_0, conf[0]);
	if (ret) {
		LOG_ERR("Error while writing conf[0] ret: %d", ret);
		return ret;
	}

	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_CONF_1, conf[1]);
	if (ret) {
		LOG_ERR("Error while writing conf[1] ret: %d", ret);
		return ret;
	}
...
static int veml6031_write_thresh_high(const struct device *dev)
{
	...
	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_WH_L, data->thresh_high & 0xFF);
	if (ret) {
		return ret;
	}

	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_WH_H, data->thresh_high >> 8);
	if (ret) {
		return ret;
	}
...
static int veml6031_write_thresh_low(const struct device *dev)
{
	...
	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_WL_L, data->thresh_low & 0xFF);
	if (ret) {
		return ret;
	}

	ret = veml6031_write(dev, VEML6031_CMDCODE_ALS_WL_H, data->thresh_low >> 8);
	if (ret) {
		return ret;
	}
...
static int veml6031_fetch(const struct device *dev)
{
	...
	ret = veml6031_read(dev, VEML6031_CMDCODE_ALS_DATA_L, &counts[0]);
	if (ret < 0) {
		return ret;
	}

	ret = veml6031_read(dev, VEML6031_CMDCODE_ALS_DATA_H, &counts[1]);
	if (ret < 0) {
		return ret;
	}

	data->als_data = counts[1] << 8 | counts[0];
	ret = veml6031_read(dev, VEML6031_CMDCODE_IR_DATA_L, &counts[0]);
	if (ret < 0) {
		return ret;
	}

	ret = veml6031_read(dev, VEML6031_CMDCODE_IR_DATA_H, &counts[1]);
	if (ret < 0) {
		return ret;
	}

	data->ir_data = counts[1] << 8 | counts[0];
	data->als_lux =
		data->als_data * veml6031_resolution[data->div4][data->gain][data->it];

	LOG_DBG("Read ALS measurement: counts=%d, lux=%d ir=%d",
			data->als_data, data->als_lux, data->ir_data);
			     ...
			     enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((enum sensor_attribute_veml6031)attr == SENSOR_ATTR_VEML6031_IT) {
		if (veml6031_check_it(val)) {
			data->it = (enum veml6031_it)val->val1;
		} else {
			return -EINVAL;
		}
	} else if ((enum sensor_attribute_veml6031)attr == SENSOR_ATTR_VEML6031_DIV4) {
		if (veml6031_check_div4(val)) {
			data->div4 = (enum veml6031_div4)val->val1;
		} else {
			return -EINVAL;
		}
	} else if ((enum sensor_attribute_veml6031)attr == SENSOR_ATTR_VEML6031_GAIN) {
		if (veml6031_check_gain(val)) {
			data->gain = (enum veml6031_gain)val->val1;
		} else {
			return -EINVAL;
		}
	} else if ((enum sensor_attribute_veml6031)attr == SENSOR_ATTR_VEML6031_PERS) {
		...
		data->thresh_low =
			val->val1 / veml6031_resolution[data->div4][data->gain][data->it];
				...
				struct sensor_value *val)
{
	...
	} else if ((enum sensor_channel_veml6031)chan == SENSOR_CHAN_VEML6031_ALS_RAW_COUNTS) {
		val->val1 = data->als_data;
	} else if ((enum sensor_channel_veml6031)chan == SENSOR_CHAN_VEML6031_IR_RAW_COUNTS) {
...
static int veml6031_init(const struct device *dev)
{
	...
	data->trig = 1;
	data->af = 1;
	data->div4 = VEML6031_SIZE_4_4;
	data->gain = VEML6031_GAIN_1;
	data->it = VEML6031_IT_100;
	data->pers = VEML6031_PERS_1;
	...
	LOG_DBG("veml6031 found package: %02d address: %02X version: %3s",
		val8 >> 6,
		val8 >> 4 & 0x03 ? 0x10 : 0x29,
		val8 & 0x0F ? "XXX" : "A01");
...
#define VEML6031_INIT(n)                                                                           \
	static struct veml6031_data veml6031_data_##n;                                             \
                                                                                                   \
