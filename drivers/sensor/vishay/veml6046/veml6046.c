 ...
 *
 * The enumerators of <tt>enum veml6046_it</tt> provide
 * indices into this array to get the related value for the
 * ALS_IT configuration bits.
 */
static const struct veml6046_it_data veml6046_it_values[] = {
 ...
 * The enumerators of <tt>enum veml6046_pdd</tt>, <tt>enum veml6046_gain</tt>
 * and <tt>enum veml6046_als_it</tt> are used for indices into this matrix.
 */
static const float
...
static int veml6046_read16(const struct device *dev, uint8_t cmd, uint8_t *data)
{
...
static int veml6046_fetch(const struct device *dev)
{
	...
	ret = veml6046_read16(dev, VEML6046_CMDCODE_R_DATA_L, (uint8_t *)&data->red_data);
	if (ret < 0) {
		return ret;
	}
	data->red_data = sys_le16_to_cpu(data->red_data);

	ret = veml6046_read16(dev, VEML6046_CMDCODE_G_DATA_L, (uint8_t *)&data->green_data);
	if (ret < 0) {
		return ret;
	}
	data->green_data = sys_le16_to_cpu(data->green_data);

	ret = veml6046_read16(dev, VEML6046_CMDCODE_B_DATA_L, (uint8_t *)&data->blue_data);
	if (ret < 0) {
		return ret;
	}
	data->blue_data = sys_le16_to_cpu(data->blue_data);

	ret = veml6046_read16(dev, VEML6046_CMDCODE_IR_DATA_L, (uint8_t *)&data->ir_data);
	if (ret < 0) {
		return ret;
	}
...
static int veml6046_perform_single_measurement(const struct device *dev)
{
	...
	while (1) {
		...
		if (val[1] & VEML6046_AF_DATA_READY) {
			break;
		}
	...
	LOG_DBG("read VEML6046_CMDCODE_INT_H: %02X (%d)", val[1], cnt);
...
static int veml6046_init(const struct device *dev)
{
	...
	if (val[0] != VEML6046_DEFAULT_ID) {
		LOG_ERR("Device ID wrong: %d", val[0]);
		return -EIO;
	}

	LOG_DBG("veml6046 found package: %02d address: %02X version: %3s",
		val[1] >> 6, val[1] >> 4 & 0x03 ? 0x10 : 0x29,
		val[1] & 0x0F ? "XXX" : "A01");
