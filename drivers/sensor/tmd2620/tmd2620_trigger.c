...
void tmd2620_work_cb(struct k_work *work)
{
	...
	struct tmd2620_data *data = CONTAINER_OF(work,
						struct tmd2620_data,
						work);
...
int tmd2620_attr_set(const struct device *dev,
...
int tmd2620_trigger_set(const struct device *dev,
				const struct sensor_trigger *trigg,
