...
static void tlv493d_update_parity(uint8_t *wr_regs)
{
	uint8_t *mode1_cfg = &wr_regs[TLV493D_WR_REG_MODE1];
	uint8_t y;

	WRITE_BIT(*mode1_cfg, 7, 1);

	y = wr_regs[0] ^ wr_regs[1] ^ wr_regs[2] ^ wr_regs[3];
	y ^= y >> 4;
	y ^= y >> 2;
	y ^= y >> 1;

	WRITE_BIT(*mode1_cfg, 7, y & 0x1);
}

static int tlv493d_write_config(const struct device *dev, uint8_t *wr_regs)
{
	const struct tlv493d_config *cfg = dev->config;

	tlv493d_update_parity(wr_regs);

	int ret = i2c_write_dt(&cfg->i2c, wr_regs, TLV493D_WR_REG_MAX);
...
static int tlv493d_fetch(const struct device *dev)
{
	...
	if (!(data->wr_regs[TLV493D_WR_REG_MODE1] & TLV493D_MODE1_OPERATING_MODE)) {
	...
	if (regs[TLV493D_RD_REG_Z2] & TLV493D_Z2_TEST) {
		LOG_ERR("Invalid sensor test mode");
		return -EIO;
	}

	if (data->wr_regs[TLV493D_WR_REG_MODE2] & TLV493D_MODE2_PT &&
	    !(regs[TLV493D_RD_REG_Z2] & TLV493D_Z2_FF)) {
		LOG_ERR("Invalid fuse parity");
		return -EIO;
	}

	if (FIELD_GET(TLV493D_TEMP_CHANNEL, regs[TLV493D_RD_REG_TEMP])) {
...
static int tlv493d_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	const struct tlv493d_config *cfg = dev->config;
...
static int tlv493d_channel_get(const struct device *dev, enum sensor_channel chan,
			       struct sensor_value *val)
{
	...
	switch (chan) {
	case SENSOR_CHAN_MAGN_XYZ:
		tlv493d_convert_to_sensor_value(data->x, &val[0]);
		tlv493d_convert_to_sensor_value(data->y, &val[1]);
		tlv493d_convert_to_sensor_value(data->z, &val[2]);
		return 0;
	case SENSOR_CHAN_MAGN_X:
		raw_value = data->x;
		break;
...
static int tlv493d_init(const struct device *dev)
{
	...
	case 0x5e:
		...
		break;
	default:
		LOG_ERR("Unsupported sensor address after reset: 0x%x", cfg->i2c.addr);
		return -EINVAL;
	}

	/* The device requires a general reset after power-up. */
	ret = i2c_write(cfg->i2c.bus, &reset_addr, sizeof(reset_addr), 0x00);

	if (ret < 0) {
		LOG_ERR("Sensor did not reset: %d", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Sensor did not respond after reset: %d", ret);
		return ret;
	}

	data->mode = cfg->mode;
	data->wr_regs[0] = 0;
	data->wr_regs[1] = data->rd_regs[TLV493D_RD_REG_RES1] & TLV493D_RD_REG_RES1_WR_MASK;
	data->wr_regs[2] = data->rd_regs[TLV493D_RD_REG_RES2] & TLV493D_RD_REG_RES2_WR_MASK;
	data->wr_regs[3] = data->rd_regs[TLV493D_RD_REG_RES3] & TLV493D_RD_REG_RES3_WR_MASK;
	data->wr_regs[TLV493D_WR_REG_MODE2] |= TLV493D_MODE2_PT;
	...
	ret = tlv493d_write_config(dev, data->wr_regs);
	if (ret < 0) {
		return ret;
	}

#ifdef CONFIG_PM_DEVICE_RUNTIME
	if (data->mode == TLV493D_OP_MODE_POWERDOWN) {
			    ...
			    enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if ((int)attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
		ret = tlv493d_get_mode_from_odr(val, &mode);
		if (ret < 0) {
			return ret;
		}

		memcpy(wr_regs, data->wr_regs, sizeof(wr_regs));
		ret = tlv493d_set_operating_mode(wr_regs, mode);
		if (ret < 0) {
			return ret;
		}

		ret = tlv493d_write_config(dev, wr_regs);
		if (ret < 0) {
			return ret;
		}

		memcpy(data->wr_regs, wr_regs, sizeof(wr_regs));
		data->mode = mode;
		return 0;
	} else {
		return -ENOTSUP;
...
static int tlv493d_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		return tlv493d_write_config(dev, data->wr_regs);
