...
#define DPS310_CFG_REG (((IFX_DPS310_T_SHIFT & 0x01) << 3) | ((IFX_DPS310_P_SHIFT & 0x01) << 2))
 ...
 * structure
 */
static void dps310_calib_coeff_creation(const uint8_t raw_coef[18], struct dps310_cal_coeff *comp)
{
	...
	comp->c10 = (((uint32_t)(raw_coef[5] & 0x0F)) << 16) + (((uint16_t)raw_coef[6]) << 8) +
		    raw_coef[7];
	/* coefficient is 2nd compliment */
...
static bool dps310_trigger_temperature(const struct device *dev)
{
	...
	static const uint8_t tmp_meas_cmd[] = {IFX_DPS310_REG_ADDR_MEAS_CFG,
...
static bool dps310_trigger_pressure(const struct device *dev)
{
	...
	static const uint8_t psr_meas_cmd[] = {IFX_DPS310_REG_ADDR_MEAS_CFG,
...
static void dps310_scale_temperature(const struct device *dev, int32_t tmp_raw)
{
	...
	int32_t tmp_p1 = (((int64_t)1000000) * comp->c1 * tmp_raw) / IFX_DPS310_SF_TMP;
...
static bool dps310_measure_tmp_psr(const struct device *dev)
{
	...
	int res =
		i2c_write_read_dt(&config->i2c, &REG_ADDR_PSR_B2, 1, &value_raw, sizeof(value_raw));
	if (res < 0) {
		LOG_WRN("I2C error: %d", res);
		return false;
	}
...
static bool dps310_measure_tmp(const struct device *dev)
{
	...
	int res =
		i2c_write_read_dt(&config->i2c, &REG_ADDR_TMP_B2, 1, &value_raw, sizeof(value_raw));
	if (res < 0) {
		LOG_WRN("I2C error: %d", res);
		return false;
	}
...
static int dps310_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
...
static int dps310_channel_get(const struct device *dev, enum sensor_channel chan,
...
#define DPS310_DEFINE(inst)                                                                        \
	...
	static const struct dps310_cfg dps310_cfg_##inst = {                                       \
		.i2c = I2C_DT_SPEC_INST_GET(inst),                                                 \
                                                                                                   ...
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, dps310_init, NULL, &dps310_data_##inst,                 \
				     &dps310_cfg_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, \
				     &dps310_api_funcs);
