...
{
	struct ism330dhcx_data *data = dev->fixed->driver_data;
...
static int ism330dhcx_accel_odr_set(struct device *dev, uint16_t freq)
{
	int odr;
...
static int ism330dhcx_gyro_odr_set(struct device *dev, uint16_t freq)
{
	int odr;
...
{
	struct ism330dhcx_data *ism330dhcx = dev->fixed->driver_data;
	uint8_t chip_id;
...
{
	const struct ism330dhcx_config * const config = dev->fixed->config_info;
	struct ism330dhcx_data *data = dev->fixed->driver_data;
