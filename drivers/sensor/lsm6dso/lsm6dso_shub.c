...
#define LSM6DSO_SHUB_MASTER_CONFIG			0x14
#define LSM6DSO_SHUB_WRITE_ONCE_MASK			BIT(6)
#define LSM6DSO_SHUB_PU_EN				BIT(3)
#define LSM6DSO_SHUB_MASTER_ON_MASK			BIT(2)
...
static inline void lsm6dso_shub_embedded_en(struct lsm6dso_data *data, bool on)
{
	...
	data->hw_tf->update_reg(data, LSM6DSO_REG_FUNC_CFG_ACCESS,
				LSM6DSO_MASK_SHUB_EN, func_en);
...
static void lsm6dso_shub_enable(struct lsm6dso_data *data, u8_t enable)
{
	...
	if (data->hw_tf->update_reg(data, LSM6DSO_SHUB_MASTER_CONFIG,
				    LSM6DSO_SHUB_MASTER_ON_MASK,
				    enable) < 0) {
				       ...
				       u8_t *value, u16_t len)
{
	...
	lsm6dso_shub_embedded_en(data, true);
	if (data->hw_tf->read_data(data, LSM6DSO_SHUB_DATA_OUT,
				   value, len) < 0) {
...
static int lsm6dso_shub_set_data_channel(struct lsm6dso_data *data)
{
	...
	slv_cfg[0] = LSM6DSO_SHUB_WRITE_ONCE_MASK | 0x3;
	if (lsm6dso_shub_write_embedded_regs(data, LSM6DSO_SHUB_MASTER_CONFIG,
					     slv_cfg, 1) < 0) {
		LOG_DBG("shub: error writing embedded reg");
		return -EIO;
	}
