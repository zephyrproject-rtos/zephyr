...
static int lsm6dso_enable_t_int(struct device *dev, int enable)
{
	...
	if (enable) {
		...
		lsm6dso->hw_tf->read_data(lsm6dso,
					  LSM6DSO_REG_OUT_TEMP_L, raw,
					  sizeof(raw));
	}
	...
	return lsm6dso->hw_tf->update_reg(lsm6dso,
					  LSM6DSO_REG_INT2_CTRL,
					  LSM6DSO_MASK_INT2_CTRL_DRDY_TEMP,
					  enable);
}
#endif
...
static int lsm6dso_enable_xl_int(struct device *dev, int enable)
{
	...
	if (cfg->int_pin == 1)
		return lsm6dso->hw_tf->update_reg(lsm6dso,
					  ...
					  LSM6DSO_MASK_INT1_CTRL_DRDY_XL,
					  enable);

	return lsm6dso->hw_tf->update_reg(lsm6dso,
					  LSM6DSO_REG_INT2_CTRL,
					  LSM6DSO_MASK_INT2_CTRL_DRDY_XL,
					  enable);
}
...
static int lsm6dso_enable_g_int(struct device *dev, int enable)
{
	...
	if (cfg->int_pin == 1)
		return lsm6dso->hw_tf->update_reg(lsm6dso,
					  LSM6DSO_REG_INT1_CTRL,
					  LSM6DSO_MASK_INT1_CTRL_DRDY_G,
					  enable);

	return lsm6dso->hw_tf->update_reg(lsm6dso,
					  LSM6DSO_REG_INT2_CTRL,
					  LSM6DSO_MASK_INT2_CTRL_DRDY_G,
					  enable);
}
...
static void lsm6dso_handle_interrupt(void *arg)
{
	...
	mask = LSM6DSO_MASK_STATUS_REG_XLDA | LSM6DSO_MASK_STATUS_REG_GDA;

#if defined(CONFIG_LSM6DSO_ENABLE_TEMP)
	mask |= LSM6DSO_MASK_STATUS_REG_TDA;
#endif

	while (1) {
		if (lsm6dso->hw_tf->read_reg(lsm6dso,
					     LSM6DSO_REG_STATUS_REG,
					     &status) < 0) {
		...
		if ((status & LSM6DSO_MASK_STATUS_REG_XLDA) &&
		    (lsm6dso->handler_drdy_acc != NULL)) {
			lsm6dso->handler_drdy_acc(dev, &drdy_trigger);
		}

		if ((status & LSM6DSO_MASK_STATUS_REG_GDA) &&
		    (lsm6dso->handler_drdy_gyr != NULL)) {
			lsm6dso->handler_drdy_gyr(dev, &drdy_trigger);
		}

#if defined(CONFIG_LSM6DSO_ENABLE_TEMP)
		if ((status & LSM6DSO_MASK_STATUS_REG_TDA) &&
...
int lsm6dso_init_interrupt(struct device *dev)
{
	...
	if (lsm6dso->hw_tf->update_reg(lsm6dso, LSM6DSO_REG_TAP_CFG0,
				       LSM6DSO_MASK_INT_LIR,
				       LSM6DSO_DIS_BIT)) {
