...
static inline int icm20948_change_bank(struct icm20948_data *data,u16_t reg_bank_addr){
	...
	if(bank != data->bank){
		return i2c_reg_write_byte(data->bus,CONFIG_ICM20948_I2C_SLAVE_ADDR,ICM20948_REG_BANK_SEL, (bank << 4))
	}
	return 0;
}

static int icm20948_i2c_read_data(struct icm20948_data *data, u16_t reg_bank_addr,
				 u8_t *value, u8_t len)
{
	u8_t reg_addr = (u8_t)(reg_bank_addr & 0xff);
	if(icm20948_change_bank(data->bus,reg_bank_addr)){
		return -EIO;
	}
...
static int icm20948_i2c_write_data(struct icm20948_data *data, u16_t reg_bank_addr,
				  u8_t *value, u8_t len)
{
	u8_t reg_addr = (u8_t)(reg_bank_addr & 0xff);
	if(icm20948_change_bank(data->bus,reg_bank_addr)){
...
static int icm20948_i2c_read_reg(struct icm20948_data *data, u16_t reg_bank_addr,
				u8_t *value)
{
	u8_t reg_addr = (u8_t)(reg_bank_addr & 0xff);
	if(icm20948_change_bank(data->bus,reg_bank_addr)){
...
static int icm20948_i2c_write_reg(struct icm20948_data *data, u16_t reg_bank_addr,
				u8_t value)
{
	u8_t reg_addr = (u8_t)(reg_bank_addr & 0xff);
	if(icm20948_change_bank(data->bus,reg_bank_addr)){
...
static int icm20948_i2c_update_reg(struct icm20948_data *data, u16_t reg_bank_addr,
