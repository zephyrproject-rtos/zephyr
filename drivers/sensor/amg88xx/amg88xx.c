...
static int amg88xx_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read_dt(&config->i2c, AMG88XX_OUTPUT_BASE, (uint8_t *)drv_data->sample,
...
static int amg88xx_channel_get(const struct device *dev, enum sensor_channel chan,
			       struct sensor_value *val)
{
	...
	for (size_t idx = 0; idx < len; idx++) {
		...
		val[idx].val1 =
			(((int32_t)drv_data->sample[idx]) * AMG88XX_TREG_LSB_SCALING) / 1000000;
		val[idx].val2 =
			(((int32_t)drv_data->sample[idx]) * AMG88XX_TREG_LSB_SCALING) % 1000000;
	}
...
static int amg88xx_init_device(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, AMG88XX_PCLT, &tmp)) {
	...
	if (tmp != AMG88XX_PCLT_NORMAL_MODE) {
		if (i2c_reg_write_byte_dt(&config->i2c, AMG88XX_PCLT, AMG88XX_PCLT_NORMAL_MODE)) {
			return -EIO;
		}
		k_busy_wait(AMG88XX_WAIT_MODE_CHANGE_US);
	}

	if (i2c_reg_write_byte_dt(&config->i2c, AMG88XX_RST, AMG88XX_RST_INITIAL_RST)) {
		return -EIO;
	}

	k_busy_wait(AMG88XX_WAIT_INITIAL_RESET_US);

	if (i2c_reg_write_byte_dt(&config->i2c, AMG88XX_FPSC, AMG88XX_FPSC_10FPS)) {
		return -EIO;
	}
	...
	static const struct amg88xx_config amg88xx_config_##inst = {                               \
		...
		IF_ENABLED(CONFIG_AMG88XX_TRIGGER,						\
			   (.int_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, int_gpios, { 0 }),)) };           \
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, amg88xx_init, NULL, &amg88xx_data_##inst,               \
				     &amg88xx_config_##inst, POST_KERNEL,                          \
