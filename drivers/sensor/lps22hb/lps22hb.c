...
{
	struct lps22hb_data *data = dev->fixed->driver_data;
	const struct lps22hb_config *config = dev->fixed->config_info;
...
{
	const struct lps22hb_config * const config = dev->fixed->config_info;
	struct lps22hb_data *data = dev->fixed->driver_data;
