...
static void mcux_eqdc_init_pins(const struct device *dev)
{
	...
	for (uint32_t i = 0; i < config->num_muxes; i++) {
...
#define MCUX_EQDC_MUX_CONTROL_DEFINE_IDX(node_id, pha, idx)		\
	MUX_CONTROL_DT_SPEC_DEFINE_BY_IDX(node_id, idx)
#define MCUX_EQDC_MUX_CONTROL_DEFINE_ALL(n)				\
	DT_INST_FOREACH_PROP_ELEM(n, mux_states,				\
			MCUX_EQDC_MUX_CONTROL_DEFINE_IDX)
DT_INST_FOREACH_STATUS_OKAY(MCUX_EQDC_MUX_CONTROL_DEFINE_ALL)
#define MCUX_EQDC_MUX_CONTROL_GET_IDX(node_id, pha, idx)		\
	MUX_CONTROL_DT_GET_BY_IDX(node_id, idx)

#define MCUX_EQDC_MUX_CONTROL_LIST_DEFINE(n)					\
	static struct mux_control *eqdc_##n##_muxes[] = {				\
	DT_INST_FOREACH_PROP_ELEM_SEP(n, mux_states,					\
				MCUX_EQDC_MUX_CONTROL_GET_IDX, (,)) 	\
	};
DT_INST_FOREACH_STATUS_OKAY(MCUX_EQDC_MUX_CONTROL_LIST_DEFINE)

#define MCUX_EQDC_GET_MUX_LIST(n) eqdc_##n##_muxes
...
#define MCUX_EQDC_DEVICE_INIT(inst)						\
	...
	static const struct mcux_eqdc_config mcux_eqdc_config_##inst = {	\
		...
		.mux_dev = DEVICE_DT_GET(DT_MUX_CTLR_BY_IDX(DT_DRV_INST(inst), 0)),	\
		.mux_controls = MCUX_EQDC_GET_MUX_LIST(inst),			\
		.num_muxes = DT_INST_PROP_LEN(inst, mux_states),	\
