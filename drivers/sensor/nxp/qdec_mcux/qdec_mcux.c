...
#define QDEC_XBAR_CONNECT(idx, n) \
	do {									\
		const struct device *xbar_dev =					\
			NXP_XBAR_DT_INST_DEVICE_GET_BY_IDX(n, idx);	\
		__ASSERT(xbar_dev != NULL, "Failed to get device binding");	\
		__ASSERT(device_is_ready(xbar_dev), "Device xbar is not ready");	\
								\
		nxp_xbar_set_connection(xbar_dev,			\
			NXP_XBAR_DT_INST_OUTPUT_GET_BY_IDX(n, idx),		\
			NXP_XBAR_DT_INST_INPUT_GET_BY_IDX(n, idx));		\
				\
		LOG_DBG("XBAR connection set: output=%u, input=%u",		\
			NXP_XBAR_DT_INST_OUTPUT_GET_BY_IDX(n, idx),		\
			NXP_XBAR_DT_INST_INPUT_GET_BY_IDX(n, idx));			\
	} while (0)
