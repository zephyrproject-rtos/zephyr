				...
				struct sensor_value *val)
{
		...
		case SENSOR_CHAN_ALL:
			if (fxls8974_get_accel_data(dev, val))
				return -EIO;
		...
		case SENSOR_CHAN_ACCEL_XYZ:
			return fxls8974_get_accel_data(dev, val);
...
static int fxls8974_get_active(const struct device *dev, enum fxls8974_active *active)
{
		const struct fxls8974_config *cfg = dev->config;
		uint8_t val;
...
static int fxls8974_set_active(const struct device *dev, enum fxls8974_active active)
{
		const struct fxls8974_config *cfg = dev->config;
