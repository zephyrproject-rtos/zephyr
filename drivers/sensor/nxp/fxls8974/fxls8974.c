...
int fxls8974_transceive(const struct device *dev, void *data, size_t length)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_read_spi(const struct device *dev, uint8_t reg, void *data, size_t length)
{
	if (dev == NULL) {
		return -ENXIO;
	}
	...
	const struct spi_buf buf[2] = {{.buf = reg_buf, .len = 3}, {.buf = data, .len = length}};
...
int fxls8974_byte_read_spi(const struct device *dev, uint8_t reg, uint8_t *byte)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_byte_write_spi(const struct device *dev, uint8_t reg, uint8_t byte)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_reg_field_update_spi(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_read_i2c(const struct device *dev, uint8_t reg, void *data, size_t length)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_byte_read_i2c(const struct device *dev, uint8_t reg, uint8_t *byte)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_byte_write_i2c(const struct device *dev, uint8_t reg, uint8_t byte)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
int fxls8974_reg_field_update_i2c(const struct device *dev, uint8_t reg, uint8_t mask, uint8_t val)
{
	if (dev == NULL) {
		return -ENXIO;
	}
...
static int fxls8974_set_odr(const struct device *dev, const struct sensor_value *val,
			    enum fxls8974_wake mode)
{
	if (dev == NULL) {
		return -ENXIO;
	}
	...
	if (mode == FXLS8974_WAKE) {
		return cfg->ops->reg_field_update(dev, FXLS8974_REG_CTRLREG3,
						  FXLS8974_CTRLREG3_WAKE_ODR_MASK, odr << 4);
	} else {
		return cfg->ops->reg_field_update(dev, FXLS8974_REG_CTRLREG3,
						  FXLS8974_CTRLREG3_SLEEP_ODR_MASK, odr);
	}
}

static int fxls8974_attr_set(const struct device *dev, enum sensor_channel chan,
...
static int fxls8974_sample_fetch(const struct device *dev, enum sensor_channel ch)
{
	 ...
	 */
	__ASSERT(FXLS8974_MAX_NUM_CHANNELS <= ARRAY_SIZE(data->raw), "Too many channels");
...
static void fxls8974_accel_convert(struct sensor_value *val, int16_t raw, uint8_t fsr)
{
	if (val == NULL) {
		return;
	}
...
static int fxls8974_get_accel_data(const struct device *dev, struct sensor_value *val,
...
static int fxls8974_channel_get(const struct device *dev, enum sensor_channel chan,
...
int fxls8974_set_active(const struct device *dev, enum fxls8974_active active)
{
	...
	return cfg->ops->reg_field_update(dev, FXLS8974_REG_CTRLREG1, FXLS8974_CTRLREG1_ACTIVE_MASK,
					  active);
}
...
static int fxls8974_init(const struct device *dev)
{
	 ...
	 * the I2C transaction.
	 */
	cfg->ops->byte_write(dev, FXLS8974_REG_CTRLREG1, FXLS8974_CTRLREG1_RST_MASK);
	 ...
	 */
	if (cfg->ops->byte_read(dev, FXLS8974_REG_WHOAMI, &data->whoami)) {
	...
	if (cfg->ops->byte_write(dev, FXLS8974_REG_CTRLREG4, FXLS8974_CTRLREG4_INT_POL_HIGH)) {
	...
	/* Set the +-2G mode */
	if (cfg->ops->byte_write(dev, FXLS8974_REG_CTRLREG1, FXLS8974_CTRLREG1_FSR_2G)) {
		LOG_ERR("Could not set range");
		return -EIO;
	}

	if (cfg->ops->byte_read(dev, FXLS8974_REG_CTRLREG1, &regVal)) {
...
#define FXLS8974_INT_PROPS(n)                                                                      \
	COND_CODE_1(CONFIG_FXLS8974_DRDY_INT1,							   \
			(FXLS8974_INTM_PROPS(n, 1)), (FXLS8974_INTM_PROPS(n, 2)))

#define FXLS8974_INT(n) COND_CODE_1(CONFIG_FXLS8974_TRIGGER, (FXLS8974_INT_PROPS(n)), ())
