...
LOG_MODULE_DECLARE(HIDS, CONFIG_SENSOR_LOG_LEVEL);
...
#if defined(CONFIG_HIDS_TRIGGER_OWN_THREAD)
	k_sem_give(&data->drdy_sem);
#elif defined(CONFIG_HIDS_TRIGGER_GLOBAL_THREAD)
...
#ifdef CONFIG_HIDS_TRIGGER_OWN_THREAD
...
#endif /* CONFIG_HIDS_TRIGGER_OWN_THREAD */

#ifdef CONFIG_HIDS_TRIGGER_GLOBAL_THREAD
...
#endif /* CONFIG_HIDS_TRIGGER_GLOBAL_THREAD */
...
int hids_init_interrupt(const struct device *dev)
{
	...
	if (WE_SUCCESS != HIDS_enableDataReadyInterrupt(&data->sensor_interface, HIDS_enable)) {
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_HIDS_THREAD_STACK_SIZE,
			(k_thread_entry_t)hids_thread, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_HIDS_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_HIDS_TRIGGER_GLOBAL_THREAD)
