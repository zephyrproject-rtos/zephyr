...
LOG_MODULE_REGISTER(HIDS, CONFIG_SENSOR_LOG_LEVEL);
...
/* Maps ODR configuration string (CONFIG_HIDS_ODR) to enum value used by HIDS driver. */
static const struct str2odr hids_odr_string_list[] = {
	{ "1", HIDS_odr1Hz },
	{ "7", HIDS_odr7Hz },
	{ "12.5", HIDS_odr12_5Hz },
};
 ...
 */
static const struct sensor_value hids_odr_list[] = { { .val1 = 0, .val2 = 0 },
						     { .val1 = 1, .val2 = 0 },
						     { .val1 = 7, .val2 = 0 },
...
static int hids_sample_fetch(const struct device *dev, enum sensor_channel channel)
{
	...
	if (WE_SUCCESS != HIDS_getRawValues(&data->sensor_interface, &rawHumidity, &rawTemp)) {
		LOG_ERR("Failed to fetch data sample.");
		return -EIO;
	}

	if (WE_SUCCESS !=
	    HIDS_convertHumidity_uint16(&data->sensor_interface, rawHumidity, &data->humidity)) {
		LOG_ERR("Failed to convert humidity sample.");
		return -EIO;
	}
	if (WE_SUCCESS !=
	    HIDS_convertTemperature_int16(&data->sensor_interface, rawTemp, &data->temperature)) {
		LOG_ERR("Failed to convert temperature sample.");
		return -EIO;
	}
...
{
	struct hids_data *data = dev->data;
	int32_t valueConverted;

	if (channel == SENSOR_CHAN_AMBIENT_TEMP) {
		valueConverted = (int32_t)data->temperature;

		/* Convert temperature from 0.01 degrees Celsius to degrees Celsius */
		value->val1 = valueConverted / 100;
		value->val2 = (valueConverted % 100) * (1000000 / 100);
	} else if (channel == SENSOR_CHAN_HUMIDITY) {
		valueConverted = (int32_t)data->humidity;
...
static int hids_odr_set(const struct device *dev, const struct sensor_value *odr)
{
	...
	if (WE_SUCCESS !=
	    HIDS_setOutputDataRate(&data->sensor_interface, (HIDS_outputDataRate_t)odr_index)) {
...
static const struct sensor_driver_api hids_driver_api = { .attr_set = hids_attr_set,
#if CONFIG_HIDS_TRIGGER
							  .trigger_set = hids_trigger_set,
#endif
							  .sample_fetch = hids_sample_fetch,
							  .channel_get = hids_channel_get };

int hids_init(const struct device *dev)
{
	...
	if (WE_SUCCESS != HIDS_getDeviceID(&data->sensor_interface, &device_id)) {
	...
	/* Check if the configured ODR is valid */
	for (idx = 0U; idx < ARRAY_SIZE(hids_odr_string_list); idx++) {
		if (!strcmp(hids_odr_string_list[idx].str, CONFIG_HIDS_ODR)) {
			break;
		}
	}

	if (idx == ARRAY_SIZE(hids_odr_string_list)) {
		/* ODR not allowed (was not found in hids_odr_string_list) */
		LOG_ERR("Invalid ODR value %s.", CONFIG_HIDS_ODR);
		return -EINVAL;
	}

	if (WE_SUCCESS !=
	    HIDS_setOutputDataRate(&data->sensor_interface, hids_odr_string_list[idx].odr)) {
		LOG_ERR("Failed to set output data rate.");
		return -EIO;
	}

	if (WE_SUCCESS != HIDS_enableBlockDataUpdate(&data->sensor_interface, HIDS_enable)) {
		LOG_ERR("Failed to enable block data update.");
		return -EIO;
	}

	if (WE_SUCCESS != HIDS_setPowerMode(&data->sensor_interface, HIDS_activeMode)) {
		LOG_ERR("Failed to set power mode.");
		return -EIO;
	}

	if (WE_SUCCESS != HIDS_readCalibrationData(&data->sensor_interface)) {
...
#define HIDS_DEFINE(inst)                                                              \
	...
	static const struct hids_config hids_config_##inst =                           \
		COND_CODE_1(DT_INST_ON_BUS(inst, spi),                                 \
			    (HIDS_CONFIG_SPI(inst)),                                   \
			    (HIDS_CONFIG_I2C(inst)));                                  \
