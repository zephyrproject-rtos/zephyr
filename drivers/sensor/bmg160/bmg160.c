...
{
	const struct bmg160_device_config *dev_cfg = dev->fixed->config_info;
	struct bmg160_device_data *bmg160 = dev->fixed->driver_data;
	uint32_t i2c_cfg;
...
{
	const struct bmg160_device_config *cfg = dev->fixed->config_info;
	struct bmg160_device_data *bmg160 = dev->fixed->driver_data;
	uint8_t chip_id = 0U;
	uint16_t range_dps;
