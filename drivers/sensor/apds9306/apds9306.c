...
static int apds9306_sensor_setup(const struct device *dev)
{
	...
	/* Check if the APDS-9306-065 is used (selected by I2C address) */
	id = APDS_9306_CHIP_ID;
	if (config->i2c.addr == 0xB3) {
		id = APDS_9306_065_CHIP_ID;
	}
...
#if CONFIG_PM_DEVICE
static int apds9306_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
...
#define APDS9306(inst)                                                                             \
	...
	PM_DEVICE_DT_INST_DEFINE(inst, apds9306_pm_action);                                        \
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, apds9306_init, PM_DEVICE_DT_INST_GET(inst),             \
				     &apds9306_data_##inst, &apds9306_config_##inst, POST_KERNEL,  \
