...
static int bh1750_opcode_read(const struct device *dev, uint8_t opcode, uint16_t *val)
{
...
static int bh1750_mtreg_write(const struct device *dev, uint8_t mtreg)
{
	...
	if (rc < 0) {
		LOG_ERR("%s, Failed to write high byte of mtreg!", dev->name);
		return rc;
	}

	rc = bh1750_opcode_write(dev, BH1750_MTREG_LOW_BYTE | low_byte);
	if (rc < 0) {
		LOG_ERR("%s, Failed to write low byte of mtreg!", dev->name);
		return rc;
	}
...
static int bh1750_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (rc < 0) {
		LOG_ERR("%s: Failed to read measurement result!", dev->name);
		return rc;
	}
...
static const struct sensor_driver_api bh1750_driver_api = {.sample_fetch = bh1750_sample_fetch,
