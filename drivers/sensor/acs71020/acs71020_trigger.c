 ...
 *
 * !!!! NOT DONE OR TESTED, Converted from mcp9808
 *
...
LOG_MODULE_DECLARE(ACS71020, CONFIG_SENSOR_LOG_LEVEL);

static int acs71020_reg_write(struct device *dev, uint8_t reg, uint16_t val)
{
	const struct acs71020_data *data = dev->data;
		     ...
		     const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LOWER_THRESH:
		reg_addr = ACS71020_REG_LOWER_LIMIT;
		break;
	case SENSOR_ATTR_UPPER_THRESH:
		reg_addr = ACS71020_REG_UPPER_LIMIT;
		break;
	default:
		return -EINVAL;
	}
	 ...
	 */
	temp = val->val1 * ACS71020_TEMP_SCALE_CEL;
...
#if defined(CONFIG_ACS71020_TRIGGER_OWN_THREAD)
	k_sem_give(&data->sem);
#elif defined(CONFIG_ACS71020_TRIGGER_GLOBAL_THREAD)
...
{
	struct acs71020_data *data =
		CONTAINER_OF(cb, struct acs71020_data, alert_cb);
...
static void acs71020_thread_main(int arg1, int arg2)
{
...
static K_KERNEL_STACK_DEFINE(acs71020_thread_stack, CONFIG_ACS71020_THREAD_STACK_SIZE);
static struct k_thread acs71020_thread;
#else /* CONFIG_ACS71020_TRIGGER_GLOBAL_THREAD */

static void acs71020_gpio_thread_cb(struct k_work *work)
{
	struct acs71020_data *data =
		CONTAINER_OF(work, struct acs71020_data, work);

	process_int(data->dev);
}

#endif /* CONFIG_ACS71020_TRIGGER_GLOBAL_THREAD */
...
int acs71020_setup_interrupt(struct device *dev)
{
	...
	k_thread_create(&acs71020_thread, acs71020_thread_stack,
			CONFIG_ACS71020_THREAD_STACK_SIZE,
			(k_thread_entry_t)acs71020_thread_main, dev, 0, NULL,
			K_PRIO_COOP(CONFIG_ACS71020_THREAD_PRIORITY), 0, K_NO_WAIT);
#else /* CONFIG_ACS71020_TRIGGER_GLOBAL_THREAD */
	data->work.handler = acs71020_gpio_thread_cb;
#endif /* trigger type */
