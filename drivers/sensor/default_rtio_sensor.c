...
static void sensor_submit_fallback_sync(struct rtio_iodev_sqe *iodev_sqe)
{
	...
	for (size_t i = 0, sample_idx = 0; i < cfg->count; ++i) {
		...
		for (int sample = 0; sample < num_samples; ++sample) {
			...
			/*
			 * Convert decimal and fraction parts separately,
			 * then add them together to prevent overflows.
			 */
			int64_t decimal = ((int64_t)value[sample].val1) << (31 - header->shift);
			int64_t fraction =
				((((int64_t)value[sample].val2) << 31) / 1000000) >> header->shift;

			q[sample_idx + sample] = decimal + fraction;
