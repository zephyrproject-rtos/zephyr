...
static struct sensor_chan_spec default_blocking_read_channel[1];
			   ...
			   size_t num_channels, int8_t *shift, q31_t *values, size_t num_values)
{
	...
	rc = sensor_read(&default_blocking_rtio_dev, &default_blocking_rtio, read_buffer,
			 sizeof(read_buffer));
	if (rc != 0) {
	...
	struct rtio_cqe *cqe = rtio_cqe_consume_block(&default_blocking_rtio);
	rc = cqe->result;
	rtio_cqe_release(&default_blocking_rtio, cqe);

	if (rc != 0) {
		LOG_ERR("Read operation failed");
		return rc;
	}
	...
	rc = decoder->get_frame_count(read_buffer, chan, &frame_count);
	if (rc != 0) {
	...
	size_t value_idx = 0;
	for (size_t channel_idx = 0; i < num_channels && value_idx < num_values; ++i) {
		size_t base_size;
		size_t frame_size;
		uint32_t fit = 0;
