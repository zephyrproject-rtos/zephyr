...
static int adcxx1c_read_reg(const struct device *dev, uint8_t reg, uint8_t *out)
{
	...
	return i2c_write_read_dt(&cfg->bus, &reg, 1, out, 1);
}
		    ...
		    const struct sensor_value *val)
{
	...
	switch (attr) {
	case SENSOR_ATTR_LOWER_THRESH:
		return adcxx1c_write_regs(dev, ADCXX1C_LOW_LIMIT_ADDR, sval << (ADCXX1C_RES_12BITS - data->bits));
	case SENSOR_ATTR_UPPER_THRESH:
		return adcxx1c_write_regs(dev, ADCXX1C_HIGH_LIMIT_ADDR, sval << (ADCXX1C_RES_12BITS - data->bits));
	case SENSOR_ATTR_HYSTERESIS:
		return adcxx1c_write_regs(dev, ADCXX1C_HYSTERESIS_ADDR, sval << (ADCXX1C_RES_12BITS - data->bits));
	case SENSOR_ATTR_ALERT:
...
static void adcxx1c_gpio_callback(const struct device *dev,
...
int adcxx1c_init_interrupt(const struct device *dev)
{
	...
	gpio_init_callback(&data->alert_cb, adcxx1c_gpio_callback,
	...
	k_thread_create(&data->thread, data->thread_stack,
			CONFIG_ADCXX1C_THREAD_STACK_SIZE,
			adcxx1c_thread, data,
			NULL, NULL, K_PRIO_COOP(CONFIG_ADCXX1C_THREAD_PRIORITY),
			0, K_NO_WAIT);
#elif defined(CONFIG_ADCXX1C_TRIGGER_GLOBAL_THREAD)
