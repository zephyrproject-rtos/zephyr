...
int adcxx1c_read_regs(const struct device *dev, uint8_t reg, int16_t *out)
{
	...
	if (i2c_transfer_dt(&cfg->bus, msgs, 2) != 0) {
		return -EIO;
	}
...
static int adcxx1c_sample_fetch(const struct device *dev,
...
static int adcxx1c_channel_get(const struct device *dev,
			      enum sensor_channel chan,
			      struct sensor_value *val)
{
	...
	if (chan == SENSOR_CHAN_VOLTAGE) {
		val->val1 = (int32_t)(data->v_sample >> (ADCXX1C_RES_12BITS - data->bits)) & GENMASK(data->bits-1, 0);
		val->val2 = 0;
	} else {
		return -ENOTSUP;
	}
...
{
	if (res != ADCXX1C_RES_8BITS &&
	    res != ADCXX1C_RES_10BITS &&
	    res != ADCXX1C_RES_12BITS) {
...
#define ADCXX1C_DEFINE(inst)									\
	...
	static const struct adcxx1c_config adcxx1c_config_##inst = {				\
		...
		ADCXX1C_TRIGGER_INIT(inst)							\
												...
												\
	SENSOR_DEVICE_DT_INST_DEFINE(inst, adcxx1c_init, NULL,					\
			      &adcxx1c_data_##inst, &adcxx1c_config_##inst, POST_KERNEL,	\
