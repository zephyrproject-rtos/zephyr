...
/**
 * Configure SMUX for channels F1–F4.
 *
 * Writes the predefined SMUX configuration sequence to route
 * spectral channels F1, F2, F3, and F4 to the ADC inputs of
 * the AS7341 sensor.
 *
 * @param dev Pointer to the AS7341 device structure.
 *
 * @retval 0 On success.
 * @retval -errno Negative error code returned by the underlying
 *                SMUX write operation on failure.
 */
...
/**
 * Configure SMUX for channels F5–F8.
 *
 * Writes the predefined SMUX configuration sequence to route
 * spectral channels F5, F6, F7, and F8 to the ADC inputs of
 * the AS7341 sensor.
 *
 * @param dev Pointer to the AS7341 device structure.
 *
 ...
 *
 * Writes the predefined SMUX configuration sequence to route the
 * flicker detection channel to ADC5 of the AS7341 sensor.
 *
 * @param dev Pointer to the AS7341 device structure.
 *
 * @retval 0 On success.
...
/**
 * @brief Get a spectral channel reading from the AS7341.
 *
 * Implements sensor_channel_get().
 */
 ...
 * 2. Reads and validates the chip ID register (expected 0x09 per datasheet).
 ...
 * 5. Enables the internal LED driver (register 0x70, bit 3, LED_SEL = 1).
 * 6. Sets the LED current limit: register 0x74 = 0x80 (LED_ACT = 1, LED_DRIVE = 0 → 4 mA).
 ...
 * 10. Sets CFG0 = 0x20 (REG_BANK = 0, LOW_POWER = 1; normal operating mode).
 *
 * Integration time in milliseconds is calculated as:
 * \[
 * T_{int} = (ATIME + 1) \times (ASTEP + 1) \times 2.78 \, \mu s
 * \]
 * With ATIME = 100 and ASTEP = 999:
 * \[
 * T_{int} = 101 \times 1000 \times 2.78 \, \mu s = 280.78 \, ms
 * \]
 *
 * @param dev Pointer to the sensor device structure.
