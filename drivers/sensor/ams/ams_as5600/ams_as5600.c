...
static int as5600_sample_fetch_chan(const struct device *dev, enum sensor_channel chan)
{
	...
	if (chan == SENSOR_CHAN_ALL) {
		...
		err = i2c_burst_read_dt(&config->i2c, AS5600_RAW_ANGLE_H, buf, AS5600_TWO_REG);
		if (err < 0) {
			return err;
		}

		err = i2c_burst_read_dt(&config->i2c, AS5600_SCALED_ANGLE_H, &buf[2],
					AS5600_TWO_REG);
		if (err < 0) {
			return err;
		}

		data->raw_angle = AS5600_REG_TO_12BIT(buf[0], AS5600_12BIT_H_MASK, buf[1]);
		data->scaled_angle = AS5600_REG_TO_12BIT(buf[2], AS5600_12BIT_H_MASK, buf[3]);
	...
	case AS5600_SENSOR_CHAN_RAW_ANGLE: {
		err = i2c_burst_read_dt(&config->i2c, AS5600_RAW_ANGLE_H, buf, AS5600_TWO_REG);
		if (err < 0) {
			return err;
		}
	...
	case AS5600_SENSOR_CHAN_SCALED_ANGLE: {
		err = i2c_burst_read_dt(&config->i2c, AS5600_SCALED_ANGLE_H, buf, AS5600_TWO_REG);
		if (err < 0) {
			return err;
		}

		data->scaled_angle = AS5600_REG_TO_12BIT(buf[0], AS5600_12BIT_H_MASK, buf[1]);
		break;
	}
 ...
 * in the provided sensor_value structure. If all channels are requested, the caller
 * must ensure that there is space for 4 sensor_value structures.
 *
