 ...
 */
bool handleError(const AS5047P_Types::ERROR_t &error, bool *errorOut = nullptr) {
    if (errorOut != nullptr) {
...
bool readAngleDegree(const AS5047P_handle h, float *angleOut, bool withDAEC, bool verifyParity, bool checkForComError,
    ...
    AS5047P dev(h);
    AS5047P_Types::ERROR_t error;
    *angleOut = dev.readAngleDegree(withDAEC, &error, verifyParity, checkForComError, checkForSensorError);
    return handleError(error);
}
