...
static int bmm150_set_power_mode(struct device *dev,
				 ...
				 int state)
{
	struct bmm150_data *data = dev->fixed->driver_data;
	const struct bmm150_config *config = dev->fixed->config_info;
...
static int bmm150_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	struct bmm150_data *drv_data = dev->fixed->driver_data;
	const struct bmm150_config *config = dev->fixed->config_info;
...
static int bmm150_attr_set(struct device *dev,
			   ...
			   const struct sensor_value *val)
{
	struct bmm150_magn_data *data = dev->fixed->driver_data;
