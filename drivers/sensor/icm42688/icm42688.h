 ...
 */
static inline void icm42688_accel_g(struct icm42688_cfg *cfg, int32_t in, int32_t *out_g, uint32_t *out_ug)
{
 ...
 */
static inline void icm42688_gyro_dps(struct icm42688_cfg *cfg, int32_t in, int32_t *out_dps, uint32_t *out_udps)
{
	...
	*out_dps = in10/sensitivity;
	...
	/* Micro deg/s */
 	*out_udps = ((int64_t)(llabs(in10) - (llabs((*out_dps))*sensitivity))*1000000LL)/sensitivity;
}


/**
 * @brief The value of gravitational constant in micro m/s^2.
 * TODO this is redefined because sensor v2 redefines SENSOR_G
 */
#define ICM42688_G 9806650LL

/**
 * @brief The value of constant PI in micros.
 * TODO this is redefined because sensor v2 redefines SENSOR_PI
 */
#define ICM42688_PI 3141592LL
 ...
 */
static inline void icm42688_accel_ms(struct icm42688_cfg *cfg, int32_t in, int32_t *out_ms, uint32_t *out_ums)
{
 ...
 */
static inline void icm42688_gyro_rads(struct icm42688_cfg *cfg, int32_t in, int32_t *out_rads, uint32_t *out_urads)
{
	int64_t sensitivity = 0; /* value equivalent for 10x gyro reading deg/s */
	...
	int64_t in10_rads = (int64_t)in*ICM42688_PI*10LL;
	
	/* Whole rad/s */
	*out_rads = in10_rads/(sensitivity*180LL*1000000LL);
	
	/* microrad/s */
 	*out_urads = ((llabs(in10_rads) - (llabs((*out_rads))*sensitivity*180LL*1000000LL)))/(sensitivity * 180LL);
}
