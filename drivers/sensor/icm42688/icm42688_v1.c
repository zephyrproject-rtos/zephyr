			     ...
			     enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	/* TODO have this write the internal config then write it out with icm42688_configure
	switch (chan) {
	case SENSOR_CHAN_ACCEL_X:
	case SENSOR_CHAN_ACCEL_Y:
	case SENSOR_CHAN_ACCEL_Z:
	case SENSOR_CHAN_ACCEL_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			res = icm42688_set_accel_odr(dev, data->accel_hz);
			...
			res = icm42688_set_accel_fs(dev, data->accel_fs);
	...
	case SENSOR_CHAN_GYRO_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			res = icm42688_set_gyro_odr(dev, data->gyro_hz);
			...
			res = icm42688_set_gyro_fs(dev, data->gyro_fs);
	...
	icm42688_configure(dev, data->dev_datacfg)

	icm42688_unlock(dev);

	*/
	
	/* Use the icm42688_configure instead */
			     ...
			     enum sensor_attribute attr, struct sensor_value *val)
{
	...
	/* TODO have this update the internal config then write it out with icm42688_configure
	icm42688_lock(dev);
...
int icm42688_init(const struct device *dev)
{
	...
	int res = icm42688_configure(dev, &data->dev_data.cfg);
