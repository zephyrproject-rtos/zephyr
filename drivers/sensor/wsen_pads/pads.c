...
LOG_MODULE_REGISTER(PADS, CONFIG_SENSOR_LOG_LEVEL);

/*
 * List of supported output data rates. Index into this list is used as
 * argument for PADS_setOutputDataRate()
 */
static const int32_t pads_odr_list[] = { 0, 1, 10, 25, 50, 75, 100, 200 };
...
static int pads_sample_fetch(const struct device *dev, enum sensor_channel channel)
{
	...
	if (WE_SUCCESS != PADS_getPressure_int(&data->sensor_interface, &data->pressure)) {
		LOG_ERR("Failed to fetch pressure sample.");
		return -EIO;
	}

	if (WE_SUCCESS != PADS_getTemperature_int(&data->sensor_interface, &data->temperature)) {
...
static int pads_odr_set(const struct device *dev, const struct sensor_value *odr)
{
	...
	if (WE_SUCCESS !=
	    PADS_setOutputDataRate(&data->sensor_interface, (PADS_outputDataRate_t)odr_index)) {
...
static const struct sensor_driver_api pads_driver_api = { .attr_set = pads_attr_set,
#if CONFIG_PADS_TRIGGER
							  .trigger_set = pads_trigger_set,
#endif
							  .sample_fetch = pads_sample_fetch,
							  .channel_get = pads_channel_get };

int pads_init(const struct device *dev)
{
	...
	if (WE_SUCCESS != PADS_getDeviceID(&data->sensor_interface, &device_id)) {
	...
	do {
		if (WE_SUCCESS != PADS_getSoftResetState(&data->sensor_interface, &swReset)) {
			LOG_ERR("Failed to get sensor reset state.");
			return -EIO;
		}
	} while (PADS_enable == swReset);

	if (WE_SUCCESS != PADS_enableBlockDataUpdate(&data->sensor_interface, PADS_enable)) {
...
#define PADS_DEFINE(inst)                                                             \
	...
	static const struct pads_config pads_config_##inst =                          \
		COND_CODE_1(DT_INST_ON_BUS(inst, spi),                                \
				(PADS_CONFIG_SPI(inst)),                              \
				(PADS_CONFIG_I2C(inst)));                             \
