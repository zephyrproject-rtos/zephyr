...
LOG_MODULE_DECLARE(PADS, CONFIG_SENSOR_LOG_LEVEL);
...
#if defined(CONFIG_PADS_TRIGGER_OWN_THREAD)
	k_sem_give(&data->drdy_sem);
#elif defined(CONFIG_PADS_TRIGGER_GLOBAL_THREAD)
		     ...
		     sensor_trigger_handler_t handler)
{
	...
	if (handler == NULL) {
		...
		if (WE_SUCCESS !=
		    PADS_enableDataReadyInterrupt(&data->sensor_interface, PADS_disable)) {
	...
	if (WE_SUCCESS != PADS_getPressure_int(&data->sensor_interface, &pressure_dummy)) {
		LOG_ERR("Failed to read sample");
		return -EIO;
	}

	/* Enable data-ready interrupt */
	if (WE_SUCCESS != PADS_enableDataReadyInterrupt(&data->sensor_interface, PADS_enable)) {
...
#ifdef CONFIG_PADS_TRIGGER_OWN_THREAD
...
#endif /* CONFIG_PADS_TRIGGER_OWN_THREAD */

#ifdef CONFIG_PADS_TRIGGER_GLOBAL_THREAD
...
#endif /* CONFIG_PADS_TRIGGER_GLOBAL_THREAD */
...
int pads_init_interrupt(const struct device *dev)
{
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_PADS_THREAD_STACK_SIZE,
			(k_thread_entry_t)pads_thread, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_PADS_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_PADS_TRIGGER_GLOBAL_THREAD)
