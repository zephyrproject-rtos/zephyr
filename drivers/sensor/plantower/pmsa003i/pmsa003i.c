...
/*
 * Polling interval between I2C read attempts when waiting for a valid
 * data frame (identified by start bytes 0x42 0x4d).
 */
#define PMSA003I_POLL_INTERVAL_MS 10
				     ...
				     uint8_t *buf, size_t len)
{
	...
	while (k_uptime_get() - start_time < timeout_ms) {
		...
		k_sleep(K_MSEC(PMSA003I_POLL_INTERVAL_MS));
	}
