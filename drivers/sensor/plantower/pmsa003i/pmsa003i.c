...
/*
 * The data manual names the reset pin and its polarity but gives it no
 * timing, so hold it for a millisecond, which is far longer than the pin
 * needs and short enough not to be felt at init.
 */
#define PMSA003I_RESET_PULSE_MS 1U
...
/*
 * Resolve a channel onto the reading that holds it, or NULL where the sensor
 * has nothing to say. Both entry points ask the same question, so they ask it
 * in the same place.
 */
static uint16_t *pmsa003i_channel_value(struct pmsa003i_data *drv_data,
					enum sensor_channel chan)
{
	switch ((int)chan) {
	case SENSOR_CHAN_PM_1_0:
		return &drv_data->pm_1_0;
	case SENSOR_CHAN_PM_2_5:
		return &drv_data->pm_2_5;
	case SENSOR_CHAN_PM_10:
		return &drv_data->pm_10;
	case SENSOR_CHAN_PM_1_0_CF:
		return &drv_data->pm_1_0_cf;
	case SENSOR_CHAN_PM_2_5_CF:
		return &drv_data->pm_2_5_cf;
	case SENSOR_CHAN_PM_10_CF:
		return &drv_data->pm_10_cf;
	case SENSOR_CHAN_PM_0_3_COUNT:
		return &drv_data->pm_0_3_count;
	case SENSOR_CHAN_PM_0_5_COUNT:
		return &drv_data->pm_0_5_count;
	case SENSOR_CHAN_PM_1_0_COUNT:
		return &drv_data->pm_1_0_count;
	case SENSOR_CHAN_PM_2_5_COUNT:
		return &drv_data->pm_2_5_count;
	case SENSOR_CHAN_PM_5_COUNT:
		return &drv_data->pm_5_0_count;
	case SENSOR_CHAN_PM_10_COUNT:
		return &drv_data->pm_10_0_count;
	default:
		return NULL;
	}
}
...
static int pmsa003i_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	/*
	 * The sensor hands over every reading in one frame, so a request for
	 * any single one of them fetches the lot; a channel it does not carry
	 * is still worth turning down.
	 */
	if (chan != SENSOR_CHAN_ALL && pmsa003i_channel_value(drv_data, chan) == NULL) {
		return -ENOTSUP;
	}
	...
	if ((pmsa003i_receive_buffer[0] != PMSA003I_START_BYTE_1) ||
		...
		LOG_ERR("invalid start bytes: 0x%02x 0x%02x", pmsa003i_receive_buffer[0],
	...
	if (frame_length != PMSA003I_FRAME_LENGTH) {
		LOG_ERR("invalid frame length: %u", frame_length);
		return -EIO;
	}
	...
	if (calculated_checksum != received_checksum) {
		LOG_ERR("invalid checksum: calculated 0x%04x, received 0x%04x", calculated_checksum,
			received_checksum);
...
{
	struct pmsa003i_data *drv_data = dev->data;
	const uint16_t *reading = pmsa003i_channel_value(drv_data, chan);
...
static int pmsa003i_init(const struct device *dev)
{
	...
	if (cfg->reset_gpio.port != NULL) {
		...
		k_msleep(PMSA003I_RESET_PULSE_MS);
