		...
		uint16_t *reg_data)
{
	...
	rc = i2c_write_read(cfg->bus, cfg->i2c_addr,
			       ...
			       enum sensor_channel chan)
{
	...
	uint16_t tmp;
	int rc;
	enum sensor_channel_ina219 chan_p = (enum sensor_channel_ina219)chan;

	if (chan != SENSOR_CHAN_ALL &&
		chan_p != SENSOR_CHAN_INA219_V_SHUNT &&
		chan_p != SENSOR_CHAN_INA219_BUS_VOLTAGE &&
		chan_p != SENSOR_CHAN_INA219_BUS_POWER &&
		chan_p != SENSOR_CHAN_INA219_BUS_CURRENT) {
		return -ENOTSUP;
	}
			      ...
			      struct sensor_value *val)
{
	...
	switch ((enum sensor_channel_ina219) chan) {
	case SENSOR_CHAN_INA219_V_SHUNT:
		if (INA219_SIGN_BIT(data->v_shunt)) {
			data->v_shunt = ~data->v_shunt + 1;
			sign = -1;
		}
		tmp = sign * data->v_shunt * INA219_SI_MUL;
		sensor_value_from_double(val, tmp);
		break;
	case SENSOR_CHAN_INA219_BUS_VOLTAGE:
		tmp = data->v_bus * INA219_V_BUS_MUL;
		sensor_value_from_double(val, tmp);
		break;
	case SENSOR_CHAN_INA219_BUS_POWER:
		tmp = data->power * cfg->current_lsb * INA219_POWER_MUL * INA219_SI_MUL;
		sensor_value_from_double(val, tmp);
		break;
	case SENSOR_CHAN_INA219_BUS_CURRENT:
...
static int ina219_set_power_state(const struct device *dev,
				  enum pm_device_state power_state)
{
	...
	} else if (power_state == PM_DEVICE_STATE_OFF) {
...
static uint32_t ina219_get_power_state(const struct device *dev,
...
static int ina219_pm_ctrl(const struct device *dev,
	...
	enum pm_device_state *state)
{
	...
	if (ctrl_command == PM_DEVICE_STATE_SET) {
		rc = ina219_set_power_state(dev, *state);
	} else if (ctrl_command == PM_DEVICE_STATE_GET) {
		rc = ina219_get_power_state(dev, state);
	}
