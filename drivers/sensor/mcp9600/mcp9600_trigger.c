...
LOG_MODULE_DECLARE(MCP9600, CONFIG_SENSOR_LOG_LEVEL);

static int mcp9600_reg_write(struct device *dev, uint8_t reg, uint16_t val)
{
	const struct mcp9600_data *data = dev->data;
...
int mcp9600_attr_set(struct device *dev, enum sensor_channel chan,
		     ...
		     const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LOWER_THRESH:
		reg_addr = MCP9600_REG_LOWER_LIMIT;
		break;
	case SENSOR_ATTR_UPPER_THRESH:
		reg_addr = MCP9600_REG_UPPER_LIMIT;
		break;
	default:
		return -EINVAL;
	}
	...
	temp += (MCP9600_TEMP_SCALE_CEL * val->val2) / 1000000;

	return mcp9600_reg_write(dev, reg_addr,
				 mcp9600_temp_reg_from_signed(temp));
}
...
#if defined(CONFIG_MCP9600_TRIGGER_OWN_THREAD)
	k_sem_give(&data->sem);
#elif defined(CONFIG_MCP9600_TRIGGER_GLOBAL_THREAD)
...
int mcp9600_trigger_set(struct device *dev,
...
static void mcp9600_thread_main(int arg1, int arg2)
{
...
static void mcp9600_gpio_thread_cb(struct k_work *work)
{
	struct mcp9600_data *data =
		CONTAINER_OF(work, struct mcp9600_data, work);

	process_int(data->dev);
}

#endif /* CONFIG_MCP9600_TRIGGER_GLOBAL_THREAD */

int mcp9600_setup_interrupt(struct device *dev)
{
	struct mcp9600_data *data = dev->data;
	const struct mcp9600_config *cfg = dev->config;
	struct device *gpio;
