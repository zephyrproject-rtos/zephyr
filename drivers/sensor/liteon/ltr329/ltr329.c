...
static int ltr329_read_register(const struct i2c_dt_spec *bus, uint8_t reg, uint8_t *value)
{
	return i2c_write_read_dt(bus, &reg, sizeof(reg), value, sizeof(*value));
}
...
static int ltr329_check_device_id(const struct i2c_dt_spec *bus)
{
	...
	rc = ltr329_read_register(bus, LTR329_PART_ID, &id);
	if (rc < 0) {
	...
	rc = ltr329_read_register(bus, LTR329_MANUFAC_ID, &id);
	if (rc < 0) {
...
static int ltr329_init_registers(const struct i2c_dt_spec *bus, const struct ltr329_config *cfg)
{
	...
	rc = ltr329_send_command(bus, LTR329_ALS_CONTR, control_reg);
	if (rc < 0) {
		LOG_ERR("Failed to set ALS_CONTR register");
		return rc;
	}

	rc = ltr329_send_command(bus, LTR329_MEAS_RATE, meas_reg);
	if (rc < 0) {
...
static int ltr329_init(const struct device *dev)
{
	...
	/* Wait for sensor to wake up from standby - this time is required by datasheet */
