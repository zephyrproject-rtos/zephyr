...
{
	struct lps25hb_data *data = dev->fixed->driver_data;
	const struct lps25hb_config *config = dev->fixed->config_info;
...
{
	const struct lps25hb_config * const config = dev->fixed->config_info;
	struct lps25hb_data *data = dev->fixed->driver_data;
