				...
				struct sensor_value *val)
{
	...
	case SENSOR_CHAN_GYRO_X:
		...
		val->val2 = ((s32_t)data->x % ADXRS290_GYR_SCALE_FACT) * 1000000;
		break;
	case SENSOR_CHAN_GYRO_Y:
		/* Angular velocity around the Y axis, in radians/s. */
		val->val1 = (s32_t)data->y / ADXRS290_GYR_SCALE_FACT;
		val->val2 = ((s32_t)data->y % ADXRS290_GYR_SCALE_FACT) * 1000000;
		break;
	case SENSOR_CHAN_TEMP:
		/* Temperature in degrees Celsius. */
		val->val1 = (s32_t)data->temp / ADXRS290_TEMP_SCALE_FACT;
		val->val2 = ((s32_t)data->temp % ADXRS290_TEMP_SCALE_FACT) * 1000000;
		break;
	default:
		return -ENOTSUP;
	}
