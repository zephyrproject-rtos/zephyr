 ...
 */
static int32_t als31300_convert_to_gauss(const struct device *dev, int16_t raw_value)
{
	ARG_UNUSED(dev);
 ...
 */
static int32_t als31300_convert_temperature(uint16_t raw_temp)
{
				...
				struct sensor_value *val)
{
	...
	converted_val = als31300_convert_to_gauss(dev, raw_val);
...
/**
 * @brief RTIO synchronous submit function
 */
static void als31300_submit_sync(struct rtio_iodev_sqe *iodev_sqe)
{
	...
	/* Get buffer for encoded data */
	int rc = rtio_sqe_rx_buf(iodev_sqe, min_buf_len, min_buf_len, &buf, &buf_len);

	if (rc != 0) {
		LOG_ERR("RTIO buffer allocation failed: %d", rc);
	...
	if (rc != 0) {
		LOG_ERR("Failed to get timestamp: %d", rc);
		rtio_iodev_sqe_err(iodev_sqe, rc);
		return;
	}
	...
	/* Set channel flags based on requested channels */
	edata->has_magn_x = 0;
	edata->has_magn_y = 0;
	edata->has_magn_z = 0;
	edata->has_temp = 0;
	...
	/* Fetch sensor data */
	rc = als31300_read_sensor_data(dev, SENSOR_CHAN_ALL, &edata->readings);
	if (rc != 0) {
		LOG_ERR("Failed to fetch sensor data: %d", rc);
		rtio_iodev_sqe_err(iodev_sqe, rc);
		return;
	}
 ...
 */
static void als31300_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	rtio_work_req_submit(req, iodev_sqe, als31300_submit_sync);
}
				   ...
				   uint32_t *fit, uint16_t max_count, void *data_out)
{
	...
	case SENSOR_CHAN_MAGN_XYZ: {
		...
		als31300_convert_raw_to_q31_magn(edata->readings.x, &out->readings[0].x);
		als31300_convert_raw_to_q31_magn(edata->readings.y, &out->readings[0].y);
		als31300_convert_raw_to_q31_magn(edata->readings.z, &out->readings[0].z);
		*fit = 1;
		return 1;
	}
	case SENSOR_CHAN_AMBIENT_TEMP: {
		...
		als31300_convert_temp_to_q31(edata->readings.temp, &out->readings[0].temperature);
		*fit = 1;
		return 1;
	}
	default:
...
static int als31300_get_decoder(const struct device *dev, const struct sensor_decoder_api **decoder)
{
	ARG_UNUSED(dev);
...
#define ALS31300_INIT(inst)                                                                        \
	...
	static const struct als31300_config als31300_config_##inst = {                             \
		.i2c = I2C_DT_SPEC_INST_GET(inst),                                                 \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, als31300_init, NULL, &als31300_data_##inst,                    \
			      &als31300_config_##inst, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY,   \
			      &als31300_api)
