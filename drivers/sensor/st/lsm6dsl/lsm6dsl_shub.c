...
static int lsm6dsl_shub_write_slave_reg(const struct device *dev, uint8_t slv_addr, uint8_t slv_reg,
...
static int lsm6dsl_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 1);
	...
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 2);
...
static int lsm6dsl_lis3mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LIS3MDL_REG_CTRL2, mag_cfg, 1);

	k_sleep(K_MSEC(10)); /* turn-on time in ms */

	/* configure mag */
	mag_cfg[0] = LIS3MDL_ODR;
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LIS3MDL_REG_CTRL1, mag_cfg, 1);

	mag_cfg[0] = LIS3MDL_MD_CONTINUOUS;
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LIS3MDL_REG_CTRL3, mag_cfg, 1);
	return 0;
}
...
static int lsm6dsl_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LPS22HB_CTRL_REG2, baro_cfg, 1);

	k_sleep(K_MSEC(1)); /* turn-on time in ms */

	/* configure device */
	baro_cfg[0] = LPS22HB_ODR_10HZ | LPS22HB_LPF_EN | LPS22HB_BDU_EN;
	lsm6dsl_shub_write_slave_reg(dev, i2c_addr, LPS22HB_CTRL_REG1, baro_cfg, 1);
...
static inline void lsm6dsl_shub_embedded_en(const struct device *dev, bool on)
{
	...
	data->hw_tf->update_reg(dev, LSM6DSL_REG_FUNC_CFG_ACCESS, LSM6DSL_MASK_FUNC_CFG_EN,
...
#ifdef LSM6DSL_DEBUG
static int lsm6dsl_read_embedded_reg(const struct device *dev, uint8_t reg_addr, uint8_t *value,
				     int len)
{
	struct lsm6dsl_data *data = dev->data;
...
static int lsm6dsl_shub_write_embedded_regs(const struct device *dev, uint8_t reg_addr,
...
static void lsm6dsl_shub_enable(const struct device *dev)
{
	...
	/* Enable Digital Func */
	data->hw_tf->update_reg(dev, LSM6DSL_REG_CTRL10_C, LSM6DSL_MASK_CTRL10_C_FUNC_EN,
				1 << LSM6DSL_SHIFT_CTRL10_C_FUNC_EN);

	/* Enable Accel @26hz */
	if (!data->accel_freq) {
		data->hw_tf->update_reg(dev, LSM6DSL_REG_CTRL1_XL, LSM6DSL_MASK_CTRL1_XL_ODR_XL,
 ...
 */
static int lsm6dsl_shub_read_slave_reg(const struct device *dev, uint8_t slv_addr, uint8_t slv_reg,
				       uint8_t *value, uint16_t len)
{
	struct lsm6dsl_data *data = dev->data;
	uint8_t slave[3];
	...
	if (lsm6dsl_shub_write_embedded_regs(dev, LSM6DSL_EMBEDDED_SLV0_ADDR, slave, 3) < 0) {
					...
					uint8_t *value, uint16_t len)
{
	...
	while (cnt < len) {
		...
		if (lsm6dsl_shub_write_embedded_regs(dev, LSM6DSL_EMBEDDED_SLV0_ADDR, slv_cfg, 2) <
		    0) {
			LOG_DBG("error writing embedded reg");
			return -EIO;
		}
		...
		slv_cfg[0] = value[cnt];
		if (lsm6dsl_shub_write_embedded_regs(dev, LSM6DSL_EMBEDDED_SLV0_DATAWRITE, slv_cfg,
						     1) < 0) {
			LOG_DBG("error writing embedded reg");
			return -EIO;
		}
	...
	if (lsm6dsl_shub_write_embedded_regs(dev, LSM6DSL_EMBEDDED_SLV0_ADDR, slv_cfg, 3) < 0) {
		LOG_DBG("error writing embedded reg");
		return -EIO;
	}
...
static int lsm6dsl_shub_set_data_channel(const struct device *dev)
{
	...
	if (lsm6dsl_shub_write_embedded_regs(dev, LSM6DSL_EMBEDDED_SLV1_ADDR, slv_cfg, 3) < 0) {
		LOG_DBG("error writing embedded reg");
		return -EIO;
	}
...
int lsm6dsl_shub_read_external_chip(const struct device *dev, uint8_t *buf, uint8_t len)
{
	struct lsm6dsl_data *data = dev->data;
...
int lsm6dsl_shub_init_external_chip(const struct device *dev)
{
	...
	for (i = 0U; i < ARRAY_SIZE(lsm6dsl_shub_sens_list[0].i2c_addr); i++) {
		...
		if (lsm6dsl_shub_read_slave_reg(dev, slv_i2c_addr, slv_wai_addr, &chip_id, 1) < 0) {
