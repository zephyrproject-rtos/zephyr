...
int iis3dwb_init_interrupt(const struct device *dev)
{
	...
	iis3dwb->drdy_gpio = (cfg->drdy_pin == 1) ? (struct gpio_dt_spec *)&cfg->int1_gpio :
	...
	iis3dwb_dataready_pulsed_t drdy = (cfg->drdy_pulsed) ? IIS3DWB_DRDY_PULSED :
							       IIS3DWB_DRDY_LATCHED;
