...
static void iis3dwb_read_fifo_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
	...
	fifo_count = (uint16_t)(iis3dwb->fifo_status[0] |
			       (iis3dwb->fifo_status[1] & 0x3) << 8);
	...
	if (data_opt == SENSOR_STREAM_DATA_NOP || data_opt == SENSOR_STREAM_DATA_DROP) {
		...
		if (data_opt == SENSOR_STREAM_DATA_DROP) {
			...
			rtio_sqe_prep_tiny_write(write_fifo_mode, iodev,
						 RTIO_PRIO_NORM, iis3dwb_fifo_mode_set,
	...
	uint8_t fifo_data_out[]  = { IIS3DWB_FIFO_DATA_OUT_TAG, };
	struct spi_buf fifo_buf[] = { {read_buf, buf_avail} };
...
static void iis3dwb_read_status_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
	...
	if (iis3dwb->status & 0x1) {
		...
		uint8_t drdy_data_out[]  = { IIS3DWB_OUTX_L_A, };
		struct spi_buf drdy_buf[] = { {read_buf, 6} };
...
void iis3dwb_stream_irq_handler(const struct device *dev)
{
	...
	if (iis3dwb->trig_cfg.int_drdy) {
		...
		uint8_t drdy_regs[]  = { IIS3DWB_STATUS_REG, };
		struct spi_buf drdy_buf[] = { {&iis3dwb->status, 1}, };
