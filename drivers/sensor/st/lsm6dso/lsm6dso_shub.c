...
static int lsm6dso_shub_write_target_reg(const struct device *dev, uint8_t trgt_addr,
					 uint8_t trgt_reg, uint8_t *value, uint16_t len);
static int lsm6dso_shub_read_target_reg(const struct device *dev, uint8_t trgt_addr,
					uint8_t trgt_reg, uint8_t *value, uint16_t len);
...
static int lsm6dso_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 1);
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 2);
...
static int lsm6dso_lis2mdl_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	cfg = (odr << 2);
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, &cfg, 1);
...
static int lsm6dso_hts221_read_conv_data(const struct device *dev, uint8_t i2c_addr)
{
	struct lsm6dso_data *data = dev->data;
	uint8_t buf[16], i;
...
static int lsm6dso_hts221_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &hum_cfg, 1);
...
static int lsm6dso_hts221_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &cfg, 1);

	lsm6dso_shub_enable(dev, 1);
	return 0;
}

static int lsm6dso_hts221_conf(const struct device *dev, uint8_t i2c_addr, enum sensor_channel chan,
...
static int lsm6dso_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG2, baro_cfg, 1);

	k_sleep(K_MSEC(1)); /* turn-on time in ms */

	/* configure device */
	baro_cfg[0] = LPS22HB_ODR_10HZ | LPS22HB_LPF_EN | LPS22HB_BDU_EN;
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG1, baro_cfg, 1);
...
static int lsm6dso_lps22hh_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG2, baro_cfg, 1);
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, baro_cfg, 1);
...
static int lsm6dso_lps22hh_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dso_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, &cfg, 1);
...
int lsm6dso_shub_fetch_external_devs(const struct device *dev)
{
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		...
		if (lsm6dso_read_reg(ctx, sp->sh_out_reg, data->ext_data[n], sp->out_data_len) <
		    0) {
...
int lsm6dso_shub_init(const struct device *dev)
{
	...
	for (n = 0; n < ARRAY_SIZE(lsm6dso_shub_slist); n++) {
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (lsm6dso_shub_read_target_reg(dev, sp->i2c_addr[i], sp->wai_addr,
							 &chip_id, 1) < 0) {
