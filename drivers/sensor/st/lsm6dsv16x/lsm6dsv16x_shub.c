...
static int lsm6dsv16x_shub_write_target_reg(const struct device *dev, uint8_t trgt_addr,
					    uint8_t trgt_reg, uint8_t *value, uint16_t len);
static int lsm6dsv16x_shub_read_target_reg(const struct device *dev, uint8_t trgt_addr,
					   uint8_t trgt_reg, uint8_t *value, uint16_t len);
...
static int lsm6dsv16x_lis2mdl_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 1);
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, mag_cfg, 2);
...
static int lsm6dsv16x_lis2mdl_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	cfg = (odr << 2);
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LIS2MDL_CFG_REG_A, &cfg, 1);
...
static int lsm6dsv16x_hts221_read_conv_data(const struct device *dev, uint8_t i2c_addr)
{
	struct lsm6dsv16x_data *data = dev->data;
	uint8_t buf[16], i;
	...
	for (i = 0; i < sizeof(buf); i += 7) {
		...
		if (lsm6dsv16x_shub_read_target_reg(
			    dev, i2c_addr, (HTS221_REG_CONV_START + i) | HTS221_AUTOINCREMENT,
			    &buf[i], len) < 0) {
...
static int lsm6dsv16x_hts221_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &hum_cfg, 1);
...
static int lsm6dsv16x_hts221_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, HTS221_REG_CTRL1, &cfg, 1);
...
static int lsm6dsv16x_lps22hb_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG2, baro_cfg, 1);

	k_sleep(K_MSEC(1)); /* turn-on time in ms */

	/* configure device */
	baro_cfg[0] = LPS22HB_ODR_10HZ | LPS22HB_LPF_EN | LPS22HB_BDU_EN;
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22HB_CTRL_REG1, baro_cfg, 1);
...
static int lsm6dsv16x_lps22hh_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG2, baro_cfg, 1);
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, baro_cfg, 1);
...
static int lsm6dsv16x_lps22hh_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22HH_CTRL_REG1, &cfg, 1);
...
static int lsm6dsv16x_lps22df_init(const struct device *dev, uint8_t i2c_addr)
{
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG2, baro_cfg, 1);
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG1, baro_cfg, 1);
...
static int lsm6dsv16x_lps22df_odr_set(const struct device *dev, uint8_t i2c_addr, uint16_t freq)
{
	uint8_t odr, cfg;
	...
	lsm6dsv16x_shub_write_target_reg(dev, i2c_addr, LPS22DF_CTRL_REG1, &cfg, 1);
...
int lsm6dsv16x_shub_fetch_external_devs(const struct device *dev)
{
	...
	for (n = 0; n < data->num_ext_dev; n++) {
		...
		if (lsm6dsv16x_read_reg(ctx, sp->sh_out_reg, data->ext_data[n], sp->out_data_len) <
		    0) {
...
int lsm6dsv16x_shub_init(const struct device *dev)
{
	...
	for (n = 0; n < ARRAY_SIZE(lsm6dsv16x_shub_slist); n++) {
		...
		for (i = 0U; i < ARRAY_SIZE(sp->i2c_addr); i++) {
			if (lsm6dsv16x_shub_read_target_reg(dev, sp->i2c_addr[i], sp->wai_addr,
							    &chip_id, 1) < 0) {
