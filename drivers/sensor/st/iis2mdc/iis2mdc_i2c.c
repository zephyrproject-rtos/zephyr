...
static int iis2mdc_i2c_read(const struct device *dev, uint8_t reg_addr, uint8_t *value,
			    uint16_t len)
{
...
static int iis2mdc_i2c_write(const struct device *dev, uint8_t reg_addr, uint8_t *value,
			     uint16_t len)
{
	const struct iis2mdc_dev_config *cfg = dev->config;
