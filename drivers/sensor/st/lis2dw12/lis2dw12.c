...
static inline void lis2dw12_channel_get_status(const struct device *dev, struct sensor_value *val)
{
	const struct lis2dw12_device_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
...
static int lis2dw12_attr_set_act_mode(const struct device *dev, enum sensor_channel chan,
				      enum sensor_attribute attr, const struct sensor_value *val)
{
	const struct lis2dw12_device_config *cfg = dev->config;
	struct lis2dw12_data *lis2dw12 = dev->data;
...
#ifdef CONFIG_LIS2DW12_WAKEUP
#define LIS2DW12_CONFIG_WAKEUP(inst) .wakeup_duration = DT_INST_PROP(inst, wakeup_duration),
#else
#define LIS2DW12_CONFIG_WAKEUP(inst)
#endif

#ifdef CONFIG_LIS2DW12_SLEEP
#define LIS2DW12_CONFIG_SLEEP(inst) .sleep_duration = DT_INST_PROP(inst, sleep_duration),
#else
...
#define LIS2DW12_CONFIG_COMMON(inst)                                                               \
	...
	LIS2DW12_CONFIG_TAP(inst) LIS2DW12_CONFIG_FREEFALL(inst) LIS2DW12_CONFIG_WAKEUP(inst)      \
...
#define LIS2DW12_DEFINE(inst)                                                                      \
	...
	static const struct lis2dw12_device_config lis2dw12_config_##inst = COND_CODE_1(DT_INST_ON_BUS(inst, spi),				\
