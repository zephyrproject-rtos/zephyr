...
void lsm6dsvxxx_submit_stream(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	if (!ON_I3C_BUS(config) || (I3C_INT_PIN(config))) {
		gpio_pin_interrupt_configure_dt(data->drdy_gpio, GPIO_INT_DISABLE);
	}
 ...
 */
static void lsm6dsvxxx_complete_op_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
 ...
 */
static void lsm6dsvxxx_read_fifo_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
	const struct device *dev = arg;
	const struct lsm6dsvxxx_config *cfg = dev->config;
	struct lsm6dsvxxx_data *data = dev->data;
	struct rtio *rtio = data->rtio_ctx;
	struct gpio_dt_spec *irq_gpio = data->drdy_gpio;
 ...
 */
static void lsm6dsvxxx_read_status_cb(struct rtio *r, const struct rtio_sqe *sqe, void *arg)
{
	const struct device *dev = arg;
	const struct lsm6dsvxxx_config *config = dev->config;
	struct lsm6dsvxxx_data *data = dev->data;
