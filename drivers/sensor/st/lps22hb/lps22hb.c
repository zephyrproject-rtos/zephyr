...
static int lps22hb_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_burst_read_dt(&config->i2c, LPS22HB_REG_PRESS_OUT_XL, out, 5) < 0) {
...
static int lps22hb_init_chip(const struct device *dev)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, LPS22HB_REG_WHO_AM_I, &chip_id) < 0) {
	...
	if (i2c_reg_update_byte_dt(&config->i2c, LPS22HB_REG_CTRL_REG1, LPS22HB_MASK_CTRL_REG1_BDU,
