...
static int lsm303dlhc_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (i2c_reg_read_byte_dt(&config->i2c, LSM303DLHC_SR_REG_M, &status) < 0) {
	...
	if (i2c_burst_read_dt(&config->i2c, LSM303DLHC_REG_MAGN_X_LSB, magn_buf, 6) < 0) {
...
static void lsm303dlhc_convert_xy(struct sensor_value *val, int64_t raw_val)
{
	val->val1 = raw_val / LSM303DLHC_MAGN_LSB_GAUSS_XY;
	val->val2 = (1000000 * raw_val / LSM303DLHC_MAGN_LSB_GAUSS_XY) % 1000000;
}

static void lsm303dlhc_convert_z(struct sensor_value *val, int64_t raw_val)
{
	val->val1 = raw_val / LSM303DLHC_MAGN_LSB_GAUSS_Z;
	val->val2 = (1000000 * raw_val / LSM303DLHC_MAGN_LSB_GAUSS_Z) % 1000000;
}

static int lsm303dlhc_channel_get(const struct device *dev, enum sensor_channel chan,
...
static int lsm303dlhc_magn_init(const struct device *dev)
{
	...
	if (i2c_reg_write_byte_dt(&config->i2c, LSM303DLHC_CRA_REG_M, LSM303DLHC_MAGN_ODR_BITS) <
	    0) {
	...
	if (i2c_reg_write_byte_dt(&config->i2c, LSM303DLHC_CRB_REG_M, LSM303DLHC_MAGN_FS_BITS) <
	    0) {
	...
	/* Continuous update */
	if (i2c_reg_write_byte_dt(&config->i2c, LSM303DLHC_MR_REG_M, LSM303DLHC_MAGN_CONT_UPDATE) <
	    0) {
