...
static const uint16_t lsm6dso16is_odr_map[] = {0, 12, 26, 52, 104, 208, 416, 833, 1667, 3333, 6667};
...
static int lsm6dso16is_accel_config(const struct device *dev, enum sensor_channel chan,
...
static int lsm6dso16is_gyro_config(const struct device *dev, enum sensor_channel chan,
				   enum sensor_attribute attr, const struct sensor_value *val)
{
	const struct lsm6dso16is_config *cfg = dev->config;
	stmdev_ctx_t *ctx = (stmdev_ctx_t *)&cfg->ctx;
...
static int lsm6dso16is_attr_set(const struct device *dev, enum sensor_channel chan,
				enum sensor_attribute attr, const struct sensor_value *val)
{
...
static int lsm6dso16is_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
#if defined(CONFIG_LSM6DSO16IS_SENSORHUB)
	struct lsm6dso16is_data *data = dev->data;
#endif /* CONFIG_LSM6DSO16IS_SENSORHUB */
...
static inline int lsm6dso16is_accel_get_channel(enum sensor_channel chan, struct sensor_value *val,
						struct lsm6dso16is_data *data, uint32_t sensitivity)
{
	uint8_t i;
...
static int lsm6dso16is_accel_channel_get(enum sensor_channel chan, struct sensor_value *val,
...
static inline int lsm6dso16is_gyro_get_channel(enum sensor_channel chan, struct sensor_value *val,
					       struct lsm6dso16is_data *data, uint32_t sensitivity)
{
	uint8_t i;
...
static int lsm6dso16is_gyro_channel_get(enum sensor_channel chan, struct sensor_value *val,
					struct lsm6dso16is_data *data)
{
...
static inline int lsm6dso16is_magn_get_channel(enum sensor_channel chan, struct sensor_value *val,
					       struct lsm6dso16is_data *data)
{
	int16_t sample[3];
	int idx;
...
static inline void lsm6dso16is_hum_convert(struct sensor_value *val, struct lsm6dso16is_data *data)
{
	float rh;
	int16_t raw_val;
...
static inline void lsm6dso16is_temp_convert(struct sensor_value *val, struct lsm6dso16is_data *data)
{
	int16_t raw_val;
	int idx;
...
static int lsm6dso16is_channel_get(const struct device *dev, enum sensor_channel chan,
...
#define LSM6DSO16IS_DEVICE_INIT(inst)                                                        \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, lsm6dso16is_init, NULL, &lsm6dso16is_data_##inst, \
				     &lsm6dso16is_config_##inst, POST_KERNEL,                \
				     CONFIG_SENSOR_INIT_PRIORITY, &lsm6dso16is_driver_api);
...
#define LSM6DSO16IS_SPI_OP (SPI_WORD_SET(8) | SPI_OP_MODE_MASTER | SPI_MODE_CPOL | SPI_MODE_CPHA)
...
#define LSM6DSO16IS_CONFIG_SPI(inst)                                               \
	{STMEMSC_CTX_SPI(&lsm6dso16is_config_##inst.stmemsc_cfg),                  \
	 .stmemsc_cfg =                                                            \
		 {                                                                 \
			 .spi = SPI_DT_SPEC_INST_GET(inst, LSM6DSO16IS_SPI_OP, 0), \
		 },                                                                \
