...
static inline void adc_enable_vrefsensor_channel(ADC_TypeDef *adc)
{
...
static inline void adc_disable_vrefsensor_channel(ADC_TypeDef *adc)
{
	const uint32_t path = LL_ADC_GetCommonPathInternalCh(__LL_ADC_COMMON_INSTANCE(adc));
...
static int stm32_vref_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	adc_enable_vrefsensor_channel(data->adc_base);
...
#ifndef CONFIG_STM32_VREF_INJECTED
	adc_disable_vrefsensor_channel(data->adc_base);
