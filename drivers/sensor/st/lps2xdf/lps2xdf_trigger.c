...
static void lps2xdf_gpio_callback(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	struct lps2xdf_data *lps2xdf = CONTAINER_OF(cb, struct lps2xdf_data, gpio_cb);
...
{
	struct lps2xdf_data *lps2xdf = CONTAINER_OF(work, struct lps2xdf_data, work);
	const struct device *dev = lps2xdf->dev;
	const struct lps2xdf_config *const cfg = dev->config;
	const struct lps2xdf_chip_api *chip_api = cfg->chip_api;
...
int lps2xdf_init_interrupt(const struct device *dev, enum sensor_variant variant)
{
	...
	k_thread_create(&lps2xdf->thread, lps2xdf->thread_stack, CONFIG_LPS2XDF_THREAD_STACK_SIZE,
			(k_thread_entry_t)lps2xdf_thread, lps2xdf, NULL, NULL,
			K_PRIO_COOP(CONFIG_LPS2XDF_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_LPS2XDF_TRIGGER_GLOBAL_THREAD)
	...
	if (!ON_I3C_BUS(cfg)) {
		...
		gpio_init_callback(&lps2xdf->gpio_cb, lps2xdf_gpio_callback,
