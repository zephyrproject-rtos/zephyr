...
static int lis2dh_stream_arm(const struct device *dev, const struct sensor_read_config *cfg)
{
	...
	status = data->hw_tf->update_reg(dev, LIS2DH_REG_CTRL3, LIS2DH_FIFO_INT1_MASK, routes);
	if (status < 0) {
		return status;
	}
...
void lis2dh_stream_submit(const struct device *dev, struct rtio_iodev_sqe *sqe)
{
	...
	for (size_t i = 0U; i < cfg->count; i++) {
		...
		has_data_action |= trig->opt != SENSOR_STREAM_DATA_NOP;
	...
	if (!data->stream_active && data->fifo_faulted) {
		status = lis2dh_fifo_stop(dev);
		if (status < 0) {
			goto reject;
		}
	}
