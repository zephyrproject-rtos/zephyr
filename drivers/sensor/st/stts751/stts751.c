...
static int stts751_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	struct stts751_data *data = dev->data;
	int16_t raw_temp;
...
static inline void stts751_temp_convert(struct sensor_value *val, int16_t raw_val)
{
	val->val1 = raw_val / 256;
	val->val2 = ((int32_t)raw_val % 256) * 10000;
}

static int stts751_channel_get(const struct device *dev, enum sensor_channel chan,
...
static int stts751_odr_set(const struct device *dev, const struct sensor_value *val)
{
	int odr;

	for (odr = 0; odr < ARRAY_SIZE(stts751_map); odr++) {
		if (val->val1 == stts751_map[odr].rate && val->val2 == stts751_map[odr].rate_dec) {
			break;
		}
	}
...
#define STTS751_DEFINE(inst)                                                                                      \
	static struct stts751_data stts751_data_##inst;                                                           \
                                                                                                                  \
	static const struct stts751_config stts751_config_##inst = {COND_CODE_1(DT_INST_ON_BUS(inst, i2c),						\
			 ...
			 IF_ENABLED(CONFIG_STTS751_TRIGGER,						\
			   (.int_gpio = GPIO_DT_SPEC_INST_GET_OR(inst, drdy_gpios, { 0 }),)) };                \
                                                                                                                  \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, stts751_init, NULL, &stts751_data_##inst,                              \
				     &stts751_config_##inst, POST_KERNEL,                                         \
