...
static inline uint32_t stts22h_val_to_treshhold(const struct sensor_value *val)
{
			    ...
			    const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LOWER_THRESH: {
		...
		uint32_t raw_value = stts22h_val_to_treshhold(val);

		if (raw_value == UINT32_MAX) {
			return -EINVAL;
		}
		if (stts22h_temp_trshld_high_set(ctx, (uint8_t)raw_value) < 0) {
			LOG_DBG("Could not set high treshhold");
			return -EIO;
		}
		return 0;
	}
	case SENSOR_ATTR_UPPER_THRESH: {
		...
		if (stts22h_temp_trshld_low_set(ctx, (uint8_t)raw_value) < 0) {
			LOG_DBG("Could not set low treshhold ");
			return -EIO;
		}
		return 0;
	}
