...
static void pat9136_submit_one_shot(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	err = pat9136_bus_read(dev, REG_BURST_READ, edata->buf, sizeof(edata->buf));
	if (err) {
		LOG_ERR("Failed to read flow data");
		rtio_iodev_sqe_err(iodev_sqe, err);
		return;
	}
	...
	for (size_t i = 0 ; i < sizeof(edata->header.resolution.buf) ; i++) {
		err = pat9136_bus_read(dev,
			REG_RESOLUTION_X_LOWER + i,
			(uint8_t *)&edata->header.resolution.buf[i],
			1);
		if (err) {
			LOG_ERR("Failed to read resolution");
			rtio_iodev_sqe_err(iodev_sqe, err);
			return;
		}
	}

	LOG_DBG("One shot reading complete");
	rtio_iodev_sqe_ok(iodev_sqe, 0);
}

static void pat9136_submit_sync(struct rtio_iodev_sqe *iodev_sqe)
{
	const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;
...
static void pat9136_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	if (req == NULL) {
		LOG_ERR("Failed to allocate work request");
		rtio_iodev_sqe_err(iodev_sqe, -ENOMEM);
		return;
	}

	rtio_work_req_submit(req, iodev_sqe, pat9136_submit_sync);
}
...
#define PAT9136_INIT(inst)									   \
	...
	BUILD_ASSERT(DT_PROP(DT_DRV_INST(inst), resolution) >= 0 &&				   \
		     ...
		     "Resolution must be in range 0-199");					   \
												   \
	RTIO_DEFINE(pat9136_rtio_ctx_##inst, 8, 8);						   \
