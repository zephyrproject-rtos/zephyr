...
static void paa3905_submit_one_shot(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	err = paa3905_bus_read(dev, REG_BURST_READ, edata->buf, sizeof(edata->buf));
	if (err) {
	...
	LOG_HEXDUMP_INF(edata->buf, sizeof(edata->buf), "Flow data");

	LOG_DBG("One shot reading complete");
	rtio_iodev_sqe_ok(iodev_sqe, 0);
}

static void paa3905_submit_sync(struct rtio_iodev_sqe *iodev_sqe)
{
	const struct sensor_read_config *cfg = iodev_sqe->sqe.iodev->data;
...
static void paa3905_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	rtio_work_req_submit(req, iodev_sqe, paa3905_submit_sync);
}
