...
static inline int paa3905_bus_read(const struct device *dev,
				   uint8_t reg,
				   uint8_t *buf,
				   uint16_t len)
{
...
static inline int paa3905_bus_write(const struct device *dev,
				    uint8_t reg,
				    const uint8_t *buf,
				    uint16_t len)
{
...
#endif /* ZEPHYR_DRIVERS_SENSOR_PAA3905_BUS_H_ */
