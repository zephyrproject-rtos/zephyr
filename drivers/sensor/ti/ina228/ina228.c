...
static int ina228_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_VOLTAGE)) {
		...
		data->bus_voltage = ina228_convert_20bits_to_signed(
			INA228_GET_BIT_4_TO_23(registervalue_32bit));
	}
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_CURRENT)) {
		...
		data->current = ina228_convert_20bits_to_signed(
			INA228_GET_BIT_4_TO_23(registervalue_32bit));
	}

	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_POWER)) {
		ret = ina228_register_read_24(dev, INA228_REG_POWER, &data->power); /* Unsigned */
		if (ret) {
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_CHARGE)) {
		...
		if (ret) {
			LOG_ERR("Failed to read INA228 cumulative charge");
			return ret;
		}
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_ENERGY)) {
		ret = ina228_register_read_40(dev, INA228_REG_ENERGY, &data->energy); /* Unsigned */
		if (ret) {
			LOG_ERR("Failed to read INA228 cumulative energy");
			return ret;
		}
	}
#endif /* CONFIG_INA228_CUMULATIVE */
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_DIE_TEMP)) {
		ret = ina228_register_read_16(dev, INA228_REG_DIETEMP,
	...
	if ((chan == SENSOR_CHAN_ALL) || (chan == SENSOR_CHAN_VSHUNT)) {
		...
		data->shunt_voltage = ina228_convert_20bits_to_signed(
			INA228_GET_BIT_4_TO_23(registervalue_32bit));
	}
#endif /* CONFIG_INA228_VSHUNT */
