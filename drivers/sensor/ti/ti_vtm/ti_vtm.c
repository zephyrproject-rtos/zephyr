...
#define TI_VTM_TRIG_COUNT (TI_VTM_TRIG_TH2 - TI_VTM_TRIG_TH0 + 1)

struct ti_vtm_sensor_data {
	int32_t temperature_mc;
	sensor_trigger_handler_t handlers[TI_VTM_TRIG_COUNT];
	const struct sensor_trigger *trigs[TI_VTM_TRIG_COUNT];
};
...
static inline struct ti_vtm_sensor_data *sensor_dev_data(const struct device *dev)
{
	return dev->data;
}

static inline const struct ti_vtm_sensor_config *sensor_dev_cfg(const struct device *dev)
{
	return dev->config;
}
				     ...
				     sensor_trigger_handler_t handler)
{
	...
	struct ti_vtm_sensor_data *data = sensor_dev_data(sensor);
	enum ti_vtm_sensor_trigger_type type = (enum ti_vtm_sensor_trigger_type)trig->type;
	int idx = type - TI_VTM_TRIG_TH0;
	...
	if (idx < 0 || idx >= TI_VTM_TRIG_COUNT) {
		return -ENOTSUP;
	}

	switch (type) {
	case TI_VTM_TRIG_TH0:
		ctrl_bit = TI_VTM_CFG1_TMPSENS_CTRL_LT_TH0_EN;
		en_set = &cfg1_regs->LT_TH0_INT_EN_SET;
		en_clr = &cfg1_regs->LT_TH0_INT_EN_CLR;
		break;
	case TI_VTM_TRIG_TH1:
		ctrl_bit = TI_VTM_CFG1_TMPSENS_CTRL_GT_TH1_EN;
		en_set = &cfg1_regs->GT_TH1_INT_EN_SET;
		en_clr = &cfg1_regs->GT_TH1_INT_EN_CLR;
		break;
	case TI_VTM_TRIG_TH2:
		ctrl_bit = TI_VTM_CFG1_TMPSENS_CTRL_GT_TH2_EN;
		en_set = &cfg1_regs->GT_TH2_INT_EN_SET;
		en_clr = &cfg1_regs->GT_TH2_INT_EN_CLR;
		break;
	default:
		return -ENOTSUP;
	}
	...
	if (enable) {
		cfg1_regs->TMPSENS[config->sensor_id].CTRL |= ctrl_bit;
		*en_set = BIT(config->vd_id);
	} else {
		cfg1_regs->TMPSENS[config->sensor_id].CTRL &= ~ctrl_bit;
		*en_clr = BIT(config->vd_id);
	}

	data->handlers[idx] = handler;
	data->trigs[idx] = handler ? trig : NULL;
				  ...
				  enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	int idx = attr - TI_VTM_ATTR_TH0_THRESH;
	uint32_t th_reg;
	int adc;
	...
	switch ((enum ti_vtm_sensor_trigger_type)(TI_VTM_TRIG_TH0 + idx)) {
	case TI_VTM_TRIG_TH0:
...
{
	const struct ti_vtm_config *config = DEV_CFG(vtm);
	int idx = trig_type - TI_VTM_TRIG_TH0;
	...
	for (int i = 0; i < config->sensor_count; i++) {
		...
		handler = sensor_data->handlers[idx];
		if (handler) {
			handler(sensor_dev, sensor_data->trigs[idx]);
		}
	}
}
