...
static const struct conversion_params temp_params = {.scale = TEMP_SCALE, .offset = TEMP_OFFSET};

static const struct conversion_params humidity_params = {
	.scale = RH_SCALE, .offset = 0 /* No offset for humidity */
};
				 ...
				 conversion_direction_t direction)
{
	...
	if (direction == RAW_TO_SENSOR) {
		...
		numerator =
			(int64_t)params->offset * 65535LL + (int64_t)params->scale * (*raw_value);
...
#define TI_HDC302X_DEFINE(inst, compat)                                                            \
	...
	DEVICE_DT_DEFINE(DT_INST(inst, compat), ti_hdc302x_init, NULL,                             \
			 &ti_hdc302x_data_##compat##inst, &ti_hdc302x_config_##compat##inst,       \
			 POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, &ti_hdc302x_api_funcs);
