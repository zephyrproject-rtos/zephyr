...
static int ina7xx_read(const struct device *dev, uint8_t reg_addr, void *reg_data, uint8_t bytes)
{
	...
	if (bytes == 2) {
		*(uint16_t *)reg_data = sys_get_be16(rx_buf);
	} else if (bytes == 3) {
		*(uint64_t *)reg_data = sys_get_be24(rx_buf);
	} else if (bytes == 4) {
		*(uint64_t *)reg_data = sys_get_be32(rx_buf);
	} else {
		*(uint64_t *)reg_data = sys_get_be40(rx_buf);
	}
...
static int ina7xx_set_config(const struct device *dev)
{
	...
	reg_data = FIELD_PREP(INA7xx_CONVDLY, cfg->convdly);
...
static int ina7xx_trigger_measurement(const struct device *dev)
{
	...
	measurement_time = samples_avg_count[cfg->avg] *
			   ((cfg->convdly * 2000) + tct_vsenct_time_us[cfg->vsenct] +
			    tct_vsenct_time_us[cfg->tct]);
	...
	for (size_t i = 0; i < MAX_RETRIES; ++i) {
		...
		rc = ina7xx_read(dev, INA7xx_REG_DIAG_ALRT, &tmp, 2);
		if (rc < 0) {
			      ...
			      struct sensor_value *val)
{
	...
	case SENSOR_CHAN_VOLTAGE:
		if (IS_BIT_SET(data->valid_readings_mask, INA7xx_READ_VOLTAGE_BIT)) {
			sensor_value_from_micro(val, data->v_bus * INA7xx_BUS_VOLTAGE_MUL_UV);
		} else {
	...
	case SENSOR_CHAN_DIE_TEMP:
		if (IS_BIT_SET(data->valid_readings_mask, INA7xx_READ_DIE_TEMP_BIT)) {
			sensor_value_from_micro(val, data->die_temp * INA7xx_TEMP_SCALE_MG);
		} else {
	...
	case SENSOR_CHAN_POWER:
		if (IS_BIT_SET(data->valid_readings_mask, INA7xx_READ_POWER_BIT)) {
			sensor_value_from_micro(val, data->power * data->power_mul_uw);
		} else {
	...
	case SENSOR_CHAN_CURRENT:
		if (IS_BIT_SET(data->valid_readings_mask, INA7xx_READ_CURRENT_BIT)) {
			if (IS_BIT_SET((data->current), INA7xx_SIGN_BIT)) {
				data->current = (uint16_t)(~data->current) + 1;
				sign = -1;
			}
			sensor_value_from_micro(val,
						(sign * (data->current * data->current_mul_ua)));
		} else {
