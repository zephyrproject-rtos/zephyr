...
#define DT_DRV_COMPAT ti_opt300x
...
LOG_MODULE_DECLARE(opt300x, CONFIG_SENSOR_LOG_LEVEL);

static void opt300x_thread_cb(const struct device *dev)
{
	const struct opt300x_config *cfg = dev->config;
	struct opt300x_data *dat = dev->data;
	uint16_t reg_cfg;
	int ret;

	/* read result then clear interrupt by reading REG_CONFIG */
	ret = opt300x_reg_read(dat->dev, OPT300X_REG_RESULT, &dat->sample);
	if (ret) {
		LOG_ERR("Failed to read result, ret: %d", ret);
		return;
	}

	ret = opt300x_reg_read(dat->dev, OPT300X_REG_CONFIG, &reg_cfg);
	if (ret) {
...
static void opt300x_isr(const struct device *dev, struct gpio_callback *gpio_cb, uint32_t pins)
{
	struct opt300x_data *dat = CONTAINER_OF(gpio_cb, struct opt300x_data, gpio_cb_int);
	const struct opt300x_config *cfg = dat->dev->config;
...
#ifdef CONFIG_OPT300X_TRIGGER_OWN_THREAD
static void opt300x_thread(void *p1, void *p2, void *p3)
{
	ARG_UNUSED(p2);
	ARG_UNUSED(p3);

	struct opt300x_data *dat = p1;

	while (1) {
		k_sem_take(&dat->gpio_sem, K_FOREVER);
		opt300x_thread_cb(dat->dev);
	}
}
#endif

#ifdef CONFIG_OPT300X_TRIGGER_GLOBAL_THREAD
static void opt300x_int_work(struct k_work *work)
{
	struct opt300x_data *dat = CONTAINER_OF(work, struct opt300x_data, work_int);
...
int opt300x_init_interrupt(const struct device *dev)
{
	...
	gpio_init_callback(&dat->gpio_cb_int, opt300x_isr, BIT(cfg->gpio_int.pin));
	...
	k_thread_create(&dat->thread, dat->thread_stack, CONFIG_OPT300X_THREAD_STACK_SIZE,
			opt300x_thread, dat, NULL, NULL,
			K_PRIO_COOP(CONFIG_OPT300X_THREAD_PRIORITY), 0, K_NO_WAIT);
	k_thread_name_set(&dat->thread, "opt300x_trigger");

#elif defined(CONFIG_OPT300X_TRIGGER_GLOBAL_THREAD)
	k_work_init(&dat->work_int, opt300x_int_work);
#endif
			...
			sensor_trigger_handler_t handler)
{
	...
	ret = opt300x_reg_update(dev, OPT300X_REG_LOW_LIMIT, OPT300X_LIMIT_EXPONENT_MASK,
				 handler ? OPT300X_LIMIT_EXPONENT_MASK
					 : OPT300X_LIMIT_EXPONENT_DEFAULT);
	if (ret) {
