			 ...
			 struct sensor_value *val)
{
	...
	float mv =
		data->adc_sample * (float)config->adc->vref_mv / (BIT(config->adc->resolution) - 1);

	for (uint8_t i = 1; i <= config->polynomial_degree; ++i) {
		flow_rate += config->polynomial_coefficients[i] * powf(mv, i);
	}

	sensor_value_from_float(val, flow_rate);

	LOG_DBG("%u mV = %u.%u mL/min", (uint32_t)mv, val->val1, val->val2);
...
static int d6f_init(const struct device *dev)
{
	...
	if (!config->adc->channel_cfg_dt_node_exists) {
		LOG_ERR("Vref not available");
		return -EINVAL;
	}
