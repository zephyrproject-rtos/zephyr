...
{
	struct ms5607_data *data = dev->fixed->driver_data;
	int err;
...
{
	const struct ms5607_data *data = dev->fixed->driver_data;
...
{
	const struct ms5607_config *const config = dev->fixed->config_info;
	struct ms5607_data *data = dev->fixed->driver_data;
	struct sensor_value val;
	int err;
