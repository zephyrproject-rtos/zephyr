...
static inline void sen6x_submit_one_shot(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	err = sen6x_encode(dev, channels, num_channels, buf);
	if (err != 0) {
	...
	if (sen6x_status_callbacks_exist(dev)) {
		err = sen6x_prep_read_device_status(dev, edata, &read_sqe);
		if (err < 0) {
			LOG_ERR("Fail to prepare status read: %d", err);
			rtio_iodev_sqe_err(iodev_sqe, err);
			return;
		}
	...
	if (edata->header.has_data_ready) {
		 ...
		 */
		err = sen6x_prep_read_data_ready(dev, edata, &read_sqe);
		if (err < 0) {
			LOG_ERR("Fail to prepare data-ready read: %d", err);
			rtio_iodev_sqe_err(iodev_sqe, err);
			return;
		}
		read_sqe->flags |= RTIO_SQE_CHAINED;
	...
	if (edata->header.has_measured_values) {
		err = sen6x_prep_read_measured_values(dev, edata, &read_sqe);
		if (err < 0) {
			LOG_ERR("Fail to prepare data read: %d", err);
			rtio_iodev_sqe_err(iodev_sqe, err);
			return;
		}
		read_sqe->flags |= RTIO_SQE_CHAINED;
			      ...
			      struct sensor_q31_data *out_temperature)
{
	...
	if (supports_get_heater_measurements) {
		...
		for (; relative_humidity == INT16_MAX || temperature == INT16_MAX;
		     attempts_left--) {
			if (attempts_left == 0) {
				return -ETIMEDOUT;
			}
		...
		if (out_relative_humidity) {
			*out_relative_humidity = (struct sensor_q31_data){
				...
				.readings[0].value =
					(((int64_t)relative_humidity) << (31 - shift)) / 100,
			};
		}
		if (out_temperature) {
...
static int sen6x_init(const struct device *dev)
{
	...
	err = sen6x_device_reset(dev);
	if (err) {
		LOG_ERR("Failed to reset %d", err);
		return err;
	}
	...
	if (cfg->model != SEN6X_MODEL_SEN60) {
		...
		err = sen6x_reg_read_rtio(dev, REG_GET_VERSION_SEN6X, version, sizeof(version),
					  K_MSEC(20));
		if (err) {
			LOG_ERR("Failed to read version %d", err);
			return err;
		}
	...
	if (cfg->start_measurement_on_init) {
		err = sen6x_start_continuous_measurement(dev);
		if (err) {
			LOG_ERR("Failed to start continuous measurement %d", err);
			return err;
		}
	}
