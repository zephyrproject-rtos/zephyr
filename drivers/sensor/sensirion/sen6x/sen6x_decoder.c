...
static int sen6x_get_channel_index_sen65(enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		return 5;
	case SENSOR_CHAN_SEN6X_VOC_INDEX:
		return 6;
	case SENSOR_CHAN_SEN6X_NOX_INDEX:
		return 7;
	default:
		return sen6x_get_channel_index_particle_number(chan);
	}
...
static int sen6x_get_channel_format(enum sensor_channel chan, bool *out_is_signed,
				    size_t *out_multiplier, size_t *out_divisor)
{
	bool is_signed;
	size_t multiplier = 1;
	size_t divisor = 1;
	...
	case SENSOR_CHAN_PM_10_COUNT:
		is_signed = false;
		/* our divisor is 10, but to convert from particles/cm^3 to particles/0.1l, we have
		 * to multiply by 100.
		 */
		multiplier = 10;
		break;
					 ...
					 uint16_t *frame_count)
{
	...
	ret = sen6x_get_channel_format(chan_spec.chan_type, &is_signed, NULL, NULL);
	if (ret != 0) {
		return ret;
	}
				 ...
				 uint32_t *fit, uint16_t max_count, void *data_out_)
{
	...
	ret = sen6x_get_channel_format(chan_spec.chan_type, &is_signed, &multiplier, &divisor);
	if (ret != 0) {
		return ret;
	}
