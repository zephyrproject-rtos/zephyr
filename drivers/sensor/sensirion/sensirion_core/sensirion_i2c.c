...
{
	return crc8(data, count, SENSIRION_CRC8_POLY, SENSIRION_CRC8_INIT, false);
}
				      ...
				      uint16_t num_words)
{
	int ret;
	uint16_t i, j;
	uint16_t size = num_words * (SENSIRION_WORD_SIZE + SENSIRION_CRC8_LEN);
				    ...
				    uint16_t expected_data_length)
{
	...
	uint16_t size = (expected_data_length / SENSIRION_WORD_SIZE) *
			(SENSIRION_WORD_SIZE + SENSIRION_CRC8_LEN);

	if (expected_data_length % SENSIRION_WORD_SIZE != 0) {
		return -EINVAL;
	}

	ret = i2c_read_dt(i2c_spec, buffer, size);
	if (ret != 0) {
		return ret;
	}
	...
	for (i = 0, j = 0; i < size; i += SENSIRION_WORD_SIZE + SENSIRION_CRC8_LEN) {
		...
		ret = sensirion_i2c_check_crc(&buffer[i], SENSIRION_WORD_SIZE,
