...
{
	float ticks = sensor_value_to_float(val) * 200.0f;
	...
	if (ticks < (float)INT16_MIN) {
		return INT16_MIN;
	}
	if (ticks > (float)INT16_MAX) {
		return INT16_MAX;
	}
...
static bool stc31_binary_gas_is_low_xsens(uint16_t binary_gas)
{
	return binary_gas >= STC31_DT_BINARY_GAS_CO2_IN_N2_100;
}

static bool stc31_binary_gas_is_valid(uint16_t binary_gas)
{
	switch (binary_gas) {
	case STC31_DT_BINARY_GAS_CO2_IN_N2_100_LN:
	case STC31_DT_BINARY_GAS_CO2_IN_AIR_100_LN:
	case STC31_DT_BINARY_GAS_CO2_IN_N2_25_LN:
	case STC31_DT_BINARY_GAS_CO2_IN_AIR_25_LN:
	case STC31_DT_BINARY_GAS_CO2_IN_N2_100:
	case STC31_DT_BINARY_GAS_CO2_IN_AIR_100:
	case STC31_DT_BINARY_GAS_CO2_IN_N2_40:
	case STC31_DT_BINARY_GAS_CO2_IN_AIR_40:
		return true;
	default:
		return false;
	}
...
static bool stc31_product_is_stc31c(uint32_t product)
{
	return ((product & STC31_PRODUCT_MASK) == STC31_PRODUCT_NUMBER) &&
	       ((product & 0xFFU) >= (STC31C_PRODUCT_NUMBER & 0xFFU));
}

static int stc31_validate_binary_gas(const struct device *dev, uint16_t binary_gas)
{
	const struct stc31_data *data = dev->data;

	if (!stc31_binary_gas_is_valid(binary_gas)) {
		LOG_ERR("Unsupported binary gas 0x%04x", binary_gas);
		return -EINVAL;
	}

	if (stc31_binary_gas_is_low_xsens(binary_gas) && !data->is_stc31c) {
		LOG_ERR("Binary gas 0x%04x requires STC31-C", binary_gas);
		return -EINVAL;
	}
...
static void stc31_wait_measure_interval(struct stc31_data *data)
{
	int64_t now = k_uptime_get();
	int64_t elapsed = now - data->last_measure_ms;

	if (data->last_measure_ms != 0 &&
	    elapsed < STC31_MEASURE_MIN_INTERVAL_MS) {
		int32_t wait_ms = STC31_MEASURE_MIN_INTERVAL_MS - (int32_t)elapsed;

		LOG_WRN("Measurement requested too soon, waiting %d ms", wait_ms);
		k_msleep(wait_ms);
	}
}
...
static int stc31_set_binary_gas(const struct device *dev, uint16_t binary_gas)
{
	...
	ret = stc31_validate_binary_gas(dev, binary_gas);
	if (ret < 0) {
		return ret;
	}
...
static int stc31_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	stc31_wait_measure_interval(data);
	...
	data->last_measure_ms = k_uptime_get();
...
int stc31_forced_recalibration(const struct device *dev, uint32_t reference_ppm)
{
	...
	if (reference_ppm > STC31_FRC_PPM_MAX) {
		return -EINVAL;
	}

	raw = stc31_vol_percent_to_raw(vol_percent);
...
static int stc31_init(const struct device *dev)
{
	...
	data->is_stc31c = stc31_product_is_stc31c(product);
