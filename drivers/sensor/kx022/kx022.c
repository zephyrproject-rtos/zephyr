...
#define kx022_start(dev)                                                                           \
	if (kx022_op(dev, true) < 0)                                                               \
	{	return -EIO;}
#define kx022_stop(dev)                                                                            \
	if (kx022_op(dev, false) < 0)                                                              \
		{return -EIO;}
...
static int kx022_init(const struct device *dev)
{
	...
	if (data->hw_tf->write_reg(data, KX022_REG_CNTL3, KX022_DEFAULT_CNTL3_50HZ) < 0) {
		LOG_DBG("Failed CNTL3");
		return -EIO;
	}

	if (data->hw_tf->write_reg(data, KX022_REG_TILT_TIMER, KX022_DEFAULT_TILT_TIMER_1S) < 0) {
		LOG_DBG("Failed TILT");
		return -EIO;
	}
