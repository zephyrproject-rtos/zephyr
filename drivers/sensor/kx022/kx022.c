...
int kx022_mode(const struct device *dev, bool mode)
{
	...
	if (mode == KX022_STANDY_MODE) {
		...
		if (data->hw_tf->update_reg(dev, KX022_REG_CNTL1, KX022_MASK_CNTL1_PC1, val) < 0) {
...
static int kx022_accel_odr_set(const struct device *dev, uint16_t freq)
{
	...
	if (data->hw_tf->update_reg(dev, KX022_REG_ODCNTL, KX022_MASK_ODCNTL_OSA, (uint8_t)freq) <
	    0) {
...
static int kx022_accel_motion_detection_timer_set(const struct device *dev, uint16_t delay)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_WUFC, (uint8_t)delay) < 0) {
...
static int kx022_accel_tilt_timer_set(const struct device *dev, uint16_t delay)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_TILT_TIMER, (uint8_t)delay) < 0) {
...
static int kx022_accel_tilt_angle_set(const struct device *dev, uint16_t angle)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_TILT_ANGLE_LL, (uint8_t)angle) < 0) {
...
static int kx022_accel_motion_detect_threshold_set(const struct device *dev, uint16_t ath)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_ATH, (uint8_t)ath) < 0) {
...
static int kx022_init(const struct device *dev)
{
	...
	if (data->hw_tf->update_reg(dev, KX022_REG_CNTL2, KX022_MASK_CNTL2_SRST,
				    KX022_MASK_CNTL2_SRST) < 0) {
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_CNTL1, val) < 0) {
		LOG_DBG("Failed CNTL1");
		return -EIO;
	}

	/* Set KX022 default odr */
	if (data->hw_tf->update_reg(dev, KX022_REG_ODCNTL, KX022_MASK_ODCNTL_OSA, cfg->odr) < 0) {
