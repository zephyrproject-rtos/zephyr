...
int kx022_trigger_init(const struct device *dev)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_INC1, val) < 0) {
...
int kx022_motion_setup(const struct device *dev, sensor_trigger_handler_t handler)
{
	...
	if (data->hw_tf->update_reg(dev, KX022_REG_CNTL1, KX022_MASK_CNTL1_WUFE, KX022_CNTL1_WUFE) <
	    0) {
		LOG_ERR("Failed set motion detect");
		return -EIO;
	}

	if (data->hw_tf->update_reg(dev, KX022_REG_CNTL3, KX022_MASK_CNTL3_OWUF, cfg->motion_odr) <
	    0) {
		LOG_ERR("Failed set motion odr");
		return -EIO;
	}

	if (data->hw_tf->write_reg(dev, KX022_REG_INC2, KX022_DEFAULT_INC2) < 0) {
		LOG_ERR("Failed set motion axis");
		return -EIO;
	}

	if (data->hw_tf->write_reg(dev, KX022_REG_WUFC, cfg->motion_detection_timer) < 0) {
		LOG_ERR("Failed set motion delay");
		return -EIO;
	}

	if (data->hw_tf->write_reg(dev, KX022_REG_ATH, cfg->motion_threshold) < 0) {
...
int kx022_tilt_setup(const struct device *dev, sensor_trigger_handler_t handler)
{
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_CNTL2, KX022_CNTL_TILT_ALL_EN) < 0) {
	...
	if (data->hw_tf->write_reg(dev, KX022_REG_TILT_TIMER, cfg->tilt_timer) < 0) {
		LOG_ERR("Failed set tilt timer");
		return -EIO;
	}

	if (data->hw_tf->write_reg(dev, KX022_REG_TILT_ANGLE_LL, cfg->tilt_angle_ll) < 0) {
		LOG_ERR("Failed set tilt angle ll");
		return -EIO;
	}

	if (data->hw_tf->write_reg(dev, KX022_REG_TILT_ANGLE_HL, cfg->tilt_angle_hl) < 0) {
