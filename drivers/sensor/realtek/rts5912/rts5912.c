...
int tach_rts5912_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	TACHO_Type *const tach = cfg->regs;

	if (chan != SENSOR_CHAN_RPM && chan != SENSOR_CHAN_ALL) {
		return -ENOTSUP;
	}

	tach->STS = TACHO_STS_CNTRDY_Msk;

	if (WAIT_FOR(tach->STS & TACHO_STS_CNTRDY_Msk, PIN_STUCK_TIMEOUT, k_msleep(1))) {
		/* See whether internal counter is already latched */
		if (tach->STS & TACHO_STS_CNTRDY_Msk) {
			tach->STS = TACHO_STS_CNTRDY_Msk;
...
static int tach_rts5912_init(const struct device *dev)
{
	...
	tach->CTRL =
		((0x01ul << TACHO_CTRL_SELEDGE_Pos) | TACHO_CTRL_READMODE_Msk | TACHO_CTRL_EN_Msk);
...
#define RTS5912_TACH_CONFIG(inst)                                                                  \
	static const struct tach_rts5912_config tach_rts5912_config_##inst = {                     \
		.regs = (TACHO_Type *const)DT_INST_REG_ADDR(inst),                                 \
...
#define TACH_RTS5912_DEVICE(id)                                                                    \
	static struct tach_rts5912_data tach_rts5912_data_##id;                                    \
	PINCTRL_DT_INST_DEFINE(id);                                                                \
	RTS5912_TACH_CONFIG(id);                                                                   \
	DEVICE_DT_INST_DEFINE(id, tach_rts5912_init, NULL, &tach_rts5912_data_##id,                \
			      &tach_rts5912_config_##id, POST_KERNEL, CONFIG_SENSOR_INIT_PRIORITY, \
			      &tach_rts5912_driver_api);

DT_INST_FOREACH_STATUS_OKAY(TACH_RTS5912_DEVICE)
