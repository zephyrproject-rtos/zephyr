...
#define M90E26_CMD_READ_MASK  (1 << 7)
...
#define M90E26_SPI_OPERATION \
  ...
  SPI_HOLD_ON_CS | SPI_LOCK_ON | SPI_MODE_CPOL | SPI_MODE_CPHA)
 ...
 */
static inline int m90e26_convert_energy_reg(const m90e26_data_value_t *reg, struct sensor_value *val)
{
 ...
 * @return int 0 on success, negative error code on failure.
 */
static inline int m90e26_convert_power_reg(const m90e26_data_value_t *reg, struct sensor_value *val)
{
 ...
 * @return int 0 on success, negative error code on failure.
 */
static inline int m90e26_convert_current_reg(const m90e26_data_value_t *reg, struct sensor_value *val)
{
  return sensor_value_from_float(val, reg->uint16 * 0.001f);
}

/**
 * @brief Convert M90E26 phase angle register value to sensor_value. Based on datasheet value scaling.
 * 
 ...
 * @return int 0 on success, negative error code on failure.
 */
static inline int m90e26_convert_phase_angle_reg(const m90e26_data_value_t *reg, struct sensor_value *val)
{
  return sensor_value_from_float(val, reg->int16 * 0.1f);
}

/**
 * @brief Convert M90E26 power factor register value to sensor_value. Based on datasheet value scaling.
 * 
 * @note 1 LSB = 0.001
 * 
 ...
 * @return int 0 on success, negative error code on failure.
 */
static inline int m90e26_convert_power_factor_reg(const m90e26_data_value_t *reg, struct sensor_value *val)
{
