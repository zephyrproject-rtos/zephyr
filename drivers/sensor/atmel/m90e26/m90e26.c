...
static inline int m90e26_read_register(const struct device *dev, 
...
static inline int m90e26_write_register(const struct device *dev, 
...
static void m90e26_gpio_callback_irq(const struct device *port, 
...
static void m90e26_gpio_callback_wrn_out(const struct device *port, 
                                           struct gpio_callback *cb, 
                                           uint32_t pins)
{
...
static void m90e26_gpio_callback_cf1(const struct device *port, 
                                       struct gpio_callback *cb, 
                                       uint32_t pins)
{
...
static void m90e26_gpio_callback_cf2(const struct device *port, 
                                       struct gpio_callback *cb, 
                                       uint32_t pins)
{
...
static int m90e26_trigger_set(const struct device *dev,
                                ...
                                sensor_trigger_handler_t handler)
{
  ...
  case M90E26_SENSOR_TRIG_TYPE_IRQ:
    if (gpio_is_ready_dt(&cfg->irq))
    {
      data->irq_ctx.trigger = *trig;
      data->irq_ctx.handler = handler;
      gpio_init_callback(&data->irq_ctx.gpio_cb, 
                         m90e26_gpio_callback_irq, 
                         BIT(cfg->irq.pin));
    ...
    break;
  case M90E26_SENSOR_TRIG_TYPE_WRN_OUT:
...
#define M90E26_CONFIG_SPI(inst)			\
	.bus.spi = SPI_DT_SPEC_INST_GET(  \
		inst, M90E26_SPI_OPERATION),		\
	.bus_io = &m90e26_bus_io_spi,
...
#define M90E26_CONFIG_UART(inst)				                                            \
    ...
    .bus.uart.config = {                                                            \
      .baudrate  = DT_PROP(DT_INST_PARENT(inst), current_speed),                    \
