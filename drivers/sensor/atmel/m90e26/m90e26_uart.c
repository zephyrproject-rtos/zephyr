...
static int m90e26_bus_check_uart(const struct device *dev)
{
  ...
  {
    LOG_ERR("Invalid UART baudrate for %s device. Supported baudrates are 9600 and 2400.", dev->name);
    return -EINVAL;
  }
  ...
  ret = uart_irq_callback_user_data_set(config->bus.uart.bus, m90e26_uart_rx_callback, (void *)dev);
  if (ret < 0)
  {
...
static int m90e26_read_reg_uart(const struct device *dev, 
...
static int m90e26_write_reg_uart(const struct device *dev, 
                                const m90e26_register_t addr, 
                                const m90e26_data_value_t *value)
{
  const struct m90e26_config* config = (const struct m90e26_config*)(dev->config);
  struct m90e26_data *data = (struct m90e26_data *)dev->data;
  const uint8_t host_checksum = (uint8_t)(((addr & M90E26_CMD_WRITE_MASK) + 
                                          ((value->uint16 >> 8) & 0xFF) + 
                                          (value->uint16 & 0xFF)) & 0xFF);
  int ret = 0;
