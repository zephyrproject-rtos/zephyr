...
static int cmd_m90e26_read_register(const struct shell *m90e26_shell, size_t argc, char **argv)
{
	...
	if (argc < 3 || argc > 4) {
		shell_error(m90e26_shell,
			    "Usage: m90e26 read_register [device] [hex_register_address] <N>");
		return -EINVAL;
	}
...
static int cmd_m90e26_write_register(const struct shell *m90e26_shell, size_t argc, char **argv)
{
	...
	if (argc != 4) {
		shell_error(
			m90e26_shell,
			"Usage: m90e26 write_register [device] [hex_register_address] [hex_value]");
		return -EINVAL;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	...
	SHELL_CMD_ARG(read_register, NULL,
		      ...
		      "<N> is optional, and if its passed, then the function takes the read as a mean of <N> last samples over the register.\n"
