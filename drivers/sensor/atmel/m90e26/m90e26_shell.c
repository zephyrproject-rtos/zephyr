...
static int cmd_m90e26_read_register(const struct shell *m90e26_shell, size_t argc, char **argv)
{
  ...
  if (argc < 3 || argc > 4)
  {
    shell_error(m90e26_shell, "Usage: m90e26 read_register [device] [hex_register_address] <N>");
    return -EINVAL;
  }
  ...
  for (size_t i = 0; i < N; i++)
  {
    ...
    if (ret < 0)
    {
      shell_error(m90e26_shell, "Error on read register from %s. [%d]", argv[1], ret);
      return ret;
    }
...
static int cmd_m90e26_write_register(const struct shell *m90e26_shell, size_t argc, char **argv)
{
  ...
  if (argc != 4)
  {
    shell_error(m90e26_shell, "Usage: m90e26 write_register [device] [hex_register_address] [hex_value]");
    return -EINVAL;
  }
