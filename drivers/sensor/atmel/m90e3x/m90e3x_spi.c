...
static int m90e3x_read_reg_spi(const struct device *dev, 
                               const m90e3x_register_t addr, 
                               m90e3x_data_value_t *value)
{
  ...
  sys_put_be16(M90E3X_SPI_READ_MASK | addr, buffer);
  ...
  value->uint16 = sys_get_be16(&buffer[2]);

  return ret;
}

static int m90e3x_write_reg_spi(const struct device *dev, 
                                const m90e3x_register_t addr, 
                                const m90e3x_data_value_t *value)
{
  const struct m90e3x_config* config = (const struct m90e3x_config*)(dev->config);
  m90e3x_register_t tx_data[2] = {
    sys_be16_to_cpu(addr & M90E3X_SPI_WRITE_MASK),
    sys_be16_to_cpu(value->uint16)
  };
  ...
  int ret = spi_transceive_dt(&config->bus, &tx, NULL);
  if (ret != 0)
  {
