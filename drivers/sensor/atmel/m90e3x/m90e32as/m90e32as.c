...
static int
m90e32as_convert_energy_values_to_sensor_values(const struct device *dev,
...
static int m90e32as_convert_fundamental_energy_values_to_sensor_values(
...
static int m90e32as_convert_harmonic_energy_values_to_sensor_values(
...
static int
m90e32as_convert_power_values_to_sensor_values(const struct device *dev,
...
static int m90e32as_convert_fundamental_power_values_to_sensor_values(
	const struct device *dev,
...
static int m90e32as_convert_harmonic_power_values_to_sensor_values(
...
static int
m90e32as_convert_urms_values_to_sensor_values(const struct device *dev,
...
static int
m90e32as_convert_irms_values_to_sensor_values(const struct device *dev,
...
static int
m90e32as_convert_peak_values_to_sensor_values(const struct device *dev,
...
static int m90e32as_convert_phase_angle_values_to_sensor_values(
	const struct device *dev, const struct m90e3x_phase_angle_data *phase_angle_values,
	struct sensor_value *sensor_values)
{
	/*
	 * Conversion formulas from datasheet:
				...
				struct sensor_value *value)
{
	...
	case SENSOR_CHAN_ALL:
		...
		break;
	case M90E3X_SENSOR_CHANNEL_ENERGY:
		ret = m90e32as_convert_energy_values_to_sensor_values(dev, &data->energy_values,
								      value);
		break;
	case M90E3X_SENSOR_CHANNEL_FUNDAMENTAL_ENERGY:
		ret = m90e32as_convert_fundamental_energy_values_to_sensor_values(
			dev, &data->fundamental_energy_values, value);
		break;
	case M90E3X_SENSOR_CHANNEL_HARMONIC_ENERGY:
		ret = m90e32as_convert_harmonic_energy_values_to_sensor_values(
			dev, &data->harmonic_energy_values, value);
		break;
	case M90E3X_SENSOR_CHANNEL_POWER:
		ret = m90e32as_convert_power_values_to_sensor_values(dev, &data->power_values,
								     value);
		break;
	case M90E3X_SENSOR_CHANNEL_FUNDAMENTAL_POWER:
		ret = m90e32as_convert_fundamental_power_values_to_sensor_values(
			dev, &data->fundamental_power_values, value);
		break;
	case M90E3X_SENSOR_CHANNEL_HARMONIC_POWER:
		ret = m90e32as_convert_harmonic_power_values_to_sensor_values(
			dev, &data->harmonic_power_values, value);
		break;
	case M90E3X_SENSOR_CHANNEL_VOLTAGE:
		ret = m90e32as_convert_urms_values_to_sensor_values(dev, &data->voltage_rms_values,
								    value);
		break;
	case M90E3X_SENSOR_CHANNEL_CURRENT:
		ret = m90e32as_convert_irms_values_to_sensor_values(dev, &data->current_rms_values,
								    value);
		break;
	case M90E3X_SENSOR_CHANNEL_PEAK:
	...
	case M90E3X_SENSOR_CHANNEL_FREQUENCY:
		...
		break;
	case M90E3X_SENSOR_CHANNEL_PHASE_ANGLE:
		ret = m90e32as_convert_phase_angle_values_to_sensor_values(
			dev, &data->phase_angle_values, value);
		break;
	case M90E3X_SENSOR_CHANNEL_TEMPERATURE:
	...
	static const struct m90e3x_config m90e32as_config_##inst =          \
		{.bus = SPI_DT_SPEC_INST_GET(inst, M90E3X_SPI_OPERATION),       \
		 .bus_io = &m90e3x_bus_io_spi,                                  \
		  ...
		  IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, irq0_gpios), (.irq0 = GPIO_DT_SPEC_INST_GET(inst, irq0_gpios),))           \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, irq1_gpios), (.irq1 = GPIO_DT_SPEC_INST_GET(inst, irq1_gpios),))           \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, wrn_out_gpios), (.wrn_out = GPIO_DT_SPEC_INST_GET(inst, wrn_out_gpios),))  \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, cf1_gpios), (.cf1 = GPIO_DT_SPEC_INST_GET(inst, cf1_gpios),))              \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, cf2_gpios), (.cf2 = GPIO_DT_SPEC_INST_GET(inst, cf2_gpios),))              \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, cf3_gpios), (.cf3 = GPIO_DT_SPEC_INST_GET(inst, cf3_gpios),))              \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, cf4_gpios), (.cf4 = GPIO_DT_SPEC_INST_GET(inst, cf4_gpios),))              \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, pm0_gpios), (.pm0 = GPIO_DT_SPEC_INST_GET(inst, pm0_gpios),))              \
IF_ENABLED(DT_INST_NODE_HAS_PROP(inst, pm1_gpios), (.pm1 = GPIO_DT_SPEC_INST_GET(inst, pm1_gpios),)) };           \
    \
