...
#define WRITE_REGISTER(ret, reg_enum, value) (ret = m90e32as_write_register(dev, reg_enum, value))
...
static int m90e32as_reload_config(const struct device *dev)
{
	...
	if (WRITE_REGISTER(ret, METEREN, &data->m90e32as_config_registers.MeterEn) ||
	...
	if (WRITE_REGISTER(ret, DETECTCTRL, &data->m90e32as_config_registers.DetectCtrl) ||
	...
	if (WRITE_REGISTER(ret, PLCONSTH, &data->m90e32as_config_registers.PLconstH) ||
	...
	if (WRITE_REGISTER(ret, POFFSETAF, &data->m90e32as_config_registers.PoffsetAF) ||
	...
	if (WRITE_REGISTER(ret, UGAINA, &data->m90e32as_config_registers.UgainA) ||
...
static int m90e32as_sample_fetch(const struct device *dev, enum sensor_channel channel)
{
	...
	case M90E3X_SENSOR_CHANNEL_ENERGY:
		if (READ_REGISTER(ret, APENERGYT, energy_values.APenergyT) ||
	...
	case M90E3X_SENSOR_CHANNEL_FUNDAMENTAL_ENERGY:
		if (READ_REGISTER(ret, APENERGYTF, fundamental_energy_values.APenergyTF) ||
	...
	case M90E3X_SENSOR_CHANNEL_HARMONIC_ENERGY:
		if (READ_REGISTER(ret, APENERGYTH, harmonic_energy_values.APenergyTH) ||
	...
	case M90E3X_SENSOR_CHANNEL_POWER:
		if (READ_REGISTER(ret, PMEANT, power_values.PmeanT) ||
	...
	case M90E3X_SENSOR_CHANNEL_POWER_FACTOR:
		if (READ_REGISTER(ret, PFMEANT, power_factor_values.PFmeanT) ||
	...
	case M90E3X_SENSOR_CHANNEL_FUNDAMENTAL_POWER:
		if (READ_REGISTER(ret, PMEANTF, fundamental_power_values.PmeanTF) ||
	...
	case M90E3X_SENSOR_CHANNEL_HARMONIC_POWER:
		if (READ_REGISTER(ret, PMEANTH, harmonic_power_values.PmeanTH) ||
	...
	case M90E3X_SENSOR_CHANNEL_VOLTAGE:
		if (READ_REGISTER(ret, URMSA, voltage_rms_values.UrmsA) ||
	...
	case M90E3X_SENSOR_CHANNEL_CURRENT:
		if (READ_REGISTER(ret, IRMSN, current_rms_values.IrmsN) ||
	...
	case M90E3X_SENSOR_CHANNEL_PEAK:
		if (READ_REGISTER(ret, UPEAKA, peak_values.UPeakA) ||
	...
	case M90E3X_SENSOR_CHANNEL_FREQUENCY:
		if (READ_REGISTER(ret, FREQ, Freq)) {
			return ret;
		}
		break;
	case M90E3X_SENSOR_CHANNEL_PHASE_ANGLE:
		if (READ_REGISTER(ret, PANGLEA, phase_angle_values.PAngleA) ||
	...
	case M90E3X_SENSOR_CHANNEL_TEMPERATURE:
		if (READ_REGISTER(ret, TEMP, Temp)) {
			return ret;
		}
		break;
	default:
...
#define M90E32AS_ASSIGN_PIN(inst, pin_name)                                                        \
...
#define M90E32AS_DEVICE(inst)                                                                      \
	...
	static const struct m90e3x_config m90e32as_config_##inst = {                               \
		...
		IF_ENABLED( \
			CONFIG_PM_DEVICE, (.pm_mode_ops = &m90e3x_pm_mode,)			\
		) \
		M90E32AS_ASSIGN_PIN(inst, irq0) \
		M90E32AS_ASSIGN_PIN(inst, irq1) \
		M90E32AS_ASSIGN_PIN(inst, wrn_out) \
		M90E32AS_ASSIGN_PIN(inst, cf1) \
		M90E32AS_ASSIGN_PIN(inst, cf2) \
		M90E32AS_ASSIGN_PIN(inst, cf3) \
		M90E32AS_ASSIGN_PIN(inst, cf4) \
		M90E32AS_ASSIGN_PIN(inst, pm0) \
		M90E32AS_ASSIGN_PIN(inst, pm1)};        \
                                                                                                   \
