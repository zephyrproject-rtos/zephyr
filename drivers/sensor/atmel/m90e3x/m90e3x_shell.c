...
static int cmd_m90e3x_set_power_mode(const struct shell *m90e3x_shell, size_t argc, char **argv)
{
	...
	case M90E3X_NORMAL:
		...
		break;
	default:
		shell_print(m90e3x_shell, "Invalid power mode. Valid values: 0 (IDLE), 1 "
					  "(DETECTION), 2 (PARTIAL), 3 (NORMAL)");
		return -EINVAL;
	}
...
static int cmd_m90e3x_read_register(const struct shell *m90e3x_shell, size_t argc, char **argv)
{
	...
	if (argc < 3 || argc > 4) {
		shell_error(m90e3x_shell,
			    "Usage: m90e3x read_register [device] [hex_register_address] <N>");
		return -EINVAL;
	}
...
static int cmd_m90e3x_write_register(const struct shell *m90e3x_shell, size_t argc, char **argv)
{
	...
	if (argc != 4) {
		shell_error(
			m90e3x_shell,
			"Usage: m90e3x write_register [device] [hex_register_address] [hex_value]");
		return -EINVAL;
	}
...
SHELL_STATIC_SUBCMD_SET_CREATE(
	m90e3x_cmds,
	SHELL_CMD_ARG(get_power_mode, NULL,
		      "Get M90E3X power mode.\n"
		      "Usage: get_power_mode [device]",
		      cmd_m90e3x_get_power_mode, 2, 0),
	SHELL_CMD_ARG(set_power_mode, NULL,
		      "Set M90E3X power mode. (0:IDLE, 1:DETECTION, 2:PARTIAL, 3:NORMAL)\n"
		      "Usage: set_power_mode [device] [mode]",
		      cmd_m90e3x_set_power_mode, 3, 0),
	SHELL_CMD_ARG(read_register, NULL,
		      "Read M90E3X register.\n"
		      "<N> is optional, and if its passed, then the function takes the read "
		      "as a mean of <N> last samples over the register.\n"
		      "Usage: read_register [<device>] [hex_register_address] <N>",
		      cmd_m90e3x_read_register, 3, 1),
	SHELL_CMD_ARG(write_register, NULL,
		      "Write M90E3X register.\n"
		      "Usage: write_register [<device>] [hex_register_address] [hex_value]",
