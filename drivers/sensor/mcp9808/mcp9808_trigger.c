...
{
	const struct mcp9808_data *data = dev->fixed->driver_data;
	const struct mcp9808_config *cfg = dev->fixed->config_info;
	uint8_t buf[3] = {
		reg,
...
{
	struct mcp9808_data *data = dev->fixed->driver_data;
