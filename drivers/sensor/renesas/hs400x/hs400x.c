...
static int hs400x_read_sample(const struct device *dev, uint16_t *t_sample, uint16_t *rh_sample)
{
	...
	*t_sample = sys_get_be16(&rx_buf[2]);
	...
	/*
	 * The senser sends a checkum after each measuremt. See datasheet "CRC Checksum 
	 * Calculation" section for more details on checking the checksum.
	*/

	crc = crc8(rx_buf, 4, crc_polynomial, crc_initial, 0);
	if (crc != rx_buf[4]) {
		LOG_ERR("CRC check failed: %u\n Should be: %u\n", crc, rx_buf[4]);
		return crc;
	}
...
static int hs400x_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	 ...
	 * from its sleep state. For this driver a Hold Measurement (HM) will be used.
	 * An HR command should consist of the 7-bit address followed by an eighth bit set to 0 (write),
	 * and the measurement command.
	 */
...
static const struct sensor_driver_api hs400x_driver_api = {.sample_fetch = hs400x_sample_fetch,
