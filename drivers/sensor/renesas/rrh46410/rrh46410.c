				...
				uint16_t *eco2_sample, uint8_t *reliaq_sample)
{
	...
	*reliaq_sample = data->read_buffer[9];
...
static int rrh46410_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	if (chan != SENSOR_CHAN_ALL && rrh46410_chan != SENSOR_CHAN_RRH46410_IAQ &&
	    rrh46410_chan != SENSOR_CHAN_RRH46410_TVOC &&
	    rrh46410_chan != SENSOR_CHAN_RRH46410_ETOH &&
	    rrh46410_chan != SENSOR_CHAN_RRH46410_ECO2 &&
	    rrh46410_chan != SENSOR_CHAN_RRH46410_RELIAQ) {
		return -ENOTSUP;
	}
...
static DEVICE_API(sensor, rrh46410_driver_api) = {
...
#define DEFINE_RRH46410(n)                                                                         \
	static struct rrh46410_data rrh46410_data_##n;                                             \
                                                                                                   \
	static const struct rrh46410_config rrh46410_config_##n = COND_CODE_1(DT_INST_ON_BUS	\
				(n, i2c),\
			    (RRH46410_CONFIG_I2C(n)),						\
			    (RRH46410_CONFIG_UART(n)));                     \
                                                                                                   \
	SENSOR_DEVICE_DT_INST_DEFINE(n, rrh46410_init, NULL, &rrh46410_data_##n,                   \
				     &rrh46410_config_##n, POST_KERNEL,                            \
				     CONFIG_SENSOR_INIT_PRIORITY, &rrh46410_driver_api);
