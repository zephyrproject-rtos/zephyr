			     ...
			     bool write, void *data, size_t length)
{
	...
	if (!write) {
		...
		return spi_transceive(&bmi160->spi, &tx, &rx);
	}

	return spi_write(&bmi160->spi, &tx);
}
...
int bmi160_init(struct device *dev)
{
	...
	bmi160->spi.dev = device_get_binding(CONFIG_BMI160_SPI_PORT_NAME);
	if (!bmi160->spi.dev) {
	...
	bmi160->spi.operation = SPI_WORD_SET(8);
	bmi160->spi.frequency = CONFIG_BMI160_SPI_BUS_FREQ;
	bmi160->spi.slave = CONFIG_BMI160_SLAVE;
