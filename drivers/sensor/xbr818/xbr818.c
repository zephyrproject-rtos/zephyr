			   ...
			   enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	/* this cast is necessary due to the additional attributes not being part of the
	 * initial enum and causing a build error without it.
	 */
	switch ((int)attr) {
...
static int xbr818_init_defaults(const struct device *dev)
{
	...
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_THRESHOLD_1");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_THRESHOLD_2, 0x01);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_THRESHOLD_2");

	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_THRESHOLD_NOISE_1, 0x55);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_THRESHOLD_NOISE_1");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_THRESHOLD_NOISE_2, 0x01);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_THRESHOLD_NOISE_2");

	/* 0.1 seconds */
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_DELAY_TIME_1, 0x80);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_DELAY_TIME_1");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_DELAY_TIME_2, 0x0C);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_DELAY_TIME_2");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_DELAY_TIME_3, 0x00);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_DELAY_TIME_3");

	/* 0.5 seconds */
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_LOCK_TIME_1, 0x80);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_LOCK_TIME_1");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_LOCK_TIME_2, 0x3E);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_LOCK_TIME_2");
	ret |= i2c_reg_write_byte_dt(&config->i2c, XBR818_LOCK_TIME_3, 0x00);
	__ASSERT(ret == 0, "Error sending XBR818 defaults for XBR818_LOCK_TIME_3");
