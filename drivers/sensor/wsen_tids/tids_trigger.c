...
LOG_MODULE_DECLARE(TIDS, CONFIG_SENSOR_LOG_LEVEL);
...
static void tids_process_threshold_interrupt(const struct device *dev)
{
	 ...
	 * to be de-asserted
	 */
	if (WE_SUCCESS != TIDS_getStatusRegister(&data->sensor_interface, &status)) {
...
#ifdef CONFIG_TIDS_TRIGGER_OWN_THREAD
...
#endif /* CONFIG_TIDS_TRIGGER_OWN_THREAD */

#ifdef CONFIG_TIDS_TRIGGER_GLOBAL_THREAD
...
#endif /* CONFIG_TIDS_TRIGGER_GLOBAL_THREAD */

int tids_threshold_set(const struct device *dev, const struct sensor_value *thresh_value,
		       bool upper)
{
	struct tids_data *data = dev->data;
	double thresh = (sensor_value_to_double(thresh_value) / 0.64) + 63;
	...
	if (upper) {
		if (WE_SUCCESS != TIDS_setTempHighLimit(&data->sensor_interface, (uint8_t)thresh)) {
			LOG_ERR("Failed to set high temperature threshold to %d.%d (%d).",
				thresh_value->val1, abs(thresh_value->val2), (uint8_t)thresh);
			return -EIO;
		}
	} else {
		if (WE_SUCCESS != TIDS_setTempLowLimit(&data->sensor_interface, (uint8_t)thresh)) {
...
int tids_init_interrupt(const struct device *dev)
{
	 ...
	 */
	upper_limit.val1 = CONFIG_TIDS_TEMP_HI_THRESHOLD;
	upper_limit.val2 = 0;
	lower_limit.val1 = CONFIG_TIDS_TEMP_LO_THRESHOLD;
	lower_limit.val2 = 0;
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_TIDS_THREAD_STACK_SIZE,
			(k_thread_entry_t)tids_thread, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_TIDS_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_TIDS_TRIGGER_GLOBAL_THREAD)
