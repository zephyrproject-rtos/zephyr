				  ...
				  enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_SAMPLING_FREQUENCY:
		...
		int odr_val = lis2dux12_freq_to_odr_val(dev, val->val1);

		if (odr_val < 0) {
			LOG_ERR("frequency not unsupported or wrong operating mode.");
			return odr_val;
		}
...
static int lis2dux12_init(const struct device *dev)
{
	...
	LOG_INF("%s: pm: %d, odr: %d", dev->name, cfg->pm, cfg->odr);
	lis2dux12_md_t mode = {
		.odr = cfg->odr,
		.fs = cfg->range,
	};
	...
	LOG_INF("%s: range is %d", dev->name, cfg->range);
