/* bmp280.c - Driver for Infineon DPS310 temperature and pressure sensor */
...
/* Convert two bytes from the calibration memory to a s16_t value */
static s16_t raw_calib_to_s16(const u8_t raw[2])
{
	/* values are store in little endian on the chip */
					...
					struct dps310_cal_coeff *comp)
{
	...
	comp->c01 = raw_calib_to_s16(&raw_coef[8]);
	comp->c11 = raw_calib_to_s16(&raw_coef[10]);
	comp->c20 = raw_calib_to_s16(&raw_coef[12]);
	comp->c21 = raw_calib_to_s16(&raw_coef[14]);
	comp->c30 = raw_calib_to_s16(&raw_coef[16]);
}
				       ...
				       const struct dps310_cfg *config)
{
	...
	k_sleep(IFX_DPS310_TMP_MEAS_TIME);
				    ...
				    const struct dps310_cfg *config)
{
	...
	/* give the sensor time to store measured values internally */
	k_sleep(IFX_DPS310_PSR_MEAS_TIME);
...
static s32_t raw_to_int24(const u8_t raw[3])
{
	...
	s32_t res = raw[0] << 16 | raw[1] << 8 | raw[2];
...
static int dps310_init(struct device *dev)
{
	...
	const struct dps310_cfg *config = dev->config->config_info;
...
#ifdef DT_INST_0_INFINEON_DPS310
static struct dps310_data dps310_data_0;
static const struct dps310_cfg dps310_cfg_0 = {
	.i2c_bus_name = DT_INST_0_INFINEON_DPS310_BUS_NAME,
	.i2c_addr = DT_INST_0_INFINEON_DPS310_BASE_ADDRESS
};

DEVICE_AND_API_INIT(dps310, DT_INST_0_INFINEON_DPS310_LABEL, dps310_init,
		    &dps310_data_0, &dps310_cfg_0, POST_KERNEL,
		    CONFIG_SENSOR_INIT_PRIORITY, &dps310_api_funcs);
#endif

#ifdef DT_INST_1_INFINEON_DPS310
static struct dps310_data dps310_data_1;
static const struct dps310_cfg dps310_cfg_1 = {
	.i2c_bus_name = DT_INST_1_INFINEON_DPS310_BUS_NAME,
	.i2c_addr = DT_INST_1_INFINEON_DPS310_BASE_ADDRESS
};

DEVICE_AND_API_INIT(dps310, DT_INST_1_INFINEON_DPS310_LABEL, dps310_init,
