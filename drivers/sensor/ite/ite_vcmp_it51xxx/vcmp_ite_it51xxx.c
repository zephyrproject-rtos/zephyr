...
const uint8_t vcmp_ctrl_reg[VCMP_CHANNEL_CNT] = {REG_VCMP_VCMP0SCTL, REG_VCMP_VCMP1SCTL,
						 REG_VCMP_VCMP2SCTL};
...
static void clear_vcmp_status(const struct device *dev, int channel)
{
	...
	sys_write8(sys_read8(reg_base + vcmp_ctrl_reg[channel]), reg_base + vcmp_ctrl_reg[channel]);
}
...
static void vcmp_it51xxx_isr(const struct device *dev)
{
	...
	for (idx = VCMP_CHANNEL_0; idx < VCMP_CHANNEL_CNT; idx++) {
		status = (sys_read8(reg_base + vcmp_ctrl_reg[idx]) & VCMP_CMPXRTIS);
		if (status) {
...
static int vcmp_it51xxx_init(const struct device *dev)
{
	...
	data->adc_ch_cfg.channel_id = config->channel_id;
	 ...
	 */
	if (config->scan_period < sys_read8(reg_vcmpscp2) >> 4) {
		sys_write8(config->scan_period << 4, reg_vcmpscp2);
	}
