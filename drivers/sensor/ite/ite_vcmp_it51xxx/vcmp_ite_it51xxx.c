...
static void clear_vcmp_status(const struct device *dev, int channel)
{
	...
	const uintptr_t reg_base = config->reg_base;

	/* W/C voltage comparator specific channel interrupt status */
	switch (channel) {
	case VCMP_CHANNEL_0:
		sys_write8(sys_read8(reg_base + REG_VCMP_VCMP0SCTL), reg_base + REG_VCMP_VCMP0SCTL);
		break;

	case VCMP_CHANNEL_1:
		sys_write8(sys_read8(reg_base + REG_VCMP_VCMP1SCTL), reg_base + REG_VCMP_VCMP1SCTL);
		break;

	case VCMP_CHANNEL_2:
		sys_write8(sys_read8(reg_base + REG_VCMP_VCMP2SCTL), reg_base + REG_VCMP_VCMP2SCTL);
		break;
	}
}
...
static int vcmp_set_threshold(const struct device *dev, enum sensor_attribute attr, int32_t reg_val)
{
	...
	if ((attr == SENSOR_ATTR_UPPER_THRESH) ||
		...
		sys_write8(sys_read8(base_ch + REG_VCMP_CH_CTL) | VCMP_GREATER_THRESHOLD,
			   base_ch + REG_VCMP_CH_CTL);
	} else {
		sys_write8(sys_read8(base_ch + REG_VCMP_CH_CTL) & ~VCMP_GREATER_THRESHOLD,
			   base_ch + REG_VCMP_CH_CTL);
	}
				     ...
				     enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_UPPER_VOLTAGE_THRESH:
		 ...
		 */
		reg_val = (val->val1 * VCMP_THRESHOLD / VCMP_MAX_MVOLT);
...
static void vcmp_it51xxx_isr(const struct device *dev)
{
	...
	int status = ((sys_read8(reg_base + REG_VCMP_VCMP0SCTL) & VCMP_CMPXRTIS) >> 6) |
		     ((sys_read8(reg_base + REG_VCMP_VCMP1SCTL) & VCMP_CMPXRTIS) >> 5) |
		     ((sys_read8(reg_base + REG_VCMP_VCMP2SCTL) & VCMP_CMPXRTIS) >> 4);
...
static int vcmp_it51xxx_init(const struct device *dev)
{
	const struct vcmp_it51xxx_config *const config = dev->config;
	struct vcmp_it51xxx_data *const data = dev->data;
	uintptr_t base_ch = config->base_ch;
	uintptr_t reg_vcmpscp2 = config->reg_base + REG_VCMP_VCMPSCP2;
	...
	sys_write8(data->adc_ch_cfg.channel_id & VCMP_CHANNEL_ID_REG_MASK, base_ch);

	/* Set minimum scan period for "all" voltage comparator */
	sys_write8(VCMP_SCAN_PERIOD << 4, reg_vcmpscp2);
...
#define VCMP_IT51XXX_INIT(inst)                                                                    \
	static const struct vcmp_it51xxx_config vcmp_it51xxx_cfg_##inst = {                        \
		...
		.reg_base = DT_REG_ADDR_BY_IDX(DT_NODELABEL(vcmp_base), 0),                        \
