...
/* 0xC0/0xD0/0xE0: Tach channel 0~2 achometer speed (2 bytes value) */
...
static bool tach_ch_is_valid(const struct device *dev, int tach_ch)
{
	...
	if (((reg_val & TACH_CH_SEL) == tach_ch) && (reg_val & TACH_CH_DVS)) {
		/* Tachometer channel match register setting and data valid */
		valid = true;
	}
