...
static int isds_2536030320001_sample_fetch(const struct device *dev, enum sensor_channel channel)
{
	...
	case SENSOR_CHAN_AMBIENT_TEMP:
		accel_step_sleep_duration =
			((uint32_t)1000000000 /
			 (uint32_t)sensor_value_to_milli(
	...
	case SENSOR_CHAN_GYRO_XYZ:
		step_sleep_duration = ((uint32_t)1000000000 /
				       (uint32_t)sensor_value_to_milli(
					       &isds_2536030320001_gyro_odr_list[data->gyro_odr]) /
				       MAX_POLL_STEP_COUNT);
		break;
...
static const struct sensor_driver_api isds_2536030320001_driver_api = {
...
#warning "ISDS driver enabled without any devices"
#endif
...
#define ISDS_2536030320001_CONFIG_COMMON(inst)                                                     \
	...
	ISDS_2536030320001_CONFIG_TAP(inst) ISDS_2536030320001_CONFIG_FREEFALL(inst)               \
...
#define ISDS_2536030320001_DEFINE(inst)                                                            \
	static struct isds_2536030320001_data isds_2536030320001_data_##inst = COND_CODE_1(        \
	...
	static const struct isds_2536030320001_config isds_2536030320001_config_##inst =           \
		COND_CODE_1(DT_INST_ON_BUS(inst, i2c), (ISDS_2536030320001_CONFIG_I2C(inst)), ())  \
			COND_CODE_1(DT_INST_ON_BUS(inst, spi),                                     \
				    (ISDS_2536030320001_CONFIG_SPI(inst)), ());                    \
	SENSOR_DEVICE_DT_INST_DEFINE(inst, isds_2536030320001_init, NULL,                          \
				     &isds_2536030320001_data_##inst,                              \
				     &isds_2536030320001_config_##inst, POST_KERNEL,               \
				     CONFIG_SENSOR_INIT_PRIORITY, &isds_2536030320001_driver_api);
