...
static inline int i2c_read_bank(const union icm40627_bus *bus, uint8_t reg, uint8_t bank, uint8_t *buf,
			 size_t len)
{
...
static inline int i2c_write_bank(const union icm40627_bus *bus, uint16_t reg, uint8_t bank, uint8_t buf)
{
	int res = i2c_reg_write_byte_dt(&bus->i2c, REG_REG_BANK_SEL, bank);
