				 ...
				 uint32_t size)
{
	...
	if (calc_crc != read_buf[size * 2]) {
		LOG_ERR("CRC Mismatch! Calc: %x, Recv: %x", calc_crc, read_buf[size * 2]);
		return -EBADMSG;
	}

	for (uint32_t i = 0; i < size; i++) {
		buf[i] = sys_get_be16(&read_buf[i * 2]);
	}
				  ...
				  uint32_t size)
{
	...
	for (uint32_t i = 0; i < size; i++) {
		sys_put_be16(buf[i], &write_buf[1 + (i * 2)]);
	}

	write_buf[size * 2 + 1] = crc8(write_buf, size * 2 + 1, 0x1D, 0xFF, false) ^ 0xFF;
