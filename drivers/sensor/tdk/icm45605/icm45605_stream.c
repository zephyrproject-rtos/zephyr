...
LOG_MODULE_REGISTER(ICM45605_STREAM, CONFIG_SENSOR_LOG_LEVEL);

enum icm45605_stream_state {
	ICM45605_STREAM_OFF = 0,
	ICM45605_STREAM_ON = 1,
	ICM45605_STREAM_BUSY = 2,
};
...
static inline void icm45605_stream_result(const struct device *dev, int result)
{
	struct icm45605_data *data = dev->data;
	const struct icm45605_config *cfg = dev->config;
	struct rtio_iodev_sqe *iodev_sqe = data->stream.iodev_sqe;

	(void)atomic_set(&data->stream.state, ICM45605_STREAM_OFF);
...
static void icm45605_complete_handler(struct rtio *ctx, const struct rtio_sqe *sqe, int result,
				      void *arg)
{
	...
	if (result < 0) {
		...
		icm45605_stream_result(dev, result);
		return;
	}
	...
	struct icm45605_encoded_data *buf;
	uint32_t buf_len;
	...
	CHECKIF(err != 0 || buf_len == 0) {
		...
		icm45605_stream_result(dev, -ENOMEM);
		return;
	}
	...
	if (should_flush_fifo(read_cfg, int_status)) {
		...
		err = icm45605_prep_reg_write_rtio_async(&data->bus, REG_FIFO_CONFIG2, &write_reg,
							 1, NULL);
		if (err < 0) {
	...
	icm45605_stream_result(dev, err);
}

static void icm45605_event_handler(const struct device *dev)
{
	struct icm45605_data *data = dev->data;
	const struct icm45605_config *cfg = dev->config;
	const struct sensor_read_config *read_cfg = data->stream.iodev_sqe->sqe.iodev->data;
	...
	if (!data->stream.iodev_sqe ||
		...
		(void)atomic_set(&data->stream.state, ICM45605_STREAM_OFF);
		(void)gpio_pin_interrupt_configure_dt(&cfg->int_gpio, GPIO_INT_DISABLE);
		err = icm45605_prep_reg_write_rtio_async(&data->bus, REG_INT1_CONFIG0, &val, 1,
							 NULL);
		if (err < 0) {
	...
	if (atomic_cas(&data->stream.state, ICM45605_STREAM_ON, ICM45605_STREAM_BUSY) == false) {
	...
	err = icm45605_prep_reg_read_rtio_async(&data->bus, REG_INT1_STATUS0 | REG_READ_BIT,
	...
	struct icm45605_encoded_data *buf;
	uint32_t buf_len;
	uint32_t buf_len_required = sizeof(struct icm45605_encoded_header);
	...
	if (should_read_fifo(read_cfg)) {
		...
		buf_len_required +=
			(num_frames_to_read * sizeof(struct icm45605_encoded_fifo_payload));
	} else if (should_read_data(read_cfg)) {
		buf_len_required += sizeof(struct icm45605_encoded_payload);
	} else {
	...
	if (should_read_fifo(read_cfg)) {
		 ...
		 */
		buf->header.accel_fs = ICM45605_DT_ACCEL_FS_16;
		buf->header.gyro_fs = ICM45605_DT_GYRO_FS_2000;
		buf->header.channels = 0x7F; /* Signal all channels are available */
		buf->header.fifo_count = should_read_all_fifo(read_cfg)
						 ? FIFO_COUNT_MAX_HIGH_RES
						 : cfg->settings.fifo_watermark;

		err = icm45605_prep_reg_read_rtio_async(
			&data->bus, REG_FIFO_DATA | REG_READ_BIT, (uint8_t *)&buf->fifo_payload,
			buf->header.fifo_count * sizeof(struct icm45605_encoded_fifo_payload),
			&data_rd_sqe);
		if (err < 0) {
		...
		err = icm45605_prep_reg_read_rtio_async(
			&data->bus, REG_ACCEL_DATA_X1_UI | REG_READ_BIT, buf->payload.buf,
			sizeof(buf->payload.buf), &data_rd_sqe);
		if (err < 0) {
			LOG_ERR("Failed to acquire RTIO SQEs");
			icm45605_stream_result(dev, -ENOMEM);
			return;
	...
	rtio_sqe_prep_callback_no_cqe(complete_sqe, icm45605_complete_handler, (void *)dev,
...
#if DT_HAS_COMPAT_ON_BUS_STATUS_OKAY(invensense_icm45605, i3c)
static int icm45605_ibi_cb(struct i3c_device_desc *target, struct i3c_ibi_payload *payload)
{
	icm45605_event_handler(target->dev);

	return 0;
}
#endif

static void icm45605_gpio_callback(const struct device *gpio_dev, struct gpio_callback *cb,
				   uint32_t pins)
{
	struct icm45605_stream *stream = CONTAINER_OF(cb, struct icm45605_stream, cb);
	const struct device *dev = stream->dev;

	icm45605_event_handler(dev);
}

static inline bool settings_changed(const struct icm45605_stream *a,
				    const struct icm45605_stream *b)
{
...
void icm45605_stream_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	 ...
	 * in such case.
	 */
	struct icm45605_stream stream = {0};
	...
	(void)atomic_set(&data->stream.state, ICM45605_STREAM_ON);
	...
	if (settings_changed(&data->stream, &stream)) {
		...
		err = icm45605_reg_read_rtio(&data->bus, REG_INT1_STATUS0 | REG_READ_BIT, &val, 1);
		if (err) {
		...
		err = icm45605_reg_write_rtio(&data->bus, REG_FIFO_CONFIG3, &val, 1);
		if (err) {
		...
		err = icm45605_reg_write_rtio(&data->bus, REG_FIFO_CONFIG0, &val, 1);
		if (err) {
		...
		if (data->stream.settings.enabled.fifo_ths ||
			...
			err = icm45605_reg_write_rtio(&data->bus, REG_FIFO_CONFIG2, &val, 1);
			if (err) {
			...
			err = icm45605_reg_write_rtio(&data->bus, REG_FIFO_CONFIG1_0,
						      (uint8_t *)&fifo_ths, 2);
			if (err) {
...
int icm45605_stream_init(const struct device *dev)
{
	...
	if (cfg->int_gpio.port) {
		...
		gpio_init_callback(&data->stream.cb, icm45605_gpio_callback,
	...
	} else if (data->bus.rtio.type == ICM45605_BUS_I3C) {
		...
		data->bus.rtio.i3c.desc->ibi_cb = icm45605_ibi_cb;
