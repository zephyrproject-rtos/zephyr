				      ...
				      size_t size, struct rtio_sqe **out)
{
	...
	if (bus->rtio.type == ICM45605_BUS_I2C) {
		read_buf_sqe->iodev_flags |= RTIO_IODEV_I2C_STOP | RTIO_IODEV_I2C_RESTART;
	} else if (bus->rtio.type == ICM45605_BUS_I3C) {
...
int icm45605_reg_read_rtio(const struct icm45605_bus *bus, uint8_t start, uint8_t *buf, int size)
{
	...
	ret = icm45605_prep_reg_read_rtio_async(bus, start, buf, size, NULL);
	if (ret < 0) {
		return ret;
	}
			    ...
			    int size)
{
	...
	ret = icm45605_prep_reg_write_rtio_async(bus, reg, buf, size, NULL);
	if (ret < 0) {
		return ret;
	}
