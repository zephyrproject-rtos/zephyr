...
LOG_MODULE_REGISTER(ICM566XX_STREAM, CONFIG_SENSOR_LOG_LEVEL);

enum icm566xx_stream_state {
	ICM566XX_STREAM_OFF = 0,
	ICM566XX_STREAM_ON = 1,
	ICM566XX_STREAM_BUSY = 2,
};
...
static inline bool should_flush_fifo(const struct sensor_read_config *read_cfg, uint8_t int_status)
{
	...
	int1_status0_t status = *(int1_status0_t *)&int_status;
	bool fifo_ths  = status.int1_status_fifo_ths;
	bool fifo_full = status.int1_status_fifo_full;
...
static inline void icm566xx_stream_result(const struct device *dev, int result)
{
...
static void icm566xx_complete_handler(struct rtio *ctx, const struct rtio_sqe *sqe, int result,
				      void *arg)
{
	...
	if (result < 0) {
		...
		icm566xx_stream_result(dev, result);
		return;
	}

	data->stream.data.events.drdy = status.int1_status_drdy;
	data->stream.data.events.fifo_ths = status.int1_status_fifo_ths;
	data->stream.data.events.fifo_full = status.int1_status_fifo_full;

	struct icm566xx_encoded_data *buf;
	uint32_t buf_len;
	...
	CHECKIF(err != 0 || buf_len == 0) {
		...
		icm566xx_stream_result(dev, -ENOMEM);
		return;
	}

	buf->header.timestamp = data->stream.data.timestamp;
	buf->header.events = *(uint8_t *)&data->stream.data.events;

	if (should_flush_fifo(read_cfg, int_status)) {
		fifo_config2_t fifo_cfg2 = { 0 };

		fifo_cfg2.fifo_flush       = 1;
		fifo_cfg2.fifo_wr_wm_gt_th = wm_gt_ths;
		uint8_t write_reg = *(uint8_t *)&fifo_cfg2;

		LOG_WRN("Flushing FIFO: %d", int_status);

		err = icm566xx_prep_reg_write_rtio_async(&data->bus, FIFO_CONFIG2, &write_reg, 1,
							 NULL);
		if (err < 0) {
	...
	icm566xx_stream_result(dev, err);
}

static void icm566xx_event_handler(const struct device *dev)
{
	struct icm566xx_data *data = dev->data;
	const struct icm566xx_config *cfg = dev->config;
	const struct sensor_read_config *read_cfg = data->stream.iodev_sqe->sqe.iodev->data;
	...
	if (!data->stream.iodev_sqe ||
		...
		(void)atomic_set(&data->stream.state, ICM566XX_STREAM_OFF);
		(void)gpio_pin_interrupt_configure_dt(&cfg->int_gpio, GPIO_INT_DISABLE);
		err = icm566xx_prep_reg_write_rtio_async(&data->bus, INT1_CONFIG0, &val, 1, NULL);
		if (err < 0) {
	...
	err = icm566xx_prep_reg_read_rtio_async(&data->bus, INT1_STATUS0 | REG_READ_BIT,
	...
	struct icm566xx_encoded_data *buf;
	uint32_t buf_len;
	uint32_t buf_len_required = sizeof(struct icm566xx_encoded_header);
	...
	if (should_read_fifo(read_cfg)) {
		...
		buf_len_required +=
			(num_frames_to_read * sizeof(struct icm566xx_encoded_fifo_payload));
	} else if (should_read_data(read_cfg)) {
		buf_len_required += sizeof(struct icm566xx_encoded_payload);
	} else {
	...
	if (should_read_fifo(read_cfg)) {
		 ...
		 */
		buf->header.accel_fs = ICM566XX_DT_ACCEL_FS_32;
		buf->header.gyro_fs = ICM566XX_DT_GYRO_FS_4000;
		buf->header.channels = 0x7F; /* Signal all channels are available */
		buf->header.fifo_count = should_read_all_fifo(read_cfg)
						 ? FIFO_COUNT_MAX_HIGH_RES
						 : cfg->settings.fifo_watermark;

		err = icm566xx_prep_reg_read_rtio_async(
			&data->bus, FIFO_DATA | REG_READ_BIT, (uint8_t *)&buf->fifo_payload,
			buf->header.fifo_count * sizeof(struct icm566xx_encoded_fifo_payload),
			&data_rd_sqe);
		if (err < 0) {
		...
		err = icm566xx_prep_reg_read_rtio_async(&data->bus, ACCEL_DATA_X_0 | REG_READ_BIT,
							buf->payload.buf, 14, &data_rd_sqe);
		if (err < 0) {
			LOG_ERR("Failed to acquire RTIO SQEs");
			icm566xx_stream_result(dev, -ENOMEM);
			return;
		...
		err = icm566xx_prep_reg_read_rtio_async(&data->bus, EXT_DATA_X | REG_READ_BIT,
							buf->payload.ext_data, 3, &ext_read_sqe);
		if (err < 0) {
			LOG_ERR("Failed to acquire RTIO ext data");
			icm566xx_stream_result(dev, -ENOMEM);
			return;
		}
	...
	rtio_sqe_prep_callback_no_cqe(complete_sqe, icm566xx_complete_handler, (void *)dev,
...
static int icm566xx_ibi_cb(struct i3c_device_desc *target, struct i3c_ibi_payload *payload)
{
	icm566xx_event_handler(target->dev);

	return 0;
}
#endif

static void icm566xx_gpio_callback(const struct device *gpio_dev, struct gpio_callback *cb,
				   uint32_t pins)
{
	struct icm566xx_stream *stream = CONTAINER_OF(cb, struct icm566xx_stream, cb);
	const struct device *dev = stream->dev;

	icm566xx_event_handler(dev);
}

static inline bool settings_changed(const struct icm566xx_stream *a,
				    const struct icm566xx_stream *b)
{
...
void icm566xx_stream_submit(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	 ...
	 * in such case.
	 */
	struct icm566xx_stream stream = {0};
	...
	(void)atomic_set(&data->stream.state, ICM566XX_STREAM_ON);

	if (settings_changed(&data->stream, &stream)) {
		fifo_config0_t fifo_cfg0 = { 0 };
		fifo_config3_t fifo_cfg3 = { 0 };
		int1_config0_t int1_cfg = { 0 };

		data->stream.settings = stream.settings;

		/* Disable all interrupts before re-configuring */
		err = icm566xx_reg_write_rtio(&data->bus, INT1_CONFIG0, &val, 1);
		if (err) {
		...
		err = icm566xx_reg_read_rtio(&data->bus, INT1_STATUS0 | REG_READ_BIT, &val, 1);
		if (err) {
		...
		val = *(uint8_t *)&fifo_cfg3;

		err = icm566xx_reg_write_rtio(&data->bus, FIFO_CONFIG3, &val, 1);
		if (err) {
		...
		int1_cfg.int1_status_en_drdy	  = data->stream.settings.enabled.drdy;
		int1_cfg.int1_status_en_fifo_ths  = data->stream.settings.enabled.fifo_ths;
		int1_cfg.int1_status_en_fifo_full = data->stream.settings.enabled.fifo_full;
		val = *(uint8_t *)&int1_cfg;
		...
		fifo_cfg0.fifo_mode  = FIFO_CONFIG0_FIFO_MODE_BYPASS;
		fifo_cfg0.fifo_depth = FIFO_CONFIG0_FIFO_DEPTH_APEX;
		val = *(uint8_t *)&fifo_cfg0;

		err = icm566xx_reg_write_rtio(&data->bus, FIFO_CONFIG0, &val, 1);
		if (err) {
		...
		if (data->stream.settings.enabled.fifo_ths ||
			...
			fifo_cfg2.fifo_flush       = 1;
			fifo_cfg2.fifo_wr_wm_gt_th = wm_gt_ths;
			val = *(uint8_t *)&fifo_cfg2;

			err = icm566xx_reg_write_rtio(&data->bus, FIFO_CONFIG2, &val, 1);
			if (err) {
			...
			err = icm566xx_reg_write_rtio(&data->bus, FIFO_CONFIG1_0,
						      (uint8_t *)&fifo_ths, 2);
			if (err) {
			...
			fifo_cfg0.fifo_mode  = FIFO_CONFIG0_FIFO_MODE_STREAM;
			fifo_cfg0.fifo_depth = FIFO_CONFIG0_FIFO_DEPTH_APEX;
			val = *(uint8_t *)&fifo_cfg0;
			...
			fifo_cfg3.fifo_if_en    = 1;
			fifo_cfg3.fifo_accel_en = 1;
			fifo_cfg3.fifo_gyro_en  = 1;
			fifo_cfg3.fifo_hires_en = 1;
			val = *(uint8_t *)&fifo_cfg3;
...
int icm566xx_stream_init(const struct device *dev)
{
	...
	if (cfg->int_gpio.port) {
		...
		gpio_init_callback(&data->stream.cb, icm566xx_gpio_callback,
	...
	} else if (data->bus.rtio.type == ICM566XX_BUS_I3C) {
#if defined(CONFIG_I3C_USE_IBI)
		const struct device *i3c_master_bus =
			(const struct device *)data->bus.rtio.iodev->api;

		data->bus.rtio.i3c.desc = i3c_device_find(i3c_master_bus, &data->bus.rtio.i3c.id);
		if (data->bus.rtio.i3c.desc == NULL) {
			LOG_ERR("Failed to find I3C device");
			return -ENODEV;
		}
		data->bus.rtio.i3c.desc->ibi_cb = icm566xx_ibi_cb;
