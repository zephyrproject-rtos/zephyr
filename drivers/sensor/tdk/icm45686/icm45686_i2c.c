...
int icm45686_read_mreg(const struct i2c_dt_spec *bus, uint16_t reg, uint8_t *buf, size_t len)
{
	int res;
	uint8_t reg_pwr_mgmt0;
...
int icm45686_write_mreg(const struct i2c_dt_spec *bus, uint16_t reg, uint8_t buf)
{
	int res;
	uint8_t reg_pwr_mgmt0;
	uint8_t bank = FIELD_GET(REG_BANK_MASK, reg);
...
{
	uint8_t temp = 0;
	int res = icm45686_read_mreg(bus, reg, &temp, 1);
	...
	return icm45686_write_mreg(bus, reg, temp);
}
