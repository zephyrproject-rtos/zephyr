...
LOG_MODULE_DECLARE(ICM45686, CONFIG_SENSOR_LOG_LEVEL);
				   ...
				   uint32_t pins)
{
	...
	struct icm45686_data *data = CONTAINER_OF(cb, struct icm45686_data, gpio_cb);
...
{
	struct icm45686_data *data = CONTAINER_OF(work, struct icm45686_data, work);
...
int icm45686_trigger_init(const struct device *dev)
{
	...
	gpio_init_callback(&data->gpio_cb, icm45686_gpio_callback, BIT(cfg->gpio_int.pin));
	...
	k_thread_create(&data->thread, data->thread_stack, CONFIG_ICM45686_THREAD_STACK_SIZE,
			icm45686_thread, data, NULL, NULL,
			K_PRIO_COOP(CONFIG_ICM45686_THREAD_PRIORITY), 0, K_NO_WAIT);
#elif defined(CONFIG_ICM45686_TRIGGER_GLOBAL_THREAD)
	data->work.handler = icm45686_work_handler;
#endif
...
int icm45686_trigger_enable_interrupt(const struct device *dev)
{
	...
	return icm45686_single_write(&cfg->bus, REG_INT1_CONFIG0, BIT_INT1_STATUS_EN_DRDY);
}
