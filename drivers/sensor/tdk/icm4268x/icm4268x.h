...
static inline uint8_t icm4268x_accel_fs_to_reg(uint8_t g)
{
	if (g >= 32) {
		...
		return ICM4268X_DT_ACCEL_FS_4;
	} else {
		return ICM4268X_DT_ACCEL_FS_2;
	}
}
...
static inline void icm4268x_accel_reg_to_fs(uint8_t fs, struct sensor_value *out)
{
	...
	case ICM4268X_DT_ACCEL_FS_16:
		...
		return;
	case ICM4268X_DT_ACCEL_FS_8:
		sensor_g_to_ms2(8, out);
		return;
	case ICM4268X_DT_ACCEL_FS_4:
		sensor_g_to_ms2(4, out);
		return;
	case ICM4268X_DT_ACCEL_FS_2:
...
static inline uint8_t icm4268x_gyro_fs_to_reg(uint16_t dps)
{
	if (dps >= 4000) {
		...
		return ICM4268X_DT_GYRO_FS_500;
	} else if (dps >= 250) {
		return ICM4268X_DT_GYRO_FS_250;
	} else if (dps >= 125) {
		return ICM4268X_DT_GYRO_FS_125;
	} else if (dps >= 62) {
		return ICM4268X_DT_GYRO_FS_62_5;
	} else if (dps >= 31) {
		return ICM4268X_DT_GYRO_FS_31_25;
	} else {
		return ICM4268X_DT_GYRO_FS_15_625;
	}
}
...
static inline void icm4268x_gyro_reg_to_fs(uint8_t fs, struct sensor_value *out)
{
	...
	case ICM4268X_DT_GYRO_FS_2000:
		...
		return;
	case ICM4268X_DT_GYRO_FS_1000:
		sensor_degrees_to_rad(1000, out);
		return;
	case ICM4268X_DT_GYRO_FS_500:
		sensor_degrees_to_rad(500, out);
		return;
	case ICM4268X_DT_GYRO_FS_250:
		sensor_degrees_to_rad(250, out);
		return;
	case ICM4268X_DT_GYRO_FS_125:
		sensor_degrees_to_rad(125, out);
		return;
	case ICM4268X_DT_GYRO_FS_62_5:
		sensor_10udegrees_to_rad(6250000, out);
		return;
	case ICM4268X_DT_GYRO_FS_31_25:
		sensor_10udegrees_to_rad(3125000, out);
		return;
	case ICM4268X_DT_GYRO_FS_15_625:
