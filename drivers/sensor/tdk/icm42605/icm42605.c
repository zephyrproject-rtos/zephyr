...
static const uint16_t icm42605_gyro_sensitivity_x10[] = {1310, 655, 328, 164};
...
static inline void icm42605_convert_temp(struct sensor_value *val, int16_t raw_val)
{
...
static int icm42605_channel_get(const struct device *dev, enum sensor_channel chan,
				struct sensor_value *val)
{
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		icm42605_convert_accel(val, drv_data->accel_x, drv_data->accel_sensitivity_shift);
	...
	case SENSOR_CHAN_ACCEL_Y:
		icm42605_convert_accel(val, drv_data->accel_y, drv_data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		icm42605_convert_accel(val, drv_data->accel_z, drv_data->accel_sensitivity_shift);
		break;
	case SENSOR_CHAN_GYRO_XYZ:
		icm42605_convert_gyro(val, drv_data->gyro_x, drv_data->gyro_sensitivity_x10);
		icm42605_convert_gyro(val + 1, drv_data->gyro_y, drv_data->gyro_sensitivity_x10);
		icm42605_convert_gyro(val + 2, drv_data->gyro_z, drv_data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_X:
		icm42605_convert_gyro(val, drv_data->gyro_x, drv_data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_Y:
		icm42605_convert_gyro(val, drv_data->gyro_y, drv_data->gyro_sensitivity_x10);
		break;
	case SENSOR_CHAN_GYRO_Z:
		icm42605_convert_gyro(val, drv_data->gyro_z, drv_data->gyro_sensitivity_x10);
		break;
...
int icm42605_tap_fetch(const struct device *dev)
{
	...
	if (drv_data->tap_en && (drv_data->tap_handler || drv_data->double_tap_handler)) {
		...
		if (drv_data->fifo_data[0] & BIT_INT_STATUS_TAP_DET) {
			result = inv_spi_read(&cfg->spi, REG_APEX_DATA4, drv_data->fifo_data, 1);
			if (drv_data->fifo_data[0] & APEX_TAP) {
				if (drv_data->tap_trigger->type == SENSOR_TRIG_TAP) {
					if (drv_data->tap_handler) {
						LOG_DBG("Single Tap detected");
						drv_data->tap_handler(dev, drv_data->tap_trigger);
					}
				} else {
					LOG_ERR("Trigger type is mismatched");
				}
			} else if (drv_data->fifo_data[0] & APEX_DOUBLE_TAP) {
				if (drv_data->double_tap_trigger->type == SENSOR_TRIG_DOUBLE_TAP) {
	...
	if (drv_data->fifo_data[0] & BIT_INT_STATUS_DRDY) {
		...
		if (drv_data->fifo_data[0] & BIT_FIFO_HEAD_ACCEL) {
			...
			if (!(drv_data->fifo_data[1] == FIFO_ACCEL0_RESET_VALUE &&
				...
				drv_data->accel_x =
					(drv_data->fifo_data[1] << 8) + (drv_data->fifo_data[2]);
				drv_data->accel_y =
					(drv_data->fifo_data[3] << 8) + (drv_data->fifo_data[4]);
				drv_data->accel_z =
					(drv_data->fifo_data[5] << 8) + (drv_data->fifo_data[6]);
			}
...
static int icm42605_attr_set(const struct device *dev, enum sensor_channel chan,
			     enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_CHAN_ACCEL_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			...
			if (val->val1 < ACCEL_FS_16G || val->val1 > ACCEL_FS_2G) {
	...
	case SENSOR_CHAN_GYRO_XYZ:
		if (attr == SENSOR_ATTR_SAMPLING_FREQUENCY) {
			...
			if (val->val1 < GYRO_FS_2000DPS || val->val1 > GYRO_FS_15DPS) {
				LOG_ERR("Incorrect fullscale value");
				return -EINVAL;
			} else {
...
static int icm42605_attr_get(const struct device *dev, enum sensor_channel chan,
...
static int icm42605_data_init(struct icm42605_data *data, const struct icm42605_config *cfg)
{
	data->accel_x = 0;
	data->accel_y = 0;
	data->accel_z = 0;
