...
static int adxl355_set_fifo_watermark(const struct device *dev, const struct sensor_value *val)
{
	...
	if (fifo_watermark > ADXL355_FIFO_MAX_ENTRIES) {
...
static int adxl355_set_ext_clk(const struct device *dev, const struct sensor_value *val)
{
	...
	if (val->val1 < ADXL355_DISABLE_EXT_CLK || val->val1 > ADXL355_ENABLE_EXT_CLK) {
...
static int adxl355_set_ext_sync(const struct device *dev, const struct sensor_value *val)
{
	...
	if (val->val1 < ADXL355_INTERNAL_SYNC || val->val1 > ADXL355_EXT_SYNC_INTP_FILT) {
...
static int adxl355_set_i2c_hs(const struct device *dev, const struct sensor_value *val)
{
	...
	if (val->val1 < ADXL355_DISABLE_I2C_HS || val->val1 > ADXL355_ENABLE_I2C_HS) {
...
static int adxl355_set_int_pol(const struct device *dev, const struct sensor_value *val)
{
	...
	if (val->val1 < ADXL355_INT_POL_ACTIVE_LOW || val->val1 > ADXL355_INT_POL_ACTIVE_HIGH) {
			      ...
			      enum sensor_channel chan)
{
	...
	case SENSOR_CHAN_ACCEL_X:
		data->extra_attr.offset_x = offset_val & GENMASK64(63, 4);
		return adxl355_reg_write(dev, ADXL355_OFFSET_X_H, buf, 2);
	case SENSOR_CHAN_ACCEL_Y:
		data->extra_attr.offset_y = offset_val & GENMASK64(63, 4);
		return adxl355_reg_write(dev, ADXL355_OFFSET_Y_H, buf, 2);
	case SENSOR_CHAN_ACCEL_Z:
		data->extra_attr.offset_z = offset_val & GENMASK64(63, 4);
...
static int is_valid_act_threshold(int64_t accel_val, enum adxl355_range range)
{
	switch (range) {
	case ADXL355_RANGE_2G:
		/* Activity Threshold limits in micro m/s^2 for ±2g range; */
		/* g = 9.81m/s^2; ±2g = ±19620000 micro m/s^2 */
		...
		break;
	case ADXL355_RANGE_4G:
		/* Activity threshold limits in micro m/s^2 for ±4g range; */
		/* g = 9.81m/s^2; ±4g = ±39240000 micro m/s^2 */
		...
		break;
	case ADXL355_RANGE_8G:
		/* Activity threshold limits in micro m/s^2 for ±8g range; */
		/* g = 9.81m/s^2; ±8g = ±78480000 micro m/s^2 */
...
#define ADXL355_DEVICE_INIT(inst)                                                                  \
	...
	SENSOR_DEVICE_DT_INST_DEFINE(inst, adxl355_init, PM_DEVICE_DT_INST_GET(inst),              \
				     ...
				     CONFIG_ADXL355_INIT_PRIORITY, &adxl355_api_funcs);
...
#define ADXL355_CONFIG(inst)                                                                       \
	...
	.ext_sync = DT_INST_PROP(inst, ext_sync), .i2c_hs = !DT_INST_PROP(inst, i2c_fast_mode),    \
