...
uint32_t samples_global = 0, array_size_global = 0;
...
void adxl345_submit_stream(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	if (fifo_watermark_irq != data->fifo_watermark_irq) {
		...
		rc = adxl345_reg_write_mask(dev, ADXL345_INT_MAP, ADXL345_INT_MAP_WATERMARK_MSK, int_value);
		if (rc < 0) {
			return;
		}
...
static void adxl345_fifo_flush_rtio(const struct device *dev)
{
    ...
    rtio_sqe_prep_tiny_write(write_fifo_addr, data->iodev, RTIO_PRIO_NORM, reg_addr_w2, 2, NULL);
    ...
    rtio_sqe_prep_tiny_write(write_fifo_addr, data->iodev, RTIO_PRIO_NORM, reg_addr_w3, 2, NULL);
...
static void adxl345_process_fifo_samples_cb(struct rtio *r, const struct rtio_sqe *sqr, void *arg)
{
	...
	array_size_global = fifo_samples;
	for (size_t i = 0; i < fifo_samples; i++) {
		struct rtio_sqe *write_fifo_addr = rtio_sqe_acquire(data->rtio_ctx);
		struct rtio_sqe *read_fifo_data = rtio_sqe_acquire(data->rtio_ctx);
		array_size_global--;
		const uint8_t reg_addr = ADXL345_REG_READ(0x32) | ADXL345_MULTIBYTE_FLAG;
		...
		rtio_sqe_prep_read(read_fifo_data, data->iodev, RTIO_PRIO_NORM, read_buf + samples_global/*(data->bufx) + samples_global*/, 0x6,
			   current_sqe);
		samples_global += 6;
		if (i == fifo_samples-1)
		{
			...
			rtio_sqe_prep_callback(complete_op, adxl345_fifo_read_cb, (void *)dev, current_sqe);
		}
		rtio_submit(data->rtio_ctx, 0);
