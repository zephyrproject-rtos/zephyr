...
static const uint8_t adxl313_cache_volatile[] = {
...
static int adxl313_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	rc = adxl313_read_sample(dev, &data->sample);
	if (rc < 0) {
			       ...
			       struct sensor_value *val)
{
	...
	case SENSOR_CHAN_ACCEL_X:
		adxl313_accel_convert(val, data->sample.x, data->selected_range,
				      data->is_full_res);
		break;
	case SENSOR_CHAN_ACCEL_Y:
		adxl313_accel_convert(val, data->sample.y, data->selected_range,
				      data->is_full_res);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		adxl313_accel_convert(val, data->sample.z, data->selected_range,
				      data->is_full_res);
		break;
 ...
 * following steps:
 * 1. Verifies that the communication bus (I2C or SPI) is ready
 * 2. Reads and validates the device ID registers
 * 3. Configures default power control and interrupt settings
 * 4. Sets the data format, output data rate (ODR), and range
 * 5. Initializes interrupt handling if configured in device tree
 * 6. Configures FIFO mode (bypassed or streaming)
 * 7. Enables measurement mode
 *
 * The initialization sequence ensures the sensor is in a known state and
 ...
 * @return -EIO if data format configuration fails
 * @return Other negative error codes from register access or interrupt initialization
 */
static int adxl313_init(const struct device *dev)
{
	...
	power_ctl = 0x00;
	if (cfg->bus_type == ADXL313_BUS_SPI) {
