...
/**
 * @brief Set Lower Temperature Threshold for MAX30210
 *
 * @param dev Device Pointer
 * @param val Sensor Value Pointer
 * @return int 0 on success, negative error code on failure
 */
static int max30210_attr_set_lower_thresh(const struct device *dev, const struct sensor_value *val)
{
	int ret;
	...
	sys_put_be16(lower_thresh, lower_thresh_data);
...
/**
 * @brief Set Upper Temperature Threshold for MAX30210
 *
 * @param dev Device Pointer
 * @param val Sensor Value Pointer
 * @return int 0 on success, negative error code on failure
 */
static int max30210_attr_set_upper_thresh(const struct device *dev, const struct sensor_value *val)
{
	int ret;
	struct max30210_data *temp_data = dev->data;
	uint16_t whole_numbers_data;
	...
	sys_put_be16(upper_thresh, upper_thresh_data);
			     ...
			     enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	case SENSOR_ATTR_LOWER_THRESH:
		return max30210_attr_set_lower_thresh(dev, val);
	case SENSOR_ATTR_UPPER_THRESH:
		return max30210_attr_set_upper_thresh(dev, val);
