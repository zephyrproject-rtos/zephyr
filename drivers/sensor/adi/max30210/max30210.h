...
#define MAX30210_RAW_TO_CELSIUS(raw)                                                               \
	((raw) * 0.005) /* Convert raw value to Temperature Celsius                                \
			 */
...
#ifdef CONFIG_MAX30210_STREAM
	uint8_t fifo_data_count;
	uint8_t fifo_watermark_irq;
	uint8_t status1;
	uint8_t fifo_counter1[2];
...
#ifdef CONFIG_MAX30210_STREAM
struct max30210_fifo_data {
	uint8_t int_status;
	uint8_t fifo_samples;
} __attribute__((__packed__));

BUILD_ASSERT(sizeof(struct max30210_fifo_data) % 4 == 0,
