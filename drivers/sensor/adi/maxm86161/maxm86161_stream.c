...
void maxm86161_submit_stream(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	if (fifo_watermark_irq != data->fifo_watermark) {
		...
		ret = i2c_reg_update_byte_dt(&config->i2c, MAXM86161_REG_INT_ENABLE1,
					     MAXM86161_MSK_INT_ENABLE1_A_FULL_EN,
					     FIELD_PREP(MAXM86161_MSK_INT_ENABLE1_A_FULL_EN,
							fifo_watermark_irq));
		if (ret) {
		...
		/* Flush FIFO */
		ret = i2c_reg_update_byte_dt(&config->i2c, MAXM86161_REG_FIFO_CONFIG2,
					     MAXM86161_MSK_FIFO_FLUSH,
					     FIELD_PREP(MAXM86161_MSK_FIFO_FLUSH, 1U));
		if (ret) {
		...
		ret = i2c_burst_read_dt(&config->i2c, MAXM86161_REG_INT_STATUS1, status,
					sizeof(status));
		if (ret) {
