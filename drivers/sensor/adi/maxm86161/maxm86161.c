...
static inline const struct i2c_dt_spec *get_i2c_spec(const struct device *dev)
{
				   ...
				   enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG2, &reg_val);
	if (ret) {
		return ret;
	}
...
static inline int maxm86161_reset_cb(const struct device *dev, enum sensor_channel channel,
			      enum sensor_attribute attr, const struct sensor_value *val)
{
				     ...
				     enum sensor_attribute attr, struct sensor_value *val)
{
	...
	ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG2, &sample_rate_code);
	if (ret) {
		return ret;
	}
	...
	ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG3, &burst_rate_code);
	if (ret) {
		return ret;
	}
			      ...
			      enum sensor_attribute attr, const struct sensor_value *val)
{
	...
	if (max_value == UINT8_MAX) {
		ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), attr_desc->reg, reg_val);
		if (ret) {
			LOG_ERR("Error in writing to register %d: %d", attr_desc->reg, ret);
			return ret;
		}
	} else {
		ret = i2c_reg_update_byte_dt(get_i2c_spec(dev), attr_desc->reg, attr_desc->mask,
					     FIELD_PREP(attr_desc->mask, reg_val));
		if (ret) {
			      ...
			      enum sensor_attribute attr, struct sensor_value *val)
{
	...
	ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), attr_desc->reg, &reg_val);
	if (ret) {
...
{
	return i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_DIE_TEMP_CONFIG, true);
}
...
static int maxm86161_poll_die_temp_meas(const struct device *dev)
{
	...
	for (size_t i = 0; i < MAXM86161_TEMP_MEAS_WAIT_TRIES; i++) {
		k_msleep(MAXM86161_TEMP_MEAS_DELAY_MS);
		ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), MAXM86161_REG_DIE_TEMP_CONFIG,
					   &reg_val);
		if (ret < 0) {
			return ret;
		}
...
static int maxm86161_fetch_die_temp(const struct device *dev)
{
	...
	ret = i2c_burst_read_dt(get_i2c_spec(dev), MAXM86161_REG_DIE_TEMP_INTEGER, temp_raw,
				sizeof(temp_raw));
	if (ret < 0) {
		return ret;
	}
...
static int maxm86161_fetch_fifo(const struct device *dev)
{
	...
	for (int i = 0; i < num_samples; i++) {
		ret = i2c_burst_read_dt(get_i2c_spec(dev), MAXM86161_REG_FIFO_DATA_REGISTER,
					fifo_raw_ptr, MAXM86161_FIFO_SAMPLE_SIZE);
		if (ret < 0) {
			return ret;
		}
		...
		if (tag >= MAXM86161_FIFO_TAG_LEDC1 && tag <= MAXM86161_FIFO_TAG_LEDC6) {
			...
			LOG_WRN("Unknown sample received");
		}
	}
...
static int maxm86161_config_fifo(const struct device *dev)
{
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_FIFO_CONFIG1, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_FIFO_CONFIG2, reg_val);
	if (ret) {
...
static int maxm86161_config_ppg_sync(const struct device *dev)
{
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_SYNC_CONTROL, reg_val);
	if (ret) {
...
static int maxm86161_config_ppg(const struct device *dev)
{
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG1, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG2, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_CONFIG3, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_PROX_INT_THRESH, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_PD_BIAS, reg_val);
	if (ret) {
...
static int maxm86161_config_picket_fence(const struct device *dev)
{
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PPG_PICKET_FENCE, reg_val);
	if (ret) {
...
static int maxm86161_config_sys(const struct device *dev)
{
	...
	ret = i2c_reg_update_byte_dt(
		get_i2c_spec(dev), MAXM86161_REG_SYSTEM_CONTROL,
		MAXM86161_MSK_SYSTEM_CONTROL_LP_MODE,
		FIELD_PREP(MAXM86161_MSK_SYSTEM_CONTROL_LP_MODE, dcfg->sys_cfg.low_power_mode));
	if (ret) {
	...
	ret = i2c_reg_update_byte_dt(
		...
		FIELD_PREP(MAXM86161_MSK_SYSTEM_CONTROL_SINGLE_PPG, dcfg->sys_cfg.single_ppg));
	if (ret) {
...
static int maxm86161_config_led(const struct device *dev)
{
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED_SEQ_REG1, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED_SEQ_REG2, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED_SEQ_REG3, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED1_PA, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED2_PA, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED3_PA, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED_PILOT_PA, reg_val);
	if (ret) {
	...
	ret = i2c_reg_write_byte_dt(get_i2c_spec(dev), MAXM86161_REG_LED_RANGE_1, reg_val);
	if (ret) {
...
static int maxm86161_probe(const struct device *dev)
{
	...
	/* Disable device */
	ret = i2c_reg_update_byte_dt(get_i2c_spec(dev), MAXM86161_REG_SYSTEM_CONTROL,
				     MAXM86161_MSK_SYSTEM_CONTROL_SHDN,
				     FIELD_PREP(MAXM86161_MSK_SYSTEM_CONTROL_SHDN, true));
	if (ret) {
	...
	ret = i2c_reg_update_byte_dt(get_i2c_spec(dev), MAXM86161_REG_SYSTEM_CONTROL,
				     MAXM86161_MSK_SYSTEM_CONTROL_SHDN,
				     FIELD_PREP(MAXM86161_MSK_SYSTEM_CONTROL_SHDN, false));
	if (ret) {
		LOG_ERR("Error shutting down device: %d", ret);
		return ret;
	}
...
static int maxm86161_init(const struct device *dev)
{
	...
	ret = i2c_reg_read_byte_dt(get_i2c_spec(dev), MAXM86161_REG_PART_ID, &reg_val);
	if (ret) {
...
#define MAXM86161_RTIO_DEFINE(inst)                                              \
	I2C_DT_IODEV_DEFINE(maxm86161_iodev_##inst, DT_DRV_INST(inst));          \
	RTIO_DEFINE(maxm86161_rtio_ctx_##inst, CONFIG_MAXM86161_STREAM_CQE_SIZE, \
		    CONFIG_MAXM86161_STREAM_SQE_SIZE);
