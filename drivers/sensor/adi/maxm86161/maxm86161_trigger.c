...
static void maxm86161_thread_cb(const struct device *dev)
{
	...
	if (data->status1_cache_ready) {
		...
		ret = i2c_reg_read_byte_dt(&config->i2c, MAXM86161_REG_INT_STATUS2, &status[1]);
		if (ret) {
		...
		ret = i2c_burst_read_dt(&config->i2c, MAXM86161_REG_INT_STATUS1,
	...
	if ((status[0] & MAXM86161_MSK_INT_STATUS1_A_FULL) &&
		...
		if (data->prox_attr.enabled &&
			...
			i2c_reg_update_byte_dt(&config->i2c, MAXM86161_REG_FIFO_CONFIG2,
					       MAXM86161_MSK_FIFO_FLUSH,
					       FIELD_PREP(MAXM86161_MSK_FIFO_FLUSH, true));
		} else {
			  ...
			  sensor_trigger_handler_t handler)
{
	...
	ret = i2c_reg_update_byte_dt(&config->i2c, int_en_reg, enable_mask,
				     FIELD_PREP(enable_mask, handler ? 1U : 0U));
	if (ret < 0) {
