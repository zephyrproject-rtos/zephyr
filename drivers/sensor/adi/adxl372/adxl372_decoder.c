...
static const uint32_t accel_period_ns[] = {
	...
	[ADXL372_ODR_1600HZ] = UINT32_C(100000000) / 1600,
	[ADXL372_ODR_3200HZ] = UINT32_C(100000000) / 3200,
	[ADXL372_ODR_6400HZ] = UINT32_C(100000000) / 6400,
};

static inline void adxl372_accel_convert_q31(q31_t *out, const uint8_t *buff)
{
	uint16_t data_in = (*buff << 4) | ((*(buff + 1) & 0xF0) >> 4);
	int32_t micro_ms2 = data_in * SENSOR_SCALING_FACTOR;
