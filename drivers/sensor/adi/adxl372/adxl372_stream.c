...
void adxl372_submit_stream(const struct device *dev, struct rtio_iodev_sqe *iodev_sqe)
{
	...
	uint8_t int_mask = 0, status1, status2;
	...
	for (size_t i = 0; i < cfg->count; i++) {
		if ((cfg->triggers[i].trigger == SENSOR_TRIG_FIFO_WATERMARK) ||
			...
			int_mask |= ADXL372_INT1_MAP_FIFO_FULL_MSK;
		}
	}

	if (int_mask) {
		rc = data->hw_tf->write_reg_mask(dev, ADXL372_INT1_MAP, int_mask, int_mask);
		if (rc < 0) {
			return;
		}

		/* Flush the FIFO by disabling it. Save current mode for after the reset. */
		enum adxl372_fifo_mode current_fifo_mode = cfg_372->fifo_config.fifo_mode;

		adxl372_configure_fifo(dev, ADXL372_FIFO_BYPASSED, cfg_372->fifo_config.fifo_format,
				       cfg_372->fifo_config.fifo_samples);

		if (current_fifo_mode == ADXL372_FIFO_BYPASSED) {
			current_fifo_mode = ADXL372_FIFO_STREAMED;
		}

		adxl372_configure_fifo(dev, current_fifo_mode, cfg_372->fifo_config.fifo_format,
				       cfg_372->fifo_config.fifo_samples);
		adxl372_set_op_mode(dev, cfg_372->op_mode);
	}

	rc = adxl372_get_status(dev, &status1, &status2, &fifo_entries); /* Clear status */
	if (rc < 0) {
		return;
	}
