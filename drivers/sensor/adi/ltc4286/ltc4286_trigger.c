...
static void ltc4286_gpio_trig_cb(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	struct ltc4286_gpio_trig *gpio_trig = CONTAINER_OF(cb, struct ltc4286_gpio_trig, gpio_cb);
	struct ltc4286_data *dev_data = gpio_trig->parent_dev->data;
...
static int ltc4286_init_trig_pin(const struct device *dev, struct ltc4286_gpio_trig *gpio_int,
				 const struct gpio_dt_spec *pin, enum ltc4286_trig_idx trig_idx)
{
	int ret;
...
int ltc4286_init_interrupts(const struct device *dev)
{
	...
	ret = ltc4286_init_trig_pin(dev, &dev_data->overpower_trig, &cfg->gpios.overpower_gpio,
				    LTC4286_TRIG_IDX_OP);
	if (ret) {
		goto cleanup;
	}

	ret = ltc4286_init_trig_pin(dev, &dev_data->overcurrent_trig, &cfg->gpios.overcurrent_gpio,
				    LTC4286_TRIG_IDX_OC);
	if (ret) {
		goto cleanup_overpower;
	}

	ret = ltc4286_init_trig_pin(dev, &dev_data->comp_out_trig, &cfg->gpios.comp_out_gpio,
				    LTC4286_TRIG_IDX_COMP);
	if (ret) {
		goto cleanup_overcurrent;
	}

	ret = ltc4286_init_trig_pin(dev, &dev_data->fault_out_trig, &cfg->gpios.fault_out_gpio,
				    LTC4286_TRIG_IDX_FET);
	if (ret) {
		goto cleanup_comp;
	}

	ret = ltc4286_init_trig_pin(dev, &dev_data->power_good_trig, &cfg->gpios.power_good_gpio,
				    LTC4286_TRIG_IDX_PWR_GD);
	if (ret) {
		goto cleanup_fault;
	}

	ret = ltc4286_init_trig_pin(dev, &dev_data->alert_trig, &cfg->gpios.alert_gpio,
