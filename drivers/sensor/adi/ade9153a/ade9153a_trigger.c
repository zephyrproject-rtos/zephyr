...
int ade9153a_init_interrupt(const struct device *dev)
{
	...
	k_tid_t tid = k_thread_create(&data->thread, data->thread_stack,
				      CONFIG_ADE9153A_THREAD_STACK_SIZE, thread, data, NULL, NULL,
				      K_PRIO_COOP(CONFIG_ADE9153A_THREAD_PRIORITY), 0, K_NO_WAIT);

	if (IS_ENABLED(CONFIG_THREAD_NAME)) {
		k_thread_name_set(tid, "ade9153a_thread");
	}
