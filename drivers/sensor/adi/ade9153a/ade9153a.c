			       ...
			       size_t length)
{
	...
	/* Swap Endianness to Transmit MSB first as per ADE9153A requirements */
	access = (uint16_t)((access & 0x00FF) << 8) | ((access & 0xFF00) >> 8);
...
{
	uint8_t data_raw[4] = {(uint8_t)(data >> 24U), (uint8_t)(data >> 16U),
				...
				struct sensor_value *val)
{
	...
	case SENSOR_CHAN_AC_FREQUENCY: {
		...
		double_value = 100 * ((4000 * 65536) / (double)(data->period_reg + 1));
		break;
	}
...
{
	__ASSERT_NO_MSG(chan == SENSOR_CHAN_ALL);
	__ASSERT_NO_MSG(((int)attr) == ((int)SENSOR_ATTR_ADE9153A_REGISTER));
