...
class Q31(object):
    ...
    def isclose(self, f: float) -> bool:
        ...
        return math.isclose(self.decode(), f, rel_tol=(1 / 10**fraction_size), abs_tol=0.0)
...
def arg_parse_handler():
    ...
    service_subparsers = main_parser.add_subparsers(title="sub_command", dest="sub_command")
    service_subparsers.add_parser("test", help="test")
    decode_parser = service_subparsers.add_parser("decode", help="Decode q1.31 to float")
    decode_parser.add_argument("q1_31", help="the q1.31 value", type=int)
    decode_parser.add_argument("shift", help="the shift calculated on the encode phase", type=int)
    ...
    encode_parser.add_argument("value", help="the float value to be encoded", type=float)
