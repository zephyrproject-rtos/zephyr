...
#define DHT20_MEASUREMENT_TIME_MS      75
/** Wait during polling after measurement requested (in ms) */
#define DHT20_MEAS_POLL_STATUS_WAIT_MS 5
...
static int dht20_read_sample(struct dht20_data *data, uint8_t *rx_buf)
{
	/*
...
static int dht20_sample_fetch(const struct device *dev, enum sensor_channel chan)
{
	...
	uint8_t measure_frame[DHT20_MEAS_FRAME_LENGTH];
	...
	do {
		k_msleep(DHT20_MEAS_POLL_STATUS_WAIT_MS);
		rc = i2c_read_dt(&cfg->bus, measure_frame, sizeof(measure_frame));
		if (rc < 0) {
			LOG_ERR("Failed to read status.");
			return rc;
		}
	} while (DHT20_IS_STATUS_BUSY(measure_frame[DHT20_MEAS_STATUS_IDX]));

	rc = dht20_read_sample(data, measure_frame);
	if (rc < 0) {
