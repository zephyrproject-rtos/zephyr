 ...
 * 4.0  ADCMUX[4:0]     Selects which photodiode(s) are connected to the ADCs for measurement.
 */
...
#define ADCPOST_THRESH_EN_0 0x00  /* Do not use THRESHOLDs */
#define ADCPOST_THRESH_EN_1 0x01  /* Interrupt when the measurement is larger than the THRESHOLD0 Global Parameters */
#define ADCPOST_THRESH_EN_2 0x02  /* Interrupt when the measurement is larger than the THRESHOLD1 Global Parameters */
#define ADCPOST_THRESH_EN_3 0x03  /* Interrupt when the measurement is larger than the THRESHOLD2 Global Parameters */
...
struct si1153_data {
	...
	char __stack thread_stack[CONFIG_SI1153_THREAD_STACK_SIZE];
#endif
