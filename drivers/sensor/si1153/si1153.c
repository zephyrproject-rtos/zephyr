...
/* @file
 *
 * Driver for Silicon Labs Proximity/Ambient Light Sensor
 *
 */
#include <kernel.h>
#include <string.h>
...
struct k_thread si1153_sensor_thread;
...
static int si115x_param_set(struct si1153_data *data, u8_t address,
			    u8_t value)
{
	int retval;
	u8_t buffer[2];
...
static int si115x_gesture_algorithm(struct si115x_samples *samples)
{
	 ...
	 * look at history to see if a gesture occurred
	 */
		...
		if ((ps_entry_time[0] != 0) && (ps_entry_time[1] != 0) &&
			...
			if ((ps_exit_time[0] - ps_entry_time[0]) > GESTURE_TIME_DIFF ||
				...
				if (((ps_time[0] < ps_time[1]) &&  (diff_y1 > diff_x)) ||
					...
					/**
					 * An up gesture occurred if the bottom LED had its midpoint first
					 */
					ret = UP;
				} else if (((ps_time[0] < ps_time[1]) && (diff_y1 > diff_x)) ||
					   ((ps_time[0] > ps_time[2]) && (diff_y2 > diff_x))) {
					/**
					 * A down gesture occurred if the bottom LED had its midpoint last
					 */
					ret = DOWN;
					...
					/**
					 * A left gesture occurred if the left LED had its midpoint last
					 */
					ret = LEFT;
					   ...
					   (diff_x > ((diff_y1 + diff_y2) / 2))) {
					/**
					 * A right gesture occurred if the right LED had midpoint later than the right LED
					 */
					ret = RIGHT;
				}
			}
...
static void si115x_handler(struct si1153_data *data, struct si115x_samples *samples)
{
	...
	if (count >= 40) {
		SYS_LOG_ERR("SI1153: ERROR interrupt timeout.");
		return;
	}
...
static int si1153_sample_fetch(struct device *dev, enum sensor_channel chan)
{
	...
	SYS_LOG_DBG("Gesture %d %s %d %d %d %d\n",
...
static int si1153_init(struct device *dev)
{
	...
	k_thread_create(&si1153_sensor_thread, data->thread_stack, CONFIG_SI1153_THREAD_STACK_SIZE,
