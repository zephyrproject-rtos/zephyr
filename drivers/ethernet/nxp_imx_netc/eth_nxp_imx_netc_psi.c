...
static int eth_nxp_imx_netc_psi_pm_action(const struct device *dev, enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_TURN_ON:
		...
		if (!cfg->pseudo_mac && device_is_ready(cfg->phy_dev)) {
			(void)phy_monitor_start(cfg->phy_dev);

			/*
			 * Re-sync MAC link state via the PHY callback.
			 * Setting the callback triggers at least one immediate call with
			 * the current link state.
			 */
			if (data->iface != NULL) {
				phy_link_callback_set(cfg->phy_dev, &netc_eth_phylink_callback,
						      (void *)dev);
			}
		}
	...
	case PM_DEVICE_ACTION_TURN_OFF:
		if (!cfg->pseudo_mac && device_is_ready(cfg->phy_dev)) {
			(void)phy_monitor_stop(cfg->phy_dev);
		}
