...
{
	/*
	 * Serialize TX path: Zephyr's send() callback can be invoked from
	 * multiple threads concurrently. Without this lock, a second thread
	 * could overwrite waiting_net_pkt before the SPI thread consumes it,
	 * causing packet loss and memory leaks.
	 */
	k_mutex_lock(&tc6->tx_pkt_mutex, K_FOREVER);
	...
	k_mutex_unlock(&tc6->tx_pkt_mutex);
