...
static int virtnet_dev_init(const struct device *dev)
{
	...
	if (virtio_read_device_feature_bit(config->vdev, VIRTIO_NET_F_MAC)) {
		f_mac = true;
		if (virtio_write_driver_feature_bit(config->vdev, VIRTIO_NET_F_MAC, 1)) {
			f_mac = false;
			LOG_ERR("could not negotiate MAC address; setting a random one");
		} else {
			data->virtio_devcfg = virtio_get_device_specific_config(config->vdev);
			if (data->virtio_devcfg == NULL) {
				LOG_ERR("could not get config struct; setting random MAC address");
			}
	...
	if (!f_mac) {
		...
		/* turn it into an address in the "administratively assigned" range */
		/* (one that can be set randomly according to IEEE 802) */
...
static struct ethernet_api virtnet_api = {.iface_api.init = virtnet_if_init,
