...
struct w6300_runtime {
	 ...
	 * Without this, SEND and RECV commands can overwrite each other when
	 * k_yield() allows both threads to issue PIO QSPI transactions concurrently.
	 */
	struct k_mutex cmd_lock;
	/* Serializes the private PIO QSPI transaction engine. */
	struct k_mutex bus_lock;
	bool link_up;
	struct w6300_pio_qspi_data {
		void *pio;
		size_t pio_sm;
		uint32_t pio_offset;
...
int w6300_pio_qspi_write(const struct device *dev, uint32_t addr, const uint8_t *data,
			 size_t len);
