...
static void eth_iface_init(struct net_if *iface)
{
	...
	if (ipv4_addr_cmd_opt != NULL) {
		if (net_addr_pton(AF_INET, ipv4_addr_cmd_opt, &addr)) {
	...
	if (ipv4_nm_cmd_opt != NULL && ipv4_addr_cmd_opt != NULL) {
		if (net_addr_pton(AF_INET, ipv4_nm_cmd_opt, &netmask)) {
...
set_gw:
	if (ipv4_gw_cmd_opt != NULL) {
		if (net_addr_pton(AF_INET, ipv4_gw_cmd_opt, &addr)) {
