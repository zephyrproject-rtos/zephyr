...
static void nxp_s32_eth_iface_init(struct net_if *iface)
{
	...
	LOG_INF("SI%d MAC: %02x:%02x:%02x:%02x:%02x:%02x", cfg->si_idx, ctx->mac_addr[0],
...
static const struct ethernet_api nxp_s32_eth_api = {.iface_api.init = nxp_s32_eth_iface_init,
						    .get_capabilities =
							    nxp_s32_eth_get_capabilities,
...
#define NETC_VSI_INSTANCE_DEFINE(n)                                                               \
	...
	static const struct nxp_s32_eth_config nxp_s32_eth##n##_cfg = {                           \
		...
		.generate_mac = nxp_s32_eth##n##_generate_mac,                                    \
                                                                                                  ...
                                                                                                  \
	ETH_NET_DEVICE_DT_INST_DEFINE(n, nxp_s32_eth_initialize_common, NULL,                     \
				      &nxp_s32_eth##n##_data, &nxp_s32_eth##n##_cfg,              \
				      CONFIG_ETH_NXP_S32_VSI_INIT_PRIORITY, &nxp_s32_eth_api,     \
				      NET_ETH_MTU);
