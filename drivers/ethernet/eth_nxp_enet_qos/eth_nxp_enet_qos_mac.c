 ...
 */
static struct net_pkt *enet_qos_tx_claim(struct nxp_enet_qos_tx_data *tx)
{
...
static void enet_qos_tx_abort(const struct device *dev)
{
	...
	struct net_pkt *pkt = enet_qos_tx_claim(&data->tx);
...
static void eth_nxp_enet_qos_tx_watchdog(struct k_work *work)
{
	...
	enet_qos_tx_abort(data->iface->if_dev->dev);
}
		...
		struct phy_link_state *state, void *eth_dev)
{
	...
	/* Link speed lives in the MAC configuration register, which may only
	 * be written with the transmitter and receiver disabled. Program it
	 * before the carrier is announced, so that the write cannot land on a
	 * frame the stack has already handed to the DMA.
	 */
	mac_configuration =
...
static void tx_dma_done(const struct device *dev)
{
	...
	struct net_pkt *pkt = enet_qos_tx_claim(tx_data);
