				      ...
				      struct net_buf **frag2)
{
	...
	*(rx_frags + (desc_id * RX_FRAGS_PER_DESC)) = (mem_addr_t)new_frag;
	 ...
	 * buffer.
	 */
	rx_desc->rdes0 = POINTER_TO_UINT(new_frag->data);
	rx_desc->rdes1 = POINTER_TO_UINT(new_frag->data) >> XGMAC_REG_SIZE_BITS;
	/**
	...
	*(rx_frags + (desc_id * RX_FRAGS_PER_DESC) + 1u) = (mem_addr_t)new_frag;
	 ...
	 * data buffer.
	 */
	rx_desc->rdes2 = POINTER_TO_UINT(new_frag->data);
	/**
