...
BUILD_ASSERT(CONFIG_NET_BUF_DATA_SIZE >= CPSW_MIN_PACKET_SIZE,
	     "CONFIG_NET_BUF_DATA_SIZE must be >= CPSW_MIN_PACKET_SIZE (64) "
	     "for TX padding");
BUILD_ASSERT(CPSW_RX_DESC_NUM >= CPSW_RX_MAX_SG_FRAGS,
	     "CPSW_RX_DESC_NUM must be >= CPSW_RX_MAX_SG_FRAGS "
	     " for RX scatter-gather");
...
/**
 * @brief Process a single RX packet completion (scatter-gather)
 *
 * @param priv Driver private data
 */
static inline void cpsw_rx_process_one_pkt(struct cpsw_priv *priv)
{
	...
	struct net_buf *frag;
	uint64_t next;
	uint32_t src_port, pkt_len, frag_len, idx;
	uint8_t n_frags = 0;
	...
	do {
		rx_desc = &dma->rx_descs[idx];
		if (!AM62L_PKTDMA_IS_COHERENT(config->rx_asel) && n_frags > 0) {
			sys_cache_data_invd_range(rx_desc, CPPI5_DESC_SIZE);
		}

		next     = rx_desc->next_desc;
		frag_len = rx_desc->buf_len;
		frag     = dma->rx_frags[idx];
		dma->rx_frags[idx] = NULL;

		if (!AM62L_PKTDMA_IS_COHERENT(config->rx_asel)) {
			sys_cache_data_invd_range(frag->data, frag_len);
		}
		frag->len = frag_len;
		net_pkt_frag_add(pkt, frag);
		cpsw_rx_refill_desc(priv, rx_desc, idx, NULL);

		n_frags++;
		idx = (idx + 1U) % CPSW_RX_DESC_NUM;
	} while (next != 0ULL && n_frags < CPSW_RX_MAX_SG_FRAGS);

	dma->rx_drain_idx += n_frags;

	LOG_DBG("RX: %u bytes, %u frag(s) (port %u)", pkt_len, n_frags, src_port);
...
drop:
	do {
		...
		frag = dma->rx_frags[idx];
		dma->rx_frags[idx] = NULL;
		cpsw_rx_refill_desc(priv, rx_desc, idx, frag);
...
static void cpsw_tx_fill_chain(const struct device *dev, uint32_t head_idx,
			       ...
			       uint8_t port_num, uint8_t n_descs)
{
	...
	NET_PKT_FRAG_FOR_EACH(pkt, frag) {
		...
		bool is_last = (i == n_descs - 1U);
		uint16_t pad_len = (is_last && pkt_len < CPSW_MIN_PACKET_SIZE) ?
				   (CPSW_MIN_PACKET_SIZE - pkt_len) : 0U;
		uint32_t buf_len = frag->len + pad_len;
		...
		if (pad_len > 0U) {
			memset(frag->data + frag->len, 0, pad_len);
		}
		...
		if (i == 0) {
			...
			desc->pkt_info0 =
				...
				(MAX(pkt_len, CPSW_MIN_PACKET_SIZE)
		...
		if (!AM62L_PKTDMA_IS_COHERENT(config->tx_asel)) {
			sys_cache_data_flush_range(frag->data, buf_len);
...
static int cpsw_tx(const struct device *dev, struct net_pkt *pkt)
{
	...
	cpsw_tx_fill_chain(dev, head_idx, pkt, pkt_len, port_num, n_descs);
