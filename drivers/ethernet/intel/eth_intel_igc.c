...
{
	uint32_t val = igc_read(base, offset);
 ...
 */
static union dma_tx_desc *eth_intel_igc_release_tx_desc(const struct device *dev,
							uint8_t queue)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
 ...
 * and ensures that the descriptor is available for DMA operation.
 */
static union dma_rx_desc *eth_intel_igc_get_rx_desc(const struct device *dev,
						    uint8_t queue)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
 ...
 * and clears the descriptor.
 */
static union dma_rx_desc *eth_intel_igc_release_rx_desc(const struct device *dev,
							uint8_t queue)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
...
/**
 * @brief Interrupt Service Routine for handling Miscellaneous interrupts.
 */
static void eth_intel_igc_misc_isr(void *arg)
{
	...
	uint32_t intr_cause, config = 0;

	intr_cause = sys_read32(data->base + INTEL_IGC_ICR);
	if (intr_cause & INTEL_IGC_LSC) {
		eth_intel_igc_misc_phy_link_isr(cfg->phy);
		config |= INTEL_IGC_LSC;
	}

	/* Re-enable interrupt */
	igc_modify(data->base, INTEL_IGC_IMS, config, true);
...
static int eth_intel_igc_connect_misc_msix_vector(pcie_bdf_t bdf, const struct device *dev)
{
	struct eth_intel_igc_mac_data *data = dev->data;
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	uint8_t msix = cfg->num_queues;
	struct eth_intel_igc_intr_info *info = &data->intr_info[msix];

	info->id  = msix;
	info->mac = dev;
	if (!pcie_msi_vector_connect(bdf, &data->msi_vec[msix],
				     (void *)eth_intel_igc_misc_isr,
				     info, 0)) {
		LOG_ERR("Failed to connect MISC interrupt handler");
		return -EIO;
	}
...
static int eth_intel_igc_pcie_msix_setup(const struct device *dev)
{
	...
	ret = eth_intel_igc_connect_misc_msix_vector(bdf, dev);
	if (ret < 0) {
		return ret;
	}
...
static void eth_intel_igc_map_intr_to_vector(const struct device *dev)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
	uint32_t config, reg_idx, msix;

	/* Set MSI-X capability */
	config = INTEL_IGC_GPIE_NSICR | INTEL_IGC_GPIE_MSIX_MODE |
		 INTEL_IGC_GPIE_EIAME | INTEL_IGC_GPIE_PBA;
	...
	for (msix = 0; msix < cfg->num_queues; msix++) {
		reg_idx = msix >> 1;
		config = igc_read(data->base, (INTEL_IGC_IVAR_BASE_ADDR + (reg_idx << 2)));
		...
		if (msix % 2) {
			...
			config |= (msix | INTEL_IGC_IVAR_INT_VALID_BIT) << 0;
			config &= INTEL_IGC_IVAR_MSI_CLEAR_TX0_TX2;
			config |= (msix | INTEL_IGC_IVAR_INT_VALID_BIT) << 8;
		}

		igc_write(data->base + (INTEL_IGC_IVAR_BASE_ADDR + (reg_idx << 2)), config);
	}

	/* Configure IVAR_MISC for Miscellaneous Interrupt */
	config = (msix | INTEL_IGC_IVAR_INT_VALID_BIT) << 8;
	igc_modify(data->base, INTEL_IGC_IVAR_MISC, config, true);
}

/**
 * @brief Helper function to write MAC address to RAL and RAH registers.
 */
static void eth_intel_igc_set_mac_addr(mm_reg_t base, int index,
				       const uint8_t *mac_addr, uint32_t rah_config)
{
	uint32_t mac_addr_hi = (mac_addr[5] <<  8) | (mac_addr[4] <<  0) | rah_config;
	uint32_t mac_addr_lo = (mac_addr[3] << 24) | (mac_addr[2] << 16) |
			       (mac_addr[1] <<  8) | (mac_addr[0] <<  0);

	igc_write(base + INTEL_IGC_RAH(index), mac_addr_hi);
	igc_reg_refresh(base);
	igc_write(base + INTEL_IGC_RAL(index), mac_addr_lo);
	igc_reg_refresh(base);
}
...
static void eth_intel_igc_phylink_cb(const struct device *phy,
...
static void eth_intel_igc_intr_enable(const struct device *dev)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
	uint32_t msix, misc_config, config = 0;

	misc_config = INTEL_IGC_LSC;
	/* Clear pending interrupt */
	igc_read(data->base, INTEL_IGC_ICR);
	igc_write(data->base + INTEL_IGC_ICS, misc_config);

	/* Prepare bit mask of Queue interrupt */
	for (msix = 0; msix < cfg->num_queues; msix++) {
		config |= BIT(msix);
	}
	/* Append Miscellaneous interrupt bit */
	config |= BIT(msix);
	...
	igc_modify(data->base, INTEL_IGC_IMS, misc_config, true);
...
static void eth_intel_igc_iface_init(struct net_if *iface)
{
	...
	/* Set MAC address */
	if (net_if_set_link_addr(data->iface, data->mac_addr,
				 sizeof(data->mac_addr), NET_LINK_ETHERNET) < 0) {
				    ...
				    const struct ethernet_config *eth_cfg)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
	const struct device *phy = cfg->phy;
	enum phy_link_speed speed = 0;
	int ret = 0;

	switch (type) {
	case ETHERNET_CONFIG_TYPE_MAC_ADDRESS:
		memcpy(data->mac_addr, eth_cfg->mac_address.addr,
	...
	case ETHERNET_CONFIG_TYPE_LINK:
		...
		ret = phy_configure_link(phy, speed);
		if (ret < 0) {
...
static struct net_stats_eth *eth_intel_igc_get_stats(const struct device *dev)
{
	...
	for (uint8_t queue = 0; queue < cfg->num_queues; queue++) {
		stats->tx_dropped += igc_read(data->base, INTEL_IGC_TQDPC(queue));
	}
				 ...
				 struct net_buf *frag, uint8_t queue)
{
	...
	*(data->tx->pkt  + queue * cfg->num_tx_desc + idx) = frag->frags ? 0 : pkt;
	...
	if (!frag->frags) {
		...
		igc_write((data->base + INTEL_IGC_TDT(queue)), data->tx->ring_wr_ptr[queue]);
	}
				      ...
				      uint32_t idx, union dma_rx_desc *desc)
{
	...
	desc->read.pkt_buf_addr = (uint64_t)sys_cpu_to_le64((uint64_t)(data->rx->buf +
...
static void eth_intel_igc_rx_data(struct eth_intel_igc_mac_data *data, uint8_t queue)
{
	...
	for (count = 0; count < completed_count; count++) {
		...
		rx_buf = data->rx->buf + ((queue * cfg->num_rx_desc + idx) *
					  ETH_MAX_FRAME_SZ);
...
static int eth_intel_igc_rx_desc_prepare(const struct device *dev)
{
	...
	for (uint8_t queue = 0; queue < cfg->num_queues; queue++) {
		...
		for (uint32_t desc_idx = 0; desc_idx < cfg->num_rx_desc; desc_idx++) {
			...
			buf = data->rx->buf + ((queue * cfg->num_rx_desc + desc_idx) *
						ETH_MAX_FRAME_SZ);
...
static void eth_intel_igc_get_nvm_mac_addr(mm_reg_t base, uint8_t *mac_addr)
{
	uint32_t mac_addr_hi, mac_addr_lo;

	/* Reading RAL/RAH will require at least 2us upon MAC reset */
	WAIT_FOR(sys_read32(base + INTEL_IGC_RAL(0)),
		 INTEL_IGC_READ_NVM_TIMEOUT, k_usleep(10));
	mac_addr_lo = igc_read(base, INTEL_IGC_RAL(0));
	mac_addr_hi = igc_read(base, INTEL_IGC_RAH(0));

	mac_addr[0] = (uint8_t)((mac_addr_lo >> 0)  & 0xFF);
	mac_addr[1] = (uint8_t)((mac_addr_lo >> 8)  & 0xFF);
	mac_addr[2] = (uint8_t)((mac_addr_lo >> 16) & 0xFF);
	mac_addr[3] = (uint8_t)((mac_addr_lo >> 24) & 0xFF);

	mac_addr[4] = (uint8_t)((mac_addr_hi >> 0) & 0xFF);
...
static void eth_intel_igc_get_mac_addr(const struct device *dev)
{
	...
	eth_intel_igc_get_nvm_mac_addr(data->base, data->mac_addr);
	if (eth_intel_igc_is_valid_mac_addr(data->mac_addr)) {
		LOG_INF("Assign MAC address from EEPROM");
		return;
	}

	LOG_INF("Please assign a valid MAC address using net shell");
}

static void eth_intel_igc_rx_addrs_init(const struct device *dev)
{
	struct eth_intel_igc_mac_data *data = dev->data;
	uint8_t reset_addr[INTEL_IGC_ALEN] = {0};
...
static int eth_intel_igc_disable_pcie_master(mm_reg_t base)
{
	...
	if (!WAIT_FOR(!(igc_read(base, INTEL_IGC_STATUS) &
...
static void eth_intel_igc_init_speed(const struct device *dev)
{
	struct eth_intel_igc_mac_data *data = dev->data;
...
static int eth_intel_igc_init_mac_hw(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("GIO master request failed");
		return ret;
	}
...
static int eth_intel_igc_init(const struct device *dev)
{
	...
	return eth_intel_igc_rx_dma_init(dev);
}
...
#define INTEL_IGC_MAC_ASSIGN_ADDR(n)                                                             \
	static void assign_mac_address_##n(uint8_t mac_addr[INTEL_IGC_ALEN])                     \
	{                                                                                        \
		const uint8_t mac_addr_dt[INTEL_IGC_ALEN] = DT_INST_PROP(n, local_mac_address);  \
		memcpy(mac_addr, mac_addr_dt, sizeof(mac_addr_dt));                              \
	}
...
#define INTEL_IGC_MAC_CONFIG_IRQ(n)                                                              \
	...
	{                                                                                        \
		...
		eth_intel_igc_init_queue_work_##n(dev);                                          \
		eth_intel_igc_init_speed(dev);                                                   \
	}
...
/**
 * @brief Initialize TX and RX work queue item for each queue.
 */
#define INTEL_IGC_INIT_QUEUE_WORK(n)                                                             \
	static void eth_intel_igc_init_queue_work_##n(const struct device *dev)                  \
	{                                                                                        \
		struct eth_intel_igc_mac_data *data = dev->data;                                 \
...
/**
 * @brief Initializes the runtime data structures of each driver instance.
 */
#define INTEL_IGC_MAC_DATA(n)                                                                    \
	static struct eth_intel_igc_mac_tx tx_##n = {                                            \
		.sem           = tx_ring_lock_##n,                                               \
	...
	static struct eth_intel_igc_mac_rx rx_##n = {                                            \
		.sem           = rx_ring_lock_##n,                                               \
...
#define INTEL_IGC_MAC_INIT(n)                                                                    \
	...
	ETH_NET_DEVICE_DT_INST_DEFINE(n, eth_intel_igc_init, NULL, &eth_data_##n,                \
