 ...
 */
static void igc_modify(mm_reg_t reg_addr, uint32_t offset, uint32_t config, bool set)
{
 ...
 */
static void igc_reg_refresh(mm_reg_t reg_addr)
{
	igc_read(reg_addr, INTEL_IGC_STATUS);
}
...
static int eth_intel_igc_pcie_msi_vector_connect(pcie_bdf_t bdf, const struct device *dev)
{
	const struct eth_intel_igc_mac_cfg *cfg = dev->config;
	struct eth_intel_igc_mac_data *data = dev->data;
	uint8_t msix;
	...
	/* Connect the last MSI-X vector to the Misc interrupt handler */
	struct eth_intel_igc_intr_info *info = &data->intr_info[msix];
...
static int eth_intel_igc_pcie_msix_setup(const struct device *dev)
{
	...
	ret = eth_intel_igc_pcie_msi_vector_connect(bdf, dev);
	if (ret < 0) {
		return ret;
	}
				    ...
				    const struct ethernet_config *eth_cfg)
{
	...
	case ETHERNET_CONFIG_TYPE_MAC_ADDRESS:
		...
		net_if_set_link_addr(data->iface, data->mac_addr.octets,
				     sizeof(data->mac_addr.octets), NET_LINK_ETHERNET);
...
#define INTEL_IGC_MAC_CONFIG(n) \
	...
	static const struct eth_intel_igc_mac_cfg eth_cfg_##n = { \
		...
		.assign_mac_addr = assign_mac_address_##n, \
