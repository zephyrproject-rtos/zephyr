...
static int eth_intel_igc_connect_misc_msix_vector(pcie_bdf_t bdf, const struct device *dev)
{
	...
	info->id = msix;
	info->mac = dev;
	if (!pcie_msi_vector_connect(bdf, &data->msi_vec[msix], (void *)eth_intel_igc_misc_isr,
				     info, 0)) {
				       ...
				       uint32_t rah_config)
{
	...
	uint32_t mac_addr_lo =
		(mac_addr[3] << 24) | (mac_addr[2] << 16) | (mac_addr[1] << 8) | (mac_addr[0] << 0);
...
static bool eth_intel_igc_is_valid_mac_addr(uint8_t *mac_addr)
{
	if (UNALIGNED_GET((uint32_t *)mac_addr) == INTEL_IGC_DEF_MAC_ADDR) {
		LOG_WRN("Wrong MAC address addr=%02X:%02X:%02X:%02X:%02X:%02X", mac_addr[0],
...
static void eth_intel_igc_get_nvm_mac_addr(mm_reg_t base, uint8_t *mac_addr)
{
	...
	WAIT_FOR(sys_read32(base + INTEL_IGC_RAL(0)), INTEL_IGC_READ_NVM_TIMEOUT, k_usleep(10));
...
static int eth_intel_igc_disable_pcie_master(mm_reg_t base)
{
	...
	if (!WAIT_FOR(!(igc_read(base, INTEL_IGC_STATUS) & INTEL_IGC_STATUS_GIO_MASTER_ENABLE),
		      timeout, k_msleep(1))) {
...
#define INTEL_IGC_MAC_CONFIG_IRQ(n)                                                                \
	...
	{                                                                                          \
		...
		eth_intel_igc_init_speed(dev->data);                                               \
	}
