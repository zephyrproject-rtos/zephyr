...
static int adin1140_default_filter_config(const struct device *dev)
{
	...
	/* Enable reception of unicast frames */
	return adin1140_filter_unicast(dev);
}
 ...
 * Supports promiscuous mode, MAC address and destination MAC address filtering.
 */
...
/* Discard the RX frame currently under assembly. */
static void adin1140_rx_frame_reset(struct adin1140_data *ctx)
{
	if (ctx->rx_pkt != NULL) {
...
/**
 * Append a chunk's payload to the frame under assembly. Chunk payloads
 * (<= tc6->cps) are packed into net_buf fragments, filling the current
 * fragment's tailroom before a new one is allocated and added.
 */
static int adin1140_rx_frame_append(struct adin1140_data *ctx, const uint8_t *data, uint16_t len)
{
	while (len > 0) {
		uint16_t copy_len;

		if (ctx->rx_frag == NULL || net_buf_tailroom(ctx->rx_frag) == 0) {
			ctx->rx_frag = net_pkt_get_frag(ctx->rx_pkt, len,
							K_MSEC(CONFIG_ETH_ADIN1140_TIMEOUT));
			if (ctx->rx_frag == NULL) {
				return -ENOMEM;
			}
			net_pkt_frag_add(ctx->rx_pkt, ctx->rx_frag);
		}

		copy_len = MIN(len, net_buf_tailroom(ctx->rx_frag));
		net_buf_add_mem(ctx->rx_frag, data, copy_len);
		data += copy_len;
		len -= copy_len;
	}
 ...
 * rx_pkt is allocated empty at start-of-frame; each chunk's payload is
 * copied into net_buf fragments appended to it, so no intermediate copy
 * buffer is needed.
 */
static int adin1140_process_rx_chunks(const struct device *dev, uint8_t num_chunks)
{
	...
	for (chunk = 0, rx_idx = 0; chunk < num_chunks; chunk++) {
		...
		if (net_pkt_get_len(ctx->rx_pkt) + len > ADIN1140_MAX_RX_FRAME_SIZE) {
		...
		if (FIELD_GET(OA_DATA_FTR_EV, ftr)) {
			rc = net_pkt_remove_tail(ctx->rx_pkt, sizeof(uint32_t));
			if (rc < 0) {
				LOG_ERR("Failed to trim FCS [%d]", rc);
				adin1140_rx_frame_reset(ctx);
				return rc;
			}

			rc = net_recv_data(ctx->iface, ctx->rx_pkt);
			if (rc < 0) {
...
static int adin1140_send_frame(const struct device *dev, struct net_pkt *pkt)
{
	...
	for (int retry = 0; total_chunks > tc6->txc && retry < ADIN1140_TXC_POLL_MAX_RETRIES;
	     retry++) {
		...
		*(uint32_t *)ctx->oa_tx_buf = sys_cpu_to_be32(poll_hdr);
		...
		if (total_chunks > tc6->txc) {
			k_sleep(K_USEC(ADIN1140_TXC_POLL_INTERVAL_US));
		}
	}
	...
	for (chunk = 1, frame_len = 0; chunk <= total_chunks; chunk++) {
		...
		*(uint32_t *)&ctx->oa_tx_buf[frame_len] = sys_cpu_to_be32(hdr);
		...
		if (copy_len < tc6->cps) {
			/* Clear the trailing bytes of the last chunk */
			memset(&ctx->oa_tx_buf[frame_len + copy_len], 0, tc6->cps - copy_len);
		}
...
/**
 * Send an ethernet frame with mutex protection.
 */
...
static int adin1140_init(const struct device *dev)
{
	...
	if (cfg->wake.port != NULL && !gpio_is_ready_dt(&cfg->wake)) {
...
#define ADIN1140_DEFINE(inst)                                                                      \
	...
	static uint8_t __aligned(4) adin1140_oa_tx_buf_##inst[ADIN1140_OA_SPI_BUF_SIZE];           \
	static uint8_t __aligned(4) adin1140_oa_rx_buf_##inst[ADIN1140_OA_SPI_BUF_SIZE];           \
	static struct adin1140_data adin1140_data_##inst = {                                       \
		...
		.oa_tx_buf = adin1140_oa_tx_buf_##inst,                                            \
		.oa_rx_buf = adin1140_oa_rx_buf_##inst,                                            \
	};                                                                                         \
