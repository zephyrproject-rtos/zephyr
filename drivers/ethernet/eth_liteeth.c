...
{
	const struct eth_liteeth_config *config = dev->fixed->config_info;
...
static int eth_tx(struct device *dev, struct net_pkt *pkt)
{
	int key;
	uint16_t len;
	struct eth_liteeth_dev_data *context = dev->fixed->driver_data;
...
{
	struct net_pkt *pkt;
	struct eth_liteeth_dev_data *context = port->fixed->driver_data;
