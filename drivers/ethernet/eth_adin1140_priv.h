 ...
 * Broadcast and the interface unicast address use reserved slots;
 * remaining slots hold multicast groups registered dynamically via
 * ETHERNET_CONFIG_TYPE_FILTER.
 */
#define ADIN1140_MAC_FILT_TABLE_BROADCAST_SLOT 2U
#define ADIN1140_MAC_FILT_TABLE_UNICAST_SLOT   4U
#define ADIN1140_MAC_FILT_TABLE_BASE_SLOT      6U
...
struct adin1140_data {
	...
	bool tx_cut_through_en;
	bool rx_cut_through_en;

	/* SPI transfer buffers */
	uint8_t *oa_tx_buf;
	uint8_t *oa_rx_buf;

	/* RX frame under assembly;
	 * Allocated empty at start-of-frame; data fragments are appended
	 * incrementally as chunks arrive, so no separate copy buffer is needed.
	 * rx_frag is the fragment currently being filled.
	 */
