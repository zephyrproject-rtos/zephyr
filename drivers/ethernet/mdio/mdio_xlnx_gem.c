				  ...
				  uint16_t *data_out)
{
	 ...
	 */
	if (data_out != NULL) {
		reg_val = sys_read32(reg_base + ETH_XLNX_GEM_PHY_MAINTENANCE_OFFSET);
...
static int xlnx_gem_mdio_initialize(const struct device *dev)
{
	...
	reg_val = sys_read32(reg_base + ETH_XLNX_GEM_NWCFG_OFFSET);
	...
	sys_write32(reg_val, reg_base + ETH_XLNX_GEM_NWCFG_OFFSET);

	/* Enable the MDIO interface */
	reg_val = sys_read32(reg_base + ETH_XLNX_GEM_NWCTRL_OFFSET);
	reg_val |= ETH_XLNX_GEM_NWCTRL_MDEN_BIT;
	sys_write32(reg_val, reg_base + ETH_XLNX_GEM_NWCTRL_OFFSET);
