...
static enum ethernet_hw_caps lan78xx_get_capabilities(const struct device *dev __unused,
						      struct net_if *iface __unused)
{
	return ETHERNET_LINK_10BASE | ETHERNET_LINK_100BASE | ETHERNET_LINK_1000BASE;
}

static int lan78xx_get_config(const struct device *dev __unused,
			      struct net_if *iface __unused,
...
static int lan78xx_set_config(const struct device *dev, struct net_if *iface __unused,
			      enum ethernet_config_type type,
...
static const struct device *lan78xx_get_phy(const struct device *dev,
					    struct net_if *iface __unused)
{
	const struct lan78xx_config *config = dev->config;
...
static int lan78xx_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	if (!ctx->connected || !ctx->udev || !ctx->iface ||
	    !net_if_is_carrier_ok(ctx->iface) ||
...
static void lan78xx_on_link_change(struct lan78xx_ctx *ctx, bool up)
{
	...
	net_eth_carrier_set(ctx->iface, up);
}
...
static void lan78xx_poll_link_state(struct lan78xx_ctx *ctx)
{
	...
	if (ret == 0) {
		if (up != net_if_is_carrier_ok(ctx->iface)) {
				    ...
				    uint8_t slot_idx)
{
	...
	if (state == LAN78XX_RX_SLOT_DONE) {
		if (!atomic_get(&ctx->rx_stopping) && ctx->udev && slot->xfer->err == 0 &&
		    slot->xfer->buf && slot->xfer->buf->len > 0 && ctx->connected && ctx->iface &&
		    net_if_is_carrier_ok(ctx->iface)) {
