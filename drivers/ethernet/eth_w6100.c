 ...
 * Sayed Naser Moravej <seyednasermoravej@gmail.com>
			 ...
			 int len)
{
	...
	const uint16_t mem_size = W6100_RX_MEM_SIZE;
			  ...
			  int len)
{
	...
	const uint32_t mem_size = W6100_TX_MEM_SIZE;
...
static int w6100_tx(const struct device *dev, struct net_pkt *pkt)
{
	...
	// LOG_DBG("S0_TX_WR, %x, is: %x", W6100_S0_TX_WR, off);
	offset = sys_get_be16(off);

	if (net_pkt_read(pkt, ctx->buf, len)) {
		return -EIO;
	}
	// LOG_DBG("Data len is: %x, in w6100 tx", len);
	...
	// LOG_DBG("S0_CR_SEND, %x, is: %x", S0_CR_SEND, off);
...
static void w6100_rx(const struct device *dev)
{
	...
	// LOG_DBG("RX RSR: %x, is: %x", W6100_S0_RX_RSR, tmp);
	if (rx_buf_len == 0) {
		return;
	}
	...
	// LOG_DBG("RX RD is: %x, is: %x", W6100_S0_RX_RD, tmp);
...
static struct ethernet_api w6100_api_funcs = {
...
{
	LOG_DBG("enterd soft reset.");
	int ret;
	uint8_t tmp = SYCR0_RST;
	ret = w6100_spi_write(dev, W6100_SYCR0, &tmp, 1);
	k_msleep(5);
	tmp = 0x80;
	ret = w6100_spi_write(dev, W6100_SYCR0, &tmp, 1);
	k_msleep(5);
	uint8_t isLock;
	w6100_spi_read(dev, W6100_SYSR, &isLock, 1);
	k_msleep(5);
	uint8_t unLock = CHPLCKR_UNLOCK;
	ret = w6100_spi_write(dev, W6100_CHPLCKR, &unLock, 1);
	if (ret < 0) {
		return ret;
	}
	uint8_t mask = 0;
	...
	k_msleep(5);
	uint8_t netUnblock = NET_UNLOCK;
	w6100_spi_write(dev, W6100_NETLCKR, &netUnblock, 1);
	k_msleep(5);
	temp = w6100_spi_write(dev, W6100_Sn_MR, &tmp, 1);
	k_msleep(5);

	/* disable interrupt */
	LOG_DBG("exit soft reset.");
...
static void w6100_set_macaddr(const struct device *dev)
{
	...
	w6100_spi_write(dev, W6100_SHAR, ctx->mac_addr, sizeof(ctx->mac_addr));
	uint8_t mac[6] = {0};
	w6100_spi_read(dev, W6100_SHAR, mac, 6 );
}
...
static int w6100_init(const struct device *dev)
{
	...
	w6100_spi_write(dev, W6100_NETLCKR, 0x00, 1);
	k_msleep(5);
