...
static void w6100_update_link_status(const struct device *dev)
{
	...
	if (w6100_spi_read(dev, W6100_PHYCFGR, &phycfgr, 1) < 0) {
		return;
	}
	...
	while (true) {
		...
		if (res == 0) {
			...
			while (gpio_pin_get_dt(&(config->interrupt))) {
				...
				if (ir) {
					/* Clear interrupt */
					w6100_spi_write(dev, W6100_S0_IR, &ir, 1);
...
static int w6100_soft_reset(const struct device *dev)
{
	...
	ret = w6100_spi_write(dev, W6100_MR, &tmp, 1);
	if (ret < 0) {
		return ret;
	}
	...
	k_msleep(5);
	tmp = MR_PB;
	w6100_spi_write(dev, W6100_MR, &tmp, 1);
