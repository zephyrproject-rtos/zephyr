...
static void dwmac_receive(const struct device *dev)
{
	...
	for (d_idx = p->rx_desc_tail; d_idx != p->rx_desc_head;
		...
		/*
		 * A non-last descriptor has exhausted its posted buffer. Size it
		 * from software because intermediate length reporting varies between
		 * integrations; GigaDevice hardware, for example, reports zero.
		 */
		if ((des0 & RDES0_LS) != 0U) {
			bytes_so_far = RX_LEN_FROM_RDES0(des0);
			if (bytes_so_far < p->rx_bytes || bytes_so_far - p->rx_bytes > frag->size) {
				eth_stats_update_errors_rx(p->iface);
				net_pkt_unref(p->rx_pkt);
				p->rx_pkt = NULL;
				net_pkt_frag_unref(frag);
