...
#define RK3588_PHP_IO_CLKSEL_GMAC0 BIT(4)
#define RK3588_PHP_IO_CLKSEL_GMAC1 BIT(9)
#define RK3588_PHP_GMAC0_CLK_GATE  BIT(1)
#define RK3588_PHP_GMAC1_CLK_GATE  BIT(6)
...
#define RK3588_GMAC_TXCLK_DLY_CFG(val)    (((val) & 0xffU) | (0xffU << 16))
#define RK3588_GMAC_RXCLK_DLY_CFG(val)    ((((val) & 0xffU) << 8) | (0xffU << 24))
#define RK3588_GMAC_TXCLK_RXCLK_DLY_CFG(tx, rx)                                                    \
	(RK3588_GMAC_TXCLK_DLY_CFG(tx) | RK3588_GMAC_RXCLK_DLY_CFG(rx))
 ...
 */
static void rk3588_grf_write(const struct device *grf, uint32_t offset, uint32_t val)
{
	syscon_write_reg(grf, offset, val);
}

static void rk3588_clrsetreg(const struct device *grf, uint32_t offset, uint32_t clr, uint32_t set)
{
	uint32_t reg;
...
static void rk3588_gmac_configure_clock_selection(const struct device *php_grf, unsigned int id,
						  bool clock_input)
{
	uint32_t cru_mask = (id == 0U) ? RK3588_PHP_IO_CLKSEL_GMAC0 : RK3588_PHP_IO_CLKSEL_GMAC1;
	uint32_t gate_mask = (id == 0U) ? RK3588_PHP_GMAC0_CLK_GATE : RK3588_PHP_GMAC1_CLK_GATE;
	uint32_t clr = gate_mask;
	uint32_t set = 0;
	...
	rk3588_clrsetreg(php_grf, RK3588_PHP_CLK_CON1, clr, set);
}
...
static void rk3588_gmac_grf_prepare(const struct rk3588_gmac_config *cfg)
{
	...
	rk3588_grf_write(cfg->php_grf, RK3588_PHP_GMAC_CON0, RK3588_GMAC_PHY_INTF_SEL_RGMII(id));
	rk3588_clrsetreg(cfg->php_grf, RK3588_PHP_CLK_CON1, BIT(id * 5U), 0);

	delay_con = (id == 0U) ? RK3588_GRF_GMAC_CON8 : RK3588_GRF_GMAC_CON9;
	rk3588_grf_write(cfg->sys_grf, delay_con,
			 RK3588_GMAC_TXCLK_RXCLK_DLY_CFG(cfg->tx_delay, cfg->rx_delay));
	...
	rk3588_grf_write(cfg->sys_grf, RK3588_GRF_GMAC_CON7, con7);
}
					    ...
					    enum phy_link_speed speed)
{
	...
	rk3588_clrsetreg(php_grf, RK3588_PHP_CLK_CON1, mask, clk_div << shift);
}
...
int dwmac_bus_init(const struct device *dev)
{
	...
	rk3588_gmac_configure_clock_selection(cfg->php_grf, cfg->id, cfg->clock_input);
	...
	/*
	 * The CRU reset and PHP/SYS GRF writes switch the RGMII clock path.
	 * Give the PHY and the transmit clock divider time to settle before
	 * dwmac_probe issues the MAC software reset.
	 */
	k_msleep(1);

	LOG_DBG("GMAC%u bus init done", cfg->id);

	return 0;
}

#define RK3588_GMAC0_REG_ADDR    0xfe1b0000
#define RK3588_GMAC1_REG_ADDR    0xfe1c0000
#define RK3588_GMAC_ID(n)        ((DT_INST_REG_ADDR(n) == RK3588_GMAC0_REG_ADDR) ? 0 : 1)
#define RK3588_GMAC_HAS_PTP(n)   DT_INST_NODE_HAS_PROP(n, ptp_clock)
#define RK3588_GMAC_PTP_CLOCK(n) DEVICE_DT_GET(DT_INST_CHILD(n, ptp_clock))
#define RK3588_GMAC_PTP_CLK(n)                                                                     \
	((clock_control_subsys_t)(uintptr_t)DT_INST_CLOCKS_CELL_BY_NAME(n, ptp_ref, clkid))
#define RK3588_GMAC_PTP_CFG(n)                                                                     \
	COND_CODE_1(RK3588_GMAC_HAS_PTP(n), \
		  (.ptp_clock = RK3588_GMAC_PTP_CLOCK(n), \
		   .ptp_clk = RK3588_GMAC_PTP_CLK(n),), ())
...
#define RK3588_GMAC_DEFINE(n)                                                                      \
	BUILD_ASSERT(DT_INST_REG_ADDR(n) == RK3588_GMAC0_REG_ADDR ||                               \
			     DT_INST_REG_ADDR(n) == RK3588_GMAC1_REG_ADDR,                         \
...
int dwmac_platform_init(const struct device *dev)
{
	...
	DWMAC_REG_WRITE(DMA_SYSBUS_MODE,
			DMA_SYSBUS_MODE_AAL | DMA_SYSBUS_MODE_EAME | DMA_SYSBUS_MODE_FB);
