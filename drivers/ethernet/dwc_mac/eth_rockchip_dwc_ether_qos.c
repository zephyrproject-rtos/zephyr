...
static void rk3588_gmac_configure_clock_selection(const struct device *php_grf, unsigned int id,
						  bool clock_input)
{
	uint32_t val = RK3588_GMAC_CLK_RMII_NOGATE(id);

	val |= clock_input ? RK3588_GMAC_CLK_SELECT_IO(id) : RK3588_GMAC_CLK_SELECT_CRU(id);

	syscon_write_reg(php_grf, RK3588_PHP_CLK_CON1, val);
}
...
static void rk3588_gmac_grf_prepare(const struct rk3588_gmac_config *cfg)
{
	...
	syscon_write_reg(cfg->php_grf, RK3588_PHP_GMAC_CON0, RK3588_GMAC_PHY_INTF_SEL_RGMII(id));
	syscon_write_reg(cfg->php_grf, RK3588_PHP_CLK_CON1, GRF_CLR_BIT(id * 5U));

	delay_con = (id == 0U) ? RK3588_GRF_GMAC_CON8 : RK3588_GRF_GMAC_CON9;
	syscon_write_reg(cfg->sys_grf, delay_con,
			 RK3588_GMAC_TXCLK_RXCLK_DLY_CFG(cfg->tx_delay, cfg->rx_delay));

	if (cfg->tx_delay != 0U) {
		con7 |= RK3588_GMAC_TXCLK_DLY_ENABLE(id);
	} else {
		con7 |= RK3588_GMAC_TXCLK_DLY_DISABLE(id);
	}
	...
	syscon_write_reg(cfg->sys_grf, RK3588_GRF_GMAC_CON7, con7);
}
					    ...
					    enum phy_link_speed speed)
{
	...
	if (PHY_LINK_IS_SPEED_1000M(speed)) {
		clk_div = RK3588_GMAC_CLK_RGMII_DIV1(id);
	} else if (PHY_LINK_IS_SPEED_100M(speed)) {
		clk_div = RK3588_GMAC_CLK_RGMII_DIV5(id);
	} else {
		clk_div = RK3588_GMAC_CLK_RGMII_DIV50(id);
	}

	syscon_write_reg(php_grf, RK3588_PHP_CLK_CON1, clk_div);
}
...
#define RK3588_GMAC_PTP_CLOCK(n) DEVICE_DT_GET_OR_NULL(DT_INST_CHILD(n, ptp_clock))
...
int dwmac_platform_init(const struct device *dev)
{
	...
	DWMAC_REG_WRITE(DMA_SYSBUS_MODE, DMA_SYSBUS_MODE_AAL | DMA_SYSBUS_MODE_EAME |
						 DMA_SYSBUS_MODE_FB | DMA_SYSBUS_MODE_BLEN16 |
						 DMA_SYSBUS_MODE_BLEN8 | DMA_SYSBUS_MODE_BLEN4 |
						 FIELD_PREP(DMA_SYSBUS_MODE_RD_OSR_LMT, 7) |
						 FIELD_PREP(DMA_SYSBUS_MODE_WR_OSR_LMT, 3));
