...
static void rk3588_gmac_configure_clock_selection(const struct device *php_grf, unsigned int id,
						bool clock_input)
{
	uint32_t cru_mask = (id == 0U) ? BIT(4) : BIT(9);
	uint32_t gate_mask = (id == 0U) ? BIT(1) : BIT(6);
	...
	rk3588s_clrsetreg(php_grf, RK3588_PHP_CLK_CON1, clr, set);
}
...
#define RK3588_GMAC_DEFINE(n)                                                                      \
	...
	static struct dwmac_dma_desc __nocache_noinit __aligned(CONFIG_DCACHE_LINE_SIZE)           \
	...
	static const struct rk3588_gmac_config rk3588_gmac_config_##n = {                          \
		...
		.clock_input = (DT_INST_ENUM_IDX(n, clock_in_out) == 0),                           \
