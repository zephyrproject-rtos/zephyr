...
/* PHY MMD device address 3 */
#define PHY_MMD_DEVICEID3              3U
/* EEE capability Register */
#define PHY_MMD3_EEE_CAP               0x14U
/* MMD3: smartEEE control 3 Register */
#define PHY_MMD3_SMARTEEE_CTRL3        0x805DU
#define PHY_MMD3_SMARTEEE_CTRL3_LPI_EN (1U << 8)

/* PHY MMD device address 7 */
#define PHY_MMD_DEVICEID7   7U
/* EEE advertisement Register */
#define PHY_MMD7_AN_EEE_ADV 0x3CU
/* EEE mask common for adv and cap */
#define PHY_EEE_1000T       0x4U
#define PHY_EEE_100TX       0x2U
...
static int qc_ar8031_init(const struct device *dev)
{
	...
	/* Close smartEEE */
	ret = qc_ar8031_mmd_set_device(dev, PHY_MMD_DEVICEID3, PHY_MMD3_SMARTEEE_CTRL3,
	...
	ret = qc_ar8031_write(dev, MII_MMD_AADR, reg_value & ~(PHY_MMD3_SMARTEEE_CTRL3_LPI_EN));
	if (ret) {
		return -EIO;
	}
	...
	if (cfg->enable_eee) {
		ret = qc_ar8031_mmd_read(dev, PHY_MMD_DEVICEID3, PHY_MMD3_EEE_CAP, &reg_value);
		if (ret) {
			return -EIO;
		}
		ret = qc_ar8031_mmd_write(dev, PHY_MMD_DEVICEID7, PHY_MMD7_AN_EEE_ADV,
					  reg_value & (PHY_EEE_1000T | PHY_EEE_100TX));
		if (ret) {
			return -EIO;
		}
	} else {
		ret = qc_ar8031_mmd_write(dev, PHY_MMD_DEVICEID7, PHY_MMD7_AN_EEE_ADV, 0);
		if (ret) {
			return -EIO;
		}
	}
...
#define AR8031_CONFIG(n)                                                                           \
	static const struct qc_ar8031_config qc_ar8031_config_##n = {                              \
		...
		.enable_eee = DT_INST_NODE_HAS_PROP(n, eee),                                       \
	};
