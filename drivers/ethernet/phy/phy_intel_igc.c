...
static int phy_intel_igc_cfg_link(const struct device *dev, enum phy_link_speed speed)
{
	...
	return phy_intel_igc_restart_aneg(dev);
}
					...
					struct phy_link_state *state)
{
	...
	ret = phy_intel_igc_read(dev, MII_BMSR, &bmsr);
	if (ret < 0) {
		return ret;
	}

	if ((bmsr & MII_BMSR_LINK_STATUS) == 0U) {
		state->is_up = false;
		return 0;
	}
	state->is_up = true;
	...
	} else if ((c1kt_val & s1kt_val & MII_ADVERTISE_1000_HALF) != 0U) {
