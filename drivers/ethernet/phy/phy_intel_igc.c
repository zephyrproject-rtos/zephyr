...
static int phy_intel_igc_restart_aneg(const struct device *dev)
{
	...
	ret = phy_intel_igc_await_phy_status(dev, MII_BMSR, MII_BMSR_AUTONEG_COMPLETE,
...
static void update_link_state(struct k_work *work)
{
	...
	ret = phy_intel_igc_get_link_state(dev, &new_state);
	if (ret < 0) {
		LOG_ERR("Failed to get PHY (%d) link state", cfg->phy_addr);
	}

	ret = phy_intel_igc_get_speed(dev, &new_state);
	if (ret < 0) {
		LOG_ERR("Failed to get PHY (%d) link Speed", cfg->phy_addr);
	}
	...
	if (new_state.is_up != data->state.is_up || new_state.speed != data->state.speed) {
		...
		if (data->state.is_up) {
			LOG_INF("PHY (%d) Link is Up with Speed %s Mbps & %s duplex",
				cfg->phy_addr,
			...
			LOG_INF("PHY (%d) Link is Down", cfg->phy_addr);
		}
	}
}
...
static int phy_intel_igc_init(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Autoneg init failed for 1 Gbps");
		return ret;
	}

	ret = phy_intel_igc_aneg_multigb(dev);
	if (ret < 0) {
		LOG_ERR("Autoneg init failed for Multi-Gigabit");
		return ret;
	}
...
#define INTEL_IGC_PHY_CONFIG(n) \
	static const struct phy_intel_igc_cfg phy_cfg_##n = { \
		...
		.mgb_support = DT_INST_PROP(n, multi_gigabit_support), \
	};
