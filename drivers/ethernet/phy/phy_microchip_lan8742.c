...
static int phy_lan8742_link_cb_set(const struct device *dev, phy_callback_t cb,
				   void *user_data)
{
	struct phy_lan8742_dev_data *const data = dev->data;
...
static int phy_lan8742_init(const struct device *dev)
{
	...
	phy_lan8742_cfg_link(dev, cfg->default_speeds, 0);
...
#define PHY_LAN8742_DEVICE(n)						  \
	...
	DEVICE_DT_INST_DEFINE(n,					  \
			      &phy_lan8742_init,			  \
			      NULL,					  \
			      &phy_lan8742_dev_data_##n,		  \
			      &phy_lan8742_dev_config_##n, POST_KERNEL,	  \
