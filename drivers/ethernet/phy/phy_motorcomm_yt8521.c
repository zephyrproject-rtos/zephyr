...
#define ANY_DYNAMIC_LINK UTIL_NOT(DT_ALL_INST_HAS_PROP_STATUS_OKAY(specified_link))
#define ANY_SPECIFIED_LINK   DT_ANY_INST_HAS_PROP_STATUS_OKAY(specified_link)
...
#if ANY_SPECIFIED_LINK
static inline uint16_t mii_bmcr_encode_specified(enum phy_link_speed speed)
{
	uint16_t bmcr;
...
#if ANY_SPECIFIED_LINK
static int mc_ytphy_initialize_specified_link(const struct device *dev)
{
	const struct mc_ytphy_config *const cfg = dev->config;
	struct mc_ytphy_data *const data = dev->data;
	...
	data->state.speed = speed_to_phy_link_speed[cfg->specified_speed];

	bmcr_val = mii_bmcr_encode_specified(data->state.speed);
	LOG_INF("PHY (%d) Specified link speed %s Mb, %s duplex", cfg->phy_addr,
		PHY_LINK_IS_SPEED_1000M(data->state.speed)
			? "1000"
			: (PHY_LINK_IS_SPEED_100M(data->state.speed) ? "100" : "10"),
		PHY_LINK_IS_FULL_DUPLEX(data->state.speed) ? "full" : "half");
...
#define MC_YTPHY_CONFIG(n)                                                                         \
	static const struct mc_ytphy_config mc_ytphy_config_##n = {                                \
		...
		.specified = DT_INST_NODE_HAS_PROP(n, specified_link),                             \
		.specified_speed = DT_INST_ENUM_IDX_OR(n, specified_link, 0),                      \
...
#else
#define MC_YTPHY_INIT &mc_ytphy_initialize_specified_link
#endif
