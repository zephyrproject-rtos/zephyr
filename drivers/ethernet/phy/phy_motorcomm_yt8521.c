			     ...
			     enum phy_cfg_link_flag flags)
{
	...
	BIT_SET_CLR(adv_speeds & LINK_HALF_10BASE, anar_reg, MII_ADVERTISE_10_HALF);
	BIT_SET_CLR(adv_speeds & LINK_FULL_10BASE, anar_reg, MII_ADVERTISE_10_FULL);

	BIT_SET_CLR(adv_speeds & LINK_HALF_100BASE, anar_reg, MII_ADVERTISE_100_HALF);
	BIT_SET_CLR(adv_speeds & LINK_FULL_100BASE, anar_reg, MII_ADVERTISE_100_FULL);

	if (bmsr_reg & MII_BMSR_EXTEND_STATUS) {
		BIT_SET_CLR(adv_speeds & LINK_HALF_1000BASE, c1kt_reg, MII_ADVERTISE_1000_HALF);
		BIT_SET_CLR(adv_speeds & LINK_FULL_1000BASE, c1kt_reg, MII_ADVERTISE_1000_FULL);
...
static int mc_ytphy_initialize_dynamic_link(const struct device *dev)
{
	...
	mc_ytphy_cfg_link(dev, data->phy_feature, 0);
