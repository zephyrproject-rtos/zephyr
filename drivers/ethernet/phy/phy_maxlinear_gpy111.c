...
#define GPY111_RESET_ASSERT_DURATION_US     2000
#define GPY111_RESET_DEASSERTION_TIMEOUT_MS 300
...
#define GPY111_PHY_ID 0xD565A401
...
static int reset(const struct device *dev)
{
	...
	if (gpio_is_ready_dt(&cfg->reset_gpio)) {
		...
		/* assertion time */
		k_busy_wait(GPY111_RESET_ASSERT_DURATION_US);
		...
		k_msleep(GPY111_RESET_DEASSERTION_TIMEOUT_MS);
...
static int check_autonegotiation_completion(const struct device *dev)
{
	...
	if (((c1kt_reg & s1kt_reg & MII_ADVERTISE_1000_FULL) != 0U)) {
		data->state.speed = LINK_FULL_1000BASE;
	} else if (((c1kt_reg & s1kt_reg & MII_ADVERTISE_1000_HALF) != 0U)) {
