...
static int phy_check_ksz9131_id(const struct device *dev)
{
	...
	do {
		BREAK_IF_ERR(ret, ksz9131_read(dev, MII_PHYID1R, &value));
		phy_id = value << 16;

		BREAK_IF_ERR(ret, ksz9131_read(dev, MII_PHYID2R, &value));
		phy_id |= value;
	} while (0);
...
static int phy_mchp_ksz9131_autonegotiate(const struct device *dev)
{
	...
	do {
		...
		BREAK_IF_ERR(ret, ksz9131_read(dev, MII_BMSR, &bmsr));
...
static int phy_mchp_ksz9131_cfg_link(const struct device *dev, enum phy_link_speed adv_speeds)
{
	...
	do {
		...
		BREAK_IF_ERR(ret, ksz9131_write(dev, MII_1KTCR, c1kt));
...
static int phy_mchp_ksz9131_get_link(const struct device *dev, struct phy_link_state *state)
{
	...
	do {
		...
		BREAK_IF_ERR(ret, ksz9131_read(dev, MII_ANLPAR, &anlpar));
...
static int phy_mchp_ksz9131_link_cb_set(const struct device *dev,
