...
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(rt_reset_gpio)
	const struct gpio_dt_spec reset_gpio;
#endif
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(rt_interrupt_gpio)
...
static int phy_rt_rtl8211f_reset(const struct device *dev)
{
	...
	/* Wait another 30 ms (for internal circuits settling time) before accessing registers */
...
#endif /* DT_ANY_INST_HAS_PROP_STATUS_OKAY(rt_reset_gpio) */
...
#else
	/* Unlike polling, link change interrupt is not disabled */
#endif /* !DT_ANY_INST_HAS_PROP_STATUS_OKAY(rt_interrupt_gpio) */
...
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(rt_reset_gpio)
#define RESET_GPIO(n) \
		.reset_gpio = GPIO_DT_SPEC_INST_GET_OR(n, rt_reset_gpio, {0}),
#else
#define RESET_GPIO(n)
#endif /* reset gpio */
...
#define INTERRUPT_GPIO(n) \
		.interrupt_gpio = GPIO_DT_SPEC_INST_GET_OR(n, rt_interrupt_gpio, {0}),
#else
#define INTERRUPT_GPIO(n)
