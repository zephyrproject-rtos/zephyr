...
{
	if (IN_RANGE(reg_addr, 0x0300U, 0xE01U)) {
		return 0x1F;
	} else if (IN_RANGE(reg_addr, 0x1000U, 0x18F8U)) {
		return 0x1;
	} else if (IN_RANGE(reg_addr, 0x3000U, 0x38E7U)) {
		return 0x3;
	} else if (IN_RANGE(reg_addr, 0x0200U, 0x020FU)) {
		return 0x7;
	}
...
static int phy_ti_dp83td510_read(const struct device *dev, uint16_t reg_addr, uint32_t *reg_data)
{
	const struct ti_dp83td510_config *cfg = dev->config;
	...
	*reg_data = 0U;
	...
	if (IN_RANGE(reg_addr, 0, 0x130)) {
		ret = mdio_read(cfg->mdio, cfg->addr, reg_addr, (uint16_t *)reg_data);
		if (ret < 0) {
			goto done;
		}
	} else {
		...
		ret = mdio_read(cfg->mdio, cfg->addr, MII_MMD_AADR, (uint16_t *)reg_data);
	}
	...
	if (ret < 0) {
		LOG_ERR("Error reading phy (%d) register (0x%04X)", cfg->addr, reg_addr);
	}
	return ret;
}

static int phy_ti_dp83td510_write(const struct device *dev, uint16_t reg_addr, uint32_t reg_data)
{
	const struct ti_dp83td510_config *cfg = dev->config;
	struct ti_dp83td510_data *data = dev->data;
	int ret;
	...
	if (IN_RANGE(reg_addr, 0, 0x130)) {
		ret = mdio_write(cfg->mdio, cfg->addr, reg_addr, (uint16_t)reg_data);
		if (ret < 0) {
			goto done;
		}
	} else {
		...
		ret = mdio_write(cfg->mdio, cfg->addr, MII_MMD_AADR, (uint16_t)reg_data);
	}
	...
	if (ret < 0) {
		LOG_ERR("Error writing phy (%d) register (0x%04X)", cfg->addr, reg_addr);
	}
	return ret;
}
...
static int phy_ti_dp83td510_autonegotiate(const struct device *dev)
{
	...
	anar |= PHY_TI_DP83TD510_ANAR_TXLVL;
...
#if DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios)
static void phy_ti_dp83td510_monitor_work_handler_irq(struct k_work *work)
{
...
static void phy_ti_dp83td510_monitor_work_handler_poll(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct ti_dp83td510_data *data =
		CONTAINER_OF(dwork, struct ti_dp83td510_data, phy_monitor_work);
...
static int phy_ti_dp83td510_init(const struct device *dev)
{
	...
	if (config->interrupt_gpio.port) {
		k_work_init_delayable(&data->phy_monitor_work,
				      phy_ti_dp83td510_monitor_work_handler_irq);
		...
		k_work_init_delayable(&data->phy_monitor_work,
				      phy_ti_dp83td510_monitor_work_handler_poll);
		k_work_reschedule(&data->phy_monitor_work, K_NO_WAIT);
	}
#else
	k_work_init_delayable(&data->phy_monitor_work, phy_ti_dp83td510_monitor_work_handler_poll);
	k_work_reschedule(&data->phy_monitor_work, K_NO_WAIT);
#endif /* DT_ANY_INST_HAS_PROP_STATUS_OKAY(int_gpios) */
	...
	if (phy_id == DP83TD510E_PHYID) {
		LOG_INF("Recognized DP83TD510E PHY_ID: 0x%X", phy_id);
	} else {
