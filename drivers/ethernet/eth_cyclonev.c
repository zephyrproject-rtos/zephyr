#ifndef ETH_CYCLONEV_SRC
#define ETH_CYCLONEV_SRC
...
int eth_cyclonev_get_software_reset_status(uint32_t instance);
int eth_cyclonev_software_reset(uint32_t instance);
...
int eth_cyclonev_bus_init(struct eth_cyclonev_priv *p);
 ...
 * Function initialise HPS interface, see https://www.intel.com/content/dam/www/programmable
...
/*	 4a. Set the physel_* field in the ctrl register of the System Manager (EMAC Group)
 *to 0x1 to select the RGMII PHY interface.
 */

	alt_replbits_word(SYSMGR_EMAC_ADDR,
			  Sysmgr_Core_Emac_Phy_Intf_Sel_Set_Msk[instance],
			  Sysmgr_Emac_Phy_Intf_Sel_E_Rgmii[instance]);

/*	 4b. Disable the Ethernet Controller FPGA interfaces by clearing the emac_* bit in the
 * module register of the System Manager (FPGA Interface group).
 */

	sys_clear_bits(SYSMGR_FPGAINTF_INDIV_ADDR, Sysmgr_Fpgaintf_En_3_Emac_Set_Msk[instance]);

/*	 7. After confirming the settings are valid, software can clear the emac bit in the
 * permodrst register of the Reset Manager to bring the EMAC out of reset.
 */
...
void eth_cyclonev_set_mac_addr(uint8_t *address, uint32_t instance, uint32_t n)
{
	...
	sys_write32(tmpreg, EMAC_GMAC_MAC_ADDR_HIGH_ADDR(Emac_Addr[instance], n));
	...
	sys_write32(tmpreg, EMAC_GMAC_MAC_ADDR_LOW_ADDR(Emac_Addr[instance], n));
}
...
int eth_cyclonev_get_software_reset_status(uint32_t instance)
{
	...
	return EMAC_DMA_MODE_SWR_GET(sys_read32(EMAC_DMAGRP_BUS_MODE_ADDR(Emac_Addr[instance])));
}

/**
 * @brief Perform software reset
 * Resets all MAC subsystem registers and logic, wait for the software reset to clear
 *
 * @param instance Number of instance (0 or 1 in Cyclone V HPS)
...
int eth_cyclonev_software_reset(uint32_t instance)
{
	...
	sys_set_bits(EMAC_DMAGRP_BUS_MODE_ADDR(Emac_Addr[instance]),
			 EMAC_DMA_MODE_SWR_SET_MSK);

	/* Wait for the software reset to clear */
	for (i = 0; i < 10; i++) {
		k_sleep(K_MSEC(10));
		if (eth_cyclonev_get_software_reset_status(instance) == 0) {
			break;
		}
	}
...
void eth_cyclonev_setup_rxdesc(struct eth_cyclonev_priv *p)
{
	...
	sys_write32((uint32_t)&p->rx_desc_ring[0],
		EMAC_DMA_RX_DESC_LIST_ADDR(Emac_Addr[p->instance]));
}
...
void eth_cyclonev_setup_txdesc(struct eth_cyclonev_priv *p)
{
	...
	sys_write32((uint32_t)&p->tx_desc_ring[0],
		EMAC_DMA_TX_DESC_LIST_ADDR(Emac_Addr[p->instance]));
}
...
static void eth_cyclonev_iface_init(struct net_if *iface)
{
	...
	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority), eth_cyclonev_isr,
		...
		const struct ethernet_config *config)
{
	...
	case ETHERNET_CONFIG_TYPE_MAC_ADDRESS:
		...
		eth_cyclonev_set_mac_addr(p->mac_addr, p->instance, 0); /* Set MAC address */
		net_if_set_link_addr(p->iface, p->mac_addr,
		sizeof(p->mac_addr), NET_LINK_ETHERNET);
		break;
#if defined(CONFIG_NET_PROMISCUOUS_MODE)
	case ETHERNET_CONFIG_TYPE_PROMISC_MODE:
		reg_val = sys_read32(EMAC_GMACGRP_MAC_FRAME_FILTER_ADDR(Emac_Addr[p->instance]));
		if (config->promisc_mode && !(reg_val & EMAC_GMACGRP_MAC_FRAME_FILTER_PR_SET_MSK)) {
		/* Turn on Promisc Mode */
			sys_set_bits(EMAC_GMACGRP_MAC_FRAME_FILTER_ADDR(Emac_Addr[p->instance]),
			EMAC_GMACGRP_MAC_FRAME_FILTER_PR_SET_MSK);
		} else if (!config->promisc_mode && (reg_val &
			EMAC_GMACGRP_MAC_FRAME_FILTER_PR_SET_MSK)) {
			/* Turn off Promisc Mode */
			sys_clear_bits(EMAC_GMACGRP_MAC_FRAME_FILTER_ADDR(Emac_Addr[p->instance]),
			EMAC_GMACGRP_MAC_FRAME_FILTER_PR_SET_MSK);
			} else {
				ret = -EALREADY;
				}
 ...
 * Sends packet of data. See: https://www.intel.com/content/dam/www/programmable/us/en/pdfs/
...
static int eth_cyclonev_send(const struct device *dev, struct net_pkt *pkt)
{
	...
	do {
		...
		if (frag) {
			memcpy(&p->tx_buf[p->tx_current_desc_number * ETH_BUFFER_SIZE],
				frag->data, len);
			}
		...
		LOG_DBG("Desc[%d] at address: 0x%08x: , Frag size: %d, Buffer Addr: %p",
			...
			(unsigned int) &p->tx_desc_ring[p->tx_current_desc_number],
			frag->len, (void *) tx_desc->buffer1_addr);
		...
		if (!frag->frags) {
			...
			LOG_DBG("Current Host Transmit Descriptor Register: 0x%08x",
				sys_read32(EMAC_DMA_CURR_HOST_TX_DESC_ADDR
					(Emac_Addr[p->instance])));
			LOG_DBG("Current Host Transmit Buffer Register: 0x%08x",
				sys_read32(EMAC_DMA_CURR_HOST_TX_BUFF_ADDR
					(Emac_Addr[p->instance])));

			/* If the DMA transmission is suspended, resume transmission.  */
			if (sys_read32(EMAC_DMAGRP_STATUS_ADDR
				(Emac_Addr[p->instance]))
				& EMAC_DMAGRP_STATUS_TS_SET_MSK){

				/* Clear TBUS ETHERNET DMA flag */
				sys_write32(EMAC_DMAGRP_STATUS_TS_SET_MSK,
					EMAC_DMAGRP_STATUS_ADDR(Emac_Addr[p->instance]));

				/* Resume DMA transmission */
				sys_write32(0, EMAC_DMA_TX_POLL_DEMAND_ADDR
					(Emac_Addr[p->instance]));
			}
		}
		frag = frag->frags;
...
void eth_cyclonev_isr(const struct device *dev)
{
	...
	irq_status = sys_read32(EMAC_DMAGRP_STATUS_ADDR
		(Emac_Addr[p->instance])) & p->interrupt_mask;
	irq_status_emac = sys_read32(EMAC_GMAC_INT_STAT_ADDR(Emac_Addr[p->instance]));
	...
	if (irq_status_emac & EMAC_GMAC_INT_STAT_RGSMIIIS_SET_MSK) {
		...
		uint32_t regval = sys_read32(GMACGRP_CONTROL_STATUS_ADDR(Emac_Addr[p->instance]));
		...
		if (p->initialised) {
			...
			cfg_reg_set = sys_read32(GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]));
		...
		set_mac_conf_status(p->instance, &cfg_reg_set);
		sys_write32(cfg_reg_set,
			GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]));
		...
		do {
			...
			if (last_desc_index != rx_search) {
				...
				if (rx_search == last_desc_index) {
					...
					if (pkt) {
						net_pkt_write(pkt,
						&p->rx_buf[rx_search * ETH_BUFFER_SIZE], buf_size);
					}
					data_remaining -= buf_size;
...
/**
 * @brief Inits device structure variables
 * Saves base address into variable, detects device instance number
 *
 * @param p Pointer to device structure
 * @retval 0 if successful, -1 otherwise
 */
...
int eth_cyclonev_bus_init(struct eth_cyclonev_priv *p)
{
	...
	if (p->base_addr == (mem_addr_t) Emac_Addr[0]) {
		p->instance = 0;
	} else if (p->base_addr == (mem_addr_t) Emac_Addr[1]) {
		p->instance = 1;
	} else {
		return -1;
		}
...
int set_mac_conf_status(int instance, uint32_t *mac_config_reg_settings)
{
	...
	ret = alt_eth_phy_get_duplex_and_speed(&phy_duplex_status, &phy_speed, instance);
	if (ret != 0) {
 ...
 * EMAC initialisation and configuration. See: https://www.intel.com/content/dam/
...
int eth_cyclonev_probe(const struct device *dev)
{
	...
	ret = eth_cyclonev_bus_init(p);
	if (ret != 0) {
		LOG_ERR("eth_cyclonev_bus_init failure!\n");
		return ret;
	}
	...
	/* Reset the PHY  */
	ret = alt_eth_phy_reset(p->instance);
	if (ret != 0) {
	...
	/* Configure the PHY */
	ret = alt_eth_phy_config(p->instance);
	if (ret != 0) {
	...
	p->feature = sys_read32(EMAC_DMA_HW_FEATURE_ADDR(Emac_Addr[p->instance]));

	/* DMA Initialisation */

	/* 1. Provide a software reset to reset all of the EMAC internal registers and logic.
	 *(DMA Register 0 (BusMode Register) – bit 0).
	 * 2. Wait for the completion of the reset process (poll bit 0 of the DMA Register
	 * 0 (Bus Mode Register), which is only cleared after the reset operation is completed).
	 */

	ret = eth_cyclonev_software_reset(p->instance);
	if (ret != 0) {
		LOG_ERR("eth_cyclonev_software_reset failure!\n");
		return ret;
	}

/* 4. Program the following fields to initialize the Bus Mode Register by setting
 * values in DMA Register 0 (Bus Mode Register):
 */

	sys_write32((tmpreg
		| EMAC_DMA_MODE_FB_SET_MSK		  /* Fixed Burst */
	),
	EMAC_DMAGRP_BUS_MODE_ADDR(Emac_Addr[p->instance]));

/*	 5. Program the interface options in Register 10 (AXI Bus Mode Register).
 * If fixed burst-length is enabled, then select the maximum burst-length
 * possible on the bus (bits[7:1]).(58)
 */

	tmpreg = sys_read32(EMAC_DMAGRP_AXI_BUS_MODE_ADDR(Emac_Addr[p->instance]));

	sys_write32(tmpreg | EMAC_DMAGRP_AXI_BUS_MODE_BLEN16_SET_MSK,
		EMAC_DMAGRP_AXI_BUS_MODE_ADDR(Emac_Addr[p->instance])); /* Set Burst Length = 16 */


/* 6. Create a proper descriptor chain for transmit and receive. In addition, ensure that
 * the receive descriptors are owned by DMA (bit 31 of descriptor should be set).
 * 7. Make sure that your software creates three or more different transmit or receive
 * descriptors in the chain before reusing any of the descriptors
 * 8. Initialize receive and transmit descriptor list address with the base address
 * of the transmit and receive descriptor (Register 3 (Receive Descriptor List Address
 * Register) and Register 4 (Transmit Descriptor List Address Register) respectively).
 */
 ...
 */
	...
	sys_write32((0
	| EMAC_DMAGRP_OPERATION_MODE_TSF_SET_MSK	 /* Transmit Store and Forward */
	...
	), EMAC_DMAGRP_OPERATION_MODE_ADDR(Emac_Addr[p->instance]));
 ...
 * bit 16, the normal interrupt summary clears this bit (DMA Register 5 (Status Register)).
 */

	interrupt_mask = EMAC_DMA_INT_EN_NIE_SET_MSK |
			 EMAC_DMA_INT_EN_RIE_SET_MSK |
			 EMAC_DMA_INT_EN_TIE_SET_MSK;

	p->interrupt_mask = interrupt_mask;

	/* Clear the selected ETHERNET DMA bit(s) */
	sys_write32(interrupt_mask, EMAC_DMAGRP_STATUS_ADDR(Emac_Addr[p->instance]));

/*	11.Enable the interrupts by programming Register 7 (Interrupt Enable Register).†*/

	sys_set_bits(EMAC_DMA_INT_EN_ADDR(Emac_Addr[p->instance]), interrupt_mask);

/* 12.Read Register 11 (AHB or AXI Status) to confirm that
 * all previous transactions are complete.
 */

	if (sys_read32(EMAC_DMAGRP_AHB_OR_AXI_STATUS_ADDR(Emac_Addr[p->instance])) != 0) {
 ...
 * management cycles for theexternal PHY. Bits[15:11] of the GMII Address Register
 * are written with the Physical Layer Address of the PHY before reading or writing.
 * Bit 0 indicates if the PHY is busy and is set before reading or writing to the
 * PHY management interface.
 * 2. Read the 16-bit data of the GMII Data Register from the PHY for link up,
 * speed of operation, and mode of operation, by specifying the appropriate address
 * value in bits[15:11] of the GMII Address Register.
 */
	...
	ret = set_mac_conf_status(p->instance, &mac_config_reg_settings);
	if (ret != 0) {
		return -1;
		}

/*	3. Provide the MAC address registers (MAC Address0 High Register through MAC Address15 High
 * Register and MAC Address0 Low Register through MAC Address15 Low Register).
 */

	memcpy(p->mac_addr, eth_cyclonev_mac_addr, sizeof(p->mac_addr));
	eth_cyclonev_set_mac_addr(p->mac_addr, p->instance, 0);
 ...
 */
	...
	sys_set_bits(EMAC_GMAC_INT_MSK_ADDR(Emac_Addr[p->instance]),
 ...
 */
	...
	sys_write32(mac_config_reg_settings,
		GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]));
 ...
 * Starts DMA and EMAC transmitter and receiver. See: https://www.intel.com/content/dam/
 ...
 */
	...
	/* Start the DMA */
	sys_set_bits(EMAC_DMAGRP_OPERATION_MODE_ADDR(Emac_Addr[p->instance]),
		EMAC_DMAGRP_OPERATION_MODE_ST_SET_MSK);
	sys_set_bits(EMAC_DMAGRP_OPERATION_MODE_ADDR(Emac_Addr[p->instance]),
		EMAC_DMAGRP_OPERATION_MODE_SR_SET_MSK);

	/* Enable the EMAC transmitter and receiver */
	sys_set_bits(GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]),
 ...
 * Stops DMA and EMAC transmitter and receiver. See: https://www.intel.com/content/dam/www/
...
static int eth_cyclonev_stop(const struct device *dev)
{
	 ...
	 * (Start or Stop Transmission Command) of Register 6 (Operation Mode Register).
	 */

	sys_clear_bits(EMAC_DMAGRP_OPERATION_MODE_ADDR(Emac_Addr[p->instance]),
		EMAC_DMAGRP_OPERATION_MODE_ST_SET_MSK);
	 ...
	 */
	sys_clear_bits(GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]),
		EMAC_GMACGRP_MAC_CONFIGURATION_TE_SET_MSK);
	sys_set_bits(EMAC_DMAGRP_OPERATION_MODE_ADDR(Emac_Addr[p->instance]),
		EMAC_DMAGRP_OPERATION_MODE_FTF_SET_MSK);	/* Flush Transmit FIFO */
	sys_clear_bits(GMACGRP_MAC_CONFIG_ADDR(Emac_Addr[p->instance]),
	...
	if (EMAC_DMAGRP_DEBUG_RXFSTS_GET(sys_read32
		(EMAC_DMAGRP_DEBUG_ADDR(Emac_Addr[p->instance]))) != 0x0) {
		return -1;
	}
...
static struct eth_cyclonev_priv eth_cyclonev_instance;

const struct ethernet_api eth_cyclonev_api = {
