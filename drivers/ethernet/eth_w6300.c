/* W6300 Stand-alone Ethernet Controller with PIO QSPI transport
 *
...
{
	return w6300_pio_qspi_read(dev, addr, data, len);
}

static int w6300_spi_write(const struct device *dev, uint32_t addr, const uint8_t *data, size_t len)
{
	return w6300_pio_qspi_write(dev, addr, data, len);
}
 ...
 * write to S0_CR. The k_yield() calls inside the PIO QSPI transport allow
 * these two threads to interleave bus transactions, so a SEND issued by
 * w6300_tx can be overwritten by a concurrent RECV from w6300_thread (or
 * vice versa). cmd_lock prevents this.
 */
...
static int w6300_set_config(const struct device *dev, struct net_if *iface,
...
static int w6300_init(const struct device *dev)
{
	...
	err = w6300_pio_qspi_init(dev);
	if (err < 0) {
		LOG_ERR("Unable to initialize W6300 PIO QSPI transport");
		return err;
	}
...
static const struct w6300_config w6300_0_config = {
	.qspi = {
		.piodev = DEVICE_DT_GET(DT_INST_PARENT(0)),
		.pin_cfg = PINCTRL_DT_INST_DEV_CONFIG_GET(0),
		.clk_gpio = GPIO_DT_SPEC_INST_GET(0, clk_gpios),
		.io0_gpio = GPIO_DT_SPEC_INST_GET(0, io0_gpios),
		.io1_gpio = GPIO_DT_SPEC_INST_GET(0, io1_gpios),
		.io2_gpio = GPIO_DT_SPEC_INST_GET(0, io2_gpios),
		.io3_gpio = GPIO_DT_SPEC_INST_GET(0, io3_gpios),
		...
		.frequency = DT_INST_PROP(0, clock_frequency),
	},
