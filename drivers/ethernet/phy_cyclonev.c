...
#define PHY_RESTART_AUTONEGOTIATION	(1 << 9)

/* Basic Status Register */
#define PHY_BSR				(1)
#define PHY_AUTOCAP			(1 << 3)	/* Auto-negotiation capability */
#define PHY_LINKED_STATUS		(1 << 2)
#define PHY_AUTONEGO_COMPLETE		(1 << 5)

/* Auto-Negotiation Advertisement */
#define PHY_AUTON			(4)
#define PHYANA_10BASET			(1 << 5)
#define PHYANA_10BASETFD		(1 << 6)
#define PHYANA_100BASETX		(1 << 7)
#define PHYANA_100BASETXFD		(1 << 8)
#define PHYASYMETRIC_PAUSE		(1 << 11)
...
#define PHYADVERTISE_1000FULL		(1 << 9)
#define PHYADVERTISE_1000HALF		(1 << 8)
...
int alt_eth_phy_write_register_extended(uint16_t emac_instance,
	uint16_t phy_reg, uint16_t phy_value);
int alt_eth_phy_read_register_extended(uint16_t emac_instance,
	uint16_t phy_reg, uint16_t *rdval);
int alt_eth_phy_config(uint16_t instance);
int alt_eth_phy_reset(uint16_t instance);
int alt_eth_phy_get_duplex_and_speed(uint16_t *phy_duplex_status,
	uint16_t *phy_speed, uint16_t instance);
...
int alt_eth_phy_write_register(uint16_t emac_instance, uint16_t phy_reg, uint16_t phy_value)
{
	...
	sys_write32(phy_value & 0xffff, EMAC_GMAC_GMII_DATA_ADDR(Emac_Addr[emac_instance]));
	/* Write the result value into the MII Address register */
	sys_write32(tmpreg & 0xffff, EMAC_GMAC_GMII_ADDR_ADDR(Emac_Addr[emac_instance]));

	/* Check the Busy flag */
	do {
		timeout++;
		tmpreg = sys_read32(EMAC_GMAC_GMII_ADDR_ADDR(Emac_Addr[emac_instance]));
...
int alt_eth_phy_read_register(uint16_t emac_instance, uint16_t phy_reg, uint16_t *rdval)
{
	...
	*rdval = sys_read32(EMAC_GMAC_GMII_DATA_ADDR(Emac_Addr[emac_instance]));
	...
	uint16_t phy_reg, uint16_t phy_value)
{
	...
	rc = alt_eth_phy_write_register(emac_instance, MII_KSZPHY_EXTREG,
		KSZPHY_EXTREG_WRITE | phy_reg);

	if (rc == -1) {
		return rc;
		}

	rc = alt_eth_phy_write_register(emac_instance, MII_KSZPHY_EXTREG_WRITE, phy_value);
	return rc;
	...
	uint16_t phy_reg, uint16_t *rdval)
{
	...
	rc = alt_eth_phy_write_register(emac_instance, MII_KSZPHY_EXTREG, phy_reg);
	...
	rc = alt_eth_phy_read_register(emac_instance, MII_KSZPHY_EXTREG_READ, rdval);
...
int alt_eth_phy_config(uint16_t instance)
{
	...
	rc = alt_eth_phy_read_register(instance, PHY_BSR, &rdval);
	 ...
	 */
	rc = alt_eth_phy_read_register(instance, PHY_AUTON, &rdval);
	if (rc == -1) {
		return rc;
		}

	rdval |= (PHYANA_10BASET | PHYANA_10BASETFD | PHYANA_100BASETX |
		PHYANA_100BASETXFD | PHYASYMETRIC_PAUSE);
	rc = alt_eth_phy_write_register(instance, PHY_AUTON, rdval);
	if (rc == -1) {
		return rc;
		}
	...
	rc = alt_eth_phy_write_register(instance, PHY_1GCTL,
		  PHYADVERTISE_1000FULL
		| PHYADVERTISE_1000HALF);
	if (rc == -1) {
		return rc;
		}
	...
	rc = alt_eth_phy_read_register(instance, PHY_BCR, &rdval);
	if (rc == -1) {
		return rc;
		}
	...
	rc = alt_eth_phy_write_register(instance, PHY_BCR, rdval);
	if (rc == -1) {
		return rc;
		}
	...
	/* Return ERROR in case of timeout */
	if ((timeout == PHY_READ_TO) || (rc == -1)) {
		alt_eth_phy_read_register(instance, PHY_BSR, &rdval);
...
int alt_eth_phy_reset(uint16_t instance)
{
	...
	if ((alt_eth_phy_write_register(instance, PHY_BCR, PHY_RESET)) != 0) {
	...
	uint16_t *phy_speed, uint16_t instance)
{
	...
	rc = alt_eth_phy_read_register(instance, PHY_CR, &regval);
