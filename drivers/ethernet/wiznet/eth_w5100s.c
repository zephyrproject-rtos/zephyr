...
static void w5100s_set_macaddr(const struct device *dev)
{
	...
	w5100s_spi_write(dev, W5100S_NETLCKR, &lock, 1);
...
static int w5100s_init(const struct device *dev)
{
	...
	err = w5100s_spi_read(dev, W5100S_VERR, &verr, 1);
	if (err != 0) {
		LOG_ERR("Failed to read chip version: %d", err);
		return err;
	}
...
#define W5100S_DEFINE(node)                                                                        \
	WIZNET_DEVICE_DEFINE(node, w5100s_init, &w5100s_phy_driver_api, &w5100s_ops,               \
			     &w5100s_regs, 500, 100)
