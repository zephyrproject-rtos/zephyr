...
static int nxp_s32_eth_configure_cgm(uint8_t port_idx)
{
	...
	if (port_idx == 0) {
		...
		while (((IP_MC_CGM_1->MUX_7_CSS & MC_CGM_MUX_7_CSS_SWIP_MASK) != 0) && (tout > 0)) {
			tout--;
		}
		while (((IP_MC_CGM_1->MUX_7_CSS & MC_CGM_MUX_7_CSS_SWTRG_MASK) >>
				MC_CGM_MUX_7_CSS_SWTRG_SHIFT !=
			1) &&
		       (tout > 0)) {
			tout--;
		}
		...
		while (((IP_MC_CGM_1->MUX_9_CSS & MC_CGM_MUX_9_CSS_SWIP_MASK) != 0) && (tout > 0)) {
			tout--;
		}
		while (((IP_MC_CGM_1->MUX_9_CSS & MC_CGM_MUX_9_CSS_SWTRG_MASK) >>
				MC_CGM_MUX_9_CSS_SWTRG_SHIFT !=
			1) &&
		       (tout > 0)) {
			tout--;
		}
...
#define NETC_VSI_GENERAL_CFG(node, prop, idx)                                                      \
	[DT_PROP_BY_IDX(node, prop, idx)] = {                                                      \
		...
		.NumberOfTxBDR = 1,                                                                \
		.SIVlanControl =                                                                   \
			(NETC_F3_PSICFGR0_SIVC_CVLAN_BIT | NETC_F3_PSICFGR0_SIVC_SVLAN_BIT),       \
...
#define NETC_VSI_RX_MSG_BUF(node, prop, idx, n)                                                    \
	BUILD_ASSERT((DT_PROP_BY_IDX(node, prop, idx) > NETC_ETH_IP_PSI_INDEX) &&                  \
			     (DT_PROP_BY_IDX(node, prop, idx) <=                                   \
			      FEATURE_NETC_ETH_NUM_OF_VIRTUAL_CTRLS),                              \
		     "Invalid VSI index");                                                         \
	static Netc_Eth_Ip_VsiToPsiMsgType _CONCAT3(nxp_s32_eth##n##_vsi,                          \
						    DT_PROP_BY_IDX(node, prop, idx), _rx_msg_buf)  \
...
#define NETC_PSI_INSTANCE_DEFINE(n)                                                                \
	...
	NETC_TX_RING(n, TX_RING_IDX, CONFIG_ETH_NXP_S32_TX_RING_LEN,                               \
	...
	static const Netc_Eth_Ip_EnetcGeneralConfigType nxp_s32_eth##n##_enetc_general_cfg = {     \
		...
		.maskMACPromiscuousUnicastEnable = (uint16_t)true,)) .RxVsiMsgCmdToPsi = {               \
						     COND_CODE_1(INIT_VSIS(n),						\
			(DT_INST_FOREACH_PROP_ELEM_SEP_VARGS(n, vsis,			\
				NETC_VSI_RX_MSG_BUF_ARRAY, (,), n)),			\
			(EMPTY)) },                   \
							     .maskMACVLANPromiscuousEnable =       \
								     (uint16)0x3U,                 \
							     .maskVLANAllowUntaggedEnable =        \
								     (uint32)0x30000U,             \
	};                                                                                         \
                                                                                                   \
	static const Netc_Eth_Ip_StationInterfaceConfigType nxp_s32_eth##n##_si_cfg = {            \
		...
		((uint32 *)MRU_MBOX_ADDR(DT_DRV_INST(n), vsi_msg)), (NULL)),               \
			    .EnableSIMsgInterrupt = true, .RxInterrupts = (uint32_t)true,          \
			    .TxInterrupts = (uint32_t)false,                                       \
			    .MACFilterTableMaxNumOfEntries =                                       \
				    CONFIG_ETH_NXP_S32_MAC_FILTER_TABLE_SIZE,                      \
	};                                                                                         \
                                                                                                   \
	...
	static const struct nxp_s32_eth_config nxp_s32_eth##n##_config = {                         \
		...
		.msix = {NETC_MSIX(DT_DRV_INST(n), rx, nxp_s32_eth_psi##n##_rx_event),             \
			 COND_CODE_1(INIT_VSIS(n),						\
			(NETC_MSIX(DT_DRV_INST(n), vsi_msg, Netc_Eth_Ip_MSIX_SIMsgEvent)),\
			(EMPTY)) },                                     \
				     .mac_filter_hash_table =                                      \
					     &nxp_s32_eth##n##_mac_filter_hash_table[0],           \
	...
	static struct nxp_s32_eth_data nxp_s32_eth##n##_data = {                                   \
		.mac_addr = DT_INST_PROP_OR(n, local_mac_address, {0}),                            \
                                                                                                   ...
                                                                                                   \
	ETH_NET_DEVICE_DT_INST_DEFINE(n, nxp_s32_eth_initialize, NULL, &nxp_s32_eth##n##_data,     \
				      &nxp_s32_eth##n##_config, CONFIG_ETH_INIT_PRIORITY,          \
				      &nxp_s32_eth_api, NET_ETH_MTU);
...
static int nxp_s32_eth_switch_init(void)
{
	...
	if (swt_status != E_OK) {
		LOG_ERR("Failed to initialize NETC Switch %d (%d)", NETC_SWITCH_IDX, swt_status);
		return -EIO;
	}
