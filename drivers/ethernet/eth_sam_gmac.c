 ...
 * Reset ring buffer
 */
static void simple_queue_reset(struct simple_queue *rb)
{
	rb->head = 0U;
	rb->tail = 0U;
}
 ...
 */
static uint32_t simple_queue_get(struct simple_queue *rb)
{
	uint32_t val;
 ...
 */
static void simple_queue_put(struct simple_queue *rb, uint32_t val)
{
	rb->buf[rb->head] = val;
...
#if GMAC_MULTIPLE_TX_PACKETS == 1
	/* Reset TX frame list */
	simple_queue_reset(&queue->tx_frag_list);
#if defined(CONFIG_PTP_CLOCK_SAM_GMAC)
	simple_queue_reset(&queue->tx_frames);
#endif
#endif
}
...
static void tx_completed(Gmac *gmac, struct gmac_queue *queue)
{
	...
	while (tx_desc_list->tail != tx_desc_list->head) {
		...
		frag = UINT_TO_POINTER(simple_queue_get(&queue->tx_frag_list));
		...
		if (tx_desc->w1 & GMAC_TXW1_LASTBUFFER) {
			...
			pkt = UINT_TO_POINTER(simple_queue_get(&queue->tx_frames));
...
#if GMAC_MULTIPLE_TX_PACKETS == 1
	struct net_buf *frag;
	struct simple_queue *tx_frag_list = &queue->tx_frag_list;
#if defined(CONFIG_PTP_CLOCK_SAM_GMAC)
	struct net_pkt *pkt;
	struct simple_queue *tx_frames = &queue->tx_frames;
#endif
#endif
...
static int eth_tx(const struct device *dev, struct net_pkt *pkt)
{
	...
	while (frag) {
		...
		simple_queue_put(&queue->tx_frag_list, POINTER_TO_UINT(frag));
	...
	simple_queue_put(&queue->tx_frames, POINTER_TO_UINT(pkt));
