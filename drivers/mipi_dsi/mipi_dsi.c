			      ...
			      size_t nparams, void *buf, size_t len)
{
	struct mipi_dsi_msg msg = {
		.tx_len = nparams, .tx_buf = params, .rx_len = len, .rx_buf = buf};
...
ssize_t mipi_dsi_generic_write(const struct device *dev, uint8_t channel, const void *buf,
			       size_t len)
{
	struct mipi_dsi_msg msg = {.tx_buf = buf, .tx_len = len};
...
{
	struct mipi_dsi_msg msg = {
		.type = MIPI_DSI_DCS_READ, .cmd = cmd, .rx_buf = buf, .rx_len = len};
...
ssize_t mipi_dsi_dcs_write(const struct device *dev, uint8_t channel, uint8_t cmd, const void *buf,
			   size_t len)
{
	struct mipi_dsi_msg msg = {.cmd = cmd, .tx_buf = buf, .tx_len = len};
