...
#endif
#ifdef MIPI_DSI_MCUX_NXP_DCNANO_LCDIF
...
#if MIPI_DSI_MCUX_NXP_DCNANO_LCDIF
				 ...
				 struct mipi_dsi_msg *msg)
{
	...
	if (((msg->cmd == MIPI_DCS_WRITE_MEMORY_START) ||
		...
		if (msg->cmd == MIPI_DCS_WRITE_MEMORY_START) {
			desc.first_write = true;
		} else {
			desc.first_write = false;
		}

		mipi_dbi_write_display(config->mipi_dbi, NULL, msg->tx_buf,
			&desc, pixfmt);
...
#define MCUX_MIPI_DSI_DEVICE(id)								\
	...
	static const struct mcux_mipi_dsi_config mipi_dsi_config_##id = {			\
		...
		COND_CODE_1(MIPI_DSI_MCUX_NXP_DCNANO_LCDIF,					\
