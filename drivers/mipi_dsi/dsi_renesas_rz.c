...
#define DEV_DATA(dev) ((struct mipi_dsi_renesas_rz_data *)((dev)->data))
#define DEV_CFG(dev)  ((const struct mipi_dsi_renesas_rz_config *)((dev)->config))

struct mipi_dsi_renesas_rz_config {
	DEVICE_MMIO_NAMED_ROM(dphy);
	DEVICE_MMIO_NAMED_ROM(link);
...
static int mipi_dsi_renesas_rz_init(const struct device *dev)
{
	...
	DEVICE_MMIO_NAMED_MAP(dev, link, K_MEM_CACHE_NONE);
...
/* Helpers to get the zephyr,panel-timing compatible child node of a renesas,rz-lcdc instance.
 * Only works for renesas,rz-lcdc node with a single zephyr,panel-timing child node.
 */
#define ZEPHYR_PANEL_TIMING_COMPAT_NODE(node_id)                                                   \
	COND_CODE_1(DT_NODE_HAS_COMPAT(node_id, zephyr_panel_timing), (node_id), ())

#define LCD_TIMINGS_NODE                                                                           \
	DT_FOREACH_CHILD_STATUS_OKAY(DT_INST(0, renesas_rz_lcdc), ZEPHYR_PANEL_TIMING_COMPAT_NODE)

BUILD_ASSERT(IS_EMPTY(LCD_TIMINGS_NODE) == false,
	     "A single child node with compatible zephyr,panel-timing is required "
	     "under the renesas,rz-lcdc node");
...
#define RENESAS_MIPI_DSI_DEVICE(id)                                                                \
	...
	static struct mipi_dsi_renesas_rz_config rz_config_##id = {                                \
		DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME(dphy, DT_DRV_INST(id)),                         \
		DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME(link, DT_DRV_INST(id)),                         \
	...
	static mipi_dsi_cfg_t g_mipi_dsi##id##_cfg = {                                             \
		...
		.eotp_enable = 1,                                                                  \
		.sync_pulse = 0,                                                                   \
		.vertical_sync_polarity = DT_PROP(LCD_TIMINGS_NODE, vsync_active) !=               \
					  DISPLAY_SIGNAL_POLARITY_HIACTIVE,                        \
		.horizontal_sync_polarity = DT_PROP(LCD_TIMINGS_NODE, hsync_active) !=             \
					    DISPLAY_SIGNAL_POLARITY_HIACTIVE,                      \
