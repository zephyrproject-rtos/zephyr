...
static inline int z_vrfy_kscan_config(const struct device *dev, kscan_callback_t callback_isr)
{
	K_OOPS(K_SYSCALL_DRIVER_KSCAN(dev, config));
	K_OOPS(K_SYSCALL_VERIFY_MSG(callback_isr == 0, "callback cannot be set from user mode"));
