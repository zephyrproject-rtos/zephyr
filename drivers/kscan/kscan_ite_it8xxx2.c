...
#define MAX_MATRIX_KEY_COLS		CONFIG_KSCAN_ITE_IT8XXX2_COLUMN_SIZE
#define MAX_MATRIX_KEY_ROWS		CONFIG_KSCAN_ITE_IT8XXX2_ROW_SIZE
...
#define DRV_REG(dev)	(struct kbs_reg *)(DRV_CONFIG(dev)->base)

static void drive_keyboard_column(const struct device *dev, int col)
{
	struct kbs_reg *const kbs_reg = DRV_REG(dev);
	int mask;
	...
	kbs_reg->KBS_KSOL = (uint8_t) (mask & 0xff);
	kbs_reg->KBS_KSOH1 = (uint8_t) ((mask >> 8) & 0xff);
#if (CONFIG_KSCAN_ITE_IT8XXX2_COLUMN_SIZE == 18)
	kbs_reg->KBS_KSOH2 = (uint8_t) ((mask >> 16) & 0xff);
#endif
}
...
static int kscan_it8xxx2_init(const struct device *dev)
{
	...
	kbs_reg->KBS_KSOCTRL = IT8XXX2_KBS_KSOPU;

#if (CONFIG_KSCAN_ITE_IT8XXX2_COLUMN_SIZE == 18)
	/* Bit[0] = 1: Enable the open-drain mode of KSO[17:0] pins */
	kbs_reg->KBS_KSOCTRL |= IT8XXX2_KBS_KSOOD;
	...
	kbs_reg->KBS_KSICTRL = IT8XXX2_KBS_KSIPU;

	/* KSO[17:0] pins output low */
	kbs_reg->KBS_KSOL = 0x00;
	kbs_reg->KBS_KSOH1 = 0x00;
#if (CONFIG_KSCAN_ITE_IT8XXX2_COLUMN_SIZE == 18)
	kbs_reg->KBS_KSOH2 = 0x00;
#endif
