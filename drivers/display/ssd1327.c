...
typedef int (*ssd13127_write_bus_cmd_fn)(const struct device *dev, const uint8_t cmd,
					 const uint8_t *data, size_t len);
typedef int (*ssd13127_write_pixels_fn)(const struct device *dev, const uint8_t *buf,
					uint32_t pixel_count,
					const struct display_buffer_descriptor *desc);

struct ssd1327_config {
	struct i2c_dt_spec i2c;
	ssd13127_write_bus_cmd_fn write_cmd;
	ssd13127_write_pixels_fn write_pixels;
...
{
	const struct ssd1327_config *config = dev->config;
	static uint8_t buf[SSD1327_MAXIMUM_CMD_LENGTH];
				     ...
				     const struct display_buffer_descriptor *desc)
{
	...
	while (pixel_count > total) {
		...
		for (; i / 2 < config->conversion_buf_size && pixel_count > total + i; i += 2) {
			config->conversion_buf[i / 2] = buf[total + i] >> 4;
			config->conversion_buf[i / 2] |= (buf[total + i + 1] >> 4) << 4;
		}
...
#define SSD1327_CONV_BUFFER_SIZE(node_id)                                                          \
	DT_PROP(node_id, width) * CONFIG_SSD1327_CONV_BUFFER_LINES / 2
