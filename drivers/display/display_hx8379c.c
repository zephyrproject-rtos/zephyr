...
static int hx8379c_configure(const struct device *dev)
{
	...
	if (ret < 0) {
		LOG_ERR("Panel SETTCON failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register SETGIP_0 failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register SETGIP_1 failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register SETGIP_2 failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel gamma configuration failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register SETVCOM failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register HX8379C_SETBANK(0) failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel color LUT 1 failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register HX8379C_SETBANK(1) failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel color LUT 2 failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel register HX8379C_SETBANK(2) failed (%d)", ret);
		return ret;
	}
	...
	if (ret < 0) {
		LOG_ERR("Panel color LUT 3 failed (%d)", ret);
		return ret;
	}
