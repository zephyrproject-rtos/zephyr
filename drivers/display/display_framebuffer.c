...
{
	const struct display_fb_common_config *cfg = dev->config;
	struct display_fb_common_data *data = dev->data;
	...
	if (desc->buf_size < ((size_t)desc->pitch * desc->height * DISPLAY_FB_BYTES_PER_PIXEL)) {
		return -EINVAL;
	}
