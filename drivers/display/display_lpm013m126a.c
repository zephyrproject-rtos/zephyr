...
LOG_MODULE_REGISTER(lpm013m126a, CONFIG_DISPLAY_LOG_LEVEL);
...
/* Panel properties */
#define LPM_WIDTH       DT_INST_PROP(0, width)
#define LPM_HEIGHT      DT_INST_PROP(0, height)
#define LPM_BPP         3
...
/* Color extraction from RGB888 */
#define RGB888_R_MASK   0x00FF0000
#define RGB888_R_SHIFT  16
#define RGB888_G_MASK   0x0000FF00
#define RGB888_G_SHIFT  8
#define RGB888_B_MASK   0x000000FF
...
/*
 * Convert a 24-bit RGB888 pixel (8 bits per channel)
 * into a compact 3-bit RGB format (1 bit per channel):
 *
 * - Takes the MSB of each R, G, B component to form a
 *   single 3-bit pixel: R(1) | G(1) | B(1).
 * - Useful for displays that only support 1-bit per
 *   channel color depth, reducing memory and bandwidth.
 */

static inline uint8_t rgb888_to_rgb3(uint32_t color)
{
	uint8_t r = (color & RGB888_R_MASK) >> RGB888_R_SHIFT;
	uint8_t g = (color & RGB888_G_MASK) >> RGB888_G_SHIFT;
	uint8_t b = (color & RGB888_B_MASK) >> RGB888_B_SHIFT;
	/* reduce to 1 bit each */
	r >>= 7; g >>= 7; b >>= 7;
	return (r << 2) | (g << 1) | b;
}
...
/* Pack one row of RGB888 pixels into panel format */
static void lpm_pack_row(uint8_t *dst, const uint32_t *src)
{
	int bitpos = 0;
	uint8_t byte = 0;

	for (int x = 0; x < LPM_WIDTH; x++) {
		uint8_t pix = rgb888_to_rgb3(src[x]);
...
static void lpm_vcom_toggle(struct k_timer *timer)
{
	...
	const struct lpm013m126a_config *cfg = dev->config;
...
static int lpm_send_line(const struct device *dev, uint16_t line, const uint8_t *buf, size_t len)
{
	const struct lpm013m126a_config *cfg = dev->config;
	uint8_t cmd = LPM_WRITELINE_CMD;
	uint8_t addr = bitrev8((uint8_t)line);
...
{
	if (x != 0 || desc->width != LPM_WIDTH) {
		LOG_ERR("Only full-width writes supported");
		return -ENOTSUP;
	}
	if ((y + desc->height) > LPM_HEIGHT) {
		LOG_ERR("Buffer out of bounds");
		return -EINVAL;
	}

	const uint32_t *src = buf;
	size_t packed_len = DIV_ROUND_UP(LPM_WIDTH * LPM_BPP, 8);
	uint8_t linebuf[packed_len];

	for (int row = 0; row < desc->height; row++) {
		lpm_pack_row(linebuf, src);
...
static void lpm_get_capabilities(const struct device *dev, struct display_capabilities *caps)
{
	...
	caps->x_resolution = LPM_WIDTH;
	caps->y_resolution = LPM_HEIGHT;
...
{
	return (pf == PIXEL_FORMAT_RGB_888) ? 0 : -ENOTSUP;
}
...
#define LPM013M126A_INIT(inst)							\
	static const struct lpm013m126a_config lpm_cfg_##inst = {		\
