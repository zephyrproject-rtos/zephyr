...
struct ssd16xx_quirks {
	...
	/*
	 * Device specific flag deciding whether to divide pass in the byte address or the bit address
	 * when setting the RAM x position window or counter in SSD16XX_CMD_RAM_XPOS_CTRL or SSD16XX_CMD_RAM_XPOS_CNTR.
	 * Expected to be `true` for the devices the byte address, `false` otherwise.
	 */
...
static int ssd16xx_clear_cntlr_mem(const struct device *dev, uint8_t ram_cmd)
{
	 ...
	 */
	if (config->quirks->x_addr_in_bytes) { 
		panel_h = (config->height / EPD_PANEL_NUMOF_ROWS_PER_PAGE) + (config->height % EPD_PANEL_NUMOF_ROWS_PER_PAGE ? 1 : 0);
	}
