...
#define CO5300_SPI_WRITE_QUAD    0x32
...
#if DT_ANY_INST_ON_BUS_STATUS_OKAY(mspi)
static void co5300_mspi_acquire(const struct device *dev, enum mspi_io_mode io_mode)
{
	const struct co5300_config *config = dev->config;
	struct co5300_data *data = dev->data;
	...
	if (config->sw_multi_periph) {
		while (mspi_dev_config(config->mspi_bus, &config->dev_id,
				       MSPI_DEVICE_CONFIG_ALL, &data->mspi_dev_config)) {
			;
		}
	} else {
		while (mspi_dev_config(config->mspi_bus, &config->dev_id,
	...
	if (data->mspi_dev_config.io_mode != io_mode) {
		data->mspi_dev_config.io_mode = io_mode;
		while (mspi_dev_config(config->mspi_bus, &config->dev_id,
				       MSPI_DEVICE_CONFIG_IO_MODE, &data->mspi_dev_config)) {
			;
		}
	}
}

static void co5300_mspi_release(const struct device *dev)
{
	const struct co5300_config *config = dev->config;
	struct co5300_data *data = dev->data;

	while (mspi_get_channel_status(config->mspi_bus, 0)) {
		;
	}
				    ...
				    uint8_t cmd, const void *tx_buf, uint32_t tx_len)
{
	...
	co5300_mspi_acquire(dev, io_mode);
				...
				const void *tx_buf, uint32_t tx_len)
{
	...
	struct spi_buf_set tx_bufs = {.buffers = tx_data, .count = sizeof(uint16_t)};
	uint8_t opcode[2] = {CO5300_SPI_CMD, 00};
	uint8_t cmd_buf[2] = {cmd, 00};

	tx_data[0].buf = &opcode;
	tx_data[0].len = sizeof(opcode);
	tx_data[1].buf = &cmd_buf;
	tx_data[1].len = sizeof(cmd_buf);
...
static int co5300_reset(const struct device *dev)
{
	...
	if (config->reset_gpios.port != NULL) {
		...
		ret = config->cmd_write(dev, MIPI_DCS_SOFT_RESET, NULL, 0);
		if (ret < 0) {
			LOG_ERR("Could not reset panel (%d)", ret);
			return ret;
	}
	}
...
static int co5300_configure_panel(const struct device *dev)
{
	...
	cmd = 0;
	ret = config->cmd_write(dev, 0x96, &cmd, sizeof(cmd));
	if (ret < 0) {
		return ret;
	}

	ret = config->cmd_write(dev, 0xC9, &cmd, sizeof(cmd));
	if (ret < 0) {
		return ret;
	}
...
#define CO5300_CONFIG_MSPI(node_id)								\
	{											\
		...
		.sw_multi_periph = DT_PROP(DT_INST_BUS(node_id), software_multiperipheral)      \
	}
