				    ...
				    const struct display_buffer_descriptor *desc)
{
	...
	while (line_count > line_total) {
		 ...
		 */
		for (; l * mipi_desc.pitch < config->conversion_buf_size &&
		       line_count > line_total + l;
		     l += 4) {
			int i_lt = line_total + l;

			for (int i = 0; i < desc->width &&
					i + l * mipi_desc.pitch < config->conversion_buf_size;
			     i++) {
				int i_l = i + l * mipi_desc.pitch;
		...
		mipi_desc.buf_size = l * mipi_desc.pitch;
		mipi_desc.width = desc->width;
		mipi_desc.height = l;
...
#define ST75256_CONV_BUFFER_SIZE(node_id)                                                          \
	DIV_ROUND_UP(DT_PROP(node_id, width) * CONFIG_ST75256_CONV_BUFFER_LINES / 4, 4)
