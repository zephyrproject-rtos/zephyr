...
static int st7565r_clear_ram(const struct device *dev)
{
	...
	if (config->width > ST7565R_MAX_WIDTH) {
		LOG_ERR("Configured width %d exceeds max %d", config->width, ST7565R_MAX_WIDTH);
		return -EINVAL;
	}

	if (config->height > ST7565R_MAX_HEIGHT) {
		LOG_ERR("Configured height %d exceeds max %d", config->height, ST7565R_MAX_HEIGHT);
		return -EINVAL;
	}
