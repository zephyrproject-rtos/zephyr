...
static int st7565r_clear_ram(const struct device *dev)
{
	...
	if (config->width > ST7565R_LCDWIDTH) {
		LOG_ERR("Configured width %d exceeds maximum supported (%d)", config->width,
			ST7565R_LCDWIDTH);
		return -EINVAL;
	}

	if (config->height > ST7565R_LCDHEIGHT) {
		LOG_ERR("Configured height %d exceeds maximum supported (%d)", config->height,
			ST7565R_LCDHEIGHT);
		return -EINVAL;
	}

	uint8_t zero_buf[ST7565R_LCDWIDTH] = {0};
	uint8_t col_cmds[2] = {ST7565R_SET_HIGHER_COL_ADDRESS_CMD,
			       ST7565R_SET_LOWER_COL_ADDRESS_CMD};
	const uint8_t num_pages = config->height / 8;
	...
	for (uint8_t page = 0; page < num_pages; page++) {
		...
		/* Reset column address to 0 for each new page. */
		if (st7565r_write_bus(dev, col_cmds, sizeof(col_cmds), true) != 0) {
			LOG_ERR("Failed to set column address");
			return -EIO;
		}

		/* Write config->width zero bytes to clear the page's columns. */
