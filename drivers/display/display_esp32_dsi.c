...
/* The DSI host is the controller's only child, and the panels hang off it, so
 * the panels are reached one level below rather than as siblings.
 */
#define DISPLAY_ESP32_DSI_PANEL(inst)                                                              \
	DT_FOREACH_CHILD_STATUS_OKAY(DT_NODELABEL(mipi_dsi), DISPLAY_ESP32_DSI_PANEL_OF)
