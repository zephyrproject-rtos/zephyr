...
/* ST730x controllers have a evil data format
...
struct st730x_config {
	...
	uint8_t hpm_gate_waveform[10];
	uint8_t lpm_gate_waveform[8];
...
static inline int st730x_set_hardware_config(const struct device *dev)
{
	...
	err = mipi_dbi_command_write(config->mipi_dev, &config->dbi_config,
				     ST730X_HPM_GATE_WAVEFORM, config->hpm_gate_waveform, 10);
	if (err < 0) {
		return err;
	}

	err = mipi_dbi_command_write(config->mipi_dev, &config->dbi_config,
				     ST730X_LPM_GATE_WAVEFORM, config->lpm_gate_waveform, 8);
	if (err < 0) {
		return err;
	}
...
{
	const struct st730x_config *config = dev->config;
	uint32_t vertical_offset = desc->width / 8;
	uint32_t i = 0;
	uint32_t ipos, ipos_zeroed;
	uint32_t max_lines = (config->conversion_buf_size / (desc->width / 8)) & ~0x1;
	...
	for (; (offset + i) < desc->height && i < max_lines; i += 2) {
		ipos = (offset + i) * vertical_offset;
		ipos_zeroed = i * vertical_offset / 2;
		for (uint32_t j = 0; j < desc->width / 8 ; j++) {
			config->conversion_buf[(ipos_zeroed + j) * 2 + 1] =
				...
				| (buf[ipos + j] & 0x10) << 3;
			config->conversion_buf[(ipos_zeroed + j) * 2+0] =
			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	uint8_t x_start = config->specifics->column_offset + (config->start_column + x) / 12;
	uint8_t x_end = config->specifics->column_offset
			+ (config->start_column + x + desc->width) / 12 - 1;
	uint8_t x_position[] = {x_start, x_end};
	uint8_t y_position[] = {y / 2, (y + desc->height) / 2 - 1};
	...
	while (desc->height > processed) {
		...
		mipi_desc.buf_size = i * desc->width / 8;
