					...
					struct display_capabilities *capabilities)
{
	...
	capabilities->x_resolution =
		data->hltdc.LayerCfg[0].WindowX1 - data->hltdc.LayerCfg[0].WindowX0;
	capabilities->y_resolution =
		data->hltdc.LayerCfg[0].WindowY1 - data->hltdc.LayerCfg[0].WindowY0;
...
static int stm32_ltdc_write(const struct device *dev, const uint16_t x, const uint16_t y,
			    const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	if ((x == 0) && (y == 0) && (desc->width == config->width) &&
	    (desc->height == config->height) && (desc->pitch == desc->width)) {
...
static int stm32_ltdc_read(const struct device *dev, const uint16_t x, const uint16_t y,
...
static int stm32_ltdc_init(const struct device *dev)
{
	...
	LL_RCC_PLLSAI_ConfigDomain_LTDC(LL_RCC_PLLSOURCE_HSE, LL_RCC_PLLSAIM_DIV_8, 192,
					LL_RCC_PLLSAIR_DIV_4, LL_RCC_PLLSAIDIVR_DIV_8);
	...
	LL_RCC_PLLSAI_Disable();
	LL_RCC_PLLSAI_ConfigDomain_LTDC(LL_RCC_PLLSOURCE_HSE, LL_RCC_PLLM_DIV_25, 384,
					LL_RCC_PLLSAIR_DIV_5, LL_RCC_PLLSAIDIVR_DIV_8);
...
static int stm32_ltdc_pm_action(const struct device *dev, enum pm_device_action action)
{
	int err;
