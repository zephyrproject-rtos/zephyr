				 ...
				 const enum display_orientation orientation)
{
	...
	case DISPLAY_ORIENTATION_NORMAL:
		return 0;
	default:
		LOG_ERR("Display panel orientation not supported");
		return -ENOTSUP;
	}
}
...
static void ramfb_get_capabilities(const struct device *dev, struct display_capabilities *caps)
{
	...
	caps->x_resolution = config->common.width;
	caps->y_resolution = config->common.height;
...
static int ramfb_init(const struct device *dev)
{
	...
	req_size = (size_t)cfg->common.width * cfg->common.height * DISPLAY_FB_BYTES_PER_PIXEL;
	...
	rc = qemu_fwcfg_find_file(fwcfg, "etc/ramfb", &select, &size);
	if (rc != 0) {
	...
	settings.stride = sys_cpu_to_be32(cfg->common.width * DISPLAY_FB_BYTES_PER_PIXEL);
...
#define RAMFB_INIT(inst)                                                                           \
	...
	static const struct ramfb_config ramfb_cfg_##inst = {                                      \
		.common = DISPLAY_FB_COMMON_CFG_FROM_DT_INST(inst),                                \
