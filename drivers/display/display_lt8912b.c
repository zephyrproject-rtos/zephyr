...
#define LT8912B_CHIP_ID_0 0x12
#define LT8912B_CHIP_ID_1 0xb2

/* Consecutive polls that have to see a video stream before re-synchronizing */
 ...
 * logic is reset while the stream is present, so poll the bridge's MIPI
 * input detection and re-synchronize whenever a video stream or a sink
 * (re)appears.
 */
static void lt8912b_sync_work_fn(struct k_work *work)
{
	struct k_work_delayable *dwork = k_work_delayable_from_work(work);
	struct lt8912b_data *data = CONTAINER_OF(dwork, struct lt8912b_data, sync_work);
	const struct lt8912b_config *config = data->dev->config;
	uint8_t h_low;
	uint8_t h_high;
	uint8_t hpd;
	bool video_present;
	...
	ret = i2c_reg_read_byte_dt(&config->i2c_main, 0x9c, &h_low);
	ret |= i2c_reg_read_byte_dt(&config->i2c_main, 0x9d, &h_high);
	ret |= i2c_reg_read_byte_dt(&config->i2c_main, 0xc1, &hpd);
	if (ret < 0) {
		LOG_WRN("Could not read the bridge status (%d)", ret);
		goto reschedule;
	}
	video_present = !(h_low == 0xff && h_high == 0xff);
	sink_present = (hpd & 0x80) == 0x80;

	if (sink_present && !data->sink_present && data->output_on) {
		/* Sink (re)connected while streaming: re-lock the DDS */
		ret = lt8912b_mipi_rx_logic_reset(config);
		if (ret < 0) {
			LOG_WRN("MIPI RX re-synchronization failed (%d)", ret);
		} else {
			LOG_INF("HDMI sink connected, MIPI RX re-synchronized");
		}
	}
	data->sink_present = sink_present;

	if (video_present && !data->output_on) {
		if (++data->stable_polls >= LT8912B_ENABLE_DEBOUNCE_POLLS) {
			ret = lt8912b_mipi_rx_logic_reset(config);
			if (ret < 0) {
				LOG_WRN("MIPI RX re-synchronization failed (%d)", ret);
			} else {
...
static int lt8912b_init(const struct device *dev)
{
	...
	struct lt8912b_data *data = dev->data;
	...
	if (id[0] != LT8912B_CHIP_ID_0 || id[1] != LT8912B_CHIP_ID_1) {
		LOG_ERR("Unexpected chip ID 0x%02x%02x", id[0], id[1]);
		return -ENODEV;
	}
	...
	/* The DDS derives its clock from the stream, so the data lanes have
	 * to stay in high speed through the blanking periods and the clock
	 * lane has to run continuously.
	 */
	mdev.mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_BURST |
			  MIPI_DSI_MODE_HS_BLANKING;
	...
	data->dev = dev;
	k_work_init_delayable(&data->sync_work, lt8912b_sync_work_fn);
	k_work_schedule(&data->sync_work, K_MSEC(200));
...
#define LT8912B_DEFINE(id)                                                                         \
	static struct lt8912b_data lt8912b_data_##id;                                              \
	...
	DEVICE_DT_INST_DEFINE(id, &lt8912b_init, NULL, &lt8912b_data_##id, &lt8912b_config_##id,   \
