...
#define SSD1306_VCOM_DESEL_LEVEL_CONV(level, span, shift) ((((level - SSD1306_VCOM_DESEL_LEVEL_MIN)\
	 * span) / (SSD1306_VCOM_DESEL_LEVEL_MAX - SSD1306_VCOM_DESEL_LEVEL_MIN)) << shift)

#define SSD1306_CLOCK_DIV_RATIO		0x0
#define SSD1306_CLOCK_FREQUENCY		0x8
#define SSD1306_PANEL_VCOM_DESEL_LEVEL(span, shift)	SSD1306_VCOM_DESEL_LEVEL_CONV( \
							      SSD1306_VCOM_DESEL_LEVEL, span, shift)
...
static inline int ssd1306_set_timing_setting(const struct device *dev)
{
	...
	uint8_t cmd_buf[] = {SSD1306_SET_CLOCK_DIV_RATIO,
			     ...
			     config->ssd1309_compatible ? SSD1306_PANEL_VCOM_DESEL_LEVEL(0x0F, 2) :
				SSD1306_PANEL_VCOM_DESEL_LEVEL(0x03, 3)};
