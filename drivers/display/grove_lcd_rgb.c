...
void glcd_print(struct device *port, char *data, uint32_t size)
{
	const struct glcd_driver * const rom = (const struct glcd_driver *)
						port->fixed->config_info;
	struct glcd_data *dev = port->fixed->driver_data;
...
{
	const struct glcd_driver * const rom = port->fixed->config_info;
	struct glcd_data *dev = port->fixed->driver_data;
	uint8_t data[] = { 0, 0 };
...
{
	struct glcd_data * const dev = port->fixed->driver_data;
