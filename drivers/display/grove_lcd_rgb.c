 ...
 *  PRIVATE FUNCTIONS
 *******************************************/
static void rgb_reg_set(const struct device * const i2c, u8_t addr, u8_t dta)
{
...
void glcd_cursor_pos_set(const struct device *port, u8_t col, u8_t row)
{
	const struct glcd_driver * const rom = (const struct glcd_driver *)
						port->config_info;
	struct glcd_data *dev = port->driver_data;
...
void glcd_display_state_set(const struct device *port, u8_t opt)
{
	const struct glcd_driver * const rom = (const struct glcd_driver *)
						port->config_info;
	struct glcd_data *dev = port->driver_data;
...
u8_t glcd_display_state_get(const struct device *port)
{
	struct glcd_data *dev = port->driver_data;
...
void glcd_input_state_set(const struct device *port, u8_t opt)
{
...
u8_t glcd_input_state_get(const struct device *port)
{
	struct glcd_data *dev = port->driver_data;
...
void glcd_color_select(const struct device *port, u8_t color)
{
	if (color > 3) {
...
void glcd_color_set(const struct device *port, u8_t r, u8_t g, u8_t b)
{
...
void glcd_function_set(const struct device *port, u8_t opt)
{
	const struct glcd_driver * const rom = port->config_info;
	struct glcd_data *dev = port->driver_data;
	u8_t data[] = { 0, 0 };
...
u8_t glcd_function_get(const struct device *port)
{
	struct glcd_data *dev = port->driver_data;
