...
static int axs15231b_get_pixelfmt(const struct device *dev)
{
	...
	if (config->dbi_config.mode == MIPI_DBI_MODE_8080_BUS_8_BIT) {
		 ...
		 * is enabled
		 */
		if (((bool)(config->madctl & AXS15231B_MADCTL_BGR)) != config->rgb_is_inverted) {
	...
	static const struct axs15231b_config axs15231b_config_##n = {                              \
		...
		.dbi_config =                                                                      \
			{                                                                          \
				...
				.color_coding = DT_INST_STRING_UPPER_TOKEN_OR(                     \
					n, color_coding, MIPI_DBI_MODE_RGB565),                    \
			},                                                                         \
		...
		.te_delay = DT_INST_PROP(n, te_delay),                                             \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, &axs15231b_init, NULL, NULL, &axs15231b_config_##n, POST_KERNEL,  \
			      CONFIG_DISPLAY_INIT_PRIORITY, &axs15231b_api);
