...
#define DT_DRV_COMPAT axstechnology_axs15231b
...
/* MADCTL register bit fields */
#define AXS15231B_MADCTL_MY     BIT(7) /* Row address order: bottom to top */
#define AXS15231B_MADCTL_MX     BIT(6) /* Column address order: right to left */
#define AXS15231B_MADCTL_MV     BIT(5) /* Row/column exchange (swap X and Y) */
#define AXS15231B_MADCTL_ML     BIT(4) /* Vertical refresh order */
#define AXS15231B_MADCTL_BGR    BIT(3) /* Sets BGR color mode */
#define AXS15231B_MADCTL_MH     BIT(2) /* Horizontal refresh order */
...
#define AXS15231B_MADCTL_RGB    (0 << (3))
...
struct axs15231b_config {
	...
	bool h_mirror;      /* Horizontal mirror (MX bit) */
	bool v_mirror;      /* Vertical mirror (MY bit) */
	bool swap_xy;       /* Swap X and Y axes (MV bit) */
	bool red_blue_swap;
};
...
static int lcd_cmd(const struct device *dev, uint8_t cmd, const uint8_t *params, size_t nparams)
{
	const struct axs15231b_config *config = dev->config;

	return mipi_dbi_command_write(config->mipi_dbi, &config->dbi_config, cmd, params, nparams);
}
...
static int axs15231b_init(const struct device *dev)
{
	...
	if (config->red_blue_swap) {
		/* Panel uses native RGB order; no BGR bit needed. */
		data->madctl = AXS15231B_MADCTL_BGR;
		LOG_DBG("MADCTL: RGB order (red_blue_swap enabled)");
	} else {
		/* Panel uses BGR order (most AXS15231B modules). */
		data->madctl &= AXS15231B_MADCTL_RGB;
		LOG_DBG("MADCTL: BGR order (red_blue_swap disabled)");
	}

	if (config->h_mirror) {
		data->madctl |= AXS15231B_MADCTL_MX;
	}

	if (config->v_mirror) {
		data->madctl |= AXS15231B_MADCTL_MY;
	}

	if (config->swap_xy) {
		data->madctl |= AXS15231B_MADCTL_MV;
	}
...
static int axs15231b_set_cursor(const struct device *dev, const uint16_t x, const uint16_t y,
...
static int draw_axs15231b_display(const struct device *dev, const uint16_t x, const uint16_t y,
				  const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	mipi_dbi_command_write(config->mipi_dbi, &config->dbi_config, 0x2C, NULL, 0);
...
#define AXS15231B_INIT(n)                                                                          \
	static const struct axs15231b_config axs15231b_config_##n = {                              \
		...
		.h_mirror = DT_INST_PROP(n, h_mirror),                                             \
		.v_mirror = DT_INST_PROP(n, v_mirror),                                             \
		...
		.te_delay = DT_INST_PROP(n, te_delay),                                             \
                                                                                                   ...
                                                                                                   \
	static struct axs15231b_data axs15231b_data_##n;                                           \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(n, &axs15231b_init, NULL, &axs15231b_data_##n,                       \
			      &axs15231b_config_##n, POST_KERNEL, CONFIG_DISPLAY_INIT_PRIORITY,    \
			      &axs15231b_api);
