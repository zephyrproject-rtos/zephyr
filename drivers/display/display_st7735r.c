				 ...
				 size_t tx_count)
{
	...
	return mipi_dbi_command_write(config->mipi_dev, &config->dbi_config, cmd, tx_data,
				      tx_count);
}
			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	ret = st7735r_transmit_hold(dev, ST7735R_CMD_RAMWR, (void *)write_data_start,
	...
	for (write_cnt = 1U; write_cnt < nbr_of_writes; ++write_cnt) {
		ret = mipi_dbi_write_display(config->mipi_dev, &config->dbi_config,
					     write_data_start, &mipi_desc, fmt);
		if (ret < 0) {
			goto out;
		}
