				    ...
				    const enum display_pixel_format pixel_format)
{
	...
	/* Update the pixel format in the device data after DCS command succeeds. */
...
static int rm67199_init(const struct device *dev)
{
	...
	if (data->pixel_format == MIPI_DSI_PIXFMT_RGB888) {
		rm67199_set_pixel_format(dev, PIXEL_FORMAT_RGB_888);
	} else if (data->pixel_format == MIPI_DSI_PIXFMT_RGB565) {
		rm67199_set_pixel_format(dev, PIXEL_FORMAT_RGB_565);
	} else {
...
static DEVICE_API(display, rm67199_api) = {
...
DT_INST_FOREACH_STATUS_OKAY(RM67199_CONTROLLER)
