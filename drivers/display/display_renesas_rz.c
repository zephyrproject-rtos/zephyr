...
#define DEV_DATA(dev) ((struct display_rz_data *)((dev)->data))
#define DEV_CFG(dev)  ((const struct display_rz_config *)((dev)->config))

struct display_rz_config {
	DEVICE_MMIO_NAMED_ROM(vcpd);
	DEVICE_MMIO_NAMED_ROM(fcpvd);
	DEVICE_MMIO_NAMED_ROM(du);
...
struct display_rz_data {
	DEVICE_MMIO_NAMED_RAM(vcpd);
	DEVICE_MMIO_NAMED_RAM(fcpvd);
	DEVICE_MMIO_NAMED_RAM(du);
			    ...
			    const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	if (x == 0 && y == 0 && desc->height == config->height && desc->width == config->width) {
		...
		for (row = 0; row < desc->height; row++) {
			memcpy(dst, src, desc->width * data->pixel_size);
...
static int display_init(const struct device *dev)
{
	...
	DEVICE_MMIO_NAMED_MAP(dev, vcpd, K_MEM_CACHE_NONE);
	DEVICE_MMIO_NAMED_MAP(dev, fcpvd, K_MEM_CACHE_NONE);
	DEVICE_MMIO_NAMED_MAP(dev, du, K_MEM_CACHE_NONE);
...
#define OUTPUT_FORMAT_PIXEL(n) DT_INST_PROP(n, pixel_format)
...
#define RENESAS_RZ_LCDC_OUTPUT_DE_POLARITY(n) DT_PROP(DT_INST_CHILD(n, display_timings), de_active)

#define RENESAS_RZ_LCDC_OUTPUT_SYNC_EDGE(n)                                                        \
	DT_PROP_OR(DT_INST_CHILD(n, display_timings), syncclk_active, 1)
...
#define RENESAS_RZ_DEVICE_INIT(id)                                                                 \
	...
	static struct display_rz_config rz_config##id = {                                          \
		DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME(vcpd, DT_DRV_INST(id)),                         \
		DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME(fcpvd, DT_DRV_INST(id)),                        \
		DEVICE_MMIO_NAMED_ROM_INIT_BY_NAME(du, DT_DRV_INST(id)),                           \
