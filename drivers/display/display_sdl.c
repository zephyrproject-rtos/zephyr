...
static void sdl_display_write_argb8888(void *disp_buf, const struct display_buffer_descriptor *desc,
				       const void *buf)
{
	__ASSERT((desc->pitch * 4U * desc->height) <= desc->buf_size, "Input buffer to small");
				     ...
				     const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	__ASSERT((desc->pitch * 3U * desc->height) <= desc->buf_size, "Input buffer to small");
				     ...
				     const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	__ASSERT((desc->pitch * 2U * desc->height) <= desc->buf_size, "Input buffer to small");

	for (h_idx = 0U; h_idx < desc->height; ++h_idx) {
		for (w_idx = 0U; w_idx < desc->width; ++w_idx) {
			pix_ptr = (const uint16_t *)buf + ((h_idx * desc->pitch) + w_idx);
				   ...
				   const void *buf, const bool one_is_black)
{
	...
	__ASSERT((desc->height % 8) == 0U, "Input buffer height not aligned per 8 pixels");
	...
	for (tile_idx = 0U; tile_idx < desc->height / 8U; ++tile_idx) {
		for (w_idx = 0U; w_idx < desc->width; ++w_idx) {
			byte_ptr = (const uint8_t *)buf + ((tile_idx * desc->pitch) + w_idx);
...
static int sdl_display_write(const struct device *dev, const uint16_t x, const uint16_t y,
			     const struct display_buffer_descriptor *desc, const void *buf)
{
	const struct sdl_display_config *config = dev->config;
	struct sdl_display_data *disp_data = dev->data;

	LOG_DBG("Writing %dx%d (w,h) bitmap @ %dx%d (x,y)", desc->width, desc->height, x, y);
	...
	if (desc->width > desc->pitch || x + desc->width > config->width ||
			    ...
			    const struct display_buffer_descriptor *desc, void *buf)
{
	...
	LOG_DBG("Reading %dx%d (w,h) bitmap @ %dx%d (x,y)", desc->width, desc->height, x, y);
...
static void sdl_display_get_capabilities(const struct device *dev,
	...
	static struct args_struct_t sdl_display_options[] = {
		{.option = "display_zoom_pct",
		 ...
		 .descript = "Display zoom percentage (100 == 1:1 scale), "
			     "by default " STRINGIFY(CONFIG_SDL_DISPLAY_ZOOM_PCT) " = CONFIG_SDL_DISPLAY_ZOOM_PCT"},
		 ARG_TABLE_ENDMARKER};
