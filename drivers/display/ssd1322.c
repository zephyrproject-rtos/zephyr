			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	const uint8_t *buf_iter;
	static uint8_t conversion_buf[CONFIG_SSD1322_BUFFER_SIZE];
	...
	while (pixel_count > 0) {
		...
		while (in_idx < pixel_count && seg_idx / 2 < sizeof(conversion_buf)) {
			in_idx = seg_idx / config->segments_per_pixel;
			color = (buf_iter[in_idx / 8] & BIT(in_idx % 8)) ? 0xF : 0;
			conversion_buf[seg_idx / 2] =
				(seg_idx % 2 == 0) ? color
						   : ((color << 4) | conversion_buf[seg_idx / 2]);
			seg_idx++;
		}

		ret = ssd1322_write_data(dev, conversion_buf, seg_idx / 2);
		if (ret < 0) {
			return ret;
		}
