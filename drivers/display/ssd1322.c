			       ...
			       uint32_t pixel_count)
{
	...
	/* 255 grey levels when 2 ram pixel per physical pixel */
	if (config->segments_per_pixel == 2) {
		for (; i < config->conversion_buf_size && pixel_count > cur_offset + i; i += 1) {
			config->conversion_buf[i] = buf[cur_offset + i];
		}
	} else {
			 ...
			 const struct display_buffer_descriptor *desc, const void *buf)
{
	...
	if (data->current_pixel_format == PIXEL_FORMAT_L_8) {
		...
		return ssd1322_write_pixels_MONO01(dev, buf, desc->width * desc->height / 8, desc);
	}
}
...
#if CONFIG_SSD1322_DEFAULT_GREYSCALE
#define SSD1322_DATA(node_id)                                                                      \
...
#define SSD1322_CONV_BUFFER_SIZE(node_id)                                                          \
	DT_PROP(node_id, width) * CONFIG_SSD1322_CONV_BUFFER_LINES / 2

#define SSD1322_DEFINE(node_id)                                                                    \
	SSD1322_DATA(node_id)                                                                      \
