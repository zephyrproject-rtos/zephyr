...
static int psi5_nxp_s32_start(const struct device *dev, uint8_t hw_channel)
{
	const struct psi5_nxp_s32_config *config = dev->config;
	struct psi5_nxp_s32_data *data = dev->data;
	struct psi5_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];
	int err;

	if (!(config->channel_mask & BIT(hw_channel))) {
		return -EINVAL;
	}
	...
	err = Psi5_Ip_SetChannelSync(config->ctrl_inst, hw_channel, true);

	if (err) {
		LOG_ERR("Failed to start PSI5 %d channel %d", config->ctrl_inst, hw_channel);
		return -EIO;
	}
...
static int psi5_nxp_s32_stop(const struct device *dev, uint8_t hw_channel)
{
	const struct psi5_nxp_s32_config *config = dev->config;
	struct psi5_nxp_s32_data *data = dev->data;
	struct psi5_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];
	...
	err = Psi5_Ip_SetChannelSync(config->ctrl_inst, hw_channel, false);

	if (err) {
		LOG_ERR("Failed to stop PSI5 %d channel %d", config->ctrl_inst, hw_channel);
		return -EIO;
	}
...
static int psi5_nxp_s32_send(const struct device *dev, uint8_t hw_channel, uint64_t psi5_data,
			     k_timeout_t timeout, psi5_tx_callback_t callback, void *user_data)
{
	...
	err = Psi5_Ip_Transmit(config->ctrl_inst, hw_channel, psi5_data);
	if (err) {
...
static int psi5_nxp_s32_add_rx_callback(const struct device *dev, uint8_t hw_channel,
...
#define PSI5_NXP_S32_CHANNEL_CALLBACK(node_id)                                                     \
	...
	{                                                                                          \
		...
		uint8_t hw_channel = DT_REG_ADDR(node_id);                                         \
		struct psi5_nxp_s32_data *data = dev->data;                                        \
		struct psi5_nxp_s32_channel_data *channel_data = &data->channel_data[hw_channel];  \
		...
		if (event.Psi5_DriverReadyToTransmit) {                                            \
			if (tx_callback->callback) {                                               \
				tx_callback->callback(dev, hw_channel, PSI5_STATE_TX_READY,        \
		...
		if (event.Psi5_TxDataOverwrite) {                                                  \
			if (tx_callback->callback) {                                               \
				tx_callback->callback(dev, hw_channel, PSI5_STATE_TX_OVERWRITE,    \
						      tx_callback->user_data);                     \
			}                                                                          \
			k_sem_give(&channel_data->tx_sem);                                         \
		}                                                                                  \
		if (event.Psi5_Psi5MessageReceived) {                                              \
			Psi5_Ip_GetPsi5Frame(config->ctrl_inst, hw_channel, &ip_frame);            \
                                                                                                   \
			...
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, hw_channel, &rx_callback->frame,        \
		...
		if (event.Psi5_Psi5MessageOverwrite) {                                             \
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, hw_channel, NULL,                       \
		...
		if (event.Psi5_Psi5MessageErrorsPresent) {                                         \
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, hw_channel, NULL, PSI5_STATE_MSG_ERR,   \
						      rx_callback->user_data);                     \
			}                                                                          \
		}                                                                                  \
		if (event.Psi5_SmcMessageReceived) {                                               \
			Psi5_Ip_GetSmcFrame(config->ctrl_inst, hw_channel, &ip_smc_frame);         \
                                                                                                   \
			if (ip_smc_frame.C) {                                                      \
