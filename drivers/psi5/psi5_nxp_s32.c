...
struct psi5_nxp_s32_rx_powertrain_frame_callback {
	psi5_rx_powertrain_frame_callback_t callback;
	struct psi5_frame frame;
	void *user_data;
};
...
struct psi5_nxp_s32_channel_data {
	...
	struct psi5_nxp_s32_rx_powertrain_frame_callback powertrain_callback;
	struct k_sem tx_sem;
	struct k_mutex lock;
};
					...
					void *user_data)
{
	...
	channel_data->powertrain_callback.callback = powertrain_callback;
	channel_data->powertrain_callback.user_data = user_data;
...
#define PSI5_NXP_S32_CHANNEL_CALLBACK(node_id)                                                     \
	...
	{                                                                                          \
		...
		struct psi5_nxp_s32_rx_powertrain_frame_callback *powertrain_callback =            \
			&channel_data->powertrain_callback;                                        \
                                                                                                   \
		if (event.Psi5_DriverReadyToTransmit) {                                            \
			...
			Psi5_Ip_GetPsi5Frame(config->ctrl_inst, channel, &ip_frame);               \
                                                                                                   \
			powertrain_callback->frame.type = PSI5_POWERTRAIN_FRAME;                   \
			powertrain_callback->frame.powertrain.data = ip_frame.DATA_REGION;         \
			powertrain_callback->frame.timestamp = ip_frame.TIME_STAMP;                \
			powertrain_callback->frame.crc = ip_frame.CRC;                             \
			powertrain_callback->frame.slot_number = ip_frame.SLOT_COUNTER;            \
                                                                                                   \
			if (powertrain_callback->callback) {                                       \
				powertrain_callback->callback(                                     \
					dev, channel, &powertrain_callback->frame,                 \
					!!(ip_frame.C | ip_frame.F | ip_frame.EM | ip_frame.E |    \
					   ip_frame.T)                                             \
						? PSI5_RX_ERR_POWERTRAIN_FRAME                     \
						: PSI5_RX_POWERTRAIN_FRAME,                        \
					powertrain_callback->user_data);                           \
			}                                                                          \
