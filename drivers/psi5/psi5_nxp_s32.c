...
static int psi5_nxp_s32_start(const struct device *dev, uint8_t channel)
{
	...
	/* Enable channel */
	config->base->CHANNEL[channel].CH_PCCR |= PSI5_CH_PCCR_PSI5_CH_EN_MASK;

	/* Enable the channel target counter for synchronous transmision */
	if (channel_data->rx_callback.callback == NULL) {
			     ...
			     k_timeout_t timeout, psi5_tx_callback_t callback, void *user_data)
{
	...
	if (channel_data->rx_callback.callback != NULL) {
		LOG_ERR("PSI5 %d channel %d is configured as a receiver,"
			"please remove the RX callback to switch to transmiter",
