			     ...
			     k_timeout_t timeout, psi5_tx_callback_t callback, void *user_data)
{
	...
	if (Psi5_Ip_Transmit(config->ctrl_inst, channel, psi5_data)) {
	...
	{                                                                                          \
		...
		if (event.Psi5_DriverReadyToTransmit) {                                            \
			...
			Psi5_Ip_GetPsi5Frame(config->ctrl_inst, channel, &ip_frame);               \
                                                                                                   \
			rx_callback->frame.data = ip_frame.DATA_REGION;                            \
			rx_callback->frame.timestamp = ip_frame.TIME_STAMP;                        \
			rx_callback->frame.crc = ip_frame.CRC;                                     \
			rx_callback->frame.slot_number = ip_frame.SLOT_COUNTER;                    \
                                                                                                   \
			...
			Psi5_Ip_GetSmcFrame(config->ctrl_inst, channel, &ip_smc_frame);            \
                                                                                                   \
			if (ip_smc_frame.C) {                                                      \
				rx_callback->frame.smc_frame.id = ip_smc_frame.ID;                 \
				rx_callback->frame.smc_frame.data =                                \
					FIELD_PREP(GENMASK(15, 12), (ip_smc_frame.IDDATA)) |       \
					FIELD_PREP(GENMASK(11, 0), (ip_smc_frame.DATA));           \
			} else {                                                                   \
				rx_callback->frame.smc_frame.id =                                  \
					FIELD_PREP(GENMASK(7, 4), (ip_smc_frame.ID)) |             \
					FIELD_PREP(GENMASK(3, 0), (ip_smc_frame.IDDATA));          \
				rx_callback->frame.smc_frame.data = ip_smc_frame.DATA;             \
			}                                                                          \
			rx_callback->frame.crc = ip_smc_frame.CRC;                                 \
			rx_callback->frame.slot_number = ip_smc_frame.SLOT_NO;                     \
                                                                                                   \
			if (rx_callback->callback) {                                               \
				rx_callback->callback(dev, channel, &rx_callback->frame,           \
