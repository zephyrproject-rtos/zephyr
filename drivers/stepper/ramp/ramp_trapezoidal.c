...
static uint64_t trapezpoidal_calculate_start_interval(uint32_t acceleration)
{
	...
	/* Using the formula: t = sqrt(2 * d / a) */
	/* where d = 1 step, a = acceleration */
	/* source https://github.com/taitpthomas/AVR446/blob/master/code/IAR/speed_cntr.c */
	/* step_intvl_ns = (NSEC_PER_SEC * isqrt( 20000000000 / acceleration) * 676) / 100000000;*/
	uint64_t step_interval_in_ns = isqrt(20000000000 / acceleration) * 6760;
						   ...
						   enum stepper_run_mode run_mode)
{
	...
	case STEPPER_RAMP_STATE_ACCELERATION:
		...
		LOG_DBG("Accelerating: %llu", new_step_interval_in_ns);
		break;
...
static void trapezoidal_reset_ramp_data(const struct stepper_ramp_config *config,
					struct stepper_ramp_common *ramp_common_data,
					const bool is_stepper_dir_changed,
					const bool is_stepper_moving, const uint32_t steps_to_move)
{
...
const struct stepper_ramp_api trapezoidal_ramp_api = {
	.reset_ramp_runtime_data = trapezoidal_reset_ramp_data,
