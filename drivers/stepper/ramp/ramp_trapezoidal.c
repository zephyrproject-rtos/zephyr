					 ...
					 const uint32_t total_steps_to_move)
{
	...
	LOG_DBG("Distance Profile: acceleration=%d const_speed=%d deceleration=%d for steps=%d",
						   ...
						   enum stepper_run_mode run_mode)
{
	...
	case STEPPER_RAMP_STATE_PRE_DECELERATION:
		if ((current_step_interval_in_ns >
			...
			LOG_DBG("Step Interval in ns: %llu", start_interval);
						...
						const uint32_t steps_to_move)
{
	...
	if (is_dir_changed_while_in_motion(
		    is_stepper_moving,
		    ramp_common_data->ramp_runtime_data.is_stepper_dir_changed)) {
	...
	case STEPPER_RAMP_STATE_ACCELERATION:
		if (!is_stepper_moving) {
			ramp_common_data->ramp_runtime_data
				.ramp_stop_step_interval_threshold_in_ns =
				trapezpoidal_calculate_start_interval(ramp_profile->acceleration);
			data->ramp_actual_position = 0;
			data->ramp_target_position = steps_to_move;
		} else {
