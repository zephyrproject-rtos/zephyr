...
static uint64_t prepare_move(struct stepper_ramp_base *ramp, const uint32_t step_count)
{
	...
	LOG_DBG("Parameters: current_interval=%llu run_interval=%llu step_count=%u "
		...
		data->current_interval, profile->run_interval, step_count,
	...
	const uint32_t accel_lim =
		avr446_acceleration_steps_needed(profile->run_interval, profile->acceleration_rate);

	/* steps needed to decelerate from the requested velocity to zero */
	const uint32_t decel_lim =
		avr446_acceleration_steps_needed(profile->run_interval, profile->deceleration_rate);
