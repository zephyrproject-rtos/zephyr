...
DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_is_moving, const struct device *, uint8_t, bool *);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_move_by, const struct device *, uint8_t, int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_set_microstep_interval, const struct device *, uint8_t,
		       uint64_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_set_reference_position, const struct device *, uint8_t,
		       int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_get_actual_position, const struct device *, uint8_t,
		       int32_t *);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_move_to, const struct device *, uint8_t, int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_run, const struct device *, uint8_t,
		       enum stepper_direction);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_stop, const struct device *, uint8_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_set_event_callback, const struct device *, uint8_t,
		       stepper_event_callback_t, void *);

static int fake_stepper_set_reference_position_delegate(const struct device *dev,
							uint8_t stepper_idx, const int32_t pos)
{
...
static int fake_stepper_get_actual_position_delegate(const struct device *dev, uint8_t stepper_idx,
						     int32_t *pos)
{
	ARG_UNUSED(stepper_idx);
	struct fake_stepper_controller_data *data = dev->data;
...
#ifdef CONFIG_ZTEST
static void fake_stepper_controller_reset_rule_before(const struct ztest_unit_test *test,
						      void *fixture)
{
	ARG_UNUSED(test);
	ARG_UNUSED(fixture);
...
ZTEST_RULE(fake_stepper_controller_reset_rule, fake_stepper_controller_reset_rule_before, NULL);
#endif /* CONFIG_ZTEST */
