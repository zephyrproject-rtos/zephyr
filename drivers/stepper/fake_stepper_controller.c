...
struct fake_stepper_motor_data {
...
DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_is_moving, const struct device *, bool *);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_move_by, const struct device *, int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_set_microstep_interval, const struct device *,
		       uint64_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_set_reference_position, const struct device *,
		       int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_get_actual_position, const struct device *,
		       int32_t *);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_move_to, const struct device *, int32_t);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_run, const struct device *,
		       enum stepper_motor_direction);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_stop, const struct device *);

DEFINE_FAKE_VALUE_FUNC(int, fake_stepper_motor_set_event_callback, const struct device *,
...
static int fake_stepper_motor_set_reference_position_delegate(const struct device *dev,
							      const int32_t pos)
{
	struct fake_stepper_motor_data *data = dev->data;
...
static int fake_stepper_motor_get_actual_position_delegate(const struct device *dev, int32_t *pos)
{
	struct fake_stepper_motor_data *data = dev->data;
...
static void fake_stepper_reset_rule_before(const struct ztest_unit_test *test, void *fixture)
{
	...
	RESET_FAKE(fake_stepper_motor_move_by);
	RESET_FAKE(fake_stepper_motor_is_moving);
	RESET_FAKE(fake_stepper_motor_set_microstep_interval);
	RESET_FAKE(fake_stepper_motor_set_reference_position);
	RESET_FAKE(fake_stepper_motor_get_actual_position);
	RESET_FAKE(fake_stepper_motor_move_to);
	RESET_FAKE(fake_stepper_motor_run);
	RESET_FAKE(fake_stepper_motor_stop);
	...
	fake_stepper_motor_set_reference_position_fake.custom_fake =
		fake_stepper_motor_set_reference_position_delegate;
	fake_stepper_motor_get_actual_position_fake.custom_fake =
		fake_stepper_motor_get_actual_position_delegate;
}
...
static DEVICE_API(stepper_motor, fake_stepper_motor_api) = {
	.move_by = fake_stepper_motor_move_by,
	.is_moving = fake_stepper_motor_is_moving,
	.set_microstep_interval = fake_stepper_motor_set_microstep_interval,
	.set_reference_position = fake_stepper_motor_set_reference_position,
	.get_actual_position = fake_stepper_motor_get_actual_position,
	.move_to = fake_stepper_motor_move_to,
	.run = fake_stepper_motor_run,
	.stop = fake_stepper_motor_stop,
	.set_event_callback = fake_stepper_motor_set_event_callback,
};
...
#define FAKE_STEPPER_MOTOR_INIT(inst)                                                              \
                                                                                                   \
	static struct fake_stepper_motor_data fake_stepper_motor_data_##inst;                      \
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, fake_stepper_init, NULL, &fake_stepper_motor_data_##inst,      \
			      NULL, POST_KERNEL, CONFIG_STEPPER_INIT_PRIORITY,                     \
			      &fake_stepper_motor_api);

#define DT_DRV_COMPAT zephyr_fake_stepper
DT_INST_FOREACH_STATUS_OKAY(FAKE_STEPPER_DRIVER_INIT)
#undef DT_DRV_COMPAT

#define DT_DRV_COMPAT zephyr_fake_stepper_motor
DT_INST_FOREACH_STATUS_OKAY(FAKE_STEPPER_MOTOR_INIT)
#undef DT_DRV_COMPAT
