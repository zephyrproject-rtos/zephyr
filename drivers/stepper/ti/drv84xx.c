...
/* Enable and wake up times of the drv84xx stepper controller family. Only after they have passed
...
static int drv84xx_enable(const struct device *dev, bool enable)
{
	int ret;
	const struct drv84xx_config *config = dev->config;
	struct drv84xx_data *data = dev->data;
