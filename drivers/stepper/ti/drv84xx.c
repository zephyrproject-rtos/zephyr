...
struct drv84xx_data {
	...
	enum stepper_amp_micro_step_resolution ustep_res;
	struct gpio_callback fault_cb_data;
	stepper_amp_event_cb_t fault_cb;
...
static int drv84xx_set_fault_cb(const struct device *dev, stepper_amp_event_cb_t fault_cb,
				void *user_data)
{
	struct drv84xx_data *data = dev->data;
				      ...
				      enum stepper_amp_micro_step_resolution micro_step_res)
{
	...
	case STEPPER_AMP_MICRO_STEP_16:
		...
		m1_value = 1;
		break;
	case STEPPER_AMP_MICRO_STEP_32:
		m0_value = 0;
		m1_value = 2;
		break;
	case STEPPER_AMP_MICRO_STEP_64:
		m0_value = 2;
		m1_value = 3;
		break;
	case STEPPER_AMP_MICRO_STEP_128:
		m0_value = 2;
		m1_value = 2;
		break;
	case STEPPER_AMP_MICRO_STEP_256:
		m0_value = 1;
		m1_value = 2;
		break;
	default:
...
void fault_event(const struct device *dev, struct gpio_callback *cb, uint32_t pins)
{
	...
	if (data->fault_cb != NULL) {
		data->fault_cb(data->dev, STEPPER_AMP_EVENT_FAULT_DETECTED,
...
static DEVICE_API(stepper_amp, drv84xx_stepper_amp_api) = {
