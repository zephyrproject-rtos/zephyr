...
static int tmc2130_stepper_enable(const struct device *dev)
{
	...
	if (config->en_pin.port == NULL) {
		...
		LOG_DBG("%s: The device is always enabled.", dev->name);
		return 0;
	}
...
static int tmc2130_stepper_move_to(const struct device *dev, int32_t target)
{
	struct tmc2130_data *data = dev->data;
...
static int tmc2130_stepper_move_by(const struct device *dev, int32_t steps)
{
	struct tmc2130_data *data = dev->data;
...
static int tmc2130_stepper_run(const struct device *dev, enum stepper_direction direction)
{
	struct tmc2130_data *data = dev->data;
...
static int tmc2130_stepper_init(const struct device *dev)
{
	...
	uint32_t reg_combined[][2] = {
		...
		{TMC2130_IHOLD_IRUN,
		 TMC2130_IHOLD_IRUN_INIT(config->iholddelay, config->irun, config->ihold)},
