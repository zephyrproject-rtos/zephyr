...
static void log_stallguard(struct tmc50xx_stepper_data *stepper_data, const uint32_t drv_status)
{
	...
	err = tmc50xx_read(stepper_config->controller, TMC50XX_XACTUAL(stepper_config->index),
			   &xactual);
	if (err != 0) {
	...
	LOG_DBG("%s xactual: %d | sg  result: %d status: %d", stepper_data->stepper->name, xactual, sg_result, sg_status);
}
