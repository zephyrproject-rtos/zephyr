...
#define DT_DRV_COMPAT adi_tmc50xx_stepper_amp
...
struct tmc50xx_stepper_amp_data {
	stepper_amp_event_cb_t drv_event_cb;
	void *drv_event_cb_user_data;
};

struct tmc50xx_stepper_amp_config {
	const uint8_t index;
...
void tmc50xx_stepper_amp_trigger_cb(const struct device *dev, const enum stepper_amp_event event)
{
	...
	struct tmc50xx_stepper_amp_data *data = dev->data;
...
static int tmc50xx_stepper_amp_set_event_callback(const struct device *stepper,
						  stepper_amp_event_cb_t callback, void *user_data)
{
	struct tmc50xx_stepper_amp_data *data = stepper->data;
...
static int tmc50xx_stepper_amp_enable(const struct device *dev)
{
	LOG_DBG("Enabling Stepper motor controller %s", dev->name);
	const struct tmc50xx_stepper_amp_config *config = dev->config;
	uint32_t reg_value;
	int err;
...
static int tmc50xx_stepper_amp_disable(const struct device *dev)
{
...
static int tmc50xx_stepper_amp_set_micro_step_res(const struct device *dev,
...
static int tmc50xx_stepper_amp_get_micro_step_res(const struct device *dev,
						  enum stepper_amp_micro_step_resolution *res)
{
	const struct tmc50xx_stepper_amp_config *config = dev->config;
	uint32_t reg_value;
...
static int tmc50xx_stepper_amp_init(const struct device *dev)
{
	const struct tmc50xx_stepper_amp_config *config = dev->config;
	int err;
	...
	err = tmc50xx_stepper_amp_set_micro_step_res(dev, config->default_micro_step_res);
	if (err != 0) {
		return -EIO;
	}
...
static DEVICE_API(stepper_amp, tmc50xx_stepper_amp_api) = {
	.enable = tmc50xx_stepper_amp_enable,
	.disable = tmc50xx_stepper_amp_disable,
	.set_micro_step_res = tmc50xx_stepper_amp_set_micro_step_res,
	.get_micro_step_res = tmc50xx_stepper_amp_get_micro_step_res,
	.set_event_cb = tmc50xx_stepper_amp_set_event_callback,
};

#define TMC50XX_STEPPER_AMP_DEFINE(inst)                                                           \
	...
	static const struct tmc50xx_stepper_amp_config tmc50xx_stepper_amp_config_##inst = {       \
	...
	static struct tmc50xx_stepper_amp_data tmc50xx_stepper_amp_data_##inst;                    \
	DEVICE_DT_INST_DEFINE(inst, tmc50xx_stepper_amp_init, NULL,                                \
			      &tmc50xx_stepper_amp_data_##inst,                                    \
			      &tmc50xx_stepper_amp_config_##inst, POST_KERNEL,                     \
			      CONFIG_STEPPER_INIT_PRIORITY, &tmc50xx_stepper_amp_api);

DT_INST_FOREACH_STATUS_OKAY(TMC50XX_STEPPER_AMP_DEFINE)
