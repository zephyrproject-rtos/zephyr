...
static void rampstat_work(const struct device *dev)
{
	...
	if (ramp_stat_values > 0) {
...
#ifdef CONFIG_STEPPER_ADI_TMC50XX_MOTION_CONTROLLER
		case TMC5XXX_STOP_LEFT_EVENT:
			LOG_DBG("RAMPSTAT %s:Left end-stop detected", data->dev->name);
			tmc50xx_motion_controller_trigger_cb(
		...
		case TMC5XXX_STOP_SG_EVENT:
			...
			tmc50xx_stallguard_enable(data->dev, false);
			tmc50xx_driver_trigger_cb(config->stepper_drivers[data->work_index],
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	for (uint8_t i = 0; i < config->num_steppers; i++) {
		data->work_index = motion_controller_index(config->motion_controllers[i]);
		rampstat_work(dev);
	}
}
...
#define TMC50XX_DEFINE(inst)                                                                       \
	...
	static const struct device *tmc50xx_motion_controllers_##inst[] =                          \
		TMC50XX_CHILD_DEVICES_ARRAY(inst, adi_tmc50xx_motion_controller);                  \
	...
	static const struct tmc50xx_config tmc50xx_config_##inst = {                               \
		...
		.num_steppers = ARRAY_SIZE(tmc50xx_stepper_drivers_##inst),                        \
