...
int tmc522x_get_actual_velocity(const struct device *dev, int32_t *velocity)
{
	...
	*velocity = sign_extend(val, TMC522X_24BIT_SIGN_INDEX);
...
int tmc522x_get_actual_acceleration(const struct device *dev, int32_t *accel)
{
	...
	*accel = sign_extend(val, TMC522X_24BIT_SIGN_INDEX);
...
int tmc522x_check_stallguard_status(const struct device *dev)
{
	...
	LOG_DBG("=== StallGuard Status ===");
	LOG_DBG("  Mode: %s", en_pwm ? "StealthChop2 (SG4)" : "SpreadCycle (SG2)");
	LOG_DBG("  TCOOLTHRS: 0x%05X (%s)", tcoolthrs,
		tcoolthrs == TMC522X_STALLGUARD_DISABLED             ? "DISABLED"
		: tcoolthrs >= TMC522X_STALLGUARD_ENABLED_ALL_SPEEDS ? "active at all speeds"
								     : "velocity-dependent");
	LOG_DBG("  SG_STOP: %s", sg_stop ? "enabled" : "disabled");
	LOG_DBG("  STALLGUARD flag: %s", sg_flag ? "ACTIVE" : "inactive");
	LOG_DBG("  CS_ACTUAL: %u", cs_actual);
	...
	if (en_pwm) {
		...
		LOG_DBG("  SG4_THRS: %u", sg4_thrs);
		LOG_DBG("  SG4_RESULT: %u (0=max load, %u=no load)", sg4_result,
			TMC522X_SG4_RESULT_MAX);
		LOG_DBG("  SG4_FILT_EN: %s", sg4_filt ? "on (averaged)" : "off (raw)");
	} else {
		...
		LOG_DBG("  SGT threshold: %d", sgt);
		LOG_DBG("  SG_RESULT: %u (0=max load, %u=no load)", sg_result,
			TMC522X_SG_RESULT_MAX);
		LOG_DBG("  SFILT: %s", (coolconf & TMC522X_COOLCONF_SFILT_MASK) ? "on" : "off");
	}
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	struct tmc522x_data *data =
		CONTAINER_OF(dwork, struct tmc522x_data, rampstat_callback_dwork);
						...
						gpio_port_pins_t pins)
{
	...
	struct tmc522x_data *data = CONTAINER_OF(cb, struct tmc522x_data, diag0_cb);
						...
						gpio_port_pins_t pins)
{
	...
	struct tmc522x_data *data = CONTAINER_OF(cb, struct tmc522x_data, diag1_cb);
...
static int tmc522x_mfd_init(const struct device *dev)
{
	...
	/* Validate the bus is ready once here, rather than on every transfer */
	err = config->bus_io->check(&config->bus);
	if (err < 0) {
