...
static int tmc522x_stepper_drv_enable(const struct device *dev)
{
	...
	/*
	 * Assert the hardware DRVEN pin when a GPIO is wired to it. Boards that
	 * tie DRVEN to VCC_IO leave drven-gpios unset, in which case this is a
	 * no-op.
	 */
	err = tmc522x_drven_enable(config->controller);
	if (err != 0) {
		return err;
	}
...
static int tmc522x_stepper_drv_disable(const struct device *dev)
{
	...
	/* Deassert the hardware DRVEN pin if one is wired (no-op otherwise). */
	err = tmc522x_drven_disable(config->controller);
	if (err != 0) {
		return err;
	}
...
/* Initialization */
static int tmc522x_stepper_driver_init(const struct device *dev)
{
	const struct tmc522x_stepper_drv_config *config = dev->config;
	uint32_t val;
	int err;
...
#define TMC522X_STEPPER_DRV_DEFINE(inst)                                                           \
	...
	DEVICE_DT_INST_DEFINE(inst, tmc522x_stepper_driver_init, NULL,                             \
