...
#define DT_DRV_COMPAT adi_tmc51xx_stepper_amp

struct tmc51xx_stepper_amp_config {
...
struct tmc51xx_stepper_amp_data {
	stepper_amp_event_cb_t drv_event_cb;
...
{
	struct tmc51xx_stepper_amp_data *data = dev->data;
...
static int tmc51xx_stepper_amp_set_event_callback(const struct device *stepper,
						  stepper_amp_event_cb_t callback, void *user_data)
{
	struct tmc51xx_stepper_amp_data *data = stepper->data;
...
static int tmc51xx_stepper_amp_enable(const struct device *dev)
{
	const struct tmc51xx_stepper_amp_config *config = dev->config;
...
static int tmc51xx_stepper_amp_disable(const struct device *dev)
{
...
static int tmc51xx_stepper_amp_set_micro_step_res(const struct device *dev,
					      enum stepper_amp_micro_step_resolution res)
{
	const struct tmc51xx_stepper_amp_config *config = dev->config;
	const struct device *controller = config->controller;
	uint32_t reg_value;
	...
	reg_value |= ((MICRO_STEP_RES_INDEX(STEPPER_AMP_MICRO_STEP_256) - LOG2(res))
...
static int tmc51xx_stepper_amp_get_micro_step_res(const struct device *dev,
					      enum stepper_amp_micro_step_resolution *res)
{
	const struct tmc51xx_stepper_amp_config *config = dev->config;
	const struct device *controller = config->controller;
	...
	*res = (1 << (MICRO_STEP_RES_INDEX(STEPPER_AMP_MICRO_STEP_256) - reg_value));
	LOG_DBG("Stepper motor controller %s get micro step resolution: %d", dev->name, *res);
	return 0;
}

static int tmc51xx_stepper_amp_init(const struct device *dev)
{
	const struct tmc51xx_stepper_amp_config *config = dev->config;
	const struct device *controller = config->controller;
	int err;
	...
	err = tmc51xx_stepper_amp_set_micro_step_res(dev, config->default_micro_step_res);
	if (err != 0) {
		return -EIO;
	}
...
static DEVICE_API(stepper_amp, tmc51xx_stepper_amp_api) = {
	.enable = tmc51xx_stepper_amp_enable,
	.disable = tmc51xx_stepper_amp_disable,
	.set_micro_step_res = tmc51xx_stepper_amp_set_micro_step_res,
	.get_micro_step_res = tmc51xx_stepper_amp_get_micro_step_res,
	.set_event_cb = tmc51xx_stepper_amp_set_event_callback,
};

#define TMC51XX_STEPPER_AMP_DEFINE(inst)                                                           \
	...
	static const struct tmc51xx_stepper_amp_config tmc51xx_stepper_amp_config_##inst = {       \
	...
	static struct tmc51xx_stepper_amp_data tmc51xx_stepper_amp_data_##inst;                    \
	DEVICE_DT_INST_DEFINE(inst, tmc51xx_stepper_amp_init, NULL,                                \
			      &tmc51xx_stepper_amp_data_##inst,                                    \
			      &tmc51xx_stepper_amp_config_##inst, POST_KERNEL,                     \
			      CONFIG_STEPPER_INIT_PRIORITY, &tmc51xx_stepper_amp_api);

DT_INST_FOREACH_STATUS_OKAY(TMC51XX_STEPPER_AMP_DEFINE)
