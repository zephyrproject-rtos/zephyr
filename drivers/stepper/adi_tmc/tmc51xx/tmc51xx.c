					      ...
					      stepper_event_callback_t callback, void *user_data)
{
	...
	if ((config->comm_type == TMC_COMM_SPI) && config->diag0_gpio.port) {
		...
		k_work_init_delayable(&stepper_data->rampstat_callback_dwork,
			rampstat_work_handler);
		...
		gpio_init_callback(&stepper_data->diag0_cb, tmc51xx_diag0_gpio_callback_handler,
				   BIT(config->diag0_gpio.pin));

		err = gpio_add_callback(config->diag0_gpio.port, &stepper_data->diag0_cb);
		if (err < 0) {
...
static void rampstat_work_handler(struct k_work *work)
{
	...
	struct tmc51xx_stepper_data *stepper_data =
		CONTAINER_OF(dwork, struct tmc51xx_stepper_data, rampstat_callback_dwork);
							       ...
							       gpio_port_pins_t pins)
{
	...
	struct tmc51xx_stepper_data *stepper_data =
		CONTAINER_OF(cb, struct tmc51xx_stepper_data, diag0_cb);
				   ...
				   const int32_t micro_steps)
{
	...
	struct tmc51xx_stepper_data *data = config->steppers[0]->data;
	int err;
			       ...
			       const enum stepper_direction direction)
{
	...
	const struct tmc51xx_config *tmc51xx_config = controller->config;
	const struct tmc51xx_stepper_config *config = tmc51xx_config->steppers[0]->config;
	struct tmc51xx_stepper_data *data = tmc51xx_config->steppers[0]->data;
	int err;
