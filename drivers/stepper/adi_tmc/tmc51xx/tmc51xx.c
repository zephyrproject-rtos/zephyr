...
static int tmc51xx_stepper_get_actual_position(const struct device *dev,
					       int32_t *position)
{
	int err;
...
static int tmc51xx_stepper_move_to(const struct device *controller, const int32_t micro_steps)
{
...
static int tmc51xx_stepper_move_by(const struct device *controller, const int32_t micro_steps)
{
	int err;
	int32_t position;

	err = tmc51xx_stepper_get_actual_position(controller, &position);
	if (err != 0) {
		return -EIO;
	}
	...
	return tmc51xx_stepper_move_to(controller, target_position);
}

static int tmc51xx_stepper_run(const struct device *controller,
