...
#define DT_DRV_COMPAT adi_tmc51xx_stepper_motor

struct tmc51xx_stepper_motor_config {
...
struct tmc51xx_stepper_motor_data {
...
void tmc51xx_stepper_motor_trigger_cb(const struct device *dev,
				      const enum stepper_motor_event event)
{
	struct tmc51xx_stepper_motor_data *data = dev->data;
...
static int read_vactual(const struct device *dev, int32_t *actual_velocity)
{
	...
	const struct tmc51xx_stepper_motor_config *config = dev->config;
...
int tmc51xx_stepper_motor_set_max_velocity(const struct device *dev, uint32_t velocity)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
...
int tmc51xx_stepper_motor_stallguard_enable(const struct device *dev, const bool enable)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	const struct device *controller = config->controller;
	uint32_t reg_value;
...
static void stallguard_work_handler(struct k_work *work)
{
	...
	struct tmc51xx_stepper_motor_data const *data =
		CONTAINER_OF(dwork, struct tmc51xx_stepper_motor_data, stallguard_dwork);
	const struct device *dev = data->dev;
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	int err;

	err = tmc51xx_stepper_motor_stallguard_enable(dev, true);
	if (err == -EAGAIN) {
...
static int tmc51xx_stepper_motor_set_event_callback(const struct device *dev,
						    stepper_motor_event_callback_t callback,
						    void *user_data)
{
	struct tmc51xx_stepper_motor_data *data = dev->data;
...
static int tmc51xx_stepper_motor_is_moving(const struct device *dev, bool *is_moving)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	const struct device *controller = config->controller;
	uint32_t reg_value;
...
static int tmc51xx_stepper_motor_set_reference_position(const struct device *dev,
...
static int tmc51xx_stepper_motor_get_actual_position(const struct device *dev, int32_t *position)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	int err;
...
static int tmc51xx_stepper_motor_move_to(const struct device *dev, const int32_t micro_steps)
{
	...
	if (config->is_sg_enabled) {
		tmc51xx_stepper_motor_stallguard_enable(dev, false);
	}
...
static int tmc51xx_stepper_motor_move_by(const struct device *dev, const int32_t micro_steps)
{
	int err;
	int32_t position;

	err = tmc51xx_stepper_motor_get_actual_position(dev, &position);
	if (err != 0) {
		return -EIO;
	}
	...
	return tmc51xx_stepper_motor_move_to(dev, target_position);
}

static int tmc51xx_stepper_motor_run(const struct device *dev,
				     const enum stepper_motor_direction direction)
{
	...
	if (config->is_sg_enabled) {
		err = tmc51xx_stepper_motor_stallguard_enable(dev, false);
		if (err != 0) {
			return -EIO;
		}
	}
	...
	switch (direction) {
	case STEPPER_MOTOR_DIRECTION_POSITIVE:
	...
	case STEPPER_MOTOR_DIRECTION_NEGATIVE:
		err = tmc51xx_write(controller, TMC51XX_RAMPMODE,
...
int tmc51xx_stepper_motor_set_ramp(const struct device *dev,
...
static int tmc51xx_stepper_motor_init(const struct device *dev)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	struct tmc51xx_stepper_motor_data *data = dev->data;
	const struct device *controller = config->controller;
...
#ifdef CONFIG_STEPPER_ADI_TMC51XX_RAMP_GEN
	err = tmc51xx_stepper_motor_set_ramp(dev, &config->default_ramp_config);
	if (err != 0) {
		return -EIO;
	}
#endif
...
static int tmc51xx_stepper_motor_stop(const struct device *dev)
{
	const struct tmc51xx_stepper_motor_config *config = dev->config;
	const struct device *controller = config->controller;
	int err;
...
static DEVICE_API(stepper_motor, tmc51xx_stepper_motor_api) = {
	.is_moving = tmc51xx_stepper_motor_is_moving,
	.move_by = tmc51xx_stepper_motor_move_by,
	.set_reference_position = tmc51xx_stepper_motor_set_reference_position,
	.get_actual_position = tmc51xx_stepper_motor_get_actual_position,
	.move_to = tmc51xx_stepper_motor_move_to,
	.run = tmc51xx_stepper_motor_run,
	.stop = tmc51xx_stepper_motor_stop,
	.set_event_callback = tmc51xx_stepper_motor_set_event_callback,
};

#define TMC51XX_STEPPER_MOTOR_DEFINE(inst)                                                         \
	IF_ENABLED(CONFIG_STEPPER_ADI_TMC51XX_RAMP_GEN,	(CHECK_RAMP_DT_DATA(inst)));               \
	static const struct tmc51xx_stepper_motor_config tmc51xx_stepper_motor_cfg_##inst = {      \
	...
	static struct tmc51xx_stepper_motor_data tmc51xx_stepper_motor_data_##inst;                \
	DEVICE_DT_INST_DEFINE(inst, tmc51xx_stepper_motor_init, NULL,                              \
			      &tmc51xx_stepper_motor_data_##inst,                                  \
			      &tmc51xx_stepper_motor_cfg_##inst, POST_KERNEL,                      \
			      CONFIG_STEPPER_INIT_PRIORITY, &tmc51xx_stepper_motor_api);

DT_INST_FOREACH_STATUS_OKAY(TMC51XX_STEPPER_MOTOR_DEFINE)
