...
#define IRX_DEFAULT_PW_END 4500
...
#define IRX_NEC_DATA_THRESHOLD	1700
/* 4.5 ms, matches NEC spec*/
#define IRX_NEC_END_THRESHOLD	4500
/* 1.3 ms ? */
#define IRX_RC5_DATA_THRESHOLD	1300
/* 2.5 ms */
#define IRX_RC5_END_THRESHOLD	2500
...
static int bflb_irx_configure(struct device const *dev)
{
	...
	if (cfg->protocol == NEC) {
		data_threshold = IRX_US_TO_PW(data->clock_rate, IRX_NEC_DATA_THRESHOLD);
		end_threshold = IRX_US_TO_PW(data->clock_rate, IRX_NEC_END_THRESHOLD);
	} else if (cfg->protocol == RC5) {
		data_threshold = IRX_US_TO_PW(data->clock_rate, IRX_RC5_DATA_THRESHOLD);
		end_threshold = IRX_US_TO_PW(data->clock_rate, IRX_RC5_END_THRESHOLD);
	} else {
	...
	tmp = sys_read32(cfg->reg + IRRX_PW_CONFIG_OFFSET);
...
static void bflb_irx_isr_handle_prot(const struct device *dev)
{
	...
	if (input_report(dev, INPUT_EV_DEVICE, INPUT_MSC_SCAN, data, true, K_FOREVER) < 0) {
		LOG_ERR("Message failed to be enqueued");
	}
	if (data_count > 32) {
		data = sys_read32(cfg->reg + IRRX_DATA_WORD1_OFFSET);
		if (data != 0) {
			if (input_report(dev, INPUT_EV_DEVICE, INPUT_MSC_SCAN,
				data, true, K_FOREVER) < 0) {
				LOG_ERR("Message failed to be enqueued");
			}
		}
	}
...
static void bflb_irx_isr_handle_pw(const struct device *dev)
{
	...
	k_timepoint_t end_timeout =
		sys_timepoint_calc(K_MSEC(IRX_WAIT_TIMEOUT_MS));
	...
	while ((sys_read32(cfg->reg + IR_FIFO_CONFIG_1_OFFSET) & IR_RX_FIFO_CNT_MASK
		...
		if (sys_read32(cfg->reg + IR_FIFO_CONFIG_1_OFFSET) & IR_RX_FIFO_CNT_MASK) {
			x = sys_read32(cfg->reg + IR_FIFO_RDATA_OFFSET);
			if (input_report(dev, INPUT_EV_DEVICE, INPUT_MSC_SCAN,
				IRX_PW_TO_US(data->clock_rate, x), true, K_FOREVER) < 0) {
				LOG_ERR("Message failed to be enqueued");
				break;
			}
		}
...
#define BFLB_IRX_DEFINE(inst)									\
	...
	static struct bflb_irx_config const bflb_irx_config_##inst = {				\
		...
		.pw_end_pulse_width =								\
			DT_INST_PROP_OR(inst, pw_end_pulse_width, IRX_DEFAULT_PW_END),		\
