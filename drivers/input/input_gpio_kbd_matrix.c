...
INPUT_KBD_STRUCT_CHECK(struct gpio_kbd_matrix_config, struct gpio_kbd_matrix_data);
...
static __maybe_unused void gpio_kbd_matrix_idle_poll_handler(const struct device *dev)
{
	...
	if (gpio_kbd_matrix_read_row(dev) == 0) {
		k_work_reschedule(cfg->idle_poll_dwork, K_USEC(common->poll_period_us));
		return;
	}
...
static bool gpio_kbd_matrix_is_gpio_coherent(const struct gpio_dt_spec *gpio, int gpio_count)
{
	const struct gpio_dt_spec *gpio0 = &gpio[0];

	for (int i = 1; i < gpio_count; i++) {
		if (gpio[i].port != gpio0->port || gpio[i].dt_flags != gpio0->dt_flags ||
...
static int gpio_kbd_matrix_init(const struct device *dev)
{
	...
	for (i = 0; i < common->row_size; i++) {
		...
		gpio_init_callback(gpio_cb, cfg->gpio_cb_handler, BIT(gpio->pin));
	...
	if (cfg->idle_poll_dwork != NULL) {
		k_work_init_delayable(cfg->idle_poll_dwork, cfg->idle_poll_handler);
	}

	data->direct_read = gpio_kbd_matrix_is_gpio_coherent(cfg->row_gpio, common->row_size);

	if (cfg->col_drive_inactive) {
		data->direct_write =
			gpio_kbd_matrix_is_gpio_coherent(cfg->col_gpio, common->col_size);
	}

	LOG_DBG("direct_read: %d direct_write: %d", data->direct_read, data->direct_write);
...
#define INPUT_GPIO_KBD_MATRIX_INIT(n)                                                                                      \
	...
	static const struct gpio_kbd_matrix_config gpio_kbd_matrix_cfg_##n = {                                             \
		...
		.idle_poll_handler = gpio_kbd_matrix_idle_poll_handler_##n,			\
						 ...
						 DT_INST_PROP(n, col_drive_inactive),                                      \
	};                                                                                                                 \
                                                                                                                           \
	...
	DEVICE_DT_INST_DEFINE(n, gpio_kbd_matrix_init, PM_DEVICE_DT_INST_GET(n),                                           \
			      &gpio_kbd_matrix_data_##n, &gpio_kbd_matrix_cfg_##n, POST_KERNEL,                            \
