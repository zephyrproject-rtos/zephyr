...
void ctsu_scan_callback(ctsu_callback_args_t *p_arg)
{
	...
	if (data->work_phase == SCANNING) {
		if (p_arg->event == CTSU_EVENT_SCAN_COMPLETE) {
			/* Scan success, start processing data */
...
#define BUTTON_GET_CONTEXT(node_id)                                                                \
	COND_CODE_1(IS_EQ(DT_ENUM_IDX(node_id, component_type), BUTTON_TYPE),                      \
	({                                                                                         \
...
#define SLIDER_GET_CONTEXT(node_id)                                                                \
	COND_CODE_1(IS_EQ(DT_ENUM_IDX(node_id, component_type), SLIDER_TYPE),                      \
	({                                                                                         \
...
#define WHEEL_GET_CONTEXT(node_id)                                                                 \
	COND_CODE_1(IS_EQ(DT_ENUM_IDX(node_id, component_type), WHEEL_TYPE),                       \
	({                                                                                         \
		.p_elements = (uint8_t[])DT_PROP(node_id, channels_num),                 \
		.config = {                                                                        \
			.p_elem_index = NULL,\
