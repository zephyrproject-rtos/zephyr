...
static void ft6146_process_touch(const struct device *dev, uint8_t status)
{
	...
	/* Read touch data */
	ret = i2c_burst_read_dt(&config->i2c, FT6146_REG_DEVICE_MODE, point_data, sizeof(point_data));
	if (ret < 0) {
		LOG_ERR("Failed to read touch data: %d", ret);
		return;
	}

	event_flag = (point_data[FT6146_REG_P1_XH] >> 6) & 0x03;
	x = ((point_data[FT6146_REG_P1_XH] & 0x0F) << 8) + point_data[FT6146_REG_P1_XL];
	y = ((point_data[FT6146_REG_P1_YH] & 0x0F) << 8) + point_data[FT6146_REG_P1_YL];
