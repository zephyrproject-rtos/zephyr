...
static void datastreamer_init(const struct device *dev);
#endif /* CONFIG_INPUT_MCHP_TOUCH_DATASTREAMER_UNI_DIR */
...
static void conf_touch_per_clk(const struct device *dev)
{
	int retval = 0u;
	const struct mchp_touch_config *config = dev->config;

	if (config->clock_config.gclk_sys != NULL) {
		retval = clock_control_on(config->clock_config.clock_dev,
					  config->clock_config.gclk_sys);
		if ((retval != TOUCH_SUCCESS) && (retval != -EALREADY)) {
			LOG_ERR("GClock Configuration Failure");
		}
	}
	...
	if (config->clock_config.mclk_sys != NULL) {
		...
		if ((retval != TOUCH_SUCCESS) && (retval != -EALREADY)) {
			LOG_ERR("MClock Configuration Failure");
		}
	}
}
...
{
	k_timer_start(&my_timer, K_MSEC(20),
		      K_MSEC(DT_INST_PROP(0, def_touch_measurement_period_ms)));
}

/*
 * Touch sensors config -
 * assign nodes to buttons / wheels / sliders / surfaces / water level / etc
 */
static touch_ret_t touch_sensors_config(const struct device *dev)
{
	uint16_t sensor_nodes = 0u;
	touch_ret_t touch_ret = TOUCH_SUCCESS;

	struct mchp_touch_data *data = (struct mchp_touch_data *)dev->data;

	/* Init acquisition module */
	touch_ret = qtm_ptc_init_acquisition_module(&data->qtlib_acq_set1);
	if (touch_ret != TOUCH_SUCCESS) {
		LOG_ERR("qtm_ptc_init_acquisition_module error: %u", (uint8_t)touch_ret);
	} else {
		touch_ret = qtm_ptc_qtlib_assign_signal_memory(&data->touch_acq_signals_raw[0]);
		if (touch_ret != TOUCH_SUCCESS) {
			LOG_ERR("qtm_ptc_qtlib_assign_signal_memory error: %u", (uint8_t)touch_ret);
		}

		/* Initialize sensor nodes */
		for (sensor_nodes = 0u; sensor_nodes < (uint16_t)DEF_NUM_CHANNELS; sensor_nodes++) {
			touch_ret = qtm_enable_sensor_node(&data->qtlib_acq_set1, sensor_nodes);
			if (touch_ret != TOUCH_SUCCESS) {
				LOG_ERR("qtm_enable_sensor_node[%u] error: %u", sensor_nodes,
					(uint8_t)touch_ret);
			}

			touch_ret = qtm_calibrate_sensor_node(&data->qtlib_acq_set1, sensor_nodes);
			if (touch_ret != TOUCH_SUCCESS) {
				LOG_ERR("qtm_calibrate_sensor_node[%u] error: %u", sensor_nodes,
					(uint8_t)touch_ret);
			}
		}
		...
		for (sensor_nodes = 0u; sensor_nodes < (uint16_t)DEF_NUM_SENSORS; sensor_nodes++) {
			touch_ret =
				qtm_init_sensor_key(&data->qtlib_key_set1, (uint8_t)sensor_nodes,
						    &data->ptc_qtlib_node_stat1[sensor_nodes]);
			if (touch_ret != TOUCH_SUCCESS) {
				LOG_ERR("qtm_init_sensor_key[%u] error: %u", sensor_nodes,
					(uint8_t)touch_ret);
			}
		}
#if defined(CONFIG_INPUT_MCHP_TOUCH_EN_SCROLLER)
		touch_ret = qtm_init_scroller_module(&data->qtm_scroller_control1);
		if (touch_ret != TOUCH_SUCCESS) {
			LOG_ERR("qtm_init_scroller_module error: %u", (uint8_t)touch_ret);
		}
#endif /* CONFIG_INPUT_MCHP_TOUCH_EN_SCROLLER */
	}
...
static void qtm_error_callback(uint8_t error)
{
	...
	LOG_ERR("Error Callback: %d", error);
}

static void touch_init(const struct device *dev)
{
	touch_timer_config();

	/* Configure touch sensors with Application specific settings */
	if (touch_sensors_config(dev) != TOUCH_SUCCESS) {
		LOG_ERR("touch_sensors_config ERROR");
	}
}
...
static void touch_process(void *p1, void *p2, void *p3)
{
	...
	while (1) {
		...
		if (data->touch_postprocess_request == 1u) {
			...
			if (TOUCH_SUCCESS == touch_ret) {
#if defined(CONFIG_INPUT_MCHP_TOUCH_EN_FREQ_AUTO_TUNE)
				touch_ret = qtm_freq_hop_autotune(
					&data->qtm_freq_hop_autotune_control1);
				if (TOUCH_SUCCESS != touch_ret) {
					qtm_error_callback(FREQ_HOP_AUTO_MODULE_ERROR);
				}
#endif /* CONFIG_INPUT_MCHP_TOUCH_EN_FREQ_AUTO_TUNE */
				touch_ret = qtm_key_sensors_process(&data->qtlib_key_set1);
				if (TOUCH_SUCCESS != touch_ret) {
					qtm_error_callback(KEY_MODULE_ERROR);
				}
#if defined(CONFIG_INPUT_MCHP_TOUCH_EN_SCROLLER)
				touch_ret = qtm_scroller_process(&data->qtm_scroller_control1);
				if (TOUCH_SUCCESS != touch_ret) {
					qtm_error_callback(SCROLLER_MODULE_ERROR);
				}
#endif /* CONFIG_INPUT_MCHP_TOUCH_EN_SCROLLER */
			} else {
				qtm_error_callback(ACQ_MODULE_ERROR);
			}
			...
			if ((data->qtlib_key_set1.qtm_touch_key_group_data->qtm_keys_status &
				...
				if ((data->qtlib_key_set1.qtm_touch_key_group_data
					     ->qtm_keys_status &
				    ...
				    QTM_KEY_DETECT) == QTM_KEY_DETECT) {
				data->is_touch = false;
				input_report_key(dev, INPUT_KEY_RESERVED, 0, true, K_FOREVER);
			} else {
				if (data->is_touch == true) {
					 ...
					 */
					data->is_touch = false;
					input_report_key(dev, INPUT_KEY_RESERVED, 0, true,
							 K_FOREVER);
				}
			}
...
{
	const struct mchp_touch_config *config = (const struct mchp_touch_config *)dev->config;
	struct mchp_touch_data *data = (struct mchp_touch_data *)dev->data;
	...
	conf_touch_per_clk(dev);

	if (pinctrl_apply_state(config->pcfg, PINCTRL_STATE_DEFAULT) != TOUCH_SUCCESS) {
		LOG_ERR("Pin Control Failed to apply state");
	}
...
static void datastreamer_init(const struct device *dev)
{
	struct mchp_touch_config *config = (struct mchp_touch_config *)dev->config;

	if (!device_is_ready(config->uart_dev)) {
		LOG_ERR("Failed to open datastreamer port");
	}
}
...
static void datastreamer_output(const struct device *dev)
{
	...
	struct mchp_touch_data *touch_data = (struct mchp_touch_data *)dev->data;
	struct mchp_touch_config *touch_config = (struct mchp_touch_config *)dev->config;
	...
	/* End token */
	datastreamer_transmit(dev, (uint8_t)(~(0x55u)));
}
#endif /* CONFIG_INPUT_MCHP_TOUCH_DATASTREAMER_UNI_DIR */
...
uint16_t get_sensor_node_signal(const struct device *dev, uint16_t sensor_node)
{
	...
	if ((dev != NULL) && (sensor_node < DEF_NUM_CHANNELS)) {
		data = (struct mchp_touch_data *)dev->data;
		signal = data->ptc_qtlib_node_stat1[sensor_node].node_acq_signals;
	}
