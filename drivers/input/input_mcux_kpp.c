/*
 * Copyright 2025, NXP
 *
 * SPDX-License-Identifier: Apache-2.0
 */

#include <zephyr/kernel.h>
#include <zephyr/device.h>
#include <zephyr/input/input.h>
#include <zephyr/logging/log.h>
#include <fsl_kpp.h>
#include <zephyr/drivers/clock_control.h>
#include <zephyr/dt-bindings/clock/imx_ccm.h>
#include <zephyr/drivers/pinctrl.h>
#include <zephyr/sys/util.h>

LOG_MODULE_REGISTER(kpp, CONFIG_INPUT_LOG_LEVEL);

#define DT_DRV_COMPAT nxp_mcux_kpp

#define INPUT_KPP_COLUMNNUM_MAX  KPP_KEYPAD_COLUMNNUM_MAX
#define INPUT_KPP_ROWNUM_MAX     KPP_KEYPAD_ROWNUM_MAX

struct kpp_config {
	KPP_Type *base;
	const struct device *ccm_dev;
	clock_control_subsys_t clk_sub_sys;
	const struct pinctrl_dev_config *pcfg;
};

struct kpp_data {
	uint32_t clock_rate;
	struct k_work work;
	kpp_interrupt_enable_t interrupt_status;
	const struct device *dev;
};

static void get_source_clk_rate(const struct device *dev, uint32_t *clk_rate)
{
	const struct kpp_config *dev_cfg = dev->config;
	const struct device *ccm_dev = dev_cfg->ccm_dev;
	clock_control_subsys_t clk_sub_sys = dev_cfg->clk_sub_sys;

	if (!device_is_ready(ccm_dev)) {
		LOG_ERR("CCM driver is not installed");
		*clk_rate = 0;
		return;
	}

	clock_control_get_rate(ccm_dev, clk_sub_sys, clk_rate);
}

static void input_kpp_data_analyze(const struct device *dev, uint8_t *data)
{
	const struct kpp_config *config = dev->config;
	int row = -1;
	int col = -1;
	int row_mask;

	for (int i = 0; i < INPUT_KPP_COLUMNNUM_MAX; i++) {
		if (data[i] != 0) {
			row_mask = data[i];
			for (int j = 0; j < INPUT_KPP_ROWNUM_MAX; j++) {
				if ((row_mask & BIT(j)) == 0) {
					row = j;
					col = i;
					continue;
				}
			}
		}
	}

	if (row == -1 || col == -1) {
		LOG_DBG("No key signal detected in matrix scan");
	} else {
		input_report_abs(dev, INPUT_ABS_X, row, false, K_FOREVER);
		input_report_abs(dev, INPUT_ABS_Y, col, false, K_FOREVER);
		KPP_SetSynchronizeChain(config->base, kKPP_ClearKeyDepressSyncChain);
		input_report_key(dev, INPUT_BTN_TOUCH, 1, true, K_FOREVER);
	}
}

static void input_kpp_process(const struct device *dev)
{
	const struct kpp_config *config = dev->config;
	struct kpp_data *drv_data = dev->data;

	uint8_t read_keys[INPUT_KPP_COLUMNNUM_MAX];
	uint16_t status = drv_data->interrupt_status;

	if ((status & kKPP_keyDepressInterrupt) != 0) {
		/* Read the key press data */
		KPP_keyPressScanning(config->base, read_keys, drv_data->clock_rate);

		/* Analyze the keypad data */
		input_kpp_data_analyze(dev, read_keys);
	} else if ((status & kKPP_keyReleaseInterrupt) != 0U) {
		input_report_key(dev, INPUT_BTN_TOUCH, 0, true, K_FOREVER);
	} else {
		LOG_ERR("No key press or release detected");
	}
}

static void kpp_work_handler(struct k_work *work)
{
	struct kpp_data *drv_data = CONTAINER_OF(work, struct kpp_data, work);

	input_kpp_process(drv_data->dev);
}

static void kpp_isr(const struct device *dev)
{
	const struct kpp_config *config = dev->config;
	struct kpp_data *drv_data = dev->data;

	uint16_t status = KPP_GetStatusFlag(config->base);

	if ((status & kKPP_keyDepressInterrupt) != 0) {
		if ((config->base->KPSR & KPP_KPSR_KDIE_MASK)) {
			drv_data->interrupt_status = kKPP_keyDepressInterrupt;
			/* Key depress report */
			k_work_submit(&drv_data->work);
		}
		/* Disable interrupts. */
		KPP_DisableInterrupts(config->base, kKPP_keyDepressInterrupt);
		/* Clear status. */
		KPP_ClearStatusFlag(config->base, kKPP_keyDepressInterrupt);
		KPP_EnableInterrupts(config->base, kKPP_keyReleaseInterrupt);
	} else if ((status & kKPP_keyReleaseInterrupt) != 0U) {
		if ((config->base->KPSR & KPP_KPSR_KRIE_MASK)) {
			drv_data->interrupt_status = kKPP_keyReleaseInterrupt;
			/* Key reelase report */
			k_work_submit(&drv_data->work);
		}
		/* Disable interrupts. */
		KPP_DisableInterrupts(config->base, kKPP_keyReleaseInterrupt);
		/* Clear status. */
		KPP_ClearStatusFlag(config->base, kKPP_keyReleaseInterrupt);

		KPP_SetSynchronizeChain(config->base, kKPP_SetKeyReleasesSyncChain);

		/* Enable interrupts. */
		KPP_EnableInterrupts(config->base, kKPP_keyDepressInterrupt);
	} else {
		LOG_ERR("No key press or release detected");
	}
}

static int input_kpp_init(const struct device *dev)
{
	const struct kpp_config *config = dev->config;
	struct kpp_data *drv_data = dev->data;
	kpp_config_t kppConfig;

	if (!device_is_ready(config->ccm_dev)) {
		LOG_ERR("CCM driver is not installed");
		return -ENODEV;
	}

	int ret = pinctrl_apply_state(config->pcfg, PINCTRL_STATE_DEFAULT);

	if (ret < 0) {
		LOG_ERR("Failed to configure pin");
		return ret;
	}

	kppConfig.activeRow = 0xFF;
	kppConfig.activeColumn = 0xFF;
	kppConfig.interrupt = kKPP_keyDepressInterrupt;

	KPP_Init(config->base, &kppConfig);

	get_source_clk_rate(dev, &drv_data->clock_rate);

	drv_data->dev = dev;
	k_work_init(&drv_data->work, kpp_work_handler);

	IRQ_CONNECT(DT_INST_IRQN(0), DT_INST_IRQ(0, priority),
		kpp_isr, DEVICE_DT_INST_GET(0), 0);
	return 0;
}

#define INPUT_KPP_INIT(n)						\
	static struct kpp_data kpp_data_##n;		\
								\
	PINCTRL_DT_INST_DEFINE(n);	\
												\
	static const struct kpp_config kpp_config_##n = {	\
		.base = (KPP_Type *)DT_INST_REG_ADDR(n),		\
		.clk_sub_sys =									\
			(clock_control_subsys_t)DT_INST_CLOCKS_CELL_BY_IDX(n, 0, name),	\
		.ccm_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(n)),	\
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n), \
	};										\
											\
	DEVICE_DT_INST_DEFINE(n,				\
			input_kpp_init,					\
			NULL,							\
			&kpp_data_##n,					\
			&kpp_config_##n,				\
			POST_KERNEL,					\
			CONFIG_INPUT_INIT_PRIORITY,		\
			NULL);

DT_INST_FOREACH_STATUS_OKAY(INPUT_KPP_INIT)
