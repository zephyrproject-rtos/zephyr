...
struct tma525b_touch_point {
	uint16_t touch_type : 8U; /* 0 for standard finger/glove, 1 for proximity. Not used*/
	uint16_t event_id : 8U;   /* Bit 0-4: touch ID, bit 5-6: touch event */
	uint16_t x;
	uint16_t y;
	uint16_t pressure : 8U;       /* Touch intensity. Not used in current driver model. */
	uint16_t major_axis_len_mm : 8U; /* Major axis length. Not used in current driver model. */
	uint16_t minor_axis_len_mm : 8U; /* Minor axis length. Not used in current driver model. */
	/* Angle between panel vertical axis and major axis. Not used in current driver model. */
	uint16_t orientation : 8U;
} __packed;
...
struct tma525b_touch_data {
	...
	uint8_t report_id;
	/* Timestamp in 100us units low byte. Not used in current driver model. */
	uint8_t timestamp_100us_l;
	/* Timestamp in 100us units high byte. Not used in current driver model. */
	uint8_t timestamp_100us_h;
