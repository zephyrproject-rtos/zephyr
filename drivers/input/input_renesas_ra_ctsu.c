...
static int renesas_ra_ctsu_init(const struct device *dev)
{
	...
	ctsu_cfg->make_thread(dev);
	ctsu_cfg->irq_config();

	return 0;
}

#define CTSU_DRV_THREAD_PRIORITY   CONFIG_INPUT_RENESAS_RA_CTSU_DRV_PRIORITY
#define CTSU_DRV_THREAD_STACK_SIZE CONFIG_INPUT_RENESAS_RA_CTSU_DRV_STACK_SIZE
...
#define RENESAS_RA_CTSU_DEFINE(inst)                                                               \
	...
	static const struct renesas_ra_ctsu_cfg renesas_ra_ctsu_cfg##inst = {                      \
		...
		.make_thread = renesas_ra_ctsu_make_thread##inst,                                  \
	};                                                                                         \
                                                                                                   \
