...
static void renesas_ra_ctsu_group_reading_handler(struct k_work *work)
{
	...
	IF_ENABLED(DT_HAS_COMPAT_STATUS_OKAY(renesas_ra_ctsu_button),
		   (renesas_ra_ctsu_group_buttons_read(dev);))

	IF_ENABLED(DT_HAS_COMPAT_STATUS_OKAY(renesas_ra_ctsu_slider),
		   (renesas_ra_ctsu_group_sliders_read(dev);))

	IF_ENABLED(DT_HAS_COMPAT_STATUS_OKAY(renesas_ra_ctsu_wheel),
