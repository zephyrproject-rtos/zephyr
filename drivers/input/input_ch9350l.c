...
K_MSGQ_DEFINE(ch9350l_msgq, sizeof(struct ch9350l_frame), CONFIG_INPUT_CH9350L_FRAME_COUNT, 4);
...
static void ch9350l_kb(const struct device *dev, const uint8_t *values, uint8_t length)
{
	...
	for (size_t j = 0; j < CH9350L_FRAME_VALUE_MAX; j++) {
		...
		for (size_t i = 0; i < length; i++) {
			if (data->last_kb_values[j] == values[i]) {
				found = true;
			}
		}
		if (!found && data->last_kb_values[j] != 0) {
			if (input_report(dev, INPUT_EV_KEY,
				ch9350l_kb_map(dev, data->last_kb_values[j]), 0, true, K_FOREVER)) {
	...
	memcpy(data->last_kb_values, values, length);
	memset(&(data->last_kb_values[length]), 0, CH9350L_FRAME_VALUE_MAX - length);
}
...
static void ch9350l_mouse(const struct device *dev, const uint8_t *values, uint8_t length)
{
	...
	const uint16_t raw_x = *(const uint16_t *)&(values[CH9350L_FRAME_MOUSE_X_BYTE]);
	const uint16_t raw_y = *(const uint16_t *)&(values[CH9350L_FRAME_MOUSE_Y_BYTE]);
	const int16_t x = raw_x & CH9350L_FRAME_MOUSE_RELNEG ?
		-(CH9350L_FRAME_MOUSE_RELMID - (const int16_t)(raw_x & CH9350L_FRAME_MOUSE_RELMID))
		: raw_x & CH9350L_FRAME_MOUSE_RELMID;
	const int16_t y = raw_y & CH9350L_FRAME_MOUSE_RELNEG ?
		-(CH9350L_FRAME_MOUSE_RELMID - (const int16_t)(raw_y & CH9350L_FRAME_MOUSE_RELMID))
		: raw_y & CH9350L_FRAME_MOUSE_RELMID;

	input_report(dev, INPUT_EV_REL, INPUT_REL_X, x, true, K_FOREVER);
	input_report(dev, INPUT_EV_REL, INPUT_REL_Y, y, true, K_FOREVER);

	for (size_t i = 0; i < 8; i++) {
		if (button & (1u << i) && !(data->last_mouse_btns & (1u << i))) {
			if (input_report(dev, INPUT_EV_DEVICE,
				ch9350l_mouse_map(dev, (1u << i)), 1, true, K_FOREVER)) {
				LOG_ERR("Input failed to be enqueued");
			}
		} else if (data->last_mouse_btns & (1u << i) && !(button & (1u << i))) {
			if (input_report(dev, INPUT_EV_DEVICE,
				ch9350l_mouse_map(dev, (1u << i)), 0, true, K_FOREVER)) {
				LOG_ERR("Input failed to be enqueued");
			}
		} else {
...
static void ch9350l_input_work_handler(struct k_work *item)
{
	...
	while (k_msgq_get(&ch9350l_msgq, &frame, K_NO_WAIT) == 0) {
		sum = 0;
...
static int ch9350l_queue_frame(struct ch9350l_data *dev_data, uint8_t *data, size_t size)
{
	...
	ret = k_msgq_put(&ch9350l_msgq, &frame, K_NO_WAIT);
	if (ret < 0) {
...
static uint8_t ch9350l_is_valid_frame(uint8_t *data, size_t size)
{
	...
	/* We dont have the full frame yet */
...
static void ch9350l_input_callback(const struct device *dev_uart, void *user_data)
{
	...
	while (true) {
		...
		for (size_t offset = MAX(data->frame_buffer_size - read - 2, 0);
