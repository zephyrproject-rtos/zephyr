...
struct rts5912_kbd_data {
	...
	/* KSI wake-up interrupt status mask */
	uint8_t ksi_pin_mask;
	/* KSO status mask */
	uint32_t kso_pin_mask;
};
...
{
	const struct rts5912_kbd_config *config = dev->config;
	struct rts5912_kbd_data *data = dev->data;
	KBM_Type *const inst = config->base;
...
static int rts5912_kbd_init(const struct device *dev)
{
	...
	/* KSI mask */
	data->ksi_pin_mask = kso_mask;

	/* KSO mask*/
	data->kso_pin_mask = ksi_mask;
...
static const struct rts5912_kbd_config rts5912_kbd_cfg_0 = {
	...
	.base = (KBM_Type *)DT_INST_REG_ADDR_BY_IDX(0, 0),
	.irq = DT_INST_IRQN(0),
