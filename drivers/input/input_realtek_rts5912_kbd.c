...
static int rts5912_kbd_init(const struct device *dev)
{
	...
	if (!device_is_ready(config->clk_dev)) {
		LOG_ERR("clock kbd device not ready: %d", ret);
		return -ENODEV;
	}
...
static uint32_t input_kbd_matrix_pm_action_suspend(const struct device *dev)
{
	const struct rts5912_kbd_config *config = dev->config;
	const struct input_kbd_matrix_common_config *common = &config->common;
	volatile struct kbm_regs *inst = config->base;
...
static uint32_t input_kbd_matrix_pm_action_resume(const struct device *dev)
{
	const struct rts5912_kbd_config *config = dev->config;
	const struct input_kbd_matrix_common_config *common = &config->common;
	volatile struct kbm_regs *inst = config->base;
...
static int input_kbd_matrix_pm_action_rts5912(const struct device *dev, enum pm_device_action action)
{
	int ret;
	...
	case PM_DEVICE_ACTION_RESUME:
		...
		if (ret != 0) {
			LOG_ERR("kbd rts5912 resume fail: %x", ret);
			return ret;
		}
		ret = input_kbd_matrix_pm_action(dev, action);
		if (ret != 0) {
			LOG_ERR("kbd pm resume fail: %x", ret);
			return ret;
		}
		break;
	case PM_DEVICE_ACTION_SUSPEND:
		...
		if (ret != 0) {
			LOG_ERR("kbd rts5912 suspend fail: %x", ret);
			return ret;
		}
		ret = input_kbd_matrix_pm_action(dev, action);
		if (ret != 0) {
			LOG_ERR("kbd pm suspend fail: %x", ret);
			return ret;
		}
		break;
	default:
