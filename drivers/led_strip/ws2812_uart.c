...
/*
 * Helper macros to get UART frame configuration from the referenced UART's devicetree node.
 */
#define _DT_UART_NODE(inst)          DT_INST_PHANDLE(inst, uart)
#define _DT_UART_DATA_BITS(inst)     DT_PROP_OR(_DT_UART_NODE(inst), data_bits, 8)
/* Only UART_CFG_STOP_BITS_1 or 1 is supported. Other values will fail the BUILD_ASSERT below. */
#define _DT_UART_STOP_BITS(inst)     DT_ENUM_IDX_OR(_DT_UART_NODE(inst), stop_bits, 1)
#define _DT_UART_HAS_PARITY(inst)    (DT_ENUM_IDX(_DT_UART_NODE(inst), parity) != \
				      UART_CFG_PARITY_NONE)
#define _DT_UART_HAS_TX_INVERT(inst) (DT_PROP_OR(_DT_UART_NODE(inst), tx_invert, 0) != 0)

/* The total number of bits for one UART frame transmission (start + data + parity + stop). */
#define UART_FRAME_BITS_FROM_DT(inst) \
	(1 + _DT_UART_DATA_BITS(inst) + _DT_UART_HAS_PARITY(inst) + _DT_UART_STOP_BITS(inst))
...
static inline const struct ws2812_uart_cfg *dev_cfg(const struct device *dev)
{
	return dev->config;
}

static inline struct ws2812_uart_data *dev_data(const struct device *dev)
{
	return dev->data;
}
...
static int ws2812_strip_update_rgb(const struct device *dev, struct led_rgb *pixels,
				   size_t num_pixels)
{
	const struct ws2812_uart_cfg *cfg = dev_cfg(dev);
	struct ws2812_uart_data *data = dev_data(dev);
	const size_t total_symbol_bits =
		num_pixels * cfg->num_colors * BITS_PER_COLOR_CHANNEL * cfg->bits_per_symbol;
	const size_t buf_len = DIV_ROUND_UP(total_symbol_bits, cfg->uart_frame_bits);
	 ...
	 */
	rc = uart_tx(cfg->uart_dev, px_buf, buf_len, SYS_FOREVER_US);
	if (rc) {
		k_mutex_unlock(&data->lock);
		return rc;
	}
...
#define WS2812_UART_BUFSZ(idx)                                                                     \
	DIV_ROUND_UP(WS2812_NUM_COLORS(idx) * BITS_PER_COLOR_CHANNEL *                             \
			     WS2812_UART_BITS_PER_SYMBOL(idx) * DT_INST_PROP(idx, chain_length),   \
		     UART_FRAME_BITS_FROM_DT(idx))

#define _WS2812_UART_CHECK(idx)                                                                    \
	BUILD_ASSERT(!_DT_UART_HAS_PARITY(idx),                                                    \
		     "The UART peripheral must be configured with parity disabled.");              \
	BUILD_ASSERT(_DT_UART_STOP_BITS(idx) == 1,                                                 \
		     "The UART peripheral's stop-bits property must be set to 1.");                \
	BUILD_ASSERT(_DT_UART_HAS_TX_INVERT(idx),                                                  \
