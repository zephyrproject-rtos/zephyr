...
static int ws2812_led_strip_start_dma_put(const struct device *dev)
{
				       ...
				       size_t num_pixels)
{
	...
	if (COND_CODE_1(CONFIG_DMA, (!config->dma_config.dev), (true))) {
		/* This branch executes if
		 *  - CONFIG_DMA is not enabled,
		 *  - CONFIG_DMA is enabled but dma_config
		 *    is not populated (when no tx channel is configured in DT).
		 */
		for (size_t i = 0; i < num_pixels; i++) {
			pio_sm_put_blocking(pio, data->sm,
					    ws2812_led_strip_map_color(dev, &pixels[i]));
		}
		...
		ws2812_led_strip_start_dma_put(dev);
...
#define WS2812_CHILD_INIT(node)                                                                    \
	...
	static struct ws2812_led_strip_data ws2812_led_strip_##node##_data = {                     \
		.dma_data = COND_CODE_1(DMA_ENABLED(node),                                         \
				(&DMA_DATA_NAME(node)), (NULL)),                                   \
	};                                                                                         \
                                                                                                   \
