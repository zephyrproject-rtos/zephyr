...
#define WS2812_BFLB_WO_MAP_COLORS_IMPL(_n, _p, _i)						   \
	IF_ENABLED(IS_EQ(LED_COLOR_ID_WHITE, DT_PROP_BY_IDX(_n, _p, _i)), (.offset_w = _i * 8,))   \
	IF_ENABLED(IS_EQ(LED_COLOR_ID_RED, DT_PROP_BY_IDX(_n, _p, _i)), (.offset_r = _i * 8,))     \
	IF_ENABLED(IS_EQ(LED_COLOR_ID_GREEN, DT_PROP_BY_IDX(_n, _p, _i)), (.offset_g = _i * 8,))   \
	IF_ENABLED(IS_EQ(LED_COLOR_ID_BLUE, DT_PROP_BY_IDX(_n, _p, _i)), (.offset_b = _i * 8,))

#define WS2812_BFLB_WO_MAP_COLORS(_inst, _prop)							\
	DT_INST_FOREACH_PROP_ELEM(_inst, _prop, WS2812_BFLB_WO_MAP_COLORS_IMPL)
...
#define WS2812_BFLB_WO_VALIDATE_COLORS(_inst, _prop)						\
	...
	BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 0)					\
		!= DT_INST_PROP_BY_IDX(_inst, _prop, 1), "Color must appear only once");	\
	BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 0)					\
		!= DT_INST_PROP_BY_IDX(_inst, _prop, 2), "Color must appear only once");	\
	BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 1)					\
		!= DT_INST_PROP_BY_IDX(_inst, _prop, 2), "Color must appear only once");	\
	IF_ENABLED(IS_EQ(DT_INST_PROP_LEN(_inst, _prop), 4), (					\
		BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 0)				\
			!= DT_INST_PROP_BY_IDX(_inst, _prop, 3), "Color must appear only once");\
		BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 1)				\
			!= DT_INST_PROP_BY_IDX(_inst, _prop, 3), "Color must appear only once");\
		BUILD_ASSERT(DT_INST_PROP_BY_IDX(_inst, _prop, 2)				\
			!= DT_INST_PROP_BY_IDX(_inst, _prop, 3), "Color must appear only once");\
		))

#define WS2812_BFLB_WO_MAX_FRAME_BITS	32
...
struct ws2812_bflb_wo_data {
	k_timepoint_t reset_timeout;
	struct k_sem lock;
#ifdef CONFIG_WS2812_STRIP_BFLB_WO_ASYNC
	uint16_t *async_buf;
#endif
};
				     ...
				     size_t num_pixels)
{
	...
	struct ws2812_bflb_wo_data *data = dev->data;
	int ret;
	uint16_t buf[WS2812_BFLB_WO_MAX_FRAME_BITS];
...
static int ws2812_bflb_wo_init(const struct device *dev)
{
	...
	if (!gpio_is_ready_dt(&config->gpio_pin)) {
		LOG_ERR_DEVICE_NOT_READY(config->gpio_pin.port);
		return -ENODEV;
	}
...
#ifdef CONFIG_WS2812_STRIP_BFLB_WO_ASYNC
#define WS2812_BFLB_WO_DEVICE_BUFFER(_s) .async_buf = (uint16_t [_s]) {},
#else
#define WS2812_BFLB_WO_DEVICE_BUFFER(_s)
#endif

#define WS2812_BFLB_WO_DEVICE(n)								\
	BL61X_WO_VALIDATE_INST(n, worldsemi_ws2812_bflb_wo);					\
	static const struct ws2812_bflb_wo_config ws2812_bflb_wo_##n##_cfg = {			\
		...
		.ttotal = DT_INST_PROP(n, time_total),						\
		WS2812_BFLB_WO_MAP_COLORS(n, color_mapping)					\
		.num_colors = DT_INST_PROP_LEN(n, color_mapping),				\
		.length = DT_INST_PROP(n, chain_length),					\
		.reset_delay = DT_INST_PROP(n, reset_delay),					\
	};											\
	static struct ws2812_bflb_wo_data ws2812_bflb_wo_##n##_data = {				\
		WS2812_BFLB_WO_DEVICE_BUFFER(DT_INST_PROP(n, chain_length)			\
			* DT_INST_PROP_LEN(n, color_mapping) * 8)				\
	};											\
	DEVICE_DT_INST_DEFINE(n, ws2812_bflb_wo_init, NULL, &ws2812_bflb_wo_##n##_data,		\
