 ...
 *
 * Each bit is one PWM period (one 16-bit EasyDMA word) in COMMON decoder
 * mode: COUNTERTOP is the bit period (time-total) in 16 MHz ticks, and the
 * compare value (time-t0h / time-t1h) selects '0' vs '1'. A frame is a
 * contiguous word array streamed by EasyDMA in a single burst.
 */
...
/* WS2812 bit period, in ticks (PWM COUNTERTOP). */
#define WS2812_NRF_PWM_TOP(idx) WS2812_NRF_PWM_NS_TO_TICKS(DT_INST_PROP(idx, time_total))

/* Reset/latch words = ceil(reset-delay us / bit-period us), bit period = top / 16 us. */
					...
					k_timeout_t timeout)
{
	...
	/* Returns a task address (or 0), not an error code; nothing to check
	 * here. A burst that never starts shows up as the wait timing out.
	 */
				     ...
				     size_t num_pixels)
{
	...
	/* Stop any in-flight burst before EasyDMA can read frame_seq while we overwrite it. */
	(void)nrfx_pwm_stop(&data->pwm, true);

	words = ws2812_nrf_pwm_encode_frame(cfg, pixels, num_pixels, data->frame_seq);
	...
	/* One word is top ticks at 16 MHz; add 1 ms slack so a long chain doesn't time out. */
	frame_us = (uint32_t)words * cfg->top / 16U;
	rc = ws2812_nrf_pwm_play_blocking(data, words, K_USEC(frame_us + 1000U));
