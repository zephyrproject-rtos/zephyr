 ...
 *
 * Arm SiP services driver ID manager. This is a helper module that
 * supports two features:
 *
 *  (1) Index Pool (aka ID Pool)
 *      Provide allocate and free APIs to manage a set of index numbers.
 *      This will be used by sip_svc driver to manage the client and
 *      transaction index.
 *
 *  (2) ID Mapping Table
 *      A generic mapping table to associate any ID (keyword) to its runtime
 *      parameters. This is used by sip_svc driver to associate the transaction
 *      ID to its corresponding client, callback function and etc.
 */
