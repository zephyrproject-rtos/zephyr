 ...
 */
/* Contains low level helpers for NWP. Each function will run one (and only one)
 * NWP command. This is a first layer of abstraction for the driver.
 *
 * The function tries to expose only the relevant arguments to the upper layers.
 * In some case, several functions may be mapped to the same NWP API. It happens
 * when API has several mode and some argument are only meaningful in some mode
 * (typically, siwx91x_nwp_fw_upgrade_write vs siwx91x_nwp_fw_upgrade_start or
 * siwx91x_nwp_flash_write vs siwx91x_nwp_flash_erase).
 *
 * We prefer to expose NWP agnostic parameters, however, for the complex API,
 * rather than exposing ridiculous number of parameters, we may expose the NWP
...
#include "device/silabs/si91x/wireless/inc/sl_si91x_driver.h"
#include "device/silabs/si91x/wireless/sl_net/inc/sl_net_si91x.h"
...
void siwx91x_nwp_ps_enable(const struct device *dev, sli_wifi_power_save_request_t *params)
{
	...
	/* FIXME: properly integrate with clock_controller */
