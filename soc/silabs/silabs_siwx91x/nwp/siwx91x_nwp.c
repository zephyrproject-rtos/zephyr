...
int siwx91x_nwp_prepare_sleep(const struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(data->cmd_queues); i++) {
		if (data->cmd_queues[i].tx_in_progress) {
			return -EBUSY;
		}
	}
	 ...
	 * (gdb) bt
	 * #0  0x0000142e in siwx91x_nwp_isr (dev=0x8236ee0 <__device_dts_ord_30>) at ...
	 * #1  0x0000043e in _isr_wrapper () at ...
	 * #2  <signal handler called>
	 * #3  __ISB () at ...
	 * #4  arch_cpu_idle () at ..
	 * #5  0x08209ab0 in pm_state_set (state=..., substate_id=...) at ...
	 * #6  0x0820cd48 in pm_system_suspend (kernel_ticks=...) at ..
	 * #7  0x0822c942 in idle (unused1=..., unused2=..., unused3=<...) at ...
	 * #8  0x082066a8 in z_thread_entry (entry=0x822c8d3 <idle>, p1=0x72a8 <_kernel>,...) at ...
				  ...
				  bool hidden_ssid, uint8_t max_num_sta)
{
	...
	/* FIXME: Temporary workaround for NWP firmwares which don't allocate enough memory for WPA2
	 * authentication
	 */
	params->feature_bit_map &= ~SL_SI91X_FEAT_WPS_DISABLE;
...
static int siwx91x_check_nwp_version(const struct device *dev)
{
	...
	LOG_INF("Get NWP firmware version: %X.%d.%d.%d.%d.%d.%d",
		version.rom_id, version.major, version.minor, version.security_version,
		version.patch_num, version.customer_id, version.build_num);
