...
Run after a build::
    ...
    west build -b <board> -t siwx91x_clock_tree

Or directly::

    python3 soc/silabs/silabs_siwx91x/siwx91x_siwx91x_clock_tree_report.py \\
...
EXTERNAL_XTAL_KHZ_PIN_PROP = "silabs,external-xtal-khz-pin"
...
def resolve_zephyr_base(explicit: str | None = None) -> Path:
...
def resolve_edt_pickle(path: str) -> Path:
    ...
    try:
        ...
        print(
            f"error: {path!r} must resolve to a file under the current working directory ({cwd})",
            file=sys.stderr,
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: west build -b siwx917_rb4338a/siwg917/cpu0 -t siwx91x_clock_tree",
        allow_abbrev=False,
    )
    parser.add_argument(
        "--zephyr-base",
    ...
    parser.add_argument(
        "--no-legend", action="store_true", help="Omit the legend printed after the tree"
    )
    return parser.parse_args()


def format_freq_hz(hz: int | None) -> str:
    if hz is None:
        return ""
...
def print_column_header(data_start: int) -> None:
    ...
    rule = f"{'-' * clock_width}{'-' * LAYOUT.col_mux}{'-' * LAYOUT.col_rate}{'-' * LAYOUT.col_div}"
...
def node_frequency(node: edtlib.Node) -> int | None:
    prop = node.props.get("clock-frequency")
    return prop.val if prop is not None else None


def clock_parent(node: edtlib.Node) -> edtlib.Node | None:
...
def clock_divider(node: edtlib.Node) -> int | None:
...
def find_clocks_root(edt: edtlib.EDT) -> edtlib.Node | None:
...
def find_external_xtal_khz_pin(managers: list[edtlib.Node]) -> int | None:
    for manager in managers:
        if manager.matching_compat != "silabs,siwx91x-cmu-aon":
            continue

        prop = manager.props.get(EXTERNAL_XTAL_KHZ_PIN_PROP)
        return prop.val if prop is not None else None
...
def apply_aon_ref_lf_children(nodes: list[TreeNode]) -> None:
    aon_ref_lf = find_tree_node(nodes, "aon_ref_lf_mux")
    if aon_ref_lf is None:
        return

    for clock_name, mux_name, note in XTAL_KHZ_AON_REF_LF_CHILDREN:
        if note == NOTE_HARDLINK:
            aon_ref_lf.children.append(hardlink_node(clock_name, mux_name))
        else:
            aon_ref_lf.children.append(fixed_lf_mux_node(clock_name, mux_name))

    aon_ref_lf.children.sort(key=lambda entry: entry.mux_name.lower())


def build_source_subtree(
    source: edtlib.Node, routes: list[edtlib.Node], external_xtal_khz_pin: int | None
) -> TreeNode:
    node = source_tree_node(source)
    node.children = build_route_subtree(source, routes)

    if node_label(source) == "xtal_khz":
        if external_xtal_khz_pin is not None:
            node.note = f"external input on UULP GPIO {external_xtal_khz_pin}"
        apply_xtal_khz_supplemental_nodes(node)

    return node


def find_tree_node(nodes: list[TreeNode], mux_prefix: str) -> TreeNode | None:
    for node in nodes:
...
def build_static_clock_gates() -> TreeNode:
    folder = TreeNode(mux_name="static_clock_gates", note="external reference, not under CPU")
...
def main() -> None:
    ...
    external_xtal_khz_pin = find_external_xtal_khz_pin(managers)
    ...
    if clocks_root is not None:
        ...
        for source in sources:
            top_level.append(build_source_subtree(source, routes, external_xtal_khz_pin))

    apply_aon_ref_lf_children(top_level)
