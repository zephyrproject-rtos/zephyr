...
#define SIWX91X_CMU_AON_NODE  DT_NODELABEL(cmu_aon)
#define SIWX91X_XTAL_KHZ_NODE DT_NODELABEL(xtal_khz)
...
static int cmd_clocks(const struct shell *sh, size_t argc, char **argv)
{
...
#if DT_NODE_HAS_PROP(SIWX91X_CMU_AON_NODE, silabs_external_xtal_khz_pin)
	shell_print(sh, "");
	shell_print(sh, "XTAL_KHZ source: external UULP GPIO %u (%u Hz)",
		    DT_PROP(SIWX91X_CMU_AON_NODE, silabs_external_xtal_khz_pin),
		    DT_PROP(SIWX91X_XTAL_KHZ_NODE, clock_frequency));
#endif
	...
	shell_print(sh, "");
	shell_print(sh, "Clock wiring diagram: west build -t siwx91x_clock_tree");
