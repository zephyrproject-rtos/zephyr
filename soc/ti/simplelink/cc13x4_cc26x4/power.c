...
void pm_state_set(enum pm_state state, uint8_t substate_id)
{
	...
	case PM_STATE_SUSPEND_TO_IDLE:
		...
		do {
			mode_vims = VIMSModeGet(VIMS_BASE);
		} while (mode_vims == VIMS_MODE_CHANGING);
...
static int unlatch_pins(void)
{
	...
	uint32_t r_src = SysCtrlResetSourceGet();

	if (r_src == RSTSRC_WAKEUP_FROM_SHUTDOWN) {
...
{
	/* Initialize the power driver for the SoC. */
	power_initialize();
