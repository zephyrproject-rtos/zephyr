...
#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)					\
	{.pinmux = (DT_PROP(DT_PROP_BY_IDX(node_id, prop, idx), pinmux) |		\
		    (DT_PROP(DT_PROP_BY_IDX(node_id, prop, idx), bias_pull_up)		\
		     << TIVA_C_PULL_UP_SHIFT) |						\
		    (DT_PROP(DT_PROP_BY_IDX(node_id, prop, idx), bias_pull_down)		\
		     << TIVA_C_PULL_DOWN_SHIFT) |					\
		    (DT_PROP(DT_PROP_BY_IDX(node_id, prop, idx), drive_open_drain)	\
		     << TIVA_C_OPEN_DRAIN_SHIFT))},
