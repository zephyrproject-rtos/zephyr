...
int gpio_pinmux_set(uint32_t port, uint32_t pin, uint32_t func)
{
	...
	{
		gpio_regs->GPIOFSxx[portnum] &= (~pinbit);
		gpio_regs->GPIOIExx[portnum] |= pinbit;     /*Input always enable for loopback*/
	}
	else
	{
		...
		case GPIO00_PWMLED0_PWM8:
			mWRITEBIT(gcfg_regs->GPIOALT,func,0);
			break;
		case GPIO01_SERRXD1_UARTSIN:
			mWRITE2BIT(gcfg_regs->GPIOMUX,func,9);
			break;
		case GPIO03_SERTXD1_UARTSOUT:
			mWRITE2BIT(gcfg_regs->GPIOMUX,func,9);
			break;
		case GPIO22_ESBDAT_PWM9:
			mWRITEBIT(gcfg_regs->GPIOALT,func,1);
			break;
		case GPIO28_32KOUT_SERCLK2:
			mWRITEBIT(gcfg_regs->GPIOALT,func,2);
			break;
		case GPIO36_UARTSOUT_SERTXD2:
			mWRITEBIT(gcfg_regs->GPIOALT,func,3);
			break;
		case GPIO5C_KSO6_P80DAT:
			mWRITEBIT(gcfg_regs->GPIOALT,func,4);
			break;
		case GPIO5D_KSO7_P80CLK:
			mWRITEBIT(gcfg_regs->GPIOALT,func,5);
			break;
		case GPIO5E_KSO8_SERRXD1:
			mWRITEBIT(gcfg_regs->GPIOALT,func,6);
			break;
		case GPIO5F_KSO9_SERTXD1:
			mWRITEBIT(gcfg_regs->GPIOALT,func,7);
			break;
		case GPIO71_SDA8_UARTRTS:
			mWRITEBIT(gcfg_regs->GPIOALT,func,8);
			break;
		case GPIO38_SCL4_PWM1:
			mWRITEBIT(gcfg_regs->GPIOALT,func,9);
			break;
		}
		gpio_regs->GPIOFSxx[portnum] |= pinbit;
		gpio_regs->GPIOIExx[portnum] |= pinbit;     /* Input always enable for loopback*/
...
int gpio_pinmux_get(uint32_t port, uint32_t pin, uint32_t *func)
{
	...
	if (func == NULL)
		return -1;
	if (!(gpio_regs->GPIOFSxx[portnum] & pinbit)){
		*func = PINMUX_FUNC_GPIO;
	}
	else {
		uint32_t GPIO = ((portnum & 0xF) << 5) | (pin & 0x1F);
		uint32_t altfunc = 0;
		switch (GPIO){
		case GPIO00_PWMLED0_PWM8:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,0);
			break;
		case GPIO01_SERRXD1_UARTSIN:
			altfunc = mREAD2BIT(gcfg_regs->GPIOMUX,9);
			break;
		case GPIO03_SERTXD1_UARTSOUT:
			altfunc = mREAD2BIT(gcfg_regs->GPIOMUX,9);
			break;
		case GPIO22_ESBDAT_PWM9:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,1);
			break;
		case GPIO28_32KOUT_SERCLK2:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,2);
			break;
		case GPIO36_UARTSOUT_SERTXD2:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,3);
			break;
		case GPIO5C_KSO6_P80DAT:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,4);
			break;
		case GPIO5D_KSO7_P80CLK:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,5);
			break;
		case GPIO5E_KSO8_SERRXD1:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,6);
			break;
		case GPIO5F_KSO9_SERTXD1:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,7);
			break;
		case GPIO71_SDA8_UARTRTS:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,8);
			break;
		case GPIO38_SCL4_PWM1:
			altfunc = mREADBIT(gcfg_regs->GPIOALT,9);
		break;
		}
		*func = 1+altfunc;
...
int gpio_pinmux_input(uint32_t port, uint32_t pin, uint8_t func)
{
	...
	if (func == PINMUX_OUTPUT_ENABLED){
		gpio_regs->GPIOOExx[portnum] |= pinbit;
		gpio_regs->GPIOIExx[portnum] |= pinbit; /*Input always enable for loopback*/
	}
	else if(func == PINMUX_INPUT_ENABLED){
		gpio_regs->GPIOOExx[portnum] &= (~pinbit);
		gpio_regs->GPIOIExx[portnum] |= pinbit;
	}
	return 0;
}
