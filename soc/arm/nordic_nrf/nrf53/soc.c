...
bool z_arm_on_enter_cpu_idle(void)
{
	...
	if (ok_to_sleep) {
		...
		if (!nrf_rtc_event_check(NRF_RTC0, NRF_RTC_EVENT_COMPARE_1) &&
		    !nrf_rtc_event_check(NRF_RTC1, NRF_RTC_EVENT_COMPARE_1) &&
		    !nrf_rtc_event_check(NRF_RTC1, NRF_RTC_EVENT_COMPARE_2)) {
...
{
	nrf_rtc_cc_set(NRF_RTC1, 1U, val - 1);
}
...
static int rtc_pretick_cpunet_init(void)
{
	...
	uint32_t evt_bt_cc = nrf_rtc_event_address_get(NRF_RTC0, NRF_RTC_EVENT_COMPARE_1);
	uint32_t evt_cc = nrf_rtc_event_address_get(NRF_RTC1, NRF_RTC_EVENT_COMPARE_1);
	uint32_t evt_overflow = nrf_rtc_event_address_get(NRF_RTC1, NRF_RTC_EVENT_COMPARE_2);
	...
	*((volatile uint32_t *)0x41203120) = 0x14;
	...
	if (IS_ENABLED(CONFIG_BT_LL_SW_SPLIT)) {
		nrfx_gppi_event_endpoint_setup(ppi_ch, evt_bt_cc);
	}
	...
	nrf_rtc_event_enable(NRF_RTC1, NRF_RTC_EVENT_COMPARE_1);
	nrf_rtc_event_enable(NRF_RTC1, NRF_RTC_EVENT_COMPARE_2);

	nrf_rtc_event_clear(NRF_RTC1, NRF_RTC_EVENT_COMPARE_1);
	nrf_rtc_event_clear(NRF_RTC1, NRF_RTC_EVENT_COMPARE_2);

	/* Set event 1 tick before overflow. */
	nrf_rtc_cc_set(NRF_RTC1, 2U, 0x00FFFFFF);
