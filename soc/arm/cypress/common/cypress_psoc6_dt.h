...
#define CY_PSOC6_PIN_PERIPH(inst, i) \
	DT_PHA(NODE_ID_FROM_PINCTRL_0(inst, i), cypress_pins, peripheral)

/* Get [default] flags cfg associated with pinctrl-0 pin at index 'i' */
#define CY_PSOC6_PIN_FLAGS_DEFAULT(inst, i) \
	DT_PHA(NODE_ID_FROM_PINCTRL_0(inst, i), cypress_pins, flags)
...
#define CY_PSOC6_PIN_FLAGS(inst, i) \
	(CY_PSOC6_PIN_FLAG(inst, i, bias_pull_up) << \
	 ...
	 CY_PSOC6_PIN_FLAG(inst, i, drive_strong) << \
		SOC_GPIO_STRONG_POS | \
	 CY_PSOC6_PIN_FLAG(inst, i, input_buffer_on) << \
		SOC_GPIO_INPUTBUFFER_POS)
...
#define CY_PSOC6_DT_PIN(inst, idx)					\
	{								\
		...
		CY_PSOC6_PIN_PERIPH(inst, idx) << SOC_GPIO_FUNC_POS |	\
		CY_PSOC6_PIN_FLAGS_DEFAULT(inst, idx) |			\
