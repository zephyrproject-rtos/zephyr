...
{
#ifdef DT_FIXED_CLOCK_XOSC_CLOCK_FREQUENCY
	if (__builtin_strcmp(src, DT_FIXED_CLOCK_XOSC_LABEL) == 0) {
...
#endif
#ifdef DT_FIXED_CLOCK_OSC32K_CLOCK_FREQUENCY
	if (__builtin_strcmp(src, DT_FIXED_CLOCK_OSC32K_LABEL) == 0) {
	...
	if (__builtin_strcmp(src, DT_FIXED_CLOCK_OSC8M_LABEL) == 0) {
...
#endif
#ifdef DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY
	if (__builtin_strcmp(src, DT_FIXED_CLOCK_FDPLL_LABEL) == 0) {
		return DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY;
	}
#endif
...
static void soc_sam0_fdpll_init(void)
{
...
#ifdef DT_FIXED_CLOCK_XOSC32K_CLOCK_FREQUENCY
	if (__builtin_strcmp(DT_FIXED_CLOCK_FDPLL_CLOCK_CONTROLLER,
...
#endif
	{
		...
		clk = device_get_binding(DT_FIXED_CLOCK_FDPLL_CLOCK_CONTROLLER);
	...
	__ASSERT(DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY >= in_frequency,
	...
	BUILD_ASSERT_MSG(DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY >= MHZ(48),
			 "FDPLL output frequency minimum is 48 MHz");
	BUILD_ASSERT_MSG(DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY <= MHZ(96),
	...
	if (reg == SYSCTRL_DPLLCTRLB_REFCLK_REF1) {
		...
		div /= (2 * DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY);
		div *= 0x1000U;
	...
	mul = DT_FIXED_CLOCK_FDPLL_CLOCK_FREQUENCY;
	mul <<= 4U;
	mul += in_frequency / 2U;
	mul /= in_frequency;
