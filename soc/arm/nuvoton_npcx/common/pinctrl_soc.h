...
#define  Z_PINCTRL_NPCX_PSL_IN_MODE(node_id)				\
	COND_CODE_0(DT_ENUM_IDX(node_id, psl_in_mode),			\
		(NPCX_PSL_IN_MODE_LEVEL), (NPCX_PSL_IN_MODE_EDGE))

#define  Z_PINCTRL_NPCX_PSL_IN_POL(node_id)			\
	COND_CODE_0(DT_ENUM_IDX(node_id, psl_in_pol),		\
		(NPCX_PSL_IN_POL_LOW), (NPCX_PSL_IN_POL_HIGH))
...
#define Z_PINCTRL_NPCX_PSL_IN_DETECT_CONF_INIT(node_id, prop)				\
	{										\
		...
		.flags.psl_in_mode = Z_PINCTRL_NPCX_PSL_IN_MODE(node_id),		\
		.flags.psl_in_polarity = Z_PINCTRL_NPCX_PSL_IN_POL(node_id),		\
