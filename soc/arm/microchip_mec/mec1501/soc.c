...
static int soc_pcr_init(void)
{
	...
	PCR_REGS->SLP_EN2 = 0;
	PCR_REGS->SLP_EN4 = 0;
	PCR_REGS->SLP_EN3 = MEC_PCR3_CRYPTO_MASK;
...
{
	new_clk32 &= MEC_VBATR_CLKEN_MASK;

	if ((VBATR_REGS->CLK32_EN & MEC_VBATR_CLKEN_MASK)
		== (uint32_t)new_clk32) {
		return;
	}

	if (new_clk32 == MEC_VBATR_USE_SIL_OSC) {
		VBATR_REGS->CLK32_EN = new_clk32;
	} else {
		/* 1. switch to internal oscillator */
		VBATR_REGS->CLK32_EN = MEC_VBATR_USE_SIL_OSC;
		/* 2. delay for PLL */
		while ((PCR_REGS->OSC_ID & MEC_PCR_OSC_ID_PLL_LOCK) == 0);
...
static int soc_clk32_init(void)
{
	...
	new_clk32 = MEC_VBATR_USE_PAR_CRYSTAL;
    #else
	new_clk32 = MEC_VBATR_USE_SE_CRYSTAL;
    #endif
  #else
	/* Use 32KHZ_PIN as 32KHz source */
	new_clk32 = MEC_VBATR_USE_32KIN_PIN;
  #endif
#else
	/* Use internal 32KHz +/-2% silicon oscillator */
	new_clk32 = MEC_VBATR_USE_SIL_OSC;
#endif
...
static int soc_ecia_init(void)
{
	...
	mec_pcr_periph_slp_ctrl(PCR_ECIA, MEC_PCR_SLEEP_DIS);

	ECS_REGS->INTR_CTRL |= MEC_ECS_ICTRL_DIRECT_EN;

	/* gate off all aggregated outputs */
	ECIA_REGS->BLK_EN_CLR = 0xFFFFFFFFul;
	/* gate on GIRQ's that are aggregated only */
	ECIA_REGS->BLK_EN_SET = MEC_ECIA_AGGR_BITMAP;

	/* Clear all GIRQn source enables and source status */
	pg = &ECIA_REGS->GIRQ08;
	for (n = 0u; n < MEC_GIRQ_ZID_MAX; n++) {
	...
	for (n = 0u; n < MEC_NUM_NVIC_REGS; n++) {
