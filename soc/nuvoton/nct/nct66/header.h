...
struct FIRMWARE_HEDAER_TYPE {
	...
	uint8_t hImageTag[8]; 			/* "%FiMg94@" */
	uint8_t hSignature[512];
	uint32_t hOtpImgHdrOffset;
	uint8_t Reserved_0[4];

	uint16_t hActiveFwAddr;
	uint16_t hRecoveryFwAddr;
	...
	union {
		...
		struct {
			...
			uint8_t hHwTrimRefOTPTable : 1; 	/*  effect when hOTPRefToTable = 1, oSecureBootEn = 0. */
			uint8_t hNotUpdateOTPRegister : 1;	/*  effect when oSecureBootEn = 0. */
			uint8_t hNotEraseOTPTable : 1;
			uint8_t hOTPRefToSrcTable : 1; 		/*  effect when oSecureBootEn = 0 , and only source exist. */
			uint8_t hNotDoBackUp : 1; 			/*  effect in secure mode. */
		} hDevMode_Bits;
	};

	uint8_t hFlashLockReg[2];
	uint8_t hEcFwRegionSize;

	uint32_t hUserFWEntryPoint;
	uint32_t hUserFWRamCodeFlashStart;
	uint32_t hUserFWRamCodeFlashEnd;
	uint32_t hUserFWRamCodeRamStart;
	...
	uint8_t hRevokeKey; 			/* 0xA0 + bit(0 - 4)*/
	...
	ptr_rom_hook hRomHook1Ptr;
	ptr_rom_hook hRomHook2Ptr;
	ptr_rom_hook hRomHook3Ptr;
	ptr_rom_hook hRomHook4Ptr;
