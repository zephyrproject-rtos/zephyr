...
/**
 * @brief Pinctrl node types in NCT series
 */
enum nct_pinctrl_type {
	NCT_PINCTRL_TYPE_PERIPH,
	NCT_PINCTRL_TYPE_DEVICE_CTRL,
	NCT_PINCTRL_TYPE_RESERVED,
};

/**
 * @brief Suppoerted peripheral device configuration type in NCT series
 */
enum nct_periph_type {
	NCT_PINCTRL_TYPE_PERIPH_PINMUX,
	NCT_PINCTRL_TYPE_PERIPH_PUPD,
};

/**
 * @brief Suppoerted IO bias type in NCT series
 */
enum nct_io_bias_type {
	NCT_BIAS_TYPE_NONE,
	NCT_BIAS_TYPE_PULL_DOWN,
	NCT_BIAS_TYPE_PULL_UP,
};

/**
 * @brief NCT peripheral device configuration structure
 *
...
struct nct_periph {
	...
	enum nct_periph_type type: 2;
	/** Reserved field. */
	uint16_t reserved: 2;
} __packed;

/**
 * @brief NCT device control structure
 *
...
/**
 * @brief Type for NCT pin configuration. Please make sure the size of this
 *        structure is 4 bytes in case the impact of ROM usage.
 */
struct nct_pinctrl {
	union {
		struct nct_periph periph;
		struct nct_dev_ctl dev_ctl;
		uint16_t cfg_word;
	} cfg;
	struct {
		/** Indicates the current pinctrl type. */
		enum nct_pinctrl_type type :2;
		/** Properties used for pinmuxing. */
		bool pinmux_gpio :1;
		/** Properties used for io-pad. */
		enum nct_io_bias_type io_bias_type :2;
		uint16_t reserved :11;
	} flags;
} __packed;

typedef struct nct_pinctrl pinctrl_soc_pin_t;

/** Helper macros for NCT pinctrl configurations. */
#define  Z_PINCTRL_NCT_BIAS_TYPE(node_id)					\
	COND_CODE_1(DT_PROP(node_id, bias_pull_up), (NCT_BIAS_TYPE_PULL_UP),	\
		(COND_CODE_1(DT_PROP(node_id, bias_pull_down),			\
		(NCT_BIAS_TYPE_PULL_DOWN), (NCT_BIAS_TYPE_NONE))))

#define Z_PINCTRL_NCT_HAS_PUPD_PROP(node_id)		\
 ...
 */
#define Z_PINCTRL_NCT_PERIPH_PINMUX_INIT(node_id, prop)				\
	{										\
		.flags.type = NCT_PINCTRL_TYPE_PERIPH,					\
		.flags.pinmux_gpio = DT_PROP(node_id, pinmux_gpio),			\
		.cfg.periph.type = NCT_PINCTRL_TYPE_PERIPH_PINMUX,			\
 ...
 * @param prop Property name for pinmux configuration. (i.e. 'pinmux')
 */
#define Z_PINCTRL_NCT_DEVICE_CONTROL_INIT(node_id, prop)			\
	{									\
		.flags.type = NCT_PINCTRL_TYPE_DEVICE_CTRL,			\
 ...
 */
#define Z_PINCTRL_NCT_PERIPH_PUPD_INIT(node_id, prop)				\
	{									\
		.flags.type = NCT_PINCTRL_TYPE_PERIPH,				\
		.flags.io_bias_type = Z_PINCTRL_NCT_BIAS_TYPE(node_id),	\
		.cfg.periph.type = NCT_PINCTRL_TYPE_PERIPH_PUPD,		\
