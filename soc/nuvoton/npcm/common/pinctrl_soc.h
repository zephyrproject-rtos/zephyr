 ...
 *
 * @param node_id Node identifier.
 * @param prop Property name for pinmux configuration. (i.e. 'lvol')
 */
#define Z_PINCTRL_NPCM_LVOL_INIT(node_id, prop)                                                    \
	{                                                                                          \
		...
		.props.id = Z_PINCTRL_NPCM_PIN_ID(node_id),                                        \
		.props.group = Z_PINCTRL_NPCM_PIN_GROUP(node_id),                                  \
		.cfg.lvol.level = DT_PROP_BY_IDX(node_id, prop, 0),                                \
	},
...
#define Z_PINCTRL_STATE_PIN_INIT_EXT(node_id, prop)                                                \
	...
	COND_CODE_1( \
		...
		())                                                                             \
	COND_CODE_1( \
		DT_NODE_HAS_PROP(node_id, nuvoton_dev_ctl), \
		(Z_PINCTRL_NPCM_DEVICE_CTRL_INIT(node_id, nuvoton_dev_ctl)), \
		())                                                                             \
	COND_CODE_1( \
		DT_NODE_HAS_PROP(node_id, nuvoton_lvol), \
		(Z_PINCTRL_NPCM_LVOL_INIT(node_id, nuvoton_lvol)), \
		())
