...
static int nordicsemi_nrf53_init(void)
{
	...
	DT_ENUM_HAS_VALUE(LFXO_NODE, load_capacitors, external) || \
	DT_ENUM_HAS_VALUE(LFXO_NODE, load_capacitors, internal)
	...
	(defined(NRF_SOC_SECURE_SUPPORTED) && \
	...
	DT_ENUM_HAS_VALUE(DT_NODELABEL(hfxo), load_capacitors, external)
