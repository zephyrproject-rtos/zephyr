...
static void tail_mask_set(atomic_t *tail_mask, size_t num_bits, size_t off)
{
	...
	atomic_or(&tail_mask[idx + 1], BIT_MASK(tail_bits - (32 - tail_off)));
}
...
static uint32_t num_bits_get(atomic_t *tail_mask, size_t off)
{
	...
	mask = (tail_mask[idx] | BIT(off)) >> off;
	num_bits = __builtin_ctz(~mask);
	if (((w_off + num_bits) == 32) && (idx < (HEAP_NUM_WORDS - 1))) {
		 ...
		 */
		num_bits_2 = __builtin_ctz(tail_mask[idx + 1]);
	} else {
		num_bits_2 = 0;
	}
	...
	if (num_bits > 1) {
		...
		atomic_and(&tail_mask[idx], ~mask);
	}

	if (num_bits_2 != 0) {
		mask = BIT_MASK(num_bits_2);
		atomic_and(&tail_mask[idx + 1], ~mask);
	}

	return num_bits + num_bits_2;
}
