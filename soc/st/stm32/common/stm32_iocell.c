...
/* Returns 1 if HSLV can be confgured */
static inline int iocell_get_hslv_safeguard(uint16_t domain)
{
	...
	return (hconf1 & BIT(shift)) == BIT(shift);
#endif
}
...
#define STM32_IOCELL_INIT(node_id)                                                                 \
	if (STM32_DT_IOCELL_DOMAIN_VALID(DT_PROP(node_id, domain))) {                              \
		if (DT_PROP(node_id, hslv_enabled) ||                                              \
		    (DT_PROP(node_id, hslv_auto) &&                                                \
...
SYS_INIT(iocell_init, PRE_KERNEL_1, CONFIG_STM32_IOCELL_PRIORITY);
...
static void iocell_hslv_check_log(int domain_id, int safeguard, int hslv, const char *domain_name)
{
	if (safeguard < 0) {
...
#define STM32_IOCELL_CHECK(node_id)                                                                \
	if (STM32_DT_IOCELL_DOMAIN_VALID(DT_PROP(node_id, domain))) {                              \
		iocell_hslv_check_log(DT_PROP(node_id, domain),                                    \
				      iocell_get_hslv_safeguard(DT_PROP(node_id, domain)),         \
				      DT_PROP(node_id, hslv_enabled), DT_NODE_FULL_NAME(node_id)); \
	} else {                                                                                   \
		LOG_ERR("Unknown power domain ID: \"" DT_NODE_FULL_NAME(node_id) "\" (%d)",        \
