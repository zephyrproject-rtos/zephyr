...
static int low_power_mode(void)
{
	...
	/* MCU enters in low-power modes, allowing any interrupt to skip WFI */
...
static void set_mode_suspend_to_ram_enter(void)
{
	...
	arch_pm_s2ram_suspend(low_power_mode);
}
...
static void set_mode_stop_enter(uint8_t substate_id)
{
	 ...
	 */
	sys_cache_instr_disable();
	/* If stop mode is greater than Stop0 (id = 1), manage flash latency and RAMCFG */
	if (substate_id > 1) {
