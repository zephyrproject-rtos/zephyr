...
/* Check PWM Clock value, PCLK 16 Mhz. */
#if ((DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_16MHZ) ||	 \
	(DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_32MHZ) || \
	(DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_64MHZ))
#if (DT_PROP(DT_PATH(soc, pwm_80140400), clock_frequency) < 62500)
	#error "PWM Clock Frequency can't be less than 62500 Hz: PCLK (16 MHz) / 256"
#endif
#endif

/* Check PWM Clock value, PCLK 24 Mhz. */
#if ((DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_24MHZ) ||	 \
	(DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_48MHZ) || \
	(DT_PROP(DT_PATH(cpus, cpu_0), clock_frequency) == CLK_96MHZ))
#if (DT_PROP(DT_PATH(soc, pwm_80140400), clock_frequency) < 93750)
	#error "PWM Clock Frequency can't be less than 93750 Hz: PCLK (24 MHz) / 256"
#endif
#endif
...
/**
 * @brief Implemented missed __ashldi3 in case if newlib is selected.
 */
...
/**
 * @brief Implemented missed __lshrdi3 in case if newlib is selected.
 */
