...
#define BFLB_IPC_LIST_BYTES      16U /* 2 lists * sizeof(sys_slist_t) */
...
static sys_slist_t *bflb_ipc_list_done(void);
...
static sys_slist_t *bflb_ipc_list_done(void)
{
	return (sys_slist_t *)((uint8_t *)&ipc_shared_env + BFLB_IPC_LIST_DONE_OFF);
}
...
static void bflb_ipc_recycle_txcfm(void)
{
	sys_slist_t *done = bflb_ipc_list_done();
	sys_snode_t *cur;

	do {
		cur = sys_slist_get(done);
	} while (cur != NULL);
}
 ...
 *   [+0..+25]   MAC hdr (Protected not reliably set by FW; ccmp_encrypt
 *               sets it; includes QoS Control byte)
...
/* Runs on every TX push -- a one-time setup is not possible: the blob
 * rebuilds the THD/BD chain for each transmission with pointers into
 * its own scattered buffers, and the frame bytes must first be copied
 * (or CCMP-encrypted) into the private contiguous buffer anyway.  Only
 * the pad-to-private-slot binding (idx) is static.
 */
static void bflb_tx_fixup(void *pad_txdesc, volatile uint32_t *hw)
{
...
{
	sys_slist_t *submit =
		(sys_slist_t *)((uint8_t *)&ipc_shared_env + BFLB_IPC_LIST_SUBMIT_OFF);
...
void bflb_wifi_ipc_isr(const void *arg)
{
	 ...
	 * Without these calls A2E_TXDESC (app-to-emb) triggers never wake
	 * ipc_emb_tx_evt.
	 */
