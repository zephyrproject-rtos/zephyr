...
/* Entry points into the firmware/PHY blobs (wifi_main comes from
 * bl60x_fw_api.h).
 */
...
static const uint16_t bflb_legacy_rate_100kbps[] = {
...
int bflb_wifi_phy_rate_kbps(uint8_t sta_idx)
{
	...
	if (((info & RATE_INFO_VALID) == 0U) ||
	    ...
	    ((info & RATE_INFO_IDX_MASK) >= ARRAY_SIZE(bflb_legacy_rate_100kbps))) {
		return 0;
	}

	return (int)bflb_legacy_rate_100kbps[info & RATE_INFO_IDX_MASK] * 100;
}
