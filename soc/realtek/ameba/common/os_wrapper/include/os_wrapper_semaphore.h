...
/**
 * @brief  Take a semaphore. The API internally determines whether it is in the interrupt state and
 *         calls the corresponding RTOS interface.
 *
 * @note   If timeout_ms is set to the maximum value, then if the semaphore cannot be obtained
 *         consistently, the log will be printed every 10 seconds.
 * @param  p_handle: Address of the semaphore.
...
/**
 * @brief  Get a semaphore's count. If the semaphore is a binary semaphore then returns 1 if
 *         the semaphore is available, and 0 if the semaphore is not available.
 * @param  p_handle: Address of the semaphore.
