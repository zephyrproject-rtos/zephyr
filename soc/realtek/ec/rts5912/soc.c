...
void soc_early_init_hook(void)
{
	...
	if (IS_ENABLED(CONFIG_RTS5912_DEBUG_SWJ)) {
		...
		if (ret < 0) {
			LOG_ERR("SWJ init failed %x", ret);
		}
	}

	if (IS_ENABLED(CONFIG_PM)) {
		/* turn off TRNG/ OTP/ LDO3 power */
		sys_write32(0ul, RTS5912_PUF_ADDRESS + RTS5912_TRNG_OFFSET + RTS5912_RNG_RUN);
		sys_write32(0ul, RTS5912_PUF_ADDRESS + RTS5912_OTP_OFFSET + RTS5912_CFG_PDSTB);
