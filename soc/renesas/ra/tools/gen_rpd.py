...
class ZephyrElf:
    ...
    def readSymbol(self, symbols, name):
    ...
    def parse(self, path):
        ...
        with open(path, "rb") as f:
            ...
            try:
                ...
                # Read memory symbols
                code_flash_s = self.readSymbol(symtab, "__FLASH_S")
                code_flash_nsc = self.readSymbol(symtab, "__FLASH_NSC")
                code_flash_ns = self.readSymbol(symtab, "__FLASH_NS")
                data_flash_s = self.readSymbol(symtab, "__DATA_FLASH_S")
                data_flash_ns = self.readSymbol(symtab, "__DATA_FLASH_NS")
                sram_s = self.readSymbol(symtab, "__RAM_S")
                sram_nsc = self.readSymbol(symtab, "__RAM_NSC")
                sram_ns = self.readSymbol(symtab, "__RAM_NS")
