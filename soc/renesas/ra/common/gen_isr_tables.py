...
class gen_isr_config:
    ...
    def __init__(self, args, syms, log):
        ...
        edt = edtlib.EDT("zephyr.dts", [os.path.join(zephyr_base, "dts", "bindings")])
        intc = edt.chosen_node("zephyr,interrupt-controller")
        self.__interrupt_id_count = intc.props["interrupt-id-count"].val
    ...
    @property
    def interrupt_id_count(self):
        return self.__interrupt_id_count
...
def parse_args():
    ...
    parser.add_argument("--interrupt-id-count", type=int, default=0,
            help="""Specifies the number of interrupt IDs.
                 Use the interrupt ID to identify ISRs instead of the IRQ number
                 if this option is not 0.""")
