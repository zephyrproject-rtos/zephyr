 ...
 */
#ifndef ZEPHYR_SOC_RENESAS_RZ_COMMON_PINCTRL_RZV_H_
#define ZEPHYR_SOC_RENESAS_RZ_COMMON_PINCTRL_RZV_H_
...
#define RZV_GET_PORT_PIN(pinmux) (pinmux & ~(0xF << 4))
#define RZV_GET_FUNC(pinmux)     ((pinmux & 0xF0) >> 4)

#define RZV_GET_PU_PD(node_id)                                                                     \
	DT_PROP(node_id, bias_pull_up) == 1 ? 1U : (DT_PROP(node_id, bias_pull_down) == 1 ? 2U : 0U)

#define RZV_GET_FILNUM(node_id) ((DT_PROP(node_id, renesas_filter) >> 2) & 0x3)

#define RZV_GET_FILCLKSEL(node_id) (DT_PROP(node_id, renesas_filter) & 0x3)

#define RZV_FILTER_ON_OFF(node_id) COND_CODE_0(DT_PROP(node_id, renesas_filter), (0), (1))

/* Process pinmux cfg */
#define Z_PINCTRL_PINMUX_INIT(node_id, state_prop, idx)                                            \
	{                                                                                          \
		.port_pin = RZV_GET_PORT_PIN(DT_PROP_BY_IDX(node_id, state_prop, idx)),            \
		.config = {.reserved = 0,                                                          \
			   .pupd_reg = RZV_GET_PU_PD(node_id),                                     \
			   .iolh_reg = DT_PROP(node_id, drive_strength),                           \
			   .pmc_reg = 1,                                                           \
			   .sr_reg = DT_ENUM_IDX(node_id, slew_rate),                              \
			   .ien_reg = DT_PROP(node_id, input_enable),                              \
			   .filonoff_reg = RZV_FILTER_ON_OFF(node_id),                             \
			   .filnum_reg = RZV_GET_FILNUM(node_id),                                  \
			   .filclksel_reg = RZV_GET_FILCLKSEL(node_id),                            \
			   .pfc_reg = RZV_GET_FUNC(DT_PROP_BY_IDX(node_id, state_prop, idx))},     \
	},
...
#endif
#endif /* ZEPHYR_SOC_RENESAS_RZ_COMMON_PINCTRL_RZV_H_ */
