...
struct bcm2712_composite_pinctrl_soc_pin {
	int type;
	union {
...
typedef struct bcm2712_composite_pinctrl_soc_pin pinctrl_soc_pin_t;

#define Z_BCM2712_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)                                       \
	UTIL_CAT(DT_BINDING_COMPAT_UPPER_TOKEN(DT_GPARENT(node_id)), _STATE_PIN_INIT)(node_id,     \
										      prop, idx)

#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)                                               \
	{                                                                                          \
		.type = DT_DEP_ORD(DT_GPARENT(node_id)),                                           \
		.DT_BINDING_COMPAT_TOKEN(DT_GPARENT(node_id)) =                                    \
			Z_BCM2712_PINCTRL_STATE_PIN_INIT(node_id, prop, idx),                      \
	},
