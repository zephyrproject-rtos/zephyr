...
#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)                                               \
	{                                                                                          \
		IF_ENABLED(DT_SAME_NODE(DT_GPARENT(node_id), DT_NODELABEL(pinctrl)), (             \
			.type = DT_DEP_ORD(DT_GPARENT(node_id)),                                   \
			.brcm_bcm2712_pinctrl =                                                    \
				BRCM_BCM2712_PINCTRL_STATE_PIN_INIT(node_id, prop, idx),           \
		))                                                                                 \
		IF_ENABLED(DT_SAME_NODE(DT_GPARENT(node_id), DT_NODELABEL(rp1_pinctrl)), (         \
			.type = DT_DEP_ORD(DT_GPARENT(node_id)),                                   \
