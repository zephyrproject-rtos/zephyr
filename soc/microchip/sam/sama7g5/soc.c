...
void soc_early_init_hook(void)
{
...
#if defined(DT_N_NODELABEL_gmac0) || defined(DT_N_NODELABEL_gmac1)
	/* config ETHPLL to 625 MHz, 24 * (0x19 + 1 + 0x2aaab / 2^22) = 625 */
	PMC_REGS->PMC_PLL_UPDT = PMC_PLL_UPDT_ID(6);
	PMC_REGS->PMC_PLL_CTRL1 = PMC_PLL_CTRL1_MUL(0x19) | PMC_PLL_CTRL1_FRACR(0x2aaab);
	...
	PMC_REGS->PMC_PLL_UPDT |= PMC_PLL_UPDT_UPDATE_Msk;
	while ((PMC_REGS->PMC_PLL_ISR0 & BIT(6)) == 0) {
	}
#endif

#if defined(DT_N_NODELABEL_gmac0)
...
#if defined(DT_N_NODELABEL_gmac1)
