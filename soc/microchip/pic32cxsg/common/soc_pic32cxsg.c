...
static void osc32k_init(void)
{
	...
	GCLK->GENCTRL[1].reg = GCLK_GENCTRL_SRC(GCLK_SOURCE_XOSC32K) | GCLK_GENCTRL_RUNSTDBY(1) |
			       GCLK_GENCTRL_GENEN(1);
}
#elif CONFIG_SOC_MICROCHIP_PIC32CXSG_OSCULP32K_AS_MAIN
static void osc32k_init(void)
{
	GCLK->GENCTRL[1].reg = GCLK_GENCTRL_SRC(GCLK_SOURCE_OSCULP32K) | GCLK_GENCTRL_RUNSTDBY(1) |
			       GCLK_GENCTRL_GENEN(1);
}
#else
...
static void dpll_init(uint8_t n, uint32_t f_cpu)
{
	...
	OSCCTRL->Dpll[n].DPLLCTRLB.reg = OSCCTRL_DPLLCTRLB_REFCLK_GCLK | OSCCTRL_DPLLCTRLB_WUF(1) |
