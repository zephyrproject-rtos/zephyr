...
#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)			\
	(DT_PROP_BY_IDX(node_id, prop, idx) |						\
	 ((FT9001_PUPD_PULLUP * DT_PROP(node_id, bias_pull_up))		\
	  << FT9001_PUPD_POS) |										\
	 ((FT9001_PUPD_PULLDOWN * DT_PROP(node_id, bias_pull_down))	\
	  << FT9001_PUPD_POS) |										\
	 ((FT9001_OTYPE_OD * DT_PROP(node_id, drive_open_drain))	\
	  << FT9001_OTYPE_POS) |									\
 ...
 */
#define FT9001_PUPD_GET(pincfg) \
	(((pincfg) >> FT9001_PUPD_POS) & FT9001_PUPD_MSK)

/**
 * @brief Obtain OTYPE field from pinctrl_soc_pin_t configuration.
 * @param pincfg pinctrl_soc_pin_t bit field value.
 */
#define FT9001_OTYPE_GET(pincfg) \
	(((pincfg) >> FT9001_OTYPE_POS) & FT9001_OTYPE_MSK)

/**
 * @brief Obtain OSPEED field from pinctrl_soc_pin_t configuration.
 * @param pincfg pinctrl_soc_pin_t bit field value.
 */
#define FT9001_OSPEED_GET(pincfg) \
	(((pincfg) >> FT9001_OSPEED_POS) & FT9001_OSPEED_MSK)
