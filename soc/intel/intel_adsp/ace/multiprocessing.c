...
void send_ipi(uint32_t cpu_bitmap, int agent_idx)
{
	...
	/* agent_idx:
	 * 0: Raise IDC-B interrupt on target CPU.
	 * 1: Raise IDC-A interrupt on target CPU.
	 *
	 * Note that there are two halves of the IDC registers which are called
	 * agents here: [0] is agent A and [1] is agent B. The hardware is
	 * designed in a way that writing to [0] will raise IDC-B interrupt
	 * on target CPU, and writing to [1] will raise IDC-A interrupt on
	 * target CPU. Also, the target CPU needs to write to the other half to
	 * response to the IDC message. For example, writing to [0] here to
	 * the target CPU, and the target CPU will write to [1] to response.
	 */
