...
static int intel_adsp_ipc_receive_cb(uint8_t msg_type, const void *msg_data, void *priv)
{
	const struct device *dev = INTEL_ADSP_IPC_HOST_DEV;
	struct intel_adsp_ipc_data *devdata = dev->data;
	struct intel_adsp_ipc_msg *msg = (struct intel_adsp_ipc_msg *)msg_data;
	bool done = true;
...
static int intel_adsp_ipc_event_cb(uint8_t evt_type, const void *evt_data, void *priv)
{
	const struct device *dev = INTEL_ADSP_IPC_HOST_DEV;
	struct intel_adsp_ipc_data *devdata = dev->data;
	...
	if (external_completion) {
		return INTEL_ADSP_IPC_EVT_RET_EXT_COMPELTE;
	} else {
		return 0;
	}
}
