...
/*
 * 3 domains can select different access policies from 8 policy registers,
 * and each master can select different domains by assigning different DIDs.
 ...
 * used only for masters the boot ROM left unassigned.
 */
...
void nxp_mcxw72_trdc_init(void)
{
	...
	/* Program access-control policy 0 for MBC0/1/2 to "full access".
	 * MBC0 ad MBC1 are for flash and TCM, MBC2 is for Pbridge peripherals.
	 */
	...
	np_assign.privilegeAttr = kTRDC_ForcePrivilege;
	np_assign.secureAttr    = kTRDC_ForceSecure;
