...
static const struct nxp_mpu_region mpu_regions[] = {
	/* Region 0 */
	MPU_REGION_ENTRY("DEBUGGER_0", 0, 0xFFFFFFFF, REGION_DEBUG_ATTR),
 ...
 */
	...
	/* Region 1 */
	MPU_REGION_ENTRY("BACKGROUND_0", 0, DT_CHOSEN_SRAM_ADDR - 1, REGION_BACKGROUND_ATTR),
	/* Region 2 */
	MPU_REGION_ENTRY("BACKGROUND_1", (DT_CHOSEN_SRAM_ADDR + DT_CHOSEN_SRAM_SIZE), 0xFFFFFFFF,
	 ...
	 */
	MPU_REGION_ENTRY("BACKGROUND_0", 0, 0xFFFFFFFF, REGION_BACKGROUND_ATTR),
#endif /* CONFIG_MPU_STACK_GUARD */
	/* Region 3 (stack guard on) / Region 2 (stack guard off) */
	MPU_REGION_ENTRY("FLASH_0", CONFIG_FLASH_BASE_ADDRESS,
			 (CONFIG_FLASH_BASE_ADDRESS + (CONFIG_FLASH_SIZE * 1024) - 1),
			 REGION_FLASH_ATTR),
	/* Region 4 (stack guard on) / Region 3 (stack guard off) */
	MPU_REGION_ENTRY("RAM_U_0", DT_CHOSEN_SRAM_ADDR,
			 (DT_CHOSEN_SRAM_ADDR + DT_CHOSEN_SRAM_SIZE - 1), REGION_RAM_ATTR),
};
