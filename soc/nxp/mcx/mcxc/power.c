...
{
	/*
	 * Keep the system OSC running across STOP modes so that ERCLK32K (and
	 * therefore the LPTMR system timer when sourced from it) keeps ticking
	 * and can wake the CPU from STOP.
	 */
...
SYS_INIT(mcxc_pm_init, PRE_KERNEL_1, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT);
