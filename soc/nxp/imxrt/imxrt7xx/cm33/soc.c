...
void __weak imxrt_pre_init_display_interface(void)
{
...
#if CONFIG_MIPI_DBI
    uint8_t bitPerCycle;
    if (DT_ENUM_IDX(DT_NODELABEL(lcdif), color_coding) < 5)
    {
        bitPerCycle = 8U;
    }
    else if (DT_ENUM_IDX(DT_NODELABEL(lcdif), color_coding) > 5)
    {
        bitPerCycle = 16U;
    }
    else
    ...
    /* When using LCDIF, with 279.53MHz DBI source clock and 16bpp format, a 14 wr period requires a 279.53MHz / 14 *
    ...
    uint32_t mipiDsiDphyBitClkFreq_Hz = DT_PROP(DT_NODELABEL(lcdif), clock_frequency) / DT_PROP_OR(DT_NODELABEL(lcdif), divider, 1) /
                                        DT_PROP(DT_NODELABEL(lcdif), wr_period) * bitPerCycle;
#else
