 ...
 * when resyncing. Only Over Drive Run is carried; __soc_clock_itcmfunc replaces
 * AT_QUICKACCESS_SECTION_CODE; the window runs on a DTCM scratch stack because
 * the stack it is entered on lives in the PSRAM the window parks; interrupts are
 * masked here.
 *
 * Clock sequence:
 ...
 *       soc_clock_handover_to_sxosc  <-- ITCM window, both XSPI parked, runs on
 *                                        soc_clock_window_stack (DTCM)
...
/*
 * Placement for the window functions below, modelled on __ramfunc: the section
 * attribute alone does not stop the compiler from inlining a static helper into
 * its flash-resident caller, and an inlined window leaves .itcm empty and runs
 * the XSPI park sequence straight out of the XIP flash it just disabled. Bind
 * the attributes together so a helper added later cannot pick up only one.
 *
 * __ramfunc itself cannot be used: its .ramfunc section is linked into the
 * RAM_REGION built from zephyr,sram, which on this board is the XSPI1 PSRAM the
 * window parks. The flash-resident caller reaches ITCM through a veneer the
 * linker places in flash and fetches before the window opens.
 */
#define __soc_clock_itcmfunc __itcm_section __noinline
 ...
 */
static void __soc_clock_itcmfunc soc_clock_park_xspi0(soc_xspi_park_step_t step)
{
	uint32_t v;
	uint32_t i;
 ...
 */
static void __soc_clock_itcmfunc soc_clock_park_xspi1(soc_xspi_park_step_t step)
{
	uint32_t v;
	uint32_t i;
	bool livePsram;
 ...
 */
static void __soc_clock_itcmfunc soc_clock_select_sxosc_ref(void)
{
 ...
 */
static void __soc_clock_itcmfunc soc_clock_handover_to_sxosc(void)
{
...
/*
 * Scratch stack for the window. The window parks XSPI1, so a stack in the PSRAM
 * behind it disappears for as long as the window is open -- and step 0 disables
 * the L1 D-cache and the LLC, so every access really does reach the parked
 * controller. The DTCM is clocked by neither XSPI. The window runs pre-kernel
 * with interrupts locked and nothing nests on it, so the depth of the window
 * call chain is all this has to hold: 40 bytes at the default optimization
 * level, the helpers it calls being leaves. Sized well above that so an
 * unoptimized build does not quietly overrun it; the DTCM is 256 KB.
 *
 * Only the running stack is moved. MSP is left where it is: a fault inside the
 * window is unrecoverable however the stacks are placed, because the handler and
 * the vector table are in the XIP flash the window disables, so exception entry
 * cannot even fetch.
 */
static uint8_t __dtcm_noinit_section __aligned(8) soc_clock_window_stack[512];

/*
 * Runs the window on soc_clock_window_stack, in soc_clock_window.S. Plain
 * assembly, so nothing the compiler emits can touch the stack being switched
 * away from; see that file for the register contract.
 */
extern void soc_clock_call_on_stack(void (*fn)(void), void *stack_top);
...
static void soc_clock_prepare(void)
{
	 ...
	 * during the window. Both XSPI controllers are parked here, so the PSRAM
	 * (XSPI1) the running stack normally lives in is off-limits, as is the NOR
	 * XIP (XSPI0). The window therefore runs on soc_clock_window_stack in the
	 * DTCM; PSPLIM is released across the switch because the scratch stack sits
	 * below the limit the current stack was given.
	 */
	key = irq_lock();
#if defined(CONFIG_BUILTIN_STACK_GUARD)
	psplim = __get_PSPLIM();
	__set_PSPLIM(0U);
#endif
	soc_clock_call_on_stack(soc_clock_handover_to_sxosc,
				&soc_clock_window_stack[sizeof(soc_clock_window_stack)]);
#if defined(CONFIG_BUILTIN_STACK_GUARD)
	__set_PSPLIM(psplim);
#endif
	irq_unlock(key);
