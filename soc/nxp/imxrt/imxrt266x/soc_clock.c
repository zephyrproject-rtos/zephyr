 ...
 * AT_QUICKACCESS_SECTION_CODE; the stacks are in DTCM (chosen zephyr,kernel-stacks)
...
static void soc_clock_prepare(void)
{
	 ...
	 * (XSPI1) and NOR XIP (XSPI0) are off-limits: the board's chosen
	 * zephyr,kernel-stacks must keep the stacks in the DTCM or another memory
	 * that neither XSPI clocks.
	 */
	key = irq_lock();
