 ...
 * locates it. 0x1000 is the fixed container position (per the boot-chapter
 * layout tables) and CONFIG_ROM_START_OFFSET is the image start.
 */
#define CONTAINER_TO_IMAGE_OFFSET (CONFIG_ROM_START_OFFSET - 0x1000U)

/*
 * AHAB boot container, placed by boot_header.ld at 0x1000 from the start of
 * the boot device.
