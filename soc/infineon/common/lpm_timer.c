...
/**
 * @brief LPM companion timer for Infineon CAT1 SoCs using MCWDT.
 *
 * Shared by the PSC3 and PSE84 SoCs.  When SysTick is the primary system timer
 * it stops during deepsleep because the HF clocks are gated. This module drives
 * the MCWDT (Multi-Counter Watchdog Timer) from the always-on 32 kHz
 * low-frequency clock (WCO on PSC3, PILO on PSE84) and implements the
...
/* Use the first enabled instance - the board DTS enables the MCWDT instance for
 * this core.
 */
...
/* MCWDT source clock frequency, taken from the lp-timer node's "clock-frequency"
 * property. Used to convert between LFCLK ticks and microseconds.
 */
#define LPM_TIMER_SRC_FREQ DT_PROP(MCWDT_NODE, clock_frequency)

/* Startup delay for Cy_MCWDT_Enable(), in microseconds (~2 LFCLK cycles at
 * 32 kHz), so the counters are guaranteed running before the first alarm.
 */
#define LPM_TIMER_ENABLE_DELAY_US 62U

/* Minimum alarm in LFCLK ticks: MCWDT match values take effect after ~2 LFCLK
 ...
 * 32 kHz; this many CPU iterations covers the worst-case propagation. It only
 * keeps a stopped LFCLK from spinning here forever.
 */
...
{
	/* Counter 1 (the alarm) is the only counter in INT mode, so it is the
	 * only source that can raise a flag. Clear it and disable the alarm until
	 * the next entry re-arms it.
	 */
...
void z_sys_clock_lpm_enter(uint64_t max_lpm_time_us)
{
	...
	if (max_lpm_time_us > ((uint64_t)LPM_MAX_DELAY_TICKS * 1000000ULL / LPM_TIMER_SRC_FREQ)) {
		...
		delay_ticks = (uint32_t)((max_lpm_time_us * LPM_TIMER_SRC_FREQ) / 1000000ULL);
	}
	 ...
	 * Bounded at 3 LFCLK ticks (~92 us) and reached on 3 of the 65536 C0 phases.
	 */
	...
	/* A match 1 or 2 ticks ahead of the live count loses the ~2 LFCLK cycle
	 * match latch and slips a whole C0 period. The LPM_MIN_DELAY_TICKS floor
	 * above guards only the total, so a delay 1-2 ticks past a period multiple
	 * still lands there, and the settle loop below cannot rescue it: waiting
	 * would step past the match. Push the delay out instead, by at most two
	 * ticks, which cannot carry into the high 16 bits. C2 still measures the
	 * true elapsed time on exit.
	 */
	if (((delay_ticks & 0xFFFFU) != 0U) && ((delay_ticks & 0xFFFFU) < LPM_MIN_DELAY_TICKS)) {
		delay_ticks += LPM_MIN_DELAY_TICKS - (delay_ticks & 0xFFFFU);
	}

	/* A match landing at or just behind the live count has ambiguous
	 * first-cascade timing. Advance past that window and fold the ticks skipped
	 * while waiting back into the remaining delay so the total stays exact.
	 * Same bound as above: 3 ticks (~92 us), for 3 of the 65536 low-16 values.
	 */
	 ...
	 */
	c0_match = (uint16_t)(c0_settle + delay_ticks);
...
uint64_t z_sys_clock_lpm_exit(void)
{
	...
	/* Convert LFCLK ticks to microseconds */
	return ((uint64_t)elapsed_ticks * 1000000ULL) / LPM_TIMER_SRC_FREQ;
}
