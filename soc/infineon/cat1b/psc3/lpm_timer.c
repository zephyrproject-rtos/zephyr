...
/**
 * @brief LPM companion timer for PSC3 using MCWDT.
 *
 ...
 * Watchdog Timer) from the WCO (32 kHz) clock and implements the
...
/* Use the first enabled instance - the board DTS enables mcwdt0 */
...
/* WCO clock frequency - the MCWDT source clock */
#define WCO_FREQ DT_PROP(DT_PATH(clocks, clk_wco), clock_frequency)

/* Minimum alarm in WCO ticks: MCWDT match values take effect after ~2 WCO
...
void z_sys_clock_lpm_enter(uint64_t max_lpm_time_us)
{
	...
	/* Convert microseconds to WCO ticks, clamped to the programmable range. */
	if (max_lpm_time_us > ((uint64_t)LPM_MAX_DELAY_TICKS * 1000000ULL / WCO_FREQ)) {
		delay_ticks = LPM_MAX_DELAY_TICKS;
	} else {
		delay_ticks = (uint32_t)((max_lpm_time_us * WCO_FREQ) / 1000000ULL);
	}
...
uint64_t z_sys_clock_lpm_exit(void)
{
	...
	/* Convert WCO ticks to microseconds */
	return ((uint64_t)elapsed_ticks * 1000000ULL) / WCO_FREQ;
}
