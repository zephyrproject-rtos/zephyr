...
/*
 * System management is a requirement for drivers that do not support runtime PM.
 * It is also required for S2RAM transitions.
 */
BUILD_ASSERT(IS_ENABLED(CONFIG_PM_DEVICE_SYSTEM_MANAGED),
	     "PM requires CONFIG_PM_DEVICE_SYSTEM_MANAGED");
...
static int ifx_pm_deep_arm_wfi(void)
{
	...
	/* Matches the PDL entry sequence; the wake must not report a stale reason. */
 ...
 * devices in init order, records the power loss that already happened, and lets
 * TURN_ON do the rebuild through pm_device_action_run(). TURN_ON leaves the
 * device SUSPENDED, which is what the later pm_resume_devices() pass expects.
 *
 * A power domain (CONFIG_PM_DEVICE_POWER_DOMAIN) models this more directly but
 * is not used here: it cannot guarantee the clock devices are rebuilt before
 * their consumers, and this path runs pre-scheduler with interrupts masked.
 */
static void ifx_pm_warm_boot_reinit_all(void)
{
	STRUCT_SECTION_FOREACH(device, dev) {
		if ((dev->pm_base == NULL) || (dev->pm_base->action_cb == NULL)) {
			continue;
		}

		/* TURN_OFF prepares a powered device for loss; here it already happened. */
 ...
 */
static void ifx_pm_s2ram_enter(void)
{
	int rc;
...
__weak void pm_state_set(enum pm_state state, uint8_t substate_id)
{
	...
	case PM_STATE_SOFT_OFF:
		if (substate_id == 1) {
			...
			/* Flash boot reads this even though DS-OFF then cold boots. */
			Cy_Syslib_SetWarmBootEntryPoint((uint32_t *)_vector_table, false);
			Cy_SysPm_SetDeepSleepMode(CY_SYSPM_MODE_DEEPSLEEP_OFF);
			if (ifx_pm_deep_arm_wfi() != 0) {
				LOG_WRN("DeepSleep-OFF entry rejected");
			}
			SCB_SCR &= (uint32_t)~SCB_SCR_SLEEPDEEP_Msk;
		} else {
...
/*
 * Release the Hibernate / DeepSleep-OFF pad freeze after wake (both cold boot).
 * Runs after all POST_KERNEL device init, so GPIO/pinctrl have reprogrammed the
 * pins and the pads skip the reset-default glitch. No-op when nothing is frozen.
 */
static int ifx_pm_io_unfreeze(void)
{
...
SYS_INIT(ifx_pm_io_unfreeze, POST_KERNEL, CONFIG_APPLICATION_INIT_PRIORITY);
