...
def _partition_bounds(node) -> tuple[int, int]:
    if not node or not node.regs:
        raise SystemExit(
            f"Devicetree node '{getattr(node, 'path', '?')}' has no 'reg' entries"
        )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--edt-pickle", required=True, help="Path to the pickled edtlib.EDT object"
    )
    parser.add_argument(
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            "Unable to locate 'ptable' labeled partition in devicetree"
        ) from exc
