...
void soc_random_enable(void)
{
	 ...
	 * Internal ADC sample freq = apb_clk / (APB_SARADC_CLKM_DIV_NUM + 1) /
	 * (APB_SARADC_SAR_CLK_DIV + 1) Read frequency has to be at least 35 times lower than the
	 * sampling frequency
	 */

	REG_SET_FIELD(APB_SARADC_APB_ADC_CLKM_CONF_REG, APB_SARADC_CLKM_DIV_NUM, 3);
	/* SAR clock divider has to be at least 2 */
	REG_SET_FIELD(APB_SARADC_CTRL_REG, APB_SARADC_SAR_CLK_DIV, 3);
	...
	/* Test internal voltage if the channel info is 0xa. */
	REG_SET_FIELD(APB_SARADC_CTRL_REG, APB_SARADC_SAR2_PATT_LEN, 0);
	WRITE_PERI_REG(APB_SARADC_SAR2_PATT_TAB1_REG,
		       0xafffff);
	/* Test internal voltage if the channel info is 0xa. */
...
void soc_random_disable(void)
{
	 ...
	 *  but as mentioned above it's better to enable Wi-Fi or BT or call random_enable() in
	 * order to get a secondary entropy source.
	 */
}
