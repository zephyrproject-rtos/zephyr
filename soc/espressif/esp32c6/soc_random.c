...
void soc_random_enable(void)
{
	...
	/* Config ADC circuit (Analog part) with I2C(HOST ID 0x69) and chose internal voltage as
	 * sampling source
	 */
	...
	/* we want channel 9 with max attenuation */
	uint32_t pattern_one = (SAR2_CHANNEL << 2) | SAR2_ATTEN;
	/* we want channel 0 with max attenuation, channel */
	uint32_t pattern_two = SAR1_ATTEN;
	/* doesn't really matter here */
	uint32_t pattern_table =
