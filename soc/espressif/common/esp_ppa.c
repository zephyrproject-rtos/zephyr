...
/* Reset is acknowledged within a few bus cycles. Bounded because this runs
 * under the queue spinlock, from the path that gives up on a transfer.
 */
...
static void esp_ppa_isr(void *arg)
{
	...
	/* Claim the semaphore under the lock, so a waiter that is timing out
	 * either sees it here or clears it there, never both.
	 */
	struct k_sem *sem = (done != NULL) ? done->done : NULL;
	ppa_event_callback_t cb = NULL;
	struct ppa_client_t *client = NULL;
	void *user_data = NULL;

	if (sem != NULL) {
		done->done = NULL;
	} else if (done != NULL) {
		/* The transaction is finished with here, so everything the
		 * callback needs is taken before the slot is handed back and
		 * the count the client is unregistered on is dropped. Both
		 * are shared with threads, so they are written under the lock.
		 */
		cb = done->cb;
		client = done->client;
		user_data = done->user_data;

		if (done->counted && client != NULL) {
...
static void esp_ppa_tx_reset(uint32_t ch)
{
	...
	if (spins == 0) {
		LOG_ERR("Transmit channel %u did not offer reset, resetting anyway", ch);
	}
...
static void esp_ppa_rx_reset(uint32_t ch)
{
	...
	if (spins == 0) {
		LOG_ERR("Receive channel %u did not offer reset, resetting anyway", ch);
	}
 ...
 * written past the end. The caller invalidates the buffer once the engine has
 * filled it, and an invalidate covers whole cache lines, so a buffer that does
 * not start and end on a line boundary drops whatever shares its first and
 * last line.
 */
static bool esp_ppa_out_buffer_valid(const void *buffer, uint32_t buffer_size, uint32_t pic_w,
				     uint32_t pic_h, uint32_t pbyte)
{
	uint32_t bytes_per_pixel = (pbyte == DMA2D_DESCRIPTOR_PBYTE_1B0_PER_PIXEL)   ? 1U
				   : (pbyte == DMA2D_DESCRIPTOR_PBYTE_2B0_PER_PIXEL) ? 2U
				   : (pbyte == DMA2D_DESCRIPTOR_PBYTE_3B0_PER_PIXEL) ? 3U
										     : 4U;
	size_t line = esp_cache_get_line_size_by_addr(buffer);

	if (line != 0 && (((uintptr_t)buffer % line) != 0 || (buffer_size % line) != 0)) {
		return false;
	}

	return (uint64_t)buffer_size >= (uint64_t)pic_w * pic_h * bytes_per_pixel;
}
...
esp_err_t ppa_unregister_client(ppa_client_handle_t ppa_client)
{
	...
	/* The interrupt drops the count as it finishes a transaction, so the
	 * check and the decision to free have to be one step, otherwise a
	 * transaction completing between them leaves the interrupt reaching
	 * into a freed client.
	 */
	k_spinlock_key_t key = k_spin_lock(&ppa_state.queue_lock);
	...
	for (int i = 0; i < ESP_PPA_MAX_PENDING; i++) {
		if (ppa_state.txns[i].in_use && ppa_state.txns[i].client == ppa_client) {
			k_spin_unlock(&ppa_state.queue_lock, key);
			return ESP_ERR_INVALID_STATE;
		}
	}
...
static esp_err_t esp_ppa_run(struct esp_ppa_txn *txn, ppa_trans_mode_t mode)
{
	...
	if (ret != 0) {
		...
		if (txn->done == NULL) {
			/* The interrupt claimed it as the wait expired, so the
			 * transfer did finish and the engine has moved on.
			 */
			k_spin_unlock(&ppa_state.queue_lock, key);
			k_sem_take(&done, K_NO_WAIT);
			txn->in_use = false;
			return ESP_OK;
		}
...
esp_err_t ppa_do_fill(ppa_client_handle_t ppa_client, const ppa_fill_oper_config_t *config)
{
	...
	if (config->out.buffer == NULL ||
	    ...
	    !esp_ppa_out_buffer_valid(config->out.buffer, config->out.buffer_size,
				      config->out.pic_w, config->out.pic_h, out_pbyte)) {
		return ESP_ERR_INVALID_ARG;
	}
				     ...
				     const ppa_srm_oper_config_t *config)
{
	...
	if (config->rotation_angle == PPA_SRM_ROTATION_ANGLE_90 ||
		...
		uint32_t swap = out_block_w;

		out_block_w = out_block_h;
		out_block_h = swap;
	}
