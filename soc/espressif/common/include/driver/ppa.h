...
typedef struct {
	...
	/**
	 * The maximum number of pending transactions for the client. By default, it will be 1,
	 * which is sufficient if all transactions are performed with `PPA_TRANS_MODE_BLOCKING`
	 */
	uint32_t max_pending_trans_num;
	/**
	 * The desired data burst length for all the transactions of the client. Use a small burst
	 * length will decrease PPA performance, but can save burst bandwidth for other peripheral
	 * usages. By default, it will be at the maximum burst length, `PPA_DATA_BURST_LENGTH_128`
	 */
 ...
 * - ESP_ERR_INVALID_STATE: Unregister the PPA client failed because there are unfinished
 * transactions
 */
 ...
 *
 * @return Whether a task switch is needed after the callback function returns, this is usually due
 * to the callback
 *         wakes up some high priority task.
 */
typedef bool (*ppa_event_callback_t)(ppa_client_handle_t ppa_client, ppa_event_data_t *event_data,
				     void *user_data);
 ...
 *
 * @note Any user private data that wants to be passed directly to callback's user_data is provided
 * per PPA transaction.
 ...
 * - ESP_ERR_INVALID_ARG: Register event callbacks for the PPA client failed because of invalid
 * argument
 */
...
/**
 * @brief A collection of configuration items for an input picture and the target block inside the
 * picture
 */
typedef struct {
	...
	/**
	 * When the color mode is any YUV color space, this field is to describe its YUV<->RGB
	 * conversion standard
	 */
...
/**
 * @brief A collection of configuration items for an output picture and the target block inside the
 * picture
 */
typedef struct {
	/**
	 * Pointer to the output picture buffer (requires alignment: internal memory needs align to
	 * L1 cache line size, external memory needs align to L1 and L2 cache line size)
	 */
	void *buffer;
	/**
	 * Size of the output picture buffer (requires alignment: internal memory needs align to L1
	 * cache line size, external memory needs align to L1 and L2 cache line size)
	 */
...
typedef enum {
	...
	/**
	 * `ppa_do_xxx` function will return immediately after the PPA operation is pushed to the
	 * internal queue
	 */
...
typedef struct {
	...
	bool rgb_swap;
	/**
	 * Whether to swap the input data in byte. Only available feature if input picture color
	 * mode is ARGB8888 or RGB565
	 */
	bool byte_swap;
	...
	union {
		/**
		 * Range: [0, 255]
		 * When PPA_ALPHA_FIX_VALUE mode is selected, alpha_fix_val is the
		 * new alpha value to replace the input alpha value
		 * (output_alpha = alpha_fix_val)
		 */
		uint32_t alpha_fix_val;
		/**
		 * Range: (0, 1)
		 * When PPA_ALPHA_SCALE mode is selected, alpha_scale_ratio is the
		 * multiplier to the input alpha value
		 * (output_alpha = alpha_scale_ratio * input_alpha)
 ...
 * @param[in] config Pointer to a collection of configurations for the SRM operation transaction,
 * ppa_srm_oper_config_t
 *
 * @return
 ...
 * - ESP_FAIL: Perform a SRM operation failed because the client's pending transactions has reached
...
typedef struct {
	...
	/* input data manipulation */
	/**
	 * Whether to swap the background input data in RGB (e.g. ARGB becomes BGRA, RGB becomes
	 * BGR)
	 */
	bool bg_rgb_swap;
	/**
	 * Whether to swap the background input data in byte. Only available feature if input BG
	 * picture color mode is ARGB8888 or RGB565
	 */
	bool bg_byte_swap;
	...
	/**
	 * Whether to swap the foreground input data in RGB (e.g. ARGB becomes BGRA, RGB becomes
	 * BGR)
	 */
	bool fg_rgb_swap;
	/**
	 * Whether to swap the foreground input data in byte. Only available feature if input FG
	 * picture color mode is ARGB8888 or RGB565
	 */
	bool fg_byte_swap;
	...
	/**
	 * When in_fg.blend_cm is PPA_BLEND_COLOR_MODE_A8/4, this field can be used to set a fixed
	 * color for the foreground, in RGB888 format
	 */
	color_pixel_rgb888_data_t fg_fix_rgb_val;

	/* color-keying
	 * A pixel, where its background element and foreground element are
	 * both out of their color-keying ranges, will follow Alpha Blending
	 */
	/**
	 * Whether to enable color keying for background If not enabled, all background pixels are
	 * considered as out of the color-keying range
	 */
	bool bg_ck_en;
	...
	/**
	 * Whether to enable color keying for foreground If not enabled, all foreground pixels are
	 * considered as out of the color-keying range
	 */
	bool fg_ck_en;
	...
	/**
	 * The color to overwrite when a pixel, where its background element and foreground element
	 * are both within their color-keying ranges, in RGB888 format
	 */
	color_pixel_rgb888_data_t ck_rgb_default_val;
	/**
	 * If this bit is set, in color-keying, for the pixel, where its background element is in
	 * the color range, but its foreground element is not in the color range, it will output the
	 * foreground element instead of the background element
	 */
 ...
 * @param[in] config Pointer to a collection of configurations for the blend operation transaction,
 * ppa_blend_oper_config_t
 *
 * @return
 ...
 * - ESP_FAIL: Perform a blend operation failed because the client's pending transactions has
...
typedef struct {
	...
	union {
		/**
		 * For any ARGB/RGB format color to be filled, use this field to fill A (if
		 * applicable)/R/G/B components
		 */
		color_pixel_argb8888_data_t fill_argb_color;
		/** For GRAY8 format color to be filled */
		color_pixel_gray8_data_t fill_gray8_color;
		/**
		 * For any YUV format color to be filled, use this field to fill Y/U/V components
		 */
		color_macroblock_yuv_data_t fill_yuv_color;
		/**
		 * The color to be filled, in a raw 32-bit value. The interpretation of the value
		 * depends on the selected `fill_cm`
		 */
 ...
 * @param[in] config Pointer to a collection of configurations for the fill operation transaction,
 * ppa_fill_oper_config_t
 *
 * @return
 ...
 * - ESP_FAIL: Perform a fill operation failed because the client's pending transactions has reached
 * its maximum capacity
 */
esp_err_t ppa_do_fill(ppa_client_handle_t ppa_client, const ppa_fill_oper_config_t *config);

/**
 * @brief Configure the RGB888 to GRAY8 color conversion coefficients for ppa_do_scale_rotate_mirror
 * and ppa_do_fill
 *
 ...
 * - ESP_ERR_NOT_SUPPORTED: Set the RGB888 to GRAY color conversion formula failed because the PPA
 * peripheral does not support this feature
 * - ESP_ERR_INVALID_ARG: Set the RGB888 to GRAY color conversion formula failed because of invalid
 * argument
 * - ESP_ERR_INVALID_STATE: Set the RGB888 to GRAY color conversion formula failed because the PPA
