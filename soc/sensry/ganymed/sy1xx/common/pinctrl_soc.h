...
#define SY1XX_PAD_PULLUP_OFFS   5
#define SY1XX_PAD_PULLDOWN_OFFS 4
...
#define Z_PINCTRL_CFG(node)                                                                        \
	(                                                                                          \
                                                                                                   \
		DT_PROP(node, input_schmitt_enable) << SY1XX_PAD_SCHMITT_OFFS |                    \
		DT_PROP(node, bias_pull_up) << SY1XX_PAD_PULLUP_OFFS |                             \
		DT_PROP(node, bias_pull_down) << SY1XX_PAD_PULLDOWN_OFFS |                         \
		DT_PROP(node, bias_high_impedance) << SY1XX_PAD_TRISTATE_OFFS |                    \
		DT_PROP(node, input_enable) << SY1XX_PAD_DIR_OFFS                                  \
                                                                                                   \
	)
