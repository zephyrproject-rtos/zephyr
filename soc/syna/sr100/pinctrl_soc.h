...
#define SRXXX_SCHMITT_TRIGGER_SHIFT 7
...
#define SRXXX_DT_PINCFG_FLAGS(node)                                                                \
	((DT_NODE_HAS_PROP(node, slew_rate) << SRXXX_SLEW_RATE_SHIFT) |                            \
	 (DT_NODE_HAS_PROP(node, input_enable) << SRXXX_INPUT_ENABLE_SHIFT) |                      \
...
#define SRXXX_DT_PINCFG_INIT(node)                                                                 \
	((DT_PROP_OR(node, slew_rate, 0) << SRXXX_SLEW_RATE_SHIFT) |                               \
	 (DT_PROP_OR(node, input_enable, 0) << SRXXX_INPUT_ENABLE_SHIFT) |                         \
...
#define SRXXX_DT_OFFSET_INIT(node_id) DT_PROP_OR(node_id, pincfg, 0)
...
#define Z_PINCTRL_STATE_PIN_INIT(node_id, prop, idx)                                               \
	{                                                                                          \
		...
		.offset = SRXXX_DT_OFFSET_INIT(DT_PROP_BY_IDX(node_id, prop, idx)),                \
	},
