...
for the STMicroelectronics Arm |reg| Cortex |reg| -M0+ core-based STM32G071RB USB Type-C |trade|
...
interoperability with other USB Type-C |trade| devices. The STM32G071B-DISCO Discovery
board is intended for discovery and display of USB Type-C |trade| port characteristics
...
as a USB Type-C |trade| and Power Delivery analyzer.
...
- USB Type-C |trade| interface plug cable and receptacle connector accessible by door
...
- On-board DC/DC converter to sustain power supply with VBUS varying from 3 V to
  20 V (+/- 5 %)
- 4 user status LEDs about USB Type-C |trade| configuration
- 3 LEDs for power and ST-LINK communication
- 4-way joystick with selection button
- 1 reset push-button
- Board external connectors:
    - USB Type-C |trade| plug cable
    - USB Type-C |trade| receptacle connector
...
- Board internal connectors:
    ...
    - USB Type-C |trade| test points for main signals
- Flexible power-supply options: ST-LINK USB VBUS or USB Type-C |trade| VBUS
