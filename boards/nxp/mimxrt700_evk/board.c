...
/*
 * Releases the second core, which expects this core's devices to be up first.
 * An anchored entry runs at the end of PRE_KERNEL, after every device ordered
 * by priority or by devicetree, which is the position PRE_KERNEL_2 was
 * standing in for.
 */
#define SYS_ANCHOR_mimxrt700_second_core SYS_ANCHOR(mimxrt700_second_core)
SYS_INIT_ANCHORED(mimxrt700_second_core, second_core_boot, PRE_KERNEL);
#endif
