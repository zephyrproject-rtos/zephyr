...
- ``zephyr,sram`` is the 32 MB XSPI1 PSRAM; ``.data`` and ``.bss`` live there.
- ``zephyr,kernel-stacks`` is the 256 KB DTCM, so no stack sits on an
  XSPI-clocked bus. It holds every kernel stack, and without
  :kconfig:option:`CONFIG_USERSPACE` that includes each
  :c:macro:`K_THREAD_STACK_DEFINE` stack an application defines, so the bank is
  the budget for all of them. Note that the DTCM is core-local: a driver that
  DMAs out of a stack buffer cannot reach it.
- ``zephyr,kernel-heap`` is the 256 KB SRAM2 bank, so neither is the heap. It
  holds both the libc ``malloc()`` arena and the ``k_malloc()``/``k_heap`` pool:
  :kconfig:option:`CONFIG_HEAP_MEM_POOL_SIZE` takes what it asks for and the
  arena takes the rest of the bank.
- ITCM holds the clock bring-up window code: reprogramming the PLLs requires
  parking both XSPI controllers, so that code cannot be in external memory
  either.

Overflowing the DTCM or SRAM2 fails the link rather than spilling silently. An
application that needs more stack or heap than a bank holds puts that consumer
back in the PSRAM by deleting the chosen property in an overlay:

.. code-block:: devicetree

   / {
           chosen {
                   /delete-property/ zephyr,kernel-heap;
           };
   };
...
LinkServer is the default runner for this board. LinkServer v26.09 or newer
is required; older versions do not include the MIMXRT2663 device. Install
:ref:`linkserver-debug-host-tools` and make sure they are in your search
path, then build and flash with:
...
The onboard debug circuit can be updated with Segger J-Link firmware by
following the instructions in :ref:`mcu-link-jlink-onboard-debug-probe`, or
attach an external :ref:`jlink-external-debug-probe` to the SWD header.
...
Open a serial terminal on the MCU-Link virtual COM port at 115200 8N1, reset
the board, and the sample prints:
