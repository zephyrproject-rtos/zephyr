...
- ``zephyr,sram`` is the 32 MB XSPI1 PSRAM; ``.data``, ``.bss`` and the kernel
  stacks and heap live there.
- ITCM holds the clock bring-up window code, and a scratch stack in the DTCM
  carries the window itself: reprogramming the PLLs requires parking both XSPI
  controllers, so for as long as the window is open neither the code nor the
  stack it runs on may be in external memory. Nothing else has to move, so an
  application keeps the full PSRAM for its own stacks and heap and the TCMs stay
  free for it to use.
