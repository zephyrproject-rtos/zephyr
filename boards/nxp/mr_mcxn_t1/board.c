...
usb_phy_config_struct_t usb_phy_config = {
...
#if defined(CONFIG_FLASH_MCUX_FLEXSPI_NOR) || defined(CONFIG_FLASH_MCUX_FLEXSPI_XIP)
...
/* FlexCAN clocked from PLL1/2 = 48 MHz (see PLL1 setup above). */
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(flexcan0))
	...
	(defined(CONFIG_USB_DC_NXP_EHCI) || defined(CONFIG_UDC_NXP_EHCI))
...
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(usb1)) && defined(CONFIG_USB_DC_NXP_EHCI)
	USB_EhciPhyInit(kUSB_ControllerEhci0, BOARD_XTAL0_CLK_HZ, &usb_phy_config);
#endif
#endif
 ...
 */
	 ...
	 * MCXN947 boot ROM on every warm reset (observed on MR-MCXN-T1
	 * hardware: power-on boots are fine, any SYSRESETREQ afterward hangs
	 * in ROM until full power removal - the OSC enable persists in the
	 * VBAT domain across resets). The RTC counts FRO16K instead until the
