...
/* Each ITRC input event has a 2-bit selection field in the OUT_SEL registers:
 * 0b01 selects the event as a trigger source, 0b10 deselects it. This pattern
 * deselects all implemented input events (reserved fields are written as 0).
 */
#define ITRC_OUT_SEL_EVENTS_DESELECTED 0xAAAAAA0AU
...
#ifdef CONFIG_SOC_MIMXRT798S_CM33_CPU0
static const clock_main_pll_config_t g_mainPllConfig_clock_init = {
...
static const clock_audio_pll_config_t g_audioPllConfig_clock_init = {
...
void board_early_init_hook(void)
{
...
#ifdef CONFIG_BOARD_FRDM_IMXRT700_MIMXRT798S_CM33_CPU0
	...
	CLOCK_SetClkDiv(kCLOCK_DivSdio0Clk, 1U);
#endif
	...
	CLOCK_SetClkDiv(kCLOCK_DivSdio1Clk, 1U);
	 ...
	 */
	ITRC->OUT_SEL[3][0] = ITRC_OUT_SEL_EVENTS_DESELECTED;
	ITRC->OUT_SEL[4][0] = ITRC_OUT_SEL_EVENTS_DESELECTED;
#endif
...
#if DT_NODE_HAS_STATUS_OKAY(DT_NODELABEL(co5300_zc143ac72mipi))
	POWER_DisablePD(kPDRUNCFG_SHUT_MEDIA_MAINCLK);
	POWER_DisablePD(kPDRUNCFG_APD_LCDIF);
	POWER_DisablePD(kPDRUNCFG_PPD_LCDIF);
	POWER_ApplyPD();
	...
	CLOCK_InitMainPfd(kCLOCK_Pfd2, 17U);
	CLOCK_SetClkDiv(kCLOCK_DivMediaMainClk, 2U);
	CLOCK_AttachClk(kMAIN_PLL_PFD2_to_MEDIA_MAIN);
#endif
...
static void BOARD_InitAHBSC(void)
{
	...
	AHBSC0->MISC_CTRL_DP_REG = 0x000086aaU;
	/* AHBSC0 MISC_CTRL_REG, disable Privilege & Secure checking. */
	AHBSC0->MISC_CTRL_REG = 0x000086aaU;

	GlikeyWriteEnable(GLIKEY0, 7U);
	/* Enable arbiter0 accessing SRAM */
	AHBSC0->COMPUTE_ARB0RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC0->SENSE_ARB0RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC0->MEDIA_ARB0RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC0->NPU_ARB0RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC0->HIFI4_ARB0RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
#endif

	GlikeyWriteEnable(GLIKEY1, 1U);
	AHBSC3->MISC_CTRL_DP_REG = 0x000086aaU;
	/* AHBSC3 MISC_CTRL_REG, disable Privilege & Secure checking.*/
	AHBSC3->MISC_CTRL_REG = 0x000086aaU;

	GlikeyWriteEnable(GLIKEY1, 9U);
	/* Enable arbiter1 accessing SRAM */
	AHBSC3->COMPUTE_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC3->SENSE_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC3->MEDIA_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC3->NPU_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC3->HIFI4_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;
	AHBSC3->HIFI1_ARB1RAM_ACCESS_ENABLE = 0x3FFFFFFFU;

	GlikeyWriteEnable(GLIKEY1, 8U);
	/* Access enable for COMPUTE domain masters to common APB peripherals.*/
	AHBSC3->COMPUTE_APB_PERIPHERAL_ACCESS_ENABLE = 0xffffffffU;
	AHBSC3->SENSE_APB_PERIPHERAL_ACCESS_ENABLE = 0xffffffffU;
	GlikeyWriteEnable(GLIKEY1, 7U);
	AHBSC3->COMPUTE_AIPS_PERIPHERAL_ACCESS_ENABLE = 0xffffffffU;
	AHBSC3->SENSE_AIPS_PERIPHERAL_ACCESS_ENABLE = 0xffffffffU;

	GlikeyWriteEnable(GLIKEY2, 1U);
	/*Disable secure and secure privilege checking. */
	AHBSC4->MISC_CTRL_DP_REG = 0x000086aaU;
	AHBSC4->MISC_CTRL_REG = 0x000086aaU;
...
static int second_core_boot(void)
{
	...
	PMC0->PDRUNCFG2 &= ~0x3FFC0000U;
	PMC0->PDRUNCFG3 &= ~0x3FFC0000U;
	...
	SYSCON3->CPU1_NSVTOR = (boot_address >> 7U);
	SYSCON3->CPU1_SVTOR = (boot_address >> 7U);
