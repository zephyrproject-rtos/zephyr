...
the back of the board. It has an ES8388 audio codec, two differential
microphones and two speaker connectors. The audio section is not described by
the devicetree yet.

The ``aipi_eyes_s1`` and ``aipi_eyes_s2`` boards use the same module and share
their devicetree. They differ in their on-board peripherals: the
``aipi_eyes_s2`` has no audio.
