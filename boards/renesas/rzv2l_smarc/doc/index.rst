...
The ``rzv2l_smarc/r9a07g054l23gbg/cm33`` board target supports the ARM Cortex-M33 System Core without FPU
...
to Cortex-M33 run Zephyr with u-boot. The minimal steps are described below.
  ...
  2. At step (4), follow step ''2. Download Multi-OS Package'' and ''3. Add the layer for Multi-OS Package''
     of ''3.2 OpenAMP related stuff Integration for RZ/V2L'' of `Release Note for RZ/V Multi-OS Package`_
     to add the layer for Multi-OS Package.

  .. code-block:: bash

      cd ~/rzv_vlp_<pkg ver>
      unzip <Multi-OS Dir>/r01an7254ej0210-rzv-multi-os-pkg.zip
  ...
  10. Follow "4.3.2 CM33 Sample Program Invocation with u-boot" from the beginning to step 4 of `Release Note for RZ/V Multi-OS Package`_

  11. Running these commands to start zephyr application with CM33 core. ''N'' denotes the partition number in which you stored zephyr.bin file.
...
.. _Release Note for RZ/V Multi-OS Package:
   https://www.renesas.com/en/document/rln/release-note-rzv-multi-os-package-v210
