...
void board_cc13xx_rf_callback(RF_Handle client, RF_GlobalEvent events, void *arg)
{
	...
	if (events & RF_GlobalEventRadioSetup) {
		...
		if (paType == RF_TxPowerTable_HighPA) {
			...
			IOCPortConfigureSet(BOARD_RF_HIGH_PA, IOC_PORT_RFC_GPO3, IOC_IOMODE_NORMAL);
			IOCPortConfigureSet(BOARD_RF_SUB1GHZ, IOC_PORT_RFC_GPO0, IOC_IOMODE_NORMAL);
		} else {
			/* RF core active --> Sub-1 GHz */
			IOCPortConfigureSet(BOARD_RF_HIGH_PA, IOC_PORT_GPIO, IOC_IOMODE_NORMAL);
			IOCPortConfigureSet(BOARD_RF_SUB1GHZ, IOC_PORT_GPIO, IOC_IOMODE_NORMAL);
