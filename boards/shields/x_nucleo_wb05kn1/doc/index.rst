...
.. note::
   ``DTM_UART_WITH_UPDATER_CONTROLLER`` and ``DTM_SPI_WITH_UPDATER_CONTROLLER`` are the compatible
   firmware, available under ``Utilities/DTM_Controller_Only`` directory in the SDK provided by ST at
   `X-CUBE-WB05N`_.
