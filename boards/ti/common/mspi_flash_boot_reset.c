...
LOG_MODULE_REGISTER(mspi_boot_reset, CONFIG_FLASH_LOG_LEVEL);

/* Generic 8D-8D-8D to 1S-1S-1S reset for any MSPI flash */
static int mspi_reset_flash_from_8d8d8d(const struct device *mspi_dev,
...
static int mspi_flash_boot_reset_init(void)
{
	LOG_INF("MSPI Flash boot mode reset initialization");
...
#define APPLY_RESET_TO_MSPI_CHILD(node_id)                                                         \
	do {                                                                                       \
		...
		LOG_INF("Attempting to reset flash from 8D-8D-8D mode for MSPI device at idx %d",  \
			dev_id.dev_idx);                                                           \
		uint32_t reset_recovery_us = DT_PROP_OR(node_id, t_reset_recovery, 0) / 1000;      \
		int ret = mspi_reset_flash_from_8d8d8d(flash_controller, &dev_id,                  \
						       reset_recovery_us);                         \
		if (ret < 0) {                                                                     \
...
#define MSPI_FLASH_RESET_INIT_PRIORITY UTIL_DEC(CONFIG_FLASH_INIT_PRIORITY)

SYS_INIT(mspi_flash_boot_reset_init, POST_KERNEL, MSPI_FLASH_RESET_INIT_PRIORITY);
