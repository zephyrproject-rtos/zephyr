...
Select the ``hexagon`` toolchain variant and point ``HEXAGON_TOOLCHAIN_PATH``
...
explicitly because Hexagon is built with an LLVM cross-compiler rather than
the Zephyr SDK:
...
.. zephyr-app-commands::
   ...
   :goals: build
   :gen-args: -DZEPHYR_TOOLCHAIN_VARIANT=hexagon -DHEXAGON_TOOLCHAIN_PATH=/opt/hexagon-toolchain

.. note::

   Replace ``/opt/hexagon-toolchain`` with the actual value of
   ``$HEXAGON_TOOLCHAIN_PATH`` set in Step 1. Both arguments can be left out
   when the environment variables of Step 1 are exported.
