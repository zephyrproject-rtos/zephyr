...
- `STM32WB5MMG reference manual`_
...
STM32WB5MMG board has 2 (LP)U(S)ARTs. The Zephyr console output is assigned to USART1.
...
STM32WB5MMG needs an external ST-Link for flashing the firmware. If you are using
one of the  ``b_u585i_iot02a`` or ``nucleo_wb55rg`` boards, you can use the on-board
ST-Link to flash the  STM32WB5MMG module. 
...
.. code-block:: console
   ...
   $ pyocd pack --update
   $ pyocd pack --install stm32wb55vg
...
Then first build and flash the application for the STM32WB5MMG module.
...
Then, build the bluetooth `observer` demo application for B-U585I-IOT02A board:
...
:zephyr:code-sample:`hci_uart` application.
