...
A. Connect ArgonKey UART to your Linux PC using, for example, a USB-TTL serial
   cable. The flashing procedure has been tested using a TTL-232RG cable with
   ...
   When the USB cable is inserted to the Linux PC the following device wiil be
   created: /dev/ttyUSBx (x is usally '0').

B. Force STM32F412CG to enter in Bootloader mode

    - Connect BOOT0 to 1V8 (link P2.1 to P3.30)
    - Press and release the RST button

C. Use stm32flash utility to flash the ArgonKey:

.. code-block:: console

    $ stm32flash  -w zephyr.bin -v -g 0x08000000 /dev/ttyUSB0

See References section for more info on stm32flash.
...
The ArgonKey has been tested using the ST-LINK/V2 tool. Once that the tool
