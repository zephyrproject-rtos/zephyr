...
Overview
********
- The M3 Starter Kit board is designed for evaluating the features and performance of the R-CAR M3 device from Renesas Electronics and it is also used for developing and evaluating application software for these R-CAR M3.

- The M3 Starter Kit, based on the R-CAR M3 SIP, comes with LPDDR4 @2GB in 2-channel, each 64-bit wide+Hyperflash @64MB, CSI2 interfaces and several communication interfaces like USB, Ethernet, HDMI and can work standalone or can be adapted to other boards, via 440pin connector on bottom side.

It is possible to order 2 different types of M3 Starter Kit Boards, one with Ethernet connection onboard and one with Ethernet connection on ComExpress.

.. figure:: img/rcar_m3ulcb_starter_kit.jpg
   :width: 460px
   :align: center
   :height: 288px
   :alt: R-Car starter kit

.. note:: The M3ULCB board can be plugged on a Renesas Kingfisher Infotainment daughter board through COM Express connector in order to physically access more I/O. CAUTION: In this case, power supply is managed by the daughter board.
...
Hardware capabilities for the M3ULCB for can be found on the `eLinux M3SK page`_ of the board.

.. figure:: img/rcar_m3ulcb_features.jpg
   :width: 294px
   :align: center
   :height: 280px
...
Here are official IOs figures from eLinux for M3ULCB board:
...
By running Zephyr on M3ULCB, the software readable push button 'SW3' can be used as input, and the software contollable LED 'LED5' can be used as output.

UART
----

M3ULCB board is providing two serial ports, only one is commonly available on the board, however, the second one can be made available either by welding components or by plugging the board on a Kingfisher Infotainment daughter board.
...
M3ULCB board provides two CAN interfaces. Both interfaces are available on the Kingfisher daughter board.
...
M3ULCB board provides two I2C buses. Unfortunately direct access to these buses is not available through connectors.

I2C is mainly used to manage and power on multiple of onboard chips on the M3ULCB and Kingfisher daughter board.
...
The "Olimex ARM-USB-OCD-H" probe needs to be connected with a SICA20I2P adapter to CN3 on M3ULCB.
...
Applications for the ``rcar_m3ulcb_cr7`` board configuration can be built in the usual way (see :ref:`build_an_application` for more details).

.. zephyr-app-commands::
   :zephyr-app: samples/hello_world
   :board: rcar_m3ulcb_cr7
   :goals: flash
...
.. _Renesas R-Car M3 chip:
   https://www.renesas.com/eu/en/products/automotive-products/automotive-system-chips-socs/r-car-m3e-r-car-m3e-m3e-2g-automotive-system-chip-soc-ideal-medium-class-automotive-computing-systems

.. _eLinux M3SK page:
	https://elinux.org/R-Car/Boards/M3SK

.. _M3SK top view:
	https://elinux.org/images/thumb/d/d6/ULCB_M3_top_lables.jpg/700px-ULCB_M3_top_lables.jpg

.. _M3SK bottom view:
	https://elinux.org/images/thumb/a/a8/ULCB_H3_bot_lables.jpg/700px-ULCB_H3_bot_lables.jpg
