...
/*
 * ARM64 atomic operations with explicit DMB SY barriers, for FVP.
 *
 ...
 *
 * Inserting an explicit DMB SY before each atomic read forces the
 * simulator to end the quantum early, letting other CPUs make progress
 * and breaking the apparent livelock. Without it, kernel scheduler code
 * (e.g. switch_handle in kernel/sched.c) and lockfree algorithms (e.g.
 * tests/lib/lockfree, k_sem and k_msgq fast paths) hang or time out on
 * fvp_base_revc_2xaem.
 *
 * Caveats:
