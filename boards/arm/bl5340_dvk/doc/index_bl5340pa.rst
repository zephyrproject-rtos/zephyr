...
Laird Connectivity BL5340PA DVK
###############################

Overview
********
The BL5340PA Development Kit provides support for the Laird
Connectivity BL5340PA module which is powered by a dual-core Nordic
Semiconductor nRF5340 ARM Cortex-M33F CPU and a Nordic Semiconductor
nRF21540 FEM (Front end module) to boost the output power. The nRF5340
inside the BL5340 module is a dual-core SoC based on the Arm®
...
The bl5340pa_dvk_cpuapp build target provides support for the
application core on the BL5340PA module. The bl5340pa_dvk_cpunet build
target provides support for the network core on the BL5340 module. If
ARM TrustZone is used then the bl5340pa_dvk_cpuapp build target
provides support for the non-secure partition of the application core
on the BL5340PA module.
...
* RADIO (Bluetooth Low Energy and 802.15.4 with FEM)
...
.. figure:: img/bl5340pa_dvk_side.png
     :width: 420px
     :align: center
     :alt: BL5340PA DVK

     BL5340PA DVK (Credit: Laird Connectivity)
...
The BL5340PA DVK has two external oscillators. The frequency of the
slow clock is 32.768KHz. The frequency of the main clock is 32MHz.

Supported Features
==================

The bl5340pa_dvk_cpuapp board configuration supports the following
hardware features:
...
The bl5340pa_dvk_cpunet board configuration supports the following
hardware features:
...
and outputs to the BL5340PA module.
...
Several external memory sources are available for the BL5340PA DVK.
...
Refer to the `Macronix MX25R6435FZNIL0 datasheet`_ for further
details.
...
Refer to the `Giantec GT24C256C-2GLI-TR datasheet`_ for further
details.

External Memory
---------------

An on-board micro SD card slot is available for use with micro SD
cards.

Sensors
=======

The BL5340PA DVK incorporates two sensors for user application
testing. These are described as follows.
...
An ST Microelectronics LIS3DH 3-Axis Accelerometer is available via
I2C for vibration and motion detection applications.

Refer to the `ST Microelectronics LIS3DH datasheet`_ for further
details.

Ethernet
========

Cabled 10/100 Base-T Ethernet Connectivity is available via a
...
controller is included with the BL5340PA DVK for user interface
application features.

Refer to the `ER_TFTM028_4 datasheet`_ for a high level overview of
the display.
...
- Implementation Defined Attribution Unit (`IDAU`_) on the application
  ...
  used to define secure and non-secure memory maps.  By default, all
  of the memory space (Flash, SRAM, and peripheral address space) is
...
The BL5340PA's application core supports the Armv8-M Security
Extension. Applications built for the bl5340pa_dvk_cpuapp board by
default boot in the Secure state.

The BL5340PA's network core does not support the Armv8-M Security
...
only applications on the BL5340PA module.

Building Secure/Non-Secure Zephyr applications with Arm |reg| TrustZone |reg|
=============================================================================

Applications on the BL5340PA module may contain a Secure and a
Non-Secure firmware image for the application core. The Secure image
can be built using either Zephyr or `Trusted Firmware M`_ (TF-M).
Non-Secure firmware images are always built using Zephyr. The two
alternatives are described below.

.. note::

   By default the the Secure image for BL5340PA's application core is
   built using TF-M.
...
1. Build the Non-Secure Zephyr application
   ...
   To invoke the building of TF-M the Zephyr build system requires
   the Kconfig option ``BUILD_WITH_TFM`` to be enabled, which is done
   by default when building Zephyr as a Non-Secure application.
   The Zephyr build system will perform the following steps
   automatically:

      * Build the Non-Secure firmware image as a regular Zephyr
        application
...
1. Build the Secure Zephyr application for the application core
   using ``-DBOARD=bl5340pa_dvk_cpuapp`` and
...
2. Build the Non-Secure Zephyr application for the application core
   using ``-DBOARD=bl5340pa_dvk_cpuapp_ns``.
3. Merge the two binaries together.
4. Build the application firmware for the network core using
   ``-DBOARD=bl5340pa_dvk_cpunet``.

When building a Secure/Non-Secure application for the BL5340PA's
...
and :ref:`application_run`), using ``-DBOARD=bl5340pa_dvk_cpuapp`` for
the firmware running on the BL5340PA's application core, and using
``-DBOARD=bl5340pa_dvk_cpunet`` for the firmware running
on the BL5340PA's network core.
...
.. note::
   ...
   Flashing and debugging applications on the BL5340PA DVK requires
   upgrading the nRF Command Line Tools to version 10.12.0 or newer.
   Further information on how to install the nRF Command Line Tools
   can be found in :ref:`nordic_segger_flashing`.

Here is an example for the :ref:`hello_world` application running on
the BL5340PA's application core.
...
Replace :code:`<tty_device>` with the port where the BL5340PA DVK
...
Testing Bluetooth on the BL5340PA DVK
*************************************
Many of the Bluetooth examples will work on the BL5340PA DVK.
Try them out:
...
Note that the FEM on the BL5340PA module can boost the power of the
output signal by up to 20dBm, however this is not compliant with
tested regulatory limits. The BL5340PA module ships with the FEM set
to a 10dBm gain mode and must not be altered to output more power to
remain in regulatory compliance. Note that Laird Connectivity accepts
no responsibility for regulatory issues with end products, such
responsibility is entirely the responsibility of the device/end
product manufacturer and/or designer.
