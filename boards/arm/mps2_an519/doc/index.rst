...
The mps2_an519 board configuration is used by Zephyr applications that run
on the MPS2+ AN519 board. It provides support for the MPS2+ AN519 ARM Cortex-M23
...
.. image:: img/mps2_an519.png
     :width: 666px
     :align: center
     :height: 546px
     :alt: ARM MPS2+ AN519

The MPS2+ AN519 is an FPGA implementation of the IoT Kit Subsystem that
uses the SIE200 components together with CMSDK peripherals to provide an
example design.
...
.. note::
   ...
   with actual MPS2 hardware systems using AN519, or any other hardware
   system. It has been tested on actual hardware, but its primary purpose is
   for use with QEMU and unit tests for the ARM Cortex-M23.


Hardware
********

ARM MPS2+ AN519 provides the following hardware components:



- ARM Cortex-M23
- Soft Macro Model (SMM) implementation of SIE-200 subsystem
- Memory
...
The mps2_an519 board provides the following user push buttons:

- ON power on
- nSRST: Cortex-M23 system reset and CoreSight debug reset
...
The mps2_an519 board configuration supports the following hardware features:
...
See the `MPS2 FPGA Website`_ for a complete list of MPS2+ AN519 board hardware
features.

The default configuration can be found in the defconfig file:
``boards/arm/mps2_an519/mps2_an519_defconfig``.

Interrupt Controller
====================

MPS2+ AN519 is a Cortex-M23 based SoC and has 15 fixed exceptions and 120 IRQs.

A Cortex-M23-based board uses vectored exceptions. This means each exception
...
The ARM MPS2+ AN519 Board has 4 CMSDK AHB GPIO controllers. Each providing 16
...
Mapping from the ARM MPS2+ AN519 Board pins to GPIO controllers:
...
For mode details refer to `MPS2+ AN519 Technical Reference Manual (TRM)`_.
...
MPS2+ AN519 has several clocks connected:
...
The MPS2+ AN519 has five UARTs. The Zephyr console output by default, uses
...
The MPS2+ AN519 implements a Serial Configuration Control (SCC) register.
...
MPS2+ AN519 supports the Armv8m Security Extension.
Applications built for the mps2_an519 board by default
boot in the Secure state.

.. note::

   Integration with TF-M is not currently supported in AN519.

.. note::

   Currently building non-secure applications in AN519 is not supported.


Building a Secure only application on MPS2+ AN519
=================================================

Build the Zephyr app in the usual way (see :ref:`build_an_application`
and :ref:`application_run`), using ``-DBOARD=mps2_an519`` for
the firmware running on the MPS2+ AN519.
...
.. code-block:: console
   ...
   Hello World! mps2_an519


Uploading an application to MPS2+ AN519
---------------------------------------
...
2. Open ``<MPS2 device name>/MB/HBI0263C/AN519/images.txt``.
3. Update the ``AN519/images.txt`` file as follows:
...
.. _MPS2+ AN519 Technical Reference Manual (TRM):
   https://developer.arm.com/documentation/dai0519/c/
