...
The Adafruit Metro ESP32-S3 is a dev board in an Arduino Uno R3 form factor based on the
ESP32-S3 WROOM N16R8 module. The board has a Lipoly charger circuit, an SD card slot,
a JTAG connector (in addition to the built-in USB-JTAG interface of the ESP module),
a STEMMA QT (I2C) connector, reset- and boot buttons, and a power switch.

.. figure:: img/adafruit_metro_esp32s3.webp
   :align: center
   :alt: Adafruit Metro ESP32-S3

   Adafruit Metro ESP32-S3 (Credit: Adafruit_)
...
**Board specific features:**
   ...
   - **Low Power friendly!** In deep sleep mode we can get down to  approximately 100uA of current draw from the Lipoly connection. Quiescent current is from the power regulator, ESP32-S3 chip, and Lipoly monitor. Turn off the NeoPixel and external I2C power for the lowest quiescent current draw.
...
ESP32-S3 allows 2 different applications to be executed in the ESP32-S3 SoC. Due to its dual-core
...
The current Zephyr implementation of the Adafruit Metro ESP32-S3 board supports the following features:
...
.. zephyr-app-commands::
   ...
   :board: adafruit_metro_esp32s3/esp32s3/procpu
   :goals: build
   :west-args: --sysbuild
   :compact:
...
The usual ``flash`` target will work with the ``adafruit_metro_esp32s3/esp32s3/procpu`` board
...
.. code-block:: console
   ...
   ***** Booting Zephyr OS vx.x.x-xxx-gxxxxxxxxxxxx *****
   Hello World! adafruit_metro_esp32s3<qualifiers>

Debugging
*********

ESP32-S3 support on OpenOCD is available upstream as of version 0.12.0.
Download and install OpenOCD from `OpenOCD`_, or add the location of the
OpenOCD included in the ESP-IDF to your PATH environment variable
(can be found at ``path/to/ESP-IDF/tools/openocd-esp32/<version>/openocd-esp32/bin``).
...
.. _Adafruit ESP32-S3 User Guide: https://learn.adafruit.com/adafruit-metro-esp32-s3
...
.. _Adafruit: https://cdn-learn.adafruit.com/guides/cropped_images/000/003/865/medium640/MS3F_top_angle.jpg
