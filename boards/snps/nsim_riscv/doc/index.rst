...
* ``nsim_riscv/rmx500`` - Synopsys RISC-V RMX500 core

.. _board_nsim_riscv_prop_files:

It is recommended to look at precise description of a particular board target in ``.props``
files in :zephyr_file:`boards/snps/nsim_riscv/support/` directory to understand
...
I.e. for the ``nsim_riscv/rmx500`` board we can check :zephyr_file:`boards/snps/nsim_riscv/nsim_riscv_rmx500.yaml`
...
.. code-block:: console
      ...
      *** Booting Zephyr OS build zephyr-v3.2.0-3948-gd351a024dc87 ***
      thread_a: Hello World from cpu 0 on nsim_riscv!
      thread_b: Hello World from cpu 0 on nsim_riscv!
      thread_a: Hello World from cpu 0 on nsim_riscv!
      thread_b: Hello World from cpu 0 on nsim_riscv!
      thread_a: Hello World from cpu 0 on nsim_riscv!
...
.. _board_nsim_riscv_verbose_build:
...
.. _board_nsim_riscv_debugging_gdb:
...
nSIM configuration is defined in :ref:`props and args files <board_nsim_riscv_prop_files>`.
...
The hardware-specific compiler options are set in corresponding SoC cmake file. For ``nsim_riscv`` board
it is :zephyr_file:`soc/snps/nsim_riscv/CMakeLists.txt`.
...
.. note::
   ...
   :ref:`running build in verbose mode <board_nsim_riscv_verbose_build>`.
