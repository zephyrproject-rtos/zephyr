...
First, ensure `Simplicity Commander`_ is properly installed on the host.

The SiWG917 contains a Network Co-Processor (NWP). Ensure the NWP is flashed
with the firmware associated with the current Zephyr version. The firmware is
provided with the blobs of hal_silabs:
...
Applications for the ``siwx917_rb4342a`` board can be built in the usual way.
