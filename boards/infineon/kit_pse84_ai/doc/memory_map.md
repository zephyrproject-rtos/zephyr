# PSE84 (KIT_PSE84_AI) Memory Map & Protection Domains in the TF-M case
...
Effectively, the default memory allocation for PSE84_AI boils down to
...
Therefore users can freely update the kit_pse84_ai_memory_map.dtsi and or the per core files (e.g. kit_pse84_ai_m33_ns.dtsi and kit_pse84_ai_m55.dtsi) and in some instance reflect the changes in the generated files located in hal/infineon/zephyr-ifx-cycfg/kit_pse84_ai .
Here's a case by case description:

### 5.1 m55_xip
1. Update kit_pse84_ai_memory_map.dtsi entry for m55_xip (if moving earlier, resizing m33_xip is also needed)
2. Update kit_pse84_ai_m55.dtsi slot0_partition address
3. Rebuild cm55 image AND cm33ns image (it needs to know the new start address for cm55)

### 5.2 m33_xip
1. Update kit_pse84_ai_memory_map.dtsi entry for m33_xip (if resizing to make it bigger, moving m55_xip might be needed)
2. Update kit_pse84_ai_m33_ns.dtsi slot0_partition address
3. Update cymem_cm33_0.h file in hal/infineon/zephyr-ifx-cycfg/kit_pse84_ai with the new offset/address/size of m33_nvm defines.
4. Rebuild cm33ns image

### 5.3 m33_data
1. Update kit_pse84_ai_memory_map.dtsi entry for m33_data (neighboring regions could be impacted)
3. Update cymem_cm33_0.h file in hal/infineon/zephyr-ifx-cycfg/kit_pse84_ai with the new offset/address/size of m33_data defines.
4. Rebuild cm33ns image

### 5.4 ITS / PS / NV_COUNTERS
1. Update kit_pse84_ai_memory_map.dtsi entry for RRAM reserved_security address of where security regions start (neighboring regions could be impacted)
2. Update cymem_cm33_S_0.h file in hal/infineon/zephyr-ifx-cycfg/kit_pse84_ai with the new offset/address/size of the ITS/PS/NV_COUNTERS regions
...
# PSE84 (KIT_PSE84_AI) Memory Map & Protection Domains in non TF-M case

The only source of truth for the non TF-M case, is kit_pse84_ai_memory_map.dtsi all details of the memory layout are already kept up to date in that file.
...
1. Update `kit_pse84_ai_memory_map.dtsi` with the new address/size of the CM55
   region(s), and `kit_pse84_ai_m55.dtsi` (`slot0_partition`) for the XIP code slot.
