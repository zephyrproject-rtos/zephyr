.. zephyr:board:: cy8cproto_040t

Overview
********

The `CY8CPROTO-040T`_ is a prototyping kit based on the PSOC™ 4000T
...
- `CY8CPROTO-040T Board Website`_
...
| Name    | GPIO Pin             |
+=========+======================+
| LED3    | P1.0 (active high)   |
+---------+----------------------+
| LED1    | P3.0 (active low)    |
+---------+----------------------+

Push Buttons
------------

The board does not define a button node by default: pin P3.0 is exposed as
``led1`` (see above) since it shares its routing with SW4 through header
**J6**. Samples that need SW4 as an input, such as
:zephyr:code-sample:`button`, provide a devicetree overlay that repurposes
this pin as a ``sw0`` button instead.

.. note::

   USER LED1 and USER button SW4 share the same routing through the
   selection header **J6**. Because they share the same selection path,
   only the function selected on J6 (LED1 or SW4) is usable at a time.
...
+-----------+-----------------+----------------------------+
| P1.0      | GPIO            | LED3 (subject to J6)       |
+-----------+-----------------+----------------------------+
| P3.0      | GPIO            | LED1 / Button SW4 (J6)     |
+-----------+-----------------+----------------------------+
...
.. note::
   ...
   Switch **SW2** must be set to the UART position to use the console over
   the KitProg3 USB-UART bridge. SW2 also selects the I2C pin routing when
   SCB0 is instead configured for I2C.
...
To use the UART console, set switch **SW2** to the UART position. Without
this, the KitProg3 USB-UART bridge is not connected to the SCB0 pins and
...
The `CY8CPROTO-040T`_ includes an onboard programmer/debugger (`KitProg3`_)
...
the `CY8CPROTO-040T`_. Use the serial terminal of your choice (minicom, PuTTY,
...
.. _CY8CPROTO-040T:
    https://www.infineon.com/cms/en/product/evaluation-boards/cy8cproto-040t/
